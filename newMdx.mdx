...
import migratePage1 from './images/user-migration-page-1.png';

This an image with using imported src:

<img src={migratePage1}/>

This an image with using relative path in src:

<img src={'./images/user-migration-page-1.png'}/>
...
* If you've already used the wizard to set up an admin on the new user model, have the admin sign in using their new user record to access the migration tool. The user migration wizard, when completed, destroys the old user record, but if you've started the user migration process without completing it, you may have users with access to both the original and new record, as shown below:
  <img src={loginMultipleAccountsView}/>
  {' '}
  <figcaption>If a user on the new model has been created and the migration process hasn't
  been completed, they may have access to both the original user record and
  the new user records.</figcaption>
