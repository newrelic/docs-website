...
<CollapserGroup>
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com) > APM > (select an app) > Transactions > Transaction trace > Trace details**: All asynchronous method calls linked with a token are indicated with an <Icon name="search"/>
    **Async** icon.

    All asynchronous method calls linked with a token are indicated with an <Icon name="search"/>
