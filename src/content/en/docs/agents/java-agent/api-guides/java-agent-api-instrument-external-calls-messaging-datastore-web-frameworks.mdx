---
title: >-
  Java agent API: Instrument external calls, messaging, datastore, web
  frameworks
contentType: page
template: basicDoc
topics:
...
[![](./images/CAT_diagram.png)](/sites/default/files/thumbnails/image/CAT_diagram.png)
...
When a traced method is reported as an external datastore call, the call is shown in the APM [Databases](https://docs.newrelic.com/docs/apm/applications-menu/monitoring/databases-slow-queries-page) page. Because datastores are external to the running application, the method is reported as datastore activity using the `reportAsExternal(ExternalParameters params)` method. The only difference is that a different builder, `DatastoreParameters`, is used to create the appropriate `ExternalParameters` object.
...
* [New Relic Java agent API Javadoc](http://newrelic.github.io/java-agent-api/javadoc/com/newrelic/api/agent/NewRelic.html) (detailed API documentation on GitHub)
* [Java agent API example](/docs/agents/java-agent/custom-instrumentation/java-agent-api-example-application-using-custom-instrumentation-cross-application-tracing-cat) (example application using custom instrumentation for cross application tracing and external datastore calls)
* [Java agent API guide](https://docs.newrelic.com/docs/agents/java-agent/custom-instrumentation/java-agent-api-guide) (goal-driven guide to using the Java agent API)
* [Java instrumentation by annotation](/docs/agents/java-agent/custom-instrumentation/java-instrumentation-annotation) (how to use annotations to instrument your application code)
