...
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-detect-analyze-errors
---
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/browser) > Browser > (select an app) > JS errors**.
...
<Table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Use the **Search** <Icon name="search"/>
...
**[one.newrelic.com](https://one.newrelic.com) > Browser > (select an app) > JS errors:** Use this page to group and filter errors, explore stack trace details, and examine trends. (If your UI page looks different, you may be using an older, [deprecated UI](#classic-ui).)
...
To notice or log your app's handled errors or other miscellaneous errors, use the Browser monitoring API's [`noticeError`](/docs/browser/new-relic-browser/browser-agent-spa-api/newrelicnoticeerror-browser-agent-api) call.
...
To view the **Event log** (if available): Go to **[one.newrelic.com](one.newrelic.com/) > Browser > (select an app) > JS errors > (select an error) > Error instances**. The event log will not be visible if there are [no events in the event log](#troubleshoot-event-log).
