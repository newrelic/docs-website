...
releaseDateTime: '2015-02-11 14:30'
releaseVersion: 2.44.0.36
...
* **Unified view for SQL database and NoSQL datastore products.**
  ...
  We also introduced a new [unified view](https://docs.newrelic.com/docs/apm/applications-menu/monitoring/database-slow-query-dashboard) within the APM UI for visualizing time spent in queries made against SQL databases and NoSQL datastore products.
...
* **Key transactions for other transactions.**
  ...
  In addition to being able to create key transactions for web transactions, it is now possible to create [key transactions](https://docs.newrelic.com/docs/apm/selected-transactions/key-transactions/key-transactions-tracking-important-transactions-or-events) for other transactions, such as background tasks executed by Celery.

  Key transactions enable the setting of a per transaction Apdex and alerts, as well as the ability to run [X-Ray sessions](https://docs.newrelic.com/docs/apm/selected-transactions/x-ray-sessions/x-ray-sessions-traces-thread-profiles-key-transactions), including transaction specific thread profiling.
