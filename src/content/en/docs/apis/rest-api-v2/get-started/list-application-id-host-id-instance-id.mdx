...
The instance ID meaning depends on the New Relic language agent being used. You can list this ID from the REST API. For Java, you can also [view the instance ID (JVM)](#UI) from New Relic APM's **Overview** page.
...
<CollapserGroup>
  ...
  >
    ...
    1. Go to **[one.newrelic.com](http://one.newrelic.com) > APM > Applications > (select an app) > JVMs**.
