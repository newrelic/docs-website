...
<CollapserGroup>
  ...
  >
    Use the `<var>` tag to highlight areas of a code block where a user is expected to supply their own value. For more context on when to use `<var>` tags, see [&lt;var> formatting guidelines](/docs/new-relic-only/basic-style-guide/writing-guidelines/code-formatting-guidelines).
