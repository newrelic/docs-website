...
<CollapserGroup>
  ...
  >
    ...
    1. Go to **[one.newrelic.com](https://one.newrelic.com) > Synthetics > (select a monitor) > Summary**.
    2. Above the selected monitor's **Availability** chart, select the ellipsis <Icon name="more-horizontal"/>
       icon.
  ...
  <Collapser
    id="enable-browser"
    title="Enable comparative charting from Browser UI"
  >
    To enable the comparison data feature from New Relic Browser monitoring:

    1. Go to **[one.newrelic.com](https://one.newrelic.com) > Browser > (select a browser app) > Page views**.
    2. Select a page monitored by synthetic monitoring.
    3. From the selected page's **Performance** tab, select the ellipsis <Icon name="more-horizontal"/>
       icon.
...
The comparative charting feature appears whenever synthetic monitoring identifies a URL match with browser monitoring and can compare it. You can compare browser and synthetic's performance either from the [Synthetics UI **Summary** page](/docs/synthetics/new-relic-synthetics/pages/synthetics-overview-page-view-monitors-performance) or from the [Browser UI **Page views** page](/docs/browser/new-relic-browser/additional-standard-features/page-views-understanding-your-sites-popularity) (for browser apps or single page apps) without needing to switch between New Relic capabilities.
...
1. From Synthetics UI: Go to **[one.newrelic.com](https://one.newrelic.com) > Synthetics > (select a monitor) > Summary**.

   OR

   From Browser UI: Go to **[one.newrelic.com](https://one.newrelic.com) > Browser > (select a browser app) > Page views**, then select a page monitored by synthetic monitoring.
2. Review the comparative charting feature's summary of overall speed percentage and number of matching URLs.
3. To view additional details, select the summary's right arrow <i className="fa fa-angle-right"><span>[right arrow icon]</span></i>
   icon.
...
To hide the comparative charting feature, select the ellipsis <Icon name="more-horizontal"/>
icon.
