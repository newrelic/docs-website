  ...
  - **Unified Helm chart:** Windows and Linux nodes are now monitored using the same `newrelic-infrastructure` chart — no separate installation needed. A single `enableWindows: true` flag in your
   `values.yaml` is all it takes.                                                                                                                                                                  
  - **Per-version DaemonSets:** The chart automatically creates one DaemonSet per supported Windows version (Windows Server 2019, Windows Server 2022), scheduling pods only to nodes with a
  matching OS build — eliminating empty DaemonSets in your cluster.                                                                                                                                
  ...
  `SystemSample`, `StorageSample`, and `NetworkSample` data.                                                                                                                          
