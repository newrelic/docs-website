...
We have [querying limits](/docs/query-your-data/nrql-new-relic-query-language/get-started/rate-limits-nrql-queries/). You likely won't encounter these limits, especially if you follow these general guidelines:
...
If New Relic applies [rate limits on your account](/docs/insights/use-insights-ui/manage-account-data/rate-limits-insights), the query API returns a 503 error. Charts may display timeout error messages.
