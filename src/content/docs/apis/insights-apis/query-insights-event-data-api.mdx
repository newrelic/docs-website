...
If New Relic applies rate limits on your account for number of queries per minute, the query API returns a 429 error.  If New Relic applies rate limits on your account for records inspected, the query API returns a 503 error and charts may display timeout error messages.
