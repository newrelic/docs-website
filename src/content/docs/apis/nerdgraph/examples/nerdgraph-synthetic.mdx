...
While NerdGraph defines attributes in its UI, the `LOCATION`, `PERIOD`, and `TAGS` attributes deserve some explanation. 
...
<CollapserGroup>
  ...
  >
To identify a private location in your scripts, use the private locatoin's entity GUID. To verify that a script executes successfully, also include the vsePassword attribute. For example: 
...
The scripts below automate the creation of your synthetic monitors, including broken links, step, and certificate check monitors. To learn more about monitor use cases, read [Types of synthetic monitors](https://docs.newrelic.com/docs/synthetics/synthetic-monitoring/getting-started/types-synthetic-monitors). 
...
<Collapser
  ...
  >
Simple browser monitors use an instance of Google Chrome to mimic a customer visit.
  ...
  <thead>
    ...
    <tr>
      ...
      <td>
        The browser to use. “CHROME” is the only accepted value.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        The version of the browser to use. 97 is the only accepted value.
      </td>
    </tr>
        <tr>
      <td>
...
<Collapser
    className="freq-link"
    id="create-API-monitor"
    title="Create an API test monitor"
  >
