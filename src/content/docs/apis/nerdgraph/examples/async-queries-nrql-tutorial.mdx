...
You can use our NerdGraph API to run an asynchronous NRQL query. These queries run on the New Relic side and therefore won't be interrupted by browser or HTTP connection timeout limits, or other issues. It's also especially useful for running queries that may take a long time to complete.
...
When you run the previous query, it will return a query ID, which you can use to get either query results, if it's finished, or else progress status for that query. 
