...
* **`badEvents.from`**, **`badEvents.where`** 
  * The NRQL query that defines bad events, `SELECT (*) FROM badEvents.from WHERE badEvents.where`, requires these `FROM` and `WHERE` clauses.  
  * If you defined an SLI from valid and bad events, leave the `goodEvents` object empty. 
* **`goodEvents.from`**, **`goodEvents.where`** 
  * The NRQL query that defines good events, `SELECT (*) FROM goodEvents.from WHERE goodEvents.where`, requires these `FROM` and `WHERE` clauses.  
