...
<Callout variant="important">
  In order to create Service Levels, the API key needs [a role with permissions](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model) to modify and delete [metric rules](/docs/data-apis/convert-to-metrics/analyze-monitor-data-trends-metrics/). 
</Callout>

## Create an SLI with an SLO [#create-sli-slo]

Please refer to [Create and edit SLIs and SLOs](https://docs-preview.newrelic.com/docs/service-level-management/create-slm#sli-entity) to learn the basic concepts in the SLI and SLO configuration, such as the entity that an SLI is associated with. Also, you can refer to that documentation to find examples of the most common indicators for APM services and browser applications.  
...
* **`badEvents.from`**, **`badEvents.where`** 
  * These are the `FROM` and `WHERE` clauses for the NRQL query that defines bad events, which will result in `SELECT (*) FROM badEvents.from WHERE badEvents.where`. 
  * If you defined an SLI from valid and bad events, leave the `goodEvents` object empty. 
* **`goodEvents.from`**, **`goodEvents.where`** 
  * These are the `FROM` and `WHERE` clauses for the NRQL query that defines good events, which will result in `SELECT (*) FROM goodEvents.from WHERE goodEvents.where`. 
  * If you defined an SLI from valid and good events, leave the `badEvents` object empty. 
* **`validEvents.from`**, **`validEvents.where`**
  * These are the `FROM` and `WHERE` clauses for the NRQL query that defines valid events, which will result in `SELECT (*) FROM validEvents.from WHERE validEvents.where`. 
...
One important field in the response is the SLI's `id`. You will need it on the `serviceLevelUpdate` mutation to [define one or more SLOs](#update-slo) for each one of the SLIs.
