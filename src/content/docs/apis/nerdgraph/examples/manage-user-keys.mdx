---
title: Admin setup for user key creation and access
tags:
...
translate:
  - jp
metaDescription: 'Role configuration for admins to create and access user API keys using NerdGraph.'
redirects:
freshnessValidatedDate: never
---
Admin users create API keys for others using NerdGraph to support automation, ensure governance compliance, and facilitate provisioning processes. To view the full API key upon creation, admin users must be enrolled in a special role that permits reading the key only at creation time.
  ...
  <TabsPages>
    <TabsPageItem id="using-ui">
      Configure the role for admin users from the <DNT>**Access Management**</DNT> page as follows:
        ...
        3. To grant permission for users in the group to read other users' API keys on creation, under <DNT>**Administrative settings**</DNT>, select <DNT>**organization.read.other_users_api_keys**</DNT>.
    </TabsPageItem>
    ...
    2. To access <DNT>**Query Builder**</DNT>, submit your [API user key](/docs/apis/intro-apis/new-relic-api-keys/#user-key). You can create a new key from the [API Key UI](https://one.newrelic.com/api-keys).
    3. Query for an existing [user group](/docs/apis/nerdgraph/examples/nerdgraph-manage-groups/#query-groups) where admin users are already members, or [create a new group](/docs/apis/nerdgraph/examples/nerdgraph-manage-groups/#create-group). Save the group ID for later use.
    ...
    6. To create a new custom role that enables admins to read other users' API keys, run the following mutation with the fetched organization ID, permission ID: `[12822]`, and a role name:
        ```graphql
          mutation {
            customRoleCreate(
...
After configuring the group, the admin users within the group can create and read API keys for other users using NerdGraph. As an admin with enabled role, run the following mutation with an account ID, name for the key, and user ID of a user to create the key.
```graphql
mutation {
  apiAccessCreateKeys(
    keys: {user: {
