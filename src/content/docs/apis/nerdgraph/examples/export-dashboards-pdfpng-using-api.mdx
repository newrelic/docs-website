...
If you experience any of the following errors while trying to export a dashboard: 
...
These can be caused due the generation of the PDF exceeds the API max response time. If you are encountering these errors, check if any of the widgetson your dashboard has a large time window, e.g. comparing data from an entire month vs previous months, if it's the case try to decrease the window. Another possible reason could be that your dashboard has too many widgets, reducing the number of widgets can sort out this problem. 
