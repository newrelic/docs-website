...
With New Relic’s [Applied Intelligence](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/introduction-applied-intelligence/), you can create custom [decisions](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/change-applied-intelligence-correlation-logic-decisions) that govern how your incidents are correlated. One kind of custom decision logic uses the concept of “topology,” which in this context is a representation of your service map. 
This doc will explain how to use our [NerdGraph API](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/) to configure your topology mapping. 

This tutorial will give example NerdGraph calls that: 
...
In NerdGraph, mutations are requests that perform an action ([learn more about NerdGraph terminology](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/#terminology)). 

In this section, we’ll show you how to use the `aiTopologyCollector` query to create, edit, or delete your topology. In this example, we’ll set up the following service map:

![Topology example](./images/topology-example.png "topology example")

### Create vertices [#create-vertices]

The following mutation creates one or more vertices that represent the entity and the source from which your incident comes from. 
...
* `vertexClass`: The classification of this vertex. The class can be `application`, `host`, `cloud service`, `cluster`, or `datastore`. This classification allows your decision logic to restrict your topology-based correlation to restrict vertices matching these classifications.
* `definingAttributes`: A unique set of attributes (key/value pairs) that match those from incident events. If an incident contains any of the key/value pairs of a vertex’s `definingAttributes`, it’s matched to that vertex. To learn more about attributes and how they’re added, see [Add attributes](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/change-applied-intelligence-correlation-logic-decisions/#add-attributes).
...
This mutation is used to create one or more edges that represent relationships between one vertex to another vertex.

The NerdGraph call below makes use of these fields: 

* `accountId`: Your New Relic account ID.
* `fromVertexName`:  The name of the vertex you’re defining the edge for.
* `toVertexName`: The name of the vertex at which you’re ending the edge.
* `directed`: Optional. A boolean flag that describes how you want to connect one vertex to another vertex. True means the edge can traverse only one direction, indicating a one-way relationship. False means the edge can traverse in both directions, indicating a two-way relationship. By default, directed = `true`. 
