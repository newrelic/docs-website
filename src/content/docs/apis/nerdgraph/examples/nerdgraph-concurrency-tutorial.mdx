...
New Relic's NerdGraph API enforces a limit of **25 concurrent requests per user**. The following code examples will illustrate how to avoid hitting these limits or receiving HTTP `429` status codes.

Both examples below follow the same use case, where a query is to be ran against many New Relic accounts in parallel, but respecting the NerdGraph concurrency limit.


## Javascript

One way to easily handle concurrency is to use a queuing mechanism. Below illustrates using the [async module](https://caolan.github.io/async/v3/docs.html#queue) to do so. A queue is created that has a set concurrency execution limit, requests are pushed as tasks onto that queue, and the queue is drained once all tasks are completed.
...
Similarly using Python, the following packages can be leveraged to handle asynchronous requests and concurrency:
...
Below sets a [Semaphore](https://docs.python.org/3/library/asyncio-sync.html?highlight=semaphore#asyncio.Semaphore) that manages the amount of simultaneous requests being made at one time. `asyncio.gather()` is then used to execute multiple coroutines concurrently, and wait for all to complete.
...
For more detail on NerdGraph limits, see [NerdGraph usage limits](https://docs.newrelic.com/docs/apis/nerdgraph/nerdgraph-usage-limits/)
