...
New Relic's NerdGraph API enforces a limit of **25 concurrent requests per user**. The following code examples can be used as starting points for avoiding these limits or receiving HTTP `429` status codes.
...
One way to easily handle concurrency is to use a worker pool. Below illustrates using the [async module](https://caolan.github.io/async/v3/docs.html#queue) to do so. A queue is created that has a set concurrency execution limit, requests are pushed as tasks onto that queue, and the queue is drained once all tasks are completed.
