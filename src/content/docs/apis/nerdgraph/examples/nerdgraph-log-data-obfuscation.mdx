...
tags:
  ...
  - Examples
  - Logs
metaDescription: "Create, query, and manage obfuscation rules and expressions for your log data with NerdGraph, the New Relic GraphQL explorer."
---
...
We also provide options for you to obfuscate parts of your customer data when it arrives at New Relic. This makes it easier and safer for you, rather than needing to track down sensitive data in all of your applications and try to fix them at the source.

You can use NerdGraph at [api.newrelic.com/graphiql](https://api.newrelic.com/graphiql) to create, query, and manage your custom data obfuscation rules and expressions for logs. NerdGraph is our GraphQL-format API explorer.

You can also manage your obfuscation expressions and rules in the New Relic One UI. For more information, see our [UI documentation about obfuscation](/docs/logs/ui-data/obfuscation-ui).

## Obfuscation: masking and hashing [#definitions]

**Masking** is when we entirely replace sensitive data by `X` characters, with no means for retrieval later. For example, if a log line `Sent 123-45-6789` becomes `Sent XXX-XX-XXXX`, that's masking.

**Hashing** is when a sensitive value is replaced with a one-way hashing algorithm. This allows lookup of related records if you have the original value and are able to hash it. (Our UI provides a simple tool to do this, for your convenience.) 

Example: A log line `Sent 123-45-6789` is hashed to this:

```
Sent 01a54629efb952287e554eb23ef69c52097a75aecc0e3a93ca0855ab6d7a31a0
```

The string following `Sent` is the calculated result of hashing `123-45-6789`.
...
Here are NerdGraph API examples to create, query, update, and delete an obfuscation expression.
...
<CollapserGroup>
  ...
  This example creates a new obfuscation expression. Required [fields](#obfuscation-fields) include:
  ...
  * `regex`
  ...
  >
  This example retrieves all the obfuscation expressions for a given account.
  ...
  * `updatedBy`
  ...
  >
  This example updates an obfuscation expression for your log data. Include the following [fields](#obfuscation-fields) as needed in your update. The `id` is required. The others are optional, but at least one must be present:
...
Here are NerdGraph API examples to create, query, update, and delete an obfuscation rule.
...
<CollapserGroup>
  ...
  This example creates a new obfuscation rule. Required [fields](#obfuscation-fields) include:
  ...
  * `updatedBy`
  ...
  This example updates an obfuscation rule for your log data. Include the following [fields](#obfuscation-fields) as needed in your update. The `id` is required. The others are optional, but at least one must be present:
