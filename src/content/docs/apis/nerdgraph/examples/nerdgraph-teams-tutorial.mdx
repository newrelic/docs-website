...
### By name [#query-by-name]

If don't have the team id, you can also use the name to find a team `id` (`guid`field):

```graphql
{
  actor {
    entitySearch(query: "type = 'TEAM' and name = 'INSERT_YOUR_NAME'") {
      results {
        entities {
          name
          guid
