...
* `parentId` refers to the parent team id in the hierarchy tree.
...
* `hierarchyLevelId`: The ID of the `TeamsHierarchyLevelEntity` that represents this team's position in the hierarchy, which is configured via organization settings (see [org settings](#update-team-settings)).
* `managers`: A list of user entity GUIDs who are managers or the main point of contact of this team. Managers must be team mebers. To find user GUIDs, see [Retrieve the user Id](#user-id).
* `onCallMembers`: A list of OnCallMember entities representing contacts outside regular working hours. Each entity requires an ID field containing the user's entity GUID. On-call members must be team mebers. To find user GUIDs, see [Retrieve the user Id](#user-id).
* `parentId`: The ID of the parent team in the hierarchy.
...
Keep the following in mind when using this query: 
* `managers` accepts a list of user entity GUIDs. To find user GUIDs, see [Retrieve the user Id](#user-id).
...
## Update a hierarchy level [#update-hierarchy-level]

Use the `entityManagementUpdateTeamsHierarchyLevel` mutation to update the name of a hierarchy level. You can find hierarchy level IDs in the `hierarchyLevelOrder` field from the [organization settings query](#update-team-settings).
...
* `name` is the only updatable attribute for a hierarchy level.
* The `id` is the ID of the `TeamsHierarchyLevelEntity` to update. You can find these IDs in the `hierarchyLevelOrder` array returned by the [organization settings query](#update-team-settings).
...
* `hierarchyLevelOrder`: The array that defines the order of hierarchy levels in descendant order, with the first element being the highest level, and the last the lowest. Each element is an `id` of `TeamsHierarchyLevelEntity`.
