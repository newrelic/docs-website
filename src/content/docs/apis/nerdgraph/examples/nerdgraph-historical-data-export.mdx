...
<Callout variant="important">
  All instances of single quotes (`'`) in your export NRQL query must be escaped with a backslash (i.e. `\'`). The `curl` commands below rely on a leading `$` in the `--data-raw` argument, the functionality of which is provided by ANSI-C quoting. More information is available through the [official GNU manual](https://www.gnu.org/software/bash/manual/html_node/ANSI_002dC-Quoting.html).
</Callout>
