...
In the NerdGraph schema, the `historicalDataExportCancelExport` endpoint can be found under `mutation > historicalDataExport`. Use a query like the one below to cancel an export. 

It's recommended to select status for the response body, as this will allow you to see whether the cancellation was successful.

```
mutation {
  historicalDataExportCancelExport(accountId: <var>YOUR_ACCOUNT_ID</var>, id: "6e6bfefa-db1c-44a3-b603-d38f32bcf3b8") {
    id
    internalStatus
    percentComplete
  }
...
Here's an example response for cancelling an export: 
{
  "data": {
    "historicalDataExportCancelExport": {
      "id": "6e6bfefa-db1c-44a3-b603-d38f32bcf3b8",
