...
4. Execute a mutation using the `id`, one or more entity guids, and whether you would like to ADD, REPLACE, or REMOVE them. For example, to add a new Browser and Mobile app to the `Production services` trace observer with id `259` that we found in the previous step, run the following:
