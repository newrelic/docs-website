...
* <DoNotTranslate>**ADD:**</DoNotTranslate> If you'd like to add new data sources to the existing ones on a trace observer, use the `ADD` `DataSourceGroupUpdateType`.  No data sources will be removed with this mutation.
* <DoNotTranslate>**REPLACE:**</DoNotTranslate> If you'd like to replace your existing data sources on a trace observer with a new set of values, use the `REPLACE` `DataSourceGroupUpdateType`. Any existing values not in the new set will have their status set to `INACTIVE`.
* <DoNotTranslate>**REMOVE:**</DoNotTranslate> If you'd like to remove one or more of your existing data sources on a trace observer, use the `REMOVE` `DataSourceGroupUpdateType`. Any values passed will have their status set to `INACTIVE`.
