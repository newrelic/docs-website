...
4. Execute a mutation using the `id`, one or more entity guids, and whether you would like to ADD, REPLACE, OR DELETE them. For example, to add a new Browser and Mobile app to the `Production services` trace observer with id `259` that we found in the previous step, run the following:
   ...
   ```
  mutation {
    edgeUpdateTraceObservers(traceObserverConfigs: {id: 4, dataSourceGroupConfig: {updateType: ADD, guids: ["MXxCUk9XU0VSfEFQUExJQ0FUSU9OfDQ5", "MXxNT0JJTEV8QVBQTElDQVRJT058MTY"]}}, accountId: 1) {
      responses {
        errors {
          message
          type
