...
New Relic's GraphQL API provides a method to "pin" a specific version of the New Relic Browser Agent, ensuring it remains consistent within a customer's platform. By pinning a version, you can prevent automatic updates that might introduce unexpected changes or behaviors. Key benefits of using this feature include:
...
1. Check agent type and which version is currently pinned, if any
1. Reference the available versions of the agent
1. Select and pin the desired version
1. Redeploy or restart your service to use the pinned version of the agent

### Step 1. Retrieve the Current Pinned Version
...
### Step 2. Retrieve the Available Versions

Find the desired agent version from the [New Relic browser agent releases page.](https://docs.newrelic.com/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/)
...
### Step 3. Pin the Desired Version

Once you've identified the version you want to pin, use the following GraphQL mutation to set it.

*Hint:* Specify the semantic version of the browser agent that you would like your app to use. Leave this blank to use the most recent version. Use `'x'` in place of a numeric digit to represent the latest release within the version range. For example, `'1.x.x'`
...
### Step 4. Decode the JavaScript Snippet (if using Copy and Paste)

The `jsLoaderScript` value is a JSON encoded string. The string **MUST** be decoded into a valid HTML `<script>` element before it can be used in your application. 

### Last step. Re-deploy your application or restart your agent
...
In either case, the next time your application serves a browser page, it should be using the pinned agent version to report data to your New Relic One account. 

Remember that you can check the pinned version and loader type by using the query in Step 1.
