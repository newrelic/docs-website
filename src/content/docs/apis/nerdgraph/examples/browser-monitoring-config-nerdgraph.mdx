...
### Retrieve the current payload capture setting [#retrieve-payload-capture]

Use this query to check the current `capturePayloads` value for your browser application.

Query:

```graphql
query FetchBrowserAjaxPayloadCapture($guid: EntityGuid!) {
  actor {
    entity(guid: $guid) {
      ... on BrowserApplicationEntity {
        browserSettings {
...
Use this mutation to set the capture mode for your browser application. Accepted values are `none` (default), `failures`, or `all`.
