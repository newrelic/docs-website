...
## Create obfuscation rules [#create-obfuscation-rules]

You can mask sensitive data before it's harvested by creating ingest obfuscation rules directly from the UI or NerdGraph. These rules apply to all browser events. Rules configured via the UI or NerdGraph apply to APM-injected applications, copy/paste installations, and NPM installations. However, copy/paste and NPM installationscan manually override UI-configured rules by adding the `obfuscate` property to their `init` configuration. To create rules via UI or JavaScript, see [Obfuscate sensitive data in the browser agent](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/).

To create obfuscation rules via NerdGraph:

1. Retrieve the current obfuscation rules for your browser application using the following query:
    ...
    <Callout variant="important">
      ...
      You must retrieve the current rules before updating them, as the `agentApplicationSettingsUpdate` mutation will overwrite any existing rules with the new set of rules you provide. If you want to keep any existing rules, be sure to include them in the new set of rules you send in the mutation.

    </Callout>

2. Update the obfuscation rules using `agentApplicationSettingsUpdate` mutation. You can add new rules or modify existing ones by defining the following properties:

    * `regex`: The regular expression pattern to match the data you want to obfuscate. For example, you can use `\b\d{3}-?\d{2}-?\d{4}\b` to match Social Security numbers.
    * `replacement`: The string that will replace the matched data. For example, you can use `XXX-XX-XXXX` to mask Social Security numbers.
    * `eventFilter`: An array of browser event types to which the rule should be applied. For example, you can specify `["AjaxRequest"]` to apply the rule only to AJAX request events.
...
## Configure AJAX payload capture [#configure-payload-capture]

You can retrieve and update the AJAX payload capture setting for your browser application using NerdGraph. This setting controls whether request and response payloads are captured for AJAX calls and corresponds to the [payload capture option in your application settings](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/#enable-payload-capture).


You can enable payload capture settings in two steps:

1. [Retrieve the current payload capture](#query-payload-capture) settings.
2. Set the [`capturePayloads` mode](#update-payload-capture) to one of the following values:
    * `none`: No payload data will be captured.
    * `failures`: Captures payloads for failed requests.
    * `all`: Captures payloads for all requests.

### Query the payload capture setting [#query-payload-capture]

Retrieve the current payload capture setting for your browser application using the following query:
...
### Update the payload capture setting [#update-payload-capture]

Update the payload capture setting using `agentApplicationSettingsUpdate` mutation:
...
Set the `guid` and `capturePayloads` options in the Variables:
