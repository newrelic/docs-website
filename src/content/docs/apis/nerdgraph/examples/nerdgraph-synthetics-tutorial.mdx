...
You can view field definitions in [NerdGraph explorer](https://api.newrelic.com/graphiql), but it may help to better understand the `LOCATION`, `PERIOD`, and `TAGS`. 
...
* <var>RUNTIME_TYPE_VERSION</var>: The runtime type version used by your monitor. "16.10.0" is the only accepted value. 
* <var>SCRIPT_LANGUAGE</var>: The language used in your monitor. "JAVASCRIPT" is the only accepted value. 
...
<Callout variant="info">
  Include the runtime object that includes runtimeType, runtimeTypeVersion, and scriptLanguage to to use a newer runtime. Do not include the runtime object and these attributes to continue using a legacy runtime.
</Callout>
...
```
  ...
  <Collapser
    ...
    >
  Use this to upgrade a simple browser, scripted API, or scripted browser monitor from using a legacy runtime to the new Chrome 100 or Node 16.10 runtime. See [optional-fields](#optional-fields) for additional details on runtime settings. This example will upgrade the runtime for a scripted browser monitor. Use the `syntheticsUpdateSimpleBrowserMonitor` mutation for simple browser monitors and `syntheticsUpdateScriptApiMonitor` for scripted API monitors.
  ```
  mutation {
    syntheticsUpdateScriptBrowserMonitor ( 
      guid: "<ENTITY_GUID>", 
