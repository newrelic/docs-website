...
* **Access control**: Admins can define who can access the secrets by setting custom roles and permissions, and specifying the scope of permissions. The scope can range from basic <dnt>**View**</dnt> to <dnt>**Purge**</dnt> permissions.
...
Use the `{customerAdministration {secretVersions}}` query to retrieve up to 10 recent versions of a secret with its details. You can retrieve secrets that are scoped only to your current account or organization.
