...
* **Access control**: Admins can define who can access the secrets by setting custom roles and permissions, and specifying the scope of permissions. The scope can range from <dnt>**View**</dnt> to <dnt>**Purge**</dnt> permissions.
...
<Callout variant="tip">
Each update operation creates a new version of the secret. The highest version is always the latest version of the secret. To [retrieve older versions](#retrieve-secret) of the secret, you can use the `{customerAdministration {secretVersions}}` query. For more details, refer to the [Retrieve all version information of a secret](/#recover-secrets-versions) section.
</Callout>
