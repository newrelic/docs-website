    ...
    <tbody>
        ...
        <tr>
            ...
            * `id`: string
            ...
            the level at which your secret is stored and accessed with the entity ID from which you can retrieve it. The scope is defined by two attributes:
       ...
       <tr>
            ...
            <td>
            Deletes all versions of the secret. If set to `true`, it deletes the secret permanently; if set to `false`, you can recover the secret with all its versions. You need necessary permissions to have this option. For more details, refer to the [Access control](#security-and-access-control) section.
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td>
             When you retrieve a list of secrets, use the cursor to navigate through the results. Without a `cursor` value, you receive the first page, which can contain up to 50 secrets. If more secrets exist, the response includes a `nextCursor` attribute with a value. To access the subsequent page, use that value in your next query for the `cursor` attribute.
            </td>
        </tr>                        
    </tbody>
</table>
...
Use `secretsManagementRecoverSecret` mutation to recover secrets that you have temporarily deleted using the `secretsManagementDeleteSecret` mutation before. If you have permanently deleted a secret, you cannot recover it.
...
Use `secretsManagementRecoverSecretVersion` mutation to recover specific versions of the secret that you have temporarily deleted using the `secretsManagementDeleteSecretVersion` mutation before. If you have permanently deleted a version of a secret, you cannot recover it.
