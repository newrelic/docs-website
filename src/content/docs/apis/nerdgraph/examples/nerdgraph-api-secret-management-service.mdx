...
* For EU customers, New Relic stores secrets in a US-based global database, which isn't GDPR compliant. You must provide your consent before using the service.
</Callout>
...
Use `secretsManagementCreateSecret` mutation to store your secrets in the secrets management service. You can generate unique keys for your secrets (API keys, passwords, certificates, or tokens) and use them to retrieve the secret value when needed.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
            (Optional) An alias is a way to assign a different name to a field or value in your Nerdgraph query. This can be useful for readability, avoiding naming conflicts, or when you want to query the same field many times with different attributes.
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td>
            (Optional) The namespace is a logical grouping for tags. It helps to organize and categorize tags, making it easier to manage and avoid potential conflicts, especially when dealing with many tags across various entities. Namespaces can be particularly useful in environments where many teams or projects are using New Relic, ensuring that tags are consistently applied and understood.
            </td>
        </tr>
        <tr>
            <td>
...
Use the `secretsManagementDeleteSecret` mutation for a soft delete operation. It doesn't delete the secret permanently from the secrets management service. You can restore the secret by creating a new version of the secret with the same key.
...
Use the `secretsManagementPurgeSecret` mutation for a hard delete operation. It deletes the secret key permanently from the secrets management service and you can't restore.
