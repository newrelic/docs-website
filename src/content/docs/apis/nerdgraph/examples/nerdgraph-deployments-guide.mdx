...
* Change Tracking Deployment Events are anchor points within the greater context of your application telemetry, logs, errors, incidents, and other health and performance indicators. Once you start recording deployments, you will see them drawn on charts in APM, Mobile, Browser, Dashboards, and elsewhere. You can also access them via the Deployments option on an Entity’s sidebar. This unlocks Change Tracking impact analysis interfaces. See [Change Tracking experience](/docs/change-tracking/change-tracking-introduction).
