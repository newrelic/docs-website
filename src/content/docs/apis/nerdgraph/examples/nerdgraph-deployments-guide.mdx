---
title: "NerdGraph tutorial: Record Deployment Markers to Enable Change Tracking Capabilities"
tags:
    - NerdGraph
    - Deployments
    - Change Tracking
metaDescription: Use NerdGraph to record deployment events enriched with contextual data like type, subtype, deep links, commit SHA and more.
---

Our [NerdGraph API](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/) allows you to record deployment markers that will enable New Relic Change Tracking features that help you understand how changes affect the performance and quality of your system. 
...
## Deployment Event Basics [#deployment-event-basics]
* Change Tracking Deployment Events are anchor points within the greater context of your application telemetry, logs, errors, incidents, and other health and performance indicators. Once you start recording deployments, you will see them drawn on charts in APM, Mobile, Browser, Dashboards, and elsewhere. You can also access them via the Deployments option on an Entity’s sidebar. This unlocks Change Tracking impact analysis interfaces. Read more about the Change Tracking experience in our [user guide](/LINK).
* Every deployment event recorded by New Relic is stored in NRDB and will be queryable via NRQL and NerdGraph. Example queries and information on where to look for deployments data are included in this guide.
* Every deployment event must be directly associated with one New Relic Entity, and for now this relationship must be established when the deployment event is created.
* Multiple deployment events can be related to one another using the `GROUPID` attribute (described in detail below). This may be useful in scenarios where a single “release” or action deploys updates to multiple parts of a system represented by New Relic Entities.
* Deployments can also be indirectly related to one another via Entity Relationships. Deployment events on different Entities can be automatically visualized and seen together on charts and in lists across New Relic if their Entities are related. For more on Entity Relationships, see this [NerdGraph tutorial](/docs/apis/nerdgraph/examples/nerdgraph-relationships-api-tutorial/). 

## Understand fields in NerdGraph explorer [#fields]

You can find field definitions in [NerdGraph GraphiQL explorer](docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/#explorer), but it may help to better understand the `deploymentId`, `COMMIT`, `VERSION`, `GROUPID`, and `DEEPLINK` and other Change Tracking fields that will help you get the most out of recorded changes and New Relic’s impact analysis features.
...
* <var>RUNTIME_TYPE</var>: The runtime type used by your monitor. "CHROME_BROWSER" is the only accepted value.
* <var>RUNTIME_TYPE_VERSION</var>: The runtime type version used by your monitor. "100" is the only accepted value. 
...
```
  ...
  <Collapser
    ...
    >
You can use the below example to update a simple browser, scripted API, or scripted browser monitor to the new Chrome 100 or Node 16.10 runtimes. The following example upgrades a scripted browser monitor to the new runtime from the legacy runtime. 
