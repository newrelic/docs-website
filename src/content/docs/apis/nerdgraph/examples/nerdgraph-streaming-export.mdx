...
tags:
  - APIs
  - NerdGraph
metaDescription: "With the New Relic streaming export feature, you can send your data as it's ingested to New Relic to an AWS Kinesis Firehose, Azure Event Hub or GCP Pub/Sub."
redirects:
freshnessValidatedDate: never
---

With the streaming export feature available through [Data Plus](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-plus), you can send your data to AWS Kinesis Firehose, Azure Event Hub, or GCP Pub/Sub. This guide explains how to create and update streaming rules using [NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) and view the existing rules. You can use the [NerdGraph explorer](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) to make these calls.

## What is streaming export? [#definition]

As data is ingested, our streaming export feature allows you to send the data directly to AWS Kinesis Firehose, Azure Event Hub, or GCP Pub/Sub. You can create custom rules using [NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) to specify which data should be exported. Additionally, you have the option to compress the data before exporting with our [Export Compression](#compression) feature.
...
To set up streaming data export to AWS, you must first set up an Amazon Kinesis Firehose. We'll walk you through that procedure in the following steps.
...
<Collapser 
    ...
    This API allows you to create a streaming export rule to send data to an AWS Kinesis Firehose.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>String</td>
          <td>Yes</td>
          <td>A NRQL to select the telemetry data to export.</td>
        </tr>
        <tr>
          ...
          <td>[payloadCompression](#compression) accepts two values:
            * `DISABLED`: Bydefault, Payloads will not be compressed before being exported.
...
```
    ...
    **Points to note**:
    ...
    You can check if the rule is updated by calling [streamingRule](view-stream-rule-details) to retrieve the rule. During the period that the rule is under updating, the rule is locked, and no other mutation action can act on the rule. If you are trying to perform another mutation action on the same rule, you will get a message saying, “The export rule is currently being updated by another request, please wait and try again later.” A user can update a rule of any status except a deleted rule.
...
To set up streaming data export to Azure, you must first set up an Event Hub. We'll walk you through that procedure in the following steps:
...
2. Click <DNT>**+Event Hub**</DNT> to create an Event Hub.
...
```
  ...
  <Collapser 
    ...
    Refer [here](#enable-stream) to enable an existing streaming rule, facilitating the continuous export of data for real-time processing and integration with other services.
  ...
  <Collapser 
    ...
    Refer [here](#disable-stream) to disable an existing streaming rule, stopping the continuous export of data.
  ...
  <Collapser 
    ...
    Refer [here](#delete-stream) to delete a streaming rule and stop all data export related to that rule.
...
To set up streaming data export to GCP, you must first set up a Pub/Sub. We'll walk you through that procedure in the following steps:

<Steps>
  <Step>
### Create a Pub/Sub topic [#create-pubsub-topic]

1. Navigate to the Pub/Sub page within your GCP Console.
2. Click **Create a topic** to start the process.
3. Enter a topic ID and click **Create**.
  </Step>
  <Step>
### Set up permissions on Pub/Sub [#setup-permissions-pubsub]

1. On the right column of the created topic, click **More actions**. A list of actions appears.
2. Select **View permissions**. A permission tap appears on the right.
...
4. Under **Assign a role** section, search for Pub/Sub Publisher and click **Save**.
  </Step>
</Steps>
...
You can choose to compress data before exporting it, although this feature is off by default. Compressing can help prevent exceeding your data limit and lower outbound data costs.
...
<CollapserGroup>
  ...
  >
    ...
    2. Click <DNT>**Deploy**</DNT> at the bottom of the page.
