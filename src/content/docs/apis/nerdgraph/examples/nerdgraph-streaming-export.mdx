...
* `payloadCompression`: Whether to compress payloads before sending them to the Kinesis stream. If specified, must be either `DISABLED` or `GZIP`. For more details, see [Export Compression](#compression).
...
Optionally, we now allow you to enable compression on payloads before they are sent out. This can allow you to save money on egress COGS and potentially avoid hitting your ingested data limit.

You can enable compression by setting the `payloadCompression` field under `awsParameters` to `GZIP`. Currently, GZIP is the only compression format available, though we may choose to make more formats available in the future. Note that this field is optional, and if not specified, compression will be disabled by default.

Export compression is currently only allowed on AWS export rules, though Azure is in the works!
...
<CollapserGroup>
  ...
  <Collapser id="collapser-2" title="Metadata-setting Lambda function">
    ...
    That's it, you're all set! The Lambda function will now start automatically adding the compression metadata to all newly-added S3 objects.
  </Collapser>
</CollapserGroup>
