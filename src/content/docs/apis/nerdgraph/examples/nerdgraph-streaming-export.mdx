...
You must have [an Amazon Kinesis Firehose set up to receive New Relic data](#firehose-setup). 
...
To set up streaming data export, you must first set up an Amazon Kinesis Firehose.

For each unique New Relic account that streams data to a Firehose, there must be an AWS role set up with the New Relic account ID.

These instructions allow New Relic to securely obtain credentials to write to your Firehose. 

Create a Firehose for use with the streaming export. Leverage "Direct PUT or other sources" and specify a destination compatible with New Relic's JSON event format (for example, S3, Redshift, or OpenSearch). Then, assign an IAM role.


### IAM Firehose write access policy [#create-policy]

To create a Firehose write-access policy: 

1. Create a policy.
2. Select the Firehose service, and select `PutRecord` and `PutRecordBatch`
3. For `Resources`, select the delivery stream, add ARN, and select the region of your stream. 
4. Enter your account number, and enter your desired delivery stream name in the name box.
5. Create the policy.

### IAM role for granting New Relic write access [#iam-role]

To set up the IAM role: 

1. Navigate to the IAM service in AWS.
2. Click on **Roles**.
3. Create a role for an AWS account, then select for **another AWS account**
...
6. Click **Permissions**, and select the policy you created above. 
...
mutation {
  streamingExportCreateRule(
    ...
    awsParameters: {
      awsAccountId: "<var>YOUR_AWS_ACCOUNT_ID</var>", 
...
mutation {
  streamingExportUpdateRule(
    id: <var>YOUR_NEW_RELIC_ACCOUNT_ID</var>, 
    ruleParameters: {
      description: "<var>RULE_DESCRIPTION</var>", 
      name: "<var>RULE_NAME</var>", 
...
{
  actor {
    account(id: <var>YOUR_NR_ACCOUNT_ID</var>) {
      streamingExport {
