...
2. Create a policy. 
3. Select the Firehose service, and select `PutRecord` and `PutRecordBatch`
4. For `Resources`, select the delivery stream, add ARN, and select the region of your stream. 
5. Enter enter your account number, and enter your desired delivery stream name in the name box.
6. Create the policy.
