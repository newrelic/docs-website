...
With our streaming export feature, available with [Data Plus](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-plus), you can send your data as its ingested to New Relic to an AWS Kinesis firehose. We'll explain how to create and update a streaming rule using NerdGraph, and how to view existing rules. 
...
* To create or edit a stream: must be a [core or full platform user](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) with ** capability
* Applicable to all [data types stored in NRDB](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data) except [metric timeslice data](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data) 
* This feature exports data as it's ingested: if you want past data, see [Historical data export](/docs/apis/nerdgraph/examples/nerdgraph-historical-data-export)

## What is streaming export? [#definition]

You can constantly send a little or a lot of data (limit up to your ingest volume). 
...
Before the rule finishes registering, you can't initiate another mutation action such as `Enable`, `Disable`, or `Update`, because the rule is locked for the creation process.
...
The creation can finish and change the status at any time within the six minutes. The status will change to `ENABLED`, `DISABLED`, or `CREATION_FAILED`. `ENABLED` means the rule is created successfully and SE starts to take traffic.
...
```
mutation {
  streamingExportDisableRule(id: 889) {
    id
    ...
  }
}
...
```
mutation {
  streamingExportEnableRule(id: 889) {
    id
    ...
  }
}
...
```
mutation {
  streamingExportDeleteRule(id: 17) {
    id
    ...
  }
}
