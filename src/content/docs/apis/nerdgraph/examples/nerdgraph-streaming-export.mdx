...
* `compressPayload`: Whether to compress payloads before sending them to the Kinesis stream. For more details, see [compression](#compression).
...
You can enable compression by setting the `payloadCompression` field under `awsParameters` to `GZIP`. Currently, GZIP is the only compression format available, though we may choose to make more formats available in the future. Note that this field is optional, and if not specified, compression will be disabled by default. Compressin
