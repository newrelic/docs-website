...
When compression is enabled on an existing export rule, the next batch may contain both compressed and uncompressed data. This is due to batching within Kinesis Firehose and has been observed in AWS, and may also be present in Azure. To avoid this, you can temporarily disable the export rule before enabling compression, or create a new Kinesis Firehose stream for the compressed data to flow through.

If you do encounter this issue, you can view the compressed part of the data by following these steps:
