...
* Created an [API key](https://docs.newrelic.com/docs/apis/getting-started/intro-apis/understand-new-relic-api-keys#user-api-key).
...
<CollapserGroup>
  ...
  >
    ...
    }</integration_slug></provider_slug></integration_slug></provider_slug>
    ```
  </Collapser>
  ...
  >
    ...
    mutation {
      cloudUnlinkAccount (
        accountId: <<var>NR_ACCOUNT_ID</var>>,
        accounts: {
    <provider_slug><integration_slug>      { linkedAccountId: <<var>LINKED_CLOUD_ACCOUNT_ID</var>> }
        }
      ) {
        unlinkedAccounts {
          id
