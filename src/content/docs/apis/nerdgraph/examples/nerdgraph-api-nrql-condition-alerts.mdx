...
* [NRQL anomaly condition](#anomaly-condition)
...
Here's an example of creating a anomaly condition:

```
mutation {
  alertsNrqlConditionAnomalyCreate(accountId: <var>YOUR_ACCOUNT_ID</var>, policyId: <var>YOUR_POLICY_ID</var>, condition: {
    name: "anomalyCondition"
    enabled: true
    anomalyDirection: UPPER_ONLY
    nrql: {
...
1. Determine the type of your existing condition by requesting the type field in a `nrqlConditionsSearch` query like this:
   ...
   <Callout variant="tip">
     The `type` returned is what you use for your update mutation. For example, if the type returned is `STATIC`, use `alertsNrqlConditionStaticUpdate`. If the type returned is `ANOMALY`, use `alertsNrqlConditionAnomalyUpdate`.
   </Callout>
...
Only fields that you provide in the update are changed. In the following example, `anomalyDirection` returns unchanged, but `name` is updated.

```
mutation {
  alertsNrqlConditionAnomalyUpdate(id: <var>YOUR_CONDITION_ID</var>, accountId: <var>YOUR_ACCOUNT_ID</var>, condition: {
...
<CollapserGroup>
  ...
  >
    ...
    For example, `valueFunction` is only relevant for static NRQL conditions and `anomalyDirection` is only relevant on anomaly NRQL conditions.
    ...
    So, when the returned type is a static condition, it will return the `valueFunction` in the object, and when the returned type is an anomaly condition, it will return `anomalyDirection` instead. Here is an example response:
