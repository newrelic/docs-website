...
You can run NRQL queries using the [query builder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) in the UI, or you can use our NerdGraph API. Both let you [query across up to 30 accounts](#cross-account-query) at once. NerdGraph also gives you the ability to do things you can't do from the UI, such as run [asynchronous queries](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial).
...
You can run a query across up to 30 accounts at once, either from the query builder in the UI or using NerdGraph:

* **UI**: Use the account dropdown in the query builder to select up to 30 accounts to run your query against.
* **NerdGraph**: Pass the [account IDs](/docs/accounts/accounts-billing/account-structure/account-id) for the accounts you want to query in the `accounts` field.

Here's an example of running a cross-account NRQL query with NerdGraph:
