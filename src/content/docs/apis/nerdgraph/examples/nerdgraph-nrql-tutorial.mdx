...
You can run NRQL queries in the New Relic platform [query builder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) or with NerdGraph. NerdGraph gives you the ability to do some things you can't do from the UI, such as [query across multiple accounts](#cross-account-query), and run [asyncronous queries](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial).
...
For tips on creating a dashboard with data from multiple accounts, see [the NerdGraph dashboard tutorial](). 
