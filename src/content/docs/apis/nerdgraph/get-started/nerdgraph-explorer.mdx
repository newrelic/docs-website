...
import graphqlDocumentation from 'images/graphql-documentation.png'

import mutationExample from 'images/mutation-example.png'

import toolsMenu from 'images/tools-menu.png'
...
Next we'll walk you through ssome steps that will help you learn to use the explorer. 
...
In GraphQL, [mutations](https://graphql.org/learn/queries/#mutations) are a way to execute queries with side effects that can alter the data by creating, updating, or deleting objects (in REST APIs, this is commonly referred to as CRUD operations).
...
Try creating more complex queries by clicking fields and expanding objects in the query builder (be careful with mutations though, as they could result in changes to your account). Check out some of [the example requests](#more-examples) below. 
