...
* `entitySearchQueries`: This field must contain all the queries as you expect them to be stored. If you want to add a new query, include it in the `query` field and do not provide any query `id`. If you want to modify an existing query, include it in the `query` field and provide its existing `id`. If you want to delete an existing query, just do not add any query with that `id` anymore.
...
However, if you just do not use the `statusConfig` object when you create a workload, the following rules will be added by default:
...
* `workload.name`: Optional. You can specify a name for the new workload. If you do not specify one, the new workload will get the name of the original workload appended with `- Copy`.
