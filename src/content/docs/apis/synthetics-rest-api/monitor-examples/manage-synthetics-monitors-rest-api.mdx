...
topics:
  ...
  - Monitor examples
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/apis/synthetics-rest-api/monitor-examples/manage-synthetics-monitors-rest-api
---
...
To use the Synthetics REST API, you must have a user role that allows that capability. You must also [activate API access and generate your Admin User's API key](/docs/apis/rest-api-v2/requirements/api-keys#creating) from your account settings. Then you can make standard API calls via the command line.

For an overview of all available New Relic APIs, see [Intro to APIs](/docs/apis/getting-started/introduction-new-relic-apis).
...
These are the [monitor types](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors#types-monitors) and how they are referred to in the API:
...
You must use your [Admin User's API key](/docs/apis/rest-api-v2/requirements/api-keys#creating) to make Synthetics REST API calls. The account's REST API key will not work.
...
For [EU-based accounts](https://docs.newrelic.com/docs/using-new-relic/welcome-new-relic/get-started/introduction-eu-region-data-center#partner-hierarchy), use the following endpoint:
...
<CollapserGroup>
  ...
  >
    To view a single Synthetics monitor, send a GET request to `{API_Endpoint}/v3/monitors/{id}`. Replace the &#x7B;id} in the following example with the specific monitor ID.
  ...
  >
    To update an existing monitor in New Relic, send a PUT request to `{API_Endpoint}/v3/monitors/{id}`. In addition, for scripted monitors, follow the procedures to [update the BASE64 encoded script](/docs/apis/synthetics-rest-api/monitor-examples/manage-synthetics-monitor-scripts-rest-api).
  ...
  >
    To patch an existing monitor in New Relic, send a PATCH request to `{API_Endpoint}/v3/monitors/{id}`.
    ...
    PATCH requests are intended to update individual attributes of your New Relic Synthetics monitor rather than updating the entire entity, so you may provide only the attributes you want to update.
  ...
  >
    To delete an existing monitor in New Relic Synthetics, send a DELETE request to `{API_Endpoint}/v3/monitors/{id}`. Replace the [&#x7B;id}](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api) in the following example with the specific monitor ID:
...
<CollapserGroup>
  ...
  >
    To view the script associated with a specific `SCRIPT_BROWSER` or `SCRIPT_API` monitor in New Relic Synthetics for your account, send a GET request to `{API_Endpoint}/v3/monitors/{id}/script`. Replace the &#x7B;id} with the specific monitor ID. For example:
  ...
  >
    To update the script associated with a specific `SCRIPT_BROWSER` or `SCRIPT_API` monitor in New Relic Synthetics for your account, send a PUT request to `{API_Endpoint}/v3/monitors/{id}/script` with a JSON payload that contains the `scriptText` (required).
