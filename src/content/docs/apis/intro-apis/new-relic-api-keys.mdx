...
In order to allow only approved people in your organization to report data to New Relic, access that data, and configure features, our monitoring solutions and APIs require one of several API keys. 
...
You can view and manage most API keys from the API keys UI page, which is at [one.newrelic.com/launcher/api-keys-ui.api-keys-launcher](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher) (from the [account dropdown](/docs/glossary/glossary/#account-dropdown), click **API keys**). 

![Screenshot of the API keys UI](./images/api-keys-ui.png 'Screenshot of the API keys UI')
<figcaption>From [one.newrelic.com/launcher/api-keys-ui.api-keys-launcher](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher), you can create and manage license keys, browser keys, and user keys. 
</figcaption>
...
Mobile app tokens are used to report [mobile monitoring](/docs/mobile-monitoring/new-relic-mobile/get-started/introduction-mobile-monitoring) data. Each token is tied to a specific mobile app.
  ...
  <td>
  See [the mobile monitoring UI for a specific app](/docs/mobile-monitoring/new-relic-mobile/maintenance/viewing-your-application-token)
  </td>
  <td>
...
## Security-related practices for API keys [#security-practices]

API keys, in the wrong hands, can present security risks. For example: 

* Access to your data ingest keys (for example, the license key) would allow someone to send arbitrary data to your account. 
* Access to a user key of someone in your organization would allow someone to view your New Relic data and configure your New Relic account. 

For these reasons, you should treat your API keys securely, as you would passwords and other sensitive information. Some recommendations: 

* For the license key and the browser key, consider implementing a key rotation strategy: creating new keys and deleting old ones on a set schedule. (Note that this isn’t possible for the mobile app token.) 
* For user keys: 
  * Instruct your team members to keep their user keys secure. 
  * When members leave your organization, even if they're basic users, ensure you remove them from New Relic. 
...
The license key is used for almost all New Relic data ingest. The exceptions are browser monitoring data (which uses a [browser key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-browser-key)) and mobile monitoring data (which uses a [mobile app token](/docs/mobile-monitoring/new-relic-mobile/maintenance/viewing-your-application-token/)). 

The license key is a 40-character hexadecimal string associated with a New Relic account. When you first [sign up](/docs/subscriptions/creating-your-new-relic-account) for New Relic, that creates an organization with a single account, and that account has its own license key. If more accounts are added, each account will have its own license key. An account's original license key cannot be deleted but you can create additional license keys that can be managed and deleted. For assistance deleting an account's first license key, [contact support](https://support.newrelic.com).
...
One of the API keys used for data ingest is the browser key. The browser key allows the ingestion of data from New Relic [browser monitoring](/docs/browser/new-relic-browser/getting-started/introduction-browser-monitoring). 
...
Besides our [main API keys explained above](#overview-keys), we have several other older API keys that some customers still use: 
