...
We temporarily provide to you the entire key during the creation of it, but after creating it we only provide the first 8 characters of the key in the API Keys UI.

If after key creation you need to obatin the full key again, you would click on the ellipses in the API Keys UI, copy the key ID and then [use NerdGraph to query for the full key ](/docs/apis/nerdgraph/examples/use-nerdgraph-manage-license-keys-user-keys/#query-keys).
