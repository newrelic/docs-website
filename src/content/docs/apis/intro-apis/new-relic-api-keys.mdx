...
## Replace API keys [#replace-keys]

Here's how to replace each of our four most common API keys:

<CollapserGroup>
  <Collapser
    id="replace-license-key"
    title="Replace license key"
  >
    ...
    By default, all New Relic accounts are created with an original account license key, which is shared by the entire organization. You can't delete this key from the API keys UI, but you can contact New Relic support to regenerate this key.

    To replace user-created license keys:

    1. Log in as the New Relic user whose key you’d like to replace.
  ...
  <Collapser
    id="replace-browser-key"
    title="Replace browser key"
  >
    ...
    By default, all New Relic accounts are created with an original account browser key, which is shared by the entire organization. You can't delete this key from the API keys UI, but you can contact New Relic support to regenerate this key.

    To replace user-created browser keys:
  ...
  <Collapser
    id="replace-mobile-app-tokens"
    title="Replace mobile app tokens"
  >
    Mobile monitoring uses a mobile app token to report data, rather than the license key. Mobile app tokens are all unique and re-generated each time an application is registered to NR1. To replace mobile keys associated with your app, re-authorize the app in New Relic.
  ...
  >
    ...
    <Callout variant="important">
      If you believe a user key has been compromised, you should replace <DNT>**all**</DNT> keys associated with that user or created by that user.
    </Callout>

    To replace your user key, you'll create a new key, update references to the old key in your software, and then delete the old key. To replace keys in the UI:
...
We also have several older or less common API key type. To replace these keys:

<CollapserGroup>
  <Collapser
    id="replace-insights-keys"
    title="Replace Insights insert key and Insights query key "
  >
    ...
    To replace these keys:

    1. Log in to New Relic on an account with admin permissions.

    2. Go to [<DNT>**one.newrelic.com/api-keys**</DNT>](https://one.newrelic.com/api-keys) (or [<DNT>**one.eu.newrelic.com/api-keys**</DNT>](https://one.eu.newrelic.com/api-keys) for our EU data center, or [<DNT>**one.jp.newrelic.com/api-keys**</DNT>](https://one.jp.newrelic.com/api-keys) for our JP data center).

    3. On the right side of the page, find the <DNT>**Looking for other keys?**</DNT> section and click the link for the key type you want to replace:
  ...
  <Collapser
    id="replace-pixie-api-key"
    title="Replace Pixie API keys"
  >
    Pixie API keys are used to authenticate custom applications to the Pixie platform. Pixie API keys can't be modified. To replace Pixie API keys, you must create a new key, then delete the old one. For more information about Pixie API keys, see [Managing API Keys](https://docs.px.dev/reference/admin/api-keys/) in the Pixie docs.
  </Collapser>

  <Collapser
    id="replace-partnership-keys"
    title="Replace partnership API keys"
  >
