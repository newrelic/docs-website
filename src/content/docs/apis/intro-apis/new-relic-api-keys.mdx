...
You can manage most API keys from the [API keys UI page](https://one.newrelic.com/api-keys). To find that: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings/#user-menu), click <DNT>**API keys**</DNT>.

To view a key you will need to copy the key ID which you will need to view the key via Nerdgraph, which you can also [use to manage keys](/docs/apis/nerdgraph/examples/use-nerdgraph-manage-license-keys-user-keys)
...
* For the license key and the browser key, consider implementing a key rotation strategy: creating new keys and deleting old ones on a set schedule. Considerations:
  * You can't delete the original ingest keys associated with an account, so to implement a strong security strategy, you must create additional ingest keys that you can later delete.
...
* For the <InlinePopover type="userKey"/>:
  ...
  * When members leave your organization, remove them from New Relic which will disable all the user keys assocaited with their user ID
