...
Note that you can't delete or change an account's original license key (the one generated upon account creation). For that, [contact New Relic support](https://support.newrelic.com/). 
...
* Requires admin-level user permissions. If you do not have access to the REST API key or the REST API explorer, it might be due to lack of permissions. Talk to your New Relic account manager, or use a [user key](/docs/apis/get-started/intro-apis/types-new-relic-api-keys#user-api-key) instead.  
