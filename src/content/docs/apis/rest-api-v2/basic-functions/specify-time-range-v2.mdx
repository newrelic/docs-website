...
If you are using the New Relic REST API Explorer (v2), you can use the diamond <img title="icon-api-diamond" alt="icon-api-diamond" src={iconApiDiamond}/> icon to select time ranges. If you have an [EU region account](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center), the above URL begins with `https://api.eu.newrelic.com`.
