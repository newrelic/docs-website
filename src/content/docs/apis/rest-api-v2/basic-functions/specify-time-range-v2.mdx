...
By default the API time input uses Universal Time Coordinated (UTC). To offset the UTC, use plus or minus `00:00` in your API call. This allows you to input times in a different time zone; however, this <DoNotTranslate>**does not**</DoNotTranslate> modify the output times, which are always UTC.
