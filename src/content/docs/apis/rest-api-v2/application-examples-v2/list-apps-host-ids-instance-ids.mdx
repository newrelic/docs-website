...
## Get a Single application ID from a List filtered by application name [#get_app_id_from_name]

You need to get an ID for an app that you have by name, or potentially do not have at all with a little you can pull just the id you need .

```
API_RESPONSE=$(curl -X GET 'https://api.newrelic.com/v2/applications.json' \
-H "X-Api-Key:<var>${API_KEY}</var>" \
-H "X-Accept: application/json" \
-d "filter[name]=<var>${APP_NAME}</var>")
APP_ID=$(jq -r '.applications[0] | .id' <<<"$API_RESPONSE")
```

* This example uses `jq` as a command line tool. [jq](https://stedolan.github.io/jq/download/) 
* You can pair this call with a `POST` to create a new deployment with the [API Explorer](https://rpm.newrelic.com/api/explore/application_deployments/create).
