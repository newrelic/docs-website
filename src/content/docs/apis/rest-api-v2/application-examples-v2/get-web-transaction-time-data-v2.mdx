  ...
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        <DNT>**Application total time calculation**</DNT>
      </th>
...
These New Relic agents will replace the `Database` metric name with `Datastore/${DBTYPE}`, where `${DBTYPE}` identifies the appropriate database; for example, Postgres, MongoDB, Microsoft SQL Server, ODBC, Redis, SQLite, Memcached, Solr, <DNT>Elasticsearch<DNT>, etc.

## Datastore total time [#datastore]

If your <DNT>**Web transaction time**</DNT> chart shows the generic <DNT>**Database**</DNT> category, do not use these calculations. Instead, use the [Database](#database) calculations.
