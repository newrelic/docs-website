...
![Average Apdex for application and browser](src/images/Apdex_average.png 'Average Apdex for application and browser')

<figcaption>
  **APM > (selected app) > Monitoring > Overview**: You can use API v2 to return
  the same Apdex scores shown in the [APM **Overview**
  page](/docs/apm/applications-menu/monitoring/applications-overview-dashboard).
</figcaption>
...
<Callout variant="tip">
  Even though this example adjusts the time zone, the output still returns as
  UTC time.
</Callout>
