...
The average response time (milliseconds) is the value that appears on the main chart for your app on the [APM <DoNotTranslate>**Summary**</DoNotTranslate> page](/docs/applications-menu/applications-overview). New Relic uses this formula to calculate response time:
...
<Callout variant="tip">
  If your app has <DoNotTranslate>**never**</DoNotTranslate> reported a queuing time, the metric `WebFrontend/QueueTime` will not exist.
</Callout>

## Response time [#app_rep_time]

On the [APM <DoNotTranslate>**Summary**</DoNotTranslate> page](/docs/applications-menu/applications-overview), [Response time](/docs/data-analysis/user-interface-functions/view-your-data/response-time) is the overlaid line within the main chart for your app. To obtain the metric values shown by this line, use the same formula and two commands described above, but remove `summarize=true` from the commands.
