...
<Callout variant="important">
  The New Relic REST API v2 (excluding Alerts endpoints) and the Deployments v0 API will [reach end of life on June 30, 2027](/eol/2026/06/eol-30-06-26-rest-api-v2). After this date, these endpoints will no longer be available. Migrate your integrations to NerdGraph before that date.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>`Api-Key` or `X-Api-Key` header</td>
      <td>`Api-Key` header, System Identities</td>
    </tr>
    <tr>
      <td>Data fetching</td>
...
<Callout variant="tip">
  If you don't have the entity GUID yet, you can filter by application ID instead: `WHERE appId = YOUR_APP_ID`.
</Callout>
...
**NerdGraph:** Use the `changeTrackingCreateEvent` mutation. This mutation was designed specifically for REST API migration — it accepts an `applicationId` directly, so you don't need to look up entity GUIDs:
...
<Callout variant="tip">
  Unlike most NerdGraph operations, `changeTrackingCreateEvent` accepts a numeric `applicationId` directly, making it the easiest migration path from the REST API. You can also use `entityGuid` instead if you have it. See [Track changes using NerdGraph](/docs/change-tracking/change-tracking-graphql/) for all available fields.
</Callout>
