...
<Callout variant="tip">
  To obtain the same information from the [New Relic API Explorer (v2)](https://rpm.newrelic.com/api/explore), select <DoNotTranslate>**Users > GET List**</DoNotTranslate>.
</Callout>
...
<DoNotTranslate>**Note:**</DoNotTranslate> Character matching is a simple string. No regular expression capability is available, so multiple matches may occur if the selected string is not unique.
