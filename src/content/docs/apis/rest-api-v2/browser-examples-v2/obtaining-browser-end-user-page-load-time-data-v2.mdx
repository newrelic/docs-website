...
This describes how to use the New Relic REST API (v2) to obtain the data shown on the <DoNotTranslate>**Browser page load time**</DoNotTranslate> chart.
...
* For calculated values, the time range you specify must be consistent in <DoNotTranslate>**all**</DoNotTranslate> of the queries; otherwise the final calculations will be incorrect.
...
The `EndUser/RB:average_dom_content_load_time` is the time spent in the browser to parse and interpret the HTML. This is measured by the browser's <DoNotTranslate>**DOM Content**</DoNotTranslate> event.
