...
In Windows systems, the agent must be installed from an Administrator account, and by default it will run as a service with the Local System logon context. It can also run as a non-admin/user account, which can be local, Active Directory, Microsoft Entra ID or Group Managed Service Account / gMSA. The included script `set-service-account.ps1` helps with the transition and sets the required privileges.
