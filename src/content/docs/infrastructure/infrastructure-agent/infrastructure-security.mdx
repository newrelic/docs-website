...
* **Installing for a non-admin/user account:** The account can be local, Active Directory, Microsoft Entra ID, or Group Managed Service Account / gMSA. The included script `set-service-account.ps1` helps with the transition and sets the required privileges.
