...
In Windows systems, the agent has the following installation and account requirements:

* **Installing from an Administrator account:** The agent runs by default as a service with the Local System logon context.
* **Installing for a non-admin/user account:** The account must be a local, Active Directory, Microsoft Entra ID, or Group Managed Service Account / gMSA. The included script `set-service-account.ps1` helps with the transition and sets the required privileges.
