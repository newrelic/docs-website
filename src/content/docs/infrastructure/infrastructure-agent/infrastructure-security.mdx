...
* Install the agent from an Administrator account.
* Run it by default as a service with the Local System logon context.
* Run it as a non-admin/user account only if the account is local, Active Directory, Microsoft Entra ID, or Group Managed Service Account / gMSA.
* Use the included script `set-service-account.ps1` to help with the transition and set the required privileges.
