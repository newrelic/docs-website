...
By default, if you don't specify a version, the CLI automatically installs the latest available version of the infrastructure agent. To install a specific version instead, append the version number to the recipe name using the `@` symbol, like this: `newrelic install -n infrastructure-agent-installer@X.XX.X` where `X.XX.X` represents the version number (for example, `1.64.0`).
