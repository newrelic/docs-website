...
## Install specific agent versions [#install-specific-versions]
...
To find available infrastructure agent versions and their release details, refer to the [Infrastructure agent release notes](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/).

For Linux:

```bash
curl -Ls https://download.newrelic.com/install/newrelic-cli/scripts/install.sh | bash && sudo NEW_RELIC_API_KEY=INSERT_YOUR_API_KEY NEW_RELIC_ACCOUNT_ID=INSERT_YOUR_ACCOUNT_ID /usr/local/bin/newrelic install -n infrastructure-agent-installer@1.64.0
```
