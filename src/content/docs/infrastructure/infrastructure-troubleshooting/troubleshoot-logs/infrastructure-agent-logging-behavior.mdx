...
Smart verbose mode prevents debug messages being logged until an error message is logged. Once the agent logs an error (calls the logger), it flushes the debug cache on the disk, then flushes the error message on disk. You only get the most recent number of configured debug messages.
