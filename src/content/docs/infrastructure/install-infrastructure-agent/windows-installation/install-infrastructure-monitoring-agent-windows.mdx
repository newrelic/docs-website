...
1. Add your <InlinePopover type="licenseKey" /> to the `license_key` attribute in your `newrelic-infra.yml` file, located in `C:\Program Files\New Relic\newrelic-infra\`. When you're finished, your `newrelic-infra.yml` file should look like the following:
  ```yml
  license_key: YOUR_LICENSE_KEY
  ```

1. To start the `newrelic-infra` service, run:
  ```shell
