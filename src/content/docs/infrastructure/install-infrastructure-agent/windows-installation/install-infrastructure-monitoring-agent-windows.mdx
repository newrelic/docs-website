...
If you'd like to do a manual installation, you can use our PowerShell script, or follow the step-by-step instructions.
...
<CollapserGroup>
  ...
  >
    ...
    3. In an administrator account, run the install script using an absolute path:
    ...
    4. Add your [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) to the `license_key` attribute in `newrelic-infra.yml`, located in `C:\Program Files\New Relic\newrelic-infra\`. When finished, the contents of `newrelic-infra.yml` should resemble the following:
