...
## <img src={otherlinux} title="Linux" alt="Linux"/>Linux: Start, stop, restart, or check agent status [#linux]
...
## <img src={windows} title="Windows Server" alt="Windows Server"/> Windows: Start, stop, restart, or check agent status [#windows]
...
### <img src={cmd} title="CMD" alt="CMD"/> Command prompt (cmd.exe)
