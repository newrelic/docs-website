...
1. Create the `newrelic-infra.yml` agent [config file](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) with your New Relic [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). For config option explanations, see [configuration settings](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings).
