...
1. Download the [packaged agent file](https://download.newrelic.com/infrastructure_agent/binaries/linux/) or use our one-liner command to fetch the given version of the agent with checksum and automatically verify it. You will need to set desired architecture (amd64, 386, arm64, arm) and [version](https://github.com/newrelic/infrastructure-agent/releases)

   ```
   V=1.27.3 ARCH=amd64; echo "https://download.newrelic.com/infrastructure_agent/binaries/linux/${ARCH}/newrelic-infra_linux_${V}_amd64.tar.gz" | { read    url; wget "${url}"{,.sum}; shasum -a 256 --check ${url##*/}.sum; }
   ```
3. Unpack the file.
4. Make sure the file unpacks with the following structure:
