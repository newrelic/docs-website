...
The reported data does <DoNotTranslate>**not**</DoNotTranslate> include information related to the container orchestrator (for example, ECS or Kubernetes). To monitor those, you can add the orchestrator's cluster and task names as [labels](https://docs.docker.com/ee/ucp/admin/configure/add-labels-to-cluster-nodes/).
