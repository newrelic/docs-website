---
title: Install the agent as a container
tags:
  - Infrastructure
...
Want to try out our infrastructure agent for Linux? Create a New Relic account for free! No credit card required.

<InlineSignup/>

## What you need [#requirements]

The containerized version of the infrastructure agent requires <DNT>Docker/DNT> 1.12 or higher.

From version 1.42, the infrastructure agent supports `containerd`, so it can be used in Kubernetes v1.24+, which removed support for <DNT>Docker/DNT>shim, or any other `containerd`-based solution. If both the `containerd` and the `dockerd` runtime sockets are available, the infrastructure agent will operate with the `containerd` runtime socket. The container must run on a [Linux distribution and version](/docs/infrastructure/install-infrastructure-agent/get-started/compatibility-requirements-infrastructure-agent#operating-systems) supported by the infrastructure agent. The container image is available and supported on AMD64 and ARM64 architectures.
...
The following are basic instructions for creating a custom <DNT>Docker/DNT> image on Linux. This allows you to deploy the infrastructure agent as a container that can monitor its underlying host.

<DNT>**Recommendation:**</DNT> Extend the [`newrelic/infrastructure` image](https://hub.docker.com/r/newrelic/infrastructure/), and use your own `newrelic-infra.yml` agent [config file](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent). Once your image is built, you can easily spin up a container without having to provide more launch time configurations. Do not provide secrets using environment variables with <DNT>Docker/DNT>.

### <DNT>Docker/DNT> CLI

1. Create the `newrelic-infra.yml` agent [config file](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) with your New Relic <InlinePopover type="licenseKey" />. For config option explanations, see [configuration settings](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings).
...
### <DNT>Docker/DNT> Compose [#basic-compose]
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Bind mounts the host's <DNT>Docker/DNT> daemon socket to the container. This allows the agent to connect to the Engine API via the <DNT>Docker/DNT> daemon socket to collect the host's container data. If you interact with the `containerd` runtime instead, replace this mount with `/run/containerd/containerd.sock:/run/containerd/containerd.sock`.
      </td>
    </tr>
  </tbody>
</table>
...
Once the infrastructure agent is running in a <DNT>Docker/DNT> container, it can collect the same [host compute data](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page) and [event data](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) that the infrastructure agent is capable of collecting when running natively on a host. For more information, see our documentation about how to [view your <DNT>Docker/DNT> container data](/docs/infrastructure/install-infrastructure-agent/linux-installation/docker-instrumentation-infrastructure#find-data).
