...
<CollapserGroup>
  ...
  >
    To set an environment variable, use the variable name from the config file in all caps and prefix with `NRIA_`; for example, `NRIA_LICENSE_KEY`. 
    
    In containerized environments, populate configuration properties such as license key and tags via environment variables. For example, a license key populated via environment variables looks like this:
