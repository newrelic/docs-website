...
The <InlinePopover type="infrastructure" /> agent can be configured in a variety of ways. The default method is to edit the [configuration variables](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings) in the `newrelic-infra.yml` file.
