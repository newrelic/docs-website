...
<CollapserGroup>
  ...
  >
    This sets the cloud provider the agent is running in. When this is set up to a value different from the default, the agent will wait until it has acquired the instance ID from the cloud provider before submitting any data to the backend.
