...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      <DoNotTranslate>**Recommendation:**</DoNotTranslate> Do not change this setting.
    </Callout>
  ...
  >
    ...
    If set to a <DoNotTranslate>**negative value**</DoNotTranslate>, the agent will keep checking the connection until it succeeds.
...
<Collapser
  ...
  <Collapser
    ...
    >
      Use this setting if you are instrumenting the host with a different source than the infrastructure agent (for example, an OpenTelemetry collector or Prometheus node exporter) and want to keep using the infrastructure agent on-host integrations to monitor other infrastructure services. 
      When enabled, the agent will report host inventory and integrations telemetry (event metrics and inventory) decorated with host metadata; however host metrics (CPU, memory, disk, network, processes) will be disabled.  
