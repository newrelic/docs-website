...
<CollapserGroup>
  ...
  <Collapser
      ...
      title="ntp_metrics"
    >
     `ntp_metrics` is a key-value map usted to configure the NTP. When enabled and an NTP Hosts list is provided,
     the agent will provide the host's NTP Offset in the System Sample. This value is requested to the provided NTP
     hosts pool every `interval` minutes. Between intervals the last requested value will be reported.
