...
translate:
  - jp
metaDescription: 'Update the infrastructure agent on Linux, Windows and macOS.'
redirects:
...
* Single hosts: see the `agentVersion` tag on any Host entity or `Agent version` in the Infrastructure UI.
* Multiple hosts: visit **Infrastructure > Navigator** and group reporting entities by `agentVersion` to see a list of active hosts per version.
* Query: use the `agentVersion` attribute on any standard agent metric to generate a report, in example, the following NRQL query shows number of unique active hosts group by agent version:
```
FROM SystemSample SELECT uniqueCount(hostname) FACET agentVersion LIMIT MAX
``` 

To manually check the version on the host, you can log onto a server and run `newrelic-infra --version`, or the applicable command for your package manager.
The directory where you'd run this differs between operating systems, but here are the default paths:
...
You'll first need to determine the latest version available from the agent [release notes]((/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes)), we recommend running versions that are no older than 3 months when possible.

To identify active hosts that are running a version that's older than a given 'major.minor' (in example: 1.45.0), you can run this NRQL query:
```
FROM SystemSample SELECT uniques(hostname) WHERE
numeric(capture(agentVersion, '(?P<major>.[0-9]*).([0-9]*).([0-9]*)')) <= 1 and
numeric(capture(agentVersion, '([0-9]*).(?P<minor>.[0-9]*).([0-9]*)')) <= 45 LIMIT MAX
```
