...
* Single hosts: See the `agentVersion` tag on any [host entity](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#find), or look for its `Agent version` in the <DoNotTranslate>**Infrastructure**</DoNotTranslate> UI.
* Multiple hosts: Go to <DoNotTranslate>**Infrastructure > Navigator**</DoNotTranslate> and group reporting entities by `agentVersion` to see a list of active hosts per version.
