...
2. List the policies attached to role:
   ...
   This should return 3 policies `AmazonECSTaskExecutionRolePolicy`, `AmazonEC2ContainerServiceforEC2Role` and a third one that should grant read access to the <LicenseKey />. In the following example the policy it's named `NewRelicLicenseKeySecretReadAccess`.
...
3. Retrieve the default policy version:
   ...
   This retrieves the policy permissions. There should be an entry for Action`secretsmanager:GetSecretValue` if you used AWS Secrets Manager to store your <LicenseKey />, or an entry for `ssm:GetParameters`if you used AWS Systems Manager Parameter Store:
