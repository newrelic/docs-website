...
Here's an example [NRQL query](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) that returns the count of containers associated with each <DNT>Docker/DNT> image in an ECS cluster named `MyClusterName` created in `us-east-1`:
...
On EC2 nodes with Cgroup version V2, the `nri-docker` integration collects all container metrics from the <DNT>Docker/DNT> API. For a list of variations in the collected metrics, see the [`ContainerSample`](/attribute-dictionary/?event=ContainerSample) attributes.
</Callout>
