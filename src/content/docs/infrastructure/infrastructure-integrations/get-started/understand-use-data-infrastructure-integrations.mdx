...
* View dashboards: You can find your dashboards in the New Relic UI. Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**</DoNotTranslate> and look for your data. 
...
<CollapserGroup>
  ...
  >
    ...
    * Metric values are treated as <DoNotTranslate>**attributes**</DoNotTranslate>: key-value pairs attached to an [event](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#event-data). For example, the MySQL integration has an 'active connections' metric; this would be found by querying the `connectionsActive` attribute of the `MysqlSample` event.
