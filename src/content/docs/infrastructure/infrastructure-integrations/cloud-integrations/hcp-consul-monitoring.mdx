...
1. We use the **dogstatsd_addr** here so we can add the **consul.source.datacenter**: tag to each metric. This allows the New Relic ONE <InlinePopover type="dashboards" /> to facet by Consul Datacenter.
