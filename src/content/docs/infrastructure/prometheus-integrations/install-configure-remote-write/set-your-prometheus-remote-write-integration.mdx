...
2. Add a new `remote_write` URL to your Prometheus YML file. Add this information under `global_config` in the file, at the same indentation level as the `global` section.
   ...
   **Any Prometheus version**
   ...
   ```
   *This approach passes credentials in the URL. We do not recommend using it unless one of these other approaches doesn't work in your environment.*
...
If you have metrics that do not follow Prometheus naming conventions, you can configure remote-write to tag the metric with a `newrelic_metric_type` label that indicates the metric type. This label is stripped when received by New Relic.
