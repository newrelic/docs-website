...
If you're using the [Prometheus Operator](https://github.com/prometheus-operator/prometheus-operator), you'll need to create a secret with the New Relic <LicenseKey /> for the account to which you want to report data. Make sure the API key is of the type `Ingest - License`.
