...
<CollapserGroup>
  <Collapser
    ...
    title="X-License Key"
  >
    Your [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) is not an API key. The license key is used for authentication and to identify which account to write data into. If you are configuring Prometheus to write into different New Relic accounts, use a different key on each remote write URL.
  </Collapser>
