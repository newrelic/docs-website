...
You can drop data you do not want to keep by changing the `remote_write` section of the YAML config file.
...
If a target is sending a noisy metric that you do not want sent to New Relic, you can specify that New Relic should drop that data. 

### Example 

Let's say you do not want to receive data for the metric `node_memory_active_bytes` from an instance running at `localhost:9100`. Using the `write_relabel_config` entry shown below, you can target the metric name using the `__name__` label in combination with the instance name.
