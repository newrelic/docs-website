...
Every target scrape generates the `up` metric with the all target metrics. If scraping is successful, this metrics have `1` as a value. If it's not successful, their value is `0`.
