...
By default, the Prometheus agent scrapes metrics only from [Prometheus integrations](/docs/infrastructure/prometheus-integrations/integrations-list/integrations-list-intro). Unless you selected to scrape all the Prometheus endpoints in the cluster, the Prometheus agent filters the endpoints to be scraped by using the following Kubernetes labels:

* `app.kubernetes.io/name`
* `app.newrelic.com/name`

Check the value of the compatible values in the [GitHub repository](https://github.com/newrelic/newrelic-prometheus-configurator/blob/64af9453f4b20d4aab88a4d1afda55cf9a6e63c4/charts/newrelic-prometheus-agent/README.md#values).
