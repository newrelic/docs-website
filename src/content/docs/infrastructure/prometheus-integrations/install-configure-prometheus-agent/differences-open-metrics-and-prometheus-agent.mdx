...
We've also added a configurator that translates our simplified configuration file to a prometheus server configuration file, to give you an easier configuration experience. We suggest that you read the documentation on the `newrelic-prometheus-agent` configuration options, since this document focuses on the differences between Prometheus OpenMetrics and the Prometheus agent.
...
We also give you a curated dashboard for self-metrics with performance and health information, and also metrics volume sent. Install the dashboard in your New Relic account [here](http://link_to_quickstart).
