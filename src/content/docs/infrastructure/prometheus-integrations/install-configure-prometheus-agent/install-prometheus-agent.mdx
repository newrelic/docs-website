---
title: "Install and configure the Prometheus agent with New Relic's Prometheus Agent"
tags:
  - Integrations
...
The New Relic's Prometheus Agent is a thin wrapper (we refer to it as Prometheus Configurator in [its repository](https://github.com/newrelic/newrelic-prometheus-configurator/blob/main/charts/newrelic-prometheus-agent/values.yaml#L184)) on top of Prometheus to run Prometheus in Agent mode and its configuration burden. The purpose of this integration to use an easier to understand YAML to create a configuration used by Prometheus ready to send the metrics to New Relic.
