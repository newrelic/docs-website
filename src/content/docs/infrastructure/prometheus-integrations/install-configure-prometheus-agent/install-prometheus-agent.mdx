...
All the targets that are in the cluster annotated with `newrelic.io/scrape: "true"` are discovered and scraped.

On the other hand, the default annotation from the Prometheus community, `prometheus.io/scrape: "true"`, is discovered but we filtered to only scrape [Prometheus Integrations](/docs/infrastructure/prometheus-integrations/integrations-list/integrations-list-intro) that we support.
