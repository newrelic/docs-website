...
- Prometheus Counter metrics <DoNotTranslate>**without**</DoNotTranslate> the name suffix `total`, `count`, `sum`, or `bucket` will be converted as <DoNotTranslate>**Gauge**</DoNotTranslate>.
- Prometheus Gauge metrics <DoNotTranslate>**with**</DoNotTranslate> the name suffix `total`, `count`, `sum`, or `bucket` will be converted as <DoNotTranslate>**Counter**</DoNotTranslate>.
...
Although our Prometheus OpenMetrics integration in Kubernetes has been replaced by the NR Prometheus agent, it will be supported for a period of time (~~June 2023~~January 2024). 
