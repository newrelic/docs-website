...
Although our Prometheus OpenMetrics integration in Kubernetes has been replaced by the NR Prometheus Agent, it will be supported for a period of time. 
New Relic wants to ensure a smooth and seamless migration to our customers.

You can keep either the Prometheus OpenMetrics integration, `nri-prometheus`, the Prometheus agent, `newrelic-prometheus-agent`, or both. 
