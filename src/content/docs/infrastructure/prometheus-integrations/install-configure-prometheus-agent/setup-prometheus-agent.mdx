...
The following example only scrapes `Pods` and `Endpoints` with the `newrelic.io/scrape: true` annotation, and the `k8s.io/app` label with `postgres` or `mysql` as values. For the endpoints, the annotation must be present in the service related to it. Take into account that the regex are anchored, e.g.: if you configure `scrape: 'true'` Prometheus will evaluate `true` as `^true$`. To unanchor it use `.*true.*` so it also matches `a-true-example`.
