...
## How to configure the Prometheus Agent?
...
That list is defined in the [values.yaml](https://github.com/newrelic/newrelic-prometheus-configurator/blob/main/charts/newrelic-prometheus-agent/values.yaml#L203) of New Relic's prometheus agent helm chart. This list can be extended or reduced as desired but it is important to note that some dashboards might not work out of the box with custom labels or values.

<Callout variant="important">
  When upgrading, new integration filters might be available. Therefore, the amount of data scraped could increase, depending on the services in your cluster, after an upgrade involving integration filters. This can be avoided by saving a fixed list of `app_values` in your values.yaml file.
</Callout>
...
If you need to scrape all the targets annotated with `prometheus.io/scrape: "true"`, you will need to perform one of the following actions, depending on the installation methos you chouse:
...
- (Using Helm) Setup the values bellow in your prometheus-agent configuration:
...
The prometheus agent defines a static target job to scrape self metrics by default, you can set up additional static targets by including additional jobs.
The example bellow includes an additional job to scrape a server managed separately, it also includes the self-metrics job to keep reporting the
prometheus agent metrics, as defined [by default](https://github.com/newrelic/newrelic-prometheus-configurator/blob/main/charts/newrelic-prometheus-agent/values.yaml#L340).
...
<Callout variant="tip">
  It is important to keep in mind that if the `static_targets` section is modified and no self-metrics job is included the agent metrics will not be reported.
</Callout>
...
They can be applied at any configuration section, but set it at remote write level makes the filtering wider.
If you set them at `newrelic_remote_write` level, the filters apply to all metrics that are being sent to New Relic while if you set them at any other section they apply to the metrics scraped by that section. The metric filter process happens after the metrics had been scraped from the targets.
