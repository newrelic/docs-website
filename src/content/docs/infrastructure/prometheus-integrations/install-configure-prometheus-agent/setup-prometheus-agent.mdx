...
Moreover, it might happen that, due to a new version of filters, a target that was already scaped by a different job is scraped again. 
In order to avoid that you could save a fixed list of `app_values` in your `values.yaml` file or you could create an alert based on the following query:
```
FROM Metric select uniqueCount(job) facet instance, cluster_name limit 10 since 2 minutes ago
```
If any value is different from 1 then you have two job scraping the same instance in the same cluster.
