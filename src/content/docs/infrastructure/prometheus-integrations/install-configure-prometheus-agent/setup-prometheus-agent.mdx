...
If you need to scrape all the targets annotated with `prometheus.io/scrape: "true"`, you will need to perform one of the following actions, depending on the installation method you chose:
...
The following example only scrapes `Pods` and `Endpoints` with the `newrelic.io/scrape: "true"` annotation, and the `k8s.io/app` label with `postgres` or `mysql` as values. For the endpoints, the annotation must be present in the service related to it. The regexes are anchored, that is, if you configure `scrape: 'true'`, Prometheus will evaluate `true` as `^true$`. To un-anchor it, use `.*true.*` so it also matches `a-true-example`.
...
prometheus agent metrics, as defined [by default](https://github.com/newrelic/newrelic-prometheus-configurator/blob/main/charts/newrelic-prometheus-agent/values.yaml).
...
They can be applied at any configuration section, but set it at remote write level makes the filtering or transformations wider.
If you set them at `newrelic_remote_write` level, the filters apply to all metrics that are being sent to New Relic while if you set them at any other section they apply to the metrics scraped by that section. The metric filter process happens after the metrics have been scraped from the targets.
