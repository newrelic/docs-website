...
## Prometheus OpenMetrics, Prometheus Configurator, or remote write integration? [#pomi-remote_write]
...
<CollapserGroup>
  ...
  >
    ...
    * Prometheus Sharding (see [Scaling Prometheus Agent instances](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/scale-prometheus-instances)).

    * No need for local storage.

    Recommendations:

    * The [scrape interval](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent#target-scrape-interval#target_scrape_interval) is the biggest factor influencing data volumes: select it based on your observability needs. For example, changing from 15s (default value) to 30s can reduce data volumes by 50%.

    * Set your filters and configure data to target (see [Filter Prometheus metrics](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/filter-prometheus-metrics)).
