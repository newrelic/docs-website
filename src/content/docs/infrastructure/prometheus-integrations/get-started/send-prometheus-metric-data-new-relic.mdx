...
If you already have a Prometheus server, we recommend getting started with the remote write integration. Otherwise, depending on your needs to may choose between the [Prometheus Agent for Kubernetes](#Agent)and  [Prometheus OpenMetrics integration for Docker](#OpenMetrics).
...
New Relic's Prometheus Agent allows you to easily scrape Prometheus metrics from a Kubernetes cluster. By leveraging on the service discovery and the Kubernetes labels you will get instant access to metrics, dashboards and alerts of the most popular workloads.

Prometheus Agent can be installed in two modes:

* [Alongside the Kubernetes integration](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/install-prometheus-agent#kubernetes-install): Prometheus Agent will be installed automatically together with the [Kubernetes integration](/docs/kubernetes-pixie/kubernetes-integration/get-started/introduction-kubernetes-integration).
...
* Leverage on [service discovery](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent.mdx#k8s-target-discovery) to automatically monitor new services once deployed.
...
New Relic's Prometheus OpenMetrics integrations for Docker allow you to scrape [Prometheus](https://prometheus.io/) endpoints and send the data to New Relic, so you can store and visualize crucial metrics on one platform.
...
* Read the how-to for completing the [Prometheus Agent for Kubernetes](/docs/integrations/prometheus-integrations/install-configure-prometheus-agent/install-prometheus-agent).
