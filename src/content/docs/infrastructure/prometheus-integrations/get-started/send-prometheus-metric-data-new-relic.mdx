...
<CollapserGroup>
  ...
  >
    ...
    * Predefined set of <InlinePopover type="dashboards"/> and alerts.
...
The Prometheus Agent of New Relic allows you to easily scrape Prometheus metrics from a Kubernetes cluster. By leveraging on the service discovery and the Kubernetes labels, you'll get instant access to metrics, dashboards, and <InlinePopover type="alerts"/> of the most popular workloads.
...
Unlike the [Prometheus Agent](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/install-prometheus-agent/) and [Docker](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics) OpenMetrics integrations, which scrape data from Prometheus endpoints, the remote write integration allows you to forward [telemetry data](/docs/telemetry-data-platform/get-started/capabilities/get-know-telemetry-data-platform) from your existing Prometheus servers to New Relic. You can leverage the full range of options for setup and management, from raw data to queries, dashboards, and beyond.
...
* Prometheus documentation for the list of all possible [exporters](https://prometheus.io/docs/instrumenting/exporters/) created by the open source community.
