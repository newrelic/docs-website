...
<img title="Docker icon" alt="Docker icon" src={dockerLogoCrop}/> **Docker:** If you configure the `http://localhost:8080/metrics` endpoint, the integration will also scrape its own metrics.

<img title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" src={imgIntegrationK8}/> **Kubernetes:** By default, the scraper will also scrape its own metrics.
