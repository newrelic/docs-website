...
1. Follow the [Cilium documentation](https://docs.cilium.io/en/stable/operations/metrics/) to add the required configuration along with an example deployment for Prometheus to discover the metrics endpoints. Cilium installation uses this helm command and deploys Cilium with operator and hubble metrics enabled.
...
4. Install the [Cilium quickstart](https://newrelic.com/instant-observability/ext-cilium-quickstart) to access built-in dashboards and [alerts](/docs/alerts-applied-intelligence/new-relic-alerts/learn-alerts/introduction-alerts/).
