...
[Argo CD](https://argoproj.github.io/) Argo CD is a Kubernetes controller, responsible for continuously monitoring all running applications and comparing their live state to the desired state specified in the Git repository.

Use New Relic to view a dashboard based on Prometheus metrics which helps you understand the Argo CD infrastructure of your k8s cluster. With New Relic you can monitor:
...
1. Follow the [Argo CD  documentation](https://argo-cd.readthedocs.io/en/stable/operator-manual/metrics/) to know more about how Argo CD expose Prometheus metrics.
...
4. Install the [Argo CD quickstart](https://newrelic.com/instant-observability/argocd-quickstart) to access built-in dashboards and and [alerts](/docs/alerts-applied-intelligence/new-relic-alerts/learn-alerts/introduction-alerts/).

  Once you imported, you can edit or clone the assets to adapt them to your specific requirements.

  <Callout variant="important">
  Some charts of the dashboard include queries with conditions that require the identification of your pod or endpoint with one of the these labels `app.kubernetes.io/name`, `app.newrelic.io/name`, `k8s-app` containing the string `argo`.
  </Callout>
...
* List unique metric names:
  ...
  ```sql
  FROM Metric SELECT uniques(metricName) WHERE metricName LIKE 'argo_%' LIMIT MAX
  ```

* Count number of metric updates:

  ```sql
  FROM Metric SELECT datapointcount() WHERE metricName LIKE 'argo_%' LIMIT MAX
  ```

* Estimate data ingestion (daily ingest, in bytes):

	```sql
  FROM Metric SELECT bytecountestimate() WHERE metricName LIKE 'argo_%' SINCE
1 day ago
  ```
