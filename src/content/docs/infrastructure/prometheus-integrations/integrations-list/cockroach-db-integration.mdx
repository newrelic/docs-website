...
4. Install the [CockroachDB quickstart](https://newrelic.com/instant-observability/?search=cockroachdb) to access built-in dashboards and [alerts](/docs/alerts-applied-intelligence/new-relic-alerts/learn-alerts/introduction-alerts/).
  ...
  <CollapserGroup>
    <Collapser
      id="metric-type-remote-write"
      title="Metric type override for Remote Write integration"
    >
      Prometheus agent default's configuration include the corresponding rules by default, but if you use Remote Write integration you need to take into account that, due to CockroachDB metrics naming, it is not possible to properly infer the type properly. Consequently, you will need to include some rules to [override the metric type](/docs/infrastructure/prometheus-integrations/install-configure-remote-write/set-your-prometheus-remote-write-integration/#override-mapping).
