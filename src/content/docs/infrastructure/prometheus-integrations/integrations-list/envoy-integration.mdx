...
Change the `remote_write`, the `scrape_configs`, and `write_relabel_configs` sections of the YAML file.
...
remote_write:
  url: https://metric-api.newrelic.com/prometheus/v1/write?prometheus_server=<REMOTE_WRITE_NAME>
...
write_relabel_configs:
  ...
  action: keep
``` 
Set the `REMOTE_WRITE_NAME` entry as `EnvoyProxy_Prometheus`.
...
Follow the troubleshooting tips from [Envoy documentation](https://argo-cd.readthedocs.io/en/stable/operator-manual/troubleshooting/) to make sure that metrics are configured as expected on your cluster.
