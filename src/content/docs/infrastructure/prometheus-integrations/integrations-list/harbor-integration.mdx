...
4. Install the [Harbor quickstart](https://newrelic.com/instant-observability/harbor-newrelic) to access built-in dashboards and and [alerts](/docs/alerts-applied-intelligence/new-relic-alerts/learn-alerts/introduction-alerts/).
...
* List unique metric names:
  ...
  ```sql
  FROM Metric SELECT uniques(metricName) WHERE metricName LIKE 'harbor_%' LIMIT MAX
  ```
...
* Estimate data ingestion (daily ingest, in bytes):
	...
	```sql
  FROM Metric SELECT bytecountestimate() WHERE metricName LIKE 'harbor_%' SINCE
1 day ago
  ```
