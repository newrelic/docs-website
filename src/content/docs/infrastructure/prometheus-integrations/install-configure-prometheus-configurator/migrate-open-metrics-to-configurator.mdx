...
Node metrics are no longer scraped by default in `newrelic-prometheus-agent` as it may cause metric duplication since it's normally used with New Relic's K8s integration.
