...
Resource attributes can be added to the Jenkins plugin configuration and will be included as attributes on all log, metric, and trace data generated by the plugin. Resource attributes with names that start with `tags.` will [also add entity tags to your Jenkins entity](/docs/opentelemetry/best-practices/opentelemetry-best-practices-resources/#tags). 

<CollapserGroup>
  <Collapser
    id="attributes-config"
    title="Defining attributes in the Jenkins plugin configuration"
  >
    Resource attributes can be added to the `configurationProperties` element in the `io.jenkins.plugins.opentelemetry.JenkinsOpenTelemetryPluginConfiguration.xml` file. 
  ...
  <Collapser
    id="attributes-env"
    title="Defining attributes via environment variables"
  >
    Resource attributes can also be defined using the `OTEL_RESOURCE_ATTRIBUTES` environment variable. 
