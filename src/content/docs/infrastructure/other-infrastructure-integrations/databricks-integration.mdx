...
Use this integration to monitor your DataBricks Spark environment.
...
This integration uses a standalone tool from the New Relic expiramental repository. This integration can be run on a host, or locally for testing. 
This integration can be run on the following host platforms:
...
4. Create a file named **config.yml** in the configs directory and copy the contents of the file [configs/config.template.yml](https://github.com/newrelic-experimental/newrelic-databricks-integration/blob/main/configs/config.template.yml) in this repository into it.
...
4. Click the button labeled **Edit** to [edit the compute's configuration](https://docs.databricks.com/en/compute/clusters-manage.html#edit-a-compute).
...
**NEW_RELIC_REGION** - The [region](https://docs.newrelic.com/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#regions-availability) of your New Relic account; one of **US** or **EU**.
