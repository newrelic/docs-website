...
Use infrastructure monitoring's **Host not reporting** [condition](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information) to notify you when we've stopped receiving data from an <InlinePopover type="infrastructure" /> agent. This feature allows you to dynamically alert on groups of hosts, configure the time window from five to 60 minutes, and take full advantage of alerts notifications.
