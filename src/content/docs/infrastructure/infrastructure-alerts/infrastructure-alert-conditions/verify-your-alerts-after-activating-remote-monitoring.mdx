...
You decide to enable remote monitoring in your integration, and set the parameter `remote_monitoring` to true. After restarting the <InlinePopover type="infrastructure" /> agent, the alert stops monitoring. This happens because the entity resulting from activating remote monitoring has a different key, and therefore the current filter does not apply.
