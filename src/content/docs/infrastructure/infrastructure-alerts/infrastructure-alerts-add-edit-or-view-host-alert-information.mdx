...
9. Optional: Set the [<DNT>**Close open alert events after**</DNT>](#incident-time-limit) time limit to automatically close open incidents after a certain amount of time (this defaults to 24 hours for infrastructure conditions).
...
You can use template substitution to insert values from the attributes in the associated incident event. The template format is `{{attributeName}}`. For the attributes you can use when creating a description, see [alert event attributes](/docs/alerts/create-alert/condition-details/alert-event-attributes).
