...
9. Optional: Set the [<DNT>**Close open alert events after**</DNT>](#alert-event-time-limit) time limit to automatically close open alert events after a certain amount of time (this defaults to 24 hours for infrastructure conditions).
...
Anyone included in the policy's [notification channels](#notification) receive alert notifications directly. In addition, anyone with permissions for your New Relic account can view infrastructure alert events and individual alert events through the user interface.
...
3. From the <DNT>**Events**</DNT> list, select the alert event.
4. To [view detailed information in alerts](/docs/alerts/alert-event-management/view-alert-event-details) about the selected alert event, select the link.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * View detailed information about [alert events](/docs/new-relic-solutions/get-started/glossary#alert-event): Use the [alerts](/docs/alerts/alert-event-management/view-alert-event-details) UI or the [notification channel](#notification) integrated with the associated policy.
      </td>
    </tr>
...
The text you place in an alert condition's <DNT>**Description**</DNT> field is passed downstream to associated alert events and notifications. A description can be used for several purposes, including:
...
You can use template substitution to insert values from the attributes in the associated alert event event. The template format is `{{attributeName}}`. For the attributes you can use when creating a description, see [alert event attributes](/docs/alerts/create-alert/condition-details/alert-event-attributes).
...
If there are entity tags associated with your alert event, then they can be accessed using the entity tag name. An example of this would be `{{tag.aws.awsRegion}}`. When entity tags are available to use, you see them included with the alert event, and displayed when you view the alert event.

This field has a maximum character size of 4,000.

## Add or edit a runbook URL [#runbook-url-infrastructure]

The [alert condition creation process](#create-condition) includes an option for setting a URL for runbook instructions. This lets you link to information or standard procedures for handling an alert event. Before adding or updating the link, make sure you use a valid URL.
...
## alert event time limit [#alert-event-time-limit]

The alert event time limit allows you to define a time period after which alert events will be force-closed. By default, alert event time limit is 24 hours for insfrastructure conditions.

To add or update an alert condition's alert event time limit:

1. Select an alert condition, and make changes to the alert event time limit.
2. Save the condition.
...
For more information, see [Best practices for filtering in infrastructure alerts](https://support.newrelic.com/s/hubtopic/aAX8W0000008Zfu/relic-solution-best-practice-for-filtering-in-infrastructure-alerts) in New Relic's Support Forum.
