...
<Callout variant="caution">
  If you have filtered your `Host Not Reporting` condition using tags or labels and then remove a critical tag or label from a targeted host, the system will open a Host Not Reporting alert event, since it will characterize that host as having lost its connection.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Email addresses (identified in the policy) will be notified automatically about [threshold](/docs/using-new-relic/welcome-new-relic/get-started/glossary#alert-threshold) alert events for any host matching the filters you have applied, depending on the policy's [alert event preferences](/docs/alerts/organize-alerts/specify-when-alerts-create-events).
      </td>
    </tr>
...
4. (Optional) Enable the <DNT>**Don't trigger alerts for hosts that perform a clean shutdown**</DNT> option to prevent false alerts when hosts are intentionally shut down via command line. This option is currently supported on Windows and systemd-based Linux systems.
  ...
  <Callout variant="tip">
    To avoid false <DNT>"Host not reporting"</DNT> alert events for intentionally shut down hosts, consider these strategies:
    ...
    * Tag the host and enable the <DNT>**Don't trigger alerts**</DNT> setting: Add the `hostStatus: shutdown` tag to your host along with checking the option mentioned above. This will stop all <DNT>**Host not reporting**</DNT> alert events from opening for that host, as long as that tag is on it, regardless of the agent version or OS. If you remove the tag, New Relic will start opening <DNT>**Host not reporting**</DNT> alert events. 
  </Callout>

Depending on the policy's [alert event preferences](/docs/alerts/organize-alerts/specify-when-alerts-create-events), it will define which notification channels to use when the defined <DNT>**Critical**</DNT> threshold for the condition passes. To avoid "false positives," the host must stop reporting for the entire time period before an alert event is opened.

<DNT>**Example:**</DNT> You create a condition to open an alert event when any of the filtered set of hosts stop reporting data for <DNT>**seven**</DNT> minutes.

* If any host stops reporting for five minutes, then resumes reporting, the condition <DNT>**does not**</DNT> open an alert event.
* If any host stops reporting for seven minutes, even if the others are fine, the condition <DNT>**does**</DNT> open an alert event.

## Investigate the problem [#view-alert-events]
...
3. Optional: Use the email notification's [<DNT>**Acknowledge**</DNT> link](/docs/alerts/get-notified/acknowledge-alert-events) to verify you are aware of and taking ownership of the alerting alert event.
4. Use the email links to examine additional details in the [<DNT>**Alert event details**</DNT> page](/docs/alerts/alert-event-management/view-alert-event-details).
