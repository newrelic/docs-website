...
By applying filters to the hosts and processes that are important to your business, you can define [alerting thresholds](/docs/alerts/overview#alert-threshold) to decide when incidents open and New Relic sends an email notification to you depending on the policy's [incident preferences](/docs/alerts/organize-alerts/specify-when-alerts-create-events). These examples illustrate how to use infrastructure monitoring's <DNT>**Process running**</DNT> condition to monitor your processes.
...
If you create the alert condition directly with infrastructure monitoring, New Relic will send an email notification when the defined threshold for the alert condition passes depending on the policy's [incident preferences](/docs/alerts/organize-alerts/specify-when-alerts-create-events). Your alert policy defines which personnel or teams and which notification channels we use.
