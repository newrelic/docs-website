...
By default, we <DNT>**highly recommend**</DNT> using the GCP primitive role `Viewer`, which grants [permissions for read-only actions that do not affect your cloud infrastructure state, such as viewing (but not modifying) existing resources or data.](https://cloud.google.com/iam/docs/understanding-roles) Google automatically manages this role and updates it when new Google Cloud services are released or modified.

## Optional role [#optional]

Alternatively, you can create your own custom role based on the [list of permissions](/docs/integrations/google-cloud-platform-integrations/get-started/integrations-custom-roles#list-permissions), which specifies the minimum set of permissions required to fetch data from each GCP integration. This gives you more control over the permissions set for the New Relic authorized account.
...
2. In each custom role, add the permissions specifically required for the cloud services you want to monitor according to the following list.
