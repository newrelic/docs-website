...
By default we <DoNotTranslate>**highly recommend**</DoNotTranslate> using the GCP primitive role `Viewer`, which grants "[permissions for read-only actions that do not affect your cloud infrastructure state, such as viewing (but not modifying) existing resources or data.](https://cloud.google.com/iam/docs/understanding-roles)" This role is automatically managed by Google and updated when new Google Cloud services are released or modified.
