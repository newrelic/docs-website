...
New Relic also collects metrics for the following Vertex AI resource types. These resource types don't create entities in the entity explorer, so query their metrics directly on the `Metric` event type, for example: `FROM Metric SELECT * WHERE metricName LIKE 'gcp.aiplatform.%'`.

<CollapserGroup>
  <Collapser
    id="metrics-only-table"
    title="Vertex AI metrics-only resource types"
  >
    <table>
      <thead>
        <tr>
...
Launch stage reflects Google's designation at the time of writing. See [Google's Vertex AI metrics documentation](https://cloud.google.com/monitoring/api/metrics_gcp_a_b#gcp-aiplatform) for current availability.
