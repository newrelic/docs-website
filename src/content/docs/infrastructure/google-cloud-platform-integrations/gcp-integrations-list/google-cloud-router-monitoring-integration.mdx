...
[New Relic integrations](/docs/infrastructure/introduction-infra-monitoring) with the [Google Cloud Platform (GCP)](https://cloud.google.com/) include one that reports [Google Cloud Router](https://cloud.google.com/network-connectivity/docs/router) data to New Relic. This document explains how to activate the GCP Cloud Router integration and describes the data it reports.
...
New Relic also collects metrics for the following Cloud Router resource types. These resource types don't create entities in the entity explorer, so query their metrics directly on the `Metric` event type, for example: `FROM Metric SELECT * WHERE metricName LIKE 'gcp.compute.%'`.

<CollapserGroup>
  <Collapser
    id="metrics-only-table"
    title="Cloud Router metrics-only resource types"
  >
    <table>
      <thead>
        <tr>
...
<Callout variant="tip">
  Network Region metrics use the `gcp.compute.*` namespace rather than the `gcp.router.*` namespace used by the Router and NAT Gateway entities above.
</Callout>

Launch stage reflects Google's designation at the time of writing. See [Google's Cloud Router metrics documentation](https://cloud.google.com/monitoring/api/metrics_gcp_p_z#gcp-router) for current availability.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Maximum number of connections from a VM to a single internet endpoint (IP:port) on the NAT gateway.
      </td>
    </tr>
  </tbody>
</table>
