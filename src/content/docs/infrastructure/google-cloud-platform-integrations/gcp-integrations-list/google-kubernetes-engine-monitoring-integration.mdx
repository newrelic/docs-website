...
All Google Kubernetes Engine metrics available in GCP Cloud Monitoring are collected as dimensional metrics in the `Metric` event type. As noted in the callout above, the Workload Identity Federation integration doesn't create GKE entities, so query these metrics directly on the `Metric` event type, for example: `FROM Metric SELECT * WHERE metricName LIKE 'gcp.container.%'`.

#### Metrics-only resource types [#metrics-only]

New Relic collects metrics for the following Google Kubernetes Engine resource types.

<CollapserGroup>
  <Collapser
    id="metrics-only-table"
    title="Google Kubernetes Engine metrics-only resource types"
  >
    <table>
      <thead>
        <tr>
...
Launch stage reflects Google's designation at the time of writing. See Google's documentation on [Metrics for Kubernetes Engine](https://cloud.google.com/monitoring/api/metrics_gcp#gcp-container) for current availability.
