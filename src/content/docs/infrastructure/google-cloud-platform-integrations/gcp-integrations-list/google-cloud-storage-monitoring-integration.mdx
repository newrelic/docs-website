...
New Relic also collects metrics for the following Cloud Storage resource types. These resource types don't create entities in the entity explorer, so query their metrics directly on the `Metric` event type, for example: `FROM Metric SELECT * WHERE metricName LIKE 'gcp.storage.%'`.

<CollapserGroup>
  <Collapser
    id="metrics-only-table"
    title="Cloud Storage metrics-only resource types"
  >
    <table>
      <thead>
        <tr>
...
Launch stage reflects Google's designation at the time of writing. See [Google's Cloud Storage metrics documentation](https://cloud.google.com/monitoring/api/metrics_gcp_p_z#gcp-storage) for current availability.
