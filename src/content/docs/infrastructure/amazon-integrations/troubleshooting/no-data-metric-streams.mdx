...
6. Confirm that you have configured your Kinesis Data Firehose with the correct destination details:
   ...
   * Ensure the right data center US (default), EU, or JP has been selected for your New Relic account. Tip: If the `license_key` starts with “eu” then you need to select the EU data center. If the `license_key` starts with “jp” then you need to select the JP data center.
