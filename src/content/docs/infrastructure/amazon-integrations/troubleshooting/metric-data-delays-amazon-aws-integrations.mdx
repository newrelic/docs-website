...
Depending on the AWS integration, the <InlinePopover type="infrastructure" /> agent may experience explicit delays or implicit delays in the timing between the API request and the metric data returned. For more on this cause, see [Cause](#cause).
