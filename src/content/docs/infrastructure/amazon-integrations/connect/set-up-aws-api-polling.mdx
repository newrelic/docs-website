...
In some cases, you may need to use a legacy solution to receive AWS data in New Relic. While our recommended method is to set up our AWS CloudWatch Metric Streams integration, certain AWS integrations are not yet supported by CloudWatch Metric Streams. In this case, you can deploy a polling fleet and make regular calls to multiple, individual AWS APIs to retrieve metrics and metadata. You should choose this method if you use the following AWS services: 
