...
* API polling requires making calls with each individual AWS service, while the CloudWatch Metric Streams integrations collects metrics from all AWS services and custom namespaces at once.
* The fastest polling interval is 5 minutes, while CloudWatch Metric Streams streams metrics in under 2 minutes.
