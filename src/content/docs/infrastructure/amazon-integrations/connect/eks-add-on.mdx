...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Logging is depends on enable Flag in below crd portion.
    </Callout>
     ...
     - To get verbose logging details using Helm and the `nri-bundle` chart, add the following configuration to your `values.yaml` file. You can also enable verbose logging for all subcharts:
        ...
        Once you have updated the desired configuration, upgrade the solution with the following command:
  ...
  <Collapser
    ...
    title="Enable pixie"
  >
    To Enable Pixie Follow Below Steps:

    <Callout variant="caution">
      To enable pixie we need pixie and deploy key.
    </Callout>
