...
* The SSM agent is installed on the Amazon Machine Image (AMI) of the instance. Note: Most AMIs already include the SSM agent by default. See list of AMIs with SSM agent preinstalled [here](https://docs.aws.amazon.com/systems-manager/latest/userguide/ami-preinstalled-agent.html).
* The EC2 instance has the necessary permissions to interact with SSM and the instance profile/role is attached with the policy `AmazonSSMManagedInstanceCore`.
...
<Callout variant="important">
    All instances are updated with the latest New Relic infrastructure agent monthly once.
</Callout>
