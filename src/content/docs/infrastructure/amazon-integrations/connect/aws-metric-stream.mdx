...
**Migrate to AWS Cloudwatch Metric Streams Button:** 
1. If you want to replicate the namespaces from Polling to metric stream, go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS and click on the “Migrate to AWS Cloudwatch Metric Streams” button which takes you to configure your AWS Metric Streams account. 
2. You need to download the “customized CloudFormation template” which contains the namespaces preconfigured based on your existing polling configuration.
3. Adding your account details will let you get the migration complete easily.

**Alternate option:** 

1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** and click on **Add an AWS account** to link your AWS account with New Relic. You need to do this even if your AWS account is already linked with polling integrations.
