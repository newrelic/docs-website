...
For example, alerts and dashboards that use `sum` or `count` will return twice the actual number. This includes alerts and dashboards that use metrics that have a `.Sum` suffix.
