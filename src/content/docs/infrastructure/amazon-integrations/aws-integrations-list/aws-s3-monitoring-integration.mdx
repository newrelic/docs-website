...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The total number of HTTP requests made to an Amazon S3 bucket, regardless of type. When using a metrics configuration with a filter, then this metric only counts HTTP requests from objects which meet the filter's requirements. If creating multiple bucket metric filters then a filter needs to be created and scoped to all the objects in the bucket as detailed in these [AWS instructions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/configure-request-metrics-bucket.html). Note that **only** that filter should be scoped to all objects in the bucket. 
      </td>
    </tr>
