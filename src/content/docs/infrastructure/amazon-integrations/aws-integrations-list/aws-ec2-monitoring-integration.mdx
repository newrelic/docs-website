...
<Callout variant="important">
  You need to install the <InlinePopover type="infrastructure" /> agent on each EC2 host to see metrics from that host. Connecting your EC2 account allows New Relic to access EC2 metadata, such as region, type, and tags.
</Callout>
