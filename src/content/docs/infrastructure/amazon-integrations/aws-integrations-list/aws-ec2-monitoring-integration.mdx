...
You can choose to keep the legacy metadata format. This will keep the legacy formats by which EC2 Custom tags and EC2 attributes can be accessed.

Legacy tag format:
  * provider.ec2Tag_\<tagName\>
  * ec2Tag_\<tagName\>
...
Having Keep legacy metadata format will mean you can access the tags with both legacy and non-legacy formats. Off means only non-legacy formats.

Keeping this setting turned off will allow you to save ingest data bandwidth and hence lower your costs. Make sure you don't have custom dashboards or alert conditions using the previous format. New EC2 integrations will have this setting turned off by default.
