...
2. Complete the pre-requisites mentioned under `data access` subscriber type as mentioned at [Security Lake Subscriber Guide](https://docs.aws.amazon.com/security-lake/latest/userguide/subscriber-data-access.html#prereqs-creating-subscriber)
...
## 2. Create a subscriber in Security Lake [#subscriber]

Follow the below steps to create a Security Lake subscriber , which will notify a SQS Queue of new Amazon S3 objects containing the logs and events from all your configured sources: 
  ...
  7. Choose `S3` for Data Access method.  
  8. For Subscriber credentials:
    * Input your AWS account ID where you plan on installing the New Relic provided serverless application.
...
From your AWS account which you mentioned at step 8 above. 

1. Install our dedicated serverless application [Amazon Security Lake log forwarder](/docs/logs/forward-logs/aws-lambda-sending-security-logs-s3/)by opening the [AWS Serverless Application Repository](https://serverlessrepo.aws.amazon.com/applications) in your browser. Ensure you are in the AWS region where Security lake logs are getting rolled up.
2. Search for `newrelic` and check **Show apps that create custom IAM roles or resource policies** to find `newrelic-securitylake-s3-processor-LogForwarder`.
3. Click the `newrelic-securitylake-s3-processor-LogForwarder` details and click **Deploy**. 
4. Provide the following parameters to set the forwarder lambda function.
