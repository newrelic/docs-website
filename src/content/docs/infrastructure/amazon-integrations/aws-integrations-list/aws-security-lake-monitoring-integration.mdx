...
4. After creating your IAM role and getting the external ID from the subscriber, you can create a subscriber with data access by following these steps. These steps create a subscriber with access to data in the current AWS Region.
...
<CollapserGroup>
  ...
  * On the Add subscribers tab, choose Create custom subscriber.
  * For Subscriber details, mention the subscriber's name as `new-relic` and (optional) description.
  ...
  * For Subscriber credentials, provide the AWS account ID, where you will installing the AWS Lambda function, which will send the logs aggregated by Security lake to New Relic and [External ID](https://docs.aws.amazon.com/security-lake/latest/userguide/subscriber-data-access.html#subscriber-external-id).
  ...
  * Run the `create-subscriber` command to create a subscriber.
  * Provide the AWS account ID, where you will installing the AWS Lambda function, which will send the logs aggregated by Security lake to New Relic and [external-id](https://docs.aws.amazon.com/security-lake/latest/userguide/subscriber-data-access.html#subscriber-external-id).
  * Populate `source-types` with the sources that you want the subscriber to consume.
  * For `access-types`, provide S3 to create a subscriber with data access.

```
aws securitylake create-subscriber --account-id `account ID` --external-id `external ID` --subscriber-name new-relic --source-types `source types` --access-types `access types`
```
Note the `subscription id` in the output of the above command
* Run the `create-subscription-notification-configuration` command to override the default method of notification (HTTPS endpoint) and create an Amazon SQS queue,. Set the `create-sqs` parameter equal to `true`.
...
To enable this integration, set up S3 log forwarder. We suggest using our application for ease and convenience, but you can also set your own.

## Configuration 

Install our dedicated [Amazon Security Lake log forwarder](/docs/logs/forward-logs/aws-lambda-sending-security-logs-s3/) but instead of Lambda trigger, use AWS SQS service. Set up variables accordingly.
