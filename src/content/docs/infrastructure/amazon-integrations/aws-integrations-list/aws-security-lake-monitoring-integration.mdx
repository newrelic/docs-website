...
Your `SecurityLakeSQSArn` and `SecurityLakeSubscriberRoleArn` are generated here. Take note of them because you'll need them for the next step.
...
4. You'll need to copy your `SecurityLakeSQSArn` and `SecurityLakeSubscriberRoleArn` into the fields with the same name here. These values were found in the previous step of creating the subscriber. 
5. Input the `ExternalID` that you added in the previous step.
6. Input your <InlinePopover type="licenseKey" /> into the `NRLicenseKey` field.
7. Acknowledge and select **Deploy**.
