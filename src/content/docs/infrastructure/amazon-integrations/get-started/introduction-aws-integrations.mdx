...
* **Automate with CloudFormation**: 
 CloudFormation is an AWS service that allows you to model and automate your integration setup using declarative templates. To reduce manual configuration errors, use this method to connect your AWS account to New Relic. You can completely design the integration on the New Relic UI and deploy it on the AWS console. 
 
 This method provisions the resources that forward your metrics to New Relic, including an <DNT>IAM</DNT> role, an <DNT>Amazon CloudWatch</DNT> metric stream, and an <DNT>Amazon Kinesis Data Firehose</DNT> delivery stream. Metric streams operate per region, so you need a metric stream and a <DNT>Kinesis Data Firehose</DNT> delivery stream in each AWS region you monitor. Refer to the following diagram for how these components connect.

 <img
     width="70%;"
     title="AWS CloudWatch Metric Streams architecture"
     alt="Diagram of an AWS account showing AWS services, an IAM role, a CloudWatch metric stream, and a Kinesis Data Firehose delivery stream forwarding metrics from one AWS region to New Relic."
     src="/images/aws-metric-streams-architecture.webp"
/>
<figcaption>
  How a CloudWatch Metric Streams integration forwards metrics to New Relic. You need a metric stream and a Kinesis Data Firehose delivery stream in each AWS region you monitor.
</figcaption>
