...
Each of these metrics are ingested as an invidicual time series. 


Some aggregations like `sum()` or `count()` will return duplicated results if the correct dimension combination is not set. Use the `IS NULL` to filter out some of the dimensions. 

### Metrics with the `by + dimension ` suffix

For historical reasons, we suffix some pairs of metrics that have multiple dimension values with the `byFunction` suffix. For example: 
