...
A metric definition includes its name and the different dimensions. Sometimes the same metric name shows up in many metrics with different dimension combinations, for example:
...
* `aws.lambda.Duration` with dimensions `aws.lambda.FunctionName` maps to `aws.lambda.Duration.byFunction`
* `aws.lambda.Duration` with no dimensions stays the same. 

We no longer add these mappings, and we'recommited to preserving all the new metrics with their original AWS name, as defined by the general rule above. The following list displays of all the metrics suffixed with `.by + dimensionName`:
