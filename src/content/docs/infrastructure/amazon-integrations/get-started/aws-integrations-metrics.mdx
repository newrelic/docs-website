...
We no longer add these mappings, and we're are commited to preserve all the new metrics with their original AWS name, as defined by the general rule above. The following list displays of all the metrics suffixed with `.by + dimensionName`:
