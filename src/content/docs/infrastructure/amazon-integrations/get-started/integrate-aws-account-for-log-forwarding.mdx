---
title: Forward AWS service logs to New Relic
tags:
  - AWS service logs
  - Log Forwarding
  - S3 log forwarding
...
2. Check the endpoint for storing the logs, and it change if required. The default endpoint is set as per your New Relic account region. 
...
4. Paste the New Relic license key in the <DNT>**New Relic Ingest License Key**</DNT> field. The other fields auto-populate the data you have entered in New Relic. You can further change the values here, if required.
5. Read through <DNT>**Launch your Cloudformation in AWS**</DNT> and acknowledge for the necessary capabilities.
...
If the logs are not appearing in New Relic for a <DNT>**Cloudwatch via Lambda**</DNT> integration setup, do the following checks in your AWS account:
...
3. Navigate to <DNT>**CloudFormation Stack > Resources > NewRelicServerlessLogForwarder (Lambda Function) > Configuration > Environment Variables**</DNT>. Set the `DEBUG_MODE` parameter to `true` in the Lambda function’s environment variables to get debug logs.
4. Navigate to <DNT>**CloudFormation Stack > Resources > NewRelicLogForwarderDLQ**</DNT>, or search for your <DNT>**DLQ**</DNT> in AWS SQS. Check the <DNT>**DLQ resource**</DNT> in the CloudFormation stack to see if the event was a genuine failure.
...
3. Check if `NewRelicServerlessLogForwarder` has the <DNT>**S3 trigger**</DNT>. Then check the output of the nested CloudFormation stacks to find this issue.
...
1. Navigate to <DNT>**Amazon Data Firehose > Firehose streams > your-stream > Edit destination settings**</DNT>. Update your API key to the correct Ingest License key.
