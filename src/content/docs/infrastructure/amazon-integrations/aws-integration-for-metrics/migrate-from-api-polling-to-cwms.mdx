...
For seamless integration of your AWS account with New Relic, [Integration via CloudFormation and CloudWatch metric stream](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation) is the most recommended approach. However, if you have already integrated your AWS account with New Relic using API Polling, refer the following procedure to migrate from API Polling to CloudWatch Metric Streams.
