...
<Callout title="Note">
Azure Cost Management data is collected through our traditional (polling) Azure integrations only. This integration is **not** available through the [Azure Monitor integration](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/). Cost data is event-based and spans multiple resources and services, so it doesn't map to a single Azure resource type the way the Azure Monitor integration requires.

If you're migrating from the traditional Azure integrations to the Azure Monitor integration, keep the traditional Azure integration enabled to continue collecting Azure Cost Management data.
</Callout>
