...
import infrastructureAzureMonitorMetricsConfigure from 'images/azure-monitor-configure.webp'
import infrastructureAzureMonitorEnableToggle from 'images/azure-monitor-enable-toggle.webp'
...
For more details, see [How to create NRQL alerts](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions).
  ...
  <Collapser
    ...
    title="Entities"
  >
A partial list of entities are available as part of the public preview. You can create custom <InlinePopover type="dashboards" /> and alerts on top of new metrics.
...
On a typical deployment, migrating from Azure API Polling to the Azure Monitor involves the following steps (we recommend trying this on a dev / staging environment first):
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure and click "Manage Services"
2. Enable the new "Azure Monitor metrics" integration and click the "Save Changes" button
<img
  title="Shows where to enable the azure monitor"
  alt="Shows where to enable the azure monitor"
  src={infrastructureAzureMonitorMetricsIntegration}
/>

3. The "Azure Monitor metrics" integration is now available
<img
  title="Shows where azure monitor metrics integration"
  alt="Shows where azure monitor metrics integration"
  src={infrastructureAzureMonitorMetricsConfigure}
/>

4. Enable and configure the new integration as described in [this section](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/#set-up)
<img
  title="Shows where azure monitor enable toggle button"
  alt="Shows where azure monitor enable toggle button"
  src={infrastructureAzureMonitorEnableToggle}
/>

5. Once the setup is done, the previous polling integrations can be disabled under the "Manage services" settings.

<Callout variant="important">
  If both integrations are enabled for the same resource type, you may see duplicate metrics.
  This can happen if you use a generic name field to filter your results that can be shared from the Azure Monitor and the other polling Azure polling integrations.
  The results from multiple resources could be joined unintentionally.
  Note that metrics can still be told apart by faceting using the field `collector.name`.
  Rate limits as enforced by Azure will be shared between any current polling integrations and Azure Monitor.
</Callout>
