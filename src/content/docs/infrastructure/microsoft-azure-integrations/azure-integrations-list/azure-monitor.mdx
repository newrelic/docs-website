...
<Callout variant="important">
  If both integrations are enabled for the same resource type, you may see duplicate metrics. This can happen if you use a generic name field to filter your results that can be shared from the Azure monitor and the other polling Azure polling integrations. The results from multiple resources could be joined unintentionally.
