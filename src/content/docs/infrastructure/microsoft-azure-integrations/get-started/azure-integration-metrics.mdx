...
<Callout variant="Important`">
  Some aggregations like `sum()` or `count()` will return duplicated results if the correct dimension combination is not specified. 
</Callout>

To avoid some aggregations like `sum()` or `count()` matching both metrics at the same time use the `IS NULL` or `IS NOT NULL` value for the dimensions. For example: 
```
