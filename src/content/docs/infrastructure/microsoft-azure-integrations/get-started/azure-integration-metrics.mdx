...
Each of these metrics ingest as an invididual time series. 

<Callout variant="Important`">
  Some aggregations like `sum()` or `count()` will return duplicated results if the correct dimension combination isn't specified. Use the `IS NULL` to filter out some of the dimensions.
</Callout>

## API Polling Metrics [#azure-metrics-table]

For a reference on available metrics from each one of the polling integrations and their names, [check out our doc on the individual integrations](https://docs.newrelic.com/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).
