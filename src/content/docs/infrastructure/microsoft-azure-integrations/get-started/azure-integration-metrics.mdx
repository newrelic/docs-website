...
To avoid some aggregations like `sum()` or `count()` matching both metrics at the same time use the `IS NULL`, `IS NOT NULL` or  value for the dimensions. For example: 
```
