...
<Callout variant="Important">
  Some aggregations like `sum()` or `count()` will return duplicated results if the correct dimension combination isn't specified. Use the `IS NULL` to filter the dimensions.
</Callout>
