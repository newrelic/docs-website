...
1. <DoNotTranslate>**Resolve the domain:**</DoNotTranslate> Use `nslookup` or `dig` to validate that the Linux system can resolve the domain to an IP address:
...
2. <DoNotTranslate>**Ping the server:**</DoNotTranslate> Try pinging the IP address obtained above to ensure you can reach the server.
...
3. <DoNotTranslate>**Traceroute:**</DoNotTranslate> Use `traceroute` to check the network path between your system and the server:
...
4. <DoNotTranslate>**Curl:**</DoNotTranslate> Use `curl` to check if the endpoint is reachable:
