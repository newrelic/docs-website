...
<CollapserGroup>
  ...
  >
    ...
    * <DNT>**Enable selected plugins:**</DNT> To enable certain plugins, insert an exception in `disable_all_plugins`. For example, the following configuration disables all plugins, but the Network Interfaces plugin reports every 120 seconds:
