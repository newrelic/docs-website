...
Here are some troubleshooting options for [non-root users](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes) if the <InlinePopover type="infrastructure" /> agent is not starting and there are no logs:
