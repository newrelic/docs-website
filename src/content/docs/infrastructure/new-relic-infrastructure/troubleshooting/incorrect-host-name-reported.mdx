...
In Linux and macOS, the New Relic <InlinePopover type="infrastructure" /> agent tries to resolve its [fully qualified domain name](https://en.wikipedia.org/wiki/Fully_qualified_domain_name) against a domain name server, which may not be properly configured or not controlled by the same user as the New Relic infrastructure agent. In Windows, it resolves the domain name using internal tools.
