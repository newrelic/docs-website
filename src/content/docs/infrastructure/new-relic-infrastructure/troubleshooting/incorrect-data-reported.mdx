...
The <InlinePopover type="infrastructure" /> agent includes trace level logs that can be enabled on-demand to help troubleshooting complex scenarios and determine if the expected data is leaving the host as expected. 
