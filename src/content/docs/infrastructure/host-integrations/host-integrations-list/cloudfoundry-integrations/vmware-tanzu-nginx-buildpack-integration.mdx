...
* **Nginx Buildpack**: This buildpack must be installed on top of the nginx_buildpack.
* **New Relic Account**: An active New Relic account with a license key is required. This key is used to bind Nginx servers to the New Relic infrastructure agent, which transmits metrics to the New Relic Platform.

For general information about adding multiple buildpacks to manifests, see [Cloud Foundry documentation: Pushing an Application with Multiple Buildpacks](https://docs.cloudfoundry.org/buildpacks/use-multiple-buildpacks.html).
...
<Steps>
    ...
    <Step>
        Go to the **Installation UI** of Ops Manager.
    </Step>
    <Step>
        Click the blue button in the top-right corner of the Installation UI to **Apply changes**.
    </Step>
</Steps>
        </TabsPageItem>
...
* `manifest.yml`:
  ...
  * The `nginx_buildpack` must be installed before `newrelic_nginx_buildpack` as the latter requires the former to function correctly.
    ...
    * See the following pages for reference:
        * [Uuse multiple buildpacks](https://docs.cloudfoundry.org/buildpacks/use-multiple-buildpacks.html)
...
![New Relic Dashboard](https://docs.newrelic.com/static/infrastructure_screenshot-nginx-dashboard-cropped.-4627e5712fed71f4f0e5f01bb72f206f.webp)
