...
You can further decorate your metrics using labels. Labels allow you to add key and value pairs attributes to your metrics which you can then use to query, filter or group your metrics on. Our default sample config file includes examples of labels but, as they are not mandatory, you can remove, modify or, add new ones of your choice.
...
The Kafka integration collects both metrics (M) and inventory (I) information. Check the <DNT>**Applies to**</DNT> column below to find which settings can be used for each specific collection:
