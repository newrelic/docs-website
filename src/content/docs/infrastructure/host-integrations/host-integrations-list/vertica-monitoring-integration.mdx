...
Vertica is an analytic database management software company and Vertica database can be used to store and analyze terabytes to petabytes of data.
...
## Configure NRI-Flex for Vertica
...
You can choose our pre-built dashboard template named `Vertica` to monitor your Vertica metrics. Follow these steps to use our pre-built dashboard template:

1. Go to **[one.newrelic.com](https://one.newrelic.com/)** and click on **+ Add data**.
2.  Click on the **Dashboards** tab.
3.  In the search box, type `Vertica`.
4.  When you see our pre-build dashboard, click on it to install it in your account.

Your Vertica dashboard is considered a custom dashboard and can be found in the **Dashboards** UI. For docs on using and editing dashboards, see [our dashboard docs](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards).

Here's a NRQL query to check the database user information such as user name, resource pool, memory cap, temporary space cap and run time cap.
