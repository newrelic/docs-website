...
freshnessValidatedDate: 2024-12-23
---
...
In the OpenTelemetry Collector configuration below:

- Update the `docker.deployment.name` value with a unique name to identify this Docker host in your New Relic account.
...
service:
  ...
  pipelines:
    logs:
      ...
      processors: [resourcedetection, resource/docker, batch]
      exporters: [otlphttp/newrelic]
```
...
2. Search for your Docker host using the `docker.deployment.name` you configured.
