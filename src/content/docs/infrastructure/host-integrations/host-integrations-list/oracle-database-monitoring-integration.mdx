...
3. Grant `SELECT` privileges to the user on the following global views:
  ...
  In order to collect PDB metrics you will need to grant additional privileges:
  ...
  <tbody>
...

    {
     ...
     <tr>
      ...
      <td>
        It can be set to `PDB` to collect application container metrics or `ALL` for CDB and PDB. Any other value will report SYS metrics for Standalone and Multitenant with CDB access only.
      </td>
