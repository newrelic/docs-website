...
- name: nri-kafka
  env:
    # A cluster name is required to uniquely identify this collection result
    CLUSTER_NAME: "Sandbox1"
    KAFKA_VERSION: "2.0.0"
    AUTODISCOVER_STRATEGY: "bootstrap"
    BOOTSTRAP_BROKER_HOST: "localhost"
    BOOTSTRAP_BROKER_KAFKA_PORT: 9092
    ...
    TOPIC_REGEX: 'topic\d+'
    METRICS: "true"
    PRODUCERS: '[{"name":"myProducer","host":"127.0.0.1"}]'
    CONSUMERS: '[{"name":"myConsumer","host":"127.0.0.1"}]'
    DEFAULT_JMX_HOST: "127.0.0.1"
...
- name: nri-kafka
  env:
    ...
    KAFKA_VERSION: "2.0.0"
    AUTODISCOVER_STRATEGY: "zookeeper"
    ZOOKEEPER_HOSTS: '[{"host": "127.0.0.1", "port": 2181}]'
    ZOOKEEPER_PATH: "/kafka-root"
    ```
    
  >
    This gives an example for collecting JMX metrics from Java consumers and producers using credentials or specific port:
