...
* Monitor or manage cluster privileges on your Elasticsearch cluster
* A [New Relic account](https://newrelic.com/signup) with a <InlinePopover type="licenseKey"/>
* OpenTelemetry Collector Contrib installed on your host system

## Why choose OpenTelemetry?

This solution uses industry-standard OpenTelemetry protocols to provide:

* **Vendor neutrality**: Your instrumentation works with any observability backend
* **Unified data collection**: Collect metrics, logs, and traces through a single pipeline
* **High performance**: Built-in batching and optimization minimize resource impact
* **Future-proof**: Built on open standards that evolve with the ecosystem
