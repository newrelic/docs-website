...
**Required access**: Elasticsearch cluster admin privileges, New Relic account with Ingest License key
</Callout>
...
* **New Relic account** - A [New Relic account](https://newrelic.com/signup) with a <InlinePopover type="licenseKey"/>
...
<Callout variant="important">
**Before you start**: You'll need to customize two key values in the configuration:
1. **Elasticsearch endpoint** - Replace `https://localhost:9200` with your actual Elasticsearch URL
2. **Cluster name** - Choose a unique name to identify your cluster in New Relic
</Callout>
...
- **Explore metrics**: All Elasticsearch metrics are stored as `Metric` [event types](/docs/using-new-relic/data/understand-data/new-relic-data-types#events-new-relic)
- **Create custom queries**: Use [NRQL](/docs/using-new-relic/data/understand-data/query-new-relic-data) to build custom charts and dashboards
