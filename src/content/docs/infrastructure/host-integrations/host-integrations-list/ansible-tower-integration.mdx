...
* Setting the New Relic platform as the `remote_write` API destination. 
* Pointing to one end `scrape_configs` points to one end point, in this case Prometheus itself (`"prometheus"`). 
* When viewing a timeseries scraped from this configuration file, you'll see  label `job=<job_name>` to any timeseries scraped from this configuration file.
