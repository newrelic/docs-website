...
We recommend using our [guided install](https://one.newrelic.com/marketplace?account=1&state=9fc6e1c7-1ffb-1ff0-862d-1ea1ed4a1c31), or you can [install via package manager](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-monitoring-agent-linux).
...
The job name is added as a label `job=<job_name>` to any timeseries scraped from this configuration file.
...
- job_name: "tower"
  tls_config:
    ...
    scrape_interval: 5s
    scheme: https
    bearer_token: udlrIsvlcb1ya3QarUJdORHGLG8tT4
