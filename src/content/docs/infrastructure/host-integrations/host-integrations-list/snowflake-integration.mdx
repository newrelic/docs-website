...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="important">
      Note - Before setting up the Snowflake metrics, we need to ensure that SNOWSQL is properly configured.
    </Callout>

    Run the below command to store Snowflake metrics in JSON format, allowing nri-flex to read it. Ensure to modify the ACCOUNT, USERNAME, and SNOWSQL_PWD accordingly.
