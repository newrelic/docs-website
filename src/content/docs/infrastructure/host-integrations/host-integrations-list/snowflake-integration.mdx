...
* Follow our [guided install](https://one.newrelic.com/nr1-core?account=3501165&state=ff641d8b-e722-855e-4c4a-c50c23969dc3) to instrument your system with the infrastructure agent. 
...
The following instructions walk you through Snowflake integration with New Relic. The information for installation is split between this doc and a New Relic repo, which you'll go to in step 4. 
...
<CollapserGroup>
    ...
    >
    Run the following command in your CLI to clone the Snowflake repo into your machine:
    ...
    >
In the cloned repo from step 1, download the [relevant binary](https://github.com/newrelic/newrelic-snowflake-integration/releases) in the `releases` folder. In other words, if you're running Linxus, then you'd download `snowflakeintegration-linux.` 

Be sure the binary is executable for your platform. In other words, run the following command:

* For Linux, run `chmod +x snowflakeintegration-linux`
* For Windows, run `chmod +x snowflakeintegration-win.exe`
* For MacOS, run `chmod +x snowflakeintegration-macos`
...
`TEXT HERE PENDING PHANEE'S RESPONSE` 
...
<CollapserGroup>
    ...
    order by 
        execution_time desc
    ```
    4. In the `flex-snowflake.yml` file, add the following snippet:
...
commands: 
    - run: <var>YOUR_PATH_TO snowflakeintegration-platform</var> <var>YOUR_PATH_TO config.yaml</var> <var>YOUR_PATH_TO longest_queries.sql</var>
```
    </Collapser>
    <Collapser
        className="freq-link"
        id="warehouse-usage"
        title="Usage by warehouse"
    >
    To find the queries that take the longest to load:
    ...
    - name: CreditUsageByWarehouse
        ...
        commands: 
        - run: <var>YOUR_PATH_TO snowflakeintegration-platform</var> <var>YOUR_PATH_TO config.yaml</var> <var>YOUR_PATH_TO credit_usage_by_warehouse.sql</var>
    ``` 
    </Collapser>
</CollapserGroup>
...
1. Go to **[one.newrelic.com](https://one.newrelic.com)**, then click 
