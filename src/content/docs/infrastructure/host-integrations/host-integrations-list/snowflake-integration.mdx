...
With our Snowflake dashboards, you can easily track query performance and all your logins, monitor your Snowflake warehouses, and get a total overview of your storage system's health, including information about potential security incidents. 
...
To see your Snowflake data in New Relic, you'll need to install our infrastructure agent. The infrastructure agent collects data from Snowflake and sends that data to New Relic, letting you see performance metrics, logins, and other data in your dashboards. 
...
With the infrastructure agent installed, you now need to integrate the agent with your Snowflake app. The following procedures will have you navigate between this doc and the New Relic Snowflake repo to fully install the integration. 
...
<CollapserGroup>
    ...
    >
Download the [`snowflakeintegration-linux`
binary](https://github.com/newrelic/newrelic-snowflake-integration/releases) from the link seen in `releases` section. Place that binary in the cloned repo from step 1.  
    </Collapser>
    <Collapser
    id="step3"
    title="Step 3. Update `config.yaml`"
    >
To ensure you're using the most recent information for the integration, update your `config.yaml` file so that the file matches the snippet found in the [Key Pair authentication](https://github.com/newrelic/newrelic-snowflake-integration#key-pair-authentication) section in the Snowflake repo. 
...
```
    ...
    ```yml
        - name: CreditUsageByWarehouse
            ...
            commands: 
            - run: YOUR_PATH_TO_DOWNLOADED_BINARY_FILE YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/config.yaml YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/credit_usage_by_warehouse.sql
    ``` 
    </Collapser>
</CollapserGroup>
