...
Our Apache integration reports data your Apache web server to the New Relic platform. You can view dashboards of your Apache metric data, create alert policies, and create your own custom queries and charts.
...
Our integration is compatible with Apache versions 2.2 and 2.4.

Before installing the integration, ensure you meet these requirements:
...
<Tabs stacked>
  <TabsBar>
    <TabsBarItem id="windows-config">Windows config</TabsBarItem>
    <TabsBarItem id="linux-config">Linux</TabsBarItem>
  </TabsBar>
...
And then edit that based on your configuration preferences.
...
To enable automatic Apache and Apache error log parsing and forwarding, copy or rename the [`apache-log-win.yml.example`](https://github.com/newrelic/nri-apache/blob/master/apache-log-win.yml.example) file to `apache-log-win.yml`. You don't need to restart the agent but if you aren't using the default locations for the Apache log files, you may need to update the YML file with that location.
    ...
    <TabsPageItem id="linux-config">Contents for tab two
...
To enable automatic Apache and Apache error log parsing and forwarding, copy or rename the `apache-log.yml.example` file to `apache-log.yml`. You don't need to restart the agent but if you aren't using the default locations for the Apache log files, you may need to update the YML file with that location.
...
We have [example configuration files](#examples). 
