...
* Requires SQL Server user, domain user, or Group Managed Service Account with [user privileges](#users-privileges) for both `CONNECT` and `VIEW SERVER STATE`, and `READ` access permissions.
...
<Callout variant="important">
  If using a Group Managed Service Account, gMSA, for SQL Server access, the agent needs to run as the gMSA.
</Callout>
