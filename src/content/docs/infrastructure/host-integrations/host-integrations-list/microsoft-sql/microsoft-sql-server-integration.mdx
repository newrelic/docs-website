...
3. [Enable your Microsoft SQL Server](#enable-microsoft-sql-server).
4. [Configure the integration](#config).
5. [Find and use data](#find-and-use).
6. Optionally, see [Microsoft SQL Server's configuration settings](/docs/infrastructure/host-integrations/host-integrations-list/microsoft-sql/microsoft-sql-server-config).
...
<CollapserGroup>
  ...
  >
    This configuration monitors the `mssql_instance1` and `mssql_instance1` instances on the same SQL Server. When connecting directly to an instance you'll need to remove the `PORT` setting. The SQL Browser service also needs to be running on the server:
