...
5. To enable automatic MSSQL access and error log parsing and forwarding, copy (or rename) the `mssql-config.yml.example` file to `mssql-log.yml`. No need to restart the agent.
   ```shell
   sudo cp /etc/newrelic-infra/logging.d/mssql-log.yml.example /etc/newrelic-infra/logging.d/mssql-log.yml
   ```
...
To find your integration data, go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services** and select one of the Microsoft SQL Server integration links.
