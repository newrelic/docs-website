...
Our Microsoft SQL Server integration collects and sends metrics and inventory data from your Microsoft SQL Server environment to the New Relic platform. The integration lets us collect both database and instance-level metrics so that you can pinpoint and resolve errors or bugs in your Microsoft SQL Server environment. 

## Compatibility and system requirements [#compat-system-req]

Before you can collect SQL server metrics and inventory data, you need to:

* [Sign up for a New Relic account](https://newrelic.com/signup) and [install the infrastructure agent](/docs/infrastructure/infrastructure-monitoring/get-started/choose-infra-install-method) on your Windows and Linux host. 
* Set up a SQL Server user, domain user, or group managed service account with [user privileges](#users-privileges) for both `CONNECT` and `VIEW SERVER STATE`, and `READ` access permissions.s
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>
                Compatible with SQL Server versions from 2014 to 2022
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                [Windows and Linux](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent/#operating-systems)
            </td>
        </tr>
        <tr>
            <td>
...
If you're using a Group Managed Service Account (gMSA) for SQL Server access, you need to run the agent as the gMSA.
</Callout>
...
<CollapserGroup>
  ...
  <Collapser
    id="domain-user"
    title="For Windows Group Managed Service Account"
  >
...
<CollapserGroup>
  ...
  <Collapser
    id="basic-config-gmsa-domain"
    title="Basic configuration with Windows Group Managed Service Account"
  >
    This is the basic configuration used to collect metrics and inventory from your localhost. In this case we use a Group Managed Service Acccount for authentication which requires a blank username and password:
...
To find your integration data, go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**</DoNotTranslate> and select one of the Microsoft SQL Server integration links.
