...
Our NVIDIA GPU integration helps you monitor the state of GPUs. It makes use of our infrastructure monitoring agent, the Flex integration integrated with NVIDIA's SMI utility and gives you a pre-built dashboard with the most important GPU metrics like GPU utilization, ECC error counts, active compute processes, clocks and performance state, temperature, fan speed and dynamic and static information about each supported device.
...
<figcaption>
After setting up our NVIDIA GPU integration, we give you a dashboard for your GPU metrics. 
</figcaption>
...
Flex comes bundled with the New Relic infrastructure agent and it can be integrated with [NVIDIA SMI](https://developer.nvidia.com/nvidia-management-library-nvml) command-line tool to monitoring the supported GPUs. To configure flex follow these steps:
