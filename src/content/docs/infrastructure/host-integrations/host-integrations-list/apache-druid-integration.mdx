...
To use the Apache Druid integration, you need to also [install the infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) on the same host. The infrastructure agent monitors the host itself, while the integration you'll install in the next step extends your monitoring with Apache Druid-specific data.    
    </Step>
    <Step>
...
```
2. Navigate to the druid download directory and run the following command to create jar files that will be called when the server starts up:
```shell
java \
  -cp "lib/*" \
