...
2. Send Metrics on Port 4191 for each Linkerd Pod
  ```shell
  kubectl port-forward --address 0.0.0.0 -n <LINKERD_NAMESPACE> <LINKERD_POD_NAME> 4191:4191 &
  ```
Replace <LINKERD_NAMESPACE> with the actual namespace where your Linkerd pods are running, <LINKERD_POD_NAME> with the actual name of each Linkerd pod.

This will forward traffic from port 4191 on the local server to port 4191 in each Linkerd pod, allowing Prometheus to scrape metrics from each pod individually.
