...
<Callout title="Public preview">
  This integration is currently a public preview and will replace in the near future the legacy integration.
  
  If you're using the [legacy](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration.mdx) version we encourage you to test this new version, check the [migration](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-migration-guide.mdx) page for guidance.
</Callout>

Our MongoDB [integration](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) collects and sends dimensional metrics from [MongoDB](https://www.mongodb.com/).
...
<figcaption>
  Dashboard installed through the [New Relic MongoDB Monitor quickstart](https://one.newrelic.com/nr1-core?state=c95ed954-7277-e44d-7fd7-351a77b3ff37).
</figcaption>
...
Our integration is compatible with MongoDB 4.0 or higher and with [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb).
Compatibility with MongoDB Atlas is coming soon.
...
In the [MongoDB shell](https://docs.mongodb.com/manual/mongo/), execute the following commands to create a user with `clusterMonitor` and `read` roles. Note: username, password values must be replaced.
...
2. Use the following commands to create a new user, and assign `clusterMonitor` and `read` roles to the user.
...
<CollapserGroup>
  ...
  >
    This is the basic configuration used to collect all metrics. 
    **Note**: Add credentials to your mongodb UR if they are required.
...
- The [New Relic MongoDB Monitor quickstart](https://one.newrelic.com/nr1-core?state=c95ed954-7277-e44d-7fd7-351a77b3ff37) is available through [Instant Observability (I/O)](https://one.newrelic.com/nr1-core?state=7f373f09-8149-ffbf-c409-9e988bbb808e). It provides a dashboard that lets you easily explore your data, understand context, and resolve problems faster.
...
<table>
  ...
  <tr>
    ...
    <td>
      Cluster name set in your [configuration file](#instance-settings). **Note:** This dimension applies to all metrics.
    </td>
  </tr>
  <tr>
    <td>
...
These are the created 3 entities: `MONGODB_INSTANCE`, `MONGODB_DATABASE`, and `MONGODB_COLLECTION`.
