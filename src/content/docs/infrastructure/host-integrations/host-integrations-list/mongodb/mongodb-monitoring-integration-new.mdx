...
Our integration is compatible with MongoDB versions 4.0 to 7.0, [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb) and MongoDB Atlas M10 or higher.
...
The steps bellow provide guidance to set up the integration in environments such as Kubernetes, where you may report prometheus metrics from different sources in a centralized way. If you just need to set up the integration in a <DNT>Docker/DNT> container check [Docker Openmetrics instructions](#docker-openmetrics) instead.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Prometheus Openmetrics Integration for <DNT>Docker/DNT> (advanced)"
  >
    If you need to run the integration from a container you can use [Prometheus Openmentrics integration for <DNT>Docker/DNT>](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics).
...
<table>
  ...
  <tr>
    ...
    <td>
      MongoDB connection URI. <DNT>**Required**</DNT>
    </td>
    <td>
      N/A
    </td>
...
The following dimensional metrics are captured scraping the exporter and linked to the entity <DNT>**MONGODB_COLLECTION**</DNT>:
...
The following dimensional metrics are captured scraping the exporter and linked to the entity <DNT>**MONGODB_DATABASE**</DNT>:
