...
The existing legacy version will be deprecated and no longer supported during FY24, furthermore this new version brings new benefits in comparison with the legacy version:
...
* Improved new [MongoDB quickstart](https://newrelic.com/instant-observability/?search=mongodb).
...
In case is needed both version can coexist in the same host and monitor the same MongoDB instance.
