...
Kindly update the below value present in the snippet:
`FAILED_MESSAGES_FILE_NAME`: Go to the path `/var/mail/` and check the file that has been created for failed messages
