...
2. Use the below snippet to update your configuration file named `sendmail-flex-config.yml`. Below are the fields which are to be updated on the below snippet.
  a. "FAILED_MESSAGES_FILE_NAME": Go to the path `/var/mail/` and check the file that has been created for failed messages.
