...
<CollapserGroup>
  ...
  >
    ...
    4. Copy the JVM configuration file:
       ...
       ```
    5. <DNT>**Optional**</DNT>: If you're interested in monitoring <DNT>Tomcat</DNT>, use this sample metrics file:

       ```sh
       sudo cp <DNT>Tomcat</DNT>-metrics.yml.sample <DNT>Tomcat</DNT>-metrics.yml
       ```
...
Example of a `configMap` to monitor JVM in a <DNT>Tomcat</DNT> application:
...
Next, use annotations to define collection configuration. For example, here's a <DNT>Tomcat</DNT> deployment with annotations:

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: <DNT>Tomcat</DNT>-deployment
  labels:
    app: java
spec:
  replicas: 1
...
JMX data is attached to the user-defined [event type](/docs/using-new-relic/data/understand-data/new-relic-data-types#events-new-relic) specified in the [configuration file](#metrics-collection). For example, if you are interested in monitoring <DNT>Tomcat</DNT> using the JMX integration, define an `event_type` called `TomcatSample`, and [query that event type](/docs/using-new-relic/data/understand-data/query-new-relic-data).
