...
The Windows services integration is bundled with the <InlinePopover type="infrastructure" /> agent. To update the integration, [update the agent](/docs/infrastructure/install-infrastructure-agent/update-or-uninstall/update-infrastructure-agent/) to the latest version.
