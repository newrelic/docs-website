...
<Callout title="BETA FEATURE">
  This feature is currently in open beta and only applies to the versions starting on [0.0.1](https://github.com/newrelic/newrelic-prometheus-exporters-packages/releases/tag/nri-ibmmq-0.0.1) released in April 2022.
</Callout>
...
Our integration is compatible with IBM MQ 9.0+.
...
* Ensure queue monitoring is enabled on MQ.
...
To install the IBM MQ integration, choose your setup:
...
<CollapserGroup>
  ...
  >
    1. Follow the instructions for [installing an integration](/docs/install-integrations-package), using the file name `nri-ibmmq`.
...
To configure the integration, edit the config in the integration's YAML configuration file `ibmmq-config.yml`.
...
The IBM MQ integration collects dimensional Metrics:
...
Metrics are attached to the Metric sample and [event types](/docs/using-new-relic/data/understand-data/new-relic-data-types#events-new-relic) of the entities `IBMMQ_MANAGER` and `IBMMQ_QUEUE`. You can [query this data](/docs/using-new-relic/data/understand-data/query-new-relic-data) for troubleshooting purposes, or to create [custom charts and dashboards](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/).
