...
Note:
1. While using the below snippet, you are free to update the below snippet with your desired metrics.
2. Update `YOUR_BROKER_NAME`, `YOUR_TOPIC_NAME` and `YOUR_QUEUE_NAME` with your actual values.
3. All the metrics captured will be stored in `event_type`.

Steps to update jvm-metrics.yml file:
...
Metrics which are stored on the `event_type` can be explored on [one.newrelic.com](https://one.newrelic.com) using [NRQL](https://docs.newrelic.com/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language/).
