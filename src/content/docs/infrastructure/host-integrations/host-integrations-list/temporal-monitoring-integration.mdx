...
1. To configure temporal metrics, include the following file in the project's root directory.
...
```
2. MetricsUtils is a utility file with the scrape endpoint configuration.
...
```
3. Add the blow dependency in build.gradle under the dependency section.

```java
implementation "io.micrometer:micrometer-registry-prometheus"
```
4. Update build.gradle with blow task  to start the worker.

```java
task startMetricsWorker(type: JavaExec) {
   mainClass = 'ProjectRoot.MetricsWorker'
