...
If Memcached is <DoNotTranslate>**not**</DoNotTranslate> running on Kubernetes or Amazon ECS, you must [install the infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic) on a host that's running Memcached. Otherwise:
