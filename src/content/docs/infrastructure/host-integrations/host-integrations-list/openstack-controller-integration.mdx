...
3. Rename the file `config/os-config.json.template` to `config/os-config.json` and edit the file as mentioned below
    ...
    * `nr_agent_home`. you need to enter the parent directory of the **config/** folder from  your cloned repository.
    * `keystone_url`. Run the below command and display the all Endpoints with name. Copy and use the  keystone endpoint for keystone_url.
      ```shell
      openstack catalog list
      ```
    * `keystone_api_version` and `neutron_api_version`. Use the below command to find the api version.
      ```shell
      curl -i <endpoint URL>
      ```
    * `admin user password`. you need to enter the openstack login password.
...
<CollapserGroup>
    ...
    <Collapser
    id=""file8"
    title="File 8 create the openstack.nova.yml file"
    >
    1. Create a file named `openstack.nova.yml` in this path.
    ```shell
    /etc/newrelic-infra/integrations.d
