...
To install the StatsD integration, run the following command and include your [New Relic account ID](/docs/accounts/install-new-relic/account-setup/account-id) and [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). This generates a TOML configuration file used by `gostatsd`.
