...
<CollapserGroup>
  ...
  >
    ...
    <DNT>**Disable timer sub-metrics:**</DNT>
  ...
  >
    The best approach to configure `nri-statsd` running in Kubernetes is to use a `configMap` and mount the `configMap` into the container. (This is a similar process to mounting the configuration file in <DNT>Docker/DNT>.)
