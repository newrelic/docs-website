        ...
        config:
          ...
          apis:
            - event_type: temporalCloudFrontendServiceError
              url: https://<your_account>.tmprl.cloud/prometheus/api/v1/query?query=
...
Here is a NRQL query to check the Temporal cloud frontend service requests count:
    ...
    ```sql
    FROM temporalCloudFrontendServiceRequest SELECT uniqueCount(valueSamples) FACET metric.temporal_namespace WHERE `metric.__name__` LIKE 'temporal_cloud_v0_frontend_service_request_count'
    ```
    </Step>
</Steps>
