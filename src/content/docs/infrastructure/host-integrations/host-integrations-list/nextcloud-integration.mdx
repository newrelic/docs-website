...
1. Create a file named `nri-flex-nextcloud.yml` in this path:

  ```shell
  cd /etc/newrelic-infra/integrations.d
  ```
2. Use our [configuration template](https://github.com/newrelic/nri-flex/blob/master/examples/json-read-cmd-example.yml) to update the fields `EVENT_TYPE` and `YOUR_DOMAIN` in the created file named `nri-flex-nextcloud.yml`.
  The value on the `event_type` is used to store metrics on the NRDB.

  Example:
    * `EVENT_TYPE` can be updated to `NextcloudFlexMetrics`
...
You can use our [log forwarding](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/) to forward Nextcloud logs to New Relic.
...
1. Before you can start reading your data, use the instructions in our [infrastructure agent docs](/docs/infrastructure/install-infrastructure-agent/manage-your-agent/start-stop-restart-infrastructure-agent/) to restart your infrastructure agent.
