...
Sendmail is a server application that gives businesses a way to send email using the Simple Mail Transfer Protocol (SMTP). Using New Relic infrastructure agent and NRI-Flex configuration, you can quickly instrument and monitor your server.
...
<figcaption>
After setting up the Sendmail integration with New Relic, your data displays in dashboards like these right out of the box. 
</figcaption>


## Install the infrastructure agent [#infra]

To get data into New Relic, install our infrastructure agent. Our infrastructure agent collects and ingests data so you can keep track of your mail server’s performance.
...
Follow below steps to create a flex configuration file:
...
2. Update the file `sendmail-flex-config.yml` with these configurations:
...
You can choose our pre-built dashboard template named `Sendmail` to monitor your Sendmail metrics. 
...
3.  In the search box, type `Sendmail`.
...
To instrument the Sendmail quickstart and to see metrics and alerts, you can also follow our [Sendmail quickstart page](https://newrelic.com/instant-observability/sendmail) that has an **Install now** button.

 Here are some example queries:

**Example:** View the latest message sent status.

```sql
SELECT latest(sent) as 'Sent', latest(deferred) as ‘Deferred’ FROM SendmailMessageStatus
```

**Example:** View the Sendmail service status.

```sql
SELECT latest(value) as 'Service Up' FROM SendmailUp TIMESERIES AUTO
```
...
## See Sendmail metrics in a dashboard [#dash]
...
To instrument the Sendmail quickstart, install our default dashboards, and to see metrics and alerts, you can also follow our [Sendmail quickstart page](https://newrelic.com/instant-observability/sendmail) by clicking on the **Install now** button.
