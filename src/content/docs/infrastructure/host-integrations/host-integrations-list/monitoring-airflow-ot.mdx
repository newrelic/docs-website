...
### Option 2: Installing from <DNT>Docker/DNT> [#install-docker]

1. Set up the Airflow <DNT>Docker/DNT> image using instructions from [Airflow's documentation](https://airflow.apache.org/docs/docker-stack/index.html).
2. Extend the pre-built <DNT>Docker/DNT> image by using a <DNT>Docker/DNT>file to install the `otel` extra. You can replace the latest tag with your desired version of the image.
