...
Monitor Apache Mesos clusters seamlessly with New Relic for comprehensive insights into performance, health, and resource utilization. Track master and slave nodes, monitor task execution and system metrics.
...
1. Edit the log file named `logging.yml` located at the following path:
  ...
  ```
2. Add the following script to the `logging.yml` file. Replace `file` with your Apache Mesos log filepath if needed:
...
You can choose our pre-built dashboard template named `Apache Mesos` to monitor your Apache Mesos application metrics. Follow these steps to use our pre-built dashboard template:
...
Your Apache Mesos dashboard is considered a custom dashboard and can be found in the <DoNotTranslate>**Dashboards**</DoNotTranslate> UI. For docs on using and editing dashboards, see [our dashboard docs](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards).

Here is a NRQL query to check the Apache Mesos master uptime:
