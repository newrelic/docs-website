...
1. Creates files named `nri-flex-apache-traffic-server-config.yml`, `nri-flex-ats-cache-config.yml`, and `nri-flex-ats-http-config.yml` in this path:
        ...
        apis:
          - event_type: ATSSampleMetrics
            ...
            remove_keys:
              ...
              - cache
  ```
  Your `nri-flex-ats-cache-config.yml` file should look like this:
  ```yml
  integrations:
    - name: nri-flex
      interval: 30s
        ...
        apis:
          - event_type: ATSCacheSampleMetrics
            ...
            keep_keys:
            	- cache
  ```
  Your `nri-flex-ats-http-config.yml` file should look like this:
