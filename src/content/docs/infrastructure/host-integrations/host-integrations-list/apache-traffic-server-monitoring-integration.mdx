...
Goto the `plugin.config` file.
  ```shell
  sudo nano /etc/trafficserver/plugin.config
  ```
Add the following line `stats_over_http.so` in your `plugin.config` file.

Make sure your metrics are exposed on the below URLs.
Metrics of name node: `http://YOUR_IP:8080/_stats`
...
1. Creates files with  named `nri-flex-apache-traffic-server-config.yml`, `nri-flex-ats-cache-config.yml` and `nri-flex-ats-http-config.yml` in this path:

  ```shell
  cd /etc/newrelic-infra/integrations.d
  ```
2. Your `nri-flex-apache-traffic-server-config.yml`, `nri-flex-ats-cache-config.yml` and `nri-flex-ats-http-config.yml` file should look like this:

  The value on the `event_type` is used to store metrics on the NRDB.

  File: `nri-flex-apache-traffic-server-config.yml`
        ...
        apis:
          - event_type: ATSSampleMetrics
            ...
            remove_keys:
              ...
              - cache
  ```
  File: `nri-flex-ats-cache-config.yml` 
  ```yml
  integrations:
    - name: nri-flex
      interval: 30s
        ...
        apis:
          - event_type: ATSCacheSampleMetrics
            ...
            keep_keys:
            	- cache
  ```
  File: `nri-flex-ats-http-config.yml`
