...
1. Create a file named `lighttpd-http-config.yml` on path `/etc/newrelic-infra/integrations.d`
  <Callout variant="tip">
    A generic file name could be &ltFILENAME&gt-config.yml
  </Callout>
...
2. Using this [sample configuration file](https://github.com/newrelic/nri-flex/blob/master/examples/lighttpd-http-example.yml) as a guide, modify your `lighttpd-http-config.yml` file:

  Before you proceed with the nri-flex configuration, keep the following in mind:
  ...
  * `event_type` is also adjustable. The value in `event_type` can be used to store metrics in the [New Relic platform](https://one.newrelic.com).
