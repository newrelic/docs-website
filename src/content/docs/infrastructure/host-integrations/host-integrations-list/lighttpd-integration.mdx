...
It's important to note the below points to proceed with the nri-flex configuration.
 - Can replace `base_url` and `url` with your application url. Refer to [nri-flex url documentation](https://github.com/newrelic/nri-flex/blob/master/docs/apis/url.md) for further understanding.
 ...
 - `event_type` is also adjustable. The value in `event_type` can be used to store metrics on one.newrelic.com platform
