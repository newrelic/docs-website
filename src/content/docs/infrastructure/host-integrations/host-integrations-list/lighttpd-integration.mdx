...
First, follow the [installation steps](https://github.com/newrelic/nri-flex/blob/master/docs/basic-tutorial.md) of nri-flex from the [nri-flex repository](https://github.com/newrelic/nri-flex)

Once you've installed the infrastructure agent on your host, the nri-flex binary is also installed.
...
2. Using this [sample configuration file](https://github.com/newrelic/nri-flex/blob/master/examples/lighttpd-http-example.yml) as a guide, modify your `lighttpd-http-config.yml` file. Keep the following in mind:

    * You can replace `base_url` and `url` with your application url. Refer to [nri-flex url documentation](https://github.com/newrelic/nri-flex/blob/master/docs/apis/url.md) for further details.
    * You can also convert the metrics format which you get through its URL by using regular expressions in `regex_matches` > `expression`.
    * The `event_type` is also adjustable. The value in `event_type` can be used to store metrics in the [New Relic platform](https://one.newrelic.com).
...
[Follow the instructions to restart the infrastructure agent](/docs/infrastructure/install-infrastructure-agent/manage-your-agent/start-stop-restart-infrastructure-agent/):
