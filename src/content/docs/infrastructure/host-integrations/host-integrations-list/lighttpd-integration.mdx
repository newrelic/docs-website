...
It's important to not that you can replace `base_url` and `url` with your application url (Please refer to [nri-flex url documentation](https://github.com/newrelic/nri-flex/blob/master/docs/apis/url.md) for more understanding) and also change url in the `commands` attribute. You can also convert the metrics format which you get through url into regex and add expressions in regex_matches. EVENT_TYPE is also adjustable. 
