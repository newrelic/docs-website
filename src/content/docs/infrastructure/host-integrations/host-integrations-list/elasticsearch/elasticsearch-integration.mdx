...
The integration requires an Elasticsearch user with `monitor` privileges in the cluster and all indices. See [elastic's authorization documentation for details](https://www.elastic.co/guide/en/elasticsearch/reference/current/authorization.html).
