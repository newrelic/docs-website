...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      ...
      If you are using agent version 1.20.0 or above you can use small caps as the agent will automatically [uppercase them](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-1200#changed).
    </Callout>
...
To update an older integration configuration to the new format there 2 possible methods:

### Assisted method

Using [Newrelic CLI](https://developer.newrelic.com/automate-workflows/get-started-new-relic-cli/), run the following command to automatically convert your old definition/config files to the new configuration format:
...
You can also convert the integration file manually with these two steps:
