...
<CollapserGroup>
  ...
  >
    ...
    * <DNT>**If the `exec` property is set**</DNT>: The `name` property only provides an identifier for the integration instance. This identifier is used in log messages and to provide a default [inventory category/source](/docs/infrastructure/infrastructure-ui-pages/infra-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) in the form `integration/<name>` (for example, `integration/nri-redis`). This inventory path can be overridden with the `inventory_source` configuration option.
    * <DNT>**If the `exec` property is not set**</DNT>: The agent looks for (and executes) an executable with the `name` value in any of the following folders:
