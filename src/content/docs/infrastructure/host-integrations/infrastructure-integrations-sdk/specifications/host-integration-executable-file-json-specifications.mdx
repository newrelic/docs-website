...
A New Relic [core on-host integration](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) will consist of at least two files: an executable file and at least one configuration file. The executable file generates JSON data that is consumed by the infrastructure monitoring agent and sent to New Relic. We refer to the JSON object as the SDK integration protocol.
