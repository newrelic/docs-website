...
You can load configurations from custom commands or 3rd-party tools / cli.
The Infrastructure Agent will detect when configurations should initially be loaded from a `command` output and can optionally re-load the configurations based on the configured `ttl`.
...
<Callout variant="important">
  ...
  Make sure the Infrastructure Agent running on the host has the required access and permissions to execute the configured `command` based on your environment requirements.
</Callout>

### JSON
The following JSON structure is expected as the main output (stdout) from the defined `command` configuration: 
```
{
  "data": {
    "key": "value"
...
If the output is JSON, then it can be used in any configuration file using the `${myVariable.key}` notation.
...
# the `token` result can be included as ${myToken.token} on any configuration.
