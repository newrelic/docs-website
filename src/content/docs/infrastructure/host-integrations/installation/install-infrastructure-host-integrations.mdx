...
To collect data from on-host services, all you need are two things: the [infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent), and the [on-host integrations](/docs/integrations/host-integrations/getting-started/introduction-host-integrations). All on-host integrations require that our <InlinePopover type="infrastructure" /> agent is [installed on the host](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent). Besides collecting system's data, the agent acts as a forwarder for integrations's data, and can forward [logs](/docs/logs/enable-log-monitoring-new-relic/enable-log-monitoring-new-relic/forward-your-logs-using-infrastructure-agent).
