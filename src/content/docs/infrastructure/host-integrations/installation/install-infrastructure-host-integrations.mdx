...
Some of our infrastructure integrations work in tandem with our infrastructure agent. This includes MySQL, Apache, Redis, and others. 

## Pick your install method [#install]

Your services may be running on a single physical host, on a VM, or in a container orchestrated by Kubernetes or ECS. Choose the appropriate installation method.

### Services in orchestrated environments [#orchestrated]
...
### Services running on VMs or on-premises hosts [#standard]
...
For the integrations that work in tandem with the infrastructure agent, the agent acts as a forwarder for that data.
