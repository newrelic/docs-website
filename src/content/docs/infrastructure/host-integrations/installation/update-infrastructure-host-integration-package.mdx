...
## Update using `msiexec.exe` (Microsoft Windows) [#windows]

Here are two approaches, depending on the integration you're using.

### Microsoft Windows services integration [#ms-services]

The Microsoft Windows services integration is bundled with the New Relic infrastructure agent, so to upgrade the integration, you need to upgrade the infrastructure agent. See [Update the infrastructure agent](/docs/infrastructure/install-infrastructure-agent/update-or-uninstall/update-infrastructure-agent).

### Non-bundled Microsoft on-host integrations [#non-bundled]

For Microsoft on-host integrations that you installed separately from the infrastructure agent, use the following:

1. Open PowerShell and run this command, replacing `INTEGRATION_FILE_NAME` with the `.msi` integration filename.

```
msiexec.exe msiexec.exe /qn /i PATH_TO\<var>INTEGRATION_FILE_NAME</var>
```
