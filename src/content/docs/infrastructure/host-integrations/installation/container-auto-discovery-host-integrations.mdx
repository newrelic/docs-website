...
<Callout variant="tip">
  Currently, the [infrastructure agent](/docs/infrastructure/install-configure-manage-infrastructure) can discover local <DNT>Docker/DNT> containers.
</Callout>
...
These examples (for <DNT>Docker/DNT>-only environments and for Kubernetes) show how to configure an [NGINX integration](/docs/infrastructure/host-integrations/host-integrations-list/nginx/nginx-integration) to monitor all local containers that have an image name containing `nginx`, and that are labeled as `env=production`. Each service will be available through different IPs and ports, so they must be variables.
...
<CollapserGroup>
  ...
  >
    This config applies to non-orchestrated scenarios, where <DNT>Docker/DNT> is used without Kubernetes or other orchestration environments.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <DNT>Docker/DNT> containers matching definition. This consists of a `match` property containing a map where:
...
The following example matches all <DNT>Docker/DNT> containers whose image starts with `httpd:` (provided as a regular expression) and are labeled as `scrapable=yes`. The results are cached with a Time-To-Live of 5 minutes.
