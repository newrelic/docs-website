...
In addition to the indexing from the previous section, ports can be also referred by its protocol name (such as `tcp`, `udp`, etc). For example, `${discovery.ports.tcp}`, `${discovery.ports.udp}`.
