...
For our infrastructure monitoring UI, the **Hosts** UI page is the main place for you to understand and troubleshoot issues with your infrastructure.    

<Callout variant="tip">
  These docs describe the new **Hosts** UI added in June of 2023. In the UI, you can switch between the new view and the **Classic** view. The classic **Hosts** UI will reach end-of-life in September of 2023.
</Callout>
...
4. You go to the **Inventory** tab and see that the issue happened right after someone updated a database service. 
...
On the **Hosts** page, the **Overview** tab gives you an overview of your hosts' performance. 

<img
  title="Hosts Overview page"
  alt="Hosts Overview page"
...
The tiles at the top of this page give you an overview of your hosts, applications, events, and alerts. Use these tiles to see more detail about a host or filter the entire **Overview** page to a specific host.

Unlike the other **Hosts** tabs, the **Overview** page only shows you the last 80 minutes of data. To explore further back, use the other tabs on this page, or [query your data](/docs/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data).  
</Collapser>
    ...
    title="System UI"
    >
    The **System** tab gives you an overview of your system.  [need more info on that...??]
...
Here are the default charts shown on the **Hosts** page. For additional chart options, select a chart's dropdown <Icon name="fe-chevron-down"/> menu.

* **CPU %**: On the **System** tab, this is a derived metric that's part of the `SystemSample` event. We don't collect the CPU percentage: it's derived from several other metrics. Specifically, the [`cpuPercent`](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#systemsample-cpu-percent) attribute is an aggregation of `cpuUserPercent`, `cpuSystemPercent`, `cpuIoWaitPercent` and `cpuStealPercent`.
...
For details about the data used to generate these charts, see [NetworkSample](attribute-dictionary/?event=SystemSample).
...
Use the **Processes** tab to get information about processes running on your hosts, and to set alerts on process metrics. The **Processes** tab shows data such as CPU percentage, I/O bytes, and memory usage for individual processes or groupings of processes.
    ...
    <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Hosts > Process**
    </figcaption>
...
For details about the data used to generate these charts, see [NetworkSample](/attribute-dictionary/?event=ProcessSample).
...
For details about the data used to generate these charts, see [NetworkSample](/attribute-dictionary/?event=StorageSample).
...
If you want to know more about infrastructure data, see [Infrastructure data](/docs/infrastructure/manage-your-data/data-instrumentation/default-infrastructure-monitoring-data/#infrastructure-events).
