...
New Relic can collect detailed information about a system's configuration per host, including system modules, configuration files, metadata, packages, services, user sessions, and more. The <DoNotTranslate>**Inventory**</DoNotTranslate> page provides a real-time, filterable, searchable view into each host's configuration.

Use the <DoNotTranslate>**Inventory**</DoNotTranslate> page to:
...
To view and search your inventory data: Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Inventory**</DoNotTranslate>.
...
Use <DoNotTranslate>**Inventory**</DoNotTranslate> page functions to find information about a particular item on your hosts:
...
<CollapserGroup>
  ...
  >
    Inventory item details provide host and system information for each host it resides on according to the New Relic [inventory item name](#item-naming). If you have different versions of the same item on other hosts, New Relic detects that and flags them on the Inventory page with the <DoNotTranslate>**variant hosts**</DoNotTranslate> label and lists each host running each version.
  ...
  >
    To view one or more host's [alert threshold incidents](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-threshold), select the host's <DoNotTranslate>**Critical**</DoNotTranslate> <Icon name="fe-x-circle"/>
    icon or <DoNotTranslate>**Warning**</DoNotTranslate> <Icon name="fe-alert-triangle"/>
    icon.
  </Collapser>
</CollapserGroup>
...
For a technical explanation about attributes used to populate the <DoNotTranslate>**Inventory**</DoNotTranslate> page, see [Default infrastructure attributes and events](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#shared-attributes). This includes a summary of [common events by operating system](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#events-os).


## Filter sets [#filter-sets]

The filter set UI component is used for filtering your hosts and creating saved views. <DoNotTranslate>**It is only available on the Events and Inventory UI pages and is in the process of being deprecated.**</DoNotTranslate> It has been replaced by the [saved views](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#ui-features) feature. Any filter sets you create from this point onward will not be migrated to the saved views feature. 
...
The default infrastructure filter set is <DoNotTranslate>**All hosts**</DoNotTranslate>, and it serves as a template for you to create filter sets.

To create a filter set:

1. Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**</DoNotTranslate> and click <DoNotTranslate>**Inventory**</DoNotTranslate> or <DoNotTranslate>**Events**</DoNotTranslate>.
2. If <DoNotTranslate>**All hosts**</DoNotTranslate> is not displayed in the left sidebar, open that filter set by selecting <DoNotTranslate>**Saved filter sets > All hosts**</DoNotTranslate>.
3. In <DoNotTranslate>**All hosts**</DoNotTranslate>, click <DoNotTranslate>**Filter hosts**</DoNotTranslate>.
4. In the list, click an item to see a list of values.
5. Click <DoNotTranslate>**Include**</DoNotTranslate> or <DoNotTranslate>**Exclude**</DoNotTranslate> (see [Filter set logic](#logic)).
...
8. To name your filter, click the <Icon name="fe-edit-2"/>
   icon, type a name, and click <DoNotTranslate>**Save**</DoNotTranslate>.

### Edit filter sets [#edit]

To change an existing filter set:

1. In the sidebar, click <DoNotTranslate>**Saved filter sets**</DoNotTranslate> to open a list.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            If you click <DoNotTranslate>**Include**</DoNotTranslate> and then enter a string that you want values to match, the filter uses the comparator `LIKE`, which means the results include any entities that are like the string.
        ...
        <tr>
          ...
          <td>
            If you click <DoNotTranslate>**Exclude**</DoNotTranslate> and then enter a string that you want values to match, the filter uses the comparator `NOT LIKE`, which means the results exclude any entities that are like the string.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            If you click <DoNotTranslate>**Include**</DoNotTranslate> and then click specific values, the filter uses the comparator `IN`, which means the filter looks for entities that exactly match one or more values in your list of selections.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            If you click <DoNotTranslate>**Exclude**</DoNotTranslate> and then click specific values, the filter set uses the comparator `NOT IN`, which means the filter returns all entities that do not exactly match one or more values in your list of selections.
          </td>
        </tr>
      </tbody>
    </table>
