...
metaDescription: "Autoscale your infrastructure with Horizontal Pod Autoscaling so you never have to question ."
---
...
One solution is Horizontal Pod Autoscaling (HPA). With HPA, your Kubernetes cluster automatically allocates more pods when demand peaks, then deallocates when demand falls. This allows you to sidestep cost concerns when anticipating demand.
...
This tutorial walks you through setting up a demo environment for Horizontal Pod Autoscaling (HPA). You will:
...
You should have two terminals that includes: 
...
* A place to run commands for the tutorial.
    ...
    <Step>
        ...
        Here's some additional context on the above flags:
        ...
        * `newrelic-k8s-metrics-adapter.accountID`: The New Relic account ID that's forwarding metrics.
...
If set up correctly, the pod autoscales the number of replicas as the average HTTP request time reported by New Relic increases. You can adjust the configuration for your own services so that New Relic and HPA automatically autoscale as needed.
