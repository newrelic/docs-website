...
Let's follow this short example of a user journey:
...
How many capabilities does this user touch? They navigate to a page, select an item, and adjust the quantity of that item, updating the cart total. Each of these actions corresponds to a potential service level, which can be monitored on a peak demand day. 
...
If you only just started reporting data to New Relic, then you won't have enough historical data to query for a baseline. We recommend you create your first set of service levels by copy/pasting some queries from our [introductory service level tutorial](/docs/journey-performance/slo-tutorial/).
...
When you add a new service level, New Relic will auto-populate a baseline for every data source in your entire app. While useful in some cases, we're wanting to monitor specific capabilities that could have catastrophic consequences on a peak demand day. 
...
<DocTiles numbered>
    ...
    <DocTile title='Create service levels for gameday' path="/docs/journey-demand/find-your-baseline/" label={{text: "You are here", color: "orange"}}>Create baseline-informed service levels </DocTile>
</DocTiles>
