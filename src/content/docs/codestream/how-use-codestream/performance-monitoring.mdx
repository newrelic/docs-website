...
<img
  ...
  alt="A screenshot of a deff with golden metrics."
  src={codeLevelDiffs}
/>

### Requirements [#clm-requirements]

* [Distributed tracing](/docs/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing) must be enabled. Note that is is on by default in recent versions of all agents, so you only need to worry about this if you're disabled distributed tracing.
* **Java:** Requires Java agent version 7.10 or higher. Code-level metrics must be [enabled in the Java agent configuration](/docs/apm/agents/java-agent/features/java-codestream-integration/). If you're using VS Code, you must also have the [Language Support for Java by Red Hat](https://marketplace.visualstudio.com/items?itemName=redhat.java) extension installed.
