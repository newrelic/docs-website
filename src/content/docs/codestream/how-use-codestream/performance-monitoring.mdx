...
* **Go:** Requires Go agent version 3.18.0 or higher. To get started, you just need to add `newrelic.ConfigCodeLevelMetricsEnabled(true)` to the application's configuration. For more options, see [Go agent code-level metrics instrumentation](/docs/apm/agents/go-agent/instrumentation/go-agent-code-level-metrics-instrument) and [Go agent code-level metrics configuration](/docs/apm/agents/go-agent/configuration/go-agent-code-level-metrics-config).
