...
Code-level metrics are only available for Python code. These metrics are shown via CodeLens text, a Visual Studio Code feature.

Additionally, your Python application code needs to be monitored by the Python APM agent and your repository associated with a New Relic service.
</Callout>

![A screenshot of a Python method with average duration, throughput, and error rate telemetry.](./images/code-level-metrics “A Python method with average duration, throughput, and error rate metrics.”)

<figcaption>When relevant, CodeStream's code-level metrics are shown in a CodeLens above the relevant methods.</figcaption>

Click the CodeLens text to see related charts.

![A screenshot of an expanded code-level metrics CodeLens that shows related charts.](./images/code-level-charts "An expanded code-level metrics CodeLens that shows related charts.")
...
If you've enabled CodeStream and are monitoring entities in New Relic, but you haven't associated your current repository with New Relic, Visual Studio Code shows a CodeLens at the top of your open file.
