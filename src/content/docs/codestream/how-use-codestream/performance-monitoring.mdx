...
**Connect**. If you signed up for CodeStream using your New Relic user
key then you'll already be connected.
...
needs to know what entities on New Relic are tied to the repositories you have
open, and there are a number of ways to make these associations.

There are instances where CodeStream wil prompt you to make an association if
one doesn't example. For example, if you try view an error from New Relic in
...
The Observability section of the CodeStream pane allows developers to discover
errors related to code that they're responsible for right from their IDE. In
addition to errors assigned to you, recent errors will be listed for reach
repository you have open in your IDE (assuming you've associated it with an
entity on New Relic).
