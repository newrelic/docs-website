...
Specifically, when CodeStream is connected to New Relic, you'll be able to jump from a stack trace error in the New Relic UI directly to the offending line of code in your IDE. Once in your IDE, you'll be able to use all of CodeStream's collaboration tools to resolve the issue and push up the fix to your repository.
...
Before you can take advantage of New Relic's observability in features in CodeStream, you'll need to connect them.
...
New Relic users can share stacktrace errors on CodeStream. Once you've connected CodeStream to New Relic, any new users you add to your CodeStream organization will be able to see those errors.
</Callout>

## See your errors and what's causing them [#errors]

Once you've connected CodeStream and New Relic, use workloads and errors inbox to jump to the offending code in your IDE.

Requirements for opening stack trace errors in your IDE:
* [New Relic account](https://newrelic.com/signup) (If you don't have a New Relic account, sign up at [newrelic.com/signup](https://newrelic.com/signup). It's free, forever!)
* [New Relic user key](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher)
* [CodeStream and New Relic connection](#connect)
* [Data being reported to New Relic via APM, browser, or mobile monitoring](https://one.newrelic.com/launcher/nr1-core.explorer?pane=eyJuZXJkbGV0SWQiOiJucjEtY29yZS5saXN0aW5nIn0=&cards[0]=eyJuZXJkbGV0SWQiOiJucjEtaW5zdGFsbC1uZXdyZWxpYy5ucjEtaW5zdGFsbC1uZXdyZWxpYyIsImFjdGl2ZUNvbXBvbmVudCI6IlZUU09FbnZpcm9ubWVudCIsInBhdGgiOiJndWlkZWQifQ==)
...
* [For browser errors, your repository url and commit hash](#browser)
* [For Android mobile errors, Git tags that match your appVersion](#android)
...
To use CodeStream's **Open in IDE** with your APM stack trace errors, configure your APM agent with your application's [commit sha](https://git-scm.com/book/en/v2/Git-Tools-Revision-Selection)
...
## Browser errors and CodeStream [#browser]

To use CodeStream's **Open in IDE** with your browser errors, update your browser source map with your repository URL and commit sha.

There are several ways to [upload or update source maps via our API](/docs/browser/new-relic-browser/browser-pro-features/upload-source-maps-api).

If you don't have a minified app with a source map, you can still add a commit sha via the [addRelease API in the browser agent](/docs/browser/new-relic-browser/browser-agent-spa-api/add-release). The `release_name` must match your file name.

## Android mobile errors and CodeStream [#android]

To use CodeStream's **Open in IDE** with your Android mobile errors, create Git tags that match your `appVersion`. The mobile agent telemetry is linked to appVersion and this ensures you're seeing the correct version of the code that caused the error.
...
<Callout variant="caution">
It's possible to add the same GitHub repository more than once, if you're different protocols. The UI warns you about this, but won't prevent you from doing so.
