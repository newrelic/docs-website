...
With CodeStream connected to New Relic, you can jump from a stack trace error directly to the offending line of code in your IDE. Once in your IDE, you can navigate the stack trace and [collaborate with your teammates to resolve the issue](/docs/codestream/how-use-codestream/performance-monitoring).

<Callout title="Preview release">
  CodeStream's integration with the New Relic platform is a preview release limited to New Relic accounts on our US data center, and your use is subject to the pre-release policy. (This does not apply to all other CodeStream functionality.)
</Callout>
...
* [New Relic user key](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher). For more information, see [API keys](/docs/apis/intro-apis/new-relic-api-keys/#user-api-key).
* You must have a user type of [core user or full platform user](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type#user-type-capabilities).

Once you have your New Relic user key, in CodeStream's **Observability** section click **Connect to New Relic**, then paste your API key and click **Connect to New Relic**.
...
## Dynamic logging with Go and Pixie [#dynamic-logs]
