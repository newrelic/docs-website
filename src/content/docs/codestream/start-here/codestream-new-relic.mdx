...
<CollapserGroup>
  ...
  <Collapser
    ...
    to the APM Summary page for the appropriate app (go to
    **[one.newrelic.com](https://one.newrelic.com) > Explorer > Services - APM >
    (select an app)**) and look for the Repositories section at the
    bottom-right.

     ![A screenshot of the connect repository button.](./images/connect-repo1.png "The connect repository button.")

     Click **Connnect repository** to find an existing repository or add a new one.
  </Collapser>
    <Collapser
    className="freq-link"
    id="nerdgraph"
