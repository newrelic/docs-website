...
CodeStream will show you errors assigned to you, as well as the five most recent errors happening in the code you have open in your IDE. You can control the timeframe via the dropdown list in the heading of this section. Note that this list may be a subset of the errors shown in the [errors inbox](/docs/errors-inbox/errors-inbox/) on New Relic. 
...
Click any frame in the stack trace to jump straight to the corresponding file and line number. If you are [associating git references with your errors](#buildsha), CodeStream will open up the version of the file that is running in the environment that triggered the error, and it will be opened in a read-only editor tab. Otherwise your local version of the file will be opened. Note that frames of the stack trace that represent code that isn't part of your repository (e.g., an external library) aren't clickable.
...
You can jump into this same errors experience directly from an error in the [errors inbox](/docs/errors-inbox/errors-inbox/) on New Relic. When viewing an error with a stack trace, click **Open in IDE** to go directly to the code in your IDE.
...
CodeStream uses the git reference to match the specific stack trace error with the version of the code running in the environment that triggered the error. While you don't need a git reference configured to investigate the error, bear in mind that you may not be looking at the version of the code that caused it.
