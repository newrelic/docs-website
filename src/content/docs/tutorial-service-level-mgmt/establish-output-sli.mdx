...
You must also establish a baseline to implementat service levels. Creating a baseline allows you to measure the current performance of a service, then use service level reports to find out if you're hitting that baseline or degrading. You can create a baseline for almost any dataset; however, you use different formulas and recommendations in different use cases.

When you create service levels, you should start with the output performance of your applications. You can use response times (latency) and percentage of non-errors (success). When you do this, you create a reliability health metric. Seasonality and peak usage doesn't handicap your performance, and the more history you include in your measurement, the more likely you're including different codebases from releases. Previous deployments, no matter how small, could skew your results.

We recommend a history of one to two weeks of performance data to establish a good baseline. Here's an example NRQL query for the recommended target of a seven day service level objective for latency:
