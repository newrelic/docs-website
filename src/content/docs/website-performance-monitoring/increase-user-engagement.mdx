...
* <DoNotTranslate>**First input delay**</DoNotTranslate>. When a user interacts with an element on your site, it takes time to process the user's interaction This number captures that time as a delay between the request (user interaction) and response (how the page changes based on the user request).
