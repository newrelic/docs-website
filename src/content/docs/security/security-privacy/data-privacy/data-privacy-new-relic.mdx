...
To surface user activity information, go to **[one.newrelic.com](https://one.newrelic.com) > All Capabilities > Metrics & Events**, then search for either `NrAuditEvent` or `NrdbQuery` from the **Event type** column. You can click the **Dimensions** dropdown to view all attributes associated with either event. For example:

<img
    title="Find user activity data in Metrics & Events"
    alt="A screenshow that shows how to find events and attributes for user activity data"
...
You can select **Raw data** to view event attributes as key-value pairs. Attribute keys like ID, description, timestamp are organized in the table's horizontal row while their value pairs are listed out in the table's columns. These attributes are subject to [data limits](/docs/data-apis/manage-data/view-system-limits). 
...
You can also use our **Query your data** tool, anchored at the bottom of the platform in your browser, to retrieve attributes and fields of interest. For example, take this example query:
...
This query surfaces data from the `NrdbQuery` event but limits the data to:
     ...
     * But scoped to this time parameter: `since 1 week ago until 1 day ago`

This surfaces data to specific parameters rather than showing broad trends about the `NrdbQuery` event like in Metrics & Events. 
