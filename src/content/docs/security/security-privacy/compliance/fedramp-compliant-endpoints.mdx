...
To ensure FedRAMP compliance when using the OpenTelemetry API, only send data to the following endpoints based on the data type and protocol:
    ...
    Due to technical limitations in the OTLP Metrics integration, we are advising FedRAMP customers to use the HTTP protocol for all OTLP traffic. However OTLP Traces and Logs do support gRPC if your application does not support HTTP. You can reference protocol support for all OTLP integrations in the table above.

    <Callout variant="important">
      Do NOT send data to the US OTLP API endpoint of `https://otlp.nr-data.net:4317`.
    </Callout>

See our [OTLP integration support matrix](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-setup#review-settings) for more details on FedRAMP compliance with OTLP.
