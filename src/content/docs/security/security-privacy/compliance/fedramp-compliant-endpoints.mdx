...
To ensure FedRAMP compliance when using the [OTLP API](/docs/integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-quick-start#review-settings), instead of sending to the US OTLP API endpoint of  `https://otlp.nr-data.net:4317`, it must be sent to `https://gov-otlp.nr-data.net:4317`.
