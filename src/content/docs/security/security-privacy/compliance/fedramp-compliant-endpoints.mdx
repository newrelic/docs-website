...
* Prometheus: 
  * [Prometheus OpenMetrics](/docs/integrations/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics): configure `metric_api_url` via environment variable to the new endpoint.
...
FedRAMP is not yet available for the Trace API itself, but note that some of our agents report trace data and you can [enable FedRAMP for some of those agents](#agents). 