...
4. **Authorized services and features**: Customer must use only FedRAMP audited and authorized New Relic [services and features](/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp/#not-scope).
...
* [Agent settings](#agents) for our [<InlinePopover type="apm"/> agents](#apm-endpoints), [infrastructure agent](#infrastructure-endpoints), [<InlinePopover type="browser"/> agent](#browser-endpoints), and [mobile monitoring agent](#mobile-endpoints)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        In your XML config next to the <InlinePopover type="licenseKey"/>:
...
This also enables FedRAMP compliancy for any [on-host integrations](/docs/integrations/host-integrations/host-integrations-list) that work with the infrastructure agent to report data.
...
To ensure FedRAMP compliance when using the OpenTelemetry API, only send data to `https://gov-otlp.nr-data.net` with the appropriate port chosen from the [OTLP integration support matrix](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol). See that page for more details on FedRAMP compliance with OTLP.
