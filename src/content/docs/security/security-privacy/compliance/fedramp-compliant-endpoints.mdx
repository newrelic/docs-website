...
3. **Authorized New Relic endpoints**: Customer must send their data only to New Relic’s FedRAMP-designated endpoints.
...
Browser agents will automatically update your account information to use the correct endpoints. Add `https://gov-bam.nr-data.net` to ensure FedRAMP compliance.
...
* Infrastructure agent: See [FedRAMP config settings for infrastructure](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#fedramp).
...
To use the Log API directly, edit the [Log API endpoint configuration](/docs/logs/log-management/log-api/introduction-log-api#endpoint).
