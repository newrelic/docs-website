...
2. **Subscription level**: Customer must have a current and valid subscription to our Enterprise edition with the [Data Plus option](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing#data-plus), or have an alternative New Relic-approved subscription.

3. **Authorized New Relic endpoints**: Customer must send their data only to New Relic’s FedRAMP-designated endpoints (see below).

4. **Authorized services and features**: Customer must use only FedRAMP audited and authorized New Relic services and features (see below).

**FedRAMP Inheritance**
A customer can have FedRAMP and non-FedRAMP enabled accounts as required by their business needs. To add [accounts] (https://docs.newrelic.com/docs/accounts/accounts-billing/account-structure/new-relic-account-structure), talk to your New Relic account representative.

Note: If FedRAMP is enabled on a parent account, the recommended configuration, all its child accounts are FedRAMP. Newly created child accounts are FedRAMP by default. FedRAMP cannot be “removed” from child accounts when the parent is FedRAMP. This option ensures compliance by default.

**FedRAMP-compliant endpoints**
This document provides information on FedRAMP-compliant endpoints in New Relic. For more information about our security accreditation for the Federal Risk and Authorization Management Program (FedRAMP), see our [data encryption] (https://docs.newrelic.com/docs/security/new-relic-security/compliance/data-encryption) documentation. For further information on New Relic networks, domains, and ports see our [networking] (https://docs.newrelic.com/docs/using-new-relic/welcome-new-relic/get-started/networks) documentation.

**IMPORTANT**
If a service's endpoint is not listed in this document and the service is not found in our list of [services not in scope] (https://docs.newrelic.com/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp-moderate), then the service's standard endpoint already meets FedRAMP compliance requirements as-is, without the need for a distinct FedRAMP endpoint.
