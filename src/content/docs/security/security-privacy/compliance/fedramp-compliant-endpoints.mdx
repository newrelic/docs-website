...
* This is not supported for [APM-agent-enabled browser monitoring](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#select-apm-app); you must use the [code-injection method](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#copy-paste-app).

To report FedRAMP-compliant data, you must edit the browser code's script element tag so that the domain is `gov-bam.nr-data.net` for both `beacon` and `error_beacon`, like this:
