...
To ensure FedRAMP compliance for data sent via the [Trace API](/docs/distributed-tracing/trace-api/introduction-trace-api/) or [telemetry integrations](/docs/integrations/open-source-telemetry-integrations/) that use the Trace API, replace the `https://trace-api.newrelic.com/trace/v1` endpoint with `https://gov-trace-api.newrelic.com/trace/v1`. 

<Callout variant="tip">
Note that distributed trace data is also reported by some agents, like our APM agents, browser agent, and mobile agent. To enable FedRAMP compliancy for that data, you would [enable FedRAMP for the applicable agent](#agents).
</Callout>