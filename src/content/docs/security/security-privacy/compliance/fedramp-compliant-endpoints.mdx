...
Federal Risk Authorization Management Program (FedRAMP) enables additional security measures to ensure sensitive data stays safe. You can configure agents and data-ingest APIs to send your data to FedRAMP-compliant endpoints. This doc walks you through that process for:

- [Agents](#agents)
- [Data-ingest APIs](#data-ingest-apis)

For additional information about FedRAMP compliancy: 

- Check out [customer requirements](#obligations) for FedRAMP.
- Visit our [data encryption](/docs/security/new-relic-security/compliance/data-encryption) documentation to learn about FedRAMP accreditation.
- Read our [networking](/docs/using-new-relic/welcome-new-relic/get-started/networks) documentation to learn about networks, domains, and ports.

<Callout variant="important">
Services not listed in either this doc or in our list of [services not in scope](/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp-moderate) already meet FedRAMP compliance requirements. They won't need a distinct FedRAMP endpoint.
</Callout>

## Agents [#agents]

You can configure APM, infrastructure, and mobile agents to send data to FedRAMP-compliant endpoints. Setting these agents to send FedRAMP-compliant data involves configuring certain settings to override default endpoints.

Browser agents will automatically update your account information to use the correct endpoints. No additional configuration is needed.

<CollapserGroup>
  <Collapser
    id="apm-configurations"
    title="APM agents"
  >  
    All [APM agent configurations](/docs/using-new-relic/cross-product-functions/install-configure/configure-new-relic-agents/) must report to `gov-collector.newrelic.com` rather than the default.  

    Code-based or environment variable configurations:
...
<CollapserGroup>
  <Collapser
    id="mobile-monitoring-configurations"
    title="Mobile monitoring agents"
  >
    You'll need to use code-based configurations when setting your mobile monitoring agents to report to `gov-mobile-collector.newrelic.com` rather than the default. Environment variables are not available. 
...
<CollapserGroup>
  <Collapser
    id="infrastructure-monitoring-new"
    title="Infrastructure monitoring for versions 1.15.0 and higher"
  >  
    You can enable the [FedRAMP configuration option](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#fedramp) in infrastructure agents version [1.15.0 or higher](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-1150). This enables FedRAMP compliancy for data reported by the infrastructure agent, and for any [on-host integrations](/docs/integrations/host-integrations/host-integrations-list) that work with the infrastructure agent to report data. 
...
<CollapserGroup>
  <Collapser
    id="infrastructure-monitoring-old"
    title="Infrastructure monitoring for versions older than 1.15.0"
  >  
...
Below are details about FedRAMP endpoints for our [ingest APIs](/docs/apis/get-started/intro-apis/introduction-new-relic-apis/#data-type-apis).
...
<CollapserGroup>
  <Collapser
    ...
    title="OTLP API"
  > 
    FedRAMP compliance for [OTLP API](/docs/integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-quick-start#review-settings) requires that you send data to `https://gov-otlp.nr-data.net:4317` instead of the US OTLP API endpoint of  `https://otlp.nr-data.net:4317`. 

    If your client does not support gRPC, you can use: 
    
    - Port 4318 for OTLP/HTTP
    - Port 443 may also be used for either transport, if necessary. 
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser
    ...
    title="Metri API"
  > 
  FedRAMP compliance for [metric API](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/introduction-metric-api/) requires that you send [metric data](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) to `https://gov-metric-api.newrelic.com/metric/v1` instead of the default endpoint of `https://metric-api.newrelic.com/metric/v1`.

  Below are instructions to configure endpoints for telemetry integrations and telemetry SDKs.
  ...
  * Prometheus: 
  ...
  The [Prometheus remote write integration](/docs/integrations/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#remote-write) is not available for use with the `https://gov-metric-api.newrelic.com/metric/v1` endpoint at this time and attempted requests will result in an `HTTP 501 Not Implemented` status code. Consider using the [Prometheus OpenMetrics](/docs/integrations/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics) integration instead.
  </Callout>
...
<CollapserGroup>
  <Collapser
    ...
    title="Event API"
  >  
    FedRAMP compliance for the [event API](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/introduction-event-api) requires that you report all traffic to `gov-insights-collector.newrelic.com` instead of `insights-collector.newrelic.com`.

    The Event API endpoint is configurable for the following [telemetry SDKs](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/telemetry-sdks-report-custom-telemetry-data). Telemetry SDKs are used by our [open-source telemetry integrations](/docs/integrations/open-source-telemetry-integrations).  
