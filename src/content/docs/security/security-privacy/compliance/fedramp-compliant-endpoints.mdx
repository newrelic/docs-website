...
* Don't send data to the US OTLP API endpoint of `https://otlp.nr-data.net:4317`. 
  * Instead, send data to `https://gov-otlp.nr-data.net:4317`. Or, if your client doesn't support gRPC, you may use port 4318 for OTLP/HTTP.
* When sending OTLP Metrics to New Relic, don't use gRPC. 
  * Instead, use HTTP and target the API endpoint: `https://gov-otlp.nr-data.net:4318`. You can configure your OTLP exporter to use HTTP by using the [`otlphttp` exporter](https://github.com/open-telemetry/opentelemetry-collector/blob/main/exporter/otlphttpexporter/README.md#otlphttp-exporter).

You can also use port 443 for either transport, if necessary.

Reference our [OTLP integration support matrix](/docs/integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-setup#review-settings) for more details on FedRAMP compliance with OTLP.
