...
* Incident intelligence is [not in-scope](/docs.newrelic.com/docs/security/security-privacy/compliance/certificates-standards-regulations/hipaa/#not-scope) of the New Relic BAA.  You may not use an existing New Relic Account for your HIPAA compliance needs if you have enabled incident intelligence for that Account. Instead you must create a new New Relic HIPAA-enabled Account before sending any PHI to New Relic.
