...
New Relic has released [Java agents 6.5.2](/docs/release-notes/agent-release-notes/java-release-notes/java-agent-652/) and [7.4.2](/docs/release-notes/agent-release-notes/java-release-notes/java-agent-742/) to address critical vulnerability in the open source log4j framework that was publicly disclosed on December 9, 2021. A malicious actor may be able to exfiltrate data or execute arbitrary code using log messages or log message parameters.

These releases also remediate CVE-2021-45046, a lower severity issue in the open source log4j framework that was publicly disclosed on December 14, 2021. A malicious actor may be able to trigger a denial of service condition in certain non-default configurations.
...
If you have already upgraded to agent versions 6.5.1 or 7.4.1, these agent versions are using CVE-2021-44228. However Apache Foundation's current [technical guidance](https://logging.apache.org/log4j/2.x/security.html#CVE-2021-44228) is that log4j2 v2.15.0 has incomplete protection against CVE 2021-44228. We recommend updating or implementing the additional workaround below at this time.

If you are on an earlier or affected version of the agent, or cannot upgrade your agent version, we recommend you disable the Java agent logging.
