...
To use New Relic's Service Levels and see SLO results, you need a [full platform user](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type). However, in order to create new SLIs and SLOs, you need the [specific capability in your role](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#capabilities) to modify events to metric rules.
...
Service Levels Management is in public beta, and no official support is offered yet through the [Support portal](/docs/using-new-relic/welcome-new-relic/get-started/find-help-use-support-portal/).
...
To view the queries that define an  SLI, you can click on the ... menu on the SLI summary card, and select the Edit option. The queries will show at the top-right corner.

## How can I view the queries that define an SLI through the API?

To view the queries that define an SLI through the API, you can use [Nerdgraph](https://docs.newrelic.com/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/). In order to view the SLI configuration, you will need to know the GUID of the entity that the SLI is attached to, and replace it in the following query.
...
As a response you are going to receive all the SLIs attached to the entity. You’ll get the SLI name and queries. Take into consideration that only one of the goodEvents or the badEvents objects is going to have content,  depending on the configuration of the SLI.
...
As entityGuid, use the entityGuid of the entity that is associated with the SLI. On the query results, you’ll get the SLI id in the serviceLevel.indicators.id field.
