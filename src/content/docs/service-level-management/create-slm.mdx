...
- **Distributed metrics**. Because data is aggregated, a new NRQL function, called Cumulative Distribution Function, needs to be implemented in NRDB. We are working to enable these types of queries.
- **[Timeslice metrics](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data)**. Any query involving `newrelic.timeslice.value` or `apm.*` attributes is unsupported. 
- **Subqueries**. Subqueries are not supported.
- **Wildcards in the SELECT clause**. Wildcards are not supported in the attributes provided in the `SELECT` clause. Note that the wildcard expression is supported in the `WHERE` clause.
...
Once you have established the "top level SLs", you might find that not all the endpoints of your service behave in the same way, and might want to split it further. For example:
