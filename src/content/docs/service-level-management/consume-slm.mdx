...
For example, for [transaction](https://docs.newrelic.com/attribute-dictionary/?event=Transaction) data: 
* To see if any of the transactions for the service is returning more unsuccessful results than the rest, add `FACET [name](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=name)` to the query.
* To learn which client is getting the highest number of unsuccessful results, add `FACET [request.uri](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=request.uri)`. 
