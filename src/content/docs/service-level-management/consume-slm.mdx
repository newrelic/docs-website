...
For example, for [transaction](https://docs.newrelic.com/attribute-dictionary/?event=Transaction) data, try `FACET [name]`(https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=name) to see if any of the transactions for the service is returning more unsuccessful results than the rest. To learn which client is getting the highest number of unsuccessful results, try `FACET [request.uri]`(https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=request.uri). 
