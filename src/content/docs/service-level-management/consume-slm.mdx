...
* Add `FACET [name]`(https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=name) to the query to see if any of the transactions for the service is returning more unsuccessful results than the rest.
* Add `FACET [request.uri]`(https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=request.uri) to learn which client is getting the highest number of unsuccessful results.
