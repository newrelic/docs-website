...
For example, for [transaction](/docs/attribute-dictionary/?event=Transaction) data, try `FACET `[`name`](/docs/attribute-dictionary/?event=Transaction&attribute=name) to see if any of the transactions for the service is returning more unsuccessful results than the rest. To learn which client is getting the highest number of unsuccessful results, try `FACET `[`request.uri`](/docs/attribute-dictionary/?event=Transaction&attribute=request.uri)`. 
