...
When an SLO is not compliant, you will want to analyze the original data to better understand what the impact is for your customers, with a special focus on what went wrong. This is what you can do with the **Analyze** option on any service level.

![Analyze SLI detractors](./images/sli_analyze.png "Analyze SLI detractors")

When you click on **Analyze**, the [query builder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/) will open and you can drill down into the data using NRQL, to better understand the cause and impact of missing your SLO. You'll start from a query that represents the original NRDB bad events, that is, those events that damaged the SLO compliance. We recommend that you use the FACET clause to analyze a certain attribute (such as account, client id, requesting source, etc.), and thus detect if the SLO is particularly damaged for one of the values. From now on we will call those values "detractors". 

For example, for [Transaction](https://docs.newrelic.com/attribute-dictionary/?event=Transaction) data, you can try adding `FACET [name](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=name)` to the query, which can tell you if one of the transactions for the service is especially returning unsuccessful results; or `FACET [request.uri](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=request.uri)`, which can tell you which client is getting the highest number of unsuccessful results.
...
When you detect that one or very few detractors are really degrading the SLO compliance, you can take several actions. The most usual thing to do is troubleshoot the issue and plan work to have the detractor meet the SLO. Or you can temporarily adjust the SLO target to a more realistic value, and plan work to improve reliability.

But if the detractor is really an exception that won't easily match the general expectations for your service performance and reliability, consider having a dedicated SLO for that case. You would need to do two things: 
* First, use a `WHERE` clause on the original SLI queries to filter out the detractor (e.g, `WHERE countryCode != 'US'`). 
* Then, create a new SLI with a `WHERE` clause on the queries that only takes into account the detractor case (e.g, `WHERE countryCode = 'US'`), and set a more realistic SLO target for it.
