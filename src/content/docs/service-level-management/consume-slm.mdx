...
If you add tags to your service levels, you can use the filter bar to scope down the service levels that you get, and to group them. Filtering can be useful to hide any testing or aspirational service levels that the team has not committed to yet, while grouping is useful when you want to focus on those service levels that refer to a certain owner, organizational unit or user flow.      

Depending on what you're trying to achieve, you can use one the following view modes: 

* **Operational**: When you're in charge of operating a service, you can use this view to see how the SLO compliance and error budget are trending for the past 2 hours, and 1, 7 and 28 days rolling windows.
* **Period over period**: For business reviews, retrospectives and prioritization meetings, you can use this view to compare the compliance per calendar week or month. 

Both views show SLO compliance. Request-based SLOs are determined from SLIs defined as the ratio of the number of good responses to the total number of requests. This means a request-based SLO is met when that ratio meets or exceeds the goal for the SLO compliance period.

### SLO state for operations [#operations-mode]

The operational view shows how your service levels are improving or degrading in different time windows. 
  
![Service level operational view](./images/service-levels-intro.png "Service level operational view")
...
### SLO state for business reviews [#business-mode]

You can use the period over period view for reporting in your planning and review meetings, which happen with a certain calendar frequency. You can switch the period between weeks and months (and soon, also quarters).  

![Service level business view](./images/service-levels-business.png "Service level business view")

The cell color works exactly as described above on the operations view. The value of this view is that it shows a longer history of your SLO compliance over time windows refer to a fix calendar period.

## Understand service levels details [#sl-details]

Click on any SLI to open the SLI details:

![SLI details](./images/sli_card.png "SLI details")

You can use the SLI details for two main purposes:
...
![Analyze SLI detractors](./images/sli_analyze_menu.png "Analyze SLI detractors")
