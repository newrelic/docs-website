---
title: Compatibility and requirements for browser agent
tags:
metaDescription: Browser agent compatibility and requirements for Streaming Videos & Ads.
redirects:
freshnessValidatedDate: never
---

Before you [install the browser agent](/install/browser-media), make sure your system meets the following requirements.

Our <InlinePopover type="browser"/> includes [strict security measures](/docs/browser/new-relic-browser/performance-quality/security-browser-monitoring) to provide a robust, standalone product with Streaming Video & Ads features. Before you [install the browser agent](/install/browser-media), make sure your system meets these requirements.
...
2. Clients accessing the application must be able to send data to New Relic through HTTP requests. HTTPS is required for the more advanced browser streaming features.
...
* [Chrome](https://www.google.com/chrome/) ([previous 2 versions](https://browsersl.ist/#q=last+2+chrome+versions))
* [Safari](https://www.apple.com/safari/) ([previous 2 versions](https://browsersl.ist/#q=last+2+safari+versions))
* [Firefox](https://www.mozilla.org/firefox/) ([previous 2 versions](https://browsersl.ist/#q=last+2+firefox+versions))
* [Edge](https://www.microsoft.com/edge) ([previous 2 versions](https://browsersl.ist/#q=last+2+edge+versions))
...
<Callout variant="important">
  When you use browser agent with cookies, New Relic's cookies are a third-party cookie on your site, and may not store or work on certain web browsers used by your visitors. See the applicable browser's websites for details about their compatibility with third-party cookies.
</Callout>

## APM agents [#apm-agents]

One way to [deploy browser agent](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser) for an app is using one of our APM agents.

If you're deploying browser for an app already being monitored by APM, make sure your APM agent version supports browser streaming:
...
<Callout variant="important">
  If your website does not allow any third-party hosted JavaScript to run, you must request a CSP exception via your organization’s standard procedures in order to install the browser streaming agent.
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Supports injecting the JavaScript snippet via the `Transaction.BrowserTimingHeader` API. You will need to write additional code to modify the script element to add the `nonce` attribute. See [Add browser streaming for Go apps](/docs/apm/agents/go-agent/features/add-browser-monitoring-your-go-apps/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Supports adding `nonce` via the `getBrowserTimingHeader` and `getBrowserTimingFooter` APIs. See the [Java agent browser streaming docs](/docs/apm/agents/java-agent/instrumentation/instrument-browser-monitoring-java-agent-api/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Supports adding `nonce` via the `GetBrowserTimingHeader` API. See the [.NET agent browser streaming docs](/docs/apm/agents/net-agent/other-features/browser-monitoring-net-agent/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Supports adding `nonce` via the `getBrowserTimingHeader` API. See the [Node.js agent browser streaming docs](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Supports adding `nonce` via the `browser_timing_header` API. See the [Ruby agent browser streaming docs](/docs/apm/agents/ruby-agent/features/new-relic-browser-ruby-agent/).
      </td>
    </tr>
  </tbody>
</table>
