...
<Step>
  ...
  <Step>
        **Initialize the Agent**: Open your main BrightScript file (e.g., `Main.brs`). Initialize the New Relic Agent by calling the `NewRelic` function with your `ACCOUNT ID`, <InlinePopover type="licenseKey"/> 'APP NAME' and 'APPLICATION TOKEN'.

        ```brightscript
        m.nr = NewRelic("33", "LICENSE_KEY", "APP_NAME", "APPLICATION_TOKEN", "US", true)
