...
3. **Initialize the Agent**: Open your main BrightScript file (e.g., `Main.brs`). Initialize the New Relic Agent by calling the `NewRelic` function with your `ACCOUNT ID` and <InlinePopover type="licenseKey"/>.
...
1. **App Started Event**: Call `nrAppStarted` to log the application start event.

   ```brightscript
   nrAppStarted(m.nr, aa)
   ```

2. **Start Event Capture**: Use `NewRelicSystemStart` and `NewRelicVideoStart` to begin capturing system and video events.

   ```brightscript
   m.syslog = NewRelicSystemStart(m.port)
   ```

3. **Process Messages**: In the main wait loop, call `nrProcessMessage` to handle system events.
