...
- Access to a New Relic account with the Streaming Video & Ads agent installed. See [Install the Streaming Video & Ads agent](/docs/streaming-video-&-ads/installation/install-the-streaming-video-&-ads-agent) if you haven't set it up yet.
...
6. Click <DNT>**Run Query**</DNT> to preview results against live data. This doesn't save the widget. Use the preview to confirm the chart renders the data you expect, verify that filter placeholders resolve correctly when a filter is active, and catch any syntax or logic errors before saving.
7. Once the preview renders, a chart type picker appears. The default is Line — select a different type if needed. Some chart types may appear grayed out if they're not recommended for your query results, but you can still select any of them: Billboard, Line, Area, Bar, Table, or Pie.
