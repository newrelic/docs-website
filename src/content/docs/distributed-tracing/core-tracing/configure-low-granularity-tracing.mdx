  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * `nr.ids` (list of exit span GUIDs that were dropped due to compression)
      </td>
    </tr>
  </tbody>
</table>

### About span compression attributes

When using compact granularity (`type: "compact"`), the agent adds additional attributes to track compressed spans:

**`nr.durations`:** Tracks the sum of the durations of all compressed spans as a separate attribute. This ensures backwards compatibility with existing span performance analysis by providing duration data independently.

**`nr.ids`:** Contains a list of exit span GUIDs that have been dropped due to span compression. This attribute is used to:
- Identify when a span is a partial granularity compressed span
- Link the span tree back together when exit spans are dropped due to compression
- Re-attach downstream service spans to the upstream service when the connecting span is compressed

The presence of the `nr.ids` attribute on a span indicates that the span is a compressed span in the UI.

### Attributes excluded from partial granularity
