...
Querying and visualizing trace data consumes compute units. While span configuration primarily addresses ingest costs, reducing data volume can also improve query performance.
...
### Strategy 1: Option 1 (Span filtering only) for services with deep call stacks

**What it does:** Removes in-process spans, keeping only entry and exit spans
...
Use [Option 1 (span filtering)](/docs/distributed-tracing/core-tracing/configure-minimal-spans-tracing) to eliminate in-process spans while keeping entry and exit spans. This dramatically reduces span volume without losing service-to-service visibility.

**Example scenario:**
Your Java services generate 5-10 in-process spans per request, but you rarely drill into that level of detail.

* Enable Option 1 on all services with deep call stacks
* Keep full distributed tracing on services where you need method-level visibility
* **Result:** ~55% cost reduction

### Strategy 2: Option 2 (Span filtering + attribute reduction) for supporting services
...
Use [Option 2](/docs/distributed-tracing/core-tracing/configure-minimal-spans-tracing) on services where you need connectivity data but not detailed metadata. You'll still see service-to-service calls in your flow maps and traces, with minimal span data.
...
* Enable Option 2 on 35 supporting services
* **Result:** ~45% overall cost reduction

### Strategy 3: Option 3 (Maximum reduction) for non-production or high-volume services

**What it does:** Removes in-process spans, reduces attributes, and compresses duplicate exit spans
...
Use [Option 3](/docs/distributed-tracing/core-tracing/configure-minimal-spans-tracing) on services where you only need the "big picture" connectivity without duplicate call tracking.

**Example scenario:**
You have extensive staging and development environments that mirror production.

* Production: Full distributed tracing on critical services, Option 1 on others
* Staging: Option 2 or 3 on all services
* Development: Option 3 on all services
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Option 2 (span filtering + attribute reduction)
      </td>
      <td>
        Need connectivity data for complete traces, but rarely need detailed attributes. Significant cost reduction.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Option 3 (maximum reduction) + reduced sampling
      </td>
      <td>
...
You can change span configuration settings anytime:
...
* Option 1: 30 core services (inventory, pricing, search)
* Option 2 or 3: 110 supporting services (logging, notifications, analytics)
