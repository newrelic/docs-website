...
**Priority system:** If the span reservoir is exceeded, full granularity transactions are kept over partial granularity transactions. Similarly, traces that the upstream service sampled are kept over traces that where not sampled by the upstream service or originated from the current service.
