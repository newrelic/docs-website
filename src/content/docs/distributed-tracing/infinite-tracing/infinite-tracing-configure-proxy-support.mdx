...
The proxy address can be specified by the `HTTPS_PROXY` environment variable. (.NET Agent version 10.6.0 and earlier also support using the `GRPC_PROXY` environment variable.)
