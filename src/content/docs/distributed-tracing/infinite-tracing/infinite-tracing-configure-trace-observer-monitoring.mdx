...
<Callout variant="tip">
  When trace observer monitoring is enabled or disabled an `NrAuditEvent` is
  written to the account where the change occurred. This event helps you keep
  track of any changes made to the trace observer.
</Callout>
...
![Screenshot showing what appears in the Edge app if you don't have access to the monitoring account that is receiving metrics.](src/images/trace_observer_monitoring_no_access.png 'trace_observer_monitoring_no_access.png')
...
- A new pane is added on the right side of the screen:
  ...
  - **How many traces were kept?** (top chart): Shows the same information as **Traces Kept**, but it shows you what this looks like over time. You can see the magnitude of sampling and any trends.
    ...
    ![Screenshot of graph showing how many traces were kept.](src/images/how_many_traces_kept.png 'how_many_traces_kept.png')
  ...
  - **Which traces were kept?** (bottom chart): Shows which categories of traces were kept. The chart shows whether traces were captured by different types of Infinite Tracing filters:
    ...
    ![Screenshot of graph showing which traces were kept.](src/images/which_traces_kept.png 'which_traces_kept.png')
