...
Once you have a trace observer, you need to specify which data sources (services, apps, or entities) should have their data sent to the Infinite Tracing trace observer. Before you select your data sources, familiarize yourself with these issues:
...
<CollapserGroup>
  <Collapser
   ...
   >
   As soon as you select your browser, mobile, and Lambda data sources, you can expect a delay before you see your results in the New Relic UI. This delay may be anywhere from one to two minutes. 

   For example, if you re-run the [sample curl test](#sample-request) test after you add browser, mobile, and Lambda data sources, you should expect a delay before the data appears in the UI.
  </Collapser>
  <Collapser
    className="freq-link"
    id="multi-named-apps"
    title="What if my APM agent has multiple names?"
  >
    If you have an APM application with multiple names, you can only select one of the names when you choose your data sources. If you try to select the same application that goes by another name, the UI will prompt you that you need to choose only one name.
  </Collapser>
  <Collapser
    className="freq-link"
    id="lambda-sampling"
...
3. Select the data sources you want to include, keeping the following in mind:
    ...
    * If your APM agent has multiple names, you can can only choose one name.
