...
Complete the trace observer setup up that fits your types of services. You may need to do set up steps in each section if you have a variety of service types:
...
8. Return to your original distributed tracing setup instructions for your agent or third-party telemetry tool and insert your new endpoint value in each service to route your data to New Relic.

9. If you have some services instrumented with New Relic browser, mobile, or lambda, follow the steps in that [section](#browser-mobile-lambda) to select their data sources (entities) in the trace observer.

## Services instrumented with New Relic browser, mobile, and lambda [#browser-mobile-lambda]

To create a new trace observer if you're using New Relic browser, mobile, or lambda, you follow similar steps to those for APM agents and third-party integrations. Where this setup differs is that you don't need to insert a new trace observer endpoint in each remote service. Instead, in the trace observer setup, you designate which services (data sources) should have their data routed automatically by New Relic to a trace observer. 

If you already created a trace observer while setting up another type of service, you can skip the steps about creating the trace observer and go right to the step 6 to select your data sources.
...
6. To set up the routing for this new trace observer endpoint, see our [GraphQL tutorial](/docs/apis/nerdgraph/examples/configure-infinite-tracing-graphql/#updating-the-data-sources) about how to designate which services should route data to the new endpoint. When you finish, return to this page.

7. Bookmark the [optional configurations](#follow-up-config) below so you can return to them after you experiment with Infinite Tracing.

8. View your Infinite Tracing data in the [New Relic UI](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/).

9. If you have some services instrumented with New Relic APM agents or third-party telemetry tools, complete the steps in that [section](#apm-and-third-party) route that data to New Relic.
