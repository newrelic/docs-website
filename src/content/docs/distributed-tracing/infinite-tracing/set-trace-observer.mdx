...
## What happens when you set up a trace observer? [#differences]

You can set up a trace observer either through the New Relic UI or directly with GraphQL. Either way you set it up, you do the following:

1. You create the trace observer that has a special endpoint.
2. You take steps to tell your services how to send trace data to the new endpoint.

The first step of creating trace observers is the same no matter what type of tool you're using to send spans (APM, browser, etc.). However, the way you get your services to send data to that special endpoint depends on the type of tool:

* **APM agents, OpenTelemetry, and Kamon**: You copy the endpoint from the trace observer setup and insert it into the tool's configuration for each associated distributed service.
* **New Relic browser, mobile, and lambda**: You go into the trace observer and specify which sources (entities) should participate in Infinite Tracing. No endpoint configuration is necessary in the remote services. 
