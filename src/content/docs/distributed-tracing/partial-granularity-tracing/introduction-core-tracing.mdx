...
* **Incomplete instrumentation and fragmentation:** When some services send New Relic data and other services send Open Telemetry data you get fragmented maps and broken trace paths. This can also happen when a services sends so much data that it exceeds the data reservoir within the Agent.It's hard to justify the cost of full instrumentation everywhere, but partial coverage makes it difficult to understand your full system behavior.
...
## How Partial Granularity Tracing solves these problems [#solution]

APM agents let you control exactly how much trace data each service sends to New Relic. You can:

* **Reduce costs by 50-85%** by selecting between three different types of partial granularity tracing that control how many spans and span attributes are captured.
* **Instrument all services at lower cost** using lower granularity trace data for supporting services
* **Mix configurations across services** keeping full granularity trace detail for critical services while using cheaper partial granularity traces elsewhere

Partial Granularity Tracing is available in the following APM agents:
* Java
* Node.js
* Python
...
## Partial Granularity Tracing type [#type]

APM agents provide three different types of partial granularity tracing that control how many spans and span attributes are captured. Below is an example of a Full Granularity Distributed Trace before Partial Granularity has been applied to keep only the most critical spans and attributes:

![full granularity distributed trace](./dt_full_granularity.png 'full-granularity-distributed-trace')

### Reduced partial granularity: Drop in-process spans
...
![reduced partial granularity distributed trace](./dt_partial_granularity_essential.png 'reduced-partial-granularity-distributed-trace')
...
### Essential partial granularity: Drop in-process spans + reduce attributes
...
![essential partial granularity distributed trace](./dt_partial_granularity_essential.png 'essential-partial-granularity-distributed-trace')
...
### Compact partial granularity: Drop in-process spans + reduce attributes + compress duplicates

Everything from essential granularity, plus compress duplicate exit spans. If your service calls the same database or downstream service multiple times in a request, all calls are represented by a single span with a combined duration (time spent calling that service).

**What you get:**
* Service-to-service visibility intact
* 1 representative span per database and external API call
* Complete distributed trace paths
* AI monitoring spans preserved
...
<Callout variant="warning">
  This setting should not be used if AI spans are present in the transaction as it will destroy the span tree heirarchy that is needed for viewing AI data.
</Callout>


![compact partial granularity distributed trace](./dt_partial_granularity_compact.png 'compact-partial-granularity-distributed-trace')

**Typical reduction:** 70-85% total data volume

**Best for:** High-volume services or services with many calls to the same downstream service where detailed code path information is not needed.
