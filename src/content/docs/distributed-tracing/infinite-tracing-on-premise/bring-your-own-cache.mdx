...
The `distributed_cache` section supports comprehensive configuration options for optimizing Redis-based distributed tail sampling.
...
- **Worst case calculation**: `PoolTimeout(3s) + Operation(0.5s) + 3 retries × (0.5s + backoff) ≈ 7s < 10s` ✅
...
| `processor_name` | string | `""` | Optional processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
3. Workers hash trace IDs to determine partition assignment
...
- Traces are assigned to partitions using **consistent hashing** on the trace ID
...
Incoming Traces
      ...
      v
┌─────────────────────────┐
│  Consistent Hash Ring   │  trace_id → hash % partitions
│  (based on trace ID)    │
└─────────────────────────┘
      |
      ├──────────┬──────────┬──────────┐
...
**Flow:**
1. **Ingestion**: Trace ID is hashed to determine partition (e.g., `hash(trace_id) % 4 = 2` → Partition 2)
2. **Storage**: Trace data stored in Redis under partition-specific keys
3. **Evaluation**: Worker assigned to Partition 2 pulls and evaluates traces
...
| **High Volume (>500k spans/sec)** | 12-24 | 12-24 | Maximize throughput |
...
- **Without compression**: Reduce CPU requirements by ~30-40% but increase network and memory needs
  ...
  <Callout variant="tip">
    **Monitoring CPU**: Watch for CPU saturation (>80% utilization) as the first indicator of scaling needs. If CPU-bound, either add cluster nodes or reduce compression overhead.
  </Callout>
...
**Latency impact:**
- **<1ms latency**: Optimal - no throughput degradation
- **1-5ms latency**: Acceptable - minimal impact with connection pooling
- **5-10ms latency**: Noticeable - increase `pool_size` and timeouts
- **>10ms latency**: Significant - may require architectural changes (regional Redis, increased timeouts)
...
- **Connection pooling**: Default `pool_size: 20` supports ~5,000-10,000 spans/sec. Increase to 30-50 for higher throughput
  ...
  <Callout variant="important">
    **Network is critical**: Round-trip time between collectors and Redis directly impacts end-to-end sampling latency. Deploy Redis with **low-latency network connectivity** (<5ms) to collectors. Use cluster mode with replica reads to distribute network load.
  </Callout>
...
| Resource | Without Compression | With lz4 Compression (25% reduction) |
|----------|---------------------|--------------------------------------|
...
```
  ...
  <Callout variant="important">
    **Sizing guidance**:
    ...
    - **Scaling**: If CPU-bound (>80% utilization), add cluster nodes. If network-bound, enable compression or add nodes
