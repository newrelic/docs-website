...
* <DoNotTranslate>**Exclusive time**</DoNotTranslate>: Entities called down to by the service you are viewing, which are contributing significantly more latency. The latency (exclusive time) contributed by an entity is the wall-clock time that it had one or more processes running but was **not** making any external calls.
