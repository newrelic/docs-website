...
When any related trace entities had a significant change in their performance impact, you're shown a list of those performance signals and their change.
If an entity is causing a significant change in performance impact, each of those signals will be shown in its own card in the list.
...
## Full-page View
This view shows the same list of significant signals, with more real estate to scroll through longer lists.
...
## No significant changes presented
If no upstream or downstream services had a significant changes in their performance impact, the all clear view is shown.
...
* **No data from related entities** If your service _is_ reported distributed tracing data, but no related entities are. Enable distributed tracing on the dependencies of your service to see insights.