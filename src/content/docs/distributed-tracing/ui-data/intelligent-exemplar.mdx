...
When there is a latency spike or error surge, your first challenge is finding *where* the problem lives. A distributed trace describes a causal dependency graph that can tell you the source of latency and errors, but manually finding the right subset of traces to see this pattern for your current context can be challenging

<DNT>Intelligent Exemplars (IE)</DNT> eliminates that search. It algorithmically surfaces the traces that best represent the anomaly, and navigates you directly to the failing span; so you go from spike to root span in clicks instead of hours.
...
Rather than showing a random or chronologically ordered sample, <DNT>Intelligent Exemplars</DNT> evaluates the full trace dataset and surfaces a curated list of up to 500 traces that best represent the anomaly. Traces are weighted toward occurrences that fall within the spike window. 
...
- Review the span attributes that might have caused the respective spike
