...
<CollapserGroup>
  ...
  >
    A preserved distributed trace is like an archived snapshot of the original trace that has been previously viewed and has surpassed the retention period. By default, unless you have purchased extended retention, a full trace is onlyavailable for 7 days. Preserved traces can exist for up to 1 year, and generally function like the original trace.
    
    However, there are some minor differences and limitations:

    * Real time span performance data can not be displayed.
    * Span anomaly data can not be displayed.
    * When an entity involved in a preserved trace is deleted, expires, or stops reporting, it may result in the preserved trace no longer being accessible.
