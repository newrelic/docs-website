...
<CollapserGroup>
  ...
  >
    ...
    However, there are some limitations to preserved traces:

    * Real time span performance data can not be displayed.
    * Span anomaly data can not be displayed.
    * If an entity involved in a preserved trace is deleted, expires, or stops reporting, the preserved trace may no longer be accessible.
