...
The latency view shows you how much latency is contributed by each entity in the trace, as well as the number of error spans per entity. The latency contributed by an entity is the wall-clock time that it had one or more processes running but was _not_ making any external or database calls to return.

Here are two examples:

* If an entity had a process lasting 500ms, but 400ms of that time it's making an external call, it's shown as contributing 100ms of latency to the trace.
* If an entity had three processes running in parallel for one second, and none of them made external calls, it's shown as contributing one second of latency to the trace.
