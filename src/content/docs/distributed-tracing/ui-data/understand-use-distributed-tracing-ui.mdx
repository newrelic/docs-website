...
3. By clicking **More filters** at the bottom of the left pane, you can see additional chart controls. See [#trace-charts] for an example.
...
When you open distributed tracing, you'll see three charts at the top (if **Group similar traces** is turned on). These charts give you a quick understanding of trace distribution by count, duration, and traces with errors.

To adjust the ranges for these charts (or for the scatter plot), open the chart controls by clicking **More filters** at the bottom of the left pane. You can adjust the sliders for **Trace duration**, **Error count**, **Service count**, and **Span count**.
...
The waterfall view is below the entity map, and displays a cascade of a trace's spans. This allows you to see see how spans relate to each other in a vertical layout.
...
### Span properties in the waterfall [#span-icons]
...
* **Performance**: You'll see charts about average duration, throughput, and span duration.
* **Attributes**: You can learn more about attributes from our [glossary](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#attribute).
...
<CollapserGroup>
  ...
  >
    ...
    * An <InlinePopover type="apm" /> agent may have hit its [1K span collection limit](#1k-limit).
