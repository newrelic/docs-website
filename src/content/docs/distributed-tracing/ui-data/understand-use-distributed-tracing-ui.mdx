...
Note that reserved traces won't display span performance data or span anomaly data. Preserved traces may also no longer be accessible if an entity involved in a preserved trace is deleted, expires, or stops reporting data.
