...
When you select a trace from the trace list, you see an [entity map](#entity-map) at the top followed by a [waterfall view](#waterfall-view) displaying that trace's spans. The top section also offers a timeline view and a latency view.
...
This view allows you to quickly scroll through the entire trace and see how latency and errors propagete through the trace and across entity boundaries.
You can select spans from this view to view span details in the waterfall view.

<img
  title="Timeline view"
  alt="New Relic distributed tracing UI - timeline view"
...
Latency contributed is the wall-clock time that an entity had one or more processes running, but was _not_ making any external or database calls to return.
For example, if one entity had a process lasting 500ms, but 400ms of that time it is making an external call, it is shown as contrinbuting 100ms of latency to the trace.
...
<img
  title="Latency view"
  alt="New Relic distributed tracing UI - latency view"
