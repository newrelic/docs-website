...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Use an attribute with an `http.` prefix. For example: `http.method`. A span will also appear as an external if it has a child that comes from a different entity. In order to be indexed on the [External Services](/docs/apm/apm-ui-pages/monitoring/external-services/external-services-intro) page, either `http.url` or `net.peer.name` must be provided.
      </td>
    </tr>
