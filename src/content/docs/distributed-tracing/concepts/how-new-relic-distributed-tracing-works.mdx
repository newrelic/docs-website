...
<CollapserGroup>
  ...
  >
    ### Modernized Sampling & OpenTelemetry Compatibility

    Our <InlinePopover type="apm"/> language agents utilize adaptive sampling to provide high-fidelity visibility while maintaining system performance. This approach ensures a representative sample of system activity is captured across diverse architectures, including native New Relic agents and OpenTelemetry (OTel) integrations.

    ### How Adaptive Sampling Works

    The first service monitored in a distributed trace—the **trace origin**—is responsible for the sampling decision. This decision propagates to all downstream services to ensure a complete end-to-end trace.

    New Relic agents use a "target-based" adaptive algorithm. The agent analyzes the previous minute's throughput to calculate the sampling probability for the current minute. For example, if your service handles 120 transactions per minute and the sampling target is 12, the agent dynamically adjusts to select approximately 1 out of every 10 transactions.

    ### Sampling Targets and Limits by Agent

    The following table outlines the modernized sampling goals and span limits designed to support high-fidelity tracing and OTel compatibility.
    ...
    <table>
      <thead>
        <tr>
          ...
          <th style={{ width: "200px" }}>
            Target Traces (per minute)
          </th>
          <th style={{ width: "200px" }}>
            Max Span Limit (per minute)
          </th>
        </tr>
      </thead>
      <tbody>
    ...
    ¹ Optimized for high-throughput enterprise environments.
    ² Configuration balanced for minimal resource overhead.
    ³ Aligns with OpenTelemetry Trace Ratio standards and supports Span Link propagation.

    <Callout variant="tip">
    If an agent generates more spans than its configured limit in a minute, some spans will be dropped, resulting in [fragmented traces](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces) in the UI. Traces are assigned a random priority when selected for sampling, so if multiple agents need to drop spans they can attempt to keep higher-priority traces intact by first dropping spans from lower-priority traces.
    </Callout>

    ### High-Fidelity & Hybrid Environments

    As part of our commitment to open standards, New Relic's latest agent versions are designed for **Hybrid Compatibility**.

    **Trace Ratio Support**: Modern agents can now align with OTel-native sampling ratios, allowing for consistent sampling logic across mixed environments. See [OpenTelemetry API support](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support) for configuration details.

    **Preservation of Upstream Decisions**: In "Mixed Mode" environments (where both OTel SDKs and New Relic agents are present), our agents prioritize preserving trace decisions initiated by upstream OTel SDKs to prevent fragmented traces.

    **Span Links**: We have introduced support for Span Links, allowing you to associate traces that were sampled independently, ensuring no loss of context in complex, asynchronous workflows. Currently supported in:
    * Java agent
    * Node.js agent
    * .NET agent

    For instructions on adjusting these targets or configuring OTel compatibility, see:
    * [APM Agent Configuration documentation](/docs/apm/agents/)
