...
<CollapserGroup>
  ...
  >
    ...
    New Relic agents use a "target-based" adaptive algorithm. The agent analyzes the previous minute's throughput to calculate the sampling probability for the current minute. For example, if your service handles 100 transactions per minute and the sampling target is 10, the agent dynamically adjusts to select approximately 1 out of every 10 transactions.

    To find the default sampling target and span limits for your agent, see the agent-specific documentation:
    ...
    * [Ruby](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent)
    ...
    <Callout variant="tip">
    **Standard distributed tracing only**: If an agent generates more spans than its configured limit in a minute, some spans will be dropped, resulting in [fragmented traces](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces) in the UI. Traces are assigned a random priority when selected for sampling, so if multiple agents need to drop spans they can attempt to keep higher-priority traces intact by first dropping spans from lower-priority traces.

    This does not apply to Infinite Tracing, which sends all trace data without sampling.
    </Callout>
