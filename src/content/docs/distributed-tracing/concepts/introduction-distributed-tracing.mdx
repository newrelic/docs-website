...
As you scale distributed tracing, you might want more control over data volume. [Partial granularity distributed traces](/docs/distributed-tracing/configure-partial-granularity-tracing) provide controlled ingestion, allowing you to capture data that really matters while managing data volumes effectively. APM agents provide configurable controls to ingest data in a controlled manner while maintaining the visibility you need.

APM agent configuration helps you:

* **Control ingestion** by configuring span volume and attributes
* **Fill instrumentation gaps** with partial granularity distributed traces from all services
* **Ingest control** with more options to [configure transaction sample rate](/docs/distributed-tracing/core-tracing/configure-sampling) per service
