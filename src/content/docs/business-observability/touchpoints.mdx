...
Touchpoints are the more granular entities within the PathPoint model and behave similarly to a specific browser application or APM (Application Performance Monitoring). Each step of a Pathpoint is connected to one or more touchpoints, and the status of every stage is determined by the culmination of its respective touchpoints.
...
Select a touchpoint to test. Right click on it and select **Queries**.
...
In this example, it says **Successfully validated**, meaning that your touchpoint is healthy.
...
When testing a touchpoint, if you get an "Incorrect validated" error, it might be something as simple as because you running the Query on the wrong AccountID. Check to verify that you're running the Query on your AccountID.
...
One way to get better data from your touchpoints is to tweak their Tuning configuration. It is important to note that the initial values you see during the tuning process are defined by the JSON file uploaded to your Pathpoint. However, it is possible to manually adjust these values later as well.
...
Right click on the particular touchpoint you want to work with and select **Queries**.
...
Here, you see the NRQL query associated with the touchpoint.
...
If you need to work on or troubleshoot a specific touchpoint in Pathpoint, you can easily disable it manually. Once you have resolved any issues, you can then reactivate the touchpoint to resume monitoring. 
