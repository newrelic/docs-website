...
New Relic provides the ability to group these remaining logs into thematic partitions and parse the logs to pull out and tag valuable attributes. By organizing your logs in this manner, you can:
...
Seperating your logs into discrete partitions provides both performance benefits and management benefits. For example, imagine you want to query for an attribute only found in front-end logs. Querying through 5 TB of logs it will take drastically longer than querying through a specific 1 TB partition of only front-end logs. 

The goals of partitioning your data should be:
...
The following steps will guide you through how to organize your logs in the New Relic UI.
...
This creates a data partition with standard data retention for all Java logs. They would follow the same steps for their infrastrucutre logs, only changing the retention to secondary and query to `logtype=infrastructure`.
...
3. Select an existing field to parse (default = message), or enter a new field name.
...
The following example guides you through a specific example on creating a parsing rule:
...
Congratulations on setting your organization up for success in managing logs! As your system grows and you ingest, you'll need to ensure an upkeep of parsing rules and partitions. If you're interested in diving deeper on what New Relic log management can do for you, check out these docs:

* [Parsing log data](/docs/logs/ui-data/parsing): a deeper look into parsing logs.
