...
In this integration step, we'll also store your New Relic <LicenseKey /> in the AWS Secrets Manager service, so that we
...
The `newrelic-lambda` CLI adds your New Relic <LicenseKey /> as a secret in [AWS Secret Manager](https://aws.amazon.com/secrets-manager/) for greater security.
