...
Since Lambda serverless function monitoring requires either an [API Polling](/docs/infrastructure/amazon-integrations/connect/connect-aws-new-relic-infrastructure-monitoring) or [Metric Streams](/docs/infrastructure/amazon-integrations/connect/aws-metric-stream) integration, the CLI will automatically install API polling if it doesn't see an integration. If you prefer metric streaming, install that now before running the CLI.
