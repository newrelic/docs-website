...
On this page, we offer details on manually instrumenting your lambda function. It is organized by runtime language.
...
<CollapserGroup>
  ...
  >
    ...
    Next, you'll [configure CloudWatch to send those logs to New Relic](instrument-your-own#cloudwatch-telemetry).
  </Collapser>
  ...
  >
    ...
    * Our AWS Lambda OpenTracing Tracer: An OpenTracing Tracer implementation designed to monitor AWS Lambda. It generates spans, error events, transaction events, error traces, and provides distributed tracing support.
    ...
    Our wrapper gathers data about the Lambda execution, generates a JSON message, and logs it to CloudWatch Logs. Next, you'll [configure CloudWatch to send those logs to New Relic](instrument-your-own#cloudwatch-telemetry).
...
```
  ...
  The New Relic decorator gathers data about the Lambda execution, generates a JSON message, and logs it to CloudWatch Logs. Next, [configure CloudWatch to send those logs to New Relic](instrument-your-own#cloudwatch-telemetry).
</Collapser>
</CollapserGroup>
