...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Ruby"
  >
    To instrument your Ruby Lambda functions automatically:
    ...
    To instrument your Ruby Lambda functions manually:

    1. Use the AWS web console to visit the "Lambda" service section and navigate to the desired Lambda function.

    2. While on the default "Code" tab, scroll down to the "Layers" section and click on the "Add a layer" button.

    2. Click on the "Create layer" button

    3. In the "Choose a layer" section of the "Add layer" page, select the "Specify an ARN" option

    4. From New Relic's list of layers available at [https://layers.newrelic-external.com/](https://layers.newrelic-external.com/), use the drop-down list to select the AWS region your Lambda function is hosted in. Then locate the ARN that matches your Lambda function's Ruby version and architecture (X86 vs ARM64). Use the "Copy to clipboard" button or manually copy the ARN string.

    5. In the "Specify an ARN" section of the AWS console form, paste in the New Relic Lambda layer ARN.

    6. On the AWS console form, click on the "Add" button to add the layer to your Lambda function.

    7. With the default "Code" tab selected on your Lambda function's page, scroll down to the "Runtime settings" section and click on the "Edit" button.

    8. Make a safe copy of the existing "Handler" value. You'll need it for a later step.

    9. Change the "Handler" value to read "newrelic_lambda_wrapper.handler" and click "Save".

    10. On your Lambda function's page, switch to the "Configuration" tab.

    11. Select the "Environment variables" sub-tab.
    ...
    13. If you wish to send your Lambda function's logs to New Relic, which will include everything your function writes to STDOUT, also set the `NEW_RELIC_EXTENSION_SEND_FUNCTION_LOGS` environment variable equal to 'true'.

    NOTE: The New Relic layer will deliver the New Relic Ruby agent automatically and it will be loaded via a Ruby `require` prior to your Lambda function's invocation. To avoid conflicts, do not include a copy of the Ruby agent anywhere else. Free free to perform any desired New Relic Ruby agent API calls within your function to take advantage of the presence of the agent.
