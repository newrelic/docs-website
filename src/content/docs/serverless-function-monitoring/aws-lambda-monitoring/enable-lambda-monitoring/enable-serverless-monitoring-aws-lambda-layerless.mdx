...
<CollapserGroup>
  ...
  >
    ...
    1. In the AWS web console navigate to the **Lambda** service section. From there,  find the Lambda function you would like to connect to New Relic.
    ...
    3. Now go to the **Choose a layer** and select the **Specify an ARN** option.
    4. Go to [New Relic's list of layers](https://layers.newrelic-external.com/) and use the drop-down list to select the AWS region your Lambda function is hosted in. From there, locate the ARN that matches your Lambda function's Ruby version and architecture. There should be options, either X86 or ARM64. Use the **Copy to clipboard** button or manually copy the ARN string.
