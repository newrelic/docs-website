...
<Callout variant="important">
  Replace `your-license-key-here` with your New Relic license key, and `otlp.nr-data.net:4317` with the endpoint appropriate for your New Relic region. See the list above.
</Callout>
...
In some cases you may see fragmented distributed traces within New Relic with this configuration. This occurs when a trace starts or involves a service that is outside the ADOT context. When this happens, trace spans are usually created by AWS X-Ray and ADOT is not aware of them beyond a trace context being passed to it. This most commonly happens with managed AWS services that ADOT may interact, but that can only be traced using AWS X-Ray.

New Relic has an [X-Ray integration](https://docs.newrelic.com/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-x-ray-monitoring-integration/) to capture these spans created by X-Ray outside of an ADOT context. However, it has not yet been updated to support OpenTelemetry. Once this integration is updated, you should be able to see a full distributed trace within New Relic using OpenTelemetry.
