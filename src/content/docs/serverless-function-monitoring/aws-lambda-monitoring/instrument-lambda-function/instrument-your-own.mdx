...
<CollapserGroup>
  ...
  >
    ...
    * Java:
      ...
      * **Additionally required for Java**: After setting the handler, go to the **Environment variables** tab and add `AWS_LAMBDA_EXEC_WRAPPER` with the value `/opt/newrelic-java-handler`. This is required for the Java agent layer to intercept invocations and is not needed for other runtimes.
