...
/>
<Callout variant="tip">
A Lambda layer contains respective runtime-specific New Relic agents and the New Relic Lambda extension. The extension is a sidecar process that sends telemetry to New Relic, bypassing CloudWatch. The agent automatically instruments your functions upon invocation, generating a payload, `NR_LAMBDA_MONITORING`, that is sent to New Relic via the extension.
</Callout>
