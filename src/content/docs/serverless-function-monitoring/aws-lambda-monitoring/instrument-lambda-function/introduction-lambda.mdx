...
<table>
    ...
    <tbody>
        <tr>
<td>
  **ONLY Metric Streaming/API Polling + NO Agent/Layer**: If only the metric streaming method is enabled, only CloudWatch-related metric data will be visible.
</td>
<td>
  **Metric Streaming/API Polling + Agent/Layer**: If both methods are enabled, both metric data and Distributed Tracing data will be visible.
</td>
<td>
  **NO Metric Streaming/API Polling + ONLY Agent/Layer**: If only the layer method is enabled, no data will be visible in the UI, as enabling metric streaming is mandatory.
</td>
        </tr>
    </tbody>
</table>
