---
title: "Instrument New Relic with your Lambda function"
metaDescription: 
redirects:
...
This doc will explain the New Relic Lambda monitoring architecture, and walk you through linking your AWS and New Relic accounts. You will need to link your accounts before you instrument your functions. 
...
There are three ways to instrument New Relic with AWS Lambda. How you choose to instrument depends on your data needs. 

    * **Cost**: The most cost-effective way to instrument with AWS Lambda is by using the Lambda extension only. This means that you avoid sending telemetry data to CloudWatch. To do this you will need to disable the extension and use the log ingestion function instead.
    * **Reliability**: Instrumenting with CloudWatch only means that you won't add the New Relic Lambda extension to your function. This means that your function will be lighter and won't have any additional processes to run when its invoked. You also won't send your logs to New Relic.  
    * **Best of both worlds**: AWS CloudWatch automatically collects telemetry data from New Relic. With the extension, you will ingest logs in both CloudWatch and New Relic. 
