...
<table>
    ...
    <tbody>
        <tr>
            <td>
             ONLY Metric Streaming/API Polling + NO Agent/Layer  = If only metric streaming method is enabled then just the cloudwatch related metric data will be visible.
            </td>
            <td>
            Metric Streaming/API Polling  + Agent/Layer = If both methods are enabled then both the metric data as well as Distributed Tracing Data will be visible.
            </td>
            <td>
             NO Metric Streaming/API Polling + ONLY Agent/Layer = If only layer methods is enabled then no data will be visible in the UI as enabling metric streaming is mandatory.
            </td>
        </tr>
    </tbody>
</table>
