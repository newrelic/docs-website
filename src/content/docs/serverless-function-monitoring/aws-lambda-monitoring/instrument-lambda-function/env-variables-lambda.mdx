...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>`true` (pre-set in layer)</td>
            <td>`true`, `false`</td>
            <td>Enables APM Lambda mode, which activates event source detection and enriches transactions with event-specific attributes and naming. Pre-configured to `true` in the layer.</td>
        </tr>
        <tr>
            <td>`NEW_RELIC_LOG_FILE_NAME`</td>
            <td>`stdout` (pre-set in layer)</td>
            <td>`stdout`</td>
            <td>Directs agent logs to stdout so they are captured by CloudWatch Logs. Pre-configured in the layer.</td>
        </tr>
        <tr>
            <td>`NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`</td>
            <td>`true`</td>
