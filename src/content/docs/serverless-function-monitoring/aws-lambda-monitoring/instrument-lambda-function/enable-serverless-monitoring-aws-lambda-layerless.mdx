...
<CollapserGroup>
  ...
  >
   To instrument your Node.js Lambda:
    ...
    10. To complete the instrumentation, add the `newrelic Lambda Extension` by getting Extension only layer arn [(NewRelicLambdaExtension)](https://layers.newrelic-external.com/) and use CLI to install or update the layer.
      ```bash
