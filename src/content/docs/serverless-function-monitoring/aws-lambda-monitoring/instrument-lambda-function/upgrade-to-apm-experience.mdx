...
<Callout variant="tip">
  **Lambda Managed Instance (LMI) users**: No upgrade steps are required. LMI functions are automatically detected and configured to run in APM mode — even without setting `NEW_RELIC_APM_LAMBDA_MODE = true`. Note that Serverless mode is not supported for LMI; only APM mode is available.
</Callout>
...
The **Distributed tracing** tab shows distributed traces that include the monitored Lambda function. For details, see [Distributed tracing](/docs/distributed-tracing/concepts/introduction-distributed-tracing/).
...
The **Logs** tab displays recent log messages from your Lambda function. For details, see [Logs](/docs/logs/new-relic-logs/get-started/introduction-new-relic-logs).
