...
<Callout variant="tip">
  **Lambda Managed Instance (LMI) users**: No upgrade steps are required. LMI functions are automatically detected and configured to run in APM mode — even without setting `NEW_RELIC_APM_LAMBDA_MODE = true`. Note that Serverless mode is not supported for LMI; only APM mode is available. See [Lambda Managed Instance (LMI) support](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/lambda-managed-instance) for full details.
</Callout>
