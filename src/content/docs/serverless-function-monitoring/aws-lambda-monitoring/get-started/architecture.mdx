...
Unfortunately, given the indirect nature of the path your CloudWatch data takes, you may see some delay between when the data is generated and when you see it in New Relic One. CloudWatch log lines take many seconds, sometimes several minutes in cases of high load, to make their way to our log collection function. This delays time to glass, and fogs the view you have of your application at high load times, when an immediate, clear view of application performance is most critical.
