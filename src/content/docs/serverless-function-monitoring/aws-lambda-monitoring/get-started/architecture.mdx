...
By adding a log subscription filter to pipe your function logs into the [`aws-log-ingestion` Lambda function](https://github.com/newrelic/aws-log-ingestion), we can recover that telemetry log line, and forward it on to New Relic, along with some interesting platform telemetry.
