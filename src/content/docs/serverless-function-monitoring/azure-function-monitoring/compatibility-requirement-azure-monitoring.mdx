...
All trigger types will be instrumented, however Distributed Tracing is supported only in the following cases:
...
* .NET Stack:
  ...
  * In-process model (.NET agent version 10.38.0 and later): 
    * HttpTrigger
      * Distributed tracing is supported when the trigger method has a parameter of type `Microsoft.AspNetCore.Http.DefaultHttpRequest`, `Microsoft.AspNetCore.Http.HttpRequest` or `System.Net.Http.HttpRequest`.
    * ServiceBusTrigger:
      * Incoming distributed trace headers will be read if the trigger method has a parameter of type `Azure.Messaging.ServiceBus.ServiceBusReceivedMessage`. Trace headers are expected to be in the `ApplicationProperties` collection of that object.
      * Distributed trace headers will be inserted into the `ApplicationProperties` collection if the trigger method has a return type of `Azure.Messaging.ServiceBus.ServiceBusMessage`.
