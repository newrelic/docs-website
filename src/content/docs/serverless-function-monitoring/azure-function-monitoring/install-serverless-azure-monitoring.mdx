...
<Tabs>
  ...
  <TabsPages>
    ...
    ### Option 1: Use the New Relic Azure Websites Extension

    The New Relic Azure Websites Extension (v1.6.0 and later) automatically installs the latest version of the .NET agent and configures instrumentation for your Windows Azure Function.
    ...
    ### Option 2: Add the `NewRelic.Agent` NuGet package to your project

    1. Add the latest version of the `NewRelic.Agent` NuGet package to your application project.
    2. Use your preferred publishing mechanism to deploy your updated application to Azure. 
      
    When you deploy your application, the .NET agent is installed in the `C:\home\site\wwwroot\newrelic` folder. You will need to configure required environment variables in the next step.
...
After you've instrumented your Azure Function with the New Relic .NET Agent, configure [environment variables](/docs/serverless-function-monitoring/azure-function-monitoring/env-variables-azure):
  ...
  2. Based on your deployment environment, paste the following values.
...
<Tabs>
  ...
  ### If using the New Relic Azure Websites Extension:
  ```json
  {
    "name": "NEW_RELIC_LICENSE_KEY",
    "value": "<your newrelic license key here>",
  ...
  ### If using the NewRelic.Agent Nuget package:
    ```json
  {
    "name": "CORECLR_ENABLE_PROFILING",
    "value": "1",
