...
<Collapser id="filter-by-transaction" title="Filter by transaction">
 ...
 3. The map redraws showing only traces that include that transaction name.
...
<Collapser id="time-picker" title="Adjust the time window">
 ...
 3. The map updates to display trace data for the selected time range. If you select a time window longer than 3 hours, the map automatically uses the most recent 3 hours within that window.
