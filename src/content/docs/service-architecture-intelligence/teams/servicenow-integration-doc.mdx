...
1. **Create Properties in ServiceNow.**
    ...
    2. Click **New**, and set the value to `True`, and submit the property.
...
7. **Create Signing Key Property.**
    ...
    2. Click **New**, set the type to **Password**, set the name (e.g., **nr_signing_key_real_time_events**), and set the value to a unique **UUID**.
...
8. **Define RESTful Communication.**
    ...
    3. Create an HTTP method by setting the authentication type to **No Authentication**, setting the endpoint, setting the method to **POST**, and submitting the method.
...
10. **Finalize Integration.**
    ...
    1. Provide the Client ID and Client Secret from the application registry, the instance name (e.g., `google.servicenow.com`), and the real-time signing key (UUID created).
