...
1. Add the following code to your `build.gradle` files found in the Gradle & Android Studio tab. You need to update both files. 
...
5. If you're using ProGuard or Dexguard, you need to add another `newrelic.properties` file to your app-level directory (projectname/app). This step only applies to ProGuard and DexGuard users.
...
Because the Android agent is not yet available as a community plugin, you need to provide the classpath through [MavenCentral](https://mvnrepository.com/artifact/com.newrelic.agent.android/agent-gradle-plugin) so the agent can monitor your Android app. If your app is configured to look for plugins through Gradle Plugin Portal, you need to repeat this step in the `settings.gradle(.kts)` file. 
...
* You can [configure ProGuard or DexGuard for Android apps](/docs/mobile-monitoring/new-relic-mobile-android/install-configure/configure-proguard-or-dexguard-android-apps).
