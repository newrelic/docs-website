...
2. **(Optional)** For ProGuard or DexGuard: 
  1. In your project’s root directory (`projectname/app`), add a `newrelic.properties` file with the following line:

    ```
    com.newrelic.application_token=<var>GENERATED_TOKEN</var>
    ```
  2. Complete the configuration steps on [Configure ProGuard or DexGuard for Android apps](docs/mobile-monitoring/new-relic-mobile-android/install-configure/configure-proguard-or-dexguard-android-apps). Then come back to this page and move on to the next step. 
