...
Your app launch time is reported with the `AppLaunch/Cold` and `AppLaunch/Hot` [metrics](https://docs.newrelic.com/docs/data-apis/understand-data/new-relic-data-types/#metrics). To track these launch time metrics, you can use the **Query builder** to create a custom dashboard. Here's a sample query: 
