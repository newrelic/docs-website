...
To enable the Android agent to report app launch time, addd the following to your application's `AndroidManifest.xml`:
...
1. Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Capabilities > Query builder**</DoNotTranslate>.
2. Run a [NRQL query](/docs/query-your-data/explore-query-data/query-builder/use-advanced-nrql-mode-query-data/), such as:
    ...
    ```
3. Click <DoNotTranslate>**Add to dashboard**</DoNotTranslate> and either add it to an existing dashboard or create a new one. 
