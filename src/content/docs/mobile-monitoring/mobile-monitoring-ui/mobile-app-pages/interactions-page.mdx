...
The Interactions page includes breakdown charts of top 20 most expensive interactions (including code executing on the main thread and code executing on worker threads), plus a list of the slowest traces.
...
1. Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > App > Interactions > (select an interaction)**.
