...
* **Automatic collection:** The iOS agent can automatically capture logs sent to standard output (`stdout`) and standard error (`stderr`). You must enable the `NRFeatureFlag_AutoCollectLogs` feature flag in your app's initialization code for automatic log colletion.
