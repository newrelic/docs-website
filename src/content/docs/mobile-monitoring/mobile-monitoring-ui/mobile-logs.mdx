...
6. Click <DNT>**Save**</DNT>.
  ...
  * **iOS Agent Requirement:** Automatic collection is **off by default**. To enable it:
      1. You must enable the `NRFeatureFlag_AutoCollectLogs` feature flag in your app's initialization code.
      2. The <DNT>**Mobile Logs**</DNT> toggle is turned **On** in the New Relic UI.
...
* **Automatic collection:** The iOS agent can automatically capture logs sent to standard output (`stdout`) and standard error (`stderr`). You must enable the `NRFeatureFlag_AutoCollectLogs` feature flag in your app's initialization code for automatic log collection.
