...
New Relic will store unsymbolicated stack traces for handled exceptions. You can view symbolicated stack traces if the appropriate `.dsym` or Android mapping files have been uploaded at build time for native iOS and Android apps. You can also [upload a dSYM](/docs/mobile-monitoring/new-relic-mobile-ios/install-configure/upload-dsyms-bitcode-apps) later for iOS apps as needed. 

New Relic supports on-demand symbolication for [Flutter](/docs/mobile-monitoring/new-relic-mobile-flutter) and [.NET MAUI](/docs/mobile-monitoring/new-relic-mobile-maui-dotnet) apps. This is an in-browser JavaScript based solution. Symbolicated stack traces are not persistent and are not stored by New Relic. Flutter Dart errors require a `.symbol` file for symbolication. .NET MAUI C# errors require both a `.pdb` and `.dll` for proper symbolication.
