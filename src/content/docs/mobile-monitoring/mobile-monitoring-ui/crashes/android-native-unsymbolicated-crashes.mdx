...
tags:
  ...
  - Crashes
metaDescription: 'How to use the Build UUID identifier with New Relic Mobile to identify a unique build of an app, and to find which symbol file to use to symbolicate an Android native crash.'
redirects:
  - /docs/mobile-monitoring/new-relic-mobile-android/troubleshoot/finding-build-uuids-unsymbolicated-crashes
  - /docs/mobile-monitoring/new-relic-mobile-android/troubleshoot/find-build-uuids-unsymbolicated-crashes
---

# [Incubating]

If the app's native symbols are not uploaded to New Relic, Android native  [mobile crash reports](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report) cannot be symbolicated. You can [upload native symbols](/docs/mobile-monitoring/new-relic-mobile-ios/install-configure/upload-android-native-symbols) manually or directly through the New Relic Mobile UI. However, every build of an application will create a new symbols, and it can be difficult managing which is attributed to which app build.

In this situation, you can use the Build UUID identifier to:

* Identify a unique build of an application.
* Find which symbole file can be used to symbolicate a crash.
...
If the `Android Native Build UUID` is not on the selected crash report's attributes list:
...
Use the Build UUID extracted from the crash details to find the missing symbol file.
...
* [Manual dSYM upload](/docs/mobile-monitoring/new-relic-mobile-ios/install-configure/ios-agent-crash-reporting) (how to manually upload dSYMs)
* [Retrieve and upload dSYMs](/docs/mobile-monitoring/new-relic-mobile-ios/install-configure/retrieve-upload-dsyms) (how to locate dSYMs from past app releases)
