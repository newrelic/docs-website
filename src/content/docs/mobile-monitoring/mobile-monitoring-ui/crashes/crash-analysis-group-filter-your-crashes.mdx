...
* Pinpoint when most crashes occur; for example, by [filtering by `Last Interaction`](#groups-filters), or by viewing the [event trail](#export-xcode) in the [**Crash type** summary](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report).
...
<CollapserGroup>
  ...
  >
    ...
    * File a ticket and resolve the crash.
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > Crashes > Crash type > (select a crash) > **(scroll down to **Stack trace**)**: This is an example of an stack trace that includes the option to resymbolicate the crash occurrence. To analyze and debug your source code using Xcode, select **Export crash details**.
      </figcaption>
  </Collapser>
</CollapserGroup>
