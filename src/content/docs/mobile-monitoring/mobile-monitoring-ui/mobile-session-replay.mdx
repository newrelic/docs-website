  ...
  <TabsPages>
    <TabsPageItem id="ios">
      ...
      **SwiftUI**
      ...
      If there is ever an instance where text is missing from the replay, please wrap the entire screen region in an `NRConditionalMaskView` without parameters to fix the issue:
      ...
      **Parameters:**
      ...
      - `blockView`: When `true`, The entire view area is obscured in the session replay. When `false` there is no blocking behavior.
