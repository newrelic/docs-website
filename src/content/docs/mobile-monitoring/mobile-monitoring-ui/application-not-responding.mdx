...
In Android mobile applications, an [Application Not Responding (ANR) errors](https://developer.android.com/topic/performance/vitals/anr) occurs when the main UI thread is blocked for more than five seconds. These errors significantly degrade the user experience. These errors can be caused by several factors, including:
...
* **Assess overall error trends:** Review ANR rate charts by sessions and users to spot anomalies or patterns in ANR occurrences. 
    ...
    * **ANR rate by users:** Shows the percentage of unique users who experienced an ANR during a period, relative to the total number of unique users. Unique users are typically identified by a `deviceUuid`.
