...
To change how ANRs are grouped (such as device type, app version, or a [custom attribute](/docs/data-apis/custom-data/custom-events/collect-custom-attributes/)), click the **Group by** and select an attribute from the list. The ANR page will update to show data for the new ANR group.
...
The **ANR occurrences by importance** chart shows the distribution of ANRs by relative importance of the app’s process before termination due to an ANR. More information about an app’s process importance can be found [here](https://developer.android.com/reference/android/app/ApplicationExitInfo#getImportance().
