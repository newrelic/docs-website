...
Before you get started, you'll need to add the Unity agent into your project. In Unity package manager, add the following github URL to `install newrelic unity` package:
...
3. Go to **Settings > Application** and copy the displayed **Application token**.
 ...
 To add the application token:

1. In your Unity IDE click **Tools **
2. Click **NewRelic**
3. Then click **Getting Started** to open the New Relic editor window
4. Paste your application token(s) into `appToken = ""`

Deploying your Unity app to both iOS and Android requires adding two separate tokens, one for each platform.
...
3. Apply the `newrelic` plugin to the top of your `launcherTemplate.gradle` file. This file is typically located in the Android folder in your Unity project.
...
1. In your Unity IDE, navigate to **Assets**
2. Then open **External Dependency Manager**
3. Click  **iOS Resolver**
4. Settings: [#configure-ios]

In the Podfile Configurations section, make sure the following settings are correct.
