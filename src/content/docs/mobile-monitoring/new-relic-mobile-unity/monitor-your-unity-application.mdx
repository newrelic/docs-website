---
title: Monitor your Unity application
tags:
    - Unity
    - Mobile monitoring
...
First, you'll need to add the Unity agent into your project. In Unity package Manager , add the following github url to install newrelic unity package : 
...
access to several collections of packages at the same time. NewRelic uses Scoped Registries to allow our users to manage, 
download and install our SDK using the built-in Unity Package Manager.
</Step>
<Step>
### In Your Unity Editor [#app-token]
The application token is used for New Relic to authenticate your Unity app's data. 
...
<img
  ...
  style={{ maxWidth: '70%' }}
/>
Make sure you paste your application token(s) into `appToken = ""` in the code above. If you deployed your Unity app to both iOS and Android platforms, you'll need to add two separate tokens: one for iOS and one for Android.
</Step>
<Step>
...
```groovy
 ...
 ```
3. 2. Apply the `newrelic` plugin to the top of your `launcherTemplate.gradle` file(typically located in the android folder in your Unity project):
...
<Step>
### (iOS-only) Please ensure that your External Dependency Manager settings match the following configuration. 

In your Unity IDE, navigate to Assets → External Dependency Manager → iOS Resolver → Settings: [#configure-ios]
