...
1. Download and unzip the [iOS SDK for New Relic](https://docs.newrelic.com/docs/release-notes/mobile-release-notes/ios-release-notes).
2. To add the New Relic framework to your Xcode project: Use **Finder** to drag the `NewRelicAgent.framework` folder into your Xcode project, and drop it onto your project in the **Project navigator** window.
...
6. Add a build script to your target's **Build Phases**. Ensure the new build script is the very last build phase, then paste the following, replacing <var>PUT_NEW_RELIC_APP_TOKEN_HERE</var> with your application token:
