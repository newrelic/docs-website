...
7. From the downloaded repo, copy the `dsym-upload-tools` and into the folder that contains your `xcodeproject` file. You should now have a folder containing these files:
...
6. Add a build script to your target's build phases. Ensure the new build script is the very last build script. Then paste the following, replacing `APP_TOKEN` with your [application token](/docs/mobile-apps/viewing-your-application-token):
 ...
 - OPTIONAL: Add the following lines to your build script above the existing lines to skip symbol upload during debugging:
    ...
    ```
7. For iOS agent 7.4.6 and higher, add the following build script to perform symbol upload steps during app builds in Xcode:
...
8. Clean and build your app, then run it in the simulator or on another device.
