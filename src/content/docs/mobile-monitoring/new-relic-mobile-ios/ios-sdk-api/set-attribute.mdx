...
NewRelic.setAttribute(<em>string</em> $name, <em>string or float</em> $value)
```
...
This static method creates a session-level custom [attribute](/docs/insights/explore-data/custom-events/insert-custom-attributes-events-new-relic-mobile#attribute-definition) with a specified name and value. When called, it overwrites its previous value and type. The created attribute is shared by multiple [Mobile event types](/docs/insights/insights-data-sources/default-data/mobile-events).
