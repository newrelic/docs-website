...
If you want your method’s performance to be included in the summary data on the <InlinePopup type="apm" /> **Overview** page, pass one of the `NRTraceType` enum values into the `_START` macro; for example:
...
If you want your method’s performance to be included in the summary data on the <InlinePopup type="apm" /> **Overview** page, pass one of the `NRTraceType` enum values into the `startTracingMethod()` macro; for example:
