...
Starting with Xcode 14, Xcode no longer builds bitcode by default and generates a warning message if a project explicitly enables bitcode. The [Xcode 14 release notes](https://developer.apple.com/documentation/xcode-release-notes/xcode-14-release-notes) goes on to say that "the capability to build apps with bitcode will be removed in a future update." So, going forward, we recommend moving away from using bitcode in your iOS projects. Instead, to capture symbolicated crash stacks from release builds, you should use the [automatic symbol uploading script] (/docs/mobile-monitoring/new-relic-mobile-ios/installation/spm-installation#configure-using-swift-package-manager) to upload your build's symbols.
  </Callout>
