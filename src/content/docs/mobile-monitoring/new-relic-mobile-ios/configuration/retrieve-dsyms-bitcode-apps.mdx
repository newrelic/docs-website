...
Apple deprecated bitcode in Xcode 14. The [Xcode 14 release notes](https://developer.apple.com/documentation/xcode-release-notes/xcode-14-release-notes) clearly state that the capability to build apps with bitcode will be removed in a future update.
This means that all projects should migrate away from using bitcode. This means that the automatic symbol uploading script can now be used to upload your builds symbols for all release builds you want to capture symbolicated crash stacks from.
...
2. Select **Apps > (selected app) > TestFlight**.
