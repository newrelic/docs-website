---
title: Adding a prefix header to an iOS project
tags:
  - Mobile monitoring
  - New Relic Mobile iOS
  - Configuration
metaDescription: 'If your iOS project does not include a prefix header, follow these steps to add one with the iOS SDK API.'
redirects:
  - /docs/mobile-monitoring/mobile-monitoring-installation/ios/adding-prefix-header-ios-project
  - /docs/mobile-monitoring/new-relic-mobile-ios/install-configure/adding-prefix-header-ios-project
---

Installing New Relic's iOS SDK API requires you to modify your project's prefix header. If your project does not have one, follow these steps to add it.

1. From your Xcode menu, select **File > New > File...**
2. From **iOS** template options, select **Other > PCH file**.
3. Name the file **&lt;target_name>-Prefix.pch**, and then select **Create**.
4. From your target’s **Build settings**, select **All**, and then add the following to the `Prefix Header` field:

   ```
   $(SRCROOT)/<target_name>-Prefix.pch
   ```

![Prefix Header file for iOS SDK API projects](src/images/screen-ios-sdk-api-prefix-header.png 'Prefix Header file for iOS SDK API projects')

<figcaption>
  Here is an example of using Xcode to add a Prefix Header file to your iOS
  project for the iOS SDK API.
</figcaption>
