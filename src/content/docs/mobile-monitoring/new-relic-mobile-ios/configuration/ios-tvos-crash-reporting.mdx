...
A <DoNotTranslate>**dSYM file**</DoNotTranslate> is an Xcode project file for <DoNotTranslate>**debug symbols**</DoNotTranslate>. It contains the debugging symbols that allow for translation of the initial crash report to human-readable information. This process is known as symbolication.
...
* <DoNotTranslate>**If the app is running in a simulator:**</DoNotTranslate> An app running through the simulator cannot be used with crash reporting. The simulator crashes cannot be symbolicated.
* <DoNotTranslate>**If the debugger is enabled:**</DoNotTranslate> There can only be one uncaught exception handler registered at a time per application. If running with the debugger attached, New Relic will not capture and report crashes.
* <DoNotTranslate>**If another crash reporter is enabled:**</DoNotTranslate> If another uncaught exception handler is registered after New Relic starts, this error message is logged:
