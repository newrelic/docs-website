...
2. In the config file above, replace `<APP-TOKEN-HERE>` with your custom application token(s) (copied from [Step 3]. If you're monitoring both Android-native and iOS-native apps, you'll need to add two different application tokens.
...
We recommend you call this method along when starting the agent. These events will only be recorded after navigation is complete. You can find this data through the data explorer in `MobileBreadcrumb` under the name `ShellNavigated`, or by a query such as:
