...
* <DNT>**Capture and symbolicate C# errors:**</DNT> Identify and fix problems quickly.
...
## Symbolicate C# errors in Errors Inbox [#symbolicate]

New Relic provides in browser symbolication for C# error stack traces to make them human-readable. This symbolicated data is not persistent, but can be exported from the browser to a text file for further debugging. .NET MAUI C# error symbolication requires both a `.pdb` and a `.dll` file.
