...
Monitor your .NET MAUI mobile app with our New Relic .NET MAUI agent. Capture C# errors, track specific network requests, use distributed tracing to drill down into handled exceptions, and create custom tracking events and metrics to fully capture your end users' experience.
...
First, you'll need to add the agent, a NuGet package, to your MAUI project:
...
<Step>
### Add the agent configuration file to your .NET MAUI project
...
2. In the config file, replace `<APP-TOKEN-HERE>` with your custom application token(s) (copied from [Step 3]. If you're monitoring both Android-native and iOS-native apps, you'll need to add two different application tokens.
...
New Relic stores your app logs as custom events, which you can query and build dashboards for using the following NRQL query:
```
...
## (Optional) Customize the agent instrumentation
...
We recommend to call this method along when starting the agent. These events will only be recorded after navigation is complete. You can find this data through the data explorer in `MobileBreadcrumb` under the name `ShellNavigated`, or by a query such as:
