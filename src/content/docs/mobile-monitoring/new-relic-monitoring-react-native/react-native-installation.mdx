...
With our React Native agent, your team can monitor your hybrid app's performance and identify any errors within your code on both iOs and Android. Our agent collects crash data, network traffic, and other information for hybrid apps using native components. This allows your team to better understand the health of your hybrid app regardless of the platform. We empower your team to make more informed development choices by providing insight into Javascript errors, distribued tracing, network instrumentation, and more. 
...
```
  ...
  <Collapser
    ...
    title="Android setup"
  >
In order to ensure that the React Native agent is compatible with Android you need to install the Android specific agent:
...
4. Make sure your app requests `INTERNET` and `ACCESS_NETWORK_STATE` permissions by adding these lines to your `AndroidManifest.xml`:
  ...
  <Collapser
    ...
    title="iOS setup"
  >
In order to ensure that the React Native agent is compatible with all iOS you need to install the New Relic XCFramework agent:
