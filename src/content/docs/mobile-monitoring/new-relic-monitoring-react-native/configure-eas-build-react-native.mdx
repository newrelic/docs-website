...
* Adds a <DNT>**Run Script**</DNT> build phase named "Upload dSYMs and Source Maps to New Relic" (after "Bundle React Native code and images") that invokes them, reading credentials from environment variables at build time — the generated script only references the credentials by name and never writes them to a file.
