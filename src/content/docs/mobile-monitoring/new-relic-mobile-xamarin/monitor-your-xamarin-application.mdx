...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > Summary**</DoNotTranslate>: View Xamarin data, track HTTP requests and errors, and monitor how your app is performing over time.
</figcaption>
...
2. Click <DoNotTranslate>**Add > Add NuGet packages**</DoNotTranslate>, then select `NewRelic.Xamarin.Plugin`.
...
2. <DoNotTranslate>**Add > Add NuGet packages**</DoNotTranslate>, then select `NewRelic.Xamarin.iOS.Binding`.
</Step>
<Step>
...
After installing the .NET MAUI agent, wait at least 5 minutes. If <DoNotTranslate>**No HTTP data appears:**</DoNotTranslate> on the HTTP errors and HTTP requests UI pages, make sure you used `HttpMessageHandler` in `HttpClient`.
