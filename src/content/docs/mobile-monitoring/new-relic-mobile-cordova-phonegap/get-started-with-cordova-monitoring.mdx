...
import mobileQueryExample from 'images/mobile_screenshot-full_hybrid-summary.png'

The official New Relic Cordova plugin for iOS and Android allows developers to easily embed the mobile agents into [Cordova applications](http://cordova.apache.org/). Written in JavaScript, the plugin automatically includes New Relic's native agents to provide mobile monitoring and performance visibility.

## Features

Once installed, the New Relic Cordova plugin will:
...
First, you'll need to get your application token, which will be used in [Step 3](#install) to connect your Cordova app with New Relic:

  1. Go to **one.newrelic.com > Browser & Mobile > (choose Android or iOS)**, then complete the instrumentation steps in the UI. 
  2. From Step 4 in the UI, copy the application token. You’ll need this when installing the Cordova plugin.
 
  <Callout variant="important">
  If your app is built on both iOS and Android, you’ll need to get application tokens for iOS and Android separately.
  </Callout> 

### Step 3: Install the Cordova plugin [#install]

1. In your Cordova project directory, add the Cordova plugin to your project with the Cordova command line tool, and paste the application token(s) (from step 2) using the `--variable` argument:
...
2. (Optional) If you're using ionic install, add this:
...
<img
  title=""
  alt=""
  src={mobileCordovaAppOverview}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (select a Cordova app)**: The Cordova plugin will collect this data and send it to New Relic, where you can view and analyze it in the UI.
</figcaption>

### (Optional) Step 5: Track JavaScript errors

You can configure the Cordova Plugin to create custom events for JavaScript errors and report them to New Relic by adding `NewRelic.recordError(error);` to your project:
...
* Use API calls to track custom data: Our Cordova Plugin utilizes the same API calls as our iOS and Android SDK agents. See [iOS SDK](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api) or [Android SDK](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api).
* Update your Cordova plugin version: If you need to update your Cordova plugin to the latest version, using the run this simple command:
