...
<img
  title="handled exceptions and event trails"
  alt="view handled exceptions and their event trails in New Relic"
  src={mobileflutterHandledException}
/>

<figcaption>
    **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile >** (select an app) **> Handled exceptions >** (click on a handled exception): Drill down into stack traces and event trails to help troubleshoot handled exceptions.
</figcaption>

With the Flutter agent, you have access to all mobile monitoring features, such as tracking crashes, Http requests, and handled exceptions. Check out these key features: 
* Distributed tracing for [iOS](/docs/mobile-monitoring/new-relic-mobile-ios/get-started/new-relic-ios-and-dt/)[Android](/docs/mobile-monitoring/new-relic-mobile-android/get-started/new-relic-android-and-dt/)
...
For more info on NRQL queries, see [Introduction to NRQL](docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language/#where).
...
1. In order to track navigation events, you'll need to add the `NewRelicNavigationObserver` to your MaterialApp, WidgetsApp, or CupertinoApp.
...
2. Add a name to route settings: `RouteSettings(name: 'Your Route Name')`. The root route name / will be replaced by root "/" for clarity's sake:
