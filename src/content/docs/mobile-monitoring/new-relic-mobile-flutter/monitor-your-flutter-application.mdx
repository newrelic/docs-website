...
* <DNT>**Capture and symbolicate Dart errors:**</DNT> Identify and fix problems quickly.
...
## Symbolicate Dart errors in Errors Inbox [#symbolicate]

New Relic provides in browser symbolication for Dart error stack traces to make them human-readable. This symbolicated data is not persistent, but can be exported from the browser to a text file for further debugging. Flutter Dart error symbolication requires a `.symbol` file.
