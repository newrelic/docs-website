...
<Steps>
  ...
  If your projec is using Plugins DSL (Flutter 3.16 or Later) to Apply the NewRelic Gradle Plugin, make the following changes:
  ...
  Or, ifyour projec is using the traditional way to apply the plugin:
    ```groovy
    buildscript {
      ...
      repositories {
    ...
    2. Add `INTERNET` and `ACCESS_NETWORK_STATE` permissions in your `AndroidManifest.xml` file:
       ```dart
