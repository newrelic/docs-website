...
import mobileNewStatusRule from 'images/mobile_screenshot-crop_new-status-rule.png'

import mobileCreateAlias from 'images/mobile_screenshot-crop_create-alias.png'

import mobileAllowHostname from 'images/mobile_screenshot-crop_allow-hostname.png'
...
3. Enter the hostname `*.ecommerce.com` and click **Add to list**. Wait up until 2 hours for the rule to be applied.
<img
  title="Allow a hostname"
  alt="Allow a hostname"
  src={mobileAllowHostname}
    />

## Ignore HTTP status codes [#ignore-status-codes]

You can ignore specific HTTP error status code on a hostname. For example, if you want to ignore `401`, `403`, and `404` error messages from all `ecommerce.com` subdomains:

1. On the **Network settings page**, select the **Status codes** tab.
2. Click **Create a status code rule**. Here's an example:
3. Enter a host, such as mycommerce.com/checkout.
4. Enter a status code(s) and click **Add to list**. Wait up until 2 hours for the rule to be applied.
<img
...
You can create aliases to do things like rename host names that make the most sense to you. For example, if you want to create an alias that groups all subdomains of ecommerce.com:
...
4. Enter the alias `ecommerce.com` and click **Save alias**. Wait up until 2 hours for the rule to be applied.
 <img
