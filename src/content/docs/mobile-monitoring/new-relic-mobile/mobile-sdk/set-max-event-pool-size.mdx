...
```
      ...
      <Callout variant="important">
        The minimum allowed value for `size` is `64`. If you pass a value lower than `64`, the agent resets it to `64` and logs a warning. You can set values higher than the recommended maximum of `1000`, but the agent logs a warning when you do.
      </Callout>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>
              Required. The maximum number of events that can be buffered in memory before they are sent to New Relic. Must be at least `64`; values lower than `64` are reset to `64`. Values above the recommended maximum of `1000` are allowed but will log a warning.
            </td>
          </tr>
        </tbody>
      </table>
