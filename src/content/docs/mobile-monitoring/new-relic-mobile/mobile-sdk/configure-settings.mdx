...
## Event persistence [#android-event-persistence]
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              Enable or disable event persistence, which lets the agent capture events during force-close and process-exit scenarios.
            </td>

            <td>
              Event persistence is enabled by default. To disable it, add the following feature flag:
...
```
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              Enable or disable offline data storage when no internet connection is available. When an event is added while the internet is not reachable by the app an [`offline` attribute](/attribute-dictionary/?event=MobileSession&attribute=offline) is added to the event. To configure the amount of offline storage, see [Set max offline storage size](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/set-max-offline-storage/#ios).
            </td>
...
```
          ...
          <tr>
            <td>
              Enable or disable automatic reporting of JavaScript errors as [`MobileJSError` events](/attribute-dictionary/?event=MobileJSError). When enabled, the agent captures unhandled JavaScript errors and reports them without any additional instrumentation.

              Default value is `true`. Available for React Native agent version 1.9.0 or higher.
            </td>
