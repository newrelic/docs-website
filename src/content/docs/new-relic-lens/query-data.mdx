...
<CollapserGroup>
  ...
  <Collapser id="groupby" title="Specify `SELECT` before `FROM`">
  ...
  <Collapser id="multi-series-line-chart" title="Show multiple metrics as separate lines on a chart">
    To plot several metrics as separate lines on the same chart, pivot each metric into its own set of `(day, series, value)` rows using `UNION ALL`, then combine them with a final query. Two details matter for the chart to render correctly:

    * Cast the day column to an actual `DATE` type instead of leaving it as a string.
    * Order by the series column first, then by day. Ordering by day first can cause <DNT>Lens</DNT> to connect points across series in the wrong sequence.

    ```sql
    WITH daily_totals AS (
      SELECT
        CAST(date(timestamp) AS DATE) AS day,
    ...
    SELECT day, 'Total transactions' AS series, total_count AS value
    FROM daily_totals
    UNION ALL
    SELECT day, 'Errors' AS series, error_count AS value
    FROM daily_errors
    ORDER BY series, day
    ```
  </Collapser>
