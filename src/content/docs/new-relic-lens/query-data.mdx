...
* Verify you have the necessary [permissions](/docs/new-relic-lens/set-up-connectors#permissions) to query data sources.
...
<CollapserGroup>
  ...
  <Collapser id="line-chart-ordering" title="Group and order data for a line chart">
    Unlike NRQL, which returns pre-bucketed time series data, <DNT>Lens</DNT> returns query results as a flat table. A query can return the expected rows in table view and still render as a broken line or area chart if the data isn't bucketed and ordered for a timeline.

    Group your timestamp into day-level buckets using `date_trunc()`, and order the results ascending by that same bucket expression. If the dataset isn't ordered by the time bucket, the chart connects points out of sequence.
