...
* **Fine-grained access control**: Users without broad RBAC permissions can still query specific <DNT>connectors</DNT> if granted fine-grained access to those <DNT>connectors</DNT>. 
...
<CollapserGroup>
  ...
  <Collapser id="iceberg-connector" title="Iceberg connector">
    ...
    <DNT>Lens</DNT> auto-generates the external ID for this <DNT>connector</DNT> after you create it. You don't enter an external ID when configuring the <DNT>connector</DNT>. See [Grant <DNT>Lens</DNT> access to your AWS resources](#grant-iceberg-access) for the full setup flow.
    ...
    <Steps>
      ...
      <Step>
        ...
        * **Permissions policy**: Search for the policy name you created in the previous step and select it.
        ...
        The trust policy should match the following format. Replace `<YOUR_EXTERNAL_ID>` with your temporary placeholder for now.
        ...
        After creating the role, open the role's summary page and copy the **Role ARN**. You'll need this ARN to configure the <DNT>Lens</DNT> <DNT>connector</DNT>.
      </Step>

      <Step>
        ### Update the trust policy with the generated external ID [#update-iceberg-external-id]

        After creating the role, complete the setup:

        1. In <DNT>Lens</DNT>, go to <DNT>**Administration > Connectors**</DNT> and create the <DNT>Iceberg</DNT> <DNT>connector</DNT> using the **Role ARN** from the previous step. <DNT>Lens</DNT> auto-generates an external ID for the <DNT>connector</DNT>.
        2. Open the <DNT>connector's</DNT> detail view and copy the auto-generated external ID.
        3. In AWS, edit the IAM role's trust policy and replace the temporary placeholder value in `sts:ExternalId` with the external ID you copied from <DNT>Lens</DNT>.
      </Step>
    </Steps>
  </Collapser>
  ...
  <Collapser id="cloudwatch-connector" title="AWS CloudWatch connector">
    ...
    <DNT>Lens</DNT> auto-generates the external ID for this <DNT>connector</DNT> after you create it. You don't enter an external ID when configuring the <DNT>connector</DNT>. See [Grant <DNT>Lens</DNT> access to CloudWatch](#grant-cloudwatch-access) for the full setup flow.
    ...
    <Steps>
      ...
      <Step>
        ...
        1. In <DNT>Lens</DNT>, go to <DNT>**Administration > Connectors**</DNT> and create the <DNT>AWS CloudWatch</DNT> <DNT>connector</DNT> using the **Role ARN** from the previous step. <DNT>Lens</DNT> auto-generates an external ID for the <DNT>connector</DNT>.
        2. Open the <DNT>connector's</DNT> detail view and copy the auto-generated external ID.
        3. In AWS, edit the IAM role's trust policy and replace the temporary placeholder value in `sts:ExternalId` with the external ID you copied from <DNT>Lens</DNT>.
      </Step>
    </Steps>
