...
<CollapserGroup>
  ...
  >
    ...
    * Because mTLS is mutual, the monitor must also trust the server. If the endpoint's certificate is issued by a private CA, mount that CA as a [custom CA certificate (#custom-ca-certificates) (Docker: `/var/lib/newrelic/synthetics/certs`; Kubernetes: `global.customCertificates.volume.secret.secretName`).
...
**Problem it solves:** A private location created in one account isn't usable by other teams by default, so each team stands up its own job managers for the same network — duplicating infrastructure, cost, and maintenance. Sharing lets one location serve many accounts.
