...
<CollapserGroup>
  ...
  >
    ...
    * Because mTLS is mutual, the monitor must also trust the server. If the endpoint's certificate is issued by a private CA, mount that CA as a [custom CA certificate](#custom-certs) (Docker: `/var/lib/newrelic/synthetics/certs`; Kubernetes: `global.customCertificates.volume.secret.secretName`).
