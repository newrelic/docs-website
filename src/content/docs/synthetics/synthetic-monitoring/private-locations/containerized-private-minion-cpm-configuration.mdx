    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Format: `"vX.Y"` API version to be used with the given <DNT>Docker/DNT> service.
        ...
        <tr>
          ...
          <td>
            The <DNT>Docker/DNT> Registry and Organization where the Minion Runner image is hosted. Use this to override `quay.io/newrelic` as the default (for example, `docker.io/newrelic`).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            The AppArmor profile name, if it has been applied to <DNT>Docker/DNT> containers running monitor scripts (for example, <DNT>Docker/DNT> Runner). The AppArmor profile name must exist and be set up on the machine to work.
          </td>
        </tr>
...
2. Once you create the custom modules directory and the `package.json` you can apply it to your CPM for <DNT>Docker/DNT> and Kubernetes.
   ...
   <CollapserGroup id="apply-to-docker-kubernetes">
     <Collapser
       ...
       title="Docker"
     >
       For <DNT>Docker/DNT>, launch CPM mounting the directory at `/var/lib/newrelic/synthetics/modules`. For example:
...
To set permanent data storage on <DNT>Docker/DNT>:
