---
title: Synthetic monitoring job manager transition guide
tags:
  - Synthetics
...
Job managers deploy your synthetics from runtime-specific container images tailored to the jobs you want to deploy. While CPMs bundled legacy runtimes and artifacts into one growing file, our job manager instead separates our Chrome, Node, and job management work into separate, smaller images.
...
Unlike CPMs, which used file sharing to coordinate jobs in private locations, job managers [communicate via HTTP](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) with web services. This removes certain requirements needed to deploy jobs, such as specifying mounted volume in your docker configuration or persistent volume in your Kubernetes configuration. 
...
You can configure both CPMs and the job managers with the same private location key, but CPMs will execute monitors that use the legacy runtime and job managers will deploy monitors that use the newest runtime. If you'd rather start fresh, you can create a new private location for use with synthetics job manager. Your private location configuration and migration path comes down to your own personal preference.
