...
Installing job manager with the same private location key as your existing CPM will simplify the migration process. CPMs will execute monitors that use the legacy runtime, job managers will execute monitors that use the newest runtime, and ping monitors will be load balanced and executed by both the CPM and job managers. 
