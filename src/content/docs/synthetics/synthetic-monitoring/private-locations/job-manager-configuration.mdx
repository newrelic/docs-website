...
If your synthetic monitors need to connect to internal HTTPS endpoints secured by a private Certificate Authority (CA), you can mount your custom CA certificates directly into the Synthetics Job Manager (SJM).
...
Select the tab or section below that matches your container orchestration platform to mount your certificates.
...
<CollapserGroup>
  <Collapser id="custom-ca-docker" title="Docker">
    ...
    **How it works:** The SJM detects the mount at startup and injects the certificates as read-only into every dynamic runtime container it spawns.
  </Collapser>
  ...
  <Collapser id="custom-ca-podman" title="Podman">
    ...
    ```sh
    podman run \
      -v /path/to/ca-certs:/var/lib/newrelic/synthetics/certs:ro,z \
      newrelic/synthetics-job-manager
    ```
  </Collapser>
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes (Helm)">
    ...
    **How it works:** When the SJM pod starts up, the secret is automatically mounted read-only and propagated down to all newly created runtime execution pods.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="docker-storage" title="For Docker">
...
<CollapserGroup>
  <Collapser
    id="docker-podman-sizing"
    title="For Docker and Podman"
  >
...
<Collapser
  id="k8s-sizing"
  title="For Kubernetes and OpenShift"
>
