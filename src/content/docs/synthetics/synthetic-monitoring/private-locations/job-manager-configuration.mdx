...
The `parallelism` setting controls how many pods of a particular runtime (node-api-runtime or node-browser-runtime) can run concurrently. The `parallelism` setting is the equivalent of the `synthetics.heavyWorkers` configuration in the containerized private minion (CPM). Ensure that your Kubernetes cluster has enough resources available to run this number of pods based on their [resource request and limit values](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#kubernetes-requirements).

The `completions` setting controls how many pods of a particular runtime (node-api-runtime or node-browser-runtime) must complete before the `CronJob` can start another job for that runtime. For improved efficiency, `completions` should be set to 6-10x the `parallelism` value. This can help to minimize the "end of completions effect" where fewer pods might be running than specified by the `parallelism` value as "Completed" pods approaches the `completions` value.

When `completions` is greater than 1, pods with a "Completed" status will remain visible in the output of `kubectl get pods` until all completions defined in the job have been met, for example 6/6 from `kubectl get jobs`. Resources are released from the node when a pod has a status of Completed or Failed.

A CronJob age of 5 minutes (300 seconds) is a conservative target to account for natural variability in job duration and rate. The following equations can be used as a starting point for `completions` and `parallelism` for each runtime. Adjustments may need to be made based on observations of private location queue growth.
...
Different runtimes will likely have different job durations and rates (jobs per minute). The following queries can be used to obtain average duration and rate for a private location.
