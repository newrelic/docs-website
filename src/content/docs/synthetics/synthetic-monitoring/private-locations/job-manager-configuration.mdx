...
The `completions` setting controls how many pods of a particular runtime must complete before the `CronJob` can start another job for that runtime. For improved efficiency, `completions` should be set to 6-10x the `parallelism` value. This can help to minimize the "end of completions effect" where fewer than the `parallelism` number pods are running as "Completed" pods approaches the `completions` value.
