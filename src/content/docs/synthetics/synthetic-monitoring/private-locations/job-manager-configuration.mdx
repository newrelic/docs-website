...
A good rule of thumb is that **1 CPU core is required for each simultaneous heavyweight monitor** (i.e., each scripted browser or scripted API test).

Below are two simple formulas to help you calculate the number of cores you need, whether you're diagnosing a current setup or planning for a future one.
...
**How it works:** This formula calculates your true job arrival rate by adding the jobs your system *is processing* to the jobs that are *piling up* in the queue. Multiplying this total load by the average job duration tells you exactly how many cores you need to clear all the work without queuing.
...
**How it works:** This calculates your expected workload from first principles: how many monitors you have, how often they run, and how long they take.
...
* **Job Failures & Retries:** When a monitor fails, it's automatically retried. These retries are additional jobs that add to the total load. A monitor that consistently fails and retries **effectively multiplies its frequency**, significantly impacting throughput.
...
**1. To Find Jobs Processed per Minute ($J\_{processed}$):**
...
**2. To Find Queue Growth per Minute ($Q\_{growth}$):**
...
**3. To Find Average Job Duration in Minutes ($D\_j$):**
...
**4. To Find Total Number of Heavyweight Monitors ($N\_m$):**
...
**5. To Find Average Heavyweight Monitor Frequency ($F\_j$):**
If the private location's `jobManagerHeavyweightJobs` queue is growing, it won't be accurate to calculate the average monitor frequency from existing results. This will need to be estimated from the list of monitors on the [Synthetic Monitors](https://one.newrelic.com/synthetics) page. Make sure to select the correct New Relic account and you may need to filter by `privateLocation`.
...
#### A Note on Ping Monitors and the `pingJobs` Queue
...
While they are less resource-intensive, a high volume of ping jobs, especially failing ones, can still cause performance issues. Keep these points in mind:
  ...
  * **Timeout & Retry:** A failing ping job can occupy a worker thread for up to **60 seconds**. It first attempts an HTTP HEAD request (30-second timeout). If that fails, it immediately retries with an HTTP GET request (another 30-second timeout).
