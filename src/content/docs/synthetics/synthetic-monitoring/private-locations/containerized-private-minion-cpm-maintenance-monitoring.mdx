...
If you are using the infrastructure agent to monitor these runner containers, configure at least one monitor to run each minute. The infrastrucute agent will have more opportunity to notice and collect the above labels from the `docker inspect` of the container before it is deleted.
