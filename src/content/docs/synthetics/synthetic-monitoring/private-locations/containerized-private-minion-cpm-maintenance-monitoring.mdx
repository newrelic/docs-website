...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src={dockimport} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/>Review <DNT>Docker/DNT> logs</>}
  >
    This is an example of a CPM log indicating that the minion is working properly in a <DNT>Docker/DNT> container system environment:
...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src={dockimport} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/>Enable <DNT>Docker/DNT> debug logs</>}
  >
    <Callout variant="tip">
...
[New Relic's infrastructure monitoring](/docs/infrastructure) supports [advanced <DNT>Docker/DNT> monitoring](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/docker-instrumentation-infrastructure) and [advanced Kubernetes monitoring](/docs/integrations/kubernetes-integration/get-started/introduction-kubernetes-integration). To add support for this, synthetic monitoring labels the containers spawned by CPM with a series of informative labels, all prefixed with `synthetics-minion-`. The CPM spawns containers called "runners" which process non-ping monitors like: simple browser, scripted browser, api test, and step function. You can use these labels to identify these runner containers. Example labels include:
