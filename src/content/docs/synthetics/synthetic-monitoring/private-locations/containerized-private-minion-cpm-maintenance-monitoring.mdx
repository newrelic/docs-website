...
Runner containers are ephemeral. One runner container is created to process one non-ping monitor job. It is created, processes the job, and is quickly deleted. A runner container may exist for only a few seconds and will be created only if there is a non-ping monitor jobs to process. Ping monitors will not trigger runner container creation, and thus the above labels will not be present.

If using the infrastructure agent to monitor these runner containers, it's best to configure at least one monitor to run each minute. This will provide the infrastrucute agent more opportunity to notice and collect the above labels from the `docker inspect` of the container before it is deleted.

Note, the `synthetics-minion-id` label refers to the ID of the minion which spawned this particular runner container. We do not track the ID of the runner itself.
