...
The benefit to this approach is to see a complete picture of how 100% of your current monitors will execute on the new runtimes. That includes potential differences in results and execution duration, both of which impact the load on the job manager and needed resources.

To just test for monitor failures, an alternative approach is to set up a second private location with a small test SJM. Then run some validation jobs to test certain monitors. This can provide you with knowledge about how existing monitors will behave on the new runtimes, but not how the new runtimes will fit with your existing infrastructure capacity.
...
If you've set up a second private location with the same set of monitors (real jobs not validations), use these queries to track your migration progress:
...
|-----------|-------------------|---------|
| SJM container memory | 3GiB | 3.256GiB |
| Browser runtime memory (`HEAVY_WORKER_MEMORY`) | 4GiB | 3.256GiB |
| Browser runtime shared memory | 2.256GiB | 2.256GiB |
| CPU per browser runtime | 1 core | 1 core |
| Ping runtime memory | 1GiB | 1GiB |
