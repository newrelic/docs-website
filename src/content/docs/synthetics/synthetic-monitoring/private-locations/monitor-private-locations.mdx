...
This guide will help you answer the following basic questions regarding private location health by using [New Relic dashboards](/docs/insights) and [NRQL alerts](https://docs.newrelic.com/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries):
...
The Private Minion dashboard example can be imported to your account using the [Dashboard API](https://docs.newrelic.com/docs/insights/insights-api/manage-dashboards/insights-dashboard-api) with the following JSON:
...
* `:8080/status`: Details about a minion's status; the same data is then [published to Insights as a `SyntheticsPrivateMinion` event](https://docs.newrelic.com/docs/insights/insights-data-sources/default-events-attributes/synthetics-default-events-attributes-insights#syntheticprivateminion-table).
