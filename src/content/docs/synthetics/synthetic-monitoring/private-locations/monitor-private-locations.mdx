            ...
            },
            {
              ...
              "rawConfiguration": {
                ...
                "nrqlQueries": [
                  {
                    "accountId": 1,
                    "query": "FROM SyntheticCheck SELECT count(*) WHERE location NOT LIKE 'AWS_%' AND result != 'SUCCESS' AND error NOT LIKE '%case:%' FACET capture(error, r'(?P<firstWord>\\S+)\\s.+') AS 'error class' TIMESERIES AUTO SINCE 2 days ago"
                  }
                ],
                "yAxisLeft": {
                  "zero": true
