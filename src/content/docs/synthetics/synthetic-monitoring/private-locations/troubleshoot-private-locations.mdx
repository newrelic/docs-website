...
* Jobs demand throughput per minute:
  ...
  ```sql
  SELECT rate(uniqueCount(jobId), 1 minute) 
...
* Identify queue growth:
  ...
  WHERE name = 'YOUR_PRIVATE_LOCATION' SINCE 2 days ago TIMESERIES
  ```
* Consumption rate:
...
* Memory usage per location:
  ...
  ```sql
  SELECT latest(minionPhysicalMemoryUsedPercentage) 
...
* Overall CPU usage:
  ...
  ```sql
  SELECT latest(minionProcessorsUsagePercentage) 
  FROM SyntheticsPrivateMinion 
  FACET minionLocation SINCE 2 weeks ago TIMESERIES MAX
  ```
