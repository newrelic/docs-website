...
You can use New Relic's synthetics job managers. These are [Docker container](https://www.docker.com/resources/what-container)-based resources that accept and execute [synthetic monitors](/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors) against your [private locations](/docs/synthetics/new-relic-synthetics/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations).
...
## General synthetics job manager features [#enhancements]

Because the job manager operates as a container instead of a virtual machine, it delivers many features:
...
Also, the job manager delivers the following features in a Kubernetes environment:

* Uses Kubernetes CronJob resources to run non-ping monitors, allowing for easy sizing and customization of these resources 
        ...
        <tr>
          ...
          <td>
            ...
            **Scripted browser:** 31
            ...
            Measured by monitoring write throughput for [https://newrelic.com](https://newrelic.com) on an AWS EKS 1.21 cluster backed by one EC2 m5.xlarge node running with: gp2 storage class, 50 GiB root volume, 20 GiB PV/PVC with RWO access mode, a default [Kubernetes synthetics job manager install](#kubernetes-install) via Helm, and 1 monitor at a time set to 1-minute frequency. Efficiency gains are expected with multiple monitors running. These values may be higher or lower than your own depending on many factors.

            *Baseline for Kubernetes synthetics job manager is 3.0 IOPS with no monitor jobs running.*
          </td>
        </tr>
...
## Install and update synthetics job manager versions [#install]
...
Unless hosting the images in a local image repository, connections to `docker.io` will need to be allowed through your firewall in order for Docker to pull the synthetics-job-manager and synthetics runtime images. The runtime images are pulled automatically on startup of the synthetics-job-manager container. See [Docker environment configuration](/docs/synthetics/synthetic-monitoring/private-locations/containerized-private-minion-cpm-configuration/#docker-env-config) and [Kubernetes environment configuration](/docs/synthetics/synthetic-monitoring/private-locations/containerized-private-minion-cpm-configuration/#kubernetes-env-config) for details on how to set a local repository and the runner registry endpoint.

## Start synthetics job manager [#start]

To start synthetics job manager, follow the applicable Docker or Kubernetes instructions.
...
<CollapserGroup>
  ...
  >
    ...
    1. Get the <var>JOB_MANAGER_POD_INSTALLATION_NAME</var> of the synthetics job manager pod you want to delete:
...
* To keep track of Docker logs and verify the health of your monitors, see [synthetics job manager maintenance and monitoring](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-maintenance-monitoring).
...
## Configure synthetics job manager [#configuration]

For more details on advanced configuration settings, see [synthetics job manager configuration](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-configuration).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        To set up communication with New Relic by proxy, use the [environment variables](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-configuration#environment-variables) named `HORDE_API_PROXY*.`
      </td>
    </tr>
