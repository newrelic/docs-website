...
The job manager can operate in a <DNT>Docker/DNT> container system environment or a Kubernetes container orchestration system environment. The job manager will auto-detect its environment to select the appropriate operating mode.
...
* Ability to leverage a <DNT>Docker/DNT> container as a [sandbox](#sandboxing-and-docker-deps) environment.
...
* Doesn't require privileged access to the <DNT>Docker/DNT> socket
...
<CollapserGroup>
  ...
  >
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            *You must also configure <DNT>Docker/DNT> to run Linux containers in order for synthetics job managers to work on Windows systems.*
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <DNT>Docker/DNT> [17.12.1-ce](https://docs.docker.com/engine/release-notes/17.12/) or higher
          </td>
        </tr>
    ...
    <Callout variant="caution">
      The <DNT>Docker/DNT> synthetics job manager is not designed for use with container orchestrators like AWS ECS, <DNT>Docker/DNT> Swarm, Apache Mesos, Azure Container Instances, etc. Running the <DNT>Docker/DNT> synthetics job manager in a container orchestrator will result in unexpected issues because it is itself a container orchestrator. If you're using container orchestration, see our [Kubernetes synthetics job manager requirements](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#kubernetes-requirements).
    </Callout>
  </Collapser>
...
If you're running more than one <DNT>Docker/DNT> private location container in the same host, you'll have port conflicts. To avoid this port contention, make sure you do the following when you start setting up job managers:
...
Unless hosting the images in a local image repository, you need to allow connections to `docker.io` through your firewall so <DNT>Docker/DNT> can pull the synthetics job manager and synthetics runtime images. When the synthetics job manager starts up, the runtime images are pulled automatically. See [Docker environment configuration](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-configuration/#docker-env-config) and [Kubernetes environment configuration](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-configuration/#kubernetes-env-config) for details on how to set a local repository and the runner registry endpoint.
...
Below are the applicable <DNT>Docker/DNT> or Kubernetes instructions to start the job manager.

<CollapserGroup>
  <Collapser
    id="docker-update"
    title={<><img src={syntheticDocker} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/> <DNT>Docker/DNT> start procedure</>}
  >
...
On a <DNT>Docker/DNT> container system environment, use the <DNT>Docker/DNT> `stop` procedure to stop the synthetics job manager from running. On a Kubernetes container orchestration system environment, use the Kubernetes `delete` procedure to stop the synthetics job manager from running.
...
Sandboxing and <DNT>Docker/DNT> dependencies are applicable to the synthetics job manager in a <DNT>Docker/DNT> container system environment.
...
<CollapserGroup>
  ...
  >
    The synthetics job manager runs in <DNT>Docker/DNT> and is able to leverage <DNT>Docker/DNT> as a sandboxing technology. This ensures complete isolation of the monitor execution, which improves security, reliability, and repeatability. Every time a scripted or browser monitor is executed, the synthetics job manager creates a brand new <DNT>Docker/DNT> container to run it in using the matching runtime.

    The synthetics job manager container needs to be configured to communicate with the <DNT>Docker/DNT> engine in order to spawn additional runtime containers. Each spawned container is then dedicated to run a check associated with the [synthetic monitor](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors) running on the private location the synthetics job manager container is associated with.
...
<CollapserGroup>
  <Collapser
    id="run-non-root"
    title={<><img src={syntheticDocker} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/>Run as non-root user for <DNT>Docker/DNT></>}
  >
...
## Understand your <DNT>Docker/DNT> or Kubernetes environments [#understand]

Below is additional information about maintaining and understanding the job manager's container environment. View license information, understand the job manager's network settings, and check out the <DNT>Docker/DNT> image repo.
...
* To keep track of <DNT>Docker/DNT> logs and verify the health of your monitors, see [synthetics job manager maintenance and monitoring](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-maintenance-monitoring).
...
Some of our open-source software is listed under multiple software licenses, and in that case we have listed the license we've chosen to use. Our license information is also available in the [our licenses documentation](/docs/licenses/new-relic-products/new-relic-synthetics/new-relic-synthetics-licenses).
  ...
  >
  For both <DNT>Docker/DNT> and Kubernetes, the synthetics job manager and its runtime containers will inherit network settings from the host. For an example of this on a <DNT>Docker/DNT> container system environment, see the [Docker site](https://docs.docker.com/config/containers/container-networking/).

  A bridge network is created for communication between the synthetics job manager and runtime containers. This means networking command options like `--network` and `--dns` passed to the synthetics job manager container at launch (such as through <DNT>Docker/DNT> run commands on a <DNT>Docker/DNT> container system environment) are not inherited or used by the runtime containers.
  ...
  >
A single synthetics job manager <DNT>Docker/DNT> image serves both the <DNT>Docker/DNT> container system environment and Kubernetes container orchestration system environment. The <DNT>Docker/DNT> image is hosted on <DNT>Docker/DNT> Hub. To make sure your <DNT>Docker/DNT> image is up-to-date, see the [Docker Hub newrelic/synthetics-job-manager repository](https://hub.docker.com/r/newrelic/synthetics-job-manager/tags).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        This applies to a <DNT>Docker/DNT> container environment only. Arguments passed to the synthetics job manager container at launch do not get passed on to the runtime containers spawned by the synthetics job manager. <DNT>Docker/DNT> has no concept of "inheritance" or a "hierarchy" of containers, and we don't copy the configuration that is passed from synthetics job manager to the runtime containers. The only shared configuration between them is the one set at the [Docker daemon](https://docs.docker.com/config/daemon/) level.
      </td>
    </tr>
  </tbody>
</table>
