...
<CollapserGroup>
  <Collapser
    ...
    title="Docker container system environment requirements"
  >
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 or higher (Recommended for v447+)<br/>
    ...
    <Callout variant="caution">
      Starting with Synthetics Job Manager v447 (which uses Ubuntu 24.04 as its base image), RHEL 7 and other distributions using Linux kernel versions older than 4.18 are no longer supported due to modern glibc and seccomp requirements. For a stable experience, use RHEL 8/9 or an equivalent modern Linux distribution.
     </Callout>
  ...
  <Collapser
    ...
    title="Podman container system environment requirements"
  >
    <table>
      <thead>
      <tr>
  ...
  <Collapser
    ...
    title="Kubernetes container orchestration system environment requirements"
  >
    <table>
      <thead>
        <tr>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> [4.18 or higher (Recommended for v447+)](https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-prerequisites#minimum-resource-requirements)<br/>
            <DNT>**macOS:**</DNT> 10.11 or higher<br/>
...
If you're running more than one Docker or Podman private location container on the same host, you'll have port conflicts. To avoid this port contention, make sure to run each job manager on a separate host.
...
<CollapserGroup>
  ...
  >
    ...
    3. Run the appropriate script for your system. Tailor the common default `/var/run/docker.sock` in the following examples to match your system.
       ...
       * Windows:
       ...
       View the logs for your job manager container:
  ...
  >
    ...
    To view the logs for your job manager container:
  ...
  <Collapser
    ...
    title="Kubernetes start procedure"
  >
    1. Locate your [private location key](#private-location-key).
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Kubernetes delete procedure"
  >
...
<CollapserGroup>
  <Collapser
    id="run-non-root"
    title="Run as non-root user for Docker"
  >
