...
<CollapserGroup>
  ...
  >
    ...
    There is a crucial dependency at launch. To enable sandboxing, ensure that you:



    1. Install Podman [5.0.0-ce](https://github.com/containers/podman/releases/tag/v5.0.0) or higher.
    ...
    3. Enable cgroups v2 (RHEL Only).
       * Edit the GRUB to enable cgroups v2, this ensures compatibility with modern container runtimes in RHEL.
         ...
         sudo reboot
         ```
    4. Enable system-wide delegation to allow cgroups delegation for Podman services.
       ```
...
<CollapserGroup>
  ...
  >
    A single synthetics job manager Docker image serves the Docker container system environment, the Podman container system environment and Kubernetes container orchestration system environment. The Docker image is hosted on Docker Hub. To make sure your Docker image is up-to-date, see the [Docker Hub newrelic/synthetics-job-manager repository](https://hub.docker.com/r/newrelic/synthetics-job-manager/tags).
  </Collapser>
</CollapserGroup>
