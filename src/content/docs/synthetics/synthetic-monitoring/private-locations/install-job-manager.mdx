...
The job manager can operate in a Docker container system environment, Podman container system environment or a Kubernetes container orchestration system environment. The job manager will auto-detect its environment to select the appropriate operating mode.
...
<CollapserGroup>
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            _You must also configure Docker to run Linux containers in order for synthetics job managers to work on Windows systems._
          </td>
        </tr>
  ...
  >
    ...
    <Callout variant="caution">
      The Podman synthetics job manager is not designed for use with container orchestrators like AWS ECS, Docker Swarm, Apache Mesos, Azure Container Instances, etc. Running the Podman synthetics job manager in a container orchestrator will result in unexpected issues because it is itself a container orchestrator. If you're using container orchestration, see our [Kubernetes synthetics job manager requirements](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#kubernetes-requirements).
    </Callout>
  </Collapser>
...
If you're running more than one Docker or Podman private location container in the same host, you'll have port conflicts. To avoid this port contention, make sure you do the following when you start setting up job managers:
...
<CollapserGroup>
  ...
  >
    1. Locate your [private location key](#private-location-key).
    2. Ensure you've enabled [Podman dependencies](#sandboxing-and-podman-deps) for sandboxing and [installed](#install-update) synthetics job manager on your system.
    3. Run the below script for your system.

    * Linux:
    Create a pod, adding the host machine's IP Address.
    ```
    podman pod create --name PODMAN_POD_NAME --add-host=podman.service:IP_ADDRESS
    ```
...
On a Docker or Podman container system environment, use the Docker or Podman `stop` procedure to stop the synthetics job manager from running respectively. On a Kubernetes container orchestration system environment, use the Kubernetes `delete` procedure to stop the synthetics job manager from running.
...
## Sandboxing and Podman dependencies [#sandboxing-and-podman-deps]

Sandboxing and Podman dependencies are applicable to the synthetics job manager in a Podman container system environment.
  ...
  >
    ...
    3. Enable cgroups v2 (RHEL Only).
       * Edit the GRUB to enable cgroups v2, this ensures compatibility with modern container runtimes in RHEL.
         ...
         sudo reboot
         ```
    4. Enable System-wide Delegation, allowing cgroups delegation needed by Podman services.
       ```
       sudo mkdir -p /etc/systemd/system/user@.service.d/
       echo -e "[Service]\nDelegate=yes" | sudo tee /etc/systemd/system/user@.service.d/delegate.conf > /dev/null
       ```
    5. Set up user-level Systemd Service.
       * Create directories for user-level systemd services.
         ```
       ...
       * Add delegate configuration to the user-level systemd service.
         ...
         ```
    6. Enable and start the Podman Socket.
       ```
       systemctl --user enable podman.socket
       systemctl --user start podman.socket
       systemctl --user status podman.socket
       ```
    7. Create and configure Podman API Service.
       ...
       * Define the service to expose the Podman's API at port 8000.
         ...
         ```
    8. Enable and start the Podman API Service.
       ```
