...
Before you get started setting up a job manager, keep in mind that you'll need to do the following to avoid port contention:

* Run job managers and CPMs on different hosts.
* Run each job manager on a separate host&mdash;never on the same host.
* Run each CPM on a different host&mdash;never on the same host.
