...
tags:
  ...
  - Private locations
metaDescription: Install New Relic's <DNT>Docker/DNT>-based private minion that accepts and runs the jobs assigned to your private locations
redirects:
...
The CPM can operate in a <DNT>Docker/DNT> container system environment or a Kubernetes container orchestration system environment. The CPM will auto-detect its environment to select the appropriate operating mode.
...
* Ability to leverage a <DNT>Docker/DNT> container as a [sandbox](#cpm-sandboxing) environment
...
* Does not require privileged access to the <DNT>Docker/DNT> socket
* Supports hosted and on-premise Kubernetes clusters
* Supports various container engines such as <DNT>Docker/DNT> and Containerd
...
<CollapserGroup>
  ...
  >
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            The OS and architecture must be a [platform supported by <DNT>Docker/DNT>](https://docs.docker.com/engine/install/#supported-platforms).

            *You must also configure <DNT>Docker/DNT> to run Linux containers in order for CPMs to work on Windows-based <DNT>Docker/DNT> hosts.*
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            *Baseline for <DNT>Docker/DNT> CPM is 0.5 IOPS with no monitor jobs running.*
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <DNT>Docker/DNT> Engine versions starting with [17.12.1-ce](https://docs.docker.com/engine/release-notes/17.12/) up to and including version 25.
          </td>
        </tr>
    ...
    <Callout variant="caution">
      The <DNT>Docker/DNT> CPM does not support <DNT>Docker/DNT> Engine 26.0 or higher due to breaking changes. Customers seeking <DNT>Docker/DNT> 26+ support should migrate to [Synthetics Job Manager](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-transition-guide/)
    </Callout>

    <Callout variant="caution">
      The <DNT>Docker/DNT> CPM is not designed for use with container orchestrators like AWS ECS, <DNT>Docker/DNT> Swarm, Apache Mesos, Azure Container Instances, etc. Running the <DNT>Docker/DNT> CPM in a container orchestrator will result in unexpected issues because it is itself a container orchestrator. If you're using container orchestration, see our [Kubernetes CPM requirements](/docs/synthetics/synthetic-monitoring/private-locations/install-containerized-private-minions-cpms/#kubernetes-requirements).
    </Callout>
  </Collapser>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            <Callout variant="caution">For Kubernetes v1.21 or newer, use minion release [v3.0.61](/docs/release-notes/synthetics-release-notes/containerized-private-minions-release-notes/containerized-private-minion-cpm-3061) or newer.
            </Callout>
          </td>
        </tr>
...
## Sandboxing and <DNT>Docker/DNT> dependencies [#sandboxing-and-docker-deps]

Sandboxing and <DNT>Docker/DNT> dependencies are applicable to the CPM in a <DNT>Docker/DNT> container system environment.
...
<CollapserGroup>
  ...
  >
    The CPM runs in <DNT>Docker/DNT> and is able to leverage <DNT>Docker/DNT> as a sandboxing technology. This ensures complete isolation of the monitor execution, which improves security, reliability, and repeatability. Every time a scripted or browser monitor is executed, the CPM creates a brand new <DNT>Docker/DNT> container to run it in called a runner.

    The minion container needs to be configured to communicate with the <DNT>Docker/DNT> engine in order to spawn additional runner containers. Each spawned container is then dedicated to run a check associated with the [synthetic monitor](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors) running on the private location the minion container is associated with.
    ...
    * Your writable <DNT>Docker/DNT> UNIX socket is mounted at `/var/run/docker.sock` or `DOCKER_HOST` [environment variable](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#environment-variables). For more information, see <DNT>Docker/DNT>'s [Daemon socket option](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-socket-option).
...
Both installing and updating the CPM use the same command to pull the latest <DNT>Docker/DNT> image from the [Quay.io](https://quay.io) repository where the CPM <DNT>Docker/DNT> image is hosted. Go to [quay.io/repository/newrelic/synthetics-minion](https://quay.io/repository/newrelic/synthetics-minion?tab=tags) for a list of all the releases.
...
On a <DNT>Docker/DNT> container system environment, use the <DNT>Docker/DNT> `stop` procedure to stop the CPM from running. On a Kubernetes container orchestration system environment, use the Kubernetes `delete` procedure to stop the CPM from running.
...
<CollapserGroup>
  ...
  >
    You can [stop a <DNT>Docker/DNT> container](https://docs.docker.com/engine/reference/commandline/stop/) either by the container name, or the container ID.
...
* To keep track of <DNT>Docker/DNT> logs and verify the health of your monitors, see [Containerized private minion (CPM) maintenance and monitoring](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-maintenance-monitoring).
...
For both <DNT>Docker/DNT> and Kubernetes, the CPM and its runner containers will inherit network settings from the host. For an example of this on a <DNT>Docker/DNT> container system environment, see the [Docker site](https://docs.docker.com/config/containers/container-networking/).

A new bridge network is created for each runner container. This means networking command options like `--network` and `--dns` passed to the CPM container at launch (such as through <DNT>Docker/DNT> run commands on a <DNT>Docker/DNT> container system environment) are not inherited or used by the runner containers.

When these networks are created, they pull from the default IP address pool configured for daemon. For an example of this on a <DNT>Docker/DNT> container system environment, see the [Docker site](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-configuration-file).

Typically, the runner network is removed after the check is completed. However, if a CPM exits while a check is still running, or exits in another unexpected circumstance, these networks may get orphaned. This can potentially use up IP address space that is available to the <DNT>Docker/DNT> daemon.

If this happens, you may see `INTERNAL ENGINE ERROR code: 31` entries in your CPM logging when trying to create a new runner container. To clean these up in <DNT>Docker/DNT> container system environments only, run `docker network prune`.
...
In a <DNT>Docker/DNT> container system environment: To change the default AppArmor profile used by containers that CPM spawns to run monitors, see the [environment variable](#environment-variables) `MINION_RUNNER_APPARMOR` (CPM version 3.0.3 or higher) or `MINION_DOCKER_RUNNER_APPARMOR` (CPM version up to v3.0.2).
...
<CollapserGroup>
  <Collapser
    id="run-non-root"
    title={<><img src={dockimport} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/>Run as non-root user for <DNT>Docker/DNT></>}
  >
    For more information, see <DNT>Docker/DNT>'s official documentation about [security](https://docs.docker.com/engine/security/security/) and [AppArmor security profiles](https://docs.docker.com/engine/security/apparmor/).
    ...
    1. Ensure that `my_user` can use the <DNT>Docker/DNT> engine on the host:
...
## <DNT>Docker/DNT> image repository

A single CPM <DNT>Docker/DNT> image serves both the <DNT>Docker/DNT> container system environment and Kubernetes container orchestration system environment. The <DNT>Docker/DNT> image is hosted on quay.io. To make sure your <DNT>Docker/DNT> image is up-to-date, see the [quay.io newrelic/synthetics-minion repository](https://quay.io/repository/newrelic/synthetics-minion?tab=tags).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        This applies to a <DNT>Docker/DNT> container environment only. Arguments passed to the CPM container at launch do not get passed on to the containers spawned by the CPM. <DNT>Docker/DNT> has no concept of "inheritance" or a "hierarchy" of containers, and we don't copy the configuration that is passed from CPM to the monitor-running containers. The only shared configuration between them is the one set at the [Docker daemon](https://docs.docker.com/config/daemon/) level.
      </td>
    </tr>
  </tbody>
</table>
