...
* The ability for your monitors to treat objects like frames and find objects inside the shadow DOM.
* [Custom timing library](SOMETHING) for scripted API monitors.
- Access to automated management of your synthetic monitors using [NerdGraph APIs](SOMETHING). 
...
## Switching to the legacy runtime from the UI [#runtime-UI]

You have the option to toggle between the legacy environment and the new runtime environment when you create a new monitor. While the legacy runtime and its features still have backwards support, we recommend that you convert to the new runtime environment as soon as possible. 
