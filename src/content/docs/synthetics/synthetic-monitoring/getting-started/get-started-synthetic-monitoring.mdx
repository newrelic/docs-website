...
<SideBySide>
  <Side>
Mix and match from a selection of 12 prebuilt steps. Your monitor can be simple, like clicking buttons to hop through different pages, or you can test more complex journeys like:

1. Navigating to a URL 
2. Typing an email into a page modal
3. Selecting an element from a dropdown, then navigating to a different page on your site
4. Typing text into a form, then clicking an element to submit that form

Whatever you build, remember to click **Validate** before saving your monitor. Validating checks that the steps you've strung together run successfully.  
  </Side>
...
You can test even more complex workflows with scripted browser monitors. Scripting is flexible, allowing you to mimic user behavior with conditional logic.  We recommend taking the leap to scripted browser monitoring if:

* You want to change timeout values
* You want functions not available in our step builder, like the ability to work with frames or a site that uses multiple tabs
* You want your monitor to respond to app behavior like a customer would
