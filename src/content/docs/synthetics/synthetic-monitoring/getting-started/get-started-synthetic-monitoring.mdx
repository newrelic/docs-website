...
Our synthetic monitors act like crash test dummies for your websites, applications, and API endpoints. When you deploy a synthetic monitor, an API check or virtual browser instance runs through your most important workflows, testing critical endpoints so you can guarantee you're delivering the best customer experience. You don't even need to install an agent. 

If you're interested, you can learn more about synthetic monitor checks and data ingest in our [Including synthetic monitor checks](/docs/synthetics/synthetic-monitoring/using-monitors/monitor-limits/) doc. 

## Synthetic monitors for proactive monitoring [#proactive]

Our synthetic monitors are more than just ping monitors. Synthetics can perform scriptable checks on your customer-facing or internal apps, down to the HTTP headers. The goal is to alert you to operational inefficiencies before anyone has a chance to notice.
...
* Run an [API test](/docs/synthetics/new-relic-synthetics/scripting-monitors/writing-api-tests) to check certificates, or to send `GET`, `POST`, or `HEAD` requests. 
* Automate performance tracking for every deployment by adding synthetic monitoring to your CI/CD pipelines. 
* Diagnose whether an issue stems from the network or AWS location, a slow third party resource, or the health of your backend services or infrastructure. 

To get an overview of our different synthetic monitors, watch this short video (5:10 minutes).
...
[Full platform users](/docs/accounts/accounts-billing/new-relic-one-pricing-users/users-roles#user-type) have theoretical access to all New Relic features. In practice, a user's access to synthetics features depends on what roles are assigned and what [synthetics-related capabilities](/docs/synthetics/new-relic-synthetics/administration/synthetics-permissions-user-groups#synthetics) those roles have. 
