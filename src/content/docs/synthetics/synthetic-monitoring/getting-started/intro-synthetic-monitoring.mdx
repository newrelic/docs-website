...
Synthetic monitors perform checks on your customer-facing or internal apps, down to the HTTP headers. By mimicking the behaviors of customers, your monitors test how well your web apps handle traffic. This lets you proactively catch and resolve issues, whether it's a broken link, a failed API call, or significant changes in your app runtime.
...
## Create your first synthetic monitor...

When you [sign up for New Relic](https://newrelic.com/signup) you can deploy synthetics without installation. This lets you: 
...
Ping monitors are the simplest type of monitor. They use a simple Java HTTP client to check that your site is online. You'll user agent is identified as `Google Chrome`. However, the HTTP client is not a full browser, and it does not execute JavaScript. If you need JavaScript functionality, use a simple browser monitor.  </Collapser>
  <Collapser
    className="freq-link"
    id="broken-links"
...
This monitor goes to a URL, then tests all the links on the page for success. If failures are detected, you can view the links that caused the failure.
  </Collapser>
  <Collapser
    className="freq-link"
    id=""
