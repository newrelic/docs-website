...
![Click Run check to recheck the monitor's status](src/images/runcheck.png "Click Run check to recheck the monitor's status")

<figcaption>
  [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > (select a
  monitor) > Failures > (select a check failure):** To view details about a
  failed monitor check, click <Icon name="fe-rotate-cw" />
  **Run check**.
</figcaption>
