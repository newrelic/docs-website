...
1. Go to **[one.newrelic.com](https://one.newrelic.com) > Synthetics > (select a monitor)**.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Synthetics > (select a monitor) > Failures > (select a check failure):** To view details about a failed monitor check, click <Icon name="fe-rotate-cw"/>
  **Run check**.
</figcaption>
