...
<DNT>**Run check**</DNT> buttons appear on monitor result details pages if:
...
1. Go to <DNT>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**</DNT>, then select a monitor.
2. From the monitor's <DNT>**Failures**</DNT> page, select a link from the table.

   OR

   From the monitor's [<DNT>**Results**</DNT> page](/docs/synthetics/new-relic-synthetics/using-monitors/view-monitor-results), use the filter or sort options to select a location indicating <DNT>**Results: Failed**</DNT>.
3. Review the failed check details, then click <Icon name="fe-rotate-cw"/>
   <DNT>**Run check**</DNT>.
4. To view the updated status, click <DNT>**See results**</DNT>.
...
<figcaption>
  <DNT>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**</DNT> (select a monitor) > <DNT>**Failures**</DNT> > (select a check failure):To view details about a failed monitor check, click <Icon name="fe-rotate-cw"/>
  <DNT>**Run check**</DNT>.
</figcaption>

When you click <Icon name="fe-rotate-cw"/>
<DNT>**Run check**</DNT>, New Relic triggers an immediate recheck using the result's location and the monitor's current configuration. Once the job is completed, the button will display the recheck's status (`Success` or `Failure`). To view the recheck <DNT>**Results**</DNT> page again, click the button again.
