...
import syntheticRuntimeUpgrades from 'images/synthetic_screenshot-crop_runtime-upgrades-tab.png'
...
* [Bulk upgrade monitors](#bulk-upgrade)
...
2. Click **General** in the left navigation panel.
...
The Synthetic monitoring capability allows you to move existing sythetic monitors from the legacy runtime to the current runtime.

To bulk upgrade your monitors:

1. Go to **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**, then select the monitor you want to upgrade.
2. Click the **Runtime upgrades** tab.

    <img
      title="Bulk upgrade monitors"
      alt="Bulk upgrade monitors"
      src={syntheticRuntimeUpgrades}
    />

3. Select the monitors you want to upgrade:

    * Click **Upgrade all (number)** if you want to upgrade all the monitors at the same time.
    * Click the checkbox to the left of the monitor name to select the monitors you want to upgrade. Then click **Upgrade now (number)**.

4. Review the monitors that failed. You can check and correct errors on monitors that have failed validation. These errors can be because the monitor was already failing on the old runtime or for changes in the Chrome runtime. All errors have a link to the specific monitor.

There can be monitors where validations can't be run because they only have legacy private locations. In this case, you need to set up the [synthetics job manager](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#install).


## View synthetic monitoring upgrade history [#query]

To see a history of monitor version upgrades, query the [`NrAuditEvent`](/docs/accounts/accounts/account-maintenance/query-account-audit-logs-nrauditevent/).
