...
<CollapserGroup>
  <Collapser
    id="NewUpgrade"
    title={<>To Chrome 100, Node 16, or Newer</>}
    >
...
<Callout variant="tip">
  If you're unsure about your monitor's runtime version, edit your monitor and check the "Runtime" drop down on the "Configure monitor" tab. You can also query the runtimeTypeVersion attribute on SyntheticCheck events where the runtimeType = 'NODE_API'.
</Callout>
...
The browser runtime will be managed separately from the API starting with the Chrome 100 runtime release. This is also known as our next-generation runtime. 
