...
Simple browser, scripted browser, and API monitors execute in the runtime environment used when the monitors were created. Your monitors won't automatically convert to newer runtimes when they release.
...
* [Review API runtime dependencies](#apidependencies)
* [Review browser runtime dependencies](#browserdependencies)
...
<CollapserGroup>
  <Collapser
    id="NewUpgrade"
    title={<>Upgrade to Chrome 100, Node 16, or newer</>}
    >
    ...
    1. Go to [one.newrelic.com](https://one.newrelic.com/nr1-core), click **Synthetics**, then select the monitor you want to upgrade.
  ...
  >
    ...
    To ensure your monitors have access to new features and perform as expected, upgrade your monitors whenever you see the **Upgrade monitors** button. You can also upgrade your monitors by going to [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > Upgrade monitors**.
...
<Callout variant="tip">
  If you're unsure about your monitor's runtime version, [edit your monitor](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime/#runtime-convert) and check the **"Runtime"** drop down on the "Configure monitor" tab. You can also query the `runtimeTypeVersion` attribute on `SyntheticCheck` events where the `runtimeType = 'NODE_API'`.
</Callout>

<CollapserGroup>
  <Collapser
    id="Node16"
    title={<>Node 16.10.0 (<strong>Latest</strong>)</>}
  >
    Node 16.10.0 details:
    * **Node:** 16.10.0
    * **Proxy settings:** Yes
