...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Explorer > Synthetic monitors**.
2. To find the type of result you're looking for, sort using the provided filters. For example, to view all **ping** monitors, sort by **Monitor Type**. You can also search for specific results using the [New Relic quick find](/docs/new-relic-one-core-ui-components), which is available across the New Relic platform.
