...
* Better support for [shadow DOM objects](/docs/synthetics/synthetic-monitoring/using-monitors/manage-monitor-runtimes).
...
Using the `$http` object or `request` module allowed some of your scripted API monitors to report timing details in the legacy runtime.  Result details were limited to a check result (pass /fail) and script logs. These timing details were not available when using other Node.js modules in a scripted API monitor.
