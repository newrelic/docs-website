...
Synthetic monitoring has a new runtime available for your public locations. New synthetic monitors default to using the new runtime. If you've created a synthetic monitor using the legacy runtime, there are extra [configuration steps](#runtime-convert).

Making the switch gives you these features:

* Access to [Chrome 100](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions-chrome100), so your scripted browser monitors use browser versions similar to the ones used by your customers. 
...
* Better support for [shadow DOM objects](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions#shadow).
...
Existing monitors default to the runtime used at their time of creation. By converting from the legacy runtime to the new runtime, you improve the backend environment that your public monitors execute from. To prevent your critical monitors from breaking during future end of life processes, we recommend converting your public monitors sooner rather than later. 
...
Using the `$http` object or `request` module allowed some of your scripted API monitors to report timing details in the legacy runtime. Result details were limited to script logs and a check result (pass /fail). These timing details were unavailable with Node.js modules.
...
We’re introducing new language that changes the script syntax in your scripted browser monitors. The new runtime is backwards compatible with legacy runtime syntax. To avoid breaking monitors during the upgrade process, you may receive a deprecation warning in your script log output.
