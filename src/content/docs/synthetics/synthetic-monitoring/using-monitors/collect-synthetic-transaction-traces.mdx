...
<Callout variant="important">
  Synthetic monitoring and <InlinePopup type="apm" /> must be on the same account for transaction traces to make these connections.
</Callout>

## Enable synthetic transaction traces [#enabling]

Synthetic transaction traces are enabled by default in the following <InlinePopup type="apm" /> agent versions:
