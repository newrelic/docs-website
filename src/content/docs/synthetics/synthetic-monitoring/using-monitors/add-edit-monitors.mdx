...
import createSyntheticMonitorUi from 'images/create-synthetic-monitor-ui.png'
...
You can monitor your web app with several [types of synthetic monitors](/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors).
...
<img
  src={createSyntheticMonitorUI}
  title="Create a synthetic monitor button"
  alt="A screenshot of the Explorer nav in the UI. The screenshot is annotated so that Synthetics is boxed and the Create monitor link is pointed to with an arrow."
/>
...
When creating your monitors, keep in mind that the default timeout for a new monitor is 180 seconds.

<Callout variant="tip">
  You can also add monitors with our [NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-synthetics-tutorial) or the [synthetic monitoring REST API](/docs/apis/synthetics-rest-api/monitor-examples/manage-synthetics-monitors-via-rest-api#add-simple-ping-monitor). For REST APIs, you can create a `GET` request to the monitor you want to use as the source for configuration, then use those key values to use in a `POST` to "copy" and create a new monitor.
</Callout>
