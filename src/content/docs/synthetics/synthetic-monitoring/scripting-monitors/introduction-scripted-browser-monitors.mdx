...
## How to create a scripted browser [#create-scripted-browser]
...
6. Click <DoNotTranslate>**Write script**</DoNotTranslate> to generate the script and add the instructions of what you want to browse.
...
All scripts begin by specifying which URL the monitor should go to. To specify a URL, call `$webDriver.get("url")`:
...
<CollapserGroup>
  ...
  >
    ...
    XPath selectors can break when your website's structure changes. For tips on building good XPath selectors, see [Optimizing XPath selectors](https://blog.newrelic.com/engineering/optimize-xpath-selectors/). For an example of locating an element by its XPath, see [Locate by XPath](#locate-by-xpath).

    <Callout variant="tip">
      To quickly find the XPath for a particular element, use your browser's developer tools. For example, from Chrome:

      1. Go to the target website from Chrome.
