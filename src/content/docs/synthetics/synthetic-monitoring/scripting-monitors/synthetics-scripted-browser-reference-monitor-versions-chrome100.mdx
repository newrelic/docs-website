---
title: Synthetic's scripted browser reference (Chrome 100 and newer)
tags:
  - Synthetics
...
This document describes scripted browser functions available for the Chrome 100 runtime and newer. If you are using a older monitor version, see the [monitor version 0.5.0 and 0.6.0 documentation](/docs/synthetics/new-relic-synthetics/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions-050) or the [monitor version 0.4.0 and lower documentation](/docs/synthetics/new-relic-synthetics/scripting-monitors/synthetics-scripted-browser-reference).
...
The synthetic [scripted browsers](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers) provide access to the [Selenium Webdriver 4.1 APIs](https://www.selenium.dev/selenium/docs/api/javascript/index.html) via the variables `$selenium` and `$webDriver` and backwards compatible access to the [Selenium Webdriver APIs 3.6.0](http://seleniumhq.github.io/selenium/docs/api/javascript/index.html) via the variables `$driver` and `$browser`. While `$driver` and `$browser` are still avaialble, you will receive deprecation warnings indicating that these may be removed from future runtime releases.

In particular:

* `$selenium` provides all the exports from the `selenium-webdriver` module (for example, `ActionSequence`, `Button`, `By`, `WebElement`, etc.). This maps to `$driver` usage in older runtimes.
...
<Callout variant="tip">
  The `waitForAndFindElement(locator, timeout)` method has been removed from the runtime and is not available via $webDriver. Backwards compatibility with `$browser.waitForAndFindElement(locator, timeout)` is available, but this method is considered deprecated and may be removed from future runtimes.

  The combination of the following methods can duplicate the functionality provided by `waitForAndFindElement`:
  ```
...
<Callout variant="tip">
  The `waitForPendingRequests(timeout)` method has been removed from the runtime and is not available via $webDriver. Backwards compatibility with `$browser.waitForPendingRequests(timeout)` is available, but this method is considered deprecated and may be removed from future runtimes.

  The following method can duplicate the functionality provided by `waitForAndFindElement`:
  ```
...
<Callout variant="tip">
  Chrome does not support using XPath locators inside of a shadow root object. Please use a different [locator option](#locators) to find elements inside of a shadow root object.
</Callout>
