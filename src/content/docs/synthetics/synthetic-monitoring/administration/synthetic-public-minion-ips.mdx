...
<Callout variant="important">
  The JSON file is updated to include a regional breakdown of both new and old IP ranges. Please note that after **April 14, 2025**, the IP range list in JSON format will be updated, as some older IPs will be replaced by the specified new ranges. It is important to update your allow list by adding the new IP ranges and removing the older ones accordingly.

  It is recommended that you add the global ranges provided by New Relic to your allow list. However, if needed, you can find the breakdown of the global ranges by region as follows:
