---
title: 'Simplify installation with Go easy instrumentation'
tags:
  - Agents
  - Go agent
  - Installation
...
In an effort to reduce that manual effort, the Go agent team created the Go easy instrumentation tool, which is currently in preview. This tool will do most of the work for you by suggesting changes to your source code that instrument your application with the New Relic Go agent.
...
<Callout variant="important">
  This tool can't detect if you already have New Relic instrumentation. Please only use this on applications without any instrumentation.
</Callout>
...
1. Clone the [Go easy instrumentation repository](https://github.com/newrelic/go-agent-assisted-instrumentation) to a directory on your system. For example:
        ```sh
...
4. When you are satisfied with the instrumentation suggestions, save the file, and then apply the changes:
        ```sh
...
If you have any suggestions or find issues, please create a [Github issue](https://github.com/newrelic/go-agent-assisted-instrumentation).  