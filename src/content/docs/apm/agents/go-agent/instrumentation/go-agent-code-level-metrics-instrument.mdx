...
has information about how you can globally change this heuristic via the `ConfigCodeLevelMetricsIgnorePrefix` setting, you may wish to  provide a custom namespace ignore prefix for a single transaction. You can do so by adding a `WithIgnorePrefix` option to the transaction:
```go
