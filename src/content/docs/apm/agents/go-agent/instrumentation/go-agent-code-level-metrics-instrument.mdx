...
You may also obtain a `CodeLocation` value from a `func` value for later use with the `WithCodeLocation` option. Compare this to the usage [shown above](#instrument-transactions-any-location-clm) to save a code location with `CodeLocation` for later reference with `WithCodeLocation`. This time we do the same thing but with a `func` value instead:
```go
func myFunc() {...}
.
.
