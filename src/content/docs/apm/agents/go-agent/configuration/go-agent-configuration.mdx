...
![New Relic Go agent: config order of precedence](src/images/go-config-precedence.png 'New Relic Go agent: config order of precedence')

<figcaption>
  If server-side configuration is enabled with the Go agent, it overrides
  **all** corresponding values in the `newrelic.Config` struct, even if the
  server-side values are left blank.
</figcaption>
...
<Callout variant="tip">
  For an overview of error configuration in New Relic, see [Manage errors in
...
<Callout variant="important">
  Enabling distributed tracing requires Go agent version 2.1.0 or higher, and it
  disables [cross application tracing](#cross-application-tracing). It also has
  effects on other features. Before enabling, read the [transition
