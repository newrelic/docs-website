...
Not all possible configuration options may be set via environment variables. The following table lists all of the available configuration functions and their corresponding environment variables. Although any named configuration option may be set by directly assigning a value to the corresponding field in the `Config` structure, we recommend using configuration functions and/or environment variables whenever possible.

In this table, if a configuration function is listed, that function may be used to set the corresponding option. See the documentation here and at [the Go documentation site](https://pkg.go.dev/github.com/newrelic/go-agent/v3@v3.20.0/newrelic#ConfigOption) for more information about how to use each function.

If an environment variable is listed, then you may set the corresponding option by setting the named environment variable. You must also include the `ConfigFromEnvironment()` function, which will cause the agent to accept all `NEW_RELIC_*` environment variables.

<table style={{fontSize: "12px"}}>
  <thead>
    <tr>
      <th>Configuration Field</th>
      <th>Configuration Function(s)</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        `ConfigAppLogForwardingMaxSamplesStored`</td>
...
### Notes:
[1] Calling one of the listed functions to enable a subordinate feature also enables the main feature and/or sets other configuration values:
