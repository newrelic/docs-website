...
[New Relic Security agent interactive applications security testing](https://docs.newrelic.com/docs/iast/introduction/) (IAST) tests your applications for any exploitable vulnerability by replaying the generated HTTP request with vulnerable payloads. Similar to the go agent config, the security agent is configured through option functions that are passed to the Init function.
It can alternatively be configured by environment variables, or from a YAML file as well. Options set using config options will take prescedence over environment or YAML configurations.
