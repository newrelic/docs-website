...
[New Relic Interactive Applications Security Testing](https://docs.newrelic.com/docs/iast/introduction/) (IAST) tests your applications for any exploitable vulnerability by replaying the generated HTTP request with vulnerable payloads. You can enable New Relic IAST by applying configurations to the Go agent that are then passed to the INIT function. You can make these configurations with the config file or with environment variables. Options set using config file take precedence over environment or YAML configurations, however we recommend setting the security agent up using a YAML file in order to be able to use config files for any New Relic Agent.
