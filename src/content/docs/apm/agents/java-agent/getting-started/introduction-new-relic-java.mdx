...
To [view your app's performance in the New Relic UI](/docs/apm/applications-menu/monitoring/apm-overview-page), go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Summary**. The APM user interface includes a dedicated [**JVM metrics** page](/docs/agents/java-agent/features/jvm-metrics-page), [transaction](/docs/apm/transactions) and [error](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) details, a [thread profiler tool](/docs/apm/applications-menu/events/thread-profiler-tool) to sample Java threads and report stack traces, and [more](/docs/apm/applications-menu).
