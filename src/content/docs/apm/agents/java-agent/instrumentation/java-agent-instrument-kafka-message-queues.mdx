...
<Callout variant="important">
  Kafka Streams runs on top of Kafka Clients so all of the instrumentation that applies to Kafka Clients also applies to Streams.
</Callout>
...
2. Use the metric explorer to locate your metrics. You can find Kafka metrics in this metric folder:
   ...
   <Callout variant="tip">
     For a full list of Kafka consumer, producer, and streams metrics, see the [Kafka documentation](https://kafka.apache.org/documentation/#remote_jmx).
   </Callout>
...
3. Use the [event explorer](/docs/insights/use-insights-ui/explore-data/event-explorer-query-chart-your-event-data) to view your Kafka events, located in the `KafkaMetrics` event type. Or, use [NRQL](/docs/insights/nrql-new-relic-query-language/using-nrql/introduction-nrql) to query your events directly. For example:
   ...
   ```
   If you are using Kafka Streams metrics, use the `KafkaStreamsMetrics` event type to access streams-specific metrics.
   
## Enable Kafka Streams transactions [#collect-kafka-streams-transactions]
If you are Kafka Streams, by default we do not enable transactions. This is to prevent unesessary overhead as Kafka applications tend to have high throughput. 

Unlike JMS metrics, Kafka Streams metrics are not per record. Instead, a transaction begins when a Kafka consumer polls records and then the resulting data gets processed.
...
Please refer to the [compatibility docs](/docs/apm/agents/java-agent/getting-started/compatibility-requirements-java-agent/) to see what instrumentation module you need to enable for your Kafka Streams version.

## Enable Kafka distributed traces [#collect-kafka-distributed-traces]

The Java agent can also collect [distributed traces](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing) from Kafka clients. Since Kafka Streams runs on top of Kafka Clients, the steps to manage distributed tracing also apply. 
Enabling traces doesn't affect the agent's normal operations; it will still report metric or event data from Kafka.
...
* If you are using Kafka Streams, you need to enable a spans instrumentation module (refer to the [Kafka Streams transaction section](#collect-kafka-streams-transactions)). Since a transaction is not per record, accepting distributed trace headers is not recommended as it will only work for one record.

The complete process of enabling this is below but at a high level it involves these basic steps: 1) enable tracing via the agent config, and 2) call the [Java agent API](/docs/agents/java-agent/api-guides/guide-using-java-agent-api) to instrument transactions on both the producer and consumer side.
...
<CollapserGroup>
  ...
  >
    ...
    ```
    <Callout variant="important"> 
      If you are using Kafka Streams, you do not need to directly start a transaction or send data to a producer.
    </Callout>
  </Collapser>
  ...
  >
    ...
    ```
    <Callout variant="important"> 
      If you are using Kafka Streams, it is not recommended to accept distributed trace headers because a transaction handles many records.
    </Callout>
  </Collapser>
</CollapserGroup>
