...
2. Use the metric explorer to locate your metrics. You can find Kafka metrics in this metric folder:
   ...
   For example, the the `poll-latency-avg` metric is located at:
   ...
   <Callout variant="tip">
     For a full list of Kafka consumer, producer, and streams metrics, see the [Kafka documentation](https://kafka.apache.org/documentation/#monitoring).
   </Callout>
...
Kafka Streams metrics unlike JMS metrics are not per record. Instead, a transaction begins when a Kafka consumer polls records and then the resulting data gets processed.
...
Please refer to the [compatability docs](https://docs.newrelic.com/docs/apm/agents/java-agent/getting-started/compatibility-requirements-java-agent/) to see what instrumentation module you need to enable for your Kafka Streams version.

## Enable Kafka distributed traces [#collect-kafka-distributed-traces]

The Java agent can also collect [distributed traces](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing) from Kafka clients. Since Kafka Streams runs on top of Kafka Clients, the steps to manage distributed tracing also applies. 
...
* If you are using Kafka Streams, you need to enable a spans instrumentation module (refer to the [Kafka Streams transaction section](#collect-kafka-streams-transactions)). Since a transaction is not per record, accepting distributed trace headers is not recomended as it will only work for one record.
...
<CollapserGroup>
  ...
  >
    ...
    ```
    <Callout variant="important"> 
      If you are using Kafka Streams, you do not need to directly start a transaction and send data to a producer.
    </Callout>
  </Collapser>
  ...
  >
    ...
    ```
    <Callout variant="important"> 
      If you are using Kafka Streams, since a transaction handles many records, it is not recommended to accept distributed trace headers.
    </Callout>
  </Collapser>
</CollapserGroup>
