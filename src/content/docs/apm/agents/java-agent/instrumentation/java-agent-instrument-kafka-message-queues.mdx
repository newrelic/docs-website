...
To see what instrumentation module you need to enable for your Kafka Streams version, 
refer to the [compatibility docs](/docs/apm/agents/java-agent/getting-started/compatibility-requirements-java-agent/) to see what instrumentation module you need to enable for your Kafka Streams version.
...
* If you are using Kafka Streams, you need to enable a spans instrumentation module (refer to the [Kafka Streams transaction section](#collect-kafka-streams-transactions)). Since a transaction is not recorded per record, accepting distributed trace headers is not recommended as it will only work for one record.
