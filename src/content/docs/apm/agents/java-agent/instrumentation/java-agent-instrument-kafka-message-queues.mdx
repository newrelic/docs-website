...
## Enable Kafka Streams transactions [#collect-kafka-streams-transactions]
If you are using Kafka Streams, by default we do not enable transactions. This is to prevent unesessary overhead as Kafka applications tend to have high throughput. 
...
* If you are using Kafka Streams, you need to enable a spans instrumentation module (refer to the [Kafka Streams transaction section](#collect-kafka-streams-transactions)). Since a transaction is not recorded per record, accepting distributed trace headers will only work for one record.
