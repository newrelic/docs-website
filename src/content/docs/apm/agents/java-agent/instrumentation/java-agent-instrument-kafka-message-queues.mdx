...
Kafka Streams metrics unlike JMS metrics are not per record. Instead, a transaction begins when many records are polled from a Kafka consumer and then processed.
