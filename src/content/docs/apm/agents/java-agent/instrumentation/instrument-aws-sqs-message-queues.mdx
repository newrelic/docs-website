...
Out of the box, our AWS SQS SDK instrumentation adds distributed trace headers as message attributes to SQS messages for versions `2.1.0` and later. 
However for message reciever operations, we don't offer an out of the box way to accept these headers.
