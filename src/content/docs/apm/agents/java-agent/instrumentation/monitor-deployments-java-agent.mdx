...
New Relic allows you to send information about application deployments from the command line. Then, you can view deployment information on the [**Deployments** page](/docs/apm/applications-menu/events/deployments-page) in the UI: Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (select an app) > Events > Deployments**.
