...
New Relic allows you to send information about application deployments from the command line. Then, you can view deployment information on the [<DoNotTranslate>**Deployments**</DoNotTranslate> page](/docs/apm/applications-menu/events/deployments-page) in the UI: Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (select an app) > Events > Deployments**</DoNotTranslate>.
...
For example, if you set the deployment marker and ran the script to push 100 instances of the app, the <DoNotTranslate>**Deployments**</DoNotTranslate> page will show one deployment for the 100 instances. If you ran the script again, it would show another deployment.
...
The Java agent does not pick up proxy configurations from the `newrelic.yml` file. <DoNotTranslate>**Recommendation:**</DoNotTranslate> If you use a proxy in your application, use either of these options to record deployments:
