...
The Java agent is a fully-featured library that automatically collects data from many popular servers, frameworks, libraries, and datastores (for a full list, see [Compatibility and requirements](/docs/agents/java-agent/getting-started/compatibility-requirements-java-agent)). It also has a significant feature set. However, there are always additional requests for functionality and instrumentation of new libraries. The Java agent provides <DoNotTranslate>**extension modules**</DoNotTranslate> for this purpose.
