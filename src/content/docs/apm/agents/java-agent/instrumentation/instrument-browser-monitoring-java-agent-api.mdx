...
4. Add the `com.newrelic.api.agent.NewRelic.getBrowserTimingHeader()` method to enable time tracking. See below for procedures for specific frameworks.
   ...
   <CollapserGroup>
     ...
     >
       Add the following to `build.sbt`, substituting the [latest agent version](/docs/release-notes/agent-release-notes/java-release-notes) for 8.x.x:
     ...
     >
       Add the following to `Build.scala`, substituting the [lastest agent version](/docs/release-notes/agent-release-notes/java-release-notes) for 8.x.x:
