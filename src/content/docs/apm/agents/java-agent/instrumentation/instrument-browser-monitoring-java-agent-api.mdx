...
4. Add the appropriate method to start time tracking, for example: `com.newrelic.api.agent.NewRelic.getBrowserTimingHeader()`.
   ...
   <CollapserGroup>
     ...
     >
       Add the following to `build.sbt`, substituting the [latest agent version](/docs/release-notes/agent-release-notes/java-release-notes) for <var>3.x.x</var>:
     ...
     >
       Add the following to `Build.scala`, substituting the [lastest agent version](/docs/release-notes/agent-release-notes/java-release-notes) for <var>2.x.x</var>:
