...
This page covers the standard installation path for the New Relic Java agent on any supported app server or JVM-based framework. Follow these steps to download the agent, configure it, and verify that data is flowing to New Relic.
...
- JVM-based languages: Java, Scala, Kotlin, and Clojure
- App servers and frameworks: Tomcat, Spring Boot, JBoss/WildFly, Glassfish, Jetty, WebLogic, WebSphere, Grails, ColdFusion, Solr, Play, Resin, Tanuki Wrapper, and Geronimo — see [compatibility and requirements](/docs/apm/agents/java-agent/getting-started/compatibility-requirements-java-agent) for supported version ranges
- Any JVM process that accepts a `-javaagent` argument
...
- Permission to edit your app server's startup scripts or JVM configuration
- Permission to create a directory to hold the agent files

## Install [#install]

<Steps>
  <Step>
    ## Download the Java agent [#install-step-1]
  ...
  <Step>
    ## Create your New Relic directory [#install-step-2]
  ...
  <Step>
    ## Move the agent files [#install-step-3]
...
When using the config file, restrict permissions to read/write for the owner of the application server process only:
...
Restart your application server. Send at least two requests, then wait 2–3 minutes and your app appears in [APM & services](https://one.newrelic.com/nr1-core?filters=%28domain%20IN%20%28%27APM%27%2C%20%27EXT%27%29%20AND%20type%20IN%20%28%27APPLICATION%27%2C%20%27SERVICE%27%29%29).
