...
The module system introduced in Java 9 can lead to a `NoClassDefFoundError: java/sql/SQLException` if the `-javaagent` property is added to the `conf/resin.conf` or `conf/resin.xml` files. If you are using Java 9+ make sure that the `-javaagent` property is not included in those files. Resin can be run on Java 9+ with the Java agent using one of the following options.

Option 1 - Specify `-javaagent` property on command line:

```
java -javaagent:/path/to/newrelic.jar -jar lib/resin.jar start
```

Option 2 - Add `-javaagent` property to the `bin/resin.sh` script:
