...
<Callout variant="important">
  ...
  Users wishing to add the global certificate to their local trust store can download the [DigiCert Global Root CA](https://www.digicert.com/kb/digicert-root-certificates.htm) from DigiCert and [Let's Encrypt Root CA ](https://letsencrypt.org/certificates/#root-certificates).
</Callout>
