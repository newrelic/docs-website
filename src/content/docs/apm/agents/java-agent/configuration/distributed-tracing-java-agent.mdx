...
Distributed tracing is available for services instrumented with the New Relic APM agent for Java. When this feature is enabled for each of your services, you can see the entire journey of your requests throughout a [distributed system](/docs/distributed-tracing/concepts/introduction-distributed-tracing). This feature is enabled by default for new agent installations. 
...
For Java services that are not currently instrumented with the New Relic agent, you can run our installation assistant that both instruments your service and enables distributed tracing.

You should run the guided installation for each service you want to instrument to ensure that each installation has a unique application name. If some of your services use other languages, simply run the [installation assistant](/docs/distributed-tracing/enable-configure/quick-start) for those other languages.

To start the installation assistant, click the link below. When you are finished, repeat this process for other services that handle your requests, and then come back here to see tips about viewing your traces.
