...
Since agent version [8.7.0]() slow transaction detection was added. When enabled, events of type `SlowTransaction` will be reported if transaction length exceed a certain threshold.
By default the treshold is 1000 milliseconds.
