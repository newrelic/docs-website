...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">For security reasons, capturing custom attributes using the [Custom Instrumentation Editor](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-instrument-ui#options) is set to `false` by default.
    </Callout>
  </Collapser>
