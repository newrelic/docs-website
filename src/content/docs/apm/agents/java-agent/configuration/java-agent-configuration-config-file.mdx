...
<CollapserGroup>
  ...
  >
    ...
    This setting is used for any sampling decision that does not have specific samplers set (that is, `root`, `remote_parent_sampled` and `remote_parent_not_sampled`). Note that this sampling target will not override any specific samplers that are explicitly set. 
    
    Valid values are 1-120, inclusive.
  ...
  <Collapser
    id="dt-sampler-full-granularity-enabled"
  ...
  <Collapser
    id="dt-sampler-partial-granularity-enabled"
  ...
  <Collapser
    id="dt-sampler-partial-granularity-type"
...
Sets the partial granularity mode. See [understanding the three modes](/docs/distributed-tracing/configure-partial-granularity-tracing#modes) for details. Options:
  ...
  >
    ...
    This is the sampler that is applied at the beginning of a distributed trace when it originates from the current service (there is not upstream sampling decision). By default, the adaptive sampler is configured as the root sampler. These are configurable separately both for full granularity (under the main `sampler` config node) as well as for partial granularity (under the `sampler.partialy_granularity` config node).  The possible values:

      - `adaptive` or `default`: Use New Relic's standard sampling rules.  See [adaptive sampler](#sampler...adaptive.sampling_target)
      - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled.  See [Trace ID Ratio-based Sampler](/docs/distributed-tracing/sampling/configure-sampling#trace-id-ratio-based-sampler).
      - `always_on`: Always sample spans that had no upstream decision passed.
      - `always_off`: Always skip sampling spans that had no upstream decision passed.
  ...
  >
    ...
    This option defines how the agent should handle sampling spans when their parent span from an upstream entity was sampled. For example, setting `remote_parent_sampled: always_on` means the agent will always sample anything sampled by an upstream entity, until the sampling reservoir is full. These are configurable separately both for full granularity (under the main `sampler` config node) as well as for partial granularity (under the `sampler.partialy_granularity` config node).  The possible values are:

      - `adaptive` or `default`: Use New Relic's standard sampling rules.  See [adaptive sampler](/docs/distributed-tracing/sampling/configure-sampling#adaptive-sampler).
      - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled.  See [Trace ID Ratio-based Sampler](#sampler...trace_id_ratio_based.ratio)
  ...
  <Collapser
    ...
    >
      ...
      This option defines how the agent should handle sampling spans when their parent span from an upstream entity was not sampled.  For example, setting `remote_parent_not_sampled: always_off` means the agent will not sample anything if the upstream entity did not sample the parent. These are configurable separately both for full granularity (under the main `sampler` config node) as well as for partial granularity (under the `sampler.partialy_granularity` config node).  The possible values are:
  ...
  <Collapser
    id="dt-sampler-adaptive-sampling-target"
    title="sampler...adaptive.sampling_target"
  >
    <table>
      <tbody>
        <tr>
  ...
  <Collapser
    id="dt-sampler-trace-id-ratio-based-ratio"
    title="sampler...trace_id_ratio_based.ratio"
  >
    <table>
      <tbody>
        <tr>
