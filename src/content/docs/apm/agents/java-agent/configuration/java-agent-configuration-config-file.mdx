...
<CollapserGroup>
  ...
  The Java agent supports adding custom labels (tags) to agent-forwarded logs. The labels to be added will be pulled from the [labels](#labels) section at the top-level of the agent config. This feature can be enabled in the `labels` section, nested under the `forwarding` section. These settings can be overridden by the `newrelic.config.application_logging.forwarding.labels` prefixed system property. Options available are:
