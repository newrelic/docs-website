...
<CollapserGroup>
  ...
  >
    ...
    The log verbosity level. This setting will be updated dynamically when changed in the agent yaml config file. Environment variable and system property will not update dynamically.
...
These events are recorded when the transaction length exceed a certain threshold. By default the treshold is 1000 milliseconds.
