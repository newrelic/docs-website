...
<CollapserGroup>
  ...
  >
    ...
    This will be the setting used for any sampling decisions made that do not have specfic samplers set (`root`, `remote_parent_sampled` and `remote_parent_not_sampled`. See below) and the sampling target set will be separate from those specific samplers, if set. Valid values are:
...
Sets the partial granularity mode. See [Understanding the three modes](/docs/distributed-tracing/configure-partial-granularity-tracing#modes) for details. Options:
  ...
  >
    ...
    This is the sampler that is applied at the beginning of a trace (when no parent span was received by an upstream entity). By default, the Adaptive sampler is configured as the root sampler. These are configurable separately both for full granularity (under the main `sampler` config node) as well as for partial granularity (under the `sampler.partialy_granularity` config node).  The possible values are:
  ...
  <Collapser
    ...
    >
      ...
      This option defines how the agent should handle sampling spans when their parent span from an upstream entity was not sampled.  For example, setting `remote_parent_not_sampled: always_off` means the agent will not try to sample anything if the upstream entity did not sample the parent. These are configurable separately both for full granularity (under the main `sampler` config node) as well as for partial granularity (under the `sampler.partialy_granularity` config node).  The possible values are:
