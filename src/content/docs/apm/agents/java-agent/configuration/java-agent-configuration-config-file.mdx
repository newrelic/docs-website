...
<CollapserGroup>
  ...
  >
    ...
    Set this to `false` to disable distributed tracing.
...
Starting with the Java agent `7.6.0` release, support for [Logs in Context](/docs/logs/logs-context/java-configure-logs-context-all/) has been added directly to the agent, making it easy to use for [supported logging frameworks](/docs/logs/logs-context/java-configure-logs-context-all/#automatic).

Changing these settings in your local agent configuration file are dynamic and does not require a restart of the agent for them to take effect.
...
<CollapserGroup>
  ...
  If you are using a supported logging framework and want to use the agent to send your application logs to New Relic, you can control that through settings under the `forwarding` stanza and can be overridden by the `newrelic.config.application_logging.forwarding` prefixed system property. Options available are:
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      Setting `max_samples_stored` to `0` does *not* disable the feature itself, though it does result in no log lines being sent to New Relic.
    </Callout>
