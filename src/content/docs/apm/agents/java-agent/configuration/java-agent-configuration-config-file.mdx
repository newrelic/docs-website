...
<CollapserGroup>
  ...
  >
    ...
    This setting is used for any sampling decision that does not have specific samplers set (i.e. `root`, `remote_parent_sampled` and `remote_parent_not_sampled`). Note that this sampling target will not override any specific samplers that are explicitly set. 
