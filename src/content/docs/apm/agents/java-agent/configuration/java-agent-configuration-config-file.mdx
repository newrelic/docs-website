...
<Collapser
  ...
  >
    ...
    The size of the queue used to store JFR events. Increasing this can reduce gaps in JFR reported data but can also cause resource issues in the agent or cause data to be dropped if backend pipeline limits are exceeded.
    Default is 250000. See [event API ingest limits](/docs/data-apis/ingest-apis/event-api/introduction-event-api/#limits) for more information.
