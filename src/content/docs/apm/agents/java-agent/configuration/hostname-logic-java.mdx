...
* Set the [`NEW_RELIC_PROCESS_HOST_DISPLAY_NAME`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#NEW_RELIC_PROCESS_HOST_DISPLAY_NAME) environment variable.  
  OR
* Set [`process_host.display_name`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#display_name) in your <DoNotTranslate>**newrelic.yml**</DoNotTranslate>.
...
For WebSphere and WebLogic users, the Java agent also reports the instance name for each active app server instance, which is visible in the APM <DoNotTranslate>**Settings > Environment**</DoNotTranslate> page. Generally, this name is provided by the app server. If an instance name is set, it will be used instead of the display name in the New Relic UI.
...
* <DoNotTranslate>**Host**</DoNotTranslate>: An identifier for your app's parent server that is derived from the Operating System. This value is not configurable by the Java agent.
* <DoNotTranslate>**Port**</DoNotTranslate>: The port associated with the app server. If the agent can't determine the port number, or your app is not running in an app server, defaults to <DoNotTranslate>**0**</DoNotTranslate>. This value is user-configurable.
* <DoNotTranslate>**Dispatcher Name**</DoNotTranslate>: The name of the app server. For example, Apache Tomcat, Glassfish, or WebLogic. If there is no active app server, defaults to <DoNotTranslate>**Unknown**</DoNotTranslate>.
