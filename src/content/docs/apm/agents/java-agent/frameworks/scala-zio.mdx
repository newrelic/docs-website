...
<Callout variant="important">
  `TraceOps.trace` only works with synchronous code so the segment will not time for 
  any ZIO operations that are done lazily/asynchrounously.
</Callout>

If you want to create segment for an asynchronous code block containing a Scala
Zio use `TraceOps.asyncTrace`. This will ensure the timing for the segment
...
val x: ZIO[_, Nothing, Int] = asyncTrace("segment name") {
  ZIO.succeed(1) <*
