...
To eliminate the erroneous duplication of transactions from being reported, intrumentation is <DoNotTranslate>**no longer**</DoNotTranslate> being applied to the `bindAndHandle` method, which starts a new HTTP server using a `akka.stream.scaladsl.Flow` instance. 
...
<Callout variant="important">
  A similar workaround, calling `bindAndHandle` when starting an HTTP server from a `akka.http.scaladsl.Route` using the Akka HTTP Routing DSL, is <DoNotTranslate>**not**</DoNotTranslate> necessary. Agent instrumentation will work normally when being called from other convenience methods.
</Callout>
