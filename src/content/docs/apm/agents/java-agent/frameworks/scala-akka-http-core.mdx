...
To eliminate the erroneous duplication of transactions from being reported, intrumentation is <DNT>**no longer**</DNT> being applied to the `bindAndHandle` method, which starts a new HTTP server using a `akka.stream.scaladsl.Flow` instance.
