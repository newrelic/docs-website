...
This section describes in more detail how New Relic handles Scala futures. 

Consider what happens when we call this transaction block, which takes a future as its parameter and chains it with a second future: 
...
You might expect `chainFutures` to produce a transaction lasting 7 seconds. Instead, the New Relic UI shows the transaction finishing immediately. What’s going on? 

New Relic instruments Scala futures when they are **submitted to the executor**. The future `f` in this example is submitted by the `Future.apply()` method when it is constructed - *before* the transaction exists. Because `f` is submitted outside the transaction, any threadhops made during its execution will not be linked together. The downstream future, `Future{ Thread.sleep(2000) }`, will also be untracked, because it is submitted in a callback after the transaction context is already gone. 

There are a few things we can do to get our transaction to capture the timing of both futures. First, although we might not be able to change when `f` is submitted, we can time it using the `asyncTrace` method discussed above. This will time `f` from the moment the transaction is started (but will not track threadhops made by `f`). Second, we can use a token to link the callback to the transaction, in order to preserve transaction context during threadhops. Finally, we can add another `asyncTrace` to time the second future. 
...
Now, the New Relic UI will show a transaction lasting about 7 seconds, with two segments named “first future” and “second future”. 
