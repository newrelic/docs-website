...
<CollapserGroup>
  <Collapser
    id="build-scala"
    title={<>Configure using the `project/build.scala` file</>}
  >
  ...
  <Collapser
    id="build.sbt"
    title={<>Configure using the `project/build.sbt` file</>}
  >
    Add the following line (replacing `S.V` with the Scala version
...
Future use `TraceOps.asyncTraceFun`. This will ensure the timing for the segment
