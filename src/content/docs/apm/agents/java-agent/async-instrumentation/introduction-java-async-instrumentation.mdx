...
In the APM UI, on the trace details waterfall view, the UI displays asynchronous activity as overlapping in the horizontal x axis. 

To get to the trace details page, go to: **[one.newrelic.com](https://one.newrelic.com) > APM > (select an app) > Transactions > Transaction trace > Trace details**.
