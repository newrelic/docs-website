...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (select an app) > Summary**: On the APM **Summary** page, asynchronous activity in a Java app can result in the response time (the blue line) being less than the total transaction time. This is because the methods don't have to wait for all preceding methods to complete before returning.
</figcaption>
