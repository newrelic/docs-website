...
* <DNT>**[Tokens](#tokens)**</DNT>: Tokens are passed between threads to link asynchronous units of work to a specific transaction. They do not perform any timing directly.
* <DNT>**[Segments](#segments)**</DNT>: Segments are used to measure an arbitrary piece of asynchronous application code, not necessarily associated with a method or thread. Segments are typically used to track external calls that are completed by a callback mechanism.
...
<CollapserGroup>
  ...
  >
    To view transaction trace details, go to: <DNT>**[one.newrelic.com](https://one.newrelic.com) APM & services > (select an app) > Transactions > Transaction trace > Trace details**</DNT>.
