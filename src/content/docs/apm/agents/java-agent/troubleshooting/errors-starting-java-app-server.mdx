---
title: Errors starting Java app server
type: troubleshooting
tags:
  - Agents
  - Java agent
  - Troubleshooting
metaDescription: 'Troubleshooting steps if you see errors (zip file, initialization, agent library) at app server startup with your New Relic Java agent.'
redirects:
  - /docs/agents/java-agent/troubleshooting/errors-starting-java-app-server
  - /docs/troubleshooting/error-starting-application-server-with-the-java-agent
  - /docs/java/error-starting-app-server
  - /docs/java/java-kb-106
  - /docs/agents/java-agent/troubleshooting/error-starting-app-server
  - /docs/agents/java-agent/troubleshooting/error-starting-app-server-java
freshnessValidatedDate: never
---

## Problem

You see error messages when starting the app server for your Java application. For example:

```
Error opening zip file: newrelic.jar 
Error occurred during initialization of VM 
Agent library failed to init: instrument
```

## Solution

The path to the Java agent `jar` file must be fully qualified in the `-javaagent` JVM switch. If the path is not correctly specified, errors will be thrown on app server startup, and the JVM will exit.

To correct this problem, specify the full path to the `newrelic.jar` file in the `-javaagent` switch. For example, if the `newrelic.jar` file is in a directory named `newrelic` inside the Tomcat home directory <DoNotTranslate>**/home/tomcat**</DoNotTranslate>, specify the switch like this:

```
-javaagent:/home/tomcat/newrelic/newrelic.jar
```
