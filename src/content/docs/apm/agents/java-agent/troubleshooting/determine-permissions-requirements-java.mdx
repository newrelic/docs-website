...
1. The owner of the Java process to which the `-javaagent` option will be passed must have read permissions for the Java agent JAR. This is because <DoNotTranslate>**Java**</DoNotTranslate> is what is being <DoNotTranslate>**executed**</DoNotTranslate>, and Java needs to <DoNotTranslate>**read**</DoNotTranslate> the `newrelic.jar`.
2. The process owner requires write/execute permissions for the directory in which the log directory will be created and execute permissions for all parent directories of the log directory. This parent directory could be any of the following:
   ...
   * `StdOut` if you have specified this location with the [`NEW_RELIC_LOG`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#ev-NEW_RELIC_LOG) environment variable
   ...
   If the New Relic log directory <DoNotTranslate>**already exists**</DoNotTranslate>, the process owner must have write/execute access in this directory so the java process can traverse into the directory and create the java agent log file.
