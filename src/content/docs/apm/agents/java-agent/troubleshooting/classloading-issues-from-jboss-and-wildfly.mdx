...
After implementing any of the below solutions, we recommend contacting support. Notifying our support team opens the door for a potential patch that makes JBoss recognize more packages. If `NoClassDefFoundError` is found in the agent logs, it means our agent is using that class. 
