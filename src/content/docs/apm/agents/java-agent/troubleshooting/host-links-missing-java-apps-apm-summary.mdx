...
Configure the hostname settings so that the New Relic infrastructure agent in a Linux agent environment will return the exact same name string as the New Relic Java agent. <DNT>**Recommendation:**</DNT> Edit the fully qualified domain name (FQDN) settings. The agents read their hostname from the operating system's FQDN settings, so setting the hostname there will ensure that both agents share a single hostname.
