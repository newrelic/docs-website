...
You cannot view links to your Linux hosts from your Java app's [APM <DoNotTranslate>**Summary**</DoNotTranslate> page](/docs/apm/applications-menu/monitoring/applications-overview-dashboard).

## Solution

Configure the hostname settings so that the New Relic infrastructure agent in a Linux agent environment will return the exact same name string as the New Relic Java agent. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Edit the fully qualified domain name (FQDN) settings. The agents read their hostname from the operating system's FQDN settings, so setting the hostname there will ensure that both agents share a single hostname.
...
<CollapserGroup>
  ...
  <Collapser
    id="aws-example"
    title="Example: Script hostname settings on <DoNotTranslate>AWS Elastic Beanstalk</DoNotTranslate>>"
  >
