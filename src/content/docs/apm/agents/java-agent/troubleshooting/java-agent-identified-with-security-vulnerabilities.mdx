...
tags:
  ...
  - Troubleshooting
redirects:
  - /docs/agents/java-agent/troubleshooting/java-agent-identified-with-security-vulnerabilities
---
...
The modules (in the `instrumentation` package) are named after the software frameworks they are designed to instrument and they are packaged as jar files inside the agent jar file (`newrelic.jar`). Some security scanning tools will detect these names and will interpret them as being the *actual* software framework itself, when it is in fact just an instrumentation module.
