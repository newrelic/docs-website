...
Suppress the false positive warnings coming from the `instrumentation` package in the newrelic.jar with your scanning tool, including all jar files whose names match the modules listed in https://github.com/newrelic/newrelic-java-agent/tree/main/instrumentation.
