...
You are using the [Solr platform](http://lucene.apache.org/solr/) in your web application, but Solr data does not appear in the <InlinePopover type="apm" /> UI for your app from **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select a Java app) > Monitor**.
