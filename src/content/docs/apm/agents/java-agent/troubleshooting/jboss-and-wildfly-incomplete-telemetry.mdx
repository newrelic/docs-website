...
It is recommended though to contact [support](https://support.newrelic.com/s/) if this problem is encountered. This is because if it is in the agent logs,
it is a class used by our agent. So being notified of this will allow later release to provide a patch to make JBoss recognize more packages.