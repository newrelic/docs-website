...
This document explains a basic installation of the <InlinePopup type="apm" /> agent for Java applications in a Docker container. We discuss required configurations and also explore some optional configurations, including:
...
1. Go to the API keys UI and get a <LicenseKey/>.
