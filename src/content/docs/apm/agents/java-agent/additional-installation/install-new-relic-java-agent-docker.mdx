---
title: Install New Relic Java agent for <DNT>Docker/DNT>
tags:
  - Agents
  - Java agent
  - Additional installation
metaDescription: How to install New Relic's Java agent for APM on a <DNT>Docker/DNT> container.
redirects:
...
This document explains a basic installation of the <InlinePopover type="apm" /> agent for Java applications in a <DNT>Docker/DNT> container. We discuss required configurations and also explore some optional configurations, including:
...
* How to use the <DNT>Docker/DNT> layer when every agent in every environment needs slightly different configuration data
* How to disable the New Relic agent in some environments and enable it in others

Although we don't discuss advanced options here, you can install the Java agent in <DNT>Docker/DNT> volumes and use your <DNT>Docker/DNT> container image in other software such as Swarm, ECS, AKS, EKS, OpenShift, and Kubernetes. Our <DNT>Docker/DNT> examples refer to <DNT>Tomcat</DNT>, so if you are using another application server, refer to your vendor’s documentation.
...
For example, with <DNT>Tomcat</DNT>, use commands like these in the `Dockerfile`:
...
For a basic <DNT>Docker/DNT> installation, complete these configurations:
...
Here’s a <DNT>Docker/DNT>file example where `tomcat` is the user who starts <DNT>Tomcat</DNT>:

```dockerfile
RUN mkdir -p /usr/local/tomcat/newrelic/logs
RUN chown -R <DNT>Tomcat</DNT>:tomcat /usr/local/tomcat/newrelic/logs
```

You can also send the logs to `STDOUT` by adding one of the following to the <DNT>Docker/DNT>file:
...
You can pass either a Java property or an environment variable to determine which of the environment-specific sections the agent uses in `newrelic.yml`. Use this approach if you prefer to have the `newrelic.yml` file control environment-specific configurations instead of passing all the configurations via <DNT>Docker/DNT>.

Here’s a `Dockerfile` example of passing the `newrelic.environment` Java system property via <DNT>Docker/DNT> to use the custom value `dev` in the environment section of `newrelic.yml`:
...
This configuration controls whether the agent is enabled. Let’s say you want the same <DNT>Docker/DNT> image for every installation. However, you don’t want to run the New Relic agent every time an engineer spins up a test app because you don’t want to run up your instance count.
...
## Additional <DNT>Tomcat</DNT> <DNT>Docker/DNT>file examples [#code-samples]
...
<CollapserGroup>
  ...
  >
    ...
    # Config to include the agent logs in <DNT>Docker/DNT>'s stdout logging
...
Now that you have a basic agent installation in <DNT>Docker/DNT>, here are some additional steps to consider:

* Review other [configurations](/docs/agents/java-agent/configuration/java-agent-configuration-config-file) for the agent.
* Read a detailed [Support Forum post](https://discuss.newrelic.com/t/relic-solution-what-you-need-to-know-about-new-relic-when-deploying-with-docker/52492) about <DNT>Docker/DNT> and New Relic.
