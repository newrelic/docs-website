...
2. Append the following text to your policy file, Ensure you preserve the dash `-` at the end of the path, which grants all jars in the target folder the necessary permissions.
   ...
   <Callout variant="tip">
      If you are using Java 9+ and encountering access denied errors you may need to grant additional permissions (e.g. `java.lang.RuntimePermission "accessSystemModules"`).
   </Callout>
