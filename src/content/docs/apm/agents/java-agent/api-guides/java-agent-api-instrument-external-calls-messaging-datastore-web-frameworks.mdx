...
<Callout variant="important">
  All of the methods of this class have the potential to expose sensitive
  private information. Use caution when creating the arguments, paying
  particular attention to URIs and string values.
</Callout>
...
<Callout variant="important">
  The messaging API relies on two-way communication between producers and
  consumers. If your producer does not receive an acknowledgment from a
  consumer, like in a fire-and-forget pattern, the messaging API will not
  accurately reflect the interactions with message queue brokers.
</Callout>
...
<CollapserGroup>
  <Collapser id="datstore" title="Datastore API implementation">
    ``` TracedMethod tracedMethod = NewRelic.getAgent().getTracedMethod();
