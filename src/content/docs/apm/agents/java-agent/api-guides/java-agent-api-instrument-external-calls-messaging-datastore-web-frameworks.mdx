...
This API call provides the same behavior as the [Datastore API call](#datastore-api) and extends it to allow [slow query](/docs/apm/apm-ui-pages/monitoring/view-slow-query-details/) information to be tracked. The same `reportAsExternal(ExternalParameters params)` method and builder are used, but with an additional builder method.
