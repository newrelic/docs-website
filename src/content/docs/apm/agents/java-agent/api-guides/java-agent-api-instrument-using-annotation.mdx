...
<CollapserGroup>
  ...
  >
    ...
    If you set the `metricName` in addition to the dispatcher, as in `@Trace(metricName="YourMessageHere", dispatcher=true)`, then the transaction name will appear as YourMessageHere in the APM <DNT>**Transactions**</DNT> page but the time spent in this method will not appear as YourMessageHere in any transaction trace.
