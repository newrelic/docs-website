...
This setting is <DoNotTranslate>**not**</DoNotTranslate> included in the `newrelic.yml` by default. If you want to disable annotations, set `enable_custom_tracing: false` (prefaced with two spaces) in the `common` section of your `newrelic.yml`.
...
<CollapserGroup>
  ...
  >
    ...
    If you set the `metricName` in addition to the dispatcher, as in `@Trace(metricName="YourMessageHere", dispatcher=true)`, then the transaction name will appear as YourMessageHere in the APM <DoNotTranslate>**Transactions**</DoNotTranslate> page but the time spent in this method will not appear as YourMessageHere in any transaction trace.
