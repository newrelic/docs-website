...
To view your JVM instances, go to **[one.newrelic.com](https://one.newrelic.com) > Explorer > Services - APM > (select an app) > Monitor > JVMs**.

[LANDING SCREENSHOT HERE]

On this view you can compare several JVMs across all of the health metrics and runtime metrics:

* Sort to find outliers.
* Use the filter bar to narrow down your search using tags. 

Select a single or multiple JVMs, or click a single JVM's name, to see a pane with data faceted in a more compact manner, organized in 3 columns:
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > Services - APM > (select an app) > Monitor > JVMs**: Depending on the metrics your app server is reporting, one or more tabs appear on your JVM page.
</figcaption>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        * **Heap memory usage:** Each chart shows the precentage of used against committed heap space for a given heap.
        * **Non-heap memory pool usage for the JVM:** The used code cache and used CMS Perm Gen in MB.
        * **Garbage collection:** The percentage of garbage collection CPU time against wall clock time.
