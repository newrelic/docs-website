...
Distributed tracing is available for services instrumented with the New Relic APM agent for Java. Here are several ways to leverage the power of troubleshooting with distributed tracing:
...
For services that are not currently instrumented with the Java agent, you can run our installation assistant that both instruments your service and enables distributed tracing.
...
To start the installation assistant, click the link below. When you are finished, repeat this process for other services in your requests.
