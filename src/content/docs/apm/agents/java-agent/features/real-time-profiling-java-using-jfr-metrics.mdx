...
<Callout variant="important">
  Apps running with the JFR daemon should expect the JFR subsystem to use
  additional memory.
</Callout>
...
![New Relic One Java Flight Recorder (JFR) UI](src/images/new-relic-one-java-flight-record-ui.jpg 'new-relic-one-java-flight-record-ui')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > (select service) >
  Realtime Profiling Java**: The JVM cluster timeline view shows the JVM
...
![How JVM health is measured](src/images/new-relic-one-JVM-health.jpg 'JVM-health-info')

<figcaption>
  Select **How is JVM health determined?** for a detailed breakdown of how JVM
...
![New Relic One Java Flight Recorder details page UI](src/images/new-relic-one-java-flight-recorder-details-ui_0.jpg 'new-relic-one-java-flight-recorder-details-ui.jpg')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > (select service) >
  Realtime Profiling Java > (select JVM)**: You can see details for each JVM.
</figcaption>

## Identify resource-intensive code paths with flame graphs [#flamegraph]

<Callout variant="important">
  The flame graph feature is only compatible with the [New Relic Java agent JFR
  service](https://github.com/newrelic/newrelic-jfr-core/blob/main/README.md#new-relic-java-agent-jfr-service)
  usage scenario.
</Callout>
...
![Screenshot of New Relic Java flame graph.](src/images/jfr_flamegraphs.png 'Screenshot of New Relic Java flame graph')

<figcaption>
  **[one.newrelic.com](one.newrelic.com/) > Explorer > (select service) >
  Realtime Profiling Java > (select JVM)**: You can see details for each JVM,
