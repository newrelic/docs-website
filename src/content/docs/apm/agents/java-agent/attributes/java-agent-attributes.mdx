...
You can configure which types of attributes, or which specific attributes, the Java agent reports to New Relic. This is often done for security reasons, when there are certain sensitive attributes you do not want reported to New Relic. To learn what settings override other settings, see the [attribute configuration rules](#attruls).
