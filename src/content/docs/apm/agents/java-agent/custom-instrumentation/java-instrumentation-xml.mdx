...
Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Instrumentation**</DoNotTranslate>. From here you can:
...
<table>
  <thead>
    <tr>
      <th width={200}>
        <DoNotTranslate>**XML file location options**</DoNotTranslate>
      </th>
...
<DoNotTranslate>**Legacy users:**</DoNotTranslate> If both a YAML and XML file exist with the same extension name, only the XML file will be instrumented. The YAML file will be ignored. If you want both the YAML and XML files to be instrumented, give each file a different extension name.
