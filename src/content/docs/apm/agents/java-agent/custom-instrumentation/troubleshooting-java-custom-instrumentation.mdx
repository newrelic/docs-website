...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (select an app) > Events > Thread profiler:** If a method is marked gray (**Instrumentation not allowed**), try expanding the thread profile tree to find a nested transaction that you can instrument (white).
</figcaption>
