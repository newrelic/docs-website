...
You can use New Relic's UI to [define instrumentation rules](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-quickly-customize-your-java-instrumentation) with the Custom Instrumentation Editor, or edit your XML file via your Java app's <DoNotTranslate>**Settings**</DoNotTranslate>.

<DoNotTranslate>**Java apps only:**</DoNotTranslate> To edit your XML file directly from the New Relic UI: Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select a Java app) > Settings > Live Instrumentation**</DoNotTranslate>. From here you can:
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Events > Thread profiler:**</DoNotTranslate> A gray icon means it cannot be instrumented. A white icon means it is uninstrumented but can be instrumented. A blue icon means it is currently instrumented.  
</figcaption>
...
Here is an example of a pointcut with several `method` values. The `nameTransaction` has been added to the XML to break out the method as a separate transaction rather than as a segment in the APM [<DoNotTranslate>**Transactions**</DoNotTranslate> breakdown table](/docs/apm/applications-menu/monitoring/transactions-page).
