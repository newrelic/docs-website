...
import apmJavaCustomInstColorCoding from 'images/apm_screenshot-crop_Java-custom-inst-color-coding.png'
...
Custom instrumentation is useful for nested transactions. For example, in the thread profiler, methods with gray icons mean that instrumentation isn't allowed because instrumentation would result in unacceptable overhead. However, children of those methods usually can be instrumented and will have white icons if they can be. 

<img
  title="Thread profiler: Nested transactions colors"
  alt="Thread profiler: Nested transactions colors"
  src={apmJavaCustomInstColorCoding}
/>
