...
![Thread profiler: Nested transactions colors](src/images/crop-java-custom-inst-colors.png 'Thread profiler: Nested transactions colors')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (select an app) >
  Events > Thread profiler:** If a method is marked gray (**Instrumentation not
  allowed**), try expanding the thread profile tree to find a nested transaction
  that you can instrument (white).
</figcaption>
