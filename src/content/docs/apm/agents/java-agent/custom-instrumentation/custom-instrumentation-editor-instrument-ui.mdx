...
New Relic's custom instrumentation editor allows Java app users to implement custom instrumentation via the New Relic user interface. The editor is the preferred choice when you cannot modify your application code and do not have that many methods to instrument. See [Java custom instrumentation](/docs/agents/java-agent/custom-instrumentation/java-custom-instrumentation) for other instrumentation options and the reasons for using each.
