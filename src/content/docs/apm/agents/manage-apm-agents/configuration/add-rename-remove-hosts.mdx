...
Once all reporting applications are shut down on the old host, it takes at least 24 hours for the old host to disappear from the New Relic list. If you've added a new host to replace the one you've removed, it can take up to 10 minutes for that host to appear in the UI. 
