...
1. Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Application**.
...
<Callout variant="tip">
When you make changes via **Application settings**, the saved changes restarts the agents.  Only the agent for this specific service will restart, but the action will not restart the app pool or any web server.  
</Callout>
