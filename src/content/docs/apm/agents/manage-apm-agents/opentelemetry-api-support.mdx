...
<CollapserGroup>
  ...
  >
    ...
    The Ruby agent detects the presence of the OpenTelemetry API installation and instruments OpenTelemetry APIs for Tracing to generate telemetry. The OpenTelemetry API must be present in your application for the hybrid agent functionality to work. When a library or your application code calls OpenTelemetry APIs, the New Relic Ruby agent translates those calls into New Relic data if the OpenTelemetry bridge is enabled.
