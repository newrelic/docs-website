...
We have introduced support for Span Links, allowing you to associate traces that were sampled independently, ensuring no loss of context in complex, asynchronous workflows. For instructions on adjusting these targets or configuring OTel compatibility, refer to :

* [APM Agent Configuration documentation](/docs/apm/agents/manage-apm-agents/configuration)
* [OpenTelemetry API support](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support)
...
<CollapserGroup>
  ...
  >
    ...
    **2. Enable New Relic OpenTelemetry support**
    ...
    <Callout variant="important">
    The Java agent instruments the OpenTelemetry trace, metrics, and logs APIs to capture signals emitted by your application. It does **not** ship or automatically enable any OpenTelemetry instrumentations.
    </Callout>
    ...
    OpenTelemetry standalone library instrumentations and native instrumentation built directly into libraries will work with the New Relic agent. However, OpenTelemetry auto-instrumentation that uses bytecode manipulation will conflict with the New Relic agent.
  </Collapser>
  ...
  >
    ...
    **Traces** and **Metrics** are both enabled by default when OpenTelemetry support is enabled.
    ...
    You can fine-tune which OpenTelemetry instrumentations are enabled for traces and metrics:
    ...
    **Using newrelic.config:**
    ...
    <configuration>
      <openTelemetry enabled="true">
        ...
        <metrics enabled="true" include="" exclude="" />
      </openTelemetry>
    </configuration>
    ```

    The `include` and `exclude` attributes accept comma-separated lists of activity source names (for traces) or meter names (for metrics). See the sections below for details on which instrumentations can be configured.
    ...
    For the current list of libraries with native OpenTelemetry instrumentation, see the [OpenTelemetry documentation for .NET traces instrumentations](https://opentelemetry.io/docs/zero-code/dotnet/instrumentations/#traces-instrumentations).
    ...
    For the current list of available metric instrumentations, see the [OpenTelemetry documentation for .NET metrics instrumentations](https://opentelemetry.io/docs/zero-code/dotnet/instrumentations/#metrics-instrumentations) and [Microsoft's built-in metrics documentation](https://learn.microsoft.com/en-us/dotnet/core/diagnostics/built-in-metrics).
    ...
    ### Handling instrumentation conflicts

    Some OpenTelemetry instrumentations may conflict with New Relic's built-in instrumentation. When both New Relic and OpenTelemetry instrument the same library, you can choose which instrumentation to use:

    * **By default**: New Relic's built-in instrumentation takes precedence
    * **To use OpenTelemetry instrumentation**: Use the `include` configuration option to explicitly enable specific OpenTelemetry instrumentations

    <Callout variant="caution">
    OpenTelemetry instrumentations are not actively tested or supported by New Relic. You should thoroughly test them in your environment before using in production.
    </Callout>
  ...
  >
    ...
    #### Span Links
    ❌ **Not supported**: Span Links are not currently available in the Ruby agent

    #### Events on Spans
    ❌ **Not supported**: Events on Spans are not currently available in the Ruby agent

    ### How it works

    The Ruby agent instruments OpenTelemetry APIs for Tracing to generate telemetry. When a library or your application code calls OpenTelemetry APIs, the New Relic Ruby agent translates those calls into New Relic data if the OpenTelemetry bridge is enabled.
    ...
    When the OpenTelemetry bridge is enabled, calling OpenTelemetry APIs in your application code will generate New Relic telemetry automatically.

    ### Known limitations

    - **Limited API support**: Only Traces API is supported in this release
    - **Metrics and Logs**: Not available
    - **Double instrumentation**: Work is ongoing to automate the prevention of duplicate telemetry when both OpenTelemetry and New Relic have instrumented the same methods. For now, you may need to manually disable one instrumentation to prevent duplicates.
