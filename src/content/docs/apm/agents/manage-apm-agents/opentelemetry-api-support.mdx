...
<CollapserGroup>
  ...
  >
    ...
    Libraries with built-in OpenTelemetry instrumentation calls:

    * **Elasticsearch**: Native OTel instrumentation (also instrumented by NR Ruby agent)
    * **Dalli 4.2.0+**: Native OTel instrumentation (also instrumented by NR Ruby agent)

    #### 2. Third-party instrumentation (opentelemetry-ruby-contrib)

    If you install gems from the [opentelemetry-ruby-contrib repository](https://github.com/open-telemetry/opentelemetry-ruby-contrib), their calls to OpenTelemetry APIs will be processed and transformed into New Relic data. The following libraries have OTel-contrib instrumentation but are **not** instrumented by the NR Ruby agent:

    * `que`
    * `pg` (covered by Active Record in NR agent)
    * `trilogy` (covered by Active Record in NR agent)
    * `faraday` (NR agent instruments most adapters it uses)
    * `active-model-serializers`
    * `anthropic`
    * `gruf`
    * `koala`
    * `lmdb`
    * `mysql2` (covered by Active Record in NR agent)
    * `racecar`
    * `restclient`
    * `factorybot` (testing library, agent not recommended for testing)
    * `rspec` (testing library, agent not recommended for testing)
