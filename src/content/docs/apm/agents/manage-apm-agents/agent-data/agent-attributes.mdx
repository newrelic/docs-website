...
* <DNT>**Message property attributes:**</DNT> These are the properties set on a message received from a queue or topic.
* <DNT>**HTTP request attributes:**</DNT> These are the parameters for an HTTP request.
* <DNT>**User attributes:**</DNT> These are attributes provided by the user through each agent's API.
* <DNT>**Agent attributes:**</DNT> These are attributes captured by the agent; for example, `http.statusCode` and `httpResponseMessage`.
...
<figcaption>
  <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > Transactions > (select a transaction) > (select a trace):**</DNT> Here is an example from the Ruby agent of the <DNT>**Request attributes**</DNT> and <DNT>**Custom attributes**</DNT> for a transaction trace. Exact attributes depend on your agent and on your attribute configuration.
</figcaption>
