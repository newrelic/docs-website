...
* [Catch errors](#error-collection) that we do not instrument by default.
* [Ignore errors](#ignore) that you do not want reported at all.
...
Sometimes the APM agent instruments an error that you do not want reported, such as errors that contain sensitive information like user login errors. If you do not want an error to report to the our [collector](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector), you can ignore the error, and the APM agent discards the error entirely.

<Callout variant="tip">
  For Java, Ruby, Node.js, and Python: If you want to report errors to APM but do not want those errors to affect your Apdex or error rate, [mark them as expected](#expected-links) instead.
</Callout>
