...
/>
You can use the **Distributed tracing insights** chart to learn whether entities downstream or upstream of your service might be causing increases in the response time, error rate, or throughput of your service. 

For example, let's say you want to dig into a spike in the response time of your service when it's related to the external call response time.
If distributed tracing recorded a downstream entity causing significantly increased latency for your service around that time, it will show that entity and its change in performance in the list.
You can click the "View trace" button to see an example distributed trace where this performance behavior was recorded.
