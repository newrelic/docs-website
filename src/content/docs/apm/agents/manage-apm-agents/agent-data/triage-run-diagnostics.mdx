...
Let's examine the sections of the screenshot below:
...
* **External** represents a call being made that we began monitoring before we know where it's going.  
*  **Middleware** most often refers to a dependency or library that is being used as part of your application's functionality. 
* **Segment** consists of the name of the function/call segment being monitored . We can see POST which is the type of call you would make to update or change something in your database as part of basic CRUD functionality. We can then see the MYSQL database operations taking place and the middleware doing whatever it's doing.
* **% Time** refers to the percentage of time each segment takes of the total transaction lifecycle. It's important to remember that some segments wait until all the other segments are completed before they're completed. In the screenshot below, the `Controller` segment is what is initiating the `POST` call, which means that all the other transactions are _nested_ within the `POST` call, which could be why it's taking longer than all the others. 
