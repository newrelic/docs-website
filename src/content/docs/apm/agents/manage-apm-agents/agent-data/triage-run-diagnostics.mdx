...
For the `Sinatra/Rubytron/POST #purchaseCart` transaction there are many different areas where the transaction could be failing: databases, servers, controllers, middleware and more. You can toggle between each different segment of the transaction to identify trends and spikes.
