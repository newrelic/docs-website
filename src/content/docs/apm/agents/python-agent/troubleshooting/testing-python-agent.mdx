...
<CollapserGroup>
  ...
  >
    <DoNotTranslate>**ERROR - No license key was set in agent configuration.**</DoNotTranslate>
...
* <DoNotTranslate>**Port:**</DoNotTranslate> A value of `0` for the port setting indicates that the appropriate port, such as <DoNotTranslate>**80**</DoNotTranslate> or <DoNotTranslate>**443**</DoNotTranslate>, will be used depending on whether or not SSL is enabled.
* <DoNotTranslate>**Proxy:**</DoNotTranslate> If you have configured the agent to use a proxy, make sure the proxy settings reflect this. If no proxy settings are specified but any HTTP connections to the wider Internet are necessary, then make sure the appropriate proxy settings appear in the agent configuration file.

### Check hostname lookup [#host-name-lookup]

If the data collector appears unreachable:

1. Check that the hostname for the primary data collector host can be resolved with UNIX system tools such as <DoNotTranslate>**dig**</DoNotTranslate> or <DoNotTranslate>**nslookup.**</DoNotTranslate>
...
<Callout variant="important">
  ...
  * TCP port <DoNotTranslate>**443**</DoNotTranslate> if using HTTPS over a SSL connection
  * TCP port <DoNotTranslate>**80**</DoNotTranslate> if using a plain HTTP connection
</Callout>

If the hostname for the data collector host can be resolved, use UNIX system tools such as <DoNotTranslate>**ping**</DoNotTranslate> and <DoNotTranslate>**traceroute**</DoNotTranslate> to determine if network traffic can be routed to it. The network traffic path will vary. As long as the path reaches the final intended host, the length of the path and its hosts are not important.
