...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (select an app) > Monitoring > Transactions > (select a transaction):** The transaction’s **Breakdown table** flags event loops with asyncio.
</figcaption>
