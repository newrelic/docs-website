...
The [**Transaction trace details** page](/docs/apm/transactions/transaction-traces/transaction-trace-details) may show `Put` and `Take` operations for RabbitMQ in the **Slowest components** section. For example:
