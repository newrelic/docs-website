...
1. At the top of the template file, add a reference to the tag library to the list of tag libraries being loaded by the load macro.
   ...
   {% load ... newrelic_tags %}
   ```
2. Add the following as close to the start of the `<head>` element of the template page as possible. Place this <DNT>**after**</DNT> any meta tags specifying a charset property or any `http-equiv` meta tag for `X-UA-Compatible`.
