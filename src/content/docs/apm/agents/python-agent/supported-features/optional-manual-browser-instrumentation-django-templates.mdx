...
Follow this procedure <DoNotTranslate>**only**</DoNotTranslate> if you need to use manual instrumentation to insert the browser monitoring agent instead of the Python agent's [automatic instrumentation](/docs/agents/python-agent/supported-features/page-load-timing-python).
...
1. At the top of the template file, add a reference to the tag library to the list of tag libraries being loaded by the load macro.
   ...
   {% load ... newrelic_tags %}
   ```
2. Add the following as close to the start of the `<head>` element of the template page as possible. Place this <DoNotTranslate>**after**</DoNotTranslate> any meta tags specifying a charset property or any `http-equiv` meta tag for `X-UA-Compatible`.
