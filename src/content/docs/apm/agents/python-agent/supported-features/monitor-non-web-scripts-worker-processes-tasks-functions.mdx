...
But you can also monitor arbitrary job-based systems, standalone scripts, worker processes, and non-web transactions using the `background_task` method. These transactions show up as [non-web transactions](/docs/apm/applications-menu/monitoring/transactions-page#tx_types) in the New Relic UI.

The Python agent also has built-in instrumentation for the task-queuing systems [Celery](/docs/agents/python-agent/back-end-services/python-agent-celery) and Gearman.

## Integrate the Python agent [#integrating]

The first requirement is getting the agent initialized and running with the process to be monitored. This is the same process described in [Standard Python install](/docs/agents/python-agent/getting-started/python-agent-quick-start) for a web application.
