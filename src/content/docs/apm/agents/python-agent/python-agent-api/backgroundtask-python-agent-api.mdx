...
If a function isn't already instrumented, use [function_trace()](/docs/apm/agents/python-agent/python-agent-api/functiontrace-python-agent-api) to create a function trace for that function within a background task.  Alternatively, the [config file](/docs/apm/agents/python-agent/custom-instrumentation/python-custom-instrumentation-config-file/#listing_functions) can be used to instrument any uninstrumented functions.
</Callout>
