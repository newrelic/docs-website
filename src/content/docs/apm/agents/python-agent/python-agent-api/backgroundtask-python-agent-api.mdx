...
* <DoNotTranslate>**The context manager**</DoNotTranslate>: If the tasks to be monitored will only be determined at runtime (and not import time), you can use the `BackgroundTask` context manager to directly wrap the execution of a block of code. (This is the context manager used by `BackgroundTaskWrapper`.)
* <DoNotTranslate>**The wrapper**</DoNotTranslate>: If you know in advance where the specific functions you want to track are, you can use the `background_task` decorator. But if you don't know all the functions that need to be traced (for example, if they're being looked up dynamically as part of a routing system), then you must use the `BackgroundTaskWrapper` to wrap the function at the time of registration or at the time of calling.
* <DoNotTranslate>**The path-based wrapper**</DoNotTranslate>: The path-based wrapper form is `wrap_background_task`.
