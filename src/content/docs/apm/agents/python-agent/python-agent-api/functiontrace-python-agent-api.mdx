...
If you know in advance where the specific functions you want to trace are, you could use the `function_trace` decorator. However, if you <DoNotTranslate>**don't**</DoNotTranslate> know all the functions that need to be traced (for example, if they're being looked up dynamically as part of a routing system), then you must use the `FunctionTraceWrapper` to wrap the function at the time of registration or at the time of calling.
