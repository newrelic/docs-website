...
* **The wrapper**: If you know in advance where the specific functions you want to track are, you can use the `web_transaction` decorator. But if you do not know all the functions that need to be traced (for example, if they're being looked up dynamically as part of a routing system), then you must use the `WebTransactionWrapper` to wrap the function at the time of registration or at the time of calling.
