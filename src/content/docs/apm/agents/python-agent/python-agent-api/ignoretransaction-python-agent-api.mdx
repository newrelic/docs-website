---
title: ignore_transaction (Python agent API)
type: apiDoc
shortDescription: Ignores the current transaction.
tags:
  - Agents
  - Python agent
  - Python agent API
metaDescription: 'Python API: This call ignores a transaction.'
redirects:
  - /docs/agents/python-agent/python-agent-api/ignoretransaction-python-agent-api
  - /docs/agents/python-agent/python-agent-api/ignore_transaction
---

## Syntax

```
newrelic.agent.ignore_transaction(flag=True)
```

Ignores the current transaction.

## Description

This call ignores the current transaction. The agent does not report any data from the transaction.

<Callout variant="tip">
  You can also ignore a web transaction in the WSGI environ dictionary. To do so, set the `newrelic.ignore_transaction` key for the specific request in the WSGI environ dictionary passed by the WSGI server into your WSGI application.
</Callout>

## Parameters

<table>
  <thead>
    <tr>
      <th width="25%">
        Parameter
      </th>

      <th>
        Description
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `flag`

        _boolean_
      </td>

      <td>
        Optional. Default is `True`. If the transaction is already ignored, you can un-ignore it by setting the flag to `False`.
      </td>
    </tr>
  </tbody>
</table>

## Return values

None.

## Examples

### Ignore a transaction [#ignore-example]

To ignore a transaction that you do not want to show up in the New Relic One, you would run the following where a transaction is being generated in your app code. This example illustrates using the API to ignore a Flask route:

```
@app.route("/")
def process_results():
    newrelic.agent.ignore_transaction(flag=True)
    return 'hello world'
```
