...
You can also use custom attributes to troubleshoot performance issues. For example, you might see occasional slow response times from a pool of memcache instances, but you do not know what instance is causing the problem. You might add an attribute to the span indicating the server, like so:
