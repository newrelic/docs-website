...
This endpoint takes in a single input, a callback, which is used to register error groups.  This callback is defined by the customer and must accept the exception that triggered the agent's [`notice_error` API](/docs/agents/python-agent/python-agent-api/noticeerror-python-agent-api).  To unset this setting, call the API again and set the callback to `None`.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Required. The callback function is defined by the customer.  Use `None` to unset.
      </td>
    </tr>
