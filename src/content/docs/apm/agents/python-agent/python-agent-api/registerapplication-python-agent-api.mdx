...
If the script itself runs the WSGI server, call `register_application` from the main program thread in the main script file. If you use Apache/mod_wsgi or uWSGI, make the call from the WSGI script file and place the call immediately after `initialize`<DoNotTranslate>**.**</DoNotTranslate>
