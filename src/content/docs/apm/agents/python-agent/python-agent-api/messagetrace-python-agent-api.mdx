...
* <DoNotTranslate>**The context manager**</DoNotTranslate>: The context manager form is `MessageTrace`. It takes the same parameters as the decorator.
* <DoNotTranslate>**The wrapper**</DoNotTranslate>: The wrapper form is `MessageTraceWrapper`. It can be used to return a wrapped function without the use of a decorator.
* <DoNotTranslate>**The path-based wrapper**</DoNotTranslate>: The path-based wrapper form is `wrap_message_trace`. This applies the `MessageTraceWrapper` to a given object through monkey patching. This takes the same parameters as the decorator plus an additional `module` and `object_path` parameter.
