...
* <DoNotTranslate>**The context manager**</DoNotTranslate>: The context manager form is `DatastoreTrace`.
* <DoNotTranslate>**The wrapper**</DoNotTranslate>: The wrapper form is `DatastoreTraceWrapper`. It can be used to return a wrapped function without the use of a decorator.
* <DoNotTranslate>**The path-based wrapper**</DoNotTranslate>: The path-based wrapper form is `wrap_datastore_trace`. This applies the `DatastoreTraceWrapper` to a given object through monkey patching. This takes the same parameters as the decorator plus an additional `module` and `object_path` parameter.
