  ...
  <tbody>
    <tr>
      <td>
        ...
        _tuple, caught BaseException_
      </td>

      <td>
        Optional and rarely used. Either a tuple containing exception information `(exception_class, exception_instance, traceback)` returned from [`sys.exc_info()`](https://docs.python.org/3/library/sys.html#sys.exc_info) or a caught instance of `BaseException` from a `try` / `except` block.
        
        _Note: A traceback is always required, either as the third element of the exception tuple or as the `__traceback__` attribute on the instance of BaseException. Tuples from `sys.exc_info()` and BaseException instances from `try` / `except` blocks will already have a traceback._
      </td>
    </tr>
