...
<CollapserGroup>
  ...
  >
    ...
    `always_on` means that a trace that has a remote parent sampled will always be sampled.
    `always_off` means that a trace that has a remote parent sampled will never be sampled.
