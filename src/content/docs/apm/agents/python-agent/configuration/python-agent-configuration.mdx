...
<CollapserGroup>
  ...
  >
    ...
    Enables [full granularity distributed tracing](/docs/distributed-tracing/configure-partial-granularity-tracing). Full granularity captures all spans and attributes within the transaction.
  
  </Collapser>
  <Collapser
    id="distributed-tracing-sampler-adaptive-sampling-target"
    title="distributed_tracing.sampler.adaptive_sampling_target"
  >
    <table>
      <tbody>
        <tr>
    ...
    Global/shared adaptive sampling target for traces from all sections (`root`, `remote_parent_sampled`, `remote_parent_not_sampled`). See [Adaptive sampling targets](/docs/distributed-tracing/sampling/configure-sampling.mdx#adaptive-targets) for details.
    - 1-120 inclusive
  </Collapser>
  <Collapser
    id="distributed-tracing-sampler-remote-parent-sampled"
  ...
  >
    ...
    Adaptive sampling target for traces that were sampled by an upstream service and do not have a matching trusted account ID.
    - 1-120 inclusive
  </Collapser>
  <Collapser
  ...
  <Collapser
    id="distributed-tracing-sampler-remote-parent-not-sampled"
  ...
  >
    ...
    Adaptive sampling target for traces that were not sampled by an upstream service and do not have a matching trusted account ID.
    - 1-120 inclusive
  </Collapser>
  <Collapser
  ...
  >
    ...
    Adaptive sampling target for traces that originate from the current service.
    - 1-120 inclusive
  </Collapser>
  <Collapser
  ...
  >
    ...
    Enables [partial granularity distributed tracing](/docs/distributed-tracing/configure-partial-granularity-tracing). Partial granularity captures a subset of spans and attributes within the transaction depending on the configuration setting [distributed_tracing.sampler.partial_granularity.type](#distributed-tracing-sampler-partial-granularity-type).
  
  </Collapser>
  <Collapser
    id="distributed-tracing-sampler-partial-granularity-type"
    title="distributed_tracing.sampler.partial_granularity.type"
  >
    <table>
      <tbody>
        <tr>
    ...
    Sets the partial granularity mode. See [Understanding the three modes](/docs/distributed-tracing/configure-partial-granularity-tracing#modes) for details. Options:
    - reduced
    - essential
    - compact
  ...
  <Collapser
    id="distributed-tracing-sampler-partial-granularity-remote-parent-sampled"
  ...
  >
    ...
    This is the sampler that is applied when the trace has a remote parent (that is, the trace originated in an upstream service) that has been sampled. The trace is captured as a [partial granularity distributed trace](/docs/distributed-tracing/configure-partial-granularity-tracing). Sampler options are:
    - `adaptive` is the default behavior and delegates the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled.
    - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled. If you use this option, the ratio is required, so use [distributed_tracing.sampler.partial_granularity.remote_parent_sampled.trace_id_ratio_based.ratio](#distributed-tracing-sampler-partial-granularity-remote-parent-sampled-trace-id-ratio-based-ratio) instead.
    - `always_on`: Always samples traces that have a sampled remote parent.
    - `always_off`: Never samples traces that have a sampled remote parent.
  ...
  <Collapser
    id="distributed-tracing-sampler-partial-granularity-remote-parent-not-sampled"
  ...
  >
    ...
    This is the sampler that is applied when a trace has a remote parent (that is, the trace originated in an upstream service) that was not sampled. The trace is captured as a [partial granularity distributed trace](/docs/distributed-tracing/configure-partial-granularity-tracing). Sampler options are:
    - `adaptive` is the default behavior and delegates the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled.
    - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled. If you use this option, the ratio is required, so use [distributed_tracing.sampler.remote_parent_not_sampled.trace_id_ratio_based.ratio](#distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio) instead.
    - `always_on`: Always samples traces that have an unsampled remote parent.
    - `always_off`: Never samples traces that have an unsampled remote parent.
  ...
  >
    ...
    This is the sampler that is applied when a trace has a remote parent (that is, the trace originated in an upstream service) that was not sampled. Sampler options are:
    - `adaptive` is the default behavior and delegates the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled.
    - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled. If you use this option, the ratio is required, so use [distributed_tracing.sampler.partial_granularity.remote_parent_not_sampled.trace_id_ratio_based.ratio](#distributed-tracing-sampler-partial-granularity-remote-parent-not-sampled-trace-id-ratio-based-ratio) instead.
    - `always_on`: Always samples traces that have an unsampled remote parent.
    - `always_off`: Never samples traces that have an unsampled remote parent.
  </Collapser>
  <Collapser
  ...
  >
    ...
    This is the sampler that is applied at the beginning of a trace. By default, the Adaptive sampler is configured as the root sampler.partial_granularity. The trace is captured as a [partial granularity distributed trace](/docs/distributed-tracing/configure-partial-granularity-tracing). Sampler options are:
    - `adaptive` is the default behavior and delegates the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled.
    - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled. If you use this option, the ratio is required, so use [distributed_tracing.sampler.partial_granularity.root.trace_id_ratio_based.ratio](#distributed-tracing-sampler-partial-granularity-root-trace-id-ratio-based-ratio) instead.
    - `always_on`: Always samples transactions that originate from the current service and do not have a distributed tracing header present for the entry call.
    - `always_off`: Never samples transactions that originate from the current service and do not have a distributed tracing header present for the entry call.
  ...
  >
    ...
    This is the sampler that is applied at the beginning of a trace. By default, the Adaptive sampler is configured as the root sampler.partial_granularity. Sampler options are:
    - `adaptive` is the default behavior and delegates the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled.
    - `trace_id_ratio_based` delegates the sampling decision to the Trace ID Ratio Based sampling algorithm to determine whether a trace will be sampled. If you use this option, the ratio is required, so use [distributed_tracing.sampler.partial_granularity.root.trace_id_ratio_based.ratio](#distributed-tracing-sampler-partial-granularity-root-trace-id-ratio-based-ratio) instead.
    - `always_on`: Always samples transactions that originate from the current service and do not have a distributed tracing header present for the entry call.
    - `always_off`: Never samples transactions that originate from the current service and do not have a distributed tracing header present for the entry call.
