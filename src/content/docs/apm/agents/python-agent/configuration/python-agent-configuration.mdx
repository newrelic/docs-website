...
Let's say an application is deployed across 10 hosts, each running 4 processes per host. To limit the number of span events to 10,000 events per minute for the entire application, divide that number by 10 hosts. Then divide again by 4 processes per host.
...
<CollapserGroup>
  ...
  <Collapser
    id="package-capturing-enabled"
    title="package_capturing.enabled"
  >
    <table>
      <tbody>
        <tr>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_PACKAGE_CAPTURING_ENABLED`
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <Callout variant="caution">
        Note disabling this setting will disable the ability to detect vulnerabilities in outdated packages.
    </Callout>
  </Collapser>
</CollapserGroup>
