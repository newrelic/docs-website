...
<CollapserGroup>
  ...
  >
    ...
    If set, the agent will use this AWS `account_id` to link AWS entities when an account ID cannot be automatically determined by the instrumentation. See [AWS Manage Account Identifiers documentation](https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-identifiers.html#awsaccountid) for an example of a valid AWS account ID and how to find it.
  </Collapser>
