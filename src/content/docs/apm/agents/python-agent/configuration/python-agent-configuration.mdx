...
<CollapserGroup>
  ...
  >
    ...
    `default` is the default behavior and behaves as it did before this change (it passes the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled). 
    `always_on` means that the trace that has a remote parent sampled  will always be sampled.
    `always_off` means that the trace that has a remote parent sampled  will never be sampled.
  ...
  >
    ...
    `default` is the default behavior and behaves as it did before this change (it passes the sampling decision to the adaptive sampling algorithm to determine whether a trace will be sampled). 
    `always_on` means that a trace that has a remote parent not sampled will always be sampled.
    `always_off` means that a trace that has a remote parent not sampled will never be sampled.
