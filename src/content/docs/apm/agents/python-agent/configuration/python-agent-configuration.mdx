...
import diagramPythonConfigPrecedence0 from 'images/diagram-python-config-precedence_0.png'
...
<img
  ...
  src={diagramPythonConfigPrecedence0}
/>
...
<Collapser
  ...
  >
    ...
    Each log receives the same priority as its associated trace span. This increases the chances that traces in the product will have associated log data. Still, some logs may not be included because they are limited by `max_samples_stored`. For example, if logging `max_samples_stored` is set to 10,000 and trace 1 has 10,000 log entries, only log entries for trace 1 will be recorded. If trace 1 has less than 10,000 logs you receive all logs for trace 1. If there is still space, you receive all the logs for trace 2, and so on.

    If after all the logs for sampled traces are recorded and they haven't reached the limit in `max_samples_stored`, then log messages for traces that were not in our sampling are sent. If there are any left, log messages outside of transactions are recorded.
  </Collapser>
