...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="tip">
      Python agent versions 2.0.0 or earlier do not provide the `proxy_scheme` setting, and the protocol scheme defaults to `http` or `https` depending whether [`ssl`](#ssl) is disabled or enabled. If you are upgrading from an older agent version and your config file doesn't include `proxy_scheme`, ensure you add the setting and set it appropriately. If you do not, the agent will continue to base the protocol scheme on the `ssl` setting for backwards compatibility. As proxies are usually only configured to accept proxy requests via the `http` protocol scheme, not setting `proxy_scheme` may result in a failure.
    </Callout>
  </Collapser>
