...
<Callout variant="important">
  The only **required** setting is your license key — set via `license_key` in `newrelic.ini` (or the `NEW_RELIC_LICENSE_KEY` environment variable). Setting `app_name` to a meaningful name is strongly recommended so your app appears correctly in New Relic (it defaults to `Python Application` if not set). All other settings are optional.
</Callout>
