---
title: 'Distributed tracing for the Python agent'
tags:
...
Distributed tracing allows you to see the entire journey of your requests throughout a [distributed system](/docs/distributed-tracing/concepts/introduction-distributed-tracing). It is turned on by default in current New Relic Python agents, but you can also enable it manually in older agents.
...
Distributed tracing is enabled through configuration settings. Review the following Python-specific sections. For general help with agent configurations, see [Configure the agent](/docs/agents/manage-apm-agents/configuration/configure-agent).
