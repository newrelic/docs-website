...
The `app_name` setting in the Python agent configuration file sets the app name displayed in the New Relic One UI.
...
To ignore these errors, set this from the New Relic One **Application settings** UI or from the agent config file. UI changes override config file changes per the [configuration precedence rules](/docs/agents/python-agent/installation-configuration/python-agent-configuration#options).
