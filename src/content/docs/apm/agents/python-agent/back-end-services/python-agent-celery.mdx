...
To ignore these errors, set this from the New Relic <DNT>**Application settings**</DNT> UI or from the agent config file. UI changes override config file changes per the [configuration precedence rules](/docs/agents/python-agent/installation-configuration/python-agent-configuration#options).

To use ignore error settings from the UI:

1. From <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT>, select <DNT>**APM > (select an app) > Settings > Application**</DNT>.
2. Select <DNT>**Server-side agent configuration**</DNT>.
3. From <DNT>**Error collection**</DNT>, enter the errors you want to ignore, separated by commas.
