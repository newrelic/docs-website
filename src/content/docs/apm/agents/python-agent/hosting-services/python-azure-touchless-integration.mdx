...
2. [Microsoft Azure App Service](https://learn.microsoft.com/en-us/azure/app-service/overview), using code

Note: Support for New Relic integration for App Services using containerized images is not available.
...
<CollapserGroup>
    ...
    <Steps>
        <Step>
        ## Add [New Relic License Key](docs/apis/intro-apis/new-relic-api-keys/#license-key) to secrets [#add-license-key-cli]
        ...
        `az storage share-rm create --resource-group $RESOURCE_GROUP --storage-account $STORAGE_ACCOUNT_NAME --name $STORAGE_SHARE_NAME --quota 4096 --enabled-protocols SMB`
        ``STORAGE_ACCOUNT_KEY=`az storage account keys list -n $STORAGE_ACCOUNT_NAME --query "[0].value" -o tsv --resource-group $RESOURCE_GROUP` ``
...
curl -L https://raw.githubusercontent.com/newrelic/newrelic-agent-init-container/refs/heads/main/src/python/download_wheels.py > download_wheels.py
...
pip install ./workspace/newrelic.tar.gz --target=./workspace/newrelic && rm ./workspace/newrelic.tar.gz
