...
>
  To install the agent in a <DNT>Docker/DNT> container, follow [these self-guided steps](/docs/apm/agents/nodejs-agent/installation-configuration/install-nodejs-agent-docker) using the command line. 
  
  This process walks you through adding the agent to `package.json`, then requiring New Relic on the app-level. By the end, you'll add your license key and app name to `docker run` so the agent runs with your <DNT>Docker/DNT> app. 
</Collapser>
