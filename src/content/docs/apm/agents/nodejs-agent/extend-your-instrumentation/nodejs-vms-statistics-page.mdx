...
![APM - Node VMs page 1.png](src/images/vm-dashboard.png 'APM - Node VMs page 1.png')

<figcaption>
  **[one.newrelic.com](http://one.newrelic.com) > APM > (select an app) > Node
  VMs**: When multiple servers are reporting to the same application, the page
  breaks charts down by server. You can view data for an individual server and
  obtain additional details by selecting a server from the **Servers** filter at
  the top of the page.
</figcaption>
...
{' '}
<Collapser id="gc-pause-frequency" title="GC pause frequency">
  The number of times GC runs were executed per minute. For a single server, the
  chart is broken down by the type of GC. For multiple servers, the chart shows
  total number of GC calls (all types combined) grouped by server.
</Collapser>
