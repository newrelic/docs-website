...
<CollapserGroup>
  ...
  >
    This example instruments a `/websocket/ping` transaction and a `/websocket/new-message` transaction within <DNT>**socket.io**</DNT>. The `/ping` example is synchronous, while the `/new-message` example is asynchronous.
