...
<figcaption>
  **[one.newrelic.com](http://one.newrelic.com) > APM > Applications > (select
  an app) > Transactions > (select a transaction):** The transaction's
...
<figcaption>
  **[one.newrelic.com](http://one.newrelic.com) > APM > Applications > (select
  an app) > Summary > (select a transaction trace):** In this example, the
