...
To view curated charts with Node.js VM data, go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Node VMs**. For more information, see the [Node.js VMs statistics page](/docs/agents/nodejs-agent/supported-features/node-vms-statistics-page).
