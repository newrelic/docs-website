...
With the Node.js agent, you can add [browser instrumentation](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) to your web pages. To use <InlinePopover type="browser"/> with your Node.js agent, ensure you have the [latest release](/docs/agents/nodejs-agent/installation-configuration/installing-maintaining-nodejs#upgrading) of the Node.js agent.
...
1. At the beginning of your html page's `head` tag, insert the results of `newrelic.getBrowserTimingHeader()` after any `CHARSET` meta tags.
   ...
   Exception: For maximum <DNT>Internet Explorer</DNT> compatibility, insert the results of `newrelic.getBrowserTimingHeader()` after any `X-UA-COMPATIBLE HTTP-EQUIV` meta tags.
...
<CollapserGroup>
  ...
  >
    ...
    Using <DNT>**hapi**</DNT>, in your `app.js`:
