...
1. At the beginning of your html page's `head` tag, insert the results of `newrelic.getBrowserTimingHeader()` after any `CHARSET` meta tags.
   ...
   <DNT>**Exception:**</DNT> For maximum Internet Explorer compatibility, insert the results of `newrelic.getBrowserTimingHeader()` <DNT>**after**</DNT> any `X-UA-COMPATIBLE HTTP-EQUIV` meta tags.
