...
See [Google's documentation for building custom runtimes](https://cloud.google.com/appengine/docs/flexible/custom-runtimes/build). This example describes how to add New Relic to your GAE flex app by building a custom runtime for <DNT>Docker/DNT>. You can deploy the app without any special configuration.
...
<CollapserGroup>
  ...
  >
    The [Dockerfile](http://docs.docker.com/engine/reference/builder/) defines the <DNT>Docker/DNT> image to be built and is required for a GAE flexible environment app.

    1. To create the <DNT>Docker/DNT>file, build the container, and deploy your app, follow the [GAE procedures for Node.js](https://cloud.google.com/appengine/docs/flexible/nodejs/run-flex-app-on-gke).
