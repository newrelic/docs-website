...
<Callout variant="important">
  For a <DNT>Next.js<DNT> app use our standalone [`@newrelic/next` agent](https://github.com/newrelic/newrelic-node-nextjs) instead of the `newrelic` agent, for example:
  ```shell
