...
6. Add `-r newrelic` to your app's startup script. For example, if your application's entry point is `./dist/server.js` then you would use the require flag like so:
  ...
  <Callout variant="important">
    If you are unable to use the `-r` require flag you can also use `require('newrelic')` as the first line of your app's main module. <DoNotTranslate>**Note**</DoNotTranslate>   If you are using [Babel](https://babeljs.io/docs/en/index.html) or a similar transpiler `require('newrelic')` will cause instrumentation issues.
