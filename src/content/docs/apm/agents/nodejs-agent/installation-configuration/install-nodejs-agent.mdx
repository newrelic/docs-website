...
6.Add `-r newrelic` to your start script to load the agent with the [Node command line option '-r'](https://nodejs.org/api/cli.html#-r---require-module). 
For example, if your application's entry point is `./dist/server.js` then you would use the require flag like so:
...
<Callout variant="important">
 If you are unable to use `-r newrelic` then add `require('newrelic');` as the first line of your app's main module.
 If you are unable to the require flag as above (e.g. asynchronously loading api keys from a remote location during application bootstrapping), you may also add stock instrumentation to an already loaded [supported module](https://github.com/newrelic/node-newrelic/blob/0113eb5f0e707dc662a17d262a841503bab88841/lib/instrumentations.js#L6#L6) by using [`newrelic.instrumentLoadedModule`](/docs/apm/agents/nodejs-agent/api-guides/nodejs-agent-api/#instrumentLoadedModule):
