...
<Callout variant="important">
  If you're using the agent in a <DNT>Docker/DNT> container, [install the agent in each container](/docs/agents/nodejs-agent/installation-configuration/install-new-relic-nodejs-agent-docker).
</Callout>
...
3. Use the command `npm install newrelic` for each application you want to monitor. If your app is using [one of these Apollo Server modules](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/apollo-server-plugin-nodejs/) install our Apollo plugin with `npm install @newrelic/apollo-server-plugin`. [More details about using `@newrelic/apollo-server-plugin` can be found here](https://github.com/newrelic/newrelic-node-apollo-server-plugin/tree/main#readme).
  ...
  <Callout variant="important">
    If you're using <DNT>Next.js<DNT> use our standalone [`@newrelic/next` agent](https://github.com/newrelic/newrelic-node-nextjs) instead of the `newrelic` agent. [Here is an example <DNT>Next.js<DNT> app](https://github.com/newrelic/newrelic-node-nextjs#example-project)
  </Callout>
...
6. Add `-r newrelic` to your app's startup script. For example, if your application's entry point is `./dist/server.js` then you would use the require flag like so:
  ...
  ```
  An example <DNT>Docker/DNT> command:
  ```dockerfile
  CMD ["node", "-r", "newrelic", "server.js"]
  ```

  <Callout variant="important">
    For <DNT>Next.js<DNT> use `-r @newrelic/next` instead of `-r newrelic`.
