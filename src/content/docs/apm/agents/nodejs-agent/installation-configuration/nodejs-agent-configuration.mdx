...
<CollapserGroup>
  ...
  >
    The config file (`newrelic.js`) contains every Node.js agent setting. When you [install the Node.js agent](/docs/apm/agents/nodejs-agent/installation-configuration/install-nodejs-agent/#installing), you must copy `newrelic.js` into your app's root directory. Most settings are empty by default; they inherit their values from `config/default.js`.
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="tip">
      For [Azure users](/docs/agents/nodejs-agent/hosting-services/nodejs-agent-microsoft-azure), the Node.js agent will use `APP_POOL_ID` if it's set, so you can use the name you chose for your Azure Web Server without setting it twice.
    </Callout>
  </Collapser>
  ...
  >
    ...
    Set to `false` to stop the agent from starting up. This is useful when debugging your code requires temporarily disabling the agent. It prevents the agent from bootstrapping its instrumentation or setting up all its pieces, which prevents the agent from starting up and connecting to the servers of New Relic.
  </Collapser>
...
<Collapser
  ...
  >
    ...
    When set to `true`, enables [high security v2](/docs/apm/agents/manage-apm-agents/configuration/high-security-mode/#version2description). You must also enable the [`ssl`](#ssl) setting and [enable high security in the UI](/docs/apm/agents/manage-apm-agents/configuration/high-security-mode/#version2enabled).
  </Collapser>
  ...
  >
    ...
    <Callout variant="important">
      Don't edit this value unless New Relic Support asks you to change it.
    </Callout>

    Hostname for the [New Relic collector](/docs/new-relic-solutions/get-started/glossary/#collector) to connect to the Internet; for example, `host: 'collector.newrelic.com'`.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Don't use `debug` or `trace` logging unless New Relic Support asks you to use them. These levels of logging can generate excessive overhead. For most situations, use `info`.
    </Callout>
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    This option enables [`newrelic.noticeError`](/docs/apm/agents/nodejs-agent/api-guides/nodejs-agent-api/#noticeError).
  </Collapser>
</CollapserGroup>
...
<Callout variant="important">
  Don't use brackets `[suffix]` at the end of your transaction name. New Relic automatically strips brackets from the name. Instead, use parentheses `(suffix)` or other symbols if needed.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Don't change this setting unless you understand [metric grouping issues](/docs/new-relic-solutions/solve-common-issues/troubleshooting/metric-grouping-issues/).
    </Callout>
...
<CollapserGroup>
  ...
  >
    ...
    Generate JavaScript headers for browser instrumentation. Although this defaults to `true`, the agent doesn't inject the browser JS code unless you've [enabled browser monitoring](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser). Even if you've enabled it and [added the browser timing header](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent/#procedures), you can [disable browser monitoring for your app](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent/#disabling) by setting this to `false`.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    When enabled, the agent will send all error gRPC status codes to New Relic, that is, nonzero status codes. If disabled, the server instrumentation won't send any nonzero status codes to New Relic.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    All attribute keys found in this list won't be sent with spans. For more information, see the [agent attribute rules](/docs/apm/other-features/attributes/agent-attributes).
  </Collapser>
</CollapserGroup>
...
To turn on Infinite Tracing, add the additional settings below. Infinite Tracing configuration is set in the `infinite_tracing` stanza.
...
Setting `NEW_RELIC_METADATA_SERVICE_VERSION` creates a tag, `tag.service.version` on event data. This tag is the version of your deployed code, in many cases a semantic version such as 1.2.3, but not always. Sending this information allows you to facet your telemetry by the version of the software deployed, so you can quickly identify which versions of your software are producing the errors. 

An upcoming release of errors inbox will automatically track which versions of your software are producing errors. 


<CollapserGroup>
  <Collapser
    id="version-tracking-collapser"
...
For a configuration example, see the [node agent url obfuscation](/docs/url-obfuscation/configure-url-obfuscation-nodejs) page.
...
<CollapserGroup>
  ...
  >
    Path to the directory containing `newrelic.js`. This is available only as an environment variable. You cann't set it in your config file.
  ...
  >
    ...
    This is available only as an environment variable. You cann't set it in your config file.
