...
<CollapserGroup>
  ...
  >
    ...
    Route parameters (aka URL parameters) are a common feature of various web frameworks, where you can create a placeholder as part of an API route definition. Route (URL) and query parameters are not collected by default, but are available as attributes prefixed with `request.parameters.route.` on `Transaction` events, `Span` events, and `TransactionTrace` root segments. 
    ...
    A config setting like this would capture all route/query parameters except for `password`:
  ...
  >
    ...
    A config setting like this would capture all route/query parameters:
