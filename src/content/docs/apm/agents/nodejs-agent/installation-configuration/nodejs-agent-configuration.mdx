...
Additionally, Distributed Tracing must be [enabled](/docs/understand-dependencies/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing#node-config). Note that Distributed Tracing is enabled by default for Node.js APM agent versions 8.3.0 and greater.
...
For some tips on configuring logs for the Node.js agent, see [Configure Node.js logs in context](/docs/logs/logs-context/configure-logs-context-nodejs). 
