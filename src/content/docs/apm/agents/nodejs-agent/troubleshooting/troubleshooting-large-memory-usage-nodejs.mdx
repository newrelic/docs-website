...
<CollapserGroup>
  ...
  >
    Many database drivers use an abstraction called a `cursor`. Cursors provide the ability to iterate through the results of queries. For example, the <DNT>**mongodb**</DNT> driver provides cursors when executing `find` queries.
