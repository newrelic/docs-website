...
<CollapserGroup>
  ...
  >
    Many database drivers use an abstraction called a `cursor`. Cursors provide the ability to iterate through the results of queries. For example, the <DoNotTranslate>**mongodb**</DoNotTranslate> driver provides cursors when executing `find` queries.
