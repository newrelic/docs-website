...
<CollapserGroup>
  ...
  >
    Route parameters (aka URL parameters) are a common feature of various web frameworks, where you can create a placeholder as part of an API route definition. Route (URL) parameters are available as attributes prefixed with `request.parameters.route.` on `Transaction` events, `Span` events, and `TransactionTrace` root segments. 
    ...
    Request parameters from the transaction are not collected by the Node agent by default. Any request parameters, such as route (`/user/:userId`) or query (`/user?userId=123`) parameters will now be prefixed with `request.parameters.` when collected. For example, the route parameter from `/user/:userId` would be collected as `request.parameters.userId`.

    All GET parameters can be captured if the `request.parameters.*` entry is added to [`attributes.include`](/docs/apm/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration/#tx-attributes-include), or specific request parameters can be added to the list, for example, `request.parameters.foo` or `request.parameters.bar`. In order to capture POST parameters, use the [`addCustomAttribute()`](https://newrelic.github.io/node-newrelic/API.html#addCustomAttribute) Node.js agent API call.
