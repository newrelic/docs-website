...
Some of these components are easier to control than others. For example, to capture timings for the <DNT>**Receiving host**</DNT> items above, make sure you have [request queue monitoring](/docs/features/configuring-request-queue-reporting) set up on the receiving application.
