...
The Typhoeus gem is maintained by the same team that maintains Ethon and depends on Ethon for its functionality. To prevent duplicate reporting for each HTTP request, the Ethon instrumentation will be disabled when Typhoeus is detected.
...
Testing has shown significant issues with segfaults when running Typhoeus on CRuby 1.8.7.
**Recommendation:** Use Ruby Enterprise Edition or CRuby 1.9.3 or higher to avoid these problems.
