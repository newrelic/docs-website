...
The APM UI shows transactions initiated via RabbitMQ message receipt (subscribe/consume messages) as <DNT>**Message**</DNT> background tasks. Message creation via RabbitMQ also appears in transaction traces.
...
<CollapserGroup>
  ...
  >
    Queue operations appear on APM's [<DNT>**Transactions**</DNT> page](/docs/apm/applications-menu/monitoring/transactions-dashboard) for the selected app. The `Put` and `Take` metrics appear in the <DNT>**Breakdown table**</DNT> and are categorized as `MessageBroker` metrics. Here is an example:
  ...
  >
    The <DNT>**Transaction trace**</DNT> page includes a dedicated <DNT>**Details**</DNT> tab that shows more detailed information about the transaction. Here is an example:
