...
The APM UI shows transactions initiated via RabbitMQ message receipt (subscribe/consume messages) as <DoNotTranslate>**Message**</DoNotTranslate> background tasks. Message creation via RabbitMQ also appears in transaction traces.
...
<CollapserGroup>
  ...
  >
    Queue operations appear on APM's [<DoNotTranslate>**Transactions**</DoNotTranslate> page](/docs/apm/applications-menu/monitoring/transactions-dashboard) for the selected app. The `Put` and `Take` metrics appear in the <DoNotTranslate>**Breakdown table**</DoNotTranslate> and are categorized as `MessageBroker` metrics. Here is an example:
  ...
  >
    The [**Transaction trace summary** page](/docs/apm/transactions/transaction-traces/transaction-trace-details-page) may show `Put` and `Take` operations in the **Slowest components** section. For example:
  ...
  >
    The <DoNotTranslate>**Transaction trace**</DoNotTranslate> page includes a dedicated <DoNotTranslate>**Details**</DoNotTranslate> tab that shows more detailed information about the transaction. Here is an example:
