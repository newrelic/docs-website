...
## Enabled Thread Tracing by Default [#thread_tracing]
  ...
  The configuration option `instrumentation.thread.tracing` is now enabled by default. This configuration allows the agent to properly monitor code that occurs inside of threads. In Ruby Agent 9.0, work being done inside of threads will be recorded and associated with the transaction that the thread was created.
...
## Removed Deprecated Instrumentation [#removed_gem_instrumentation]
  ...
  The following gems had been previously deprecated and are now removed.
...
## Removed API method [#removed_api_methods]
  ...
  The following deprecated API method has been removed.
