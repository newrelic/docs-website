...
## Enabled thread tracing by default [#thread_tracing]
  ...
  This may be a breaking change if you're currently using custom thread instrumentation. New transactions inside of threads will no longer start if one already exists.
