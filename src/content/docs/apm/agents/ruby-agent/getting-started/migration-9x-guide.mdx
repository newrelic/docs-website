...
This may be a breaking change if you're currently using custom thread instrumentation. New transactions inside of threads will no longer be started if one already exists.
  ...
  Asynchronous segments may not be finished before the current transaction finishes. This can cause segments to have inaccurate timing information. In version 9.0.0 of the agent, a `NoMethodError` may be raised in these scenarios while a transaction finishes: 
  
  ```
  [2023-03-06 15:13:18 +0100 web-01 (8675309)] ERROR : Exception during Transaction#finish
  [2023-03-06 15:13:18 +0100 web-01 (8675309)] ERROR : NoMethodError: undefined method `name' for nil:NilClass
  ```

  This exception is thrown while issuing a `warn`-level log stating a segment without a parent was unfinished when the transaction finished. This exception should not stop transactions from being harvested.
  
  We've issued [a fix](https://github.com/newrelic/newrelic-ruby-agent/pull/1876/) for the error to do two things: 
  1. Make sure the `NoMethodError` is not raised (after 9.0.0)
  2. Decrease the severity of the logged messages to `debug` level instead of `warn` level when a transaction contains a Thread, Fiber, or concurrent-ruby segment

  For more details, see [Issue \#1859](https://github.com/newrelic/newrelic-ruby-agent/issues/1859).
