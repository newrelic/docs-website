...
The `extends_newrelic_rpm` and `newrelic_mongo` gems both provided basic instrumentation for `mongo`. New Relic does not recommend running Mongo instrumentation via these gems alongside the Ruby agent's built-in Mongo support. This might double-count metrics such as overall database time.
