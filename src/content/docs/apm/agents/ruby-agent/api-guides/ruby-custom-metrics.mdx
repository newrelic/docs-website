...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Metrics and stats in the <DoNotTranslate>Ruby</DoNotTranslate> agent"
  >
    There are two basic data structures used to collect metric data: `MetricSpec` and `Stats`.

    A `MetricSpec` is an identifier for which we have data. The following pseudo <DoNotTranslate>Ruby</DoNotTranslate>> defines a metric:
