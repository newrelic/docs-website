...
`add_method_tracer` takes an optional metric name and a hash of options. For more information, see [add_method_tracer in the New Relic RubyDoc](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent/MethodTracer/ClassMethods#add_method_tracer-instance_method) .
...
To support applications that want to control which threads are instrumented by New Relic, a class is available in 8.7.0 and above, `NewRelic::TracedThread`, that will create a thread that includes New Relic instrumentation. 
Use this class as a replacement for the native Thread class. Please see our [API documentation](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/TracedThread) for more details. 
```
  NewRelic::TracedThread.new do 
    # custom code to run inside of the thread
  end

```
To have the agent automatically instrument all threads, please see the configuration `instrumentation.thread.tracing` [documentation](https://docs.newrelic.com/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#instrumentation).
