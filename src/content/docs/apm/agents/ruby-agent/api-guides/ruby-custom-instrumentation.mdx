...
To support applications that want some threads instrumented by New Relic, a class is available in 8.7.0 and above, `NewRelic::TracedThread`, that will create a thread that includes New Relic instrumentation. 
...
```
  NewRelic::TracedThread.new do 
    # custome code to run inside of the thread
  end
