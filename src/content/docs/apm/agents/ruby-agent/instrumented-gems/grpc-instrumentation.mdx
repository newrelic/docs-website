...
The [New Relic Ruby agent](/docs/agents/ruby-agent/getting-started/new-relic-ruby) automatically instruments [gRPC](https://grpc.io/) clients and servers that use the [grpc](https://rubygems.org/gems/grpc) gem (gem version 1.0.0 or higher). All communication types - unary, bidirectional, client-streaming, and server-streaming - are supported.
...
The instrumentation of gRPC clients and servers can be configured separately. By default, both instrumentations are set to `auto`, which is short for "automatic". The agent's automatic behavior for each is to enable the instrumentation by attempting to leverage Ruby module prepend behavior at first and falling back on Ruby method chaining behavior if known compatibility issues are encountered. Optionally, the instrumentation setting can be set to a value of `disabled` to disable the instrumentation, `prepend` for forced module prepend behavior, or `chain` for forced method chaining behavior.

<Callout variant="tip">
  By default, the New Relic Ruby agent has instrumentation for both gRPC clients and servers set to `auto`.

  By modifying the New Relic config file or by setting environment variables, the agent's gRPC instrumentation behavior can be altered with one of the following values:

  - **auto:** the default, enabled via automatic Ruby method behavior selection
...
As long as the instrumentation is working properly (see "Verification" above to confirm this), all instrumentation performed by the New Relic Ruby agent should be fully automatic and not require any modifications whatsoever to any of your existing gRPC client and/or server code.

The New Relic Ruby agent's gRPC instrumentation can be used for New Relic Distributed Tracing in conjunction with any other New Relic agent that supports gRPC. gRPC requests can be sent from a Ruby-based client to a server based on another language, and a Ruby-based server can field requests coming from a client based on another language.
...
<Callout variant="important">
  ...
  gRPC server request handlings will always generate a new New Relic transaction and will not require nor roll-up to any pre-existing New Relic transaction.
</Callout>


## Viewing Data

To view the collected gRPC data in the New Relic web UI...
...
On the [Distributed tracing page](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent/), select (click on) a client trace group, and then click on the trace group name again. Then from the resulting page's list, click on an individual gRPC server trace and select (click on) the Attributes tab on the right. The `request.grpc_type`, `request.method`, and `request.uri` attributes should all appear.
...
When contacting New Relic for support and/or feedback related to the New Relic Ruby agent's gRPC instrumentation, answers the following questions would be greatly appreciated.
