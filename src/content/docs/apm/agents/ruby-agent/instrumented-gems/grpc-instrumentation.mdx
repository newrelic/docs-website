...
5. Click on a trace group.
6. Choose a trace from the list, and click on it.
7. Click **Expand all**.
8. From the resulting list of trace spans, click on an individual trace span.
9. Click the **Attributes** tab in the right pane.
...
Whether the gRPC client requests are being sent to a gRPC server or the gRPC server-handled requests originated from a gRPC client with New Relic gRPC instrumentation (based on Ruby or another language), the distributed tracing trace map should do the following:
