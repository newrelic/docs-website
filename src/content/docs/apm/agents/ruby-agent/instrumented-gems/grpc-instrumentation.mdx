...
On the [Distributed tracing page](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent/), do the following:

1. Click on a trace group.
2. Choose a trace from the list, and click on it.
3. Click **Expand all**.
4. From the resulting list of trace spans, click on an individual gRPC client trace span.
5. Click the **Attributes** tab in the right pane.

The `grpc_status`, `grpc_type`, `http.method`, and `http.url` attributes should all appear. In the event of a failed client request (which will have a `grpc_status` value other than `0`), an additional `grpc_message` attribute will appear with the gRPC status message.
...
1. Click on a client trace group.
2. Choose a trace from the list, and click on it.
3. Click **Expand all**.
3. From the resulting list of trace spans, click on an individual gRPC server span.
4. Click the **Attributes** tab in the right pane.

The `request.grpc_type`, `request.method`, and `request.uri` attributes should all appear.
