...
1. Add the `newrelic_rpm` gem to Gemfile.
   ```
   gem 'newrelic_rpm'
   ```
2. You must manually call `require 'newrelic_rpm'`. In your Roda app, below the Roda **require** directive, add `require 'newrelic_rpm'`. For example:
