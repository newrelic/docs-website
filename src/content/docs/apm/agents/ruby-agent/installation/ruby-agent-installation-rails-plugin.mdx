...
<Callout variant="important">
  We strongly recommend [installing the Ruby agent as a gem](/docs/agents/ruby-agent/installation-configuration/ruby-agent-installation#Installing_the_Gem) in order to have better control over versions and dependencies.
</Callout>
...
After installing the agent, copy the `newrelic.yml` file into the `config` subdirectory of your application. You can download a fresh `newrelic.yml` that includes your [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) from the **Account settings** when logged in to New Relic.
