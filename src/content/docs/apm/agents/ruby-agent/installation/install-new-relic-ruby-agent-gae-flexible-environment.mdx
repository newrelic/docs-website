...
* <DNT>Docker/DNT> installation using a [custom runtime](https://cloud.google.com/appengine/docs/flexible/custom-runtimes/about-custom-runtimes)
...
When using Google App Engine ["native mode"](/docs/accounts-partnerships/partnerships/google-cloud-platform-gcp/google-app-engine-environment#native-mode) installation, you provide your app code and an `app.yaml` file. Google App Engine then deploys to a standard prebuilt <DNT>Docker/DNT> image.
...
See [Google's documentation for building custom runtimes](https://cloud.google.com/appengine/docs/flexible/custom-runtimes/build). This example describes how to add New Relic to your GAE flex app by building a custom runtime for <DNT>Docker/DNT>. The example uses a Sinatra app for Ruby.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="3. Configure a <DNT>Docker/DNT>file"
  >
    The [Dockerfile](http://docs.docker.com/engine/reference/builder/) defines the <DNT>Docker/DNT> image to be built and is required for a GAE flexible environment app. To create the recommended base image for apps monitored by the New Relic Ruby agent:
  ...
  <Collapser
    ...
    title="4. Build a <DNT>Docker/DNT> image"
  >
    Be sure to include the period at the end of the code, to indicate the current directory contains the build files.

    ```
    docker build -f <DNT>Docker/DNT>file -t custom_ruby_app_container:latest .
    ```

    After running this command, verify that you have a <DNT>Docker/DNT> image named `custom_ruby_app_container` and tagged `latest`.
  </Collapser>

  <Collapser
    id="deploy-docker-image-to-gae"
    title="5. Deploy <DNT>Docker/DNT> image to initialized GAE flexible environment"
  >
...
* To connect to the GAE instance and start a shell in the <DNT>Docker/DNT> container running your code, see [Debugging an instance](https://cloud.google.com/appengine/docs/flexible/go/debugging-an-instance).
