...
* To <DoNotTranslate>**handle**</DoNotTranslate> health checks, add a route for `_ah/health` in your app.
* To <DoNotTranslate>**ignore**</DoNotTranslate> health check requests, set the `rules.ignore_url_regexes` config setting in the application’s Ruby agent config to include `'_ah/health'`.
