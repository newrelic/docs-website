...
<Callout variant="important">
  If you are using the agent in a <DNT>Docker/DNT> container, install the agent <DNT>**within each**</DNT> container.
</Callout>
...
2. To use Infinite Tracing, the Infinite Tracing gem is also available from rubygems.org as `newrelic-infinite_tracing`. For applications using Bundler, additionally include the Infinite Tracing gem in the Gemfile:
   ...
   ```
3. The next step varies depending on if you are using Rails, <DNT>Roda</DNT>, or Sinatra:

   <table>
     <thead>
       <tr>
         <th style={{ width: "200px" }}>
           <DNT>**Ruby installation**</DNT>
         </th>
     ...
     <tbody>
       <tr>
         <td>
           If using Rails, <DNT>Roda</DNT>, or Sinatra
         </td>
         ...
         <td>
           ...
           * <DNT>**Roda, Sinatra**</DNT>: If you're using <DNT>Roda</DNT>, Sinatra or another framework, you must manually call `require 'newrelic_rpm'`. Additionally, if you're using Infinite Tracing, manually call `require 'newrelic/infinite_tracing'`.
