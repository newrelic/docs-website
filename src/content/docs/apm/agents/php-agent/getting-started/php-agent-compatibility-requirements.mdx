...
<img
  title="Verify installed PHP version"
  alt="The New Relic guided install showing the php --version command with expected output, including a note about ZTS builds being unsupported."
  src="/images/apm_screenshot-full_php-guided-install-verify-php-version.png"
/>

<figcaption>
  Expected output from `php --version`. If you see a ZTS build (Zend Thread Safety), the PHP agent does not support it — see [ZTS](#zts) below.
</figcaption>
...
<ButtonLink
  role="button"
  to="/docs/apm/agents/php-agent/installation/php-agent-installation-overview"
  variant="primary"
>
