...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>
                7.0 and 7.1 are for EOL in December 2023.
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                When PHP 8.0, 8.1, or 8.2 detects an observability extension (the PHP agent),  [Just-In-Time compilation](https://php.watch/articles/jit-in-depth) is disabled.
...
See [PHP agent release notes](/docs/release-notes/agent-release-notes/php-release-notes/) for the latest PHP agent release.
  ...
  * For PHP builds that are compiled with Zend Thread Safety (ZTS), only [PHP agent 9.19](/docs/release-notes/agent-release-notes/php-release-notes/php-agent-9190309) and later supports these builds.
  * If you're using an unsupported PHP version or platform (such as a 32 bit OS, FreeBSD, or ZTS builds), we recommend disabling the auto-update mechanisms for the PHP agent packages. Failure to prevent automatic upgrades could lead to the installation of a newer agent version, which may no longer support critical features essential for APM data collection, resulting in disruptions to monitoring capabilities. 
    ...
    To disable auto-update, you have two options: 
      * Add the PHP agent packages to an exclusion list for package upgrades
      OR
...
* [A file storing the agent log files](/docs/php/php-agent-phpini-settings#inivar-daemon-logfile)
* [A file storing the agent socket endpoint](/docs/php/php-agent-phpini-settings#inivar-daemon-port)
* [A file storing the agent process ID](/docs/php/php-agent-phpini-settings#inivar-daemon-pidfile)
...
When vendors announce end of life (such as on [Ubunto's End of Standard Support page](https://wiki.ubuntu.com/Releases)), we will continue to support those latest versions for one year; howevever, if the PHP version you're using is no longer officially supported, then support could end sooner than one year. This is why we recommend always using the latest version of the OS that is officially supported by the vendor.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * ARM64 is only supported with PHP versions 8.0, 8.1, and 8.2.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        The PHP agent versions 7.0 - 7.4 and 8.0 - 8.2 report code-level metrics (starting with release 10.4.0).
      </td>
    </tr>
    ...
    The PHP agent supports our logs in context feature for the following libraries:
