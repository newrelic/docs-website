---
title: Start the PHP daemon manually
tags:
  - Agents
  - PHP agent
...
<Callout variant="tip">
  **Most users do not need this page.** If you installed the PHP agent via a package manager (yum, apt) or tar archive on a standard Linux server, the daemon starts automatically — no extra configuration needed. Come here only if you have a specific reason to run the daemon manually: Docker with separate containers, fleet deployments that need controlled startup order, multi-tenant hosts, or init-script based service management. If you're unsure, use [agent mode](#daemon-autostart).
</Callout>
...
* [PHP agent configuration](/docs/apm/agents/php-agent/configuration/php-agent-configuration) — Configure the agent in `newrelic.ini`: license key, app name, logging, and `newrelic.daemon.*` keys for agent mode.
* [Daemon settings reference](/docs/apm/agents/php-agent/configuration/proxy-daemon-newreliccfg-settings) — Full reference for `newrelic.cfg` settings used in external mode, including socket address, proxy, log level, and audit log.
* [View your data in APM](/docs/apm/applications-menu/monitoring/apm-overview-page) — After the daemon is running and your web server is restarted, confirm data is flowing here.
* [Troubleshoot the daemon](/docs/agents/php-agent/troubleshooting/verifying-php-daemon) — How to confirm the daemon is running, inspect its log, and diagnose connection issues.
