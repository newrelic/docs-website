...
<DocTiles>
  ...
  >
    Configure the agent in `newrelic.ini`: license key, app name, logging, and `newrelic.daemon.*` keys for agent mode.
  </DocTile>

  <DocTile
    title="Daemon settings reference"
    path="/docs/apm/agents/php-agent/configuration/proxy-daemon-newreliccfg-settings"
  >
    Full reference for `newrelic.cfg` settings used in external mode, including socket address, proxy, log level, and audit log.
  </DocTile>
  ...
  >
    After the daemon is running and your web server is restarted, confirm data is flowing here.
  </DocTile>
  ...
  >
    How to confirm the daemon is running, inspect its log, and diagnose connection issues.
  </DocTile>
</DocTiles>
