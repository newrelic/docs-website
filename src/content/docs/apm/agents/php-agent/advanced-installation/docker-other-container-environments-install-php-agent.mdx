...
metaDescription: How to install New Relic's PHP agent inside a <DNT>Docker/DNT> container or other container to monitor your PHP apps.
redirects:
...
You can install the PHP agent on a <DNT>Docker/DNT> container or other container to monitor one or more of your PHP applications. This is supported for containers that meet the standard [PHP agent compatibility and requirements](/docs/agents/php-agent/getting-started/php-agent-compatibility-requirements).
...
If you use <DNT>Docker/DNT>, you can pull our daemon image from [Docker Hub](https://hub.docker.com/r/newrelic/php-daemon):
...
To set up the PHP agent container for <DNT>Docker/DNT>:

1. Make sure a PHP installation is available in the container. For example, use a published <DNT>Docker/DNT> image like `php:7.1`.
...
To set up the PHP agent and daemon in the same <DNT>Docker/DNT> container:

1. Make sure a PHP installation is available in the container. For example: you might use a published <DNT>Docker/DNT> image like `php:7.1`.
2. To install the agent, download the PHP agent package from New Relic's [tar file download site](/docs/agents/php-agent/installation/php-agent-installation-tar-file) and run the `newrelic-install` script with the `install` argument.
...
<CollapserGroup>
  ...
  >
    ...
    You must edit three parts of this example <DNT>Docker/DNT>file:
...
<Callout variant="tip">
  Here are some <DNT>Docker/DNT> troubleshooting resources:
