...
Even though New Relic PHP Agent does not support PHPUnit version 11+, there's a known incompatibility between PHPUnit version 11+ and New Relic PHP Agent.
Therefore we recommend disabling the agent while running unit tests by setting `newrelic.enabled` to `false`. This setting can be passed to `phpunit` like this:
```
phpunit -d newrelic.enabled=false ...remaining arguments...
```
