...
If you use [PHPUnit](https://phpunit.de/) to manage and run your unit tests, the New Relic PHP agent can automatically capture the test summary results and send them to [New Relic](/docs/insights/new-relic-insights/understanding-insights/new-relic-insights) as an [event](/docs/insights/new-relic-insights/understanding-insights/new-relic-insights#database) where you can query and visualize test data at a glance. This feature supports PHPUnit versions <DNT>**8.x**</DNT> only.

<Callout>
Even though New Relic PHP Agent does not support PHPUnit version 11+, there's a known incompatibility between PHPUnit version 11+ on PHPs 8.3+ and New Relic PHP Agent. These are the only possible workarounds:
* disable New Relic PHP agent when running unit tests which use PHPUnit version 11 on PHPs 8.3+. This workaround disables all instrumentation of unit tests.
* disable New Relic PHP agent’s exception handler by adding `newrelic.special=no_exception_handler` to INI. This will prevent the agent from reporting any exceptions which occur during a test run, but perhaps will be a suitable workaround.
</Callout>
