...
While we don’t support PHPUnit version 11+, the agent needs to be explicitly disabled to avoid memory issues while running unit tests for PHPUnit 11+ by setting `newrelic.enabled` to `false`. This setting can be passed to `phpunit` like this:
```
