...
While we don’t support PHPUnit version 11+, the agent needs to be <DNT>**explicitly disabled**</DNT> by setting `newrelic.enabled` to `false` to avoid out-of-memory issues while running unit tests with PHPUnit 11+. The agent can be disabled while using `phpunit` like this:
```
