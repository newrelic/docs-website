...
If you [install New Relic for WordPress websites](/docs/agents/php-agent/installation/install-php-agent-shared-hosting-service), the PHP agent receives additional metrics. A **WordPress** page appears in the New Relic user interface: Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select a WordPress app)**.
