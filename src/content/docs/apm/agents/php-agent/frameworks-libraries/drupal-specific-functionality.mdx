...
The PHP agent captures special metrics for Drupal websites. When these metrics are collected, a **Drupal** tab appears in the New Relic user interface: Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (select a Drupal app)**.
