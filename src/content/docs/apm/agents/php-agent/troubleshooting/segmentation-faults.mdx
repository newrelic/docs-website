...
There is a known incompatibility of ionCube with OAPI. Using ionCube encoded files with any extension that utilizes PHP's language feature Observer API (such as the New Relic PHP Agent) can result in seg faults. Due to this issue, New Relic cannot offer support while ionCube is installed.
