...
Your site generates a segmentation faults when the New Relic PHP Agent (version >= 10.18.0.8) is enabled on PHP 8.0+.
...
Luckily, we have an easy way to test if the issue is PHP and or the agent. PHP has a built-in extension called zend_test which has a test observer. Enabling this (while disabling the agent), allows the OAPI codepaths to be executed by something other than our agent. Here are the steps to test this:
...
Here is a link to issues on php-src that are related to OAPI and segmentation faults: https://github.com/php/php-src/issues?q=is%3Aissue+segfault+observer
