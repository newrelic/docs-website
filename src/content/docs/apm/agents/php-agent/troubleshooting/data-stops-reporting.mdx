...
* The option to delete the application appears in the application's <DoNotTranslate>**Settings**</DoNotTranslate> menu when you select the <Icon name="fe-settings"/>
  icon.
...
* <DoNotTranslate>**Recommendation:**</DoNotTranslate> If you have long or infrequent processes, run a cron job every five minutes with a simple transaction, such as `phpinfo()`. This will help ensure the application does not enter an idle state.
...
When the PHP agent is in an idle state, <DoNotTranslate>**two**</DoNotTranslate> transactions must occur before it will start reporting again:
