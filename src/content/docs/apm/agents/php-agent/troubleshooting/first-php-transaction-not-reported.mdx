---
title: First PHP transaction is not reported
type: troubleshooting
tags:
  - Agents
  - PHP agent
  - Troubleshooting
metaDescription: Why you need to run two processes for New Relic's PHP agent to start reporting after initial installation.
redirects:
  - /docs/agents/php-agent/troubleshooting/first-php-transaction-not-reported
  - /docs/agents/php-agent/troubleshooting/first-transaction-getting-missed
  - /docs/agents/php-agent/troubleshooting/first-transaction-not-reported
freshnessValidatedDate: never
---

## Problem

You installed the New Relic PHP agent and generated some traffic, but the agent does not send the first transaction.

## Solution

This is expected behavior. When the [PHP daemon](/docs/agents/php-agent/getting-started/new-relic-daemon-processes) is in its initial state, <DoNotTranslate>**two**</DoNotTranslate> transactions must occur before it will start reporting:

* With the first transaction, the PHP daemon adds your newly-reporting application to its own application information table.
* Once the application is added, the PHP daemon reports the second transaction to New Relic.

<DoNotTranslate>**Recommendation:**</DoNotTranslate> Run a trivial php process, such as `phpinfo()`, before running the processes you want reported.

## Cause

This is expected behavior by the PHP daemon, which initially requires two transactions to be reported.
