...
tags:
  ...
  - PHP agent
  - Troubleshooting
metaDescription: Using the PHP Agent with an application with long running tasks can cause high memory usage.
---

## Problem

Using the PHP Agent with an application with long running tasks causes high memory usage.
...
To implement this, first ignore the initial automatic transaction with newrelic_end_transaction(true), then use newrelic_start_transaction() and newrelic_end_transaction() to instrument each transaction in turn.  The following code sample shows how to do this.  In actual code one would replace the `while()` structure with a proper PHP looping statement and insert the function logic in the place of the ellipses. 
