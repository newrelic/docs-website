...
### Add custom attributes such as nonce [#nonce-attribute]

If your site uses a [Content Security Policy (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) that requires a `nonce` attribute, you can pass `false` to both API functions so they return the raw JavaScript string without `<script>` tags. You can then build the tags yourself with the required attributes:
...
Passing `false` as the argument suppresses the script tags that the functions normally output, so you can wrap the raw JavaScript string with your own `<script>` tags with any attributes you need.
