...
<Callout variant="important">
  As of PHP Agent version 9.21.0[put full version number when known], distributed tracing is enabled by default and the number of spans is now configurable with a default of 2000 and maximum of 10000. This replaces cross-application tracing which will be deprecated. Please ensure the distributed tracing settings correspond with your ingest needs.
</Callout>
...
As [spans are sampled](/docs/apm/distributed-tracing/getting-started/how-new-relic-distributed-tracing-works), the PHP agent will prioritize spans related to external calls above other spans, which are then recorded in descending order of their duration. There is [an ini setting](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-span-events-max-samples-stored) to determine the maximum number of spans sampled per transaction, potentially impacting performance.
