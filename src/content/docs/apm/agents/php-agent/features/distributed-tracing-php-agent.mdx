...
<Callout variant="important">
  When distributed tracing is enabled, it implicitly disables cross application
  tracing. The PHP agent will ignore the [cross application tracing
  configuration
  value](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-cross-app-tracer-enabled),
  and act as though this value is set to `false`.
</Callout>
...
<Callout variant="important">
  For PHP agent versions 8.4 to 8.7: When distributed tracing is enabled, these
  versions behave as if
  [`newrelic.transaction_tracer.detail`](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-tt-detail)
  is set to `0` (as described above), which results in PHP function calls not
  generating spans. To get spans related to PHP function calls, update to
...
![Logs in context of APM trace details](src/images/logs-in-context-traces.png 'Logs in context of APM trace details')

<figcaption>
  With logs in context, you can examine log data directly within your trace
  details.
</figcaption>
...
<Callout variant="tip">
  When you **create a payload** or **insert headers**, you're telling New Relic
  you want this request or transaction or request to participate in a
  distributed trace. When you **accept** them, you're linking the current
  request or transaction with its parent request or transaction.
</Callout>
