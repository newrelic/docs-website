...
<Steps>
  <Step>
    ...
    #### 2. Trust the New Relic GPG key [#trust-gpg-key]

       ```bash
       wget -O- https://download.newrelic.com/<NEW_KEY_ID>.gpg | sudo gpg --import --batch --no-default-keyring --keyring /usr/share/keyrings/download.newrelic.com-newrelic.gpg
       ```
  ...
  <Step>
    ...
    <Callout variant="important">
      ...
      ```
      GPG error: https://apt.newrelic.com/debian newrelic Release: The following signatures were invalid: B60A3EC9BC013B9C23790EC8B31B29E5548C16BF (untrusted public key algorithm: dsa1024)
      The repository 'http://apt.newrelic.com/debian newrelic Release' is not signed.
      ```
      please make sure you have the correct New Relic GPG key installed. See the [Trust the New Relic GPG key](#trust-gpg-key) section for instructions how to install the New Relic GPG key.
    </Callout>
  </Step>
