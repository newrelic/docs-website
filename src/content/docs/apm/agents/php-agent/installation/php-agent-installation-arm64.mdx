...
2. Once you've created `newrelic.ini` and put it in the correct location, edit the file to add the following:
   ...
   * Add a valid [license key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) to the line `newrelic.license = "INSERT_YOUR_LICENSE_KEY"`.
   * Change the application name that's shown in [one.newrelic.com](https://one.newrelic.com) on the line `newrelic.appname = "PHP Application"` (optional).
