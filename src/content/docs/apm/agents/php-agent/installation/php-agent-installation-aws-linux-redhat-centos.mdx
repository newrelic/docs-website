...
<Callout variant="important">
    The New Relic PHP Agent RPM signing key will be rotated in March 2026. To continue using the agent uninterrupted, the new key must be imported on all systems installing via rpm/yum with the following command. For more information, refer [PHP agent installation: RPM key rotation](/docs/apm/agents/php-agent/installation/php-agent-installation-rpm-key-rotation).
...
2. Add the new RPM key (Only applicable after the rotation of the PHP Agent RPM signing key in March 2026.)
   ```bash
   rpm --import https://download.newrelic.com/php_agent/NEWRELIC_RPM_BD2E199C.public
   ```
