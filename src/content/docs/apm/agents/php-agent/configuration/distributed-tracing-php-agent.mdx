...
Distributed tracing is available for services instrumented with the New Relic APM agent for PHP. When this feature is enabled for each of your services, you can see the entire journey of your requests throughout a [distributed system](/docs/distributed-tracing/concepts/introduction-distributed-tracing). This feature is enabled by default for new agent installations.

The PHP agent automatically instruments with distributed tracing a number of native PHP functions, as well as some third party HTTP clients, such as:
...
For PHP services that are not currently instrumented with the New Relic agent, you can run installation assistant that both instruments your service and enables distributed tracing.
