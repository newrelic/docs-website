...
This document explains how to configure the New Relic PHP agent **after installation**.

## Minimum configuration [#minimum-config]

Most installations only need two lines in `newrelic.ini` to start reporting data. The `newrelic-install` script sets `newrelic.license` for you — confirm the value is correct and add your app name:
...
**That's all that's required.** Restart your web server and data flows to New Relic. Every other setting on this page is optional and has a sensible default.

Find `newrelic.ini` on your system:

```bash
php --ini | grep newrelic
```

Typical locations: `/etc/php.d/newrelic.ini`, `/etc/php/8.x/cli/conf.d/newrelic.ini`, or the directory shown by `php-config --ini-dir`.

## Before you begin [#before-you-begin]

If you haven't installed the PHP agent yet, see our [PHP agent installation overview](/docs/apm/agents/php-agent/installation/php-agent-installation-overview) to:
- Download the agent
- Complete the installation process

After the agent is installed, return here to fine-tune your configuration settings.

## Two configuration files: which one controls what [#two-config-files]

The PHP agent uses two separate configuration files. Knowing which file controls which component prevents confusion and speeds up troubleshooting.
...
<table>
  ...
  <tbody>
    <tr>
      <td>`newrelic.ini`</td>
      <td>**PHP extension (agent)** — what data is collected, how your app is identified, logging, tracing, attributes</td>
      <td>**Always.** This is the primary config file for every installation.</td>
    </tr>
    <tr>
      <td>`newrelic.cfg`</td>
      <td>**Proxy daemon (`newrelic-daemon`)** — how the daemon communicates with New Relic, proxy settings, socket address</td>
      <td>**Only for external/manual daemon mode.** Most installations never need this file. See [Proxy daemon settings](/docs/apm/agents/php-agent/configuration/proxy-daemon-newreliccfg-settings).</td>
    </tr>
  </tbody>
</table>

<Callout variant="important">
  If `/etc/newrelic/newrelic.cfg` exists on your system, the agent will **not** auto-start the daemon. Only create this file if you intend to manage the daemon manually (for example, in Docker or fleet deployments).
</Callout>

## Required and recommended settings [#required-recommended]

Most `newrelic.ini` settings have sensible defaults, but two settings must be set before your app reports data to New Relic.

<table>
  <thead>
    <tr>
      <th width={220}>Setting</th>
  ...
  <tbody>
    <tr>
      <td>[`newrelic.license`](#inivar-license)</td>
      <td>**Required**</td>
      <td>Your New Relic license key. Without this, no data is sent. Find it in **Account settings** or via the [API keys UI](https://one.newrelic.com/api-keys).</td>
    </tr>
    <tr>
      <td>[`newrelic.appname`](#inivar-appname)</td>
      <td>**Highly recommended**</td>
      <td>A descriptive name for your app. Defaults to `"PHP Application"`, which causes all PHP apps on the account to merge their data together. Always set a unique name per application.</td>
    </tr>
    <tr>
      <td>[`newrelic.enabled`](#inivar-enabled)</td>
      <td>Optional</td>
      <td>Defaults to `true`. Set to `false` to disable the agent entirely for a specific environment (for example, local development) without uninstalling it.</td>
    </tr>
    <tr>
      <td>[`newrelic.loglevel`](#inivar-loglevel)</td>
      <td>Optional</td>
      <td>Defaults to `"info"`. Set to `"debug"` temporarily when troubleshooting. Do not leave debug logging on in production.</td>
    </tr>
    <tr>
      <td>[`newrelic.daemon.address`](#inivar-daemon-port)</td>
      <td>Optional</td>
      <td>Defaults to `/tmp/.newrelic.sock`. Change this only if you run the daemon in a separate container or on a non-standard socket path.</td>
    </tr>
  </tbody>
</table>

## Where these files come from [#file-origins]

New users often wonder where `newrelic.ini` and `newrelic.cfg` come from and whether they need to create them. Here's the full picture:
...
<table>
  ...
  <tbody>
    <tr>
      <td>`newrelic.ini`</td>
      <td>**The installer** (`newrelic-install` script)</td>
      <td>Created automatically during every install — package manager, tar, or guided install</td>
      <td>Run `php --ini | grep newrelic` to find it. Typical locations: `/etc/php.d/newrelic.ini` (RedHat/CentOS), `/etc/php/X.X/conf.d/newrelic.ini` (Ubuntu/Debian)</td>
    </tr>
    <tr>
      <td>`newrelic.cfg`</td>
      <td>**You** — only if needed</td>
      <td>Not created by default. Only create this if you need external/manual daemon mode (Docker separate containers, fleet deployments). A template is placed at `/etc/newrelic/newrelic.cfg.template` during install.</td>
      <td>Create it with: `cp /etc/newrelic/newrelic.cfg.template /etc/newrelic/newrelic.cfg`</td>
    </tr>
    <tr>
      <td>`newrelic-daemon` binary</td>
      <td>**The installer**</td>
      <td>Installed alongside the agent automatically</td>
      <td>`/usr/bin/newrelic-daemon` on Linux. In agent mode (default) the agent calls this for you — you never need to touch it directly.</td>
    </tr>
    <tr>
      <td>`.user.ini`</td>
      <td>**You** — only if needed</td>
      <td>Not part of New Relic installation. Create this manually only if you need per-app settings on NGINX/PHP-FPM and can't use pool config.</td>
      <td>Place in your app's web root: `echo 'newrelic.appname = "My App"' > /var/www/myapp/.user.ini`</td>
    </tr>
  </tbody>
</table>

<Callout variant="important">
  Creating `/etc/newrelic/newrelic.cfg` has an immediate side effect: the agent stops auto-starting the daemon. That single file's presence is what switches the agent from automatic mode to external mode. Only create it if you specifically need external mode — otherwise leave it absent.
</Callout>

## Which instrumentation method applies to which environment [#instrumentation-by-environment]

The PHP agent supports three instrumentation approaches. The right one depends on your environment and how you serve PHP.

<table>
  <thead>
    <tr>
      <th width={220}>Instrumentation method</th>
      <th>Environment</th>
  ...
  <tbody>
    <tr>
      <td>**Auto-instrumentation** (default)</td>
      <td>Standard web server setups: Apache mod_php, Nginx + PHP-FPM, CLI scripts. All supported frameworks are instrumented automatically — no code changes required.</td>
      <td>`newrelic.ini` — set `newrelic.license` and `newrelic.appname`, then restart the web server.</td>
    </tr>
    <tr>
      <td>**Per-directory INI settings**</td>
      <td>Single web server hosting multiple apps, Apache VirtualHosts, PHP-FPM pools with per-pool configuration, or Nginx with separate PHP-FPM pools per site.</td>
      <td>Apache: `httpd.conf` / `.htaccess` using `php_value` directives. PHP-FPM: `php_value` entries in pool config. See [Per-directory INI settings](/docs/apm/agents/php-agent/configuration/php-directory-ini-settings).</td>
    </tr>
    <tr>
      <td>**Custom instrumentation / API**</td>
      <td>Background jobs, CLI scripts, unsupported frameworks, or any code path not captured by auto-instrumentation.</td>
      <td>PHP code using `newrelic_add_custom_tracer()`, `newrelic_name_transaction()`, and other [PHP agent API](/docs/apm/agents/php-agent/php-agent-api/guide-using-php-agent-api) functions. Also see [PHP custom instrumentation](/docs/apm/agents/php-agent/features/php-custom-instrumentation).</td>
    </tr>
  </tbody>
</table>
...
## Full configuration reference [#inivar-settings]

All available `newrelic.ini` settings are listed below. Each entry shows its scope (SYSTEM = global only, PERDIR = can also be set per-directory or per-virtual-host), type, and default value. You only need to set a value if the default doesn't suit your needs.
...
<CollapserGroup>
  ...
  >
    ...
    This option defines how the agent should handle sampling spans when their parent span from an upstream entity wasnot sampled. For example, setting remote_parent_not_sampled: always_off means the agent will not sample anything if the upstream entity didn't sample the parent.
...
<DocTiles>
  ...
  >
    After restarting your web server, send a few requests and confirm transactions, errors, and throughput are appearing.
  </DocTile>
  ...
  >
    Running multiple apps on one server? Override `newrelic.appname` and other PERDIR settings per VirtualHost or PHP-FPM pool.
  </DocTile>

  <DocTile
    title="Custom instrumentation"
    path="/docs/apm/agents/php-agent/features/php-custom-instrumentation"
  >
    Capture background jobs, CLI scripts, or code paths not covered by auto-instrumentation using the PHP agent API.
  </DocTile>
  ...
  >
    Check that `newrelic.license` is correct, the web server was restarted, and the daemon is running.
  </DocTile>
</DocTiles>
