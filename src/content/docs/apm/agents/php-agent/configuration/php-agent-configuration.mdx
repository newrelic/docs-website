...
Starting with the PHP agent 10.1.0 release, support for [logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context]) has been added directly to the agent. For instructions on using using logs in context, see [Configure PHP logs in context](/docs/logs/logs-context/configure-logs-context-php). 
...
If you're using a [supported logging framework](/docs/logs/logs-context/configure-logs-context-php) and want to use the agent to send your application logs to New Relic, you can control that through settings `newrelic.config.application_logging.forwarding` prefixed INI settings. Options available are:
...
<CollapserGroup>
  ...
  >
    ...
    When `enabled` = `true`, logs are forwarded directly from the PHP agent with timestamp, application context, log message, and log level included in the event. The PHP agent doesn't write any metadata into the application log output. When `enabled` = `false`, no log data is sent to New Relic and the application log output is unchanged.
