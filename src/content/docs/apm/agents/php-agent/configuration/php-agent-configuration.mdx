...
Keep in mind that changing your settings in your local agent configuration file (`newrelic.ini`) requires a restart of the webserver for them to take effect. If you don't, they may not [take effect immediately](/docs/agents/php-agent/troubleshooting/ini-settings-not-taking-effect-immediately).
...
<CollapserGroup>
  ...
  >
    ...
    * Set the value to a positive integer to specify the maximum number of logs lines per minute to send.
...
<Callout variant="tip">
  This refers to the number of logs sent *per minute*. Setting `max_samples_stored` to a value less than 12 will have the effect of causing no logs to be sent. Any non-negative integer is valid, but currently the maximum supported value is 20,000.
</Callout>
...
When a log level is specifed for the agent, then all messages of this log level and higher will be reported. For example, if you set your log level to `ALERT`, then you will receive logs for the `ALERT` and `EMERGENCY` severity. The default log level is `WARNING`.  
...
The PHP agent captures context data for the Monolog library and adds that context as attributes to your forwarded logs. You can control what context your agent adds through settings under the `context_data` stanza, which is nested under the `forwarding` stanza. 

* `newrelic.application_logging.forwarding.context_data.enable` adds context data from Monolog to your logs attributes
...
<CollapserGroup>
  ...
  >
    ...
    * If log context attributes are not appearing as expected, then you should review the `newrelic.attributes.include` ruleset to see if your missing attributes are part of the exclusion. 
    ...
    * If log context attributes are not appearing as expected, first set `newrelic.loglevel=verbosedebug` to enable verbose log messages from the agent. Once that's updated, look for messages from the agent about excluded attributes. This can clue you in on how to adjust the include/exclude rules.
