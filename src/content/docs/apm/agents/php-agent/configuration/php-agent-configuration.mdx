...
<Callout variant="important">
  Always restart your web server after changing INI settings. Otherwise, they
  may not [take effect
  immediately](/docs/agents/php-agent/troubleshooting/ini-settings-not-taking-effect-immediately).
</Callout>
...
<figcaption>
  With New Relic's PHP agent, API settings override [per-directory configuration
  settings](/docs/agents/php-agent/configuration/php-directory-ini-settings).
  Per-directory settings override the `php.ini` file settings. Server-side
...
<Callout variant="important">
  If the file `/etc/newrelic/newrelic.cfg` exists, the agent ignores these
  settings, and the agent will **not** start the daemon automatically.
</Callout>
