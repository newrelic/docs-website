...
the agent send log data and linking metadata to allow 
...
<CollapserGroup>
  ...
  >
    ...
    Seting this to `0` will effectively disable the sending of custom events.   See also the setting [`newrelic.custom_insights_events.enabled`](#inivar-custom-events-enabled) as the preferred way to enable or disable custom event generation and logging.
    ...
    <Callout variant="important">
      The `newrelic.custom_events.max_samples_stored` INI variable was added in the 10.4.0.nnn release.
    </Callout>
