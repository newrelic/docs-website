...
When a log level is specifed for the agent, all messages of this log level and higher will be reported. For example, if you set your log level to `ALERT`, then you will receive logs for the `ALERT` and `EMERGENCY` severity. The default log level is `WARNING`.  
