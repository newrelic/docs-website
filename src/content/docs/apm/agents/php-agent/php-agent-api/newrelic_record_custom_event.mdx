...
<Callout variant="tip">
  ...
  * The agent records a maximum of 30,000 events per minute by default, and can be configured to record up to 100,000 events per minute with [this ini setting](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#inivar-custom-events-maxsamples). Prior to version 10.4.0.316, the agent recorded a maximum of 10,000 events per minute.
  * Limit the number of unique event type names that you create, and do not generate these names dynamically.
