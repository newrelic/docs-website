...
<Callout variant="tip">
  ...
  * Ensure you do not exceed the [event size and rate restrictions](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/).
</Callout>
  * If `newrelic_ignore_transaction` is called within the same transaction as `newrelic_record_custom_event`, the custom event(s) will not reported.
