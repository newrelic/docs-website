...
```
The value `samantha@newrelic.com` gets recorded as the `user_email` custom attribute in the web transaction, and  `billy@newrelic.com` from earlier call is overwritten.