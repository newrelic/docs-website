...
If `newrelic_add_custom_parameter` is called multiple times with the same value used for the `$key` parameter within the same web transaction, the `$value` from the last called takes precedence and is recorded as custom attribute in the web transaction. 
...
```
The value `samantha@newrelic.com` gets recorded as `user_email` custom attribute in the web transaction, and  `billy@newrelic.com` from earlier call is overwritten.