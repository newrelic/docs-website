...
How you set up the .NET agent in your Docker containers will vary, depending on your setup. Here's an overview of requirements and considerations: 
* The agent must be installed on the containers you want to monitor.
* You'll install the agent with one of our standard install procedures: [Windows](/docs/apm/agents/net-agent/installation/install-net-agent-windows) | [Linux](/docs/apm/agents/net-agent/installation/install-net-agent-linux)
* You'll configure your Docker setup to work with the agent. We've given some examples of Docker files in this doc.  
* You'll enable the .NET agent by setting the [required environment variables](/docs/agents/net-agent/configuration/net-agent-configuration#environment-variables), as applicable.
* The .NET agent must be installed and enabled at runtime.

If your application is receiving traffic, data should appear within a few minutes. If it doesn't, see [No data appears](/docs/apm/agents/net-agent/troubleshooting/no-data-appears-net).
