...
After installing New Relic's .NET agent, you see an increase in `Working Set` memory usage in monitoring tools such as the Microsoft Windows <DoNotTranslate>**Task Manager**</DoNotTranslate>. The increase is usually about 100 MB per monitored application. This can occur on both physical and virtual machines.
...
* Monitor performance to see if the increased memory usage shown in the <DoNotTranslate>**Task Manager**</DoNotTranslate> is actually impacting performance. If it is, release memory and share with other processes.
