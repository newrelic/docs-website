...
With the .NET agent enabled for an Asp.Net Core "Minimal APIs" application, endpoints with the same route path but different HTTP request methods appear as a single web transaction.
