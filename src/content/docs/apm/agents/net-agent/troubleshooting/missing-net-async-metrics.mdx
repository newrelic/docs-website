...
<CollapserGroup>
  ...
  >
    If you want to specify the .NET Framework version you target, be aware that this imposes strict runtime checks that can cause compatibility issues. <DoNotTranslate>**Recommendation:**</DoNotTranslate> If possible, [tell .NET to use the new pipeline](#use-new-pipeline) instead of using this option.
