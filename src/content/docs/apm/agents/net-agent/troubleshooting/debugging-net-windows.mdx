...
tags:
  ...
  - NET agent
  - Troubleshooting
metaDescription: Troubleshooting tips for New Relic's .NET agent for Linux.
redirects:
...
After installing New Relic's .NET agent for Linux, you have one or more of the following problems: 
...
* Note that [ASP.NET Core apps are automatically instrumented](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-core#instrumented-features), but other application frameworks require [custom instrumentation](/docs/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation)

## Step 1: Check for agent logs of the application [#step-one]

Some important caveats on where the agent is installed: 
...
* Make sure that the user your application process is running under has read/write/execute permissions to the directory where the .NET Core agent was extracted on your system (`CORECLR_NEWRELIC_HOME`), and all of its sub-directories.
* If you made any changes, restart your app and go back to [Step 1](#step-one).

If the above steps did not address the issue, we recommend you contact support or ask for help in [our Explorers Hub forum](https://discuss.newrelic.com).
