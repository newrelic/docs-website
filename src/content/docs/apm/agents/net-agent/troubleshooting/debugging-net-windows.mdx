...
* If you installed the .NET agent, or installed the agent with the `NewRelic.Agent` NuGet package, you'll find a `logs` folder in the directory where the agent was extracted on your system.
...
If the correct variables are set and the application you're trying to monitor has access to them, you should see a particular set of environment variables, depending on whether your app (agent??) is .NET Framework or .NET Core. 
...
To check for environment variables, you'd use a command like: 
