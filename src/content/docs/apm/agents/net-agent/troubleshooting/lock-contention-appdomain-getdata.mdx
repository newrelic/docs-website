...
When the .NET agent is attached to high throughput <DoNotTranslate>**.NET Framework**</DoNotTranslate> applications, a few customers reported observing high thread contention. Upon reviewing the process dumps and stack traces, many threads may be blocked by calls to `AppDomain.GetData()`.
