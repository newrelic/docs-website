...
<CollapserGroup>
  ...
  >
    In this example, New Relic reports an error from `GetResponse()` <DoNotTranslate>**unless**</DoNotTranslate> the method `Foo()` is instrumented. As long is `Foo` is instrumented, New Relic begins a transaction when `Foo` is called and ends the transaction when `Foo` ends.
