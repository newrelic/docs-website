...
<CollapserGroup>
  ...
  >
    IIS-hosted [WCF services](/docs/agents/net-agent/instrumentation/instrumenting-wcf-applications) do not properly nest the <DoNotTranslate>**WCF**</DoNotTranslate> segment under the `ExecuteRequestHandler` segment. The two segments will appear to be siblings within a transaction trace, even though their reported [total time](/docs/data-analysis/user-interface-functions/response-time#response-time-total-time) will be accurate.
  </Collapser>
