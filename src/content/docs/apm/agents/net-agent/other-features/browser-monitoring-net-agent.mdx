...
[Browser auto-instrumentation](/docs/agents/net-agent/configuration/net-agent-configuration#browsermon-autoInstrument) is enabled by default for .NET Framework web apps but is disabled by default for .NET Core v6 and later web apps; see [Enabling browser auto-instrumentation for .NET Core v6+](#enable_netcore6plus) for further information. With browser auto-instrumentation, the .NET Framework agent automatically injects the browser JavaScript header into any page that has a `content-type` of `text/html` and also has `<head>` tag within the page.
...
## Enabling Browser Auto-Instrumentation for .NET Core v6+ web applications [#enable_netcore6plus]

To enable browser auto-instrumentation in .NET Core v6+ web apps (.NET agent v10.19.2 and later): 

1. Ensure that auto-instrumentation is enabled globally by editing **`newrelic.config`** and set the [`browserMonitoring` `autoInstrument` flag](/docs/agents/net-agent/installation-configuration/net-agent-configuration#browsermon-autoInstrument) to `true`:
