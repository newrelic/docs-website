...
2. Enable auto-instrumentation for .NET Core 6+ web apps by editing <DoNotTranslate>**`newrelic.config`**</DoNotTranslate> and adding a new item in the `appSettings` section (or add the `appSettings` section if it doesn't exist) - Note that is **not** the same as the `appSettings` section in a `web.config` file:
