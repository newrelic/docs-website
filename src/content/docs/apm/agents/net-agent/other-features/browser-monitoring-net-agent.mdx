...
[Browser auto-instrumentation](/docs/agents/net-agent/configuration/net-agent-configuration#browsermon-autoInstrument) is enabled by default for .NET Framework web apps but is disabled by default for .NET Core v6 and later web apps; see [Enabling browser auto-instrumentation for .NET Core v6+](#enable_netcore6plus) for further information. With browser auto-instrumentation, the .NET  agent automatically injects the browser JavaScript header into any page that has a `content-type` of `text/html` and also has a `<head>` tag within the page.
