...
1. Ensure that you've enabled auto-instrumentation globally by editing <DNT>**`newrelic.config`**</DNT> and setting the [`browserMonitoring` `autoInstrument` flag](/docs/agents/net-agent/installation-configuration/net-agent-configuration#browsermon-autoInstrument) to `true`:
...
2. Enable auto-instrumentation for .NET Core 6+ web apps by editing <DNT>**`newrelic.config`**</DNT> and adding a new item in the `appSettings` section (or add the `appSettings` section if it doesn't exist) - Note that is **not** the same as the `appSettings` section in a `web.config` file:
