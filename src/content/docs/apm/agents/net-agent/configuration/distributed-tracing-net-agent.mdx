...
The Infinite Tracing setup builds on the instrumentation step from the [installation assistant](#quick-start-apm) for standard distributed tracing.
...
Distributed tracing is enabled through configuration settings. Review the following .NET-specific configurations. For general help with agent configurations, see [Configure the agent](/docs/agents/manage-apm-agents/configuration/configure-agent).
...
<CollapserGroup>
  ...
  >
    ...
    2. Invoke the agent API call for generating a distributed trace payload: [.NET](/docs/agents/net-agent/net-agent-api/itransaction#CreateDistributedTracePayload).
  ...
  >
    ...
    4. Invoke the call for accepting the payload: [.NET](/docs/agents/net-agent/net-agent-api/itransaction#AcceptDistributedTracePayload).
  </Collapser>
</CollapserGroup>
