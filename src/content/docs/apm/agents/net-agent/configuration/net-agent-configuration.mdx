...
<CollapserGroup>
  ...
  >
    ...
    If after all the logs for sampled transaction are recorded and they haven’t reached the limit in `maxSamplesStored`, then log messages for transaction that were not in our sampling are sent. If there are any left, log messages outside of transactions are recorded.
