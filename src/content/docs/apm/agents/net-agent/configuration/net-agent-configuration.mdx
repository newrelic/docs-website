...
<CollapserGroup>
  ...
  >
    ...
    When `true`, the agent captures `EXPLAIN` statements for slow queries during transaction finalization. Note: Enabling this feature can have performance implications as explain plans are captured optimistically by the .NET agent.
  </Collapser>
