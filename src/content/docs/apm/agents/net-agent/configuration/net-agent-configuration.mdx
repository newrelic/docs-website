...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      If the agent is disabled in the local or global `newrelic.config`, the `NewRelic.AgentEnabled` settings in these files will <DoNotTranslate>**not**</DoNotTranslate> enable the agent.
    </Callout>
  </Collapser>
