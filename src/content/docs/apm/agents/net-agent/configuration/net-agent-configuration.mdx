...
<CollapserGroup>
  ...
  >
    ...
    When `true`, the agent captures `EXPLAIN` statements for slow queries with durations exceeding `explainThreshold`. Note: Enabling this feature can have performance implications as up to `maxExplainPlans` explain plans can be captured for every transaction.
  </Collapser>
