...
2. Log forwarding: When enabled, the agent will capture log data and send it to New Relic.
    ...
    * Log level filtering:  When configured with one or more log levels in a comma-separated list, prevents messages at those levels from being captured and forwarded.
...
<CollapserGroup>
  ...
  >
    ...
    Use the `logLevelDenyList` attribute to filter out log messages of the specified log levels.  The value for this attribute is a comma-separated case insensitive list of levels.  Outside of case, the log level string must exactly match the log level being filtered, wildcards and regex are not supported.  For example, if the value were set to `logLevelDenyList="debug,warn"`, any log messages with the a log level of Debug or Warn will not be captured or forwarded, but a log with a level of Warning would still be sent to New Relic.
