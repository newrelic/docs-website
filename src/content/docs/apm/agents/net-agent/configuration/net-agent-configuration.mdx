...
The `codeLevelMetrics` element is a child of the `configuration` element. Use `codeLevelMetrics` to enable code level metrics
support in CodeStream via additional instrumented method metadata captured as attributes on span events.

For more details, see [New Relic CodeStream Integration](/docs/apm/agents/net-agent/other-features/net-codestream-integration).
