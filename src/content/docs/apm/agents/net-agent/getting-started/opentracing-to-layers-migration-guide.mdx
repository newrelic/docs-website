...
Instrumenting .NET AWS Lambda functions no longer uses the deprecated NewRelic.OpenTracing.AmazonLambda.Tracer package and instead leverages the full New Relic .NET Agent.  In most cases, the .NET Agent can detect and automatically instrument your AWS Lambda function.  When instrumenting an AWS Lambda function, the .NET Agent switches to a new "serverless mode" that works to reduce the impact of the agent on limited AWS Lambda function resources and that relies on the New Relic Lambda Extension to send data to New Relic.  The .NET Agent is made available via a pair of AWS Lambda Layers includes both the .NET Agent and the required New Relic Lambda Extension. As part of streamlining the installation process, the layer can be configured using the New Relic Lambda CLI tool, a Serverless Framework plugin, or manually.  It is also possible to manually add and configure the .NET Agent to your AWS Lambda function using Nuget along with manually adding the New Relic Lambda Extension, but it is not recommended.  There can be differences in the metric names and timing that are reported by the .NET Agent compared to the NewRelic.OpenTracing.AmazonLambda.Tracer package since the way the function is instrumented has fundamentally changed.  Any dashboard, alerts, or queries that are based on the old metrics will need to be updated after migrating.  The NewRelic.OpenTracing.AmazonLambda.Tracer package conflicts with the .NET Agent and must be removed.

Main changes include:

* The full New Relic .NET Agent will be used to instrument the AWS Lambda function instead of NewRelic.OpenTracing.AmazonLambda.Tracer
* The NewRelic.OpenTracing.AmazonLambda.Tracer package conflicts with the .NET Agent and must be removed
* The instrumentation of the AWS Lambda function is automatic in most cases
* The .NEY Agent relies on the New Relic Lambda Extension for sending data to New Relic when instrumenting AWS Lambda functions
...
The NewRelic.OpenTracing.AmazonLambda.Tracer package must be removed before using the full .NET Agent and any changes made to your AWS Lambda functions to support it must be undone.
...
2. The full .NET Agent does not support using OpenTracing APIs to pass in custom data.  Any changes to your code that relied on OpenTracing APIs, whether directly or via the NewRelic.OpenTracing.AmazonLambda.Tracer package, need to be removed or migrated to use .NET Agent APIs.
