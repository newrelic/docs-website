...
<Callout variant="important">
  Running your application from Visual Studio in <DoNotTranslate>**debug**</DoNotTranslate> mode may prevent some methods from appearing in New Relic traces. To ensure methods appear in New Relic, run the application in release mode via the command line. 
</Callout>
