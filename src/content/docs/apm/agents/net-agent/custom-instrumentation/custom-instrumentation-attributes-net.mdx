...
<Callout variant="important">
 If some of your methods still do not show up in traces after adding the `[Trace]` attribute, disable method inlining for those methods with `[MethodImpl(MethodImplOptions.NoInlining)]`.
</Callout>
