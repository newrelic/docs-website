...
* <DNT>Docker/DNT>: [for Windows](/install/dotnet/?deployment=windowsInstall&docker=yesDocker) | for [Linux](/install/dotnet/?deployment=linux&docker=yesDocker)
...
<Callout variant="important">
  ...
  * Updating the agent requires <DNT>**Administrator**</DNT> rights in your Windows admin group. For more information, see the [permissions documentation](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework/#user-rights).
    ...
    <table>
      ...
      <tbody>
        ...
        <tr id="220">
          ...
          <td>
            Version 2.19.3.0 was the last .NET agent version that required both the `GetBrowserTimingFooter()` and the `GetBrowserTimingHeader()` API calls for <InlinePopover type="browser" />. In agent version 2.20.24.0 or higher, you only need to call `GetBrowserTimingHeader()`. `GetBrowserTimingFooter()` has no effect. <DNT>**Recommendation:**</DNT> Remove these references from your code.
          </td>
        </tr>
