...
If <DoNotTranslate>**Service**</DoNotTranslate> files are nested within a <DoNotTranslate>**Solution**</DoNotTranslate> folder, the NuGet installer cannot locate or update the necessary files. This will cause issues with the .NET agent setup, which will in turn prevent the agent from reporting metrics on your Cloud Service.

<DoNotTranslate>**Recommendation:**</DoNotTranslate> Place the `Web` or `Worker` role at the <DoNotTranslate>**root**</DoNotTranslate> of the solution before installing the NuGet package. Once the New Relic .NET agent is installed, you can move the Cloud role back into the <DoNotTranslate>**Solution**</DoNotTranslate> folder.
...
<CollapserGroup>
  ...
  >
    ...
    1. In your Azure Cloud project, add the instrumentation file to your `Worker` role inside the <DoNotTranslate>**Roles**</DoNotTranslate> folder.
    ...
    3. Find the statement `IF %ERRORLEVEL% EQU 0` within the `:INSTALL_NEWRELIC_AGENT` block, and add the following statement to the conditional:
       ...
       In this example, the `newrelic.cmd` batch file copies the custom instrumentation file to the <DoNotTranslate>**`Extensions`**</DoNotTranslate> folder in `D:\ProgramData\New Relic\.NET Agent\`, or `%NR_HOME%`. This example uses `MyInstrumentation.xml`, but any name will work as long as the file name and copy command match, and the file is valid XML.
  </Collapser>
</CollapserGroup>
