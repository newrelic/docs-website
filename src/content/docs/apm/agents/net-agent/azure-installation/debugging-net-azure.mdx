...
After [installing New Relic's .NET agent for Windows](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/), you have one or more of the following problems: 
...
* You **must** [disable Application Insights](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) in order for the .NET agent to function properly.
* If you use Azure Pipelines, be aware of a known issue where it could [wipe out NewRelic.Azure.WebSites.Extension directories](https://docs.newrelic.com/docs/apm/agents/net-agent/troubleshooting/azure-pipelines-wipes-out-newrelicazurewebsitesextension-directories/)
...
* If you installed the agent with the `NewRelic.Agent` or `NewRelic.Azure.WebSites` NuGet package, you'll find the `logs` folder in the directory where the agent was extracted on your system.
...
1. Uee Kudu's built-in Process Explorer (Windows only; for Linux installs, reference the commands in [this article](https://docs.newrelic.com/docs/apm/agents/net-agent/troubleshooting/debugging-net-core-agent-linux/).)
2. In Process Explorer, find the process you're trying to monitor, and make note of the number in the PID column. If you're trying to monitor a web application, the process name will be `w3wp.exe` *without* the SCM label.
3. When you've located your application's process, click on the `Properties` button.
4. Click on the `Modules` tab and look for `NewRelic.Profiler.dll` — this DLL is required for the agent to monitor your application. If it is not present, make sure that the correct environment variables are present (more on this in [Step 3](#step-three), and [Microsoft Application Insights is disabled](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/)).

If you don't see `NewRelic.Profiler.dll`, continue to the next step. If you do get results, if the app is receiving traffic, you should be seeing logs per [Step 1](#step-one)

## Step 3: Check for required environment variables [#step-three]

To check for the required environment variables: 

1. Locate your application's process, and click on the `Properties` button.
...
<Callout variant="important">
.NET Framework only:
If the application you want to monitor is not hosted in IIS (such as a self-hosted Windows service), you must explicitly enable the agent for that application, as noted in our [installation docs](/docs/apm/agents/net-agent/installation/install-net-agent-windows#enabling-the-agent).
</Callout>
...
Ensure your `newrelic.config` file's **log** node includes a directory attribute. (**Note:** If Visual Studio prevents you from editing the `newrelic.config` file that was added to your project by NuGet then you will need to make a local copy of this in your application):
...
<Callout variant="tip">
For further reading on these environment variables and their funcionality, refer to this [document]().
</Callout>
