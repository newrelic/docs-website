...
* Beginning with .NET agent version 8.22.181.0, Azure's `Always On` setting may be enabled without interfering with the agent. If your application targets .NET Framework 4.0 or lower, `Always On` will still interfere with the agent and the workarounds [here](/docs/agents/net-agent/azure-troubleshooting/azure-web-apps-using-always-no-data-appears) are still applicable.
...
2. In Process Explorer, find the process you're trying to monitor, and make note of the number in the PID column. If you're trying to monitor a web application, the process name will likely be `w3wp.exe` *without* the SCM label.
3. When you've located your application's process, click on the `Properties` button.
4. Click on the `Modules` tab and look for `NewRelic.Profiler.dll` — this DLL is required for the agent to monitor your application. If it is not present, make sure the correct environment variables are present (more on this in [Step 3](#step-three)), and [Microsoft Application Insights is disabled](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/).
...
<Callout variant="tip">
For further reading on these environment variables and their functionality, refer to this [document](insert alex's new doc here).
</Callout>
