...
If you're using containers in your Service Fabric environment, see the [install instructions for <DNT>Docker/DNT>](/install/dotnet).
...
3. <DNT>**For the .NET Framework only**</DNT>: Edit your `app.config` file and add the `NewRelic.AgentEnabled` app setting:
