...
If you need to add a custom XML instrumentation, or modify the base `newrelic.config` file, you'll find the .NET Framework agent at `%HOME%\NewRelicAgent\newrelic`, and the .NET Core agent at`%HOME%\NewRelicAgent\newrelic_core`.
</Callout>
