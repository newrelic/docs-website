...
In addition, WebJobs may need [custom instrumentation](/docs/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation) for transactions to appear in APM, and [additional configuration](/docs/agents/net-agent/configuration/net-agent-configuration#service) if they run for less than one minute.
