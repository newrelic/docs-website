...
5. The New Relic Azure Site Extension installs the .NET Framework agent to `%HOME%\NewRelicAgent\newrelic` and the .NET Core agent to `%HOME%\NewRelicAgent\newrelic_core` in the case that you need to add custom instrumentation XML, or modify the base newrelic.config.
...
5. From **Configuration**, select **+** **New application setting** and add these key/value pairs (**NOTE:** These examples all assume that your Azure Web App uses a `D:` Filesystem root. If your Azure Web uses a `C:` root then you will need to use the correct drive letter when configuring these paths):
