...
After [installing New Relic's .NET agent for Azure](/install/dotnet/?deployment=azure), you have one or more of the following problems:
...
* Ensure that the [installation and execution steps have been followed](/install/dotnet/?deployment=azure), including restarting your web app.
* You <DNT>**must**</DNT> [disable Application Insights](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) in order for the .NET agent to work properly.
* The app <DNT>**must**</DNT> be getting traffic for the .NET agent to startup.
...
4. Make note of the process ID (PID) your application is running under, so you can verify if a log is being created for that process. You can find the PID by using Kudu's built-in <DNT>**Process Explorer**</DNT>.
...
2. In <DNT>**Process Explorer**</DNT>, find the process you're trying to monitor, and make note of the number in the PID column. If you're trying to monitor a web application, the process name will likely be `w3wp.exe` without the SCM label, which refers to the Kudu process itself. The agent shouldn't instrument the SCM (Kudu) process.
3. When you've located your application's process, click on the `Properties` button.
4. Click the <DNT>**Modules**</DNT> tab, and look for `NewRelic.Profiler.dll` — this DLL is required for the agent to monitor your application. If you can't find it, make sure the correct environment variables are present (more on this in [step 3](#step-three)), and [Microsoft Application Insights is disabled](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/).
...
1. In <DNT>**Process Explorer**</DNT>, locate your application's process, and click on the `Properties` button.
2. Select the <DNT>**Environment**</DNT> tab.
...
<CollapserGroup>
  ...
  >
    For the NuGet package, the environment variables [must be added as application settings](/install/dotnet/?deployment=azure&azure=azurenugetframework).
    ...
    If you made any changes to the environment variables, make sure to restart your application via the Azure Portal. Then inspect the new process PID in <DNT>**Process Explorer**</DNT> in order to ensure the changes were successful.
  </Collapser>
  ...
  >
    For the NuGet package, the environment variables [must be added as application settings](/install/dotnet/?deployment=azure&azure=azureframeworkcore). The environment variables that appear in the <DNT>**Environment**</DNT> tab in Kudu's <DNT>**Process Explorer**</DNT> should be similar to these:
  ...
  >
    For the NuGet package, the environment variables [must be added as application settings](/install/dotnet/?deployment=azure&azure=azureframeworkcore).
    ...
    Ensure your `newrelic.config` file's <DNT>**log**</DNT> node includes a directory attribute. If Visual Studio prevents you from editing the `newrelic.config` file that was added to your project by NuGet, you need to make a local copy of this in your application:
