...
1. Create a `.ebextension` folder at the root of your application and create a file named `newrelic-dotnet-agent-install.config`. (The exact filename isn't important, but it must have a `.config` extension.)
