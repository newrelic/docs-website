...
<Callout variant="tip">
  The agent adds the attributes only to the traced error, and does not send them to New Relic. To add custom attributes, see [`AddCustomAttribute()`](/docs/apm/agents/net-agent/net-agent-api/itransaction/#addcustomattribute).
</Callout>
