...
All other configuration values are optional, and typically the default settings don't need to be changed. However, when necessary, you can adjust New Relic's  <DNT>C SDK</DNT> configuration.

This document is a quick reference for using some basic configuration options with the  <DNT>C SDK</DNT> API. For detailed information about changing your configuration settings, including code values and examples, see the [ <DNT>C SDK</DNT>'s configuration GUIDE.md on GitHub](https://github.com/newrelic/c-sdk/blob/master/GUIDE.md#configuration).
...
You can change your [application's alias](/docs/agents/manage-apm-agents/app-naming/name-your-application#differences) from the <DNT>**Application**</DNT> settings page in the New Relic UI. This is useful, for example, to give your application a different name, yet keep historic data under the new alias. For more information, see [Name your application](/docs/agents/manage-apm-agents/app-naming/name-your-application).

New Relic's C SDK does not support [server-side configuration](/docs/agents/manage-apm-agents/configuration/server-side-agent-configuration). However, you can also use this <DNT>**Application**</DNT> settings page in the UI to set your application's [Apdex T](/docs/apm/new-relic-apm/apdex/apdex-measure-user-satisfaction) threshold.

To change the application's alias or Apdex T threshold in the UI go to <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Application**</DNT>.
