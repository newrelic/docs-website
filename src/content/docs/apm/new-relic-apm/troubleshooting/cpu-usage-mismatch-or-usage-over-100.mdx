...
The CPU percent usage data in APM seems contradictory with that in infrastructure monitoring, or your CPU usage exceeds 100%.

## Cause

CPU usage data for APM is different than CPU usage data for infrastructure monitoring:

* In APM, CPU percent usage measures aggregate usage of all **instances** of an application or service on a given server.
* In infrastructure, CPU percent usage is measured differently depending on if you're viewing CPU usage in relation to [hosts](/docs/infrastructure/infrastructure-ui-pages/hosts-new-view#systems), or to [processes](/docs/infrastructure/infrastructure-ui-pages/hosts-new-view/#processes).
       ...
       <table>
         ...
         <tbody>
           <tr>
             ...
             <td>
               On the **Hosts** page, `CPU %` is a metric derived from other `SystemSample` metrics. For more on this, see [System UI](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#system). 
           ...
           <tr>
             ...
             <td>
               ...
               For example, with a 32-core server, a process showing `cpuPercent` of 50% is only taking half of a single core, or about 1.5625% of the CPU cycles available on the whole server. This can result in CPU % exceeding 100% on the **Processes** page. When this occurs, the process is demanding more than a single core's worth of CPU cycles.
             </td>
           </tr>
         </tbody>
       </table>
