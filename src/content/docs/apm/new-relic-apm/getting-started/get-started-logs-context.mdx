...
* Startup logs or host information are not available.
* If you are using Kubernetes, you will not get decoration from the Kubernetes API.
* When an APM event is triggered, you may not see all stack traces in the associated logs in context. (As a workaround you could change your drop filter rules.)
