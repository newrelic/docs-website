...
* Startup logs or host information are not available. This is because logs are not collected until after the app is loaded.
* If you are using Kubernetes, the logs will not get decoration from the Kubernetes API. This is because the logs never touch the host or exist in a place where the API can be called.
* When an APM event is triggered, you may not see all stack traces in the associated logs in context for Java or .NET agents. As a workaround, you can change your drop filter rules.

For example, Fluentd can add the `processID` from the entity that generated the log, but APM logs can't see that. In Kubernetes, the API is called to add metadata, but that can't be seen from within the application. In these situations, we recommend that you use automatic logs in context but do not ship the logs from the application. Instead, continue to use Fluentd, Fluent Bit, or another solution to forward the log files.
