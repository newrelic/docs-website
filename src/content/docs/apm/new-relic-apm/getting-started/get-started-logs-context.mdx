...
The new versions of our APM agents have both of these features enabled by default, so installing an APM agent for a supported implementation will get you logs reporting, out-of-the-box.  

You can also disable one or both parts of this feature for a specific APM agent. For example, you might want to have an APM agent add New Relic metadata, so you use the logs in context feature, but disable the forwarding and instead use another forwarder (for example, our infrastructure agent, or a third-party log service). The configuration instructions will vary per agent. For information on disabling logs, see [Manage automatic logs for APM](/docs/logs/logs-context/disable-automatic-logging).
