...
* Decorate your app logs with some important New Relic metadata (like `span.id`, `trace.id`, `hostname`, `entity.guid`, `entity.name`, and more) that allows you to see your logs in context with other New Relic platform experiences. 
...
You can also disable one or both parts of this feature for a specific APM agent. For example, you might want to have an APM agent add New Relic metadata, so you get the logs in context feature, but disable the forwarding and use another forwarder (for example, our infrastructure agent or a third-party log service). The configuration instructions will vary per agent. For information on disabling logs, see [Manage automatic logs for APM](/docs/logs/logs-context/disable-automatic-logging).
