...
You can also query deployments data via NerdGraph (New Relic's GraphQL API) or NRQL, our bespoke query language for the New Relic database (NRDB). For more on using GraphQL/NerdGraph queries, see HERE(LINK) and NRQL HERE(LINK). 

Here's a simple NRQL query to get you started. You can run this from query builder:

```sql
SELECT * FROM Deployment SINCE 30 days AGO
```

Here's an example NerdGraph query:
