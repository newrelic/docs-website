...
* To configure an agent to monitor some things but not others, use [custom instrumentation](/docs/apm/agents/manage-apm-agents/agent-data/custom-instrumentation/).
...
<CollapserGroup>
  ...
  >
    ...
    * Use the [`agent_enabled`](/docs/agents/ruby-agent/configuration/ruby-agent-configuration#agent_enabled) configuration setting. This can be done in the configuration file or in the per-environment sections. Restart your app server afterward.
