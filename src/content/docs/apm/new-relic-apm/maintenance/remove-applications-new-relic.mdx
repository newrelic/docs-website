...
<CollapserGroup>
  ...
  >
    If you do not have relevant [permissions](/docs/accounts/accounts-billing/account-structure/factors-affecting-access-features-data), you won't be able to remove an application.
  </Collapser>
  ...
  <Collapser
    id="name-confusion"
    title="You do not know where the app's data is coming from"
  >
    If you do not know where an app's data is coming from, it may be because the [app's name has been changed](/docs/agents/manage-apm-agents/app-naming/rename-your-application). When an app name is changed in the UI, it does not change the underlying app name being reported; it only changes how the app name appears in the UI.
  ...
  >
    ...
    If these steps do not resolve the issue, it is likely due to an end-user's browser cache that hasn't yet cleared. Wait until those caches clear. If your app has internal users, you may be able to identify the users and clear those caches.
  </Collapser>
</CollapserGroup>