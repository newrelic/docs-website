...
Before removing an application monitored by New Relic APM, browser monitoring, or mobile monitoring, the app must first stop reporting data. This can be done by disabling the agent, which we explain how to do below, or by [uninstalling the agent completely](/docs/agents/manage-apm-agents/installation/uninstall-agent).
