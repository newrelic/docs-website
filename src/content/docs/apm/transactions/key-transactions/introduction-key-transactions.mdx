...
![Screen capture showing the overview page for key transactions.](src/images/key-trx-overview.png 'key-trx-overview.png')

<figcaption>
  The key transaction's page includes response time and throughput charts,
  service-level agreement information, a cross application tracing map (if
  available), and links to alert policy options.
</figcaption>
