...
Now that your key transaction is part of a workload and tagged, the next step is to create alerts so you're notified when there is any unusual behavior in your transaction. You can set up both automatic alerts and custom [anomalies](/docs/alerts-applied-intelligence/applied-intelligence/anomaly-detection/anomaly-detection-applied-intelligence/) on key transactions using the powerful and flexible [NRQL alerting system](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions/) and now you can also add your key transactions to dashboards. If you want to set up alerts or add the key transactions to dashboards, you can do so directly from the key transaction charts by clicking the ellipsis. 
