...
import apmTransactionTraceDbQueriesView from '/images/apm_screenshot-crop_transaction-traces-database-queries-view.webp'

import apmTransactionTraceStacktraceView from 'images/apm_screenshot-crop_transaction-traces-stacktrace-view.webp'
...
<img
  title="APM-transaction-trace-database-queries-view.png"
  alt="APM - Transaction trace - Database queries tab"
  src={apmTransactionTraceDbQueriesView}
/>

<figcaption>
  Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitor > Transactions > (select a transaction trace) > Database queries**.
</figcaption>
...
### View Stacktrace & Query
When a stacktrace associated with a database query exists, click the row in the database queries table to view the formatted stacktrace.
Both the stacktrace and query in the view have copy to clipboard capabilities, in addition to an optional action that shows Framework code in the stacktrace.
<img
  title="APM-transaction-trace-stacktrace-view.png"
  alt="APM - Transaction trace - Database query stacktrace"
