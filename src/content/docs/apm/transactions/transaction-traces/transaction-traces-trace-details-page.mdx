...
A transaction is a grouping of function and method calls that comprise a single unit of work. Transactions often map to a URL, but not always. In [APM](/docs/apm/new-relic-apm/getting-started/welcome-new-relic-apm) you can view all of your transactions for a particular entity under the **Transactions** tab. If you click on a specific transaction under the transactions tab, you'll see data for all users across all time. 

But if you want to learn more about what happened for a _specific_ user, during a _specific_ instance, you need to use **Transaction traces**. With the ** Trace details tab** tab you can:

* **Identify slow segments**: Clearly marks segments that took more than 25% of the total trace time as "slow spans," directing attention to potential bottlenecks.
* **Visualize your transactions**:  See a graphical representation of the transaction's timeline, showcasing the duration of different segments (database calls, external calls, code execution).
* ** Efficiently troubleshoot**: Allows filtering to focus exclusively on slow spans. 


## How it works [#how-it-works]

The **Trace details** tab provides a intricate deep dive into one user's experience with a transaction during a single instance. Information is displayed in three views: timeline, latency, and waterfall.  
...
The trace timeline view contains an interactive condensed visual display of the trace. Node color presented in this view matches the waterfall legend for the category of each segment.
...
** Focus on Slow Spans** : If your trace has slow spans, click the checkbox to instantly filter out other segments and focus on potential bottlenecks.
...
The waterfall is broken up into different segments:
...
<img
    title="uninstrumented segement"
    alt="An icon that represents uninstrumented segments in the UI"
...
Each segment in the waterfall displays a bar that represents their duration relative to the trace duration. The color of this bar indicates whether the segment belongs to Application code, Database calls, or External calls.
...
Each segment can be identified by these icons:
...
To make complex transaction traces easier to understand, New Relic automatically groups fast and repeated method calls within the waterfall. This helps you focus on potentially problematic areas without getting overwhelmed by too many segments.

Here's how it works:

* **Fast calls**: When you see a segment titled like 7 fast method calls, it means New Relic has grouped multiple calls that completed quickly (under 7.5 ms each). These are usually not performance concerns, but you can expand the group to view individual segments if needed.
...
When you select a segment, segment details will appear. With segment details you can: 

1. Performance tab: 
  * **Visualize performance trends: charts showcase the average duration and throughput for a segment's operation, revealing patterns and potential outliers.
...
3. Details tab: 
  * **Foundational information**: View key details like the segment's full name, data source, and relevant metadata. You can learn more in our [distributed tracing docs](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#prettified-span-names).
