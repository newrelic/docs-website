...
If server-side configuration settings are available for your New Relic agent, you can see them at the following: **[one.newrelic.com](https://one.newrelic.com) > APM > (select an app) > Settings > Application > Server-side agent configuration**.
