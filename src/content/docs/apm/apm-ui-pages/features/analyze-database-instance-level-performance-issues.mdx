...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (select an app) > Databases > (select a database operation) > (select a slow query) > Trace details:** Here is an example of a slow query trace identifying a specific database and instance.
    </figcaption>
  </Collapser>
