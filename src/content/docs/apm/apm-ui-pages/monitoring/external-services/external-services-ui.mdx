...
The UI for external services is a great place to analyze a single service along with its upstream and downstream services. The UI starts you off with a map of your selected service, along with performance metrics showing the top five results for response time, call count, and error count. If you prefer, you can also see the same map details represented in a table format.
...
### Drill down in map for transaction details [#drill-down]
...
## The external services table view [#table-view]

As a companion to the map view, the table view lists all the map services in a table format. You can click on specific entities (services) to see transactions in the drill-down tables.

The tables display percentage change for each of the three performance metrics, and you can click on **Traces** to switch over to distributed tracing details.
