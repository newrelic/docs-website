...
![Screenshot showing the opening map for external services](src/images/intro-example.png 'Screenshot showing the opening map for external services')
...
![Screenshot showing the table version of the external services map](src/images/table-view.png 'Screenshot showing the table version of the external services map')
...
3. View a breakdown of transactions between the two services.
   ![Screenshot showing calls to various endpoints in downstream service](src/images/downstream-endpoints.png 'Screenshot showing calls to various endpoints in downstream service')
   <figcaption>
     In this example, one of the thicker edges (lines) goes from the
     Order-Composer service to the warehouse endpoint in the Order Status
     service.
   </figcaption>
4. If you decide that a particular transaction is taking the most time, click on that transaction to focus specifically on its dependencies.
   ![Screenshot showing the transaction between the Order-Composer service and the warehouse endpoint in the Order-Status service](src/images/specific-transaction.png 'Screenshot showing the transaction between the Order-Composer service and the warehouse endpoint in the Order-Status service')
   <figcaption>
     In this drill-down view, you can see the transaction between the
     Order-Composer service and the warehouse endpoint in the Order-Status
     service.
   </figcaption>
...
6. If you reach a point in the drill-down where you want to see distributed tracing, click **List** in the upper right, and then click **Traces** in the table.
   ![Screenshot showing the trace link in the table view](src/images/tracing-link-in-table.png 'Screenshot showing the trace link in the table view')
