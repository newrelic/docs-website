...
In APM, when [transaction traces](/docs/apm/transactions/transaction-traces/introduction-transaction-traces) are collected, there may be additional <DoNotTranslate>**Slow query**</DoNotTranslate> data available on the [<DoNotTranslate>**Databases**</DoNotTranslate>](/docs/apm/applications-menu/monitoring/databases-slow-queries-page) page. Let's look at what slow query data is, where to find it, and how to configure it.

## Sampling of slow query data [#about]

When [transaction traces](/docs/apm/transactions/transaction-traces/introduction-transaction-traces) are reported, the slow queries feature samples the slowest database queries in those traces and reports data about them on the [<DoNotTranslate>**Databases**</DoNotTranslate>](/docs/apm/applications-menu/monitoring/databases-slow-queries-page) page. Slow query data can include:
...
You can view slow data right in our APM UI or you can use our GraphQL API to show the details.

<Callout variant="tip">
  Data about slow database queries isn't available via NRQL.
</Callout>

### Look in the UI [#view-in-ui]

To view your slow query data:
...
1. Select an app.
1. Under the <DoNotTranslate>**Monitor**</DoNotTranslate> section, click <DoNotTranslate>**Databases**</DoNotTranslate>.
...
### Use the GraphQL API [#view-in-graphiql]

If you'd like a more programmatic approach, you could make calls to our GraphQL API. For a quick way to try out some queries, use our GraphiQL tool at <DoNotTranslate>[one.newrelic.com > All capabilities > Apps > NerdGraph API explorer](https://one.newrelic.com/nerdgraph-graphiql)</DoNotTranslate>. 

Here's a sample query to get you started. It has several variables that you can pass in by clicking **Variables** at the bottom of the NerdGraph screen: 

* `$entityGuid`
* `$databaseMetricNamePattern`
* `$startTimeMin`
* `$startTimeMax` 
    ...
    entity(guid: $entityGuid) {
      ... on ApmApplicationEntity {
        sqlTraces(
          query: {
            limit: 100
            orderBy: { field: MAX_CALL_TIME, direction: DESC }
            databaseMetricNamePattern: $databaseMetricNamePattern
