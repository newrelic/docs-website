...
The Kubernetes summary page works with a mix of instrumentation providers (New Relic and OpenTelemetry). However, if Kubernetes uses OpenTelemetry and APM uses New Relic, install <Link to="https://github.com/newrelic/k8s-metadata-injection/tree/main/charts/nri-metadata-injection">nri-metadata-injection</Link> for it to work properly by providing the necessary Kubernetes metadata to the APM metrics.
</Callout>
