...
The Kubernetes summary page works with a mix of instrumentation providers (New Relic and OpenTelemetry). However, if Kubernetes uses OpenTelemetry and APM uses New Relic, install `nri-metadata-injection` for it to work properly by providing the necessary Kubernetes metadata to the APM metrics.
</Callout>
