...
### Hybrid scenarios [#hybrid]

When the applications are monitored by New Relic but the K8s cluster is monitored by OpenTelemetry, you will require to install [nri-metadata-injection](https://github.com/newrelic/k8s-metadata-injection/tree/main/charts/nri-metadata-injection), to ensure it works properly by providing the necessary Kubernetes metadata to the APM metrics.

In the other possible hybrid scenario, applications monitored by OpenTelemetry and the K8s cluster monitored by New Relic agents, [nri-metadata-injection](https://github.com/newrelic/k8s-metadata-injection/tree/main/charts/nri-metadata-injection) is already included by default as part of our [nri-bundle] (https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) as explained in the previous [section](https://docs.newrelic.com/docs/apm/apm-ui-pages/monitoring/kubernetes-summary-page/#nr-proprietary)
