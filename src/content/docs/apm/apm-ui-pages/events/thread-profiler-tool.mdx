...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Events > Thread profiler**.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Events > Thread profiler**: Use this page to define the settings for the thread profiler duration and to view the results.
</figcaption>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            The thread profiler depends on the ability to capture thread backtraces from within your Ruby application. For this reason, it requires **CRuby 1.9.2 or higher** (for the `Thread#backtrace` method).
          </td>
        </tr>
