...
After a deployment is recorded, using the REST API or the newer [GraphQL API](/docs/change-tracking/change-tracking-introduction), you can notify your team members using a webhook.

While you can send deployment data to any webhook destination, here are the steps to set this up with Slack:

1. Log in to your Slack account as an admin, and then go to **Apps**.
2. Search for **New Relic Alerts**, and when you see that tile, click on it.
