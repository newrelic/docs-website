...
<CollapserGroup>
  ...
  >
    ...
    The following queries are the ones New Relic uses to generate the numbers in your weekly performance report email. They rely on APM timeslice data exposed through `newrelic.timeslice.value` on the `Metric` event, so you can run them in the [query builder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) or pin them to a [dashboard](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards) to reproduce any value in the email.

    Replace the placeholders with values for your application before running a query:
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            The APM application name (matches `appName` in your data).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            The entity GUID of the APM application (matches `entity.guid`).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            The numeric application ID used by browser events (matches `appId` on `PageView`, `JavaScriptError`, and `BrowserInteraction`).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            The name of a [Synthetic monitor](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring) configured for the application.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ISO 8601 timestamps for the start and end of the reporting week, at Monday `00:00` UTC (for example, `'2026-04-20T00:00:00Z'` and `'2026-04-27T00:00:00Z'`).
          </td>
        </tr>
    ...
    **Request queue time (ms)** — Average time requests spent waiting in the web front-end queue, in milliseconds.

    ```sql
    SELECT average(newrelic.timeslice.value) * 1000 AS 'Request Queue Time ms'
    FROM Metric
    WHERE metricTimesliceName = 'WebFrontend/QueueTime'
      AND entity.guid = '<APP_GUID>' AND appName = '<app_name>'
    SINCE '<monday>' UNTIL '<next_monday>'
    ```

    **Availability (%)** — The percentage of Synthetic monitor checks that succeeded during the week.
    ...
    <Callout variant="important">
      The Availability query uses `SyntheticCheck`, not `Metric`. The original APM "Availability" metric is a legacy feature; the weekly report now derives availability from a [Synthetic monitor's](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring) success rate. Replace `<MONITOR_NAME>` with the name of the monitor you've configured for the application.
    </Callout>
    ...
    **Throughput — app-server apps (rpm)**
    ...
    **Throughput — browser-instrumented apps (ppm)**
    ...
    **Page views** — Weekly count of `PageView` events.
    ...
    **JavaScript errors** — Weekly count of `JavaScriptError` events.

    ```sql
    SELECT count(*) AS 'JS Errors'
    FROM JavaScriptError
    WHERE appId = '<APP_ID>'
    SINCE {N} weeks ago TIMESERIES 1 week
    ```

    **Browser interactions** — Weekly count of `BrowserInteraction` events (route changes and custom interactions in single-page apps).
