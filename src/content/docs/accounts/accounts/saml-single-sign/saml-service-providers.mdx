...
Depending on your [subscription level](https://newrelic.com/pricing), you may have the option to put your domain name(s) on an allow list (whitelist), which will streamline the SAML SSO enablement process. When your Owner or Admins add new users and those users' email address has a domain that matches your claimed domain, New Relic automatically adds them as **Active** users and retains their previous user type. 
...
1. Optional but recommended: read about [an option for claiming your domain to streamline the SAML SSO process]](#domain-allow-list).  
