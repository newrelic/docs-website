...
As an additional security measure for SAML Single Sign On accounts, users are not added until they complete the email confirmation that New Relic sends automatically. Users in the pending state (not yet confirmed) will not receive notifications, such as alerts.

For accounts **without** SAML SSO integration, the account Owner and Admins can add new users without requiring confirmation.

<Callout variant="tip">
  Access to this feature depends on your [subscription level](https://newrelic.com/application-monitoring/pricing).
</Callout>
