...
* Some identity providers have a `suspended` user state. We don't support that state. `suspended` users remain provisioned in New Relic and that state may result in unexpected New Relic behavior. 
