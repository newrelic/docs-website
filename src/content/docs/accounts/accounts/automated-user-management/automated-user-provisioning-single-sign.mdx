...
* [User model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model#user-models)-related requirements:
  ...
  * Implementing AUM requires the [**Authentication domain manager** and the **Organization manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#standard-roles) (both available with the [default group **Admin**](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#groups)).
...
5. Optional: Whether you use one of the dedicated apps (Azure AD, Okta, OneLogin) or not, you have the option to manage user type via our SCIM API. For more, see [Manage user type](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type). 
6. **Highly recommended**: Set a time zone for your users in your identity provider. How you do this will vary by identity provider. If not set in your identity provider, our UI shows UTC time zone dates/times. Time zone is specified in IANA Time Zone database format, also known as the "Olson" time zone database format (e.g., "America/Los_Angeles").
