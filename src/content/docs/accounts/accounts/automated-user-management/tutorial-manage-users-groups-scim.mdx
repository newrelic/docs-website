...
Once your SCIM API integration is complete, all users brought into New Relic start out as full users, which are billable on the New Relic One pricing plan. You can use our default method for managing [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type), which is using the [User management UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where). Optionally, you can use our SCIM API instead. To do this, you can set update your authentication domain configuration to [Delegate control of user type](#TODO) to your identity provider or custom application.
