...
Before starting this tutorial, please read the [requirements for automated user management](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) and for [the SCIM API](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management). 

Note that the API does **not** give you the ability to manage access grants (assigning your New Relic users access to accounts and roles). This means that after you're done creating an integration with this API, you must next use the **Organization and access** UI to create access grants. For more information, see [user management concepts](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts).
...
* [Our primary SCIM API reference](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management/)
...
* `emails` The single email address of the user (despite it being called `emails`, only one is accepted).
* `active` Boolean indicating whether or not the user should be active or inactive within New Relic.

It's recomended to provide the following attributes for the best user experience:
...
In this tutorial we'll search for specific users and groups, but that's not the only way to view users and groups. Refer to the [SCIM  API reference](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management/) and [RFC 7644](https://datatracker.ietf.org/doc/html/rfc7644) for all the available query options.
...
By default, a user's [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) (basic user or full user) can only be viewed and edited in the [User management UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where), but alternatively you can manage this via the SCIM API using the [user type schema](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management/#user-type). You can do this either as part of using the SCIM API for your entire automated user management implementation, or if you want to supplement your use of the [identity provider New Relic apps](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign). If your [authentication domain is not already enabled for SCIM](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#source-users), you'll need to enable it.
