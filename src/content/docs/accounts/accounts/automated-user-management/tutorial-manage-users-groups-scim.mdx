...
This tutorial will walk you through some common procedures for managing users on the [New Relic One user model](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) using the SCIM API. The SCIM API allows you to view, create, update, and delete users and groups programmatically, outside of the [User management UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where).
