...
1. Go to the New Relic [authentication domain UI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#manage-user-type-scim) and click **Enable Manage user type with SCIM**. Note that when this is enabled, you can’t manage user type from the New Relic UI and can only manage it from Okta or via our [SCIM API](https://docs.newrelic.com/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim/).

2. Go into your Okta instance. The rest of these instructions are done from Okta.

3. Navigate to your Okta SSO application for New Relic, select the **Provision tab** and ensure that **Provision to App: Update User Attributes** is set to **Enable**:

<img
     title="Update User Attribute: Enable"
     alt="set Update User Attribute to Enable"
     src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyDy}
   />

4. On that same page, scroll down to the SSO application's Attribute Mappings and select go to profile editor:

<img
     title="Profile Editor"
     alt="navigate to Profile Editor"
     src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyE8}
   />

5. On the **Profile Editor** for the SSO Application, scroll down to **attributes** and select **add attribute**:
  In the form you will set the following:
  * display name -- this can be any name that you want
  * variable name -- this can also be any value you want but you'll need to call this variable later in the user profile
  * external name -- New Relic *requires* this to be `nrUserType`
  * external namespace -- New Relic *requires* this to be `urn:ietf:params:scim:schemas:extension:newrelic:2.0:User`

   <img
     title="Adding new attribute in Okta app"
     alt="Adding new attribute in Okta app"
     src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyED}
   />

 All other fields can be left blank or set to your desired specifications. Select **Save**.

6. Next, you'll configure your Okta **user profile** to have this field:
   * In the **Profile editor**, go to **Users** and click the **User (default)** profile.
   * Select **Add Attribute** and fill out the following fields:
      * display name -- this can be anything you want.
      * Variable name -- this will be the same variable name you set in the previous attribute

<img title="Set Okta user profile to have new attribute" alt="Set Okta user profile to have new attribute" src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyEX}/>

All other fields can be left blank or set to your desired specifications. Select **Save**

Note that this is merely *one way* to map attributes in Okta and how you set this will depend on your own setup and preferences for defining user type. For more on this, please     ([see Okta user profile instructions](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas)).
     

7. Navigate to **directory > users** and select a user to set their user type. Select **profile** and select **edit** to now set this user's new *nrUserType* attribute. 

<img title="set user type" alt="set user type" src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyEh}/>
<img title="set user type" alt="set user type" src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyEm}/>

Valid user types you can pass to New Relic are: `Full User`, `Core User` and `Basic User`. Select **Save**.

Keep in mind this is merely one way of setting the user type for your users and how you do this will depend on your setup and preferences. For example, you may choose to set this manually by setting each user’s user type as above, or you may use Okta to manage these in bulk.

8. Navigate back to your SSO application > **provisioning** tab and scroll down to the **Attribute Mappings**. Select **show unmapped attributes**. You should see your new `nrUserType` attribute here. Select the **pencil icon** to map this attribute.

<img title="map new attrobute" alt="smapping the new nrUserType atrribute" src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyEw}/>

  Set the **attribute value** to **map from okta profile** (unless you're choosing to map the user type differently for your organization. See [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type). [Learn more about Okta attribute mappings.](https://help.okta.com/en/prod/Content/Topics/users-groups-profiles/usgp-about-attribute-mappings.htm)). Set the **attribute** to the new custom *nrUserType* string. Select a user in the preview window to see a preview of the mapping and ensure it displays the user type you just specified for your user. If the preview cites *null* something may be wrong with the mapping. Set the **apply on* radio button to either **Create** (this will *only* update the user type when the user is created in New Relic) or **Create and Update** (this will update the user type both when created or updated in Okta). If it looks correct, select save.

<img title="map attribute" alt="map attribute" src={https://knowledge.newrelic.com/servlet/rtaImage?eid=ka08W000000T1qG&feoid=00N8W00000P7BGm&refid=0EM8W00000BqyF6}/>

9. If the user you want to update already exists in New Relic, select **force sync** on the **provisioning > attribute mappings** page. If this user does not exist yet, navigate to the **assignments tab** and assign them to this SSO application (this will provision the user and include the new attribute you just set).
