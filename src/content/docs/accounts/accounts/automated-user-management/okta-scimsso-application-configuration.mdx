...
![Adding new attribute in Okta app](src/images/okta-define-user-type.png 'Adding new attribute in Okta app')

4. Next, you'll configure your Okta user profile to have this field. Steps:
   _ In the **Profile editor**, go to **Users** and click the **User (default)** profile.
   _ Add a new **New Relic user type** attribute to that profile ([see Okta user profile instructions](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas)). How you set this will depend on your own setup and preferences for defining user type. Note that the expected values for user type are `Basic user`, `Core user`, and `Full user`. Below is an example with information filled in.
   ![Set Okta user profile to have new attribute](src/images/okta-add-user-type-to-profile.png 'Set Okta user profile to have new attribute')
