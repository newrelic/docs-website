...
* Manage user type from New Relic using the [User management UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#add-users), OR
* Manage user type from Okta (described below). 

To manage your users' user type from Okta: 

1. Go to the [authentication domain UI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#manage-user-type-scim) and click **Enable Manage user type with SCIM**. Note that when this is enabled, you can’t manage user type from the New Relic UI and can only manage it from Okta.
2. Next, you'll configure Okta to be able to send a new attribute `nrUserType`. Steps: 
  ...
  ![Adding new attribute in Okta app](./images/Master-Sub_Hierarchy.png "Adding new attribute in Okta app")

3. Next, you'll configure your Okta user profile to have this field. Steps: 
	* In the **Profile editor**, go to **Users** and click the **User (default)** profile. 
	* Add a new **New Relic user type** attribute to that profile ([see Okta user profile instructions](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas)). How you set this will depend on your own setup and preferences for defining user type (e.g., using an enumerated list). Note that the expected values for user type are `Basic user` and `Full user`. Below is an example with information filled in. 
  ![Set your user profile to have the new attribute](./images/new-relic-okta-app-add-user-type.png "Set your user profile to have the new attribute")
4. In the **People** section, define the user type for your users. How you do this will depend on your setup and preferences. For example, you may choose to set this manually by setting each user’s user type, or you may use Okta to manage these in bulk.5. Next, you’ll set up mapping for that attribute. Steps: 
