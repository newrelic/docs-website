...
For users on our [original user model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model#user-models): how user roles and permissions work on this model, and how to add and manage users.
...
If you're on the New Relic One pricing model, see some [important considerations](#full-user-count).
...
For tips on editing user type, see [Manage user type](#update-user-type).
  ...
  >
    ...
    * If you change a user's type to a higher type (either via the UI or via an upgrade request email), their user type is changed to that higher user type across all accounts in the organization. 

To update your users' user type:
    1. For the account you want to manage, go to: **[account dropdown](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) > Account settings > Users and roles > Users**.
    2. Either select a user and edit their type or [bulk update the type for multiple users](#bulk).
    3. If your organization has multiple accounts, you won't be able to see all your organization's users in a single UI location. You'll have to go to the **Users and roles** UI for each account. (Note that the ability to more easily manage your users is one benefit of our [newer user model](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models).)
...
The user type is a billing factor only for organizations on our [New Relic One pricing model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models). 
...
* **Custom add-on roles** can grant feature-specific permissions across different New Relic products. For example, a group of [Users](/docs/accounts-partnerships/accounts/account-billing-usage/users-roles#roles) could have the ability to acknowledge [incidents](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/acknowledge-alert-incidents) and [close violations](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/close-violations-manually) in [alerts](/docs/alerts), but not have the ability to modify your existing alert [preferences](/docs/alerts/new-relic-alerts/configuring-alert-policies/specify-when-new-relic-creates-incidents).
...
## Add-on roles [#add-on-roles]

![Original user model add-on roles UI](./images/original-user-model-roles-ui.png "Original user model add-on roles UI")
<figcaption>A view of the capabilities UI for the alerts manager</figcaption>

The Owner and Admin roles have all capabilities. We also have add-on manager roles that have capabilities related to certain areas. You can assign these add-on manager roles to Users and Restricted Users. 
...
To understand the capabilities that a role has, go to: **[account dropdown](/docs/glossary/glossary/#account-dropdown) > Account settings > Users and roles > Roles** and select a specific role. You can hover over specific capabilities to learn more details. 

To create a custom role with custom capabilities, go to **[account dropdown](/docs/glossary/glossary/#account-dropdown) > Account settings > Users and roles > Roles** and click **New custom add-on role**. 
...
![Screenshot of roles related to applied intelligence and incident intelligence](./images/original-user-model-ai-ii-capabilities.png "Screenshot of roles related to applied intelligence and incident intelligence")
