...
metaDescription: >-
  "For New Relic's original user model: explanation of how to manage users, and how user roles work."
redirects:
...
<CollapserGroup>
  ...
  <Collapser
    id="update-user-type"
    title="Manage user type (basic vs full)"
  >
    ...
    When basic users attempt to upgrade to full users, an upgrade request is sent to all admins.

    For more about our user types, see [User type](#user-type).
  </Collapser>
...
* **Basic user**. Details:
  ...
  * Basic users can request to become full users in the UI. They cannot self-upgrade. Basic users will see prompts when attempting to access unavailable features. Requests to upgrade are sent to all admins on that account. 
