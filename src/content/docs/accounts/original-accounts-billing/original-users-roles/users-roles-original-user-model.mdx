...
/>
<figcaption>If a user's email is associated with more than one login, they'll see a "multiple accounts found" note when logging in.</figcaption>
...
<CollapserGroup>
  ...
  >
    ...
    * If you upgrade a user to a higher user type (either via the UI or via an upgrade request email), their user type is changed across all accounts in the organization.
    ...
    2. Click **Upgrade requests**. Note that if your organization has multiple accounts, you won't be able to see all your users in a single UI location but will have to go to the **Users and roles** UI for each account.
