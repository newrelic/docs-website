...
If your New Relic account has users on our [original user model](#who-is-doc-for), you can use the **Users and roles** UI. To find the **Users and roles** UI: click the [account dropdown](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown), click **Account settings**, and then click **Users and roles**.

Some tips for managing users:
* For organizations with multiple [parent accounts](/docs/accounts/original-accounts-billing/original-users-roles/parent-child-account-structure): you won't be able to see all your users in a single UI. You'll have to go to the **Users and roles** UI for each parent account.
* Some features in the UI are visible only to account Owners and Admins.
* You can use the [New Relic REST API](/docs/apis/rest-api-v2/account-examples-v2/listing-users-your-account) to obtain a list of users and roles for an account.

Here are some specific tips for common user management tasks:
...
<CollapserGroup>
  ...
  >
    ...
    Note that billing-related aspects of your user count only apply if you're on [New Relic One pricing](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models) (and not the [original pricing model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models)). 

    To update a user's [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type):

    1. Go to: **[account dropdown](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) > Account settings > Users and roles > Users**.
    2. Either select a user and edit their type or [bulk update the type for multiple users](#bulk).
    3. If you have multiple [parent accounts](/docs/accounts/original-accounts-billing/original-users-roles/parent-child-account-structure), you won't be able to see all the users in your organization in one UI location. You'll have to go to the **Users and roles** UI for each parent account.
    ...
    * **Require review**: With this option, your admins get a notification when users request an upgrade and the admins must upgrade them first. They can be approved either from the notification email or from the user's record in the **Users and roles** UI.
