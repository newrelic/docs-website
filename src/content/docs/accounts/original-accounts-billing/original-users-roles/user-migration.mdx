...
The new user records created for your users have the same login credentials: there's no need to reset passwords. If a user has a pending email verification status (pending being verified), that will also be transitioned over. For your users currently logged in to New Relic, once you complete this step, they'll be logged out of New Relic when their browser session expires. 
