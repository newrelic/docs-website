...
* You must be on our original user model. The user migration process is what converts your users to the New Relic One user model. 
* You must have the [Owner role](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#roles) to use the user migration wizard. 
...
* If your organization has more than 20 accounts, we recommend **not** using the user migration wizard unless you're getting help from a New Relic representative. (Note that this refers to having 20 or more accounts, **not** users.)
* If you frequently [add accounts](/docs/accounts/accounts-billing/account-structure/mastersub-account-structure/#creating) to your organization, we recommend **not** using the wizard yet. Currently, users on the new user model require New Relic assistance to add accounts. 
* We have some changes in functionality pertaining to alerts that may affect some customers. [Read about user model impacts.](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#limitations)
* Roles related to user management (ability to add and update users, change user type, create access grants) currently apply across an organization and can't be assigned at an account level. If you require account-level user management role assignments, we recommend not using the wizard. 
* For customers using SAML SSO and/or SCIM: If you were previously using one of the New Relic-specific Azure, Okta, or OneLogin apps, you'll need to use a different app made for the new user model. This is described in more detail in the instructions but we mention this here so you know that this can add work to the user migration. 
