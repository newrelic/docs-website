...
If your New Relic organization is a [customer partnership](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partnership-accounts-users-subscriptions) (a complex multi-tenant structure) and you were migrated to the new user model, and you're consuming data from `NrConsumption`, `NrMTDConsumption` or `NrDailyConsumption`, you may see an increase in data ingest in NRQL queries you've created. Your ingested data hasn't actually changed but due to the migration, you may experience some discrepancies.

For details about how to create working queries, see [Usage queries](src/content/docs/accounts/accounts-billing/new-relic-one-pricing-billing/usage-queries-alerts#multitenant-ingest).
