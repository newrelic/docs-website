...
We have some recommendations below that apply to organizations who choose to self-serve. Note that **these caveats don't apply if you're being helped by a New Relic representative**. 

Before using [the user migration wizard](#find), review these recommendations:

* If your organization has more than 20 accounts, we recommend **not** using the user migration wizard, unless you're getting help from a New Relic representative. (Note that this refers to having 20 or more accounts in an organization, not users.)
* If you frequently [add accounts](/docs/accounts/accounts-billing/account-structure/mastersub-account-structure/#creating) to your organization, we recommend **not** using the wizard yet. Currently, users on the new user model require our assistance to add accounts. 
