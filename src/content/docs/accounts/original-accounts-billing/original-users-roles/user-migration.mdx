...
* If you have admin-level roles assigned but get an error message when trying to access New Relic features, it may be because you've been assigned [admin settings](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings) (**Organization settings** and/or **Authentication domain settings**) but not any roles. To access New Relic features in a specific account, you'll need at least one role (for example, **All product admin** or a custom role).
...
### Multi-tenant organization migration [#multi-tenant]

If your New Relic organization was a [customer partnership](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partnership-accounts-users-subscriptions) (a complex multi-tenant structure) and you were migrated to the new user model, you may see a seeming increase in data ingest in your custom dashboards. Your ingested data does not change but due to a change in usage tracking, you may experience some discrepencies. 

To resolve this, you'll need to adjust any queries of ingested data to more accurately reference your organization type. Here's an example of an updated query. The addition of the `version`, `customerStructure`, and `consumingAccountId` attributes are what are needed to make your query accurate.  

```
FROM NrConsumption
SELECT rate(sum(GigabytesIngested), 1 day) 
AS avgGbIngestTimeseries 
WHERE productLine = 'DataPlatform' and (version = '0.4.2' or customerStructure='customer_contract') and consumingAccountId is not null 
FACET usageMetric,version 
LIMIT MAX 
TIMESERIES AUTO 
SINCE 30 days AGO
```
