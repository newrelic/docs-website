...
Before using [the user migration wizard](#find), please review the guidelines below. 
...
* If your organization has more than 20 accounts, we recommend **not** using the user migration wizard. **You can disregard this requirement if you're being helped by New Relic representatives.** (Note that this refers to having 20 or more accounts in your [organization](/docs/accounts/accounts-billing/account-structure/new-relic-account-structure/#new-model), not users.)
...
* Roles related to user management (ability to add and update users, change user type, create access grants) currently apply for an entire organization and can't be assigned at an account level. If you require account-level user management role assignments, we recommend not using the wizard. 
* For customers using SAML SSO or SCIM: If you were previously using one of the New Relic-specific Azure, Okta, or OneLogin apps, note that you'll need to use a newer app version titled "New Relic by organization" (the previous one was titled "New Relic by account").

If you don't meet these requirements but still want to migrate your users in order to take advantage of the new model benefits, talk to your New Relic account representative. 
