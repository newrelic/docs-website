...
During the user migration procedure, you'll have a choice to either grant all your users complete access to all your organization's accounts, or to customize which roles and accounts your users have access to (aka, assign **access grants**). 
...
* If you've already used the wizard to set up an admin on the new user model, have the admin sign in using their new user record to access the migration tool. The user migration wizard, when completed, destroys the old user record, but if you've started the user migration process without completing it, you may have users with access to both the original and new record, as shown below: 
    ...
    <figcaption>
	  If a user on the new model has been created and the migration process hasn't been completed, they may have access to the original user record and the new login(s). 
    </figcaption>
* If you plan on migrating only a portion of your users to the new user model to start, we recommend leaving some original model admins so that you have an admin to manage the original model users. 
...
* Once you complete this step and create new user records, we highly recommend completing the migration process without delay. If you don't complete the steps to migrate assets and delete the original user record, a user may see two user records when logging in (see image below) or else may be missing assets they expect to see (like dashboards). 

 ![A screenshot of what is shown when you have an email address associated with multiple New Relic logins](./images/login-multiple-accounts-view.png "Login UI for email addresses with multiple user records")

    <figcaption>
If a user with access to multiple organizations is not migrated for all organizations, they'll retain an "original" user record, in addition to the new record created. When logging in, they'll have a choice of which to log into. 
    </figcaption>
...
This step allows you to determine the level of access their users have. If you need to map user access to specific accounts and roles, then you'll want to set up groups and access grants at this stage. 

You'll need to create an access grant for each account that they want a group to have access to. For example, a DevOps group might have five different access grants, each giving the group access to a different account.  

This is a procedure done separately from the user migration wizard procedure. For more on how to create access grants, see the [user management tutorial](/docs/accounts/accounts-billing/new-relic-one-user-management/tutorial-add-new-user-groups-roles-new-relic-one-user-model/#group-access).
...
If a user has access to several organizations (for example, if that user is a contractor), the user won't be fully deleted until all organizations migrate their users. It's possible that such a user will find themselves with both the original user record and one or more new user records (see image below). 
