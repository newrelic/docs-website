...
<CollapserGroup>
  <Collapser
    id="manual-provision"
    title="Add and manage users from the New Relic UI"
  >
...
* If using username/password for your users' login, they'll have the same login credentials on the new user record created: there's no need to reset passwords. If a user has a pending email verification status (pending being verified), that will also be transitioned over. 
...
In a previous step (step #4), you set up your identity provider to provision your users in New Relic. This means you don't have to upload your users for this step. Note that you won't be able to edit users or groups in New Relic: all user and group changes must be done from your identity provider. 
