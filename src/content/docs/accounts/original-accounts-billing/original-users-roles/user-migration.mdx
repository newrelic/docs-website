...
**Recommended**: Download the full list of existing original user model users before choosing to import users. This will be a useful resource and serve as a backup, if you need it. You can also use this list ot help you figure out which users have access to which accounts, and also help you create groups and access grants for a later step. 

There are two ways of provisioning users with the New Relic One user model:
 * Manually adding users using the New Relic UI
 * Automated provisioning users using your Identity provider(SCIM)  
...
**Automated provisioning users using your Identity provider(SCIM)**
Once users and the associated groups are assigned to the New Relic SCIM setup that was done in the prior step, the users will be automatically reflected as New Relic one users. You will not be able to edit the users or the groups in New Relic and all user or group changes will have to be done in the identity provider. 


If using username/password for your users, they will have the same login credentials on the new user record created: there's no need to reset passwords. If a user has a pending email verification status(pending being verified), that will also be transitioned over. 
