...
Before using [the user migration wizard](#find), please review the guidelines below. The wizard was initially created for fairly simple New Relic organizations. We'll continue to add to the user migration features so that it works better for more complex organizations. 
...
* If your organization has more than 20 accounts in it, we recommend not using the wizard and waiting until we offer more multi-account functionality.  
* If your organization has a fairly complex parent/sub-account structure, or if your a [partnership accounts](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partnership-accounts-users-subscriptions/#partnerships) or use our [Partnership API](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partner-api/), we recommend **not** using the wizard and waiting until we have more functionality.  
* If you frequently add accounts to your organization, we recommend **not** using the wizard. On the new model, there isn't yet a way to add accounts on your own. 
* Currently roles related to user management (ability to add and update users, change user type, create access grants) are applied for an entire organization and can't be assigned at the account level. If you require account-level user management role assignment, we recommend not using the wizard. 

If you don't meet these requirements but want to migrate your users in order to take advantage of the benefits, contact your New Relic account representative. 
