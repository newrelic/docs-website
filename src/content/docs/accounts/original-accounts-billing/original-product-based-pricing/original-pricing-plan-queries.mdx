...
On April 18, 2023, the usage UI for our original pricing model reaches end-of-life (EoL). In this doc are NRQL queries that customers on the original pricing model can use to understand their usage. 

## What customers are affected? [#who-is-affected]

This affects New Relic organizations on our [original pricing model](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing).
...
* Querying with the [query builder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) in the UI will query in a specific account, and any child accounts. IS THAT ACCURATE?? You can also [use NerdGraph for cross-account querying](/docs/apis/nerdgraph/examples/nerdgraph-nrql-tutorial/#cross-account-query). 
