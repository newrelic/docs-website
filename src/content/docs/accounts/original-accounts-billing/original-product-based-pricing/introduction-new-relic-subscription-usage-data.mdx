...
<CollapserGroup>
  ...
  >
    For accounts on our [original pricing model](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing), our usage UI lets you predictably track your data usage, and see how that may affect your pricing. This document explains how New Relic calculates billable usage for <InlinePopover type="apm" /> accounts that have CU-based pricing (not host-based pricing), and also provides available subscription usage attributes and [example NRQL queries](#query-examples) to use in the New Relic UI or with API calls.

    ### Data generation [#details]

    Once per day, an <InlinePopover type="apm" /> account will generate an `NrDailyUsage` [event](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) for:
    ...
    * Every unique host on which an application instance existed over the last 24 hours
    ...
    3. Multiply the **Avg daily compute units** by the number of days in the current month.
       ...
       Some previous <InlinePopover type="apm" /> agents may miscount containers as hosts, which may lead to over-reporting of compute unit (CU) usage.
    ...
    * Every unique host on which an application instance ran during the last 24 hours
    ...
    3. See the **Avg daily equivalent hosts** for an account or grouping of accounts.
       ...
       <CollapserGroup>
         ...
         >
           This query returns data once provided in the "host usage report": a listing of applications running on each host. Note that while <InlinePopover type="apm" /> usage is counted on an hour-by-hour basis, per-hour usage data is no longer available via the API or UI.
       ...
       Some previous New Relic <InlinePopover type="apm" /> agents may miscount containers as hosts, which may lead to over-reporting of host-based usage.
