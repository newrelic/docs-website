...
If your New Relic organization is a multi-tenant organization (an organization with sub-organizations) and you're consuming data from `NrConsumption`, `NrMTDConsumption` or `NrDailyConsumption`, you may see an increase in data ingest in custom NRQL queries that you've created. Your ingested data has **not** changed but due to your migration to our new user model, you may experience some discrepancies.

To get accurate usage, you must include some specific attributes in order to get accurate usage results. In the example query below, use the `WHERE` clause exactly as it's shown. 
