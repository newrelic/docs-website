...
For how to determine an estimate of cost, see [Estimated cost queries](#estimated-cost).
  ...
  <Collapser
    id="multitenant-ingest"
    title="Multi-tenant organization usage"
  >

If your New Relic organization is a multi-tenant organization (an organization with sub-organizations) and you're consuming data from `NrConsumption`, `NrMTDConsumption` or `NrDailyConsumption`, you may see an increase in data ingest in custom NRQL queries that you've created. Your ingested data has **not** changed but due to some enhancements to our user model, you may experience some discrepancies in your query results.

To get accurate usage, you must include some specific attributes in your usage queries. In the example query below, use the entire `WHERE` clause exactly as it's shown. 
