...
For multi-tenant New Relic organizations (organizations that contain multiple sub-organizations), you must include some specific attributes in order to get accurate usage results. In the example query below, the `version`, `customerStructure`, and `consumingAccountId` attributes and values are required to make the query accurate for multi-tenant organizations.
