...
To return your estimated cost, you'll have to know your organization's per-GB ingested data cost and run a query multiplying that by the data ingested, like this:

```
FROM NrMTDConsumption SELECT latest(GigabytesIngestedBillable)*<var>YOUR_PER_GB_COST_OF_DATA</var> SINCE this month
```

For tips on determining costs for billable users, see [User count queries](#user-queries).
...
```
SELECT latest(FullPlatformUsersBillable)* <var>COST_PER_FULL_PLATFORM_USER</var> FROM NrMTDConsumption
```

Here's one for core users: 

```
SELECT latest(CoreUsersBillable)* <var>COST_PER_CORE_USER</var> FROM NrMTDConsumption
```
...
```
SELECT latest(FullUsersBillable)* <var>COST_PER_FULL_PLATFORM_USER</var> FROM NrMTDConsumption
```
