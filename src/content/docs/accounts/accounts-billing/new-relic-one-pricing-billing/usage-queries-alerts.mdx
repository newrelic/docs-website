...
<CollapserGroup>
  ...
  >
In May of 2022, we're removing the `estimatedCost` attribute ([learn more](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-usage-removal)). If you are using this attribute, we recommend adjusting your queries using the guidance below.

This query shows the original query with `estimatedCost` to determine the :
