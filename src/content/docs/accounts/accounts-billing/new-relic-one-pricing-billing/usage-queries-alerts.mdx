...
The [usage UI](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/billing-ui) displays your user count over time. If you need more detail than the UI provides, you can use the example NRQL queries below. For details about how users are calculated, see [Billable user calculations](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/user-count-billing).

We have two versions of New Relic One pricing, and the version you're on impacts how your billable users are calculated, and affects your NRQL query format: 

* New Relic One pricing **without core users** (with only basic users and full platform users): This is the primary version of our New Relic One pricing model. To query your users: 
  * User data is available on all three [usage-related data types](#data-types).
* New Relic One pricing **with core users**: This pricing version is in [limited availability](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/core-users-release). To query your users: 
  * Use only the `NrMTDConsumption` event. 
  * User attributes are: `BasicUsersBillable`, `CoreUsersBillable`, and `FullPlatformUsersBillable`. 
...
If your organization is on the [New Relic One with core users pricing version](#user-queries), here are some example NRQL queries for determining user count: 
