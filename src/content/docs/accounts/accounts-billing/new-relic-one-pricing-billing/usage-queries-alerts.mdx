...
In May of 2022, we deprecated the `estimatedCost` attribute from usage events and removed the current month's estimated cost from the UI ([learn more](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal)). If you currently use the `estimatedCost` attribute in your queries or alert conditions, you should remove them, or replace them with the recommended queries below. 
