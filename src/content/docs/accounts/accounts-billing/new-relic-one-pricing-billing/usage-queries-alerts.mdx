...
```
  ...
  >
In May of 2022, we're removing the `estimatedCost` attribute ([learn more](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal)). If you're using this attribute, we recommend adjusting your queries using the guidance below.

This query shows the original query with `estimatedCost`:
...
Below is the newer query you should use. Note that you'll need to know your organization's per-GB ingested data cost.

```
FROM NrMTDConsumption SELECT latest(GigabytesIngestedBillable)*<var>YOUR_PER_GB_COST</var>
```

For tips about querying costs for billable users, see [User count queries](#user-queries).
...
We have [two versions of the New Relic One pricing model](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/core-users-release/#overview), and the version you're on impacts how your billable users are calculated, and thus affects how you query those users:
...
### Month-to-date costs
To return the cost for your billable users for the current month, you'll have to know your organization's per-core-user and per-full-platform-user cost, and run a query multiplying that by the user count.

Here's one for full platform users: 

```
FROM NrMTDConsumption SELECT latest(FullPlatformUsersBillable)* <var>YOUR_PER_FULL_PLATFORM_USER_COST</var> 
```

Here's one for core users: 
...
### Month-to-date costs
To return the cost so far for that month, you'll have to know your organization's cost per full platform user, and run a query multiplying that by the billable user count, like this:
...
Note that this query applies for organizations with the primary version of New Relic One pricing, with core users. For more about this and about how to query user counts, see [Query users](#user-queries).
  </Collapser>
  ...
  >
In May of 2022, the `estimatedCost` attribute will be deprecated ([learn more](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#estimated-cost-removal)). If you're using that attribute, here's some guidance on replacing that query. 
    
The original query for exceeding a cost threshold looks like: 
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        **This will be removed in May of 2022.** For details, see [Removal of estimated cost](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal). 
