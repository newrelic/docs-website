...
## What are billable users? [#billable-users]
...
Note that our [Standard edition](https://newrelic.com/pricing) comes with one free full platform user.  
...
* Another way to determine your version is by going to the [**Manage your plan** UI](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/billing-ui). If you see "New Relic One - Users", that means you're on the "New Relic One without core users" pricing version.
...
Some Pro and Enterprise edition organizations have access to tiered pricing for billable users (core users and full platform users). The prices and tier levels will vary depending on an organization's specific contract. Here's an example of how tiered pricing might work for an organization's full platform users: 

* Users #1-10 at price x
* Users #21-30 at price z (lower than y)
* Users #11-20 at price y (lower than x)

Note that the billable users in lower tiers remain billable at the cost of their tier, regardless of how many other, less expensive tiers are unlocked for subsequent users. In other words, unlocking less expensive tiers by adding users doesn't change the price of users on the lower tiers. 

In the example above, if the customer has 29 users for the month, then the billable users for the month would be the sum of: a) 10 users multiplied by price x, b) 10 users multiplied by price y, and c) 9 users multiplied by price z. 
