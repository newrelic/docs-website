...
Note that the billable users in lower tiers remain billable at that tier’s cost, regardless of how many other, less expensive tiers are unlocked for subsequent users. In the example above, if the customer has 29 users for the month, then the billable users for the month would be the sum of: a) 10 users multiplied by price x, b) 10 users multiplied by price y, and c) 9 users multiplied by price z. 
