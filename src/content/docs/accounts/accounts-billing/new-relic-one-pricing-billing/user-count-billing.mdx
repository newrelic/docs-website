...
Because the [annual pool of funds plan](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#usage-plans) involves a longer-term commitment, we limit how many times a full platform user can be downgraded. On a monthly level (not within a month), a full platform user can only switch to a lower user type twice over one "contract year" (a year starting at your contract's starting point, or at the anniversary of that point). Once a full platform user has downgraded twice, if they switch back to be a full platform user in a later month, they are billed as a full platform user for the remaining months in that year. 

Here's an example of how that works in practice: Let’s say your organization has a year-long contract that started March 1st. One of your full platform users switches to become a basic user for the month of May, then goes back to being a full platform user for the month of June, and then becomes a basic user for the month of August. That user has reached the two-downgrades-per-contract-year limit. If that user then becomes a billable full platform user again for a subsequent calendar month in that remaining contract year, that user counts as a full platform user for the remaining months in that year, regardless of what their user type is changed to.

Note that the downgrade limit doesn't apply within a month. A user can be switched as many times within a month as needed, but remember that the highest user type a user is set to during a month is considered their [billable user type](#user-count) for that month. 
