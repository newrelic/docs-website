...
<Callout variant="important">
  Publish an internal list of Organization Managers so users know who to contact for limit increases.

  Designate Organization Managers who can promptly approve limit increases for users blocked during a service incident.
...
<Steps>
  ...
  <Step>
    ...
    1. Choose how to allocate budgets:
      - **Percentage of organization budget**: Allocate a percentage of your organization-level compute budget (based on your minimum commitments) to each user. For example, if your organization has 800,000 core CCUs and you set 1% per user, each user receives 8,000 core CCUs. The total percentage across all users doesn't need to equal 100%. You can set the total below 100% (leaving some budget unallocated) or above 100%. Since users typically don't consume their full allocation, setting the aggregate above 100% is acceptable.
...
If a user is blocked due to budget consumption:
  ...
  4. You can increase the budget to restore access within approximately 15 minutes, or
