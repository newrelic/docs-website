...
## Exceeded session and IP address limits [#session-limits]
...
In this context, a session represents a unique login to the New Relic platform. The session is tracked by a token assigned for that session. 
