...
* **Email**: The user's email address. Some notes on user statuses you may see: 
  * `Pending`: This user has been added but hasn't yet logged in for the first time. Even in the `Pending` state, a user is considered provisioned and is therefore billable if they're a core or full platform user. 
