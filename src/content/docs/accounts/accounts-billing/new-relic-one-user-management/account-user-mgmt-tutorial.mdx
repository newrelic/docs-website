...
* <DoNotTranslate>**Admin**</DoNotTranslate>: can use and configure observability features for that initial account, and the ability to view and configure organization-wide administration settings (like adding accounts, managing authentication settings, and adding/removing users).
* <DoNotTranslate>**User**</DoNotTranslate>: can use and configure observability features (but lacks access to the admin abilities that the <DoNotTranslate>**Admin**</DoNotTranslate> group has).

You can see the access assigned for these default groups by going to the <DoNotTranslate>**Access management**</DoNotTranslate> UI:

1. Log in to [one.newrelic.com](https://one.newrelic.com/).
2. Click your user name, then <DoNotTranslate>**Administration > Access Management**</DoNotTranslate>.
...
You can see how the <DoNotTranslate>**User**</DoNotTranslate> group has the <DoNotTranslate>**All product admin**</DoNotTranslate> role and access to that initially created account. And you can see how the <DoNotTranslate>**Admin**</DoNotTranslate> group has some administration settings assigned. The `Default` next to those group names refers to them being in the original, default authentication domain.

If you want to create admins who can only add and remove users, and who don't have the more powerful organization-wide admin capabilities that the <DoNotTranslate>**Admin**</DoNotTranslate> group has, you can use the [<DoNotTranslate>**Group admin**</DoNotTranslate> role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#group-admin). 
...
We have several default-available roles, which we call [standard roles](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles). Some of these are assigned to the <DoNotTranslate>**Admin**</DoNotTranslate> and <DoNotTranslate>**User**</DoNotTranslate> groups that are available by default. 
...
* Users are assigned to groups (for example, the default <DoNotTranslate>**Admin**</DoNotTranslate> and <DoNotTranslate>**User**</DoNotTranslate> groups), and those groups are assigned various roles and accounts. Put another way: it's not the group that gives users access to New Relic permissions: it's the roles. 
...
To view existing roles: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), click <DoNotTranslate>**Administration**</DoNotTranslate>, then click <DoNotTranslate>**Access management**</DoNotTranslate>, and then click <DoNotTranslate>**Roles**</DoNotTranslate>.

To create a custom role, click <DoNotTranslate>**Add new custom role**</DoNotTranslate>. Review the list of available permissions and decide which ones your custom role needs.
...
If you have a relatively flat organizational structure, and are okay with all or many of your users having wide administrative access and access to all accounts, you'll probably only need at most a few extra group configurations. For example, you might decide to add more accounts to the existing default <DoNotTranslate>**Admin**</DoNotTranslate> or <DoNotTranslate>**User**</DoNotTranslate> groups. Or, if you need more granular definition over roles and permissions, you'd create new groups with access to specific roles (either our standard roles or custom-defined roles).
...
To view existing groups: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), click <DoNotTranslate>**Administration**</DoNotTranslate>, and then click <DoNotTranslate>**Access management**</DoNotTranslate>. Under the <DoNotTranslate>**Groups**</DoNotTranslate> tab, you'll see the existing groups and what access they've been assigned.
...
1. From the <DoNotTranslate>**Access management**</DoNotTranslate> UI, click <DoNotTranslate>**Groups**</DoNotTranslate>. 
2. You can create a new group or edit an existing one: 
    * To create a new group, click <DoNotTranslate>**Create new group**</DoNotTranslate> and follow the instructions. 
    * To edit an existing one, click the <Icon name="fe-more-horizontal"/> icon for the group you want to edit. Click <DoNotTranslate>**View group details**</DoNotTranslate> and edit the group's access. 
...
* When selecting from amongst our [standard roles](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles), it's important to understand the difference between <DoNotTranslate>**All product admin**</DoNotTranslate> and <DoNotTranslate>**Standard user**</DoNotTranslate>. <DoNotTranslate>**All product admin**</DoNotTranslate> is a more popular choice because it gives the ability to configure platform features. If you wanted to have your users be able to use platform features but not configure the settings for those features, you'd choose <DoNotTranslate>**Standard user**</DoNotTranslate>.
...
1. To view users and see their groups: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), click <DoNotTranslate>**Administration**</DoNotTranslate>, and click <DoNotTranslate>**User management**</DoNotTranslate>. 
...
3. To add a user, click <DoNotTranslate>**Add user**</DoNotTranslate>. Complete the prompts in the UI, including choosing the [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) and group. Any custom groups you've added are available from the group dropdown. If the custom group you choose has a role and account assigned, once you add the user to that group, that user will have access.
...
* Go to the <DoNotTranslate>**User management**</DoNotTranslate> UI and <DoNotTranslate>**Access management**</DoNotTranslate> UI and see if the groups and grants assignments look correct and make sense.  
