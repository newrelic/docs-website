...
If you're reading this, you likely already have a [New Relic organization](/docs/accounts/accounts-billing/account-structure/new-relic-account-structure). When you [sign up for New Relic](https://newrelic.com/signup), your New Relic organization is created. The organization structure represents a New Relic customer: it's what contains everything relevant to a customer's use of New Relic: their accounts, their users, and their data.

When a New Relic organization is created, it contains a single account. If you're on our free tier or Standard edition, your organization can have only a single account, while Pro and Enterprise editions allow you to add accounts. Each account has its own [account ID](/docs/accounts/accounts-billing/account-structure/account-id), and that ID is used for some account-specific tasks, like making API calls.
