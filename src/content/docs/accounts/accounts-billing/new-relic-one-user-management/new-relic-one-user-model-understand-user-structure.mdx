...
translate:
  - jp
metaDescription: >-
  Explanation of the New Relic One user model: the user type, the roles, the permissions.
redirects:
  - /docs/users-roles
...
Your New Relic organization users can be on one of two account/user structures that we have: this doc explains the New Relic One user model. 
...
Have questions about logins or account access? See [Factors affecting access](/docs/accounts/accounts-billing/general-account-settings/factors-affecting-access-features-data/#account-access). 
...
<CollapserGroup>
  ...
  >
Below is a table comparing what basic users and full users can do. In short: basic users have access to our [Telemetry Data Platform](https://newrelic.com/platform) and some [Applied Intelligence](https://newrelic.com/platform) features, while full users have access to that and our Full Stack Observability features. Note that this access is theoretical: in practice, some users may be in groups with [roles](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#roles) that restrict their capabilities.
...
* **Admin**: This group has access to all [standard roles](#standard-roles). 
...
Custom groups can have either our default standard roles, or custom roles.
      ...
      <td>
A basic user's abilities aren't directly related to roles. A basic user can best be described as having the [**All product admin** role](#standard-roles) but without access to our Full Stack Observability features ([learn more about user type](#user-type)).  
...
<Callout variant="important">
  This is for users on our [New Relic One user model](/docs/accounts/original-accounts-billing/original-product-based-pricing/transition-guide-new-pricing-user-changes#user-models). For our original user model, see [Original user docs](/docs/accounts/original-accounts-billing/original-users-roles).
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Provides access to use and configure our observability platform features, but lacking these admin abilities: managing users (**Authentication domain manager** role) and managing organization-level settings (**Organization manager** role). 
