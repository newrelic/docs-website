...
When adding users to New Relic, two important decisions you must make are: 

* What [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) they should be. The user type is what governs the maximum allowed New Relic capabilities that user can access. 
* What group to place a user in: one of our [default groups](#groups) (**Admin** or **User**) or a custom group. A group is what gives you the ability to a) limit a user's capabilities further than the user type and b) grant users access to specific accounts in a New Relic organization. 

Here's a diagram showing how user type restrictions and group-related restrictions relate to each other: 
...
<figcaption>A user's group can restrict their capabilities. The [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) can also restrict capabilities and takes precedence over any group restrictions. For example: a basic user may be assigned to a group that has full New Relic access, but that basic user will never have more than [basic user capabilities](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type).</figcaption>
