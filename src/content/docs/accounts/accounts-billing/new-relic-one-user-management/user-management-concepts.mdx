...
There are two main ways access to New Relic features are controlled for New Relic users: 

* **User type** (basic user, core user, full platform user): This is primarily a billing consideration, based on how much you expect your users to use New Relic. User type should **not** be used to control permissions: for that you should use roles. For more about user type, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type). 
* **Role-based access**: This is what you use for governing what accounts and permissions you want your users to have. User **groups** contain **roles**, and those roles contain **permissions**. When you assign users to a group, you also choose the account-related scope of their access (one or more accounts in an organization, or the entire organization).

These are two separate systems. For you to be able to access a New Relic feature, both your user type and role-based permissions must allow that access. For more about the difference between user type and roles, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type#user-type-and-roles). The remainder of this doc is about role-based access. 
...
* A user's access is based on the access granted to them by their user type and their assigned roles ([learn more](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type#user-type-and-roles)).
...
* If a user is a [basic user](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type), they only have the access of a basic user, no matter their assigned roles. For more about user access, see [User management concepts](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/).
