...
## How New Relic Access Control works [#understand-concepts]

New Relic provides two complementary access control mechanisms to manage user permissions:

* **Role-Based Access Control (RBAC)**: Controls what features and capabilities users can access
* **Data Access Control (Fine-Grained Access)**: Controls what specific data users can view
...
## Data Access Control (Fine-Grained Access) [#data-access-control]

While roles control what features users can access, data access policies control what specific data users can view. Currently, data access control is available for log data, allowing you to restrict access to specific [log partitions](/docs/logs/ui-data/data-partitions/).

<Callout variant="important">
Data access control requires a [Pro or Enterprise](https://newrelic.com/pricing) edition account.
</Callout>

### How data access policies work [#how-policies-work]

Data access policies define which log partitions users can access. These policies are assigned to user groups through access grants. Users in groups with data access policies will only be able to view data from the allowed log partitions.

Key concepts:

* **Data access policy**: Defines access rules for log partitions using either allow-lists or deny-lists
* **Policy assignment**: Policies are assigned to groups via the Access Management UI
* **Multiple policies**: If a user belongs to multiple groups with different policies, the most permissive access applies
* **Wildcard matching**: Use `%` to match multiple partitions (e.g., `log_prod%` matches all partitions starting with "log_prod")

### Setting up data access control [#setup-data-access]

To set up data access control for your log data:

1. Create data access policies that define which log partitions users can access
2. Assign these policies to user groups through access grants
3. Users in those groups will automatically have their log data access restricted according to the policies

For detailed setup instructions, see [Data access control](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control).

To manage data access policies via API, see [NerdGraph data access control tutorial](/docs/apis/nerdgraph/examples/nerdgraph-data-access-control).
