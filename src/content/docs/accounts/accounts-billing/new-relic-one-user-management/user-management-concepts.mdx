...
Another way to think about the relationship between a user's user type and their assigned roles: a user's user type overrides all role restrictions. For example: a basic user might be assigned a role that has broad New Relic access, but restrictions related to [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) take precedence over any role-granted access. To see how user type relates to our roles, see our [Roles table](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#standard-roles). 

This doc is focused on role-related access. For details about user types, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type). 
