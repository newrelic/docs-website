...
* A user's **[user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type)**: For organizations on [our usage-based pricing](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing), the user type is mainly a billing-related decision based on how much value an organization expects to get out of a team member using New Relic. The user type sets the maximum allowed capabilities a user can access. User type is not meant to be used for controlling permissions: for that, you should use roles. 
