...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        This role includes all New Relic platform capabilities **except** the ability to manage organization-level settings, users, and billing. It's an admin role in the sense that it allows the configuration of our platform features (for example, the ability to configure <InlinePopup type="apm" /> settings), but it doesn't provide organization-level admin capabilities (those require [the administration settings](#admin-settings)). 
