...
* A user's [user type](#user-type-groups-relation) overrides role-related access. In other words, if you're a basic user, you'll be limited by basic user boundaries no matter what roles you're assigned. 
* Roles govern the use of New Relic observability-related features, while access to more administration-related features are governed by [administration settings](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings).
...
You can add various **Administration settings** to a group. Basic users cannot be assigned these settings.

Settings include: 

* **Organization settings**: Provides capabilities related to organization settings, including adding accounts, and changing the name of the organization and accounts.
* **Authentication domain settings**: Provides capabilities related to adding and managing users, including configuring authentication domains and customizing groups and roles. Options within this include: 
...
With this feature, you can give selected users the ability to add and remove users for a specific group. This can sometimes be preferable to granting users the more powerful and organization-scoped [authentication domain settings](#admin-settings).

To use the **Group admin** role, a user must have at least one of the authentication domain admin settings.
