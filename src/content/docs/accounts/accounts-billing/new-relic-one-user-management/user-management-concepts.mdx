...
When a user is created in New Relic, that user is given a user ID. To find a user's ID, you can go to the **User management** UI and look for a specific user. One shortcut for finding your own user ID is to inspect the source code for a New Relic UI page load and search that page for `userId`. 

If a user is added to a New Relic organization and that user has the same email address as a user already in the organization, those user records will be treated as the same user, and have the same user ID. 

For user records in a New Relic organization that share the same email address, if you change the email address for one of those user records, it will retain the underlying user ID and still be treated as the same user. 
...
* A **capability** is an ability to use or edit a specific, granular New Relic feature. Examples of capabilities:
  * The ability to modify <InlinePopover type="apm" /> settings
