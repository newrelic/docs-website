...
**Roles** are groupings of [capabilities](/docs/accounts/accounts-billing/new-relic-one-user-management/user-capabilities), which are the granular things New Relic users can do (for example, the ability to manage data retention). We have several [default roles](#standard-roles), and Pro and Enterprise edition organizations can create their own custom roles.

Important points about roles: 

* Roles are additive: users with multiple roles assigned have the total of all the permissions granted by those roles. For example, if you're in a group that gives you `Custom role A` in an account, and in another group that gives you `Custom role B` in that same account, you have both roles and are not restricted by one or the other. 
