...
translate:
  - jp  
metaDescription: 'For users on our newer user model: how permissions work, including how groups, roles, and access work.'
redirects:
...
* The roles assigned: after a user's user type is decided, **roles** can be used to more precisely control a user's access. Roles are sets of **capabilities**, which are granular New Relic abilities (for example, the ability to modify APM settings). Roles are assigned to [user groups](#groups) via access grants.
...
For information on specific capabilities, see [Capabilities](/docs/accounts/accounts-billing/new-relic-one-user-management/user-capabilities).
