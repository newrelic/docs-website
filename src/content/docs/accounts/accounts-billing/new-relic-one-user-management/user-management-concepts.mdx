...
When a user is created in New Relic, that user is given a user ID. To find a user's ID, you can go to the **User management** UI and look for that user's ID. One shortcut for finding your own user ID is to inspect the source code for a New Relic platform page load and search that page for `userId`. 
...
For user records in the same organization that share the same email address, if you change the email address of one of those users, it will retain the underlying user ID and still be treated as the same user. 
