...
A New Relic user is given permission to use a New Relic feature by the combination of their a) user type, and their b) role permissions. For a New Relic user to access something, their user type and the role(s) assigned to them must allow that. 
