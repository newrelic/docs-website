...
* The ability to manage how user upgrade requests are handled. These options include: 
  * **Allow users to self-upgrade**: this allows basic users to be able to quickly upgrade on their own, in order to quickly respond to issues. 
  * **Require review by admins**: in this case, admins refer to users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles). With this option, admins receive an email when an upgrade request is made, and can approve or deny those requests in the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where). 
