...
If you have existing domains, they'll be on the left. Note that most organizations have, at most, two domains: one with the manual, default settings and one for the identity provider-related settings. 
...
At the bottom of the [authentication domain UI](#ui), one of the settings is controlling how basic users upgrade to billable [full users](/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure). You have two choices: 

* Allow basic users to self-upgrade, which allows basic users to be able to quickly upgrade in order to respond to issues. 
