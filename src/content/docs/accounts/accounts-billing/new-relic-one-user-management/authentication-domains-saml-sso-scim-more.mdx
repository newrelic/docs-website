...
Note that how you enable SAML SSO differs depending on the identity provider you use, and whether you're also configuring SAML: 

* SAML only: 


Doing SAML only: 
	For IDP apps: follow procedures. 
	Without IDP apps: follow procedures. 

Doing SAML + SCIM: 
	For IDP apps: follow SCIM procedures first. 
	Without IDP apps: follow SAML procedures and use SCIM API. 


This integration may also require you to read your identity provider docs. 
...
2. If you **only** want to enable SAML SSO and not SCIM, and if you use Okta, Azure, or OneLogin, follow these instructions for configuring the relevant app: 
  ...
  * If you're implementing only SAML but using a different identity provider not mentioned above, you'll need to attempt to integrate using the SAML instructions below. Note that your identity provider must use the SAML 2.0 protocol, and must require signed SAML assertions.
