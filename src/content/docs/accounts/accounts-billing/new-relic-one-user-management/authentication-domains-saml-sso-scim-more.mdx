...
When you add users to New Relic, you add them to a specific authentication domain. Your organization will likely have either one or two authentication domains: one for the manual, default methods and one for the methods associated with an identity provider. Learn more in this short video (4:26 minutes):
...
To view and configure authentication domains: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), go to **Administration > Access management > Authentication domains**.

If you have existing domains, they'll be on the left. Note that most organizations will typically have either one or two authentication domains: one for the manual, default settings and one for the identity provider-associated settings.
...
To learn how to migrate users to a different authentication domain, see [Move users](#move-users). 
...
3. Next, you'll go to our authentication domain UI. From the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), click **Access management**, and then click **Authentication domains**. If you don't already have one, [create a new domain](#ui) to be used for your SAML-authenticating users.
...
## Move user records from one domain to another [#move-users]

You can move your users from one authentication domain to another. The auth domain you're moving users from is referred to as the **source** domain. The auth domain you're moving users to is referred to as the **destination** domain. 

Reasons to use this feature:

* You want an authentication domain's users to be switched from the manual email+password login method to using SAML SSO, or to go from manual provisioning to SCIM provisioning.
...
There are some additional rules depending on whether your destination domain has SCIM for its [source of users](#source-users) or has a manual source of users:  
...
For a destination domain that has a **Manual** source of users (isn't using SCIM):

* Both users and groups are moved. 
* If groups already exist in the destination domain: 
  * Groups with the same name will be overwritten by the groups (and users? does it try to match users even if groups aren't matched?) in the source domain. 
  * Group names that are unique in the destination domain remain.
* If users already exist in the destination domain: 
  * If there are matching user records (based on email address) in the destination domain, the user records in the destination domain are overwritten. 
  * User records in the destination domain that are unique will remain in the destination domain. 
</Collapser>
  ...
  >
For a destination domain that is using SCIM: 

* Only users are moved. Groups are not a consideration because your desired groups and users should already be set up in the destination domain. Users in the source domain are migrated to their matching user records (based on email address) in the destination domain. 
* If there are matching user records (based on email address) in the destination domain, the user records in the destination domain are overwritten. User records in the destination domain that are unique will remain in the destination domain. 
</Collapser>
</CollapserGroup>

To move users from one authentication domain to another: 

1. Set up the destination authentication domain with the settings you want. For example, if you want the destination domain to use SAML and/or SCIM, first set up a new authentication domain using these instructions: [SAML only](#saml) | [SCIM + SAML](#source-users). We'll give more tips for SAML and SCIM setups below. 
2. Go to the [authentication domain UI](#ui) and click **Move users**. Follow the instructions. 

To verify the migration has occurred, go to the **Access management** UI, click the **Groups** tab, and review the groups and their associated authentication domains.  
