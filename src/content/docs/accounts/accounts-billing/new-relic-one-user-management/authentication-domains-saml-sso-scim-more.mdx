---
title: 'Authentication domains: control how your users are provisioned, how they log in, their session controls, and more'
...
An authentication domain, also referred to as an auth domain, is a grouping of New Relic users and user groups governed by the same user management settings, like [how they're provisioned](#source-users) (added and updated), [how they're authenticated](#authentication) (logged in), [session settings](#session-mgmt), and [how user upgrades are managed](#session-mgmt).
...
Those default settings would be under one auth domain. Another auth domain might be set up like this:
...
When you add users to New Relic, you add them to a specific auth domain. Your organization will likely have either one or two auth domains: one for the manual, default methods and one for the methods associated with an identity provider. Learn more in this short video (4:26 minutes):
...
Auth domains only apply to users on [our newer user model](/docs/accounts/accounts-billing/new-relic-one-pricing-users/users-roles). If your users are on our original user model, see [Original accounts](/docs/accounts/original-accounts-billing/original-users-roles/original-account-structure).

Requirements to manage auth domains:

* Your organization must be either [Pro or Enterprise edition](https://newrelic.com/pricing) to have editable auth domains.
* To view or edit auth domains, a user must:
  ...
  * Be in a group with the [**Authentication domain** admin setting](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings).
...
## Create and configure an auth domain [#ui]

If you meet the [requirements](#requirements), you can add and manage auth domains.

To view and configure auth domains: from the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), go to **Administration > Access management > Authentication domains**.

If you have existing auth domains, they'll be on the left. Note that most organizations will typically have either one or two auth domains: one for the manual, default settings and one for the identity provider-associated settings.

To create a new domain from the auth domain UI page, click **Create new**. For more about the configuration options, keep reading.

## Multiple auth domains [#switch]

If you have user records in more than one auth domain, you can [switch between domains](/docs/accounts/accounts-billing/account-setup/multiple-logins-found). 

To learn how to migrate existing users to another auth domain, see [Move users](#move-users). 
...
From the [auth domain UI](#ui), you can set one of two options for how users are added to New Relic:
...
* You can't toggle **Source of users**. This means if you want to change this for an auth domain that's already been set up, you'll need to create a new one.
...
The authentication method is the way in which New Relic users log in to New Relic. All users in an auth domain have a single auth method. There are two authentication options:
...
3. Next, you'll go to our auth domain UI. From the [user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), click **Access management**, and then click **Authentication domains**. If you don't already have one, [create a new domain](#ui) to be used for your SAML-authenticating users.
...
9. If you're using an identity provider app, you'll need to input the auth domain ID in the app. That ID is found at the top of our auth domain UI page.

10. Optional: In our auth domain UI, you can adjust [other settings](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#session-mgmt), like browser session length and user upgrade method. You can adjust these settings at any time.
...
In the [auth domain UI](#ui), under **Management**, you can control some other settings for the users in that domain, including:
...
In the [auth domain UI](#ui), under **Management**, you can control how your users' [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) is managed. This includes how the user type can be edited and how upgrade requests are handled.
...
<CollapserGroup>
  ...
  >
    ...
    To enable this setting, your auth domain **Source of users** must have [SCIM enabled](#source-users).
...
## Move users to another auth domain [#move-users]

You can move your users from one auth domain to another. The auth domain you're moving users from is referred to as the **source** auth domain. The auth domain you're moving users to is referred to as the **destination** auth domain. 

Reasons to use this feature:

* You want an auth domain's users to be switched from the manual email+password login method to using SAML SSO, or to go from manual provisioning to SCIM provisioning.
* Migrating user records lets those users keep many of their assets and settings (dashboards, user keys, favorites, time zone settings, notification preferences, and more), but creating new user records does not.

Before moving your users, you should understand some important points:

* Migrating your users is done with a single bulk action. In other words, you can't select specific users. 
* For users or groups that are in the destination auth domain: once those records are overwritten with the new incoming records, the old records are gone. This will typically not be a concern for auth domains you've just created, but may be a concern for pre-existing auth domains you're attempting to merge. 

There are some additional rules depending on whether your destination auth domain has SCIM for its [source of users](#source-users) or has a manual source of users:  

<CollapserGroup>
  <Collapser
    id="move-users-manual"
    title="Source of users: manual"
  >

For a destination auth domain that has a **Manual** source of users (isn't using SCIM):

* Both users and groups are moved. 
* If groups already exist in the destination auth domain: 
  * Groups in the destination will be overwritten by groups in the source auth domain that have the same name. 
  * Group names that are unique in the destination auth domain remain.
* If users already exist in the destination auth domain: 
  * If there are user records with the same email address in the source and destination auth domains, the user records in the destination auth domain are overwritten with the matching ones in the source. 
  * User records in the destination auth domain that are unique will remain in the destination auth domain. 
</Collapser>

  <Collapser
    id="move-users-scim"
    title="Source of users: SCIM"
  >
For a destination auth domain that is using SCIM: 

* Only users are moved. Groups are not a consideration because your desired groups and users should already be set up in the destination auth domain. Users in the source auth domain are migrated to their matching user records (based on email address) in the destination auth domain. 
* If there are matching user records (based on email address) in the destination auth domain, the user records in the destination auth domain are overwritten. User records in the destination auth domain that have no email address match with users in the source auth domain will remain in the destination auth domain. 
</Collapser>
</CollapserGroup>

To move users from one auth domain to another: 

1. Set up the destination auth domain with the settings you want. For example, if you want the destination auth domain to use SAML and/or SCIM, first set up a new auth domain using these instructions: [SAML only](#saml) | [SCIM + SAML](#source-users). We'll give more tips for SAML and SCIM setups below. 
2. Go to the [auth domain UI](#ui) and click **Move users**. Follow the instructions. 

To verify the migration has occurred, go to the **Access management** UI, click the **Groups** tab, and review the groups and their associated auth domains.  
