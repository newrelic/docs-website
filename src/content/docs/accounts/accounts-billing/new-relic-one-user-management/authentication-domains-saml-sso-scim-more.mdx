...
## Managing user type [#user-upgrade]
...
Currently the only identity provider we have instructions for is [Okta](/docs/accounts/accounts/automated-user-management/okta-scimsso-application-configuration). For other identity provider services, you can use our [SCIM API instructions for sending `nrUserType`](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim/#manage-user-type).

Notes on using this: 
* To enable this setting, you must first [enable SCIM](#source-users) under the **Source of users**. 
* When you enable this setting, you're prompted to create a custom URL and message that basic users see when they go to New Relic UI pages that are not available to them. That custom message helps them understand your specific process for upgrading. 
