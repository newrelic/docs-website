...
<Callout variant="important">
Managing New Relic user type is available for our SCIM API. It is not yet available for the [Azure AD](/docs/accounts/accounts/automated-user-management/azure-ad-scimsso-application-configuration), [Okta](/docs/accounts/accounts/automated-user-management/okta-scimsso-application-configuration), or [OneLogin](/docs/accounts/accounts/automated-user-management/onelogin-scimsso-application-configuration) New Relic apps. 
</Callout>

If you've [enabled SCIM](#source-users) in the authentication domain UI, you can use our [SCIM API](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type) to manage user type. The upgrade request UI allows you to create a custom URL and message that basic users will see when they go to New Relic features that are off limits to them, and that message will help them understand your specific process for upgrading. 
