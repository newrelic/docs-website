...
Note that once you enable SCIM for an authentication domain, you can't later turn it off. If you need an authentication domain without SCIM, you must create a new one. 
...
At the bottom of the [authentication domain UI](#ui), one of the settings is controlling how basic users upgrade to billable [full users](/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure). You have different options depending on whether you are managing user type in New Relic or via SCIM: 
...
<CollapserGroup>
  ...
  >
If you don't manage your users via [SCIM](#source-users), you have access to these options: 
...
<Collapser
  ...
  >
If you've enabled the **Source of users** as SCIM and if you've selected **Manage user type with SCIM**, you can use our [SCIM API](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type) to set this up. The upgrade request UI allows you to create a custom message and custom URL that's displayed when basic users see New Relic UI that is off limits to them, and this message helps them understand the process you've given them for requesting an upgrade. 
</Collapser>
