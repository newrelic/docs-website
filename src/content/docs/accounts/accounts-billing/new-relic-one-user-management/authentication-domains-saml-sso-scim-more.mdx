...
To create a new domain from the authentication domain UI page, click <DNT>**Create authentication domain**</DNT>. To manage or delete an authentication domain, select the menu item for each authentication domain. For more about the configuration options, keep reading.
