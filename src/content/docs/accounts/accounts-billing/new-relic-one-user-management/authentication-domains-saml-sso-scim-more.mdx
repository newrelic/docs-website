...
To manage their users, New Relic organizations can configure "authentication domains," which control how users are added to a New Relic account, how they’re authenticated, and some other session settings.
...
<<<<<<< Updated upstream
From the [authentication domain UI](#ui), you can configure a variety of settings for the users in that domain, including:

* Length of time users can remain logged in.
* Amount of idle time before a users' session expires. 
* User access requests: This controls how basic users become full users. You can either allow basic users to self-upgrade or you can require review by admins (users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#standard-roles)). If **Require review** is set, admins receive an email when an upgrade request is made. Allowing your users to self-upgrade allows them to quickly respond to issues. For more about basic and full user capabilities, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type). (Note that users on the original user model don't have access to this UI and have their own [user management procedures](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#user-type).)
=======
From the [authentication domain UI](#ui), you can configure a variety of settings for the users in that authentication domain, including:

* Length of time users can remain logged in.
* Amount of idle time before a users' session expires. 
* User access requests: You can allow your basic users to either self-serve upgrade to become full users or you can require approval first from admins (users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#standard-roles)). Applicable admins will receive an email when a request is made. Allowing your users to self-upgrade allows them to quickly respond to issues. For more on basic user versus full user capabilities, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type).
>>>>>>> Stashed changes
