...
<Callout variant="important">
  This doc is for managing users on teh [New Relic One user model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models). For managing users on our original user model, see [Original users](/docs/accounts/original-accounts-billing/original-users-roles).
</Callout>
...
To view and configure authentication domains: from the [account dropdown](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown), go to **Administration &gt; Organization and access &gt; Authentication domains**. 
...
* The ability to manage how requests to become full platform users are handled. These options include: 
  * **Allow users to self-upgrade**: this allows basic users and core users to be able to immediately upgrade to full platform user on their own, in order to quickly respond to issues. 
  * **Require review by admins**: in this case, admins refer to users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles). With this option, admins receive an email when a request to become a full platform user is made, and can approve or deny those requests in the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where). 

Note that upgrading from a basic user to a core user doesn't require review.   
</Collapser>
