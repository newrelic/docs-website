...
You can move user records and groups from one authentication domain (the **source** domain) to another (the **destination** domain). 

Reasons to use this feature:

* You want an authentication domain's users to be switched from an email+password login method to using SAML SSO, or to go from manual provisioning to SCIM provisioning.
...
1. First, set up the destination domain with the settings you want. When moving users between domains, you can't migrate domain settings, so the domain must be set up beforehand. For additional instructions for migrated to a SCIM-provisioned domain, see below. 

  <CollapserGroup>
    <Collapser
      id="scim"
      title="SCIM instructions"
    >
    Before migration, you must first set up the domain and your identity provider according to our [automated user management instructions](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign). 
    
    The migration only migrates users, not groups, so before migration, you should ensure you see the groups and users from your identity provider in the New Relic UI. Upon migration, the user records (and associated assets) in the source domain will be moved over to replace the user records in the destination domain.

    Replacement of user records is based on records having the same email address. If no record with a matching email is found, that user record isn't **??**
    </Collapser>
  </CollapserGroup>

2. Go to the [authentication domain UI](#ui) and click **Move users**...**?**
