...
1. First, set up the destination authentication domain with the settings you want. When moving users between domains, you can't migrate domain settings, so the domain must be set up beforehand. For details and caveats about how user migration works for SCIM and non-SCIM (manual) destination domains, select one:
...
* If there are matching user records (based on email address) in the destination domain, the user records in the destination domain are overwritten. User records that are unique in the destination domain will remain in the destination domain. 
...
* For a SCIM destination domain, the **Move users** feature works only on user records: groups are not touched at all.  
* The migrating of user records is based on records having the same email address. For user records without matches: 
* If a user record in the source domain doesn't have a matching record in the destination domain, that user record isn't migrated and the migration process notifies you of that. 
