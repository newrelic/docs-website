...
You can move user records and groups from one authentication domain to another. The domain you're moving users from we refer to as the **source** domain. The domain you're moving users to we refer to as the **destination** domain. 
...
**Note that migrating your users is final and can't be undone.**

To migrate your users from one domain to another:

1. First, set up the destination authentication domain with the settings you want. For example, if you want the destination domain to use SAML and/or SCIM, first follow those instructions: [SAML only](#saml) | [SCIM + SAML](#source-users). 

  For some important notes about how moving users works for SCIM and non-SCIM (manual) destination domains, select one:
...
Some caveats about moving users to a domain with a manual **Source of users**:
...
* If there are groups in the destination domain: groups with the same name in the destination domain are overwritten. Unique group names in the destination domain remain. 
  </Collapser>  
  <Collapser
    id="move-users-scim"
...
Some important caveats about moving users to a SCIM-provisioned authentication domain: 

* Before migration, first set up your identity provider and the destination authentication domain according to our [automated user management instructions](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign). Before moving users, you should ensure the users and groups you want are present in the destination authentication domain. 
* For a SCIM destination domain, the **Move users** feature works only manipulates user records: groups are not manipulated at all because the groups should already exist in the destination domain.  
* The migrating of user records is based on records having the same email address. For user records without matches: 
  * If a user record in the source domain doesn't have a matching record in the destination domain, that user record isn't migrated. 
  * If a user in the destination domain doesn't have a matching record in the source domain, that user record remains in the destination domain.
