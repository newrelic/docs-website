...
* Changing users' user type in the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where).
* Managing how basic users' requests to be full users are handled. These options include: 
  * Allow basic users to self-upgrade: this allows basic users to be able to quickly become full users on their own, in order to quickly respond to issues. 
  * Require review by admins (admins in this case being users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#standard-roles)). With this setting, admins receive an email when an upgrade request is made, and can approve or deny these requests in the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where). 
