...
metaDescription: >-
  New Relic user authentication: how your users are added, SAML SSO, SCIM, automated user management, and more.
redirects:
  - >-
...
* Users are added and managed automatically from an identity provider
* Users are logged in using SAML single sign-on (SSO) from an identity provider

When you add users to New Relic, they’re added to a specific authentication domain. Typically organizations have either one or two authentication domains: one for the manual, default methods and one for the methods tied to their identity provider.
...
* **SCIM:** enabling SCIM lets you use our automated user management (AUM) to import/manage users from your identity provider. For instructions, see [Automated user management](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign). Note that enabling SCIM also requires you to [enable SAML authentication](#saml). 
...
Note that how you enable SAML SSO differs depending on the identity provider you use. You may have to also read your identity provider docs to understand how to integrate with New Relic. 

1. If you're using Azure, Okta, or OneLogin, your first step is to enable the New Relic integration for that service. To do this, see the instructions for your service: 
...
6. From the **Add New Relic by Organization** page, click on **Save**.
    ...
    If you're using a different service, note that your identity provider must use the SAML 2.0 protocol, and must have the “signed SAML assertions” option enabled.

2. Next, you'll go to New Relic's authentication domain UI. From the [account dropdown](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#account-dropdown), click **Organization and access**, and then click **Authentication domains**. If you don't already have one, create a new authentication domain to be used for your SAML authentication users. 
3. Under **Authentication**, click **Configure**. Under **Method of authenticating users**, select **SAML SSO**.
4. Under **Provided by New Relic**, we have New Relic-specific information. You'll need to place these in the relevant places in your identity provider app or service. If you're not sure where to place these, you may need to consult your identity provider docs. 
5. Under **Provided by you**, input the **Source of SAML metadata**. This URL is supplied by your identity provider and should conform to SAML V2.0 metadata standards. If your identity provider doesn't support dynamic configuration, you can do this by using **Upload a certificate**. This should be a PEM encoded x509 certificate. 
6. Under **Provided by you**, set the **SSO target URL** supplied by your identity provider. You can find this by going to the **Source of SAML metadata** and finding the POST binding URL. It looks like: `https://newrelic.oktapreview.com/app/newreliclr/1234567890abcdefghij/sso/saml`.
7. If your identity provider has a redirect URL for logout, enter it in the **Logout redirect URL**; otherwise, leave it blank.
8. If you’re using an ID provider app, you’ll need to input the authentication domain ID in the app. That ID is found at the top of New Relic’s authentication domain UI page. 
9. Optional: In New Relic’s authentication domain UI, you can adjust [other settings](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#session-mgmt), like browser session length and user upgrade method. You can adjust these settings at any time.
10. When complete, go to the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) to verify that users have been added.
11. Next, you'll need to give your users access to specific roles and accounts. It may help you to read [a summary of how that process works](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts).
