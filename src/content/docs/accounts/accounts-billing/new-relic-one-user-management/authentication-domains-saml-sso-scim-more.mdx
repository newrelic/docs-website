...
* The ability to manage how user type upgrade requests are handled. These options include: 
  * **Allow users to self-upgrade**: this allows basic users to automatically be able to immediately upgrade to a higher user type on their own. This setting allows these users to be able to more quickly respond to issues. 
