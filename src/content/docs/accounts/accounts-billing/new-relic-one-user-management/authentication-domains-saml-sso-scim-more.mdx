...
* The ability to manage how user upgrade requests are handled. These options include: 
  ...
  * **Require review by admins**: in this case, admins refer to users with the [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles). With this option, admins receive an email when an upgrade request is made, and can approve or deny those requests in the [**User management** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where). 
