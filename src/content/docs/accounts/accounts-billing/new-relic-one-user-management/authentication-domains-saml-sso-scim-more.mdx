...
## Move users and groups from one domain to another [#move-users]

You can move users and groups from one authentication domain (the **source** domain) to another (the **destination** domain). One reason to do this is that you want an authentication domain's users to be switched from an email+password login method to using SAML SSO, or to go from manual provisioning to SCIM provisioning. 

Some notes on migrating users between authentication domains: 
* You can only migrate users and groups: you can't migrate domain settings. 

To migrate users between domains: 

* First, set up the destination authentication domain with the settings you want.
  * **For SCIM**: If you want [SCIM provisioning](#source-users) for the destination domain, you must set up the domain and your identity provider according to our [automated user management instructions](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign). When you migrate users, only those with user records in the destination domain, based on matching email address, will be migrated. 
* Go to the [authentication domain UI](#ui) and click **Move users **
* 
