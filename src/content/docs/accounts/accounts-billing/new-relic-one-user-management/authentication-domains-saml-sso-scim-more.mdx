...
Our [SCIM API](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type) lets to manage user type (basic user versus full user) from your identity provider instead of in the New Relic user management UI. When you enable this, you can't manage user type from the New Relic side.

Currently the only identity provider we have instructions for is [Okta](/docs/accounts/accounts/automated-user-management/okta-scimsso-application-configuration). For other identity provider services, you must use our [SCIM API instructions for sending `nrUserType`](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim/#manage-user-type).

Notes on using this: 
* To enable this setting, note that you must first [enable SCIM](#source-users) under the **Source of users**. 
* Once you enable this setting, you're prompted to create a custom URL and message that basic users see when they go to New Relic features that are off limits to them. That message will help them understand your specific process for upgrading. 

</Collapser>

</CollapserGroup>

For more about basic users and full users, see [User type](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type). 
