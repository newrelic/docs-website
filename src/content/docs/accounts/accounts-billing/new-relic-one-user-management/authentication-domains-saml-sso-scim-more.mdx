...
Note that once you enable SCIM for an authentication domain, you can't later turn it off. This means that if you later decide to disable SCIM for that domain, you must instead create a new one. 
...
<Collapser
  ...
  >
If you've enabled the **Source of users** as SCIM and if you've selected **Manage user type with SCIM**, you can use our [SCIM API](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type) to configure this. The upgrade request UI allows you to create a custom message and custom URL that's displayed when basic users see New Relic UI that is off limits to them, and this message helps them understand the process you've given them for requesting an upgrade. 
</Collapser>
