...
If an alert condition exists querying log data with a FACET on sensitive data, users with access to that account can:

* Target any alert condition in an account they access for notifications, including conditions on restricted partitions
