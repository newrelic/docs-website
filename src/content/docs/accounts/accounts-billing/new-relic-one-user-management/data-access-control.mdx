...
Large organizations generate massive amounts of logs across various teams, services, and environments. Security teams need to monitor critical security logs that may contain personally identifiable information (PII), while development teams only need to see logs relevant to their specific microservices. <DNT>**Data access control**</DNT> lets you create policies that define exactly which log partitions each team can access.
...
When alert conditions query logs containing sensitive data, users with access to that account can:

* Receive notifications from any alert condition, including conditions that query restricted partitions
* Query these events which may contain restricted data: `NrAiSignal`, `NrAiIncident`, `NrAiIssue`, `NrAiNotification`, or `NrIntegrationError`
* View condition detail pages and issue pages even when conditions query restricted partitions
