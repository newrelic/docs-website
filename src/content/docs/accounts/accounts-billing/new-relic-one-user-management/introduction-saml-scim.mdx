...
<CollapserGroup>
  ...
  >
For users on the [New Relic One user model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models), you'll use the [authentication domain UI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more) to set up SAML and SCIM. Note that Okta, OneLogin, and Azure have dedicated New Relic apps: if you use those, you must choose the one called "New Relic by organization", and not "New Relic by account". 
...
* SCIM + SAML: SCIM provisioning creates users on the New Relic One user model. This is why we recommend you to first migrate your users to the new user model before using SCIM. For more on how to migrate to the new model, see [Migrate users](/docs/accounts/original-accounts-billing/original-users-roles/user-migration).
  </Collapser>
