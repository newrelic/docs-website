...
A New Relic user's **user type** determines the maximum set of New Relic capabilities they can access. The user type is meant to be a fairly long-term setting based on a user's expected responsibilities over the next several months or longer. 
