...
<CollapserGroup>
  <Collapser
    id="domain-capture"
    title="Capture users with email addresses from your domain"
  >
To avoid having your users accidentally sign up for New Relic and create unwanted New Relic organizations and accounts, set up [domain capture](/docs/accounts/accounts-billing/account-setup/domain-capture).  
</Collapser>
  ...
  >
A New Relic user can have a maximum of three concurrent active sessions in use at any given time, or three unique IP addresses in use at any given time. 

In this context, a session represents a unique login to the New Relic platform. The session is tracked by a token being assigned for that session. 

If you exceed this limit, you'll see an `Active session limit reached` message upon login and an option to close open sessions.

For how to control your users' sessions, see [Session settings](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more#session-mgmt).  
</Collapser>
