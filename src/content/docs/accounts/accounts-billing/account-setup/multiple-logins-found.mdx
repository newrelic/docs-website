...
When you log in to New Relic, you can go to the [user menu](/docs/new-relic-solutions/get-started/glossary/#account-dropdown) to switch to other user records that have the same email address. When you choose a user record from the menu, you'll be prompted to log in for that user record (either directly to New Relic or via your identity provider).

 If the desired user record has a manual login (not single sign-on), you'll be  records for non-SSO users.
...
To access other user records that use a different email address, you'll have to log out and log back in.
