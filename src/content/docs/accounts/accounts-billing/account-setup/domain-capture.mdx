...
With **domain capture**, you can register your domains with New Relic. When you do that, users in your organization who attempt to sign up for New Relic will be given the option to join your New Relic organization. You can also control how those users are routed into New Relic: their user type and user group. The larger your organization is, the more useful domain capture will be. 
...
When you're done, the **Domain management** UI page will show the status for that domain as **Pending**. When we've verified that domain, its status will be **Verified**. 

When domain capture is created, it's disabled but your users will see a message notifying them that a New Relic organization already exists. For how to enable domain capture and adjust other settings, see [User settings](#user-settings).
...
When a domain capture is first set up, it's disabled, corresponding to **New user access** being set to **None**. The options for **New user access** include: 
