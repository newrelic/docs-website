...
metaDescription: For apps monitored by the Node.js agent, learn how to obfuscate URLS in distributed traces and transaction traces using a regex pattern to protect sensitive data.
---
...
To enable URL obfuscation, add the following to your agent configuration:
...
Replacement pattern is optional although recommended. If you don’t specify it, the agent will use the default replacement pattern, which is `''` an empty string.
