...
Modern monitoring systems can give you important insight into the details of a service when they report the right data. Distributed trace, byte code, or script instrumentation allows teams to quickly collect detailed service telemetry. Unfortunately, operations teams usually aren't in the best position to evaluate the quality of the telemetry they get.

Revealing improperly instrumented services to users creates a situation that can put customer satisfaction at risk. This burn-in period is difficult to escape as new features can come from code bases without a strong link between software delivery and observability programs. Service instrumentation planning is the approach used to describe a single service runtime through telemetry, and this guide focuses on the metrics of your application's code as well as external measurements through synthetic testing. 
...
Involving developers in this process has the following benefits:
...
**Best practices:*** follow the [Alert quality management guide](/docs/tutorial-create-alerts/manage-alert-quality/) to help you understand the behavior of your services through using alerts to help improve your service delivery.
  </Collapser>
</CollapserGroup>
