---
title: Introduction to Partnership API
tags:
  - New Relic partnerships
  - Partner integration guide
  - Partner account maintenance
metaDescription: 'New Relic offers a web service-based API for partners to create, edit, upgrade, downgrade, and cancel New Relic accounts on behalf of their customers.'
redirects:
  - /docs/accounts-partnerships/partnerships/partner-integration-guide/partner-api
  - /docs/accounts-partnerships/partner-integration-guide/partner-apis/partner-api
  - /docs/accounts-partnerships/partner-integration-guide/partner-account-maintenance/partner-api
  - /docs/accounts-partnerships/partnerships/partner-api
  - /docs/new-relic-partnerships/partnerships/partner-api/partner-api-reference
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-api-account-object
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-api-keys
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-api-sub-account-object
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-api-subscription-object
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-api-user-object
  - /docs/new-relic-partnerships/partnerships/partner-api/partnership-billing-integration-api
  - /docs/new-relic-partnerships/partnerships/partner-api/product-buckets
  - /docs/new-relic-partnerships/partnerships/partner-api/typical-integration-example
  - /docs/apis/nerdgraph/examples/provisions-your-subscriptions-nerdgraph
freshnessValidatedDate: 2024-07-31
---

<Callout variant="important" title="EOL NOTICE">
  The Partnership API reached end of life (EOL) in March 2023 and is no longer supported or available for use.  
</Callout>

Our Partnership API allowed New Relic partners, and New Relic accounts set up as [customer partnerships](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partnership-accounts-users-subscriptions/#partnerships), to manage accounts, users, and subscription-related settings.

Since the Partnership API is no longer available, you can use our Nerdgraph API to do many of the same tasks:

* [Manage accounts](/docs/apis/nerdgraph/examples/manage-accounts-nerdgraph)
* [Manage users](/docs/apis/nerdgraph/examples/nerdgraph-manage-users)
* [Manage user groups](/docs/apis/nerdgraph/examples/nerdgraph-manage-groups)
