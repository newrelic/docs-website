...
<DoNotTranslate>**Before using the Partnership API, first read [the requirements](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partner-api/).**</DoNotTranslate>
...
For some New Relic organizations, child accounts can also be created via the parent account's [<DoNotTranslate>**Account settings**</DoNotTranslate> page](/docs/accounts/install-new-relic/account-setup/manage-apps-or-users-sub-accounts) in the New Relic UI.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DoNotTranslate>**Create**</DoNotTranslate> a child account with the given parameters.
      </td>
...
<DoNotTranslate>**Note that this is just an example, and that for some accounts, the [`users` attribute](#account-attributes) is unnecessary and will be ignored.**</DoNotTranslate>
