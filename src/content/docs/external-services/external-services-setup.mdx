...
---
The external services feature is available for New Relic APM agents and OpenTelemetry. Review the following to understand the steps to set up external services for APM agents and OpenTelemetry. After you set up external services, you may need to adjust the [sampling rate](#adjust-sampling) if you don't see enough data.

## APM agents [#externals-and-apm]

For some agents you only need to install the agent, while others require some additional steps. Here’s what you need to know about setting up external services:

1. If you haven’t already done so, install the appropriate New Relic APM agent.
2. New APM agents have distributed tracing turned on by default, but if you have an older agent, complete the steps to enable either standard distributed tracing or Infinite Tracing. If you were using cross-application tracing, the configuration for distributed tracing automatically overrides cross-application tracing.
