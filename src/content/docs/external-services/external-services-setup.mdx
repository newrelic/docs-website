...
---
The external services feature is available for New Relic APM agents and OpenTelemetry. After you follow the steps below to set up external services, you may need to adjust the [sampling rate](#adjust-sampling) if you don't see enough data.

## APM agents [#externals-and-apm]

These setup steps are primarily for the external services feature that is now based on distributed tracing. The legacy version of external services is still available if you need to add a new service to a chain of agents already using legacy external services. To install legacy external services, you enable cross-application tracing and disable distributed tracing in the steps below.
...
2. Make sure distributed tracing is turned on in the agent: 
    * If you installed a new agent, standard distributed tracing is turned on by default. If your service talks to other services that are using Infinite Tracing, see your agent documentation for help with Infinite Tracing.
...
<Callout variant="tip">
These steps don't apply to legacy external services.
</Callout>
...
If you have questions about how to understand the UI, see our [tips](/docs/external-services/external-services-ui). 