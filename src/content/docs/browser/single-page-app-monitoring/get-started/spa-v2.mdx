...
* New API: The function [`newrelic.interaction({ waitForEnd: true })`](/docs/browser/new-relic-browser/browser-apis/interaction/) allows you to customize when a user interaction ends.
* Deprecated API: The function `createTracer`: While still functional, the `createTracer` function is deprecated as it no longer keeps interactions open or times callbacks. Note: `BrowserTiming` events are no longer created when using `createTracer` with the new SPA feature.
