...
freshnessValidatedDate: 2024-03-18
---

<Callout variant="important">
    This release is in limited preview and only available to select customers. Going forward, we plan on gradually phasing out the existing browser SPA experience.
    </Callout> 

For customers monitoring browser single page applications (SPA), we're excited to announce an overhaul of our SPA monitoring functionality, meant to address numerous customer pain points:
...
* Aligned with soft navigation heuristics: The new agent adopts Google Chrome's soft navigation, providing more accurate interaction tracking and improved alignment with browser behavior.
...
* Focus on key metrics: While reported data remains largely unchanged, the new agent no longer tracks JavaScript execution or callback duration within interactions, streamlining the reported information.
...
* New API: The function [`newrelic.interaction({ waitForEnd: true })`](https://docs.newrelic.com/docs/browser/new-relic-browser/browser-apis/interaction/) allows you to customize when a user interaction ends.
* Deprecated API: The function [`createTracer`](https://docs.newrelic.com/docs/browser/new-relic-browser/browser-apis/createtracer/): While still functional, the `createTracer` function is deprecated as it no longer keeps interactions open or times callbacks. Note: `BrowserTiming` events are no longer created when using `createTracer` with the new SPA feature.
...
<Steps>
    ...
    <Step>
        ...
        If your agent was installed with the [APM method](https://docs.newrelic.com/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#apm-agent), contact our support team and we'll enable the feature flag on your account.
        
        If your agent was installed with NPM or the UI-based, [copy/paste method](https://docs.newrelic.com/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#copy-paste), add the following feature flag your browser agent code:
...
* `AjaxRequest` association:
    ...
    * Manual extension: If you need a request to be attributed to a specific interaction, use the `waitForEnd: true` option within `newrelic.interaction()` to keep the interaction open until the request fires, then `.end()` the interaction. This will associate the request to the interaction.
