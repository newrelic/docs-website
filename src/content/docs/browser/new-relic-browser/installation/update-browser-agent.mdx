...
Verifying your browser version number can be done by one of the following:
1. Inspect the agent harvest payloads (`Any Browser Agent Version`)
...
2. Inspect the source code of the agent snippet
    * `v1217` and Newer:
    ...
    * `v1216` and Older:
      3. View the source code of a page which includes the JavaScript snippet.
      4. Search the page source for `js-agent.newrelic.com/nr-`.
      5. The numbers that follow `nr-` are your current version. For example, `js-agent.newrelic.com/nr-593.min.js` indicates you are running version `593` of the browser script.
3. Inspect the newrelic global object (v1224+)
   1. Open the developer tools of your browser.
...
To verify the version of the browser script loader on your page:
1. Inspect the newrelic global object (v1224+)
   1. Open the developer tools of your browser
   2. Open the console view
   3. In the console, enter `newrelic.initializedAgents`
   4. In the resulting object, expand the relevant agent object
...
## To check if your version is out of date
   1. See the [browser agent end of life](https://js-agent.newrelic.com/nr-loader-full-current.min.js) table.
