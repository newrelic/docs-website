...
NerdGraph can be used as an alternative to turn the setting on and off. To do so, run a mutation query on the `browserSettings` graphQL object for the `browserConsentMode` property, setting `enabled` to `true` or `false`.
...
- If [session storage](/docs/browser/new-relic-browser/configuration/browser-app-settings-page/#browser-session-storage) is enabled, decisions passed into the consent API will persist across pages.

- If [session storage](/docs/browser/new-relic-browser/configuration/browser-app-settings-page/#browser-session-storage) is disabled, the consent API will need to be called on every page load.
