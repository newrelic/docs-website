...
`init.proxy.assets` is used verbatim in the copy-and-paste installation to fetch agent code.
...
`init.proxy.beacon` is used by both installation to allow proxying of observability data through a different domain. For example, if this field is set to be `nrbeacon.abc.com`, all observability data will be sent to `https://nrbeacon.abc.com/<observability endpoint>` instead of what is configured automatically on the New Relic account and found in the `NREUM.info.beacon` configuration.
