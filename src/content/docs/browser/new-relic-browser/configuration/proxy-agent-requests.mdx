...
When using a proxy to send website performance data over a first-party subdomain, the proxy server acts as an middleman between your site and New Relic. Your site sends the monitoring data to the proxy server, which then forwards it to New Relic.

To set up proxies, you'll need to configure the following browser agent properties: 
