...
Browser monitoring uses URL patterns for data grouping instead of server-side controller action names, such as `ApplicationsController#show`. It includes links to related backend APM events as applicable. However, this are not the primary grouping strategy for browser as a standalone service.
