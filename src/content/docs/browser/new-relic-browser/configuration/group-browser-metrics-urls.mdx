...
tags:
  ...
  - Browser monitoring
  - Configuration
metaDescription: 'To override the default URL patterns that New Relic Browser uses to group metrics, add URLs to your allow list in the Browser Settings.'
redirects:
...
URL groups are created automatically when an application first reports data to New Relic. The inputs to Browser's grouping algorithm include:
...
After the URL has been filtered, Browser analyzes requests and splits URLs into segments on separator characters (dots for domains, slashes for paths). Browser also preserves useful terms and uses them in the automated grouping. Domain information is separated by port number automatically, so that (for example) HTTP and HTTPS endpoints are listed separately.
