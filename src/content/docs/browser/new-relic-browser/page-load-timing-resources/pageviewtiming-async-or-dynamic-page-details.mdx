...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        The `pageHide` event, available with [agent v1177 or higher](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1177), is sent when the document becomes hidden to the user. In modern practice, this signals the _potential end_ of an user session most reliably. This event always accompanies `windowUnload` if that occurs, but it can also fire separately when the user switches tab, for example, in which case the unload isn't fired.
