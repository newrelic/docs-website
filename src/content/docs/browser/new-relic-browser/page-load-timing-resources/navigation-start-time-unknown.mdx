...
If a browser does not use the [Navigation timing API](/docs/browser/new-relic-browser/browser-agent-apis/navigation-timing-api) for page load timing, the browser uses a cookie to determine the navigation start time. If a user navigates from a site without page load timing to a site <DoNotTranslate>**with**</DoNotTranslate> page load timing (for example, to the first page on your page load timing-enabled site), special considerations for navigation start time arise.
...
<DoNotTranslate>**Note:**</DoNotTranslate> For browsers that have the Navigation Timing API, this is not an issue, as the navigation start time can be determined without using a cookie.
