...
## Advanced debugging with payload capture

If you need to see the full request and response data for your AJAX calls to debug specific failures, you can enable [AJAX payload capture](/docs/browser/new-relic-browser/configuration/capture-ajax-payloads). This captures request/response headers, bodies, and query parameters, which is particularly helpful for debugging GraphQL APIs and other complex network interactions.
