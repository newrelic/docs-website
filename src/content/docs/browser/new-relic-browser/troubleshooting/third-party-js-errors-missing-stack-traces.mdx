...
The most common cause of the missing stack trace problem is the same-origin policy. This is a security policy that prevents one web resource from accessing data from another resource unless they both have the same origin. In this context, <DNT>**origin**</DNT> is the combination of the URI scheme, the hostname, and the port number. The policy prevents malicious code from gaining access to other web resources.
