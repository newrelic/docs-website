...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Required<DoNotTranslate>**.**</DoNotTranslate> When an error occurs, the callback is called with the error object as a parameter. The callback will be called with each error, so it is not specific to one error.
      </td>
    </tr>
  </tbody>
</table>
