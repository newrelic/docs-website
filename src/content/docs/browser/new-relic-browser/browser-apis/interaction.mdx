...
* <DNT>**Page load interaction:**</DNT> If the `targetPageLoad` option is used, the handle is always bound to the initial page load interaction regardless of any soft navigation that may be in progress.
  * Be extremely cautious using `waitForEnd: true` together with this option. That will keep the initial page load interaction open indefinitely until `.end()` is called, changing the default definition of the IPL. This is an advanced use case.
