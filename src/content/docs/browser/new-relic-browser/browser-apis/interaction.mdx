...
* The `waitForEnd` option requires v1.285.0+
* The `targetPageLoad` option requires v1.313.1+
...
* <DNT>**Page load interaction:**</DNT> If the `targetPageLoad` option is used, the handle is always bound to the initial page load interaction regardless of any soft navigation that may be in progress.
<Callout variant="caution">
  Using `waitForEnd: true` together with `targetPageLoad: true` will keep the initial page load interaction open indefinitely until `.end()` is called, changing the default definition of the IPL. This is an advanced use case and should be used with caution.
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `targetPageLoad` - Default is `false`. If set to `true`, the returned handle is bound to the initial page load interaction instead of creating or targeting a soft navigation interaction. This is the only way to target the initialPageLoad interaction.
      </td>
    </tr>
  </tbody>
</table>
