...
Upon executing this function with a valid value, the browser agent records the value as the `enduser.id` attribute with all events until the attribute is manually unset. The identifier will be stored in the browser, so that subsequent page visits of the _same_ origin attach it on events <DoNotTranslate>**within a session**</DoNotTranslate> span. Note that this functionality may fluctuate depending on end-user browser privacy settings. If this function is called with a `value = null`, any existing user ID will be deleted from <DoNotTranslate>**both**</DoNotTranslate> the current page's events and the storage.
