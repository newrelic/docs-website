...
For persist param or `null` value support, agent version 1.230.0 or higher is required.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        Passing a `null` value will act as an un-setting of any existing attribute by the same name.
    ...
    <tr>
      ...
      <td>
        Optional. If set to `true`, the name-value pair will also be set into the browser's storage API, and on the following instrumented page loads that is within the same session, the pair will be re-applied as a custom attribute.
