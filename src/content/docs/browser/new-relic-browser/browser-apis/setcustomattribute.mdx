...
With the `persist` flag, the attribute can also be in stored in the browser, so that subsequent page visits of the _same_ origin <DoNotTranslate>**within a session**</DoNotTranslate> span retain it on events. Do note that this functionality may fluctuate depending on end-user browser privacy settings. If this function is called with a `value = null`, the attribute will be deleted from <DoNotTranslate>**both**</DoNotTranslate> the current page's events and the storage, _regardless_ of the `persist` flag.
