...
This example records a PageAction event whenever a user selects the <DoNotTranslate>**Try Me**</DoNotTranslate> link. The event is recorded with an `actionName` of `clickedTryMe`:
...
You can then query the number of times the <DoNotTranslate>**Try Me**</DoNotTranslate> button was clicked with the following NRQL query:
...
This example captures user input (email addresses) from a form called <DoNotTranslate>**Signup**</DoNotTranslate>. The event is recorded with an `actionName` of `userSignup`:
