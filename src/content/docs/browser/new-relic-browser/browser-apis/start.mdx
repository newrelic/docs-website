...
This API will work for any browser edition (Browser Lite, Pro, or Pro+SPA). The configuration required to use this API is not currently connected to the larger deployment system within New Relic. As such, calling this API will only have an effect in Copy/Paste or NPM installations until further changes are made.
</Callout>
