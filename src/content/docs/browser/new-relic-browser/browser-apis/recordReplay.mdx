...
`newrelic.recordReplay()` can be called to manually force a replay to begin recording. All prerequisites of recording a replay must be met in order to succesfully trigger a replay to record.  This includes:
...
* The Session Replay feature is set to `autoStart: false` and has not been started
