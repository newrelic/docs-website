...
Browser monitoring supports the uploading of [source maps](/docs/new-relic-browser-source-maps), which are used to un-minify error stack traces on the [**Errors** page](/docs/errors-inbox/browser-tab). This document explains how to use the API to upload source maps using the browser API.
