...
<CollapserGroup>
  ...
  >
    ...
    curl -H "Api-Key: YOUR_NEW_RELIC_USER_API_KEY" \ 
        https://sourcemaps.service.newrelic.com/v2/applications/YOUR_NEW_RELIC_APP_ID/sourcemaps
    ```
    You can fetch upto `500` source maps at a time. By default, `20` source maps are retrieved. To retrieve a different number of source maps, append `?limit=<NEW-LIMIT>` to the end of the URL, where `<NEW-LIMIT>` is your desired limit. You can retrieve up to `500` source maps at a time.
