...
4. To view specific performance metrics, select any of the [attributes](#filter) below the funnel <Icon name="fe-filter"/> icon.
...
Charts on the **Key attribute charts** tabs automatically refresh with comparative performance details for the selected location. Charts provide insights into performance issues related to specific locations, browsers, devices, URLs, or browser versions.
...
Use the **Filter** <Icon name="fe-filter"/> to select and group data by attributes in various categories, including:
...
<CollapserGroup>
  <Collapser
    id="example-asn"
    title="Unsatisfactory network performance by ASN"
  >
  ...
  <Collapser
    id="example-promo"
    title="Page rendering performance during sales promotion"
  >
    ...
    7. To close the filter list, select the funnel <Icon name="fe-filter"/> icon.
