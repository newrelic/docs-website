...
- **Independent error tracking** with proper attribution
...
For applications using webpack module federation or similar technologies:
...
// Cleanup when the micro frontend unmounts
...
6. **Decorates telemetry** with the appropriate `parent.id` attributes necessary for cross-linking between the container and the MFE.
...
- **Performance timings** (when applicable to the micro frontend's initialization)
...
2. **Register each micro frontend** within your application using the `register` API in your component code
3. **Start using the returned APIs** to capture micro frontend-specific data
4. **Find your new entities** in New Relic under "Browser" > "Micro Front Ends"
5. **Query your data and set up dashboards and alerts** for your individual micro frontend entities
