---
title: "Browser agent third party cookies (deprecated)"
tags:
  - Browser
  - Browser monitoring
...
The browser agent uses the sessionStorage API to set a timestamped identifier in the end user's browser local storage. This action enables session tracking, which is documented in [our doc about cookie collection and session tracking](/docs/browser/browser-monitoring/page-load-timing-resources/cookie-collection-session-tracking/). That page also includes instructions on how to disable session tracking for an app.
