...
* The browser monitoring `licenseKey` (`browser_monitoring_key`) is a unique key that is linked to, but is not the same as, your main [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). The browser key can't be used to determine your account's license key.
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > Settings**.
2. Select and copy the generated JavaScript, and paste it into a text file.
3. Delete everything in the text file except the browser monitoring [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) and app ID information (just above the ending `</script>` tag), and save.
