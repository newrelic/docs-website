...
* The browser monitoring `licenseKey` (`browser_monitoring_key`) is a unique key that is linked to, but is not the same as, your main <LicenseKey />. The browser key can't be used to determine your account's license key.
...
3. Delete everything in the text file except the browser monitoring <LicenseKey /> and app ID information (just above the ending `</script>` tag), and save.
