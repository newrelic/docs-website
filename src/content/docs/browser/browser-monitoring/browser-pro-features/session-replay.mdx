...
By default, all user input is replaced with asterisks. Here's an example of how a replay looks with default privacy:
...
If you need to customize what content or user input is captured, you have options to mask, block, or ignore specific classes and attributes. Make sure to check with your privacy team before changing the privacy settings.
...
You can ignore changes to a user input field, which means the input field will still be displayed in the replay, but you won't display changes to the value. For example, you could ignore the class containing the email address and password fields.
...
If needed, you can programmatically invoke a session replay using API methods. Here are some use cases:
