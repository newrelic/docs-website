...
Is New Relic's session replay feature compliant with data privacy laws (for example, GDPR/ CCPA)? New Relic acts as a data processor under the GDPR and as a service provider under the CCPA. As such, we process data according to your instructions. Compliance with either law is a matter for you as the New Relic customer and depends on how you deploy session replay. 

If you enable session replay, we'll capture your customers' end-user experiences with your site or application (for example, recording events as visitors interact with your site or application). By default, all text and user inputs, including visitors' personal data or information in the screen capture will be masked. We do offer you the ability to selectively unmask elements if that is what you prefer, but the default setting is that none of the elements on the screen will be readily identifiable in the screen capture.
