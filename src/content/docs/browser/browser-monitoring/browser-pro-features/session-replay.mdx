...
<Callout title="Important">
  This feature is in limited preview. While not everyone can access the Session Replay just yet, you can request access to the preview [here](https://newrelic.com/platform/session-replay-early-access).
</Callout>
...
### Not seeing data? [#troubleshoot-missing-data]

If you have enabled the session replay feature but you are not seeing any data, here are some possible reasons:
...
* Is your agent version v1.240.0 or greater? We recommend updating to the latest agent version as many improvements have been added.
...
* If you're using the copy/paste deployment method, did you deploy your updated snippet?
...
### Replays not displaying correctly [#troubleshoot-replay-display]

Experiencing issues with replays not displaying correctly?  If your CSS files are hosted on a different domain than your site, you may need to make a quick adjustment to ensure they're accessible for replay recording. Here's a breakdown of what's happening and how to fix it:

#### How replays recreate the user experience

Replays function by rebuilding the end user's DOM (Document Object Model), which includes your website's structure and visual elements. To achieve this, our agent needs to access your CSS files to accurately capture styling information. We then inline these styles directly into the replay recording, eliminating the need to reference the original CSS files during playback.

#### Potential CORS issue

If your CSS files are hosted on a separate domain, Cross-Origin Resource Sharing (CORS) security measures may prevent our agent from accessing them. This can result in incomplete or inaccurate replays, as essential styling information may be missing.

#### Solution

To resolve this, simply add the `crossorigin="anonymous"` attribute to your `<link rel="stylesheet">` elements within your HTML code. This attribute instructs the browser to allow cross-origin access for those specific CSS files, enabling our agent to successfully record and integrate the styling information.

Here's an example of `yoursite.com` referencing stylesheets hosted on `assets.yoursite.com`:
