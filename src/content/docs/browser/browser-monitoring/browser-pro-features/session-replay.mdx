...
We take data privacy seriously, so we built session replay with robust default privacy settings, designed to limit the potential sharing of sensitive customer information. To meet your specific privacy requirements, see [Configure privacy settings](#configure-privacy-settings).
...
*Is New Relic’s Session Replay feature compliant with data privacy laws- e.g., GDPR/ CCPA?*

New Relic acts as a data processor under the GDPR, and as a service provider under the CCPA and processes data according to our customer's instructions. Compliance with either law is a matter for the New Relic customer, and will depend on how the customer deploys Session Replay. If a New Relic customer enables Session Replay, New Relic will capture a customer’s end user's experiences with that customer’s site or application, i.e., what actions that visitor takes on their site/ application. By default, all text and user inputs, including that visitor’s personal data/ information in the screen capture will be masked. We do offer customers the ability to selectively unmask elements if that is what they choose, but the default setting is that none of the elements on the screen will be readily identifiable in the screen capture.
