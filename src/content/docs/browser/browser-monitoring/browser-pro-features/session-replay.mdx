...
<Steps>
  ...
  <Step>
    ...
    Set a sampling rate (0%-100%) for the following samples:
        ...
        * <DoNotTranslate>**Errored session**</DoNotTranslate> records a random sample of all errored sessions. The agent buffers a sliding window of data for up to 30 seconds on the client-side, and if errors occur, the buffered data will be harvested and the remainder of the session will be recorded. If an error does not occur, the replay is never harvested. 
...
* Make sure your browser agent meets these requirements:
  ...
  * Session Tracking and Session Trace features are enabled.
* Your end-user's browser supports the browser-level APIs needed to record a session replay
...
If needed, you can programmatically `force` a session replay to record or stop recording using API methods. Here are some use cases:
...
* Allow users to opt in for interaction recording. For example, if you have a pop-up asking "Allow this session to be recorded for performance analysis?":
    1. Upon user consent, call `newrelic.recordReplay()` to initiate recording. 
...
## Prevent session replays from automatically tracking [#auto-replays]

If needed, session replays can be prevented from automatically starting, `even when sampled`.  Here are some use cases:

* Allow users to opt in for interaction recording. For example, if you have a pop-up asking "Allow this session to be recorded for performance analysis?":
    1. Upon user consent, call `newrelic.start()` to import the session replay recording module.
* Record sessions based on specific criteria. For example:
    * Start recording only for sessions on specific URLs such as a product checkout flow.

<Callout variant="important">
  Calling `newrelic.start()` does not guarantee that a session replay will be recorded, only that it will wait until this moment to import the module and evaluate the sampling results.  If you need to force a recording, see [Manually record session replays](#manual-replays)
</Callout>
