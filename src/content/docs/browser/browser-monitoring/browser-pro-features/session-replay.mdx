...
<Callout variant="important">
  Some settings have been moved server side for easier use. Both enabling session replay on/off and its sample rates (overall and errors) are server side. This means if you are using copy/paste install method, you do not need to re-deploy the script for these settings changes. This applies to browser agent 1.259.0 or higher.
</Callout>
