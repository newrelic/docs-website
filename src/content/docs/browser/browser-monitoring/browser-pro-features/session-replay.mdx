...
Session replay consumption will be billable starting May 8, 2024. See [Data consumption](#view-consumption) for details.
</Callout>
...
Here's how a sample session replay looks:
...
Data retention depends on your account. By default, browser event data is stored 8 days, but DataPlus customers have 98 days, and others may have even longer retentions.
...
* [Impact on app performance](#performance-impact): Learn how session replay affects your browser app performance
...
New Relic takes your data privacy seriously. The default privacy setting masks everything by replacing all text with asterisks. Here's an example of how a replay looks with default settings:
...
If you need to change the default privacy settings, you can choose specific classes and attributes to:

* Mask user input
* Block site content
* Ignore user input

### Mask user input [#mask-input]

You can mask user input, which means the input will be replaced with asterisks (*). For example, a user's login information will render as asterisks in session replay.
...
You can ignore user input entirely, which means the input field won't show up at all in session replay. For example, if you want to ignore a user's email address and password, you could ignore the element that contains those fields.
...
* The agent attempts to collect all CSS, but if it can't, it sends the replay with broken styles along with attributes `hasCrossOriginCSS:true` and `inlinedCrossOriginCSS:false` so the session replay UI knows some styles are broken and provides a warning message with a link to a fix. The agent is set up to report:
    * `invalidStylesheetsDetected` will resolve to true if we find cross-origin stylesheets
    * `inlinedAllStylesheets` will resolve to false if any stylesheets weren’t able to be inlined by the agent. If false, you'll see a message in the UI to fix it.
...
If needed, you can programmatically invoke a session replay using these two API methods:
...
* Allow users to opt in for interaction recording. For example, if you have a pop-up asking "Allow this session to be recorded for performance analysis?":
    1. Upon user consent, call `NREM.startSessionReplay` to initiate recording. 
    2. Once the session is complete or consent is withdrawn, use `NREM.stopSessionReplay` to stop capturing data.
* Record sessions based on specific criteria. For example:
...
To ensure all user interactions are captured during manual recording, set sampling rates to 0% in your application settings.
...
Between April 8 and May 8, 2024, while session replay bytes are free, you can view your consumption to help with planning once the bytes become paid:
...
After May 8, 2024, session replay will follow the same consumption pricing as your other browser bytes. The amount of bytes produced depends on the count, length, and user-activity levels of sessions, as well as the complexity of your site's DOM.
...
We take data privacy seriously, so we built session replay with robust default privacy settings, designed to limit the potential sharing of sensitive customer information. These settings [can be further customized](#configure-privacy-settings) to meet your privacy requirements.
