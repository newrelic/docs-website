...
Replays are not displaying correctly if your CSS files are hosted on a separate domain. This is because Cross-Origin Resource Sharing (CORS) security measures may prevent our agent from accessing the files. This can result in incomplete or inaccurate replays, as essential styling information may be missing.
