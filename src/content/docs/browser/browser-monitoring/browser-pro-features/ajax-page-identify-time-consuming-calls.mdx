...
<CollapserGroup>
  ...
  <Collapser
    id="payload-capture"
    title="Inspect request and response payloads"
  >
    <Callout variant="important">
      Payload capture requires browser agent version **TBD `{{INFORMATION_NEEDED}}`** or higher.
    </Callout>

    When metadata like URL, status code, and duration aren't enough to diagnose a failure, payload capture gives you the full picture. It adds four new attributes to each `AjaxRequest` event, each truncated at 4KB and queryable in NRQL:
    ...
    * `response.body`
    ...
    To enable payload capture from the UI:
    ...
    2. In the <DNT>**AJAX Request Payload Capture**</DNT> section, select the capture mode from the following options:
      * `none`: No payloads are captured. This is the default.
      * `failures`: Captures payloads only for requests with error status codes (4xx, 5xx).
      * `all`: Captures payloads for all requests, including successful ones.
    3. Select <DNT>**Save application settings**</DNT>.

    ### How payload capture interacts with the deny list [#deny-list-interaction]

    Payload capture respects your existing [AJAX request deny list](/docs/browser/new-relic-browser/configuration/filter-ajax-request-events/). For any AJAX request:
    ...
    * If the URL is not on the deny list, **capture mode** determines whether payloads are captured alongside the standard metadata.
    ...
    The browser agent automatically obfuscates common sensitive data patterns in captured payloads before ingestion, including credit card numbers, Social Security Numbers, and common keys like `password` and `token`. You can also configure custom regex rules to mask business-specific sensitive data. For more information, see [Obfuscate Browser Agent Data](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/).
...
* Control which URLs are eligible for payload capture by configuring the [AJAX request deny list](/docs/browser/new-relic-browser/configuration/filter-ajax-request-events/).
