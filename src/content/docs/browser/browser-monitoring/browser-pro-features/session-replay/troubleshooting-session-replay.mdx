...
## Payload size or cost higher than expected [#high-data-volume]

### Problem

  Session replay is working as expected, but your data volume or cost is higher than you anticipated.

### Solution

  Payload size depends on your site's DOM complexity and how often it mutates, not just how many sessions you record. See [Optimize session replay data volume](/docs/browser/browser-monitoring/browser-pro-features/session-replay/optimize-session-replay-data-volume) for how to identify and reduce the patterns that drive up payload size, such as inline SVG-heavy widgets and high-frequency DOM mutations.
