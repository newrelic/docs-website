...
For guidance on reducing payload size at the source — such as inline SVG usage and high-frequency DOM mutations — see [Optimize session replay data volume](/docs/browser/browser-monitoring/browser-pro-features/session-replay/optimize-session-replay-data-volume).
