...
redirects:
  - /docs/browser/browser-monitoring/browser-agent-spa-api/session-replay-privacy-settings/
...
<Callout variant="important">
  **You must redeploy after changing privacy settings**

  Changes you make in the New Relic UI won't take effect until you redeploy your application:

  * **Copy/paste snippet**: Update and redeploy the JavaScript snippet in your app's source code
  * **NPM installations**: Redeploy your app to pick up the new configuration  
  * **Server-side installations**: Restart your app server so the agent picks up updated settings
  * **Exception**: Sampling rates and on/off toggles work immediately without redeployment

  **Remember**: Once data's masked during capture, you can't unmask it later—even if you change your privacy settings.
</Callout>
...
* **Custom privacy settings**: This mode provides granular control over two distinct areas:
  - **Text-masking and blocking**: Controls static text, labels, buttons, and general page content. You manage these using the <DNT>Mask selectors</DNT> and <DNT>Block selectors</DNT> fields.
  - **User input masking**: Controls interactive form elements. You manage these using checkboxes for specific input types, such as text, email, numbers, and dates.
    ...
    <Callout variant="important">
      ...
      Clearing all <DNT>User input masking</DNT> checkboxes only unmasks form inputs. Your static text (such as labels and buttons) remains masked unless you also configure the <DNT>Mask selectors</DNT> field.
    </Callout>


## Mask specific PII [#unmask-everything]

If you want to display all static text (such as labels, buttons, and headings) and only mask specific sensitive user inputs (such as credit card numbers or SSNs), follow these steps:
...
4. On the <DNT>**Privacy settings**</DNT> section, select <DNT>Custom privacy settings</DNT>.

5. In the <DNT>Text-masking and blocking</DNT> section, clear the <DNT>Mask selectors</DNT> and <DNT>Block selectors</DNT> fields. These fields only appear when you use custom privacy mode.
...
* In the UI: On the <DNT>**Application settings**</DNT> page, add CSS selectors to the <DNT>Mask selectors</DNT> field. To unmask all static text, clear this field.
...
## Troubleshooting privacy settings [#troubleshooting]

Privacy changes not working? Try these troubleshooting steps:

### Verify redeployment
1. **Check that you've redeployed** your app after making privacy setting changes in the New Relic UI
2. **Verify the configuration's** in your browser agent initialization code
3. **Clear browser cache** or test in an incognito/private window to make sure you're not seeing a cached version of your previous snippet

### Common issues
* **Settings not applying**: Most privacy setting changes need redeployment to work
* **Static text still masked**: Unchecking user input checkboxes only affects form inputs—use the "Mask selectors" and "Block selectors" fields to control static text
* **Cache issues**: Your browser might be running a cached version of the previous snippet—test in incognito mode

<Callout variant="tip">
  **Verification tip**: Use incognito or private browsing mode to test privacy changes. This makes sure your browser isn't loading a cached version of your JavaScript snippet.
</Callout>