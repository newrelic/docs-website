...
<Callout variant="important">
  ...
  To apply updates made in the <DNT>New Relic</DNT> platform for <DNT>Privacy settings</DNT>, you must update your browser agent configuration:
...
* <DNT>**Default privacy settings (recommended)**</DNT>: This mode masks all page content by replacing all text and inputs with asterisks (\*)
* <DNT>**Custom privacy settings**</DNT>: This mode provides granular control over two distinct areas:
  - <DNT>**Text-masking and blocking**</DNT>: Controls static text, labels, buttons, and general page content. You manage these using the <DNT>Mask selectors</DNT> and <DNT>Block selectors</DNT> fields.
  - <DNT>**User input masking**</DNT>: Controls interactive form elements. You manage these using checkboxes for specific input types, such as text, email, numbers, and dates.
...
5. In the <DNT>Text-masking and blocking</DNT> section, clear the <DNT>Mask selectors</DNT> and <DNT>Block selectors</DNT> fields. This will unmask all static text on your site, such as labels and buttons.

6. In the <DNT>User input masking</DNT> section:
...
## Block site content [#block-content]
  ...
  * Manually add our CSS class `nr-block` or attribute `data-nr-block` to your webpage HTML. For example, if you wanted to block irrelevant text and images from session replay, add `nr-block` to `<div class>`:
  ...
  Blocking content also reduces the amount of data captured, since blocked elements and their children are excluded from the recording entirely. This makes it useful for high-churn decorative elements, such as animations or carousels, in addition to privacy-sensitive content. See [Optimize session replay data volume](/docs/browser/browser-monitoring/browser-pro-features/session-replay/optimize-session-replay-data-volume/#frequent-mutations) for details.
...
## Ignore user input [#ignore-input]
  ...
  Ignoring a high-frequency input field also reduces the number of mutation events captured for it, which can help if a field's value changes often but the value itself doesn't matter for replay. See [Optimize session replay data volume](/docs/browser/browser-monitoring/browser-pro-features/session-replay/optimize-session-replay-data-volume/#frequent-mutations) for details.
