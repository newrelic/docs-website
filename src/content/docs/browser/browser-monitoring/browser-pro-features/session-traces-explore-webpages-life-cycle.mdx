...
<figcaption>
  Go to **[one.newrelic.com](https://one.newrelic.com) > Browser > (select an app) > Session traces**: Search by URL or session trace ID, or sort any column, then click the link to use the interactive session trace details.
</figcaption>
