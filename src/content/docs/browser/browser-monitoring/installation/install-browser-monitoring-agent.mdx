...
To get started enabling browser: Go to **[one.newrelic.com](https://one.newrelic.com)**, select **Browser**, and then select **Add more data**. The UI gives you instructions. 
...
We have three types of browser agent: Lite, Pro, and Pro + SPA. Note that in this context, "Pro" is not referring to our [Pro pricing edition](https://newrelic.com/pricing) and is not directly tied to any pricing tiers: it is just a name for the agent. Details about the agent types: 

* Pro + SPA: This is the default installed agent when using the [enable procedures](#enable). After install, you have the option to downgrade to the less advanced agents. This gives you access to the [Browser Pro features](/docs/browser/new-relic-browser/browser-pro-features) and to [Single Page App (SPA) monitoring](/docs/browser/single-page-app-monitoring/get-started/introduction-single-page-app-monitoring/), which is beneficial for getting detailed page timing data for all app types, and not only useful for single page applications. 
* Pro: Gives you access to the [Browser Pro features](/docs/browser/new-relic-browser/browser-pro-features).
* Lite: Gives you information about some basic page load timing and browser user information, but lacks the Browser Pro features and SPA.  
