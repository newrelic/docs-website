...
To install the browser agent: 

1. Go to [one.newrelic.com](https://one.newrelic.com) and click **Add data**.
2. On the left, click **Browser and mobile**. 
3. Click **Browser monitoring** and complete the instructions.

Hav questions about the install process? See [Install methods](#choose-deployment).

## Look for your data in New Relic [#generate-traffic]

When you're done installing the browser agent, wait a few minutes and see if data is present in [our browser monitoring UI](/docs/browser/browser-monitoring/getting-started/browser-summary-page).
...
* APM + browser monitoring: you can use an APM agent to get browser monitoring 
* Browser monitoring only: Add our JavaScript snippet to your front-end HTML 

Here's some guidance on these two methods. 

### Use APM agent [#apm-agent]
...
### Add JavaScript code [#copy-paste]

Use this method if you can't use APM for whatever reason (for example, if you have a [SPA](/docs/browser/single-page-app-monitoring/get-started/introduction-single-page-app-monitoring) app, or if your APM agent isn't supported for auto-injection, as is true for Node.js and Java). When you go this route, you'll receive a customized JavaScript snippet at the end of the install process in the New Relic UI, and you'll add that to the HTML of any pages you want to monitor in your app/site.
...
You can use our APM agent APIs to add the JavaScript snippet and customize its behavior. Check out these agent-specific instructions:
