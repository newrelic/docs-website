...
redirects:
  - /docs/opentelemetry/nginx-plus/nginx-plus-otel
...
- Network access from the Linux host to:
    ...
    - A [New Relic OTLP](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) endpoint
...
### Configure NGINX Plus [#configure-nginx-nrdot]
...
<CollapserGroup>
  <Collapser id="step1-enable-api" title="Step 1: Enable HTTP API module">
    Add the HTTP API configuration to your `nginx.conf` file.
  ...
  <Collapser id="step1-test-reload" title="Step 2: Test and reload configuration">
  ...
  <Collapser id="step1-verify-api" title="Step 3: Verify API endpoint">
    Confirm the API endpoint path (including version) exposed in your configuration. Record the full API endpoint path (without the leading slash) and the port that serves it. Common defaults are `api/9` on port `<YOUR_LOCAL_PORT>`.

    Use `curl` to confirm your API endpoint is reachable:

    ```bash
    curl -I http://<NGINX_PLUS_SERVER_IP>:<YOUR_LOCAL_PORT>/api/9 2>/dev/null | head -n 1 | cut -d$' ' -f1,2
    ```
...
### Install and configure the NRDOT collector [#configure-nrdot-collector]

Configure the NRDOT collector to scrape metrics from your NGINX Plus Prometheus exporter and send them to New Relic.

<CollapserGroup>
  <Collapser id="step2-install-exporter-nrdot" title="Step 1: Install NGINX Prometheus exporter">
    ...
    nginx-prometheus-exporter -nginx.scrape-uri=http://<NGINX_PLUS_SERVER_IP>:<YOUR_LOCAL_PORT>/api/9
    ```

    By default, the exporter runs on port `9113` and exposes metrics at `/metrics`.
  </Collapser>

  <Collapser id="step2-config-collector-nrdot" title="Step 2: Configure collector configuration">
      ...
      config:
        scrape_configs:
          - job_name: 'nginx-plus'
            scrape_interval: 30s
            static_configs:
              - targets: ['<NGINX_PLUS_SERVER_IP>:<EXPORTER_PORT>']
  processors:
    batch:
    ...
    resource/nginx:
      attributes:
        - key: nginx.server.endpoint
          value: "http://<NGINX_PLUS_SERVER_IP>:<YOUR_LOCAL_PORT>/api/9"  # Replace with your endpoint
          action: insert
...
<tbody>
  <tr>
    <td>`<NGINX_PLUS_SERVER_IP>`</td>
    <td>Enter your NGINX Plus server's IP address (for example `127.0.0.1` for local server)</td>
  </tr>
  <tr>
    <td>`<YOUR_LOCAL_PORT>`</td>
    <td>Enter your NGINX Plus API port number (for example `8080`, `8081`)</td>
  </tr>
  <tr>
    <td>`<EXPORTER_PORT>`</td>
    <td>Enter the Prometheus exporter port number (default is `9113`)</td>
  </tr>
  <tr>
    <td>`<DEPLOYMENT_NAME>`</td>
    <td>Replace with a unique deployment name for this NGINX Plus server (for example `production-web-01`, `staging-api`, `prod-lb-01`)</td>
  </tr>
  <tr>
  ...
  <Collapser id="step2-enable-additional-metrics-nrdot" title="Step 3: (Optional) Enable additional metrics">
    ...
    2. Move the corresponding metric prefix from `exclude` to `include` in the `filter/nginx_metrics` processor.
      ...
      1. Find this line in the `exclude` section:
        ```yaml
        filter/nginx_metrics:
          metrics:
            include:
      ...
      2. Move it to the `include` section:
        ```yaml
        filter/nginx_metrics:
          metrics:
            include:
...
Determine your OTLP endpoint based on your New Relic region. See [Configure endpoint, port, and protocol](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) for the complete list of endpoints and supported ports for your region.
...
<CollapserGroup>
  <Collapser id="configure-nginx-log-format" title="Step 1: Configure NGINX Plus log format">
  ...
  <Collapser id="configure-nrdot-log-forwarding" title="Step 2: Configure the NRDOT collector for log forwarding">
...
<CollapserGroup>
  <Collapser id="step4-start-collector-nrdot" title="Step 1: Start the collector">
  ...
  <Collapser id="step4-verify-data-nrdot" title="Step 2: Verify data collection">
...
### Configure NGINX Plus [#configure-nginx-otelcol]

Configure and enable the [HTTP API module](https://nginx.org/en/docs/http/ngx_http_api_module.html) to expose metrics from your NGINX Plus server.

<CollapserGroup>
  <Collapser id="step1-enable-api-otelcol" title="Step 1: Enable HTTP API module">
  ...
  <Collapser id="step1-test-reload-otelcol" title="Step 2: Test and reload configuration">
    After updating `nginx.conf`, test and reload the service:
  ...
  <Collapser id="step1-verify-api-otelcol" title="Step 3: Verify API endpoint">
    Confirm the API endpoint path (including version) exposed in your configuration. Record the full API endpoint path (without the leading slash) and the port that serves it. Common defaults are `api/9` on port `<YOUR_LOCAL_PORT>`.
...
<CollapserGroup>
  <Collapser id="step2-install-exporter-otelcol" title="Step 1: Install NGINX Prometheus exporter">
    Install and configure the [NGINX Prometheus exporter](https://github.com/nginx/nginx-prometheus-exporter) to expose NGINX Plus API metrics in Prometheus format:
  ...
  <Collapser id="step2-config-collector-otelcol" title="Step 2: Configure collector configuration">
...
<tbody>
  ...
  <tr>
    <td>`scrape_interval`</td>
    <td>Interval to collect metrics. The default value is set to `30s`</td>
  </tr>
  <tr>
    <td>`NEW_RELIC_LICENSE_KEY`</td>
    <td>Enter your New Relic license key</td>
  ...
  <tr>
    <td>`timeout`</td>
    <td>Maximum time to wait before sending a batch. The default value is set to `30s`</td>
  </tr>
  <tr>
    <td>`nginx.display.name`</td>
    <td>Display name format for NGINX server in New Relic (automatically generated as "server:`<DEPLOYMENT_NAME>`")</td>
  </tr>
</tbody>
</table>
  ...
  <Collapser id="step2-enable-additional-metrics-otelcol" title="Step 3: (Optional) Enable additional metrics">
...
<CollapserGroup>
  <Collapser id="configure-nginx-log-format-otelcol" title="Step 1: Configure NGINX Plus log format">
    Before forwarding logs, configure NGINX Plus to use a structured log format. Refer to the [NGINX logging documentation](https://docs.nginx.com/nginx/admin-guide/monitoring/logging/) for guidance on configuring access and error logs.
  </Collapser>

  <Collapser id="configure-otelcol-log-forwarding" title="Step 2: Configure the OpenTelemetry Collector for log forwarding">
    1. Note the full paths to your NGINX access and error log files. Defaults are usually `/var/log/nginx/access.log` and `/var/log/nginx/error.log`.
...
<CollapserGroup>
  <Collapser id="step4-start-collector-otelcol" title="Step 1: Start the collector">
  ...
  <Collapser id="step4-verify-data-otelcol" title="Step 2: Verify data collection">
    3. Check startup logs:
       ```bash
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            The type of log being collected (applicable to logs only). Used by New Relic's built-in [parsing rules](/docs/logs/ui-data/built-log-parsing-rules/#nginx). This attribute is only available when log forwarding is enabled.
          </td>
          <td>
...
- [Monitor self-hosted NGINX with OpenTelemetry](/docs/opentelemetry/integrations/nginx/nginx-otel-host/) - For standard NGINX deployments
- [Monitor NGINX on Kubernetes with OpenTelemetry](/docs/opentelemetry/integrations/nginx/nginx-otel-kubernetes/) - For containerized environments
