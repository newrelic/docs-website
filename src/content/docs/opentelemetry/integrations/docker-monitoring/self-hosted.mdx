...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="nrdot-install">
      ...
      Documentation on installation can be found in the [installation section](https://github.com/newrelic/nrdot-collector-releases/blob/main/distributions/README.md#installation) of the nrdot-collector-releases repository.
    </TabsPageItem>
...
Save the file and ensure the `nrdot-collector` system user can read it. The NRDOT Collector will merge this configuration with its default config when started with multiple `--config` flags (configured in Step 4).
    </TabsPageItem>
...
<Collapser
  id="configure-logs"
  title="Step 3: Optional - Configure log collection"
>
...
<Collapser
  ...
  title="Step 5: Restart the collector"
>
