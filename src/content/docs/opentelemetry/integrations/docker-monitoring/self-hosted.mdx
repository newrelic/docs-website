...
<Collapser
  ...
  title="Step 4: Configure authentication and start the collector"
>
    ...
    <TabsPageItem id="nrdot-env">
      ...
      **Setup steps:**
      ...
      2. Create `environment.conf` with your configuration. Replace `YOUR_LICENSE_KEY` with your New Relic license key and `YOUR_OTLP_ENDPOINT` with the appropriate endpoint for your region:
         ...
         Environment="OTELCOL_OPTIONS=--config=/etc/nrdot-collector/config.yaml --config=/etc/nrdot-collector/docker-config.yaml"
         EOF
         ```
      ...
      <Callout variant="tip">
        The `OTELCOL_OPTIONS` variable loads both the default NRDOT configuration and your Docker-specific configuration. The configs are [merged together](https://github.com/newrelic/nrdot-collector-releases/blob/main/distributions/README.md#configuration) with later configs overriding earlier ones. Add additional `--config` flags for log collection if needed.
      </Callout>
    </TabsPageItem>
