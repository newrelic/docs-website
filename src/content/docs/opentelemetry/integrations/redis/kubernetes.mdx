...
Redis Cluster monitoring currently requires the Prometheus receiver approach (using `redis_exporter`). The NRDOT Collector's native Redis receiver does not yet support the `CLUSTER INFO` command needed for cluster metrics. Use the [Prometheus receiver](/docs/opentelemetry/integrations/redis/kubernetes/#prometheus) tab for cluster monitoring setup.
...
Redis Cluster monitoring currently requires the Prometheus receiver approach (using `redis_exporter`). The OTel Collector Contrib's native Redis receiver does not yet support the `CLUSTER INFO` command needed for cluster metrics. Use the [Prometheus receiver](/docs/opentelemetry/integrations/redis/kubernetes/#prometheus) tab for cluster monitoring setup.
