...
Add the hostmetrics receiver to your `otel-collector-config.yaml`. The `root_path: /hostfs` setting is required when running inside a Docker container so the collector reads the host filesystem (not the container's):
