...
To see all unique metric names arriving for your cluster:

```sql
FROM Metric SELECT uniques(metricName) WHERE kafka.cluster.name = 'my-kafka-cluster' SINCE 30 minutes ago
```
...
To see throughput by topic and operation:

```sql
FROM Span SELECT count(*) WHERE kafka.cluster.name = 'my-kafka-cluster'
FACET messaging.destination.name, messaging.operation SINCE 30 minutes ago
```
