...
<img
  title="Self-hosted Kafka monitoring architecture with OpenTelemetry"
  src="/images/otel-kafka-self-host-architecture.webp"
  alt="Self-hosted Kafka monitoring architecture with OpenTelemetry"
/>
...
* OTLP endpoint: Uses `https://otlp.nr-data.net:4317` (US region), `https://otlp.eu01.nr-data.net:4317` (EU region), or `https://otlp.jp.nr-data.net:4317` (JP region).
...
## Advanced: Customize metrics collection [#customize-metrics]
...
<CollapserGroup>
  ...
  >
    ...
    **1. Remove the Additional metrics section from the JMX config**
    ...
    In `~/opentelemetry/jmx-custom-config.yaml`, delete everything below this comment (through the end of the file):
    ...
    **2. Disable consumer offset metrics in the kafkametrics receiver**
    ...
    In `~/opentelemetry/kafka-config.yaml`, add to the `kafkametrics` receiver `metrics` section:
