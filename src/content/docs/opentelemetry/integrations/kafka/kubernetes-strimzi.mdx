...
* Kafka deployed via [Strimzi operator](https://strimzi.io/) with JMX enabled. Ensure your Kafka cluster has JMX enabled in the Strimzi Kafka resource:
...
### Create secret with license key [#create-secret]
...
### Deploy OpenTelemetry Collector [#deploy-collector]

#### 3.1 Build custom collector image [#build-image]

Create a custom collector image with Java runtime and JMX scraper. Choose between NRDOT Collector (New Relic distribution) or OpenTelemetry Collector:
  ...
  <TabsPages>
    <TabsPageItem id="nrdot">
      ...
      <Callout variant="important">
        **Version Compatibility**: This guide uses JMX Scraper 1.52.0 and NRDOT Collector. For best results, use the latest versions.

        <CollapserGroup>
          <Collapser
            id="nrdot-version-compatibility"
            title="Check for latest NRDOT Collector version"
          >
            **Check for latest versions**:
            - NRDOT Collector: Visit [NRDOT Collector releases](https://github.com/newrelic/nrdot-collector-releases/releases/latest) to find the latest version and replace `{NRDOT_VERSION}` in the Dockerfile
        ...
        **Target Architecture**: The example below uses `linux_amd64`. Refer to the [NRDOT Collector releases](https://github.com/newrelic/nrdot-collector-releases/releases/latest) page to find the correct binary for your system architecture (e.g., `linux_amd64`, `linux_arm64`). Update the download URL in the Dockerfile accordingly.
      </Callout>
    ...
    <TabsPageItem id="otel">
      <Callout variant="important">
        **Version Compatibility**: This guide uses JMX Scraper 1.52.0 and OpenTelemetry Collector 0.145.0. Older collector versions may not include this scraper's hash in their compatibility list. For best results, use the latest versions as shown in this guide.
...
After saving the Dockerfile, build and push the image to your container registry:
