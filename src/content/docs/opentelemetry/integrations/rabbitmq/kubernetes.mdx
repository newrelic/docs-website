...
Before deploying the integration, ensure your environment meets these requirements:
...
To collect logs from RabbitMQ pods in addition to metrics, you can add a Kubernetes logs receiver to your collector configuration.

<Callout variant="important">
  Log collection requires additional RBAC permissions for the collector. The configuration below includes the necessary permissions.
</Callout>

**Update your `otel-collector-values.yaml`** to add log collection:
...
opentelemetry-collector:
  ...
  config:
    # ... existing extensions and receivers ...

    receivers:
      receiver_creator/rabbitmq:
        # ... existing rabbitmq receiver ...

      # Add Kubernetes logs receiver
      k8scluster:
        auth_type: serviceAccount
    ...
    service:
      # ... existing extensions and pipelines ...
      ...
      pipelines:
        ...
        logs/rabbitmq:
          ...
          processors: [k8sattributes, batch, transform/rabbitmq_logs]
          exporters: [otlphttp]
```
...
```bash
helm upgrade rabbitmq-otel-collector \
  open-telemetry/opentelemetry-collector \
  --namespace newrelic \
  --values otel-collector-values.yaml
```
