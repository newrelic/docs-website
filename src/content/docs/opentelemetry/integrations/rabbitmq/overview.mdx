...
## Choose your installation method [#deployment-options]
...
<DocTiles>
  ...
  <DocTile title="Kubernetes RabbitMQ" path="/docs/opentelemetry/integrations/rabbitmq/kubernetes">
    **Kubernetes clusters**: Deploy with Helm and automatic pod discovery for containerized RabbitMQ
  </DocTile>
</DocTiles>
...
<CollapserGroup>
  ...
  <Collapser id="k8s-req" title="Kubernetes requirements">
    For RabbitMQ running in Kubernetes:

    * `kubectl` access to your target cluster
    * Helm 3.x installed on your local machine
    * RBAC permissions to create ServiceAccounts, ClusterRoles, and ClusterRoleBindings
    * RabbitMQ management plugin enabled in your pods
  </Collapser>
</CollapserGroup>

## Explore your data in New Relic [#find-data]
...
3. **<DNT>Dashboards</DNT>**  
   Build custom visualizations with your RabbitMQ data
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * `rabbitmq.server.endpoint` - Management API URL
        * `host.name` / `k8s.pod.name` - Instance identifier
      </td>
    </tr>
  </tbody>
</table>
...
Both self-hosted and Kubernetes deployments support forwarding RabbitMQ logs to New Relic. See the deployment-specific guides for configuration details.
...
<CollapserGroup>
  <Collapser id="no-data" title="No data appearing in New Relic">
    ...
    **Review collector logs:**
    ...
    # Kubernetes
    kubectl logs deploy/rabbitmq-otel-collector -n newrelic
    ```
  ...
  <Collapser id="connection-errors" title="Connection errors">
    ...
    **Test network connectivity:**
    ...
    # Kubernetes
    kubectl exec -it <collector-pod> -n newrelic -- curl http://<rabbitmq-service>:15672
    ```
...
<DocTiles>
  ...
  <DocTile title="Kubernetes installation" path="/docs/opentelemetry/integrations/rabbitmq/kubernetes">
    Deploy with Helm and automatic pod discovery
  </DocTile>
