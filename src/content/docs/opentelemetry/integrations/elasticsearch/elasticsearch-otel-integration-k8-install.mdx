...
* **Cluster name** - In `resource/cluster_name_override`, replace `elasticsearch-cluster` with your Elasticsearch `cluster.name`; it **must match** the name Elasticsearch reports (see the note below). In `resource/cluster`, replace `elasticsearch-cluster` with a name that identifies your Kubernetes cluster (`k8s.cluster.name`).
...
<Callout variant="important">
The `elasticsearch.cluster.name` set in `resource/cluster_name_override` **must exactly match** the `cluster.name` configured in Elasticsearch. The metrics, logs, and traces pipelines all key on it, so if it differs they resolve to separate entities in New Relic (for example, the cluster entity shows no metrics data). Choose a `cluster.name` that is unique within your New Relic account.
</Callout>
...
Enable the `otlp` receiver and a `traces` pipeline in your collector values or ConfigMap, then redeploy and make sure the collector exposes gRPC port `4317`. If you used the New Relic distribution, both ship as `null` placeholders — replace them.

Two processors keep the relationship clean. Without them the cluster appears to "call itself":

* `filter/drop_rootless_es` drops Elasticsearch spans that have no parent application span — for example, Kubernetes health probes and the Elasticsearch metrics receiver's own scrape calls. Because these spans aren't part of an application's trace, New Relic has no calling service to attribute them to and draws a self-referential relationship on the cluster instead.
