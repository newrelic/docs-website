...
Enable the `otlp` receiver and a `traces` pipeline in your collector values or ConfigMap, then redeploy and make sure the collector exposes gRPC port `4317`. If you used the New Relic distribution, both ship as `null` placeholders — replace them. The `transform` strips Elasticsearch's own address from its spans — without it, the cluster appears to "call itself."
...
Open the cluster entity: your apps appear connected through a `CALLS` relationship, with Elasticsearch spans inline in the distributed trace.
