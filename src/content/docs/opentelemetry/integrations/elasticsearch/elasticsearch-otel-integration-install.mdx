...
Replace the `endpoint` value with your Elasticsearch cluster endpoint and update `elasticsearch.cluster.name` in the processor block with a unique name to identify your cluster in New Relic. This name **must exactly match** the `cluster.name` configured in Elasticsearch — the metrics, logs, and traces pipelines all key on it, so if it differs they resolve to separate entities in New Relic (for example, the cluster entity shows no metrics data).
</Callout>
...
* `<elasticsearch-cluster-name>`: Your Elasticsearch cluster name, used to identify the cluster in New Relic. Set this to your actual Elasticsearch `cluster.name` — it **must exactly match** the name Elasticsearch reports on its spans (`es.cluster.name`), otherwise your metrics and traces resolve to separate entities in New Relic (for example, the cluster entity shows no metrics data). Choose a `cluster.name` that is unique within your New Relic account.
...
## Correlate APM with Elasticsearch [#correlate-apm]

Link your applications to the Elasticsearch cluster they call, so a slow transaction leads straight to the cluster that served it. Pick how deep you want the connection:

| Approach | What you get | What you set up |
| --- | --- | --- |
| **Distributed tracing** (recommended) | End-to-end app → Elasticsearch traces **and** an automatic service → cluster relationship | Turn on Elasticsearch's built-in tracing — no per-app changes |
| Metrics-only | The cluster shows up as a related entity | Add `es.cluster.name` to your app's telemetry (any version) |

### Set up distributed tracing [#correlate-tracing]

Elasticsearch emits its own OpenTelemetry spans, joins your app's trace through the `traceparent` header, and stamps `es.cluster.name` on each one, so New Relic wires up the relationship automatically. Requires **Elasticsearch 9.4+**.
...
Add to `elasticsearch.yml` on every node, set `-Dtelemetry.otel.traces.enabled=true` (via `ES_JAVA_OPTS` or `jvm.options`), then restart.
...
Extend your install-time collector config, then restart it. Two processors keep the relationship clean. Without them the cluster appears to "call itself":

* `filter/drop_rootless_es` drops Elasticsearch spans that have no parent application span — for example, health checks and the Elasticsearch metrics receiver's own scrape calls. Because these spans aren't part of an application's trace, New Relic has no calling service to attribute them to and draws a self-referential relationship on the cluster instead.
* `transform/strip_es_host` removes the node's own address from Elasticsearch spans, which New Relic would otherwise resolve back to the cluster.
