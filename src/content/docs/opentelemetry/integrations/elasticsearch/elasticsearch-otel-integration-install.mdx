...
Extend your install-time collector config, then restart it. The `transform` strips Elasticsearch's own address from its spans — without it, the cluster appears to "call itself."
...
service:
  pipelines:
    traces:
      receivers: [otlp]
      processors: [transform/strip_es_host, batch]
      exporters: [otlphttp]
```
