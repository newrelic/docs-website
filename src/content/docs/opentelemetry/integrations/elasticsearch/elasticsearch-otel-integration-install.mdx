...
Replace the `endpoint` value with your Elasticsearch cluster endpoint and update `elasticsearch.cluster.name` in the processor block with a unique name to identify your cluster in New Relic. Make sure this name matches the `cluster.name` configured in Elasticsearch.
</Callout>
...
* `<elasticsearch-cluster-name>`: Your Elasticsearch cluster name, used to identify the cluster in New Relic. Set this to the `cluster.name` configured in Elasticsearch, and make sure it's unique within your New Relic account.
