...
| Metrics-only | The cluster shows up as a related entity | To connect your APM application and Elasticsearch cluster, include the resource attribute `es.cluster.name="your-cluster-name"` in your APM metrics. This enables cross-service visibility and faster troubleshooting within New Relic. |

For the step-by-step tracing setup, see [Correlate APM with Elasticsearch using distributed tracing](/docs/opentelemetry/integrations/elasticsearch/distributed-tracing).
