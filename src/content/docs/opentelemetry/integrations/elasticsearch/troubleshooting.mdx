...
<CollapserGroup>
  ...
  <Collapser id="troubleshoot-cluster-self-loop" title="The cluster shows a relationship to itself (calls itself)">
    ...
    <p>Elasticsearch server spans carry the node's own address (<InlineCode>http.request.headers.host</InlineCode>), which New Relic resolves back to the cluster and draws as a self-referential relationship.</p>
    <p><strong>Resolution</strong></p>
    <p>Add the <InlineCode>transform/strip_es_host</InlineCode> processor to the <InlineCode>traces</InlineCode> pipeline so those address attributes are removed from Elasticsearch spans, then restart the collector:</p>
  ...
  <Collapser id="troubleshoot-name-mismatch" title="Traces and metrics resolve to different cluster entities">
    ...
    - The traces path keys on <InlineCode>es.cluster.name</InlineCode> (stamped by Elasticsearch on its spans) and the metrics path on <InlineCode>elasticsearch.cluster.name</InlineCode>. By default both derive from the configured <InlineCode>cluster.name</InlineCode>, so they resolve to the same entity.
    - If you override the cluster name for metrics in the collector (for example with a <InlineCode>resource/cluster_name_override</InlineCode> processor), make sure the override matches the actual <InlineCode>cluster.name</InlineCode> Elasticsearch reports on its spans, otherwise traces and metrics will resolve to two different entities.
  </Collapser>
