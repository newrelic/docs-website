...
<CollapserGroup>
  ...
  <Collapser id="troubleshoot-k8s-collector-not-running" title="Collector pods not running or crashing">
    ...
    * Check pod events for errors: <InlineCode>kubectl describe pod</InlineCode>
  ...
  <Collapser id="troubleshoot-k8s-logs-not-collected" title="Logs not being collected in Kubernetes">
    ...
    * Verify you're using <InlineCode>mode: daemonset</InlineCode> (deployment mode cannot access node logs)
  ...
  <Collapser id="troubleshoot-k8s-cluster-name-missing" title="Metrics missing k8s.cluster.name attribute">
    ...
    * Verify <InlineCode>K8S_CLUSTER_NAME</InlineCode> environment variable is set in values.yaml
    * Check the <InlineCode>resource/cluster</InlineCode> processor is in the metrics pipeline
    * Query to verify:
      ```sql
