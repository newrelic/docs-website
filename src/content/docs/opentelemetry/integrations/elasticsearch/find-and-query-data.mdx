...
## View distributed traces and APM correlation [#traces-correlation]

If you enabled [APM correlation with distributed tracing](/docs/opentelemetry/integrations/elasticsearch/elasticsearch-otel-integration-install/#correlate-apm), you can query the Elasticsearch spans and see how your applications connect to the cluster.

Confirm Elasticsearch is emitting spans tagged with the cluster name:

```sql
FROM Span SELECT count(*)
WHERE es.cluster.name = '<elasticsearch-cluster-name>'
FACET name
SINCE 30 minutes ago
```

See which applications are calling Elasticsearch:

```sql
FROM Span SELECT uniques(service.name)
WHERE db.system = 'elasticsearch'
SINCE 30 minutes ago
```

You can also open the Elasticsearch cluster entity and view its **service map**: instrumented applications appear connected to the cluster through a `CALLS` relationship, and any distributed trace that touches Elasticsearch shows the Elasticsearch spans inline alongside your application's spans.
