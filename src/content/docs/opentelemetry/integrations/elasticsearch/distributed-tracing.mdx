...
Setup has two parts: turn on tracing in Elasticsearch, and add a traces pipeline to your collector.
...
Add the `otlp` receiver and the `filter/drop_rootless_es` and `transform/strip_es_host` processors to a `traces` pipeline:
