...
Get complete visibility into your HAProxy load balancers with OpenTelemetry. You can collect metrics from HAProxy's CSV stats endpoint (via the [haproxyreceiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/haproxyreceiver)) or from its built-in Prometheus endpoint (HAProxy 2.0+). HAProxy 3.4+ also supports an [OpenTelemetry filter](https://www.haproxy.com/documentation/haproxy-configuration-tutorials/alerts-and-monitoring/opentelemetry/overview/) for distributed tracing, which creates [service relationships](/docs/new-relic-solutions/new-relic-one/ui-data/service-maps/service-maps/) between your applications and the HAProxy entity. New Relic supports both self-hosted Linux and Kubernetes deployments.
...
- **Distributed tracing**: HAProxy 3.4+ with the OTel filter participates in distributed traces, creating service relationships between your applications and the load balancer visible in [service maps](/docs/new-relic-solutions/new-relic-one/ui-data/service-maps/service-maps/)
