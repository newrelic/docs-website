      ...
      receiver_creator/nginx:
        ...
        receivers:
          nginx:
            ...
            config:
              endpoint: 'http://`endpoint`:<YOUR_LOCAL_PORT>/basic_status'  # Update port and path as needed
              metrics:
                nginx.requests:
                  enabled: true
            ...
            resource_attributes:
              nginx.server.endpoint: 'http://`endpoint`:<YOUR_LOCAL_PORT>/basic_status'  # Update port and path as needed
...
<tbody>
  ...
  <tr>
    <td>`endpoint`</td>
    <td>Dynamic endpoint placeholder used by the receiver creator (automatically populated by k8s_observer)</td>
  </tr>
  <tr>
    <td>`app` and `role` labels</td>
    <td>Pod labels used to identify NGINX pods (update the rule to match your labels)</td>
