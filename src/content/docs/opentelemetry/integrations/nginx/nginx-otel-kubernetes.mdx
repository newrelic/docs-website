...
2. Update the [`deployment-configmap.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-k8s-otel-collector/examples/k8s/rendered/deployment-configmap.yaml) for NGINX monitoring.
    ...
    **Option 1: Replace all existing OpenTelemetry components** (recommended for NGINX-only monitoring)
    ...
    If you choose this option, you'll also need to delete the DaemonSet collector in step 4 to prevent ingestion of Kubernetes metrics you don't want to monitor.
    </Callout>
...
3. Customize the NGINX configuration.
    ...
    - Update the `endpoint` under `otlphttp/newrelic` exporter. Refer to the OTLP endpoint configuration [documentation](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) to select the appropriate endpoint for your region.
