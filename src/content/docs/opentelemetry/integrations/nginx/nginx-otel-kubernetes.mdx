...
2. Update the [`deployment-configmap.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-k8s-otel-collector/examples/k8s/rendered/deployment-configmap.yaml) for NGINX monitoring:
    ...
    <Collapser id="option1-nginx-only" title="Option 1: Replace all existing OpenTelemetry components (recommended for NGINX-only monitoring)">
    ...
    The following table describes the key configuration parameters for Option 1:
    ...
    <Collapser id="option2-k8s-nginx" title="Option 2: Merge with existing configuration (Monitor K8s + Nginx metrics)">
    ...
    The following table describes the key configuration parameters for Option 2:
...
3. Apply the updated manifests and restart the deployment.
    ...
    If you chose Option 1 (NGINX-only monitoring), run these commands:
    ```bash
    ...
    If you chose Option 2 (K8s + NGINX monitoring), run these commands:
    ```bash
    kubectl apply -n newrelic -R -f rendered
    kubectl rollout restart deployment nr-k8s-otel-collector-deployment -n newrelic
    ```
...
1. Create custom values.yaml configuration
