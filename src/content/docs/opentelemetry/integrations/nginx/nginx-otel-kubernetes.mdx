...
<CollapserGroup>
  ...
  <tbody>
    <tr>
      <td>`<CLUSTER_NAME>`</td>
      <td>Your Kubernetes cluster name. Used for identification in New Relic.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>`app` and `role` labels</td>
      <td>Pod labels used to discover NGINX pods. Update the `rule` to match your pod labels.</td>
    </tr>
    <tr>
      <td>`name == "metrics"`</td>
      <td>The named container port on `9113` that the receiver_creator watches for. Must match the port name in your pod spec.</td>
    </tr>
    <tr>
  ...
  <Collapser id="prometheus-k8s-otelcol-helm" title="OTel Collector Contrib — Helm install">

  <CollapserGroup>

    <Collapser id="prometheus-otelcol-step1" title="Step 1: Create custom values.yaml configuration">
  ...
  <tbody>
    ...
    <tr>
      <td>`<YOUR_NEWRELIC_OTLP_ENDPOINT>`</td>
      <td>Your region's OTLP endpoint. See [OTLP endpoint documentation](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol).</td>
    </tr>
    <tr>
      <td>`app` and `role` labels</td>
      <td>Pod labels used to discover NGINX pods. Update the `rule` to match your pod labels.</td>
    ...
    <Collapser id="prometheus-otelcol-step2" title="Step 2: Install with Helm">
  ...
  ```
    ...
    <Collapser id="prometheus-otelcol-step3" title="Step 3: Verify deployment and data collection">
