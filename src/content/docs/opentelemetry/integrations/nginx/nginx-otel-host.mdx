...
- One of the following collectors installed on a Linux host:
  - (Recommended) [NRDOT Collector](https://github.com/newrelic/nrdot-collector-releases), or
...
1. Add the following server block to your NGINX config file (`/etc/nginx/nginx.conf`):
   ...
   {/* Replace `<PORT>` with an available port number such as `8080`, `8081`. */}
...
3. Verify stub status is working:

   Test the endpoint:
   ```bash
   curl http://<SERVER_IP>:<PORT>/nginx_status
   ```

   Replace `<SERVER_IP>` with your NGINX server's IP address (e.g., `127.0.0.1` for local, or the actual server IP for remote access).
   ...
   ```bash
   curl -I http://<SERVER_IP>:<PORT>/nginx_status
   ```
...
- Your stub status URL from Step 1 (default: `http://<SERVER_IP>:<PORT>/nginx_status`)
- A deployment name (e.g., `production-web-01`, `staging-api`)
...
receivers:
  nginx:
    endpoint: http://<SERVER_IP>:<PORT>/nginx_status  # Replace with your stub status URL
  ...
  resource/nginx:
    attributes:
      - key: nginx.server.endpoint
        value: "http://<SERVER_IP>:<PORT>/nginx_status"  # Replace with your endpoint
        action: upsert
...
<tbody>
  <tr>
    <td>`<SERVER_IP>`</td>
    <td>Enter your NGINX server's IP address (e.g., `127.0.0.1` for local server)</td>
  </tr>
  <tr>
    <td>`<PORT>`</td>
    <td>Enter your NGINX stub status port number (e.g., `8080`, `8081`)</td>
  </tr>
  <tr>
    <td>`<DEPLOYMENT_NAME>`</td>
    <td>Replace with a unique deployment name for this NGINX server (e.g., `production-web-01`, `staging-api`, `prod-lb-01`)</td>
  </tr>
  <tr>
...
1. **Endpoint URL**: Change `http://<SERVER_IP>:<PORT>/nginx_status` to match your stub status configuration from Step 1
...
1. Configure structured logging in NGINX:
...
2. Configure the Collector for log forwarding:

  Add these sections to your `/etc/nrdot-collector/nginx-config.yaml`:
...
3. Set permissions and apply configuration:
...
<CollapserGroup>
  <Collapser id="step2-understanding-otelcol" title="Understand the configuration">
  ...
  <Collapser id="step2-prepare-otelcol" title="Prepare your environment">
    Before editing the configuration, you'll need two key pieces of information:

    1. **Your stub status URL** - From Step 1 (default: `http://<SERVER_IP>:<PORT>/nginx_status`)
  ...
  <Collapser id="step2-configuration-otelcol" title="Add the Collector configuration">
...
<CollapserGroup>
  <Collapser id="step5-configure-nginx-logs-otelcol" title="Configure structured logging in NGINX">
    First, configure NGINX to output JSON-formatted logs that are easier to parse and query.
  ...
  <Collapser id="step5-configure-collector-logs-otelcol" title="Configure the Collector for log forwarding">
  ...
  <Collapser id="step5-permissions-apply-otelcol" title="Set permissions and apply configuration">
    **Allow the Collector to read NGINX log files:**
    ```bash
    # Add collector user to adm group (has read access to logs)
