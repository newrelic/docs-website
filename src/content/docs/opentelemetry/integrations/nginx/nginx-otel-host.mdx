...
This guide provides complete setup instructions for monitoring self-hosted NGINX with OpenTelemetry. Follow these steps to configure your NGINX server, NRDOT Collector (recommended) or OpenTelemetry Collector Contrib, and send metrics, telemetry data to New Relic.
...
## Step 1: Configure NGINX stub status [#configure-nginx-nrdot]
...
<CollapserGroup>
  ...
  <Collapser id="step2-prepare-nrdot" title="Prepare your environment">
    ...
    1. **Your stub status URL** - From Step 2 (default: `http://127.0.0.1:8080/nginx_status`)
...
```
  ...
  <Collapser id="step2-customize-nrdot" title="Customize the configuration">
    **Replace these values in the configuration above:**
    ...
    1. **Endpoint URL**: Change `http://127.0.0.1:8080/nginx_status` to match your stub status configuration from Step 2
...
## Step 1: Configure NGINX stub status [#configure-nginx-otelcol]
