...
<CollapserGroup>
  ...
  <Collapser id="configure-nrdot-collector-nrdot" title="Step 1: Configure NRDOT collector">
...
<tbody>
  <tr>
    ...
    <td>Replace with the NGINX stub status endpoint (for example `http://127.0.0.1/basic_status`)</td>
  </tr>
  <tr>
...
- Keep it short but meaningful for easy filtering and alerting */}
  ...
  <Collapser id="configure-environment-nrdot" title="Step 2: Configure environment and update config path">
...
```
  ...
  <Collapser id="forward-nginx-logs-nrdot" title="Step 3: (Optional) Forward NGINX logs">
...
4. Set permissions and apply configuration. Allow the collector to read NGINX log files:
  ...
  <Collapser id="start-monitoring-nrdot" title="Step 4: Start monitoring">
...
<CollapserGroup>
  ...
  <Collapser id="configure-otelcol-collector" title="Step 1: Configure the OpenTelemetry Collector">
...
- Keep it short but meaningful for easy filtering and alerting */}
  ...
  <Collapser id="set-up-authentication-otelcol" title="Step 2: Set up authentication">
...
- Replace `YOUR_LICENSE_KEY_HERE` with your actual license key from above */}
  ...
  <Collapser id="forward-nginx-logs-otelcol" title="Step 3: (Optional) Forward NGINX logs">
...
4. Set permissions and apply configuration. Allow the collector to read NGINX log files:
  ...
  <Collapser id="start-monitoring-otelcol" title="Step 4: Start monitoring">
...
<CollapserGroup>
  ...
  <Collapser id="configure-prometheus-host" title="Step 1: Configure the collector">
...
<tbody>
  <tr>
    <td>`<YOUR_STUB_STATUS_ENDPOINT>`</td>
    <td>Your NGINX stub status URL (for example `http://127.0.0.1:8080/nginx_status`). Used as the `nginx.server.endpoint` resource attribute.</td>
  </tr>
  <tr>
    <td>`<DEPLOYMENT_NAME>`</td>
    <td>Unique name for this NGINX server (for example `production-web-01`). Appears in New Relic dashboards.</td>
  </tr>
  <tr>
    <td>`targets`</td>
    <td>Address of the nginx-prometheus-exporter (default `localhost:9113`). Update if running on a different host or port.</td>
  </tr>
  <tr>
    <td>`scrape_interval`</td>
    <td>How often to scrape metrics from the exporter. Default: `30s`.</td>
  </tr>
</tbody>
</table>
  ...
  <Collapser id="env-vars-prometheus-host" title="Step 2: Set up environment variables">
...
Replace `YOUR_NEW_RELIC_LICENSE_KEY` with your <InlinePopover type="licenseKey"/>.

<Callout variant="tip">
  For EU accounts, use `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.eu01.nr-data.net:4318`. See [Configure endpoint, port, and protocol](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) for the full list of regional endpoints.
</Callout>

  </Collapser>

  <Collapser id="start-prometheus-host" title="Step 3: Start the collector">
...
Expected output: `Active: active (running)` with no errors.
  ...
  <Collapser id="verify-prometheus-host" title="Step 4: Verify data in New Relic">
