...
<Collapser
  ...
  title="Check NGINX status module"
>
...
<CollapserGroup>
  <Collapser id="step2-understanding-nrdot" title="Understand the configuration">
  ...
  <Collapser id="step2-prepare-nrdot" title="Prepare your environment">
  ...
  <Collapser id="step2-configuration-nrdot" title="Add the Collector configuration">

For NRDOT Collector, create a standalone NGINX monitoring configuration file that replaces the default configuration.
...
Create `/etc/nrdot-collector/nginx-config.yaml` with the following content:
...
<CollapserGroup>
  <Collapser id="step1-add-config-otelcol" title="Add stub status configuration">
    Add this configuration to your NGINX config file (`/etc/nginx/nginx.conf`):
  ...
  <Collapser id="step1-apply-config-otelcol" title="Apply the configuration">
    1. Test your NGINX configuration:
       ```bash
       sudo nginx -t
       ```
  ...
  <Collapser id="step1-verify-otelcol" title="Verify stub status is working">
    Test the endpoint:
    ```bash
    curl http://127.0.0.1:8080/nginx_status
    ```
