...
NGINX 1.25.3 and later includes the [`ngx_otel_module`](https://nginx.org/en/docs/ngx_otel_module.html), a native OpenTelemetry module that adds distributed tracing support directly in the web server. The module participates in distributed traces by extracting incoming [W3C Trace Context](https://www.w3.org/TR/trace-context/) headers, creating a span for the request as it passes through NGINX, and injecting updated trace context into requests proxied to upstream servers.
...
Because the collector stamps the same `nginx.deployment.name` and `nginx.server.endpoint` identity that the NGINX metrics use, the NGINX spans resolve to the **same** `NGINXSERVER` entity as your NGINX metrics — not a separate, duplicate service entity. These relationships are visible in [service maps](/docs/new-relic-solutions/new-relic-one/ui-data/service-maps/service-maps/) and the [maps experience](/docs/service-architecture-intelligence/maps/advanced-maps/).
  ...
  resource/nginx:
    attributes:
      ...
      - key: nginx.deployment.name
        value: "<DEPLOYMENT_NAME>"             # a stable name for this NGINX deployment
        action: upsert
...
- **`resource/nginx`**: Adds `nginx.server.endpoint` and `nginx.deployment.name`. These two attributes form the NGINX entity's identity — applying them to **both** pipelines is what makes the NGINX spans and metrics resolve to the same `NGINXSERVER` entity instead of a duplicate service entity.
- **`transform/nginx_*`**: Adds `nginx.display.name` for a friendly entity name.

<Callout variant="important">
  The `nginx.server.endpoint` value in `resource/nginx` and the `nginx` receiver `endpoint` must be identical — together with `nginx.deployment.name`, they form the NGINXSERVER entity's identity. This is a one-time, static value per NGINX instance. When you add or remove backend and client applications, no changes to the collector configuration are needed — relationships form automatically through trace context propagation.
</Callout>
...
- Check that your client applications propagate W3C `traceparent` headers. Without trace context propagation, NGINX spans are not connected to the calling service.
