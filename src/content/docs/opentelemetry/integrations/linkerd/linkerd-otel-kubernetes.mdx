...
Add an `otlp` receiver and a `traces` pipeline to `deployment.configMap.extraConfig` in `values.yaml`:
...
To enable distributed tracing on the manifest path, add the same `otlp` receiver, `traces` pipeline, and `k8sattributes`/`transform` processors shown in the Helm install above to the same ConfigMap, then complete the same three one-time steps (mesh the collector, mark its OTLP port as gRPC, enable proxy tracing).
