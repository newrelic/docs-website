...
Re-run the `helm upgrade` command from [Monitor Linkerd on Kubernetes with OpenTelemetry](/docs/opentelemetry/integrations/linkerd/nrdot-helm) with the updated `values.yaml`, then complete these one-time steps:
...
Add the following to the same `config` section of your `values.yaml` from [OTel Collector Contrib with Helm](/docs/opentelemetry/integrations/linkerd/otel-helm#install):
...
Add the same three blocks above (Receivers, Processors, Pipelines) to the `config` key of the ConfigMap in your `otel-collector.yaml` from [OTel Collector Contrib with manifest](/docs/opentelemetry/integrations/linkerd/otel-manifest#install). The Service in that manifest already exposes port `4317` (grpc) and `4318` (http), so no port changes are needed. Re-apply and restart. Re-applying the ConfigMap alone doesn't restart the running collector pod, so it won't pick up the new config without the second command:
...
<Callout variant="tip">
  `spec.exporter.endpoint` below points at the [OTel Collector Contrib with manifest](/docs/opentelemetry/integrations/linkerd/otel-manifest#install) page's Service. If you deployed the NRDOT collector, use `http://nr-k8s-otel-collector-gateway.newrelic.svc.cluster.local:4317` instead.
</Callout>
...
- [Introduction to Linkerd monitoring with OpenTelemetry](/docs/opentelemetry/integrations/linkerd/introduction/) - Metrics setup, if you haven't completed it yet
- [Find and query your Linkerd data](/docs/opentelemetry/integrations/linkerd/find-data/) - NRQL queries and dashboard navigation
- [Linkerd metrics reference](/docs/opentelemetry/integrations/linkerd/metrics-reference/) - Complete metrics and attributes reference