...
<CollapserGroup>
  <Collapser id="nrdot" title="For NRDOT Collector (Recommended)">
...
```
  ...
  <Collapser id="otel" title="For OpenTelemetry Collector Contrib">
...
Then confirm IBM MQ metrics and entities in New Relic using the verification queries in [find and query your data](/docs/opentelemetry/integrations/ibm-mq/view-data). For the meaning of status values, see the [MQ status-code reference](/docs/opentelemetry/integrations/ibm-mq/metrics#status-codes).
