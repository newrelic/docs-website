...
Monitor Amazon ECS tasks running on EC2 instances by deploying OpenTelemetry Collector Contrib as a sidecar container. This comprehensive guide walks you through creating task definitions, configuring the collector, and deploying monitoring for your ECS on EC2 workloads.
...
<CollapserGroup>
  <Collapser id="ecs-reqs" title="ECS requirements">
    ...
    * **Launch type**: EC2 launch type (this guide doesn't apply to Fargate)
  ...
  "containerDefinitions": [
    ...
    },
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/otel-collector-ec2",
          "awslogs-create-group": "true",
          "awslogs-region": "us-east-1",
  ...
  "containerDefinitions": [
    ...
    },
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/otel-collector-windows",
          "awslogs-create-group": "true",
          "awslogs-region": "us-east-1",
          "awslogs-stream-prefix": "otel"
        }
...
2. **Verify in New Relic**: Navigate to **[one.newrelic.com](https://one.newrelic.com) > All Capabilities > Infrastructure** and look for your ECS hosts and containers.

3. **Check metrics**: Run this NRQL query to verify container metrics are being received:
   ```sql
   FROM Metric SELECT latest(container.cpu.utilization) WHERE instrumentation.provider = 'opentelemetry' FACET ClusterName, ServiceName, TaskId SINCE 10 minutes ago
   ```
...
- **Metric filtering**: Modify filter processors to include/exclude specific metrics
- **Memory limits**: Adjust the `memory_limiter` processor to control collector memory usage
