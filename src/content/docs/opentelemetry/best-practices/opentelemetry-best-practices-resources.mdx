...
<Callout variant="important">
  The relationships described below that are supported by instrumentation
  leverage OpenTelemetry semantic conventions.
  ...
  The relationships not supported by instrumentation require you to manually
  emit telemetry to signal to New Relic that a relationship should be created.
  To do so, you must emit a log (preferred) or a span with the attribute
  `newrelic.aws_metric_streams.arn` containing the value of the
  [ARN](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference-arns.html)
  for the resource you are calling. Note that relationships created have a TTL
  of 75 minutes, so this telemetry must continue to be sent periodically.
</Callout>
