...
#### Service to AWS Elasticache Memcached Cluster 

Relationships between a [service](https://docs.newrelic.com/docs/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services) and AWS Memcached Cluster entity requires the following:

The service should generate spans with the following attributes:

* `db.system`: `memcached`
* `net.peer.name`: AWS connection endpoint of the memcached cluster ending with `cache.amazonaws.com`
