...
Additionally, configure your OTLP exporter to use the [`OTLP/HTTP` binary protobuf version](https://opentelemetry.io/docs/specs/otlp/#binary-protobuf-encoding) of the protocol if available. While New Relic supports all stable versions of OTLP, we recommend `HTTP/protobuf` as the default for data sent over the public internet. Proxies, firewalls, and load balancers in the export path can mishandle long-lived `gRPC` `HTTP/2` multiplexed connections, causing dropped connections or uneven load distribution. Discrete `OTLP/HTTP` requests traverse standard HTTP infrastructure cleanly, with no measurable performance or egress traffic cost.
