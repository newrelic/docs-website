...
<table>
  ...
  <tbody>
    ...
    <tr id="collector-filter-processor">
      ...
      <td>
        The Collector filter processor can be used to filter out span, metric, and log records from your observability pipeline. For spans, it can function as a simple tail sampler, acting on completed spans but without access to the full trace (Note: this can result in broken traces). For metrics, it can be used to drop metrics or series which are not high value. For logs, it can be used to drop log records which are not high value (e.g. logs with fine grain severity or from noisy loggers). For a working example of using the filter processor to suppress errors before they reach New Relic, see the [ignored errors example](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors).
      </td>
    </tr>
