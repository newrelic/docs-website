...
**NGINX Plus vs NGINX:** NGINX Plus is the commercial version of NGINX that provides enterprise-grade features beyond the open-source version. Key advantages include:

- **Advanced load balancing** - Health checks, session persistence, and dynamic configuration
- **Enhanced monitoring** - Rich HTTP API with detailed real-time metrics and status information
- **Enterprise support** - 24/7 technical support and professional services
- **Security features** - JWT authentication, rate limiting, and advanced access controls
- **High availability** - Active health checks and graceful failover capabilities
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-dashboard" title="Verify data in New Relic">
    Wait 2-3 minutes for initial data to appear in New Relic, then [access your NGINX dashboard](/docs/opentelemetry/nginx/find-and-query-your-data#find-data) to verify data collection.
  </Collapser>
</CollapserGroup>
