...
<CollapserGroup>
  <Collapser id="step2-understanding" title="Understanding the configuration">
  ...
  <Collapser id="step2-validate" title="Validate the configuration">
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-data" title="Verify data collection">
    ...
    5. **Wait 2-3 minutes** for initial data to appear in New Relic, then [access your NGINX dashboard](/docs/opentelemetry/nginx/find-and-query-your-data#find-data) to verify data collection.
  </Collapser>
</CollapserGroup>
...
In addition to metrics, you can send NGINX access and error logs to New Relic for comprehensive monitoring and troubleshooting. These logs complement the [core NGINX metrics](/docs/opentelemetry/nginx/nginx-otel-metrics-reference#core-metrics) and provide detailed request-level insights.
...
<CollapserGroup>
  ...
  <Collapser id="troubleshoot-collector-config" title="Collector configuration issues">
    **YAML syntax errors:**
    ```bash
    sudo otelcol-contrib --config /etc/otelcol-contrib/config.yaml --dry-run
    ```
    ...
    **Merging with existing configuration:**
    ...
    - Add processors to your existing `processors:` section
  ...
  <Collapser id="troubleshoot-authentication" title="Authentication errors (401/403)">
    ...
    **Solutions:**
    ...
    - Verify OTLP endpoint matches your New Relic region (US vs EU)
    - Check for extra spaces or characters in environment configuration file
    - Reload systemd after credential changes: `sudo systemctl daemon-reload`
  </Collapser>
  ...
  <Collapser id="troubleshoot-no-data" title="No data appears in New Relic">
    **Step-by-step diagnosis:**
    ...
    4. **Verify authentication and endpoints:**
       ```bash
       sudo systemctl show otelcol-contrib.service -p Environment
       ```
  ...
  <Collapser id="troubleshoot-log-forwarding" title="Log forwarding issues">
    ...
    **Common issues:**
    ...
    - **Log rotation** - NGINX may have rotated logs, restart collector
