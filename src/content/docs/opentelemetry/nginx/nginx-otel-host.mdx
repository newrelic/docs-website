...
### Troubleshooting stub status configuration
...
- **Receivers** - Connect to your NGINX stub status endpoint every 30 seconds to collect metrics
- **Processors** - Add server identification and batch metrics for efficient transmission
- **Exporters** - Send the processed metrics to your New Relic account via OTLP
...
### Backup and edit the Collector configuration
...
- Must be unique across all your NGINX servers in New Relic
...
<CollapserGroup>
  <Collapser id="troubleshoot-yaml-syntax" title="YAML syntax errors">
    **Problem:** `--dry-run` shows syntax errors
  ...
  <Collapser id="troubleshoot-merge-config" title="Merging with existing configuration">
    **Problem:** You already have a Collector configuration

    **Solutions:**
    - Add the `nginx` receiver to your existing `receivers:` section
    - Add the processors to your existing `processors:` section
    - Create a new pipeline or modify an existing metrics pipeline
    - Don't duplicate section headers - merge content under existing ones
  </Collapser>
</CollapserGroup>
...
**2. Your OTLP Endpoint (choose your region):**
- **US region**: `https://otlp.nr-data.net:4318`
- **EU region**: `https://otlp.eu01.nr-data.net:4318`

<Callout variant="tip">
**Why environment variables?** They keep credentials secure by avoiding hardcoded secrets in configuration files and preventing accidental exposure in version control.
</Callout>
...
5. **Wait 2-3 minutes** for initial data to appear in New Relic, then check your dashboard.

### Troubleshooting startup issues

<CollapserGroup>
  <Collapser id="troubleshoot-service-failed" title="Service shows 'failed' or 'inactive'">
  ...
  <Collapser id="troubleshoot-auth-errors" title="Authentication errors (401/403)">
    ...
    **Solutions:**
    - Verify license key is complete and correct
    - Ensure endpoint URL matches your New Relic region
    - Check for extra spaces or characters in environment file
  </Collapser>

  <Collapser id="troubleshoot-nginx-connection" title="Cannot connect to NGINX">
    **Problem:** Logs show "connection refused" to NGINX endpoint

    **Verify NGINX connectivity:**
    ```bash
    # Test stub status endpoint
    curl http://127.0.0.1:8080/nginx_status
    ...
    **Solutions:**
    - Ensure NGINX stub status configuration is correct
    - Verify endpoint URL in Collector matches NGINX config
...
**Quick start:** Go to [one.newrelic.com](https://one.newrelic.com) → **Add Data** → Search "NGINX OTel" → **View dashboard**
...
<CollapserGroup>
  <Collapser id="troubleshoot-no-data-newrelic" title="No data appears in New Relic">
    **Step-by-step diagnosis:**
    ...
    2. **Check Collector service is running:**
       ```bash
       sudo systemctl status otelcol-contrib.service
       ```
    ...
    4. **Verify authentication:**
       ...
       sudo systemctl show otelcol-contrib.service -p Environment
       ```
       Ensure NEWRELIC_LICENSE_KEY and NEWRELIC_OTLP_ENDPOINT are set.

    5. **Wait 2-5 minutes** - Initial data ingestion can take time.
  </Collapser>

  <Collapser id="troubleshoot-collector-wont-start" title="Collector service won't start">
    ...
    **Common causes:**
    - YAML syntax errors (check indentation with spaces, not tabs)
    - Invalid endpoint URLs in configuration
  ...
  <Collapser id="troubleshoot-authentication" title="Authentication errors (401/403)">
    **Verify your credentials:**
    ```bash
    # Test manual authentication
    curl -H "api-key: YOUR_LICENSE_KEY" https://otlp.nr-data.net:4318/v1/metrics
    ...
    **Solutions:**
    - Ensure license key is complete and correct (starts with "NRAK-")
    - Verify OTLP endpoint matches your New Relic region
    - Check for extra spaces in environment configuration file
  </Collapser>

  <Collapser id="troubleshoot-nginx-connectivity" title="Can't connect to NGINX">
    ...
    **Solutions:**
    - Ensure NGINX stub status configuration is correct
    - Verify endpoint URL in Collector config matches NGINX config
