...
Note: These charts are only populated if your collector uses components that communicate via HTTP or gRPC (like OTLP receivers or exporters) and requires <DNT>`INTERNAL_TELEMETRY_METRICS_LEVEL=detailed`</DNT>.
