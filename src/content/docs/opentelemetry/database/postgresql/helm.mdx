...
Multi-instance mode has no plain-username/password path — every instance needs its own Kubernetes secret with monitoring credentials, and, if you want the setup job to create each monitoring role for you, its own admin-credentials secret too:
...
Omit each entry's `postgresAdmin.existingSecret` and set `setupJob.enabled: false` if you've already created the monitoring roles and their per-database grants yourself.
...
<table>
  ...
  <tbody>
    <tr>
      <td>`postgresqlMulti.enabled`</td>
      <td>Set to `true` to monitor multiple instances from one release instead of `postgresql.*`. Defaults to `false`.</td>
    </tr>
    <tr>
      <td>`postgresqlMulti.topology`</td>
      <td>`self-hosted` or `rds`, same meaning as `postgresql.topology`, shared by every instance in this release.</td>
    </tr>
    <tr>
      <td>`postgresqlMulti.instances`</td>
      <td>List of instances to monitor. Each entry requires a unique `name`, `server` (`port` defaults to `5432`), `existingSecret`, and a **required, non-empty** `databases` list of database names on that instance; add `postgresAdmin.existingSecret` per entry only when `setupJob.enabled` is `true`.</td>
    </tr>
  </tbody>
</table>
