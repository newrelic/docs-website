...
### Connect and create the monitoring user

Connect to your PostgreSQL instance as a superuser (or another user with sufficient permissions):
...
### Create the monitoring schema and grant access

* Required on PostgreSQL 15+ only — role membership defaults changed in PG15. On PostgreSQL 14, roles inherit privileges by default and this is a no-op — safe to run on any supported version either way:
...
* (Optional) To collect `EXPLAIN` plans on locking or write statements without granting write access to the monitoring user, create the EXPLAIN helper function in each database:
...
receivers:
  nrpostgresql:
    ...
    # regardless of the databases list above.
    ...
    # see the metrics reference for the full default-on/default-off breakdown.
    metrics:
...
**Under the Events section, enable these event types to collect their data:**

| Event | Description |
| --- | --- |
| `db.server.query_sample` | Point-in-time snapshot of currently-running statements from `pg_stat_activity`, including SQL text, wait state, and blocking details. |
| `db.server.top_query` | Aggregated per-normalized-query execution stats from `pg_stat_statements`, reported as incremental (delta) values, including query plans when obtainable. |

**Configure the collection settings for each event type:**

| Configuration | Parameter | Default value |
| --- | --- | --- |
| `top_query_collection` | `max_rows_per_query` | `1000` |
| `top_query_collection` | `top_n_query` | `200` |
| `top_query_collection` | `collection_interval` | `60s` |
| `top_query_collection` | `max_explain_each_interval` | `1000` |
| `top_query_collection` | `query_plan_cache_size` | `1000` |
| `top_query_collection` | `query_plan_cache_ttl` | `1h` |
| `query_sample_collection` | `max_rows_per_query` | `1000` |
