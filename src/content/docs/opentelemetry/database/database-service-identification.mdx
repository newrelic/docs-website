...
### Enable APM-DB correlation for Java agent [#enable-correlation-java]

To enable this correlation, add the `sql_metadata_comments` property to the `transaction_tracer` section of your Java agent configuration file (`newrelic.yml`). For more information, refer to the [Java agent configuration documentation](/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file/).
...
### Enable APM-DB correlation for .NET agent [#enable-correlation-dotnet]

To enable this correlation, add the `sqlMetadataComments` property to the `transactionTracer` section of your .NET agent configuration file (`newrelic.config`). For more information, refer to the [.NET agent configuration documentation](https://docs.newrelic.com/docs/apm/agents/net-agent/configuration/net-agent-configuration/).

**Option 1: Via Environment Variables (Script):**
If you are using a startup or wrapper script to launch your executable, set the `NEW_RELIC_TRANSACTION_TRACER_SQL_METADATA_COMMENTS_ENABLED` variable to `1`.
```powershell
...
**Option 2: Via `newrelic.config`:**
Alternatively, you can enable this directly within the `<transactionTracer>` block of your `newrelic.config` file:
```xml
