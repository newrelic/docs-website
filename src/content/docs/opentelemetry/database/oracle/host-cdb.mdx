---
title: "Install & configure NRDOT Collector for on-host CDB monitoring"
tags:
  - OpenTelemetry
...
<Callout variant="tip">
  ...
  The `V_$ASM_DISKGROUP_STAT` and `V_$ASM_DISK_STAT` grants are only required if you want to collect [ASM diskgroup and disk metrics](/docs/opentelemetry/database/oracle/metrics-reference/#asm).
...
<DocTiles>
  ...
  <DocTile title="Docker install" path="/docs/opentelemetry/database/oracle/docker">Learn how to run the NRDOT Collector as a sibling container to an Oracle Database instance already running in Docker.</DocTile>
</DocTiles>
