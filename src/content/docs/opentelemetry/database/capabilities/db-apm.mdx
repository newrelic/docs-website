...
**Note**: The relationship between an APM entity and a database entity is established by matching the connection address your application reports against the address your database monitoring is configured with. This works reliably when both sides use the same address and port. This might not reliably link if your application connects through a connection pooler, load balancer, or sharding proxy, or if your application and your database monitoring use different representations of the same address (for example, a domain name on one side and an IP address on the other).
