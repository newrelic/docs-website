...
<Callout variant="important">
  gMSA authentication uses integrated security, which uses the same configuration approach as Windows Domain authentication. Update the `datasource` server value to your RDS endpoint. The NRDOT service will authenticate using the gMSA account you configured in Step 4.
</Callout>
