...
Update your NRDOT Collector configuration file at `C:\Program Files\NRDOT Collector Host\mssql-config.yaml`.
...
BEGIN
    EXECUTE('USE "' + @name + '"; CREATE USER [DomainName\gMSA] FOR LOGIN [DomainName\gMSA];' );
    FETCH next FROM db_cursor INTO @name
END
CLOSE db_cursor
DEALLOCATE db_cursor
