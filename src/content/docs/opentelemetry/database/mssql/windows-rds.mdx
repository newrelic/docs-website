...
In your SQL Server Management Studio (SSMS), run the following script to create a login for your gMSA account and grant the necessary permissions. Replace all occurrences of `<YOUR_DOMAIN>\<YOUR_GMSA_USER>` with your actual domain and account name.
...
Configure the NRDOT Collector service to run as your gMSA account using PowerShell. Replace `<YOUR_DOMAIN>\<YOUR_GMSA_NAME>$` with your domain and gMSA account name. You must include the `$` at the end, as it is required for gMSA accounts:
