...
Set up Microsoft SQL Server monitoring using the NRDOT Collector on Windows self-hosted environments including physical servers, virtual machines, and standalone Windows installations. To monitor in a Windows RDS environment, refer to [Windows RDS instrumentation](/docs/opentelemetry/database/mssql/windows-rds).
...
<CollapserGroup>
  ...
  <Collapser
    id="same-host"
    title="For SQL Server and NRDOT Collector on the same Windows host"
  >
...
3. (Optional) If you don't have the necessary permissions, from your SSMS or `sqlcmd` utility, run the following command to grant the required permissions to your Windows account. Replace all occurrences of `<ACCOUNT_USERNAME>` with your Windows username retrieved from step 1:
  ...
  <Collapser
    id="different-host"
    title="For SQL Server and NRDOT Collector on the different Windows host"
  >
...
<Callout variant="tip">
  To securely manage sensitive information, such as database credentials, store them in [secret management](/docs/opentelemetry/database/capabilities/db-apm/#secret-management) tools. This helps to enhance the security of your monitoring setup by avoiding hardcoding sensitive data in configuration files.
</Callout>
