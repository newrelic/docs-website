...
Multi-instance mode has no plain-username/password path — every instance needs its own Kubernetes secret with monitoring credentials, and, if you want the setup job to create each monitoring login for you, its own admin-credentials secret too:
...
Omit each entry's `sqlAdmin.existingSecret` and set `setupJob.enabled: false` if you've already created the monitoring logins yourself.
