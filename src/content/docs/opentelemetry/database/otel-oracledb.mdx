...
3. Establish SSH agent forwarding to your Oracle Database host(s), then run:

    ```bash
    ssh -A <ssh-user>@<control-host>
...
Omit each entry's `oracleAdmin.existingSecret` and set `setupJob.enabled: false` if you've already created the monitoring users yourself.
