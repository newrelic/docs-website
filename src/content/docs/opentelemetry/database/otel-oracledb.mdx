...
<CollapserGroup>
  <Collapser
      ...
      ```bash
      nc -zw3 <YOUR_ENDPOINT> <YOUR_PORT> && echo "PORT OPEN - Network connection successful!" || echo "CONNECTION FAILED - Port closed or firewall blocking"
      ```
  ...
  <Collapser
      ...
      nc -zw3 "$HOST" "$PORT" && echo "SUCCESS - OTLP endpoint reachable" || echo "FAILED - Cannot reach OTLP endpoint"
      ```
      **Expected output:** `SUCCESS - OTLP endpoint reachable`
    </Collapser>
</CollapserGroup>
