...
<Tabs>
  <TabsBar>
    ...
    <TabsBarItem id="adb">ADB</TabsBarItem>
  </TabsBar>
  <TabsPages>
    <TabsPageItem id="cdb">
...
<Steps>
  ...
  <Step>
    ...
    Run these commands as the SYS user inside your ADB. This gives the monitoring tool the basic access it needs to collect metrics, track events, and find your database instances.
...
## SQL query receiver [#sqlqueryreceiver]
...
<CollapserGroup>
  ...
  <Collapser
    title="SQL query receiver configuration"
    id="otelreceiver"
  >

### Prerequisites [#sqlquery-prerequisites]
...
**Note:** For Oracle CDB users (`C##` prefix), encode `#` as `%23` in the datasource URL.

Example: `c##newrelic` → `c%23%23newrelic`

</Callout>

### Naming convention [#sqlquery-naming]

Metric names **must** start with `oracledb.` (Oracle) or `sqlserver.` (SQL Server) to be associated with the correct entity in New Relic. Metrics with a different prefix will be ingested but will not appear under the database entity.

### Entity synthesis — making custom metrics appear under the same entity [#sqlquery-entity]
...
### Pipeline configuration [#sqlquery-pipeline]
...
**Note:** `resource/add_host` must come before `batch` in the processor list.

</Callout>

### NRQL to validate data [#sqlquery-validate]
...
If you encounter any issues during SQL Query Receiver configuration, refer to our [Troubleshooting section](#troubleshoot) for common solutions and fixes.
...
<CollapserGroup>
  ...
  <Collapser
      id="sql-query-receiver-issues"
      title="SQL Query Receiver configuration issues"
      >
      ...
      | Metrics not under database entity | Metric name missing required prefix | Ensure metric names start with `oracledb.` (Oracle) or `sqlserver.` (SQL Server) |
