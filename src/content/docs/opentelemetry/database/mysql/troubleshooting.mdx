...
<Collapser
  id="no-query-plansa"
  title="Query plans missing for MySQL 5.x"
>

**Symptoms:** No query plans available in `db.server.top_query` events.

**Cause:** Expected behavior — `query_sample_text` doesn't exist on MySQL versions before 8.0.

**Solutions:** Upgrade to MySQL 8.0+ for query plan support
