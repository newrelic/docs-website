...
2. Update your cluster name in both `daemonset-configmap.yaml` and `deployment-configmap.yaml` files:
    ...
    Locate instances of `k8s.cluster.name` and replace `<cluster_name>` with your desired cluster name:
...
After successfully installing ATP in your Kubernetes environment:
...
* [Troubleshoot ATP issues](/docs/opentelemetry/nrdot/atp/troubleshooting)