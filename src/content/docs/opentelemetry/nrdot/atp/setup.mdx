...
Once the collector is installed, you need to edit the collector's configuration file to enable and customize ATP. The configuration file paths vary depending on your installation method:
...
1. Open the configuration file with a text editor. For DEB/RPM installations, you may need root permissions:

  ```bash
  sudo nano /etc/nrdot-collector-host/config.yaml
  ```

2. In your configuration file, add the following ATP processor configuration code snippet under the `processors` section:
...
3. Update the ATP configuration as required. For more details on configuration options, see the [Configuration parameters](#config-parameters).
