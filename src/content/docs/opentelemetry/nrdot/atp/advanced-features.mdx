...
`storage_path`: File path where the processor stores historical metric data. This is required for persistence across restarts.

* Default: `/var/lib/nrdot-collector/adaptiveprocess.db`
* Use case: Change this if you need a different storage location (must be under `/var/lib/nrdot-collector/`)
...
`metric_thresholds`: Static threshold values for each metric. A process gets flagged when it **exceeds** this value.
...
* When to enable: You want to catch sudden changes (process gone rogue)
* When to disable: You only care about absolute thresholds
...
After you've configured advanced ATP features:
