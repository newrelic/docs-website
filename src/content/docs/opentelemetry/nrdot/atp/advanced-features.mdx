...
ATP includes advanced intelligence layers that provide sophisticated, adaptive telemetry processing beyond simple threshold-based filtering.

## Advanced configuration options [#advanced-config]

The following configurations are **optional parameters** that can be added to your ATP processor configuration to enable advanced features.

### Dynamic thresholds [#dynamic-thresholds]

Automatically adjusts thresholds based on historical behavior.
...
Evaluates multiple metrics together for holistic process health.
...
Detects sudden metric spikes above historical averages.
...
`storage_path`: File path where the processor stores historical metric data. Required for persistence across restarts.

* Default: `/var/lib/nrdot-collector/adaptiveprocess.db`
* Use case: Change if you need a different storage location (must be under `/var/lib/nrdot-collector/`)

`retention_minutes`: How long (in minutes) to keep tracking a process after it last exceeded a threshold.

* Default: 30 (max: 30)
* Tuning: Lower values (5-10) for short-lived processes; keep at 30 for stable workloads

### Include process list

`include_process_list`: List of processes that always bypass all filters and get reported regardless of thresholds.

* Use case: Critical processes you always want to monitor (e.g., database, web server)
* Security: Use full paths (`/usr/bin/postgres`) instead of basenames (`postgres`) to prevent spoofing
...
`metric_thresholds`: Static threshold values for each metric. A process is flagged when it **exceeds** this value.

**How to tune:**

* Start with baseline values from your average workload
* Increase thresholds to reduce noise (fewer alerts)
...
* When to enable: Processes with fluctuating behavior (batch jobs, API servers)
...
`min_thresholds`: Floor values - thresholds won't drop below these.

* Use case: Prevent thresholds from becoming too sensitive

`max_thresholds`: Ceiling values - thresholds won't exceed these.

* Use case: Prevent thresholds from becoming too lenient
...
* Example: `cpu: 0.5, memory: 0.5` = equal importance

### Anomaly detection

Detects sudden spikes or unusual behavior by comparing current values against historical patterns.

`enable_anomaly_detection`: Turns on spike detection.

* When to enable: Want to catch sudden changes (process gone rogue)
* When to disable: Only care about absolute thresholds
...
2. Observe for 1-2 days, adjust thresholds to reduce false positives
...
* **Stable production services**: Static thresholds only
* **Batch jobs**: Dynamic thresholds + anomaly detection
* **Resource-intensive apps**: Multi-metric scoring
* **Critical processes**: Add to `include_process_list`

## What's next [#whats-next]

After configuring advanced ATP features:

* [Monitor and troubleshoot ATP](/docs/opentelemetry/nrdot/atp/troubleshooting)
