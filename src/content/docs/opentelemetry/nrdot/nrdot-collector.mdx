...
<Callout variant="tip">
  ...
  The NRDOT Collector supports [FIPS 140-3](https://csrc.nist.gov/pubs/fips/140-3/final) compliance. To use the FIPS-compliant version, use the Docker image that includes the `-fips` suffix in the version tag (for example, `newrelic/nrdot-collector:1.12.0-fips`). This version uses BoringCrypto ([NIST Certificate](https://csrc.nist.gov/projects/cryptographic-module-validation-program/certificate/4735)) to ensure all TLS communications use approved cipher suites. For more information, refer to [NRDOT FIPS documentation](https://github.com/newrelic/nrdot-collector-releases/blob/main/fips/README.md).
