...
example, `otel.status_code = ERROR`), or if the span's attributes map includes an `error.type` attribute.
...
<Callout variant="tip">
  You can filter errors out before they're sent to New Relic. See the [ignored errors example](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors) for a working reference.
</Callout>
