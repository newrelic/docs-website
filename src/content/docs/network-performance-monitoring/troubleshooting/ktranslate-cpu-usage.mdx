...
Flow containers primarily see CPU usage increases due to the number of inbound flow events. As a general estimate, you should be able to ingest roughly 2,000 flows per second for each CPU, but loads vary slightly for the different flow protocols.
...
Syslog containers primarily see CPU usage increases due to the number of inbound syslog events. As a general estimate, you should be able to ingest roughly 2,000 messagess per second.
