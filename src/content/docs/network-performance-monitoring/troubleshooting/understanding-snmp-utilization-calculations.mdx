...
<CollapserGroup>
  ...
  >
    ...
    It's important to note that there's a difference between utilization, which is typically represented as bits (lowercase `b`), and throughput, which many tools display in bytes (uppercase `B`). The raw measurement in SNMP is counting octets, which in this context would be synonymous with bytes.

    As a result, you can easily calculate throughput directly through the reported metric without multiplying the octets to convert into bits like you see in utilization queries.
  </Collapser>
  <Collapser
    id="interface-errors"
  ...
  >
    <DoNotTranslate>**Metric Name**</DoNotTranslate>: Various
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td> Used to convert hexadecimal values into integer format. Options for <DoNotTranslate>**current**</DoNotTranslate>: `LittleEndian` | `BigEndian`. Options for <DoNotTranslate>**desired**</DoNotTranslate>: `uint16` | `uint32` | `uint64` </td>
        </tr>
        <tr>
