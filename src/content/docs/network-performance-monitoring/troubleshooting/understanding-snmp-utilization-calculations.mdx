...
<CollapserGroup>
  ...
  >
    **Metric Name**: `kentik.snmp.ifHCInOctets` | `kentik.snmp.ifHCOutOctets`
    ...
    It's important to note that there is a difference between utilization, which is typically represented as bits (lowercase `b`), and throughput which many tools display in bytes (uppercase `B`). The raw measurement in SNMP is counting octets, which in this context would be synonymous with bytes.

    As a result, calculation of throughput can be achieved directly through the reported metric, without multiplying the octets to convert into bits like you see in utilization queries.
  </Collapser>
  <Collapser
    id="interface-errors"
