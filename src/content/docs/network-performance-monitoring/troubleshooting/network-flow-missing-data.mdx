...
<SideBySide>
  <Side>
    All network flow telemetry is stored in the [KFlow event type](/docs/network-performance-monitoring/setup-performance-monitoring/network-flow-monitoring/#find-your-metrics). You can query this directly in NRQL. Absence of this event type indicates your account is not receiving the data.
  </Side>
  <Side>
    ```sql
    FROM KFlow SELECT *
...
<CollapserGroup>
  ...
  >
    `ktranslate` can only collect one type of flow template in a running container or service. During runtime, you will configure the type with the [-nf.source](/docs/network-performance-monitoring/advanced/ktranslate-container-management/#container-runtime-options) argument, which tells `ktranslate` which template to expect so it can translate the packets.

    A common misstep here is running multiple containers or services, each collecting a different flow template type, but not isolating the incoming traffic to a unique port.

    When running multiple containers or services of a different type, you need to set each to a different value for [-nf.port](/docs/network-performance-monitoring/advanced/ktranslate-container-management/#container-runtime-options) at runtime (default is `9995`). Otherwise, each running instance of `ktranslate` will async grab incoming packets and there's a high risk that the wrong templates will be applied, resulting in errors in the logs and/or no data in New Relic. You will also need to update the flow exporter configuration on your source network devices to target their specific port.
  </Collapser>
  <Collapser
  ...
  >
    Vendors will have their own documentation on watching flow export counters via their device CLI/UI. A lack of counter growth on the device indicates the flow export is not configured correctly on the device.

    Both the Docker container and Linux service deployment options for `ktranslate` utilize the host's network to receive data on the mapped port. In order to validate flow records are being received by the host, you can use the [tcpdump](https://www.tcpdump.org) utility to create a packet capture (`.pcap`) file that you can later investigate in [Wireshark](https://www.wireshark.org/).
    ...
    <Callout variant="tip">
      One of the most common issues found is a network configuration/firewall rule that blocks the packets from the source network devices to the target `ktranslate` host. If you're not getting any results with the `tcpdump` utility, confirming your network rules is the best starting point for resolution.
    </Callout>
  </Collapser>
  <Collapser
  ...
  >
    [Wireshark](https://www.wireshark.org/) is a free utility that you can utilize for inspection of the packet capture file following these steps:
        ...
        <CollapserGroup>
          ...
          >
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td> Datagram version </td>
                  <td> The version of this sFlow packet. </td>
                </tr>
                <tr>
                  <td> Agent address type </td>
                  <td> IPv4 (1) or IPv6 (2) </td>
                </tr>
                <tr>
                  <td> Agent address </td>
                  <td> IP address the flows are being exported from. This is where you have your flow exporter configured. </td>
                </tr>
                <tr>
                  <td> Sub-agent ID </td>
                  <td> In sFlow v5, you can run multiple exporter processes. This is their unique identifier. </td>
                </tr>
                <tr>
                  <td> Sequence number </td>
                  <td> The number of sFlow packets sent by the agent device. </td>
                </tr>
                <tr>
                  <td> SysUptime </td>
                  <td> Time since the agent device last rebooted. </td>
                </tr>
                <tr>
                  <td> NumSamples </td>
                  <td> The count of sFlow samples contained in the current packet. </td>
                </tr>
              </tbody>
            </table>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td> Enterprise </td>
                  <td> This field annotates custom sFlow enterprise configurations that administrators can optionally enable when configuring their sFlow exports. (`0` by default) </td>
                </tr>
                <tr>
                  <td> sFlow sample type </td>
                  <td> This is the designation for the type of sample being used when an enterprise customizes their sFlow exports. You can find definitions in [the sFlow documentation](http://www.sflow.org/developers/diagrams/sFlowV5Sample.pdf). </td>
                </tr>
                <tr>
                  <td> Sample length </td>
                  <td> Length of the sample, in bytes. </td>
                </tr>
                <tr>
                  <td> Sequence number </td>
                  <td> Counter value incremented every time the agent takes a sample. </td>
                </tr>
                <tr>
                  <td> Sampling rate </td>
                  <td> 1 in `X` packets are sampled. </td>
                </tr>
                <tr>
                  <td> Sample pool </td>
                  <td> Total possible packets that could have been sampled, including the actual sampled packets. </td>
                </tr>
                <tr>
                  <td> Dropped packets </td>
                  <td> Number of packets that were dropped due to resource constraints. </td>
                </tr>
                <tr>
                  <td> Input interface </td>
                  <td> SNMP ifIndex of the interface the packet arrived in from. </td>
                </tr>
                <tr>
                  <td> Flow record </td>
                  <td> description </td>
                ...
                <tr>
                  <td> Raw packet header | ipv4 packet </td>
                  <td> Number of sampled records contained in this sample. </td>
                </tr>
              </tbody>
            </table>
          </Collapser>
