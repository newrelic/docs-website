...
<CollapserGroup>
  ...
  >
    ...
    When running multiple containers or services of a different type, you need to set each to a different value for [-nf.port](/docs/network-performance-monitoring/advanced/ktranslate-container-management/#container-runtime-options) at runtime (default is `9995`). Otherwise, each running instance of `ktranslate` will asynchronously grab incoming packets, and there's a high risk that the wrong templates will be applied, resulting in errors in the logs and/or no data in New Relic. You will also need to update the flow exporter configuration on your source network devices to target their specific port.
  </Collapser>
  <Collapser
  ...
  >
    ...
    <Callout variant="tip">
      One of the most common issues found is a network configuration/firewall rule that blocks the packets from the source network devices to the target `ktranslate` host. If you're not getting any results with the `tcpdump` utility, the best place to start troubleshooting is to confirm your network rules.
    </Callout>
  </Collapser>
  <Collapser
  ...
  >
    [Wireshark](https://www.wireshark.org/) is a free utility to help you inspect the packet capture file. Follow these steps:
