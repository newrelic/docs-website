...
3. If you have a large set of devices that are being skipped because of the port scan you can edit the `snmp-base.yaml` file and enabling the option for [check_all_ips](https://docs.newrelic.com/docs/network-performance-monitoring/advanced/advanced-config/#discovery) to skip the port scan completely and just go directly to testing SNMP credentials against every address in your discovery.  Keep in mind that this option will dramatically increase the time it takes to complete a discovery.
