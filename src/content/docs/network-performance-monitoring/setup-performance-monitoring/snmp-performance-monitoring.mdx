...
### Docker method prerequisites [#prerequisites-docker]
...
* SSH or console access to that host
* Ability to launch new containers via command line
* Network access as defined in the [network security prerequisites](https://docs.newrelic.com/install/npm/)


### Linux host-based method prerequisites [#prerequisites-linux-host]

The Docker container method above is recommended, but ktranslate can be installed directly onto Linux.  You will need:

* SSH or console access to the host
* Ability to install/remove applications and services
* Network access as defined in the [network security prerequisites](https://docs.newrelic.com/install/npm/)
...
4. If you are using the non-Docker host-based installation method and you have a proxy, add the proxy details to `/etc/default/ktranslate.env` after installation, but before starting the `ktranslate` service. The `ktranslate.env` file will have the following format:
  ```
...
If you are using the container method, proxy details are instead passed as a [container runtime option](/docs/network-performance-monitoring/advanced/ktranslate-container-management/#container-runtime-options).
...
<CollapserGroup>
  ...
  >
    Before reading about installing SNMP manually, consider using our guided install process to avoid errors:
