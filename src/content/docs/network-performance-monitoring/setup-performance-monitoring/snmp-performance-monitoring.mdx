...
<InlineSignup/>
* For manual install you'll need the following (guided install inputs them automatically): 
...
The Docker container method (below) is recommended, but ktranslate can be installed directly onto Linux.  You will need:
...
<Callout variant="important">
  If you have a proxy, you will need to ensure it is added to the /etc/default/ktranslate.env file after installation but before starting the ktranslate service.  The .env file will have the following format:
...
Kentik's ktranslate docker container is the recommended method of deploying SNMP monitoring.  To use it you will need:
...
<CollapserGroup>
  ...
  >
    ...
    Installing manually is not recommended as it is more error prone and support may be unable to assist if issues arise.  If possible, please use the guided install:
    <ButtonLink
      data-tessen="stitchedPathLinkClick"
      role="button"
      to="https://one.newrelic.com/nr1-core?state=c805c263-f7a8-9dad-0e24-54c64f4fa615"
    ...
    If you still prefer to setup the container manually, proceed with the following steps.
