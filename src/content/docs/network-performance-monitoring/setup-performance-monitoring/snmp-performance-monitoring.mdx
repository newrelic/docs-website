...
It's recommended to deploy the SNMP monitoring agent as a container for either <DNT>Docker/DNT> or Podman. If you need it, you can alternatively install it as a baremetal service in Linux.

<CollapserGroup>

    <Collapser id="docker" title="Docker installation prerequisites">
    We recommend using a <DNT>Docker/DNT> container to deploy the agent for SNMP monitoring. To use it, you need:

        * <DNT>Docker/DNT> installed on one of the [Linux platforms supported by <DNT>Docker/DNT>](https://docs.docker.com/engine/install/#supported-platforms).
        * Ability to launch new containers via command line.
    </Collapser>

    <Collapser id="podman" title="Podman installation prerequisites">
    ...
    <Collapser id="linux" title="Baremetal Linux service installation prerequisites">
...
<CollapserGroup>
    ...
    <Collapser id="net-sec" title="Network security prerequisites">
...
<Collapser id="snmpv3-options" title="SNMP v3 options">
...
<CollapserGroup>
  ...
  <Collapser id="guided-install-setup" title="Guided install setup">
    ...
    3. Follow the steps outlined in the guided installation process. You can use <DNT>Docker/DNT>, Podman, or Linux.
  ...
  <Collapser id="manual-container-setup" title="Manual container setup">
    ...
    <Tabs>
      ...
      <TabsBar>
        ...
        <TabsBarItem id="dockerInstall">Docker container</TabsBarItem>
        <TabsBarItem id="podmanInstall">Podman container</TabsBarItem>
        <TabsBarItem id="linuxInstall">Linux service</TabsBarItem>
      ...
      <TabsPages>
        ...
        <TabsPageItem id="dockerInstall">
          1. On a Linux host with <DNT>Docker/DNT> installed, download the <DNT>**ktranslate**</DNT> image by running one of the following:
          ...
          2. Copy the `snmp-base.yaml` file to the local `$HOME` directory of your <DNT>Docker/DNT> user, and discard the container by running:
...
2. Change the <DNT>Docker/DNT> container's arguments slightly, from `-p 162:1620/udp` to `-p $src:1620/udp` where `$src` is the port your `v3` traps are arriving on.
...
<Collapser id="trap-container-setup" title="Manual container setup for SNMP traps">
    ...
    <Tabs>
      ...
      <TabsBar>
        ...
        <TabsBarItem id="dockerTrapInstall">Docker container</TabsBarItem>
        <TabsBarItem id="podmanTrapInstall">Podman container</TabsBarItem>
      ...
      <TabsPages>
        ...
        <TabsPageItem id="dockerTrapInstall">
          ...
          1. On a Linux host with <DNT>Docker/DNT> installed, create the configuration file you'll use to run the container:
            ```shell
