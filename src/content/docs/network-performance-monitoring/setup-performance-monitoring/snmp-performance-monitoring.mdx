...
>
  ...
  >
    ...
    4. Run `ktranslate` to poll target devices and listen for incoming SNMP Traps, making sure to replace `YOUR_NR_LICENSE_KEY`, `YOUR_NR_ACCOUNT_ID`, and `UNIQUE_NAME`:
