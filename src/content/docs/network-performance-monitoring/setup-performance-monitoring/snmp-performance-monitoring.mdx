...
If you have a proxy, add it to the `ktranslate.env` file in `/etc/default/` after installation, but before starting the `ktranslate` service. The `ktranslate.env` file needs to be similar to:
