...
5. Run `ktranslate` to poll target devices by running:
    ...
    kentik/ktranslate:v2 \
      ...
      -tee_logs=true \
      nr1.snmp
      ## If your account is located in Europe, add the following option before the nr1.snmp line:
      ## -nr_region=EU \
      ## FedRAMP customers need to add the following flag before the nr1.snmp line to use the FedRAMP Authorized endpoints
      ## -nr_region=GOV \
    ```
