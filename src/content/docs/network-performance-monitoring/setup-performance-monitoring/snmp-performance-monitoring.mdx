...
<CollapserGroup>
  ...
  >
    <Steps>
    <Step>
    Go to [one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Add more data.
    </Step>
    <Step>
...
In some circumstances, it is beneficial to isolate the collection of SNMP trap messages into a dedicated container. This is helpful to control scale in large environments as well as creating a distributed monitoring footprint with lower risk of full outages if a container fails.
...
>
  ...
  <Callout variant="tip">
    You can also control device names by providing a [-dns container argument](/docs/network-performance-monitoring/advanced/ktranslate-container-management/#container-runtime-options) at runtime which will allow the container to run a lookup on the source IP address and attempt name resolution.
  </Callout>
  </Step>
  <Step>
  ...
  <Callout variant="tip">
    It's important to remember that SNMP trap messages are unsolicited events generated by the source device. If you are not seeing messages in New Relic, ensure your devices have actually created messages. Vendor documentation on sending sample messages varies, but you can use [snmptrap](http://www.net-snmp.org/docs/man/snmptrap.html) on your Docker host to send a test message like this:
