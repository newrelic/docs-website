...
<CollapserGroup>
    ...
    <Collapser id="docker" title="Docker prerequisites">
    ...
    <Collapser id="podman" title="Podman prerequisites">
    ...
    <Collapser id="linux" title="Linux host prerequisites">
    ...
    <Collapser id="net-sys" title="Network syslog devices prerequisites">
      <Callout variant="important">
...
<CollapserGroup>
  ...
  <Collapser id="guided-install-setup" title="Set up with the guided install">
    ...
    2. Scroll down until you see <DoNotTranslate>**Network**</DoNotTranslate> and click <DoNotTranslate>**Syslog**</DoNotTranslate>.
    3. Follow the steps outlined in the guided installation process. You can use Docker or Linux.
      ...
      <figcaption>
        <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Add more data > Network > Syslog**</DoNotTranslate> to set up syslog message monitoring.
      </figcaption>
      ...
      <TabsPages>
        ...
        <TabsPageItem id="dockerInstall">
          ...
          4. Run `ktranslate` to listen for network flows with the command:
            ...
            ```
            <Callout variant="tip">
              The default port the container listens for syslog messages on is `514/udp`. If your messages are being sent over a different port, you will need to change the argument `-p 514:5143/udp` to `-p $srcPort:5143/udp`.
            </Callout>
        ...
        <TabsPageItem id="podmanInstall">
          ...
          4. Rootless Podman containers are not able to bind to ports under 1024. To handle packet redirection for your syslog messages, you will need to create an `iptables` rule that targets the port your packets are arriving on UDP (`$scrPort`) with the command:
            ```shell
