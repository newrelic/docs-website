...
Set up your AWS VPC flow logs to send them to New Relic. If you're not familiar with Amazon VPC, check [Amazon's official documentation](https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html).
...
* [Amazon Resource Names (ARNs)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-arn-format.html):
    ...
    * ARN for the [Kinesis Data Firehose with S3 bucket access](https://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#using-iam-s3).
    <Callout variant="tip">
        We recommend you to create a new data firehose with our guided install the first time you configure VPC Flow Logs for a specific region. For subsequent VPCs and subnets in the same region, you can reuse the existing data firehose.
    </Callout>
...
You can set up your AWS VPC flow logs by install a bundle that includes a dashboard designed for AWS VPC flow logs.
...
4. In the **Select your install method** section, continue with **CLI** and click **Continue**.
    <Callout variant="tip">
        The **CloudFormation script** option isn't available yet.
    </Callout>
...
<CollapserGroup>
    <Collapser
    id="define-lambda"
    title="Define Sampling Lambda"
    > 
    In the **Define Sampling Lambda** section, do the following:
    ...
    <Collapser
    id="create-lambda"
    title="Create Sampling Lambda"
    > 
    In the **Create Sampling Lambda** section, click **Generate CLI Command** and:
...
<img title="Generate Kinesis Firehose" alt="Generate Kinesis Firehose" src={kinesisFirehoseGuidedInstall}/>
