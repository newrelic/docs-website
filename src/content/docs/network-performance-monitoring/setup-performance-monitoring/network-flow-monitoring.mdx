...
<CollapserGroup>
    ...
    <Collapser id="net-flow" title="Network flow data devices prerequisites">
...
<Collapser id="flow-types" title="Network flow types">
...
<CollapserGroup>
  ...
  <Collapser id="guided-install-setup" title="Guided installation setup">
      ...
      <TabsPages>
        ...
        <TabsPageItem id="podmanInstall">
          ...
          4. Run `ktranslate` to listen for network flows with the command:
            ...
            ```
            <Callout variant="tip">
              Rootless Podman containers are not able to bind to ports under 1024. If your network flows are sent on a port under 1024 instead of the default (9995), you will need to create an `iptables` rule to handle packet redirection with the command:
              ```shell
...
<CollapserGroup>
  ...
  <Collapser id="kflow-fields-collapser" title="KFlow Fields">
