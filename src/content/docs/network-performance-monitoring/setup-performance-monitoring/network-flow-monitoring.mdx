...
- Configuration of network device(s) to send flow data to the host running the **ktranslate** docker container:
  - NetFlow data
...
> ***This is only a sample of configuration examples. There are many other devices supported.***

## Supported network flow data types [#supported-network-flow-data-types]
...
3. Follow the steps in New Relic One.
  ![Network Flows guided setup](./img/network-flows-guided-install.png "Network Flows guided setup")
    <figcaption>
    **[one.newrelic.com](https://one.newrelic.com)** > **Add more data** > **Network performance monitoring** > **Network Flows** to set up network flow data monitoring.
    </figcaption>
...
<CollapserGroup>
  ...
  <Callout variant="tip">
  This command assumes collection of `sflow` data. If you are collecting other flow types, you should change the suffix in the `--name` flag for the container and update the `-nf.source` argument as necessary
  </Callout>
