...
This document serves as an overview of the best practices available for deployment of the [KTranslate container](https://github.com/kentik/ktranslate) used in New Relic’s network monitoring capability.
The guidance here is based on the recommendations of various subject matter experts combined with common patterns of deployment that customers are using in their production environments today.
...
The diagram visualizes common network architecture and includes the following:

* 3 containers on 1 host serving the ‘DC_01’ location in America. These containers process SNMP polling, NetFlow v5 collection, and Syslog collection
* 1 container on 1 host serving the ‘OFFICE_01’ location in Sydney, Australia. This container processes SNMP polling and SNMP Trap collection.
* 3 containers on 1 host service the new ‘DC_02’ location in Dublin, Ireland. These containers process NetFlow v9, IPFIX, and sFlow collection.

## Architectural considerations [#archictural-considerations]

### Container task

There are some basic rules around what an individual container is capable of doing which are the main drivers when designing your deployment.
...
#### About Network Flow and Syslog containers [#about-containers]

Technically speaking, there is no requirement for a custom configuration file when using one of these containers. The default from the container image will work fine as all of your container settings will be placed by flags at runtime. 

However, without a configuration file with entries in the devices section, the results sent to New Relic’s APIs will use device_name entries resolved via DNS from the IP address in the respective packet. There is support for providing a custom DNS server location at runtime, but full control with tagging is generally accomplished by providing your own entries with the flow_only setting set to true. 
Administrators will generally want to control this name so it aligns with the name sent in from SNMP polling the same device.
...
### Summary of architectural considerations [#summary]

Summary
In our architecture sample above, the following considerations are in place:

* Each geographical location has its own local containers used to collect and forward data to New Relic.
* DC_01 has 3 total containers:

    * 1 - SNMP polling*
    * 1 - NetFlow v5 collection
    * 1 - Syslog collection
...
* OFFICE_01 uses a single container:

    * 1 - SNMP polling with discovery against a /24 subnet

* DC_02 has 3 containers, all dedicated to different templates in network flow collection:
...
The most common need for ongoing maintenance is in keeping the list of target devices accurate. There are 3 main patterns supported for this goal:

* Automatic discovery (preferred)
* Manual discovery
* Manual device addition

### Automatic discovery (preferred) [#automatic-discovery]

“Discovery” is the process used by KTranslate to scan a target list of IP addresses and/or ranges, perform a liveness probe, and then run a basic SNMP walk of the MIB-2 System MIB to attempt matching the device to a known SNMP profile.

The container has embedded container runtime flags (-snmp_discovery_min and -snmp_discovery_on_start) that allow administrators to build a schedule of recurring SNMP discovery events. In effect, this will automate discovery jobs against the targets from the discovery section in the configuration file, and then automatically update the file with new devices along with refreshing the service so it accepts said changes. 
...
Using our example office location; your default initial configuration file would look like this:
...
And you associated Docker run command would look like this, replacing `$NR_LICENSE_KEY` and `$NR_ACCOUNT_ID`:
...
This pattern uses the same discovery mechanism as the automated option, but gives the administrator more control by allowing you to run a bespoke container ad hoc where you have a chance to review and manipulate the results as your needs dictate. This is generally the motion used in environments where tagging is prevalent or there is a good amount of control from a centralized team adding new devices into the network, reducing the need for full subnet scanning.
...
This pattern follows the original deployment option for KTranslate, with the [steps outlined in our documentation](/docs/network-performance-monitoring/setup-performance-monitoring/snmp-performance-monitoring/#manual-container-setup). At a high level, the process contains:
...
An example of the device settings needed to successfully monitor an APC UPS:
...
Required settings are outlined in detail in our documentation for the [devices](/docs/network-performance-monitoring/advanced/advanced-config/#devices) and [global blocks](/docs/network-performance-monitoring/advanced/advanced-config/#global).