...
In the [common architecture example](#common-architecture) above, the following considerations are in place:
...
The most common need for ongoing maintenance is in keeping the list of target devices accurate. There are three options you can take to support this goal:

* (Recommended) Option 1: [Automatic discovery](#automatic-discovery)
* Option 2: [Manual discovery](#manual-discovery)
* Option 3: [Manual device addition](#manual-device-addition)
...
The container has embedded container runtime flags (`-snmp_discovery_min` and `-snmp_discovery_on_start`) that allow administrators to build a schedule of recurring SNMP discovery events. This automates discovery jobs against the targets from the discovery section in the configuration file, and then automatically update the file with new devices along with refreshing the service so it accepts said changes. 
...
### Manual device addition [#manual-device-addition]
