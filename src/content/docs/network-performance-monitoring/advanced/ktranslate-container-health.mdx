...
The **ktranslate** container image has the `-tee_logs=true` and `-metrics=jchf` settings available during runtime, which allow it to send health metrics into New Relic One directly. These are enabled by default when installing network performance monitoring via the New Relic One guided install and it's a recommended setting when installing manually.
