...
The **ktranslate** container image has the `-tee_logs=true` and `-metrics=jchf` settings available during runtime, which allow it to send health metrics into New Relic One directly. These are enabled by default when installing network monitoring via the New Relic One guided install. We recommend you to set them up when installing network monitoring manually.
