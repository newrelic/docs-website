---
title: KTranslate <DNT>Docker/DNT> container health monitoring
tags:
  - Integrations
  - Network monitoring
...
While running the KTranslate <DNT>Docker/DNT> container for New Relic network monitoring, you can monitor the health of the container to proactively detect potential issues.
...
<Callout variant="tip">
  If you want to check the logs locally from the <DNT>Docker/DNT> host, run `docker logs $CONTAINER_NAME`. For example, `docker logs ktranslate-snmp`.
</Callout>
...
<CollapserGroup>
  ...
  <Collapser
    id="docker-arguments"
    title="What arguments were passed to <DNT>Docker/DNT> at runtime?"
  >
    Logs UI:
