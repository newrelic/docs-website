...
Guided install is fairly technically complex. As we identify areas where the guided install fails, we'll document them here and provide some guidance.

### MySQL: Incorrect user permissions

To monitor MySQL health data, you need a valid username and password for a user with specific permissions. 

These commands will create a user and grant the required permissions:

Creates a user `newrelic@localhost` with a specific password.
...
Once done, tour next guided install attempt should work.
...
There are a different ways to set `status_url`, depending on how NGINX was installed:
