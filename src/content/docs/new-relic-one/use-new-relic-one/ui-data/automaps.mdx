...
To use automap, you need a New Relic account. If you do not already have one, you can sign up [here](https://newrelic.com/signup) for a free account.
...
To identify exactly when and where an incident originated, simply move the **Timewarp** cursor to see health status changes for the entities on the map. If you do not see the **Timewarp** cursor, click the clock icon in the upper-right of the automap. When you scroll through the history, you see when anomalous behavior was detected by New Relic Lookout.
