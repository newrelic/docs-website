...
```
You can find information about fetching relationships in this [relationships-api-tutorial](/docs/apis/nerdgraph/examples/nerdgraph-relationships-api-tutorial/).
...
* For each entity, a user can manually define up to 2000 relationships. When the limit is reached, the API will return a `LIMIT_EXCEEDED` error.
* Each mutation can fail if the user doesn't have access to one of the two entities (source/target).
