...
![New Relic filtering workloads using tags](src/images/new-relic-tags-workloads_0.png 'New Relic filtering workloads using tags')

<figcaption>
  Here you can see an `accountId` tag being used to filter
...
<Callout variant="tip">
  Tags were previously called `labels` in the New Relic UI. You may sometimes
  see `label`in our code; for example, some of our APM agent config files use a
  `label` config option for tags.
</Callout>
...
![New Relic advanced tag filter query](src/images/new-relic-advanced-tag-filter-query.png 'New Relic advanced tag filter query')

<figcaption>
  Use the filter field at the top of the [New Relic
  Explorer](/docs/new-relic-one/use-new-relic-one/ui-data/entity-explorer-view-performance-across-apps-services-hosts)
  to filter down to the entities you care about. You can use multiple filter
  conditions.
</figcaption>
