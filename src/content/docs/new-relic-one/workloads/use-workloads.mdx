...
To view [workloads](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-one-workloads-isolate-resolve-incidents-faster), go to [one.newrelic.com](https://one.newrelic.com) and find them on the **Explorer**. There are three main tabs (Health, Activity, and Owner) plus the header.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > Workloads > Create a workload:** When you [create a workload](#create), you choose the associated accounts and monitored entities.
</figcaption>
...
<CollapserGroup>
  ...
  >
    ...
    4. From a workload’s **Overview** page, select a dashboard.

    5. Add search terms to filter the dashboard to a view that’s relevant for that workload.

    6. Select **Save filter for this workload**.
  </Collapser>
</CollapserGroup>
