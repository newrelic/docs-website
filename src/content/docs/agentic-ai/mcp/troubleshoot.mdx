...
### Resolution
An administrator in your organization must implement one of the following two solutions to grant you access:
1. Grant the group an organization-level role:
    - The administrator must add an appropriate organization-level role (or the specific required grant) to the custom group you belong to.
2. Move you to an authorized group:
    - The administrator must add you to an existing user group that already has the necessary organization-level access privileges for the MCP Server.
