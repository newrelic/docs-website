...
Your organization administrator must enable MCP access via [feature control manager](/docs/accounts/accounts-billing/new-relic-one-user-management/feature-control-manager/). If you cannot connect, ask your administrator to verify the following toggles are enabled:

- **New Relic MCP Server**: Main toggle that must be on for any MCP access to work
- **Local Clients**: Required for local client connections (Claude Code, Claude Desktop, VS Code, Windsurf, Kiro)
