...
* **Permission to create a user API key**: Invoking <DNT>Autopilot</DNT> from <DNT>Slack</DNT> requires a user API key on the accounts used by <DNT>Workflow Automation</DNT>. Only users whose role includes permission to create a user API key on those accounts can use the <DNT>Slack</DNT> integration.
