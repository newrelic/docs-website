...
redirects:
  ...
  - /docs/agentic-ai/knowledge-base/overview
---
...
- Easily find solutions to problems that have been solved before. Ask questions like:
  ...
  At this time, all indexed documents can be retrieved by all users within your organization. Please verify that only appropriate content is indexed, as there's currently no option to restrict access or redact information after indexing.
...
<img
  title="High-level visual of New Relic AI Knowledge"
  alt="High-level visual of New Relic AI Knowledge"
...
You have two options to assign the Organization Manager role:
...
- **Create a dedicated group:** For more granular control, create a new user group specifically for this purpose and assign the Organization Manager role to that group.
...
<Steps>
  ...
  <Step>
      Search for <DNT>**NRAI Knowledge**</DNT>.
  </Step>
...
When interacting with the New Relic AI Agent in chat, you can control how your knowledge bases are searched:

* **Targeted search**: To restrict the AI to a specific knowledge source, type `/{connector-name}` before your question (for example: `/Confluence-Runbooks How do I triage a high CPU alert?`). The New Relic AI Agent will exclusively search that specific connector to formulate its answer.
* **Broad search**: If you do not specifically select a connector, the New Relic AI Agent will perform a broad search across all configured knowledge connectors. It automatically determines which sources to pull from based on how closely your question aligns with each connector's configured description and knowledge category.
