...
- Appropriate user permissions are configured for indexing (you'll need the Organization Manager role, which allows you to perform actions—such as set up and manage the knowledge connectors—that may have future billing implications).

You have two options to assign the Organization Manager role:
- **Apply to an existing user group:** Add the Organization Manager role to an existing group of users who will be responsible for managing the knowledge connectors.
