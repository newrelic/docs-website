...
From the query results, look for permissions related to agentic AI or AI Integration in the `feature` or `category` fields. Note down the corresponding `id` values as you'll need them in the next step.
</Callout>
...
After you have the permission IDs, create a custom role for agentic integration access. Replace `<your org id>` with your actual organization ID and `$id` with the permission ID(s) from Step 1:
...
3. Provide a descriptive name for your group (for example, Agentic AI Users).
...
From the results, copy the `id` of the user group you created.
...
To revoke access, perform one of these:
