...
NRQL also includes some advanced mathematical capabilities that can be used for complex calculations. This is helpful if you want to process data to display it more effectively in the UI, or make calculations on queried results in a single step.
...
<Callout variant="tip">
  You can also use this to whether something returns NULL or zero. `(zero) OR 1` returns 0. `(NULL) OR 1` returns 1.
</Callout>
