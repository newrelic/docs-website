...
<CollapserGroup>
  ...
  >
    ...
    * `start:` The beginning of the histogram range.
    ...
    #### First and last buckets include out of range values
    Values that fall out side the defined histogram range are included in the
    first or last buckets. The first bucket count will include items that
    precede the histogram range, and the last bucket count will include items
    that succeed the histogram range. To exclude these values from the histogram results,
    include a filter to the query's where clause.
    (Example: `WHERE attribute >= [start] AND attribute <= [start + width]`)
      ...
      <CollapserGroup>
        ...
        >
          ...
          This means each bucket covers a 0.25 range of values. (10 / 40 = 0.25).
        ...
        >
          This query results in a histogram of response times ranging up to 5 seconds over 10 buckets.
        ...
        >
          This query results in a histogram of response times ranging from 1 to 4 seconds over 3 buckets.
    ...
    * path - An optional string value that is used to directly reference a particular piece of the JSON within the `attribute` parameter. See the [JSON Parse Path Syntax Reference](#json-parse-path-syntax-reference) section below.
    ...
    <Collapser id="json-parse-path-syntax-reference" title="JSON Parse Path Syntax Reference">
      ...
      <table id="join-types">
        <thead>
        <tr>
          <th><DNT>**Path Syntax Example**</DNT></th>
          <th><DNT>**Result Description**</DNT></th>
          <th><DNT>**Result**</DNT></th>
        </tr>
        </thead>
