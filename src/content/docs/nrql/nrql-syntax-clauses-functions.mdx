...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="func-uniqueCount"
    title={<InlineCode>uniqueCount(attribute(s) [, precision: number])</InlineCode>}
  >

    Use the `uniqueCount()` function to get the number of unique values recorded for an attribute over a specified time range. You will get an exact result for up to 256 unique values when you call it without the `precision` argument. For more than 256 unique values, the result is approximate. You can specify a `precision` value to increase the threshold for exact results, up to 50,000 unique values.

    Use the `uniqueCount()` function by specifying the attributes and optionally set the precision argument as follows:

    ```sql
    uniqueCount(attribute(s), precision: number)
    ```
