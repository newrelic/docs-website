...
<CollapserGroup>
  ...
  This feature is currently provided as part of a preview program pursuant to our [pre-release policies](/docs/licenses/license-information/referenced-policies/new-relic-pre-release-policy).
    ...
    With the `PREDICT` clause, you can add predictions of future data trends into line charts based on historical data.
    
    Some important information about this clause:
    ...
    * Does not support [metric timeslice data](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) while in Public Preview.
    ...
    The `PREDICT` clause has the following default behavior:
    * **Seasonality**: An automatic seasonality detection is performed to identify what seasonality the data exhibits, if any.
    * **How far to predict into the future**: Projects 20% of the total time specified in your query window.
    * **Amount of historical data used for training**: Processes historical data from the current query window and the two preceding ones to generate predictions.
    * **Time interval**: Matches the data point interval of the prediction on the query window, ensuring consistency in data projection.
    ...
    You can customize the prediction by manually setting the model hyperparameters and adding the `USING` and `BY` keywords.

    <CollapserGroup>
      <Collapser id="hyperparameters" title="Setting model hyperparameters">
      ...
      <table id="hyperparameters">
        ...
        <tbody>
          <tr>
            ...
            <td>
              Indicates the seaonal behavior to be used for the predictions. There are three supported options:
              * **`seasonality: <season length>`**: Sets the seasonality of the time series to a known season length.
              * **`seasonality: AUTO`**: Automatically detects the seasonality of the time series.
              * **`seasonality: NONE`**: Disables the seasonality of the time series and results in a non-seasonal prediction.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Sets the trend damping parameter. Allowed values are 0 <= `phi` <= 1.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <Collapser id="by" title="Adding the `BY` keyword">
      Use the `BY` keyword to set how far ahead in time you want the model to predict. For example, `PREDICT BY 3 hours` will generate a prediction 3 hours into the future from the latest point in the time series. Must use `interger units` for the 
      amount of time.
      </Collapser>

      <Collapser id="by" title="Adding the `USING` keyword">
      Use the `USING` keyword to specify how much historical data is to be used to train the model. For example, `PREDICT USING 1 day` will use the query window plus 1 days worth of history to train the model. Must use `interger units` for the 
      amount of time.
      </Collapser>
    </CollapserGroup>
