...
<CollapserGroup>
  ...
  >
    ...
    * Label the results returned in each argument with [the `AS` clause](#sel-as).
    ...
    You can select both `*` as a wildcard and individual attributes, [NRQL variables](#with-as-nrql-var), functions, or math expressions in the same `SELECT` statement. Only [non-aggregator functions](#non-aggregator-functions) are supported with `*`. `*` may appear at the beginning or end of the list, and the additional values selected will appear at the beginning or end of the results table, respectively:
