...
With [NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql), you can use funnels to evaluate sets of related actions. The actions generally share a relationship with an identifier, such as a user ID or session ID.
...
1. The anchor step, which always represents 100% of the results.
2. One or more steps that describe the number of users who have also completed additional actions, typically than the 100% from the anchor step. However, it could be 100% if every user who completes action (A) also completes the additional actions you're querying.

## Funnel query technical details

You can use funnel queries to calculate the completion rate for which multi-step, ordered sequences over a given timeframe, like in the New Relic signup [example](#why-matters). You can also use funnels to calculate overlap between several distinct actions that don't have a sequential relationship to one another. In other words, the order of steps won't impact the calculations performed.

The way funnel queries compute results varies depending on the size of the data set:

* If the funnel deals with fewer than 256 funnel attribute values, it will calculate the value exactly.
...
This means that for large data sets, there may be occasional "false positives" at the level of individual data points. These don't interfere greatly with the accuracy of the numerical estimates provided in query results.
  ...
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        <DoNotTranslate>**Technical details**</DoNotTranslate>
      </th>
