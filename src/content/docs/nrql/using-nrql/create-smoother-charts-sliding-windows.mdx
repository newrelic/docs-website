...
For example, if there is one event in a 10-minute timeseries window duration and the slide-by interval is 2 minutes, the event will be considered for analysis in 5 overlapping windows. The repetition of matched event is calculated by dividing the timeseries window duration by the slide-by interval.
