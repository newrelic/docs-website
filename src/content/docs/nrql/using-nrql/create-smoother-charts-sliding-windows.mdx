...
## Pricing [#pricing]
When you convert a query with the sliding window function to an alert, you may incur additional CCU charges if you are on the [Advanced and Core Compute pricing plans](https://newrelic.com/pricing/compute#pricing_plan-compute). While this method enhances data analysis by smoothing out fluctuations, its use may lead to increased costs over other methods. Events that occur within overlapping windows are counted multiple times, once for each window they appear in. This means that the same event can be matched several times due to the overlap.

For example, if there is one event in a 10-minute timeseries window duration and the slide-by interval is 2 minutes, the event can be considered for analysis in up to 5 overlapping windows. The repetition of matched event can be calculated by dividing the timeseries window duration by the slide-by interval. However, if an event occurred in the 3rd minute of the 10-minute timeseries window, it will only be considered in the first and second overlapping windows. From the third overlapping window onward, the event will not appear and thus will not be counted.
