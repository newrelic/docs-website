...
If you want to generate charts using the `SLIDE BY` clause in conjunction with the `TIMESERIES` clause, you can use a technique called sliding windows. This technique enables you to gather data in time "windows" that overlap with each other.
...
In contrast to sliding windows, "tumbling" or "cascading" windows don't overlap. For example, in this `TIMESERIES 3 minutes` NRQL query, the windows are 3 minutes in length, with each beginning when the other ends without overlap in the measurement interval.
...
When applicable, a PromQL-style query is translated into a NRQL sliding window query. For example, if your PromQL style query uses `rate(request_count[5m])` for the past 60 minutes with a 1-minute window overlap, the NRQL translation would be the query below.
