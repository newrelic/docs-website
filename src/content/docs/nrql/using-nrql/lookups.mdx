...
* The following NRQL clauses aren't supported with lookup queries:
  ...
  * `COMPARE WITH`
  * `EXTRAPOLATE`
  *  <DoNotTranslate>**Note:**</DoNotTranslate> You can use these clauses if you cointain the lookup query in an inner query. See [this query](#item-ids) for an example.
