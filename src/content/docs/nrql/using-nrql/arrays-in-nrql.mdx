...
As part of our support for OpenTelemetry Protocol (OTLP), attributes can contain arrays within New Relic. But what are arrays? Essentially, arrays are data structures containing an ordered collection of values that you can query with NRQL. Typically, arrays display in a format like `[1, 2, 3, 4, 5]` or `["US", "CA", "UK"]`.

Currently, we only support simple arrays. You can use simple arrays to store a list of values of the same primitive type; for example, numeric, boolean, or string. To quickly verify that an array contains any data, you can use the JSON chart type to see if the data displays in an array structure.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> Consider a simple array, `[100, 110, 90, 100, 105]`, stored in a durations attribute.

    `getfield(durations, 2)` will return `90`.

    Alternatively, you can use the square brackets `[ ]` shorthand instead of the function `getfield()`.

    <DoNotTranslate>**Example:**</DoNotTranslate> `durations[4]` will return `105`.
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> Consider a simple array, `countries`, with the value `["US", "CA", "UK"]`.
  ...
  >
    Use the `contains()` function to determine the presence of an element in an array.
