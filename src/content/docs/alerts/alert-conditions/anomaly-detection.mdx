...
We recommend follow these guidelines when you're setting condition thresholds:

* Start with a default threshold value and adjust based on the specific use case and data characteristics.
* Monitor and analyze anomaly detection results to refine the threshold settings over time.
* Consider using multiple thresholds for different severity levels to provide more nuanced alerting.


## Configure anomaly sensitivity thresholds [#configure-custom-anomalies]

You can create anomaly sensitivity thresholds from an [alert condition](/docs/alerts/alert-conditions/create-alert-conditions). Here are some tips for setting anomaly thresholds:
...
1. Go to <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts**</DNT> and click <DNT>**Alert Conditions**</DNT> on the left navigation pane.

2. Click <DNT>**+ New alert condition**</DNT>.

3. Choose an option: <DNT>**[Use guided mode](/docs/alerts/alert-conditions/create-alert-conditions/#create-guided-mode)**</DNT> or <DNT>**[Write your own query](/docs/alerts/alert-conditions/create-alert-conditions/#create-own-query)**</DNT>.

4. Go through the guided steps until you get to the <DNT>**Set thresholds**</DNT> section.

5. Select <DNT>**Anomaly**</DNT>.

    <img
      width="80%;"
      title="Set anomaly thresholds"
      alt="Screenshot of the anomaly thresholds options"
      src="/images/alerts_screenshot-crop_conditions-anomaly-thresholds.webp"
    />

6. Configure the settings for one or more thresholds. Anomaly detection predicts the next data point based on previous activity. The anomaly detection threshold controls the sensitivity of the alert condition to tolerate how far off the actual value is from the predicted value. The threshold is how many standard deviations your signal value is from the predicted value. We track the standard deviation between predicted and actual values for the last 7 days of data. To configure the threshold, you'll need to:

    * Set the <DNT>**threshold direction**</DNT> to either <DNT>**Upper and lower**</DNT>, <DNT>**Lower only**</DNT>, or <DNT>**Upper only**</DNT>. This means that we'll only create an incident if the signal value (the output of the query) is above the predicted value, below the predicted value, or either, respectively.
    ...
    * Set the threshold duration. Think of this as how long the signal value must remain outside the threshold before an incident is opened.   Conversely, it's also how long a signal must be within the threshold for an incident to be closed. This field answers to the time period mentioned above. It's how long the signal exceeds the threshold being defined. This is the actual threshold duration.
...
6. Add the details of the alert condition and click <DNT>**Next**</DNT>.
...
You can choose whether you want the condition to look for behavior that goes above the predicted value (<DNT>**Upper only**</DNT>) or that goes below the predicted value (<DNT>**Lower only**</DNT>), or that goes either above or below. You choose these with the prediction direction selector.

Example use cases for this:

* You might use the <DNT>**Upper only**</DNT> setting for a data source like error rate, because you generally are only concerned if it goes up, and aren't concerned if it goes down.
* You might use the <DNT>**Lower only**</DNT> setting for a data source like throughput, because sudden upward fluctuations are quite common, but a large sudden downswing would indicate a problem.
