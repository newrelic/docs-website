...
In alerts, an <DoNotTranslate>**entity**</DoNotTranslate> is defined as any target for monitoring, such as an application, a browser, or a host. The [alerts UI](/docs/alerts/new-relic-alerts/defining-conditions/select-product-targets-alert-condition) shows available entities that you can select. You can also use the REST API and API Explorer to [add](https://rpm.newrelic.com/api/explore/alerts_entity_conditions/add) or [remove](https://rpm.newrelic.com/api/explore/alerts_entity_conditions/remove) entities for a condition.
