...
<CollapserGroup>
  ...
  >
    This field allows you to control the impact of an alert condition on the [health status](/docs/alerts/create-alert/examples/view-entity-health-status-find-entities-without-alert-conditions) of the associated entity. Set it to `true` to ensure that the associated entity's health status remains unchanged, even when alert incidents are triggered. It corresponds to the <DNT>**Do not report system health status**</DNT> toggle when [setting thresholds for alert conditions](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#disable-health-status-reporting) in the UI.
  </Collapser>
