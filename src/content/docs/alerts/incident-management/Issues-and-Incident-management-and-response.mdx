...
An issue's priority is derived from the highest priority of its constituent incidents. For example, an issue with four high priority incidents and one critical incident would be assigned a **critical** priority.
</Callout>
