...
Currently an Alert condition only supports 2 priority levels: Warning and Critical .
An alert condition is where the priority is set for the incidents that it creates. A warning threshold on the condition will create a high priority on the incident and a critical threshold will create a critical priority on the incident.
An issue’s priority is set based on the highest priority of the incidents that it contains.  For example, if there are 4 high / warning incidents and 1 critical incident, the priority is set to critical.
</Callout>
