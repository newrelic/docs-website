...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        A failed location check will be considered failed until it performs a successful check, <strong>up to a maximum of 24 hours</strong>. The evaluation buffer for Multi-Location Synthetics conditions has a 24-hour TTL (time-to-live). For example: a location may fail and then immediately become available, but we'll consider it failed until a scheduled check is reported as successful. If no recovery signal is received within 24 hours, the failure is eventually dropped from the buffer.
      </td>
    </tr>
  </tbody>
</table>
