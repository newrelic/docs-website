...
* If <DNT>**yes**</DNT>, its health status indicator on the selected index (APM, browser, etc.) will be color-coded to the current state. To view a summary of current alert incidents, mouse over its health status indicator. However, if the [<DNT>**Do not report system health status**</DNT> toggle is enabled](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#disable-health-status-reporting) while setting thresholds for alert conditions, the indicator will not change color, even if an alert incident occurs.
