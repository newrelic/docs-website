...
* If <DNT>**yes**</DNT>, its health status indicator on the selected index (APM, browser, etc.) will be color-coded to the current state. To view a summary of current alert events, place the mousepointer over its health status indicator. However, if the [<DNT>**Do not report system health status**</DNT> toggle is enabled](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#disable-health-status-reporting) while setting thresholds for alert conditions, the indicator will not change color, even when an alert event occurs.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The entity is operational. We are collecting data that you can view in the appropriate UI. No alert events are currently reported for it.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * [Notifications](/docs/alerts/get-notified/destinations) have been sent based on the selected [alert event rollup preference](/docs/alerts/organize-alerts/specify-when-alerts-create-events/).
        * The alert event appears in the [<DNT>**Alert events**</DNT> index](/docs/alerts/alert-event-management/view-alert-event-details).
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The entity is evaluated for at least one condition, and the results show there are no alert events present.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        There's at least one open alert event at the time the entity is undergoing the evaluation.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        The last open alert event associated to the entity has been closed.
      </td>
    </tr>
  </tbody>
</table>
...
Here's an example of an app listed on the <InlinePopover type="apm"/> index that <DNT>**is**</DNT> associated with one or more conditions. Its color-coded health status is green, because we are collecting data for it, and currently there are no Warning (yellow) or Critical (red) alert events.
...
<Callout variant="tip">
  To view the index listing currently [open alert events](/docs/alerts/alert-event-management/view-alert-event-details) across all products, not just this entity, select <DNT>**View all alert events**</DNT>.
</Callout>
