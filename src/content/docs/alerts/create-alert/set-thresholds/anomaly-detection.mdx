...
* <DNT>**New Relic calculation**</DNT> (default): Automatically determines the seasonality for each signal based on several factors, including the age of data, consistency of data, and regular fluctuations.
  ...
  **Example:** A dynamic microservices architecture where you monitor dozens of distinct services in a single condition. User-facing services might experience daily traffic spikes, while background worker services might not have any predictable pattern. This setting allows the system to adapt to each specific signal without manual tuning.

* <DNT>**Hourly**</DNT>: Applies an hourly pattern to all signals within the condition for anomaly detection.

  **Example:** A server that runs a resource-intensive cron job at the top of every hour, such as an hourly data synchronization script or batch queue processor. This setting helps detect anomalies in CPU and memory usage patterns.

* <DNT>**Daily**</DNT>: Applies a daily pattern to all signals within the condition for anomaly detection.

  **Example:** An internal corporate application, like an HR portal or intranet, that consistently sees high login rates starting at 8:00 AM and drops off steeply after 5:00 PM every day.

* <DNT>**Weekly**</DNT>: Applies a weekly pattern to all signals within the condition for anomaly detection.

  **Example:** A B2B software platform that experiences peak traffic from Monday through Thursday, a noticeable drop on Fridays, and near-zero traffic over the weekend.

* <DNT>**None**</DNT>: Disables seasonality entirely, ensuring no seasonal patterns are considered when evaluating signals.

  **Example:** A critical system error rate monitor, such as HTTP 500s or database connection timeouts. These metrics should ideally remain at a flat baseline near zero at all times. A spike at 2:00 PM is an anomaly regardless of whether it happened yesterday at 2:00 PM as well.
