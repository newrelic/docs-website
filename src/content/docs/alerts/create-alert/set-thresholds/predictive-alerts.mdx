...
Using your historical data, this feature fits a predictive model to your signal. For each incoming data point, a set of predictions is generated up to 360 steps into the future, with each step equaling the signal's [window duration](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#window-duration). If any predicted data points breach your static threshold settings, you'll receive an alert. By getting alerts before a problem might occur, you can take corrective measures to prevent potential issues.
...
### Seasonal data
For data with seasonality, New Relic supports hourly, daily, and weekly seasonal lengths. Monthly and yearly seasonalities are not supported due to computational limitations like data retention and real-time evaluation. To model seasonal patterns, you need at least three times the season length of data: 3 hours for hourly, 3 days for daily, and 3 weeks for weekly.

The possible season lengths that can be detected are based upon your signal's [window duration](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#window-duration). In addition to there being a minimum window duration for each season length, there is a maximum allowed value. The window duration cannot be longer than or equal to the season length. If you want the model to use a particular season length, the window duration must fall within the acceptable ranges:
