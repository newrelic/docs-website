...
Although <DNT>**Predictive Alerts**</DNT> are useful for many situations, there are some where they are not appropriate, particularly signals that exhibit "fail fast" or spiky behavior. An example would be for something like error count where the signal is steady but experiences short lived spikes. We are unable to predict that type of behavior. In a situation like this, a [static threshold](/docs/alerts/create-alert/set-thresholds/set-thresholds-alert-condition) condition would be more appropriate and would provide the alert coverage needed.
...
Additionally, the amount of historical data used to train the predictive model is dependent upon your time series' window duration. The maximum amount of historical data used to train the model is 3660 times the set window duration.
...
If no repeating patterns are detected in the historical data, the algorithm will use the non-seasonal model. This model is based only on the trend and level of the data, allowing it to make predictions without relying on seasonal patterns.
