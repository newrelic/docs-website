...
An alert condition is the core element that defines when an [alert event](/docs/alerts/create-alert/condition-details/alert-event-attributes/#definition) is created. It acts as the essential starting point for building any meaningful alert. Alert conditions contain the parameters or thresholds met before you're informed.  They can mitigate excessive alerting or tell your team when new or unusual behavior appears.
...
An alert condition is a continuously running query that measures a given set of events against a defined threshold and opens an [alert event](/docs/alerts/organize-alerts/specify-when-alerts-create-events) when the threshold is met for a specified window of time.
...
For all methods except for our guided mode, the process for creating an alert condition will be _exactly_ the same as described in the steps below. As you configure your condition, see [alerts best practices](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices) for guidance on naming conditions, adding tags, setting effective thresholds, and reducing noise.
...
<Steps>
  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser
        ...
        title="Delay"
      >
        The delay feature in alert conditions safeguards against potential issues arising from inconsistent data collection. It acts as a buffer, allowing extra time for data to arrive and be processed before triggering an alert. This helps prevent false positives and ensures more accurate alert event creation.
  ...
  <Step>
    ...
    If an alert condition is a container, then thresholds are the rules each alert condition must follow. As data streams into your system, the alert condition searches for any alert events of these rules. If the alert condition sees data from your system that has met all the conditions you've set, it will create an alert event. An alert event signals that something is off in your system, and you should look.
    ...
    <CollapserGroup>
      ...
      >
        ...
        3. Define breach criteria:
           * Start with the default settings: open an alert event when a query returns a value that deviates from the predicted value for at least five minutes by three standard deviations.
      ...
      >
        Unlike anomaly thresholds, a static threshold doesn't look at your data set as a whole and determines what behavior is unusual based on your system's history. Instead, a static threshold will open an alert event whenever your system behaves differently than the criteria that you set.
      ...
      >
        The [lost signal threshold](/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/#signal-loss) determines how long to wait before considering a missing signal lost. If the signal doesn't return within that time, you can choose to open a new alert event or close any related ones. You can also choose to skip opening an alert event when a signal is expected to terminate. Set the threshold based on your system's expected behavior and data collection frequency. For example, if a website experiences a complete loss of traffic, or throughput, the corresponding telemetry data sent to New Relic will also cease. Monitoring for this loss of signal can serve as an early warning system for such outages.
      </Collapser>
    </CollapserGroup>
  </Step>
  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      >
        ...
        Learn more about creating policies [here](/docs/alerts/organize-alerts/specify-when-alerts-create-events).
      </Collapser>
      ...
      >
        ...
        1. One issue per policy (default):
           ...
           * Cons: Groups all alert events under one issue, even if triggered by different conditions. It's not ideal for multiple pageview concerns.
        ...
        3. An issue for every alert event:
      ...
      >
        An alert event automatically closes when the targeted signal returns to a non-breaching state for the period indicated in the condition's thresholds. This wait time is called the recovery period.

        For example, if you're measuring latency and the breaching behavior is that `duration` to load pages in your `WebPortal` application has increased to more than 3 seconds, the alert event will automatically close when `duration` is equal to or lower than 3 seconds for 5 consecutive minutes.
        ...
        2. The evaluation resets and restarts from when the previous alert event ended.
           ...
           All conditions have an alert event time limit setting that automatically force close a long-lasting alert event.

           New Relic automatically defaults to 3 days and recommends that you use our default settings for your first alert.

           Another way to close an open alert event when the signal does not return data is by configuring a `loss of signal` threshold. Refer to the lost signal threshold section above for more details.
      </Collapser>

      <Collapser
        id="custom-alert event-description"
        title="Set a custom alert event description"
      >
        Since you're creating an alert condition that lets you know if there are any latency issues with your `WebPortal` application, you want to make sure your developers have all the information they need when notified about this alert event. You will use workflows to notify a team Slack channel when an alert event is created.
...
<CollapserGroup>
  ...
  <Collapser
    id="instance-alert-events"
    title="Java instance conditions"
  >
    You can set thresholds that open an alert event when they are breached by any of your Java app's instance metrics.
  ...
  >
    For Java apps monitored by APM, you can set [thresholds](/docs/using-new-relic/welcome-new-relic/get-started/glossary#alert-threshold) that open an alert event when the heap size or number of threads for a single JVM is out of the expected operating range.

    We evaluate alerting threshold breaches individually for each of the app's [selected instances](instance-alert-events). When creating your condition, select <DNT>**JVM health metric**</DNT> as the type of condition for your Java app's alert policy, then select any of the available thresholds:
    ...
    * Garbage collection CPU time
      ...
      alert events will automatically close when the inverse of the threshold is met, but by using the UI you can also change the time when an [alert event](/docs/new-relic-solutions/get-started/glossary#alert-event) force closes for a JVM health metric. Default is 24 hours.
  </Collapser>
  ...
  >
    ...
    2. To define the thresholds that open the alert event, type the <DNT>**Percentile nth**</DNT> response time value, then select its frequency (above, below, or equal to this value).
  ...
  >
    ...
    Using dynamic targeting with your condition also requires that you set an [alert event close timer](/docs/alerts/alert-event-management/how-alert-condition-events-are-closed#time-limit).
...
To [add more conditions to a policy](/docs/alerts/create-alert/create-alert-condition/alert-conditions):
...
6. Select <DNT>**Copy condition**</DNT>.
   ...
   By default, the selected alert policy will add the copied condition in the **Disabled** state. Follow standard procedures to [add](/docs/alerts/create-alert/create-alert-condition/alert-conditions) or copy more conditions to the alert policy, and then [**Enable** the condition](/docs/alerts/new-relic-alerts-beta/updating-alert-policies/disable-or-delete-alert-policies-conditions#disable-conditions) as needed. Additionally, the new condition will not copy any [tags](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/#add-via-ui-api) added to the original condition.
...
<DocTiles>
  ...
  <DocTile
    title="Get notified"
    path="/docs/alerts/get-notified/alert-event-workflows/"
  >
