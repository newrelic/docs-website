...
Each of these APIs have two additional fields in their response:
...
* `actionOnMutingRuleWindowEnded`: Enum with valid values of `CLOSE_ISSUES_ON_INACTIVE` or `DO_NOTHING`.
* `mutingRuleLifecycleEventPublishedAt`: A `DateTime` field containing the last time the mute rule was applied.
