...
A muting rule uses a set of conditions that match against attributes in an [incident event](/docs/alerts/create-alert/condition-details/alert-event-attributes). The muting rules tell us how to:
    ...
    <tr>
      ...
      <td>
        ...
        * `conditions`: The set of individual expressions (sub-conditions) that target attributes within an incident. These are evaluated together based on the `operator`. You can have a maximum of 20 sub-conditions for a single muting rule.
          ...
          * `attribute`: A single attribute within an incident. Go here for a list of [Incident event attributes](/docs/alerts/create-alert/condition-details/alert-event-attributes).
