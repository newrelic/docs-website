...
What does [this migration](https://support.newrelic.com/s/hubtopic/aAX8W0000008dKO/plan-to-upgrade-alert-notification-channels-to-workflows-and-destinations) mean for your team? In our previous notification model, your team would create an alert condition with different thresholds and parameters. If that condition was associated with a specific policy, was breached, and you wanted your team to know about it right away, you would add <DNT>**notification settings**</DNT>. Our notification settings would tell New Relic what data to send where.
...
* All \_url attribute values will change and lead to issue based pages, not alert event based pages.
...
* `open_violations_count.critical` will contain the count of all open alert events across all priorities. Priority specific counts are unavailable.
* `open_violations_count.warning` will always be `0`. Priority specific counts are unavailable.
* `closed_violations_count.critical` will contain the count of all closed alert events across all priorities. Priority specific counts are unavailable.
...
The `incident_id` on PagerDuty, Webhook, VictorOps, OpsGenie and xMatters notifications refers to the classic alert event id. The classic alert event id is deprecated. Consumers should start using the `issue_id` instead.
...
* To limit the impact to VictorOps, OpsGenie and xMatters notifications, the `incident_id` will be populated by the issue id. That will cause the New Relic steps to acknowledge or close an alert event in xMatters to no longer work.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        The number of closed alert events across all priorities.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        There is no replacement for warning counts.  All closed alert event counts will be represented as critical to avoid double counting alert events.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        The custom alert event description, if one is defined.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        Open alert event counts of all alert event regardless of priority.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        Open alert event counts of all alert events regardless of priority.
      </td>
    </tr>
