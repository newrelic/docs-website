...
<CollapserGroup>
  ...
  >
    ...
    When you authenticate, OAuth handshake creates a token that authenticates with Slack and the user's personal token is saved. Saving a personal token separate from the OAuth token which creates a connection to a private slack channel. 
    
    Each additional authentication will save that user's personal token. Even if the original user is removed from the Slack workspace, the destination continues to send notifications.
