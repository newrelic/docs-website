...
4. Select the [issue preference](/docs/alerts/organize-alerts/specify-when-alerts-create-events).
...
You can continue from here to [define the conditions](/docs/alerts/create-alert/create-alert-condition/alert-conditions) and [setup notifications using workflows](/docs/alerts/get-notified/alert-event-workflows). If you decide to continue later, select the policy name from the <DNT>**Alert policies**</DNT> index.
...
<CollapserGroup>
  ...
  >
    ...
    2. From the policy row, click the ellipses (...) dropdown menu and select <DNT>**Delete**</DNT>.
       ...
       In a deleted policy, [issues](/docs/alerts/alert-event-management/view-alert-event-details) and [alert events](/docs/alerts/alert-event-management/view-alert-event-details) are provided as a read only resource.
  </Collapser>
  ...
  >
    To select how [alert events](/docs/alerts/organize-alerts/specify-when-alerts-create-events/) are grouped into issues (by policy, by condition, or by target and condition):
