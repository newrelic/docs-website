...
* [Some infrastructure data](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/query-infrastructure-dimensional-metrics-nrql)
  ...
  >
    ...
    For example: an <InlinePopup type="apm" /> metric timeslice for time spent in a particular transaction is named `WebTransaction/URI/foo`, and might have a response time of 0.793 for a one-minute time slice from 10:20am to 10:21am. These metrics usually follow a pattern like `<category>/<class>/<method>`.
