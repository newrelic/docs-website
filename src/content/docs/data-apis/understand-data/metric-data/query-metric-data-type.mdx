...
## Return results for invididual fields using `getField()` [#use-getField]

Different metric types have different fields associated with them. For example, the `gauge` metric type has different fields than the `summary` type. For more on that, see getField. 

If you would like to extract one of those values using a query, you can do that with `getField()`.
