...
This feature has an add-on charge per SAP System ID (SID). An SAP System ID (SID) is a three letter identifier representing a unique instance of an SAP system. Each production and pre-production SAP system is counted as separate SIDs. A stand-alone HANA database is also counted as an SID. A single SID can contain multiple resources such as multiple application server instances, and still count as one SID.
