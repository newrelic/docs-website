...
<CollapserGroup>
  ...
  >
    If you added custom attributes to the <InlinePopup type="apm" /> `Transaction` event via an APM agent, you can forward those custom attributes to the `PageView` event automatically:
