...
You can create custom session-level attributes for default mobile monitoring events using the mobile agent SDK. For example, to record a `username` attribute for some part of your iOS or Android app, you would use the [`setAttribute` API](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/create-attribute). These attributes are session-related information and are shared by multiple mobile event types.
