...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Attribute names can be a combination of alphanumeric characters, colons (`:`), periods (`.`), and underscores (`_`). We recommend starting the name with a letter: if it starts with something else, you'll need to put backticks around the name when querying. For more on when backticks are required, see [NRQL reference](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-how-nrql-works/#syntax).
