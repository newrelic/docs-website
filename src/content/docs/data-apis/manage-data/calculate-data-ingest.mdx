...
The amount of ingested data can vary from one New Relic organization to the next, based on what kinds of things are monitored, what features are used, the monitored applications' behaviors, and more. Use case understanding is vital to properly use the included factors where in the approximate range that ingest will fall. For example, our logs-in-context feature will add more data to the logs and the percentage amount will vary based on the size of the log lines (smaller log events = higher % increase). Given such variability, the best way to estimate your costs is to set up a test New Relic account and then extrapolate your usage from that. You can use the [cost estimator spreadsheet](#cost-estimator) for directional guidance and approximations based on simplified inputs that you provide with the built-in assumptions (that may not be true for your use case), but it is not a guarantee of your actual costs. Actual costs of your usage are shown in the [data management UI](/docs/data-apis/manage-data/manage-data-coming-new-relic).
