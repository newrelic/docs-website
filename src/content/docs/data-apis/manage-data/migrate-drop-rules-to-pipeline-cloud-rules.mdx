...
Although New Relic automatically migrated all drop rules to Pipeline cloud rules in the New Relic application, Terraform users still have a couple of migration steps. As Terraform maintains its own state file, it only knows about the `newrelic_nrql_drop_rule` resources you explicitly configured. It doesn't automatically understand that these are now linked to `newrelic_pipeline_cloud_rule` resources in the backend.
...
- New Relic Terraform Provider v3.68.0 or above: This version adds support for the `pipeline_cloud_rule_entity_id` attribute on `newrelic_nrql_drop_rule` resources and introduces the `newrelic_pipeline_cloud_rule` resource.
...
- **Terraform or OpenTofu v1.7 or above:** Required for the automated migration approaches.
- **New Relic Terraform Provider v3.72.1 or above:** This version adds support for the `pipeline_cloud_rule_entity_id` attribute and the `newrelic_pipeline_cloud_rule` resource.
- **New Relic CLI:** Required to run the automated migration commands (specifically for Phase 2).
        ...
        <Steps>
          ...
          <Step>
            ...
            <Callout title="Note">
              ...
              If you have state access in your CI/CD environment, you can use this bash command to automatically generate the list format (note: this is experimental and may need adjustment based on your specific state structure):
          ...
          <Step>
            ...
            **Prerequisite**: Your modules must export an attribute `all_rules` that holds references to all `newrelic_nrql_drop_rule` resources managed by the module.
            ...
            <Callout title="Note">
              If you have state access in your CI/CD environment, you can use this bash command to automatically generate the list format for modular rules (note: this is experimental and may need adjustment based on your state structure):
          ...
          <Step>
            ...
            This means you're not on New Relic Terraform Provider >= v3.72.1. Upgrade your provider version in your Terraform configuration to v3.72.1 or above, then run `terraform plan` again.
          </Step>
        ...
        <Steps>
          ...
          <Step>
            ...
            <Callout variant="Note">
              These commands remove the resources from Terraform state without deleting them from New Relic. The old drop rules remain in New Relic but are no longer managed by Terraform.
            </Callout>
          </Step>
      ...
      >
        ...
        <Steps>
          ...
          <Step>
            ...
            The command also displays recommendations for removing old drop rules from Terraform state, which you'll execute manually in Phase 3 using `terraform state rm` commands.
          </Step>
        </Steps>
      </Collapser>
