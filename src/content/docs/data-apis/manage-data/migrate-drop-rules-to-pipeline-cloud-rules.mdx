...
2. New Relic CLI automation tools: New Relic provides CLI tools that automate the native Terraform workflow, available in two versions:
   - GitOps (CI/CD): For teams using CI/CD tools, i.e., Terraform tooling in which the state file is not readily accessible within workspaces, and is often, hosted in an undisclosed remote location 
   - Local Terraform: For teams executing Terraform commands on their local machines, or any sort of Terraform tooling where the state file is readily accessible within workspaces, allowing resources in the state to be read
