...
Terraform users must manually migrate their `newrelic_nrql_drop_rule` resources to `newrelic_pipeline_cloud_rule` resources. Although New Relic has already migrated the underlying drop rules to Pipeline cloud rules in the backend, Terraform maintains its own state file and only knows about the resources you explicitly configured. Since Terraform treats these as two completely different resource types, you'll need to import the Pipeline cloud rules into Terraform's state and remove the old drop rule resources.
