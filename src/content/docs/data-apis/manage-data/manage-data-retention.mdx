...
Raw [dimensional metric](/docs/data-apis/understand-data/new-relic-data-types/#dimensional-metrics) data (the `Metric` data type) is aggregated (rolled up) at different time ranges and those aggregations are retained for different lengths of time. Raw sub-minute dimensional metric data is stored for 30 days. 1-minute resolution roll-up data is created and that's retained for 13 months. Only the raw data ingest counts towards usage: the aggregated data does not.
