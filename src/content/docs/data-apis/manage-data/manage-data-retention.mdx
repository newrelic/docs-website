...
This table shows the default retention settings for the [namespaces](/docs/glossary/glossary) that are available for editing in the UI: 
...
## Metrics and infrastructure data [#data-details]

Here are some details about how metrics and infrastructure data are stored and retained: 
...
Raw [dimensional metric](/docs/data-apis/understand-data/new-relic-data-types/#dimensional-metrics) data (the `Metric` data type) is aggregated (rolled up) at different time ranges and those aggregations are retained for different lengths of time. Details on retention periods:   
* **Raw metric data.** Raw data has a less-than-one-minute resolution. This raw data is retained for 30 days. 
* **Aggregated data.** Raw data is aggregated into data points representing a one-minute period. That aggregated data is retained for 13 months. 
