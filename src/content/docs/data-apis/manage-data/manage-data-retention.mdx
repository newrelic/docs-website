...
* **Upgrade your retention plan (Max per contract)**: Your **Max per contract** column specifies the maximum number you can configure for the **Effective retention** of a data type. This maximum allowance always applies to all accounts in your organization if you have more than one account. To increase your **Max per contract**, you need a user who is in a group that has the **Billing** administrative setting enabled for it. This administrative setting applies to all users in that group. Those users will have a **Billing User** role assigned to them, but this method is the only way to assign that specific role. Only these users will see an **Upgrade plan** button in the top right of the **Data retention** UI.
    <Callout variant="tip">
...
* **Change the current length of retention (Effective retention)**: This is the current retention time that you want a type of data to be stored and available to query. The highest number you can set this number to is in the corresponding **Max per contract** column. The users described below will be able to click the **Edit retention** button at the top right of the **Data retention** UI:
