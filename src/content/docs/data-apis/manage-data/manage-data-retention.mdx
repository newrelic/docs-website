...
With New Relic's **Data retention** UI page, you can review and manage how long different types of data are stored. Keep data for shorter periods to protect sensitive information and return queries faster, or keep it for longer periods to be able to see and analyze data and trends over a longer period. 
...
/>
<figcaption>
  From the **Data retention** UI page, you can click **Edit settings** to adjust the data retention for each source.
</figcaption>
...
Extending data retention beyond the default period [costs $0.05 per 30 days](/docs/licenses/license-information/usage-plans/new-relic-one-usage-plan-descriptions/#list-price). 
...
If you meet [the requirements](#requirements), you can increase the retention for the data types listed in the **Data retention** UI. To do that: on that page, click **Adjust retention**. 

Important things to note about increasing retention: 

* Retention is increased across the board, for all data types listed in the **Data retention** UI. It can't be increased for specific data types. 
* For each additional 30 days of retention, the cost is $0.05 per GB ingested per month. 
* Once a higher retention period is set, reducing retention for specific data sources has no impact on billing.

### Other notes on reducing retention [#reduce-retention]

When you reduce the retention period for a specific data source, existing data of that type won't immediately disappear. That data might be visible and returned in queries for up to several weeks. We have safeguards in place to ensure you don't accidentally delete data, but this also means that reducing retention values isn't a good strategy for quickly [removing data](#data-deletion). 
