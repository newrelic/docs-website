...
Limit metrics are only visible if you're sending data in to a corresponding `dataType` or `limitName` API. For example, if you send in data via the Metric API, you'll see the Metric API resource metrics, but if you don't send any <InlinePopup type="apm" /> data in, you won't see APM resource metrics.
