...
The warning appears as a **yellow warning icon** on the affected chart. Hover over the icon to read the full message. The exact wording depends on which cardinality limit was reached — account-wide, one metric, or multiple metrics.

**When your account's overall cardinality budget is exceeded:**

> Results for this query may be incomplete because account `<accountID>` exceeded its cardinality budget during the query's time window. Manage your budget in the **Cardinality Management UI**.
>
> **Please see the docs for more information.**

**When one specific metric exceeds its per-metric cardinality budget:**

> Results for this query may be incomplete because the following metric(s) in account `<accountID>` exceeded their cardinality budget during the query's time window: `<metric-name>`. Manage your budget in the **Cardinality Management UI**.
>
> **Please see the docs for more information.**

**When two or three metrics exceed their per-metric cardinality budgets in the same query:**

> Results for this query may be incomplete because the following metric(s) in account `<accountID>` exceeded their cardinality budget during the query's time window: `<metric-1>, <metric-2>, <metric-3>`. Manage your budget in the **Cardinality Management UI**.
>
> **Please see the docs for more information.**

**When four or more metrics exceed their per-metric cardinality budgets in the same query** — the warning summarizes rather than listing every metric by name:

> Results for this query may be incomplete because multiple metrics in account `<accountID>` exceeded their cardinality budget during the query's time window. Manage your budget in the **Cardinality Management UI**.
>
> **Please see the docs for more information.**
...
<Callout variant="tip">
  **Account-level warnings take precedence.** If your account has exceeded its overall cardinality budget, you'll see the account-level variant only — you will not additionally see metric-level warnings for metrics inside that account, even if some of those metrics have also hit their per-metric caps. Once you bring the account back under its overall budget, any remaining per-metric limits still in effect will surface on subsequent queries.
</Callout>

<Callout variant="tip">
  If your query spans multiple accounts and more than one is over budget, you'll see **a single warning that groups every affected account together** — one block per account, each block listing the metrics limited in that account (or naming just the account if the whole-account budget was exceeded). The account-level precedence rule above applies per account: any account block where the whole-account budget is exceeded will show just the account, without listing metrics.
</Callout>
...
* **Your query is not broken.** The same query will keep working. Only the pre-aggregated results are incomplete for the time window when the cardinality limit was active.
* **Nothing is wrong with New Relic.** The warning is New Relic telling you your account is doing exactly what it's supposed to do when a cardinality limit is enforced.

### Query raw data while you remediate [#query-raw-data]

While you work on bringing cardinality back under budget, you can retrieve exact values from the underlying raw data by adding the `RAW` keyword to your NRQL query. Aggregation limits apply only to pre-computed rollups — raw data continues to be ingested and stored regardless of cardinality limits.
