...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        50K limit on unique metric-name/attribute-value combinations for a single metric in a 30 second time window. Normal [cardinality limits on rollups](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes/#requirements) will apply.

        If the 50k in a 30 second window limit is exceeded, the rule is disabled and an [`NrIntegrationError` event](/docs/telemetry-data-platform/manage-data/nrintegrationerror) is created in that account that includes:
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <DNT>**Example:**</DNT> If you included availability zone in your metric name, it would mean, for that metric, you wouldn’t be able to see results across all availability zones.
      </td>
    </tr>
