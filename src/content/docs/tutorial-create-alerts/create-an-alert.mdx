---
title: "Create an alert using New Relic"
...
You can create an alert policy in New Relic using 1 of 3 options: creating an alert from a chart, creating an alert from a NRQL query, or creating an alert using golden metrics. While the beginning of the policy creation process is different depending on which option you choose, all the subsequent steps for creating your are the same. This means that once you've created an alert one way, the experience you gain from doing so will transfer over to alerts created using either of the other options.

For this tutorial, we've included steps for all 3 of the aforementioned options, so you can get started with your first alert any way you want to. Just select the path you want to follow in the first step, and by the end of the procedure you'll have successfully made your first alert using New Relic!
...
<Tabs>
	<TabsBar>
        ...
        <TabsBarItem id="2">
            Create from a NRQL query
        </TabsBarItem>
        <TabsBarItem id="3">
            Create from golden metrics
        </TabsBarItem>
    </TabsBar>
...
<SideBySide>
    <Side>
Starting an alert from a chart is one of the easiest ways to set up a New Relic alert condition. Instead of creating a NRQL query from scratch, charts let you use pre-existing NRQL conditions to begin the process. 
    
In this example, we'll set up an alert condition using an error response chart. Follow the steps below with any type of chart you like to begin creating an alert from a chart.
    </Side>
    <Side>
    <img
...
<SideBySide>
    <Side>
One of the easiest ways to get started is to use our golden metrics alerts option. When selected, this capability gives you a guided creation path for creating your alert. All you have to do is select the options you want and let us do the rest!

The following example will use specific options, but you can change whatever you need to make an alert that fits your specific needs.
    </Side>
    <Side>
<img
...
4. Choose any of the listed capabilities to determine what the policy will monitor. For this example, **Browser applications** is selected, but you can choose any other option that better suits your needs.

5. Select the entity you want the policy to cover. If you have many entities to choose from, you can use the filter field to shorten the list and find what you need.

6. Select the signal you want to monitor. This example uses **First input delay (75 percentile)**, but you can use any other option that better suits your needs.

7. Select **Next** once you've finished to save your progress and move on to the next step.
    </Side>
</SideBySide>
        </TabsPageItem>
    </TabsPages>
