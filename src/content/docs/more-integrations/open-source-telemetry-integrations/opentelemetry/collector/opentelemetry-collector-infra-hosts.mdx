---
title: Collector with New Relic (Infrastructure hosts)
tags:
  - Integrations
...
The configuration examples are meant to serve as a starting point from which you can extend, customize, and validate before using these in production.
...
When using the host receiver as part of the collector configuration, New Relic will automatically detect host metrics as part of a `Host` entity and will synthesize its golden metrics providing the same experience as with the New Relic infrastructure agent. The following are the configuration requirements to enable the `Host` entity experience in New Relic UI:
...
A variety of other settings are available for your configuration.
...
For troubleshooting tips, see [metric troubleshooting (GitHub)](https://github.com/open-telemetry/opentelemetry-collector/blob/main/docs/troubleshooting.md#metrics) and [monitoring collector metrics (GitHub)](https://github.com/open-telemetry/opentelemetry-collector/blob/main/docs/monitoring.md).
