...
receivers:
  dockerstats: #TODO: include additional metrics to be enabled by default
    ...
    # needed resource_attributes are enabled by default. TODO: should them be enabled them here explicitly?

    metrics:
      container.cpu.usage.total: # golden, silver, dashboard
        enabled: true
      container.cpu.throttling_data.periods: # dashboard
        enabled: true
      container.cpu.throttling_data.throttled_periods: # dashboard
        enabled: true

      # golden, silver, dashboard
      # TODO: enable `container.cpu.utilization` and disable `container.cpu.percent` when corresponding queries are updated.
      container.cpu.utilization:
        enabled: false
      container.cpu.percent:
        enabled: true

      container.memory.usage.limit: # dashboard
        enabled: true
      container.memory.usage.total: # golden, dashboard
        enabled: true
      container.memory.percent: # dashboard
        enabled: true
      container.blockio.io_service_bytes_recursive: # golden, silver, dashboard
        enabled: true
      container.network.io.usage.rx_bytes: # silver, dashboard
        enabled: true
      container.network.io.usage.tx_bytes: # golden, silver, dashboard
        enabled: true
      container.network.io.usage.rx_dropped: # dashboard
        enabled: true
      container.network.io.usage.tx_dropped: # dashboard
        enabled: true
      container.network.io.usage.rx_errors: # dashboard
        enabled: true
      container.network.io.usage.tx_errors: # dashboard
        enabled: true
      container.network.io.usage.rx_packets: # dashboard
        enabled: true
      container.network.io.usage.tx_packets: # dashboard
        enabled: true
      container.pids.count: # silver
        enabled: true
processors:
  resource:
    attributes:
    - key: nr.entity_type # Needed to synthesize container entities from metrics for now. TODO: update/remove when entities rules are defined.
      value: container
      action: upsert
