...
Before you can start, you'll need to [sign up for a New Relic account](https://newrelic.com/signup). You'll also need to install the OpenTelemetry collector following [Opentelemetry docs](https://opentelemetry.io/docs/collector/getting-started/). Once you've done that, you need to make the Prometheus metrics endpoint accessible from the OpenTelemetry Collector. The collector will use the Prometheus receiver component.

* Depending on the software you want to monitor the prerequites may change.
* Prometheus metrics endpoint could be enabled by defaut or may need some setup in the configuration to enable it.
...
<Steps>
    <Step>
    ## Install the OpenTelemetry Collector Contrib Distro 

In some cases but not all, install the [OpenTelemetry Contrib Distory](https://github.com/open-telemetry/opentelemetry-collector-releases/tree/main/distributions/otelcol-contrib). Complete this step if you use open source projects like the Core distribution, as it contains components that the collector will need. In other cases, the contrib distro 
