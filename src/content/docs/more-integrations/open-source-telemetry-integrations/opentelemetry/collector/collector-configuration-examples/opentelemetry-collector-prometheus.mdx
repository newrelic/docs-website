...
* Install the OpenTelemetry collector by following the steps in the [OpenTelemetry docs](https://opentelemetry.io/docs/collector/getting-started/). In some cases but not all, you may use an open source project that requires you to install the OpenTelemetry contrib distro. For example, if you use the Core distribution, the contrib distro contains additional components that the collector needs. 
* Regardless of your distribution, you must have the [OTLP exporter](https://pkg.go.dev/go.opentelemetry.io/collector/exporter/otlpexporter) and the [Prometheus receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/prometheusreceiver) components installed.
