...
<img
  title="Redis Open Telemetry quickstart dashboard"
  alt="Image of the dashboard available through the Redis Open Telemetry quickstart"
...
Notice that, since no identificator is attached by the otel receiver to redis metrics, an attribute processor is needed to decorate `server.address` and `server.port` attributes to metrics. 
Moreover, separate pipelines with different instances of `resource/redis_metrics` are needed in case multiple Redis servers are monitored from a single otel collector.

This is needed to be able to query and facet data differienciating between Redis servers and to create NewReclic entities. 
...
Telemetry data originating from the Open Telemetry Redis receiver, if properly decorated, generates redis entities.
