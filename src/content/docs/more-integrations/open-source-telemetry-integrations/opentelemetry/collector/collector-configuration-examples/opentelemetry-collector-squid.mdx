...
The label `squid_server: SQUID_SERVER_NAME` (where SQUID_SERVER_NAME should be a unique identifier) is required in order for the `SQUID_CACHEMGR` entity to be created and for the associated dashboards to run.
        ...
        - job_name: "squid"
          static_configs:
            - targets: [ "SQUID_EXPORTER_HOSTNAME:SQUID_EXPORTER_PORT" ]
              labels:
                squid_server: SQUID_SERVER_NAME
...
You may use the [metrics explorer](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer) to check the metrics being ingested. All metrics reported by the Squid Prometheus exporter are grouped in a `SQUID_CACHEMGR` entity with the name set in `SQUID_SERVER_NAME` and start with `squid_` prefix.


### Metric data [#squid-metrics]

The two groups of metrics reported by the squid cache manager are scraped:
- Squid counters
- Squid service times
