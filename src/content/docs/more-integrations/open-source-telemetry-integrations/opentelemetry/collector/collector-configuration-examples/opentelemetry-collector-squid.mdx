...
<Callout variant="tip">
  Squid Prometheus exporter v1.10.4 or later is required, the latest available version is recommended.
</Callout>
...
<CollapserGroup>
  ...
  If that's your case, you will need temporarily need to include an attribute to your squid metrics in order to report them as cumulative counters before the transition EOL, as suggested in the [transition guide](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/cumulative-metrics-transition-guide/#preview).
