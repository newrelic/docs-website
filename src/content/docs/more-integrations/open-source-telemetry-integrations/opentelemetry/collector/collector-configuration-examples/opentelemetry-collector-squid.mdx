...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Metrics names do not match"
  >

  Squids dashboards and entities use prometheus metrics naming conventions in order to match the metrics being reported by squid prometheus exporter. However, the OpenTelemetry prometheus receiver has prometheus names normalization enabled by default after [v076.3](https://github.com/open-telemetry/opentelemetry-collector-contrib/releases/tag/v0.76.3).

  To keep prometheus metrics names you may disable the corresponding feature-gate when running the collector. Example:
