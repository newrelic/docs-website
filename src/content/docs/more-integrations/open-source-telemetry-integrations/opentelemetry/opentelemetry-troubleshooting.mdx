...
![Screenshot showing the data point count for logs, metrics, and traces faceted by instrumentation.provider and newrelic.source](src/images/otlp-troubleshooting-facet-query.png 'Screenshot showing the data point count for logs, metrics, and traces faceted by instrumentation.provider and newrelic.source')
...
<Callout variant="important">
  The ingest limits for metrics, logs, and traces via OTLP are the same as our
  other [data ingest API
  limits](/docs/data-apis/manage-data/view-system-limits/#data-ingest-api-limits).
</Callout>
...
<Callout variant="tip">
  For more OpenTelemetry examples with New Relic, visit the
