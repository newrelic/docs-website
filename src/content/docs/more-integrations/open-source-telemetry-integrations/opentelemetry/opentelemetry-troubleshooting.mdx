...
You've correlated your logs with your service using `service.id`([OpenTelemetry logs: Best practices](content/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-logs)) so you can see logs associated with traces, but you don't see any logs in the New Relic UI. In this scenario, the log data has made it to New Relic, but isn't showing up in the the distributed trace UI with corresponding spans.
