...
* {gaDateAndYear}:
  * The ingest of cumulative monotonic sums as the `cumulativeCount` metric type is released (non-monotonic cumulative sums are <DoNotTranslate>**not**</DoNotTranslate> affected). The prior experience of ingesting these metrics as the `gauge` metric type is deprecated.
...
Cumulative sums that are non-monotonic will <DoNotTranslate>**not**</DoNotTranslate> be affected.
</Callout>
