...
Starting March 14, 2023, new accounts will have access to this new `cumulativeCount` type, but if your account was sending cumulative monotonic sums prior to March 14th, you won't have access to that metric type until June 13. On that date, the use of our `gauge` metric type for cumulative sums will be deprecated and reach end-of-life (EoL), and your cumulative sums will be stored as the `cumulativeCount` type. 
...
Let's take an example query. Up until the EoL, a query like: 

```
FROM Metric SELECT latest(<metricName>)
```

would be evaluating only `gauge` metrics. Once the EoL happens, it would be evaluating both `gauge` and `cumulativeCount` metrics. Depending on the alert, that could cause false results.
