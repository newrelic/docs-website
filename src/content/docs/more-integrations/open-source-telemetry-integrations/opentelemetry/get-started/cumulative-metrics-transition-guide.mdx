...
Starting March 14, 2023, we will ingest cumulative monotonic metrics according to the following end-of-life (EoL) timeline:

* March 14, 2023: The ingest of cumulative monotonic sums as the `cumulativeCount` metric type is released (non-monotonic cumulative sums are **not** affected). The prior experience of ingesting these metrics as the `gauge` metric type is deprecated.
...
* March 14 through June 13, 2023: Accounts using the deprecated `gauge` type ingest can opt-in to `cumulativeCount` type ingest. **Early opt-in is the best way to ensure affected queries and alerts are not disrupted by the EoL**.
* June 13, 2023: All accounts will be migrated to `cumulativeCount` type ingest for cumulative monotonic sums.

If you were ingested cumulative monotonic sums prior to March 14, this 90 day period gives you time to update queries and alerts that make use of the affected metrics.
...
If your New Relic account was sending OTLP cumulative sum metrics to New Relic before March 14, 2023, then you'll be impacted by the EoL on June 13, 2023. 
