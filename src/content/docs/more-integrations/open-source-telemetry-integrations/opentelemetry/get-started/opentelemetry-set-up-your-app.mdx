...
<CollapserGroup>
  ...
  >
    OTLP standards designate gRPC traffic to port `4317`, and HTTP traffic to port `4318`. The New Relic US FedRamp Native OTLP endpoint adheres to those specifications. It also allows gRPC traffic on port `443`.
  </Collapser>
  <Collapser
    className="freq-link"
    id="note-endpoints"
