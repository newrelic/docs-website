...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="note-preference"
    title="Which version of the protocol should I use?"
  >
    OTLP has [three different versions](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/protocol/exporter.md#specify-protocol) of the protocol: `grpc`, `http/protobuf`, and `http/json`. Support for the different versions varies by language.

    While New Relic supports all versions, if given the choice we recommend using `http/protobuf`. This is the default recommendation from the specification, and has proved to be more robust than `grpc` without any apparent reduction in performance.
  </Collapser>
  <Collapser
    className="freq-link"
    id="note-endpoints"
