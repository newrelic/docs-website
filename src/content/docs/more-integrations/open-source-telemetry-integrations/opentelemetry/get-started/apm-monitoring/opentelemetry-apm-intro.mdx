...
2. The OpenTelemetry Protocol (OTLP) exporter to send data to the
...
<Callout variant="important">
  You can use any language that OpenTelemetry supports to send data to New Relic.
  Though, the examples above reflect only a subset of the
  [supported languages](https://opentelemetry.io/docs/languages/).

  If there's a language you'd like to see an example for, please submit a request!
</Callout>
...
- Find your entity at **All entities -> Services - OpenTelemetry**. The entity
  name is set to the value of the app's `service.name` resource attribute.
  Refer to this
  [documentation](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services)
  for more information on how New Relic service entities are derived from
