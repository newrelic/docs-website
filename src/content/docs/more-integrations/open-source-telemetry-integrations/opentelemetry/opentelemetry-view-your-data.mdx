...
## View errors in your service in errors inbox  [#errorsinbox]
...
NOTE: Add image here with the following caption, image will be provided on Slack channel 
Caption: Scoped errors inbox through an OpenTelemetry entity.
...
Spans where the otel.status_code = ‘ERROR’ and the span kind is either ‘server’ or ‘consumer’ are treated as an individual error instance. Error groups are sets of unique error instances that share a unique fingerprint. 
