...
* [Use the OpenTelemetry Java agent:](#java-agent) Instead of using our pre-instrumentated demo app, you'll use the OpenTelemetry Java agent that will automatically monitor our demo app. No manual instrumentation is required.
* [Instrument the demo app manually:](#manual-instrum) In this track, you'll roll up your sleeves and tinker with the engine of the car. You'll manually insert instrumentation into our demo app to capture telemetry and you'll configure the SDK to send that data to New Relic. Manual instrumentation allows you go control the level of granularity of reported telemetry.
...
6. Click your new entity (service) called `getting-started-java` and explore the UI. For more tips about what to look for in the UI, see [View your data in New Relic](#view-data).
...
OpenTelemetry Java provides an agent that automatically instruments your app and doesn't require you to  modify your source code. The agent is a quick way for you to get started so you can get sample data into New Relic.

The auto-instrumentation agent is a JAR that dynamically injects bytecode to capture telemetry from popular libraries and frameworks. You can also use it to capture data such as inbound requests, outbound HTTP calls, and database calls. It can be attached to any Java 8+ application.
...
Complete these steps to monitor our demo app with the OpenTelemetry Java agent:

1. If you haven't already downloaded our demo app, run the following:

    ```
    git clone git@github.com:reese-lee/getting-started-guides.git
    ```

2. Download the [agent JAR file](https://github.com/open-telemetry/opentelemetry-java-instrumentation/releases/latest/download/opentelemetry-javaagent.jar), which contains the agent and instrumentation libraries. Place it in your preferred directory and note the path, which you will use later.

3. Navigate to the application's `java` directory and build the demo app with this command:
...
4. Export the following environment variables to configure the agent. Please note the details about each of them:
    ...
    <table>
      <thead>
        ...
        <tr>
          <td>
            `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.nr-data.net:4317` (US)

            `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.eu01.nr-data.net:4317` (EU)

          </td>
          <td>
            Export data to New Relic's OTLP endpoint using the appropriate URL.
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td>
            You have the option to specify the name of your application.
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td>
            Replace INSERT_YOUR_OWN_ID_HERE with a unique instance id.
          </td>
        </tr>
        <tr>
          <td>
        ...
        <tr>
          ...
          <td>
            (Recommended) Gzip compression has good performance and lowers data egress.
          </td>
        </tr>
        <tr>
          <td>
        ...
        <tr>
          ...
          <td>
            (Recommended) Disable the `process.command_line` resource attribute which often exceeds the New Relic maximum attribute length limit.
          </td>
        </tr>
        </tbody>
    </table>

5. Launch the agent with your app from the `java` directory:
    ```
    java -javaagent:path/to/opentelemetry-javaagent.jar \
          -jar ./build/libs/fibonacci-java.jar
    ```
6. Generate traffic from the application by opening a new terminal in the `java` directory and running the load generator:
    ```
    ./load-generator.sh
    ```
...
The previous section helped you explore automatic instrumentation with the OpenTelemetry Java agent. Sometimes, while using the Java agent, you may want to extend the details of what you're monitoring. You can do that through manual instrumentation with the OpenTelemetry Java SDK. Also, manual instrumentation may be the best option if you're using an unsupported framework.

<Callout variant="tip">
  When you configure the SDK, you'll have the option to do it with the autoconfigure extension or manually.
</Callout>
...
3. Add the following highlighted items to the `dependencies` block:
    ...
    dependencies {
        ...
        <var>implementation 'io.opentelemetry:opentelemetry-sdk-extension-autoconfigure'</var>
...
### Configure the SDK with the autoconfigure extension [#auto-config-ext]
...
3. Insert the highlighted lines:
    ...
    public class Application {
        ...
        public static void main(String[] args) {
            <var>// Use autoconfigure to configure SDK from environment variables and system properties</var>
            <var>OpenTelemetrySdk openTelemetrySdk = AutoConfiguredOpenTelemetrySdk.builder()</var>
                    <var>.setResultAsGlobal(false)</var>
                    <var>.build()</var>
    ...
    ```
4. Optimize your New Relic experience, we recommend that you export the following environment variables:

```
    export OTEL_EXPORTER_OTLP_HEADERS=api-key=INSERT_YOUR_NEW_RELIC_LICENCE_KEY_HERE
...
```
    ...
    * OTEL_EXPORTER_OTLP_HEADERS: This determines which New Relic account your data will be sent to. Make sure to use your [ingest license key](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#license-key).
    * OTEL_EXPORTER_OTLP_ENDPOINT: This is the endpoint your application telemetry will be exported to. In this case, you are setting it to one of our OTLP endpoints. If you are based in the EU, use https://otlp.eu01.nr-data.net:4317.
    * OTEL_SERVICE_NAME: This is the name that associates your application telemetry with an entity in New Relic.
    * OTEL_RESOURCE_ATTRIBUTES: Configure additional resource attribute key value pairs. The `service.instance.id` uniquely identifies a particular instance of a service and is required to light up certain features in New Relic.
    * OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT: Setting a limit on the maximum attribute value length will prevent span data from getting dropped if it exceeds [New Relic’s limit](https://docs.newrelic.com/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-attributes/).
    * OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE: New Relic supports metrics in delta temporality, instead of the default cumulative.
    * OTEL_EXPORTER_OTLP_METRICS_DEFAULT_HISTOGRAM_AGGREGATION: We recommend you use exponential histogram aggregation instead of the default explicit bucket histogram aggregation for better data compression.
    * OTEL_LOGS_EXPORTER: This enables log export over OTLP. Log export is disabled by default as the logs signal is still in experimental status.
    * OTEL_EXPORTER_OTLP_COMPRESSION: Setting `gzip` compression allows you to maximize the amount of data you can send per request. The New Relic maximum allowed payload size is 1MB (10^6 bytes). Gzip compression has good performance and lowers data egress.
    * OTEL_EXPERIMENTAL_EXPORTER_OTLP_RETRY_ENABLED: Enables experimental export retry to help cope with the unreliability of the internet.
    * OTEL_EXPERIMENTAL_RESOURCE_DISABLED_KEYS: Disables the `process.command_line` resource attribute which often exceeds the New Relic maximum attribute length limit and may contain sensitive information.


### Add base instrumentation: traces [#base-traces]
...
### Add base instrumentation: metrics [#base-metrics]
...
### Add base instrumentation: logs [#base-logs]
...
2. Insert the following highlighted lines.
    ...
    public class Application {
        ...
        public static void main(String[] args) {
            ...
            <var>// Set GlobalLoggerProvider, which is used by Log4j2 appender</var>
            <var>GlobalLoggerProvider.set(openTelemetrySdk.getSdkLoggerProvider());</var>
...
3. Create a directory called resources in `Instrumented/java/src/main`
4. Create a file called `log4j2.xml` with the following contents:
    ```
...
### Custom trace instrumentation: Create span attribute constants [#custom-attribs]
...
Insert the following highlighted lines into `Controller.java`:
    ...
    public class Controller {
        ...
        <var>private final Tracer tracer;</var>
        <var>private final LongCounter myCounter;</var>

        <var>@Autowired</var>
        <var>Controller(OpenTelemetry openTelemetry) {</var>
            <var>// Initialize tracer</var>
            <var>tracer = openTelemetry.getTracer(Controller.class.getName());</var>
...
You may want to record exceptions as they happen. We recommend you do this in conjunction with setting the span status. First, set your span as the current span, set the status code to error in the event of an exception, and then end the span:
    ...
    private long fibonacci(long n) {
        ...
        try <var>(var scope = span.makeCurrent())</var> {
            if (n < 1 || n > 90) {
                throw new IllegalArgumentException("n must be 1 <= n <= 90.");
            }
            // Base cases
...
### Custom metric instrumentation [#metrics-cust]
...
#### Add a custom metric counter [#cust-metric-count]
...
Initialize the logger (as previously stated, this is from the `java.util.logging` library. The Logger is not an OpenTelemetry component, but the application has been configured to send Log4j logs to the OpenTelemetry Log SDK):
    ...
    public class Controller {
      ...
      <var>// Logger (note that this is not an OTel component)</var>
...
Insert the following highlighted lines:
    ...
    private long fibonacci(long n) {
      ...
      try (var scope = span.makeCurrent()) {
        ...
        // Counter to capture valid inputs and the number of times each occurs
        myCounter.add(1, Attributes.of(ATTR_VALID_N, true));
        <var>// Log the result of a valid input</var>
        <var>LOGGER.info("An output of " + last + " was recorded");</var>
        return last;
      } catch (IllegalArgumentException e) {
        // Record the exception and set the span status
        span.recordException(e).setStatus(StatusCode.ERROR, e.getMessage());
        // Counter to capture invalid inputs and the number of times each occurs
        myCounter.add(1, Attributes.of(ATTR_VALID_N, false));
        <var>// Log when no output was recorded</var>
        <var>LOGGER.info("An exception occurred and no output was recorded");</var>
        throw e;
      } finally {
        // End the span
        span.end();
...
1. Go to the directory `getting-started-guide/Uninstrumented/java` and run this command:
    ```
    ./gradlew bootRun
    ```
...
2. Click your new entity (service) called `getting-started-j>ava`.
