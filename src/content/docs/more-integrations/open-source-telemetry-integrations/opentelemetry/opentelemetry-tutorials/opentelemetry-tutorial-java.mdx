...
<Callout variant="tip">
  When you configure the SDK, you see that we only discuss the autoconfigure extension, but you can also configure the SDK manually.
</Callout>
...
3. Insert the highlighted lines:
        ...
        ```
4. Optimize your New Relic experience, we recommend that you export the following environment variables (see the explanations that follow):
...
### D. Add instrumentation libraries: traces [#base-traces]
...
### E. Add instrumentation libraries: metrics [#base-metrics]
...
### F. Add instrumentation libraries: logs [#base-logs]
...
2. Generate traffic from the application by opening a new terminal in the `java` directory and running the load generator:
    ...
    ```
    <Callout variant="tip">
      Alternatively, you can reach the endpoint in the browser at this URL: `http://localhost:8080?n=[input]`. Replace `[input]` with whatever integer you want to generate successful computations, or insert invalid values, such a letters, to generate errors.
    </Callout>
