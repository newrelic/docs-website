...
You'll find three tracks below to help you get acquainted with New Relic and OpenTelemetry. Try out the tracks that are interesting to you:
...
* [Use the OpenTelemetry Java agent:](#java-agent) Instead of using our pre-instrumented demo app, you can use the OpenTelemetry Java agent to automatically monitor our demo app. You don't need to open the demo code and make any changes&mdash;just install and run the agent alongside the app.
...
This guide covers the first option. If you wish to export your data via a collector, check out this [documentation](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/collector/opentelemetry-collector-basic/) for details.
</Callout>
...
1. Run the following to clone the demo app and navigate to the `Instrumented` directory.
    ```
