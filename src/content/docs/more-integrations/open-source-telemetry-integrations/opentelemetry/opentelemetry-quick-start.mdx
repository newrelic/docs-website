...
You have two choices for exporting data to New Relic via OTLP:
  ...
  * Directly from your app:
  ...
  ![Diagram showing a direct export to New Relic to your app](./images/native-otlp-no-collector.png "Diagram showing a direct export to New Relic to your app")
  <figcaption>
  ...
  * Export from an OpenTelemetry collector:
  ...
  <figcaption>
  You can set up the collector as a gateway that receives, processes, and exports to the New Relic OTLP receiver.
</figcaption>
...
2. Based on your integration, be ready configure the [endpoint](#note-endpoints) where the exporter sends data to New Relic. The endpoints are region-specific, so use the one according to where your account is based. For example, use the EU endpoint if your account is based in Europe. Use the table below as a guide and see the additional considerations about ports and endpoints:
...
Now that you are familiar with the New Relic-specific configurations for exporting, follow the links in the sections below for specific instructions about exporting.

### Direct export from your apps [#direct-export]
...
### Export from a collector [#collector-export]
...
Choose one of the following to see examples for setting up your collector to export. After you finish, return here to [step 5](#view-data) to see your data.
