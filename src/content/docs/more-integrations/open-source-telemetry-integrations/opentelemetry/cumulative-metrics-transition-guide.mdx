...
export const eolDate = 'October 22';
...
* {gaDate} through {eolDate}:
  * Accounts using the deprecated `gauge` type ingest can opt-in to `cumulativeCount` type ingest. <DoNotTranslate>**Early opt-in is the best way to ensure affected queries and alerts are not disrupted by the EoL**</DoNotTranslate>.
* {eolDateAndYear}: All accounts will be migrated to `cumulativeCount` type ingest for cumulative monotonic sums.

If you were ingesting cumulative monotonic sums prior to {gaDate}, this 180 day period gives you time to update queries and <InlinePopover type="alerts" /> that make use of the affected metrics.
...
If you're using an OTLP instrumentation library within your code base, you should reference the [appropriate language SDK](https://opentelemetry.io/docs/languages/o) for your application. Any method of adding the attribute `newrelic_metric_type=cumulativeCount` to individual metric data points or to the resource attribute level should be sufficient.
