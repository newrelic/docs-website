...
<CollapserGroup>
  ...
  >
    ...
    The New Relic US FedRamp Native OTLP endpoint adheres to those specifications, as well as allowing gRPC traffic on port `443`. For FedRAMP accounts, the OTLP Metrics integration **only** supports the HTTP endpoint. See [FedRAMP compliance](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints/#otlp-api) for more details.

    However, non-FedRamp New Relic endpoints accept both gRPC and HTTP traffic on any of the ports listed in the chart above.
  </Collapser>
  <Collapser
    className="freq-link"
    id="note-endpoints"
