...
<CollapserGroup>
  ...
  >
    The New Relic US FedRamp Native OTLP endpoint currently **only** supports HTTP traffic. This is due to technical limitations in the OTLP Metrics integration and we hope to expand support to gRPC in the future. See [FedRAMP compliance](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints/#otlp-api) for more details.
