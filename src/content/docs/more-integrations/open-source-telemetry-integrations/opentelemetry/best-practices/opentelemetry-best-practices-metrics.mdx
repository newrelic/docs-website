...
* Monotonic cumulative sums are mapped to our [cumulativeCount](/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type. This is true with the exception of accounts existing before March 14, 2023, which use `gauge`: see [our transition guide](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/cumulative-metrics-transition-guide).
