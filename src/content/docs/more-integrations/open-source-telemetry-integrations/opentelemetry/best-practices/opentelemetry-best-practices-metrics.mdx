...
* Monotonic cumulative sums are mapped to our [cumulativeCount](/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type.

#### Sum configuration examples [#config-sums]

To understand how to configure aggregation temporality, see these examples using the [Java](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-java) and [Python](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-python) OpenTelemetry SDKs.
...
New Relic customers that reported cumulative sums before {gaDateAndYear}, have their cumulative sums converted to our `gauge` type.
