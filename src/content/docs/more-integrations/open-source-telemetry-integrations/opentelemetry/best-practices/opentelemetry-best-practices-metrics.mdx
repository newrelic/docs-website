...
* Monotonic delta sums are mapped to the [count](https://docs.newrelic.com/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type.
...
Cumulative sums are handled differently depending on whether they are monotonic or non-monotonic:

* Monotonic cumulative sums are mapped to the [cumulativeCount](https://docs.newrelic.com/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type.
* Non-monotonic cumulative sums are mapped to the [gauge](https://docs.newrelic.com/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type.
...
OpenTelemetry gauge metric data points represent a sampled value at a given time. These values are converted to the New Relic [gauge](https://docs.newrelic.com/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) metric type. OpenTelemetry gauges do not have an aggregation temporality, but the sampled values can be aggregated at query time.
...
Cumulative histograms will be converted to a [Delta histogram](#delta-histogram). For more information one our translation process, see [Cumulative metric reporting details (OTel and Prometheus)]([url](https://docs.newrelic.com/docs/data-apis/understand-data/metric-data/cumulative-metrics/)).
...
<CollapserGroup>
  ...
  >
    ...
    ```sql
    SELECT latest(totalApiBytesSent[cumulative]) FROM Metric TIMESERIES FACET description, statusCode
    ```
    (TODO: update image)
    <img
      ...
      src={opentelemetryQueryCumulativeSums}
    />
  </Collapser>
  ...
  >
    ...
    ```sql
    SELECT rate(sum(totalApiBytesSent), 1 second) FROM Metric TIMESERIES 5 MINUTES SLIDE BY 1 MINUTE FACET description, statusCode
    ```
