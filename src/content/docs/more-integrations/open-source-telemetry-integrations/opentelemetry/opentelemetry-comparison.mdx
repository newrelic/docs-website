...
import moreintegrationsOtelMatrixFilterBars from 'images/more-integrations_screenshot-crop_otel-matrix-filter-bars.png'
...
<CollapserGroup>
  ...
  >
    ...
    The New Relic APM agent can automatically inject a JavaScript agent into rendered content, so that the front-end JS assets are instrumented.
...
<CollapserGroup>
  ...
  >
    ...
    Host entities with golden metrics are available independent of the data source: New Relic <InlinePopover type="infrastructure" /> agent vs. OpenTelemetry collector host receiver.
  </Collapser>
  <Collapser
    className="freq-link"
    id="inventory"
