...
<CollapserGroup>
  ...
  >
    ...
    You can report custom events from OpenTelemetry by using the New Relic [Event API](/docs/data-apis/ingest-apis/introduction-event-api/). Alternatively, you can report on custom events using the OpenTelemetry events API via OTLP. These events are currently stored and queried as New Relic logs.
  </Collapser>
  <Collapser
    className="freq-link"
    id="workloads"
