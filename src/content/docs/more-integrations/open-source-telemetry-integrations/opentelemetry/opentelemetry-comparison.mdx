...
* 👀 Limited support in OpenTelemetry
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        👀 OpenTelemetry specification is experimental. See [Custom events](#cust-evnts) below.
      </td>
    </tr>
    <tr>
      <td>
...
New Relic agents offer automatic log collection by default for services. OpenTelemetry agents can be configured to send logs from a variety of sources.

## Service map [#service-maps]

New Relic agents: ✅ &nbsp; &nbsp; &nbsp; OpenTelemetry in New Relic: ❌

The Service Map displays an entire set of interconnected entities for a given service and is currently only supported by New Relic agents. The interconnected services must be actively reporting to appear in this map. 
...
<CollapserGroup>
  ...
  >
    ...
    Not all OTel language SDKs/agents specify or report `process.runtime.*` metrics yet.
  </Collapser>
  <Collapser
    className="freq-link"
    id="flight-recorder"
...
<CollapserGroup>
  ...
  >
    ...
    Not all of the telemetry that the New Relic Mobile SDK collects can yet be collected by OTel instrumentation, however.
  </Collapser>
  <Collapser
    className="freq-link"
    id="interactions"
...
This view shows, for APM and/or OpenTelemetry services or hosts:
...
<CollapserGroup>
  ...
  >
    ...
    The **Kubernetes** page does work for either New Relic APM agent or OTel-instrumented services.
...
<CollapserGroup>
  ...
  >
    ...
    You can report custom events from OpenTelemetry by using the New Relic [Event API](/docs/data-apis/ingest-apis/introduction-event-api/). Alternatively, you can report on custom events using the experimental OpenTelemetry events API via OTLP. These events are currently stored and queried as New Relic logs. For more about logs, see [New Relic data types](/docs/data-apis/understand-data/new-relic-data-types/#log-data).
  </Collapser>
  <Collapser
    className="freq-link"
    id="workloads"
