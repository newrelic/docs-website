...
<Callout variant="caution">
  New Relic does not currently support cumulative histograms. You must convert cumlative histograms to delta temporality.
  For configuring your SDK to use delta temporality, see the specification for the [OTLP metric exporter](https://github.com/open-telemetry/opentelemetry-specification/blob/ce50e4634efcba8da445cc23523243cb893905cb/specification/metrics/sdk_exporters/otlp.md#opentelemetry-metrics-exporter---otlp).
  
  You can query your account to determine if metrics are being dropped due to unsupported temporality.
