...
tags:
  ...
  - Airflow
  - Quickstart
metaDescription: Monitor Airflow data via OpenTelemetry to New Relic.
redirects:
...
Airflow sends metrics using OTLP over HTTP. To send this data to the OpenTelemetry collector, you'll need to use port `4318`. Airflow has multiple methods of [setting configuration options](https://airflow.apache.org/docs/apache-airflow/stable/howto/set-config.html).
