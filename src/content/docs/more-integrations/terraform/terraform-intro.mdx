...
[Log in to New Relic](https://login.newrelic.com/login) and [navigate to **Alert Policies**](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy#alert-policy-view) to confirm that Terraform created your new policy.
...
As the "newrelic_alert_condition" is deprecated and will be removed at some point in the future, you will want to use NRQL alerts in the future.  You can use the following configuration to create a NRQL alert performing the same function as above:
