...
We have two heroes at any given time: the GitHub/Slack hero (for issues and pull requests and for questions through Slack). Our second hero we call the sidekick, who handles customer feedback coming in. Every hero's job is to keep things moving for Relics and users. We change hero and a sidekick once a week (we used to do daily shifts, but we've found the daily cadence to be too disruptive and lead to work that bleeds over to the next day).
...
Most customer feedback that comes in isn't actionable, either because it verges on nonsense or because the feedback is too non-specific to act on.
...
2. Add a category label based on the URL, such as `doc_apm` or `doc_errors-inbox`.
3. Set the Workflow status to **Closed** 
4. Set the Resolution to **Done**.
5. Add a link to the PR.
6. Work the PR to its merge at your leisure.

### Backlog customer feedback

Our goal is to never have more than 13 customer feedback tickets in our backlog. If you're adding number 14, please check the other tickets to see if there's an old one that you can close or add to a sprint. 

Whether or not to add a piece of customer feedback to our backlog is the trickiest question to answer. Sometimes a comment is quite valid, but the scope of the work required to address it isn't something our team can commit to right now. Sometimes a comment requires technological knowledge that we don't have (sometimes no one at New Relic has!). Sometimes a doc simply doesn't get enough traffic to warrant fixing a problem right away.

If you're not sure whether a ticket belongs in our backlog, ask your manager.

Follow this process to put a ticket in the backlog:
