...
Both roles are here to keep things moving for Relics and users. and to buffer the rest of the team from interrupts. We change hero and a sidekick once a week (we used to do daily shifts, but we've found the daily cadence to be too disruptive and lead to work that bleeds over to the next day).
