...
- <DNT>**Find and fix exploitable vulnerabilities**</DNT>: No need to wait for scan results.
- <DNT>**Ship code faster**</DNT>: Unmatched detection accuracy and substantially fewer false positives.
- <DNT>**See and secure every application**</DNT>: Full visibility into your code, web components, and configuration data before using them in the production environment.
- <DNT>**Reduce time and cost**</DNT>: When you eliminate vulnerabilities earlier in the software development lifecycle.
...
IAST is a part of the New Relic APM agent, and only requires a security configuration to be enabled. It can be fully integrated with [New Relic Vulnerability Management](/docs/vulnerability-management/overview/), allowing you to continuously and quickly find, fix, and verify high-risk vulnerabilities across the software development lifecycle.
...
<CollapserGroup>
    ...
    <Collapser 
        id="faq-jira-support"
        title= "Is there a JIRA Integration available?"
        >
        No, but we are planning to add this in the near future.
     ...
     <Collapser
        id="faq-iast-in-production"
        title="Why should IAST not be used in production?"
        >
        IAST sends new requests to the application based on observed requests. This can result in undesirable manipulation of customer’s data, for example, multiple charges to a credit card. New Relic currently recommends using IAST in a non-production environment where test cases are based on synthetic data.
     </Collapser>
</CollapserGroup>
