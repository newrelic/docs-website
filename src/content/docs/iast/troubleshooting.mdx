...
<CollapserGroup>
    ...
    >
    IAST sends new requests to the application based on observed requests. This can result in undesirable manipulation of customer's data, for example several charges to a credit card. New Relic currently recommends using IAST in a non-production environment. We base test cases on synthetic data in these environments. Our plan is to offer a version of IAST for using it in production.
