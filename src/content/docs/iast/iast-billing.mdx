...
IAST analyzes applications for exploitable vulnerabilities. It observes the behavior of APIs, method calls, and traces exercised during its testing of real-world attack simulations. Each simulation generates a response based on the number of parameters tested. The more APIs and method calls exercised, the higher the number of parameters that will need testing for an accurate risk assessment. The more complex an application is and the higher the efficacy of your test coverage, the more compute capacity units will be consumed for each test run.
