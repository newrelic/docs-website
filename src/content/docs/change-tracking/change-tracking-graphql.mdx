...
The change tracking feature allows you to track the effect of various changes on your customers and systems. For example, if you make some deployments, you can use the change tracking feature to see the results on charts in New Relic and get access features that help you understand how those changes affect the performance and quality of your system.

Get started with the change tracking feature by telling us which changes you want to monitor in your environment. While you can do this using the [CLI](/docs/change-tracking/change-tracking-cli) or [Jenkins](/docs/change-tracking/ci-cd/change-tracking-jenkins), the steps here will help you send GraphQL mutations and queries to designate and list changes to track.

You can send mutations and queries programmatically to our [GraphQL API](https://api.newrelic.com/graphql), or you can use [NerdGraph](https://api.newrelic.com/graphiql), our GraphiQL integrated envionment (IDE).
...
* Relevant user permissions: Any user or role that can use NerdGraph can create deployment markers and track other changes, but only full users can access change tracking UIs and tools.
* Keep in mind that the GraphQl endpoint will vary depending on your region:
    * Main GraphQL endpoint: `https://api.newrelic.com/graphql`
    * EU GraphiQL endpoint: `https://api.eu.newrelic.com/graphql`
    * NerdGraph (main GraphiQL IDE): `https://api.newrelic.com/graphiql`
    * NerdGraph (EU GraphiQL IDE): `https://api.eu.newrelic.com/graphiql`
...
<CollapserGroup>
  ...
  >
    To create a new change tracking deployment event, you must set the following:
    <table>
      <thead>
        <tr>
        ...
        <tbody>
          <tr>
            ...
            <td>
              The `entityGuid` value is the unique identifier assigned by New Relic to your system components during instrumentation and setup processes. For more information on New Relic Entities and finding Entity GUIDs, see this [guide](https://docs.newrelic.com/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#find).
            </td>
          </tr>
          <tr>
            <td>
  ...
  >
    ...
    You can define some optional and useful attributes to add context to the change tracking deployment event. These attributes can also enable certain change tracking features.
    ...
    <table>
      <thead>
        ...
        <tr>
          ...
          <td>
            The time at which the recorded event occurred or the NerdGraph call was received. For example: `timestamp: 2019-10-08T00:15:36Z`.
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td>
            A unique identifier generated when the deployment event is recorded. While you cannot set `deploymentId`, you can use it in your NerdGraph and NRQL queries to locate specific deployments. Here's an example: `deploymentId: “8a3a594c-e726-4bc2-8078-26dffec9a3d8”`.
          </td>
        </tr>
      </tbody>
    </table>
...
To record a deployment event, you'll need to include the required fields, and then decide if you want to include any optional fields.
...
## Query deployment event data [#query-events]

You can use GraphQL and NRQL queries to fetch data about deployments. See the examples below to try out some queries. You can also learn about GraphQL queries in the [NerdGraph entities API tutorial](https://docs.newrelic.com/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/).

<CollapserGroup>
  <Collapser
    className="freq-link"
    id="query-deployments"
    title="Use GraphQL to query deployments"
  >
    The examples below show how to get information about deployments and their attributes. Be sure to insert your own values for `guid`, along with Unix epoch timestamps for `startTime` and `endTime` in milliseconds:

    <CollapserGroup>
      <Collapser
        className="freq-link"
        id="query-one"
        title="Query example one"
      >
```graphql
{
  actor {
...
```
  ...
  <Collapser
    className="freq-link"
    id="key-signals"
    title="Use NRQL to render a chart with a key signal and deployment markers"
  >
    After you create a marker using GraphQL, you can use the [query builder](https://docs.newrelic.com/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/) to create time series charts, draw curves for a telemetry signal over a span of time, and render deployments as markers.

    Here are some NRQL examples you could run in query builder to create some charts:

    <CollapserGroup>
      <Collapser
        className="freq-link"
        id="nrql-one"
        title="NRQL example one"
      >
```
SELECT COUNT(*)
