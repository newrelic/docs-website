...
You can designate which changes you want to monitor in your system by using our CLI instead of GraphQL or a CI/CD integration. Although you can designate which changes you want to monitor via the CLI, you'll need to run either a [GraphQL query](#create-list) or execute a [NRQL query](/docs/change-tracking/change-tracking-graphql#key-signals) against the New Relic database to retrieve a list of those changes. We have some [examples](#key-signals) to get you started.
...
Once you've designated some changes you want to track, you can list them by running a GraphQL query in NerdGraph or you execute a query using NRQL in the New Relic UI.
