  ...
  >
      ...
      <tbody>
        ...
        <tr>
          ...
          <td> No </td>
          <td>
            Use a custom `category`, `type`, or a non-standard combination, set the `dataHandlingRules` argument and the `validationFlags` field to `ALLOW_CUSTOM_CATEGORY_OR_TYPE`.

            If you don't set `validationFlags` to `ALLOW_CUSTOM_CATEGORY_OR_TYPE`, New Relic will return an error for any custom or non-standard value. For example, pairing a standard `category` like `Deployment` with a custom `type` such as `Rainbow` requires adding both `ALLOW_CUSTOM_CATEGORY_OR_TYPE` to `validationFlags`.
