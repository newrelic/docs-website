...
redirects:
  - /docs/change-tracking/change-tracking-events
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            The `dataHandlingRules` field specified with a subfield of `validationFlags` allows you to specify how you would like the API to handle validation failures. The `validationFlags` field accepts an array of any of the following values:

            * `ALLOW_CUSTOM_CATEGORY_OR_TYPE`
              * When you define a custom category, type, or a non-standard combination of category and type, you must include the `ALLOW_CUSTOM_CATEGORY_OR_TYPE` flag.
              * If the `ALLOW_CUSTOM_CATEGORY_OR_TYPE` flag is not provided when using such custom or non-standard values, New Relic will return an error if a category or type value does not exactly match a predefined standard value. For example, even if you use a standard category such as `Deployment`, pairing it with a custom type such as `Rainbow` which is not a New Relic standard combination requires you to set `validationFlags` to `ALLOW_CUSTOM_CATEGORY_OR_TYPE`.
            * `FAIL_ON_FIELD_LENGTH`
              * Will validate all string fields to be within max size limit (4096 characters). An error is returned and data is not saved if any of the fields exceeds max size limit.
              * If the `FAIL_ON_FIELD_LENGTH` flag is not provided and any of the fields exceed the max size limit, the data will be trimmed to length with an ellipses.
            * `FAIL_ON_REST_API_FAILURES`
              * For APM entities, a call is made to the legacy New Relic v2 REST API. An error is returned and data is not saved if the call to the legacy New Relic v2 REST API fails.
              * If the `FAIL_ON_REST_API_FAILURES` is not provided and the call fails for any reason, an error will be returned containing the failure message.
          </td>
        </tr>
      </tbody>
    </table>
