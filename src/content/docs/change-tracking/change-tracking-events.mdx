  ...
  >
      ...
      <tbody>
        <tr>
          ...
          <td>String</td>  
          <td>
            A `category` attribute allows you to group changes using various `categories` and `types`. You can either use our predefined [standard categories and types](#supported-types) or define your own custom ones. 
            
            * To use Custom categories: If you create a custom category, you must:
...
```
      ...
      <tr>
        <td>To filter change event data by `timeWindow`, `accountIds`, and `entityGuids`</td>
        <td>NerdGraph</td>
        <td>
```graphql
    {
    actor {
        ...
        changeTrackingSearch(
            ...
            entityGuids: INSERT_ENTITYGUIDS
            }
        ) {
            changeEvents {
            shortDescription
