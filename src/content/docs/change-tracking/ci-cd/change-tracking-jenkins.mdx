...
1. Download the plugin from [here](newrelic-deployment-notifier.hpi), and if necessary, rename it to `newrelic-deployment-notifier.hpi`.
    <Callout variant="tip">
      Note that this is a temporary location for the Jenkins plugin. We'll let you know when you can download this directly in Jenkins.
    </Callout>
