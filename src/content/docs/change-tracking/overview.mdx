...
* **Flexible integration**: Configure via APIs, CLI, or CI/CD pipeline integrations
...
* **User permissions**: Any [user type](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) can create change events and deployment markers via NerdGraph, but only full platform users can access the curated change tracking UI experiences
...
<Tabs>
  ...
  <TabsPageItem id="event">
    ...
    You can configure change tracking events using one of the following methods:
      - **[NerdGraph API](/docs/change-tracking/nerdgraph)**: Create change events via GraphQL mutations
      - **[New Relic CLI](/docs/change-tracking/change-tracking-cli)**: Use CLI for quick setup
      - **[GitHub Actions](/docs/change-tracking/ci-cd/change-tracking-github-actions)**: Automate change events in CI/CD pipelines
    ...
    You can configure change tracking deployments using one of the following methods:
      - **[NerdGraph API](/docs/change-tracking/change-tracking-graphql)**: Record deployments via GraphQL mutations
      - **[New Relic CLI](/docs/change-tracking/change-tracking-cli)**: Command line deployment tracking
      - **[Jenkins](/docs/change-tracking/ci-cd/change-tracking-jenkins)**: Automated tracking with Jenkins plugin
      - **[GitHub Actions](/docs/change-tracking/ci-cd/change-tracking-github-actions)**: Configure deployment tracking in workflows
...
* Access the unified interface to view all change events across entities and accounts, with filtering by team, type, custom attributes, and time correlation with metrics. For more information, refer to [view and analyze your data](/docs/change-tracking/change-tracking-view-analyze).

* Use [NRQL](/docs/change-tracking/change-tracking-events/#query-change-events-nrql) and [NerdGraph](/docs/change-tracking/change-tracking-view-analyze/#query-deployments) to build custom analysis and dashboards.
...
After setting up change tracking, explore these capabilities:

* **[View and analyze changes](/docs/change-tracking/change-tracking-view-analyze/)**: Learn advanced techniques for correlating change data with performance metrics and system behavior
* **[Set up webhooks](/docs/change-tracking/change-tracking-webhooks)**: Configure notifications to keep your team informed about tracked changes
* **[Query change data](/docs/change-tracking/change-tracking-events/#query-change-events-nrql)**: Build custom analysis using NRQL queries and create personalized dashboards

<DocTiles>
  <DocTile title="Set up using NerdGraph" path="/docs/change-tracking/change-tracking-graphql">Learn how to set up change tracking for your entities using NerdGraph to monitor them in New Relic.</DocTile>
  <DocTile title="Set up using CLI" path="/docs/change-tracking/change-tracking-cli">Learn how to set up change tracking for your entities using CLI to monitor them in New Relic.</DocTile>
  <DocTile title="View and analyze changes" path="/docs/change-tracking/change-tracking-view-analyze/">Learn advanced techniques for correlating change data with performance metrics and system behavior.</DocTile>
</DocTiles>