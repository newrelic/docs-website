...
* HTTP (including HTTP/2) request spans & golden metrics (throughput, latency, error rate)
* DNS request spans & latency metrics
* MySQL request spans & golden metrics
* PostgreSQL request spans & golden metrics
* Cassandra request spans & golden metrics
* Redis request spans & golden metrics
* Kafka messages & golden metrics
* JVM metrics
