...
4. On the **Choose install method** page, select either **Helm** or **manifest**.
   ...
   <CollapserGroup>
     ...
     >
       ...
       Helm installs a bundle containing the New Relic <InlinePopover type="infrastructure" /> agent, an integration to gather Prometheus metrics and Kubernetes events, and the Pixie integration. The deployment takes a few minutes to complete. To see the status of the install to the cluster, run `kubectl get pods -n newrelic`.
