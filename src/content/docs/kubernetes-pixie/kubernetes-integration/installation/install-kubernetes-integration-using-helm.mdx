...
5. Install the Kubernetes integration by running the command without `--debug` and `--dry-run`:
  ...
  <Callout variant="important">
  Make sure you're using Kubernetes version 1.26.x or [a lower version that we support](/docs/kubernetes-pixie/kubernetes-integration/get-started/kubernetes-integration-compatibility-requirements) for full featured support. 
  If you would like to use Kubernetes 1.27.x, please note that support for Kubernetes 1.27.x is currently in an experimental release phase and some metrics may not be available. 
  </Callout>
