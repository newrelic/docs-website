...
We use the namespaceSelector in the same way K8s does. In order to include only namespaces matching a label, change the `namespaceSelector` by adding the following to your `values-newrelic.yaml`, under the `newrelic-infratructure` section:
...
You can also use k8s expressions to include/exclude certain namespaces using the following syntax (Note the expressions will be "ANDed"):
...
A full list of the settings that can be modified can be found at the [chart's README](https://github.com/newrelic/nri-kubernetes/tree/main/charts/newrelic-infrastructure).
