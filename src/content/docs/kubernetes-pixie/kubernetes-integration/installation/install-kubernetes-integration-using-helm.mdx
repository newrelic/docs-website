...
5. Install the Kubernetes integration by running the command without `--debug` and `--dry-run`:
  ...
  Make sure you are using kubernetes version 1.24.X or previous versions because Pod Security Policies were dropped in 1.25+ versions.
  </Callout>

  If you use version 1.25+, you will get this error when running the above command:
    ```shell
