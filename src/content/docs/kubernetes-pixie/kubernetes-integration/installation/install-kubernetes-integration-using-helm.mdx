...
To install the integration using Helm, we recommend our [Kubernetes guided install](/docs/kubernetes-pixie/kubernetes-integration/installation/guided-install), which will prompt for some configuration options and autopopulate secrets and values for you. Additionally, our guided install also allows installing our integration as plain manifests rather than a Helm release.
...
5. Install the Kubernetes integration by running the command without `--debug` and `--dry-run`:
  ...
  <Callout variant="important">
  Make sure you're using Kubernetes version 1.24.x or [lower versions that we support](/docs/kubernetes-pixie/kubernetes-integration/get-started/kubernetes-integration-compatibility-requirements). Pod Security Policies were dropped in versions 1.25 and higher.
  </Callout>

  If you have version 1.25 or higher, you'll get this error when running the above command:
    ```shell
...
* [`nri-prometheus`](https://github.com/newrelic/nri-prometheus/tree/main/charts/nri-prometheus) and [`newrelic-prometheus-agent`](https://github.com/newrelic/newrelic-prometheus-configurator/tree/main/charts/newrelic-prometheus-agent)
