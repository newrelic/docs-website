...
<Steps>
    ...
    <Step>
        ...
        newrelic-logging:
        # Deploys the New Relic's <DNT>Fluent Bit</DNT> daemonset to EC2 nodes.  Disable for Fargate-only clusters.
        enabled: true
...
New Relic logging isn't available on Fargate nodes because of security constraints imposed by AWS, but here are some logging options:
  ...
  * If you're using <DNT>Fluentbit</DNT> for logging, see [Kubernetes plugin for log forwarding](/docs/logs/forward-logs/kubernetes-plugin-log-forwarding/).
