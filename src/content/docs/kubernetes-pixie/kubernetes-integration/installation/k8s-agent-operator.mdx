...
Here's what the instrumentation CR lets you map out:
   ...
   * Where it will apply the instrumentation CR (thanks to selectors)
...
Here you have a complete [instrumentation.yaml](https://github.com/newrelic/k8s-agents-operator/blob/main/local/example-cr.yml) for your reference.

## How to use selectors [#selectors]

To know when the instrumentation CR is going to inject APM agents, we need to use selectors. There are 3 selectors available that you can use together (they act as a logical AND (`&&`) operator) or by separate depending on your needs.
...
Defines at the namespace level the auto-instrumented pods.
...
### containerSelector
Defines at the container level the auto-instrumented container.

 <Callout title="TIP">
Without a `containerSelector` defined, the first (non-init) container defined in the pod will be instrumented.
 </Callout>

It provides 4 methods to select the appropiate container.
...
Find more information and examples here: [Link](https://github.com/newrelic/k8s-agents-operator/blob/main/instrumentation.md#imageSelector).
...
Find more information and examples here: [Link](https://github.com/newrelic/k8s-agents-operator/blob/main/instrumentation.md#envSelector).


#### nameSelector
Select containers based on the container names, and if they are init containers or regular containers.
...
Find more information and examples here: [Link](https://github.com/newrelic/k8s-agents-operator/blob/main/instrumentation.md#nameSelector).

#### namesFromPodAnnotation

Select containers based on the container names included in a chosen pod annotation.
...
Find more information and examples here: [Link](https://github.com/newrelic/k8s-agents-operator/blob/main/instrumentation.md#namesFromPodAnnotation).
