...
## Installation of the Kubernetes agent operator [#install-k8s-agent-operator]
...
## Configuring a namespace to enable the operator [#configuring-k8s-operator]
...
1.- Create a secret containing a valid [New Relic ingest license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) for each namespace you want the operator to instrument.
    ...
      --namespace <NAMESPACE_TO_BE_MONITORED> \
      --from-literal=new_relic_license_key=<YOUR_INGEST_LICENSE_KEY>
    ```

    Make sure to replace `<YOUR_INGEST_LICENSE_KEY>` with your valid New Relic license key.

2.-  Apply the following CRD to each namespace you want to auto instrument

  ```shell
    kubectl apply -f <YAML_FILE> -n <NAMESPACE_TO_BE_MONITORED>    
  ```

  YAML file to be applied
  ```yaml
...
## Upgrading the Kubernetes agent operator [#upgrading-k8s-operator]

Use the `helm upgrade` command to upgrade to a newer version of the Kubernetes agent operator. 


    ```shell
    helm upgrade k8s-agents-operator newrelic/k8s-agents-operator -n k8s-agents-operator
    ```

## Uninstalling the Kubernetes agent operator [#uninstall-k8s-operator]

To uninstall and delete the Kubernetes agent operator, use this command:

    ```shell
    helm uninstall k8s-agents-operator -n k8s-agents-operator
    ```
...
<Collapser
    id="modify-conf-apm"
    title="How can I modify the configuration of the APM agents??"
>
...
My applications are not instrumented, what should I check?
...
## Kubernetes agent operator supported versions [#k8s-supported-versions]

The Kubernetes agent operator currently supports the latest version of the following APM agents: Java, .NET, Node.js, Python, and Ruby
