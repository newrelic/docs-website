...
After APM Auto-attach is all set up in your cluster, the next step is just to roll out the configs required to get it operational. 
That involves having at least one instrumentation Custom Resource (CR) active in the cluster.
...
```
    ...
    <Callout title="TIP">
Keep in mind, applying the kubernetes.io/metadata.name label is essentially the same as selecting based on the namespace's name.
    </Callout>
...
You've got to specify the APM agent and its version within the instrumentation CR.
...
The license key used by default was created at installation time, if you need to send the APM telemetry to a different account, there's a few steps to knock out.

* Create a secret containing a new license key
...
* Reference the secret from the instrumentation CR
...
<CollapserGroup>
  <Collapser
    ...
    title="Python agent: Pods with a specific label/value and overriding app name"
  >
```yaml
...
By default, the Kubernetes APM Auto-attach automatically installs the latest available versions of the corresponding [APM agent](/docs/apm/new-relic-apm/getting-started/introduction-apm/).
...
To remove the APM instrumentation from an application, you must modify the matching label selector inside either the `podLabelSelector` or `namespaceLabelSelector` used  or simply delete the instrumentation CR, then restart the deployment. 
In a few seconds, you will see that the APM agents have been automatically removed.

## Updating the Kubernetes APM Auto-attach [#upgrading-k8s-auto-attach]
...
Uninstall the nri-bundle chart or if you only want to remove K8s APM auto-attach, run a helm upgrade with the following parameter:
...
<CollapserGroup>
  ...
  >
    Yes, you just need to add additional secrets in the cluster. Check needed steps in the "License keys (optional)" section included above.
  ...
  <Collapser
    id="modify-conf-apm"
    title="Can I install the K8s APM auto-attach if my applications are already instrumented?"
  >
  ...
  <Collapser
    ...
    title="Can I install the K8s APM auto-attach if my applications are running on a read-only file system?"
  >
    No, because the APM agents are injected at application runtime, it needs access to write to the application container's file system.
  </Collapser>

  <Collapser
    id="setup-operator"
    title="Can I setup K8s APM auto-attach in Windows nodes?"
  >
    Unfortunately, it only supports Linux nodes.
    Windows nodes support is expected on FY25Q1.

  </Collapser>

  <Collapser
    id="setup-operator"
    title="Can I setup K8s APM auto-attach in Fargate nodes?"
  >
    It hasn't been tested on AWS Fargate nodes with EKS.
...
* Please be sure to redeploy or deploy new applications after K8s APM auto-attach has been installed. Please be advised that it only auto-instruments new applications deployed in the cluster.
...
* Get logs from the APM auto-attach pod.
...
It is advised to uninstall any versions preceding 0.14 and proceed with the installation of the most recent release. 
Utilizing the label selectors within the instrumentation CR will enable the precise deployment of APM agents, thereby obviating the requirement for annotations.
...
Once is on general availability, the most recent 3 versions of each of the APM agents will be supported.
