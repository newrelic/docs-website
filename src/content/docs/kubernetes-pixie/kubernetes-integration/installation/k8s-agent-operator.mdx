...
<Callout title="PREVIEW">
    The K8s agent operator is in preview.
</Callout>
...
* Please ensure that your applications are redeployed or newly deployed after the operator has been installed. Note that the operator only auto-instruments new applications deployed in the cluster
...
* Check that the pods has the annotations that enables auto instrumentation
...
* Ensure init container has been injected and sucessfully executed inside the application's pod
