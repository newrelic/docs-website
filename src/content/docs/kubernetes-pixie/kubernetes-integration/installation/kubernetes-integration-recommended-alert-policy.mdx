...
<CollapserGroup>
 ...
 <Collapser
  ...
  ```json
  {
    ...
    "nrql": {
      "query": "from K8sPersistentVolumeSample select uniqueCount(volumeName) where statusPhase in ('Failed','Pending') and clusterName in ('your-cluster-name') facet volumeName, clusterName"
    },
    "signal": {
      "aggregationDelay": 60,
      "aggregationMethod": "EVENT_FLOW",
