...
<CollapserGroup>
  ...
  "nrql": {
    "query": "from K8sNodeSample select average(allocatableCpuCoresUtilization) where clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
  ...
  "nrql": {
    "query": "from K8sNodeSample select average(allocatableMemoryUtilization) where clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sNodeSample select latest(condition.Ready) where clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "FROM K8sPodSample, K8sNodeSample select ceil(filter(uniqueCount(podName), where status = 'Running') / latest(capacityPods) * 100) as 'Pod Capacity %' where nodeName != '' and nodeName is not null and clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sNodeSample select latest(unschedulable) where clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sNodeSample select average(fsCapacityUtilization) where clusterName in ('nr-quickstarts-sandbox') facet nodeName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "nrql": {
    "query": "FROM K8sPodSample select latest(isReady) where status not in ('Failed', 'Succeeded') and clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sPodSample select uniqueCount(podName) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') and status = 'Failed' facet namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "FROM K8sPodSample select latest(isScheduled) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "nrql": {
    "query": "from K8sContainerSample select sum(restartCountDelta) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') FACET containerName, podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  ```json
  {
    ...
    "query": "from K8sContainerSample select average(cpuCoresUtilization) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet containerName, podName, namespaceName, clusterName",
    "signal": {
      "aggregationDelay": 60,
      "aggregationMethod": "EVENT_FLOW",
      "aggregationTimer": null,
  ...
  "nrql": {
    "query": "from K8sContainerSample select average(memoryWorkingSetUtilization) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet containerName, podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sContainerSample select sum(containerCpuCfsThrottledPeriodsDelta) / sum(containerCpuCfsPeriodsDelta) * 100 where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet containerName, podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sContainerSample select uniqueCount(podName) WHERE status = 'Waiting' and clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') FACET containerName, podName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<Collapser
  ...
  "nrql": {
    "query": "from K8sStatefulsetSample select latest(podsMissing) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet statefulsetName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "nrql": {
    "query": "from K8sDaemonsetSample select latest(podsMissing) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet daemonsetName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  ```json
  {
    ...
    "query": "FROM K8sHpaSample select latest(maxReplicas - currentReplicas) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet displayName, namespaceName, clusterName",
    "signal": {
      "aggregationDelay": 60,
      "aggregationMethod": "EVENT_FLOW",
      "aggregationTimer": null,
  ...
  "nrql": {
    "query": "FROM K8sHpaSample select latest(desiredReplicas - currentReplicas) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet displayName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "name": "Job failed",
  "nrql": {
    "query": "from K8sJobSample select uniqueCount(jobName) where failed = 'true' and clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet jobName, namespaceName, clusterName, failedPodsReason"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "nrql": {
    "query": "from K8sDeploymentSample select latest(podsMissing) where clusterName in ('nr-quickstarts-sandbox') and namespaceName not in ('test') facet deploymentName, namespaceName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
...
<CollapserGroup>
  ...
  "nrql": {
    "query": "from K8sEtcdSample select max(processFdsUtilization) where clusterName in ('nr-quickstarts-sandbox') facet displayName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
  ...
  "nrql": {
    "query": "from K8sEtcdSample select min(etcdServerHasLeader) where clusterName in ('nr-quickstarts-sandbox') facet displayName, clusterName"
  },
  "signal": {
    "aggregationDelay": 60,
    "aggregationMethod": "EVENT_FLOW",
