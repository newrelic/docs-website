...
The easiest way to install the Kubernetes integration is to use our guided install to generate a manifest. It bundles not just the integration DaemonSets, but also other New Relic Kubernetes configurations, like [Kubernetes events](/docs/integrations/kubernetes-integration/kubernetes-events/install-kubernetes-events-integration), [Prometheus Agent](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/install-prometheus-agent/), and [New Relic log monitoring](/docs/logs).

You have these options to install the Kubernetes integration:

* Download the [New Relic Kubernetes quickstart](https://newrelic.com/instant-observability/kubernetes). This quickstart includes pre-built resources like dashboards, instrumentations, and alerts.

* Follow the [automated and guided install](https://one.newrelic.com/launcher/k8s-cluster-explorer-nerdlet.cluster-explorer-launcher?pane=eyJuZXJkbGV0SWQiOiJucjEtaW5zdGFsbC1uZXdyZWxpYy5ucjEtaW5zdGFsbC1uZXdyZWxpYyIsInBhdGgiOiJndWlkZWQiLCJlbnYiOiJrdWJlcm5ldGVzIiwiaW5pdGlhbEFjdGlvbkluZGV4IjpudWxsLCJhY3Rpb25JbmRleCI6MX0=). If you follow Kubernetes guided install [docs](#guided-install) you have these options to install the Kubernetes integration:
  * New Relic CLI
...
Make sure you are using Kubernetes version 1.16 to 1.25 as Kubernetes 1.26 is not currently supported.
</Callout>
...
<CollapserGroup>
  ...
  >
    The Kubernetes integration monitors worker nodes. In GKE, Google manages masternodes and they are abstrated from the Kubernetes platforms.
