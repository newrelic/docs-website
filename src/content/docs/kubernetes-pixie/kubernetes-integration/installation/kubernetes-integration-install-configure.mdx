...
<Tabs>
  <TabsBar>
    <TabsBarItem id="1">Guided install (recommended) </TabsBarItem>
    <TabsBarItem id="2">In Windows</TabsBarItem>
    <TabsBarItem id="3">In EKS Fargate</TabsBarItem>
    <TabsBarItem id="4">Manual Helm</TabsBarItem>
    <TabsBarItem id="5">In GKE Autopilot</TabsBarItem>
  </TabsBar>
...
<TabsPageItem id="5">
## Requirements
1. Use unprivileged mode: GKE Autopilot doesn't allow privileged containers.
2. Turn off controlPlane: GKE Autopilot doesn't give access to the control plane. 
3. Add "http" scheme and port "10255" to kubelet config
4. Disable logging: The logging integration is currently not supported in GKE Autopilot.
5. Disable pixie: Pixie is currently not supported in GKE Autopilot.

<Callout variant="important">
  Pixie and logging won't work in unprivileged mode. 
</Callout>

## Installation [#installation]

You can choose between two options for installing the Kubernetes integration in your GKE Autopilot cluster:
...
### Manifest command [#manifest-command]
* Update the manifest command to set privileged, controlPlane and logging to `false`.
* Add `"newrelic-infrastructure.kubelet.config.scheme":"http"` to payload
* Add `"newrelic-infrastructure.kubelet.config.port":"10255"` to payload
* Remove pixie payload from the command since pixie isn't currently supported in GKE Autopilot. 
  ...
  * Example manifest command:
    ...
    -d '{
      "global.cluster": "<Your Cluster Name>",
      "global.namespace": "newrelic",
      "newrelic-infrastructure.privileged": "false",
      "newrelic-infrastructure.controlPlane.enabled": "false",
      "newrelic-infrastructure.kubelet.config.scheme": "http",
      "newrelic-infrastructure.kubelet.config.port": "10255",
      ...
      "global.licenseKey": "<Your License Key>"
    }'
    ```

### Guided Install
Currently, GKE Autopilot isn't supported with the guided install option. 
