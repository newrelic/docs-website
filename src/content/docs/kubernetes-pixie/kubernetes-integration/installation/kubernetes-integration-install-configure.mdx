...
<table>
  <thead>
    ...
    <tr>
      <td>
        [Guided install](https://one.newrelic.com/marketplace?state=fc4edc3d-8214-a314-7432-3e62d7de43d7)
      </td>
      <td>
...
Follow the instructions in the **Guided install** tab above. 

The guided install will ask if you are installing in a GKE Autopilot cluster. After you enter `Y`, we will set `privileged`, `controlPlane`, `logging`, and `pixie` to `false`. The install will also set the kubelet scheme to http and kubelet port to 10255. 
...
Update your Helm command with the following flags:
...
* Set `privileged`, `controlPlane` and `logging` to `false`.
