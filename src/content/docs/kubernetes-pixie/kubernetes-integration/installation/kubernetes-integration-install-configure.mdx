...
* Make sure you whitelist the domains ([newrelic dockerhub](https://hub.docker.com/u/newrelic) and Google's registry `registry.k8s.io`) the installation will pull container images from. Note, you may need to follow the [commands](https://kubernetes.io/blog/2023/03/10/image-registry-redirect/#how-can-i-check-if-i-am-impacted) to identify the exact Google registry domain to be whitelisted.
