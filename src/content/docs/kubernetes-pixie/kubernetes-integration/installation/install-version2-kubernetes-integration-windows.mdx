...
<Callout variant="important">
    The command above allows installing kube-state-metrics, which is a mandatory dependency of the integration, on a linux node. By any chance if kube-state-metrics are scheduled on a non-linux node, deployment might fail. So, the simplest way is use nodeSelector and get kube-state-metrics to deploy on a linux node. This can be done by editing kube-state-metrics deployment. If your cluster does not have any linux node, the command above might not work. New Relic cannot offer support on how to install kube-state-metrics on non-linux nodes.
</Callout>
