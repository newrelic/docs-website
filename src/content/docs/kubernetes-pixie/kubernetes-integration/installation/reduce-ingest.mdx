...
If you've reviewed your [data ingestion](/docs/data-apis/manage-data/manage-data-coming-new-relic/) and want to send less data, we've got a simple solution for you. You can easily achieve this by setting up our [nri-bundle](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle)  Helm chart. Our charts let you fine-tune your settings. This reduces the ingested data, but may make the information slightly less detailed. These setting, also allow you to focus on the most critical data points, helping you identify the most important insights.

You can reduce the data in your charts and works for all installation methods: the guided install, Helm, or Manifest. Our charts can help you adjust settings to reduce the data you take in, which may result in less detailed information. These settings allow you to focus on the most important data, which can help you identify the most important insights.

It's also possible to reduce data ingestion through parameters. However, the modification of the parameters will depend on the method used to install Kubernetes.

## Reduce data ingested by setting `global.lowDataMode` [#reduce-data-ingest]

If you're looking to cut costs, you can modify the `global.lowDataMode` key value. This will cut down on the number of metrics sent. The modification of this key will depend on the method used to install Kubernetes:

    * **Guided install** (New Relic CLI, Helm, Manifest). The installation command includes the `lowDataMode` mode by default, thanks to these parameters: `NRI_CLI_LOW_DATA_MODE`, `NRI_CLI_PROMETHEUS_AGENT_LOW_DATA_MODE`, and `NRI_CLI_LOGGING_LOW_DATA_MODE`.

    * **Helm**. Set the `global.lowDataMode` key in the [`nri-bundle` chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) to true.

    * **Manifest**. `LowDataMod`e is only for Helm Charts, and Helms are templates applied to Manifests. When someone applies `LowDataMode` in the Helm chart, it makes a few small changes to the final Manifest. The changes in the Manifest are about specific settings, but any of them is about a parameter named `LowDataMode`.

`lowDataMode` key affects these specific components of the [`nri-bundle` chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle):
...
The New Relic Kubernetes integration allows changing the scrape interval at which metrics are gathered from the cluster. This lets you choose the right balance between data resolution and usage. We suggest choosing a scrape interval between 15 and 30 seconds for the best experience.
...
The modification of the scrape interval will depend on the method used to install Kubernetes:
...
If you want to change the scrape interval, just add the interval value under the `newrelic-infrastructure` section. Take a look at this example of the `values-newrelic.yaml` file to see how it looks:
