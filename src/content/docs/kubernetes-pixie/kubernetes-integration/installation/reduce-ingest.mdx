...
If you're looking to cut costs, you can modify the `global.lowDataMode` parameter value. This will cut down on the number of metrics sent. The modification of this parameter will depend on the method used to install Kubernetes:
    ...
    * **Manifest**. `LowDataMode` is only for Helm charts, and Helms charts are templates applied to manifest. When someone applies `LowDataMode` in the Helm chart, it makes a few small changes to the final manifest. The changes in the manifests are about specific settings, but any of them is about a parameter named `LowDataMode`.
...
If you want to change the scrape interval, just add the interval value under the `newrelic-infrastructure` section. Take a look at this example of the `values-newrelic.yaml` file to see how it looks for Helm:
