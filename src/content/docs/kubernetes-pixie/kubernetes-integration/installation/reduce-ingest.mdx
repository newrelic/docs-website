...
Our charts can help you adjust settings to reduce the data you take in, which may result in less detailed information. These settings allow you to focus on the most important data, which can help you identify the most important insights.


## Reduce data ingested by setting `global.lowDataMode` [#reduce-data-ingest]

If you're looking to cut costs, you can set the `global.lowDataMode` key in the [`nri-bundle` chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) to true. This will cut down on the number of metrics sent. 
