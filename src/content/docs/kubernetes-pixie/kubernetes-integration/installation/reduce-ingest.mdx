...
If you've checked your data ingestion and you'd like to reduce the amount you have, you can do it through charts and through parameters.
...
If you're looking to cut costs, you can modify the `global.lowDataMode` key value. This will cut down on the number of metrics sent. The modification of this key will depend on the method used to install Kubernetes:
    ...
    * **Guided install** (New Relic CLI, Helm, Manifest). The`global.lowDataMode` key is enabled by default.
    ...
    * **Manifest**. Set the `global.lowDataMode` key in the Manifest configuration yaml file to true. If you followed our [installation instructions](/install/kubernetes/?dropdown1=manifest), the file was called `newrelic-manifest.yaml`.

`lowDataMode` key affects these specific components of the `nri-bundle` chart:
...
The New Relic Kubernetes integration allows changing the interval at which metrics are gathered from the cluster. This lets you choose the right balance between data resolution and usage. We suggest choosing an interval between 15 and 30 seconds for the best experience.

The modification of this interval will depend on the method used to install Kubernetes:

    * **Guided install** (New Relic CLI, Helm, Manifest). You can't modify this value following our guided install flow.

    * **Helm**. Set `interval` in the [`nri-bundle` chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) the value you want.

    * **Manifest**. Set `interval` in the Manifest configuration yaml file to the value you want. If you followed our [installation instructions](/install/kubernetes/?dropdown1=manifest), the file was called `newrelic-manifest.yaml`.
...
<Callout variant="important">
    Setting `interval` to values larger than `40s` isn't allowed.
</Callout>
