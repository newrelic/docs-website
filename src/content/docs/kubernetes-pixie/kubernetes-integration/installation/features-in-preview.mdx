...
The Helm chart will create one DaemonSet per each version of Windows that is in the list and use NodeSelector to deploy the corresponding pod per node.
    ...
    <Collapser
        id="example-k8s-windows-busybox"
        title="Example: Get Kubernetes for Windows from a BusyBox container"
    >
...
* Volume:
    * `fsUsedBytes`: zero, so `fsUsedPercent` is zero
    ...
    <Collapser
        id="windows-kubelet"
        title="Known issues with the Windows Kubelet"
    >
...
<Callout variant="important">
    ...
    <Collapser
        id="fargate-update-version"
        title="Update to the latest version or to a new configuration"
    >
...
```
    ...
    <Collapser
        id="fargate-uninstall"
        title="Uninstall the Fargate integration"
    >
