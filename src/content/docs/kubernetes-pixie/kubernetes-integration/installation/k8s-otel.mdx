...
* Manual installation
  ...
  * Installation Using Your Own OpenTelemetry Collector
...
4. Make sure New Relic is getting the data it needs, including metrics, events, and logs, by running the right queries. See [Introduction to the query buisqlder](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/) for more information.
...
{/* 5. If you're using a GKE AutoPilot cluster, it's necessary to apply the following configuration in your `values.yaml` file to ensure compatibility and proper functionality of the OpenTelemetry Collectors.
  ...
  <Collapser
    ...
    title="Manual Helmless installation"
  >
...
<Callout variant="tip">
  ...
  When deploying without Helm, components deployed via the rendered manifests might include a prefix, such as `example-`. This prefix is typically used in Helm chart templates for dynamic naming based on the Helm release name. If you prefer cleaner naming conventions, you can adjust these prefixes directly in the manifest files before applying them.
    ...
    title="Installation using your own OpenTelemetry Collector"
  >
  ...
  2. Provide your custom OpenTelemetry Collector with a configuration file (typically `config.yaml`). This file defines how the collector receives, processes, and exports telemetry data. Ensure your `config.yaml` includes:

    * **Receivers:** Necessary receivers to collect Kubernetes telemetry (for example, `kubeletstats`, `k8sevents`, `hostmetrics`, `prometheus` for `kube-state-metrics` and `cAdvisor`, `filelog` for container logs, and `otlp` for application traces and metrics). Refer to the [Prerequisites](#prerequisites) section for a list of essential components.
...
nr-k8s-otel-collector:
  extraConfig:
    receivers:
      # Ensure the prometheus receiver is enabled or defined if you're scraping
      prometheus:
        config:
          scrape_configs:
            - job_name: "cadvisor-custom"
              kubernetes_sd_configs:
                - role: node
              relabel_configs:
                - source_labels: [__address__]
                  regex: "(.*):10255" # Example: target Kubelet's cAdvisor endpoint
                  target_label: __address__
                  replacement: "$1:10255"
    processors:
      filter/keep_cadvisor_version_info:
        metrics:
          include: # Use 'include' to collect only this specific metric
            match_type: strict
            metric_names:
              - "cadvisor_version_info"
    exporters:
      # Your existing OTLP exporter to New Relic is usually named 'otlphttp/newrelic'
      otlphttp/newrelic:
        endpoint: "[https://otlp.nr-data.net:4317](https://otlp.nr-data.net:4317)" # Adjust if your New Relic region is different
        headers:
          api-key: ${env:NEW_RELIC_LICENSE_KEY} # Ensure this environment variable is set
    pipelines:
      metrics/additional_metrics:
        ...
        processors:
          ...
          - k8sattributes # Essential for Kubernetes metadata enrichment
            ...
            extraConfig:
              exporters:
                otlphttp/secondAccount: # Unique name for this exporter
                  endpoint: "[https://otlp.nr-data.net:4317](https://otlp.nr-data.net:4317)" # Adjust for your New Relic region
                  headers:
...
1. Optional 1: Use the `proxy` section in your `values.yaml` file to the `nr-k8s-otel-collector` chart configuration.

  ```yaml

    nr-k8s-otel-collector:
    proxy: 'http://your-proxy-server:3128' # Example: [http://squid-proxy.squid:3128](http://squid-proxy.squid:3128)
