...
<Callout variant="tip">
  ...
  Windows node monitoring is now generally available. For the latest installation instructions, see the [Kubernetes integration installation guide](/install/kubernetes).
</Callout>
...
The New Relic agent for Windows sends [Kubernetes data samples](/docs/kubernetes-pixie/kubernetes-integration/understand-use-data/find-use-your-kubernetes-data/#event-types), such as `K8sNodeSample` and `K8sPodSample`. The limitations that apply depend on whether you are running in privileged (HostProcess) mode or unprivileged mode.

### Unprivileged mode limitations [#unprivileged-mode-limitations]
...
Additionally, `hostNetwork: true` cannot be set in the pod spec for standard Windows containers. This is a Kubernetes limitation that does not apply when using HostProcess pods in privileged mode.

### Windows kubelet metric gaps [#kubelet-gaps]

The following [Kubernetes metrics](/docs/kubernetes-pixie/kubernetes-integration/understand-use-data/find-use-your-kubernetes-data/#event-types) are unavailable on all Windows nodes regardless of mode, because the Windows kubelet either does not expose them or they are not applicable on Windows:
