...
The following [Kubernetes metrics](/docs/integrations/kubernetes-integration/understand-use-data/understand-use-data#metrics) are unavailable on all Windows nodes regardless of mode, because the Windows kubelet either does not expose them or they are not applicable on Windows:
