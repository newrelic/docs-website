...
You have completed the [installation procedure](/docs/kubernetes-monitoring-integration#install) for New Relic's Kubernetes integration with Helm for the `nri-bundle`, but the values in your `values.yaml` file are not being respected. Some [global values](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle#values) have been found to not be respected by our Helm templates. 
