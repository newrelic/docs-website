...
<CollapserGroup>
  ...
  >
    For the Kubernetes integration, the infrastructure agent adds a log entry only in the event of an error. If you are doing a more in-depth investigation on your own or with New Relic Support, you can enable verbose mode.
    ...
    <Callout variant="caution">
     ...
     - To get verbose logging details using Helm it is enough to add to the values.yaml `newrelic-infrastructure.verboseLog` or`global.verboseLog` to enable them in all subcharts.
        ```yaml
        ...
        Once you have edited the files upgrade the solution with the following command. Then wait some time, collect the logs, revert the change in the values.yaml and upgrade again.
     ...
     - To get verbose logging details for an integration manually using a manifest file you need to add two different environment variables:
       1. `NRIA_VERBOSE` set to `"1"` for all 'agent' containers for example below the `NRIA_LICENSE_KEY` environment variable.
       2.  `NRI_KUBERNETES_VERBOSE`set to `"true"` for all components of the integration, for example below the `NRI_KUBERNETES_CLUSTERNAME` environment variable.
  ...
  >
    Collecting the newrelic logs consider that there are different component of the integration and that in each pod two containers are running.
    ...
    If you interested into the logs of the previous execution add the `--previous` flag
