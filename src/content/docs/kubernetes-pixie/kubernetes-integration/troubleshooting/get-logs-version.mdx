...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
     ...
     - To get verbose logging details using Helm and `nri-bundle` chart it is enough to add to the values.yaml `newrelic-infrastructure.verboseLog`. Notice that you can enable them in all subcharts setting `global.verboseLog` .
        ...
        Then wait some time, collect the logs, revert the change in the values.yaml and upgrade again.
     ...
     - To get verbose logging details for an integration manually using a manifest file you need to set two different environment variables:
       1. `NRIA_VERBOSE` set to `"1"` for all `agent` and `forwarder` containers. For example below the `NRIA_LICENSE_KEY` environment variable. This environment variable enables agent verbose logs 
       2. set `NRI_KUBERNETES_VERBOSE` to "true" in all components  of the integration. This environment variable enables nitegration verbose logs
  ...
  >
    Collecting the newrelic logs consider that there are different components of the integration and that in each instance two containers are running.
