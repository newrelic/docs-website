...
Some [global values](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle#values) have been found to not be respected by our helm templates, so if you find that a global value isn't being respected by the appropriate chart, directly specify the chart. This is a workaround and the issue will be fixed. 
