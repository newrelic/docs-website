...
<CollapserGroup>
  ...
  >
    ...
    To resolve this problem make sure you specify a valid [license key](/docs/accounts/install-new-relic/account-setup/license-key).
  ...
  >
    ...
    * There are multiple versions of `kube-state-metrics` running and the Kubernetes integration is not finding the correct one.
    ...
    The Kubernetes integration automatically discovers `kube-state-metrics` in your cluster using by default logic:

    1. it looks for a service tagged with label `"app.kubernetes.io/name=kube-state-metrics"`.
...
Below are some solutions to the most common Kubernetes integration errors. These errors will show up in the standard (non-verbose) Infrastructure agent logs. If you need more detailed logs (for example, when working with New Relic support), see [Kubernetes logging](/docs/integrations/host-integrations/host-integrations-list/kubernetes-monitoring-integration#logs-versions).
