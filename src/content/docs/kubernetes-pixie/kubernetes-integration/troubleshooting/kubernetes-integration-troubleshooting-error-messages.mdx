...
<CollapserGroup>
  ...
  >
    ...
    * If the pod is in `Pending` or `ContainerCreating` status, please use `kubectl logs newrelic-bundle-nrk8s-kubelet-***<DoNotTranslate>** -n newrelic` and `kubectl logs newrelic-bundle-nrk8s-kubelet-**</DoNotTranslate>*** -n newrelic -c kubelet` to figure out the potential reasons from the logs.
  </Collapser>
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        ...
        * In the <DoNotTranslate>**# of K8s objects**</DoNotTranslate> chart, that data is missing.
