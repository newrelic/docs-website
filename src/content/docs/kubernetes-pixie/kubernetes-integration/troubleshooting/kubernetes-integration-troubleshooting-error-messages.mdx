...
<CollapserGroup>
  ...
  >
    ...
    * If the pod is in `Pending` or `ContainerCreating`, please use `kubectl logs newrelic-bundle-nrk8s-kubelet-hlpm7 -n newrelic` and `kubectl logs newrelic-bundle-nrk8s-kubelet-hlpm7 -n newrelic -c kubelet` to figure out the potential reasons from the logs.
  </Collapser>
</CollapserGroup>
