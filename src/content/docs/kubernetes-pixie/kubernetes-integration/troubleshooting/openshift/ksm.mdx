...
* Try the suggestions at [Failed to discover kube-state-metrics](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/common-error-messages/failed-discover-kube).
* Specify the namespace the `kube-state-metrics` container is in (by default, there is one created with the kubernetes integration bundle in the `newrelic` namespace)
