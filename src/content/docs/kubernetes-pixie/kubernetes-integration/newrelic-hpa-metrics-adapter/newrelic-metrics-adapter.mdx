...
1. First install the Kubernetes integration following the [installation documentation](https://docs.newrelic.com/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#installer) if not installed yet.
2. Upgrade the installation to include the New Relic Metrics Adapter by running the following command:
