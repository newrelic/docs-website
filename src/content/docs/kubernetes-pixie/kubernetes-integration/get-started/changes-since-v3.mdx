...
<Callout variant="note">
  Cloud provider tags like `aws.ec2.*` are no longer present in KSM component metrics since these are aggregated from the whole cluster therefore decorating them with the tags associated to the KSM component instance will not be always correct. Samples affected are: `K8sNamespaceSample`, `K8sDeploymentSample`, `K8sReplicasetSample`, `K8sDaemonsetSample`, `K8sStatefulsetSample`, `K8sServiceSample` and `K8sHpaSample`.
