...
tags:
  ...
  - Changelog
metaDescription: What has changed in Kubernetes Integration version 3
---
...
The `ksm.config` object has the following schema:
...
* `customAttributes` has changed in format to a yaml object. Previous format, a manually json-encoded string (e.g. `{"team": "devops"}), is still accepted although discouraged
...
While not related to the integration configuration, the following miscellaneous options for the helm chart have changed as well:

* `runAsUser` has been replaced by the more configurable `securityContext`, which is templated directly into the pods.
