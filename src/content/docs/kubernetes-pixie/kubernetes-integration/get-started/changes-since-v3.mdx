...
The reason for this change is that previously, these tags were erroneously sourced from a single, arbitrary node in the cluster. This resulted in misleading situations such as `K8sServiceSample` having a `hostname` property, with said `hostname` being that of a node and not the service, or a `K8sDeploymentSample` having a `numCores` property with those again being from an arbitrary node in the cluster, and not the total number of cores associated to the Deployment.
If any attribute needs to be added to these samples `customAttributes` can be used.
</Callout>
