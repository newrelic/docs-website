  ...
  <tbody>
    <tr>
      ...
      <td>
        1.16 to 1.26
        <Callout variant="important">
        Starting from Kubernetes version 1.26, the `@autoscaling/v2beta2` API is no longer available and replaced by the `@autoscaling/v2`. It is important that `kube-state-metrics` version 2.7+ is installed on the Kubernetes version 1.26+ clusters so that Horizontal Pod Autoscaling related metrics can continue to be reported. This is due to only `kube-state-metrics` v2.7+ can support the `@autoscaling/v2` API.
        </Callout>
      </td>
    </tr>
  </tbody>
