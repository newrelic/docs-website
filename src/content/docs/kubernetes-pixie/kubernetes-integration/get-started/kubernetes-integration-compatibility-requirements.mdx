...
If your cluster has Windows nodes and you would like to enable monitoring on these, please note that only Windows LTSC 2019 and Windows LTSC 2022 nodes are supported at this time. There may be differences in which Windows versions are supported by different cloud providers, depending on the version of Kubernetes the cluster is running.

In addition, some components of the Kubernetes integration are not supported for Windows nodes. Your Kubernetes cluster must include at least one Linux node to run the integration.

Due to limitations in cloud provider offerings and Kubernetes itself, there are a few key installation scenarios that are not supported for Windows, including but not limited to:
- Windows nodes running in OpenShift clusters.
- EKS Fargate clusters, as Fargate only supports Linux nodes.
- GKE Autopilot clusters, as Autopilot only supports Linux nodes.
