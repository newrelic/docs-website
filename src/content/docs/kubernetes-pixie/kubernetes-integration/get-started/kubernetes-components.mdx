...
The `newrelic-infrastructure` DaemonSet is the main component of the integration. It's divided in three different components: `nrk8s-ksm`, `nrk8s-kubelet`, and `nrk8s-controlplane`. The first is a deployment and the next two are DaemonSets.
...
Helm is the primary means we offer to deploy our solution into your clusters.Chart manages one deployment and two DaemonSets where each has slightly different configurations. This gives you more flexibilty to adapt the solution to your needs, without the need to apply manual patches on top of the chart, and the generated manifests.
