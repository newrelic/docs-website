...
The Kubernetes events integration is installed by default both when using our [Guided Install](/install/kubernetes), or when [installing through Helm](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm).
...
2. When creating or updating your chart, select the chart type <DoNotTranslate>**table**</DoNotTranslate> and use the following event query, replacing `YOUR_CLUSTER_NAME` with the name of your cluster:
...
1. [Navigate to the events page](https://one.newrelic.com): Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Events**</DoNotTranslate>.
2. In <DoNotTranslate>**Category**</DoNotTranslate>, select <DoNotTranslate>**kubernetes**</DoNotTranslate>.
3. Use the <Icon name="fe-search"/> <DoNotTranslate>**Search events**</DoNotTranslate> field to look for specific events. To focus on a specific set of events, select or change the filter set. 
...
* <DoNotTranslate>**In New Relic**</DoNotTranslate>: Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes cluster explorer**</DoNotTranslate>.

Then, to view the pod details:

1. In the Kubernetes cluster explorer, select a pod.
2. Select <DoNotTranslate>**Show pod events**</DoNotTranslate>.
