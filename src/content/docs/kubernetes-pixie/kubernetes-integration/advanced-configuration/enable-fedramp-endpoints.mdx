...
<Callout title="VERBOSE LOGS REQUIRED">
  You'll need to enable verbose logs to view the `collectorURL`. You can do this using the `--set newrelic-infrastructure.verboseLog=true` setting for the `nri-bundle` helm chart.  Don't forget to disable verbose logs once your validation is complete!
</Callout>
