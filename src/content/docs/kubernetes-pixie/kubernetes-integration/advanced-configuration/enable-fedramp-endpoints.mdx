...
For information about FedRAMP compliance at New Relic, visit our [FedRAMP Moderate documentation page](https://docs.newrelic.com/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp-moderate/).

### Helm [#helm]

FedRAMP support can be enabled globally by setting the `global.fedramp.enabled` parameter to `true` when performing a Helm install.  This global setting will cascade down to all sub-charts contained within the `nri-bundle` parent chart, resulting in all agents being configured to send their data to New Relic's FedRAMP-compliant endpoints.
...
There are a couple ways to validate that the FedRAMP endpoints have been configured successfully.  First, validate the the ConfigMaps contain `fedramp: true`, where appropriate.  The output from your cluster may vary depending on which components were installed.
...
Additionally, you can check the `collectorURL` value in the pod logs.  The New Relic Infrastructure agent connects to this collector for data transmission.  

<Callout title="VERBOSE LOGS REQUIRED">
  You'll need to enable verbose logs to view the `collectorURL`.  This can be done using the `--set newrelic-infrastructure.verboseLog=true` setting for the `nri-bundle` helm chart.
</Callout>

To validate that FedRAMP endpoints were enabled successfully, run the `kubectl` command below and look for the `collectorURL` in the log output. Be sure to replace the  `newrelic-bundle-nrk8s-kubelet-f74g2` pod name with a pod from your cluster.
