...
* Installing with charts `newrelic-k8s-metrics-adapter`:
  * The proxy can be configured setting the configuration option `proxy` in the [values.yaml](https://github.com/newrelic/newrelic-k8s-metrics-adapter/blob/main/charts/newrelic-k8s-metrics-adapter/values.yaml).
    This is used to create the `HTTPS_PROXY` environment variable in the container.
* Installing with chart `nri-bundle` as a dependency:

  * The proxy can be configured setting the configuration option `newrelic-k8s-metrics-adapter.proxy`. The configuration option is passed [down to the dependency](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle#configure-dependencies) `newrelic-k8s-metrics-adapter` modifying the `values.yaml` of `nri-bundle`:
