...
When you first deploy the New Relic Kubernetes integration to an account, we deploy a default set of pre-built alert policies to that account. Once you install the integration, they are ready to use. You can add them to your account, and then customize the [conditions](/docs/alerts/create-alert/create-alert-condition/alert-conditions/) to meet your needs.


<Callout variant="important">
  The pre-defined alert policies doesn't have a [notification channel](/docs/apis/nerdgraph/examples/nerdgraph-api-notifications-channels/) by default to prevent unwanted notifications.
</Callout>

You can modify the thresholds for these pre-built policies and configure your [alert notifications](/docs/alerts/get-notified/intro-notifications/) so that you receive the information that is most valuable to you.
