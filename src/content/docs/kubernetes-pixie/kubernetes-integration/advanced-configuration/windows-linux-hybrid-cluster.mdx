...
This document clarifies the installation of the Kubernetes integration for a mixed Windows and Linux cluster.  The instrumentation process involves the following steps:

- The `nri-bundle` chart will be installed on Linux nodes first
- As part of the `nri-bundle` install, if enabled, the `newrelic-logging` sub-chart will also be configured to run on **both Windows and Linux nodes**
- After installing the `nri-bundle` chart, the `newrelic-infrastructure` chart will be installed as a standalone release in order to successfully ensure Windows nodes are monitored.  Windows support has not been ported over to the new [nri-kubernetes v3 architecture](https://docs.newrelic.com/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/changes-since-v3/) yet, therefore, the previous architecture is installed to support Windows workloads.
...
The `newrelic/nri-bundle` chart contains the latest updates for Linux-based nodes. The `newrelic-logging` chart can also be configured during this step to install to both Linux and Windows nodes. This feature for `newrelic-logging` is reliable and supported.

Other sub-charts in `nri-bundle` can be installed, but you must configure a `NodeSelector` to avoid scheduling to Windows. If an installation fails on a mixed cluster using default values from guided install, add the `nodeSelector` values.
