...
If you get the following event while trying to create a pod
...
remember to add in `values.yaml` the `tolerations` described in (Automatic injection installation)[#auto-injection-install].
...
check if there is a Fargate profile with a selector that names the namespace where installation is occurring.
