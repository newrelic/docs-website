...
2. If a secret doesn't exist, create one in the same namespace as the pod containing the New Relic [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key), which is needed for the sidecar to report data.
...
4. Create a secret containing the New Relic [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). Each namespace needs its own secret.
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="Secret"
     >
       Create the following `Secret` that has a license with the Base64 encoded value of your [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). One secret is needed in each namespace where a pod you want to monitor is running.
...
<Callout variant="important">
  The agent cannot hot load the New Relic [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). After updating the secret, you need to rollout the deployments again.
</Callout>
