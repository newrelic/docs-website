...
<Callout variant="important">
Openshift 4.0 or higher requires [Dynamic Admission Control](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers) to enable the admission webhooks, since the `master-config.yaml` file isn't supported in those versions.
</Callout>
