...
To get the API server logs:
    ...
    5. If there are no log entries in either the API server logs or the metadata injection deployment, it means that the webhook was not registered properly.

    6. Ensure the metadata injection setup job ran successfully by inspecting the output of this command:

       ```shell
       kubectl get job newrelic-metadata-setup
       ```

    7. If the job isn't completed, investigate the logs of the setup job:

       ```shell
       kubectl logs job/newrelic-metadata-setup
       ```

    8. Ensure the `CertificateSigningRequest` is approved and issued by running this command:

       ```shell
       kubectl get csr newrelic-metadata-injection-svc.default
       ```

    9. Ensure the TLS secret is present by running this command:
    ...
    11. Ensure the `TargetPort` of the <DoNotTranslate>**Service**</DoNotTranslate> resource matches the <DoNotTranslate>**Port**</DoNotTranslate> of the <DoNotTranslate>**Deployment**</DoNotTranslate>'s container:
