...
When you set `windows.privileged=false`, containers run as standard `ContainerUser` without host network access. The agent operates in `NRIA_IS_SECURE_FORWARD_ONLY=true` mode - it forwards data from the kubelet integration scraper but doesn't directly access host resources. Node-level samples (`SystemSample`, `StorageSample`, `NetworkSample`, `ProcessSample`) aren't collected in this mode.
