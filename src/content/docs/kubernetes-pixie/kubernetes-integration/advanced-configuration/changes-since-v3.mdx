...
In order to upgrade from the Kubernetes integration version 2 (included in [nri-bundle chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) versions 3.x), we strongly encourage you to create a `values-newrelic.yaml` file with your desired [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) and configuration. If you had previously installed our chart from the CLI directly, for example using a command like the following:
