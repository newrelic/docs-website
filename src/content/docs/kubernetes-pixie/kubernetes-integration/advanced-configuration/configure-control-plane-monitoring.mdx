...
* Most managed clusters, including AKS, EKS and GKE, do not allow outside access to their control plane components. For this reason, on managed clusters New Relic will only be able to obtain control plane metrics for the API server, and not for etcd, the scheduler, or the controller manager.
