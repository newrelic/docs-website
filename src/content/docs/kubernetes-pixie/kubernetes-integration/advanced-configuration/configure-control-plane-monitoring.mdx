...
In previous versions, when the integration is deployed using `privileged: false`, the `hostNetwork` setting for the control plane component will be also be set to `false`.

With the latest version, `privileged` only affect node pods adding privileged to the security context so we can get more metrics from the host. All the components of this integration has `hostNetwork: false` by default except the pods that get metrics from the control plane. This is because most users and Kubernetes distributions configure the control plane metrics endpoints to listen only in the loopback interface, i.e. `localhost`.
