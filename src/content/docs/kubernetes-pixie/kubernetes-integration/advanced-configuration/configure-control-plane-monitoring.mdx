...
<Callout variant="tip">
  Unless otherwise specified, this page refers to the [Kubernetes integration
  v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3).
  Details on how to configure control plane monitoring for v2 can be found in a
...
[//]: # 'TODO: This should be a TLS secret when https://github.com/newrelic/nri-kubernetes/issues/342 is fixed.'
...
<Callout variant="important">
  If you are using `staticEndpoint` pointing to an out-of-node (i.e. not
  `localhost`) endpoint, you must change `controlPlane.kind` from `DaemonSet` to
  `Deployment`.
</Callout>
...
[//]: # 'TODO: When https://github.com/newrelic/nri-kubernetes/issues/342 is merged, we might be able to point our integration to the secret OpenShift provides directly, greatly simplifying this section.'
...
{' '}
<Callout variant="important">
  Even though the custom `ETCD_ENDPOINT_URL` is defined, etcd requires HTTPS and
  mTLS authentication to be configured. For more on configuring mTLS for etcd in
  OpenShift, see [Set up mTLS for etcd in OpenShift](#mtls-how-to-openshift).
</Callout>
...
3. Optionally, change the secret name and namespace to something meaningful.
...
6. Configure the integration to use the newly created secret as described in [the mtls section](#mtls).
...
![New Relic One - Kubernetes Cluster Explorer - Control Plane section](src/images/new-relic-one-k8s-cluster-explorer-control-plane-parameters.png 'new-relic-one-k8s-cluster-explorer-control-plane-parameters.png')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Kubernetes Cluster
  Explorer**: Use the Kubernetes cluster explorer to monitor and collect metrics
  from your cluster's Control Plane components.
</figcaption>
...
<Callout variant="tip">
  If you still can't see Control Plane data, try the solution described in
  [Kubernetes integration troubleshooting: Not seeing
  data](/docs/integrations/kubernetes-integration/troubleshooting/kubernetes-integration-troubleshooting-not-seeing-data).
</Callout>
