...
With the latest version, `privileged` only affects node pods adding privileged to the security context so we can get more metrics from the host. All the components of this integration are set up as `hostNetwork: false` by default, except the pods that get metrics from the control plane. This is because most users and Kubernetes distributions configure the control plane metrics endpoints to only listen in the loopback interface, i.e. `localhost`.
