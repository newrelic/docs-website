...
<Callout variant="important" title="Deprecated">
  **Hybrid cluster monitoring is now generally available in Kubernetes integration v3.** This v2 hybrid installation method is deprecated and will be removed in a future release.

  The new [Kubernetes integration v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) supports both Windows and Linux nodes natively through the [guided install process](/install/kubernetes/), eliminating the need for separate chart installations.

  For existing v2 hybrid installations, see [Upgrade from version 2](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/upgrade-v2) for migration guidance.
</Callout>
