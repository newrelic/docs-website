...
<Callout variant="important">
  The new [`nri-kubernetes` v3 architecture](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) now supports Windows. Please follow your choice of [guided install workflows](/src/install/kubernetes/intro) to enable Windows node monitoring in your cluster. Linux nodes will also be monitored, so hybrid cluster installations are supported by default.
</Callout>
