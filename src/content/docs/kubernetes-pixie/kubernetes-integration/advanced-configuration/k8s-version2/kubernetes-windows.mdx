...
<Callout variant="important" title="Deprecated">
  **Windows monitoring is now generally available in Kubernetes integration v3.** This v2 Windows installation method is deprecated and will be removed in a future release. 
  
  For new installations, use the [current Kubernetes integration installation guide](/install/kubernetes) which includes full Windows GA support. For existing v2 Windows installations, see [Upgrade from version 2](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/upgrade-v2) for migration guidance.
</Callout>
