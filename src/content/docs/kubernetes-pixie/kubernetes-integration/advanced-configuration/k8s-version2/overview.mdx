...
<Callout variant="caution">
    Keep in mind that we have replaced version 2 and you should not use it. We maintain this documentation for users who are still using version 2. See [Introduction to the Kubernetes integration](/docs/kubernetes-pixie/kubernetes-integration/get-started/introduction-kubernetes-integration/) to get started with the current version of Kubernetes.
</Callout>
...
For the Kubernetes Integration version 2, you need to add an entry for this ConfigMap in the `volumes` and `volumeMounts` section of the DaemonSet's `spec`, to ensure all the files in the ConfigMap are mounted in `/etc/newrelic-infra/integrations.d/`.
