...
<CollapserGroup>
  ...
  >
    ...
    When you use the Kubernetes integration, the infrastructure agent is distributed as a <DNT>Docker/DNT> image that contains both the infrastructure agent and the Kubernetes integration. The <DNT>Docker/DNT> image is tagged with a version, and the infrastructure agent also has its own version.

    Once the agent is sending data to New Relic, you can retrieve the versions of the infrastructure agent for Kubernetes (the <DNT>Docker/DNT> image) you are running in your clusters by using the following NRQL query:
