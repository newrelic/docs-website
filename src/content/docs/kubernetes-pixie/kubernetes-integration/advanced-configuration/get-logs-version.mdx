...
<CollapserGroup>
  ...
  >
    ...
    For the Kubernetes integration, the <InlinePopover type="infrastructure" /> agent is distributed as a Docker image that contains the infrastructure agent and the Kubernetes integration. The Docker image is tagged with a version, and the infrastructure agent also has its own version.
