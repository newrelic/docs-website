...
New Relic recommends using the [New Relic distribution of OpenTelemetry (NRDOT) for Kubernetes](https://github.com/newrelic/nrdot-collector-releases/tree/main/distributions/nrdot-collector-k8s), which is pre-configured to work seamlessly with New Relic. This distribution includes all necessary components and configurations to monitor your Kubernetes cluster effectively. However, if you prefer to use your own OpenTelemetry Collector, you can do so by ensuring it includes the required components for Kubernetes monitoring.
...
<Callout variant="tip">
    ...
    There are several advanced configuration options available to configure. For more information, refer to [advanced configuration](/docs/kubernetes-pixie/k8s-otel/advance-config).
...
The Helm installation method is the recommended approach for deploying OpenTelemetry on Kubernetes. You can use the NRDOT or your own OpenTelemetry Collector. This method simplifies the deployment process and allows for easy configuration management.

New Relic recommends using the NRDOT for Kubernetes, which is pre-configured to work seamlessly with New Relic. This distribution includes all necessary components and configurations to monitor your Kubernetes cluster effectively. However, if you prefer to use your own OpenTelemetry Collector, you can do so by ensuring it includes the required components for Kubernetes monitoring.

**To install Helm chart:**

1. Download the [Helm chart values file](https://github.com/newrelic/helm-charts/tree/master/charts/nr-k8s-otel-collector/values.yaml#L20-L24) adapt it to meet your specific requirements. For more information on the parameters, refer to the [parameters in `values.yaml` file](#config-params).
...
<Callout variant="tip">
    ...
    If you want to change your OpenTelemetry Collector to collect Kubernetes telemetry data, update the `image` settings in your `values.yaml` file to specify your desired OpenTelemetry Collector:
  ...
  The Manifest installation provides a way to set up OpenTelemetry for Kubernetes without using Helm. This method is suitable for users who prefer a more hands-on approach or have specific requirements that necessitate manual configuration.
...
<DocTiles>
  <DocTile title="Advanced configuration" path="/docs/kubernetes-pixie/k8s-otel/advance-config/">
    Learn how to customize your OpenTelemetry Collector configuration in New Relic.
  </DocTile>
  <DocTile title="Troubleshooting" path="/docs/kubernetes-pixie/k8s-otel/troubleshooting/">
