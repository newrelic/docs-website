...
New Relic recommends using the New Relic distribution of OpenTelemetry (NRDOT) for Kubernetes, which is pre-configured to work seamlessly with New Relic. This distribution includes all necessary components and configurations to monitor your Kubernetes cluster effectively. However, if you prefer to use your own OpenTelemetry Collector, you can do so by ensuring it includes the required components for Kubernetes monitoring.

This document provides guidance on how to install and configure OpenTelemetry for Kubernetes using the New Relic distribution of OpenTelemetry (NRDOT) or your own OpenTelemetry Collector. 
...
<Callout variant="tip">
    ...
    There are several advanced configuration options available to configure. For more information, refer to [advanced configuration options](/docs/kubernetes-pixie/k8s-otel/advance-config).
