...
<CollapserGroup>
    ...
    <Collapser
        id="otel-components"
        title="OpenTelemetry components for Kubernetes monitoring"
    >
...
### Configure using NRDOT [#configure-nrdot]

1. Download the [Helm chart values file](https://github.com/newrelic/helm-charts/tree/master/charts/nr-k8s-otel-collector/values.yaml#L20-L24) adapt it to meet your specific requirements. For more information on the parameters, refer to the [parameters in `value.yaml` file](#config-params).
...
### Configure using your own OpenTelemetry Collector [#configure-own-otel]
...
To configure your own OpenTelemetry Collector to send data to New Relic:
  ...
  2. Provide your custom OpenTelemetry Collector with a configuration file. This file must define how the collector receives, processes, and exports telemetry data. Ensure your `config.yaml` includes:
