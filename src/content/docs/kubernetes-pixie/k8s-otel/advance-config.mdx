...
If your provider is EKS, please make either of the following updates to your cluster's terraform or in the AWS console to enable the `resourcedetection` processor:

  * increase IMDS hop limit to 2 for node group(s) using `http_put_response_hop_limit`
  * enable pod identity for eks module using addon `eks-pod-identity-agent`
