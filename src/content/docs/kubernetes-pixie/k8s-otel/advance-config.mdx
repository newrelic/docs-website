...
New Relic provides a [Helm chart](https://github.com/newrelic/helm-charts/blob/master/charts/nr-k8s-otel-collector/README.md) to deploy OpenTelemetry Collector in your Kubernetes cluster. This Helm charts can be customized to meet your specific needs, including advanced configurations for various use cases.
...
The `LowDataMode` option is enabled by default to ingests only the metrics required by our Kubernetes UIs. This mode reduces the amount of data collected, focusing on essential metrics for Kubernetes monitoring. 
