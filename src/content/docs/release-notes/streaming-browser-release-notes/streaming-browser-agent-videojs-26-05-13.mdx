---
subject: Streaming Video & Ads for Browser
title: Media agent for Video JS v4.1.2
releaseDate: '2026-05-13'
version: 4.1.2
metaDescription: Release notes for Media agent for Video JS version 4.1.2
category: Video JS
---

## Highlights

- Added MediaTailor SSAI tracker initialization on loadstart after source load detection.
- Added DAI stream manager support through the stream-manager event.
- Improved bitrate reporting with:
  - Playback bitrate (AVERAGE-BANDWIDTH/BANDWIDTH)
  - Manifest max bitrate
  - Segment download bitrate
  - Network throughput bitrate

## Improvements

- Refined ad/content event handling to avoid duplicate or incorrect content events while ads are active:
  - Pause/Resume
  - Seek Start/End
  - Buffer Start
- Improved end-of-content handling for ad-enabled playback paths (IMA/Freewheel scenarios).
- Added safer fallback logic for tech wrappers (Hls.js, Shaka, contrib-hls) when bitrate data is unavailable in VHS.

## Technical notes

- Tracker metadata and playback context methods remain aligned with Video.js/Brightcove integrations:
  - Title/ID/Duration from mediainfo when available
  - Source and rendition data from active tech when available
- Listener registration/unregistration updated to include ad and stream manager lifecycle events.
