...
features: ["Serverless Mode for AWS Lambda", "Add support for <DNT>Elasticsearch<DNT> 8.13.0"]
...
Version 9.9.0 introduces support for AWS Lambda serverless function observability, adds support for <DNT>Elasticsearch<DNT> 8.13.0, and adds the 'request.temperature' attribute to chat completion summaries in ruby-openai instrumentation.
...
- **Feature: Add support for <DNT>Elasticsearch<DNT> 8.13.0**

  <DNT>Elasticsearch<DNT> 8.13.0 increased the number of arguments used in the method the agent instruments, `Elastic::Transport::Client#perform_request`. Now, the agent supports a variable number of arguments for the instrumented method to prevent future `ArgumentError`s.
