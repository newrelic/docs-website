...
features: []
bugs: ["Correctly store and send unscoped metrics when instrumentation.thread.tracing is enabled", "gRPC instrumentation no longer causes ArgumentError when other Google gems are present"]
security: []
---
