...
features: ["Distributed tracing enabled by default", "Cross Application Tracing is deprecated and disabled by default", "Update configuration default value for span_events.max_samples_stored from 1000 to 2000", "add_method_tracer now uses Module prepending instead of alias_method chaining", "Add support for ARM64/Graviton2 platform using Ruby 3.0.2+", "Remove previously deprecated method External.start_segment", "Remove support for Excon versions below 0.19.0," "Remove support for Mongo versions below 2.1", "Remove support for Merb"]
bugs: ["Correctly load the configuration options error_collector.ignore_messages and error_collector.expected_messages from newrelic.yml"]
security: []
---
