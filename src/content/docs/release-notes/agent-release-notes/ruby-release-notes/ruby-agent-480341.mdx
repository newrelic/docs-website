...
topics:
  ...
  - Agent release notes
  - Ruby release notes
releaseDateTime: '2018-01-31 11:05'
releaseVersion: 4.8.0.341
downloadLink: 'https://rubygems.org/downloads/newrelic_rpm-4.8.0.341.gem'
---
...
* `request.uri` Collected as an Agent Attribute
  ...
  Users can now control the collection of `request.uri` on errors and transaction traces. Previously it was always collected without the ability to turn it off. It is now an agent attribute that can be controlled via the attributes config. For more information on agent attributes [see here](https://docs.newrelic.com/docs/agents/manage-apm-agents/agent-data/agent-attributes).
