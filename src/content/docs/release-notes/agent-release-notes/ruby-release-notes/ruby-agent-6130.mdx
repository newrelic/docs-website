...
features: []
bugs: ["Agent uses correct host for calls to preconnect", "Calling add_custom_attributes no longer modifies caller params", "http.url query param spans are now obfuscated", "Allocations reduced for segment attributes", "Eliminate errors around Rake::VERSION with Rails", "Prevent connecting agent thread from hanging on shutdown", "Fix error messages about Rake instrumentation"]
