...
features: ["AWS: Support IMDSv2 by using a token with metadata API calls", "Add updated configuration options for transaction events", "Deprecate and replace select configuration options"]
