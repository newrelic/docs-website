...
features: ["Add official support for Ruby 3.0", "Add support for Rails 6.1", "Add support for Sidekiq 6.0, 6.1", "Add force_install_exit_handler config flag"]
bugs: ["Distributed tracing no longer overwrites Sidekiq trace data", "Errors correctly reported for spans with no other attributes", "'DistributedTracing.insert_distributed_trace_headers' supportability metric now recorded"]
security: []
---
