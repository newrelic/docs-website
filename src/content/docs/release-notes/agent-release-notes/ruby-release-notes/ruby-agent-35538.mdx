...
* Updates to logging across the agent
  ...
  We've carefully reviewed the logging messages that the agent outputs, adding details in some cases, and removing unnecessary clutter. We've also altered the startup sequence to ensure that we do not spam STDOUT with messages during initialization.
