...
features: ["Browser agent can now run on applications using CSP nonces", "Distributed tracing may now be set in server-side application configuration"]
bugs: ["Fixes conflicts with use of Module#Prepend", "In response to a change in the Rails codebase, we've updated our ActiveRecord 6.1+ instrumentation for spans to contiue displaying as expected"]
security: []
---
