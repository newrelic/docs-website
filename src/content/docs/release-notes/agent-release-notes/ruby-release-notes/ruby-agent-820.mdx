...
features: ["Add instrumentation for Tilt gem", "Remove Rails 2 instrumentation", "Deprecate the configuration option error_collector.ignore_errors"]
bugs: ["Don't record span events when using newrelic_ignore", "Scrub non-unicode characters from DecoratingLogger", "Stop distributed tracing headers from emitting errors when the agent is not connected"]
security: []
---
