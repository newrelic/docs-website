...
version: 3.13.0
downloadLink: 'https://github.com/newrelic/go-agent/tree/v3.13.0'
features:["Replaced the NR AWS SDK V2 integration for the v3 agent with a newer version"]
bugs:["Fixed a bug where grpc errors would not record error responses when using the UnaryServerInterceptor() or StreamServerInterceptor() functions"]
security: []
---
