...
* Fixed error if `process.config.variables.node_prefix` missing.
  ...
  If `process.config.variables.node_prefix` is falsey (which can happen if using electron, leading to this issue [https://support.newrelic.com/s/hubtopic/aAX8W0000008ZMO/new-relic-on-electron-nodejs](https://support.newrelic.com/s/hubtopic/aAX8W0000008ZMO/new-relic-on-electron-nodejs)) the `getGlobalPackages` function in `lib/environment.js` will give an err when it shouldn't.
