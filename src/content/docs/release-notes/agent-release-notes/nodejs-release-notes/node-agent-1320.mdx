---
title: Node Agent 1.32.0
contentType: releaseNote
template: releaseNote
topics:
  - Release notes
  - Agent release notes
  - Node.js release notes
releaseDateTime: '2016-10-20 15:05'
releaseVersion: 1.32.0
downloadLink: 'https://www.npmjs.com/package/newrelic'
---

### New features

* The agent now collects [database instance information](/docs/agents/nodejs-agent/supported-features/nodejs-instance-level-database-information) for MySQL and MongoDB operations. This information (database server and database name) is displayed in transaction traces and slow query traces.
* Datastore instance configuration can now be done through environment variables. These can be set through `NEW_RELIC_DATASTORE_INSTANCE_REPORTING_ENABLED` and `NEW_RELIC_DATASTORE_DATABASE_NAME_REPORTING_ENABLED`

### Bug fixes

* The agent will no longer crash the process when an express param handler is executed when a transaction is not active.
