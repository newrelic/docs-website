...
security: []
bugs: []
features: ["Added `server.address` to amqplib spans","Removed support for `redis` < 2.6.0","Removed legacy context manager","Removed instrumentation for `director`","Updated `mongodb` instrumentation to drop support for versions 2 and 3","Dropped support for Node.js 16","Migrated instrumentation for `next` into agent"]
---
