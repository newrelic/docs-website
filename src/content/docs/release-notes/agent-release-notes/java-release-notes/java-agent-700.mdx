...
bugs: ["Fixes an issue that could cause multiple versions of akka-http-core instrumentation to apply at the
same time", "The agent will now log dropped events at FINE instead of WARN to decrease
verbosity", "Fixes Javadoc comments that incorrectly stated that, when calling the noticeError API multiple times,
