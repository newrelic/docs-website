...
bugs: ["Fixed an issue that was preventing <DNT>Jetty<DNT> JMX threads metrics from being pulled", "Fixed bug in the recordMetric API", "Fixed a memory leak in the Async Servlet instrumentation", "Fixed a bug in the setRequestAndResponse API"]
security: []
---
<ButtonGroup>
  <ButtonLink
...
* Fixed an issue that was preventing <DNT>Jetty<DNT> JMX threads metrics from being pulled in later versions of <DNT>Jetty<DNT> 9.X.
