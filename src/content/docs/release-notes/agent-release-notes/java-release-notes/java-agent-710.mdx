...
* [Real-time profiling for Java using JFR metrics](/docs/agents/java-agent/features/real-time-profiling-java-using-jfr-metrics/)
  ...
  This feature is now enabled by default.
    ...
    <Callout variant="important">
      This feature will cause an increase in the consumption of data.
      The amount depends on the application. <span id="disabling-jfr">It can be disabled by adding the
      following to the agent yaml config nested under the `common` stanza:</span>
    ```
      jfr:
        enabled: false
    ```
