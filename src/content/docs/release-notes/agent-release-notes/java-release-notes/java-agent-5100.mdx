...
bugs: ["Changes to several config property names", "The strip_exception_messages config now excludes nested exception messages", "The agent will now use a lightweight tracer for SQL tracers beyond the segment_limit", "The agent would fail to report the <DNT>Docker/DNT> containerId for some combinations of systemd and Kubernetes", "The agent logger could fail to log"]
security: []
---
<ButtonGroup>
  <ButtonLink
...
* The agent would fail to report the <DNT>Docker/DNT> `containerId` for some combinations of systemd and Kubernetes
