...
version: 2.4.0
features: ["Add stall_threshold setting for reporting stall errors"]
bugs: ["ClassNotFoundException with JBoss AS 7", "NoClassDefFoundError with Glassfish 3.1.2", "ClassCastException with Glassfish 3.1.1", "Agent stops reporting web transactions", "Instrumentation error in <DNT>Jetty<DNT> 7.5.4", "Possible unknown host for external call with HttpClient 3.0.1", "Stall error reporting broken", "agent_enabled setting not working"]
security: []
---
...
* Bug fix: Instrumentation error in <DNT>Jetty<DNT> 7.5.4 (introduced in 2.2.0)
