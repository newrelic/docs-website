...
bugs: ["Fixed a memory leak in async continuations instrumentation for <DNT>Jetty<DNT> 7 and 8", "Improved error handling with invalid JMX yaml extension files", "Improved Circuit Breaker logging", "Fixed a bug in <DNT>Tomcat</DNT> 7+ instrumentation where request parameters could occasionally get stripped"]
security: []
---
<ButtonGroup>
  <ButtonLink
...
* Fixed a memory leak in async continuations instrumentation for <DNT>Jetty<DNT> 7 and 8.
...
* Fixed a bug in <DNT>Tomcat</DNT> 7+ instrumentation where request parameters could occasionally get stripped when using an async servlet with `capture_params` set to `true`.
...
* Fixed in 3.23.0
  * This release may experience a memory leak within the agent for <DNT>Tomcat</DNT> containing a large number of static JSPs (5000+).
