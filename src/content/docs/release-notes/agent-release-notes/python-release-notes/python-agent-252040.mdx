...
metaDescription: Release notes for Python Agent 2.52.0.40.
redirects:
  - /docs/release-notes/agent-release-notes/python-release-notes/python-agent-2520xx
---
...
A new configuration setting has been added: [`process_host.display_name`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#process_host-display_name). When set in the `newrelic.ini` configuration file, the [display name will be used in the APM UI](/docs/apm/new-relic-apm/maintenance/add-rename-remove-hosts#display_name), in place of the hostname that the agent automatically captures. In addition, the display name can be set using the `NEW_RELIC_PROCESS_HOST_DISPLAY_NAME` environment variable.
