...
This release of the Python agent adds instrumentation support for [Google ADK](https://pypi.org/project/google-adk), adds token counting for multiple AI providers ([Anthropic](https://pypi.org/project/anthropic), [Google GenAI](https://pypi.org/project/google-generativeai), [OpenAI](https://pypi.org/project/openai), and [AWS Bedrock](https://pypi.org/project/botocore)), reduces outbound `tracestate` header size, and fixes a crash in [graphql-core](https://pypi.org/project/graphql-core) instrumentation.
...
* Add instrumentation for Google ADK
  ...
  * Instrumented components include:
    * `LlmAgent` linked to Gemini or VertexAI backends via [`google.genai`](https://pypi.org/project/google-generativeai)
...
* Add token counting for Anthropic, Gemini, OpenAI, and Bedrock
  ...
  * The Python agent now reports token counts for all LLM invocations across multiple AI providers:
    ...
    * [Google GenAI SDK](https://pypi.org/project/google-generativeai)
