...
features: ['Exclude newrelic header by default', 'Add exclude and include settings for Hybrid Agent traces', 'Add `http.statusCode` attribute to transaction events', 'Anthropic streaming instrumentation', ', 'Improve caching of IS_PYPY in gc_data.py for performance', 'Gemini response streaming']
