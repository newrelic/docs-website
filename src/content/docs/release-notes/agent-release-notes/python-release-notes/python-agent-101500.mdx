...
This release of the Python agent adds support for [oracledb](https://pypi.org/project/oracledb/), fixes 'MessageTransaction' object has no attribute 'destination_name' bug, and fixes an authentication issue when recording AWS metadata.
...
* Fix an authentication issue when recording AWS metadata

  * When the agent is left continuously running on Kubernetes and AWS EC2 (but not EKS), the authentication token cannot be retreived for AWS metadata on agent restart. Now, the authentication token is cached so AWS metadata can be queried on agent restart.
