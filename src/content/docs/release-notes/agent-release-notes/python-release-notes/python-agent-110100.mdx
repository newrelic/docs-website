...
bugs: ['Fix a bug in aiomysql which caused RecursionError when using ConnectionPool', 'Fix a bug where properties were not properly passed in kombu's producer', 'Fix an error when shutdown_agent is called from within the harvest thread']
---
