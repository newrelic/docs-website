...
features: ['Updates support for Bedrock invoke_model']
...
This release of the Python agent updates support for Bedrock's `invoke_model`.  Additionally, it provides fixes for early activation in Django instrumentation as well as server side overrides for `max_samples_stored` being ignored.
