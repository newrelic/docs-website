...
Fixed a DeprecationWarning emitted by pacakge version detection used internally for Python 3.10+. When importlib.metadata is available, prefer using it to avoid checking deprecated __version__ attributes on modules. (Thanks @renanivo for the contribution!)
