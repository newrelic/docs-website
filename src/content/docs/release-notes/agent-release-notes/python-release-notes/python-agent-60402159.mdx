...
* **Sanic instrumentation updates**
  ...
  The agent has updated support for all Sanic versions, with testing done on all versions >=18.12.0. Additionally, blueprint middleware are now supported.

* **Pyramid transaction naming**

  Previously transaction names from exception views would override route names, this priority has been reversed to be more in line with other instrumentation.
...
* **HTTPX Crash**
  ...
  A crash was possible in HTTPX when the agent was initialized after creating an HTTPX client. This has been corrected.
