...
features: ['Support dict type logs and formatting in `NewRelicLogForwardingHandler`']
...
This release of the Python agent updates the `NewRelicLogForwardingHandler` to support dictionary type logs and apply formatting and fixes a bug in structlog so that it handles processors of all iterable types.
