...
This release of the Python agent adds instrumentation for <DNT>Elasticsearch<DNT> as a new datastore product and a more granular breakdown of various SQL operations in the “Databases” tab in the APM UI. In addition, the stack traces captured by the agent are now being trimmed to remove any code snippets.
...
* **Elasticsearch Support**
  ...
  Instrumentation support for the official <DNT>Elasticsearch<DNT> client module and the separate pyelasticsearch module have been added. Time spent in calls made to <DNT>Elasticsearch<DNT> will be listed in both the main overview chart, as well as in the Databases tab in the UI. Previously, calls to <DNT>Elasticsearch<DNT> would have been shown as time spent in external web service calls.
