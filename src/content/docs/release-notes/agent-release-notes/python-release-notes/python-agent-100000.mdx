...
* Remove Python 2.7 Support
  ...
  * Removes compatibility with Python 2.7. Also removes support for the following libraries that were only supported on Python2.7:
...
* Add environment variable for garbage collector runtime metrics
  ...
  * Garbage collector runtime metircs can now be enabled/disabled via the following environment variable: `NEW_RELIC_GC_RUNTIME_METRICS_ENABLED`. 
