...
features: ['Add new configuration options for logging to control log file size, log retention and log rollover strategy.','Allows instrumentation to be disabled from a newrelic.config file.']
bugs: ['Certain Profiler log messages did not print correctly when running under Linux.','Use original regex to get container id from cgroup v1 in fallback case.']
security: []
---
