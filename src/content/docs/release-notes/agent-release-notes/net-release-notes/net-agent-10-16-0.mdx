...
features: ['Add 32bit profiler path to IIS registry when installing 64bit agent.','Prevent using different bitness installer when in-place upgrading.']
bugs: ['Fix misleading log message on transaction name change.','Fix NRHttpClientFactory so that it creates only one client.','Prevent broken traces when HttpClient content headers contain tracing headers.','Remove the retained file count limit for Agent log files.']
security: []
---
