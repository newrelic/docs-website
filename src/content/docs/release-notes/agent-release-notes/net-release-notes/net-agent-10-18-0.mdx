...
features: ['Enable configuration of High Security Mode via environment variable (NEW_RELIC_HIGH_SECURITY).','Enable startup logging to Event Log for all applications on Windows.','Enable startup logging to EventLog for .NET 6+ applications on Windows.']
bugs: ['Detect Docker container id on cgroup v2.','Update Serilog EventLog sink configuration to enable event source creation (if the app is running with admin privileges).']
security: []
---
