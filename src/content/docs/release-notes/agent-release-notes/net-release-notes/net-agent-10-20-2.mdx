...
features: []
bugs: ['Eliminate intermittent deadlock on agent startup.','Path to agentinfo.json was built incorrectly, leading to file not found errors when running on Linux.','Prevent multiple SessionCache instances from being created.','Prevent null reference exceptions while serializing LoadedModuleWireModelCollection.']
security: []
---
