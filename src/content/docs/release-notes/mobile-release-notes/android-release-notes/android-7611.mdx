...
- Fixed a `NullPointerException` issue in memory sampling process.
- Added safe casting during the initialization of app launch times to prevent `ClassCastException` compatibility in Microsoft Intune MAM SDK.
- Improved okhttp callback protection to prevent agent crashes from user code exceptions.
