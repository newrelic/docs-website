...
Agent version 6.0.0 introduced changes to the way the agent plugin processes crashes at runtime. _Deferred crash reporting_ is now the default crash reporting behavior, in which case crash reports are recorded when they occur, but not uploaded until the next application launch. This agent release:
