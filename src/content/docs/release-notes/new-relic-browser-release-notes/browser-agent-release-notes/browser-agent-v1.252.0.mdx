...
version: 1.252.0
features: ["add types mappings for esm distribution","Report config changes away from default state for UX improvement","Create more granular metrics about stylesheet fix success","align browser reported uncaught syntax errors","Capture Internal Metrics for Session Replay Configurations"]
bugs: ["Add safe logic to snapshots","Fix adblock memory leak"]
security: []
---
...
Fixed memory leaks caused by request resolve failure (e.G. due to adblocker). Thanks to community member [@Joe-Degler ](https://github.com/Joe-Degler) for contributing this fix.
