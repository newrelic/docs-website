...
#### Standardize Feature Buffering Behavior
Establish one pattern for holding data before harvests as well as for retry logic that can be shared by the all the features. This reduces bespoke logic per feature and establishes a pattern for use with future architecture changes.

#### Add WebSocket wrapper and supportability metrics
Add a wrapper to detect usage of WebSockets. This is intended to help evaluate viability of the agent tracking WebSocket data and the overall usage patterns of that mechanism.
...
#### removing internal barrel exports
Reducing loader bundle size by reworking the internal source code imports and exports.

#### add payload size evaluation to generic events feature
Harvest early when the payload reaches 64kb and prevent harvests from sending if they exceed the 1MB ingest limit.
...
#### relative timestamps in session trace
