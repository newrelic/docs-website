...
version: 1.243.0
features: ["Do not report ajax nodes in session traces if in deny list","Remove deprecated API inlineHit"]
bugs: ["Re-update assets proxy to accept host URL strings","Adjust Session Replay meta and timestamps","Tap session entity into storage api for changes across tabs","Set Session Replay first chunk flags more reliably"]
security: []
---
