...
Expose a webpack splitChunks cacheGroup matcher for optional use for those building the agent with NPM.  This cacheGroup matcher ensures that any lazy chunks generated from the agent code can be de-duplicated and merged into a single chunk, loaded after the page has finished loading.  This behavior matches the process we implement when building the agent which is shipped for APM and copy/paste implementation.
