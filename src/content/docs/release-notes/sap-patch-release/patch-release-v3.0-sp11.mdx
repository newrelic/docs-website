...
**Important**: Select Monitoring Group `CALM_LMS` for this monitored system.

### New RFC check program

The RFC check program has been updated with a maximum wait time to receive responses from each destination. Results now include latency measurements from each destination.
