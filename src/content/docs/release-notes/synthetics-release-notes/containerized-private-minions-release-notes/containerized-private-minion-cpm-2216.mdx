---
title: Containerized Private Minion (CPM) 2.2.16
contentType: releaseNote
template: releaseNote
topics:
  - Release notes
  - Synthetics release notes
  - Containerized private minions release notes
releaseDateTime: '2019-11-11 13:12'
releaseVersion: 2.2.16
downloadLink: ''
---

### New features

Minions now capture more page metrics when you run a Synthetics simple or scripted browser checks!

* `firstPaint and firstContentfulPaint` paint performance values are now being collected.
* `longRunningTaskCount` is now being computed `( longRunningTasksAvgTime, longRunningTasksCount, longRunningTasksMaxTime, longRunningTasksMinTime)`.
* Navigation timing metrics are now being collected: `loadEventStart, loadEventEnd, unloadEventStart, unloadEventEnd, domComplete, domInteractive, domContentLoadedEventStart, domContentLoadedEventEnd`.
* All of these values are available as attributes on the `SyntheticRequest` event, in Insights, NR1, and on the [monitor results page](https://docs.newrelic.com/docs/synthetics/new-relic-synthetics/using-monitors/view-monitor-results#finding).

Refer to Google's Performance and navigation timing API's for more details:

* Google's [User-centric Performance Metrics](https://developers.google.com/web/fundamentals/performance/user-centric-performance-metrics) contains detailed information about its Paint Timing API and longRunningTasksCount.
* Google's [Navigation and Resource Timing Load Performance Assessment](https://developers.google.com/web/fundamentals/performance/navigation-and-resource-timing) contains detailed information about its Navigation Timing API.
