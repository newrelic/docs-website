...
The New Relic's one-step observability AWS services logs onboarding experiences we recently released included some undesired attributes in the ingested logs. We have fixed this in this release.

### Changed

* The AWS Lambda and Firehose log shipping integrations [released on December 18th 2024](/docs/release-notes/logs-release-notes/logs-24-12-18) will no longer include the `aws.region`, `aws.realm` and `aws.accountId` attributes in the ingested logs. These attributes, meant for internal use in the Logs product, shouldn't appear as attributes on log data.
