...
Starting on July 1st the Kubernetes Cluster Explorer will only be available at **[one.newrelic.com](https://one.newrelic.com) > Kubernetes Cluster Explorer**. The Kubernetes Cluster Explorer will only be compatible with Kubernetes integrations v.1.8 and newer from Wednesday, August 12th.
