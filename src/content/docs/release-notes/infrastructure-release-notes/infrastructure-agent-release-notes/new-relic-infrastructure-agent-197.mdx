...
* Updated bundled `nri-docker` integration version to [v1.1.1](https://github.com/newrelic/nri-docker/releases/tag/v1.1.1).
  * This version fixes missing <DNT>Docker/DNT> container metrics improving Linux `cgroup` path detection. This issue happened when `cgroup` was not mounted in the standard path `/sys/fs/cgroup`. Now it can discover different `cgroup` paths.
