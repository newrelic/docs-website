...
Inventory deltas are JSON diffs and they need to be applied in order aiming to provide a consistent state. Both the <InlinePopover type="infrastructure" /> agent and the New Relic database stored their latest **delta ID** received/submitted.
