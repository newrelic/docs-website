...
We are upgrading our Infrastructure Cloud Alerting to use the New Relic Streaming Alerts back end. While you will not see any changes to the frontend UI or API, the back-end implementation is moving to our new streaming alerts pipeline, with streaming algorithms that are specifically tuned for polling based cloud integrations.
