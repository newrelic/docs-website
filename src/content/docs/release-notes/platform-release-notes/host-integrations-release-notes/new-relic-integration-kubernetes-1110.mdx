---
title: New Relic integration for Kubernetes 1.11.0
...
releaseDateTime: '2019-12-11 15:30'
releaseImpact:
  - Changes
  - New
---
...
* **Added**: KSM can now be discovered using custom pod labels. By default we look for pods with the labels `k8s-app`, `app` or `app.kubernetes.io/name` with the value `kube-state-metrics`.  
  ...
  ```
  # Label a specific KSM pod. Always set the value to the string "true".kubectl label pod kube-state-metrics newrelic-ksm=true
  ```

  Update the newrelic-infrastructure-k8s manifest to use this labeled KSM pod:

  ```
  env:- name: KUBE_STATE_METRICS_POD_LABEL  value: newrelic-ksm
  ```
