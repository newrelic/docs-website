---
subject: NRDOT
releaseDate: '2026-04-23'
version: 1.13.0
metaDescription: Release notes for NRDOT Collector version 1.13.0
category: Collector
features: ["Added StateDirectory for ATP storage in systemd service"]
enhancements: ["**Breaking:** Removed the nrdot-collector-k8s and nrdot-collector-host distributions in favor of nrdot-collector"]
bugs: []
security: ["Fixed CVE-2026-39883"]
---

## Changelog
This release marks the removal of the `nrdot-collector-k8s` and `nrdot-collector-host` distribution. The last published version of those distros is `1.12.0`. Please migrate to using `nrdot-collector` instead which is a superset in terms of functionality and acts as a drop-in replacement with the two removed distros being implemented as [use cases](https://github.com/newrelic/nrdot-collector-releases/blob/1.13.0/distributions/nrdot-collector/README.md#use-cases).

### Features
* feat: add StateDirectory for ATP storage in systemd service (#545)

### Bug Fixes
* fix: CVE-2026-39883 (#560)

### Docs
* docs: update FIPS documentation (#565)


