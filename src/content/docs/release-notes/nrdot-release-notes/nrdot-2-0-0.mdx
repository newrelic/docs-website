...
category: Collector
features: ["Added sqlqueryreceiver to nrdot-collector-experimental"]
enhancements: ["Bumped OTel component versions from v0.156.0 to v0.157.0", "Bumped nroracledbreceiver and nrsqlserverreceiver to v0.157.2"]
bugs: ["**Breaking:** Removed receiver/jmxreceiver from nrdot-collector and nrdot-collector-experimental following its upstream removal", "Renamed configuration node cumulativetodelta to cumulative_to_delta (deprecated alias still available)", "Disabled system.cpu.logical.count in host config to restore previous behavior", "Removed the redundant system.cpu.utilization mean aggregation rule from host config"]
security: ["Fixed CVE-2026-56852"]
---
