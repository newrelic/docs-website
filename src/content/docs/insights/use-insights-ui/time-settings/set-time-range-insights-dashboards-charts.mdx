...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        To include the time zone label in the queried range, add a [time zone acronym <i className="fa fa-external-link fileinfo"><span>\[external link\]</span></i>](http://en.wikipedia.org/wiki/List_of_time_zone_abbreviations "Link opens in a new window.") to the timestamp. For example, to clarify that the query shows Eastern Standard Time, insert `EST` at the end of the timestamp.
