...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        To adjust UTC time to a [specific time zone <i className="fa fa-external-link fileinfo">
        \[external link\]
        </i>](http://en.wikipedia.org/wiki/List_of_time_zone_abbreviations "Link opens in a new window.") , include the hours adjustment in the query. For example, include a minus sign (-) for time zones west of UTC or a plus sign (+) for time zones east of UTC with the number of hours.
      ...
      >
        To include the time zone label in the queried range, add a [time zone acronym <i className="fa fa-external-link fileinfo">
        \[external link\]
        </i>](http://en.wikipedia.org/wiki/List_of_time_zone_abbreviations "Link opens in a new window.") to the timestamp. For example, to clarify that the query shows Eastern Standard Time, insert `EST` at the end of the timestamp.
