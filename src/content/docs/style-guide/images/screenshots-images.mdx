...
Because our translated i18n directory mdx files reference the same image files that our English-language mdx files do, when image files are deleted or renamed, this can cause a conflict error. Here are some tips for deleting or renaming an image file: 

* **Never delete image files**: just leave them there. We will instead run an occasional script that removes image files that aren't referenced by any mdx files, so this means we don't have to deal with the conflict error and don't have to touch the mdx files in the i18n directory. 
* **If you want to rename an image file, question if that's necessary to do.** The image file name is fairly unimportant. If you do decide you want to rename an image file, you must go into any mdx file that references the image file (including the i18n directory files) and update that file name. 
