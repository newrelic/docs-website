...
Our doc site images are stored in individual `images` directories at the root level of each taxonomy category. These `images` directories contain all the images used in the docs for that category. Do not link to an external file, all images used in docs must be stored in [our GitHub repo](https://github.com/newrelic/docs-website).
...
```
![An image showing an overview of the synthetic monitoring UI](src/images/synthetics-ui-overview.png "Synthetics UI overview")
```
...
```
![Dashboards in New Relic One](src/images/NR1-dashboards-image.png "Dashboards in New Relic One")
...
```
The UI includes a multiple app names indicator. ![Multiple app names indicator in New Relic One](src/images/new-relic-distributed-tracing-multiapp-icon.png "Multiple apps name indicator in New Relic One")
```
...
```
Select your app's settings ![settings icon](src/images/icon-settings.png "settings icon") icon.
```
