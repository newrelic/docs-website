...
Do not link to an external file, all images used in docs must be stored in [our GitHub repo](https://github.com/newrelic/docs-website).
...
For `.md` files, images are stored in individual `images` directories at the root level of each taxonomy category. These `images` directories contain all the images used in the docs for that category.
...
2. Continue with the following procedure to embed the image in a doc.

## Embed an image [#embed]

<CollapserGroup>
  <Collapser title={<>Embed in <InlineCode>.mdx</InlineCode> file</>}>
    Import your image in an `import` statement near the top of the `.mdx` file. Ensure there are new lines above and below the import. The basic structure:
  ...
  <Collapser title={<>Embed in <InlineCode>.md</InlineCode> file</>}>
...
<CollapserGroup>
  <Collapser title={<>Update in <InlineCode>.mdx</InlineCode> file</>}>
    To update an image:
  ...
  <Collapser title={<>Update in <InlineCode>.md</InlineCode> file</>}>
    To update an image:
...
<img src={nr1DashboardsImages} alt="Dashboards in New Relic One" title="Dashboards in New Relic One" />
...
* Add any optional height/width in the `style` prop.

```
import exampleImage from 'images/example.png';

The UI includes a multiple app names indicator <img src={exampleImage} class="inline" style={{height: "30px", width: "30px"}} alt="Multiple app names indicator in New Relic One" title="Multiple apps name indicator in New Relic One" />.
```
...
Select your app's settings <img src={exampleImage} class="inline" style={{height: "30px", width: "30px"}} alt="Multiple app names indicator in New Relic One" title="Multiple apps name indicator in New Relic One" /> icon.
```
