...
import nestingBulletListOne from 'images/nested-bullet-lists-1.png'

import nestingBulletListTwo from 'images/nested-bullet-lists-2.png'
...
Lists are a great way to explain procedures (numbered steps) or simply itemize items (bullet lists). While single-level lists are pretty straightforward, when you start nesting them, you can run into indentation issues.
...
When you nest bulleted lists under number lists, indent each child list by four spaces. Look at the example below where the initial child bullet list is not fully indented:
