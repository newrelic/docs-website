...
Adding a new top-level nav entry doesn't require any special configuration besides creating the necessary `.yml` file and related directories. 

1. Copy an existing file from the `src/nav` directory. 
2. Add a set of directories to `src/content` to support that nav file. You should have at least one level of nesting; otherwise, doc titles in the auto-generated nav pages will render as H2 headings. (For example, `src/content/level-one/level-two/doc` would work, while `src/content/level-one/doc` will produce strange formatting. 
3. Customize the file with the right list of docs and directories. 
