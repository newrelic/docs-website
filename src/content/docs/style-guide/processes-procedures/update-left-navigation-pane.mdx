...
If an index page path is in more than one nav `YAML` file, we use the nav that matches the top level path with the index page.
...
### Nav requirements for auto indexing

In order to populate the auto index page with nav `YAML` data, you must add the index page path to the most relevant nav `YAML` file.
...
Each nav item `path` must not have a trailing slash.



<Callout variant="important">
  In order for the auto index page to display the nav `YAML` content:

  - Ensure a nav section path references a valid file directory in `src/content`. The directory must not be empty.
</Callout>
