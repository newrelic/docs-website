...
* **Sidebar**: On almost all docs, the sidebar is what is visible on the left hand side, showing the structure of that category of docs. The sidebar and index view are both determined by the structure set in the nav file. 
...
### What determines the doc site structure? [#folders-vs-categories]
...
It's important to understand the above point. The divergence of the docs folder structure and the displayed docs site structure is necessary, because we need a way to control and set the displayed site structure that doesn't require us to keep the docs folder structure and folder and doc names completely paralleled with the desired site structure. But because these two things are so separate, it means we have some fairly complex behind-the-scenes coding to get them to work together, and this means that there can be fairly unintuitive aspects of how this logic works. 

We do have [procedures for common use cases](#procedures), but it will help you a lot to understand how the docs site works: it will help you solve problems on your own, and solve niche cases.
...
* If you rename an mdx file name or a folder name, that changes its URL, so that means you would have to add a redirect to that doc for its original URL. (In most cases, to change doc names, you should use the mdx file's `title` field, or else change the nav file to edit that doc's short category-view title.)

### What's in a nav file? [#nav-file-contents]
...
* The doc, set by `title` (e.g., "NGINX integration") and `path` (the doc's URL).
...
The structure set in the nav files are exposed in two places: 
...
If it finds that doc in two nav files, it attempts to find the right nav file by choosing the nav that matches the top level category. For example, if a doc with a relative URL `/docs/accounts/accounts-billing/account-setup/create-your-new-relic-account` was in two nav files, the docs site code would search to see if one of the nav files has `docs/accounts` as its top level path. If it finds a match, it uses that nav file. 

Sometimes, the same doc URL can be used in more than one nav file. This is done when a doc is pertinent to two areas, and we want to expose that doc prominently in the sidebar. In those instances, the doc will almost always have a nav file that it is more associated with, which means that some clicks of docs in the sidebar will take you away to another category. For an example of this, click the **Manage data** doc in [this category view](/docs/accounts/accounts-billing/general-account-settings) and see how it loads the nav file for the area it "lives in" ("lives in" is in parentheses because it isn't necessarily located in such a location; remember, the nav file is totally separate from the folder structure).


### When you click a category in the sidebar, what determines how that category view displays? [#category-view]
