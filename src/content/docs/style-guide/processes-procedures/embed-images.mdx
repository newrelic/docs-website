...
1. Find the images directory for your doc. For example, if your doc lives in `docs/style-guide/processes-procedures`, use the `docs/style-guide/processes-procedures/images` directory. If the `images` directory does not exist, create one in [VScode](https://code.visualstudio.com/docs/getstarted/tips-and-tricks#_create-or-open-a-file) or Finder.
2. Place your image in the `images` directory. Give the image a descriptive file name; for example, `fso-ui-overview.png` is much better than `123-go-dawgs.png`. 
3. Continue with the following procedure to embed the image in a doc. 
