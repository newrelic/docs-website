...
### Guidelines for features that can have high data ingest

Some New Relic tools and features can result in high levels of data ingest and, if a customer is on the New Relic One pricing plan, higher billing. Some guidelines for instances where it's thought that the possibility for spikes in data ingest should be addressed in the documentation: 

* For our New Relic One pricing plan, data ingest is a major billing factor and therefore it should be obvious that customers should keep an eye on their usage and attempt to understand the data ingest impacts when adding new monitoring solutions or changing configuration. Because there are many ways our tools and various configurations might result in higher-than-expected data ingest, we should generally only mention this as a concern in the docs in these situations:
    * When New Relic has changed something and that change has a good chance of resulting in an unexpected data ingest spike, we should add a note about this in the docs where most relevant. If we document such an impact, we should treat that addition as a temporary addition (for example, removing that note after several months). 
    * When it's known that a certain fairly common way of setting up one of our tools is especially likely to generate high amounts of data; in that case, we can recommend a solution for avoiding that. 
* If it's decided that we should document a risk of a data ingest spike, focus on the increase in data ingest, not on the billing impacts. This is for two reasons: 
    * Billing impacts are only a factor for customers on the New Relic One pricing model; we still have customers on the original pricing model, where data ingest is not a billing factor. 
    * For organizations on the New Relic One pricing model, it should be understood that data ingest is equivalent to billing.
