...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Start most UI paths with `go to **[one.newrelic.com](https://one.newrelic.com)**`.
      </td>

      <td>
        Almost any path within our primary UI should start with this stock phrase. Avoid other introductory words like `from` or `login to`. And remember, UI paths are always in **bold**. 
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        A good UI path is short, conversationial, and unambiguous. We often use `>` to indicate the user path to keep things brief, but when the UI is confusing you can also use positional words like `left navigation` or `upper-right`. 
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        The length of the path should influence your approach. A simple three-step navigation can be fully conversational. A multiple-step procedure may be an ordered list. And for something buried eight steps deep, consider using the `****[one.newrelic.com](https://one.newrelic.com)** > y > z**` convention.

        Here's an example of a simple navigation:

        Go to **[one.newrelic.com](https://one.newrelic.com)** and  click the **Query builder** icon to start querying your data.
