...
For languages that are casual about indendataion, we recommend that you look for similar examples and follow those patterns. In the case of NRQL, which works with a wide range of indentation and grouping, we recommend you do the following to make it easier to read:

* Capitalize keywords (clauses, functions, and operators), such as FROM, SELECT, FACET, WHERE, AND, OR, AS, SINCE, ORDER BY, JOIN and MAX.
...
* Left-justify keywords except in the following cases:
    ...
    * If the keyword doesn't have any modifiers (it stands alone), you can tack it at the end of a line that already has keywords (For example, TIMESERIES, AUTO, and EXTRAPOLATE).
    ...
    SINCE 60 minutes AGO EXTRAPOLATE
    ```
* For keywords inside of parentheses, indent them two spaces. For example, the WHERE keyword is indented two spaces like this:
    ```sql
    FROM PageView
    SELECT count(*)
    FACET CASES
...
We recommend that you add a code fence at the top of your snippet that is sandwiched between backticks. The code fence is simply the name of the language following the first three snippets. See the collapsers below for examples.
