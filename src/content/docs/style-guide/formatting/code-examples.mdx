...
<CollapserGroup>
  <Collapser
    id="inline-code"
    title="Inline code or code blocks"
  >
    To format small code snippets within a sentence, surround the code with single backtick ( `` ` `` ) characters.
  </Collapser>
  ...
  <Collapser
    ...
    title="Code blocks"
  >
    To format one or more lines of code, insert three backticks ( `` ``` `` ) above and below your code. This essentially acts like an HTML #lt;pre#gt; tag.

   To add syntax highlighting to a code block, insert the language following the first three backtacks. Here's an example with handlebars formatting:
...
To figure out if the language you want to format is already set up in Gatsby, see if it's listed in `gatsby-config.js` under `prism`. For example, look in this list:
...
If you want to add formatting support for a language that isn't listed, create a PR where you add the language to the list. Be sure to check with engineering to confirm that the language is supported by Gatsby.
  </Collapser>
