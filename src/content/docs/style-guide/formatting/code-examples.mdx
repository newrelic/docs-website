...
Sometimes we want to use placeholders for variables whose value will depend on the customer or the situation. For example, you might need to use `YOUR_NR_ACCOUNT_ID` to indicate a placeholder where the customer is meant to input their New Relic account ID.  
