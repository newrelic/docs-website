...
To highlight lines of code in a code block, you can reference the specific lines of text you want highlighted in the same place where you note the syntax. Here's an example: 
...
The first line reads: `js lineHighlight=1,3-5`, which tells it to use JavaScript syntax highlighting, and tells it to highlight the first line, and the third through fifth lines. Highlighting lines requires the simultaneous use of syntax highlighting: you must indicate a language if you want to highlight lines. 

## Placeholders for customer-specific variables [#value-placeholders]

Sometimes we want to use placeholders for variables whose value will depend on the customer or the situation. For example, you might need to use `YOUR_NR_ACCOUNT_ID` to indicate that that's a placeholder for the customer to put in their New Relic account ID.  
...
* Use `NR` when you think it may not be clear if a component is something of ours or a third party's. For example, when writing about connecting AWS to New Relic, you may want to specific that an account or API key is New Relic's. For example: `YOUR_NR_ACCOUNT_ID`.
...
<CollapserGroup>
  ...
  >
    ...
    * Account IDs and other IDs/#s: `YOUR_ACCOUNT_ID`, `YOUR_NR_USER_KEY`, etc. **This should be the general style used.**
