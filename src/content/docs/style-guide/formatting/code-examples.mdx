...
When you sandwich your code between three backticks, you need to consider both the code layout (indentation) and the syntax highlighting.
...
For languages that are casual about indendataion, we recommend that you look for similar examples and follow those patterns. In the case of NRQL, which works with a wide range of indentation and grouping, we recommend you follow some simple guidelines to make it easier to read. You should use your discretion if you think these guidlines don't apply to whatever snippet you're adding:

* Capitalize keywords (clauses, functions, and operators), such as FROM, SELECT, FACET, WHERE, AND, OR, AS, SINCE, ORDER BY, and MAX.
* The first line should start with FROM, althought queries run fine when you start with SELECT.
* Left-justify keywords except in the following cases:
    * If the keyword is closely tied to a preceding keyword (for example, AGO is closely tied to SINCE), so it belongs on the same line.
    * If the keyword doesn't have any modifiers (it stands alone), you can tack it at the end of a line that already has keywords (For example, TIMESERIES, AUTO, and EXTRAPOLATE).
    ```sql
    FROM Transaction
    SELECT count(*)
    WHERE appName='interestingApplication'
