...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Code blocks"
  >
  To format one or more lines of code, insert three backticks ( `` ``` `` ) above and below your code. This essentially acts like an HTML &lt;pre&gt; tag.

  To add syntax highlighting to a code block, insert the language name following the first three backtacks. Here's an example with handlebars formatting:
...
```
  ...
  To figure out if the language you want to format is already set up in Gatsby, see if it's listed in `gatsby-config.js` under `prism`. The list of currently supported languages would look like this:
...
```
  ...
  If you want to add formatting support for a language that isn't listed, create a PR where you add the language to the list in `gatsby-config.js`. 
  </Collapser>
