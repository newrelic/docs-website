...
* A capability is a collection of features that enable a customer to achieve a use case. A capability is considered a superset of features and often tends to be an outside-in term that customers associate with an existing category such as application performance monitoring, applied intelligence, <InlinePopover type="infrastructure" />, and log management. In other words, capabilities are the things we'd treat as SKUs if we sold them all separately.
