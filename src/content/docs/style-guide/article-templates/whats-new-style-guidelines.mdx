...
* **Filenames:** Don't include any spaces in your filenames. If you are a writer removing filename spaces from a contributor, be sure to ensure the name is unique so you avoid collisions with existing posts. One way to do this is to include the date in the filename.
* **IDs:** For writers who publish posts, be sure to drop any changes to `whats-new-ids.json` before committing. 