...
3. Open [homepage.yml](https://github.com/newrelic/docs-website/blob/develop/src/data/homepage.yml), find the spot where the new tile will be, and add a new line with the relative link for the new tile. For example, `- (/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster)`
