...
* Browse [the branched install docs we've made](#uses) and see what's possible. Select one you'd like to use as a template. 
...
You can build the branched install component in many ways, and it will likely take you a good amount of effort and struggle for you to get it working, as it can be complex, especially for more complex and nested processes. But, that said, here's a high level view of what a creation process will often look like: 
...
3. Change the top-level folder name to match your project, and change the `agentName` in the yaml file to match. The `agentName` is what tells the yaml file which folder pull its content from. (See [our example YAML files](#yaml-examples).)
...
* Vulnerability management: [Import dependabot data](/install/vm/). See a labeled image of the vulnerability management YAML. Details: 
...
* [.NET install](/install/dotnet). Details: 
  ...
  * It uses the guidedInstall component (contained in the appInfo-2 file). 
  * It does not use the config file component, because most customers edit the config file via environment variables or other options and not through direct use of the file. 
* [Python install](/install/python). Details: 
  ...
  * It makes good use of comments, which helps understanding of the code and improved maintenance.  
