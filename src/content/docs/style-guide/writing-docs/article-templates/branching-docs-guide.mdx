...
* numbered steps
* embedded, customizable config (or text) file that can be copied or downloaded
...
The tab switcher, like collapsers, is a way of presenting sets of content in a single location.
...
The config file is added to the `/install/config/agent-config` directory. Specific lines can be targeted in the config file for update by the reader.
...
The config (yaml) file is a highly customizable way to generate a wide variety of custom, interactive content. Being a YAML file, it’s extremely important to pay attention to indents and to make sure that every piece of the YAML file is correct. 

If you swap in .mdx content in your YAML file, the .mdx content needs to be local to your install doc directory (for example, Java). Although it might be tempting to link to content anywhere on the Docs site for the purposes of reuse, this will cause your doc to break.

The config file is extremely flexible and customizable. 
...
When working with this install framework, there are specific .mdx files to be aware of.

* Intro
    * Referenced in the YAML by `introFilePath`
    * This is the content at the top of the page.
    * Use for intro content.
* default 
    * free mdx file
* appInfoConfigOption 
    * this is for dropdowns, use whenever you want dropdowns
* agentConfig 
    * this is for the APM agent yml config file
* appInfo 
    * this step is the one where you display the content (includes recommendedGuided content)
...
The `agentConfig` MDX file points to the appropriate config file in the install directory and controls what lines in the config file are editable.
