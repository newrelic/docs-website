...
* **Example requests.** Tips for creating example requests: 
  * Use `<var>` tags for customer-specific values in example queries and mutations. Remember that you don't need to use `<var>` tags in example responses or error messages. 
