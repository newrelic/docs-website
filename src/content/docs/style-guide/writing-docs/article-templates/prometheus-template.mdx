...
Install the [integration quickstart] \[link to quickstart guide] to access built-in dashboards and alerts.
...
The different sets of metrics exposed by [add here the integration name] are defined in the [Integration_name documentation](\[link to the provider's documentation]).
...
Follow the troubleshooting tips from [integrationName documentation](\[link to the provider's documentation]) to make sure that metrics are configured as expected on your cluster.
