...
At the very top of the installation stages and content pages is a description that comes from either the internal [data source files](https://source.datanerd.us/nr1-dev-experience/catalog-service-elixir/tree/master/apps/db/priv/data_sources) or the [community files](https://github.com/newrelic/newrelic-quickstarts/tree/main/data-sources).
...
* Determine the default, most common installation path and focus on that.
...
* Try to reduce the amount of links outside the experience, but if you must link to external resources, move those links below the installation steps.
* Begin each stage with the default action.
* Minimize yyour use of optional material, but if you need to include it, move it to the bottom of each stage.
...
* Repeat the stage name as the title in the main body.
    ...
    />
* Don't include any links to other sites or supporting information&mdash;only the **See your data button**. This may mean you need to skip other helpful information such as NRQL queries to find data.
