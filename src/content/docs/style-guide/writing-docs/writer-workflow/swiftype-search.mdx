...
Swiftype is the elastic-based search engine that powers the search in `docs.newrelic.com` and the produdt UI.
...
The average re-index time for docs is 4 days.

### Manually add new page to Swiftype index [#manually-index]

You may want to manually add a new page to the Swiftype index when the crawl is too slow and you want the page available right away. Keep in mind that updating the idex may product quick results for the docs site, you may experience a lag for the docs in the product UI.

To manually add a new page:

1. Ensure the page you want indexed does not have meta robots noindex set and that it's available publicly.
2. Go to Swiftype admin UI > Content.
3. In the text input that says **Filter pages**, paste the URL of the page you want indexed.
...
Swiftype should index it within a minute or so. To check if it's indexed, refresh the page and paste the URL. Note that sometimes the admin UI is cached or lags. You can ask a colleague to check, use the REST API to check, clear the cache, or do a hard refresh of the page and check again.

## Manually update existing Swiftype record [#manual-index]

Similar to manually adding a new page to the index, you can update an existing record if the crawl is too slow and you want the index updated right away. Use this method if you've changed a page's URL and the old URL is redirected to the new one. See below for more information about [changing URLs](#url-changes).

To manually update / reindex an existing record:

1. Go to **Swiftype admin UI > Content**.
...
## Rank results for given search term [#rank-results]
...
1. Go to **Swiftype admin UI > Result Rankings**.
...
When you set a result ranking, you can also add aliases. An alias is similar to a synonym, but is only applicable to a specific result set.

To add aliases, go to **Results Rankings**, enter the search term, and add the alias in the text input.
...
To add synonyms, go to: https://app.swiftype.com/engines/new-relic/synonyms.

See https://swiftype.com/documentation/site-search/guides/synonyms for official docs.


## Best practice: Important search terms [#important-search-terms]

We probably want to ensure that results are ranked and accurate for search terms that:

* Are commonly pre-populated in NR1 Help Center
* Are known to be really vague with noisy results (example: browser)
* Are what users are searching for most on our site
* Include glossary terms

## Best practice: URL changes [#url-changes]

If a page that's already indexed has its URL changed, Swiftype will remove that record associated with old URL. And, if a redirect exists to a new URL, it will create a new record for the page that's associated with the new URL. When that happens, result rankings for the old record are lost and must be re-added for the new record with the new URL.

Note that this could impact search results in the New Relic product UI. If URL paths change for any pages, Swiftype will be behind what's in the product (storing the record for the old page at the old URL) until it recrawls and re-indexes, which can inexplicably take days to more than a week.
...
## Best practice: Canonical links [#cannonical-links]

If the canonical link in the head does not match the resolved URL, then Swiftype will not index the page.

For example, if the canonical link is:

```
<link rel="canonical" href="https://newrelic.com/instant-observability/apache/ad5affab-545a-4355-ad48-cfd66e2fbf00/" data-react-helmet="true">
```

And, the actual page does not have a trailing slash and redirects instead to:

```
https://newrelic.com/instant-observability/apache/ad5affab-545a-4355-ad48-cfd66e2fbf00
```

Swiftype will not index it due to the redirect.

For details, see [Swiftype Canonical Element Crawling](https://swiftype.com/documentation/site-search/crawler-configuration/canonical-element-crawling).

## How to update search results for New Relic UI pages [#update-ui-search]
...
2. Copy the value of the search term that is automatically populated by that page in the search bar.
    ...
    />
3. Log into Swiftype.
4. Click **New Relic** and then **Result Rankings**.
