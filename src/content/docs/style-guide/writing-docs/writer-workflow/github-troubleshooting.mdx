...
The Gatsby build fails due to missing formatting of files, tags, links, or filenames.
...
You're running with issues with your local build (localhost:8000).
...
## My redirect throws a 404 error [#redirect-404]

### Problem

My redirect throws a 404 error when testing it in my local build.

### Solution

Redirects take a long time to load in a local build. When you click on a redirect, wait until it throws a 404, and then wait ~1-2 minutes. It should redirect you after a while. If it doesn’t, ensure you set up the redirect correctly.
...
<Callout variant="important">
  To merge a PR, you only need the `required` checks to pass. 
  
  For merges to develop, the required checks are:
  * `run linter`
  * `run tests`
  * `license/cla`
  * `unpaired translations removed for
  For merges to main, the only required check is  `build the docs site`.
  
  If a required check fails, the failure must be addressed before the PR can be merged. If an optional check fails, reach out in the help channel so that the hero can look into the failure, but feel free to merge the PR since optional checks don't block releases.
</Callout>

Rarely, a build or check will fail due to some internal error. 
...
## My merge conflicts [#merge-conflict]

###  Problem

You get a conflic merge when submitting a PR. 
...
* From Github Desktop, click the option to resolve these in VS Code.
* From the GitHub website editor, click the *Resolve conflict* button.

###  Problem

You get a conflict merge with what's new posts. This is because the `whats-new-ids.json` file, which is automatically updated when the site builds, can get out-of-date pretty fast.
