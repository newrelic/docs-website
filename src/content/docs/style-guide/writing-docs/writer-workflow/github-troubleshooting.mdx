...
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="sign-out"
    title="Log out/log in to Github Desktop"
  >
In GitHub Desktop, go to Preferences and log out/back in.
  ...
  <Collapser
    className="freq-link"
    id="ssh"
    title="Check for a SSH authentication issue"
  >
...
When you create a PR, your Gatsby build fails.

### Solutions

Check your changed files for these common markdown mistakes:
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="nav-files"
    title="nav files indentation"
  >
  ...
  <Collapser
    ...
    title="Special characters in frontmatter fields"
  >
  ...
  <Collapser
    className="freq-link"
    id="brackets-and-tags"
    title="Missing brackets and tags"
  >
  ...
  <Collapser
    className="freq-link"
    id="images"
    title="Mismatch of image file names and paths"
  >
  Check for these common issues with images:
  * Mismatch of image file names and paths. Be careful when renaming images and their filename paths. A mismatch can cause the entire local build to fail.
  * Image file names are case-sensitive. Using the wrong capitalization results in a missing image in the doc.
  * Image file names have encoded values (like `%`) in the filename. Try to avoid using special characters if possible. 
  * Imported images: Be especially careful when dealing with image files that are imported.
  </Collapser>

  <Collapser
    className="freq-link"
    id=""
    title="Diagnose markdown errors with yarn-verify"
  >

  If you know which file is causing an error, you can further troubleshoot by running the `yarn verify-mdx path/to/file.mdx` command. This command returns more information about the error than the normal Gatsby output. This includes things such as the specific character and line number causing the error.
...
To stop all services running on localhost:8000, run this command in your VS Code terminal:
  ```
...
Your local build is throwing 404 errors.
...
#### Merge conflicts from what's new posts

If get a conflict merge with "what's new" posts, it's because the `whats-new-ids.json` file (which is automatically updated when the site builds) can get out-of-date pretty fast.
