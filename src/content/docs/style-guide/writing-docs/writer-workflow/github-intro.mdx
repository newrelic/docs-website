...
---
Tech writers (TW) are responsible for the docs. We [write and edit the docs](/docs/style-guide/writer-workflow/tech-writer-workflow/) and [exchange peer edits](/docs/style-guide/writer-workflow/peer-editor-workflow/), maintain our backlog, and `hero`. To make it all happen, we use our [Docs Team GitHub board](https://github.com/newrelic/docs-website) to track changes, file issues, and initiate pull requests (PRs).

## The who's who of an issue or PR [#who-is-who]
GitHub tracks every stage of an issue or pull request's life cycle, including the people behind the scenes.
...
Dramatis personae of issues or PRs:

* **Creator**: Opens the issue or PR. The creator could be someone on the Docs team, another Relic, or an external user. We'll label the issue or PR differently depending on who created it.
...
1. Create a branch of `develop` using Github Desktop by clicking **Current Branch**. in the top header, clicking **New Branch** in the dropdown, and then selecting **Develop**.
...
3. Push your changes with **Push Origin** in GitHub Desktop.
...
- `content`: All issues will use this label. `content` indicates the issue is content-related rather than a design or engineering issue.
...
2. The hero marks it using the `docs-issue-candidate` label.
3. Every week, another designated docs team member assesses all issues with the `docs-issue-candidate` label.
4. Once approved, that same designated docs team member will change the label from `docs-issue-candidate` to `docs-issue`. We then migrate the issue into the customer feedback Jira project.
