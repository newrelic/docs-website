...
## 4. Install npm (Node package manager)[#npm]

You need to install `npm` so that you can also install `nvm` (Node version manager). In your terminal, type `brew install npm`.

## 5. Install nvm (Node version manager) [#nvm]

Node version manager (`nvm`) is what you use to manage the version of Node installed on your computer. In your terminal, type `npm i -g nvm`.
...
Once you've installed `nvm`, you can install the correct version of Node. Node, or Node.js, is the JavaScript engine that powers our Gatsby site and its dependencies. Type `nvm use 16` to install Node version 16.
...
## Build the local site (faster) [#local-site-speed]
...
* The folder/s you specify must be at the root level of the docs directory. This doesn't include any subdirectories. For example, `browser` is a valid folder, but `browser-monitoring` is not because it's a subdirectory of `browser`.
