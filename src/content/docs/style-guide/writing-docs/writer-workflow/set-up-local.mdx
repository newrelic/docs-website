...
* The folder(s) you specify must be at the root level of the `/src/content/docs/` directory. Tis doesn't include any subdirectories. For example, `browser` is a valid folder, but `browser-monitoring` is not because it's a subdirectory of `browser`.
