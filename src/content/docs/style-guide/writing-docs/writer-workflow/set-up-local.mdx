...
## 0. Enable VS Code in your terminal [#code-terminal]
...
To achieve this you can add this variable to your yarn start command:
`BUILD_FOLDERS=mlops yarn start`
or
`BUILD_FOLDERS=apm,logs,errors-inbox yarn start`

Tips: 
* The folder/s you specify must be at the root level of the docs directory, this doesn't include any subdirectories. `browser` is a valid folder, but `browser-monitoring` is not because it's a subdirectory of browser.

* Make sure there are no spaces after the commas in your folder list `BUILD_FOLDERS=apis,infrastructure` not `BUILD_FOLDERS=apis, infrastructure`
