...
The only checks needed to merge a PR are the checks marked as `required` on the PR. These are `run linter` and `run tests` for merges to develop, and `build the docs site` for merges to main. If a required check fails, the failure must be addressed in order for the PR to be merged. If an optional check fails, reach out to the help channel hero so that they can look into the failure, but feel free to merge the PR since optional checks don't block releases.
</Callout>
