...
## Find the source of an error with logs and stack traces [#analyze]

The errors summary tells the story behind your error groups. Depending on how you've configured New Relic, each service might display different kinds of information about an error. For example, disabling distributed tracing limits trace details about external services. If you haven't set up your log forwarder yet, the **Logs details** section might lack all the attributes associated with your logs.

For the full errors analysis experience, we recommend that you set up these capabilities:

* Distributed tracing, which is enabled by default. Check that it hasn't been disabled. 
* Log forwarding with New Relic, which expands your logs in context experience
...
From the stack traces view, you can open the section of code directly in your IDE.
