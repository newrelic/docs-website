...
<Callout variant="tip">
  You can continue using the legacy externals for now, but we encourage you to try out the new externals feature to see the additional control you can find with your services. If you've already been using the legacy externals, you can simply switch to the new externals tab in the UI. To really see the benefit, you can also turn on distributed tracing.
</Callout>
...
Although the new externals feature uses a map that looks similar to other maps in New Relic, the big difference is that it includes transaction details between services. Also, the scope of the new externals is on a single service and it's relationships with the upstream and downstream service. Our other mapping tools do not show transaction-level detail:
  ...
  * Trace group maps: These maps show aggregations of requests in distributed tracing that have the same starting point (root entry span).
