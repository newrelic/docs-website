...
In this step, we focus on how configure your service to export data directly to New Relic like this:

![Diagram showing OpenTelemetry using New Relic's OTLP endpoint.](./images/native_otlp.svg "native_otlp.png")

<Callout variant="tip"> 
If you prefer to export your data to a collector, see our [instructions](#collector) below.
</Callout>

To configure the export of data directly to New Relic from your service, complete the following steps:

1. Follow the instructions at the link for your language:
...
2. Configure the OTLP exporter to add a header ( ```api-key``` ) whose value is your [Account License Key](https://one.newrelic.com/launcher/api-keys-ui.launcher).
3. Configure the endpoint for the exporter to point to New Relic. You will receive the endpoint URL when you sign up for the [pre-release program](https://docs.google.com/forms/d/e/1FAIpQLSdIJVEAYaP7TXe9LmQA64yIObGvt-nOiz5kXYsjxLBbvut_1A/viewform). 
...
![Diagram showing OpenTelemetry using the OpenTelemetry Collector and New Relic's OTLP endpoint.](./images/native_otlp_with_collector.svg "native_otlp_with_collector.svg")
