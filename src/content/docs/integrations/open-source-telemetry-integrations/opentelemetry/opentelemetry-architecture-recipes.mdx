...
### Use the Tail Sampling Processor with the OpenTelemetry collector [#tail-sampling-processor]

Another option for tail-based sampling is to configure the OpenTelemetry collector to use the [Tail Sampling Processor](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/tailsamplingprocessor) to sample traces prior to sending to New Relic.
