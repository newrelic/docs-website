---
title: How OpenTelemetry Works with New Relic
tags:
  - Integrations
...
Trace data is the most mature OpenTelemetry data type. Because of this, New Relic's OpenTelemetry user experience is largely based on trace data and is therefore influenced by your sampling strategry.

Sampling can be configured in a number of different ways: at the service level using the OpenTelemetry SDK for your language, or if you're running your own instance of the OpenTelemtry Collector you can configure it to do more sophisticated forms of sampling like tail-based sampling.

You can read more about how to configure sampling by referring to the following OpenTelemetry documentation:
...
New Relic's Infinite Tracing can also be used in conjunction with your OpenTelemetry instrumented services. In setting up Infinite Tracing, you need to override the default span endpoint and send telemetry data to the New Relic trace observer:

<Callout variant="important">
Currently, Infinite Tracing does not support OTLP ingest. You must run your own instance of the OpenTelemetry Collector and configure it to use the [New Relic exporter](/docs/).
</Callout>
...
| --------- | --------------- |
| Collector | [Batch Processor](https://github.com/open-telemetry/opentelemetry-collector/blob/main/processor/batchprocessor/README.md) |
| Go SDK | [BatchSpanProcessor](https://pkg.go.dev/go.opentelemetry.io/otel/sdk/trace#NewBatchSpanProcessor)
| JS SDK | [BatchSpanProcessor](https://open-telemetry.github.io/opentelemetry-js/classes/_opentelemetry_tracing.batchspanprocessor.html)
| Python SDK | [BatchExportSpanProcessor](https://open-telemetry.github.io/opentelemetry-python/sdk/trace.export.html#opentelemetry.sdk.trace.export.BatchExportSpanProcessor) |
...
| Metric Type | Aggregation Temporality | Supported |
| ----------- | --------- | --------- |
| Gauge || ✅ |
| Summary || ✅ |
| Sum | **Delta** | ✅ |
| Sum | **Cumulative** | 🟡 Converted to Gauge metric |
| Histogram || ❌ |
