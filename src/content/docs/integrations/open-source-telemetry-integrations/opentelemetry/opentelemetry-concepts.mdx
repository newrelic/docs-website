...
Familiarize yourself with these trace topics so you can ensure your traces and spans show up in New Relic.

### Start and end time [#start-end]

The `startTimeUnixNano` and `endTimeUnixNano` fields are required according to the OpenTelemetry specification for trace data. When `startTimeUnixNano` is not present the span will be dropped. When `endTimeUnixNano` is not present the duration of your span will be large and negative.

### IDs for traces and spans [#ids]

The `traceId` and `spanId` fields are required according to the OpenTelemetry specification for trace data. When `traceId` or `spanId` are not present the span will be dropped.
...
* **Collector:** If you're running your own instance of the OpenTelemetry collector, you can configure it to do more sophisticated forms of sampling, such as tail-based sampling.
...
Here are examples demonstrating how to configure aggregation temporality using the [Java](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/java/sdk-nr-config) and [Go](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/go/go-metrics) OpenTelemetry SDKs.
...
OpenTelemetry metrics and other signals may include attributes that consist of a homogenous array of primitive types. These attributes are not currently supported by New Relic.
...
Here are some tips for building NRQL queries in New Relic.
