...
New Relic's Infinite Tracing can also be used in conjunction with your OpenTelemetry instrumented services. You must run your own instance of the OpenTelemetry Collector and configure it to use the New Relic exporter (Infinite Tracing does not support OTLP ingest at this time). In setting up Infinite Tracing, you need to override the default span endpoint and send telemetry data to the New Relic trace observer:
...
New Relic supports ingesting OpenTelemetry metric data with the following notable limitation:
...
At the moment, language SDKs that have metric support produce metrics with a cumulative temporality by default. However, New Relic requires metrics to be sent with a delta temporality. You will need to refer to the OpenTelemetry SDK for your language to determine how to configure metrics to be emitted with a delta temporality.
