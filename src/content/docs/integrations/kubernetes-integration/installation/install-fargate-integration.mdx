...
Regardless of the approach you choose, the experience is exactly the same after it's installed. The only difference is how the container is injected.
...
This operator accepts a variety of advanced configuration options that can be used to narrow or widen the scope of the injection. It does this through the use of label selectors for both pods and namespaces.
...
In order to install the operator in charge of injecting the infrastructure sidecar, all you have to do is add the following flags to the the [install bundle command](https://docs.newrelic.com/docs/integrations/kubernetes-integration/installation/install-kubernetes-integration-using-helm/#install-k8-helm):
...
<Callout variant="important">
In the case of a hybrid cluster (EC2 and Fargate), make sure after deploying that the solution is not selected by any Fargate profiles; otherwise, the `DaemonSet` instances will be stuck in a pending state. Fargate-only environments don't have this limitation because no `DaemonSet` instances are created.
...
<Callout variant="tip">
Injecting the sidecar into deployments scheduled into EC2 nodes may lead to incorrect or duplicate data, especially if those nodes are already being monitored with the `DaemonSet`. Take this into account when designing custom selectors for the automatic injection.
</Callout>
...
2. At the moment, it's not possible to use the injected sidecar to monitor services running in the pod. The sidecar will only monitor Kubernetes itself. However, if you are an advanced user, you may want to exclude these pods from automatic injection. You would then manually inject a customized version of the sidecar with on-host integrations enabled by configuring them and mounting their configurations in the proper place. For help, see this [tutorial](/docs/integrations/kubernetes-integration/link-apps-services/tutorial-monitor-redis-running-kubernetes/).
...
### DaemonSet replicas are being deployed into Fargate nodes [#daemonset-replicas]
