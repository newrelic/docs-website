...
<Callout variant="tip">
In order for the sidecar to be injected on, and therefore to get metrics from pods deployed before the operator has been installed, the user needs to manually perform a rollout (restart) of the affected deployments. This way, when the pods are created, the operator will be able to inject the monitoring sidecar. New Relic has chosen not to do this automatically in order to prevent unexpected service disruptions and resource usage spikes. 
</Callout>
...
<Callout variant="tip">
These setup steps are for a default setup. Before completing these, we suggest you to take a look at the [Configuration](#config-auto) section below to see if you want to modify any aspects of the automatic injection.
</Callout>
