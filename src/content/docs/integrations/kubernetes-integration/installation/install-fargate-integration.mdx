...
<Callout variant="important">
In the case of a hybrid cluster (EC2 and Fargate), make sure after deployment that the solution is not selected by any Fargate profiles; otherwise, the `DaemonSet` instances will be stuck in a pending state. Fargate-only environments don't have this limitation because no `DaemonSet` instances are created.
