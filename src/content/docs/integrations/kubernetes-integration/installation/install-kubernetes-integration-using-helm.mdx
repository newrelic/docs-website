...
We recommend using the [`nri-bundle`](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle) chart for Kubernetes deployments, as it provides the easiest way to select, install, and upgrade individual components at your will. This is the same chart that is used and referenced by our automated installer.
...
- `webhook.enabled=true`: Will install our minimal webhook, which adds environment variables that, in turn, allows linking applications instrumented with New Relic APM with infrastructure data.
...
### Helm 2 using nri-bundle (recommended)
