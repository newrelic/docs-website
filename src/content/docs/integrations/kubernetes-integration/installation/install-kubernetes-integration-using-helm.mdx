...
<Callout variant="tip">
  To use Kubernetes integrations and infrastructure monitoring, as well as the rest of our [observability
  platform](https://one.newrelic.com), join the New Relic family! [Sign up](https://newrelic.com/signup) to create your
  free account in only a few seconds. Then ingest up to 100GB of data for free each month. Forever.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    New Relic has several charts for the different components that offer different features for the platform:

    - [`newrelic-infrastructure`](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-infrastructure):
    Contains the main Kubernetes integration and the New Relic Agent. This is the bare minimum needed to get data from
    your cluster.
    ...
    [plugin](https://github.com/newrelic/newrelic-fluent-bit-output), to easily forward your logs to [New Relic
    ...
    reports cluster events (i.e. `kubectl get events`) to New Relic.
    - [`nri-metadata-injection`](https://github.com/newrelic/helm-charts/tree/master/charts/nri-metadata-injection):
    Automatically injects metadata in applications deployed to the cluster to correlate New Relic APM monitoring with
    ...
    You can flip the `ksm.enabled` flag to `true` if KSM is not already present in your cluster.
    ...
    The `nri-bundle` Chart will pull the following dependencies, if the corresponding flags are enabled:
    dependencies:
    ...
    that the individual components of the chart, as listed at the beginning of this section, can be supplied their
    configuration values by prefixing them with the name of the chart. For example, to run the New Relic Agent in
    ...
    is a configuration value for said chart.

    Please refer to [each of the chart's README](https://github.com/newrelic/helm-charts/tree/master/charts/) to see all
  ...
  >
    ...
    2. Make sure that `kube-state-metrics` is installed on your machine:
    ...
    3. If your cluster uses role-based access, create a service account for `tiller` (Helm's service which runs inside
    the Kubernetes cluster) using a Kubernetes manifest file:

    4. Paste the following in a manifest file `rbac-config.yaml`:
    ...
    A full list of the configuration parameters can be found in the [`newrelic-infrastructure` chart README](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-infrastructure)
