...
Alternatively, we also offer fully manual instructions for [deploying our integration using Helm](/docs.newrelic.com/docs/integrations/kubernetes-integration/installation/install-kubernetes-integration-using-helm).
...
Our Kubernetes integration is under active development and we regularly release updates which include bug fixes, new features, and support for newer Kubernetes versions and cloud providers. We strongly recommend all our customers to regularly update the Kubernetes integraiton to get the best experience.
...
### Upgrading using the automated installer using manifests
