...
<CollapserGroup>
  ...
  >
    ...
    1. `kube-state-metrics` service has been customized to listen on port 80. If that is the case, you may see an error like the following in the `verbose` logs:
       ...
       As a workaround, [increase the kube-state-metrics client timeout](https://docs.newrelic.com/docs/integrations/kubernetes-integration/installation/kubernetes-installation-configuration#kube-state-metrics-timeout-change).
