...
Gives replication priviliges to `newrelic@localhost` with a maximum of 5 connections.

       ```
       sudo mysql -e "GRANT REPLICATION CLIENT ON *.* TO 'newrelic'@'localhost';"
       ```

Gives select priviliges to `newrelic@localhost` with a maximum of 5 connections.
