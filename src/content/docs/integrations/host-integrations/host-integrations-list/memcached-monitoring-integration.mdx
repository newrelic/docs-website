...
Specific settings related to Memcached are defined using the `env` section of the configuration file. These settings control the connection to your Memcached instance as well as other security settings and features. The list of valid settings is described in the next section of this document.
...
The Memcached integration collects both Metrics(<strong>M</strong>) and Inventory(<strong>I</strong>) information. Check the **Applies To** column below to find which settings can be used for each specific collection:
