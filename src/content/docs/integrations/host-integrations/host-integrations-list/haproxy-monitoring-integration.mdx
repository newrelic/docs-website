...
Specific settings related to HAProxy are defined using the `env` section of the configuration file. These settings control the connection to your HAProxy instance as well as other security settings and features. The list of valid settings is described in the next section of this document.
...
<table>
  ...
  <tr>
    ...
    <td>
      A URL pointing to the stats page for HAProxy.
    </td>
    <td>
      N/A
    </td>
...
<CollapserGroup>
  ...
  >
    In this configuration we are using the environment variable `HAPROXY_STATS` to populate the STATS_URL setting of the integration:
  ...
  >
    In this configuration we are monitoring multiple HAProxy servers from the same integration. For the first instance (`STATUS_URL: https://1st_haproxy-instance/stats`) we are collecting metrics and inventory while for the second instance (`STATUS_URL: https://2nd_haproxy-instance/stats`) we will only collect metrics.
