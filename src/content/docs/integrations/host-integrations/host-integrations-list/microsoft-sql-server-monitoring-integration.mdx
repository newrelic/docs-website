...
* `custom_metrics_config`: full path and file name for an external queries yaml file. This should be used instead of `custom_metrics_query` if you need multiple custom queries to be executed. For an example configuration, see the [example config file](#example-config).
