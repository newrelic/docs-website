...
* If installed on-host: edit the config in the integration's YAML config file, `mssql-config.yml`.
...
Specific settings related to MS SQL are defined using the `env` section of the configuration file. These settings control the connection to your MS SQL server instance as well as other security settings and features. The list of valid settings is described in the next section of this document.
