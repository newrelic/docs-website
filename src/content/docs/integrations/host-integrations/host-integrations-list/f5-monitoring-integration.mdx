...
<Callout variant="tip">
  Administrator-level permissions may be required to collect some [system sample](#system-sample) metrics or [system inventory](#system-inventory) configuration data. For more information on user permission levels, see the [official F5 documentation on user role access descriptions](https://techdocs.f5.com/en-us/bigip-15-0-0/big-ip-systems-user-account-administration/user-roles.html#GUID-41F9AC55-5594-4D88-89C4-717DACAABA6F).
</Callout>
...
<table>
  ...
  <tr>
    ...
    <td>
      A JSON array of BIG-IP partitions to collect from. See this [metrics-only with partition filtering example](#filters).
    </td>
    <td>
      ["Common"]
    </td>
