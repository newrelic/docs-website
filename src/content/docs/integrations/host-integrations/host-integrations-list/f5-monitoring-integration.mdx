...
* F5 BIG-IP [user account](#enable-instance) with Access Auditor-level user privileges and iControl REST API access permissions.
...
### Enabling your F5 instance [#enable-instance]
...
* If installed on-host: edit the config in the integration's YAML config file, `f5-config.yml`.
...
Specific settings related to F5 are defined using the `env` section of the configuration file. These settings control the connection to your F5 instance as well as other security settings and features. The list of valid settings is described in the next section of this document.
...
<table>
  ...
  <tr>
    ...
    <td>
      A JSON array of BIG-IP partitions to collect from. See [example below](#filters).
    </td>
    <td>
      ["Common"]
    </td>
...
<CollapserGroup>
  ...
  >
    In this configuration we are using the environment variable `F5_HOST` to populate the HOSTNAME setting of the integration:
  ...
  >
    In this configuration we are collecting metrics only and adding `"MyOtherPartition"` to the list of partitions to be sampled. By default we only sample the `"Common"` partition:
  ...
  >
    In this configuration we are monitoring multiple F5 servers from the same integration. For the first instance (`HOSTNAME: 1st_f5_host`) we are collecting metrics and inventory while for the second instance (`HOSTNAME: 2nd_f5_host`) we will only collect metrics.
