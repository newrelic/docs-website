 ...
 * Only one node agent using the `all` command, as metrics are collected for the whole cluster. The rest using the `inventory` command.
 * All nodes using the `all` command with the `master_only` set to true, so only the elected master will collect the metrics and the rest will collect only the inventory.
### Arguments
...
* `master_only`: `true` or `false` to only collect metrics if the node is an elected master.
