 ...
 * All nodes using the `all` command with `master_only` set to true, so only the elected master collects the metrics. The rest of agents collect only the inventory.
...
* `master_only`: `true` or `false`. If `true` the node only collects metrics if it's an elected master.
