...
* You can obfuscate JSON blocks if you need to reference multiple values in your configuration YML file. Note that you may need to escape the quotation marks `""` with backslashes on Windows hosts. For example: `'{\"attribute\":\"value\"...`. In addition, it is recommended to use Powershell on windows hosts, rather than Command Prompt, since Command Prompt can handle quotation marks in unexpected ways. 
