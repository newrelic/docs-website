...
* You can obfuscate JSON blocks if you need to reference multiple values in your configuration YML file. Note: you may need to escape the quotation marks `""` with backslashes on Windows hosts. For example: `'{\"attribute\":\"value\"...`
...
The following example will allow retrieving the Nginx user and password that has been obfuscated using the New Relic CLI. This example uses dot notation when retrieving the credentials, since in this example, the obfuscated value was a JSON block:
