...
Your Prometheus OpenMetrics integration for Docker or Kubernetes exceeded allowable metric rate limits. You want to see more details about why the `NrIntegrationError` event has been applied to your New Relic account.

## Solution

To examine rate limit errors:

1. Use the following NRQL query:
...
To help prevent this from happening, you can use filters to control the types and amount of data that your integration sends to New Relic. For more information, see [Ignore or include Prometheus metrics](https://docs.newrelic.com/docs/integrations/prometheus-integrations/install-configure/ignore-or-include-prometheus-metrics).
