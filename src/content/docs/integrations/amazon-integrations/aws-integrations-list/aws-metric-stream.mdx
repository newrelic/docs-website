...
```
Note that not all metrics will have their custom tags as dimensions. Currently, only metrics linked to entities in New Relic Explorer will have their custom tags associated. The AWS CloudWatch metric stream does not include tags as part of the stream message, hence, additional processing is required on the New Relic side.
