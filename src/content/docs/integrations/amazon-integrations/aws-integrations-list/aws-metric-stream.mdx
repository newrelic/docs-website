...
AWS Metric Streams is temporarily disabled for accounts hosted in the New Relic EU datacenter. 
</Callout>
...
Similarly as with custom tags, New Relic also pulls metadata information from relevant AWS services in order to decorate AWS CloudWatch metrics with enriched metadata collected from AWS Services APIs. Information is accessible in New Relic as additional dimensions to the metrics provided by AWS CloudWatch. This is an optional capability that's complementary to the CloudWatch Metric Streams integration. 
...
* Consider using [drop data rules](https://docs.newrelic.com/docs/telemetry-data-platform/manage-data/drop-data-using-nerdgraph/) to discard metrics based on custom filters (e.g. drop metrics by namespace and tag, tag value or any other valid NRQL criteria).
...
* AWS CloudWatch Metric Stream is focused on metrics only. Integrations such as AWS Health, AWS Billing and AWS CloudTrail collect additional instrumentation that's not available with metric streams. Please consider keeping those enabled as polling integrations.
