...
* Check that the same function is used on the metrics (e.g. average, min, max).
* On New Relic side, make sure you filter the same timestamp / timeframe (considering timezone) to show the exact same time as in AWS CloudWatch.
...
Note that AWS states the maximum resolution for every metric reported in AWS CloudWatch (for example 1 minute, 5 minutes, etc). 
...
* Running: The stream is running correctly. Note that there may not be any metric data been streamed due to the configured filters. Although there is no data corresponding to the configured filters, the status of the Metric Stream can still be running.
* Stopped: The stream has been explicitly set to the halted state (not because of an error). You can start - stop your Metric Stream and this state is useful to temporarily stop the streaming of data without deleting the configuration.
