...
After executing the Diagnostics CLI from your terminal, you will see the results for each task as they are completed. Tasks that result in a `Warning` or `Failure` status code will log additional details regarding possible issues found during execution, along with troubleshooting suggestions and relevant links to documentation.  If you use the `-attach` flag, you will be able to view your results in [New Relic One](./interpret-nrdiag-output/#diag-in-nr1)
