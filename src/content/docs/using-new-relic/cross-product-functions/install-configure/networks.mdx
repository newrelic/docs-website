...
Your browser must be able to communicate with a number of domains for New Relic One to work properly. Blocking domains can cause issues with individual product features or prevent pages from loading altogether.

Connections can be made to additional domains that do not affect the usage of the product when blocked. Note that Nerdpacks or other features that communicate with external services might have additional domain requirements.
