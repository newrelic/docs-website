...
Your browser must be able to communicate with a number of domains for New Relic One to work properly. See [User-facing domains](#user-facing-domains) for the list of domains to be allowed through your firewall.
...
Update your allow list to ensure New Relic can communicate with a number of integral domains that are listed in this section. Blocking domains can cause issues with individual product features or prevent pages from loading altogether.
...
If your organization uses a firewall that restricts outbound traffic, follow specific procedures for the operating system and the firewall software you use to add the following domains to the allow list.
...
Synthetic private minions report to a specific endpoint based on region. Follow specific procedures for the operating system and the firewall software you use to allow the private minion to access the endpoint or the static IP addresses associated with the endpoint. These IP addresses may change in the future.
...
These network blocks also apply to third-party [ticketing integrations](/docs/using-new-relic/user-interface-functions/share-your-data/ticketing-integrations) and [New Relic cloud integrations](/docs/integrations/infrastructure-integrations/get-started/introduction-infrastructure-integrations/#cloud-integrations).
