...
<CollapserGroup>
  ...
  >
    ![A screenshot of the two account dropdown menus.](./images/account-dropdowns.png "The two account dropdown menus.")

    <figcaption>The account dropdown on the left lets you switch accounts. The account dropdown on the right gets you to your account settings.</figcaption>

    To access your account settings, in the top right, click your profile icon for all your [account-related abilities](/docs/accounts).

    In the top left, use the **All accounts**  dropdown menu to switch accounts. 
  ...
  <Collapser
    ...
    title="agent"
  >
    At New Relic, an agent is a piece of monitoring software that provides [integrations](#integration) with various technologies (for example, web frameworks, host operating systems, or database types). The agents send that data to New Relic, usually on a specific cadence.

    For more information, see:

    * [New Relic integrations](https://newrelic.com/integrations)
  ...
  >
    A unit of measurement that determines your pricing for some New Relic products governed by our original product-based pricing plan. For more information, see [Compute unit pricing](/docs/accounts/accounts/subscription-pricing/compute-unit-cu-pricing-vs-host-based-pricing-apm-infrastructure#compute-unit).
  </Collapser>
  ...
  <Collapser
    ...
    title="event"
  >
    The word **event** is a [general term that can have many meanings](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data).

    At New Relic, **event** can have several meanings:

    * At New Relic, **event data** is one of our core data types. Event data represents a record of a single event at a particular moment in time. Events can vary by type (for example, [`Transaction`](/docs/insights/insights-data-sources/default-data/apm-default-event-attributes-insights) or [`Mobile`](/docs/insights/insights-data-sources/default-data/mobile-events-attributes), and will have associated [attributes](#attribute) (for example, `timestamp` or `transactionName`). For more details, see [Event data](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data).
    * For our infrastructure monitoring, the word **event** can be used to refer to important system and host activity. For example, a configuration change for a monitored host would be registered on Infrastructure's [**Events** UI page](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change).
    * For alerts, the [**Events** UI page](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/view-events-their-products) displays a list of alerts-related incidents for your monitored entities. Events are reported for a [violation](#alert-violation) opening and for closing.
    * In some contexts, **event** can refer to any [NRQL-queryable data type](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql#what-you-can-query). For example, when you run a NRQL query, you will see a count of [inspected events](/docs/insights/use-insights-ui/manage-account-data/rate-limits-insights#inspected-count-limits): this refers to a count of all data types queried.
  </Collapser>
  ...
  <Collapser
    ...
    title="exporter"
  >
    At New Relic, an **exporter** is a type of [integration](https://newrelic.com/integrations) that reports telemetry data to New Relic from a third-party (non-New Relic) telemetry tool. For examples, see [Exporters](/docs/data-ingest-apis/get-data-new-relic/new-relic-sdks/telemetry-sdks-send-custom-telemetry-data-new-relic#external-data), or search our [integrations](https://newrelic.com/integrations).
  </Collapser>
  ...
  <Collapser
    ...
    title="Flex"
  >
    [New Relic Flex](/docs/introduction-new-relics-flex-integration) is an application-agnostic, all-in-one [integration](/docs/integrations). With it, you can build your own integration that collects metric data from a wide variety of services, and that can instrument any app that exposes metrics over a standard protocol (HTTP, file, shell) in a standard format (for example, JSON or plain text) to the terminal.
  ...
  >
    The collection of features and tools you can use to easily analyze and troubleshoot problems across your entire software stack. For more information, read our [Introduction to Full-Stack Observability.](/docs/introduction-full-stack-observability)
  </Collapser>
  ...
  <Collapser
    ...
    title="Insights"
  >
[Insights](/docs/insights/newrelic-insights) was the name for the New Relic product that previously governed the reporting of custom events, and the ability to query and chart your New Relic data. These features are now separate and no longer related to Insights. To learn more about these features: 
...
For historical reasons, the word "Insights" is still used in some contexts. Some examples of this:

* The API key used for the Event API is still called the [Insights insert key](/docs/apis/intro-apis/new-relic-api-keys/#insights-insert-key).
* Some APM agents use Insights language in their codebase. For example, the Java agent [`custom_insights_events` configuration](/docs/agents/java-agent/configuration/java-agent-configuration-config-file/#Custom_Events).
* For customers on our [original pricing plan](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans), Insights Pro is still the product name governing custom event data ingest and retention.  
  ...
  >
    ...
    See [Integrations](https://newrelic.com/integrations).
  </Collapser>
  ...
  <Collapser
    ...
    title="metric"
  >
    A **metric** is a numeric measurement. Metric data is a broad category because there are several ways to make and report measurements. For more about how metrics are reported at New Relic, see [New Relic data types](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics).
  </Collapser>
  ...
  >
    ...
    For how to query this type of data, see [Query metric timeslice data](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/query-apm-metric-timeslice-data-nrql).
  </Collapser>
  ...
  >
    [NRQL](/docs/insights/using-nrql) is a query language, similar in form to SQL, that allows you to query the data stored in your New Relic account.
  </Collapser>
  ...
  <Collapser
    ...
    title="owner"
  >
    For accounts on our [original pricing plan](/docs/accounts/accounts-billing/new-relic-one-pricing-users/transition-guide-new-pricing-plan), this is a type of user role: the user who initially created the account. For more information, see [Users](/docs/subscriptions/users-and-roles).
  </Collapser>
  ...
  >
  New Relic organizations can have a parent/child account structure. This structure was much more important for organizations on our [original account/user model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#user-models), but is still used for some features for organizations on the New Relic One account/user model. [Learn more about account structure.](/docs/accounts/accounts-billing/account-structure/new-relic-account-structure)
  ...
  <Collapser
    ...
    title="Plugins"
  >
    [Plugins](/docs/plugins) provides an open platform to monitor critical information about your entire stack. New Relic partners, PaAS and SaAS providers, third-party vendors, and plugin users can:
  ...
  >
    ...
    New Relic offers a variety of [integrations](/docs/integrations) that allow you to report data from your services.
  </Collapser>
  ...
  <Collapser
    id="telemtry-data-platform"
    title="Telemetry Data Platform"
  >
    The collection of features and tools you can use to ingest, visualize, and alert on all your telemetry data in one place. For more information, read the [Introduction to the Telemetry Data Platform](/docs/data-ingest-apis/get-data-new-relic/getting-started/get-started-telemetry-data-platform).
  </Collapser>
  ...
  >
    ...
    * In New Relic [labels](/docs/data-analysis/user-interface-functions/labels-categories-organize-your-apps-servers), `TIER` can be used to define or classify the client-server architecture; for example, front-end and back-end tiers.
    ...
    "Tier" may sometimes be used to refer to our [Full Stack Observability pricing editions](https://newrelic.com/pricing).
  </Collapser>
  ...
  <Collapser
    ...
    title="workload"
  >
    A **workload** represents a group of [entities](#entity) that work together to provide a digital service. For more information, see [Workloads](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-one-workloads-isolate-resolve-incidents-faster).
  </Collapser>
</CollapserGroup>
