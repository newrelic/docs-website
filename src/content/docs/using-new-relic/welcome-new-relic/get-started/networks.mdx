...
The [Pixie integration](https://github.com/newrelic/newrelic-pixie-integration) runs in your Kubernetes cluster and pulls a set of curated observability data from Pixie to send it to New Relic using OpenTelemetry Protocol (OTLP).

The Pixie integration requires outbound network access to the following:

* work.withpixie.ai:443
* otlp.nr-data.net:443 (US region accounts)
* otlp.eu01.nr-data.net:443 (EU region accounts)
