---
title: Introduction to New Relic integrations
tags:
  - Instrument everything
  - Get started
translate:
  - jp
metaDescription: Learn how to access hundreds of technologies that New Relic can instrument by using our integrations.
redirects:
  - /docs/full-stack-observability/instrument-everything/get-started-new-relic-instrumentation/introduction-new-relic-integrations
  - /node/39691
  - /docs/integrations/intro-integrations/get-started/introduction-new-relic-integrations
  - /docs/plugins/plugin-developer-resources/develop-plugins/checklist-developing-plugins
  - /docs/plugin-dev/choosing-a-development-language-for-your-plugin
  - /docs/plugin-dev/checklist-for-developing-plugins
  - /docs/plugin-dev/documenting-your-plugin
  - /docs/platform/publishing-your-plugin-to-plugin-central
  - /docs/plugin-dev/publishing-your-plugin-to-plugin-central
  - /docs/plugin-dev/publishing-your-plugin
  - /docs/plugin-dev/supporting-your-plugin
  - /docs/platform/developing-plugins/structuring-your-plugin/checklist-developing-plugins
  - /docs/platform/developing-plugins/sharing-your-plugin/documenting-your-plugin
  - /docs/plugins/developing-plugins/publishing-your-plugin
  - /docs/plugins/developing-plugins/structuring-your-plugin/checklist-developing-plugins
  - /docs/plugins/developing-plugins/sharing-your-plugin/documenting-your-plugin
  - /docs/plugins/developing-plugins/sharing-your-plugin/publishing-your-plugin
  - /docs/plugins/developing-plugins/sharing-your-plugin/supporting-your-plugin
  - /docs/plugins/plugin-developer-resources/develop-plugins/create-plugin
  - /docs/plugin-dev/viewing-your-plugins-usage
  - /docs/plugin-dev/saas-plugin-example
  - /docs/plugin-dev/saas-developer-plugins
  - /docs/plugin-dev/multiple-agents-for-plugins-example
  - /docs/plugin-developer-resources/multiple-agents-for-plugins-example
  - /docs/plugin-dev/using-the-java-sdk
  - /docs/plugin-dev/using-the-net-sdk
  - /docs/plugin-dev/using-the-ruby-sdk
  - /docs/plugin-dev/writing-chef-and-puppet-installation-scripts
  - /docs/platform/developing-plugins/writing-code/saas-developer-plugins
  - /docs/plugins/developing-plugins/writing-code/writing-chef-and-puppet-installation-scripts
  - /docs/plugins/plugin-developer-resources/developer-reference/multiple-agents-plugins-example
  - /docs/plugins/developing-plugins/writing-code/using-java-sdk
  - /docs/plugins/developing-plugins/writing-code/using-ruby-sdk
  - /docs/plugins/developing-plugins/writing-code/using-net-sdk
  - /docs/plugins/developing-plugins/writing-code/use-plugin-agent-sdk
  - /docs/plugins/developing-plugins/writing-code/writing-chef-puppet-installation-scripts
  - /docs/plugins/developing-plugins/writing-code/saas-developer-plugins
  - /docs/plugins/developing-plugins/writing-code/multiple-agents-example
  - /docs/plugins/developing-plugins/sharing-your-plugin/viewing-your-plugins-usage
  - /docs/plugins/plugin-developer-resources/develop-plugins/maintain-plugin-versions
  - /docs/plugin-dev/versioning-your-plugin
  - /docs/platform/versioning-your-extension
  - /docs/platform/versioning-your-plugin
  - /docs/platform/developing-plugins/sharing-your-plugin/versioning-your-plugin
  - /docs/plugins/developing-plugins/sharing-your-plugin/versioning-your-plugin
  - /docs/plugins/plugin-developer-resources/develop-plugins/plugin-summary-metrics-dashboards
  - /docs/plugin-dev/changing-plugin-settings
  - /docs/plugin-dev/working-with-plugin-dashboards
  - /docs/platform/developing-plugins/structuring-your-plugin/working-plugin-dashboards
  - /docs/plugins/developing-plugins/structuring-your-plugin/working-plugin-dashboards
  - /docs/plugins/developing-plugins/structuring-your-plugin/work-plugin-dashboards
  - /docs/plugins/plugin-developer-resources/develop-plugins/work-plugin-dashboards
  - /docs/plugins/developing-plugins/structuring-your-plugin/creating-summary-metrics-plugins
  - /docs/plugins/developing-plugins/structuring-your-plugin/changing-plugin-settings
  - /docs/plugin-dev/creating-summary-metrics-for-plugins
  - /docs/plugins/plugin-developer-resources/develop-plugins/use-new-relic-platform-installer-npi-utility
  - /docs/plugin-dev/platform-installer
  - /docs/platform/developing-plugins/sharing-your-plugin/using-platform-installer-utility-npi
  - /docs/plugins/developing-plugins/sharing-your-plugin/using-platform-installer-utility-npi
  - /docs/plugins/developing-plugins/sharing-your-plugin/using-new-relic-platform-installer-npi-utility
  - /docs/plugins/plugin-developer-resources/developer-reference/get-plugin-data-rest-api
  - /docs/plugins/plugin-developer-resources/developer-reference/extracting-plugin-data-new-relics-rest-api
  - /docs/plugins/plugin-developer-resources/developer-reference/licenses-plugin-developers
  - /docs/plugin-dev/plugin-licenses
  - /docs/plugin-developer-resources/plugin-licenses
  - /docs/platform/plugin-developer-resources/planning-your-plugin/plugin-licenses
  - /docs/plugins/plugin-developer-resources/planning-your-plugin/plugin-licenses
  - /docs/plugins/plugin-developer-resources/planning-your-plugin/licenses-plugin-developers
  - /docs/plugins/plugin-developer-resources/developer-reference/metric-data-plugin-api
  - /docs/platform/metric-api-and-naming-reference
  - /docs/plugin-dev/metric-naming-reference
  - /docs/plugin-dev/metric-coding-examples
  - /docs/plugin-dev/metric-naming-examples
  - /docs/plugin-developer-resources/metric-naming-examples
  - /docs/plugin-dev/metric-value-reference
  - /docs/plugin-developer-resources/metric-value-reference
  - /docs/plugin-dev/metric-units-reference
  - /docs/plugin-developer-resources/metric-units-reference
  - /docs/plugin-dev/metric-data-for-the-plugin-api
  - /docs/plugin-developer-resources/metric-data-for-the-plugin-api
  - /docs/platform/plugin-developer-resources/developer-reference/metric-data-plugin-api
  - /docs/plugins/plugin-developer-resources/developer-reference/metric-naming-reference
  - /docs/plugins/plugin-developer-resources/developer-reference/metric-naming-examples
  - /docs/plugins/plugin-developer-resources/developer-reference/metric-value-reference
  - /docs/plugins/plugin-developer-resources/developer-reference/metric-units-reference
  - /docs/plugin-developer-resources/metric-naming-reference
  - /docs/plugins/plugin-developer-resources/developer-reference/parts-plugin
  - /docs/plugin-dev/the-parts-of-a-plugin
  - /docs/plugin-developer-resources/the-parts-of-a-plugin
  - /docs/platform/plugin-developer-resources/planning-your-plugin/parts-plugin
  - /docs/plugins/plugin-developer-resources/planning-your-plugin/parts-plugin
  - /docs/plugins/plugin-developer-resources/developer-reference/plan-plugin-metrics
  - /docs/plugin-dev/planning-plugin-metrics
  - /docs/plugin-developer-resources/planning-plugin-metrics
  - /docs/platform/plugin-developer-resources/planning-your-plugin/planning-plugin-metrics
  - /docs/plugins/plugin-developer-resources/planning-your-plugin/planning-plugin-metrics
  - /docs/plugins/plugin-developer-resources/planning-your-plugin/plan-plugin-metrics
  - /docs/plugins/plugin-developer-resources/developer-reference/plugin-data
  - /docs/plugin-dev/how-new-relic-works-with-plugin-data
  - /docs/plugin-developer-resources/how-new-relic-works-with-plugin-data
  - /docs/platform/plugin-developer-resources/developer-reference/how-new-relic-plugins-works-plugin-data
  - /docs/plugins/plugin-developer-resources/developer-reference/how-new-relic-plugins-works-plugin-data
  - /docs/plugins/plugin-developer-resources/developer-reference/use-plugin-api
  - /docs/plugin-dev/working-directly-with-the-plugin-api
  - /docs/plugin-developer-resources/working-directly-with-the-plugin-api
  - /docs/platform/http-api
  - /docs/plugin-dev/plugin-api-specification
  - /docs/plugin-developer-resources/plugin-api-specification
  - /docs/plugin-dev/plugin-api-responses-and-error-codes
  - /docs/plugin-developer-resources/plugin-api-responses-and-error-codes
  - /docs/platform/plugin-developer-resources/developer-reference/working-directly-plugin-api
  - /docs/plugins/plugin-developer-resources/developer-reference/plugin-api-responses-and-error-codes
  - /docs/plugins/plugin-developer-resources/developer-reference/working-directly-plugin-api
  - /docs/plugins/plugin-developer-resources/developer-reference/work-directly-plugin-api
  - /docs/plugins/plugin-developer-resources/developer-reference/plugin-api-specification
  - /docs/plugins/plugin-developer-resources/developer-reference/plugin-api-responses-error-codes
  - /docs/plugins/plugin-developer-resources/developer-reference/use-plugins-api  
  - /docs/plugins/plugins-new-relic/get-started/get-plugin-help
  - /docs/plugins/plugins-new-relic/get-started/introduction-plugin-central
  - /docs/plugins/plugin-feedback-policy
  - /docs/plugins/plugin-ratings
  - /docs/plugins/plugin-reviews
  - /docs/platform/platform-terminology
  - /docs/platform/plugin-central-terminology
  - /docs/plugins/plugin-terminology
  - /docs/plugins/welcome-to-plugin-central
  - /docs/platform/plugins-new-relic/getting-started/welcome-plugin-central
  - /docs/platform/plugins-new-relic/getting-started/plugin-terminology
  - /docs/platform/plugins-new-relic/getting-started/plugin-ratings
  - /docs/platform/plugins-new-relic/getting-started/plugin-reviews
  - /docs/platform/plugins-new-relic/getting-started/plugin-feedback-policy
  - /docs/plugins/plugins-new-relic/getting-started/plugin-terminology
  - /docs/plugins/plugins-new-relic/getting-started/welcome-plugin-central
  - /docs/plugins/plugins-new-relic/getting-started/plugins-glossary
  - /docs/plugins/plugins-new-relic/getting-started/plugin-ratings
  - /docs/plugins/plugins-new-relic/getting-started/introduction-new-relic-plugins
  - /docs/plugins/plugins-new-relic/get-started/introduction-new-relic-plugins
  - /docs/full-stack-observability/instrument-everything/instrument-core-services-applications/new-relic-plugins
  - /docs/plugins/plugins-new-relic/get-started/plugin-security
  - /docs/plugins/plugin-security
  - /docs/platform/plugins-new-relic/getting-started/plugin-security
  - /docs/plugins/plugins-new-relic/getting-started/plugin-security  
  - /docs/plugins/plugins-new-relic/install-plugins/install-plugin-central
  - /docs/plugins/installing-a-plugin
  - /docs/plugins/installing-an-npi-compatible-plugin
  - /docs/plugins/java-plugin-example
  - /docs/plugins/java-plugin-installation-example
  - /docs/plugins/plugin-installation-with-chef-and-puppet
  - /docs/plugins/ruby-plugin-installation-example
  - /docs/plugins/ruby-plugin-example
  - /docs/plugins/saas-plugin-configuration-example
  - /docs/platform/plugins-new-relic/getting-started/installing-plugin
  - /docs/plugins/plugins-new-relic/getting-started/installing-plugin
  - /docs/platform/plugins-new-relic/installing-plugins/installing-npi-compatible-plugin
  - /docs/platform/plugins-new-relic/installing-plugins/java-plugin-installation-example
  - /docs/platform/plugins-new-relic/installing-plugins/ruby-plugin-installation-example
  - /docs/platform/plugins-new-relic/installing-plugins/saas-plugin-configuration-example
  - /docs/plugins/plugins-new-relic/installing-plugins/plugin-installation-chef-and-puppet
  - /docs/plugins/plugins-new-relic/installing-plugins/download-npi
  - /docs/plugins/plugins-new-relic/installing-plugins/npi-plugin
  - /docs/plugins/plugins-new-relic/installing-plugins/installing-npi-compatible-plugin
  - /docs/plugins/plugins-new-relic/installing-plugins/installing-plugin
  - /docs/plugins/plugins-new-relic/installing-plugins/install-new-relic-plugin
  - /docs/plugins/plugins-new-relic/installing-plugins/plugin-installation-chef-puppet
  - /docs/plugins/plugins-new-relic/installing-plugins/saas-plugin-configuration-example
  - /docs/plugins/plugins-new-relic/installing-plugins/install-plugin-central
  - /docs/plugins/plugins-new-relic/install-plugins/use-plugin-central-plugin
  - /docs/plugins/deleting-a-plugin
  - /docs/plugins/using-a-plugin
  - /docs/plugins/viewing-plugin-alerts
  - /docs/plugins/viewing-the-plugin-dashboards
  - /docs/plugins/viewing-the-plugin-summary
  - /docs/platform/plugins-new-relic/using-plugins/using-plugin
  - /docs/plugins/plugins-new-relic/installing-plugins/view-plugins-alert-policies-conditions
  - /docs/plugins/plugins-new-relic/using-plugins/using-plugin
  - /docs/plugins/plugins-new-relic/using-plugins/viewing-plugins-dashboards
  - /docs/plugins/plugins-new-relic/using-plugins/viewing-plugin-alerts
  - /docs/plugins/plugins-new-relic/installing-plugins/use-plugin-central-plugin
  - /docs/plugins/plugin-developer-resources
  - /docs/plugins/plugin-developer-resources/develop-plugins
  - /docs/plugins/plugin-developer-resources/developer-reference
  - /docs/plugins/plugins-new-relic
  - /docs/plugins/plugins-new-relic/get-started
  - /docs/plugins/plugins-new-relic/install-plugins  
  - /docs/more-integrations/open-source-telemetry-integrations
---

We provide [hundreds of integrations and quickstarts](https://newrelic.com/instant-observability/) to get your data into New Relic so you can analyze the data in one place. They give you a steady flow of useful data to fix problems quickly, maintain complex systems, improve your code, and accelerate your digital transformation.

You can bring in data from hundreds of applications, frameworks, services, operating systems, and other technologies. Our integrations gather the data, and the agents send it to New Relic. The solution you need may require you to install both an integration and an agent. In some cases, you can just install our agents that contain integrations, such as our APM agents.

Whatever data you need to bring in, chances are that we have options for your environment. If you prefer to make your own solutions, we also offer tools to get you started.

## Choose what's right for you [#choose-an-integration]

We offer a wide range of solutions so you can easily collect data across your environment. You may only need one of our solutions to get the data you need, or you can choose a variety of options to capture a broader range of data types.

Go to [New Relic Instant Observability](https://newrelic.com/instant-observability/) to find solutions that fit your environment. Here is a sample of what you’ll find there:

* **Application performance monitoring (APM):** C, Go, Java, Node, .NET, PHP, Python, and Ruby
* **Mobile apps:** Android and iOS
* **Browser monitoring:** Google Chrome, Mozilla Firefox, Microsoft Internet Explorer, and Apple Safari
* **Host monitoring:** Linux and Microsoft Windows
* **Cloud platform monitoring:** Amazon Web Services (AWS), Microsoft Azure, and Google Cloud Platform (GCP)
* **Core infrastructure services:** Kubernetes, NGINX, MySQL, and more
* **Open source telemetry integrations:** Prometheus, Micrometer, OpenTelemetry, and more

## Create your own solutions [#create]

If you are looking for custom options, we have tools to help you create your own:

* Use [New Relic Flex](/docs/integrations/host-integrations/host-integrations-list/flex-integration-tool-build-your-own-integration) to create lightweight monitoring solutions using infrastructure monitoring.
* Use [New Relic Telemetry SDKs](/docs/data-ingest-apis/get-data-new-relic/new-relic-sdks/telemetry-sdks-send-custom-telemetry-data-new-relic) to build custom solutions for sending metrics, traces, and more.
* Build your own [New Relic One applications](/docs/new-relic-one/use-new-relic-one/build-new-relic-one/new-relic-one-build-your-own-custom-new-relic-one-application) that you can share with your colleagues, or edit open source applications in our [catalog](/docs/new-relic-one/use-new-relic-one/build-new-relic-one/discover-manage-applications-new-relic-one-catalog).
