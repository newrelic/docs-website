...
You created a custom New Relic Infrastructure on-host integration using the [Integrations SDK](/docs/integrations/integrations-sdk/getting-started/introduction-infrastructure-integrations-sdk), but you're not seeing data in the Infrastructure UI.
...
Your integration is reporting data, but few inventory attributes are missing.

## Solution

The following inventory attribute names are used internally by our platform. If
customer-provided integrations send inventory items containing attributes with
the same name, they will remain invisible to the user:
