...
<Callout variant="important">
  ...
  * Verify that the removal of this release is necessary and aligns with your infrastructure management strategy.
  ...
  Always proceed with caution when making invasive changes to your infrastructure, and ensure you have appropriate rollback procedures in place should anything go awry.
</Callout>

* Run `terraform plan`: After removing the Helm release resource block from your configuration, execute the `terraform plan` command. This command enables you to review the changes Terraform intends to make to your infrastructure. Carefully check the plan output to ensure that only the intended resources are marked for deletion. This step is crucial for verifying that no unintended deletions or changes occur.
