...
2. **Make the change persistent** by editing `/etc/security/limits.conf`:
   ```bash
...
1. **Update your Helm values** to increase resource limits:
   ```yaml
   # values.yaml
   agent:
     resources:
...
3. **Verify the pods restart successfully:**
   ```bash
...
1. **Reduce monitoring scope** if increasing limits isn't feasible:
...
2. **Disable specific protocol monitoring** to reduce file descriptor usage:
...
1. **Check that the error no longer appears** in the logs:
...
2. **Confirm the agent is functioning normally** by looking for:
   ```
...
- The default limit (usually 1024) may be insufficient for large-scale deployments
