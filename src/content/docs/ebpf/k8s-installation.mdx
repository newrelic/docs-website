...
You can configure eBPF Dashboard to view your TCP and DNS related-network data in the New Relic UI. You can view the dashboard :
...
<Callout variant="tip">
    ...
    The eBPF agent automatically generates entity names differently depending on the environment. In Kubernetes, names are derived from the service name for example, `mysql-database-service` or in Docker, names are a combination of the process name, its directory or container ID, and the listening port for example, `java:f4aead533895:[8080]`.
