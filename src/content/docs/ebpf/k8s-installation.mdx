...
The eBPF agent supports HTTP proxies only — `https://` as a proxy scheme isn't supported by gRPC's HTTP CONNECT implementation. Your telemetry data stays **TLS-encrypted end-to-end** between the agent and the New Relic OTLP endpoint; the proxy only sees an encrypted tunnel. The `http://` scheme refers to the connection from the agent to the proxy itself, not to your data in transit. For full setup instructions, see [Configure a proxy for the eBPF agent](/docs/ebpf/proxy-configuration).
