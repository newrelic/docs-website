...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Define a regex to match k8s service names to drop. Example `"kube-dns|otel-collector|\\bblah\\b"`(RENAMED from `dropDataServiceNameRegex` for clarity. The old name is deprecated but still supported for backward compatibility).</td>
            <td>`String`</td>
            <td>`""`</td>
        </tr>
        <tr>
