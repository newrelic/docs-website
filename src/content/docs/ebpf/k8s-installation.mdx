...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Enable network metrics reporting. When enabled, the agent collects and reports network metrics including TCP statistics(RENAMED from 'tcpStatsReporting' for clarity. The old name is deprecated but still supported for backward compatibility).</td>
            <td>`String`</td>
            <td>`true`</td>
        </tr>
    </tbody>
...
This section configure filters to drop all types of data (Network Metrics and APM data) based on config provided.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Drop all data for applications/entities that have NewRelic/OTEL APM agents running.</td>
            <td>`Boolean`</td>
            <td>`"false"`</td>
        </tr>
    </tbody>
