...
6. On the Configure the Host integration screen, enter the deployment name for the host, then click **Continue**.
7. On the Install the agent screen:

    1. To download the `newrelic-ebpf-agent.conf` configuration file, click **Download**. 
    2. Save the `newrelic-ebpf-agent.conf` in the `/etc/newrelic-ebpf-agent` directory. For more on the configuration file, refer to [host configuration file](#host-config-params).
    2. (Optional) Update the configuration file with any additional settings you want to apply and save it.
    3. To install the eBPF agent, run the following command in your terminal:
    
        ```bash
            curl -s http://nr-downloads-ohai-staging.s3-website-us-east-1.amazonaws.com/nr-ebpf-agent/scripts/install.sh | sudo bash
        ```
