...
### 3. Add log collection for complete MELT

The eBPF agent can also collect application logs and enrich them with New Relic metadata by default, without deploying a separate log forwarder. This capability is complementary and can be used with or without eBPF APM and network metrics. For more information, refer to [eBPF logs](/docs/ebpf/logs/).
...
### 4. Create a unified monitoring approach

By following the practices above, you can use the single eBPF agent to handle infrastructure, application, and log monitoring needs. This unifies your observability, allowing the agent to automatically gather critical application performance data (eBPF APM), network metrics (eBPF network metrics), and application logs (eBPF logs) to populate existing dashboards, providing deep context without code changes or restarts.

**Benefits:**
* APM insights that populate New Relic APM UI automatically.
* Network insights from the same agent.
* Metadata-enriched log collection from the same agent, with no additional configuration.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>`protocols.<protocol>.enabled`</td>
...
<DocTiles>
    ...
    <DocTile title="eBPF logs" path="/docs/ebpf/logs/">Learn how to collect application logs with the New Relic eBPF agent.</DocTile>
