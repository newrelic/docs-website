...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>`samplingLatency`, `samplingErrorRate`</td>
      <td>`p50` latency</td>
    </tr>
    <tr>
      <td>Unlinked spans (no parent)</td>
      <td>Yes, capped</td>
      <td>`max_unlinked_spans`, `unlinked_spans_error_quota_percentage`</td>
      <td>`100` per protocol, `30`% reserved for errors</td>
    </tr>
    <tr>
      <td>Logs</td>
      <td>Yes, reservoir</td>
      <td>`maxSamplesPerMinute`</td>
      <td>`10000` per minute</td>
    </tr>
  </tbody>
</table>
