...
The eBPF agent can also collect application logs and enrich them with New Relic metadata by default, without deploying a separate log forwarder. This capability runs on the same agent, so you can turn it on alongside eBPF APM and network metrics without deploying anything extra. For more information, refer to [eBPF logs](/docs/ebpf/logs/).
