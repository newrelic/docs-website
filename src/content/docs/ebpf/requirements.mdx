...
<Callout variant="important">
  ...
  Some Linux distributions allow you to run older kernel versions on newer OS releases. Even if your distribution is [listed below](#supported-linux-distributions), the eBPF agent won't function if the kernel version is lower than `5.8`. The agent also won't function on kernels newer than the supported range listed above. Kernel `v7.x` and newer aren't currently supported.
