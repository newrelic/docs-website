...
New Relic's log ingest pipeline automatically obfuscates credit card and Social Security numbers in the `message` field for all accounts, independent of PCG. For other sensitive data, or for fields other than `message`, configure PCG routing to redact it before ingestion. For example, to redact a `password` field from eBPF log messages:
...
This replaces any `password:` value in the log body with `***` before it reaches New Relic. For more redaction patterns, refer to [Redact PII](/docs/new-relic-control/pipeline-control/gateway/transform-processor#example-5-redact-pii).
