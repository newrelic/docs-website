...
Ensure [consolidated billing is set up for AWS Organizations](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/consolidated-billing.html).
...
<Steps>
  ...
  <Step>
    ...
    ```
    <Callout variant="important">
    Replace `bucket-name` with your S3 bucket name from Step 1.
    </Callout>
    4. Click **Next**.
  ...
  <Step>
    ...
    4. Click **Next**.
    5. Attach the policy created in Step 3 and click **Next**.
  ...
  ```
    <Callout variant="important">
    Set `sts:ExternalId` to the external ID from Step 6. Activate STS for regions where it's not active by default (e.g., `ap-east-1`, `eu-south-1`).
    </Callout>
  </Step>
