...
* The New Relic region and the ingest license key provided to the Lambda function or Firehose may not match. Verify and match it.
  ...
  For example, if the New Relic region is EU, provide a New Relic ingest license key from the EU region. Similarly, if the New Relic region is JP, provide a New Relic ingest license key from the JP region.
