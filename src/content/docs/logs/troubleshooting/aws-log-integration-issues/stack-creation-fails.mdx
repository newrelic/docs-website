...
Possible reasons for the fail and its solution:
...
* If you have not select an IAM role, make sure your account has enough [permissions](/docs/accounts/accounts-billing/new-relic-one-user-management/user-permissions/#logs).
* If the failure message appears as following, delete or rename the existing resources with that name, and retry.
  **  Resource type `resource-type` with identifier `resource-identifier` already exists.**
