...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="tip">
      The steps above add live archives on top of standard retention, so a log lands in standard storage first. If you have logs you never need in standard storage at all, such as compliance or audit logs, select <DNT>**Direct to live archive**</DNT> as the partition's namespace instead of <DNT>**Standard**</DNT>. Logs sent to that partition skip standard storage entirely and go straight into live archives. See [Choosing a namespace](/docs/logs/ui-data/data-partitions/#namespace) for details.
    </Callout>
  </Step>
