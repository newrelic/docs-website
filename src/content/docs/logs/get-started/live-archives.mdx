...
There are a few ways to query live archives: Lucene, NRQL, or the NerdGraph API. For all three methods, you'll need the **Live archives query** capability to query live archives.

### Query live archives with logs syntax [#query]
If you want to use logs syntax that you're familiar with outside of New Relic, we suggest querying in the <DoNotTranslate>**Logs**</DoNotTranslate> UI. 
...
You can also query your live archives data using the NerdGraph API. You must include the `{eventNamespaces: "Logging:Archive"}` in the NRQL options to specify the live archives `eventNamespace`.  
