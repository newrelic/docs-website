...
There are two ways to query live archives: with Lucene or NRQL. If you want to use logs syntax that you're familiar with outside of New Relic, we suggest querying in the <DoNotTranslate>**Logs**</DoNotTranslate> UI. To query live archives you will need the **Live archives query** capability. 
...
If you want to query using the same language you use for errors inbox or alerts, we suggest querying in NRQL. To query live archives you will need the **Live archives query** capability. 
...
You can also query your live archives data using the Nerdgraph API. To query live archives you will need the **Live archives query** capability. You must include the `{eventNamespaces: "Logging:Archive"}` in the nrql options to specify the live archives eventNamespace.  
