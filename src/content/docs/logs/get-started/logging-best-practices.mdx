...
THIS IS A TEST EDIT FOR A DEMO.
...
* To [search data partitions](/docs/logs/ui-data/data-partitions#search) in the <DoNotTranslate>**Logs**</DoNotTranslate> UI, you must select the appropriate partition(s), open the partition selector and check the partitions you want to search. If you're using NRQL, use the `FROM` clause to specify the `Log` or `Log_<partion>` to search. For example:
...
Parsing your logs at ingest is the best way to make your log data more usable by you and other users in your organization. When you parse out attributes, you can easily use them to search in the <DoNotTranslate>**Logs**</DoNotTranslate> UI and in NRQL without having to parse data at query time. This also allows you to use them easily in <InlinePopover type="alerts" /> and dashboards.
...
* Use our <DoNotTranslate>**Parsing**</DoNotTranslate> UI to test and validate your Grok rules. Using the `Paste log` option, you can paste in one of your log messages to test your Grok expression before creating and saving a permanent parsing rule.
...
* Use the <DoNotTranslate>**Parsing**</DoNotTranslate> UI to test your Grok patterns. You can paste sample logs in the <DoNotTranslate>**Parsing**</DoNotTranslate> UI to validate your Grok or Regex patterns and if they're extracting the attributes as you expect.
...
* Install the <DoNotTranslate>**Logs analysis**</DoNotTranslate> and/or <DoNotTranslate>**APM logs monitoring quickstart**</DoNotTranslate> dashboards to quickly gain more insight into your log data. To add these dashboards go to <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > Add Data > Logging > Dashboards**</DoNotTranslate>.
