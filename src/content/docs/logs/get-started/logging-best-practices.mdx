...
## Securing log data access [#securing-log-data]
...
* **Use data access control**: Implement [data access control](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control) policies to restrict which users can view specific [log partitions](/docs/logs/ui-data/data-partitions/). This is especially important when logs contain personally identifiable information (PII), security data, or business-sensitive information.

* **Be aware of data exposure risks**: Certain capabilities can bypass data access restrictions. Review [potential data exposure risks](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control#data-exposure-risks) and follow these best practices:
  * Limit who has <DNT>**Modify**</DNT> access to the <DNT>**Data partitions rules**</DNT> capability, as this allows users to reconfigure partitions and potentially access restricted data.
  * Restrict <DNT>**Modify**</DNT> access to the <DNT>**Query keys**</DNT> capability to trusted administrators only, as query keys can bypass user-level data access restrictions.
