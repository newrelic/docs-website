...
Next, we'll explain both of these features in more detail. 

### Automatic log obfuscation [#auto-obfuscation]

The automatic log obfuscation feature is enabled for all New Relic accounts, by default. This feature automatically masks number patterns for items such as credit cards or Social Security numbers. All integers, including spaces and hyphens that may be used as delimiters, are replaced with a string of `X`'s.

Numbers that appear to be a credit card (13-16 digits) are obfuscated as `XXXXXXXXXXXXXXXX`. For example:
