...
While log obfuscation helps protect sensitive data, you may also need to control which users can access specific log data. With [data access control](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control), you can restrict user access to specific [log partitions](/docs/logs/ui-data/data-partitions/).

This feature is useful when you have:

* Log data containing sensitive information that only specific teams should access.
* Multiple teams that should only view logs relevant to their services or applications.
* Compliance requirements that restrict who can access certain log data.

Data access control works alongside [role-based access control](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts). While roles control what features users can access, data access control specifies which log partitions they can view.
