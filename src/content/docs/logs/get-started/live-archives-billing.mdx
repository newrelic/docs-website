...
The live archives feature offers long-term storage of log data that you don't intend to query often, for a lower storage cost, plus an additional cost for queries against that data. Live archives is available to full platform users. The live archives features includes the following three billing factors:
