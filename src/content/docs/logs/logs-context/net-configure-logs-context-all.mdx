...
<CollapserGroup>
  ...
  This option can also be enabled by setting the environment variable `NEW_RELIC_APPLICATION_LOGGING_FORWARDING_ENABLED=true`. The environment variable value, if present, takes precedence over the config file value.
