...
Agent release 10.1.0 has this feature enabled in the agent configuration file by default. As of agent release 10.3.0 the log forwarding feature is enabled by default. 
</Callout>

All you need to do is install an agent version with log forwarding capabilities ([PHP agent 10.1.0 or higher](/docs/release-notes/agent-release-notes/php-release-notes/)), and update your configuration to enable forwarding (agent release 10.3.0 and later have forwarding enabled by default).
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      Agent release 10.3.0 and later have this feature enabled in the agent configuration file by default.
