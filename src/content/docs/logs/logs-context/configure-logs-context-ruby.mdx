...
<CollapserGroup>
  ...
  **Optional adjustments:**
  ...
  1. If you are currently sending logs using the old logs-in-context method with the `NewRelic::Agent::Logging::DecoratingFormatter`, be sure to remove that code from your [specific log forwarder](/docs/logs/forward-logs/enable-log-management-new-relic#log-forwarding) before enabling local decoration. Otherwise, your app may send double log lines. Depending on your account, this could result in double billing.
  ...
  Some attributes may be empty if the log occurred outside a transaction or if they are not applicable to your application's context. We recommend this option over the old decorating formatter, `NewRelic::Agent::Logging::DecoratingFormatter`.
