...
<CollapserGroup>
  <Collapser
    id="1st-level"
    title="First collapser"
  >
    Text for first collapser. 
    Then add the beginning and ending code for the nested collapser group, then add the closing `/Collapser` for this first collapser, and the closing `/CollapserGroup` for the first level collapser.

    <CollapserGroup>
      <Collapser
        id="2nd-level"
        title="Collapser within collapser"
      >
        Example of collapser code within a collapser.
      </Collapser>
    </CollapserGroup>
  </Collapser>
</CollapserGroup>
