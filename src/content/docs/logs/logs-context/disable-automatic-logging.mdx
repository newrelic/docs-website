...
After you disable via the New Relic UI, it may take several minutes for the agent to restart and stop automatic log forwarding.
  ...
  >
    ...
    Additional logging metrics are displayed in the **Logs** chart on the APM **Summary** page. If you disable log forwarding, these logging metrics won't be affected. To disable these metrics, you must do this via APM agent configuration (for example, [this config option for PHP](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#cfg-application_logging_metrics-enabled)).
  </Callout>
