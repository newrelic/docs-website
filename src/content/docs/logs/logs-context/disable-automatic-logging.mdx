...
Automatic logs collection is not applicable if your APM agent has [high security mode](/docs/apm/agents/manage-apm-agents/configuration/high-security-mode/) enabled.
...
* The [Data management hub](#manage-ui) in the UI
...
For organizations with a [parent/child account structure](/docs/accounts/original-accounts-billing/original-users-roles/parent-child-account-structure/), how this works can vary. More detail on parent/child account settings are below. 

#### Parent account settings [#parent-account]
...
#### Child account settings [#child-account]
...
### Manage via NerdGraph API [#manage-api]

You can use the `dataManagementUpdateFeatureSettings` [NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) query to disable APM logs. 
...
{
  actor {
    account(id: INSERT_NEW_RELIC_ACCOUNT_ID) {
      dataManagement {
        featureSettings {
          enabled
          key
...
mutation {
  dataManagementUpdateFeatureSettings(
    accountId: INSERT_NEW_RELIC_ACCOUNT_ID
    setting: {
      enabled: false
      locked: true
      featureSetting: { 
