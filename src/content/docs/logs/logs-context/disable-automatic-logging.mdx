...
After you disable via the New Relic UI, it may take several minutes for the agent to restart and stop automatic log forwarding.
  ...
  >
    ...
    Separate from APM logs in context, there are logging metrics that are displayed in the **Logs** chart on the APM **Summary** page. If you disable log forwarding, these metrics aren't affected. To disable them, you have to disable logs metrics via APM configuration (for example, [this config option for PHP](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#cfg-application_logging_metrics-enabled)).
  </Callout>
