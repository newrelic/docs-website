...
1. Use APM agent alone and disable log forwarder:
* Update to the latest version of the [APM agent](https://docs.newrelic.com/docs/new-relic-one/install-configure/update-new-relic-agent/)
...
2. Use APM agent with log forwarder (recommended only if a forwarder is already in place)
  * [Update](https://docs.newrelic.com/docs/new-relic-one/install-configure/update-new-relic-agent/) the APM agent
  * Configure the APM agent to ensure local log decoration is enabled and log forwarding is disabled.
  * Revert previous changes that were made to the application’s logging framework configuration to enable [manual logs in context](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs).
...
If [logs in context](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) has been configured, customers using our Kubernetes log forwarding [integration](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#installation) will need to revert the changes made to the application’s logging framework configuration when implementing logs in context [manually](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs). This is necessary to preserve the Kubernetes metadata enrichment performed by our integration.
...
1. [Update](https://docs.newrelic.com/docs/new-relic-one/install-configure/update-new-relic-agent/) the APM agent
2. Configure the APM agent to ensure local log decoration is enabled and log forwarding is disabled.
3. Revert previous changes that were made to the application’s logging framework configuration to enable [manual](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs) logs in context.
