...
This guide is applicable to customers who have previously configured [log forwarding](/docs/logs/forward-logs/enable-log-management-new-relic/) or [logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/); if you have not configured either feature, we recommend upgrading to the latest version of the <InlinePopover type="apm"/> agent to take advantage of [automatic log forwarding and local decoration](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/).
...
1. Update to the latest [APM agent](/docs/new-relic-one/install-configure/update-new-relic-agent/), which automatically forwards logs, and then disable the log forwarder:
