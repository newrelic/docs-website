...
This guide is applicable to customers who have previously configured [log forwarding](https://docs.newrelic.com/docs/logs/forward-logs/enable-log-management-new-relic/) or [logs in context](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/); if you have not configured either feature, we recommend upgrading to the latest version of the <InlinePopup type="apm" /> agent to take advantage of [automatic log forwarding and local decoration](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/).
...
Local decoration needs to be enabled on the <InlinePopup type="apm" /> agent to ensure your logs are contextualized. No changes need to be made to your log forwarding configuration. 
