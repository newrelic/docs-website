...
* Reports some logs metrics. These are displayed on a **Logs** chart on the APM **Summary** UI page. 
...
The logging metrics reported by APM agents are displayed on the **Logs** chart on the APM **Summary** page. These metrics are separate from the [log forwarding feature](#apm-logs-defined). To disable logging metrics, use the dedicated APM configuration (for example, [this `logging_metrics` config option for PHP](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#cfg-application_logging_metrics-enabled)).
