...
Our newer APM agent versions have both of these features enabled by default. 

You can disable one or both aspects of this feature. For reasons you might want to disable decoration or log forwarding, see [Limitations](#limitations). For example, if you want to have an APM agent add New Relic metadata, you can use the log decoration feature, but disable log forwarding and instead use another logging agent (for example, our infrastructure agent, or a third-party logging agent). 
...
When an APM agent (with exception of Node.js) decorates the log with our metadata and forwards it, it might have attributes like this in New Relic: 
...
If you require the complete log data, you can keep the New Relic log decoration in place but disable APM agent log forwarding and instead use your own log forwarding solution. For more information, see the agent-specific logs-in-context docs. 
