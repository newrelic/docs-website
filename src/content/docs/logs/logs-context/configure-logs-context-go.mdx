...
<CollapserGroup>
  ...
  >
    ...
    Using this option, your logs will be collected by the go agent, and sent to new relic with attributes added that automatically link your logs to spans, traces, and other telemetry, making it easier to troubleshoot.

    All you need to do is install an agent version with log forwarding capabilities (Go agent version 3.17.0 or higher). Then you have two options for automatic log ingestion:

    <CollapserGroup>
      <Collapser
        id="1-nrzerolog-ingestion"
        title="Zerolog Logs in Context Plugin: The easiest way to use logs in context."
      >
        The NrZerolog plugin enables automatic logs in context ingestion with the zerolog logging framework. Once your application is configured it use it, the go agent will automatically ingest any logs written in zerolog.
        ...
        2. Register the zerolog plugin, and enable the collection of metrics and log forwarding:
      ...
      >
        ...
        Note that the timestamp is a required field, and must be formatted as Unix Milliseconds.
