...
Automatic application log forwarding is now enabled by default in versions v3.20.0+ of the Go agent. Applications using these versions of the Go agent do not need to change their config options to enable automatic log forwarding.

If you're using an older version of the Go agent, modify its config to enable application log forwarding:
