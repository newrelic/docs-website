...
4. Dowload or copy the `parsers.conf` file from [this Github repository](https://github.com/newrelic/fluent-bit-package/blob/main/parsers.conf), and save it as `/var/db/newrelic-infra/newrelic-integrations/logging/parsers.conf`.
...
2. Create a `logging.yml` configuration file, and add the parameters you need. The `logging.d` directory has various `.yml.example` files you can use as a reference or starting point.
    ...
    <Callout variant="tip">
      If you install the infrastructure agent via [Add data](https://one.newrelic.com/marketplace?state=78678a7f-91c5-ca40-ac55-e6b74a50085c) in the UI, the file `logging.yml` is created automatically.
    </Callout>
...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="many-cfg-files-in-tmp"
    title="Many configuration files get created under /tmp/nr_fb_config*, or Fluent Bit is constantly restarted"
  >
   The Infrastructure Agent takes the `*.yml` files under the `logging.d` configuration directory, generates a Fluent Bit configuration file out of them (following the naming `nr_fb_config*`) and launches a Fluent Bit instance with this resulting configuration file. The Fluent Bit process will be restarted and its associated `nr_fb_config*` will be regenerated if any of the following events happen on the host:
    * The Infrastructure Agent is restarted.
    * Any of the `*.yml` files under `/etc/newrelic-infra/logging.d/*.yml` are updated (config hot-reloading capability).
    * The `hostname` of the host where the Infrastructure Agent is installed changes.
    
    On specific environments, it might happen that the `hostname` changes very frequently, which can result in Fluent Bit getting constantly restarted and the temporary directory where the Fluent Bit configuration files are generated getting filled up with `nr_fb_config*` files. 
    To solve this issue, the hostname reported by the Infrastructure Agent can be explicitly defined by setting the [`override_hostname`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname) configuration option. If the previous option did not solve your issue, try setting the [`override_hostname_short`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname-short) configuration as well. Note that by using these settings, the agent will not report the DNS name anymore.
    [Learn more](https://docs.newrelic.com/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#hostname-variables) about Hostname settings for the Infrastructure Agent.
