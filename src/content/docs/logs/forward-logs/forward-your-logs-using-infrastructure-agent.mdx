...
To solve it, do the following in this order:
    ...
    1. Define the hostname reported by the infrastructure agent by seting the [`override_hostname`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname) configuration option.
    2. Set also the [`override_hostname_short`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname-short) configuration.
  
    <Callout variant="tip">
      If you use these settings, the agent will not report the DNS name anymore.
    </Callout>
  
Check the [Hostname setttings](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#hostname-variables) for the infrastructure agent for more information.
  </Collapser>
</CollapserGroup>
