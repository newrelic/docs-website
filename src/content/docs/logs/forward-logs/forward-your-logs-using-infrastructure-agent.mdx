...
<CollapserGroup>
  ...
  >
    The Infrastructure Agent takes the `*.yml` files under the `logging.d` configuration directory, generates a Fluent Bit configuration file out of them (following the naming `nr_fb_config*`) and launches a Fluent Bit instance with this resulting configuration file. Nevertheless, this Fluent Bit process will be restarted and its associated `nr_fb_config*` regenerated if any of the following events happen:
    # The Infrastructure Agent itself is restarted
    # Any of the `*.yml` files under `/etc/newrelic-infra/logging.d/*.yml` change
    # The `hostname` of the host where the Infrastructure Agent is installed changes
    
    We've encountered several situation in which the `hostname` changes very frequently, which can result in Fluent Bit getting constantly restarting and the temporary directory where the Fluent Bit configuration files are generated getting filled up with `nr_fb_config*` files.
    
    This situation is external to the Infrastructure Agent. A workaround for this is to fix the hostname reported by the Infrastructure Agent by setting the [`override_hostname`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname) and [`override_hostname_short`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#override-hostname-short) configuration options. Note though that by using this workaround, the Infrastructure Agent will not report the DNS name anymore. 
