...
<CollapserGroup>
  ...
  >
    ...
    * The `hostname` of the host where the Infrastructure Agent is installed changes
    ...
    We've encountered several situation in which the `hostname` changes very frequently, which can result in Fluent Bit getting constantly restarted and the temporary directory where the Fluent Bit configuration files are generated getting filled up with `nr_fb_config*` files.
