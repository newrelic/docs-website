...
<CollapserGroup>
  ...
  * `The user limit on the total number of inotify watches was reached or the kernel failed to allocate a needed resource`
  ...
  The following instructions summarize how to increase these limits if you wanted to tail 10.000 log files. Also, it assumes an Infrastructure Agent installation in privileged mode and must be run using the `root` user.
  ...
  ```
  root    soft    nofile  10100  # replace root by nri-agent for non-privileged installations
  ```
