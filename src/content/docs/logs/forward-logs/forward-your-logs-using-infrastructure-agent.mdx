...
**BEGIN new text for Minerva**

**Questions for reviewers:**

1. Would it be better to keep the automated Minerva solution as a new, standalone doc in the Minerva taxonomy, and keep it out of the existing infra doc and log forwarder taxonomy? Pro: Makes a clear distinction between old vs. new. Steer people directly to the automated solution. Not confuse them about where to start. Con: Would separate docs make it more confuring for customers to know which solution to use?

2. What will the onboarding scenario consist of? Is it simply a matter of updating to the latest infra agent version? Will the infra solution support all these options (Amazon, Docker, Linux, Debian, Ubuntu, etc.) or only specific ones?

3. Can you give some examples of some good use cases? Do we need a workflow diagram rather than text? For example, Kubernetes and Pegasus changes, how they tie into using logs?

4. Is there a good test account (Demotron?) with infra installed so I can get some screenshots? What kind of telemetry would be particularly interesting for infra users?

5. Do we need to include anything about the data ingest and security considerations? (That's already addressed in the new intro doc.)

6. Anything else?

## Automated telemetry in context [#automated-tic]

As of March 2022, we offer an automated solution to forward your infrastructure host logs to New Relic. Our telemetry in context functionality gives you an easy way to troubleshoot problems across your apps, services, other entities, and data types, without having to leave the current UI page you're viewing. Best of all, there's no additional configuration required!

Ready to get started?

1. If you don't have one already, [create a New Relic account](https://newrelic.com/signup). It's free, forever.
2. Update to infrastructure monitoring agent [v. CONFIRM or higher](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes).
...
3. Ensure you have [installed the infrastructure agent](#install), version 1.11.4 or higher. (If you use our guided install in the UI, skip the following steps.)
...
Here are examples for supported on-host integrations. This option is available only for our [supported Linux platforms](#requirements).
...
Simply copy (or rename) the `elasticsearch-log.yml.example` file to `elasticsearch-log.yml` to enable automatic Elastic Search JSON formatted log parsing and shipment to New Relic. No agent restart is required.
...
Simply copy (or rename) the `nginx-log.yml.example` file to `nginx-log.yml` to enable automatic NGINX access and error log parsing and shipment to New Relic. No agent restart is required.
