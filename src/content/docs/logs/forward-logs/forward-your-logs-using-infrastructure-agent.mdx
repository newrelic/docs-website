...
<Callout variant="important">
If you are using guided install method to instrument your Docker containers, you cann't forward your logs using the same Docker image.
To forward logs from a Docker container, choose one of the following options:
- Use this [Docker image](https://hub.docker.com/r/newrelic/newrelic-fluentbit-output) as a base image to instrument your Docker containers. 
- Install an [on-host integration](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations/) instead of deploying the agent directly on the Docker containers.
