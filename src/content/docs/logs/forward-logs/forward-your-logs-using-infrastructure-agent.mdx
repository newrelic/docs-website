...
<Callout variant="important">
During guided install, log forwarding capabilities for infrastructure agent installed from docker image is not supported. 
If you want to forward logs for your docker container, use this [docker image](https://hub.docker.com/r/newrelic/newrelic-fluentbit-output) instead. 
Alternatively, for log forwarding capabilities, deploy the agent on the host instead of deploying it directly on the docker container.
</Callout>
