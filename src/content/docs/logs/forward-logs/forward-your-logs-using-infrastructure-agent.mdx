...
If you do not have a New Relic account yet, or if you prefer to follow the procedure manually, see our [tutorial to install the package manager](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-agent-linux-using-package-manager).
...
<CollapserGroup>
  ...
  **Linux**
  For `HTTP` proxies you do not need to setup any certificates. The plugin loads the system certificates and New Relic sends logs into the logging endpoint. However, you can specify the proxy self-signed certificate (PEM file) using either the `caBundleFile` or `caBundleDir` parameters.

  **Windows**
  - For `HTTP` proxies you do not need to setup any certificates. The plugin loads the system certificates.
  ...
  * Using the `caBundleFile` and `caBundleDir` parameters
  ...
  3. On Windows and systems that do not use `systemd` or where `journald` is inaccessible, `verbose:3` causes the agent to write the logs on the disk. Revert to `verbose:0` to prevent this.
  ...
  Check whether you are using the [`fluentbit`] option. When setting `verbose: 3` **and** `trace: ["log.fw"]`, ensure that you do not define any `[OUTPUT]` section pointing to `stdout` in an external Fluent Bit configuration file,
  </Callout>
