...
<CollapserGroup>
  ...
  >
  The infrastructure agent takes the `*.yml` files under the `logging.d` configuration directory, generates a Fluent Bit configuration file out of them (following the naming `nr_fb_config*`), and launches a Fluent Bit instance with this resulting configuration file. The Fluent Bit process will restart and its associated `nr_fb_config*` will be regenerated if any of the following events happen on the host:
