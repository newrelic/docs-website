...
<Callout variant="important">
To forward logs from a Docker container, use this [Docker image](https://hub.docker.com/r/newrelic/newrelic-fluentbit-output) as the base image to be used with kubernetes logs integration, or you can build your own container with custom configurations for different environments.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="pattern"
  >
    Regular expression for filtering records. Only supported for the `file`, `systemd`, `syslog`, and `tcp` (only with format `none`) sources.
...
* In the infrastructure UI, from the host table, click the <Icon name="fe-more-horizontal"/> icon for a specific host, and then click <DNT>**View logs**</DNT>.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Install the latest Fluent Bit version (Linux)"
  >
...
To disable log forwarding capabilities, go to your `logging.d` directory, and remove files with the `.yml` extension that were originally added during the [configuration](#step-1-configure-the-infrastructure-agent) process.
