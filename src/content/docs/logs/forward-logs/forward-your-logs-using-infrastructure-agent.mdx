...
Your first step is to [create a New Relic account](). After that, use this doc to set up log forwarding. 
...
Starting with version 1.11.4, the infrastructure agent forwards logs to New Relic. You can install the infrastructure agent with our guided install, or with a manual install. Our guided install is a lightweight route that installs our infrastructure agent, an APM agent, and more. This is a good choice for smaller organizations or any organization that wants to test out New Relic. 
...
Although these configuration parameter's aren't required, we still recommend you set them up so you get the most functionality out of log forwarding.
