...
While our APM agents automatically collect application-level logs, use our infrastructure agent to collect host-level logs, regardless of the of host you use.

To get started installing and configuring the infrastructure agent, we offer a few methods:

* [Guided installation in the UI](#infra-guided) (recommended)
* [Manual installation](#infra-manual)
* [Linux tarball installation](#tarball-install)

## Review the requirements

Before you begin, make sure your infrastructure agent and system meet these requirements:
...
## Automatically forward host logs with guided install [#infra-guided]

When you use our guided install to install the infrastructure agent, the log forwarding feature is automatically configured during the install process. If you haven't already, create your free New Relic account below.

<InlineSignup/>

To initiate the guided install, choose your deployment method: 
...
## Enable log forwarding on agent installed manually [#infra-manual]
...
### Step 4: (Optional) Collect Docker host logs [#docker]

If your app is running in a Docker container, be sure tail the `/var/log/containers/*.log` directory for `Docker-logs`, which tells our logging system to continuously monitor the `/var/log/containers/*.log` directory for new log entries.
...
### Step 5. View your log data [#find-data]
