...
6. Ensure that the new limits have been enforced by running:
  ...
  >
    ...
    To upgrade to a particular Fluent Bit version, just run (in the following examples, version `3.0.4` is used):
    ...
    sudo yum install fluent-bit-3.0.4-1
    ```

    DEB:

    ```bash
    sudo apt install fluent-bit=3.0.4
    ```
  </Collapser>
  <Collapser
    className="freq-link"
