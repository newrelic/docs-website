...
<CollapserGroup>
  ...
  >
    ...
    `max_line_kb` must be larger than `Buffer_Chunk_Size` of Fluent Bit to work.Therefore, `max_line_kb` must be larger than `33`.
    If you want to set a smaller value than max_line_kb, you must use fluentbit config to reduce buffer_chunk_size.
  </Collapser>
