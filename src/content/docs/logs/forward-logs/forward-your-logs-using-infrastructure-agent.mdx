...
<Callout variant="important">
  Trace logging generates a lot of data very quickly. When finished generating logs, be sure to set `level: info` (or lower) to reduce disk space consumption and data ingest when forwarding logs to New Relic.
</Callout>
    ...
    4. Sometimes you can have issues with the log forwarder itself. For example, there may be problems accessing a specific channel when shipping Windows log events or when accessing a particular log file. In these situations, you can also enable the verbose mode for the log forwarder:
       ...
       log:
         ...
         include_filters:
           traces:
             - supervisor # Required to see logs from the fluentbit itself
       ```
