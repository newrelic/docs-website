...
## Automatically forward logs with the infrastructure agent [#infra]

The infrastructure agent is what enables log forwarding to New Relic. When you install via our guided install, your infrastructure agent automatically forwards your log data without any additional configuration. While logged in to your New Relic account, choose your deployment method to initiate your install: 
...
If you don't see your deployment method, then manual installation is a good choice for you. To install manually, follow our [tutorial to install the package manager](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-agent-linux-using-package-manager), or check out our [MSI installer](/docs/infrastructure/install-configure-manage-infrastructure/windows-installation/install-infrastructure-windows-server-using-msi-installer) (Windows). 

Some [additional setup is needed](#configure) to enable log forwarding if you've implemented the infrastructure agent with Linux tarball or a Windows ZIP installation. The rest of the doc walks you through configuring your `logging.yml` file so the infrastructure agent forwards log data to New Relic.  
...
## Step 1. Configure the infrastructure agent [#configure]
