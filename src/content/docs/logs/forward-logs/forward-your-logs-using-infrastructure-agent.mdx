...
Forwarding your logs to New Relic makes all of your logging data available in one location, providing deeper visibility into both your application and your platform performance data. With your logs in one spot, you get enhanced log management capabilities that let you collect, process, explore, query, and alert on errors or anomalies found in your log data. 

Your first step is to [create a New Relic account](https://newrelic.com/signup). After that, use this doc to set up log forwarding. 
...
## Step 1. Automatically forward logs with the infrastructure agent [#infra]

The infrastructure agent enables log forwarding to New Relic, so the first step to access the feature is to choose how you want to install your agent. You can install with either our guided install or through a manual installation. 

* Guided install is a great choice if you're a smaller organization or if you want to test out New Relic. Guided install automatically configures the infrastructure agent to forward logs to New Relic. 
* Manual installation is a good choice if your deployment method isn't listed below. To install manually, follow our [tutorial to install the package manager](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-agent-linux-using-package-manager), or check out our [MSI installer](/docs/infrastructure/install-configure-manage-infrastructure/windows-installation/install-infrastructure-windows-server-using-msi-installer) (Windows). 
* Some [additional setup is needed](#configure) to enable log forwarding if you've implemented the infrastructure agent with Linux tarball or a Windows ZIP installation.  

While logged in to your New Relic account, choose your deployment method to get started with our guided install: 
...
## Step 2. Configure the infrastructure agent [#configure]
...
## Step 3. Log forwarding parameters [#parameters]
...
Although these configuration parameters aren't required, we still recommend you set them up so you get the most functionality out of log forwarding.
