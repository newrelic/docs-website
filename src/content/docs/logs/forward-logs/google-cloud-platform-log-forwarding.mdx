...
* **Using a Dataflow job.** This solution is suited for **larger log volumes**, since it groups log records in batches before sending them to New Relic. This reduces the number of API calls and allows to reduce your quota usage. Nevertheless, this option **might incur extra costs at your GCP subscription**, due to the execution of a Dataflow job in your premises.
