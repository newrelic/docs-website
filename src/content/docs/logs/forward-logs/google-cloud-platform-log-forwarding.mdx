  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * Reduces the number of API calls and allows to reduce your quota usage.
...
* [Apache Maven 3.2 or higher](https://maven.apache.org/). Earlier versions have been observed to fail during the compilation process.
...
<CollapserGroup>
  ...
  2. Enter the created directory to execute the following commands:
  ...
  To set the required configuration to compile and run the Dataflow forwarder, run the following commands. The only required values are:
  ...
  * `INPUT_SUBSCRIPTION_NAME`
  ...
  This command only requires you to specify:
  
  * The input PubSub subscription used to read log messages
  * The New Relic license key used to send your logs
  
  The command uses default configuration settings that you can further [customize as needed](#dataflow-config).
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          New Relic's Log API url. This should be routable from the VPC in which the Dataflow pipeline runs.
