...
Fluent Bit needs to know the location of the New Relic plugin and the New Relic [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key) to output data to New Relic. To configure your Fluent Bit plugin:
...
3. In the `fluent-bit.conf` file, add the following line under the `service` block:
   ...
   ```
4. At the bottom of the `fluent-bit.conf` file, add the following to set up the input, filter, and output stanzas. Replace the placeholder text with your [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key):
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The [license key](/docs/apis/intro-apis/new-relic-api-keys/#license-key). Use either `licenseKey` (**recommended**) or `apiKey`, not both. Default: `none`
      </td>
    </tr>
