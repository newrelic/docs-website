...
Forwarding your Fluent Bit logs to New Relic will give you enhanced <InlinePopover type="logs"/> capabilities to collect, process, explore, query, and alert on your log data. You can install the Fluent bit via:

  1. Kubernetes installation
  2. On-host installation
...
we've published a container with the plugin installed , serves as a base image for our Kubernetes integration. If you have your own custom kubernetes integration, we recommend you use our [docker image](https://hub.docker.com/r/newrelic/newrelic-fluentbit-output) that comes with the [New Relic’s fluent bit output plugin](https://github.com/newrelic/newrelic-fluent-bit-output) . Or you can use the docker image as a base image and layer your own custom configuration files.
