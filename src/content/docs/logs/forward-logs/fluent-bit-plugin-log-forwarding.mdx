...
Before you upgrade your Fluent Bit plugin, run the following NRQL query to find the current versions of the plugin being used in your system:

```sql
    FROM
    K8sContainerSample select latest(containerImage) where podName like '%newrelic-logging%' facet clusterName
```

<Callout variant="caution">
    Fluent Bit versions 2.0.7 through 3.0.3 are affected by a [security vulnerability (CVE-2024-4323)](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-4323). If you're using one of these versions, upgrade version 3.0.4 or higher. For more info on this, see our [security bulletin NR24-01 - Fluent Bit](https://docs.newrelic.com/docs/security/new-relic-security/security-bulletins/security-bulletin-nr24-01/).
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The <InlinePopover type="licenseKey" />. Use either `licenseKey` (<DoNotTranslate>**recommended**</DoNotTranslate>) or `apiKey`, not both. Default: `none`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DoNotTranslate>**Deprecated.**</DoNotTranslate> Takes a New Relic [Insights insert key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register), but using the `licenseKey` attribute is preferred. Use either `licenseKey` or `apiKey`, not both.
      </td>
    </tr>
