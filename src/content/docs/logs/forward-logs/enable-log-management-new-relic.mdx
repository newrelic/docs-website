...
import forwardLogs from 'images/log-forward-options4.png'
...
* If you want to collect logs that are written to a local file on disk, you can use log forwarder solutions such as our infrastructure monitoring agent, Fluent Bit, Fluentd, Logstash, and syslog/TCP. Use our [guided install for infrastructure monitoring](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent) as a lightweight data collector for your apps' and hosts' logs.
* If you want to collect logs directly from your apps, regardless of where they are hosted, you can use our APM agent and infrastructure monitoring agent. Let your app's APM agent forward log data directly to New Relic with [APM logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context). No need to install or maintain additional third-party software!
* If you want to use your cloud forwarding platform for Amazon AWS, Google Cloud Platform, Microsoft Azure, and Heroku, you can collect cloud platform service-level logs, application logs, infrastructure logs, or all of the above.
* If you want to collect logs from apps running in Docker containers (either standalone or when using Kubernetes), you can use our Kubernetes plugin or our infrastructure monitoring agent with Docker.
