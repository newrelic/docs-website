...
import kubernetesGatherLogData from 'images/kubernetes_screenshot-crop_gather-log-data.webp'
...
<CollapserGroup>
    <Collapser
        id="helm-guided-install"
        title="Use our guided install"
    >
    ...
    The best way to install this is through our [guided install](/install/kubernetes/?dropdown1=guided) process. This guided install generates the command required to install it. 

    </Collapser>
    <Collapser
        id="manual-helm-install"
        title="Manual installation using Helm"
    >

    Alternately, you can install it manually using Helm. Run this command to install the repo:
