...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="cannot-connect"
    title="Cannot connect to the <DNT>Docker/DNT> daemon"
  >
    When using Serverless to deploy the Lambda function from your machine, this error may occur:

    ```
    Error: docker: Cannot connect to the <DNT>Docker/DNT> daemon at unix:///var/run/docker.sock. Is the docker daemon running?
    ```

    This may mean that either <DNT>Docker/DNT> isn't running or that it hasn't been correctly set up. Make sure <DNT>Docker/DNT> is running on your machine when deploying the Lambda function using Serverless.
  </Collapser>
