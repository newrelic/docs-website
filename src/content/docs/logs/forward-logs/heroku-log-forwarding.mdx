...
If your log data is already being monitored by Heroku's built-in
[Logplex](https://devcenter.heroku.com/articles/logplex) router, you can use
our integration to forward and enrich your log data in New Relic.

Heroku Syslog drains on the counterpart doesn't allow you to add this custom
metadata so it's harder to differentiate between the configured drains on the
logs.

Forwarding your Heroku logs to New Relic will give you enhanced log management
capabilities to collect, process, explore, query, and alert on your log data.

We've support for
[Heroku HTTPS drains](https://devcenter.heroku.com/articles/log-drains#https-drains)
and also
[Heroku Syslog drains](https://devcenter.heroku.com/articles/log-drains#syslog-drains).

Using the Heroku HTTPS drains you'll be able to add custom metadata to your
logs, that way, you can set different attributes for each of your drains like
the Application name, or other values you may need. Also, the whole process of
registering new drains and deleting them is easier that with syslog.

## Heroku HTTPS drain [#heroku-https-drains]
...
2. Log in to [New Relic's Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) and click **Add more data sources**.
3. Under **Logging**, click the `Heroku` tile.
4. If you have multiple accounts, select the account to which you want to send the logs.
5. (Optional) Click **Add metadata** if you want to add custom metadata to the
log drain and fill up the list with the key pairs you need.
6. Click **Generate URL**. A headerless URL will be generated and shown along
the heroku cli command to setup the drain token.
7. Copy the heroku cli command to your clipboard clicking the **Copy to
clipboard**
button.
8. Go to your terminal where you've the heroku cli set.
9. Paste the command and replace `{{YOUR_APP_NAME}}` with your heroku
application name.
10. (Optional) Click **Test for logs** if you want to check if logs are reaching New Relic.

## Heroku Syslog drain [#heroku-syslog-drain]

### Create a Heroku syslog drain

We recommend you to use Heroku HTTPS drains because it's easier to setup and
it allows you to add attributes to your logs, but as this is not a need for
everyone, you can also use the Heroku Syslog drains as follows:
...
1. Log in to [New Relic's Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) and click **Add data**.
2. Click the **Heroku (syslog)** tile under **Logging**.
3. If you have multiple accounts, select the account in which you want to register the drain token.
...
1. Make sure your New Relic user account has the [admin role](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#roles) assigned to it.
2. Log in to [New Relic's Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) and click **Add data**.
3. Click the **Heroku (syslog)** tile under **Logging**.
