...
8. Go to your terminal where you've set up the Heroku CLI.
...
We suggest to use [Heroku HTTPS drains](#heroku-https-drains) whenever possible as they're simple to install, remove and also allows adding custom attributes to your logs.

### Create a Heroku Syslog drain [#create-syslog-drain]

We recommend you use Heroku HTTPS drains because they're easier to set up and they let you add attributes to your logs. Nevertheless, if you still need to use a syslog drain for some reason, here's the procedure:
