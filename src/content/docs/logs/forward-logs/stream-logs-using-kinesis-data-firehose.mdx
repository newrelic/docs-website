...
8. Set a Buffer size of `1 MiB` under `Buffer hints/Buffer size`. Note that the `Buffer hints` section is folded by default. See [this section](#buffer-settings) for a more thorough explanation of this setting.
...
When selecting the New Relic Kinesis Firehose integration for Logs in AWS, the wizard hides a small section called `Buffer hints`, at the bottom of the screen. This section contains to very important configuration options: `Buffer size` and `Buffer interval`.

![Firehose Buffer Hints](./images/firehose-buffer-hints.png "Firehose Buffer Hints")

The default values are 5 MiB for `Buffer size`, and 60 seconds for `Buffer interval`. This means that Firehose will accumulate logs until either they contain 5 MiB of data, or until 60 seconds have passed since the last time they were flushed to New Relic. These default settings are **not appropriate for New Relic**, we **strongly advise to use 1 MiB as `Buffer size`**.

The reason is the following: New Relic accepts at most [payloads of 1 MiB](https://docs.newrelic.com/docs/logs/log-api/introduction-log-api/#limits) for an individual HTTP request to its Logs API. If during a given 60 second accumulation period the size of the accumulated logs exceed 1 MiB, New Relic will reject such logs with a `413` HTTP error.
