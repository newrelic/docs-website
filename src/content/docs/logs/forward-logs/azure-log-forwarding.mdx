...
<Callout variant="tip">
  Looking to forward Azure VNet Flow Logs? See [Set up Azure VNet Flow Logs monitoring](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) for dedicated VNet flow log forwarding with incremental delta processing. ARM and Bicep deployment templates are available for automated setup.
</Callout>
...
13. Optional: Set to `true` the [Azure subscription activity logs](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) you want to forward. See [the subscription information](#subscription-activity-logs) in this document for more details.
14. Click <DNT>**Review + create**</DNT>, review the data you've inserted, and click <DNT>**Create**</DNT>.
...
* **Enterprise scaling mode**: Provides advanced scaling capabilities with dedicated compute resources and more control over instance scaling. This mode offers:
  * Autoscaling functionality for both the Function App and EventHub.
...
### Optional: Configure authentication mode [#eventhub-authentication]
...
* Creates a system-assigned managed identity on the Function App.
* Grants the identity the `Azure Event Hubs Data Receiver` role on the Event Hub namespace.
* Configures the Function App with `EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace` (the Event Hub namespace hostname) instead of a connection string. The Azure Functions runtime automatically uses Azure AD tokens for authentication.
