...
In May 2013, we included some security enhancements in our ARM templates that modify the settings in the Storage Account and the Function App deployed using those templates. Azure resources are secured by default and these changed settings prevent this secure default state from being modified.
...
<CollapserGroup>
  ...
  >
    Modifying this setting is quite complex, as it also involves creating another private-access Storage Account, moving all the file shares and blobs there, creating private endpoints for the Storage Account, create DNS records for them and modifying the network configuration of the Function App accordingly. Due to this extra complexity, New Relic recommends that customers redeploy our ARM template from scratch instead of attempting to manually modify this setting.
    ...
    Customers who cannot complete a full redeployment can find instructions on how to achieve this for an existing Storage Account in the [Microsoft docs page](https://learn.microsoft.com/en-us/azure/azure-functions/configure-networking-how-to?tabs=portal#existing-function-app), as well as in this [blog post](https://techcommunity.microsoft.com/t5/apps-on-azure-blog/secure-storage-account-linked-to-function-app-with-private/ba-p/2644772). When following the instructions, avoid adding the `WEBSITE_VNET_ROUTE_ALL` option.
  </Collapser>
  ...
  >
    ...
    2. In the left menu, navigate to the “Configuration” tab under “Settings”.
  ...
  >
    ...
    2. In the left menu, navigate to the “Networking” tab under “Settings”.
