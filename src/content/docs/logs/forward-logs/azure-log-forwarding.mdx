...
import disablePublicNetworkAccessStorageAccount from 'images/disable-public-access-storage-account.webp'
...
1. Make sure you have a <InlinePopover type="licenseKey" />.
2. Log in to [New Relic's Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) and click **Add more data sources** on the top right of the page.
...
On May 2013, we included some security enhancements around the Storage Account and the Function App that our ARM templates deploy. Azure resources are secured by default and our ARM templates performed a correctly secured setup. Nevertheless, missing these configuration settings could be potentially exploited to turn the existing setup into an insecure one.

Those users who had already deployed our ARM templates in the past are advised to modify their existing setup by:
...
Keeping customers secure is New Relic’s top priority. For any support needs related to this notice, please visit us at support.newrelic.com.

<CollapserGroup>
  <Collapser
    id="disabling-public-network-access-storage-account"
    title="Disabling public network access to the Storage account"
  >
    Modifying this is quite complex, as it involves creating another private-access Storage Account, moving all the file shares and blobs there, and modifying the network configuration of the Function App. This is why our recommendation is to redeploy our ARM template from scratch to avoid doing this rather complex setup.

    <img
     title="Disabling public network access from the Storage Account"
     alt="Disabling public network access from the Storage Account"
     src={disablePublicNetworkAccessStorageAccount}
    />

    Nevertheless, if you can’t do a clean redeployment for some reason, you can find instructions on how to achieve this for an existing Storage Account in the [Microsoft docs page](https://learn.microsoft.com/en-us/azure/azure-functions/configure-networking-how-to?tabs=portal#existing-function-app), as well as in this [blog post](https://techcommunity.microsoft.com/t5/apps-on-azure-blog/secure-storage-account-linked-to-function-app-with-private/ba-p/2644772) (you can follow it thoroughly, but skip adding the `WEBSITE_VNET_ROUTE_ALL` option).
  </Collapser>
  ...
  >
    1. Access to the Storage account that was created by the ARM template

    2. Navigate to the left menu “Configuration” tab under the “Settings” section.

    3. In the “Allow Blob public access” section, select “Disabled”:
  ...
  >
    1. Access to the Function App that was created by the ARM template.

    2. Navigate to the left menu “Configuration” tab under the “Settings” section.

    3. Select the “General settings” tab.

    4. Select “Disabled” for the “FTP state” option under “Platform settings”:
  ...
  >
    ...
    4. Select “On” for the “HTTPS Only” option under “Platform settings”:
  ...
  >
    ...
    2. Navigate to the left menu “Networking” tab under the “Settings” section.

    3. Click on “Access restriction”, inside the “Inbound Traffic” box:

    4. Uncheck the “Allow public access” checkbox under “App access”:
