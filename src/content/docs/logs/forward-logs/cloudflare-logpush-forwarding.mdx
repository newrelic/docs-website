...
redirects:
  - /docs/logs/enable-log-management-new-relic/enable-log-monitoring-new-relic/cloudflare-logpush-forwarding
---
...
7.  Enter the **New Relic Logs Endpoint**:
    ...
    Use the region that matches the one that has been set on your New Relic account. Replace `<NR_LICENSE_KEY>` in the above with your New Relic license key. can be retrieved from your New Relic account by following [these steps](docs/apis/intro-apis/new-relic-api-keys/#manage-license-key).
...
- **destination_conf** - A log destination consisting of an endpoint URL, a license key and a format in the string format below.
  ...
  - `<NR_LICENSE_KEY>`: This key can be found in your New Relic account using [these steps](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key).
...
- **max_upload_bytes** (optional) - The maximum uncompressed file size of a batch of logs. This must be at least 5 MB. Note that there is no way to set a minimum file size. This means that log files may be much smaller than this batch size. Regardless, it is recommended to set this parameter to 5,000,000.
