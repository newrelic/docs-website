...
Configure the [Cloudflare Logpush](https://developers.cloudflare.com/logs/about/) service to send logs to New Relic using the Cloudflare dashboard or via API.
...
7.  Enter the **New Relic Logs Endpoint**:
    ...
    Use the region that matches the one that has been set on your New Relic account. Replace `<NR_LICENSE_KEY>` with your New Relic license key. (You can [retrieve your license key](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key) from the UI or API.)
...
- `destination_conf`: A log destination consisting of an endpoint URL, a license key, and a format in the following string format:
  ...
  `<NR_LICENSE_KEY>`: You can [retrieve your license key](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key) from the UI or API.
