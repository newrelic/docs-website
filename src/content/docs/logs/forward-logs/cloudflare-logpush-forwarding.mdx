...
7.  Enter the **New Relic Logs Endpoint**:
    ...
    Use the region that matches the one that has been set on your New Relic account. Replace `<NR_LICENSE_KEY>` in the above with your New Relic license key. can be retrieved from your New Relic account by following [these steps](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key).
...
- **destination_conf** - A log destination consisting of an endpoint URL, a license key and a format in the string format below.
  ...
  - `<NR_LICENSE_KEY>`: This key can be found in your New Relic account using [these steps](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key).
