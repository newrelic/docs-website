...
Forwarding Kong Gateway logs to New Relic provides valuable insights into API gateway performance, health, security, and usage. This document outlines how to direct the File Log plugin to forward logs from Kong Gateway to New Relic.
...
The File Log plugin for Kong Gateway enables the forwarding of logs to New Relic by directing them to standard output (`/dev/stdout`). This setup allows the New Relic Kubernetes logs plugin integration to recognize logs from Kong Gateway, collect them, and then forward to the New Relic platform.
