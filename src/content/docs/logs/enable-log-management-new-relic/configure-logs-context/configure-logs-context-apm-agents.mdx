...
3. Add `<NewRelicLayout/>` to use a `NewRelicLayout` element within one of the appenders. For example:
   ...
   If you're using a properties file, change only the `layout.type`:
...
<CollapserGroup>
  ...
  1. Make sure you're using [Log4net 2.0.8 or higher](https://logging.apache.org/log4net/).
  ...
  ```
   ...
   Once you have configured the log4net extension and updated your logging file, you can configure your extension to send log data to New Relic. Here's an example configuration:
  ...
  1. Make sure you're using [NLog 4.5 or higher](https://nlog-project.org/).
  ...
  **Example: Don't collect MDC or the MDLC data (default)**  
   ...
   **Example: Collect MDC or the MDLC data**  
   ...
   var logger = LogManager.GetLogger("Example");
   ```
Once you have configured the NLog extension and updated your logging file, you can configure your extension to send data to New Relic. Here's an example configuration:
...
You can also configure the NLog extension with file-based configuration providers. The following example code creates a logger based on settings contained in an `App.config` file.
  ...
  1. Make sure you're using [Serilog 2.5 or higher](https://serilog.net/) and [Serilog File Sinks v4.0 or higher](https://www.nuget.org/packages/Serilog.Sinks.File/).
...
Once you have configured the Serilog extension and updated your logging file, you can configure your extension to send data to New Relic. Here's an example configuration:
