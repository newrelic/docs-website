...
* [C SDK](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/configure-logs-context-c-sdk)
* Go
* Java
* .NET
* Node.js
...
4. Configure logs in context for Java with your selected log extension. If you use Spring or Spring Boot and aren't sure which extension you need, see our Spring documentation. 
...
logging:
  ...
  **Maven:** Add the following to your `pom.xml` file:
     ...
     If you can't edit the Gradle `build.gradle` file or Maven `pom.xml` file, you can instead add the jars directly to the application classpath.
  ...
  The New Relic log format is JSON with telemetry metadata we use to correlate transactions and logs together. At this time we don't support any customization of that format.
  ...
  <Collapser
    id="java-util-logging-classpath"
    title="java.util.logging classpath"
  >

If you can't edit the Gradle `build.gradle` file or Maven `pom.xml` file, you can instead add the jars directly to the application classpath. Before you modify the classpath:

  * You must know the logging framework the application is using.
  * You must be able to make changes to your logging framework to accommodate the logs-in-context configuration.
...
Add the following three jars to the classpath if they aren't already present. Generally, we recommend taking the latest versions published on Maven Central.
  ...
  * Group ID (`com.newrelic.logging`) and Artifact ID: Select the artifact named after your application's logging framework. See the [Maven.org documentation](https://search.maven.org/search?q=g:com.newrelic.logging).
  * Group ID (`com.fasterxml.jackson.core`) and Artifact ID (`jackson-core`): See the [Maven.org documentation for jackson-core](https://search.maven.org/search?q=g:com.fasterxml.jackson.core%20a:jackson-core).
  * Group ID (`com.newrelic.agent.java`) and Artifact ID (`newrelic-api`): See the [Maven.org documentation for newrelic.api](https://search.maven.org/search?q=g:com.newrelic.agent.java%20a:newrelic-api).
