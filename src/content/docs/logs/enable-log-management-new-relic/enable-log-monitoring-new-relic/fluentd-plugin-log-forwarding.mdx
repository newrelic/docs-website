...
By default the Fluentd plugin forwards logs to New Relic's US endpoint: `https://log-api.newrelic.com/log/v1`. If your account is on New Relic's [EU datacenter](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers/) then you will need to manually set the `base_uri` property to the EU endpoint. For example:
