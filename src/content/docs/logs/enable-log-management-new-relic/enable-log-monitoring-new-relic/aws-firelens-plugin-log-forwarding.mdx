...
* New Relic [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register)
...
To prevent exposing your Insights Insert key in your task definition, we strongly recommend using the [AWS Secrets Manager](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data.html) service.
...
During configuration, outlined in [FireLens Task Definitions](https://github.com/aws-samples/amazon-ecs-firelens-examples), use the `logConfiguration` block suggested below, replacing <var>INSERT_API_KEY</var> with your New Relic [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register).
