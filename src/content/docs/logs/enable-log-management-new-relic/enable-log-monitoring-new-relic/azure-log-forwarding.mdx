...
## Azure ARM template use cases

The Azure ARM (Azure Resources Manager) template provided by New Relic could cover mainly two use cases:

1. Setup Azure to forward logs from EventHub to New Relic.
2. Setup Azure Activity Logs forwarding to New Relic through EventHub.
...
1. Login to [New Relic Logs](https://one.newrelic.com/launcher/logger.log-launcher) and click **Add more data sources** on the top right of the page.
  > ![Add more data sources button](./images/add-data-light.png)
2. Click on the **Microsoft Azure** tile under **Log ingestion**
  > ![Azure tile](./images/azure-tile.png "Azure tile")
3. If the account picker drawer appears, choose the account you want to send the logs from the selector and click **Continue**.
4. Click on the **Generate API Key** button
5. Copy the generated **API Key** to your clipboard. You can use the **Copy to clipboard** button.
6. Click on **Deploy to Azure**, a new tab will be open with the ARM template loaded in Azure.
7. Select the Resource Group in which you want to create the necessary resources.
8. Select the Region
9. Paste the previously copied **API Key** in the New Relic License Key field.
10. Ensure the New Relic endpoint set in is the one corresponding to your account.
11. **Optional step to send Azure Activity Logs** set to true the activity logs you want to forward from the list.
...
12. Click **Review + create**, review the data you've inserted, and click **Create**. That action could take a while since it
needs to create a few new resources and connect them all.
...
Or like in the New Relic Logs UI, if you want to only query for logs coming from Azure, you could execute the following query:
...
* Take a look at how the New Relic ARM template and Azure function works in the [newrelic-azure-functions](https://github.com/newrelic/newrelic-azure-functions) repository.
