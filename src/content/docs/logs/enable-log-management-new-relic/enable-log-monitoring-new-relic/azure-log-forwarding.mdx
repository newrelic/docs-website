...
By default, this template  configures the needed resources to forward logs to New Relic and the forwarding of activity logs if you set that to true when installing the template, but it doesn't configure your resources to forward logs. You need to do that manually from the resource that you want. For that you need to create a diagnostic setting for the given resource.

As an example, if you have a function running on Azure and you want to forward the logs to New Relic, you need to configure
a diagnostic setting that send the logs to the EventHub configured by this template, then the New Relic function will grab those logs from EventHub and forward them to New Relic. You can found more information about how to
...
Activate the Azure Activity Logs forwarding is an optional step but they will give you more visibility about what's happening to your Azure resources.

Those logs contain all the events occurred in Azure resources and give additional information about the event like the moment in which it happened, what user produces this event to happen (if any), and other interesting data.

This allows you to have more control over the resources, be aware of wrong or unintentional changes on your resources and even unexpected actions. You can read more about this kind of event on the [Azure Activity Log event schema](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log-schema) docs page.
