...
* OpenSSL library 1.1.0 (or higher) is a requirement for infra-agent v1.16.4 or higher. Operating systems previous year 2016 (like Suse12 or CentOS7) need to update OpenSSL from 1.0.* to 1.1.*. 
...
For Linux versions previous 2016 you may need to update OpenSSL library to 1.1.0 (or higher). To check if you have this problem. 

1. See if fluent-bit has been started by the infra-agent by doing 
```
    ps -aux | grep fluent-bit
```

2. If it is not running go to `/var/db/newrelic-infra/newrelic-integrations/logging` and run 

```
    ./fluent-bit -i systemd -o stdout
```
3. If you get this error:  `error while loading shared libraries: libssl.so.1.1: cannot open shared object file: No such file or directory`
Please update OpenSSL to 1.1.0 or higher. 
