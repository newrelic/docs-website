...
* [New Relic license key](/docs/accounts/install-new-relic/account-setup/license-key) **(recommended)** or [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register)
...
1. In your `logstash.conf` file, add the following block of data. Be sure to replace the placeholder text with your New Relic license key or Insights Insert key.
   ...
   Configure with the [New Relic license key](/docs/accounts/install-new-relic/account-setup/license-key) **(recommended)**:
   ...
   Or, configure with the New Relic [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register):

   ```
   output {
     newrelic {
       api_key => "<var>INSERT_API_KEY</var>"
     }
   }
   ```
