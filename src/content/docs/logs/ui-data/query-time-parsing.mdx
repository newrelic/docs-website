...
Here's a guide to creating query time parsing rules. The example shows how to list all the logs that contain the string `Running script`. 
...
<Steps>
    <Step>
        ...
        You can start creating a query time parsing rule by selecting an attribute value to parse.  
            ...
            1. In the log table or in the **Formatted** tab of the log details view, highlight an anchor string that contains the string you want to extract. In this case, you'd highlight `level=info msg="Running script"`. It looks like this in the logs table:
                <img
                    title="Screenshot showing a highlighted section of a log"
                    alt="Screenshot showing a highlighted section of a log"
                    src={logsExampleLogRecordtoHighlight}
                />
