...
* Table names must conform to [the rules for event names](/docs/data-apis/ingest-apis/event-api/introduction-event-api/#limits), including: 
  * Must start with an alphabetic character, and can contain alphanumeric characters, underscores (`_`) and colons (`:`) 
...
* Table data is not obfuscated. Avoid uploading personal and sensitive data.
