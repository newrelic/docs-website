...
### Ingest-time parsing methods [#ingest-time]
  ...
  * **Custom parsing rules:** When your logs are unique to your application, custom parsing rules let you define exactly which fields matter to your business.
    * **No-code parsing (UI):** Detects patterns in your log samples. Best for users who want to point-and-click to extract fields.
...
Go to <DNT>**[one.newrelic.com](https://one.newrelic.com) > Logs**</DNT> (or select any entity that has logs, such as APM, Browser, or Mobile, and navigate to <DNT>**Logs**</DNT>).

</Step>
<Step>

Click on the field you want to parse (for example, `message`) and select <DNT>**Create ingest time parsing rule**</DNT>.
...
In the in-context rule creation flow, if a filter condition was previously set, the rule matching condition will be automatically populated based on that filter. Otherwise, set a NRQL condition or paste a sample log to define which logs this rule should match.
...
Name the rule and review the <DNT>**Patterns we detected**</DNT> in the selected log sample and the rule that was authored.

</Step>
<Step>

Click into the detected patterns to see how the rule was authored and make changes to patterns configuration.
...
### Option 2: Write your own custom grok/Regex [#grok]
...
<CollapserGroup>
  ...
  <Collapser
    id="grok-patterns"
    title="Supported Grok pattern names"
  >
    New Relic supports the following Grok pattern names:
