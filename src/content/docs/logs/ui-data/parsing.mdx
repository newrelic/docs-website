...
Log <DNT>parsing</DNT> transforms unstructured log data into meaningful attributes that you can use to gain deeper insights from your logs. These attributes allow you to filter, facet, and alert on your data with precision. You can parse data at two different stages: when it first arrives (ingest-time) or when you view it (query-time).

* **Query-time parsing:** Use this for ad hoc troubleshooting and investigations. It creates temporary attributes using NRQL that exist only during query execution, making it ideal for instant analysis of existing data without waiting for new logs to flow in. Best for exploratory analysis on small datasets, one-time investigations, or when you want to extract attributes from logs already stored in NRDB. Learn more about [query-time parsing](/docs/logs/ui-data/query-time-parsing).

* **Ingest-time parsing:** Best when you have high volumes of logs that need to be queried continuously, or when you need parsed attributes for alerts, dashboards, and continuous monitoring. It creates permanent attributes using Grok and Regex patterns stored with your log data in NRDB, resulting in faster queries. Parsing happens during log ingestion, before data is written to storage. Two ways to implement ingest-time parsing:
  ...
  * **Custom parsing rules:** When your logs are unique to your application, custom parsing rules let you define exactly which fields matter to your business. Create your own parsing logic for custom or proprietary log formats using automatic pattern detection or write Grok/Regex patterns directly.
...
## How custom parsing works [#how-it-works]

Here's an overview of how New Relic implements custom ingest-time parsing:
...
## Create a rule with automatic pattern detection [#custom-parsing]
...
Choose your entry point to create a parsing rule with automatic pattern detection. The in-context approach automatically sets the field and filter based on your chosen log, while the without-context approach requires manual configuration.
  ...
  >
    Creating a parsing rule in context lets you structure your log data on the fly while troubleshooting an incident. This prevents the need to context switch when you're in the middle of an investigation and need your log data structured to speed up your analysis.
...
In the <DNT>**Create a parsing rule**</DNT> page, provide a name for the rule and review the <DNT>**Patterns we detected**</DNT> section.
...
To have more granular control over the patterns, click and drag to highlight sub-strings in <DNT>**message data**</DNT> to interact with the patterns in the following ways:

  * <DNT>**Auto detect patterns**</DNT>: Automatically detects patterns in your log samples and highlights them for easy identification. As you select and configure patterns, the parsing rule is authored automatically. Click on any detected pattern to refine its configuration.
  * <DNT>**Select text to parse**</DNT>: For more granular control, manually highlight any substring in your log sample and click <DNT>**Select to parse**</DNT>. This opens a configuration modal where you can define the pattern. After configuring, click <DNT>**Add pattern to rule**</DNT> to see the updated rule and preview output. This lets you build your parsing rule pattern by pattern.
  * <DNT>**Remove patterns**</DNT>: If a detected pattern is not relevant or is extracting unwanted data, you can remove it in two ways:
    * Click and drag to highlight the pattern, then click <DNT>**Remove selected patterns**</DNT>.
    * Click on the pattern to open its configuration modal and set <DNT>**Ignore selected string**</DNT> to <DNT>**Yes**</DNT>.
...
<CollapserGroup>
  ...
  >
    ...
    5. Review the <DNT>**Preview output**</DNT> panel to verify your pattern matches the sample logs correctly and fields will be extracted at ingest time.
    6. Click <DNT>**Save rule**</DNT> to activate immediately, or click <DNT>**Save as draft**</DNT> to activate later.
