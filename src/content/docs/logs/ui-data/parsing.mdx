...
Variable names must be explicitly set and be lowercase like `%{URI:uri}`. Expressions such as `%{URI}` or `%{URI:URI}` would not work.
...
<CollapserGroup>
  ...
  >
    ...
    2. Identify the NRQL `WHERE` clause that acts as a pre-filter for the incoming logs; for example, `entity.name='Inventory Service'`. This pre-filter narrows down the number of logs that need to be processed by your rule, removing unnecessary processing.
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              JSON structured data, take a look to [Parsing JSON mixed with plain text](#parsing-json) for more information
            </td>
          </tr>
  ...
  >
    The New Relic logs pipeline parses your log JSON messages by default, but sometimes you have JSON log messages that are mixed with plain text. In this situation, you also want to be able to parse them and then be able to filter using the JSON attributes.
  ...
  <Collapser
    ...
    title="Parsing CSV"
  >
    If your system is sending CSV logs and you need to parse them in New Relic, you can use `csv` [grok type](#grok-syntax), which will parse the CSV captured by the grok pattern.
    This format relies on 3 main parts: the grok syntax, the prefix you would like to assign to the parsed json attributes, and the `csv` [grok type](#grok-syntax). Using the `csv` [grok type](#grok-syntax), you can extract and parse CSV from logs.
    ...
    - If you want to ignore any column you can set "_" (underscore) as column name and it will be dropped from the resulting object.

    ### Optional configuration options:

    The columns configuration is mandatory but there are other optional configurations that could help you in some cases:

    - **dropOriginal**: (Defaults to `true`) Drop the CSV snippet that was used in parsing. When set to `true` (default value), the parsing rule will drop the original field.
    ...
    - **separator**: (Default to `,`) Defines the character/string that split each column.
      - Another common scenario is TSV, for that you should indicate `\t` as separator, ex. `%{GREEDYDATA:log:csv({"columns": ["timestamp", "status", "method", "url", "time", "bytes"], "separator": "\t"})`
