...
The log obfuscation UI is available as part of our [Data Plus option](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing#data-prices). 

## What is log obfuscation? [#overview]

Our [log management service](/docs/logs/get-started/get-started-log-management) automatically masks number patterns that we identify as likely being sensitive items, such as credit card or Social Security numbers. If you need additional obfuscation, one option is to manually configure the log-reporting tool (for example, configure an APM agent or infrastructure agent). 

But an easier option is to use our log obfuscation UI, available with [Data Plus](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing#data-prices). This UI lets you set up log obfuscation rules directly from the log management UI, without lengthy manual configuration. You'll define regular expressions matching your sensitive information, and then create rules to obfuscate that data. You can choose either to have sensitive information masked or hashed.
