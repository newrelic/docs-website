...
<CollapserGroup>
  ...
  >
    ...
    The examples below show regular expressions you'd use directly in the UI. Later in this document, we discuss how you could use escaped versions of these regular expressions for [NerdGraph](#rules-create).
  ...
  >
    ...
    **Using NerdGraph:**
    ...
    <Callout variant="important">
      You must escape the `regex` when introducing it via NerdGraph: ```((?:(?:4\\d{3})|(?:5[1-5]\\d{2})|6(?:011|5[0-9]{2}))(?:-?|\\040?)(?:\\d{4}(?:-?|\\040?)){3}|(?:3[4,7]\\d{2})(?:-?|\\040?)\\d{6}(?:-?|\\040?)\\d{5})```
    </Callout>
  </Collapser>
