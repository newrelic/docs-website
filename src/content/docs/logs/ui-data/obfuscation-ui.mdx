...
Our [log management service](/docs/logs/get-started/get-started-log-management) automatically masks number patterns that we identify as likely being sensitive items, such as credit card or Social Security numbers. If you need additional obfuscation, one option is to adjust the configuration of the log forwarder you use (for example, our infrastructure agent). 
