...
With log obfuscation rules, you can prevent certain types of information from being saved in New Relic. Although our [log management service](/docs/logs/get-started/get-started-log-management) automatically masks a number patterns that we identify as sensitive, such as credit card or Social Security numbers, you can add obfuscation to a wide range of sensitive data. You have the option to hash (hide) or mask (remove) information such as driver's license numbers, national IDs, biometrics, other values.
...
<CollapserGroup>
  ...
  >
    Here are some examples of regex expressions that obfuscate some of the most common sensitive data types. Note, obfuscation expressions must be created for each account where those expressions will be in use.
