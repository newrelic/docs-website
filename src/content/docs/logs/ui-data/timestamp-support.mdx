...
A log event leaves behind a timestamp attribute, which tells you the exact moment a certain event happened. Timestamps are critical for ordering events. With timestamps, you can search logs for specific timeframes, letting you answer questions about what happened and when.  

Timestamps have many formats without a single standard. In most cases, timestamps appear at the beginning of the log event, but they can sometimes appear much later in the log, or not at all.
