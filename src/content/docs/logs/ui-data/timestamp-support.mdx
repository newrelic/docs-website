...
If we receive a JSON log like this with `timestamp` in **epoch** format (with no ms precisio)n:
...
Or, if we receive a JSON log like this with timestamp in **ISO_8601** format like this:
...
- You can use the `datetime` Grok type to parse timestamps in non-supported formats.  To do this you must include the `datetime` grok type with the `pattern` matching your logs timestamp format.  The `pattern` must use Java Simple Date and Time Patterns found here.
  - Example: 
...
- To parse an ISO_8601 formatted timestamp: `%{TIMESTAMP_ISO8601:timestamp}`

Examples:

For the following log message with timestamp in ISO_8601 format:
...
For the following log message with timestamp in epoch format:
...
And for the following log message with an un-supported timestamp format:
