...
<Callout variant="caution">
  Use caution when deciding to drop data. The data you drop is not recoverable.
  Before using this feature, review the [responsibilities and considerations for
  dropping data](#caution).
</Callout>
...
![New Relic Logs - Architecture - Drop filter](src/images/ingest-pipeline.png 'New Relic Logs - Architecture - Drop filter')

<figcaption>
  During the ingestion process, customer log data can be parsed, transformed, or
  dropped before being stored in New Relic's database.
</figcaption>
...
![Log drop filters](src/images/log-drop-filter-create.png 'Log drop filters')

<figcaption>
  Filter or query the set of logs that contain the data you want to drop. Then,
  from **Manage data** on the left nav of the Logs UI, click **Create drop
  filter**.
</figcaption>
...
If you create drop filter rules for log events using NerdGraph, set the `source` attribute to `Logging`.

The logging UI only displays drop filters with the `source` attribute.
...
<Callout variant="tip">
  We recommend this method for removing fields that could contain personal
  identifiable information (PII) or other sensitive attributes without losing
  valuable monitoring data.
</Callout>

## View or delete drop filter rules [#drop-rules-delete]

![View log drop filters](src/images/log-drop-filter042721.png 'View log drop filters')
