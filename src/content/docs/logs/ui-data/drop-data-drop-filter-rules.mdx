...
5. The NRQL field will be pre populated by the query in the search bar. If the query still needs adjusted you are able to edit it here.
...
6. The NRQL field will be pre populated by with the attribute’s key and value. If the query still needs adjusted you are able to edit it here.
...
4. Give the drop rule a meaningful name.
5. Fill out the NRQL field to filter logs that contain the data to be dropped.
6. Choose to either [drop the entire log event](#drop-events) that matches the query or just a [specific subset of attributes](#drop-attributes) in the matching events.
7. Save the drop filter rule.
