...
If you are create drop filters for Log events using the NerdGraph, you will need to set the <code>source</code> attribute to <code>Logging</code>. The Logging UI will only display drop filters with that <code>source</code> attribute.
