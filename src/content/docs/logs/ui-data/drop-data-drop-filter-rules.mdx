...
To manage your drop filter rules programmatically, you can use [NerdGraph](/docs/accounts/accounts/data-management/drop-data-using-nerdgraph) to create, query, and delete your drop filter rules. While managing the drop filters using the NerdGraph, need to set the source attribute to logging for the changes to appear in the Logs UI
