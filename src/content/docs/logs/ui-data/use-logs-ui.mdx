...
import logsLogsUiExport from 'images/logs_screenshot-crop_logs-export.png'
...
## Find the logs UI [#find]

To find the New Relic logs UI, go to: **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** and click **Logs**. 

## Explore your log data [#ui-workflow]

Use the left nav in our logs UI as an easy workflow through all logs, attributes, patterns, live-tail logging, and queries. Manage your log data by dropping or parsing data, creating data partitions, and setting up alerts. Hash or mask any sensitive data in your logs with [obfuscation expressions and rules](/docs/logs/ui-data/obfuscation-ui). Get more details about specific logs and their attributes from the center nav.
...
For information on how user type governs access to New Relic features, see the [user type table](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type/#user-type-comparison-table). 

For details on how capabilities govern certain log features, see the [log-related capabilities](/docs/accounts/accounts-billing/new-relic-one-user-management/user-capabilities/#logs).  
...
## Export and share logs [#export-logs]

You can export your log data in either `CSV` or `JSON` format. This lets you share logs with people outside of New Relic. The limit on the number of exported rows is 20,000. 
...
1. Run a query in the logs UI. **Export won't work without a query being run first.**
2. Click **Export** and follow the instructions. 

## Examples of using the logs UI [#examples]
