...
Use our Logs UI at **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** or our [EU region data center](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers/) (if applicable) at **[one.eu.newrelic.com](https://one.eu.newrelic.com)** to:
...
<CollapserGroup>
  ...
  * **Getting more context:** To see logs related to other telemetry data for your apps and hosts, use [logs in context](/docs/logs/logs-context/logs-in-context).
  ...
  Use any of the [core New Relic UI functions](/docs/new-relic-one/use-new-relic-one/get-started/new-relic-one-core-ui-components) (specific account, time range, data explorer, query builder, etc.) to share the data with charts, add to dashboards, etc. For more information, see the [examples](#examples) in this document.
  </Collapser>
...
You can export your log data into either `CSV` or `JSON` format. This will allow users to easily share logs with people who may be outside of their New Relic account. The limit on the number of rows exported is 20,000. To export logs:

<img
  title="Logs UI Export"
  alt="Screenshot of Logs UI Export"
  src={logsLogsUiExport}
/>

1. Run a query in the logs UI so the result can be filtered appropriately. **Note:** this first step is required to export logs.
2. Click **Export** in the Logs UI, right button above the log results. See screenshot above.
3. A small slide-out screen will appear on the right hand side. Choose the format in the drop down menu, `CSV` or `JSON`.
4. Click the **Download** button. The logs will then download to your local machine in the chosen format.
