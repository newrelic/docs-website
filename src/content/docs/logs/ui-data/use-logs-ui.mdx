...
Take control of how you interact with your log data using the table settings.
...
Choose how the rows in your log table are displayed:
- **Condensed:** Display each log entry in a single line, allowing for rapid scanning of a large volume of logs.
- **Expanded:** View the full content of each log entry by default, providing immediate access to all details without needing to click.

### Set the Log Detail Exploration: 

Select your preferred method for examining individual log entries:
- **Side Panel:** Clicking a log row will open a convenient slide-out panel from the right, displaying the complete log details including logs in context.
...
- **Table Row:** Clicking a log row will expand it directly within the table, revealing the full log content. If the log message contains a JSON object, it will be automatically formatted and prettified for easier analysis. Additionally, a "Show log detail" button will appear on hover, providing an alternative way to open the side panel.
