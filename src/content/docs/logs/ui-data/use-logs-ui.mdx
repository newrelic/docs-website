...
* Schedule queries to run automatically and receive results via email (preview feature).
...
<CollapserGroup>
  ...
  <Collapser
    id="schedule-queries"
    title="Schedule queries to run automatically (preview)."
  >
    You can schedule NRQL queries to run automatically and receive results via email:

    1. Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Logs**.
    2. Create your query; for example, `service_name:"your service" "error"`.
    3. Click **Schedule search** from the three-dot menu or clock icon.
    4. Configure your schedule, email destination, and output format.

    For complete details, see [Schedule queries](/docs/data-insights/schedule-queries).
  </Collapser>
</CollapserGroup>
