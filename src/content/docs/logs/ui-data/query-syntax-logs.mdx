...
## See NRQL query or add to dashboard [#create-dashboard]

To see the NRQL version of your query, click **NRQL** to the right of the logs query bar. You can modify the query there, or add it to a dashboard. 

For example, you may want search Apache logs for 503 response codes, covert that to a NRQL query using `FACET`, view it as pie chart, and add that chart to a dashboard. Here's how:

1. In the **Logs** UI, search for all Apache logs with 503 response codes, using the query `logtype:apache response:503`
2. Click **NRQL**.
3. Replace the `SINCE <timestamp> UNTIL <timestamp> TIMESERIES MAX` part of the query with `FACET verb` and click **Run**. You'll see a table with response codes grouped by method(verb). 
