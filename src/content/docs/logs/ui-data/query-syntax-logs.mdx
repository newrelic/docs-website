...
Use our <DoNotTranslate>**Logs**</DoNotTranslate> UI at <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> to quickly search through your log data in seconds. Each log lists available attributes in the `log_summary` column. To drill down into additional details, click any highlighted attribute.
...
Using <DoNotTranslate>**Logs**</DoNotTranslate>, you can search through your log data by entering either simple keywords, such as `new` and `relic`, or phrases such as `"new relic agent"`, directly into the search field. 
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Exception:**</DoNotTranslate> Wildcard searches are case insensitive for attribute values.
      </td>
    </tr>
...
To see the NRQL version of a logs chart, click the ellipses menu on that chart and click <DoNotTranslate>**View query**</DoNotTranslate>. This opens up a view of the NRQL that generated that chart. You can modify the query there, or add it to a dashboard. 

To switch from a Lucene syntax query to a NRQL query, to the right of the query builder click [the <DoNotTranslate>**NRQL**</DoNotTranslate> button](/docs/logs/ui-data/use-logs-ui#workflow-search). 
...
1. From the <DoNotTranslate>**All logs**</DoNotTranslate> UI page, search for all Apache logs with 503 response codes, using the query `logtype:apache response:503`
2. From the log chart ellipses menu, click <DoNotTranslate>**View query**</DoNotTranslate>. 
3. In the query, replace `SINCE <timestamp> UNTIL <timestamp> TIMESERIES MAX` with `FACET verb` and click <DoNotTranslate>**Run**</DoNotTranslate>. You'll see a table with response codes grouped by method (verb). 
4. Change the chart type to <DoNotTranslate>**Pie**</DoNotTranslate>. 
5. Click <DoNotTranslate>**Add to dashboard**</DoNotTranslate> and complete the steps. 
