...
<Callout variant="important">
  Logs are routed to partitions during the ingestion process, before data is
  written to NRDB. Partition rules will not affect logs that were ingested
  before the rule was created.
</Callout>

### Required roles and permissions [#permissions]

Users require an [Admin role](/docs/accounts/accounts-billing/account-structure/factors-affecting-access-features-data) to create and modify partition rules.
...
![Log data partitions UI](src/images/log-partitions-crop.png 'Log data partitions UI')

<figcaption>
  From the left nav in the Logs UI, select **Data partitions**, then create a
  Log_ partition name with the retention namespace, optional description, and
  matching criteria.
</figcaption>
...
![Log data partitions rule](src/images/log-partitions.png 'Log data partitions rule')

<figcaption>
  To view a list of data partitions: From **Manage Data** on the left nav of the
  Logs UI, click **Data partitions**.
</figcaption>
