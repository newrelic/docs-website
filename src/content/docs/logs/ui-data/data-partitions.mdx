...
In order to get a sense of how many events are in a given partition simple run `FROM count(*) FROM <partition_name> SINCE <time window>`.   Note also that if you use multiple paritions in a query you will lose most of the benefits of partitioning on query performance.
