...
In addition You can understand your actual query usage and performance against a partition by analyzing the `NrDbQuery` event.  This has a variety of useful attributes including the duration of the query, the actual NRQL statement itself and the time window used in the query.

For example the following query will give you histogram of the time window sizes used in your log queries for a given partition:
