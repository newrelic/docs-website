...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Logs > Patterns**: Log patterns help you spot suspicious spikes or drops in log messages without having to spend extra time searching.
</figcaption>
...
You can create a pattern for them directly from this tab by clicking **Configure pattern**. Catching these logs early gives you the opportunity to ask developers to correct any deviations in their log output. The more consistent people are in the way log data is generated, the easier it becomes to use logs across a diverse set of teams.
