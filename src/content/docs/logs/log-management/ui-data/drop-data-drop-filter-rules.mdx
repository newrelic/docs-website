...
This creates an explicit demarcation between the logs being forwarded from your domain and the data that New Relic collects. Since the data removed by the drop filter rule does not reach our backend, it cannot be queried: the data is gone and cannot be restored.
...
<figcaption>
  **[one.newrelic.com](http://one.newrelic.com) > Logs**: Filter or query the set of logs that contain the data you want to drop. Then, from **Manage Data** on the left nav of the Logs UI, click **Create drop filter**.
</figcaption>
