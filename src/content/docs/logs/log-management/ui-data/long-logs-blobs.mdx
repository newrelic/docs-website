...
For long string value that are longer than can be stored in NRDB (4,094 characters), we split the storage of the long string into three pieces:
- The first 4,094 characters are stored in `Log` event field with the same name
  - So a long `message` value would have its first 4,094 characters stored in a `message` field.
- The next 128,000 _UTF-8 bytes_ of the string are stored in a "blob" field with the name with `newrelic.ext.` prepended
  - So a long `message` value would have characters past the first 4,094 characters stored in a `newrelic.ext.message` field as a "blob"
