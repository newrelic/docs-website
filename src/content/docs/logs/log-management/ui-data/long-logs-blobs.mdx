...
To query for extended log data stored as blob, run :

```
SELECT * FROM Log
```

![Query log extended data (blobs)](./images/long-logs-blobs.png "Query log extended data (blobs)")

<figcaption>
  To query extended blob data in your logs, be sure to include backticks in your attribute's blob syntax.
</figcaption>

To expand the blob data, run the following query, using `message` or any other attribute. For example:

```
SELECT message, <var>other-attribute</var>, blob(`newrelic.ext.message`), blob(`newrelic.ext.<var>another-attribute</var>) FROM Log
```
