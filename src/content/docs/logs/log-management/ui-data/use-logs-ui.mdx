...
Use the Logs UI at **[one.newrelic.com](https://one.newrelic.com)** or our EU region data center [(if available)](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers/) at **[one.eu.newrelic.com/](https://one.eu.newrelic.com)** to:
...
2. **Look for patterns:**
   ...
   * To spot suspicious spikes or drops in log messages, click the [log patterns](/docs/logs/log-management/ui-data/find-unusual-logs-log-patterns-beta) icon on the left nav.
...
3. **Narrow your focus:**
   ...
   * To make your log messages easier to query and understand, use our [built-in parsing rules](/docs/logs/log-management/ui-data/built-log-parsing-rulesets/), or [create your own parsing rules](/docs/logs/log-management/ui-data/parsing/) for attributes that match a particular value.
   * To manage the amount of log data collected and to store fewer logs, create [drop filter rules](/docs/logs/log-management/ui-data/drop-data-drop-filter-rules/) that avoid collecting data you don't need.
4. **Examine log details:**
   ...
   * To help troubleshoot problems related to a specific value in the log details, click the [**Show surrounding logs** <Icon name="fe-eye"/> icon](/docs/logs/log-management/troubleshooting/find-issues-cause-or-impact-surrounding-logs) for the attribute's details.
...
5. **Search:**
   ...
   For more information, see the [Logs query syntax documentation](/docs/logs/log-management/ui-data/query-syntax-logs/).

<Callout variant="tip">
  To organize data within an account and to optimize query performance, create [data partition rules](/docs/logs/log-management/ui-data/data-partitions/).
</Callout>
...
6. **Get related logs:**
   ...
   * To immediately see how your system responds to deployments or other app changes, enable [live-tail logging](/docs/logs/log-management/troubleshooting/view-log-messages-real-time-live-tail).
   * To view all the logs for a specific value, review the attributes list in the selected log's **Log details**.
   * To help identify an issue's root cause before it occurred or its impact after an event, click the [**Show surrounding logs** <Icon name="fe-eye"/> icon](/docs/logs/log-management/troubleshooting/find-issues-cause-or-impact-surrounding-logs).
...
<CollapserGroup>
  ...
  >
    To troubleshoot errors this way, you must be able to see [logs in context](/docs/enable-logs-context-logs). Then, to have a better understanding of what was happening on the host at the time an error occurred in your app:
  ...
  >
    To troubleshoot latency this way, you must be able to see [logs in context](/docs/enable-logs-context-logs). Then, to have a better understanding of how your systems were operating when performance noticeably slowed:
