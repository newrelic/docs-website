---
title: No log data appears in the UI
type: troubleshooting
tags:
  - Logs
  - Log management
  - Troubleshooting
metaDescription: "If no data appears with the New Relic Logs UI, follow these troubleshooting tips."
redirects:
  - /docs/logs/new-relic-logs/troubleshooting/no-data-appears-logs
---

## Problem

After enabling log management capabilities in New Relic, no data appears in your Logs UI after about five minutes.

## Solution

If no data appears after you send some log payloads and wait about five minutes, try the following:

<table>
  <thead>
    <tr>
      <th>
        Logs troubleshooting
      </th>

      <th>
        Comments
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Access to data
      </td>

      <td>
        See [Factors affecting access to features and data](/docs/accounts/accounts-billing/account-structure/factors-affecting-access-features-data/).
      </td>
    </tr>

    <tr>
      <td>
        Compatibility
      </td>

      <td>
        Make sure you've installed a [compatible log forwarder](https://docs.newrelic.com/docs/logs/forward-logs/enable-log-management-new-relic/#log-forwarding).
      </td>
    </tr>

    <tr>
      <td>
        Status codes
      </td>

      <td>
        Check the response status code being returned from the New Relic log collection endpoint. For example, you might see:

        ```
        HTTP Error 403: Forbidden. Review your license key.
        ```

        This error means that you're using an invalid security key. New Relic requires a [license key](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) to enable log shipping.

        An `HTTP 202` response indicates success.
      </td>
    </tr>

    <tr>
      <td>
        Errors
      </td>

      <td>
        Run a query using the [`NrIntegrationErrors` event](/docs/telemetry-data-platform/manage-data/nrintegrationerror/) to see if any errors are related to logging. For example, look for messages like:

        ```
        Error unmarshalling message payload
        ```
      </td>
    </tr>

    <tr>
      <td>
        Query `Log`
      </td>

      <td>
        Try querying the `Log` data type in New Relic One:

        ```
        SELECT * FROM Log
        ```

        If no data appears in the query builder, then no data will appear in the Logs UI. For more information, see our documentation about [data query options](https://docs.newrelic.com/docs/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data/) in New Relic.
      </td>
    </tr>
  </tbody>
</table>
