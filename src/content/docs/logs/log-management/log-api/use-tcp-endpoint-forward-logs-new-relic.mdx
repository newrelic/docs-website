...
5. Add the following to `newrelic.conf`, replacing `YOUR_INSERT_KEY` with your New Relic [Insights API Insert key](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/use-event-api-report-custom-events#register):
   ...
   $InputRunFileMonitor ## Template expected by the New Relic Syslog endpoint 
   ...
   ## Configure TLS and log forwarding 
   $DefaultNetstreamDriverCAFile /etc/ssl/certs/ca-certificates.crt 
   $ActionSendStreamDrive gtls # use gtls netstream driver 
   $ActionSendStreamDriverMode 1 # require TLS for the connection 
   $ActionSendStreamDriverAuthMode x509/name 
   $ActionSendStreamDriverPermittedPeer *.nr-data.net *.* @@syslog.nr-data.net:6514;NRLogFormat
   ```
6. Restart the `rsyslog` service by running:
...
5. Define the New Relic `syslog` format and add your New Relic [Insights API Insert key](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/use-event-api-report-custom-events#register):
   ...
   ```
   template NRLoggingTemplate { template("<var>YOUR_INSERT_KEY</var> ${PRI}>1 ${ISODATE} ${HOST:--} ${PROGRAM:--} ${PID:--} ${MSGID:--} ${SDATA:--} $MSG\n");
     template_escape(no);
   };
   ```
...
8. Restart `syslog-ng` by running:
   ...
   ```
9. [Check your New Relic account](/docs/logs/log-management/ui-data/explore-your-data-log-analytics) for logs.
