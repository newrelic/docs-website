...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Use either your [insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register) (with `Api-Key`) or [license key](/docs/accounts/install-new-relic/account-setup/license-key) (with `X-License-Key`), or take a look below to see how to send it as query string parameters.
      </td>
    </tr>
...
The Insert API key can also be passed as a query string parameter. This can be useful when sending logs from cloud-based sources that don't allow custom HTTP request headers.
