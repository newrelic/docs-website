---
title: Manage log collector lifecycle for hosts
...
- Gather and process logs from all your host applications.
...
## Set up [setup]
To install the Fluent Bit agent on hosts:
- Install the New Relic on-host integration?. This integration includes the on-host plugin for logs.
- Use the configuration options available in ? during the guided install?.

For details, refer to Enable hosts for log management?.
...
5. In the Create configuration dialog box, select **Host** as the environment, select **Fluent Bit** as the agent type, and enter a name for the configuration. Then, click **Continue**.
