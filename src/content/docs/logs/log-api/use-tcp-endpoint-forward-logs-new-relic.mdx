...
4. Check whether the directories under `/etc/rsyslog.d/` will include the newly created `newrelic.conf`. If not, explicitly include `newrelic.conf` in the `/etc/rsyslog.d/` directory:
