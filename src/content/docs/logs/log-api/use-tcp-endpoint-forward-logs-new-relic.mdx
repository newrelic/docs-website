...
6. (Optional): Add custom attributes to your `syslog` data.
 ...
 To make your `syslog` data consistent with the rest of your telemetry data, you can add custom attributes. There are two ways to do this:
   ...
   * <DoNotTranslate>**Add custom attributes directly in the template string.**</DoNotTranslate> You'll need to escape the double quotes (`"`) for the string values. For example:
   ...
   * <DoNotTranslate>**Add custom attributes using a custom property variable in the template string.**</DoNotTranslate> This method avoids the need to escape string values.
