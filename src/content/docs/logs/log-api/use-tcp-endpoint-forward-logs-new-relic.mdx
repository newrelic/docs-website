...
5. Add the following to `newrelic.conf`. Replace the highlighted values. For YOUR_LICENSE_KEY, use your New Relic <InlinePopover type="licenseKey" />:
   ...
   ```
6. Optional: Add custom attributes to your syslog data. Replace the `%structured-data%` section in the template string with custom tags/attributes. This can accomplished in two ways.
   a. Add custom attributes directly in the template string. Take note to escape the double quotes `"` for the string values. This allows for custom tagging with values related to the operating environment:
