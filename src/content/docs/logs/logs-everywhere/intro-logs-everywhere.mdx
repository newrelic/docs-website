...
<Callout variant="caution" title="Question to Reviewer">
I'm not convinced I really understand what is going on here and how to take advantage of the chart data that's easily available from Explorer. Please edit ruthlessly! Also, I will need to replace these screenshots with a test account like Demotron. I just wanted to get a sense of how this worked, thanks to a permalink to from Skyler H.
</Callout>

To see the related logs, use the Explorer in New Relic One. Here is an example:

With logs in context, your app's **Summary** page shows a chart with related logs. The UI shows the number of logs faceted by the level of severity for the app. These log trends and totals are the golden metrics that can quickly expose if a problem can be detected in logs or solvable by viewing detailed log entries.

<Callout variant="caution" title="Question to Reviewer">
The following screenshot, which shows a logs chart in the middle of the APM summary page, is from 11-22-21. Does this view still exist? I can't find it in Demotron or other accounts as of 12-29-21. Thanks for any clues!
</Callout>
...
![Logs for an app's service](./images/demotron-services-logs-122921.png "Logs for an app's service")
...
![App's logs in context in APM Summary](./images/logs-demotron-122921.png "App's logs in context in APM Summary")

## Example: See related logs for host [#example-host]

To view logs for hosts, you can stay on your host's UI page to get context immediately.

1. From the New Relic One **Explorer**, select **Hosts**, then select a host.
2. Select **Logs**.

![Host's logs in context in infrastructure hosts](./images/demotron-host-logs-122921.png "Host's logs in context in infrastructure hosts")

<Callout variant="caution" title="Question to Reviewer">
Can you please provide a permalink to a host (Demotron?) that shows "logs everywhere" or "logs in Explorer" in action? Not sure what this is currently called, and the screenshot in this section (above) is the only thing I could find in Demotron as of 12-29-21. I'm not sure what they will be seeing and the value they get from it. Thanks!
</Callout>
...
For example, you can add host metadata to your APM log.

<Callout variant="caution" title="Question to Reviewer">
Is this right, or do I have it backward, that you want to add APM metadata to your host log?
</Callout>
...
<Callout variant="caution" title="Question to Reviewer">
What do they with their logging.d config file?
</Callout>
...
<Callout variant="caution" title="Question to Reviewer">
The Fluent Bit doc already has a section about [associating logs with a specific entity](/docs/logs/forward-logs/fluent-bit-plugin-log-forwarding/#associate-logs-entities) by adding a `FILTER` block. What would they do differently for logs everywhere?
</Callout>
...
<Callout variant="caution" title="Question to Reviewer">
What's next? Is there special setup required, or will it automatically be presented in the UI?
</Callout>
