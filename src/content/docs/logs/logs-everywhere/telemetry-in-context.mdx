...
Our telemetry in context functionality gives you an easy way to troubleshoot problems across your apps, services, other entities, and data types, without having to leave the current UI page you're viewing.
...
**REVIEWER: These bullets came from several different project docs. Not sure if they are all saying the same thing, just in different ways. :-) Are all these bullets needed to get customers excited to take action?**

If you're troubleshooting an error with your app, you need to easily see related network activity, Kubernetes pod health, cloud provider information, database storage, load balancer resources, logs, transaction data, spans and traces, and more, all in the context of the APM error. With telemetry in context, now your logs are:

* **Relevant.** See the counts and percentages of logs associated with your apps, hosts, and other related entities for a specific timeframe, and quickly gain context at that point in time. Navigate within multiple types of telemetry data and have them correlate back to the original issue.
* **Precise.** Cut through the noise of thousands of logs when troubleshooting time-critical issues, so you automatically see only the most relevant logs.
...
* **Patterns that might have been overlooked.** See highly correlated log data related to your errors, traces, and infrastructure, so that you can understand if there's a signal outside of telemetry data that's relevant to the problem.
...
* **Actionable.** Easily share relevant data with other teams or tools, without losing context.
