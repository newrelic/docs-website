...
To [review your data limits in the UI](/docs/data-apis/manage-data/view-system-limits): From the [account dropdown](/docs/glossary/glossary/#account-dropdown) in the New Relic One UI, select **Manage your data > Limits**.

See the [response time example](#response-time-example) in this document. It describes a situation where an engineering team needs to temporarily increase their log data collection to troubleshoot the root cause behind a sudden increase in errors and latency for their app. The workflow includes resetting the default data collection level after the problem is resolved.
