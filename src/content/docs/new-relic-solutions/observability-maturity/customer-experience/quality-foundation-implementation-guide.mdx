...
WHERE http.statusCode >= 200 AND <Ajax Request filter>
...
FROM AjaxRequest 
SELECT percentage(count(*), WHERE http.statusCode >= 400) 
WHERE http.statusCode >= 200 AND pageUrl=<problematic page> AND appName = <corresponding app> <Ajax Request filter> 
