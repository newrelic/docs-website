...
---
This guide walks you through improving and optimizing the quality of your alerting. It's part of our [series on observability maturity](/docs/new-relic-solutions/observability-maturity/introduction).  It has been updated to reflect the changed alerting architecture and terminology.
...
This results in slower incident response and increased scope and severity when true business impacting issues occur.
...
You should measure the value of an incident by the amount of attention it receives. Engagement is measured by whether or not an incident has been acknowledged.
...
Note: Prior releases of this implementation guide relied on a custom event (**nrAQMIncident**) which was generated by a webhook to collect the required KPIs.  That dependency has changed. AQM now uses the default **NrAiIncident** event type instead.  See the [migration guide](#migration-guide) section for more details.
