...
* The business costs resulting from a suboptimal diagnostic process, and the benefits of optimizing that process.  
...
* The difference between the *problem* and the *underlying root cause* of that problem.  
* The basic diagnostic steps for finding and solving problems, which includes: 
  * Creating a problem statement
  * Finding the source of the problem
  * Finding the change in that source that caused the problem 
...
### Example problem example [#example-problem]
...
### Another problem example [#example-problem-2]
...
## Identifying pattern anomalies [#pattern-anomalies]
...
Once you've identified a negative change in the recent behavior patterns of your system, you can then trace that behavior to its source. Remember that [the problem matrix](#problem-matrix) can be helpful for determining the source. In most cases the problem is output performance in latency, or errors impacting the user experience. Often, the question is if the endpoint API application (the first application receiving the request) is causing the problem or if it's a dependency behind that endpoint.

Next, we'll walk through how you'd find the source in New Relic. Note that this workflow relies on these New Relic features: 

* [APM](/docs/apm/new-relic-apm/getting-started/introduction-apm), including [APM logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)
* [Distributed tracing](/docs/distributed-tracing/concepts/introduction-distributed-tracing)

APM and distributed tracing are used to help you **find the source**, while logs in context can then be used to help you **debug the problem**. 
...
First, we want to find the application causing the latency or errors experienced by the end user. This doesn't mean the application code is the problem, but finding that application leads us closer to the source. You can then rule out components starting with code, database, configuration, resources, and similar.

We must first identify the source transaction that's at fault. This means we must first look at APM transactions to identify if one, some, or all the transactions are affected by latency. 
...
Once the impacted transactions are identified, you'd use distributed tracing to review the traces for that transaction. Those traces give you a more complete and holistic view of the end-to-end services within that transaction group, which helps you quickly identify where the latency is or where the errors are occurring.

One you have found the transaction group impacted, you can use APM logs-in-context to dig down into what the logs are telling you about that problem, which will help you figure out how to resolve the incident. 

Here's a brief summary of the finding-the-source steps:

1. Examine the applications tied to the impacted performance.
2. Identify if the problem is latency or errors.
3. Trace the latency or error to the source application.
4. Find the problem.

## Finding the change [#find-change]

[PLACHOLDER: DO WE WANT TO ADD A SECTION ON FINDING THE CHANGE? IF WE DO, WE CAN MOVE SOME OF THE CONTENT FROM ABOVE, INCLUDING LOGS IN CONTEXT STUFF, DOWN HERE]
