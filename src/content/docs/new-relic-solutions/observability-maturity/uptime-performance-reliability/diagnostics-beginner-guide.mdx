---
title: "Reliability engineering diagnostics: a guide to troubleshooting application performance"
tags:
...
This guide an introduction to improving your skill at diagnosing customer-impacting issues. You'll be able to recover from application performance issues more quickly by following the procedures in this guide. 

This guide is part of our [series on observability maturity](/docs/new-relic-solutions/observability-maturity/introduction).

## Prerequisites

To complete this guide, you'll need to have certain New Relic solutions enabled.  

* New Relic observability coverage: 
  * **Required** : [APM with distributed tracing](/docs/apm/apm-ui-pages/monitoring/apm-summary-page-view-transaction-apdex-usage-data), [logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context), and [infrastructure monitoring](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring)
...
Time-to-resolve is often referred to as MTTR (mean-time to restore/repair/resolve). Time-to-repair is the time it takes the impact to be resolved, with resolution representing the product-tier service levels returning to expected levels. This is measured from the start of the performance degradation, as measured by service levels, and the time the service level returns to the expected level of performance.

**Note**: Time-to-resolve does not mean the root cause has been identified and permanently fixed. Permanent fixes are part of the "problem management" process post incident management. Please do your research on root cause versus direct cause and "symptoms of root causes."
...
* **Improve time to know.** Our guides on that: 
...
* **Improve time to understand and resolve.** Our guide on that:
  * Reliability engineering diagnostics (this guide)
...
### Availability, a.k.a "It doesn't work"
...
### Performance, a.k.a "It's too slow"
...
### Step 1: Create your problem statement [#create-problem-statement]
...
In IT there are three primary breakpoints sources: **Output**, **Input**, and **Client**. Measuring these categories is covered in [our service level management guide](/docs/new-relic-solutions/observability-maturity/uptime-performance-reliability/optimize-slm-guide). To understand how to use them in diagnostics, keep reading.

### Step 3: Find the change [#find-change]
...
* __Category__: Output performance
* __Source__: login middleware
* __Problem__: TCP queue times from additional request load
* __Solution__: increased TCP connection limit and scaled resources
* __Root-cause__: insufficient capacity planning and quality assurance testing on downstream service impacting login middleware
...
* __Category__: Output performance degredation leading to Input performance failure
* __Source__: middleware service calling database
* __Problem__: 10x increased database queries after code deployment
* __Solution__: deployment rollback
* __Root-cause__: insufficient quality assurance testing
...
3. Trace the latency or error to the source application.
4. Find the problem.
...
1. Use service level management to measure product level health data points on capabilities.
2. Use [operational key performance indicators](#operational-kpis) to measure success.
3. Use performance patterns to help identify the abnormal behavior in errors, latency or throuput.
3. Define [clear problem statements](#diagnostic-steps).
4. Find the source application affecting latency or errors.
5. Determine the change that is impacting latency or errors. That change is the direct-cause also known as the "problem".

Go forth an be a great SRE!
