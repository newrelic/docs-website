...
Find an application related to the latency or errors experienced by the end user. This doesn't mean the application or the code is the problem, but finding any application within the flow (*first*) leads you closer to the source - faster. Once this application is found you more quickly rule out components like code, host, database, configuration, network, etc..

Once the application is identified, the question is where is what transactions within that application are part of the problem. Use the application you identified as experiencing a performance issue and identify which transaction or transactions are impacted. Here you may repeat the performance pattern anomoly skill described above in [Identifying performance pattern anomalies](#pattern-anomalies), but now on the transactions themselves.

The folling resources will help you learn how to use the platform to identify problem transactions:

1. [Docs: Transactions page: Find specific performance problems](/docs/apm/apm-ui-pages/monitoring/transactions-page-find-specific-performance-problems/)
2. [Docs: Slow transactions on the service summary page](/whats-new/2021/03/slow-transactions/)
...
1. [Docs: Intro to Distributed tracing](/docs/distributed-tracing/concepts/introduction-distributed-tracing)
2. [Distributed tracing documentation "how to"](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/)
3. [Distributed tracing free online webinars](https://learn.newrelic.com/new-relic-distributed-tracing-tracking-across-your-application-stacks)
4. [Direct Cause Analysis with Distributed Tracing](https://www.youtube.com/watch?v=r9ImAQ5J5h4)
...
Success issues error messages can also be seen in traces; however, the best details for success problems are usually found in the application logs.

Either way, if you are the first tier incident responder or an SRE - finding the direct cause would fall to the subject matter experts (SMEs) who are typically the developers and engineers responsible for the source component found.

The most effective next step after discovering the source component is to contact the subject matter experts for said compenent. Show them the data discovered in the triage and diagnostics you've completed for a head start on the troubleshooting.
