...
<img
  title="Set the service level type"
  alt="Set type"
  src={etoSetServiceLevelTypeMobileCrash}
/>
* For Step 4, add a tag to the service level. You can use the default tag `category:success`.
...
<img
  title="Set the service level type"
  alt="Set type"
  src={etoSetServiceLevelTypeMobileRequestError}
/>
* For Step 4, add a tag to the service level. You can use the default tag `category:success`.
  </Collapser>
...
### Use CodeStream [#codestream]
Many of your error groups are going to require code changes to resolve. [Integrate CodeStream with New Relic](/docs/codestream/start-here/what-is-codestream) to allow you to open the offending code directly in your IDE to investigate the code directly. You can also leave notes and comments on specific lines of code for developers to review and vice versa. 

The CodeStream integration gives you more context on your error groups, such as being able to see version numbers or commit SHAs. Additionally, using errors inbox as a centralized place to identify, discuss, and rectify code issues allows you to respond to code problems efficiently and avoid duplicating work.
