...
### 4) Create your service level
  ...
  **NOTE:** Service levels that use different datasets should **not** be combined. The easy way to understand this is only combine service levels that use the same dataset and same count of valid transactions. For example, Latency and Success both use `Transactions` and are compared against the same quantity of valid transactions. Synthetic checks use a different dataset; therefore, it should not be combined with `Transaction`-based service levels.
