---
title: Release quality management use case implementation guide 
tags:
...
---
## Overview [#overview]
A development team is ultimately measured by the frequency and success of their releases.  Teams that release too slowly will not be able to keep pace with business demands and innovation.  Teams that create too many unsuccessful releases will have a negative impact on customer satisfaction, revenue, and overall system stability.

The DevOps Research and Assessment (DORA) team has identified four key metrics that indicate the performance of a software development organization.  OMA’s Innovation and Growth value driver uses those metrics to create an overall program that drives more efficient and responsive development teams, along with more reliable and performant applications. The Release Quality (RelQual) use case contributes to that program by driving deployment frequency, application performance, and application reliablity.

## Desired outcome [#desired-outcome]
You will use RelQual’s processes to increase the frequency of deployments while reducing their size and scope.  This will result in a faster time for an idea to be implemented in production code, while also reducing the impact (or blast radius) of software defects.
...
RelQual’s overall goal is to create a continuous cycle of performance improvement and to drive a pattern of more releases with fewer changes.  By doing this, you will reduce the risks to business reputation and earnings and increase application performance and reliability.

## Key Concepts

### Communicate, Remediate, Innovate
One of the central themes of OMA is "Communicate, Remediate, Innovate".  RelQual supports that theme by enabling you to communicate the current state of your development practices and applications to stakeholders using specific KPIs.  You will then use those KPIs to adjust your development practices and to identfy slow and unreliable application components so they can be fixed in subsequent development sprints.  Finally, you will use those KPIs to acclerate your development practices, thus adding additional time to innovate.

### Trunk Based Development
Trunk based development is a practice identified by the DevOps Research and Assessment (DORA) organization as a key capability that drives faster delivery and higher organizational performance.  It is a required practice for CI/CD.

In short, trunk based development divides development work into small batches which are performed against branches of a single trunk.  As soon as the batch of work is completed, the branch is merged back into the trunk. Each branch is expected to have a short lifetime, thus making merges back into the truck simple and ensuring every developer is working from a recent release of the code base.
...
### IT Service Boundary
The RelQual use case works at the level of the IT service boundary.  By measuring the service at the boundary, we get a picture of what is happening upstream from it.  As you may have seen, OMA’s SLM use case uses the service boundary concept to measure the response time and error rate of the service and its dependencies.  RelQual uses the same concept to measure the impact that your development practices have on the service and then to improve your development team’s responsiveness, ability to innovate, and application stability as the IT service evolves.
...
<CollapserGroup>
  ...
  >
    ...
    - The release's unique identiifer (i.e. SHA signature, version number, etc).
  ...
  <Collapser
      id="kpi-release-size-scope KPI"
      title="Release size / scope"
  >
  ...
  >
    App responsiveness and errors measures the release's 95 percentile response time and volume of errors.  
    
    You should use observability instrumentation collect this KPI for every transaction at the application's service boundary.  The KPI's attributes should, at minimum, include the following:
  ...
  >
    Production impact measures the number of failed releases (i.e. releases that were rolled back or failed to deploy)
  ...
  >
    ...
    **Best practices:**  
    ...
    - Analyze support tickets to undertand the kinds of issues encountered and use that info to drive continuous improvement.
...
Your baseline collection and evaluation cycle should be aligned with your Agile sprints and you should periodically ensure that DevQual event data is accumulating as expected.
...
1. Review your release KPIs and trends - You will review the release rate and release size/scope KPIs to ensure you are making progress towards implementing trunk based development.  Your goal is to increase your release rate while reducing the size/scope of new releases.

1. Review your application KPIs and trends - Here, you will review your application's performance and error KPIs to identify and prioritize your efforts towards improving application reliability and performance.

1. Make technical recommendations - Here, you and the relevant stakeholders will identify and review technical recommendations, such as making changes to your release workflows or observability strategies.
...
After you are on the path to those goals, you should move along to the [release cadence](#release-cadence) and [development quality](#dev-qual) use cases.
