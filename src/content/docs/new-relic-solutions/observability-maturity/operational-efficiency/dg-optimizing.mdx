...
Data volume will also vary based on whether you are using [infinite tracing](https://docs.newrelic.com/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing/) or not.
...
`lowDataMode` affects three specific components of the `nri-bundle` chart: 1) Increase Infrastructure agent interval from `15` to `30` seconds. 2) Prometheus OpenMetrics Integration will exclude a few metrics as below doc detailed. 3) Labels and Annotations details will be dropped from Logs.

More details about this configuration can be found in the below doc:
