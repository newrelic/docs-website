---
title: "Service characterization: a guide to establishing optimal instrumentation and telemetry" 
tags:
...
This guide walks you through determining if you have the instrumentation and telemetry data you need for monitoring and improving your digital properties. It's part of our [series on observability maturity](/docs/new-relic-solutions/observability-maturity/introduction). 

## Overview [#overview]

**"Do I have all the telemetry I need to adequately measure my service?"**

The process of onboarding a service for production monitoring tends to start from the end and work its way to the beginning. Typically, a service is instantiated with agent-based monitoring and the team responsible for service delivery needs to read the telemetry coming out of the agent (a process that can be similar to [reading tea leaves](https://en.wikipedia.org/wiki/Tasseography)). They work their way backward to understand how the service functions based on what can be observed.

At New Relic, through tens of thousands of observability deployments, we've discovered that the more involved designers, architects, and developers are in the definition of optimal service delivery measurement, the better. The emergence of the IT aphorism ["shift left"](https://en.wikipedia.org/wiki/Shift-left_testing) speaks to the need of involving developers more directly in the software lifecycle activities that happen after the development stage is complete.

In the case of service observability, we find there is little specific guidance on how developers can meaningfully contribute to production telemetry definitions. This guide is intended to provide practical suggestions for developers to evaluate the state of your telemetry and suggest paths to improve it. Observability programs that closely link developer expectations with the runtime behavior of production systems are much more effective to diagnose and remediate aberrant conditions. That more-direct developer connection also produces services that are more robust and performant.
...
Before you begin to introduce a service instrumentation practice into your development processes, get acquainted with New Relic fundamentals available from [New Relic University](https://learn.newrelic.com/).
...
Instrumentation is the process to acquire telemetry from a software system and its associated services for the purpose of describing that software system's runtime behavior and business function. Monitoring systems tend to provide generic capabilities for telemetry acquisition that can be fine-tuned to close gaps when monitoring the function of a software system. This use case assumes that your observability program has completed the [Customer experience - Quality foundation guide](/docs/new-relic-solutions/observability-maturity/customer-experience/quality-foundation-implementation-guide) and that you have a well considered and deployed telemetry collection architecture for your services.

With the possible exception of alert definition, instrumentation offers the most open-ended and customizable activities related to observability. The New Relic platform provides features for highly customizing your instrumentation. Because of this, you should carefully consider the amount of time and effort you're going to put into instrumenting your services.

Like all service instrumentation assets and dependencies, the introduction of instrumentation will require ongoing oversight and maintenance, and therefore is a form of technical debt you'll accrue for your project. As you begin the process of instrumentation, you want to continually ask yourself the question:
...
<Callout variant="tip">
  The New Relic agents provide a variety of options for runtime configuration. Please refer to the [APM agnet config docs](/docs/new-relic-one/install-configure/configure-new-relic-agents) for the options specific to your runtime.
</Callout>
...
* [Using the New Relic .NET SDK API](/docs/agents/net-agent/net-agent-api/)
* [Environment variables](/docs/agents/net-agent/configuration/net-agent-configuration/#environment-variables)
* [Config file options](/docs/agents/net-agent/configuration/net-agent-configuration/#setup)
