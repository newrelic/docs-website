...
It is configured simply by providing the max number of logs that will be stored in the APM agents logging queue.  It operates based on a custom priority queue.   All log messages are given a priority.  Logs that occur within a transaction get the transaction's priority.

The queue for logs sorts based on the priority and when the log arrives.  Higher priority wins and if needed, the newest log will win (we keep a count for each entry in the queue).  Logs are added individually to the queue (even ones in a transaction) and when the limit is reached, the log at the end of the queue is pushed out in favor of the newer log.  In the resources section below there is a [quickstart dashboard](https://onenr.io/0Bj3BlEZkRX) that will allow you to track log volume in a very simple way.  This will enable you to adjust or disable sampling rate to suit your observability needs.
...
The infra agent adds some attributes by default, including any custom tags the customer added to the host.  It is possible that your configurations bring in many more than that including a large number of AWS tags which will appear in New Relic with the form `aws.[attributename]`.  These attributes are quite important to proper observability therefore it is imperative that you evaluate your visualization, analytics, and alerting needs in light of any planned configuration changes.  For example logs from a Kubernetes cluster will not be very useful if they do not contain metadata such as:
