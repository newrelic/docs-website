---
title: Observability Center of Excellence
tags:
  - Observability maturity
  - Operational efficiency
  - OE
  - Center of Excellence
  - Observability Center of Excellence
  - Implementation guide
redirects:
  - /docs/new-relic-solutions/observability-maturity/

---
import omaCoeScaleStandardsSpeed from 'images/oma_diagram_coe-value.png'

import omaCoeElements from 'images/oma_diagram_coe-elements.png'

import omaCoeFramework from 'images/oma_diagram_coe-framework.png'

import omaCoeStakeholders from 'images/oma_diagram_coe-stakeholders.png'

import omaCoeModel from 'images/oma_diagram_coe-model.png'

import omaCoeRamp from 'images/oma_diagram_coe-ramp.png'

import omaCoeChannels from 'images/oma_diagram_coe-channels.png'

import omaCoeKnowledgeBase from 'images/oma_diagram_coe-knowledge-base.png'

This guide walks you through ideas and processes that will help you set up an Observability Center of Excellence at your organization. This is part of our [series on observability maturity](/docs/new-relic-solutions/observability-maturity/introduction). 

## What is a CoE? [#coe-definition]

By definition a center of excellence is...

_A team of dedicated individuals, from multiple functional areas of an organization, that lead the way in exploring and adopting tools, techniques and practices._ 

_The Center of Excellence promotes collaboration, standards, best practice and adoption. It is supported by a community with a rich collaborative knowledge-base reinforced with frequent enablement and learning._

Centers of excellence exist around a host of disciplines including things like security, applications, cloud services and automation. This guide is focused on how to create a Center of Excellence around observability best practices. This will help you drive standards, speed and scale across your organization, and maximize the return on the investment in observability solutions.

<img
  title="CoE purpose venn diagram"
  alt="CoE purpose venn diagram"
  src={omaCoeScaleStandardsSpeed}
/>

Here's a 15-minute video explaining the Center of Excellence concept: 

<Video
  id="d1ddzUKSLrk"
  type="youtube"
/>

## Why build an Observability CoE? [#coe-value]
Creating an Observability Center of Excellence delivers benefits in three key areas.

### Adoption
* Leads to wider adoption of observability practices, tools and processes
* Reduces startup time for new teams
* Breaks down skill silos

### Implementation
* Maintains standards and provisions frameworks for implementation
* Creates a supported framework for enablement, learning and training
* Keeps observability fresh, new and part of the every-day

### Expertise
* Encourages collaboration and sharing of success and best practice
* Enables practitioners to be supported by their peers
* Helps prevent expertise decay

Successful implementation of an Observability Center of Excellence helps organizations to reduce risk, drive operational efficiencies and improve their service delivery. This in turn helps businesses to achieve their goals and objectives as their applications and services evolve to meet changing demands.

## The Observability CoE core elements [#coe-components]

<img
  title="OCoe elements"
  alt="OCoe elements"
  src={omaCoeElements}
/>

An Observability Center of Excellence primarily consists of three core elements: the framework, key stakeholders, and the operating model.

## The Observability CoE framework [#coe-framework]

<img
  title="OCoe framework"
  alt="OCoe framework"
  src={omaCoeFramework}
/>


## The Observability CoE stakeholders [#coe-stakeholders]

The most important elements of the Observability Center of Excellence are the stakeholder groups. They are:

* Responsible for the creation, maintenance and support of the OCoE 
* Responsible for selecting the standards and best practices to be included in the OCoE
* The primary consumers and contributors to the OCoE itself

These stakeholders fall into the three categories (these may be known by other names in your organization).   

<img
  title="OCoe stakeholders"
  alt="OCoe stakeholders"
  src={omaCoeStakeholders}
/>

### The Observability CoE core team [#ocoe-core-team]

This is central team responsible for maintaining the Observability Center of Excellence.

Their responsibilities include:

* Maintaining a relationship with New Relic
* Administration and coordination of accounts and users
* Onboarding of new teams and individuals
* Maintenance of resources and knowledge base
* Promotion of collaboration and sharing amongst teams

Typically this would be a team of 1-3 people with a background in observability and strong familiarity with New Relic. They work 2-3 hours per week on the OCoE alongside their regular role.

### The Observability Council [#observability-council]

A group of leaders responsible for selecting or producing, ratifying and promoting observability standards across the organization. 

Their responsibilities include:
* Thought leadership and direction
* Defining and agreeing on standards and tooling
* Ensuring implementation quality

The size of this team will vary based upon your organization's size and structure. It will consist of individuals from across the organization that have a vested interest in ensuring observability standards are implemented and maintained across the business. Ideal candidates for this function include those with leadership roles in operations, development and support. This is an advisory role that does not require a significant commitment of time.

### The Observability Guild [#observability-guild]

This is a cross functional group of individuals across the organization who have experience and passion for observability, tooling, and are willing to help others. They are the _heroes_ for your Observability Center of Excellence and the main contributors to supporting others and generating content for it.

Guild members are not appointed but should volunteer based upon their passion and expertise for observability.

Their responsibilities include:

* Answering questions from others
* Contributing to knowledge base
* Demonstrating best practice
* Fostering an environment of self help and collaboration
* Attending New Relic technical enablement workshops and events

The number of guild members in any organization will vary and generally increase as the adoption of observability best practices increases across your teams. Being part of the guild becomes just part of doing their day job and something which helps both them and others to benefit from the OCoE.

## The Observability CoE operating model [#coe-model]

Finally having understood the entities that make up your OCoE and the key stakeholders who will interact with it, the last element is the Observability Center of Excellence operating model, which brings all these elements together. Below is a visualization of the model which will be covered in more detail later in this guide.

<img
  title="OCoe model"
  alt="OCoe model"
  src={omaCoeModel}
/>
    
    
The creation and operation of an Observability Center of Excellence breaks down into two key stages:

* Stage 1: Creating the OCoE framework that will support the adoption of the selected observability best practices.
* Stage 2: The ongoing maintenance and support of the OCoE to ensure that the best practices and standards, along with the content and enablement materials are kept up to date and relevant.

<img
  title="OCoe stages"
  alt="OCoe stages"
  src={omaCoeRamp}
/>

### Stage 1: Ramp up [#coe-stage1]

The ramp up stage for an OCoE involves introducing the concepts behind the initiative, running a workshop to understand the objectives and primary stakeholders, selecting supporting technologies and the best practices to adopt, and then seeding your OCoE with the assets to support them.

#### Steps [#coe-steps]
<CollapserGroup>
  <Collapser
    id="step1"
    title="1. Understand your objectives"
  >
    What are you looking to achieve through building an OCoE? The desired outcomes will dictate how you will measure the success of the OCoE.
  </Collapser>
  
  <Collapser
    id="step2"
    title="2. Select your KPIs for success"
  >
    Based upon your objectives these might include things like: 
    * Time to observability
    * MTTD
    * MTTR
    * Success stories created
    * Number of users onboarded
  </Collapser>
  
  <Collapser
    id="step3"
    title="3. Establish leadership sponsorship"
  >
    It's important to have senior leadership sponsorship for the OCoE initiative, sponsors that are aligned to the objectives and desired outcomes, and agree on the measures for success.
  </Collapser>
  <Collapser
    id="step4"
    title="4. Identify the key stakeholders"
  >
    * Observability CoE core team: Who will maintain the relationship with New Relic and maintain the OCoE platform?
    * Observability council: Who will determine the standards and best practices to be implemented?
  </Collapser>
  
  <Collapser
    id="step5"
    title="5. Select & implement supporting technologies"
  >
   * Collaboration channels: Understand what the preferred collaboration tools are and implement the recommended channels on the preferred solution. (see collaboration channels below)
   * Learning resources: Select the appropriate learning resources to support your onboarding and enablement processes. (See [https://learn.newrelic.com/](https://learn.newrelic.com/) for New Relic learning resources.)
   * Knowledge base: Determine what platforms are used for knowledge sharing. Implement the OCoE launchpad on that platform to create the framework for your knowledgebase. (see the knowledge base section below)
  </Collapser>
    
  <Collapser
    id="step6"
    title="6. Select your best practices"
  >
  * Observability standards - determine which standards you wish to implement as part of your OCoE. These could include instrumentation standards, naming and tagging conventions, custom attributes and events etc. For further information see the [best practice guides](/docs/new-relic-solutions/overview) for the observability solutions you are using.
  * Observability use cases - determine which Observability Maturity use cases you wish to implement as part of your OCoE. Typically most organizations start with [service level management](/docs/new-relic-solutions/observability-maturity/uptime-performance-reliability/optimize-slm-guide), but based upon your own business's focus, other use cases may be a priority. 
  </Collapser>
    
  <Collapser
    id="step7"
    title="7. Recruit observability guild members"
  >
    This final stage of the ramp up is the most important, without the guild members the OCoE will not succeed. However guild members need to volunteer and as such this process is about showcasing and evangelizing the successes and benefits of the OCoE to draw more stakeholders to it.
  </Collapser>
</CollapserGroup>

#### Collaboration channels [#coe-collab-channels]

<img
  title="OCoe channels"
  alt="OCoe channels"
  src={omaCoeChannels}
/>

Having determined the collaboration tool of choice for you OCoE we would recommend setting up the following regular clinic and collaboration channels.

#### New Relic clinic [#coe-clinic]

A regular open session for the OCoE core team to answer questions, share information and address issues with the practitioners.

Typical agenda for this session would be:
* Help and support
* Platform updates
* Best practice sharing
* Demos and examples

Typical attendees would be:
* OCoE core team (coordinators)
* Guild heroes
* Practitioners

#### Messaging Channels [#coe-channels]

__#help-newrelic__
A channel for all users of New Relic to ask questions, share knowledge and get help. 

Typical members of this channel would be:
* All observability practitioners
* Guild heroes (providing active support)
* OCoE core team (providing active support)

__#core-newrelic__
A channel for core team and New Relic CSS to freely communicate and escalate issues and problems.

Typical members of this channel would be:
* OCoE core team
* Guild heroes (possibly)
* New Relic CSM / SA / TAM

#### Knowledge base

<img
  title="OCoe knowledge base"
  alt="OCoe knowledge base"
  src={omaCoeKnowledgeBase}
/>

The knowledge base provides a single destination for practitioners to find standards, configuration, examples, code snippets, quick start guides and other related material that is focused on your organization's configuration of New Relic and implementation of observability best practices.

It provides links out to New Relic documentation and learning tools to support self passed enablement and onboarding.

To seed your knowledge base, New Relic provides a starter kit for building your own wiki-like knowledge base intended to jump start your Observability Center of Excellence.

The wiki skeleton consists of a number of markdown pages that cover many areas of the knowledge base from account information to quick start reference guides. 

It is available in [Github](https://github.com/newrelic/newrelic-ocoe-launch-pad) where you can clone and customize the content to suit your needs.

### Stage 2: Ongoing support [#coe-stage2]

Once the framework of your OCoE is up and running it needs to be maintained to keep the content current and relevant to your objectives. To achieve this New Relic provides ongoing support in four key areas to help get the most value from your OCoE.

<CollapserGroup>
  <Collapser
    id="support-documentation"
    title="1. Support and documentation (available to all users)"
  >
  * Technical support tickets for help with all aspects of  New Relic setup, integration and usage available at [https://support.newrelic.com](https://support.newrelic.com)  
  * Comprehensive documentation on all aspects of the New Relic product and best practices for observability available at [https://docs.newrelic.com](https://docs.newrelic.com) 
  </Collapser>

  <Collapser
    id="nru-training"
    title="2. New Relic University (NRU) and training (available to all users)"
  >
    * Multi-day enablement courses covering the entire New Relic Observability platform and all products delivered by New Relic University instructors.
  * Self paced courses and webinars via the New Relic University (NRU) training programme. 
  * Learning tracks available [https://learn.newrelic.com/](https://learn.newrelic.com)
  </Collapser>

  <Collapser
    id="updates-enablement"
    title="3. Feature updates and enablement (organized through the NR customer success team)"
  >
  * Introduction and demonstration of new and under-used parts of the platform.
  * Road map and future upcoming product releases.
  * Technical deep dives with New Relic product management
  </Collapser>

  <Collapser
    id="recurring-meeting"
    title="4. OCoE catch up (scheduled recurring meeting)"
  >
Agenda: 

* Progress updates
* Session and engagement planning
* Success stories
* Platform updates
* Support review

Attendees: 
* New Relic CSM / SA / TAM
* Customer core team

  </Collapser>
</CollapserGroup>

## Conclusion [#conclusion]

By implementing some or all of the guidance outlined in this document you will have put in place the key components to create collaborative culture around your observability practice. An environment that will foster passion and develop strong expertise around observability, while also delivering business benefits aligned to improving Operational Efficiency, Uptime, Performance and Reliability, Customer Experience, and Innovation and Growth.

The Observability Center of Excellence helps your organization develop and retain the skills and best practices that are essential to developing and operating today's complex modern application architectures. 

