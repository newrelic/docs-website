...
Like any other New Relic tool, the Diagnostics CLI service is designed to protect data privacy for you and your customers. The Diagnostics CLI inspects system information and New Relic product artifacts (logs and config files) that are relevant for performing diagnostic checks that assess New Relic product configuration and operability.
...
Any support ticket attachments made using `nrdiag` or containing `nrdiag` data that are less than or equal to 20MB are stored by New Relic.
