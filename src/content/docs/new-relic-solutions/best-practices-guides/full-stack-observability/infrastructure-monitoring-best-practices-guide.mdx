...
This allows you to filter down to a part of your infrastructure using the same AWS tags (example, `ECTag_Role='Kafka'`), and slice-and-dice your data in multiple ways. Additionally, our alerts and [entity filter views](/docs/new-relic-solutions/new-relic-one/core-concepts/search-filter-entities) are tag-driven and dynamic, so they automatically add/remove instances matching these tags to give our users the most real-time views that scale with your cloud infrastructure.
