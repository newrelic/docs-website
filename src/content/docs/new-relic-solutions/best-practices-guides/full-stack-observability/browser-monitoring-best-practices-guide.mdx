...
With the different alerting policies that you're setting up, you'll want to make sure to take advantage of the different [alert notification channels](/docs/alerts/get-notified/intro-notifications/#channels) available so that they're integrated into team workflows. After all, what good are alerts if no one knows about them?

You can route alerts through Slack, PagerDuty, webhooks, email, and more. You also have the opportunity to align alert notifications with your response processes, such as integrating with ChatOps or [linking runbooks](/docs/alerts/alert-conditions/provide-runbook/) to your alerts. Be sure to evaluate alert policies on a regular basis to ensure that they are always valid.
...
2. Under <DNT>**Issue Creation Preference**</DNT>, set up different [notification channels](/docs/alerts/get-notified/intro-notifications/#channels), which can then be used within different [alerting policies](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy).
