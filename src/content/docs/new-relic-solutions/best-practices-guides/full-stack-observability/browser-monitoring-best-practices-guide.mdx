...
Browser SPA monitoring is enabled by default for new [browser agent installs](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent). To edit this configuration, go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Settings > Application settings**.
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Service levels**.
2. Click **Add a service level**.
...
New Relic provides [unified alerting](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/basic-alerting-concepts) across all our products, including browser monitoring, so that you'll always be in the know. We recommend setting up alerts to monitor your core web vitals, along with these sample alerts to get you started:

* **Core web vitals:** Alert if first input delay or largest contentful paint are above the `Needs improvement` threshold.
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alerts conditions (Policies)**.
...
4. Click the checkbox for the applications you want to alert on, and click **Next, define thresholds**.
5. Pick the **Page view throughput** and define the normal range you want to trigger the alert.
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > New alert**.
2. Under **Issue Creation Preference**, set up different [notification channels](/docs/alerts/new-relic-alerts/managing-notification-channels/notification-channels-controlling-where-send-alerts), which can then be used within different [alerting policies](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy).
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Dashboards > Browser examples (imported from https://newrelic.com/instant-observability/browser-examples)**
</figcaption>
...
* Visit **[one.newrelic.com](https://one.newrelic.com/all-capabilities)**, and query on the custom data that is now available.
...
* Ask a question in our community forum, the [New Relic Support Forum](https://discuss.newrelic.com/).
