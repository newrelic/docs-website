 ...
 * Author:      New Relic
 * Version:     1.0
 * Purpose:     Demo some advanced concepts for a New Relic Synthetics scripted API monitor with async/await, secure credentials for authentication,
...
async function makeGetRequest(url, options, maxRetries, retryCount = 0) {
  try {
    console.log(`Attempt ${retryCount} of ${maxRetries}`)
    const response = await $http.get(url, options)

    // Use assert to throw an error if the status code is not 200.
    ...
    // Return parsed JSON data for further processing, if needed
    console.log("Parsed JSON data:", jsonData)
    return jsonData
  } catch (exception) {
    if (retryCount < maxRetries) {
      console.log(`Retry count not reached, attempting request again.`)
      await sleep(RETRY_DELAY_MS)
      await makeGetRequest(url, options, maxRetries, (retryCount = 0))
    } else {
      $util.insights.set("myAttribute", false)
      throw exception
    }
  }
}
...
// This shows up in SyntheticCheck as `custom.myAttribute`
$util.insights.set("myAttribute", true)
```
    </Step>
</Steps>
