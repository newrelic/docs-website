...
There are [seven types of synthetic monitors](/docs/synthetics/synthetic-monitoring/getting-started/types-synthetic-monitors). Which monitors you choose to deploy will depend on what you want to monitor and how you want to create the monitor. For example, a Step monitor is a great way to create what is essentially a Scripted browser monitor behind the curtains, without having to write code.

### How to do it [#howto-1]

1. To add a monitor, go to **[one.newrelic.com](http://one.newrelic.com) > Synthetics** (or [one.eu.newrelic.com](http://one.eu.newrelic.com) if you have an [EU-based account](/docs/using-new-relic/welcome-new-relic/get-started/introduction-eu-region-data-center)) and click **Create monitor** at the top right of the page.
    ![](/src/images/synthetics-create-monitor.png)
2. Select a monitor type and then fill in all required fields.
3. Optionally add tags, change the Period or Runtime (available for Simple browser, Scripted browser and API), or add a validation string (available for Ping and Simple browser) or advanced options, which enable substring monitoring for the following types of response validation:
   ...
   * **Bypass HEAD request (for Ping).** This option skips the default HEAD request and instead uses the GET verb with a ping check. GET requests will always happen if the HEAD request fails.
