...
Try to adjust the [delay/time](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions/#delay-timer) with your data's behavior. A short delay may trigger false alerts because of incomplete data and a large delay may increase the time you get notified. New Relic doesn't know how much data to expect nor how late that data might reach an endpoint of New Relic. Depending of the log shipper and configuration, log data can be batched and see significant delay for low volumes logs.
