...
After Terraform successfully provisions the workload, navigate to the <DNT>Workloads</DNT> section in the New Relic UI to verify the new resource. You can inspect the automatically discovered entities in the transaction flow, review the alert policy bindings, and verify that the status rollups reflect the correct workload health.
