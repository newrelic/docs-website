...
There are three methods available to configure your health status. Available options vary depending on whether you are using a Standard Workload or an Intelligent Workload.
...
The third method for determining workload health status (available for an Intelligent Workload only) is to set up custom alert policies and conditions. This method allows you to define specific conditions that will trigger workload health status changes based on alert states.
...
2. If no static status is set, the status is determined by <DNT>**Automatic rules**</DNT> (for a Standard Workload) or <DNT>**Alert policy and conditions**</DNT> (for an Intelligent Workload).
