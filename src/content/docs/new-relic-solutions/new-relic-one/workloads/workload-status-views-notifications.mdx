...
We provide a status value for all your monitored entities, which is based on the results of your [alert events](/docs/alerts). You can check the [color-coded alert status](/docs/alerts/new-relic-alerts/configuring-alert-policies/identify-entities-without-alert-policies#colors) for each entity from the <DNT>**All entities**</DNT> view.
