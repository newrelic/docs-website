...
* **Low sampling rate**: If trace sampling is too low for certain service connections, New Relic may not have enough trace data for that edge during your query window to display those relationships.
  * **What you can do**: Some APM agents are already shipping with new sampling options, and more sampling improvements are coming in FY27. Check your agent's latest version and sampling configuration to take advantage of enhanced sampling capabilities.
...
False positive matches can occur where unrelated services appear in your map. These can show up either:
* Connected to the main graph (randomly)
* As isolated fragments (more common, especially in large systems where many more entities are unrelated to a given app than related to it)

Recent improvements filter out most disconnected service fragments, significantly reducing false positives. With this change:
* Some false positives may still appear, but far fewer overall
* The reduction is especially dramatic in large, complex environments with many entities
