...
Make sure that the entities are being monitored with an agent. If you have a mix of agents with some having [distributed tracing](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing) turned on and some having it turned off you will not see the relations between those agents. The solution for this scenario is to run all agents either with distributed tracing turned on (preferred) or turn it off for all agents. If this does not remedy the issue, the service you're trying to view require manual instrumentation.
