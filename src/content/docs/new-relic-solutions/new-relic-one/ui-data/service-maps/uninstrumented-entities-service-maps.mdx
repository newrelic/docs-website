...
<Callout variant="tip">
  Note that you'll see the uninstrumented entities slightly different than the instrumented ones. Both are hexagons, but uninstrumented entities have a dotted border instead of a solid one.
</Callout>

Knowing what is not instrumented increases your visibility over your platform and gives you more control over it. To instrument an uninstrumented entity, you need to click on it, choose `instrument`, and you'll see a tutorial on how to perform the instrumentation depending on the type of entity. After this process is finished, the service map displays the entities as usual, using the solid border.
