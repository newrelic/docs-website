...
Metric normalization rules are essential for managing the high cardinality of [metric timeslice data](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data), which occurs when a large number of unique metric names are reported to New Relic. Most of this occur with web transactions metrics named from URLs, resulting in high cardinality. Metric normalization rules help to reduce [Metric Grouping Issues](/docs/using-new-relic/cross-product-functions/troubleshooting/metric-grouping-issues) (MGIs), ensuring that application performance data remains meaningful. 

To reduce high cardinality and prevent MGIs, New Relic supports rules for grouping or filtering out metric timeslice data using metric normalization rules. In some cases, when MGIs with a very high number of metrics are detected, a `DENY_NEW_METRICS` rule is created automatically to protect the platform from performance degradation. This `DENY_NEW_METRICS` rule does not affect any metrics previously reported to New Relic. You can create and manage these rules both automatically and manually to prevent performance degradation.


<Callout variant="tip">
  You can also create metric normalization rule using [NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/). For more information, refer to [Metric normalization rules using NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-metric-normalization-rule/).
</Callout>
...
## Managing the metric normalization [#manage-rules]
...
2. Provide the following details:
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Select one of the following action to perform on the metrics:
        <ul>
          <li>**Replace**: Replace matched metrics with the value in the <DNT>Replacement</DNT> field.</li>
          <li>**Ignore**: Block any metrics that match the defined **Regular expression** from reporting to New Relic..</li>
          <li>**Ignore new metrics**: Write only metrics that have already been reported, ignoring new matches.</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>**Replacement**</td>
      <td>This field is only required when **Replace** option has selected in the previous field. It replaces any metric name matched defined in the **Regular expression** field.</td>
    </tr>
    <tr>
      <td>**Order of rule in sequence**</td>
      <td>Specify the order in which the rule is applied relative to other rules. Lower the numbers, higher the evaluated priority. Recommend value is `2000`</td>
    </tr>
    <tr>
      <td>**Terminate**</td>
...
- **Rule Modification Capability**: Allows users to deactivate their own `DENY_NEW_METRICS` rules directly from the MGI Details page, as well as modify `REPLACE` and `IGNORE` rules without support tickets. For more information, refer to [Enable or disable a rule](#enable-disable-rule).
...
### Disable a rule [#enable-disable-rule]

You can deactivate a `DENY_NEW_METRICS` rule from the **MGI details** page, when the `Denied rate (minute)` is below the `resolution rate (minute)` of 1000.

To deactivate a rule, on the **MGI details** page, click **Deactivate**.

image required
```<img
  title="Deactivate rule"
  alt="Deactivate rule"
  src="/images/deactivate-rule.webp"
/>```