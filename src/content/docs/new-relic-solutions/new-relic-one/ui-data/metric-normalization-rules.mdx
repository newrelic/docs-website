...
2. Provide the following details:
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Enter a [regular expression](https://regexone.com/) pattern that matches the metrics to be normalized. This field must start with a `^` and ends with a `$`. It defines a pattern to identify which metrics should be affected by the normalization rule. For example:
...
On the <DNT>**Metric normalization rules**</DNT> page, select the rule and click the <Icon name="fe-more-horizontal"/> icon, then choose **MGI details**.
