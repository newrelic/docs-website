...
The entity filter bar is available on several New Relic experiences, including the main entity list view, APM, the browser monitoring UI, the infrastructure UI, and more. To learn how to use this to filter entities and save views, see [Entity filter](/docs/new-relic-solutions/new-relic-one/core-concepts/search-filter-entities). 
...
* To find an entity's GUID and other metadata: from any [list of entities](/docs/new-relic-solutions/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts#find), click an entity's <Icon name="fe-more-horizontal"/> icon, and click **See metadata and tags**.
...
* To learn technical details about entity types, see our [entity docs in our GitHub repo](https://github.com/newrelic/entity-definitions/tree/main). In an entity type's `definition` file, you'll see information like:
...
Using your telemetry, New Relic can detect entities which are not being instrumented right now but are used by your application. For example, one of your instrumented entities may be calling an Amazon RDS database which is not currently instrumented by New Relic, so we cannot provide any tooling like monitoring or alerting for it.
