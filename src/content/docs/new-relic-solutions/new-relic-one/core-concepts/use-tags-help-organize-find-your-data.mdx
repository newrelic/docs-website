...
<CollapserGroup>
  ...
  >
    ...
    Tags applied from <InlinePopup type="apm" /> agent configuration can result in multiple values for a given tag key on a given entity, because each deployed instance of the agent could be configured to send a different value.
  </Collapser>
...
You can query <InlinePopup type="apm" /> tags using NRQL. These are the supported [APM agents](/docs/using-new-relic/cross-product-functions/install-configure/update-new-relic-agent):
