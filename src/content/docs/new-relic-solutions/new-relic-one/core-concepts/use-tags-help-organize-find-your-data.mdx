...
<CollapserGroup>
  ...
  >
    ...
    * Avoid including several values like `regions: EMEA | US | LATAM` in a single tag. We recommend using three different tags for that, like `region: emea`, `region: us`, and `region: latam`. If you want to match multiple tags, you can do that using the advanced options in the filter UI.
      * A caveat for [alert incidents](/docs/alerts-applied-intelligence/overview/#concepts-terms) and [NRQL queries](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language): these only support one key name instance. If you do use multiple key names, only one will be selected to be used.
      * Infrastructure Agent and Cloud Infrastructure entities don't support multiple values for the same name in NRDB queries. In those cases, only one of the values will be added to the metric.
  </Collapser>
