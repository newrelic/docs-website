...
Creates a line chart. Data can either be obtained by performing a NRQL query against a particular account, or it can be passed through the `data` prop.
...
```js
<LineChart accountIds={[1]} query="SELECT count(*) FROM `Synthetics` SINCE 1 DAY AGO TIMESERIES AUTO FACET jobType" />
```

#### With multiple accounts

```js
<LineChart accountIds={[1, 1067061]} query="SELECT count(*) FROM `Synthetics` SINCE 1 DAY AGO TIMESERIES AUTO FACET jobType" />
```
