...
| `sizeType`##### enum               | Determines the width of the layout item. We recommend you use the small size for navigation purposes (ex. sidebars) and the medium size for showing contextualized information (ex. activity stream).Size type only works on SPLIT_LEFT and SPLIT_RIGHT items.##### REPLACE WITH CUSTOM COMPONENT&lt;One ofLayoutItem.SIZE_TYPE.MEDIUMLayoutItem.SIZE_TYPE.SMALL>        |
