...
[**Get started with the guided install.**](https://one.newrelic.com/marketplace?account=1&state=83ad7308-eba3-c490-39ae-54547650309e)

### Manual install for infrastructure agent and on-host integrations [#manual-infrastructure]

As an alternative or a supplement to the [guided install](#infra-guided-install), below are some manual procedures for installing the infrastructure agent and some infrastructure service integrations: 
...
Our infrastructure agent instruments hosts, and also supports our many integrations for other infrastructure-related services. The agent can b0e installed on a host (​​whether that's a node in a Kubernetes cluster, a VM in your own on-prem data center, or a VM in the cloud) or in a Docker container. Select a relevant option: 
...
### Some resources for using the infrastructure UI 
...
Our browser monitoring gives you insight about the experiences your end users are having with your digital properties. Our browser agent is optimized for a wide variety of application architectures. It can monitor dynamic applications, and can track route changes, initial page loads, and synchronous and asynchronous activity during browser interactions. You can also see your real user data in your distributed traces, alongside your application data. 

It's easy to set up browser monitoring, especially if you have an APM agent installed already. To get started, see [the **Add your data** UI page](https://one.newrelic.com/marketplace?state=dbe79256-a6cf-ea3e-baac-c5dab3a0b5cb) and click **Browser monitoring**. When you've completed the setup, look for your data in the [browser summary UI page](/docs/browser/browser-monitoring/getting-started/browser-summary-page) and in [the distributed tracing UI](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui).

Want to watch a short video on setting up browser monitoring? 
...
<figcaption>
Reviewing service maps is one way to ensure you've instrumented everything you need to monitor. </figcaption>
