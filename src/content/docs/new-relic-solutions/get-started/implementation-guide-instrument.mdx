...
* Set up application monitoring, which can include one or more of these:
  * Our <InlinePopup type="apm" /> agents, which monitor some of the most language-specific app frameworks.
...
Our <InlinePopup type="apm" /> agents are one of our most popular tools, so next we'll give some APM tips and considerations. 
