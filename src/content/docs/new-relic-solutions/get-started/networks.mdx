...
For all data ingest applications, with the [exception of OpenTelemetry](#otel-ports), we recommend port 443, a secure channel for encrypted HTTPS traffic and our default. These also make port 80 available, an unsecured channel open to all HTTP traffic.

Some ingest can be configured to use both port 80 and port 443, but we recommend that you use port 443 (default). If you have an existing configuration that uses port 80, you can update it to use port 443.
