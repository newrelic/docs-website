...
In previous sections we described the main elements within the pathpoint UI: stages, steps, and touchpoints.  In pathoint the highest level grouping of these elements is the *flow*.  A flow is usually used to model a "business journey" or "customer journey".   The stages of a flow represent higher level concpets that compose the journey.  When we think about what kinds of stages to use it helps to think about what our business process or "journey" is and how we think of it in sequency.   Often it helps to look at other flows within a similar industries.  Examples of industries or verticals are:
...
5. Supporting infrastructure and platforms relevant to each step (often cloud servcies such as load balancers, container services, or managed databases or messaging systems).
...
It’s better to have a well thought out flow with some signal gaps than a flow that is put together hastily with any available signals.
