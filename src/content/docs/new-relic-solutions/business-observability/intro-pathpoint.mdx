...
A Stage will take on the status of it's worst step.  If a stage has two green steps and one red step this stage will become red.  That rule is not configurable.  However the step status rule is configurable.  
    
Too view or configure your step's status rule enter edit mode and do the following:

    1. Click on the ellipses to the right of step name that you want to configure
    2. Under *Status Configuration* choose the rule that you want to use
    
By default it uses *Rollup the worst status*. You can refine this by checking *refine only when* and choosing a percentage of signals needed to change the step status (i.e. you have 10 signals, and want to change status at 50% - when 5 signals are either red or yellow, the status will be changed to reflect the worst status among the five unhealthy signals)


In addition to this, you will be able to exclude certain signals from contributing to the status of the step. This allows users to eliminate noisy/known bad signals from impacting the status.
    
You may also choose *Rollup the best status*.  Exactly as it sounds with this option the step will take on the status uf it's best signal.
