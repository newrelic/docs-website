...
A Stage will take on the status of its worst step.  If a stage has two green steps and one red step this stage will become red.  That rule is not configurable.  However the step status rule is configurable.  
...
By default it uses *Rollup the worst status*. You can refine this by checking *Apply only when* and choosing a percentage of signals needed to change the step status (e.g. you have 10 signals, and want to change status at 50% - when 5 signals are either red or yellow, the status will be changed to reflect the worst status among the five unhealthy signals), or a set count of signals needed to change the step status (e.g. at least 2 of 5 signals must be unhealthy to change the step status).


In addition to this, you will be able to exclude certain signals from contributing to the status of the step. This allows you to eliminate noisy/known bad signals from impacting the status.
