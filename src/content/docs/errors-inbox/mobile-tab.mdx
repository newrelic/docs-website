...
Frequent mobile app crashes significantly hinder the development lifecycle and degrade the user experience. The **group crashes** page is a powerful tool designed to help your development team deliver a more stable and reliable mobile app experience to your users. With the **group crashes** page you can:
    ...
    * **Filter crash data**: Isolate specific crashes by time, user attributes, network conditions, or device types.
    * **Identify crash trends**: Discover patterns in crash occurrences to proactively address underlying issues.
...
To get the most out of mobile monitoring's <DoNotTranslate>**Crash analysis**</DoNotTranslate> UI, use this basic workflow: 
    ...
    * **Assess overall crash trends**: Begin by reviewing the Crash rate percentage and Crash-free users percentage charts to quickly identify any anomalies or patterns in crash occurrences within the selected time frame.
    ...
    * **Investigate individual crashes**: Select a crash report to view its interaction trail, event trail, thread details, attributes, and other relevant information. If necessary, resymbolicate or export the details to Xcode for debugging.
    * **Update crash status**: Upon completion, mark the crash as "Resolved" or choose from other status options, such as "Resolved in specific version."
...
<CollapserGroup>
  ...
  >
    ...
    * Is there a [period](#time-period) when the percentage of users has been affected significantly by the crash? (If the <DoNotTranslate>**Crash-free users percentage**</DoNotTranslate> chart is empty, there were no user sessions during the selected period.)
  </Collapser>
...
When you select a crash group, a summary panel appears on the left, providing aggregated data for all occurrences of the crash, while detailed information for each individual crash is displayed on the right.

The crash summary panel includes:

    * **Occurrences chart**: Visualizes the frequency of this specific crash over the selected time period since its initial detection. If multiple occurrences are recorded, you can navigate through each instance.
...
The triage section associates the specific error occurrence you're viewing with a [system-created error group](/docs/errors-inbox/errors-inbox/#groups) identified by a unique fingerprint. This unique fingerprint streamlines triage of the entire error group by allowing you to apply status updates or assignments. The error groups found in the **Triage tab** are these system-created groups. 

For more info on how error groups are generated, see [How error groups work](/docs/errors-inbox/errors-inbox/#how-groups-work), and to learn more about status and assignments, see [Error tracking](/docs/errors-inbox/errors-inbox/#assign).
...
The event trail provides a chronological log of events leading up to a crash, aiding root cause analysis. This timeline adheres to your subscription's data retention policy and encompasses both New Relic default events and custom events you've implemented.

By default, the event trail is sorted chronologically, beginning with the oldest event, which is typically the app launch. You can modify the following:
...
The stack trace for the crashing thread is displayed beneath the crash details. Each frame within the stack indicates the associated library/package name and method. The agent attempts to highlight the specific stack frame containing the programming error responsible for the crash, if detected.
...
## Why are profiles disabled?  [#profiles-disabled]
