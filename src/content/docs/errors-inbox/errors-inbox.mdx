...
ioejfiwjefismdufiwnefj iqwjdi9
Resolve errors faster with errors from across your stack, including all APM, browser (RUM), mobile, and serverless (AWS Lambda) data, displayed on one screen. Errors are grouped to cut down on noise, and collaborating across teams is easy with shared visibility to the same error data.asmdkoams asd 
...
<Callout variant="important">
This integration currently supports Atlassian-Jira Classic (company-managed) projects.
</Callout>
