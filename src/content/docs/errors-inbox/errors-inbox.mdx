...
An error group is tagged with an "Expected" tag if all error occurrences in the group are expected. It is tagged with a "Contains expected" tag if some error occurrences are expected and some are not. These tags are available on errors seen since January 21, 2023.

See more on how to mark errors as expected <!-- Add hyperlink for mark errors as expected -->

<!-- Add image shared in Gdoc -->
...
Errors inbox enables you to filter out expected errors for APM entities. You can mark errors as expected <!-- Add link in Gdoc -->, and filter them out of your inbox to remove noise.
...
* In all cases, expected errors are removed from notifications.
* All of the features above as well as the expected errors tags are available on errors seen since January 21, 2023.
