...
* **Resolve in specific version**: This is another option to resolve an error group if you know the error group will be resolved in a specific version, if ​​you know the error group is resolved in an existing version, or if you'd like to enter a specific version. In order to enable this option, you need to set up [version tracking](/docs/errors-inbox/version-tracking/) for your application or service. If an error occurrence with an equivalent or higher semantic version is detected, an error group is auto-unresolved, marked with a regression, and triggers a Slack notification. 
