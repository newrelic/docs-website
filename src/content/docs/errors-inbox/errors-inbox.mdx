...
Access to [log management capabilities](/docs/logs/logs-context/logs-in-context) is limited by user type. See [user type capabilities](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type#user-type-capabilities).
...
We understand it’s very useful to know when an error group was first seen in order to correlate it with a change in the code/system. The accuracy of first and last seen dates depend on the three scenarios outlined below: 
...
* Scenario 3: If an error group occurs once per week or less. First and last seen dates are estimates based on the time of the single occurrence. We only track first and last seen accurately for errors that show up more than once per week. 
