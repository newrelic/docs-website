...
<CollapserGroup>
  ...
  >
    If you see "like" error events grouped into different error groups incorrectly, review our best practices section for guidance on how to adjust error messages. <!-- Hyperlink to the best practices section please --> 
...
For example, although the two error messages below look similar, the error occurrences from below are less likely to be grouped properly due to numbers that are shorter than 4 digits (124). If there is additional information (e.g. SQL statements, ids, etc) you would like to include in the error message, it can be sent via custom attributes.  
