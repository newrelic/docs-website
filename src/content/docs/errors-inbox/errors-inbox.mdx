...
Using the dropdown in the top right, you can sort the list of grouped errors by the number of occurrences or by the error that was last seen in the time range selected. For example, if the time picker selected is Jan 24 5:00pm ET -- Jan 24 5:45pm ET and you sort by last seen, you wi.ll see error groups sorted with those containing the most recent occurrence first.
