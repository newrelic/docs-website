...
Having well grouped errors helps errors inbox better triage and diagnose problems in your system. While errors inbox provides a set of default grouping, it is occasionally necessary to make adjustments.

There are two options for adjusting error groups: updating the error message and [using custom attributes](/docs/errors-inbox/errors-inbox/#best-practices) or creating your own error groups.
...
Even if we don't support your language you can still set `error.group.name` as a custom attribute. This will signal to us that the event belongs to a custom group.  However, the events captured will not have other agent attributes.
