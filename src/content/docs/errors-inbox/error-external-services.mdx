...
When an inbox is connected to Slack, new and resurfaced error groups are sent to a Slack channel moments after they occur. This enables your team to quickly identify any new errors or regressions and resolve them quickly with direct links to the stack trace.
...
Connect errors inbox to Jira to easily create tickets for your errors. Jira templates allow you to quickly create a ticket containing error details and links directly to the stack trace and APM for quick access and resolution. 
...
New Relic supports both team-mananged and company-managed [Atlassian-Jira projects](https://support.atlassian.com/jira-software-cloud/docs/what-are-team-managed-and-company-managed-projects/).
</Callout>
...
## How do I remove a Jira workspace and/or template? 

From the global navigation items, select Detection & AI, inside that page on the left side select 'Destinations'and on the right side of the screen select the 'Destinations' tab once more. Search for your Jira destination and all the way to the right side, select the ... which will open the option to 'Delete destination' option. 

NOTE: Include image attached to the PR (provided via Slack) 
