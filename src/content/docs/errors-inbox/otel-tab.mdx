...
OpenTelemetry for errors inbox includes spans where `otel.status_code = ERROR` and the `span.kind` or kind  is either `server` or `consumer`. These spans are treated as an individual error instance.
