...
1. Go to <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Errors**</DoNotTranslate>.
...
* <DoNotTranslate>**Dynamic grouping:**</DoNotTranslate> The default grouping for error occurrences is based on the error message. You have the option to change the grouping options by any attribute, up to five attributes at a time.
...
/>
<figcaption>
<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Errors (errors inbox) > View profiles**</DoNotTranslate>
</figcaption>
...
The <DoNotTranslate>**Occurrences**</DoNotTranslate> tab includes not only error frequency, occurrence details, and stack traces, it also includes triage information. 
...
### Solutions for reporting stack traces [#solutions]
    ...
    * Use the <DoNotTranslate>**JS errors**</DoNotTranslate> UI page to get more information, including the [event trail](#event-trail). The <DoNotTranslate>**JS errors**</DoNotTranslate> page is designed to give you actionable information even without a stack trace.
...
Here are some  reasons for events in the event log not being present:
    * <DoNotTranslate>**Agent version.**</DoNotTranslate> To capture event log data your site must have [SPA monitoring](/docs/browser/single-page-app-monitoring/get-started/install-single-page-app-monitoring-new-relic-browser) enabled (browser agent version 1071 or higher).
    * <DoNotTranslate>**JS error not part of a browser [interaction](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#interaction).**</DoNotTranslate> If the JS error occurs outside of an interaction, the related browser interaction details cannot be captured (for example: if an async JS file experiences an error).
    * <DoNotTranslate>**A click triggered the JS error.**</DoNotTranslate> In some cases if the error is a direct result of a click, no associated interactions are captured.
