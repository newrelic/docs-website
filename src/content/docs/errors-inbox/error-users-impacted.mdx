...
The metric captures the approximate number of unique users in the time period selected. Additional attributes provided by the userImpact metric which can be used in a FACET clause are `error.group.guid`, `entity.guid`, `application.version`.
