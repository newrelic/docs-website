...
In some cases, an error event may not have attributes that identify the end user affected by that error event. You can manually add custom attributes so our agents captures user-identifying metadata at the time of ingest. 
