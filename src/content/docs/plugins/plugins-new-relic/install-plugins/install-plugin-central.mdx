...
As of December 2, 2020, plugin access is limited to accounts that have accessed a plugin in the past 30 days. We will provide access to add plugins on a per-account basis. If you’d like to continue to use plugins for your accounts, please contact your New Relic account team, and we will work with you on a per-account situation. The plugin experience will reach end of life (EoL) in June 2021. For more information, see our [Explorers Hub post](https://discuss.newrelic.com/t/first-notice-limiting-access-to-new-relic-plugins/121832).
...
## Typical plugin installation procedures [#plugin_procedures]
...
## Install an NPI-compatible plugin [#npi-utility]
...
<CollapserGroup>
  ...
  >
    ...
    5. Unix-based systems: If you need to set a default user, in your terminal, run:
       ...
       ```
       ./npi set user <<var>USER NAME</var>>
       ```
  ...
  >
    ...
    * For a list of plugins that are NPI-compatible, run the command `./npi available`.
    ...
    ```
    ./npi config set proxy_host <<var>HOST</var>>
    ./npi config set proxy_port <<var>PORT</var>>
    ./npi config set proxy_username <<var>USER NAME</var>>
    ./npi config set proxy_password <<var>PASSWORD</var>>
    ```
...
## Install with Chef or Puppet [#chef_puppet_installation]
...
<CollapserGroup>
  ...
  >
    ...
    * New Relic plugin modules: See the [Puppet Forge community site](https://forge.puppet.com/modules/newrelic/newrelic_plugins).
