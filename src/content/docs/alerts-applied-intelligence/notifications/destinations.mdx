...
import destination_status from 'images/notifications_destination_status.png'
...
To track errors, it is recommanded to visit the [Notifications-log](/docs/alerts-applied-intelligence/notifications/notification-integrations/#notifications-log) and the [NrIntegrationError event](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/#error-details)
...
* THROTTLING_WARNING. This destination isn't receiving notifications due to a throttling error. The number of notification requests exeecds your third-party system's rate limitation.
...
It is also possible to review the state using the Nerdgraph API ('status' property.)

<img
  title="Destination status in the Destinations table."
  alt="Destination status in the Destinations table."
  src={destination_status}
/>
