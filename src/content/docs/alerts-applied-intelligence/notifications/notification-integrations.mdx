...
<Collapser
  ...
  4. [Download the **Installation Guide**](https://store.servicenow.com/sn_appstore_store.do#!/store/application/d117597d1b9b9d9078faddf7b04bcba7/1.0.0?referer=%2Fstore%2Fsearch%3Flistingtype%3Dallintegrations%25253Bancillary_app%25253Bcertified_apps%25253Bcontent%25253Bindustry_solution%25253Boem%25253Butility%25253Btemplate%26q%3Dnew%2520relic&sl=sh) from the servicenow store and follow its instructions.
  </Collapser>
...
Our Slack integration isn't tied to a specific user. If the user who originally authenticates with Slack leaves the organization, or their account is deleted, the Slack destination will continue to function. When you authenticate with Slack, our platform goes to Slack and asks for an OAuth token. This token expires at some point, and it's refreshed with the token from the original transaction.

As long as the Slack workspace still exists, it'll continue to receive notifications even if the original Slack account is deleted. The individual user account has nothing to do with the authentication. That account gives us permission to execute the handshake with Slack. Once we have that data, we are authenticated.
