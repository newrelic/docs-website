...
## Newrelic offers two ways to integrate with PagerDuty. [#integration-types]
* **Account Level Integration using REST API Keys(recommended)**: this integration supports two-way sync and the ability to define multiple PagerDuty services on a single New Relic destination.
* **Service Level Integration using EVENTS API keys** : this single service integration using the service level integration key. This integration requires a separate New Relic destination for each unique PagerDuty Service

### Roles [pagerduty-roles]

* Account Level Integration (recommended)
...
* Service Level Integration
    ...
    * The Events API is available to all customers, and any user with a Manager role or above can generate an integration key ([see PagerDuty instructions](https://support.pagerduty.com/docs/api-access-keys#events-api-keys)).
        

  

### Setting up a Destination  [#set-up-pagerduty]

To create a PagerDuty Destination, choose the integration method tab
...
* API Key : Paste the ([General](https://support.pagerduty.com/docs/generating-api-keys#section-generating-a-general-access-rest-api-key) or [Personal](https://support.pagerduty.com/docs/generating-api-keys#section-generating-a-personal-rest-api-key)) API key from PagerDuty
    

#### Service Level Integration

* Service Name: Custom name to identify the destination
    
* Integration Key : Paste the service integration API key from PagerDuty
    

### Two Way Integration Sync [two-way-pagerduty]

Customers using the account level integration can configure a two-way integration with PagerDuty incidents so that when state updates for the incident (acknowledged/resolved) the corresponding New Relic Issue state will be updated (acknowledged/closed)
...
### Sending a Test Notification [test-notification-pagerduty]
