...
<CollapserGroup>
  ...
  >
    ...
    * Acknowledgment of New Relic Issue is triggered when the Servicnow incident state changes from open.
      ...
      To help you get started, we automatically populate the required and recommended fields and values. Required fields must be set with a value.
      You can add or remove optional fields using the `X` mark on their right side. 
      You can enter a custom value for supporting fields using freetext or a dynamic value from the issue payload.
