...
Once the event template conforms to a valid JSON, you can send a test notification to AWS EventBridge.
  ...
  >
    ...
    * The acknowledgment of a New Relic issue is triggered when the PagerDuty incident is acknowledged.
    ...
    4. Send details to the Custom Details section in Pagerduty 
       You can use the default payload or customize it using free text or dynamic variables from the issue paylod. Pick variables from the [variables menu](/docs/alerts-applied-   intelligence/notifications/message-templates/#variables-menu) and apply [handlebars syntax](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) to enrich your paylod.
       The **Preview** section on the right hand-side shows an expected payload after the template is rendered. If the payload doesn't form a valid JSON, an error will be shown and it won't be possible to save the template.
