...
<CollapserGroup>
 ...
 <Callout variant="important">
   The Email Destination is created automatically when you create an Email from workflows(Notifier), so you don't need to configure them from the 'Destinations' menu. Each email destination is unique to the workflow associated with and this means that you may appear as duplicate in the destinations feed.
    </Callout>
