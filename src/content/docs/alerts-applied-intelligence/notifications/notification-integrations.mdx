...
## Create an event source rule on AWS EventBridge [#eventbridge-rule]

To send notifications from New Relic to EventBridge, you need to associate an event source with your AWS EventBridge account. 
...
New Relic is an [AWS EventBridge supported SaaS partner integration](https://console.aws.amazon.com/events/#/partners/newrelic.com?page=overview). 

1. In AWS EventBridge, create a new rule and associate your event source with an event bus.

2. Create a rule for the event bus.

By default, the event bus gets all events from the associated New Relic account. When you create a rule, it's applied to all incoming New Relic events.
