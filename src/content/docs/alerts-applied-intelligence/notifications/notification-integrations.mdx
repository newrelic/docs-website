...
When enabled, a Jira webhook is created in your Jira account for the selected project. The webhook contains access details to New Relic One (URL and New Relic API-KEY).

By default, any update to a Jira issue created by New Relic One syncs back to New Relic One. After enabling two-way integration, when Jira issue changes to closed the corresponding workflow issue will change to closed.
...
To help you get started, we automatically present recommended fields and values. You can add or remove fields and change the values to customize the notification to your needs.
...
Click a test notification to use default field values to see how the issue will appear in Jira If successful, a Jira ticket is created and a link to that ticket appears. 
...
2. Open and download this [XML file](https://storage.googleapis.com/newrelic-notifications/snow-two-way-sync/Newrelic-Snow-Sync-Configurations.xml), which includes the business rule triggering events back to New Relic One.
...
To help you get started, we'll recommend some default fields and values.
...
The preview section on the right side shows an expected payload after the template is rendered. 
