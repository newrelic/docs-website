...
<figcaption>The variables menu shows all the options you have when mapping New Relic notification fields onto the fields in your external service.</figcaption>
...
Iterate a variable of type array and aggregate the values to string:

```handlebars
{{#each tags}}{{this}}{{#unless @last}}, {{/unless}}{{/each}}
```

The result contains the tags, seperated by commas(trailing comma is omitted):
