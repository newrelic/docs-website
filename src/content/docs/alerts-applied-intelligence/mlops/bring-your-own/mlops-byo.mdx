...
1. **Get your API key:** Use an [ingest-keys](http://ingest-keys), either the license key or the Insights insert key.
...
1. **Run the example notebooks ([notebook example](link coming soon)):** You'll have to define your API insert key (the [ingest-keys](http://ingest-keys)) as an environment variable, or send it as a parameter.  
