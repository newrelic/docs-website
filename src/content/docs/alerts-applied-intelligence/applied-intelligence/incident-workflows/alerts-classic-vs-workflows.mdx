...
The classic notification system for alerts and applied intelligence is being replaced by workflows. This is good news because workflows is an improved, flexible notification system that helps your team learn about potential errors faster so you can take immediate action.
...
Now, we've added workflows. Going forward, instead of configuring classic notification channels and associating them to policies, notification destinations will be created and associated to workflows. Workflows can process data from one to many policies, or based on other attributes like tags or priority.
...
4. Notifications will not substantially change, specify which values and for which notification channel types.
