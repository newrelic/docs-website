...
At the end of January 2023, the classic notification system for alerts and applied intelligence will be deprecated and we will be implementing the use of workflows instead. This is good news because workflows is an improved, flexible notification system that helps your team learn about potential errors faster so you can take immediate action.
...
Now, we've added workflows. Workflows are flexible and they don't bind your notification options or your ability to organize alert conditions into polices. For example, you can filter your workflows by a team tag, or check for a "priority" to route issues to the most appropriate destination.

## Configuring custom payloads [#configure-custom-payloads]

To move from notifications to workflows your team might need to make some tweaks to your custom payloads. Workflows still function in the same way as notifications in that, when a condition is violated then a notification is sent to a webhook and when it is sent, it goes with it's custom payload. The migration from notification channels to workflows requires changing some of the terminology in this payload.
...
## Alerts (classic) default payload in workflows [#alerts-classic-payload
]
