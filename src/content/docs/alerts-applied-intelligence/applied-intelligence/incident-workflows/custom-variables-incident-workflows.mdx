...
<CollapserGroup>
  ...
  >
    If the alert conditions query is faceted, you can use the facet value triggering the incident in a query. for example, if the alert condition is
    ```
    FROM BrowserInteraction SELECT average(duration) facet appName 
    ```
    You can then query using the specific appName
    ```
