...
The workflow is configured with the recommended triggers. However, you can control when to trigger a notification for each destination by clicking on the 3 dots and then choosing to Notify when.
The notification triggers can't be missing Activated.
