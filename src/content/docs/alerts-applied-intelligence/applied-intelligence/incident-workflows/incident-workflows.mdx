...
A Workflow is triggered by issue events for issues that match the workflow filter and their matching notification triggers.
Each destination is notified by a specific set of notification triggers.
To change the notifications sent to each destination it's possible to configure them in the workflow itself.
The notification triggers for notifier can't be missing Activated, and only part of the destinations can send an Acknowledged notification.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        An issue is acknowledged by a user either through the issue page, notification, or a third party integration
      </td>
    ...
    <tr>
      ...
      <td>
        An incident in the issue has moved from High priority to Critical which caused the issue to turn to a Critical priority issue
      </td>
...
<CollapserGroup>
  ...
  >
    ...
    * (Optional) To select specific notifications mark which notifications you want to get for each notifier. Otherwise, the default notifications will be sent to each notifier.
  </Collapser>
</CollapserGroup>
