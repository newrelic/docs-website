...
tags:
  ...
  - Incident workflows
metaDescription: 
  "Incident workflows in New Relic let you customize webhooks to establish two-way integrations with external incident management tool."
redirects:
...
With incident workflow control when and where you want to receive notifications about issues, tunnel the right information to the relevant person/team, and enrich your issue's notifications with additional New Relic ata.
...
The maximum number of enrichments per workflow is 10. The maximum number of variables an enrichment can contain is 1.
</Callout>
...
<table>
  <thead>
    ...
    <tr>
      <td>
        Getting the first element by writing:
      </td>
<td>
```
...
<tr>
    <td>Chain the element into a comma delimited string:</td>
    <td>
```
{{#each annotations.description }}{{this}},{{/each}}
```
    </td>
    </tr>
    </tbody>
...
An example of how the field population would look like for this example is below:
...
2. Open and download this XML file (which includes the business rule for Incident Workflows).
