...
<Callout variant="important">
  The maximum number of workflows you can add is 50 per [environment](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence#1-configure-environment), and 50 per account.
</Callout>
...
<Callout variant="tip">
  To set up notifications on `WARNING` violations, filter by issues with a priority equal to `HIGH (Warning)`.
</Callout>

4. Build a query by selecting an attribute, operator and value/s to narrow down the right issues you want to be passed on.

5. Optional: Enrich your data.
