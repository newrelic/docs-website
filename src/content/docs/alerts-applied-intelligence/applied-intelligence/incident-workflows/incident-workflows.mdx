...
3. Workflows filters the issues you want to send to the relevant destination. You can build a query to filter the right issues.
...
9. Optionally, test your workflow. We'll use existing data from your account to test what you've configured and send a sample notification. The test only works if there are existing issues that match the filter. If there isn't any data, you'll see this message: `We don’t see any issues matching your filter. That doesn’t mean it won’t work`. 
