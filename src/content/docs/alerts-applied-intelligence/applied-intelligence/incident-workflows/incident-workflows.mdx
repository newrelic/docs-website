...
<Callout variant="tip">
 The workflows filter will auto populate with data from past issues as well as some static values from the condition metadata (i.e.: condition name). You may not find the value you're looking for, in such case type in the value and select 'Enter new'.
You may see a Warning that the filter doesn't match any past issue, which is Ok. 
</Callout>
...
<Callout variant="important">
  The initial limit to the number of workflows you can add is 1000 per account and the intial limit to filter size is 4096 characters per workflow Please reach out to your account representitive if you would like this number to be increased.
</Callout>
