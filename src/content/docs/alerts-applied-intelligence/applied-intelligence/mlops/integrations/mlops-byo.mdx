...
New Relic has entered the [MLOps](/docs/alerts-applied-intelligence/applied-intelligence/mlops/get-started/intro-mlops/) observability space, and is now allowing data scientists and machine learning engineers to monitor and observe the performance and effectiveness of their machine-learning models in a production environment. 
...
1. **Inference data:** Stream your model’s feature and prediction values. Choose between: 
  a. **“Online” instrumentation:** Stream the values while the model is being invoked in production by easily wrapping your model through the `wrap_model` function. The data is streamed automatically on each invocation.
  b. **"Offline" instrumentation:** Send the data (features and predictions) as a dataset (as an np.array, or pandas dataframe).
...
1. **Get your API key:** Use [ingest-keys](http://ingest-keys), either the license key or the Insights insert key.

2. **Stream your data to New Relic** by using the [new-relic-ml-performance-monitoring](LINK) package. Follow the example below to see how to send data from your code.
...
3. **View your data in the New Relic Platform:**
  ...
  * **Create your own dashboard:** Build your own [dashboard](https://docs.newrelic.com/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/) to visualize the metrics you sent, or view the distributions of your features and predictions. See an example dashboard below.

  * **Explore entities:** When you stream data to New Relic, an entity of the type `machine learning model` will be automatically created, one per each model name. You can explore your model entities by selecting **Explorer** on [New Relic One](https://one.newrelic.com), and going to the Machine Learning section on the left navigation menu. 
...
1. **Run the example notebooks ([notebook example](link coming soon)):** You will have to define your api insert key ([ingest-keys](http://ingest-keys)) as an environment variable, or send it as a parameter.  
