...
Our [incident intelligence](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence) uses logic to correlate your [issues](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/alerts-ai-overview-page/#issues). These logical operations are called **decisions**. We have built-in decisions, and you can create and customize your own on the applied intelligence **Decisions** UI page. The more optimally you configure decisions, the better we will group and correlate your issue events, resulting in reduced noise and increased context for on-call teams.
...
For the [incident](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/alerts-ai-overview-page/#incidents) events being sent into applied intelligence from your various alerting engines, the most recently created and active issues for these incidents are available for correlation. Correlation occurs between two events whenever the criteria in any decision are met.
...
### What types of logic can be used in a decision?

At a high level, decisions allow you to define logic based on time (duration between events), frequency (number of events), context (metadata structure and values), and [topology](#topology) (entity relationships).

Decisions can be added to your account in three types:
* **Global decision**: A broad set of [default decisions](#global-decisions) is enabled when you start using applied intelligence. A global decision marks Creator as New Relic AI with a globe icon.
* **Suggested decision**: Correlation engine constantly evaluates your event data to [suggest decisions](#suggested-decisions) that capture correlation patterns to reduce noise. You can preview [simulation](#simulations) results of a suggested decision, and choose to activate.
* **Custom decision**: [Customize decisions](#customize) based on your use case to enhance correlation effectiveness. Decision UI gives you flexibility to configure all dimensions in a decision.
...
To get started, click **Suggested decisions** panel on the topic of **Decisions** UI page. You'll see the logic behind the suggested decision, and the estimated correlation rate by clicking **Why was this suggested**.
...
Define your logic filters. Remember correlation occurs between two incidents. If no filters are defined, all incoming incidents will be considered by the decision. 
Define your filters for the first segment of incidents, and the second segment of incidents. Types of filter [operators](#operators) range from substring matching to [regex matching](#regex) to help you target the incident events you want and exclude those you don't.
...
### Step 3: Apply topology correlation (optional)
