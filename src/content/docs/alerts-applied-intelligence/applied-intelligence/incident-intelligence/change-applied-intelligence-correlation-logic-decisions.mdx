...
<table id="decision-guidelines">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Topology correlation](#topology) is currently in limited release. To use it, you'll first use the aiTopologyCollector APIs to define a graph of your entities and their relationships. Event correlation using topology operators works as follows:

        1. First, steps 1 and 2, if configured, are evaluated. All pairs of incidents where steps 1 and 2 are true, and that are also within the defined time window in advanced settings, are gathered.
        2. Then we attempt to connect each incident to one of the vertices in the topology graph using a vertex's defining attributes and all attributes in the incident.
        3. Then all pairs of vertices are tested to using the "topologically dependent" operator to determine if these vertices are connected to each other.

        [Learn more about setting up topology correlation](#topology).
      </td>
    </tr>
...
When you are [building a custom decision](#build-decision), you can use topology correlation to find relationships between incident sources to determine if [events](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence#issue-feed) should correlate. Our topology correlation correlates incidents if they came from two entities that are dependent on each other within 5 hops in a connectivity graph. This is a powerful way to increase the quality of your correlations and the speed at which they're found.

### Requirements [#topology-requirements]

Topology correlation is currently in limited release. Requirements:

- Access to Incident Intelligence
- Granted access by a New Relic representative
...
1. A topology graph has been created using the NerdGraph aiTopologyCollector APIs.
2. Vertices with "defining attributes" exist in the topology and they have been connected together with edges to indicate relationship.
3. The defining attributes on the vertices are also present on your incident events within Incident Intelligence.

### How does topology correlation work? [#topology-explained]

Here's a step-by-step explanation of how topology correlation works:

1. First, all relevant incidents are gathered. This includes incidents where [decision-logic steps 1 and 2](#build-decision) are true, and that are also within the defined time window in advanced settings.

![New Relic topology explained 1](./images/topology-1.png 'topology-1.png')

<figcaption>
  In this example, all of the incidents in the dotted-line selection have met
  these requirements: they've gone through the [decision logic in steps 1 and
  2](#build-decision), and all contextual comparisons made in [decision logic
  step 2](#build-decision) are true.
</figcaption>

2. Next, we attempt to associate each incident to a vertex in the topology graph using a vertex's defining attributes and the available attributes on the incident.

![New Relic topology explained 2](./images/topology-2.png 'topology-2.png')

<figcaption>
  Here's an example of the steps for associating incidents with the information
...
### Create your topology graph [#create-topology-graph]

To create and maintain your topology graph use the aiTopologyCollector APIs in [Nerdgraph](https://docs.newrelic.com/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) to create vertices and connect them together with edges. Note that deleting a vertex will remove all edges connected with it.

![New Relic topology explained 4](./images/topology-4.png 'topology-4.png')

Here are definitions of key topology concepts:

- **Vertices:** You can think of a vertex as an entity and the source from which your incident events are coming from or describing the problematic symptom about. To create a vertex you need:
  ...
  - Defining attributes: A vertex require a set of special attributes (key/value pairs) to allow it to be associated with incoming incident events. Unique identifiers are recommended such as entity GUIDs or other IDs.
  - Vertex class (optional): An optional class can be set such as `application`, `host`, or `datastore`. This allows your decision logic to require the available vertices the "topologically dependent" operator is allowed to traverse looking for a connectivity path to those you define. In other words, "test if vertex A is connected to vertex B traversing only application type vertices".
- **Edges:** An edge is simply a connection between two vertices. Use edges to describe how everything is connected together. Important concepts:
  ...
  - `directed`: This is not currently used, but future functionality may rely on it. This is a boolean indicating that the edge connection goes only in one direction. The direction, if thought of as an arrow, starts at the `from vertex name` and points at the `to vertex name`.

![New Relic topology explained 5](./images/topology-5.png 'topology-5.png')

### Add attributes to incident events [#add-attributes]

You may want to connect an incident to vertices by defining attributes. In the example topology graph under [Topology explained](#topology-explained), each vertex has a defining attribute "CID" with a unique value. Incoming incident events are inspected to see if they have a CID attribute and value. If that is true, and the attribute and value are also present on a vertex in the topology graph, the incident and vertex are associated.
