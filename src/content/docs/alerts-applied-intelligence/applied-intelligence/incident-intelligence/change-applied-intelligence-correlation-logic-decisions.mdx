...
import nraiDecisionsPage from 'images/NRAI_Decisions_Page.png'

import screenShot20200727At65041Am from 'images/Screen-Shot-2020-07-27-at-6.50.41-AM.png'

import decisionBuilderFilter from 'images/decision-builder-filter.png'

import decisionBuilderContextual from 'images/decision-builder-contextual.png'

import decisionBuilderTopology from 'images/decision-builder-topology.png'

import decisionBuilderName from 'images/decision-builder-name.png'

import decisionBuilderSettings from 'images/decision-builder-settings.png'

import decisionPolicy from 'images/decision-policy.png'
...
With applied intelligence's correlation logic, alerts and events from any source are automatically grouped into related issues.  As events come into your system they are instantly correlated based on time, alert context, and relationship data. If multiple events are related then our correlation logic will funnel the related incidents into a single, comprehensive [issue](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence#issue-feed). This ensures that your team is alerted to relevant issues as soon as possible while also reducing distracting and redundant alerts.

We call this correlation logic **decisions**. We have built-in decisions but you can also create and customize your own on the decisions page **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions**. The more you configure your decisions to best suit your needs,  the better New Relic can correlate your incidents, reduce noise and provide increased context for on-call teams.

<img
  title="NRAI_Decisions_Page.png"
  alt="A screenshot that shows applied intelligence decisions UI."
  src={nraiDecisionsPage}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Correlate > Decisions**: Our UI shows how each decision correlates incidents.
</figcaption>

### What is correlation and how does it work? [#what-is-correlaton]

Your most recent and active incidents are available for our correlation logic. For example, let's say your system has received two alerts saying a synthetic monitor is failing in Australia and London. These two alerts will have created their own, unique issues. Those issues will generate their own, unique incidents based on your teams existing [incident creation policy](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/specify-when-alerts-create-incidents/#preference-target). New Relic's correlation logic will then test those incidents against each other to find similarities. In this case, it's the same monitor that is failing across multiple locations, so New Relic will merge both incidents into a single incident that contains each relevant event.
...
New Relic's correlation logic is available to your team in three different decision types:
...
* **Custom decision**: Your team can customize decisions based on your use case to enhance correlation effectiveness. New Relic's decision UI gives you flexibility to configure all dimensions in a decision.

## Review your active decisions [#decisions]

Decisions determine how incident intelligence correlates issues together. To review existing decisions:

1. Go to [one.newrelic.com](https://one.newrelic.com) &gt; **Alerts & AI** &gt; **Correlate**, and click **Decisions**.
...
New Relic will routinely analyze your decisions for their efficacy as well as other best practices, and make recommendations for your review.

### Global decisions [#global-decisions]

Global decisions cover a variety of correlation scenarios. Review descriptions to determine fit for your use cases.
...
<table id="global-decision-descriptions">
  ...
  <tbody>
    <tr>
      ...
      <td>
        Correlation activated because the New Relic NRQL violation target entity names, and NRQL query are the same respectively. Does not apply to REST source. Relevant events from the same [NRQL alert condition](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions) will be identified. It helps relate issues like same transaction query latency deviation, etc.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the New Relic non-NRQL violation target entity names are the same. Does not apply to REST source. Non-NRQL entity refers to [entity](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/), typically APPLICATION, HOST types, see [New Relic GitHub repo on entity synthesis](https://github.com/newrelic/entity-definitions#entity-definitions). Relevant issues from the same entity will be identified. For example, host high memory issue and host not-reporting issue could be highly possible due to the same cause.</td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the New Relic [condition IDs](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/violation-event-attributes/#attributes) are the same. For example, cpu usage increase with related services will trigger incidents from the same cpu usage condition, and thus be identified. This logic is valuable beyond [alert policy issue creation preference option](/docs.newrelic.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/specify-when-alerts-create-incidents#preference-options) for one issue per condition, due to condition-level granularity and flexibity in defining correlation time window.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the New Relic [condition names and titles](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/violation-event-attributes/#attributes) are the same respectively. This is a refined option by comparing titles in addition to conditions to reveal tighter relevance with the same alert message.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the kubernetes deployments are the same. Many incidents are from single deployment changes. This decision is to reduce issues from the same troublesome kubernetes entity deployment.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because custom application name, policy and custom id are the same respectively. This is to reduce application issues, particularly cater to custom tag users. Learn more about [tags](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/). Customed tag id could be defined by condition family id or other id values used as a key to identify connections between data.
      </td>
    </tr>
...
The data from your selected sources is continuously inspected for patterns to help reduce noise. Once patterns have been observed in your data, decisions are suggested that would allow these events to correlate in the future.

To get started, click **Suggested decisions** panel on the topic of **Decisions** UI page. You can see the logic behind the suggested decision, and the estimated correlation rate by clicking **Why was this suggested**.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Correlate > Decisions**: Suggested decision block. If you have less than 5000 incidents per month, you probably won't have suggested decisions.
</figcaption>

To enable the suggested decision, click **Activate decision**. The decision will appear in your decision collection marked Creator as New Relic AI with a robot icon. If the decision isn't relevant to your needs, click **Dismiss**.

## Create custom decisions [#customize]

You can reduce noise and improve correlation by building your own custom decisions. To start building a decision, go to [one.newrelic.com](https://one.newrelic.com) &gt; **Alerts & AI** &gt; **Correlate** &gt; **Decisions**, and click **Add a decision**.
...
Correlation occurs between two incidents. If no filters are defined, all incoming incidents will be considered by the decision. Define your filters for the first segment of incidents, and the second segment of incidents. Filter [operators](#operators) range from substring matching to [regex matching](#regex) to help you target the incident events you want and exclude those you don't.

<img
  title="Decision - filter your data"
  alt="A screenshot of the decision builder showing how to filter incidents."
  src={decisionBuilderFilter}
/>

<figcaption>
  Click **Samples** to view past incidents that met these filters for each segment in the right-hand side panel.
</figcaption>
...
After you configure your decision logic, give it a recognizable name and description. This is used in notifications and other areas of the UI to indicate which decision caused a pair of incidents to be correlated together. If you do not want to update default advanced settings in the next step, click **Create decision** to finish creation.

<img
  title="Decision - give it a name"
  alt="A screenshot of the decision builder showing how to name a decision."
  src={decisionBuilderName}
/>
...
1. Go to [one.newrelic.com](https://one.newrelic.com) &gt; **Alerts & AI** &gt; **Analyze** &gt **Issues & activity**, then click **Incidents** tab.
2. Check the boxes of incidents you'd like to correlate. Then, at the bottom of the incident feed, click **Correlate incidents**.
...
For New Relic's applied intelligence, topology is a representation of your service map: how the services and resources in your infrastructure relate to one another.
