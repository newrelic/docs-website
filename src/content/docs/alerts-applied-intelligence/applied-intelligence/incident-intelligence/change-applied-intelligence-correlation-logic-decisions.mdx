...
Decisions determine how incident intelligence correlates incidents together. By default, a broad set of [global decisions](#global-decisions) is enabled when you start using incident intelligence.

To review existing decisions:

1. Go to **[one.newrelic.com](https://one.newrelic.com)> Alerts & AI >  Incident intelligence > Decisions**.
...
<table id="global-decision-descriptions">
  ...
  <tbody>
    <tr>
      ...
      <td>
        Correlation activated because the New Relic NRQL violation target entity names, and NRQL query are the same respectively. Does not apply to REST source. Relevant incidents from the same [NRQL alert condition](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions) will be identified. It helps relate incidents like same transaction query latency deviation, etc.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the New Relic non-NRQL violation target entity names are the same. Does not apply to REST source. Non-NRQL entity refers to [entity](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/), typically APPLICATION, HOST types, see [New Relic GitHub repo on entity synthesis](https://github.com/newrelic/entity-definitions#entity-definitions). Relevant incidents from the same entity will be identified. For example, host high memory incidents and host not-reporting incidents could be highly possible due to the same cause.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because the kubernetes deployments are the same. Many incidents are from single deployment changes. This decision is to reduce incidents from the same troublesome kubernetes entity deployment.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because custom application name, policy and custom id are the same respectively. This is to reduce application incidents, particularly cater to custom tag users. Learn more about [tags](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/). Customed tag id could be defined by condition family id or other id values used as a key to identify connections between data.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Correlation activated because incidents have similar titles, and are from the same entity. This is to reduce incidents from the same entity that are caused by similar [alert conditions](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions/).
      </td>
    </tr>
...
You can reduce noise and improve correlation by building your own custom decisions. To start building a decision, go to **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Incident intelligence > Decisions**, then click **Add a decision**.
...
1. Go to **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Issues & Activity > Incidents** tab.
...
3. Then, the pairs of vertices which were associated with incidents are tested using the "topologically dependent" operator to determine if these vertices are connected to each other. This operator checks to see if there is any path in the graph that connect the two vertices within five hops.
...
<CollapserGroup>
  <Collapser
    ...
    title="Alerts"
  >
    By integrating incident intelligence with your alerts incidents, you can get context and correlations from what you're monitoring.
