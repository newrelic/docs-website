...
import environmentadminaccess from 'images/environment-admin-access.png'
...
*2. `@aaa-bot` -
In slack, message the `@aaa-bot` directly. Write “help” in order to see the available commands.
...
If the user doesn't have the permissions to view workflows they will see the following error (we have a task to make this more user friendly):
...
For missing **entitlements**,  contact the `@hero` in the `#help-product_and_subs` slack channel to add the entitlement to the relevant account.
