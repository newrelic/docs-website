...
To find your incident intelligence settings, go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > General**.

## Grace period policy (Correlation only) [#grace-period]

You can set a grace period for issues that are candidates for our correlation logic. The grace period is the time between when an issue is created and when it's possible for that issue to create notifications. Grace periods are used to cut down on the number of unnecessary notifications your team sees.  

For each priority level, you can set the grace period from 0 to 20 minutes. The longer the time period, the more time our system has to analyze and correlate new activity with past activity.
