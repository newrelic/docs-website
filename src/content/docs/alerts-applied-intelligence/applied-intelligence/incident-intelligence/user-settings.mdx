...
The grace period for issue that are candidtats for correlation is used to cut down on the number of unnecessary notifications you see. Specifically, the grace period is the time between when an issue is created and when it's possible for that issue to create notifications. 
