...
You can set a grace period for issues that are canidates for our correlation logic. The grace period is the time between when an issue is created and when it's possible for that issue to create notifications. Grace periods are used to cut down on the number of unnecessary notifications your team sees.  
