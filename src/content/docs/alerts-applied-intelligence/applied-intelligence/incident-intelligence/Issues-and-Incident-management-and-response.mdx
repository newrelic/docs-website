...
import accountsEntityMap from 'images/accounts_screenshot-crop_entity-map.webp'
...
import accountsIncidentIntelligenceTimeline from 'images/accounts_screenshot-crop_incident-intelligence-timeline.webp'
...
import accountsIssueFeed20222 from 'images/accounts_screenshot-full_issue-feed-20222.webp'


## Issue feed [#issue-feed]

The **Issues** page is where you can find an overview of all your issues, along with helpful information about them. You can also click any individual issue for more detail, including its analysis summary, event log, and details about correlated issues.
...
The **Issues & activity** page provides you with bottom line insights so you can first understand the problem, and then minimize the time to resolve.
...
4. **Number of Incidents:** At the top of the incident activity section, you’ll see the total number of incidents within the issue
5. **Incident List:** On the left side of the incident activity section, you’ll see the incident activity list, which displays the most relevant information about each incident such as the priority, state, name of the incident, the date and time it was created, and the duration. You can also sort the list by duration, incidents that are muted, newest to oldest and critical to low incidents. Finally, you can filter by only open incidents by enabling the 'Show open only' button. 
6. **Incident Activity Chosen:** Clicking on an incident from the incident activity list will open the incident on the right in a full view mode, which will include information like alert policy, the alert condition, and the alert condition type.
...
8. **User actions (above the chart):** Clicking on an incident from the incident activity list will display certain user actions that you can take.
If this condition has a Runbook URL defined, it will be there, an entity overview button that redirects the user to the entity summary page with the incident time window (any entity type), 'See errors' button will appear only when there are error groups from Errors inbox that related to the same entity and occurred on the same time window as the incident.
You will also. see a 3 dots button that has a dropdown menu that contains the option to close a certain incident (only when the issue has more than 1 incident) and the incident payload. 
9. **Incident Entity Section (below the chart):** Clicking on an incident from the incident activity list will also include the impacted entity, entity type and the account name of this incident belongs to.
You will also find a list of tags (entity & condition tags) and their values, click on the 'show all' button to see all the tags displayed. 
...
## Issue timeline & Log [#visual-timeline]
