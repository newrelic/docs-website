...
You can use the incident event REST API to report custom events to New Relic. 

The incident API is built on our [event API](/docs/data-apis/ingest-apis/introduction-event-api). 

The incident event API is an asynchronous endpoint. This means you can send a large volume of POSTS, reliably, with a low-response latency.
...
1. Generate a [license key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) for the account you want to report data to.
...
Use curl to send your JSON payload.

```bash
gzip -c example_incidents.json | curl --data-binary @- -X POST -H "Content-Type: application/json" -H "X-Insert-Key: <var>YOUR_INSERT_KEY</var>" -H "Content-Encoding: gzip" 
https://staging-insights-collector.newrelic.com/v1/accounts/<var>YOUR_ACCOUNT</var>/events
```

```bash
Response 
{"success":true, "uuid":"<var>YOUR_NEW_RELIC_ACCOUNT_ID</var>"}%
```

Once you've successfully sent your incident event data, see it in the UI or via [NRDB, the New Relic database](/docs/data-apis/get-started/nrdb-horsepower-under-hood).

### Example NRDB query for incident events [#nrdb-example]

You can query the New Relic database with [Nerdgraph, our GraphQL API](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/).
...
For descriptions of our value types, see [our event API JSON guidelines](/docs/data-apis/ingest-apis/introduction-event-api/#json-guidelines).
    ...
    <tr>
      ...
      <td>
        **REQUIRED**
        ...
        Aggregations tags with reserved keywords are filtered out. For example, in a JSON payload with

          `aggregationTag.priority: 2`

        this key-value pair will be omitted in the incident event with these reserved keywords: 
    ...
    <tr>
      ...
      <td>
        **REQUIRED**
        ...
        Whether the event should trigger a new incident or resolve an existing one. 
      </td>
    </tr>
