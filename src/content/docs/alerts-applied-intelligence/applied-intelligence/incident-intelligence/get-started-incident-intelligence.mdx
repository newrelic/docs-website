...
An environment is an account representing one or more accounts. It allows you to store cross-account applied intelligence actions such as correlation, configuration, and data into a particular account. The environment account should be one that your team's users have permissions to and are authorized to view the data in.

It is important to note that, by default, each account is in its own environment if it was created after November 2021. This is the recommended setting. If your account was enabled on incident intelligence prior to November 2021, you would have one environment. You can change the settings as needed. Custom environments should be created only if cross-account actions are required.

**What is stored in the incident intelligence environment account?**
* Workflows (configuration)
* Decisions (configuration)
* Incidents
* Issues created from incidents
* Workflow enricher queries are executed here

<img
  title="The incident intelligence environment account."
  alt="A screenshot of the incident intelligence environment account dropdown."
  src={environment}
/>

<figcaption>
  Use the environment account dropdown to see your cross-account applied intelligence activity.
</figcaption>
...
When moving accounts from one environment to another, the configurations aren't deleted, but they may not be available in the new environment.
...
Consider giving read access to all users in the incident intelligence environment account.
</Callout>
...
<img
  title="Set up an environment with NerdGraph."
  alt="A screenshot of the GraphiQL NerdGraph UI."
  src={accountDiagram}
/>
