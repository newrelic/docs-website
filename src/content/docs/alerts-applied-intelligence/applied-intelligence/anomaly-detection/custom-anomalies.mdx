...
You can create custom anomalies thresholds from an [alert condition](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy/). Here are some tips for setting anomaly thresholds:

* Set the [anomaly direction](#anomaly-direction) to monitor incidents that happen either above or below the anomaly.
* Set the preview chart to either [2 days or 7 days](#preview-chart) of displayed data. This isn't applicable for [NRQL alert conditions](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries).
...
* Optional: You can create a [**Warning** threshold](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition/#threshold-levels) (the darker gray area around the anomaly).
* For NRQL alerts, see the [allowed types of NRQL queries](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#threshold-types).
...
1. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI** and select **Alerts conditions (policies)** in the left navigation panel.
...
6. After selecting 'anomaly' for threshold type, you'll need to set your [aggregation window](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/streaming-alerts-key-terms-concepts/#window-duration) duration. This sets the criteria that you want to be notified on. 

  To do this you'll need to:

    * Set whether you want the condition to check for upper incidents, lower incidents, or both.
    * Set the behavior that will trigger an incident (for example: "deviating for more than five minutes") 
    * Adjust how sensitive the condition is to fluctuations in the data source by adjusting the sliding bar to more incidents or fewer incidents. This bar calculates how many standard deviations away from the prediction you want to be notified.
...
The single time series shows the anomaly, threshold band, and one or more incident regions, if there are any.
