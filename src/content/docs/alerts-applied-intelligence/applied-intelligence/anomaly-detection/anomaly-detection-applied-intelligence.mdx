...
At New Relic, we have our own developers who monitor the health of our own application. Our developers want to know if there are any outliers in our system's performance as soon as possible because we want to make sure that our customers are always able to access the data they need. New Relic's anomaly detection monitors three of the golden signals: throughput, error rate and latency. Since we monitor our own application, New Relic developers can understand our baseline performance for these golden signals. 

Let's say that one afternoon there's a spike in response time and it's taking longer than usual for our customers to access the New Relic application. New Relic will flag this anomalous behavior because our latency metric data has deviated from its baseline. This doesn't necessarily mean there is a problem, it is just and indication that New Relic has registered something out of the ordinary in our system and it thinks we should take a deeper look.

We can learn about this unusual behavior in a few ways.  First, our team uses the anomaly dashboard so we can see what changed and when. 
...
## How anomalies function in your system [#anomaly-functions]
