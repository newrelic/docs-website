...
**Correlation**: Anomalies are correlated with other incidents, creating one issue that can be viewed in the issues feed. Anomalies provide additional context as to what is changing around the time of an issue. 

**Receiving notifications:** You have the option to send notifications when we detect anomalous changes in throughput, error rate, or response time. We reccomend to send notifications to selected Slack channels or email channels. When the anomaly goes back to normal, a recovery message is sent. If you don't want to receive notifications, you still have access to the data via [NRQL query](#anomaly-events).
...
note: configurations will be going away soon in favor of workflows.
...
Alternatively you can set up a workflow (//docs.newrelic.com/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/ ) and use pre-defined destination (https://docs.newrelic.com/docs/alerts-applied-intelligence/notifications/destinations/) channel of your choice, however we recommend sending anomalies to a low priotiy notification channel such as slack or email. To use workflows, you will need to do the following.
...
In addition to viewing anomalies in the [Anomalies Feed], anomalies are correlated to other incidents and grouped into issues. Anomalies provide context to what has changed around a critical issue. Out of the box anomalies will be correlated with incidents of the same entity type. Correaltes unique to your use case can be added using Decisions (//docs.newrelic.com/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/change-applied-intelligence-correlation-logic-decisions/)
