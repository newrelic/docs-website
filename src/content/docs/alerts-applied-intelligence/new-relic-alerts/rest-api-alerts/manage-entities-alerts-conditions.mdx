...
4. Review the policy output; for example:
   ...
   ```
   {
     "policies": [
       {
         "id": 85,      <---<<<  <code><var>{policy_id}</var></code>
...
6. Use API requests to add entities to or remove entities from the policy's condition:
   ...
   ```
   curl -X PUT 'https://api.newrelic.com/v2/alerts_entity_conditions/<code><var>12345</var></code>.json' \
        -H 'Api-Key:<a href="/docs/apis/rest-api-v2/getting-started/introduction-new-relic-rest-api-v2#api_key"><var>$API_KEY</var></a>' -i \
        -H 'Content-Type: application/json' \
        -G -d 'entity_type=<b>Application</b>&condition_id=<var>234567</var>'
   ```
