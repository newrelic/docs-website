...
Once you set up an alert condition, the entity status color changes to green to let you know that it's being monitored. If the entity status color is gray, that can mean a few things: your entity is not being monitored, there was a loss of signal, or the data collection might be intermittent or incomplete. Some customers like to use gap-filling strategies to ensure that the entity status remains green in the case of incomplete or sporadic data. 
  
Gap-filling strategies provide a method for filling gaps of missing data points between valid ones with estimated values, ensuring that alert conditions can still function effectively even with gaps in the data stream.
  ...
  * <DoNotTranslate>**Cost**</DoNotTranslate>: In an alert condition you can use a `WHERE` clause to filter for the specific transactions in an application you want to count. For example, if you're monitoring your `WebPortal` application and there are no transactions during a designated time period; the alert condition will not register any data, the entity status will change from green to gray, and, in some cases, incidents won't be created or closed. Suppose this gap is less than two hours. In that case, it can be addressed using the gap-filling strategy outlined in our [Create NRQL alert conditions documentation](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#example-null-zero). For cases where the data gaps exceed  2 hours or if a new data point is expected, we recommend using [signal loss threshold](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-conditions/#lost-signal). **Remember that this strategy can increase compute power usage, affecting your bill**.

If up to 2-hour gap, this can be addressed using the gap-filling strategy... documentation. 
