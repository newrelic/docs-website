...
Creating a buffer:
  ...
  * <DoNotTranslate>**Cost**</DoNotTranslate>: In an alert condition you can use a `WHERE` clause to filter for the specific transactions in an application you want to count. For example, if you're monitoring your `WebPortal` application and there are no transactions during a designated time period, the alert condition will not register any data, the entity status will change from green to gray and, in some cases, incidents won't be created or closed. This can be addressed using the gap-filling strategy outlined in our [Create NRQL alert conditions documentation](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#example-null-zero). However, keep in mind that this strategy can increase data ingest, affecting your bill.. 
