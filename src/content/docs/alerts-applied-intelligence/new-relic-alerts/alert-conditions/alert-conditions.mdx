...
Creating a buffer:
 ...
 >
  ...
  Once you set up an alert condition, the entity status color changes to green to let you know that it's being monitored. If the entity status color is gray that can mean a few things: your entity is not being monitored, there was a loss of signal, or the data collection might be intermittent or incomplete. Some customers like to use gap-filling strategies to make sure that, in the case of incomplete or intermittent data, the entity status remains green. 
  
  Gap-filling strategies provide a method for substituting missing data points with estimated values, ensuring that alert conditions can still function effectively even with gaps in the data stream. 
