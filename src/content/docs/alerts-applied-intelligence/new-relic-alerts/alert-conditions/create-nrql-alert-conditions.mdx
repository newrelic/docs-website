...
<Callout variant="caution">
  A small number of our older charts do not include the option to create an alert condition.
</Callout>
...
<Callout variant="tip">
  Baseline alert condition thresholds do not support editing the aggregation window. They use the 1 minute default.
</Callout>
...
Gap filling lets you customize the values to use when your signals do not have any data. You can fill gaps in your data streams with one of these settings:

* **None**: (Default) Choose this if you do not want to take any action on empty aggregation windows. On evaluation, an empty aggregation window will reset the threshold duration timer. For example, if a condition says that all aggregation windows must have data points above the threshold for 5 minutes, and 1 of the 5 aggregation windows is empty, then the condition won't be in violation.
