...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        NRQL conditions evaluate data based on how it's aggregated, using aggregation windows from 15 seconds to 15 minutes, in increments of 15 seconds. For best results, we recommend using the event flow or event timer aggregation methods.
        
        The implicit `SINCE ... UNTIL` clause specifying which minute to evaluate is controlled by your [**delay/timer**](#time) setting. Since very recent data may be incomplete, you may want to query data from 3 minutes ago or longer, especially for:
