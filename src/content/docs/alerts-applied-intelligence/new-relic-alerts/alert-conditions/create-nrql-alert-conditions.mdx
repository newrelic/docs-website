...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert Conditions & Policies > + New alert condition > Write your own query**: Follow the steps in the UI to create an alert condition based on a NRQL query.
</figcaption>
...
Add a NRQL query for data point you want to monitor. You can alert on basic NRQL queries that return a numerical value. For example, if you want to get alerts for transaction errors, add this query:

```sql
SELECT count(*) FROM TransactionErrors
```

For help with writing queries, see [How to use NRQL: the mechanics of querying](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-how-nrql-works/).

Once you add the query, click **Run > Next**.

</Step>

<Step>
### Set thresholds and adjust your signal

Customize [alert thresholds](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition/) to determine when alerts get triggered, and make adjustments for frequency, choppiness, or gaps in your signal. Here, you can configure the [data aggregation](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/choose-your-aggregation-method/), the [gap-filling strategy](#data-gaps), and the [evaluation delay](#evaluation-delay).

Once you finish your alert configuration, click **Next**.
...
Give your alert condition a name and connect it to a policy that will determine when incidents are created from threshold violations. You can select an existing policy or create a new one. 

If you select **New policy**:
        
1. Add a [meaningful name](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy/#best-practices-policies) for the policy with a maximum of 64 characters.

2. Select the way you want to [group your incidents](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/specify-when-alerts-create-incidents/).

3. Configure the [additional settings](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/your-first-nrql-condition/#additional-settings) if you need to.

4. Click **Save & set up notifications**.
...
4. Follow the steps explained in the [Create a NRQL alert condition from a policy](#alert-condition-from-policy) but be aware that [Step 2](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#step2) is a bit different because, now, the query is built with the data you've previously configured.
