...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        NRQL conditions evaluate data based on how it's aggregated, using aggregation windows from 30 seconds to 15 minutes, in increments of 15 seconds. For best results, we recommend using the event flow or event timer aggregation methods.
...
## Condition changes can reset condition evaluation [#evaluation-resets]

When you edit alert conditions in some specific ways (detailed below), their evaluations are reset, meaning that any evaluation up until that point is lost, and the evaluation starts over from that point. 

For [baseline conditions](#threshold-types), this has the following effects:
* Any baseline learning it has made is lost and it starts over again gathering baseline data.
* If you're using a threshold of "for at least x minutes," there will be a delay of at least x minutes before any violations may be reported because the condition's evaluation window has been reset.
