...
<figcaption>
  Go to **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Alert conditions (Policies) > (select ) > then **Add a condition**. Loss of signal is only available for NRQL conditions.
</figcaption>
