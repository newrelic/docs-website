...
In 2021, we made it easier to create NRQL alert conditions from charts and queries throughout New Relic One. To demonstrate, we'll show an example of how to create an APM external service condition using NRQL.
...
8. This will launch the NRQL alert condition builder in a modal with the query pre-populated.
...
2. In **Define your signal**, observe that any `LIMIT`, `SINCE...AGO`, and `TIMESERIES` clauses are removed so that the NRQL query syntax is valid.
   ...
   <Callout variant="tip">
     ...
     If is is not correct, you may need to close out of the modal and double-check that the correct account is selected in the account picker in the upper-left of the page.
   </Callout>
