...
The delay feature protects you against inconsistent data collection. It gives the alert condition a little wiggle room before deciding to create an incident. If in any given minute your data arrives at New Relic with timestamps from only a single minute, then a low delay setting is enough. On the other hand, if during that minute New Relic receives data points with timestamps from several minutes past or several minutes forward, then your signal is more inconsistent and will need a higher delay setting.
  ...
  Key things to note about gap filling:
  * We generally recommend leaving the **gap-filling strategy** set to **none**.
  * A popular use of gap filling is to insert a value of `0` for windows with no data received.
  * If you're using an anomaly threshold, be aware that the gap-filling value will be equal to the number of standard deviations that you'd like the value to be filled. So, if you add gap-filling with a value of `0` for an anomaly threshold type, it will fill the gaps with a value that is 0 standard deviations from the last value seen.
...
/>
  ...
  >
    If you want to create a new policy for this alert condition, here's your chance. It's a fine balance between ensuring that you learn about any issues with `pagviews` on your `WebPortal` application as quickly as possible and making sure that you don't get so many alerts that your developers experience fatigue and start missing out on important information because of information overload. 
    ...
    Or you could create one issue per condition. This means that any time the **Response time: Example app** condition opens an incident, all those incidents will be rolled into one issue that is connected to your condition. For this specific use case, you should choose this option because it meets the primary goal, which is to monitor latency issues with `pageviews`. 

    Or you could create an issue for every incident. This option is the most noisy but can work well if you like to send information to an external system. 
