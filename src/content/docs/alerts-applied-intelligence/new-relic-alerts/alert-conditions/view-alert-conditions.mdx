...
This doc will walk you through how to edit and change [an _existing_ alert condition](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/view-alert-conditions/#edit-existing-alert-condition), and creating [new_ alert condition_](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/view-alert-conditions/#create-alert-condition]). 

## Create a new alert condition [#create-alert-condition]

For this example, we are going to show you how to manually create a new alert condition using the **Alert Condition** UI. 

But, if you're just beginning to use New Relic, we suggest creating an alert condition from a chart. To learn how to do this, review our [tutorial about creating your first alert](/docs/tutorial-create-alerts/create-an-alert/). 
...
```
Using this query for your alert condition tells New Relic that you want to know the average pageviews from your `WebPortal` application.
...
We're going to customize these advanced signal settings for our condition that is looking for latency issues in the `WebPortal` application. 
...
## Additional details [#additional-details]
  ...
  If there are any latency issues with `pageviews`, we'd like to receive a notification as soon as possible. The best quick and efficient action is to create an alert condition that will open an incident if `pageviews` drop below a normal amount.
