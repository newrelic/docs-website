...
<CollapserGroup>
  ...
  >
    See [APM metric alert conditions](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/apm-metric-alert-conditions) for tips about creating <InlinePopup type="apm" /> conditions using NRQL.
  </Collapser>
  ...
  >
    ...
    1. Select **Web transactions percentiles** as the type of condition for your <InlinePopup type="apm" /> app's condition, then select a single app. (To alert on more than one app, create an individual **Web transactions percentiles** condition for each.)
