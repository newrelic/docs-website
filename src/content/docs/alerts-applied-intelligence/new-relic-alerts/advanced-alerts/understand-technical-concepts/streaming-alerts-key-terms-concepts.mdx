...
The streaming alerts platform checks for incidents based on data that’s present or missing in your stream of data, or [signal](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal), coming into New Relic.
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      If your data arrives consistently and predictably, use **event flow**. If your data arrives inconsistently and unpredictably, use **event timer**.
    </figcaption>
  </Collapser>
</CollapserGroup>
...
Once the delay/timer time has elapsed, New Relic aggregates the data into a single data point. Alerts then evaluates the data point using the NRQL condition in order to determine whether it meets the incidents threshold criteria.

Even if a data point meets the criteria for an incident, an incident may not be triggered. An incident is only triggered when data points consistently meet the threshold criteria over a period of time. This is the threshold duration. If the data points are in violation for an entire threshold duration, we'll send you a notification based on your policy settings.
...
<table class="alternate">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        A specific duration that determines if an incident is created. If your specified NRQL condition meets the threshold criteria over the threshold duration, a incident occurs.
...
In order to avoid unnecessary notifications, you can choose how long to wait before you're notified by a loss of signal incidents. You can use loss of signal detection to open incidents and be notified when a signal is lost. Alternately, you can use a loss of signal to close incidents for ephemeral services or sporadic data, such as error counts.
