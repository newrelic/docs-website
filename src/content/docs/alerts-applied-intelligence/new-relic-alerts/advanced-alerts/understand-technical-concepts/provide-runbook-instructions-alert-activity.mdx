...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Alert conditions (policies) > (select a policy) > (select a condition)**: You can add a runbook URL so that personnel handling the incident that triggered the alert will know what to do.
</figcaption>

To edit the runbook URL associated with a condition:

1. Go to **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Alert conditions (policies) > (select a policy) > (select a condition)**.
