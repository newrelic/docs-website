...
Here's an example of an app listed on the <InlinePopover type="apm" /> index that **is not** associated with any conditions. Its color-coded health status is light grey, which indicates no alert conditions are set up for that entity.
...
Follow [standard procedures](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/basic-alerting-concepts#workflow) to add it to an existing condition or to create a new condition for it.
