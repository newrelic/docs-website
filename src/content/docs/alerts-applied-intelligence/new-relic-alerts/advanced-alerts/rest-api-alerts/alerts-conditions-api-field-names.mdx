...
<CollapserGroup>
  ...
  >
    ...
    In the UI, under **Advanced signal settings**, this is the **Use sliding window aggregation** toggle.

    Default is based on the current window duration. The slide by interval must divide evenly into your aggregation window duration. Slide by must also be less than the window duration. 
