...
<CollapserGroup>
  ...
  >
    ...
    So, when the returned type is a static condition, it will return the `valueFunction` in the object, and when the returned type is a baseline condition, it will return `baselineDirection` instead. Here is an example response:
