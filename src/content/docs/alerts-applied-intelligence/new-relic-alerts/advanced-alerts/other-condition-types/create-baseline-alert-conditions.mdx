...
![A screenshot of an example baseline condition.](src/images/new-relic-alerts-baseline-thresholds.png 'An example baseline condition.')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > AI & Alerts > Policies >
  (create or select policy) > Create alert condition:** Baseline alert
  conditions give you the ability to set intelligent, self-adjusting thresholds
  that only generate violations when abnormal behavior is detected.
</figcaption>
...
- Use the slider bar to adjust the ![icon-alert-critical.png](src/images/icon-alert-critical.png 'Alerts v3: Critical alert icon') **Critical** threshold sensitivity, represented in the preview chart by the light gray area around the baseline. The tighter the band around the baseline, the more sensitive it is and the more violations it will generate.
- Optional: You can create a ![icon-alert-warning.png](src/images/icon-alert-warning.png 'Alerts v3: Warning alert icon') [**Warning** threshold](/docs/alerts/new-relic-alerts/defining-conditions/set-thresholds-alert-condition#threshold-levels) (the darker gray area around the baseline).
...
![A screenshot of an example baseline condition with facets.](src/images/faceted-baseline-condition-query.png 'An example of a baseline condition with facets.')

<figcaption>
  NRQL alerts conditions support faceted baselines. In the chart legend, click a
  time series to scope it to a single facet.
</figcaption>

Once you've run a faceted NRQL query for a baseline condition, you can scope your results to a single baseline.

![A screenshot of a baseline scope to a single time series facet.](src/images/baseline-one-facet.png 'An example of a baseline condition scoped to a single time series facet.')
...
![New Relic baseline alerts direction image examples](src/images/new-relic-baseline-alerts-direction-image-examples_0.png 'new-relic-baseline-alerts-direction-image-examples.png')
