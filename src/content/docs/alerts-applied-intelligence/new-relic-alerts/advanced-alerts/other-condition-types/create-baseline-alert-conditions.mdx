...
* Use the slider bar to adjust the <img
    src={iconAlertCritical}
    title="Alerts v3: Critical alert icon"
    alt="icon-alert-critical.png"
  /> **Critical** threshold sensitivity, represented in the preview chart by the light gray area around the baseline. The tighter the band around the baseline, the more sensitive it is and the more violations it will generate.
* Optional: You can create a <img
    src={iconAlertWarning}
    title="Alerts v3: Warning alert icon"
    alt="icon-alert-warning.png"
  /> [**Warning** threshold](/docs/alerts/new-relic-alerts/defining-conditions/set-thresholds-alert-condition#threshold-levels) (the darker gray area around the baseline).
