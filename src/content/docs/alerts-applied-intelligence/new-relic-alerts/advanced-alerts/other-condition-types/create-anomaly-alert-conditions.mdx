...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > AI & Alerts > Policies > (create or select policy) > Create alert condition:** Anomaly alert conditions give you the ability to set intelligent, self-adjusting thresholds that only generate incidents when abnormal behavior is detected.
</figcaption>
