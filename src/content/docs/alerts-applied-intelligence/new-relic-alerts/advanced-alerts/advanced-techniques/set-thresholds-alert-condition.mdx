...
<CollapserGroup>
  ...
  >
    Different condition types have different minimum time intervals. For example, some condition types have a minimum time interval of 5 minutes (for example, <InlinePopup type="apm" /> metric alert conditions), and others have a minimum time interval of 1 minute (for example, NRQL alert conditions). The same happens with the maximum time interval, where the amount is 120 minutes.
  </Collapser>
