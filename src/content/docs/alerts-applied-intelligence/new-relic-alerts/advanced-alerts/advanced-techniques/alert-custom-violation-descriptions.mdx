...
![Screen capture of the alerts condition description field used to pass useful, custom information along.](src/images/alerts_condition_description_field.png 'The alerts condition description field with some example content.')

<figcaption>
  In the **[one.newrelic.com](https://one.newrelic.com)** top nav, click
  **Alerts & AI**, click **Alert policies**, **(select a policy)**, then
  **(select a condition)**: Click **+ Add custom violation description** to open
  the field.
</figcaption>
...
![New Relic alert violation description ](src/images/new-relic-alert-violation-description-screenshot.png 'new-relic-alert-violation-description-screenshot.png')
...
<Callout variant="important">
  Note that the Slack convention for channel-wide notifications in Incoming
  Webhooks is to use a `!` prefix instead of `@`. For example, `!here` or
  `!channel`.
</Callout>
