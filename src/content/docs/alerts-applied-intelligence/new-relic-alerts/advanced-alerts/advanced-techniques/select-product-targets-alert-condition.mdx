...
<figcaption>
  <DoNotTranslate>**Alerts & AI > Alert conditions (Policies) > (selected policy) > (selected condition) > Notification channels:**</DoNotTranslate> As part of the policy setup process, select one or more entities monitored by the selected product that will be the target entity for the condition.
</figcaption>
