...
You can create, enable, disable, and manage muting rules. Go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**. Enable or disable muting rules at any time.
...
<Callout variant="tip">
  Before creating muting rules, you'll need to [create policies and conditions](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy) that generate notifications.
</Callout>
...
Next, build the incidents filter. You can use a subset of [incident event attributes](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes) (specifically `accountId`, `conditionId`, `conditionName`, `entity.guid`, `nrqlEventType`, `nrqlQuery`, `policyId`, `policyName`, `runbookUrl` (as `conditionRunbookUrl`), `tags.<NAME>`, and `targetName`) and [sub-condition operators](#sub-conditions). Values can be compared against one of your incident attributes, such as an alerts policy ID or a condition name.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        The set of individual expressions that define which incidents to target. A muting rule condition has:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        An issue opens because of an incident that is **not** muted
      </td>

      <td/>

      <td>
        An “Open issue” notification will be sent (default).
      </td>
    </tr>

    <tr>
      <td>
        An issue opens due to an incident that **is** muted
      </td>

      <td/>

      <td>
        An “Open issue” notification will **not** be sent (muted).
      </td>
    </tr>
    ...
    <tr>
      <td
        ...
        style={{ backgroundColor: "rgb(238, 238, 238)" }}
      >
        **Event**: Incident added to an open issue
      </td>
    </tr>

    <tr>
      <td>
        A new **muted** incident is added to an already open issue
      </td>
    ...
    <tr>
      <td>
        A new **unmuted** incident is added to an open issue
      </td>

      <td>
        An “Open issue” notification **has not been sent**
      </td>

      <td>
        An “Open issue” notification **will be sent**.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        An “Open issue” notification **has already been sent**
      </td>
    ...
    <tr>
      ...
      <td>
        A “Close issue” notification **will not be sent**.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        An “Open issue” notification **has been sent**
      </td>

      <td>
        A “Close issue” notification **will be sent**.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        An “Issue acknowledged” notification **will not be sent**.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        An “Issue acknowledged” notification **will be sent**.
      </td>
    </tr>
  </tbody>
</table>
...
For a step-by-step guide on how to set up your workflows, check out an example demo below:
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Incidents.**
    </figcaption>
  </Collapser>
  ...
  >
    Incidents and issues are marked as **Muted** in the **State** column:
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Events**
    </figcaption>
  </Collapser>
...
<Callout variant="tip">
  Note that all sub-condition operator values are case-sensitive. For example, if you use `policyName STARTS_WITH 'PROD'` a policy name that starts with 'Prod' will not get picked up.
</Callout>

* `EQUALS`: Where the supplied value equals the incident attribute value.
* `NOT_EQUALS`: Where the supplied value does not equal the incident  attribute value.
* `IN`: Where the incident attribute value is present in a list of supplied values (up to 500).
* `NOT_IN`: Where the incident  attribute value is not present in a list of supplied values (up to 500).
* `CONTAINS`: Where the supplied value string is present in the incident attribute value.
* `NOT_CONTAINS`: Where the supplied value string is not present in the incident  attribute value.
* `ANY`: **Caution:** A condition with this operator will mute **all** incidents on the account.
...
* `NOT_STARTS_WITH`: Where the incident attribute value does not begin with the supplied value string.
* `ENDS_WITH`: Where the incident attribute value ends with the supplied value string.
* `NOT_ENDS_WITH`: Where the incident attribute value does not end with the supplied value string.
