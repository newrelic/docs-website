...
<Callout variant="tip">
  Before creating muting rules, you'll need to [create policies and conditions](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy) that generate incident notifications.
</Callout>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * `attribute`: A single attribute within a incident. Go here for a list of [Violation event attributes](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes).
...
1. Identify individual incidents after they're created, but before an incident is opened.
2. Override their default condition to indicate that they should be "muted."

Currently, muting an incident means that the normal alerts incident lifecycle will be maintained, except that an incident that only contains muted incidents will not send out any notifications.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        An incident opens because of an incident that is **not** muted
      </td>
    ...
    <tr>
      <td>
        An incident opens due to an incident that **is** muted
      </td>
    ...
    <tr>
      <td
        ...
        style={{ backgroundColor: "rgb(238, 238, 238)" }}
      >
        **Event**: Incident added to an open incident
      </td>
    </tr>

    <tr>
      <td>
        A new **muted** incident is added to an already open incident
      </td>
    ...
    <tr>
      <td>
        A new **unmuted** incident is added to an open incident
      </td>
...
A triggered incident has a 1:1 ratio with an incident so if an incident is muted then the matching incident will be muted as well. 
...
For a step-by-step guide on how to setup your workflows, check out an example demo below:
...
<Callout variant="tip">
  Note that all sub-condition operator values are case-sensitive. If you use, for example, `policyName STARTS_WITH 'PROD'`, a policy name that starts with 'Prod' will not get picked up.
</Callout>
