...
<CollapserGroup>
  ...
  >
    To create a muting rule and return the muting rule's `id` for muting incidents from <InlinePopover type="infrastructure" /> conditions where the target host is `host-1` or the incident was from synthetic or mobile monitoring:
