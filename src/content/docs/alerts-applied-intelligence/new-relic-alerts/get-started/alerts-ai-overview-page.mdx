...
A New Relic incident metadata comes from up to 3 sources
- it inherits all the tags from the alert condtion that triggered it
- if the alert condition NRQL query contains a facet or where condition, all the facets values and where conditions are added as tags
- if the result of the alert condition NRQL query can be mapped to a single entity, all the tags from that entity are also added

[External incident](/docs/data-apis/ingest-apis/event-api/incident-event-rest-api/) medadata is what was received as tag.*.

The incident feed includes all of your environment’s incidents. These include incidents coming from New Relic, PagerDuty, and all of our other supported sources.
...
### Incident feed columns [#incident-feed]
...
On the **Issues and activity** page, anomalies are outliers in your system's performance and operation that might be the sign of a problem. See every anomaly for the last seven days in one place. You can filter your anomalies by anomaly state, application, configuration, configuration type, and category.
