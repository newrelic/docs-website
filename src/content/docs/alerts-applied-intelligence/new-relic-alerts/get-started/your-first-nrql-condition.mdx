...
For this example, we're going to create an alert that notifies our team of any latency issues with web transaction time. An alert condition is essentially a container that you create to define what conditions need to be met before you're notified of any unusual behavior. 

So, in our case, if we want to make sure that web transactions never take longer than 50 milliseconds then we're going to build a container, or an alert condition, that recognizes whenever web transaction time goes beyond 50 milliseconds and creates an incident. 
...
<CollapserGroup>
  ...
  >
  Sometimes an incoming signal can be lost and it's important to understand if this is simply a delay or if it's an indication of a wider problem. Our **loss signal threshold** lets New Relic know how many seconds the system should wait from the time the last data point was detected before considering the signal to be lost. If the signal does not return before the time limit that you set, you can choose one or both of two actions: open a new incident and close all open incidents. You can close any related open incidents if the lost signal supersedes other incidents on this entity, or if the signal loss was expected.
...
The delay feature protects you against inconsistent data collection. It gives the alert condition a little wiggle room before deciding to create an incident. If, in any given minute, your data arrives at New Relic with timestamps from only a single minute, then a low delay setting is enough. On the other hand, if, during that minute, New Relic receives data points with timestamps from several minutes past or several minutes forward, then your signal is more inconsistent and will need a higher delay setting.
  ...
  If you have a more inconsistent data set that sends signals to New Relic once every twenty-four hours, then, we'd recommend customizing this feature based on your team's specific needs. 
...
## Connect your condition to a policy [#connect-condition-policy]
  ...
  The policy is the sorting system for the incident. When you create a policy, you're creating the tool that organizes all of your incoming incidents. You can connect policies to **[worflows](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/)** that tell New Relic where you want all this incoming information to go, how often you want it to be sent, and where. 
...
<CollapserGroup>
  ...
  >
    ...
    First, we could create a policy that only attaches a single issue per policy, our default option. One issue per policy reduces noise but also requires immediate action. But this means that if we've multiple conditions attached to this policy, not just this one, **Response time: Example app**, then no matter what, **all incidents in this policy will be grouped into one single issue**.

    Or we could create one issue per condition. This means that any time the **Response time: Example app** condition opens an incident, all those incidents will be rolled into one issue that is connected to our condition. For our specific use case, this is the option we're going to choose because it meets our primary goal: to learn about any latency issues with web transaction time. 
...
<CollapserGroup>
  ...
  >
If you'd like to add a runbook url, here is your chance. 
  </Collapser>
</CollapserGroup>
