...
1. Go to the APM throughput chart (or another chart that is built from a NRQL query). Click the `...` icon on that chart and click **Create alert condition**. Complete the steps there. For tips on the options, see [NRQL alert conditions](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions). You'll likely be adding this to a new policy: for tips on those options, see the [alert policy docs](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/specify-when-alerts-create-incidents/#preference-policy).
2. Set up a workflow, which controls how alert violations are turned into notifications. From [one.newrelic.com](https://one.newrelic.com), click **Detection and AI**, and then click **Workflows**. For tips on using that, see the [workflow docs](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows) and [this post on best practices for workflows](https://discuss.newrelic.com/t/relic-solution-patterns-for-implementing-alerts-workflows/188605).
3. Set up one or more destinations (the places notifications are sent). From **Detection and AI**, click **Destinations**. For tips, see the [destination docs](/docs/alerts-applied-intelligence/notifications/destinations).
...
You can also write your own [NRQL query](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) and use that as the basis for a NRQL alert condition. 

The following will walk you through how to create a condition. 
