...
A violation will automatically close when the targeted [entity](/docs/using-new-relic/welcome-new-relic/get-started/glossary#entity) returns to a non-violating state for the time period indicated in the condition's thresholds. This wait time is called the [recovery period](/docs/using-new-relic/welcome-new-relic/get-started/glossary#recovery-period).
...
You can manually close an open violation. This is useful, for example, when an entity has violated the threshold, but it no longer exists and also doesn't have a [violation time limit](#time-limit). (A time limit would automatically close a long-lasting violation.)
