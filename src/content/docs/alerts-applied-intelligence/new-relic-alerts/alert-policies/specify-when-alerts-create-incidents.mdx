...
* **Incidents**. Incidents are events that describe the symptoms affecting your system over time. See [View event details for incidents](/docs/alerts-applied-intelligence/new-relic-alerts/alert-incidents/view-event-details-incidents/) for more information.
...
<figcaption>
  Go to **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert Policies > (select a policy)**. The selected policy page shows how <InlinePopover type="alerts" /> rolls up issues for alert notifications and UI details. The default option is **One issue per policy**. To choose a different option for this policy, click **Issue creation preference**.
</figcaption>

3. Modify the **Issue creation preference**, select your choice of available [options](#preference-options), and then **Save**.
