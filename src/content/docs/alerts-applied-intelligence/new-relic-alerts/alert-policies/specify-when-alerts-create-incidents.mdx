...
When a policy condition breaks a threshold, an incident is opened and an issue record can be created with detailed information to help you respond efficiently. The issue record may also include any other incidents occurring after the opening incident.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        [One issue per condition and signal](#preference-condition)
      </td>

      <td>
        An issue will be created for every signal on every condition in your policy.
      </td>

      <td>
        * More notifications
        * Useful for policies containing conditions that focus on [entities](/docs/new-relic-solutions/get-started/glossary/#alert-target) that perform the same job; for example, hosts that all serve the same application(s)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * Useful if you need to be notified of every incident in an entity or if you have an external system where you want to send alert notifications
      </td>
    </tr>
  </tbody>
</table>
...
<figcaption>
  When you select the default option **One issue per policy** as your issue preference, it will group all condition incidents into the same issue.
</figcaption>
...
* If a target breaches a condition for the alert policy, an incident opens and it's created an issue record.
* If the same target breaches another condition for the alert policy, another incident opens and it's created another issue record.
* If additional targets open incidents on the same condition after the issue opens, these incidents are added to the issue record already opened for that condition.
...
<figcaption>
  When you select the option **One issue per condition** as your issue preference, it will group all incidents by any target for a specific condition into separate issues.
</figcaption>


## One issue per condition and signal [#preference-target]
