...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        One issue will be open at a time for every signal on every condition.

        * The most notifications
        * Useful if you need to be notified of every signal in violation or if you have an external system where you want to send alert notifications
      </td>
    </tr>
  </tbody>
</table>
...
<figcaption>
  When you select **One issue per condition and signal** as your issue preference, it will only group incidents based on every signal on every condition. A new issue will be created for every signal on every condition that opens a new incident. Note that this may result in _some_ incident rollup, as it is possible to get both a warning and a critical incident on the same signal from the same condition.
</figcaption>
