...
import journeyWebTrans from 'src/images/journey_screenshot-web-transactions.webp'
...
Once you've installed an agent, go to **[one.newrelic.com](https://one.newrelic.com/nr1-core)** and select your app. If you don't see much data just yet, step away for a while and let the agent gather real-time data as your application runs.
...
The tiles below explain what each segment represents and will take you to instructions on how to optimize them if you see any spikes or high load:

<DocTiles>
  <DocTile title='Blue line: application code' path="/docs/journey-app-slow/problematic-transactions" >The blue line represents application code, specifically time spent in your application related to the code that isn't external or database calls.</DocTile>
  <DocTile title='Yellow segment: database queries' path="/docs/journey-app-slow/slow-database-queries" >The yellow segment represents time spent calling and waiting on database queries. </DocTile>
  <DocTile title='Green segment: external services' path="/docs/journey-app-slow/external-services/" >The green segment represents the time spent calling a HTTP service or endpoint. This could be a HTTP endpoint as part of the same app, another service as part of the same system, or it could be a completey external third party API.</DocTile>
</DocTiles>
