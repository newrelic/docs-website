...
## Scope out the problem with app data  [#diagnose]

Our database UI displays charts built with NRQL queries, creating timeslice data from the `apm.service.datastore.operation.duration` metric. These charts track the response time of your top database processes and measure the time consumed by your operations, their query time, and your app's throughput. 
