...
<CollapserGroup>
  ...
  Follow these steps to visualize your custom metric through the metrics explorer:
    ...
    4. Select your custom metric and the dimension of the metric previously selected.

  </Collapser>
  <Collapser
    className="freq-link"
    id="nrql"
    title="Generic queries with the `newrelic.timeslice.value` metric"
  >

  Use the dimensional metric named `newrelic.timeslice.value`. Learn how to use it [here](/docs/data-apis/understand-data/metric-data/query-apm-metric-timeslice-data-nrql/#generic-queries).
</Collapser>
</CollapserGroup>
