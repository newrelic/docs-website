...
Need to perform a more detailed search? Read how to [query your data](/docs/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data/).
...
Once you've created your custom metric, you have these ways to visualize it:

<CollapserGroup>
  <Collapser
    className="freq-link"
    id="metrics-explorer"
    title="Through the metrics explorer"
  >
  ...
  Follow these steps to visualize your custom metric through the metrics explorer:
    ...
    4. Select your custom metric and dimensions.
...
Follow these steps to visualize your custom metric through [NRQL](/docs/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data/):

    1. Go to **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Query your data** and select the **Query builder** tab.

    2. Select your account and add your query. For example:

    ```sql
      FROM Metric SELECT count(newrelic.timeslice.value) WHERE appName = 'your_appName' AND metricTimesliceName = 'Custom/Cart/ChargeAmount'
    ```
