...
Optionally, you can set **Good**, **Warning**, **Approaching critical**, **Critical**, or **Neutral thresholds** on billboards. Once set, if your service is working correctly, the chart's full box is green. When a threshold is crossed, the chart turns yellow (warning) or red (critical). Access the threshold fields on the right side of the chart edit pane in the query builder.
