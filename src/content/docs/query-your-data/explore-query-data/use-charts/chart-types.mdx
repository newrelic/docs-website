...
Pie charts require a `FACET` clause when used with the following [aggregator functions](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#functions): `average`, `apdex`, `min`, `max`, `percentage`, and `percentile`. Without a `FACET`, these aggregator functions cannot be visualized as pie charts. For example:
- **Pie compatible**: `SELECT percentile(duration) FROM Transaction FACET appName`
- **Not pie compatible**: `SELECT percentile(duration) FROM Transaction`
