...
{/* TODO (PM: musandizaga@newrelic.com / anovikov@newrelic.com): Add the production screenshot showing thresholds, then uncomment this figure. Staging-only assets can't be published. The figcaption is drafted below.
<img
  title="A gauge chart."
  alt="A screenshot of a gauge chart that fills an arc to show where a single value falls within a defined range."
  src="/images/queries-nrql_screenshot-crop_gauge-chart.webp"
/>

<figcaption>
  The gauge chart fills an arc to show where a single value sits between its minimum and maximum. Threshold colors and markers indicate when the value enters a warning or critical range.
</figcaption>
*/}
...
{/* TODO (PM): Confirm the exact threshold label terminology shown in the gauge settings UI. Internal config keys are success, warning, severe, and critical; the labels above mirror the billboard chart's documented vocabulary on this page. */}
...
<table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
        Select what the gauge shows. You can choose **Auto**, **Value only**, **Name only**, **Value and name**, or **None**.
        </td>
      </tr>
      <tr>
        <td>
...
{/* TODO (PM): Confirm the exact UI labels for the display mode options (config values are auto, value, name, all, and none). */}
