...
You can customize the predictions by manually setting the training model's seasonality, hyperparameters, and adding the `USING` and `BY` keywords. To learn more about NRQL and its syntax, refer to [NRQL documentation](/docs/nrql/get-started/introduction-nrql-new-relics-query-language). Generally, most users will only ever want to adjust the seasonality. The other hyperparameters for the Holt-Winters algorithm are accessible and changeable, but default values are optimized based on the historical data for your metric.

    <CollapserGroup>
    <Collapser id="seasonality" title="Set seasonality.">
      We currently support forecasts using non-seasonal and seasonal versions of 
      ...
      * `seasonality` Specifies whether you want to use a known season length, automatic seasonality detection, or no seasonality (`<season length>`, `AUTO`, `NONE`). 
      ...
      <Collapser id="hyperparameters" title="Set Holt-Winters model hyperparameters.">
