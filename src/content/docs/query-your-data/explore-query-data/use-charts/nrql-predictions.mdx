...
<Collapser id="hyperparameters" title="Customize the query using hyperparameters and its smoothing levels">
 As <DNT>**Holtwinters**</DNT> is the only prediction model we support currently, you can customize the chart by quering only the hyperparameters and its smoothing factors with `holtwinters`. For example, `PREDICT holtwinters(alpha: 1)`.
...
You can set the smoothening factors for each hyperparameter from 0 to 1. Higher values indicate more weight on recent data, while lower values give more weight to historical data.
