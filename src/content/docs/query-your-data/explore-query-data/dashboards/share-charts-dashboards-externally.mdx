...
When using live dashboard URLs, it's important to understand the associated costs and how to manage them effectively. These URLs are billed for every query run when accessed, as part of the Compute Pricing Plan (Add the appropriate document link here). Customers can monitor their usage costs through the Compute Usage UI (Add the appropriate document link here).
