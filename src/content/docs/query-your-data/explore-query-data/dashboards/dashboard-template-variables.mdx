...
* Clutter the variables bar, such as [nested variables](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Control dashboard logic without requiring viewer input
* Derive values automatically from other variables

To hide a variable, use the <DNT>**Hide on variables bar**</DNT> option when [defining a template variable](#step1). To configure this programmatically, use the `hiddenOnVariablesBar` field in the [NerdGraph dashboards API](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).

<Callout variant="tip">
  To hide multiple variables at once, use the [NerdGraph dashboards API](/docs/apis/nerdgraph/examples/nerdgraph-dashboards) to update multiple variables programmatically, as the UI only allows hiding variables one at a time.
</Callout>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Optional. Hide the variable from the variables bar in View mode. Hidden variables continue to function normally. In Edit mode, hidden variables display with a <Icon name="fe-eye-off"/> icon. For more information, see [Show and hide variables](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>
