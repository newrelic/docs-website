...
The <DNT>**Include variable**</DNT> toggle allows you to set a desired value and include or exclude that value in your dashboards. See the [template variables](#create-variables) section for more info.
...
You can hide template variables from the variables bar in View mode while they continue to function normally behind the scenes. In Edit mode, hidden variables display with a <Icon name="fe-eye-off"/> icon, so dashboard editors can identify and manage them.

Consider hiding variables that:

* Add clutter to the variables bar, such as [nested variables](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Power dashboard logic without needing viewer input
* Derive values automatically from other variables

To configure visibility, use the <DNT>**Hide on variables bar**</DNT> option when [defining a template variable](#step1). For programmatic access, use the `hiddenOnVariablesBar` field in the [NerdGraph dashboards API](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).

## Requirements and limitations [#requirements]

Template variables can only be used in the context of making widgets for custom dashboards. For this reason, some query-related features don't work. For example, the `Export dashboard as PDF` option doesn't support widgets with variables, and [<DNT>Lens</DNT>](/docs/new-relic-lens/query-data) SQL queries don't support template variables. See [NRQL variables](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#with-as-nrql-var) for using variables in a NRQL query.
...
Define a template variable that you'll use in a NRQL query to create a widget.
    ...
    1. From a new dashboard without variables, click the edit <Icon name="fe-edit-2"/> button, and then click <DNT>**+ Add variable**</DNT> button.
    ...
    2. Complete the <DNT>**Add variable**</DNT> workflow. Following are some rules and tips for each of the fields.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Optional. Enable this toggle to hide the variable from the variables bar in View mode. The variable continues to function normally behind the scenes. In Edit mode, hidden variables display with a <Icon name="fe-eye-off"/> icon. For more information, see [Show and hide variables](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>
...
<CollapserGroup>
  ...
  >
    ...
    You can add variables to widget titles either through the UI when editing a widget or by using the [dashboard JSON](/docs/query-your-data/explore-query-data/dashboards/manage-your-dashboard/#manage-json).
    ...
    <img
      title="Screenshot showing the widget edit options"
      alt="Screenshot showing the widget edit options"
      src="/images/variables_in_widget_titles.webp"
  />
  ...
  >
    ...
    **Example 1: Dynamic text content**
    ...
    You can include the variable (`{{products}}`) directly in your markdown text to create dynamic documentation that updates based on the selection:
      ...
      <img
      title="Screenshot showing the markdown text and its preview"
      alt="Screenshot showing the markdown text and its preview"
      src="/images/text_edit.webp"
      />
    ...
    **Example 2: Dynamic Mermaid diagrams**
    ...
    You can also use the variable (`{{products}}`) within the markdown text of Mermaid diagrams to create flowcharts that update dynamically:
      ...
      <img
      title="Screenshot showing the markdown text and its mermaid diagram preview"
      alt="Screenshot showing the markdown text and its mermaid diagram preview"
      src="/images/markdown_mermaid_diagram.webp"
      />
