...
To export a dashboard as a PDF or PNG file:

1. In the dashboard's top right corner, click the <DNT>**...**</DNT> icon and select <DNT>**Download dashboard**</DNT>.

2. Choose <DNT>**PDF**</DNT> or <DNT>**PNG**</DNT> as the output format.

3. (Optional) Set the export width and height, in pixels. Both default to 1920 x 1080.

4. Click <DNT>**Download**</DNT>.
...
* Export always uses UTC time.
* Export doesn't support aggregator functions like `average()`.
* Export doesn't support the `COMPARE_WITH` clause.
* Export doesn't always replicate the UI's formatting exactly, which can cause small discrepancies (for example, -1.1E-17 renders as 0 in the UI but not in the CSV).
* Export doesn't support cross-account query results.
