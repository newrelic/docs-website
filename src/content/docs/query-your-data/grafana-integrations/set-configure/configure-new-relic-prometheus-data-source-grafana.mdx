...
1. In New Relic, [create a new Insights query key](/docs/apis/intro-apis/new-relic-api-keys/#insights-query-key).
   ...
   <Callout variant="important">
     ...
     If your account is in the **JP region**, Insights query keys are not supported. Create a [User API Key](/docs/apis/intro-apis/new-relic-api-keys/#user-key) instead, and use it with the `x-api-key` header in step 7.
   </Callout>
...
7. Under <DNT>**Custom Headers**</DNT>, select <DNT>**Add Header**</DNT>. For US and EU accounts, set the <DNT>**Header**</DNT> name to <DNT>**X-Query-Key**</DNT> and enter the Insights query key from step 1. For JP accounts, set the <DNT>**Header**</DNT> name to `x-api-key` and enter the User API Key from step 1.
...
<CollapserGroup>
  ...
  >
    ...
    **JP region exception**: The JP region does not support Insights query keys. Use a [User API Key](/docs/apis/intro-apis/new-relic-api-keys/#user-key) with the header name `x-api-key` instead of `X-Query-Key`.
  </Collapser>
