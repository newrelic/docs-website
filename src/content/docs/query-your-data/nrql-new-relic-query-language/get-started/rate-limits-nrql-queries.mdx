...
Our New Relic query language, NRQL, has rate limits in place for each New Relic organization to ensure that a high rate of queries by one organizations doesn't affect other organizations.
...
* Data Plus: 
  * 2 minutes with UI query
  * Up to 10 minutes with [API queries](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial)
