...
Once the limit has been reached for a given time period, limiting will be imposed on custom queries, widgets on dashboards that run queries, curated UI that includes summary pages of all NewRelic components that try to fetch data with a customer initiated action. This will not impact the alert condition evaluation and notifications in case of incidents, but the UI where the user will be directed to from links on the notification can result in an exception as this UI also tries to fetch data from the backend and is a customer initiated action. After the time period has passed, if query volume drops below the limit, restrictions will be removed automatically
