...
We can separate NRQL query limits into two main types: 

* Limits on the number of queries in a given time range
* Limits on the number of data points inspected in a given time range
...
In this context, the "events" term is used in a general sense to refer to all [NRQL-available data objects](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql#what-you-can-query) that have been inspected in a query. This includes events, metrics, logs, and distributed tracing (span) data.

Every New Relic account has limits on the total number of inspected data points that can be inspected in a given time range. These limits vary depending on which [data option](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing#data-prices) you have: 
...
* Limits on inspected data points (events): one trillion data points per 30 minutes and 100 billion data points per minute
...
The query duration limit is how long a NRQL query can run before it stops and errors out. This limit depends on your [data option](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing#data-prices): 
