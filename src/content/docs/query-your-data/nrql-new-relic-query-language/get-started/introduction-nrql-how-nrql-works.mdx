...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Events and attributes that we report by default have names that contain alphanumeric characters, colons (`:`), periods (`.`), and underscores (`_`). Default-reported names will start with a letter, and won't include nonstandard characters. Custom names that don't follow these guidelines must be enclosed with backticks ``` `` ``` in NRQL queries. For example:
