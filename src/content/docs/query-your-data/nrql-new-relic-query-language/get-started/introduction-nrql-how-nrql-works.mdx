...
The result is a very basic NRQL query using the required clause and statement (`FROM` and `SELECT`), and it provides a list of <InlinePopup type="apm" /> transactions and the associated `appId` for each, as shown below.
