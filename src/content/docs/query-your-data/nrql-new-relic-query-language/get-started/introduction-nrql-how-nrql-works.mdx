...
Before using any tool, it's best to know how to use it, and NRQL is no exception to that rule. There's a process to creating queries with NRQL, and how to structure and write NRQL queries. Understanding the rules of querying with NRQL enables you to get the most from your data. Even if you've never queried anything before, once you understand the basics you can find ways to access (almost) any data you need and visualize them in [charts](/docs/query-your-data/explore-query-data/use-charts/use-your-charts) and [dashboards](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards).
...
Pressing space again causes the interface to suggest available attributes. `appId` is chosen.
...
Here's an example of a slightly more in-depth NRQL query of `Transaction` data, which is reported by [APM](/docs/apm). For this query, `Transaction` is the chosen data type, `Select` is used to determine the average duration, results are grouped by appName using `Facet`, and `Timeseries` is used to display the data over an automated timespane.
