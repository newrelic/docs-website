...
<CollapserGroup>
  ...
  <SideBySide>
  <Side>
Let's start by looking at a single event type in NRDB called **Transaction**, gathered by New Relic APM. Every NRQL query must have `SELECT` and `FROM` clauses: you must `SELECT` some data and tell us where it is `FROM`. Start with this basic query that selects everything from the **Transaction** event type.
