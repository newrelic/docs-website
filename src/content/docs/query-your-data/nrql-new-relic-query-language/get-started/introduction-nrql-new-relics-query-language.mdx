...
The New Relic Query Language (NRQL) is one of the most versatile and powerful tools you can use to [query your data](/docs/using-new-relic/data/understand-data/query-new-relic-data). Don't worry! We'll guide you through everything you need to know about NRQL, including what it is, how to use it, and provide some tips and tricks that will help you get the most out of your queries. Here's a quick video to help introduce you to using NRQL by showing you how to find a query from a [dashboard](docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/) and modify it in the [query builder](docs/query-your-data/explore-query-data/query-builder/introduction-query-builder).
...
If you haven't already, be sure to [sign up for a New Relic account](https://newrelic.com/signup) (it's free, forever). Let's get started!

## What is NRQL?

NRQL is an acronym of New Relic query language. It's a query language similar to ANSI SQL ([see the syntax](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language#syntax)), and is used to retrieve detailed New Relic data and get insight into your applications, hosts, and business-important activity. Have you ever wanted to:
* Create a new chart
...
NRQL allows you to do all of these things (and so many more) quickly and efficiently. You can use NRQL to create simple queries, such as fetching rows of data in a raw tabular form to give insight on individual events. You can also use NRQL to run powerful calculations on the data before it's presented to you, such as crafting funnels based on how end users are using your site or application.
...
NRQL is a powerful tool for accessing your data, and you can use it in many places to its full potential. Those places include: 
...
You can run a NRQL query from anywhere within New Relic by enabling the [NRQL console](/docs/query-your-data/explore-query-data/query-builder/nrql-console) from your user profile. This allows you to quickly query your data without losing your place within the platform.
  </Collapser>
  <Collapser
...
NRQL puts the power of several New Relic data types at your fingertips, including:
...
* The [`Log` data type](/docs/using-new-relic/data/understand-data/new-relic-data-types#log-data) (data from New Relic's log management capabilities)
...
One way to start using NRQL and to understand what data you have available is to go to a NRQL interface (for example, our [query builder](/docs/chart-builder/use-chart-builder/get-started/introduction-chart-builder)), type `FROM`, and press space. The interface will suggest available types of data:
