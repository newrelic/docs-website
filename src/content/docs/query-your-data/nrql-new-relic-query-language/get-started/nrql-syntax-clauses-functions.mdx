...
Note: `aparse()` is case-insensitive.
  ...
  >
    ...
    * cidrFormat - An optional boolean value that is used to determine if the network address output should be formatted
    ...
    <CollapserGroup>
      <Collapser title="Find which subnets are sending the most requests">
        Assuming each record corresponds to a request, the following query returns the subnets which are sending the
        most requests.

        ```sql
        FROM Event SELECT count(*) FACET cidrAddress(ipAddress, 24)
        ```
      ...
      <Collapser title="Query all IP addresses that exist in a CIDR range">
        This query returns all IP addresses in the `ipAddress` attribute that exist within the CIDR range of 10.0.0.0 to
        10.0.0.255.

        ```sql
        FROM Event SELECT uniques(ipAddress) WHERE cidrAddress(ipAddress, 24) = '10.0.0.0/24'
        ```
      </Collapser>
      <Collapser title="Filter IP addresses that exist in a specific CIDR range from query results">
        This query returns a count of all records while excluding records which contain an `ipAddress` value that falls
        into the CIDR range of 10.0.0.0/24 or 10.10.1.0/24.

        ```sql
        FROM Event SELECT count(*) WHERE cidrAddress(ipAddress, 24) NOT IN ('10.0.0.0/24', '10.10.1.0/24')
        ```
      </Collapser>
    </CollapserGroup>
  </Collapser>
