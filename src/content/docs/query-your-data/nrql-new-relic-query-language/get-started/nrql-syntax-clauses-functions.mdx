...
<CollapserGroup>
  ...
  >
    ...
    * Custom events reported via <InlinePopup type="apm" /> agent APIs
      ...
      This clause will be useful when a <InlinePopup type="apm" /> agent reports so many [events](/docs/using-new-relic/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#event-data) that it often passes its [harvest cycle](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#harvest-cycle) reporting limits. When that occurs, the agent begins to sample events.
...
<CollapserGroup>
  ...
  >
    ...
    * Omit [`TIMESERIES`](#sel-timeseries) to generate a billboard showing a single rate value averaged over time.
      ...
      Here's a basic query that will generate a line chart showing the rate of throughput for <InlinePopup type="apm" /> transactions per 10 minutes over the past 6 hours:
