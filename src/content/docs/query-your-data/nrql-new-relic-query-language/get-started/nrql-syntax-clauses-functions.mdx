...
Note: `aparse()` is case-insensitive.
  ...
  >
    ...
    The largest unit of time is the Julian `year`, which is always 365.25 days and does not account for leap-year adjustments.

    <CollapserGroup>
      <Collapser title="Convert an attribute from milliseconds to minutes">
        ```sql
        FROM NrdbQuery SELECT convert(durationMs, 'ms', 'min') AS durationMin
        ```
      </Collapser>
      ...
      <Collapser title="Convert between the stored unit and megabytes ">
        This query assumes you have the unit information stored as a string attribute in the event itself, and that you'd like to standardized the values to megabytes.

        ```sql
        FROM DataSample SELECT convert(dataSize, units, 'MBy')
        ```
      </Collapser>
    </CollapserGroup>
  </Collapser>
