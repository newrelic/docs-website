...
Here are some example queries:
  ...
  >
    ...
    If you include the `WITH TIMEZONE` clause without specifying a time zone in a date time, the provided time zone is honored on the with and until clauses.

    If you don't include the `WITH TIMEZONE` clause, but a time zone in a date time string, your date time string time zone is honored.
    ...
    These are some examples being today 2022-05-19:
    ...
    * Time zone in date time string, using the `WITH TIMEZONE` clause America/Los_Angeles (which is -0700):
