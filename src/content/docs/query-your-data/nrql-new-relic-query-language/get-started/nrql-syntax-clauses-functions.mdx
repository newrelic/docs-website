...
import exampleFacetedLeftJoin from 'images/nrql_screenshot-example-faceted-left-join.webp'

import exampleFacetedInnerJoin from 'images/nrql_screenshot-example-faceted-inner-join.webp'

import exampleColumnarInnerJoin from 'images/nrql_screenshot-example-columnar-inner-join.webp'
...
<CollapserGroup>
  ...
  >
    Use the `JOIN` clause to combine data from one event type with another based on a common attribute or key.
    ...
    * The `ON` clause must immediately follow the subquery.
    ...
    <table id="join-on">
      ...
      <tr>
        ...
        <td>
          Defines the key values to compare in the subqeury and the outer query. The only comparison
          operator allowed is equality.
          * The left-hand side is always the key used in the parent query and may be an attribute or
          function.
          * The right-hand side is used for the subquery value, and must only be an identifier.
        </td>
      </tr>
    ...
    * The use of `TIMESERIES` in the joined subquery is not supported. If your outer query uses
    `TIMESERIES`, keep in mind that the joined subquery is providing a single result for the full
    query timespan.
    * Like all subqueries, you cannot use joined subqueries in alert conditions.
    * While SELECT * is supported in the parent query, it is not supported in the joined subquery.
    * The cardinality of the join is limited to 1:100, meaning a single join key cannot map to up to
    one hundred rows of in the subquery result.

    <CollapserGroup>
      <Collapser title={<><InlineCode>INNER JOIN</InlineCode> with a faceted subquery</>}
                 id="example-faceted-inner-join">
        This query finds the count of events faceted by `browserTransactionNamed` from the
        `PageView` event type, and then by `currentUrl` from the `PageAction` event type. This joins
        the two event types based on the common `session` attribute values.
      ...
      <Collapser title={<><InlineCode>LEFT JOIN</InlineCode> with a faceted subquery</>}>
        ...
        in the `PageView` table that do not have matching `session` values the the results of the
        `PageAction` subquery.
      ...
      <Collapser title={<><InlineCode>INNER JOIN</InlineCode> with a columnar subquery</>}>
        Here we are performing a columnar subquery, with the outer query finding the count of events
        faceted by `currentUrl` from the `PageAction` event type, and then by
        `browserTransactionNamed` from the `PageView` event type. This joins
        the two event types based on the common `session` attribute values.

        Note that the `session` value `34d5ce6acf4c60be` have two `browserTransactionName` values from
        ...
        <figcaption>
          Columnar <InlineCode>INNER JOIN</InlineCode> example
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
