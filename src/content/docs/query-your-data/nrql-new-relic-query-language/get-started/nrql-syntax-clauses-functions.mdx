...
Note: `aparse()` is case-insensitive.
  ...
  <Collapser
    className="freq-link"
    id="func-toTimestamp"
    title={<InlineCode>toTimestamp(timestamp[, pattern [, timezone]])</InlineCode>}
  >
    ...
    * pattern - An optional date time pattern used to try and parse the datestring parameter. See the [Patterns for toTimestamp and toDatetime](#toTimestamp-toDatetime-patterns) section below.
    ...
    * timezone - An optional timezone value that is used to interpret the datestring parameter(ex. PST).
    ...
    Alias: `fromDatetime()` is an alternative name for `toTimestamp()` function.
    ...
    <CollapserGroup>
      ...
      <Collapser id="toTimestamp-toDatetime-patterns" title="Patterns for toTimestamp and toDatetime">
    ...
    <Callout variant="important">
      The UI will automatically detect the `toTimestamp()` value as a timestamp and format it as a date time value.  To display the actual numeric timestamp, wrap the `toTimestamp()` function in a `string()` function.
    </Callout>
  </Collapser>
  ...
  * occurrence - indicates which occurrence of substr of which to return the position.
  ...
  >
    ...
    * pattern - An optional date time pattern used to format the result. See the [Patterns for toTimestamp and toDatetime](#toTimestamp-toDatetime-patterns) in the [toTimestamp()](#func-toTimestamp) section above.
      * This must be a constant string value and will default to `yyyy-MM-dd'T'HH:mm:ss[.SSS][XXX]` if a pattern is not provided.
