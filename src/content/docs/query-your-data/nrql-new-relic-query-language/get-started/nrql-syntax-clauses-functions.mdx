...
<CollapserGroup>
  ...
  >
    ...
    * A pattern string with anchor strings and extract characters, e.g. 'www.*.com' to extract the domain from a URL.
    ...
    extracted string, noted by '*'. aparse() uses the following characters in pattern strings:
    ...
    <CollapserGroup>
      <Collapser title="aparse() specific values">
    ...
    <CollapserGroup>
      <Collapser title="aparse() multiple values">
  ...
  >
    ...
    * Regular expression with capture syntax. Regex expressions in NRQL use Python-like syntax, `r'...'`.
    ...
    Currently, only one capture group is supported.
