...
Here are some example queries:
  ...
  >
    ...
    If you include the `WITH TIMEZONE` clause without specifying a time zone in a date time, the provided time zone is honored on the since and until clauses.

    If you don't include the `WITH TIMEZONE` clause, but you include a time zone in a date time string, your date time string time zone is honored.

    <Callout variant="important">
      The default time zone is always UTC if one is not specified. The raw timestamp values (as seen in the JSON view) in the results are always UTC. The UI displays the results in the time zone you have specified in your account settings.
      A time zone in a time stamp string always works. It supersedes the WITH TIMEZONE zone.
    </Callout>

    For example, the query clause `SINCE Monday UNTIL Tuesday WITH TIMEZONE 'America/New_York'` will return data recorded from Monday at midnight, America/New York time, until midnight Tuesday, America/New York time.
