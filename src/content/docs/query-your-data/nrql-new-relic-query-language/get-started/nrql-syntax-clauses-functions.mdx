...
<CollapserGroup>
  ...
  >
    ...
    The facets are sorted in descending order by the first field you provide in the `SELECT` clause. If you are faceting on attributes with more than 2,000 unique values, a subset of facet values is selected and sorted according to the query type. Note that if a timeseries chart returns no data (NRQL matches no matching data, invalid NRQL, etc.) then it will only show a flat line with the label matching the first table in the `FROM` clause.
  ...
  >
    The `ORDER BY` clause allows you to specify how you want to sort your query results in queries that select event attributes _by row_.
...
Note: `aparse()` is case-insensitive.
  ...
  <Collapser
    className="freq-link"
    id="func-fromDateTime"
    title={<InlineCode>fromDateTime(timestamp[, pattern [, timezone]])</InlineCode>}
  >
    Use the `fromDateTime()` function to parse a timestamp in epoch milliseconds from a datetime string.

    `fromDateTime()` takes the following arguments:
    ...
    * pattern - An optional date time pattern used to try and parse the datestring parameter. See the [Patterns for fromDateTime and toDateTime](#fromDateTime-toDateTime-patterns) section below.
    ...
    * timezone - An optional timezone value that is used to interpret the datestring parameter(ex. PST).
    ...
    <CollapserGroup>
      <Collapser id="partial-datetime-interpretation" title="Partial datetime interpretation when parsing datetime strings">
        <table>
          <thead>
          <tr>
            <th>
          ...
          <tbody>
          <tr>
            <td>Missing time zone</td>
            <td>Will use timezone argument, or WITH TIMEZONE value. Defaults to UTC.</td>
            <td>yyyy-MM-dd HH:mm:ss.SSS</td>
            <td>2023-10-18 15:27:03.123</td>
            <td>2023-10-18T15:27:03.123Z</td>
          </tr>
          <tr>
            <td>Missing datetime field</td>
            <td>
              Any missing time field will be replaced with 0.<br/>
              Any missing date field will be replaced with 1.<br/>
              If a field is present, all higher precedence fields must also be present.<br/>
              Note that day of year and quarter of year patterns are supported.
            </td>
            <td>
            ...
            <td>
              2023-10-18T15:27:03.000Z<br/>
              2023-10-18T15:27:00.000Z<br/>
              2023-10-18T15:00:00.000Z<br/>
              2023-10-18T00:00:00.000Z<br/>
              2023-10-18T00:00:00.000Z<br/>
              2023-10-01T00:00:00.000Z<br/>
              2023-10-01T00:00:00.000Z<br/>
              2023-01-01T00:00:00.000Z<br/>
            </td>
          </tr>
          <tr>
            <td>Time only</td>
            <td>
              If a time pattern is used without a date, the millisecond of the day will be calculated.  Technically, this is the unix timestamp on January 1, 1970.<br/>
            ...
            <td>
              1971-01-01T00:30:00.000Z<br/>
              1971-01-01T01:30:00.000Z<br/>
              1969-12-31T23:30:00.000Z
            </td>
          </tr>
          <tr>
            <td>12 hour time</td>
            <td>If a 12 hour pattern (lower case h) is used, then the am-pm-of-day pattern (a) must also be used. Note, in the datetime string AM/PM must be upper case.</td>
            <td>
              yyyy-MM-dd h:mm a<br/>
              yyyy-MM-dd h:mm<br/>
              yyyy-MM-dd h:mm a
            </td>
            <td>
              2023-10-18 3:27 PM<br/>
              2023-10-18 3:27<br/>
              2023-10-18 3:27 pm
            </td>
            <td>
              2023-10-18T15:27:00.000Z<br/>
              Unsupported pattern<br/>
              null (due to lowercase pm)
            </td>
          </tr>
          <tr>
            <td>Field precedence</td>
            <td>If a field is present, all higher precedence fields must also be present.</td>
            <td>yyyy dd</td>
            <td>2023 18</td>
            <td>Unsupported pattern (dd is day-of-month, and month is missing)</td>
          </tr>
          </tbody>
        </table>
      </Collapser>
      <Collapser id="fromDateTime-toDateTime-patterns" title="Patterns for fromDateTime and toDateTime">
        Patterns are based on a simple sequence of letters and symbols. For example, `d MMM uuuu` will format
        ...
        **Text:** The text style is determined based on the number of pattern letters used. Less than 4 pattern letters
        will use the short form. Exactly 4 pattern letters will use the full form. Exactly 5 pattern letters will use
        the
        narrow form. Pattern letters 'L', 'c', and 'q' specify the stand-alone form of the text styles.

        **Number:** If the count of letters is one, then the value is output using the minimum number of digits and
        without padding. Otherwise, the count of digits is used as the width of the output field, with the value
        zero-padded as necessary. The following pattern letters have constraints on the count of letters. Only one
        letter of 'c' and 'F' can be specified. Up to two letters of 'd', 'H', 'h', 'K', 'k', 'm', and 's' can be
        ...
        **Fraction:** Outputs the nano-of-second field as a fraction-of-second. The nano-of-second value has nine
        digits, thus the count of pattern letters is from 1 to 9. If it is less than 9, then the nano-of-second value is
        truncated, with only the most significant digits being output.

        **Year:** The count of letters determines the minimum field width below which padding is used. If the count of
        letters is two, then a reduced two digit form is used. For printing, this outputs the rightmost two digits. For
        parsing, this will parse using the base value of 2000, resulting in a year within the range 2000 to 2099
        inclusive.

        **ZoneId:** This outputs the time-zone ID, such as 'Europe/Paris'. If the count of letters is two, then the
        time-zone ID is output. Any other count of letters is invalid.

        **Zone names:** This outputs the display name of the time-zone ID. If the pattern letter is 'z' the output is
        the daylight savings aware zone name. If there is insufficient information to determine whether DST applies, the
        name ignoring daylight savings time will be used. If the count of letters is one, two or three, then the short
        name is output. If the count of letters is four, then the full name is output. Five or more letters is invalid.

        If the pattern letter is 'v' the output provides the zone name ignoring daylight savings time. If the count of
        letters is one, then the short name is output. If the count of letters is four, then the full name is output.
        Two, three and five or more letters is invalid.

        **Offset X and x:** This formats the offset based on the number of pattern letters. One letter outputs just the
        hour, such as '+01', unless the minute is non-zero in which case the minute is also output, such as '+0130'. Two
        letters output the hour and minute, without a colon, such as '+0130'. Three letters output the hour and minute,
        with a colon, such as '+01:30'. Four letters output the hour and minute and optional second, without a colon,
        such as '+013015'. Five letters outputs the hour and minute and optional second, with a colon, such as
        '+01:30:15'. Six or more letters is invalid. Pattern letter 'X' (upper case) will output 'Z' when the offset to
        be output would be zero, whereas pattern letter 'x' (lower case) will output '+00', '+0000', or '+00:00'.

        **Offset O:** With a non-zero offset, this formats the localized offset based on the number of pattern letters.
        One letter outputs the short form of the localized offset, which is localized offset text, such as 'GMT', with
        hour without leading zero, optional 2-digit minute and second if non-zero, and colon, for example 'GMT+8'. Four
        letters output the full form, which is localized offset text, such as 'GMT, with 2-digit hour and minute field,
        optional second field if non-zero, and colon, for example 'GMT+08:00'. If the offset is zero, only localized
        text is output. Any other count of letters is invalid.

        **Offset Z:** This formats the offset based on the number of pattern letters. One, two or three letters output
        the hour and minute, without a colon, such as '+0130'. The output will be '+0000' when the offset is zero. Four
        letters outputs the full form of localized offset, equivalent to four letters of Offset-O. The output will be
        the corresponding localized offset text if the offset is zero. Five letters output the hour, minute, with
        optional second if non-zero, with colon. It outputs 'Z' if the offset is zero. Six or more letters is invalid.

        **Optional section:** The optional section markers work mark a part of the pattern as optional. All elements in
        the optional section are treated as optional. When formatting a timestamp (toDateTime()), all optional
        sections will be included in the output datetime string. During parsing, the whole section may be missing from
        the parsed string. Optional sections can be nested.

        **Pad modifier:** Modifies the pattern that immediately follows to be padded with spaces. For example, 'ppH'
        outputs the hour-of-day padded on the left with spaces to a width of 2.

        Any unrecognized letter is an error. Any non-letter character, other than '[', ']', '{', '}', '#' and the single
        quote will be output directly. Despite this, it is recommended to use single quotes around all characters that
    ...
    <CollapserGroup>
      <Collapser title="Parse a datetime string using the default pattern">
        The following query parses the datetime string '2023-10-18T15:27:03.123Z' using the default pattern of `yyyy-MM-dd'T'HH:mm:ss[.SSS][XXX]`.

        ```sql
        SELECT fromDateTime('2023-10-18T15:27:03.123Z') FROM Event
        ```
      </Collapser>
      <Collapser title="Parse a datetime string using the timezone parameter">
        The following query parses the datetime string '2023-11-03 11:00:32' with the timezone set to 'America/Los_Angeles'.

        ```sql
        SELECT fromDateTime('2023-11-03 11:00:32', 'yyyy-MM-dd HH:mm:ss', timezone:'America/Los_Angeles') FROM Event
        ```
      </Collapser>
      <Collapser title={<>Parse a datetime string using the timezone in the <InlineCode>WITH TIMEZONE</InlineCode> clause</>}>
        The following query parses the datetime string '2023-11-03 11:00:32' with the timezone provided in the `WITH TIMEZONE` clause.

        ```sql
        SELECT fromDateTime('2023-11-03 11:00:32', 'yyyy-MM-dd HH:mm:ss') FROM Event WITH TIMEZONE 'America/Los_Angeles'
        ```
      </Collapser>
    </CollapserGroup>

    <Callout variant="important">
      The UI will automatically detect the `fromDateTime()` value as a timestamp and format it as a date time value.  To display the actual numeric timestamp, wrap the `fromDateTime()` function in a `string()` function.
    </Callout>
  </Collapser>
  ...
  * occurrence - indicates which occurrence of substr of which to return the position.
  ...
  >
    ...
    * timestamp - A numeric timestamp to be translated into a datetime string. This can be a numeric value or an attribute.
    * pattern - An optional date time pattern used to format the result. See the [Patterns for fromDateTime and toDateTime](#fromDateTime-toDateTime-patterns) in the [fromDateTime()](#func-fromDateTime) section above.
      * This must be a constant string value and will default to `yyyy-MM-dd'T'HH:mm:ss[.SSS][XXX]` if a pattern is not provided.
    * timezone - An optional timezone value that is used to interpret the datetime string(ex. UTC).
    ...
    <CollapserGroup>
      <Collapser title="Translate a timestamp using the default pattern">
        The following query translates the `timestamp` attribute using the default pattern of `yyyy-MM-dd'T'HH:mm:ss[.SSS][XXX]`.

        ```sql
        SELECT toDateTime(timestamp) FROM Event
        ```
      </Collapser>
      <Collapser title="Translate a timestamp using the timezone parameter">
        The following query translates the `timestamp` attribute using the pattern string `yyyy-MM-dd` with the timezone set to 'America/Los_Angeles'.

        ```sql
        SELECT toDateTime(timestamp, 'yyyy-MM-dd', timezone:'America/Los_Angeles') FROM Event
        ```
      </Collapser>
      <Collapser title={<>Translate a timestamp using the timezone in the <InlineCode>WITH TIMEZONE</InlineCode> clause</>}>
        The following query translates a timestamp using the timezone provided in the `WITH TIMEZONE` clause.

        ```sql
        SELECT toDateTime(timestamp, 'yyyy-MM-dd') FROM Event WITH TIMEZONE 'America/Los_Angeles'
        ```
      </Collapser>
    </CollapserGroup>
  </Collapser>
