...
Here are available NRQL functions in NRQL, with example NRQL queries. 
...
* If you're using an aggregator function multiple times in the same query (e.g., `SELECT median(one_metric</var>), median(another_metric)`), it can cause problems in displaying results. To solve this, use the [`AS` function](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#sel-as). For example: 
...
* For how to get results over time, see [Group results over time](/docs/insights/new-relic-insights/features/cohort-analysis).
