...
Note: `aparse()` is case-insensitive.
  ...
  >
    ...
    For a numeric type, it returns 1 if the attribute is less than or equal to threshold, otherwise 0. For a distribution, it returns the count in the dataset represented by the distribution. For all other types, it returns 0.
