...
import exampleJoinedPercentile from 'images/nrql_screenshot-example-joined-percentile.webp'

import exampleJoinedMedian from 'images/nrql_screenshot-example-joined-median.webp'
...
<CollapserGroup>
  ...
  >
    ...
    * Because median is simply a shortcut for `percentile(attribute, 50)`, a `median()` result from a
    ...
    To reference the actual median value in the outer query, you can use the [`getField()`](#func-getfield) function.
  ...
  Percentile in a [`JOIN`](#sel-join) clause:
    ...
    * When using percentiles in a joined subquery, please note the results from the subquery are a
    compound data type, which maps each double percentile to its calculated value.

    To reference any of the individual percentile values in the outer query, you can use the [`getField()`](#func-getfield) function.
