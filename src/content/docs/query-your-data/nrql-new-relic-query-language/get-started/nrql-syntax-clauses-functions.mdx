          ...
          <td>
            ...
            * `ip-10-351-14-15`
            ...
            <Callout variant="important">
              ...
              * To use a capture group from the regex, please see Regex Capture.
            </Callout>
          </td>
        </tr>
...
## Aggregator functions [#functions]
...
<CollapserGroup>
  ...
  <Collapser
    ...
    >
      ...
      * Attribute name
      * Regular expression with capture syntax. The regular expression must start with `r` and be contained within single quotes.
      Then the capture syntax within the expression is `(?P<captureName>)`. You may add any additional regular expression matchers within the parentheses.
      Please see the examples below.

      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> within a <InlineCode>SELECT</InlineCode> clause condition</>}>
          This will select the domain name of the website, removing https:// and any paths following the .com

          ```
          SELECT capture(pageUrl, r'https://(?P<baseUrl>.*.com)/.+') FROM PageView SINCE 1 day ago
          ```

          This will capture only the first word of the error message.
          ```
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> within a <InlineCode>FACET</InlineCode> clause condition</>}>
          This will facet by the captured HTTP method.
          ```
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> within a <InlineCode>WHERE</InlineCode> clause condition</>}>
          This will filter the results based on Log events with `message` attribute that matches the regular expression where the captured job name is `ExampleJob`.
          ```
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> with a numeric cast</>}>
          This will capture sum of CPU Time from log lines. You must explicitly cast to numeric to do mathematical operations.
