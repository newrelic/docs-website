...
<CollapserGroup>
  ...
  >
    ...
    * The joined subquery will continue to have a default [`LIMIT`](#sel-limit) of 10, with a
    maximum `LIMIT` of 5000. Note that the outer query's `LIMIT` does not affect the inner query.
  ...
  >
    ...
    The maximum allowed value for the `LIMIT` clause is 5000. Queries can use the `LIMIT MAX` clause instead of a specific value, which automatically defaults to the current maximum value. You can use this if you always want to post the maximum number of results, even if it changes in the future. If you want your query's behavior to remain unchanged, specify an explicit value instead of using `LIMIT MAX`.
