...
* [NRQL tutorial](https://learn.newrelic.com/writing-nrql-queries) from New Relic University
...
<CollapserGroup>
  ...
  >
    ...
    Use the `WITH ... AS` clause to define NRQL Variables to store row-wise values into variables which can be referenced anywhere in the query.

    * The `WITH ... AS` clause can go before, in between, or directly after the `FROM`/`SELECT` clause
    * Only row-wise functions, e.g. `capture()`, can be defined as a variable. Aggregator functions, such as `average()` are not supported.
    * Only one `WITH` can be used, but as many NRQL Variables may be defined as needed if separated by a comma
    * If a defined NRQL variable uses the same name as an existing attribute, the NRQL Variable will take precedence
    * NRQL Variable names cannot include the % symbol
    ...
    <CollapserGroup>
      ...
      >
        ...
        ```
        Learn more about Anchor Parse, aparse(), [here](#func-aparse).
      </Collapser>

      <Collapser
        id="nrql-var-other-clauses"
        title={<>Use in other clauses</>}
      >
        In this example a NRQL Variable, unitPrice, is used to create another NRQL Variable, unitPriceNum, converting the extracted string into a number.
        The NRQL Variables are then used in the `SELECT`, `WHERE` and `FACET` clauses.
...
<CollapserGroup>
  ...
  >
    Use the Anchor Parse function, `aparse()`, as an alternative to `capture()` to extract specific values from a string.
    ...
    * A pattern string with anchor strings and extract characters, e.g. 'www.*.com' to extract the domain from a URL.
    ...
    When using `aparse()` the pattern string should contain anchors, like 'www.' and '.com' above to identify the location of the intended
    extracted string, noted by '*'.
    ...
    In practice, the anchor strings often occur in the middle of a string attribute, i.e. not at the beginning or end.
    In this case, use the '%' wildcard to ignore unwanted values, e.g. '%www.*.com%'.
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>aparse()</InlineCode> specific values</>}>
        To extract a value from the middle of a string, use the non-capturing wildcard, '%', at the beginning and end of the pattern string.
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>aparse()</InlineCode> multiple values</>}>
        When extracting multiple values to NRQL Variables, note that the order matters.
  ...
  >
    ...
    * Regular expression with capture syntax. Regex expressions in NRQL use Python-like syntax, `r'...'`.
    ...
    Multiple values can be captured by specifying additional capture groups in a regular expression, e.g. `...(?P<name1> pattern1)...(?P<name2> pattern2)...`
  ...
  >
    ...
    * falseValue - this optional value is returned if boolean expression is false, or if not provided NULL, is returned
    ...
    <CollapserGroup>
      <Collapser title={<>Usage with <InlineCode>AND</InlineCode> and <InlineCode>OR</InlineCode></>}>
        ```
        FROM Log
          SELECT count(*)
