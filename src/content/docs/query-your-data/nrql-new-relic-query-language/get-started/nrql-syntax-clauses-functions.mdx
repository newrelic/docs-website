...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      The `ORDER BY` clause does not apply to `FACET` queries. It should not be confused with the `FACET ... ORDER BY` clause, which guides facet selection but does not control sorting of query results. For more information, see [`FACET ... ORDER BY`](sel-facet-order).
    </Callout>
  </Collapser>
