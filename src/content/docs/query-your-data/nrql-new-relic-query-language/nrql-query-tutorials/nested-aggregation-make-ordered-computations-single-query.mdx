...
<CollapserGroup>
  ...
  <Collapser
      id="x-slowest-entity"
      title="Find entities out of the top N transactions that are slower than X"
    >
      In this example, the inner query finds the top entity guids with the slowest average transaction duration. The outer query then computes how many of those queries are slower than a threshold. In this example, the top 50 queries are selected, and the duration threshold is 1 second.
