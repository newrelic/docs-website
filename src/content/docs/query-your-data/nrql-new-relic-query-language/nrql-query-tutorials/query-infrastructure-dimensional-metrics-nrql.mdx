...
* [Functions](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#functions) used on multiple metrics may fail or return incorrect results, for example:
  ...
  ```
* Using the `RAW` keyword has no effect on queries on infrastructure metrics. For example, the following code does not actually show raw data. The query is transformed internally to an equivalent query on event data, which is aggregated:
  ```sql
