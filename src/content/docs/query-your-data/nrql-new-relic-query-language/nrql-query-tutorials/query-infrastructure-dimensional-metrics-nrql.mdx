...
* [Functions](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#functions) used on multiple metrics may fail or return incorrect results, for example:
  ...
  ```
* When you include `RAW` in a query, the request transforms internally and prints the result for an equivalent query for event data. The event data is aggregated. Including `RAW` in your query will not print raw data. Refer to the example query to see this behavior:
