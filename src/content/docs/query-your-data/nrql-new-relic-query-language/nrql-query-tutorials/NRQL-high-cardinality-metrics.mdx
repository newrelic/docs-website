...
```
<Callout variant="tip">
  Note that you should include the `RAW` clause in cardinality queries. This is because in the event your cardinality has been limited, queries like `SINCE today` will query rollups that are no longer reporting and so need to look at the raw data points to perform the necessary analysis.
</Callout>
