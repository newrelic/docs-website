...
Cardinality is generally defined as the number of elements in a set. You you can query the cardinality of a metric in New Relic with this NRQL query format:

```
FROM Metric SELECT cardinality(metric.name) SINCE today RAW
```

For example, to query the cardinality of the metric `memory.heap` and find out how many individual values are is in the set comprised by this metric, run the following query:

```
FROM Metric SELECT cardinality(memory.heap) SINCE today RAW
```

The basics of cardinality are not difficult to understand, but high cardinality can be a little more complicated. Understanding how high cardinality  works is important, though, because it can impact how quickly you reach your data limits. 
...
For dimensional metrics, the set in question is the collection of observed unique combinations of attributes for a given metric in a one-day period. Each group of attributes is a mapping of key-value that's counted as a single element of the set. 
...
This metric has a cardinality of 8, with each container counted as a single element in the set. If a new measurement for this metric with an identical attributes map as one that had previously arrived later, no new cardinality would be counted.
...
Finally, the account cardinality is essentially the sum of each metric's cardinality, so adding in a simple FACET query can help find the highest cardinality metric:
...
The next step is to determine which dimensions in a given metric contribute the most to its cardinality. You can do this by checking your current dimensions then removing dimensions to check their impact on cardinality. 

First, let's look at ways you can inspect existing dimensions. This query shows the attributes map of each metric that was sent:

```
FROM Metric SELECT dimensions() WHERE metricName = 'memory.heap' SINCE today RAW
```

Looking through these could reveal some dimensions containing a unique ID or other highly variable  value that might be worth removing. 

Next, you can look at `keySet()`:

```
FROM Metric SELECT keySet() WHERE metricName = 'memory.heap' SINCE today RAW
```

Understanding the dimensions that have the most influence on your cardinality comes down to understanding how each key's values correlate with each other. You can experiment with what your cardinality would be without a dimension simply by adding it to the exclude list:
...
To summarize, the more familiar you are with your data, the easier it will be to know which keys to include or exclude comes down to familiarity with your data. Things that look like unique identifiers are a good place to start, but sometimes it's no single key but the combination of two or more keys. 
