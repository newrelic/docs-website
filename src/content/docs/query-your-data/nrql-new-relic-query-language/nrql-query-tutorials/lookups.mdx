...
This query maps `shopId` values to a broader `storeCategory` value by using a `JOIN` clause on lookup table data. This enables us to perform aggregations on `StoreEvent` records based on these broader categories and extract new insights from our data.
...
**Note:** When using a lookup table within a subquery, the outer query `LIMIT` is bound by the standard maximum value unless it also uses a lookup table.
...
* The following NRQL clauses are not supported with lookup queries:
  * `TIMESERIES`
  * `COMPARE WITH`
  * `EXTRAPOLATE`
  *  **Note:** These clauses can be used if the lookup query is contained in an inner query. For an example of this, see [this query](#item-ids).
