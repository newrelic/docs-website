...
Much of the data stored within New Relic relates to other data: `Transaction` and `TransactionError`, `PageView` and `PageAction`, `Log` and infrastructure events, and more. Now you can finally perform analysis and calculate correlations between these events using subquery joins.
...
<CollapserGroup>
  ...
  >
    ...
    Tip: Include LIMIT MAX in the inner subquery to maximize the number of results joined to the outer query. The current max result limit is 5000 rows.
  </Collapser>
