...
maximum `LIMIT` of 5,000. Note that the outer query's `LIMIT` doesn't affect the inner query.
...
<CollapserGroup>
  ...
  >
   ...
   Include `LIMIT MAX` in the inner subquery to maximize the number of results joined to the outer query. The current max result limit is 5,000 rows.
  </Collapser>
