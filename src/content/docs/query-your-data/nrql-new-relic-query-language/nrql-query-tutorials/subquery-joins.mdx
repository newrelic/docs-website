...
* complex attributes can't be the primary keys
* Only equality JOIN conditions are supported
* We do no coercion of attribute types in the JOIN condition (that's already in there, as I recall)
* Metric wildcards aren't supported in the JOIN condition
* The subquery can't be a Metric row-wise query
* The right-hand side of the JOIN condition must be an identifier. e.g. functions aren't supported
* The subquery can't select or facet on uniques()
* The subquery can't have TIMESERIES or COMPARE WITH
* The subquery must be row-wise or FACETed
