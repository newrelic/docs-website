...
One way to use funnel queries is to calculate the rate at which multi-step, ordered sequences were completed over a given timeframe, like in the New Relic One signup [example](#why-matters). But you can also use funnels to calculate overlap between several distinct actions that do not have a sequential relationship to one another. In other words, the order of steps doesn't impact the calculations performed.
