...
## MobileNetworkRequestSettings query examples (#network-request-settings)

Below are some common NRQL queries for the following mobile network requests:
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="hostnames"
    title="Blocked or allowed hostnames"
  >
  Use case example...
  ```sql
    add query here
...
<Collapser
    className="freq-link"
    id="status-codes"
    title="Ignored HTTP status codes from certain hosts"
  >
  Use case example...
  ```sql
    add query here
...
<Collapser
    className="freq-link"
    id="aliases"
    title="Aliases for hosts and/or subdomains"
  >
   Use case example...
  ```sql
    add query here
