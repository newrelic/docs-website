...
redirects:
  - /docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/lookup-service-rest-api/
---

Use the lookups REST API to create and manage [lookup tables](/docs/logs/ui-data/lookup-tables-ui/#overview). 

## Before you start

The Lookups Service API allows you to manage lookup tables programmatically. As another option you can also [manage lookup tables through our UI](/docs/logs/ui-data/lookup-tables-ui/). 
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Downloads a table that was previously uploaded.
      </td>
    </tr>
    <tr>
      <td>
...
The variables required in the above Lookups Service API endpoints are defined below. 
...
<table>
    ...
    <tbody>
        ...
        <td>
            A name for the stored table.  Table names must comply with the [custom event type standards](/docs/data-apis/ingest-apis/event-api/introduction-event-api/#limits):
            * Maximum length: 255 (per the [Create custom events (Go)](/docs/apm/agents/go-agent/features/create-custom-events-go/) page)
...
Your <InlinePopover type="licenseKey" /> serves to authenticate your request to the Lookups Service API and needs to be passed as an HTTP header.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Indicates whether to include the table value in the response. Ignored when content type is `text/csv`.
      </td>
    </tr>
  </tbody>
</table>
...
<Collapser
    id="list-tables"
    title="Listing tables udated for an account"
  >
...
If a request is unsuccessful, the error reponse payload will be in the format below.
```json
{
