...
When using the time functions from the table below in NRQL queries, the results are returned in UTC. To adjust the results to your time zone, include the [`WITH TIMEZONE` clause](/docs/insights/nrql-new-relic-query-language/nrql-resources/nrql-syntax-components-functions#sel-timezone) in your query.

## Facet your NRQL query time range [#cohorts]

<Callout variant="tip">
In these examples, `account_created` is a custom timestamp attribute submitted with the PageView events. To facet by the time the PageView events were ingested, you could use the `timestamp` attribute instead.
</Callout>
