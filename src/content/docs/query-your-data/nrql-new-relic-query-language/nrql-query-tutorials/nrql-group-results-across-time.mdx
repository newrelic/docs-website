...
When using the time functions from the table below in NRQL queries, the results return in UTC format. To adjust the results to your time zone, include the [`WITH TIMEZONE` clause](/docs/insights/nrql-new-relic-query-language/nrql-resources/nrql-syntax-components-functions#sel-timezone) in your query.

## Facet your NRQL query time range [#cohorts]

<Callout variant="tip">
In these examples, we use a custom timestamp attribute submitted with PageView events called `account_created`. To facet by the time of PageView event were ingestion, you could use the `timestamp` attribute instead.
</Callout>
