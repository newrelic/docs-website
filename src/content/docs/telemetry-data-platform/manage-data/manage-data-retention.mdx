...
When you reduce the number of days that you want to retain data, existing data of that source type won't immediately disappear and might be visible and returned in queries for up to several weeks. However, the change is registered in the system within 12 hours, and after the change, new data of the adjusted source type will be retained for the new duration. 

We have safeguards in place to ensure you don't accidentally delete data, but this also means that reducing retention values isn't a good way to quickly remove data. If you need to delete data, contact your customer support representative.
...
Once [telemetry data](/docs/using-new-relic/data/understand-data/new-relic-data-types) (events, metrics, logs, traces) is reported to New Relic and available for querying, that data cannot be edited or deleted. This is a purposeful design decision that optimizes New Relic's speed and performance. Data will expire after its [data retention period](/docs/telemetry-data-platform/manage-data/manage-data-retention/#adjust-retention) ends. If you need to delete data, contact your customer support representative. 
