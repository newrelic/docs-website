...
For example, if you want to use a single value within a metric to do a comparison in a `WHERE` clause, you can do that with `getField(metricName, field)` or `metricName[field]`.
...
<CollapserGroup>
  ...
  The average value of a metric is computed as `total` over `count`, so the following query would return metric data where the result of the `average()` value function is greater than 2.
    ```
