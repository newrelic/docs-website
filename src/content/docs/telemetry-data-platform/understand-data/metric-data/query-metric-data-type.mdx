...
<CollapserGroup>
  ...
  >
  This example query result will be the `average()` value function computed as total over `count`, for all values over 2. 
    ```
    FROM Metric SELECT uniques(metricName) WHERE %[total] / %[count] > 2
    ```
  </Collapser>
</CollapserGroup>
