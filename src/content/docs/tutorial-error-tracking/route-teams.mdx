...
From the <DoNotTranslate>**Error group summary**</DoNotTranslate> page, you can assign the error group to the correct team. 
...
* If an error group is expected, you can mark the error as <DoNotTranslate>**Ignored**</DoNotTranslate>. Expected errors are known to you and the team—they can be non-critical bugs, or they can be errors associated with the end user (like someone using an incorrect password). 
...
* New Relic tracks the status of an error over time. For example, if you mark an error group as <DoNotTranslate>**Resolved**</DoNotTranslate> but it appears at a later point with a new deployment, New Relic will mark that error as a <DoNotTranslate>**Regression**</DoNotTranslate>.
