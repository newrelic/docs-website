...
This page provides a reference for the `ansible.job.getStatus` action available in the workflow automation actions catalog. Use this action to retrieve the status and details of an Ansible Automation Platform (Automation Controller) job. Use this action to poll a job created by [`ansible.job.launch`](/docs/workflow-automation/setup-and-configure/actions-catalog/ansible/ansible-job-launch). This action builds on the `http.get` base action.
