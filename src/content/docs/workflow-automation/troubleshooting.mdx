...
3. **Review permissions:** Ensure your IAM role or user has the specific AWS permissions your workflow requires. Check the [Required permissions](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials#required-permissions) table.
...
For detailed instructions on viewing workflow status and execution history, see [View workflows](/docs/workflow-automation/manage-workflows/managing-workflow#view-workflows).
...
**Next step:** Fix the issue, then [manually trigger the workflow](/docs/workflow-automation/workflow-automation-apis/start-workflow-run) to verify it works.
...
**Solutions:**
1. **Verify [CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) configured**: Required for real-time EC2 metrics
...
**"<DNT>Autopilot</DNT> RCA Failed" message in Slack:**
`analysisRaw` was empty — <DNT>Autopilot</DNT> returned no `finalAnswer`. Verify `issueId` is a valid, active alert issue UUID and the <DNT>Autopilot</DNT> prompt is well-formed.
...
<DocTiles>
  <DocTile title='Workflow best practices' path="/docs/workflow-automation/limitations-and-faq/workflow-best-practices">Error handling and testing strategies to prevent issues</DocTile>
  <DocTile title='Set up AWS credentials' path="/docs/workflow-automation/setup-and-configure/set-up-aws-credentials">Fix AWS authentication and permission errors</DocTile>
  <DocTile title='Create destinations' path="/docs/workflow-automation/setup-and-configure/create-destinations">Configure Slack and notification integrations correctly</DocTile>
  <DocTile title='Workflow limits' path="/docs/workflow-automation/limitations-and-faq/workflow-limits">Understand system constraints and timeouts</DocTile>
</DocTiles>
