...
**What loops enable:**
  ...
  - Process each item with `element` and track position with `index` variables
  - Control loop flow with `break` and `continue` statements
  - Nest loops for complex data processing
...
<DocTiles>
  <DocTile title='Conditional logic' path="/docs/workflow-automation/workflow-examples/basic-patterns/conditional-logic">Combine loops with switch statements</DocTile>
  <DocTile title='REST API polling' path="/docs/workflow-automation/workflow-examples/integrations/rest-api-polling">Use loops for API polling workflows</DocTile>
  <DocTile title='Definition schema: Loop' path="/docs/workflow-automation/workflow-automation-apis/definition-schema#loop">Complete loop step reference</DocTile>
</DocTiles>
