...
3. **Wait for a signal.** The wait step pauses the run for the duration set by its `seconds` input, up to a [maximum of 86400 seconds / 1 day](/docs/workflow-automation/limitations-and-faq/workflow-limits). If you don't set `seconds`, the 86400-second default applies. During this window, the person clicks the URL, which opens a New Relic UI listing the available signals. If they aren't signed in, they sign in first, then land on the signals page and select a signal to send.
4. **Route based on the signal.** Each signal in the wait step defines a `next` step. The signal received determines which branch the workflow runs. The signal must be sent before the wait step's duration ends (and within the workflow's overall [7-day maximum](/docs/workflow-automation/limitations-and-faq/workflow-limits)); otherwise the wait step's `next` step runs as the timeout path.
