...
When templates don't fit your needs, [Create Your Own](https://onenr.io/0OQM47KgxjG) lets you build custom workflows. Use the drag-and-drop interface to chain actions from the [actions catalog](/docs/workflow-automation/setup-and-configuration/actions-catalog) into automation that matches your process.
...
|-------------|------------------|-------------|
| **Inputs and secrets** | Parameters for credentials and configuration | `${{ :secrets:awsKeyId }}` for credentials, `${{ .inputs.region }}` for config |
| **Actions catalog** | Pre-built integrations (AWS, Slack, databases, APIs) | Drag `aws.ec2.stopInstances` into workflow canvas |
| **Data flow** | Pass outputs between steps | `${{ .steps.getAlert.outputs.entityGuid }}` |
| **Switches** | Create different paths based on conditions | Route by CPU > 90% vs. 70-90% vs. < 70% |
| **Loops** | Process lists or poll for completion | `workflow.forEach` for items, `workflow.repeat` for polling |
...
<Callout variant="tip">
  **Learn by doing:** Each concept is demonstrated in the [Example walkthrough](#example-walkthrough) below. You'll see inputs, switches, loops, and approval gates working together in a real workflow.
</Callout>
...
1. **Open the builder**: Navigate to **[one.newrelic.com](https://one.newrelic.com) > All Capabilities > Workflow Automation** and select **[Create Your Own](https://onenr.io/0OQM47KgxjG)**
2. **Add inputs**: Define parameters for credentials (from secrets manager: `${{ :secrets:keyName }}`), configuration (regions, instance types), and runtime data (account IDs, alert IDs)
3. **Build the flow**: Drag actions from the [catalog](/docs/workflow-automation/setup-and-configuration/actions-catalog), connect them with `${{ .steps.stepName.outputs.field }}` syntax to pass data
4. **Add logic**: Insert switches for conditional branching, loops for processing lists or polling, approval gates for human decisions
5. **Test and deploy**: Run after each section to catch errors early, then [start or schedule](/docs/workflow-automation/create-a-workflow-automation/start-schedule) your workflow
...
- name: decideAction
  ...
  inputs:
    value: "${{ .steps.checkMetric.outputs.result }}"
    cases:
      - when: "${{ .value > 90 }}"  # Critical threshold
        steps: [...]
      - when: "${{ .value > 70 }}"  # Warning threshold
        steps: [...]
      - default:  # Normal state
        steps: [...]
```

**See it in action:** [Handle team response](#handle-team-response) section shows switches routing based on Slack reactions.

### Loops for processing lists or polling

**Use loops when:** Processing multiple items or waiting for long-running operations to complete
...
- name: processItems
  ...
  inputs:
    items: "${{ .steps.getList.outputs.data }}"
    steps: [...]

# Poll until complete
- name: pollStatus
  action: workflow.repeat
  inputs:
    maxIterations: 20
    delaySeconds: 10
    steps: [...]
...
**See it in action:** [Execute the resize](#execute-the-resize) section uses loops for status updates and polling.
...
- name: waitForReaction
  action: workflow.wait
  inputs:
    condition: "slack_reaction"
...
- **[Actions catalog](/docs/workflow-automation/setup-and-configuration/actions-catalog)**: AWS, Azure, GCP, HTTP, data transformations.
