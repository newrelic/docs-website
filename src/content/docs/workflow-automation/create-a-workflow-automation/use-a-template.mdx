...
<CollapserGroup>
  ...
  <Collapser
    id="nrlens-sql-query"
    title="Execute NRLens SQL query"
  >
    **What it does:**

    Runs SQL queries against New Relic's data catalog using NRLens (advanced query tool).

    **When to use it:**

    Complex analysis beyond NRQL: multi-table joins, statistical analysis, custom reports combining metrics/events/logs.

    **What you'll need:**

    - **NRLens**: Credentials for SQL Query Gateway
    - **New Relic**: Organization and account IDs

    **How it works:**

    1. Connects to NRLens
    2. Executes SQL
    3. Retrieves results
    4. Logs to New Relic
    5. Supports scheduling

    <Callout variant="tip">
      **NRLens extends NRQL.** Use NRQL for real-time queries, NRLens SQL for complex analytical workloads.
    </Callout>
