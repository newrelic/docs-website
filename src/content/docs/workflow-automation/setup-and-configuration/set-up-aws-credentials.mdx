...
|------------|--------------|--------------|----------------------|
| **IAM role** (recommended) | Production workflows | ⭐⭐⭐⭐⭐ Auto-rotating credentials | Medium - requires cross-account setup |
| **IAM user with access keys** | Testing/development | ⭐⭐⭐ Long-lived keys (manual rotation) | Easy - standard key generation |
| **Session tokens** | CI/CD pipelines, local dev | ⭐⭐⭐⭐ Time-limited (1-12 hours) | Easy - requires AWS CLI |

<Callout variant="important">
  **Security recommendation:** Use IAM roles for production workflows. Roles provide temporary credentials, eliminate key rotation requirements, and follow AWS security best practices.
</Callout>
...
|---------------|-------------------------|-------------|
| **EC2 management** | `ec2:DescribeInstances`<br/>`ec2:StopInstances`<br/>`ec2:StartInstances`<br/>`ec2:ModifyInstanceAttribute` | Stop, start, or modify EC2 instances in response to alerts |
| **SQS messaging** | `sqs:SendMessage`<br/>`sqs:GetQueueAttributes` | Send messages to SQS queues for downstream processing |
| **API Gateway** | `apigateway:GET`<br/>`apigateway:PUT` | Roll back API Gateway deployments or configurations |
| **Systems Manager** | `ssm:CreateDocument`<br/>`ssm:DeleteDocument`<br/>`ssm:StartAutomationExecution`<br/>`ssm:GetAutomationExecution` | Execute automation runbooks |
| **DynamoDB** | `dynamodb:Query`<br/>`dynamodb:GetItem`<br/>`dynamodb:PutItem` | Read from or write to DynamoDB tables |
...
See [Policy examples](#policy-examples) below for complete IAM policy templates.
  ...
  <TabsPages>
    <TabsPageItem id="iam-role-1">
      ...
      **IAM role characteristics:**
      ...
      ### Create the role in AWS
        ...
        5. In the **External ID** field, enter your New Relic account ID
          - Don't have it? [Find your account ID here](/docs/accounts/accounts-billing/account-structure/account-id)
        6. Click **Next**.
        7. On the **Add permissions** page, attach policies based on your workflows:
          ...
          - For other services: See [Required AWS permissions](#required-aws-permissions) below
        8. Click **Next**.
        9. Enter a role name: `NewRelicWorkflowAutomationRole` (or your preferred name)
        10. Optionally add a description: "Allows New Relic workflow automation to perform actions in AWS"
    ...
    <TabsPageItem id="iam-user-1">
      ...
      **Use cases:**
      ...
      Never hardcode AWS credentials in workflows. Store them in New Relic's [secrets manager](/docs/infrastructure/host-integrations/installation/secrets-management/) instead.
...
## Troubleshoot credentials [#troubleshoot]

Common authentication issues and solutions:

| **Problem** | **Solution** |
|-------------|-------------|
| "Access denied" errors | Verify you used account ID `253490767857` (not `754728514883` from cloud integrations) |
| "Invalid external ID" | Confirm external ID matches your New Relic account ID exactly |
| "Role cannot be assumed" | Check trust policy includes correct New Relic account ARN |
| "Credentials expired" (session tokens) | Session tokens expire after 1-12 hours—generate new ones |
| "Secret not found" | Verify secret key names match exactly: `awsAccessKeyId`, `awsSecretAccessKey` |

For detailed troubleshooting, see [Troubleshoot Workflow Automation](/docs/workflow-automation/troubleshooting#access-denied-errors).
...
After configuring AWS credentials, you can create workflows:
  ...
  **Create workflows:**
  1. **[Browse AWS actions](/docs/workflow-automation/setup-and-configuration/actions-catalog)**: View available AWS operations (EC2, SQS, DynamoDB, etc.)
  2. **[Use a template](/docs/workflow-automation/create-a-workflow-automation/use-a-template)**: Deploy pre-configured workflows for common AWS automation tasks
  3. **[Create a custom workflow](/docs/workflow-automation/create-a-workflow-automation/create-your-own)**: Build workflows for your specific requirements

  **Configure notifications:**
  - **[Set up destinations](/docs/workflow-automation/setup-and-configuration/create-destinations)**: Configure Slack, PagerDuty, email, or webhook notifications for workflow events
