...
**Example**: Send Slack notification if EC2 resize fails:
...
- name: resizeInstance
  ...
  onError:
    - name: notifyFailure
      ...
      inputs:
        text: "EC2 resize failed for ${{ .inputs.instanceId }}"
```
...
1. Navigate to **[one.newrelic.com](https://one.newrelic.com) > All Capabilities > Workflow Automation**
...
When actions don't depend on each other, run them in parallel:

```yaml
- name: notifyTeams
  action: workflow.parallel
  inputs:
    steps:
      - name: sendToSlack
      - name: sendToPagerDuty
      - name: updateJira
```
