...
**Requirements:**
  ...
  - Email destination configured in New Relic (see [Send notifications from workflows](/docs/workflow-automation/setup-and-configuration/create-destinations))
  - Workflow scheduled using [CreateSchedule API](/docs/workflow-automation/workflow-automation-apis/create-schedule)
...
**To schedule this workflow**, use the [CreateSchedule API](/docs/workflow-automation/create-a-workflow-automation/start-schedule#scheduled) with a cron expression like `*/10 * * * *` (every 10 minutes). Remember that the minimum scheduling interval is 10 minutes. See [workflow limits](/docs/workflow-automation/limitations-and-faq/workflow-limits) for details.
