...
Use this API to create a workflow definition. It returns an error if the workflow definition already exists.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>No</td>
      <td>Tags to apply to the workflow definition. Each tag requires a `key` (String) and `values` ([String]).</td>
    </tr>
  </tbody>
</table>
