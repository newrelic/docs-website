...
A step that fails the workflow with a custom message. It validates the `condition` if present; when the condition is `true`, the workflow execution stops with the provided `message`. Otherwise, the default message `failed as requested from step: failureStep` is used.
