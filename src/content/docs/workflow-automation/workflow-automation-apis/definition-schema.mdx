...
### Action [#action]
  ...
  A step that runs a specific action. Refer to the [Action catalog](/docs/workflow-automation/setup-and-configuration/actions-catalog/actions-catalog) for the available options.
...
<CollapserGroup>
    ...
    **Jump within a loop**
    ...
    Jump between named steps belonging to the same for loop is allowed. Jumping in or out of a for loop, to an inner or outer loop, or between two different for loops, is not allowed.
    ...
    **Use break or continue in a loop**
