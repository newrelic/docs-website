...
The `workflowInputs` map has the following properties:
...
Each input entry supports the following fields:
  ...
  - `workflowInputs.<inputName>.enumValues` (Optional)
    - **Type**: List (`String`).
    - **Description**: Default value of this field is empty List `{}`. This field is required when the `workflowInputs` type is `Enum`.

  - `workflowInputs.<inputName>.validations` (Optional)
    - **Type**: Array of maps.
    - **Description**: The validations to run on the workflow inputs the user provides. This is an optional field. The properties described here are present in all validation types. Specific validation types support additional properties as discussed in the [Validation types](#validation-types) section.
...
<Callout variant="important">
  Keep the following in mind when using workflow constants:

  - Workflow constants are read-only and automatically provided by the runtime.
  ...
  - Unlike `workflowInputs`, constants don't need to be declared in the workflow definition.
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>array of maps</td>
      <td></td>
      <td>The steps to run when the workflow definition executes. There must be at least one step. The properties described here are present on all step types. Specific step types support additional properties as described in the [Step types](#step-types) section.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>**Max length:** 100</td>
      <td>The name of the step that `steps[*].next` references. It cannot be the special keywords `end`, `continue`, or `break`, as these indicate a terminating step, a continuation of a loop, or to break out of a loop.</td>
    </tr>
    <tr>
      <td>`steps[*].type`</td>
      <td>Required</td>
    ...
    <tr>
      ...
      <td></td>
      <td></td>
      <td>The `ignoreErrors` is a configuration option within a step of a workflow. It defaults to false. By setting `ignoreErrors` to `true`, the workflow ensures that any errors during this step's execution will not cause the entire workflow to fail. Instead, the workflow will continue executing subsequent steps.</td>
    </tr>
  </tbody>
</table>
...
<Callout variant="important">
  Keep the following in mind when using loops:
  ...
  - `element` and `index` are automatically assigned per iteration. `index` is zero-based. `element` can be a complex type.
  - Variables created inside the loop (loop variables and step outputs) are only accessible within the loop. The loop clears them on exit.
...
You can jump between steps within the same loop. You cannot jump in/out of loops, between different loops, or to parent/child loops.
...
The wait step can also listen for one or more signals. Each signal must define a corresponding next step. If the wait step receives a signal, it processes the first one received and executes its defined next step. If no signal is received, the workflow continues normally after the wait period ends.
...
A step that stores key/value pairs in the workflow's shared state. Values persist for the duration of the workflow run and are accessible in subsequent steps using the `.workflowState` accessor. If multiple state steps set the same key, each later value overwrites the earlier one.

The state step accepts the following input:

- `steps[*].inputs` (Required)
  - **Type**: Map of key/value pairs
  - **Description**: The key/value pairs to store in the workflow state. Keys must be valid identifiers. Values can be any type — string, integer, boolean, list, or map (including nested structures). Values can include expressions. The runtime doesn't resolve secret references — it stores them as-is.
...
- `workflowState` — Object containing key/value pairs stored by `state` steps during the current workflow run. Supports flat key access, nested property access, and array index access.

  **Examples**: `${{ .workflowState.myKey }}` · `${{ .workflowState.myMap.nestedProperty }}` · `${{ .workflowState.myList[0] }}`
...
Let's assume `myArray` has a value of `[1, 2, 3]`.
...
Use secret values in actions by providing reference strings that specify the name of a secret to look up in the Secrets Service. To reference a secret in a workflow definition, use the syntax:
  ...
  - `${{ :secrets:<SCOPE>:<NAMESPACE>:<SECRET_NAME> }}` for a secret in scope and namespace. scope accepts `ACCOUNT` or `ORGANIZATION`.
