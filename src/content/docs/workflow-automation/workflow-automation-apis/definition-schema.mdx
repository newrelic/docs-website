...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>**`name`** values are case-insensitive. For example, `ExampleWorkflow`, `exampleworkflow`, and `EXAMPLEWORKFLOW` are all considered to represent the same workflow definition.</td>
    </tr>
    <tr>
      <td>**`description`**</td>
      <td>Optional</td>
...
<table>
  ...
  **Example**:
  ...
  - **`workflowInputs.<inputName>.validations`** (Optional)
    - **Type**: Array of maps.
    - **Description**: The validations to be performed on the workflow inputs provided by user. This is an optional field.
    
    The properties described here are present in all Validation types. Specific Validation Types support additional properties as discussed in [Validation Types](#validation-types) section.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <p>
        The steps to be performed when the workflow definition is run. There must be at least one step.
        </p>

        <p>
        The properties described here are present on all step types. Specific step types support additional properties as described in the [Step types](#step-types) section.
        </p>
      </td>
    </tr>
  </tbody>
...
<CollapserGroup>
    ...
    A loop iterates over a given collection defined by `in`. It will automatically create a loop variables `index` and `element` for each of its iterations. These loop variables are accessible within the loop with JQ expression `${{ .steps.<loopStepName>.loop.element }}` or `${{ .steps.<loopStepName>.loop.index }}`
    ...
    **Examples**: 
      ...
      The following shows how to execute the loop workflowInputs. Note, that the result must be a type of collection.
  ...
  ```
      <Callout variant="important">
        ...
        - `index` is zero-based.
        - `element` can be a complex type if you have a collection of complex elements.
    ...
    **Jump within a loop**
    ...
    Jump between named steps belonging to the same for loop is allowed. Jumping in or out of a for loop, to an inner/outer loop, or between two different for loops, is not allowed.
...
A step that pauses workflow execution for a specified number of seconds before
continuing.
...
<table>
  ...
  <tbody>
    <tr>
      <td>**`regex`**</td>
      <td>`validations[*].pattern` (Required)</td>
      <td>Required</td>
      <td>String</td>
    ...
    <tr>
      <td>**`maxIntValue`**</td>
      <td>`validations[*].maxValue` (Required)</td>
      <td>Required</td>
      <td>Integer</td>
    ...
    <tr>
      <td>**`minIntValue`**</td>
      <td>`validations[*].minValue` (Required)</td>
      <td>Required</td>
      <td>Integer</td>
    ...
    <tr>
      <td>**`maxLength`**</td>
      <td>`validations[*].length` (Required)</td>
      <td>Required</td>
      <td>Integer</td>
...
### Expression strings [#expression-strings]
  ...
  **Example**: 
  ...
  Get the length of a workflow input string: `${{ .workflowInputs.myString | length }}`
...
- `workflowInputs`
  - Object containing the inputs passed to the workflow on start.

**Example**: `${{ .workflowInputs.myInput }}`

    - `steps`
      - Object containing a property for each step in the workflow
        - `steps.<stepName>`
          - Object containing properties for a specific step
        - `steps.<stepName>.outputs`
          - Object containing result properties, specific to the step/action.
...
### Expression evaluation results [#expression-evaluation-results]
  ...
  <Callout variant="important">
    When a jq expression is evaluated to null, a null node will be returned. For example the expression `${{ .workflowInputs.missingInput }}` will return null if `missingInput` is not given as workflow input.
  </Callout>
...
Properties that can be used in expressions must conform to the following regex: `^[A-Za-z_][A-Za-z0-9_]*$`
