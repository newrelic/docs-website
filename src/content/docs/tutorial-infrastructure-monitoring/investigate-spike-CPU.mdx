...
Since we're dealing with CPU percentage, we want to determine if the problem is app related or not. To do that, you want to check the **Processes running** table on the host page.

* CPU usage is running hot and flatlining near 100%.
* When you check processes, you see that the `ruby` process is using up 77.34% of your CPU. 

Since you've correlated CPU percentage with a process, you can conclude that an app is hogging your resources rather than the problem coming from the host itself. 

    </Step>
    <Step>
    ### Determine if you should allocate more resources to that app

At this stage you might need to make a decision: should you allocate more resources to this instance, or should you notify the dev team that there's a problem somewhere in their app? 
