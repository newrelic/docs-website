...
Since we're dealing with CPU percentage, we want to determine if the problem is app related or machine related. To do that, you want to check the **Processes running** table against the **Latest events** in the sidebar. This step helps you disambiguate if a problem is occurring because a change was made directly to the machine, or if a process is exhausting resources. 
...
Identifying a resource problem exists doesn't mean you should always allocate more resources. If there's a problem with app optimization, your job is to notify the team that there's a problem innate to the app. That said, maybe external factors are causing high CPU. If more end users are accessing that component, then provisioning more resources is necessary to meet that load.
...
* If load is causing stress to your system, your Network traffic would show an increase paralell to your metric graphs. You might also notice a slow degradation over time, since processes would stall out as resources run out. In other words, an increase in load met with inadquate resources would eventually cause slow response time from your app.
* Network traffic, however, is staying consistent. It doesn't appear to change over time as resources run out. The peaks and troughs are regular without any uptick and steady decline.
...
When you collaborate with other teams, you want everyone to look at the same data. This ensures that when decisions are made for potential changes, those decisions are based on the same data used in troubleshooting. 
...
<UserJourneyControls
    previousStep={{path: "/docs/tutorial-infrastructure-monitoring/diagnose-app-infra-data", title: "Previous step", body: "Diagnose app failure with host data"}}
    nextStep={{path: "/docs/tutorial-infrastructure-monitoring/create-metrics-dashboards", title: "Next step", body: "Make dashboards to tell a data-driven story"}}
/>