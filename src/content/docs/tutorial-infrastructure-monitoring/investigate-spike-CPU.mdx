...
* Make a decision about resource allocation using data in New Relic
...
So far we've covered how to use infrastructure data to troubleshoot an incident that's causing an app outage. We've covered how to scope down from thousands of hosts to a set of hosts and we've explored how to correlate a single host's behavior to make resource recommendations. But what if you're noticing a pattern of behavior that's routinely anomolous? That is, at a regular cadence, a change happens, then resolves itself.

The next tutorial in this series walks you through creating custom metric charts that help you troubleshoot infrastructure-related edge cases. 
