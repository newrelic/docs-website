...
<Callout variant="important">
  You can add only one configuration per agent type to a fleet. Multiple configurations of the same agent type (for example, different configurations of the Kubernetes infrastructure agent) are not supported.<p/>
  An infrastructure configuration and an NRDOT configuration cannot be in the same fleet. Also, a Fluent Bit configuration must be accompanied by either an infrastructure or an OTel configuration.
</Callout>
