...
<Callout title="Important: Auto-injected values for Kubernetes">
  When deploying to a Kubernetes environment, you do not need to manually specify the `licenseKey`, `cluster`, `lowDataMode`, or `verboseLog` values in your configuration. These are automatically injected as environment variables from the Agent Control configuration. For more detail, see our documentation on [secrets management](https://www.google.com/search?q=/docs/new-relic-control/fleet-control/configuration).
</Callout>
