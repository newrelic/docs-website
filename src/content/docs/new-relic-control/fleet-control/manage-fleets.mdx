...
<img
  title="Fleet actions"
  alt="Reveal a number of fleet actions by clicking the ellipsis button in that fleet's corresponding row."
  src="/images/fleet-control-fleet-options.webp"
/>
...
<img
  title="Fleet permissions modal"
  alt="The Fleet Permissions modal showing dropdowns to select a user group and the Fleet Manager role."
  src="/images/fleet-control-fleet-fga.webp"
/>
...
<img
  title="Tag and Metadata buttons"
  alt="View the tagging and metadata information associated with a fleet by clicking the associated button."
  src="/images/fleet-control-metadata-button.webp"
/>
...
<img
  title="Viewing effective configuration"
  alt="By viewing an agents details under the agents tab in Fleet Control you can view its effective configuration."
  src="/images/fleet-control-agents-effective-config.webp"
/>
...
| Status                                                                                                                                      | Description                                                                                                                                                                                                                                                    |
|:--------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <span style="background-color: #007bff; color: white; padding: 2px 8px; border-radius: 4px; display: inline-block;">IN PROGRESS</span>      | The deployment is actively being rolled out to the managed entities in the fleet.                                                                                                                                                                              |
| <span style="background-color: #28a745; color: white; padding: 2px 8px; border-radius: 4px; display: inline-block;">COMPLETED</span>        | The deployment has successfully finished, and all intended actions have been applied.                                                                                                                                                                          |
| <span style="background-color: #dc3545; color: white; padding: 2px 8px; border-radius: 4px; display: inline-block;">FAILED</span>           | The deployment finished, but at least one agent on a managed entity reported an issue. This could mean an agent failed to start up, shut down correctly, or apply its new configuration. The deployment may be partially successful on other managed entities. |
| <span style="background-color: #fd7e14; color: white; padding: 2px 8px; border-radius: 4px; display: inline-block;">INTERNAL FAILURE</span> | The deployment could not be completed due to an unexpected issue within the Fleet Control system. This is not an error with your agents or configurations. Please try the deployment again, and if the issue persists, contact New Relic support.              |
...
4. **AgentDeploymentCompleted** — Each agent reports back with its status (CONFIG_APPLIED, FAILED, TIMEOUT, or NOT_REPORTING).
...
6. **FleetDeploymentCompleted** — After all rings complete (or a ring fails), the deployment finishes with a final status of COMPLETED, FAILED, or INTERNAL_FAILURE.

<img
  title="Fleet deployment event flow"
  alt="Visual diagram showing the fleet deployment event flow with rings, agent deployments, and status outcomes."
  src="/images/fleet-control-deployment-flow.webp"
/>
...
<img
  ...
  alt="The Agent Control auto-upgrade notification banner shown in the deployment pane."
...
<img
  title="Agent actions"
  alt="Reveal a number of agent actions by clicking the ellipsis button in that agent's corresponding row in the deployment pane."
  src="/images/fleet-control-agent-options.webp"
/>
...
<img
  title="Removing a managed entity from a fleet"
  alt="The verification screen for confirming the removal of an managed entity from a fleet."
...
<img
  title="Adding a managed entity to a fleet"
  alt="The selection pane for adding a managed entity to a fleet via a deployment."
...
<img
  ...
  alt="Set your managed entities for canary deployments via the canary selection action in the deployment pane."
  src="/images/fleet-control-canary-action.webp"
/>

fleet-control-canary-action
...
<Callout title="Important: Auto-injected values for Kubernetes" variant="important">
  When deploying to a Kubernetes environment, you do not need to manually specify the `licenseKey`, `cluster`, `lowDataMode`, or `verboseLog` values in your configuration. These are automatically injected as environment variables from the Agent Control configuration. For more detail, see our documentation on [secrets management](/docs/new-relic-control/fleet-control/configuration).
</Callout>
...
<img
  ...
  alt="From the configuration details pane, you can copy, download, clone, create and delete a configuration."
...
<img
  title="The fleet managed tab on the managed entities page"
  alt="By selecting the fleet managed tab under the managed entities page, you can view all of your managed entities that are under fleet control management."
  src="/images/fleet-control-me-page.webp"
/>
...
<img
  title="The unassigned tab on the managed entities page"
  alt="By selecting the unassigned tab under the managed entities page, you can view all of your unassigned managed entities."
...
<img
  ...
  alt="The agents page gives you a global overview of all of your agents managed by Fleet Control."
