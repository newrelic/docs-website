...
* **See fleet distribution:** View a breakdown of your fleets by type (for example, Kubernetes clusters vs. hosts).
...
<Callout title="Important: Fleet configuration rules">
  ...
  * **Exclusive infrastructure instrumentation:** The system prevents you from deploying both the New Relic Infrastructure agent and the New Relic Distribution of OpenTelemetry Collector (NRDOT) to the same fleet. This validation helps prevent accidentally double-instrument your managed entities for infrastructure monitoring.
...
<Callout title="Important: Upgrade Failure and Recovery">
...
* **Type:** The managed entity type (for example, Kubernetes or Host).
...
2. In the modal that appears, you must:
   ...
   * Select the **Agent type** (for example, New Relic Infrastructure, NRDOT collector, Fluent Bit).
...
<Callout title="Important: Deletion Restriction">
