...
The New Relic User API key is never a CLI flag. Provide it via:
  ...
  -e, --environment  us | eu | jp | staging (default: us)
...
if [[ -z "${NEW_RELIC_API_KEY:-}" ]]; then
  ...
  # stdin isn't a tty here, but that alone doesn't mean data is actually
  # coming: a caller that forgot to redirect stdin (a cron job, a systemd
  # service, a script invoked without `< /dev/null` or a real pipe) leaves
  # stdin open with nothing arriving, and a plain `read` would then block
  # forever. -t 10 turns that into a clear, bounded failure instead of a
  # silent hang.
  if ! IFS= read -r -t 10 NEW_RELIC_API_KEY; then
    echo "Error: no API key found on stdin (or none arrived within 10s). Set NEW_RELIC_API_KEY, or pipe the key in on stdin." >&2
    usage
    exit 1
  fi
fi
...
case "$ENVIRONMENT" in
  ...
  jp) GRAPHQL_URL="https://api.jp.newrelic.com/graphql" ;;
  staging) GRAPHQL_URL="https://staging-api.newrelic.com/graphql" ;;
  *)
    echo "Error: --environment must be one of us, eu, jp, staging (got '$ENVIRONMENT')." >&2
    exit 1
    ;;
esac
...
KEY_CONFIRMED=0
cleanup() {
  # Capture the real exit code FIRST -- an EXIT trap's own last command
  # otherwise becomes the process's reported exit status, which would turn a
  # fully successful run (KEY_CONFIRMED=1, so the rm below is correctly
  # skipped, and `[[ ... ]]` being false returns 1) into a script that prints
  # success but reports failure to anything checking $? or using `&&`.
  local exit_code=$?
...
curl -sS https://api.newrelic.com/graphql \
  ...
  --data-binary '{"query":"query { customerAdministration { systemIdentityGroups(filter: {organizationId: {eq: \"'"$NEW_RELIC_ORGANIZATION"'\"}, name: {eq: \"NR Control Group\"}}) { items { id } } } }"}'
