...
bootstrap = JSON.parse(`NEW_RELIC_API_KEY=#{node['newrelic']['api_key']} /tmp/newrelic-auth-cli create-bootstrap-identity secret \
...
This is the one step on this page where the API key is a `--api-key` flag rather than an environment variable, which means it's briefly visible in this process's `ps` output and shell history for as long as it runs. That's a real gap, not an oversight: an environment-variable alternative for this specific command isn't shipping (deprioritized — no customer has asked for it, and `--api-key` already works). If that gap matters more for your environment than the convenience of this one-liner, wrap the call so the key comes from your secrets manager directly into the process environment instead of a literal flag value.
