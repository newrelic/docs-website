...
## Where does Agent Control store the agent state?
...
Agent Control get's the content to create a file from variables of type `yaml`, and the content for multiple files inside a directory from variables of type `string_map`.
...
Notice that since all sub-agents have access to that shared directory, they can see each other's files.
...
## How does Agent Control fetch agent type definitions?

Agent Control fetches agent type definitions from a remote OCI registry, identified by the `namespace`, `name`, and `version` declared in the definition's metadata section. Taking into account the environment where it's running (i.e. kubernetes, linux or windows).

By default, Agent Control pulls from `docker.io` using the `newrelic/agent-control-agent-types` repository, once it could verify the signature against New Relic's public key.
