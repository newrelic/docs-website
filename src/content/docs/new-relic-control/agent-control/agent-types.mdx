...
<Callout variant="caution" title="NRDOT on Windows hosts is experimental">
  The New Relic OpenTelemetry Collector (NRDOT) on Windows is available but not officially tested or documented. The default bundled configuration is designed for Linux and may produce warnings or errors on Windows (for example, from `filelogreceiver` paths). No default configuration is bundled for Windows — you must supply your own collector configuration. Use NRDOT on Windows only in non-critical or testing environments until full Windows support is released.
</Callout>
