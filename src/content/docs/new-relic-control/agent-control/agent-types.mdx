...
Agent type definitions are fetched by Agent Control from a remote OCI registry, identified by the `namespace`, `name`, and `version` declared in the definition's metadata section.

By default, Agent Control pulls from `docker.io`, using the `newrelic/agent-control-agent-types` repository.

Each definition is signature-verified against New Relic's public key before pull.

This registry can be mirrored the same way as agent packages. See [Configure an OCI registry mirror](/docs/new-relic-control/agent-control/mirror-setup) for details.
