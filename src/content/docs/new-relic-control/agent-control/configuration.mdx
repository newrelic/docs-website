...
### Value providers for Kubernetes

On Kubernetes, Agent Control can inject values from Kubernetes secrets (`nr-kubesec`) and ConfigMaps (`nr-kubecm`) into agent configurations, so sensitive data doesn't need to be hard-coded. For the full reference, see [Value providers](#value-providers).
...
Value providers let Agent Control resolve values from external sources when a configuration is rendered, so sensitive data such as license keys, tokens, or passwords doesn't need to be hard-coded in your configuration files.
