...
To utilize secrets, you must define them within your Agent-Control configuration YAML file. The process involves three key steps:

1. **Define secrets_providers Section:** This section is the central location for configuring your secret providers. Each entry in this section should correspond to a supported provider.
2. **Configure Secret Sources:** For each provider, you must specify one or more sources. A source includes the necessary configuration details (e.g., URL, token) for Agent Control to connect to and retrieve a group of secrets.
...
### Kubernetes secrets
Kubernetes secrets do not require a separate sources configuration. Instead, Agent Control directly accesses secrets from the Kubernetes API. For this to work, the agent-control pod must have the necessary permissions (e.g., via a Service Account and Role-Based Access Control) to access the specific secrets and namespaces it needs.
...
Example of the full placeholder format:
```
