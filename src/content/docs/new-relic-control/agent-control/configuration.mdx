...
Agent Control currently only supports **Kubernetes**. The configurations and examples provided in this document are specific to this environment. We are working on a public preview for on-host Linux systems, and support for other environments will be added in the future.
</Callout>
...
### The Local `values.yaml` File

The local configuration file, used during installation, contains all the settings for Agent Control and its managed agents.
...
### Remote Configuration in New Relic Control
