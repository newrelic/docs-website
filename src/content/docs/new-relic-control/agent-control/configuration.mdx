...
Agent Control can retrieve sensitive data (such as passwords) from secret providers. Currently, Hashicorp Vault and Kubernetes (K8s) Secrets are supported.

#### Supported secrets providers
- Hashicorp Vault (nr-vault)
- K8s Secrets (nr-kubesec)

### Define secrets
To use secrets in a configuration YAML file:

1. **Define the secrets providers section:** Each entry should be a different provider from the supported list.
2. **Include the source configuration:** For each provider, specify the configuration needed to retrieve a group of secrets.
3. **Set placeholders in agent configurations:** Use the correct placeholder that will be automatically replaced with the secret from the specified source.

<Callout variant="important">
    If secrets retrieval fails, the configuration won't render, and the agent won't be executed.
</Callout>

For example, the following Agent-Control configuration defines two sources to retrieve secrets from Vault:
...
Then, In an agent configuration, you can add a vault placeholder with the correct path to retrieve a secret from those sources, for example:
...
Agent Control will render these secrets from the specified source and path, storing the result in a K8s secret or private config file for use by the corresponding agent.
...
- **mount**
- **path**
- **specific key** in the secret to be retrieved.
...
### Kubernetes secrets
For Kubernetes, there is no need for sources, but the agent-control pod must have the rights to access the required secrets.

In the configuration file, each secret value can be accessed by setting a placeholder with:
- **namespace**
- **name**
- **specific key** in the secret to be retrieved.
