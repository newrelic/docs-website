...
Agent Control's monitoring capabilities currently apply only to **Kubernetes** environments. We are working on a public preview for on-host Linux systems, and the centralized monitoring experience in New Relic Control will be available for all supported environments as they become available.
</Callout>
...
### Using the Local Status Endpoint
...
The ultimate goal of the Agent Control solution is to provide a comprehensive, centralized monitoring experience within the New Relic UI. This will move you away from needing to perform manual kubectl commands for health checks and give you a unified view of your entire fleet. This experience will be available for all environments supported by Agent Control, including the upcoming on-host support.


### Fleet-level Health Dashboard
In the future, the New Relic Control UI will provide a single dashboard where you can see the health status of all your fleets and the agents they manage. You will be able to:

View the aggregate health of a fleet.

See the status of each managed agent (e.g., Healthy, Unhealthy, Applying).

Check which version of a configuration is currently deployed.
