...
---
Agent Control is a lightweight agent supervisor that runs on Kubernetes clusters and hosts to manage the lifecycle of New Relic and open-source instrumentation agents. Communicating with Fleet Control, it remotely configures, updates, and monitors agent health across fleets. Agent Control and New Relic Control are generally available for Kubernetes, Linux, and Windows hosts.
...
<Callout variant="tip" title="Key behaviors">
  ...
  - **Remote install and upgrade on Linux:** On Linux hosts, you can install, configure, and upgrade agents remotely through Fleet Control — no manual intervention required on each host.
...
### Linux hosts
For host-based systems, Agent Control requires elevated permissions. The supervisor needs to run with root, administrator, or equivalent privileges to perform agent lifecycle tasks such as:
...
<Callout variant="important" title="Windows support limitations">
  **Supported agents:** On Windows hosts, Agent Control currently supports only the **New Relic Infrastructure Agent** and the **New Relic OpenTelemetry Collector (NRDOT)**. Other agent types (APM agents, Fluent Bit, Prometheus, eBPF, etc.) are not yet supported.

  **Minimum version requirements:** Agent Control on Windows can only manage agents that meet these minimum versions:
  - Infrastructure Agent: version 1.71.0 or higher
  - NRDOT (OpenTelemetry Collector): version 1.5.0 or higher

  Attempting to deploy older versions will fail.
  ...
  **No automatic migration:** Agent Control on Windows does not support automatic migration of existing ad-hoc agents. If you have agents already installed on your Windows hosts, you must uninstall them first, then manage them through Agent Control and Fleet Control.

  **Fleet type requirement:** When setting up Agent Control on Windows, you must create or use a fleet of type **"Host - Windows"**. This is mandatory for proper operation on Windows environments.
