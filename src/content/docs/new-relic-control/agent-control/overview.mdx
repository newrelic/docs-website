...
<Callout variant="important" title="Windows support limitations">
  **Supported agents:** On Windows, Agent Control currently supports only the **New Relic Infrastructure Agent** and the **New Relic OpenTelemetry Collector (NRDOT)**. Other agent types (APM agents, etc.) are not yet supported on Windows hosts.
