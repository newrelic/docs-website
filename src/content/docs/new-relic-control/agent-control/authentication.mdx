...
During the guided installation process, the first system identity is created using client credentials, which are included
 in the Helm chart's values. This system identity has a TTL of 12 hours. When it expires, the Agent Control Helm chart
 deployment will fail to authenticate with the Fleet Control service, resulting in the following error:
```
