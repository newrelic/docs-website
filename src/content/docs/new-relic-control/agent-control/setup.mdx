...
4. Follow the steps to complete the installation and configuration process.
  ...
  <Callout variant="important" title="Important">
    ...
    **Fleet type requirement:** Fleets are separated by type. You must select the right type for your hosts (Window or Linux) or k8s. Using a fleet of a different type will cause installation or operation issues.
...
<Callout variant="important" title="Migration">
  **Existing agents:** If you have the New Relic Infrastructure Agent already installed on your host, you must uninstall it before installing Agent Control. After installing Agent Control, you can manage the Infrastructure Agent [migrating your local configuration to Fleet Control](/docs/new-relic-control/agent-control/instrumentation/). APM agents (which are not currently managed by Agent Control on Windows) can remain installed and will continue to operate independently.
</Callout>
