...
<Callout variant="tip">
  Agent Control currently supports Kubernetes environments, with a public preview for on-host Linux systems in progress. More environments to be added soon.
</Callout>
...
Agent Control is designed to be a "super agent", able to manage your instrumentation, which requires it to perform powerful actions within your environment. To ensure a secure and functional setup, it's critical to understand and grant the required permissions.
...
### On-host Systems
For on-host systems (currently in public preview), Agent Control also requires elevated access. The supervisor needs to run with root or equivalent privileges to perform tasks such as:
...
Similar to Kubernetes, the on-host supervisor's power is balanced by the declarative model: it only takes actions that are defined in its configuration. This ensures that its elevated privileges are used to maintain the desired state of the agents, not to perform arbitrary, unapproved actions.

### Agent-Specific Permissions and Security
...
For detailed information on the specific permissions required by each agent, refer to the [supported Agent Types documentation page](/docs/new-relic-control/agent-control/agent-types). We are continually working to improve our permissions model to give you granular control, so you can maintain a secure environment with only the necessary access granted.