...
New Relic diagnostics tool `NRDiag` is a utility that can also be used to gather all the resources and logs related to agent-control in your cluster for debugging.
...
2. Run the K8s agent control suite (kubectl and helm should be installed).
    - This command will run in the namespace set in kubeconfig's context.
    ```bash
    ./nrdiag -suites K8s-agent-control
    ```

    - The command can also be run with the `--k8s-namespace` flag to specify a different namespace:
    ```bash
...
4. All the logs and resources related to the agent-control will be collected in a zip file called `nrdiag_output.zip` in the current directory.
You can analize the contents of the zip file or open a support ticket with [New Relic support](https://support.newrelic.com) to get further assistance.
