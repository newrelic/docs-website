...
<CollapserGroup>
  ...
  * Check the status of the `newrelic-agent-control` service:
    ...
    If no insights are available, check how to [run the agent in debug mode](/docs/new-relic-control/agent-control/troubleshooting/#debug) to access detailed logs explaining why the service cannot be started.
