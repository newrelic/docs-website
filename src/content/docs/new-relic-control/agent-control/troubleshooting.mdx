...
```yml
  ...
  <Collapser id="remote-config-debug" title="Invalid or unexpected remote configuration with Fleet Control">
    ...
    Additionally, configurations might have a valid format (for example, valid .yaml structure) but include unexpected values for certain settings (in example, string when integer is expected).
