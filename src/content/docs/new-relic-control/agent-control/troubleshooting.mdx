...
<CollapserGroup>
  ...
  <Collapser id="k8s-helm-uninstall-logs" title="Helm view uninstallation errors">
    When uninstalling, a job deletes all the created resources and the charts.
...
```yml
  ...
  <Collapser id="remote-config-debug" title="Invalid or unexpected remote configuration with Fleet Control">
    ...
    Additionally, configurations might have a valid format (for example, valid .yaml structure) but include unexpected values for certain settings (for example, string when integer is expected).
