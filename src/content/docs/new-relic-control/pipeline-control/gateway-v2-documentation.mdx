...
Pipeline Control Gateway offers two configuration approaches depending on your needs:

### UI-Based Configuration (Extending the Default Pipeline)

The Gateway UI lets you add rules to the default pipeline without writing any YAML. This is the simplest way to get started.

**What you can do:**
- Add transformation rules to modify attributes
- Create drop rules to filter out unwanted data
- Configure sampling rates with conditional logic

**How it works:**
The Gateway comes with a default pipeline that New Relic manages. When you use the UI to create rules, you're adding **processor extensions** to specific processors in that default pipeline. Your rules run alongside the default processing logic without modifying the core pipeline structure.

**Best for:**
- Quick setup for common use cases
- Drop rules and basic transformations
- Sampling configurations
- Teams that prefer visual interfaces

**Limitations:**
- Can't add new processor types
- Can't reorder or remove processing steps
- Limited to the processors New Relic provides in the default pipeline

### YAML-Based Configuration (Custom Pipelines)

For advanced scenarios, you can create a custom pipeline using YAML configuration. This gives you complete control over the processing logic.

**What you can do:**
- Everything the UI can do, plus:
- Define the exact sequence of processing steps
- Add multiple instances of the same processor type
- Create complex multi-step transformations
- Configure advanced processor settings

**How it works:**
When you write a YAML configuration, you're creating a **custom pipeline** that replaces the default pipeline for your gateway. You define each processing step, its configuration, and how data flows between steps.

**Best for:**
- Complex data processing requirements
- Organizations with infrastructure-as-code practices
- Advanced transformations requiring multiple steps
- Scenarios requiring specific processor ordering

**Trade-offs:**
- Requires understanding of YAML syntax
- More configuration to maintain
- Need to manage configuration in version control

### Choosing Your Approach

| Scenario | Recommended Approach |
|----------|---------------------|
| Drop test environment data | UI-based rules |
| Sample 10% of logs, keep all errors | UI-based rules |
| Add environment tags to metrics | UI-based rules |
| Parse logs, enrich, filter, then sample in sequence | YAML configuration |
| Need to reorder processing steps | YAML configuration |
| Want to version control all configuration | YAML configuration |
| Testing and iterating on rules | UI-based rules, then export to YAML |

<Callout variant="tip">
  You can start with UI-based rules and export the configuration to YAML later if you need more advanced capabilities. The UI shows you the generated YAML in real-time.
</Callout>

## Configuration Guide

<Callout variant="tip">
  This section covers YAML-based configuration for creating **custom pipelines**. If you're using the Gateway UI to add rules, you don't need to write YAML—the UI generates it for you. See [Gateway UI Configuration](#gateway-ui-configuration) for the form-based approach.
</Callout>
...
<Callout variant="important">
  The UI adds **processor extensions** to the Gateway's default pipeline. This means your rules extend the built-in processing without replacing it. For full control over the pipeline structure, use [YAML-based configuration](#configuration-guide) to create a custom pipeline.
</Callout>
