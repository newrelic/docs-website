...
Pipeline rules encompass two types: gateway rules and cloud rules. Cloud rules are authored using the NRQL-based methods described on this page and run in New Relic's infrastructure. Gateway rules use YAML-based configuration (not NRQL) and run in your infrastructure—refer to the [Gateway v2.0 documentation](/docs/new-relic-control/pipeline-control/gateway/overview) for gateway configuration. Cloud rules are account-specific, while gateway rules apply organization-wide.

## Gateway vs cloud rules

Pipeline Control offers two processing layers:

**Gateway rules** run in your infrastructure:
- Process data before it leaves your network
- Organization-wide configuration
- OpenTelemetry-based (YAML configuration)
- Full transformation, filtering, and sampling capabilities
- You manage the infrastructure (Kubernetes clusters)

**Cloud rules** run in New Relic's infrastructure:
- Process data after it reaches New Relic
- Account-specific rules
- NRQL-based configuration via UI
- Simple drop rules for data and attributes
- No infrastructure management required

Use gateway when you need data transformation, want to reduce network egress costs, or need dropped data to never leave your network. Use cloud rules for simple drop rules if you don't need transformation capabilities and prefer not to manage gateway infrastructure.

This page covers cloud rules. For gateway configuration, refer to [Gateway documentation]().
