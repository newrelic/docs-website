...
- **Sample based on HTTP status codes**: Keep all 4xx/5xx errors, sample 2xx successes
- **Sample by severity level**: Keep all ERROR/FATAL, sample INFO

### For Traces

Traces support **only global rate-based sampling**:

- **Apply a single sampling percentage to all traces**: Reduce overall trace volume with a uniform sampling rate
- **Note**: Conditional sampling rules are NOT supported for traces. The global sampling percentage applies to ALL traces, including errors and slow requests.

### For Metrics

Metrics sampling is **not currently supported** by the sampling processor. Use the [filter processor](/docs/new-relic-control/pipeline-control/gateway/filter-processor) to drop unwanted metrics instead.

<Callout variant="important">
**Important limitation for traces**: Unlike logs and events, traces do not support conditional sampling rules. When you set a `global_sampling_percentage` for traces, it applies uniformly to ALL traces—including error traces and slow traces.
</Callout>
...
- `conditionalSamplingRules`: Array of conditional rules (evaluated in order) - **only supported for logs and events**
...
For traces, you can only configure the global sampling percentage. This percentage applies to **all traces uniformly**, including error traces and slow traces:
