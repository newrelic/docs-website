...
8. Follow the step-by-step Helm installation commands shown in the UI wizard:
   ...
   **Step 1: Install Pipeline Control gateway**
   ...
   ```bash
   kubectl create namespace newrelic && helm upgrade --install newrelic-pcg newrelic/pipeline-control-gateway -n newrelic --values pipeline-control-gateway-values.yaml
   ```
   ...
   **Step 2: Install Agent Control**
   ...
   <Callout variant="tip">
   The UI wizard provides the complete values files (`pipeline-control-gateway-values.yaml` and `agent-control-deployment-values.yaml`) with your specific configuration. Download these files and use them with the Helm commands.
   </Callout>
...
2. Run:
   ...
   ```bash
      helm upgrade newrelic-pcg newrelic/pipeline-control-gateway --namespace newrelic --values pipeline-control-gateway-values.yaml
   ```
