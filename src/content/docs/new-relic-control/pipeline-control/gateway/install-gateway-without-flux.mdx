...
metaDescription: 'Install Pipeline Control gateway with reduced permissions and manual infrastructure management.'
freshnessValidatedDate: never
---

Install Pipeline Control gateway in configuration-only mode. New Relic continues to manage your pipeline configuration lifecycle (sampling, filtering, transforms) through the UI. You manage gateway infrastructure (scaling, versions) manually.
...
- You're comfortable managing Kubernetes resources manually.
- You're deploying in compliance-driven environments.

**What you get:**
- New Relic manages your pipeline configuration (sampling, filtering, transforms) automatically through the UI.
- Reduced permissions required (`ConfigMap` access only, not cluster-admin).
...
6. Configure the gateway:
   - Enter a unique, recognizable name for your new cluster.
   - Enter a namespace for gateway and Agent Control installations. Default is `newrelic`.
7. Configure gateway settings (minimum replicas, maximum replicas, CPU threshold). These settings create the initial Kubernetes HPA configuration.
8. Follow the step-by-step Helm installation commands shown in the UI wizard:

   **Step 1: Install Pipeline Control gateway**
   
   ```bash
   kubectl create namespace newrelic && helm upgrade --install newrelic-pcg newrelic/pipeline-control-gateway -n newrelic --values pcg-values.yaml
   ```

   **Step 2: Install Agent Control**
   
   ```bash
   helm upgrade --install agent-control-deployment newrelic/agent-control-deployment -n newrelic --values agent-control-deployment-values.yaml
   ```

   <Callout variant="tip">
   The UI wizard provides the complete values files (`pcg-values.yaml` and `agent-control-deployment-values.yaml`) with your specific configuration. Download these files and use them with the Helm commands.
   </Callout>
...
After installation, the gateway settings page in the New Relic UI displays fleet-level settings as read-only for scaling and version fields. However, these two toggles remain editable:
...
```bash
kubectl edit hpa newrelic-pcg-hpa -n newrelic
```

**Option 2: Update via Helm**
1. Edit your `pcg-values.yaml` file
2. Run:

   ```bash
      helm upgrade newrelic-pcg newrelic/pipeline-control-gateway --namespace newrelic --values pcg-values.yaml
   ```
...
1. Update the chart version in your Helm command:
   ```bash
   helm upgrade newrelic-pcg newrelic/pipeline-control-gateway --namespace newrelic --version <NEW_VERSION> --values pcg-values.yaml
   ```
...
**These work automatically through the UI** - no manual steps needed:
- Sampling rules
- Drop filters
- Transform rules
