    ...
    steps:
      ...
      probabilistic_sampler/Traces:
        ...
        output:
          - filter/Traces
        config:
          default_sampling_percentage: 70
      filter/Logs:
        ...
        config:
          ...
          rules:
            - name: drop entire metrics
              ...
              conditions:
                - (name == "humidity_level_metric" and IsMatch(resource.attributes["process_group_id"], "pcg_.*"))
              context: metric
            - name: drop datapoint
              description: drop datapoint on the basis of unit
              conditions:
                - (attributes["unit"] == "Fahrenheit" and (IsMatch(attributes["testId"], "pcg_.*") or IsMatch(resource.attributes["testId"], "pcg_.*")))
              context: datapoint
      filter/Traces:
        ...
        config:
          ...
          rules:
            - name: delete spans
              ...
              conditions:
                - (attributes["host"] == "host123.example.com" and (IsMatch(attributes["testId"], "pcg_.*") or IsMatch(resource.attributes["testId"], "pcg_.*")))
              context: span
