...
- **Without Flux:** These fields can be set during initial installation to configure the Horizontal Pod Autoscaler (HPA). However, post-installation changes via the UI are ignored. You must manually update scaling via HPA and versions via Helm upgrades. See [Managing gateway without Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway) for details.
</Callout>
