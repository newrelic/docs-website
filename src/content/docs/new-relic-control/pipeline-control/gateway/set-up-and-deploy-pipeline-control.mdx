...
<Callout variant="tip">
**Looking for more control over infrastructure management?** This guide covers full management installation where New Relic handles gateway infrastructure (scaling, versions, upgrades) through the UI. If your organization requires reduced permissions or prefers manual infrastructure control, see [Install Pipeline Control gateway without Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux) for configuration-only installation.
</Callout>
