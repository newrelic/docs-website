...
At New Relic, "integration" refers to any software technology that New Relic can make a connection to. So, for example, our agents contain various integrations (ways to report data from various app frameworks, or operating systems, or types of databases). Other integrations are configurations that allow a third-party service (for example, AWS Lambda or PagerDuty) to send data to us. 
