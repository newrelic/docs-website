...
4. On the **Choose install method** page, select either **Helm** or **manifest**. 
  ...
  <Collapser
    ...
    title="Helm method"
  >
  Copy the Helm command that's provided, and then run it on your command line. 

  If you want to reduce the amount of Pixie data that New Relic ingests, you can add the following parameters to your Helm chart. Note that the data still exists in Pixie:

  * `excludeNamespacesRegex` - use to set namespaces that you don't want to send observability data to New Relic. If empty, observability data for all namespaces is sent to New Relic. Example: `--set newrelic-pixie.excludeNamespacesRegex="examplenamespace-1|examplenamespace-2"`
  * `excludePodsRegex` - use to set pods across all namespaces that you do not want to send observerability data to New Relic. If empty, observability data for all pods (except those in excluded namespaces) is sent to New Relic. Example: `--set newrelic-pixie.excludePodsRegex="examplepod-1|examplepod-2"`
  
  Learn more about the available parameters for the `newrelic-pixie` Helm chart [here](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-pixie#chart-details). See also [this page about installing the Kubernetes integration using Helm](/docs/integrations/kubernetes-integration/installation/install-kubernetes-integration-using-helm/#h2-compatibility-and-requirements).
