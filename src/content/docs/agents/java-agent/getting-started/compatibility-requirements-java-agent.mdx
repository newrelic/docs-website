...
<CollapserGroup>
  ...
  >
    <Callout variant="tip">
      ...
      In order to continue to innovate and efficiently provide new capabilities to our customers who run on the JVM, support for older JVM versions will occasionally be dropped. In such cases you can continue using a version of the agent that supports an older JVM but note that new features and fixes will not be backported to old agent versions. We recommend upgrading to a currently supported JVM version to take advantage of the latest agent releases. Please see the table of compatible agent versions to determine which JVM versions are compatible.
    </Callout>
