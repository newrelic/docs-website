...
tags:
  ...
  - Java agent
  - Features
metaDescription: 'New Relic APM for Java: Real-time Java profiling using Java Flight Recorder (JFR) metrics.'
redirects:
...
* An [Insert API key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#event-insert-key) or an [APM license key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/new-relic-api-keys#ingest-license-key).
...
The application that you wish to monitor with the JFR daemon must be configured to expose JFR data over remote JMX by adding the following system properties:
...
<Callout variant="tip">
  By default, the JFR daemon will connect via JMX to localhost on port 1099 and send data to New Relic's US production metric and event ingest endpoints. Additionally, if you need the JFR daemon to report through a proxy that will require special configuration settings.
  
  To configure the JFR daemon's behavior, please see the [fully documented configuration options](https://github.com/newrelic/newrelic-jfr-core#how-to-use).
</Callout>
...
2. [Register an Insert API key](/docs/insights/insights-data-sources/custom-data/introduction-event-api#register) or an [APM license key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/new-relic-api-keys#ingest-license-key) for publishing data to New Relic.
3. **Required**: Assign your key to the `INSIGHTS_INSERT_KEY` environment variable. If you are using an APM license key then you will also need to set the `USE_LICENSE_KEY=true` environment variable.
4. **Recommended**: Set the name of the application being monitored to the `NEW_RELIC_APP_NAME` environment variable. If this is not set, then the [default app name](https://github.com/newrelic/newrelic-jfr-core#how-to-use) will be used instead.
...
* At this time, this requires deployment of both the [JFR daemon and Java agent](#requirements) and the daemon must be reporting as the same entity as the agent.
