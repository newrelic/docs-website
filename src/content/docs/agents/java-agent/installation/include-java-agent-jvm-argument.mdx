...
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/agents/java-agent/installation/include-java-agent-jvm-argument
---
...
<CollapserGroup>
  ...
  >
    The version of Apache Commons Daemon (jsvc) included with Tomcat 6 does not support the `-javaagent` argument used by New Relic. However, a build of the jsvc daemon from the trunk source will support the `-javaagent` argument via the `-X` prefix. See the [Apache bug tracking the issue](http://issues.apache.org/jira/browse/DAEMON-84).

    There is a fix in the Apache Commons source repository. For more information:

    * See the [Apache source repository documentation](http://commons.apache.org/svninfo.html).
    * Go directly to the [SVN source repository](http://svn.apache.org/repos/asf/commons/proper/daemon/).
  </Collapser>
</CollapserGroup>
