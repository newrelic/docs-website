...
<CollapserGroup>
  ...
  >
    ...
    Specifies a path to a custom SSL certificate bundle. These certificates are added to the certificates shipped as part of the Java Agent.

    <Callout variant="important">
      When using this option, you must also specify `use_private_ssl: true` or this setting will have no effect.
    </Callout>
  </Collapser>
...
<Callout variant="important">
  Requirements: Java 14, Java agent version 5.11.0 or higher, and the agent extension for JFR. You must also configure the [`insert_api_key`](#cfg-insert_api_key). For details, see [Real-time Java profiling using JFR metrics](https://docs.newrelic.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics).
</Callout>

To set these options, use the `jfr` stanza. To [override](#System_Properties) them, use a `newrelic.config.jfr` prefixed system property.
...
<CollapserGroup>
  ...
  >
    ...
    Set to `true` to enable Java Flight Recorder metrics.
  </Collapser>
  ...
  >
    ...
    Set to `true` to enable the JFR extension to log additional data to the agent log.
  </Collapser>
</CollapserGroup>
