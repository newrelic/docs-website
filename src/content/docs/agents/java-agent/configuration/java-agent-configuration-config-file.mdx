...
<CollapserGroup>
  ...
  >
    ...
    This setting is **required**. You must specify the [license key](/docs/accounts-partnerships/accounts/account-setup/license-key) associated with your New Relic account. This key binds your agent's data to your account in the New Relic service.
  </Collapser>
  ...
  >
    ...
    A valid [Insert API Key](https://docs.newrelic.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#event-insert-key) for your account. This is only required for [Real-time Java profiling using JFR metrics](https://docs.newrelic.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics).
  </Collapser>
...
The Java agent uses JMX to collect JVM data as well as to communicate with the JFR (Java Flight Recorder) daemon for [Real-time Java profiling](http://docs.newrelic.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics).
...
<CollapserGroup>
  ...
  >
    ...
    This setting can be enabled to allow the Java agent to expose linking metadata to the [JFR daemon](https://github.com/newrelic/newrelic-jfr-core). Doing so allows the JFR daemon to obtain the entity GUID generated by the Java agent and link JFR data with the same APM application that is being monitored by the agent instead of as a separate entity.
...
<CollapserGroup>
  ...
  >
    ...
    Defines a threshold (in seconds) for when the agent collects stack traces from an SQL call. When SQL statements exceed this threashold, the agent will cature the current stack trace. This is useful for pinpointing where long SQL calls originate from.
...
APM lets you [record custom event data](https://docs.newrelic.com/docs/insights/insights-data-sources/custom-data/insert-custom-events-new-relic-apm-agents) via the New Relic language agent APIs, which [you can then query](/docs/query-your-data/explore-query-data/explore-data/introduction-querying-new-relic-data).
...
<CollapserGroup>
  ...
  >
    ...
    Enables distributed tracing. Case sensitive: use `true` or `false`. For more information, see the [distributed tracing section](#distributed-tracing).
  </Collapser>
...
[Span events](https://docs.newrelic.com/docs/apm/distributed-tracing/ui-data/span-event) are reported for [distributed tracing](#distributed-tracing). Distributed tracing must be enabled to report span events.
