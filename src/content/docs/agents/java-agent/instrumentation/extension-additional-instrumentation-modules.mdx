...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="jfr-reporter"
    title="JFR Reporter for real-time Java profiling"
  >
    <Table>
      <tbody>
        <tr>
          ...
          <td>
            [Download the module](https://repo1.maven.org/maven2/com/newrelic/agent/extension/jfr-reporter/0.1.0/)
          </td>
        </tr>
      </tbody>
    </Table>

    The Java Flight Recorder (JFR) reporter extension [uses the JFR Event Streaming API](https://blogs.oracle.com/javamagazine/java-flight-recorder-and-jfr-event-streaming-in-java-14) and reports a continuous stream of JFR events as [dimensional metrics](/docs/using-new-relic/data/understand-data/new-relic-data-types#dimensional-metrics). These metrics are used in the **Real-time Java Profiling** dashboard in New Relic One, and you can also [query that data](/docs/using-new-relic/data/understand-data/query-new-relic-data) to create your own charts. This forms a great foundation on which to build JVM monitoring and observability tools. See [Real-time Java profiling with JFR metrics](https://docs.newrelic.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics) to learn more about the reporter's functionality.

    You configure the Java Flight Recorder Reporter extension the same way you configure the Java Agent, using environment variables, system properties, or `newrelic.yml`. The available properties, with defaults, are below. See [Java Agent Configuration](https://docs.newrelic.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#jfr) for more information about configuring this extension.
  </Collapser>
</CollapserGroup>
