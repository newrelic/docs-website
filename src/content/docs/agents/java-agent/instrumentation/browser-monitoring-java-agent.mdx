...
[Browser monitoring](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) gives you visibility into how your users are interacting with your Java application by using a JavaScript snippet (or "agent") to instrument your app's webpages. To enable Browser in the user interface, follow the procedures to [install the Browser agent](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent). For example, you can:
...
<Callout variant="tip">
  Before you enable Browser monitoring for use with the Java agent, make sure you are using the [latest version](/docs/release-notes/agent-release-notes/java-release-notes) of the Java agent.
</Callout>

## Instrument Browser using Java agent API

If your framework does not allow you to enable Browser monitoring from the UI, with the copy/paste method, or by using the REST API, you can manually instrument Browser by using the [Java agent API](/docs/agents/java-agent/api-guides/guide-using-java-agent-api).
...
2. [Access the Java agent API class](/docs/agents/java-agent/api-guides/guide-using-java-agent-api#api) by adding `newrelic-api.jar` to your application class path.
