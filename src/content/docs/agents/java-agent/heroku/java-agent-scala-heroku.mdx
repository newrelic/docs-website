...
[Heroku](https://devcenter.heroku.com/articles/newrelic) is a Platform as a Service (PaaS) solution for hosting web applications in various agent languages, including Scala. With New Relic, you can extend Heroku with metrics from [APM](/docs/apm/new-relic-apm) and [Browser](/docs/browser/new-relic-browser).
...
5. Also in `newrelic.yml`, add your [license key](/docs/accounts/install-new-relic/account-setup/license-key) to the [`license_key`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-license_key) setting.
...
* [Java agent and Heroku](/docs/agents/java-agent/heroku/java-agent-heroku) (for non-Scala installation on Heroku)
* [No data appears with Heroku](/docs/agents/java-agent/heroku/no-data-appears-heroku-java) (troubleshotoing steps after installation)
* [Scala installation for Java](/docs/agents/java-agent/frameworks/scala-installation-java) (Scala installation if you don't use Heroku)
