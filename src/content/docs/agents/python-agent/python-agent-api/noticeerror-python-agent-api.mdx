...
By default, the Python agent only reports unhandled exceptions. Use `notice_error` to record any Python exception as an error, which can then be found in New Relic One. If no parameters are provided, the details of the exception currently being handled will be used.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Optional and rarely used. A tuple containing exception information (`exc`, `value`, and `tb`) returned from [`sys.exc_info()`](https://docs.python.org/2/library/sys.html#sys.exc_info).
      </td>
    </tr>
    ...
    <tr>
      <td>
        ...
        _string, integer, callable(exc, value, tb)_
      </td>

      <td>
        Optional. The exception status code. This value can be a string, integer, or a callable that returns a status code.
      </td>
    </tr>
...
In large majority of cases, you won't need to pass any parameters. You would just call the following where you want to report an exception:
