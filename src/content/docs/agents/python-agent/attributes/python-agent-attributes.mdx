...
The following sections list the attributes that can be configured in the Python agent for traces started by outgoing libraries, like [httplib, boto3, requests,](https://docs.newrelic.com/docs/agents/python-agent/getting-started/instrumented-python-packages#external-web-services) and so on, or when using the [external trace API](/docs/agents/python-agent/python-agent-api/external-trace). For more information, see [Python agent configuration: attributes](/docs/python/python-agent-configuration#attributes).
...
The following sections list the attributes that can be configured in the Python agent for [span events](https://docs.newrelic.com/docs/apm/distributed-tracing/ui-data/span-event). For more information, see [Python agent configuration: attributes](/docs/python/python-agent-configuration#attributes).
...
<CollapserGroup>
  <Collapser
    ...
    title="error.class"
  >
    The class of exception, if one exists, that is recorded on the span event. Exception details can be recorded on spans with the [record_exception](https://docs.newrelic.com/docs/agents/python-agent/python-agent-api/record_exception) API.
  ...
  <Collapser
    ...
    title="error.message"
  >
    The exception message, if one exists, that is recorded on the span event. Exception details can be recorded on spans with the [record_exception](https://docs.newrelic.com/docs/agents/python-agent/python-agent-api/record_exception) API.
...
When using the `newrelic.agent.record_exception()` API call, you can pass in a dictionary of `params` that will be recorded as attributes on the traced error.
