...
With CPU runtime metrics, you can monitor CPU usage by examining user and system CPU utilization. You will also see time spent in different components of the CPU.
...
Our memory profiling capabilities allow you to monitor your system’s overall memory consumption and utilization by process and host.
...
The garbage collector finds unused data objects and reclaims that memory space for use by another process. The Python Agent provides comprehensive garbage collection metrics to give you insight into the objects being collected, the time spent in collection, and much more.

<Callout variant="important">
Garbage collection profiling is available in [agent version 6.2.0.156 and higher](https://docs.newrelic.com/docs/agents/python-agent/installation-configuration/upgrade-python-agent/) and is disabled by default in the agent. If you would like to enable it, check out our documentation [here](https://docs.newrelic.com/docs/agents/python-agent/configuration/python-agent-configuration/#garbage-collection-runtime-metrics).
</Callout>

**Object Count by Type**: Gain insight into the most common object types being collected by the garbage collector by viewing their associated collection counts. You can customize the maximum number of object types you would like to see reported by this metric type by configuring the [top_object_count_limit setting](https://docs.newrelic.com/docs/agents/python-agent/configuration/python-agent-configuration/#garbage-collection-runtime-metrics).
...
**Total Time by Generation**: Analyze the amount of time the garbage collector is spending in each generation of collection with this metric.
