...
<CollapserGroup>
  ...
  >
    The `analyticsEvents` element in **newrelic.config** has been obsolete. Enable the [`transactionEvents`](/docs/agents/net-agent/installation-configuration/net-agent-configuration#transaction_events) element in **newrelic.config**:
  ...
  >
    The capture attributes flag on `browserMonitoring`, `transactionTracer`, `transactionEvents`, and `errorCollector` has been obsolete. Instead, use the `attributes` sub-element to configure attribute settings for each of these destinations.
