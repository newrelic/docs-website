...
1. Create a new `.xml` file in the `Extensions` directory within your .NET Agent directory. The location of this direction depends on your OS:
...
2. Copy this template into the file you created. This template defines two separate class and methods as transactions but more can be added:
   ...
   <extension xmlns="urn:newrelic-extension">
     <instrumentation>
       ...
       <!-- Define the method which triggers the creation of a transaction. -->
       <tracerFactory name="NewRelic.Agent.Core.Tracer.Factories.BackgroundThreadTracerFactory" metricName="Category/Name2">
...
1. In the file you created, customize the attribute values `Category/Name`, `AssemblyName`, `NameSpace.ClassName`, and `MethodName`. Customize these values for both the trigger method and for any methods called by the trigger method.
   * `Category/Name`: Defines the transaction type. The New Relic UI groups transactions under these names in the [transaction type field](#custom-txn-screen). The category and name must both be specified, and must be separated by a slash.
