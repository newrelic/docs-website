...
tags:
  ...
  - NET agent
  - NET agent API
metaDescription: New Relic .NET agent API call to provides access to a distributed trace payload.
redirects:
...
<Callout variant="caution">
  This api usage is the result of the `CreateDistributedTracePayload()` api usage. However, the `CreateDistributedTracePayload()` is obsolete in the .NET Agent v9.0. Please use the [`InsertDistributedTraceHeaders`](/docs/agents/net-agent/net-agent-api/itransaction#insertdistributedtraceheaders) as it replaces the `CreateDistributedTracePayload`.
</Callout>
