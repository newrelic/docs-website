...
1. Remove all settings related to Application Insights from **Application** > **Settings** > **Application Settings** and **Save**. This may include the following keys:
    1. Any key that begins with `APPINSIGHTS_`
    2. Any key that begins with `APPLICATIONINSIGHTS_`
    3. Any key that begins with `XDT_MicrosoftApplicationInsights_`
    4. `DiagnosticServices_EXTENSION_VERSION` 
    5. `InstrumentationEngine_EXTENSION_VERSION`
