...
<Callout variant="tip">
  Older versions of Mocha can cause issues with our updated `add_method_tracer`. Mocha version 1.2.0 fixes this bug, so if after upgrading agent versions, you run into errors in your test suite such as
