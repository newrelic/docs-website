...
1. Follow standard procedures to [install New Relic's Node.js agent](/docs/agents/nodejs-agent/installation-configuration/install-nodejs-agent), including your [license key](/docs/accounts-partnerships/accounts/account-setup/license-key). Be sure to save the `newrelic` module to the `package.json` file.
