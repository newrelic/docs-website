...
Ignore the whimsy at your peril!
</Callout>
...
<Table>
  ...
  <tbody>
    <tr>
      <td>A cycling monkey</td>
      <td>One old-timey shark</td>
    </tr>
    <tr>
      <td>Nothing to see here, folks</td>
      <td>A long-overdue nap</td>
    </tr>
  </tbody>
</Table>

## All the collapsers in the world

  <CollapserGroup>
     <Collapser
       id="whimsy"
       title="All the whimsy"
     >
      Does the content go here? Fnord!
     </Collapser>
     <Collapser
       id="no-whimsy"
       title="No whimsy"
     >
      There's nothing but seriousness here.
     </Collapser>
  </CollapserGroup>
...
1. Monkey
2. Tuba
3. Alphabet numbskull
    * one
    * two
    * three


## Code blocks til the end of time
...
<CollapserGroup>
  ...
  >
    ...
    This setting is required. Your New Relic [license key](/docs/accounts-partnerships/accounts/account-setup/license-key). For example, `license_key: '40HexadecimalCharacters'`.
  </Collapser>
  ...
  >
    ...
    Set your Apdex T [via the New Relic UI](https://docs.newrelic.com/docs/apm/new-relic-apm/apdex/changing-your-apdex-settings).
  </Collapser>
  ...
  <Collapser
    ...
    title="labels"
  >
    Use [labels and categories to organize your apps](/docs/apm/new-relic-apm/maintenance/labels-categories-organize-your-apps-servers). Specify your labels as objects or a semicolon-delimited string of colon-separated pairs (for example, `Server:One;Data Center:Primary`).
  ...
  >
    ...
    The agent communicates with New Relic via HTTPS by default, and New Relic [requires HTTPS](/docs/apis/rest-api-v2/troubleshooting/301-response-rest-api-calls) for all traffic to New Relic and the New Relic REST API. When enabled, the agent uses SSL to connect to the [New Relic collector](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector). Must be `true` to enable [`high_security`](#high_security).

    <Callout variant="caution">
      By default the agent is compliant with [high security v1](/docs/accounts-partnerships/accounts/security/high-security#version1descritpion). However, if you disable `ssl` and enable [`capture_params`](#params), the agent can exit high security mode, which prevents the agent from connecting with high-security accounts. If high security mode fails, the log files show an error message such as `ERROR Account Security Violation`.
    </Callout>
  </Collapser>
</CollapserGroup>
