...
translate:
  - jp
metaDescription: 'Recommendation: When you first deploy your app in New Relic, edit your config file and change the default name (typically My Application) to a descriptive name.'
redirects:
...
New Relic connects metrics to individual applications using your [license key](/docs/accounts-partnerships/accounts/account-setup/license-key) and the primary application name. You can change the app's name in your config file, or assign an alias to the app to change how it appears in the New Relic UI.
...
## Multiple names for an app [#multiple-names]
...
The application name you set in the New Relic user interface is a human-readable **alias**. If you rename your app, the visible name is updated everywhere in the New Relic site, including [New Relic Browser](/docs/browser/new-relic-browser/installation-configuration/rename-browser-apps).
...
1. Go to **[rpm.newrelic.com/apm](https://rpm.newrelic.com/apm) > (select an app) > Settings**.
