...
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/agents/manage-apm-agents/app-naming/use-multiple-names-app
---
...
Normally, when two instances report with the same app name, agent language, and license key, New Relic aggregates their data into a single app within the New Relic user interface. You can also view data for the individual hosts and instances from the app's New Relic [APM **Overview** page](/docs/apm/applications-menu/monitoring/apm-overview-page#infra-server).
...
By default, an app with multiple names will generate multiple Insights events for transactions (a duplicate transaction for each name). For example, if you give your app three names, that's three times the number of Insights events for transactions.

To avoid duplicate Insights events, disable collection for each of the duplicate app names:

1. Go to **[insights.newrelic.com](https://insights.newrelic.com) > Manage data > Configure data sources**.
2. Toggle data collection on/off for duplicate app names, then save.

![Insights transaction events](./images/Insights-ToggleOnOff.png "Insights transaction events")

To prevent duplicate events from being collected for an app with multiple names, go to **[insights.newrelic.com](https://insights.newrelic.com) > Manage data > Configure data sources**.
...
<CollapserGroup>
  ...
  >
    ...
    This will report four applications in New Relic APM:
  ...
  >
    ...
    This example configuration will report seven applications in the New Relic APM UI:
...
If you do not want to apply multiple names to your apps, you can organize them with [labels and categories](/docs/data-analysis/user-interface-functions/organize-your-data/labels-categories-organize-apps-servers-monitors). This allows you to easily sort, filter, and page through them from their product index pages in the New Relic UI.
