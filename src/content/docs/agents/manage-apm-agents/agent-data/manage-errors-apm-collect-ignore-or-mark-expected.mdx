...
topics:
  ...
  - Agent data
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected
---
...
* Filter out noise from [expected errors](#expected) so you can focus on the errors that are affecting performance. (Java, Ruby, Node, and .NET agents only)
...
Sometimes the APM agent instruments an error that you don't want reported, such as errors that contain sensitive information like user login errors. If you don't want an error to report to the our [collector](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector), you can ignore the error, and the APM agent discards the error entirely.

<Callout variant="tip">
  For Java, Ruby, and Node.js: If you want to report errors to APM but don't want those errors to affect your Apdex or error rate, [mark them as expected](#expected-links) instead.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    * **Python**: [`error_collector.ignore_errors`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#error-ignore) or [`error_collector.ignore_status_codes`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#error-ignore-status-codes).
...
## Expected errors (Java, Node.js, Ruby, and .NET only) [#expected]

For the Ruby and Java agents, you can mark errors as expected. These errors will be reported to APM and available for viewing, but they won't affect the [Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) or [error rate](/docs/apm/applications-menu/error-analytics/error-analytics-explore-events-behind-errors) (or [alert conditions](/docs/alerts/new-relic-alerts/defining-conditions/define-alert-conditions) based on error rate).
