...
Once you've installed the baseline dashboard `link here`, you should keep an eye on it to detect any ingest anomalies as often as possible. While you have to wait until you have enough data to optimize your ingest, you can use your dashboard to find issues with your ingest much sooner. You can use the baseline dashbaord as is, or download optional dashboards to provide you even greater anomaly detection options.
...
## Use the Lookout view to monitor anomalies [#lookout]

The baseline dashboard you set up in the previous step of this tutorial has a feature called **Lookout**. In a nutshell, this feature lets you provide a NRQL query to instantly search for ingest anomalies over any period of time. For example, the following NRQL query will give you information on all ingest anomalies by `usageMetric` over the last 24 hours:
...
Previously in this tutorial, you installed the ingest baseline dashboard that uses `NrConsumption` as its primary source. But you may find that you also want information on estimating ingest traffic for other events or metrics. If you do, you can install the entity breakdown dashboard, which helps you create other visualizations that use `bytescountestimate()` to help estimate your ingest data. 
...
`Need a link to the dashboard package and how to install it`
...
<UserJourneyControls
    nextStep={{path: "/docs/tutorial-optimize-telemetry/optimize-your-ingest/", title: "Next step", body: "Learn how to optimize your telemetry data and help reduce your ingest costs"}}
