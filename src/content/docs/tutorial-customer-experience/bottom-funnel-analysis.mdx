---
title: "Analyze the bottom-of-funnel"
tags:
...
Even though [improving customer experience](/docs/tutorial-customer-experience/improve-customer-experience/) is often done comprehensively, you may find that focusing your efforts on improving conversion can also provide a large increase in customer experience quality. This is what's known as bottom-of-the-funnel analysis is about improving conversion by focusing on performance at the end of the user journey instead of the customer experience as a whole. 
...
When a user completes a journey, it's reffered to as a conversion. All conversions have a value, and the best way to improve the conversion rate is to start at the bottom of the funnel, when the intent to complete the user journey is clear.
...
* Higher return on investment by focusing on the stage where users are already more likely to convert. Addressing performance issues here will see an immediate impact on the bottom line.
...
The final steps of a user's journey is likely to be a mix of full page loads and AJAX calls. You'll need to know all pages and AJAX requests for the next step. If you're not sure which requests are running from the page in question, you can run:
...
<Callout variant="tip">
A common question is: Should I apply segmentation (breaking out performance into cohorts, such as region and device type)? It depends on your needs, but segmentation is a good idea if:
...
After you've followed the procedure above, it's a good idea to re-analyze the bottom-of-the-funnel whenever you can. Each organization will have different needs, and so the procedures you use will be unique to your organzation. However, you can keep the following suggestions in mind:
