...
All REST API v2 functionality has been replaced by NerdGraph, New Relic's GraphQL API, which has been the recommended API for over six years. NerdGraph offers a single unified endpoint, precise data fetching, and strong typing.
...
* Alerts endpoints (`/v2/alerts*`) are not included in this EOL.
...
You may be affected if your account has made calls to the REST API v2 within the past 3 months. To determine what action you need to take:

<Callout variant="tip">
  If your account only uses `/v2/alerts*` endpoints, no action is required.
</Callout>
