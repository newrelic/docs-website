...
1. Declare props with default values in a JSX comment: `{/* PROPS: propName="defaultValue" */}`
2. Use `{{propName}}` or `{{ propName }}` as placeholders in your content (spaces around the name are allowed)
3. Pass values when using the component: `<Component propName="value" />`
...
| Props | `{{propName}}` | replaced with prop value at render time |
