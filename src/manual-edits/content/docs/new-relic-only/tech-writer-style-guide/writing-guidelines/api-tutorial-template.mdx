...
**This document is a template for an agent API tutorial document**: You might create this when you want to explain a commonly used procedure that uses one or more agent API calls. For example, see [Java async API](/docs/agents/java-agent/async-instrumentation/java-agent-api-asynchronous-applications). Please skim the entire template first to understand the expected structure for this type of doc.

**For the doc title**: Doc should be named in a practical, use-case-focused way. Example: _Add custom attributes to transactions_

## Introduction (this heading won't be visible)
...
Explain how a user would know they'd completed the task correctly. In particular, how would the user find the new change or data in the New Relic UI. What NR products and pages would the change be noticed on? If new data is available for querying, what data types is it attached to?
