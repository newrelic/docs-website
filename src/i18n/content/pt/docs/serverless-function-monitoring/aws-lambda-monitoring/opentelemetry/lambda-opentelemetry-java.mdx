...
3. Em
   ...
   , cole um dos ARNs da camada para a arquitetura da sua função na lista abaixo. Substitua `{region}`pela sua região da AWS, como `us-east-1`.
...
5. Para
   ...
   defina-o como `AWS_LAMBDA_EXEC_WRAPPER` e defina

   <DoNotTranslate>**Value**</DoNotTranslate>

   como uma das opções a seguir (depende do tipo de manipulador). Em seguida, escolha
