...
<CollapserGroup>
  ...
  >
    framework sem servidor é uma ferramenta popular de desenvolvimento e implantação de aplicativos sem servidor. Ele foi escrito em Node.js e, para AWS, atua principalmente como uma abstração de nível superior sobre modelos CloudFormation. Funciona bem para funções Node, Python, Ruby, Java e .NET.
...
<CollapserGroup>
  ...
  >
    ...
    5. Defina o <DNT>**Subscription filter pattern to**</DNT> `?REPORT ?NR_LAMBDA_MONITORING ?"Task timed out" ?RequestId`. Como alternativa, se você estiver usando a variável de ambiente `LOGGING_ENABLED` [para transmitir todo o seu log](#stream-all-logs) para o nosso log, deixe este campo em branco.
