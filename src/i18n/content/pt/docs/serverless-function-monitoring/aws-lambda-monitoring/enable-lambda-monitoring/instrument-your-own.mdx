...
<CollapserGroup>
  ...
  >
    Framework Serverless é uma ferramenta popular de desenvolvimento e implantação para aplicativos serverless. Ele foi escrito em Node.js e, para AWS, atua principalmente como uma abstração de nível superior sobre modelos CloudFormation. Funciona bem para funções Node, Python e Java.
...
Embora seja mais sujeito a erros e trabalhoso do que as abordagens acima, é possível alterar manualmente a configuração de uma função do Lambda para usar o New Relic do AWS Lambda Console, para Node.js, Python e Java.
...
2. Para Python, Node.js e Java, também incluímos:
