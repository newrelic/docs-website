...
<Callout variant="tip">
  **Instância gerenciada do Lambda (LMI)**: a instrumentação em camadas é compatível com funções LMI. Anexe a camada do New Relic Lambda como faria para qualquer função do Lambda — as funções LMI são executadas automaticamente no modo APM sem exigir `NEW_RELIC_APM_LAMBDA_MODE = true`.
</Callout>
...
<CollapserGroup>
  ...
  <Collapser id="cloudformation" title="Modelos CloudFormation/SAM">
    ...
    1. Instale a [camada Lambda do New Relic](https://layers.newrelic-external.com/). Abaixo estão exemplos para funções do Lambda do agente Node.js e Java.
       ...
       **Exemplo de Java legado (baseado no Open Tracing):**

       <Callout variant="important">
         Recomendado para clientes que usam o monitoramento Serverless para Java antes de 10 de abril de 2026 ou se estiverem usando o Java 8 ou 11.
       </Callout>
       ...
       **Novo exemplo de agente Java (camada slim, recomendado):**

       <Callout variant="important">
         Esta é uma solução separada da solução baseada em Open Tracing e só foi lançada desde 10 de abril de 2026. A nova camada do agente Java requer Java 17 ou posterior. Os runtimes do Java 11 e anteriores não são suportados.
       </Callout>
  ...
  <Collapser id="console" title="Instrumentação manual AWS">
    Embora seja mais propenso a erros e trabalhoso do que as abordagens acima, você pode alterar manualmente a configuração de uma função do Lambda para usar o New Relic a partir do console do AWS Lambda, para Node.js, Python, Ruby e Java (que inclui a solução legada do Java baseada em Open Tracing e a nova solução alternativa baseada em agente APM).
    ...
    5. Vá para a [lista de camadas do New Relic](https://layers.newrelic-external.com/) e use a lista suspensa para selecionar a região AWS onde sua função do Lambda está hospedada. A partir daí, localize o ARN que corresponde à versão e arquitetura Ruby (ou de qualquer outra linguagem) da sua função do Lambda. Deve haver duas opções: X86 e ARM64. Use o botão **Copy to clipboard** ou copie manualmente a string ARN.

       * Os clientes que usam Java encontrarão camadas Java da convenção antiga e camadas do agente Java da nova convenção, que representam **2 soluções separadas para o monitoramento do lambda Java**.

         * **A camada Java legada** é baseada no Open Tracing, que é recomendado para clientes que usam nosso monitoramento Serverless antes de 10 de abril de 2026 e para clientes no Java 11 e/ou 8. Todos os ARNs contêm a substring `NewRelicJava`. Um exemplo de ARN de tal camada é:

           ```
           arn:aws:lambda:us-east-1:451483290750:layer:NewRelicJava17:21
           ```

         * **Nova camada do agente Java** é baseada no agente Java do APM e é recomendada para novos aplicativos Java. Há uma camada completa e uma camada slim que reduz a visibilidade, mas melhora o desempenho de inicialização a frio. **Este não é um substituto imediato para a solução legada do Open Tracing**. Confira nossa [documentação do agente APM](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda) para obter mais detalhes. Todos os ARNs contêm a substring `NewRelicAgentJava`. Um exemplo de ARN de tal camada é:
    ...
    * Java (solução legada baseada em Open Tracing):
    ...
    * Para a nova solução do agente Java, você não precisa definir o manipulador.
