...
<CollapserGroup>
  <Collapser id="go" title="Go">
    ...
    7. Invoque o Lambda pelo menos uma vez. Isso cria um grupo log do CloudWatch, que deve estar presente para que a próxima etapa funcione.
       ...
       Nosso wrapper coleta dados sobre a execução do Lambda, gera uma mensagem JSON e a registra no CloudWatch Logs. Em seguida, será necessário [configurar o CloudWatch para enviar esses logs para o New Relic](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/cloud-watch-fallback).
  </Collapser>

  <Collapser id="java" title="Java">
    <Callout variant="tip">
      Esta solução não usa o agente APM Java. Se você deseja usar o agente APM Java no AWS Lambda, consulte a nossa [Documentação da Solução Serverless do Agente Java](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda). A solução baseada no agente APM não deve ser executada com a solução baseada em SDK.
    </Callout>

    O monitoramento para AWS Lambda em Java usando a solução baseada em SDK usa duas dependências do Open Tracing:
  ...
  <Collapser id="python" title="Python">
    ...
    9. Invoque o Lambda pelo menos uma vez. Isso cria um grupo log do CloudWatch, que deve estar presente para que a próxima etapa funcione.
       ...
       O decorador do New Relic coleta dados sobre a execução do Lambda, gera uma mensagem JSON e a registra no CloudWatch Logs. Em seguida, [configure o CloudWatch para enviar esses logs para o New Relic](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/cloud-watch-fallback).
  </Collapser>
