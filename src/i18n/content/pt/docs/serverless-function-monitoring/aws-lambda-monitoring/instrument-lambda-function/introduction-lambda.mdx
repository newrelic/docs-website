...
<Callout variant="tip">
  O monitoramento do New Relic Lambda oferece suporte a funções de instância gerenciada do Lambda (LMI). As funções LMI são detectadas automaticamente e executadas no modo APM — nenhuma configuração extra é necessária.
</Callout>
...
## Configurar o monitoramento de AWS Lambda [#config]
...
<Steps>
  ...
  <Step>
    ...
    <Tabs>
      ...
      <TabsPages>
        <TabsPageItem id="1">
          Para que New Relic monitore sua função Lambda, você precisa configurar uma integração entre New Relic e AWS. Recomendamos o uso do [Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) para ajudar a reduzir a latência e evitar a limitação de nossas requests de polling.
        </TabsPageItem>
