...
<Callout variant="tip">
  Se variáveis aninhadas poluírem a barra de variáveis do seu dashboard, você pode ocultar variáveis pai ou de suporte que não precisam de interação do visualizador. Variáveis ocultas continuam funcionando normalmente nos bastidores. Para mais informações, consulte [Mostrar e ocultar variáveis](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility).
</Callout>