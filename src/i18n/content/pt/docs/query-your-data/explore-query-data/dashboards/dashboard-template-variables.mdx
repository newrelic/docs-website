...
tags:
  ...
  - Hidden variables
  - Variable visibility
metaDescription: 'For New Relic custom dashboards: use template variables to dynamically filter your dashboard views, and control variable visibility to create cleaner interfaces.'
...
## Exibir e ocultar variáveis [#variable-visibility]

Você pode ocultar variáveis de template da barra de variáveis no modo de Visualização enquanto elas continuam funcionando normalmente. No modo de edição, as variáveis ocultas são exibidas com um ícone <Icon name="fe-eye-off" />, para que os editores do dashboard possam identificá-las e gerenciá-las.

Recomendamos ocultar variáveis que:

* Poluir a barra de variáveis, como [variáveis aninhadas](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Controle a lógica do dashboard sem exigir a entrada do visualizador
* Derivar valores automaticamente de outras variáveis

Para ocultar uma variável, use a opção <DNT>**Hide on variables bar**</DNT> ao [definir uma variável de template](#step1). Para configurar isso programaticamente, use o campo `hiddenOnVariablesBar` na [API de dashboards do NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).

<Callout variant="tip">
  Para ocultar múltiplas variáveis de uma só vez, use a [API de dashboards do NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards) para atualizar múltiplas variáveis programaticamente, pois a UI só permite ocultar variáveis uma por vez.
</Callout>

## Requisitos e limitações [#requirements]

As variáveis de modelo só podem ser usadas no contexto da criação de widgets para dashboards personalizados. Por esse motivo, algumas funcionalidades relacionadas a consultas não funcionam. Por exemplo, a opção `Export dashboard as PDF` não suporta widgets com variáveis, e as consultas SQL [<DNT>Lens</DNT>](/docs/new-relic-lens/query-data) não suportam variáveis de template. Veja [variáveis NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#with-as-nrql-var) para usar variáveis em uma consulta NRQL.
...
<Steps>
  <Step>
    ...
    Defina uma variável de template que você usará em uma consulta NRQL para criar um widget.

    Para definir uma variável:

    1. A partir de um novo dashboard sem variáveis, clique no botão de edição <Icon name="fe-edit-2" /> e, em seguida, clique no botão <DNT>**+ Add variable**</DNT>.
    ...
    2. Conclua o fluxo de trabalho <DNT>**Add variable**</DNT>. A seguir estão algumas regras e dicas para cada um dos campos.
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>
            <DNT>Name to use in queries</DNT>
          </td>
        ...
        <tr>
          <td>
            <DNT>Fetch data from this account</DNT>
          </td>
        ...
        <tr>
          ...
          <td>
            ...
            A opção **Include variable** determinará a configuração padrão, incluir ou excluir, para essa variável no dashboard. O usuário visualizando o dashboard pode modificar esta configuração usando a opção **Include variable** no menu dropdown de variáveis. A configuração selecionada pelo usuário será válida durante toda a sessão. Para obter detalhes sobre onde encontrar essa opção e como ela afeta as consultas, consulte [Incluir e Excluir Variáveis](#include-variable).
        ...
        <tr>
          <td>
            <DNT>Hide on variables bar</DNT>
          </td>

          <td>
            Opcional. Ocultar a variável da barra de variáveis no modo de Visualização. As variáveis ocultas continuam funcionando normalmente. No modo de Edição, as variáveis ocultas são exibidas com um ícone <Icon name="fe-eye-off" />. Para obter mais informações, consulte [Mostrar e ocultar variáveis](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>

    Após definir sua variável de template, você pode adicionar um widget que utilize a variável de template criada.
  </Step>
  ...
  <Step>
    ...
    5. (Opcional) Para personalizar o gráfico, selecione o ícone <DNT>**See customization options**</DNT> ao lado do menu suspenso <DNT>**Chart type**</DNT>. Para opções disponíveis, consulte [Personalize seus gráficos](/docs/query-your-data/explore-query-data/use-charts/use-your-charts#customize-charts).

    6. Clique <DNT>**Add to dashboard**</DNT>
...
A opção <DNT>**Include variable**</DNT>, ativada por padrão, controla se o dashboard filtra pelo valor selecionado da variável. Como essa configuração é específica para essa única variável, em vez de todas as variáveis no dashboard, ela fica dentro do próprio dropdown de valor da variável, em vez de na barra de variáveis compartilhadas. Você pode:

* É possível excluir a variável da consulta desativando o botão de alternância. Isso substitui a condição da variável por um valor booleano neutro (`true`).
* Restaurar tanto o botão de alternância quanto o valor para os padrões configurados da variável usando <DNT>**Reset to default value**</DNT>.

Esta seleção é válida apenas para a sessão atual. Para definir o estado padrão da opção ao definir uma variável, consulte [Valor Padrão](#step1).
...
Este recurso é útil quando:

* A variável tem mais valores do que o máximo permitido (por exemplo, 5.000 resultados máximos para exclusivos por padrão) ou um número muito alto de valores. Desativar a variável por padrão reduz a carga de consulta.
