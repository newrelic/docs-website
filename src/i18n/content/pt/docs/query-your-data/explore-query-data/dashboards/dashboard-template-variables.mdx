...
O botão de alternância <DNT>**Include variable**</DNT> permite definir um valor desejado e incluir ou excluir esse valor nos seus dashboards. Consulte a seção [variáveis de modelo](#create-variables) para mais informações.
...
Você pode ocultar variáveis de template da barra de variáveis no modo de Visualização, enquanto elas continuam funcionando normalmente em segundo plano. No modo de edição, as variáveis ocultas são exibidas com um ícone <Icon name="fe-eye-off" />, para que os editores de dashboard possam identificá-las e gerenciá-las.

Considere ocultar variáveis que:

* Adicione desordem à barra de variáveis, como [variáveis aninhadas](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Potencialize a lógica do dashboard sem precisar de entrada do visualizador
* Derivar valores automaticamente de outras variáveis

Para configurar a visibilidade, use a opção <DNT>**Hide on variables bar**</DNT> ao [definir uma variável de template](#step1). Para acesso programático, use o campo `hiddenOnVariablesBar` na [API de dashboards do NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Opcional. Ative esta opção para ocultar a variável da barra de variáveis no modo de Visualização. A variável continua funcionando normalmente nos bastidores. No modo de edição, as variáveis ocultas são exibidas com um ícone <Icon name="fe-eye-off" />. Para mais informações, consulte [Mostrar e ocultar variáveis](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>
