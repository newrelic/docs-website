...
Here's an example of the resulting widget, on the right, with the `country` dropdown to the left.<img title="Country template variable example" alt="Country template variable example" src={dashboardsCountryVariableExample}/>
