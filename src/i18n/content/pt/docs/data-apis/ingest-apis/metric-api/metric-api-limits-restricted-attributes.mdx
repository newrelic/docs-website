...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Máximo de séries temporais únicas (cardinalidade) por conta por dia
      </td>

      <td>
        1-15 milhões [(saiba mais)](#additional-considerations)

        Uma série temporal é uma combinação única de um nome de métrica e qualquer atributo.
      </td>
    </tr>

    <tr>
      <td>
        Máximo de séries temporais exclusivas (cardinalidade) por nome de métrica por dia
      </td>
    ...
    <tr>
      ...
      <td>
        Tamanho ou comprimento máximo total: <DoNotTranslate>**1MB (10^6 bytes) maximum per POST**</DoNotTranslate>. É altamente recomendável usar compactação.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        Máximo de séries temporais exclusivas de contagem e resumo (cardinalidade) por conta em intervalo de 5 minutos
      </td>

      <td>
        1-15 milhões [(saiba mais)](#additional-considerations)

        Uma série temporal é uma combinação única de um nome de métrica e qualquer atributo. As séries temporais recebidas acima deste limite são descartadas. Este limite é aplicado antes e além dos limites métricos padrão.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  <Collapser
    ...
    title="Máximo de pontos de dados por minuto (DPM)"
  >
  ...
  <Collapser
    ...
    title="Máximo de séries temporais exclusivas por conta por dia"
  >
    ...
    Se o limite da série temporal métrica exclusiva por conta e por dia (cardinalidade) for excedido durante um período de 24 horas, o endpoint continuará a receber e armazenar dados métricos brutos. No entanto, a New Relic irá parar de criar rollups agregados adicionais (1 minuto, 5 minutos, etc.) durante o restante do período de 24 horas. (Esses rollups são usados por padrão para consultar janelas de tempo superiores a 60 minutos.)
  ...
  <Collapser
    id="incident-unique-timeseries"
    title="Máximo de séries temporais exclusivas por nome de métrica por dia"
  >
    Uma série temporal é uma combinação única de um nome de métrica e qualquer atributo atribuído a essa métrica. Por exemplo, se uma métrica `CPU utilization` com um único atributo `hostname` for enviada de dez hosts diferentes, isso equivale a dez valores distintos para o atributo `hostname` e dez séries temporais de métricas únicas.

    Se o limite de série temporal de métrica exclusiva por dia (cardinalidade) por nome por métrica for excedido durante um período de 24 horas, o endpoint continuará a receber e armazenar dados de métrica bruta. No entanto, a New Relic irá parar de criar rollups agregados adicionais (1 minuto, 5 minutos, etc.) durante o restante do período de 24 horas. (Esses rollups são usados por padrão para consultar janelas de tempo superiores a 60 minutos.)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Evite usar [palavras reservadas](/docs/insights/insights-data-sources/custom-data/insights-custom-data-requirements-limits#reserved-words), como `accountId`, `appId` e `eventType`. Você também deve evitar o uso de termos de sintaxe NRQL, a menos que você os coloque com acento grave (``` `` ```).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Exception:**</DoNotTranslate> Você pode usar `name` como chave de atributo.
      </td>
    </tr>
  </tbody>
</table>
