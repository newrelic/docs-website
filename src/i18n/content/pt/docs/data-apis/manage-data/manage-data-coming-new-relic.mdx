...
## UI de ingestão de dados [#ui]

A guia <DNT>**Data ingestion**</DNT> está localizada na [UI de gerenciamento de dados](/docs/data-apis/manage-data/manage-your-data). Ele mostra um resumo do seu uso de dados por fonte, ajudando você a entender quais dados estão sendo enviados para o New Relic.
...
A UI de ingestão de dados fornece ferramentas para ajudar você a monitorar e analisar seus dados:

<DNT>**Filters and controls:**</DNT>

* <DNT>**Time range selector:**</DNT> Escolha entre as visualizações **Auto**, **Daily**, **Weekly** ou **Monthly** para analisar as tendências de ingestão de dados durante o período de sua preferência.

<Callout variant="important">
  A visualização <DNT>**Auto**</DNT> não suporta o seletor de hora. Como mostra seus dados em intervalos de 5 minutos, pode exibir até um dia por vez. Para ver um intervalo de tempo maior, mude para as visualizações <DNT>**Daily**</DNT>, <DNT>**Weekly**</DNT> ou <DNT>**Monthly**</DNT>.
</Callout>

* <DNT>**Data source filtering:**</DNT> Use o menu suspenso e a funcionalidade de pesquisa para filtrar e encontrar fontes de dados específicas.

<DNT>**Visualizations:**</DNT>

* <DNT>**Data source table:**</DNT> Veja todas as fontes de dados ativas com seus respectivos volumes de ingestão. A tabela mostra a contribuição de cada fonte para a sua ingestão total de dados (como **BrowserEventsBytes**, **CustomEventsBytes**, **ApmEventsBytes**).
* <DNT>**Visual trend chart:**</DNT> Exibe padrões de ingestão ao longo do tempo com faixas codificadas por cores representando diferentes fontes de dados, facilitando a identificação de picos ou tendências.
* <DNT>**Usage by account:**</DNT> Um gráfico de pizza mostrando a distribuição da ingestão de dados entre suas contas, exibindo tanto o volume total quanto o detalhamento percentual.
* <DNT>**Usage by data source:**</DNT> Um gráfico de pizza detalhando sua ingestão total por fonte de dados (por exemplo, **BrowserEventsBytes** pode mostrar 95,17% da sua ingestão total, **CustomEventsBytes** a 3,33%).

Para definições detalhadas das métricas exibidas, consulte [Fontes de ingestão de dados](#sources-list).

<Callout variant="important">
  **Filtragem de contas multi-tenant**

  Para organizações com contas multi-tenant, você pode filtrar a ingestão de dados por organização ou conta usando os menus suspensos **Select Organization** e **Select Account**. Isso permite que você visualize dados de todas as organizações e contas, ou restrinja a opções específicas.
</Callout>

### Obtenha mais detalhes [#detail]

Para visualizar detalhes adicionais sobre uma fonte de dados específica:

1. Clique no nome da fonte de dados na tabela
2. Use as opções adicionais de filtragem que aparecem para detalhar ainda mais essa fonte de dados específica.

<DNT>**Advanced options:**</DNT>

* Clique em <DNT>**View NRQL**</DNT> para ver a consulta subjacente de qualquer gráfico.
* Veja [Configurações da conta](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings/#video-tour) para um tour em vídeo desta e de outras UIs de administração
* Para detalhes técnicos sobre como os gráficos exibem dados e mensagens de erro, consulte [Detalhes do gráfico](#technical-details-chart)

## Estimar a ingestão futura de dados [#estimate]

A página <DNT>**Data Ingestion**</DNT> inclui recursos preditivos que ajudam a prever o uso futuro de dados da sua organização.
...
3. Visualize a previsão de uso de dados para 1 semana tanto no gráfico quanto na tabela.

Para remover a previsão, selecione <DNT>**Remove prediction**</DNT> no menu do gráfico.

<Callout variant="important">
  A previsão está disponível apenas para clientes <DNT>**Advanced Compute**</DNT> e apenas no <DNT>**Daily view**</DNT> do <DNT>**Data source time series chart**</DNT>.
</Callout>
