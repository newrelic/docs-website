...
A guia <DoNotTranslate>**Data ingestion**</DoNotTranslate> está localizada na [interface de gerenciamento de dados](/docs/data-apis/manage-data/manage-your-data). A interface de ingestão de dados mostra um resumo do seu uso por fonte de dados. Para organizações com múltiplas contas, você também pode visualizar o uso de contas específicas.
...
<figcaption>
  No [menu do usuário](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), selecione <DoNotTranslate>**Manage your data**</DoNotTranslate>.
</figcaption>

Para obter definições detalhadas da métrica mostrada no gráfico, consulte [Fontes de ingestão de dados](#sources-list).

Para ver a consulta NRQL subjacente usada para criar esse gráfico, clique em <DoNotTranslate>**View NRQL**</DoNotTranslate>.
...
<CollapserGroup>
  ...
  >
    ...
    * [Defina pontuações Apdex apropriadas](/docs/apm/new-relic-apm/apdex/change-your-apdex-settings/), por exemplo, para frequência de rastreamento.
  ...
  >
    ...
    * [Login automático no contexto](/docs/logs/logs-context/disable-automatic-logging): desative ou ative por meio da interface ou API ou ajuste as configurações de configuração do lado do cliente.
...
<CollapserGroup>
  <Collapser
    ...
    title="Métrica"
  >
    No gráfico de ingestão de dados, <DoNotTranslate>**Metrics**</DoNotTranslate> é uma combinação de dois tipos de métrica: [dados métrica de frações de tempo](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data) e [métrica dimensional](/docs/data-apis/understand-data/new-relic-data-types/#dimensional-metrics).
  ...
  >
    A atividade de Monitoramento de Alterações é armazenada no namespace <DoNotTranslate>**Deployment**</DoNotTranslate> . As alterações individuais que você rastreia têm em média 200 a 250 bytes em sua ingestão.
  </Collapser>
  ...
  <Collapser
    ...
    title="Segurança"
  >
    Os eventos reportados pelo recurso de segurança do New Relic são armazenados no namespace <DoNotTranslate>**Security**</DoNotTranslate> . Estes são principalmente relatórios de vulnerabilidades fornecidos pelo recurso Gerenciamento de vulnerabilidades, mas podem ser expandidos para incluir produtos adicionais no futuro.

    Os volumes esperados de eventos deste tipo são muito baixos, pois a ocorrência de relatos de vulnerabilidades é rara.

    Os recursos de segurança ainda em visualização pública usam um namespace <DoNotTranslate>**Security:Preview**</DoNotTranslate> separado e não são faturáveis.
