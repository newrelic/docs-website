...
<figcaption>
  A interface <DoNotTranslate>**Data retention**</DoNotTranslate> é onde você pode ver os períodos de retenção de dados e editar a retenção de dados. Para visualizar explicações sobre os cabeçalhos das colunas da tabela, passe o mouse sobre um cabeçalho.
</figcaption>

## Requisitos [#requirements]

Ser capaz de configurar a retenção de dados requer Pro ou edição Enterprise com direitos de administrador de cobrança. As características da conta que não atendem a esses critérios impedirão que algumas organizações da New Relic ajustem a retenção por conta própria. Se você for uma organização Pro ou Enterprise com uma conta de administrador de cobrança e não puder editar a retenção, fale com seu representante de conta da New Relic.

Observe que nossa [opção Data Plus](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-plus) oferece até 90 dias adicionais de retenção.

Requisitos relacionados à permissão para visualização e edição de retenção:

* [Modelo de usuário mais recente](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models):

  * Definir um contrato com retenção de dados mais longa para toda a organização requer a [configuração de administração](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings)
  ...
  * Ajustar a retenção dentro dos limites de um contrato para tipos específicos de dados ou para uma conta específica requer a [permissão](/docs/accounts/accounts-billing/new-relic-one-user-management/user-permissions/#data-retention)
...
* [Modelo de usuário original](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model): requer a função de Proprietário ou função Add-On do Gerenciador de retenção de dados.

## A interface de retenção de dados [#find-ui]

A página da interface <DoNotTranslate>**Data retention**</DoNotTranslate> está localizada no [hub de gerenciamento de dados](https://one.newrelic.com/data-management-hub).

## Períodos de retenção padrão [#retention-periods]

A melhor maneira de entender a retenção de dados do New Relic é acessar a página da interface <DoNotTranslate>**Data retention**</DoNotTranslate> . Recomendamos isso porque sua organização pode ter editado suas configurações e, portanto, elas podem não ser as configurações padrão mostradas abaixo.
...
<CollapserGroup>
  ...
  <Collapser
    id="metric-timeslice"
    title="Métrica de fração de tempo de dados (APM, navegador, celular)"
  >
    [Os dados de métrica de fração de tempo](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data) são gerados por <InlinePopover type="apm"/>(ambos métrica personalizada e métrica padrão), <InlinePopover type="mobile"/>e agente navegador. Uma métrica de fração de tempo consiste em três partes: um nome da métrica, o segmento de tempo que a métrica representa (a "fatia de tempo") e um valor numérico (a medida). APM, <InlinePopover type="browser"/>e agente móvel podem coletar milhares de intervalos de tempo métricos por minuto para uma variedade de métricas de desempenho, como taxas de erros, uso de largura de banda e tempo de coleta de lixo.
      ...
      <table>
        <tbody>
          ...
          <tr>
            ...
            <td>
              * [Apdex](/docs/site/apdex-measuring-user-satisfaction), para navegador
              * Tempo de carregamento da página do navegador
              * Taxas de transferência total. Após 90 dias, não há detalhamento por tipo de navegador e apenas o valor médio combinado fica disponível para o intervalo de tempo solicitado.
            </td>
          </tr>
  ...
  <Collapser
    ...
    title="Namespace do navegador"
  >
    Todos os namespaces do navegador têm o mesmo período de retenção padrão. Aqui estão os detalhes sobre o evento em cada namespace do navegador:
    ...
    * `Browser page view timing` namespace: `PageViewTiming`
    ...
    Para mais informações sobre esses eventos, consulte [o evento do navegador no dicionário de dados](/attribute-dictionary/?dataSource=Browser+agent).
  </Collapser>
