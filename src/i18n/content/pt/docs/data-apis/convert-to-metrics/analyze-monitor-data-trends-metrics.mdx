...
* <DoNotTranslate>**Events, logs, spans:**</DoNotTranslate>

  Esses tipos de dados representam um único registro em um momento específico. Por exemplo, você pode ter um evento para cada solicitação ao sistema. Esses dados são ideais para resolução de problemas e análises aprofundadas.

* <DoNotTranslate>**Metrics:**</DoNotTranslate>

  Eles fornecem uma visão agregada do seu evento, log ou períodos. métricas são melhores para mostrar tendências em intervalos de tempo mais longos. Por exemplo, você pode agregar o número total de solicitações por serviço a uma métrica e depois examinar essas informações mês após mês.
...
* <DoNotTranslate>**Mutations**</DoNotTranslate>

  , que são operações que fazem alterações em regras ou configurações existentes (por exemplo, criando uma nova regra métrica).

* <DoNotTranslate>**Queries**</DoNotTranslate>

  , para buscar dados existentes (por exemplo, buscar regras métricas existentes).
...
<CollapserGroup>
  ...
  <Collapser
    id="enable-disable"
    title="Habilitar ou desabilitar uma regra"
  >
    <Callout variant="important">
      Esta operação modifica as configurações de produção, por isso recomendamos revisar minuciosamente suas alterações antes de executar a operação.
    </Callout>
...
<CollapserGroup>
  <Collapser
    id="list-all-rules"
    title="Liste todas as regras para uma conta New Relic"
  >
  ...
  <Collapser
    ...
    title="Listar regra por ID de regra"
  >
...
2. Vá para
   ...
   <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**</DoNotTranslate>

   e cole sua consulta na caixa.

3. Para executar a operação, clique no botão vermelho

   <DoNotTranslate>**Execute query**</DoNotTranslate>

   . Ou, para obter o formato curl, selecione

   <DoNotTranslate>**Tools > Copy as CURL**</DoNotTranslate>
