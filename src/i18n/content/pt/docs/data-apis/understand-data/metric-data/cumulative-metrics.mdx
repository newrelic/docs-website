...
Ao coletar dados métricos de uma aplicação, é importante considerar *como* esses dados foram medidos ao decidir como usá-los e interpretá-los no momento da consulta. O [tipo de métrica](/docs/data-apis/understand-data/metric-data/metric-data-type/#metric-types) é um fator importante, com certas funções de agregação do New Relic funcionando com alguns tipos e não com outros. Mas outro fator importante é o <DNT>**temporality**</DNT> da métrica.
...
Ao converter uma soma em delta, New Relic também emitirá o valor cumulativo junto com o delta para manter a capacidade de você consultar o valor cumulativo mais recente. Para acessar o valor acumulado, você pode usar [`getField()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#func-getfield) (veja [Como consultar métrica](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-metrics/#query) para exemplos).
