...
<Callout variant="important">
  Se você estava relatando métricas cumulativas do OpenTelemetry para New Relic antes de gaDateAndYear, a forma como lidamos com essas métricas está mudando. Para obter detalhes, consulte o [guia de transição de métricas cumulativas](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/cumulative-metrics-transition-guide).
</Callout>
...
Como a conversão delta é uma operação com estado, devemos estar cientes das séries temporais que podem parar de relatar e, eventualmente, abandonar seu estado. Se uma série temporal não relatou nenhum novo ponto de dados para <DoNotTranslate>**5 minutes**</DoNotTranslate>, liberaremos o estado que temos, incluindo deltas computacionais em quaisquer lacunas armazenadas em buffer. Isso significa que se um ponto de dados chegar em um momento posterior, ele será tratado como se fosse o início daquela série temporal, perdendo efetivamente o delta entre o último ponto de dados antes da liberação e o primeiro ponto de dados após a liberação. Isso significa que os intervalos de relatórios métricos devem ser menores que <DoNotTranslate>**5 minutes**</DoNotTranslate> para obter o benefício da conversão delta.
