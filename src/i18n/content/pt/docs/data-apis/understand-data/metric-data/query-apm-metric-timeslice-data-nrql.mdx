...
A New Relic reporta [métricas de diversas maneiras](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics). Uma variedade de dados métricos que chamamos de <DoNotTranslate>**metric timeslice data**</DoNotTranslate>; este é o tipo de dados usado para gerar muitos dos gráficos em APM, <InlinePopover type="mobile"/>e navegador (para mais detalhes, veja [métrica de fração de tempo data](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data)).
...
## Consulta genérica com a métrica `newrelic.timeslice.value` [#generic-queries]
...
<CollapserGroup>
  <Collapser
    ...
    title="Facetar por vários nomes de aplicativos"
  >
  ...
  <Collapser
    id="rate"
    title="Taxas de transferência por minuto"
  >
  ...
  <Collapser
    id="facet-by-host"
    title="Faceta por atributo relacionado ao host"
  >
  ...
  <Collapser
    id="average"
    title="Consulta de duração média de uma métrica"
  >
  ...
  <Collapser
    id="average"
    title="Divida o tempo médio gasto em uma transação por seus segmentos componentes"
  >
