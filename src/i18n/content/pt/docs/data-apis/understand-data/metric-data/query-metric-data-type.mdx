...
Existem [vários tipos de dados `Metric` ](/docs/data-apis/understand-data/metric-data/metric-data-type/#metric-types)(por exemplo, `gauge` e `count`) e cada tipo tem vários <DoNotTranslate>**fields**</DoNotTranslate> associados. Para obter detalhes sobre os tipos de campos disponíveis, consulte [`getField()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#func-getfield).

Você pode usar `getField()` para extrair esses campos. Por exemplo, se quiser usar um único valor dentro de uma métrica para fazer uma comparação em uma cláusula `WHERE` , você pode usar `getField(metricName, field)` ou `metricName[field]`.
