...
Estes são os serviços de endpoint da New Relic disponíveis por meio do AWS PrivateLink. Os endpoints dos EUA (padrão) são divididos em três zonas de disponibilidade da AWS (us-east-1, us-east-2, us-west-2), mostradas como guias separadas abaixo:
...
### Endpoints PrivateLink FedRAMP [#fedramp]

Para obter informações sobre endpoints PrivateLink em conformidade com FedRAMP, consulte [Conectividade do AWS PrivateLink](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints#privatelink) em nosso guia do FedRAMP.
