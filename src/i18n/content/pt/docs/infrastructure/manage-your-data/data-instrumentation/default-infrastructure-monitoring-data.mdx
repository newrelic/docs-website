...
Se você estiver usando integração, consulte o documento dessa integração para obter mais informações sobre os dados relatados. Para obter atributos comuns AWS , consulte [DadosAWS ](#shared-attributes).
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        `SystemSample` Contém dados que descrevem o estado geral atual de todo o servidor, incluindo CPU, memória, disco e rede. Tiramos um snapshot desses dados a cada 5 segundos e os empacotamos em um evento `SystemSample` , que é então enviado para a New Relic. Esses dados aparecem na [guia de interface<DoNotTranslate>**Systems**</DoNotTranslate> ](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#system).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `ProcessSample` reúne informações detalhadas sobre o uso de recursos de programas executados em um único sistema. Tiramos um snapshot desses dados a cada 20 segundos para cada processo ativo e os empacotamos em um evento `ProcessSample` , que é então enviado para a New Relic. Esses dados aparecem na [página da interface do usuário<DoNotTranslate>**Processes**</DoNotTranslate> ](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#processes).
    ...
    <tr>
      ...
      <td>
        `StorageSample` representa um único dispositivo de armazenamento associado a um servidor. Cada amostra reúne informações descritivas sobre o dispositivo, o tipo de sistema de arquivos que ele usa e seu uso e capacidade atuais. Tiramos um snapshot desses dados a cada 20 segundos para cada sistema de arquivos montado e os empacotamos em um evento `StorageSample` , que é então enviado ao New Relic. Esses dados aparecem na [página da interface do usuário<DoNotTranslate>**Storage**</DoNotTranslate> ](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#storage).
        ...
        <CollapserGroup>
          <Collapser
            id="linux-supported"
            title="Sistemas de armazenamento Linux suportados"
          >
          ...
          <Collapser
            ...
            title="Sistemas de armazenamento Windows suportados"
          >
    ...
    <tr>
      ...
      <td>
        `ContainerSample` coleta as informações descritivas e de estado de cada contêiner Docker . Inclui ID do contêiner, nome, imagem, nome da imagem, além de métricas sobre CPU, memória e rede. Tiramos um snapshot desses dados a cada 15 segundos para cada contêiner e os empacotamos em um evento `ContainerSample` , que é então enviado para a New Relic. Esses dados aparecem na [página de interface<DoNotTranslate>**Containers**</DoNotTranslate> ](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#containers). Para obter mais informações, consulte [Docker monitoramento](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/docker-instrumentation-infrastructure).
      </td>
    </tr>
...
Se uma integração AWS estiver habilitada, seu evento de infraestrutura também poderá ter [um atributoAWS ](#shared-attributes)anexado.
...
<CollapserGroup>
  ...
  <Collapser
    id="ec2-general"
    title="Outros atributos Amazon EC2"
  >
