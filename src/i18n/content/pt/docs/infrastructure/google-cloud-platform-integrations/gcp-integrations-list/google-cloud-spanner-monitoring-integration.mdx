...
<CollapserGroup>
  <Collapser id="entities-table" title="Entidades do Spanner">
    Uma única entidade `GCPSPANNERINSTANCE` é sintetizada para cada instância do Spanner. Métricas emitidas para ambos os tipos de recursos monitorados do GCP `spanner_instance` e `spanner.googleapis.com/Instance` são anexadas a essa entidade, facetadas por `database` quando aplicável.
