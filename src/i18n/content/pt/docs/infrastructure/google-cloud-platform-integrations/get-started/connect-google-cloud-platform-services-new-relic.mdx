...
<CollapserGroup>
  <Collapser id="service" title="Conta de serviço (recomendado)">
    ...
    Se a sua organização usa uma [restrição de domínio](https://cloud.google.com/resource-manager/docs/organization-policy/restricting-domains), você terá que atualizar a política para permitir a New Relic, dependendo de qual restrição sua organização aplica:

    * **`iam.allowedPolicyMemberDomains`**: permita o ID do domínio da New Relic `C02x1gp26`.
    * **`iam.managed.allowedPolicyMembers`**: permita o ID da organização GCP do New Relic `740422149647` ou o número do projeto GCP `397727728820`.
