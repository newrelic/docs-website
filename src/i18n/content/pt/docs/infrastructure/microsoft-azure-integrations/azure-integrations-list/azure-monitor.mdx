...
<Callout variant="important">
  ...
  Observe que a métrica ainda pode ser diferenciada facetando-se usando o campo `collector.name`. Observe também que os limites de taxa aplicados pelo Azure serão compartilhados entre qualquer integração de pesquisa atual e o Azure Monitor.
</Callout>