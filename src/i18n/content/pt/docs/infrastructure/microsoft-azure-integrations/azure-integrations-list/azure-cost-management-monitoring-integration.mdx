...
<Callout title="Observação">
  Os dados do Azure Cost Management são coletados apenas por meio de nossa integração tradicional (polling) do Azure. Esta integração **não** está disponível por meio da [integração do Azure Monitor](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/). Os dados de custo são baseados em eventos e abrangem vários recursos e serviços, portanto, não são mapeados para um único tipo de recurso do Azure da maneira que a integração do Azure Monitor exige.

  Em caso de migração da integração tradicional do Azure para a integração do Azure Monitor, a integração tradicional do Azure deve ser mantida habilitada para continuar a coleta de dados do Azure Cost Management.
</Callout>