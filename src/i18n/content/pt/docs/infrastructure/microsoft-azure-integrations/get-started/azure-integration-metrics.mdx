---
title: Azure integração métrica
tags:
  - Integrations
...
## Convenção de nomenclatura métrica dimensional [#metric-naming-convention]

As métricas obtidas da Azure Monitor API do são armazenadas no New Relic como [métricas dimensionais](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) seguindo esta convenção:

* Métrica são prefixadas pelo tipo de recurso do Azure, todas em letras minúsculas, onde `/` é substituído por `.` e `Microsoft.` é substituído por `azure.`:
...
* O nome da métrica original do Azure mantém o seu invólucro original, despojado de todos os espaços em branco, e é prefixado com o tipo de recurso, conforme descrito acima:
...
Se o nome da métrica resultante não começar com `azure.` após essas transformações, o prefixo será acrescentado adicionalmente.

Para obter mais informações sobre os tipos de recursos do Azure suportados, consulte o [site de documentação métricaAzure Monitor ](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/supported-metrics/metrics-index).

### O nome da métrica corresponde ao nome de uma das dimensões

Quando o nome da métrica e o nome de uma das dimensões são exatamente iguais, o nome desta dimensão não é maiúsculo, portanto ambos os valores são diferentes:

* `azure.network.applicationgateways.TlsProtocol` com dimensões `azure.network.applicationgateways.tlsprotocol`

### Métrica com muitas combinações de dimensões

Uma definição de métrica inclui seu nome e as diferentes dimensões. Às vezes, o mesmo nome de métrica aparece em muitas métricas com diferentes combinações de dimensões, por exemplo:

* `azure.cache.redis.percentProcessorTime` com dimensões `azure.cache.redis.ShardId`
* `azure.cache.redis.percentProcessorTime` sem dimensões.

Cada uma dessas métricas é ingerida como uma série temporal individual.

<Callout variant="tip">
  Algumas agregações como `sum()` ou `count()` retornarão resultados duplicados se a combinação correta de dimensões não for especificada.
</Callout>

Para evitar que algumas agregações como `sum()` ou `count()` correspondam às duas métricas ao mesmo tempo, use `IS NULL` ou `IS NOT NULL` para filtrar as dimensões. Por exemplo:

Corresponde às duas métricas ao mesmo tempo:

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC
```

Corresponde apenas à métrica sem a dimensão ShardId:

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NULL
```

Corresponde apenas à métrica com a dimensão:

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NOT NULL
```

## Métrica de pesquisa API [#azure-metrics-table]

Para uma referência sobre as métricas disponíveis de cada uma das integrações de votação e seus nomes, [consulte nosso documento sobre a integração individual](/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).

A tabela a seguir é uma lista não abrangente das métricas coletadas pela integração do Azure Polling e suas traduções [de métricas dimensionais](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) .
  ...
  <tbody>
    <tr>
      ...
      <th style={{ width: "275px" }}>
        Nome da métrica dimensional (novo)
      </th>

      <th style={{ width: "275px" }}>
        Nome da métrica de amostra (anterior)
      </th>
    </tr>
    ...
    <tr>
      <td>
        Banco de dados Azure para MariaDB
      </td>
    ...
    <tr>
      <td>
        Conjuntos de escala de VM do Azure
      </td>
