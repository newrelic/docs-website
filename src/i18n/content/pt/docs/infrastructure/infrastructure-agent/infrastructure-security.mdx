...
Em sistemas Windows, o agente possui os seguintes requisitos de instalação e conta:

* A instalação requer uma conta de administrador.
* Por padrão, o agente é executado como um serviço com o contexto de logon do Sistema Local, o que significa que não tem acesso a outros serviços na rede que exigem autenticação.
* Como alternativa ao Local System, o serviço poderia opcionalmente ser alterado para um usuário local, um usuário do Active Directory/Microsoft Entra ID ou uma Group Managed Service Account/gMSA. O script `set-service-account.ps1` incluído ajuda na transição e define os privilégios necessários.
