...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="logging-http-client-timeout" title="logging_http_client_timeout">
    Define o tempo limite do cliente HTTP (em segundos) para o plugin de saída fluent-bit do New Relic. Isso controla por quanto tempo o encaminhador de logging aguardará uma resposta ao enviar dados de log para o New Relic.

    Para obter mais informações, consulte a [documentação do plugin de saída fluent-bit](https://github.com/newrelic/newrelic-fluent-bit-output#httpclienttimeout).
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="safe-bin-dir" title="safe_bin_dir">
    Diretório onde o agente procura executáveis para integrações.
  ...
  <Collapser className="freq-link" id="agent-temp-dir" title="agent_temp_dir">
    Diretório onde o agente armazena arquivos temporários (por exemplo, configuração do encaminhador de logs e dados de descoberta de integração). Esse diretório é excluído a cada reinicialização do agente, mas apenas se ainda corresponder ao seu valor padrão.
  ...
  <Collapser className="freq-link" id="logging-configs-dir" title="logging_configs_dir">
    Diretório contendo arquivos de configuração para o encaminhador de logs.
  ...
  <Collapser className="freq-link" id="logging-home-dir" title="logging_home_dir">
    Diretório contendo binários e outros arquivos necessários para o encaminhador de logs.
  ...
  <Collapser className="freq-link" id="logging-bin-dir" title="logging_bin_dir">
    Diretório contendo binários para o encaminhador de logs.
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="http-server-cert" title="http_server_cert">
    O caminho para um certificado codificado em PEM usado para escutar payloads de integração via HTTPS. Quando configurado com `http_server_key`, o agente usa HTTPS para o servidor de payload de integração.
  ...
  <Collapser className="freq-link" id="http-server-key" title="http_server_key">
    O caminho para uma chave codificada em PEM usada para escutar payloads de integração via HTTPS. Quando configurado com `http_server_cert`, o agente usa HTTPS para o servidor de payload de integração.
  ...
  <Collapser className="freq-link" id="http-server-ca" title="http_server_ca">
    O caminho para um certificado CA codificado em PEM usado para impor a validação do certificado do cliente para solicitações HTTPS. Quando configurado, o agente verifica os certificados do cliente em relação a esta CA para habilitar a autenticação mútua TLS (mTLS) para o servidor de payload de integração.
