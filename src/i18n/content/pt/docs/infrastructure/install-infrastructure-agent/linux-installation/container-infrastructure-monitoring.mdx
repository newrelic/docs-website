...
<DNT>**Recommendation:**</DNT> Estenda a [imagem`newrelic/infrastructure` ](https://hub.docker.com/r/newrelic/infrastructure/)e use seu próprio [arquivo de configuração](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) do agente `newrelic-infra.yml` . Depois que sua imagem for construída, você poderá facilmente criar um contêiner sem precisar fornecer mais configuração de tempo de lançamento. Não forneça segredos usando variáveis de ambiente com Docker.
