...
1. Adicione seu <InlinePopover type="licenseKey"/>ao atributo `license_key` em seu arquivo `newrelic-infra.yml` , localizado em `C:\Program Files\New Relic\newrelic-infra\`. Quando terminar, seu arquivo `newrelic-infra.yml` deverá ter a seguinte aparência:

   ```yml
   license_key: YOUR_LICENSE_KEY
   ```

2. Para iniciar o serviço `newrelic-infra` , execute:
