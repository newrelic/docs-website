...
<CollapserGroup>
  <Collapser
    id="config-file"
    title={<>Localização e descrição do arquivo de configuração (<InlineCode>newrelic-infra.yml</InlineCode>)</>}
  >
  ...
  >
    ...
    * <DoNotTranslate>**From inside the config file:**</DoNotTranslate> Adicione uma entrada ao [arquivo de configuração da infraestrutura](https://github.com/newrelic/infrastructure-agent/blob/master/assets/examples/infrastructure/newrelic-infra-template.yml.example) para permitir que o agente obtenha as variáveis de ambiente que você definiu.
    ...
    * <DoNotTranslate>**From the command line:**</DoNotTranslate> Use outra variável de ambiente para passar por essas configurações.
...
## Estrutura do arquivo de configuração [#config_structure]
