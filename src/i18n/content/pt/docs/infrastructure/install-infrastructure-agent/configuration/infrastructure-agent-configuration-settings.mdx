...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      <DNT>**Recommendation:**</DNT> Não mude esta configuração.
    </Callout>
...
<CollapserGroup>
  ...
  >
    Use esta configuração se você instrumentou o host de uma fonte diferente do agente de infraestrutura (por exemplo, um coletor OpenTelemetry ou um exportador de nó Prometheus) e deseja continuar usando o agente de integração de infraestrutura no host para monitor outros serviços de infraestrutura. Quando habilitado, o agente reporta inventário de host e integração telemétrica (evento métrico e inventário) decorado com metadados de host, mas métricas de host (CPU, memória, disco, rede, processos) ficam desabilitadas.
...
<CollapserGroup>
  ...
  >
    ...
    Utilize uma lista de atributos personalizados para anotar os dados desta instância do agente. Separe chaves e valores com dois pontos `:`, como em `KEY: VALUE`, e separe cada par de valores principais com uma quebra de linha. As chaves podem ser qualquer YAML válido, exceto barras `/`. Os valores podem ser qualquer string YAML, incluindo espaços.
  ...
  >
    ...
    O envio de todos os dados do processo pode aumentar o volume de dados enviados para a New Relic. Para ajustar quais processos você deseja monitor, configure [`include_matching_metrics`](#include-matching-metrics).
...
<CollapserGroup>
  ...
  >
    ...
    Retorna informações sobre a entidade agente/host. Um código de status de resposta _204_ ("Sem Conteúdo") será retornado quando o agente ainda não tiver informações sobre a entidade agente/host. Portanto, podem ser necessárias várias solicitações até que o agente forneça os dados da entidade.
