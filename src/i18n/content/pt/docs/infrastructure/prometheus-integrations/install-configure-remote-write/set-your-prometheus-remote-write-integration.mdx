...
Vá para o [inicializador de configuração de gravação remota do Prometheus na interface](https://one.newrelic.com/launcher/nr1-core.settings?pane=eyJuZXJkbGV0SWQiOiJwcm9tZXRoZXVzLXJlbW90ZS13cml0ZS1pbnRlZ3JhdGlvbi1uZXJkbGV0cy5zZXR1cC1wcm9tZXRoZXVzIn0=) e conclua estas etapas para adicionar dados do Prometheus.
...
1. Insira um nome para o servidor Prometheus a ser conectado e seu URL `remote_write` . <DoNotTranslate>**Important:**</DoNotTranslate> O nome inserido para o servidor criará um atributo em seus dados. Também será o nome que identificará qual servidor Prometheus está enviando dados para o New Relic.
...
2. Adicione um novo URL `remote_write` ao arquivo YML do Prometheus. Adicione essas informações em `global_config` no arquivo, no mesmo nível de recuo da seção `global` .
   ...
   <DoNotTranslate>**European Union accounts:**</DoNotTranslate> Se você estiver se conectando na UE, use o seguinte URL:

   ```
   https://metric-api.eu.newrelic.com/prometheus/v1/write
   ```

   <DoNotTranslate>**Kubernetes and Helm remote write integrations:**</DoNotTranslate> Adicione o URL de gravação remota ao arquivo Helm [`values.yaml`](https://github.com/helm/charts/blob/c40486ab2eba0391119b7cc1509d6958fd21c31d/stable/prometheus/values.yaml#L631) . Substitua `remoteWrite: []` por duas linhas semelhantes ao exemplo a seguir. Certifique-se de usar seu URL de gravação remota e usar um recuo que corresponda ao restante do arquivo:
...
<DoNotTranslate>**Example:**</DoNotTranslate> Você tem uma métrica de contador chamada `my_counter`, que não possui nosso sufixo de convenção de nomenclatura `_bucket`, `_count` ou `_total`. Nessa situação, sua métrica seria identificada como um medidor e não como um contador. Para corrigir isso, adicione a seguinte configuração de reetiquetagem ao seu `prometheus.yml`:
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title={<><InlineCode>prometheus_server</InlineCode> Parâmetro URL</>}
  >
