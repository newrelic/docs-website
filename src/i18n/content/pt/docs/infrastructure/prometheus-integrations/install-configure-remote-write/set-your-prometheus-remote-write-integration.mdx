...
1. Insira um nome para o servidor Prometheus a ser conectado e seu URL `remote_write` . <DNT>**Important:**</DNT> O nome inserido para o servidor criará um atributo em seus dados. Também será o nome que identificará qual servidor Prometheus está enviando dados para o New Relic.
...
2. Adicione um novo URL `remote_write` ao arquivo YML do Prometheus. Adicione essas informações em `global_config` no arquivo, no mesmo nível de recuo da seção `global` .
   ...
   <DNT>**European Union accounts:**</DNT> Se você estiver se conectando na UE, use o seguinte URL:

   ```
   https://metric-api.eu.newrelic.com/prometheus/v1/write
   ```

   <DNT>**Kubernetes and Helm remote write integrations:**</DNT> Adicione o URL de gravação remota ao arquivo Helm [`values.yaml`](https://github.com/helm/charts/blob/c40486ab2eba0391119b7cc1509d6958fd21c31d/stable/prometheus/values.yaml#L631) . Substitua `remoteWrite: []` por duas linhas semelhantes ao exemplo a seguir. Certifique-se de usar seu URL de gravação remota e usar um recuo que corresponda ao restante do arquivo:
