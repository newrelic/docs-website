...
Você pode instalar nossa [integraçãoKubernetes ](/docs/kubernetes-pixie/kubernetes-integration/get-started/introduction-kubernetes-integration)para obter uma observabilidade completa do seu cluster do Kubernetes, que inclui o agente Prometheus.

1. Instale nossa integração Kubernetes. Veja como instalá-lo [aqui](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure).

<Callout variant="tip">
  Como alternativa, também oferecemos instruções manuais para [implantar nossa integração usando Helm](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm).

  Você precisa ativar o agente Prometheus definindo a opção `newrelic-prometheus-agent.enabled=true` .
</Callout>

2. Para garantir que a integração foi configurada corretamente, acesse <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Query your data**</DoNotTranslate> e execute esta consulta NRQL para ver se os dados foram relatados:
...
1. Instale o [dashboard do agente Prometheus](https://newrelic.com/instant-observability/prometheus-agent) em sua conta New Relic .
