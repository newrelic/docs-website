---
title: Recursos PromQL compatíveis
tags:
  - Integrations
...
New Relic suporta consulta estilo PromQL, e nosso criador de consulta oferece um modo de consulta estilo PromQL que traduz consulta com sintaxe PromQL para a aproximação NRQL mais próxima. Embora o método de aproximação signifique que alguns casos extremos não sejam totalmente suportados, ele fornece cobertura para uma esmagadora maioria das consultas, suportando mais de 99,5% das consultas nos 7,8 milhões de principais downloads de dashboard do Grafana.

Saiba como trabalhamos com consulta PromQL, bem como as diferenças entre o PromQL padrão e nossa linguagem de consulta semelhante ao PromQL.
...
<CollapserGroup>
  ...
  <Collapser title="Funções de ordenação">
  ...
  <Collapser title="Funções utilitárias">
    * [`label_join()`](https://prometheus.io/docs/prometheus/latest/querying/functions/#label_join)
  </Collapser>

  <Collapser title="Recursos específicos do Grafana">
    A lógica de tradução também tem algum suporte limitado para a sintaxe do Grafana que não faz parte da linguagem promQL.
...
No entanto, como o NRDB converte contadores acumulativos estilo PromQL em contadores `delta`, nossa implementação não trata essas funções no tipo de dados incorreto e produzirá respostas diferentes ou incorretas.
...
* Para garantir a estabilidade e o desempenho do nosso sistema para todos os usuários, colocamos alguns limites sobre quais consultas podem ser executadas. Em todos os casos, aplicamos um limite de 366 etapas em consultas de intervalo. Também retornamos apenas 100 séries temporais de consultas por padrão.
