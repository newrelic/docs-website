...
Salvo indicação em contrário, as opções de configuração para a integração do Prometheus OpenMetrics com New Relic se aplicam aos ambientes Docker e Kubernetes . No mínimo, os seguintes valores de configuração são <DoNotTranslate>**required**</DoNotTranslate>:

* <InlinePopover type="licenseKey"/>
* [Nome do cluster](#definitions-configuration-file)

<DoNotTranslate>**Recommendation:**</DoNotTranslate> Configure sua chave de licença do New Relic como uma variável de ambiente chamada `LICENSE_KEY`. Isso fornece um ambiente mais seguro, pois o New Relic pode carregar sua variável de ambiente a partir de um [segredo de autenticação TLS mútuo](/docs/integrations/prometheus-integrations/install-configure/add-mutual-tls-prometheus-endpoints).
...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}>Porta do Kubernetes e caminho do endpoint</img></>}
  >
  ...
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}>Exemplo: rótulos para porta e caminho do Kubernetes</img></>}
  >
...
A integração do Prometheus OpenMetrics <DoNotTranslate>**does not**</DoNotTranslate> recarrega automaticamente a configuração quando você faz alterações no arquivo de configuração.
...
Para recarregar a configuração, reinicie a integração. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Reduza a implantação para zero réplicas e depois reduza para uma réplica:
