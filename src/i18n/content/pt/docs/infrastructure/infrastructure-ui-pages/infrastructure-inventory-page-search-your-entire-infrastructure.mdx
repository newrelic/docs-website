...
A New Relic pode coletar informações detalhadas sobre a configuração de um sistema por host, incluindo módulos do sistema, arquivos de configuração, metadados, pacotes, serviços, sessões de usuário e muito mais. A página <DoNotTranslate>**Inventory**</DoNotTranslate> fornece uma visualização em tempo real, filtrável e pesquisável da configuração de cada host.

Use a página <DoNotTranslate>**Inventory**</DoNotTranslate> para:
...
Para visualizar e pesquisar seus dados de inventário: Vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Inventory**</DoNotTranslate>.
...
Use <DoNotTranslate>**Inventory**</DoNotTranslate> funções de página para encontrar informações sobre um item específico em seus hosts:

<CollapserGroup>
  <Collapser
    id="search"
    title="Pesquisar inventário"
  >
    Pesquise um item de inventário usando a função de pesquisa.

    Por exemplo, se você quiser encontrar informações relacionadas ao OpenSSL, pesquise `openssl`. O termo de pesquisa corresponde novamente ao nome do item de inventário.
  </Collapser>

  <Collapser
    id="view"
    title="Ver detalhes do item de inventário"
  >
    Os detalhes do item de inventário fornecem informações de host e sistema para cada host em que ele reside, de acordo com o [nome do item de inventário](#item-naming) da New Relic. Se você tiver versões diferentes do mesmo item em outros hosts, o New Relic detectará isso e os sinalizará na página Inventário com o rótulo <DoNotTranslate>**variant hosts**</DoNotTranslate> e listará cada host executando cada versão.
  ...
  <Collapser
    id="alerts"
    title="Ver o alerta limite incidente do anfitrião"
  >
    Para visualizar um ou mais [alertas de incidentes limitados](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-threshold) do anfitrião, selecione o <DoNotTranslate>**Critical**</DoNotTranslate> do anfitrião <Icon name="fe-x-circle"/> ícone ou <DoNotTranslate>**Warning**</DoNotTranslate> <Icon name="fe-alert-triangle"/> ícone.
  </Collapser>
</CollapserGroup>

## Coleta de dados de inventário [#data-collection]

O inventário é coletado do coletor de dados integrado do agente de infraestrutura, da integração do Amazon Elastic computar Cloud (EC2), da integração do agente fornecida pela New Relic e da integração criada pelo cliente. Os dados aparecem na página <DoNotTranslate>**Inventory**</DoNotTranslate> e em outros locais da nossa interface de infraestrutura.

<CollapserGroup>
  <Collapser
    id="linux"
    title="Dados do agente integrado do Linux"
  >
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Agente de linguagem APM metadados
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            display_name do agente, provedor de nuvem `instance-id, os.Hostname(), hostname -f, hostname`
          </td>
        </tr>
  ...
  <Collapser
    id="windows"
    title="Dados do agente integrado do Windows"
  >
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Display_name do agente, provedor de nuvem da instance-id, os.Hostname(), Registry (SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters (Domain, DhcpDomain, Hostname)
          </td>
        </tr>
  ...
  <Collapser
    id="cloud"
    title="AWS integração na nuvem inventário"
  >
...
Para obter uma explicação técnica sobre o atributo usado para preencher a página <DoNotTranslate>**Inventory**</DoNotTranslate>, consulte [Infraestrutura padrão atributo e evento](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#shared-attributes). Isso inclui um resumo de [eventos comuns do sistema operacional](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#events-os).

## Conjuntos de filtros [#filter-sets]

O componente de interface do conjunto de filtros é usado para filtrar seus hosts e criar visualizações salvas. <DoNotTranslate>**It is only available on the Events and Inventory UI pages and is in the process of being deprecated.**</DoNotTranslate> Foi substituído pelo recurso de [visualizações salvas](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#ui-features) . Quaisquer conjuntos de filtros criados a partir deste ponto não serão migrados para o recurso de visualizações salvas.

<CollapserGroup>
  <Collapser
    id="filter-set"
    title="Como usar conjuntos de filtros"
  >
    ...
    * Ambientes de teste
    ...
    O conjunto de filtros de infraestrutura padrão é <DoNotTranslate>**All hosts**</DoNotTranslate> e serve como modelo para você criar conjuntos de filtros.
    ...
    2. Se
       ...
       não for exibido na barra lateral esquerda, abra esse conjunto de filtros selecionando
    ...
    5. Clique em
       ...
       (consulte [Lógica do conjunto de filtros](#logic)).
    ...
    8. Para nomear seu filtro, clique no botão

       <Icon name="fe-edit-2"/>

       ícone, digite um nome e clique em
    ...
    <CollapserGroup>
      <Collapser
        id="match-string"
        title="Recomendado: selecione valores combinando uma string"
      >
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                Se você clicar em <DoNotTranslate>**Include**</DoNotTranslate> e inserir uma string com os valores que deseja corresponder, o filtro usará o comparador `LIKE`, o que significa que os resultados incluem qualquer entidade que seja semelhante à string.
            ...
            <tr>
              ...
              <td>
                Se você clicar em <DoNotTranslate>**Exclude**</DoNotTranslate> e inserir uma string com os valores que deseja corresponder, o filtro usará o comparador `NOT LIKE`, o que significa que os resultados excluem qualquer entidade que seja semelhante à string.
      ...
      <Collapser
        ...
        title="Selecione os valores individualmente"
      >
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                Se você clicar em <DoNotTranslate>**Include**</DoNotTranslate> e depois clicar em valores específicos, o filtro usará o comparador `IN`, o que significa que o filtro procura entidades que correspondam exatamente a um ou mais valores na sua lista de seleções.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                Se você clicar em <DoNotTranslate>**Exclude**</DoNotTranslate> e depois clicar em valores específicos, o conjunto de filtros usará o comparador `NOT IN`, o que significa que o filtro retornará todas as entidades que não correspondem exatamente a um ou mais valores na sua lista de seleções.
              </td>
            </tr>
          </tbody>
        </table>
