...
2. Siga as instruções adicionais para o seu tipo de lançamento:
   ...
   <CollapserGroup>
     ...
     <Collapser id="fargate-cloudformation" title="Tipo de lançamento AWS Fargate">
       ...
       ### Configurar NRIA\_IS\_FORWARD\_ONLY para Fargate [#fargate-isforwardonly]

       Ao implantar o agente de infraestrutura da New Relic como um sidecar em ambientes Fargate, é necessário definir a variável de ambiente `NRIA_IS_FORWARD_ONLY` como `true`. Esta é uma configuração crítica para a operação adequada em implantações Fargate sem servidor.

       *Por que esta configuração é necessária:*

       * **Evita a criação incorreta de entidade de host:** o Fargate é sem servidor e a AWS gerencia os recursos de computar subjacentes. Sem esta configuração, o agente de infraestrutura pode tentar criar entidade de host para uma infraestrutura inexistente.
       * **Evita discrepâncias de faturamento:** a medição baseada em entidade cobra dos clientes com base no número de entidade monitorada. A criação incorreta de entidade de host causa problemas de faturamento.
       * **Habilita a vinculação adequada de APM a contêiner:** Essa configuração garante que os aplicativos APM sejam vinculados corretamente a entidades de contêiner, em vez de entidades de Host sintéticas.

       Este exemplo de definição de tarefa inclui a configuração necessária do Fargate:
