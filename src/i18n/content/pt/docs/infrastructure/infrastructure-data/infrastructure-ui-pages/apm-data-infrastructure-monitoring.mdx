...
<Steps>
  ...
  <Step>
    ...
    * A primeira coisa que você vai fazer é acessar a aplicação `Billing Service` no APM e abrir a página <DNT>**Summary**</DNT> para obter uma visão geral da saúde do seu sistema. Uma pontuação Apdex alta, que é uma medida de satisfação do usuário, pode indicar que há um problema no seu sistema. Aqui você pode ver que a pontuação é 0,79 e disparou um evento de alerta crítico.
  ...
  <Step>
    ...
    Agora que você sabe que uma implantação recente na sua aplicação `Billing Service` causou um pico de erros e eventos de alerta críticos do Apdex, você pode querer analisar um host específico para obter mais clareza. Clique no nome do host que você deseja inspecionar. Isso exibirá uma barra lateral que importa todas as informações relevantes da página <DNT>**Infrastructure**</DNT>. Isso permite que você acesse todas as informações necessárias sobre o seu host e quaisquer erros de serviço, sem sair do restante dos seus dados.
