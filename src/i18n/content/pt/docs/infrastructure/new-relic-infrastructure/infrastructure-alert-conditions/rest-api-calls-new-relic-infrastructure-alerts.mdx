...
<CollapserGroup>
  <Collapser
    id=""
    title="Exemplo GET uma lista de condições"
  >
...
<CollapserGroup>
  <Collapser
    id=""
    title="Exemplo GET uma condição específica"
  >
...
<Callout variant="important">
  Se você quiser alterar a condição `type`, não use PUT. Em vez disso, exclua a condição existente e adicione (POST) uma nova condição com os novos campos de condição `type` e <DoNotTranslate>**all**</DoNotTranslate> .
</Callout>
...
<CollapserGroup>
  <Collapser
    id=""
    title="Exemplo de atualização (PUT) de uma condição"
  >
    ```
...
<CollapserGroup>
  ...
  <Collapser
    id="metric-api"
    title="Dados API de condições métricas"
  >
  ...
  <Collapser
    ...
    title="Host não relata condição"
  >
    ...
    * O campo `no_trigger_on` é opcional. Quando definido como `["shutdown"]` , ativa a opção de condição de infraestrutura <DoNotTranslate>**Don't trigger alerts for hosts that perform a clean shutdown**</DoNotTranslate> .
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <DoNotTranslate>**Condition type:**</DoNotTranslate> todos
        ...
        * O `warning_threshold` é opcional e só pode ser usado com `infra_metric` condições.
          ...
          <DoNotTranslate>**Condition type:**</DoNotTranslate> `infra_metric` formato:
          ...
          <DoNotTranslate>**Condition type:**</DoNotTranslate> `infra_process_running` formato:
          ...
          <DoNotTranslate>**Condition type:**</DoNotTranslate> `infra_host_not_reporting` formato:
    ...
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Recommendation:**</DoNotTranslate> Use `where_clause` ao criar uma nova condição.
      </td>
    </tr>
    ...
    <tr>
      <td>
        `integration_provider` (_corda_)
      </td>

      <td>
        <DoNotTranslate>**Condition type:**</DoNotTranslate> `infra_metric`

        Para alertas sobre integração, use `integration_provider` em vez de `event_type`. Para ver valores válidos: Na documentação New Relic para seus serviços na nuvem, verifique a seção <DoNotTranslate>**Find and use data**</DoNotTranslate>.

        Exemplo: na documentação [de integração de monitoramentoAWS RDS](/docs/integrations/amazon-integrations/aws-integrations-list/aws-rds-monitoring-integration) , você pode ver que o tipo de evento `DatastoreSample` pode ser usado com um valor `integration_provider` de `RdsDbInstance` para instância de banco de dados ou `RdsDbCluster` para cluster de banco de dados Aurora.
      </td>
    </tr>
    ...
    <tr>
      <td>
        `violation_close_timer` (_inteiro_)
      </td>
