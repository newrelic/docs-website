...
Quando APM e o agente de infraestrutura estão instalados nos mesmos hosts e usam o mesmo New Relic <InlinePopover type="licenseKey"/>ou chave de licença de contas na mesma organização, os dados APM devem aparecer em <DoNotTranslate>**infrastructure**</DoNotTranslate> e vice-versa. Caso você não veja essa ligação APM-infraestrutura, siga estas dicas de resolução de problemas.
...
Se você concluiu o [APM/infraestrutura integração](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/new-relic-apm-data-infrastructure) mas não vê os dados APM em <DoNotTranslate>**infrastructure**</DoNotTranslate>, tente estes procedimentos de resolução de problemas.

<CollapserGroup>
  <Collapser
    id="restart-app-server"
    title="Reinicie o servidor de aplicativos."
  >
  ...
  <Collapser
    id="hostnames"
    title="Certifique-se de que o nome do host seja o mesmo na interface do APM e da infraestrutura."
  >
    ...
    2. Na página APM
       ...
       do aplicativo, observe o nome do host associado ao aplicativo. Compare esse nome com o mesmo nome do host no monitoramento de infraestrutura.
  ...
  <Collapser
    id="gap-fqdn-issue"
    title="Verifique se há problemas de reconhecimento de FQDN do host substituto."
  >
  ...
  <Collapser
    id="gap-fqdn-issue"
    title="Certifique-se de que as etapas de integração do Kubernetes foram concluídas."
  >
