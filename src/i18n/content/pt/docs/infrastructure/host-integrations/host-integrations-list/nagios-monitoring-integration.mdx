...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Isso aponta para um [arquivo yaml](#service-checks-yaml) contendo definições das verificações de serviço que serão executadas pela integração. <DoNotTranslate>**Required.**</DoNotTranslate>
      </td>
...
<CollapserGroup>
  <Collapser
    ...
    title={<>Exemplo de configuração <InlineCode>nagios-config.yml</InlineCode></>}
  >
    ```
    integrations:
      - name: nri-nagios
  ...
  <Collapser
    ...
    title={<>Exemplo de configuração <InlineCode>nagios-service-checks.yml</InlineCode></>}
  >
    ```
    service_checks:
      - name: check_users
...
Para encontrar seus dados de integração, acesse <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**</DoNotTranslate> e selecione um dos links de integração do Nagios.
...
<CollapserGroup>
  <Collapser
    ...
    title="Erro de falha na análise de configuração"
  >
