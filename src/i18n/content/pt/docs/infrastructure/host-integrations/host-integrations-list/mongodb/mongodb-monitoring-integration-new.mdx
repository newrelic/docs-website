---
title: MongoDB monitoramento integração
tags:
  - Integrations
...
Nossa integração MongoDB coleta e envia métricas dimensionais do [MongoDB](https://www.mongodb.com/).
...
<figcaption>
  dashboard instalado através do [New Relic MongoDB monitor início rápido](https://newrelic.com/instant-observability/mongodb-prometheus).
</figcaption>

Para obter dados da instância do MongoDB, nossa integração MongoDB usa um [exportador Prometheus](https://github.com/percona/mongodb_exporter) de terceiros, que expõe a métrica do Prometheus na porta especificada na configuração. A integração recolhe essas métricas, transforma-as em entidade, filtra-as e depois envia-as para a New Relic.

Para instalar a integração de monitoramento do MongoDB, execute as seguintes etapas:

1. [Verifique os requisitos](#req).
2. [Instale e ative a integração](#install).
3. [Configure a integração](#configuration).
4. [Encontre e use dados](#find-and-use).

## Compatibilidade e requisitos [#req]

### Versões do MongoDB [#MongoDB-versions]

Nossa integração é compatível com MongoDB versões 4.0 a 7.0, [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb) e MongoDB Atlas M10 ou superior.
...
* Usuário MongoDB com [permissões`clusterMonitor` e `read` ](#enable-instance). Consulte [Configurando seu MongoDB](#setup-instance) para obter mais informações.
* Para monitor o MongoDB Atlas o usuário ainda precisa de um ambiente hospedado para instalar o agente de infraestrutura e integração e conectar-se remotamente à instância do Atlas.
...
<CollapserGroup>
  ...
  >
    1. Instale [o agente de infraestrutura](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install) e substitua a variável `INTEGRATION_FILE_NAME` por `nri-mongodb3`.
    ...
    4. Edite o arquivo de configuração `mongodb3-config.yml` com seu editor favorito. Confira alguns [exemplos de arquivos de configuração](#examples).

    5. Para ativar a análise automática de log e encaminhamento de erros do Mongodb, crie um arquivo YAML com o seguinte conteúdo em `/etc/newrelic-infra/logging.d/elasticsearch-log.yml`. Não há necessidade de reiniciar o agente.
  ...
  >
    1. Baixe o `nri-mongodb3` [instalador MSI](https://download.newrelic.com/infrastructure_agent/windows/integrations/nri-ibmmq/nri-mongodb3-amd64.msi).
    ...
    4. Edite o arquivo `mongodb3-config.yml` conforme descrito nas [definições de configuração](#config).
  </Collapser>

  <Collapser
    id="containerized-environments"
    title="Ambientes em contêineres"
  >
    Está disponível uma [imagem Docker](https://hub.docker.com/r/bitnami/mongodb-exporter/tags) que empacota exatamente o mesmo exportador Prometheus usado pela integração. Portanto, uma vez configurado e raspado o exportador, as métricas são as mesmas fornecidas pela integração.

    As etapas abaixo fornecem orientação para configurar a integração em ambientes como Kubernetes, onde você pode reportar métricas do Prometheus de diferentes fontes de forma centralizada. Se você precisar apenas configurar a integração em um contêiner Docker , verifique [as instruções Docker Openmetrics](#docker-openmetrics) .

    #### Executando o exportador

    Para monitor uma instância do MongoDB no Kubernetes ou ECS, você precisa executar a imagem do exportador do MongoDB como um arquivo secundário ou como uma workload separada.

    Observe que ambos os gráficos MongoDB Bitnami Helm fornecem uma maneira padrão de implantar e configurar o exportador MongoDB que você pode usar como referência.

    Por exemplo, o seguinte `values.yaml` do gráfico `bitnami/mongodb` configura cada workload do MongoDB com um exportador em execução como um arquivo secundário configurado como a integração NewRelic:
    ...
    Observe que, em vez de `--collect-all`, você também pode passar o coletor um por um e desabilitar algum deles se necessário: `--collector.dbstats --collector.collstats --collector.indexstats --collector.replicasetstatus --collector.topmetrics --collector.diagnosticdata`.

    #### Raspe a métrica

    Existem diferentes maneiras de extrair métricas do Prometheus e encaminhá-las para o New Relic, consulte [nossa documentação para saber mais](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent).

    Executando e raspando o exportador Bitnami ou `percona/mongodb_exporter`, as métricas extraídas são as mesmas e o <InlinePopover type="dashboards"/>funciona. No entanto, a integração adiciona um rótulo comum a todas as métricas extraídas de um cluster MongoDB específico `mongodb_cluster_name` para poder agrupar métricas provenientes de clusters diferentes.

    <Callout variant="tip">
      O atributo `mongodb_cluster_name` para todas as métricas do MongoDB é necessário pelo dashboard do MongoDB e você precisa adicioná-lo manualmente. Dependendo do agente que raspa a métrica do Prometheus, existem diferentes maneiras de adicionar rótulos comuns.
    </Callout>

    Por exemplo, a configuração do agente Prometheus a seguir adiciona a todas as métricas do Prometheus correspondentes a `mongodb_.*` o rótulo `mongodb_cluster_name=cluster-name-example`.
    ...
    Se você estiver usando vários pods `percona/mongodb_exporter` para fornecer métricas de vários clusters do MongoDB, poderá rotular cada um com um `mongodb_cluster_name` separado usando uma configuração como esta:
    ...
    Por outro lado, em um servidor Prometheus, você pode adicionar atributo personalizado a todas as métricas com `external_labels`, ou por job com `static_configs`, ou para cenários mais complexos `metric_relabel_config`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Prometheus Openmetrics integração para Docker (avançado)"
  >
    Se você precisar executar a integração a partir de um contêiner você pode usar [o Prometheus Openmentrics integração for Docker](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics).

    Execute o [mongodb-exporter](https://hub.docker.com/r/percona/mongodb_exporter/tags), que é o mesmo que o pacote de integração, junto com [nri-prometheus](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics). Você pode usar o exemplo `docker-compose` abaixo como base para sua configuração:
    ...
    Onde `config.yaml` é o arquivo de configuração do nri-prometheus. Verifique [os documentos correspondentes](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics) para obter detalhes adicionais.
    ...
    Supondo que o arquivo Docker-compose esteja na mesma pasta em que as variáveis de ambiente `config.yaml` e `${NEW_RELIC_LICENSE_KEY}` contêm sua chave de licença, você pode executá-lo da seguinte maneira:
...
### Configurando seu MongoDB [#setup-instance]
...
<CollapserGroup>
  <Collapser
    id="setup-hosted"
    title="Servidores MongoDB ou Percona hospedados"
  >
    <Callout variant="tip">
      Conclua estas etapas nos mongos a serem monitorados. Se você deseja coletar métricas no nível do mongod, como estatísticas de host ou conjunto de réplicas, crie também a função e o usuário em cada mongod.
    </Callout>

    1. No shell do MongoDB, mude para o banco de dados admin.

       ```
       use admin
       ```

    2. Use o comando a seguir para criar um novo usuário e atribuir funções `clusterMonitor` e `readAnyDatabase` ao usuário (ajuste o nome de usuário e a senha conforme necessário).
    ...
    3. Edite o arquivo de configuração YAML `mongodb3-config.yml` e informe os parâmetros necessários `mongodb_cluster_name` e `mongodb_uri`. Altere outras [opções de configuração,](#instance-settings) se necessário.

       Para mais informações consulte a [documentação do MongoDB Atlas](https://docs.mongodb.com/manual/reference/method/db.createUser/index.html).
  </Collapser>
  ...
  <Collapser
    ...
    title="Atlas"
  >
    1. Usando a CLI do Atlas, execute o comando a seguir para criar um novo usuário e atribuir funções `clusterMonitor` e `readAnyDatabase` ao usuário (ajuste o nome de usuário e a senha conforme necessário).

       ```
       atlas dbuser create -u username -p password --role clusterMonitor,readAnyDatabase
       ```

       Alternativamente, você pode usar a interface do Atlas para criar seu usuário e atribuir os mesmos privilégios específicos. Para mais informações consulte a [documentação do MongoDB Atlas](https://www.mongodb.com/docs/atlas/security-add-mongodb-users/#add-database-users).

    2. Edite o arquivo de configuração YAML `mongodb3-config.yml` e informe os parâmetros necessários `mongodb_cluster_name` e `mongodb_uri`. Altere outras [opções de configuração,](#instance-settings) se necessário.
  </Collapser>
</CollapserGroup>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        URI de conexão do MongoDB. <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
...
### Exemplo de configuração (mongodb3-config.yml) [#examples]
...
<CollapserGroup>
  ...
  >
    Esta é a configuração básica utilizada para coletar todas as métricas. Adicione credenciais ao seu URI mongodb, se forem necessárias. NOTA: Se sua senha contiver caracteres especiais, você precisará colocar aspas simples no URI
  ...
  <Collapser
    ...
    title="Desativando coletor"
  >
    Você pode desabilitar seletivamente o coletor. Neste exemplo `TOP` métrica não será coletada adicionando `top_stats: false`. Observe que desabilitar o coletor pode afetar seu painel:
  ...
  <Collapser
    ...
    title="Monitoramento multiinstância"
  >
    Esta é a mesma configuração básica usada para coletar métricas mostrando como executar a integração duas vezes conectando-se a dois hosts. Observe que cada instância da integração gerará um exportador extra escutando em uma porta diferente. Cada instância precisará de `mongodb_cluster_name` e `exporter_port` exclusivos.
  ...
  >
    Esta é a configuração básica utilizando filtros para restringir as coleções de monitores.<br/>Neste exemplo monitor todas as coleções do banco de dados `db1` e monitor apenas `collection2` do banco de dados `db2`. Adicione outros filtros separados por vírgulas:
  ...
  <Collapser
    id="auth-source"
    title="Fonte de autenticação alternativa"
  >
    Por padrão, a fonte de autenticação para qualquer conexão é `admin`. Se precisar definir uma fonte de autenticação alternativa, você pode adicioná-la ao URI do MongoDB como `/auth_source`:
  ...
  <Collapser
    ...
    title="Conexões TLS"
  >
    Se for necessário usar o TLS para se conectar à sua instância, use o URI do MongoDB para adicionar seu parâmetro TLS. Você pode verificar a lista de parâmetros permitidos na [documentação oficial do Mongo](https://www.mongodb.com/docs/manual/reference/connection-string/#tls-options):
  ...
  <Collapser
    id="atlas-config"
    title="Conjunto de réplicas do Atlas"
  >
    Ao conectar-se a uma instância do Atlas, certifique-se de que o endereço IP onde a integração está sendo executada tenha permissão para se conectar à sua instância do Atlas. Defina `mongo_direct_connect` como falso, necessário para conexões Atlas:
...
* O [New Relic MongoDB monitor início rápido](https://newrelic.com/instant-observability/mongodb-prometheus) está disponível através da [observabilidade instantânea (I/O)](https://newrelic.com/instant-observability/). Ele fornece um dashboard que permite explorar facilmente seus dados, entender o contexto e resolver problemas com mais rapidez.
...
* métrica estão anexadas à amostra métrica da [entidade](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) `MONGODB_INSTANCE`, `MONGODB_DATABASE` e `MONGODB_COLLECTION`.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Tipo de afirmação conforme [lista oficial](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#asserts).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * _vazio_ - para instância autônoma e Atlas
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Nível de preocupação para operações de consulta de leitura (disponível, linearizável, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#opreadconcerncounters).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo ou estado das conexões com o banco de dados (ativa, disponível, total, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#connections).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Contagem Tipo para estado de bloqueio global do banco de dados (leitores, gravadores, total). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#globallock).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Cursor Tipo para cursores abertos de dados (fixados, noTimeout, total, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.metrics.cursor.open).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operação de acesso ao documento (inserido, excluído, retornado, atualizado). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.metrics.document).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operação para contadores de operação (delete, getmore, insert, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#opcounters).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Modo para bloqueios de dados (R, W, r e w). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.locks).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Estado-membro em formato de string dos [códigos de status oficiais](https://www.mongodb.com/docs/manual/reference/replica-states/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Nome do cluster definido em seu [arquivo de configuração](#instance-settings). Observe que esta dimensão se aplica a todas as métricas.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operação para latência (leituras, gravações, comandos). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#oplatencies).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Latência buckets Métrica de desempenho de leitura/escrita do WiredTiger (bucket1, bucket2, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#wiredtiger).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Recurso ou tipo para bloqueios de dados (Global, Mutex, ParallelBatchWriterMode, ...). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.locks).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Status do conjunto de réplicas. Um número inteiro que representa o status da réplica seguindo os [códigos de status oficiais](https://www.mongodb.com/docs/manual/reference/replica-states/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de transação simultânea para mecanismo de armazenamento WiredTiger (gravação, leitura). [Mais informações](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.wiredTiger.concurrentTransactions).
      </td>
    </tr>
  </tbody>
</table>
...
Estas são as 3 [entidades](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) criadas: `MONGODB_INSTANCE`, `MONGODB_DATABASE` e `MONGODB_COLLECTION`.

As seguintes métricas dimensionais são capturadas raspando o exportador e vinculadas à entidade <DoNotTranslate>**MONGODB_INSTANCE**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="diagnostic-metrics"
    title="Métrica diagnóstica"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    id="oplog-metrics"
    title="Métrica log de operações"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    ...
    title="Métrica do conjunto de réplicas"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    ...
    title="Métrica de status do servidor"
  >
    <table>
      <thead>
        <tr>
...
As seguintes métricas dimensionais são capturadas raspando o exportador e vinculadas à entidade <DoNotTranslate>**MONGODB_COLLECTION**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="collection-metrics"
    title="Métrica de Status da Coleção"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    id="index-metrics"
    title="Métrica de status do índice"
  >
    <table>
      <thead>
        <tr>
...
As seguintes métricas dimensionais são capturadas raspando o exportador e vinculadas à entidade <DoNotTranslate>**MONGODB_DATABASE**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="dbstats-metrics"
    title="Banco de dados Status métrico"
  >
    <table>
      <thead>
        <tr>
...
## Migrar da versão herdada [#migrate]

Se você estiver usando a versão [herdada](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration) , encorajamos você a testar esta nova versão. Consulte a página [de migração](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-migration-guide) para obter orientação.
...
Além disso, esta integração aproveita um [exportador Prometheus criado pela comunidade](https://github.com/newrelic/newrelic-prometheus-exporters-packages/tree/main/exporters/mongodb3).
...
<CollapserGroup>
  <Collapser
    ...
    title="Prometheus ganha tempo"
  >
    Em algumas situações, quando um grande número de métricas deve ser reportado, a integração pode expirar ao tentar raspar o exportador Prometheus. Um erro semelhante deve ser encontrado no [arquivo de log](/docs/infrastructure/new-relic-infrastructure/troubleshooting/generate-logs-troubleshooting-infrastructure):
    ...
    Considere aumentar o valor padrão `scrape_timeout` para atenuar esse problema. Para obter mais informações sobre as opções de configuração, verifique as [configurações](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration-new#instance-settings) da instância do MongoDB.
  </Collapser>
</CollapserGroup>