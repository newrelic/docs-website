...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="important">
      Antes de configurar o Snowflake métrica, certifique-se de que `SNOWSQL` esteja configurado corretamente.
    </Callout>

    Execute o comando abaixo para armazenar o Snowflake métrica no formato JSON, permitindo que `nri-flex` o leia. Certifique-se de modificar `ACCOUNT`, `USERNAME` e `SNOWSQL_PWD` adequadamente.
