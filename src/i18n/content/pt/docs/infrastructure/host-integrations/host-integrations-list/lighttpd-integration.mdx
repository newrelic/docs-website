...
<Steps>
  ...
  <Step>
    ...
    Para instrumentar o Lighttpd início rápido e ver métricas e alertas, você também pode seguir nossa [página Lighttpd início rápido](https://newrelic.com/instant-observability/lighttpd) que possui um botão <DoNotTranslate>**Install now**</DoNotTranslate> .

    Aqui estão alguns exemplos de consulta:

    <DoNotTranslate>**Example:**</DoNotTranslate> Veja a métrica capturada no New Relic criador de consulta.

    ```sql
    SELECT * FROM LighttpdStatusSample  LIMIT MAX
    ```

    <DoNotTranslate>**Example:**</DoNotTranslate> Verifique a conexão ativa por servidor.
...
Para instrumentar o Lighttpd início rápido, instalar nosso dashboard padrão, e para ver métricas e alertas, você também pode seguir nossa [página Lighttpd início rápido](https://newrelic.com/instant-observability/lighttpd) clicando no botão <DoNotTranslate>**Install now**</DoNotTranslate> .
