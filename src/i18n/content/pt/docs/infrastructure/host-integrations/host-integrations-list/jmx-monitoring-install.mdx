...
<CollapserGroup>
  ...
  <Collapser id="linux-install" title="Linux">
    ...
    5. <DNT>**Optional**</DNT>: Se você estiver interessado em monitoramento Tomcat, use este exemplo de arquivo métrico:
  ...
  <Collapser id="windows-install" title="Windows">
    1. [Baixe o .exe instalador para integração jmx do New Relic](https://download.newrelic.com/infrastructure_agent/windows/integrations/nri-jmx/nri-jmx-amd64-installer.exe)
...
<CollapserGroup>
  <Collapser id="jmx-instance" title="Configurações da instância JMX">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            `COLLECTION_CONFIG` é útil para [configurar `nri-jmx` no Kubernetes usando anotações](#metrics-collection-kubernetes-annotations).
          </td>
        ...
        <tr>
          ...
          <td>
            Número de métricas que podem ser cobradas por entidade. Se este limite for ultrapassado a entidade não será reportada. Um limite de `0` não implica nenhum limite.
          </td>
  ...
  <Collapser id="labels" title="Etiquetas e atributo personalizado">
...
<CollapserGroup>
  <Collapser id="domains" title="Domínio domínio">
  ...
  <Collapser id="beans" title="Feijões">
...
<CollapserGroup>
  <Collapser id="jmx-configuration-configmap-example-using-helm" title="Exemplo de configMap usando gráficos de leme (recomendado)">
    ```yaml
    nri-bundle:
  ...
  <Collapser id="jmx-configuration-configmap-example-plain-manifest" title="Exemplo de configMap usando manifesto simples">
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
<CollapserGroup>
  <Collapser id="host-connection-example" title="Exemplo de arquivo de conexão de host">
    ```yml
    integrations:
      - name: nri-jmx
        env:
  ...
  <Collapser id="collection-example" title="Exemplo de arquivo de coleta métrica">
    ```yml
    collect:
...
Recomendação: Use o mesmo esquema de nomenclatura para métricas semelhantes em aplicativos diferentes.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        O nome do domínio JMX para essas métricas com o tipo de entidade `domain:` anexado.
      </td>
    </tr>
...
A integração JMX captura o parâmetro de configuração da integração JMX. Os dados estão disponíveis na [página Inventário](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure), na origem <DNT>**config/jmx**</DNT> . Para obter mais informações sobre dados de inventário, consulte [Compreender os dados de integração](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data).
...
<CollapserGroup id="troubleshooting-list">
  <Collapser id="integration-errors" title="Registro de pesquisa em busca de erros">
  ...
  <Collapser id="metrics-limit" title="Limite métrico excedido">
  ...
  <Collapser id="missing-metrics" title="Métrica ausente">
  ...
  <Collapser id="missing-dashboard" title="Dashboard não aparece na IU de infraestrutura">
  ...
  <Collapser id="jmxterm" title="Resolução de problemas via JMXTerm">
