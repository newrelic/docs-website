...
<Callout variant="tip">
  Esta integração é lançada como [código aberto no GitHub](https://github.com/newrelic/newrelic-unix-monitor). Se você encontrar algum problema, registre-o em ([https://github.com/newrelic/newrelic-unix-monitor/issues](https://github.com/newrelic/newrelic-unix-monitor/issues)).
</Callout>
...
* `insights_data_center` (em `insights`, padrão: `us`): se estiver usando o data center NR EU para sua conta, altere isso para `eu` ou `EU`. Se estiver usando o data center NR JP, altere isso para `jp` ou `JP`. Caso contrário, você pode deixar isso como está ou omitir essa configuração totalmente.
