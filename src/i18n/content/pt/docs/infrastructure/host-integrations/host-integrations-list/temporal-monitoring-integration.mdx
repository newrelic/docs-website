...
<Steps>
  ...
  <Step>
    ...
    6. Confirme se sua instância está sendo executada corretamente nos seguintes URLs:
       ...
       * A métrica do servidor Temporal estará disponível na página `http://YOUR_DOMAIN:8080/metrics`
  </Step>
  ...
  <Step>
    ## Configurar a integração [#configure-prometheus]

    Após a instalação bem-sucedida, você deve configurar sua configuração:
