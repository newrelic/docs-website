---
title: Integração temporal na nuvem
tags:
...
<Steps>
  <Step>
    ...
    Para utilizar a integração temporal na nuvem, é necessário primeiro [instalar o agente de infraestrutura](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) no mesmo host. O agente de infraestrutura monitora o próprio host, enquanto a integração que você instalará na próxima etapa estende seu monitoramento com dados específicos do tempo, como banco de dados e instância métrica.
  </Step>

  <Step>
    ## Configurar `nri-flex` para nuvem temporal [#configure-flex]

    Depois que as métricas da Nuvem Temporal forem expostas no endpoint do Prometheus, acessível via `https://<account-id>.tmprl.cloud/prometheus`, você precisará definir sua configuração adequadamente.
  ...
  <Step>
    ...
    Aqui está uma consulta NRQL para verificar a contagem de solicitações de serviço de front-end de nuvem temporal:
