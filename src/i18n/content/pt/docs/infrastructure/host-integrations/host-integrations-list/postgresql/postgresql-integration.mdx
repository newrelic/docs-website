...
4. Edite o arquivo de configuração `postgresql-config.yml` com seu editor favorito. Confira alguns [exemplos de arquivos de configuração.](#examples).

5. Antes de reiniciar o agente de infraestrutura, [crie um usuário](#create-user) com permissões `READ` nas funções necessárias.
