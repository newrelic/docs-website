...
<Steps>
  ...
  <Step>
    ...
    2. Use nosso [modelo de configuração](https://github.com/newrelic/nri-flex/blob/master/examples/json-read-cmd-example.yml) para atualizar os campos `YOUR_EVENT_TYPE` e `YOUR_HOST_IP` no arquivo chamado `nri-flex-nextcloud.yml`. O valor em `event_type` é usado para armazenar métrica no NRDB. Por exemplo, você pode atualizar `EVENT_TYPE` para `NextcloudFlexMetrics`
...
* Veja como [gerenciar seu dashboard](/docs/query-your-data/explore-query-data/dashboards/manage-your-dashboard) para ajustar o <InlinePopover type="dashboards" />modo de exibição ou adicionar mais conteúdo ao dashboard.