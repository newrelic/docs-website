...
Nossa HashiCorp Consul [integração no host](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) coleta e envia inventário e métricas do seu ambiente de datacenter Consul para a New Relic, onde você pode ver a saúde do seu ambiente. Coletamos dados nos níveis do datacenter e do agente/nó.
...
<CollapserGroup>
  ...
  <Collapser
    id="metrics-nofanout"
    title="Métrica somente com token e sem Fan Out"
  >
    Nesta configuração utilizamos um token ACL para conectar e não estamos coletando métricas de outros nós cônsul conectados (FAN_OUT: false):
...
Os dados desta integração podem ser encontrados em: <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services > Apache**</DoNotTranslate>.
...
### Métrica de amostra de datacenter Consul [#consul-datacenter-sample]
...
<Callout variant="tip">
  <DoNotTranslate>**Note**</DoNotTranslate>: seções aninhadas em `Config` e `DebugConfig` não são coletadas.
</Callout>

Os dados estão disponíveis na [página Inventário](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure), na origem <DoNotTranslate>**config/consul**</DoNotTranslate> . Para obter mais informações sobre dados de inventário, consulte [Compreender os dados de integração](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data).
