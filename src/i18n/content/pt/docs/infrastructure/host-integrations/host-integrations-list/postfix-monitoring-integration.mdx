...
<Steps>
  <Step>
    ...
    Para usar a integração do Postfix, você também precisa [instalar o agente de infraestrutura](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) no mesmo host. O agente de infraestrutura monitora o próprio host, enquanto a integração que você instalará na próxima etapa amplia seu monitoramento com dados específicos do Postfix.
  </Step>
  ...
  <Step>
    ...
    4. Ao ver nosso dashboard pré-construído, clique nele para instalá-lo em sua conta.
       ...
       <CollapserGroup>
         <Collapser
           id="latest"
           title="Veja o status do seu registro de mensagem mais recente"
         >
           ```sql
           SELECT * 
