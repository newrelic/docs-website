...
<CollapserGroup>
  <Collapser
    ...
    title="Ansible"
  >
    Comece a usar nosso [agente de monitoramento de infraestrutura](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/) usando [ferramentas de gerenciamento de configuração](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-configuration-management-tools).
  </Collapser>
...
A integração RabbitMQ coleta informações métricas(<strong>M</strong>) e de Inventário(<strong>I</strong>). Verifique a coluna <DoNotTranslate>**Applies To**</DoNotTranslate> abaixo para descobrir quais configurações podem ser usadas para cada coleção específica:
...
A integração captura o parâmetro de configuração do RabbitMQ no arquivo `/etc/rabbitmq/rabbitmq.conf` . dados de inventário são capturados apenas no [RabbitMQ versão 3.7](http://www.rabbitmq.com/blog/2018/02/05/whats-new-in-rabbitmq-3-7/) ou superior; os [dados de inventário](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data) aparecerão na [página<DoNotTranslate>**Inventory**</DoNotTranslate> ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)da interface da infraestrutura, na fonte `config/rabbitmq` .

<Callout variant="caution">
  Esteja ciente de que qualquer informação sensível que você colocar no arquivo `rabbit.conf` aparecerá na página <DoNotTranslate>**Inventory**</DoNotTranslate> na interface da infraestrutura. Isso inclui itens como os seguintes da AWS:
