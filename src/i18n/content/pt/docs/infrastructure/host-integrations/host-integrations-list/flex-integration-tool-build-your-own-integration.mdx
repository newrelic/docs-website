...
2. Consulte o documento [requisitos para o agente de infraestrutura](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent/) para garantir que seu sistema e quaisquer integrações on-host que você configurar atendam aos requisitos.
3. O Flex vem junto com nosso [agente de infraestrutura](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/) versão 1.10.7 ou superior, rodando em Linux, Windows ou Kubernetes.
...
<Steps>
  ...
  <Step>
    ...
    O objetivo é processar a saída do comando `df`, mostrando o sistema de arquivos e blocos de 1 byte, enquanto exclui sistemas de arquivos já suportados pelo agente. Se os sistemas de arquivos não suportados não estiverem montados, remova os argumentos `-x`.
