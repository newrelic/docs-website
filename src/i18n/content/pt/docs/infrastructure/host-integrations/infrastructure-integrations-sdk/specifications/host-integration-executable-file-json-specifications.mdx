...
Uma [integração no host do agente de infraestrutura](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) New Relic consistirá em pelo menos dois arquivos: um arquivo executável e pelo menos um arquivo de configuração. O arquivo executável gera dados JSON que são consumidos pelo agente de monitoramento de infraestrutura e enviados para a New Relic. Referimo-nos ao objeto JSON como protocolo de integração SDK.
...
<DoNotTranslate>**Recommendation:**</DoNotTranslate> Use Go para criar integração; é a linguagem que usamos para criar integração no host e as [ferramentas de construção de integração](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources). No entanto, você pode criar uma integração em qualquer idioma.
...
<CollapserGroup>
  ...
  >
    ...
    Use JSON padrão, não JSON "bem impresso", para a saída. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Inclua uma opção de linha de comando opcional (por exemplo, `--pretty`) para tornar o JSON "bem impresso" para fins de depuração.
  </Collapser>
...
Uma carga útil mínima seria um objeto JSON com apenas os campos de cabeçalho. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Se não houver dados para coletar, use o código de retorno do programa e a mensagem do log escrita em `stderr`.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Recommendation:**</DoNotTranslate> Use nomes de domínio reversos para gerar nomes de integração exclusivos.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Obrigatório. O tipo de entidade. Ele será utilizado pelo agente de infraestrutura como namespace para compor um <DoNotTranslate>**unique identifier**</DoNotTranslate> em conjunto com o `name`.
      </td>
    </tr>
...
<CollapserGroup>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> Para garantir que os tipos de eventos usados pela sua integração sejam exclusivos, prefixe o tipo de evento com o nome ou sigla da sua empresa. Por exemplo, se sua integração personalizada capturar a métrica do nó Cassandra e a métrica da família de colunas Cassandra como amostras diferentes, armazene-as em tipos de eventos diferentes, como `MyOrgCassandraSample` e `MyOrgCassandraColumnFamilySample`.
          ...
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> Prefixe sua métrica com uma categoria para ajudar na navegação pela métrica na interface do New Relic . A integração New Relic usa atualmente:
          ...
          <tr>
            ...
            <td>
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> Especifique a unidade de medida usando um sufixo de unidade se ainda não estiver incluído no nome da métrica original, como nos exemplos a seguir:
              ...
              * Medições de bytes: Use `Bytes`.

                <DoNotTranslate>**Recommendation:**</DoNotTranslate>

                Se uma métrica for capturada em uma unidade diferente, como `Megabytes`, converta-a em `Bytes`. Por exemplo: `db.allMemtablesOffHeapSizeBytes`.

              * Medições de tempo: Use `Milliseconds`.

                <DoNotTranslate>**Recommendation:**</DoNotTranslate>

                Se uma métrica for capturada em uma unidade diferente, como `Seconds`, converta-a em `Milliseconds`. Por exemplo: `query.readLatency50thPercentileMilliseconds`
            </td>
          </tr>
  ...
  >
    ...
    * Criando uma nova tabela
      ...
      Você pode visualizar esses dados na [página infraestrutura <DoNotTranslate>**Events**</DoNotTranslate> ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change)e [infraestrutura evento mapa de calor](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range). Você também pode consultar o tipo de evento `InfrastructureEvent` no New Relic.
  ...
  >
    ...
    * Outros metadados do sistema
      ...
      Você pode visualizar esses dados na [página<DoNotTranslate>**Inventory**</DoNotTranslate> ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)e [infraestrutura do evento mapa de calor](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range). Você também pode consultar dados relacionados a alterações de estoque.
    ...
    * Os valores podem ser do tipo escalar (string ou número) <DoNotTranslate>**or**</DoNotTranslate> ou aplicar objeto hash de valor principal. New Relic suporta hierarquia, mas os nós de valor final devem ser escalares.
