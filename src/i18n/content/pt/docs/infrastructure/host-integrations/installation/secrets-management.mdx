...
Com o gerenciamento de segredos, é possível configurar o agente e a integração no host para usar dados confidenciais (como senhas) sem precisar gravá-los como texto simples nos arquivos de configuração. Atualmente, são suportados Hashicorp Vault, AWS KMS, CyberArk, Azure Key Vault, ofuscação da CLI do New Relic e comandos personalizados.
...
Ao usar variáveis de ambiente em arquivos de configuração de integração on-host, a configuração [`passthrough_environment`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#passthrough_-environment) deve ser definida.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Configuração de recuperação de segredo do Azure Key Vault](#azure-key-vault-secrets)
      </td>
    </tr>
...
## Segredos do Azure Key Vault [#azure-key-vault-secrets]

Para recuperar segredos do Azure Key Vault, defina as seguintes propriedades na sua seção `azure-key-vault`. Ambas as chaves são obrigatórias:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        A URL da instância do Azure Key Vault da qual recuperar o segredo

        Exemplo: `https://my-vault.vault.azure.net/`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        O nome do segredo a ser recuperado. O agente sempre recupera a versão mais recente do segredo.
      </td>
    </tr>
  </tbody>
</table>

O agente se autentica no Azure Key Vault usando a cadeia de credenciais padrão do SDK do Azure, tentando os seguintes métodos em ordem até que um seja bem-sucedido. Nenhuma configuração adicional do agente é necessária:

* Variáveis de ambiente `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` e `AZURE_TENANT_ID`
* Uma identidade gerenciada atribuída à VM do Azure ou ao pod do AKS que executa o agente (recomendado para produção)
* Credenciais da CLI do Azure (`az login`), úteis para desenvolvimento e testes

<Callout variant="tip">
  Apenas valores secretos são suportados. A recuperação de certificados ou chaves do Azure Key Vault não é suportada.
</Callout>

O exemplo a seguir recupera um segredo chamado `mssql-password` de um Azure Key Vault e o utiliza como senha para uma integração no host do MSSQL:
