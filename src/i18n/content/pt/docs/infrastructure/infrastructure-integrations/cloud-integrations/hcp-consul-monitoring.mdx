...
Gere o arquivo de implantação do Terraform na página <DoNotTranslate>**Overview**</DoNotTranslate> da sua conta do Portal HCP.
...
Implantar o cluster Consul seguindo os passos para o ambiente apropriado na [visão geral ponta a ponta](https://learn.hashicorp.com/tutorials/cloud/consul-end-to-end-overview). <DoNotTranslate>**Important:**</DoNotTranslate> Certifique-se de seguir <DoNotTranslate>**Consul CLI configuration**</DoNotTranslate> na parte inferior da página <DoNotTranslate>**Deploy**</DoNotTranslate> .

## instalação [gostatsd](https://github.com/atlassian/gostatsd) [#statsd-install]

Copie [implantar-gostatsd.yaml](https://github.com/newrelic-experimental/hashicorp-quickstart-annex/blob/main/hcp-consul/deploy-gostatsd.yaml) e [rbac-gostatsd.yaml](https://github.com/newrelic-experimental/hashicorp-quickstart-annex/blob/main/hcp-consul/rbac-gostatsd.yaml) para seu cliente Kubernetes local.
...
1. Substitua
   ...
   pelo ID da sua conta New Relic.
...
### Editar cônsul-Client-configMap

Adicione [a configuração de telemetria do Consule](https://www.consul.io/docs/agent/telemetry) editando o ConfigMap do Cliente Consule:

```bash
kubectl edit cm consul-client-config
```

Adicione o seguinte em <DoNotTranslate>**data**</DoNotTranslate>:
...
1. Usamos
   ...
   aqui para que possamos adicionar a tag

   <DoNotTranslate>**consul.source.datacenter**</DoNotTranslate>

   : a cada métrica. Isso permite filtrar seu painel por Consul Datacenter.

2. Certifique-se de substituir

   <DoNotTranslate>**&lt;YOUR_CONSUL_DATACENTER_NAME_HERE>**</DoNotTranslate>

   pelo nome do seu Consul Datacenter.
...
Habilite o Consule métrica na implantação editando a implantação:

```bash
kubectl edit deploy consul-connect-injector
```

Procure por <DoNotTranslate>**default-enable-metrics**</DoNotTranslate> e defina-o como <DoNotTranslate>**true**</DoNotTranslate>:
...
Acesse o [HCP Consul início rápido](https://newrelic.com/instant-observability/hcp-consul) em New Relic Instant observabilidade e clique em <DoNotTranslate>**+ Install now**</DoNotTranslate>.
