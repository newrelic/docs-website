...
Recomendamos enviar os dados para uma conta New Relic que não seja de produção, onde você possa fazer testes com segurança. Se isso não for uma opção, os filtros do AWS CloudWatch Metric Streams estarão disponíveis para incluir ou excluir determinados namespaces que podem causar problemas.
...
## Métricas não disponível com CloudWatch Metric Streams

Quando métricas estão disponíveis para AWS CloudWatch Metric Streams com mais de 2 horas de atraso, essas métricas não são incluídas no stream.

Exemplos de namespace AWS que podem conter métricas agregadas e expostas após 2 horas incluem: AWS DMS, AWS RDS, AWS DocDB, AWS S3 e AWS DAX. Consulte [a documentação da AWS](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-metric-streams-setup.html) para saber mais.