...
<Callout variant="tip">
  As métricas do API Gateway v1 (REST) estão disponíveis tanto pela [integração do CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-metric-stream/) quanto pela integração de pesquisa de API para o Amazon API Gateway.
</Callout>
...
Para habilitar esta integração, use um dos seguintes métodos:

* <DNT>**CloudWatch Metric Streams**</DNT> (recomendado): siga o procedimento para [configurar o CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/connect/aws-metric-stream-setup/).
* <DNT>**API polling**</DNT>: siga os procedimentos padrão para [conectar os serviços da AWS ao New Relic](/docs/infrastructure/infrastructure-integrations/getting-started/connect-aws-integrations-infrastructure).
...
A forma de consultar os dados depende do método de ingestão:

* <DNT>**CloudWatch Metric Streams**</DNT>: os dados são reportados como métricas dimensionais. Vá para <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Metrics &amp; Events**</DNT> e filtre por `aws.apigateway`.
* <DNT>**API polling**</DNT>: os dados são reportados para o [tipo de evento](/docs/data-apis/understand-data/new-relic-data-types/#event-data) `ApiGatewaySample`, que é possível [consultar e explorar](/docs/using-new-relic/data/understand-data/query-new-relic-data).
