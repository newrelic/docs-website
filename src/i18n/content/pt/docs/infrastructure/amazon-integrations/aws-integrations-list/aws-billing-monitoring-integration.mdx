...
[A integração New Relic Infrastructure ](/docs/infrastructure/integrations-getting-started/getting-started/introduction-infrastructure-integrations)inclui uma integração para relatar seus dados de faturamento AWS para New Relic. Este documento explica como ativar esta integração e descreve os dados que podem ser reportados.

## Recurso [#features]

New Relic AWS A integração de faturamento da coleta dados financeiros de todos os seus aplicativos e AWS contas e os agrupa para facilitar às equipes a obtenção de visibilidade de custos. Essa integração coleta dados de diferentes subcontas e serviços e também avalia seus [orçamentosAWS ](http://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/budgets-managing-costs.html)em termos de gastos reais e gastos previstos.
...
* Ative a caixa de seleção
  ...
  <DoNotTranslate>**Receive Billing Alerts**</DoNotTranslate>

  em

  <DoNotTranslate>**AWS console > Billing and cost management > Billing preferences**</DoNotTranslate>

  . Isso é necessário para que a Amazon relate taxas de uso por meio do CloudWatch. Você não é obrigado a receber alertas da AWS.

<Callout variant="important">
  Você só precisa ativar a caixa de seleção <DoNotTranslate>**Receive Billing Alerts**</DoNotTranslate> para acessar o dashboard de faturamento (custos) .
</Callout>
...
1. No console de gerenciamento da AWS, selecione <DoNotTranslate>**Services > IAM > Roles**</DoNotTranslate>.

2. Selecione a função que você configurou para New Relic e selecione <DoNotTranslate>**Permissions**</DoNotTranslate>.

3. Na guia <DoNotTranslate>**Inline policy**</DoNotTranslate> da função selecionada, selecione a opção expandir <Icon name="fe-chevron-down"/> ícone se já existir, ou selecione a opção para adicionar uma nova política em linha à função.
...
6. Insira a seguinte declaração de permissão em <DoNotTranslate>**Policy Document**</DoNotTranslate>:
...
Para encontrar seus dados de integração na interface de infraestrutura, acesse <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate> e selecione um dos links de integração AWS Billing.
...
<CollapserGroup>
  ...
  >
    ...
    * Despesas previstas
      ...
      <DoNotTranslate>**NRQL example**</DoNotTranslate>: para obter uma tabela de diferentes orçamentos com gastos reais, previsões, limites e uso versus custo monetário, execute a seguinte consulta NRQL:
      ...
      <Callout variant="tip">
        O `providerAccountId` é o ID da sua conta de serviços na nuvem no New Relic. Você pode encontrá-lo em <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate>, próximo a <DoNotTranslate>**Provider account**</DoNotTranslate>.
      </Callout>
  </Collapser>

  <Collapser
    id="billing-service-cost"
    title="Custos de serviço agregados"
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: para agrupar provedores por nomes de serviço, execute a seguinte consulta NRQL:
  ...
  <Collapser
    ...
    title="Custos agregados de conta/subconta"
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: para agrupar provedores por nomes de conta, execute a seguinte consulta NRQL:
  ...
  <Collapser
    ...
    title="Custos não agregados de contas/subcontas e serviços da AWS"
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: para entender a divisão de custos dos serviços da AWS para um ID de conta da AWS específico, execute a seguinte consulta NRQL:
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: para entender a divisão de custos das contas AWS para seu serviço AWS EC2, execute a seguinte consulta NRQL:
