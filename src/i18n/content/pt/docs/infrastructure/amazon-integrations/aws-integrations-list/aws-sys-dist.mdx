...
Você pode usar o console do AWS Systems Manager para instalar ou atualizar o pacote do distribuidor <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> uma vez ou pode definir uma programação de instalação.
...
1. Abra o [console](https://console.aws.amazon.com/systems-manager) do AWS Systems Manager. No painel de navegação, escolha <DoNotTranslate>**Distributor**</DoNotTranslate>, selecione <DoNotTranslate>**Third party**</DoNotTranslate> e selecione o bloco <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> .

2. Selecione o modo de instalação. Você pode instalar ou atualizar o pacote <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> uma vez ou agendar a instalação. Para obter informações detalhadas sobre a instalação do <DoNotTranslate>**Distributor packages**</DoNotTranslate>, consulte [a documentação do Distribuidor do AWS Systems Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor-working-with-packages-deploy.html).
...
6. Na seção <DoNotTranslate>**Targets**</DoNotTranslate> , escolha os nós gerenciados nos quais deseja executar esta operação especificando a tag, selecionando instância ou dispositivos manualmente ou especificando um grupo de recursos. Se você não encontrar um nó gerenciado na lista, consulte [estes documentos da AWS](https://docs.aws.amazon.com/systems-manager/latest/userguide/troubleshooting-managed-instances.html).

7. Para <DoNotTranslate>**Rate Control**</DoNotTranslate> para <DoNotTranslate>**Concurrency**</DoNotTranslate>, especifique um número ou uma porcentagem de destino no qual executar o comando ao mesmo tempo.
...
8. Para <DoNotTranslate>**Error threshold**</DoNotTranslate>, especifique quando parar a execução do comando em outro destino após ele falhar em um número ou em uma porcentagem de nós gerenciados. Por exemplo, se você especificar três erros, o Systems Manager interromperá o envio do comando quando o quarto erro for recebido. Os nós gerenciados que ainda processam o comando também podem enviar erros.

9. <DoNotTranslate>**Verify the installation**</DoNotTranslate>. Depois de executar a instalação, verifique o progresso na área Status do comando. Quando você vê o status de Sucesso, significa que a instalação foi bem-sucedida.
