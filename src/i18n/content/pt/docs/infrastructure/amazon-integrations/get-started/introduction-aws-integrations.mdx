...
* **Automatize com CloudFormation**: CloudFormation é um serviço da AWS que permite modelar e automatizar sua configuração de integração usando modelos declarativos. Para reduzir erros de configuração manual, use este método para conectar sua conta da AWS ao New Relic. Você pode projetar completamente a integração na interface do New Relic e implantá-la no console AWS.

Este método provisiona os recursos que encaminham suas métricas para o New Relic, incluindo uma função <DNT>IAM</DNT>, um fluxo de métricas <DNT>Amazon CloudWatch</DNT> e um fluxo de entrega <DNT>Amazon Kinesis Data Firehose</DNT>. Os fluxos de métricas operam por região, portanto, você precisa de um fluxo de métricas e de um fluxo de entrega <DNT>Kinesis Data Firehose</DNT> em cada região da AWS que você monitora. Consulte o diagrama a seguir para ver como esses componentes se conectam.
...
<figcaption>
  Como uma integração do CloudWatch Metric Streams encaminha métricas para o New Relic. Você precisa de um fluxo de métricas e de um fluxo de entrega do Kinesis Data Firehose em cada região da AWS que você monitora.
</figcaption>
