...
9. Opcional: Defina o limite de tempo [<DNT>**Close open alert events after**</DNT>](#alert-event-time-limit) para fechar automaticamente eventos de alerta abertos após um determinado período de tempo (o padrão é 24 horas para condições de infraestrutura).
...
Qualquer pessoa incluída nos [canais de notificação](#notification) da política recebe notificações de alerta diretamente. Além disso, qualquer pessoa com permissões para sua conta New Relic pode visualizar eventos de alerta de infraestrutura e eventos de alerta individuais pela interface do usuário.
...
3. Da lista <DNT>**Events**</DNT>, selecione o evento de alerta.
4. Para [visualizar informações detalhadas em alertas](/docs/alerts/alert-event-management/view-alert-event-details) sobre o evento de alerta selecionado, selecione o link.
...
O [Alerts](/docs/alerts/new-relic-alerts-beta/getting-started/new-relic-alerts) oferece uma ferramenta de alertas única e coordenada em todos os seus produtos New Relic. Isso permite gerenciar [políticas de alerta](/docs/alerts/overview#alert-policy) e [condições](/docs/alerts/overview#alert-condition) que focam nas métricas de [entidades](/docs/alerts/overview#alert-entity) que são mais importantes para você, como contêineres Docker, JVMs e muito mais.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * Veja informações detalhadas sobre [eventos de alerta](/docs/new-relic-solutions/get-started/glossary#alert-event): Use a UI de [alertas](/docs/alerts/alert-event-management/view-alert-event-details) ou o [canal de notificação](#notification) integrado à política associada.
      </td>
    </tr>
...
O texto inserido no campo <DNT>**Description**</DNT> de uma condição de alerta é repassado aos eventos de alerta e notificações associados. Uma descrição pode ser usada para diversas finalidades, incluindo:
...
Você pode usar a substituição de template para inserir valores dos atributos no evento de alerta associado. O formato do modelo é `{{attributeName}}`. Para os atributos que você pode usar ao criar uma descrição, consulte [atributos de evento de alerta](/docs/alerts/create-alert/condition-details/alert-event-attributes).
...
Se houver tags de entidade associadas ao seu evento de alerta, então elas podem ser acessadas usando o nome da tag de entidade. Um exemplo disso seria `{{tag.aws.awsRegion}}`. Quando as tags de entidade estão disponíveis para uso, você as vê incluídas no evento de alerta e exibidas ao visualizar o evento de alerta.

Este campo tem um tamanho máximo de caracteres de 4.000.

## Adicionar ou editar um URL de runbook [#runbook-url-infrastructure]

O [processo de criação de condição de alerta](#create-condition) inclui uma opção para definir uma URL para instruções de runbook. Isso permite vincular informações ou procedimentos padrão para lidar com um evento de alerta. Antes de adicionar ou atualizar o link, certifique-se de usar uma URL válida.
...
## Limite de tempo do evento de alerta [#alert-event-time-limit]

O limite de tempo de evento de alerta permite definir um período após o qual os eventos de alerta serão encerrados forçadamente. Por padrão, o limite de tempo do evento de alerta é de 24 horas para condições de infraestrutura.

Para adicionar ou atualizar o limite de tempo do evento de alerta de uma condição de alerta:

1. Selecione uma condição de alerta e faça alterações no limite de tempo do evento de alerta.
2. Salve a condição.
...
Para mais informações, consulte [Práticas Recomendadas para Filtragem em Alertas de Infraestrutura](https://support.newrelic.com/s/hubtopic/aAX8W0000008Zfu/relic-solution-best-practice-for-filtering-in-infrastructure-alerts) no Fórum de Suporte da New Relic.