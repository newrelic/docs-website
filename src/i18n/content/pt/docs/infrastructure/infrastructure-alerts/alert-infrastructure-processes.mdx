...
Ao aplicar filtros aos hosts e processos que são importantes para o seu negócio, você pode definir [limites de alerta](/docs/alerts/overview#alert-threshold) para decidir quando os eventos de alerta são abertos e o New Relic envia uma notificação por e-mail para você, dependendo das [preferências de evento de alerta](/docs/alerts/organize-alerts/specify-when-alerts-create-events) da política. Estes exemplos ilustram como usar a condição <DNT>**Process running**</DNT> do monitoramento de infraestrutura para monitorar seus processos.

<CollapserGroup>
  <Collapser id="processes-load" title="Certifique-se de que processos suficientes estejam em execução para satisfazer a carga">
    <DNT>**Problem:**</DNT> Alguns balanceadores de carga e servidores de aplicação funcionam executando muitos processos de trabalho em paralelo. Aqui, por exemplo, você pode querer um evento de alerta quando menos de oito processos estiverem em execução para um serviço como o gunicorn.
  ...
  <Collapser id="one-critical" title="Monitor a inicialização em busca de processos críticos que requerem atenção especial">
    ...
    <DNT>**Solution:**</DNT> Use o limite <DNT>**At least one process is running**</DNT> com filtros de condição definidos para um nome de usuário e um executável específico, para que a New Relic possa abrir um evento de alerta quando o processo estiver em execução.
  </Collapser>

  <Collapser id="one-job-length" title="Certifique-se de que um trabalho não demore muito">
    <DNT>**Problem:**</DNT> Você tem um job que é executado periodicamente e deseja abrir um evento de alerta quando ele estiver em execução por mais tempo do que um número esperado de minutos.
...
Se você criar a condição de alerta diretamente com o monitoramento de infraestrutura, o New Relic enviará uma notificação por e-mail quando o limite definido para a condição de alerta for ultrapassado, dependendo das [preferências de eventos de alerta](/docs/alerts/organize-alerts/specify-when-alerts-create-events) da política. Sua política de alertas define quais pessoas ou equipes e quais canais de notificação usamos.