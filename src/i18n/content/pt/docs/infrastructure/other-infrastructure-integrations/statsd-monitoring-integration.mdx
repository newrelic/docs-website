...
* É necessário garantir que o data center correto, US (padrão) ou EU, tenha sido selecionado para a conta da New Relic. Dica: Se a license\_key começar com &quot;eu&quot;, será necessário usar a flag `NR_EU_REGION=true`.
* Para a região JP, o endpoint é selecionado automaticamente com base na sua chave de licença, portanto, nenhuma flag adicional é necessária.
* Certifique-se de que não há [`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/) relacionados à integração do StatsD.
