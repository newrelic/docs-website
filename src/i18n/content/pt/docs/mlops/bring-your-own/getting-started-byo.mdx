...
Obtenha seu <InlinePopover type="licenseKey" />(também referenciado como `ingest - license`) e defina-o como variável de ambiente: `NEW_RELIC_INSERT_KEY`. [Clique aqui](/docs/apis/intro-apis/new-relic-api-keys/#license-key) para obter mais detalhes e instruções. Você está reportando dados para a região da UE ou do JP da New Relic? clique [aqui](#EU-account-users) para obter mais instruções.
...
## Usuário da conta da UE e JP [#EU-account-users]
