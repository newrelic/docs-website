...
Se a atualização do Angular não for uma solução viável, uma alternativa é colocar o trecho copiar/colar do navegador do agente em um arquivo JavaScript separado e referenciá-lo no arquivo HTML de índice.

1. Crie um novo arquivo em seu diretório `src` ou `assets` chamado `newrelic.js`

2. Copie o trecho do agente navegador da página de configurações do aplicativo do navegador New Relic One
