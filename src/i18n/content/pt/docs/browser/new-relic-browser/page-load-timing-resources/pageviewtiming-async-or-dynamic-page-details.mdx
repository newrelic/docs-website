...
Se sua aplicação utiliza páginas assíncronas ou dinâmicas, você pode precisar de detalhes adicionais sobre o carregamento do site ou de componentes. Mas as páginas podem carregar conteúdo de muitas maneiras diferentes e o usuário pode controlar quando interage com esse conteúdo. É por isso que algumas métricas de desempenho centradas no usuário acontecem fora do onload padrão da janela (tempo de carregamento da página) no browser do agente.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        A pintura de maior conteúdo é uma das três métricas identificadas pelo Google como os [Core Web Vitals](https://web.dev/vitals/). Valores de LCP de até 2,5 segundos são considerados "Bom", entre 2,5-4,0 segundos são considerados "Precisa Melhorar" e acima de 4,0 segundos são considerados "Ruim".
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        NOTA: Embora este evento esteja disponível como um recurso experimental, os dados dele não são coletados automaticamente. Deve ser habilitado na configuração do browser do agente usando uma flag no objeto `init` , por exemplo `init: { page_view_timing: { long_task: true } }`.
...
* O relatório deste evento requer [o browser do agente versão 1153 ou superior](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes).

Siga nossas [notas de lançamento do browser do agente](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes) para saber quando novas métricas são lançadas.
