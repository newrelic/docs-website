...
**Consulta 1**: Esta consulta NRQL recupera todos os `BrowserPerformance` eventos para o `appName` especificado (&amp;quot;Meu aplicativo&amp;quot;) onde o `entryName` é `mark` ou `measure`.

```nrql
FROM BrowserPerformance SELECT * WHERE appName = 'My Application' AND entryName = 'mark' OR entryName = 'measure'
```

**Consulta 2**: Esta consulta NRQL calcula a média `entryDuration` para eventos de marca e medida dentro do `appName` especificado. A cláusula `FACET entryName` agrupa os resultados pelo campo `entryName` , fornecendo durações médias separadas para eventos de marcação e medição. Isso pode ser útil para comparar o desempenho médio das notas em relação às medidas.
