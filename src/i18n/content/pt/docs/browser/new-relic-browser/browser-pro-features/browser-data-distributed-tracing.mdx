---
title: Dados do navegador em distributed tracing
...
Se você usar o New Relic para monitor a atividade do navegador do usuário final, poderá aproveitar as vantagens do [distributed tracing](/docs/distributed-tracing/concepts/introduction-distributed-tracing/) para ver o rastreamento do lado do navegador originado da experiência do usuário final.

## Trace atividade do frontend ao backend [#benefits]

Ao relatar os dados do navegador no distributed tracing, você pode ver a conexão entre a atividade de front-end e a atividade backend . Os dados de rastreamento da New Relic em uma transação completa, desde o tempo gasto por um usuário final no navegador da web, até a atividade de rede e serviços backend associados. Isso ajuda você:

* Detecte latências, erros e anomalias no navegador ou na rede com mais facilidade.
...
* Para distributed tracing: [agente navegador Pro+SPA](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent#agent-types) com [distributed tracing habilitado](#enable) e [navegador agente versão 1153](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1153) ou superior
* Para [compartilhamento de recursos de origem cruzada](#cors): [navegador agente versão 1158](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1158) ou superior
* Para suporte [W3C Trace Context](https://www.w3.org/TR/trace-context/) : [navegador do agente versão 1173](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1173) ou superior
...
Para habilitar distributed tracing para monitoramento do navegador:

1. Certifique-se de que seu agente de monitoramento de navegador e agente APM sejam [compatíveis](#requirements) com distributed tracing. Atualize para a versão mais recente, se aplicável.
...
5. [Reimplante o agente de monitoramento do navegador](/docs/browser/new-relic-browser/installation/upgrade-browser-agent), reiniciando o agente APM associado ou atualizando a instalação de copiar/colar do navegador.

6. Se você tiver aplicativos ou serviços downstream do seu aplicativo de navegador e eles usarem o Infinite Tracing, conclua as [etapas de configuração do Infinite Tracing](#infinite-tracing).

## Cabeçalhos W3C Trace Context [#w3c]

Com o lançamento do agente de monitoramento de navegador versão 1173, oferecemos suporte aos cabeçalhos W3C Trace Context (`traceparent` e `tracestate`), portanto, certifique-se de permiti-los em sua configuração. W3C Trace Context define um par de cabeçalhos HTTP de contexto padronizados que servem para propagar informações de correlação de contexto entre serviços:
...
2. Configure o monitoramento do navegador em seu
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      A New Relic não pode realizar nenhuma validação para garantir que os serviços nas outras origens foram configurados corretamente. Se não tiver certeza sobre como permitir esses cabeçalhos, <DoNotTranslate>**do not**</DoNotTranslate> adicione recursos de origem cruzada à lista aprovada na interface do New Relic.
    </Callout>
  </Collapser>
  ...
  >
    ...
    2. Certifique-se de que seu agente e <InlinePopover type="apm"/>agente de monitoramento do navegador sejam [compatíveis](#requirements) com distributed tracing para CORS. Atualize para a versão mais recente, se aplicável.

    3. Vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Settings > Application settings**</DoNotTranslate>.

    4. Ative o botão de alternância <DoNotTranslate>**Distributed tracing**</DoNotTranslate> se ainda não estiver ativado.

    5. Ative o botão <DoNotTranslate>**Cross-origin resource sharing (CORS)**</DoNotTranslate> .

    6. Em <DoNotTranslate>**Cross-origin resource sharing (CORS)**</DoNotTranslate>, adicione recursos de origem cruzada à lista aprovada. Certifique-se de que seus recursos de origem cruzada incluam o prefixo `http://` ou `https://` e o nome de domínio. Inclua o número da porta se for diferente do padrão para HTTP (porta 80) ou HTTPS (porta 443).

    7. Selecione <DoNotTranslate>**Save application settings**</DoNotTranslate> para atualizar a configuração do agente.

    8. [Reimplante o navegador do agente](/docs/browser/new-relic-browser/installation/upgrade-browser-agent) (reiniciando o APM do agente associado ou atualizando a instalação do navegador copiar/colar).
  </Collapser>
</CollapserGroup>

## Habilitar rastreamento infinito [#infinite-tracing]

Oferecemos um tail-based distributed tracing recurso chamado [Infinite Tracing](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing). Se os aplicativos do seu navegador tiverem serviços downstream, certifique-se de habilitá-los. Isso garante que seu intervalo raiz (o aplicativo de navegador inicial) seja incluído no restante dos intervalos rastreados pelo Infinite Tracing.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * Consulte pelo nome do aplicativo do navegador configurando `browserApp.name`.
        * Consulta para rastreamento contendo pelo menos um intervalo de aplicativo de navegador com `browserApp.name is not null`.
        * Consulta para rastreamento contendo pelo menos um aplicativo backend com `appName is not null`.
        * Consulta para rastreamento contendo extensões de navegador e backend , combinando as duas condições anteriores.
      </td>
    </tr>
  </tbody>
</table>
