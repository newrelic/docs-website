...
Os recursos de página detectados pelo agente do browser poderão ser consultados por meio do tipo de evento `BrowserPerformance` . Você pode usar esses dados para criar uma consulta e um painel personalizados no [New Relic One](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one).
