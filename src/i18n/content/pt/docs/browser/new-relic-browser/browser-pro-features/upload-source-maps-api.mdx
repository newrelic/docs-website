...
Nosso <InlinePopover type="browser"/>oferece suporte ao upload de [mapas de origem](/docs/new-relic-browser-source-maps), que são usados para desminificar o rastreamento de pilha de erros na [página<DoNotTranslate>**Errors**</DoNotTranslate> ](/docs/errors-inbox/browser-tab). Este documento explica como usar a API para fazer upload de mapas de origem usando a API do navegador.

## Prepare-se para usar a API do mapa de origem [#questions]

Para fazer upload de mapas de origem para o navegador por meio da API, você precisará de:
...
<CollapserGroup>
  <Collapser
    id="what-url"
    title="Qual é o URL JavaScript?"
  >
    Cada vez que o agente captura um erro no seu código, ele é associado à URL do JavaScript em que ocorreu. Este é o atributo `src` da tag de script no seu HTML. Este URL JavaScript completo é necessário ao enviar mapas de origem ao navegador.

    Você pode encontrar o URL do arquivo JavaScript de um erro no navegador, na página <DoNotTranslate>**JS errors**</DoNotTranslate> . Consulte [os mapas de origem do monitoramento do navegador](/docs/new-relic-browser-source-maps) para obter mais informações sobre como encontrar esses erros na interface.
  </Collapser>

  <Collapser
    id="release-id"
    title="É necessário um nome e ID de lançamento?"
  >
    ...
    * `https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.js`
      ...
      Se os URLs do seu aplicativo tiverem automaticamente as informações da versão anexadas, o navegador do agente terá tudo o que precisa para corresponder os erros ao seu código. Você pode avançar para [a geração de mapas de origem](#generate).

      Se este <DoNotTranslate>**doesn't**</DoNotTranslate> se aplicar a você e os URLs JS **não** tiverem informações de versão anexadas, você terá que ajudar o agente especificando um [nome de versão e um ID com a API](/docs/browser-api-newrelicaddrelease).
  </Collapser>

  <Collapser
    id="repo-url"
    title="É necessário um URL de repositório ou um hash de aplicação de commit de construção?"
  >
  ...
  <Collapser
    id="limits"
    title="Existem limites para uploads de mapas de origem?"
  >
  ...
  <Collapser
    id="app-id"
    title="Qual ID do aplicativo deve ser usado?"
  >
...
Agora que você tem um ou mais mapas de origem, está pronto para publicá-los no navegador. Você pode usar qualquer um destes métodos para enviar mapas de origem ao navegador:
...
<CollapserGroup>
  <Collapser
    id="npm-publish"
    title="npm linha de comando: Publicar"
  >
  ...
  <Collapser
    id="npm-list"
    title="npm linha de comando: Lista mapas publicados"
  >
  ...
  <Collapser
    id="npm-delete"
    title="linha de comando npm: Excluir"
  >
...
<CollapserGroup>
  <Collapser
    ...
    title="npm via script Node.js: Publicar"
  >
  ...
  <Collapser
    id="npm-client-list"
    title="npm via script Node.js: Listar mapas publicados"
  >
  ...
  <Collapser
    ...
    title="npm via script Node.js: Excluir"
  >
...
Quando terminar, acesse a [página<DoNotTranslate>**JS errors**</DoNotTranslate> ](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time)no navegador, selecione um agrupamento de erros e veja se o rastreamento de pilha de erros não foi minimizado.
...
<CollapserGroup>
  <Collapser
    id="curl-post"
    title="curl: Carregar mapas"
  >
    Um exemplo de uso de API via curl para publicar mapas no navegador:
  ...
  <Collapser
    id="curl-list"
    title="curl: Lista mapas existentes"
  >
