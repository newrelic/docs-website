...
Nosso <InlinePopover type="browser"/>oferece suporte ao upload de [mapas de origem](/docs/new-relic-browser-source-maps), que são usados para desminificar o rastreamento de pilha de erros na [página<DoNotTranslate>**Errors**</DoNotTranslate> ](/docs/errors-inbox/browser-tab). Este documento explica como usar a API para fazer upload de mapas de origem usando a API do browser.
...
<CollapserGroup>
  ...
  >
    ...
    Você pode encontrar o URL do arquivo JavaScript de um erro no browser, na página <DoNotTranslate>**JS errors**</DoNotTranslate> . Consulte [os mapas de origem do monitoramento do browser](/docs/new-relic-browser-source-maps) para obter mais informações sobre como encontrar esses erros na interface.
  </Collapser>
...
Quando terminar, acesse a [página<DoNotTranslate>**JS errors**</DoNotTranslate> ](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time)no browser, selecione um agrupamento de erros e veja se o rastreamento de pilha de erros não foi minimizado.
...
Se você estiver tendo problemas para gerar mapas de origem a partir do seu sistema de compilação, ou se seus erros no browser permanecerem minimizados, consulte a documentação [de resolução de problemas dos mapas de origem](/docs/new-relic-browser-source-maps#troubleshoot) .