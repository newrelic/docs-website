...
Os eventos de erro de JavaScript (JS) são compilados na [<InlinePopover type="browser"/>página de erros de JS](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time). Por padrão, eles são agrupados por mensagem de erro. Ao selecionar um único evento de erro, você verá mais detalhes em sua lista <DoNotTranslate>**Error Instances**</DoNotTranslate> . Em alguns casos, uma ou mais instâncias podem não ter um stack trace.

<Callout variant="tip">
  Em vez disso, você verá a mensagem "<DoNotTranslate>**This error is likely coming from a third-party site and no stack trace is available**</DoNotTranslate>."
</Callout>

## Solução

A ativação do compartilhamento de recursos de origem cruzada (CORS) pode eliminar esse problema, expondo todos os detalhes do erro JS ao seu aplicativo e permitindo que o stack trace do erro seja capturado pelo navegador do agente.
...
2. No seu aplicativo, faça com que o navegador carregue arquivos JavaScript usando o atributo CORS `crossorigin` :
