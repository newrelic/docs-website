...
Nosso <InlinePopover type="browser"/>inclui [medidas de segurança rigorosas](/docs/browser/new-relic-browser/performance-quality/security-new-relic-browser) para fornecer um produto robusto e independente com monitoramento de recursos do navegador. Antes de [instalar o navegador do agente](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser), certifique-se de que seu sistema atenda a esses requisitos.

## Requisitos básicos [#requirements]

Para reportar dados à New Relic, seu aplicativo deve atender a dois requisitos básicos:

1. O tráfego para o aplicativo deve ser proveniente de clientes capazes de carregar e executar o código JavaScript do navegador.
2. Os clientes que acessam o aplicativo devem ser capazes de enviar dados para a New Relic por meio de solicitações HTTP. HTTPS é necessário para o monitoramento mais avançado do recurso do navegador.
...
A maioria dos aplicativos de navegador típicos atendem a esses requisitos, mas alguns aplicativos de navegador em ambientes restritos podem ter dificuldade em relatar dados à New Relic. Por exemplo:
...
Instrumentação e recurso específico podem ser compatíveis com outros navegadores ou versões.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [`PageAction` pode](/docs/insights/new-relic-insights/adding-querying-data/inserting-custom-events-attributes-insights-javascript-api) exigir um navegador que suporte totalmente cross-domínio `XMLHttpRequests`. O Internet Explorer versões 9 e anteriores não podem registrar o evento `PageAction` . Saiba mais sobre [como consultar seus dados](/docs/using-new-relic/data/understand-data/query-new-relic-data).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [trace da sessão](/docs/browser/new-relic-browser/dashboard-details/session-traces-exploring-webpages-life-cycle) são reportados somente a partir de navegadores que suportam a [Especificação de Tempo de Recursos](http://www.w3.org/TR/resource-timing/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        A New Relic só pode monitor navegadores compatíveis com [SHA-256](https://support.globalsign.com/customer/portal/articles/1499561-sha-256-compatibility).
      </td>
    </tr>
  </tbody>
</table>
...
<Callout variant="important">
  Quando você usa monitoramento de navegador com cookies, os cookies da New Relic são cookies de terceiros em seu site e podem não armazenar ou funcionar em determinados navegadores usados por seus visitantes. Consulte os sites do navegador aplicável para obter detalhes sobre sua compatibilidade com cookies de terceiros.
</Callout>
...
Se você estiver implantando navegador para um aplicativo que já está sendo monitorado pelo APM, certifique-se de que a versão do seu agente APM suporta monitoramento de navegador:

* [Go](/docs/release-notes/agent-release-notes/go-release-notes): Versão 2.5.0 ou superior
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Se você estiver usando AngularJS e quiser usar nosso [recurso de monitoramento SPA](/docs/browser/single-page-app-monitoring/get-started/welcome-single-page-app-monitoring), observe que as versões 0.6.18 a 0.6.24 do Zone.js não são compatíveis com o navegador. Em todos os casos, o navegador do agente <DoNotTranslate>**must**</DoNotTranslate> deve ser carregado antes do Zone.js para evitar erros.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        O navegador não consegue instrumentar adequadamente os métodos `open` e `send` de solicitações quando os testes são executados no Cypress. Isso fará com que o seguinte erro seja gerado:

        ```
        Cannot set property 'status' of undefined
        ```

        Isso afetará apenas os clientes que executam testes com o navegador agente presente em seu código. O aplicativo de produção não deve ser afetado.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        O navegador agente não é compatível com versões do MooTools anteriores a `1.6.0` ou com qualquer versão que inclua a camada de compatibilidade do MooTools. Caso migrar do MooTools não seja uma opção, recomendamos usar a versão `1.6.0-nocompat`.
      </td>
    </tr>
  </tbody>
</table>
...
### SPA: versão do navegador do agente [#spa-browser-version]

O monitoramento do SPA requer uma versão específica do SPA do trecho do navegador, disponível para [o agente do navegador versão 885 ou superior](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes). Esta versão do agente está habilitada por padrão.

### SPA: tipos de navegador [#spa-browser-types]

O monitoramento do SPA requer a [API do navegador`addEventListener` ](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener)e a [API Navigation Timing](/docs/browser/new-relic-browser/browser-agent-apis/navigation-timing-api). Ambas API estão disponíveis em todos os navegadores modernos, incluindo Google Chrome, Mozilla Firefox, Apple Safari e Microsoft Internet Explorer (IE) versões 9 ou superior.

### SPA: requisitos framework [#spa-framework]

Como a instrumentação SPA funciona agrupando API do navegador de baixo nível, ela é independente framework . A instrumentação SPA é compatível com a maioria das estruturas SPA, como Angular, Backbone, Ember e React. Também pode instrumentar solicitações feitas usando JSONP.

Abaixo estão os problemas de compatibilidade conhecidos:

* Se seu aplicativo usa AngularJS e você deseja usar os recursos [de monitoramento SPA](/docs/browser/single-page-app-monitoring/get-started/welcome-single-page-app-monitoring) do navegador, as versões 0.6.18-0.6.24 do Zone.js não são compatíveis com o agente SPA.
...
<Callout variant="important">
  Se o seu site não permitir a execução de JavaScript hospedado por terceiros, você deverá solicitar uma exceção CSP por meio dos procedimentos padrão da sua organização para instalar o agente de monitoramento do navegador.
</Callout>

Ao solicitar uma exceção de CSP, siga os procedimentos padrão da sua organização para entrar em contato com a equipe da Web ou de segurança. As exceções de CSP para seu caso de uso dependem da versão do navegador do agente que você está executando atualmente. Recomendamos sempre atualizar para a versão mais recente do navegador do agente.

Você pode adicionar todas essas exceções à diretiva substituta `default-src` (em vez de `script-src` e `connect-src`). O navegador do agente requer as seguintes exceções de CSP:
...
### `nonce` apoiar [#nonce-support]

A partir do navegador do agente [1.247.0](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1.247.0/), o agente respeitará um atributo `nonce` anexado ao script embutido ao carregar blocos de código adicionais. Se o seu site ou infraestrutura for capaz de injetar um atributo `nonce` no script do navegador do agente embutido, você só precisará solicitar uma exceção CSP para `connect-src`.

<Callout variant="important">
  Nosso suporte integrado `nonce` não se aplica ao uso do pacote npm do navegador agente. Com o pacote npm, o agente passa a fazer parte do código do seu site ou aplicativo. Quaisquer blocos adicionais carregados serão feitos por meio do código do seu aplicativo e você terá controle total sobre quando e se um atributo `nonce` será usado.
</Callout>

Alguns APM de agente capazes de injetar o navegador do agente também suportam a adição de um atributo `nonce` ao script injetado.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Oferece suporte à injeção do trecho JavaScript por meio da API `Transaction.BrowserTimingHeader` . Você precisará escrever código adicional para modificar o elemento de script para adicionar o atributo `nonce` . Consulte [Adicionar monitoramento de navegador para aplicativos Go](/docs/apm/agents/go-agent/features/add-browser-monitoring-your-go-apps/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Suporta adição de `nonce` por meio da API `getBrowserTimingHeader` e `getBrowserTimingFooter` . Consulte a [documentação do monitoramento do navegador do agente Java](/docs/apm/agents/java-agent/instrumentation/instrument-browser-monitoring-java-agent-api/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Suporta adição `nonce` por meio da API `GetBrowserTimingHeader` . Consulte a [documentação do monitoramento do navegador do agente .NET](/docs/apm/agents/net-agent/other-features/browser-monitoring-net-agent/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Suporta adição `nonce` por meio da API `getBrowserTimingHeader` . Consulte a [documentação do agente Node.js sobre monitoramento do navegador](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Suporta adição `nonce` por meio da API `browser_timing_header` . Veja a [documentação do agente Ruby sobre monitoramento do navegador](/docs/apm/agents/ruby-agent/features/new-relic-browser-ruby-agent/).
      </td>
    </tr>
  </tbody>
</table>
