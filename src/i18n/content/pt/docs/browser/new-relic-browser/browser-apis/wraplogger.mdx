...
* Se você estiver instalando o agente do browser via npm e criando um agente personalizado com recursos selecionados, você deve habilitar o recurso `logging` ao criar a instância `Agent`. Por exemplo, adicione o seguinte no array`features`:
