...
* Se você estiver instalando o agente do browser via npm e criando um agente personalizado com recursos selecionados, você deve habilitar o recurso `jserrors` ao criar a instância `Agent`. No array `features`, adicione o seguinte:
