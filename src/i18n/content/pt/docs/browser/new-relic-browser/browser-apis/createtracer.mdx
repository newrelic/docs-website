...
<Callout variant="important">
  O método `createTracer` na API do SPA foi descontinuado. A maneira recomendada de trace a duração de uma tarefa é capturar uma [marca](https://developer.mozilla.org/en-US/docs/Web/API/Performance/mark) e/ou [medida](https://developer.mozilla.org/en-US/docs/Web/API/Performance/measure) de desempenho. Versões futuras do navegador do agente capturarão marcas e medidas automaticamente, momento em que o suporte para `createTracer` cessará.
</Callout>
