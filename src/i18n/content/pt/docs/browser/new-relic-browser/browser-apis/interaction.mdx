...
* A opção `waitForEnd` requer v1.285.0+

* A opção `targetPageLoad` requer v1.313.1+
...
* <DNT>**Page load interaction:**</DNT> Se a opção `targetPageLoad` for usada, o handle é sempre vinculado à interação de carregamento inicial de página, independentemente de qualquer navegação suave que possa estar em andamento.

<Callout variant="caution">
  Usar `waitForEnd: true` junto com `targetPageLoad: true` manterá a interação de carregamento inicial da página aberta indefinidamente até que `.end()` seja chamado, alterando a definição padrão do IPL. Este é um caso de uso avançado e deve ser usado com cautela.
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `targetPageLoad` - O padrão é `false`. Se definido como `true`, o handle retornado é vinculado à interação de carregamento inicial da página em vez de criar ou direcionar uma interação de navegação suave. Esta é a única maneira de definir a interação initialPageLoad como destino.
      </td>
    </tr>
  </tbody>
</table>
