...
<Callout variant="tip">
  Esta chamada de API se aplica a dados em [visualizações de páginas SPA](/docs/browser/single-page-app-monitoring/get-started/introduction-single-page-app-monitoring) no browser e ao tipo de evento [`BrowserInteraction`](/docs/insights/insights-data-sources/default-data/browser-default-events-attributes-insights#browserinteraction-attributes) . Para definir um nome personalizado para visualizações de página padrão e o tipo de evento <DoNotTranslate>**PageView**</DoNotTranslate> , consulte [`setPageViewName`](/docs/browser/new-relic-browser/browser-agent-spa-api/set-pageview-name). É recomendado usar as duas chamadas juntas.
</Callout>
