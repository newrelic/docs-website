...
O NerdGraph pode ser usado como uma alternativa para ativar e desativar essa configuração. Para fazer isso, execute uma consulta de mutação no objeto graphQL `browserSettings` para a propriedade `browserConsentMode`, definindo `enabled` como `true` ou `false`.
