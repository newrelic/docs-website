---
title: Atualizar o navegador do agente
tags:
  - Browser
  - Browser monitoring
  - Installation
...
* Inspecione o código fonte do agente trecho:
  ...
  * Para `v1216` ou menos:
    ...
    3. Os números que seguem `nr-` são sua versão atual. Por exemplo, `js-agent.newrelic.com/nr-593.min.js` indica que você está executando a versão `593` do script do navegador.
...
Para verificar se a versão do seu navegador está desatualizada:

1. Consulte a tabela [de fim de vida útil do navegador do agente](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/) .
2. Se o número da versão mais recente for superior ao número da versão que você está executando atualmente, atualize o navegador do agente.

## Verifique o método de implantação [#deploy-method]

Seu agente navegador pode ter sido [implantado](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#options) de duas maneiras: usando uma implementação manual (copiando e colando o trecho) ou automaticamente usando um agente <InlinePopover type="apm"/>. Para verificar qual método de implantação foi usado, execute este comando no console JavaScript:

```
> newrelic.info.sa
```

Se o comando retornar `1`, então seu navegador agente usou o [método copiar/colar](#upgrading-copy-paste).

## Atualize sua instalação gerenciada pelo APM [#upgrading-apm]

Para atualizar [a instalação do navegador gerenciado pelo APM](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#options), reinicie o agente APM. Seu aplicativo será atualizado automaticamente para o trecho JavaScript mais recente.
...
Use qualquer uma das opções a seguir para acessar o trecho JavaScript do navegador necessário para atualizar uma instalação de copiar/colar. Depois de acessar o trecho, certifique-se de substituir <DoNotTranslate>**all**</DoNotTranslate> do seu trecho existente pela nova versão.
...
<CollapserGroup>
  <Collapser
    id="snippet"
    title="Recuperar o trecho da interface"
  >
    Baixe a versão mais recente do trecho: Vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Application settings**</DoNotTranslate>.
  ...
  <Collapser
    id="rest_api"
    title="Extraia o trecho com a API REST"
  >
    Para atualizar o trecho usando a API REST do New Relic, siga o processo para [visualizar um aplicativo de navegador específico](/docs/apm/apis/browser-examples-v2/adding-or-listing-browser-apps-api-v2#specific-browser-app). O atributo `loader_script` na sua resposta incluirá o trecho JavaScript mais recente.

    Essa pode ser uma boa solução se você tiver muitos aplicativos para gerenciar ou se o aplicativo do seu navegador estiver vinculado a um aplicativo <InlinePopover type="apm"/>.
  </Collapser>

  <Collapser
    id="end_point"
    title="Atualizar usando o endpoint do carregador"
  >
    Para atualizar o trecho do endpoint do carregador New Relic, você pode escolher qual tipo de [navegador de agente](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#agent-types) deseja:
    ...
    * [endpoint Lite](https://js-agent.newrelic.com/nr-loader-rum-current.min.js)
    ...
    1. Encontre o ID do aplicativo do navegador e a chave de licença: siga os procedimentos padrão para usar a [interface do New Relic](/docs/browser/new-relic-browser/installation-configuration/copying-your-browser-monitoring-license-key-app-id) ou a [API REST](/docs/apm/apis/browser-examples-v2/adding-or-listing-browser-apps-api-v2#specific-browser-app).

    2. Defina o ID do aplicativo do navegador e a chave de licença imediatamente após o trecho.
