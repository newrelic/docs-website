---
title: Instale o navegador do agente
tags:
  - Browser
  - Browser monitoring
  - Installation
...
Para começar a usar <InlinePopover type="browser"/>, você precisará adicionar nosso navegador agente ao HTML das visualizações que deseja monitor. O navegador agente é um trecho de código JavaScript que monitora o desempenho do seu aplicativo/site e envia esses dados para a New Relic. Você pode adicionar esse código manualmente ao seu site ou usar nosso agente APM para injetar esse código para você.

## Antes de você começar [#before-start]

Antes de começar, você pode verificar se o aplicativo do seu navegador atende aos nossos [requisitos de compatibilidade](/docs/browser/new-relic-browser/getting-started/compatibility-requirements-browser-monitoring).
...
Para instalar o navegador agente: Acesse <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > Add data > Browser and mobile > Browser monitoring**</DoNotTranslate> e siga as instruções.
...
<ButtonGroup>
  <ButtonLink
    ...
    variant="primary"
  >
    Instalar monitoramento de navegador
  </ButtonLink>
</ButtonGroup>
...
* Agora que você já conhece o agente navegador, aprofunde seus dados com nosso [tutorial Melhore seu site](/docs/journey-performance/guide-to-monitoring-core-web-vitals).
* Aprenda como otimizar sua experiência de monitoramento de navegador com nosso [guia de melhores práticas para monitoramento de navegador](/docs/new-relic-solutions/best-practices-guides/full-stack-observability/browser-monitoring-best-practices-guide).

## Métodos de instalação [#choose-deployment]

Quando se trata de adicionar o navegador agente às suas páginas da web, você tem dois métodos de implantação:

* APM + monitoramento de Browser: Utilize um de nossos agentes APM para auto-instrumento de monitoramento de Browser
* Somente monitoramento de navegador: Adicione nosso navegador agente manualmente como um trecho JavaScript ao HTML do seu frontend

Aqui estão algumas orientações sobre esses dois métodos.

### APM + monitoramento de navegador [#apm-agent]

Se você estiver usando um agente APM (monitoramento de desempenho de aplicativos (APM)) para monitor o backend do seu aplicativo de navegador, recomendamos usar este método. Recomendamos esta opção para aplicativos não[SPA](/docs/browser/single-page-app-monitoring/get-started/introduction-single-page-app-monitoring) porque o agente APM implantará automaticamente o agente navegador em todas as páginas do seu site e você obterá automaticamente a versão mais recente do agente navegador sempre que reiniciar o agente APM.
...
* [.LÍQUIDO](/docs/agents/net-agent/features/page-load-timing-net)
...
* [Pitão](/docs/agents/python-agent/supported-features/page-load-timing-python)
* [Ruby](/docs/agents/ruby-agent/features/page-load-timing-ruby)

### Monitoramento apenas do navegador [#copy-paste]
...
Por possível desempenho ou outros motivos, você pode atrasar a inicialização do navegador do agente desde o início do ciclo de carregamento da página. Você não poderá usar a API `newrelic` até que o agente seja totalmente inicializado. Esse atraso também cria dados incompletos na interface de monitoramento do seu navegador.
