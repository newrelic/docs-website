...
O suporte a mapas de origem é útil principalmente para "decodificar" JavaScript minificado. JavaScript reduzido resulta em rastreamento de pilha de erros praticamente inútil na página <DoNotTranslate>**Errors**</DoNotTranslate> do navegador. O upload de mapas de origem converte esses erros em rastreamento de pilha compreensível, com referências úteis às linhas de código. Este recurso também pode ser útil para código JavaScript empacotado ou transpilado.

O navegador oferece duas opções para usar mapas de origem:

1. Carregar mapas de origem através da interface do navegador
2. Publique mapas de origem no navegador por meio da API
...
Você pode arrastar e soltar ou fazer upload de um arquivo de mapa de origem na interface do navegador para associá-lo a um arquivo JavaScript específico. O New Relic converterá o rastreamento de pilha minificado em rastreamento não minificado e código-fonte visível na página Erros.
...
2. Vá para
   ...
   <DoNotTranslate>**Errors without a stack trace**</DoNotTranslate>
...
3. Na seção
   ...
   <DoNotTranslate>**Upload source map**</DoNotTranslate>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Em um quadro stack trace expandido, selecione <DoNotTranslate>**Remove file**</DoNotTranslate>.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  <Collapser
    id="generate-example"
    title="Gere mapas de origem com UglifyJS"
  >
    <DoNotTranslate>**Generate source maps using UglifyJS:**</DoNotTranslate> Ao "enfeioar" os arquivos de origem, especifique um nome de arquivo de mapa de origem e inclua o conteúdo de origem original:
  ...
  <Collapser
    id="generate-example"
    title="Gere mapas de origem com webpack"
  >
    <DoNotTranslate>**Generate source maps using [Webpack](https://webpack.github.io/docs/configuration.html#devtool):**</DoNotTranslate> Na configuração do webpack de produção, basta especificar `source-map` para a propriedade `config.devtool` . A propriedade `sourceMapFilename` de `config.output` é opcional e o padrão é `[name].js.map`.
