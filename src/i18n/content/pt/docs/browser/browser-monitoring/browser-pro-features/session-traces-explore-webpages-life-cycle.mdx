...
<Steps>
  <Step>
    ...
    5. Verifique se <DoNotTranslate>**Session traces**</DoNotTranslate> está ATIVADO. Eles estão habilitados por padrão.
  ...
  <Step>
    ...
    * <DoNotTranslate>**Default**</DoNotTranslate> coleta 90 rastreamentos por hora, independentemente do tráfego do seu aplicativo. Escolha este modo se você precisar apenas de uma pequena amostra do trace da sessão. Observe que nem sempre há rastreamento disponível nas sessões em que você está resolvendo problemas.
    * <DoNotTranslate>**Custom**</DoNotTranslate> permite coletar rastreamento de 0,000001 a 100% de todas as sessões e de 0,000001 a 100% de sessões com erros. Escolha este modo se desejar mais sessões de rastreamento disponíveis para solucionar problemas de tempo de carregamento da página ou erros de JavaScript.
...
A partir de 10 de junho de 2024, os dados de rastreamento da sessão contribuirão para o consumo mensal geral de dados. Isso significa que será cobrado de acordo com seu pedido existente. Essa mudança reflete a atualização de armazenamento para o banco de dados New Relic (NRDB), que permite [a personalização de sua taxa de amostragem](#configure-sampling-rates), retenção de dados estendida e desempenho aprimorado.

No [criador de consulta](https://one.newrelic.com/data-exploration/query-builder), execute a consulta abaixo para visualizar seu consumo mensal atual de dados de trace da sessão. Observação: se você executá-lo a partir de uma [conta principal](/docs/accounts/original-accounts-billing/original-users-roles/parent-child-account-structure/#hierarchy), os resultados refletirão o consumo de todas as contas secundárias.
...
Para uma visão mais granular, execute a seguinte consulta para ver o consumo diário de uma única conta infantil (se aplicável):
...
Você pode diminuir o consumo de dados de rastreamento da sessão [ajustando suas taxas de amostragem](#configure-sampling-rates) ou desativando o rastreamento de sessão.
