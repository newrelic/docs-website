...
Esta página fornece soluções para problemas comuns que podem ser encontrados ao usar a automação de fluxo de trabalho. Para suporte adicional, consulte o [Fórum de suporte da New Relic](https://support.newrelic.com/s/) ou entre em contato com o [Suporte da New Relic](https://support.newrelic.com/).
...
3. **Revisar permissões:** Garanta que sua função ou usuário do IAM tenha as permissões específicas da AWS que seu fluxo de trabalho exige. Consulte a tabela [Permissões necessárias](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials#required-permissions).
...
Para instruções detalhadas sobre como visualizar o status e o histórico de execução do fluxo de trabalho, consulte [Visualizar fluxos de trabalho](/docs/workflow-automation/manage-workflows/managing-workflow#view-workflows).
...
**Próximo passo:** Corrija o problema e, em seguida, [acione manualmente o fluxo de trabalho](/docs/workflow-automation/workflow-automation-apis/start-workflow-run) para verificar se funciona.
...
1. **Remova todos os fluxos de trabalho agendados primeiro**: Se você desativou o FCM, certifique-se de que todos os fluxos de trabalho agendados futuros sejam removidos para evitar confusão ou alertas:
   ...
   * Exclua todos os agendamentos ativos usando a [API DeleteSchedule](/docs/workflow-automation/workflow-automation-apis/delete-schedule)

2. **Verifique se todos os agendamentos foram removidos**: Confirme se não há execuções agendadas ativas antes de desativar o FCM

3. **Desativar o FCM**: Assim que todos os agendamentos forem removidos, você pode desativar a Workflow Automation com segurança por meio das configurações do FCM
...
4. **Verificar token nos segredos**: Verifique se o token do Slack armazenado no [gerenciador de segredos](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials) está correto e não expirou.
...
1. **Verifique as permissões do SSM**: Adicione estas permissões à sua política de função do IAM:
...
1. **Verifique se o [CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) está configurado**: Necessário para métricas do EC2 em tempo real
2. **Verifique se a [integração de monitoramento do EC2](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-ec2-monitoring-integration#activate) está ativa**: Garante que os dados da instância fluam para o New Relic
...
### Fluxo de trabalho de RCA do Slack do Autopilot

**Mensagem vazia do Slack sem conteúdo de RCA:** `analysisRaw` retornou uma resposta, mas `analysisText` não conseguiu extrair o conteúdo — `finalAnswer` não estava na estrutura `.card.body` esperada. Confirme se o seletor `expression` está em uma única linha e se o prompt <DNT>Autopilot</DNT> retorna uma resposta de cartão estruturada. Consulte [RCA de alerta com Autopilot e Slack](/docs/workflow-automation/workflow-examples/integrations/autopilot-slack-rca) para obter o padrão de seletor canônico.

**Mensagem &quot;<DNT>Autopilot</DNT> RCA Failed&quot; no Slack:** `analysisRaw` estava vazio — <DNT>Autopilot</DNT> não retornou nenhum `finalAnswer`. Verifique se `issueId` é um UUID de problema de alerta ativo e válido e se o prompt <DNT>Autopilot</DNT> está bem formado.

**Fluxo de trabalho falha na validação de entrada:** tanto `issueId` quanto `slackDestinationId` devem estar no formato UUID. Use `{{ issueId }}` ao acionar a partir de fluxos de trabalho de alerta.
