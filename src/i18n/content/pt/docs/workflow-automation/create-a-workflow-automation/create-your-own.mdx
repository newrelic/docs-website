...
Crie fluxos de trabalho personalizados do zero quando os modelos não atenderem às suas necessidades. Use o construtor [crie o seu próprio](https://onenr.io/0OQM47KgxjG) para arrastar e soltar ações do [catálogo de ações](/docs/workflow-automation/setup-and-configure/actions-catalog/actions-catalog) em uma automação que corresponda ao seu processo.
...
* **Aprenda os conceitos principais primeiro**: Leia [Conceitos principais](#core-concepts) e [padrões de fluxo de trabalho](#workflow-patterns) para entender os fundamentos e, em seguida, aplique-os
...
* **Padrões de referência**: Use a seção [padrões de fluxo de trabalho](#workflow-patterns) como referência rápida ao criar seus próprios fluxos de trabalho
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        [Entradas e segredos](/docs/workflow-automation/workflow-examples/basic-patterns/workflow-inputs)
      </td>
    ...
    <tr>
      <td>
        [Ações](/docs/workflow-automation/setup-and-configure/actions-catalog/actions-catalog)
      </td>
    ...
    <tr>
      <td>
        [Fluxo de dados](/docs/workflow-automation/workflow-examples/basic-patterns/data-passing)
      </td>
    ...
    <tr>
      <td>
        [Atribuir](/docs/workflow-automation/workflow-automation-apis/definition-schema#assign)
      </td>
    ...
    <tr>
      <td>
        [Espere](/docs/workflow-automation/workflow-automation-apis/definition-schema#wait)
      </td>
    ...
    <tr>
      <td>
        [Erro](/docs/workflow-automation/workflow-automation-apis/definition-schema#error)
      </td>

      <td>
        Falhar o fluxo de trabalho com uma mensagem personalizada, opcionalmente sujeito a uma condição
      </td>
    </tr>
  </tbody>
</table>
...
<Callout variant="tip">
  **Aprenda na prática:** Cada conceito é demonstrado no [exemplo de passo a passo](#example-walkthrough). Você verá entradas, switches, loops e gates de aprovação trabalhando juntos em um fluxo de trabalho real.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Falhar fluxo de trabalho com mensagem
      </td>

      <td>
        `type: error` com `message:` e opcional `condition:`
      </td>

      <td>
        Parar o fluxo de trabalho com uma mensagem de falha personalizada quando uma condição for atendida
      </td>
    </tr>
  </tbody>
</table>

Para padrões detalhados de tratamento de erros, consulte [Melhores práticas](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#handle-errors).

<Callout variant="important">
  Para obter as melhores práticas e limitações de segurança e privacidade, incluindo a exclusão de dados, consulte [melhores práticas de segurança](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#security).
</Callout>
...
2. Defina parâmetros para credenciais (do [gerenciador de segredos](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials): `${{ :secrets:keyName }}`), configuração (regiões, tipos de instância) e dados de tempo de execução (IDs de conta, IDs de alerta)
3. Arraste ações do [catálogo](/docs/workflow-automation/setup-and-configure/actions-catalog/actions-catalog), conecte-as com a sintaxe `${{ .steps.stepName.outputs.field }}` para passar dados
4. Insira [switches](#conditional-branching-with-switches) para ramificação condicional, [loops](#loops-for-processing-lists) para processamento de listas ou polling, [gates de aprovação](#approval-gates-and-waiting) para decisões humanas
5. Execute após cada seção para detectar erros antecipadamente, depois [inicie ou agende](/docs/workflow-automation/manage-workflows/workflow-entity-overview) seu fluxo de trabalho

## Padrões de fluxo de trabalho principais [#workflow-patterns]

Quatro padrões essenciais lidam com a maioria dos cenários de automação. Cada padrão é demonstrado no [passo a passo de exemplo](#example-walkthrough) abaixo.
...
**Exemplo:** [Tratar a resposta da equipe](#handle-team-response), [Verificar e limpar](#verify-and-clean-up)
...
**Exemplo:** [Executar o redimensionamento](#execute-the-resize)
...
**Exemplo:** [Solicitar aprovação da equipe](#request-team-approval)
...
* **Gerenciador de segredos**: Configurado, consulte [gerenciador de segredos](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials)
...
## Referência completa de parâmetros [#parameter-reference]

Este fluxo de trabalho requer credenciais, configuração e contexto de tempo de execução como entradas. Valores sensíveis vêm do [gerenciador de segredos](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials) usando a sintaxe `${{ :secrets:keyName }}`.
...
<DocTiles>
  ...
  <DocTile title="Práticas medidas" path="/docs/workflow-automation/limitations-and-faq/workflow-best-practices">
