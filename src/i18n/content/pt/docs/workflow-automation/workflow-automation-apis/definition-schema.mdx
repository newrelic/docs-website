...
* **Tipo**: mapa de mapas
* Tamanho máx.: 100
* Um mapa de entradas de fluxo de trabalho que o fluxo de trabalho aceita.
* Exemplo:
...
Constantes de fluxo de trabalho podem ser usadas com expressões JQ para transformar valores:
...
Convertendo carimbo de data/hora com a ação `DateTime`
...
<Callout variant="important">
  * As constantes do fluxo de trabalho são somente leitura e fornecidas automaticamente pelo tempo de execução
  ...
  * Ao contrário de `workflowInputs`, as constantes não precisam ser declaradas na definição do fluxo de trabalho
</Callout>
...
Uma etapa que encerra o fluxo de trabalho e relata uma falha com uma mensagem personalizada. Se um `condition` estiver presente, ele é validado primeiro, e o fluxo de trabalho para apenas quando a condição for `true`. Se você não fornecer um `message`, a mensagem padrão `failed as requested from step: failureStep` é usada.
...
Um exemplo completo de fluxo de trabalho que demonstra vários recursos de fluxo de trabalho, incluindo constantes de fluxo de trabalho, seletores, etapas de espera, consulta NRDB e instruções switch.
