...
<CollapserGroup>
  <Collapser id="moreforloop" title="Loop para iterar por uma lista, mapa ou coleção.">
    Você pode usar `loop` para iterar por uma lista, mapa ou coleção.

    Um loop itera sobre uma coleção dada definida por `in`. Ele criará automaticamente variáveis de loop `index` e `element` para cada uma de suas iterações. Essas variáveis de loop são acessíveis dentro do loop com a expressão JQ `${{ .steps.<loopStepName>.loop.element }}` ou `${{ .steps.<loopStepName>.loop.index }}`
    ...
    **Exemplos**:
    ...
    O seguinte mostra como executar o loop `workflowInputs`. Note que o resultado deve ser um tipo de coleção.
    ...
    <Callout variant="important">
      * `for`: (obrigatório). Este é o elemento de nível superior que indica o início de um laço &quot;for&quot;.
      * `in`: (obrigatório). Para que a coleção de entrada seja iterada, ela deve ser convertível em uma matriz Java.
      * `steps`: (obrigatório). Em cada iteração, os passos serão executados.
      * `element` e `index` são atribuídos automaticamente como parte do loop.
      * `index` é de base zero.
      * `element` Pode ser um tipo complexo se você tiver uma coleção de elementos complexos.
    ...
    **Saltar dentro de um loop**
    ...
    É permitido saltar entre etapas nomeadas pertencentes ao mesmo laço \`for\`. Não é permitido entrar ou sair de um laço \`for\`, seja para um laço interno ou externo, ou entre dois laços \`for\` diferentes.
    ...
    **Use \`break\` ou \`continue\` em um loop.**
