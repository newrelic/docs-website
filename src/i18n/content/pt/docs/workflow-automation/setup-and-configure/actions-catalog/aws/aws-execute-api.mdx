...
Esta página fornece uma referência abrangente para a AWS Execute API disponível no catálogo de ações de automação de workflow. Essas ações permitem que você execute qualquer operação da API da AWS.
...
Para usar esta ação, você deve configurar as credenciais da AWS. Consulte [Configurar credenciais da AWS](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials/) para instruções detalhadas sobre a criação de uma função do IAM ou de um usuário do IAM.
...
## Executar uma API da AWS [#aws-execute-api]
  ...
  <TabsPages>
    ...
    <TabsPageItem id="awsexecuteoutputs">
      ...
      <Callout variant="important">
        ...
        * Consulte [credenciais da AWS](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials/) para obter instruções.
