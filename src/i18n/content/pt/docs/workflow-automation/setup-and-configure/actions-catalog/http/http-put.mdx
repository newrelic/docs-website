...
Esta página fornece uma referência completa para o comando http put disponível no catálogo de ações de automação de fluxo de trabalho. Essas ações permitem que você execute operações de requisição HTTP PUT.
...
## Fazer o PUT de um recurso em uma URL [#http-put]
