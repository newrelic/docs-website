...
<CollapserGroup>
  <Collapser id="cloud-cost-attributes" title="Atributo CloudCost">
    No New Relic, navegue até **All Capabilities &gt; CloudCost Intelligence**. Abra a gaveta inferior do console Consulta e insira o seguinte NRQL Consulta:
...
<CollapserGroup>
  <Collapser id="kubernetes-cost-attributes" title="Tipo de evento de custo do Kubernetes">
    ...
    Para calcular custos precisos Kubernetes, multiplique o parâmetro `cpu_weightage + memory_weightage` em sua consulta de custos. O exemplo a seguir mostra o formato de consulta correto:

    ```sql
    SELECT SUM(net_unblended_cost * (cpu_weightage + memory_weightage)) FROM KubernetesCost SINCE '2025-09-01' UNTIL '2025-09-10'
    ```

    Esta consulta multiplica o `net_unblended_cost` pela soma da ponderação da CPU e da memória para fornecer alocações de custos precisas com base no uso de recursos.
