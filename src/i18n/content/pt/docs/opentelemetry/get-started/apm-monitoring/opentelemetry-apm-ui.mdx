...
Os dados assumidos pela consulta são produzidos pela [biblioteca de instrumentação de tempo de execuçãoOpenTelemetry Go](https://opentelemetry.io/docs/specs/semconv/runtime/go-metrics/).

<Callout variant="important">
  As [convenções semânticas](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/runtime/go-metrics.md) para o ambiente de execução Go Métrica ainda estão em desenvolvimento. A página de tempo de execução do Go, destino, é uma versão mais antiga das convenções. Você deve habilitar as convenções antigas ativando a variável de ambiente `OTEL_GO_X_DEPRECATED_RUNTIME_METRICS`. Para obter mais informações, consulte a [documentação](https://github.com/open-telemetry/opentelemetry-go-contrib/blob/9a6e0bf68e1da495be52faf152b5b4e6fb09d475/instrumentation/runtime/internal/x/README.md) da instrumentação.

  A New Relic eventualmente dará suporte às novas convenções assim que elas se tornarem estáveis.
</Callout>
...
## Como os erros são representados na New Relic [#errors]

As informações de erro do seu aplicativo aparecem em dois lugares na interface do APM:

* **Taxa de erros**: exibida na [página de resumo](#summary-page), derivada de dados de métrica. Quando uma métrica do OpenTelemetry, como `http.server.request.duration` ou `rpc.server.call.duration`, inclui um atributo `error.type`, esse ponto de dados contribui para a taxa de erros.
* **Errors Inbox**: veja a [página Errors Inbox](#errors-inbox-page), orientada por dados de span. Um span é classificado como um erro quando seu status é definido como `ERROR` (por exemplo, `otel.status_code = ERROR`).

### Erro ignorado [#ignore-errors]

Os agentes de idiomas da New Relic podem ser configurados para erro ignorado por código de status HTTP ou classe de erro, evitando que afetem a taxa de erros ou apareçam na Errors Inbox. Com o OpenTelemetry, não há configuração de agente equivalente, mas o mesmo efeito pode ser alcançado manipulando os dados subjacentes antes de chegarem ao New Relic.

#### Erro ignorado na taxa de erros

A taxa de erros é derivada dos mesmos dados de métrica dos quais [derivamos as transações](#transaction-mapping). Quando uma métrica como `http.server.request.duration` ou `rpc.server.call.duration` inclui um atributo `error.type`, esse ponto de dados contribui para a taxa de erros. Remover o atributo `error.type` de um ponto de dados de métrica evita que ele seja contado como um erro.

<Callout variant="important">
  O atributo `error.type` não existia em versões anteriores das convenções semânticas do OpenTelemetry. Para oferecer suporte a instrumentação mais antiga que segue versões anteriores das convenções semânticas HTTP e RPC, a New Relic aproveita os atributos `http.status_code` e `rpc.grpc.status_code` para gerar a taxa de erros. Esses atributos estavam presentes nas métricas `http.server.duration` e `rpc.server.duration` mais antigas.
</Callout>

#### Erro ignorado na Errors Inbox

O Errors Inbox é orientado por dados de span. Um span é classificado como um erro quando seu status é definido como `ERROR` (ou seja, `otel.status_code = ERROR`). Remover a definição do status do span para que ele não seja mais `ERROR` impede que o span apareça no Errors Inbox.

#### Exemplo: transformar dados com o OpenTelemetry Collector

O [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) pode ser usado para transformar dados de métrica e span em trânsito para suprimir erros específicos. O [exemplo de erro ignorado](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors) fornece uma configuração funcional do coletor demonstrando como:

* Remova o atributo `error.type` dos pontos de dados de métricas para excluí-los da taxa de erros
* Remova o status do span para evitar que spans de erro apareçam no Errors Inbox
