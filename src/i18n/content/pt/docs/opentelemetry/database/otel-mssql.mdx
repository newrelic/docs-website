...
Você pode usar o NRDOT Collector para monitorar seus bancos de dados Microsoft SQL Server. O coletor NRDOT é uma distribuição pré-configurada que inclui componentes específicos da New Relic.

Você pode configurar o NRDOT Collector usando os seguintes métodos:

* [Instalação guiada](/docs/opentelemetry/database/otel-mssql/#guided): os métodos de instalação guiada fornecem instruções passo a passo na plataforma New Relic para ajudar você a instalar e configurar o NRDOT Collector para o monitoramento do SQL Server.
* [Instalação manual](/docs/opentelemetry/database/otel-mssql/#manual): o método de instalação manual permite configurar o NRDOT Collector por conta própria seguindo as instruções nesta documentação. Este método é ideal para usuários que preferem uma abordagem prática ou têm requisitos específicos de configuração.

<Tabs>
  <TabsBar>
    <TabsBarItem id="guided">Instalação guiada</TabsBarItem> <TabsBarItem id="manual">Instalação manual</TabsBarItem>
  </TabsBar>
  ...
  <TabsPages>
    <TabsPageItem id="guided">
      ...
      7. Após a instalação, na tela <DNT>Test your integration</DNT>, clique em **Testar conexão** para verificar se o NRDOT Collector consegue se conectar à sua instância do SQL Server e enviar dados para o New Relic. Se o teste for bem-sucedido, você deve ver uma mensagem de confirmação.

      ### Configurar a correlação entre APM e banco de dados [#optional-service-id]
      ...
      <Steps>
        ...
        <Step>
          ...
          * **Linux:** `/etc/nrdot-collector-host/config.yaml`
          ...
          <Callout variant="important">
            Se você ativar as alternâncias de categoria de métrica principal e notar lacunas de dados, aumente o `timeout` e o `collection_interval` para dar aos scrapers mais tempo para concluir. Mantenha sempre o valor do tempo limite menor que o `collection_interval` para evitar ciclos de coleta sobrepostos. Por exemplo:
