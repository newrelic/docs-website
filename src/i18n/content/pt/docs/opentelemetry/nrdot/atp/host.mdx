...
## Configuração de armazenamento [#storage-config]
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Habilite o armazenamento persistente. Os caminhos de armazenamento são determinados automaticamente com base no contexto de execução:

            * **Serviço systemd do Linux**: `/var/lib/nrdot-collector/adaptiveprocess.db` (gerenciado automaticamente pelo systemd)
            * **Binário manual do Linux**: `~/.local/state/nrdot-collector/adaptiveprocess.db` (padrão XDG, não requer root)
            * **Windows**: `%LOCALAPPDATA%\nrdot-collector\adaptiveprocess.db`

            Se nenhum caminho gravável estiver disponível, o armazenamento é desabilitado automaticamente e o ATP opera apenas com estado em memória (a retenção funciona durante o tempo de execução, mas o estado é perdido nas reinicializações).
          </td>
        </tr>
