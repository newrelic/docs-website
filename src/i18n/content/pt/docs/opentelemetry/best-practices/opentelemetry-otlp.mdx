...
<Callout variant="important">
  Embora a definição de atributo padrão seja geralmente usada, os atributos de registro log são excepcionais e oferecem suporte a valores complexos (por exemplo, O tipo de atributo do registro log é [`map<string, any>`](https://opentelemetry.io/docs/specs/otel/logs/data-model/#field-attributes)). Apesar disso, New Relic atualmente suporta apenas atributos de registro log que estejam em conformidade com a definição de atributo padrão.
</Callout>
