...
Sua infraestrutura hospedada na AWS pode ser monitorada com a integração do New Relic com o [Amazon CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/). Esta não é uma solução OpenTelemetry para o monitoramento da sua infraestrutura. No entanto, o New Relic oferece suporte a várias relações entre serviços instrumentados pelo OpenTelemetry e a infraestrutura monitorada usando o Amazon CloudWatch Metric Streams. As relações suportadas estão listadas abaixo.
