...
Esta página descreve como New Relic lida com OpenTelemetry as métricas que recebe por meio do [New Relic OTLP do endpoint](/docs/opentelemetry/best-practices/opentelemetry-otlp). A integração da métrica OpenTelemetry com New Relic requer a configuração da origem da métrica para exportar para este endpoint . Revise os requisitos de configuração endpoint . Para informações sobre configuração de serviços com métrica OpenTelemetry veja [OpenTelemetry APM monitoramento](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-intro).
