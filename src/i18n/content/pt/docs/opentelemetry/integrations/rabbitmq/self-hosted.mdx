...
<Steps>
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="software-reqs" title="Requisitos de software">
        * **OpenTelemetry Collector**: Contrib versão 0.140.0 ou superior
  ...
  <Step>
    ### Instale o OpenTelemetry Collector [#install-collector]
  ...
  <Step>
    ### Configurar o coletor [#configure]
  ...
  <Step>
    ### Verifique seus dados no New Relic [#verify]
...
<CollapserGroup>
  <Collapser id="no-data" title="Nenhum dado aparecendo no New Relic">
    ...
    **5. Verifique sua chave de licença:**
    ...
    Certifique-se de que `NEWRELIC_LICENSE_KEY` esteja definido corretamente e não tenha espaços extras ou aspas.
  </Collapser>
  ...
  <Collapser id="connection-refused" title="Erros de conexão recusada">
    ...
    **Verifique a configuração do endpoint:**\
    Certifique-se de que o `endpoint` em `/etc/otelcol-contrib/config.yaml` corresponda à sua configuração do RabbitMQ (hostname, porta).
  </Collapser>
  ...
  <Collapser id="auth-failures" title="Falhas de autenticação (401 Não autorizado)">
    ...
    **Verifique as credenciais:**\
    Verifique se o nome de usuário e a senha em `/etc/otelcol-contrib/config.yaml` correspondem ao seu usuário do RabbitMQ.
