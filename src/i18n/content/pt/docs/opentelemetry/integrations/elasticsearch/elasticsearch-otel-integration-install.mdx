...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="nrdot">
      <Steps>
        <Step>
          ...
          * Se não estiver instalado, siga o [guia oficial de instalação do NRDOT](/docs/opentelemetry/nrdot/nrdot-collector/) para instalar o coletor usando o gerenciador de pacote (DEB/RPM) que configura o serviço systemd automaticamente
    ...
    <TabsPageItem id="prometheus">
      Use esta abordagem se você já tiver um [Prometheus Elasticsearch exporter](https://github.com/prometheus-community/elasticsearch_exporter) em execução no seu ambiente, ou se estiver migrando de uma stack de monitoramento baseada no Prometheus.
      ...
      <Steps>
        <Step>
          ### Instale e configure o exportador do Elasticsearch [#install-exporter]

          O [exportador do Elasticsearch para Prometheus](https://github.com/prometheus-community/elasticsearch_exporter) expõe métricas do Elasticsearch no formato Prometheus na porta `9114`.
        ...
        <Step>
          ...
          * **OTel Collector Contrib:** `/etc/otelcol-contrib/config.yaml`
          ...
          <Collapser id="prometheus-full-config" title="Configuração completa do receptor Prometheus">
            ```yaml
            receivers:
        ...
        <Step>
          ### Configurar variáveis de ambiente [#env-vars-prom]
