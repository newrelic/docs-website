...
Esta página documenta as métricas coletadas ao monitorar o Apache Kafka com o OpenTelemetry. As métricas são coletadas via [Kafka metrics receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver), métricas JMX (do OpenTelemetry Java Agent ou Prometheus JMX Exporter) e [OpenTelemetry Java agent](https://opentelemetry.io/docs/languages/java/automatic/) para métricas do lado do cliente.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Número de réplicas em sincronia para uma partição. Esta métrica é filtrada pelo pipeline do coletor — apenas o `kafka.partition.replicas_in_sync.total` agregado por tópico é mantido.
          </td>
...
* **Kafka auto-hospedado**: [agente Java do OpenTelemetry](/docs/opentelemetry/integrations/kafka/self-hosted#java-agent) ou [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/self-hosted#prometheus)
* **Kubernetes (autogerenciado)**: [agente Java do OpenTelemetry](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#java-agent) ou [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#prometheus)
* **Kubernetes (Strimzi)**: [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-strimzi#configure-jmx-exporter) com configuração personalizada do New Relic

Ambos os métodos coletam o mesmo conjunto de métricas de broker Kafka e JVM documentadas abaixo:

<CollapserGroup>
  <Collapser id="jmx-kafka-metrics" title="Métricas do broker Kafka">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            A contagem de eleição do líder. Aparece apenas quando ocorre uma eleição de líder; não é emitido em clusters estáveis.
          </td>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Tamanho da fila de requisições no broker
          </td>
...
## Métricas do cliente Kafka [#kafka-client-metrics]
...
<CollapserGroup>
  <Collapser id="producer-metrics" title="Métricas do produtor">
    ...
    | `kafka.producer.incoming_byte_rate`        | Taxa de bytes de entrada (respostas dos brokers)  | client-id, node-id |
    | `kafka.producer.incoming_byte_total`       | Total de bytes de entrada (respostas dos brokers) | client-id, node-id |
  ...
  <Collapser id="consumer-metrics" title="Métricas do consumidor">
    ...
    | `kafka.consumer.partition_assigned_latency_avg` | Latência média de atribuição de partição (ms). Emitido apenas durante rebalanceamentos do grupo de consumidores.  | client-id |
    | `kafka.consumer.partition_assigned_latency_max` | Latência máxima de atribuição de partição (ms). Emitido apenas durante rebalanceamentos do grupo de consumidores. | client-id |
    | `kafka.consumer.partition_revoked_latency_avg`  | Latência média de revogação de partição (ms). Emitido apenas durante rebalanceamentos do grupo de consumidores.   | client-id |
    | `kafka.consumer.partition_revoked_latency_max`  | Latência máxima de revogação de partição (ms). Emitido apenas durante rebalanceamentos do grupo de consumidores.  | client-id |
    | `kafka.consumer.partition_lost_latency_avg`     | Latência média de perda de partição (ms). Emitido apenas durante rebalanceamentos de grupos de consumidores.      | client-id |
    | `kafka.consumer.partition_lost_latency_max`     | Latência máxima de perda de partição (ms). Emitido apenas durante rebalanceamentos do grupo de consumidores.      | client-id |
...
* [Consulte e visualize seus dados](/docs/opentelemetry/integrations/kafka/find-and-query-data) - Encontre métricas na interface do New Relic, escreva consultas NRQL, crie dashboards e configure alertas
* [Consultar tipos de dados de métricas](/docs/data-apis/understand-data/metric-data/query-metric-data-type/) - Aprenda técnicas avançadas para trabalhar com métricas do OpenTelemetry