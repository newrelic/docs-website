...
* **[Receptor JMX](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver)**: Conecta-se à porta JMX (normalmente 9999) para coletar métricas detalhadas do broker e métricas da JVM
...
## Métricas do coletor JMX [#jmx-metrics]
...
1. **Sistema de destino Kafka padrão** - Métricas específicas do Kafka integradas de `target_system: kafka` ([kafka.yaml](https://github.com/open-telemetry/opentelemetry-java-contrib/blob/main/jmx-scraper/src/main/resources/kafka.yaml))
...
<CollapserGroup>
  <Collapser id="jmx-cluster-metrics" title="Métricas no nível do cluster">
  ...
  <Collapser id="jmx-broker-metrics" title="Métricas do broker">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
...
Essas métricas Kafka adicionais e métricas JVM são coletadas ao usar um arquivo de configuração JMX personalizado (conforme mostrado na [configuração self-hosted](/docs/opentelemetry/integrations/kafka/self-hosted#jmx-config)). A configuração personalizada permite que você colete métricas específicas do Kafka adicionais além do conjunto padrão, bem como métricas de integridade da JVM.

<CollapserGroup>
  <Collapser id="jmx-cluster-level-metrics" title="Métricas no nível do cluster">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-broker-specific-metrics" title="Métricas específicas do broker">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-per-topic-metrics" title="Métricas por tópico">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
...
* **[Consultar e visualizar seus dados](/docs/opentelemetry/integrations/kafka/find-and-query-data)** - Encontre métricas na interface do usuário da New Relic, escreva consultas NRQL, crie dashboards e configure alertas
* **[Consultar tipos de dados de métricas](/docs/data-apis/understand-data/metric-data/query-metric-data-type/)** - Aprenda técnicas avançadas para trabalhar com métricas OpenTelemetry