---
title: Solucionar problemas de monitoramento de contêineres Docker
tags:
  - Integrations
  - OpenTelemetry
  - Docker
...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="nrdot-troubleshooting">
      <Callout variant="tip">
        **Solução de problemas gerais do NRDOT**: Para problemas de inicialização do coletor, validação de configuração, conectividade de rede e diagnósticos de NrIntegrationError, consulte o [Guia de solução de problemas do NRDOT](https://github.com/newrelic/nrdot-collector-releases/blob/main/distributions/TROUBLESHOOTING.md). Os problemas abaixo são específicos do monitoramento de contêineres Docker.
      </Callout>

      <CollapserGroup>
        <Collapser id="nrdot-no-metrics" title="Nenhuma métrica do Docker aparecendo no New Relic">
          ...
          **Soluções**:
          ...
          1. **Acesso ao socket do Docker**: Adicione o usuário nrdot-collector ao grupo docker:
          ...
          2. **Versão da API do Docker**: Certifique-se de que a versão da API do Docker seja 1.25+:
        ...
        <Collapser id="nrdot-permission-errors" title="Erros de permissão negada">
        ...
        <Collapser id="nrdot-incomplete-metrics" title="Métricas de contêiner incompletas ou ausentes">
          **Problema**: Alguns contêineres exibem métricas enquanto outros não, ou as métricas são esporádicas.
          ...
          * **Limites de recursos**: Aumente os valores de tempo limite do coletor se os contêineres estiverem lentos para responder. Edite seu `/etc/nrdot-collector/docker-stats-config.yaml`:
            ```yaml
            receivers:
              docker_stats:
            ...
            ```
          * **Namespaces de rede**: Alguns contêineres em namespaces de rede restritos podem não estar acessíveis
        </Collapser>

        <Collapser id="nrdot-no-logs" title="A coleta de logs não está funcionando">
          ...
          **Soluções**:
          ...
          1. **Permissões de arquivo**: Conceda acesso ao diretório de logs:
          ...
          2. **Exposição de portas de contêiner**: Exponha portas para descoberta de contêineres:
          ...
          3. **Verifique a configuração de logs**: Certifique-se de que sua configuração de coleta de logs esteja incluída:
    ...
    <TabsPageItem id="otelcol-troubleshooting">
      <CollapserGroup>
        <Collapser id="collector-issues" title="Coletor não iniciando ou falhando">
          ...
          * **Variáveis de ambiente**: Verifique se `NEWRELIC_LICENSE_KEY` e `NEWRELIC_OTLP_ENDPOINT` estão definidas corretamente
        </Collapser>

        <Collapser id="no-metrics" title="Nenhuma métrica aparecendo no New Relic">
          ...
          **Soluções**:
          ...
          1. **Acesso ao socket do Docker**: Adicione o usuário do coletor ao grupo docker:
          ...
          2. **Problemas com a chave de licença**: Verifique se sua chave de licença está correta e tem as permissões adequadas:
          ...
          3. **Conectividade de rede**: Teste a acessibilidade do endpoint OTLP:
          ...
          4. **Versão da API do Docker**: Certifique-se de que a versão da API do Docker seja 1.25+:
        ...
        <Collapser id="permission-errors" title="Erros de permissão negada">
          **Problema**: Os logs do Coletor mostram &quot;permissão negada&quot; ao acessar o socket do Docker.
        ...
        <Collapser id="incomplete-metrics" title="Métricas de contêiner incompletas ou ausentes">
          **Problema**: Alguns contêineres exibem métricas enquanto outros não, ou as métricas são esporádicas.
        ...
        <Collapser id="performance-issues" title="Alto uso de memória ou CPU pelo coletor">
          ...
          * **Desative métricas custosas**: Desative as métricas por CPU se não forem necessárias:
          ...
          * **Configure o processamento em lote**: Otimize o processamento em lote para seu ambiente:
        ...
        <Collapser id="no-logs" title="A coleta de logs não está funcionando">
          **Problema**: Coleta de logs configurada, mas os logs do contêiner não aparecem no New Relic.
          ...
          **Soluções**:
          ...
          3. **Configuração do observador**: Verifique as configurações do observador do Docker:
        ...
        <Collapser id="wrong-account" title="Dados não aparecem na conta New Relic correta">
          ...
          * **Contas dos EUA**: Use `https://otlp.nr-data.net`
          * **Contas da UE**: Use `https://otlp.eu01.nr-data.net`
          * **Contas JP**: use `https://otlp.jp.nr-data.net`
          * **Verifique a chave de licença**: Certifique-se de que você está usando a chave de licença da conta correta
        </Collapser>
      </CollapserGroup>

      Para problemas persistentes, consulte o [guia de solução de problemas do OpenTelemetry Collector](https://opentelemetry.io/docs/collector/troubleshooting/) e a [documentação do OpenTelemetry da New Relic](/docs/opentelemetry/opentelemetry-introduction/).
    </TabsPageItem>
  </TabsPages>
</Tabs>
...
* **Voltar à configuração**: Retorne ao [guia de configuração do monitoramento do Docker](/docs/opentelemetry/integrations/docker-monitoring/self-hosted) para concluir sua configuração
* **Explore seus dados**: Saiba como consultar e visualizar suas métricas do Docker com o [guia de referência de métricas](/docs/opentelemetry/integrations/docker-monitoring/metrics-reference)
