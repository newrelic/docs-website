...
1. Configure um pequeno conjunto (-15–35) de agentes de não produção para enviar dados de telemetria para o gateway. Certifique-se de que esses agentes sejam representativos dos tipos de agentes e cargas de telemetria que você pretende enviar ao gateway em produção (por exemplo, New Relic Infrastructure, Java APM e Fluent Bit). Anote o número de agentes de cada um.
...
4. Extrapole linearmente com base no número de agentes que você configurou, no número de agentes que você espera executar no pico de produção e no uso da CPU na etapa 3. Por exemplo, se você estiver executando 25 agentes APM Java através do gateway e observar apenas 1 vCPU operando com 65% de carga, você deve esperar ser capaz de executar 200 agentes APM Java com `<=` 8 vCPUs.
...
<Callout variant="important">
  **Diferenças no modo de instalação:** as configurações são definidas no nível da frota e se aplicam a todos os clusters nessa frota. Se a sua frota utiliza a [instalação sem Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux), a página de configurações exibe os valores atuais como somente leitura para as configurações de dimensionamento e versão. No entanto, os dois botões de alternância de resolução de problemas (coletar logs de diagnóstico, ignorar regras de gateway) permanecem editáveis em ambos os modos. As alterações de configuração do Pipeline (regras, amostragem, transformações) continuam sendo implantadas automaticamente via `ConfigMap` em ambos os modos de instalação. Para frotas sem Flux, você gerencia o escalonamento pós-instalação via Horizontal Pod Autoscaler (HPA) e as versões via atualizações do Helm. Consulte [Gerenciando o Gateway sem Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway-after-installation) para obter instruções.
</Callout>
...
### Gerenciamento de versão do Gateway

* **Seleção de versão:** escolha qual versão do software de gateway (versão do chart do Helm) implantar em todos os clusters da frota. As versões disponíveis são exibidas em um dropdown. Para frotas instaladas com o Flux, selecione uma nova versão, clique em **Save settings** para criar um rascunho de implantação e, em seguida, implante o rascunho para atualizar todos os clusters da frota. Para aplicar as alterações, consulte [Implantar suas alterações](/docs/new-relic-control/pipeline-control/gateway/ui-guide#deploy-your-changes). Para frotas instaladas sem o Flux, o campo de versão é somente leitura. Você deve atualizar manualmente usando comandos do Helm, conforme descrito em [Gerenciando o Gateway sem o Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#upgrade-gateway-version).
