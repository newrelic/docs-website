...
* Declaração de versão: a versão: `2.0.1` especifica o esquema de configuração
...
2. Você implanta via interface (upload de YAML) na sua frota de gateways.
3. O Gateway aplica a configuração a todos os clusters de gateway na frota.
4. Todos os clusters na frota processam dados de forma idêntica usando as mesmas regras.

**Gerenciamento de versão:**

* Cada alteração de configuração cria uma nova versão (por frota)
* Visualize o histórico de versões e reverta se necessário.
* O selo &quot;Precisa de implantação&quot; mostra alterações pendentes.
* Diferentes frotas podem manter diferentes configurações e versões.

## Próximos passos

**Começar a usar a instalação do gateway:**

* [Instalar com Flux](/docs/new-relic-control/pipeline-control/gateway/set-up-and-deploy-pipeline-control) - Gerenciamento completo do ciclo de vida do gateway via interface (instalar, atualizar, escalar)
* [Instalar sem o Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux) - Modo apenas de configuração para permissões reduzidas
...
**Explore os tipos de processador:**
