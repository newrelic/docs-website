...
A UI do gateway do Pipeline Control fornece uma interface baseada em formulários para criar e gerenciar regras de gateway sem escrever YAML diretamente. A interface do usuário organiza a lógica de processamento em três categorias de regras: taxa de amostragem, filtro e transformação. Cada categoria é configurável por meio de diálogos modais guiados.
...
Crie regras de filtro baseadas em OTTL para filtrar condicionalmente registros de telemetria inteiros ou atributos específicos. Suporta:
...
8. Clique em **Save draft** para salvar sua versão de configuração recém-criada.
...
2. Na nova página de implantação, revise as informações pré-preenchidas:

   * Um nome e uma descrição padrão são fornecidos. Você pode editá-los para ajudar a acompanhar suas alterações.
   * A implantação inclui automaticamente os agentes associados à sua frota atual.
   * O sistema assume como padrão a sua versão de configuração. Se desejar, você pode alterar esses detalhes.

3. Clique em <DNT>**Start deployment**</DNT> para aplicar a configuração.

4. Você deve confirmar novamente que deseja iniciar a implantação. Clique em <DNT>**Start deployment**</DNT> novamente para confirmar.

Leva alguns minutos para que a implantação seja concluída.
