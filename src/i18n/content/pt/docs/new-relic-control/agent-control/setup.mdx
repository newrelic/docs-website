---
title: Configurar e instalar o Agent Control
...
Por padrão, o Helm chart do Agent Control utiliza uma instância embutida do Flux CD para gerenciar o ciclo de vida dos seus agentes no Kubernetes. Dependendo dos requisitos do seu ecossistema, você pode configurar o Controle do Agente para aproveitar uma instalação personalizada existente do Flux v2 ou ignorar totalmente os componentes de infraestrutura de entrega contínua.
...
**Requisitos e compatibilidade** Para usar uma instalação externa do Flux com o Agent Control, seu ambiente deve atender aos seguintes requisitos. Configurações que divergem destas especificações não são validadas.
...
**Configuração** Para habilitar este modo, você deve desabilitar explicitamente os componentes do Flux empacotados na configuração do Helm chart do Agent Control.
...
**Limites de suporte** Ao usar uma instalação externa do Flux, o suporte da New Relic limita-se ao software Agent Control (a geração de manifestos de configuração válidos). Neste modo, você deve considerar um esquema de responsabilidade compartilhada como o seguinte:
...
#### Instalar o Agent Control sem Flux [#install-without-flux]

Você pode instalar o Agent Control sem o Flux quando sua função se limitar a entregar configuração a agentes gerenciados e não a instalá-los ou atualizá-los. Neste modo, o Agent Control não pode gerenciar o ciclo de vida do agente — ele não instala ou atualiza agentes e apenas envia configurações via Controle de Agentes. Você mesmo é responsável por instalar e atualizar o Agent Control e qualquer agente gerenciado. O principal caso de uso é o [gateway do Controle de Pipeline](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux): o Controle de agente entrega as alterações de configuração do gateway do Controle de Pipeline.

Para instalar o Controle do agente sem o Flux, defina `agentControlCd.enabled` como `false` em seus valores do Helm. O Controle do agente não criará nem monitorará nenhum objeto do Flux.
...
<Callout variant="important">
  Sem o Flux, o Controle de Agente não pode instalar ou atualizar o agente de gateway do Controle de Pipeline — você é responsável por instalá-lo e atualizá-lo (por exemplo, com `helm upgrade`). O próprio chart de controle do agente também não é atualizável remotamente nesta configuração. Para atualizar o Controle do Agente, execute `helm upgrade` no mesmo arquivo de valores. Para que o Agent Control instale e atualize o agente do gateway para você, mantenha o Flux ativado — seja com a instalação empacotada ou com [sua própria instalação do Flux](#support-for-existing-flux-installations).
</Callout>

Quando você instala o gateway do Controle de Pipeline pela instalação guiada do New Relic, o arquivo de valores gerado já define `agentControlCd.enabled: false`. Você não precisa editá-lo manualmente.

<Callout title="Controle de acesso" variant="important">
  **Não são necessárias permissões extras de cluster:** como o Flux está desativado neste modo, você não precisa criar as amplas permissões de `ClusterRole` em todo o cluster exigidas para configurações personalizadas do Flux. O Controle do agente permanece completamente contido, precisando apenas de permissões básicas para ler `Secrets` e `ConfigMaps` dentro de seu próprio namespace `newrelic-agent-control`.
</Callout>
