...
* **Credenciais:** O Agent Control pode obter credenciais sensíveis de provedores de valores externos, como o Vault, injetando-as na configuração do agente em tempo de execução. Isso aumenta a segurança ao manter as credenciais fora da própria configuração.
...
* **Postura de segurança:** Padronize como as credenciais são obtidas de provedores de valores para centralizar o gerenciamento, automatizar rotações e reduzir a exposição.
...
## Definições de tipo de agente

Todo agente gerenciado pelo Agent Control é apoiado por um **tipo de agente**, uma especificação com versão que descreve como o Agent Control deve baixar, configurar e executar esse agente. O suporte a um novo agente sempre requer uma definição de tipo de agente; sem ela, o Agent Control não tem conhecimento de como gerenciá-lo.

Os tipos de agente são identificados por um nome qualificado e versão (por exemplo, `newrelic/com.newrelic.infrastructure:0.1.0`). Ao adicionar um subagente à configuração, faz-se referência a um tipo de agente; o Agent Control usa essa definição para buscar o artefato correto, aplicar os padrões de configuração e gerenciar o ciclo de vida do agente.

Para obter a lista completa de tipos de agente disponíveis, consulte [Tipos de agente](/docs/new-relic-control/agent-control/agent-types#agent-types).
...
Para informações detalhadas sobre as permissões específicas exigidas por cada agente, consulte [Permissões necessárias por tipo de agente](/docs/new-relic-control/agent-control/agent-types#required-permissions-per-agent-type).
