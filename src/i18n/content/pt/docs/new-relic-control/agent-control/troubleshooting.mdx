...
<CollapserGroup>
  ...
  <Collapser id="debug" title="Diagnosticar problemas com os logs do agente">
    ...
    * `error`
    ...
    3. If the `file` log is enabled, check the most recent log file in the directory from the `path` setting (files are dated, e.g. `2026-09-18.newrelic-agent-control.log`). Or use your preferred log troubleshooting tool such as `journalctl -u newrelic-agent-control`.
  </Collapser>
