...
<CollapserGroup>
  ...
  <Collapser id="k8s-helm-failure" title="Falha na liberação Helm">
    ...
    O agente Control requer uma credencial de autenticação válida para se conectar com segurança ao Controle de Agentes. Inicialmente, essa credencial é gerada automaticamente através da interface de instalação do agente Control e é representada pelos campos `systemIdentity.parentIdentity.clientId` e `systemIdentity.parentIdentity.clientSecret` no arquivo de valores. Por motivos de segurança, a credencial necessária para instalar o Agente Control expira após 12 horas.
...
<CollapserGroup>
  ...
  <Collapser id="debug" title="Diagnosticar problemas com os logs do agente">
    ...
    log:
      ...
      file:
        enabled: true
        # Add a custom path if needed, default directory: /var/log/newrelic-agent-control/agent-control/
        # Logs rotate daily, so the file name is prefixed with the current date, e.g. 2026-09-18.newrelic-agent-control.log
    ...
    * `error`
    ...
    3. Se o log `file` estiver habilitado, verifique o arquivo de log mais recente no diretório da configuração `path` (os arquivos são datados, por exemplo, `2026-09-18.newrelic-agent-control.log`). Ou use sua ferramenta de resolução de problemas de log preferida, como `journalctl -u newrelic-agent-control`.
  </Collapser>
    ...
    ```json
      ...
      "agents": {
        "nr-otel-collector": {
          ...
          "agent_start_time_unix_nano": 1730000000000000000,
          "health_info": {
            "healthy": true,
            "start_time_unix_nano": 1730000000000000000,
            "status_time_unix_nano": 1730000010000000000
          }
        },
        "nr-infra-agent": {
...
Visualize o arquivo de log mais recente (arquivos de log são nomeados com um prefixo de data, como `2026-02-09.newrelic-agent-control.log`):
...
Execute o Agent Control a partir da linha de comando para contornar problemas específicos do serviço. Ele lê sua configuração a partir do caminho de configuração local padrão (`local-data\agent-control\local_config.yaml`) automaticamente, portanto, nenhuma flag é necessária:
...
## Solucionar problemas de controle do agente no host com o NRDiag

A ferramenta de diagnóstico do New Relic `NRDiag` é um utilitário que coleta recursos e logs relacionados ao Controle do Agente no host para depuração. Isso se aplica a hosts Linux e Windows; a suíte correspondente detecta automaticamente seu SO e executa as verificações relevantes.

Siga estas etapas para coletar todos os dados:

1. No seu host, instale a ferramenta `NRDiag` usando o [guia de primeiros passos](/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started).

2. Execute a suíte de controle do agente no host:
...
3. A saída esperada deve ser semelhante ao seguinte relatório:
   ...
   [output] Check Results
   [output] -------------------------------------------------
   [output] Success  AgentControl/Config/Agent [AgentControl agent identified from binary: /usr/bin/newrelic-agent-control]
   [output] Success  AgentControl/Config/DirTree [Collected agent-control directory tree.]
   [output] Success  AgentControl/Agent/Connect [All agent-control endpoints reachable and identity key valid...]
   [output] Success  AgentControl/Agent/Logs [Collected agent-control logs from journalctl.]
   [output] Success  AgentControl/Agent/StatusServer [agent-control status server response: ...]
   [output] See nrdiag-output.json for full results.
   ```

   <Callout variant="tip" title="dica">
     No Windows, a tarefa `AgentControl/Agent/Logs` coleta os arquivos de log diretamente de `C:\ProgramData\New Relic\newrelic-agent-control\log\agent-control\` em vez de `journalctl`.
   </Callout>
