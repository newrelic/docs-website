...
Você pode definir um dos três status e filtrar sua caixa de entrada por status.

* `Unresolved`: este é o status padrão dos grupos de erros.
* `Resolved`: definir um erro como resolvido irá ocultá-lo da visualização padrão da caixa de entrada, a menos que os filtros sejam atualizados para incluir erros resolvidos. Se ocorrer um evento correspondente à impressão digital do grupo de erros após marcar um grupo de erros como resolvido, o status será redefinido automaticamente para `Unresolved`. Isso pode ser útil para identificar regressões.
* `Ignored`: isso ocultará o grupo de erros da visualização da caixa de entrada, a menos que os filtros sejam atualizados para incluir erros ignorados ou até que você pare de ignorar o grupo de erros.
