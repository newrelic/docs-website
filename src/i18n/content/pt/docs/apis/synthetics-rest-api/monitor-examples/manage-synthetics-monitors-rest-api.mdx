...
Esta API pode ser usada para todos os monitores Sintético. (Métodos API adicionais para [navegador com script e monitor de teste de API](#scripted-api-monitors-api) também estão disponíveis para atualizar o script associado a esses monitores.) Todos os dados de monitoramento sintético estão disponíveis através da API. Exemplos de API mostram o comando curl.
...
<CollapserGroup>
  ...
  >
    ...
    Além disso, para <DoNotTranslate>**add the script for a scripted monitor**</DoNotTranslate> por meio da chamada de API REST, um [endpoint adicional de API](#scripted-api-monitors-api) para enviar o script para o monitor recém-criado. Se você estiver usando localização privada com [execução de script verificada](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations) habilitada, consulte [locais de script com execução de script verificada](#scriptlocations).
  ...
  >
    ...
    Todos os campos são necessários. Porém, o `TYPE` do monitor <DoNotTranslate>**cannot**</DoNotTranslate> será alterado.
  ...
  <Collapser
    className="freq-link"
    id="patch-monitor"
    title="Patch de um monitor existente"
  >
...
## API de script para navegador com script e monitor de teste de API [#scripted-api-monitors-api]

Além da API geral, existem vários métodos de API para o navegador com script (`SCRIPT_BROWSER`) e navegador de teste de API (`SCRIPT_API`). Esses exemplos mostram o comando curl.
...
<CollapserGroup>
  ...
  <Collapser
    id="scriptlocations"
    title="Usando scripts de localização privada com execução script verificada"
  >
