...
Você pode criar um monitor de API com script usando a mutação <DNT>`syntheticsCreateScriptApiMonitor`</DNT>. Essa mutação permite configurar testes API personalizados que executam seu código JavaScript para validar os endpoints da API.
...
Você pode atualizar um monitor de API com script existente usando a mutação <DNT>`syntheticsUpdateScriptApiMonitor`</DNT>. Isso permite modificar a configuração de um monitor de API com script que já foi criado.
...
Quando um monitor de API com script não for mais necessário, você poderá removê-lo permanentemente usando a mutação <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Consulta de monitores de API com script [#query-scripted-api-monitors]

Use o NerdGraph para recuperar metadados, scripts, IDs de mapa ou verificar o status dos seus monitores de API com script.
