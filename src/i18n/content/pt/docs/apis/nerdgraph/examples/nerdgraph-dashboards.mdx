...
### Atualizar widgets em uma página [#update-widgets-in-page]
...
#### Links e descrições do widget [#widget-links-descriptions]

Você também pode usar `dashboardUpdateWidgetsInPage` para adicionar links e descrições aos widgets. Os links de widgets aparecem como links clicáveis nos títulos dos widgets, e as descrições dos widgets são exibidas como dicas de ferramenta quando os usuários apontam para o widget.

<Callout variant="important">
  URLs de link de widget devem usar os protocolos `http://` ou `https://` e não podem exceder 2.048 caracteres.
</Callout>

<CollapserGroup>
  <Collapser id="update-widget-link-description" title="Atualizar o link e a descrição do widget">
    ```graphql
    mutation UpdateWidgetsInPage($pageGuid: EntityGuid!, $widgets: [DashboardUpdateWidgetInput!]!) {
      dashboardUpdateWidgetsInPage(guid: $pageGuid, widgets: $widgets) {
        errors {
  ...
  <Collapser id="remove-widget-link-description" title="Remover link e descrição do widget">
    Para remover um link ou descrição, defina o campo como `null`:
...
### Visibilidade da variável de template [#variable-visibility]

Você pode ocultar variáveis de template da barra de variáveis no modo de Visualização enquanto elas continuam funcionando normalmente. Use o campo `hiddenOnVariablesBar` no objeto `options` da variável.

* **Nome do campo:** `hiddenOnVariablesBar`
* **Tipo:** Booleano
* **Padrão:** `false` (as variáveis estão visíveis)

<CollapserGroup>
  <Collapser id="create-hidden-variable" title="Criar um dashboard com uma variável oculta">
    ```graphql
    mutation {
      dashboardCreate(
  ...
  <Collapser id="update-variable-visibility" title="Atualize uma variável para ocultá-la ou exibi-la">
    Para atualizar a visibilidade de uma variável, inclua a configuração completa do dashboard com o valor `hiddenOnVariablesBar` atualizado:
...
Para mais informações sobre a visibilidade de variáveis de modelo, consulte [Mostrar e ocultar variáveis](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility).
