...
* `container`:
  ...
  * `id`: (String) O identificador exclusivo da sua organização. Substitua organização pelo seu ID de organização real.
  * `type`: (String) O tipo de contêiner. Atualmente, o único tipo suportado é &amp;quot;organização&amp;quot;.
  * `name`: (String) O nome atribuído à função personalizada. Exemplo: &amp;quot;MINHA FUNÇÃO PERSONALIZADA&amp;quot;.
...
* `scope`: (String) O nível no qual as permissões da função se aplicam. Nesta instância, o escopo é &amp;quot;CONTA&amp;quot;.
