---
title: 'Exportação de dados de streaming: envie seus dados para um AWS Kinesis Firehose ou Azure Event Hub'
tags:
  - APIs
  - NerdGraph
...
### Etapa 1. Crie um Firehose para exportação de streaming [#create-firehose]

Crie um Firehose dedicado para transmitir seus dados do New Relic para:

1. Acesse Amazon Kineses Data Firehose.
...
4. Use
   ...
   <DoNotTranslate>**Direct PUT or other sources**</DoNotTranslate>

   e especifique um destino compatível com o formato de evento JSON do New Relic (por exemplo, S3, Redshift ou OpenSearch).

### Etapa 2. Criar política de acesso de gravação do IAM Firehose [#create-policy]

1. Acesse o IAM e selecione
...
### Etapa 3. Criar função IAM para conceder acesso de gravação ao New Relic [#iam-role]

Para configurar a função do IAM:

1. Navegue até o IAM e clique em

   <DoNotTranslate>**Roles**</DoNotTranslate>

   .

2. Crie uma função para uma conta da AWS e selecione

   <DoNotTranslate>**for another AWS account**</DoNotTranslate>
...
4. Selecione
   ...
   <DoNotTranslate>**Require external ID**</DoNotTranslate>

   e insira o [ID](/docs/accounts/accounts-billing/account-structure/account-id) da conta New Relic da qual você deseja exportar.

5. Clique em

   <DoNotTranslate>**Permissions**</DoNotTranslate>

   e selecione a política que você criou acima.

6. Adicione um nome de função (que será usado no registro de exportação) e uma descrição.

7. Crie a função.

Quando isso for feito, você poderá configurar suas regras de exportação usando o NerdGraph. Para saber mais sobre isso, continue lendo.
...
### Etapa 1. Crie um namespace de hubs de eventos [#create-event-hubs-namespace]
...
4. Depois que seu namespace de Event Hubs for implantado, clique em

   <DoNotTranslate>**Go to resource**</DoNotTranslate>

   .

### Passo 2. Crie um Event Hub [#create-event-hub]

1. Na coluna da esquerda, clique em

   <DoNotTranslate>**Event Hubs**</DoNotTranslate>

   .

2. Em seguida, clique em

   <DoNotTranslate>**+Event Hub**</DoNotTranslate>

   para criar um Event Hub.
...
4. Assim que o Event Hub for criado, clique no Event Hub.

### Passo 3. Crie e anexe uma política de acesso compartilhado [#event-hub-policy]

1. Na coluna da esquerda, vá para

   <DoNotTranslate>**Shared access policies**</DoNotTranslate>
...
2. Clique em
   ...
   próximo ao topo da página.

3. Escolha um nome para sua política de acesso compartilhado.

4. Marque

   <DoNotTranslate>**Send**</DoNotTranslate>
...
5. Clique na política criada e copie o

   <DoNotTranslate>**Connection string–primary key**</DoNotTranslate>

   . Salve isto, pois usaremos isso para autenticar e enviar dados para o seu Event Hub.
...
## Compactação de exportação [#compression]

Opcionalmente, podemos compactar cargas antes de serem exportadas, embora isso esteja desabilitado por padrão. Isso pode ajudar a evitar atingir o limite de dados ingeridos e economizar dinheiro em custos de entrada.
...
<CollapserGroup>
  <Collapser
    id="collapser-1"
    title="Ponto de acesso do Object Lambda"
  >
    ...
    4. [Crie um ponto de acesso do Object Lambda](https://docs.aws.amazon.com/AmazonS3/latest/userguide/olap-create.html). Este ponto de acesso deve ter estas configurações:
       ...
       * O
         ...
         <DoNotTranslate>**Supporting Access Point**</DoNotTranslate>

         neste ponto de acesso Lambda precisará ser definido para o ponto de acesso que você configurou no bucket S3.

       * Em

         <DoNotTranslate>**Transformation Configuration**</DoNotTranslate>

         :

       * A caixa

         <DoNotTranslate>**GetObject**</DoNotTranslate>
  ...
  <Collapser
    id="collapser-2"
    title="Função de configuração de metadados do Lambda"
  >
    ...
    * Esta função deve ter uma política com estas permissões: `s3:PutObject` e `s3:GetObject`.
    ...
    4. Clique em <DoNotTranslate>**Create function**</DoNotTranslate>.

    5. Selecione o ambiente de tempo de execução Java 11.

    6. Clique em <DoNotTranslate>**Change default execution role > Use an existing role**</DoNotTranslate>. Insira aqui a função que você criou na etapa 2.

    7. Role para baixo e clique em <DoNotTranslate>**Create function**</DoNotTranslate>.

    8. Depois que a função for criada, clique em <DoNotTranslate>**Upload from**</DoNotTranslate> e selecione <DoNotTranslate>**.zip or .jar file**</DoNotTranslate> no dropdown.

    9. Clique em <DoNotTranslate>**Upload**</DoNotTranslate> na caixa exibida e selecione o arquivo ZIP criado na etapa 1.

    10. Quando o upload terminar, clique em <DoNotTranslate>**Save**</DoNotTranslate> para sair da caixa pop-up.

    11. Edite <DoNotTranslate>**Runtime settings**</DoNotTranslate> adicionando o manipulador. Em nossa função fornecida, o manipulador é <DoNotTranslate>`metadatasetter.App::handleRequest`</DoNotTranslate>.

    12. Tudo o que resta fazer agora é habilitar esta função do Lambda para ser acionada na criação do objeto S3. Clique em <DoNotTranslate>**Add trigger**</DoNotTranslate> para começar a configurar isso.

    13. No dropdown, selecione <DoNotTranslate>**S3**</DoNotTranslate> como sua fonte.

    14. Insira o nome do bucket S3 ao qual você deseja aplicar os metadados no campo <DoNotTranslate>**Bucket**</DoNotTranslate> .

    15. Remova o <DoNotTranslate>**All object create events**</DoNotTranslate> padrão dos tipos de eventos. Na dropdown de tipos de eventos, selecione <DoNotTranslate>**PUT**</DoNotTranslate>.

    16. Marque a caixa <DoNotTranslate>**Recursive invocation**</DoNotTranslate> e clique em <DoNotTranslate>**Add**</DoNotTranslate> no canto inferior direito.
...
2. No painel esquerdo do seu trabalho de análise de streaming, clique em

   <DoNotTranslate>**Inputs**</DoNotTranslate>

   e, em seguida, clique na entrada que você configurou.
...
5. Clique em
   ...
   <DoNotTranslate>**Query**</DoNotTranslate>

   na lateral da tela. Usando a guia

   <DoNotTranslate>**Input preview**</DoNotTranslate>

   , agora você poderá consultar o hub de eventos nesta tela.