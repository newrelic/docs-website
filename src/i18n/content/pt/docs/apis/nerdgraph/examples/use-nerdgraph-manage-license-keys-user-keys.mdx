...
Este documento contém tutoriais sobre como usar [o NerdGraph](/docs/introduction-new-relic-graphql) para gerenciar programaticamente algumas chaves de API do New Relic: a chave de licença, a chave do navegador e o <InlinePopover type="userKey"/>. Para informações gerais sobre nossas chaves, veja [chave de API](/docs/apis/get-started/intro-apis/types-new-relic-api-keys).
...
* Chaves de ingestão de dados, incluindo:
  ...
  * O
    ...
    : necessário para a ingestão da maioria dos dados para o New Relic, exceto para dados
  ...
  * [Chave do navegador](/docs/browser/new-relic-browser/configuration/copy-browser-monitoring-license-key-app-id): necessária para a ingestão de dados de monitoramento do navegador

Um caso de uso comum desse recurso é a capacidade de alternar chaves para fins de segurança. Observe que você não pode usar esta funcionalidade do NerdGraph para excluir a chave de licença ou a chave do navegador que foi originalmente criada com uma conta; essas chaves originais não podem ser excluídas. Você só pode criar chaves de licença adicionais e gerenciar aquelas que você criou.
...
* Em relação à chave de licença:
  ...
  * Chave de licença são categorizadas pelo NerdGraph como

    <DoNotTranslate>**ingest keys**</DoNotTranslate>

    . Isso ocorre porque seu principal uso é permitir a ingestão de dados.
...
<CollapserGroup>
  <Collapser
    ...
    title="Crie uma chave de usuário"
  >
