...
Você pode criar um monitor de navegador simples usando a mutação <DNT>`syntheticsCreateSimpleBrowserMonitor`</DNT>.
...
Você pode atualizar um monitor de navegador simples existente usando a mutação <DNT>`syntheticsUpdateSimpleBrowserMonitor`</DNT>. Isso permite modificar a configuração de um monitor de navegador simples que já foi criado.
...
Quando um monitor de navegador simples não for mais necessário, você poderá removê-lo permanentemente usando a mutação <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Consultar monitor de browser simples [#query-simple-browser-monitors]

Use o NerdGraph para recuperar metadados, mapear IDs ou verificar o status dos seus monitores de browser simples.
