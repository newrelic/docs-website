...
Você pode criar um monitor de links quebrados usando a mutação <DNT>`syntheticsCreateBrokenLinksMonitor`</DNT>. Essa mutação permite configurar o monitoramento de links quebrados em qualquer página da web.
...
Você pode atualizar um monitor de links quebrados existente usando a mutação <DNT>`syntheticsUpdateBrokenLinksMonitor`</DNT>. Isso permite modificar a configuração de um monitor de links quebrados que já foi criado.
...
Quando um monitor de links quebrados não for mais necessário, você poderá removê-lo permanentemente usando a mutação <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Consultar monitores de links quebrados [#query-broken-links-monitors]

Use o NerdGraph para recuperar metadados, mapear IDs ou verificar o status dos seus monitores de links quebrados.
