...
* `configuration`: A [configuração tipada](#widget-typed) do widget. É uma representação GraphQL convenientemente tipada dos widgets mais comuns: [área](#area), [barras](#bar), [billboard](#billboard), [linha](#line), [markdown](#markdown), [pizza](#pie) e [tabela](#table).
* `rawConfiguration`: A [configuração não tipada](#widget-untyped) do widget. É uma forma livre (escalar JSON) de armazenar propriedades de widget não cobertas pela configuração tipada, como [mapa de calor](#heatmap), [histograma](#histogram), [bullet](#bullet), [funil](#funnel), etc. Neste caso, `configuration` é nulo e `rawConfiguration` contém todos os dados necessários para o widget.
* `description`: Texto opcional que fornece contexto adicional sobre o widget, exibido como uma dica de ferramenta quando os usuários apontam para o widget.
* `link`: URL opcional que torna o título do widget clicável, permitindo a navegação para dashboards relacionados, documentação ou recursos externos. Deve usar o protocolo `http://` ou `https://` e não pode exceder 2.048 caracteres.
