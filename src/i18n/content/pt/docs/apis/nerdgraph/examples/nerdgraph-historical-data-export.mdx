---
title: 'Exportação de dados históricos: retorne respostas NRQL maiores'
tags:
  - APIs
  - NerdGraph
  - Examples
...
* [Dados FedRAMP](/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp-moderate/)
...
Os seguintes [componentes NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions) <DoNotTranslate>**cannot**</DoNotTranslate> serão usados na consulta:

* Funções de agregação (`sum`, `count`, `average`, `max`)
* Funções avaliáveis (`numeric`, `log`, `concat`) são suportadas _apenas_ na cláusula `WHERE`
* `FACET`
* `TIMESERIES`
* `COMPARE WITH`
* `JOIN`
...
* O endpoint
  ...
  permite que um usuário especifique o ID da conta e o NRQL que deseja executar como uma exportação.

* O endpoint

  <DoNotTranslate>**get details for export**</DoNotTranslate>

  permite que um usuário especifique um ID de conta e um ID de exportação (encontrado no corpo da resposta do endpoint de criação) e use-os para recuperar o status da exportação. Quando a exportação for concluída, os resultados, na forma de um ou mais URLs de download, poderão ser encontrados na resposta deste endpoint.

* O endpoint

  <DoNotTranslate>**get details for account exports**</DoNotTranslate>

  permite que um usuário especifique um ID de conta e recupere os detalhes de todas as exportações ativas e não expiradas dessa conta.
...
Pode ser útil consultar o histórico de exportações de dados de forma programática, a partir de um script. O seguinte comando curl pode ser útil para começar. Para exemplos de respostas, consulte as respostas na seção Exemplo de consulta.
