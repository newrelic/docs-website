...
<Callout variant="tip">
  ...
  Para saber mais sobre perda de sinal, preenchimento de lacunas e como solicitar acesso a esses recursos, consulte [esta publicação do Fórum de suporte](https://support.newrelic.com/s/hubtopic/aAX8W0000008b5p/announcing-new-relic-one-streaming-alerts-for-nrql-conditions).
</Callout>
...
* `expiration.ignoreOnExpectedTermination`: Se `true`, um incidente não será aberto quando se espera que o sinal seja perdido. Para indicar que se espera que um sinal seja perdido, a tag `termination: expected` deve estar presente na entidade. O padrão é `false`. Para entidades de host de infraestrutura, a tag `hostStatus: shutdown` também indicará que o sinal deve parar e impedirá a abertura de um evento de alerta.
