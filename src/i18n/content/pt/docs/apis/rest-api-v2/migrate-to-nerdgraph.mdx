...
Esta seção aborda a migração do registro de implantação de ambas as APIs REST que chegarão ao fim da vida útil em 31 de julho de 2027:

* Os endpoints de implantação da API REST v2 (`https://api.newrelic.com/v2/applications/{app_id}/deployments.json`), mostrados abaixo.
* A API de implantação v0 legada (`deployments.xml`), disponibilizada em ambos os hosts `rpm` e `api` em cada região — `https://rpm.newrelic.com/deployments.xml` e `https://api.newrelic.com/deployments.xml` (EUA), além de seus equivalentes `.eu.`.

Ambas migram para o Monitoramento de Alterações do NerdGraph. Registre a implantação com a mutação `changeTrackingCreateEvent` e consulte-a com NRQL.

<Callout variant="tip">
  Não sabe ao certo onde ainda registra a implantação por meio dessas APIs REST? O Monitoramento de Alterações adiciona um atributo `newrelic.source` a cada evento, para que você possa encontrar o uso existente de REST em todo o seu ambiente a partir da interface da New Relic ou com NRQL. Veja [como suas alterações são registradas em todo o seu ambiente](/docs/change-tracking/view-analyze-data/#recording-source) na documentação do Monitoramento de Alterações.
</Callout>

<Callout variant="tip">
  A mutação NerdGraph `changeTrackingCreateDeployment` mais antiga é uma mutação separada, não REST, que **não** faz parte deste fim de vida e continua funcionando. A New Relic recomenda `changeTrackingCreateEvent` para novos trabalhos porque aceita `entitySearch` e oferece suporte a atributos personalizados, categorias e tipos. Consulte [Rastrear alterações usando o NerdGraph](/docs/change-tracking/config/nerdgraph/) para ambas as mutações.
</Callout>
...
<Callout variant="tip">
  Como algumas outras operações do NerdGraph, `changeTrackingCreateEvent` aceita um `entitySearch`, tornando-o o caminho de migração mais fácil a partir da API REST. Você pode pesquisar por `name`, bem como por `entityGuid`, se o tiver. Se a sua integração REST existente tiver apenas o ID do aplicativo numérico, você pode resolver a entidade diretamente com `entitySearch: { query: "domainId = 'YOUR_APP_ID' AND domain = 'APM'" }` — sem necessidade de pesquisa de GUID. Consulte o [Exemplo de implantação por ID do aplicativo](/docs/change-tracking/config/nerdgraph/#deployment-by-appid) e [Rastrear alterações usando o NerdGraph](/docs/change-tracking/config/nerdgraph/) para ver todos os campos disponíveis.
</Callout>
...
Para obter mais informações, consulte [Rastrear alterações usando o NerdGraph](/docs/change-tracking/config/nerdgraph/).
