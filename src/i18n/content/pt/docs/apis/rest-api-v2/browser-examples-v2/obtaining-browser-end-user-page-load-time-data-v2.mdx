---
title: Obtenção de dados de tempo de carregamento da página do navegador (usuário final) (v2)
tags:
  - APIs
  - REST API v2
  - Browser examples (v2)
...
Os [dados da métrica de fração de tempo](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) apresentados no gráfico <DoNotTranslate>**Browser page load time**</DoNotTranslate> na [página<DoNotTranslate>**Summary**</DoNotTranslate> ](/docs/browser/new-relic-browser/getting-started/browser-overview-website-performance-glance)do seu aplicativo dependerão da configuração do seu aplicativo web. Os possíveis componentes podem incluir:
...
Isto descreve como usar a API REST New Relic (v2) para obter os dados mostrados no gráfico <DoNotTranslate>**Browser page load time**</DoNotTranslate> .
...
## Tempo de renderização da página [#page_rendering_time]
...
## Tempo de processamento do DOM [#DOM_processing_time]

O `EndUser/RB:average_dom_content_load_time` é o tempo gasto no navegador para analisar e interpretar o HTML. Isso é medido pelo evento <DoNotTranslate>**DOM Content**</DoNotTranslate> do navegador.
...
## Hora do aplicativo da web [#web_application_response]
...
## Solicitar tempo de fila [#request_queue_time]
