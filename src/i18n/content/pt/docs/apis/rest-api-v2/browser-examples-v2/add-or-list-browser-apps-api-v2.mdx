...
Para visualizar um aplicativo de browser específico se você souber seu ID, substitua `${API_KEY}` pela [chave de API do New Relic](/docs/apis/rest-api-v2/requirements/api-keys#rest-api-key) e substitua `${ID}` pelo ID do aplicativo do browser no seguinte comando:
