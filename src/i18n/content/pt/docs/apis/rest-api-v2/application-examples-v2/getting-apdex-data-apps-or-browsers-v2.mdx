---
title: Obtendo dados Apdex para aplicativos ou navegador (v2)
tags:
  - APIs
  - REST API v2
...
Aqui estão alguns exemplos de como usar a API REST New Relic (v2) para obter dados [Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) para seu aplicativo e navegador, para um [ID do aplicativo](/docs/apm/apis/requirements/identification-code) e [chave de API](/docs/apis/rest-api-v2/requirements/rest-api-key#viewing) específicos. Por padrão, isso fornecerá uma lista de valores a cada minuto durante os [últimos 30 minutos](/docs/apm/apis/api-v2-examples/specifying-time-range-v2) no formato JSON.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        `score`: pontuação Apdex do navegador
      </td>
    </tr>

    <tr>
      <td>
        `s`: contagem de satisfeitos do navegador
      </td>
    </tr>

    <tr>
      <td>
        `t`: contagem tolerante do navegador
      </td>
    </tr>

    <tr>
      <td>
        `f`: contagem frustrada do navegador
      </td>
    </tr>

    <tr>
      <td>
        `count`: contagem de solicitações do navegador
      </td>
    </tr>
  </tbody>
</table>
...
Para obter o Apdex `score` [médio](/docs/apm/apis/requirements/calculating-average-metric-values-summarize) (aplicativo e navegador) para um [período específico](/docs/apm/apis/api-v2-examples/specifying-time-range-v2), use este comando. Este exemplo mostra 24 horas em formato XML para seu [ID do aplicativo](/docs/apm/apis/requirements/identification-code) e sua [chave de API](/docs/apis/rest-api-v2/requirements/rest-api-key#viewing) correspondente.
...
Isso retornará as mesmas pontuações Apdex mostradas na sua [página APM <DoNotTranslate>**Overview**</DoNotTranslate> ](/docs/apm/applications-menu/monitoring/applications-overview-dashboard).
...
<DoNotTranslate>**Tip:**</DoNotTranslate> Para retornar uma série de pontuações Apdex em vez de uma média, omita `&summarize=true`.
