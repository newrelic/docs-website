...
Para obter as taxas médias de transferência para um aplicativo não web de um único host, para um determinado período de tempo, use um único comando para obter a métrica `OtherTransaction`:`requests_per_minute` e especifique o numérico $&#x7B;HOST}. Para identificar o host, use a [interface ou a API REST](/docs/apis/rest-api-v2/requirements/listing-host-instance-application-server-ids#locating_host_id).
