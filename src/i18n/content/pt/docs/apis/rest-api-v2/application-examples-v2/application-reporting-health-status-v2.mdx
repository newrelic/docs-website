...
<DoNotTranslate>**Exception:**</DoNotTranslate> A API REST usa `orange` em vez de `yellow`. Se o aplicativo não estiver associado a uma política de alertas, ele será exibido como `unknown`.