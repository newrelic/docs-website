...
O tempo médio de resposta (milissegundos) é o valor que aparece no gráfico principal do seu aplicativo na [página <DoNotTranslate>**Summary**</DoNotTranslate> do APM](/docs/applications-menu/applications-overview). A New Relic usa esta fórmula para calcular o tempo de resposta:
...
<Callout variant="tip">
  Se seu aplicativo <DoNotTranslate>**never**</DoNotTranslate> relatou um tempo de fila, a métrica `WebFrontend/QueueTime` não existirá.
</Callout>

## Tempo de resposta [#app_rep_time]

Na [página <DoNotTranslate>**Summary**</DoNotTranslate> do APM](/docs/applications-menu/applications-overview), [o tempo de resposta](/docs/data-analysis/user-interface-functions/view-your-data/response-time) é a linha sobreposta no gráfico principal do seu aplicativo. Para obter os valores de métrica mostrados por esta linha, use a mesma fórmula e dois comandos descritos acima, mas remova `summarize=true` dos comandos.