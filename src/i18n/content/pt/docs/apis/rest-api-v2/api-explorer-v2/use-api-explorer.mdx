...
O REST API Explorer (v2) da New Relic facilita o teste e o envio de solicitações para qualquer endpoint de API disponível. Depois de selecionar sua escolha de funções para o tipo de chamada de API (aplicativo, navegador, etc.), a interface do usuário fornece um formulário interativo para visualizar requisitos e testar seus valores de parâmetros. Ele também fornece uma fonte ativa de documentação sobre os valores da API.
...
3. Na barra lateral, selecione um recurso (aplicativo, navegador, etc.) e uma [função](/docs/apm/apis/api-explorer-v2/parts-api-explorer#functions) de API disponível:
