...
A coleta de dados de solicitação HTTP pode expor dados potencialmente confidenciais. A New Relic chama os dados de valor principal coletados durante uma solicitação HTTP <DoNotTranslate>**attributes**</DoNotTranslate>. Por padrão, a coleta de atributo é limitada apenas a informações HTTP básicas que não sejam fonte de dados confidenciais (por exemplo, código de resposta e host); estes fazem parte do grupo de atributo básico que a New Relic chama de [<DoNotTranslate>**agent attributes**</DoNotTranslate>](/docs/agents/manage-apm-agents/agent-data/agent-attributes).
