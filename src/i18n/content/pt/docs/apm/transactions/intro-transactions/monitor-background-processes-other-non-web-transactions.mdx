...
Este documento explica o que são <DoNotTranslate>**non-web transactions**</DoNotTranslate> no APM e como você pode instrumentar tarefas em segundo plano e outras atividades arbitrárias para serem relatadas como transação fora da web.

## O que são transações fora da web? [#define]

As duas principais categorias [de transação](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) do APM são <DoNotTranslate>**web**</DoNotTranslate> e <DoNotTranslate>**non-web**</DoNotTranslate>.

* <DoNotTranslate>**Web transactions**</DoNotTranslate>

  são transações que detectamos como iniciadas por uma solicitação da web; são o tipo de transação mais comum relatado pela APM.

* <DoNotTranslate>**Non-web transactions**</DoNotTranslate>

  são transações onde não detectamos que foram iniciadas por uma solicitação da web. Isso inclui processos e trabalhos que não tratam de solicitações da Web, como processamento de mensagens e tarefas em segundo plano.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title=".NET"
  >
    Por padrão, o agente APM .NET instrumento IIS ASP workers. Para utilizar outro aplicativo .NET (não IIS), como serviços autônomos (WCF), aplicativos de console e outros serviços do Windows, consulte [aplicativo customizado instrumentado](/docs/agents/net-agent/features/instrumenting-custom-applications) e [aplicativo WCF instrumentado](/docs/agents/net-agent/features/instrumenting-wcf-applications).
...
Para visualizar os dados de transações fora da web na interface do New Relic, acesse o gráfico principal nas páginas a seguir e selecione a opção <DoNotTranslate>**Non-web**</DoNotTranslate> no dropdown de tipo de transação:
