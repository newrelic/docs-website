...
3. Instale o agente Java usando nosso inicializador ou seguindo os [procedimentos de instalação padrão](/docs/agents/java-agent/installation/java-agent-manual-installation). Dependendo de suas ferramentas e estrutura, consulte [procedimentos de instalação adicionais](/docs/apm/agents/java-agent/installation/java-install-overview) para instalar ou configurar o agente Java.
...
Para [visualizar o desempenho do seu aplicativo na interface do New Relic](/docs/apm/applications-menu/monitoring/apm-overview-page), acesse <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Summary**</DoNotTranslate>. A interface de usuário do APM inclui uma [página<DoNotTranslate>**JVM metrics**</DoNotTranslate> ](/docs/agents/java-agent/features/jvm-metrics-page)dedicada, detalhes [de transação](/docs/apm/transactions) e [erro](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) , uma [ferramenta de criação de perfil de thread](/docs/apm/applications-menu/events/thread-profiler-tool) para amostrar threads Java e relatar rastreamento de pilha e [muito mais](/docs/apm/applications-menu).
...
* [Instrumentação do navegador:](/docs/browser/new-relic-browser/page-load-timing-resources/page-load-timing-process) integre o agente Java ao
