...
Para acessar a classe API, adicione `newrelic-api.jar` ao caminho da classe do seu aplicativo. O jar está no arquivo de instalação `zip` do agente Java New Relic. Você pode chamar a API quando o agente Java não estiver em execução. Os métodos da API são apenas stubs; a implementação é adicionada quando o agente Java carrega a classe.
...
Para obter mais informações sobre como correlacionar dados log com outros dados de telemetria, consulte nossa documentação [de logs em contexto](/docs/logs/logs-context/java-configure-logs-context-all/) .
