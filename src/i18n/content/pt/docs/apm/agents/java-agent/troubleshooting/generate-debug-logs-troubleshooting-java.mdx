...
Se você alterar apenas a configuração do arquivo de log no arquivo `newrelic.yml` , não será necessário reiniciar o servidor ou a JVM. Entretanto, se você fizer qualquer outra alteração, <DoNotTranslate>**must**</DoNotTranslate> reinicie o servidor ou a JVM.
...
1. Edite seu arquivo `newrelic.yml` e altere [`log_level`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-log_level) para `finest`. Certifique-se de manter essas linhas [recuadas <DoNotTranslate>**two spaces**</DoNotTranslate>](#requirements).
