...
* Padrão (amostragem baseada no início): Antes de qualquer trace chegar, determinamos uma porcentagem definida de traces para aceitar e analisar. Isso oferece um ponto de partida sólido para ver como o rastreamento pode ajudar você. Está ativado por padrão nos agentes Java 7.4.0 e superiores. Por padrão, o agente Java amostra 120 traces por minuto, com um limite máximo de 2000 spans por minuto.
