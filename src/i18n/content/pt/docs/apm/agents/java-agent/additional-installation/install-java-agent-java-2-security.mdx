---
title: 'Instalação de segurança do Java 2 para WebLogic, WebSphere e Solr v9'
tags:
  - Agents
  - Java agent
...
Se você é usuário do WebLogic, WebSphere ou Solr v9 e usa o modelo de segurança do Java 2, nosso agente Java requer configuração além da [instalação do agente Java](/install/java).
...
## Solr v9: Segurança do Java 2 [#solr-java-2]
...
1. Defina a variável de ambiente `SOLR_SECURITY_MANAGER_ENABLED=false` ou no arquivo de inicialização `solr.in.sh/solr.in.cmd`.

2. Anexe o seguinte texto ao seu arquivo de política, certifique-se de preservar o traço `-` no final do caminho, o que concede a todos os jars na pasta de destino as permissões necessárias.
   ...
   <Callout variant="tip">
     Se você estiver usando o Java 9+ e encontrar erros de acesso negado, pode ser necessário conceder permissões adicionais (como `java.lang.RuntimePermission "accessSystemModules"`).
   </Callout>
