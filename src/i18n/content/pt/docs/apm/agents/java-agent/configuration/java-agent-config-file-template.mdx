---
title: Modelo de arquivo de configuração do agente Java
tags:
  - Agents
  - Java agent
  - Configuration
...
Este documento contém uma versão de modelo do arquivo `newrelic.yml` que o agente Java lê para configuração. O formato do arquivo de configuração é YAML. Embora nenhum conhecimento de YAML seja necessário para editar o arquivo, tome cuidado para manter o recuo adequado. Saiba mais sobre YAML na [Wikipédia](http://en.wikipedia.org/wiki/yaml).

Ao fazer alterações no arquivo `newrelic.yml` , use um validador YAML para garantir que a sintaxe esteja precisa e siga estas regras:

* Os arquivos YML diferenciam maiúsculas de minúsculas.
* Todos os recuos estão em incrementos de dois caracteres. Os dados na mesma seção do arquivo devem usar o mesmo nível de recuo. Recue as subseções em dois espaços adicionais.

Para obter mais informações sobre estrutura, diretórios e configurações, consulte [Configuração do agente Java](/docs/agents/java-agent/configuration/java-agent-configuration-config-file).
