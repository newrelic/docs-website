...
<Callout variant="tip">
  Também temos uma integração Kafka, que \[faz XYZ] Para detalhes sobre isso, veja [Kafka integração de monitoramento](/docs/integrations/host-integrations/host-integrations-list/kafka-monitoring-integration).
</Callout>
