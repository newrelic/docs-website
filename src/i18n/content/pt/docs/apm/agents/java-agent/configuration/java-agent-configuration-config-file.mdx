...
O agente pode ser desabilitado seletivamente com base na classe principal de inicialização ou no arquivo jar executável extraído da linha de comando. Isso é útil em situações em que a variável de ambiente &quot;JAVA\_TOOL\_OPTIONS&quot; está presente e contém o sinalizador -javaagent, mas não queremos aplicar a instrumentação a todos os aplicativos Java no ambiente - por exemplo, em um contêiner Kubernetes.

<Callout variant="important">
  A configuração de ignorar só é suportada por meio de uma variável de ambiente devido ao início do ciclo de vida do agente em que a configuração de ignorar precisa ser verificada.
</Callout>

Para fins de diagnóstico, se a variável `NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS` for definida, o agente imprimirá a linha de comando extraída e o token de salto analisado no stdout:
...
Se o agente for desabilitado com base na variável skip definida, o seguinte será despejado no stdout:
...
<CollapserGroup>
  <Collapser id="cfg-skip-configuration" title="NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS">
    ...
    Uma lista separada por vírgulas de classes principais, arquivos jar executáveis ou ferramentas/aplicativos baseados em Java que o agente NÃO deve instrumentar. Por exemplo: `NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS=keytool,myapp.jar,IgnoreThisClass`.
  </Collapser>
</CollapserGroup>
