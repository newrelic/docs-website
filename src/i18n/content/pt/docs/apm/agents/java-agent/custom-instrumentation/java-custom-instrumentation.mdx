...
Você também pode monitor Java Management Extensions (JMX) via instrumentação personalizada. JMX é uma forma de monitor e gerenciar aplicativos, dispositivos e serviços. Você pode implementar o monitoramento JMX por meio de um [arquivo <DoNotTranslate>**YAML**</DoNotTranslate> externo](/docs/java/custom-jmx-instrumentation-by-yml).