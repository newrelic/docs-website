...
O agente Java não seleciona a configuração de proxy do arquivo `newrelic.yml` . <DNT>**Recommendation:**</DNT> Se você usa um proxy em seu aplicativo, use uma destas opções para registrar a implantação:
