...
O New Relic permite enviar informações sobre a implantação do aplicativo pela linha de comando. Em seguida, você poderá visualizar as informações de implantação na [página<DoNotTranslate>**Deployments**</DoNotTranslate> ](/docs/apm/applications-menu/events/deployments-page)da interface: Vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (select an app) > Events > Deployments**</DoNotTranslate>.
...
Por exemplo, se você definir o marcador de implantação e executar o script para enviar 100 instâncias do aplicativo, a página <DoNotTranslate>**Deployments**</DoNotTranslate> mostrará uma implantação para a instância 100. Se você executasse o script novamente, ele mostraria outra implantação.
...
O agente Java não seleciona a configuração de proxy do arquivo `newrelic.yml` . <DoNotTranslate>**Recommendation:**</DoNotTranslate> Se você usa um proxy em seu aplicativo, use uma destas opções para registrar a implantação:
