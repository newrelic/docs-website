...
<CollapserGroup>
  ...
  >
    Para visualizar detalhes do rastreamento da transação, acesse: <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) APM & services > (select an app) > Transactions > Transaction trace > Trace details**</DoNotTranslate>.
...
<CollapserGroup>
  ...
  >
    O exemplo de código a seguir mostra um segmento começando no método `storeItem` para medir quanto tempo a instrução Lambda está aguardando no pool de threads. Para parar de cronometrar o segmento, você deve chamar `.end()` ou `.ignore()`. Se você <DoNotTranslate>**don't**</DoNotTranslate> desejar relatar o segmento como parte de sua transação pai, chame `.ignore()`. Caso contrário, para relatar o segmento como parte de sua transação pai, chame `.end()`.
