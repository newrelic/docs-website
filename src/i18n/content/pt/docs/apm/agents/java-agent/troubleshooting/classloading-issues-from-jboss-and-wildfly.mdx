...
<Tabs>
  <TabsBar>
    <TabsBarItem id="problem-incomplete-telemetry">
      Telemetria incompleta do agente
    </TabsBarItem>

    <TabsBarItem id="problem-application-runtime">
      O aplicativo falha ao inicializar ou outros problemas com o carregamento de classe no tempo de execução do aplicativo
    </TabsBarItem>
  </TabsBar>
  ...
  <TabsPages>
    <TabsPageItem id="problem-incomplete-telemetry">
      ...
      * O nome do seu despachante está incorreto
      ...
      O agente weaver pode instrumentar uma classe específica em um aplicativo usando o JBoss, mas as classes modificadas podem usar pacotes e módulos que não são reconhecidos pelo JBoss EAP/Wildfly. O resultado é que o carregador de classe do módulo JBoss não consegue encontrar classes específicas, o que gerará um `NoClassDefFoundError`.
      ...
      ### Correção 1: atualize seu agente

      Certifique-se de que seu agente Java seja da versão `8.6` ou superior.

      ### Correção 2: marque os pacotes a serem carregados pelo carregador de classe do sistema"
      ...
      ### Correção 3: configurar manualmente os módulos JBoss"

      Um dos problemas com as correções anteriores é que alguns pacotes não estão disponíveis através do carregador de classes do sistema. Um caso aqui é com o pacote `javax.management` . Como resultado, seu aplicativo pode falhar na inicialização ou pode não funcionar corretamente devido a `ClassNotFoundException`s serem lançados.
      ...
      Podemos encontrar o arquivo de configuração para um módulo JBoss específico em seu servidor JBoss:
      ...
      5. Dentro da tag `<dependencies>` , adicione o elemento `<module name="$PLACEHOLDER FOR MODULE NAME"/>`. Para `java.util.logging`, o elemento se torna `<module name="$PLACEHOLDER FOR MODULE NAME"/>`.
    ...
    <TabsPageItem id="problem-application-runtime">
      ...
      Esta é a solução se o seu agente Java for da versão `8.6` ou superior e você estiver usando a API de gerenciamento Jakarta/J2EE (JSR-777), mas seu aplicativo não for inicializado. Nesta instância, você verá `ClassNotFoundException`s onde o nome do pacote incluído na classe é `javax.management.j2ee`

      ## Solução [#solution-runtime]

      Se você atualizou o agente Java para `8.6` ou superior, seu aplicativo poderá falhar na inicialização se você estiver usando a API de gerenciamento Jakarta/J2EE. A solução para isso é atualizar seu agente para `8.7` ou superior e depois adicionar a propriedade do sistema:

      ```
      -Dcom.newrelic.jboss.jsr77.fix=true
      ```

      A consequência de definir a propriedade do sistema como verdadeira é que o agente não rotulará `javax.management` como uma classe de sistema. Como resultado, você precisará de configuração adicional. Siga esses passos:
