...
1. Faça backup do [diretório raiz do agente Java](/docs/agents/manage-apm-agents/troubleshooting/find-agent-root-directory#java-agent) <DoNotTranslate>**entire**</DoNotTranslate> em outro local. Renomeie esse diretório para `NewRelic_Agent#.#.#`, onde `#.#.#` é o número da versão do agente.
...
4. (Opcional) Se você estiver usando qualquer API do agente, como nossa [API do agente Java](/docs/apm/agents/java-agent/api-guides/guide-using-java-agent-api/) ou uma [API Scala](/docs/apm/agents/java-agent/frameworks/scala-installation-java/#using-the-scala-api), recomendamos que você atualize sua dependência de API para a versão mais recente. A versão mais recente do agente ainda suportará a API, mas pode estar faltando um novo recurso. Se você estiver usando uma ferramenta de construção como o maven, atualize as versões da API para as mais recentes. Por exemplo, se você estiver usando o agente Java API com gradle, substitua `${agent.version}` pela versão mais recente:
