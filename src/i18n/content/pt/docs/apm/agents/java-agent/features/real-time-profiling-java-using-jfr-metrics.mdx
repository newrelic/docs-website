...
* [Serviço JFR do agente Java New Relic](https://github.com/newrelic/newrelic-jfr-core/blob/main/README.md#new-relic-java-agent-jfr-service) (RECOMENDADO) - monitoramento JFR integrado ao principal [agente Java New Relic](/docs/agents/java-agent/). Nenhuma configuração adicional é necessária, basta [instalar o agente Java](/docs/agents/java-agent/installation/install-java-agent/), certificar-se de que o [serviço JFR esteja ativado](/docs/agents/java-agent/configuration/java-agent-configuration-config-file/#jfr-real-time-profiling) e os dados JFR fluirão para o mesmo aplicativo
  ...
  * <DoNotTranslate>**Note**</DoNotTranslate>
...
<Callout variant="tip">
  Com o agente Java versão 8.0.0 ou superior, é possível alterar dinamicamente a configuração de criação de perfil JFR sem reiniciar suas JVMs. Faça isso alternando a configuração em <DoNotTranslate>**Settings > Application > Java Flight Recorder**</DoNotTranslate> ou alterando a configuração `jfr.enabled` .
</Callout>

## Ver seus dados [#find-data]

Para visualizar seus dados, vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & Services > (select service) > More views > Realtime Profiling Java**</DoNotTranslate>.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & Services > (select service) > Realtime Profiling Java**</DoNotTranslate>: a visualização da linha do tempo do cluster JVM mostra o comportamento da JVM no cluster.
</figcaption>
...
<figcaption>
  Selecione <DoNotTranslate>**How is JVM health determined?**</DoNotTranslate> para obter uma análise detalhada de como a integridade da JVM é calculada.
</figcaption>

## Detalhes da JVM [#details]

Para localizar a página de detalhes da JVM, acesse: <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & Services > (select service) > Realtime Profiling Java > (select JVM)**</DoNotTranslate>.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/) > APM & Services > (select service) > Realtime Profiling Java > (select JVM)**</DoNotTranslate>: você pode ver detalhes de cada JVM, incluindo gráficos em degradê.
</figcaption>