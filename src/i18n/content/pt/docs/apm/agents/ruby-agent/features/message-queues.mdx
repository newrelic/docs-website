...
A interface APM mostra a transação iniciada por meio do recebimento de mensagens RabbitMQ (inscrever/consumir mensagens) como <DoNotTranslate>**Message**</DoNotTranslate> tarefas em segundo plano. A criação de mensagens via RabbitMQ também aparece no trace da transação.
...
<CollapserGroup>
  ...
  >
    A página <DoNotTranslate>**Transaction trace**</DoNotTranslate> inclui uma guia <DoNotTranslate>**Details**</DoNotTranslate> dedicada que mostra informações mais detalhadas sobre a transação. Aqui está um exemplo:
