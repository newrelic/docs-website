...
freshnessValidatedDate: '2026-08-11T00:00:00.000Z'
...
<Callout variant="tip">
  A New Relic aceita contribuições da comunidade para o agente <DNT>Ruby</DNT>. Caso haja uma gem a ser instrumentada ou um recurso a ser proposto, no repositório `newrelic/newrelic-ruby-agent` no GitHub, é possível compartilhar ideias abrindo uma [issue](https://github.com/newrelic/newrelic-ruby-agent/issues/new/choose) ou enviando um [pull request](https://github.com/newrelic/newrelic-ruby-agent).
</Callout>

## Encontrar extensões de terceiros

Qualquer um pode escrever uma joia baseada no agente <DNT>Ruby</DNT>. Exemplos antigos disso estão disponíveis em um repositório arquivado chamado [extends\_newrelic\_rpm](https://github.com/newrelic/extends_newrelic_rpm) para rastrear essas extensões e fornecer links para outras gems que constroem o agente <DNT>Ruby</DNT>.

Essas extensões **não** são suportadas pela New Relic. A New Relic reuniu esses links como um serviço aos nossos clientes. Problemas com essas gems devem ser relatados aos respectivos projetos no GitHub.

## Extensões como joias

Caso se opte por criar uma extensão de terceiros, recomenda-se uma gem por biblioteca instrumentada. Por exemplo, `newrelic-redis` fornece instrumentação para a gem `redis` (que também existe automaticamente dentro do nosso agente).
