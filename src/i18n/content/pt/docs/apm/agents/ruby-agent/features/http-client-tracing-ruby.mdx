...
O agente Ruby suporta a versão Excon <DoNotTranslate>**0.56.0 or higher.**</DoNotTranslate> A instrumentação Excon depende da capacidade de adicionar um middleware Excon à chave `:middlewares` de `Excon.defaults`, portanto, se seu aplicativo modificar `Excon.defaults` você deve garantir que preserva o valor de `:middlewares` chave.

## Notas de meio-fio [#curb]

O agente Ruby suporta a versão Curb <DoNotTranslate>**0.9.3 or higher.**</DoNotTranslate> A instrumentação Curb em JRuby **não** é suportada.
