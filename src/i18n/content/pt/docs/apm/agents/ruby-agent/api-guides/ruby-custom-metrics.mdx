...
## Nomenclatura métrica [#metric_names]
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Métrica e estatísticas no agente Ruby"
  >
    Existem duas estruturas de dados básicas usadas para coletar dados métricos: `MetricSpec` e `Stats`.

    Um `MetricSpec` é um identificador para o qual temos dados. O seguinte pseudo Ruby define uma métrica:
