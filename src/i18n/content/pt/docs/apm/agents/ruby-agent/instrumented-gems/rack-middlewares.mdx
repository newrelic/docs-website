...
As duas maneiras mais comuns de configurar middlewares Rack são a API `Rack::Builder` (geralmente de <DoNotTranslate>**config.ru**</DoNotTranslate>) e a configuração stack de middleware Rails:
...
<CollapserGroup>
  ...
  >
    ...
    O agente Ruby instrumento automaticamente middlewares adicionados via `ActionDispatch::MiddlewareStack` em <DoNotTranslate>**Rails 3.0 or higher**</DoNotTranslate>. Para obter mais informações sobre como configurar middlewares com Rails, consulte o [guia Ruby on Rails](http://guides.rubyonrails.org/rails_on_rack.html).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    O gráfico principal na [página APM <DoNotTranslate>**Summary**</DoNotTranslate> ](/docs/apm/applications-menu/monitoring/applications-overview-dashboard)do seu aplicativo inclui uma barra roxa que mostra o tempo médio gasto por solicitação em todos os middlewares Rack do seu aplicativo.
    ...
    <figcaption>
      <DoNotTranslate>**APM > (selected application) > Summary:**</DoNotTranslate> O tempo do middleware aparece em roxo no gráfico Visão geral principal do seu aplicativo.
    </figcaption>
  </Collapser>
  ...
  >
    Você também pode ver o tempo de middlewares individuais para um nome de transação específico na [página APM<DoNotTranslate>**Transactions**</DoNotTranslate> ](/docs/apm/applications-menu/monitoring/transactions-page)do seu aplicativo.
    ...
    <figcaption>
      <DoNotTranslate>**APM > (selected application) > Monitor > Transactions > (selected transaction) > Trace details:**</DoNotTranslate> Aqui está um exemplo de tempo de middleware para uma transação selecionada para seu aplicativo.
    </figcaption>
  </Collapser>
  ...
  >
    ...
    <figcaption>
      <DoNotTranslate>**APM > (selected application) > Monitor > Transactions > (selected transaction trace):**</DoNotTranslate> Aqui está um exemplo de detalhes de middleware em um rastreamento de transação.
    </figcaption>
  </Collapser>
</CollapserGroup>
