...
Para enviar dados de erro que você está processando em seu próprio código para o New Relic, use a chamada da <DNT>Ruby</DNT> API do agente [`NewRelic::Agent.notice_error`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#notice_error-instance_method) em seu gerenciador de erros.
...
Transações e erros podem ser associados a um ID de usuário se um for conhecido pelo agente New Relic Ruby. Use a API do agente Ruby [`NewRelic::Agent.set_user_id`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#set_user_id-instance_method) para fornecer ao agente um ID de usuário.
