...
2. No seu aplicativo Sinatra, imediatamente abaixo da diretiva Sinatra <DoNotTranslate>**require**</DoNotTranslate> , adicione:

   ```
   require 'newrelic_rpm'
   ```

3. Configure `RACK_ENV` para o ambiente correspondente às definições de configuração em seu arquivo <DoNotTranslate>**newrelic.yml**</DoNotTranslate> ; por exemplo, desenvolvimento, preparação, produção, etc.

<DoNotTranslate>**Note:**</DoNotTranslate> O modo desenvolvedores só funciona com Rails.
