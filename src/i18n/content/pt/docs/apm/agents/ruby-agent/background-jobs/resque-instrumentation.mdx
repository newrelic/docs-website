...
<DoNotTranslate>**Exception:**</DoNotTranslate> Se você tiver chamadas restantes para métodos `NewRelic::Agent` dos ganchos `before_first_fork`, `before_fork` ou `after_fork` do Resque de quando você estava executando uma versão mais antiga do Resque, certifique-se de <DoNotTranslate>**remove**</DoNotTranslate> essas chamadas após atualizar para o Resque 1.23 .1 ou superior.
