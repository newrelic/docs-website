...
<Callout variant="important">
  Se você estiver usando o agente em um contêiner Docker , instale o contêiner do agente <DoNotTranslate>**within each**</DoNotTranslate>.
</Callout>
   ...
   <table>
     ...
     <tbody>
       <tr>
         ...
         <td>
           * <DoNotTranslate>**Rails**</DoNotTranslate>: Rails chamará automaticamente `Bundler.require` e fará com que `newrelic_rpm` seja necessário durante a inicialização do seu aplicativo.

           * <DoNotTranslate>**Roda, Sinatra**</DoNotTranslate>: se estiver usando Roda, Sinatra ou outro framework, você deverá chamar manualmente `require 'newrelic_rpm'`. Além disso, se você estiver usando o Rastreamento Infinito, chame manualmente `require 'newrelic/infinite_tracing'`.
...
## Instalar em versões mais antigas do Rails [#Older_Rails_installations]

Se você estiver instalando o agente Ruby no Rails 2.x e não estiver usando o Bundler, siga estes procedimentos.
...
<table>
  ...
  <tbody>
    <tr id="Rails2_Without_Bundler">
      <td>
        Rails 2.1 - 2.3 sem Bundler
      </td>

      <td>
        1. Instale a gema usando `gem install newrelic_rpm`.

        2. Edite `environment.rb` e adicione ao bloco inicializador:

           ```
           config.gem "newrelic_rpm"
           ```

        3. Se estiver ativando o Rastreamento Infinito, adicione o seguinte à próxima linha do arquivo `environment.rb` :
    ...
    <tr id="Older_Rails_installations">
      ...
      <td>
        A New Relic não suporta oficialmente versões do Rails anteriores à 2.1. No entanto, se você quiser usar o New Relic for Rails versões 2.0.\*, edite `environment.rb` e adicione esta instrução após o bloco `initializer:` :
        ...
        Se estiver ativando o Rastreamento Infinito, adicione o seguinte à próxima linha do arquivo `environment.rb` :
