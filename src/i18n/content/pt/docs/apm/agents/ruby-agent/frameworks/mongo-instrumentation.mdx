...
Além disso, as gemas `extends_newrelic_rpm` e `newrelic_mongo` usam uma configuração distinta que pode ser usada para desativá-las se a remoção completa dessas gemas não for possível em seu ambiente. Para essas joias de terceiros, defina `disable_mongodb: false` em seu <DoNotTranslate>**newrelic.yml**</DoNotTranslate>.

## Desativando instrumentação [#Disabling_instrumentation]

Para desativar a instrumentação do Mongo no agente, adicione o seguinte ao seu <DoNotTranslate>**newrelic.yml**</DoNotTranslate>:
