...
<CollapserGroup>
  ...
  <Collapser id="escape_utils" title="escape_utils">
    <DNT>**Problem:**</DNT> A gem [escape\_utils](https://github.com/brianmario/escape_utils) é incompatível com a instrumentação automática para o recurso de tempo de carregamento de página do New Relic (às vezes chamado de monitoramento de usuário real ou RUM). Devido à forma como `escape_utils` monkey corrige o Rack, toda a sua resposta HTML pode ser escapada.
  ...
  <Collapser id="right_http_connection" title="right_http_connection">
    <DNT>**Problem:**</DNT> Se a gem [right\_http\_connection](http://rubygems.org/gems/right_http_connection) for carregada após `newrelic_rpm`, ela corrige a classe `Net::HTTP` de uma forma que faz com que a instrumentação externa da New Relic seja perdida.
