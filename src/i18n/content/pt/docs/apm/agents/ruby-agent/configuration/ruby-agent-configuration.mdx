...
<CollapserGroup>
  ...
  <Collapser id="api_key" title="api_key">
    ...
    Sua New Relic <InlinePopover type="userKey" />. Obrigatório ao usar a API REST v2 do New Relic para registrar a implantação usando o comando `newrelic deployments` .
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-explain_enabled" title="transaction_tracer.explain_enabled">
    ...
    Se `true`, habilita a coleta de planos explicativos no rastreamento da transação. Essa configuração também será aplicada para explicar planos em rastreamento SQL lento se [`slow_sql.explain_enabled`](#slow_sql-explain_enabled) não for definido separadamente.
  </Collapser>
  ...
  <Collapser id="transaction_tracer-record_sql" title="transaction_tracer.record_sql">
    ...
    Por padrão, isso é definido como `obfuscated`, que elimina os literais numéricos e de string.
...
<CollapserGroup>
  ...
  <Collapser id="application_logging-forwarding-log_level" title="application_logging.forwarding.log_level">
    ...
    Isso é baseado nos valores inteiros das constantes `Logger::Severity` do Ruby: https://github.com/ruby/ruby/blob/master/lib/logger/severity.rb
...
<CollapserGroup>
  <Collapser id="code_level_metrics-enabled" title="code_level_metrics.enabled">
    ...
    Se `true`, o agente reportará a métrica de nível de código-fonte para métodos de rastreamento. Consulte: https://docs.newrelic.com/docs/apm/agents/ruby-agent/features/ruby-codestream-integration/
  </Collapser>
</CollapserGroup>

## Tracer multiaplicativo [#cross-application-tracer]
...
<CollapserGroup>
  <Collapser id="cross_application_tracer-enabled" title="cross_application_tracer.enabled">
    ...
    Se `true`, habilita [o rastreamento multiaplicativo](/docs/agents/ruby-agent/features/cross-application-tracing-ruby/) quando `distributed_tracing.enabled` estiver configurado como `false`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    Se `true`, desabilita o middleware do agente para Sinatra. Este middleware é responsável pelo suporte avançado a recursos como [rastreamento multiaplicativo](/docs/apm/transactions/cross-application-traces/cross-application-tracing), [tempo de carregamento de página](/docs/browser/new-relic-browser/getting-started/new-relic-browser) e [coleta de erros](/docs/apm/applications-menu/events/view-apm-error-analytics).

    <Callout variant="important">
      O rastreamento multiaplicativo foi descontinuado em favor do [distributed tracing](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing). distributed tracing está ativado por padrão para o agente Ruby versões 8.0.0 e superiores. Middlewares não são necessários para suportar distributed tracing.
...
<CollapserGroup>
  ...
  <Collapser id="security-detection-rci-enabled" title="security.detection.rci.enabled">
    ...
    Se `true`, habilita a detecção de RCI (injeção remota de código)
  </Collapser>
  ...
  <Collapser id="security-detection-rxss-enabled" title="security.detection.rxss.enabled">
    ...
    Se `true`, habilita a detecção de RXSS (script entre sites refletidos)
  </Collapser>
  ...
  <Collapser id="security-detection-deserialization-enabled" title="security.detection.deserialization.enabled">
    ...
    Se `true`, habilita a detecção de desserialização
  </Collapser>
...
<CollapserGroup>
  <Collapser id="serverless_mode-enabled" title="serverless_mode.enabled">
    ...
    Se `true`, o agente operará em um modo simplificado adequado para uso com função serverless de curta duração. NOTA: Somente a função do AWS Lambda é suportada atualmente e esta opção não se destina ao uso sem a oferta da [camada Ruby Lambda da New Relic](https://docs.newrelic.com/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/monitoring-aws-lambda-serverless-monitoring/).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="strip_exception_messages-enabled" title="strip_exception_messages.enabled">
    ...
    Se for verdade, o agente remove mensagens de todas as exceções, exceto aquelas na [lista de permissões](#strip_exception_messages-allowlist). Ativado automaticamente no [modo de alta segurança](/docs/accounts-partnerships/accounts/security/high-security).
  </Collapser>
