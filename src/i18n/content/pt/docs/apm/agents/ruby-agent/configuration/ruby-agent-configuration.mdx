...
Para obter informações sobre erros ignorados e esperados, [consulte esta página em Error Analytics no APM](/docs/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected/). Para definir erro esperado através do método Ruby `NewRelic::Agent.notice_error` , [consulte a API Ruby do agente](/docs/agents/ruby-agent/api-guides/sending-handled-errors-new-relic/).
...
<CollapserGroup>
  ...
  >
    ...
    Define o número máximo de quadros em um backtrace de erro. Os backtraces acima desse valor são truncados no início e no final.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    * No modo de alta segurança, o padrão é `false`.

    * Permite que o agente instrumente um aplicativo. Oferecemos suporte a `prepend` para prefixação de módulo e `chain` para cadeia de método de alias.

      * O agente Ruby usa `prepend` por padrão.
      * Se você possui várias bibliotecas que atualizam a mesma classe, você pode atualizar esta configuração para `chain`.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    Isso é baseado nos valores inteiros das constantes `Logger::Severity` do Ruby: [https://github.com/ruby/ruby/blob/master/lib/logger/severity.rb](https://github.com/ruby/ruby/blob/master/lib/logger/severity.rb)
...
Esta seção inclui configuração do agente Ruby para configurar AI Monitoring. [distributed tracing](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#distributed-tracing) deve ser ativado para capturar dados trace e feedback. Ele está ativado por padrão no agente Ruby 8.0.0 e superior.
...
<CollapserGroup>
  ...
  >
    ...
    Se `true`, o agente reportará métricas de nível de código-fonte para métodos de rastreamento. consulte: [https://docs.newrelic.com/docs/apm/agents/ruby-agent/features/ruby-codestream-integration/](https://docs.newrelic.com/docs/apm/agents/ruby-agent/features/ruby-codestream-integration/)
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**DEPRECATED**</DoNotTranslate> Consulte: [distributed_tracing.enabled](#distributed_tracing-enabled).
...
<CollapserGroup>
  ...
  >
    ...
    * Ao configurar o agente para [AI Monitoring](/docs/ai-monitoring/intro-to-ai-monitoring), defina o valor máximo `100000`. Garante que o agente capture a quantidade máxima de eventos LLM.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    * Ao configurar o agente para [AI Monitoring](/docs/ai-monitoring/intro-to-ai-monitoring), defina o valor máximo `10000`. Garante que o agente capture a quantidade máxima de rastreio distribuído.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    Especifique uma lista de exceções que você não deseja que o agente remova quando [strip_exception_messages](#strip_exception_messages-enabled) for `true`. Separe as exceções com vírgula. Por exemplo, `"ImportantException,PreserveMessageException"`.
  </Collapser>
</CollapserGroup>
