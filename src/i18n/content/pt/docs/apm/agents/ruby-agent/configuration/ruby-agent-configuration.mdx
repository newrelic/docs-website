...
<CollapserGroup>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Se `true`, o agente usará o nome da tabela do modelo do Active Record em vez do nome da classe ao nomear métricas, spans e segmentos de trace da transação do Active Record. Isso pode reduzir a cardinalidade quando múltiplos modelos compartilham uma tabela de banco de dados. O padrão é `false`.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    Define o número máximo de frames em um backtrace de erro. Backtraces acima desse valor são truncados no meio por padrão, preservando o início e o fim do stack trace. Consulte `error_collector.backtrace_truncate_location` para personalizar o local de truncamento.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Uma matriz de nomes de Rails.event para capturar como eventos de log estruturados. Por exemplo,

    * user.signup
    * payment.processed
    * order.created Deixe em branco para capturar todas as notificações do Rails.event. Os eventos são registrados com o nível UNKNOWN (garantindo que nunca sejam filtrados) a menos que sobrescrito via chave :level na carga do evento.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Se `true`, o agente relata o ID da instância do GCP como o nome do host ao ser executado no Google Cloud Run.
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    Se `true`, o agente adiciona o valor `K_REVISION` como prefixo ao ID da instância do GCP para formar o nome do host (`{K_REVISION}-{instance id}`) no Google Cloud Run. Não tem efeito a menos que `utilization.gcp_cloud_run.use_instance_as_host` também seja `true`.
  </Collapser>
