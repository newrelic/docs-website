...
<CollapserGroup>
  ...
  <Collapser id="api_key" title="api_key">
    ...
    # DESCONTINUADO: A configuração `api_key` agora está obsoleta. Seu único uso era para o comando `newrelic deployments` , que está sendo removido na versão 10.0.0 do agente.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Esta configuração controla o comportamento da amostragem de transações quando um pai remoto é amostrado e o sinalizador trace é definido no traceparent. Os valores disponíveis são `default`, `always_on` e `always_off`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Esta configuração controla o comportamento da amostragem de transações quando um pai remoto não é amostrado e o sinalizador trace não está definido no traceparent. Os valores disponíveis são `default`, `always_on` e `always_off`.
  </Collapser>
</CollapserGroup>
