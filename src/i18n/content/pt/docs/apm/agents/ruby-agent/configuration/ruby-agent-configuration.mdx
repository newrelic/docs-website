...
<CollapserGroup>
  ...
  <Collapser id="log_level" title="log_level">
    ...
    Define o nível de detalhe das mensagens do log. Os níveis de log possíveis, em ordem crescente de verbosidade, são: `fatal`, `error`, `warn`, `info` ou `debug`.
  </Collapser>
  ...
  <Collapser id="active_support_custom_events_names" title="active_support_custom_events_names">
    ...
    Uma matriz de nomes de eventos personalizados do ActiveSupport para assinar e instrumentar. Por exemplo: \[one.custom.event, another.event, a.third.event]
  </Collapser>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Se `true`, o agente usará o nome da tabela do modelo ActiveRecord em vez do nome da classe ao nomear métricas, spans e segmentos de trace da transação do ActiveRecord. Isso pode reduzir a cardinalidade quando vários modelos compartilham uma tabela de banco de dados. O padrão é `false`.
  </Collapser>
  ...
  <Collapser id="capture_memcache_keys" title="capture_memcache_keys">
    ...
    Se `true`, o agente captura chaves do memcache no trace da transação.
  </Collapser>
  ...
  <Collapser id="config_path" title="config_path">
    ...
    Caminho para `newrelic.yml`. Se indefinido, o agente verifica os seguintes diretórios (em ordem): `config/newrelic.yml` -&gt; `newrelic.yml` -&gt; `$HOME/.newrelic/newrelic.yml` -&gt; `$HOME/newrelic.yml`
  </Collapser>
  ...
  <Collapser id="exclude_newrelic_header" title="exclude_newrelic_header">
    ...
    Se `true`, suprime o cabeçalho de distributed tracing `newrelic` em requests de saída.
  </Collapser>
  ...
  <Collapser id="force_install_exit_handler" title="force_install_exit_handler">
    ...
    Se `true`, força o agente a instalar seu manipulador de saída (que envia todos os dados em cache para o coletor antes de encerrar), mesmo em casos em que o agente normalmente o ignoraria, como quando detecta o Sinatra sendo executado como um serviço embutido dentro de outro framework. O Sinatra executa todo o aplicativo dentro de seu próprio bloco `at_exit`, portanto, o agente ignora seu próprio manipulador `at_exit` por padrão para evitar conflitos. Esta configuração substitui isso. Observação: `send_data_on_exit` também deve ser `true` para que esta configuração tenha algum efeito.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    Define um caminho para o arquivo de log do agente, excluindo o nome do arquivo. Se desejar enviar os logs do agente para a saída padrão, defina isso como `STDOUT`.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    ...
    Se `true`, assim que [`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments) for atingido, o agente também parará de registrar o tempo exclusivo para quaisquer segmentos criados posteriormente nessa transação. Isso pode reduzir o uso de memória para transações de execução muito longa com muitos segmentos, à custa de dados de tempo menos precisos para segmentos na transação se o limite de segmentos for atingido.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-expected_status_codes" title="error_collector.expected_status_codes">
    ...
    Uma lista separada por vírgulas de códigos de status HTTP e/ou intervalos de códigos de status (por exemplo, `404, 429, 500-599`). Os erros associados a um código de status correspondente, se o erro tiver um, serão tratados como esperado.
  </Collapser>
  ...
  <Collapser id="error_collector-ignore_status_codes" title="error_collector.ignore_status_codes">
    ...
    Uma lista separada por vírgulas de códigos de status HTTP e/ou intervalos de códigos de status (por exemplo, `404, 429, 500-599`). Os erros associados a um código de status correspondente, se o erro tiver um, serão ignorados.
  </Collapser>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    Define o número máximo de frames em um backtrace de erro. Backtraces acima desse valor são truncados no meio por padrão, preservando o início e o fim do stack trace. Consulte [`error_collector.backtrace_truncate_location`](#error_collector-backtrace_truncate_location) para personalizar o local de truncamento.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    Especifica onde truncar no backtrace quando o número de frames exceder [`error_collector.max_backtrace_frames`](#error_collector-max_backtrace_frames). As opções são `top` (remover frames do início), `middle` (remover frames do meio, preservando o início e o fim) ou `end` (remover frames do fim).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser_monitoring-version" title="browser_monitoring.version">
    ...
    Fixe a versão do loader do agente do browser que a New Relic injeta, como `1.317.0`. Quando não definido, a New Relic injeta a versão mais recente disponível. Consulte a [política de EOL do agente do browser](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/) para saber quais versões estão disponíveis e são suportadas no momento.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="application_logging-forwarding-log_level" title="application_logging.forwarding.log_level">
    ...
    Define o nível mínimo que um evento de log deve ter para ser encaminhado para o New Relic. Os logs no nível configurado e em qualquer gravidade superior são encaminhados, por exemplo, `debug` encaminha todo evento de log, enquanto `error` encaminha apenas evento `error`, `fatal` e `unknown`. Valores válidos, da menor para a maior gravidade: `debug`, `info`, `warn`, `error`, `fatal`, `unknown` (não diferencia maiúsculas de minúsculas).

    Essa ordenação é baseada nos valores inteiros das constantes `Logger::Severity` do Ruby (consulte [https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb](https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb)).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="ai_monitoring-record_content-enabled" title="ai_monitoring.record_content.enabled">
    ...
    Se `false`, a instrumentação de LLM (apenas OpenAI por enquanto) não capturará o atributo `content` nos eventos LlmChatCompletionMessage ou o atributo `input` nos eventos LlmEmbedding. Esta é uma configuração de segurança opcional para evitar o registro de dados confidenciais enviados para e recebidos dos LLMs.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="audit_log-endpoints" title="audit_log.endpoints">
    ...
    Lista de endpoints permitidos para incluir no log de auditoria.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="cloud-aws-account_id" title="cloud.aws.account_id">
    ...
    O ID da conta da AWS associado a este aplicativo.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="custom_insights_events-max_samples_stored" title="custom_insights_events.max_samples_stored">
    ...
    Especifique um número máximo de eventos personalizados para armazenar em buffer na memória por vez. Ao configurar o agente para [AI Monitoring](/docs/ai-monitoring/intro-to-ai-monitoring), defina o valor máximo `100000`. Isso garante que o agente capture a quantidade máxima de eventos de LLM.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="datastore_tracer-database_name_reporting-enabled" title="datastore_tracer.database_name_reporting.enabled">
    ...
    Se `false`, o agente não adicionará o parâmetro `database_name` à transação ou aos traces de SQL lento.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="disable_active_job_step_names" title="disable_active_job_step_names">
    ...
    Se `true`, os nomes das etapas do Rails Active Job Continuations não serão incluídos nos nomes das métricas. Isso reduz a cardinalidade da métrica, mas também reduz a visibilidade do desempenho da etapa individual. Defina isso como `true` apenas se estiver enfrentando problemas de cardinalidade de métrica devido a jobs com muitas etapas ou etapas nomeadas dinamicamente.
  </Collapser>
  ...
  <Collapser id="disable_active_record_notifications" title="disable_active_record_notifications">
    ...
    Se `true`, desabilita a instrumentação para o Active Record 4+.
  </Collapser>
  ...
  <Collapser id="disable_puma_instrumentation" title="disable_puma_instrumentation">
    ...
    Se `true`, desabilita a instrumentação de estatísticas do servidor Puma que coleta amostras de `Ruby/Puma/*` métricas do processo mestre do Puma. Esta instrumentação está desabilitada por padrão. Quando habilitado, o agente inicia uma thread de relatórios no processo mestre do Puma para entregar essas métricas.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    Esta configuração controla o comportamento da amostragem de transações para transações sem um pai remoto (traces que se originam nesta instância do agente New Relic). Os valores disponíveis são `adaptive` (o padrão), `always_on`, `always_off` e `trace_id_ratio_based`.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="instrumentation.active_support_notifications.active_support_events">
    ...
    Uma lista de permissões de nomes de eventos de notificação do ActiveSupport (principalmente eventos de cache e eventos de serializador de mensagens) que o agente assina. Qualquer nome de evento não incluído aqui é ignorado. Forneça apenas nomes de eventos completos, como `cache_fetch_hit.active_support`. Não forneça asteriscos ou padrões de regex e não escape nenhum caractere com barras invertidas.

    Para a lista completa de possíveis nomes de eventos, consulte a lista de [nomes de cache](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) e [nomes de mensagens](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) na documentação do Rails.
  </Collapser>
  ...
  <Collapser id="instrumentation-grpc-host_denylist" title="instrumentation.grpc.host_denylist">
    ...
    Uma lista separada por vírgulas de padrões de expressões regulares correspondentes aos nomes do host gRPC cujo tráfego a New Relic deve ignorar. A instrumentação do cliente gRPC da New Relic ignora o rastreamento de chamadas feitas a um host que corresponda a qualquer um desses padrões, e sua instrumentação do servidor gRPC ignora o rastreamento de requests recebidos por um servidor em execução em um host correspondente. Por exemplo, `"private.com$,exception.*" Empty by default, meaning no traffic is ignored.`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger" title="instrumentation.rails_event_logger">
    ...
    Controla a instrumentação de Rails.event como logs estruturados. Pode ser um dos seguintes: `enabled`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Uma matriz de nomes de `Rails.event` para capturar como eventos de log estruturados. Por exemplo: \[user.signup, payment.processed, order.created]. Deixe em branco para capturar todas as notificações de `Rails.event`. Os eventos são registrados com o nível `UNKNOWN` (garantindo que nunca sejam filtrados), a menos que sejam substituídos por meio de uma chave `:level` na carga do evento.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Uma opção de configuração global para desabilitar todos os sinais do OpenTelemetry enviados pelo New Relic. Se `false`, nenhum sinal do OpenTelemetry será enviado para a New Relic. Se `true`, a opção de configuração habilitada específica do sinal (por exemplo, opentelemetry.traces.enabled) determina se a telemetria desse tipo de sinal será reportada ao New Relic.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="process_host-display_name" title="process_host.display_name">
    ...
    Especifique um nome do host personalizado para [exibição na interface do New Relic](/docs/apm/new-relic-apm/maintenance/add-rename-remove-hosts#display_name).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="puma-sample_rate" title="puma.sample_rate">
    ...
    Número de segundos entre as amostras de estatísticas do servidor Puma coletadas no processo mestre do Puma.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rake-tasks" title="rake.tasks">
    ...
    Especifique uma matriz de tarefas Rake para instrumentar automaticamente. Esta opção de configuração converte a matriz em uma lista RegEx. Nenhuma tarefa rake será instrumentada a menos que seja adicionada a esta lista. Caso deseje permitir todas as tarefas por padrão, utilize `rake.tasks: [.+]`. Para mais informações, visite a [documentação de instrumentação do New Relic Rake](/docs/apm/agents/ruby-agent/background-jobs/rake-instrumentation).
  </Collapser>
  ...
  <Collapser id="rake-connect_timeout" title="rake.connect_timeout">
    ...
    Define o número de segundos a aguardar a conclusão de uma conexão antes de executar uma tarefa do Rake.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rules-ignore_url_regexes" title="rules.ignore_url_regexes">
    ...
    Defina as transações que o agente deve ignorar especificando uma lista de padrões que correspondam aos URIs a serem excluídos. Para mais detalhes, consulte [a documentação sobre como ignorar transações específicas](/docs/agents/ruby-agent/api-guides/ignoring-specific-transactions/#config-ignoring).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="security-agent-enabled" title="security.agent.enabled">
    ...
    Se `true`, o agente de segurança é carregado (um &apos;require&apos; do Ruby é executado).
  </Collapser>
  ...
  <Collapser id="security-enabled" title="security.enabled">
    ...
    Se `true`, o agente de segurança é iniciado (o agente é executado em seu loop de evento).
  </Collapser>
  ...
  <Collapser id="security-mode" title="security.mode">
    ...
    Define o modo de operação do agente de segurança. Atualmente, apenas `IAST` é suportado.
  </Collapser>
  ...
  <Collapser id="security-validator_service_url" title="security.validator_service_url">
    ...
    Define a URL do endpoint para o envio de dados relacionados à segurança.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-insecure_settings" title="security.exclude_from_iast_scan.iast_detection_category.insecure_settings">
    ...
    Se `true`, desativa a detecção de configurações inseguras de baixa gravidade. Por exemplo, aplicar hash, criptografia, cookie, geradores aleatórios e limite de confiança.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-nosql_injection" title="security.exclude_from_iast_scan.iast_detection_category.nosql_injection">
    ...
    Se `true`, desativa a detecção de injeção NoSQL em varreduras IAST.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-xpath_injection" title="security.exclude_from_iast_scan.iast_detection_category.xpath_injection">
    ...
    Se `true`, desativa a detecção de injeção de XPath em varreduras do IAST.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-ssrf" title="security.exclude_from_iast_scan.iast_detection_category.ssrf">
    ...
    Se `true`, desabilita a detecção de Server-Side Request Forgery (SSRF) em varreduras IAST.
  </Collapser>
  ...
  <Collapser id="security-scan_schedule-duration" title="security.scan_schedule.duration">
    ...
    Especifica a duração (em minutos) durante a qual a varredura do IAST é executada.
  </Collapser>
  ...
  <Collapser id="security-scan_controllers-iast_scan_request_rate_limit" title="security.scan_controllers.iast_scan_request_rate_limit">
    ...
    Define o número máximo de requests HTTP permitidos por minuto para a verificação do IAST. Deve ser um número inteiro entre 12 e 3.600 (padrão).
  </Collapser>
  ...
  <Collapser id="security-iast_test_identifier" title="security.iast_test_identifier">
    ...
    Um identificador de teste exclusivo ao executar IAST em um ambiente CI/CD para diferenciar entre diferentes execuções de teste. Por exemplo, um número de compilação.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="sidekiq-args-include" title="sidekiq.args.include">
    ...
    Uma matriz de strings que servirá coletivamente como uma lista de permissões para filtrar quais argumentos de trabalho do Sidekiq são reportados para a New Relic. Para capturar quaisquer argumentos do Sidekiq, `job.sidekiq.args.*` também deve ser adicionado à opção de configuração `attributes.include` separada. Cada string nesta matriz será transformada em uma expressão regular via `Regexp.new` para permitir a correspondência avançada. Para aplicar hash aos argumentos de trabalho, se uma chave ou valor corresponder, o par será incluído. Todos os elementos da matriz de argumentos de trabalho e escalares de argumentos de trabalho correspondentes serão incluídos.
  </Collapser>
  ...
  <Collapser id="sidekiq-args-exclude" title="sidekiq.args.exclude">
    ...
    Uma matriz de strings que servirá coletivamente como uma lista de negação para filtrar quais argumentos de trabalho do Sidekiq são relatados à New Relic. Para capturar quaisquer argumentos do Sidekiq, `job.sidekiq.args.*` também deve ser adicionado à opção de configuração `attributes.include` separada. Cada string nesta matriz será transformada em uma expressão regular via `Regexp.new` para permitir a correspondência avançada. Para aplicar hash em argumentos de trabalho, se uma chave ou valor corresponder, o par será excluído. Todos os elementos da matriz de argumentos de trabalho e escalares de argumentos de trabalho correspondentes serão excluídos.
  </Collapser>
  ...
  <Collapser id="sidekiq-separate_transactions" title="sidekiq.separate_transactions">
    ...
    Se `true`, o agente inicia uma nova transação quando um job do Sidekiq é executado durante uma transação da web. Isso impede que o tempo de execução do job do Sidekiq seja incluído nas métricas de transação da web.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="slow_sql-use_longer_sql_id" title="slow_sql.use_longer_sql_id">
    ...
    Se `true`, o agente gera um `sql_id` mais longo para trace de SQL lento. `sql_id` é usado para agregação de consulta semelhante.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="span_events-max_samples_stored" title="span_events.max_samples_stored">
    ...
    Define o número máximo de eventos de span relatados a partir de uma única coleta. Qualquer número inteiro entre `1` e `10000` é válido. Ao configurar o agente para [AI Monitoring](/docs/ai-monitoring/intro-to-ai-monitoring), defina o valor máximo `10000`. Isso garante que o agente capture a quantidade máxima de distributed trace.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="strip_exception_messages-enabled" title="strip_exception_messages.enabled">
    ...
    Se `true`, o agente remove as mensagens de todas as exceções, exceto daquelas na [lista de classes permitidas](#strip_exception_messages-allowed_classes). Habilitado automaticamente no [modo de alta segurança](/docs/accounts-partnerships/accounts/security/high-security).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Se `true`, o agente reporta o ID da instância do GCP como o nome do host quando executado no Google Cloud Run (Serviços, Pools de workers e Jobs).
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    Se `true`, o agente adiciona o nome da revisão do Cloud Run como prefixo ao ID da instância do GCP para formar o nome do host (`{revision}-{instance id}`) no Google Cloud Run. O nome da revisão vem de `K_REVISION` em um Serviço do Cloud Run, `CLOUD_RUN_REVISION` em um Worker Pool do Cloud Run e `CLOUD_RUN_EXECUTION` em um Job do Cloud Run. Não tem efeito a menos que `utilization.gcp_cloud_run.use_instance_as_host` também seja `true`.
  </Collapser>
...
Configurar uma das seguintes variáveis de ambiente ajudará você a identificar quais versões do seu software produziram os erros.

* `NEW_RELIC_METADATA_SERVICE_VERSION` cria `tags.service.version` nos dados de evento contendo a versão do seu código que é implantada, em muitos casos, uma versão semântica como `1.2.3`, mas nem sempre.
* `NEW_RELIC_METADATA_RELEASE_TAG` cria `tags.releaseTag` nos dados do evento contendo a tag de lançamento (como `v0.1.209` ou `release-209`).
* `NEW_RELIC_METADATA_COMMIT` cria `tags.commit` nos dados do evento contendo o SHA do commit. O SHA completo pode ser usado ou apenas os primeiros sete caracteres (por exemplo, `734713b`).

O [Errors Inbox](/docs/errors-inbox/version-tracking/) rastreará automaticamente quais versões do seu software estão produzindo erros.