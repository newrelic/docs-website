...
<CollapserGroup>
  ...
  >
    Neste exemplo, o New Relic relata um erro de `GetResponse()` <DoNotTranslate>**unless**</DoNotTranslate> o método `Foo()` é instrumentado. Contanto que `Foo` seja instrumentado, o New Relic inicia uma transação quando `Foo` é chamado e termina a transação quando `Foo` termina.
