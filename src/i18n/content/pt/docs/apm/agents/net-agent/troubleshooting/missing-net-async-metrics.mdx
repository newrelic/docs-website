...
<CollapserGroup>
  ...
  >
    Se você quiser especificar a versão do .NET framework desejada, esteja ciente de que isso impõe verificações rigorosas de tempo de execução que podem causar problemas de compatibilidade. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Se possível, [diga ao .NET para usar o novo pipeline](#use-new-pipeline) em vez de usar esta opção.
