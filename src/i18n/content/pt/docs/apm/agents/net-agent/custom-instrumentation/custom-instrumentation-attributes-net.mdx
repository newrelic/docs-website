...
Os métodos decorados com o atributo `[Transaction]` só criarão uma nova transação quando já existir uma. Quando um método decorado com `[Transaction]` é chamado a partir de <DNT>**within**</DNT> uma transação iniciada anteriormente, ele será tratado como o atributo `[Trace]` e fornecerá mais informações sobre a transação existente.
