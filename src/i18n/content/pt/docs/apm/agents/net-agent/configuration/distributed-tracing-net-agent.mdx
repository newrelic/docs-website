...
<CollapserGroup>
  <Collapser id="calling-service" title="Instrumento o serviço de chamada">
    ...
    1. Certifique-se de que a mensagem de saída seja enviada dentro do contexto de uma transação. [instrumentação personalizada](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/) pode ser necessária para isso.
    2. Invoque a chamada da API do agente para inserir dados de rastreamento na mensagem que está sendo enviada (consulte a [API do agente .NET](/docs/agents/net-agent/net-agent-api/itransaction#InsertDistributedTraceHeaders)).
  </Collapser>

  <Collapser id="called-service" title="Instrumento o serviço chamado">
    Para instrumentalizar o serviço chamado:

    1. Certifique-se de que a mensagem recebida seja recebida dentro do contexto de uma transação. [instrumentação personalizada](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/) pode ser necessária para isso.
    2. Invoque a chamada da API do agente para aceitar dados de rastreamento da mensagem recebida (consulte a [API do agente .NET](/docs/agents/net-agent/net-agent-api/itransaction#AcceptDistributedTraceHeaders)).
  </Collapser>
</CollapserGroup>