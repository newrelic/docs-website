...
Quando o agente .NET é anexado ao aplicativo de altas taxas de transferência <DoNotTranslate>**.NET Framework**</DoNotTranslate> , alguns clientes relataram observar alta contenção de threads. Ao revisar os dumps do processo e o rastreamento de pilha, muitos threads podem ser bloqueados por chamadas para `AppDomain.GetData()`.
