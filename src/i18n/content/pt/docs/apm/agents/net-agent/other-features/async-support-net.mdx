...
<CollapserGroup>
  ...
  >
    [Os serviços WCF](/docs/agents/net-agent/instrumentation/instrumenting-wcf-applications) hospedados pelo IIS não aninham corretamente o segmento <DoNotTranslate>**WCF**</DoNotTranslate> no segmento `ExecuteRequestHandler` . Os dois segmentos parecerão irmãos dentro do rastreamento da transação, embora [o tempo total](/docs/data-analysis/user-interface-functions/response-time#response-time-total-time) relatado seja preciso.
  </Collapser>
