...
<CollapserGroup>
  <Collapser id="HttpClient" title="Métodos assíncronos HttpClient">
  ...
  <Collapser id="SqlCommand" title="Métodos assíncronos SqlCommand">
  ...
  <Collapser id="SqlDataReader" title="Métodos assíncronos SqlDataReader">
  ...
  <Collapser id="NpgsqlCommand" title="Métodos assíncronos NpgsqlCommand (Postgres)">
...
<CollapserGroup>
  <Collapser id="response-time1" title="O tempo de resposta é menor que o tempo total">
  ...
  <Collapser id="asp-pipeline" title="Requer pipeline ASP atualizado">
  ...
  <Collapser id="task-not-void" title="Tipo de retorno Requisitos de tarefa">
  ...
  <Collapser id="begin-end-style" title="Nenhuma instrumentação para o estilo inicial e final">
  ...
  <Collapser id="manual-created-threads-capture" title="Métricas/segmentos com escopo definido em threads criados manualmente">
    O agente .NET não captura métricas ou segmentos com escopo definido em threads criados manualmente pelo seu aplicativo.
  </Collapser>

  <Collapser id="async-await" title="Métodos assíncronos instrumentados devem usar wait, não Task.Result">
  ...
  <Collapser id="continue-with-timing" title="O bloco ContinueWith pode afetar as medições de tempo">
  ...
  <Collapser id="iis-wcf-nesting" title="Problema com o aninhamento em aplicativos WCF hospedados no IIS">
    [Os serviços WCF](/install/dotnet/?deployment=WCF) hospedados no IIS não aninham corretamente o segmento <DNT>**WCF**</DNT> sob o segmento `ExecuteRequestHandler` . Os dois segmentos parecerão irmãos dentro de um trace de transação, embora [o tempo total](/docs/data-analysis/user-interface-functions/response-time#response-time-total-time) relatado seja preciso.
  </Collapser>

  <Collapser id="stack-traces-segments" title="Segmentos não criam rastreamento de pilha automaticamente">
