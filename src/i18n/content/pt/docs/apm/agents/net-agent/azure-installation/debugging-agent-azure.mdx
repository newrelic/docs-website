...
Depois de [instalar o agente .NET da New Relic para Azure](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/), você terá um ou mais dos seguintes problemas:
...
* Certifique-se de que as [etapas de instalação e execução foram seguidas](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/).

* Você

  <DoNotTranslate>**must**</DoNotTranslate>

  [desativa o aplicativo Insights](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) para que o agente .NET funcione corretamente.
...
4. Anote o ID do processo (PID) em que seu aplicativo está sendo executado, para que você possa verificar se um log está sendo criado para esse processo. Você pode encontrar o PID usando

   <DoNotTranslate>**Process Explorer**</DoNotTranslate>
...
2. Em
   ...
   , encontre o processo que você está tentando monitor e anote o número na coluna PID. Se você estiver tentando monitor um aplicativo Web, o nome do processo provavelmente será `w3wp.exe` sem o rótulo SCM, que se refere ao próprio processo Kudu. O agente não deve instrumentalizar o processo SCM (Kudu).
...
1. Em
   ...
   , localize o processo do seu aplicativo e clique no botão `Properties` .
...
<CollapserGroup>
  ...
  >
    As variáveis de ambiente que aparecem na guia <DoNotTranslate>**Environment**</DoNotTranslate> no <DoNotTranslate>**Process Explorer**</DoNotTranslate> do Kudu devem ser semelhantes a estas:
  ...
  >
    Para o pacote NuGet, as variáveis de ambiente [devem ser adicionadas como configurações do aplicativo](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/#nuget-install-net-framework).
    ...
    Se você fez alguma alteração nas variáveis de ambiente, reinicie seu aplicativo por meio do Portal do Azure. Em seguida, inspecione o novo PID do processo em <DoNotTranslate>**Process Explorer**</DoNotTranslate> para garantir que as alterações foram bem-sucedidas.
  </Collapser>
  ...
  >
    Para o pacote NuGet, as variáveis de ambiente [devem ser adicionadas como configurações do aplicativo](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/#nuget-install-net-core). As variáveis de ambiente que aparecem na guia <DoNotTranslate>**Environment**</DoNotTranslate> no <DoNotTranslate>**Process Explorer**</DoNotTranslate> do Kudu devem ser semelhantes a estas:
  ...
  >
    Para o pacote NuGet, as variáveis de ambiente [devem ser adicionadas como configurações do aplicativo](/docs/apm/agents/net-agent/azure-installation/install-net-agent-azure-web-apps/#nuget-install-net-core).
    ...
    Certifique-se de que o nó <DoNotTranslate>**log**</DoNotTranslate> do seu arquivo `newrelic.config` inclua um atributo de diretório. Se o Visual Studio impedir que você edite o arquivo `newrelic.config` que foi adicionado ao seu projeto pelo NuGet, será necessário fazer uma cópia local dele no seu aplicativo:
