...
O agente .NET obtém sua configuração do arquivo `newrelic.config` , que é gerado como parte do [processo de instalação](/docs/agents/net-agent/installation//install/dotnet-agent). Por padrão, apenas um arquivo `newrelic.config` global é criado, mas você também pode criar arquivos `newrelic.config` locais do aplicativo para um controle mais preciso sobre um sistema com vários aplicativos. Outras maneiras de definir opções de configuração incluem: usar variáveis de ambiente ou definir a configuração no lado do servidor a partir da interface. Para obter mais informações sobre as diversas opções de configuração e o que substitui o quê, consulte [Precedência das configurações de configuração](/docs/agents/net-agent/configuration/net-agent-configuration#config-options-precedence).
...
<Callout variant="important">
  <DoNotTranslate>**For IIS**</DoNotTranslate>: após alterar o arquivo `newrelic.config` ou `app.config` , execute um `IISRESET` em um prompt de comando administrativo. Os ajustes de nível de log não exigem redefinição.
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * As configurações `NewRelic.AgentEnabled` em `appsettings.json`

          <DoNotTranslate>**will enable the agent**</DoNotTranslate>

          .

        * As configurações `NewRelic.AgentEnabled` em um arquivo `web.config` ou `app.config`
...
<CollapserGroup>
  <Collapser
    ...
    title="Variáveis de ambiente do .NET framework"
  >
  ...
  <Collapser
    ...
    title="Variáveis de ambiente do .NET Core"
  >
    ...
    O instalador do agente Windows .NET os adicionará ao IIS por padrão ou como variáveis de ambiente de todo o sistema ao ativar `Instrument All`. Exceção: `CORECLR_ENABLE_PROFILING` precisa ser definido manualmente para que o aplicativo .NET Core não hospedado no IIS seja um instrumento. Consulte [Compatibilidade e requisitos do .NET framework](/docs/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework) para o aplicativo .NET framework .
...
<CollapserGroup>
  <Collapser
    ...
    title="Explicação do conflito profiler"
  >
...
<CollapserGroup>
  <Collapser
    ...
    title="Suporte a aplicativos de 32 bits em sistemas de 64 bits."
  >
...
Use essas opções para instalar e configurar seu agente por meio do arquivo newrelic.config. O agente .NET oferece suporte às seguintes categorias de opções de configuração:
...
<CollapserGroup>
  ...
  >
    ...
    Se for falso, o agente usará um encadeamento de pool para concluir o processamento da transação.
  ...
  >
    ...
    Se for falso, o agente tenta reutilizar a transação existente quando métodos instrumentados customizados são encontrados em cenários assíncronos/threaded.

    Se verdadeiro, o agente tenta criar uma nova transação quando métodos instrumentados customizados são encontrados em cenários assíncronos/threaded.
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `2000` ([`sendDataOnExit`](#service-sendDataOnExit) ativado)

            `120000` ([`sendDataOnExit`](#service-sendDataOnExit) desativado)
          </td>
        </tr>
...
<CollapserGroup>
  ...
  >
    ...
    Envie mensagem do log para o console, além do arquivo de log. Como alternativa, defina a variável de ambiente `NEWRELIC_LOG_CONSOLE` no ambiente do aplicativo.
  </Collapser>
  ...
  >
    ...
    Se desativado, nenhum registro será tentado. Destinado a sistemas de arquivos somente leitura. Como alternativa, defina a variável de ambiente `NEWRELIC_LOG_ENABLED` no ambiente do aplicativo.
  </Collapser>
  ...
  >
    ...
    O diretório para armazenar o arquivo de log gerado pelo agente. Se isso for omitido, um diretório chamado <DoNotTranslate>**logs**</DoNotTranslate> na área de instalação do agente New Relic será usado por padrão.

    Como alternativa, defina a variável de ambiente `NEWRELIC_LOG_DIRECTORY` no ambiente do aplicativo.
  </Collapser>
  ...
  >
    ...
    Define o número de arquivos de log do agente a serem retidos. Quando o número máximo de arquivos de log for atingido, o arquivo de log mais antigo será excluído. Um valor `0` especifica que não há número máximo de arquivos de log.
...
Se o rótulo do nome do host padrão na interface do usuário do APM não for útil, você poderá decorar esse nome na interface do usuário do New Relic com um nome de exibição. Depois que o processo do aplicativo for reiniciado e o agente .NET reportar novamente, o nome de exibição aparecerá na lista suspensa <DoNotTranslate>**Servers**</DoNotTranslate> . Essa configuração de nome de host não afeta a lista de hosts na [página <DoNotTranslate>**Summary**</DoNotTranslate> do seu aplicativo](/docs/apm/applications-menu/monitoring/apm-overview-page-view-transaction-apdex-usage-data).
...
<CollapserGroup>
  <Collapser
    id="host-name-config"
    title="Definir usando arquivo de configuração"
  >
  ...
  <Collapser
    ...
    title="Definir usando variável de ambiente"
  >
...
Um atributo é um par de valores principais que determina as propriedades de um evento ou transação. Cada atributo é enviado para o rastreamento APM da transação, rastreamento de erro APM, evento Transaction, evento TransactionError ou evento PageView. O elemento `attributes` principal ativa ou desativa a coleta de atributos para o agente .NET e define atributos específicos a serem coletados ou excluídos. Você também pode definir configurações de atributo com base em seu destino: [coleta de erros](#error-attributes), [rastreamento da transação](#tracer-attributes), [instrumentação do browser](#browser-attributes) e [evento de transação](#transaction-attributes).

Neste exemplo, o agente exclui todos os atributos cuja chave começa com <DoNotTranslate>**myApiKey**</DoNotTranslate> (<DoNotTranslate>**myApiKey.bar**</DoNotTranslate>, <DoNotTranslate>**myApiKey.value**</DoNotTranslate>), mas coleta o atributo personalizado <DoNotTranslate>**myApiKey.foo**</DoNotTranslate>.
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Ao usar a variável de ambiente `NEW_RELIC_ATTRIBUTES_INCLUDE` , o conjunto padrão de inclusões no newrelic.config é substituído. Veja [aqui](#allow-all-headers-enabled) uma lista detalhada de inclusões e exclusões padrão.
    </Callout>
  </Collapser>
  ...
  >
    ...
    <Callout variant="caution">
      Ao usar a variável de ambiente `NEW_RELIC_ATTRIBUTES_EXCLUDE` , o conjunto padrão de exclusões no newrelic.config é substituído. Veja [aqui](#allow-all-headers-enabled) uma lista detalhada de inclusões e exclusões padrão.
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="ignoreClasses"
  >
    Uma lista de nomes de classes totalmente qualificados a serem ignorados. O número máximo de combinações de classe de erro e mensagem que <DoNotTranslate>**SHOULD**</DoNotTranslate> serão relatadas é 50. Se mais de 50 estiverem listados, apenas os primeiros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate> serão usados
  </Collapser>
  ...
  >
    Um mapa opcional de nomes de classes totalmente qualificados para uma lista de strings que correspondem a uma substring da mensagem de erro. O número máximo de combinações de classe de erro e mensagem que <DoNotTranslate>**SHOULD**</DoNotTranslate> serão relatadas é 50. Se mais de 50 estiverem listados, apenas os primeiros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate> serão usados
  </Collapser>
  ...
  >
    ...
    Lista códigos de erro HTTP específicos para não reportar à New Relic. Você pode usar códigos de erro HTTP integrais padrão, como apenas 401, ou pode usar códigos de status completos da Microsoft com pontos decimais, como 401.4 ou 403.18. Os códigos de status devem ser iguais ou superiores a 400.
  ...
  >
    Uma lista de nomes de classes totalmente qualificados a serem marcados conforme esperado. O número máximo de combinações de classe de erro e mensagem que <DoNotTranslate>**SHOULD**</DoNotTranslate> serão relatadas é 50. Se mais de 50 estiverem listados, apenas os primeiros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate> serão usados
  </Collapser>
  ...
  >
    Uma lista separada por vírgulas de códigos de status. A lista pode incluir intervalos de números inteiros, usando um único traço (-) e incluirá o número inteiro inicial e final do intervalo.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Quando distributed tracing e/ou o rastreamento infinito estão habilitados, as informações dos eventos de transação são aplicadas ao evento Span raiz da transação. Considere aplicar quaisquer configurações de atributo para evento de transação para abranger o evento e/ou aplicá-las como configurações de atributo globais.
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    Por padrão, o agente injeta automaticamente o JavaScript do browser do agente. Para desativar a injeção automática, defina esse atributo como `false`.
  ...
  >
    Utilize este subelemento para evitar que o browser do agente seja injetado em páginas específicas. O elemento é usado da seguinte forma:
    ...
    O agente não injetará o browser do agente em páginas cuja URL corresponda a uma das expressões regulares especificadas. A expressão regular deve seguir [as diretrizes da Microsoft para a classe Regex](https://msdn.microsoft.com/en-us/library/system.text.regularexpressions.regex(v=vs.110).aspx).
...
<CollapserGroup>
  ...
  >
    ...
    O valor padrão é `apdex_f`, que define o limite para quatro vezes o valor <DoNotTranslate>**apdex_t**</DoNotTranslate> do aplicativo. Para obter mais informações sobre <DoNotTranslate>**apdex_t**</DoNotTranslate>, consulte [Apdex](/docs/site/apdex-measuring-user-satisfaction).
  ...
  >
    ...
    Por padrão, `maxStackTrace` é definido como 0, o que desativa o rastreamento de pilha como parte de um rastreamento da transação. Se esse valor for maior que 0, o rastreamento de pilha será capturado para rastreamento da transação.
  </Collapser>
...
<CollapserGroup>
  <Collapser
    ...
    title="Desativar evento span via arquivo de configuração"
  >
  ...
  <Collapser
    ...
    title="Desabilitar evento span via variável de ambiente"
  >
...
Para obter mais detalhes, consulte [nossos documentos sobre como usar o log do agente .NET no contexto](/docs/logs/logs-context/net-configure-logs-context-all).
...
<CollapserGroup>
  ...
  >
    Use este subelemento para ativar o encaminhamento do log do seu aplicativo para o New Relic. O valor padrão do atributo `enabled` é `true`. O valor padrão do atributo `maxSamplesStored` é `10000`.
    ...
    Use o atributo `logLevelDenyList` para filtrar a mensagem do log do nível de log especificado. O valor desse atributo é uma lista de níveis separados por vírgula e que não diferenciam maiúsculas de minúsculas. Fora do caso, a string do nível de log deve corresponder exatamente ao nível de log que está sendo filtrado; curingas e regex não são suportados. Por exemplo, se o valor for definido como `logLevelDenyList="debug,warn"`, qualquer mensagem do log com nível de log `Debug` ou `Warn` não será capturada ou encaminhada, mas um log com nível `Warning` ainda será enviado para New Relic.
  ...
  <Collapser
    id="contextData"
    title="Dados de contexto (atributo personalizado)"
  >
    ...
    Defina o atributo `enabled` como `true` para ativar a captura e o encaminhamento de dados de contexto de registro para o New Relic. O valor padrão é `false`.

    O atributo `include` é uma lista separada por vírgulas de nomes de atributos a serem incluídos quando a captura de dados de contexto estiver ativada. O valor padrão é uma string vazia, que significa “incluir tudo”.

    O atributo `exclude` é uma lista separada por vírgulas de nomes de atributos a serem excluídos quando a captura de dados de contexto está ativada. O valor padrão é uma string vazia, que significa "não excluir nada".
  ...
  <Collapser
    id="localDecorating"
    title="Decoração log locais"
  >
    Use este subelemento para ativar a decoração do log do seu aplicativo com metadados de vinculação do New Relic. O valor padrão do atributo `enabled` é `false`. Algumas alterações de configuração adicionais são necessárias para a decoração log local. Consulte [.NET: Configurar logs no contexto](/docs/logs/logs-context/net-configure-logs-context-all#2-decorate) para obter mais informações.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser
    ...
    title="Habilitar e desabilitar o agente"
  >
    ```xml
    <appSettings>
  ...
  <Collapser
    id="app-cfg-location"
    title="Alterar localização do newrelic.config"
  >
