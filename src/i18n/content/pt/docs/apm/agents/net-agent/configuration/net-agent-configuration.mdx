...
O elemento `applications` é filho do elemento `instrumentation`. O elemento `applications` suporta `application` elementos filhos que especificam quais aplicativos não web instrumentar. O elemento `application` contém um atributo `name`. Além disso, a partir da versão 10.48.0 do agente, há um atributo `include` opcional com um valor padrão de &quot;true&quot;. Defina esse atributo como &quot;false&quot; para impedir que o agente instrumente esse processo.
...
Alternativamente, a partir da versão 10.48.0 do agente, defina a(s) variável(is) de ambiente `NEW_RELIC_INCLUDED_APPLICATION_NAMES` e/ou `NEW_RELIC_EXCLUDED_APPLICATION_NAMES` no ambiente do aplicativo:
...
Observe que a exclusão tem prioridade sobre a inclusão.
...
<CollapserGroup>
  ...
  <Collapser id="tracer-sqlMetadataCommentsEnabled" title="sqlMetadataCommentsEnabled">
    ...
    Quando definido como `true`, o agente adiciona um comentário SQL que contém o GUID da entidade de serviço ao início de cada instrução SQL instrumentada antes que o banco de dados execute a instrução. O comentário tem o seguinte formato:

    ```sql
    /*nr_service_guid="YOUR_ENTITY_GUID"*/
    ```

    Usar esta opção para correlacionar os dados do APM com o [monitoramento de desempenho de consulta](/docs/infrastructure/infrastructure-data/query-performance-monitoring). O agente altera o texto da instrução que envia para o banco de dados, para que o banco de dados receba o comentário completo.

    O agente também registra o texto comentado. O comentário aparece em trace de consulta lenta, em segmentos de trace da transação e no atributo `db.statement` de eventos de span. Com o valor padrão de [`recordSql`](#tracer-recordSql) de `obfuscated`, o agente substitui o GUID da entidade entre aspas por `?`, então esses registros mostram `/*nr_service_guid=?*/`. Definir `recordSql` como `raw` para manter o GUID da entidade nesses dados do agente.

    Esta opção está disponível a partir da versão 10.52.0 do agente .NET. Também pode ser definida com a variável de ambiente `NEW_RELIC_TRANSACTION_TRACER_SQL_METADATA_COMMENTS_ENABLED`.

    <Callout variant="important">
      Esta opção altera o texto de cada instrução SQL instrumentada. Um banco de dados que armazena em cache planos de consulta ou instruções preparadas por texto de instrução vê cada instrução comentada como uma nova instrução. Isso pode diminuir a taxa de ocorrência no cache e aumentar o trabalho de planejamento de consulta no banco de dados. O efeito é maior quando mais de um serviço consulta o mesmo banco de dados, porque cada serviço adiciona um GUID de entidade diferente à mesma instrução.

      Recomenda-se habilitar esta opção em um ambiente de teste primeiro e medir o efeito no banco de dados antes de habilitá-la em produção.
    </Callout>
  </Collapser>
...
O elemento `<traceIdRatioBased>` tem um atributo obrigatório `ratio`, que é um valor entre `0.0` e `1.0` especificando a porcentagem de traces que devem ser amostrados.
...
### Métrica Kafka [#kafka\_metrics][#kafka_metrics]

O elemento `kafka` é filho do elemento `configuration`. Use `kafka` para configurar as métricas do Kafka que o agente coleta dos produtores e consumidores do Kafka do seu aplicativo.

Suportado no agente .NET v10.55.0 e superior.

O elemento `kafka` suporta o subelemento `metrics`. Todos os atributos de `metrics` são opcionais.
...
Você também pode configurar esses atributos com variáveis de ambiente:
...
O subelemento `metrics` suporta os seguintes atributos:
...
<CollapserGroup>
  <Collapser id="kafka-metrics-debug-enabled" title="debugEnabled">
    ...
    Ative ou desative a coleta das métricas internas do Kafka que o agente cria a partir das estatísticas que o Kafka relata a ele.

    Este atributo tem precedência sobre a configuração do aplicativo `NewRelic.KafkaInternalMetricsEnabled`. Se você não definir `debugEnabled`, o agente ainda respeitará essa configuração do aplicativo.

    Ou use a variável de ambiente `NEW_RELIC_KAFKA_METRICS_DEBUG_ENABLED` .
  </Collapser>

  <Collapser id="kafka-metrics-interval" title="intervalo">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Este atributo define o intervalo, em segundos, no qual o Kafka relata estatísticas ao agente.

    O valor mínimo é de 5 segundos. O agente eleva um valor menor para 5.

    Se você não definir este atributo, o agente derivará o intervalo do ciclo de coleta de métricas. Se você configurar um intervalo de estatísticas no próprio cliente Kafka, esse valor terá precedência sobre este atributo.

    Ou use a variável de ambiente `NEW_RELIC_KAFKA_METRICS_INTERVAL` .
  </Collapser>
  ...
  <Collapser id="kafka-metrics-cluster-metrics-enabled" title="clusterMetricsEnabled">
    ...
    Ative ou desative as métricas de produção e consumo do Kafka por cluster. Defina este atributo como `true` para que o agente registre as métricas do cluster.

    Ou use a variável de ambiente `NEW_RELIC_KAFKA_METRICS_CLUSTER_METRICS_ENABLED` .
  </Collapser>
</CollapserGroup>
