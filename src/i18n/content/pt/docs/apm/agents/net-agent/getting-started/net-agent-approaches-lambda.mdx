...
Os seguintes tipos de função do AWS Lambda são detectados e instrumentados automaticamente sem nenhuma configuração adicional (a partir da versão 10.29.0 do agente):

* `Amazon.Lambda.AspNetCoreServer.APIGatewayProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.APIGatewayHttpApiV2ProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.ApplicationLoadBalancerFunction`

Outros tipos de handlers (como SQS, SNS, Kinesis, S3, gatilhos do DynamoDB e handlers personalizados) também são instrumentados quando você define a variável de ambiente `NEW_RELIC_LAMBDA_HANDLER` para o caminho totalmente qualificado do handler (por exemplo, `MyAssembly::MyNamespace.MyClass::MyMethod`). Isso inclui qualquer método manipulador que aceite um parâmetro `ILambdaContext`.
...
* O distributed tracing de entrada do SQS exige que o produtor upstream injete os cabeçalhos de trace do W3C (`traceparent`, `tracestate`) como atributos de mensagem do SQS. O agente os lê automaticamente quando presentes.
