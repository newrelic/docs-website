...
## Verifique a versão do seu agente .NET [#your_version]

1. Consulte as [notas de versão](/docs/releases/dotnet) para encontrar a versão mais recente do agente.

2. Verifique a versão do agente .NET que você instalou atualmente usando

   <DoNotTranslate>**Windows Programs and Features**</DoNotTranslate>

   ou usando seu gerenciador de pacote Linux.
...
## Atualizar o agente .NET (Windows) [#updating_net]

<Callout variant="important">
  Antes de atualizar:

  * A atualização do agente requer direitos

    <DoNotTranslate>**Administrator**</DoNotTranslate>

    no seu grupo de administradores do Windows. Para obter mais informações, consulte a [documentação de permissões](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework/#user-rights).
  ...
  * Se seu aplicativo estiver hospedado no IIS, certifique-se de

    <DoNotTranslate>**stop IIS**</DoNotTranslate>

    antes de atualizar o agente .NET.
</Callout>
...
<CollapserGroup>
  ...
  >
    1. Certifique-se de ter [o .NET 4.5 ou superior instalado em seu sistema](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework/#net-version).

    2. Pare o IIS.

    3. Baixe e execute o pacote de instalação MSI apropriado para sua [arquitetura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework/#architecture):
...
## Atualizar o agente .NET (Linux) [#updating_net_core]
...
<CollapserGroup>
  <Collapser
    ...
    title="arquivo tar.gz ou deb"
  >
    1. Desligue seu(s) aplicativo(s).
    2. Baixe o arquivo apropriado para sua [arquitetura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-core/#architecture) no [site de download](https://download.newrelic.com/dot_net_agent/latest_release/).
  ...
  <Collapser
    id="yum"
    title="yum (CentOS, Oracle Linux ou RHEL)"
  >
    1. Desligue seu(s) aplicativo(s).
  ...
  <Collapser
    id="apt"
    title="apt (Debian, Linux Mint ou Ubuntu)"
  >
    1. Desligue seu(s) aplicativo(s).
  ...
  <Collapser
    id="rpm"
    title="rpm (CentOS, Oracle Linux ou RHEL)"
  >
    1. Desligue seu(s) aplicativo(s).

    2. Faça download do arquivo rpm apropriado para sua [arquitetura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-core/#architecture) no [site de download](https://download.newrelic.com/dot_net_agent/latest_release/).
...
## Atualizar agente do .NET framework mais antigo (inferior a 7.0) [#updating_older_net]
...
<CollapserGroup>
  <Collapser
    ...
    title="Detalhes da versão do agente legado do .NET framework"
  >
    <Callout variant="important">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr id="50">
          ...
          <td>
            ...
            * Você está atualizando do agente do .NET framework para a versão 5.0 ou superior.
            ...
            2. Adicione este `appSetting` ao seu <DoNotTranslate>**web.config**</DoNotTranslate>:
        ...
        <tr id="500">
          ...
          <td>
            Métricas que não fazem parte de uma transação não serão exibidas na interface. <DoNotTranslate>**Exception:**</DoNotTranslate> Você pode visualizar [o banco de dados](/docs/apm/apm-ui-pages/monitoring/databases-page-view-operations-throughput-response-time/) e as métricas [externas](/docs/apm/apm-ui-pages/monitoring/external-services/external-services-intro/) em suas respectivas páginas na interface do APM. Outras métricas desse tipo (como métodos instrumentados) podem se tornar visíveis "envolvendo-as" em uma [transação personalizada](/docs/apm/agents/net-agent/custom-instrumentation/create-transactions-xml-net/).
        ...
        <tr id="220">
          ...
          <td>
            A versão 2.19.3.0 foi a última versão do agente .NET que exigia a chamada de API `GetBrowserTimingFooter()` e `GetBrowserTimingHeader()` para <InlinePopover type="browser"/>. Na versão do agente 2.20.24.0 ou superior, você só precisa ligar para `GetBrowserTimingHeader()`. `GetBrowserTimingFooter()` não tem efeito. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Remova essas referências do seu código.
          </td>
        </tr>
