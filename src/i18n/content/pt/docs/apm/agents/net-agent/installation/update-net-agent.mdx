...
## Verifique a versão do seu agente .NET [#your_version][#your_version]
...
## Atualizar o agente .NET (Windows) [#updating_net][#updating_net]

<Callout variant="important">
  Antes de atualizar:

  * A atualização do agente requer direitos <DNT>**Administrator**</DNT> no seu grupo de administradores do Windows. Para obter mais informações, consulte a [documentação de permissões](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework/#user-rights).
...
## Atualizar o agente .NET (Linux) [#updating_net_core][#updating_net_core]
...
## Atualizar agente do .NET framework mais antigo (inferior a 7.0) [#updating_older_net][#updating_older_net]
