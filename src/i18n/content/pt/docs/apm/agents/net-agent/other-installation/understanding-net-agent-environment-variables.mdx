...
## Defina variáveis de ambiente para aplicativos hospedados no IIS [#iis]

Se o aplicativo estiver hospedado no IIS, é possível definir variáveis de ambiente por pool de aplicativos editando o arquivo `ApplicationHost.config` do IIS diretamente, em vez de (ou além de) defini-las como variáveis de ambiente em todo o sistema. Isso é útil quando se deseja ativar ou configurar o agente .NET apenas para pools de aplicativos específicos, em vez de para todos os processos na máquina.

Por padrão, o `ApplicationHost.config` é encontrado em `C:\Windows\System32\inetsrv\config\ApplicationHost.config`. Dentro da seção `<applicationPools>`, cada elemento `<add>` representando um pool de aplicativos pode conter um elemento filho `<environmentVariables>`, que por sua vez contém entradas `<add name="..." value="..." />` para cada variável de ambiente a ser definida para esse pool de aplicativos.

Por exemplo, para ativar o agente .NET e definir algumas opções de configuração comuns para um pool de aplicativos chamado `TestApplication`:
...
Também é possível utilizar `applicationPoolDefaults` para definir variáveis de ambiente que se aplicam a todos os pools de aplicativos por padrão, da mesma forma que as variáveis de ambiente de todo o sistema fariam, a menos que as configurações `environmentVariables` do próprio pool as substituam.

<Callout title="Observação">
  A edição direta de `ApplicationHost.config` requer privilégios de administrador, e é necessário reciclar um pool de aplicativos (ou reiniciar o IIS) para que as alterações entrem em vigor. Sempre faça backup do arquivo antes de editá-lo, pois uma configuração malformada pode impedir que o IIS inicie sites ou pools de aplicativos.
</Callout>

Para obter mais detalhes sobre esse elemento de configuração, consulte a documentação da Microsoft sobre [applicationPoolDefaults environmentVariables](https://learn.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/applicationpooldefaults/environmentvariables/).