...
<CollapserGroup>
  ...
  <Collapser id="cfg-application_logging_forwarding_labels-exclude" title="newrelic.application_logging.forwarding.labels.exclude">
    ...
    <Callout variant="tip">
      O valor INI `newrelic.labels` permite definir rótulos que contêm o caractere vírgula (&amp;quot;,&amp;quot;). Esses rótulos não podem ser filtrados usando o valor INI `exlude` , pois ele depende de vírgulas para separar as regras de exclusão. É recomendável não usar o caractere vírgula nos valores dos rótulos caso esses rótulos não sejam adicionados à mensagem do log.
    </Callout>
  </Collapser>
</CollapserGroup>
