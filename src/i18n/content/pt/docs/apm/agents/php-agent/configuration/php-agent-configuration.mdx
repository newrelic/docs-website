...
Este documento explica como configurar o agente PHP do New Relic **após a instalação**.

## Antes de você começar [#before-you-begin]

Se você ainda não instalou o agente PHP, consulte nossa [visão geral da instalação do agente PHP](/docs/apm/agents/php-agent/installation/php-agent-installation-overview) para:

* Baixar o agente
* Concluir o processo de instalação

Após a instalação do agente, retorne aqui para ajustar suas configurações.
...
<CollapserGroup>
  ...
  <Collapser id="inivar-err-enabled" title="newrelic.error_collector.enabled">
    ...
    Habilitar ou desabilitar o coletor de erros. Quando habilitado, o agente coleta e relata erros e exceções do PHP para a interface do usuário do New Relic.

    ### Como o agente PHP coleta erros e exceções

    O agente PHP coleta automaticamente erros e exceções das seguintes maneiras:

    * **Exceções não capturadas**: O agente captura automaticamente quaisquer exceções não capturadas (instâncias de `Exception` no PHP 5 ou `Throwable` no PHP 7+) que ocorrem durante a execução da transação.
    * **Erros PHP**: O agente captura erros PHP com base nos níveis de erro configurados (consulte [`error_collector.ignore_errors`](#inivar-err-ignore-errors) abaixo).
    * **Relatório manual de erros**: Use a API [`newrelic_notice_error()`](/docs/agents/php-agent/php-agent-api/newrelic_notice_error) para relatar manualmente erros ou exceções que não são capturados automaticamente, como exceções tratadas ou condições de erro personalizadas.
