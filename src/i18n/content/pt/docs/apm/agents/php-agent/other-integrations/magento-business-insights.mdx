---
title: Insights de negócios Magento
tags:
...
Nosso dashboard de insights de negócios Magento fornece uma visão abrangente do desempenho do seu aplicativo de comércio eletrônico Magento. Com o dashboard, você pode acompanhar facilmente as principais métricas de negócios, como receitas em risco, pedidos em risco devido a período de inatividade e baixo desempenho do sistema. Você também pode usar o dashboard para diagnosticar problemas no desempenho do seu aplicativo e otimizar seu código.
...
<figcaption>
  Nosso dashboard de insights de negócios Magento oferece uma visão central dos dados do seu aplicativo Magento.
</figcaption>

Para iniciar o monitoramento do seu aplicativo Magento no New Relic, siga estas etapas para enviar os dados do seu aplicativo para o New Relic e visualizá-los no dashboard de insights de negócios do Magento.

<Steps>
  <Step>
    ## Instalar o agente de infraestrutura [#install-the-infra-agent]

    Nosso agente de infraestrutura monitora seu host e reporta métricas como uso de CPU, uso de memória, uso de armazenamento, uso de disco e tráfego de rede.

    Para instalar o agente, escolha um destes métodos de instalação:

    * Siga as etapas de nossa [instalação guiada](https://one.newrelic.com/launcher/nr1-core.explorer?pane=eyJuZXJkbGV0SWQiOiJucjEtY29yZS5saXN0aW5nIn0=&cards%5B0%5D=eyJuZXJkbGV0SWQiOiJucjEtaW5zdGFsbC1uZXdyZWxpYy5ucjEtaW5zdGFsbC1uZXdyZWxpYyIsImFjdGl2ZUNvbXBvbmVudCI6IlZUU09FbnZpcm9ubWVudCIsInBhdGgiOiJndWlkZWQifQ==) para um processo de instalação rápido e fácil.
    * [Instale manualmente o agente](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent/) para um processo de instalação mais avançado.
  </Step>

  <Step>
    ## Instale o agente PHP [#install-the-PHP-agent]

    Nosso agente PHP relata métricas relacionadas ao aplicativo e ao browser, como desempenho AJAX, taxas de erros, duração, core web vitals e taxas de transferência.

    Para instalar o agente, escolha um destes métodos de instalação:

    * Siga as etapas de nossa [instalação guiada](https://one.newrelic.com/launcher/nr1-core.settings?pane=eyJuZXJkbGV0SWQiOiJ0dWNzb24ucGxnLWluc3RydW1lbnQtZXZlcnl0aGluZyJ9&cards%5B0%5D=eyJuZXJkbGV0SWQiOiJzZXR1cC1uZXJkbGV0cy5zZXR1cC1waHAtaW50ZWdyYXRpb24iLCJhY2NvdW50SWQiOjI2NDA0MDl9&platform%5BaccountId%5D=1) para um processo de instalação rápido e fácil.
    * [Instale manualmente o agente](/docs/apm/agents/php-agent/installation/php-agent-installation-overview/) para um processo de instalação mais avançado.
  </Step>

  <Step>
    ## Instale a integração MySQL [#install-the-mysql-integration]

    Nossa integração MySQL monitora seus bancos de dados e coleta métricas sobre a utilização e conexões do banco de dados, como consulta mais lenta e conexões ativas por nó.

    Para instalar a integração:

    1. Vá para

       <DNT>**[one.newrelic.com](https://one.newrelic.com/) > Integrations & Agents**</DNT>

       .

    2. Pesquise e clique em

       <DNT>**MySQL**</DNT>

       .

    3. Selecione uma conta e siga os passos da instalação guiada.

    Caso precise instalar o agente de uma forma diferente, veja [Integração de monitoramento MySQL](/install/mysql/).
  </Step>

  <Step>
    ## Instale a integração Apache [#install-the-apache-integration]

    Nossa integração Apache coleta métricas de rede como conexões TCP, pesquisa de DNS, HTTPS, SSL e status de servidor e trabalhador.

    Para instalar a integração, siga as etapas de nossa [instalação guiada](https://one.newrelic.com/launcher/nr1-core.explorer?pane=eyJuZXJkbGV0SWQiOiJucjEtY29yZS5saXN0aW5nIn0=&cards%5B0%5D=).

    Caso precise instalar a integração de uma forma diferente, veja [Integração de monitoramento do Apache](/docs/infrastructure/host-integrations/host-integrations-list/apache-monitoring-integration).
  </Step>

  <Step>
    ## Instale a integração do Elasticsearch [#install-the-elasticsearch-integration]

    Nossa integração do Elasticsearch coleta métricas como status do cluster e fragmentos atuais. Se você estiver usando [Magento versão 2.4 e superior](https://magentip.com/install-magento-2-4-x-on-ubuntu-with-elasticsearch/), precisará instalar nossa integração Elasticsearch.
    ...
    2. Pesquise e clique em
       ...
       <DNT>**Elasticsearch**</DNT>

       .

    3. Selecione uma conta e siga os passos da instalação guiada.

    Caso precise instalar a integração de uma forma diferente, consulte [Integração de monitoramento do Elasticsearch](/docs/infrastructure/host-integrations/host-integrations-list/elasticsearch/elasticsearch-integration).
  </Step>

  <Step>
    ## Configurar monitoramento sintético [#synthetic-monitoring]

    Nosso monitor Sintético testa seu aplicativo em diferentes locais e captura o período de inatividade do servidor. Você pode visualizar esses dados no dashboard de insights de negócios do Magento.

    Para configurar o monitor Sintético, siga os passos de nossa [instalação guiada](https://onenr.io/0EjOXrnxgQ6).
  </Step>

  <Step>
    ## Crie uma integração Flex [#install-the-flex-integration]

    Nossa integração Flex customizada permite capturar métricas de negócios através da consulta ao banco de dados do seu aplicativo.

    Para criar sua integração Flex:

    1. Crie um arquivo de configuração `nri-flex` :

       * Caminho para o arquivo de configuração: `/etc/newrelic-infra/integrations.d/`
       * Nome do arquivo de configuração: `mysql-database-config.yml`

    2. Adicione o seguinte trecho de código ao arquivo de configuração. Certifique-se de alterar a substituição `USERNAME`, `PASSWORD`, AND `DATABASE_NAME`:
  ...
  <Step>
    ## Instale o dashboard de insights de negócios Magento [#install-dashboard]

    1. Vá para

       <DNT>**[one.newrelic.com > Integrations & Agents](https://one.newrelic.com) > Dashboards**</DNT>

       .

    2. Na barra de pesquisa, digite “Magento business insights”. O dashboard deve aparecer.

    3. Clique nele para instalar.

    Seu dashboard Magento é considerado um painel personalizado e pode ser encontrado na interface <DNT>**Dashboards**</DNT>. Para obter informações sobre como usar e editar o painel, consulte [nossa documentação dashboard ](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards).
  </Step>
</Steps>

## Consulte seus dados Magento [#query-data]

Interessado em consultar os dados do seu aplicativo Magento? Aqui está um exemplo de consulta NRQL verificando o total de clientes registrados em sua plataforma Magento:
