...
Para ver um aplicativo de exemplo, acesse o [Fórum de suporte da New Relic](https://knowledge.newrelic.com/s/article/PHP-Agent-And-Daemon-Containers). Se estiver usando contêineres de aplicativo de curta duração, recomendamos o uso de um contêiner separado para o daemon do agente PHP.
...
<Callout variant="caution">
  Por padrão, a primeira transação faz com que o agente acione o início de um daemon e a inicialização da conexão do aplicativo. Por motivos de desempenho, o agente não aguarda a conclusão dessas operações antes de inicializar a conexão. Isso pode resultar na perda das primeiras transações após o início de um contêiner. Para evitar essa perda, defina as entradas `newrelic.daemon.start_timeout` e `newrelic.daemon.app_connect_timeout` no arquivo `newrelic.ini` para os nossos [valores recomendados](https://support.newrelic.com/s/hubtopic/aAX8W0000008bSo/troubleshooting-frameworks) de 5s e 15s, respectivamente.
</Callout>
...
<Callout variant="tip">
  ...
  * [Monitoramento de contêineres que executam um único script PHP](https://support.newrelic.com/s/hubtopic/aAX8W0000008aNC/relic-solution-single-php-script-docker-containers)
  * [Dicas de resolução de problemas para o agente PHP](https://support.newrelic.com/s/hubtopic/aAX8W0000008bSo/troubleshooting-frameworks)
</Callout>
