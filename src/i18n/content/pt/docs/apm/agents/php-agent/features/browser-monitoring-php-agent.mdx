...
## Use ou desative a instrumentação automática [#auto\_instrumentation][#auto_instrumentation]
...
## Instrumento manualmente via API do agente [#manual\_instrumentation][#manual_instrumentation]
...
### Adicione atributos personalizados, como nonce [#nonce-attribute]

Se o seu site usa uma [Política de Segurança de Conteúdo (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) que requer um atributo `nonce`, você pode passar `false` para ambas as funções da API, para que elas retornem a string JavaScript bruta sem as tags `<script>`. Em seguida, você pode criar as tags com os atributos obrigatórios:
...
Passar `false` como argumento suprime as tags de script que as funções normalmente geram, para que você possa envolver a string JavaScript bruta com suas próprias tags `<script>` com quaisquer atributos necessários.
...
## Instrumento manual para vários frameworks [#manual\_generic][#manual_generic]
