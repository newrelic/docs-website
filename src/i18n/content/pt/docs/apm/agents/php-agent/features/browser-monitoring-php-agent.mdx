...
<CollapserGroup>
  ...
  >
    ...
    2. Insira uma chamada para gerar o rodapé logo antes da última tag de fechamento do layout:
       ...
       <Callout variant="important">
         Drupal 10.2 introduced a [new change](https://www.drupal.org/node/3298551) that causes it to set a `content-length` header. New Relic PHP agent is unable to auto-inject the browser auto-instrumentation when the HTTP header field `Content-Length` is set. To keep using browser monitoring, disable browser auto-instrumentation and manually insert the JavaScript header and footer into your templates.
       </Callout>
  </Collapser>
</CollapserGroup>
