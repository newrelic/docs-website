...
<CollapserGroup>
  ...
  >
    ...
    Pode ser definido na linha de comando pela opção do daemon `-l` . Definir esse valor na linha de comando substituirá o valor definido em `newrelic.cfg` Embora o próprio daemon não forneça um nome padrão para o arquivo de log, o script de inicialização do daemon usa a opção <DoNotTranslate>**-l**</DoNotTranslate> para definir o local e o nome padrão como `/var/log/newrelic/newrelic-daemon.log`.
  </Collapser>
  ...
  >
    ...
    * `debug`
      ...
      Se você estiver tendo problemas com o daemon, configure o nível de log <DoNotTranslate>**debug**</DoNotTranslate> pelo menos por tempo suficiente para reproduzir o problema. Um arquivo de log <DoNotTranslate>**debug**</DoNotTranslate> geralmente é necessário para depurar problemas com o daemon de proxy.
  ...
  >
    ...
    * <DoNotTranslate>**Recommended.**</DoNotTranslate> Para usar um arquivo especificado como um soquete de domínio UNIX (UDS), forneça um nome de caminho <DoNotTranslate>**absolute**</DoNotTranslate> como uma string.
