...
* Padrão (amostragem baseada no início): Antes de qualquer trace chegar, determinamos uma porcentagem definida de traces para aceitar e analisar. Isso oferece um ponto de partida sólido para ver como o rastreamento pode ajudar você. Está ativado por padrão no agente Python da New Relic. Por padrão, o agente Python amostra 10 traces por minuto, com um limite máximo de 2000 spans por minuto.
