...
## Decoradores e introspecção [#decorators_and_introspection]
...
Para garantir que os módulos e funções sejam nomeados corretamente, use o decorador `wraps()` do módulo <DoNotTranslate>**functools**</DoNotTranslate> da biblioteca padrão para encapsular a função do decorador interno:
...
## Identificando uma transação [#identifying_a_transaction]
...
O valor usado para um parâmetro personalizado pode ser qualquer valor que possa ser serializado pelo codificador JSON fornecido pelo módulo <DoNotTranslate>**json**</DoNotTranslate> . Isso inclui tuplas, listas e dicionários.

<DoNotTranslate>**Recommendation:**</DoNotTranslate> Restrinja os valores a valores inteiros, float ou string. Se você usar estruturas de dados mais complexas, evite tentar associar uma estrutura de dados muito grande, pois ela precisará ser armazenada em cache até ser colhida, empacotada e então enviada para o aplicativo principal.
...
## Gravação métrica personalizada [#recording_custom_metrics]
...
Para registrar métrica personalizada, utilize a função `record_custom_metric()` do módulo <DoNotTranslate>**newrelic.agent**</DoNotTranslate> . São necessários dois argumentos, sendo o nome da métrica e o valor. O valor deve ser um valor inteiro ou de ponto flutuante.
