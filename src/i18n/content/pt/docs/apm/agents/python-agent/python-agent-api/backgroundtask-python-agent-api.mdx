...
* <DNT>**The context manager**</DNT>: se as tarefas a serem monitoradas forem determinadas apenas no tempo de execução (e não no tempo de importação), você poderá usar o gerenciador de contexto `BackgroundTask` para agrupar diretamente a execução de um bloco de código. (Este é o gerenciador de contexto usado por `BackgroundTaskWrapper`.)
...
* <DNT>**The path-based wrapper**</DNT>: o formulário wrapper baseado em caminho é `wrap_background_task`.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Obrigatório. O nome da transação. Pode ser uma função que aceita um parâmetro callable\_name. O valor padrão é `None`. Por padrão, o nome será o nome da função à qual o decorator foi aplicado, mas você pode sobrescrever isso fornecendo este nome.
      </td>
    </tr>
...
Um exemplo de uso de `BackgroundTask` com um nome dinâmico:
