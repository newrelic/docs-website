...
Se estiver usando `sudo` para inicializar seu aplicativo WSGI como outro usuário, esteja ciente de que a configuração padrão <DoNotTranslate>**sudo**</DoNotTranslate> geralmente será tal que as variáveis de ambiente do usuário do shell no qual <DoNotTranslate>**sudo**</DoNotTranslate> é executado serão ignoradas. Nesse caso, você precisaria alterar a configuração <DoNotTranslate>**sudoers**</DoNotTranslate> para permitir a passagem de variáveis de ambiente específicas. Como alternativa, crie um script de shell que configure as variáveis de ambiente e execute seu aplicativo WSGI em `newrelic-admin`. Para iniciar seu aplicativo WSGI como um usuário diferente, execute <DoNotTranslate>**sudo**</DoNotTranslate> nesse script em vez de diretamente em seu aplicativo WSGI.

O uso de um script wrapper adicional para definir as variáveis de ambiente e executar seu aplicativo WSGI em `newrelic-admin` também pode ser necessário ao usar <DoNotTranslate>**supervisord**</DoNotTranslate> se algum problema for encontrado com configurações de variáveis de ambiente dentro da configuração de <DoNotTranslate>**supervisord**</DoNotTranslate> que não estão sendo transmitidas corretamente .
...
<CollapserGroup>
  ...
  >
    ...
    Se você usar um sistema de gerenciamento de processos como <DoNotTranslate>**supervisord**</DoNotTranslate> em que as variáveis de ambiente devem ser definidas em uma definição de configuração separada, não será possível defini-las na mesma linha do comando.

    Por exemplo, em <DoNotTranslate>**supervisord**</DoNotTranslate> você pode usar o seguinte. Certifique-se de substituir suas opções de comando existentes por `YOUR_COMMAND_OPTIONS`.
  ...
  >
    ...
    Usar `run-python` sempre usará o mesmo executável `python` instalado na instalação do Python ou no ambiente virtual em que <DoNotTranslate>**newrelic-admin**</DoNotTranslate> está instalado.
  </Collapser>
  ...
  >
    O programa <DoNotTranslate>**newrelic-admin**</DoNotTranslate> que você executa deve vir da mesma instalação do Python ou do mesmo ambiente virtual que seu aplicativo está usando. Você não pode misturar programas/componentes de diferentes instalações do Python. Se isso for feito, o agente não será executado corretamente.

    Se você usar um sistema de gerenciamento de processos como <DoNotTranslate>**supervisord**</DoNotTranslate> em um ambiente virtual, poderá usar esta configuração:
