...
Siga este procedimento <DoNotTranslate>**only**</DoNotTranslate> caso precise utilizar instrumentação manual para inserir o agente de monitoramento de Browser em vez da [instrumentação automática](/docs/agents/python-agent/supported-features/page-load-timing-python) do agente Python.
...
2. Adicione o seguinte o mais próximo possível do início do elemento `<head>` da página do modelo. Coloque esta <DoNotTranslate>**after**</DoNotTranslate> em qualquer meta tag que especifique uma propriedade charset ou em qualquer meta tag `http-equiv` para `X-UA-Compatible`.
