...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Opcional. O nome da função. Pode ser uma função que aceita um parâmetro callable\_name. Se não for definido, o padrão é o nome capturado da função.
      </td>
    </tr>
...
Se você quiser nomear a métrica dinamicamente (em vez de nomear a métrica com base no identificador da função que está sendo chamada), você pode fornecer o nome como uma função quando o objeto `FunctionTraceWrapper` é criado.
