...
Se o próprio script executar o servidor WSGI, chame `register_application` no thread do programa principal no arquivo de script principal. Se você usar Apache/mod_wsgi ou uWSGI, faça a chamada a partir do arquivo de script WSGI e faça a chamada imediatamente após `initialize`<DoNotTranslate>**.**</DoNotTranslate>
