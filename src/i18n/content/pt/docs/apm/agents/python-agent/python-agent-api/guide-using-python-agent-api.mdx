...
## Nomear dinamicamente segmentos e atributos de segmento [#dynamically-name-segments-and-segment-attributes]

Para nomear dinamicamente segmentos e valores de atributo de segmento, use chamadas de API de instrumentação personalizada, que aceitam uma função em vez de um valor estático para vários parâmetros. Antes da versão 13.0.0 do agente, você também poderia fazer isso no arquivo de configuração usando uma função do Lambda. Isso agora é suportado apenas em chamadas de API de instrumentação personalizada.

As seguintes chamadas de API suportam nomenclatura dinâmica:
