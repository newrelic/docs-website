...
Com o agente Python, você pode adicionar [monitoramento de browser](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) às suas páginas da web de forma automática ou manual. Para ativar <InlinePopover type="browser"/>na interface do usuário, siga os procedimentos para [instalar o browser do agente](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent). Em seguida, siga os procedimentos nesta seção para configurar o agente Python.

Para usar o browser do agente com seu agente Python, certifique-se de ter a versão mais recente. Para obter mais informações, consulte as [instruções de instalação do agente](/docs/agents/python-agent/installation-and-configuration/python-agent-installation).

## Suporte para instrumentação automática [#restrictions_on_instrumentation]
...
## Use instrumentação automática [#automatic_instrumentation]
...
## Desabilitar instrumentação para páginas específicas [#disabling_instrumentation]
...
Se você estiver usando um servidor WSGI que permite [configuração por solicitação](/docs/agents/python-agent/installation-configuration/python-agent-configuration#per-request-configuration), como Apache/mod_wsgi, também poderá desativar a instrumentação automática definindo a chave [`newrelic.disable_browser_autorum`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#disable_browser_autorum) para essa solicitação no dicionário de ambiente WSGI transmitido para seu aplicativo de monitor.
...
## Habilitar manualmente o monitoramento do browser para páginas específicas [#manual_instrumentation]
...
As chamadas neste procedimento devem ser feitas na solicitação <DoNotTranslate>**every**</DoNotTranslate> que você deseja monitor. Você <DoNotTranslate>**cannot**</DoNotTranslate> chama essas funções uma vez no escopo global e armazena o resultado em cache para uso posterior.

<Callout variant="important">
  A partir do agente Python versão 9.4.0+, a API get_browser_timing_footer está obsoleta e sempre retorna uma string vazia. A funcionalidade anterior de `get_browser_timing_footer()` agora está incluída em `get_browser_timing_header()`.

  Em versões mais antigas do agente, o rodapé é necessário para carregar o browser do agente.
</Callout>

1. Desative a instrumentação automática: no arquivo <DoNotTranslate>**newrelic.ini**</DoNotTranslate> , defina [`browser_monitoring.auto_instrument`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#browser-auto) como `false`.

2. Reinicie seu aplicativo.

3. Coloque essas chamadas no gerenciador de solicitações da página <DoNotTranslate>**every**</DoNotTranslate> que você deseja monitor:
...
3. Coloque essas chamadas no gerenciador de solicitações da página <DoNotTranslate>**every**</DoNotTranslate> que você deseja monitor, substituindo "NONCE" pelo seu CSP nonce:
