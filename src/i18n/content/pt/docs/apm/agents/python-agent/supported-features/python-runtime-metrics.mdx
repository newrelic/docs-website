...
O agente Python registra métricas de tempo de execução para permitir que você analise o desempenho de seus processos e intérpretes Python. Isso inclui métricas relacionadas ao uso de CPU, uso de memória e coleta de lixo.
