...
<CollapserGroup>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Este é o amostrador aplicado quando o trace tem um pai remoto (ou seja, o trace originou-se em um serviço upstream) que foi amostrado. As opções do amostrador são:

    * `adaptive` é o comportamento padrão e delega a decisão de amostragem ao algoritmo de amostragem adaptável para determinar se um trace será amostrado.
    * `trace_id_ratio_based` delega a decisão de amostragem ao algoritmo de amostragem Baseado na Proporção de Trace ID para determinar se um rastreamento será amostrado. Se você usar esta opção, a proporção é obrigatória, então use [distributed\_tracing.sampler.remote\_parent\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio) em vez disso.
    * `always_on`: Sempre amostra rastreamentos que têm um pai remoto amostrado.
    * `always_off`: Nunca amostra rastreamentos que têm um pai remoto amostrado.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio" title="distributed_tracing.sampler.remote_parent_sampled.trace_id_ratio_based.ratio">
    ...
    * Um número entre 0 (exclusivo) e 1 inclusive.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Este é o amostrador aplicado quando um trace tem um pai remoto (ou seja, o trace se originou em um serviço upstream) que não foi amostrado. As opções do amostrador são:

    * `adaptive` é o comportamento padrão e delega a decisão de amostragem ao algoritmo de amostragem adaptável para determinar se um trace será amostrado.
    * `trace_id_ratio_based` delega a decisão de amostragem ao algoritmo de amostragem Trace ID Ratio Based para determinar se um trace será amostrado. Se você usar esta opção, a proporção é obrigatória, então use [distributed\_tracing.sampler.remote\_parent\_not\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio) em vez disso.
  ...
  <Collapser id="distributed-tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    Este é o amostrador aplicado no início de um rastreamento. Por padrão, o Adaptive Sampler é configurado como o amostrador raiz. As opções do amostrador são:

    * `adaptive` é o comportamento padrão e delega a decisão de amostragem ao algoritmo de amostragem adaptável para determinar se um trace será amostrado.
    * `trace_id_ratio_based` delega a decisão de amostragem ao algoritmo de amostragem Trace ID Ratio Based para determinar se um trace será amostrado. Se você usar esta opção, a proporção é obrigatória, então use [distributed\_tracing.sampler.root.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-root-trace-id-ratio-based-ratio) em vez disso.
    * `always_on`: Sempre amostra transações que se originam do serviço atual e não possuem um cabeçalho de rastreamento distribuído presente na chamada de entrada.
    * `always_off`: Nunca amostra transações que se originam do serviço atual e não possuem um cabeçalho de rastreamento distribuído presente para a chamada de entrada.
  </Collapser>
...
## Configurações do agente híbrido [#otel-settings]

<Callout variant="important">
  Requer o [agente Python versão 11.5.0 ou superior](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
</Callout>

Para obter mais informações sobre as funcionalidades do agente híbrido, consulte [esta página](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support).

As seguintes configurações estão disponíveis para a configuração do agente híbrido:
...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="opentelemetry.enabled">
    ...
    De v11.5.0 a v13.0.0: Se habilitado, qualquer API OpenTelemetry e a maioria dos [frameworks instrumentados pelo OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) instalados serão utilizados. De v13.0.0+: Se ativado, qualquer API e framework do OpenTelemetry que seja [suportado pelo OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme), mas não pelo New Relic, será usado.

    <Callout variant="important">
      Embora a maioria das bibliotecas nos [frameworks instrumentados do OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) seja suportada, as bibliotecas de instrumentação AWS e gRPC continuarão a usar a instrumentação da New Relic. Bibliotecas de ML/IA com instrumentação nativa estão explicitamente desabilitadas por enquanto e serão suportadas em uma versão futura.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.enabled" title="opentelemetry.traces.enabled">
    ...
    De v11.5.0 a v13.0.0: Se ativado, quaisquer APIs de span e trace do OpenTelemetry serão usadas, independentemente de existir uma biblioteca de instrumentação do New Relic. De v13.0.0+: Se ativado, qualquer chamada manual usando a API de span do OpenTelemetry e spans de frameworks que são [suportados pelo OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme), mas não pelo New Relic, serão usados.
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.exclude" title="opentelemetry.traces.exclude">
    ...
    Permite que os usuários excluam tracer(s) de serem reportados ao New Relic. Consulte a [seção abaixo](#include-exclude-rules) para obter mais detalhes.

    <Callout variant="important">
      Requer o [agente Python versão 13.0.0 ou superior](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.include" title="opentelemetry.traces.include">
    ...
    Permite que os usuários incluam tracer(s) para serem reportados ao New Relic. Consulte a [seção abaixo](#include-exclude-rules) para obter mais detalhes.
...
Regras de Inclusão e Exclusão \[#include-exclude-rules]

New Relic segue estas regras ao determinar quais tracers incluir ou excluir para monitoramento.

<CollapserGroup>
  <Collapser id="rule-precedence" title="Ativado tem precedência sobre incluir e excluir.">
    `opentelemetry.traces.enabled` : esta flag tem precedência sobre as configurações de inclusão (include) e exclusão (exclude).
  ...
  <Collapser id="rule-trace-included" title="O trace é incluído se o tracing estiver habilitado.">
    Se o rastreamento estiver ativado, quaisquer tracers personalizados do OpenTelemetry e tracers de bibliotecas do OpenTelemetry não suportados pela New Relic são monitorados por padrão.
  ...
  <Collapser id="rule-exclude-wins" title="Excluir sempre substitui incluir.">
    Se o mesmo tracer estiver listado nas listas de inclusão e exclusão, então o tracer será excluído.
  ...
  <Collapser id="rule-tracers-case-sensitive" title="Os tracers diferenciam maiúsculas de minúsculas.">
    Os tracers diferenciam maiúsculas de minúsculas.
...
<CollapserGroup>
  ...
  <Collapser id="cross-application-tracer" title="cross_application_tracer.enabled">
    ...
    <Callout variant="caution">
      [O rastreamento entre aplicações (CAT)](/docs/apm/transactions/cross-application-traces/introduction-cross-application-traces) foi completamente removido na versão 12.0.0.
    </Callout>
  </Collapser>
