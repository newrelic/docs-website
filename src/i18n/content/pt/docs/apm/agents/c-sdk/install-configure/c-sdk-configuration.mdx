...
Todos os outros valores de configuração são opcionais e normalmente as configurações padrão não precisam ser alteradas. No entanto, quando necessário, você pode ajustar a configuração do C SDK da New Relic.

Este documento é uma referência rápida para usar algumas opções básicas de configuração com a API C SDK. Para obter informações detalhadas sobre como alterar suas definições de configuração, incluindo valores de código e exemplos, consulte o [GUIDE.md de configuração do C SDK no GitHub](https://github.com/newrelic/c-sdk/blob/master/GUIDE.md#configuration).
...
Você pode alterar [o alias do seu aplicativo](/docs/agents/manage-apm-agents/app-naming/name-your-application#differences) na página de configurações <DoNotTranslate>**Application**</DoNotTranslate> na interface do New Relic. Isso é útil, por exemplo, para dar um nome diferente ao seu aplicativo e ainda assim manter os dados históricos sob o novo alias. Para obter mais informações, consulte [Nomeie seu aplicativo](/docs/agents/manage-apm-agents/app-naming/name-your-application).

O C SDK da New Relic não suporta [configuração no lado do servidor](/docs/agents/manage-apm-agents/configuration/server-side-agent-configuration). No entanto, você também pode usar esta página de configurações <DoNotTranslate>**Application**</DoNotTranslate> na interface para definir o limite [de Apdex T](/docs/apm/new-relic-apm/apdex/apdex-measure-user-satisfaction) do seu aplicativo.

Para alterar o alias do aplicativo ou o limite do Apdex T na interface, vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Application**</DoNotTranslate>.
