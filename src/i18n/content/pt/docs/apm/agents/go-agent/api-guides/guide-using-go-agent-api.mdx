...
O [agente New Relic Go](/docs/agents/go-agent/get-started/introduction-new-relic-go) monitora seu aplicativo e microsserviços na linguagem Go para ajudá-lo a identificar e resolver problemas de desempenho. A API do agente Go é uma das várias [APINew Relic](/docs/apis/getting-started/introduction-new-relic-apis) disponíveis.
...
Transações e erros podem ser associados a um ID de usuário se este for conhecido pelo agente New Relic Go. Use a API do agente Go `txn.SetUserID("example user ID")` para fornecer ao agente um ID de usuário.

Esta chamada de API requer um único argumento de uma string que representa um identificador exclusivo para um usuário final. Essa string pode ser um UUID, um ID de banco de dados ou similar. A chamada de API deve ser feita pelo menos uma vez por transação para informar ao agente New Relic Go a qual ID de usuário associar a transação. Então, por sua vez, quando o agente perceber erros durante a vida útil da transação, os erros terão um atributo de agente `enduser.id` que contém o valor do ID do usuário.
...
Para obter mais informações sobre como correlacionar dados log com outros dados de telemetria, consulte nossa documentação [de logs em contexto](/docs/logs/logs-context/configure-logs-context-go/) .
