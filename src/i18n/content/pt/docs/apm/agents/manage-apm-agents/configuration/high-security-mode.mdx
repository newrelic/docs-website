...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Obtenha o [ID da conta](/docs/accounts/accounts-billing/account-structure/account-id) para a conta que você deseja habilitar. Em seguida, use isso para acessar este URL: `https://one.newrelic.com/admin-portal/apm-agents/hsm?account=YOUR_ACCOUNT_ID`. Nessa página, você pode configurar o modo de alta segurança.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Impede o perfilamento da aplicação
      </td>

      <td>
        O modo de alta segurança não permite que dados de perfil da aplicação sejam reportados para o seguinte:

        * [Profiling em tempo real para Java usando JFR](/docs/apm/agents/java-agent/features/real-time-profiling-java-using-jfr-metrics/)
      </td>
    </tr>
  </tbody>
</table>
