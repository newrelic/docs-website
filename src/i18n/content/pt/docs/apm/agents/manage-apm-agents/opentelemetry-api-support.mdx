...
<CollapserGroup>
  ...
  <Collapser id="python-config" title="Configuração do agente Python">
    ...
    **Usando variável de ambiente:**
    ...
    <Callout variant="important">
      Quando o suporte ao OpenTelemetry estiver habilitado, o agente detectará automaticamente se `opentelemetry-api` está instalado e passará a usar a instrumentação do OpenTelemetry para quaisquer bibliotecas suportadas, mesmo se a New Relic tiver um hook de instrumentação para essa biblioteca.
    </Callout>
    ...
    1. **Instrumentação nativa**: para bibliotecas com instrumentação nativa do OpenTelemetry integrada e que não são suportadas pela New Relic, o agente usará a instrumentação nativa do OpenTelemetry. Se o usuário quiser usar a instrumentação do OpenTelemetry em vez dos hooks de instrumentação integrados do New Relic, essa biblioteca pode ser adicionada à configuração `opentelemetry.traces.include`.

    2. **Instrumentação de biblioteca**: se uma biblioteca de instrumentação do OpenTelemetry for detectada e a New Relic ainda não fornecer um hook de instrumentação, o agente usará a instrumentação do OpenTelemetry. Semelhante à instrumentação nativa, se o usuário quiser usar a instrumentação do OpenTelemetry em vez dos hooks de instrumentação integrados do New Relic, essa biblioteca pode ser adicionada à configuração `opentelemetry.traces.include`.
    ...
    <Callout variant="caution">
      As bibliotecas de instrumentação da AWS e gRPC continuarão a usar a instrumentação da New Relic. Bibliotecas de ML/IA com instrumentação nativa estão explicitamente desabilitadas por enquanto e serão suportadas em uma versão futura.
    </Callout>
    ...
    **Exemplo:**
    ...
    2. **Troca automática**: se encontrada e o New Relic ainda não oferecer suporte a esta biblioteca, o agente usará a instrumentação da biblioteca OpenTelemetry
  ...
  <Collapser id="ruby-config" title="Configuração do agente Ruby">
    ...
    <Callout variant="important">
      Quando o suporte ao OpenTelemetry estiver habilitado, o agente detectará automaticamente se `opentelemetry-api` está instalado. Esta gem deve ser instalada para usar os recursos do agente híbrido.
    </Callout>
    ...
    **Exemplo:**
    ...
    ✅ **Suportado**: links de span usando a API `OpenTelemetry::Trace::Span#add_link`

    #### Eventos em Spans

    ✅ **Suportado**: eventos em spans usando a API `OpenTelemetry::Trace::Span#add_event`
