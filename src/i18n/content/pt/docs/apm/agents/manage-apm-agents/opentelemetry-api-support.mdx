...
<CollapserGroup>
  ...
  <Collapser id="ruby-config" title="Configuração do agente Ruby">
    ...
    O agente Ruby detecta a presença da instalação da API do OpenTelemetry e instrumenta as APIs do OpenTelemetry para Rastreamento a fim de gerar telemetria. Quando uma biblioteca ou o código da sua aplicação chama as APIs do OpenTelemetry, o agente Ruby da New Relic traduz essas chamadas em dados da New Relic se o suporte ao OpenTelemetry estiver habilitado.

    * **Requisito**: A API OpenTelemetry deve estar presente em sua aplicação para que a funcionalidade do agente híbrido funcione.

    ### Instrumentações suportadas

    O agente Ruby funciona com três tipos de instrumentação OpenTelemetry:

    #### 1. Instrumentação nativa

    Bibliotecas com chamadas de instrumentação OpenTelemetry integradas são suportadas. Quando essas bibliotecas fazem chamadas para a API do OpenTelemetry, o agente Ruby captura e as traduz em telemetria da New Relic.

    #### 2. Instrumentação de terceiros (opentelemetry-ruby-contrib)

    Se você instalar gems do [repositório opentelemetry-ruby-contrib](https://github.com/open-telemetry/opentelemetry-ruby-contrib), suas chamadas para as APIs do OpenTelemetry serão processadas e transformadas em dados do New Relic.

    Para uma lista completa de instrumentação Ruby disponível, consulte o [registro do OpenTelemetry](https://opentelemetry.io/ecosystem/registry/).

    #### 3. Instrumentação personalizada/manual

    Quando o suporte ao OpenTelemetry está habilitado, chamar as APIs do OpenTelemetry no código da sua aplicação gerará telemetria da New Relic automaticamente.

    ### Limitações conhecidas

    * **Suporte limitado à API**: Apenas a API de Traces é suportada nesta versão
    * **Métricas e Logs**: Não disponível
    * **Lista de exclusão**: O trabalho está em andamento para automatizar a prevenção de telemetria duplicada quando tanto o OpenTelemetry quanto o New Relic tiverem instrumentado os mesmos métodos. Por enquanto, pode ser necessário desabilitar manualmente uma instrumentação para evitar duplicatas.
