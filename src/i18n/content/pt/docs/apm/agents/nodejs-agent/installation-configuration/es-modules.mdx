...
O agente APM Node.js suporta a adição de instrumentação personalizada ao seu aplicativo de módulo ES. Você pode criar instrumentação no aplicativo do módulo ES usando a maioria dos [métodos de registro de instrumentação na API](https://newrelic.github.io/node-newrelic/API.html#instrument). Devido à natureza das importações do módulo ES, não podemos oferecer suporte `newrelic.instrumentLoadedModule`. As vinculações do módulo ES não são atribuíveis, o que impede que o agente substitua os membros exportados de um módulo pela instrumentação após o módulo ter sido carregado.

Para ver uma demonstração de como usar a API de instrumentação personalizada em um aplicativo de módulo ES, confira nosso [exemplo](https://github.com/newrelic/newrelic-node-examples/blob/4284ee7eab69708238db0a44f97ff7e839e165cf/esm-app/custom-instrumentation/index.js) no GitHub.
