...
<CollapserGroup>
  ...
  >
    Muitos drivers do banco de dados usam uma abstração chamada `cursor`. Os cursores fornecem a capacidade de iterar pelos resultados da consulta. Por exemplo, o driver <DoNotTranslate>**mongodb**</DoNotTranslate> fornece cursores ao executar a consulta `find` .
