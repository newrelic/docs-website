...
6. Adicione `-r newrelic` ao script de inicialização do seu aplicativo. Por exemplo, se o ponto de entrada do seu aplicativo for `./dist/server.js` , você usaria o sinalizador require da seguinte forma:
   ...
   <Callout variant="important">
     ...
     Se nenhuma dessas opções funcionar para você (por exemplo, carregar chave de API de forma assíncrona de um local remoto durante o aplicativo inicialização), você também pode adicionar instrumentação de estoque a um [módulo suportado](https://github.com/newrelic/node-newrelic/blob/0113eb5f0e707dc662a17d262a841503bab88841/lib/instrumentations.js#L6#L6) já carregado usando [`newrelic.instrumentLoadedModule`](https://newrelic.github.io/node-newrelic/API.html#instrumentLoadedModule):
