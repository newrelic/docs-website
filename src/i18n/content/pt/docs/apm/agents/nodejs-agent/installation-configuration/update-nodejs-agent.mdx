...
## Atualize para a versão 14 do agente Node.js [#node-agent-v14]

Antes de atualizar para o Node.js versão 14, revise estas informações sobre as principais alterações.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suporte ao Node.js 20 removido.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Instrumentação `@apollo/server` migrada de um plug-in `@newrelic/apollo-server-plugin` para instrumentação tradicional.
      </td>

      <td>
        * Os usuários do servidor Apollo não precisarão mais instalar e usar `@newrelic/apollo-server-plugin`. A instrumentação será aplicada automaticamente a cada instância do Apollo Server.
        * Para obter mais informações, consulte o [guia de migração do Apollo Server](#apollo-server-migration-guide).
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Removida a instrumentação para `koa-router` e `koa-route`.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: removido o suporte para políticas de licença, aplicativo e segurança (`LASP`).
      </td>

      <td>
        * Este recurso foi descontinuado e era configurável com `config.security_policies_token`.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suporte removido para a funcionalidade de Rastreamento multiaplicativo (`CAT`).
      </td>

      <td>
        * O CAT foi descontinuado no agente v8.4.0 e substituído pelo distributed tracing para propagar o contexto entre serviços.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Atualizado `config.distributed_tracing.exclude_newrelic_header` para ser definido como `true` por padrão.
      </td>

      <td>
        * Isso foi substituído por cabeçalhos W3C por padrão.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: método de API `shim.argsToArray` removido em favor de parâmetros rest.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: método de API `shim.prefixRouteParameters` removido.
      </td>

      <td>
        * A instrumentação do Fastify foi atualizada para usar diagnósticos de canal de rastreamento, portanto, este método não é mais necessário.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Segmentos estáticos `MySQL` `Pool#query` descartados em `mysql` e `mysql2`.
      </td>

      <td>
        * Isso criou um segmento desnecessário, já que segmentos `Database/*` são mais úteis.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Versão mínima suportada atualizada para as seguintes bibliotecas:
      </td>

      <td>
        * `@nestjc/cli` e `@nestjs/core` para 10.0.0
        * `bluebird` para 3.0.0
        * `mysql2` para 3.0.0
        * `next` para 14.0.0
        * `cassandra-driver` para 4.0.0
        * `fastify` para 4.0.0
      </td>
    </tr>
    ...
    <tr>
      <td>
        Atualizada a forma como o tempo exclusivo e o tempo total do trace são calculados.
      </td>
...
### Guia de Migração do Apollo Server [#apollo-server-migration-guide]

A partir da versão 14.0.0 do agente, os clientes do apollo server não precisarão mais instalar e usar `@newrelic/apollo-server-plugin`. A instrumentação será aplicada automaticamente a cada instância do servidor Apollo. No entanto, se os clientes configuraram o plug-in, isto detalhará o plano de migração.
...
* `captureScalars` agora é `config.apollo_server.scalars` ou `NEW_RELIC_APOLLO_SERVER_SCALARS`
* `captureIntrospectionQueries` agora é `config.apollo_server.introspection_queries` ou `NEW_RELIC_APOLLO_SERVER_INTROSPECTION_QUERIES`
* `captureServiceDefinitionQueries` agora é `config.apollo_server.service_definition_queries` ou `NEW_RELIC_APOLLO_SERVER_SERVICE_DEFINITION_QUERIES`
* `captureHealthCheckQueries` agora é `config.apollo_server.health_check_queries` ou `NEW_RELIC_APOLLO_SERVER_HEALTH_CHECK_QUERIES`
* `captureFieldMetrics` agora é `config.apollo_server.field_metrics` ou `NEW_RELIC_APOLLO_SERVER_FIELD_METRICS`

#### Chamada de API

Havia duas configurações anteriores que registravam funções a serem chamadas que adicionavam os valores principais como atributos personalizados nos segmentos de operação ou de resolvedor. O agente segue um padrão em que os clientes podem registrar retornos de chamada via métodos de API, e esses dois casos de uso se encaixam nisso.

##### customOperationAttributes

Isso agora é `newrelic.setApolloOperationAttributesCallback`. Ele recebe um retorno de chamada e, quando presente, o mesmo argumento de antes é passado ao retorno de chamada. `requestContext`
...
Isso agora é `newrelic.setApolloResolverAttributesCallback`. Ele recebe um retorno de chamada e, quando presente, o mesmo argumento de antes é passado ao retorno de chamada. `{ source, args, contextValue, info }`
...
A métrica `MySQL` `host:port` verificada é registrada
