---
title: Compatibilidade e requisitos para o agente Node.js
tags:
  - Agents
  - Nodejs agent
  - Getting started
metaDescription: 'The New Relic Node.js agent supports these frameworks, architectures, and operating systems.'
freshnessValidatedDate: never
translationType: machine
---

Nosso agente Node.js inclui instrumentação integrada do framework Node.js mais popular, servidores de aplicativos, banco de dados e sistemas de enfileiramento de mensagens. Para frameworks e bibliotecas que não são instrumentados prontos para uso, você pode estender o agente com nossa [API Node.js do agente](https://newrelic.github.io/node-newrelic/API.html).

Nosso agente Node.js está disponível publicamente no [repositório Node gerenciador de pacote (npm),](https://npmjs.org/package/newrelic) bem como no [GitHub](https://github.com/newrelic/node-newrelic). Pronto para experimentar o agente Node.js da New Relic? [Crie uma conta New Relic](https://newrelic.com/signup)!

## Requisitos para instalar o agente

Antes de [instalar o agente](/docs/apm/agents/nodejs-agent/installation-configuration/install-nodejs-agent), verifique se o seu sistema atende aos requisitos mínimos. Para obter melhor desempenho, use a versão mais recente do suporte ativo de longo prazo (LTS) do Node.js.

<CollapserGroup>
  <Collapser id="operating systems" title="Sistema operacional compatível">
    O agente Node.js é compatível com o seguinte sistema operacional:

    * Linux
    * SmartOS
    * macOS 10.7 e superior
    * Windows Server 2008 e superior
  </Collapser>

  <Collapser id="host" title="Serviços de hospedagem">
    Também é compatível com estes serviços de hospedagem:

    * [Ambiente flexível do Google App Engine (GAE)](/docs/agents/nodejs-agent/hosting-services/install-new-relic-nodejs-agent-gae-flexible-environment)
    * AWS EC2
    * [Microsoft Azure](/docs/nodejs/nodejs-agent-on-microsoft-azure)
    * [Heroku](/docs/nodejs/nodejs-agent-on-heroku)
  </Collapser>

  <Collapser id="security" title="Requisitos de segurança">
    Como [medida de segurança padrão para coleta de dados](/docs/accounts-partnerships/accounts/security/data-security), o servidor do seu aplicativo deve oferecer suporte a SHA-2 (256 bits). SHA-1 não é compatível.
  </Collapser>

  <Collapser id="system" title="Requisito do sistema">
    Daremos suporte às [versões mais recentes dos lançamentos do Node.js](https://nodejs.org/en/about/previous-releases) no início do seguinte cronograma de suporte ativo de longo prazo. A política de suporte de versão não substitui nossa política geral de fim de vida útil (EOL).

    A seguir estão os intervalos de tempo propostos. A data de lançamento real pode variar.

    <table>
      <thead>
        <tr>
          <th style={{ width: "100px" }}>
            Versão do Node.js.
          </th>

          <th>
            Data de início do suporte ativo de longo prazo (LTS)
          </th>

          <th>
            Data de lançamento inicial do agente Node.js com suporte
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            26
          </td>

          <td>
            Outubro de 2026
          </td>

          <td>
            7 de julho de 2026 com o agente Node.js v14.2.0
          </td>
        </tr>

        <tr>
          <td>
            24
          </td>

          <td>
            Outubro de 2025
          </td>

          <td>
            30 de junho de 2025 com o agente Node.js v12.23.0
          </td>
        </tr>

        <tr>
          <td>
            22
          </td>

          <td>
            Outubro de 2024
          </td>

          <td>
            28 de junho de 2024 com agente Node.js v11.22.0
          </td>
        </tr>
      </tbody>
    </table>

    A seguir estão os intervalos de tempo propostos para EOL em versões mais antigas do Node.js. Normalmente abandonaremos o suporte 2 a 4 meses após a [data de EOL publicada do Node.js.](https://nodejs.org/en/about/previous-releases)

    <table>
      <thead>
        <tr>
          <th style={{ width: "100px" }}>
            Versão do Node.js.
          </th>

          <th>
            Data de fim de vida (EOL)
          </th>

          <th>
            Data de lançamento inicial do agente Node.js abandonando o suporte
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            26
          </td>

          <td>
            Abril de 2029
          </td>

          <td>
            A definir
          </td>
        </tr>

        <tr>
          <td>
            24
          </td>

          <td>
            Abril de 2028
          </td>

          <td>
            A definir
          </td>
        </tr>

        <tr>
          <td>
            22
          </td>

          <td>
            Abril de 2027
          </td>

          <td>
            A definir
          </td>
        </tr>

        <tr>
          <td>
            20
          </td>

          <td>
            Abril de 2026
          </td>

          <td>
            A partir de 18 de maio de 2026, descontinuamos o suporte ao Node.js 20 com a v14 do agente Node.js.
          </td>
        </tr>

        <tr>
          <td>
            18
          </td>

          <td>
            Abril de 2025
          </td>

          <td>
            A partir de 23 de julho de 2025, descontinuamos o suporte ao Node.js 18 com a v13 do agente Node.js.
          </td>
        </tr>

        <tr>
          <td>
            16
          </td>

          <td>
            Setembro de 2023
          </td>

          <td>
            A partir de 31 de julho de 2024, descontinuamos o suporte para Node.js 16 com v12 do agente Node.js.
          </td>
        </tr>

        <tr>
          <td>
            14
          </td>

          <td>
            Abril de 2023
          </td>

          <td>
            A partir de 28 de agosto de 2023, descontinuamos o suporte para Node.js 14 com v11 do agente Node.js.
          </td>
        </tr>

        <tr>
          <td>
            12
          </td>

          <td>
            Abril de 2022
          </td>

          <td>
            A partir de 3 de agosto de 2022, descontinuamos o suporte para Node.js 12 com v9 do agente Node.js.
          </td>
        </tr>

        <tr>
          <td>
            10
          </td>

          <td>
            Abril de 2021
          </td>

          <td>
            A partir de 26 de julho de 2021, descontinuamos o suporte ao Node.js 10 com a v8 do agente Node.js. Para mais detalhes, incluindo como se preparar facilmente para essa transição, consulte nossa [publicação no Fórum de Suporte](https://support.newrelic.com/s/hubtopic/aAX8W0000008bz0/important-upcoming-changes-to-support-and-capabilities-across-browser-nodejs).
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>
</CollapserGroup>

## Outro software de monitoramento de terceiros [#other-monitoring-software]

Se o seu aplicativo usar algum software de monitoramento adicional, não podemos garantir que nosso agente funcionará corretamente e não poderemos oferecer suporte técnico. Para obter mais informações, consulte [Erros ao usar outro software de monitoramento](/docs/apm/new-relic-apm/troubleshooting/errors-while-using-new-relic-apm-alongside-other-apm-software).

{/* The content between begin: compat-table and end: compat-table is automatically updated by the Node.js agent team bot. Don't change the contents and don't change the formatting of the data between the comments.*/}

{/* begin: compat-table */}

## Módulos instrumentados

Após a instalação, o agente automaticamente instrumento com nosso catálogo de bibliotecas e frameworks Node.js suportados. Isso lhe dá acesso imediato a informações granulares específicas de seus aplicativos e servidores da web. Para estruturas ou bibliotecas não suportadas, você mesmo precisará instrumentar o agente usando a [API Node.js do agente](https://newrelic.github.io/node-newrelic/API.html).

**Observação**: a versão mais recente publicada pode não refletir a versão mais recente suportada pelo agente.

| Nome do pacote                    | Versão mínima suportada | Última versão publicada | Introduzido em\* |
| --------------------------------- | ----------------------- | ----------------------- | ---------------- |
| `@anthropic-ai/sdk`               | 0.33.0                  | 0.123.0                 | 13.19.0          |
| `@apollo/gateway`                 | 2.3.0                   | 2.14.4                  | 14.0.0           |
| `@apollo/server`                  | 4.0.0                   | 5.5.1                   | 14.0.0           |
| `@aws-sdk/client-bedrock-runtime` | 3.377.0                 | 3.1124.0                | 11.13.0          |
| `@aws-sdk/client-dynamodb`        | 3.377.0                 | 3.1124.0                | 8.7.1            |
| `@aws-sdk/client-sns`             | 3.377.0                 | 3.1124.0                | 8.7.1            |
| `@aws-sdk/client-sqs`             | 3.377.0                 | 3.1124.0                | 8.7.1            |
| `@aws-sdk/lib-dynamodb`           | 3.377.0                 | 3.1124.0                | 8.7.1            |
| `@aws-sdk/smithy-client`          | 3.47.0                  | 3.374.0                 | 8.7.1            |
| `@azure/functions`                | 4.7.0                   | 4.16.2                  | 12.18.0          |
| `@elastic/elasticsearch`          | 7.16.0                  | 9.5.1                   | 11.9.0           |
| `@google/adk`                     | 1.1.0                   | 2.0.0                   | 13.20.0          |
| `@google/genai`                   | 1.1.0                   | 2.20.0                  | 12.21.0          |
| `@grpc/grpc-js`                   | 1.4.0                   | 1.14.4                  | 8.17.0           |
| `@hapi/hapi`                      | 20.1.2                  | 21.4.10                 | 9.0.0            |
| `@hapi/vision`                    | 5.0.0                   | 7.0.3                   | 9.0.0            |
| `@koa/router`                     | 12.0.1                  | 15.7.0                  | 3.2.0            |
| `@langchain/aws`                  | 0.1.3                   | 1.4.5                   | 13.8.0           |
| `@langchain/core`                 | 0.2.0                   | 1.2.9                   | 11.13.0          |
| `@langchain/langgraph`            | 1.0.0                   | 1.4.13                  | 13.12.0          |
| `@langchain/openai`               | 0.2.0                   | 1.5.11                  | 11.13.0          |
| `@modelcontextprotocol/sdk`       | 1.13.0                  | 1.30.0                  | 13.2.0           |
| `@nestjs/core`                    | 10.0.0                  | 12.0.1                  | 10.1.0           |
| `@opensearch-project/opensearch`  | 2.1.0                   | 3.6.0                   | 12.10.0          |
| `@prisma/client`                  | 5.0.0                   | 7.10.0                  | 11.0.0           |
| `@smithy/smithy-client`           | 2.0.0                   | 4.15.2                  | 11.0.0           |
| `amqplib`                         | 0.5.0                   | 2.0.1                   | 2.0.0            |
| `aws-sdk`                         | 2.2.48                  | 2.1693.0                | 6.2.0            |
| `bluebird`                        | 3.0.0                   | 3.7.2                   | 1.27.0           |
| `bunyan`                          | 1.8.12                  | 1.8.15                  | 9.3.0            |
| `cassandra-driver`                | 4.0.0                   | 4.9.0                   | 1.7.1            |
| `connect`                         | 3.0.0                   | 3.7.0                   | 2.6.0            |
| `express`                         | 4.15.0                  | 5.2.1                   | 2.6.0            |
| `fastify`                         | 4.0.0                   | 5.12.1                  | 8.5.0            |
| `generic-pool`                    | 3.0.0                   | 3.9.0                   | 0.9.0            |
| `ioredis`                         | 4.0.0                   | 6.0.0                   | 1.26.2           |
| `iovalkey`                        | 0.1.0                   | 0.4.0                   | 13.9.0           |
| `kafkajs`                         | 2.0.0                   | 2.2.4                   | 11.19.0          |
| `koa`                             | 2.0.0                   | 3.2.1                   | 3.2.0            |
| `memcached`                       | 2.2.0                   | 2.2.2                   | 1.26.2           |
| `mongodb`                         | 4.1.4                   | 7.6.0                   | 1.32.0           |
| `mysql`                           | 2.16.0                  | 2.18.1                  | 1.32.0           |
| `mysql2`                          | 3.0.0                   | 3.24.2                  | 1.32.0           |
| `next`                            | 14.0.0                  | 16.3.4                  | 12.0.0           |
| `openai`                          | 4.0.0                   | 7.8.0                   | 11.13.0          |
| `pg`                              | 8.2.0                   | 8.23.0                  | 9.0.0            |
| `pg-native`                       | 3.0.0                   | 3.9.0                   | 9.0.0            |
| `pino`                            | 8.0.0                   | 10.3.1                  | 8.11.0           |
| `q`                               | 1.3.0                   | 1.5.1                   | 1.26.2           |
| `redis`                           | 3.1.0                   | 6.2.1                   | 1.31.0           |
| `restify`                         | 11.0.0                  | 12.0.0                  | 2.6.0            |
| `superagent`                      | 3.0.0                   | 10.3.0                  | 4.9.0            |
| `undici`                          | 5.0.0                   | 8.10.1                  | 11.1.0           |
| `when`                            | 3.7.0                   | 3.7.8                   | 1.26.2           |
| `winston`                         | 3.0.0                   | 3.19.0                  | 8.11.0           |

\*Quando o pacote não é especificado, o suporte está dentro do pacote `newrelic` .

## Suporte AI Monitoring

O agente Node.js oferece suporte à seguinte plataforma e integração de IA.

### Amazon Bedrock

Por meio do módulo `@aws-sdk/client-bedrock-runtime` , oferecemos suporte a todos os modelos (somente texto) por meio da API `Converse` ; para a API `InvokeModel` , oferecemos suporte aos seguintes modelos:

| Modelo           | Imagem | Texto | Visão |
| ---------------- | ------ | ----- | ----- |
| Amazon Titan     | ❌      | ✅     | -     |
| Anthropic Claude | ❌      | ✅     | ❌     |
| Cohere           | ❌      | ✅     | -     |
| Meta Lhama3      | ❌      | ✅     | -     |

Nota: se um modelo suportar streaming, também instrumentamos a variante de streaming.

### Google ADK

Por meio do módulo `@google/adk`, oferecemos suporte aos seguintes recursos:

| Agentes | Ferramentas |
| ------- | ----------- |
| ✅       | ✅           |

Modelos/provedores são geralmente suportados transitivamente pela nossa instrumentação do módulo do provedor.

| Fornecedor    | Suportado | Transitivamente |
| ------------- | --------- | --------------- |
| Google Gemini | ✅         | ✅               |

### LangChain

Os seguintes recursos gerais do LangChain são suportados:

| Agentes via LangGraph | Chains | Ferramentas | Vectorstores |
| --------------------- | ------ | ----------- | ------------ |
| ✅                     | ✅      | ✅           | ✅            |

Modelos/provedores são geralmente suportados transitivamente pela nossa instrumentação do módulo do provedor.

| Fornecedor     | Suportado | Transitivamente |
| -------------- | --------- | --------------- |
| Azure OpenAI   | ❌         | ❌               |
| Amazon Bedrock | ✅         | ✅               |
| OpenAI         | ✅         | ✅               |

### SDK do Protocolo de Contexto do Modelo

Através do módulo `@modelcontextprotocol/sdk` , oferecemos suporte a:

| Recuperação de Prompt | Leitura de Recursos | Chamadas de Ferramentas |
| --------------------- | ------------------- | ----------------------- |
| ✅                     | ✅                   | ✅                       |

O MCP é agnóstico de provedor. Qualquer provedor de LLM que use ferramentas MCP é suportado transitivamente.

| Fornecedor | Suportado | Transitivamente |
| ---------- | --------- | --------------- |

### Anthropic

Através do módulo `@anthropic-ai/sdk` , oferecemos suporte a:

| Bater papo | Conclusões | Arquivos |
| ---------- | ---------- | -------- |
| ✅          | ✅          | ❌        |

### Google Gen AI

Através do módulo `@google/genai` , oferecemos suporte a:

| Áudio | Cache | Bater papo | Incorporações | Imagem | PDF | Texto | Vídeo |
| ----- | ----- | ---------- | ------------- | ------ | --- | ----- | ----- |
| ❌     | ❌     | ✅          | ✅             | ❌      | ❌   | ✅     | ❌     |

### OpenAI

Através do módulo `openai` , oferecemos suporte a:

| Áudio | Bater papo | Conclusões | Incorporações | Arquivos | Imagens |
| ----- | ---------- | ---------- | ------------- | -------- | ------- |
| ❌     | ✅          | ✅          | ✅             | ❌        | ❌       |

{/* end: compat-table */}

## Instrumentação de agente híbrido

Algumas instrumentações são suportadas por meio do nosso recurso de [agente híbrido](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support).

### Next.js v16+

A partir da [versão 14.1.0](/docs/release-notes/agent-release-notes/nodejs-release-notes/node-agent-14-1-0) do agente, o suporte para o framework Next.js é baseado na instrumentação OpenTelemetry integrada do próprio Next.js. Para obter detalhes sobre como configurar o agente, por favor, revise nosso [aplicativo Next.js de exemplo](https://github.com/newrelic/newrelic-node-examples/tree/734e1e0be7302210bfb3a113ad93ad5eeb0e0fa8/nextjs/nextjs-app-router).

### Prisma 7

O Prisma oferece instrumentação direta de telemetria do Open Telemetry por meio de seu pacote [@prisma/instrumentation](https://www.npmjs.com/package/@prisma/instrumentation). A partir da versão 7 do Prisma e da [versão 14.1.0](/docs/release-notes/agent-release-notes/nodejs-release-notes/node-agent-14-1-0) do agente, utilizamos essa instrumentação nativa para obter insights sobre as operações do Prisma. Consulte nosso [aplicativo de exemplo do Prisma](https://github.com/newrelic/newrelic-node-examples/tree/734e1e0be7302210bfb3a113ad93ad5eeb0e0fa8/prisma/prisma-7-js) para obter detalhes sobre como utilizar o Prisma com o agente da New Relic.

## Conecte o agente a outro recurso New Relic [#digital-intelligence-platform]

O agente Node.js se integra a outro recurso para fornecer observabilidade em toda a stack:

<table>
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        Produtos
      </th>

      <th>
        Capacidade
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        [AI Monitoring](/docs/ai-monitoring/intro-to-ai-monitoring)
      </td>

      <td>
        Se você tiver a versão 11.13.0 do agente Node.js, poderá coletar dados de IA de determinadas bibliotecas e estruturas de IA. Consulte [Suporte de Monitoramento de IA](#ai-monitoring-support) acima para obter a lista de compatibilidade atual.
      </td>
    </tr>

    <tr>
      <td>
        [Monitoramento de browser](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)
      </td>

      <td>
        O agente Node.js pode adicionar os benefícios do <InlinePopover type="browser" />ao [ativar a instrumentação automática](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent#select-apm-app). Após habilitar a injeção de monitoramento de Browser, basta seguir nosso [guia de instalação de monitoramento de Browser com o agente Node.js.](/docs/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent/) Depois de concluir essas etapas, você poderá visualizar seus dados do browser na [página APM <DNT>**Summary**</DNT> ](/docs/apm/applications-menu/monitoring/apm-overview-page)e alternar rapidamente entre o APM e os dados do browser para um aplicativo específico. Para opções de configuração e instrumentação manual, consulte [monitoramento de Browser e agente Node.js.](/docs/agents/nodejs-agent/supported-features/new-relic-browser-nodejs-agent)
      </td>
    </tr>

    <tr>
      <td>
        [Monitoramento de infraestrutura](/docs/infrastructure/new-relic-infrastructure/getting-started/introduction-new-relic-infrastructure)
      </td>

      <td>
        Ao instalar a infraestrutura e o agente APM no mesmo host, eles detectam um ao outro automaticamente. Você pode então visualizar uma lista de hosts na interface do APM e filtrar seus hosts por aplicativo APM na interface de infraestrutura. Para obter mais informações, consulte [Dados APM no monitoramento de infraestrutura](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/new-relic-apm-data-infrastructure).
      </td>
    </tr>

    <tr>
      <td>
        [Painel New Relic](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights)
      </td>

      <td>
        O agente Node.js envia [evento e atributo padrão](/docs/insights/insights-data-sources/default-events-attributes/apm-default-event-attributes) para consulta NRQL. Você também pode [gravar eventos personalizados](/docs/insights/insights-data-sources/custom-data/insert-custom-events-new-relic-apm-agents) para análise avançada.
      </td>
    </tr>

    <tr>
      <td>
        [Monitoramento sintético](/docs/synthetics/new-relic-synthetics/getting-started/introduction-new-relic-synthetics)
      </td>

      <td>
        [Sintético trace da transação](/docs/synthetics/new-relic-synthetics/using-monitors/collect-synthetic-transaction-traces) conecta solicitações do monitor Sintético à transação APM subjacente.
      </td>
    </tr>
  </tbody>
</table>