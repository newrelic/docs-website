...
<figcaption>
  <DoNotTranslate>**Node.js configuration hierarchy:**</DoNotTranslate> Configuração no lado do servidor as configurações substituem as variáveis de ambiente. As variáveis de ambiente substituem o arquivo de configuração do agente. O arquivo de configuração substitui os padrões do agente.
</figcaption>
...
## Exporta variáveis [#exports_config]
...
<CollapserGroup>
  ...
  >
    ...
    Os dados de todos os aplicativos com o mesmo nome serão mesclados na interface do New Relic, portanto, defina isso com cuidado. Solicitamos <DoNotTranslate>**highly recommend**</DoNotTranslate> que você substitua o nome padrão por um nome descritivo para evitar confusão e agregação não intencional de dados.
...
## Variáveis de registro [#logging_config]
...
<CollapserGroup>
  ...
  >
    ...
    * Para gravar todos os registros em
...
## Registro de auditoria [#audit_log]
...
## Configuração de API [#api_config]
...
<CollapserGroup>
  ...
  >
    ...
    Esta opção ativa [`newrelic.noticeError`](/docs/apm/agents/nodejs-agent/api-guides/nodejs-agent-api/#noticeError).
  </Collapser>
</CollapserGroup>
...
## Variáveis do coletor de erros [#error_config]
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Os erros registrados usando [`newrelic.noticeError()`](/docs/apm/agents/nodejs-agent/api-guides/nodejs-agent-api/#noticeError) não obedecem a este valor de configuração.
    </Callout>
  </Collapser>
...
## Variáveis tracer de transação [#tx_tracer_config]
...
<CollapserGroup>
  ...
  >
    ...
    Transações são nomeadas com base na solicitação e `top_n` refere-se às "transações principais e mais lentas" agrupadas por esses nomes. O módulo substitui um trace gravado por um novo trace apenas se o novo trace for mais lento que o trace anterior mais lento com esse nome. O valor padrão para esta configuração é `top_n: 20`, porque a [página<DoNotTranslate>**Transactions**</DoNotTranslate> ](/docs/apm/transactions/transaction-traces/transaction-traces)também assume como padrão as 20 transações mais lentas.
  ...
  >
    ...
    Limite de transação da web tempo de resposta em segundos além do qual uma [transação](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#transaction) é elegível para [rastreamento de transação](/docs/apm/transactions/transaction-traces/transaction-traces). O valor padrão é `apdex_f`; isso define o limite de trace para quatro vezes [o Apdex T](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#apdex_t) do seu aplicativo. Você também pode inserir um valor de tempo específico em milissegundos.

    <DoNotTranslate>
      **Example: Threshold set to `apdex_f`**
    </DoNotTranslate>

    O `apdex_t` padrão é 100 milissegundos. Se o seu limite de transação estiver definido como `apdex_f`, uma transação "lenta" terá 400 milissegundos.
  </Collapser>
...
## Variáveis de regras [#rules_config]
...
## Variáveis de evento de transação [#tx_events]
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="max_samples_stored (DESCONTINUADO)"
  >
    <table>
      <tbody>
        <tr>
  ...
  <Collapser
    ...
    title="max_samples_per_minute (DESCONTINUADO)"
  >
    <table>
      <tbody>
        <tr>
...
## Variáveis personalizadas do evento [#custom_events]
...
<CollapserGroup>
  ...
  >
    ...
    Quando `false`, o agente redigirá as mensagens de erros capturados.
  </Collapser>
</CollapserGroup>
...
## Instrumentação de servidor gRPC [#grpc_server]
...
Para obter dicas sobre como configurar o log para o agente Node.js, consulte [Configurar o log do Node.js no contexto](/docs/logs/logs-context/configure-logs-context-nodejs).
...
<CollapserGroup>
  ...
  >
    ...
    Permite a geração automática [de log no contexto](/docs/logs/logs-context/configure-logs-context-nodejs/).
