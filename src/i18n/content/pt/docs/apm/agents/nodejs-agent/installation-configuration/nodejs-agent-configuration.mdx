...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-root" title="sampler.root">
    ...
    Controla como as decisões de amostragem são feitas para spans raiz — transações que não têm um pai remoto (o que significa que não possuem um cabeçalho de distributed tracing `traceparent` ou `newrelic` de entrada). Os valores válidos são:

    * `adaptive` (padrão): usa o algoritmo de amostragem adaptável integrado do New Relic.
    * `always_on`: amostra todos os spans.
    * `always_off`: não faz a amostragem de nenhum span.
    * `trace_id_ratio_based`: faz a amostragem de uma proporção fixa de traces, determinada de forma determinística pelo ID do trace.

    Por exemplo, para definir isso no arquivo de configuração, você usaria:
    ...
    Quando definido como `trace_id_ratio_based`, você também deve fornecer um `ratio` (um número entre `0` e `1`):
    ...
    Quando você define isso como `adaptive`, pode opcionalmente substituir o destino de amostragem (para 1–120 transações/minuto) especificamente para este amostrador:
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="sampler.remote_parent_sampled">
    ...
    Quando o cabeçalho traceparent tem um sinalizador de amostragem de `01`, isso controla como lidar com a amostragem de spans. Os valores de string permitidos são:
    ...
    * `trace_id_ratio_based`: faz a amostragem de uma proporção fixa de traces, determinada de forma determinística pelo ID do trace.
    ...
    Esta configuração tem precedência sobre [`remote_parent_not_sampled`](#dt-sampler-remote-parent-not-sampled) quando ambas puderem ser aplicadas.
    ...
    Quando você define isso como `trace_id_ratio_based`, deve fornecer um valor `ratio` de `0`–`1`. Quando você define isso como `adaptive`, pode opcionalmente sobrescrever o destino de amostragem especificamente para este amostrador. Para a sintaxe equivalente, consulte os exemplos de formato de objeto e de variável de ambiente em [`sampler.root`](#dt-sampler-root) e substitua `remote_parent_sampled` por `root`.
  </Collapser>
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    Quando o cabeçalho traceparent tem um sinalizador de amostragem de `00`, isso controla como lidar com a amostragem de spans. Os valores de string permitidos são:
    ...
    * `trace_id_ratio_based`: faz a amostragem de uma proporção fixa de traces, determinada de forma determinística pelo ID do trace.
    ...
    Esta configuração só entra em vigor quando [`remote_parent_sampled`](#dt-sampler-remote-parent-sampled) não se aplica (ou seja, o sinalizador de amostragem traceparent é `00`).
    ...
    Quando você define isso como `trace_id_ratio_based`, deve fornecer um valor `ratio` de `0`–`1`. Quando você define isso como `adaptive`, pode opcionalmente sobrescrever o destino de amostragem especificamente para este amostrador. Para a sintaxe equivalente, consulte os exemplos de formato de objeto e de variável de ambiente em [`sampler.root`](#dt-sampler-root) e substitua `remote_parent_not_sampled` por `root`.
  </Collapser>
  ...
  <Collapser id="dt-sampler-adaptive-sampling-target" title="sampler.adaptive_sampling_target">
    ...
    O destino de amostragem para amostragem adaptativa, em transações por minuto, usado ao configurar o amostrador padrão/adaptativo. Deve estar dentro do intervalo `[1, 120]` (inclusive). Ao se conectar ao New Relic, esse valor é enviado ao coletor, e o valor `sampling_target` retornado na resposta de conexão é usado como o destino de amostragem efetivo para amostragem adaptativa no agente.
...
Configure como o agente detecta as informações de ambiente de host e cloud para calcular com precisão o seu preço baseado na utilização. Essas configurações aparecem na seção `utilization` do arquivo de configuração `newrelic.js` do seu aplicativo.
...
<CollapserGroup>
  <Collapser id="utilization-detect-aws" title="detect_aws">
    ...
    Se o agente tenta entrar em contato com a AWS para obter informações sobre a VM na qual o processo está sendo executado.
  </Collapser>
  ...
  <Collapser id="utilization-detect-pcf" title="detect_pcf">
    ...
    Se o agente tenta detectar se o processo está em execução no Pivotal Cloud Foundry.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azure" title="detect_azure">
    ...
    Se o agente tenta entrar em contato com o Azure para obter informações sobre a VM em que o processo está em execução.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azurefunction" title="detect_azurefunction">
    ...
    Se o agente tenta ler as variáveis de ambiente e o contexto de invocação para obter informações sobre o Azure Function em que está em execução.
  </Collapser>
  ...
  <Collapser id="utilization-detect-docker" title="detect_docker">
    ...
    Se o agente tenta ler arquivos para obter informações sobre o contêiner no qual o processo está sendo executado.
  </Collapser>
  ...
  <Collapser id="utilization-detect-gcp" title="detect_gcp">
    ...
    Se o agente tenta entrar em contato com o GCP para obter informações sobre a VM na qual o processo está sendo executado.
  </Collapser>
  ...
  <Collapser id="utilization-detect-kubernetes" title="detect_kubernetes">
    ...
    Se o agente tenta entrar em contato com o Kubernetes para obter informações sobre o contêiner em que o processo está em execução.
  </Collapser>
  ...
  <Collapser id="utilization-logical-processors" title="logical_processors">
    ...
    Substitui o número de processadores lógicos que o agente relata para o host.
  </Collapser>
  ...
  <Collapser id="utilization-billing-hostname" title="billing_hostname">
    ...
    Substitui o nome do host que o agente relata para fins de faturamento/utilização.
  </Collapser>
  ...
  <Collapser id="utilization-total-ram-mib" title="total_ram_mib">
    ...
    Substitui a quantidade de RAM total (em MiB) que o agente relata para o host.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    <Callout variant="important">
      Obsoleto e será removido na versão 15 do agente. Em vez disso, use `gcp_cloud_run.use_instance_as_host`.
    </Callout>

    Quando habilitado, ele usará o ID de metadados do GCP para definir o nome do host do aplicativo em execução (Serviços, Worker Pools e Jobs).
  </Collapser>
...
Essas configurações controlam os vários recursos baseados no OpenTelemetry fornecidos pelo agente. Elas aparecem na seção `opentelemetry` do arquivo de configuração `newrelic.js` do seu aplicativo.

<Callout variant="important">
  Essa configuração está sujeita a alterações enquanto o conjunto de recursos do OpenTelemetry estiver em desenvolvimento.
</Callout>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Chave global para todo o recurso OpenTelemetry. Se definido como `false`, nenhum outro sub-recurso do OpenTelemetry (como `traces`, `logs` ou `metrics`) será ativado, independentemente da própria configuração desse sub-recurso.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    Ativa a ponte de instrumentações do OpenTelemetry (por exemplo, `@fastify/otel`) para o agente do New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-logs-enabled" title="opentelemetry.logs.enabled">
    ...
    Quando `true`, o agente configura automaticamente a API de logs do OpenTelemetry para enviar logs emitidos por meio da API específica do OTel para o New Relic.

    <Callout variant="important">
      Este recurso depende do [encaminhamento de logs de aplicativo](#application-logging-forwarding-enabled) — `application_logging.forwarding.enabled` também deve ser `true`.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-enabled" title="opentelemetry.metrics.enabled">
    ...
    Quando `true`, o agente configura automaticamente a API de métricas do OpenTelemetry para enviar métricas para o New Relic, anexadas à entidade do aplicativo instrumentado pelo agente.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-interval" title="opentelemetry.metrics.export_interval">
    ...
    O número de milissegundos entre cada tentativa de enviar métricas do OpenTelemetry para o New Relic. Deve ser maior ou igual a `opentelemetry.metrics.export_timeout`.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-timeout" title="opentelemetry.metrics.export_timeout">
    ...
    O número de milissegundos permitidos para uma operação de exportação de métricas do OpenTelemetry antes que ela precise ser concluída com sucesso. Se o tempo limite for excedido, isso é relatado por meio da API de diagnóstico do OpenTelemetry.
  </Collapser>
</CollapserGroup>

## Instrumentação do Apollo Server [#apollo-server-instrumentation]

Essas configurações personalizam o comportamento da instrumentação do Apollo Server (GraphQL) do agente. Elas aparecem na seção `apollo_server` do arquivo de configuração `newrelic.js` do seu aplicativo.
...
<CollapserGroup>
  <Collapser id="apollo-server-scalars" title="apollo_server.scalars">
    ...
    Habilita a captura de tempo para campos resolvidos com o tipo de retorno `GraphQLScalarType`. Isso pode ser desejado ao realizar cálculos demorados para retornar um valor escalar. Não recomendado para consultas que retornam um grande número de campos escalares pré-calculados.

    <Callout variant="tip">
      Os resolvedores de consulta/mutação são sempre capturados, mesmo quando retornam um tipo escalar.
    </Callout>
  </Collapser>
  ...
  <Collapser id="apollo-server-introspection-queries" title="apollo_server.introspection_queries">
    ...
    Ativa a captura de tempos para uma [IntrospectionQuery](https://www.graphql-js.org/api-v16/utilities/#introspectionquery).
  </Collapser>
  ...
  <Collapser id="apollo-server-service-definition-queries" title="apollo_server.service_definition_queries">
    ...
    Ativa a captura de tempos para uma [consulta de definição de serviço](https://www.apollographql.com/docs/federation/federation-spec/#fetch-service-capabilities) recebida de um Apollo Federated Gateway Server.
  </Collapser>
  ...
  <Collapser id="apollo-server-health-check-queries" title="apollo_server.health_check_queries">
    ...
    Ativa a captura de tempos para uma [consulta de verificação de integridade](https://www.apollographql.com/docs/federation/api/apollo-gateway/#servicehealthcheck) recebida de um Apollo Federated Gateway Server.
  </Collapser>
  ...
  <Collapser id="apollo-server-field-metrics" title="apollo_server.field_metrics">
    ...
    Ativa a captura de métricas para cada campo e argumento de resolvedor visto para uma consulta do Apollo. Destinado a identificar campos não utilizados em seu esquema do GraphQL.
  </Collapser>
</CollapserGroup>
