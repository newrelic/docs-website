...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    Quando ativado, o agente usa o ID da instância de metadados do GCP para definir o nome do host do aplicativo em execução.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    ...
    Quando ativado, e `gcp_use_instance_as_host` também estiver ativado, a revisão do GCP Cloud Run (`K_REVISION`) é anexada ao início do ID da instância ao definir o nome do host do aplicativo em execução, resultando em um nome do host de `${K_REVISION}-${instance_id}`.
  </Collapser>
</CollapserGroup>
