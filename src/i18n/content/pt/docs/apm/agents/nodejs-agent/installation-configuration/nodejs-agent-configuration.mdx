...
<CollapserGroup>
  ...
  <Collapser id="tx-attributes-exclude" title="attributes.exclude">
    ...
    Prefixo do atributo a ser excluído do evento de transação. Permite `*` como curinga no final. Por exemplo, no arquivo de configuração, isso incluiria todos os parâmetros, exceto `somethingSecret`:
  ...
  <Collapser id="tx-attributes-include" title="attributes.include">
    ...
    Insira a seguinte definição de rota expressa e solicite URL:
    ...
    O `_route_ parameter` é `id` e tem um valor de `abc123`. Isso se torna `request.parameters.route.id: abc123` na transação, segmento raiz e atributo Span. Este exemplo também tem um `_query_ parameter `de `id`, que tem um valor de `true`. Isso se tornaria `request.parameters.id: true` na transação, segmento raiz e atributo Span.
  </Collapser>
</CollapserGroup>
