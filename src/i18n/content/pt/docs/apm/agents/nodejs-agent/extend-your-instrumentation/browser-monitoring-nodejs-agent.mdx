...
1. No início da tag `head` da sua página HTML, insira os resultados de `newrelic.getBrowserTimingHeader()` após qualquer metatag `CHARSET`.
   ...
   <DoNotTranslate>**Exception:**</DoNotTranslate> Para obter compatibilidade máxima com o Internet Explorer, insira os resultados de `newrelic.getBrowserTimingHeader()` <DoNotTranslate>**after**</DoNotTranslate> qualquer meta tag `X-UA-COMPATIBLE HTTP-EQUIV` .
...
<CollapserGroup>
  ...
  >
    ...
    Usando <DoNotTranslate>**hapi**</DoNotTranslate>, no seu `app.js`:
