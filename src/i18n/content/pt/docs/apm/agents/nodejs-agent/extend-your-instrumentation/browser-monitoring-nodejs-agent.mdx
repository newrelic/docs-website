...
1. No início da tag `head` da sua página HTML, insira os resultados de `newrelic.getBrowserTimingHeader()` após qualquer metatag `CHARSET`.
   ...
   <DNT>**Exception:**</DNT> Para obter compatibilidade máxima com o Internet Explorer, insira os resultados de `newrelic.getBrowserTimingHeader()` <DNT>**after**</DNT> qualquer meta tag `X-UA-COMPATIBLE HTTP-EQUIV` .
