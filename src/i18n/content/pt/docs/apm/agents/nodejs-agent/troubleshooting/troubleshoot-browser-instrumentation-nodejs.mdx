...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        O browser do agente foi chamado fora de uma transação da web. Isso poderá ocorrer se você tentar gerar os dados do browser uma vez e depois armazená-los em cache ou se você os chamar em uma tarefa em segundo plano.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        A transação não tem nome. Se você não estiver usando Express ou Restify e não nomeou explicitamente a transação, este erro aparecerá. Isso evita a rolagem do nome da transação para <DoNotTranslate>\*\*/\*\*</DoNotTranslate>\*. Para obter mais informações, consulte [como nomear sua transação](/docs/nodejs/nodejs-agent-api).
      </td>
    </tr>
