...
Algumas métricas de VM do Node.js são coletadas por padrão usando APIs integradas do Node.js, enquanto outras exigem o pacote opcional [`@newrelic/native-metrics`](https://www.npmjs.com/package/@newrelic/native-metrics):

* **Amostradores padrão (nenhum módulo nativo necessário):** memória (`Memory/*`), tempo de espera da fila do loop de eventos (`Events/wait`) e CPU (`CPU/*`) são sempre coletados usando `process.memoryUsage()`, medição de atraso de `setTimeout` e `process.cpuUsage()`, respectivamente.
* **Amostradores de módulo nativo:** coleta de lixo (`GC/*`) e uso preciso de CPU do loop de eventos (`Nodejs/EventLoop/CPU/Usage`) requerem `@newrelic/native-metrics`. Se o módulo não estiver instalado ou falhar ao carregar, essas métricas não aparecerão e o agente continuará em execução apenas com os amostradores padrão.
...
<CollapserGroup>
  <Collapser id="time-in-garbage-collection" title="Tempo na coleta de lixo">
    <Callout variant="important">Requer `@newrelic/native-metrics`. Esta métrica não é coletada por padrão.</Callout>
  ...
  <Collapser id="memory" title="Memória">
    <Callout variant="tip">Coletado por padrão. Nenhum módulo nativo necessário.</Callout>
  ...
  <Collapser id="cpu" title="CPU">
    <Callout variant="tip">Coletado por padrão no Node.js v6.1.0 ou superior. Nenhum módulo nativo necessário.</Callout>
  ...
  <Collapser id="event-loop" title="Ciclo de eventos">
    <Callout variant="important">Requer `@newrelic/native-metrics`. Esta métrica não é coletada por padrão.</Callout>
