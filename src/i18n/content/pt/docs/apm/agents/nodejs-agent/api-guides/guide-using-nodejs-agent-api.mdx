...
Por padrão, o agente envia log diretamente no contexto dos erros e rastreamento do seu aplicativo. Para obter mais informações sobre como correlacionar dados log com outros dados de telemetria e estrutura compatível, consulte [nossos registros em documentos de contexto](/docs/logs/logs-context/configure-logs-context-nodejs).

Se estiver usando um mecanismo de registro que não é instrumentado pela New Relic, você poderá usar diretamente esta chamada de API para registrar e encaminhar seu registro no contexto:
...
## Chamadas de instrumento para serviços externos [#external-services]
...
Você também pode controlar o browser do agente por meio da chamada de API do agente APM. Para mais informações, consulte [monitoramento de Browser e agente Node.js.](/docs/agents/nodejs-agent/supported-features/new-relic-browser-nodejs-agent)
