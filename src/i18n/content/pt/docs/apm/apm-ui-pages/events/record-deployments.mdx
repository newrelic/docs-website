...
<Callout title="A API REST para implantações está chegando ao fim da vida útil" variant="important">
  Os endpoints de implantação da API REST v2 (`/v2/applications/{app_id}/deployments.json`) documentados nesta página, junto com a API de implantações v0 legada, [chegarão ao fim da vida útil em 31 de julho de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Após essa data, eles não estarão disponíveis.

  Em vez disso, registre implantações com o recurso de [Monitoramento de Alterações](/docs/change-tracking/overview/), usando a mutação `changeTrackingCreateEvent` do NerdGraph. Para mapeamentos de endpoint por endpoint, consulte o guia [Migrar da API REST v2 para o NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). As instruções REST abaixo permanecem como referência para integrações existentes que estão sendo migradas.
</Callout>

<Callout variant="tip">
  O Monitoramento de Alterações está disponível para <InlinePopover type="apm" />, <InlinePopover type="browser" />e <InlinePopover type="mobile" />.
</Callout>
...
Você pode registrar novas implantações e recuperar uma lista de implantações anteriores com a [API REST v2](/docs/apis/rest-api-v2/requirements/new-relic-rest-api-v2-getting-started) da New Relic ou com o [NerdGraph](/docs/change-tracking/config/nerdgraph/). Use o NerdGraph: os endpoints de implantação da API REST v2 [chegam ao fim da vida útil em 31 de julho de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Além disso, alguns agentes APM oferecem métodos específicos do agente para registrar implantações automaticamente.
...
<Callout title="Esses endpoints REST chegam ao fim da vida útil em 31 de julho de 2027" variant="important">
  Os endpoints de implantação da API REST v2 mostrados abaixo [chegam ao fim da vida útil em 31 de julho de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2) e deixarão de funcionar após essa data. Migre para a mutação `changeTrackingCreateEvent` do NerdGraph, consulte o guia [Migrar da API REST v2 para o NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). Essas instruções permanecem para referência durante a migração de integrações existentes.
</Callout>

Você pode usar a API REST v2 do New Relic para registrar a implantação e obter uma lista de implantações anteriores.

* Os exemplos neste documento usam `curl` como uma ferramenta de linha de comando. No entanto, você pode usar qualquer método para fazer suas requests REST. Você também pode criar e visualizar implantações com o [API Explorer](https://api.newrelic.com/docs/#/Applications/post_applications__application_id__deployments_json).
...
<Callout variant="important">
  Os métodos da API REST v2 referenciados abaixo [chegam ao fim da vida útil em 31 de julho de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Registre as implantações com a mutação `changeTrackingCreateEvent` do NerdGraph em vez disso, consulte o [guia de migração](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments).
</Callout>

Alguns agentes possuem métodos adicionais para registrar a implantação:

* <DNT>**All agents**</DNT>: use a [API REST v2](#api-instructions) da New Relic.
* <DNT>**C**</DNT>: sem métodos específicos do SDK. Use a [API REST](#api-instructions).
* <DNT>**Go**</DNT>: sem métodos específicos do agente. Use a [API REST](#api-instructions).
...
* <DNT>**Node.js**</DNT>: sem métodos específicos do agente. Use a [API REST](#api-instructions).
