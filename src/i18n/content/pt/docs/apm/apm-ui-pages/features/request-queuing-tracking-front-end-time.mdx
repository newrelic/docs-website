...
O APM rastreia o tempo após uma solicitação entrar em seus sistemas de produção e antes de chegar ao seu aplicativo. Chamamos essa parte do ciclo de vida da sua solicitação de <DoNotTranslate>**request queuing**</DoNotTranslate>. Dependendo das especificidades dos seus sistemas de produção, essa medição de tempo pode ou não incluir uma fila real na qual as solicitações entram. Também pode representar outras funções (como balanceamento de carga ou latência de rede interna).
...
Você deve configurar seu [agente](/docs/features/configuring-request-queue-reporting) e [servidor](/docs/features/request-queue-server-configuration-examples) New Relic para relatar o enfileiramento de solicitações. Em seguida, as informações aparecerão no gráfico <DoNotTranslate>**Requests time**</DoNotTranslate> do aplicativo selecionado para transação da web (da lista <DoNotTranslate>**Applications**</DoNotTranslate> do APM, selecione o aplicativo), bem como em outros locais da interface do usuário. A legenda do gráfico indica qual cor representa o enfileiramento de solicitações.
