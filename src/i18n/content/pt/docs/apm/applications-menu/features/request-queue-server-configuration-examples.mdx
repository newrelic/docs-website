...
O módulo <DoNotTranslate>**mod_headers**</DoNotTranslate> do Apache inclui uma variável `%t` formatada corretamente. Para habilitar relatórios de fila de solicitações, adicione este código à configuração do Apache:
