...
Com exceção do nosso recurso [Infinite Tracing](#tail-based), a maioria das nossas ferramentas de rastreamento usa uma abordagem de amostragem baseada no início. Isso aplica filtros a spans individuais antes que todos os spans em um trace cheguem, o que significa que as decisões sobre aceitar spans são tomadas no início (a &quot;head&quot;) do processo de filtragem. Essa estratégia de amostragem pode ser útil para capturar uma amostra representativa da atividade, evitando problemas de armazenamento e desempenho.
...
<CollapserGroup>
  <Collapser id="trace-origin-sampling" title="Agente de linguagem: amostragem adaptativa">
    ### Amostragem modernizada e compatibilidade com OpenTelemetry

    Nossos agentes de linguagem <InlinePopover type="apm" />utilizam amostragem adaptativa para fornecer visibilidade de alta fidelidade enquanto mantêm o desempenho do sistema. Essa abordagem garante que uma amostra representativa da atividade do sistema seja capturada em diversas arquiteturas, incluindo agentes nativos da New Relic e integrações OpenTelemetry (OTel).

    ### Como funciona a Amostragem Adaptativa

    O primeiro serviço monitorado em um rastreamento distribuído—a **origem do rastreamento**—é responsável pela decisão de amostragem. Essa decisão se propaga para todos os serviços downstream para garantir um rastreamento completo de ponta a ponta.

    Os agentes do New Relic usam um algoritmo adaptativo &quot;baseado em alvo&quot;. O agente analisa o throughput do minuto anterior para calcular a probabilidade de amostragem para o minuto atual. Por exemplo, se o seu serviço processa 100 transações por minuto e a meta de amostragem é 10, o agente se ajusta dinamicamente para selecionar aproximadamente 1 a cada 10 transações.

    Para encontrar a meta de amostragem padrão e os limites de span do seu agente, consulte a documentação específica do agente:

    * [Java](/docs/apm/agents/java-agent/configuration/distributed-tracing-java-agent)
    * [Node.js](/docs/apm/agents/nodejs-agent/installation-configuration/distributed-tracing-nodejs-agent)
    * [.NET](/docs/apm/agents/net-agent/configuration/distributed-tracing-net-agent)
    * [Go](/docs/apm/agents/go-agent/configuration/distributed-tracing-go-agent)
    * [Python](/docs/apm/agents/python-agent/configuration/distributed-tracing-python-agent)
    * [Ruby](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent)

    <Callout variant="tip">
      **Apenas rastreamento distribuído padrão**: Se um agente gerar mais spans do que o limite configurado em um minuto, alguns spans serão descartados, resultando em [rastreamentos fragmentados](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces) na interface. Os traces recebem uma prioridade aleatória quando selecionados para amostragem; portanto, se vários agentes precisarem descartar spans, eles podem tentar manter intactos os traces de maior prioridade, descartando primeiro os spans de traces de menor prioridade.

      Isso não se aplica ao Infinite Tracing, que envia todos os dados de rastreamento sem amostragem.
    </Callout>

    ### Alta Fidelidade e Ambientes Híbridos

    Como parte de nosso compromisso com padrões abertos, as versões mais recentes do agente New Relic são projetadas para **Compatibilidade Híbrida**.

    **Suporte à Taxa de Rastreamento**: Agentes modernos agora podem se alinhar às taxas de amostragem nativas do OTel, permitindo uma lógica de amostragem consistente em ambientes mistos. Consulte [Suporte à API do OpenTelemetry](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support) para detalhes de configuração.

    **Preservação de decisões de upstream**: Em ambientes de &quot;Modo Misto&quot; (onde tanto SDKs do OTel quanto agentes New Relic estão presentes), nossos agentes priorizam a preservação de decisões de rastreamento iniciadas por SDKs do OTel upstream para evitar rastreamentos fragmentados.
  </Collapser>
