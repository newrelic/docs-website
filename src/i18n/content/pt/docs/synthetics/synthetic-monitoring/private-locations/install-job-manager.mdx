...
Nossos gerenciadores de jobs de monitoramento sintético são recursos baseados em [contêiner](https://www.docker.com/resources/what-container)que aceitam e executam [monitores sintéticos](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring/#types-of-synthetic-monitors) atribuídos a [localizações privadas](/docs/synthetics/synthetic-monitoring/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations). O gerenciador de tarefas pode operar em um ambiente de sistema de contêiner Docker, um ambiente de sistema de contêiner Podman, um ambiente de sistema de orquestração de contêiner Kubernetes ou um ambiente de sistema de contêiner OpenShift.

<Callout variant="important">
  Se você estiver migrando dos runtimes do Node.js 16/Chrome 134 para o Node.js 22/Chrome 146, consulte o [guia de transição de runtime da localização privada](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) para estratégias de migração, exemplos de implantação e etapas de resolução de problemas.
</Callout>
...
<CollapserGroup>
  <Collapser id="docker-requirements" title="Requisitos do ambiente do sistema de contêiner Docker">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou superior (Recomendado para v447+)<br /> <DNT>**macOS:**</DNT> 10.11 ou superior<br /> <DNT>**Windows:**</DNT> Windows 10 64 bits ou superior
    ...
    <Callout variant="caution">
      Começando com o Synthetics Job Manager v447 (que usa o Ubuntu 24.04 como sua imagem base), RHEL 7 e outras distribuições usando versões do kernel Linux anteriores a 4.18 não são mais suportadas devido aos requisitos modernos do glibc e seccomp. Para uma experiência estável, use RHEL 8/9 ou uma distribuição Linux moderna equivalente.
    </Callout>
  </Collapser>

  <Collapser id="podman-requirements" title="Requisitos do ambiente do sistema de contêiner Podman">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou superior (Recomendado para v447+)<br />
          </td>
        </tr>
  ...
  <Collapser id="kubernetes-requirements" title="Requisitos do ambiente do sistema de orquestração de contêineres Kubernetes">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou superior (Recomendado para v447+) <br /><DNT>**macOS:**</DNT> 10.11 ou superior<br />
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> [4.18 ou superior (Recomendado para v447+)](https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-prerequisites#minimum-resource-requirements)<br /> <DNT>**macOS:**</DNT> 10.11 ou superior<br />
...
Se você estiver executando mais de um container de local privado Docker ou Podman no mesmo host, terá conflitos de porta. Para evitar esse conflito de porta, certifique-se de executar cada gerenciador de jobs em um host separado.
...
<CollapserGroup>
  <Collapser id="docker-update" title="Procedimento de inicialização do Docker">
    ...
    3. Execute o script apropriado para o seu sistema. Adapte o padrão comum `/var/run/docker.sock` nos exemplos a seguir para corresponder ao seu sistema.
       ...
       * Windows:
       ...
       Visualize os logs do seu contêiner do job manager:
  ...
  <Collapser id="podman-update" title="Procedimento de inicialização do Podman">
    1. Localize sua [chave de localização privada](#private-location-key).
    ...
    Para visualizar os logs do seu contêiner do gerenciador de jobs:
  ...
  <Collapser id="kubernetes-install" title="Procedimento de inicialização do Kubernetes">
    1. Localize sua [chave de localização privada](#private-location-key).
...
<CollapserGroup>
  <Collapser id="docker-stop" title="Procedimento de parada do Docker">
  ...
  <Collapser id="podman-stop" title="Procedimento de parada do Podman">
  ...
  <Collapser id="kubernetes-delete" title="Procedimento de exclusão do Kubernetes">
...
<CollapserGroup>
  <Collapser id="docker-dependencies" title="Dependência do Docker">
...
<CollapserGroup>
  <Collapser id="podman-dependencies" title="Dependências do Podman">
...
<CollapserGroup>
  <Collapser id="run-non-root" title="Execute como usuário não root para Docker">
