...
No monitoramento sintético, um local privado é uma coleção de gerenciadores de jobs de sintéticos. Os gerenciadores de jobs do Synthetics são aplicativos conteinerizados que recebem e gerenciam jobs configurados por meio da interface do Synthetics.

Um local privado pode conter qualquer número de gerenciadores de jobs do Synthetics. Locais privados permitem estender sua cobertura de monitoramento sintético para novas localizações geográficas e monitorar sites por trás do seu firewall (como um site de intranet).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Para contas baseadas nos EUA (padrão): `https://synthetics-horde.nr-data.net/`
        ...
        Para contas baseadas no JP: `https://synthetics-horde.jp.nr-data.net/`
      </td>
    </tr>
    ...
    <tr id="compatibility-outbound-network">
      ...
      <td>
        O gerenciador de jobs de sintéticos precisa se conectar ao endpoint horde do monitoramento sintético para receber e processar jobs. Se suas regras de firewall não permitirem acesso direto, você deve [configurar o acesso via proxy](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#FAQ).
...
Antes de [instalar os gerenciadores de jobs do Synthetics](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager), você precisa criar um local privado.
...
4. Opcional: Defina estas configurações adicionais:
   ...
   * <DNT>**[Verified script execution](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations)**</DNT>: Exigir uma frase secreta ao atribuir scripts a este local ou ao adicionar gerenciadores de jobs ao local.

5. Selecione <DNT>**Create**</DNT>.

Após criar o local, o monitoramento sintético lista seu <DNT>**Private location key**</DNT> na aba <DNT>My location</DNT> para que você possa [instalar um gerenciador de jobs](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager).
...
Cada gerenciador de jobs de synthetics pode executar cerca de 75 verificações de monitor de ping por minuto. Se a fila de jobs de um local específico estiver aumentando, [adicione mais gerenciadores de jobs](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager). A capacidade de um gerenciador de jobs pode variar dependendo do desempenho da rede e dos recursos de hardware.
...
O monitoramento sintético inclui ferramentas para gerenciar locais e gerenciadores de jobs de sintéticos. Você também pode [instalar gerenciadores de jobs do Synthetics](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) e limpar a fila de jobs se ela ficar congestionada.
...
<CollapserGroup>
  <Collapser id="clear-job-queue" title="Limpe a fila de trabalhos">
    ...
    Isso é útil quando o número de tarefas agendadas aumenta mais rápido do que os gerenciadores de tarefas conseguem processá-las, como quando o gerenciador de tarefas está offline.
  </Collapser>
  ...
  <Collapser id="view-status" title="Ver status">
    Para visualizar o status de um gerenciador de jobs de synthetics individual, selecione o nome do local pai na lista.

    O [indicador de status de integridade](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#health-status) verde identifica gerenciadores de jobs do Synthetics ativos.
  </Collapser>
  ...
  <Collapser id="delete-location" title="Excluir um local">
    ...
    Isso não encerra nenhum gerenciador de jobs atribuído a esse local. Os gerenciadores de jobs devem ser encerrados manualmente ou reatribuídos.
  </Collapser>
  ...
  <Collapser id="enable-disable-vse" title="Habilitar ou desabilitar a execução de script verificada">
    ...
    * Você precisa passar a variável de ambiente [VSE\_PASSPHRASE](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-configuration/#environment-variables) para o gerenciador de jobs. Em seguida, na aba <DNT>**Private locations**</DNT>, clique em <DNT>**Edit**</DNT> e, no menu, habilite a opção <DNT>**Verified script execution**</DNT>.
  </Collapser>

  <Collapser id="rotate-private-location-keys" title="Girar chaves de localização privada">
    Locais privados usam uma chave de local privado. Este token permite que o gerenciador de tarefas de local privado obtenha tarefas a serem executadas e reporte o resultado de volta à plataforma New Relic.
    ...
    <Steps>
      <Step>
        ...
        5. Atualize seus gerenciadores de jobs com o novo local privado. A seção <DNT>**Configure and deploy**</DNT> inclui código de exemplo:
