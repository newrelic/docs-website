...
Continue lendo para saber como configurar seu [minion privado em contêiner (chamadas por minuto)](/docs/install-containerized-private-minions-cpms).

Você pode fazer o seguinte para personalizar suas chamadas por minuto:

* Configure [módulos personalizados](#custom-modules) para [navegador com script](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers) no New Relic.
* Preserve os dados de lançamento com [armazenamento permanente de dados](#preserve-data-volume).
* Use [variáveis de ambiente](#environment-variables) em sua configuração.
...
Todos os diretórios e arquivos <DoNotTranslate>**must**</DoNotTranslate> recebem propriedade de grupo como `3729` com permissões de leitura/gravação. Isso garante que o Runner, que usa `uid: 1000` e `gid: 3729`, tenha acesso a todos os volumes montados. No entanto, o minion é capaz de executar como root (`uid: 0`) ou com qualquer `uid` entre o intervalo de `[2000, 4000]`, inclusive. Para obter mais informações, consulte executando como [não root no Kubernetes](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#kubernetes-env-config) ou [Docker](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#docker-env-config).
...
* Os diretórios e arquivos devem ser adicionados antes da inicialização minion , caso contrário, o minion deverá ser reiniciado para detectar as atualizações
...
1. Crie um diretório que contenha um `package.json`, seguindo as [diretrizes oficiais do npm](https://docs.npmjs.com/files/package.json), na raiz do diretório. Tudo o que estiver contido no campo `dependencies` será instalado pelas chamadas por minuto no início, e disponibilizado ao executar o monitor naquele minion privado.
   ...
   <CollapserGroup id="custom-module-dir">
     ...
     >
       Você pode declarar um `package.json` por versão do Node.js que substituirá o nível raiz `package.json`. Isso permite que um script de monitor seja atualizado por tempo de execução do monitor caso a [versão Node.js de um tempo de execução](/docs/synthetics/new-relic-synthetics/scripting-monitors/scripted-monitor-runtime-environment) não seja mais compatível com sua dependência. Conforme mostrado no [primeiro exemplo](#example-module-directory), os módulos locais ainda podem ser definidos em um diretório específico da versão. Se um `package.json` não estiver definido para uma versão específica do Node.js, então o nível raiz `package.json` será usado para instalar a dependência.
...
Junto com módulos locais e hospedados, você também pode usar [módulos Node.js.](/docs/synthetics/new-relic-synthetics/scripting-monitors/import-nodejs-modules) Para alterar os módulos personalizados usados pelas suas chamadas por minuto, modifique `package.json` e reinicie as chamadas por minuto. Ele detectará a alteração na configuração durante a reinicialização e, em seguida, limpará e reinstalará.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            O minion Network Healthcheck está desativado, para gerenciar as chamadas por minuto e verificar o acesso público à Internet. O padrão é 'false', quando definido como 'true' as chamadas por minuto irão ignorar esta verificação de saúde.
          </td>
        </tr>
