...
2. Depois de criar o diretório de módulos personalizados e o `package.json`, aplique-os ao seu SJM para docker e Kubernetes.
   ...
   <CollapserGroup>
     ...
     <Collapser id="kubernetes" title="Kubernetes">
       ...
       2. Neste ponto, o `mount-custom-mods-pod` deve ser criado e configurado para usar o volume.
       ...
       3. Verifique se há algum aviso relacionado ao PV, PVC ou `mount-custom-mods-pod`.
       ...
       1. Não é necessário copiar `node_modules` pois ele será gerado pelo SJM em `npm install`.
       ...
       2. Verifique se o `mount-custom-mods-pod` está em execução.
       ...
       4. Verifique se `/var/lib/newrelic/synthetics/modules/custom-modules/package.json` existe no PV.
       ...
       2. Procure o seguinte no registro pod `synthetics-job-manager`:
