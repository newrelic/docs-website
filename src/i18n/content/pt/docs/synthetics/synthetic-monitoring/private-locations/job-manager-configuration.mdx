...
$$C\_req = N\_m \vezes D\_j \vezes \frac1P\_m$$

* $C\_req$ = **Núcleos de CPU necessários**
* $N\_m$ = O **número** total de monitores pesados que você planeja executar.
* $D\_j$ = A **duração média** de um trabalho em minutos.
* $P\_m$ = O **período** do monitor em minutos (por exemplo, um monitor que é executado a cada 5 minutos tem um período de 5).
...
**3. Encontre a duração média da tarefa em minutos ($D\_j$):** esta consulta encontra a duração média de execução de tarefas não ping concluídas e converte o resultado de milissegundos para minutos. `executionDuration` representa o tempo que o trabalho levou para ser executado no host.
...
$$Paralelismo\_máximo \aprox. 15 \vezes \mathrmajd\_m$$

Onde $\mathrmajd\_m$ é a duração média do trabalho em minutos.
...
$$Complementos = \frac300ajd\_s$$

Onde $\mathrmajd\_s$ é a duração média do trabalho em segundos.

$$Paralelismo = \fracsj\_5mCompletions$$

Onde $\mathrmsj\_5m$ é o número de trabalhos Sintéticos por 5 minutos.
