    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Para contas baseadas nos EUA (padrão), o endpoint é: `https://synthetics-horde.nr-data.net.`
            ...
            Para contas baseadas no Japão, o endpoint é: `https://synthetics-horde.jp.nr-data.net/`
...
Módulos de nó personalizados são fornecidos no SJM. Eles permitem criar um conjunto personalizado de [módulos do Node](https://docs.npmjs.com/about-packages-and-modules) e usá-los em monitores com script (API com script e navegador com script) para monitoramento sintético.
...
## Configurar certificados de CA personalizados para localizações privadas [#custom-ca-certificates]
...
<CollapserGroup>
  <Collapser id="custom-ca-docker" title="Docker">
    Monte o diretório do host local contendo seus arquivos PEM no caminho do contêiner SJM `/var/lib/newrelic/synthetics/certs` como um volume somente leitura.
    ...
    **Como isso funciona:** o SJM detecta essa montagem na inicialização e propaga os certificados como somente leitura para cada contêiner dinâmico que ele gera.
  </Collapser>

  <Collapser id="custom-ca-podman" title="Homem-Pod">
    Para ambientes Podman — especialmente aqueles executados com o SELinux ativado — você deve anexar o sinalizador `:z` ou `:Z` à montagem de volume para configurar o contexto de segurança correto.
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes (Helm)">
    Ao implantar o SJM em um cluster do Kubernetes via Helm, você pode gerenciar seu pacote de certificados de CA personalizado como um secret genérico do Kubernetes.

    **1. Crie o segredo do Kubernetes**

    Armazene seu(s) arquivo(s) de certificado PEM em um secret dentro do namespace do agente da New Relic:
    ...
    **2. Atualize seus valores do Helm**

    Referencie o segredo recém-criado em sua configuração `values.yaml` personalizada:
    ...
    **Como isso funciona:** na inicialização, o SJM monta esse secret e o repassa automaticamente para todos os pods de execução recém-criados.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="mtls-docker" title="Docker">
    Montar um diretório contendo `cert-map.json` e os arquivos de certificado e chave referenciados:
    ...
    Se os nomes do host do servidor mTLS não puderem ser resolvidos via DNS dentro dos contêineres de tempo de execução, adicione mapeamentos de nome do host usando `RUNTIME_EXTRA_HOSTS`:
  ...
  <Collapser id="mtls-podman" title="Homem-Pod">
    Montar o diretório de certificados do cliente. Para o SELinux, adicionar `:z` ou `:Z`:
  ...
  <Collapser id="mtls-kubernetes" title="Kubernetes">
    Crie um Kubernetes Secret contendo `cert-map.json` e todos os arquivos de certificado e chave referenciados:
    ...
    Referenciar o secret nos valores do Helm:
    ...
    O secret é montado como somente leitura em todos os pods de tempo de execução automaticamente.
  </Collapser>
</CollapserGroup>

## Limitações do suporte a certificados [#certificates-support-limitations]
...
## Considerações de dimensionamento [#vm-sizing]

<CollapserGroup>
  <Collapser id="docker-podman-sizing" title="Para Docker e Podman">
    ...
    * $R\_\{growth}$ = A **taxa** em que sua fila `jobManagerHeavyweightJobs` está **crescendo** por minuto.
    * $D\_\{avg,m}$ = A **duração média** de trabalhos pesados em **minutos**.
    ...
    **2. Encontre a taxa de crescimento da fila por minuto ($R\_\{growth}$)**: Esta consulta calcula o crescimento médio por minuto da fila `jobManagerHeavyweightJobs` em um gráfico de série temporal. Uma linha acima de zero indica que a fila está aumentando, enquanto uma linha abaixo de zero significa que ela está diminuindo.
    ...
    **4. Encontre a duração média do job em minutos ($D\_\{avg,m}$)**: Esta consulta encontra a duração média de execução de jobs não-ping concluídos e converte o resultado de milissegundos para minutos. `executionDuration` representa o tempo que o job levou para executar no host.
    ...
    **5. Encontre o período médio do monitor heavyweight ($P\_\{avg,m}$):** Se a fila de `jobManagerHeavyweightJobs` da localização privada estiver aumentando, não é exato calcular o período médio do monitor a partir dos resultados existentes. Isso precisará ser estimado a partir da lista de monitores na página [Monitores sintéticos](https://one.newrelic.com/synthetics). Certifique-se de selecionar a conta New Relic correta e pode ser necessário filtrar por `privateLocation`.
  ...
  <Collapser id="k8s-sizing" title="Para Kubernetes e OpenShift">
    ...
    **Paralelismo Estimado:** Isso determina quantos workers (pods) você precisa executar simultaneamente para lidar com sua carga de trabalho de 5 minutos.
    ...
    Onde $N\_m$ é o seu **número de jobs por 5 minutos**. Este valor $P\_\{est}$ é o seu **paralelismo estimado**.
    ...
    **5. Escale horizontalmente se necessário**
    ...
    Para mais informações sobre como adicionar implantações, consulte [Escalonamento horizontal com múltiplas implantações do SJM](#scaling-out-with-multiple-sjm-deployments).
