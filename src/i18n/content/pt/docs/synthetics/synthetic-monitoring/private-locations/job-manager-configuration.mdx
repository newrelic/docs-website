...
## Considerações de dimensionamento para Docker, Kubernetes e OpenShift [#kubernetes-sizing]

### Docker [#docker]

Para garantir que sua localização privada seja executada com eficiência, você deve provisionar recursos de CPU suficientes no seu host Docker para lidar com sua workload de monitoramento. Muitos fatores influenciam o dimensionamento, mas você pode estimar rapidamente suas necessidades.

Você precisará **de 1 núcleo de CPU para cada monitor pesado simultâneo** (ou seja, cada browser com script ou teste de API com script).

Abaixo estão duas fórmulas para ajudar você a calcular o número de núcleos necessários, seja para diagnosticar uma configuração atual ou planejar uma futura.

#### Fórmula 1: Para diagnosticar um local existente
...
**A equação:** $$C\_req = (J\_processed + Q\_growth) \times D\_j$$

* $C\_req$ = **Núcleos de CPU necessários**
* $J\_processed$ = A taxa de trabalhos sendo **processados** por minuto.
* $Q\_growth$ = A taxa de **crescimento** da sua fila `jobManagerHeavyweightJobs` por minuto.
* $D\_j$ = A **duração média** de um trabalho em minutos.
...
#### Fórmula 2: Para prever uma localização nova ou futura
...
**A equação:** $$C\_req = N\_m \times F\_j \times D\_j$$

* $C\_req$ = **Núcleos de CPU necessários**
* $N\_m$ = O **número** total de monitores pesados que você planeja executar.
* $F\_j$ = A **frequência** média dos monitores em trabalhos por minuto (por exemplo, um monitor executado a cada 5 minutos tem uma frequência de 1/5 ou 0,2).
* $D\_j$ = A **duração média** de um trabalho em minutos.
...
* **Duração do trabalho ($D\_j$):** Sua média deve incluir trabalhos que **expiram** (geralmente \~3 minutos), pois eles mantêm um núcleo durante toda a sua duração.
* **Falhas e novas tentativas de trabalho:** quando um monitor falha, ele é automaticamente repetido. Essas tentativas são trabalhos adicionais que aumentam a carga total. Um monitor que falha e tenta repetidamente **multiplica efetivamente sua frequência**, impactando significativamente as taxas de transferência.
...
Você pode executar essas consultas no [criador de consultas](/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data/) para obter as entradas para a fórmula de diagnóstico. Certifique-se de definir o intervalo de tempo para um período longo o suficiente para obter uma média estável.

**1. Encontrar trabalhos processados por minuto ($J\_processed$):** esta consulta conta o número de trabalhos não ping (pesados) concluídos no último dia e mostra a taxa média por minuto.
...
**2. Encontre o crescimento da fila por minuto ($Q\_growth$):** esta consulta calcula o crescimento médio por minuto da fila `jobManagerHeavyweightJobs` em um gráfico de série temporal. Uma linha acima de zero indica que a fila está crescendo, enquanto uma linha abaixo de zero significa que ela está diminuindo.
...
**3. Encontre a duração média da tarefa em minutos ($D\_j$):** esta consulta encontra a duração média de execução de tarefas não ping concluídas e converte o resultado de milissegundos para minutos. Por que usar `executionDuration`? Ele representa o tempo que o trabalho levou para ser executado no host, que é o que queremos medir.
...
**4. Encontre o número total de monitores pesados ($N\_m$):** Esta consulta encontra a contagem exclusiva de monitores pesados.
...
**5. Encontre a frequência média do monitor de peso pesado ($F\_j$):** Se a fila `jobManagerHeavyweightJobs` da localização privada estiver crescendo, não será preciso calcular a frequência média do monitor a partir dos resultados existentes. Isso precisará ser estimado a partir da lista de monitores na página [Monitores Sintético](https://one.newrelic.com/synthetics). Certifique-se de selecionar a conta New Relic correta e talvez seja necessário filtrar por `privateLocation`.
...
* **Dimensionamento:** embora a fórmula de dimensionamento seja diferente, os mesmos princípios se aplicam. Para lidar com um grande volume de trabalhos de ping, talvez seja necessário aumentar os recursos do seu host ou expandir implantando mais gerenciadores de trabalhos para manter a fila `pingJobs` limpa e evitar atrasos.

### Kubernetes e OpenShift [#k8s]

Cada tempo de execução usado pelo gerenciador de tarefas Kubernetes e do OpenShift Sintético pode ser dimensionado independentemente definindo valores no [gráfico do Helm](https://github.com/newrelic/helm-charts/tree/master/charts/synthetics-job-manager).
