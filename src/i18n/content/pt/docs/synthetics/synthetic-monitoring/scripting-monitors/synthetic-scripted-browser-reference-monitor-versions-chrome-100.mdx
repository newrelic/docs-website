...
<CollapserGroup>
  <Collapser id="waitForAndFindElement" title="Método waitForAndFindElement (localizador, tempo limite)">
    ```js
  ...
  <Collapser id="waitForPendingRequests" title="Método waitForPendingRequests (tempo limite)">
    ```js
  ...
  <tbody>
    <tr id="options-manage-addCookie">
      ...
      <td>
        ...
        `spec` é um objeto de registro que descreve um cookie do browser. Para obter mais informações, consulte a [documentação do Webdriver](https://www.w3.org/TR/webdriver1/#cookies).
...
* Permite o uso do nível superior `await` porque executamos o script inteiro dentro de uma função assíncrona nos bastidores.
