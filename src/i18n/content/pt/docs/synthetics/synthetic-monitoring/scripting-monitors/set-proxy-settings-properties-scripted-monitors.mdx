...
<Callout variant="important">
  É possível definir a configuração do servidor proxy para monitores com script Sintético para versões de monitor 0.4.0 ou superior.
</Callout>
...
<CollapserGroup>
  ...
  >
    <Callout variant="tip">
      Este método é <DoNotTranslate>**Chrome-specific**</DoNotTranslate>: aplica-se apenas aos tipos de script do browser com monitor. Ele pode ser usado para permitir uma configuração de proxy mais flexível e complexa. Na maioria dos casos, este método não será necessário.
    </Callout>
