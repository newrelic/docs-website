...
Para obter uma lista de todas as funções disponíveis, consulte [Referência do browser com script](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100/).

O monitor com script é controlado pelo [Selenium WebDriverJS](https://seleniumhq.github.io/selenium/docs/api/javascript/index.html). Cada vez que seu script é executado, a New Relic cria um browser Google Chrome totalmente virtualizado baseado em Selenium que navega em seu site e segue cada etapa do script. O monitoramento sintético inclui um editor script estilo IDE que sugere funções, localizador e outros elementos para simplificar o script.
