...
## Use diferentes ambientes de execução [#how_to]

O monitor existente é padronizado para o tempo de execução usado no momento da criação. Para evitar que seu monitor crítico quebre durante futuros processos de fim de vida útil, recomendamos que você converta seu monitor público o quanto antes. Converter:
...
Você não precisa importar `$env` porque ele está prontamente disponível, semelhante às variáveis `$browser` e `$http` . Por exemplo:
...
<Callout variant="tip">
  Se não tiver certeza sobre a versão de tempo de execução do seu monitor, [edite- monitor](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime/#runtime-convert) e verifique o menu suspenso <DoNotTranslate>**"Runtime"**</DoNotTranslate> na guia "Configurar monitor". Você também pode consultar o atributo `runtimeTypeVersion` no evento `SyntheticCheck` onde está o `runtimeType = 'NODE_API'`.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Browser**</DoNotTranslate>
      ...
      : Chrome 117 (Requer a versão 2.2+ da imagem Sintético-node-navegador-runtime para localização privada. O Chrome 112 ou 100 será usado por imagens de tempo de execução de navegador mais antigas.)

    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>

      : 4.1.0 (exposto via `$selenium` e `$webDriver` com sintaxe Selenium Webdriver 3.6 compatível com versões anteriores exposta via `$browser` e `$driver`)
...
<Callout variant="tip">
  Se não tiver certeza sobre a versão do seu monitor, acesse <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets) > Upgrade monitors**</DoNotTranslate>. Você não verá a opção <DoNotTranslate>**Upgrade monitors**</DoNotTranslate> se estiver usando a versão de tempo de execução mais recente.
</Callout>

Aqui estão os detalhes da versão do monitor para todos [os tipos de monitor](/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors) , exceto ping:
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 3.6.0 (exposto via `$browser` e `$driver`)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 3.5.0 (exposto via `$browser>` e `$driver`)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 2.47.0 (exposto via `$browser>` e `$driver`)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 2.44.0 (exposto via `$browser>` e `$driver`)
