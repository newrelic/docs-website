...
Você pode definir credenciais seguras no New Relic ou com a [API NerdGraph](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials). As credenciais são armazenadas de forma segura utilizando criptografia AES-GCM de 256 bits em repouso, com chaves gerenciadas pelo [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/).
...
Você pode adicionar ou atualizar credenciais seguras usando a interface do usuário ou a [API NerdGraph de monitoramento sintético](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials). Observe que os valores não podem ser visualizados, apenas as chaves.
...
2. Para adicionar uma nova credencial segura, procure o botão <DNT>**Create secure credential +**</DNT> . Se você já tiver credenciais adicionadas, este botão estará no canto superior direito.
   ...
   * Dicas para criar o <DNT>**Value**</DNT>: Use qualquer combinação de caracteres alfanuméricos ou especiais. Máximo de 10000 caracteres. Este campo não é acessível via [API do Nerdgraph](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials).
