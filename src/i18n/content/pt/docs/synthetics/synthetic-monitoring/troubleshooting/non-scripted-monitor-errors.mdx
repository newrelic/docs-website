...
<CollapserGroup id="non-scripted-errors">
  <Collapser className="freq-link" id="exceed-duration" title="ERRO: O trabalho expirou após 65 segundos">
    ### Problema
  ...
  <Collapser className="freq-link" id="xxx-response" title="HTTPError: o servidor respondeu com código de resposta HTTP XXX">
    ### Problema
  ...
  <Collapser className="freq-link" id="ssl-error" title="SSLVerificationError: (ERRO)">
    ### Problema
  ...
  <Collapser className="freq-link" id="response-error" title="ResponseValidationError: a resposta não continha a string esperada">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-timeout" title="NetworkError: Tempo limite de leitura esgotado">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-socket-closed" title="NetworkError: O soquete está fechado">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-host-unreachable" title="NetworkError: Nenhuma rota para o host (Host inacessível)">
    ### Problema
    ...
    Se a falha estiver ocorrendo em um local privado de monitoramento sintético, certifique-se de que a configuração de rede do Synthetics Job Manager esteja configurada corretamente e que o hostname de destino seja resolvível e alcançável via interface de linha de comando virtual do Synthetics Job Manager.
  ...
  <Collapser className="freq-link" id="httperror-redirects" title="HTTPError: O servidor nos enviou muitos redirecionamentos (20)">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-resolution" title="NetworkError: falha na resolução de DNS para host: (HOST)">
    ### Problema
    ...
    <DNT>**Private synthetic monitoring’s locations:**</DNT> Verifique se a configuração de rede do Synthetics Job Manager está correta. Se o hostname de destino for interno, certifique-se de que o Synthetics Job Manager esteja usando o serviço de DNS interno da sua rede que seja capaz de resolver o host. O Synthetics Job Manager e os contêineres runner que ele cria no host (para executar verificações que não sejam de ping) devem herdar a configuração de DNS do host `/etc/resolv.conf`.

    <DNT>**Docker:**</DNT> Argumentos de rede como `–dns` ou `-network` usados no comando Docker run no Synthetics Job Manager serão usados apenas pelo contêiner do job manager, mas não pelos contêineres runner. Se o DNS apontar para a interface de loopback, como `127.0.0.1`, defina uma configuração de DNS no [nível do daemon do Docker](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-configuration-file) ou use uma ferramenta como o `dnsmasq` para garantir que o runner encaminhe as solicitações de DNS na interface de bridge do Docker.
    ...
    Se você estiver vendo falhas de monitor relacionadas à resolução de DNS em um local privado de monitoramento sintético, isso geralmente é causado por uma configuração de rede inválida no Synthetics Job Manager desse local.
  </Collapser>
