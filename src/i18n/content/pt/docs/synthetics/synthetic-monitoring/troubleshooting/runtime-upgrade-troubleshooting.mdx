...
<Callout variant="tip">
  Se você estiver executando monitores em localizações privadas, consulte o [guia de transição de runtime de localização privada](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) para obter as etapas de implantação do SJM e a resolução de problemas específicos da localização privada.
</Callout>
...
Certifique-se de que todas as promises tenham `.then()`, `.catch()` ou sejam `await`ed.
...
Aguarde corretamente a resposta usando async/await:
...
### Conexões HTTP não fechadas [#unclosed-http]

**Sintoma:** A validação do monitor falha com erros de open handle **Causa:** Conexões HTTP feitas em loops sem aguardar ou realizar limpeza
...
Aguarde e trate adequadamente as conexões:
