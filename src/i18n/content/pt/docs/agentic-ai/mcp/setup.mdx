...
## Configuração da CLI do Codex [#codex-cli]

### Método da Chave de API [#api-key]

1. Certifique-se de ter a [ferramenta CLI do Codex instalada](https://developers.openai.com/codex/cli/) em sua máquina.

2. Configure o servidor MCP editando o arquivo de configuração localizado em `~/.codex/config.toml` e adicione a seguinte configuração:
...
3. Autenticar e conectar:
   ...
   * Abra seu terminal e execute `codex`.
   * Digite o comando `/mcp` e pressione Enter.
   * Você verá o servidor New Relic listado com uma URL.
...
3. Autenticar e conectar:
   ...
   * O fluxo Oauth o levará à página de autenticação onde você precisará fornecer suas credenciais para autenticar.
   * Após a autenticação, o navegador confirmará o sucesso.
   * Abra seu terminal e execute `codex`.
   * Digite o comando `/mcp` e pressione Enter.
   * Você verá o servidor New Relic listado com um URL. Abra esse endereço URL no seu navegador.
