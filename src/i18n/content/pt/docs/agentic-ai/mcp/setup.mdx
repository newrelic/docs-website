...
O Claude Code oferece suporte aos métodos de autenticação OAuth (recomendado) e chave de API. O OAuth requer a versão 2.1.186 do Claude Code ou posterior. Caso utilize versões mais antigas, a autenticação deve ser feita via `/mcp` no Claude em vez de `claude mcp login`.
...
2. Configure o servidor MCP abrindo (ou criando) `~/.kiro/settings/mcp.json` e adicione a seguinte configuração:
...
3. Abra seu terminal e execute `kiro-cli`. O servidor MCP da New Relic se conectará automaticamente.
...
O VS Code continua sendo totalmente compatível com ambos os métodos de autenticação. É necessário o VS Code (versão 1.91 ou posterior).
