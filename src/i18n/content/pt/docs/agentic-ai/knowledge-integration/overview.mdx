...
* **Resolva incidentes mais rapidamente:** obtenha acesso imediato ao contexto relevante de seus runbooks e documentação internos diretamente no New Relic AI, resultando em um tempo médio de resolução (MTTR) mais rápido.

* **Obtenha recomendações práticas e personalizadas:** as respostas fornecidas são específicas para seu ambiente e baseadas em suas próprias práticas recomendadas e dados históricos.

* **Responda a perguntas críticas e específicas do contexto:** encontre facilmente soluções para problemas que já foram resolvidos antes. Faça perguntas como:
...
Para começar a indexar seu conteúdo do Confluence, você precisa:
