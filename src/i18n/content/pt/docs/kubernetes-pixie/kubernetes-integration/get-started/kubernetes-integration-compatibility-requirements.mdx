...
A [integração do Kubernetes](/docs/integrations/kubernetes-integration/get-started/introduction-kubernetes-integration) é compatível com várias plataformas diferentes, incluindo GKE, EKS, AKS, OKE, OpenShift e outras. Cada um tem uma compatibilidade diferente com a nossa integração. Você pode encontrar mais informações nesta página.
...
O monitoramento do Windows suporta dois modos:

* **Modo privilegiado**, que usa contêineres HostProcess do Windows para coletar métricas completas de nível de nó. O modo privilegiado está habilitado por padrão.
* **Modo não privilegiado**, que é executado como um usuário de contêiner padrão com visibilidade reduzida do host.

Para obter detalhes sobre como esses modos funcionam e orientações sobre como configurá-los com segurança, consulte [Modo privilegiado vs. não privilegiado](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode). Para instruções completas de configuração, consulte [Monitorar nós do Windows](/docs/kubernetes-pixie/kubernetes-integration/installation/windows).

## Requisitos do modo privilegiado

Por padrão, o chart de integração do Kubernetes da New Relic configura o sidecar do Infrastructure Agent para ser executado em **modo privilegiado** (`privileged: true`). Este nível de acesso é necessário para coletar métricas detalhadas do sistema (como E/S de armazenamento, interfaces de rede física e processos do sistema) dos sistemas de arquivos `/proc` e `/sys` do host subjacente.

Se as políticas de segurança do seu cluster (por exemplo, Pod Security Standards ou SCCs do OpenShift) não permitirem contêineres privilegiados, você pode precisar desativar essa configuração.

Para instruções detalhadas sobre como desabilitar o modo privilegiado e uma lista completa das métricas que ficarão indisponíveis, consulte [Modo privilegiado vs. não privilegiado](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode).
...
* Modo privilegiado do Windows em clusters do Google GKE, pois o GKE não suporta contêineres Windows `HostProcess`. Você ainda pode monitorar nós do Windows no GKE em [modo não privilegiado](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode/#windows-nodes).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        O OKE usa o CRI-O como seu runtime de contêiner padrão. Ao configurar o auto-attach do APM, adicione `docker.io/` como prefixo aos caminhos de imagem do Agent. Por exemplo: `image: docker.io/newrelic/newrelic-java-init:latest`.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Compatível com a versão 3.x do VKS, em execução no vSphere Supervisor 9.0 a 9.1 (vSphere 8.x e 9.x)
      </td>
    </tr>
  </tbody>
</table>
