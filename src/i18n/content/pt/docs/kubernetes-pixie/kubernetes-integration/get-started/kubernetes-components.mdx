...
O gráfico `newrelic-infrastructure` é dividido em três componentes diferentes: `nrk8s-ksm`, `nrk8s-kubelet` e `nrk8s-controlplane`. O primeiro é uma implantação e os próximos dois são DaemonSets.

Cada um dos componentes possui dois contêineres:
...
Os clientes deverão permitir a monitorização de qualquer destino métrica no KSM caso essas métricas não estejam habilitadas por defeito. Por exemplo, KSM v2+ desabilita rótulos e anotações métricas por padrão. os clientes podem habilitar o monitoramento dos rótulos de destino e métricas de anotações usando as opções `metricLabelsAllowlist` ou `metricAnnotationsAllowList` descritas [aqui](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/cli-arguments.md) em seu cluster do Kubernetes.
...
Você pode verificar detalhes completos sobre todas as opções que podem ser invertidas no [`README.md`do gráfico](https://github.com/newrelic/nri-kubernetes/blob/main/charts/newrelic-infrastructure/README.md).
