---
title: Integração New Relic eAPM
tags:
  - Integrations
  - eBPF integration
  - eAPM
...
## Principais diferenças entre eAPM e APM [#key-differences]
...
## instalação do agente eAPM [#installation]
...
6. Para instalar o gráfico Helm do agente eAPM, substitua `your-custom-values.yaml` pelo caminho para seu arquivo `values.yaml` e execute:
...
<Callout variant="tip" title="dica">
  O gráfico eAPM Helm oferece diversas opções de configuração. Revise o arquivo [`value.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-ebpf-agent/values.yaml) para adaptar a instalação às suas necessidades.
</Callout>

## Acesse a interface do usuário do APM com tecnologia eAPM [#eapm-access]
...
## Como o agente eAPM e o APM funcionam juntos [#eapm-apm]
