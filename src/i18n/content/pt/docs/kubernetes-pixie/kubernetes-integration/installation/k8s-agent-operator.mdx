...
Execute o seguinte comando para instalar o CR de instrumentação:
...
<CollapserGroup>
  ...
  <Collapser id="namespace-label-selector" title="NomeEspaçoRótuloSeletor">
    Define o pod auto-instrumentado no nível namespace. Permitido quando o CR e o operador estão no mesmo namespace. Caso contrário, este seletor deve ser deixado em branco.
...
<Callout variant="tip">
  Se o seu cluster usa um runtime de contêiner que requer um prefixo de registro explícito (como o CRI-O), adicione `docker.io/` ao início do caminho da imagem. Por exemplo: `image: docker.io/newrelic/newrelic-java-init:latest`.
</Callout>
...
* Obtenha insights de suas aplicações e resolva eventos de alerta usando a página [Resumo do APM](/docs/apm/agents/manage-apm-agents/agent-data/triage-run-diagnostics/).
