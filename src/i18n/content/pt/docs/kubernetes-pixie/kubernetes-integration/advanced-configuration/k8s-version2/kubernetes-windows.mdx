...
<Callout variant="important">
  **O monitoramento de nós do Windows agora está em disponibilidade geral (GA) na integração v3 do Kubernetes da New Relic.** A abordagem de instalação v2 descrita nesta página está descontinuada e será removida em uma versão futura. Use o [guia de instalação da integração do Kubernetes](/install/kubernetes) para instruções de instalação atuais, que oferecem suporte nativo a nós do Windows.
</Callout>

Use esta opção quando você tiver um sistema Kubernetes baseado em Windows. Observe que a integração do Windows tem várias [limitações](#k8-windows-limitations).
