...
### Descoberta de nós do plano de controle e componentes do plano de controle [#discover-nodes-components]

A integração do Kubernetes depende das convenções de rotulagem [`kubeadm`](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm/) para descobrir os nós do plano de controle e os componentes do plano de controle. Isso significa que os nós do plano de controle devem ser rotulados com `node-role.kubernetes.io/control-plane=""`.
...
<CollapserGroup>
  ...
  <Collapser id="logs-pod-kubestatemetrics" title="Obter log de um pod em execução em um nó do plano de controle">
    Para obter o log de um pod em execução em um nó do plano de controle:

    1. Obtenha os nós rotulados como plano de controle:
