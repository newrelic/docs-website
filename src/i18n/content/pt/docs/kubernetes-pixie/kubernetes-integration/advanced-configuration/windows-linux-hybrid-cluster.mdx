...
Este documento esclarece a instalação da integração Kubernetes para um cluster com nós Windows e Linux. O processo de instrumentação instala o gráfico `nri-bundle` nos nós do Linux e, em seguida, cria um subgráfico `newrelic-logging` . O subgráfico permite que a integração seja executada em nós Windows e Linux.

Observe que a nova [arquitetura`nri-kubernetes` v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) ainda não oferece suporte ao Windows. Instalamos o gráfico `newrelic-infrastructure` como uma versão independente, que permite monitor sua carga de trabalho do Windows.
