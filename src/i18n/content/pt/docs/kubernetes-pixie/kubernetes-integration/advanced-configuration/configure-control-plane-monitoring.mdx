...
<Callout variant="tip">
  Se você ainda não conseguir ver os dados do plano de controle, tente a solução descrita em [Resolução de problemas de integraçãoKubernetes : Não ver dados](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/kubernetes-integration-troubleshooting-not-seeing-data/).
</Callout>