...
* Observe que o KSM v2+ desativa os rótulos métricos por padrão. Você pode ativar o monitoramento da métrica dos rótulos de destino usando as `metric-labels-allowlist` opções descritas [aqui](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) no seu cluster do Kubernetes.
