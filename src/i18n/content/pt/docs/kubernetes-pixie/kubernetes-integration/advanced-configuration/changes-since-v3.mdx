...
* Observe que o KSM v2+ desativa os rótulos métricos por padrão. Você pode ativar o monitoramento da métrica dos rótulos de destino usando as `metricLabelsAllowlist` opções descritas [aqui](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/cli-arguments.md) no seu cluster do Kubernetes.
...
As seguintes opções de agente foram previamente "aliadas" na raiz do arquivo `values.yml` e são <DoNotTranslate>**no longer available**</DoNotTranslate>:
...
Depois de fazer isso e adaptar qualquer outra configuração que possa ter sido alterada de acordo com o [guia de migração acima](#migration-guide), você poderá atualizar seu `nri-bundle` executando o seguinte comando:
...
<Callout variant="important">
  A sinalização `--reuse-values` não é compatível com upgrade da v2 para a v3.
</Callout>