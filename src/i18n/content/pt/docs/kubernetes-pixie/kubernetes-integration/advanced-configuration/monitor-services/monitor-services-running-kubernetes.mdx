...
<CollapserGroup>
  <Collapser
    ...
    title="Obtenha o YAML de configuração para a integração"
  >
...
<Callout variant="tip">
  Este formato se aplica à [integração do Kubernetes v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3). Para a versão v2 mais antiga, verifique a seção [abaixo](#configure-values-v2).
</Callout>
...
#### (Legado v2) Adicione a configuração ao `values.yaml` [#configure-values-v2]

<CollapserGroup>
  <Collapser
    id="example-config-map"
    title="Exemplo de configuração para v2"
  >
    <Callout variant="tip">
      Este formato se aplica ao legado Kubernetes integração v2. Para versões mais recentes, verifique a seção [acima](#configure-values).
    </Callout>

    Este exemplo mostra a configuração YAML para a integração do Redis adicionada ao arquivo `values.yml` do gráfico `nri-bundle` .
...
Vá para <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**</DoNotTranslate>, selecione <DoNotTranslate>**Third party services**</DoNotTranslate> e selecione o dashboard do serviço. Você deverá ver os dados sendo relatados.
...
* A integração do Kubernetes não é atualizada automaticamente. Para obter melhores resultados, [atualize](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure#update) regularmente o .
...
A configuração da nossa integração Kubernetes segue o formato [ConfigMap](https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/) . Usar um `ConfigMap` nos permite dissociar a configuração da integração da imagem do Kubernetes . O outro benefício é que um `ConfigMap` pode ser atualizado automaticamente sem recarregar o contêiner em execução.
...
Para que o trecho acima seja reconhecido pela integração, ele deverá ser colocado dentro do ConfigMap designado. Se você estiver usando nossa [integração Kubernetes v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3), um ConfigMap já deverá ter sido gerado, com um nome terminando em `-integrations-cfg`. Localize o mapa de configuração e adicione o trecho modificado a ele, para que fique assim:
...
Se você estiver usando nossa [integração Kubernetes v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3), este ConfigMap já estará montado no contêiner necessário.
