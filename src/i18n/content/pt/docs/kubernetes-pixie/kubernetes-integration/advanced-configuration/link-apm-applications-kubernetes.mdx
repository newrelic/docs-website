...
<CollapserGroup>
  ...
  <Collapser id="no-metadata-in-transactions-attributes" title="Não há metadados Kubernetes na transação atributo">
    ...
    1. Verifique se as variáveis de ambiente foram injetadas corretamente nos novos pods seguindo as instruções descritas na seção [Validar a injeção de metadados](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection).

    2. Se eles não existirem, primeiramente, acompanhe os logs do serviço de webhook:

       ```shell
       kubectl logs -l app.kubernetes.io/name=nri-metadata-injection -f
       ```

    3. Em outro terminal, crie um novo pod e inspecione os logs do deployment de injeção de metadados em busca de erros. Consulte a seção [Validar a injeção de metadados](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) para criar um novo pod. Para cada pod criado, deve haver uma entrada de log, assumindo que o [nível de log (logLevel)](https://github.com/newrelic/k8s-metadata-injection/blob/main/charts/nri-metadata-injection/README.md#values) esteja em INFO ou inferior:
    ...
    7. Certifique-se de que os jobs de configuração de injeção de metadados foram executados com sucesso inspecionando a saída deste comando:
