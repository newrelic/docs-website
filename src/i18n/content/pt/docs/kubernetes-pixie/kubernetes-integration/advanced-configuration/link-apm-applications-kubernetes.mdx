...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Não há metadados Kubernetes na transação atributo"
  >
    ### Problema
    ...
    5. Inicie um proxy para o servidor API Kubernetes executando este comando em uma janela de terminal e mantenha-o em execução.

       ```shell
       kubectl proxy --port=8001
       ```

    6. Crie um novo pod em seu cluster, isso fará com que o servidor API tente se comunicar com o webhook. Este comando criará um busybox.

       ```shell
       kubectl create -f https://git.io/vPieo
       ```

    7. Recuperar o log do servidor API .

       ```shell
       curl localhost:8001/logs/kube-apiserver.log > apiserver.log
       ```

    8. Exclua o contêiner do busybox.

       ```shell
       kubectl delete -f https://git.io/vPieo
       ```

    9. Inspecione o log em busca de erros.
    ...
    10. Se não houver entradas log no log do servidor API ou nos metadados de inserção de implantação, significa que o webhook não foi registrado corretamente.

    11. Certifique-se de que o trabalho de configuração da injeção de metadados foi executado com êxito inspecionando a saída deste comando:

        ```shell
        kubectl get job newrelic-metadata-setup
        ```

    12. Se o trabalho não for concluído, investigue o log do trabalho de configuração:

        ```shell
        kubectl logs job/newrelic-metadata-setup
        ```

    13. Certifique-se de que `CertificateSigningRequest` seja aprovado e emitido executando este comando:

        ```shell
        kubectl get csr newrelic-metadata-injection-svc.default
        ```

    14. Certifique-se de que o segredo TLS esteja presente executando este comando:

        ```shell
        kubectl get secret newrelic-metadata-injection-secret
        ```

    15. Certifique-se de que o pacote CA esteja presente na configuração do webhook mutante:

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    16. Certifique-se de que o `TargetPort` do recurso <DNT>**Service**</DNT> corresponda ao <DNT>**Port**</DNT> do contêiner do <DNT>**Deployment**</DNT>:
