...
Para vincular seu aplicativo e Kubernetes, você deve ser capaz de implantar `MutatingWebhookConfiguration`s em seu cluster do Kubernetes.
...
A injeção de metadados está incluída quando você [instala nossa integração usando Helm](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm). Apenas certifique-se de que, ao configurar o gráfico `nri-bundle` , o webhook que injeta os metadados esteja ativado:
...
No entanto, se você tiver [o cert-manager](https://cert-manager.io/) instalado, poderá configurar nosso gráfico para usar o cert-manager, o que pode simplificar significativamente o processo de implantação:
...
* Se o seu par de certificado/chave estiver em outro formato, consulte a [base de conhecimento da Digicert](https://knowledge.digicert.com/solution/SO26449.html) para obter mais ajuda.
...
E execute novamente o [comando de instalação](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm#install-k8-helm).
...
<CollapserGroup>
  <Collapser
    ...
    title="Não há metadados Kubernetes no APM ou transação distributed tracing"
  >
    ### Problema
    ...
    2. O segredo será criado corretamente e o pod `k8s-metadata-injection` poderá ser iniciado.
    3. Observe que a nova versão do manifesto e do `nri-bundle` já está atualizada com a versão correta da imagem.
  </Collapser>

  <Collapser
    id="no-metadata-in-transactions-attributes"
    title="Não há metadados Kubernetes no atributo da transação"
  >
    ### Problema
    ...
    1. Verifique se as variáveis de ambiente estão sendo injetadas corretamente seguindo as instruções descritas na etapa [Validar sua instalação](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) .

    2. Se eles não estiverem presentes, obtenha o nome do pod de injeção de metadados executando:
    ...
    3. Em outro terminal, crie um novo pod (por exemplo, consulte [Validar sua instalação](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection)) e inspecione o log da implantação de metadados em busca de erros. Para cada pod criado deve haver um conjunto de 4 novas entradas no log como:
    ...
    4. Para verificar se o apiserver não está conseguindo se comunicar com o webhook, você deve inspecionar o log do apiserver em busca de erros como:
       ...
       5. Inspecione o log em busca de erros.
          ...
          <Callout variant="tip">
            Um dos [requisitos para a injeção de metadados](#network-req) é que o apiserver tenha permissão de saída para o pod em execução no cluster. Se você encontrar erros relacionados a tempos limite de conexão ou falhas de conexão, verifique os grupos de segurança e as regras de firewall do cluster.
          </Callout>

    5. Se não houver entradas log no log do apiserver ou nos metadados de inserção de implantação, significa que o webhook não foi registrado corretamente.

    6. Certifique-se de que o trabalho de configuração de injeção de metadados foi executado com êxito inspecionando a saída de:

       ```shell
       kubectl get job newrelic-metadata-setup
       ```

    7. Se o trabalho não for concluído, investigue o log do trabalho de configuração:

       ```shell
       kubectl logs job/newrelic-metadata-setup
       ```

    8. Certifique-se de que `CertificateSigningRequest` seja aprovado e emitido executando:

       ```shell
       kubectl get csr newrelic-metadata-injection-svc.default
       ```

    9. Certifique-se de que o segredo TLS esteja presente executando:

       ```shell
       kubectl get secret newrelic-metadata-injection-secret
       ```

    10. Certifique-se de que o pacote CA esteja presente na configuração do webhook mutante:

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    11. Certifique-se de que o `TargetPort` do recurso <DoNotTranslate>**Service**</DoNotTranslate> corresponda ao <DoNotTranslate>**Port**</DoNotTranslate> do contêiner do <DoNotTranslate>**Deployment**</DoNotTranslate>:
