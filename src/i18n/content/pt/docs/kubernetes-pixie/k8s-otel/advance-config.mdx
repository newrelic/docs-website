...
<CollapserGroup>
  <Collapser id="proxy-chart" title="Usando a seção `proxy` no gráfico do Helm">
    Você pode usar a seção `proxy` no seu arquivo [`values.yaml`](/docs/kubernetes-pixie/k8s-otel/install/#config-params) para a configuração do gráfico `nr-k8s-otel-collector`.
