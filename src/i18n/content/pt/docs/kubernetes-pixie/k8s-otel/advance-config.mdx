...
## Compatibilidade com Variantes &amp; Provedores do Kubernetes [#enable-provider]
...
Se o seu provedor for o GKE Autopilot, não altere o valor de `daemonset.containerSecurityContext.privileged` do padrão, pois contêineres privilegiados não são suportados por este provedor.

Se você usa o Amazon EKS, atualize a configuração do Terraform do seu cluster ou use o AWS Console para habilitar o processador `resourcedetection` por meio de um dos seguintes métodos:

* Aumente o limite de saltos do IMDS: defina o `http_put_response_hop_limit` como 2 para seus grupos de nós.
* Habilite a identidade de pod: adicione o complemento `eks-pod-identity-agent` ao seu módulo EKS.
...
Se o seu provedor for o OpenShift e você tiver o `receivers.filelog` habilitado (o padrão), faça as seguintes atualizações em `daemonset.containerSecurityContext` para garantir que o logging tenha os privilégios necessários para funcionar:
