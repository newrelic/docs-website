...
## Garantindo a segurança do acesso aos dados de log. [#securing-log-data]

Ao trabalhar com dados de log que podem conter informações confidenciais, é importante controlar quem pode acessar logs específicos. Aqui estão as práticas recomendadas para proteger seus dados de log:

* **Utilize o controle de acesso a dados**: Implemente políticas [de controle de acesso a dados](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control) para restringir quais usuários podem visualizar [partições de log](/docs/logs/ui-data/data-partitions/) específicas. Isso é especialmente importante quando os logs contêm informações de identificação pessoal (PII), dados de segurança ou informações comerciais confidenciais.

* **Esteja ciente dos riscos de exposição de dados**: certas funcionalidades podem contornar as restrições de acesso a dados. Analise [os potenciais riscos de exposição de dados](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control#data-exposure-risks) e siga estas práticas recomendadas:

  * Limitar quem tem acesso <DNT>**Modify**</DNT> à capacidade <DNT>**Data partitions rules**</DNT>, pois isso permite que o usuário reconfigure partições e potencialmente acesse dados restritos.
  * Restrinja o acesso <DNT>**Modify**</DNT> à capacidade <DNT>**Query keys**</DNT> apenas ao administrador confiável, pois a chave de consulta pode contornar as restrições de acesso a dados em nível de usuário.
  * Rotacione ou exclua as chaves de API legadas do Insights que não respeitam as políticas de acesso a dados.
  * Crie funções personalizadas sem recursos de gerenciamento de alertas para usuários que não deveriam acessar dados confidenciais por meio de condição de alerta ou notificação.

* **Alinhe as partições com o controle de acesso**: Ao planejar sua estratégia de particionamento, considere quais equipes devem ter acesso a quais logs. Agrupe os logs por equipe, serviço ou nível de sensibilidade para facilitar o gerenciamento do controle de acesso.
