...
Com nosso <InlinePopover type="apm"/>agente de linguagem Go, você pode obter o <DoNotTranslate>**logs in context**</DoNotTranslate>, que permite ver o registro do seu aplicativo no contexto de outros dados New Relic . Para informações gerais sobre este recurso, consulte [logs contextualizados APM](/docs/apm/new-relic-apm/getting-started/get-started-logs-context).
...
<CollapserGroup>
  ...
  >
    ...
    3. Configure e crie um novo formatador nrlogrus. Deve ser passado um aplicativo válido e qualquer formatador logrus válido. O formatador passado determinará a aparência do log que será gravado.

    ```go
    nrlogrusFormatter := nrlogrus.NewFormatter(newRelicApplication, &logrus.TextFormatter{})
    ```

    4. Defina o formatador do seu agente para o formatador nrlogrus recém-criado.
  ...
  >
    ...
    Ao registrar-se dentro de uma transação, você deve passar essa transação para o nrslog. Isso criará um novo objeto agente para aquela transação que vincula o log tratado pelo agente à transação e aos períodos em que ocorreu.
    ...
    Caso contrário, você pode simplesmente usar o ponteiro do objeto de transação assim:
