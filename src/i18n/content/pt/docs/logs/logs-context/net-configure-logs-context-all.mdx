...
Com nosso <InlinePopover type="apm"/>agente .NET, você pode obter o <DoNotTranslate>**logs in context**</DoNotTranslate>, que permite ver o registro do seu aplicativo no contexto de outros dados New Relic . Para informações gerais sobre este recurso, consulte [logs contextualizados APM](/docs/apm/new-relic-apm/getting-started/get-started-logs-context).
...
<CollapserGroup>
  ...
  >
    ...
    2. Habilite a decoração de log em sua configuração e reinicie o agente para começar a decorar seu log.
       ...
       <DoNotTranslate>**`PatternLayout`/`DynamicPatternLayout`**</DoNotTranslate>: atualize `conversionPattern` para incluir a propriedade `NR_LINKING` no final da linha.
       ...
       <DoNotTranslate>**`log4net.Ext.Json`**</DoNotTranslate>: atualize o layout para incluir um membro chamado `NR_LINKING` na propriedade JSON da mensagem.

       <DoNotTranslate>**`log4net.Appender.AdoNetAppender`**</DoNotTranslate>: Este appender tem uma configuração ligeiramente diferente de outros layouts. Você precisará atualizar o parâmetro `message` para incluir os metadados. O exemplo a seguir demonstra isso usando um `PatternLayout`.
       ...
       <DoNotTranslate>**Plain text using an `outputTemplate`**</DoNotTranslate>: atualize `outputTemplate` para incluir a propriedade `NR_LINKING` no final da linha.
...
Normalmente, seu log começará a aparecer menos de um minuto depois de você ativar o logs contextualizados APM. Verifique a seção <DoNotTranslate>**Triage > Logs**</DoNotTranslate> do seu aplicativo. Você também começará a ver [Padrões de log](/docs/logs/ui-data/find-unusual-logs-log-patterns) relacionados ao erro ali.
...
<CollapserGroup>
  <Collapser
    id="example-log4net"
    title="Diagrama de fluxo de trabalho log4net"
  >
    ...
    <DoNotTranslate>**Layout:**</DoNotTranslate> O `NewRelicLayout` formata o evento de log enriquecido no formato JSON esperado pela New Relic. O anexador, ao qual esse layout é atribuído, instrui o log4net a enviar o JSON para um arquivo no local esperado pelo direcionador de logs.

    <DoNotTranslate>**Log Forwarder**</DoNotTranslate>: O encaminhador de logs monitora uma pasta de saída e envia incrementalmente as informações adequadamente formatadas e enriquecidas log para o de New Relic registro endpoint.
  </Collapser>

  <Collapser
    id="config-log4net"
    title="Configuração log4net 2.0.8 ou superior"
  >
...
<CollapserGroup>
  <Collapser
    id="example-nlog"
    title="Diagrama de fluxo de trabalho do NLog"
  >
    ...
    <DoNotTranslate>**New Relic JSON Layout**</DoNotTranslate>: O `NewRelicJsonLayout` adiciona informações contextuais do agente .NET (usando a API) ao evento de log gerado pelo aplicativo e gera mensagem do log no formato JSON esperado pelo New Relic. Essas informações contextuais, conhecidas como linking metadados, são utilizadas pela New Relic para vincular mensagens do log à transação e aos spans onde foram criadas.

    Como `NewRelicAppender` é do tipo `ForwardingAppender` , ele precisa ser o primeiro anexador na cadeia. Também requer outro anexador que possa gravar em um destino de saída real como seu filho para funcionar.

    <DoNotTranslate>**File Target**</DoNotTranslate>: Um `FileTarget` define um arquivo no disco onde as mensagens do log são gravadas. Adicionar `NewRelicJsonLayout` a esse destino permite que a saída seja formatada corretamente para encaminhamento ao New Relic.

    <DoNotTranslate>**Log Forwarder**</DoNotTranslate>: o encaminhador de logs é configurado para enviar os log dados devidamente formatados e enriquecidos da `FileTarget` saída para o de geração de registros New Relic endpoint.
  ...
  <Collapser
    id="config-nlog"
    title="Configuração NLog 4.5 ou superior"
  >
    ...
    2. No código do aplicativo, atualize a configuração de geração de registros para adicionar `NewRelicJsonLayout` e decida se deseja coletar dados MappedDiagnosticsContext (<DoNotTranslate>**MDC**</DoNotTranslate>) ou MappedDiagnosticsLogicalContext (<DoNotTranslate>**MDLC**</DoNotTranslate>).
  ...
  <Collapser
    id="nlog-file"
    title="Configuração baseada em arquivo NLog"
  >
...
<CollapserGroup>
  <Collapser
    id="example-serilog"
    title="Diagrama de fluxo de trabalho Serilog"
  >
    ...
    <DoNotTranslate>**New Relic Enricher**</DoNotTranslate>: O `NewRelicEnricher` adiciona informações contextuais do agente .NET (usando a API) ao evento de log gerado pelo aplicativo. Essas informações contextuais, chamadas de metadados de vinculação, são utilizadas pela New Relic para vincular mensagens do log à transação e aos spans onde foram criadas.

    <DoNotTranslate>**New Relic Formatter**</DoNotTranslate>: o `NewRelicFormatter` traduz o evento de log enriquecido para o formato JSON esperado pela New Relic. Um coletor instrui o Serilog a enviar o JSON para um arquivo no local esperado pelo direcionador de logs.

    <DoNotTranslate>**New Relic Log Forwarder**</DoNotTranslate>: o encaminhador de logs é configurado para enviar os log dados devidamente formatados e enriquecidos da `FileTarget` saída para o de geração de registros New Relic endpoint.
  ...
  <Collapser
    id="config-serilog"
    title="Configuração do Serilog 2.5 ou superior"
  >
  ...
  <Collapser
    id="serilog-file"
    title="Configuração baseada em arquivo Serilog"
  >
