...
3. Adicione um trecho ao seu arquivo `vector.yaml` (localizado em `/etc/vector` por padrão), substituindo `YOUR_LICENSE_KEY` pela sua New Relic <InlinePopover type="licenseKey" />:
