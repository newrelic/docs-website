...
1. Navegue até [o repositório de plug-ins Fluent Bit da New Relic no GitHub](https://github.com/newrelic/newrelic-fluent-bit-output).
2. Na página do repositório, [clone ou baixe o repositório](https://help.github.com/en/articles/cloning-a-repository).
3. Execute o seguinte comando para criar seu plug-in:

```
cd newrelic-fluent-bit-output && make all
```

4. Armazene `out_newrelic.so` ou `out_newrelic_winXX.dll` em um local que possa ser acessado pelo daemon `fluent-bit` .
...
* Obtenha visibilidade mais profunda dos dados de desempenho do seu aplicativo e da sua plataforma encaminhando seu log com nossos recursos [de contexto de log](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/) .
