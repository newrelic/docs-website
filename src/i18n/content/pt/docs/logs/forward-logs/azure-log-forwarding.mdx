...
<Callout variant="tip">
  Deseja encaminhar os logs de VNet Flow do Azure? Consulte [Configurar o monitoramento de logs de VNet Flow do Azure](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) para o encaminhamento de logs de VNet Flow dedicado com processamento delta incremental. Você pode usar modelos de implantação ARM e Bicep para configuração automatizada.
</Callout>
...
11. Opcional: selecione <DNT>**Authentication Mode**</DNT>. O padrão é `Local Authentication`. Escolha `Managed Identity` se o seu ambiente do Azure tiver desabilitado a Autenticação Local (chaves de acesso compartilhado) para conformidade de segurança. Quando definido como `Managed Identity`, o Function App recebe uma identidade gerenciada atribuída pelo sistema e a função `Azure Event Hubs Data Receiver` no namespace do Hub de Eventos — nenhuma string de conexão é armazenada. Consulte [as informações do modo de autenticação](#eventhub-authentication) para obter mais detalhes.

12. Opcional: Configure o parâmetro de agrupamento do EventHub (disponível na versão 2.8.0 ou superior) para otimizar o desempenho:
...
13. Opcional: defina como `true` os [logs de atividades de assinatura do Azure](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) que você deseja encaminhar. Consulte [as informações de assinatura](#subscription-activity-logs) neste documento para obter mais detalhes.

14. Clique em <DNT>**Review + create**</DNT>, revise os dados inseridos e clique em <DNT>**Create**</DNT>.
...
### Opcional: configurar o modo de autenticação [#eventhub-authentication]

A partir da versão 3.2.0, o template ARM suporta dois métodos de autenticação para conectar o Function App ao Hub de Eventos:

* <DNT>**Local Authentication**</DNT> (padrão): o Function App se conecta usando uma cadeia de conexão de chave de acesso compartilhado armazenada nas configurações do aplicativo. Esta é a abordagem tradicional e corresponde às versões anteriores do template.

* <DNT>**Managed Identity**</DNT>: o Function App usa uma identidade gerenciada atribuída pelo sistema com a função interna `Azure Event Hubs Data Receiver` no namespace do Hub de Eventos. Nenhum segredo ou string de conexão é armazenado. Use esta opção quando:

  * Sua organização desativou a autenticação local (chaves de acesso compartilhado) nos namespaces do Hub de Eventos para conformidade de segurança.
  * Você prefere autenticação sem chave gerenciada totalmente pelo Azure AD.

Quando `Managed Identity` é selecionado, o template automaticamente:

* Cria uma identidade gerenciada atribuída pelo sistema no Function App.
* Concede à identidade a função `Azure Event Hubs Data Receiver` no namespace do Hub de Eventos.
* Configura o Function App com `EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace` (o nome do host do namespace do Hub de Eventos) em vez de uma string de conexão. O runtime do Azure Functions usa automaticamente tokens do Azure AD para autenticação.
