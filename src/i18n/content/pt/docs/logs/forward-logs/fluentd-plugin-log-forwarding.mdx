...
4. Se aplicável, [configure o endpoint da UE ou do Japão](#eu-configuration)
...
## Configure o plug-in do Fluentd para contas da UE ou do Japão [#eu-configuration]

Por padrão, o plug-in do Fluentd encaminha os logs para o nosso endpoint dos EUA: `https://log-api.newrelic.com/log/v1`. Se a sua organização da New Relic estiver em nosso [data center da UE ou do Japão](/docs/accounts/accounts-billing/account-setup/choose-your-data-center), você deverá definir manualmente a propriedade `base_uri` para o endpoint apropriado:

* UE: `https://log-api.eu.newrelic.com/log/v1`
* Japão: `https://log-api.jp.nr-data.net/log/v1`

Por exemplo, para encaminhar logs para o endpoint da UE:
...
Para contas do Japão, substitua o `base_uri` no bloco `<match>` pelo endpoint do Japão:
...
## Configurar a transformação UTF-16LE para UTF-8 [#configure-utf-16le]
...
Os arquivos de log em japonês codificados em Shift-JIS (CP932) são convertidos para a [codificação UTF-8 exigida](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#general) e são necessários para importar e enviar dados de log para o New Relic.
