...
4. Dependendo de onde os dados da sua conta estão armazenados na New Relic, use o nome DNS `log-api.newrelic.com` (EUA), `log-api.eu.newrelic.com` (UE) ou `log-api.jp.nr-data.net` (Japão) para configuração.
