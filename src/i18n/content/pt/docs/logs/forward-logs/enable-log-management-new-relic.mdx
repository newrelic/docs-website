...
4. Dependendo de onde os dados da conta estão armazenados no New Relic, utilize o nome DNS `log-api.newrelic.com` (EUA, padrão), `log-api.eu.newrelic.com` (UE) ou `log-api.jp.nr-data.net` (Japão) para configuração.
