...
Se seus log dados já estão sendo monitorados Heroku pelo roteador Logplex [integrado](https://devcenter.heroku.com/articles/logplex) log New Relicdo , você pode usar nossa integração para encaminhar e enriquecer seus dados no .
...
2. Vá para o [MercadoNew Relic ](https://one.newrelic.com/marketplace).
...
Para desativar os recursos de encaminhamento de logs, siga os procedimentos padrão na [documentaçãoHeroku Syslog](https://devcenter.heroku.com/articles/log-drains#syslog-drains). Você não precisa fazer mais nada no New Relic.