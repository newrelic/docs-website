...
* CentOS versão 7, 8 e 9 Stream (Rocky Linux e AlmaLinux também são suportados)
* RedHat versão 7, 8 e 9
* Debian versão 9 (Strech), 10 (Buster) e 11 (Bullseye).
* SUSE Linux Enterprise Server (SLES) versões 12 e 15 (ARM64 não suportado).
* Versões do Ubuntu 16.04.x, 18.04.x, 20.04.x, 22.04.x (versões LTS).
* Windows Server 2012, 2016, 2019 e 2022 e seus service packs.
* Windows 10, Windows 11.

## Encaminhar log automaticamente com instalação guiada [#infra]

Ao usar nossa instalação guiada para instalar o agente de infraestrutura, o recurso de encaminhamento de logs é configurado automaticamente durante o processo de instalação. Se ainda não o fez, crie sua conta New Relic gratuita abaixo para começar a monitorar seus dados hoje mesmo.
...
Os arquivos de configuração direcionam seu sistema para encaminhar as fontes de log que você deseja que apareçam no New Relic. Você pode adicionar quantos arquivos de configuração desejar. Nosso agente de infraestrutura usa arquivos `.yml` para configurar a geração de registros. Se você instalar o agente de infraestrutura por meio de [Incluir dados](https://one.newrelic.com/marketplace?state=78678a7f-91c5-ca40-ac55-e6b74a50085c) na interface, o arquivo `logging.yml` será criado automaticamente.
...
<CollapserGroup>
  ...
  >
    <Callout variant="important">
      Disponível desde o agente de infraestrutura v.1.24.3 Compatível apenas com Windows Server 2019 e posterior. Use [o winlog](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/#winlog) para versões anteriores.
    </Callout>
    ...
    * `use-ansi`: Use codificação ANSI em mensagens winevtlog. Usamos a codificação ANSI por padrão no Windows Server 2016 e versões anteriores, e UTF-8 nas versões mais recentes. Você pode substituir esse comportamento por este parâmetro de configuração se esses padrões não forem adequados ao seu caso de uso.
  ...
  >
    <Callout variant="important">
      O Winlog só pode coletar log de eventos clássicos. A tentativa de capturar outras pessoas coletará silenciosamente o log do aplicativo.
    </Callout>
    ...
    * `use-ansi`: Use codificação ANSI em mensagens winlog. Usamos a codificação ANSI por padrão no Windows Server 2016 e versões anteriores, e UTF-8 nas versões mais recentes. Você pode substituir esse comportamento por este parâmetro de configuração se esses padrões não forem adequados ao seu caso de uso.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="atributo inserido automaticamente pelo agente de infraestrutura"
  >
  ...
  <Collapser
    ...
    title="max_line_kb"
  >
    Tamanho máximo de entradas/linhas de log em KB. Se as entradas de log excederem o limite, elas serão ignoradas. O padrão é `128`.
  </Collapser>
...
* Na interface de infraestrutura, na tabela de host, clique no botão

  <Icon name="fe-more-horizontal"/>

  ícone para um host específico e clique em
...
<CollapserGroup>
  <Collapser
    ...
    title="Registro do Elasticsearch"
  >
...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="no-data"
    title="Nenhum dado aparece ao seguir um arquivo"
  >
  ...
  <Collapser
    className="freq-link"
    id="syslog"
    title="Nenhum dado aparece durante a captura por meio de um soquete Syslog"
  >
  ...
  <Collapser
    className="freq-link"
    id="proxy"
    title="Nenhum dado aparece usando o proxy do agente de infraestrutura"
  >
  ...
  <Collapser
    className="freq-link"
    id="agent-logs"
    title="Enviando o log do agente de infraestrutura para o New Relic"
  >
  ...
  <Collapser
    className="freq-link"
    id="fluentbit-logs"
    title="Habilitando modo detalhado no direcionador de logs (Fluent Bit)"
  >
  ...
  <Collapser
    className="freq-link"
    id="no-fb"
    title="Fluent Bit não inicia com o infra-agente"
  >
    <Callout variant="important">
  ...
  <Collapser
    ...
    title="Erro de tempo de execução no Windows"
  >
  ...
  <Collapser
    className="freq-link"
    id="too-many-files"
    title="Erros ao seguir uma grande quantidade de arquivo de log (Linux)"
  >
  ...
  <Collapser
    ...
    title="Instale uma versão específica do Fluent Bit (Linux)"
  >
    ...
    Para atualizar para uma versão específica do Fluent Bit, basta executar (nos exemplos a seguir, a versão `2.2.2` é usada):
  ...
  <Collapser
    ...
    title="Reverter para fluente-bit 1.x após o agente de infraestrutura 1.42.0 no Linux"
  >
...
Para desativar os recursos de encaminhamento de logs, acesse o diretório `logging.d` e remova os arquivos com a extensão `.yml` que foram adicionados originalmente durante o processo de [configuração](#configure) .
