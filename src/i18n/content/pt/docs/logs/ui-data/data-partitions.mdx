...
O namespace de uma partição determina o seu período de retenção. Oferecemos três opções de retenção:
...
* <DNT>**Direct to live archive:**</DNT> Ignora totalmente a retenção padrão. Os logs enviados para uma partição usando este namespace vão direto para o armazenamento de longo prazo do Live Archives em vez de primeiro chegarem ao armazenamento padrão. Este namespace funciona melhor para logs que raramente precisam ser consultados, mas devem ser retidos por um longo tempo, como logs de conformidade ou auditoria, já que não é necessário pagar pelo armazenamento de nível padrão para dados que não serão analisados no dia a dia. Esta opção requer que os Live Archives sejam habilitados na conta. Para obter mais informações, consulte [Armazenamento de Logs de Longo Prazo com Live Archives](/docs/logs/get-started/live-archives/).
