...
Log <DoNotTranslate>parsing</DoNotTranslate> é o processo de conversão de dados log não estruturados em atributo (pares de valor principal) com base nas regras que você define. Você pode usar esses atributos em sua consulta NRQL para facetar ou filtrar o registro de maneiras úteis.
...
6. Selecione um log correspondente, se existir, ou clique na guia
   ...
   para colar um log de amostra . Observe que se você copiar texto da interface de log ou do criador de consulta para colar na interface de análise, certifique-se de que seja a versão
