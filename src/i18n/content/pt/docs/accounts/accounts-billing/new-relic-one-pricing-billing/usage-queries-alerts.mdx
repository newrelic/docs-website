...
* Use consulta para criar

  <InlinePopover type="alerts"/>

  para saber se está se aproximando de um limite.
...
<CollapserGroup>
  ...
  <Collapser
    id="metrics-analysis"
    title="Ingestão métrica por fonte"
  >
  ...
  <Collapser
    id="month-gb"
    title="Uso de dados acumulado no mês"
  >
  ...
  <Collapser
    id="month-cost"
    title="Custo estimado de dados acumulado no mês"
  >
...
<CollapserGroup>
  <Collapser
    ...
    title="Usuário consulta para organização sem usuário principal"
  >
    ...
    * Os atributos usados para consultar por tipo de usuário são: `BasicUsersBillable` e `FullUsersBillable`.
    ...
    Esta consulta mostra uma contagem projetada mensal de usuários completos da plataforma. Esta consulta não seria adequada para uso em um dashboard; requer valores baseados em a) os dias restantes do mês, b) no início do mês. Aqui está um exemplo de consulta à contagem projetada para o final do mês com 10 dias restantes nesse mês:
    ...
    O [](/docs/accounts/accounts-billing/new-relic-one-pricing-users/pricing-billing#billing-usage-ui)mostra a contagem de usuário completo da plataforma e usuário básico. A consulta usada é:
...
<CollapserGroup>
  <Collapser
    id="TotalUsage"
    title="Consumo total de Sintético este mês"
  >
    ```sql
  ...
  <Collapser
    id="UsageByMonth"
    title="Consumo de Sintético por mês"
  >
    ```sql
    SELECT latest(freeConsumption) AS 'Included checks', latest(billableConsumption) AS 'Extra checks'
    FROM NrMTDConsumption
  ...
  <Collapser
    id="UsageByAccount"
    title="Consumo Sintético por conta"
  >
    ```sql
  ...
  <Collapser
    ...
    title="Consumo Sintético por tipo de monitor"
  >
    ```sql
    SELECT (sum(syntheticsFailedCheckCount) + sum(syntheticsSuccessCheckCount)) 
    AS 'Total Checks' 
  ...
  <Collapser
    ...
    title="Consumo Sintético por nome do monitor"
  >
    ```sql
    SELECT (sum(syntheticsFailedCheckCount) + sum(syntheticsSuccessCheckCount)) 
    AS 'Total Checks' 
  ...
  <Collapser
    id="UsageByLocation"
    title="Consumo Sintético por localidade"
  >
    ```sql
    SELECT (sum(syntheticsFailedCheckCount) + sum(syntheticsSuccessCheckCount)) 
    AS 'Total Checks' 
...
<CollapserGroup>
  <Collapser
    id="month-cost"
    title="Obtenha o custo estimado dos dados para o mês"
  >
    ...
    Aqui está um exemplo desta consulta usando um [custo por GB de US$ 0,30](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-prices):
  ...
  <Collapser
    id="user-cost"
    title="Obtenha o custo faturável do usuário para o mês"
  >
...
<CollapserGroup>
  <Collapser
    id="gb-exceeds-value"
    title="Os gigabytes ingeridos excedem o limite"
  >
  ...
  <Collapser
    ...
    title="O uso excede o limite de GB"
  >
  ...
  <Collapser
    id="user-threshold"
    title="O uso excede o limite de contagem de usuários"
  >
  ...
  <Collapser
    ...
    title="O uso excede o limite de custo estimado"
  >
...
* Existem vários tipos de dados que [não contam para a ingestão](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#usage-calculation) , mas são retornados pela função `bytecountestimate()` . Isso significa que esta função às vezes mostra

  <DoNotTranslate>**more**</DoNotTranslate>

  dados que realmente são contabilizados para seu uso. Raramente poderá apresentar um valor inferior, mas, se o fizer, será apenas ligeiramente inferior.
