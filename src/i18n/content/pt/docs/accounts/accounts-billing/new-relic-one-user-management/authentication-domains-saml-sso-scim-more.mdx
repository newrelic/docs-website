...
<CollapserGroup>
  ...
  <Collapser id="manage-user-type-scim" title="Gerencie o tipo de usuário com SCIM">
    ...
    * Azure Active Directory: Consulte a discussão no fórum da New Relic sobre a configuração do Azure AD para SCIM com tipos de usuário [aqui](https://support.newrelic.com/s/hubtopic/aAX8W0000008d6MWAQ/relic-solution-make-full-core-basic-users-in-azure-active-directory-w-scim).
...
* [Nome de usuário/senha](#username-authentication): seus usuários fazem login por e-mail e senha.
* [SSO SAML](#saml): seu usuário faz login por meio de logon único (SSO) SAML usando seu provedor de identidade. Para saber como configurar isso, continue lendo.

### Autenticação por nome de usuário/senha [#username-authentication]

Este é um método de autenticação padrão. Os usuários fazem login usando seu endereço de e-mail e senha. Para segurança adicional, você também pode habilitar a [autenticação multifator (MFA)](#mfa).
...
A autenticação multifator (MFA) adiciona uma camada de segurança aos seus dados ao exigir que os usuários em um domínio de autenticação verifiquem sua identidade.

Quando habilitado, os usuários recebem um e-mail de verificação sempre que fazem login. Durante o primeiro login com exigência de MFA, os usuários também recebem dez códigos de backup. Esses códigos são usados se eles não conseguirem acessar seu e-mail. Aconselhe seus usuários a armazenar esses códigos em um local seguro.

Para concluir a verificação da MFA para o aplicativo New Relic Mobile, insira seu e-mail no aplicativo e, em seguida, siga o prompt para abrir a tela de login no navegador do seu dispositivo móvel.

**Para ativar a MFA:**

1. Acesse [one.newrelic.com &gt; Administration &gt; Authentication Domains](https://onenr.io/08jqqPYdvjl)
2. Selecione o domínio necessário.
3. Ative a chave de MFA. Depois de habilitado, os usuários nesse domínio de autenticação recebem um prompt para verificar seu e-mail no próximo login e em todos os logins subsequentes.
