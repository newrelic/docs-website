...
Qualquer solicitação no restante deste tutorial receberá uma resposta <DoNotTranslate>**401 Unauthorized**</DoNotTranslate> se o token de acesso à API estiver ausente ou for inválido.
...
Você pode usar a API SCIM para enviar uma solicitação `POST` a `/scim/v2/Users` para criar um usuário. Os seguintes atributos de usuário são <DoNotTranslate>**required**</DoNotTranslate>:
...
Você pode usar a API SCIM para enviar uma solicitação `POST` a `/scim/v2/Groups` para criar um grupo. O único atributo de grupo <DoNotTranslate>**required**</DoNotTranslate> é:
