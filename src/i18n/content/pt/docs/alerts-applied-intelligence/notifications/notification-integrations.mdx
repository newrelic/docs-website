...
 
    ...
    8. Clique em <DoNotTranslate>**Save destination**</DoNotTranslate>. 9. Utilize este destino com seu fluxo de trabalho de notificação.

    <Callout variant="tip">
      Você pode atualizar o atributo Servicenow diretamente na tabela de destino (ex. **incidente**) diretamente do New Relic. Para fazer isso, tag a entidade relacionada (por exemplo condição do alerta, entidade APM , Sintético, hosts, etc…) com **ServiceNowFields.\[serviceNow_value]** ou **ServiceNowFields.dv\_\[serviceNow_name]**. Por exemplo, para preencher sua entidade APM como item de configuração na tabela de incidentes, tag a com **ServiceNowFields.dv_configuration_item : my_ci**
    </Callout>
