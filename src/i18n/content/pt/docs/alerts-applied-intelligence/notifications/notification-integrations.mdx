---
title: Alerta e inteligência aplicada notificação integração
tags:
...
<CollapserGroup>
  ...
  >
    ...
    * O reconhecimento do problema do New Relic é acionado quando o status do problema do Jira muda para `in-progress`.
    ...
    ### Configure o modelo de mensagem no fluxo de trabalho [#configure-jira-message-template]
  ...
  <Collapser
    ...
    title="FreshService"
  >
    Integre o New Relic ao FreshService para criar e resolver automaticamente tickets do FreshService.

    ### Configure o destino FreshService usando Webhook
    ...
    2. Insira as seguintes informações:
       ...
       * Nome do webhook: nome de referência do webhook do FreshService.

       * URL endpoint : [https://seudominio.freshservice.com/api/v2/tickets](https://yourdomain.freshservice.com/api/v2/tickets) (substitua seu domínio pelo nome do help desk do FreshService)

       * Habilite Autorização e selecione Autorização Básica. Insira as seguintes informações para nome de usuário e senha:

         * <DoNotTranslate>**Username:**</DoNotTranslate>

           [Chave de API FreshService](https://support.freshservice.com/en/support/solutions/articles/50000000306-where-do-i-find-my-api-key-)

         * <DoNotTranslate>**Password:**</DoNotTranslate>

           X (a senha é apenas X maiúsculo)

    3. Salvar destino.

    ### Crie um Workflow e personalize o atributo FreshService com variáveis de Workflow
    ...
    2. Insira um nome para o fluxo de trabalho.

    3. Em Filtrar dados, crie os critérios de filtro desejados para enviar a notificação a um destino.

    4. Em Notificar, selecione

       <DoNotTranslate>**Webhook**</DoNotTranslate>

       e personalize a mensagem de notificação com as seguintes informações:

       * No campo Destino, selecione o destino FreshService criado nas etapas acima.

       * Personalize a carga útil para enviar [variáveis de fluxo de trabalho](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/custom-variables-incident-workflows/) para [o parâmetro FreshService](https://api.freshservice.com/#create_ticket) relevante. (Veja Exemplo de carga útil)

         * Consulte [Sintaxe do guidão](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) para obter informações adicionais sobre funções auxiliares para variáveis de fluxo de trabalho.
         * Consulte [Campos personalizados do FreshService](https://support.freshservice.com/en/support/solutions/articles/154126-customizing-ticket-fields) para adicionar campos personalizados.
         * O tipo de variável de fluxo de trabalho deve corresponder ao tipo de parâmetro FreshService.

    5. Teste a notificação clicando em “Enviar notificação de teste”. Navegue até o ticket do FreshService para visualizar ticket recém-criado.
  ...
  >
    ...
    ### Configure um destino de push móvel [#set-mobile_push-destination]

    Para criar um destino de push móvel, você precisa do seguinte:
  ...
  >
    ...
    * No ServiceNow você pode enviar e ver como imagens os enriquecimentos da New Relic.
    ...
    9. [Baixe](https://store.servicenow.com/sn_appstore_store.do#!/store/application/d117597d1b9b9d9078faddf7b04bcba7/1.0.0?referer=%2Fstore%2Fsearch%3Flistingtype%3Dallintegrations%25253Bancillary_app%25253Bcertified_apps%25253Bcontent%25253Bindustry_solution%25253Boem%25253Butility%25253Btemplate%26q%3Dnew%2520relic&sl=sh) e instale o aplicativo New Relic na loja ServiceNow.
