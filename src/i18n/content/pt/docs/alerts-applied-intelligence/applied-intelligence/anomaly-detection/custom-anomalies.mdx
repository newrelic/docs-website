---
title: Detecção de anomalia personalizada
tags:
...
A anomalia personalizada permite à sua equipe maior versatilidade ao detectar comportamentos incomuns em seu sistema. Além de serem flexíveis e dinâmicos, as anomalias personalizadas fornecem à sua equipe a capacidade de alertar sobre qualquer entidade ou sinal e de ajustar e otimizar seu limite. As anomalias personalizadas são criadas usando as mesmas configurações de ajuste avançado dos alertas estáticos, para que você possa garantir que sua equipe veja apenas os incidentes que são importantes para você.

Você também pode enriquecer sua configuração personalizada de detecção de anomalias com metadados adicionais para fornecer mais contexto e adicionar descrições personalizadas de incidentes que podem fornecer instruções adicionais ao seu engenheiro de plantão.

## Configurar limite de anomalia personalizado [#configure-custom-anomalies]

Você pode criar um limite de anomalia personalizado a partir de uma [condição do alerta](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/). Aqui estão algumas dicas para definir o limite de anomalia:

* Defina a [direção da anomalia](#anomaly-direction) para monitor incidentes que ocorram acima ou abaixo da anomalia.

* Use a barra deslizante para ajustar a sensibilidade do limite

  <DoNotTranslate>**Critical**</DoNotTranslate>

  , representada no gráfico de visualização pela área cinza claro ao redor da anomalia. Quanto mais estreita for a faixa ao redor da anomalia, mais sensível ela será e mais incidentes irá gerar.
...
Siga estas etapas para criar sua anomalia personalizada:
...
2. Clique em <DoNotTranslate>**+ New alert condition > Use guided mode**</DoNotTranslate>.
...
5. Defina as configurações para um ou mais limites. a detecção de anomalias faz uma previsão sobre qual será o próximo ponto de dados com base na atividade anterior. O valor limite para detecção de anomalia personalizada é o número de Desvio padrão em que o valor do seu sinal está longe do valor previsto.
...
* <DoNotTranslate>**Age of data**</DoNotTranslate>
  ...
  Na criação inicial, a previsão é calculada usando de 1 a 4 semanas de dados, dependendo da disponibilidade dos dados e do tipo de previsão. Após a sua criação, o algoritmo leva em consideração as flutuações contínuas dos dados durante um longo período de tempo, embora seja dado maior peso aos dados mais recentes. Para dados que existem há pouco tempo, o valor previsto provavelmente flutuará bastante e não será muito preciso. Isso ocorre porque não há dados suficientes para determinar seus valores e comportamento habituais. Quanto mais histórico os dados tiverem, mais precisos se tornarão a previsão e o limite.

* <DoNotTranslate>**Consistency of data**</DoNotTranslate>

  Para valores métricos que permanecem num intervalo consistente ou que apresentam tendências lentas e constantes, o seu comportamento mais previsível significa que o seu limite se tornará mais apertado em torno da previsão. Dados mais variados e imprevisíveis terão limites mais flexíveis (mais amplos).
