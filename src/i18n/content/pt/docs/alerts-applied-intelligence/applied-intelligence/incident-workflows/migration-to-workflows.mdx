...
* `open_violations_count.warning` será sempre 0. Contagens específicas de prioridade não estão disponíveis.
...
* `closed_violations_count.warning` será sempre 0. Contagens específicas de prioridade não estão disponíveis.
...
* Para limitar o impacto na notificação VictorOps, OpsGenie e xMatters, o incidente será preenchido pelo incident_id. Isso fará com que as etapas do New Relic para reconhecer ou encerrar um incidente no xMatters não funcionem mais.
