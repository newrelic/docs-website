---
title: Gerenciando Scorecards
description: Aprenda a criar e gerenciar Scorecards no New Relic One.
tags:
  - Scorecards
  - New Relic One
  - service catalog
  - service architecture intelligence
freshnessValidatedDate: never
translationType: machine
---

Um Scorecard serve como um contêiner para várias regras que avaliam coletivamente a adesão a padrões definidos. Os scorecards usam regras para monitorar a conformidade entre diferentes entidades e equipes, identificar áreas para melhoria e garantir o alinhamento com os requisitos internos e externos. Este documento fornece instruções sobre como criar e gerenciar Scorecards e suas regras no New Relic.

## Criar um novo Scorecard [#create-scorecard]

Você pode criar um Scorecard usando um dos modelos disponíveis ou seus próprios Scorecards e adicionar suas regras para atender às suas necessidades. Para mais informações, consulte:

* [Crie um Scorecard usando o modelo](#create-scorecard-using-templates)
* [Crie um Scorecard personalizado](#create-custom-scorecard)

<Callout variant="tip" title="dica">
  Use mutações do NerdGraph para criar ou gerenciar seus Scorecards. Para mais informações, consulte [o tutorial Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>

**Pré-requisitos:**

* É necessário um ID de conta New Relic ativo para criar este Scorecard.
* Função adequada para criar scorecards. Consulte [Funções e permissões](#roles-permissions).

### Crie um Scorecard usando modelos [#create-scorecard-using-templates]

**Para criar um Scorecard usando [modelos](/docs/service-architecture-intelligence/scorecards/templates):**

1. Login em sua conta New Relic .

2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards &gt; Create Scorecard**.

3. Na tela **Create a Scorecard** , clique em **Use a template**.

4. Na tela **Choose a template**, selecione um modelo de Scorecard na lista e clique em **Next**.

5. Na tela **Manage the rules** :

   * Habilite as regras necessárias.
   * Na lista suspensa **Run this rule in** , selecione as contas para executar esta regra.
   * Clique em **Next**.

6. Na tela **Define maturity levels**, arraste e solte cada regra no bucket de maturidade apropriado (**Below basic**, **Basic maturity**, **Intermediate maturity** ou **Advanced maturity**) e, em seguida, clique em **Next**. Para obter mais informações, consulte [Maturidade do serviço](/docs/service-architecture-intelligence/scorecards/service-maturity).

7. Na tela **Review your Scorecard details** , se necessário, atualize o nome e a descrição do Scorecard.
   * Clique em **Confirm &amp; Create**.

8. Após a criação do Scorecard, você também pode adicionar regras adicionais, conforme necessário. Para obter mais informações, consulte [Criar regra de Scorecard](#create-scorecard-rules).

### Crie um Scorecard personalizado [#create-custom-scorecard]

**Para criar seu próprio Scorecard:**

1. Login em sua conta New Relic .
2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards &gt; Create Scorecard**.
3. Na tela **Create a Scorecard** , clique em **Build your own**.
4. Forneça um nome e uma descrição e clique em **Create**.
5. Após a criação do Scorecard, adicione e configure regras para definir seus critérios de conformidade específicos. Para obter mais informações, consulte [Criar regra de Scorecard](#create-scorecard-rules).

## Criar uma regra de Scorecard [#create-scorecard-rules]

As regras nos Scorecards são usadas para avaliar a entidade em relação a critérios de conformidade específicos. Você pode definir regras para automatizar o processo de validação se seus serviços, aplicativos ou outros recursos atendem aos requisitos específicos.

Você pode criar regras para Scorecards usando NRQL consulta. Cada regra pode abranger várias entidades e validar se cada entidade atende a condições específicas, avaliando como verdadeiro ou falso.

**Pré-requisitos:**

* Você precisa ter um ID de conta New Relic ativo para criar esta regra.
* Você criou um Scorecard. Para obter mais informações, consulte [Criar um novo Scorecard.](#create-scorecard)
* Usuários com a função <DNT>**Org Product Admin**</DNT> podem criar, atualizar e excluir Scorecards e regras. Se você precisar delegar essas permissões a outros usuários, você pode [criar uma função personalizada para Scorecards pela interface](/docs/accounts/accounts-billing/new-relic-one-user-management/account-user-mgmt-tutorial/#roles)

<Callout variant="tip" title="dica">
  Utilize mutações do NerdGraph para criar ou gerenciar as regras de Scorecards. Para obter mais informações, consulte o [tutorial de Scorecards do NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>

**Para criar uma regra:**

1. Login em sua conta New Relic .

2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards**.

3. Selecione o Scorecard necessário.

4. Clique em **+ Create Rule**.

5. Selecione as contas New Relic que contêm a entidade a ser avaliada e, opcionalmente, uma pequena lista de contas associadas, se a consulta exigir a associação da entidade com dados de outras contas.

6. Insira uma consulta NRQL que especifique suas condições. Sua consulta deve conter estes dois campos:

   * Um `FACET` por `entityGUID` por exemplo: `FACET id` como `entityGuid`

   * Uma coluna de pontuação para avaliar pontuações para `0` ou `1`. Por exemplo, `SELECT if(latest(reporting), 1, 0) as 'score'`.

     Exemplo NRQL:

     ```sql
     SELECT latest(if(tags.language = 'ruby', 0, 1)) as 'score'
     FROM Entity
     WHERE tags.environment = 'production' and type = 'APM-APPLICATION'
     SINCE 1 day ago
     FACET id, tags.nr.team as 'team'
     ```

7. Para validar sua consulta, clique em Executar esta consulta. Após a execução bem-sucedida, uma resposta de visualização será exibida.

   <Callout variant="tip" title="dica">
     Na lista suspensa Usar consulta existente, selecione a consulta criada anteriormente para ser executada na regra.
   </Callout>

8. Clique em **Next**.

9. Forneça um nome e uma descrição para a regra.

10. Clique em **Save and create**.

<Callout variant="tip">
  É possível atribuir uma regra a um [nível de maturidade](/docs/service-architecture-intelligence/scorecards/service-maturity) ao definir os níveis de maturidade durante a criação do Scorecard ou ao editar a regra posteriormente. Para definir o peso de uma regra ou criar níveis de maturidade personalizados, utilize o [tutorial do NerdGraph Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>

## Atualizar um Scorecard [#update-scorecard]

Você pode atualizar seu Scorecard existente, duplicá-lo ou excluir um Scorecard existente usando a opção mais.

**Para atualizar um Scorecard:**

1. Login em sua conta New Relic .
2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards**.
3. Selecione o Scorecard necessário. No canto superior direito, clique no ícone mais e selecione uma das seguintes opções:

<table>
  <thead>
    <tr>
      <th>
        Opções
      </th>

      <th>
        Descrição
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Editar nome e descrição
      </td>

      <td>
        Para atualizar o nome e a descrição existentes do Scorecard.
      </td>
    </tr>

    <tr>
      <td>
        Scorecard duplicado
      </td>

      <td>
        Para duplicar o Scorecard selecionado.
      </td>
    </tr>

    <tr>
      <td>
        Excluir Scorecard
      </td>

      <td>
        Para excluir o Scorecard selecionado.
      </td>
    </tr>
  </tbody>
</table>

## Atualizar regras [#update-rule]

Também é possível habilitar ou desabilitar regras existentes para um Scorecard. Ao editar uma regra, é possível alterar seu [nível de maturidade](/docs/service-architecture-intelligence/scorecards/service-maturity); os pesos das regras são definidos por meio da [API NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).

**Para atualizar as regras:**

1. Login em sua conta New Relic .
2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards**.
3. Selecione o Scorecard necessário.
4. No canto superior direito, clique em <DNT>more icon</DNT> e selecione **Activate and deactivate rules**.
5. Habilite ou desabilite as regras necessárias e clique em **Save**.

## Configurar notificações de regra [#rule-notifications]

É possível receber uma notificação quando a pontuação de uma regra cair abaixo de um limite definido. As notificações do Scorecards são criadas com base nos [alertas](/docs/alerts/overview/) do New Relic: ao criar uma, o Scorecards cria automaticamente uma [política de alertas](/docs/alerts/organize-alerts/create-edit-or-find-alert-policy/) e uma [condição](/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/) para a regra e anexa um [fluxo de trabalho](/docs/alerts/get-notified/alert-event-workflows/) que entrega a notificação ao destino escolhido.

**Para configurar uma notificação:**

1. Login em sua conta New Relic .

2. Vá para o **painel de navegação esquerdo &gt; All Capabilities &gt; Scorecards** e selecione um Scorecard.

3. Abra a regra sobre a qual deseja receber uma notificação.

4. Na página da regra, clique em **Set up a notification** e, em seguida, conclua o assistente:

   1. **Definir um limite**: uma notificação é enviada quando a pontuação da regra cai abaixo desse nível.
   2. **Selecione uma conta**: a conta onde a política de alertas e a condição estão armazenadas. O Scorecards cria a política e a condição automaticamente e nomeia a política com o nome da regra.
   3. **Adicione um fluxo de trabalho**: escolha o fluxo de trabalho que entrega a notificação ao [destino](/docs/alerts/get-notified/destinations/) selecionado.
   4. **Revisar e criar**: revise as configurações e crie a notificação.

<img title="Set up a notification on a Scorecard rule" alt="A Scorecard rule page showing the Set up a notification and Manage notifications buttons" src="/images/scorecards-rule-notification.webp" />

Para editar ou remover uma notificação posteriormente, abra a página da regra e clique em **Manage notifications**.

## Práticas recomendadas por Scorecards [#scorecards-best-practices]

* Revise regularmente os Scorecards para garantir o alinhamento com as metas estratégicas.
* Use dados históricos para identificar tendências e tomar decisões informadas.