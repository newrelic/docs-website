...
<Collapser
  id="broker-legacy"
  title="Obtenha a chave para corretores de serviços v1.12.12 e anteriores"
>
...
3. Clique no sinal
   ...
   <DoNotTranslate>**Add next**</DoNotTranslate>

   na descrição do produto carregado na visualização de navegação à esquerda do Ops Manager para adicionar esse produto à sua área de preparação.
...
1. Log login no Gerenciador de operações.

2. Clique em

   <DoNotTranslate>**Import a Product**</DoNotTranslate>

   e importe o bloco New Relic Service Broker para VMware Tanzu.
...
3. Clique no botão
   ...
   à direita de “New Relic Service Broker” que você carregou.
...
8. Crie um plano de serviço com sua chave de licença New Relic.
   ...
   * Log login em sua conta New Relic e navegue até a página Configurações da conta no menu suspenso no canto superior direito da página. Copie a chave de licença da sua conta New Relic e cole-a aqui.
...
16. Refaça o aplicativo. Você pode selecionar o link no topo da página Bind ou em uma janela de terminal e executar cf restage para fazer as alterações.
...
Use a variável de ambiente `JAVA\_OPTS` para especificar variáveis de ambiente específicas do agente New Relic no grupo de ambiente de preparação para que o buildpack Java possa usá-las e enviá-las para o local correto. Especificar JAVA_OPTS no grupo de variáveis de ambiente Runtime não produzirá nada, pois o buildpack não saberá disso.
...
Para obter mais informações, consulte [Precedência das definições de configuração](https://docs.newrelic.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#config-options-precedence).
