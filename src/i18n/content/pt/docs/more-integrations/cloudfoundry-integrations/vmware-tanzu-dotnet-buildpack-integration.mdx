...
Esta documentação descreve o Buildpack de extensão .NET da New Relic para VMware Tanzu e fornece instruções sobre como instalar o bloco. O bloco pode vincular o agente New Relic a aplicativos .NET Core ou .NET framework para que você possa monitorá-los em um ambiente VMware Tanzu.

O New Relic .NET Extension Buildpack para VMware Tanzu permite vincular seus aplicativos ..NET Core e framework ao agente New Relic .NET. Isso permite monitor a integridade e o desempenho desses aplicativos, analisar os dados capturados pelos agentes e, adicionalmente, correlacionar os dados capturados dos agentes com as métricas de infraestrutura do VMware Tanzu e os eventos coletados pelo [New Relic Firehose Nozzle](https://network.pivotal.io/products/nr-firehose-nozzle/).
...
O New Relic .NET Extension Buildpack para VMware Tanzu requer o seguinte:
...
A tabela a seguir fornece informações de versão e suporte de versão sobre o New Relic .NET Extension Buildpack para VMware Tanzu.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Pacote de extensão .NET New Relic 1.1.13
      </td>
    </tr>
...
O Buildpack de extensão .NET New Relic para VMware Tanzu pode ser instalado por meio do bloco no Ops Manager. Como alternativa, você pode extrair o arquivo `.Pivotal` e instalar buildpacks de extensão individuais usando o comando cf linha de comando Interface (CLI) `cf create-buildpack`.
...
O Buildpack de extensão .NET New Relic para VMware Tanzu instala um ou mais dos seguintes buildpacks, dependendo da configuração do bloco (total de 8 buildpacks de extensão):
...
* Novo Relic .NET Core Extension Cached Buildpack para aplicativo .NET Core em execução no Ubuntu Jammy 1.\* em implantação desconectada (isolada) do VMware Tanzu. Isto é para apoiar ambientes isolados onde não há acesso ao mundo exterior.
...
* 3 New Relic HWC Extension Cached Buildpacks para aplicativo .NET framework em execução na pilha do Windows 2019 em implantação desconectada (isolada) do VMware Tanzu. Isto é para apoiar ambientes isolados onde não há acesso ao mundo exterior.
...
Se você tiver uma solicitação de recurso, dúvidas ou informações sobre um bug, envie um problema no [github](https://github.com/newrelic/newrelic-dotnet-buildpack-tile/issues).