---
title: Acompanhe as alterações usando a CLI
tags:
  - APM
  - APM UI pages
  - Events
...
O recurso Monitoramento de Alterações é uma boa forma de monitor os efeitos de mudanças específicas em seus clientes e sistemas. Você faz isso designando quais alterações deseja monitor e, em seguida, visualizando os resultados na interface do New Relic.

Embora você possa designar quais alterações deseja monitor usando GraphQL ou uma integração CI/CD, você também pode usar nossa CLI para fazer a mesma coisa. Se você usar a CLI para designar as alterações que deseja monitor, poderá executar uma [consulta GraphQL](#list-with-graphql) ou uma [consulta NRQL](/docs/change-tracking/change-tracking-graphql#key-signals) no banco de dados New Relic para recuperar uma lista dessas alterações.
...
Temos uma nova versão da [CLI](/docs/new-relic-solutions/tutorials/new-relic-cli) que permite enviar mutações do GraphQL para criar marcadores. Mesmo se você estiver usando a CLI, precisará atualizá-la para aproveitar esse recurso.

Siga as etapas [aqui](https://github.com/newrelic/newrelic-cli#installation--upgrades) para instalar ou atualizar a CLI.

## Faça um marcador de mudança com a CLI [#make-marker-cli]

A CLI tem diversas opções (mostradas abaixo), mas o comando `create` marca uma alteração específica para uma determinada entidade New Relic.
