...
<Steps>
  ...
  <Step>
    ...
    3. Sua [região New Relic](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers#verifying-account). Sua região será `US` se o URL do New Relic for `one.newrelic.com`, `EU` se o seu URL estiver em `one.eu.newrelic.com` ou `JP` se o seu URL estiver em `one.jp.newrelic.com`.
