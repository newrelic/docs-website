...
<CollapserGroup>
  ...
  <Collapser id="prometheus-metrics-sources" title="Fontes métricas do Prometheus">
    ...
    **Parâmetro de configuração POMI:**
    ...
    ### POMI: estimando a cardinalidade

    Embora a cardinalidade não esteja diretamente associada à ingestão faturável por GB, o New Relic mantém determinados limites de taxa na cardinalidade. Poder visualizar a cardinalidade de um cluster Prometheus pode ser muito importante.

    <Callout variant="tip">
      As contas do New Relic têm um limite de cardinalidade de 1M, mas você pode solicitar até 15M de cardinalidade. Para solicitar alterações, entre em contato com seu representante de conta New Relic. Para mais informações, consulte [Limites da API de métrica](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes).
    </Callout>

    Se você já estiver executando o Prometheus Server, você pode executar estimativas de cardinalidade lá antes de ativar o POMI ou `remote_write`.
...
Responder às seguintes perguntas ajudará você a desenvolver confiança em sua capacidade de desenvolver e executar planos de otimização. Você pode querer usar o dashboard [Baseline de ingestão de dados](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-dashboard) e [Detalhamento de entidade de ingestão de dados](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-entity-breakdown-dashboard) da seção `Baselining`. Instale esse dashboard conforme descrito e veja quantas dessas perguntas você consegue responder.
