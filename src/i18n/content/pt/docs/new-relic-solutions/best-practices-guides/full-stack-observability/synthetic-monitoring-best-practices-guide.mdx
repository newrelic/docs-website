...
## 1. Escolha o seu monitor Sintético [#add-monitor]

Os monitores Sintético são navegadores virtuais que medem o desempenho do seu site e capturam números agregados de tempo de carregamento, tempo de operação e tamanho médio de download. Você também terá acesso a estatísticas detalhadas sobre o período de inatividade incidente e cada recurso da página.

Temos [sete tipos de monitor Sintético](/docs/synthetics/synthetic-monitoring/getting-started/types-synthetic-monitors) para você escolher, dependendo do que você deseja monitor e como gostaria de fazê-lo. Por exemplo, um [monitor de etapas](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors#stepmonitor) é uma ótima maneira de criar o que funciona essencialmente como um navegador com script, sem escrever código. Se você procura um gerenciamento automatizado do seu monitor Sintético, nossa [API NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-synthetics-tutorial) permite criar, atualizar e excluir seu monitor Sintético através de chamada de API.

### Adicione seu monitor Sintético [#howto-1]

1. Para adicionar um monitor, vá para
   ...
   . (Se você tiver uma [conta na UE](/docs/using-new-relic/welcome-new-relic/get-started/introduction-eu-region-data-center), vá para
...
2. Selecione um tipo de monitor e preencha todos os campos obrigatórios.

3. Você pode adicionar tag, alterar o período ou selecionar uma [versão de tempo de execução](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime) diferente. Para ping e monitor de navegador simples, você pode adicionar uma string de validação. Você pode usar as opções avançadas para ativar o monitoramento de substring para os seguintes tipos de validações de resposta:
...
4. Selecione os locais onde você deseja que seu monitor seja executado. Recomendamos escolher pelo menos três locais para evitar falso positivo. Em outras palavras, se pelo menos um local retornar um resultado bem-sucedido, o endpoint deverá estar ativo e o acionamento de um alerta poderá ser evitado.

5. Dependendo do tipo de monitor, você será solicitado a <DoNotTranslate>**Save monitor**</DoNotTranslate>, <DoNotTranslate>**Validate**</DoNotTranslate> ou <DoNotTranslate>**Write script**</DoNotTranslate>.

6. Veja seus resultados conforme eles são recebidos na [página de resumo](#summary-page).

## 2. Visualize a página de resumo do seu monitor Sintético [#summary-page]

Na página de resumo você verá informações sobre o status do seu monitor Sintético. Se algo criou um incidente ativo que acionou um alerta, clique no alerta crítico desse monitor para abrir em uma nova guia. Você também pode acessar todas as suas políticas de alertas para seu monitor clicando para gerenciar políticas de alertas para todos os monitores na parte superior da página.
...
## 3. Obtenha uma visão geral do desempenho do seu aplicativo [#synthetics-monitors]

Para garantir que os serviços da Web estejam funcionando conforme o esperado e livres de erros, você desejará acesso contínuo aos resultados do desempenho do seu aplicativo. O monitoramento sintético oferece esse tipo de garantia realizando testes automatizados em seu aplicativo web para cada local selecionado. Seu monitor Sintético registrará o período de inatividade (chamado de "violação") e coletará os números agregados, resultados e estatísticas detalhadas para cada recurso da página.

Para descobrir rapidamente o monitor com falha, você pode usar o [índice da página do monitor Sintético](/docs/new-relic-one-monitors-index) para ver qual monitor possui violação aberta, taxa de sucesso em 24 horas, número de locais com falha, período do monitor e tipo monitor . Ao clicar em um monitor, você será levado à [página Resumo](/docs/synthetics/new-relic-synthetics/pages/synthetics-overview-page-view-monitors-performance) , onde serão apresentadas informações para avaliar onde esse monitor específico está falhando, por que está falhando (última mensagem de erro, códigos de resposta de erro, duração por domínio) e mais, para fornecer insights mais profundos sobre o desempenho do seu aplicativo ao longo do tempo.
...
2. Em[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) >
...
Os resultados fornecem uma visão atualizada dos carregamentos de página mais lentos para cada local do monitor.

## 5. Compreenda o impacto no tempo de carregamento de cada recurso [#impact]

A página [de recursos do Sintético](/docs/synthetics/new-relic-synthetics/pages/synthetics-resources-understand-load-times) mostra como os diferentes componentes do seu site (por exemplo, CSS, JavaScript, imagens, HTML, etc.) estão afetando sua carga geral. Você pode detalhar métricas coletadas em tempo de execução, localizar informações de desempenho sobre o tempo gasto por recursos de terceiros e identificar códigos de resposta HTTP para cada recurso. Para fazer isso, você:
...
## 6. Configure e desenvolva um teste de navegador com script [#scripted-test]

Usando um navegador com script, você pode construir um fluxo de trabalho de monitoramento complexo com as ligações do Selenium JavaScript Webdriver. Por exemplo, você pode log no aplicativo, navegar até um link específico e aguardar o carregamento de um elemento da página e adicionar uma asserção. Para fazer isso, você:
...
3. Insira o nome e os detalhes do seu monitor (por exemplo, `sitename.com` navegador com script)

4. Selecione os locais onde deseja que seu monitor seja executado (por exemplo, Mumbai, Seul, Columbus e Montreal).

5. Escolha uma frequência para a frequência com que seu monitor funcionará (por exemplo, cinco minutos).

6. Defina um método de notificação para alertar sua equipe quando ocorrer um incidente de desempenho.

7. Agora você está pronto para escrever seu script. (Abaixo está um exemplo de script para testar o desempenho de newrelic.com e verificar se determinados elementos foram carregados.)
