...
<Steps>
  <Step>
    ...
    `nr1.json` é o [arquivo de metadados](/explore-docs/custom-viz/configuration-options/) da visualização da **Circular progress bar**. Use este arquivo para adicionar uma opção `colorScale` configurável, que corresponde ao campo `colorScale` em `VictoryPie`.
  </Step>

  <Step>
    Adicione [`collection`](/explore-docs/custom-viz/configuration-options/#collection) de [`string`](/explore-docs/custom-viz/configuration-options/#string) campos para você personalizar as cores do seu gráfico:
  ...
  <Step>
    ...
    Isso reverterá os cantos da barra para os 90 graus padrão, em vez de serem arredondados. Enquanto seu servidor local está em execução, ele reconhece automaticamente as alterações em _index.js_. Portanto, visualize sua visualização em seu navegador para ver a atualização.
