...
Nosso agente APM e agente de infraestrutura são projetados para relatar logs prontos para uso para algumas estruturas e configurações de log comuns. Chamamos isso de log de dados _de logs contextualizados_ porque você os vê não apenas em nossa interface de log principal, mas também junto com seus dados de aplicativo e host no APM e na interface de infraestrutura.
