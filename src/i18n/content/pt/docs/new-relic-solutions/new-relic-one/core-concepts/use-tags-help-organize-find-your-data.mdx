...
<figcaption>
  Aqui você pode ver uma tag `accountId` sendo usada para filtrar [cargas de trabalho](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)) para uma conta específica.
</figcaption>
...
* Organize e pesquise melhor <InlinePopover type="dashboards" />e [cargas de trabalho](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)).
...
<CollapserGroup>
  ...
  <Collapser id="simple-tags" title="Mantenha a tag simples">
    ...
    * Evite incluir vários valores como `regions: EMEA | US | LATAM` em uma única tag. Recomendamos usar três tags diferentes para isso, como `region: emea`, `region: us` e `region: latam`. Se quiser combinar várias tags, você pode fazer isso usando as opções avançadas na interface do filtro.
      * Uma ressalva para [eventos de alerta](/docs/alerts-applied-intelligence/overview/#concepts-terms): eles suportam apenas uma instância de nome de chave. Se você usar vários nomes de chave, um será selecionado aleatoriamente para ser adicionado a esse problema.
  </Collapser>
