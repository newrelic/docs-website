...
O workload padrão permite agrupar manualmente entidades relacionadas usando consultas, tags ou seleção explícita. Este documento explica como criar e configurar um workload padrão. Caso seja necessária a descoberta automática de entidades com base em distributed traces, consulte [Workloads Inteligentes](/docs/new-relic-solutions/new-relic-one/workloads/workloads-overview#intelligent-workloads) em vez disso.
...
Os workloads padrão oferecem controle total sobre quais entidades incluir. A associação é definida usando consultas de entidade, tags ou selecionando entidades específicas. Ao contrário dos [Workloads inteligentes](/docs/new-relic-solutions/new-relic-one/workloads/create-intelligent-workload), que descobrem automaticamente entidades a partir de traces, os workloads padrão são atualizados apenas quando as consultas correspondem a novas entidades ou são modificados manualmente.
