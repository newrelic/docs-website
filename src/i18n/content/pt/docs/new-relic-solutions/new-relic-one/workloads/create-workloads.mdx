---
title: Criar uma carga de trabalho padrão
tags:
  - New Relic
  - Use New Relic
  - Workloads
...
A carga de trabalho padrão permite agrupar manualmente entidades relacionadas usando consultas, tags ou seleção explícita. Este documento explica como criar e configurar uma carga de trabalho padrão. Se você precisar de descoberta automática de entidades com base em traces distribuídos, consulte [cargas de trabalho inteligentes](/docs/new-relic-solutions/new-relic-one/workloads/workloads-overview#intelligent-workloads) em vez disso.

<Callout variant="tip">
  Não tem certeza de qual tipo usar? Consulte [Visão geral de workloads](/docs/new-relic-solutions/new-relic-one/workloads/workloads-overview) para comparar workloads Standard e Intelligent.
</Callout>

As entidades que você adiciona a uma carga de trabalho Standard dependem de seus objetivos e da estrutura organizacional. Você também pode ler sobre como as cargas de trabalho alinham as equipes no nosso [tutorial de Cargas de trabalho](/docs/tutorial-peak-demand/organize-data-workloads).
...
* O <DNT>**workload account**</DNT> é onde reside a workload e seus dados. A conta usada para criar uma workload, então, é considerada a conta workload . Esta conta determina qual usuário pode ver e gerenciar a workload, com base nas permissões de conta que lhe foram atribuídas.
* As <DNT>**scope accounts**</DNT> são contas das quais a workload busca dados de telemetria. O usuário sem acesso a todas as contas de escopo de uma workload pode não conseguir ver todos os dados de uma workload. Para membros da equipe que usam uma workload, adicione as contas relevantes da equipe das quais workload extrai dados.
...
## Criar cargas de trabalho padrão [#create]

Cargas de trabalho padrão oferecem controle total sobre quais entidades incluir. Você define a associação usando consultas de entidade, tags ou selecionando entidades específicas. Ao contrário das [Cargas de trabalho inteligentes](/docs/new-relic-solutions/new-relic-one/workloads/create-intelligent-workload), que descobrem automaticamente entidades a partir de traces, as Cargas de trabalho padrão são atualizadas apenas quando suas consultas correspondem a novas entidades ou você as modifica manualmente.

Existem algumas maneiras diferentes de criar cargas de trabalho Standard, embora a que você escolher dependa inteiramente de você. Recomendamos escolher o método mais conveniente para alinhar suas equipes.

<CollapserGroup>
  <Collapser id="UI" title="Criar uma carga de trabalho Padrão na UI">
    ...
    <figcaption>
      <DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &gt; Workloads &gt; Create a workload:**</DNT> Ao [criar um workload Standard](#create), você escolhe as contas associadas e as entidades monitoradas usando consultas ou seleção manual.
    </figcaption>

    Veja como criar um workload Standard a partir da interface de workloads:
    ...
    5. Localize e escolha a entidade que compõe a workload. Quando tiver os resultados que procura, você pode adicionar entidade específica ou adicionar a consulta para atualizar dinamicamente a entidade na workload.
       ...
       * Clique em <DNT>**+ Add this query**</DNT> para criar uma lista de entidades atualizadas dinamicamente para sua workload. Recomendamos isso se você quiser que sua workload atualize sua entidade à medida que seu sistema muda.
       * Clique em <DNT>**+ Add**</DNT> ao lado de uma entidade para adicioná-la à sua workload. Esta é uma boa escolha se você sabe que a entidade permanecerá útil mesmo quando seu sistema mudar.
  ...
  <Collapser id="errors-inbox" title="Criar na Errors Inbox">
  ...
  <Collapser id="api" title="Criar via API">
...
<CollapserGroup>
  <Collapser id="tags" title="Use tag para definir o conteúdo da workload">
  ...
  <Collapser id="query-logic" title="Como funciona a lógica de consulta dinâmica">
  ...
  <Collapser id="add-dashboards" title="Adicionar painel à carga de trabalho">
