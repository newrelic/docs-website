...
O kit de desenvolvimento de software (SDK) New Relic One permite criar, servir, publicar e implantar aplicativos na plataforma New Relic. Ele também fornece uma série de componentes React para coletar dados, apresentar informações, lidar com a interação do usuário e muito mais. Você usa componentes como `Button` e `Dropdown` para criar uma experiência interativa que parece nativa do New Relic. Você usa os componentes `Table` e `Chart` para exibir dados de sua conta New Relic ou de outro lugar. Ao criar seu aplicativo de teste A/B, você usaria o componente `NrqlQuery` do SDK para buscar dados do Browser de sua conta.
