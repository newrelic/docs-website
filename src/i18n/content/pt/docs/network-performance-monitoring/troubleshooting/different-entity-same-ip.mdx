...
Se o `device_name` tiver sido alterado devido a uma alteração única, como a substituição de uma peça de hardware ou a atualização das convenções de nomenclatura, edite o `snmb-base.yaml` e exclua a entrada com o nome do dispositivo antigo. A entidade antiga ainda será exibida no menu <DoNotTranslate>**Explorer**</DoNotTranslate> , mas deixará de ter novos dados associados a ela e, eventualmente, desaparecerá do sistema. Na maioria dos casos, isso acontecerá 8 dias após a interrupção da chegada dos dados.
