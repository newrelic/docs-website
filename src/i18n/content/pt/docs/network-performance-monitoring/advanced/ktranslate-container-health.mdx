...
2. Em
   ...
   , insira `collector.name:"ktranslate"` e clique em
...
<CollapserGroup>
  <Collapser
    ...
    title="Qual versão do KTranslate estou executando?"
  >
  ...
  <Collapser
    id="docker-arguments"
    title="Quais argumentos foram passados para Docker em tempo de execução?"
  >
    Interface de registro:
  ...
  <Collapser
    id="container-errors"
    title="Que erros estou enfrentando?"
  >
  ...
  <Collapser
    id="match_attributes"
    title="Meu filtro match_attributes está funcionando no meu dispositivo?"
  >
    Interface de registro:
...
<CollapserGroup>
  <Collapser
    ...
    title="Quais são as versões atuais do meu aplicativo KTranslate?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="top-level-health"
    title="Qual é a integridade do meu aplicativo KTranslate?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="delivery-health"
    title="Qual é a saúde das minhas entregas na New Relic?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="snmp-health"
    title="Qual é a integridade geral da minha coleção SNMP?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    ...
    title="Quais dispositivos estão falhando na coleta de SNMP?"
  >
    ```sql
    SELECT
    max(snmp_fail)
  ...
  <Collapser
    id="netflow-health"
    title="Qual é a integridade da minha coleta de dados de fluxo?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="syslog-health"
    title="Qual é a integridade da minha coleção de syslog?"
  >
    ```sql
    FROM Metric
    SELECT
