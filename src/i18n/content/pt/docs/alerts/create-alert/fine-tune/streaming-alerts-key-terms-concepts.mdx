...
A plataforma de streaming <InlinePopover type="alerts" />verifica se há eventos de alerta com base em dados presentes ou ausentes no seu fluxo de dados, ou [sinal](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal), que chega ao New Relic.
...
À medida que os dados fluem para o New Relic, eles são filtrados pela condição NRQL. Antes que os dados sejam avaliados, eles devem atender aos critérios definidos pela [cláusula `WHERE` da consulta NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions#sel-where). Em vez de avaliar esses dados imediatamente em busca de eventos de alerta, as condições de alerta NRQL coletam os dados ao longo de um período de tempo conhecido como janela de agregação. Um atraso/temporizador adicional permite que pontos de dados mais lentos cheguem antes que a janela seja agregada.

Assim que o tempo de atraso/temporizador tiver decorrido, a New Relic agrega os dados em um único ponto de dados. O Alerts então compara o ponto de dados com os critérios de limite da condição para determinar se um evento de alerta deve ser aberto.

Mesmo que um ponto de dados atenda aos critérios para um evento de alerta, um evento de alerta pode não ser aberto. Um evento de alerta só é aberto quando os pontos de dados atendem consistentemente aos critérios de limite durante um período de tempo. Esta é a duração do limiar. Se os pontos de dados excederem o limite durante toda a duração do limite, enviaremos uma notificação com base nas configurações da sua política.
...
<table class="alternate">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Uma duração específica que determina se um evento de alerta é criado. Se a condição NRQL especificada atender aos critérios de limite durante a duração do limite, ocorrerá um evento de alerta.
...
A perda de sinal ocorre quando nenhum dado corresponde à condição NRQL durante um período de tempo específico. A perda de sinal é causada por diversos fatores. A cláusula `WHERE` em sua consulta NRQL pode filtrar dados antes de serem avaliados para eventos de alerta. Isso também pode significar que um serviço ou entidade está offline ou que uma tarefa periódica falhou na execução e nenhum dado está sendo enviado para a New Relic.

Para evitar notificações desnecessárias, você pode escolher quanto tempo esperar antes de ser notificado por um evento de alerta de perda de sinal. Você pode usar a detecção de perda de sinal para abrir eventos de alerta e ser notificado quando um sinal for perdido. Alternativamente, você pode usar a perda de sinal para fechar eventos de alerta para serviços efêmeros ou dados esporádicos, como contagens de erros.
...
<Callout variant="tip">
  ...
  Para saber mais sobre a perda de sinal e o preenchimento de lacunas, consulte [esta publicação do fórum de suporte](https://knowledge.newrelic.com/s/article/How-Can-I-Figure-Out-When-To-Use-Gap-Filling-and-Loss-of-Signal).
</Callout>