...
<CollapserGroup>
  ...
  <Collapser id="signal-loss" title="Perda de sinal (somente NRQL)">
    Uma perda de sinal é um período de tempo em que nenhum dado é recebido pelo New Relic. Isso pode ser resultado de uma entidade ou serviço que ficou offline, um problema com um agente ou coletor, ou problemas de rede em um data center ou na internet. Você pode usar a detecção de perda de sinal para criar um novo incidente quando um sinal para inesperadamente. Você também pode usar isso para determinar quando um serviço efêmero é interrompido e definir a ação para fechar qualquer incidente aberto que exista para essa condição. Outra opção na perda de configurações de sinal é pular a abertura de um incidente quando se espera que um sinal seja encerrado. Consulte a [documentação sobre perda de sinal](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#signal-loss) para obter mais informações.
