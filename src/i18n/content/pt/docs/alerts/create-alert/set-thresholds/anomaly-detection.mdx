...
6. Defina as configurações para um ou mais limites. a detecção de anomalias faz uma previsão sobre qual será o próximo ponto de dados com base na atividade anterior. O valor limite para detecção de anomalia controla a sensibilidade da condição do alerta para tolerar o quão distante o valor real está do valor previsto. O limite é o número de Desvio padrão em que o valor do seu sinal está distante do valor que foi previsto. Rastreamos o Desvio padrão entre o valor previsto e o valor real dos dados dos 7 dias anteriores.
   ...
   * Defina a &apos;direção do limite&apos; como superior, inferior ou ambos. Isso significa que criaremos um evento de alerta apenas se o valor do sinal (a saída da consulta) estiver acima do valor previsto, abaixo do valor previsto ou em ambos, respectivamente.
     ...
     Este campo determina quantos pontos de dados durante um período de tempo especificado devem estar fora do limite. As opções são <DNT>**for at least**</DNT> e <DNT>**at least once in**</DNT>. Selecionar <DNT>**for at least**</DNT> significa que TODOS os pontos de dados do seu sinal devem estar fora do limite durante o período de tempo especificado antes que um evento de alerta seja aberto. O inverso deve ser verdadeiro para fechar o evento de alerta. A opção <DNT>**at least once in**</DNT> significa simplesmente que, assim que qualquer um dos pontos de dados do seu sinal estiver fora do limite, um evento de alerta será aberto. Com esta opção, a duração não é relevante para determinar quando abrir um evento de alerta. No entanto, é relevante para fechar eventos de alerta. Todos os pontos de dados do seu sinal devem estar dentro do limite durante o período de tempo especificado
...
* <DNT>**New Relic calculation**</DNT> (padrão): determina automaticamente a sazonalidade de cada sinal com base em vários fatores, incluindo a idade dos dados, a consistência dos dados e flutuações regulares.
  ...
  **Exemplo:** Uma arquitetura de microsserviços dinâmica onde você monitora dezenas de serviços distintos em uma única condição. Serviços voltados ao usuário podem enfrentar picos diários de tráfego, enquanto serviços de processamento em segundo plano podem não ter nenhum padrão previsível. Essa configuração permite que o sistema se adapte a cada sinal específico sem ajuste manual.

* <DNT>**Hourly**</DNT>: Aplica um padrão horário a todos os sinais dentro da condição de detecção de anomalias.

  **Exemplo:** Um servidor que executa um cron job com uso intensivo de recursos no início de cada hora, como um script de sincronização de dados horário ou um processador de fila em lote. Esta configuração ajuda a detectar anomalias nos padrões de uso de CPU e memória.

* <DNT>**Daily**</DNT>: Aplica um padrão diário a todos os sinais dentro da condição de detecção de anomalias.

  **Exemplo:** Uma aplicação corporativa interna, como um portal de RH ou intranet, que registra consistentemente altas taxas de login a partir das 08:00 e diminui drasticamente após as 17:00 todos os dias.

* <DNT>**Weekly**</DNT>: Aplica um padrão semanal a todos os sinais dentro da condição de detecção de anomalias.

  **Exemplo:** Uma plataforma de software B2B que registra picos de tráfego de segunda a quinta-feira, uma queda notável às sextas-feiras e tráfego quase zero durante o fim de semana.
...
* <DNT>**None**</DNT>: Desativa completamente a sazonalidade, garantindo que nenhum padrão sazonal seja considerado ao avaliar sinais.
  ...
  **Exemplo:** Um monitor de taxa de erros críticos do sistema, como erros HTTP 500 ou timeouts de conexão com o banco de dados. Idealmente, essas métricas devem permanecer em uma linha de base plana próxima de zero em todos os momentos. Um pico às 14:00 é uma anomalia, independentemente de ter ocorrido ontem às 14:00 também.
