...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Uma verificação de local com falha será considerada com falha até que realize uma verificação bem-sucedida, **até um máximo de 24 horas**. O buffer de avaliação para condições de Multi-Location Synthetics tem um TTL (time-to-live) de 24 horas. Por exemplo: um local pode falhar e, em seguida, ficar imediatamente disponível, mas o consideraremos com falha até que uma verificação agendada seja relatada como bem-sucedida. Se nenhum sinal de recuperação for recebido dentro de 24 horas, a falha acabará sendo descartada do buffer.
      </td>
    </tr>
  </tbody>
</table>
...
Para usar a API REST de alertas para gerenciar condições de vários locais, use o [explorador da API REST](https://api.newrelic.com/docs/#/Deprecation%20Notice%20-%20Alerts%20Location%20Failure%20Conditions).