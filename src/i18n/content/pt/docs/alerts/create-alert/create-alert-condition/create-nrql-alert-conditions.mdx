...
<Callout variant="important">
  Caso nenhum evento (linhas) seja reportado, a perda de sinal `cannot` pode ser evitada mesmo com as alterações mencionadas acima. Recomendamos estabelecer ou manter um `Lost Signal Threshold` para acionar um incidente se o evento parar de ser relatado completamente.
</Callout>
