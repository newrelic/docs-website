...
Digamos que você tenha um site de comércio eletrônico e tenha criado uma condição de alerta para acionar um incidente sempre que seu site gerar erros de resposta 404. Você pode usar um modelo de título para tornar o incidente dessa condição facilmente identificável.

Você nomeou esta condição do alerta `high error rate - checkout`. Mas você deseja que o título do incidente reflita onde os erros estão sendo gerados. Neste caso, os erros deste incidente tiveram origem na sua entidade de checkout.
...
<CollapserGroup>
  <Collapser
    id="template-ui"
    title="Crie um modelo de título na interface"
  >
  ...
  <Collapser
    id="template-ui"
    title="Crie um modelo de título no GraphQL"
  >
...
* **Tag atributos**: Para tag, use o formato `{{tag.TAG_NAME}}`.

* **Tag de cláusula de faceta**: se uma condição NRQL usar uma cláusula `FACET`, você poderá usar a formatação `tag` para usar esses valores em seu modelo de título. Por exemplo, se a consulta NRQL incluísse `FACET hostName, cluster`, você poderia usar isto:
