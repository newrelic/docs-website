...
* Tag atributo: Para tags, você usaria o formato `{{tag.TAG_NAME}}`. Existem dois tipos de tags:
  ...
  * Tag de cláusula de faceta: <DNT>**For NRQL conditions only.**</DNT> se uma condição NRQL usar uma cláusula `FACET`, você poderá usar a formatação `tag` para usar esses valores em seu modelo de descrição. Por exemplo, se a consulta NRQL incluísse `FACET hostName, cluster`, você poderia usar isto:
