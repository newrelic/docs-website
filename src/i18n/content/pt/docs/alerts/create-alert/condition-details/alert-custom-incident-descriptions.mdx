...
O campo <DoNotTranslate>**Custom incident description**</DoNotTranslate> permite utilizar uma estrutura de modelo geral que, quando um incidente é gerado, substitui as seções genéricas por valores específicos. Por exemplo, digamos que você usou isso em uma descrição:
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Alert conditions (Policies) > (select a policy) > (select a condition)**</DoNotTranslate>: clique em <DoNotTranslate>**+ Add custom incident description**</DoNotTranslate> para abrir o campo.
</figcaption>
