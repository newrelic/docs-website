...
* O `id` da condição (disponível no [API Explorer:](https://api.newrelic.com/docs/#/Alerts%20Conditions) <DNT>**Alerts Conditions &gt; GET &gt; List**</DNT>)
* Por padrão, este endpoint usa o data center dos EUA. Se a sua organização hospeda dados no data center da UE, certifique-se de usar os [endpoints de API adequados para contas da região da UE](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center#endpoints).
