...
<CollapserGroup>
  ...
  >
    ...
    * O reconhecimento de um problema do New Relic é acionado quando o incidente do PagerDuty é reconhecido.
    ...
    6. Escolha um destino ou crie um novo.

    7. (Opcional) Edite o resumo do incidente padrão.
  ...
  >
    ...
    * Para buscar usuário para a coluna `caller` , precisamos de permissões de leitura para a tabela `sys_user` .
    ...
    3. Vá para <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Workflows**</DNT> e clique em um fluxo de trabalho existente ou no botão <DNT>**+ Add a new workflow**</DNT>

    4. Escolha um destino do ServiceNow.

    5. Após uma conexão bem-sucedida, as colunas da tabela de incidentes do ServiceNow são buscadas em sua conta e mapeadas automaticamente para sua instância do ServiceNow.
  ...
  >
    ...
    6. Vá para <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Workflows**</DNT> e clique em um fluxo de trabalho existente ou no botão <DNT>**+ Add a workflow**</DNT> .

    7. Selecione um destino, também conhecido como espaço de trabalho, e um canal do Slack para enviar mensagens. Você poderá criar um novo destino se não houver um destino predefinido para o espaço de trabalho necessário.

    8. Você também pode optar por receber atualizações de notificação, como quando um problema é confirmado ou encerrado, no canal. Isso também é conhecido como transmissão de thread.<img
