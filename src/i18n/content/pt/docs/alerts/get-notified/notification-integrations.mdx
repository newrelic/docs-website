...
<CollapserGroup>
  <Collapser className="freq-link" id="jira" title="Jira Atlassiana">
    ...
    * O reconhecimento do problema do New Relic é acionado quando o status do problema do Jira muda para `in-progress`.
    ...
    Clique em uma notificação de teste com valores de campo padrão para visualizar a issue do Jira. Se for bem-sucedido, você terá um link para visualizar o evento de alerta no Jira.
  ...
  <Collapser className="freq-link" id="email" title="E-mail">
    O destino de e-mail é criado automaticamente quando você escolhe <DNT>**Email**</DNT> como o canal de notificação para um [fluxo de trabalho](/docs/alerts/get-notified/alert-event-workflows/), portanto, você não precisa configurar isso no menu <DNT>**Destinations**</DNT>. Cada destino de e-mail é exclusivo do fluxo de trabalho associado a ele, e isso significa que ele pode aparecer como uma duplicata no feed de destinos.
  ...
  <Collapser className="freq-link" id="pagerduty" title="PagerDuty">
    Integre o New Relic com o PagerDuty para criar, atualizar, reconhecer e resolver automaticamente eventos de alerta do PagerDuty.
  ...
  <Collapser className="freq-link" id="slack" title="Slack">
    ...
    Seu workspace do Slack precisa ter o [aplicativo New Relic](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(ou o [App da UE](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) para clientes `one.eu.newrelic`, ou o [App do JP](https://newrelic.slack.com/marketplace/A0ATV7NGLLD-new-relic-jp) para clientes `one.jp.newrelic`) instalado. Um administrador do workspace deve aprovar o aplicativo antes que você possa instalá-lo individualmente.
