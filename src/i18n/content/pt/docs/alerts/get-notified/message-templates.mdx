...
No menu de variáveis, digite `{{` para selecionar em uma lista de variáveis. Conforme você digita, os nomes das variáveis aparecem via preenchimento automático. O tipo de variável está escrito no lado direito. Se o fluxo de trabalho tiver [enriquecimentos](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/#enrichments), eles aparecerão no topo da lista após digitar `{{`.
