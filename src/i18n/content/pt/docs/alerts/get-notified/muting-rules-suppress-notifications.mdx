...
## Como suprimir a notificação [#notify]

Se uma regra de silenciamento estiver ativa, por exemplo, manutenção planejada, você não receberá notificação quando um incidente for acionado.

Quando a regra de silenciamento não estiver mais ativa, você terá estas opções:

* <DNT>**Close ongoing incidents**</DNT>: Isso significa que se um incidente for acionado ou ainda estiver aberto logo após a regra de silenciamento expirar ou ser desativada, você receberá uma notificação. Recomendamos que você mantenha esta configuração padrão.

* <DNT>**Continue ongoing incidents**</DNT>: Isso significa que se um incidente iniciado durante uma janela de silenciamento continuar após a regra de silenciamento ficar inativa, você não será notificado.
