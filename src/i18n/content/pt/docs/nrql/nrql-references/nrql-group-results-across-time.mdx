...
Para as funções que possuem um parâmetro `format` opcional, os valores aceitos são `string` e `numeric`. O valor do formato padrão será `string` se omitido.
...
<img title="NRQL facet by time" alt="A screenshot displaying a NRQL query faceted by time" src={queriesFacetByTime} />
...
<img title="NRQL facet by time with two functions" alt="NRQL facet by time with two functions" src={queriesFacetByTimeTwoFunctions} />
...
<CollapserGroup>
  <Collapser id="facet-time-example" title="Resultados do grupo por mês">
  ...
  <Collapser id="facet-other-examples" title="Outros exemplos de agrupamento com cláusula FACET">
  ...
  <Collapser id="timeseries-chart-examples" title="Exemplo de criação de um gráfico especificando TIMESERIES">
