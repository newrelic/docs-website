...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> Considere uma matriz simples, `[100, 110, 90, 100, 105]`, armazenada em um atributo de duração.

    `getfield(durations, 2)` retornará `90`.

    Você pode usar a abreviação de colchetes `[ ]` em vez da função `getfield()`.

    <DoNotTranslate>**Example:**</DoNotTranslate> `durations[4]` retornará `105`.
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> Considere uma matriz simples, `countries`, com o valor `["US", "CA", "UK"]`.
