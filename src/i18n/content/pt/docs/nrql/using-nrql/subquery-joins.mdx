...
Este exemplo une dados de infraestrutura com dados de transação:
...
A imagem acima contém dois conjuntos de dados: a porcentagem média de CPU dos contêineres de infraestrutura (`ProcessSample`) e a duração média da transação do aplicativo por contêiner. Muitas vezes, os dados de diferentes fontes estão correlacionados. Nesse caso, você pode determinar se o maior uso de CPU de um contêiner está causando lentidão na transação usando a seguinte junção de subconsulta:
