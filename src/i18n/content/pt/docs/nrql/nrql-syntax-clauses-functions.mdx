...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="tip" title="Observação">
      **Como funciona a contagem exata com menos de 256 valores**: Ao contar menos de 256 valores de string únicos, `uniqueCount()` usa um método de contagem baseado em hash para determinar a unicidade. Este método gera um código hash inteiro de 32 bits a partir dos caracteres em cada string. Embora essa abordagem distribua de forma eficiente os valores de hash de aplicação para a maioria das strings, colisões de hash de aplicação podem ocorrer ocasionalmente — o que significa que duas strings diferentes podem gerar o mesmo código de hash de aplicação. Isso é mais provável de acontecer ao contar sequências de caracteres extremamente semelhantes entre si. Em casos raros em que ocorre uma colisão de hash ao aplicar, a função pode subestimar ligeiramente o número real de valores exclusivos.
    </Callout>
