...
<CollapserGroup>
  <Collapser
    ...
    title="Novo na New Relic?"
  >
    Se você é novo no New Relic e deseja ver como pode ser fácil usar o NRQL, você pode começar a usar a plataforma gratuitamente e seguir o tutorial. Tudo o que tem a fazer é:

    1. Se ainda não o fez, crie sua conta New Relic gratuita abaixo para começar a monitorar seus dados hoje mesmo.

    <InlineSignup/>

    2. Use nosso [agente e integração](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations/) para coletar automaticamente dados de estruturas e ferramentas comuns.
    3. Visite o [guia de lançamento rápido](/docs/new-relic-solutions/get-started/quick-launch-guide) para obter um caminho recomendado sobre como configurar New Relic. Você pode começar a ingerir dados para consulta e estar pronto para usar este ou qualquer um de nossos tutoriais.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="state-select"
    title={<>Obrigatório: declaração <InlineCode>SELECT</InlineCode></>}
  >
    ```sql
    SELECT attribute ...
    ```
    ...
    * Rotule os resultados retornados em cada argumento com [a cláusula `AS` ](#sel-as).
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Tempo médio de resposta desde a semana passada"
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-from"
    title={<>Obrigatório: cláusula <InlineCode>FROM</InlineCode></>}
  >
    ```sql
    SELECT ...
      FROM data type
    ...
    <CollapserGroup>
      <Collapser
        id="one-event"
        title="Consulte um tipo de dados"
      >
        Esta consulta retorna a contagem de todas as [transaçõesAPM ](/docs/insights/new-relic-insights/decorating-events/insights-attributes#transaction-defaults)nos últimos sete dias:
      ...
      <Collapser
        id="multiple-events"
        title="Consultar vários tipos de dados"
      >
      ...
      <Collapser
        id="from-lookups"
        title="Consultar dados de uma tabela de consulta"
      >
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        id="math-as"
        title={<>Consulta usando função matemática e <InlineCode>AS</InlineCode></>}
      >
      ...
      <Collapser
        id="funnel-as"
        title={<>Consulta usando funis e <InlineCode>AS</InlineCode></>}
      >
  ...
  >
    ...
    * Sem `TIMESERIES`, `COMPARE WITH` gera um outdoor com o valor atual e a variação percentual do valor `COMPARE WITH` .
    ...
    <DoNotTranslate>**Example**</DoNotTranslate>: esta consulta retorna dados como um gráfico de linhas mostrando o percentil 95 da semana anterior em comparação com o mesmo intervalo da semana anterior. Primeiro como um valor único e depois como um gráfico de linhas.
  ...
  >
    ...
    * Evento personalizado reportado via <InlinePopover type="apm"/>API do agente
      ...
      Quando `EXTRAPOLATE` é usado em uma consulta NRQL que suporta seu uso, a proporção entre <DoNotTranslate>**reported events**</DoNotTranslate> e <DoNotTranslate>**total events**</DoNotTranslate> é usada para extrapolar uma grande aproximação do total de dados sem amostragem. Quando é utilizado numa consulta NRQL que não suporta a sua utilização ou que não utilizou dados amostrados, não tem efeito.
    ...
    * `stddev`
      ...
      <CollapserGroup>
        <Collapser
          ...
          title="Exemplo de extrapolação de taxas de transferência"
        >
        ...
        <Collapser
          ...
          title="Exemplo de extrapolação de taxas de transferência como uma série temporal"
        >
  ...
  >
    ...
    <CollapserGroup>
      ...
      <Collapser
        id="cohort-analysis"
        title="Agrupando resultados ao longo do tempo"
      >
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title={<>Uso básico com <InlineCode>WHERE</InlineCode></>}
      >
        ```sql
      ...
      <Collapser
        ...
        title="Grupo baseado em atributos múltiplos"
      >
      ...
      <Collapser
        ...
        title={<>Rotular grupos com <InlineCode>AS</InlineCode></>}
      >
      ...
      <Collapser
        ...
        title={<>Facetar dados não correspondentes com <InlineCode>OR</InlineCode></>}
      >
  ...
  >
    ...
    <Callout variant="tip">
      Como as operações são executadas antes da aplicação da cláusula `LIMIT` , `FACET ... ORDER BY` não afeta o tipo dos resultados finais da consulta, o que será particularmente perceptível nos resultados de consultas que não sejam de série temporal.
    </Callout>
  ...
  >
    ...
    * A cardinalidade da junção é limitada a 1:100, o que significa que uma única chave de junção não pode mapear mais de cem linhas no resultado da subconsulta.
    ...
    Para uma análise aprofundada da cláusula JOIN, consulte o tutorial [de junções de subconsulta NRQL](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/subquery-joins) .

    <CollapserGroup>
      <Collapser
        title={<><InlineCode>INNER JOIN</InlineCode> com uma subconsulta facetada</>}
      ...
      >
        ...
        <figcaption>
          Exemplo <InlineCode>INNER JOIN</InlineCode> facetado
        </figcaption>
      </Collapser>

      <Collapser title={<><InlineCode>LEFT JOIN</InlineCode> com uma subconsulta facetada</>}>
        ...
        <figcaption>
          Exemplo <InlineCode>LEFT JOIN</InlineCode> facetado
        </figcaption>
      </Collapser>

      <Collapser title={<><InlineCode>INNER JOIN</InlineCode> com uma subconsulta não agregada</>}>
        ...
        <figcaption>
          Exemplo <InlineCode>INNER JOIN</InlineCode> não agregado
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser title={<>Consultar usando <InlineCode>LIMIT</InlineCode></>}>
  ...
  >
    A cláusula `ORDER BY` permite especificar como deseja ordenar os resultados da consulta em consultas que selecionam evento atributo _por linha_.
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        id="avg-resp-time-query"
        title="Tipos de dados no último dia"
      >
  ...
  >
    ...
    O valor <DoNotTranslate>**default**</DoNotTranslate> é <DoNotTranslate>**1 hour ago**</DoNotTranslate>.
    ...
    Ao usar NRQL, você pode definir um timestamp /hora UTC , uma hora relativa ou uma string DateTime. Consulte [Especificando um horário](/docs/query-your-data/nrql-new-relic-query-language/get-started/query-time-range/#spec-time).
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title={<>Use <InlineCode>SLIDE BY</InlineCode> com intervalo <InlineCode>MAX</InlineCode> ou <InlineCode>AUTO</InlineCode></>}
      >
  ...
  >
    ...
    * `TIMESERIES 30 seconds`
    ...
    <CollapserGroup>
      <Collapser
        id="set-interval"
        title="Use um intervalo definido"
      >
      ...
      <Collapser
        id="timeseries-auto"
        title="Use um intervalo definido automaticamente"
      >
      ...
      <Collapser
        id="timeseries-max"
        title={<>Usar intervalo <InlineCode>MAX</InlineCode></>}
      >
  ...
  >
    ...
    O valor <DoNotTranslate>**default**</DoNotTranslate> é <DoNotTranslate>**NOW**</DoNotTranslate>. Use apenas `UNTIL` para especificar um ponto final diferente do padrão.
    ...
    * Se você especificar mais de uma condição, separe as condições pelos operadores `AND` ou `OR`.
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            Exemplo: `animalType IN ('cat', 'dog', 'fish')`
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            O argumento de string para o operador `LIKE` aceita o sinal de porcentagem (%) como curinga em qualquer lugar da string.
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Exemplo de consulta com três condições"
      >
  ...
  <Collapser
    className="freq-link"
    id="with-as-nrql-var"
    title={<><InlineCode>WITH ... AS</InlineCode> cláusula (variáveis NRQL)</>}
  >
    ```sql
    FROM ...
    ...
    * A cláusula `WITH ... AS` pode vir antes, no meio ou diretamente depois da cláusula `FROM`/`SELECT`
    ...
    * Os nomes de variáveis não podem incluir o símbolo `%` .
    ...
    <CollapserGroup>
      <Collapser
        id="basic-nrql-var"
        title="Uso básico de uma variável"
      >
        ```sql
        FROM Transaction
      ...
      <Collapser
        ...
        title="Usando múltiplas variáveis"
      >
        ```sql
        FROM Log
      ...
      <Collapser
        ...
        title="Use variável em outras cláusulas"
      >
...
* [consulta métrica de fração de tempo data](/docs/query-data/nrql-new-relic-query-language/nrql-query-tutorials/query-metric-timeslice-data-nrql), que é nosso tipo de dados métrico original relatado por nosso APM, monitoramento de Mobile e
...
<CollapserGroup>
  ...
  >
    Use a função `apdex` para retornar uma [pontuação Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#score) para uma única transação ou para todas as suas transações. A pontuação Apdex padrão é 0,5 segundos. O [atributo](/docs/insights/new-relic-insights/decorating-events/insights-attributes) pode ser qualquer atributo baseado no tempo de resposta, como [`duration`](/docs/insights/insights-data-sources/default-events-attributes/apm-default-event-attributes#txn-duration) ou [`backendDuration`](/docs/insights/insights-data-sources/default-events-attributes/browser-default-events-attributes-insights#backend-duration). O argumento `t:` define um limite [de Apdex T](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) na mesma unidade de tempo do atributo escolhido. Por exemplo, se o atributo for medido em segundos, `t` será um limite em segundos.

    A pontuação Apdex retornada pela função `apdex()` é baseada apenas no tempo de execução. Não leva em conta erros de APM. Se uma transação incluir um erro, mas for concluída no [Apdex T](/docs/apm/new-relic-apm/getting-started/glossary#apdex_t) ou menos, essa transação será classificada como [satisfatória](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#bullet-satisfied) pela função `apdex ()` .

    <CollapserGroup>
      <Collapser
        id="apdex-cust-attributes"
        title="Obtenha Apdex para clientes específicos"
      >
      ...
      <Collapser
        ...
        title="Obtenha Apdex para transações específicas"
      >
        Use o atributo `name` para retornar uma pontuação para uma transação específica ou retorne um Apdex geral omitindo `name`. Esta consulta retorna uma pontuação Apdex para a transação <DoNotTranslate>**Controller/notes/index**</DoNotTranslate> na última hora:
  ...
  >
    Use a função `filter()` para limitar os resultados de uma das funções agregadoras na instrução SELECT. Você pode usar `filter()` em conjunto com `FACET` ou `TIMESERIES`. O filtro só é útil ao selecionar várias agregações diferentes, como:
  ...
  >
    Use a função `histogram()` para gerar o histograma. São necessários três argumentos:

    * Nome do atributo

    * Valor máximo do intervalo de amostragem

    * Número total de buckets (entre 1 e 500, inclusive)

      <CollapserGroup>
        <Collapser
          id="histogram-response-times"
          title="Histograma do tempo de resposta do evento PageView"
        >
          Esta consulta resulta em um histograma de tempo de resposta que varia de até 10 segundos em 20 baldes.
        ...
        <Collapser
          ...
          title="Intervalos de histograma do Prometheus"
        >
        ...
        <Collapser
          ...
          title="Métrica de distribuição New Relic"
        >
        ...
        <Collapser
          ...
          title="Histograma com cláusula FACET"
        >
  ...
  >
    Use a função `median()` para retornar a mediana ou percentil 50 de um atributo. Para mais informações sobre consulta de percentil, veja [percentile()](#func-percentile).

    <Callout variant="tip">
      A consulta `median()` só está disponível ao utilizar o [criador de consulta](/docs/chart-builder/use-chart-builder/get-started/introduction-chart-builder).
    </Callout>
  ...
  >
    ...
    O primeiro argumento requer uma [função agregadora](#functions) contra o atributo desejado. Use exatamente dois argumentos (os argumentos após os dois primeiros serão ignorados). Se o atributo não for numérico, esta função retornará um valor de 100%.
...
<CollapserGroup>
  ...
  >
    ...
    * `*`: captura de curinga, semelhante ao uso de captura regex
    ...
    <CollapserGroup>
      ...
      <Collapser
        title={<><InlineCode>aparse()</InlineCode> vários valores</>}
  ...
  >
    ...
    * Consulta contendo chamadas para `blob()` tem um valor LIMIT máximo de 20
    ...
    * `blob()` não pode ser usado em consulta facetada ou consulta de série temporal
      ...
      <CollapserGroup>
        <Collapser title={<>Usando <InlineCode>blob()</InlineCode> no registro estendido</>}>
          ```sql
  ...
  >
    ...
    <CollapserGroup>
      <Collapser title="Obtenha vários valores de duração do PageView como uma string formatada">
        Esta consulta retorna as durações backend e de rede do evento PageView, formatado com duas casas decimais e rótulos, como uma única string.
  ...
  >
    ...
    As unidades _diferenciam_ maiúsculas de minúsculas. Todas as unidades estão em minúsculas, a menos que sua especificação exija que estejam em maiúsculas. Por exemplo, as unidades de dados `'bits'` são válidas para bits e `'By'` devem ter `B` maiúsculo para bytes.
  ...
  >
    ...
    * Expressão regular com sintaxe de captura (expressões regex em NRQL usam sintaxe semelhante a Python, `r'...'`)
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de uma condição de cláusula <InlineCode>SELECT</InlineCode></>}>
        ...
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de uma condição de cláusula <InlineCode>FACET</InlineCode></>}>
        ...
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de uma condição de cláusula <InlineCode>WHERE</InlineCode></>}>
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> com uma conversão numérica</>}>
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> vários valores com variáveis NRQL</>}>
  ...
  >
    ...
    * 'base64url': Usa o [padrão RFC4648 base-64 com URL e nome de arquivo em alfabeto seguro](https://datatracker.ietf.org/doc/html/rfc4648#section-5)
      ...
      <CollapserGroup>
        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> em um atributo de string</>}>
          ```sql
          FROM Span SELECT entity.guid, decode(entity.guid, 'base64') WHERE entity.guid IS NOT NULL
          ```
        </Collapser>

        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> em um <InlineCode>FACET</InlineCode></>}>
          ```sql
          FROM Span SELECT count(*) WHERE entity.guid IS NOT NULL FACET entity.guid, decode(entity.guid, 'base64')
          ```
        </Collapser>

        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> em uma cláusula <InlineCode>WHERE</InlineCode></>}>
          ```sql
          FROM Span SELECT count(*) WHERE entity.guid IS NOT NULL AND decode(entity.guid, 'base64') NOT LIKE '%APM%'
          ```
        </Collapser>

        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> com um atributo de tipo de blob</>}>
          ```sql
          FROM Log
  ...
  >
    ...
    * Incluir: se presente, a lista de inclusão limita `dimensions()` a esses atributos.

    * Excluir: se presente, o cálculo `dimensions()` ignora esses atributos.
  ...
  >
    ...
    * 'base64url': Usa o [padrão RFC4648 base-64 com URL e nome de arquivo em alfabeto seguro](https://datatracker.ietf.org/doc/html/rfc4648#section-5)
      ...
      <CollapserGroup>
        <Collapser title={<>Usando <InlineCode>encode()</InlineCode> em um atributo</>}>
          ```sql
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        id="filter-eventtype"
        title={<>Use <InlineCode>eventType()</InlineCode> na função <InlineCode>filter()</InlineCode></>}
      >
      ...
      <Collapser
        id="facet-eventtype"
        title={<>Use <InlineCode>eventType()</InlineCode> com <InlineCode>FACET</InlineCode></>}
      >
  ...
  >
    ...
    * falseValue - este valor opcional é retornado se a expressão booleana for falsa, ou se não for fornecido NULL, será retornado
    ...
    <CollapserGroup>
      ...
      <Collapser title={<>Use com <InlineCode>AND</InlineCode> e <InlineCode>OR</InlineCode></>}>
        ```sql
        FROM Log
          SELECT count(*)
      ...
      <Collapser title={<>Aninhado <InlineCode>If()</InlineCode></>}>
  ...
  >
    ...
    * path - Um valor de string opcional usado para fazer referência direta a uma parte específica do JSON no parâmetro `attribute` . Consulte a seção [Referência de sintaxe do caminho de análise JSON](#json-parse-path-syntax-reference) abaixo.
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Referenciando uma chave/índice usando a sintaxe de colchetes"
      >
        <DoNotTranslate>
          **Referencing a key**
        </DoNotTranslate>
      ...
      <Collapser
        ...
        title="Mapeamento de chaves JSON para atributo"
      >
    ...
    <Collapser
      ...
      title="Referência de sintaxe do caminho de análise JSON"
    >
    ...
    <CollapserGroup>
      ...
      <Collapser
        ...
        title="Analisar um valor específico de uma mensagem do log"
      >
      ...
      <Collapser
        ...
        title="Analisar vários valores aninhados de uma string JSON"
      >
    ...
    _Funções relacionadas:_ [`mapKeys()`](#func-mapKeys), [`mapValues()`](#func-mapValues)
  </Collapser>
      ...
      * Se esse argumento for avaliado como nulo, a função `lower()` retornará nulo.
      ...
      <CollapserGroup>
        <Collapser title="Letras minúsculas em uma string">
          ...
          <figcaption>
            <InlineCode>lower(str)</InlineCode> exemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Função relacionada: [<InlineCode>upper()</InlineCode>](#func-upper)_
  </Collapser>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Extraia uma lista de chaves em uma string JSON"
      >
        ```sql
      ...
      <Collapser
        ...
        title="Extraia uma lista de chaves de uma string JSON com chaves aninhadas"
      >
        ```sql
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Extraia uma lista de valores dentro de uma string JSON"
      >
        ```sql
      ...
      <Collapser
        ...
        title="Extraia uma lista de valores de uma string JSON com valores aninhados"
      >
        ```sql
  ...
  >
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>mod()</InlineCode> dentro de uma condição de cláusula <InlineCode>WHERE</InlineCode></>}>
        ```sql
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>mod()</InlineCode> dentro de uma cláusula <InlineCode>FACET</InlineCode></>}>
        ```sql
  ...
  >
    ...
    * ocorrência - indica qual ocorrência do substr da qual retornar a posição.
      ...
      * Se for negativo, encontre a enésima ocorrência do substr no final de str. A última ocorrência de substr seria a ocorrência -1.
      ...
      Alias: <InlineCode>indexOf(str, substr , occurrence)</InlineCode> - `indexOf()` é um nome alternativo para a função `position()`
    ...
    <CollapserGroup>
      <Collapser title="Encontre o índice posicional de uma substring em um URL">
        Esta consulta demonstra o uso da função <InlineCode>position()</InlineCode> para localizar o índice posicional de várias substrings em uma string. O uso da função <InlineCode>position()</InlineCode> nos argumentos da função [<InlineCode>substring()</InlineCode>](#func-substring) também é demonstrado aqui.
        ...
        <figcaption>
          <InlineCode>position(str, substr , occurrence)</InlineCode> exemplo
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
  ...
  >
    ...
    * comprimento - o comprimento, ou número de caracteres, da substring a ser extraída de str.
      ...
      * _Opcional_ - se o comprimento não for fornecido, todos os caracteres em str após a posição inicial resolvida serão incluídos.
      ...
      <CollapserGroup>
        <Collapser title="Extraia várias partes de uma string">
          ...
          <figcaption>
            <InlineCode>substring(str, start , length)</InlineCode> exemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Consulte a função [<InlineCode>position()</InlineCode>](#func-position) para obter exemplos de uso de <InlineCode>substring()</InlineCode> e <InlineCode>position()</InlineCode> juntos._
  </Collapser>
  ...
  >
    ...
    * timestamp - Um timestamp de data/hora numérico a ser traduzido em uma string de data e hora. Pode ser um valor numérico ou um atributo e será convertido em `long` internamente.

    * padrão - Um padrão de data e hora opcional usado para formatar o resultado. Consulte a seção _Padrões para formatação e análise_ na [documentação do DatetimeFormatter](https://docs.oracle.com/javase/8/docs/api/java/time/format/DateTimeFormatter.html) para saber como construir uma string padrão.
    ...
    * timezone - Um valor opcional de fuso horário usado para interpretar a string de data e hora (ex. UTC).
    ...
    <CollapserGroup>
      ...
      <Collapser title="Traduzir um timestamp usando o parâmetro fuso horário">
        A consulta a seguir traduz o atributo `timestampValue` usando a string padrão `yyyy-MM-dd` com o fuso horário definido como 'América/Los_Angeles'. Isso retornará a string de data e hora `1969-12-31`.
      ...
      <Collapser title={<>Traduzir um timestamp usando o fuso horário na cláusula <InlineCode>WITH TIMEZONE</InlineCode></>}>
  ...
  >
    ...
    * padrão - Um padrão de data e hora opcional usado para analisar o parâmetro datestring. Consulte a seção _Padrões para formatação e análise_ na [documentação do DatetimeFormatter](https://docs.oracle.com/javase/8/docs/api/java/time/format/DateTimeFormatter.html) para saber como construir uma string padrão.
    ...
    * timezone - Um valor opcional de fuso horário usado para interpretar o parâmetro datestring (ex. PST).
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Interpretação parcial de data e hora ao analisar strings de data e hora"
      >
        <table>
          <thead>
            <tr>
    ...
    <CollapserGroup>
      ...
      <Collapser title="Analisar uma string de data e hora usando o parâmetro fuso horário">
        A consulta a seguir analisa a string de data e hora '2023-11-03 11:00:32' com o fuso horário definido como 'América/Los_Angeles'. Isso retorna o valor do timestamp `1699034432000`.
      ...
      <Collapser title={<>Analisar uma string de data e hora usando o fuso horário na cláusula <InlineCode>WITH TIMEZONE</InlineCode></>}>
    ...
    * Se esse argumento for avaliado como nulo, a função `upper()` retornará nulo.
      ...
      <CollapserGroup>
        <Collapser title="Uma string em maiúscula">
          ...
          <figcaption>
            <InlineCode>upper(str)</InlineCode> exemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Função relacionada: [<InlineCode>lower()</InlineCode>](#func-lower)_
  </Collapser>
</CollapserGroup>
...
* Use a função `numeric()` para converter um número com formato de string em um valor numérico. A função pode ser incorporada em uma consulta que usa funções matemáticas nos resultados da consulta ou funções agregadoras NRQL , como `average()`.
