...
<CollapserGroup>
  ...
  >
    ...
    * Não oferece suporte [a dados de métrica de fração de tempo](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) durante a Visualização Pública.
    ...
    Para saber mais sobre o comportamento padrão do modelo de previsão, consulte [Previsões NRQL](/docs/query-your-data/explore-query-data/use-charts/nrql-predictions/#defaults). Você pode personalizar as previsões definindo manualmente os hiperparâmetros do modelo de treinamento e adicionando as palavras-chave `USING` e `BY` .
