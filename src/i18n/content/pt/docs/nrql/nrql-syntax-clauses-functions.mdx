...
<CollapserGroup>
  ...
  >
    ...
    Com a cláusula `PREDICT` , você pode incorporar previsões sobre tendências de dados futuras em gráficos de linhas com base em dados históricos. Esta cláusula:
    ...
    * Define o intervalo de previsão para 20% da janela de consulta por padrão.
    * Processa dados históricos da janela de consulta atual e das duas anteriores para gerar previsões por padrão.
    * Não oferece suporte [a dados de métrica de fração de tempo](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) durante a Visualização Pública.
    ...
    <Callout variant="important">
      A cláusula `PREDICT` não é suportada quando usada com a cláusula [`COMPARE WITH`](/docs/nrql/nrql-syntax-clauses-functions/#sel-compare) ou [agregações aninhadas](/docs/nrql/using-nrql/nested-aggregation-make-ordered-computations-single-query) dentro [de subconsultas](/docs/nrql/using-nrql/subqueries-in-nrql/).
    </Callout>

    Você também pode acrescentar `PREDICT <model>`, `BY <time range>` e `USING <time range>` para refinar a tendência de previsão de acordo com suas necessidades específicas. Para indicar o intervalo de tempo e o período de dados históricos, use `integer units`.
...
* Consulte os tutoriais New Relic University para [consulta de filtro](http://newrelic.wistia.com/medias/8k4xbxlfwj), [consulta de Apdex](http://newrelic.wistia.com/medias/46dvx1mkm0) e [consulta de percentual](http://newrelic.wistia.com/medias/j3htrz66l0). Ou acesse o curso online completo [Writing NRQL consulta](https://learn.newrelic.com/writing-nrql-queries).
...
<CollapserGroup>
  ...
  >
    Utilize a função `uniqueCount()` para retornar o número de valor exclusivo registrado para um atributo no intervalo de tempo especificado.
...
<CollapserGroup>
  ...
  >
    Use a função `getField()` para extrair um campo de tipos de dados compostos, como [dados métricos dimensionais](/docs/data-apis/understand-data/metric-data/metric-data-type).
