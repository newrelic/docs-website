...
No [primeiro tutorial de NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-tutorial/), exploramos os fundamentos da criação de consultas. Abordamos a estrutura básica da consulta, a definição de janelas de tempo e como selecionar atributos específicos para observar. Também aprendemos como começar a agregar e exibir dados usando gráficos e outras visualizações.
...
* Pesquise para incluir/excluir usando curingas com `LIKE` ou `NOT LIKE`, ou limite os resultados àqueles `IN` uma lista ou `NOT IN` essa lista
* consulta em janelas de tempo mais avançadas usando datas, época e `WITH TIMEZONE`
* Agrupe dados em janelas de tempo usando coorte baseada em tempo
...
<Steps>
  <Step>
    Após concluir o [primeiro tutorial de NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-tutorial/), você usou `count()`, `average()`, `sum()`, `max()` e `min()` para transformar seus dados de maneiras significativas. Mas e se você quiser encontrar e contar valores exclusivos? Para encontrar o número de valores exclusivos registrados para um atributo em um intervalo de tempo especificado, você pode usar a função `uniqueCount()`. Você só precisa fornecer o atributo que queremos inspecionar como um argumento. Por exemplo, aqui está uma consulta para exibir todas as chamadas de API públicas exclusivas:
    ...
    Para especificar um valor limite diferente do padrão de 1.000, use `uniques(attribute[,limit])`, que aceita um máximo de 10.000.
  </Step>
  ...
  <Step>
    Pode ser necessário usar porcentagens em vez de contagens, somas ou médias. O uso da função `percentage()` permite calcular a porcentagem de um valor no conjunto de dados que corresponde a uma condição especificada. Essa função aceita dois argumentos: o primeiro é uma função de agregador para o atributo desejado, como `count()`. O segundo é uma condição `WHERE` para especificar o subconjunto de dados a ser consultado.
...
Como você pode ver, a agregação oferece controle flexível sobre seus dados. Você pode usar `uniqueCount()` para contar as entradas exclusivas de um atributo específico. Mas também é possível usar isso para identificar uma contagem de máquinas exclusivas, contêineres de relatórios ou até mesmo quantos pontos de dados personalizados são enviados para a New Relic. E, se quiser saber quais valores exclusivos estão disponíveis para consulta, sempre é possível solicitar uma lista usando `uniques()`.

As funções `latest()` e `earliest()` têm um valor particular ao lidar com dados esparsos ou ao investigar quando algo começou ou parou de relatar (presumindo que o New Relic ainda armazene os dados). A função `percentage()` pode mostrar qual porcentagem de eventos correspondeu a um qualificador em comparação com o conjunto de resultados geral. Por fim, você pode usar `percentile()` como um principal indicador de desempenho definindo uma meta para garantir que 90% de todas as transações do usuário final excedam a duração fornecida.
...
<Steps>
  <Step>
    O NRQL oferece suporte a operadores matemáticos básicos. Você pode realizar adição (`+`), subtração (`-`), multiplicação (`*`) e divisão (`/`) tanto em valores de atributo numéricos quanto em resultados de funções de agregador.

    Por exemplo, capturamos tanto o tempo de resposta total (como `duration`) quanto o tempo de resposta do banco de dados (como `databaseDuration`) quando registramos um evento de transação. Mas e se você quiser encontrar o tempo médio gasto fora do processamento do banco de dados? Você pode começar calculando esse valor para cada evento em seu conjunto de dados usando uma consulta como esta.
  ...
  <Step>
    Ótimo! Você acabou de realizar algumas operações matemáticas básicas. Isso é útil para ajudar você a listar eventos individuais. Mas e se você quiser saber a duração média das transações sem o tempo do banco de dados? Você também pode fazer a aritmética dentro da função:
...
O New Relic relata tempos como parte de seus eventos, para que você possa usá-los para calcular pontos de dados ou até mesmo gerar resultados percentuais. Você pode estender isso ainda mais aplicando esses operadores matemáticos a ações ou eventos personalizados, ou a dados personalizados que você envia para o New Relic. Por exemplo, uma plataforma de comércio eletrônico que relata dados sobre tamanhos de pedidos e métodos de pagamento pode usar matemática para calcular coisas como a taxa de conversão de pedidos em relação a visitas exclusivas de clientes.
...
<Steps>
  <Step>
    À medida que você começa a executar funções NRQL mais complexas, pode se perguntar se é possível tornar os nomes exibidos para os atributos de consulta mais úteis, especialmente para outras pessoas que não conhecem o NRQL. Vamos considerar um exemplo usando o que você aprendeu no tutorial anterior.
    ...
    Você pode usar a cláusula `AS` após uma função ou atributo para dar ao resultado um nome mais legível e significativo. Isso ajuda você e sua equipe a entender exatamente o que um gráfico representa.
  ...
  Isso pode parecer meramente estético, mas ao criar dashboards detalhados, é importante rotular claramente seus dados. Isso torna a fonte de dados clara para qualquer pessoa que visualize seus widgets, billboards, gráficos de linha ou tabelas.
...
<Steps>
  ...
  As comparações podem responder rapidamente a perguntas sobre o que está acontecendo em seus aplicativos. Os diferentes valores de vendas, desempenho, MTTR ou erros estão maiores ou menores em comparação com a semana passada? E, se você estiver investigando um problema, achará útil comparar um período de desempenho problemático com um período de desempenho normal.
</Steps>
...
<Steps>
  <Step>
    ...
    Em nossa consulta de amostra, você pode obter o número de transações com o termo &quot;amazonaws&quot; em qualquer lugar (início, meio ou fim) do nome.
  ...
  <Step>
    Você pode usar `%` como um curinga no início e no fim, o que significa que a New Relic verifica se o valor do atributo que você escolheu contém o termo, como “Web” em qualquer lugar do texto. Da mesma forma, você pode usar `%Web` OU `Web%` para corresponder a algo que termina com “Web” ou começa com “Web”, respectivamente.
...
Agora você pode controlar seus dados e manipulá-los para fazer o que precisa, permitindo construir dashboards e alertas. A seguir, você aprenderá como especificar intervalos de tempo usando NRQL.

## Especifique intervalos de tempo

<Steps>
  <Step>
    As cláusulas `SINCE` e `UNTIL` fazem mais do que fornecer informações sobre intervalos de tempo relativos: você também pode fornecer a elas uma data ou hora específica. Na seguinte consulta de exemplo, você pode usar uma data `SINCE` no formato `YYYY-MM-DD`.
    ...
    Você pode usar isso para criar relatórios de SLA para um período de tempo especificado. Você pode até incluir uma hora específica com o formato `YYYY-MM-DD HH:MM`. Nesta consulta, você pode ver que os dados estão definidos para as 18h.
  ...
  <Step>
    Às vezes, você pode receber um tempo de evento no tempo epoch (unix). Você também pode usar timestamps epoch com as cláusulas `SINCE` e `UNTIL` para não precisar traduzir manualmente esses valores para outro formato de data.
...
Antes desta lição, todos os seus mecanismos de controle de tempo dependiam de tempos relativos a partir de agora. Agora, você pode ajustar a visualização dependendo da localização de alguém no mundo. Talvez um cliente na Costa Leste relate um problema para sua equipe de engenharia localizada na Costa Oeste. Sua equipe pode criar um dashboard e traduzir a visualização para mapear para o fuso horário que um cliente citaria. Portanto, se um cliente relatar um problema às 9h na Costa Leste, você pode garantir que, ao olhar para as 9h, não precisará traduzir mentalmente a diferença.

Quando você precisa focar em datas específicas de um evento de alerta e deseja investigar os dados sem uma janela de tempo móvel relativa ao horário atual, isso ajudará você a obter dados em uma janela de tempo estática. Na próxima e última lição, você continuará aprendendo a usar o tempo em seus relatórios com coortes baseadas no tempo.

## Use coorte baseada em tempo
...
<Steps>
  <Step>
    ...
    Quando você usa a cláusula `SINCE` para durações, você recupera toda a extensão de tempo da consulta. Mas esses dados podem nem sempre contar toda a história! E se você precisar analisar o desempenho dentro de um período de tempo mais de perto? Com coortes baseadas no tempo, você pode classificar ainda mais os dados em agrupamentos lógicos baseados no tempo.
  ...
  <Step>
    Depois de executar a consulta acima, você pode ver o tempo de resposta mais lento com base na hora do dia. A New Relic oferece muitas opções diferentes para facetar com base no tempo. O exemplo anterior agrupa por hora, mas você também pode agrupar por dia da semana para determinar quais dias têm o melhor e o pior tempo de resposta.
...
A coorte baseada em tempo expõe problemas que ocorrem em minutos, horas, dias ou semanas específicos. Não importa quais dados você envie para o New Relic, a NRQL permite que você divida, detalhe, organize e visualize seus dados em qualquer formato que precisar.

Há também muitas outras opções disponíveis para agrupar, incluindo semana, mês e ano, dependendo da sua retenção de dados. Para ver a lista completa, acesse nossa [página de documentação Group Results Across Time](/docs/query-data/nrql-new-relic-query-language/nrql-query-examples/group-results-across-time).

## Próximos passos

Com o conhecimento adquirido aqui, você pode criar visualizações de dashboard e controlar os aspectos dos seus dados nos quais tem mais interesse. Você pode usar essas técnicas para restringir o foco para alertas mais granulares e específicos e produzir visualizações mais detalhadas. Quando estiver pronto, você pode avançar para o nosso [terceiro tutorial](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-tutorial-advanced-dashboarding/), onde aprenderá habilidades adicionais de NRQL, incluindo filtros, casos de faceta, histograma, apdex, filtragem para eventTypes, substituição de valores e extrapolação.