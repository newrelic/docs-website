...
<Steps>
  <Step>
    O Desvio padrão mede a quantidade de variação ou dispersão dentro de um conjunto de valores. Utiliza uma escala de baixo (valores próximos da média) a alto (valores distantes da média). A função `stddev()`permite observar as entrelinhas das médias e compreender os valores relatados em um nível mais profundo. Neste exemplo, você compara o Desvio padrão do tempo de resposta da transação ("duração") do último dia com o dia anterior.
