...
<Steps>
  ...
  <Step>
    ...
    O token do aplicativo é usado pelo New Relic para autenticar os dados do seu aplicativo .NET MAUI.

    Para visualizar e copiar o token do seu aplicativo na interface do New Relic:
  ...
  <Step>
    ...
    O plug-in móvel do .NET MAUI permite rastrear eventos de navegação no [.NET MAUI Shell](https://learn.microsoft.com/en-us/dotnet/maui/fundamentals/shell/navigation). Existem duas maneiras de inicializar isso, dependendo do tempo de inicialização do seu aplicativo em `App.xaml.cs`.

    <DNT>**Option 1: In the App Constructor**</DNT> Você pode chamar o método de rastreamento diretamente no construtor imediatamente após definir o `MainPage`.
    ...
    <DNT>**Option 2: In the OnStart Method (Recommended for timing/NullReferenceException issues)**</DNT> Se o shell do seu aplicativo não estiver totalmente inicializado durante a execução do construtor, chamar o rastreador de navegação pode resultar em um `System.NullReferenceException`. Para evitar isso, mova a chamada para o método de ciclo de vida `OnStart()` do MAUI:
...
### Dados de rastreamento distribuídos ausentes na interface [#missing-distributed-tracing]
...
### Falhas no aplicativo com TrackShellNavigatedEvents [#track-shell-crash]

Se você encontrar um `System.NullReferenceException` ao chamar `CrossNewRelic.Current.TrackShellNavigatedEvents()` no construtor `App()`, é provável que seja porque o shell do aplicativo ainda não concluiu totalmente a inicialização. Para resolver isso, mova a chamada para fora do construtor e para o método de ciclo de vida `OnStart()` do MAUI dentro do seu arquivo `App.xaml.cs` (consulte a seção [Eventos de Rastreamento de Tela](#screen-tracking-events) para obter exemplos de código).