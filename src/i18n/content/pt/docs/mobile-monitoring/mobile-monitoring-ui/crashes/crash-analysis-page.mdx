...
Mobile app crashes can result in users submitting negative reviews, affecting your organization's reputation. Crashes can also result in users deleting your app altogether, which affects your organization's profitability. With the Crash analysis page, you can identify and deal with one of the most significant bottlenecks in the development lifecycle: resolving unanticipated exceptions.
...
6. View the user journey to see the most common paths leading up to the crash, determine whether there's one or multiple ways to cause the crash, and get a list of steps to reproduce the crash locally.
...
<CollapserGroup>
  ...
  <Collapser
    id="user-journeys"
    title="User journeys: See the most common paths leading up to the crash"
  >
    User journeys offer a comprehensive view of user interactions leading up to a crash. By analyzing these combined journeys, you can identify recurring patterns and pinpoint root causes. This not only accelerates crash resolution but also leads to a smoother user experience overall.

    User journeys can help you:

    * **Pinpoint the problem users**: Drill down into specific user sessions that led to crashes and identify any shared attributes, like device type or network conditions, that might be contributing factors.
    * **Localize the fix**: Extract the steps and environment variables needed to reproduce the crash locally, allowing for efficient troubleshooting and resolution.
    * **Multiple culprits or a single sequence?**: User journeys help you determine if crashes follow a common thread or occur randomly.
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > Errors > Crash analysis > View journeys**</DoNotTranslate>: See how users encountered the crash.
    </figcaption>
  </Collapser>
