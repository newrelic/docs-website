...
1. Vá para
   ...
   . Em seguida, encontre seu aplicativo e clique em

   <DoNotTranslate>**Crashes > Crash Analysis**</DoNotTranslate>
...
3. Na [página](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report#viewing)

   <DoNotTranslate>[**Crash Details**](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report#viewing)</DoNotTranslate>

   [ ](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report#viewing)do relatório de falha selecionado, procure por `App Image Uuid` na lista de atributos.
...
2. Procure o UUID de compilação na [seção](#binary-example)

   <DoNotTranslate>[**Binary images**](#binary-example)</DoNotTranslate>

   [ ](#binary-example)do relatório de falhas exportado.

Use o Build UUID extraído dos detalhes da falha para encontrar o dSYM ausente.

## Exemplo de imagens binárias [#binary-example]

Aqui está um exemplo da seção <DoNotTranslate>**Binary images**</DoNotTranslate> de um relatório de falha exportado. Esta seção lista o Build UUID para cada biblioteca vinculada dinamicamente incluída no aplicativo, bem como o Build UUID do aplicativo principal.
