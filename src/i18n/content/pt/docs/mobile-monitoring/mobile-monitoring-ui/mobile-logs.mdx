...
* **Requisito do Agente iOS:** A coleta automática está **desativada por padrão**. Para ativá-lo:

  1. Você deve ativar o sinalizador de recurso `NRFeatureFlag_AutoCollectLogs` no código de inicialização do seu aplicativo.
  2. A chave <DNT>**Mobile Logs**</DNT> está **On** na interface do New Relic.
...
* **Coleta automática:** O agente iOS pode capturar automaticamente logs enviados para a saída padrão (`stdout`) e erro padrão (`stderr`). Você deve habilitar a feature flag `NRFeatureFlag_AutoCollectLogs` no código de inicialização do seu aplicativo para a coleta automática de logs.
