...
Após instalar o agente .NET MAUI, aguarde pelo menos 5 minutos. Se <DNT>**No HTTP data appears:**</DNT> estiver nas páginas de interface de erros HTTP e solicitações HTTP, certifique-se de ter usado `HttpMessageHandler` em `HttpClient`.
