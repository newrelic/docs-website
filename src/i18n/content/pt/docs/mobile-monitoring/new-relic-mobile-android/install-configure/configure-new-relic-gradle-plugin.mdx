...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Especifique quais tipos de compilação de variante devem relatar mapas do ProGuard para o New Relic. Para aplicativos React Native, isso também controla quais tipos de compilação de variante fazem upload automático do mapa de origem do JavaScript. Padrão: `Release`
      </td>
    </tr>
    ...
    <tr>
      <td>
        `webviewInstrumentationEnabled`
      </td>

      <td>
        Defina como `false` para desabilitar a instrumentação de webview.

        * Padrão: `true`

        Suportado quando usado com o agente Android v7.6.15 e posterior.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  <Collapser id="app-level" title="Ativar uploads de mapas para diversas variantes">
    Vários tipos de build podem ser especificados, separados por vírgulas. Para aplicativos React Native, adicionar `Debug` aqui também ativa o upload automático do source map de JavaScript para builds de depuração:
  ...
  <Collapser id="app-level" title="Desativar a Instrumentação de Webview">
    Para desativar a instrumentação de webview
