...
<Callout variant="important" title="Alteração com quebra de compatibilidade no relatório de erros (1.9.0)">
  A partir do agente React Native versão 1.9.0, Os erros de JavaScript são relatados por meio de um novo tipo de evento: `MobileJSError`. Com o novo Mobile Errors Protocol, os erros de JavaScript do React Native não são mais armazenados no tipo de evento `MobileHandledException`.

  Caso utilize a versão 1.9.0 ou superior, atualize quaisquer condições do alerta e dashboards que consultam `MobileHandledException` em busca de erros da camada JavaScript para utilizar `MobileJSError` em seu lugar. Por exemplo, substitua `SELECT count(*) FROM MobileHandledException WHERE platform = 'reactnative'` por `SELECT count(*) FROM MobileJSError`. Atualize também os scripts de build para a versão mais recente para que [os source maps sejam carregados corretamente](/docs/mobile-monitoring/new-relic-monitoring-react-native/react-native-agent-js-error-reporting) para o novo tipo de evento.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    Se você compila com o [EAS Build](https://docs.expo.dev/build/introduction/), consulte [Configure o agente React Native para o EAS Build](/docs/mobile-monitoring/new-relic-monitoring-react-native/configure-eas-build-react-native) para configurar também o upload automático de arquivos de source map, dSYM e de mapeamento do Android.
  </Step>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Registrar erros personalizados](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-errors/#react)
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="v1.9.0" title="Agente React Native versão 1.9.0 e superior">
    Erros de JavaScript e rejeições de promessas são registrados como eventos `MobileJSError`. É possível ver a trilha de evento, os atributos e o stack trace de cada erro de JavaScript na New Relic.
  ...
  <Collapser id="v1.2.0" title="Agente React Native versão 1.2.0 a 1.8.x">
...
<CollapserGroup>
  <Collapser id="logs-v1.4.1" title="Agente React Native versão 1.4.1 e superior">
    A partir da versão 1.4.1 do agente React Native, Os logs do console JavaScript são relatados como eventos de log. É possível visualizá-los na [interface de logs](/docs/logs/ui-data/use-logs-ui) ou consultá-los acessando <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Query your data**</DNT> e executando:
  ...
  <Collapser id="logs-v1.4.0" title="Agente React Native versão 1.4.0 e inferior">
