...
2. Visualizar <DNT>**Crashes**</DNT>.
3. Selecione uma falha específica do <DNT>**Total crashes**</DNT> em Falhas do grupo.
...
Em algumas circunstâncias, o upload automático de arquivos dSYM do New Relic pode falhar. Se o upload do dSYM falhar, será criado um arquivo `upload_dsym_results`. Por exemplo, se houver uma falha de rede e o upload do dSYM não for concluído, `upload_dsym_results` conterá um log do que deu errado. Para obter informações adicionais sobre como o New Relic lida com uploads de dSYM, consulte a [Comunidade Técnica Online do New Relic](https://support.newrelic.com/s/hubtopic/aAX8W0000008Yv9/relic-solution-ios-dsym-upload-deepdive).
