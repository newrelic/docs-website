...
Agende três trabalhos de coleta de dados com variantes focadas nos seguintes grupos de provedores de dados. Para obter instruções sobre como agendar tarefas do gerenciador de coleta antes de `SP03`, consulte [Variantes e tarefas antes do SP03](/docs/sap-solutions/additional-resources/variants-and-jobs-before-sp03). Separe os provedores de dados em três grupos para agendamento.

* **TRACE**: Neste grupo estão os provedores de dados relacionados ao desempenho banco de dados (STAD):

  * Inclui provedores relacionados ao desempenho do banco de dados, como `NR.SAP.TRANSACTION` (provedor de dados trace ), `NR.SAP.TRANSACTION.COMPLETE` (coleta completa de desempenho do banco de dados com processo RFC de segundo plano paralelo), `NR.SAP.SYSTEM.PROCESS`, `NR_SAP_PROGRAM`, `NR_SAP_WORKLOAD_COMPLETE` e `NR.SAP.WORKLOAD.COMPLETE`.
...
* **IDOC**: Nos sistemas de produção, a coleta de IDOC pode levar um tempo significativo:
  * Inclui provedores relacionados ao IDOC, como `NR_SAP_IDOC` (principal provedor de dados de eventos de status do IDOC), `NR_SAP_IDOC_COMPLETE` (recuperação completa do IDOC com coleta de BG RFC) e `NR_SAP_IDOC_Configuration` (baixar dados mestres do IDOC para melhorar o desempenho do provedor de dados do IDOC).

* **SYSTEM**: Restante dos provedores de dados.
