---
title: Traga seus próprios dados
metaDescription: 'Bring your own ML model telemetry into New Relic, and start getting value from your ML model data in just a few minutes'
freshnessValidatedDate: never
translationType: machine
---

import modelperformancemonitoringPredictions from 'images/model-performance-monitoring_screenshot-full_predictions.webp'

import modelperformancemonitoringPerformanceDrift from 'images/model-performance-monitoring_screenshot-full_performance-drift.webp'

import modelperformancemonitoringModelFeatures from 'images/model-performance-monitoring_screenshot-full_model-features.webp'

import modelperformancemonitoringFeatureDistribution from 'images/model-performance-monitoring_screenshot-full_feature-distribution.webp'

import modelperformancemonitoringStandardDeviation from 'images/model-performance-monitoring_screenshot-full_standard-deviation.webp'

import modelperformancemonitoringModelPerformanceTab from 'images/model-performance-monitoring_screenshot-full_model-performance-tab.webp'

A solução MLOps da New Relic fornece uma biblioteca Python que facilita o monitor de seus modelos de aprendizado de máquina. **Machine Learning Model Monitoring** de New Relic Bring Your Own (BYO) é uma oferta do conjunto de produtos New Relic que permite ao usuário monitor o desempenho de seus próprios modelos de aprendizado de máquina, independentemente da plataforma ou framework que está usando. Isso pode ajudar o usuário a identificar problemas com seus modelos e tomar medidas corretivas antes que esses problemas afetem a precisão ou o desempenho do aplicativo.

Num espaço digital cada vez mais complexo, as equipas de dados dependem fortemente de motores de previsão para tomar decisões. O monitoramento do modelo de aprendizado de máquina da New Relic permite que sua equipe dê um passo atrás e observe o cenário completo. Nosso monitoramento de desempenho do modelo permite que sua equipe examine seus modelos de ML para identificar problemas de forma eficiente e tomar decisões, aproveitando o recurso principal do NR, como definir alerta em qualquer métrica e ser notificado assim que seu desempenho de ML mudar.

## Fácil de integrar [#integrate]

O pacote [ml-desempenho-monitoramento](https://github.com/newrelic-experimental/ml-performance-monitoring) python, baseado em [newrelic-telemetry-sdk-python](https://github.com/newrelic/newrelic-telemetry-sdk-python)​​, permite enviar os valores de recurso e predição do seu modelo, bem como métrica personalizada, bastando adicionar algumas linhas ao seu código.

Use o pacote python para enviar os seguintes tipos de dados para o New Relic:

* **Inference data:** transmita os valores de recurso e previsão do seu modelo. Os dados de inferência são transmitidos como um [evento personalizado](/docs/data-apis/ingest-apis/introduction-event-api/) denominado InferenceData.
* **Data metrics:** em vez de enviar todos os seus dados brutos de inferência, selecione a opção de estatísticas agregadas sobre o recurso e as previsões (por exemplo, mínimo, máximo, média ou percentil). Serão enviados automaticamente como [métrica](/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/).
* **Custom metrics:** Calcule sua própria métrica, monitor o desempenho do seu modelo ou dados do modelo e transmita-os para o New Relic usando a função record_metrics. Serão enviados como [métrica](/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/).

## Ver previsões do modelo [#view-predictions]

Ao monitorar o desempenho dos seus modelos de machine learning, você pode identificar quaisquer problemas que possam estar impactando sua precisão ou desempenho. Você verá informações vitais em seu dashboard , como previsões totais do modelo e distribuição ao longo do tempo. Além disso, você pode definir <InlinePopover type="alerts"/>para rastrear quaisquer alterações nas previsões totais ou escolhas altas/baixas.

<img
  title="An image of the model performance and predictions in New Relic"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringPredictions}
/>

## Detectar modelo e imprecisão de dados [#detect-drift]

Com o monitoramento em tempo real, você pode identificar rapidamente problemas em seus modelos de aprendizado de máquina, detectando modelo e imprecisão de dados. Modelo e imprecisão de dados são mudanças no seu ambiente do mundo real que podem afetar o poder preditivo do seu modelo. Ao obter dados em tempo real sobre seu modelo e imprecisão de dados, você pode tomar ações corretivas rapidamente antes que esses problemas afetem o desempenho do seu aplicativo.

<img
  title="An image of the model and data drift in New Relic"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringPerformanceDrift}
/>

## Mergulhe mais fundo no recurso Modelo [#view-model-features]

Obtenha insights e dados estatísticos sobre seu recurso e receba alertas sobre qualquer Desvio de comportamento padrão e esperado.

<img
  title="An image of the model features in New Relic"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringModelFeatures}
/>

<img
  title="An image of the feature distribution in New Relic"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringFeatureDistribution}
/>

<img
  title="An image of the model count, missing values, and standard deviation in New Relic"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringStandardDeviation}
/>

## Como começar a monitorar o desempenho do seu modelo de ML [#get-benefits]

Primeiro, use a documentação [de primeiros passos](/docs/mlops/bring-your-own/getting-started-byo/) para iniciar o streaming dos dados do seu modelo. Em seguida, clique em **Model Performance** na página **All Capabilities** na interface do New Relic (não se esqueça de fixá-lo) e você verá a visualização do desempenho do seu modelo com base nos dados que seu modelo está enviando.

<img
  title="An image of the model performance tab in the New Relic navigation menu"
  alt="An image of the New Relic MLOps dashboard."
  src={modelperformancemonitoringModelPerformanceTab}
/>