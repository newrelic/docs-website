...
GoエージェントはLinux、macOS、WindowsでGolang 1.7以上を必要とします。詳細については、「[Goエージェントの互換性と要件](/docs/agents/go-agent/get-started/go-agent-compatibility-requirements)」をご覧ください。

## Goエージェントのインストール [#get-new-relic]

Goエージェントをインストールするには、[New Relicのライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)が必要です。ライセンスキーを入手した後、エージェントをインストールするには次の手順に従います。
...
2. `github.com/newrelic/go-agent/v3/newrelic`パッケージをアプリケーションにインポートします。
