...
![안내식 설치 CLI의 스크린샷.](./images/guided-install-cli.png "가이드 설치 CLI.")
...
개별 온-호스트 통합을 설치하려면 다음 명령을 실행합니다.
...
<CollapserGroup>
  ...
  >
    ...
    특정 암호를 사용하여 `newrelic@localhost` 사용자를 만듭니다.
  ...
  >
    ...
    `status_url` : 상태 모듈을 사용하여 메트릭을 제공하기 위해 설정한 URL입니다. 기본값 `127.0.0.1` 이 잘못된 경우 시스템의 주소/FQDN/URL을 대체하십시오. 예시:

    `status_url: http://127.0.0.1/status`

    다음 NGINX 문서에서 `status_url` 에 대해 자세히 읽을 수 있습니다.
    ...
    * NGINX Plus: [HTTP 상태 모듈](https://nginx.org/en/docs/http/ngx_http_status_module.html) 및 [HTTP API 모듈](http://nginx.org/en/docs/http/ngx_http_api_module.html)
    ...
    NGINX가 설치된 방법에 따라 `status_url` 을 설정하는 다양한 방법이 있습니다.
