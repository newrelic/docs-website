...
워크로드에 있는 엔터티의 경고 상태에서 파생된 워크로드 상태는 [워크](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-one-workloads-isolate-resolve-incidents-faster) 로드가 어떻게 작동하는지 알려줍니다.
...
[New Relic One](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one) 은 [New Relic 경고](/docs/alerts) 의 결과를 기반으로 모든 엔티티에 대한 상태 값을 제공합니다. [New Relic Explorer](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer-view-performance-across-apps-services-hosts) 에서 각 개체에 대한 [색상으로 구분된 경고 상태](/docs/alerts/new-relic-alerts/configuring-alert-policies/identify-entities-without-alert-policies#colors) 를 확인하거나 API를 통해 경고 상태 값을 사용할 수 있습니다. 예를 들어 심각한 위반이 진행 중임을 나타내는 빨간색 경고 상태가 표시될 수 있습니다.
...
1. [one.newrelic.com](https://one.newrelic.com/ "링크가 새 창에서 열립니다.") 으로 이동하여 **더보기** > **워크로드 보기** 를 클릭합니다.
2. 보기 추가를 클릭합니다.
3. 보기에 의미 있는 이름(예: 팀 또는 사업부 이름)을 지정하고 보기를 연결할 계정을 선택하십시오.
4. 이름 또는 태그별로 보기에 포함할 작업 부하를 선택합니다.
5. 새 보기를 저장합니다.

![워크로드\_views.png](./images/workloads_views.png "워크로드_views.png")
...
모든 워크로드의 상태는 정기적으로 계산되며 그 결과는 `WorkloadStatus` 이벤트를 통해 NRDB에 저장됩니다. 이를 통해 워크로드가 Disrupted 또는 Degraded 상태가 될 때마다 알려주는 경고 조건을 설정할 수 있습니다.

경고 조건을 설정하려면 다음 단계를 따르십시오.

1. [one.newrelic.com](https://one.newrelic.com) 으로 이동하여 **경고 및 AI** 를 선택합니다. 새 경고 조건을 추가할 정책을 선택하거나 적절한 알림 채널을 사용하여 새 정책을 만듭니다. 그런 다음 **조건 만들기 를** 클릭합니다.
...
4. * 워크로드 UI에서 **메타데이터 보기 및 태그 관리** 를 클릭하여 워크로드 GUID를 얻을 수 있습니다.
