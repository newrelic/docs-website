...
New Relic의 관점에서 **엔티티** 는 의도적으로 광범위한 개념입니다. 엔터티는) New Relic에 데이터를 보고하거나 우리가 액세스할 수 있는 데이터를 포함하고 b) 고유한 엔터티 ID로 식별한 모든 것입니다. 대부분의 엔터티에서 ID는 `entityGuid` [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#attribute) 으로 표시됩니다.
...
또한 [엔티티 간의 관계](/docs/apis/nerdgraph/examples/nerdgraph-relationships-api-tutorial) 가 매우 중요합니다. 우리의 배후 관계 매핑은 엔티티가 어떻게 연결되고 서로 영향을 미치는지 이해하는 데 도움이 됩니다. 이를 통해 가져오는 데이터가 기존 엔터티와 관련되는 방식 또는 다른 엔터티와 관련되는 방식을 구성할 수 있는 권한을 제공할 수 있습니다.
...
* 엔티티의 `entityGuid` , `entityName` 및 기타 메타데이터를 찾으려면 [New Relic Explorer](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer-view-performance-across-apps-services-hosts) 의 모니터링되는 엔티티 목록에서 엔티티의

  <Icon name="fe-more-horizontal"/>

  아이콘을 클릭하고 **메타데이터 및 태그 보기** 를 클릭합니다.

* 대부분의 엔터티에서 해당 GUID는 [`entityGuid`](/?event=SyntheticCheck&attribute=entityGuid) 속성으로 보고됩니다. 워크로드의 경우 `workloadGuid` 입니다. [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 를 실행하여 GUID로 엔터티를 찾을 수 있습니다.
...
* 엔터티 유형에 대한 기술적인 세부 정보를 알아보려면 [GitHub 리포지토리](https://github.com/newrelic/entity-definitions/tree/main/definitions) 를 참조하세요. 항목 유형의 `definition` 파일에 다음과 같은 정보가 표시됩니다.
  ...
  * `entityExpirationTime` : 해당 엔티티의 데이터가 UI에서 지속되는 기간( [데이터베이스 데이터 보존](/docs/telemetry-data-platform/manage-data/manage-data-retention) 과 다릅니다.
...
### 어떤 관계가 생성됩니까? [#relationship-created]
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            New Relic 에이전트가 모니터링하는 애플리케이션 간의 관계는 `DurationByCaller` 메트릭을 사용하여 보고됩니다. 수신자가 메트릭을 보고합니다.

            예를 들어 `DurationByCaller/Mobile/100/1234/HTTP/all` 메트릭은 호출자가 계정 100에 대한 `APPLICATION 1234` 임을 나타냅니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            New Relic이 모니터링하는 서비스(피호출자)를 호출하는 애플리케이션(호출자)은 `ExternalApp` 지표를 사용하여 호출자가 보고하는 관계를 생성합니다.

            예를 들어 `ExternalApp/dirac.vips.net/100#1234/all` 메트릭은 수신자가 계정 100의 `APPLICATION 1234` 임을 나타냅니다.
        ...
        <tr>
          ...
          <td>
            New Relic 에이전트가 서비스(호출자)가 New Relic에 의해 계측된 다른 서비스(호출 수신자)를 호출하는 것을 감지하면 호출 수신자는 메트릭 `ClientApplication` 을 보고합니다.

            예를 들어 `ClientApplication/100#1234/all` 메트릭은 호출자가 계정 100에 대한 `APPLICATION 1234` 임을 나타냅니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            애플리케이션이 데이터 저장소 인스턴스를 호출하면 `DatastoreInstance` 측정항목과 함께 보고되는 관계가 생성됩니다.

            예를 들어 `Datastore/instance/MySQL/172.16.16.3/3306` 메트릭은 데이터 저장소 인스턴스가 공급업체: MySQL, 호스트: 172.16.16.3, 포트: 3306임을 나타냅니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `nr.syntheticsMonitorId` 속성을 포함하는 에이전트 이벤트는 APM 애플리케이션(대상)과 상호 작용하는 합성 모니터(소스)로 설명되었습니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            이 관계는 DaemonSet이 New Relic Kubernetes 통합으로 계측된 클러스터에서 포드를 생성할 때 생성됩니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            브라우저 애플리케이션은 외부 서비스(URL)를 호출할 때 `Ajax/HostTransaction` 메트릭을 보고합니다.

            예시 메트릭: `Ajax/HostTransaction/api.segment.io:443/CallbackTime'`
          </td>
        </tr>
...
### 수동으로 관계 만들기 [#manual-relationships]

[GraphQL API인 NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/) 를 사용하여 추가 관계를 생성할 수 있습니다.
...
돌연변이 `entityRelationshipUserDefinedDelete` 를 사용하여 관계를 삭제합니다. `source` 와 `target` 은 필수이지만 `type` 은 그렇지 않습니다. 돌연변이가 `type` 없이 실행되면 두 엔티티 간의 모든 관계가 제거됩니다.
...
[관계 API 자습서](/docs/apis/nerdgraph/examples/nerdgraph-relationships-api-tutorial/) 에서 관계 가져오기에 대한 정보를 찾으십시오.

#### 몇 가지 고려 사항 [#some-relationships-considerations]
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            현대 및 클라우드 인프라에 공통적인 관계 및 계층적 사용 사례. 예를 들어 `HOST` 에 컨테이너가 포함되어 있음을 나타낼 수 있습니다.
          </td>
        </tr>
