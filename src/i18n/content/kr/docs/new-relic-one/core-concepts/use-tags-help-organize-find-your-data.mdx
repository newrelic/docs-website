...
<img
  title="태그를 사용한 New Relic 필터링 워크로드"
  alt="태그를 사용한 New Relic 필터링 워크로드"
  src={newRelicTagsWorkloads0}
/><figcaption>여기에서 특정 계정에 대한 [워크로드](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster) 를 필터링하는 데 사용되는 `accountId` 태그를 볼 수 있습니다.</figcaption>
