...
<Callout variant="important">
  2025년 5월 21일부터 신규 고객은 더 이상 NRQL 삭제 규칙을 사용할 수 없습니다. [NRQL 폐지 규칙은 2026년 1월 7일에 종료됩니다](/eol/2025/05/drop-rule-filter).
      ...
      <Collapser id="gitops-step3" title="3단계: 연속 통합/연속 배포(CI/CD) 환경에서 Pipeline cloud 규칙 가져오기">
        ...
        <Steps>
          ...
          <Step>
            ...
            <Callout title="메모">
              이러한 명령은 뉴렐릭에서 리소스를 삭제하지 않고 Terraform 상태에서 리소스를 제거합니다. 이전 삭제 규칙은 수명 종료일(2026년 1월 7일)에 뉴렐릭에서 제거되지만, 이 단계 이후로는 Terraform에서 더 이상 관리되지 않습니다.
            </Callout>
          </Step>
  ...
  <Collapser id="local-terraform" title="로컬 Terraform 사용">
    ...
    <CollapserGroup>
      ...
      <Collapser id="local-step3" title="3. Terraform 상태에서 이전 삭제 규칙 제거">
        이 단계에서는 뉴렐릭 CLI를 사용하여 Terraform 상태에서 이전 NRQL 삭제 규칙 리소스를 안전하게 제거합니다. 이 명령은 Terraform 관리에서만 해당 규칙을 제거합니다. 실제 삭제 규칙은 수명 종료일(2026년 1월 7일)에 뉴렐릭에서 제거됩니다.
