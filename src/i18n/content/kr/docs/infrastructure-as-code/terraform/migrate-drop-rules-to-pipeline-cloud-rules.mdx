...
<Callout variant="important">
  2025년 5월 21일부터 신규 고객은 더 이상 NRQL 드롭 규칙을 사용할 수 없습니다. [NRQL 드롭 규칙은 2026년 6월 30일에 사용이 종료됩니다](/eol/2025/05/drop-rule-filter).
      ...
      <Collapser id="gitops-step3" title="3단계: 연속 통합/연속 배포(CI/CD) 환경에서 Pipeline cloud 규칙 가져오기">
        ...
        <Steps>
          ...
          <Step>
            ...
            <Callout title="메모">
              이 명령어들은 뉴렐릭에서 리소스를 삭제하지 않고 테라폼 상태에서 리소스를 제거합니다. 기존 드롭 규칙은 뉴렐릭의 서비스 종료일(2026년 6월 30일)에 완전히 제거될 예정이지만, 이후에는 Terraform에서 더 이상 관리되지 않습니다.
            </Callout>
          </Step>
  ...
  <Collapser id="local-terraform" title="로컬 Terraform 사용">
    ...
    <CollapserGroup>
      ...
      <Collapser id="local-step3" title="3. Terraform 상태에서 이전 삭제 규칙 제거">
        이 단계에서는 뉴렐릭 CLI를 사용하여 Terraform 상태에서 기존 NRQL 드롭 규칙 리소스를 안전하게 제거합니다. 이 명령은 Terraform 관리에서만 해당 항목을 제거하는 것이며, 실제 삭제 규칙은 서비스 종료일(2026년 6월 30일)에 뉴렐릭에서 완전히 제거될 예정입니다.
