...
영상에서는 Terraform을 설치하고 New Relic 경고를 설정하는 추가 단계를 검토합니다. Terraform을 시작하는 데 도움이 필요한 경우 [New Relic 및 Terraform 시작하기에서는](/docs/more-integrations/terraform/terraform-intro/) Terraform을 설치하고 New Relic 경고를 설정하고 알림 채널을 지정하는 방법을 보여줍니다.
...
이 가이드를 사용하려면 New Relic과 Terraform에 대한 기본 지식이 있어야 합니다. 아직 New Relic 오픈 소스 에이전트를 배포하지 않은 경우 애플리케이션에 [New Relic을 설치하세요](/docs/agents/manage-apm-agents/installation/install-agent) . 또한 [Terraform CLI를 설치합니다](https://www.terraform.io/intro/getting-started/install.html).
...
<Steps>
  <Step>
    ...
    모듈 디렉터리에서 HostConditions라는 새 모듈에 대한 새 디렉터리를 만들고 `main.tf` 이라는 새 파일을 만듭니다.
    ...
    루트 프로젝트의 `main.tf` 파일에서 두 개의 공지 조건을 제거하고 이를 `HostConditions` 디렉터리의 새 `main.tf` 파일에 복사합니다.
    ...
    모듈 디렉터리에 공급자 세부 정보가 부족하여 콘솔에 오류가 표시됩니다. 오류를 수정하려면 `HostConditions` 디렉터리에 루트 공급자.tf의 복사본을 만듭니다.
    ...
    오류는 `./modules/HostConditions/provider.tf` 에 사용된 정책 ID가 존재하지 않기 때문에 발생합니다. 모듈에 전달하려면 변수가 필요합니다.
  </Step>
  ...
  <Step>
    ...
    다음으로 리소스 블록에서 기존 `policyId` 새 변수로 바꿉니다.
    ...
    루트 디렉터리 `main.tf` 에서 모듈 블록을 업데이트하여 `policyId` 변수를 추가합니다.
    ...
    `HostConditions/main.tf` 에 새 변수를 추가합니다.
    ...
    `HostConditons/main.tf` 에 추가된 새 변수를 사용하려면 공지 조건을 업데이트하세요.
    ...
    `HostConditions/main.tf` 에 기본 변수 값을 추가합니다.
    ...
    루트 디렉터리 `main.tf` 에서 모듈 블록을 업데이트하여 `cpu_critical`, `cpu_warning` 및 `diskPercentage` 변수를 추가합니다.
  ...
  <Step>
    ...
    이미 존재하는 블루베리 정책에 연결하여 모듈을 재사용할 수 있습니다. 시작하기 전에 뉴켈릭 계정에서 **Preexisting Policy** 라는 새 공지를 생성하세요.
    ...
    다음으로 두 번째 모듈 블록 이름 `HostConditions2` 을 만듭니다. 기존 정책에 공지 조건을 추가합니다.
    ...
    `terraform init` 실행하여 새 모듈을 초기화하고 `terraform apply` 실행하여 뉴렐릭 계정에 변경 사항을 적용합니다.
  ...
  <Step>
    ...
    먼저 HostModules 디렉터리 내에서 새 Github 리포지터리를 초기화합니다. 커밋을 위해 단계에 `main.tf` 및 `provider.tf` 추가합니다.
    ...
    Github 리포지터리를 확인하여 이제 `main.tf` 및 `provider.tf` 이 리포지터리에 있는지 확인하세요. GitHub 리포지터리의 웹 URL을 복사하여 리포지터리를 복제하세요.

    GitHub를 `HostConditions` 의 소스로 사용하여 루트 `main.tf` 파일을 업데이트합니다.
  ...
  <Step>
    ...
    규정의 `provider.tf` 에서 Amazon S3용 terraform 백앤드 블록을 추가합니다.
