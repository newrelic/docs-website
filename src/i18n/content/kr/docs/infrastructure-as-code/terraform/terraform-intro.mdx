...
<Steps>
  ...
  <Step>
    ...
    3. 사용자의 [뉴렐릭 지역](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers#verifying-account). 뉴렐릭 URL이 `one.newrelic.com`인 경우 지역은 `US` 이고, URL이 `one.eu.newrelic.com`인 경우 `EU` 이며, URL이 `one.jp.newrelic.com`인 경우 `JP` 입니다.
