...
느리게 실행되는 애플리케이션 문제를 해결하려면 성능 문제의 근본 원인을 식별하는 데 체계적인 접근 방식을 취해야 합니다. 다음 단계들은 애플리케이션 속도에 부정적인 영향을 미치는 문제를 진단 및 해결하는 데 필요한 정보를 제공합니다.

## 애플리케이션 계측을 통한 성능 모니터링 구현 [#prereq]

사이트 성능을 향상하려면, 먼저 현재 상황을 파악할 수 있어야 합니다. 뉴렐릭 모니터링은 항상 통합(<DoNotTranslate>**integration**</DoNotTranslate>) 즉, 관련 기술에서 데이터를 수집하여 뉴렐릭에 보고하는 코드로 시작됩니다. 애플리케이션 성능을 모니터링하려면 앱이 사용하는 언어용으로 특별히 생성된 에이전트를 사용해야 합니다.
...
에이전트를 설치한 후 <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/nr1-core?filters=(domain%3D'APM'ANDtype%3D'APPLICATION'))**</DoNotTranslate>으로 이동하여 앱을 선택합니다. 아직 많은 데이터가 표시되지 않으면 애플리케이션이 실행되어 에이전트가 실시간 데이터를 수집할 수 있도록 잠시 기다립니다.

## 애플리케이션 문제 식별 [#identify]

<SideBySide>
  <Side>
    웹 트랜잭션 시간(<DoNotTranslate>**Web transactions time**</DoNotTranslate>) 차트를 살펴봅니다. 이 차트는 앱 내 특정 메트릭들의 평균 응답 시간을 표시합니다. 차트가 데이터로 채워지면 모든 선이나 세그먼트에서 비정상적인 스파이크가 있는지 확인해 기록해 둡니다. 동시에, 지속적으로 많은 시간이 걸리는 세그먼트나 선을 기록해 둡니다.
...
<DocTiles>
  ...
  >
    파란색 선은 애플리케이션 코드(<DoNotTranslate>**application code**</DoNotTranslate>)입니다. 애플리케이션에서 외부 또는 데이터베이스 호출이 아닌 코드에 소요된 시간을 의미합니다. 이 문제를 해결하려면 여기를 클릭하여 튜토리얼 2단계를 확인합니다.
  </DocTile>
  ...
  >
    노란색 세그먼트는 데이터베이스 쿼리(<DoNotTranslate>**database queries**</DoNotTranslate>)입니다. 데이터베이스 쿼리 호출 및 대기에 소요된 시간을 의미합니다. 이 문제를 해결하려면 여기를 클릭하여 튜토리얼 3단계를 확인합니다.
  </DocTile>
  ...
  >
    녹색 세그먼트는 외부 서비스(<DoNotTranslate>**external services**</DoNotTranslate>)입니다. HTTP 서비스 또는 엔드포인트를 호출하는 데 소요된 시간을 의미합니다. 이는 동일한 앱에 속한 HTTP 엔드포인트일 수도 있고, 동일한 시스템에 속한 다른 서비스일 수도 있으며, 완전한 서드파티 외부 API일 수도 있습니다. 이 문제를 해결하려면 여기를 클릭하여 튜토리얼 4단계를 확인합니다.
  </DocTile>
</DocTiles>