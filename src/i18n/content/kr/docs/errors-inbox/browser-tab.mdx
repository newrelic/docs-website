...
* <DNT>**Dynamic grouping:**</DNT>
  ...
  오류 발생에 대한 기본 그룹화는 오류 메시지를 기반으로 합니다. 한 번에 최대 5개 속성까지 속성별로 그룹화 옵션을 변경할 수 있습니다.
...
* [이벤트 추적을](#event-trail) 포함한 자세한 정보를 얻으려면
  ...
  페이지는 스택 추적 없이도 실행 가능한 정보를 제공하도록 설계되었습니다.
...
* <DNT>**Agent version.**</DNT>
  ...
  이벤트 로그 데이터를 캡처하려면 사이트에 [SPA 모니터링이](/docs/browser/single-page-app-monitoring/get-started/install-single-page-app-monitoring-new-relic-browser) 활성화되어 있어야 합니다(브라우저 에이전트 버전 1071 이상).
...
* <DNT>**JS error not part of a browser [interaction](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#interaction).**</DNT>
  ...
  복합체 외부에서 JS 오류가 발생하는 경우, 관련 브라우저의 바이오합성 내역을 캡쳐할 수 없습니다. (예: 비동기 JS 파일에 오류가 발생한 경우)

* <DNT>**A click triggered the JS error.**</DNT>

  어떤 경우에는 오류가 클릭의 직접적인 결과인 경우 관련 복합체 합성이 캡처되지 않습니다. 이벤트 로그나 스택 추적이 없으면 [오류 세부 정보 차트를](#error-details) 사용하여 브라우저 유형, 장치 유형 및 URI별로 오류가 미치는 영향을 분석할 수 있습니다.
