...
<DNT>**Group errors**</DNT> 탭을 사용하면 심층적인 분석을 위해 오류를 동적으로 필터링하고 그룹화할 수 있습니다. 그룹 오류는 JavaScript 오류가 발생하는 위치를 보여주고 근본 원인을 파악하는 데 도움이 되는 도구를 제공합니다. 또한 **Sessions impacted** \[영향을 받은 세션을] 통해 오류 범위를 추적할 수 있으며, 이는 각 오류 그룹에 의해 영향을 받는 브라우저 세션 수를 자동으로 보여줍니다. 또한, [쿼리 빌더](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) 에서 JS 오류 데이터의 대시보드를 쿼리하고 만들거나, 브라우저 API를 사용하여 처리된 오류를 모니터링할 수 있습니다.
...
## 보안 정책 위반 이벤트(CSP 위반) [#policy-violations]

뉴렐릭은 브라우저에서 발생하는 `securitypolicyviolation` 이벤트를 캡처하고 그러한 발생을 보고합니다. 하지만 이 기능은 버전 1.313.0부터 시작되었으며, 아직 오류 수신함(errors inbox) 보기와 같은 UI 에 통합되지 않았습니다. 따라서 현재로서는 이 데이터는 NRQL을 통해서만 이용할 수 있습니다. 보고된 속성 목록은 [속성 사전 이벤트를](/attribute-dictionary/?event=SecurityPolicyViolation) 참조하세요.
