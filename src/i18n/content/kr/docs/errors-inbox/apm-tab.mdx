...
특정 오류 그룹에 대한 심층 분석: 오류 급증을 일으키는 특정 호스트 그룹을 식별했을 수 있습니다. 상세 보기는 상황에 맞는 세부 정보를 제공합니다.

**Details view** 포함됩니다.

* **적용된 필터 및 그룹화 값:** 특정 항목의 퍼머링크를 받을 수 있습니다. 발생 컨텍스트를 이해하려면 필터 및 그룹화 기준 선택을 아는 것이 중요합니다.
* **프로필:** 프로필 탭은 특정 오류 그룹 내의 모든 오류 발생에 대한 흥미로운 추세를 관찰하는 데 도움이 됩니다. 추가 분석을 위해 이 정보를 CSV 파일로 다운로드할 수 있습니다. 속성 막대를 클릭하면 속성 및 개수의 분석을 보여주는 테이블이 렌더링됩니다.
