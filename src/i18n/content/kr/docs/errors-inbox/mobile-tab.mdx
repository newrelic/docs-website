...
<CollapserGroup>
  ...
  <Collapser id="insights" title="쿼리 및 이미지 링크: 충돌 데이터를 쿼리하고 다른 사람과 차트를 공유합니다.">
    모바일 모니터링의 <DNT>**Crash analysis**</DNT> 차트는 [모바일 이벤트용 기본 속성](/docs/data-apis/understand-data/event-data/events-reported-mobile-monitoring)과 함께 이 이벤트 유형에 추가한 모든 커스텀 속성을 사용합니다. 데이터를 보거나 공유하려면 줄임표 아이콘을 클릭하십시오.

    * <DNT>**Add to dashboard**</DNT> 링크: [차트를 확인](/docs/nrql/get-started/introduction-nrql-new-relics-query-language)하고, 새 대시보드나 기존 대시보드에 복사하십시오.
    * <DNT>**View query**</DNT> 링크: 차트 데이터를 계산하는 데 사용된 [NRQL 쿼리](/docs/nrql/get-started/introduction-nrql-new-relics-query-language) 를 확인하십시오.
