---
title: ktranslate 도커 컨테이너 상태 모니터링
tags:
  - Integrations
  - Network monitoring
...
New Relic 네트워크 모니터링을 위해 **ktranslate** Docker 컨테이너를 실행하는 동안 컨테이너의 상태를 모니터링하여 잠재적인 문제를 사전에 감지할 수 있습니다.

**ktranslate** 컨테이너 이미지에는 런타임 중에 사용할 수 있는 `-tee_logs=true` 및 `-metrics=jchf` 설정이 있어 상태 측정항목을 New Relic에 직접 보낼 수 있습니다. New Relic 가이드 설치를 통해 네트워크 모니터링을 설치할 때 기본적으로 활성화됩니다. 네트워크 모니터링을 수동으로 설치할 때 설정하는 것을 권장합니다.
...
2. **로그 찾기 위치** 에 `collector.name:"ktranslate"` 을 입력하고 **쿼리 로그** 를 클릭합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="어떤 버전의 ktranslate를 실행하고 있습니까?"
  >
    로그 UI:
  ...
  <Collapser
    id="docker-arguments"
    title="런타임에 Docker에 어떤 인수가 전달되었습니까?"
  >
    로그 UI:
  ...
  <Collapser
    id="match_attributes"
    title="내 match_attributes 필터가 내 기기에서 작동합니까?"
  >
    로그 UI:
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 으로 이동 **하여 데이터 쿼리** 를 클릭합니다.
2. 다음 NRQL 쿼리 중 하나를 입력합니다.

<CollapserGroup>
  <Collapser
    id="ktranslate-versions"
    title="내 ktranslate 애플리케이션의 현재 버전은 무엇입니까?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="top-level-health"
    title="내 ktranslate 애플리케이션의 상태는 어떻습니까?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="delivery-health"
    title="New Relic으로 배송된 상품의 상태는 어떻습니까?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="snmp-health"
    title="내 SNMP 컬렉션의 전반적인 상태는 어떻습니까?"
  >
    ```sql
    FROM Metric
    SELECT
