...
다음은 **ktranslate** 도커 이미지가 SNMP 및 흐름 데이터 기기를 폴링하는 데 사용하는 `snmp-base.yaml` 파일에서 사용할 수 있는 다양한 구성 옵션의 예입니다. [GitHub의 ktranslate 리포지토리](https://github.com/kentik/ktranslate/blob/main/config/snmp.yaml.sample) 에서 주석이 많이 달린 샘플을 볼 수도 있습니다.
...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            모든 SNMP 폴링을 비활성화하고 `true` 일 때 [응답 시간](#response_time-attribute) 폴링을 활성화합니다. 이 설정은 `global.response_time` 속성을 재정의하는 데 사용됩니다. 기본적으로 `false` 로 설정됩니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            SNMP 트랩을 수신하기 위한 수신 IP 포트입니다. 기본적으로 `127.0.0.1:1620` 으로 설정됩니다. SNMP 트랩 기본값 `162` 을 사용하려면 실행 명령에서 `--net=host` 를 `-p 162:1620/udp` 으로 바꿔야 합니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            SNMP 트랩을 수신하기 위한 SNMPv1/v2c 커뮤니티 문자열입니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            시간(밀리초) SNMP 쿼리 시간 초과입니다. `devices.<deviceName>.timeout_ms` 속성을 사용하여 기기별로 재정의할 수 있습니다. 기본적으로 다음으로 설정되어 있습니다. `5000`
          </td>
        </tr>
  ...
  >
    **아리스타 eAPI**<table>
    ...
    <Callout variant="tip">
      `aws.sm.$SECRET_NAME` 구문을 사용하여 eAPI 구성에서 기본적으로 [AWS Secrets Manager](https://aws.amazon.com/secrets-manager/) 를 사용할 수 있으며, Docker 런타임 중에 **ktranslate** 가 자격 증명을 가져오도록 필요에 따라 `$SECRET_NAME` 을 바꿀 수 있습니다.
    </Callout>
  </Collapser>
</CollapserGroup>
...
이 기능은 평균, 최소, 최대 및 stddev RTT(왕복 시간)를 수집하기 위해 [go-ping](https://pkg.go.dev/github.com/sparrc/go-ping) 패키지를 사용하여 권한이 없는 UDP 패킷을 장치에 보냅니다. 이 패키지는 또한 `ktranslate` 에서 기기 IP 주소로 초당 1개의 패킷을 보내는 것을 기반으로 하는 엔드포인트의 패킷 손실 비율을 표시합니다. 이는 `devices.<deviceName>.ping_interval_sec` 속성을 설정하여 재정의할 수 있습니다. Docker 런타임 중에 `KENTIK_PING_PRIV=true` 환경 변수를 설정하여 권한 있는 ICMP 패킷의 사용을 활성화할 수 있습니다.
