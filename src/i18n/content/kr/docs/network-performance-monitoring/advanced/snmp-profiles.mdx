...
이것은 가장 일반적인 시나리오입니다. 세부 정보가 포함된 문제를 열면 모든 커뮤니티 구성원, 관리자 또는 기타 사용자가 문제를 해결하고 프로필 저장소를 업데이트할 수 있습니다.
...
<Callout variant="tip">
  SNMP 워크를 제공하는 것은 매우 중요하며 반드시 제공되어야 합니다. 이것은 MIB 파일과 동일 **하지 않습니다** .
</Callout>

리포지토리 관리자는 귀하가 제공한 정보를 검토하고 누락된 정보가 있으면 후속 조치를 취할 것입니다. 필요한 모든 데이터가 제공되면 구축할 프로필 대기열에 추가합니다. 리포지토리에 추가되면 문제가 종료됩니다. 최신 SNMP 프로필을 추가하려면 환경에서 [Ktranslate 도커 컨테이너를 업데이트하기](/docs/network-performance-monitoring/advanced/ktranslate-container-management#updating-the-container/) 만 하면 됩니다.
