...
이 문제를 해결하려면 먼저 차트의 오른쪽 상단 아이콘에서 **쿼리 보기** 작업을 선택하여 sankey 차트에 대한 기본 NRQL 쿼리를 조사하고 쿼리에서 null 또는 빈 값을 반환하는 필드를 식별합니다.다음 단계는 흐름 내보내기에서 흐름 템플릿을 업데이트하여 필요한 모든 필드가 예상대로 ktranslate 에이전트로 전송되도록 하는 것입니다.