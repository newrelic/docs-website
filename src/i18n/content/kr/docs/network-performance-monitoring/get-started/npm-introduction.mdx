...
* <DNT>**Device performance via SNMP**</DNT>: SNMP(Simple Network Management Protocol)는 네트워크 장치 간에 관리 정보를 교환하기 위한 애플리케이션 계층 프로토콜입니다. SNMP 데이터를 뉴렐릭으로 보내려면 [SNMP 데이터 모니터링을 설정](/docs/network-performance-monitoring/setup-performance-monitoring/snmp-performance-monitoring)합니다.
* <DNT>**Network syslogs**</DNT>: 변경된 사항, 오류 및 기타 장치 이벤트에 대한 가시성을 제공하여 성능 텔레메트리의 변경과 관련된 이유를 신속하게 파악할 수 있도록 합니다. 네트워크 syslog 데이터를 보내려면 [네트워크 syslog 모니터링을 설정](/docs/network-performance-monitoring/setup-performance-monitoring/network-syslog-monitoring)합니다.
* <DNT>**Cloud flow logs**</DNT>: 소스 및 대상 IP 주소 및 포트, 두 엔드포인트를 모두 설명하는 메타데이터, 주요 네트워크 성능 텔레메트리, 특정 통신 시도 상태를 캡처하여 호스트와 다른 서비스 간의 통신을 자세히 설명합니다. [AWS VPC](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/aws-vpc-flow-log-monitoring), [Azure VNet](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) 또는 [GCP VPC](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/gcp-vpc-flow-log-monitoring)에 대한 클라우드 흐름 로그 모니터링을 설정합니다.
* <DNT>**Network flow logs**</DNT>: 온프레미스 네트워크의 네트워크 인터페이스로 들어오고 나가는 IP 트래픽에 대한 정보를 캡처합니다. 네트워크 플로우 데이터를 뉴렐릭으로 전송하려면 [네트워크 플로우 데이터 모니터링을 설정](/docs/network-performance-monitoring/setup-performance-monitoring/network-flow-monitoring)합니다.
