...
네트워크 모니터링은 [뉴렐릭](https://one.newrelic.com)에서 이미 수집한 애플리케이션 및 인프라 데이터에 네트워크 데이터의 컨텍스트를 추가합니다. 네트워크 데이터를 모니터링하여 다음을 수행할 수 있습니다.
...
신용 카드는 필요하지 않습니다. 이미 계정이 있으십니까? [로그인](http://one.newrelic.com/)하십시오.
...
* **SNMP를 통한 장치 성능: SNMP** (Simple Network Management Protocol)는 네트워크 장치 간에 관리 정보를 교환하기 위한 애플리케이션 계층 프로토콜입니다. SNMP 데이터를 [뉴렐릭](https://one.newrelic.com)으로 보내려면 [SNMP 데이터 모니터링을 설정](/docs/network-performance-monitoring/setup-performance-monitoring/snmp-performance-monitoring)합니다.
...
* **네트워크 흐름 로그**: 온프레미스 네트워크의 네트워크 인터페이스로 들어오고 나가는 IP 트래픽에 대한 정보를 캡처합니다. 네트워크 흐름 데이터를 [뉴렐릭](https://one.newrelic.com)으로 보내려면[네트워크 흐름 데이터 모니터링을 설정](/docs/network-performance-monitoring/setup-performance-monitoring/network-flow-monitoring)합니다.
