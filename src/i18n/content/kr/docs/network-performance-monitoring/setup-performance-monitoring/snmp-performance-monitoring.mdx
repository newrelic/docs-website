...
3. 안내된 설치 프로세스에 설명된 단계를 따릅니다. 도커 또는 리눅스를 사용할 수 있습니다.
   ...
   <figcaption>
     **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Add more data > Network > SNMP** 통해 SNMP 데이터 모니터링을 설정합니다.
   </figcaption>

4. 프록시가 있는 경우 설치 후 `ktranslate` 서비스를 시작하기 전에 `/etc/default/` 의 `ktranslate.env` 파일에 프록시를 추가하십시오. `ktranslate.env` 파일은 다음과 유사해야 합니다.
...
5. [New Relic에서 네트워크 성능 데이터를 시각화하십시오](/docs/network-performance-monitoring/monitoring-network-data/visualize-network-data) .
