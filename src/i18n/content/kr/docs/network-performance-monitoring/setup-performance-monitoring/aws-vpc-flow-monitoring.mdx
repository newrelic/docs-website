...
<Callout variant="important">
  흐름 로그의 [기본 형식](https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html#flow-logs-default) 에는 `ktranslate` 이(가) 제대로 작동하는 데 필요한 모든 필수 필드가 포함되어 있지 않습니다. 아래 필드가 추가되었는지 확인해야 합니다. 그렇지 않으면 New Relic One에서 수신할 데이터가 불완전합니다.
</Callout>
