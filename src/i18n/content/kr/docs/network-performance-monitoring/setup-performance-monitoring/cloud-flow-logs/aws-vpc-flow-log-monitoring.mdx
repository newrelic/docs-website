...
1. <DNT>**Choose Setup Options**</DNT> 섹션에서:
...
2. <DNT>**Define Kinesis Firehose**</DNT> 섹션에서:
   ...
   * <DNT>
     ...
     필드에 전달에 실패한 메시지를 저장하는 데 사용할 [S3 버킷의 ARN을](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-arn-format.html) 입력합니다. ARN은 `arn:my_string` 형식을 따라야 합니다.

   * <DNT>
       **Firehose IAM Role**
     </DNT>

     필드에 Kinesis Data Firehose에서 사용할 [IAM 역할의 ARN을](https://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#firehose-assume-role) 입력합니다. ARN은 `arn:my_string` 형식을 따라야 합니다.
   ...
   * 선택적으로 VPC 흐름 로그를 샘플링하려면
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="샘플링 람다 사용"
     >
       * 먼저 <DNT>**Define Sampling Lambda**</DNT> 섹션에서 다음을 수행합니다.
       ...
       * 그런 다음 <DNT>**Create Sampling Lambda**</DNT> 섹션에서 <DNT>**Generate CLI Command**</DNT> 클릭하고 다음을 수행합니다.
...
3. <DNT>**Generate Kinesis Firehose**</DNT> 섹션에서 <DNT>**Generate CLI Command**</DNT> 클릭하고 다음을 수행합니다.
...
4. <DNT>**Define Flow Logs**</DNT> 섹션에서 다음을 수행합니다.
...
5. <DNT>**Create Flow Logs**</DNT> 섹션에서 <DNT>**Generate CLI Command**</DNT> 클릭하고 구문 블록의 콘텐츠를 복사합니다. 그런 다음 AWS CLI에서 이를 실행하여 지정된 리소스에 대한 흐름 로그 생성을 시작합니다.

6. <DNT>**Continue**</DNT> 클릭하여 뉴렐릭의 네트워크 모니터링 섹션에서 Amazon VPC 흐름 로그 탐색을 시작하세요.
