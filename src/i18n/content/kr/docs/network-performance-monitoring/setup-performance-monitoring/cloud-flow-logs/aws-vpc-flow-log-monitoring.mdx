...
<figcaption>
  **[New Relic 계정에 AWS VPC 흐름 로그를](https://one.newrelic.com/nr1-core?&state=bd947742-3034-63b7-7196-8baaf36dd8d9) 추가합니다** .
</figcaption>
...
<Callout variant="important">
  Kinesis Data Firehose를 통한 AWS VPC 흐름 로그는 아직 FedRAMP 고객에게 지원되지 않습니다. 그동안 [FedRAMP 수집 API](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints#data-ingest-apis) 를 사용할 수 있습니다.
</Callout>

* 환경:

  * [AWS CLI(v 1.9.15+) 설치](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html) .
  * AWS VPC 흐름 로그를 수집하는 AWS 리전 목록입니다.
...
## New Relic에서 AWS VPC 흐름 로그 모니터링 설정 [#setup-aws-vpc-flow-logs]

AWS VPC 흐름 로그용으로 설계된 대시보드가 포함된 번들을 설치하여 AWS VPC 흐름 로그를 설정할 수 있습니다.

1. [AWS VPC 흐름 로그 ](https://newrelic.com/instant-observability/aws-vpc-flow-logs/61d2ff67-f519-492c-a8de-6019b47d5bb2)에 대한 New Relic Instant Observability 페이지로 이동합니다.<img title="AWS VPC flow logs I/O page" alt="AWS VPC flow logs instant observability page" src={awsVPCflowLogsIO}/>

   <figcaption>
     AWS VPC 흐름 로그 모니터링을 설정하기 위한 **[New Relic Instant Observability](https://newrelic.com/instant-observability/aws-vpc-flow-logs/61d2ff67-f519-492c-a8de-6019b47d5bb2)** .
   </figcaption>
...
## 추가 AWS VPC 흐름 로그 보내기 [#additional-aws-vpc-flow-logs]

또한 더 많은 AWS VPC 흐름 로그를 설치해야 하거나 사용자 지정 대시보드와 함께 번들을 설치하지 않으려면 다음 단계를 따르십시오.

1. **[안내된 설치 프로세스를](https://one.newrelic.com/nr1-core?&state=bd947742-3034-63b7-7196-8baaf36dd8d9)** 시작합니다.
2. **계정** 선택 드롭다운에서 AWS VPC 흐름 로그를 보낼 계정을 선택하고 **계속** 을 클릭합니다.
3. **데이터 선택** 섹션에서 **AWS VPC 흐름 로그** 를 선택하고 **계속** 을 클릭합니다.
4. **설치 방법 선택** 섹션에서 **CLI** 를 계속하고 **계속** 을 클릭합니다.

이러한 단계가 끝나면 AWS Kinesis Firehose 서비스를 통해 AWS VPC 흐름 로그를 New Relic으로 전송하도록 설정하는 데 도움이 되는 새 마법사 팝업이 나타납니다.
...
* **흐름 소스 ID** 필드에 AWS VPC 흐름 로그를 생성할 VPC ID( `vpc-MY_STRING` ) 또는 서브넷 ID( `subnet-MY_STRING` )를 입력합니다.
...
6. **계속** 을 클릭하여 New Relic의 네트워크 모니터링 섹션에서 AWS VPC 흐름 로그 탐색을 시작합니다.
