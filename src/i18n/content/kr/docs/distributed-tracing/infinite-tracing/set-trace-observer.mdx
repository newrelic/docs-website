...
3. 트레이스 관찰자가 이미 존재하지 않으면 <DNT>**New trace observer**</DNT> 클릭하여 추가하고 정보를 입력한 후 <DNT>**Create**</DNT> 클릭합니다. 우리는 다음 지역을 제공합니다.
   ...
   * <DNT>**eu-central-1 (EU):**</DNT> EU 요청에서 데이터를 수집하고 EU 데이터센터에 보고하는 EU 기반 뉴렐릭 계정이 있는 경우 이 지역을 선택하세요.

   * <DNT>**eu-west-1 (US):**</DNT> EU 요청에서 데이터를 수집하고 미국 데이터센터에 보고하는 미국 기반 뉴렐릭 계정이 있는 경우 이 지역을 선택하세요.
...
6. (선택 사항) 추적 관찰자가 작동하는지 확인하려면 샘플 추적 페이로드를 보내는 것이 좋습니다. [Trace API를](/docs/understand-dependencies/distributed-tracing/trace-api/introduction-trace-api) 직접 사용하는 경우 API 작동 방식을 배우는 데 도움이 됩니다.
   ...
   <CollapserGroup>
     <Collapser id="sample-request" title="샘플 페이로드 보내기">
       ...
       5. 테스트는 성공을 나타내는 `HTTP/1.1 202 Accepted` 을 반환해야 합니다. 그렇지 않은 경우 다음과 같은 일반적인 문제를 확인하십시오.
          ...
          * [<DNT>**For other integrations**</DNT> 엔드포인트](#endpoints) 값을 사용했는지 확인하세요.
...
<CollapserGroup>
  <Collapser className="freq-link" id="timing" title="New Relic UI에서 내 데이터를 보려면 얼마나 걸리나요?">
  ...
  <Collapser className="freq-link" id="multi-named-apps" title="내 브라우저 앱이 여러 이름을 가진 APM 에이전트에 연결되어 있으면 어떻게 됩니까?">
  ...
  <Collapser className="freq-link" id="lambda-sampling" title="Lambda 샘플링은 어떻게 작동합니까?">
...
3. 다음 사항을 염두에 두고 포함할 데이터 소스를 선택하십시오.
   ...
   * 브라우저 앱이 여러 이름을 가진 <InlinePopover type="apm" />에이전트에 연결된 경우 이름은 하나만 선택할 수 있습니다.
