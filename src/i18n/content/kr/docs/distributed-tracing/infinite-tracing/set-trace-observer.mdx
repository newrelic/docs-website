...
[분산 추적을 설정하는 단계를](/docs/distributed-tracing/enable-configure/quick-start) 따르고 무한 추적이라는 고급 옵션에 대한 단계를 계속하는 경우 추적 관찰자를 설정해야 합니다. 추적 관찰자는 원격 측정 데이터를 New Relic으로 보내는 짧은 지연 시간과 저렴한 비용의 방법을 제공하는 Infinite Tracing을 위한 AWS 기반 서비스입니다. 추적 데이터를 수신하면 추적 관찰자는 [테일 기반 샘플링을](/docs/distributed-tracing/concepts/how-new-relic-distributed-tracing-works/#tail-based) 사용하여 무엇을 보관하고 New Relic으로 보낼지 결정합니다.
...
뉴렐릭 <InlinePopover type="apm"/>에이전트 또는 타사 통합을 사용하는 경우 새 트레이스 관찰자를 생성하려면 다음을 수행하세요.

1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infinite Tracing settings**</DNT> 으)로 이동합니다.
...
3. 트레이스 관찰자가 이미 존재하지 않으면 <DNT>**New trace observer**</DNT> 클릭하여 추가하고 정보를 입력한 후 <DNT>**Create**</DNT> 클릭합니다. 우리는 다음 지역을 제공합니다.
   ...
   * <DNT>
     ...
     EU 요청에서 데이터를 수집하고 EU 데이터센터에 보고하는 EU 기반 뉴렐릭 계정이 있는 경우 이 지역을 선택하세요.

   * <DNT>
       **eu-west-1 (US):**
     </DNT>

     EU 요청에서 데이터를 수집하고 미국 데이터센터에 보고하는 미국 기반 뉴렐릭 계정이 있는 경우 이 지역을 선택하세요.
...
4. <DNT>**Endpoints**</DNT> 드롭다운 메뉴에서 <DNT>**For other integrations**</DNT> 아래의 엔드포인트 값을 복사하여 준비하세요. 이 값은 코드 예제에서 `YOUR_TRACE_OBSERVER_URL` 로 참조됩니다. 이는 [트레이스 관찰자를](#sample-payload) 테스트하고, [텔레메트리 통합](/docs/distributed-tracing/enable-configure/quick-start) 을 구성하고, [트레이스 API](/docs/distributed-tracing/trace-api/introduction-trace-api/) 통해 데이터를 전송하는 데 사용됩니다.

5. APM 에이전트를 설정하는 경우 <DNT>**For language agents**</DNT> 아래의 엔드포인트 값도 복사하여 준비해야 합니다. 이 값은 코드 예제에서 `YOUR_TRACE_OBSERVER_HOST` 로 참조됩니다. 이는 [Traces 관찰자에게 데이터를 보내기 위해 우리 언어 에이전트를 구성하는 데](/docs/distributed-tracing/enable-configure/quick-start) 사용됩니다.
...
6. (선택 사항) 추적 관찰자가 작동하는지 확인하려면 샘플 추적 페이로드를 보내는 것이 좋습니다. [Trace API를](/docs/understand-dependencies/distributed-tracing/trace-api/introduction-trace-api) 직접 사용하는 경우 API 작동 방식을 배우는 데 도움이 됩니다.
   ...
   <CollapserGroup>
     ...
     >
       ...
       1. 데이터를 보고하려는 계정에 대한 <InlinePopover type="licenseKey"/>를 가져와서 준비하세요.
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  이를 수집 <InlinePopover type="licenseKey"/>으로 바꾸세요.
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  이를 이전 단계에서 복사한 <DNT>**For other integrations**</DNT> 엔드포인트 값으로 바꿉니다.
                </td>
              </tr>
            </tbody>
          </table>
       ...
       5. 테스트는 성공을 나타내는 `HTTP/1.1 202 Accepted` 을 반환해야 합니다. 그렇지 않은 경우 다음과 같은 일반적인 문제를 확인하십시오.
          ...
          * <DNT>
            ...
            [엔드포인트](#endpoints) 값을 사용했는지 확인하세요.
...
다른 유형의 서비스(예: <InlinePopover type="apm"/>에이전트)를 설정하는 동안 트레이스 관찰자를 이미 생성한 경우 트레이스 관찰자 생성 단계를 건너뛰고 [브라우저, 모바일 및 Lambda에 대한 초기 트레이스 관찰자 설정](#initial-config) 으로 바로 이동할 수 있습니다. 데이터 소스를 선택합니다.
...
4. <DNT>**Endpoints**</DNT> 드롭다운 메뉴에서 <DNT>**For other integrations**</DNT> 아래의 엔드포인트 값을 복사하여 준비하세요. 이 값은 코드 예제에서 `YOUR_TRACE_OBSERVER_URL` 로 참조됩니다. [트레이스 옵저버를](#sample-payload) 테스트하는데 사용됩니다.
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  이것을 <InlinePopover type="licenseKey"/>으로 바꾸세요.
                </td>
              </tr>
...
1. 트레이스 관찰자 앱(<DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infinite Tracing settings**</DNT>)으로 돌아갑니다.

2. 트레이스 관찰자의 경우 행 끝으로 이동하여 줄임표 아이콘 <DNT>**...**</DNT> 클릭한 다음 <DNT>**Edit mobile, browser, lambda sources**</DNT> 클릭합니다.
...
3. 다음 사항을 염두에 두고 포함할 데이터 소스를 선택하십시오.
   ...
   * 브라우저 앱이 여러 이름을 가진
     ...
     에이전트에 연결된 경우 이름은 하나만 선택할 수 있습니다.
