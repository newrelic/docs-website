...
| `processor_name`          | 끈  | `""`  | processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
| `num_traces`           | 정수 | `50000` | if rate\_limiter is enabled, it uses the num\_traces as max number of concurrent processing traces |
...
| `connection.pool_size`          | 정수 | `10 * cores` | Base number of socket connections                                                                                                            |
| `connection.min_idle_conns`     | 정수 | `0`          | Minimum number of idle connections which is useful when establishing new connection is slow. The idle connections are not closed by default. |
| `connection.max_idle_conns`     | 정수 | `0`          | Maximum number of connections allocated by the pool at a given time. 0 no limit                                                              |
| `connection.conn_max_idle_time` | 지속 | `30m`        | Maximum amount of time a connection may be idle. Should be less than server&apos;s timeout.                                                  |
| `connection.conn_max_lifetime`  | 지속 | `0m`         | Maximum amount of time a connection may be reused.                                                                                           |
...
| 매개변수                                    | 유형 | 기본값     | 설명                                                                                         |
| --------------------------------------- | -- | ------- | ------------------------------------------------------------------------------------------ |
| `connection.replica.read_only_replicas` | 부울 | `true`  | Enable routing read commands to replica nodes. Default is `true` for improved scalability. |
...
| **표준 생산량 (초당 15,000개 스팬)**  | 4-12  | 4-12   | Balanced           |
