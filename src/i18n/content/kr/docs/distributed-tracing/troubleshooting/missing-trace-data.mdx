...
* 샘플링 속도를 포함하여 트레이스 리소스의 개요를 보려면 [로그 상태 앱을](https://one.newrelic.com/dt-setup-assistant) 사용하세요. 이는 누락된 범위와 조각화된 트레이를 이해하는 데 도움이 될 수 있습니다. 이를 찾으려면 다음으로 이동하세요. <DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &gt; Apps &gt; Distributed tracing instrumentation status**</DNT>.
...
<CollapserGroup>
  <Collapser id="not-enabled" title="분산 추적이 활성화되지 않은 서비스로 인해 누락된 범위">
  ...
  <Collapser id="manual-instrumentation" title="누락된 앱/서비스는 수동 계측이 필요할 수 있습니다.">
    ...
    * <DNT>**Transactions not automatically instrumented**</DNT>. 자동으로 로그아웃되도록 하려면 사용 중인 뉴렐릭 에이전트에 대한 [호환성 및 요구 사항 문서를](/docs/apm/new-relic-apm/getting-started/introduction-apm) 읽어보세요. 로그가 자동으로 로그아웃되지 않거나 특정 활동의 로그를 추가하려는 경우 [사용자 정의 로그를](/docs/apm/agents/manage-apm-agents/agent-data/custom-instrumentation/#implementing) 참조하세요.
    * <DNT>**All Go applications**</DNT>. Go 에이전트는 다른 에이전트와 달리 코드를 수동으로 업로드해야 합니다. 지침은 [소스 추가를](/docs/agents/go-agent/instrumentation/instrument-go-transactions) 참조하세요.
    * <DNT>**A service doesn&apos;t use HTTP**</DNT>. 서비스가 HTTP를 통해 통신하지 않는 경우, 방사능 에이전트는 방사능 추적 헤더를 보내지 않습니다. 이는 웹이 아닌 일부 애플리케이션이나 메시지 대기열의 경우일 수 있습니다. 이 문제를 해결하려면, 호출하거나 호출한 것에 대한 [수신 거부 API](/docs/apm/distributed-tracing/enable-configure/enable-distributed-tracing#agent-apis) 사용하세요.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="advanced-infinite" title="무한 추적: 누락된 범위">
  ...
  <Collapser id="export" title="누락된 스팬이 내보내지지 않음">
  ...
  <Collapser id="sampling" title="샘플링 프로세스로 인한 누락된 스팬">
  ...
  <Collapser id="agent-limits" title="범위 제한이 초과되어 누락된 범위">
  ...
  <Collapser id="late-spans" title="스팬이 늦게 전송되어 스팬이 누락됨">
    스팬을 저장하려면 지난 24시간 이내에 전송해야 합니다. 스팬의 타임스탬프가 하루보다 오래된 경우 삭제됩니다. 이 문제는 시스템 간에 클럭 스큐(타이밍 차이)가 있거나 장기 실행 백그라운드 작업이 있을 때 자주 발생합니다.

    <Callout variant="important">
      [OpenTelemetry 프로토콜](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/protocol/otlp.md) (줄여서 OTLP)을 통해 전송된 스팬 중 24시간보다 긴 스팬은 [NRDB](/docs/data-apis/get-started/nrdb-horsepower-under-hood/)에 기록되지 않으며, 다음과 같은 NrIntegrationError가 발생합니다:
...
<CollapserGroup>
  <Collapser id="middleware-failure" title="미들웨어는 독점 New Relic 헤더를 인식하지 못합니다.">
  ...
  <Collapser id="proxy" title="중개자가 누락되었거나 추적 컨텍스트를 전달하지 않습니다.">
  ...
  <Collapser id="mixed-sourcees" title="혼합 소스의 범위를 함께 스티칭">
  ...
  <Collapser id="account-access" title="추적 세부정보가 난독화됨">
  ...
  <Collapser id="mismatched-trace-details" title="추적 목록 정보와 추적 세부 정보가 일치하지 않습니다.">
    추적 목록은 첫 번째 범위가 수신된 후 20분 창에 캡처된 추적 색인에 의해 생성됩니다.

    일반적으로 이것은 [늦은 스팬](#late-spans) 때문입니다.
  </Collapser>

  <Collapser id="long-traces-short-backend" title="백엔드 시간이 짧은 긴 추적">
...
<CollapserGroup>
  <Collapser id="spans-browser" title="브라우저 애플리케이션을 활성화한 후 스팬 및 트랜잭션 누락">
  ...
  <Collapser id="browser-spans-missing" title="브라우저 앱 최종 사용자 범위가 표시되지 않음">
  ...
  <Collapser id="orphan-browser-spans" title="브라우저 범위가 다른 범위에 연결되어 있지 않습니다.">
...
<CollapserGroup>
  <Collapser id="multiple-app-names" title="entity.name으로 검색하여 연결된 앱 이름을 찾지 못함">
  ...
  <Collapser id="supporting-open-telemetry" title="OpenTelemetry 지원">
    OpenTelemetry와의 통합에 대한 질문은 [지원 포럼](https://support.newrelic.com/s/)에 문의하십시오.
  </Collapser>
</CollapserGroup>
