...
<CollapserGroup>
  <Collapser
    ...
    title="무한 추적: 누락된 범위"
  >
    APM 에이전트가 추적 관찰자에게 충분히 빠르게 데이터를 쓸 수 없는 경우 `queue_size` 은 에이전트가 보유할 범위 수를 제한하는 추가 APM 에이전트 구성입니다. 에이전트에 대한 다음 예를 참조하세요.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Infinity_tracing.span_queue_size = 100000
          </td>
        </tr>
  ...
  >
    APM에 대한 표준 분산 추적은 [적응 샘플링](/docs/apm/distributed-tracing/getting-started/how-new-relic-distributed-tracing-works#sampling) 을 사용합니다. 즉, 서비스에 대한 호출 비율이 분산 추적의 일부로 보고됩니다. 서비스에 대한 특정 호출이 샘플링되도록 선택되지 않았을 수 있습니다.
  </Collapser>
  ...
  >
    추적 인덱스에서 캡처하려면 지난 20분 이내에 스팬을 보내야 합니다. 20분보다 오래되었지만 하루보다 최신인 스팬을 보내도 스팬 데이터는 계속 작성됩니다. 그러나 분산 추적 UI의 추적 목록을 제어하는 추적 색인으로 롤링되지 않습니다.

    범위에 하루보다 오래된 타임스탬프가 있으면 삭제됩니다. 이는 시스템 또는 오래 실행되는 백그라운드 작업 간에 클럭 스큐(시간 차이)가 있을 때 자주 발생합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    일부 APM 에이전트의 이전 버전은 브라우저 애플리케이션의 분산 추적과 호환되지 않습니다. 브라우저 응용 프로그램이 호환되지 않는 에이전트를 실행하는 APM 응용 프로그램에 AJAX 요청을 하는 경우 APM 에이전트는 해당 요청에 대한 트랜잭션 및 범위 데이터를 기록하지 않을 수 있습니다.
  ...
  >
    일부 APM 에이전트의 이전 버전은 브라우저 애플리케이션의 분산 추적과 호환되지 않습니다. APM 범위가 브라우저 응용 프로그램을 포함하는 추적에서 일관되게 누락된 경우 [분산 추적 요구 사항의 브라우저 데이터](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#requirements) 를 참조하고 모든 응용 프로그램에서 지원되는 버전의 APM 에이전트를 실행하고 있는지 확인하십시오.
...
<CollapserGroup>
  ...
  >
    OpenTelemetry와의 통합에 대한 질문은 [Explorers Hub](https://discuss.newrelic.com) 로 이동해야 합니다.
  </Collapser>
</CollapserGroup>
