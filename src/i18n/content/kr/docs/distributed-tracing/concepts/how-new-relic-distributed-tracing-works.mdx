...
<CollapserGroup>
  <Collapser id="trace-origin-sampling" title="언어 에이전트: 어댑티브 샘플링">
    ...
    트레이스 오리진 서비스는 기본적으로 분당 10개의 트레이스를 샘플링합니다. 일정 시간에 대한 대표적인 샘플을 얻기 위해 이러한 10개의 트레이스를 1분 동안 분산시킵니다. 정확한 샘플링 속도는 이전 분의 트랜잭션 수에 따라 다르며, 속도는 트랜잭션 처리량의 변화에 대응합니다
