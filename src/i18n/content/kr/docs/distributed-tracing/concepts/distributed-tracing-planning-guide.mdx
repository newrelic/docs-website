...
New Relic 분산 추적을 처음 사용하는 경우 분산 추적을 활성화하기 전에 다음을 읽는 것이 좋습니다.
...
[분산 추적을 활성화](/docs/understand-dependencies/distributed-tracing/get-started/introduction-distributed-tracing#enable) 하면 현재 사용 중인 일부 APM 기능에 영향을 미칠 수 있습니다. 이러한 변경 사항은 분산 추적이 활성화된 에이전트가 모니터링하는 응용 프로그램에만 영향을 미치며 계정 수준에서는 적용되지 않습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Monitor > Transactions > (트랜잭션 추적 선택)** : 분산 추적이 활성화되기 전의 트랜잭션 추적 UI를 연결된 트랜잭션에 대한 링크와 함께 표시합니다. 분산 추적이 활성화되면 서비스의 URL이 표시됩니다.
    </figcaption>
...
* 현재 APM 사용자인 경우 [APM 기능에 미치는 영향을](/docs/apm/distributed-tracing/getting-started/transition-guide-distributed-tracing) 참조하십시오.
...
* 대규모 모놀리식 서비스에 APM을 사용하는 경우 추적당 많은 하위 프로세스 범위가 있을 수 있으며 [APM 제한](/docs/understand-use-distributed-tracing-data#rules-limits) 으로 인해 예상보다 적은 추적이 발생할 수 있습니다. APM 에이전트 계측을 사용하여 중요하지 않은 데이터 보고를 비활성화하면 이 문제를 해결할 수 있습니다.

* 분산 추적은 요청 경로의 서비스에서 서비스로 [헤더 정보를 전파](/docs/understand-dependencies/distributed-tracing/get-started/how-new-relic-distributed-tracing-works#headers) 하여 작동합니다. 일부 서비스는 헤더를 자동으로 전파하지 않는 프록시 또는 기타 중개 서비스를 통해 통신할 수 있습니다. 이 경우 `newrelic` 헤더 값이 소스에서 대상으로 전파될 수 있도록 해당 프록시를 구성해야 합니다.

## 분산 추적 사용 [#enable]

APM 기능에 대한 영향을 알고 있고 롤아웃에 대해 생각했다면 분산 추적을 설정할 준비가 된 것입니다. [설정 옵션을](/docs/distributed-tracing/concepts/quick-start) 참조하십시오.