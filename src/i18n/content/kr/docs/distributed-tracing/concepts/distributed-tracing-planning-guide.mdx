...
또한 이전 버전의 APM 에이전트를 사용하고 분산 추적을 사용하지 않은 경우 분산 추적을 롤아웃하기 전에 [APM 기능에 미치는 영향을](#changes) 검토하는 것이 좋습니다.
...
* 대규모 모놀리식 서비스에 APM을 사용하는 경우 추적당 많은 하위 프로세스 범위가 있을 수 있으며 [APM 제한](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#rules-limits) 으로 인해 예상보다 적은 수의 추적이 발생할 수 있습니다. 중요하지 않은 데이터의 보고를 비활성화하는 APM 에이전트 계측을 사용하여 이 문제를 해결할 수 있습니다.
