...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="3">
      ...
      * **Focus on** \[초점]: 스팬에 오류나 이상이 있는 경우 각 유형에 대한 체크박스가 표시됩니다. 이 체크박스를 선택하면 폭포 보기에서 다른 모든 유형의 범위가 축소되고 색상으로 구분된 오류나 비정상적인 범위만 표시됩니다. 이는 문제가 있는 범위를 찾는 빠른 방법입니다.<img style={{ width: "70%",align: "left" }} title="Screenshot showing the color coding of focus on" alt="Screenshot showing the color coding of focus on" src={distributedtracingFocusonExample}/>
