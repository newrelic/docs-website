...
2. **Group traces by** 에서 다음 옵션 중 하나를 선택합니다.
   ...
   * **서비스 진입 스팬**: APM에서 현재 보고 있는 서비스의 스팬 이름별로 그룹화합니다. 예를 들어 서비스 A가 서비스 B를 호출하고 서비스 B가 서비스 C를 호출하는 추적의 경우

     <InlinePopover type="apm"/>

     이 그룹화를 선택하면 추적이 서비스 B 스팬 이름으로 표시됩니다. 서비스의 추적에 여러 스팬이 있는 경우 이 그룹화 옵션은 해당 서비스의 첫 번째 진입점을 사용합니다.
