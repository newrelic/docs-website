...
트레이스 목록에서 트레이스를 선택하면 상단에 [엔터티 맵이](#entity-map) 표시되거나 [타임라인](#timeline-view) 보기와 [대기 시간](#latency-view) 보기 간에 전환할 수 있습니다. 이 상단 섹션 아래에는 해당 트레이스의 범위를 표시하는 [폭포 보기가](#waterfall-view) 있습니다.
...
### 대기 시간 보기 [#latency-view]

대기 시간 보기는 추적의 각 엔터티가 기여하는 대기 시간과 엔터티당 오류 범위 수를 보여줍니다. 엔터티에 의해 발생하는 대기 시간은 하나 이상의 프로세스가 실행 중이지만 외부 또는 데이터베이스 호출을 **하지 않는** 벽시계 시간입니다.
...
* **집중**: 스팬에 오류나 이상이 있는 경우 이러한 각 유형에 대한 확인란이 표시됩니다. 이 체크박스를 선택하면 폭포 보기가 다른 모든 유형의 범위를 축소하고 색상으로 구분된 오류 또는 비정상적인 범위만 표시합니다. 이는 문제가 있는 스팬을 찾는 빠른 방법입니다.
