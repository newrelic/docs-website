...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        **필수의.** Trace API에는 <InlinePopover type="licenseKey" /> 가 필요합니다. 이것이 헤더와 쿼리 매개변수로 모두 제공되는 경우 값이 일치해야 합니다.
      </td>
    </tr>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              인증 오류. 유효하지 않은 <InlinePopover type="licenseKey" /> 가 있거나 Trace API를 사용하는 데 필요한 권한이 없는 경우 발생할 수 있습니다.
            </td>
          </tr>
...
* `NrIntegrationError` 이벤트(비동기). JSON 페이로드 오류 또는 기타 의미론적 오류는 <InlinePopover type="licenseKey" /> 가 요청과 연결된 계정에 저장된 [`NrIntegrationError` 이벤트](/docs/telemetry-data-platform/manage-data/nrintegrationerror) 를 통해 비동기식으로 신호를 보냅니다. 이 유형의 모든 오류에 대해 속성 `newRelicFeature` 은 `Distributed Tracing` 이고 `requestId` 은 엔드포인트 응답의 `requestId` 입니다.
