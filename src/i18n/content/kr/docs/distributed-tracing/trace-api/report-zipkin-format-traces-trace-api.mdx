...
데이터를 보고하려는 계정의 <InlinePopover type="licenseKey" />를 가져옵니다.

아래에서 curl 요청을 실행합니다. 이에 대한 참고 사항:

* 라이선스 키 자리 표시자를 라이선스 키로 바꿉니다.
* 무한 추적을 사용하는 경우 표준 엔드포인트 대신 [YOUR\_TRACE\_OBSERVER\_URL](/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer#ui-endpoints) 값을 사용하십시오.
...
Infinite Tracing을 사용 중이거나 EU 또는 JP 지역 뉴렐릭 계정이 있는 경우 [엔드포인트](/docs/distributed-tracing/trace-api/trace-api-general-requirements-limits#requirements) 가 다릅니다.
