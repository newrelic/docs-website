...
<CollapserGroup>
  <Collapser id="check-policy-status" title="계정에서 Live URL 생성 정책 상태 확인">
    인증 도메인 관리자는 특정 계정의 <DNT>**Live URL Creation**</DNT> 정책의 현재 상태를 확인할 수 있습니다. [NerdGraph API 탐색기를](https://one.newrelic.com/nerdgraph-graphiql) 사용하여 다음과 같이 [그래프 API 호출을](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) 구성합니다. `1, 2, 3` 실제 계정 ID로 바꾸세요. 계정 ID를 제공하지 않으면 호출은 승인된 계정의 상태를 반환합니다. 최대 100개 계정에 대해 동시에 쿼리를 실행할 수 있습니다.
