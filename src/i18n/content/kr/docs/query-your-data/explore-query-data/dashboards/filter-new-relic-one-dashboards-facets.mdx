...
## Keep facet links tied to the current dashboard [#selfentity]

By default, facet linking that filters the current dashboard stores that dashboard&apos;s GUID. If you duplicate the dashboard, the copy&apos;s facet links still point back at the original, not the duplicate. You can use the `{{selfEntity}}` variable instead, so facet linking always resolves to whichever dashboard the widget is on, including any duplicates.

* For existing dashboards: Disable facet linking in the widget used for filtering and apply the changes. Enable facet linking again in the same widget and apply the changes. This will replace the dashboard GUID with the `{{selfEntity}}` variable.

* 새 대시보드의 경우: 패싯 연결을 활성화하면 `{{selfEntity}}` 변수를 자동으로 사용합니다.

* For Terraform or JSON dashboards:

  * For existing dashboards: Remove the hardcoded `linkedEntityGuids` at the widget level and replace it with `{{selfEntity}}` in the raw configuration.
  * 새 대시보드의 경우: 패싯 기준으로 사용할 위젯의 원시 설정에 이 줄을 추가하십시오:
...
이 기능에 대한 자세한 내용은 [패싯 필터링에 대한 지원 포럼 게시물](https://support.newrelic.com/s/hubtopic/aAX8W0000008aPj/facet-linking-in-new-relic-one-now-you-can-use-facets-to-filter-your-new-reli)을 참조하십시오.
