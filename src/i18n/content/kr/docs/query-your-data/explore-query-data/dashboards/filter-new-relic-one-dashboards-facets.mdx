...
## 패싯 링크를 현재 대시보드에 연결된 상태로 유지하십시오. [#selfentity]

기본적으로 현재 대시보드를 필터링하는 패싯 연결은 해당 대시보드의 GUID를 저장합니다. 대시보드를 복제하는 경우, 복사본의 패싯 연결은 복제본이 아닌 원본을 계속 가리킵니다. 대신 `{{selfEntity}}` 변수를 사용할 수 있으며, 이 경우 패싯 연결은 복제본을 포함하여 위젯이 있는 대시보드로 항상 연결됩니다.

* 기존 대시보드의 경우: 필터링에 사용되는 위젯에서 패싯 연결을 비활성화하고 변경 사항을 적용하십시오. 동일한 위젯에서 패싯 연결을 다시 활성화하고 변경 사항을 적용하십시오. 이것은 대시보드 GUID를 `{{selfEntity}}` 변수로 대체합니다.

* 새 대시보드의 경우: 패싯 연결을 활성화하면 `{{selfEntity}}` 변수를 자동으로 사용합니다.

* Terraform 또는 JSON 대시보드의 경우:

  * 기존 대시보드의 경우: 위젯 수준에서 하드코딩된 `linkedEntityGuids`을(를) 제거하고 원시 설정에서 `{{selfEntity}}` (으)로 교체하십시오.
