...
템플릿 변수 기능은 현재 제한된 미리 보기로 제공되며 일부 고객만 사용할 수 있습니다. 이 기능에 대한 액세스 권한이 있으면 대시보드 상단에 **변수 추가** 가 표시됩니다.

템플릿 변수는 사용자 정의 대시보드용 위젯을 만드는 컨텍스트에서만 사용할 수 있습니다. (NRQL 쿼리에서 변수를 사용하려면 [NRQL 변수](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#with-as-nrql-var) 를 참조하십시오.)

템플릿 변수는 대시보드 컨텍스트에서만 사용할 수 있습니다. 이러한 이유로 표준 쿼리에서와 같이 작동하지 않는 몇 가지 쿼리 관련 기능이 있습니다. 예를 들어:

* [**쿼리 보기** 옵션](/docs/new-relic-solutions/new-relic-one/ui-data/basic-ui-features/#data-analysis) 을 사용하면 쿼리에 변수가 포함되지 않습니다.
* 차트 공유 또는 차트 링크 옵션이 없습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        선택 과목. 이것은 대시보드가 필터링할 기본값입니다. 예를 들어 위의 `country` 쿼리를 사용한 경우 `ES` 을 기본값으로 입력하면 대시보드가 자동으로 해당 값으로 필터링됩니다.
      </td>
    </tr>
  </tbody>
</table>
...
* 쿼리는 `unique` 구문을 사용해야 합니다. 예를 들어: `From PageAction select uniques(countryCode)`
