...
다음은 결과 위젯의 예입니다(오른쪽). 왼쪽에는 `country` 드롭다운이 있습니다.<img title="Country template variable example" alt="Country template variable example" src={dashboardsCountryVariableExample}/>
