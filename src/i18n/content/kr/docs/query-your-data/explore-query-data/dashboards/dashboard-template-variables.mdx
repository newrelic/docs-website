...
<DNT>**Include variable**</DNT> 토글을 사용하면 원하는 값을 설정하고 대시보드에 해당 값을 포함하거나 제외할 수 있습니다. 자세한 내용은 [템플릿 변수](#create-variables) 섹션을 참조하세요.
...
템플릿 변수는 보기 모드에서 변수 표시줄에서 숨길 수 있지만, 내부적으로는 정상적으로 작동합니다. 편집 모드에서 숨겨진 변수는 다음과 같이 표시됩니다. <Icon name="fe-eye-off" /> 아이콘을 통해 대시보드 편집자가 해당 항목을 식별하고 관리할 수 있습니다.

다음과 같은 변수를 숨기는 것을 고려해 보세요.

* [중첩 변수](/docs/query-your-data/explore-query-data/dashboards/nested-variables)와 같은 요소를 추가하여 변수 표시줄을 복잡하게 만드세요.
* 시청자 입력 없이도 강력한 대시보드 로직을 구현할 수 있습니다.
* 다른 변수로부터 값을 자동으로 도출합니다.

가시성을 구성하려면 [템플릿 변수를 정의할](#step1) 때 <DNT>**Hide on variables bar**</DNT> 옵션을 사용하십시오. 프로그래밍 방식으로 액세스하려면 [NerdGraph 대시보드 API](/docs/apis/nerdgraph/examples/nerdgraph-dashboards) 에서 `hiddenOnVariablesBar` 필드를 사용하세요.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            선택 과목. 보기 모드에서 변수 표시줄에 해당 변수가 표시되지 않도록 하려면 이 토글을 활성화하십시오. 해당 변수는 백그라운드에서 정상적으로 계속 작동합니다. 편집 모드에서 숨겨진 변수는 다음과 같이 표시됩니다. <Icon name="fe-eye-off" /> 상. 자세한 내용은 [변수 표시 및 숨기기를](#variable-visibility) 참조하세요.
          </td>
        </tr>
      </tbody>
    </table>
