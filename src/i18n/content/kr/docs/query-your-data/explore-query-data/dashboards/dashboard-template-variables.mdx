      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            * **String** \[문자열]: 숫자가 아닌 텍스트 값에 사용합니다.
            * **Number** \[숫자]: 숫자 값에 사용합니다.
...
다음은 결과 위젯의 예입니다. 오른쪽에는 `country` 드롭다운 메뉴가 왼쪽에 있습니다.<img title="Country template variable example" alt="Country template variable example" src={dashboardsCountryVariableExample}/>
...
<CollapserGroup>
  ...
  >
    ...
    그러면 지난 30분 동안의 값이 나옵니다. 타임 피커 값을 **Default** \[기본값] 으로 설정하고 쿼리에 `SINCE` 절이 있는 경우 최신 값을 사용하여 쿼리를 실행합니다.
