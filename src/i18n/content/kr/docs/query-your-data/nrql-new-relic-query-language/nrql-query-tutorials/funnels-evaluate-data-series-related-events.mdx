...
이 예에서는 브라우저 모니터링 [`PageView`](/attribute-dictionary/?event=PageView) 이벤트 및 해당 속성을 쿼리합니다. 등록에서 계정 업그레이드까지 진행된 고유한 브라우저 세션을 쿼리합니다. `AS` 키워드로 표시되는 각 단계에 대한 레이블이 포함됩니다.
...
<figcaption>
  계정에 등록하고 기능 비교 페이지를 방문하고 업그레이드한 사용자 수를 표시하는 NRQL 유입경로 쿼리의 예입니다.
</figcaption>
