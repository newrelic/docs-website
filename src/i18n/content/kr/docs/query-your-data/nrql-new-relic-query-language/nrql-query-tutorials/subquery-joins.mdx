...
* 조인된 하위 쿼리는 계속해서 기본 [`LIMIT`](#sel-limit) 10과 최대 `LIMIT` 5000을 가집니다. 외부 쿼리의 `LIMIT` 는 내부 쿼리에 영향을 미치지 않습니다.
...
<CollapserGroup>
  ...
  >
    ...
    팁: 외부 쿼리에 조인된 결과 수를 최대화하려면 내부 하위 쿼리에 LIMIT MAX를 포함합니다. 현재 최대 결과 제한은 5000행입니다.
  </Collapser>
