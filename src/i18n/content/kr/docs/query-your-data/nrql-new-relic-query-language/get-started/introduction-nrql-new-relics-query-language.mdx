...
* 문제 해결 또는 비즈니스 분석에 대한 질문에 답하기 위해
* 새 차트를 생성하기 위해
* 뉴렐릭 데이터의 API 쿼리를 만들기 위해 (예: [NerdGraph](/docs/apis/graphql-api/tutorials/nerdgraph-graphiql-nrql-tutorial) API 사용)
...
* 다음을 포함한 모든 뉴렐릭 제품의 [이벤트 데이터](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data):
...
특정 데이터 유형에 사용할 수 있는 [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)을 보려면 `FROM DATA_TYPE SELECT`를 입력하고 스페이스바를 누릅니다. 인터페이스는 사용 가능한 속성을 제안합니다. 예:
