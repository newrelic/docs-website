...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        기본적으로 보고되는 이벤트에는 영숫자, 콜론( `:` ) 및 밑줄( `_` )이 포함된 이름이 있습니다. 속성 이름에는 해당 문자와 마침표( `.` )가 포함될 수 있습니다. 기본적으로 보고되는 이름은 문자로 시작합니다. 이러한 지침을 따르지 않는 커스텀 이름은 NRQL 쿼리에서 백틱 기호로 묶어야 합니다. 예를 들어:
