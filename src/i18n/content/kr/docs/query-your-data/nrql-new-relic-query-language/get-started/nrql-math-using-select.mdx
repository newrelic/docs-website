...
import queriesnrqlNRQLMathClamp from 'images/queries-nrql_screenshot-crop_NRQL-math-clamp.png'

import queriesnrqlNRQLMathFloorCeiling from 'images/queries-nrql_screenshot-crop_NRQL-math-floor-ceiling.png'
...
`pow(n, m)` _n_ 을 _m_ 으로 거듭제곱한 값을 계산합니다. (즉 `n * n * ... * n` , _n개의_ 복사본 _m_ 개 포함)
...
`pow(n, m)` _n_ 을 _m_ 으로 거듭제곱한 값을 계산합니다. (예: `n * n * ... * n` , _n_ 의 _m_ 개 사본 포함)
