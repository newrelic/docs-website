  ...
  >
    ...
    * Variable names cannot include the `%` symbol.
    ...
    <CollapserGroup>
      ...
      >
        In this example a NRQL variable, `unitPrice`, is used to create another variable, `unitPriceNum`, converting the extracted string into a number. The variables are then used in the `SELECT`, `WHERE` and `FACET` clauses.
