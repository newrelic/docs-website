...
<CollapserGroup>
  ...
  >
    ...
    * 에이전트 API를 통해 보고되는 커스텀 이벤트<InlinePopover type="apm"/>
      ...
      이 절은 <InlinePopover type="apm"/>에이전트가 너무 많은 [이벤트를](/docs/using-new-relic/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#event-data) 보고하여 종종 [하베스트 주기](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#harvest-cycle) 보고 제한을 초과하는 경우에 유용합니다. 이 경우 에이전트는 이벤트 샘플링을 시작합니다.
  ...
  >
    ...
    * 결합된 하위 쿼리는 계속해서 기본 [`LIMIT`](#sel-limit) 10과 최대 `LIMIT` 5000을 가집니다. 외부 쿼리의 `LIMIT`는 내부 쿼리에 영향을 미치지 않습니다.
...
<CollapserGroup>
  ...
  >
    ...
    `apdex()` 함수가 반환한 Apdex 점수는 실행 시간만 기준으로 합니다. 이는 <InlinePopover type="apm"/>오류를 고려해 넣지 않습니다. 트랜잭션에 오류가 있지만 [Apdex T](/docs/apm/new-relic-apm/getting-started/glossary#apdex_t) 이하에서 완료되면, 해당 트랜잭션은 `apdex ()` 함수에 의해 [만족스러운](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#bullet-satisfied) 것으로 평가됩니다.
  ...
  >
    ...
    * 시간 경과에 따른 평균 단일 요율 값을 표시하는 빌보드를 생성하려면 [`TIMESERIES`](#sel-timeseries)를 생략합니다.
      ...
      지난 6시간 동안 10분당 <InlinePopover type="apm"/>트랜잭션의 처리 속도를 보여주는 선형 차트를 생성하는 기본 쿼리는 다음과 같습니다.
