...
* `LIKE`/`NOT LIKE` 와 함께 와일드카드를 사용하여 포함/제외 검색하거나 결과를 `IN` 목록 또는 `NOT IN` 목록으로 제한
...
<Steps>
  <Step>
    첫 [번째 NRQL 튜토리얼을](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-tutorial/) 완료한 후 `count()`, `average()`, `sum()`, `max()` 및 `min()` 를 사용하여 의미 있는 방식으로 데이터를 변환했습니다. 하지만 고유한 값을 찾고 계산하려면 어떻게 해야 할까요? 지정된 시간 범위 동안 속성에 대해 기록된 고유 값의 수를 확인하려면 `uniqueCount() function` 를 사용할 수 있습니다. 검사하려는 속성을 인수로 제공하기만 하면 됩니다. 예를 들어, 다음은 모든 고유 공개 API 호출을 표시하는 쿼리입니다.
...
`latest()`/`earliest()` 함수는 희소 데이터를 처리하거나 보고가 시작되거나 중지된 시점을 조사할 때(New Relic이 여전히 데이터를 저장한다고 가정할 때) 특별한 값을 갖습니다. `percentage()` 함수는 전체 결과 세트와 비교하여 한정자와 일치하는 이벤트의 비율을 표시할 수 있습니다. 마지막으로 모든 최종 사용자 트랜잭션의 90%가 제공된 기간을 초과하도록 목표를 설정하여 `percentile()` 핵심 성과 지표로 사용할 수 있습니다.

데이터 제어 능력이 훨씬 더 발전하고 있습니다! 다음 단계에서는 NRQL을 사용하여 기본적인 수학을 수행하는 방법을 배우게 됩니다.
...
<Steps>
  ...
  이는 순전히 미학적으로 보일 수 있지만 세부적인 대시보드를 구축할 때는 데이터에 명확하게 레이블을 지정하는 것이 중요합니다. 이를 통해 위젯, 빌보드, 꺾은선형 차트 또는 표를 보는 모든 사람에게 모호함이 전혀 없습니다.
...
<Steps>
  ...
  <Step>
    때로는 epoch(unix) 시간으로 이벤트 시간을 받을 수도 있습니다. 편리하게도`SINCE` 및 `UNTIL` 절과 함께 에포크 타임스탬프를 사용할 수 있으므로 이러한 값을 다른 날짜 형식으로 수동으로 변환할 필요가 없습니다.
...
데이터를 파헤치는 일은 언제나 즐겁습니다! 시간 기반 코호팅은 특정 분, 시간, 일 또는 주에 발생하는 문제를 노출하는 쉬운 방법입니다. New Relic에 어떤 데이터를 보내든 NRQL을 사용하면 원하는 대로 데이터를 분할하고, 정리하고, 시각화할 수 있습니다.
