...
주어진 시간 동안 제한에 도달하면 다음 항목에 제한이 적용됩니다.

* 사용자가 실행하는 맞춤 쿼리
* 쿼리를 실행하는 대시보드의 위젯
* 사용자가 시작한 작업으로 데이터를 가져오려고 하는 New Relic의 요약 페이지
* UI에서 고객이 시작한 작업으로 인해 로드되는 New Relic UI의 선별된 보기. 예: APM **요약** , APM **트랜잭션** , **분산 추적** , 브라우저 모니터링 **요약** 및 NRDB에서 원격 측정 정보를 가져오는 기타 페이지

이는 인시던트의 경우 경고 조건 평가 및 알림에 영향을 주지 않습니다.그러나 알림의 링크에서 리디렉션되는 New Relic 영역에서는 예외가 발생할 수 있습니다.기간이 지난 후 쿼리 볼륨이 제한 아래로 떨어지면 제한이 자동으로 제거됩니다.
