...
1. New Relic에서 [새 Insights 쿼리 키를 만듭니다](/docs/apis/intro-apis/new-relic-api-keys/#insights-query-key) .
   ...
   <Callout variant="important">
     참고: Grafana에서는 이를 사용자 정의 <DoNotTranslate>**X-Query-Key**</DoNotTranslate> HTTP 헤더(아래 7단계 참조)에 넣어야 하지만 이는 뉴렐릭 쿼리 키와 동일한 엔터티입니다.
   </Callout>

2. [Grafana](https://grafana.com/login) <DoNotTranslate>**Home**</DoNotTranslate> 화면에서 <DoNotTranslate>**Configuration > Data Sources**</DoNotTranslate> 로 이동하여 <DoNotTranslate>**Add data source**</DoNotTranslate> 클릭합니다.

3. <DoNotTranslate>**Time series databases**</DoNotTranslate> 옵션 아래의 <DoNotTranslate>**Add data source**</DoNotTranslate> 화면에서 <DoNotTranslate>**Prometheus**</DoNotTranslate> 선택합니다.

4. 새 Prometheus 데이터 소스에 사용할 <DoNotTranslate>**Name**</DoNotTranslate> 를 입력합니다.

5. Prometheus 쿼리의 기본 데이터 소스로 사용할지 여부에 따라 <DoNotTranslate>**Default**</DoNotTranslate> 토글을 켜기 또는 끄기 위치로 설정합니다.

   * 꺼짐: 기본 데이터 소스가 아닙니다.
   * 켜짐: 기본 데이터 소스입니다.

6. 올바른 <DoNotTranslate>**URL**</DoNotTranslate>) 입력하세요.
...
7. <DoNotTranslate>**Custom Headers**</DoNotTranslate> 아래에서 <DoNotTranslate>**Add Header**</DoNotTranslate> 선택합니다. <DoNotTranslate>**Header**</DoNotTranslate> 이름을 <DoNotTranslate>**X-Query-Key.**</DoNotTranslate> 으로 설정합니다. <DoNotTranslate>**Value**</DoNotTranslate> 의 경우 1단계에서 생성한 쿼리 키를 입력합니다.

8. <DoNotTranslate>**Save & Test**</DoNotTranslate> 을(를) 클릭합니다.

<Callout variant="tip">
  그래프가 연결된 선이 아닌 점 그룹으로 나타나는 경우 대신 선을 표시하도록 그래프 스타일을 변경할 수 있습니다. 이렇게 하려면 Grafana의 <DoNotTranslate>**[Graph panel](https://grafana.com/docs/grafana/latest/panels/visualizations/graph-panel/)**</DoNotTranslate> 으로 이동하여 <DoNotTranslate>**Stacking and null value > connected**</DoNotTranslate> 선택합니다.
</Callout>
...
<CollapserGroup>
  ...
  >
    [쿼리 키](/docs/apis/get-started/intro-apis/types-new-relic-api-keys#x-query-key) 검증은 뉴렐릭으로 인증하고 지표가 포함된 계정을 식별하는 데 필요합니다. Grafana는 이를 <DoNotTranslate>**X-Query key**</DoNotTranslate>이라고 부르지만 뉴렐릭 UI 에서는 <DoNotTranslate>**Query key**</DoNotTranslate> 로 표시됩니다.
