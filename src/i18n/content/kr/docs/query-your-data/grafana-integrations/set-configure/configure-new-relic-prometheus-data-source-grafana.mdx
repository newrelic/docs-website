...
1. New Relic에서 [새 Insights 쿼리 키를 만듭니다](/docs/apis/intro-apis/new-relic-api-keys/#insights-query-key) .
   ...
   <Callout variant="important">
     Grafana에서는 이를 사용자 지정 <DNT>**X-Query-Key**</DNT> HTTP 헤더에 넣어야 하지만(아래 7단계 참조), 뉴렐릭 쿼리 키와 동일한 엔티티입니다.

     계정이 **JP 리전**에 있는 경우 Insights 쿼리 키는 지원되지 않습니다. 대신 [사용자 API 키](/docs/apis/intro-apis/new-relic-api-keys/#user-key) 를 생성하고 7단계에서 <DNT>**api-key**</DNT> 헤더와 함께 사용하세요.
   </Callout>
...
7. <DNT>**Custom Headers**</DNT>에서 <DNT>**Add Header**</DNT>을(를) 선택합니다. 미국 및 EU 계정의 경우 <DNT>**Header**</DNT> 이름을 <DNT>**X-Query-Key**</DNT> (으)로 설정하고 1단계의 Insights 쿼리 키를 입력합니다. 일본 계정의 경우 <DNT>**Header**</DNT> 이름을 <DNT>**api-key**</DNT> (으)로 설정하고 1단계의 사용자 API 키를 입력합니다.
...
<CollapserGroup>
  <Collapser id="" title="X-Query-Key(쿼리 키)">
    ...
    **JP 리전 예외**: JP 리전은 Insights 쿼리 키를 지원하지 않습니다. <DNT>**X-Query-Key**</DNT> 대신 헤더 이름이 <DNT>**api-key**</DNT> 인 [사용자 API 키](/docs/apis/intro-apis/new-relic-api-keys/#user-key) 를 사용하세요.
  </Collapser>

  <Collapser id="" title="X-프로메테우스 전용">
    <Callout variant="important">
  ...
  <Collapser id="" title="X-Prometheus-서버">
...
1. <DNT>**Configuration &gt; Data Sources**</DNT> 으)로 이동합니다.
2. 삭제할 데이터 소스를 클릭합니다.
3. 페이지 하단의 <DNT>**Delete**</DNT> 버튼을 클릭하세요.