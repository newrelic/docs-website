...
## 배포를 위한 웹훅 알림 구성 [#configure-webhook]
...
배포 알림이 활성화되고 배포를 생성하면 웹훅은 `application/x-www-form-urlencoded` 유형의 페이로드가 포함된 `POST` 요청을 수신합니다. 키와 값은 `&` 로 구분된 키-값 튜플로 인코딩되며 키와 값 사이에는 `=` 기호가 있습니다. 키와 값 모두에서 영숫자가 아닌 문자는 URL로 인코딩됩니다.
