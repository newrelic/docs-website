...
<CollapserGroup>
  <Collapser
    ...
    title="필수 배포 속성"
  >
    배포와 같은 변경 사항을 추적하려면 다음을 설정해야 합니다.<table>
      <thead>
        <tr>
...
GraphQL 및 NRQL 쿼리를 사용하여 배포와 관련된 변경 사항에 대한 데이터를 가져올 수 있습니다. 몇 가지 쿼리를 시도하려면 아래 예를 참조하십시오. [NerdGraph 엔터티 API 자습서](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/) 에서 GraphQL 쿼리에 대해 알아볼 수도 있습니다.

<CollapserGroup>
  <Collapser
    className="freq-link"
    id="query-deployments"
    title="GraphQL을 사용하여 배포 쿼리"
  >
    아래 예제는 배포 및 해당 속성에 대한 정보를 얻는 방법을 보여줍니다. `startTime` 및 `endTime` 에 대한 Unix epoch 타임스탬프(밀리초 단위)와 함께 `guid` 에 대한 고유한 값을 삽입해야 합니다.
  ...
  <Collapser
    className="freq-link"
    id="key-signals"
    title="NRQL을 사용하여 주요 신호 및 배포 마커가 있는 차트를 렌더링합니다."
  >
    GraphQL을 사용하여 마커를 생성한 후 [쿼리 빌더](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/) 를 사용하여 시계열 차트를 생성하고 일정 기간 동안 원격 측정 신호에 대한 곡선을 그리고 마커로 배포를 렌더링할 수 있습니다.

    다음은 일부 차트를 생성하기 위해 쿼리 빌더에서 실행할 수 있는 몇 가지 NRQL 예제입니다.
