    ...
    배포와 같은 변경 사항을 추적하려면 다음을 설정해야 합니다.<table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            버전 속성 값은 무엇이든 가능하지만 빌드 프로세스 중에 설정된 버전 번호를 사용하는 것이 좋습니다. 이를 수행하는 방법에 대한 예를 보려면 아래 

            [Jenkins 배포 플러그인 문서를](#jenkins-markers)

             참조하세요. 이미 New Relic에 버전 번호를 보고한 경우 

            <InlinePopover type="mobile"/>

            또는 다른 방법으로 여기에서 동일한 방법을 사용하여 버전별로 데이터를 패싯하고 상관시킬 수 있는 향후 기능을 설정하는 것이 좋습니다.
          </td>
        </tr>
      </tbody>
    </table>
