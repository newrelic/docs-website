...
1. Jenkins를 열고 **Manage Jenkins** > **Plugins**로 이동합니다.
...
2. **Available plugins**를 클릭한 다음 `newrelic-deployment-notifier`를 검색합니다. **Install**을 클릭합니다.
...
1. Jenkins 작업 설정에서 **Post-build Actions 섹션**으로 이동하여, **Add post-build action**을 클릭한 다음 **New Relic Deployment Notifications**를 선택합니다.
   ...
   <table>
     ...
     <tbody>
       ...
       <tr>
         ...
         <td>
           배포를 수행하는 사용자입니다. **팁:** Jenkins 사용자에 관한 환경 변수를 갖기 위해 [Build User Vars 플러그인](https://wiki.jenkins.io/display/JENKINS/Build+User+Vars+Plugin)을 설치하고, `${BUILD_USER}` 같이 이 필드에서 사용할 수 있습니다.
