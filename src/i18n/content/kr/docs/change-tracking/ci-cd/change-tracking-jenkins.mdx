...
변경 추적 배포를 기록하려면 특별한 Jenkins 플러그인이 필요합니다. Jenkins 인스턴스에서 플러그인을 쉽게 추가하거나 제거할 수 있습니다. Jenkins 관리자로 다음 단계를 완료하십시오.

### 플러그인 추가 [#add-plugin]

플러그인을 추가하려면 다음을 완료하십시오.

1. **Jenkins 관리** 를 클릭합니다.
...
4. **플러그인 배포** 아래에서 **파일 선택** 을 클릭하고 미리 보기용으로 제공한 `newrelic-deployment-notifiier.hpi` 파일을 업로드합니다.

### 플러그인 제거 [#remove-plugin]

다음 단계에 따라 플러그인 관리자에서 플러그인을 제거할 수 있습니다.
