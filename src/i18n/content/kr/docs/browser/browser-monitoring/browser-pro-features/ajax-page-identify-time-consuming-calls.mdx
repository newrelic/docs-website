...
3. 성능 또는 페이지 보기별로 특정 AJAX 호출에 대한 세부 정보를 검토하려면 링크를 클릭하십시오.

<img
  title="New Relic browser monitoring: AJAX summary"
  alt="A screenshot of the AJAX summary UI in New Relic"
  src={nrOneAjaxBrowser}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > AJAX** : 사이트에서 웹페이지의 일부를 업데이트하는 AJAX 호출에 시간이 오래 걸리거나 실패하여 문제를 식별합니다.
</figcaption>
...
<CollapserGroup>
  ...
  >
    ...
    * 네 가지 기간( **애플리케이션 시간** , **대기열 시간** , **네트워크 시간** 및 **콜백 시간** ): 이러한 AJAX 호출은 애플리케이션 간 추적 헤더를 제공하는 APM 에이전트가 모니터링하는 앱에 대해 수행됩니다.
