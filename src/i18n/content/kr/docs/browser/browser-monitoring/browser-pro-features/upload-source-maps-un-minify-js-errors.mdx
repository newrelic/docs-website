...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > [JS 오류](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time)** 로 이동한 다음 JS 오류 그룹을 선택합니다. ( **Errors without a stack trace** 선택하지 마십시오.)
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="웹팩으로 소스 맵 생성"
  >
    **[Webpack](https://webpack.github.io/docs/configuration.html#devtool) 을 사용하여 소스 맵 생성:** 프로덕션 웹팩 구성에서 config.devtool 속성에 대해 `source-map` 을 지정하기만 하면 됩니다. config.output의 sourceMapFilename 속성은 선택 사항이며 기본값은 `[name].js.map` 입니다.
