...
우리의 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)소스 맵 업로드를 지원합니다.

소스 맵 지원은 주로 축소된 JavaScript를 "디코딩"하는 데 유용합니다. 축소된 JavaScript로 인해 브라우저의 **Errors** \[오류] 페이지에 대부분 쓸모없는 오류 스택 추적이 발생합니다. 소스 맵을 업로드하면 이러한 오류가 코드 라인에 대한 유용한 참조와 함께 이해하기 쉬운 스택 추적으로 변환됩니다. 이 기능은 번들 또는 트랜스파일된 JavaScript 코드에도 유용할 수 있습니다.
...
2. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > Errors** 한 다음 오류 그룹을 클릭하세요. ( **Errors without a stack trace** \[스택 추적 없이 오류라고] 표시된 그룹을 선택하지 마십시오.)
3. **Stack trace** \[스택 추적] 섹션에서 **Upload source map** \[업로드 소스 맵을] 클릭합니다.
