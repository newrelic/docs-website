...
[브라우저 모니터링을 활성화](#enable)하면<InlinePopover type="apm"/> 에이전트를 사용해 자동으로 브라우저 모니터링 JavaScript 스니펫을 삽입할 수 있습니다. 이는 APM으로 이미 모니터링 중인 앱을 위해 에이전트를 설치하는 가장 쉬운 방법입니다.
...
* <InlinePopover type="apm"/>
  ...
  표준 서버 측 앱과 같이 브라우저 앱과 밀접하게 연결되지 않은 앱(예: 클라이언트 측 앱이 REST API 백엔드와 통신하는 경우)
...
뉴렐릭의 <InlinePopover type="apm"/>에이전트는 페이지 로드 타이밍에 필요한 JavaScript로 웹 페이지를 계측할 수 있습니다. APM 에이전트의 API를 사용하여 JavaScript 스니펫을 웹 페이지에 수동으로 추가하는 경우 계측 스니펫을 최대한 맨 위에 삽입해야 합니다. 그래야 브라우저의 [AJAX 호출](/docs/browser/new-relic-browser/dashboard-details/ajax-dashboard) 및 [JavaScript 오류](/docs/browser/new-relic-browser/dashboard-details/javascript-errors-dashboard)에 대한 자세한 정보를 활용할 수 있습니다.
