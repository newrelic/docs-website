...
방금 애플리케이션을 구성한 경우 <InlinePopover type="browser"/>, 데이터가 나타날 때까지 몇 분 정도 기다리십시오. 이미 몇 분 정도 기다렸는데도 여전히 데이터가 표시되지 않으면 다음 문제 해결 팁을 시도해 보세요. 이러한 팁 중 일부는 브라우저 모니터링을 통해 APM 모니터링 앱에 JavaScript 조각을 자동으로 삽입하도록 허용하여 [앱을 배포](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser) 했는지 아니면 JavaScript 조각을 직접 삽입했는지에 따라 달라집니다.
