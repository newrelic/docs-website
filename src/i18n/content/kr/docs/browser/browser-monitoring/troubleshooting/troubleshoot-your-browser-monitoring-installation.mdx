...
APM 에이전트로 모니터링되는 계정에 대해 브라우저 모니터링 에이전트가 배포된 경우 [APM 배포](#apm-tips) 를 참조하십시오. 브라우저 모니터링 에이전트가 JavaScript 복사/붙여넣기 방법을 사용하여 배포된 경우 [복사/붙여넣기 배포](#manual-tips) 를 참조하세요. 어떤 것이 있는지 확실하지 않은 경우 진단 도구에서 배포 방법을 반환합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="APM 에이전트 업데이트"
  >
    최신 APM 에이전트는 페이지 로드 타이밍 스크립트를 더 정확하게 배치합니다. 이러한 문제 해결 팁을 계속하기 전에 APM 에이전트에 대한 [최신 릴리스](/docs/apm/new-relic-apm/maintenance/keeping-new-relic-date) 가 있는지 확인하십시오.
