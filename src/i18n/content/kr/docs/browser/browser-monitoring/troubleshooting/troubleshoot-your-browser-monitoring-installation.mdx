...
방금 <InlinePopover type="browser"/>로 애플리케이션을 구성한 경우 데이터가 나타날 때까지 몇 분 정도 기다리십시오. 이미 몇 분 정도 기다렸는데도 여전히 데이터가 표시되지 않으면 다음 문제 해결 팁을 시도해 보세요. 이러한 팁 중 일부는 브라우저 모니터링이 JavaScript [](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser)스니펫을 APM-모니터 앱에 자동으로 삽입하도록 허용하여 앱을 구현하거나 배포하는지 아니면 JavaScript 스니펫을 직접 삽입하는지에 따라 달라집니다.

## 해결책

<DNT>**Recommendation:**</DNT> 특정 문제 해결 절차를 따르기 전에 [진단 브라우저 검사를](/docs/agents/manage-apm-agents/troubleshooting/new-relic-diagnostics) 실행하십시오. Diagnostics CLI가 다운로드되면, 목표는 버섯줄 옵션: `/nrdiag -browser-url YOUR-WEBSITE-URL -suites browser` 실행하여 브라우저를 확인합니다. Diagnostics CLI에서 반환된 정보는 뉴렐릭 지원팀과 통신할 때 사용할 수 있습니다.

귀하의 브라우저 배포 에이전트가 <InlinePopover type="apm"/>에이전트가 있는 계정 모니터에 대해 배포했다면 [APM 배포를](#apm-tips) 참조하세요. 브라우저 배포 에이전트가 JavaScript 복사/붙여넣기 방법을 사용하여 배포했다면 [복사/붙여넣기 배포를](#manual-tips) 참조하세요. 어떤 것이 있는지 확실하지 않은 경우 진단 도구는 배포 방법을 반환합니다.
...
<CollapserGroup>
  ...
  >
    ...
    1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Settings**</DNT>
  ...
  >
    ...
    * .NET 에이전트: `flush_dotnet_temp.cmd` 명령을 사용하여

      <DNT>**asp.net**</DNT>

      캐시 디렉터리가 깨끗한지 확인하세요. 이렇게 하면 앱 서버가 페이지 로드 타이밍 계측을 사용하여 ASP를 다시 컴파일하게 됩니다.
  </Collapser>
