...
브라우저 에이전트에 대한 프록시 URL을 설정하면 타사 도메인에 대한 클라이언트 요청을 차단하는 광고 차단기와 보안 VPN을 피하는 데 도움이 될 수 있으므로 유용할 수 있습니다.

광고 차단기와 보안 VPN은 웹에서 사용자를 추적하는 데 사용될 수 있기 때문에 종종 타사 도메인을 차단합니다. 자사 하위 도메인을 통해 요청을 프록시함으로써 이러한 차단기를 우회하고 관측 가능성 데이터가 계속 수집되도록 할 수 있습니다.
...
프록시를 설정하려면 브라우저 에이전트를 설치하는 데 사용한 배포 방법에 따라 아래 지침을 참조하세요.
...
* **New Relic CDN에서 브라우저 에이전트 코드를 로드하고 클라이언트에 다시 전달합니다.** `assets` 필드를 사용하는 경우 프록시는 `https://js-agent.newrelic.com/` 에서 브라우저 에이전트 코드를 로드하고 이를 클라이언트에 다시 전달해야 합니다. 예를 들어 브라우저 에이전트 v1.240.0의 경우 `https://assets.mydomain.com/nr-spa.3b61d78f-1.240.0.min.js` 에 대한 요청은 업스트림 `https://js-agent.newrelic.com/nr-spa.3b61d78f-1.240.0.min.js` 에서와 동일한 응답을 생성해야 합니다.
* **클라이언트 페이로드를 New Relic으로 전달합니다.** `beacon` 프록시는 클라이언트 페이로드를 New Relic으로 전달합니다. 원래 엔드포인트는 [브라우저 애플리케이션 설정 페이지](/docs/browser/new-relic-browser/configuration/browser-app-settings-page/) 에서 찾을 수 있습니다. JavaScript 스니펫에서 `;NREUM.info={beacon:"_____",...` 찾으세요. 대부분의 앱에서 이는 `(https://)bam.nr-data.net` 입니다. 그러나 규정 준수 및 법적 이유로 인해 달라질 수 있습니다. 예를 들어, `https://yourproxy.com/1/asdf?a=123` 에 대한 요청은 `https://bam.nr-data.net/1/asdf?a=123` 로 전달되어야 합니다.
* **요청과 응답을 수정하지 마십시오.** 프록시 서버를 통한 요청 및 응답은 수정되어서는 안 됩니다.
* **관측성 데이터 수집으로 생성된 대역폭과 트래픽을 지원합니다.** 시스템은 프록시 로드를 지원할 수 있어야 하지만 생성되는 데이터의 양은 사이트 사용량에 따라 달라집니다.