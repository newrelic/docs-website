...
우리의 <InlinePopover type="browser"/>는 [) <DNT>**Page views**</DNT> 페이지](/docs/browser/new-relic-browser/additional-standard-features/page-views-insights-your-sites-popularity) 와 [<DNT>**AJAX**</DNT> 페이지](/docs/browser/new-relic-browser/browser-pro-features/ajax-dashboard-identifying-time-consuming-calls) 의 데이터에 대해 URL 패턴을 사용합니다. URL 패턴을 사용하면 다음과 같은 이점이 있습니다.
...
여러 세그먼트를 나열할 때 순서는 중요하지 않습니다. 하지만 추가한 세그먼트가 URL에 표시되는 방식과 동일하게 <DNT>**exactly match**</DNT> 하는지 확인하세요. URL 경로 세그먼트가 `.html` 또는 `.jsp` 로 끝나는 경우 확장자가 세그먼트에 포함되어야 합니다. 세그먼트는 대소문자를 구분합니다.

애플리케이션의 허용 목록에 새 URL 세그먼트를 추가하려면 [GraphQL을](/docs/apis/nerdgraph/examples/browser-monitoring-config-nerdgraph#browser-segments) 사용하여 프로그래밍 방식으로 추가하거나 New Relic 브라우저 모니터링 UI에서 세그먼트 허용 목록을 사용할 수 있습니다.
...
3. <DNT>
   ...
   페이지의 그룹에 표시할 [세그먼트를](#format) 입력합니다.
...
브라우저 모니터링은 `ApplicationsController#show` 과 같은 서버 측 컨트롤러 작업 이름 대신 데이터 그룹화를 위해 URL 패턴을 사용합니다. 해당하는 경우 관련 백앤드 <InlinePopover type="apm"/>이벤트에 대한 링크가 포함되어 있습니다. 그러나 이는 독립형 서비스로서의 브라우저에 대한 기본 그룹화 전략이 아닙니다.
