...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 으로 이동, **브라우저** 를 클릭합니다.
2. 앱을 선택하세요.
3. 왼쪽 탐색 메뉴에서 **앱 설정** 을 클릭합니다.
4. **Ajax 요청 거부 목록** 아래에서 앱에 적용할 필터를 추가합니다.
5. **애플리케이션 설정 저장** 을 선택하여 에이전트 구성을 업데이트합니다.
6. [브라우저 에이전트를 다시 배포합니다](/docs/browser/new-relic-browser/installation/upgrade-browser-agent) (연결된 APM 에이전트를 다시 시작하거나 브라우저 설치 복사/붙여넣기 업데이트).
