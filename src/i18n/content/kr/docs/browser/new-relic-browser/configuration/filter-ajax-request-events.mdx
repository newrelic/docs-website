...
[에이전트 버전 1211](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/) 이상에서는 페이지에서 수행된 모든 네트워크 요청이 `AjaxRequest` 이벤트로 기록됩니다. **Application settings** \[애플리케이션 설정] 페이지의 거부 목록 구성 옵션을 사용하여 기록 이벤트를 요청하는 항목을 필터링할 수 있습니다.
