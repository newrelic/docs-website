...
import browserPageLoadTimeline from 'images/browser-page-load-timeline.png'

import pageLoadOverview from 'images/page_load_overview.png'
...
<img
  ...
  src={pageLoadOverview}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > Summary** : 로드 시간 차트가 **Summary** 및 **Page views** 페이지에 나타납니다.
</figcaption>
...
<CollapserGroup>
  <Collapser
    ...
    title="웹 애플리케이션"
  >
    웹 응용 프로그램 시간에는 응용 프로그램 서버에서 보낸 시간이 포함됩니다. 이는 페이지 로드 타이밍에 대한 브라우저 계측이 APM 에이전트에서 제공한 경우에만 결정할 수 있습니다.
  </Collapser>
  ...
  >
    ...
    네트워크 시간 측정은 링크를 처음 클릭할 때부터 시작됩니다. 네트워크 시간에는 DNS가 포함되며 사이트에 리디렉션이 있는 경우 TCP( [요청 대기열 시간 모니터링](/docs/apm/other-features/request-queueing/request-queuing-tracking-front-end-time) 을 구성하지 않은 경우 방화벽 포함) 및 SSL 연결 시간에 둘 이상의 조회가 포함될 수 있습니다.
    ...
    * 요청 대기열 시간 모니터링을 구성 **하지 않은** 경우 네트워크 시간에는 모든 요청 대기열 시간 **이** 포함됩니다.
      ...
      이 백엔드 시간이 브라우저의 관점에서 어떻게 분류되는지에 대한 자세한 내용은 [**세션 추적** 페이지](/docs/browser/new-relic-browser/browser-pro-features/session-traces-exploring-webpages-life-cycle) 를 참조하세요. 세션 추적은 사용 가능한 모든 네트워크 관련 이벤트에 대해 보고하므로 브라우저가 DNS 조회 및 기타 네트워크 이벤트에 시간을 어떻게 소비하는지 사례별로 확인할 수 있습니다.

      <Callout variant="important">
        요청 대기열이 구성된 경우에도 프런트 엔드 서버의 설정은 네트워크 시간에 여전히 영향을 미칠 수 있습니다. 이는 프런트 엔드 서버가 실제로 요청을 수락하고 처리할 때까지 대기열 시간 헤더를 추가하지 않기 때문입니다. 프런트 엔드 서버가 요청이 백로깅을 시작하고 프런트 엔드 서버가 연결을 수락하는 데 사용하는 수신기 소켓에 대기하도록 하는 방식으로 구성된 경우 브라우저 모니터링을 위한 네트워크 시간이 증가하는 것을 볼 수 있습니다. 대기열 시간 헤더는 이 수신기 소켓의 백로그를 설명할 수 없습니다.
      </Callout>
  </Collapser>
  ...
  <Collapser
    ...
    title="대기열 요청"
  >
    계정에 브라우저와 APM이 모두 연결된 경우 **요청 대기열** 이 로드 시간 차트에 표시됩니다. New Relic에서 [요청 큐잉](/docs/apm/applications-menu/features/request-queuing-tracking-front-end-time) 은 프로덕션 시스템에 들어오는 요청과 애플리케이션에 도달하는 사이의 시간을 나타냅니다. 프로덕션 인프라의 특성에 따라 이 시간에는 요청이 입력되는 실제 대기열이 포함되거나 시간이 걸리는 다른 기능(예: 로드 밸런싱 또는 내부 네트워크 대기 시간)을 나타낼 수 있습니다.
  </Collapser>
</CollapserGroup>
