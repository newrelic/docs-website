...
API를 사용하여 스크립트 초기화 중 또는 인라인 이벤트 핸들러에서 발생하는 오류와 같이 세부 정보 없이 보고될 오류를 확인할 수도 있습니다. 오류는 New Relic이 일반적으로 감지하는 다른 오류와 함께 [**JavaScript 오류** 페이지](/docs/errors-inbox/browser-tab) 에 나타납니다. 또한 [`JavaScriptError` 이벤트](/attribute-dictionary/?event=JavaScriptError) 로 기록됩니다. 예를 들어:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        필수의. 브라우저의 [**JavaScript 오류** 페이지](/docs/errors-inbox/browser-tab) 에서 데이터를 분석할 때 사용할 수 있는 의미 있는 오류 메시지를 제공하십시오.
      </td>
    </tr>
