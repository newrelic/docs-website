...
<Callout variant="important">
  SPA API의 `createTracer` 메서드는 더 이상 사용되지 않습니다. 작업 기간을 추적하는 권장 방법은 성능 [마크](https://developer.mozilla.org/en-US/docs/Web/API/Performance/mark) 및/또는 [측정값을](https://developer.mozilla.org/en-US/docs/Web/API/Performance/measure) 캡처하는 것입니다. 향후 브라우저 에이전트 버전에서는 마크와 측정값을 자동으로 캡처하게 되며, 이 시점에서 `createTracer` 에 대한 지원이 중단됩니다.
</Callout>
