...
<Callout variant="important">
  이 API는 모든 브라우저 버전(Browser Lite, Pro 또는 Pro+SPA)에서 작동합니다. 이 API를 사용하는 데 필요한 구성은 현재 New Relic 내의 대규모 배포 시스템에 연결되어 있지 않습니다. 따라서 추가 변경이 이루어질 때까지 복사/붙여넣기 또는 NPM 설치에만 영향을 미칩니다.
</Callout>
...
지연된 상태에서 실행될 때 에이전트 기능을 시작하는 데 사용되는 브라우저 API

## 요구 사항

에이전트 버전 [1.239.0](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes) 이상.
