...
유효한 값으로 이 함수를 실행하면, 도구 에이전트는 `autoStart: false` 설정에 의해 연기된 관련 기능을 시작합니다. 인수 없이 호출되면 메서드는 연기된 모든 기능을 시작합니다. 기능 이름을 나타내는 문자열 목록과 함께 호출되면 문자열과 일치하는 기능 이름이 시작됩니다. [기능 이름](#feature-names) 목록은 기능 이름을 참조하세요.
