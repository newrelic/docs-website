...
URL 구조가 유용한 그룹화를 제공하지 않거나 브라우저 에이전트가 라우팅 정보가 저장된 URL 부분을 캡처하지 못하는 경우, 사용자 지정 페이지 이름을 사용하면 페이지 조회수를 보다 효과적으로 그룹화할 수 있습니다. `PageView` [이벤트를](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data) 쿼리할 때 사용자 지정 이름은 `browserTransactionName` 속성으로 노출됩니다. 사용자 지정 이름은 <InlinePopover type="browser" />UI에도 표시됩니다.
