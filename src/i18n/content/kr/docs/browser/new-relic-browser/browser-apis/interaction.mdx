...
* `waitForEnd` 옵션은 v1.285.0 이상이 필요합니다.

* `targetPageLoad` 옵션은 v1.313.1+가 필요합니다.
...
* <DNT>**Page load interaction:**</DNT> `targetPageLoad` 옵션을 사용하면 진행 중일 수 있는 소프트 탐색에 관계없이 핸들은 항상 초기 페이지 로드 상호작용에 바인딩됩니다.

<Callout variant="caution">
  `waitForEnd: true` 을(를) `targetPageLoad: true` 와(과) 함께 사용하면 `.end()` 이(가) 호출될 때까지 초기 페이지 로드 상호작용이 무기한 열려 있게 되어 IPL의 기본 정의가 변경됩니다. 이는 고급 사용 사례이며 주의해서 사용해야 합니다.
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `targetPageLoad` - 기본값은 `false`입니다. `true`(으)로 설정하면 반환된 핸들은 소프트 탐색 상호작용을 생성하거나 타겟팅하는 대신 초기 페이지 로드 상호작용에 바인딩됩니다. 이것이 initialPageLoad 상호작용을 타겟으로 하는 유일한 방법입니다.
      </td>
    </tr>
  </tbody>
</table>
