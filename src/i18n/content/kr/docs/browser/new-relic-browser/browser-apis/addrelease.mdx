...
~ 안에 <InlinePopover type="browser"/>, 릴리스는 현재 실행 중인 애플리케이션 버전에 대한 정보로 오류에 태그를 지정하는 방법입니다. 이는 서로 다른 팀이 공유 구성 요소를 소유하거나 여러 애플리케이션이 독립적으로 배포되지만 동일한 페이지에서 실행되는 사이트에 유용합니다.
