...
JavaScript(JS) 오류 이벤트는 [<InlinePopover type="browser"/>JS 오류 페이지](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time). 기본적으로 오류 메시지별로 그룹화됩니다. 단일 오류 이벤트를 선택하면 해당 **Error Instances** \[오류 인스턴스] 목록에서 자세한 내용을 볼 수 있습니다. 경우에 따라 하나 이상의 인스턴스에 스택 추적이 없을 수 있습니다.

<Callout variant="tip">
  대신 "**This error is likely coming from a third-party site and no stack trace is available** \[이 오류는 타사 사이트에서 발생한 것일 수 있으며 스택 추적을 사용할 수 없습니다]." 라는 메시지가 표시됩니다.
</Callout>
