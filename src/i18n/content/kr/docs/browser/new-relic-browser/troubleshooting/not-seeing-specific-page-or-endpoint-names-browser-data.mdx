...
특정 페이지나 엔드포인트에 대한 성능 분석을 찾고 있습니다. 페이지는 <InlinePopover type="browser"/>[**Page views** \[페이지 조회수\] UI 페이지](/docs/browser/new-relic-browser/additional-standard-features/page-views-insights-your-sites-popularity) 또는 [**AJAX** UI 페이지를 봅니다 .](/docs/browser/new-relic-browser/browser-pro-features/ajax-dashboard-identifying-time-consuming-calls) UI에 찾고 있는 URL 그룹이 표시되지 않거나 `/` 및 `/*` 만 표시됩니다. 이 동작은 `BrowserInteraction` 이벤트의 SPA 데이터에도 나타나며 `browserInteractionName`, `targetGroupedUrl` 및 `previousGroupedUrl` 속성에 적용됩니다.
