...
<InlinePopover type="browser"/>[<DNT>**Page views**</DNT> UI 페이지](/docs/browser/new-relic-browser/additional-standard-features/page-views-insights-your-sites-popularity) 또는 [<DNT>**AJAX**</DNT> UI 페이지](/docs/browser/new-relic-browser/browser-pro-features/ajax-dashboard-identifying-time-consuming-calls) 에서 특정 페이지 또는 엔드포인트에 대한 성능 분석을 찾고 있습니다. UI에 찾고 있는 URL 그룹이 표시되지 않거나 `/` 및 `/*` 만 표시됩니다. 이 동작은 `BrowserInteraction` 이벤트의 SPA 데이터에도 나타나며 `browserInteractionName`, `targetGroupedUrl` 및 `previousGroupedUrl` 속성에 적용됩니다.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        2. 새 규칙을 만드는 대신 기존 규칙을 편집하고 UI의
    ...
    <tr>
      ...
      <td>
        사이트가 단일 페이지 애플리케이션이고 페이지 조회수 아래에 `/` 만 표시되는 경우 그룹화가 올바르게 작동하는 것입니다. 뉴렐릭은 AJAX가 로드될 때 후속 요청을 기록하며 [<DNT>**AJAX**</DNT> 페이지에](/docs/browser/new-relic-browser/browser-pro-features/ajax-dashboard-identifying-time-consuming-calls) 더 자세한 정보가 포함됩니다. 그렇지 않은 경우 이 문서의 팁을 따르십시오.
      </td>
    </tr>
