...
종종 앱 서버 트랜잭션 수(분당 요청 또는 **rpm**)는 동일한 애플리케이션에 대한 브라우저 트랜잭션 수(분당 페이지 또는 **ppm**)보다 큽니다. 모든 페이지가 웹에 접하는 사이트(API 또는 백엔드 전용 구성 요소 없음)도 기록된 브라우저 페이지 보기보다 2배에서 10배 더 많은 트랜잭션이 있을 수 있습니다.
