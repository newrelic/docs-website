...
<CollapserGroup>
  <Collapser className="freq-link" id="instrumentation" title="수단">
    앱의 모든 페이지를 계측하지 않거나 모든 페이지에서 자동 계측을 사용하지 않습니다.
  </Collapser>

  <Collapser className="freq-link" id="webpage-format" title="웹페이지 형식">
    웹페이지는 <DNT>**text/html**</DNT> 이 아닙니다. (데이터 손상을 방지하기 위해 브라우저는 HTML이 아닌 페이지는 브라우저에서 구문 분석할 의도가 아니기 때문에 이를 소비하지 않습니다.)
  </Collapser>

  <Collapser className="freq-link" id="ajax-requests" title="AJAX 요청">
...
브라우저 확장 프로그램이나 플러그인 형태로 제공되거나 개인 DNS나 라우터를 통해 설치된 광고 차단기는 에이전트가 페이지 뷰를 로드하고 캡처하는 것을 방지합니다. 이에 대한 유일한 해결 방법은 [에이전트 코드를 가져오고 데이터를 보낼 때 반전스 앱을](/docs/browser/new-relic-browser/configuration/proxy-agent-requests/) 사용하는 것입니다.