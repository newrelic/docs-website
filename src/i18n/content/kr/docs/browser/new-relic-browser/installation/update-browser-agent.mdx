...
1. [브라우저 에이전트 수명 종료 테이블을](https://docs.newrelic.com/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/) 참조하십시오.
2. 최신 버전 번호가 현재 실행 중인 버전 번호보다 높으면 브라우저 에이전트를 업데이트하세요.

## 배포 방법 확인 [#deploy-method]

브라우저 에이전트는 수동 [구현](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#options) (스니펫 복사 및 붙여넣기)을 사용하거나 <InlinePopover type="apm"/>대리인. 사용된 배포 방법을 확인하려면 JavaScript 콘솔에서 다음 명령을 실행합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="UI에서 스니펫 검색"
  >
    스니펫의 최신 버전 다운로드: **[one.newrelic.com > All capabilties](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > Application settings** 으로 이동합니다.
  ...
  >
    ...
    관리할 애플리케이션이 많거나 브라우저 앱이 <InlinePopover type="apm"/>앱.
  </Collapser>
