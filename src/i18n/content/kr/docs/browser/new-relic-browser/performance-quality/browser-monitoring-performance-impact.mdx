...
* <DoNotTranslate>**User perception:**</DoNotTranslate>
...
* <DoNotTranslate>**Webserver and systems:**</DoNotTranslate>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        `PageView` 이벤트는 `load` 페이지 수명 주기 이벤트 직후에 한 번 수집됩니다. 이 데이터는 뉴렐릭 UI 브라우저 대시보드에서 페이지 조회수를 생성하는 역할을 합니다. 이는 `bam.nr-data.net` 또는 `bam-cell.nr-data.net` 에 대한 JSONP 호출입니다. 모든 에이전트는 `PageView` 이벤트를 수집하기 위해 <DoNotTranslate>**required**</DoNotTranslate> 입니다.
      </td>
    </tr>
