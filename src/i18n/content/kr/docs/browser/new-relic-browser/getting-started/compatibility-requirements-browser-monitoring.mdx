...
우리의 <InlinePopover type="browser"/>브라우저 모니터링 기능을 갖춘 강력한 독립형 제품을 제공하기 위한 [엄격한 보안 조치가](/docs/browser/new-relic-browser/performance-quality/security-new-relic-browser) 포함되어 있습니다. [브라우저 에이전트를 설치](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser) 하기 전에 시스템이 이러한 요구 사항을 충족하는지 확인하세요.
