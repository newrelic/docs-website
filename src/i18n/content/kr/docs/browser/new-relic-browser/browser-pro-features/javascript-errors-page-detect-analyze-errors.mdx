...
3. 선택 사항: UI에서 [이벤트 로그](#event-log) 또는 [스택 트레이스](#stack-trace)를 검토합니다.
  ...
  <tbody>
    ...
    <tr id="error-details">
      ...
      <td>
        ...
        * 가능한 경우 **오류 인스턴스**를 선택하면 [이벤트 로그](#event-log) 및 [스택 트레이스](#stack-trace)를 확인할 수 있습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [**이벤트 로그**](#event-log)에는 오류를 유발한 브라우저 상호 작용, AJAX 호출 및 트레이스가 표시됩니다.
      </td>
    </tr>
...
이벤트 로그에는 오류를 유발한 브라우저 상호 작용, AJAX 호출 및 트레이스가 표시됩니다. 이는 오류의 근본 원인을 해결하는 데 도움이 될 수 있습니다.
...
<CollapserGroup>
  ...
  >
    이벤트 로그에 이벤트가 표시되지 않는 몇 가지 이유:

    * **에이전트 버전.** 이벤트 로그 데이터를 캡처하려면 사이트에서 [SPA monitoring](/docs/browser/single-page-app-monitoring/get-started/install-single-page-app-monitoring-new-relic-browser)이 활성화되어야 합니다.(브라우저 에이전트 버전 1071 이상)
    ...
    * **클릭으로 유발된 JS 오류.** 오류가 클릭의 직접적인 결과인 일부 경우 연결된 상호 작용이 캡처되지 않습니다.
      ...
      이벤트 로그 또는 스택 트레이스가 없으면, [오류 세부 정보 차트](#error-details)를 사용해 브라우저 유형, 디바이스 유형 및 URI별로 오류가 미치는 영향을 분석할 수 있습니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    * [이벤트 로그](#event-log)를 포함해 보다 자세한 정보를 얻으려면 **JS errors** UI 페이지를 참조하십시오. **JS errors** 페이지는 스택 트레이스 없이도 실행 가능한 정보를 제공하도록 설계되었습니다.
  </Collapser>
</CollapserGroup>
