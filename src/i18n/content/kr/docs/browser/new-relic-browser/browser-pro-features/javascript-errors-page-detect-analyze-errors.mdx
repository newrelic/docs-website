...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > JJS errors > (오류 선택) > Error instances** : [SPA 모니터링](/docs/browser/single-page-app-monitoring/get-started/install-single-page-app-monitoring-new-relic-browser) 이 활성화된 경우 이벤트 로그는 오류로 이어지는 활동을 보여줍니다.
</figcaption>
