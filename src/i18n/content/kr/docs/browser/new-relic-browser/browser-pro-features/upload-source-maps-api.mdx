...
는 <InlinePopover type="browser"/>[<DoNotTranslate>**Errors**</DoNotTranslate> 페이지](/docs/errors-inbox/browser-tab) 에서 오류 스택 추적을 축소 해제하는 데 사용되는 [소스 맵](/docs/new-relic-browser-source-maps) 업로드를 지원합니다. 이 문서에서는 API를 사용하여 브라우저 API를 사용하여 소스 맵을 업로드하는 방법을 설명합니다.
...
<CollapserGroup>
  ...
  >
    ...
    브라우저의 <DoNotTranslate>**JS errors**</DoNotTranslate> 페이지에서 오류의 자바스크립트 파일에 대한 URL을 찾을 수 있습니다. UI에서 이러한 오류를 찾는 방법에 대한 자세한 내용은 [브라우저 모니터링 소스 맵을](/docs/new-relic-browser-source-maps) 참조하세요.
  </Collapser>
  ...
  >
    ...
    * `https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.js`
      ...
      이 <DoNotTranslate>**doesn't**</DoNotTranslate> 이 귀하에게 적용되고 JS URL에 추가된 버전 정보가 **없는** 경우 [API를 사용하여 릴리스 이름과 ID를](/docs/browser-api-newrelicaddrelease) 지정하여 에이전트를 지원해야 합니다.
  </Collapser>
...
완료되면 브라우저의 [<DoNotTranslate>**JS errors**</DoNotTranslate> 페이지](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time) 로 이동하여 오류 그룹화를 선택하고 오류 스택 추적이 축소 해제되었는지 확인하세요.
...
완료되면 브라우저의 [<DoNotTranslate>**Group errors**</DoNotTranslate> 탭](docs/errors-inbox/browser-tab) 으로 이동하여 오류 그룹화를 선택하고 오류 스택 추적이 축소 해제되었는지 확인하세요.
