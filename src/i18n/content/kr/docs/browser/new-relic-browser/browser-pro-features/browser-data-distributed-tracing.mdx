...
브라우저 모니터링 에이전트 및 APM 에이전트에 필요한 최소 버전이 있는지 확인하십시오.
...
2. **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 으로 이동, **브라우저** 를 클릭합니다. 앱을 선택한 다음 **애플리케이션 설정** 을 클릭합니다.
3. **분산 추적** 토글을 켭니다.
...
<CollapserGroup>
  ...
  >
    ...
    2. 브라우저 모니터링 에이전트 및 APM 에이전트가 CORS에 대한 분산 추적과 [호환](#requirements) 되는지 확인하십시오. 해당되는 경우 최신 버전으로 업데이트하십시오.

    3. **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 으로 이동, **브라우저** 를 클릭합니다. 앱을 선택한 다음 **애플리케이션 설정** 을 클릭합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        분산 추적 UI에서 최종 사용자 범위는 <img title="distributed-tracing-browser-span-icon.png" alt="New Relic distributed tracing browser span icon" src={distributedTracingBrowserSpanIcon} style={{height: "30px", width: "30px"}}>으로 표시됩니다.</img>
