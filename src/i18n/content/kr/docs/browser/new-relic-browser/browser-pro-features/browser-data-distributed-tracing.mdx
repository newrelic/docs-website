...
브라우저 앱에 [Infinite Tracing](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing) 이라는 고급 유형의 분산 추적을 사용하는 일부 다운스트림 서비스가 있는 경우 브라우저 앱에 대해 이 기능을 활성화해야 합니다. 이렇게 하면 Infinite Tracing에서 추적하는 나머지 범위에 루트 범위(시작 브라우저 앱)가 포함됩니다.
...
* 분산 추적 UI에서 최종 사용자 범위는
  ...
  <img title="distributed-tracing-browser-span-icon.png" alt="New Relic distributed tracing browser span icon" src={distributedTracingBrowserSpanIcon}>으로 표시됩니다.</img>
