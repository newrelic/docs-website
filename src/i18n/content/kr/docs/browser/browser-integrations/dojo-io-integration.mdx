...
뉴렐릭의 Dojo 대시보드의 도움으로 멋진 코어 웹 바이탈, 사용자가 사이트에서 보내는 시간, 초기 페이지 로드 및 Javascript 오류가 있는 페이지와 같은 중요한 브라우저 성능 지표를 추적할 수 있습니다. <InlinePopover type="browser"/>에이전트로 구축된 Dojo 대시보드를 사용하면 가장 중요한 모든 데이터를 한 곳에서 볼 수 있습니다.
...
1. [설치 실행기](https://one.newrelic.com/marketplace?account=3510613&state=79352429-fa28-4c25-1123-30c4b23ff83c) 로 이동하여

   <DNT>
     **Browser monitoring**
   </DNT>

   클릭하여 브라우저 에이전트를 설치합니다.
...
앱과 함께 배포 에이전트가 설치되고 로그된 경우 [지표 및 이벤트](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/) 에서 원시 데이터를 볼 수 있습니다. 기본 <InlinePopover type="dashboards"/>는 원시 데이터를 시스템 상태에 대한 높은 수준의 보기를 제공하는 차트와 그래프로 변환합니다. 기본 대시보드를 설치하려면 [즉시 제공되는 옵저버티빌리 페이지](https://newrelic.com/instant-observability/)로 이동하세요.
