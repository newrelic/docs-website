...
이전 버전 2.0에서는 상호작용과 관련된 AjaxRequest가 콜백 타이밍을 보고하거나 추적하지 않아 위에 언급된 세 가지 속성에 대한 값이 무효화되었습니다. 현재 업데이트에서는 장기 작업을 유발하는 이벤트 리스너를 포함하는 인프라의 일부인 `XMLHttpRequest` (하지만 `fetch` 은 아님)의 `jsDuration`, `timeToSettle` 및 `timeToLastCallbackEnd` 속성이 장기 작업의 타이밍을 반영하도록 업데이트됩니다.
