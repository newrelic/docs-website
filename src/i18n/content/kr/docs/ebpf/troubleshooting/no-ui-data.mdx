...
3. **데이터 필터링 설정 확인**: 귀하의 파티션이 `allDataFilters.dropServiceNameRegex` 또는 `allDataFilters.dropNamespaces` 등의 설정 초기화에 의해 제외되지 않았는지 확인하세요.

4. **지속적인 gRPC/HTTP/2 연결이 있는 애플리케이션 다시 시작**: 에이전트가 엔티티를 올바르게 생성하고 트래픽을 모니터할 수 있도록 eBPF 에이전트가 시작된 후 지속적인 gRPC/HTTP/2 연결이 있는 애플리케이션을 다시 시작해야 합니다. 에이전트가 시작되기 전에 gRPC 또는 HTTP/2 애플리케이션이 실행 중이었다면, 에이전트가 새로 설정된 연결을 후킹할 수 있도록 애플리케이션을 다시 시작하십시오.
