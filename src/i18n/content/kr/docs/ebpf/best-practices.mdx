...
위의 방법을 따르면 단일 eBPF 에이전트를 사용하여 인프라 및 애플리케이션 모니터링 요구 사항을 모두 처리할 수 있습니다. 이는 옵저버티빌리를 통합하여 에이전트가 중요한 성능 데이터(eBPF APM) 및 네트워크 지표(eBPF 네트워크 지표)를 자동으로 수집하여 기존 대시보드를 채우고 코드 변경이나 재시작 없이 심층적인 컨텍스트를 제공할 수 있도록 합니다.
...
* **확장을 위해 eBPF APM 으로 시작:** 많은 디자인에 걸쳐 대규모로 구현, 배포하다 모니터링이 필요하고 복잡한 조정 없이 &quot;충분히 좋은&quot; 지표를 원하는 경우 eBPF 기반 APM 으로 시작하세요.
