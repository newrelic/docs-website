---
title: Kubernetes용 뉴렐릭 eBPF 에이전트 설치
metaDescription: Learn how to install and configure the New Relic eBPF agent for your Kubernetes cluster using Helm charts.
tags:
  - New Relic integrations with eBPF
  - New Relic eBPF agent
  - eBPF integration
  - eBPF APM
  - Kubernetes cluster
  - New Relic eBPF agent for Kubernetes
freshnessValidatedDate: never
translationType: machine
---

쿠버네티스 클러스터에 뉴렐릭 eBPF 에이전트를 설치하여 전체 시스템 상태를 모니터링할 수 있습니다. eBPF 에이전트는 코드 변경이나 구현, 배포할 필요 없이 언어별 에이전트에 대한 심층적인 가시성을 제공합니다.

## 뉴렐릭 eBPF 에이전트 설치

<Steps>
  <Step>
    ### 시작하기 전에 [#requirements]

    쿠버네티스를 사용하려면 [eBPF 호환성 및 요구 사항](/docs/ebpf/requirements#k8s) 문서에 설명된 전제 조건을 충족해야 합니다.
  </Step>

  <Step>
    ### eBPF 에이전트 설치 [#install]

    eBPF 에이전트를 설치하려면:

    1. New Relic 계정에 로그인하세요.

    2. <DNT>**[left navigation pane &gt; + Integration &amp; Agents &gt; eBPF Agent](https://onenr.io/0oR8XWW36RG)**</DNT> 으)로 이동합니다.

    3. <DNT>Select an account</DNT> 화면에서 eBPF 에이전트를 설치할 계정을 선택하고 <DNT>**Continue**</DNT> 클릭합니다.

    4. <DNT>Select an installation method</DNT> 페이지에서 <DNT>**Kubernetes**</DNT> 선택하고 <DNT>**Continue**</DNT> 클릭합니다.

    5. <DNT>Enter your user key</DNT> 화면에서 다음 옵션 중 하나를 선택한 다음 <DNT>**Continue**</DNT> 클릭하십시오.

       * <DNT>**Use an existing key**</DNT>이미 사용자 키가 있는 경우 사용자 키를 제공하십시오. 자세한 내용은 [사용자 키를](/docs/apis/intro-apis/new-relic-api-keys/#user-key) 참조하십시오.
       * <DNT>**Create a new key**</DNT>사용자 키가 없으면 <DNT>**Create a new key**</DNT> 클릭하여 생성하세요.

    6. <DNT>Configure the Kubernetes integration</DNT> 화면에서:

       1. Kubernetes 의 구현, 배포 이름을 입력합니다.
       2. (선택 사항) 통합에 대한 네임스페이스를 입력합니다. 기본 네임스페이스는 <DNT>`newrelic`</DNT> 입니다.
       3. <DNT>**Continue**</DNT> 을(를) 클릭합니다.

       <Callout variant="tip">
         뉴럴릭 측정에 대해 사용자 정의 쿼리스페이스를 선택하는 경우(기본값 <DNT>`newrelic`</DNT> 대신) 해당 쿼리스페이스를 <DNT>`allDataFilters.dropNamespaces`</DNT> 설정 변수에 추가하여 모델링에서 제외하는 것이 좋습니다. 이는 eBPF 에이전트가 측정, 로그 필드 자체를 모니터링하는 것을 방지합니다. 예를 들어, <DNT>`newrelic-mon`</DNT> 네임스페이스로 사용하는 경우 <DNT>`allDataFilters.dropNamespaces: ["kube-system", "newrelic-mon"]`</DNT> 설정합니다.
       </Callout>

    7. <DNT>Install the Kubernetes integration</DNT> 화면에서:

       1. Helm 사용하여 쿠버네티스 클러스터에 eBPF 에이전트를 설치하려면 표시된 명령을 복사하여 붙여넣으세요.

       2. (선택 사항) <DNT>`values.yaml`</DNT> 설정 파일을 다운로드하려면 <DNT>**Download**</DNT> 클릭하세요. 설정 방법에 대한 자세한 내용은 [K8s 설정 방법을](#config-params) 참조하세요.

       3. (선택 사항) 필요에 따라 <DNT>`values.yaml`</DNT> 파일을 업데이트하고 저장합니다.

       4. (선택 사항) 설정 변경 사항을 적용하려면 다음 명령을 실행하십시오.

          ```bash
              helm repo update ; helm upgrade --install nr-ebpf-agent newrelic/nr-ebpf-agent -n newrelic --values values.yaml
          ```

       5. 설치를 확인하려면 다음 명령을 실행하세요.

          ```bash
              kubectl get pods -n newrelic
          ```
  </Step>

  <Step>
    ### 뉴렐릭에서 eBPF 데이터에 액세스 [#access-data]

    eBPF 에이전트가 설치되면 Linux 호스트에서 자동으로 데이터를 수집하기 시작합니다. 이 데이터는 뉴렐릭의 OpenTelemetry UI 에서 액세스할 수 있습니다. 뉴렐릭 OpenTelemetry UI 에 대한 자세한 내용은 [OpenTelemetry APM UI](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-ui) 참조하세요.

    **뉴렐릭에서 eBPF 데이터를 보려면:**

    1. <DNT>**[one.newrelic.com](https://one.newrelic.com) &gt; APM &amp; Services**</DNT> 으)로 이동합니다.
    2. 검색 배너에서 검색 기준을 <DNT>`instrumentation.name = nr_ebpf`</DNT> 으로 설정합니다.
       <img style={{ align: 'left' }} title="eBPF filters" alt="eBPF filter for eBPF data in New Relic OpenTelemetry UI" src="/images/ebpf_filters.webp" />
  </Step>
</Steps>

{/*
Once your app is instrumented and configured to export data to New Relic, you should be able to find your data in the New Relic UI:

    * Find your entity at <DNT>**All entities > Services - OpenTelemetry**</DNT>. The entity name is set to the value of the app's `service.name` resource attribute. For more information on how New Relic service entities are derived from OpenTelemetry resource attributes, see [Services](/docs/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services).
    * Use [NRQL](/docs/nrql/get-started/introduction-nrql-new-relics-query-language/) to query directly for [traces](https://one.newrelic.com/launcher/nr1-core.explorer?overlay=), [metrics](https://one.newrelic.com/launcher/nr1-core.explorer?overlay=), and [logs](https://one.newrelic.com/launcher/nr1-core.explorer?overlay=).
    * See [OpenTelemetry APM UI](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-ui) for more information.

If you can't find your entity and don't see your data with NRQL, see [OTLP troubleshooting](/docs/opentelemetry/best-practices/opentelemetry-otlp-troubleshooting).
You can find the data collected by the eBPF agent in the New Relic Opentelementry UI.

 */}

## eBPF 에이전트 업그레이드 [#upgrade]

쿠버네티스 클러스터에서 eBPF 에이전트를 업그레이드하려면:

* **표준 업그레이드의 경우**: 다음 Helm 명령을 사용하여 최신 버전으로 업그레이드하세요.

```bash
KSM_IMAGE_VERSION="v2.13.0" && helm repo add newrelic https://helm-charts.newrelic.com && helm repo update && kubectl create namespace "newrelic" ; helm upgrade --install nr-ebpf-agent newrelic/nr-ebpf-agent --set licenseKey=<key> --set cluster="<cluster-name>" --namespace=newrelic
```

* **특정 버전 업그레이드의 경우:** 특정 버전으로 업그레이드하려면 `--version` 플래그를 사용하세요.

```bash
KSM_IMAGE_VERSION="v2.13.0" && helm repo add newrelic https://helm-charts.newrelic.com && helm repo update && kubectl create namespace "newrelic" ; helm upgrade --install nr-ebpf-agent newrelic/nr-ebpf-agent --set licenseKey=<key> --set cluster="<cluster-name>" --namespace=newrelic --version=0.2.5
```

`<key>` 클러스터 이름으로 바꾸고 `<cluster-name>` 뉴렐릭 클러스터 키로 바꾸세요.

## 구성 매개변수 [#config-params]

[`values.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-ebpf-agent/values.yaml) 파일에는 다음 설정 섹션이 포함되어 있습니다.

<Callout variant="tip">
  eBPF 에이전트는 환경에 따라 다음과 같이 이름을 자동으로 다르게 생성합니다.

  * 호스트나 도커에서 이러한 이름은 프로세스 이름, 디렉토리 또는 컨테이너 ID, 수신 포트의 조합입니다. 예를 들어, `ruby:/home/ubuntu/app:[5678]` 또는 `java:f4aead533895:[8080]`.

  * Kubernetes에서 이러한 이름은 `mysql-database-service` 과 같은 서비스 이름에서 파생됩니다.

  애플리케이션에 사용자 정의 이름 지정:

  * Kubernetes 및 온호스트 제작 모두에 대해 `NEW_RELIC_APP_NAME` 환경 변수를 설정하여 맞춤 이름을 할당할 수 있습니다.
</Callout>

<CollapserGroup>
  <Collapser id="general-configuration" title="일반 설정">
    이러한 매개변수는 eBPF 에이전트의 핵심 ID와 데이터 대상을 제어합니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `cluster`
          </td>

          <td>
            쿠버네티스 클러스터의 이름을 지정합니다. 이 필드는 필수입니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"production-cluster"`
          </td>
        </tr>

        <tr>
          <td>
            `licenseKey`
          </td>

          <td>
            귀하의 블루 클러스터 키를 지정합니다. 

            `customSecretName`

             사용하지 않으면 필수입니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"8356...FFFFNRAL"`
          </td>
        </tr>

        <tr>
          <td>
            `customSecretName`
          </td>

          <td>
            라이선스 키가 포함된 Kubernetes 비밀의 이름을 지정합니다. 이 기능을 사용하면 키를 직접 제공하지 않아도 됩니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"newrelic-license-secret"`
          </td>
        </tr>

        <tr>
          <td>
            `customSecretLicenseKey`
          </td>

          <td>
            라이선스 키 값이 저장된 비밀 내의 키를 지정합니다. 

            `customSecretName`

             와 함께 사용됩니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"license"`
          </td>
        </tr>

        <tr>
          <td>
            `namespaceLicenseKeys`
          </td>

          <td>
            다중 계정 OTLP 내보내기 라우팅을 위해 각 Kubernetes 네임스페이스를 뉴렐릭 라이선스 키에 매핑합니다. 맵에 없는 네임스페이스는 글로벌 

            `licenseKey`

            (으)로 대체됩니다.
          </td>

          <td>
            `Object`
          </td>

          <td>
            `{"team-a": "NRAK-aaa...", "team-b": "NRAK-bbb..."}`
          </td>
        </tr>

        <tr>
          <td>
            `customSecretNamespaceLicenseKeys`
          </td>

          <td>
            JSON으로 인코딩된 네임스페이스-라이선스 키 매핑이 있는 

            `NAMESPACE_LICENSE_KEY_MAP`

             키를 포함하는 기존 Kubernetes 시크릿을 지정합니다. values.yaml에 라이선스 키를 저장하지 않으려면 

            `namespaceLicenseKeys`

             대신 이것을 사용합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"namespace-license-keys-secret"`
          </td>
        </tr>

        <tr>
          <td>
            `region`
          </td>

          <td>
            뉴렐릭 계정 지역을 지정합니다. 허용되는 값: 

            `US`

            , 

            `EU`

            , 

            `JP`

            , 

            `GOV`

            . 

            `US`

             (기본값)의 경우 비워 둡니다. 

            `customSecretName`

            을(를) 사용할 때 필요합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"US"`
          </td>
        </tr>

        <tr>
          <td>
            `fedramp.enabled`
          </td>

          <td>
            FedRAMP 호환 엔드포인트를 활성화합니다(

            `gov-otlp.nr-data.net`

            ). 이 방법은 FedRAMP 규정 준수를 활성화하는 데 선호되는 방법이며 리전 기반 감지(

            `region="GOV"`

            과(와) 동일)보다 우선합니다. 

            `global.fedramp.enabled`

            을(를) 사용하여 구성할 수도 있습니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `false`
          </td>
        </tr>

        <tr>
          <td>
            `logLevel`
          </td>

          <td>
            에이전트에 대한 로깅 세부 정보 수준을 정의합니다. 유효한 옵션: 

            `OFF`

            , 

            `FATAL`

            , 

            `ERROR`

            , 

            `WARNING`

            , 

            `INFO`

            , 

            `DEBUG`

            .
          </td>

          <td>
            `String`
          </td>

          <td>
            `"INFO"`
          </td>
        </tr>

        <tr>
          <td>
            `logFilePath`
          </td>

          <td>
            로그 출력을 위한 에이전트 컨테이너 내부의 파일 경로를 지정합니다. 경로가 유효하지 않으면 로그가 stdout으로 전송됩니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"/var/log/nr-ebpf-agent.log"`
          </td>
        </tr>

        <tr>
          <td>
            `downloadedPackagedHeadersPath`
          </td>

          <td>
            eBPF 에이전트가 사용할 필수 Linux 헤더를 수동으로 다운로드하여 배치하는 전체 디렉토리의 절대 경로를 설정합니다. 이 기능은 에이전트가 필요한 Linux 헤더를 다운로드할 수 없는 제한된 환경에서 유용합니다. 필수 헤더는 커널 버전을 기반으로 에이전트에 의해 식별됩니다. K8s의 경우 절대 경로 앞에는 필요한 경우 /host를 붙여야 합니다. NR 지원 권장 이후에만 사용하세요.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"/path/to/downloaded/headers/dir"`
          </td>
        </tr>

        <tr>
          <td>
            `distroKernelHeadersPath`
          </td>

          <td>
            eBPF 에이전트가 사용할 Linux 헤더가 있는 전체 디렉토리의 절대 경로를 설정합니다. 이 기능은 필요한 Linux 헤더를 설치할 수 없거나 경로를 결정할 수 없는 경우에 유용합니다. K8s의 경우 절대 경로 앞에는 필요한 경우 /host를 붙여야 합니다. NR 지원 권장 이후에만 사용하세요.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"/host/usr/src/linux-headers-6.8.0-pl"`
          </td>
        </tr>

        <tr>
          <td>
            `reportApmData`
          </td>

          <td>
            APM 데이터 보고를 제어합니다. 허용되는 값: 

            `"true"`

             (항상 보냄), 

            `"false"`

             (절대 보내지 않음), 

            `"auto"`

             (APM이나 OTel 에이전트가 모두 연결되지 않은 경우에만 보냄).
          </td>

          <td>
            `String`
          </td>

          <td>
            `"auto"`
          </td>
        </tr>

        <tr>
          <td>
            `reportNetworkMetrics`
          </td>

          <td>
            네트워크 메트릭 보고를 제어합니다. 활성화되면 에이전트는 TCP 통계를 포함한 네트워크 메트릭을 수집하고 보고합니다. 허용되는 값: 

            `"true"`

             (항상 보냄), 

            `"false"`

             (보내지 않음), 

            `"auto"`

            .
          </td>

          <td>
            `String`
          </td>

          <td>
            `"auto"`
          </td>
        </tr>

        <tr>
          <td>
            `reportLogs`
          </td>

          <td>
            애플리케이션 로그 보고를 제어합니다. 허용되는 값: 

            `"true"`

             (항상 전송), 

            `"false"`

             (전송 안 함), 

            `"auto"`

             (연결된 APM 에이전트가 자체적으로 로그를 적극적으로 수집하거나 OTel 에이전트가 연결되어 있는 경우를 제외하고 전송). 자세한 내용은 

            [자동 감지 및 백오프](/docs/ebpf/logs/#automatic-backoff)

             를 참조하십시오.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"false"`
          </td>
        </tr>

        <tr>
          <td>
            `agentMetadataReporting`
          </td>

          <td>
            에이전트 및 엔티티 메타데이터 보고를 활성화합니다. 기본적으로 에이전트는 HTTP를 통해 에이전트 및 엔티티 메타데이터를 뉴렐릭으로 내보냅니다. 메타데이터 보고 HTTP 내보내기 경로를 완전히 비활성화하려면 이를 

            `false`

             (으)로 설정하십시오.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `customOtlpEndpoint`
          </td>

          <td>
            사용자 지정 OTLP 엔드포인트 URL. 설정된 경우, 이는 리전 기반 정적 엔드포인트보다 우선합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `customOtlpEndpointTlsEnabled`
          </td>

          <td>
            OTLP 엔드포인트에서 TLS가 활성화되어 있는지 여부입니다. 

            `customOtlpEndpoint`

             및 이 필드가 모두 명시적으로 설정된 경우에만 재정의됩니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `customOtlpEndpointTlsCertSecret`
          </td>

          <td>
            사용자 지정 OTLP 엔드포인트의 CA 인증서가 포함된 Kubernetes 시크릿의 이름입니다. 

            `customOtlpEndpoint`

             이 설정된 경우에만 사용됩니다. 

            `customOtlpEndpointTlsCertSecretKey`

            와(과) 함께 제공되어야 합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `customOtlpEndpointTlsCertSecretKey`
          </td>

          <td>
            CA 인증서가 포함된 시크릿 내의 키입니다. 

            `customOtlpEndpoint`

             이 설정된 경우에만 사용됩니다. 

            `customOtlpEndpointTlsCertSecret`

            와(과) 함께 제공되어야 합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.host`
          </td>

          <td>
            기업 프록시를 통해 OTLP 텔레메트리 데이터를 라우팅하기 위한 HTTP/HTTPS 프록시 호스트명입니다. 비어 있는 경우, 프록시가 사용되지 않습니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"proxy.example.com"`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.port`
          </td>

          <td>
            OTLP 프록시용 프록시 포트입니다.
          </td>

          <td>
            `Integer`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.scheme`
          </td>

          <td>
            프록시 URL 스킴. 허용되는 값: 

            `http`

            , 

            `https`

            .
          </td>

          <td>
            `String`
          </td>

          <td>
            `"http"`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.user`
          </td>

          <td>
            프록시 기본 인증을 위한 사용자 이름. 비어 있는 경우 인증이 사용되지 않습니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.password`
          </td>

          <td>
            프록시 기본 인증 비밀번호. 

            `otlpProxy.user`

            와(과) 함께만 사용됩니다. 보안을 위해 비밀번호를 직접 설정하는 대신 

            `otlpProxy.existingSecret`

             을(를) 사용하는 것을 권장합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.url`
          </td>

          <td>
            전체 프록시 URL. 설정 시 이는 개별 

            `otlpProxy.host`

            , 

            `port`

            , 

            `scheme`

            , 

            `user`

             및 

            `password`

             설정을 재정의합니다. 포맷: 

            `http://[user:pass@]host:port`

            .
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `otlpProxy.existingSecret`
          </td>

          <td>
            프록시 자격 증명이 포함된 기존 Kubernetes 시크릿의 이름. 시크릿에는 

            `proxy-user`

             및 

            `proxy-password`

             키가 포함되어야 합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `ai_monitoring.enabled`
          </td>

          <td>
            생성형 AI 텔레메트리 보고를 제어합니다. 허용되는 값: 

            `"true"`

             (항상 전송), 

            `"false"`

             (전송 안 함), 

            `"auto"`

             (언어 APM 또는 OTel 에이전트가 엔티티에 대한 AI 모니터링 데이터를 아직 보고하지 않은 경우에만 전송).
          </td>

          <td>
            `String`
          </td>

          <td>
            `"false"`
          </td>
        </tr>

        <tr>
          <td>
            `ai_monitoring.samplingLatency`
          </td>

          <td>
            생성형 AI 스팬을 내보내기 위한 지연시간 기반 샘플링 임계값을 정의합니다. 

            `p0`

            에서 

            `p99`

             사이의 모든 백분위수를 지원합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"p50"`
          </td>
        </tr>

        <tr>
          <td>
            `ai_monitoring.samplingErrorRate`
          </td>

          <td>
            생성형 AI 라우트에 대한 오류율 임계값을 정의하며, 이를 초과하면 라우트의 해당 스팬이 내보내집니다. 옵션: 

            `1`

            \-

            `100`

            .
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `ai_monitoring.genAICaptureMessageContent`
          </td>

          <td>
            `true`

            인 경우, 생성형 AI 상호작용에 대한 전체 프롬프트 및 완료 콘텐츠를 캡처합니다. 

            `ai_monitoring.enabled`

            이(가) 

            `true`

             또는 

            `auto`

            인 경우에만 적용됩니다. 주의해서 사용하십시오.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `false`
          </td>
        </tr>

        <tr>
          <td>
            `httpPathNormalizationPatterns`
          </td>

          <td>
            HTTP 경로 정규화 패턴을 정의합니다. 각 패턴에는 하나 이상의 

            `*`

             와일드카드가 포함되어야 합니다. 경로를 있는 그대로 유지(자동 클러스터링 건너뛰기)하려면 제외 패턴에 

            `!`

             접두사를 사용하십시오. 에이전트는 자동 클러스터링 전에 이러한 패턴을 적용하며, 이 패턴이 우선 적용됩니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `["/api/users/*", "/api/orders/*", "!/health/*"]`
          </td>
        </tr>

        <tr>
          <td>
            `httpBodyLimitBytes`
          </td>

          <td>
            HTTP 요청/응답 본문에서 캡처되는 최대 바이트 수를 설정합니다. 기본값: 

            `ai_monitoring.enabled`

             이(가) 

            `true`

             또는 

            `auto`

            인 경우 

            `1048576`

             (1 MB), 그렇지 않으면 

            `1024`

             (1 KB). 뉴렐릭이 AI 모니터링 페이로드를 자르는 경우 이 값을 늘리십시오.
          </td>

          <td>
            `Integer`
          </td>

          <td>
            `2097152`
          </td>
        </tr>

        <tr>
          <td>
            `entityLabels`
          </td>

          <td>
            eBPF 에이전트가 보고하는 모든 엔티티에 추가될 사용자 지정 레이블입니다. 이 레이블은 

            `"key1:value1;key2:value2"`

             형식의 

            `NEW_RELIC_LABELS`

             환경 변수로 전송됩니다.
          </td>

          <td>
            `Map`
          </td>

          <td>
            `{}`
          </td>
        </tr>
      </tbody>
    </table>

    <Callout variant="important">
      <DNT>eBPF agent</DNT> 은(는) <DNT>`http://`</DNT> 스킴만 사용하는 HTTP CONNECT 프록시를 지원합니다. gRPC 제한으로 인해 <DNT>`https://`</DNT> 스킴(프록시로 직접 연결되는 TLS 암호화 연결)은 지원되지 않습니다. 그러나 텔레메트리 데이터는 에이전트와 뉴렐릭 OTLP 엔드포인트 간에 항상 **엔드투엔드** 로 TLS 암호화됩니다. <DNT>`http://`</DNT> 접두사는 에이전트가 터널을 설정하기 위해 프록시와 통신하는 방법만 지시합니다. 프록시 자체는 암호화된 트래픽만 볼 수 있으며 사용자의 데이터에는 절대 접근할 수 없습니다. 전체 설정 지침은 [<DNT>eBPF agent</DNT>용 프록시 구성](/docs/ebpf/proxy-configuration)을 참조하세요.
    </Callout>
  </Collapser>

  <Collapser id="all-data-filters" title="모든 데이터 필터">
    이 섹션에서는 제공된 설정에 따라 모든 유형의 네트워크 범위 및 APM 데이터를 제외하도록 필터를 구성합니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `allDataFilters.dropNewRelicBundle`
          </td>

          <td>
            NewRelic 

            `namespace`

             및 

            `newrelic-bundle`

             서비스에서 데이터를 삭제합니다. 이 필드는 

            `dropDataNewRelic`

             에서 이름이 변경되었습니다. 이전 이름은 더 이상 사용되지 않지만 하위 호환성을 위해 계속 지원됩니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.dropNamespaces`
          </td>

          <td>
            에이전트가 모든 데이터를 삭제해야 하는 Kubernetes 네임스페이스 목록입니다. 네임스페이스를 필터링하려면 dropNamespaces 또는 keepNamespaces 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepNamespaces가 우선합니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `["kube-system"]`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.keepNamespaces`
          </td>

          <td>
            에이전트가 모든 데이터를 전송해야 하는 Kubernetes 네임스페이스 목록입니다. 네임스페이스를 필터링하려면 dropNamespaces 또는 keepNamespaces 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepNamespaces가 우선합니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `[]`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.dropPodLabels`
          </td>

          <td>
            모든 데이터를 필터링하기 위해 일치시킬 파드 레이블. 빈 맵은 레이블 기반 필터링이 없음을 의미합니다. 예: 

            `{ "app": "frontend", "env": "production" }`

            . 파드 레이블을 기반으로 필터링하려면 dropPodLabels 또는 keepPodLabels 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepPodLabels가 우선합니다.
          </td>

          <td>
            `Map`
          </td>

          <td>
            `{}`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.keepPodLabels`
          </td>

          <td>
            모든 데이터를 유지하기 위해 일치해야 하는 파드 레이블입니다. 빈 맵은 레이블 기반 필터링이 없음을 의미합니다. 예: 

            `{ "app": "frontend", "env": "production" }`

            . 파드 레이블을 기반으로 필터링하려면 dropPodLabels 또는 keepPodLabels 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepPodLabels가 우선합니다.
          </td>

          <td>
            `Map`
          </td>

          <td>
            `{}`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.dropServiceNameRegex`
          </td>

          <td>
            삭제할 k8s 서비스 이름과 일치하는 정규식을 정의합니다. 예를 들어 

            `"kube-dns|otel-collector|\\bblah\\b"`

            . 서비스 이름을 필터링하려면 dropServiceNameRegex 또는 keepServiceNameRegex 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepServiceNameRegex가 우선합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.keepServiceNameRegex`
          </td>

          <td>
            이 구성은 

            `dropServiceNameRegex`

             구성에 대한 우회 역할을 합니다. 이 정규식과 일치하는 서비스 이름은 

            `dropServiceNameRegex`

            에 의해 데이터가 삭제되지 않습니다. 서비스 이름을 필터링하려면 dropServiceNameRegex 또는 keepServiceNameRegex 중 하나만 사용하고 둘 다 사용하지는 마십시오. 둘 다 제공되는 경우, keepServiceNameRegex가 우선합니다. 이 필드는 

            `allowServiceNameRegex`

            에서 이름이 변경되었습니다. 이전 이름은 더 이상 사용되지 않지만 이전 버전과의 호환성을 위해 계속 지원됩니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `""`
          </td>
        </tr>

        <tr>
          <td>
            `allDataFilters.dropApmAgentEnabledEntity`
          </td>

          <td>
            NewRelic 또는 OTEL APM 에이전트가 실행 중인 애플리케이션 또는 엔티티의 모든 데이터를 삭제합니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `false`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="apm-data-filters" title="APM 데이터 필터">
    제공된 설정에 따라 ebpf APM 데이터를 제외하도록 필터를 구성합니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `apmDataFilters.dropPodLabels`
          </td>

          <td>
            APM 데이터 필터링을 위해 일치시킬 파드 레이블입니다. 지도가 비어 있으면 레이블 기반 필터링이 적용되지 않습니다. 예를 들면: dropPodLabels: 

            `{ "app": "frontend", "env": "production" }`
          </td>

          <td>
            `Map`
          </td>

          <td>
            `{}`
          </td>
        </tr>

        <tr>
          <td>
            `apmDataFilters.dropEntityName`
          </td>

          <td>
            eBPF APM 데이터를 삭제할 엔티티 이름 목록
          </td>

          <td>
            `List`
          </td>

          <td>
            `[]`
          </td>
        </tr>

        <tr>
          <td>
            `apmDataFilters.keepEntityName`
          </td>

          <td>
            APM 데이터를 항상 보관해야 하는 엔티티 이름 목록입니다. 기본적으로 모든 엔티티가 유지/활성화됩니다. 이 설정은 

            `dropEntityName`

             필터를 우회합니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `[]`
          </td>
        </tr>

        <tr>
          <td>
            `apmDataFilters.jvmMetricsReporting`
          </td>

          <td>
            JVM 메트릭 보고를 활성화합니다. 활성화되면 에이전트는 인스턴스에 대한 JVM 지표를 수집하고 보고합니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="network-metric-data-filters" title="네트워크 지표 데이터 필터">
    제공된 설정에 따라 네트워크 메트릭 데이터를 삭제/유지하도록 필터를 구성합니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `networkMetricsDataFilter.dropPodLabels`
          </td>

          <td>
            네트워크 범위를 필터링하기 위해 일치시킬 파드 레이블입니다. 지도가 비어 있으면 레이블 기반 필터링이 적용되지 않습니다. 예를 들면: dropPodLabels: 

            `{ "app": "frontend", "env": "production" }`
          </td>

          <td>
            `Map`
          </td>

          <td>
            `{}`
          </td>
        </tr>

        <tr>
          <td>
            `networkMetricsDataFilter.dropEntityName`
          </td>

          <td>
            네트워크 메트릭 데이터를 삭제할 엔티티 이름 목록입니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `[]`
          </td>
        </tr>

        <tr>
          <td>
            `networkMetricsDataFilter.keepEntityName`
          </td>

          <td>
            네트워크 메트릭 데이터를 항상 보관할 엔티티 이름 목록입니다. 기본적으로 모든 엔티티가 유지/활성화됩니다. 이 설정은 

            `dropEntityName`

             필터를 우회합니다.
          </td>

          <td>
            `List`
          </td>

          <td>
            `[]`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="log-data-filters" title="로그 데이터 필터">
    eBPF 에이전트가 수집하고 보고하는 애플리케이션 로그를 제어하도록 필터를 구성합니다. 이 설정은 `reportLogs` 이(가) `"true"` 또는 `"auto"`(으)로 설정된 경우에만 적용됩니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `logDataFilters.applicationLogReporting.enabled`
          </td>

          <td>
            아래 필터와 일치하는 엔티티에서 로그 수집을 활성화합니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `logDataFilters.applicationLogReporting.fileRegex`
          </td>

          <td>
            포함할 로그 파일 이름과 일치하는 정규 표현식입니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `".*.log$"`
          </td>
        </tr>

        <tr>
          <td>
            `logDataFilters.applicationLogReporting.logLevelThreshold`
          </td>

          <td>
            보고할 최소 로그 레벨입니다(예: 

            `TRACE`

            , 

            `DEBUG`

            , 

            `INFO`

            , 

            `WARN`

            , 

            `ERROR`

            ).
          </td>

          <td>
            `String`
          </td>

          <td>
            `"INFO"`
          </td>
        </tr>

        <tr>
          <td>
            `logDataFilters.applicationLogReporting.maxSamplesPerMinute`
          </td>

          <td>
            엔티티에서 분당 수집할 최대 로그 샘플 수입니다. 이 한도에 도달하면 하베스트 주기 전체에 걸쳐 균등한 분포를 유지하기 위해 이벤트가 샘플링됩니다.
          </td>

          <td>
            `Integer`
          </td>

          <td>
            `10000`
          </td>
        </tr>

        <tr>
          <td>
            `logDataFilters.applicationLogReporting.keepStdStreamEntityRegex`
          </td>

          <td>
            STDOUT 및 STDERR에서 로그를 유지하기 위해 엔티티 이름과 일치하는 정규 표현식입니다. 모든 엔티티의 로그를 전달하려면 

            `.*`

             을(를) 사용하십시오.
          </td>

          <td>
            `String`
          </td>

          <td>
            `".*"`
          </td>
        </tr>

        <tr>
          <td>
            `logDataFilters.applicationLogReporting.keepFileEntityRegex`
          </td>

          <td>
            로그 파일에서 로그를 유지하기 위해 엔티티 이름과 일치하는 정규 표현식입니다. 모든 엔티티의 로그를 전달하려면 

            `.*`

             을(를) 사용하십시오.
          </td>

          <td>
            `String`
          </td>

          <td>
            `".*"`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="protocol-tracing" title="프로토콜 추적 설정">
    이 섹션에서는 특정 네트워크 프로토콜에 대한 모니터링을 활성화하고 트레이스 데이터(span)가 수집되는 방식을 구성할 수 있습니다. HTTP, MySQL 등과 같은 프로토콜에 대해 모니터링을 활성화 또는 비활성화하고 지연 시간 또는 오류율을 기반으로 범위 수집에 대해 파장을 설정할 수 있습니다. 다음 프로토콜이 지원됩니다.

    * HTTP
    * 절약
    * MySQL
    * MariaDB
    * Aurora MySQL
    * PostgreSQL
    * MongoDB
    * 아파치 카산드라
    * Redis
    * 다이나모DB
    * MSSQL
    * 카프카
    * AMQP
    * DNS

    두 엔진 모두 MySQL과 동일한 와이어 프로토콜을 사용하므로 MySQL 프로토콜 추적(`protocols.mysql`)은 MariaDB 및 Aurora MySQL도 포함합니다. 설정에는 별도의 `mariadb` 또는 `aurora_mysql` 섹션이 필요하지 않으며, `protocols.mysql`을(를) 활성화하거나 비활성화하면 지원되는 모든 엔진에 대한 추적을 제어합니다. 에이전트는 서버 핸드셰이크 중에 연결당 특정 데이터베이스 종류를 자동으로 감지하고 각 레코드에 보고합니다. 이 감지는 수동으로 설정할 수 없습니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `protocols.global.max_unlinked_spans`
          </td>

          <td>
            프로토콜당 보고되는 연결되지 않은 스팬의 최대 수를 제어합니다. 제한을 비활성화하려면 0으로 설정합니다. 연결되지 않은 스팬은 eBPF 에이전트가 캡처하지만 상위 스팬과 연결할 수 없는 스팬입니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"100"`
          </td>
        </tr>

        <tr>
          <td>
            `protocols.global.unlinked_spans_error_quota_percentage`
          </td>

          <td>
            연결되지 않은 스팬 할당량의 구성 가능한 백분율을 오류 스팬을 위해 예약하여, 정상 스팬이 먼저 도착하더라도 오류 가시성을 보장합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"30"`
          </td>
        </tr>

        <tr>
          <td>
            `protocols.<protocol-name>.enabled`
          </td>

          <td>
            `true`

             인 경우 지정된 프로토콜(예: 

            `http`

            , 

            `mysql`

             및 기타)에 대한 모니터링이 활성화됩니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `protocols.<protocol-name>.spans.enabled`
          </td>

          <td>
            `true`

             인 경우, 활성화된 프로토콜에 대한 트레이스 스팬을 내보냅니다.
          </td>

          <td>
            `Boolean`
          </td>

          <td>
            `true`
          </td>
        </tr>

        <tr>
          <td>
            `protocols.<protocol-name>.spans.samplingLatency`
          </td>

          <td>
            스팬 내보내기에 대한 지연시간 기반 샘플링 임계값을 정의합니다. 

            `p0`

             에서 

            `p99`

            까지의 값을 지원합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"p50"`
          </td>
        </tr>

        <tr>
          <td>
            `protocols.<protocol-name>.spans.samplingErrorRate`
          </td>

          <td>
            HTTP 및 Thrift에서만 사용 가능합니다. 수출은 오류율이 지정된 백분율(1-100%)을 초과하는 모든 경로에서 발생합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"5"`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="daemonset-configs" title="DaemonSet 설정">
    이 섹션에서는 솔루션의 주요 구성 요소에 대한 구현 및 배포 설정을 제어합니다. 별표 `(*)` 구성 요소 이름을 나타냅니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `*.image.repository`
          </td>

          <td>
            구성 요소의 컨테이너 이미지 저장소를 지정합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"docker.io/newrelic/newrelic-ebpf-agent"`
          </td>
        </tr>

        <tr>
          <td>
            `*.image.pullPolicy`
          </td>

          <td>
            컨테이너 이미지에 대한 풀 정책을 정의합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"IfNotPresent"`
          </td>
        </tr>

        <tr>
          <td>
            `*.image.tag`
          </td>

          <td>
            컨테이너 이미지의 버전 태그를 구현하다, 배포하다로 지정합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"agent-0.2.4"`
          </td>
        </tr>

        <tr>
          <td>
            `*.resources.limits.memory`
          </td>

          <td>
            컨테이너가 사용할 수 있는 최대 메모리를 정의합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"2Gi"`
          </td>
        </tr>

        <tr>
          <td>
            `*.resources.requests.cpu`
          </td>

          <td>
            컨테이너가 시작될 때 요청되는 최소 CPU를 정의합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"100m"`
          </td>
        </tr>

        <tr>
          <td>
            `*.resources.requests.memory`
          </td>

          <td>
            컨테이너가 시작될 때 요청되는 최소 메모리를 정의합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"250Mi"`
          </td>
        </tr>

        <tr>
          <td>
            `*.tolerations`
          </td>

          <td>
            특정 오염이 있는 노드에서 스케줄링을 허용하기 위해 파드 허용 범위를 정의합니다.
          </td>

          <td>
            `Objects`
          </td>

          <td>
            `[{"key": "special", "operator": "Exists"}]`
          </td>
        </tr>

        <tr>
          <td>
            `*.affinity`
          </td>

          <td>
            스케줄링을 위한 파드 친화성 및 반친화성 규칙을 정의합니다.
          </td>

          <td>
            `Object`
          </td>

          <td>
            `{}`
          </td>
        </tr>

        <tr>
          <td>
            `*.podAnnotations`
          </td>

          <td>
            컴포넌트에 추가할 사용자 지정 주석을 지정합니다.
          </td>

          <td>
            `Object`
          </td>

          <td>
            `{"iam.amazonaws.com/role": "my-role"}`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>

  <Collapser id="global-pod-scheduling" title="글로벌 패드 및 일정 설정">
    이러한 기준은 구성 요소별 설정으로 재정의되지 않는 한 Helm 차트를 통해 모든 파드 구현, 배포에 적용됩니다.

    <table>
      <thead>
        <tr>
          <th>
            매개변수
          </th>

          <th>
            설명
          </th>

          <th>
            데이터 형식
          </th>

          <th>
            예시
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `podLabels`
          </td>

          <td>
            차트별로 모든 파드 구현하다, 배포하다에 적용할 추가 레이블을 지정합니다.
          </td>

          <td>
            `Object`
          </td>

          <td>
            `{"team": "observability"}`
          </td>
        </tr>

        <tr>
          <td>
            `priorityClassName`
          </td>

          <td>
            모든 파드에 대해 

            `PriorityClass`

             을 지정합니다.
          </td>

          <td>
            `String`
          </td>

          <td>
            `"high-priority"`
          </td>
        </tr>

        <tr>
          <td>
            `nodeSelector`
          </td>

          <td>
            레이블이 일치하는 노드에서만 실행되도록 제한합니다.
          </td>

          <td>
            `Object`
          </td>

          <td>
            `{"disktype": "ssd"}`
          </td>
        </tr>
      </tbody>
    </table>
  </Collapser>
</CollapserGroup>

## eBPF 에이전트 제거 [#uninstall]

쿠버네티스 클러스터에서 eBPF 에이전트를 제거하려면:

```bash
helm uninstall nr-ebpf-agent -n newrelic
```

<Callout variant="tip">
  이 명령을 실행하면 클러스터에서 모든 eBPF 에이전트 구성 요소가 제거됩니다. 네임스페이스는 명시적으로 삭제하지 않는 한 그대로 유지됩니다.
</Callout>

<DocTiles>
  <DocTile title="eBPF Linux 설치" path="/docs/ebpf/linux-installation/">
    Linux 호스트에 뉴렐릭 eBPF 에이전트를 설정하는 방법을 알아보세요.
  </DocTile>

  <DocTile title="문제 해결, eBPF 해결" path="/docs/ebpf/troubleshooting/no-ui-data/">
    뉴렐릭 eBPF 에이전트 관련 문제를 해결하는 방법을 알아보세요.
  </DocTile>

  <DocTile title="eBPF 모범 사례" path="/docs/ebpf/best-practices/">
    뉴렐릭 eBPF 에이전트 사용 우수 우수 사례에 대해 알아보세요.
  </DocTile>
</DocTiles>