  ...
  <TabsPages>
    ...
    <TabsPageItem id="linux">
      ...
      * 리눅스 커널 버전:
        ...
        * `x86-64` (amd64): `v5.8` 을(를) 다음으로 `v7.1`
        * `arm64` (aarch64): `v5.8` 에서 `v7.1`

      <Callout variant="important">
        일부 Linux 배포판에서는 최신 OS 릴리스에서 이전 커널 버전을 실행할 수 있습니다. 배포판이 [아래에 나열되어](#supported-linux-distributions) 있더라도 커널 버전이 `5.8`보다 낮으면 eBPF 에이전트가 작동하지 않습니다. 최신 커널에도 동일하게 적용됩니다: 에이전트는 지원되는 범위 이상의 버전에서는 작동하지 않습니다.
      </Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `otlp.eu01.nr-data.net:443` (EU 리전)
        * `otlp.jp.nr-data.net:443` (JP 리전)
        * `gov-otlp.nr-data.net:443` (GOV/FedRAMP 계정)
...
환경에서 기업용 HTTP 프록시를 통해 아웃바운드 트래픽을 라우팅하는 경우, 설정 지침은 [<DNT>eBPF agent</DNT>용 프록시 구성](/docs/ebpf/proxy-configuration) 을 참조하세요.
