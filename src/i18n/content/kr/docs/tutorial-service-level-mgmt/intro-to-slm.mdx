...
서비스 수준 관리는 **업타임, 성능 및 신뢰성** 관행에 가장 잘 적용되지만 **고객 경험**, **혁신 및 성장**, **운영 효율성** 등 다른 관행에도 적용됩니다([이러한 관행에 대해 자세히 알아보십시오](/docs/new-relic-solutions/observability-maturity/introduction)). 개선하려는 영역이 무엇이든 SLM을 사용하면 비즈니스 성과와 운영 결과라는 두 가지 주요 영역에 영향을 미칠 수 있습니다.
...
## 서비스 수준 관리를 사용하는 이유는 무엇입니까? [#why-slm]
...
일반적인 방법은 먼저 하나의 디지털 제품에 대한 출력 성능과 입력 성능 서비스 레벨과 그 핵심 성능을 확립하는 것입니다. 여기에는 일반적으로 각 엔드포인트 애플리케이션(보통 1개 또는 2개)에 대해 하나의 [전체 출력](/docs/tutorial-service-level-mgmt/establish-output-sli) 및 [입력 서비스 레벨이](/docs/tutorial-service-level-mgmt/establish-input-sli) 포함되며, 엔드포인트 트랜잭션에서 측정된 가정된 중요 기능에 대해 약 4-7개의 출력 성능 서비스 레벨이 포함됩니다.
...
<Collapser
  ...
  title="서비스 수준"
>
  [서비스 레벨은](https://en.wikipedia.org/wiki/Service_level) 시스템의 성능을 측정합니다. 지표를 사용하고 이를 예상 성능 및 결과에 대한 일련의 목표와 비교하여 서비스 레벨을 측정합니다.
</Collapser>
...
<Collapser
  ...
  title="오류 예산"
>
  오류 예산은 SLO(서비스 목표) 준수 여부를 측정하고 전달합니다. [서비스 수준 관리 문서](/docs/service-level-management/intro-slm) 에 이 고급 방법에 대한 자세한 정보가 있습니다.
...
* [APM 측정](/docs/apm/new-relic-apm/getting-started/introduction-apm) 등을 통해 기본 애플리케이션에 대한 SLI를 모니터링하고 설정할 데이터가 있습니다.
* 또한 애플리케이션의 [신세틱 테스트를](/docs/synthetics/synthetic-monitoring/getting-started/get-started-synthetic-monitoring) 사용하여 고객에게 의존하지 않고 모니터링할 데이터를 생성할 수도 있습니다.
* 필수는 아니지만 뉴렐릭 [대시보드와 NRQL을](https://learn.newrelic.com/webinar-getting-started-with-dashboards-nrql) 통해 기본 기술을 습득하는 것이 좋습니다.

## 이 시리즈에는 무엇이 있습니까? [#the-series]
...
<DocTiles>
  ...
  >
    SLM 기술을 사용하여 신뢰성을 향상시키는 방법을 알아보세요.
  </DocTile>
</DocTiles>