...
<Collapser
  id="broker-legacy"
  title="서비스 브로커 v1.12.12 이하의 키 가져오기"
>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        2.10.x, 2.11.x, 2.12.x, 2.13.x, 3.x 및 4.x
      </td>
    </tr>
...
3. 이 제품을 준비 영역에 추가하려면 Ops Manager 왼쪽 탐색 보기에서 업로드된 제품 설명 옆에 있는 **+** 기호 또는 **Add next** \[추가를] 클릭하세요.
...
2. **Import a Product** \[제품 가져오기를] 클릭하고 VMware Tanzu용 New Relic Service Broker 타일을 가져옵니다.
...
3. 업로드한 “New Relic Service Broker” 오른쪽에 있는 **+** 버튼을 클릭하세요.
...
16. 애플리케이션을 다시 준비하십시오. 바인딩 페이지 상단이나 터미널 창에서 링크를 선택하고 cf restage를 실행하여 변경할 수 있습니다.
...
`JAVA\_OPTS` 환경 변수를 사용하여 스테이징 환경 그룹에서 New Relic Agent 관련 환경 변수를 지정하면 Java 빌드팩이 이를 사용하고 올바른 위치에 푸시할 수 있습니다. 런타임 환경 변수 그룹에 JAVA_OPTS를 지정하면 빌드팩이 이에 대해 알지 못하므로 아무 것도 산출되지 않습니다.
...
자세한 내용은 [구성 설정 우선 순위를](https://docs.newrelic.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#config-options-precedence) 참조하세요.
