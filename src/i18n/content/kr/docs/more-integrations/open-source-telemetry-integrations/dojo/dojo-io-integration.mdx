...
New Relic의 Dojo 대시보드를 사용하면 애플리케이션의 핵심 웹 바이탈, 사용자가 사이트에서 소비하는 시간, 초기 페이지 로드 및 Javascript 오류가 있는 페이지와 같은 중요한 브라우저 성능 지표를 추적할 수 있습니다. 우리의 <InlinePopover type="browser"/>에이전트인 경우 Dojo 대시보드를 사용하면 가장 중요한 모든 데이터를 한 곳에서 볼 수 있습니다.
