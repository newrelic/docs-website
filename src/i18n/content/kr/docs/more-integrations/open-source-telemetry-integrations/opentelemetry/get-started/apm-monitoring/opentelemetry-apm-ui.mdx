...
OpenTelemetry APM UI는 [서비스 엔터티](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services) 에 사용할 수 있는 선별된 환경입니다. 뉴렐릭 및 OpenTelemetry 로 서비스를 구성하지 않은 경우 [OpenTelemetry APM 모니터링을](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-intro/) 참조하세요.
...
엔터티 탐색기 내에서 [엔터티 태그](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) 별로 필터링할 수 있습니다. 엔터티 태그가 컴퓨트인 방법에 대한 자세한 내용은 [뉴렐릭의OpenTelemetry 리소스를](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources) 참조하세요.
...
* **제외**: 서비스가 수평으로 확장될 때 골든 시그널이 제외됩니다. `service.instance.id` 리소스 속성에 따라 다릅니다(자세한 내용은 [서비스 엔터티 속성](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services) 참조).

## 페이지: 분산 추적 [#distributed-tracing-page]

혐의 추적 페이지에서는 OpenTelemetry Traces 데이터에 대한 자세한 인사이트를 제공합니다. 페이지 사용 정보는 [분산 추적을](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/) 참조하세요. Traces 데이터가 [OpenTelemetry 뉴렐릭으로 수집되는](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-traces) 방법에 대한 자세한 내용은 뉴렐릭의 Traces를 참조하세요.OpenTelemetry
...
JVM 런타임 페이지는 Java 서비스의 JVM 문제를 식별하고 분석하기 위한 도구를 제공합니다. 이 페이지는 [OpenTelemetry java를](https://opentelemetry.io/docs/languages/java/) 사용하는 서비스에 대해서만 표시됩니다. 고유한 서비스를 구별하려면 페이지에서 `service.instance.id` 리소스 속성을 설정해야 합니다(자세한 내용은 [서비스 엔터티 속성](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services) 참조).
...
Go 런타임 페이지는 Go 서비스의 런타임 문제를 식별하고 분석하기 위한 도구를 제공합니다. 이 페이지는 [OpenTelemetry Go를](https://opentelemetry.io/docs/languages/go/) 사용하는 서비스에만 표시됩니다. 고유한 서비스를 구별하려면 페이지에서 `service.instance.id` 리소스 속성을 설정해야 합니다(자세한 내용은 [서비스 엔터티 속성](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-resources/#services) 참조).
