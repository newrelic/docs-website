...
* 단순 누적 합계 매핑은 다음에 따라 달라집니다.

  * [cumulativeCount](/docs/telemetry-data-platform/understand-data/metric-data/metric-data-type/#metric-types) 지표 유형에 대한 액세스 권한이 있는 경우 이것이 사용됩니다. 변환 프로세스에 대한 자세한 내용은 [누적 메트릭](/docs/data-apis/understand-data/metric-data/cumulative-metrics) 을 참조하십시오.
  * `cumulativeCount` 에 대한 액세스 권한이 없으면 [`gauge` 유형](/docs/data-apis/understand-data/metric-data/cumulative-metrics) 에 매핑됩니다.
...
[`cumulativeCount`](/docs/data-apis/understand-data/metric-data/cumulative-metrics) 에 대한 액세스 권한이 있으면 누적 히스토그램이 [델타 히스토그램](#delta-histogram) 으로 변환됩니다. 번역 프로세스에 대한 자세한 내용은 [누적 메트릭](/docs/data-apis/understand-data/metric-data/cumulative-metrics) 을 참조하십시오.
