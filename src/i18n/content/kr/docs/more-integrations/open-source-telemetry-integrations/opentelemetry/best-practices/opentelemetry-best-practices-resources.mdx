...
OpenTelemetry의 모든 데이터는 [리소스](https://opentelemetry.io/docs/concepts/resources) 와 연결됩니다. 자원은 텔레메트리를 자원 속성으로 생산하는 엔터티의 표현입니다. 리소스 속성은 뉴렐릭의 [엔터티](docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic) 개념에 리소스를 적용하는 데 사용됩니다.
...
OpenTelemetry를 사용하여 서비스 엔터티를 모니터링하려면 [설명서와 예제를](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/opentelemetry-set-up-your-app) 참조하세요.

#### 필수 속성

* [`service.name`](https://opentelemetry.io/docs/specs/semconv/attributes-registry/service): UI 에 표시되는 엔터티 이름은 이 값에서 따온 것입니다.
* [`host.name`](https://opentelemetry.io/docs/specs/semconv/attributes-registry/host/): UI에 표시되는 호스트 이름은 이 값에서 나옵니다.

#### 권장 속성 [#recommended-attributes]

* [`service.instance.id`](https://opentelemetry.io/docs/specs/semconv/attributes-registry/service)

  * 동일한 서비스의 여러 인스턴스 간 패싯을 활성화합니다.

* [`telemetry.sdk.language`](https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry)

  * 존재하는 경우 런타임별 UI의 표시를 구동합니다. 예를 들어 Java 애플리케이션용 JVM 런타임 페이지가 있습니다.
...
OpenTelemetry 수집기용 호스트 메트릭 수신기를 사용하여 호스트 엔터티를 모니터링하는 방법은 [설명서를](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/collector/opentelemetry-collector-infra-hosts) 참조하세요.
