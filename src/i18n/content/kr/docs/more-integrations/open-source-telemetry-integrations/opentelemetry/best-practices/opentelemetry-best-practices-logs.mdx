...
애플리케이션 로그는 애플리케이션에서 생성된 다른 텔레메트리 데이터와 상호 연관이 되면 더 유용해집니다. [서비스에 대한 OpenTelemetry 시맨틱 규칙은](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/README.md) `service.name`을 필수 필드로 지정합니다. 동일한 `service.name`과 함께 뉴렐릭으로 전송된 모든 애플리케이션 메트릭, 트레이스 및 로그 데이터는 동일한 [엔터티](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic)와 연관됩니다.
...
* 전용 [Collector 에이전트](https://opentelemetry.io/docs/collector/getting-started/#agent) 인스턴스와 함께 애플리케이션을 배포할 수 있습니다. 이 인스턴스는 `service.name` 속성으로 로그에 주석을 추가하도록 [리소스 프로세서](https://github.com/open-telemetry/opentelemetry-specification/tree/main/specification/resource)로 구성할 수 있습니다.

선택적으로 추가 애플리케이션 [트레이스 컨텍스트](https://github.com/open-telemetry/opentelemetry-specification/tree/main/specification/trace/sdk_exporters)(실행 컨텍스트라고도 함)를 로그 메시지로 전달할 수 있습니다. 그 설정 및 가용성은 애플리케이션에서 사용하는 언어 및 로깅 프레임워크에 따라 다릅니다. 일반적인 전략은 정형화된 JSON 로그를 작성하도록 애플리케이션을 설정하고 트레이스 컨텍스트를 사용 가능한 로그 메시지에 지정된 [트레이스 컨텍스트 필드](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/logs/data-model.md#trace-context-fields)로 추출하도록 구성하는 것입니다. 보다 자세한 내용은 [UI의 OpenTelemetry: 로그 페이지](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/view-your-data/opentelemetry-logs-page)를 참조하십시오.

[GitHub의 Log4j2와 문맥적 로그 예시](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/java/logs-in-context-log4j2)는 Log4j2를 사용하는 간단한 Java 애플리케이션에 대한 포괄적인 작업 예시를 보여줍니다.
