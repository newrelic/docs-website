...
맵 또는 엔터티 관계를 표시하는 다른 기능으로는 **Related entities** \[관련 엔터티] 및 **Trace map** \[추적 맵이] 있습니다.

## 종속성 [#dependencies]

New Relic 에이전트: ✅ New Relic의 OpenTelemetry: ❌

OpenTelemetry, **Service map** \[서비스 맵] 또는 **Related entities** \[관련 엔터티] 의 경우 보기를 통해 특정 서비스에 대한 모든 종속성을 확인할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    OpenTelemetry에는 New Relic과 동등한 것이 없습니다. <InlinePopover type="browser"/>New Relic APM 에이전트와 함께 사용할 수 있는 에이전트 또는 브라우저 계측. OpenTelemetry-JavaScript SDK를 사용하는 경우 현재 최종 사용자로부터 브라우저 데이터를 생성하고 수집하는 데 사용할 수 있는 자동 계측 라이브러리가 있습니다. iOS 및 Android 애플리케이션(Java 및 Swift)과 브라우저 애플리케이션(JavaScript)을 작성하는 데 사용할 수 있는 언어의 경우 클라이언트 측 원격 측정을 계측하는 사양이 있지만 아직 사용할 수 없습니다.
...
구성할 수 있습니다 <InlinePopover type="alerts"/>NRQL로 작성된 조건을 사용하여 OpenTelemetry 데이터에 대해 시작하려면 차트의 "**...** " 메뉴를 클릭하세요.
...
<CollapserGroup>
  ...
  >
    ...
    New Relic 에이전트의 높은 보안 모드 기능으로 시행되는 데이터에 대한 제한은 OpenTelemetry Collector를 구성하거나 수정하여 시행할 수 있지만, OTLP가 그러한 제어를 지정하지 않기 때문에 New Relic에서는 이를 시행할 수 없습니다.
  </Collapser>
