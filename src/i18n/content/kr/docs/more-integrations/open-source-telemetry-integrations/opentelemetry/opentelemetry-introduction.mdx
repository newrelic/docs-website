...
OpenTelemetry 로그는 New Relic의 로그와 호환됩니다. OpenTelemetry 로그에는 선택적으로 쿼리를 통해 로그 데이터를 패싯하거나 필터링하는 데 사용할 수 있는 차원에 직접 매핑되는 속성(이름-값 쌍) 및 리소스 속성이 포함됩니다. OpenTelemetry 로그 메타데이터(예: `name`, `severity_text` 및 `trace_id`)도 New Relic의 차원에 직접 매핑됩니다. <InlinePopover type="logs"/>능력. 현재 모든 OpenTelemetry 로그 메시지 유형을 지원합니다.
