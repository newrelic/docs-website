...
OpenTelemetry는 서비스 및 호스트 계측을 위한 안전한 벤더 중립적 [사양을](https://github.com/open-telemetry/opentelemetry-specification) 제공하므로 New Relic과 같이 선택한 고유한 백엔드로 데이터를 내보낼 수 있습니다. OpenTelemetry는 서비스 및 호스트에 대한 원격 측정 데이터를 수집하고 전송하는 방법을 표준화하는 단일 API 및 라이브러리 세트를 제공합니다.
...
OpenTelemetry 고려할 때, 텔레메트리 데이터도 캡처하는 뉴렐릭 <InlinePopover type="apm"/>에이전트를 살펴보실 수도 있습니다.
...
일반적으로 뉴렐릭의 <InlinePopover type="apm"/>및 서비스 에이전트는 서비스 및 호스트에 대한 더 많은 텔리메트리 데이터를 수집합니다. 또한 광범위한 설정 옵션과 광범위한 자동 측정, 자동 로그 기능 세트를 제공합니다.
...
<Callout variant="important">
  뉴렐릭의 OpenTelemetry 내보내기 기능은 이제 OTLP를 사용하여 뉴렐릭으로 데이터를 내보내는 대신 더 이상 사용되지 않습니다.
</Callout>
...
New Relic은 메트릭 신호의 OTLP 수집을 지원합니다.

다음은 우리가 지원하는 OpenTelemetry 데이터 유형과 관련 매핑입니다. 자세한 내용은 [OpenTelemetry 측정항목: 모범 사례를](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-metrics) 참조하세요.
...
New Relic은 로그 신호의 OTLP 수집을 지원합니다. 업스트림 사양의 성숙도는 [안정적](https://github.com/open-telemetry/opentelemetry-specification/blob/87a5ed7f0d4c403e2b336f275ce3e7fd66a8041b/specification/versioning-and-stability.md#stable) 입니다.

OpenTelemetry 로그는 뉴렐릭의 로그와 호환됩니다. OpenTelemetry 로그에는 쿼리를 사용하여 로그 데이터를 패싯하거나 필터링하는 데 사용할 수 있는 차원에 직접 매핑되는 속성(이름-값 쌍) 및 리소스 속성이 선택적으로 포함됩니다. OpenTelemetry 로그데이터(예: `name`, `severity_text` 및 `trace_id`)도 뉴렐릭의 <InlinePopover type="logs"/>기능 차원에 직접 매핑됩니다. 현재 모든 OpenTelemetry 로그 메시지 유형을 지원합니다.
