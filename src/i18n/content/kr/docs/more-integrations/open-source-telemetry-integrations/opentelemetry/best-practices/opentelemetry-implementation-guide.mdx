...
상위 수준에서 경고는 하나 이상의 경고 조건을 포함하는 경고 정책을 통해 구성됩니다. 경보 조건의 인시던트는 문제로 함께 그룹화됩니다. 마지막으로 이슈는 이슈를 알림으로 전환하는 논리를 포함하는 워크플로를 트리거할 수 있습니다. 이러한 개념을 이해하는 데 도움이 되는 다이어그램은 [경보 개념을](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/basic-alerting-concepts) 참조하십시오.
