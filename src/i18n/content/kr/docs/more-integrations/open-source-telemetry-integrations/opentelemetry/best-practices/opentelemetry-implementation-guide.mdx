...
* <DoNotTranslate>**Use the right endpoint**</DoNotTranslate>
  ...
  . OTLP 데이터를 엔드포인트로 내보내도록 구성해야 합니다. 이에 대해서는 [OpenTelemetry 수집기에서 데이터 전송에 대한 문서](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/opentelemetry-set-up-your-app/#review-settings) 에 설명되어 있습니다.

* <DoNotTranslate>**Avoided dropped data**</DoNotTranslate>

  .데이터가 삭제되는 것을 방지하려면 길이가 4095자를 초과하는 속성을 [자르는](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-otlp/#attribute-limits) 것이 좋습니다. 이러한 사례를 적용하는 예는 [이 수집기 구성을](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/collector/nr-config) 참조하세요.

* <DoNotTranslate>**Infrastructure**</DoNotTranslate>

  . 지표를 수집하려면 수집기 구성에 [호스트 지표 수신기를](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/collector/opentelemetry-collector-infra-hosts) 포함하세요. 수집기 설정의 일부로 호스트 수신기를 사용하는 경우 호스트 엔터티의 일부로 호스트 지표를 자동으로 감지하고 해당 골든 지표를 합성합니다(간단히 말하면 우리의 클러스터 에이전트와 동일한 경험을 얻습니다).

* <DoNotTranslate>**Scaling**</DoNotTranslate>

  . 대규모 OpenTelemetry 배포에서는 성능과 복원력 이점 모두를 위해 수집기 확장 전략을 선택해야 합니다. 수집기 크기 조정에 대한 자세한 내용은 [OpenTelemetry 문서를](https://opentelemetry.io/docs/collector/scaling) 참조하세요.

* <DoNotTranslate>**Kubernetes**</DoNotTranslate>

  .OpenTelemetry OpenTelemetry-계측된 서비스가 Kubernetes 환경에서 실행되고 [OpenTelemetry있는 경우 계측된 서비스를 에 연결 의 Kubernetes](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/link-otel-applications-kubernetes) 단계를 따르세요. 이렇게 하면 Kubernetes 데이터가 OpenTelemetry 데이터와 상호 연관됩니다.

많은 고객이 에이전트 및 게이트웨이 배포 방법을 모두 사용하여 수집기를 실행하도록 선택합니다.

* <DoNotTranslate>**Agent**</DoNotTranslate>

  : 이 방법을 사용하면 수집기가 애플리케이션(예: 바이너리, 사이드카 또는 데몬셋)과 동일한 호스트에서 실행됩니다.

* <DoNotTranslate>**Gateway**</DoNotTranslate>

  : 이 방법을 사용하면 클러스터, 데이터센터 또는 지역별로 하나 이상의 수집기 제외가 독립형 서비스로 실행됩니다.
...
상위 수준에서 <InlinePopover type="alerts"/>(는) 하나 이상의 공지 조건을 포함하는 공지를 통해 구성됩니다. 공지 조건의 인시던트는 이슈로 그룹화되어 있습니다. 마지막으로 이슈는 이슈를 공지로 바꾸는 로직을 포함하는 플로우를 촉발할 수 있습니다. 이러한 개념을 이해하는 데 도움이 되는 다이어그램을 보려면 [경고 개념 을](/docs/alerts-applied-intelligence/overview/#concepts-terms) 참조하세요.
...
OpenTelemetry 서비스를 관련 엔터티와 함께 그룹화하는 워크로드를 만드는 것이 좋습니다. 여기에는 NVIDIA의 엔터티 모니터, <InlinePopover type="apm"/>, Kubernetes 모니터링, <InlinePopover type="browser"/>및 기타 엔터티가 포함될 수 있습니다. 워크로드를 사용하면 엔터티를 보다 관리하기 쉬운 단위로 그룹화할 수 있으며 일반적으로 해당 엔터티를 담당하는 팀과 연계됩니다. 이는 수천 개의 모니터링되는 엔터티가 있을 수 있는 대규모 환경에 특히 유용합니다.
