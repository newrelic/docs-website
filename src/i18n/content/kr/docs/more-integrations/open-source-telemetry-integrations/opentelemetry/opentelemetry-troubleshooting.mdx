...
* 구성된 OTLP 엔드포인트는 [문서화된 엔드포인트](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-otlp) 중 하나와 일치하고 형식이 적절하며 [OTLP/gRPC의 기본 포트인](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.8.0/specification/protocol/otlp.md#otlpgrpc-default-port) 4317 또는 [OTLP/HTTP의 기본 포트인](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.8.0/specification/protocol/otlp.md#otlphttp-default-port) 4318을 포함합니다. 두 전송 모두에 대해 포트 443도 지원됩니다. 해당하는 경우 FedRAMP 규정 준수에 대한 특정 엔드포인트를 참고하세요.
...
* 클라이언트는 TLS 1.2 이상을 사용하도록 구성되었으며 요청에는 유효한 뉴렐릭

  <InlinePopover type="licenseKey"/>

  이 있는 `api-key` 헤더가 포함되어 있습니다.
...
<InlinePopover type="logs"/>의 경우 구조화된 로그 템플릿을 사용하여 `service.name` 을 삽입할 수 있습니다. 예제는 [Log4j2를 사용한 컨텍스트의 로그를](https://github.com/newrelic/newrelic-opentelemetry-examples/blob/e3f5ee85b4dcd8dd29f8f69d78d122b82a9638ba/other-examples/java/logs-in-context-log4j2/Log4j2EventLayout.json#L2) 참조하세요.
