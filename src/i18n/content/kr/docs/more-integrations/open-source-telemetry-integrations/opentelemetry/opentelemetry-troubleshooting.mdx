...
원격 측정 데이터가 당사 플랫폼에 성공적으로 전달되었는지 확인하는 데 사용할 수 있는 도구가 많이 있습니다. 좋은 첫 번째 단계는 [데이터 관리 허브](/docs/data-apis/manage-data/manage-your-data/) 를 확인하여 [데이터 수집을 패싯](/docs/data-apis/manage-data/manage-data-coming-new-relic/#facet-data-ingest) 하고 다양한 소스에서 도착하는 데이터의 양을 확인하는 것입니다. [데이터 탐색기](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/#use-case-troubleshooting) 또는 [쿼리 빌더](/docs/query-your-data/explore-query-data/query-builder/use-advanced-nrql-mode-query-data) 를 사용하여 `instrumentation.provider` 또는 `newrelic.source` 속성으로 분류된 데이터를 찾을 수도 있습니다.
