...
2. 다음과 같이 변경한 후 OpenTelemetry 수집기를 실행합니다.
   ...
   * `OTLP_ENDPOINT_HERE` 을 적절한 [엔드포인트](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-setup#review-settings) 로 바꿉니다.
   * `YOUR_KEY_HERE` 을 계정의 [라이선스 키로 바꾸](https://one.newrelic.com/launcher/api-keys-ui.launcher) 십시오.
