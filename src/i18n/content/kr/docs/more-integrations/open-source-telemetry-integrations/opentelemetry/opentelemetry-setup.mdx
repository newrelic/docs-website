...
2. 통합을 기반으로 내보내기가 New Relic으로 데이터를 보내는 엔드포인트를 구성할 준비를 하십시오. 엔드포인트는 지역별로 다르므로 계정 기반 위치에 따라 엔드포인트를 사용하세요. 예를 들어, 계정이 유럽에 있는 경우 EU 엔드포인트를 사용하십시오. 아래 표를 지침으로 사용하고 다음에 나오는 [포트](#note-ports) 및 [엔드포인트](#note-endpoints) 에 대한 추가 고려 사항을 확인하십시오.
