...
<Steps>
  ...
  <Step>
    ...
    * OTel 수신기는 Redis 측정항목에 식별자를 연결하지 않으므로 측정항목을 `server.address` 및 `server.port` 메타데이터로 장식하려면 속성 프로세서가 필요합니다. Redis 서버를 구별하는 데이터 패싯을 쿼리하고 New Relic 엔터티를 생성하려면 `server.address` 및 `server.port` 속성이 필요합니다.
