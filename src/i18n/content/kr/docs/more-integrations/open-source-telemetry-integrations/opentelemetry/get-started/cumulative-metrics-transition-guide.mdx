...
내보내기 const 연도 = 2023 내보내기 const gaDate = '4월 4일' 내보내기 const eolDate = '7월 3일' 내보내기 const gaDateAndYear = gaDate + ', ' + 연도 내보내기 const eolDateAndYear = eolDate + ', ' + 연도
...
gaDate 이전에 누적 단순 합계를 수집한 경우 이 90일 기간 동안 영향을 받는 메트릭을 사용하는 쿼리 및 알림을 업데이트할 수 있습니다.

아래 섹션에서 다음 내용을 배울 수 있습니다.

* 계정이 영향을 받았는지 확인하는 방법
* EoL을 준비하는 방법
...
## 계정에 미치는 영향 확인 [#impact]

New Relic 계정이 gaDateAndYear 이전에 New Relic에 OTLP 누적 합계 메트릭을 보낸 경우 eolDateAndYear에 대한 EoL의 영향을 받습니다.

계정이 영향을 받았는지 확인하려면 다음 NRQL 쿼리를 실행하십시오.
