...
이는 New Relic에 데이터를 보내고 UI에서 보는 것이 어떤 것인지 빠르게 확인할 수 있도록 계측 작업을 수행하려는 경우 훌륭한 옵션입니다.
...
2. 다음 두 환경 변수를 설정하여 New Relic 계정으로 데이터를 보내십시오.
   ...
   * [입수 라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 를 사용해야 합니다.
...
1. 왼쪽 창의 **모니터 **섹션에서 **분산 추적 **을 클릭한 다음 **피보나치 **추적 그룹을 클릭합니다.
...
3. 추적이 열리면 `Show in-process spans` 를 클릭한 다음 결과 스팬을 클릭하면 오른쪽에 세부정보 패널이 열립니다. 사용자 입력이 유효하지 않을 때 기록한 예외를 보려면 `View span events` 을 클릭하십시오.
      ...
      <tbody>
        <tr>
          <td>
            ...
            * 헤더: 데이터를 보낼 New Relic 계정을 결정합니다. `YOUR_LICENSE_KEY` 를 [수집 라이선스 키로](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 바꿉니다.
          </td>
        </tr>
