...
이 가이드에서는 코드 품질을 향상시키는 데 도움이 되는 특정 핵심 성과 지표와 프로세스를 식별합니다. 그 다음에는 [릴리스 품질 가이드가](/docs/new-relic-solutions/observability-maturity/innovation-growth/release-quality-implementation-guide) 따르며, 이 가이드에 나와 있는 단계를 완료한 후에만 수행할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 빌드 성공률을 거의 100%로 높입니다.

    <DoNotTranslate>**Best practices:**</DoNotTranslate> 코드 빌드 이벤트에는 다음 속성이 포함되어야 합니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 테스트 성공률을 높입니다.

    <DoNotTranslate>
      **Best practices:**
    </DoNotTranslate>

    단위 테스트는 다음을 수행해야 합니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 코드 적용 범위를 100%로 늘립니다.

    <DoNotTranslate>**Best practices:**</DoNotTranslate> 코드 테스트 적용 범위가 100% 목표에 도달하도록 [단위 테스트](#kpi-unit-test-success) 모범 사례를 따르세요.
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 새로운 결함 수를 줄입니다. <DoNotTranslate>**Best practices:**</DoNotTranslate> 특정 서비스 및 코드 모듈과 관련시켜 새로운 결함의 원인을 식별합니다.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 속도와 안정성 사이의 최적의 균형을 식별하려면 코드 커밋 비율을 결함 양과 연관시켜야 합니다.

    <DoNotTranslate>**Best practices:**</DoNotTranslate> 코드 커밋 이벤트에는 다음 속성이 포함되어야 합니다.
...
<Steps>
  <Step>
    ...
    위의 [핵심 성과 지표](#key-perf-indicators) 섹션에서 이 사용 사례에 필요한 KPI 및 최소 필수 속성을 찾을 수 있습니다. 일반적으로 개발 도구 체인의 API를 사용하여 KPI와 해당 속성을 추출한 다음 [사용자 정의 이벤트 API](/docs/data-apis/ingest-apis/introduction-event-api/) 를 사용하여 New Relic에 제출합니다.
  ...
  <Step>
    ...
    <InlinePopover type="dashboards"/>이 개선 프로세스에서 사용하는 기본 방법입니다. 현재 KPI를 보여주고 개선이 필요한 영역을 식별하는 데 도움이 됩니다. [GitHub의 뉴렐릭 OMA 리소스 센터에서](https://github.com/newrelic/oma-resource-center) 샘플 대시보드를 찾을 수 있습니다.
...
<UserJourneyControls nextStep={{"path":"/docs/tutorial-innovation-and-growth/release-quality-mgmt/","title":"다음 단계","body":"New Relic을 사용하여 릴리스 품질을 향상시키는 방법을 알아보세요."}}/>