...
Google의 DORA( [DevOps Research and Assessment](https://www.devops-research.com/research.html) ) 팀은 소프트웨어 개발 조직의 성능을 나타내는 4가지 핵심 지표를 식별했습니다. 우리의 <DoNotTranslate>**Innovation and Growth**</DoNotTranslate> 가치 동인은 이러한 지표를 사용하여 더욱 안정적인 기능과 함께 보다 효율적이고 대응력이 뛰어난 개발 팀을 만드는 전체 프로그램을 만듭니다. 이 릴리스 품질 가이드는 배포 빈도, 애플리케이션 성능 및 애플리케이션 안정성을 향상하는 데 도움이 됩니다.
...
트렁크 기반 개발은 "개발자가 <DoNotTranslate>**trunk**</DoNotTranslate> 이라는 단일 분기에서 코드에 대해 공동 작업하는 소스 제어 분기 모델은 문서화된 기술을 사용하여 수명이 긴 다른 개발 분기를 생성하라는 압력에 저항합니다."로 정의됩니다. 즉, 개발 작업을 단일 트렁크의 분기에 대해 수행되는 작은 배치로 나눕니다. 하나의 작업 배치가 완료되자마자 해당 분기가 다시 트렁크로 병합됩니다. 각 브랜치는 수명이 짧기 때문에 트렁크로 다시 병합하는 것이 간단하고 모든 개발자가 코드베이스의 최신 릴리스에서 작업할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 이 릴리스가 새로운 릴리스인지 롤백 버전인지를 나타내는 표시기
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 릴리스 속도를 높입니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 각 릴리스의 크기/범위를 줄입니다.
  ...
  >
    ...
    * 요청을 처리한 컴퓨팅 리소스
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 응답 시간을 개선하고 오류를 줄입니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 실패한 릴리스 수를 줄입니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 100% 적용 범위
  ...
  >
    ...
    * 심각도별 평균 마감 시간
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 지원포켓 수를 줄이세요.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 예측 가능하고 안정적인 비용 모델을 만듭니다.
