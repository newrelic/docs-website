...
에이전트를 설치한 후 **[one.newrelic.com](https://one.newrelic.com/nr1-core?filters=(domain%3D'APM'ANDtype%3D'APPLICATION'))** 으로 이동하여 앱을 선택하세요. 아직 많은 데이터가 표시되지 않으면 잠시 물러나 애플리케이션이 실행될 때 에이전트가 실시간 데이터를 수집하도록 하세요. 또한 이 튜토리얼에서는 귀하가 다음 사항에 어느 정도 익숙하다고 가정합니다. <InlinePopover type="alertsTutorial"/>, 아직 첫 번째 알림을 만들지 않은 경우에도 마찬가지입니다.
