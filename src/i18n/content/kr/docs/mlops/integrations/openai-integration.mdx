...
<Callout variant="important">
  OpenAI 기반 앱을 계측하려면 이 이전 설치 방법보다 AI 모니터링을 사용하는 것이 좋습니다. [시작하려면 문서를 참조하십시오](/docs/ai-monitoring/intro-to-ai-monitoring/).
</Callout>
...
<CollapserGroup>
  <Collapser id="install-with-python" title="Python 애플리케이션용으로 설치">
    <Steps>
      <Step>
        ### pip로 OpenAI 설치
  ...
  <Collapser id="install-with-node" title="Node.js로 설치">
    <Steps>
      <Step>
...
## EU 및 JP 계정 사용자 [#EU-account-users]

EU 또는 JP 계정을 사용하는 경우 `EVENT_CLIENT_HOST` 을(를) 다음과 같이 설정해야 합니다:
...
2. <DNT>**Model Performance**</DNT> 을 클릭합니다.
3. <DNT>**Applications**</DNT> 아래에서 검색하세요.
