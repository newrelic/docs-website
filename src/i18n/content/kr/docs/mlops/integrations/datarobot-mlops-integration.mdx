...
1. **API 키 가져오기:** [one.newrelic.com](https://one.newrelic.com) 계정 메뉴에서 **API 키** 를 클릭한 다음 의미 있는 이름으로 계정에 대한 사용자 키를 만듭니다. 나중을 위해 이 이름을 기록해 두십시오. API 키에 대한 자세한 내용 [은 문서를 참조하십시오](/docs/apis/get-started/intro-apis/new-relic-api-keys/) .
