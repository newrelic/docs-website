...
1. **API 키 가져오기:** **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 계정 메뉴에서 **API 키** 를 클릭한 다음 의미 있는 이름으로 계정에 대한 사용자 키를 만듭니다. 나중에 사용할 수 있도록 이 이름을 기록해 두십시오. API 키에 대한 자세한 내용 [은 문서를 참조하십시오](/docs/apis/get-started/intro-apis/new-relic-api-keys/) .
2. **대시보드 만들기:** **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 에서 **Dashboards** 로 이동한 다음 대시 **보드 가져오기** 버튼을 클릭합니다. JSON 코드를 복사 **하여 JSON 필드 코드 붙여넣기** 에 붙여넣습니다.
