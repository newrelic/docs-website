...
3. **알림 알림 설정:** 대시보드를 만들고 나면 데이터에 대한 알림을 받을 수 있습니다. NRQL을 생성하려면 <InlinePopover type="alerts"/>차트에서 조건을 보려면 차트 메뉴를 클릭하세요. <Icon name="fe-more-horizontal"/>을 클릭한 다음 **Create alert condition** \[경고 조건 생성 을] 클릭합니다. 조건의 이름을 지정하고 사용자 정의한 후에는 이를 기존 정책에 추가하거나 새 정책을 생성할 수 있습니다.
