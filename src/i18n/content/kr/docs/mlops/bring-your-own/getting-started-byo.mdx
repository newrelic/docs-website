...
당신의 <InlinePopover type="licenseKey"/>( `ingest - license` 이라고도 함) 환경 변수 `NEW_RELIC_INSERT_KEY` 로 설정합니다. 자세한 내용과 지침을 보려면 [여기를 클릭하세요](/docs/apis/intro-apis/new-relic-api-keys/#license-key) . New Relic EU 지역에 데이터를 보고하고 있습니까? 자세한 지침을 보려면 [여기를](#EU-account-users) 클릭하세요.
