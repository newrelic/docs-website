...
<InlinePopover type="licenseKey" />( `ingest - license` 이라고도 함)을 가져와서 환경 변수로 설정합니다: `NEW_RELIC_INSERT_KEY`. 자세한 내용과 지침을 보려면 [여기를 클릭하세요](/docs/apis/intro-apis/new-relic-api-keys/#license-key) . 뉴렐릭 EU 지역에 데이터를 보고하고 있나요? 자세한 지침을 보려면 [여기를](#EU-account-users) 클릭하세요.
...
완료! 귀하의 기능을 확인하려면 <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT> 으로 이동하여 데이터를 확인하세요.
...
EU 계정을 사용하는 경우 환경 변수가 설정되지 않았다면 `MLPerformanceMonitoring` 호출에서 컴포트, 에러로 보내세요.

* `EVENT_CLIENT_HOST` 그리고 `METRIC_CLIENT_HOST`
...
`MLPerformanceMonitoring` 통화에서 시위로 보낼 수도 있습니다.