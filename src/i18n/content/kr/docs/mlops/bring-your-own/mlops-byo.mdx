...
* **Inference data:** 모델의 기능과 예측 값을 스트리밍합니다. 추론 데이터는 InferenceData라는 [사용자 지정 이벤트](https://docs.newrelic.com/docs/data-apis/ingest-apis/introduction-event-api/) 로 스트리밍됩니다.
* **Data metrics:** 모든 원시 추론 데이터를 보내는 대신 기능 및 예측(예: 최소, 최대, 평균 또는 백분위수)에 대해 집계된 통계 옵션을 선택합니다. 이는 자동으로 [측정항목](https://docs.newrelic.com/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/) 으로 전송됩니다.
* **Custom metrics:** 자체 측정항목을 계산하고, 모델 성능 또는 모델 데이터를 모니터링하고, Record_metrics 함수를 사용하여 New Relic으로 스트리밍합니다. 그것들은 [측정항목](https://docs.newrelic.com/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/) 으로 전송됩니다.
