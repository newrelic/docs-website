...
모델 성능 모니터링을 사용하면 기계 학습 모델의 품질을 높이고 관리 프로세스를 단순화하며 대규모 프로덕션 환경에서 기계 학습 배포를 자동화할 수 있습니다.
...
[적용된 인텔리전스](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/introduction-applied-intelligence/) 내에서 모델 성능 모니터링을 사용하려면 다음과 같은 몇 가지 옵션이 있습니다.

1. **자체 데이터 가져오기(BYOD):** 다른 라이선스에 가입하고 싶지 않거나 Amazon SageMaker를 사용하지 않는 경우 자체 ML 모델 원격 분석을 New Relic으로 쉽게 가져와 ML에서 가치를 얻을 수 있습니다. 모델 데이터. 몇 분 만에 특성 분포, 통계 데이터 및 예측 분포를 얻을 수 있습니다. [문서에서 BYOD에 대해 자세히 알아보세요](/docs/alerts-applied-intelligence/mlops/bring-your-own/mlops-byo) .
...
3. **파트너십:** New Relic은 특정 사용 사례 및 모니터링 기능을 제공하는 7개의 서로 다른 기계 학습 공급업체와 파트너 관계를 맺었습니다. 파트너는 선별된 성능 대시보드 및 기타 관찰 가능성 도구에 액세스할 수 있는 좋은 방법이며 모델에 대한 즉각적인 가시성을 제공하는 즉시 사용 가능한 대시보드를 제공합니다.
