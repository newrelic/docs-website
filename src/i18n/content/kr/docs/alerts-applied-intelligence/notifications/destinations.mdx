...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Enrich and Notify > Destinations** 으로 이동합니다. **Destinations** \[대상] 테이블에는 기존 대상에 대한 정보가 표시되며 이를 통해 사용자는 활성화, 비활성화 및 수정할 수 있습니다.
...
채널 템플릿을 설정하는 동안 **Destinations** \[대상] 테이블과 **Select a destination** \[대상 선택] 단계에서 대상 상태를 볼 수 있습니다. `status` 속성과 함께 Nerdgraph API를 사용하여 상태를 검토할 수도 있습니다.
...
이는 **Alert conditions (policies)** \[경고 조건(정책)] 및 [워크플로](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/) 기능을 사용하는 경우 특히 유용합니다. 이렇게 하면 알림 오류가 발생할 때 트리거되는 조건을 구축할 수 있으며 새 알림을 다른 곳으로 보낼 수 있습니다.
