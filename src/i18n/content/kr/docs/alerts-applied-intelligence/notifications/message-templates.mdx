...
변수 메뉴에서 `{{` 입력하여 변수 목록에서 선택합니다. 입력하는 동안 변수 이름이 자동 완성을 통해 표시됩니다. 변수 유형은 오른쪽에 기록됩니다. 워크플로우에 [Enrichments](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/#enrichments) 가 있는 경우 `{{` 입력한 후 목록 상단에 표시됩니다.
