...
<CollapserGroup>
  ...
  >
    ...
    Slack 워크스페이스 [에는 New Relic 애플리케이션](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(또는 one.eu.newrelic 고객의 경우 [EU 앱](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) )이 설치되어 있어야 합니다. 애플리케이션을 개별적으로 설치하기 전에 워크스페이스 관리자가 애플리케이션을 승인해야 합니다. 앱 지원은 [support@newrelic.com](mailto:support@newrelic.com) 으로 문의하십시오.
    ...
    **허용** 을 클릭하고 대상 페이지로 돌아갑니다.
    ...
    메시지를 보낼 목적지(Workspace)와 Slack 채널을 선택하세요. 필요한 작업 공간에 대해 미리 정의된 대상이 없는 경우 새 대상을 생성할 수 있습니다. 개인 정보 보호를 위해 사용자는 비공개 채널을 선택하기 위해 한 번 인증을 받아야 합니다. 비공개 채널을 선택하면 봇이 자동으로 채널에 추가됩니다.

    기본 알림을 사용하거나 사용자 정의 세부 정보로 보강할 수 있습니다. [변수 메뉴](/docs/alerts-applied-intelligence/notifications/message-templates/#variables-menu) 에서 변수를 선택하고 [핸들바 구문](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) 을 적용하여 페이로드를 보강하십시오.
  ...
  >
    ...
    <Callout variant="important">
      워크플로의 알림 채널로 **이메일을** 선택하면 이메일 대상이 자동으로 생성되므로 **대상** 메뉴에서 구성할 필요가 없습니다. 각 이메일 대상은 연결된 워크플로우에 대해 고유하며 이는 대상 피드에서 중복으로 표시될 수 있음을 의미합니다.
    </Callout>
  ...
  >
    ...
    * PagerDuty 인시던트가 확인되면 New Relic 문제에 대한 확인이 트리거됩니다.
    ...
    3. 목록에서 New Relic 통합을 찾아 표시하고 **추가를**<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}>클릭하십시오.</img>

    4. **통합 키를**<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}>보고 복사하려면 오른쪽을 클릭하십시오.</img>
  ...
  >
    ...
    * **사용자 ID:** 현재 로그인한 사용자를 기준으로 자동으로 채워집니다.
      ...
      ### 푸시 알림 수신 시기 구성 [#mobile]

      모바일 푸시를 구성하려면 모바일 알리미를 클릭하고 원하는 대상을 선택합니다.
  </Collapser>
  ...
  >
    ...
    Webhook 템플릿을 사용하여 Workflows에서 Splunk On-call로 알림 보내기 워크플로 [용 Splunk On-Call Webhook 템플릿](https://help.victorops.com/knowledge-base/new-relic-integration-guide/#about-splunk-on-call-and-new-relic)
  </Collapser>
