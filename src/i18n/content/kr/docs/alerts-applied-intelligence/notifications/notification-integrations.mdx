...
<CollapserGroup>
  ...
  >
    ...
    * `caller` 열에 대한 사용자를 가져오려면 `sys_user` 테이블에 대한 읽기 권한이 필요합니다.
      ...
      기본 제공되지 않는 세분화된 역할 `personalize_choices` , `personalize_dictionary` , `rest_service` 및 `itil` 에는 위의 권한이 있습니다.
  ...
  >
    ...
    * **AWS 계정 ID** : 귀하의 AWS 계정 ID입니다. 12자리 숫자입니다.
    ...
    2. **이벤트 버스 **에 대한 규칙을 만듭니다. New Relic에서 보낸 알림에 대응하려면 New-Relic 이벤트를 필터링하는 이벤트 패턴으로 규칙을 만들어야 합니다.
