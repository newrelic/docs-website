...
<CollapserGroup>
  ...
  >
    ...
    * **API 토큰:** Atlassian 계정에서 생성됩니다.
      ...
      * JIRA 문제 상태가 진행 중으로 변경되면 New Relic Issue의 승인이 트리거됩니다.
      ...
      1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하여 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭합니다.

      2. 목적지를 선택하십시오. 이 단계에서 새 대상을 만들 수 있습니다.
  ...
  >
    <Callout variant="important">
      출시 예정: ServiceNow 앱 스토어에서 New Relic Workflows와의 새로운 통합을 사용할 수 있습니다. 자세한 내용을 보려면 [여기를](https://forum.newrelic.com/s/hubtopic/aAX8W00000006D7/coming-soon-a-new-integration-between-new-relic-workflows-and-servicenow) 클릭하십시오.
    </Callout>
    ...
    * 새 유물 문제의 승인은 Servicnow 사건 상태가 열린 상태에서 변경될 때 트리거됩니다.
      ...
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하고 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭하고 ServiceNow 대상을 선택합니다.
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하여 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭합니다. 메시지를 보낼 목적지(Workspace)와 Slack 채널을 선택하세요. 필요한 작업 공간에 대해 미리 정의된 대상이 없는 경우 새 대상을 생성할 수 있습니다. 개인 정보 보호를 위해 사용자는 비공개 채널을 선택하기 위해 한 번 인증을 받아야 합니다. 비공개 채널을 선택하면 봇이 자동으로 채널에 추가됩니다.
  ...
  >
    ...
    * **권한 부여 메커니즘(_선택 사항_):**. `basic authentication` 또는 `bearer token`일 수 있습니다.
    ...
    4. 요청의 페이로드를 구성합니다.
       ...
       **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하여 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭하고 Webhook 대상을 선택합니다.
  ...
  >
    ...
    * **AWS 계정 ID** : 귀하의 AWS 계정 ID입니다. 12자리 숫자입니다.
    ...
    2. 이벤트 버스에 대한 규칙을 만듭니다.
       ...
       워크플로로 이동하여 기존 워크플로 또는 "새 워크플로 추가" 버튼을 클릭하고 eventbridge 대상을 선택합니다. New Relic에서 EventBridge로 알림을 보낼 때 메시지 템플릿을 사용하여 해당 알림을 사용자 지정할 수 있습니다. EventBridge 대상에 대한 메시지 템플릿을 구성하려면 다음 단계를 따르십시오.
  ...
  >
    ...
    * PagerDuty 인시던트가 확인되면 New Relic 문제에 대한 확인이 트리거됩니다.
    ...
    1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하여 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭하고 Pagerduy 알리미를 선택합니다.
    2. 목적지를 선택하십시오. 이 단계에서 새 대상을 만들 수 있습니다.
    ...
    3. 목록에서 New Relic 통합을 찾아 표시하고 **Add**\[추가를]클릭하십시오.<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}/>

    4. **Integration Key**\[통합 키를]보고 복사하려면 오른쪽을 클릭하십시오.<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}/>
  ...
  >
    ...
    * **사용자 ID:** 현재 로그인한 사용자를 기준으로 자동으로 채워집니다.
      ...
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** 로 이동하여 기존 워크플로를 클릭하거나 **Add a new Workflow** 버튼을 클릭하고 모바일 알리미를 선택합니다. 모바일 푸시를 구성하려면 모바일 알리미를 클릭하고 원하는 대상을 선택합니다.
  </Collapser>
