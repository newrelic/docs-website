...
<CollapserGroup>
  ...
  >
    ...
    * **비밀번호:** 사용자의 비밀번호입니다.
    ...
    4. 열 헤더 중 하나에서 메뉴 아이콘을 클릭하고 **XML 가져오기** 를 선택한 다음 다운로드한 XML 파일을 업로드합니다.
       ...
       대상이 저장되면 New Relic API 키가 `api_key_credentials` 에 보관됩니다. 키는 New Relic에 대한 콜백 REST 호출의 일부로 헤더로 전송됩니다.
    ...
    * 새 유물 문제의 승인은 Servicnow 사건 상태가 열린 상태에서 변경될 때 트리거됩니다.
      ...
      **테스트 알림** 을 클릭하여 기본 필드 값으로 ServiceNow 사건을 확인합니다. 성공하면 생성된 인시던트에 대한 링크가 생성됩니다.
  </Collapser>
  ...
  >
    ...
    Slack 워크스페이스에는 [New Relic 애플리케이션](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info) 이 설치되어 있어야 합니다. 애플리케이션을 개별적으로 설치하기 전에 워크스페이스 관리자가 애플리케이션을 승인해야 합니다. 앱 지원은 [support@newrelic.com](mailto:support@newrelic.com) 으로 문의하십시오. 또한 참여하려는 각 개인 채널에 대한 초대장이 필요합니다.
    ...
    **허용** 을 클릭하고 대상 페이지로 돌아갑니다.
    ...
    메시지를 보낼 목적지(Workspace)와 Slack 채널을 선택하세요. 필요한 작업 공간에 대해 미리 정의된 대상이 없는 경우 새 대상을 생성할 수 있습니다. 개인 정보 보호를 위해 사용자는 비공개 채널을 선택하기 위해 한 번 인증을 받아야 합니다.
  ...
  >
    ...
    * 이름이나 이메일 주소를 검색할 때 New Relic 계정이 있는 사용자를 찾습니다.
  ...
  >
    ...
    EventBridge 대상을 생성하려면 **대상** 으로 이동하고 **AWS EventBridge** 를 클릭합니다. 그런 다음 다음 정보를 입력합니다.
  ...
  >
    ...
    * PagerDuty 인시던트가 확인되면 New Relic 문제에 대한 확인이 트리거됩니다.
    ...
    3. 목록에서 New Relic 통합을 찾아 표시하고 **추가 **를 클릭하십시오.<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}/>

    4. **통합 키 **를 보고 복사하려면 오른쪽을 클릭하십시오.<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}/>
