...
<CollapserGroup>
  ...
  >
    ...
    4. 알림에서 **Webhook** \[웹후크를] 선택하고 다음 정보로 알림 메시지를 사용자 지정합니다.
       ...
       * [Workflow 변수를](https://docs.newrelic.com/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/custom-variables-incident-workflows/) 관련 [FreshService 매개변수](https://api.freshservice.com/#create_ticket) 로 보내도록 페이로드를 맞춤설정하세요. (샘플 페이로드 참조)

         * 워크플로 변수의 도우미 함수에 대한 자세한 내용은 [핸들바 구문을](https://docs.newrelic.com/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) 참조하세요.
  ...
  >
    ...
    4. Servicenow 스토어에서 [**Installation Guide** \[설치 가이드를\] 다운로드](https://store.servicenow.com/sn_appstore_store.do#!/store/application/d117597d1b9b9d9078faddf7b04bcba7/1.0.0?referer=%2Fstore%2Fsearch%3Flistingtype%3Dallintegrations%25253Bancillary_app%25253Bcertified_apps%25253Bcontent%25253Bindustry_solution%25253Boem%25253Butility%25253Btemplate%26q%3Dnew%2520relic&sl=sh) 하고 해당 지침을 따르세요.
  </Collapser>
  ...
  >
    ...
    Slack 통합은 특정 사용자에게 연결되어 있지 않습니다. 원래 Slack으로 인증한 사용자가 조직을 떠나거나 해당 계정이 삭제된 경우에도 Slack 대상은 계속 작동합니다. Slack으로 인증하면 당사 플랫폼이 Slack으로 이동하여 OAuth 토큰을 요청합니다. 이 토큰은 특정 시점에 만료되며 원래 거래의 토큰으로 새로 고쳐집니다.

    Slack 작업 공간이 여전히 존재하는 한 원래 Slack 계정이 삭제되더라도 계속 알림을 받게 됩니다. 개별 사용자 계정은 인증과 관련이 없습니다. 해당 계정은 Slack과의 핸드셰이크를 실행할 수 있는 권한을 제공합니다. 해당 데이터가 있으면 인증됩니다.
