...
<CollapserGroup>
  ...
  >
    ...
    Slack 워크스페이스에는 [New Relic 애플리케이션](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info) 이 설치되어 있어야 합니다. 응용 프로그램은 사용자가 개별적으로 설치하기 전에 작업 영역 관리자의 승인을 받아야 합니다. 앱 지원은 [support@newrelic.com](mailto:support@newrelic.com) 으로 문의하십시오. 또한 참여하려는 각 개인 채널에 대한 초대장이 필요합니다.
    ...
    **허용** 을 클릭하고 대상 페이지로 돌아갑니다.
    ...
    기본 알림을 사용하거나 문제 페이로드의 자유 텍스트 또는 동적 변수를 사용하여 더 자세한 정보로 알림을 강화할 수 있습니다. [변수 메뉴](/docs/alerts-applied-intelligence/notifications/message-templates/#variables-menu) 에서 변수를 선택하고 [핸들바 구문을](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) 적용하여 페이로드를 보강하십시오.
  ...
  >
    ...
    * **권한 부여 메커니즘( _선택 사항_ ):** . `basic authentication` , `OAuth 2.0 `또는 `bearer token` 일 수 있습니다.
      ...
      \###웹후크 이벤트 템플릿 구성 \[#configure-webhook-event-template]
  ...
  >
    ...
    * **AWS 계정 ID** : 귀하의 AWS 계정 ID입니다. 12자리 숫자입니다.
    ...
    3. 이벤트 소스를 이벤트 버스와 연결합니다. AWS EventBridge 콘솔의 탐색 창에서 파트너 이벤트 소스를 선택합니다.
    ...
    4. 이벤트 버스에 대한 규칙을 만듭니다.

       New Relic에서 보낸 알림에 반응하려면 New-Relic 이벤트를 필터링하는 이벤트 패턴으로 규칙을 만들어야 합니다.<img title="Event pattern with the newrelic partner prefix." alt="A screenshot of rule Event pattern." src={accountsBridgeEventPattern}/>
  ...
  >
    ...
    * PagerDuty 인시던트가 확인되면 New Relic 문제에 대한 확인이 트리거됩니다.
      ...
      4. Pagerduty의 사용자 정의 세부 정보 섹션에 세부 정보를 보냅니다. 기본 페이로드를 사용하거나 문제 페이로드의 자유 텍스트 또는 동적 변수를 사용하여 사용자 정의할 수 있습니다. \[변수 메뉴](/docs/alerts-applied- intelligence/notifications/message-templates/#variables-menu)에서 변수를 선택하고 [핸들바 구문을](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) 적용하여 페이로드를 보강하세요. 오른쪽의 **미리보기** 섹션에는 템플릿이 렌더링된 후 예상되는 페이로드가 표시됩니다. 페이로드가 유효한 JSON을 형성하지 않으면 오류가 표시되고 템플릿을 저장할 수 없습니다.
    ...
    3. 목록에서 New Relic 통합을 찾아 표시하고 **추가 **클릭하십시오.<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}/>
  ...
  >
    ...
    웹후크 템플릿을 사용하여 워크플로에서 Opsgenie로 알림 보내기[워크플로용 Opsgenie 웹후크 템플릿](https://support.atlassian.com/opsgenie/docs/integrate-opsgenie-with-new-relic-alerts-new/)
  </Collapser>
  ...
  >
    ...
    웹후크 템플릿을 사용하여 워크플로에서 Splunk On-call로 알림 보내기 [워크플로용 Splunk On-Call 웹후크 템플릿](https://help.victorops.com/knowledge-base/new-relic-integration-guide/#about-splunk-on-call-and-new-relic)
  </Collapser>
</CollapserGroup>
