...
## 워크플로우 로그 [#workflows-log]

워크플로 로그는 사용자에게 워크플로에서 생성된 모든 알림을 추적할 수 있는 자세한 기록을 제공합니다. 이 정보는 문제를 해결하고 워크플로 성능을 모니터링하며 알림이 올바른 대상으로 전송되는지 확인합니다. 결과적으로 사용자는 발생할 수 있는 모든 문제를 신속하게 식별하고 필요에 따라 수정 조치를 취할 수 있습니다.

워크플로 로그에 액세스하려면 **[one.newrelic.com > Alerts & AI > Workflows](https://one.newrelic.com/alerts-ai/workflows)**로 이동한 다음 **Workflows Log**클릭합니다.

### 워크플로 로그 탐색 [#navigate-workflows-log]

워크플로 로그에는 다음 열이 표시됩니다.
...
3. **Destination:** 이 열에는 워크플로 알림을 받는 대상 유형(예: Slack, 이메일 등)이 표시됩니다.
...
8. **Notification Result:** 이 열에는 알림이 성공적으로 전송된 경우 타사의 증거(예: Slack 스레드, JIRA 티켓 등)에 대한 링크가 표시됩니다.

### 추가 세부정보 보기 [#view-details]

사용자는 워크플로 로그의 행을 클릭하여 추가 세부 정보를 볼 수 있습니다. 그러면 오류 세부 정보(해당하는 경우)와 타사의 알림 링크가 표시됩니다.