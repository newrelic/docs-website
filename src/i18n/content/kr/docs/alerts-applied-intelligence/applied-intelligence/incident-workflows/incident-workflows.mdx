...
1. [one.newrelic.com](one.newrelic.com) 으로 이동하여 **Alerts & AI** 를 클릭 **하고 Enrich and Notify** 아래의 왼쪽 탐색에서 **Workflow** 를 클릭한 다음 **Add a workflow** 를 클릭합니다.
2. 워크플로의 이름을 지정합니다. 이 필드는 필수이며 고유해야 합니다.
3. 워크플로는 관련 대상으로 보내려는 문제를 필터링합니다. 모든 문제 **보내기** 를 선택하여 모든 문제를 보내거나 올바른 문제를 필터링하는 쿼리를 작성할 수 있습니다.
...
5. 선택 사항: 팀 태그로 문제를 필터링하여 모든 팀이 해당 엔터티가 문제에 포함될 때 알림을 받을 수 있도록 합니다.단계는 아래 데모에서 찾을 수 있습니다.
  ...
  >
    ...
    * 참고: 쿼리 이름은 워크플로에서 보낼 알림의 페이로드에 결과를 포함하는 방법을 결정하는 데 이름을 사용하기 때문에 고유해야 합니다.
      ...
      <Callout variant="important">
        ...
        워크플로당 최대 보강 수는 5입니다. 보강이 포함할 수 있는 최대 변수 수는 1입니다.
      </Callout>
  </Collapser>
</CollapserGroup>
...
9. 선택적으로 워크플로를 테스트합니다.계정의 기존 데이터를 사용하여 구성한 내용을 테스트하고 샘플 알림을 보냅니다.테스트는 데이터가 들어오는 경우에만 작동합니다.데이터가 없으면 다음 메시지가 표시됩니다. `We don’t see any issues matching your filter. That doesn’t mean it won’t work` .
10. **활성화** 를 클릭하여 워크플로를 완료합니다.

<Callout variant="important">
  추가할 수 있는 워크플로 수의 초기 제한은 [환경](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence#1-configure-environment) 당 50개, 계정당 50개입니다. 이 숫자를 늘리려면 계정 담당자에게 문의하세요.
</Callout>