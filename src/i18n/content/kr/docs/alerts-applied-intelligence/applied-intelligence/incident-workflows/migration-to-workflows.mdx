...
워크플로는 다음을 위한 기존 알림 시스템을 대체합니다. <InlinePopover type="alerts"/>그리고 응용지능. 워크플로가 개선되고 유연한 알림 시스템이기 때문에 이는 좋은 소식입니다. 워크플로는 팀이 스택의 더 큰 맥락에서 잠재적인 오류에 대해 학습하여 즉각적이고 효율적인 조치를 취할 수 있도록 도와줍니다.

[이 마이그레이션은](https://discuss.newrelic.com/t/plan-to-upgrade-alert-notification-channels-to-workflows-and-destinations/188205) 팀에 어떤 의미가 있습니까? 이전 알림 모델에서 팀은 다양한 임계값과 매개변수를 사용하여 알림 조건을 생성했습니다. 해당 조건이 특정 정책과 연결되어 있고 위반되었으며 팀이 이에 대해 즉시 알 수 있도록 하려면 **notification settings** \[알림 설정을] 추가합니다. 우리의 알림 설정은 New Relic에 어떤 데이터를 어디로 보낼지 알려줍니다.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        에만 유효 <InlinePopover type="apm"/>정황.
      </td>
    </tr>
