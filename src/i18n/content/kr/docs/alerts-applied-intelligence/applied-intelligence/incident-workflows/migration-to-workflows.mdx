...
* issue_id가 추가되었습니다. 소비자는 event_id가 어느 시점에서 제거될 것이므로 issue_id 대신 issue_id를 사용하도록 모든 통합을 전환해야 합니다.
...
PagerDuty, Webhook, VictorOps, OpsGenie 및 xMatters 알림의 사건 ID는 클래식 사건 ID를 참조합니다. 클래식 사고 ID는 곧 제공될 예정이며 소비자는 대신 issue_id를 사용하기 시작해야 합니다. 이전에는 모든 알림에 event_id가 있었습니다. 이제 첫 번째 알림이 전송될 때까지 모든 문제에 추가된다는 보장이 없습니다.
