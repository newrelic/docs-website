...
**Issues** 페이지에서는 모든 문제에 대한 개요와 유용한 정보를 찾을 수 있습니다. 분석 요약, 이벤트 로그 및 상관 문제에 대한 세부 정보를 포함하여 자세한 내용을 보려면 개별 문제를 클릭할 수도 있습니다.
...
사후 분석을 만드는 방법에 대한 자세한 단계를 보거나 데모 데모를 보려면 [사후 문서](https://docs.newrelic.com/docs/alerts-applied-intelligence/applied-intelligence/postmortems-applied-intelligence/) 페이지를 방문하십시오.
...
* **배포 이벤트에는**배포 및 관련 배포의 두 가지 유형이 있습니다. **[모든 배포 이벤트가](/whats-new/applied-intelligence-deployment-events-issue-feed)** 도착하면 모든 배포 이벤트를 보려면 모든 배포 표시를 클릭하거나 특정 배포를 클릭하여

  <InlinePopover type="apm"/>

  배포 페이지. [APM 배포 페이지에는](/docs/apm/apm-ui-pages/events/deployments-page-view-impact-your-app-users) 최근 배포와 최종 사용자 및 앱 서버의 Apdex 점수, 응답 시간, 처리량 및 오류에 미치는 영향이 나열됩니다. 이 섹션은 New Relic이 배포된 영향을 받는 엔터티에서 애플리케이션을 식별한 경우에만 표시됩니다.
