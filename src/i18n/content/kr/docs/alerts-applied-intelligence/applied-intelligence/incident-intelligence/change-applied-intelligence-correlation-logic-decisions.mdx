...
적용된 인텔리전스의 상관 관계 논리를 사용하면 관련 문제가 함께 그룹화되어 산만하고 중복된 경고가 줄어듭니다. 이벤트가 시스템에 들어오면 상관 관계 논리에 적합합니다. 적격한 문제는 시간, 경고 컨텍스트 및 관계 데이터를 기반으로 평가됩니다. 여러 문제가 관련된 경우 상관 관계 논리가 관련 사건을 하나의 포괄적인 [문제](/docs/alerts-applied-intelligence/overview/#concepts-terms)로 모을 것입니다.
