...
<CollapserGroup>
  <Collapser
    ...
    title="경고"
  >
    사고 인텔리전스를 활성화하여 <InlinePopover type="alerts"/>정책을 사용하면 모니터링 중인 항목에서 컨텍스트와 상관 관계를 얻을 수 있습니다. 알림에서 데이터를 가져오려면 다음 안내를 따르세요.
...
제안된 결정을 활성화하려면 **Add to your decisions** \[결정에 추가를] 클릭합니다. 활성화되면 결정이 팀의 기본 결정 테이블에 표시됩니다. 제안된 모든 결정은 작성자를 New Relic AI로 표시합니다(이는 New Relic 응용 인텔리전스를 나타냄).
