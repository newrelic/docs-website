...
<table id="global-decision-descriptions">
  ...
  <tbody>
    <tr>
      ...
      <td>
        임계값을 초과한 엔터티 이름과 NRQL 쿼리가 동일한 경우 상관 관계가 활성화됩니다. 동일한 [NRQL 경고 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions) 의 관련 이벤트가 식별됩니다. 이 결정은 예를 들어 동일한 트랜잭션 쿼리 대기 시간 편차가 있는 문제를 관련시키는 데 도움이 됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        New Relic [조건 ID](/docs/new-relic-solutions/get-started/glossary/#condition_id) 와 딥링크 URL이 동일하기 때문에 상관관계가 활성화되었습니다. 딥링크 URL은 [알림 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions/) 외에도 시계열 및 시간 범위 정보를 제공합니다. 이러한 문제를 연관시키면 시간 범위 지표를 사용하여 사고 대응 흐름에서 관련 사고를 더 쉽게 살펴보고 심층 분석을 수행할 수 있습니다. New Relic 경고 조건에 의해 사건이 트리거되면 딥 링크 URL이 자동으로 생성될 수 있지만, REST 소스의 경우 [deepLinkUrl은](/docs/data-apis/ingest-apis/event-api/incident-event-rest-api/#api-specs) 사용자가 정의해야 합니다.
      </td>
    </tr>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="New Relic에서 엔티티에 태그 지정"
  >
    하나 이상의 [패싯](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions#syntax) 이 정의된 [NRQL 경고 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions) 을 생성하면 데이터가 속성별로 그룹화됩니다. 또한 발생된 인시던트 이벤트는 해당 속성 및 값으로 보강됩니다. 인시던트의 경우 패싯 속성은 동일한 형식을 따릅니다. `newrelic/tags/ATTRIBUTE_NAME`
  </Collapser>
</CollapserGroup>
