...
우리는 이것을 상관 논리 **결정** 이라고 부릅니다. 기본 제공 결정이 있지만 결정 페이지에서 직접 만들고 사용자 지정할 수도 있습니다. 결정 페이지를 찾으려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions** 으로 이동합니다. 필요에 가장 잘 맞도록 의사 결정을 더 많이 구성할수록 New Relic은 사건을 더 잘 연관시키고 소음을 줄이며 대기 중인 팀에 더 많은 컨텍스트를 제공할 수 있습니다.
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Applied intelligence > Incident intelligence > Decisions**: 우리의 UI는 각 결정이 인시던트와 어떻게 연관되는지 보여줍니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Applied intelligence > Incident intelligence > Decisions**: 결정 UI의 몇 가지 예제 통계입니다.
</figcaption>
...
사용자 지정 결정을 구축하여 잡음을 줄이고 상관 관계를 개선할 수 있습니다. 의사결정을 시작하려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions** 이동한 다음 **Create new decision** \[새 의사결정 작성을] 클릭하십시오.
...
**이 기능은 현재 미리 보기 상태이며 일부 고객만 사용할 수 있습니다.** 액세스 권한이 없는 경우 [고급 의사 결정 빌더](#advanced-decision-builder)에 대한 지침을 참조하십시오.
...
1. `Filter by specific values` 섹션을 열고 추가 필터를 보려면 **See more options**\[추가 옵션 보기] 를 클릭합니다.
2. 속성을 선택합니다.
...
**Filter by related entities**\[관련 엔터티로 필터링을] 클릭하고 엔터티 클래스를 선택합니다.
...
**Simulate**를클릭하여 시뮬레이션을 수동으로 트리거할 수도 있습니다. 의사결정에서 무언가가 변경된 경우 수행할 수 있습니다.
...
이름 및 설명 패널에 액세스하려면 **Create decision**\[결정 만들기 를]클릭합니다. 시스템은 귀하의 결정에 따라 이름을 생성합니다. 이름과 설명을 원하는 대로 사용자 지정합니다.
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions** 으로 이동합니다.
2. **Create new decision**\[새 결정 만들기를]클릭한 다음 **Use advanced builder**\[고급 빌더 사용을]클릭합니다.

사용 가능한 옵션에 대한 자세한 내용은 계속 읽으십시오.

중요 용어:

* 논리 필터: [](#operators)[속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/violation-event-attributes/#attributes)에 대한 연산자 로 정의된 논리 조건입니다.
...
* **빈도**: 트리거할 결정을 위해 결정 논리를 충족해야 하는 최소 인시던트 수를 수정합니다.
* **유사성**: 결정 논리에서 `similar to` 연산자를 사용하는 경우 알고리즘 목록에서 선택하고 민감도를 설정할 수 있습니다. 이는 귀하의 결정에 있는 모든 `similar to` 연산자에 적용됩니다.
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Issues & activity > Incidents** 탭으로 이동합니다.
...
* **인시던트 예:** 규칙의 속성 및 값은 물론 각 쌍의 기타 인기 있는 속성을 포함하여 의사결정이 상관 관계가 있는 인시던트 쌍의 목록입니다. 사건을 클릭하면 세부 정보를 볼 수 있습니다.
