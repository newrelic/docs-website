...
응용 인텔리전스의 상관 관계 논리를 통해 관련 문제를 그룹화하여 산만하고 중복되는 경고를 줄입니다. 이벤트가 시스템에 들어오면 상관 관계 논리를 사용할 수 있습니다. 적격 문제는 시간, 경고 컨텍스트 및 관계 데이터를 기반으로 평가됩니다. 여러 문제가 관련되어 있는 경우 상관 관계 논리는 관련 사고를 하나의 포괄적인 [문제](/docs/alerts-applied-intelligence/overview/#concepts-terms) 로 분류합니다.

우리는 이것을 상관 논리 <DoNotTranslate>**decisions**</DoNotTranslate> 라고 부릅니다. 우리는 기본적으로 결정을 내렸지만 결정 페이지에서 직접 결정을 만들고 사용자 정의할 수도 있습니다. 결정 페이지를 찾으려면 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions**</DoNotTranslate> 로 이동하세요. 요구사항에 가장 적합하도록 결정을 더 많이 구성할수록 뉴렐릭은 인시던트의 상관관계를 더 잘 파악하고, 소음을 줄이며, 대기 중인 팀에 더 많은 컨텍스트를 제공할 수 있습니다.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Applied intelligence > Incident intelligence > Decisions**</DoNotTranslate>: 우리의 UI 각 결정이 인시던트의 상관관계를 보여줍니다.
</figcaption>
...
<figcaption>
  공지에 대한 상관관계를 활성화하려면 <DoNotTranslate>**Correlate and suppress noise**</DoNotTranslate> 확인란을 선택하세요.
</figcaption>
...
* <DoNotTranslate>**Global decision**</DoNotTranslate>
  ...
  : 응용 인텔리전스를 사용하기 시작하면 다양한 기본 결정이 자동으로 활성화됩니다.
...
1. <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)> Alerts & AI > Incident intelligence > Decisions**</DoNotTranslate>
...
<CollapserGroup>
  ...
  >
    ...
    1. <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> 에서 <DoNotTranslate>**Alerts**</DoNotTranslate>) 클릭합니다.

    2. <DoNotTranslate>**incident intelligence**</DoNotTranslate> 아래 왼쪽에서 <DoNotTranslate>**Sources**</DoNotTranslate> 클릭한 다음 <DoNotTranslate>**Alerts**</DoNotTranslate> 클릭합니다.

    3. 응용 인텔리전스에 연결하려는 정책을 선택하고 <DoNotTranslate>**Connect**</DoNotTranslate> 클릭합니다.

       공지사항을 추가하거나 이미 연결한 정책을 제거할 수 있습니다. <DoNotTranslate>**Sources > Alerts**</DoNotTranslate>.
...
시작하려면 <DoNotTranslate>**Decisions**</DoNotTranslate> UI 페이지 주제에서 <DoNotTranslate>**Suggested decisions**</DoNotTranslate> 탭을 클릭하세요. 제안된 결정의 논리와 각 제안된 결정을 클릭하면 예상 상관관계율을 확인할 수 있습니다.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions**</DoNotTranslate>: 의사결정 UI의 통계 예시입니다.
</figcaption>

제안된 결정을 활성화하려면 <DoNotTranslate>**Add to your decisions**</DoNotTranslate> 클릭하세요. 활성화되면 결정이 팀의 기본 결정 테이블에 표시됩니다. 제안된 모든 결정에는 작성자가 뉴렐릭 AI로 표시됩니다(이는 뉴렐릭 인텔 응용리전스 참조).

제안된 결정이 귀하의 요구사항과 관련이 없으면 <DoNotTranslate>**Dismiss**</DoNotTranslate> 클릭하세요.

## 맞춤 결정 만들기 [#customize]

사용자 정의 결정을 내려 노이즈를 줄이고 상관 관계를 개선할 수 있습니다. 결정을 내리려면 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions**</DoNotTranslate> 으로 이동한 다음 <DoNotTranslate>**Create new decision**</DoNotTranslate> 클릭하세요.
...
<DoNotTranslate>**This feature is currently in preview and available for only some customers.**</DoNotTranslate> 액세스 권한이 없는 경우 [고급 의사 결정 작성 도구](#advanced-decision-builder) 에 대한 지침을 참조하세요.
...
<DoNotTranslate>**Filter by related entities**</DoNotTranslate> 클릭하고 엔터티 클래스를 선택합니다.
...
<DoNotTranslate>**Simulate**</DoNotTranslate> 클릭하여 시뮬레이션을 수동으로 트리거할 수도 있는데, 결정에서 변경된 사항이 있는 경우 수행할 수 있습니다.
...
이름 및 설명 패널에 액세스하려면 <DoNotTranslate>**Create decision**</DoNotTranslate> 클릭합니다. 시스템은 귀하의 결정에 따라 이름을 생성합니다. 원하는 대로 이름과 설명을 사용자 정의합니다.
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Decisions**</DoNotTranslate>

   으)로 이동합니다.

2. <DoNotTranslate>**Create new decision**</DoNotTranslate>
...
이는 공지 및 UI 의 다른 영역에서 어떤 결정으로 인해 인시던트 쌍이 서로 연관되었는지 나타내는 데 사용됩니다. 다음 단계에서 기본 고급 설정을 업데이트하지 않으려면 <DoNotTranslate>**Create decision**</DoNotTranslate> 클릭하여 생성을 완료하세요.
...
Decision은 인시던트의 속성 값이 논리 필터에서 평가되는 방식을 유연하게 정의하는 데 도움이 되는 일련의 연산자를 제공합니다. 기본적인 것들은 <DoNotTranslate>**equals**</DoNotTranslate>, <DoNotTranslate>**contains**</DoNotTranslate>, <DoNotTranslate>**starts with**</DoNotTranslate>, <DoNotTranslate>**ends with**</DoNotTranslate>, <DoNotTranslate>**exists**</DoNotTranslate> 이며 그에 따른 부정 연산자입니다. 예를 들어, <DoNotTranslate>**does not equal**</DoNotTranslate>.

유사성 연산자 <DoNotTranslate>**is similar to**</DoNotTranslate> 이 있으며, 이 연산자에 대해 기본 [유사성 알고리즘을](#algorithms) 지정할 수 있습니다. 기본적으로 Levenshtein Distance를 사용합니다.

<DoNotTranslate>**contains (regex)**</DoNotTranslate> 연산자를 사용하면 [정규식](#regex) 조건을 정의할 수 있습니다. 임의의 데이터 값을 일치시키는 데 강력합니다.
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            응용 인텔리전스 결정에 대한 기본 유사성 값은 편집 거리 3입니다. 의사결정 작성 도구의 <DoNotTranslate>**Advanced mode**</DoNotTranslate> 에서 이를 변경할 수 있습니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DoNotTranslate>**fuzzy wuzzy**</DoNotTranslate> 유사성 측정 계열은 여러 플랫폼에서 서로 다른 레이블이 있는 동일한 이벤트에 대한 지원 케이스를 찾는 데 도움이 되도록 [SeatGeek에서 개발](https://chairnerd.seatgeek.com/fuzzywuzzy-fuzzy-string-matching-in-python/) 했습니다. 두 문자열의 퍼지 워지 비율은 백분율로 표시되며 숫자가 높을수록 문자열이 더 유사함을 나타냅니다. 이는 Python difflib의 [SequenceMatcher 알고리즘을](https://docs.python.org/3/library/difflib.html) 기반으로 합니다.
          </td>
        </tr>
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Issues & activity > Incidents**</DoNotTranslate>
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Common attributes:**</DoNotTranslate> 이 분석은 선택한 모든 인시던트 간에 정확히 동일한 속성과 값을 강조합니다.

    * <DoNotTranslate>**Similar attributes:**</DoNotTranslate> 유사성 분석은 거리가 3인 Levenshtein 알고리즘을 사용하여 3개 이하의 문자 변경이 수행될 경우 값이 동일할 속성을 찾습니다. 숫자 값과 단일 문자 값은 결과에서 필터링됩니다. 유사 속성은 2개의 인시던트를 선택해야 하며, 3개 이상의 인시던트를 선택한 경우 유사성 분석을 수행하지 않습니다.

      최선의 결정을 내리려면 인시던트에서 빈도가 낮은 공통 속성을 선택하는 것이 좋습니다. 다음은 저주파 또는 고주파 속성 선택이 결정에 미치는 영향을 이해하기 위한 팁입니다.

    * <DoNotTranslate>**Low frequency:**</DoNotTranslate> 예를 들어 빈도 열에서 0%가 있는 속성은 고유 식별자이거나 지난 달 데이터에서 최근에 보고된 속성일 가능성이 높습니다. 낮은 빈도 속성을 선택하면 소수의 이벤트와 상관 관계가 있을 수 있습니다.

    * <DoNotTranslate>**High frequency:**</DoNotTranslate> 반면에 빈도가 100%인 속성은 모든 데이터에 존재하는 속성입니다. 이러한 속성을 선택하면 모든 이벤트가 서로 연관됩니다.
