...
import alertsDecisionBuilderFilter from 'images/alerts_screenshot-full_decision-builder-filter.webp'

import alertsDecisionBuilderContextual from 'images/alerts_screenshot-full_decision-builder-contextual.webp'

import accountsDecisionBuilderTopology from 'images/accounts_screenshot-full_decision-builder-topology-.webp'

import alertsDecisionBuilder from 'images/alerts_screenshot-full_decision-builder-.webp'
...
적용된 인텔리전스의 상관 관계 논리를 사용하면 관련 문제가 함께 그룹화되어 산만하고 중복된 경고가 줄어듭니다. 이벤트가 시스템에 들어오면 상관 관계 논리에 적합합니다. 적격한 문제는 시간, 경고 컨텍스트 및 관계 데이터를 기반으로 평가됩니다. 여러 문제가 관련된 경우 상관 관계 논리가 관련 사건을 하나의 포괄적인 [문제](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/basic-alerting-concepts/)로 모을 것입니다.
...
<CollapserGroup>
  ...
  >
    ...
    2. **사고 인텔리전스** 아래 왼쪽에서 **소스** 를 클릭한 다음 **경고** 를 클릭합니다.
...
<CollapserGroup>
  ...
  >
    ...
    * **유사 속성:** 유사성 분석은 거리가 3인 Levenshtein 알고리즘을 사용하여 3개 이하의 문자 변경이 수행되는 경우 값이 동일한 속성을 찾습니다. 숫자 값과 단일 문자 값은 결과에서 필터링됩니다. 유사한 속성은 2개의 인시던트를 선택해야 하며, 3개 이상의 인시던트를 선택하면 유사성 분석을 수행하지 않습니다.
