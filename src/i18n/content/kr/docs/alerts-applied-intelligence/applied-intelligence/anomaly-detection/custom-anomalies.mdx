...
사용자 지정 이상 현상을 통해 팀은 시스템에서 비정상적인 동작을 감지할 때 가장 다양한 기능을 사용할 수 있습니다. 유연하고 역동적일 뿐만 아니라 맞춤형 이상 현상은 팀에 모든 엔터티 또는 신호에 대해 경고하고 임계값을 조정 및 최적화할 수 있는 기능을 제공합니다. 사용자 지정 이상 항목은 정적 경고와 동일한 고급 조정 설정을 사용하여 구축되므로 팀에서 중요한 사건만 볼 수 있습니다.
...
* 슬라이더 막대를 사용하여 미리보기 차트에서 이상 작동 주변의 밝은 회색 영역으로 표시되는

  <DoNotTranslate>**Critical**</DoNotTranslate>

  레버 값을 조정합니다. 이상 행동 주변의 밴드가 더 촘촘할수록 더 민감해지고 더 많은 인시던트가 생성됩니다.
...
2. <DoNotTranslate>**+ New alert condition > Use guided mode**</DoNotTranslate> 을(를) 클릭합니다.
...
5. 하나 이상의 임계값에 대한 설정을 구성합니다. 이상 탐지는 이전 활동을 기반으로 다음 데이터 포인트가 무엇인지 예측합니다. 사용자 지정 이상 탐지에 대한 임계값은 신호 값이 예측된 값에서 벗어난 표준 편차 수입니다.
...
* <DoNotTranslate>**Age of data**</DoNotTranslate>
  ...
  초기 생성 시 데이터 가용성 및 예측 유형에 따라 1-4주 간의 데이터를 사용하여 예측이 계산됩니다. 생성 후 알고리즘은 장기간에 걸쳐 진행되는 데이터 변동을 고려하지만 최신 데이터에 더 큰 가중치가 부여됩니다. 짧은 시간 동안만 존재한 데이터의 경우 예측 값이 크게 변동할 가능성이 높으며 정확도가 매우 낮습니다. 이는 일반적인 값과 동작을 결정하기 위한 데이터가 충분하지 않기 때문입니다. 데이터의 이력이 많을수록 예측과 관측값이 더 정확해집니다.

* <DoNotTranslate>**Consistency of data**</DoNotTranslate>

  일관된 범위에 유지되거나 느리고 꾸준한 추세를 보이는 지표 값의 경우 예측 가능성이 더 높다는 것은 해당 측정값, 관련 값이 예측을 중심으로 더 엄격해짐을 의미합니다. 더 다양하고 예측할 수 없는 데이터일수록 더 느슨한(더 넓은) 레버 값을 갖게 됩니다.
