...
2. 조건이 완료되면 경고 위반이 알림으로 전환되는 방식을 제어하는 워크플로를 설정합니다. [one.newrelic.com](https://one.newrelic.com) 에서 **탐지 및 AI** 를 클릭한 다음 **워크플로** 를 클릭합니다. 이를 사용하는 방법에 대한 팁은 [워크플로 문서](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows) 및 워크 [플로 모범 사례에 대한 이 게시물을](https://discuss.newrelic.com/t/relic-solution-patterns-for-implementing-alerts-workflows/188605) 참조하세요.
3. 하나 이상의 목적지를 설정하십시오(장소 알림이 전송됨). **탐지 및 AI** 에서 **대상** 을 클릭합니다. 팁은 [대상 문서](/docs/alerts-applied-intelligence/notifications/destinations) 를 참조하십시오.
