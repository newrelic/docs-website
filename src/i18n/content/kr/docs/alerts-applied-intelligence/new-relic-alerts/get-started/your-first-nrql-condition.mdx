...
애플리케이션을 New Relic에 연결하고 차트와 대시보드 탐색을 시작한 후에는 데이터의 비정상적인 동작에 대해 팀에 최신 정보를 제공하는 경고를 생성하는 것이 좋습니다. 그만큼 <InlinePopover type="alerts"/>이 기능은 단순히 데이터를 수집하는 것에서 사려 깊고 효과적인 조치를 취하는 것까지 New Relic 경험을 향상시킵니다.
