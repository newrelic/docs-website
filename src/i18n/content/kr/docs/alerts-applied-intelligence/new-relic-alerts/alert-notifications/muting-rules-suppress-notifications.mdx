...
<Callout variant="tip">
  숨기기 규칙을 만들기 전에 인시던트 알림을 생성하는[정책 및 조건을 만들어야](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy) 합니다.
</Callout>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * `attribute`: 인시던트 내의 단일 속성입니다. [위반 이벤트 속성](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes) 목록을 보려면 여기로 이동하십시오.
...
1. 인시던트가 생성된 후 인시던트가 열리기 전에 개별 인시던트를 식별합니다.
2. 기본 조건을 재정의하여 "음소거"되어야 함을 나타냅니다.

현재 인시던트 음소거는 음소거된 인시던트만 포함하는 인시던트가 알림을 보내지 않는다는 점을 제외하고 정상적인 경보 인시던트 수명 주기가 유지됨을 의미합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        이미 열려 있는 사건에 새 **음소거된** 사건이 추가되었습니다.
      </td>
...
트리거된 사건은 사건과 1:1의 비율을 가지므로 사건이 음소거되면 일치하는 사건도 음소거됩니다. 워크플로는 하나 이상의 사건이 있을 수 있는 문제에 의해 트리거되므로 음소거된 사건과 음소거되지 않은 사건이 결합된 시나리오가 있을 수 있습니다.
