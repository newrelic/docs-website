...
음소거 규칙을 생성, 활성화, 비활성화 및 관리할 수 있습니다. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules** 으로 이동합니다. 언제든지 음소거 규칙을 활성화하거나 비활성화할 수 있습니다.
...
<Callout variant="tip">
  숨기기 규칙을 만들기 전에 알림을 생성하는[정책 및 조건을 만들어야](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy) 합니다.
</Callout>

뮤팅 규칙을 생성하려면 **뮤팅 규칙** 화면에서 **+ 규칙 추가** 를 클릭하세요. 뮤팅 규칙의 이름과 설명을 입력하고 규칙을 적용할 계정을 선택합니다.

다음으로 인시던트 필터를 빌드합니다. [인시던트 이벤트 속성](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes) 의 하위 집합(구체적으로 `accountId` , `conditionId` , `conditionName` , `entity.guid` , `nrqlEventType` , `nrqlQuery` , `policyId` , `policyName` , `runbookUrl` ( `conditionRunbookUrl` ), `tags.<NAME>` 및 `targetName` ) 및 [하위 조건 연산자](#sub-conditions) . 경보 정책 ID 또는 조건 이름과 같은 인시던트 속성 중 하나와 값을 비교할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        대상으로 삼을 인시던트를 정의하는 개별 표현식 세트입니다. 음소거 규칙 조건은 다음과 같습니다.
...
숨기기 규칙은 알림을 억제하거나 숨기기 위해 기본 경고 수명 주기가 끝날 때 적용됩니다. 기존 정책이나 조건을 비활성화하지 않습니다. 예를 들어 유지 관리 기간 및 배포와 같은 알려진 시스템 중단 중에 알림을 음소거할 수 있습니다. 시스템 중단 사건에 대한 알림이 음소거된 경우에도 시스템 중단 사건은 계속 식별됩니다.
...
워크플로우를 설정하는 방법에 대한 단계별 가이드는 아래 예제 데모를 확인하십시오.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="음소거된 사건"
  >
    인시던트 및 문제는 **상태** 열에서 **음소거** 됨으로 표시됩니다.
...
<Callout variant="tip">
  모든 하위 조건 연산자 값은 대소문자를 구분합니다. 예를 들어 `policyName STARTS_WITH 'PROD'` 을 사용하는 경우 'Prod'로 시작하는 정책 이름은 선택되지 않습니다.
</Callout>

* `EQUALS`: 여기서 제공된 값은 인시던트 속성 값과 같습니다.
* `NOT_EQUALS`: 제공된 값이 인시던트 속성 값과 같지 않은 경우.
* `IN`: 인시던트 속성 값이 제공된 값 목록에 있는 경우(최대 500개).
* `NOT_IN`: 인시던트 속성 값이 제공된 값 목록에 없는 경우(최대 500개).
* `CONTAINS`: 제공된 값 문자열이 인시던트 속성 값에 있는 경우.
* `NOT_CONTAINS`: 제공된 값 문자열이 인시던트 속성 값에 없는 경우.
* `ANY`: **주의:** 이 연산자가 있는 조건은 계정의 **모든** 사건을 음소거합니다.
...
* `NOT_STARTS_WITH`: 인시던트 속성 값이 제공된 값 문자열로 시작하지 않는 경우.
