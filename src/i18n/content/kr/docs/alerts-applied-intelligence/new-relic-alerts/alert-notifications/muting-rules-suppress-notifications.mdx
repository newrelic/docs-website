...
알림은 시스템에 문제가 있을 때 적시에 공지를 보냅니다. 때로는 볼 필요가 없다는 것을 알고 있는 공지가 있습니다. <DoNotTranslate>**muting rules**</DoNotTranslate> 사용하면 주의가 필요하지 않은 메시지가 쏟아지는 것을 막을 수 있습니다.
...
음소거 규칙을 생성, 활성화, 비활성화 및 관리할 수 있습니다. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate> 로 이동합니다. 언제든지 음소거 규칙을 활성화하거나 비활성화할 수 있습니다. 다음을 클릭하세요. <Icon name="fe-more-horizontal"/> 규칙을 편집하고 제거하려면 각 규칙의 행에 있는 아이콘을 클릭하세요.
...
* <DoNotTranslate>**Scheduled**</DoNotTranslate>
  ...
  : 음소거가 활성화되어 있지만 아직 활성화되지 않았습니다(향후 일정이 있습니다).

* <DoNotTranslate>**Ended**</DoNotTranslate>

  : 음소거가 활성화되었지만 더 이상 활성화되지 않습니다(향후 일정이 없음).
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate>: 원치 않는 공지의 작은 또는 큰 집합에 대해 복잡한 음소거 규칙을 생성할 수 있습니다.
</figcaption>
...
음소거 규칙을 만들려면 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate> 으로 이동하여 <DoNotTranslate>**+ Add a rule**</DoNotTranslate> 클릭하세요. 음소거 규칙의 이름과 설명(선택 사항)을 입력하고 규칙을 적용할 계정을 선택하세요.

다음으로 인시던트 필터를 빌드합니다. [인시던트 이벤트 속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 의 하위 집합을 사용할 수 있습니다. 속성, [연산자](#sub-conditions), 값을 선택합니다. 속성은 다음과 같습니다: `accountId`, `conditionId`, `conditionName`, `conditionType`, , `entity.guid`, `nrqlEventType`, `nrqlQuery`, `policyId`, `policyName`, `product`(예 `runbookUrl` `conditionRunbookUrl`), `tags.<NAME>` 및 `targetName`). 값은 공지 ID 또는 조건 이름과 같은 인시던트 속성 중 하나와 비교됩니다. 더 많은 필터를 포함하려면 <DoNotTranslate>**Add another condition**</DoNotTranslate> 클릭하세요.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate>: 음소거 규칙 예약을 위한 유연하고 강력한 옵션입니다.
</figcaption>
...
<Callout variant="important">
  <DoNotTranslate>**Repeat**</DoNotTranslate> 요일 확인란은 <DoNotTranslate>**Starts**</DoNotTranslate> 및 <DoNotTranslate>**Ends**</DoNotTranslate> 날짜 필드를 재정의합니다. 시작 날짜를 설정하고 요일도 선택하면 시작 날짜 이후 1일에 음소거 규칙이 적용됩니다.
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      >
        <DoNotTranslate>**Event**</DoNotTranslate>: 이슈가 활성화되었습니다
      </td>
    </tr>
    ...
    <tr>
      <td>
        <DoNotTranslate>**is**</DoNotTranslate> 이(가) 음소거한 인시던트로 인해 문제가 활성화되었습니다.
      </td>
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Issues**</DoNotTranslate> 으로 이동하여 음소거된 문제를 클릭하세요.
    </figcaption>
  </Collapser>
  ...
  >
    인시던트와 이슈는 <Icon name="fe-bell-off"/> <DoNotTranslate>**Muted**</DoNotTranslate> 열의 아이콘:
