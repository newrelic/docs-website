...
원치 않는 알림에서 공통 요소를 발견한 후에는 해당 요소를 구체적으로 대상으로 하는 음소거 규칙을 정의하고 다른 알림은 통과시킬 수 있습니다. 알림이 음소거된 경우에도 <InlinePopover type="alerts"/>여전히 해당 사건에 대한 데이터를 수집합니다. 음소거 규칙은 알림 프로세스를 방해하지 않으며 알림이 전송되기 직전 시점에 적용됩니다.
...
다음으로 사건 필터를 구축합니다. [인시던트 이벤트 속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 의 하위 집합을 사용할 수 있습니다. 속성, [연산자](#sub-conditions), 값을 선택합니다. 속성은 다음과 같습니다: `accountId`, `conditionId`, `conditionName`, `conditionType`, `entity.guid`, `nrqlEventType`, `nrqlQuery`, `policyId`, `policyName` `product`,`runbookUrl` (예: `conditionRunbookUrl`), `tags.<NAME>` 및 `targetName`). 값은 경고 정책 ID 또는 조건 이름과 같은 인시던트 속성 중 하나와 비교됩니다. 더 많은 필터를 포함하려면 **Add another condition** \[다른 조건 추가를] 클릭하세요.
...
음소거 규칙은 [사고 이벤트](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 의 속성과 일치하는 일련의 조건을 사용합니다. 음소거 규칙은 다음 방법을 알려줍니다.
...
음소거 규칙은 문제 내에서 알림을 트리거한 첫 번째 이벤트에 따라 결정됩니다. 즉, 음소거 상태로 인해 첫 번째 알림 이벤트가 음소거된 경우 문제의 나머지 부분도 음소거됩니다.
...
<CollapserGroup>
  ...
  >
    음소거 규칙을 생성하고 대상 호스트가 `host-1` 이거나 인시던트가 합성 또는 인시던트인 인프라 조건에서 인시던트를 음소거하기 위한 음소거 규칙의 `id` 를 반환하려면 <InlinePopover type="mobile"/>:
