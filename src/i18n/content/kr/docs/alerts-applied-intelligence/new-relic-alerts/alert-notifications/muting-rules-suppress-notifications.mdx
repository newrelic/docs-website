...
다음으로 인시던트 필터를 빌드합니다. [인시던트 이벤트 속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes)의 하위 집합을 사용할 수 있습니다. 속성, [연산자](#sub-conditions)및 값을 선택합니다. 속성은 다음과 같습니다. `accountId` , `conditionId` , `conditionName` , `conditionType` , `entity.guid` , `nrqlEventType` , `nrqlQuery` , `policyId` , `policyName` , `product` , `runbookUrl` (as `conditionRunbookUrl` ), `tags.<NAME>` 및 `targetName` ). 알림 정책 ID 또는 조건 이름과 같은 인시던트 속성 중 하나와 값을 비교합니다. 더 많은 필터를 포함하려면 **다른 조건 추가를** 클릭합니다.
    ...
    <tr>
      ...
      <td>
        ...
        * `conditions`: 인시던트 내의 특성을 대상으로 하는 개별 표현식(하위 조건) 집합입니다. 이들은 `operator` 에 따라 함께 평가됩니다. 단일 음소거 규칙에 대해 최대 20개의 하위 조건을 가질 수 있습니다.
          ...
          * `attribute`: 인시던트 내의 단일 속성입니다. [인시던트 이벤트 속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 목록을 보려면 여기로 이동하십시오.
...
음소거 규칙은 [인시던트 이벤트](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 의 속성과 일치하는 일련의 조건을 사용합니다. 음소거 규칙은 다음 방법을 알려줍니다.
