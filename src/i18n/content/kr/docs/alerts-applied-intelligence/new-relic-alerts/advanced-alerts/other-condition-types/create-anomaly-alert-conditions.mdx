...
* 위반을 여는 동작을 설정합니다(예: "5분 이상 편차").
* 조건에서 상위 위반, 하위 위반 또는 둘 다를 확인할지 여부를 설정합니다.

데이터가 예측된 "정상적인" 동작을 벗어나고 선택한 기준을 충족하면 문제 생성 기본 설정에 따라 인시던트가 열리고 알림을 받을 수 있습니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) >Alerts & AI > Policies > (정책 생성 또는 선택) > Create alert condition:** 비정상 경고 조건은 비정상적인 동작이 감지된 경우에만 위반을 생성하는 지능형 자체 조정 임계값을 설정할 수 있는 기능을 제공합니다.
</figcaption>

이상 조건을 생성하려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Policies > (정책 생성 또는 선택) > Create alert condition** 으로 이동합니다. [조건](/docs/alerts/new-relic-alerts/defining-conditions/define-alert-conditions#create-condition) 생성을 시작할 때 다음 [데이터 소스](#availability) 중 하나를 선택합니다.
...
* 이상 현상 위 또는 아래에서 발생하는 예기치 않은 동작을 모니터링하도록 [이상 방향](#upper-lower) 을 설정합니다.
...
* 슬라이더 막대를 사용하여
  ...
  <img title="Alerts v3: Critical alert icon" alt="icon-alert-critical.png" src={alertsIconAlertCritical}>을 조정합니다.</img>

  **임계값 **임계값 감도, 미리 보기 차트에서 이상 현상 주변의 밝은 회색 영역으로 표시됩니다. 이상 주변의 밴드가 더 조밀할수록 더 민감해지고 더 많은 위반이 생성됩니다.

* 선택 사항:

  <img title="Alerts v3: Warning alert icon" alt="icon-alert-warning.png" src={alertsAlertWarning}>을 생성할 수 있습니다.</img>
