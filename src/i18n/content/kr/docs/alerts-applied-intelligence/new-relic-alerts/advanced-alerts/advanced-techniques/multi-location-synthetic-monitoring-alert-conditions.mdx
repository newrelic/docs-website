...
3. **New alert condition** \[새 알림 조건을] 선택합니다.
4. **Build a classic alert** \[기본 알림 작성 을] 선택합니다.
5. **Synthetics** \[신세틱스] 및 **Multiple location failures** \[다중 위치 오류 를] 선택합니다.
6. **Next** \[다음을] 클릭하여 목표로 삼을 모니터를 선택하세요.
7. **Next** \[다음 을] 클릭합니다. **Set thresholds** \[레버 값 설정, 릴레이] 단계에서 필드를 작성합니다.
8. **Save condition** \[조건 저장 을] 클릭합니다.
