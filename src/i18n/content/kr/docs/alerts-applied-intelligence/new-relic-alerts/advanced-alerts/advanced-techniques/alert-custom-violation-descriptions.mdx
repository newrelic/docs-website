...
## 사용자 지정 위반 설명이란 무엇입니까? [#description]

일부 [조건 유형의](#requirements) 경우 조건 및 해당 위반 다운스트림에 대한 정보를 전달하는 사용자 지정 위반 설명을 생성할 수 있습니다. 설명을 사용하여 다음을 수행할 수 있습니다.
...
**사용자 지정 위반 설명** 필드를 사용하면 위반이 생성될 때 [일반 섹션을 특정 값으로 대체하는](#substitution-format) 일반 템플릿 구조를 사용할 수 있습니다. 예를 들어 설명에서 이것을 사용했다고 가정해 보겠습니다.
...
## 사용자 지정 위반 설명 만들기 [#create-description]
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com)** 상단 탐색에서 **경고 및 AI** , **경고 조건(정책)** , **(정책 선택** ), ( **조건 선택** )을 차례로 클릭합니다. **+ 사용자 지정 위반 설명 추가** 를 클릭하여 필드를 엽니다.
</figcaption>

[NRQL 경고에 대한 전용 필드를](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/) 사용하거나 [인프라 경고에 대해 이 조건 설명 섹션을](/docs/infrastructure/new-relic-infrastructure/infrastructure-alert-conditions/infrastructure-alerts-add-edit-or-view-host-alert-information/) 사용하여 사용자 지정 위반 설명을 만들 수 있습니다.

## 사용자 정의 위반 설명 템플릿 예 [#example]
...
사용자 정의 위반 설명에서 사용할 수 있는 속성은 [위반 이벤트](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes) 에 첨부된 속성의 하위 집합입니다. 다음은 사용 가능한 속성 및 사용 방법에 대한 몇 가지 세부 정보입니다.

* 위반 이벤트 속성: [사용자 정의 위반 설명에 사용할 수 있는 표준 속성](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes) 의 경우 `{{AN_ATTRIBUTE}}` 형식을 사용합니다. 예를 들어:
...
## NRQL을 사용하여 사용자 지정 위반 설명 만들기 [#create-description-nrql]
...
* 인프라 모니터링 조건 [의 경우 인프라 모니터링 경고 조건을 생성](/docs/infrastructure/new-relic-infrastructure/infrastructure-alert-conditions/infrastructure-alerts-add-edit-or-view-host-alert-information) 할 때 사용자 지정 위반 설명을 생성할 수 있습니다.
* 사용자 지정 위반 설명을 구성하는 방법에 대해 알아보려면 [예제 템플릿](#example) 및 [속성/태그 지침](#attributes-tags) 을 참조하세요.
