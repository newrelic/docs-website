...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (정책 선택) > (조건 선택)**: **+ Add custom incident description** \[+ 사용자 지정 사고 설명 추가를] 클릭하여 필드를 엽니다.
</figcaption>
...
사용자 정의 인시던트 설명에서 사용할 수 있는 속성은 [인시던트 이벤트](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 에 첨부된 속성의 하위 집합입니다. 다음은 사용 가능한 속성과 사용 방법에 대한 몇 가지 세부 정보입니다.

* 인시던트 이벤트 속성: [사용자 정의 인시던트 설명에 사용할 수 있는 표준 속성](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/incident-event-attributes) 의 경우 `{{AN_ATTRIBUTE}}` 형식을 사용합니다. 예를 들어:
...
* 태그 속성: 태그의 경우 `{{tags.TAG_NAME}}` 형식을 사용합니다. 두 가지 유형의 태그가 있습니다.
  ...
  * 엔터티 관련 태그: **인프라 조건 전용입니다.** [엔터티](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer-view-performance-across-apps-services-hosts) 탐색기 로 이동하고 서비스의 **Metadata and tags** \[메타데이터 및 태그 아래를] 보거나 인시던트 세부 정보를 확인하여 사용 가능한 엔터티 관련 [태그를 찾을 수 있습니다.](/docs/new-relic-one/use-new-relic-one/core-concepts/tagging-use-tags-organize-group-what-you-monitor#) 다음은 사용자 정의 사건 설명에서 태그를 사용하는 예입니다.
