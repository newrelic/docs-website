...
스트리밍 <InlinePopover type="alerts"/>플랫폼은 New Relic으로 들어오는 데이터 스트림 또는 [신호](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal) 에 존재하거나 누락된 데이터를 기반으로 사고를 확인합니다.
