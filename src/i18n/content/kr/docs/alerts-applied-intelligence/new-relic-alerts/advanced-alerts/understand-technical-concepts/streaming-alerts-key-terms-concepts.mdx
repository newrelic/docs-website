...
import accountsStreamingAlertsAggregationFlowchart from 'images/accounts_diagram_streaming-alerts-aggregation-flowchart.png'

import accountsStreamingAlerts from 'images/accounts_diagram_streaming-alerts.png'

스트리밍 알림 플랫폼은 New Relic에 들어오는 데이터 또는 [신호](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal) 스트림에 존재하거나 누락된 데이터를 기반으로 위반 사항을 확인합니다.
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      데이터가 일관되고 예측 가능하게 도착하는 **경우 이벤트 흐름** 을 사용하십시오. 데이터가 일관되지 않고 예측할 수 없이 도착하는 **경우 이벤트 타이머** 를 사용하십시오.
    </figcaption>
  </Collapser>
</CollapserGroup>
...
데이터가 New Relic으로 스트리밍되면 NRQL 조건으로 필터링됩니다. 데이터가 평가되기 전에 [NRQL 쿼리의 `WHERE` 절](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions#sel-where) 에 정의된 기준을 충족해야 합니다. 위반에 대해 해당 데이터를 즉시 평가하는 대신 NRQL 경고 조건은 집계 창으로 알려진 기간 동안 데이터를 수집합니다. 추가 지연/타이머를 사용하면 창이 집계되기 전에 더 느린 데이터 포인트가 도착할 수 있습니다.

지연/타이머 시간이 경과하면 New Relic은 데이터를 단일 데이터 포인트로 집계합니다. 그런 다음 경고는 위반 임계값 기준을 충족하는지 확인하기 위해 NRQL 조건을 사용하여 데이터 포인트를 평가합니다.

데이터 포인트가 위반 기준을 충족하더라도 위반이 트리거되지 않을 수 있습니다. 위반은 데이터 포인트가 일정 기간 동안 임계값 기준을 일관되게 충족하는 경우에만 트리거됩니다. 임계값 지속 시간입니다. 데이터 포인트가 전체 임계값 기간 동안 위반되면 정책 설정에 따라 알림을 보내드립니다.
...
<table class="alternate">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        위반 생성 여부를 결정하는 특정 기간입니다. 지정된 NRQL 조건이 임계값 기간 동안 임계값 기준을 충족하는 경우 위반이 발생합니다.
...
집계 기간을 **30초** 에서 **15분** 사이로 설정할 수 있습니다. 기본값은 **1분** 입니다.
...
지연 기본값은 **2분** 입니다. 타이머 기본값은 **1분** 이며 최소값은 **5초** 입니다.

### 신호 감지 손실 [#signal-loss]

신호 손실은 특정 기간 동안 NRQL 조건과 일치하는 데이터가 없을 때 발생합니다. 신호 손실은 다양한 원인에 의해 발생합니다. NRQL 쿼리의 `WHERE` 절은 위반이 평가되기 전에 데이터를 필터링할 수 있습니다. 또한 서비스 또는 엔터티가 오프라인 상태이거나 정기 작업이 실행되지 않아 New Relic으로 데이터가 전송되지 않고 있음을 의미할 수도 있습니다.

불필요한 알림을 피하기 위해 신호 위반 손실로 알림을 받기까지 기다릴 시간을 선택할 수 있습니다. 신호 손실 감지를 사용하여 위반을 열고 신호가 손실될 때 알림을 받을 수 있습니다. 또는 신호 손실을 사용하여 오류 수와 같은 임시 서비스 또는 산발적인 데이터에 대한 위반을 닫을 수 있습니다.
...
<Callout variant="tip">
  ...
  신호 손실 및 간격 채우기에 대해 자세히 알아보려면 [이 Explorers Hub 게시물](https://discuss.newrelic.com/t/relic-solution-how-can-i-figure-out-when-to-use-gap-filling-and-loss-of-signal/120401) 을 참조하세요.
</Callout>