...
스트리밍 알림 플랫폼은 New Relic으로 들어오는 데이터 스트림 또는 [신호](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal) 에 존재하거나 누락된 데이터를 기반으로 사건을 확인합니다.
...
데이터 포인트가 인시던트 기준을 충족하더라도 인시던트가 열리지 않을 수 있습니다. 데이터 포인트가 일정 기간 동안 일관되게 임계값 기준을 충족하는 경우에만 인시던트가 열립니다. 임계값 기간입니다. 데이터 포인트가 전체 임계값 기간 동안 위반하는 경우 정책 설정에 따라 알림을 보냅니다.
