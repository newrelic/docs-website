...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (select a policy) > Add a condition**으로 이동합니다. **NRQL**을 클릭한 후 **Next를 클릭하고 임계값을 정의합니다**.
</figcaption>
...
null 값을 처리하는 방법을 결정하려면 [alerts 조건 UI](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#signal-loss)에서 신호 손실 및 간격 채우기 설정을 조정합니다.
...
* 슬라이드 바이 간격 변경( [슬라이딩 창 집계](https://docs.newrelic.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#sliding-window-aggregation) 조건에서만)

다음 작업(위 목록에서 다루지 않은 다른 작업과 함께)은 평가를 재설정**하지 않습니다**.

* 신호 손실 기간 변경(만료 기간)
* 시간 함수 변경 (**for at least(최소)**에서 **at least once in(동안 최소 한 번)**으로 또는 그 반대로 전환)
* **손실된 신호 추가 임계값** 설정
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (정책 선택)**으로 이동한 후 **Add a condition**을 선택합니다. 신호 손실은 NRQL 조건에서만 사용할 수 있습니다.
</figcaption>
...
[세 가지 스트리밍 집계 방법](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#aggregation-methods) 중에서 선택하여 각 조건에 대한 최상의 평가 결과를 얻을 수 있습니다.
...
* **None**: (기본값) 빈 집계 기간에 대해 어떤 작업도 수행하지 않으려면 선택합니다. 평가 후, 빈 집계 기간이 임계값 기간 타이머를 재설정합니다. 예를 들어, 조건에 모든 집계 기간에 5분 동안 임계값 이상의 데이터 포인트가 있어야 하고 집계 기간 5개 중 1개가 비어 있어야 한다는 사실이 포함된 경우, 이 조건은 인시던트를 열지 않습니다.
...
* NerdGraph는 이에 대한 권장 API이지만 REST API를 사용하는 경우, REST API 탐색기에서 [Alert NRQL conditions API](https://api.newrelic.com/docs/#/Alerts%20NRQL%20Conditions)의 **"signal"** 섹션 아래에서 이 설정을 찾을 수 있습니다.