...
<CollapserGroup>
  ...
  >
    ...
    앱의 [선택된 각 인스턴스](#instance-incidents) 에 대해 경고 임계값 위반을 개별적으로 평가합니다. 조건을 만들 때 Java 앱의 경고 정책에 대한 조건 유형으로 **JVM health metric** \[JVM 상태 지표를] 선택한 다음 사용 가능한 임계값을 선택합니다.
  ...
  >
    ...
    2. 인시던트를 여는 임계값을 정의하려면 **Percentile nth** \[백분위수 n번째] 응답 시간 값을 입력한 다음 해당 빈도(이 값보다 높거나 낮거나 같음)를 선택합니다.
  ...
  >
    ...
    조건과 함께 동적 대상 지정을 사용하려면 [인시던트 종료 타이머를](/docs/alerts-applied-intelligence/new-relic-alerts/alert-incidents/how-alert-condition-incidents-are-closed#time-limit) 설정해야 합니다.
