...
[NRQL 쿼리](/docs/insights/new-relic-insights/using-new-relic-query-language/using-nrql)를 사용하여 알림 조건을 생성할 수 있습니다. 신호를 정의한 후에는 알림 및 위험의 임계값 수준을 추가로 정의할 수 있습니다. 이는 알림 위반이 생성되는 시기를 결정합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `WHERE` 절을 사용하여 하나 이상의 조건을 지정합니다. 모든 [연산자](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#where-operators)가 지원됩니다.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `COMPARE WITH` 절은 두 개의 다른 시간 범위에 대한 값을 비교하는 데 사용됩니다. 이 유형의 쿼리는 NRQL 경고와 호환되지 않습니다. 특정 신호에 대한 편차를 동적으로 감지하려면 [기준 알람 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-baseline-alert-conditions)을 사용하는 것이 좋습니다.
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    소수의 주요 고객 또는 데이터 범위와 같은 데이터의 특정 세그먼트를 대상으로 하는 제한된 알람을 생성합니다. `WHERE` 절을 사용하여 이러한 조건을 정의합니다.
...
즉, `count()` 및 `uniqueCount()` 같은 집계자는 0 값을 반환하지 않습니다. 개수가 0이면 `SELECT` 절이 무시되고 데이터가 반환되지 않으므로 `NULL` 값이 반환됩니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="[timeslicemetrics](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data)에 대한 중첩 쿼리는 현재 지원되지 않습니다."
  >
    내부 쿼리에 다음 용어가 포함된 중첩 쿼리는 NRQL 알림 조건에서 허용되지 않습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        신호 손실 감지를 사용하여 데이터(텔레메트리 신호)가 손실된 것으로 간주되어야 하는 시기를 알릴 수 있습니다. 신호 손실은 서비스 또는 엔터티가 더 이상 온라인 상태가 아니거나 정기 작업 실행에 실패했음을 나타낼 수 있습니다. 신호가 들어오지 않을 때 오류 카운트같은 산발적 데이터에 대한 위반이 닫히도록 하는 데 이 기능을 사용할 수도 있습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * 위반 및 알림에 포함될 [조건에 대한 사용자 지정 위반 설명](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions)을 제공합니다.
...
* **신호 손실 조치** 신호가 손실된 것으로 간주되면 열린 위반을 닫거나, 새 위반을 열거나, 둘을 모두 수행할 수 있습니다.
  ...
  * 새 위반 열기: 신호가 손실된 것으로 간주될 때 새 위반이 열립니다. 이러한 위반은 신호 손실로 인한 것임을 나타냅니다. 인시던트 기본 설정에 따라 알림을 트리거해야 합니다. 이에 대한 graphQL 노드 이름은 ["openViolationOnExpiration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)"입니다.
...
5. 신호가 손실되었을 때 어떤 일이 일어나길 원하는지 선택합니다. **Close all current open violations** 및 **Open new "lost signal" violation** 중 하나 또는 둘 다를 확인할 수 있습니다. 이는 특정 조건에서 신호 위반 손실을 처리하는 방법을 제어합니다.
