...
[NRQL 쿼리](/docs/insights/new-relic-insights/using-new-relic-query-language/using-nrql) 를 사용하여 경고 조건을 생성할 수 있습니다. 신호를 정의한 후에는 경고 및 위험 임계값 수준을 추가로 정의할 수 있습니다. 경고 위반이 생성되는 시점을 결정합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com)** 으로 이동하여 **경고 및 AI** 를 클릭하고 왼쪽 사이드바에서 **경고 조건(정책)** 을 클릭하고 정책을 선택한 다음 **조건 추가** 를 클릭합니다. **NRQL** 을 클릭하고 다음 **으로 임계값을 정의** 합니다.
</figcaption>

<Callout variant="tip">
  NRQL 경고 조건 및 스트리밍 경고와 관련된 주요 개념에 대한 자세한 내용은 [스트리밍 경고: 주요 용어 및 개념](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts) 을 참조하십시오.
</Callout>

시작할 준비가 되셨습니까? 아직 [가입하지 않았다면 New Relic 계정](https://newrelic.com/signup) 에 가입하십시오. 영원히 무료입니다.

## NRQL 경고 조건 생성 [#create]

정책에 대한 NRQL 경고 조건을 생성하려면:

* [one.newrelic.com](https://one.newrelic.com) 의 헤더에서 **경고 및 AI** 를 클릭한 다음 왼쪽 사이드바에서 **경고 조건(정책)** 을 클릭합니다.
* 기존 정책을 선택하거나 **새 경고 정책** 을 클릭 [하여 새 정책을 만듭니다](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy) .
* **조건 추가 를** 클릭합니다.
* **제품 선택에서** **NRQL** 을 클릭한 후 **다음, 임계값 정의** 를 클릭합니다.
...
<figcaption>
  차트에서 NRQL 경고 조건을 생성하려면 차트 메뉴를 클릭하십시오. <Icon name="fe-more-horizontal"/>을 클릭한 **다음 경고 조건 생성** 을 클릭합니다.
</figcaption>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `WHERE` 절을 사용하여 일련의 하나 이상의 조건을 지정합니다. 모든 [연산자](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#where-operators) 가 지원됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [임계값 유형](#threshold-types) (정적 또는 기준선)에 따라 NRQL 구문에 선택적 `FACET` 절을 포함합니다.

        [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-resources/nrql-syntax-components-functions) 절을 사용하여 결과를 속성별로 구분하고 각 속성에 대해 개별적으로 경고합니다. `LIMIT` 절은 허용되지 않지만 모든 쿼리는 가능한 최대 수의 패싯을 받습니다.

        패싯 쿼리는 [정적 및 기준](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries#threshold-types) 조건에 대해 최대 5000개의 값을 반환할 수 있습니다.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        NRQL 조건은 창 쿼리 결과의 끝없는 스트림을 생성하므로 쿼리 범위를 특정 시점으로 지정하는 `SINCE` 및 `UNTIL` 키워드는 호환되지 않습니다. 편의상 차트 컨텍스트에서 조건을 생성할 때 쿼리에서 `SINCE` 및 `UNTIL` 을 자동으로 제거합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        `histogram()` NRQL 경고와 호환되지 않습니다. 히스토그램 집계는 시계열 형식으로 지정할 수 없습니다. 히스토그램의 일부(예: 95번째 백분위수)에서 경고를 생성하려면 [`percentile()`](/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#functions) 집계 함수를 사용합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `COMPARE WITH` 절은 두 개의 다른 시간 범위에 대한 값을 비교하는 데 사용됩니다. 이 유형의 쿼리는 NRQL 경고와 호환되지 않습니다. 특정 신호에 대한 편차를 동적으로 감지하려면 [기준선 경고 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-baseline-alert-conditions) 을 사용하는 것이 좋습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        UI에서 슬라이딩 창을 활성화할 수 있습니다. 조건을 생성하거나 편집할 때 **고급 신호 설정 미세 조정** > **데이터 집계 설정** > **슬라이딩 윈도우 집계 사용** 으로 이동합니다.
      </td>
    </tr>
...
<CollapserGroup>
  ...
  <Collapser
    id="max-min-avg"
    title="데이터의 최대, 최소, 평균 알림"
  >
    데이터가 특정 최대값, 최소값 또는 평균에 도달하면 경고를 생성합니다. 예를 들어 지속 시간 또는 응답 시간이 특정 임계값을 초과하지 않도록 합니다.
...
즉, `count()` 및 `uniqueCount()` 과 같은 애그리게이터는 0 값을 반환하지 않습니다. 개수가 0이면 `SELECT` 절이 무시되고 데이터가 반환되지 않으므로 `NULL` 값이 반환됩니다.
...
### 예: null 대 0 값이 반환됨 [#example-null-zero]

null 값을 처리하는 방법을 결정하려면 [경고 조건 UI](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#signal-loss) 에서 신호 손실 및 간격 채우기 설정을 조정합니다.
...
자세한 내용은 0 대 null 값 문제 해결에 대한 [블로그 게시물](https://discuss.newrelic.com/t/relic-solution-how-can-i-figure-out-when-to-use-gap-filling-and-loss-of-signal/120401) 을 확인하세요.

## 중첩 집계 NRQL 경고 [#h2-nested-aggregation-nrql-alerts]
...
<CollapserGroup>
  <Collapser
    ...
    title="패싯 처리되지 않은 가장 안쪽 쿼리가 포함된 중첩 쿼리는 현재 지원되지 않습니다."
  >
  ...
  >
    1분의 경고 집계 창에서 내부 쿼리는 30초의 더 작은 두 개의 창을 생성합니다. 이론상 이 두 창은 외부 쿼리에 의해 집계될 수 있습니다. 그러나 이것은 현재 지원되지 않습니다.
  ...
  >
    신호 손실에 대한 자세한 내용은 [NerdGraph API: 신호 손실 및 간격 채우기 를](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) 참조하십시오.
  </Collapser>

  <Collapser
    id="with_metric_format"
    title="내부 쿼리에 'WITH METRIC_FORMAT'이 포함된 중첩 쿼리는 현재 지원되지 않습니다."
  >
    내부 쿼리에 `WITH METRIC_FORMAT` 이 포함된 중첩 쿼리를 사용하여 NRQL 경고 조건을 생성할 수 없습니다.
  </Collapser>
</CollapserGroup>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        NRQL 조건 유형에는 [정적 및 기준선](#threshold-types) 이 포함됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        NRQL 조건의 경우 사용자 지정 [설명](/docs/alerts/new-relic-alerts/defining-conditions/alert-condition-descriptions) 을 만들어 각 위반에 추가할 수 있습니다. 특정 위반의 메타데이터를 기반으로 하는 변수 대체로 설명이 향상될 수 있습니다.

        자세한 내용은 [설명](/docs/alerts/new-relic-alerts/defining-conditions/alert-condition-descriptions) 을 참조하십시오.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        케이던스 집계 방법의 경우 평가할 분을 지정하는 암시적 `SINCE ... UNTIL` 절은 [**지연/타이머**](#time) 설정에 의해 제어됩니다. 매우 최근 데이터는 불완전할 수 있으므로 특히 다음과 같은 경우 3분 이상 전의 데이터를 쿼리할 수 있습니다.
    ...
    <tr>
      ...
      <td>
        신호 손실 감지를 사용하여 데이터(원격 측정 신호)가 손실된 것으로 간주되어야 하는 시기를 알릴 수 있습니다. 신호 손실은 서비스 또는 엔터티가 더 이상 온라인 상태가 아니거나 정기 작업 실행에 실패했음을 나타낼 수 있습니다. 신호가 들어오지 않을 때 오류 카운트와 같은 산발적인 데이터에 대한 위반이 닫히도록 이 기능을 사용할 수도 있습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        이러한 설정은 때때로 누락될 수 있는 연속 스트리밍 데이터 신호를 더 잘 처리하기 위한 옵션을 제공합니다. 이러한 설정에는 집계 창 기간, 지연/타이머 및 데이터 간격 채우기 옵션이 포함됩니다. 사용에 대한 자세한 내용은 [고급 신호 설정](#advanced-signal) 을 참조하십시오.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * 간결하고 설명적인 [조건 이름](/docs/alerts/new-relic-alerts/configuring-alert-policies/define-alert-conditions#rename-condition) 을 만듭니다.
        * 위반 및 알림에 포함될 [조건에 대한 사용자 지정 위반 설명을](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions) 제공합니다.
        * 인시던트 처리를 위한 조직의 절차를 포함하도록 Runbook URL을 추가합니다. [사용자 정의 위반 설명](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions) 에 이 정보를 추가할 수도 있습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [최대값](/docs/alerts/new-relic-alerts/getting-started/minimum-maximum-values) 을 참조하십시오.
      </td>
    </tr>
...
기존 NRQL 조건을 편집할 때 조건 엔터티와 연결된 태그를 추가하거나 제거하는 옵션이 있습니다. 이렇게 하려면 조건 이름 아래에 있는 **태그 관리** 버튼을 클릭합니다. 팝업 메뉴에서 태그를 추가하거나 삭제합니다.
...
* "신호 손실 시 위반 닫기" 설정 변경
* 간격 채우기 설정 변경
* 기준선 방향 변경(해당되는 경우) – 더 높거나 낮거나 높거나 낮음
...
* 신호 손실 기간 변경(만료 기간)
* 시간 기능 변경("적어도 한 번"에서 "적어도 한 번"으로 또는 그 반대로 전환)
* "신호 손실 시 개방 위반" 설정 토글

## 경고 조건 유형 [#threshold-types]
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        [기준선](/docs/alerts/new-relic-alerts/defining-conditions/create-baseline-alert-conditions) (동적)
      </td>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com)** 으로 이동하여 **경고 및 AI** 를 클릭하고 왼쪽 사이드바에서 **경고 조건(정책)** 을 클릭하고 정책을 선택한 다음 **조건 추가** 를 클릭합니다. 신호 손실은 NRQL 조건에서만 사용할 수 있습니다.
</figcaption>

GraphQL API(권장) 또는 REST API를 사용하여 이러한 설정을 관리할 수도 있습니다. 특정 [GraphQL API 예제](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) 를 보려면 여기로 이동하십시오.
...
* **신호 손실 만료 시간**
  ...
  * UI 레이블: **다음 이후에 신호가 손실됩니다.**
  * GraphQL 노드: [만료](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) .expirationDuration
...
* **신호 손실 조치 신호가 손실된** 것으로 간주되면 열린 위반을 닫거나 새 위반을 열거나 둘 모두를 수행할 수 있습니다.

  * 현재 열려 있는 모든 위반 닫기: 특정 신호와 관련된 모든 열려 있는 위반을 닫습니다. 조건에 대한 모든 위반을 반드시 닫는 것은 아닙니다. 임시 서비스 또는 산발적인 신호에 대해 경고하는 경우 위반이 제대로 종료되도록 이 작업을 선택하는 것이 좋습니다. 이에 대한 GraphQL 노드 이름은 ["closeViolationsOnExpiration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) "입니다.
  * 새 위반 열기: 신호가 손실된 것으로 간주될 때 새 위반이 열립니다. 이러한 위반은 신호 손실로 인한 것임을 나타냅니다. 인시던트 기본 설정에 따라 알림을 트리거해야 합니다. 이에 대한 graphQL 노드 이름 [은 "openViolationOnExpiration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) "입니다.
  * 두 작업을 모두 활성화하면 먼저 모든 열린 위반을 닫은 다음 신호 손실에 대한 새 위반을 엽니다.

UI에서 신호 손실 감지로 구성된 NRQL 경고를 생성하려면:

1. 정책의 경우 조건을 생성할 때 **제품 선택 에서** **NRQL** 을 클릭한 후 **다음, 임계값 정의** 를 클릭합니다.
2. 경고하려는 값을 반환하는 [NRQL 쿼리](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries#syntax) 를 작성하십시오.
3. **임계값 유형** 으로 **정적** 또는 **기준선** 을 선택합니다.
4. **+ 손실된 신호 임계값 추가** 를 클릭한 다음 **신호가 손실된 후** 필드에서 신호 만료 기간을 분 또는 초 단위로 설정합니다.
5. 신호가 손실되었을 때 어떤 일이 일어나길 원하는지 선택하십시오. **현재 열려 있는 모든 위반 닫기** 및 **새 "신호 손실" 위반 열기** 중 하나 또는 둘 다를 확인할 수 있습니다. 이는 조건에 대한 신호 위반 손실을 처리하는 방법을 제어합니다.
6. 저장하기 전에 조건의 이름을 지정했는지 확인하십시오.

신호 손실로 인해 위반이 열리면 닫힙니다.

* 신호가 돌아옵니다. 새로 열린 손실 신호 위반은 새 데이터가 평가되면 즉시 닫힙니다.
...
<figcaption>
  NRQL 경보 조건을 생성할 때 고급 신호 설정을 사용하여 [스트리밍 경보 데이터](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts) 를 제어하고 잘못된 경보를 방지하십시오.
</figcaption>
...
이러한 설정이 무엇이며 서로 어떻게 관련되는지에 대한 설명을 읽으려면 [스트리밍 알림 개념](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts) 을 참조하세요. 다음은 구성 방법에 대한 지침과 팁입니다.

### 집계 기간 [#window-duration]

[집계 창 기간](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts#aggregation-window) 을 설정하여 데이터가 집계되기 전에 스트리밍 시간 창에서 데이터가 축적되는 기간을 선택할 수 있습니다. 30초에서 120분 사이로 설정할 수 있습니다. 기본값은 1분입니다.

### 슬라이딩 윈도우 집계 [#sliding-window-aggregation]

[슬라이딩 창](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/create-smoother-charts-sliding-windows) 을 사용하여 더 부드러운 차트를 만들 수 있습니다. 이것은 데이터의 겹치는 창을 만들어 수행됩니다.
...
<Callout variant="important">
  새로운 슬라이딩 윈도우 경고 조건을 생성하거나 [평가 재설정](#evaluation-resets) 을 유발할 수 있는 작업을 수행한 직후, 조건은 첫 번째 집계 윈도우 기간 동안 "집계된 버퍼"를 구축하는 데 시간이 필요합니다. 이 시간 동안에는 위반이 트리거되지 않습니다. 단일 집계 기간이 지나면 완전한 "버퍼"가 구축되고 조건이 정상적으로 작동합니다.
</Callout>

### 스트리밍 방식 [#streaming]

[세 가지 스트리밍 집계 방법](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/streaming-alerts-key-terms-concepts/#aggregation-methods) 중에서 선택하여 조건에 대한 최상의 평가 결과를 얻으십시오.
...
<Callout variant="important">
  AWS Cloudwatch 또는 Azure와 같은 [Infrastructure Cloud Integrations](/docs/infrastructure/infrastructure-integrations/get-started/introduction-infrastructure-integrations) 에서 수집한 데이터에 대한 NRQL 조건을 생성할 때 이벤트 타이머 방식을 사용하는 것이 좋습니다.
</Callout>
...
* **없음** : (기본값) 빈 집계 창에서 작업을 수행하지 않으려면 이 옵션을 선택합니다. 평가 시 빈 집계 창은 임계값 지속 시간 타이머를 재설정합니다. 예를 들어 조건에 모든 집계 기간이 5분 동안 임계값을 초과하는 데이터 포인트가 있어야 하고 5개 집계 기간 중 1개가 비어 있으면 조건이 위반되지 않습니다.
* **사용자 정의 정적 값** : 사용자 정의 정적 값이 평가되기 전에 빈 집계 창에 삽입하려면 이 옵션을 선택합니다. 이 옵션에는 사용해야 하는 정적 값을 지정하는 추가 필수 매개변수 `fillValue` (API에 명명됨)이 있습니다. 기본값은 `0` 입니다.
* **마지막으로 알려진 값** : 이 옵션은 평가가 발생하기 전에 마지막으로 본 값을 삽입합니다. 마지막으로 본 값의 상태를 2시간 동안 유지합니다.
...
* NRQL 조건 UI에서 **조건 설정** > **고급 신호 설정** > **데이터 간격 채우기** 로 이동하고 옵션을 선택합니다.
