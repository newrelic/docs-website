...
import queriesPolicyNewAlertCondition from 'images/queries-nrql_screenshot-crop_create-alert-condition.png'

import queriesMetricExplorer from 'images/queries-nrql_screenshot-crop_metrics-create-alert-condition.png'
...
**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (policies)**으로 이동한 후 정책에서 NRQL 알림 조건을 생성합니다. **+ New alert condition**을 클릭합니다.

<img
  width="80%;"
  title="Create a NRQL alert condition from a policy"
  alt="Create a NRQL alert condition from a policy"
  src={queriesPolicyNewAlertCondition}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (policies) > + New alert condition**으로 이동합니다.
</figcaption>
...
<CollapserGroup>
  <Collapser
    ...
    title="1단계. 신호 정의"
  >
    1. **Build a query** 옵션을 선택합니다. 이 옵션을 사용하면 신호를 식별하고 NRQL 쿼리를 추가할 수 있습니다. 쿼리를 추가하는 데 도움이 필요한 경우 [NRQL 사용 방법: 쿼리 메커니즘](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-how-nrql-works/)을 참조하십시오.
    2. **Next**를 클릭합니다.
  </Collapser>
  ...
  <Collapser
    ...
    title="3단계. 임계값 정의"
  >
    인시던트를 여는 항목을 결정하기 위해 [임계값(thresholds)](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition/)을 정의할 수 있습니다. 정의했으면 **Next**를 클릭합니다.
  </Collapser>
  ...
  >
    ...
    * **Save & set up notifications**을 클릭합니다.

    **Existing policy**를 선택한 경우 정책을 선택한 후 **Save condition**을 클릭합니다.
  </Collapser>
</CollapserGroup>
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services**로 이동하여 앱을 선택합니다.

2. 왼쪽 창에서 **More views > Metrics explorer**를 선택하고 구성을 추가합니다.

3. <Icon name="fe-more-horizontal"/>아이콘을 클릭하고 **Create alert condition**을 선택합니다.

   <img
     title="Create alert condition from the metrics explorer"
     alt="Create alert condition from the metrics explorer"
     src={queriesMetricExplorer}
   />

4. [정책에서 NRQL 알림 조건 생성](#alert-condition-from-policy)에 설명된 단계를 따르지만 이전에 구성한 데이터로 이미 빌드된 쿼리가 표시되기 때문에 1단계가 약간 다르다는 점에 주의하시기 바랍니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [데이터 유형 하나](/docs/data-apis/understand-data/new-relic-data-types/)만 타깃할 수 있습니다.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        UI에서 슬라이딩 창을 활성화할 수 있습니다. 조건을 생성하거나 수정하는 경우 **Adjust to signal behavior > Data aggregation settings > Use sliding window aggregation**으로 이동합니다.
      </td>
    </tr>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * 인시던트 및 알림에 포함될 [조건에 대한 커스텀 인시던트에 대한 설명](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions)을 제공합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies)**, then **+ New alert condition**으로 이동합니다. 신호 손실은 NRQL 조건에서만 사용할 수 있습니다.
</figcaption>
...
* **신호 손실 조치** 신호가 손실된 것으로 간주되면 시작된 인시던트를 닫거나 새 인시던트를 열거나, 아니면 둘 다 수행할 수 있습니다.
...
4. **+ Add lost signal threshold**를 클릭한 다음 **Consider the signal lost after** 필드에서 신호 만료 기간을 분 또는 초 단위로 설정합니다.
5. 신호가 손실되었을 때 어떻게 되기를 원하는지 선택합니다.**Close all current open incidents** 및 **Open new "lost signal" incidents** 중 하나 또는 두 가지 모두를 선택할 수 있습니다. 이는 조건에 대해 신호 인시던트의 손실을 처리하는 방법을 제어합니다.
...
* **Close all current open incidents** 옵션을 사용해 인시던트를 수동으로 닫았습니다.
...
* **None**: (기본값) 빈 집계 기간에 대해 어떤 작업도 수행하지 않으려면 선택합니다. 평가 시 빈 집계 기간이 임계값 기간 타이머를 재설정합니다. 예를 들어 조건에 모든 집계 기간에 5분 동안 임계값 이상의 데이터 포인트가 있어야 하고 집계 기간 5개 중 1개가 비어 있는 경우 이 조건은 인시던트가 아닙니다.
...
* NRQL 조건 UI에서 **Adjust to signal behavior > Use evaluation delay**로 이동합니다.
