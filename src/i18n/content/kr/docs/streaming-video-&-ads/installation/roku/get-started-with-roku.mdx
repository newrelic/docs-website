...
<Steps>
  ...
  <Step>
    **에이전트 초기화**: 기본 BrightScript 파일을 엽니다(예: `Main.brs`). `ACCOUNT ID`, <InlinePopover type="licenseKey" />, `APP NAME`, `APPLICATION TOKEN` 및 `ENDPOINT` 으로 `NewRelic` 함수를 호출하여 뉴렐릭 에이전트를 초기화합니다. \*\*환경에 따라 다른 엔드포인트를 사용하도록 에이전트를 구성하려면 main.brs 파일을 수정해야 합니다. 기본 설정값은 US이며, 사용하고자 하는 환경에 따라 EU 또는 스테이징으로 변경할 수 있습니다.
