...
## 로그 데이터 액세스 보안 [#securing-log-data]

민감한 정보가 포함될 수 있는 로그 데이터를 사용하는 경우, 특정 로그에 누가 액세스할 수 있는지 제어하는 것이 중요합니다. 로그 데이터를 보호하기 위한 모범 사례는 다음과 같습니다.

* **데이터 액세스 제어 사용**: [데이터 액세스 제어](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control) 정책을 구현하여 특정 [로그 파티션을](/docs/logs/ui-data/data-partitions/) 볼 수 있는 사용자를 제한합니다. 특히 로그에 개인 식별 정보(PII), 보안 데이터 또는 비즈니스에 민감한 정보가 포함되어 있는 경우 이 점이 매우 중요합니다.

* **데이터 노출 위험에 주의하세요**. 특정 기능은 데이터 접근 제한을 우회할 수 있습니다. [잠재적인 데이터 노출 위험을](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control#data-exposure-risks) 검토하고 다음 모범 사례를 따르세요.

  * <DNT>**Data partitions rules**</DNT> 기능에 대한 <DNT>**Modify**</DNT> 액세스 권한이 있는 사람을 제한합니다. 이를 통해 사용자는 파티션을 재구성하고 제한된 데이터에 액세스할 수 있습니다.
  * 쿼리 키는 사용자 수준 데이터 액세스 제한을 우회할 수 있으므로 신뢰할 수 있는 관리자에게만 <DNT>**Query keys**</DNT> 기능에 대한 <DNT>**Modify**</DNT> 액세스를 제한합니다.
  * 데이터 액세스 정책을 준수하지 않는 Insights API 키를 회전하거나 삭제합니다.
  * 공지 조건 또는 공지를 통해 민감한 데이터에 액세스해서는 안 되는 사용자를 위해 알림 관리 기능 없이 사용자 정의 역할을 만듭니다.

* **액세스 제어를 통해 파티션 정렬**: 파티션 전략을 계획할 때 어떤 팀이 어떤 로그에 액세스해야 하는지 고려하세요. 팀, 서비스 또는 민감도 수준별로 로그를 그룹화하여 액세스 제어 관리를 보다 쉽게 만듭니다.
