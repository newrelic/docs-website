...
* 로그를 전달할 때 가능하면 항상 당사의 [New Relic 인프라 에이전트](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent) 및/또는 [APM 에이전트](/docs/apm/new-relic-apm/getting-started/get-started-logs-context#agents) 를 사용하십시오. New Relic 에이전트를 사용할 수 없는 경우 지원되는 다른 에이전트(FluentBit, Fluentd, Logstash 등)를 사용하십시오.
...
<Callout variant="warning">
  로그가 기본 호스트/컨테이너의 디렉터리에 저장되고 로그를 수집하기 위해 인프라 에이전트에 의해 계측되는 경우 인프라 에이전트와 APM 에이전트 모두에서 수집된 중복 로그가 표시될 수 있습니다. APM 로그를 사용하도록 선택할 수 있습니다. 이 경우 인프라 에이전트 구성을 편집하여 로그 수집을 중지하거나 그 반대로 해야 합니다.
</Callout>
...
* 다음과 같은 다른 일반적인 데이터 유형에 대한 로그를 전달하려면 New Relic 통합을 사용하십시오.
...
* 추출할 속성 값의 유형을 지정하려면 Grok 유형을 사용하십시오. 생략하면 값이 문자열로 추출됩니다. 이러한 속성에서 NRQL 기능(즉, `monthOf()` , `max()` , `avg()` , & `lt;` , `&gt;` 등)을 사용할 수 있기를 원하는 경우 숫자 값에 특히 중요합니다.
...
* **Query Builder** 를 사용하여 NRQL을 사용하여 검색을 실행합니다. **쿼리 빌더** 를 사용하면 NRQL에서 사용할 수 있는 모든 고급 기능을 사용할 수 있습니다.
* 대시보드를 생성하거나 사용 가능한 **Quick Start** 대시보드를 사용하여 로그에 대한 일반적인 질문에 답하고 시계열 그래프에서 시간 경과에 따른 로그 데이터를 살펴보십시오. 여러 패널이 있는 대시보드를 생성하여 다양한 방식으로 로그 데이터를 분할합니다.
...
* **로그 분석** 및/또는 **APM 로그 모니터링 빠른 시작** 대시보드를 설치하여 로그 데이터에 대한 더 많은 통찰력을 빠르게 얻으십시오. 이러한 대시보드를 추가하려면 **one.newrelic.com > Add Data > Logging > Dashboards** 보드로 이동하십시오.
