...
<Steps>
  ...
  <Step>
    ...
    라이브 아카이브를 쿼리하는 방법에는 Lucene 또는 NRQL 두 가지 방법이 있습니다. 뉴렐릭 외부에서 익숙한 로그 구문을 사용하려면 <DoNotTranslate>**Logs**</DoNotTranslate> UI에서 쿼리하는 것이 좋습니다. 라이브 아카이브를 쿼리하려면 **Live archives query** \[라이브 아카이브 쿼리] 기능이 필요합니다.
  ...
  <Step>
    ...
    에러스 인박스(errors inbox)나 알림에 사용하는 것과 동일한 언어를 사용하여 쿼리하려면 NRQL에서 쿼리할 것을 권합니다. 라이브 아카이브를 쿼리하려면 **Live archives query** \[라이브 아카이브 쿼리] 기능이 필요합니다.
