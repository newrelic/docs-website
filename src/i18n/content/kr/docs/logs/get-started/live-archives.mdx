...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="tip">
      위의 단계는 표준 보존에 더해 라이브 아카이브를 추가하므로, 로그가 먼저 표준 스토리지에 저장됩니다. 컴플라이언스 또는 감사 로그와 같이 표준 스토리지에 필요하지 않은 로그가 있는 경우, 파티션의 네임스페이스로 <DNT>**Standard**</DNT> 대신 <DNT>**Direct to live archive**</DNT>를 선택합니다. 해당 파티션으로 전송된 로그는 표준 스토리지를 건너뛰고 라이브 아카이브로 바로 이동합니다. 보다 자세한 내용은 [네임스페이스 선택](/docs/logs/ui-data/data-partitions/#namespace)을 확인하십시오.
    </Callout>
  </Step>
