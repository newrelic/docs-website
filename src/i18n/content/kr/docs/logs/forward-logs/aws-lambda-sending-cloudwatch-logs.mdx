...
CloudWatch 로그를 뉴렐릭으로 전달하면 로그 데이터를 수집, 처리, 탐색, 쿼리 및 알림 설정할 수 있는 <InlinePopover type="logs"/>기능을 향상할 수 있습니다.
...
3. **Triggers**에서 **Add Triggers**를 클릭하고 드롭다운에서 **CloudWatch Logs**를 선택합니다.
