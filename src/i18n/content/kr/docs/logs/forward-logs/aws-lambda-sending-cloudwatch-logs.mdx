...
CloudWatch 로그를 뉴렐릭으로 전달하면 로그 데이터에 대한 수집, 처리, 탐색, 쿼리 및 알림을 위한 <InlinePopover type="logs"/>기능을 향상할 수 있습니다.
...
3. `newrelic`을 검색하고 <DoNotTranslate>**Show apps that create custom IAM roles or resource policies**</DoNotTranslate>를 확인하여 `newrelic-log-ingestion`을 찾습니다.

4. `newrelic-log-ingestion` 세부정보를 열고 <DoNotTranslate>**Deploy**</DoNotTranslate>를 클릭합니다.

5. 함수의 <DoNotTranslate>**Configure**</DoNotTranslate> 메뉴에서 <DoNotTranslate>**Environment Variables**</DoNotTranslate>로 이동하고 다음 환경 변수를 사용하여 로그 포워딩을 구성합니다.
...
6. 앱이 커스텀 IAM 역할을 생성한다는 것을 확인한 후, <DoNotTranslate>**Deploy**</DoNotTranslate>를 클릭합니다.
...
5. 적절한 엔드포인트로 `NR_LOGGING_ENDPOINT`를 업데이트합니다.
   ...
   * US 고객의 경우: `https://log-api.newrelic.com/log/v1`
   * EU 고객의 경우: `https://log-api.eu.newrelic.com/log/v1`
...
예를 들어 위의 기본 설정에서 첫 번째 재시도는 1초 후에 발생하고, 두 번째 재시도는 2초 후에, 세 번째 재시도는 4초 후에 발생합니다.
