...
7. New Relic의 **Logs Endpoint** 를 입력합니다. `<NR_LICENSE_KEY>` 을 <InlinePopover type="licenseKey" /> 십시오.
   ...
   * 둘 이상의 Cloudflare 영역이 있으면 각 영역에 대해 새
...
* `destination_conf`: 엔드포인트 URL, <InlinePopover type="licenseKey" /> 및 다음 문자열 형식의 형식으로 구성된 로그 대상입니다.
  ...
  * `<NR_LICENSE_KEY>`: [UI 또는 API에서 검색](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key) 할 수 있는 New Relic
