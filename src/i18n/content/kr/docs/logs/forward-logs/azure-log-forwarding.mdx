...
Azure 로그를 New Relic으로 전달하면 향상된 기능을 얻을 수 있습니다. <InlinePopover type="logs"/>로그 데이터를 수집, 처리, 탐색, 쿼리 및 경고하는 기능입니다.
...
2. **[one.newrelic.com](https://one.newrelic.com/launcher/logger.log-launcher)** 에서, 왼쪽 탐색 메뉴에서 **Add data** \[데이터 추가를] 클릭합니다.
...
2. **[로그 UI](https://one.newrelic.com/launcher/logger.log-launcher)** 로 이동하여 왼쪽에서 **Add data** \[데이터 추가를] 클릭합니다.
...
<CollapserGroup>
  ...
  >
    ...
    3. **Allow Blob public access** \[Blob 공개 액세스 허용] 아래에서 **Disabled** \[비활성화됨을] 선택합니다.<img title="Disabling public access to the blobs/containers in the Storage account" alt="Disabling public access to the blobs/containers in the Storage account" src={disablePublicBlobAccessStorageAccount}/>
    4. **저장** 을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    4. **Platform settings** \[플랫폼 설정] 에서 **FTP state** \[FTP 상태] 에 대해 **Disabled** \[비활성화됨을] 선택합니다.<img title="Disabling FTP access to the Function App" alt="Disabling FTP access to the Function App" src={disableFtpAccessStorageAccount}/>
    5. **저장** 을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    2. 왼쪽 메뉴에서 **Settings > Configuration**으로 이동합니다.
    3. **General settings** \[일반 설정] 탭을 선택합니다.
    4. **Platform settings** \[플랫폼 설정] 에서 **HTTPS Only** \[HTTPS 전용] 에 대해 **On** \[켜짐 을] 선택합니다.<img title="Enforcing HTTPS to the Function App" alt="Enforcing HTTPS to the Function App" src={enforceHttpsFunctionApp}/>
    5. **저장** 을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    3. **Inbound Traffic** \[인바운드 트래픽] 상자 내에서 **Access restriction** \[액세스 제한을] 클릭합니다.<img title="Disabling public network access to the Function App" alt="Disabling public network access to the Function App" src={disablePublicNetworkAccessFunctionApp}/>
