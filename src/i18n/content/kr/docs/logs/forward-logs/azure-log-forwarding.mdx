...
3. **Logging** \[로깅] 아래에서 `Microsoft Azure Event Hub` 타일을 클릭합니다.
...
<CollapserGroup>
  ...
  >
    ...
    3. **Allow Blob public access** \[Blob 공개 액세스 허용] 에서 **Disabled** \[비활성화]를 선택합니다.<img title="Disabling public access to the blobs/containers in the Storage account" alt="Disabling public access to the blobs/containers in the Storage account" src={disablePublicBlobAccessStorageAccount}/>
    4. **저장** 을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    4. **Platform settings** \[설정] 에서 **FTP state** \[FTP 상태] 에 대해 **Disabled** \[비활성화]를 선택합니다.<img title="Disabling FTP access to the Function App" alt="Disabling FTP access to the Function App" src={disableFtpAccessStorageAccount}/>
    5. **저장** 을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    3. **Inbound Traffic** \[인바운드 트래픽] 상자 내에서 **Access restriction** \[액세스 제한]을 클릭합니다.<img title="Disabling public network access to the Function App" alt="Disabling public network access to the Function App" src={disablePublicNetworkAccessFunctionApp}/>
