...
3. **Log ingestion** 에서 `Microsoft Azure Event Hub` 타일을 클릭합니다.

![Microsoft Azure Event Hub 데이터 원본 단추](./images/azure-eventhub-tile.png)
...
9. [New Relic One 엔드포인트](/docs/logs/log-api/introduction-log-api/#endpoint) 가 귀하의 계정에 해당하는 엔드포인트로 설정되어 있는지 확인하십시오.
...
기본적으로 이 템플릿은 Event Hub에서 New Relic One으로 로그를 전달하는 데 필요한 기능과 리소스만 구성합니다. 구독 활동 로그가 전달되도록 구성할 수도 있지만 Azure 리소스에서 전달되는 기본 로그는 없습니다. 로그를 생성하는 리소스에서 로그를 전달하려면 지정된 리소스에 대한 진단 설정을 만들어 구성해야 합니다.

예를 들어 Azure에서 실행 중인 기능이 있고 로그를 New Relic One으로 전달하려는 경우 로그를 Event Hub로 전달하도록 진단 설정을 구성해야 합니다. 자세한 내용은 [플랫폼 로그 및 메트릭을 다른 대상으로 보내기 위한 진단 설정을 만드는 Microsoft 설명서를](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/diagnostic-settings?tabs=CMD) 참조하십시오.
...
![Kubernetes 서비스 버튼](./images/azure-eventhub-k8s-service.png)

3. 왼쪽 메뉴에서 **모니터링 `>` 진단 설정** 을 선택합니다.

![진단 설정 메뉴](./images/azure-eventhub-diagnosticsettings.png)

4. **진단 설정 추가를** 클릭합니다.

![진단 설정 추가 버튼](./images/azure-eventhub-add-diagnostic-setting.png)

5. 새 설정에 의미 있는 이름을 지정합니다.

![진단 설정 이름](./images/azure-eventhub-diagnosticsetting-name.png)

6. 수집하려는 Kubernetes(제어판) 로그를 선택합니다.

![쿠버네티스 로그](./images/azure-eventhub-diagnostic-logs.png)

7. **대상 세부 정보** 에서 **이벤트 허브로 스트리밍을** 선택하고 `Event hub namespace` , `Event hub name` 및 `Event hub policy name` 을 구성합니다. [ARM 템플릿 배포](#eventhub-arm-setup) 중에 새 Event Hub 및 네임스페이스를 만들기로 선택한 경우 자동으로 생성된 다음 설정을 선택합니다(네임스페이스 이름의 접미사는 다름).

![목적지 세부정보 설정](./images/azure-eventhub-destination-details-settings.png)

8. **저장** 을 클릭하여 Kubernetes 로그를 New Relic에 전달하기 시작합니다.

![진단 설정 저장 버튼](./images/azure-eventhub-save-button.png)
...
<Callout variant="important">
  새로운 Relic Blob Storage ARM 템플릿은 지정된 컨테이너 내의 모든 Blob 파일 **을 현재 상태로** 전달하는 함수를 배포합니다. 이후에 이러한 Blob이 수정되면 파일의 **전체** 내용이 다시 전송됩니다.

  이 솔루션은 정적 blob 파일을 New Relic으로 전달하는 것을 목표로 하며 파일 테일링을 지원하지 않습니다. 로그 스트림을 전달해야 하는 경우 응용 프로그램 로그를 `Event Hub` 로 보내고 대신 [Event Hub 기반 템플릿](#azure-eventhub) 을 사용하는 것이 좋습니다.
</Callout>
...
1. [New Relic 라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 가 있는지 확인하십시오.
2. [New Relic Logs](https://one.newrelic.com/launcher/logger.log-launcher) 에 로그인하고 페이지 오른쪽 상단의 **데이터 소스 추가** 를 클릭합니다.
3. **로그 수집** 에서 `Microsoft Azure Blob Storage` 타일을 클릭합니다.

![Microsoft Azure Blob Storage 데이터 원본 단추](./images/azure-blobstorage-tile.png)
...
10. [New Relic One 엔드포인트](/docs/logs/log-api/introduction-log-api/#endpoint) 가 귀하의 계정에 해당하는 엔드포인트로 설정되어 있는지 확인하십시오.
11. **검토 + 만들기** 를 클릭하고 삽입한 데이터를 검토한 **다음 만들기** 를 클릭합니다.
