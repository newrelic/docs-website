...
<Callout variant="tip">
  Azure VNet Flow 로그를 포워딩하시겠습니까? 증분 델타 처리를 통한 전용 VNet flow 로그 포워딩에 대해서는 [Azure VNet Flow 로그 모니터링 설정](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) 을 참조하세요. 자동화된 설정을 위해 ARM 및 Bicep 배포 템플릿을 사용할 수 있습니다.
</Callout>
...
11. 선택 사항: <DNT>**Authentication Mode**</DNT>을 선택합니다. 기본값은 `Local Authentication`입니다. 보안 규정 준수를 위해 Azure 환경에서 로컬 인증(공유 액세스 키)을 비활성화한 경우 `Managed Identity` 을(를) 선택하세요. `Managed Identity`(으)로 설정하면 Function App에 시스템 할당 관리 ID가 할당되고 이벤트 허브 네임스페이스에 대한 `Azure Event Hubs Data Receiver` 역할이 부여됩니다 ― 연결 문자열은 저장되지 않습니다. 자세한 내용은 [인증 모드 정보](#eventhub-authentication) 를 참조하십시오.

12. 선택 사항: EventHub 일괄 처리를 구성하여 성능을 최적화합니다(v2.8.0 이상에서 사용 가능):
...
13. 선택 사항: 전달할 [Azure 구독 활동 로그](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) 를 `true` 로 설정합니다. 자세한 내용은 이 문서 [의 구독 정보](#subscription-activity-logs) 를 참조하세요.

14. <DNT>**Review + create**</DNT> 클릭하고 삽입한 데이터를 검토한 후 <DNT>**Create**</DNT> 클릭합니다.
...
### 선택 사항: 인증 모드 구성 [#eventhub-authentication]

버전 3.2.0부터 ARM 템플릿은 Function App을 이벤트 허브에 연결하기 위한 두 가지 인증 방법을 지원합니다:

* <DNT>**Local Authentication**</DNT> (기본값): Function App은 애플리케이션 설정에 저장된 공유 액세스 키 연결 문자열을 사용하여 연결합니다. 이는 전통적인 접근 방식이며 이전 버전의 템플릿과 일치합니다.

* <DNT>**Managed Identity**</DNT>: Function App은 이벤트 허브 네임스페이스에서 `Azure Event Hubs Data Receiver` 기본 제공 역할을 가진 시스템 할당 관리 ID를 사용합니다. 시크릿이나 연결 문자열은 저장되지 않습니다. 다음 경우에 이 옵션을 사용하십시오:

  * 귀하의 조직은 보안 규정 준수를 위해 이벤트 허브 네임스페이스에서 로컬 인증(공유 액세스 키)을 비활성화했습니다.
  * Azure AD에서 전적으로 관리하는 키 없는 인증을 선호합니다.

`Managed Identity` 이(가) 선택되면 템플릿이 자동으로 다음을 수행합니다:

* Function App에 시스템 할당 관리 ID를 생성합니다.
* 이벤트 허브 네임스페이스에서 ID에 `Azure Event Hubs Data Receiver` 역할을 부여합니다.
* 연결 문자열 대신 `EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace` (이벤트 허브 네임스페이스 호스트명)으로 Function App을 구성합니다. Azure Functions 런타임은 인증을 위해 Azure AD 토큰을 자동으로 사용합니다.
