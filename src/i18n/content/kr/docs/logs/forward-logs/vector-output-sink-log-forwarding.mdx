...
## Vector New Relic 로그 싱크 구성 [#configure-sink]
...
3. `vector.toml` 파일(기본적으로 `/etc/vector` 에 있음)에 스니펫을 추가하고 <var>YOUR_LICENSE_KEY</var> 를 New Relic 라이선스 키로 대체합니다.
...
## Vector New Relic 로그 싱크 테스트 [#test-sink]

New Relic 로그 싱크가 이벤트를 전달하는지 테스트하려면:
...
2. 테스트 메시지에 대한 [New Relic Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) 를 검색합니다.
