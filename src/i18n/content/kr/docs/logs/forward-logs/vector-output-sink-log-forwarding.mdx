...
벡터 로그를 뉴렐릭으로 전달하면 로그 데이터에 대한 수집, 처리, 탐색, 쿼리 및 공지를 위한 향상된 <InlinePopover type="logs"/>기능이 제공됩니다.
...
3. `vector.toml` 파일(기본적으로 `/etc/vector` 에 위치)에 스니펫을 추가하고 `YOUR_LICENSE_KEY` 뉴렐릭 <InlinePopover type="licenseKey"/>으로 바꿉니다.
