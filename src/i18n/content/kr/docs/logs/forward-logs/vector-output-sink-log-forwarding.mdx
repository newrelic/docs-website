...
Vector 로그를 New Relic으로 전달하면 향상된 기능을 얻을 수 있습니다. <InlinePopover type="logs"/>로그 데이터를 수집, 처리, 탐색, 쿼리 및 경고하는 기능입니다.
...
3. `vector.toml` 파일(기본적으로 `/etc/vector` 에 위치)에 스니펫을 추가하여 `YOUR_LICENSE_KEY` New Relic으로 바꿉니다. <InlinePopover type="licenseKey"/>:
