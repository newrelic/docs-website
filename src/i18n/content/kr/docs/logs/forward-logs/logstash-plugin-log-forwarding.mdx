...
2. `logstash.conf` 파일에 다음 데이터 블록을 추가하십시오. 자리 표시자 텍스트를 New Relic 라이센스 키로 교체해야 합니다.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * 미국 엔드포인트: `https://log-api.newrelic.com/log/v1`
        * EU 엔드포인트: `https://log-api.eu.newrelic.com/log/v1`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        메시지 전송을 재시도하는 최대 횟수입니다. `0` 으로 설정하면 재시도하지 않습니다. 기본값: `3`
      </td>
    </tr>
  </tbody>
</table>
...
1. `logstash.conf` 파일에 다음을 추가하십시오.
...
4. `test message` 에 대한 [New Relic Logs UI](https://one.newrelic.com/launcher/logger.log-launcher) 를 검색합니다.
