...
Logstash 로그를 뉴렐릭으로 전달하면 로그 데이터에 대한 수집, 처리, 탐색, 쿼리 및 공지를 위한 향상된 <InlinePopover type="logs"/>기능이 제공됩니다.
...
2. `logstash.conf` 파일에 다음 데이터 블록을 추가합니다. 플레이스홀더 텍스트를 뉴렐릭 <InlinePopover type="licenseKey"/>으로 바꾸세요.
