...
6. <DNT>**HTTP endpoint URL**</DNT>에서 <DNT>**New Relic logs - US**</DNT>을(를) 선택합니다. 이것은 미국 엔드포인트(기본값)입니다([https://aws-api.newrelic.com/firehose/v1](https://aws-api.newrelic.com/firehose/v1)). EU 엔드포인트를 사용하려면 이 절차의 나머지 단계를 완료한 다음 [EU 계정 설정](#configure-eu-stream)으로 이동하십시오.
...
## EU 또는 JP 계정으로 로그를 보내도록 스트림 구성 [#configure-eu-stream]

EU 또는 JP 데이터센터에 뉴렐릭 계정이 있는 경우:
...
5. <DNT>**HTTP endpoint URL**</DNT> 의 값을 해당 지역에 적합한 Firehose 엔드포인트로 업데이트합니다:
