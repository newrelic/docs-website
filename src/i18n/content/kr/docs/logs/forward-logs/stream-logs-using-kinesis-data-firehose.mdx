...
CloudWatch Logs 또는 Kinesis 스트림과 호환되는 기타 로그를 뉴렐릭으로 전달하면 로그 데이터를 수집, 처리, 탐색, 쿼리 및 알림 설정을 하기 위한 <InlinePopover type="logs"/>기능을 확보할 수 있습니다.

## New Relic의 전송 스트림 생성 [#create-delivery-stream]

Kinesis Data Firehose에서 New Relic으로 로그를 전달하려면:

1. 당신이 가지고 있는지 확인 <InlinePopover type="licenseKey"/>.
...
6. **HTTP 끝점 URL** 에서 **New Relic logs-US** 를 선택합니다. 이것은 미국 엔드포인트( [https://aws-api.newrelic.com/firehose/v1](https://aws-api.newrelic.com/firehose/v1) )입니다. EU 엔드포인트를 사용하려면 이 절차의 나머지 단계를 완료한 다음 [EU 계정 구성](#configure-eu-stream) 으로 이동합니다.
7. <InlinePopover type="licenseKey"/>를 **API key** \[API 키] 필드에 붙여 넣습니다.
