...
로그 전달을 위한 New Relic의 Kubernetes 플러그인은 클러스터에서 New Relic 로그로 로그를 보내는 것을 단순화합니다. 독립 실행형 Docker 이미지를 사용하고 DaemonSet으로 실행되어 중앙 집중식 분석 및 문제 해결을 위해 로그를 원활하게 수집합니다. Kubernetes 로그를 New Relic으로 전달하면 로그 데이터를 수집, 처리, 탐색, 쿼리 및 경고할 수 있는 향상된 로그 관리 기능이 제공됩니다.
