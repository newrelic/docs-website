...
2. [선택적으로 newrelic-logging 리포지토리](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration) 에서 사용할 수 있는 다양한 구성 옵션을 사용하여 안내 설치의 [3단계에서 설치를 추가로 조정할 수 있습니다.](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#kubernetes-install-navigation) 그러나 대부분의 사용자에게 유효한 표준 설정을 권장합니다.
   ...
   <Callout variant="important">
     [Kubernetes 보안 비밀을 사용하여](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-logging/values.yaml#L8-L25) 뉴렐릭 라이선스 키를 저장하는 경우 `newrelic-logging` 차트는 기본적으로 로그를 US API 엔드포인트로 전송합니다. 인스턴스 키가 EU 또는 Fedramp 계정에 속하고 비밀이 키 저장에 사용되는 경우 [API 참조 문서](https://docs.newrelic.com/docs/logs/log-api/introduction-log-api/#endpoint) 의 적절한 값으로 엔드포인트 설정을 업데이트해야 합니다. EU 계정에 대해 이를 설정하는 방법의 예는 다음과 같습니다.
