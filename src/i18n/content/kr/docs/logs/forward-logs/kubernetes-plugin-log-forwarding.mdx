...
2. 선택적으로 클립 에서 [`newrelic-logging` 사용할](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration) 수 있는 다양한 설정 옵션을 사용하여 [안내 설치 에서 설치를 더욱 세부적으로](/install/kubernetes) 조정할 수 있습니다. 하지만 대부분 사용자에게 유효하므로 표준 설정을 권장합니다.
   ...
   <Callout variant="important">
     뉴렐릭 <InlinePopover type="licenseKey" />을(를) 저장하기 위해 [Kubernetes 시크릿을 사용하는](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-logging/values.yaml#L8-L25) 경우, `newrelic-logging` 차트는 기본적으로 미국 API 엔드포인트로 로그를 전송합니다. 라이선스 키가 EU, 일본 또는 FedRAMP 계정에 속하고 키 저장소에 시크릿이 사용되는 경우, [API 참조 문서](/docs/logs/log-api/introduction-log-api/#endpoint)의 적절한 값으로 엔드포인트 설정을 업데이트해야 합니다. EU 및 일본 계정에 대해 이를 설정하는 방법의 예는 다음과 같습니다:
...
<DocTiles>
  <DocTile title="로그 UI" path="/docs/logs/ui-data/use-logs-ui/">
    로그 UI를 사용하여 플랫폼 전반의 로깅 데이터 탐색
  </DocTile>

  <DocTile title="컨텍스트의 로그인" path="/docs/logs/logs-context/logs-in-context/">
    컨텍스트 기능의 로그를 통해 로그를 전달하여 애플리케이션과 플랫폼 성능 데이터에 대한 더 깊은 가시성을 확보하세요.
  </DocTile>

  <DocTile title="경고" path="/docs/alerts/overview/">
    가장 중요한 문제에 대한 정보를 받기 위해 알림을 만드세요.
  </DocTile>

  <DocTile title="대시보드 만들기" path="/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/">
