...
import gcpAddMoreData from 'images/gcp-add-more-data.png'

import gcpCreateTopic from 'images/gcp-create-topic.png'

import gcpCreateSubscription from 'images/gcp-create-subscription.png'

import gcpCreateSubscriptionType from 'images/gcp-create-subscription-type.png'

import gcpCreateSink from 'images/gcp-create-sink.png'
...
<CollapserGroup>
  ...
  >
    먼저 GCP Pub/Sub 주제에 대한 수집 URL을 생성합니다.

    <img
      title="더 많은 데이터 소스 추가"
      alt="더 많은 데이터 소스 추가"
      src={gcpAddMoreData}
    />

    1. [**Logs** in New Relic One](https://one.newrelic.com/launcher/logger.log-launcher) 으로 이동하고 **데이터 소스 추가** 를 클릭합니다.

    2. **Google Cloud Platform** 을 클릭한 다음 로그를 전달할 New Relic 계정을 선택하고 **계속** 을 클릭합니다.
  ...
  >
    ...
    <img
      title="GCP Pub/Sub 주제 구독 만들기"
      alt="GCP Pub/Sub 주제 구독 만들기"
      src={gcpCreateSubscription}
    />
    ...
    3. 아래로 스크롤하여 **구독** 탭을 선택한 다음 **구독 만들기** 를 클릭 **하고 간단한 구독 만들기** 를 선택합니다.
       ...
       <img
         title="GCP Pub/Sub 주제 구독 유형 선택"
         alt="GCP Pub/Sub 주제 구독 유형 선택"
         src={gcpCreateSubscriptionType}
       />
  ...
  >
    ...
    <img
      title="GCP Pub/Sub 로그 라우터 싱크 만들기"
      alt="GCP Pub/Sub 로그 라우터 싱크 만들기"
      src={gcpCreateSink}
    />
...
<CollapserGroup>
  ...
  >
    ...
    4. 후속 명령에서 사용되므로 `GOOGLE_APPLICATION_CREDENTIALS` 환경 변수를 사용하여 서비스 계정 키 파일을 참조하세요.
    ...
    * 로그를 보내는 데 사용되는 New Relic 라이센스 키
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="유럽 지역 계정"
      >
        유럽 계정은 `https://log-api.eu.newrelic.com/log/v1` 을 값으로 사용하여 `logsApiUrl` 매개변수도 추가해야 합니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            전체가 아닌 일괄 처리에서 마지막 로그 레코드를 수신한 후 New Relic으로 플러시하기 전에 추가 로그(최대 `batchCount` )를 기다리는 시간(초)입니다.
        ...
        <tr>
          ...
          <td>
            New Relic Logs API로 전송된 페이로드를 압축(GZIP으로)합니다.
