...
<CollapserGroup>
  <Collapser id="ingest-url" title="1. GCP Pub/Sub 수집 URL을 생성합니다.">
  ...
  <Collapser id="create-gcp-topic" title="2. GCP Pub/Sub 주제를 생성합니다.">
    ...
    2. <DNT>**Create Topic**</DNT> 을(를) 클릭합니다.
    3. 의미 있는 <DNT>**Topic ID**</DNT> 입력한 다음 필요에 따라 다른 옵션을 구성합니다.
    4. <DNT>**Create Topic**</DNT> 을(를) 클릭합니다.
  </Collapser>

  <Collapser id="configure-gcp-log-forwarding" title="3. 로그를 New Relic에 전달할 GCP Pub/Sub Topic을 준비합니다.">
  ...
  <Collapser id="forward-logs" title="4. 로그를 New Relic으로 전달할 라우팅 싱크를 생성합니다.">
    ...
    2. <DNT>**Create Sink**</DNT> 을(를) 클릭합니다.
    3. <DNT>**Sink name**</DNT> 및 <DNT>**Sink description**</DNT> 을 입력한 다음 <DNT>**Next**</DNT> 클릭합니다.
    4. <DNT>**Select sink service**</DNT> 아래에서 <DNT>**Cloud Pub/Sub topic**</DNT> 선택하고 이전에 생성한 [Pub/Sub 주제를](#create-gcp-topic) 선택합니다.
    5. 필요에 따라 나머지 필터를 구성한 다음 <DNT>**Create sink**</DNT> 클릭하여 설정을 완료합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="dataflow-login-gcp" title="1. Google Cloud Platform 계정에 로그인합니다.">
  ...
  <Collapser id="dataflow-repo" title="2. DataflowTemplates GitHub 리포지토리를 복제합니다.">
  ...
  <Collapser id="dataflow-configuration" title="3. Dataflow 전달자를 컴파일하고 실행합니다.">
  ...
  <Collapser id="dataflow-bucket" title="4. Dataflow 템플릿용 GCP 버킷을 생성합니다.">
  ...
  <Collapser id="dataflow-account" title="5. 서비스 계정을 생성합니다.">
    다음 명령을 실행합니다.
  ...
  <Collapser id="dataflow-compile" title="6. PubsubToNewRelic 템플릿을 컴파일하고 게시합니다.">
    다음 명령을 실행합니다.
  ...
  <Collapser id="dataflow-job" title="7. 템플릿을 Dataflow 작업으로 실행합니다.">
    ...
    * 로그를 보내는 데 사용된 뉴렐릭<InlinePopover type="licenseKey" />

    기본적으로 이 명령은 미국 데이터센터로 로그를 전송합니다. EU 또는 일본 계정의 경우 아래 섹션을 참조하십시오.

    <CollapserGroup>
      <Collapser id="eu-customers-gcp-dataflow" title="유럽 및 일본 리전 계정">
        유럽 및 일본 계정은 적절한 엔드포인트와 함께 `logsApiUrl` 파라미터도 추가해야 합니다.
        ...
        gcloud dataflow jobs run ${JOB_NAME} \
            --gcs-location=gs://${BUCKET_NAME}/template/PubsubToNewRelic \
            --region=${REGION} \
            --parameters "inputSubscription=projects/${PROJECT_ID}/subscriptions/${INPUT_SUBSCRIPTION_NAME},licenseKey=${NR_LICENSE_KEY},logsApiUrl=https://log-api.jp.nr-data.net/log/v1"
        ```
      </Collapser>
    </CollapserGroup>
  ...
  <Collapser id="dataflow-config" title="8. 선택 사항: Dataflow 로그 전달자 작업을 조정합니다.">
