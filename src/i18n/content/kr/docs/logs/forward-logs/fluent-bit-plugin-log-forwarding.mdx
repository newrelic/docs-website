...
1. plugins 디렉토리에서 `plugins.conf` 파일을 찾거나 생성하십시오.
...
인프라 호스트와 같은 [엔터티](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic/) 와 로그 줄을 연결하려면 `FILTER` 블록을 추가합니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        [New Relic 라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) . licenseKey( **권장** ) 또는 `licenseKey` 둘 다 사용하지 `apiKey` . 기본값: `none`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        페이로드가 보낸 최대 크기(바이트)입니다. 기본값: `256000`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        **더 이상 사용되지 않습니다.** New Relic [Insights 삽입 키](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register) 를 사용하지만 `licenseKey` 속성을 사용하는 것이 좋습니다. licenseKey 또는 `licenseKey` 둘 다 사용하지 `apiKey` .
      </td>
    </tr>
  </tbody>
</table>
