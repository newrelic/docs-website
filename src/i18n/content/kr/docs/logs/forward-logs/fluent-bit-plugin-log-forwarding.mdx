...
Fluent Bit는 뉴렐릭 플러그인의 위치와 뉴렐릭 <InlinePopover type="licenseKey" />를 알아야 뉴렐릭으로 데이터를 출력할 수 있습니다. Fluent Bit 플러그인을 구성하려면:
...
4. `fluent-bit.conf` 파일 하단에 다음을 추가하여 입력, 필터 및 출력 스탠자를 설정합니다. 자리 표시자 텍스트를 <InlinePopover type="licenseKey" />로 교체합니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <InlinePopover type="licenseKey" />입니다. `licenseKey` (**권장 **) 또는 `apiKey` 중 하나만 사용합니다. 기본값: `none`
      </td>
    </tr>
