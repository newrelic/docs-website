...
* [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key)
...
작업 정의에서 New Relic [라이선스 키가](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 노출되지 않도록 하려면 [AWS Secrets Manager](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data-secrets.html) 서비스를 사용하는 것이 좋습니다.

비밀을 추가할 때 **일반 텍스트** 탭을 사용하십시오. 비밀을 Secrets Manager에 추가한 후에는 `logConfiguration` 블록을 사용하여 참조할 수 있으며 SECRET_NAME을 AWS 비밀의 이름으로 바꿉니다. 예를 들어:
...
`logConfiguration` 블록을 사용하여 INSERT_API_KEY를 New Relic [라이센스 키로](/docs/apis/intro-apis/new-relic-api-keys/#license-key)바꿉니다. 구성 세부 정보 [는 GitHub의 FireLens 작업 정의를](https://github.com/aws-samples/amazon-ecs-firelens-examples) 참조하십시오.
