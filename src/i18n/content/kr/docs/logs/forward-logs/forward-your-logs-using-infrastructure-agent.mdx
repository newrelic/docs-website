...
로그를 New Relic에 전달하면 로그 데이터를 수집, 처리, 탐색, 쿼리 및 경고하는 향상된 로그 관리 기능이 제공됩니다. 앱 및 호스트 컨텍스트의 로그가 문제의 근본 원인을 찾는 데 어떻게 도움이 되는지 보려면 다음 짧은 비디오(약 3분 40초)를 시청하세요.
...
4. 인프라 에이전트의 `logging.d` 디렉토리에 `logging.yml` [구성 파일](#configure) 을 만듭니다.
...
2. 다음과 유사한 명령을 실행하여 New Relic의 [fluent-bit-package(RPM)](https://github.com/newrelic/fluent-bit-package/releases) 를 다운로드하고 설치합니다.

   ```shell
    yum localinstall td-agent-bit-<some-version>.rpm`
   ```

3. New Relic의 [fluentbit 플러그인](https://github.com/newrelic/newrelic-fluent-bit-output/releases) 을 다운로드하고 `/var/db/newrelic-infra/newrelic-integrations/logging/out_newrelic.so` 으로 저장합니다.
...
1. 로그 전달자 구성 폴더를 탐색하십시오.
   ...
   * 리눅스: `/etc/newrelic-infra/logging.d/`
   * Windows: `C:\Program Files\New Relic\newrelic-infra\logging.d\`

2. `logging.yml` 구성 파일을 만들고 필요한 매개변수를 추가합니다. `logging.d` 디렉토리에는 참조 또는 시작점으로 사용할 수 있는 다양한 `.yml.example` 파일이 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `channel:` 채널 로그의 이름이 수집됩니다. 맞춤 채널에서는 작동하지 않습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      `attributes` 구성 매개변수는 외부 Fluent Bit 구성(예: `fluentbit` 구성 매개변수 사용)을 통해 전달된 로그에 맞춤 속성을 추가하지 않습니다. 이 시나리오에서는 [Fluent Bit 문서](https://docs.fluentbit.io/manual/) 에서 `record_modifier` 옵션을 참조해야 합니다.
    </Callout>

    `attributes` 구성 매개변수의 일반적인 용도 중 하나는 `logtype` 속성을 지정하는 것입니다. 이 속성을 사용하면 New Relic의 로그 관리 기능이 지원하는 [내장 구문 분석 규칙](https://docs.newrelic.com/docs/logs/log-management/ui-data/parsing/#built-in-rules) 중 하나를 활용할 수 있습니다.
  ...
  >
    ...
    런타임 파일은 모든 기본 Fluent Bit 구성 값을 그대로 두고 [`[SERVICE]` 섹션](https://docs.fluentbit.io/manual/administration/configuring-fluent-bit/configuration-file#config_section) 을 정의하지 않습니다. 외부 Fluent Bit 구성 파일에 고유한 `[SERVICE]` 섹션을 정의하고 `fluentbit` 옵션을 통해 포함하여 Fluent Bit의 기본 설정을 무시할 수 있습니다.
...
다음은 YAML 형식의 `logging.d/` 구성 파일의 예입니다. 더 많은 구성 예 [는 인프라 에이전트 저장소 를 참조하십시오](https://github.com/newrelic/infrastructure-agent/tree/master/assets/examples/logging) .
...
* New Relic UI에서 선택한 호스트의 **요약** 페이지: **[one.newrelic.com](https://one.newrelic.com) > Explorer** 또는 **Infrastructure > 호스트 > (엔티티 선택) > 로그로** 이동합니다.
...
<CollapserGroup>
  ...
  >
    로그 전달 기능을 사용하려면 에이전트에 데이터 소스를 읽을 수 있는 권한이 있어야 합니다. 인프라 에이전트를 [특권 또는 비특권 모드](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes) 로 실행할 때 전달하려는 로그 파일(및 해당 경로의 모든 중간 디렉토리)을 `nri-agent` 을(를) 실행하는 사용자가 읽을 수 있는지 확인하십시오.
  ...
  >
    ...
    * Unixドメインソケットファイルを使用している場合は、`nri-agent`ユーザーがこれらのファイルにアクセスできること（前のセクションを参照）、および`nri-agent`以外のユーザーがそれらに書き込むことができるように読み取りおよび書き込み権限（`666`）を持っていることを確認してください。
  ...
  >
    ...
    * 시스템 풀로 프록시 인증서 가져오기(권장) MMC 도구를 사용하여 프록시 자체 서명 인증서(PEM 파일)를 가져옵니다. [이 링크](https://www.ssls.com/knowledgebase/how-to-import-intermediate-and-root-certificates-via-mmc/) 를 참조하고 **2단계** 에서 `Intermediate Certification Authorities` 대신 `Trusted Root Certification Authorities` 에서 가져오도록 하십시오.
  ...
  >
    ...
    2. [`verbose: 3`](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#verbose) 을(를) 추가하여 문제 해결 모드에서 에이전트 로깅을 활성화합니다.
...
로그 전달 기능을 비활성화하려면 `logging.d` 디렉토리로 이동하여 [구성](#configure) 프로세스 중에 원래 추가된 확장명이 `.yml` 인 파일을 제거합니다.
