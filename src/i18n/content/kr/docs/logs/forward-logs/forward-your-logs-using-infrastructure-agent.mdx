...
[뉴렐릭에 로그를 보고하는](/docs/logs/get-started/get-started-log-management#integrate-logs) 한 가지 옵션은 인프라 모니터링 에이전트를 사용하는 것입니다. 이를 통해 모든 로깅 데이터를 한 위치에 모아 애플리케이션과 플랫폼의 성능 데이터에 대해 더 큰 가시성을 확보할 수 있습니다.
...
뉴렐릭 로그 관리에 사용할 수 있는 모든 옵션에 대한 개괄적인 설명은 [로그 소개](/docs/logs/get-started/get-started-log-management#integrate-logs)를 참조하십시오.

인프라 에이전트를 사용하여 로그를 보고하는 경우 두 가지 주요 경로를 사용할 수 있습니다.

* 인프라 에이전트, APM 에이전트 등을 설치하는 데 사용할 수 있는 보다 간단한 설치 경로인 [가이드 설치](/docs/infrastructure/host-integrations/installation/new-relic-guided-install-overview)를 사용할 수 있습니다. 가이드 설치는 소규모 조직이나 뉴렐릭을 테스트하려는 모든 조직에 적합합니다.
* 다른 경로는 일반적인 방식대로 인프라 에이전트를 수동으로 설치하는 것입니다. 이렇게 하면 가이드 설치보다 더 많은 작업이 필요하지만 더 많은 기능을 사용할 수 있습니다.

인프라 모니터링을 통해 로그 보고를 설정하는 단계는 다음과 같습니다.

1. 아직 계정이 없으시면, 지금 영구 무료 [계정을 신청](https://newrelic.com/signup)하십시오.
2. 로그 구성에 필요한 [시스템 요구 사항](#requirements)을 확인합니다.
3. [인프라 에이전트 버전 1.11.4 이상을 설치](#install) 했는지 확인합니다. (UI에서 안내 설치 프로세스를 사용하는 경우 다음 단계 건들은 건너뜁니다.)
...
7. 로그 UI에서 [로그 데이터를 둘러](#what-next)봅니다. 뉴렐릭에서 인프라 에이전트가 보고한 로그 데이터를 찾습니다.

다음은 호스트 UI에 대한 로그의 예입니다. 선택한 기간 동안 이벤트의 문맥적 로그를 볼 수 있으며, 강조 표시된 속성에 대한 자세한 데이터를 분석할 수 있습니다. 더 자세한 데이터를 검토하려면 쿼리를 실행하거나 **Open in logs**를 클릭합니다.
