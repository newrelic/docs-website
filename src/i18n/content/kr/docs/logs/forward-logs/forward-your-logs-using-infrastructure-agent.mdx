...
인프라 모니터링 에이전트를 통해 로그를 전달하려면 다음을 수행하십시오.

1. 아직 없는 경우 [새 리릭 계정을 작성하십시오](https://newrelic.com/signup). 무료입니다. 영원히요.
2. 로그 구성에 필요한 [시스템 요구사항](#requirements) 을 확인하십시오.
3. [인프라 에이전트가 설치되어 있는지](#install), 버전 1.11.4이상이 있는지 확인하십시오. (UI 에서 안내 설치를 사용하는 경우 다음 단계를 생략하십시오.)
4. 인프라 에이전트의 `logging.d` 디렉토리에 `logging.yml` [구성 파일](#configure) 을 작성하십시오.
5. 로그 소스 및 기타 매개변수를 [구성](#parameters) 하십시오.
6. 일부 트래픽을 생성하고 몇 분 동안 대기한 다음 데이터에 대해 [계정을 확인](#find-data) 하십시오.
7. 로그 UI에서 [로그 데이터를 탐색](#what-next) 하고 인프라 에이전트가 자동으로 삽입한 로그 속성을 활용하십시오.
...
![호스트에 대한 컨텍스트 로그인](./images/demotron-host-logs-123021.png "호스트에 대한 컨텍스트 로그인")

<figcaption>
  다음은 이벤트와 관련된 호스트 로그의 예제입니다.
</figcaption>

## 호스트 내 통합에 대한 로깅 활성화 [#on-host]
...
호스트 간 통합 로그 전달 기능을 사용하려면 다음을 수행하십시오.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="MySQL 로그"
  >
    `mysql-log.yml.example` 파일을 `mysql-log.yml` 에 복사(또는 이름 변경)하여 자동 MySQL 오류 로그 구문 분석을 활성화하고 New Relic으로 전달합니다. 에이전트를 다시 시작할 필요가 없습니다.
  ...
  <Collapser
    ...
    title="NGINX 로그"
  >
    `nginx-log.yml.example` 파일을 `nginx-log.yml` 에 복사(또는 이름 변경)하여 자동 NGINX 액세스 및 오류 로그 구문 분석 및 New Relic으로의 전달을 활성화합니다. 에이전트를 다시 시작할 필요가 없습니다.
  ...
  <Collapser
    ...
    title="Rabbitmq 로그"
  >
    `rabbitmq-log.yml.example` 파일을 `rabbitmq-log.yml` 에 복사(또는 이름 변경)하여 자동 Rabbitmq 오류 로그 구문 분석 및 New Relic으로 전달을 활성화합니다. 에이전트를 다시 시작할 필요가 없습니다.
  ...
  <Collapser
    ...
    title="Redis 로그"
  >
    `redis-log.yml.example` 파일을 `redis-log.yml` 에 복사(또는 이름 변경)하여 자동 Redis 오류 로그 구문 분석 및 New Relic으로 전달을 활성화합니다. 에이전트를 다시 시작할 필요가 없습니다.
...
* [유창한 비트](https://fluentbit.io/) . 인프라 에이전트는 이미 최신 버전을 설치합니다. 특정 버전으로 업데이트/다운그레이드하려면 [이 섹션](#install-fb-version) 을 참조하십시오.
* 버전 1.16.4부터 인프라 에이전트에는 OpenSSL 라이브러리 1.1.0 이상이 필요합니다.
* 인프라 에이전트 [1.26.0](https://github.com/newrelic/infrastructure-agent/releases/tag/1.20.6)에 Linux 시스템 (예: AWS Graviton 아키텍처) 에 ARM64 아키텍처에 대한 내장 지원이 추가되었습니다.

<Callout variant="important">
  로그 전달 기능은 [인프라 모니터링 에이전트의 도킹 (Docker) 컨테이너](/docs/infrastructure/install-infrastructure-agent/linux-installation/docker-container-infrastructure-monitoring/)에서 지원되지 않습니다.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        버전 16.04.x, 18.04.x및 20.04.x (LTS 버전)
      </td>
    </tr>
...
새 Relic 계정이 아직 없거나 프로시저를 수동으로 수행하려는 경우에는 [자습서를 참조하여 패키지 관리자 설치](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-agent-linux-using-package-manager)를 참조하십시오.

## 인프라 에이전트 구성 [#configure]

구성 파일은 전달되는 로그 소스를 설명합니다. 인프라 에이전트는 `.yml` 파일을 사용하여 로깅을 구성합니다. 원하는 만큼 구성 파일을 추가할 수 있습니다.

로그 전달 기능에 대한 새 구성 파일을 추가하려면 다음을 수행하십시오.

1. 로그 전달자 구성 폴더를 탐색하십시오.

   * Linux: `/etc/newrelic-infra/logging.d/`
   * Windows: `C:\Program Files\New Relic\newrelic-infra\logging.d\`

2. `logging.yml` 구성 파일을 작성하고 필요한 매개변수를 추가하십시오. `logging.d` 디렉토리에는 참조 또는 시작점으로 사용할 수 있는 다양한 `.yml.example` 파일이 있습니다.

에이전트는 인프라 모니터링 서비스를 다시 시작하지 않고도 자동으로 새 구성 파일을 처리합니다. 이에 대한 유일한 예외는 사용자 정의 유동성 비트 구성을 구성하는 경우입니다.

## 전달 매개변수 로그 [#parameters]
...
<CollapserGroup>
  <Collapser
    ...
    title="파일"
  >
    로그 파일 또는 파일에 대한 경로입니다. 에이전트는 `tail -f shell`과 유사한 방식으로 로그 파일의 변경사항을 추적합니다.
    ...
    **예시:**
    ...
    <Callout variant="important">
      와일드카드를 사용하면 파일 설명자의 수가 크게 증가할 수 있으며 inotify는 Fluent Bit 프로세스가 열린 상태를 유지하여 호스트의 파일 설명자 제한에 도달하면 로그 수집을 방해할 수 있습니다. 많은 수의 파일을 처리하려면 운영 체제에서 허용하는 파일 설명자와 inotify 감시자의 최대 수를 늘려야 할 수 있습니다. 로그 파일을 늘리는 방법에 대한 자세한 내용 [은 많은 양의 로그 파일을 추적할 때 발생하는 오류를](#too-many-files) 참조하십시오.
    </Callout>
  </Collapser>
  ...
  <Collapser
    ...
    title="systemd"
  >
    Linux 환경에서 `journald` 데몬이 수집한 로그 메시지를 전달하려면 `systemd` 매개변수를 사용하십시오. 이 입력 유형을 사용하려면 에이전트가 [루트 모드](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes) 에서 실행되어야 합니다.
  ...
  >
    ...
    * `unix_permissions:` 도메인 소켓의 경우 기본값은 `0644` 입니다. 이는 루트로 실행되는 프로세스에 대한 항목을 제한합니다. `0666` 을 사용하여 사용자 자신의 위험에 따라 루트가 아닌 프로세스를 청취할 수 있습니다.
    ...
    [권한 모드](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes)에서 에이전트를 실행하는 경우 다른 프로세스가 소켓에 로그를 기록할 수 있도록 `0666` 파일 권한과 함께 `nri-agent`가 포트 및 소켓을 사용하거나 소유해야 합니다.
...
다음 구성 매개변수는 필요하지 않지만 여전히 권장됩니다.
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      `attributes` 구성 매개변수는 외부 Fluent Bit 구성(예: `fluentbit` 구성 매개변수 사용)을 통해 전달된 로그에 사용자 정의 속성을 추가하지 않습니다. 이 시나리오에서는 [Fluent Bit 문서](https://docs.fluentbit.io/manual/) 의 `record_modifier` 옵션을 참조해야 합니다.
    </Callout>

    `attributes` 구성 매개변수의 한 가지 일반적인 사용은 `logtype` 속성을 지정하는 것입니다. 이 속성을 사용하면 새 릴릭 로그에서 지원하는 [기본 제공 구문 분석 규칙](https://docs.newrelic.com/docs/logs/log-management/ui-data/parsing/#built-in-rules) 중 하나를 활용할 수 있습니다.
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            인프라 에이전트는 실행 중인 호스트를 식별하기 위해 New Relic에서 할당한 [Entity GUID](/attribute-dictionary/span/entityguid) 를 삽입합니다. `entity.guids` 필드에서 사용할 수 있습니다.

            참고: 캡처된 로그가 APM을 사용하여 인스트루먼트된 애플리케이션에 속하는 경우, `entity.guids` 필드에는 인프라의 엔티티 GUID와 APM의 GUID가 포함되며, 이는 파이프 (|) 로 구분됩니다. 분리문자입니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            로그를 캡처하는 데 사용되는 기본 [F부유층 Bit 입력 플러그인 유형](https://docs.fluentbit.io/manual/pipeline/inputs) 입니다. 현재 값은 `tail`, `systemd`, `winlog`, `syslog`및 `tcp`입니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            로그를 캡처하는 데 사용되는 유틸리티를 표시합니다. 이 경우 인프라 에이전트 자체이므로 이 속성에는 항상 `nri-agent`값이 있습니다.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <Collapser
    ...
    title="pattern"
  >
    레코드 필터링에 대한 정규식입니다. `tail`, `systemd`, `syslog`및 `tcp` (형식이 `none`인 경우에만) 에 대해서만 지원됩니다.

    이 필드는 Unix 시스템에서 `grep -E` 와 유사한 방식으로 작동합니다. 예를 들어, 캡처되는 주어진 파일의 경우 다음을 사용하여 `WARN` 또는 `ERROR` 를 포함하는 레코드를 필터링할 수 있습니다.
  ...
  <Collapser
    ...
    title="fluentbit"
  >
    외부 [유동성 비트](https://fluentbit.io/) 구성 및 구문 분석기 파일. 정의된 경우, 인프라 에이전트가 생성한 기존 구성 및 구문 분석기 파일과 병합됩니다.
    ...
    런타임 파일은 [`[SERVICE]` 섹션](https://docs.fluentbit.io/manual/administration/configuring-fluent-bit/configuration-file#config_section) 을 정의하지 않고 모든 기본 Fluent Bit 구성 값을 유지합니다. 외부 Fluent Bit 구성 파일에서 고유한 `[SERVICE]` 섹션을 정의하고 `fluentbit` 옵션을 통해 포함하여 Fluent Bit의 기본 설정을 무시할 수 있습니다.
...
다음은 YAML 형식의 `logging.d/ `구성 파일의 예입니다. 더 많은 구성 예는 [인프라 에이전트 저장소 ](https://github.com/newrelic/infrastructure-agent/tree/master/assets/examples/logging)을 참조하십시오.
...
<CollapserGroup>
  ...
  >
    로그 전달 기능을 사용하려면 에이전트에 데이터 소스를 읽을 수 있는 권한이 있어야 합니다. 인프라 에이전트를 [특권 또는 비특권 모드](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes) 로 실행할 때 전달하려는 로그 파일(및 해당 경로의 모든 중간 디렉토리)을 `nri-agent` 를 실행하는 사용자가 읽을 수 있는지 확인하십시오.
    ...
    **예: Linux에서 파일 액세스 확인**
    ...
    파일이 `nri-agent` 사용자에게 표시되지 않으므로 이 명령이 실패했습니다. 이전 출력을 검사하여 `restrictedLogs` 디렉토리에 `others`의 실행 플래그가 누락되었음을 발견할 수 있습니다.
    ...
    이 예제에서 파일에는 `others` 그룹 ( `vagrant` 및 `vagrant` 사용자 그룹 이외의 사용자) 에 대한 읽기 권한이 없습니다. `others`에게 읽기 권한을 부여하여 이를 수정할 수 있지만, 응용프로그램은 다시 시작할 때 이러한 권한을 변경할 수 있습니다.

    이를 피하려면 `vagrant` 사용자 그룹에 `nri-agent` 사용자를 추가하는 것이 더 좋습니다.
  </Collapser>

  <Collapser
    className="freq-link"
    id="syslog"
    title="시스템 로그 소켓을 통해 캡처할 때 데이터가 표시되지 않습니다."
  >
    로그 전달 기능을 사용하려면 에이전트에 데이터 소스를 읽을 수 있는 권한이 있어야 합니다. [특권 또는 권한이 부여되지 않은 모드](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes)에서 인프라스트럭처 에이전트를 실행하는 경우:

    * Unix 도메인 소켓 파일을 사용하는 경우, `nri-agent` 사용자가 이 파일에 액세스할 수 있는지 확인하고 (이전 섹션을 참조하십시오.) `nri-agent` 가 아닌 다른 사용자가 이 파일에 쓸 수 있도록 읽기 및 쓰기 권한 (`666`) 이 있는지 확인하십시오.

    * IP 소켓을 사용하는 경우, 사용 중인 포트가 시스템 예약 포트 (예: 포트 `80`) 가 아닌지 확인하십시오.

      로그 관리를 사용 가능하게 한 후 데이터가 표시되지 않으면 [표준 로그 관리 문제점 해결 프로시저](/docs/logs/new-relic-logs/troubleshooting/no-data-appears-logs)를 따르십시오.
  </Collapser>
  ...
  >
    ...
    **Linux** `HTTP` 프록시의 경우 인증서를 설정할 필요가 없습니다. 플러그인은 시스템 인증서를 로드하고 새 릴릭은 로깅 엔드포인트로 로그를 전송합니다. 그러나 `caBundleFile` 또는 `caBundleDir` 매개변수를 사용하여 프록시 자체 서명 인증서 (PEM 파일) 를 지정할 수 있습니다.
    ...
    * 프록시 인증서를 시스템 풀로 가져오십시오 (권장) MMC 도구를 사용하여 프록시 자체 서명 인증서 (PEM 파일) 를 가져오십시오. [이 링크](https://www.ssls.com/knowledgebase/how-to-import-intermediate-and-root-certificates-via-mmc/)를 참조하고 **단계 2** 에서 `Intermediate Certification Authorities`대신 `Trusted Root Certification Authorities`으로 가져오도록 하십시오.
    * Windows에서 `caBundleFile` 및 `caBundleDir` 매개변수를 사용하여, 시스템 인증 풀과 `caBundleFile` `caBundleDir` 매개변수로 지정된 인증서를 둘 다 로드할 수 없습니다. 따라서 `caBundleFile` 또는 `caBundleDir`을 사용하는 경우, 다음 인증서가 동일한 PEM 파일 ( `caBundleFile`을 사용하는 경우) 또는 동일한 디렉토리 ( `caBundleDir`사용 시) 에 있는지 확인하십시오.
  ...
  <Collapser
    className="freq-link"
    id="agent-logs"
    title="인프라 에이전트의 로그를 New Relic에 전송"
  >
    자체 로그를 New Relic에 전송하도록 인프라 에이전트를 구성할 수 있다. 이는 로그 전달, 에이전트 또는 [지원](https://support.newrelic.com/)에 문의할 때 문제점을 해결하는 데 유용합니다.

    인프라 에이전트 로그를 새 항목으로 전달하려면 다음을 수행하십시오.

    1. `newrelic-infra.yml` 파일을 편집하십시오.

    2. 자세한 정보를 추가하여 문제 해결 모드에서 에이전트 로깅을 활성화합니다 [`verbose: 3`](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#verbose) .

    3. `systemd` 를 사용하지 않거나 `journald` 에 액세스할 수 없는 Windows및 시스템에서 `verbose: 3` 은 에이전트가 디스크에 로그를 기록하게 합니다. 이를 방지하려면 `verbose: 0` 로 되돌립니다.

    4. **(권장):** JSON 형식의 에이전트 로깅을 `log_format: json` 으로 활성화합니다.

    5. 새 설정을 로드하려면 [에이전트를 다시 시작하십시오](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status) .

       이 구성은 에이전트를 문제 해결 모드로 설정하지만 로그 전달자( [Fluent Bit](https://fluentbit.io/) 기반)는 상세하지 않은 모드에서 계속됩니다.
    ...
    6. `verbose` 를 `0`이외의 값으로 설정하십시오.

    7. 구성 옵션 `trace: ["log.fw"]`을 추가하십시오.

       <Callout variant="caution">
         \[`fluentbit`] 옵션을 사용 중인지 확인하십시오. `verbose: 3` **및** `trace: ["log.fw"]`을 설정할 때 외부 F유동성 비트 구성 파일에서 `stdout` 을 가리키는 `[OUTPUT]` 섹션을 정의하지 않았는지 확인하십시오.
       </Callout>
  </Collapser>
  ...
  >
    ...
    1. `infra-agent` 를 실행하여 유동성 비트를 시작했는지 확인하십시오.
  ...
  >
    ...
    이 문제를 해결하려면 해당하는 경우 [Microsoft Visual C++ 재배포](https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads) 가능 패키지를 설치하십시오.
  ...
  >
    ...
    * `The user limit on the total number of inotify watches was reached or the kernel failed to allocate a needed resource`
      ...
      로그를 전달하는 데 사용하는 기본 기술인 [Fluent Bit ](https://fluentbit.io/)은 하나의 파일 설명자를 열고 전달하도록 구성한 각 파일에 대해 inotify 감시를 설정합니다. 게다가, 이 섹션을 작성하는 시점에서 Fluent Bit는 정상 작동을 위해 32개의 추가 파일 디스크립터 세트를 사용하고 종료될 때 또 다른 추가 파일 디스크립터를 사용합니다. 따라서 **많은 양의 파일을 캡처하려면 파일 설명자와 inotify 감시 제한이 모두 추적하려는 로그 파일의 양보다 약간 더 큰지 확인해야 합니다 **.

      다음 지침은 10,000개의 로그 파일을 추적하려는 경우 이러한 제한을 늘리는 방법을 요약합니다. 또한 인프라 에이전트가 `root` [실행 모드](/docs/infrastructure/install-infrastructure-agent/linux-installation/linux-agent-running-modes/) 로 설치되어 있다고 가정하므로 `root` 사용자를 사용하여 실행해야 합니다.
    ...
    2. `/etc/security/limits.conf`에 다음 행을 추가하십시오. `10000` 대신 여기에 `10100` 한계를 지정하여 작업해야 할 수 있는 여분의 파일 설명자를 할당할 수 있습니다.

       ```
       root    soft    nofile  10100  # replace root by nri-agent for non-root (privileged and unprivileged) installations
       ```

    3. 다음 행을 `/etc/pam.d/common-session` 에 추가하여 재시작 시 이전 한계가 적용되도록 하십시오.

       ```
       session required pam_limits.so
       ```

    4. `/etc/sysctl.conf` 에 다음 행을 추가하여 사용자당 허용되는 Inotify 관측자의 양을 늘리십시오. `root` 사용자에게 여전히 `8192` 사용 가능한 Inotify 시계 (기본값) 가 있도록 `18192` 의 한계를 `10000` 으로 지정했습니다.
    ...
    6. 다음을 실행하여 새 한계가 적용되었는지 확인하십시오.
       ...
       파일 열기 제한 [을 늘리거나 inotify 감시를 늘리는](https://dev.to/rubiin/ubuntu-increase-inotify-watcher-file-watch-limit-kf4) [방법에 대해 자세히 알아보세요](https://tecadmin.net/increase-open-files-limit-ubuntu/) .
  </Collapser>

  <Collapser
    className="freq-link"
    id="install-fb-version"
    title="특정 F넉넉 비트 버전 설치 (Linux)"
  >
    버전 [1.19.0](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-1190) (또는 SLES 12.5의 경우 버전 [1.20.3](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-1203) ) 이전에 Linux 인프라 에이전트가 유동성 비트 바이너리로 번들되었습니다. 이 버전에서 시작하여, F넉넉 Bit는 이제 별도의 `recommended` 패키지 종속성으로 포함됩니다.
...
## 로그 전달 사용 안함 [#uninstall]

로그 전달 기능을 사용하지 않으려면 `logging.d` 디렉토리로 이동하여 원래 [구성](#configure) 프로세스 중에 추가된 확장자가 `.yml` 인 파일을 제거하십시오.
