...
* 인프라 UI의 호스트 테이블에서
  ...
  <DoNotTranslate>**View logs**</DoNotTranslate>
...
<CollapserGroup>
  ...
  >
    ...
    * `HTTPS`의 경우 다음 방법 중 하나로 구성할 수 있습니다.
      ...
      * (권장) 프록시 인증서를 시스템 풀로 가져옵니다. MMC 툴을 사용하여 프록시 자체 서명 인증서(PEM 파일)를 가져옵니다. [이 링크](https://www.ssls.com/knowledgebase/how-to-import-intermediate-and-root-certificates-via-mmc/)를 참조하고, 2단계(<DoNotTranslate>**Step 2**</DoNotTranslate>)에서는 `Trusted Root Certification Authorities` 대신 `Intermediate Certification Authorities`에서 가져옵니다.
