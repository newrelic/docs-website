...
* 인프라 UI의 호스트 테이블에서 특정 호스트에 대한

  <Icon name="fe-more-horizontal"/>

  아이콘을 클릭한 다음 **View logs**를 클릭합니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `HTTPS`의 경우 다음 방법 중 하나로 구성할 수 있습니다.
      ...
      * (권장) 프록시 인증서를 시스템 풀로 가져옵니다. MMC 툴을 사용하여 프록시 자체 서명 인증서(PEM 파일)를 가져옵니다. [이 링크](https://www.ssls.com/knowledgebase/how-to-import-intermediate-and-root-certificates-via-mmc/)를 참조하고 **2단계**에서는 `Trusted Root Certification Authorities` 대신 `Intermediate Certification Authorities`에서 가져오도록 합니다.
