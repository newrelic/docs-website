...
## tarball을 통해 설치된 인프라 에이전트에 로그 포워더 추가 [#tarball-install]
...
2. 다음과 유사한 명령을 실행하여 New Relic의 [fluent-bit-package(RPM)](https://github.com/newrelic/fluent-bit-package) 를 다운로드하고 설치합니다.

   ```shell
    yum localinstall td-agent-bit-<some-version>.rpm`
   ```

3. New Relic의 [fluentbit 플러그인( `out_newrelic.so` )](https://github.com/newrelic/newrelic-fluent-bit-output) 을 다운로드하고 생성한 `/var/db/newrelic-infra/newrelic-integrations/logging/` 폴더에 복사합니다.

4. [이 Github 저장소](https://github.com/newrelic/fluent-bit-package/blob/main/parsers.conf) 에서 `parsers.conf` 파일을 복사하고 생성한 `/var/db/newrelic-infra/newrelic-integrations/logging/` 폴더에 저장합니다.
