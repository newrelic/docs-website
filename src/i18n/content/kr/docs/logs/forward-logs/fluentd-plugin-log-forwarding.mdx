...
4. 해당하는 경우 [EU 또는 일본 엔드포인트를 구성](#eu-configuration)합니다.
...
## EU 또는 일본 계정용 Fluentd 플러그인 구성 [#eu-configuration]

기본적으로 Fluentd 플러그인은 미국의 엔드포인트 `https://log-api.newrelic.com/log/v1`으로 로그를 전달합니다. 뉴렐릭을 담당하는 조직이 [EU 또는 일본 데이터센터](/docs/accounts/accounts-billing/account-setup/choose-your-data-center)에 있는 경우 수동으로 `base_uri` 속성을 적절한 엔드포인트로 설정해야 합니다:

* EU: `https://log-api.eu.newrelic.com/log/v1`
* 일본: `https://log-api.jp.nr-data.net/log/v1`

예를 들어, 로그를 EU 엔드포인트로 전달하려면:
...
일본 계정의 경우 `<match>` 블록의 `base_uri` 를 일본 엔드포인트로 바꿉니다:
