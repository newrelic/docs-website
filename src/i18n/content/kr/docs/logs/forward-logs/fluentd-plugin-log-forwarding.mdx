...
1. `fluent.conf` 파일에서 또는 td-agent를 사용하는 경우 `td-agent.conf`에서 다음 데이터 블록을 추가하고 자리 표시자 텍스트를 [뉴렐릭 라이선스 키](/docs/accounts/install-new-relic/account-setup/license-key)로 바꿉니다.
