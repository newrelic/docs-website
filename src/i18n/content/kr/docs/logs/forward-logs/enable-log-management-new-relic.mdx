...
뉴렐릭의 <InlinePopover type="logs" />기능은 로그 데이터를 수집, 처리, 탐색, 쿼리 및 알림 설정을 하는 데 도움이 됩니다. 로그를 뉴렐릭으로 가져오려면 다음 옵션 중 하나를 사용할 수 있습니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        뉴렐릭의 솔루션을 사용해 인프라 에이전트,<DNT>Fluent Bit</DNT>, <DNT>Fluentd</DNT>, <DNT>Logstash</DNT> 및 <DNT>syslog/TCP</DNT> 등에서 로그를 [운영 체계로 전달](#forward-infra)합니다. 앱 및 호스트의 로그를 위한 경량 데이터 수집기로 [인프라 모니터링을 위한 안내 설치](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent)를 사용합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        쿠버네티스 환경의 경우 [쿠버네티스 플러그인](/docs/logs/forward-logs/kubernetes-plugin-log-forwarding/)을 사용합니다.

        Docker 환경의 경우 몇 가지 옵션이 있습니다.

        * [APM 문맥적 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 사용
        * 인프라 에이전트를 사용하여 호스트 수준 로그를 수집합니다. `logging.yml` 구성 파일에서 `/var/log/containers/*.log` 디렉터리를 추적해야 합니다. 예:
          ...
          ```
        * [독립 실행형 Docker 이미지](https://hub.docker.com/r/newrelic/newrelic-fluentbit-output) 사용
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <InlinePopover type="apm" />에이전트와 인프라 에이전트를 사용합니다. 앱의 APM 에이전트가 [APM의 문맥적 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)를 사용해 로그 데이터를 뉴렐릭으로 직접 전달하도록 합니다. 추가로 서드파티 소프트웨어를 설치하거나 유지 관리할 필요가 없습니다!
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * <DNT>syslog</DNT> 프로토콜을 사용해 [TCP 엔드포인트](/docs/logs/log-api/use-tcp-endpoint-forward-logs-new-relic)를 통해 로그를 전달합니다.
...
1. 로그를 보고하려는 계정에 대한 뉴렐릭 <InlinePopover type="licenseKey" />를 준비합니다.
2. [호환되는 로그 전달자](#log-forwarding)를 설치합니다.
3. TCP 포트 443의 아웃바운드 연결이 해당 [지역](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)과 일치하는 [CIDR 범위](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure)로 허용되는지 확인합니다.
4. 계정 데이터가 뉴렐릭의 어느 위치에 저장되었는지에 따라, 구성에 DNS 이름 `log-api.newrelic.com` 또는 `log-api.eu.newrelic.com`을 사용합니다.
5. 해당되는 경우 [수동 문맥적 로그](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/) 절차에 따라 앱의 <InlinePopover type="apm" />에이전트를 구성합니다.
...
* [<DNT>Fluent Bit</DNT> 플러그인](/docs/logs/forward-logs/fluent-bit-plugin-log-forwarding)
* [<DNT>Fluentd</DNT> 플러그인](/docs/logs/forward-logs/fluentd-plugin-log-forwarding)
* [<DNT>Logstash</DNT> 플러그인](/docs/logs/forward-logs/logstash-plugin-log-forwarding/)
...
뉴렐릭은 다음 로그 전달자를 지원합니다. 이러한 옵션 중 일부는 퀵스타트가 포함되며, 퀵스타트는 즉시 사용할 수 있는 <InlinePopover type="dashboards" />와 알림을 제공합니다. 로그 전달을 설정한 후 빠른 시작을 설치할 수 있습니다.
...
예를 들어 CDN, 하드웨어 장치 또는 관리형 서비스 같은 일부 상황에서는 로그 전달자가 없을 수 있습니다. `rsyslog` 및 `syslog-ng` 같은 <DNT>syslog</DNT> 프로토콜을 사용해 [TCP EndPoint](/docs/logs/log-management/log-api/use-tcp-endpoint-forward-logs-new-relic/)를 통해 로그를 뉴렐릭에 전달할 수 있습니다.
...
다음 다이어그램은 로그 데이터가 <InlinePopover type="apm" />에이전트 메타데이터(컨텍츄얼 로깅)로 보강되어 포맷팅되고 뉴렐릭으로 전달되기까지 앱의 로그 메시지가 거치는 수명 주기를 보여줍니다.
...
로그 전달자(뉴렐릭 인프라 에이전트, Fluentd, Logstash 등)를 구성할 때 로그 데이터를 확장하고 강화할 수도 있습니다. [문맥적 로그](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/)를 구성함으로써, 로그 보강 프로그램은 포맷된 로그 데이터를 애플리케이션 또는 호스트의 추가 트랜잭션 정보와 연결합니다.
...
* [인프라 에이전트](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/)를 통해 로그를 전달하면 애플리케이션과 플랫폼 성능 데이터에 대해 더 심도 있는 가시성을 확보할 수 있습니다. UI에서 [인프라 로그](/docs/logs/ui-data/use-logs-ui/#links)를 검토할 수 있습니다.
