...
<CollapserGroup>
  ...
  >
    ...
    ![컨텍스트 다이어그램의 New Relic 로그](./images/new-relic-logs-in-context-diagram.jpg "New Relic에 로그를 전달하는 수명 주기")
...
인프라 로그 전달에 호스트 로그를 자동으로 포함할 필요가 없거나 이미 로그 전달자가 있는 경우 지원되는 다른 옵션을 사용하여 로그를 New Relic에 전달할 수 있습니다.
