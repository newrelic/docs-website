...
APM 에이전트가 [컨텍스트 솔루션의 자동 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context#agents)를 아직 지원하지 않는 경우, 뉴렐릭의 수동 문맥적 로그 솔루션을 계속 사용할 수 있으며, 인프라 에이전트 또는 지원되는 타사 전달자를 통해 로그를 전달할 수 있습니다. 방법은 다음과 같습니다.

1. 아직 계정이 없으시면, 지금 영구 무료 [계정을 신청](https://newrelic.com/signup)하십시오.
2. 뉴렐릭 계정의 [라이선스 키](/docs/accounts/install-new-relic/account-setup/license-key)가 있어야 합니다.
...
뉴렐릭은 로그를 뉴렐릭으로 전달하기 위한 몇 가지 솔루션을 제공합니다.

### 인프라 에이전트(권장) 및 OS 솔루션 [#forward-infra]

가이드 설치 프로세스를 사용하여 [로그 관리 및 인프라 모니터링](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/)을 함께 빠르고 쉽게 설치할 수 있습니다! 이는 앱의 로깅 데이터**와** 호스트의 로깅 데이터를 모두 뉴렐릭으로 가져오는 가장 쉬운 솔루션입니다. [작동 방식](#enable)에 대한 비디오를 시청해 보십시오.

인프라 로그 전달에 자동으로 호스트 메트릭을 포함시킬 필요가 없거나 이미 로그 전달자가 있는 경우, 다음 지원 옵션 중 하나를 사용하여 운영 체제에서 뉴렐릭으로 로그를 전달할 수 있습니다.
...
인프라 에이전트의 안내 설치 프로세스를 사용하여 로그 데이터를 빠르고 쉽게 전달할 수 있습니다. 뉴렐릭에서 로깅 데이터를 사용하는 방법의 예를 포함하여 방법을 배우려면 YouTube에서 Nerdlog 비디오(14분 46초)를 시청하십시오.
