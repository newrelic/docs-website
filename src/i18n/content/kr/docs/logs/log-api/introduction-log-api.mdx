...
활성화한 후 데이터가 나타나지 않으면 <InlinePopover type="logs"/>기능을 사용하려면 [문제 해결 절차를](/docs/logs/log-management/troubleshooting/no-log-data-appears-ui/) 따르세요.
...
당신의 <InlinePopover type="licenseKey"/>Log API에 대한 요청을 인증하고 제출된 로그 메시지가 기록될 New Relic 계정을 결정합니다. HTTP 헤더 또는 쿼리 문자열 매개변수로 전달되어야 합니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        당신의 <InlinePopover type="licenseKey"/>. [HTTP 헤더를 통해 보낼](#auth-header)수도 있습니다.
      </td>
    </tr>
  </tbody>
</table>
