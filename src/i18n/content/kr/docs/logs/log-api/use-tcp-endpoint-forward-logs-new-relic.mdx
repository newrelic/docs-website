...
5. `newrelic.conf` 에 다음을 추가합니다. 강조 표시된 값을 바꿉니다. YOUR_LICENSE_KEY의 경우 New Relic <LicenseKey/> 를 사용하세요.
...
2. `newrelic.conf` 에 다음을 추가합니다. 강조 표시된 모든 값을 바꿉니다. YOUR_LICENSE_KEY의 경우 New Relic <LicenseKey/> 를 사용하세요.
...
5. New Relic `syslog` 형식을 정의합니다. YOUR_LICENSE_KEY에는 New Relic <LicenseKey/> 를 사용하세요.
