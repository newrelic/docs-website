...
6. (선택사항): `syslog` 데이터에 맞춤 속성을 추가합니다.
   ...
   * <DoNotTranslate>**Add custom attributes directly in the template string.**</DoNotTranslate> 문자열 값에 대해서는 큰따옴표(`"`)를 이스케이프해야 합니다. 예를 들어:
   ...
   * <DoNotTranslate>**Add custom attributes using a custom property variable in the template string.**</DoNotTranslate> 이 방법을 사용하면 문자열 값을 이스케이프할 필요가 없습니다.
