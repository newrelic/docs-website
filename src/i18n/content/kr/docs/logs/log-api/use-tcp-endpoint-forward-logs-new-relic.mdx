...
우리는 [로그 데이터를 New Relic으로 가져오기](/docs/logs/forward-logs/) 위한 광범위한 솔루션을 제공합니다. 그러나 로그 전달자가 없는 다른 상황(예: CDN, 하드웨어 장치 또는 관리 서비스)에서는 TCP 끝점을 통해 syslog 프로토콜을 사용할 수 있습니다. [`rsyslog`](https://www.rsyslog.com/) 및 [`syslog-ng`](https://www.syslog-ng.com/) 와 같은 syslog 클라이언트를 사용하여 로그를 New Relic에 전달할 수 있습니다.
...
`rsyslog` 를 사용하여 New Relic에 로그를 전달하려면 다음을 수행하십시오.

1. `rsyslog` 가 TLS 암호화를 사용하여 로그를 보낼 수 있도록 하는 데 필요한 패키지를 설치합니다.

   ```
   sudo yum install rsyslog-gnutls ca-certificates
   ```

2. 선택 사항: `rsyslog` 를 구성하여 로그 파일을 추적하고 해당 내용을 New Relic으로 전달합니다. 텍스트 파일 입력 모듈을 활성화하려면 `/etc/rsyslog.conf` 파일에 다음을 추가하십시오.
...
3. `/etc/rsyslog.d/` 디렉토리에서 `newrelic.conf` 라는 텍스트 파일을 만듭니다.
...
5. 다음을 `newrelic.conf` 에 추가하고 `YOUR_LICENSE_KEY` 를 [New Relic 라이선스 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 대체합니다.
...
5. `newrelic.conf` 에 다음을 추가하고 \`YOUR_LICENSE_KEY를 [New Relic 라이센스 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 대체합니다.
...
`syslog-ng` 를 사용하여 New Relic에 로그를 전달하려면 다음을 수행하십시오.

1. `syslog-ng` 가 TLS 암호화를 사용하여 로그를 보낼 수 있도록 하는 데 필요한 `ca-certificates` 를 설치합니다.
...
4. 선택 사항: `Sources` 구성 블록에 다음을 추가하여 tail 파일에 `syslog-ng` 를 구성합니다.
...
5. New Relic `syslog` 형식을 정의하고 New [Relic 라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 추가하십시오.
...
8. 다음을 실행하여 `syslog-ng` 를 다시 시작합니다.
