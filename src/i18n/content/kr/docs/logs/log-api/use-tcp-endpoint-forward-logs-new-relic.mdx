...
<Callout variant="important">
  리전 계정을 사용하는 경우 `newrelic.syslog.nr-data.net` 대신 적절한 엔드포인트를 사용하세요:

  * EU 계정: `newrelic.syslog.eu.nr-data.net`
  * JP 계정: `newrelic.syslog.jp.nr-data.net`

  설정 파일에서 이를 교체하는 것을 잊지 마십시오, 리전 계정에 미국 엔드포인트를 사용하면 작동하지 않습니다.
</Callout>
