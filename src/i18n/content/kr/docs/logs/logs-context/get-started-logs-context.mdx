...
우리의 <InlinePopover type="apm"/>에이전트 **에는 컨텍스트에서 로그**라는 기능이 있습니다. 이 기능의 이점에 대한 자세한 내용은 [컨텍스트의 로그를](/docs/logs/logs-context/logs-in-context)참조하십시오.
...
설정하기 <InlinePopover type="apm"/>컨텍스트에서 로그인:
...
우리의 최신 <InlinePopover type="apm"/>에이전트에는 기본적으로 활성화된 컨텍스트의 로그(로그 장식 및 로그 전달)가 있습니다. 다음은 로그 인 컨텍스트 및 로그 전달을 지원하는 APM 에이전트에 대한 정보입니다.
...
* Node.js 에이전트를 제외하고
  ...
  로그 전달은 전체 로그를 보내지 않습니다. [로그 전달 세부 정보에 대해 알아보세요.](#forwarding-details)
...
모든 <InlinePopover type="apm"/>에이전트(Node.js 제외)에서 로그 전달 옵션은 전체 로그를 전달하지 않습니다. 로그 전달로 전송되는 항목에 대한 자세한 내용은 아래 축소기를 참조하십시오.
...
보고된 로깅 메트릭 <InlinePopover type="apm"/>에이전트는 APM **요약** 페이지의 **로그** 차트에 표시됩니다. 이러한 메트릭은 [로그 전달 기능](#apm-logs-defined) 과 별개이며 계정 수준에서 비활성화할 수 없습니다. 로깅 측정항목을 비활성화하려면 전용 APM 구성 문서(예: [PHP용 `logging_metrics` 구성 옵션](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#cfg-application_logging_metrics-enabled))를 참조하세요.
...
<CollapserGroup>
  ...
  >
    사용량이 기가바이트의 데이터에 대해 고정된 월별 임계값을 초과할 때 경고를 생성하려면 경고 조건에 다음 [NRQL 쿼리](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions/) 를 추가합니다.
...
자세한 사용 사례는 다음과 같습니다. <InlinePopover type="apm"/>문제의 근본 원인을 파악하기 위해 컨텍스트에 로그인합니다.
...
<CollapserGroup>
  ...
  >
    ...
    **어떤 엔터티가 관련되어 있습니까?**
    ...
    동일한 UI 페이지에서 **Logs** \[로그] 차트를 클릭하면 **Log summary** \[로그 요약] 보기를 검토할 수 있습니다. 이 보기는 계측된 로그가 수집된 시기와 해당 로그의 심각도 수준이 `Error` 이하인 시기를 보여줍니다. 또한 로그 패턴 목록과 각 패턴에 맞는 모든 로그의 비율을 보여줍니다.
  </Collapser>
  ...
  >
    엔지니어는 로그 레코드에 포함된 모든 값을 보기 원하므로 **로그 세부 정보** 보기를 클릭합니다. 이는 로그 자체가 의미가 있는지 확인하는 데 도움이 됩니다. 또한 앱이 실행되는 Kubernetes 환경이나 분산된 추적 또는 <InlinePopover type="apm"/>로그 레코드와 관련된 오류.
