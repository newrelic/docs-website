...
<CollapserGroup>
  ...
  >
    ...
    2. 구성에서 로그 데코레이션을 활성화한 다음 에이전트를 다시 시작하여 로그 데코레이션을 시작합니다.
       ...
       **`PatternLayout`/`DynamicPatternLayout`**: 줄 끝에 `NR_LINKING` 속성을 포함하도록 `conversionPattern` 를 업데이트합니다.
       ...
       **`log4net.Ext.Json`**: 메시지 JSON 속성에 `NR_LINKING` 이라는 멤버를 포함하도록 레이아웃을 업데이트합니다.

       **`log4net.Appender.AdoNetAppender`**: 이 어펜더는 다른 레이아웃과 구성이 약간 다릅니다. 메타데이터를 포함하려면 `message` 매개변수를 업데이트해야 합니다. 다음 예에서는 `PatternLayout` 를 사용하여 이를 보여줍니다.
       ...
       **`outputTemplate`을 사용하는 일반 텍스트**: 줄 끝에 `NR_LINKING` 속성을 포함하도록 `outputTemplate` 를 업데이트합니다.
       ...
       **자동 속성 출력이 포함된 JSON 포맷터**: 기본 JsonFormatter는 메시지 속성에 `NR_LINKING` 메타데이터를 쓰지 않습니다. 대신 각 속성을 자체 JSON 개체에 씁니다. 컨텍스트의 로그가 작동하려면 먼저 `message` 속성의 끝에 `NR_LINKING` 데이터를 쓰도록 사용자 정의 JSON 포맷터를 만들어야 합니다.
