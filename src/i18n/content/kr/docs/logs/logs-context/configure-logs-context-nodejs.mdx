...
5. 로그 어펜더를 올바르게 구성했는지 확인하려면 애플리케이션을 실행한 다음 쿼리 연산자 `has:span.id has:trace.id` 사용하여 [New Relic의 로그 데이터](/docs/logs/log-management/ui-data/use-logs-ui/) 를 확인하십시오.

모든 것이 올바르게 구성되고 데이터가 풍부한 메타데이터와 함께 New Relic으로 전달되는 경우 이제 로그가 JSON으로 내보내지고 `trace.id` 및 `span.id` 필드가 포함되어야 합니다. UI에 로그 데이터가 표시되지 않으면 [문제 해결 절차](/docs/logs/log-management/troubleshooting/no-log-data-appears-ui/) 를 따르세요. 컨텍스트에서 작동하는 로그의 예를 보려면 [예제 애플리케이션](https://github.com/newrelic/newrelic-node-examples/blob/main/logs-in-context/README.md) 을 살펴보십시오.
