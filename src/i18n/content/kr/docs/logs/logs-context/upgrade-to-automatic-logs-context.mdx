...
이 가이드는 이전에 [로그 전달을](/docs/logs/forward-logs/enable-log-management-new-relic/) 구성했거나 [컨텍스트에 따라 로그인](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) 한 고객에게 적용됩니다. 두 기능 중 하나도 구성하지 않은 경우 최신 버전으로 업그레이드하는 것이 좋습니다. <InlinePopover type="apm"/>[자동 로그 전달 및 로컬 장식을](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) 활용하는 에이전트입니다.
...
1. 로그를 자동으로 전달하는 최신 [APM 에이전트](/docs/new-relic-one/install-configure/update-new-relic-agent/) 로 업데이트한 다음 로그 전달자를 비활성화합니다.
...
* [루비](/docs/logs/logs-context/configure-logs-context-ruby)
