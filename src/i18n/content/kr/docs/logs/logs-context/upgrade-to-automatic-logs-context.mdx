...
이 가이드는 이전에 [로그 전달](https://docs.newrelic.com/docs/logs/forward-logs/enable-log-management-new-relic/) 또는 [컨텍스트 로그인을](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) 구성한 고객에게 적용됩니다. 두 기능 중 하나를 구성하지 않은 경우 [자동 로그 전달 및 로컬 데코레이션](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) 을 활용하려면 최신 버전의 APM 에이전트로 업그레이드하는 것이 좋습니다.
...
2. 로그 포워더와 함께 APM 에이전트 사용(포워더가 이미 있는 경우에만 권장)

   * APM 에이전트 [업데이트](https://docs.newrelic.com/docs/new-relic-one/install-configure/update-new-relic-agent/)
   * 로컬 로그 장식이 활성화되고 로그 전달이 비활성화되도록 APM 에이전트를 구성합니다.
   * [컨텍스트에서 수동 로그](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs) 를 활성화하기 위해 애플리케이션의 로깅 프레임워크 구성에 대한 이전 변경 사항을 되돌립니다.
...
[컨텍스트의 로그](https://docs.newrelic.com/docs/apm/new-relic-apm/getting-started/get-started-logs-context/) 가 구성된 경우 Kubernetes 로그 전달 [통합](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#installation) 을 사용하는 고객은 컨텍스트의 로그를 [수동으로](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs) 구현할 때 애플리케이션의 로깅 프레임워크 구성에 대한 변경 사항을 되돌려야 합니다. 이는 통합으로 수행된 Kubernetes 메타데이터 보강을 유지하는 데 필요합니다.

애플리케이션의 로깅 프레임워크는 애플리케이션 로그를 STDOUT에 기록하도록 구성해야 합니다.

로그가 컨텍스트화되도록 하려면 APM 에이전트에서 로컬 데코레이션을 활성화해야 합니다. 로그 전달 구성을 변경할 필요가 없습니다.

Kubernetes 모니터링을 업그레이드하려면 아래 단계를 따르세요.

1. APM 에이전트 [업데이트](https://docs.newrelic.com/docs/new-relic-one/install-configure/update-new-relic-agent/)
2. 로컬 로그 장식이 활성화되고 로그 전달이 비활성화되도록 APM 에이전트를 구성합니다.
3. 컨텍스트에서 [수동](https://docs.newrelic.com/docs/logs/logs-context/logs-in-context/#enable-logs) 로그를 활성화하려면 애플리케이션의 로깅 프레임워크 구성에 대한 이전 변경 사항을 되돌립니다.
4. 애플리케이션의 로깅 프레임워크가 앱 로그를 STDOUT 또는 로그 전달자가 모니터링/감시하는 파일에 기록하도록 구성되었는지 확인합니다.
5. APM 에이전트 다시 시작/변경 사항 배포