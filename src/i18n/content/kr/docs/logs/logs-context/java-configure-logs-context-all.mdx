...
APM 문맥적 로그는 앱, 호스트 및 기타 엔터티에 대한 모든 텔레메트리 데이터와 로그를 연결합니다. 이 모든 데이터를 단일 툴로 통합하면 다음을 신속하게 수행할 수 있습니다.

* 시간을 다투는 문제를 해결할 때, 수천 개의 로그 노이즈를 제거하여 가장 관련성이 높은 로그만 자동으로 볼 수 있습니다.
* 여러 유형의 텔레메트리 데이터를 탐색하고 데이터를 원래 문제와 다시 연관시킬 수 있습니다.
* UI의 동일한 위치에서 더 자세한 정보로 쉽게 분석할 수 있습니다.
* 문제를 식별하고 해결하는 데 필요한 로그 행을 찾을 수 있습니다.

예시를 포함한 보다 자세한 내용은 [APM 문맥적 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)를 시작하는 방법을 확인하십시오.
...
<CollapserGroup>
  ...
  >
    ...
    이 옵션을 사용하면 로그에 `span.id`, `trace.id`, `hostname`, `entity.guid` 및 `entity.name`이 포함됩니다. 이러한 속성은 로그를 스팬, 트레이스 및 기타 텔레메트리에 자동으로 연결하므로 문제를 더 쉽게 해결할 수 있습니다.
...
일반적으로 APM 문맥적 로그를 활성화한 후 1분 이내에 로그가 나타나기 시작합니다. 앱의 **Triage > Logs** 섹션을 확인합니다. 또한 오류와 관련된 [로그 패턴](/docs/logs/ui-data/find-unusual-logs-log-patterns)이 표시되기 시작합니다.
...
APM 문맥적 로그는 APM 에이전트 로그 데이터를 자동으로 전달하며 기본적으로 활성화되어 있습니다. 이는 보안, 규정 준수, 청구 또는 시스템 성능에 부정적인 영향을 미칠 수 있습니다. 보다 자세한 내용을 확인하거나 기본 설정을 조정해야 하는 경우 절차에 따라 [자동 로깅을 비활성화](/docs/logs/logs-context/disable-automatic-logging)합니다.
