...
<CollapserGroup>
  <Collapser id="1-agent" title="옵션 1(권장): 에이전트가 로그를 전달하도록 합니다.">
    ...
    <Callout variant="tip">
      이것은 *분당* 전송된 로그 수를 나타냅니다. `max_samples_stored` 을 12보다 작은 값으로 설정하면 로그가 전송되지 않는 효과가 있습니다.
    </Callout>
  ...
  <Collapser id="2-decorate" title="옵션 2: 고유한 로그 전달자를 사용합니다.">
...
<CollapserGroup>
  <Collapser id="java-dropwizard" title="Dropwizard 1.3 이상">
  ...
  <Collapser id="java-adding-classpath" title="java.util.logging 클래스 경로 추가">
  ...
  <Collapser id="java-log4j-1x" title="로그4j 1.x">
    <Callout variant="tip">
  ...
  <Collapser id="java-log4j-2x" title="로그4j 2.x">
    <Callout variant="tip">
      Java용 [컨텍스트 솔루션의 자동 로그를](#automatic) 활용하십시오. 이 수동 프로세스를 사용할 필요가 없습니다!
    </Callout>
  ...
  <Collapser id="java-logback" title="로그백 버전 1.2.0 이상">
    <Callout variant="tip">
      Java용 [컨텍스트 솔루션의 자동 로그를](#automatic) 활용하십시오. 이 수동 프로세스를 사용할 필요가 없습니다!
    </Callout>
  ...
  <Collapser id="spring" title="스프링과 스프링부트">
