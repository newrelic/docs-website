---
title: New Relic 전체에서 필요할 때 상황에 맞게 데이터를 기록합니다.
tags:
  - Logs
...
APM 에이전트, 인프라 에이전트 또는 OpenTelemetry 통합을 사용하여 로그 데이터를 보고하면 다른 플랫폼 UI 환경의 컨텍스트에서 로그 데이터를 볼 수 있는 기능인 **컨텍스트에서 로그** 를 얻습니다. 예를 들어 APM, 인프라 모니터링, 분산 추적 및 오류 받은 편지함과 같은 UI 환경에서 로그 데이터를 볼 수 있습니다. 이러한 상황 정보를 사용하면 로그 데이터를 수동으로 검색할 필요 없이 앱 또는 호스트의 성능을 더 깊이 파고들 수 있습니다.
...
APM 에이전트에는 기본적으로 활성화된 로그 전달을 포함하여 컨텍스트에 로그가 있습니다. 컨텍스트에서 APM 로그 가져오기 및 사용에 대한 자세한 내용은 [APM 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 를 참조하십시오.
...
로그 보고를 위해 APM 에이전트를 사용하는 것보다 더 강력한 대안으로 또는 APM 로그를 보완하기 위해 [인프라 에이전트를 사용하여 로그를 전달할](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent) 수 있습니다.
