...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="기력"
  >
    nrzap 플러그인은 Zap 로깅 프레임워크를 사용하여 컨텍스트 수집에서 자동 로그를 활성화합니다. 애플리케이션이 이를 사용하도록 구성되면 go-agent는 Zap으로 작성된 모든 로그를 자동으로 수집합니다. 이렇게 하려면 설치된 go-agent 버전이 3.22.0이어야 합니다. 또는 더 높게.
