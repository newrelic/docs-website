...
기본적으로 저희 <InlinePopover type="apm"/>에이전트는 중요한 뉴렐릭 메타데이터로 귀하의 로그 로그를 자동으로 장식하고 해당 로그를 뉴렐릭으로 전달합니다. 이 기능의 작동 방식과 이 기능이 활성화된 에이전트 버전에 대한 자세한 내용은 [컨텍스트에서 APM 로그를](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 참조하세요.
...
<CollapserGroup>
  <Collapser
    ...
    title="UI를 통한 설정 관리"
  >
    <DNT>**Data management**</DNT> UI 의 <DNT>**Log settings**</DNT> UI 페이지에서는 계정에 대해 [APM 로그](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) (로그 장식 및 로그 포워딩 모두)를 활성화하거나 비활성화할 수 있습니다.
    ...
    하위 계정 수준에서는 상위 계정이 이 설정을 잠그지 않는 한 <InlinePopover type="apm"/>로그인 포워딩을 활성화하거나 비활성화할 수 있습니다. 상위 계정이 APM 로그 포워딩을 잠그면 하위 계정은 상위 계정의 활성화 또는 비활성화 설정을 상속받으며 변경할 수 없습니다.
  ...
  <Collapser
    ...
    title="API를 통해 관리"
  >
    NerdGraph API 탐색기에서 `dataManagementUpdateFeatureSettings` 사용합니다: <DNT>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**</DNT>. GraphiQL UI에 채워진 예제와 관련 문서를 참조하세요.
  ...
  >
    ...
    <Callout variant="important">
      로그 포워딩을 비활성화해도 APM <DNT>**Summary**</DNT> 페이지의 <DNT>**Logs**</DNT> 차트에 표시되는 로깅 지표에는 영향을 미치지 않습니다. 자세한 내용은 [로그 기록 을](/docs/logs/logs-context/get-started-logs-context#apm-logs-defined) 참조하세요.
    </Callout>

    로그 포워딩 설정 옵션에 대한 자세한 내용은 특정 <InlinePopover type="apm"/>에이전트에 대한 절차를 참조하세요.
...
<CollapserGroup>
  ...
  >
    API 또는 UI 통해 자동 로그 포워딩이 비활성화되면 기본적으로 로그를 보낼 수 있는 모든 <InlinePopover type="apm"/>언어 에이전트는 해당 계정에 대해 앞으로 로깅이 꺼지게 됩니다.
  </Collapser>
  ...
  >
    <InlinePopover type="apm"/>로그를 끄면 로그인 데이터가 전달되지 않습니다. 로그 검사의 다른 모든 측면은 동일하게 유지됩니다. 에이전트에서 모든 로깅 관련 활동을 완전히 비활성화하려면 에이전트의 [클라이언트 측 설정을](#disable-config) 로그인 비활성화로 변경하세요.
  </Collapser>
  ...
  >
    <InlinePopover type="apm"/>데이터 수집에는 변화가 없지만 로그 데이터 수집은 중단됩니다. 뉴렐릭에서의 데이터 관리에 대한 자세한 내용은 [데이터 수집 문서를](/docs/data-apis/manage-data/manage-data-coming-new-relic) 참조하세요.
  </Collapser>
</CollapserGroup>
