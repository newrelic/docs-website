...
4. 로그 어펜더를 올바르게 구성했는지 확인하려면 애플리케이션을 실행한 다음 쿼리 연산자 `has:span.id has:trace.id` 사용하여 [New Relic의 로그 데이터](/docs/logs/log-management/ui-data/use-logs-ui/) 를 확인하십시오.
