...
Python에서 모니터링하는 APM 앱의 컨텍스트에서 로그를 활성화하려면 수동 설치 옵션을 사용할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    Python에서 로그를 활성화하는 것은 로그 포맷터를 인스턴스화하고 로그 핸들러에 추가하는 것만큼 간단합니다. 이 예에서는 기본적으로 `sys.stderr` 에 로그를 기록하는 `StreamHandler` 을 사용하지만 모든 핸들러를 사용할 수 있습니다. 로그 처리기 구성에 대한 자세한 내용은 [Python.org 설명서](https://docs.python.org/3/library/logging.handlers.html) 를 참조하십시오.
