...
Python <InlinePopover type="apm" />에이전트를 사용하면 다른 뉴렐릭 데이터의 컨텍스트에서 앱 로그를 볼 수 있는 <DNT>**logs in context**</DNT>(를) 얻을 수 있습니다. 이 기능에 대한 일반 정보는 [컨텍스트의 APM 로그를](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 참조하세요.
...
<CollapserGroup>
  <Collapser id="1-agent" title="옵션 1(가장 단순함): 에이전트가 로그를 장식하고 전달하도록 합니다.">
