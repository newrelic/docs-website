...
로그 데이터를 APM의 오류 및 분산 추적과 같은 다른 원격 분석 데이터와 연관시키려면 [컨텍스트에서 APM 로그를](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 사용할 수 있습니다. 로깅 프레임워크가 컨텍스트 솔루션의 기존 로그에서 지원되지 않는 경우 APM 에이전트 API를 사용하여 로깅 라이브러리를 구성할 수 있습니다.
