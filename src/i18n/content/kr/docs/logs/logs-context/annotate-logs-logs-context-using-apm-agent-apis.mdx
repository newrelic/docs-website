...
로그 데이터를 APM의 오류 및 분산 추적과 같은 다른 원격 측정 데이터와 상관시키기 위해 [컨텍스트 솔루션의 로그를](/docs/logs/new-relic-logs/enable-logs-context/enable-logs-context-apm-agents) 사용할 수 있습니다. 컨텍스트 솔루션의 기존 로그에서 로깅 프레임워크를 사용할 수 없는 경우 API 호출을 사용하여 로그에 주석을 추가하여 로깅 라이브러리를 구성할 수 있습니다.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        [C SDK(해당 사항 없음)](/docs/logs/logs-context/c-sdk-configure-logs-context/)
      </td>

      <td>
        [Log API 문서를](/docs/logs/log-api/introduction-log-api/) 참조하십시오.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * [newrelic_get_linking_metadata](/docs/apm/agents/php-agent/php-agent-api/newrelicgetlinkingmetadata/)
          ...
          PHP의 경우 컨텍스트의 로그는 APM UI의 **로그** 탭이 아닌 분산 추적 UI에서만 지원됩니다.
      </td>
    </tr>
...
또한 컨텍스트 확장에서 자체 로그에 대한 소스 코드를 검토하여 이러한 API를 사용하는 방법을 확인하십시오.

* C SDK: 해당 없음
* 이동: [Logrus 확장](https://github.com/newrelic/go-agent/blob/master/_integrations/logcontext/nrlogrusplugin/nrlogrusplugin.go)
