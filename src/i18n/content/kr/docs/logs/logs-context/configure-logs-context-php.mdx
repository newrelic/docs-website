...
PHP APM 에이전트를 사용하면 컨텍스트에서 **로그를** 가져올 수 있으므로 다른 New Relic 데이터 컨텍스트에서 앱 로그를 볼 수 있습니다. 이 기능에 대한 일반 정보 [는 컨텍스트의 APM 로그를](/docs/apm/new-relic-apm/getting-started/get-started-logs-context) 참조하십시오.
