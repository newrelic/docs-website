...
import logParsingRuleUi from 'images/log-parsing-rule-ui.png'

import logParsingRules from 'images/log-parsing-rules.png'

import logParsingRulesList from 'images/log-parsing-rules-list.png'
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * 일치 프로세스를 단순화하려면 로그에 [`logtype`](#logtype) 속성을 추가하는 것이 좋습니다. 그러나 `logtype` 사용에 국한되지 않습니다. 모든 속성을 일치 기준으로 사용할 수 있습니다.
      </td>
    </tr>
...
구문 분석 패턴은 로그 메시지 구문 분석을 위한 업계 표준인 Grok를 사용하여 지정됩니다. `logtype` 필드가 있는 모든 수신 로그는 [기본 제공 패턴](#built-in-rules) 과 비교하여 확인되며 가능한 경우 연결된 Grok 패턴이 로그에 적용됩니다.

Grok은 리터럴 복잡한 정규식 대신 사용할 내장 명명된 패턴을 추가하는 정규식의 상위 집합입니다. 예를 들어, 정수가 정규식 `(?:[+-]?(?:[0-9]+))` 과 일치할 수 있다는 것을 기억하는 대신 `%{INT}` 를 사용하여 사용할 수 있습니다. 동일한 정규식을 나타내는 Grok 패턴 `INT` .
...
<CollapserGroup>
  ...
  >
    ...
    1. 규칙에 이름을 지정합니다. 예를 들어 `Inventory Services error parsing` .

    2. 들어오는 로그에 대한 사전 필터 역할을 하는 속성/값 쌍을 식별합니다. 예: `entity.name` 및 값 `Inventory Service` . 이 사전 필터는 규칙에서 처리해야 하는 로그 수를 줄여 불필요한 처리를 제거합니다.
    ...
    * `product_id` : Inventory Service의 제품 ID
    ...
    5. 구문 분석 규칙을 활성화하고 저장합니다.
       ...
       곧 Inventory Service 로그가 `error_message` 및 `product_id` 라는 두 개의 새로운 필드로 강화된 것을 볼 수 있습니다. 여기에서 이러한 필드에 대해 쿼리하고, 차트 및 대시보드를 만들고, 경고를 설정하는 등의 작업을 수행할 수 있습니다.
  ...
  >
    ...
    * `OPTIONAL_EXTRACTED_ATTRIBUTE_NAME` 은 제공된 경우 패턴 이름과 일치하는 값으로 로그 메시지에 추가될 속성의 이름입니다. 정규식을 사용하여 명명된 캡처 그룹을 사용하는 것과 같습니다. 이것이 제공되지 않으면 구문 분석 규칙은 문자열의 영역과 일치하지만 해당 값으로 속성을 추출하지는 않습니다.

    * `OPTIONAL_TYPE` 은 추출할 속성 값의 유형을 지정합니다. 생략하면 값이 문자열로 추출됩니다. 예를 들어, `"File Size: 123"` 에서 값 `123` 을 숫자로 `file_size` 속성으로 추출하려면 `value: %{INT:file_size:int}` 를 사용하십시오.
...
<img
  title="새로운 Relic Logs 구문 분석 규칙 UI"
  alt="로그 구문 분석 규칙 UI"
  src={logParsingRuleUi}
/>
...
공통 로그 형식에는 이미 생성된 잘 정립된 구문 분석 규칙이 있습니다. 기본 제공 구문 분석 규칙의 이점을 얻으려면 로그를 전달할 때 `logtype` 속성을 추가하십시오. 값을 다음 표에 나열된 값으로 설정하면 해당 유형의 로그에 대한 규칙이 자동으로 적용됩니다.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        [`아파치`](/docs/logs/ui-data/built-log-parsing-rules#apache)
      </td>
    ...
    <tr>
      <td>
        [`장백의`](/docs/logs/ui-data/built-log-parsing-rules#application-load-balancer)
      </td>
    ...
    <tr>
      <td>
        [`클라우드 프론트 웹`](/docs/logs/ui-data/built-log-parsing-rules#cloudfront)
      </td>
    ...
    <tr>
      <td>
        [`엘브`](/docs/logs/ui-data/built-log-parsing-rules#elastic-load-balancer)
      </td>
    ...
    <tr>
      <td>
        [`모니터링`](/docs/logs/ui-data/built-log-parsing-rules#monit)
      </td>
    ...
    <tr>
      <td>
        [`mysql 오류`](/docs/logs/ui-data/built-log-parsing-rules#mysql-error)
      </td>
    ...
    <tr>
      <td>
        [`nginx 오류`](/docs/logs/ui-data/built-log-parsing-rules#nginx-error)
      </td>
    ...
    <tr>
      <td>
        [`루트 53`](/docs/logs/ui-data/built-log-parsing-rules#route53)
      </td>
...
로그를 집계할 때 해당 로그를 쉽게 구성, 검색 및 구문 분석할 수 있는 메타데이터를 제공하는 것이 중요합니다. 이를 수행하는 한 가지 간단한 방법은 배송될 때 로그 메시지에 속성 `logtype` 을 추가하는 것입니다. 기본 제공 [구문 분석 규칙](#built-in-rules) 은 특정 `logtype` 유형 값에 기본적으로 적용됩니다.

<Callout variant="tip">
  `logType` , `logtype` 및 `LOGTYPE` 필드는 모두 기본 제공 규칙에 대해 지원됩니다. 검색을 쉽게 하려면 조직의 단일 구문에 맞추는 것이 좋습니다.
</Callout>

다음은 [지원되는 배송 방법](/docs/logs/enable-new-relic-logs) 중 일부에서 보낸 로그에 `logtype` 을 추가하는 방법의 몇 가지 예입니다.
...
<CollapserGroup>
  ...
  >
    `logtype` 을 [`attribute`](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent#attributes) 으로 추가합니다. 명명된 각 소스에 대해 로그 유형을 설정해야 합니다.
  ...
  <Collapser
    ...
    title="유창한 예"
  >
    새 필드를 추가하기 위해 `record_transformer` 를 사용하는 `.conf file` 에 필터 블록을 추가하십시오. 이 예에서는 `nginx` 의 `logtype` 을 사용하여 기본 제공 NGINX 구문 분석 규칙을 트리거합니다. 다른 [Fluentd 예제](https://github.com/newrelic/fluentd-examples) 를 확인하십시오.
  ...
  <Collapser
    ...
    title="Fluent Bit 예제"
  >
    새 필드를 추가하기 위해 `record_modifier` 를 사용하는 `.conf` 파일에 필터 블록을 추가하십시오. 이 예에서는 `nginx` 의 `logtype` 을 사용하여 기본 제공 NGINX 구문 분석 규칙을 트리거합니다. 다른 [Fluent Bit 예제](https://github.com/newrelic/fluentbit-examples) 를 확인하십시오.
  ...
  <Collapser
    ...
    title="로그스태시 예시"
  >
    `add_field` mutate 필터를 사용하여 새 필드를 추가하는 Logstash 구성에 필터 블록을 추가하십시오. 이 예에서는 `nginx` 의 `logtype` 을 사용하여 기본 제공 NGINX 구문 분석 규칙을 트리거합니다. 다른 [Logstash 예제](https://github.com/newrelic/logstash-examples) 를 확인하십시오.
  ...
  <Collapser
    ...
    title="로그 API 예시"
  >
    New Relic으로 전송된 JSON 요청에 속성을 추가할 수 있습니다. 이 예에서는 `nginx` 값의 `logtype` 속성을 추가하여 기본 제공 NGINX 구문 분석 규칙을 트리거합니다. [Logs API](/docs/logs/new-relic-logs/log-api/introduction-log-api) 사용에 대해 자세히 알아보세요.
...
<img
  title="로그 구문 분석 규칙"
  alt="로그 구문 분석 규칙"
  src={logParsingRulesList}
/>

<figcaption>
  사용자 정의 구문 분석 규칙 목록을 보려면 로그 UI의 왼쪽 탐색 메뉴에 있는 **데이터 관리** 에서 **구문 분석** 을 클릭합니다.
</figcaption>
