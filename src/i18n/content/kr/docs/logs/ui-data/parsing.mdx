...
로그 <DoNotTranslate>parsing</DoNotTranslate>은 정의한 규칙에 따라 정형화되지 않은 로그 데이터를 속성(키:값 쌍)으로 변환하는 프로세스입니다. NRQL 쿼리에서 이 속성을 사용하면 편리하게 로그를 패싯하거나 필터링할 수 있습니다.
...
6. 매치하는 로그가 있으면 선택하거나
   ...
   <DoNotTranslate>_Unformatted_</DoNotTranslate>
