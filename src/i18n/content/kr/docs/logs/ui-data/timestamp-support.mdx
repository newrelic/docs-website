...
* 기본적으로 New Relic은 로그가 수집되는 정확한 시간에 로그 `timestamp` 속성(예약된 키워드) 값을 할당합니다. 이를 수집 시라고 합니다.
* 로그에 타임스탬프가 없으면 New Relic은 수집 시 타임스탬프를 할당합니다.
...
1. 지원되는 시간 형식 중 하나의 `timestamp` 속성이 포함된 [JSON 형식 로그를](#timestamps_in_json_logs) 수신하면 수집 타임스탬프 값을 재정의합니다. 대신 JSON 속성의 타임스탬프 값이 나타납니다.
...
`timestamp` 속성이 포함된 JSON 형식 로그를 수신하면 기본 수집 `timestamp` 값을 원래 JSON 값으로 재정의합니다. 타임스탬프 필드가 없거나 필드 형식이 잘못된 경우 메시지는 기본적으로 수집 타임스탬프로 설정됩니다. 48시간보다 오래된 타임스탬프가 있는 페이로드는 삭제된다는 점에 유의하세요.
...
* 유효한 JSON 메시지인 경우 "메시지" JSON 필드 내부. 우리의 로그 관리 기능은 모든 메시지 속성을 JSON으로 구문 분석합니다. 구문 분석된 메시지의 결과 JSON 속성이 로그에 추가됩니다.
...
<CollapserGroup>
  ...
  >
    **epoch** 형식(ms 정밀도 없음)의 `timestamp` 이 포함된 이와 같은 JSON 로그를 수신하는 경우:
...
`datetime` Grok 유형을 사용하여 지원되지 않는 형식의 타임스탬프를 구문 분석할 수 있습니다. 이렇게 하려면 로그 타임스탬프 형식과 일치하는 `pattern` 와 함께 `datetime` Grok 유형을 포함해야 합니다. `pattern` 은(는) [여기에](https://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html) 있는 Java 단순 날짜 및 시간 패턴을 사용해야 합니다. Grok 표현식이 지원되지 않는 타임스탬프 형식을 지원되는 형식으로 변환하는 방법을 보여주는 다음 예를 살펴보세요.
...
<CollapserGroup>
  ...
  >
    ...
    **ISO8601** 형식의 타임스탬프가 포함된 이 로그 메시지:
  ...
  >
    ...
    **epoch** 형식의 타임스탬프가 포함된 다음 로그 메시지:
