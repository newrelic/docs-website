...
New Relic은 기본 제공 규칙에 따라 일반적인 로그 형식을 구문 분석할 수 있으므로 고유한 구문 분석 규칙을 만들 필요가 없습니다. 다음은 로그 구문 분석 규칙, [Grok](https://grokdebug.herokuapp.com/patterns#) 패턴 및 구문 분석되는 필드입니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `cassandra.log.level`: 메시지의 로그 레벨(all, trace, debug, info, warn, error, off)
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `x-edge-result-type` 이 `Error` **이 아닌** 경우 이 필드는 `x-edge-result-type` 와 동일한 값을 포함합니다. `x-edge-result-type` **이** `Error` 인 경우 이 필드에는 특정 유형의 오류가 포함됩니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            쿼리 문자열(있는 경우)을 포함하지만 도메인 이름이 없는 전체 요청 URL입니다. 예: `/images/cat.jpg?mobile=true` .
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `x_edge_result_type` 이 `Error` **이 아닌** 경우 이 필드는 `x_edge_result_type` 와 동일한 값을 포함합니다. `x_edge_result_type` **이** `Error` 인 경우 이 필드에는 특정 유형의 오류가 포함됩니다.
          </td>
        </tr>
  ...
  >
    ...
    * `redis.log.level`: 메시지의 로그 수준(. debug, - verbose, \* 주의, # warning)
  ...
  >
    ...
    * `sd.<var>sd-id</var>.<var>sd-param-name</var>`참고: `structured.data` 콘텐츠는 사전 정의된 명명 규칙 `sd.<var>sd-id</var>.<var>sd-param-name</var>` 에 따라 별도의 속성으로도 구문 분석됩니다. 다음에 나오는 구조화된 데이터 구문 분석 예제를 참조하세요.
