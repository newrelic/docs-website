...
<CollapserGroup>
  ...
  >
    ...
    * `message` 및 `creditCardNumber` 속성에 있는 신용 카드 번호를 `HASH` 합니다.
    * `MASK` `message` 속성에 있는 생년월일.
    * `message` 및 `ssn` 속성에 있는 사회 보장 번호를 `HASH` 합니다.
  </Collapser>
  ...
  >
    ...
    난독화 표현식은 재사용 **할 수** 있습니다. 민감한 데이터를 포함하는 로그 속성의 이름을 지정하는 방법에 대해 완전히 불가지론적입니다. 예를 들어, 위에서 정의한 사회 보장 표현식은 `ssn` , `socialSecurityNumber` 또는 `socSecNum` 이라는 로그 속성에 적용될 수 있습니다.

    그러나 여전히 로그 속성의 형식과 밀접하게 결합된 재사용 불가능한 난독화 표현식(예: `Born date (loginService specific)` 을 작성할 수 있습니다. 예를 들어, 다음 `born on` 와 그 `before` 에 나오는 모든 것을 사용할 수 있습니다.
  </Collapser>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            필터는 NRQL 형식을 사용하여 로그인 서비스에서 오는 대상 로그를 식별하는 방법을 시스템에 알려줍니다. 이 예에서는 `serviceName = loginService` 및 `department = sales` 인 로그를 쿼리합니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            * 이 데이터를 난독화하기 위해 적용할 난독화 방법( `HASH_SHA256` 또는 `MASK` )
              ...
              GraphQL API를 통해 난독화 규칙을 정의할 때 이름 대신 난독화 표현식의 `id` 를 지정해야 합니다. 이전 예제를 더 읽기 쉽게 만들기 위해 대신 난독화 표현식 이름을 사용했습니다.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  >
    마지막 예로, `serviceName = checkoutService` 속성과 신용 카드 정보가 포함된 `ccn` 속성이 있는 "Checkout Service"라는 다른 서비스에서 오는 로그를 난독화해야 한다고 가정해 보겠습니다.
    ...
    이 서비스의 로그를 난독화하려면 이러한 특정 로그를 대상으로 하는 다른 난독화 규칙만 정의하면 되며 이전에 생성된 `Credit card number` 난독화 표현식을 재사용하기만 하면 됩니다.
...
<CollapserGroup>
  ...
  >
    ...
    **로그 UI 사용:**
    ...
    ![로그 UI를 사용하여 난독화 표현식 만들기](./images/obfuscation-create-expression-ui.png "로그 UI를 사용하여 난독화 표현식 만들기")

    **NerdGraph 사용:**

    `logConfigurations` 아래에서 `logConfigurationsCreateObfuscationExpression` 변경자를 사용하십시오. GraphiQL의 채워진 예제와 [**api.newrelic.com/graphiql**](https://api.newrelic.com/graphiql#query=mutation%20%7B%0A%20%20logConfigurationsCreateObfuscationExpression%28accountId%3A%201234%2C%20expression%3A%20%7Bname%3A%20%22Credit%20Card%20Number%22%2C%20regex%3A%20%22%28%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D%29%22%2C%20description%3A%20%22Reusable%20expression%20to%20capture%20credit%20card%20numbers%22%7D%29%20%7B%0A%20%20%20%20id%0A%20%20%20%20name%0A%20%20%20%20regex%0A%20%20%20%20description%0A%20%20%20%20createdAt%0A%20%20%20%20createdBy%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%20%20email%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D%0A)의 관련 문서를 참조하십시오.

    <Callout variant="important">
      NerdGraph를 통해 도입할 때는 `regex`를 이스케이프해야 합니다:`(\\d{4}-\\d{4}-\\d{4}-\\d{4})`
    </Callout>
  </Collapser>
  ...
  >
    ...
    **NerdGraph 사용:**
    ...
    `actor.account.logConfigurations` 아래의 `obfuscationExpressions` 페처를 사용하십시오. GraphiQL의 채워진 예제와 [**api.newrelic.com/graphiql**](https://api.newrelic.com/graphiql?#query=%7B%0A%20%20actor%20%7B%0A%20%20%20%20account%28id%3A%201234%29%20%7B%0A%20%20%20%20%20%20logConfigurations%20%7B%0A%20%20%20%20%20%20%20%20obfuscationExpressions%20%7B%0A%20%20%20%20%20%20%20%20%20%20id%0A%20%20%20%20%20%20%20%20%20%20name%0A%20%20%20%20%20%20%20%20%20%20regex%0A%20%20%20%20%20%20%20%20%20%20description%0A%20%20%20%20%20%20%20%20%20%20createdAt%0A%20%20%20%20%20%20%20%20%20%20createdBy%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20name%0A%20%20%20%20%20%20%20%20%20%20%20%20email%0A%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%20%20updatedAt%0A%20%20%20%20%20%20%20%20%20%20updatedBy%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20name%0A%20%20%20%20%20%20%20%20%20%20%20%20email%0A%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%7D%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D%0A)의 관련 문서를 참조하십시오.
  </Collapser>
  ...
  >
    ...
    **NerdGraph 사용:**
    ...
    `logConfigurations` 아래에서 `logConfigurationsUpdateObfuscationExpression` 변경자를 사용하십시오. GraphiQL의 채워진 예제와 [**api.newrelic.com/graphiql**](https://api.newrelic.com/graphiql?#query=mutation%20%7B%0A%20%20logConfigurationsUpdateObfuscationExpression%28accountId%3A%201234%2C%20expression%3A%20%7Bname%3A%20%22Credit%20Card%20Number%20-%20EDITED%22%2C%20regex%3A%20%22%28%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D-%5C%5Cd%7B4%7D%29%22%2C%20description%3A%20%22Reusable%20expression%20to%20capture%20credit%20card%20numbers%20-%20EDITED%22%2C%20id%3A%20%225678%22%7D%29%20%7B%0A%20%20%20%20id%0A%20%20%20%20name%0A%20%20%20%20regex%0A%20%20%20%20description%0A%20%20%20%20createdAt%0A%20%20%20%20createdBy%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%20%20email%0A%20%20%20%20%7D%0A%20%20%20%20updatedAt%0A%20%20%20%20updatedBy%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%20%20email%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D%0A)의 관련 문서를 참조하십시오.
  ...
  >
    ...
    **NerdGraph 사용:**
    ...
    `logConfigurations` 아래에서 `logConfigurationsDeleteObfuscationExpression` 변경자를 사용하십시오. GraphiQL의 채워진 예제와 [**api.newrelic.com/graphiql**](https://api.newrelic.com/graphiql?#query=mutation%20%7B%0A%20%20logConfigurationsDeleteObfuscationExpression%28accountId%3A%201234%2C%20id%3A%205678%29%20%7B%0A%20%20%20%20id%0A%20%20%20%20name%0A%20%20%20%20regex%0A%20%20%20%20description%0A%20%20%20%20createdAt%0A%20%20%20%20createdBy%20%7B%0A%20%20%20%20%20%20email%0A%20%20%20%20%20%20name%0A%20%20%20%20%7D%0A%20%20%20%20updatedAt%0A%20%20%20%20updatedBy%20%7B%0A%20%20%20%20%20%20email%0A%20%20%20%20%20%20name%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D%0A)의 관련 문서를 참조하십시오.
...
<CollapserGroup>
  ...
  >
    ...
    **NerdGraph 사용:**
    ...
    `logConfigurations` 아래에서 `logConfigurationsDeleteObfuscationRule` 변경자를 사용하십시오. GraphiQL의 채워진 예제와 [**api.newrelic.com/graphiql**](https://api.newrelic.com/graphiql?#query=mutation%20%7B%0A%20%20logConfigurationsDeleteObfuscationRule%28accountId%3A%201234%2C%20id%3A%205678%29%20%7B%0A%20%20%20%20id%0A%20%20%20%20name%0A%20%20%20%20description%0A%20%20%20%20filter%0A%20%20%20%20enabled%0A%20%20%20%20actions%20%7B%0A%20%20%20%20%20%20attributes%0A%20%20%20%20%20%20expression%20%7B%0A%20%20%20%20%20%20%20%20name%0A%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20method%0A%20%20%20%20%7D%0A%20%20%20%20createdAt%0A%20%20%20%20createdBy%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%20%20email%0A%20%20%20%20%7D%0A%20%20%20%20updatedAt%0A%20%20%20%20updatedBy%20%7B%0A%20%20%20%20%20%20name%0A%20%20%20%20%20%20email%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D%0A)의 관련 문서를 참조하십시오.
