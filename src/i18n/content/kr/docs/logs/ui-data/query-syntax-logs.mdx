...
<Callout variant="tip" title="Apache 로그 정보">
...
<CollapserGroup>
  <Collapser id="503-responce-code" title="`503` 응답 코드가 포함된 모든 Apache 로그 보기:">
    ```
    logtype:apache response:503
    ```
  </Collapser>

  <Collapser id="multiple-response-codes" title="`4xx` 또는 `5xx` 응답 코드가 있는 모든 Apache 로그 보기:">
    ```
    logtype:apache response:4* OR response 5*
    ```
  </Collapser>

  <Collapser id="method-example" title="`Method=GET` 및 `Response=2xx`로 모든 Apache 로그 보기">
    ```
...
<Callout variant="tip" title="CDN 로그 정보">
...
<CollapserGroup>
  <Collapser id="chase-miss-example" title="캐시 미스를 초래한 모든 액세스 로그 보기">
    ```
    logtype:cloudfront-rtl x_edge_result_type:Miss
    ```
  </Collapser>

  <Collapser id="edge-locations" title="애틀랜타 엣지 위치에서 캐시 미스가 발생한 모든 액세스 로그 보기">
    ```
    logtype:cloudfront-rtl x_edge_result_type:Miss x_edge_location:ATL*
    ```
  </Collapser>

  <Collapser id="methods" title="cs_method=GET을 사용해 애틀랜타 엣지 위치에서 4xx 또는 5xx 오류를 야기한 모든 액세스 로그 보기">
    ```
