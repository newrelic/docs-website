...
**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**에서 뉴렐릭의 **Logs** \[로그] UI를 사용하여 몇 초 만에 로그 데이터를 빠르게 검색할 수 있습니다. 각 로그는 `log_summary` 열에 사용 가능한 속성을 나열합니다. 추가 세부 정보를 살펴보려면 강조 표시된 속성을 클릭합니다.

시작할 준비가 되셨습니까?아직 계정이 없으시면, 아래에서 무료 뉴렐릭 계정을 생성해 지금 바로 데이터 모니터링을 시작하십시오.<InlineSignup/>

## 쿼리 구조 [#query-structure]

**Logs** \[로그] 를 사용하면 `new` 및 `relic` 같은 간단한 키워드나 `"new relic agent"` 같은 구를 검색 필드에 직접 입력하여 로그 데이터를 검색할 수 있습니다.
...
1. **All logs** \[모든 로그] UI 페이지에서 쿼리를 사용하여 503 응답 코드가 있는 모든 Apache 로그를 검색합니다. `logtype:apache response:503`
