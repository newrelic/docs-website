...
<img
  title="New Relic Logs architecture for drop filters"
  alt="New Relic Logs architecture for drop filters"
  src={ingestPipeline}
/>
...
NerdGraph를 사용하여 로그 이벤트에 대한 삭제 필터 규칙을 생성하는 경우 `source` 속성을 `Logging` 으로 설정하십시오. 로그 UI는 `source` 속성이 있는 드롭 필터만 표시합니다.
