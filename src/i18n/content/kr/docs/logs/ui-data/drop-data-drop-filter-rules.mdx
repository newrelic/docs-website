...
import logDropFilterCreate from 'images/log-drop-filter-create.png'

import logDropFilter042721 from 'images/log-drop-filter042721.png'

로그 이벤트 데이터가 New Relic으로 전송된 후 NRDB 데이터베이스에 저장되거나 삭제(폐기)될 수 있습니다. 로그 데이터를 삭제하려면 이 문서에 설명된 로그 관리 UI를 사용할 수 있습니다. 또는 [NerdGraph를 사용하여 데이터를 삭제할](/docs/accounts/accounts/data-management/drop-data-using-nerdgraph) 수 있습니다.
...
삭제 필터 규칙은 쿼리를 기반으로 데이터를 일치시킵니다. 트리거되면 삭제 필터 규칙은 NRDB에 쓰기 전에 수집 파이프라인에서 일치하는 데이터를 제거합니다.
...
<img
  title="New Relic Logs - 아키텍처 - 필터 삭제"
  alt="New Relic Logs - 아키텍처 - 필터 삭제"
  src={ingestPipeline}
/>

<figcaption>
  수집 프로세스 동안 고객 로그 데이터는 New Relic의 데이터베이스에 저장되기 전에 구문 분석, 변환 또는 삭제될 수 있습니다.
</figcaption>
...
새 삭제 필터 규칙을 생성하기 위해 신규 또는 기존 [로그 쿼리](/docs/logs/new-relic-logs/ui-data/query-syntax-logs) 를 사용할 수 있습니다.
...
4. **권장 사항:** 삭제 규칙의 기본 이름을 의미 있는 이름으로 변경하십시오.
5. 쿼리와 일치 [하는 전체 로그 이벤트를 삭제](#drop-events) 하거나 일치하는 이벤트에서 [속성의 특정 하위 집합](#drop-attributes) 만 삭제하도록 선택합니다.
6. 이 삭제 규칙이 적용되는 로그 파티션을 검토하십시오.
7. 드롭 필터 규칙을 저장합니다.

## NerdGraph API를 통해 드롭 필터 규칙 관리 [#nerdgraph]

드롭 필터 규칙을 프로그래밍 방식으로 관리하려면 [NerdGraph](/docs/accounts/accounts/data-management/drop-data-using-nerdgraph) 를 사용하여 드롭 필터 규칙을 생성, 쿼리 및 삭제할 수 있습니다.

NerdGraph를 사용하여 로그 이벤트에 대한 삭제 필터 규칙을 생성하는 경우 `source` 속성을 `Logging` 으로 설정하십시오.

로깅 UI는 `source` 속성이 있는 드롭 필터만 표시합니다.
...
3. **삭제** 를 클릭
   ...
   제거하려는 드롭 필터 규칙 옆에 있는 아이콘.

삭제된 규칙은 더 이상 수집된 로그 이벤트를 필터링하지 않습니다.
