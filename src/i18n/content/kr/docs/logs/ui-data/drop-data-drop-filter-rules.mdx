...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > 로그** 로 이동합니다.
...
NerdGraph를 사용하여 로그 이벤트에 대한 삭제 필터 규칙을 생성하는 경우 `source` 속성을 `Logging` 로 설정합니다. 로그 UI는 `source` 속성이 있는 드롭 필터만 표시합니다.
