...
프로그래밍 방식으로 데이터 파티션을 관리하려면 NerdGraph API 탐색기( **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**)를 사용할 수 있습니다. [NerdGraph 데이터 파티션 자습서는](/docs/apis/nerdgraph/examples/nerdgraph-data-partition-rules-tutorial/) 이 API를 사용하여 데이터 파티션을 쿼리, 생성 및 삭제하는 방법을 보여줍니다.
...
<figcaption>
  로그 쿼리 표시줄 왼쪽에서 **Data partitions** \[데이터 파티션을]클릭한 다음 보존 네임스페이스, 선택적 설명 및 일치 기준을 사용하여 파티션 이름을 생성합니다.
</figcaption>

새 파티션 규칙을 생성하려면:

1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Logs** 로 이동합니다.
2. 로그 쿼리 표시줄 왼쪽에서 **Partition** \[파티션 을] 클릭한 다음 **Create new** \[새로 만들기 를]클릭합니다.
...
<figcaption>
  파티션을 보려면 **Partition**\[파티션] 드롭다운을 클릭합니다.
</figcaption>
...
파티션 검색: 로그 쿼리 표시줄 왼쪽에서 **Partition** \[파티션 을]클릭하고 파티션 검색 표시줄을 사용합니다.