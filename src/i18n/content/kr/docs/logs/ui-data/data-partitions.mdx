...
파티션의 네임스페이스는 보존 기간을 결정합니다. 세 가지 보존 옵션을 제공합니다:
...
* <DNT>**Direct to live archive:**</DNT> 표준 보존을 완전히 건너뜁니다. 이 네임스페이스를 사용하여 파티션으로 전송된 로그는 먼저 표준 스토리지에 저장되는 대신 장기 라이브 아카이브 스토리지로 바로 이동합니다. 이 네임스페이스는 매일 볼 계획이 없는 데이터에 대해 표준 티어 스토리지 비용을 지불하지 않으므로, 규정 준수 또는 감사 로그와 같이 쿼리할 필요는 거의 없지만 장기간 보존해야 하는 로그에 가장 적합합니다. 이 옵션을 사용하려면 계정에서 라이브 아카이브를 활성화해야 합니다. 자세한 내용은 [라이브 아카이브로 장기 로그 저장](/docs/logs/get-started/live-archives/)을 참조하십시오.
...
## 파티션에 대한 액세스 제어 [#data-access-control]

로그 데이터를 구성하는 것 외에도 파티션은 특정 로그 데이터에 대한 사용자 액세스를 제어하는 기반으로도 사용됩니다. [데이터 액세스 제어를](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control) 사용하면 특정 사용자 그룹에 대한 특정 파티션에 대한 액세스를 제한하거나 허용하는 정책을 만들 수 있습니다.

이 기능은 다음과 같은 경우에 유용합니다.

* 각 팀이 해당 서비스와 관련된 로그만 볼 수 있도록 해야 합니다.
* 민감한 로그 데이터에 대한 액세스를 제한하는 규정 준수 요구 사항이 있습니다.
* 개인 식별 정보(PII)가 포함된 로그에 대한 무단 액세스를 방지하려고 합니다.