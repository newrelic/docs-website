...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        처음 4,094자는 동일한 이름의 `Log` 이벤트 필드에 저장됩니다. 따라서 긴 `message` 값은 처음 4,094자를 `message` 필드에 저장합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        문자열의 다음 128,000 UTF-8 바이트는 이름이 `newrelic.ext.` 인 `blob` 필드에 저장됩니다. 추가. 따라서 긴 `message` 값은 `newrelic.ext.message` 필드에 `blob` 으로 저장된 처음 4,094자를 초과하는 문자를 갖습니다.
...
<Callout variant="tip">
  문자열 속성의 처음 4,094자를 검색할 수 있습니다. 처음 4,094자에 대한 알림을 생성할 수도 있습니다. 그러나 `blob` Storage는 검색할 수 없으므로 처음 4,094자를 초과하는 텍스트는 검색하거나 경고할 수 없습니다.
</Callout>
...
Blob 데이터를 확장하려면 `message` 또는 기타 특성을 사용하여 다음 쿼리를 실행합니다. blob의 속성을 백틱으로 묶어야 합니다. 예를 들어:
...
NRDB는 한 달 동안 Blob 레코드를 유지합니다. `LogExtendedRecord` 로 저장된 기존의 긴 로그 메시지가 있는 경우 해당 데이터도 NRDB에서 한 달 동안 계속 사용할 수 있습니다.
