...
[데이터를 쿼리하는](/docs/using-new-relic/data/understand-data/query-new-relic-data) 방법에는 여러 가지가 있습니다. 이 문서에서는 [NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) 쿼리의 몇 가지 예를 보여줍니다. <InlinePopover type="mobile"/>데이터. 모바일에서 보고된 이벤트 및 사용 가능한 속성에 대한 설명을 보려면 [모바일 이벤트를](/docs/insights/insights-data-sources/default-data/mobile-default-event-attributes-insights) 참조하세요.
