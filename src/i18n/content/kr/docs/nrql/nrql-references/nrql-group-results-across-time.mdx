...
선택적 `format` 시범이 있는 함수의 경우 허용되는 값은 `string` 및 `numeric` 입니다. 생략되면 기본 형식 값은 `string` 이 됩니다.
...
<CollapserGroup>
  <Collapser id="facet-time-example" title="월별 결과 그룹화">
  ...
  <Collapser id="facet-other-examples" title="FACET 절이 있는 기타 그룹화 예">
  ...
  <Collapser id="timeseries-chart-examples" title="TIMESERIES를 지정하여 차트를 생성하는 예">
