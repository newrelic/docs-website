...
이 예에서는 인프라 데이터를 트랜잭션 데이터와 결합합니다.
...
위 이미지에는 두 개의 데이터세트, 즉 인프라 컨테이너의 평균 CPU 비율(`ProcessSample`)과 컨테이너별 앱 트랜잭션의 평균 기간이 포함되어 있습니다. 종종 다양한 소스의 데이터가 상호 연관되어 있습니다. 이 경우 다음 하위 쿼리 조인을 사용하여 컨테이너의 높은 CPU 사용량으로 인해 트랜잭션 속도가 느려지는지 확인할 수 있습니다.
