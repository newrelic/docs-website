...
* 유입경로가 256개 미만의 유입경로 속성 값을 처리하는 경우 값을 정확하게 계산합니다.
* 256개 이상의 퍼널 속성 값을 처리하는 경우 MinHash라는 알고리즘을 적용하여 최적화된 성능을 위한 대략적인 결과를 계산합니다.
...
이 예에서는 다음을 쿼리합니다. <InlinePopover type="browser"/>[`PageView`](/attribute-dictionary/?event=PageView) 이벤트 및 해당 속성. 제품 탐색부터 장바구니 추가, 결제까지 진행된 고유한 브라우저 세션을 쿼리합니다. 키워드 `AS` 로 표시된 각 단계에 대한 레이블이 포함됩니다.
