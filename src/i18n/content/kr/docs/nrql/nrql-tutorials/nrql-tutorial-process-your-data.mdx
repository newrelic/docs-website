...
공지 이벤트의 특정 날짜에 대한 데이터를 집중적으로 분석하고, 현재 시간을 기준으로 시간 범위를 변경하지 않고 고정된 기간의 데이터를 얻고자 할 때 이 기능을 사용하면 유용합니다. 다음이자 마지막 수업에서는 시간 기반 코호팅을 통해 보고서 작성에 시간을 활용하는 방법을 계속해서 배우게 됩니다.
