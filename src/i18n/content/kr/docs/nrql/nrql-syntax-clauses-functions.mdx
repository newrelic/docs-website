...
<CollapserGroup>
  ...
  >
    ...
    `PREDICT` 절을 사용하면 이력 데이터를 기반으로 미래 데이터 동향에 대한 예측을 선형 차트에 추가할 수 있습니다. 이 절을 사용할 때 다음 사항을 고려해야 합니다.
    ...
    * 퍼블릭 프리뷰에서는 [메트릭 타임슬라이스 데이터](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data)를 지원하지 않습니다.
    ...
    훈련 모델의 하이퍼파라미터를 수동으로 설정하고 `USING` 및 `BY` 키워드를 추가하면 예측을 맞춤화할 수 있습니다. NRQL과 구문에 대해 자세히 알아보려면 [NRQL 문서](/docs/nrql/get-started/introduction-nrql-new-relics-query-language)를 참조하십시오.

    <CollapserGroup>
      <Collapser id="predict-defaults" title="기본값">
        쿼리의 PREDICT 절에는 다음과 같은 기본 동작이 적용됩니다.

        계절성(Seasonality): 이력 데이터에 계절성이 있는지 자동으로 감지합니다. 계절성이 감지되면 식별된 계절 길이가 Holt-Winters 계절 알고리즘에 사용됩니다. 계절성이 발견되지 않으면 비계절 모델을 구성합니다.

        하이퍼파라미터(Hyperparameters): 계절성과 이력 데이터를 기반으로 Holt-Winters 알고리즘에 대한 하이퍼파라미터가 설정됩니다.

        예측 범위: 쿼리 범위에 지정된 총 시간 범위의 20%에 해당하는 범위가 예측됩니다.

        훈련용 이력 데이터: 마지막 세 개 쿼리 범위(현재 범위와 그 앞 두 범위)에 해당하는 이력 데이터를 사용합니다. 그러나 쿼리 범위 보다 큰 `seasonality` 하이퍼파라미터가 제공되는 경우, 모델은 시즈널리티의 3배에 해당하는 범위에 해당하는 이력 데이터를 대신 사용합니다.

        시간 간격: 데이터 예측의 일관성을 보장하기 위해 쿼리 범위에서 시계열의 데이터 포인트 간격에 맞게 정렬됩니다.
      </Collapser>

      <Collapser id="hyperparameters" title="계절성과 Holt-Winters 모델 하이퍼파라미터를 설정합니다.">
        현재 <DNT>**NRQL predictions**</DNT>는 계절 및 비계절 시계열 모두에 대해 지수 평활이라고도 불리는 <DNT>**[Holt-Winters](https://en.wikipedia.org/wiki/Exponential_smoothing#Triple_exponential_smoothing_\(Holt_Winters\))**</DNT> 알고리즘만 지원합니다. 이는 전망 및 예측 작업에 일반적으로 사용되는 표준 알고리즘입니다. 뉴렐릭은 Holt-Winters 모델에 대해 시간별, 일별, 주별 계절 길이를 지원합니다. Holt-Winters 알고리즘의 다른 하이퍼파라미터는 액세스하여 변경할 수 있지만 기본값은 지표의 이력 데이터를 기반으로 결정됩니다. 쿼리의 `PREDICT` 절 뒤에 `holtwinters(<hyperparameter>: <value>)`를 추가하여 하이퍼파라미터를 수정할 수 있습니다. 다음과 같은 파라미터의 조합을 추가하여 예측 결과를 수정할 수 있습니다.

        * `seasonality` 알려진 계절 길이, 자동 계절성 감지 또는 계절성 없음(`<season length>`, `AUTO`, `NONE`)을 사용할지 여부를 지정합니다.
          * `<season length>` 시간의 정수(`integer unit`)여야 합니다(예: `1 day` 또는 `3 hours`)
        * `alpha` 더 큰 값이 최근 값에 더 큰 가중치를 부여하는 수준 평활 인수(`[0, 1.0]`)
        * `beta` 더 큰 값이 최근 값에 더 큰 가중치를 부여하는 추세 평활 인수(`[0, 1.0]`)
        * `gamma` 더 큰 값이 최근 값에 더 큰 가중치를 부여하는 계절 평활 인수(`[0, 1.0]`)
          * 비계절 모델은 `gamma` 하이퍼파라미터를 사용하지 않으므로 `seasonality: NONE` 옵션과 함께 사용할 수 없습니다. 지정하면 쿼리가 오류를 반환합니다. `holtwinters(seasonality: NONE, gamma: <value>)`
        * `phi` 추세 완화 파라미터(`[0.98, 1.0]`)로, 값이 작을수록 추세가 더 많이 완화되어 예측이 평준화됩니다.
      ...
      >
        `BY` 키워드를 사용하여 모델이 예측할 시간을 설정합니다. 예를 들어, `PREDICT BY 3 hours`는 시계열의 최신 지점으로부터 3시간 후의 예측을 생성합니다. 시간 범위는 정수(`integer unit`)여야 합니다. 예측할 수 있는 최대 기간 수는 360개입니다(즉, 시계열 기간 크기에 360을 곱한 값).

        `BY` 키워드를 사용하여 앞으로 30분에 대한 예측을 생성하는 쿼리 예시:
      ...
      >
        `USING` 키워드를 사용하여 모델을 학습하는 데 사용할 역량 데이터의 양을 지정합니다. 예를 들어, `PREDICT USING 1 day`는 이전 날의 데이터와 쿼리 기간을 사용해 모델을 훈련시킵니다. 시간 범위는 정수(`integer unit`)여야 합니다. 모델을 훈련시키는 데 사용할 수 있는 데이터 양에는 제한이 있으며, 한도를 초과하는 양을 지정하면 오류 메시지가 표시됩니다. 한도는 3,660개의 데이터 포인트이며, 시계열 기간을 사용하면 그 시간을 파악할 수 있습니다. 예를 들어, 시계열 기간 1분으로 쿼리하는 경우 최대 3,660분, 즉 61시간 또는 약 2.5일의 훈련 데이터 양만 지정할 수 있습니다.

        2일간의 이력 데이터를 사용하며 `USING` 키워드가 포함된 쿼리 예시:
    ...
    `BY` 및 `USING` 키워드를 추가하며 커스텀 하이퍼파라미터를 지정하는 `PREDICT` 절이 있는 쿼리 예시:
    ...
    이 쿼리는 계절성을 자동 감지로 설정하고, 레벨 평활 인수를 조정하여 이력 데이터를 강조합니다. 이는 1시간 뒤의 미래를 예측하고 이전 2시간의 이력 데이터와 쿼리 기간 길이를 사용하여 모델을 훈련시킵니다.
