...
<CollapserGroup>
  ...
  >
    <Callout title="미리보기">
      이 기능은 아직 개발 단계에 있지만, 시험적으로 사용해 보시길 권합니다.

      이 기능은 현재 [출시 전 정책](/docs/licenses/license-information/referenced-policies/new-relic-pre-release-policy)에 따라 프리뷰 프로그램의 일부로 제공됩니다.
    </Callout>
    ...
    `PREDICT` 절을 사용하면 미래 데이터 동향에 대한 예측을 이력 데이터 기반의 선형 차트에 통합할 수 있습니다. 이 절은

    * `TIMESERIES` 절이 있는 쿼리에만 적용됩니다.
    * 예측 데이터 포인트의 간격으로 `TIMESERIES <time period>`를 사용합니다.
    * 예측 범위는 기본적으로 쿼리 기간의 20%로 설정됩니다.
    * 기본적으로 현재 쿼리 기간과 앞의 두 쿼리 기간의 이력 데이터를 처리하여 예측을 생성합니다.
    * 퍼블릭 프리뷰에서는 [메트릭 타임슬라이스 데이터](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data)를 지원하지 않습니다.
    ...
    <Callout variant="important">
      [`COMPARE WITH`](/docs/nrql/nrql-syntax-clauses-functions/#sel-compare) 절이나 [하위](/docs/nrql/using-nrql/nested-aggregation-make-ordered-computations-single-query) [쿼리](/docs/nrql/using-nrql/subqueries-in-nrql/) 내부의 중첩 집계와 함께 사용되는 경우 `PREDICT` 절은 지원되지 않습니다.
    </Callout>

    또한 `PREDICT <model>`, `BY <time range>`, `USING <time range>`를 추가하여 예측 추세를 특정 요구 사항에 맞게 세분화할 수 있습니다. 시간 범위와 이력 데이터 스팬을 나타내려면 `integer units`를 사용합니다.
