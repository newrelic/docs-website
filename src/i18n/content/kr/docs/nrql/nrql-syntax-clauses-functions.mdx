...
<CollapserGroup>
  ...
  >
    ...
    * 퍼블릭 프리뷰에서는 [메트릭 타임슬라이스 데이터](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data)를 지원하지 않습니다.
    ...
    `PREDICT` 절의 기본 동작에 대해 알아보려면 [NRQL 예측](/docs/query-your-data/explore-query-data/use-charts/nrql-predictions/#defaults)을 참조하십시오. 학습 모델의 하이퍼파라미터를 수동으로 설정하고 `USING` 및 `BY` 키워드를 추가함으로써 예측을 맞춤화할 수 있습니다. NRQL과 구문에 대해 자세히 알아보려면 [NRQL 문서](/docs/nrql/get-started/introduction-nrql-new-relics-query-language)를 참조하십시오.
