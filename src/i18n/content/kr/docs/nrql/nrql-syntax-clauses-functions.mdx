...
<CollapserGroup>
  ...
  >
    ...
    * 퍼블릭 프리뷰에서는 [메트릭 타임슬라이스 데이터](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data)를 지원하지 않습니다.
    ...
    예측 모델의 기본 동작에 대해 알아보려면 [NRQL 예측](/docs/query-your-data/explore-query-data/use-charts/nrql-predictions/#defaults)을 참조하십시오. 훈련 모델의 하이퍼파라미터를 수동으로 설정하고 `USING` 및 `BY` 키워드를 추가하면 예측을 맞춤화할 수 있습니다.

    <CollapserGroup>
      <Collapser id="hyperparameters" title="모델 하이퍼파라미터를 설정합니다.">
        현재, <DNT>**Holt-Winters**</DNT>가 지원되는 유일한 예측 알고리즘이며, 이 알고리즘에는 예측을 미세하게 조정할 수 있는 여러 가지 하이퍼파라미터가 포함되어 있습니다. 다음 표를 참조하여 각 하이퍼파라미터의 동작을 이해하고 사용 사례에 맞는 하이퍼파라미터를 선택합니다.

        쿼리를 수정하려면 `PREDICT` 절 뒤에 `holtwinters(<hyperparameters>: <value>)`를 추가합니다.
        ...
        <table id="hyperparameters">
          ...
          <tbody>
            <tr>
              ...
              <td>
                시계열의 계절성을 특정 시즌 길이로 설정합니다. 시즌 길이는 정수(`integer unit`)의 시간이어야 합니다.\
                **예**: `1 day` 또는 `3 hours`
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                시계열의 계절성을 자동으로 감지하고 식별된 계절성을 사용하여 예측을 생성합니다.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                시계열의 계절성을 비활성화하고 비계절적인 예측을 생성합니다.

                <Callout variant="important">
                  `seasonality: NONE` `gamma` 하이퍼파라미터와 함께 사용될 수 없습니다. 이 하이퍼파라미터는 시즌 평활화 요소를 설정합니다.
                </Callout>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                ...
                * **값이 낮을수록**: 예측 곡선에 더 많은 감쇠 효과가 나타나 점차 평평한 곡선이 됩니다.
      ...
      >
        `BY` 키워드를 사용하여 모델이 예측할 시간을 설정합니다. 예를 들어, `PREDICT BY 3 hours`는 시계열의 최신 지점으로부터 3시간 후의 예측을 생성합니다. 시간 범위는 정수(`integer unit`)여야 합니다.
      </Collapser>
      ...
      >
        `USING` 키워드를 사용하여 모델을 훈련하는 데 사용할 이력 데이터의 양을 지정합니다. 예를 들어, `PREDICT USING 1 day`는 이전 날의 데이터와 쿼리 기간을 사용해 모델을 훈련시킵니다. 시간 범위는 정수(`integer unit`)여야 합니다.
      </Collapser>
    </CollapserGroup>

    `PREDICT`와 하이퍼파라미터, `BY` 및 `USING` 키워드를 함께 사용하는 예:
