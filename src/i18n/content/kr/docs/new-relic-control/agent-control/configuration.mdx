...
에이전트 컨트롤은 전용 시크릿 제공자로부터 비밀번호 및 API 키와 같은 민감한 데이터를 가져와 관리하는 강력한 메커니즘을 제공합니다. 이를 통해 민감한 정보가 설정 파일에 직접 하드코딩되지 않도록 보장합니다. 시스템은 현재 [값 제공자](#value-providers)에 설명된 제공자를 지원합니다.
...
Windows 에서 에이전트 Control 설정의 기본 경로는 다음과 같습니다.

* `Local` 설정 파일: `C:\Program Files\New Relic\newrelic-agent-control\local-data\agent-control\local_config.yaml`
* `Remote` 설정 파일: `C:\ProgramData\New Relic\newrelic-agent-control\fleet-data\agent-control\remote_config.yaml` (플릭스 위험에서 동기화됨)
* 서비스 실행 파일: `C:\Program Files\New Relic\newrelic-agent-control\newrelic-agent-control.exe`
* 로그 디렉토리: `C:\ProgramData\New Relic\newrelic-agent-control\logs\`

기본적으로 Windows의 에이전트 제어는 인프라 에이전트와 OpenTelemetry 수집기를 오케스트레이션할 수 있습니다.
...
<Callout variant="tip">
  리전 계정의 경우 적절한 엔드포인트와 `token_url`을(를) 사용하세요:

  * EU 지역: `https://opamp.service.eu.newrelic.com/v1/opamp` 및 `https://system-identity-oauth.service.eu.newrelic.com/oauth2/token`
  * JP 지역: `https://opamp.service.jp.newrelic.com/v1/opamp` 및 `https://system-identity-oauth.service.jp.newrelic.com/oauth2/token`
</Callout>
...
* 파일 경로는 백슬래시(`\\`)와 Windows 스타일 경로(예: `C:\\ProgramData\\...`)를 사용합니다.
* 에이전트 바이너리는 `.exe` 확장자를 사용합니다.
* 서비스는 `LocalSystem` 으로 실행됩니다. 설정 파일의 쓰기 권한이 제한되어 있는지 확인하십시오.
* Linux 환경의 템플릿 및 설정은 Windows 호환성을 위해 경로 조정이 필요할 수 있습니다.

### 에이전트 구성 [#windows-agents-config]

Windows 의 Agent Control은 현재 다음 에이전트 유형을 관리할 수 있습니다.

* 뉴렐릭 인프라 에이전트: `newrelic/com.newrelic.infrastructure`. 내장된 Fluent Bit 통해 [온호스트 통합](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations/) 및 로그 포워딩을 지원합니다.
* OpenTelemetry 용 뉴렐릭 배포: `newrelic/com.newrelic.opentelemetry.collector`

에이전트 로컬 설정을 사용자 정의하려면 에이전트의 값 디렉터리에 `values.yml` 파일을 배치하세요. 사용 가능한 변수는 [Linux 호스트](#agents-config) 의 경우와 동일합니다.
...
<CollapserGroup>
  <Collapser id="windows-remote-config-infra" title="로그 포워딩을 갖춘 에이전트 에이전트">
    Windows 로그 포워딩을 사용한 에이전트 에이전트 설정. 유효한 `license_key` 있는 `config_agent` 은 항상 필수입니다.
  ...
  <Collapser id="windows-remote-config-agent-control" title="에이전트 Control 설정">
    에이전트 관리 방법을 보여주는 Windows 의 에이전트 제어 자체에 대한 유효한 설정:
...
값 공급자를 사용하면 설정이 렌더링될 때 에이전트 컨트롤이 외부 소스에서 값을 해석할 수 있으므로 라이선스 키, 토큰 또는 비밀번호와 같은 민감한 데이터를 설정 파일에 하드코딩할 필요가 없습니다.

값은 에이전트 설정 내에서 `${NAMESPACE:NAME}` 플레이스홀더 구문을 사용하여 참조됩니다. 에이전트 컨트롤은 원격 설정 업데이트를 포함하여 설정이 적용될 때마다 이를 해석하므로, 에이전트 컨트롤을 다시 시작하지 않고도 교체된 값이 반영됩니다.

<Callout variant="important">
  에이전트 컨트롤이 값을 해석하지 못하면 설정 렌더링이 실패하고 에이전트가 시작되지 않습니다. 이는 에이전트가 불완전하거나 잘못된 설정으로 실행되는 것을 방지합니다.
</Callout>

성공적으로 해석된 후, 에이전트 컨트롤은 해당 에이전트가 사용할 수 있도록 렌더링된 결과를 Kubernetes 시크릿 또는 비공개 설정 파일에 저장합니다.
...
HashiCorp Vault만 명시적인 설정이 필요합니다. 에이전트 제어 설정의 `value_providers` 키 아래에 정의하십시오. 구형 키 `secrets_providers`은(는) 여전히 별칭으로 허용됩니다.
...
`sources` 아래의 각 항목은 다음 설정을 사용하여 명명된 Vault 소스를 정의합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Vault requests에 대한 HTTP 타임아웃입니다. 기본값은 `30s`입니다. 소스별이 아닌 `vault` 수준에서 설정합니다.
      </td>
    </tr>
  </tbody>
</table>
...
호스트 환경에서 값을 읽습니다(에이전트 컨트롤 프로세스에서 사용할 수 있음).
...
Vault KV 시크릿에서 키를 읽습니다. 위에서 설명한 `value_providers.vault` 설정이 필요합니다.
...
* **`SOURCE_NAME`**: `value_providers`에 정의된 Vault 소스의 이름입니다.
* **`MOUNT`**: 시크릿 엔진 마운트의 이름입니다.
* **`PATH`**: 시크릿에 대한 경로입니다.
* **`KEY`**: 검색할 비밀 내의 키입니다.
...
여기에서 `${nr-vault:local-instance:secret:my_secret:username}`은(는) `local-instance` 소스를 사용하여 `secret/my_secret`에 있는 시크릿에서 `username` 키의 값을 가져오고, `${nr-vault:remote:my_mount:my_path:organization}`은(는) `remote` 소스에서 `organization` 키를 가져옵니다.

### 로컬 파일: `nr-file`

로컬 파일의 내용을 읽고 앞뒤 공백을 제거합니다.

플레이스홀더는 파일의 절대 경로를 사용합니다:
...
### Kubernetes 시크릿: `nr-kubesec`

Kubernetes 시크릿에서 키를 읽습니다. Kubernetes에서만 사용할 수 있으며, 에이전트 컨트롤 파드가 서비스 계정 및 RBAC를 통해 필요한 시크릿 및 네임스페이스에 액세스할 수 있는 권한이 있는 한 추가 설정이 필요하지 않습니다.
...
* **`NAMESPACE`**: 비밀이 있는 네임스페이스입니다.
* **`SECRET_NAME`**: Kubernetes 시크릿 객체의 이름입니다.
* **`KEY`**: 검색할 비밀 내의 키입니다.

### Kubernetes ConfigMap: `nr-kubecm`

Kubernetes ConfigMap에서 키를 읽습니다. Kubernetes에서만 사용할 수 있으며, 에이전트 컨트롤 파드가 필요한 ConfigMap 및 네임스페이스에 액세스할 수 있는 권한이 있는 한 추가 설정이 필요하지 않습니다.
...
* **`NAMESPACE`**: ConfigMap이 있는 네임스페이스입니다.
* **`CONFIGMAP_NAME`**: Kubernetes ConfigMap 객체의 이름입니다.
* **`KEY`**: 검색할 ConfigMap 내의 키입니다.