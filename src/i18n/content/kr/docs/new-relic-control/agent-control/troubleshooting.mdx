...
<CollapserGroup>
  ...
  <Collapser id="k8s-helm-failure" title="Helm 릴리스 실패">
    ...
    Agent Control이 플릿위험에 안전하게 연결하려면 유효한 인증 자격 증명이 필요합니다. 처음에 이 자격 증명은 에이전트 Control 설치 UI 통해 자동 생성되며 값 파일의 `systemIdentity.parentIdentity.clientId` 및 `systemIdentity.parentIdentity.clientSecret` 필드로 표시됩니다. 보안상의 이유로 에이전트 Control 설치에 필요한 인증정보는 12시간 후 만료됩니다.
...
<CollapserGroup>
  ...
  <Collapser id="debug" title="에이전트 로깅 관련 문제 진단">
    ...
    * `error`
    ...
    2. 에이전트 제어를 다시 시작하십시오:

    ```bash
      sudo systemctl restart newrelic-agent-control
    ```

    3. `file` 로그가 활성화된 경우, `path` 설정의 디렉터리에서 가장 최근 로그 파일을 확인하십시오(파일에는 날짜가 지정되어 있습니다. 예: `2026-09-18.newrelic-agent-control.log`). 또는 `journalctl -u newrelic-agent-control` 등 선호하는 로그 문제 진단, 해결 도구를 사용하십시오.
  </Collapser>
...
가장 최근의 로그 파일을 확인하십시오(로그 파일은 `2026-02-09.newrelic-agent-control.log`과(와) 같이 날짜 접두사로 이름이 지정됩니다):
...
서비스 관련 문제를 우회하려면 명령줄에서 에이전트 제어를 실행하십시오. 기본 로컬 설정 경로(`local-data\agent-control\local_config.yaml`)에서 설정을 자동으로 읽으므로 플래그가 필요하지 않습니다:
...
## NRDiag를 사용하여 호스트의 에이전트 제어 문제 해결

뉴렐릭 진단 도구 `NRDiag`은(는) 디버깅을 위해 호스트의 에이전트 제어와 관련된 리소스 및 로그를 수집하는 유틸리티입니다. 이는 Linux 및 Windows 호스트 모두에 적용되며, 해당 제품군은 OS를 자동으로 감지하고 관련 검사를 실행합니다.

모든 데이터를 수집하려면 다음 단계를 따르십시오:

1. 호스트에서 [시작 가이드를](/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started) 사용하여 `NRDiag` 도구를 설치합니다.

2. 온호스트 에이전트 제어 제품군을 실행하십시오:
...
3. 예상되는 출력 보고서는 다음과 같습니다.
   ...
   <Callout variant="tip" title="팁">
     Windows에서 `AgentControl/Agent/Logs` 작업은 `journalctl` 대신 `C:\ProgramData\New Relic\newrelic-agent-control\log\agent-control\`에서 직접 로그 파일을 수집합니다.
   </Callout>
