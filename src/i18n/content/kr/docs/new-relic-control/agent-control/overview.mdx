...
* **자격 증명:** 에이전트 컨트롤은 Vault와 같은 외부 값 제공자로부터 민감한 자격 증명을 가져와 런타임에 에이전트의 설정에 주입할 수 있습니다. 이는 설정 자체에 자격 증명을 보관하지 않음으로써 보안을 강화합니다.
...
* **보안 태세:** 값 제공자로부터 자격 증명을 가져오는 방법을 표준화하여 처리를 중앙 집중화하고, 교체를 자동화하며, 노출을 줄입니다.
...
Agent Control에서 관리하는 모든 에이전트는 Agent Control이 해당 에이전트를 다운로드, 설정 및 실행하는 방법을 설명하는 버전이 지정된 사양인 **에이전트 유형**을 기반으로 합니다. 새로운 에이전트를 지원하려면 항상 에이전트 유형 정의가 필요하며, 정의가 없으면 Agent Control은 이를 관리하는 방법을 알 수 없습니다.

에이전트 유형은 정규화된 이름과 버전으로 식별됩니다(예: `newrelic/com.newrelic.infrastructure:0.1.0`). 설정에 하위 에이전트를 추가할 때 에이전트 유형을 참조하며, Agent Control은 해당 정의를 사용하여 올바른 아티팩트를 가져오고, 설정 기본값을 적용하고, 에이전트의 수명 주기를 관리합니다.

사용 가능한 에이전트 유형의 전체 목록은 [에이전트 유형](/docs/new-relic-control/agent-control/agent-types#agent-types)을 참조하십시오.
...
각 에이전트에 필요한 특정 권한에 대한 자세한 내용은 [에이전트 유형별 필수 권한](/docs/new-relic-control/agent-control/agent-types#required-permissions-per-agent-type)을 참조하십시오.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        에이전트 컨트롤은 제조업체의 수명 종료 시점까지 다음 운영 시스템을 지원합니다:
