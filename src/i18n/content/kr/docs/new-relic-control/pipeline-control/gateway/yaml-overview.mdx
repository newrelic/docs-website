...
<Callout variant="tip">
  **설치 모드 참고:** `version` 및 `autoscaling` 필드의 동작은 플릿의 설치 모드에 따라 달라집니다:

  * **Flux 사용 시:** 이러한 필드는 UI를 통해 완전 관리형으로 관리됩니다. 변경 사항은 플릿의 모든 클러스터에 자동으로 배포됩니다.
  * **Flux 미사용 시:** 이 필드는 초기 설치 중에 수평 파드 오토스케일러(HPA)를 구성하도록 설정할 수 있습니다. 하지만, 설치 후 UI를 통한 변경 사항은 무시됩니다. `configuration.simplified/v2` 섹션(파이프라인 규칙)만 `ConfigMap`을(를) 통해 계속해서 자동으로 배포됩니다. HPA를 통한 스케일링과 Helm 업그레이드를 통한 버전은 수동으로 업데이트해야 합니다. 자세한 내용은 [Flux 없이 게이트웨이 관리](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway-after-installation) 를 참조하세요.
</Callout>

### 최상위 구조

* `version`: 설정 형식 버전 (현재 `"2.0.1"`)
* `autoscaling`: 게이트웨이 복제본 확장 설정
* `configuration.simplified/v2`텔레메트리 파이프라인 정의를 위한 간소화된 추상화 계층
* `troubleshooting`디버그 설정

### 선택적 재정의

* `nrExporterEndpoints`: 파이프라인별로 기본 뉴렐릭 수집 엔드포인트를 재정의합니다.
...
게이트웨이 설정은 방향성 비순환 그래프(DAG) 구조를 따르며, 여기서 각 단계는 동작을 정의하고 출력 필드를 사용하여 파이프라인의 다음 단계를 가리킵니다. 이는 명시적인 데이터 흐름을 생성합니다: 데이터는 리시버를 통해 들어오고, 프로세서(변환, 필터, 샘플)를 통해 흐르며, 익스포터를 통해 나갑니다.
...
| configuration.simplified/v2               | 물체 | 네   | -   |
| 문제 해결, 해결.proxy                           | 부울 | 아니요 | 잘못된 |
| 문제 해결, 해결.requestTraceLogs                | 부울 | 아니요 | 잘못된 |
| nrExporterEndpoints.nrHost                | 끈  | 아니요 | -   |
| nrExporterEndpoints.metricsEndpoint       | 끈  | 아니요 | -   |
| nrExporterEndpoints.tracesEndpoint        | 끈  | 아니요 | -   |
| nrExporterEndpoints.logApiEndpoint        | 끈  | 아니요 | -   |
| nrExporterEndpoints.eventApiEndpoint      | 끈  | 아니요 | -   |
| nrExporterEndpoints.infraEventApiEndpoint | 끈  | 아니요 | -   |
| nrExporterEndpoints.otlpEndpoint          | 끈  | 아니요 | -   |

각 `nrExporterEndpoints` 필드는 독립적입니다: 재정의해야 하는 필드(예: `nrHost`만)만 설정하십시오. 생략한 필드는 계정의 기본 뉴렐릭 수집 엔드포인트로 대체됩니다.

`nrExporterEndpoints` 필드는 두 개의 독립적인 재정의 그룹으로 나뉩니다:

* `nrHost`, `metricsEndpoint`, `tracesEndpoint`, `logApiEndpoint`, `eventApiEndpoint` 및 `infraEventApiEndpoint`는 뉴렐릭 전용(구형 에이전트) 텔레메트리의 대상을 재정의합니다.
* `otlpEndpoint` OTLP 네이티브 텔레메트리의 대상을 별도로 재정의합니다. 한 그룹에서 필드를 설정해도 다른 그룹에는 영향을 미치지 않습니다.

`otlpEndpoint` 포트가 포함되지 않은 기본 호스트 및 체계(예: `https://otlp.example.com`)여야 합니다. 포트를 추가하면 설정이 유효성 검사에 실패하여 저장되지 않습니다.
