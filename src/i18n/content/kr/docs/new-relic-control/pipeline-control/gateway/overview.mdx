...
* 버전 선언: version: `2.0.1` 설정 스키마를 지정합니다.
...
2. UI(YAML 업로드)를 통해 게이트웨이 플릿에 배포합니다.
3. 게이트웨이는 플릿 내의 모든 게이트웨이 클러스터에 설정을 적용합니다.
4. 플릿 내 모든 클러스터는 동일한 규칙을 사용하여 데이터를 동일하게 처리합니다.

**버전 관리:**

* 각 설정 변경은 새로운 버전(플릿별)을 생성합니다.
* 버전 기록을 확인하고 필요한 경우 롤백하세요.
* &quot;배포 필요&quot; 배지는 보류 중인 변경 사항을 표시합니다.
* 서로 다른 플릿은 각기 다른 설정과 버전을 유지할 수 있습니다.

## 다음 단계

**게이트웨이 설치 시작하기:**

* [Flux로 설치](/docs/new-relic-control/pipeline-control/gateway/set-up-and-deploy-pipeline-control) - UI를 통한 전체 게이트웨이 수명 주기 관리(설치, 업그레이드, 확장)
* [Flux 없이 설치](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux) - 권한 축소를 위한 설정 전용 모드
