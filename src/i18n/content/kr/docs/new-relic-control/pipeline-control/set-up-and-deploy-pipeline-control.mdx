...
**쿼리 함수:** 게이트웨이는 메트릭 쿼리의 WHERE 절에서 특정 함수를 지원하지 않습니다. 해당 기능은 다음과 같습니다.
...
**쿼리 예:** `FROM Metric SELECT * WHERE (%[type]='summary' AND %[count]=0) OR (%[type]='gauge' AND %[count]=1 AND %[latest]=0)`
...
6. 디렉토리에 설정 파일 **values-newrelic-gateway.yaml**을 다운로드합니다.
