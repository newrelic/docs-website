...
중복 알림은 소음 속에서 중요한 알림 이벤트를 묻습니다. 기존 알림 시스템을 평가하는 것은 우선순위 설정 전략의 핵심 부분입니다. 알림 시스템의 품질은 팀이 공지 이벤트에 얼마나 잘 대응하는지에 직결되기 때문입니다. 너무 많은 소음이 발생하면 비즈니스에 거의 또는 전혀 영향을 미치지 않는 우선순위가 낮은 공지 이벤트로 팀이 피로해질 위험이 있습니다. 그러나 공지에 실패한 공지 이벤트는 고객 경험에 영향을 미치는 중단으로 이어집니다.
...
AQM은 팀이 진정한 비즈니스 영향력을 발휘하는 <InlinePopover type="alerts" />에 집중할 수 있도록 성가신 공지 이벤트의 수를 줄이는 데 중점을 둡니다. 다음과 같은 조건을 충족한다면 AQM에 적합한 후보자입니다:
...
1. [<DNT>**Alert Quality Management**</DNT> 인스턴트 옵저버빌리티 페이지로 이동하세요.](https://newrelic.com/instant-observability/alert-quality-management)
2. <DNT>**Install now**</DNT> 버튼을 클릭하세요.
...
* 누적 지속 시간이 수천 분 이상인 &quot;상시 켜짐&quot; 공지 이벤트를 생성합니다.
* 공지사항의 30% 이상이 공개된 시간이 5분 미만인 경우
* 평균 마감 시간이 30분 이상인 경우.
* 매주 350개 이상의 공지이벤트를 생성하세요.
...
<CollapserGroup>
  <Collapser id="constrained-alerts" title="데이터의 특정 세그먼트에 대한 알림">
  ...
  <Collapser id="nth-percentile" title="데이터의 N번째 백분위수에 대한 알림">
  ...
  <Collapser id="max-min-avg" title="데이터가 최대, 최소 또는 평균에 도달하면 경고">
  ...
  <Collapser id="percentage" title="데이터 비율 알림">
  ...
  <Collapser id="apdex" title="T 값이 있는 Apdex에 대한 경고">
...
<DocTiles>
  <DocTile title="품질 경고로 소음 감소" number="3" path="/docs/journey-demand/create-quality-alerts/" label={{text: "You are here", color: "orange"}}>
    알림 품질 관리로 알림 평가
  </DocTile>
