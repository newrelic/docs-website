...
<Steps>
  <Step>
    ...
    Google의 <InlinePopover type="browser"/>도구는 앱 데이터를 모니터링하는 좋은 방법이므로 디지털 환경 개선을 시작하는 가장 좋은 방법은 브라우저 대시보드를 검토하는 것입니다. 먼저, 브라우저 앱과 페이지를 검토하여 뉴렐릭에 보고할 것으로 예상되는 모든 항목이 실제로 보고되고 있는지 확인해야 합니다. 수집 중인 각 앱에 대한 브라우저 모니터링 UI에서 <DoNotTranslate>**Page Views**</DoNotTranslate> 탭을 검토하거나 다음 NRQL 쿼리를 실행하여 이를 수행할 수 있습니다.
    ...
    * 일주일 만에 모바일 브라우저 앱의 페이지 조회수는 700,000회에 달하는데, 데스크톱의 경우 300,000회에 달합니다.
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="지역 및 위치"
      >
        <DoNotTranslate>**Basic:**</DoNotTranslate> 국가별로 그룹화합니다. 브라우저 이벤트에는 요청의 국가 코드가 자동으로 포함되므로 이를 더 자세히 분석하기 위해 수행할 작업이 없습니다.

        <DoNotTranslate>**Advanced:**</DoNotTranslate> 브라우저 모델링에서 [사용자 정의](/docs/telemetry-data-platform/custom-data/custom-events/report-browser-monitoring-custom-events-attributes/) 속성을 사용하여 자체 지역 속성을 생성하여 지역 그룹화가 지역 SLO 그룹과 일치하도록 만듭니다.
      ...
      >
        ...
        * 호텔 예약은 물론 렌터카 서비스도 제공하는 여행사입니다.
        ...
        <DoNotTranslate>**Basic:**</DoNotTranslate> 제품별 성능 분석:
        ...
        * `appName` 및 패싯별로 그룹화: 하나의 제품을 지원하는 브라우저에서 여러 앱을 hsbr하는 경우 이 접근 방식을 사용합니다.
        ...
        <DoNotTranslate>**Advanced:**</DoNotTranslate> [사용자 정의 속성을 사용하여 브라우저 페이지에 사용자 정의 속성](/docs/telemetry-data-platform/custom-data/custom-events/report-browser-monitoring-custom-events-attributes) 으로 제품 제공을 추가합니다.
      </Collapser>
