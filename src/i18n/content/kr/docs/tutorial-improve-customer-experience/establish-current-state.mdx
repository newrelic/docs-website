...
<Steps>
  <Step>
    ...
    우리의 <InlinePopover type="browser"/>도구는 앱 데이터를 모니터링할 수 있는 좋은 방법이므로 디지털 경험 개선을 시작하는 가장 좋은 방법은 브라우저 대시보드를 검토하는 것입니다. 먼저, 브라우저 앱과 페이지를 검토하여 New Relic에 보고할 것으로 예상되는 모든 것이 실제로 보고되고 있는지 확인해야 합니다. 수집 중인 각 앱에 대한 브라우저 모니터링 UI에서 **Page Views** \[페이지 보기] 탭을 검토하거나 다음 NRQL 쿼리를 실행하여 이를 수행할 수 있습니다.
