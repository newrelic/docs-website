...
Pixie로 자동 원격 측정을 시작하고 실행하려면 설치 안내부터 시작합니다. 안내식 설치는 클러스터에 New Relic의 Kubernetes 통합과 함께 Pixie를 배포합니다. Pixie 사용을 시작하기 위해 추가 구성이나 설치를 수행할 필요가 없습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        이미 두 가지 유형의 계정이 있고 각각에 대해 동일한 이메일 주소를 사용한 경우 Pixie UI에서 New Relic 아이콘을 클릭하십시오. 그러면 안내 설치 [의 HELM 명령/매니페스트(yaml) 파일 구성](#install-helm-command-manifest) 섹션으로 이동합니다. 그런 다음 단계를 따릅니다.
    ...
    <tr>
      ...
      <td>
        Pixie UI에서 New Relic에 대한 링크를 따라가고 New Relic 계정이 없는 경우 먼저 계정을 만들어야 합니다. Pixie UI에서 New Relic 아이콘을 클릭하고 단계에 따라 New Relic 계정을 만드세요. 그렇게 하면 Pixie 계정이 연결됩니다. 그런 다음 [다음 단계에 따라 안내 설치 프로세스를](#install-helm-command-manifest) 계속합니다.
      </td>
    </tr>
...
2. 설치 안내에 사용할 계정을 선택하고 **계속** 을 클릭합니다. 참고: 단일 계정이 있는 경우 이 옵션이 표시되지 않습니다.
3. **Kubernetes** 를 선택하고 다음 섹션의 1단계를 계속합니다.

## HELM 명령/매니페스트 구성(yaml) 파일에서 설치 [#install-helm-command-manifest]

Pixie 또는 New Relic 내에서 링크를 따라 안내 설치 프로세스에 도달했다면 여기에서 단계를 시작합니다.

1. 설치를 위한 계정 및 클러스터를 선택하십시오. 필요한 경우 네임스페이스를 선택합니다.

   <Callout variant="important">
     현재 Pixie는 최대 100개의 노드가 있는 클러스터에서 가장 잘 수행됩니다(100개를 초과하는 노드는 과도한 메모리 사용량과 스크립트 실행 실패로 이어질 수 있음). 친숙한 알림: 자동 크기 조정은 노드 수를 빠르게 증가시킬 수 있습니다.
   </Callout>
...
4. **설치 방법 선택** 페이지에서 **Helm** 또는 **매니페스트** 를 선택합니다.
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="투구 방식"
     >
       명령줄에서 제공된 Helm 명령을 실행합니다. 수집할 Pixie 데이터의 양이 걱정된다면 수집 [을 줄이기 위한 전략을](/docs/auto-telemetry-pixie/manage-pixie-data) 확인하십시오.
       ...
       Kubernetes 통합 설치에 대한 일반 정보는 [이 Helm 설치 정보를 참조하십시오](/docs/integrations/kubernetes-integration/installation/install-kubernetes-integration-using-helm/#h2-compatibility-and-requirements) .
     </Collapser>
