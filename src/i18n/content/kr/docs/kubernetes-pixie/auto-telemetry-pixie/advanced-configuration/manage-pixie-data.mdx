...
Kubernetes에서는 nodeSelector, taint/tolerations 및 노드 선호도/반선호도를 사용하여 클러스터 노드의 특정 하위 집합에 포드를 할당할 수 있습니다. 그렇게 하면 모든 노드가 아니라 선택한 노드에 대한 메트릭만 수집합니다. 이는 예를 들어 10개의 클러스터 노드 중 5개에만 Pixie를 사용한 자동 원격 측정을 배포하려는 경우에 유용합니다. 지정된 5개의 노드가 우선 순위가 높은 워크로드를 담당하거나 클러스터에서 Linux 및 Windows 노드를 모두 실행 중이고 Windows 노드가 현재 지원되지 않기 때문에 Linux 노드에만 배포하려고 할 수 있습니다. 이제 안내식 설치 명령에 추가 옵션을 제공하여 노드의 하위 집합에 포드를 할당할 수 있습니다. 이 옵션은 이스케이프된 JSON 문자열을 Pixie 차트가 있는 Auto-telemetry 차트에 전달하여 레이블이 `pixie=allowed` 인 노드에서만 PEM DaemonSet를 예약하는 `nodeSelector` 을 활성화합니다.
