...
New Relic의 UI는 단기 및 장기 저장소에서 Pixie 원격 측정 데이터를 가져옵니다.[여기](/docs/kubernetes-pixie/auto-telemetry-pixie/understand-use-data/overview) 에서 New Relic Pixie 통합을 위한 하이브리드 스토리지 모델에 대해 자세히 알아보십시오.

## 서비스 상태 모니터링 [#service-health]

Pixie는 한 엔드포인트가 클러스터 외부에 있더라도 서비스 간에 전송된 HTTP/2 요청을 자동으로 추적합니다.

New Relic Pixie 통합은 OpenTelemetry 프로토콜을 사용하여 장기 저장을 위해 HTTP/2 범위를 Pixie에서 New Relic으로 내보냅니다.**서비스 - OpenTelemetry** 보기에는 서비스 응답 시간, 처리량 및 오류율에 대한 높은 수준의 개요와 함께 Pixie가 자동으로 식별하는 서비스가 나열됩니다.
...
이 목록에서 서비스를 선택하면 이 서비스에 대한 모든 HTTP/2 요청에 대한 응답 시간, 처리량 및 오류율 그래프를 보여주는 서비스 **요약** 보기가 열립니다.
...
OpenTelemetry 서비스 보기의 왼쪽 사이드바에서 **서비스 맵** 탭을 선택하십시오.이 서비스 맵 보기는 서비스가 통신하는 모든 엔터티의 그래프를 표시하여 애플리케이션의 종속성을 시각화하는 데 도움이 됩니다.
...
서비스를 선택하면 Pixie에서 수집한 HTTP/2 처리량, 오류율 및 응답 시간 데이터가 포함된 서비스 세부 정보 사이드바가 표시됩니다.
...
이 맵 보기는 서비스가 통신하는 모든 엔터티의 그래프를 표시하여 애플리케이션의 종속성을 시각화하는 데 도움이 됩니다.
...
<figcaption>
  Pod 세부 정보 보기에 표시된 서비스 맵은 Pixie에서 수집한 HTTP/2 추적을 사용하여 구성됩니다.
</figcaption>
...
**TCP 재전송** **보기 및 삭제된 패킷 보기** 링크는 클러스터의 TCP 재전송 및 TCP 삭제 그래프를 표시합니다.이러한 보기에 대한 결과를 보려면 `RUN` 버튼을 눌러야 합니다.TCP drops 출력은 다음과 같습니다.
