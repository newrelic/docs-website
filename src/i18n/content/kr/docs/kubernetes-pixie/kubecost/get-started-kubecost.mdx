...
* 내 워크로드에 대한 네임스페이스별 비용 분석은 무엇입니까?
* 클라우드 공급자가 제공한 가격/할인을 기반으로 비용 추정을 조정하려면 어떻게 해야 합니까?

## 시작하다

Kubecost 및 New Relic을 시작하려면 New Relic에서 Prometheus Remote Write를 설정해야 합니다. 그런 다음 Kubecost 에이전트를 설치해야 합니다.
...
이제 Helm을 통해 Kubecost 에이전트를 설치하겠습니다.
...
3. 671행으로 이동하고 이전 단계에서 Prometheus Remote Write 통합을 위해 생성한 URL 값을 포함하도록 `YOUR_URL_HERE` 를 업데이트합니다. `https://metric-api.newrelic.com/prometheus/v1/write?prometheus_server=kubecost` 와 같아야 합니다.
4. 672행으로 이동하고 이전 단계에서 Prometheus 원격 쓰기 통합을 위해 생성한 전달자 토큰의 값을 포함하도록 `YOUR_BEARER_TOKEN_HERE` 를 업데이트합니다.
5. Helm 명령을 실행하여 Kubecost 에이전트를 클러스터에 추가합니다. New Relic으로 데이터 전송을 시작해야 합니다.
...
6. 몇 분만 기다리세요. 원격 쓰기 설정 이전 탭에서 "내 데이터 보기" 버튼을 클릭하여 데이터 수신 여부를 확인합니다.
7. 데이터를 쿼리합니다.
