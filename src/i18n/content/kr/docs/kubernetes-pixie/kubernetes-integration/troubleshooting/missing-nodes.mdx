...
1. 다음을 실행하여 모든 노드에서 인프라 에이전트를 예약할 수 있는지 확인합니다.
   ...
   <Callout variant="caution">
     Kubernetes 통합의 V2 버전에서는 이 명령이 변경됩니다. [K8s v2 문제 해결: 노드 누락을](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/troubleshooting/missing-nodes) 참조하세요.
   </Callout>
...
3. 클러스터 탐색기에 표시되지 않는 노드의 [인프라 에이전트에서 로그를 검색하고](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/get-logs-version/) [오류 메시지](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/common-error-messages/error-messages) 가 없는지 확인합니다.
