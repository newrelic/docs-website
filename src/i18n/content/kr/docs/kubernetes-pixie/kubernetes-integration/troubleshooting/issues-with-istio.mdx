...
## Istio 클러스터의 xds\_grpc 지표 [#istio-cluster-grpc-metric]
...
## 메타데이터 삽입 포드에 대해 Istio 비활성화 [#disable-istio]
...
newrelic 라벨스페이스 내 `nri-metadata-injection` 파드에 대한 Istio 사이드카를 비활성화하는 것이 좋습니다. 이를 달성하려면 `nri-bundle` 을 설치할 때 `helm` 명령에 `--set-string nri-metadata-injection.labels."sidecar\.istio\.io/inject"=false` 포함하세요.