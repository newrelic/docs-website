...
<CollapserGroup>
  ...
  >
    ...
    따라서 이러한 문제를 완화하려면 newrelic 네임스페이스 내 `nri-metadata-injection` 포드에 대한 Istio 사이드카 삽입을 비활성화하는 것이 좋습니다. 이를 수행하려면 `nri-bundle`을(를) 설치할 때 `helm` 명령에 `--set-string nri-metadata-injection.podLabels."sidecar\.istio\.io/inject"=false` ) 포함하십시오.
  </Collapser>
</CollapserGroup>