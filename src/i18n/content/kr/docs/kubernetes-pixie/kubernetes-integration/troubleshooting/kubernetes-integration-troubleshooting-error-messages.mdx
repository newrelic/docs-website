---
title: 'Kubernetes 통합 문제 해결: 오류 메시지'
...
New Relic 인프라 로그에 [New Relic Kubernetes 통합](/docs/integrations/host-integrations/host-integrations-list/kubernetes-monitoring-integration) 에 대한 오류 메시지가 표시됩니다.

## 해결책

다음은 가장 일반적인 Kubernetes 통합 오류에 대한 몇 가지 솔루션입니다. 이러한 오류는 표준(세부 정보가 아닌) 인프라 에이전트 로그에 표시됩니다. 더 자세한 로그가 필요한 경우(예: New Relic 지원 작업 시) [Kubernetes 로깅](/docs/integrations/host-integrations/host-integrations-list/kubernetes-monitoring-integration#logs-versions) 을 참조하세요.
...
<CollapserGroup>
  ...
  >
    사용 중인 라이선스가 유효하지 않은 경우 로그에 다음과 같은 오류가 표시됩니다.
    ...
    이 문제를 해결하려면 유효한 [라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 지정해야 합니다. 키는 따옴표로 묶고 선행 또는 후행 공백이 없어야 합니다. 예시:
  ...
  >
    에이전트가 New Relic 서버에 연결할 수 없는 경우 로그에 다음과 같은 오류가 표시됩니다.
