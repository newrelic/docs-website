...
<CollapserGroup>
  ...
  >
    가이드 설치 중에 아래와 같은 출력 오류는 Kubernetes 클라이언트와 Kubernetes API 서버 간에 네트워킹 연결 문제가 발생했음을 나타냅니다. 가이드 설치를 다시 실행하기 전에 Kubernetes 클라이언트가 Kubernetes 서비스에 연결할 수 있는지 확인하십시오.
