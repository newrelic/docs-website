...
Kubernetes Events 통합은 [Guided Install](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure) 을 사용하거나 [Helm을 통해 설치할](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm) 때 기본적으로 설치됩니다.
...
2. 차트를 생성하거나 업데이트할 때 차트 유형 **테이블** 을 선택하고 다음 이벤트 쿼리를 사용하여 <var>YOUR_CLUSTER_NAME</var> 을 클러스터 이름으로 바꿉니다.
...
클러스터의 최신 이벤트에 대한 세부 정보를 보려면 다음 쿼리를 사용하여 <var>YOUR_CLUSTER_NAME</var> 을 클러스터 이름으로 바꿉니다.
...
1. [인프라 이벤트 페이지](https://one.newrelic.com) 로 이동합니다. **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Events** 로 이동합니다.
...
* **New Relic에서** : **[one.newrelic.com](https://one.newrelic.com) > Kubernetes cluster explorer** 로 이동합니다.
...
## 선택 사항: 이벤트 수집기의 메트릭 수집 [#events-mentrics]

이벤트 수집기 자체에 대한 메트릭 데이터를 수집하려면 New Relic Prometheus OpenMetrics 통합을 설치하십시오. 시작하려면:

1. New Relic [Prometheus OpenMetrics 통합](/docs/integrations/prometheus-integrations/prometheus-kubernetes/new-relic-prometheus-openmetrics-integration-kubernetes) 을 설치하십시오.
