...
[Kubernetes 통합은](/docs/integrations/kubernetes-integration/get-started/introduction-kubernetes-integration) GKE, EKS, AKS, OKE, OpenShift 등 다양한 플랫폼과 호환됩니다. 각각은 당사 통합 기능과의 호환성이 다릅니다. 이 페이지에서 더 자세한 정보를 확인하실 수 있습니다.
...
Windows 모니터링은 두 가지 모드를 지원합니다:

* **권한 있는 모드**, Windows HostProcess 컨테이너를 사용하여 전체 노드 수준 메트릭을 수집합니다. 권한 있는 모드는 기본적으로 활성화되어 있습니다.
* **권한 없는 모드**, 호스트 가시성이 축소된 표준 컨테이너 사용자로 실행됩니다.

이러한 모드의 작동 방식에 대한 자세한 내용과 안전하게 구성하는 방법에 대한 지침은 [권한 있는 모드 vs. 권한 없는 모드](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode)를 참조하십시오. 전체 설정 지침은 [Windows 노드 모니터](/docs/kubernetes-pixie/kubernetes-integration/installation/windows)를 참조하세요.

## 특권 모드 요구 사항

기본적으로 뉴렐릭 Kubernetes 통합 차트는 서비스 에이전트 사이드카가 **권한 모드** (`privileged: true`)에서 실행되도록 구성합니다. 이 수준의 액세스 권한은 기본 호스트의 `/proc` 및 `/sys` 파일 시스템에서 심층 시스템 메트릭(예: 스토리지 I/O, 물리적 네트워크 인터페이스 및 시스템 프로세스)을 수집하는 데 필요합니다.

클러스터의 보안 정책(예: 파드 보안 표준 또는 OpenShift 표준 조항 계약(SCC))이 권한 있는 컨테이너 계약을 허용하지 않는 경우 이 설정을 비활성화해야 할 수 있습니다.

특권 모드를 비활성화하는 방법에 대한 자세한 지침과 사용 불가능해지는 메트릭의 전체 목록은 [특권 모드와 비특권 모드 비교를](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode) 참조하세요.
...
* Google GKE 클러스터의 Windows 권한 모드, GKE가 Windows `HostProcess` 컨테이너를 지원하지 않기 때문입니다. [권한 없는 모드](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode/#windows-nodes)에서 GKE의 Windows 노드를 여전히 모니터할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        오라클 Kubernetes Engine (OKE)
      </td>

      <td>
        OKE는 CRI-O를 기본 컨테이너 런타임으로 사용합니다. APM 자동 연결을 구성할 때 에이전트 이미지 경로 앞에 `docker.io/` 추가하십시오. 예를 들면: `image: docker.io/newrelic/newrelic-java-init:latest`.
      </td>
    </tr>
