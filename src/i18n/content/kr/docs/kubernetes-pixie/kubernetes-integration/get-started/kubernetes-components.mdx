...
고객은 대상 지표가 기본적으로 활성화되지 않은 경우 KSM에서 모니터링할 모든 대상 지표를 활성화해야 합니다. 예를 들어 KSM v2+는 기본적으로 레이블 및 주석 메트릭을 비활성화합니다. 고객은 Kubernetes 클러스터에서 [여기](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) 에 설명된 `metricLabelsAllowlist` 또는 `metricAnnotationsAllowList` 옵션을 사용하여 모니터링할 대상 라벨 및 주석 측정항목을 사용 설정할 수 있습니다.
