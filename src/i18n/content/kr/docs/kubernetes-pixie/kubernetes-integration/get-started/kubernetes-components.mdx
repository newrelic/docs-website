...
KSM에서 해당 지표가 기본적으로 활성화되어 있지 않은 경우, 커리, 목표 지표를 활성화해야 합니다. 예를 들어 KSM v2+는 기본적으로 레이블과 주석 지표를 비활성화합니다. [여기](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) 쿠버네티스 클러스터에 설명된 `metric-labels-allowlist` 또는 `metric-annotations-allowlist` 옵션을 사용하여 디버그, 목표 라벨 및 주석 지표를 모니터로 활성화할 수 있습니다.
