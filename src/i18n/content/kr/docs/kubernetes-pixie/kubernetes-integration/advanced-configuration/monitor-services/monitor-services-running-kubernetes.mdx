...
Kubernetes에서 실행되는 서비스가 있고 [인프라 에이전트 통합](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 중 하나(Cassandra, Redis, MySQL, Apache 등)에서 해당 서비스를 모니터링할 수 있는 경우 Kubernetes 통합을 통해 이러한 통합을 활성화할 수 있습니다.

## 시작하다 [#overview]

Kubernetes 통합은 [인프라 에이전트 통합](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 과 함께 번들로 제공됩니다. 이렇게 하면 매니페스트 내에서 ConfigMap으로 존재하는 Kubernetes 통합 구성에 섹션을 추가하여 해당 서비스에 대한 데이터를 얻을 수 있습니다.
...
* Kubernetes 통합 버전 1.16.0 이상([install](/docs/integrations/kubernetes-integration/installation/kubernetes-installation-configuration) \| [check version](/docs/integrations/kubernetes-integration/installation/kubernetes-installation-configuration#update) \| [update](/docs/integrations/kubernetes-integration/installation/kubernetes-installation-configuration#update))을 실행하는 Kubernetes 클러스터.
* 요구 사항을 충족하는 ECS에서 실행되는 [지원 서비스입니다](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations#overview) .

이 설치 방법의 경우 RabbitMQ 및 Apache 통합은 인벤토리 데이터를 보고하지 않습니다.

## Helm 차트를 사용하여 서비스 모니터링 활성화 [#enable]

Kubernetes의 모니터링 서비스는 [인프라 에이전트 통합](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 과 자동 검색 메커니즘을 활용하여 지정된 선택기가 있는 포드를 가리키는 방식으로 작동합니다.
