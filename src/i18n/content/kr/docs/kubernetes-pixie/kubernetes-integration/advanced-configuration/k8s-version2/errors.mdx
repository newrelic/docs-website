...
<CollapserGroup>
  <Collapser id="unable-find-kube-state-metrics-v2" title="kube-state-metrics 검색 실패">
  ...
  <Collapser id="metrics-missing" title="네임스페이스, 배포 및 ReplicaSet에 대한 측정항목 누락">
    ...
    <CollapserGroup>
      <Collapser id="indicators" title="누락된 데이터 표시기">
        누락된 네임스페이스, 배포 및 `ReplicaSet` 데이터 표시기:

        * <DNT>**# of K8s objects**</DNT> 차트에서 해당 데이터가 누락되었습니다.
  ...
  <Collapser id="cannot-list-pods-for-cluster" title="클러스터 범위에서 포드를 나열할 수 없습니다.">
...
<CollapserGroup>
  <Collapser id="invalid-license" title="제어 평면 노드에 올바른 레이블이 있는지 확인하세요.">
    ...
    제어 평면 노드가 [제어 평면 구성 요소](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/configure-control-plane-monitoring/#component) 에 정의된 라벨링 규칙을 따르는 경우 다음과 같은 출력이 표시됩니다.
  ...
  <Collapser id="unable-connect" title="제어 평면 노드에서 통합이 실행 중인지 확인하세요.">
    제어 평면 노드에서 실행 중인 통합 패드를 식별하려면 다음 명령의 `NODE_NAME` 이전 단계에 나열된 노드 이름 중 하나로 바꾸세요.
  ...
  <Collapser id="indicators" title="제어 평면 구성 요소에 필요한 레이블이 있는지 확인하십시오.">
    [제어 평면 노드 및 구성 요소 문서 섹션을](/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring#discover-nodes-components) 참조하고 통합이 구성 요소를 검색하는 데 사용하는 레이블을 찾으세요. 그런 다음 다음 명령을 실행하여 이러한 레이블이 있는 파드가 있는지와 해당 파드가 실행되는 노드가 있는지 확인하세요.
  ...
  <Collapser id="cannot-list-pods-for-cluster" title="제어 평면 노드에서 실행 중인 통합 중 하나의 자세한 로그를 검색하고 제어 평면 구성 요소 작업을 확인합니다.">
    로그를 검색하려면 [제어 평면 노드에서 실행되는 파드에서 로그를 가져오는 방법](/docs/integrations/kubernetes-integration/troubleshooting/get-logs-version) 에 대한 지침을 따르세요. 모든 구성 요소에 대한 통합 로그는 다음 메시지 `Running job: COMPONENT_NAME` 를 기록합니다. 예를 들어:
  ...
  <Collapser id="cannot-list-pods-for-cluster" title="구성 요소의 메트릭을 수동으로 쿼리">
