...
뉴렐릭의 FedRAMP 규정 준수에 대한 자세한 내용을 보려면 [FedRAMP 설명서 페이지 를](/docs/security/security-privacy/compliance/certificates-standards-regulations/fedramp/) 방문하세요.
...
설치에 `values.yaml` 파일을 사용하는 경우 동일한 레스토랑을 활성화해야 합니다.
...
전체 `nri-bundle` `values.yaml` 파일 예는 [여기에서](https://github.com/newrelic/helm-charts/blob/master/charts/nri-bundle/values.yaml) 확인할 수 있습니다.
