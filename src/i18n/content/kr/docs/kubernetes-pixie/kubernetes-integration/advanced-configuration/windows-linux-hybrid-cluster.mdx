...
새로운 [`nri-kubernetes` v3 아키텍처는](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/changes-since-v3/) 아직 Windows를 지원하지 않습니다. Windows 워크로드를 모니터링할 수 있는 독립 실행형 릴리스로 `newrelic-infrastructure` 차트를 설치합니다.
...
<Steps>
  ...
  <Step>
    ### Windows 노드에 대해 `newrelic-infrastructure` 정의 [#newrelic-infra]
    ...
    다른 Windows 노드 그룹 또는 노드 풀 버전을 실행하는 경우 `windowsOsList` 섹션을 적절한 값으로 조정하세요. [Windows 설치](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#windows-install) 섹션에서 이러한 값을 볼 수 있습니다.
  </Step>
  ...
  <Step>
    ...
    데이터를 생성하고 나면 뉴렐릭에서 볼 수 있습니다. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes**</DoNotTranslate> 으로 이동한 후 클러스터를 선택하세요.
  </Step>
</Steps>
