...
<Callout variant="tip">
  달리 지정하지 않는 한 이 페이지는 [Kubernetes 통합 v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3) 을 참조합니다. v2에 대한 컨트롤 플레인 모니터링을 구성하는 방법에 대한 자세한 내용은 아래의 특정 섹션에서 찾을 수 있습니다.
</Callout>
...
* <DNT>**etcd**</DNT>
  ...
  : 리더 정보, 상주 메모리 크기, OS 스레드 수, 합의 제안 데이터 등 지원되는 지표 목록은 [etcd data 를](/docs/integrations/kubernetes-integration/understand-use-data/understand-use-data#etcd-data) 참조하세요.

* <DNT>**API server**</DNT>

  : `apiserver` 요청 비율, HTTP 메소드 및 응답 코드별 `apiserver` 요청 분석 등 지원되는 지표의 전체 목록은 [API 서버 데이터를](/docs/integrations/kubernetes-integration/understand-use-data/understand-use-data#api-server-data) 참조하세요.

* <DNT>**Scheduler**</DNT>

  : 요청된 CPU/메모리와 노드에서 사용 가능한지, 오염에 대한 허용, 설정된 선호도 또는 반선호도 등 지원되는 지수의 전체 목록은 [스케줄러 데이터를](/docs/integrations/kubernetes-integration/understand-use-data/understand-use-data#scheduler-data) 참조하세요.

* <DNT>**Controller manager**</DNT>

  : 상주 메모리 크기, 생성된 OS 스레드 수, 현재 존재하는 고루틴 등 지원되는 지표의 전체 목록은 [컨트롤러 관리자 데이터를](/docs/integrations/kubernetes-integration/understand-use-data/understand-use-data#controller-manager-data) 참조하세요.
...
이러한 버전에는 덜 유연한 자동 검색 옵션이 있었고 외부 끝점을 지원하지 않았습니다. 가능한 한 빨리 [버전 3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3) 으로 업데이트하는 것이 좋습니다. Kubernetes 통합 [의 변경 사항을 확인하십시오](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3) .
...
<figcaption>
  <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes Cluster Explorer**</DNT>: 클러스터의 컨트롤 플레인 구성 요소에서 클러스터를 모니터링하고 수집하려면 쿠버네티스 클러스터 탐색기를 사용하세요.
</figcaption>
