...
import kubernetesIntegrationCP from 'images/kubernetes-integration-cp.png'

import kubernetesIntegrationCPExternal from 'images/kubernetes-integration-cp-external.png'
...
* 제어 평면 모니터링 지원은 관리되는 클러스터에 대해 제한됩니다. 이는 대부분의 클라우드 공급자가 컨트롤 플레인 구성 요소에 대한 메트릭 엔드포인트를 노출하지 않아 New Relic이 액세스할 수 없기 때문입니다.
...
<Callout variant="important">
  노드 외부(즉, `localhost` 아님) 엔드포인트를 가리키는 `staticEndpoint` 를 사용하는 경우 `controlPlane.kind` 를 `DaemonSet` 에서 `Deployment` 로 변경해야 합니다.
</Callout>

`staticEndpoint` 사용 시 모든 `nrk8s-controlplane` 포드는 해당 엔드포인트에 도달하여 긁어모으려고 합니다. 즉, `nrk8s-controlplane` 가 DaemonSet(기본값)이면 DaemonSet의 모든 인스턴스가 이 끝점을 스크랩합니다. `localhost` 을 가리키는 경우에는 문제가 없지만 엔드포인트가 노드에 로컬이 아닌 경우 잠재적으로 메트릭을 복제하고 청구 가능한 사용량을 증가시킬 수 있습니다. `staticEndpoint` 를 사용하고 로컬이 아닌 URL을 가리키는 경우 `controlPlane.kind` 를 배포로 변경해야 합니다.
...
2. 선택적으로 비밀 이름과 네임스페이스를 의미 있는 것으로 변경합니다.
...
5. [mtls 섹션](#mtls) 에 설명된 대로 새로 생성된 비밀을 사용하도록 통합을 구성합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Kubernetes Cluster Explorer** : Kubernetes 클러스터 탐색기를 사용하여 클러스터의 제어 평면 구성 요소에서 메트릭을 모니터링하고 수집합니다.
</figcaption>
