...
<Callout variant="important">
  **이제 뉴렐릭 Kubernetes 통합 v3에서 Windows 노드 모니터링이 정식 출시(GA)되었습니다.** 이 페이지에 설명된 하이브리드 설치 방식(Windows용 별도의 `newrelic-infrastructure` v2 차트 사용)은 더 이상 사용되지 않으며 향후 릴리스에서 제거될 예정입니다. 대신 [안내 설치 프로세스](/install/kubernetes/) 를 사용하세요. 기본적으로 단일 설치에서 Linux와 Windows 노드를 모두 지원합니다.
</Callout>
...
<Steps>
  <Step>
    ...
    * 이 파일에 표시되지 않은 다른 하위 차트가 `nri-bundle` 에 존재할 수 있습니다. 해당 하위 차트를 추가하려면 yaml 파일에 키 또는 값 쌍을 추가합니다.
