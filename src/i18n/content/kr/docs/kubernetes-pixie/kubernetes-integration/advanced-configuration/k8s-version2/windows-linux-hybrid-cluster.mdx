...
<Callout variant="important">
  뉴렐릭 Kubernetes 통합 v3는 이제 Windows 새로운 [`nri-kubernetes` v3를 통해 노드](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) 모니터링을 지원합니다. Windows 노드에 대한 모니터링을 활성화하려면 [안내 설치 프로세스를 사용하세요.](/src/install/kubernetes/intro) 이 프로세스는 Linux와 Windows 노드를 모두 지원하므로 기본적으로 하이브리드 클러스터 설치가 가능합니다.
</Callout>
