...
* 참고로 KSM v2+는 기본적으로 레이블 측정항목을 비활성화합니다. Kubernetes 클러스터에서 [여기](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) 에 설명된 `metric-labels-allowlist` 옵션을 사용하여 모니터링할 대상 라벨 측정항목을 활성화할 수 있습니다.
