...
<CollapserGroup>
  ...
  <Collapser id="no-metadata-in-transactions-attributes" title="트랜잭션 속성에 Kubernetes 메타데이터가 없습니다.">
    ...
    1. [메타데이터 주입 유효성 검사](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) 섹션에 설명된 지침에 따라 환경 변수가 새 파드에 올바르게 주입되었는지 확인하십시오.

    2. 만약 해당 로그가 존재하지 않는다면, 우선 웹훅 서비스의 로그를 확인하십시오.

       ```shell
       kubectl logs -l app.kubernetes.io/name=nri-metadata-injection -f
       ```

    3. 다른 터미널에서 새 파드를 생성하고 로그 데이터를 게시하여 배포에서 오류를 검사하세요. 새 파드를 생성하려면 [메타데이터 주입 유효성 검사](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) 섹션을 참조하세요. [로그 레벨(logLevel)](https://github.com/newrelic/k8s-metadata-injection/blob/main/charts/nri-metadata-injection/README.md#values) 이 INFO 이하인 경우, 생성된 각 파드에 대해 로그 항목이 생성되어야 합니다.
    ...
    7. 다음 명령의 출력을 확인하여 해당 주입 설정 작업이 성공적으로 실행되었는지 확인하십시오.
