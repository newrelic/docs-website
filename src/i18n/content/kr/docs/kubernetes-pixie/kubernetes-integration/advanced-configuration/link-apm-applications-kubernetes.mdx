...
Kubernetes `MutatingAdmissionWebhook`과 통신하려면 마스터 노드(또는 클러스터 설정 방법에 따라 API 서버 컨테이너)가 클러스터의 다른 모든 노드에 있는 파드에 대한 포트 443의 HTTPS 트래픽 송신을 허용해야 합니다.
...
<CollapserGroup>
  ...
  >
    ...
    5. 터미널 창에서 이 명령을 실행하여 Kubernetes API 서버에 대한 프록시를 시작하고 계속 실행합니다.

       ```shell
       kubectl proxy --port=8001
       ```

    6. 클러스터에 새 파드를 생성하면 API 서버가 웹훅과 통신을 시도하게 됩니다. 이 명령은 busybox를 생성합니다.
    ...
    10. API 서버 로그인이나 메타데이터에 게시 항목에 로그인 항목이 없으면 웹훅이 제대로 등록되지 않았다는 의미입니다.

    11. 다음 명령의 출력을 검사하여 메타데이터 주입 설정 작업이 성공적으로 실행되었는지 확인하세요.

        ```shell
        kubectl get job newrelic-metadata-setup
        ```

    12. 작업이 완료되지 않으면 설정 작업 로그를 조사합니다.

        ```shell
        kubectl logs job/newrelic-metadata-setup
        ```

    13. 다음 명령어를 실행하여 `CertificateSigningRequest` 이 승인되고 발급되었는지 확인하세요.

        ```shell
        kubectl get csr newrelic-metadata-injection-svc.default
        ```

    14. 다음 명령을 실행하여 TLS 비밀이 있는지 확인하세요.

        ```shell
        kubectl get secret newrelic-metadata-injection-secret
        ```

    15. 변경하는 웹훅 구성에 CA 번들이 있는지 확인합니다.

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    16. <DNT>**Service**</DNT> 리소스의 `TargetPort` <DNT>**Deployment**</DNT> 컨테이너의 <DNT>**Port**</DNT> 와 일치하는지 확인하세요.
