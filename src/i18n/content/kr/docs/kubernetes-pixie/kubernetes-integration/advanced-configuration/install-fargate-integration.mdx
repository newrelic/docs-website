...
2. 시크릿이 존재하지 않는 경우 사이드카가 데이터를 보고하는 데 필요한 New Relic 라이선스 키가 포함된 포드와 동일한 네임스페이스에 생성합니다.
...
4. New Relic 라이선스 키가 포함된 비밀을 만듭니다. 각 네임스페이스에는 고유한 비밀이 필요합니다.
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="비밀"
     >
       NewRelic 라이선스 키의 Base64 인코딩 값으로 라이선스가 있는 다음 `Secret` 을 만듭니다. 모니터링하려는 팟(Pod)이 실행 중인 각 네임스페이스에 하나의 시크릿이 필요합니다.
...
<Callout variant="important">
  에이전트는 New Relic 라이센스 키를 핫 로드할 수 없습니다. 보안 비밀을 업데이트한 후 배포를 다시 롤아웃해야 합니다.
</Callout>
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Kubernetes** 로 이동하고 다음 중 하나를 수행합니다.
