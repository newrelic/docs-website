...
4. New Relic <LicenseKey/> 를 포함하는 비밀을 만듭니다. 각 네임스페이스에는 자체 비밀이 필요합니다.
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="비밀"
     >
       <LicenseKey/> 의 Base64 인코딩 값으로 라이센스가 있는 다음 `Secret` 을 생성합니다. 모니터링하려는 팟(Pod)이 실행 중인 각 네임스페이스에 하나의 시크릿이 필요합니다.
...
<Callout variant="important">
  에이전트는 New Relic <LicenseKey/> 를 핫 로드할 수 없습니다. 암호를 업데이트한 후 배포를 다시 롤아웃해야 합니다.
</Callout>
