...
<Callout variant="important">
  **이제 뉴렐릭 Kubernetes 통합 v3에서 Windows 노드 모니터링이 정식 출시(GA)되었습니다.** 이 페이지에 설명된 v2 설치 방식은 더 이상 사용되지 않으며 향후 릴리스에서 제거될 예정입니다. Windows 노드를 기본적으로 지원하는 현재 설치 지침은 [Kubernetes 통합 설치 가이드](/install/kubernetes) 를 참조하세요.
</Callout>

Windows 기반 Kubernetes 시스템이 있는 경우 이 옵션을 사용하십시오. Windows 통합에는 몇 가지 [제한 사항](#k8-windows-limitations)이 있다는 점에 유의하십시오.
