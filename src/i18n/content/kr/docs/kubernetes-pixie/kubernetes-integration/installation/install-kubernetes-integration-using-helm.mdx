...
* [`nri-statsd`](https://github.com/newrelic/helm-charts/tree/master/charts/nri-statsd) : 새로운 Relic StatsD 통합.
...
<Callout variant="tip">
  `--devel` 을(를) 지정하면 현재 베타 버전이고 2022년 봄에 일반 공급될 예정인 솔루션 버전 3을 설치하게 됩니다. v2에 비해 상당한 개선 사항이 포함되어 있으므로 사용해 보시기 바랍니다. [변경된 사항을 확인하십시오](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3) .
</Callout>
...
수정할 수 있는 설정의 전체 목록은 [차트의 README](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-infrastructure-v3) 에서 찾을 수 있습니다.
...
`lowDataMode` 아래에 설명된 `nri-bundle` 차트의 세 가지 특정 구성요소에 영향을 미칩니다.
...
### 새로운 유물 픽시 통합 [#nr-pixie-integration]
