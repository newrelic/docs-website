...
뉴렐릭을 사용하면 원하는 특정 데이터를 정의하고, 사용자 지정 가능한 [범위 및](/docs/alerts/create-alert/set-thresholds/set-thresholds-alert-condition/) [한계를 설정하고, 수신할 알림을](/docs/alerts/get-notified/intro-notifications/) 구성하여 Kubernetes 문제에 대한 정보를 지속적으로 받을 수 있습니다. 필요에 맞게 조정할 수 [있는 권장 공지 정책](/docs/kubernetes-pixie/kubernetes-integration/installation/recommended-alert-policies) 세트를 추가하거나 환경에 맞게 완전히 사용자 지정 <InlinePopover type="alerts" />정책 및 [참조우를](/docs/alerts/get-notified/alert-event-workflows/) 만들 수 있습니다.
...
4. 이 정책에서 [공지 이벤트를 그룹화](/docs/alerts/organize-alerts/specify-when-alerts-create-events/#preference-options) 할 방식을 선택하세요.
...
공지 조건, 정책, [활동 내용](/docs/alerts/get-notified/alert-event-workflows/) 을 구성하고 나면, 공지 조건에 구성된 규칙값, 규칙이 실행될 때 공지를 받기 시작합니다. 다음과 같은 옵션이 있습니다.

* [공지이벤트 보기](/docs/alerts/alert-event-management/view-alert-event-details/)

* [공지사항 확인](/docs/alerts/get-notified/acknowledge-alert-events/)

* 해당 항목의 이름을 선택하여 공지이벤트 데이터 차트로 이동하세요. 공지를 촉발한 특이사항은 공지 메시지 상단 근처에 나타납니다. 푸시의 형식은 공지 유형에 따라 다릅니다.
