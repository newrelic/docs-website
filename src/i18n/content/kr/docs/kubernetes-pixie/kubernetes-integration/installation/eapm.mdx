...
## eAPM과 APM의 주요 차이점 [#key-differences]
...
6. eAPM 에이전트 Helm 차트를 설치하려면 `your-custom-values.yaml` `values.yaml` 파일 경로로 바꾸고 다음을 실행합니다.
...
<Callout variant="tip" title="팁">
  eAPM Helm 차트는 다양한 설정 옵션을 제공합니다. [`value.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-ebpf-agent/values.yaml) 파일을 검토하여 필요에 맞게 설치를 조정하세요.
</Callout>

## eAPM 기반 APM UI에 액세스 [#eapm-access]
...
## eAPM 에이전트와 APM이 함께 작동하는 방식 [#eapm-apm]
