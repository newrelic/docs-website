...
쿠버네티스 통합을 설치하려면, 안내 설치에 대한 지침은 아래를 참조하시기 바랍니다. 서버, VM 및 [권한이 없는](/docs/infrastructure/install-infrastructure-agent/linux-installation/linux-agent-running-modes) 환경는 이 양방향 설치 툴을 권장합니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        필수 값이 채워진 Helm 명령을 통해, 또는 Helm을 사용하지 않는 경우는 일반 매니페스트를 통해 설치 프로세스를 안내합니다. 어떤 기능을 활성화하고 어떤 데이터를 수집할지 제어할 수 있습니다.

        또한, 쿠버네티스 클러스터에 대한 즉각적인 가시성을 얻을 수 있도록 쿠버네티스 통합과 함께 대시보드, 알림 등 선택 사용할 수 있는 일부 사전 구축된 리소스가 포함된 퀵스타트 옵션을 제공합니다.
      </td>

      <td>
        * [퀵스타트를 통한 안내 설치](https://one.newrelic.com/launcher/catalog-pack-details.launcher/?pane=eyJuZXJkbGV0SWQiOiJjYXRhbG9nLXBhY2stZGV0YWlscy5jYXRhbG9nLXBhY2stY29udGVudHMiLCJxdWlja3N0YXJ0SWQiOiI4OGE3OWY1Mi04MWMxLTRmYTItOTlmOC0zY2I1YjAxMmYxNjAifQ==): 뉴렐릭 조직이 [EU](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers) ****데이터센터를 이용하지 않으며 퀵스타트의 알림과 일부 보너스 대시보드를 설치하고자 하는 경우
        * [안내 설치](https://one.newrelic.com/nr1-core?account=2498654&state=d1aae74b-0ad6-b0f3-093d-cc89ecf89234): 뉴렐릭 조직이 [EU](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers) 데이터센터를 **이용하지 않는 경우**
        * [안내 설치(EU)](https://one.eu.newrelic.com/nr1-core?account=2498654&state=d1aae74b-0ad6-b0f3-093d-cc89ecf89234): 뉴렐릭 조직이 [EU](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers) 데이터센터를 이용하는 경우
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Windows 기반 쿠버네티스 시스템이 있는 경우 사용합니다. Windows 통합에는 [여러 가지 제한 사항](/docs/kubernetes-pixie/kubernetes-integration/installation/install-version2-kubernetes-integration-windows/#k8-windows-limitations)이 있습니다.  
    ...
    <tr>
      ...
      <td>
        안내 설치에서는 일부 구성 옵션을 묻는 메시지가 표시되고 암호와 값이 자동으로 채워지므로 이 옵션은 권장하지 않습니다. 안내 설치를 사용하면 Helm 릴리스가 아닌 일반 매니페스트로 통합을 설치할 수도 있습니다.
      </td>
    ...
    <tr>
      ...
      <td>
        EKS Fargate에서 쿠버네티스 워크로드를 모니터링하는 경우에 사용합니다. 이 통합은 모니터링해야 하는 각 포드에 인프라 에이전트와 nri-kubernetes 통합이 포함된 사이드카를 자동으로 주입합니다.
      </td>
    ...
    <tr>
      ...
      <td>
        쿠버네티스 오퍼레이터는 쿠버네티스 리소스를 일련의 커스텀 구성 또는 커스텀 리소스로 추상화하여 복잡한 애플리케이션을 관리하는 데 도움을 줍니다.

        애플리케이션을 관리하기 위해 커스텀 리소스와 연동하기만 하면 되므로 사용자 부담이 줄어듭니다. 또한 오퍼레이터를 통해 애플리케이션을 배포, 업그레이드 및 관리할 수 있습니다.
      </td>
      ...
      <td>
        ...
        연산자 문서에 포함된 지침을 따르고 필요에 따라 이 문서 참조
      </td>
    </tr>
  </tbody>
</table>

이 문서의 나머지 부분에서는 안내 설치 프로세스에 대해 설명합니다.
...
<Steps>
  ...
  <Step>
    ...
    * **Helm 3**: [Helm](https://helm.sh/)을 사용하여 쿠버네티스 통합을 설치 및 구성하려는 경우 이 옵션을 사용합니다. 이 옵션은 여기에 설명된 옵션으로 추가로 구성할 수 [있는](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm/#configure) [`nri-bundle` Helm 차트](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm/#install-k8-helm)를설치합니다. 여기에서 [뉴렐릭 연산자](docs/kubernetes-pixie/kubernetes-integration/installation/install-with-operator/)를 활성화할 수도 있습니다.
