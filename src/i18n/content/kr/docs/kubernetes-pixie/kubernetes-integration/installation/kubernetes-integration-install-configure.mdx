...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="guided-install">
      ...
      <Callout variant="tip">
        다음 단계는 퀵스타트에 필요한 사전 단계들을 건너뜁니다. 퀵스타트를 통한 안내 설치를 선택한 경우, <DNT>**Confirm your Kubernetes quickstart installation**</DNT> 및 설치 계획<DNT>**Installation plan**</DNT> 페이지를 클릭하면 아래 설명된 기본 안내 설치 페이지로 이동합니다.
      </Callout>
      ...
      <Steps>
            ...
            <CollapserGroup>
              ...
              >
                ...
                3. `Roles`) 및 `ClusterRoles`를 생성할 수 있는 권한이 있는지 확인합니다. 확실하지 않으면
                   ...
                   <DNT>**Kubernetes Engine Cluster Admin**</DNT>
        ...
        <Step>
          ...
          <DNT>**Configure the Kubernetes Integration**</DNT> 페이지에서 다음 필드를 완료합니다.
        ...
        <Step>
          ...
          <DNT>**Select the additional data you want to gather**</DNT> 페이지에서 적합한 옵션을 선택합니다.
          ...
          <CollapserGroup>
            ...
            >
              메트릭 중복 여부에 관계없이 모든 Prometheus 메트릭에서 Prometheus의 메트릭 명명 규칙을 유지하려면 <DNT>**Scrape all Prometheus endpoints**</DNT>를 선택합니다.
            </Collapser>
          ...
          [Pixie](https://docs.px.dev/about-pixie/what-is-pixie/)는 eBPF를 사용해 텔레메트리 데이터를 자동으로 수집하는 쿠버네티스 애플리케이션을 위한 오픈소스 옵저버빌리티 툴입니다. 클러스터에 Pixie가 설치되어 있지 않지만 [뉴렐릭 플랫폼](/docs/kubernetes-pixie/auto-telemetry-pixie/get-started-auto-telemetry-pixie/)에서 Pixie의 강력한 텔레메트리 데이터 수집 및 시각화를 활용하려면 <DNT>**Enable service-level insights, full-body requests, and application profiles through Pixie**</DNT>를 선택합니다.

          이미 Community Cloud를 사용하고 있는 경우 <DNT>**Community Cloud hosted Pixie is already running on this cluster**</DNT>를 선택합니다. [Pixie를 호스팅할 수 있는](https://docs.px.dev/installing-pixie/install-guides/#title) 다양한 방법이 존재한다는 사실을 염두에 두십시오. 뉴렐릭은 각 Pixie 호스팅 옵션에 대해 여러 수준의 통합 지원을 제공합니다.
          ...
          <CollapserGroup>
            ...
            >
              ...
              Pixie Cloud를 자체 호스팅하고 있고 Pixie 텔리메트리 데이터를 뉴렐릭으로 내보내려면 <DNT>**Community Cloud hosted Pixie option**</DNT>을 선택하지 않고, 쿠버네티스 통합에서 Pixie를 활성화하면 됩니다. 쿠버네티스 통합은 Pixie가 클러스터에서 실행 중임을 감지하고 데이터 가시성과 인사이트를 즉각적으로 확보할 수 있도록 데이터 내보내기를 활성화합니다.
    ...
    <TabsPageItem id="eks-fargate">
      ...
      * [Amazon ECS에서 뉴렐릭으로 로그를 전달하는 세 가지 방법](https://newrelic.com/blog/how-to-relic/forward-logs-from-amazon-ecs-to-new-relic)을 참조하십시오.
      ...
      1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Kubernetes**</DNT>
