...
서버, VM 및 [권한이 없는 ](/docs/infrastructure/install-infrastructure-agent/linux-installation/linux-agent-running-modes)환경에는 안내 설치를 사용하길 권합니다. 안내 설치는 필수 값이 채워진 Helm 명령을 제공하며, Helm을 사용하지 않는 경우 일반 매니페스트를 제공합니다. 또한 어떤 기능을 활성화하고 어떤 데이터를 수집할지 제어할 수 있습니다. 매니지드 클라우드에 통합을 설치하는 경우, 계속하기 전에 이 [사전 준비 사항](#cloud-platforms)을 살펴보시기 바랍니다.
