...
* [자동 주입(권장)](#fargate-automatic)
* [수동 주입](#fargate-manual)
...
<Steps>
  ...
  <Step>
    ...
    인프라 사이드카 삽입을 담당하는 연산자를 설치하려면 `values.yaml` 이라는 파일을 만듭니다. 이 파일은 귀하의 설정을 정의합니다:
...
### 자동 주입: 알려진 제한사항 [#known-limitations]
...
### 자동 등록: 설정 [#automatic-configuration]

자동 주입의 다양한 측면을 구성할 수 있습니다. 기본적으로 운영자는 `Job` 또는 `BatchJob` 의 일부가 아닌 Fargate 노드에 배포된 모든 포드에 모니터링 사이드카를 삽입합니다.

이 동작은 설정 옵션을 통해 변경할 수 있습니다. 예를 들어, 주입되는 파드의 선택 범위를 좁히거나 넓히는 선택기를 정의하고, 운영자에게 리소스를 할당하고, 사이드카를 튜닝할 수 있습니다. 또한 다른 속성, 라벨, 환경 변수를 추가할 수도 있습니다. [`README.md`](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-infra-operator/README.md) 및 [`values.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-infra-operator/values.yaml) 차트를 참조하세요.
...
### 최신 버전 또는 새 구성으로 업데이트 [#automatic-update]
...
자동 주입에 대해 우려 사항이 있는 경우 Fargate 노드에 예약될 예정인 워크로드의 매니페스트를 수정하여 직접 사이드카를 주입할 수 있습니다. EC2 노드에 예약된 배포에 사이드카를 추가하면 데이터가 잘못되거나 중복될 수 있습니다. 특히 해당 노드가 이미 `DaemonSet` 으로 모니터링되고 있는 경우에는 더욱 그렇습니다.

사이드카가 데이터를 성공적으로 보고하려면 다음 개체가 필요합니다.

* `nri-kubernetes` 통합에 필요한 권한을 제공하는 `ClusterRole` 입니다.
* `ClusterRole` 과 파드의 서비스 계정을 연결하는 `ClusterRoleBinding` 입니다.
* 각 Fargate 지우스페이스에 뉴렐릭 `licenseKey` 을(를) 저장하는 비밀입니다.
* 모니터링되는 워크로드의 특정 템플릿에 있는 사이드카 컨테이너입니다.

<Callout variant="tip">
  이러한 수동 설정 단계는 일반 설치를 위한 것입니다. 이 작업을 완료하기 전에 아래 [설정](#manual-configuration) 섹션을 살펴보고 자동 입력의 어떤 측면을 수정하고 싶은지 확인하세요.
</Callout>
...
<Steps>
  <Step>
    ...
    `ClusterRole` 이 존재하지 않는 경우 이를 생성하고 메트릭 엔드포인트에 도달하는 데 필요한 권한을 부여합니다. 동일한 클러스터에서 여러 애플리케이션을 모니터링하는 경우에도 이 작업은 한 번만 수행하면 됩니다. 이 스니펫을 아무런 변경 없이 아래와 같이 사용할 수 있습니다.
  ...
  <Step>
    ### 주입된 사이드카 [#manual-injected-sidecar]

    모니터링하려는 각 워크로드에 대해 `newrelic/infrastructure-k8s` 이미지에 대한 추가 사이드카 컨테이너를 추가합니다. 다음 스니펫의 컨테이너를 가져와 모니터링할 워크로드에 삽입하고 `customAttributes` 변수에 `FargateProfile` 이름을 지정합니다. 볼륨은 `emptyDir: {}` 으로 정의될 수 있습니다.
  ...
  <Step>
    ...
    `ClusterRoleBinding` 을 만들거나 모니터링할 애플리케이션의 `ServiceAccount` 을(를) 이전에 만든 항목에 추가합니다. 모든 워크로드는 동일한 `ClusterRoleBinding` 를 공유할 수 있지만 각 워크로드의 `ServiceAccount` 를 여기에 추가해야 합니다.
  ...
  <Step>
    ### 비밀이 포함된 [#secret-containing]

    뉴렐릭<InlinePopover type="licenseKey"/>을 포함하는 시크릿을 생성합니다. 각 네임스페이스에는 고유한 시크릿이 필요합니다.
...
### 수동 주입: 최신 버전으로 업데이트 [#manual-update-version]
...
### 수동 주입: Fargate 통합 제거 [#manual-uninstall]
...
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="daemonset"
    title="DaemonSet 복제본이 Fargate 노드에 배포되고 있습니다."
  >
    Fargate 노드에서 Infra `DaemonSet` 복제본이 예약되고 있는 것을 발견하면 `nodeAffinity` 규칙이 제대로 구성되지 않았기 때문일 수 있습니다.

    명령줄(`--set global.fargate=true`) 또는 `values.yaml` 파일을 통해 `true` 에 대한 `global.fargate` 옵션을 사용하여 솔루션이 설치되었는지 다시 확인하세요. 설치 방법이 Helm이 아닌 경우 Fargate 노드를 제외하려면 `nodeAffinity` 규칙을 수동으로 추가해야 합니다.
  </Collapser>

  <Collapser
    className="freq-link"
    id="event-untolerated"
    title={<>허용되지 않는 오염으로 인한 이벤트 <InlineCode>FailedScheduling</InlineCode></>}
  >
    포드를 생성하려고 시도하는 동안 다음 이벤트가 발생하는 경우 [자동 삽입 설치](#auto-injection-install) 에 설명된 `tolerations` 를 `values.yaml` 파일에 추가해야 합니다.
  ...
  <Collapser
    className="freq-link"
    id="event-many-pods"
    title={<>포드가 너무 많아 이벤트 <InlineCode>FailedScheduling</InlineCode></>}
  >
...
1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Kubernetes**</DNT>로 이동하여 다음 중 하나를 수행합니다.
