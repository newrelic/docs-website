...
* 계측기 이름 CR
* 계측 CR을 적용할 위치( `podLabelSelector` 및 `namespaceLabelSelector` 덕분에)
* APM 에이전트(CR당 하나)
* APM 에이전트 버전
...
측정, 로그 CR이 APM 에이전트를 주입할 시기를 알려면 선택기를 사용해야 합니다. 사용자의 요구 사항에 따라 함께 사용할 수 있는 2개의 레이블 선택기가 있습니다(논리적 AND(`&&`) 연산자로 작동) 또는 별도로 사용할 수 있습니다.
...
두 선택기 모두 `matchLabel` 및 `matchExpressions` 지원합니다.
