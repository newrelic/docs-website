...
<CollapserGroup>
  ...
  <Collapser id="namespace-label-selector" title="네임스페이스 레이블 선택기">
    네임스페이스 수준에서 자동 계측된 파드를 정의합니다. CR과 오퍼레이터가 동일한 네임스페이스에 있는 경우 허용됩니다. 그렇지 않으면 이 선택기를 비워 두어야 합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Windows 2022의 .NET](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2022-init/general)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [Windows 2025의 .NET](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2025-init/general)
      </td>
    </tr>
...
<Callout variant="tip">
  클러스터에서 명시적인 레지스트리 접두사(예: CRI-O)가 필요한 컨테이너 런타임을 사용하는 경우 이미지 경로 앞에 `docker.io/` 추가하십시오. 예를 들면: `image: docker.io/newrelic/newrelic-java-init:latest`.
</Callout>
...
* [APM 요약](/docs/apm/agents/manage-apm-agents/agent-data/triage-run-diagnostics/) 페이지를 이용하여 인사이트를 확인하고 공지사항을 해결하세요.
...
<CollapserGroup>
  ...
  <Collapser id="setup-operator" title="Windows 노드에서 Kubernetes APM 자동 연결을 설정할 수 있나요?">
    현재 Kubernetes APM 자동 연결은 Windows Server 2022 및 Windows Server 2025 노드에서 실행되는 .NET 애플리케이션에만 지원됩니다.
  </Collapser>
