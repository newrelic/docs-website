...
## Kubernetes 플레이버 및 프로바이더와의 호환성 [#enable-provider]
...
공급자가 GKE Autopilot인 경우, 해당 공급자는 권한 있는 컨테이너를 지원하지 않으므로 `daemonset.containerSecurityContext.privileged` 값을 기본값에서 변경하지 마십시오.

Amazon EKS를 사용하는 경우, 클러스터의 Terraform 설정을 업데이트하거나 AWS Console을 사용하여 다음 방법 중 하나를 통해 `resourcedetection` 프로세서를 활성화하십시오:

* IMDS 홉 한도 늘리기: 노드 그룹에 대해 `http_put_response_hop_limit` 를 2로 설정합니다.
* 파드 ID 활성화: EKS 모듈에 `eks-pod-identity-agent` 애드온을 추가합니다.
...
제공업체가 OpenShift이고 `receivers.filelog` 이 활성화된 경우(기본값), 로깅이 제대로 작동하는 데 필요한 권한이 부여되도록 `daemonset.containerSecurityContext` 에서 다음 업데이트를 수행하십시오.
