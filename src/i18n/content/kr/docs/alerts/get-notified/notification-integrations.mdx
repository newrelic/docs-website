...
<CollapserGroup>
  <Collapser className="freq-link" id="jira" title="아틀라시안 지라">
    ...
    * Jira 문제 상태가 `in-progress` 으로 변경되면 New Relic 문제 확인이 트리거됩니다.
    ...
    기본 필드 값이 포함된 테스트 공지를 클릭하여 Jira 이슈를 확인하세요. 성공적으로 완료되면 Jira에서 공지 이벤트를 볼 수 있는 링크가 제공됩니다.
  ...
  <Collapser className="freq-link" id="email" title="이메일">
    [알림우의](/docs/alerts/get-notified/alert-event-workflows/) 공지 채널로 <DNT>**Email**</DNT> 선택하면 이메일 수신처가 자동으로 생성되므로 <DNT>**Destinations**</DNT> 메뉴에서 설정할 필요가 없습니다. 각 이메일 수신처는 연결된 이메일 주소에 따라 고유하며, 따라서 수신처 피드에 중복으로 나타날 수 있습니다.
  ...
  <Collapser className="freq-link" id="pagerduty" title="호출기 의무">
    뉴렐릭을 PagerDuty 와 통합하여 PagerDuty 공지 이벤트를 자동으로 생성, 업데이트, 확인 및 해결합니다.
  ...
  <Collapser className="freq-link" id="slack" title="느슨하게">
    ...
    Slack 작업 공간에 [뉴렐릭 애플리케이션](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(`one.eu.newrelic` 고객의 경우 [EU 앱](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) 또는 `one.jp.newrelic` 고객의 경우 [JP 앱](https://newrelic.slack.com/marketplace/A0ATV7NGLLD-new-relic-jp))이 설치되어 있어야 합니다. 개별적으로 설치하려면 먼저 작업 공간 관리자가 애플리케이션을 승인해야 합니다.
