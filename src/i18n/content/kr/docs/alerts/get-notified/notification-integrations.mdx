...
<CollapserGroup>
  ...
  >
    ...
    * PagerDuty 인시던트가 확인되면 New Relic 문제에 대한 확인이 트리거됩니다.
    ...
    6. 대상을 선택하거나 새 대상을 만드세요.

    7. (선택 사항) 기본 인시던트 요약을 편집합니다.
  ...
  >
    ...
    * `caller` 열에 대한 사용자를 가져오려면 `sys_user` 테이블에 대한 읽기 권한이 필요합니다.
    ...
    3. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Workflows**</DNT> 으로 이동하여 기존의 워크플로우를 클릭하거나, <DNT>**+ Add a new workflow**</DNT> 버튼을 클릭하세요.

    4. ServiceNow 대상을 선택합니다.

    5. 연결에 성공하면 ServiceNow 인시던트 테이블 열이 계정에서 가져와서 ServiceNow 인스턴스에 자동으로 매핑됩니다.
  ...
  >
    ...
    6. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Workflows**</DNT> 으로 이동하여 기존의 워크플로우 또는 <DNT>**+ Add a workflow**</DNT> 버튼을 클릭하세요.

    7. 메시지를 보낼 대상(워크스페이스라고도 함)과 Slack 채널을 선택하세요. 필요한 워크스페이스에 대해 사전 정의된 대상이 없는 경우 새 대상을 생성할 수 있습니다.

    8. 문제가 확인되거나 종료된 경우와 같은 공지 업데이트를 채널에서도 수신하도록 선택할 수 있습니다. 이를 스레드 브로드캐스트라고도 합니다.<img title="thread braodcast" alt="a screenshot depicting the thread broadcast in the UI." src="/images/alerts_screenshot-crop_thread-broadcast.webp"/>

    9. 기본 알림을 사용하거나 사용자 정의 세부 정보로 보강할 수 있습니다. [변수 메뉴](/docs/alerts-applied-intelligence/notifications/message-templates/#variables-menu) 에서 변수를 선택하고 [핸들바 구문](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax) 을 적용하여 페이로드를 보강하십시오.

    10. 사전 정의된 예제 페이로드가 포함된 테스트 공지를 채널로 보내려면 <DNT>**Send test notification**</DNT> 버튼을 클릭하세요. 그러면 선택한 Slack 채널에 메시지가 생성됩니다.
  ...
  >
    ...
    2. 다음 정보를 입력하십시오.
       ...
       * <DNT>
         ...
         (선택사항) `Basic Authentication` 또는 `Bearer Token` 중에서 선택할 수 있습니다.
