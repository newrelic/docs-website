...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="email" title="이메일">
    ...
    6. 데이터를 보강하려면 <DNT>**Additional settings**</DNT> 클릭하세요. NRQL 쿼리를 작성하려면 <DNT>**Enrich your data**</DNT> 활성화하세요.

    7. <DNT>**Save and exit**</DNT> 을(를) 클릭합니다.

    8. 공지 방법으로 <DNT>**Email**</DNT> 선택합니다.
    ...
    9. 알림을 보낼 이메일을 추가합니다. 하나 이상의 수신자를 추가할 수 있습니다.
       ...
       * 기존 대상과 이미 연결된 이메일 주소로 대상을 생성하면 둘을 병합할 수 있는 옵션을 제공하는 IM이 나타납니다. <DNT>**Merge**</DNT> 선택하면 대상이 결합되어 공지가 의도한 수신자에게 효율적으로 라우팅됩니다.
  ...
  <Collapser className="freq-link" id="slack" title="느슨하게">
    ...
    귀하의 Slack 작업공간에는 [뉴킹릭 피규어](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(또는 `one.eu.newrelic` 고객을 위한 [EU 앱](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) )가 설치되어 있어야 합니다. 애플리케이션을 개별적으로 설치하려면 먼저 워크스페이스 관리자가 애플리케이션을 승인해야 합니다. 도움이 필요한 경우 UI에서 다음 단계를 따르세요.

    1. 뉴렐릭에 로그인하세요.

    2. [도움말 센터를](https://one.newrelic.com/help-xp) 방문하세요.

    3. **Create a Support Case** \[지원 사례 생성 을] 클릭합니다.

       ### Slack 대상 설정 [#set-slack-destination]

    4. <DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &amp;gt; Alerts**</DNT> 으로 이동하여 <DNT>**Destinations**</DNT> 클릭한 다음 <DNT>**Slack**</DNT> 선택합니다.

    5. <DNT>**Authenticate in one click**</DNT> 버튼을 클릭하여 Slack 방문 페이지로 이동하고 OAuth2 인증 프로세스를 계속하세요. 필수 워크스페이스에 로그인하지 않은 경우 로그인을 위해 Slack으로 리디렉션됩니다.
    ...
    8. <DNT>**Allow**</DNT> 클릭하고 대상 페이지로 돌아갑니다.
