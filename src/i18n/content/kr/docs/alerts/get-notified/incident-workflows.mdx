...
3. 워크플로를 사용하여 문제를 필터링하고 관련 대상으로 보냅니다. 올바른 문제를 필터링하는 쿼리를 작성할 수 있습니다.
   ...
   <Callout variant="tip">
     `WARNING` 사건에 대해 별도의 알림을 설정하려면 우선순위가 `HIGH` 인 문제로 필터링하세요.
   </Callout>
...
6. 선택 사항: 데이터를 강화합니다.
   ...
   <CollapserGroup>
     <Collapser className="freq-link" id="enrichments" title="데이터를 보강하는 방법">
       현재는 JSON 또는 숫자 출력만 Webhook 및 Jira [대상](/docs/alerts-applied-intelligence/notifications/destinations/) 으로 보낼 수 있습니다. 또한 Slack, PagerDuty, ServiceNow(앱) 및 이메일과 같은 다른 대상으로만 이미지를 보낼 수 있습니다. Webhook 및 Jira에서 더 나은 결과를 얻으려면 `count`, `min` 또는 `max` 와 같은 단일 값 출력이 있는 쿼리를 사용하십시오.
