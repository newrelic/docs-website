...
예를 들어 계획된 유지 관리와 같이 음소거 규칙이 활성화된 경우, 알림이 트리거되어도 알림을 받지 못합니다.

음소거 규칙이 더 이상 활성화되지 않으면 다음과 같은 옵션을 사용할 수 있습니다.

* <DNT>**Close ongoing incidents**</DNT>: 즉, 음소거 규칙이 만료되거나 비활성화된 직후에 인지던트가 발생하거나 계속 열려 있는 경우 알림을 받게 됩니다. 이 기본 설정을 유지하는 것이 좋습니다.

* <DNT>**Continue ongoing incidents**</DNT>: 즉, 음소거 기간 중에 시작된 인시던트가 음소거 규칙이 비활성화된 후에도 계속되면 알림을 받지 못하게 됩니다.
