...
<Steps>
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="검색어를 선택하세요."
      >
        먼저 <DoNotTranslate>**Web transactions time**</DoNotTranslate> 라벨이 붙은 차트로 이동하여 <DoNotTranslate>**create an alert condition**</DoNotTranslate> 클릭합니다.
      ...
      <Collapser
        ...
        title="조건 이름 지정"
      >
        공지 조건에 기술 이름을 부여하는 것이 중요합니다. 이 조건의 이름을 <DoNotTranslate>**response time**</DoNotTranslate> 으로 지정한 다음 완전히 다른 애플리케이션에 대한 또 다른 조건을 만들고 해당 조건에도 <DoNotTranslate>**response time**</DoNotTranslate> 라벨을 붙인다고 가정해 보겠습니다. 이런 일이 발생하면 어떤 조건이 어떤 것인지 구별할 수 없게 됩니다. 따라서 조건에 구체적이고 고유한 이름을 지정해야 합니다. 이 경우 이 조건의 이름을 <DoNotTranslate>**Response time: Example-app**</DoNotTranslate> 로 지정합니다.
      </Collapser>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="이상 임계값(권장)"
      >
        귀하의 팀에서는 이 공지 조건을 작성 중이므로 웹 프로세서 시간이 평소보다 오래 걸릴 경우 알림을 받게 됩니다. 그러나 웹 트랜잭션 시간이 얼마나 길어지는지는 신경 쓰지 않고 트랜잭션 시간이 비정상적으로 작동하는지 알고 싶다고 가정해 보겠습니다. 이 특정 사용 사례의 경우 <DoNotTranslate>**anomaly threshold**</DoNotTranslate> 사용하는 것이 좋습니다. 우리의 이상 감지 기능은 귀하의 데이터를 지속적으로 평가하여 시스템이 정상적으로 작동하는 방식을 이해합니다. 이상 실험 값을 설정하면 웹 프로세서 시간이 예상 성능에서 벗어나는 경우 이상 감지 기능을 사용하여 팀에 알릴 수 있습니다. 웹 프로세서 시간이 비정상적으로 작동하는지 여부만 알고 싶으므로 _모든_ 공지를 원하므로 <DoNotTranslate>**upper and lower**</DoNotTranslate> 선택합니다. 그러나 웹 트랜잭션 시간이 평소보다 _오래_ 걸릴 경우에만 알림을 받으려면 다음을 선택하세요. <DoNotTranslate>**upper only.**</DoNotTranslate>

        다음으로 우선순위 수준을 설정해야 합니다. 우선순위 수준에 따라 인시던트가 발생하는 항목이 결정됩니다. 첫 번째 공지에 대해서는 우선순위 수준을 <DoNotTranslate>**critical**</DoNotTranslate> 으로 설정하는 것이 좋습니다. [공지 조건 문서](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition#threshold-levels) 에서 우선순위 수준에 대해 자세히 알아볼 수 있습니다.

        다음으로, <DoNotTranslate>**critical anomaly threshold breach**</DoNotTranslate> 을 정의하는 것을 선택해야 합니다. 첫 번째 공지의 경우 기본 설정을 사용하고 필요에 따라 필요에 맞게 조정하는 것이 좋습니다. 따라서 "쿼리가 반환한 값이 예측 값에서 벗어난 경우: <DoNotTranslate>**for at least five minutes**</DoNotTranslate> x <DoNotTranslate>**3 standard deviations**</DoNotTranslate>" 인시던트를 열도록 설정을 그대로 둡니다.
      ...
      <Collapser
        ...
        title="정적 임계값"
      >
        이상 가리켜 값, 정적 레버 값과 달리, 기대감은 데이터 세트를 전체적으로 살펴보지 않고 시스템 기록을 기반으로 어떤 동작이 비정상적인지 결정합니다. 대신, 정적 레버값은 시스템이 <DoNotTranslate>**you set**</DoNotTranslate> 기준과 다르게 동작할 때마다 인시던트를 엽니다. 정적 공지 레버 값은 훨씬 더 사용자 정의가 가능하며, 데이터와 찾고 있는 내용에 대해 잘 알고 있는 경우 권장됩니다.
      ...
      >
        때때로 수신 신호가 손실될 수 있으며, 이것이 단순한 지연인지 아니면 더 광범위한 문제를 나타내는 것인지 이해하는 것이 중요합니다. <DoNotTranslate>**loss signal threshold**</DoNotTranslate> 는 신호 손실을 고려하기 전에 마지막 데이터 포인트가 감지된 시간부터 시스템이 기다려야 하는 시간(초)을 알려줍니다. 설정한 시간 제한 이전에 신호가 반환되지 않으면 두 가지 작업 중 하나 또는 둘 다를 선택할 수 있습니다. 즉, 새 인시던트를 열거나 열려 있는 모든 인시던트를 닫을 수 있습니다. 손실된 신호가 이 엔터티의 다른 인시던트를 대체하거나 신호 손실이 예상된 경우 관련 열려 있는 인시던트를 닫을 수 있습니다.

        분실된 신호 레버 값을 설정하려면 시스템에 대한 지식과 이해하려는 내용이 필요합니다. 웹 프로세서 시간의 경우, 뉴렐릭이 1분마다 신호를 수집한다고 가정해 보겠습니다. 신호 손실은 훨씬 더 큰 지연 시간 문제를 나타낼 수 있습니다. 따라서 편안한 수준으로 시간을 설정한 다음 <DoNotTranslate>**open a new lost signal incident**</DoNotTranslate> 확인란을 선택하는 것이 좋습니다.
  ...
  <Step>
    ...
    뉴렐릭은 귀하의 창작물에서 우리 시스템으로 스트리밍되는 데이터를 지속적으로 관찰합니다. 그러나 모든 애플리케이션이 동일한 주파수나 케이던스로 신호를 보내는 것은 아닙니다. 일부 이벤트는 1분에 한 번씩 우리 시스템에 신호를 보내는 반면 다른 이벤트는 하루에 한 번만 뉴렐릭에 데이터를 보고할 수 있습니다. 공지 조건은 <DoNotTranslate>**specific**</DoNotTranslate> 사용 사례를 위해 설계된 <DoNotTranslate>**specific**</DoNotTranslate> 컨테이너입니다. 공지 조건을 생성할 때 이 섹션은 평가 중인 데이터에 대해 가장 적합하게 사용자 정의할 수 있습니다.
    ...
    <CollapserGroup>
      ...
      >
        하루 종일 데이터가 특수에서 뉴렐릭으로 스트리밍됩니다. 인시던트에 대해 해당 데이터를 즉시 평가하는 대신 공지 조건은 <DoNotTranslate>**aggregation window**</DoNotTranslate> 이라고 알려진 기간 동안 데이터를 수집합니다. 추가 지연을 사용하면 창이 집계되기 전에 더 느린 데이터 포인트가 도착할 수 있습니다.
      ...
      <Collapser
        ...
        title="스트리밍 방식"
      >
        일반적으로 <DoNotTranslate>**event flow**</DoNotTranslate> 스트리밍 방법을 사용하는 것이 좋습니다. 이는 시스템에 자주 그리고 꾸준히 들어오는 데이터에 가장 적합합니다. <DoNotTranslate>**event timer**</DoNotTranslate> 선택하는 것이 더 나은 방법일 수 있는 특별한 경우가 있지만 첫 번째 공지에는 기본값인 <DoNotTranslate>**event flow**</DoNotTranslate> 권장합니다. 어떤 스트리밍 방법을 선택해야 하는지 더 잘 이해하려면 이 간단한 비디오를 시청하는 것이 좋습니다.
      ...
      <Collapser
        ...
        title="갭 메우기 전략"
      >
        우리는 웹 프로세서 시간과 관련된 지연시간 문제를 우리 팀에 알리기 위해 공지 조건을 만들고 있습니다. 이 경우 우리의 계약은 지속적으로 뉴렐릭 데이터를 전송합니다. 우리의 특수에서 뉴렐릭으로 지속적인 신호 흐름이 전송되고 있으며 신호에 예상되는 공백이 없으므로 공백 채우기 전략을 선택할 필요가 없습니다. 이 사용 사례와 첫 번째 공지의 경우 <DoNotTranslate>**gap-filling strategy**</DoNotTranslate> 를 <DoNotTranslate>**none**</DoNotTranslate> 로 설정된 상태로 두는 것이 좋습니다.
  ...
  <Step>
    ...
    정책은 인시던트의 분류 시스템입니다. 정책을 생성하면 수신되는 모든 인시던트를 구성하는 도구가 생성됩니다. 뉴렐릭에게 들어오는 모든 정보를 어디로 보낼지, 얼마나 자주 보낼지, 어디로 보낼지 알려주는 정책을 <DoNotTranslate>**[workflows](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/)**</DoNotTranslate> 에 연결할 수 있습니다.
    ...
    <CollapserGroup>
      ...
      >
        ...
        먼저, 정책당 하나의 문제만 첨부하는 정책을 생성할 수 있습니다(기본 옵션). 정책당 하나의 문제로 인해 소음이 줄어들지만 즉각적인 조치가 필요합니다. 그러나 이는 <DoNotTranslate>**Response time: Example app**</DoNotTranslate> 뿐만 아니라 이 정책에 여러 조건을 연결한 경우 무슨 일이 있어도 이 정책의 모든 인시던트가 하나의 단일 문제로 그룹화된다는 것을 의미합니다.

        또는 조건당 하나의 문제를 생성할 수도 있습니다. 이는 <DoNotTranslate>**Response time: Example app**</DoNotTranslate> 조건이 인시던트를 열 때마다 모든 인시던트가 우리 조건과 연결된 하나의 문제로 롤링된다는 것을 의미합니다. 이 특정 사용 사례의 경우 웹 프로세서 시간으로 지연시간 문제를 모니터링하는 주요 목표를 충족하므로 이 옵션을 선택해야 합니다.
