  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser
        ...
        title="정적 임계값"
      >
        이상 가리켜 값, 정적 레버 값과 달리, 기대감은 데이터 세트를 전체적으로 살펴보지 않고 시스템 기록을 기반으로 어떤 동작이 비정상적인지 결정합니다. 대신, 정적 레버값은 시스템이 <DNT>**you set**</DNT> 기준과 다르게 동작할 때마다 인시던트를 엽니다. 정적 공지 레버 값은 훨씬 더 사용자 정의가 가능하며, 데이터와 찾고 있는 내용에 대해 잘 알고 있는 경우 권장됩니다.
