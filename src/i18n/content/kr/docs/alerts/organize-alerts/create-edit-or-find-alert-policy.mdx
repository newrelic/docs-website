...
[정책](/docs/alerts/overview/#concepts-terms) 이란 하나 이상의 조건들을 묶은 것입니다. 이 문서에서는 정책을 생성하는 방법과 기존 정책을 확인하는 방법을 설명합니다. [NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-api-alerts-policies/) (당사의 GraphQL) 또는 [REST API를](/docs/alerts/rest-api-alerts/new-relic-alerts-rest-api/rest-api-calls-new-relic-alerts) 사용할 수도 있습니다.

정책이 공지 조건, 워크플로우 및 기타 구성 요소와 어떻게 상호 작용하고 연관되는지에 대한 설명은 [개념](/docs/alerts/overview/#concepts-terms) 및 [워크플로우를](/docs/alerts/get-notified/alert-event-workflows/) 참조하세요.

## 정책 이름 지정 및 구성 모범 사례 [#best-practices-policies]

뉴렐릭 <InlinePopover type="alerts" />정책은 공지 용어를 일관되고 관리 가능한 단위로 구성하는 데 도움이 됩니다. 단순히 분류하는 것을 넘어, 공지 이벤트 간의 상관관계와 잠재적 문제에 대한 공지 확산에 대한 어느 정도의 통제력을 제공합니다.
...
4. [문제 해결 방식을](/docs/alerts/organize-alerts/specify-when-alerts-create-events) 선택하세요.
5. 정책을 생성하려면 <DNT>**Create &amp; close**</DNT> 선택하거나 정책을 [연결](/docs/alerts/get-notified/alert-event-workflows) 우에 연결하려면 <DNT>**Set up notifications**</DNT> 선택하세요. 새로운 문제가 발생하거나 기존 문제에 대한 업데이트가 있을 경우, *PagerDuty* 또는 *Slack* 과 같은 서비스를 통해 알림을 받게 됩니다.

여기에서 [조건을 정의](/docs/alerts/create-alert/create-alert-condition/alert-conditions) 하고 [설정을 사용하여 공지를 설정할](/docs/alerts/get-notified/alert-event-workflows) 수 있습니다. 나중에 계속 진행하려면 <DNT>**Alert policies**</DNT> 인덱스에서 정책 이름을 선택하십시오.
...
<CollapserGroup>
  ...
  <Collapser id="delete-policy" title="정책 삭제">
    ...
    2. 정책 행에서 줄임표(...) 드롭다운 메뉴를 클릭하고 <DNT>**Delete**</DNT> 선택합니다.
       ...
       삭제된 정책의 경우, [이슈](/docs/alerts/alert-event-management/view-alert-event-details) 및 [공지 이벤트는](/docs/alerts/alert-event-management/view-alert-event-details) 읽기 전용 리소스로 제공됩니다.
  </Collapser>
  ...
  <Collapser id="alert events" title="공지 이벤트가 문제로 롤업되는 방식 변경">
    [공지 이벤트를](/docs/alerts/organize-alerts/specify-when-alerts-create-events/) 이슈별로 그룹화하는 방법을 선택하려면(정책별, 조건별, 그룹, 목표 및 조건별):
  ...
  <Collapser id="link-to-ai" title="공지 이벤트에 대한 컨텍스트 확인">
    뉴렐릭 알림을 사용하면 [공지 이벤트 인텔리전스](/docs/alerts/overview/) 에 조건을 연결할 수 있습니다.
