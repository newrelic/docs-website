...
필터 논리를 추가하면 시스템은 지난 7일간의 인지던트 데이터를 사용하여 자동으로 [시뮬레이션을](#simulations) 실행하여 결정을 적용하기 전에 그 유효성을 검사하는 데 도움을 줍니다.
...
* <DNT>**Simulation**</DNT>: 시뮬레이션 상관관계는 시뮬레이션 조건에서의 관계를 이해하기 위해 두 개의 별도 인지던트를 분석하는 것을 포함합니다. 이러한 인지문제는 동일한 근본 문제에서 비롯될 수도 있고, 서로 다른 문제에서 비롯될 수도 있습니다. 이 연구는 개인 인지던트 간의 잠재적인 원인 요인이나 공유되는 특성을 파악하는 데 중점을 둡니다. 시뮬레이션을 통해 이력 데이터에 대한 상관관계 논리를 적용하기 전에 이를 테스트하고 검증할 수 있습니다.

* <DNT>**Real-time correlation (decisions)**</DNT>: 대조적으로, 구간 상관관계는 목표가 뚜렷한 이슈로, 각 이슈는 잠재적으로 여러 인시던트를 포괄합니다. AI 모니터링은 여러 인지던트에서 패턴을 감지하고 연결하여 잠재적인 문제를 식별하고 보다 효율적인 상관관계를 구축하는 것입니다. 실시간 상관관계는 라이브 데이터 스트림을 활용하여 새로운 문제를 식별하고 대응할 수 있도록 합니다.

### 시뮬레이션 사용 [#simulations]

시뮬레이션은 지난주 데이터에서 두 개의 별도 인지던트를 분석하여 상관관계 논리를 테스트하고, 얼마나 많은 상관관계가 발생했을지 보여줍니다. 이를 통해 문제의 실시간 상관관계에 적용하기 전에 의사 결정 논리를 검증할 수 있습니다. 시뮬레이션할 때 표시되는 결정 미리보기 정보의 세부 내용은 다음과 같습니다.
