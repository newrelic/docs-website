...
일부 조건 유형의 경우, 더 나은 공지 이벤트 응답을 위해 또는 다른 시스템에서 사용할 수 있도록 유용한 정보를 하위 시스템으로 전달하는 설명 템플릿을 정의할 수 있습니다. 사용자 인터페이스(UI) 또는 GraphQL을 사용하여 설명 템플릿을 만들 수 있습니다.
...
일부 [조건 유형의](#requirements) 경우, 조건 및 해당 공지 이벤트에 대한 정보를 하위 항목으로 전달하는 설명 템플릿을 만들 수 있습니다. 설명을 사용하면 다음과 같은 작업을 수행할 수 있습니다.
...
<DNT>**Description template**</DNT> 필드를 사용하면 공지 이벤트가 생성될 때 일반 섹션을 특정 값으로 대체하는 일반 템플릿 구조를 사용할 수 있습니다. 예를 들어, 설명에 다음과 같은 내용을 사용했다고 가정해 보겠습니다.
...
이 조건으로 인해 알림 이벤트가 발생하면 세부 정보가 채워지고 다음과 같은 알림을 받게 됩니다.
...
설명 템플릿에서 사용할 수 있는 속성은 [공지 이벤트](/docs/alerts/create-alert/condition-details/alert-event-attributes) 에 연결된 속성의 하위 집합입니다. 사용 가능한 속성과 사용 방법에 대한 자세한 내용은 다음과 같습니다.

* 공지 이벤트 이벤트 속성: [설명 템플릿에 사용할 수 있는 표준 속성](/docs/alerts/create-alert/condition-details/alert-event-attributes) 의 경우 `{{AN_ATTRIBUTE}}` 형식을 사용합니다. 예를 들어:
...
* 태그 속성: 태그의 경우 `{{tags.TAG_NAME}}` 형식을 사용합니다. 두 가지 유형의 태그가 있습니다.
  ...
  * 부품 관련 태그: <DNT>**For infrastructure conditions only.**</DNT> [분리 관련 태그는](/docs/new-relic-one/use-new-relic-one/core-concepts/tagging-use-tags-organize-group-what-you-monitor#) [부품 탐색기](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer-view-performance-across-apps-services-hosts) 로 이동하여 서비스의 <DNT>**Metadata and tags**</DNT> 아래를 보거나 공지 이벤트 세부 정보를 확인하여 찾을 수 있습니다. 다음은 설명 템플릿에서 태그를 사용하는 예입니다.
