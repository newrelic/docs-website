...
전자 상거래 사이트를 운영하고 사이트에서 404 응답 오류가 발생할 때마다 인시던트를 트리거하는 공지 조건을 만들었다고 가정해 보겠습니다. 제목 템플릿을 사용하면 이 상태에서 인시던트를 쉽게 식별할 수 있습니다.

이 공지 조건의 이름을 `high error rate - checkout` 로 지정하셨습니다. 하지만 인시던트 제목에 오류가 생성되는 위치가 반영되기를 원합니다. 이 경우 이 인시던트에 대한 오류는 결제 엔터티에서 발생했습니다.
...
* **태그 속성**: 태그의 경우 `{{tag.TAG_NAME}}` 형식을 사용합니다.

* **패싯 절 태그**: NRQL 조건이 `FACET` 절을 사용하는 경우 `tag` 형식을 사용하여 제목 템플릿에서 해당 값을 사용할 수 있습니다. 예를 들어 NRQL 쿼리에 `FACET hostName, cluster` 포함된 경우 다음을 사용할 수 있습니다.
