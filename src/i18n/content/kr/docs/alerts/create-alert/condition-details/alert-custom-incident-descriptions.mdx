...
* 태그 속성: 태그의 경우 `{{tag.TAG_NAME}}` 형식을 사용합니다. 두 가지 유형의 태그가 있습니다.
  ...
  * 패싯 절 태그: <DNT>**For NRQL conditions only.**</DNT> NRQL 조건이 `FACET` 절을 사용하는 경우 `tag` 형식을 사용하여 설명 템플릿에서 해당 값을 사용할 수 있습니다. 예를 들어 NRQL 쿼리에 `FACET hostName, cluster` 포함된 경우 다음을 사용할 수 있습니다.
