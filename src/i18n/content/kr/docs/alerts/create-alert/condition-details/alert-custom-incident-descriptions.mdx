...
<DNT>**Custom incident description**</DNT> 필드를 사용하면 인시던트가 생성될 때 일반 섹션을 특정 값으로 대체하는 일반 템플릿 구조를 사용할 수 있습니다. 예를 들어 설명에 다음을 사용했다고 가정해 보겠습니다.
...
<figcaption>
  <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Alert conditions (Policies) > (select a policy) > (select a condition)**</DNT>: 필드를 열려면 <DNT>**+ Add custom incident description**</DNT> 클릭하세요.
</figcaption>
...
* 태그 속성: 태그의 경우 `{{tag.TAG_NAME}}` 형식을 사용합니다. 두 가지 유형의 태그가 있습니다.
  ...
  * 패싯 절 태그: <DNT>**For NRQL conditions only.**</DNT> NRQL 조건이 `FACET` 절을 사용하는 경우 `tag` 형식을 사용하여 사용자 정의 인시던트 설명에서 해당 값을 사용할 수 있습니다. 예를 들어 NRQL 쿼리에 `FACET hostName, cluster` 포함된 경우 다음을 사용할 수 있습니다.
