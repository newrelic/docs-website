...
<Callout title="시사">
  ...
  이 기능은 현재 [사전 출시 정책](/docs/licenses/license-information/referenced-policies/new-relic-pre-release-policy) 에 따른 미리보기 프로그램의 일부로 제공되며, 한정된 프로모션 기간 동안만 제공됩니다. 이 기간이 끝나면 다음과 같은 변경 사항이 발생합니다.

  * 예측 공지 이벤트는 더 이상 UI 에서 사용할 수 없습니다.
  * 공지 조건은 노인값, 문자열의 정적 부분만으로 다시 등록됩니다. 이 재등록은 집계 창을 재설정하여 프로세스 중에 조건의 경제적 한계가 위반될 경우 알림 감지가 지연될 가능성이 있습니다.
</Callout>
