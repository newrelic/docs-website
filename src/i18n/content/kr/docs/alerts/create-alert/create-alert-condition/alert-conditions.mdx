...
<DocTiles>
  <DocTile title="Create your first New Relic alert" path="/docs/tutorial-create-alerts/create-new-relic-alerts/" label={{text: "Start here", color: "orange"}}>초보자를 위한 알림에 대한 집중 강좌 </DocTile><DocTile title="Advanced alert conditions" path="/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions/">이미 공지사항을 설정했다면 고급 설정으로 더욱 깊이 파고들기</DocTile> <DocTile title="Get notified" path="/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/">펠릭스우를 사용하여 캠프에서 비정상적인 동작이 있을 때 알림을 받으세요.</DocTile>
</DocTiles>