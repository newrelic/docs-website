...
* 스마트 알림(공개 프리뷰)을 사용하여 한 번에 여러 엔티티에 표준 커버리지를 적용할 수 있는 [<DNT>**Alerts Overview**</DNT> 페이지](/docs/alerts/smart-alerts-coverage)
...
가이드 모드를 제외한 모든 방법의 경우, 공지 조건을 생성하는 프로세스는 아래 단계에 설명된 것과 *정확히* 동일합니다. 조건을 구성할 때 조건 명명, 태그 추가, 효과적인 임계값 설정 및 노이즈 감소에 대한 지침은 [알림 모범 사례](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices) 를 참조하세요.
