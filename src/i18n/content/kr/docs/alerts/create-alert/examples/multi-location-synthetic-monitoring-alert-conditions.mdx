...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        위치 확인에 실패한 경우, 성공적인 확인이 이루어질 때까지 **(최대 24시간)** 실패한 것으로 간주됩니다. 다중 위치 신세틱스 조건에 대한 평가 버퍼는 24시간의 TTL(time-to-live)을 갖습니다. 예를 들어, 위치 확인에 실패했다가 즉시 다시 사용 가능해질 수 있지만, 예정된 확인이 성공으로 보고될 때까지는 실패한 것으로 간주합니다. 24시간 이내에 복구 신호가 수신되지 않으면 해당 오류는 결국 버퍼에서 삭제됩니다.
      </td>
    </tr>
  </tbody>
</table>
...
알림 REST API를 사용하여 다중 위치 조건을 관리하려면 [REST API 탐색기](https://api.newrelic.com/docs/#/Deprecation%20Notice%20-%20Alerts%20Location%20Failure%20Conditions)를 사용하세요.