...
* 대부분의 AWS CloudWatch 지표는 AWS 지표 스트림(폴링 아님)에서 나옵니다. 주요 예외는 일부 AWS CloudWatch 데이터가 스트리밍이든 폴링이든 상관없이 매우 드물다는 것(예: S3 볼륨 데이터)이며, 이 경우
...
신호가 10분 동안 손실될 때 새 인시던트를 열도록 경보 조건을 설정하면 신호 서비스 손실이 데이터 포인트가 도착하는지 감시합니다. 새 데이터 포인트가 10분 이내에 도착하지 못하면 신호 손실로 인시던트가 시작됩니다.
