...
* 조건의 `id` ([API Explorer:](https://api.newrelic.com/docs/#/Alerts%20Conditions) <DNT>**Alerts Conditions &gt; GET &gt; List**</DNT>에서 사용 가능)
* By default, this endpoint uses the US data center. If your organization hosts data in the EU data center, ensure you are using the proper [API endpoints for EU region accounts](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center#endpoints).
...
2. 정책 ID를 사용하여 해당 정책과 연결된 조건을 반환하는 API를 호출합니다. 네 가지 조건 범주가 있습니다. 범주를 모르는 경우 조건을 찾기 위해 최대 4개의 API 호출이 필요할 수 있습니다.
   ...
   <CollapserGroup>
     <Collapser id="types-conditions-API" title="조건 ID 검색에 대한 세부 정보">
       ...
       * [<InlinePopover type="apm" />, <InlinePopover type="browser" />, 그리고<InlinePopover type="mobile" />](/docs/alerts/new-relic-alerts/rest-api-alerts/rest-api-calls-new-relic-alerts#conditions-list)\
       ...
       * [신세틱 모니터링](/docs/alerts/new-relic-alerts/rest-api-alerts/rest-api-calls-new-relic-alerts#synthetics-conditions-list)\
...
5. 업데이트 API 요청을 통해 편집된 JSON을 제출하여 조건을 업데이트합니다. 우리의 다른 제품에는 다른 API 요청이 필요합니다.
   ...
   <CollapserGroup>
     <Collapser id="update-api-requests" title="업데이트 API 요청에 대한 세부정보">
       ...
       * [APM, 브라우저 및 모바일용 조건](/docs/alerts/new-relic-alerts/rest-api-alerts/rest-api-calls-new-relic-alerts#conditions-update)\
         사용 가능한 조건: `apm_app_metric`, `apm_kt_metric`, `browser_metric` 및 `mobile_metric`\
         [API Explorer PUT&gt;Update link](https://api.newrelic.com/docs/#/Alerts%20Conditions/put_alerts_conditions__condition_id__json)

       * [외부 서비스 조건](/docs/alerts/new-relic-alerts/rest-api-alerts/rest-api-calls-new-relic-alerts#ext-conditions-update)\
         사용 가능한 조건: `apm_external_service`, `mobile_external_service`\
       ...
       * [신세틱 모니터링 조건](/docs/alerts/new-relic-alerts/rest-api-alerts/rest-api-calls-new-relic-alerts#synthetics-conditions-update))\
