...
<Steps>
  <Step>
    ...
    * `host-tower-portland` 심각한 경고 위반이 있습니다.
  ...
  <Step>
    ...
    우리는 CPU 비율을 다루고 있으므로 문제가 앱과 관련된 것인지 아니면 컴퓨터와 관련된 것인지 확인하고 싶습니다. 그렇게 하려면 사이드바의 **Latest events** \[최신 이벤트] 와 비교하여 **Processes running** \[실행 중인 프로세스] 테이블을 확인해야 합니다. 이 단계에서는 시스템에 직접 변경이 이루어졌기 때문에 문제가 발생하는지, 아니면 프로세스에서 리소스가 소모되는지 여부를 명확하게 합니다.
  ...
  <Step>
    ...
    * 더 많은 최종 사용자가 해당 구성 요소에 액세스하고 시스템에 스트레스를 가중시키고 있습니다. 이 경우 해당 로드를 충족하기 위해 더 많은 리소스를 프로비저닝해야 합니다.
    ...
    `host-tower-portland` 에 대한 요약 페이지를 보면 서로 다른 차트 간의 시계열 패턴을 연관시켜 이 상황에 어떤 시나리오가 적용되는지 식별할 수 있습니다. 네트워크 트래픽을 인프라 측정항목과 비교해 보겠습니다.

    * 이 경우 **Network traffic** \[네트워크 트래픽] 그래프는 CPU 사용량, 메모리 사용량 등에 대한 지표 그래프와 유사한 추세를 보일 것으로 예상됩니다.
    * 리소스가 로드가 아닌 앱 문제와 관련된 경우 네트워크 시계열은 급증이나 최저점 없이 일반적인 방식으로 추세를 나타냅니다.
    * 그러나 로드가 높은 CPU의 원인인 경우 네트워크 시계열이 다른 측정항목 그래프의 추세를 미러링하는 것을 확인할 수 있습니다.

    **CPU usage** \[CPU 사용량] 과 **Network traffic** \[네트워크 트래픽을] 비교해 보겠습니다.
    ...
    * 그러나 네트워크 트래픽은 일관된 동작을 보이고 있습니다. 리소스가 고갈되어도 전반적인 동작은 시간이 지나도 변하지 않는 것 같습니다. 대신 시계열에는 정기적인 최고점과 최저점이 표시됩니다.
  ...
  <Step>
    ...
    3. 보기 이름을 지정한 다음 **Save** \[저장 을] 클릭합니다.
