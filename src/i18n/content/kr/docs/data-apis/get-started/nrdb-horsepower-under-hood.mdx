...
뉴렐릭의 내부에는 모든 것을 구동하는 엔진인 뉴렐릭 데이터베이스(NRDB)가 있습니다. 이 리소스에서는 NRDB가 귀하의 옵저버빌리티 목표 달성에 어떻게 도움이 되는지 설명합니다.
...
1. <DNT>
   ...
   클라우드에 호스팅되는 NRDB의 분산 아키텍처는 사실상 무제한 확장이 가능한 용량을 갖추고 있습니다.

2. <DNT>
     **Monitoring and analysis:**
   </DNT>

   이러한 이중 목적을 염두에 두고 NRDB는 운영 모니터링과 데이터 분석을 동일하게 처리합니다. 즉, NRDB는 속도 저하 없이 실시간 경고, 초고속 쿼리 및 차트 작성 기능을 제공하는 동시에 엄청난 양의 데이터를 수집할 수 있습니다.

3. <DNT>
     **Resources when you need them:**
   </DNT>

   수만 명의 고객을 지원하는 멀티 테넌트 시스템인 NRDB는 필요할 때 필요한 리소스를 제공합니다(단일 테넌트 시스템은 비교할 수 없음).
