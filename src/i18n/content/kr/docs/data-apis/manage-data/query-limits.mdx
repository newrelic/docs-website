...
한도에 도달했을 때의 대응은 몇 가지 요인에 따라 달라집니다. 한도 [유형, 한도를 초과](/docs/telemetry-data-platform/manage-data/view-system-limits/#violations) 하는 기간, 빈도, 금액입니다. 한도를 초과한다고 해서 항상 데이터 삭제, 트래픽 거부 또는 하루 종일 데이터 꺼짐과 같은 제한 이벤트가 발생하는 것은 아닙니다. 때때로 제한을 적용하기 전에 작은 버퍼를 허용합니다. 즉, 100%를 초과하여 소비되는 리소스는 언제든지 제한 영향의 위험이 있습니다.

많은 비율 제한이 비례적으로 적용됩니다. 즉, 제한을 거의 초과하지 않는 경우 200%를 초과하는 경우보다 조치를 덜 취합니다.

제한 측정항목은 데이터를 해당 `dataType` 또는 `limitName` API로 보내는 경우에만 표시됩니다. 예를 들어 Metric API를 통해 데이터를 보내면 Metric API 리소스 메트릭이 표시되지만 APM 데이터를 보내지 않으면 APM 리소스 메트릭이 표시되지 않습니다.

<Callout variant="tip">
  영향 메트릭은 영향에 관계없이 생성됩니다. 영향이 없으면 0이 표시됩니다.
</Callout>

[`NrIntegrationError` 이벤트](/docs/telemetry-data-platform/manage-data/nrintegrationerror) 는 영향을 받을 때 생성되며 제한 이벤트가 발생하는지 빠르게 확인할 수 있는 좋은 방법입니다. 자세한 내용은 [시스템 제한 보기](/docs/telemetry-data-platform/manage-data/view-system-limits/) 를 참조하십시오.
...
다음 쿼리를 사용하여 이 대시보드를 만들었습니다. [New Relic](https://one.newrelic.com/) 에서 이와 같은 대시보드를 만들려면 대시보드를 선택한 다음 **대시** **보드 만들기** 를 선택합니다. 그런 다음 정기적으로 모니터링하려는 각 쿼리에 대해 새 차트를 추가합니다. 이러한 쿼리에 포함된 세 가지 제한 메트릭은 아래의 별도 섹션에 설명되어 있습니다.
