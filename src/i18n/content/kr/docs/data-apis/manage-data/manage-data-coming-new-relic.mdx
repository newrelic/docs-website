...
New Relic에 데이터를 보고하면 수신한 데이터를 처리하고 데이터 삭제 및 변환 규칙을 적용합니다. 그런 다음 JSON과 같은 표준 형식으로 데이터를 나타내는 데 필요한 바이트 수를 계산합니다. [New Relic One 가격 책정 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-users/pricing-billing) 을 사용 중인 경우 월별 무료 금액을 초과하거나 초과하는 데이터베이스에 기록된 바이트 수에 대해 요금이 부과됩니다.

데이터 수집 비용을 추정하려는 경우 데이터 수집 [추정](/docs/data-apis/manage-data/calculate-data-ingest) 을 참조하십시오.
...
<Callout variant="tip">
  복잡한 조직의 데이터 수집 관리에 대한 자세한 내용은 [데이터 수집 거버넌스](/docs/new-relic-solutions/observability-maturity/operational-efficiency/-data-governance) 를 참조하세요.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        일반 `Mobile` 이벤트, `MobileRequestError` , `MobileBreadcrumb` , `MobileSession` , `MobileHandledException` , `MobileCrash` 를 포함한 [모바일 이벤트](/docs/insights/insights-data-sources/default-data/mobile-default-events-insights) .
...
데이터 수집 UI 페이지에서 사용량을 보다 자세히 분석할 수 있습니다. 수집된 데이터와 데이터의 출처 및 시간이 지남에 따라 어떻게 변하는지 이해하는 데 시간을 할애하면 가치가 있을 수 있습니다. 수집 기준선을 알게 되며 수집 스파이크와 같은 이상 현상을 보다 쉽게 발견하고 출처를 이해할 수 있습니다.
...
## 데이터 수집 조정 [#adjust-ingest]

다음은 데이터 관리에 대한 몇 가지 아이디어입니다.
...
* 트랜잭션 이벤트에 대한 [샘플링 속도](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#Transaction_Events) 를 구성합니다.
