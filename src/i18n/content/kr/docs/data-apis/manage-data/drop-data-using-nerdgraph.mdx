...
데이터 삭제 규칙을 사용하여 New Relic 조직에 저장하지 않으려는 데이터 유형을 지정할 수 있습니다. 이 데이터는 [데이터 수집](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing) 또는 수집 청구에 포함되지 않습니다.

이 NerdBytes 비디오(7:09분)에서 자세히 알아보세요.
...
* 다음 주의 사항이[있는 기본 인프라 모니터링 이벤트](/docs/infrastructure/manage-your-data/data-instrumentation/default-infrastructure-monitoring-data) 및 [인프라 통합](/docs/integrations/infrastructure-integrations/get-started/introduction-infrastructure-integrations) 이벤트:

  * 원시가 아닌 집계된 `SystemSample`, `ProcessSample`, `NetworkSample` 및 `StorageSample` 은 삭제할 수 없습니다(자세한 내용은 [데이터 보존](/docs/data-apis/manage-data/manage-data-retention/#infrastructure-data)참조). 이 데이터는 데이터 수집에 포함되지 않습니다.
  * 원시 인프라 데이터는 경고에 사용되므로 이러한 이벤트를 삭제하는 경우 기간이 59분보다 긴 차트(예: 경고 조건 미리 보기 차트)에서 데이터를 보더라도 해당 이벤트에 대해 경고할 수 없습니다.
...
삭제 규칙을 사용하여 이미 수집 및 저장된 데이터를 삭제할 수 없습니다. 해당 데이터는 [편집하거나 삭제할 수 없습니다](/docs/telemetry-data-platform/ingest-manage-data/manage-data/manage-data-retention#data-deletion).
