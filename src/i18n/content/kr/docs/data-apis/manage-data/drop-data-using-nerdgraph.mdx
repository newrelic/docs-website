...
* [차원 측정항목](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#dimensional-metrics) ( `Metric` 데이터 유형). 몇 가지 주의 사항:
...
* `DROP_ATTRIBUTES` "raw" 속성이 있는 `SELECT` 사용해야 합니다(속성 [, 기능이](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#functions) 적용되지 않음). 이는 또한 `SELECT *` 사용할 수 없음을 의미합니다. 또한 해당 데이터 유형 및

  <DNT>
    **cannot be dropped**
  </DNT>

  (예: 이벤트 데이터의 `timestamp` )에 필수적인 몇 가지 속성이 있습니다. 이를 포함하면 등록이 실패합니다.
