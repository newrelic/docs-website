...
[New Relic One 가격 책정 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 을 사용하는 New Relic 조직의 경우 사용 중인 에디션( [Standard, Pro 또는 Enterprise](https://newrelic.com/pricing) ), 청구 가능한 사용자 수, 수집하는 데이터 양과 같은 여러 요인이 비용에 영향을 미칩니다. 이 리소스는 New Relic 데이터 수집 비용을 추정하는 데 도움이 됩니다.
...
데이터 수집 속도는 모니터링 대상의 종류, 사용되는 기능, 모니터링되는 애플리케이션의 동작 등에 따라 New Relic 조직마다 다를 수 있습니다. 이러한 변동성을 감안할 때 가장 좋은 방법은 테스트 New Relic 계정을 설정한 다음 이를 통해 사용량을 추정하는 것입니다. 테스트 계정을 설정할 수 없거나 설정하지 않으려면 [비용 추정기 스프레드시트](#cost-estimator) 를 사용하십시오.
