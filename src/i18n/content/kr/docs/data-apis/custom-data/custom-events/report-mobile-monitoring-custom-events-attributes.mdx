...
모바일 에이전트 SDK를 사용하여 기본 모바일 모니터링 이벤트에 대한 사용자 정의 세션 수준 속성을 생성할 수 있습니다. 예를 들어 iOS 또는 Android 앱의 일부에 대한 `username` 속성을 기록하려면 [`setAttribute` API를](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/create-attribute) 사용합니다. 이러한 속성은 세션 관련 정보이며 여러 모바일 이벤트 유형에서 공유됩니다.

[`recordCustomEvent` API를](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-custom-events)사용하여 완전히 새로운 맞춤 이벤트 유형을 생성하고 고유한 맞춤 속성을 할당할 수도 있습니다.
