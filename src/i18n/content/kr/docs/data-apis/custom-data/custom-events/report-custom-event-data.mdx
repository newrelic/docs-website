...
사용자 정의 속성은 기존 이벤트에 중요한 비즈니스 및 운영 컨텍스트를 추가하는 데 자주 사용됩니다. 예를 들어 New Relic의 경우 <InlinePopover type="browser"/>, 느리거나 실패한 요청과 관련된 사용자 이름을 추적하기 위해 사용자 정의 속성을 생성할 수 있습니다. 그러면 쿼리와 사용자 정의 차트를 만들어 해당 데이터를 분석할 수 있습니다.

사용자 정의 속성은 New Relic 솔루션(예: APM, 브라우저, <InlinePopover type="mobile"/>, 인프라 및 종합 모니터링) 및 기존 이벤트를 자신만의 메타데이터로 꾸미고 싶어합니다.
