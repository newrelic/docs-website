...
<CollapserGroup>
      ...
      <tbody>
        <tr id="xml-java">
          ...
          <td>
            ...
            4. [뉴렐릭 UI를](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-quickly-customize-your-java-instrumentation) 사용하여 XML 측정, 로그 파일을 정의합니다. 이를 위해서는

               <DoNotTranslate>**newrelic.yml**</DoNotTranslate>

               의 `common:` 블록에 추가 구성이 필요할 수 있습니다. 자세한 내용은 [측정, 로그아웃 옵션](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-instrument-ui#options) 아래의
  ...
  >
    ...
    1. 속성을 기록하려는 각 메소드에 대해 [`AddCustomAttribute`](/docs/apm/agents/net-agent/net-agent-api/itransaction/#addcustomattribute) 호출하세요.
  ...
  <Collapser
    ...
    title="루비"
  >
    Ruby에서는 사용자 정의 속성 컬렉션이 기본적으로 활성화되어 있습니다. 맞춤 속성을 수집하려면 [`add_custom_attributes`](http://www.rubydoc.info/github/newrelic/newrelic-ruby-agent/NewRelic/Agent#add_custom_attributes-instance_method) 메소드를 호출하세요. 예를 들어, `@user_id` 이라는 변수를 기록하려면 상위 메소드에 다음 코드를 포함하십시오.
...
* <InlinePopover type="mobile"/>
  ...
  사용자 정의 데이터에 대한 개요는 [사용자 정의 대시보드 및 속성 삽입을](/docs/insights/insights-data-sources/custom-events/insert-custom-events-attributes-mobile-data)참조하세요.
