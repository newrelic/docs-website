...
<CollapserGroup>
  ...
  <Collapser id="ruby-att" title="루비">
    Ruby에서는 사용자 정의 속성 컬렉션이 기본적으로 활성화되어 있습니다. 맞춤 속성을 수집하려면 [`add_custom_attributes`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#add_custom_attributes-instance_method) 메소드를 호출하세요. 예를 들어, `@user_id` 이라는 변수를 기록하려면 상위 메소드에 다음 코드를 포함하십시오.
