...
[쿼리 및 차트 작성](/docs/using-new-relic/data/understand-data/query-new-relic-data) 에 사용할 수 있는 사용자 정의 APM 이벤트 및 [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute) 을 추가할 수 있습니다. 이것은 [맞춤 데이터를 보고](/docs/insights/insights-data-sources/custom-data/send-custom-data-insights) 하는 여러 방법 중 하나입니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="C SDK"
  >
    C SDK에서 모니터링하는 앱에 맞춤 이벤트를 추가하려면 트랜잭션을 시작하고 `newrelic_create_custom_event` 및 `newrelic_record_custom_event` 함수를 사용하세요. 자세한 내용 [은 C SDK API 사용 가이드를](/docs/agents/c-sdk/instrumentation/guide-using-c-sdk-api#custom-events) 참조하세요. 그런 다음 C SDK 앱에 대한 [사용자 지정 속성](/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes#c-sdk-att) 을 추가할 수 있습니다.
  </Collapser>
  ...
  >
    ...
    첫 번째 인수는 이벤트 유형의 이름을 정의하고 두 번째 인수는 사용자 지정 이벤트에 대한 특성이 있는 IEnumerable입니다. 생성하는 고유한 이벤트 유형 이름의 수를 제한하고 이러한 이름을 동적으로 생성하지 않도록 하십시오. 이벤트 유형 이름에 대한 제한 사항은 [제한 및 제한된 문자](/docs/telemetry-data-platform/custom-data/custom-events/data-requirements-limits-custom-event-data) 및 [NRQL 예약어](/docs/insights/event-data-sources/custom-events/data-requirements-limits-custom-event-data/#reserved-words) 에 대한 설명서를 참조하십시오.
  ...
  >
    ...
    일반적으로 특정 기간과 관련된 이벤트 기반 지표를 기록하려면 recordCustomEvent를 사용하십시오. eventType은 255자 미만의 영숫자 문자열이어야 합니다. 속성은 키 및 값 쌍의 개체여야 합니다. 키는 255자 미만이어야 하고 값은 문자열, 숫자 또는 부울이어야 합니다. 이벤트 유형 이름에 대한 제한 사항은 [제한 및 제한된 문자](/docs/telemetry-data-platform/custom-data/custom-events/data-requirements-limits-custom-event-data) 및 [NRQL 예약어](/docs/insights/event-data-sources/custom-events/data-requirements-limits-custom-event-data/#reserved-words) 에 대한 설명서를 참조하십시오.
