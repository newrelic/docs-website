...
<CollapserGroup>
  ...
  >
    APM 에이전트를 통해 APM `Transaction` 이벤트에 사용자 정의 속성을 추가한 경우 해당 사용자 정의 속성을 `PageView` 이벤트에 자동으로 전달할 수 있습니다.
       ...
       <table>
         ...
         <tbody>
           ...
           <tr>
             ...
             <td>
               앱 `newrelicjs` 구성 파일의 `browser_monitoring: {` 섹션에 [`attributes: {enabled: true}`](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#browser-debug-enabled) 을 추가합니다.
             </td>
           </tr>
