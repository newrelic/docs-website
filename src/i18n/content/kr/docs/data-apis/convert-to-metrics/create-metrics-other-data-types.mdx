...
뉴렐릭의 [지표 API 서비스를](/docs/introduction-events-metrics-api) 사용하여 이벤트, 로그, 범위 등 다른 유형의 데이터에서 [지표를](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type) 생성하기 위한 규칙을 정의하세요. 권장사항: 시작하기 전에 [규칙 생성에 대한 요구사항과 팁을](/docs/telemetry-data-platform/ingest-manage-data/convert-data-metrics/creating-metric-rules-requirements-tips) 검토하세요.
...
3. [고유한 메트릭 이름/속성-값 조합의 카디널리티 제한에](#attributes-limit) 따라 메트릭에 연결할 속성을 결정합니다.
   ...
   권장 사항: 별도의 쿼리를 실행하여 30초 동안 최대 카디널리티가 50,000을 초과하지 않는지 확인하세요. 예를 들어 다음 쿼리는 `awsRegion`, `awsAvailabilityZone` 및 `commandName` 속성을 포함할 때 `ProcessSample` 이벤트에 대해 지난 3시간 동안 30초 동안 발생한 최대 카디널리티를 찾습니다.
...
<CollapserGroup>
  <Collapser id="api-examoke" title="NerdGraph API 요청의 예">
  ...
  <Collapser id="example-response" title="NerdGraph API 응답의 예">
    다음은 반환된 응답의 예입니다.
...
<CollapserGroup>
  <Collapser id="summary" title="요약 메트릭 예시">
  ...
  <Collapser id="uniqueCount" title="카운트 메트릭 예시">
  ...
  <Collapser id="distribution" title="분포 메트릭 예시">
