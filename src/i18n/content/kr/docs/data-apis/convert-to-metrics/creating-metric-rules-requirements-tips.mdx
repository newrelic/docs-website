...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        각 기간(현재 1분)에 대한 요약 지표 데이터 포인트를 생성합니다. NRQL 쿼리가 요약 지표 유형(예: `average`, `sum`, `min` 또는 `max`)에서 지원하는 [변수자 함수를](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#functions) 사용하는 경우 이 옵션을 사용하세요.
    ...
    <tr>
      ...
      <td>
        각 1분 기간에 대한 분포 지수 데이터 포인트를 생성합니다. NRQL 쿼리가 `percentile`, `histogram`, `min`, `max`, `average`, `sum`, `count` 등의 [변수자 함수를](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#functions) 사용하는 경우 이를 사용하세요. 관심 있는 속성만 인수로 사용하고 `percentile` 또는 `histogram` 의 나머지 인수는 삭제합니다. 생성된 지표는 `percentile` 또는 `histogram` 의 모든 인수를 지원합니다.
    ...
    <tr>
      ...
      <td>
        ...
        <DNT>**Example:**</DNT> `foo`이라는 프로세서를 계산하는 `foo.count` 이라는 메트릭을 생성하려는 경우 NRQL 다음과 같습니다.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <DNT>**Example:**</DNT> 지표 이름에 가용성 영역을 포함하면 해당 지표에 대해 모든 가용성 영역에 대한 결과를 볼 수 없다는 의미입니다.
      </td>
    </tr>
