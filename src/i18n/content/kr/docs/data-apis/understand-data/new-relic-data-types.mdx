...
<CollapserGroup>
  ...
  >
    ...
    예를 들어 특정 트랜잭션에 소요된 시간에 대한 <InlinePopover type="apm"/>메트릭 타임슬라이스의 이름은 `WebTransaction/URI/foo`이며, 오전 10시 20분에서 10시 21분 사이의 1분 타임슬라이스에 대해 응답 시간이 0.793일 수 있습니다. 이러한 메트릭은 일반적으로 `<category>/<class>/<method>` 같은 패턴을 따릅니다.
