...
뉴렐릭 플랫폼은 완전하고 효과적인 시스템 모니터링에 필요하다고 생각하는 4가지 기본 텔레메트리 데이터 유형인 [메트릭](#metrics), [이벤트](#event-data), [로그](#log-data) 및 [트레이스](#trace-data)(옵저버빌리티 업계에서는 ‘MELT’라고 불림)를 기반으로 구축되었습니다. [무료 뉴렐릭 계정을 신청](https://newrelic.com/signup)하고 모니터링 서비스를 [설치](/docs/using-new-relic/cross-product-functions/install-configure/install-new-relic/)하면, 바로 데이터를 활용할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    뉴렐릭에서 이 메트릭 데이터는 [`Metric`](/docs/data-ingest-apis/get-data-new-relic/metric-api/view-query-you-metric-data) 데이터 유형에 연결됩니다. 이는 뉴렐릭의 기본 메트릭 데이터 유형이며 다음을 포함한 많은 툴에서 사용됩니다.
    ...
    * [메트릭 API](/docs/introduction-new-relic-metric-api)(위 툴들에서 사용되는 기본 API) 및 [Telemetry SDK](/docs/telemetry-sdks-send-custom-telemetry-data-new-relic)
    * [이벤트-메트릭 서비스](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-events-metrics-service)

    다른 메트릭 데이터 유형 중 일부는 디멘셔널 메트릭으로 쿼리할 수 있습니다. 예:
    ...
    * [일부 인프라 데이터](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/query-infrastructure-dimensional-metrics-nrql)
      ...
      이 데이터를 쿼리하고 [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)(‘디멘션’)을 보려면 다음과 같은 NRQL 쿼리를 사용할 수 있습니다.
      ...
      이 데이터에 대한 보다 자세한 내용은 [`Metric`데이터 유형](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type)를 확인하시기 바랍니다. 이 데이터가 수집 및 저장되는 방법은 [Metric API 문서](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api)를, 쿼리에 대한 팁은 [메트릭 쿼리 예제](/docs/data-ingest-apis/get-data-new-relic/metric-api/view-query-you-metric-data)를 참조하십시오.
  </Collapser>
  ...
  >
    ...
    메트릭 타임슬라이스 데이터는 경량 데이터 유형이며 [디멘셔널 메트릭](#metric-time-series)에 있는 세부 정보가 부족합니다.
      ...
      <CollapserGroup>
        ...
        >
          ...
          **ActiveRecord**
          ...
          ActiveRecord는 Ruby on Rails 애플리케이션에서 사용되는 객체 관계형 매핑 API입니다. 여기에 표시된 메트릭은 ActiveRecord의 `find` 및 `save` 메서드의 성능을 측정합니다.
          ...
          보다 자세한 내용은 [ActiveRecord용 API 문서](http://api.rubyonrails.org/classes/ActiveRecord/Base.html)를 참조하십시오.
          ...
          **컨트롤러**
          ...
          보다 자세한 내용은 [ActionController용 API 문서](http://api.rubyonrails.org/classes/ActionController/Base.html)를 참조하십시오.
          ...
          **View**
          ...
          ActionView는 html 페이지 또는 xml 문서 같은 http 요청에 대한 응답인 출력을 렌더링하는 데 사용되는 Rails의 패키지입니다. View는 요청을 처리하는 컨트롤러에 의해 렌더링됩니다.

          View 메트릭이 컨트롤러 응답 시간의 많은 부분을 나타내는 경우, View 템플릿 자체 내에서 많은 데이터베이스 작업이 수행되고 있음을 의미할 수 있습니다.
          ...
          보다 자세한 내용은 [ActionView용 API 문서](http://api.rubyonrails.org/classes/ActionView/Base.html)를 참조하십시오.
        </Collapser>
      </CollapserGroup>
  </Collapser>
