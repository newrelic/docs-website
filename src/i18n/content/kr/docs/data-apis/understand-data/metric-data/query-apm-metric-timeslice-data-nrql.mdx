...
우리는 [여러 가지 방법으로 측정항목을](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics) 보고합니다. **metric timeslice data** \[메트릭 타임슬라이스 데이터] 라고 부르는 다양한 메트릭 데이터 중 하나입니다. 이는 APM에서 많은 차트를 생성하는 데 사용되는 데이터 유형입니다. <InlinePopover type="mobile"/>및 브라우저(자세한 내용은 [메트릭 타임슬라이스 데이터](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data) 참조)
