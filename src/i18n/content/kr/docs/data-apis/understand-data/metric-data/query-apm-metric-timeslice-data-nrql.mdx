...
뉴렐릭은 [여러 가지 방법으로 지표를](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics) 보고합니다. <DoNotTranslate>**metric timeslice data**</DoNotTranslate> 이라고 부르는 다양한 측정항목 데이터 중 하나입니다. 이는 APM, <InlinePopover type="mobile"/>및 브라우저에서 많은 차트를 생성하는 데 사용되는 데이터 유형입니다(자세한 내용은 [지표 타임슬라이스 데이터](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data) 참조).
