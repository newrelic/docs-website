...
[메트릭 API](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api) 를 통해 New Relic에 메트릭이 보고되면(해당 API를 사용하는 통합 포함) 데이터는 [`Metric` 데이터 유형](/docs/using-new-relic/data/understand-data/new-relic-data-types#dimensional-metrics) 으로 보고되며 쿼리에 사용할 수 있습니다.
...
메트릭을 쿼리하려면 다음 쿼리 형식을 사용합니다.

```
FROM Metric SELECT function(metric_name) WHERE attribute=value FACET attribute TIMESERIES
```

다음은 각 [측정항목 유형](/docs/report-metrics-metric-api#payload-metrics) 에 지원되는 기능입니다.
