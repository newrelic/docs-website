...
메트릭에는 여러 필드가 연결되어 있으며 각 필드에는 고유한 값이 있습니다. 쿼리를 사용하여 이러한 값 중 하나를 추출하려면 `getField()` 를 사용하여 수행할 수 있습니다.

예를 들어 측정항목 내에서 단일 값을 사용하여 `WHERE` 절에서 비교를 수행하려는 경우 `getField(metricName, field)` 또는 `metricName[field]` 를 사용할 수 있습니다.
