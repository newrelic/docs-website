...
[여러 유형의 `Metric` 데이터](/docs/data-apis/understand-data/metric-data/metric-data-type/#metric-types) (예: `gauge` 및 `count`)가 있으며 각 유형에는 여러 개의 연관된 <DoNotTranslate>**fields**</DoNotTranslate> 이 있습니다. 사용 가능한 필드 유형에 대한 자세한 내용은 [`getField()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#func-getfield) 참조하세요.
