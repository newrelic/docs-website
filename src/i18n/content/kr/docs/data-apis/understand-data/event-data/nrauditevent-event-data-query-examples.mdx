...
<CollapserGroup>
  ...
  <Collapser
    id="synth-changes"
    title="합성: 모니터에 어떤 변경 사항이 있습니까?"
  >
    특정 기간 동안 Synthetics 모니터 업데이트를 쿼리하려면 쿼리에 [`actionIdentifier`](/attribute-dictionary/nrauditevent/actionidentifier) 속성을 포함하세요. 예를 들어:
    ...
    이 합성 기능에 대한 자세한 내용은 [합성 감사 로그](/docs/synthetics/new-relic-synthetics/administration/audit-synthetics-account-changes) 를 참조하십시오.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser
    id="synth-user"
    title="합성: 특정 사용자가 만든 모니터는 무엇입니까?"
  >
    특정 사용자가 수행한 Synthetics 모니터 업데이트를 쿼리하려면 쿼리에 [`actionIdentifier`](/attribute-dictionary/nrauditevent/actionidentifier) 및 [`actorEmail`](/attribute-dictionary/nrauditevent/actoremail) 속성을 포함합니다. 예를 들어:
