...
Metric API로 생성된 측정항목에 대해 경고하려면 [NRQL 경고 조건을](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) 사용하세요. 조건을 정의할 때 NRQL 카테고리를 선택한 다음 `FROM Metric ...` NRQL 쿼리 구문을 사용하여 표현합니다. 이러한 경고 조건을 생성하면 <InlinePopover type="alerts"/>사용 가능한 가장 세밀한 데이터(원시 메트릭 데이터 포인트)를 자동으로 사용하여 경고를 평가합니다.
