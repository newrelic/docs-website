...
3. 유효한 에포크 타임스탬프로 `timestamp` 을 업데이트합니다. fix(Metric API): 불필요한 들여쓰기 제거 이 예는 `memory.heap` 이라는 측정항목에 대한 단일 측정항목 데이터 포인트를 생성하지만 [측정항목 유형](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type) 을 지정하거나 [선택적 `common` 블록](#optional-map-attributes) 을 추가하여 추가 속성 또는 데이터 포인트를 생성할 수 있습니다.
