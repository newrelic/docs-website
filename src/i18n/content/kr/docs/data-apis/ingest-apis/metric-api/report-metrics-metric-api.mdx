...
HTTP POST 요청을 통해 뉴렐릭에 지표 데이터를 제출합니다. 각 요청을 메트릭에 대한 `name`, `timestamp`, `value` 로 구성된 하나 이상의 메트릭 데이터 포인트로 구성합니다.

다음 예에 따라 첫 번째 메트릭 데이터 포인트를 New Relic으로 보냅니다.

1. 데이터를 보고하려는 계정의 <InlinePopover type="licenseKey" />를 가져옵니다.

2. 라이선스 키를 다음 JSON에 삽입한 다음 JSON을 [엔드포인트](#api-endpoint) 로 보냅니다.

3. `timestamp` 의 경우 `INSERT_CURRENT_TIMESTAMP` 유효한 [에포크 타임스탬프](#json-payload-keys) 로 바꾸세요. 이 예제에서는 `memory.heap` 라는 이름의 메트릭에 대해 단일 메트릭 데이터 포인트를 생성하지만 [메트릭 유형을](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type) 지정하거나 [선택적 `common` 블록을](#optional-map-attributes) 추가하여 추가 속성이나 데이터 포인트를 생성할 수 있습니다.
...
<Callout variant="tip">
  조직이 EU 데이터 센터에서 데이터를 호스팅하는 경우 [EU 지역 엔드포인트를](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center#endpoints) 사용하고 있는지 확인하세요. 이 API의 경우 EU 엔드포인트는 다음과 같습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required (usually set automatically by the HTTP client)**</DNT>. 청크 인코딩으로 전송되지 않는 한 옥텟(8비트 바이트) 단위의 요청 본문 길이입니다. 이 헤더는 일반적으로 데이터를 보내는 기본 HTTP 클라이언트에 의해 기본적으로 설정되며 대부분의 경우 최종 사용자의 추가 작업이 필요하지 않습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required.**</DNT> 데이터를 보고하려는 계정의 <InlinePopover type="licenseKey" />입니다. 헤더와 쿼리 활동 모두로 제공되는 경우 값이 일치해야 합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required if GZIP.**</DNT> 값은 `GZIP` 또는 `Identity.` 이어야 합니다. 값이 없으면 `Identity` 가 가정됩니다.
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="json-format-example" title="2개의 메트릭을 생성하는 JSON 페이로드">
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. 메트릭의 이름입니다. 값은 255자 미만이어야 합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. [지표 종류](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type) 에 따라 값이 달라집니다. `gauge` 및 `count` 의 경우 값은 단일 숫자여야 합니다. `summary` 의 경우 값은 개수, 합계, 최소값 및 최대값을 지정하는 키/값 쌍이 있는 맵이어야 합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. [Unix 시간](https://currentmillis.com/) 으로 메트릭의 시작 시간입니다. 기본값은 UTC 시간대를 사용합니다. 이 필드는 초, 마이크로초, 나노초도 지원합니다. 그러나 데이터는 저장 및 쿼리를 위해 밀리초로 변환됩니다. 보고된 시간을 기준으로 과거 48시간 이상 또는 미래 24시간 이상 타임스탬프가 있는 메트릭은 삭제됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required for `count` and `summary` [metric types](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type)**</DNT>. 기간의 길이입니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        추천합니다. 이는 [지원되는 지표 유형](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type) 중 하나여야 합니다. 유형을 지정하지 않으면 기본값은 `gauge` 입니다.
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="metric-types-example" title="세 가지 측정항목 유형이 있는 JSON 페이로드">
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required for `count` and `summary`.**</DNT>기간의 길이입니다.
      </td>
    </tr>
