...
TCP 포트 443의 아웃바운드 연결이 해당 [CIDR 범위](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure) 와 일치하는 [지역](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)으로 허용되는지 확인합니다. 선호하는 설정 방법은 DNS 이름 `insights-collector.newrelic.com` (미국, 기본값), `insights-collector.eu01.nr-data.net` (유럽) 또는 `insights-collector.jp.nr-data.net` (일본)를 사용하는 것입니다.
...
<CollapserGroup>
  <Collapser id="ruby-example" title="Ruby 앱에서 이벤트 API 호출">
...
<Callout variant="tip">
  조직이 EU 또는 JP 데이터센터에서 데이터를 호스팅하는 경우 적절한 지역 엔드포인트를 사용하고 있는지 확인하십시오. [EU 지역 엔드포인트](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#eu-endpoints) 또는 [JP 지역 엔드포인트](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#jp-endpoints)를 참조하십시오.
</Callout>
...
<CollapserGroup>
  <Collapser id="json-guidelines" title="JSON 형식 지침">
  ...
  <Collapser id="json-example" title="JSON 예시">
  ...
  <Collapser id="limits" title="제한 및 제한된 문자">
...
<CollapserGroup>
  <Collapser id="numberone" title="Linux/bash 예제">
    ```bash
  ...
  <Collapser id="numbertwo" title="Windows/PowerShell 예제">
    ```powershell
...
<CollapserGroup>
  <Collapser id="success" title="성공 응답 코드">
    <table>
      <thead>
        <tr>
  ...
  <Collapser id="errors-submission" title="제출 오류">
  ...
  <Collapser id="errors-parsing" title="구문 분석 오류">
