...
1. 데이터를 보고하려는 계정에 대해 <InlinePopover type="licenseKey"/>를 가져옵니다.
2. 맞춤형 대시보드 또는 속성을 생성하기 전에 [NRQL에서 사용하는 예약어](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#reserved-words) 목록을 검토하세요.
3. 애플리케이션을 계측하거나 API를 쿼리하거나 다른 방법을 사용하여 이벤트에 대한 [JSON을 생성](#instrument) 합니다.
4. <DNT>**POST**</DNT> 요청에서 컬을 사용하여 [압축된 JSON 페이로드](#submit-event) (예: `gzip` 또는 `deflate`)를 HTTPS 엔드포인트에 제출합니다.
5. 권장 사항: 구문 분석 오류가 발생할 때 [알림을 받을](#errors-parsing) [NRQL 수 있도록 공지 조건을 설정하세요.](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="JSON 예시"
  >
    다음은 API를 통해 전송하기 위한 일반적인 JSON 데이터 세트의 예입니다. 이 호출은 두 개의 <DNT>**Purchase**</DNT> 유형 이벤트를 JSON으로 다시 보냅니다. JSON을 다시 사용하여 단일 HTTP 호출에 여러 이벤트를 추가할 수 있습니다.
  ...
  >
    ...
    * Event API로 전송되는 [분당 HTTP 요청 수에는 속도 제한이](#post-limit) 있습니다.

      일부 속성에는 추가 제한 사항이 있습니다.
...
* `Api-Key` 에는 올바른 <InlinePopover type="licenseKey"/>이 포함되어 있습니다.
...
권장 사항: 성공 메시지를 확인하는 것 외에도 데이터에 대한 NRQL 쿼리를 만들어 사용 가능한지 확인하세요.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            누락되었거나 잘못된 키: 잘못된 인스턴스 키입니다. 유효한 <InlinePopover type="licenseKey"/>을(를) 등록하세요. `License` 유형의 키를 사용해야 합니다. `Browser`, `Mobile` 또는 `User` 키는 작동하지 않습니다.
          </td>
        </tr>
...
[`NrIntegrationError` 이벤트를](/docs/telemetry-data-platform/manage-data/nrintegrationerror) 사용하면 뉴렐릭 계정으로 전송되는 맞춤 데이터에 대해 쿼리하고 <InlinePopover type="alerts"/>을 설정할 수 있습니다. 권장 사항: 구문 분석 오류에 대한 알림을 받으려면 `NrIntegrationError`에 대한 [NRQL 공지 조건을 만드세요](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) . 다음 예제 NRQL 쿼리를 사용하세요.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        오류를 생성한 요청에 사용된 <InlinePopover type="licenseKey"/>의 처음 6개 문자입니다.
      </td>
    </tr>
