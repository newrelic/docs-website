...
New Relic Event API는 [사용자 지정 이벤트](/docs/insights/insights-data-sources/custom-data/report-custom-event-data) 를 New Relic에 보고하는 한 가지 방법입니다. 이벤트 API를 사용하면 POST 명령으로 사용자 지정 [이벤트 데이터](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data) 를 New Relic 계정으로 보낼 수 있습니다. 그런 다음 이러한 이벤트는 [NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) 을 사용하여 쿼리하고 차트로 작성할 수 있습니다.
...
* 이벤트 데이터 보존 방법에 대한 자세한 내용은 [이벤트 데이터 보존](/docs/insights/use-insights-ui/manage-account-data/extend-event-data-retention) 을 참조하십시오.
...
1. 데이터를 보고할 계정에 대한 [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 가져옵니다.
...
이 방법은 NRQL 인터페이스나 쿼리 API에서 액세스할 수 있는 계정으로 이벤트를 직접 보냅니다.

이벤트 API는 사용자 정의 이벤트에 허용되는 [크기, 비율 및 문자를 제한](#limits) 합니다. 또한 NRQL에서 사용할 수 있는 다른 이벤트와 마찬가지로 사용자 지정 이벤트는 생성된 후에 업데이트하거나 삭제할 수 없습니다. 사용자 지정 이벤트에 문제가 있는 경우 [문제 해결 절차](#verify) 를 따르거나 새 사용자 지정 이벤트를 만드십시오.

## 라이센스 키 받기 [#register]

[라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 가 필요합니다. 라이선스 키는 특정 사용자가 아닌 계정과 연결됩니다. 즉, 해당 키에 대한 액세스 권한이 있는 계정의 모든 사용자가 키를 사용할 수 있습니다.

동일한 라이센스 키를 사용하여 동일한 계정에 여러 이벤트 유형을 제출할 수 있습니다. 그러나 보안을 보장하기 위해 서로 다른 애플리케이션 또는 데이터 소스에 대해 서로 다른 키를 사용하는 것이 좋습니다.
...
* `Api-Key` 에는 올바른 [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 가 포함되어 있습니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            키 누락 또는 유효하지 않음: 유효하지 않은 라이센스 키입니다. 유효한 라이센스 키를 등록하십시오.
          </td>
        </tr>
    ...
    * JSON 페이로드에 형식이 잘못된 JSON 또는 누락된 필수 데이터가 포함되어 있습니다.
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              속성 `appId` 이(가) 정수가 아니어서 <var>X</var> 이벤트가 거부되었습니다.
            </td>
          ...
          <tr>
            <td>
              `eventType` 은(는) 다음 문자를 포함할 수 없기 때문에 <var>X</var> 이벤트가 거부되었습니다. `[., \]`
            </td>
          ...
          <tr>
            <td>
              속성에 속성 이름이 없기 때문에 <var>X</var> 이벤트가 거부되었습니다.
            </td>
          ...
          <tr>
            <td>
              속성 이름이 최대 길이를 초과했기 때문에 <var>X</var> 이벤트가 거부되었습니다.
            </td>
          ...
          <tr>
            <td>
              속성 값이 최대 길이를 초과했기 때문에 <var>X</var> 이벤트가 거부되었습니다.
            </td>
          ...
          <tr>
            <td>
              이벤트가 최대 속성 수를 초과했기 때문에 <var>X</var> 이벤트가 거부되었습니다.
            </td>
          ...
          <tr>
            <td>
              필수 속성이 누락되어 <var>X</var> 이벤트가 거부되었습니다. `eventType`
            </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        오류를 생성한 요청에 사용된 [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 의 처음 6자.
      </td>
    </tr>
