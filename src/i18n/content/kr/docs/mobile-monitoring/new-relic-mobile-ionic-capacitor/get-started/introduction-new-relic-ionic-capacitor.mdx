...
<Steps>
  <Step>
    ...
    1. **[one.newrelic.com](https://onenr.io/0EjOpynz0Q6) > Browser & Mobile > (Android 또는 iOS 선택)** 로 이동한 다음 UI에서 계측 단계를 완료하세요.
    2. UI의 **Step 4** \[4단계] 에서 애플리케이션 토큰(해당되는 경우 iOS 및 Android용 토큰)을 복사합니다.
  </Step>
  ...
  <Step>
    ...
    `main.ts` (Angular 또는 Vue) 또는 `index.tsx` (React)에서 앱 초기화 시 Capacitor 에이전트를 시작합니다.

    1. NewRelic을 시작하려면 다음 코드를 추가하세요(적절한 애플리케이션 토큰을 입력하는 것을 잊지 마세요):
    ...
    2. `appToken` 1단계의 애플리케이션 토큰으로 바꿉니다.
  </Step>
...
<CollapserGroup>
  ...
  >
    ...
    4. **Add to dashboard** \[대시보드에 추] 가 를 클릭합니다.<img title="Query builder" alt="This is an image of a sample query in the Query builder." src={mobileQueryExample}/>
