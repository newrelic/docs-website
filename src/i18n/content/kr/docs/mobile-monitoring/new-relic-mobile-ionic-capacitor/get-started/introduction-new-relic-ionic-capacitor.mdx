...
iOS 및 Android용 공식 New Relic Capacitor 에이전트를 통해 개발자는 모바일 에이전트를 [Capacitor 애플리케이션](https://capacitorjs.com/)에 쉽게 임베드할 수 있습니다. JavaScript로 작성된 플러그인은 New Relic의 기본 에이전트를 자동으로 포함하여 모바일 모니터링 및 성능 가시성을 제공합니다.
...
Capacitor 에이전트는 iOS 및 Android SDK 에이전트와 동일한 API 호출을 사용합니다. [iOS SDK 문서](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api) 또는 [Android SDK 문서를](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api)참조하십시오.
...
4. **Add to dashboard** \[대시보드에 추가를]클릭합니다.<img title="Query builder" alt="This is an image of a sample query in the Query builder." src={mobileQueryExample}/>
