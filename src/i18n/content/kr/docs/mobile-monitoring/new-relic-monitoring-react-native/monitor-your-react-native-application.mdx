...
<Callout variant="important" title="오류 보고 관련 주요 변경 사항(1.9.0)">
  React Native 에이전트 버전 1.9.0부터, JavaScript 오류는 새로운 이벤트 유형인 `MobileJSError`를 통해 보고됩니다. 새로운 모바일 오류 프로토콜의 경우, 리액트 네이티브 JavaScript 오류가 더 이상 `MobileHandledException` 이벤트 유형에 저장되지 않습니다.

  버전 1.9.0 이상을 사용하는 경우, JavaScript 계층 오류와 관련해`MobileHandledException`을 쿼리하는 모든 알림 조건 및 대시보드를 업데이트하여 `MobileJSError`를 대신 사용하십시오. 예를 들어, `SELECT count(*) FROM MobileHandledException WHERE platform = 'reactnative'`를 `SELECT count(*) FROM MobileJSError`로 교체해야 합니다. 또한 새로운 이벤트 유형에 대해 [소스 맵이 올바르게 업로드되도록](/docs/mobile-monitoring/new-relic-monitoring-react-native/react-native-agent-js-error-reporting) 빌드 스크립트를 최신 버전으로 업데이트하십시오.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [커스텀 오류 기록](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-errors/#react)
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="v1.9.0" title="React Native 에이전트 버전 1.9.0 이상">
    JavaScript 오류 및 promise 거부는 `MobileJSError` 이벤트로 기록됩니다. 뉴렐릭에서 각 JavaScript 오류에 대한 이벤트 추적, 속성 및 스택 트레이스를 확인할 수 있습니다.
  ...
  <Collapser id="v1.2.0" title="React Native 에이전트 버전 1.2.0-1.8.x">
...
<CollapserGroup>
  <Collapser id="logs-v1.4.1" title="React Native 에이전트 버전 1.4.1 이상">
    React Native 에이전트 버전 1.4.1부터, JavaScript 콘솔 로그는 로그 이벤트로 보고됩니다. [로그 UI](/docs/logs/ui-data/use-logs-ui)에서 이를 확인하거나 <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Query your data**</DNT> 로 이동하여 다음을 실행함으로써 쿼리할 수 있습니다:
  ...
  <Collapser id="logs-v1.4.0" title="React Native 에이전트 버전 1.4.0 이하">
