...
2. 해당하는 경우: **Mobile Apps** \[모바일 앱] 목록에서 **Add a new app** \[새 앱 추가를] 선택합니다.
...
3. NewRelic 패키지 제품을 선택하고 대상을 선택한 후 **Finish**를 선택합니다.
...
5. iOS 에이전트 버전에 따라 아래 빌드 스크립트를 대상의 **Build Phases** 에 추가하세요. 스크립트가 마지막 빌드 스크립트인지 확인하고 `APP_TOKEN` [애플리케이션 토큰](/docs/mobile-apps/viewing-your-application-token) 으로 바꾸세요.
