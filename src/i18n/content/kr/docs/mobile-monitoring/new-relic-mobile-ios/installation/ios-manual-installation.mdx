...
2. Xcode 프로젝트에 New Relic 프레임워크를 추가하려면: **Finder** 를 사용하여 `NewRelic.xcframework` 폴더를 Xcode 프로젝트로 드래그하고 **프로젝트 탐색기** 창의 **Frameworks, Libraries 및 Embedded Content** 아래에 있는 프로젝트에 드롭합니다. **포함** 드롭다운 메뉴에서 **포함 및 서명** 이 선택되어 있는지 확인합니다.
