...
2. <DNT>**Crashes**</DNT> 보기.
3. 그룹 충돌에서 <DNT>**Total crashes**</DNT> 중 특정 충돌을 선택하세요.
...
일부 상황에서는 뉴렐릭의 dSYM 파일 자동 업로드가 실패할 수 있습니다. dSYM 업로드가 실패하면 `upload_dsym_results` 파일을 생성합니다. 예를 들어, 네트워크 오류가 발생하여 dSYM 업로드가 완료되지 않은 경우 `upload_dsym_results`에 무엇이 잘못되었는지에 대한 로그가 포함됩니다. 뉴렐릭이 dSYM 업로드를 처리하는 방법에 대한 자세한 내용은 [뉴렐릭의 온라인 기술 커뮤니티](https://support.newrelic.com/s/hubtopic/aAX8W0000008Yv9/relic-solution-ios-dsym-upload-deepdive)를 참조하십시오.
...
<CollapserGroup>
  ...
  <Collapser id="command-line-manual-upload" title="명령줄을 통해">
    ...
    2. 다음 명령을 사용하여 dSYM zip 파일을 업로드합니다.
       ...
       * <DNT>**US accounts (default)**</DNT> 의 경우:
