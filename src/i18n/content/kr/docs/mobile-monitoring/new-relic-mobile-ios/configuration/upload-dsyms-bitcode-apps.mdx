...
Xcode에서 대상의 빌드 단계에 있는 빌드 스크립트에서 실행하도록 의도된 iOS 에이전트에 Swift 스크립트가 포함되어 있습니다. 스크립트는 백그라운드에서 dSYM 파일을 자동으로 업로드(또는 dSYM을 New Relic 맵 파일 형식으로 변환)한 다음 충돌 기호화에 필요한 파일을 New Relic에 백그라운드 업로드를 수행합니다.
...
New Relic iOS 에이전트 버전 7.4.0에서 우리는 Swift를 사용하는 새로운 심볼 업로드 스크립트를 도입했습니다. 기호 업로드 스크립트의 Python 버전은 계속 사용할 수 있지만 향후 릴리스에서 제거될 예정입니다. New Relic iOS Agent 버전 7.3.8에서는 Python 버전의 자동 스크립트가 Python 3을 사용합니다. macOS 12.3 [macOS 12.3(Monterey)](https://developer.apple.com/documentation/macos-release-notes/macos-12_3-release-notes) 을 사용하는 경우 Python 3은 기본적으로 Xcode와 함께 설치됩니다. 그러나 macOS 12.2 이하에서 python 자동 스크립트를 사용하는 경우 [Python 3](https://www.python.org/downloads/mac-osx/) 을 수동으로 설치해야 할 수 있습니다.
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) >Mobile** 로 이동합니다. 그런 다음 목록에서 앱을 선택합니다.
2. **충돌 분석** 보기 .
...
<CollapserGroup>
  ...
  >
    ...
    1. 다음 명령을 사용하여 dSYM 파일을 압축합니다. '&#x7B;~}'/ZIPPED_DSYM_PATH를 새 dSYM 아카이브 경로 및 파일 이름(예: Users/my-name/desktop)으로 바꿉니다. 또한 '&#x7B;~}'/dSYM_PATH를 기존 dSYM 파일 경로로 바꿉니다.
