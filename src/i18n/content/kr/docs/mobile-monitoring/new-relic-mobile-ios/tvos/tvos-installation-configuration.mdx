...
iOS 에이전트를 설치하고 구성하려면 다음 지침을 따르십시오.
...
몇 분 안에 iOS 앱에 대한 데이터가 표시되기 시작합니다. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) >Mobile > (앱 선택)으로** 이동합니다. 그렇지 않은 경우 [데이터가 표시되지 않음](/docs/mobile-monitoring/mobile-monitoring-installation/ios/no-data-appears-ios) 을 참조하십시오.
...
기본적으로 New Relic 에이전트는 `info` 수준에서 로그합니다. 로그 수준을 `verbose` 이상 우선순위로 높이는 것은 릴리스 빌드가 아닌 디버깅을 위해서만 권장됩니다. 자세한 수준 이상의 우선 순위는 New Relic 서버와의 모든 API 상호 작용을 기록합니다.

모바일 모니터링에는 6가지 로그 수준을 사용할 수 있습니다.
