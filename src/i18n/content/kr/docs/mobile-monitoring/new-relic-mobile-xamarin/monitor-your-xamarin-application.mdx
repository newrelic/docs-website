...
<Steps>
  ...
  <Step>
    ...
    2. **Add > Add NuGet packages** 클릭한 다음 `NewRelic.Xamarin.Plugin` 선택합니다. 또한 `Xamarin.iOS` 프로젝트에 iOS 에이전트 바인딩을 추가해야 합니다.
    3. 에이전트를 추가할 Xamarin.iOS 프로젝트를 선택하고 컨텍스트 메뉴를 엽니다.
    4. **Add > Add NuGet packages** \[추가 > NuGet 패키지 추가를 선택한] 다음 `NewRelic.Xamarin.iOS.Binding`선택합니다.
  </Step>
