...
뉴렐릭은 ANR 그리드 추적에서 배터리 그리드 프레임을 자동으로 상징화하여 플랫폼에서 직접 읽을 수 있는 메서드 이름과 라인 번호를 제공합니다.

<Callout variant="tip" title="메모">
  현재 네이티브(NDK) 스택 프레임은 심볼화되지 않습니다. 기본 그리드 프레임의 경우 뉴렐릭에서 그리드 추적을 다운로드하고 `ndk-stack` 와 같은 오프라인 도구를 사용하여 수동으로 기호화할 수 있습니다.
</Callout>
