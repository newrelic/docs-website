...
HTTP 및 네트워크 요청을 모니터링하면 앱이 어떻게 작동하는지에 대한 통찰력을 얻을 수 있고 앱을 개선하는 데 도움이 될 수 있는 데이터를 얻을 수 있습니다. HTTP 및 네트워크 요청과 오류를 찾으려면 다음에서 확인할 수 있습니다. [<InlinePopover type="mobile"/>UI](#view-mobile) 또는 NRQL을 사용하여 [`MobileRequest` 및 `MobileRequestError` 이벤트를](#query-insights) 쿼리합니다.
