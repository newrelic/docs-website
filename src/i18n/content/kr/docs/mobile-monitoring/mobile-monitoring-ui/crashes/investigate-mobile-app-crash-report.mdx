...
**Crash report** \[충돌 보고서] <InlinePopover type="mobile"/>다음을 포함하여 충돌에 대한 요약 및 심층 세부정보를 제공합니다.
...
[New Relic Android 에이전트 버전 6.7.0](/docs/release-notes/mobile-release-notes/android-release-notes/android-670) 부터 네이티브 크래시를 추적하고 진단하는 데 도움이 되도록 보고 및 분석이 런타임 동안 [네이티브 코드](https://developer.android.com/ndk/guides) 수준에서 발생하는 신호 위반 및 기타 오류를 포함하도록 향상되었습니다.
...
* 기본 런타임 예외 기본 에이전트는 런타임 중에 앱에서 발생한 처리되지 않은 C++ 예외를 보고하고 이를 [처리된 예외](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/introduction-mobile-handled-exceptions/) 로 보고합니다. 처리되지 않은 예외는 일반적으로 치명적이며 응용 프로그램을 중단시킵니다.
...
Android 네이티브 에이전트 아티팩트는 [MavenCentral](https://search.maven.org/search?q=g:com.newrelic.agent.android%20AND%20a:agent-ndk) 에서 찾을 수 있으며 의미론적 버전 관리 규칙을 따릅니다. 릴리스 및 다운로드에 대한 자세한 내용은 [Android 에이전트 릴리스 노트를](/docs/release-notes/mobile-release-notes/android-release-notes/) 참조하세요.
