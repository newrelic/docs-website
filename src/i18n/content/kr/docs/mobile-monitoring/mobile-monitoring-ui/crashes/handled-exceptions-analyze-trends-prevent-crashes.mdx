...
[그룹 및 필터](#groups-filters) 에서 선택한 예외 유형의 패턴을 찾거나 정렬하려면 **Top exception locations**\[상위 예외 위치] 표를 사용하세요. 여기에는 다음이 포함됩니다.
...
정렬 순서나 필터 옵션을 변경하여 귀하와 귀하의 팀에 가장 중요한 예외 유형에만 집중할 수 있습니다. [각 예외 발생에 대한 추가 스레드 세부 정보를](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/handled-exceptions-occurrences) 보려면 **Top exceptions location** \[상위 예외 위치] 테이블에서 레코드를 선택합니다.
...
2. **Top exception locations** \[상위 예외 위치] 테이블의 행에서 처리된 예외를 선택합니다.
...
* 이벤트 유형을 [`MobileHandledException`](/docs/insights/insights-data-sources/default-data/mobile-events-attributes#mobilehandledexception-attributes) [쿼리합니다 .](/docs/insights/nrql-new-relic-query-language/using-nrql/query-page-create-edit-nrql-queries) 자세한 내용은 [NRQL 예제를](/docs/insights/nrql-new-relic-query-language/nrql-query-examples/insights-query-examples-new-relic-mobile#mobilehandledexception-examples) 참조하세요.
