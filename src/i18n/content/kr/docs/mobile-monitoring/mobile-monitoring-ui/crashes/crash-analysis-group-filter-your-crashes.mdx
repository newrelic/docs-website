...
* 대부분의 충돌이 발생하는 시점을 정확히 파악합니다. 예를 들어 [`Last Interaction` 으로 필터링](#groups-filters) 하거나 [**충돌 보고서** 페이지](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report) 에서 [상호작용 추적](#export-xcode) 을 봅니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 다른 과거 패턴을 찾으려면 선택한 [기간](#time-period) 을 변경하세요.
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Exceptions > Crash analysis:** 다음은 **Crash locations** 그룹으로 필터링된 **Top 5 occurrences** 차트의 예입니다. 그룹이나 필터를 선택하여 원하는 방식으로 충돌 데이터를 분석하세요.
      </figcaption>
  </Collapser>
  ...
  >
    ...
    * 예외가 처음이자 마지막으로 발생한 때는 언제입니까?
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) >Exceptions > Crash analysis** : **충돌 보고서** 표의 패턴을 분석하기 위해 열을 정렬한 다음 보고서를 선택하여 충돌 세부 정보를 검토합니다.
      </figcaption>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Exceptions> Crash analysis > Crash profiles** : 충돌이 발생한 세션과 충돌이 없는 세션을 비교합니다.
    </figcaption>
  </Collapser>
  ...
  >
    **오류 보고서** 표에서 특정 [**오류 보고서**](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report) 로 드릴다운할 수 있습니다. 여기에서 다음을 수행할 수 있습니다.

    * 세션 시작부터 충돌 이벤트까지의 [상호 작용 추적](/docs/mobile-monitoring/mobile-monitoring-ui/mobile-app-pages/interactions-page) 을 검사합니다.

    * 관련 스레드 분석을 살펴보세요.

    * Xcode를 사용하여 소스 코드를 검사할 수 있도록 **충돌 세부 정보 내보내기** 를 선택합니다.

    * 더 쉬운 디버깅을 위해 충돌 보고서를 다시 상징화합니다.
    ...
    * 티켓을 제출하고 충돌을 해결하십시오.
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Crashes > Crash report:** 충돌 발생을 다시 상징화하는 옵션이 포함된 상호 작용 추적의 예입니다. Xcode를 사용하여 소스 코드를 분석하고 디버그하려면 **충돌 세부 정보 내보내기** 를 선택합니다.
      </figcaption>
  </Collapser>
</CollapserGroup>
