...
모바일 앱 충돌로 인해 사용자가 부정적인 리뷰를 제출하게 되어 조직의 평판에 영향을 미칠 수 있습니다. 충돌로 인해 사용자가 앱을 완전히 삭제하게 되어 조직의 수익성에 영향을 미칠 수도 있습니다. 크래시 분석 페이지를 통해 개발 생활주기에서 가장 큰 병목현상 중 하나인 병목지점, 즉 예상치 못한 예외를 해결하는 데 걸리는 시간을 식별하고 처리할 수 있습니다.

## 충돌 분석 기능 사용 [#features]

개발팀이 [이메일로 충돌 메시지를](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/receive-crash-notifications-email) 받으면 이메일 링크를 선택하여 사용자 인터페이스에서 충돌에 대한 자세한 정보를 검토하거나 [쿼리를 통해](/docs/nrql/nrql-examples/nrql-query-examples-mobile-monitoring/) 직접 차트 데이터를 분석할 수 있습니다. 개별 **Crash details** \[크래시 세부정보] 페이지에는 팀이 보고부터 해결까지 문제를 추적할 수 있는 추적 도구도 포함되어 있습니다.

**Crash analysis** \[충돌 분석] 페이지는 개발자에게 다음을 지원합니다.
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Exceptions > Crash analysis** 으로 이동합니다.
   ...
   [모바일 앱의 **Summary** \[요약\] 페이지](/docs/mobile-monitoring/mobile-monitoring-ui/mobile-app-pages/mobile-apps-summary-page/) 에서 직접 필터링 가능한 **Crash analysis** \[충돌 분석] 페이지를 보려면 **Crash occurrences** \[충돌 발생] 차트 제목을 선택하세요.
...
3. [**Top 20 occurrences** \[상위 20개 항목\] 차트를 사용](#top20-occurrences) 하면 [그룹 및 필터](#groups-filters) 에서 선택한 항목에 대한 광범위한 패턴을 식별할 수 있습니다. 이러한 패턴을 연관시키거나 선택한 [기간](#time-period) [동안의 비정상 종료 비율](#percentage-charts) 과 같은 정보를 찾을 수 있습니다.
...
<CollapserGroup>
  ...
  >
    **Top 20 occurrences** \[상위 20개 발생] 차트를 사용하면 충돌 유형과 선택한 [기간](#time-period) 동안 발생한 충돌 수를 식별할 수 있습니다. 이 차트에는 선택한 [그룹 및 필터](#groups-filters) 별로 상위 20개 충돌 발생이 표시됩니다. 예를 들어 이메일 주소별로 그룹화한 다음(사용자 정의 속성으로 구성된 경우) 특정 이메일로 필터링하여 해당 사용자의 경험을 조사할 수 있습니다.
    ...
    * 다른 과거 패턴을 찾으려면 선택한 [기간](#time-period) 을 변경하세요.
      ...
      <figcaption>
        **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Errors > Crash analysis:** 다음은 **Crash locations** \[충돌 위치] 그룹으로 필터링된 **Top 20 occurrences** \[상위 20개 발생] 차트의 예입니다. 그룹이나 필터를 선택하여 원하는 방식으로 충돌 데이터를 분석하세요.
      </figcaption>
  </Collapser>
  ...
  >
    그만큼 <InlinePopover type="mobile"/>충돌 이벤트 추적은 모바일 앱의 충돌로 이어지는 이벤트를 보여줍니다. 이는 기본적으로 모니터링되는 이벤트이거나 사용자가 만든 사용자 정의 이벤트일 수 있습니다. 자세한 내용은 [충돌 이벤트 추적을](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/mobile-crash-event-trail) 참조하세요.
  </Collapser>
