...
* **iOS 에이전트 요구 사항:** 자동 수집은 **기본적으로 비활성화** 되어 있습니다. 활성화하려면:

  1. 앱 초기화 코드에서 `NRFeatureFlag_AutoCollectLogs` 기능 플래그를 활성화해야 합니다.
  2. 뉴렐릭 UI 에서 <DNT>**Mobile Logs**</DNT> 토글이 **On** \[켜져] 있습니다.
...
* **자동 수집:** iOS 에이전트는 표준 출력(`stdout`) 및 표준 오류(`stderr`)로 전송되는 로그를 자동으로 캡처할 수 있습니다. 자동 로그 수집을 위해서는 앱 초기화 코드에서 `NRFeatureFlag_AutoCollectLogs` 기능 플래그를 활성화해야 합니다.
