...
4. **상위/하위 우선순위 규칙**
   ...
   충돌하는 규칙이 있는 경우, 상위 항목의 마스킹이 하위 트리 규칙보다 우선한다는 점에 유의하십시오.

   *예:* 상위 항목에 `nr-block` 속성이 있지만 하위 항목에 `nr-unmask`이(가) 있는 경우, 전체 하위 트리가 콘텐츠를 차단하는 검은색 직사각형 플레이스홀더로 대체되며, 하위 트리의 하위 콘텐츠는 캡처되지 않습니다.
