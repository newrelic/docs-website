...
<Steps>
  ...
  <Step>
    ...
    .NET MAUI 모바일 플러그인을 사용하면 [.NET MAUI Shell](https://learn.microsoft.com/en-us/dotnet/maui/fundamentals/shell/navigation) 내에서 탐색 이벤트를 추적할 수 있습니다. `App.xaml.cs`에서 앱의 시작 타이밍에 따라 이를 초기화하는 두 가지 방법이 있습니다.

    <DNT>**Option 1: In the App Constructor**</DNT> `MainPage`을(를) 설정한 직후 생성자에서 직접 추적 메서드를 호출할 수 있습니다.
    ...
    <DNT>**Option 2: In the OnStart Method (Recommended for timing/NullReferenceException issues)**</DNT> 생성자 실행 중에 애플리케이션 셸이 완전히 초기화되지 않은 경우, 내비게이션 트래커를 호출하면 `System.NullReferenceException`이(가) 발생할 수 있습니다. 이를 방지하려면 호출을 MAUI `OnStart()` 수명 주기 메서드로 이동하세요:
...
### UI에서 분산 추적 데이터가 누락됨 [#missing-distributed-tracing]
...
### TrackShellNavigatedEvents 사용 시 앱 충돌 [#track-shell-crash]

`App()` 생성자에서 `CrossNewRelic.Current.TrackShellNavigatedEvents()` 을(를) 호출할 때 `System.NullReferenceException` 이(가) 발생하는 경우, 애플리케이션 셸이 아직 완전히 부트스트랩되지 않았기 때문일 가능성이 높습니다. 이 문제를 해결하려면 호출을 생성자에서 `App.xaml.cs` 파일 내의 MAUI `OnStart()` 수명 주기 메서드로 이동하세요(코드 예제는 [화면 추적 이벤트](#screen-tracking-events) 섹션을 참조하세요).
