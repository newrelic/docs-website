...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="android">
      ...
      기본적으로, <InlinePopover type="mobile"/>이벤트 수집 주기(기본적으로 길이는 600초)당 최대 1,000개의 이벤트를 수집합니다. 이 방법은 다음 하베스트 주기까지 메모리에 저장되는 이벤트 풀의 최대 크기를 제어합니다. 풀 크기 제한에 도달하면 New Relic Android 에이전트는 [이벤트 샘플링을](/docs/agents/manage-apm-agents/agent-data/new-relic-events-limits-sampling) 시작하고 이벤트 풀이 다음 수집 주기와 함께 전송될 때까지 일부 이전 이벤트와 새 이벤트를 삭제합니다. 이 방법을 사용하면 해당 이벤트 풀의 최대 크기를 재정의할 수 있습니다.
