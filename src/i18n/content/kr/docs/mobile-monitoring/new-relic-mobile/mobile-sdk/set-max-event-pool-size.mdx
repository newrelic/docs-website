...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="android">
      ...
      기본적으로 <InlinePopover type="mobile" />은(는) 이벤트 하베스트 주기당 최대 1,000개의 이벤트를 수집하며, 이 주기의 기본 길이는 60초입니다. 이 메서드는 다음 하베스트 주기까지 메모리에 저장되는 이벤트 풀의 최대 크기를 제어합니다. 풀 크기 제한에 도달하면 뉴렐릭 안드로이드 에이전트는 다음 하베스트 주기와 함께 이벤트 풀이 전송될 때까지 일부 이전 이벤트와 새로운 이벤트를 폐기하면서 [이벤트 샘플링](/docs/agents/manage-apm-agents/agent-data/new-relic-events-limits-sampling)을 시작합니다. 이 메서드를 사용하면 해당 이벤트 풀의 최대 크기를 재정의할 수 있습니다.

      이벤트 하베스트 주기의 기본값은 60초입니다. 이벤트 하베스트 주기의 길이를 변경하려면 [최대 이벤트 버퍼 시간 설정](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/set-max-event-buffer-time)을 참조하십시오.
    ...
    <TabsPageItem id="ios">
      ...
      <Callout variant="important">
        이벤트 풀 크기를 `1000`보다 크게 설정할 수 있지만, 에이전트는 네트워크 데이터에 엄격한 내부 제한을 적용합니다. 이벤트 풀을 얼마나 크게 구성하든 관계없이, 하베스트 주기당 최대 **1,000개의 네트워크 트랜잭션** 이 기록되고 전송됩니다.
      </Callout>

      이벤트 하베스트 주기의 기본값은 60초입니다. 이벤트 하베스트 주기의 길이를 변경하려면 [최대 이벤트 버퍼 시간 설정](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/set-max-event-buffer-time/) 도 참조하십시오.
