...
New Relic을 설치하면 <InlinePopover type="mobile"/>기능은 iOS 또는 Android 앱의 일부가 됩니다. 이러한 기능은 애플리케이션의 "샌드박스" 내에 있으므로 모바일 앱의 성능 데이터 외에는 어떤 것도 액세스할 수 없습니다. 당사는 배터리 잔량 등 기기 자체에 대한 성능 데이터를 수집하지 않습니다.
