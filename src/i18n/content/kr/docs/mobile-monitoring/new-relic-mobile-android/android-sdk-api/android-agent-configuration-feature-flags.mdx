...
<CollapserGroup>
  <Collapser
    ...
    title="크래시 보고 활성화됨"
  >
    지연된 충돌 보고를 활성화하거나 비활성화합니다.

    [지연된 충돌 보고](/docs/mobile-monitoring/new-relic-mobile-android/install-configure/android-agent-crash-reporting/#deferred-crash-reporting) 에 대한 자세한 내용은 지연된 충돌 보고를 참조하세요.
...
<CollapserGroup>
  ...
  >
    [측정항목](https://docs.newrelic.com/docs/data-apis/understand-data/new-relic-data-types/#metrics) 에 대한 보고 앱 실행 시간(콜드 및 핫 타임) 활성화 또는 비활성화

    * 콜드 스타트 시간은 앱이 처음부터 시작되는 것을 의미합니다. 이는 앱의 프로세스가 그때까지 시스템에 의해 생성되지 않았음을 의미합니다.

    * 핫 시작 시간은 앱의 프로세스가 이미 백그라운드에서 실행 중인 시간을 나타냅니다.

      Android 에이전트 버전 6.9.0 이상에서 사용 가능합니다.
...
<CollapserGroup>
  ...
  >
    ...
    Android 에이전트 버전 6.11.0 이상에서 사용할 수 있습니다.
