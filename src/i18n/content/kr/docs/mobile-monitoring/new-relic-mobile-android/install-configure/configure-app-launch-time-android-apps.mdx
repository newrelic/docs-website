...
앱 실행 시간은 `AppLaunch/Cold` 및 `AppLaunch/Hot` [측정항목](https://docs.newrelic.com/docs/data-apis/understand-data/new-relic-data-types/#metrics) 으로 보고됩니다. 이러한 시작 시간 메트릭을 추적하기 위해 **쿼리 빌더** 를 사용하여 사용자 지정 대시보드를 만들 수 있습니다. **쿼리 빌더** 를 사용하려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Capabilities > Query builder** 로 이동하십시오.
