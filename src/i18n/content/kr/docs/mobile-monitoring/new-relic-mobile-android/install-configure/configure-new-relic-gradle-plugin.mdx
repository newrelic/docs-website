...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        어떤 변형 빌드 유형이 ProGuard 맵을 뉴렐릭에 보고해야 하는지 지정하십시오. 리액트 네이티브 앱의 경우, 이는 어떤 변형 빌드 유형이 JavaScript 소스 맵을 자동으로 업로드할지도 제어합니다. 기본: `Release`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        웹뷰 계측을 비활성화하려면 `false` (으)로 설정합니다.

        * 기본: `true`

        안드로이드 에이전트 v7.6.15 이상과 함께 사용할 때 지원됩니다.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  <Collapser id="app-level" title="여러 변형에 대해 지도 업로드 활성화">
    쉼표로 구분하여 여러 빌드 유형을 지정할 수 있습니다. 리액트 네이티브 앱의 경우, 여기에 `Debug`을(를) 추가하면 디버그 빌드에 대한 JavaScript 소스 맵의 자동 업로드도 활성화됩니다:
  ...
  <Collapser id="app-level" title="Webview 계측 비활성화">
    웹뷰 계측을 비활성화하려면
