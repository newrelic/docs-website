...
isTutorial: true
freshnessValidatedDate: '2023-12-29T00:00:00.000Z'
...
* **Capture C# and Native C++ errors:** 문제를 신속하게 식별하고 수정합니다.
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Summary**: Unity 데이터를 보고, HTTP 요청 및 오류를 추적하고, 시간 경과에 따른 앱 성능을 모니터링합니다.
</figcaption>
...
<Steps>
  ...
  <Step>
    ...
    ```
    https://github.com/newrelic/newrelic-unity-agent
    ```
  ...
  <Step>
    ...
    1. New Relic에서 애플리케이션 토큰을 검색합니다.
    ...
    2. Unity 프로젝트에 토큰을 추가합니다.
       ...
       1. Unity IDE를 엽니다.
       2. **Tools > New Relic > Getting Started** 로 이동하여 New Relic 편집기 창을 엽니다.
       3. 복사한 애플리케이션 토큰을 `appToken = ""` 필드에 붙여넣습니다. 앱이 iOS와 Android 모두에 배포되는 경우 각 플랫폼마다 별도의 토큰을 제공해야 합니다.
  ...
  <Step>
    ...
    Android 앱을 구성하려면 Gradle 템플릿을 맞춤설정해야 합니다. Unity 2019 이상을 사용하는 경우 다음 단계를 완료하세요.

    1. Gradle 빌드 설정에 New Relic Maven 저장소 URL을 포함합니다. 이렇게 하려면 `mainTemplate.gradle` 파일을 엽니다. 이 파일은 일반적으로 `Assets/Plugins/Android folder` 에 있습니다. 이제 다음과 같이 New Relic Maven URL을 엽니다.
    ...
    2. 프로젝트 수준 `baseProjectTemplate.gradle` 파일에 New Relic `classpath` 을 추가합니다. 일반적으로 이 파일은 Unity 프로젝트의 Android 폴더에서 찾을 수 있습니다.
    ...
    이전 버전의 Unity Studio를 사용하는 경우 낮은 버전의 클래스 경로를 Gradle 버전 7.0 이하를 지원하는 종속성으로 통합할 수 있습니다.
    ...
    3. `launcherTemplate.gradle` 파일 상단에 `newrelic` 플러그인을 적용합니다. 이 파일은 일반적으로 Unity 프로젝트의 Android 폴더에 있습니다.
    ...
    4. `AndroidManifest.xml` 파일에 `INTERNET` 및 `ACCESS_NETWORK_STATE` 권한을 추가합니다.
  ...
  <Step>
    ...
    1. Unity IDE에서 **Assets** \[자산] 으로 이동하여 **External Dependency Manager** \[외부 종속성 관리자를] 엽니다.
    2. **iOS Resolver** \[iOS 리졸버 를] 클릭합니다.
    3. Podfile 구성 섹션에서 다음 설정을 확인합니다.

    * **Add use_frameworks! to Podfile** 기능이 비활성화되었습니다
    * **Always add the main target to Podfile** 활성화되어 있습니다.
