...
1. 패키지 관리자를 사용하여 인프라 에이전트가 설치되었는지 확인하십시오.
   ...
   <CollapserGroup>
     <Collapser id="apt-install-verify" title="apt(Debian 또는 Ubuntu) 설치 확인">
     ...
     <Collapser id="yum-install-verify" title="yum(Amazon Linux, CentOS 또는 RHEL) 설치 확인">
     ...
     <Collapser id="windows-install-verify" title="Windows Server용 설치 확인">
...
2. [초기화 시스템](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status#init-system) 을 사용하여 에이전트가 실행 중인지 확인합니다.
   ...
   <CollapserGroup>
     <Collapser id="systemd-verify-status" title="SystemD로 상태 확인">
     ...
     <Collapser id="systemv-verify-status" title="System V로 상태 확인">
     ...
     <Collapser id="upstart-verify-status" title="Upstart로 상태 확인">
     ...
     <Collapser id="windows-verify-status" title="Windows에서 상태 확인">
