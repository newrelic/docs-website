...
인프라 에이전트는 기본 로그 회전 또는 압축 메커니즘을 제공하지 않습니다. 대신, 일반적으로 대부분의 Linux 배포판에 기본적으로 설치되는 [Linux logrotate 도구](http://man7.org/linux/man-pages/man8/logrotate.8.html) 와 같은 통합 로그 회전 도구를 사용하는 것이 좋습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `maxage 7`: `logrotate` 이(가) 7일 후에 회전된 파일을 제거합니다.
    ...
    <Callout variant="tip">
      `logrotate` 구성 옵션에 대한 전체 설명은 [Linux Logrotate 문서](https://linux.die.net/man/8/logrotate) 를 참조하세요.
    </Callout>
  </Collapser>
</CollapserGroup>
