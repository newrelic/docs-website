...
<CollapserGroup>
  <Collapser id="k8s-manifest-examples" title="Kubernetes 매니페스트 예시">
  ...
  <Collapser id="k8s-helm-chart" title="Kubernetes 투구 차트">
...
<CollapserGroup>
  <Collapser id="config-example" title="사용자 지정 구성의 예">
    ```ini
  ...
  <Collapser className="freq-link" id="docker-config" title="Docker: 기본 구성 덮어쓰기">
  ...
  <Collapser className="freq-link" id="k8s-config" title="Kubernetes: 기본 구성 덮어쓰기">
...
<CollapserGroup>
  <Collapser id="counter" title="카운터">
  ...
  <Collapser id="timer" title="시간제 노동자">
...
<CollapserGroup>
  <Collapser id="tags-nri-statsd" title="모든 측정항목에 적용되는 기본 태그 추가">
  ...
  <Collapser id="tags-app-code" title="측정항목 수준 태그 추가">
...
<CollapserGroup>
  <Collapser id="alert-example" title="경고 예">
...
* <InlinePopover type="licenseKey" />에 40자리 16진수 라이선스 키가 포함되어 있고 선택한 뉴렐릭 계정 ID에 대한 유효한 라이선스인지 확인하세요.
* 귀하의 뉴렐릭 계정에 대해 올바른 데이터 센터(미국 또는 EU)가 선택되었는지 확인하세요. 팁: License\_key가 &quot;eu&quot;로 시작하는 경우 `NR_EU_REGION=true` 플래그를 사용해야 합니다.
