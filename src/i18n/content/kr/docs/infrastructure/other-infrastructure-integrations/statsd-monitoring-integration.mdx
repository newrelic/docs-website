...
<CollapserGroup>
  <Collapser id="alert-example" title="경고 예">
    ...
    50 이상의 값을 가지는 지표가 전송되면 공지이벤트가 생성되어 통보된다. 공지이벤트는 24시간 이후 자동으로 마감됩니다. 알림이 제대로 작동하는지 확인하려면 다음 명령어를 실행하세요.
...
* Ensure the right data center, US (default) or EU, has been selected for your New Relic account. Tip: If the license\_key starts with &quot;eu&quot; then you must use the `NR_EU_REGION=true` flag.
* JP 리전의 경우 라이선스 키를 기반으로 엔드포인트가 자동으로 선택되므로 추가 플래그가 필요하지 않습니다.
* StatsD 통합과 관련된 [`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/) 이(가) 없는지 확인하세요.
