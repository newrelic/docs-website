...
New Relic [인프라 에이전트 온호스트 통합은](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 최소 두 개의 파일(실행 파일 및 최소 하나의 구성 파일)로 구성됩니다. 실행 파일은 인프라 모니터링 에이전트에서 소비되고 New Relic으로 전송되는 JSON 데이터를 생성합니다. JSON 개체를 SDK 통합 프로토콜이라고 합니다.
