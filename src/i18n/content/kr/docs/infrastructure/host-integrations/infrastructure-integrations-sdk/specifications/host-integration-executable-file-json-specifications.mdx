...
<DoNotTranslate>**Recommendation:**</DoNotTranslate> Go를 사용하여 통합을 생성하세요. 우리가 온호스트 통합과 [통합 구축 도구를](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources) 만드는 데 사용하는 언어입니다. 그러나 어떤 언어로든 통합을 생성할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    출력에는 "예쁘게 인쇄된" JSON이 아닌 표준 JSON을 사용합니다. <DoNotTranslate>**Recommendation:**</DoNotTranslate> 선택적 명령줄 스위치(예: `--pretty`)를 포함하여 디버깅 목적으로 JSON을 '보기 좋게 인쇄'할 수 있습니다.
  </Collapser>
...
최소 페이로드는 헤더 필드만 있는 JSON 객체입니다. <DoNotTranslate>**Recommendation:**</DoNotTranslate> 수집할 데이터가 없으면 프로그램 반환 코드와 `stderr` 에 기록된 로그 메시지를 사용합니다.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Recommendation:**</DoNotTranslate> 고유한 통합 이름을 생성하려면 역방향 도메인 이름을 사용하세요.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        필수의. 엔터티의 종류입니다. 인프라 에이전트는 `name` 과 함께 <DoNotTranslate>**unique identifier**</DoNotTranslate> 을 구성하기 위한 네임스페이스로 사용됩니다.
      </td>
    </tr>
...
<CollapserGroup>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> 통합에 사용되는 이벤트 유형이 고유한지 확인하려면 이벤트 유형 앞에 회사 이름이나 약어를 붙이십시오. 예를 들어 사용자 정의 통합이 Cassandra 노드 지표와 Cassandra 열 계열 지표를 서로 다른 샘플로 캡처하는 경우 이를 `MyOrgCassandraSample` 및 `MyOrgCassandraColumnFamilySample` 등의 서로 다른 이벤트 유형에 저장합니다.
          ...
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> 뉴렐릭 UI 에서 지표를 탐색할 때 도움이 되도록 카테고리 앞에 지표를 붙이세요. 뉴렐릭 통합은 현재 다음을 사용하고 있습니다:
          ...
          <tr>
            ...
            <td>
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> 다음 예와 같이 원래 지수 이름에 측정 단위가 아직 포함되어 있지 않은 경우 단위 접미사를 사용하여 측정 단위를 지정합니다.
              ...
              * 바이트 측정: `Bytes`) 사용하세요.
                ...
                지수가 `Megabytes` 와 같은 다른 단위로 캡처된 경우 이를 `Bytes` 으로 변환합니다. 예: `db.allMemtablesOffHeapSizeBytes`.

              * 시간 측정: `Milliseconds`) 사용하세요.

                <DoNotTranslate>**Recommendation:**</DoNotTranslate>

                지수가 `Seconds` 와 같은 다른 단위로 캡처된 경우 이를 `Milliseconds` 으로 변환합니다. 예를 들어: `query.readLatency50thPercentileMilliseconds`
            </td>
          </tr>
  ...
  >
    ...
    * 새 테이블 만들기
      ...
      [인프라 <DoNotTranslate>**Events**</DoNotTranslate> 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 및 [인프라 이벤트 히트맵](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range) 에서 이 데이터를 볼 수 있습니다. 뉴렐릭에서 `InfrastructureEvent` 이벤트 유형을 쿼리할 수도 있습니다.
  ...
  >
    ...
    * 기타 시스템 메타데이터
      ...
      [<DoNotTranslate>**Inventory**</DoNotTranslate> 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 및 [인프라 이벤트 히트맵](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range) 에서 이 데이터를 볼 수 있습니다. 재고 변경과 관련된 데이터를 쿼리할 수도 있습니다.
    ...
    * 값은 스칼라 유형(문자열 또는 숫자) <DoNotTranslate>**or**</DoNotTranslate> 핵심 가치의 또 다른 객체일 수 있습니다. 뉴렐릭은 계층 구조를 지원하지만 최종 값 노드는 스칼라여야 합니다.
