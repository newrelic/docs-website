...
인프라 모니터링을 위해 [통합 SDK](/docs/integrations/integrations-sdk/getting-started/introduction-infrastructure-integrations-sdk) 를 사용하여 맞춤형 [호스트 통합](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) 을 구축하는 경우 통합은 실행 파일과 하나 이상의 구성 파일 등 최소 3개의 파일로 구성됩니다. 실행 파일은 인프라 모니터링 에이전트에서 사용하고 New Relic으로 전송되는 JSON 데이터를 생성합니다. JSON 개체를 SDK 통합 프로토콜이라고 합니다.
...
**권장 사항:** Go를 사용하여 통합을 생성하십시오. 호스트 통합 및 [통합 구축 도구](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources) 를 만드는 데 사용하는 언어입니다. 그러나 모든 언어로 통합을 생성할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    출력에 "예쁜 인쇄" JSON이 아닌 표준 JSON을 사용합니다. **권장 사항:** 디버깅 목적으로 JSON을 "예쁜 인쇄"로 만들려면 선택적 명령줄 스위치(예: `--pretty` )를 포함합니다.
  </Collapser>
...
최소 페이로드는 헤더 필드만 있는 JSON 객체입니다. **권장 사항:** 수집할 데이터가 없으면 프로그램 반환 코드를 사용하고 `stderr` 에 기록된 메시지를 기록합니다.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        **권장 사항:** 역 도메인 이름을 사용하여 고유한 통합 이름을 생성하십시오.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        필수의. 엔터티의 종류입니다. 인프라 에이전트는 `name` 과 함께 **고유 식별자** 를 구성하기 위한 네임스페이스로 사용합니다.
      </td>
    </tr>
...
인프라 에이전트 [버전 1.2.25 이상](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes) 부터 프로토콜 v3은 에이전트 수준에서 엔티티 이름에 로컬 주소 대체를 추가하여 원격 엔티티 고유성을 개선합니다.
...
인프라 에이전트는 v3 통합 프로토콜에 대한 루프백 주소 교체를 자동으로 활성화합니다. [에이전트 구성 플래그](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#general) `replace_v2_loopback_entity_names` 를 통해 v2에 대해 이를 활성화할 수도 있습니다. 이 경우 v2를 사용하는 에이전트가 실행하는 모든 통합은 로컬 주소를 전달할 때마다 이름이 바뀝니다.
...
<CollapserGroup>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              **권장 사항:** 통합에서 사용하는 이벤트 유형이 고유한지 확인하려면 이벤트 유형에 회사 이름 또는 약어를 접두어로 붙이십시오. 예를 들어 사용자 지정 통합이 Cassandra 노드 측정항목과 Cassandra column family 측정항목을 다른 샘플로 캡처하는 경우 `MyOrgCassandraSample` 및 `MyOrgCassandraColumnFamilySample` 과 같은 다른 이벤트 유형에 저장합니다.
          ...
          <tr>
            ...
            <td>
              필수(최소 하나). 이름(키)과 해당 값을 포함하는 메트릭 측정입니다. 인프라 기능과의 최대 호환성을 위해 이러한 항목이 일반적으로 엔터티 유형의 사양을 준수하는지 확인합니다.

              **권장 사항:** New Relic UI에서 메트릭을 탐색할 때 도움이 되도록 메트릭에 범주를 접두사로 붙입니다. New Relic 통합은 현재 다음을 사용합니다.
          ...
          <tr>
            ...
            <td>
              **권장 사항:** 다음 예와 같이 원래 메트릭 이름에 아직 포함되지 않은 경우 단위 접미사를 사용하여 측정 단위를 지정합니다.

              * 백분율: `Percent` 사용 ; 예: `cpuUtilPercent` .
              * 요금: `PerSecond` 과 같은 형식을 사용합니다. 초는 표준 속도 측정이지만 `PerMinute` 또는 `PerDay` 와 같은 다른 단위를 사용할 수도 있습니다.
              * 바이트 측정: `Bytes` 을 사용합니다. **권장 사항:** 측정항목이 `Megabytes` 과 같은 다른 단위로 캡처된 경우 이를 `Bytes` 로 변환합니다. 예: `db.allMemtablesOffHeapSizeBytes` .
              * 시간 측정: `Milliseconds` 을 사용합니다. **권장 사항:** 측정항목이 `Seconds` 과 같은 다른 단위로 캡처된 경우 이를 `Milliseconds` 로 변환합니다. 예를 들어: `query.readLatency50thPercentileMilliseconds`
            </td>
          </tr>
  ...
  >
    ...
    * 새 테이블 만들기
      ...
      [인프라 **이벤트** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 및 [인프라 이벤트 히트맵](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range) 에서 이 데이터를 볼 수 있습니다. New Relic에서 `InfrastructureEvent` 이벤트 유형을 쿼리할 수도 있습니다.
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              선택 과목. 인프라 제품에 사용된 기존 범주 중 하나 또는 새 범주의 문자열 값입니다. 기본값은 `notifications` 입니다. 카테고리의 예:
  ...
  >
    ...
    * 기타 시스템 메타데이터
      ...
      [**인벤토리** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 및 [인프라 이벤트 히트맵](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range) 에서 이 데이터를 볼 수 있습니다. 인벤토리 변경과 관련된 데이터를 쿼리할 수도 있습니다.
    ...
    * 값은 스칼라 유형(문자열 또는 숫자) **이거나** 키/값의 다른 해시 개체일 수 있습니다. New Relic은 계층 구조를 지원하지만 최종 값 노드는 스칼라여야 합니다.
