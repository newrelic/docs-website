...
<DNT>**Recommendation:**</DNT> Go를 사용하여 통합을 생성하세요. 우리가 온호스트 통합과 [통합 구축 도구를](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources) 만드는 데 사용하는 언어입니다. 그러나 어떤 언어로든 통합을 생성할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    출력에는 "예쁘게 인쇄된" JSON이 아닌 표준 JSON을 사용합니다. <DNT>**Recommendation:**</DNT> 선택적 명령줄 스위치(예: `--pretty`)를 포함하여 디버깅 목적으로 JSON을 '보기 좋게 인쇄'할 수 있습니다.
  </Collapser>
...
최소 페이로드는 헤더 필드만 있는 JSON 객체입니다. <DNT>**Recommendation:**</DNT> 수집할 데이터가 없으면 프로그램 반환 코드와 `stderr` 에 기록된 로그 메시지를 사용합니다.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <DNT>**Recommendation:**</DNT> 고유한 통합 이름을 생성하려면 역방향 도메인 이름을 사용하세요.
      </td>
    </tr>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <DNT>**Recommendation:**</DNT> 통합에 사용되는 이벤트 유형이 고유한지 확인하려면 이벤트 유형 앞에 회사 이름이나 약어를 붙이십시오. 예를 들어 사용자 정의 통합이 Cassandra 노드 지표와 Cassandra 열 계열 지표를 서로 다른 샘플로 캡처하는 경우 이를 `MyOrgCassandraSample` 및 `MyOrgCassandraColumnFamilySample` 등의 서로 다른 이벤트 유형에 저장합니다.
          ...
          <tr>
            ...
            <td>
              ...
              <DNT>**Recommendation:**</DNT> 뉴렐릭 UI 에서 지표를 탐색할 때 도움이 되도록 카테고리 앞에 지표를 붙이세요. 뉴렐릭 통합은 현재 다음을 사용하고 있습니다:
          ...
          <tr>
            ...
            <td>
              <DNT>**Recommendation:**</DNT> 다음 예와 같이 원래 지수 이름에 측정 단위가 아직 포함되어 있지 않은 경우 단위 접미사를 사용하여 측정 단위를 지정합니다.
