...
우리의 온호스트 통합은 `remote_monitoring` 옵션을 `true` 로 설정하여 <DNT>**remote entity,**</DNT> 이라는 자체 엔터티를 생성하도록 구성할 수 있습니다. `false` 으로 설정하면 통합이 로컬 엔터티로 간주되고 이와 관련된 데이터가 에이전트가 생성하는 호스트 엔터티에 연결됩니다. 원격 모니터링에는 인프라 에이전트 [버전 1.2.25 이상이](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes) 필요합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="경고 확인"
  >
    이 새로운 기능의 영향을 받는 값을 사용하는 경우에는 [<DNT>**Enabling remote monitoring can affect your configured alerts**</DNT>](/docs/verify-your-alerts-after-activating-remote-monitoring) 입니다. 기존 <InlinePopover type="alerts"/>(를) 확인하여 예상대로 계속 작동하는지 확인하는 것이 좋습니다.
  </Collapser>
