...
[인프라 에이전트 통합](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 은 `remote_monitoring` 옵션을 `true` 으로 설정하여 **remote entity** \[원격 엔터티] 라는 자체 엔터티를 생성하도록 구성할 수 있습니다.`false` 로 설정하면 통합이 로컬 엔터티로 간주되고 관련 데이터가 에이전트가 생성하는 호스트 엔터티에 연결됩니다. 원격 모니터링에는 인프라 에이전트 [버전 1.2.25 이상이](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes) 필요합니다.
