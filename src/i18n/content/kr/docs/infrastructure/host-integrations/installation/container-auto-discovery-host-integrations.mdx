...
<CollapserGroup>
  <Collapser id="docker-example" title="Docker 예시(비조정)">
  ...
  <Collapser id="kubernetes-example" title="쿠버네티스 예시">
...
검색을 구성하려면 다음 속성을 사용하여 통합 구성 YAML에서 `discovery` 항목을 설정합니다. <DNT>**all**</DNT> 속성이 정의된 값과 일치하면 컨테이너가 일치됩니다.

<Callout variant="tip">
  일치하는 값이 `/.../` 사이에 설정되지 않은 경우 속성 값은 <DNT>**exact string**</DNT> 과 일치됩니다. 슬래시 안의 내용은 [정규식](https://en.wikipedia.org/wiki/Regular_expression) 으로 일치됩니다.
</Callout>
