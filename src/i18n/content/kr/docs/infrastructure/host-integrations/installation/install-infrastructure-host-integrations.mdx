...
일부 통합은 인프라 에이전트와 함께 작동합니다. 즉, 인프라 에이전트와 함께 통합 패키지를 설치하게 됩니다.

이 문서에서는 이러한 통합을 위한 기본 설치 절차를 설명합니다. 이러한 통합의 작동 방식에 대한 기술적인 세부 정보는 [온호스트 통합 데이터 보고를](/docs/infrastructure/host-integrations/understand-use-data/host-integration-data-collection-reporting) 참조하세요.

## 보고된 데이터 [#data]

인프라 에이전트 보고서 측정항목 및 인벤토리 데이터와 함께 작동하는 통합입니다. 이 데이터에 대한 세부정보는 각 통합에 대한 특정 문서에 설명되어 있습니다.

또한 로그 보고를 활성화하는 옵션도 있습니다. 로그 활성화에 대한 지침은 통합 관련 문서에 있습니다.
