...
비밀 관리를 사용하면, 민감한 데이터(예: 비밀번호)를 설정 파일에 일반 텍스트로 쓰지 않고도 사용하도록 에이전트 및 온-호스트 통합을 설정할 수 있습니다. 현재 Hashicorp Vault, AWS KMS, CyberArk, Azure Key Vault, 뉴렐릭 CLI 난독화 및 사용자 지정 명령이 지원됩니다.
...
온-호스트 통합 설정 파일에서 환경 변수를 사용하는 경우 [`passthrough_environment`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#passthrough_-environment) 설정을 정의해야 합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Azure Key Vault 시크릿 검색 설정](#azure-key-vault-secrets)
      </td>
    </tr>
...
## Azure Key Vault 시크릿 [#azure-key-vault-secrets]

Azure Key Vault에서 시크릿을 가져오려면 `azure-key-vault` 섹션에 다음 속성을 설정하십시오. 두 키가 모두 필요합니다:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        시크릿을 가져올 Azure Key Vault 인스턴스의 URL

        예시: `https://my-vault.vault.azure.net/`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        가져올 시크릿의 이름입니다. 에이전트는 항상 최신 버전의 시크릿을 가져옵니다.
      </td>
    </tr>
  </tbody>
</table>

에이전트는 Azure SDK의 기본 자격 증명 체인을 사용하여 Azure Key Vault에 인증하며, 성공할 때까지 다음 방법을 순서대로 시도합니다. 추가 에이전트 설정은 필요하지 않습니다:

* 환경 변수 `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` 및 `AZURE_TENANT_ID`
* 에이전트를 실행하는 Azure VM 또는 AKS 파드에 할당된 관리형 ID(프로덕션 환경에 권장됨)
* 개발 및 테스트에 유용한 Azure CLI(`az login`)의 자격 증명

<Callout variant="tip">
  시크릿 값만 지원됩니다. Azure Key Vault 인증서 또는 키 검색은 지원되지 않습니다.
</Callout>

다음 예제는 Azure Key Vault에서 `mssql-password` (이)라는 이름의 시크릿을 검색하여 MSSQL 온-호스트 통합의 비밀번호로 사용합니다:
