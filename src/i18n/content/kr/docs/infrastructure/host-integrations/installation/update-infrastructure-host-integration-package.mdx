...
<CollapserGroup>
  ...
  <Collapser
    id="yum"
    title={<>yum으로 업데이트(<img src={osAmazonLinux} title="amazon linux.png" alt="amazon linux.webp" style={{ height: '32px', width: '32px', verticalAlign: 'middle' }}/>아마존 리눅스,<img src={osCentos} title="centos icon" alt="centos icon" style={{ height: '32px', width: '32px', verticalAlign: 'middle' }}/>CentOS,<img src={osRedhat} title="redhat icon" alt="redhat icon" style={{ height: '32px', width: '32px', verticalAlign: 'middle' }}/>RHEL)</>}
  >
    **권장 사항:** 인프라 에이전트를 [업데이트](/docs/infrastructure/new-relic-infrastructure/installation/update-infrastructure-agent) 합니다( [버전 1.0.703 이상](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes) ).
