...
[인프라 에이전트 통합은](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 자동으로 업데이트되지 않습니다. 이를 업데이트하려면 패키지 관리자를 사용하여 프로그램 및 해당 종속성을 최신 버전으로 업데이트하십시오. [인프라 에이전트](/docs/infrastructure/new-relic-infrastructure/installation/update-infrastructure-agent) 와 통합 파일을 모두 최신 상태로 유지하는 것이 좋습니다.
