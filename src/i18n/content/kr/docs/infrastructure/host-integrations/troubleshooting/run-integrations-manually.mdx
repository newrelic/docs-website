---
title: 인프라 에이전트 통합을 수동으로 실행(테스트 실행)
...
[인프라 에이전트 통합이](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 예상대로 구성되고 메트릭을 수집하는지 알고 싶습니다. 이를 진단하려면 인프라 에이전트와 함께 테스트 실행 플래그를 사용하거나 통합을 수동으로 실행하십시오.
