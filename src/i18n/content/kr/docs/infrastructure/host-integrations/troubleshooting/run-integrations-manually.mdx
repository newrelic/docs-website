...
통합이 예상대로 구성되고 메트릭을 수집하는지 알고 싶습니다. 이를 진단하려면 인프라 에이전트와 함께 테스트 실행 플래그를 사용하거나 통합을 수동으로 실행하십시오.

## 해결책

### 통합 드라이런

버전 1.27.0부터 인프라 에이전트는 문제 해결을 위해 테스트 실행 모드에서 통합을 실행할 수 있습니다. 이 모드에서 에이전트는 제공된 구성에서 통합을 실행하고 통합 출력을 표준 출력으로 인쇄합니다.
