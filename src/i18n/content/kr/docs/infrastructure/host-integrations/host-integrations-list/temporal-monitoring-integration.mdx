...
<Steps>
  <Step>
    ...
    임시 통합을 사용하려면 먼저 동일한 호스트에 [인프라 에이전트를 설치](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) 해야 합니다. 클러스터 에이전트 모니터는 호스트 자체를 모니터링하는 반면, 다음 단계에서 설치하게 될 통합은 데이터베이스 및 측정 지표와 같은 임시 데이터로 모니터링을 확장합니다.  
  ...
  <Step>
    ## 통합 구성 [#configure-prometheus]

    성공적으로 설치한 후에는 설정을 구성해야 합니다.
