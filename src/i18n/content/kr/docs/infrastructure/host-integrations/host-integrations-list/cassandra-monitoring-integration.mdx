...
<CollapserGroup>
  ...
  >
    ...
    5. 자동 Cassandra 오류 로그 구문 분석 및 전달을 활성화하려면 `cassandra-log.yml.example` 파일을 `cassandra-log.yml` 에 복사(또는 이름 변경)하세요.에이전트를 다시 시작할 필요가 없습니다.
...
Cassandra 통합은 지표(<DNT>**M**</DNT>) 및 인벤토리(<DNT>**I**</DNT>) 정보를 모두 수집합니다. 표에서 각 컬렉션에 사용할 수 있는 설정에 대한 <DNT>**Applies To**</DNT> 열을 사용합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="환경 변수 교체"
  >
    이 구성에서는 `CASSANDRA_HOST` 환경 변수를 사용하여 통합의 `HOSTNAME` 설정을 채웁니다.
...
이 통합의 데이터는 다음으로 이동하여 찾을 수 있습니다. <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services > Cassandra**</DNT>.
