...
특정 Linux 버전의 전체 목록은 [호환되는 운영 체제](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent/#operating-systems) 표를 확인하세요.
...
* `LD_LIBRARY_PATH` 환경 변수에서 라이브러리 경로를 확인하거나 추가하세요.
...
1. [통합 설치](/docs/install-integrations-package) 지침을 따르고 `nri-ibmmq` 을 파일 이름으로 사용합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        IBM MQ 서비스에 인증하기 위한 사용자 이름입니다. 암호를 지정하지 않으면 사용자/암호 인증이 비활성화되고 구성에서 사용자 이름을 지정해서는 안 됩니다. 내보내기는 통합을 실행하는 사용자에게 clientID로 전송을 연결합니다. 에이전트 설치 모드에 따라 "루트" 또는 "nri-agent"가 될 수 있습니다.
      </td>
...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/newrelic-prometheus-exporters-packages "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.
