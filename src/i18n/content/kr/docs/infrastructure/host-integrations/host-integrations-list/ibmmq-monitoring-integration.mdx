...
<CollapserGroup>
  ...
  >
    ...
    4. 선호하는 편집기로 `ibmmq-config.yml` 구성 파일을 편집합니다. 일부 [구성 파일 예제를](#examples) 확인하십시오.
  </Collapser>
...
IBM MQ 인프라를 모니터링하려면 통합 구성에 지정된 `username` 에 특정 권한을 부여해야 합니다. 이러한 권한은 모니터링되는 대기열, 채널 및 주제에 따라 다릅니다.

일반적으로 `username` 에는 다음 권한이 있어야 합니다.

* `DISPLAY` 및 `INQUIRY` 이 활성화되어 있고 큐 관리자에 연결할 수 있는 기능이 있어야 합니다.
* 통합 구성에 지정된 채널에 액세스합니다(기본적으로 `SYSTEM.DEF.SVRCONN`).
* `DISPLAY` 권한이 있는 큐 `SYSTEM.DEFAULT.MODEL.QUEUE` 및 `SYSTEM.ADMIN.COMMAND.QUEUE` 에서 권한 명령을 실행합니다.
* 주제 `SYSTEM.ADMIN.TOPIC` 및 `DISPLAY` 및 `SUBSCRIBE` 사용하여 액세스 `SYSTEM.BROKER.ADMIN.STREAM`

특정 설정에 따라 일부 추가 권한이 필요할 수 있다는 점을 기억하세요.
