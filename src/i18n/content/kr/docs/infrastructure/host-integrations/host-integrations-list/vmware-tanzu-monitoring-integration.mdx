...
import infrastructureVmwareTanzuDashboard from 'images/infrastructure_screenshot-crop_vmware-tanzu-dashboard.png'

import infrastructureVmwareTanzuAlertChart from 'images/infrastructure_screenshot-crop_vmware-tanzu-alert-chart.png'
...
* 선택적으로 LogMessage 봉투에 대한 [컨텍스트 지원](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents) 의 로그를 포함하여 LogMessage 및 HttpStartStop 봉투를 New Relic의 [Logs UI](/docs/logs/log-management/get-started/get-started-log-management) 로 보냅니다.

## 호환성 및 요구 사항 [#requirements]

우리의 통합은 VMware Tanzu Application Service 버전 2.7 - 2.13 및 Ops Manager 버전 2.5 - 2.10과 호환됩니다. BOSH 줄기세포는 Ubuntu Xenial을 기반으로 해야 합니다.
...
VMware Tanzu 통합을 설치하고 활성화하면 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Third-party services > VMware Tanzu dashboard** 에서 데이터 및 사전 정의된 차트를 찾을 수 있습니다. 데이터를 쿼리하여 사용자 지정 차트와 대시보드를 만들고 계정에 추가할 수 있습니다.
...
HTTP 요청의 전체 수명 주기입니다. 모든 공유 [데코레이션](#decoration) 필드를 포함합니다. 이러한 이벤트는 선택적으로 [로그 UI](/docs/logs/new-relic-logs/ui-data/explore-your-data-new-relic-logs-ui) 에서 시각화를 위해 New Relic으로 보낼 수 있습니다.
...
로그 라인 및 관련 메타데이터. 모든 공유 [Aggregation](#aggregation) , [App](#app) 및 [장식](#decoration) 필드를 포함합니다. 이러한 이벤트는 선택적으로 [로그 UI](/docs/logs/new-relic-logs/ui-data/explore-your-data-new-relic-logs-ui) 에서 시각화를 위해 New Relic으로 보낼 수 있습니다.
