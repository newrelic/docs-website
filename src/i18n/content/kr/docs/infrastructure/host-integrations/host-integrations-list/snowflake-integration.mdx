...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="important">
      Snowflake 메트릭을 설정하기 전에 `SNOWSQL` 이 올바르게 구성되었는지 확인하세요.
    </Callout>

    아래 명령을 실행하여 Snowflake 메트릭을 JSON 형식으로 저장하고 `nri-flex` 에서 읽을 수 있도록 합니다. `ACCOUNT`, `USERNAME`, `SNOWSQL_PWD` 을 이에 따라 수정하세요.
  ...
  <Step>
    ...
    1. [one.newrelic.com](https://one.newrelic.com) 에서, <DNT>**+ Integrations &amp; Agents**</DNT> 페이지로 이동
