...
1. 절차에 따라 파일 이름 `nri-postgresql` 을 사용 [하여 인프라 통합 패키지를 설치하십시오](/docs/integrations/host-integrations/installation/install-host-integrations-built-new-relic) .
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="PostgreSQL Azure/AWS 구성 파일"
  >
    * Azure/AWS SSL 사용 옵션: AWS RDS/Aurora 및 Azure Flexible 관리형 데이터베이스 제품은 일반적으로 연결에 SSL이 필요합니다. 이 요구 사항을 수용하려면 이러한 설정을 `true` 으로 설정해야 합니다.
    ...
    * 요약: 이러한 설정이 완료되면 전체 Azure/AWS 구성 파일은 아래와 같아야 합니다. 참고: 인프라 에이전트 및 Postgresql 통합은 데이터베이스 인스턴스에 대한 네트워크 액세스 권한이 있는 호스트에 설치해야 합니다. 여러 인스턴스 구성의 경우 아래와 같이 추가 인스턴스마다 `-name:` 으로 시작하는 섹션을 반복해야 합니다.
