...
<CollapserGroup>
  <Collapser
    ...
    title="리눅스 설치"
  >
    1. 파일 이름 `nri-varnish` 을(를) 사용하여 [통합 설치](/docs/install-integrations-package) 지침을 따르십시오.
...
Varnish Cache 통합은 메트릭( <strong>M</strong> ) 및 인벤토리( <strong>I</strong> ) 정보를 모두 수집합니다. 아래의 **적용** 대상 열을 확인하여 각 특정 컬렉션에 사용할 수 있는 설정을 찾으십시오.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        New Relic에서 이 인스턴스의 데이터를 식별하기 위한 사용자 정의 이름입니다. **필수** .
      </td>
    ...
    <tr>
      ...
      <td>
        ...
        * `/etc/sysconfig/varnish/varnish.params`
          ...
          참고: Varnish 구성 파일의 위치와 이름은 다를 수 있습니다. 자세한 내용 [은 Varnish 구성 파일의 다른 위치를](https://book.varnish-software.com/4.0/chapters/Getting_Started.html#different-locations-of-the-varnish-configuration-file) 참조하십시오. 6보다 낮은 Varnish 버전의 경우 이 매개변수가 필요하지 않으며 통합은 메트릭 수집용으로만 설정해야 합니다. [Varnish 6의 예를](#example6) 참조하십시오.
      </td>
    ...
    <tr>
      ...
      <td>
        사용자 정의 `-n` 플래그로 `varnishd` 데몬을 실행할 때 사용되는 이름입니다. **선택 사항** .
      </td>
...
인프라에서 통합 데이터를 찾으려면 [**one.newrelic.com**](http://one.newrelic.com) **> Infrastructure > Third-party services** 로 이동하여 Varnish Cache 통합 링크 중 하나를 선택하십시오.
...
데이터는 **config/varnish** 소스 아래의 [인벤토리 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 에서 사용할 수 있습니다. 인벤토리 데이터에 대한 자세한 내용은 [통합 데이터 이해](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data) 를 참조하십시오.

## 소스 코드 확인 [#source-code]

이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-varnish "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.