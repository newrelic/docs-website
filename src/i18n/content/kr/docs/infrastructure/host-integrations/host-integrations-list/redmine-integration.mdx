...
Redmine 대시보드를 사용하면 웹 프로세서 속도, 충실도 및 데이터베이스를 쉽게 추적할 수 있습니다. 루비 에이전트로 구축된 Redmine 퀵스타트는 가장 핵심 데이터 올인원 장소를 볼 수 있는 기본 <InlinePopover type="dashboards"/>세트를 제공합니다.
...
설치가 완료되면 <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capaibilities)**</DNT> 으로 이동한 다음 <DNT>**Metrics & events**</DNT> 클릭하여 뉴렐릭이 Redmine 데이터를 수집하고 있는지 확인하세요. 다음 NRQL 쿼리를 실행하여 뉴렐릭이 Redmine 데이터를 수신하고 있는지 확인하세요.
...
기본 대시보드는 해당 원시 데이터를 차트와 그래프로 변환하여 시스템 상태에 대한 조감도를 제공합니다. 기본 대시보드를 설치하려면 [Redmine 즉시 관찰 페이지](https://newrelic.com/instant-observability/redmine) 로 이동하세요.
...
3. <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT> 으로 이동한 다음 <DNT>**Metrics & events**</DNT> 클릭하세요. 다음 쿼리를 실행하여 뉴렐릭이 맞춤 지표를 보고하는지 확인하세요.
