...
1. [인프라 에이전트를 설치하고](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install) `nri-nginx` 를 파일 이름으로 사용합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="HTTP 기본 인증"
  >
    이 구성은 기본 인증으로 보호되는 로컬 호스트에서 지표 및 인벤토리를 수집합니다. STATUS_URL의 `username` 및 `password` 을 사용자 인증 정보로 바꿉니다.
  ...
  <Collapser
    ...
    title="환경 변수 교체"
  >
    이 구성에서는 환경 변수 `NGINX_STATUS` 를 사용하여 통합의 STATUS_URL 설정을 채웁니다.
