...
4. 선호하는 편집기로 `nginx-config.yml` 구성 파일을 편집합니다. 일부 [구성 파일 예제를 확인하십시오.](#examples).

5. 자동 NGINX 액세스 및 오류 로그 구문 분석 및 전달을 활성화하려면 `nginx-log.yml.example` 파일을 `nginx-log.yml` 에 복사(또는 이름 변경)하십시오. 에이전트를 다시 시작할 필요가 없습니다.
