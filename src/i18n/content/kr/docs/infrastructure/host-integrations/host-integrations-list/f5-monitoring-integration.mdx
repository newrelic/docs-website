...
<CollapserGroup>
  <Collapser
    ...
    title="리눅스 설치"
  >
    1. 파일 이름 `nri-f5` 을(를) 사용하여 [통합 설치](/docs/install-integrations-package) 지침을 따르십시오.
...
2. 사용자가 생성되면 사용자에게 [iControl REST 사용자 권한](https://devcentral.f5.com/articles/icontrol-rest-fine-grained-role-based-access-control-30773) 을 할당합니다.
...
* `{{}}` 표기법을 사용하여 환경 변수의 값을 바꿉니다. 이를 위해서는 인프라 에이전트 v1.14.0 이상이 필요합니다. [통과 사용에 대한](/docs/infrastructure/install-infrastructure-agent/configuration/configure-infrastructure-agent/#passthrough) 자세한 내용을 읽거나 [환경 변수 교체 예제를](#envvar-replacement) 참조하십시오.
...
통합 데이터를 찾으려면 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Third-party services** 로 이동하여 F5 BIG-IP 통합 링크 중 하나를 선택하십시오.
...
데이터는 **config/f5** 소스 아래의 인프라 [인벤토리 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 에서 사용할 수 있습니다. 인벤토리 데이터에 대한 자세한 내용은 [통합 데이터 이해](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data) 를 참조하십시오.
...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-f5 "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.