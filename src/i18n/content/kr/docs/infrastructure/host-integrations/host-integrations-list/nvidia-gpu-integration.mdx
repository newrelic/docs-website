...
NVIDIA GPU 통합은 GPU 상태를 모니터링하는 데 도움이 됩니다. 이 통합은 NVIDIA의 SMI 유틸리티와 원활하게 통합되는 인프라 에이전트 및 Flex 통합을 활용합니다. GPU 사용률, ECC 오류 수, 활성 컴퓨팅 프로세스, 클럭 및 성능 상태, 온도, 팬 속도, 지원되는 각 장치에 대한 동적 및 정적 정보를 포함하여 중요한 GPU 지표가 포함된 사전 구축된 대시보드를 제공합니다.
...
데이터를 New Relic으로 가져오려면 인프라 에이전트를 설치하십시오. 당사의 인프라 에이전트는 GPU 성능을 추적할 수 있도록 데이터를 수집하고 수집합니다.
...
<Callout variant="important">
  NVIDIA-smi는 Linux 및 Windows Server에 NVIDIA GPU 디스플레이 드라이버가 사전 설치된 상태로 배송됩니다.
</Callout>

Flex를 구성하려면 다음 단계를 따르십시오.
...
2. 다음 구성으로 `nvidia-smi-gpu-monitoring.yml` 파일을 업데이트합니다.
