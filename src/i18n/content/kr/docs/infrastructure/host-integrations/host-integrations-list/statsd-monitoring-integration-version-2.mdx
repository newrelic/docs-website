...
이 통합은 [Metric API](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api) 와 [Event API](/docs/insights/insights-data-sources/custom-data/introduction-event-api) 를 사용하여 데이터를 수집합니다. 이러한 API를 사용하려면 <InlinePopover type="licenseKey" /> 가 필요합니다.
...
StatsD 통합을 설치하려면 다음 명령을 실행하고 [New Relic 계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 및 <InlinePopover type="licenseKey" /> 를 포함합니다. 그러면 `gostatsd` 에서 사용하는 TOML 구성 파일이 생성됩니다.
...
<CollapserGroup>
  ...
  >
    다음은 Kubernetes 환경에서 StatsD를 배포하고 `newrelic-statsd` 라는 StatsD 서비스를 생성하기 위한 Kubernetes 매니페스트의 예입니다. [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 와 <InlinePopover type="licenseKey" /> 를 입력해야 합니다.
