...
환경 변수를 사용하여 다음과 같은 구성 설정을 제어할 수 있습니다. <InlinePopover type="licenseKey"/>, 그런 다음 인프라 에이전트로 전달됩니다. 통과 기능을 사용하는 방법에 대한 지시사항은 [인프라 에이전트 구성 을](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#passthrough)참조하십시오.
