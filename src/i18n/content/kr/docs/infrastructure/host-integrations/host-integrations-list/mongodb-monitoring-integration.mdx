...
[MongoDB 셸](https://docs.mongodb.com/manual/mongo/) 에서 다음 명령을 실행하여 `listCollections` 역할과 새 사용자를 만든 다음 새 사용자에게 `clusterMonitor` 및 `listCollections` 역할을 할당합니다. 참고: 사용자 이름, 비밀번호 및 이와 유사한 사용자별 값은 교체해야 합니다.
