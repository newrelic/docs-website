...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. 파일 이름 `nri-couchbase` 을(를) 사용하여 [통합 설치](/docs/install-integrations-package) 지침을 따르십시오.
...
Couchbase 통합은 Couchbase 데이터베이스의 구성 매개변수를 캡처합니다. 데이터는 **config/couchbase** 소스 아래의 인프라 [인벤토리 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 에서 사용할 수 있습니다. 인벤토리 데이터에 대한 자세한 내용은 데이터 [이해](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data) 를 참조하십시오.
...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-couchbase "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.