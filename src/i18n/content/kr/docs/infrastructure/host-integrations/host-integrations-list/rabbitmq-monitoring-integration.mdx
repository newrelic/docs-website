...
RabbitMQ 통합은 지표(<strong>M</strong>) 및 인벤토리(<strong>I</strong>) 정보를 모두 수집합니다. 각 특정 컬렉션에 사용할 수 있는 설정을 알아보려면 아래의 <DNT>**Applies To**</DNT> 열을 확인하세요.
...
통합은 `/etc/rabbitmq/rabbitmq.conf` 파일에 RabbitMQ의 설정 시위를 캡처합니다. [분석 데이터는 RabbitMQ 버전 3.7 이상에서만](http://www.rabbitmq.com/blog/2018/02/05/whats-new-in-rabbitmq-3-7/) 캡처됩니다. 데이터 [](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data)분석은 [<DNT>**Inventory**</DNT> ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)뷰 UI 의 페이지 , 소스 아래에 `config/rabbitmq` 표시됩니다.

<Callout variant="caution">
  `rabbit.conf` 파일에 입력한 민감한 정보는 인프라 UI의 <DNT>**Inventory**</DNT> 페이지에 표시됩니다. 여기에는 AWS의 다음과 같은 항목이 포함됩니다.
