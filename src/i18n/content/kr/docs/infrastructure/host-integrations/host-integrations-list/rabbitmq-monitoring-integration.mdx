...
<CollapserGroup>
  <Collapser
    ...
    title="앤서블"
  >
    [배치 관리](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-configuration-management-tools) 도구를 사용하여 [군대 에이전트를](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/) 시작하세요.
  </Collapser>
  ...
  >
    ...
    5. 자동 Rabbitmq 오류 로그 구문 분석 및 전달을 활성화하려면 `rabbitmq-log.yml.example` 파일을 `rabbitmq-log.yml` 에 복사(또는 이름 변경)하세요. 에이전트를 다시 시작할 필요가 없습니다.
...
* <DoNotTranslate>**Advanced:**</DoNotTranslate>
  ...
  통합은 [Tarball 형식](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations/#tarball) 으로도 제공되므로 헤드셋 매니저 외부에 설치할 수 있습니다.
...
RabbitMQ 통합은 지표(<strong>M</strong>) 및 인벤토리(<strong>I</strong>) 정보를 모두 수집합니다. 각 특정 컬렉션에 사용할 수 있는 설정을 알아보려면 아래의 <DoNotTranslate>**Applies To**</DoNotTranslate> 열을 확인하세요.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        처리할 수 있는 최대 대기열 양을 정의합니다. 이 수에 도달하면 모든 대기열이 삭제됩니다. '0'으로 정의되면 제한이 적용되지 않으며 에이전트의 메모리 증가를 방지하기 위해 `DISABLE_ENTITIES=true` 과 함께 사용됩니다.
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        노드 디스크 경보(`0` 또는 `1`). `0` 는 알람이 작동되지 않았음을 나타내고 `1` 은 알람이 작동되었음을 나타냅니다. RabbitMQ에서는 이는 `disk_free_alarm` 로 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        호스트 메모리 경보(`0` 또는 `1`). `0` 는 알람이 작동되지 않았음을 나타내고 `1` 은 알람이 작동되었음을 나타냅니다. RabbitMQ에서는 이는 `mem_alarm` 로 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        실행 중인 노드(`0` 또는 `1`). `0` 는 노드가 실행 중이 아님을 표시하고 `1` 은 노드가 실행 중임을 표시합니다. RabbitMQ에서는 이는 `running` 로 표시됩니다.
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        클라이언트에 전달되었지만 아직 확인되지 않은 초당 메시지 비율입니다. RabbitMQ에서는 이는 `message_stats.deliver_no_ack_details.rate` 으로 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        승인 모드에서 초당 대기열당 소비자에게 전달되는 메시지 비율입니다. RabbitMQ에서는 이는 `message_stats.deliver_details.rate` 으로 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        대기열당 확인 모드에서 소비자에게, 비승인 모드에서 소비자에게, `basic.get` 에 대한 응답으로 승인 모드, `basic.get` 에 대한 응답으로 비승인 모드로 전달된 메시지의 합계입니다. RabbitMQ에서는 이는 `message_stats.deliver_get` 로 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        대기열당 확인 모드, 소비자에 대한 비확인 모드, `basic.get` 에 대한 응답의 확인 모드, `basic.get` 에 대한 응답의 비확인 모드에서 전달된 메시지 합계의 초당 비율입니다. RabbitMQ에서는 이는 `message_stats.deliver_get_details.rate` 로 표시됩니다.
      </td>
    </tr>
...
통합은 `/etc/rabbitmq/rabbitmq.conf` 파일에 RabbitMQ의 설정 시위를 캡처합니다. [분석 데이터는 RabbitMQ 버전 3.7 이상에서만](http://www.rabbitmq.com/blog/2018/02/05/whats-new-in-rabbitmq-3-7/) 캡처됩니다. 데이터 [](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data)분석은 [<DoNotTranslate>**Inventory**</DoNotTranslate> ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)뷰 UI 의 페이지 , 소스 아래에 `config/rabbitmq` 표시됩니다.

<Callout variant="caution">
  `rabbit.conf` 파일에 입력한 민감한 정보는 인프라 UI의 <DoNotTranslate>**Inventory**</DoNotTranslate> 페이지에 표시됩니다. 여기에는 AWS의 다음과 같은 항목이 포함됩니다.
