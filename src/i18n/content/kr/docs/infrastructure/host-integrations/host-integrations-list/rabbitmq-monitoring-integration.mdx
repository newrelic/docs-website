...
클러스터 환경에서 통합은 하나의 노드에 연결된 하나의 통합 인스턴스만 있는 모든 클러스터에서 메트릭을 수집합니다. 이 인스턴스는 `METRICS=true` 및 `true`사용해야 합니다. 나머지 노드의 경우 인벤토리만 `METRICS=false` 및 `INVENTORY=true`를 사용하여 수집해야 합니다. 이에 대한 자세한 내용은 [인스턴스 설정을](#instance-settings)참조하십시오.
...
<Callout variant="important">
  통합에는 수집할 수 있는 대기열 수에 대한 제한이 있습니다. 대기열 수가 2000개를 초과하면 대기열 필터를 적용한 후 통합에서 대기열을 보고하지 않습니다.
</Callout>
