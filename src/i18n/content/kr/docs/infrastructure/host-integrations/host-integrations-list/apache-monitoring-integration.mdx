...
<CollapserGroup>
  ...
  >
    아래에서 설치 옵션을 선택하고 해당 단계를 완료하여 `INTEGRATION_FILE_NAME` 이라고 표시된 곳에 `nri-apache` 입력한 다음 여기로 돌아와서 [통합 구성](#configuration) 에 대한 자세한 지침을 확인하세요.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="리눅스 구성"
  >
    통합 디렉터리 `/etc/newrelic-infra/integrations.d/` 에서 다음을 실행하여 샘플 구성 파일의 복사본을 만듭니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="HTTP 기본 인증"
  >
    이 구성은 기본 인증으로 보호되는 로컬 호스트에서 지표와 인벤토리를 수집합니다. `STATUS_URL` 의 `username` 및 `password` 사용자 인증 정보로 바꿉니다.
