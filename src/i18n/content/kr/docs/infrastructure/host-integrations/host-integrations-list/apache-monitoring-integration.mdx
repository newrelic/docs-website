...
통합은 Apache 버전 2.2 및 2.4와 호환됩니다.
...
<Tabs stacked>
  <TabsBar>
    <TabsBarItem id="windows-config">
      윈도우 구성
    </TabsBarItem>

    <TabsBarItem id="linux-config">
      Linux
    </TabsBarItem>
  </TabsBar>

  <TabsPages>
    <TabsPageItem id="windows-config">
    ...
    <TabsPageItem id="linux-config">
      ...
      자동 Apache 및 Apache 오류 로그 구문 분석 및 전달을 활성화하려면 `apache-log.yml.example` 파일을 `apache-log.yml` 로 복사하거나 이름을 바꿉니다. 에이전트를 다시 시작할 필요는 없지만 Apache 로그 파일의 기본 위치를 사용하지 않는 경우 YML 파일을 해당 위치로 업데이트해야 할 수 있습니다.
...
구성 파일에는 `interval`, `timeout`, `inventory_source` 와 같은 이 통합 및 다른 온호스트 통합에 적용할 수 있는 공통 설정이 있습니다. 이러한 공통 설정에 대해 알아보려면 [온호스트 구성 형식을](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-newer-configuration-format/#configuration-basics) 참조하십시오.
