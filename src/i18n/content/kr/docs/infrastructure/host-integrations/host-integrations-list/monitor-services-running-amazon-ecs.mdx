...
Amazon ECS의 Docker 컨테이너에서 실행되는 서비스가 있고 해당 서비스가 [인프라 에이전트 통합](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) (Cassandra, Redis, MySQL 등)에서 지원되는 경우 ECS 통합을 통해 이러한 통합을 활성화할 수 있습니다. 이것은 모니터링되는 서비스, 호스트 및 컨테이너의 데이터를 보고합니다.
