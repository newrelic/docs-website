...
에이전트가 자동으로 인스턴스를 감지하는지 테스트하려면 Auto Scaling 그룹에서 [EC2 인스턴스를 종료합니다](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/terminating-instances.html) . 이제 새 사용자 데이터로 대체 인스턴스가 시작됩니다. 5분 후 [호스트 페이지](/docs/infrastructure/infrastructure-ui-pages/infrastructure-ui/infrastructure-hosts-page) 에 새 호스트의 데이터가 표시되어야 합니다.
...
4. NGINX 상태 페이지가 포트 80에서 `STATUS_URL` 의 요청을 처리하도록 설정된 경우 인프라 에이전트가 이를 모니터링하기 시작합니다. 5분 후 NGINX 데이터가 인프라 UI( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third party services** 또는 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third party services**)에 나타나는지 확인합니다.
...
5. 구성이 작동하면 EC2 시작 구성에 배치합니다.
   ...
   6. **사용자 데이터** 섹션에서 `write_files` 섹션( `text/cloud-config` 으로 표시된 부분)을 수정합니다.
