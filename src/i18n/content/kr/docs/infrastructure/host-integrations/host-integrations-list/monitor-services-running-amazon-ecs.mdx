...
4. NGINX 상태 페이지가 포트 80에서 `STATUS_URL` 의 요청을 처리하도록 설정된 경우 인프라 에이전트가 모니터링을 시작합니다. 5분 후 NGINX 데이터가 인프라 UI( [**one.newrelic.com**](http://one.newrelic.com) **> Infrastructure > Third-party services** 또는 **[one.newrelic.com](https://one.newrelic.com) > Explorer > On-host** )에 나타나는지 확인합니다.
