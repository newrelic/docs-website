...
인프라 에이전트를 설치하고 앱과 함께 구성하면 **Metrics & events**에서 원시 데이터를 볼 수 있습니다. 당사의 기본 대시보드는 원시 데이터를 차트와 그래프로 변환하여 시스템 상태에 대한 조감도를 제공합니다. 기본 대시보드를 설치하려면 Ansible Automation Controller [Instant Observability 페이지](https://newrelic.com/instant-observability/ansible-automation-controller)로 이동하십시오.
