...
<CollapserGroup>
  ...
  >
    ...
    그런 다음 구성 템플릿을 사용하여 "nri-prometheus-config.yml"이라는 생성된 파일을 업데이트합니다.

    그런 다음 아래 나열된 필드를 업데이트합니다.
    ...
    5. `nri-prometheus-config.yml` 파일은 다음과 같아야 합니다.
  ...
  >
    ...
    Linux 시스템에서 로그 파일은 다음 위치에 있어야 합니다.

    ```
    /etc/newrelic-infra/logging.d/
    ```

    위에 나열된 경로에 로그 파일이 표시되지 않으면 로그 파일을 만들어야 합니다.
