...
<CollapserGroup>
  ...
  >
    Ansible Automation 플랫폼에 로그인한 후 <DoNotTranslate>**Users**</DoNotTranslate> 섹션으로 이동하여 <DoNotTranslate>**admin**</DoNotTranslate> 선택한 다음 사용자에 <DoNotTranslate>**token**</DoNotTranslate> 를 추가합니다. 토큰을 복사하여 원하는 경로에 저장하세요.
...
에이전트가 설치되고 앱과 함께 배포되면 <DoNotTranslate>**Metrics & events**</DoNotTranslate> 에서 원시 데이터를 볼 수 있습니다. 기본 대시보드는 해당 원시 데이터를 차트와 그래프로 변환하여 시스템 상태에 대한 조감도를 제공합니다. 기본 대시보드를 설치하려면 Ansible Automation Controller [인스턴트 옵저버빌리티 페이지](https://newrelic.com/instant-observability/ansible-automation-controller) 로 이동하세요.
