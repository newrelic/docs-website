...
New Relic Flex는 애플리케이션에 구애받지 않는 올인원 도구로 다양한 서비스에서 메트릭 데이터를 수집할 수 있습니다. 인프라 에이전트와 함께 번들로 제공됩니다. 표준 형식(예: JSON 또는 일반 텍스트)의 표준 프로토콜(HTTP, 파일, 셸)을 통해 메트릭을 노출하는 모든 앱을 계측할 수 있습니다. YAML 구성 파일을 만들고 인프라 에이전트를 시작하면 데이터가 보고됩니다. 새로운 유물에.
...
Flex 통합은 인프라 에이전트와 함께 번들로 제공됩니다. [요구 사항에 대해 자세히 알아보십시오](#requirements) .

자세한 내용은 GitHub의 전체 설명서를 참조하세요.
