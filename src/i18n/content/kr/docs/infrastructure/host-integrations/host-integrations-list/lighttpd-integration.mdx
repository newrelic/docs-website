...
<Steps>
  ...
  <Step>
    ...
    4. 미리 빌드된 대시보드가 표시되면 클릭하여 계정에 설치하십시오.
       ...
       Lighttpd 퀵스타트를 사용하고 지표 및 알림을 보려면 <DNT>**Install now**</DNT> 버튼이 있는 [Lighttpd 퀵스타트 페이지를](https://newrelic.com/instant-observability/lighttpd) 팔로우할 수도 있습니다.

       다음은 몇 가지 예시 쿼리입니다.

       <DNT>**Example:**</DNT> 뉴렐릭 쿼리 빌더에서 캡처된 지표를 확인하세요.

       ```sql
       SELECT * FROM LighttpdStatusSample  LIMIT MAX
       ```

       <DNT>**Example:**</DNT> 서버별 활성 연결을 확인하세요.
...
## 대시보드에서 Lighttpd 지표 보기 [#dashboard]

앱에 부하 에이전트가 설치되어 있기만 하면 지표 및 이벤트에서 원시 데이터를 볼 수 있습니다. 기본 <InlinePopover type="dashboards"/>은 원시 데이터를 차트와 그래프로 변환하여 시스템 상태를 한눈에 볼 수 있게 해줍니다.

Lighttpd 퀵스타트를 사용하려면 기본 대시보드를 설치하고, 지표스타 알림을 보려면 <DNT>**Install now**</DNT> 버튼을 클릭하여 [Lighttpd 퀵스타트 페이지를](https://newrelic.com/instant-observability/lighttpd) 팔로우할 수도 있습니다.
