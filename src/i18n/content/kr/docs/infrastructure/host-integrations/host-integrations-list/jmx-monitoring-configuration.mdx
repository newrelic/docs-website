...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Kubernetes"
  >
    Kubernetes 통합 설치에 대한 지시사항은 [Kubernetes에서 실행 중인 모니터 서비스를](/docs/monitor-service-running-kubernetes) 참조하십시오.
  </Collapser>
  ...
  >
    ...
    6. [구성 설정](/docs/infrastructure/host-integrations/host-integrations-list/jmx-monitoring-configuration/) 에 설명된 대로 `jmx-config.yml` 파일을 편집합니다.
  ...
  >
    ...
    4. [구성 설정을](/docs/infrastructure/host-integrations/host-integrations-list/jmx-monitoring-configuration/) 사용하여 `jmx-config.yml` 구성 파일을 편집합니다.
