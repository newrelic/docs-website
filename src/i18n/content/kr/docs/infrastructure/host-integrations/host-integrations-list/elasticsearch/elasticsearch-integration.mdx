...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="다중 인스턴스 모니터링"
  >
    이 구성에서는 동일한 통합에서 여러 Elasticsearch 서버를 모니터링하고 있습니다. 첫 번째 인스턴스( `HOSTNAME: 1st_elasticsearch_host` )의 경우 측정항목과 인벤토리를 수집하는 반면 두 번째 인스턴스( `HOSTNAME: 2nd_elasticsearch_host` )의 경우 측정항목만 수집합니다.
