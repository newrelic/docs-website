...
환경 변수는 <LicenseKey/> 와 같은 구성 설정을 제어하는 데 사용할 수 있으며 인프라 에이전트로 전달됩니다. 이 기능을 사용하는 방법에 대한 지시사항 [은 인프라 에이전트 구성](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#passthrough) 을 참조하십시오.
