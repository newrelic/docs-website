...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-kafka "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 자체 포크를 만들어 빌드할 수 있습니다.

## 레이블 및 사용자 정의 속성 [#labels]

환경 변수를 사용하여 라이센스 키와 같은 구성 설정을 제어한 다음 인프라 에이전트로 전달할 수 있습니다. 이 기능을 사용하는 방법에 대한 지침 [은 인프라 에이전트 구성](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#passthrough) 을 참조하십시오.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        모니터링 중인 클러스터를 고유하게 식별하기 위한 사용자 정의 이름. **필수** .
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        수집하는 생산자. 각 공급자에 대해 `name` , `hostname` , `port` , `username` 및 `password` 를 JSON 형식으로 제공할 수 있습니다. `name` 는 Kafka에 나타나는 생산자 이름입니다. `hostname` , `port` , `username` 및 `password` 는 선택적 JMX 설정이며 지정되지 않은 경우 기본값을 사용합니다. KafkaProducerSample을 생성하는 데 필요합니다.

        **예시: `[{"name": "myProducer", "host": "localhost", "port": 24, "username": "me", "password": "secret"}]`**
      </td>
    ...
    <tr>
      ...
      <td>
        수집할 소비자. 각 소비자에 대해 `name` , `hostname` , `port` , `username` 및 `password` 를 JSON 형식으로 지정할 수 있습니다. `name` 는 Kafka에 표시되는 소비자의 이름입니다. `hostname` , `port` , `username` 및 `password` 는 선택적 JMX 설정이며 지정되지 않은 경우 기본값을 사용합니다. KafkaConsumerSample을 생성하는 데 필요합니다.

        **예시: `[{"name": "myConsumer", "host": "localhost", "port": 24, "username": "me", "password": "secret"}]`**
      </td>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        참고: `consumer_groups` 은(는) 더 이상 사용되지 않습니다. 대신 이 인수를 사용하세요. 이 옵션은 CONSUMER_OFFSET이 true일 때 설정해야 합니다.
      </td>
