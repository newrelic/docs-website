...
환경 변수는 <InlinePopover type="licenseKey"/>등의 구성 설정을 제어하는 데 사용될 수 있으며, 그런 다음 인프라 에이전트를 통해 전달됩니다. 이 기능을 사용하는 방법에 대한 지침은 [인프라 에이전트 구성을](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#passthrough) 참조하세요.
...
Kafka 통합은 지표 및 인벤토리 정보를 모두 수집합니다. 각 컬렉션에 사용 가능한 설정을 보려면 아래의 <DNT>**Applies To**</DNT> 열을 확인하세요.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        브로커 바이너리 이름이 `kafka_2.12-2.7.0` 인 경우 사용할 Kafka API 버전은 2.7.0이고 이전 2.12는 Scala 언어 버전입니다.
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        특정 리스너를 사용하여 브로커에 연결합니다. 설정하지 않으면 성공적인 연결 테스트를 통과한 첫 번째 리스너가 사용됩니다. 지원되는 값은 `PLAINTEXT`, `SASL_PLAINTEXT`, `SSL` 및 `SASL_SSL` 입니다.
...
Kafka 통합은 지표(<strong>M</strong>) 및 인벤토리(<strong>I</strong>) 정보를 모두 수집합니다. 각 특정 컬렉션에 사용할 수 있는 설정을 알아보려면 아래의 <DNT>**Applies To**</DNT> 열을 확인하세요.
...
Kafka 통합은 지표 및 인벤토리 정보를 모두 수집합니다. 각 특정 컬렉션에 사용할 수 있는 설정을 알아보려면 아래의 <DNT>**Applies To**</DNT> 열을 확인하세요.
