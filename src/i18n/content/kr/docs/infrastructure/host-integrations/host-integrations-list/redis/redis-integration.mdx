...
* <DNT>
  ...
  통합은 [Tarball 형식](/docs/integrations/host-integrations/installation/install-host-integrations-built-new-relic#tarball) 으로도 제공되므로 클라이언트 매니저 외부에서 설치가 가능합니다.
...
<CollapserGroup>
  ...
  >
    ...
    6. 자동 Redis 오류 로그 구문 분석 및 전달을 활성화하려면 `redis-log.yml.example` 파일을 `redis-log.yml` 에 복사(또는 이름 변경)하십시오. 에이전트를 다시 시작할 필요가 없습니다.
...
Redis 통합은 지표(<DNT>**M**</DNT>) 및 인벤토리(<DNT>**I**</DNT>) 정보를 모두 수집합니다. 표에서 각 컬렉션에 사용할 수 있는 설정에 대한 <DNT>**Applies To**</DNT> 열을 사용합니다.
