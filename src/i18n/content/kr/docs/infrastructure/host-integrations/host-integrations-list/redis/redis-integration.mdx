...
* <DNT>**Advanced:**</DNT> 통합은 [Tarball 형식](/docs/integrations/host-integrations/installation/install-host-integrations-built-new-relic#tarball) 으로도 제공되므로 클라이언트 매니저 외부에서 설치가 가능합니다.
...
<CollapserGroup>
  ...
  <Collapser id="linux-install" title="Linux">
    ...
    4. Redis 서버 연결 방법에 따라 `redis-config.yml` 파일을 편집합니다.
       ...
       <CollapserGroup>
         <Collapser id="socket-connect" title="유닉스 소켓과 연결">
         ...
         <Collapser id="tcp-connect" title="TCP와 연결">
...
<CollapserGroup>
  ...
  <Collapser id="basic-auth" title="기본 인증">
  ...
  <Collapser id="acl-auth" title="REDIS 6+ ACL 인증">
  ...
  <Collapser id="metrics-tls-connection" title="TLS 연결이 있는 측정항목 전용">
  ...
  <Collapser id="metrics-connect-socket" title="소켓을 통한 메트릭 전용 연결">
  ...
  <Collapser id="renamed-commands" title="이름이 바뀐 명령 사용">
  ...
  <Collapser id="multi-instance-keys-filtering" title="키 필터링을 통한 다중 인스턴스 모니터링">
