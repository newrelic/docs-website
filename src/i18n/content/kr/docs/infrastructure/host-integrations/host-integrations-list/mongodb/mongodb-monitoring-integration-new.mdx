...
<CollapserGroup>
  ...
  >
    ...
    Bitnami 내보내기 또는 `percona/mongodb_exporter` 를 실행하고 스크래핑하면 스크래핑된 측정항목은 동일하며 <InlinePopover type="dashboards"/>일하다. 그러나 통합은 다른 클러스터에서 오는 측정항목을 그룹화할 수 있도록 특정 MongoDB 클러스터 `mongodb_cluster_name` 에서 스크랩된 모든 측정항목에 공통 라벨을 추가합니다.
