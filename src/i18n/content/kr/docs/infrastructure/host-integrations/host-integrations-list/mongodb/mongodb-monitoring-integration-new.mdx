...
* [`clusterMonitor` 및 `read` 권한이](#enable-instance)있는 MongoDB 사용자.
...
메트릭을 검색하려면 통합에 `clusterMonitor` 및 `readAnyDatabase` 역할이 있는 사용자가 필요합니다. 환경에 따라 다음 단계를 따르십시오.
...
<CollapserGroup>
  ...
  >
    [사용자 생성](https://docs.mongodb.com/manual/reference/method/db.createUser/index.html)에 대한 자세한 내용은 MongoDB 설명서를 참조하십시오.
...
<CollapserGroup>
  <Collapser
    ...
    title="아틀라스"
  >
    [사용자 생성](https://www.mongodb.com/docs/atlas/security-add-mongodb-users/#add-database-users)에 대한 자세한 내용은 MongoDB Atlas 설명서를 참조하십시오.
    ...
    1. Atlas CLI를 사용하여 다음 명령을 실행하여 새 사용자를 생성하고 사용자에게 `clusterMonitor` 및 `readAnyDatabase` 역할을 할당합니다(필요에 따라 사용자 이름과 비밀번호 조정).
       ...
       또는 Atlas UI를 사용하여 사용자를 생성하고 동일한 특정 권한을 할당할 수 있습니다.
  </Collapser>
</CollapserGroup>
...
## mongodb3-config.yml 샘플 파일 [#examples]
...
* 측정항목은 엔티티 `MONGODB_INSTANCE` , `MONGODB_DATABASE` 및 `MONGODB_COLLECTION` 의 측정항목 샘플에 연결됩니다.
...
`MONGODB_INSTANCE` , `MONGODB_DATABASE` 및 `MONGODB_COLLECTION` 의 3개 항목이 생성되었습니다.
