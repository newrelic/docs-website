...
우리의 통합은 MongoDB 버전 4.0-7.0, [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb) 및 MongoDB Atlas M10 이상과 호환됩니다.
...
<ButtonGroup>
  <ButtonLink
    role="button"
    to="https://one.newrelic.com/marketplace/install-data-source?state=e927b8cd-c047-36b9-040c-3e5c150efa49"
    variant="primary"
  >
    안내 설치
  </ButtonLink>
  ...
  <ButtonLink
    role="button"
    to="https://one.eu.newrelic.com/marketplace/install-data-source?state=e927b8cd-c047-36b9-040c-3e5c150efa49"
    variant="primary"
  >
    EU 가이드 설치
  </ButtonLink>
...
<CollapserGroup>
  ...
  >
    ...
    4. 선호하는 편집기로 `mongodb3-config.yml` 구성 파일을 편집합니다. 일부 [구성 파일 예제를](#examples) 확인하십시오.

    5. 자동 Mongodb 오류 로그 구문 분석 및 전달을 활성화하려면 `/etc/newrelic-infra/logging.d/elasticsearch-log.yml`에 다음 콘텐츠가 포함된 YAML 파일을 생성합니다. 에이전트를 다시 시작할 필요가 없습니다.
  ...
  >
    ...
    Bitnami 내보내기 또는 `percona/mongodb_exporter` 을 실행하고 스크래핑하면 스크래핑된 지표는 동일하며 <InlinePopover type="dashboards"/>이 작동합니다. 그러나 통합은 특정 MongoDB 클러스터 `mongodb_cluster_name` 에서 스크랩한 모든 지표에 공통 레이블을 추가하여 다른 클러스터에서 오는 지표를 그룹화할 수 있습니다.
    ...
    여러 `percona/mongodb_exporter` 패드를 사용하여 여러 MongoDB 클러스터에서 지표를 제공하는 경우 다음과 같은 설정을 사용하여 각각 별도의 `mongodb_cluster_name` 레이블을 지정할 수 있습니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        모니터링 중인 클러스터를 고유하게 식별하는 사용자 정의 이름입니다. <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
    ...
    <tr>
      ...
      <td>
        MongoDB 연결 URI입니다. <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
...
<CollapserGroup>
  <Collapser
    ...
    title="기본 구성"
  >
    모든 지표를 수집하는데 사용되는 기본 설정입니다. 필요한 경우 mongodb URI에 자격 증명을 추가하세요. 참고: 비밀번호에 특수 문자가 포함된 경우 URI 앞뒤에 작은따옴표를 넣어야 합니다.
...
다음 차원 측정항목은 내보내기를 스크랩하여 캡처되고 엔티티 <DoNotTranslate>**MONGODB_INSTANCE**</DoNotTranslate> 에 연결됩니다.
...
다음 차원 측정항목은 내보내기를 스크랩하여 캡처되고 엔티티 <DoNotTranslate>**MONGODB_COLLECTION**</DoNotTranslate> 에 연결됩니다.
...
다음 차원 측정항목은 내보내기를 스크랩하여 캡처되고 엔티티 <DoNotTranslate>**MONGODB_DATABASE**</DoNotTranslate> 에 연결됩니다.
...
또한 이 통합은 [커뮤니티에서 생성한 Prometheus 내보내기 도구를](https://github.com/newrelic/newrelic-prometheus-exporters-packages/tree/main/exporters/mongodb3) 활용합니다.
