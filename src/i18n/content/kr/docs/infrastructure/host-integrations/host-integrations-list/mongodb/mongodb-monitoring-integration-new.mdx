...
메트릭을 검색하려면 통합에 `clusterMonitor` 및 `read` 역할이 있는 사용자가 필요합니다. 환경에 따라 다음 단계를 따르십시오.
...
<CollapserGroup>
  ...
  >
    ...
    2. 다음 명령을 사용하여 새 사용자를 생성하고 사용자에게 `clusterMonitor` 및 `read` 역할을 할당합니다(필요에 따라 사용자 이름과 비밀번호 조정).
