...
<Callout title="공개 미리보기">
  이 통합은 현재 공개 미리 보기이며 가까운 장래에 레거시 통합을 대체할 것입니다.

  [레거시](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration) 버전을 사용 중인 경우 이 새 버전을 테스트하는 것이 좋습니다. [마이그레이션](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-migration-guide) 페이지에서 안내를 확인하세요.
</Callout>
...
<figcaption>
  [New Relic MongoDB Monitor 빠른 시작을](https://one.newrelic.com/nr1-core?state=c95ed954-7277-e44d-7fd7-351a77b3ff37)통해 설치된 대시보드.
</figcaption>
...
통합은 MongoDB 4.0 이상 및 [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb)와 호환됩니다. MongoDB Atlas와의 호환성이 곧 제공될 예정입니다.
...
통합을 구성하려면 통합의 YAML 구성 파일 `mongodb3-config.yml` 에서 구성을 편집합니다. 통합의 YAML 형식 구성은 필수 로그인 자격 증명을 배치하고 데이터 수집 방법을 구성할 수 있는 곳입니다. 변경하는 옵션은 설정 및 기본 설정에 따라 다릅니다. 구성 파일에는 모든 통합에 적용할 수 있는 공통 설정이 있습니다.

### MongoDB 서버 활성화 [#enable-instance]

MongoDB 셸에서 다음 명령을 실행하여 `clusterMonitor` 및 `read` 역할을 가진 사용자를 만듭니다. 참고: 사용자 이름, 암호 값을 바꿔야 합니다.
...
2. 다음 명령어를 사용하여 새 사용자를 만들고 사용자에게 `clusterMonitor` 및 `read` 역할을 할당합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="기본 구성"
  >
    모든 메트릭을 수집하는 데 사용되는 기본 구성입니다. **참고**: 필요한 경우 mongodb UR에 자격 증명을 추가하십시오.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [구성 파일](#instance-settings)에 설정된 클러스터 이름입니다. **참고:** 이 측정기준은 모든 측정항목에 적용됩니다.
      </td>
    </tr>
