...
Bitnami 내보내기를 실행하고 스크랩하면 스크랩된 메트릭이 동일하고 대시보드가 작동합니다. 그러나 통합은 다른 클러스터에서 오는 측정항목을 그룹화할 수 있도록 특정 MongoDB 클러스터 `mongodb_cluster_name` 에서 스크랩한 모든 측정항목에 공통 라벨을 추가합니다.
