...
<Callout variant="important">
  <DoNotTranslate>**Large environments:**</DoNotTranslate> 800개가 넘는 가상 머신이 있는 환경에서는 통합이 모든 데이터를 보고할 수 없으며 실패할 수 있습니다. 모든 지표와 이벤트를 보존하는 해결 방법을 제공하지만, 엔터티 등록이 비활성화됩니다. 해결 방법을 적용하려면 설정 파일에 다음 환경 변수를 추가하세요.
...
<CollapserGroup>
  ...
  >
    ...
    이벤트는 <DoNotTranslate>**Events**</DoNotTranslate> 페이지에서 사용할 수 있으며 [NRQL을](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) 통해 `vSphereEvent` 아래의 `InfrastructureEvent` 로 쿼리할 수 있습니다. 다음은 vSphere 이벤트 데이터의 예입니다.
  ...
  >
    성능 지표는 VMware 리소스의 현재 상태에 대한 더 나은 이해를 제공하며 기본적으로 수집되는 지표에 대해 <DoNotTranslate>**in addition**</DoNotTranslate> 까지 수집할 수 있으며 페이지 하단에 설명된 샘플에 포함됩니다.
