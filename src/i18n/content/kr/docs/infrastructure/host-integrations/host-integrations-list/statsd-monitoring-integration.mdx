...
이 통합은 [지표 API](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api) 와 [이벤트 API를](/docs/insights/insights-data-sources/custom-data/introduction-event-api) 사용하여 데이터를 수집합니다. 이러한 API를 사용하려면 <InlinePopover type="licenseKey"/>이 필요합니다.
...
StatsD 통합을 설치하려면 다음 명령을 실행하고 [뉴렐릭 계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 와 <InlinePopover type="licenseKey"/>을 포함하세요. 그러면 `gostatsd` 에서 사용하는 TOML 설정 파일이 생성됩니다.
...
<CollapserGroup>
  ...
  >
    다음은 Kubernetes 환경에서 StatsD를 구현하고 배포하고 `newrelic-statsd` 이라는 StatsD 서비스를 생성하기 위한 Kubernetes 매니페스트의 예입니다. 귀하의 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 와 <InlinePopover type="licenseKey"/>을(를) 삽입해야 합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        지표를 수신할 주소를 나타냅니다. 기본값: `:8125`. nri-statsd `v2.3.0` (goStatsD `v34.2.1`)부터 Unix 소켓 소켓(UDS)을 통한 연결이 지원됩니다. 설정에서 `[host]:port` 대신 `metrics-addr=/some/path/newrelic-statsd.socket` 사용하십시오.
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required.**</DNT> [지표 유형](#metric-types):
    ...
    <tr>
      ...
      <td>
        <DNT>**Optional**</DNT> 간단한 카운터 또는 타이머 카운터의 경우. 많은 지표를 전송해야 하는 경우 샘플링을 사용하여 네트워크 트래픽을 줄일 수 있습니다. 단점은 데이터 해상도가 감소한다는 것입니다.
    ...
    <tr>
      ...
      <td>
        <DNT>**Optional.**</DNT> 지표에 첨부된 태그는 속성(핵심 가치 쌍)으로 변환됩니다. 태그 지정 옵션에 대한 자세한 내용은 [태그 를](#add-tags) 참조하세요.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  >
    ...
    NRQL 공지 조건을 생성할 때 반드시 <DNT>**Condition name**</DNT> 을 설정하세요.
...
* StatsD 통합과 관련된 [`NrIntegrationError`](/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/) 없는지 확인하세요.
