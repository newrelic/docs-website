...
<CollapserGroup>
  <Collapser id="jmx-instance" title="JMX 인스턴스 설정">
  ...
  <Collapser id="labels" title="라벨 및 사용자 정의 속성">
...
<CollapserGroup>
  <Collapser id="domains" title="도메인">
...
<CollapserGroup>
  <Collapser id="jmx-configuration-configmap-example-using-helm" title="helm 차트를 사용하는 configMap의 예(권장)">
    ```yaml
    nri-bundle:
  ...
  <Collapser id="jmx-configuration-configmap-example-plain-manifest" title="일반 매니페스트를 사용하는 configMap의 예">
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
<CollapserGroup>
  <Collapser id="host-connection-example" title="호스트 연결 파일의 예">
    ```yml
    integrations:
      - name: nri-jmx
        env:
  ...
  <Collapser id="collection-example" title="메트릭 수집 파일의 예">
    ```yml
    collect:
...
권장 사항: 서로 다른 요소에 걸쳐 유사한 지표에 대해 동일한 명명 체계를 사용하세요.
...
<CollapserGroup id="troubleshooting-list">
  <Collapser id="integration-errors" title="오류에 대한 로그 검색">
  ...
  <Collapser id="metrics-limit" title="측정항목 한도 초과">
  ...
  <Collapser id="missing-metrics" title="측정항목 누락">
  ...
  <Collapser id="missing-dashboard" title="인프라 UI에 대시보드가 표시되지 않음">
    구성 `jvm-metrics.yml` 파일이 업데이트되었고 파일 경로가 `jmx-config.yml` 파일에 열거되어 있는지 확인합니다.
  </Collapser>

  <Collapser id="jmxterm" title="JMXTerm을 통한 문제 해결">
