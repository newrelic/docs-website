...
4. 즐겨 사용하는 편집기로 `nginx-config.yml` 구성 파일을 수정합니다. 몇 가지 [구성 파일 예시](#examples)를 확인해보십시오.

5. 인프라 에이전트를 재시작합니다. [다양한 Linux 환경에서 인프라 에이전트를 다시 시작하는 방법](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status/#linux)을 참조하십시오.

6. 자동 NGINX 액세스와 오류 로그 구문 분석 및 전달을 활성화하려면 `nginx-log.yml.example` 파일을 `nginx-log.yml`로 복사(또는 이름 변경)합니다. 에이전트를 다시 시작할 필요가 없습니다.
...
통합 데이터를 찾으려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**로 이동하여 Microsoft SQL Server 통합 링크 중 하나를 선택합니다.
