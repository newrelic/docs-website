...
통합은 Microsoft SQL Server 2014 이상과 호환됩니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="멀티 인스턴스 연결"
  >
    이 구성은 동일한 SQL Server에서 `mssql_instance1` 및 `mssql_instance1` 인스턴스를 모니터링합니다. 인스턴스에 직접 연결할 때는 `PORT` 설정을 제거해야 합니다. SQL Browser 서비스도 서버에서 실행 중이어야 합니다.
