...
<CollapserGroup>
  <Collapser id="32-bit-path" title="32비트 Windows">
    ```shell
    msiexec.exe /qn /i PATH_TO\nri-mssql-amd386.msi
    ```
  </Collapser>

  <Collapser id="64-bit-path" title="64비트 Windows">
    ```shell
    msiexec.exe /qn /i PATH_TO\nri-mssql-amd64.msi
    ```
  </Collapser>
...
Microsoft SQL Server를 모니터링하려면 `CONNECT`, `VIEW SERVER STATE` 및 `VIEW ANY DEFINITION` 액세스 권한이 있는 사용자가 필요합니다.

<Callout variant="tip">
  Microsoft SQL Server에서 [로그인](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-login-transact-sql?view=sql-server-2017) 및 [사용자](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-user-transact-sql?view=sql-server-2017) 생성에 대한 자세한 내용은 Microsoft 문서를 확인하십시오. [서버 범위 권한](https://learn.microsoft.com/en-us/sql/t-sql/statements/grant-server-permissions-transact-sql?view=sql-server-ver16#remarks)(뉴렐릭 사용자는 3을 사용) 및 [데이터베이스 역할](https://learn.microsoft.com/en-us/sql/relational-databases/security/authentication-access/database-level-roles?view=sql-server-ver16#fixed-database-roles)(뉴렐릭 사용자는 부여받지 않으며 `public`으로 유지)에 대한 문서도 있습니다.
</Callout>

새 로그인을 생성하고 필요한 권한을 부여합니다.

<CollapserGroup>
  <Collapser id="sql-user" title="SQL 사용자의 경우">
    1. 새 로그인을 만들고 `CONNECT`, `VIEW SERVER` 및 `VIEW ANY DEFINITION` 권한을 부여합니다.
    ...
    2. 다음 명령문을 사용하여 사용자에게 `public` DB 액세스 권한을 부여합니다.
  ...
  <Collapser id="domain-user" title="Windows 도메인 사용자의 경우">
    1. 새 로그인을 만들고 `CONNECT`, `VIEW` 및 `VIEW ANY DEFINITION` 권한을 부여합니다.
...
<CollapserGroup>
  ...
  <Collapser id="basic-config-domain" title="Windows 도메인 승인과 기본 구성">
  ...
  <Collapser id="multi-instance" title="멀티 인스턴스 연결">
  ...
  <Collapser id="metrics-connect-ssl" title="SSL을 통한 메트릭 전용 연결">
  ...
  <Collapser id="disable-buffer_reserve" title="버퍼 풀 및 파티션 리저브 메트릭 최적화">
...
<CollapserGroup>
  <Collapser id="database-metric" title="데이터베이스 메트릭">
  ...
  <Collapser id="instance-metric" title="인스턴스 메트릭">
  ...
  <Collapser id="wait-metric" title="대기 메트릭">
