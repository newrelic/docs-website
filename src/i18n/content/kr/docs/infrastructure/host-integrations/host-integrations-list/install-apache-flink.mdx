...
Apache Flink 대시보드를 사용하면 로그를 쉽게 추적하고, 로그 소스를 감시하고, 모든 로그를 제외하고 인력타임 및 다운타임에 대한 개요를 얻을 수 있습니다. 우리의 에이전트 에이전트와 Prometheus OpenMetrics 통합으로 구축된 Flink <InlinePopover type="dashboards"/>는 OpenMetrics 엔드포인트 스크래핑을 활용하므로 올인원 위치에서 가장 중요한 데이터를 모두 볼 수 있습니다.
...
[로그 전달](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/) 문서를 사용하여 애플리케이션별 로그를 New Relic에 전달할 수 있습니다.
...
Apache Flink 퀵스타트를 사용하고 징 및 알림을 보려면 <DNT>**Install now**</DNT> 버튼을 클릭하여 [Apache Flink 퀵스타트 페이지를](https://newrelic.com/instant-observability/apache-flink) 팔로우할 수도 있습니다.
...
Apache Flink 대시보드를 NRQL 추가로 맞춤설정하려면 <InlinePopover type="dashboards"/>뉴렐릭 에서 쿼리 구축 및 관리에 대해 자세히 알아볼 수 UI 있습니다.
