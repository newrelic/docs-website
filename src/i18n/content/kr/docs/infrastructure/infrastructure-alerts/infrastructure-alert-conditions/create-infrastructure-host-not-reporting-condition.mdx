...
인프라 모니터링의 **Host not reporting** \[호스트가 보고하지 않음] [조건을](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information) 사용하면 인프라 에이전트로부터 데이터 수신이 중단되었을 때 알림을 받을 수 있습니다. 이 기능을 사용하면 호스트 그룹에 대해 동적으로 경고하고, 기간을 5-60분으로 구성하고, <InlinePopover type="alerts"/>공지를 최대한 활용할 수 있습니다.
...
4. 명령줄을 통해 호스트가 종료되도록 설정된 경우 허위 경고를 방지하려면 **Don't trigger alerts for hosts that perform a clean shutdown** \[완전 종료를 수행하는 호스트에 대해 경고를 트리거하지 않음] 옵션을 활성화합니다.현재 이 기능은 systemd를 사용하는 모든 Windows 시스템 및 Linux 시스템에서 지원됩니다.

   <Callout variant="tip">또한 위에 언급된 옵션을 확인하면서 호스트에 `hostStatus: shutdown` [태그를](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) 추가할 수 있습니다. 이렇게 하면 에이전트 버전이나 OS에 관계없이 해당 태그가 있는 한 인시던트를 **Host not reporting** \[보고하지 않는 모든 호스트가] 해당 호스트에 대해 열리는 것을 중지합니다. 태그를 제거하면 시스템이 해당 호스트에 대해 인시던트를 **Host not reporting** \[보고하지 않는 모든 호스트가] 다시 열 수 있습니다.</Callout>
