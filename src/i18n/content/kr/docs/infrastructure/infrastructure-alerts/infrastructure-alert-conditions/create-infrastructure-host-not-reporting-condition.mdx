...
인프라 모니터링의 **Host not reporting** \[호스트가 보고하지 않음] [조건을](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information) 사용하면 인프라 에이전트로부터 데이터 수신이 중단되었을 때 알림을 받을 수 있습니다. 이 기능을 사용하면 호스트 그룹에 대해 동적으로 경고하고, 시간 창을 5-60분으로 구성하고, 다음과 같은 이점을 최대한 활용할 수 있습니다. <InlinePopover type="alerts"/>알림.
...
4. 명령줄을 통해 종료하도록 호스트를 설정한 경우 잘못된 경고를 방지하려면 **Don't trigger alerts for hosts that perform a clean shutdown** \[완전한 종료를 수행하는 호스트에 대해 경고를 트리거하지 않음 옵션을] 활성화합니다. 현재 이 기능은 systemd를 사용하는 모든 Windows 시스템 및 Linux 시스템에서 지원됩니다.

   <Callout variant="tip">또는 위에서 언급한 옵션을 확인하면서 호스트에 `hostStatus: shutdown` [태그를](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) 추가할 수 있습니다. 이렇게 하면 에이전트 버전이나 OS에 관계없이 해당 태그가 있는 한 해당 호스트에 대한 모든 **Host Not Reporting** 인시던트가 열리지 않습니다. 태그를 제거하면 시스템에서 해당 호스트에 대한 **Host Not Reporting** \[호스트 보고 안 함] 사건을 다시 열 수 있습니다.</Callout>

정책의 [인시던트 기본 설정](/docs/alerts/new-relic-alerts/configuring-alert-policies/specify-when-new-relic-creates-incidents) 에 따라 조건에 대해 정의된 **Critical** \[중요] 임계값이 통과할 때 사용할 알림 채널을 정의합니다. "가양성"을 방지하려면 호스트는 인시던트가 열리기 전 전체 기간 동안 보고를 중지해야 합니다.

**예:** 필터링된 호스트 세트 중 하나가 **7** 분 동안 데이터 보고를 중지하는 경우 인시던트를 여는 조건을 생성합니다.

* 호스트가 5분 동안 보고를 중지한 다음 보고를 다시 시작하면 조건이 인시던트를 열지 **않습니다** .
* 호스트가 7분 동안 보고를 중지하면 다른 호스트는 정상이더라도 조건이 인시던트를 엽니 **다** .
...
4. [**Incident details** \[사건 세부 정보\] 페이지](/docs/alerts-applied-intelligence/new-relic-alerts/alert-incidents/view-event-details-incidents) 에서 추가 세부 정보를 검토하려면 이메일 링크를 사용하세요.
