...
3. 공지를 트리거하기 위한
   ...
   레버 값을 정의합니다: 최소 5분, 최대 60분.

4. 배열을 통해 호스트가 종료되도록 설정된 경우 잘못된 알림을 방지하려면

   <DNT>**Don't trigger alerts for hosts that perform a clean shutdown**</DNT>

   옵션을 활성화하세요. 현재 이 기능은 systemd를 사용하는 모든 Windows 시스템 및 Linux 시스템에서 지원됩니다.

   <Callout variant="tip">또한 위에 언급된 옵션을 확인하면서 호스트에 `hostStatus: shutdown` [태그를](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) 추가할 수 있습니다. 이렇게 하면 에이전트 버전이나 OS에 관계없이 해당 태그가 있는 한 해당 호스트에 대해 모든 <DNT>**Host not reporting**</DNT> 인시던트가 열리지 않습니다. 태그를 제거하면 시스템이 해당 호스트에 대해 <DNT>**Host not reporting**</DNT> 인시던트를 다시 열 수 있습니다.</Callout>
...
* 호스트가 5분 동안 보고를 중지한 후 보고를 재개하면 조건

  <DNT>**does not**</DNT>

  이 인시던트를 엽니다.

* 호스트가 7분 동안 보고를 중지하면 다른 호스트는 괜찮더라도 조건
...
2. 전자 메일 공지의 링크를 사용하여 어셈블리 의
   ...
   [ 페이지 에서 환경의 진행 중인 변경](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) UI 사항을 모니터링할 수 있습니다. 예를 들어,

   <DNT>**Events**</DNT>

   페이지를 사용하면 루트 사용자가 호스트에 대한 설정을 변경한 직후 호스트 연결이 끊어졌는지 확인하는 데 도움이 됩니다.

3. 선택 사항: 이메일 공지의

   <DNT>[**Acknowledge**](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/acknowledge-alert-incidents)</DNT>

   [ 링크를](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/acknowledge-alert-incidents) 사용하여 알림 인시던트를 인지하고 있으며 이에 대한 소유권을 갖고 있는지 확인하세요.

4. 이메일 링크를 사용하여

   <DNT>[**Incident details**](/docs/alerts-applied-intelligence/new-relic-alerts/alert-incidents/view-event-details-incidents)</DNT>

   [ 페이지](/docs/alerts-applied-intelligence/new-relic-alerts/alert-incidents/view-event-details-incidents) 의 추가 세부정보를 검토하세요.
