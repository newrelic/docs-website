...
귀하의 비즈니스에 중요한 호스트와 프로세스에 필터를 적용함으로써, [알림을](/docs/alerts/overview#alert-threshold) 정의할 수 있습니다. 공지 이벤트가 열리는 시기를 결정하고 뉴렐릭이 정책의 [공지 이벤트 기본](/docs/alerts/organize-alerts/specify-when-alerts-create-events) 설정에 따라 공지 이메일을 귀하에게 보냅니다. 이 예제들은 인프라 모니터링의 <DNT>**Process running**</DNT> 조건을 사용하여 프로세스를 모니터링하는 방법을 보여줍니다.

<CollapserGroup>
  <Collapser id="processes-load" title="로드를 충족하기에 충분한 프로세스가 실행 중인지 확인">
    <DNT>**Problem:**</DNT> 일부 로드 밸런싱 장치와 애플리케이션 서버는 여러 작업자 프로세스를 병렬로 실행하여 작동합니다. 예를 들어, gunicorn과 같은 서비스에 대해 실행 중인 프로세스가 8개 미만일 때 알림 이벤트를 발생시키고 싶을 수 있습니다.
  ...
  <Collapser id="one-critical" title="특별한 주의가 필요한 중요한 프로세스의 시작을 모니터링합니다.">
    ...
    <DNT>**Solution:**</DNT> 뉴렐릭이 프로세스가 실행 중일 때 공지 이벤트를 열 수 있도록 사용자 이름과 특정 실행 파일로 설정된 조건 필터와 함께 <DNT>**At least one process is running**</DNT> 레버 값을 사용하세요.
  </Collapser>

  <Collapser id="one-job-length" title="작업이 너무 오래 걸리지 않는지 확인">
    <DNT>**Problem:**</DNT> 주기적으로 실행되는 작업이 있는데, 예상 시간(분)보다 오래 실행되었을 때 알림 이벤트를 열고 싶습니다.
...
공개 모니터링을 사용하여 공지 조건을 직접 생성하는 경우, 뉴렐릭은 정책의 [공지 이벤트 기본 설정](/docs/alerts/organize-alerts/specify-when-alerts-create-events) 에 따라 공지 조건에 대해 정의된 규칙 값이 통과되면 공지 이메일을 보냅니다. 귀하의 공지사항은 당사가 사용하는 인사나 팀 및 공지채널을 정의합니다.