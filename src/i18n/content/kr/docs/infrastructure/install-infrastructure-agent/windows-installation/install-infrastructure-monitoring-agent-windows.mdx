...
스크립트 설치의 경우 구성 매개변수를 전달할 수 있습니다. 먼저 `GENERATE_CONFIG=true` 및 `LICENSE_KEY=YOUR_LICENSE_KEY` 추가해야 하며 그런 다음 다음 선택적 매개변수를 추가할 수 있습니다.
...
다음 예에서는 <LicenseKey/> 설정하고 아웃바운드 통신을 위한 프록시 서버를 구성하고 하나의 사용자 정의 [속성을](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)추가합니다.
...
4. `C:\Program Files\New Relic\newrelic-infra\` 에 있는 `newrelic-infra.yml` 파일의 `license_key` 속성에 <LicenseKey/> 추가합니다. 완료되면 `newrelic-infra.yml` 파일은 다음과 같아야 합니다.
