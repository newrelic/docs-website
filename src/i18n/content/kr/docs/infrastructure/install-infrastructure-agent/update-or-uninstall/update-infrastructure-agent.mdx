...
* 단일 호스트: 모든 [호스트 항목](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#find) 에서 `agentVersion` 태그를 확인하거나 **Infrastructure** \[인프라] UI에서 해당 `Agent version` 를 찾습니다.

* 여러 호스트: **Infrastructure > Navigator** 로 이동하고 보고 항목을 `agentVersion` 별로 그룹화하여 버전별 활성 호스트 목록을 확인합니다.
