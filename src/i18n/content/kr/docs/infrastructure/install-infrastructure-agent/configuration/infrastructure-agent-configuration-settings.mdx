...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      <DoNotTranslate>**Recommendation:**</DoNotTranslate> 이 설정을 변경하면 안됩니다.
    </Callout>
  ...
  >
    ...
    <DoNotTranslate>**negative value**</DoNotTranslate>(음수값)으로 설정하면 에이전트는 성공할 때까지 연결을 계속 확인합니다.
...
<CollapserGroup>
  ...
  >
    ...
    에이전트/호스트 엔터티에 대한 정보를 반환합니다. 에이전트에 에이전트/호스트 엔터티에 대한 정보가 없으면 응답 상태 코드 _204_ ("콘텐츠 없음")가 반환됩니다. 따라서 에이전트가 엔터티 데이터를 제공할 때까지 여러 요청이 필요할 수 있습니다.
