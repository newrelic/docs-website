...
<CollapserGroup>
  <Collapser
    ...
    title="추적하다"
  >
    지정된 기능에 대한 로그에 추적을 인쇄할 수 있습니다. 추적은 특정 기능의 디버깅 목적으로 사용되는 추가 로그 항목이며 [자세한 로그 구성 옵션](#verbose) 이 활성화된 경우에만 표시됩니다.

    현재 두 가지 기능 `connect` 및 `attributes` )에 정의된 추적이 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    기본 설치는 로그 디렉토리를 만들고 `log_file` 구성 옵션에서 이 파일 경로 값을 설정합니다. 이 로그 디렉토리는 아래와 같이 OS마다 다릅니다.
  ...
  <Collapser
    ...
    title="말 수가 많은"
  >
    `verbose` 이 `0` 로 설정되면 자세한 로깅이 해제되지만 에이전트는 여전히 로그를 생성합니다. 에이전트 [문제 해결](/docs/infrastructure/new-relic-infrastructure/troubleshooting/generating-logs-troubleshooting-infrastructure) 에 사용할 자세한 로그를 생성하려면 이것을 `1` 로 설정하십시오. [스마트 상세 정보 표시 모드](/docs/infrastructure/infrastructure-troubleshooting/troubleshoot-logs/infrastructure-agent-logging-behavior#smart-verbose-mode) 로깅을 사용하려면 `verbose` 을 `2` 로 설정하십시오. 스마트 상세 정보 표시 모드는 오류가 기록될 때 마지막 `smart_verbose_mode_entry_limit` 디버그 메시지를 기록합니다. `verbose` 을 3으로 설정하면 에이전트 로그를 New Relic의 로그 UI로 전달할 수 있습니다.

    <Callout variant="important">
      자세한 로깅은 많은 데이터를 매우 빠르게 생성할 수 있습니다. 문제를 재현하는 데 필요한 기간 동안만 상세 모드에서 에이전트를 실행한 다음 `verbose: 0` 을 설정하고 [에이전트를 다시 시작하여](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status) 상세 로깅을 비활성화합니다. 또는 `verbose` : `2` 를 설정할 수 있습니다. 그러면 스마트 상세 정보 표시 모드가 활성화됩니다.
    </Callout>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            [1.9.0](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-190) 스마트 모드 사용 시( `2` )

            [1.11.4](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-1114-0) 로그를 New Relic( `3` )에 전달할 때
          </td>
        </tr>
      </tbody>
    </table>
  ...
  >
    `smart_verbose_mode_entry_limit` 오류가 기록될 때 기록될 이전 디버그 메시지의 수를 나타냅니다. 예를 들어 제한이 `5` 으로 설정된 경우 디버그 로그는 오류가 기록될 때까지 메모리에 캐시되며 이 시점에서 이전 5개의 디버그 메시지도 기록됩니다.

    <Callout variant="important">
      이 구성 옵션은 `verbose` 이 `2` (Smart Verbose Mode 활성화됨)로 설정된 경우에만 사용됩니다.
    </Callout>
