...
Linux용 인프라 모니터링 에이전트는 기본적으로 [Docker 환경을 지원합니다](/docs/infrastructure/install-infrastructure-agent/linux-installation/docker-instrumentation-infrastructure) . 컨테이너 OS를 실행 중이거나 에이전트를 컨테이너로 배포해야 하는 제한 사항이 있는 경우 인프라 모니터링 에이전트의 컨테이너화된 버전을 실행할 수 있습니다. 이를 통해 컨테이너 자체와 기본 호스트에 대한 메트릭을 모니터링할 수 있습니다.
...
Linux용 인프라 모니터링 에이전트를 사용해 보고 싶으십니까? 무료 [로 New Relic 계정을 만드](https://newrelic.com/signup) 십시오! 신용 카드가 필요하지 않습니다.

## 필요한 것 [#requirements]

인프라 에이전트의 컨테이너화된 버전에는 Docker 1.12 이상이 필요합니다. 컨테이너는 에이전트에서 지원하는 모든 [Linux 배포 및 버전](/docs/infrastructure/install-infrastructure-agent/get-started/compatibility-requirements-infrastructure-agent#operating-systems) 을 실행해야 합니다. 컨테이너 이미지는 AMD64 및 ARM64 아키텍처에서 사용 가능하고 지원됩니다.
...
**권장 사항:** [`newrelic/infrastructure` 이미지](https://hub.docker.com/r/newrelic/infrastructure/) 를 확장하고 고유한 `newrelic-infra.yml` 에이전트 [구성 파일](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) 을 사용합니다. 이미지가 빌드되면 추가 시작 시간 구성을 제공하지 않고도 컨테이너를 쉽게 회전할 수 있습니다. Docker에서 환경 변수를 사용하여 비밀을 제공하지 마십시오.

### 도커 CLI

1. New Relic [라이선스 키로](/docs/accounts/install-new-relic/account-setup/license-key) `newrelic-infra.yml` 에이전트 [구성 파일](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) 을 만듭니다. 구성 옵션 설명은 [구성 설정](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings) 을 참조하십시오.
...
3. New Relic [라이선스 키로](/docs/accounts/install-new-relic/account-setup/license-key) `newrelic-infra.yml` 에이전트 [구성 파일](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) 을 만듭니다. 구성 옵션 설명은 [구성 설정](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings) 을 참조하십시오.
...
인프라 에이전트가 Docker 컨테이너에서 실행되면 인프라 에이전트가 호스트에서 기본적으로 실행할 때 수집할 수 있는 것과 동일한 [호스트 컴퓨팅 데이터](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage) 및 [이벤트 데이터](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 를 수집할 수 있습니다. 자세한 내용은 [Docker 컨테이너 데이터를 보는](/docs/infrastructure/install-infrastructure-agent/linux-installation/docker-instrumentation-infrastructure#find-data) 방법에 대한 설명서를 참조하십시오.
...
이 통합은 오픈 소스 소프트웨어입니다. [소스 코드를 탐색하고](https://github.com/newrelic/infrastructure-bundle "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 자체 포크를 만들어 빌드할 수 있습니다.