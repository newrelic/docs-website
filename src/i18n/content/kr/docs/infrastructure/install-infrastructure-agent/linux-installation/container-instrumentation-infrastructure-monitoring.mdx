...
[요구 사항을](#requirements) 충족하고 올바른 인프라 모니터링 에이전트를 설치한 경우 컨테이너 모니터링을 활성화하기 위한 추가 단계는 없습니다. 컨테이너 런타임이 실행 중인 경우 데이터가 자동으로 보고됩니다.

인프라 모니터링 에이전트가 포함된 컨테이너 이미지를 사용할 수도 있습니다. 자세한 정보는 [인프라 모니터링을 위한 컨테이너를](/docs/infrastructure/install-infrastructure-agent/linux-installation/container-infrastructure-monitoring)참조하십시오.
...
* <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Hosts > Containers**</DoNotTranslate> 으)로 이동합니다.

  또는

* <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**</DoNotTranslate>으로 이동하여 docker관련 링크를 선택하세요.
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI**</DoNotTranslate> 으)로 이동합니다.
   ...
   <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Settings > Alerts**</DoNotTranslate> 으)로 이동합니다.

2. 새 [경고 조건](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions) 을 만듭니다.

3. 조건 유형으로 <DoNotTranslate>**Container metrics**</DoNotTranslate> 선택합니다.
