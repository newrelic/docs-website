...
Linux용 인프라 에이전트를 사용해 보고 싶으십니까? 무료로 뉴렐릭 계정을 만들어보세요! 신용 카드가 필요하지 않습니다.
...
<DoNotTranslate>**Recommendation:**</DoNotTranslate> [`newrelic/infrastructure` 이미지를](https://hub.docker.com/r/newrelic/infrastructure/) 확장하고 자체 `newrelic-infra.yml` 에이전트 [구성 파일을](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) 사용합니다. 일단 이미지가 구축되면 추가 설치 시간 설정을 제공하지 않고도 컨테이너를 쉽게 시작할 수 있습니다. docker 와 함께 환경 변수를 사용하여 비밀을 제공하지 마세요.
