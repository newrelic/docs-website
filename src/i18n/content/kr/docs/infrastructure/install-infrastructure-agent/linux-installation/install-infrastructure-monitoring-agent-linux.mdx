   ...
   <CollapserGroup>
     ...
     >
       ...
       <Callout variant="important">
         zypper 패키지 관리자에 알려진 문제가 있어 GPG 키가 예상대로 검증되지 않을 수 있습니다. `Signature verification failed` 같은 오류가 발생한 경우, 뉴렐릭의 [Support Forum](https://discuss.newrelic.com/t/new-relic-infrastructure-installation/121241/3)에서 자세한 내용을 살펴보십시오. 권장되는 대안은 `rpm --import` ( `gpg --import`의 인스턴스)를 사용하여 gpg 키를 rpm 신뢰할 수 있는 키 데이터베이스로 가져오는 것입니다. 나머지는 Zypper가 처리합니다.
       </Callout>
     </Collapser>
   </CollapserGroup>
