...
<Callout variant="important">
  [버전 1.4.0](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/new-relic-infrastructure-agent-140) 부터 인프라 에이전트 패키지에는 실행 중인 에이전트의 문제를 해결하는 데 사용되는 추가 `newrelic-infra-ctl` 바이너리가 포함됩니다. 이 바이너리는 에이전트를 실행하는 데 필요하지 않지만 경로에 추가하는 것이 좋습니다. [자세한 내용은 문서](/docs/infrastructure/install-configure-manage-infrastructure/manage-your-agent/troubleshoot-running-agent) 를 참조하세요.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        에이전트의 권한 수준입니다. 가능한 값은 `ROOT` , `PRIVILEGED` 또는 `UNPRIVILEGED` 입니다. 자세한 내용은 [에이전트 실행 모드](/docs/infrastructure/new-relic-infrastructure/installation/install-infrastructure-linux#agent-mode-intro) 에 대한 설명서를 참조하십시오.
