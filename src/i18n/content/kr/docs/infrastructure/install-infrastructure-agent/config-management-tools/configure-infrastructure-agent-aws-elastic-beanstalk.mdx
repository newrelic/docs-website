...
인프라 에이전트를 구성하려면 에이전트 구성 파일을 조정하거나 환경 변수를 사용하여 요구 사항에 따라 [에이전트 설정을](https://docs.newrelic.com/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/) 결정합니다. 아래 나열된 방법 중 하나를 사용하여 설정을 적용할 수 있습니다.
