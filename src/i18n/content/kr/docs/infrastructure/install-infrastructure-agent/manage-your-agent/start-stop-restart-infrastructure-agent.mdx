...
* <DoNotTranslate>**SystemD**</DoNotTranslate> (Amazon Linux 2, SLES 12, CentOS 7 이상, Debian 8 이상, RHEL 7 이상, Ubuntu 15.04 이상):

  ```shell
  sudo systemctl <start|stop|restart|status> newrelic-infra
  ```

* <DoNotTranslate>**System V**</DoNotTranslate> (Debian 7, SLES 11.4, RHEL5):

  ```shell
  sudo /etc/init.d/newrelic-infra <start|stop|restart|status>
  ```

* <DoNotTranslate>**Upstart**</DoNotTranslate> (Amazon Linux, RHEL 6, Ubuntu 14.04 이하):
...
<Callout variant="important">
  에이전트를 시작, 중지 또는 재시작하려면 `cmd.exe` 또는 <DoNotTranslate>**Administrator**</DoNotTranslate>로서 PowerShell을 실행해야 합니다.
</Callout>
