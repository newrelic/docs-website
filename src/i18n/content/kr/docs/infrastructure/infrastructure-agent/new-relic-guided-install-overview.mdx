...
## 특정 에이전트 버전 설치 [#install-specific-versions]
...
사용 가능한 에이전트 에이전트 버전과 해당 릴리스 세부 정보를 찾으려면 [에이전트 에이전트 릴리스 노트를](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/) 참조하세요.
