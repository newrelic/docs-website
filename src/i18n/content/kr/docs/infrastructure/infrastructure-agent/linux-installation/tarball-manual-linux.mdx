...
<CollapserGroup>
  <Collapser id="guess-service-manager" title="서비스 관리자 결정">
...
<CollapserGroup>
  <Collapser id="install-systemd" title="시스템화">
  ...
  <Collapser id="install-sysv" title="시스템">
  ...
  <Collapser id="install-upstart" title="건방진 녀석">
...
<CollapserGroup>
  <Collapser id="config-location-systemd" title="시스템화">
  ...
  <Collapser id="config-location-sysv" title="시스템">
  ...
  <Collapser id="config-location-upstart" title="건방진 녀석">
...
<CollapserGroup>
  <Collapser id="pidfile-agent" title="에이전트가 pid 파일을 생성하는 경우">
  ...
  <Collapser id="pidfile-env-var" title="에이전트가 PIDFILE 환경 변수를 사용하는 경우">
    <Callout variant="caution">
...
1. 서비스 스크립트를 편집합니다.
   ...
   <CollapserGroup>
     <Collapser id="mode-systemd" title="시스템화">
       1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/systemd/newrelic-infra.service` 을(를) 엽니다.
       2. `[Service]` 행을 검색합니다.
     ...
     <Collapser id="mode-sysv" title="시스템">
       1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/sysv/newrelic-infra` 을(를) 엽니다.
     ...
     <Collapser id="mode-upstart" title="건방진 녀석">
       1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/upstart/newrelic-infra` 을(를) 엽니다.
       2. `exec /usr/bin/newrelic-infra` 행을 검색합니다.
...
<CollapserGroup>
  <Collapser id="binary-location-systemd" title="시스템화">
    1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/systemd/newrelic-infra.service` 을(를) 엽니다.
    2. `[ExecStart=/usr/bin/newrelic-infra]` 행을 검색합니다.
    3. 경로를 바꿉니다.
    4. 파일을 저장합니다.
  ...
  <Collapser id="binary-location-sysv" title="시스템">
    1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/sysv/newrelic-infra` 을(를) 엽니다.
    2. `DAEMON=/usr/bin/$NAME` 행을 검색합니다.
    3. 경로를 바꿉니다.
    4. 파일을 저장합니다.
  ...
  <Collapser id="binary-location-upstart" title="건방진 녀석">
    1. 서비스 스크립트 `./newrelic-infra/etc/init_scripts/upstart/newrelic-infra` 을(를) 엽니다.
    2. `exec /usr/bin/newrelic-infra` 행을 검색합니다.
    3. 경로를 바꿉니다.
    4. 파일을 저장합니다.
