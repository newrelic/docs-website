...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="logging-http-client-timeout" title="logging_http_client_timeout">
    뉴렐릭 fluent-bit 출력 플러그인에 대한 HTTP 클라이언트 타임아웃(초)을 설정합니다. 이 설정은 로깅 포워더가 뉴렐릭으로 로그 데이터를 전송할 때 응답을 기다리는 시간을 정합니다.

    보다 자세한 내용은 [fluent-bit 출력 플러그인 문서](https://github.com/newrelic/newrelic-fluent-bit-output#httpclienttimeout)를 참조하십시오.
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="safe-bin-dir" title="safe_bin_dir">
    에이전트가 통합에 대한 실행 파일을 찾는 디렉터리입니다.
  ...
  <Collapser className="freq-link" id="agent-temp-dir" title="agent_temp_dir">
    에이전트가 임시 파일(예: 로그 전달자 설정 및 통합 검색 데이터)을 저장하는 디렉터리입니다. 이 디렉터리는 에이전트를 다시 시작할 때마다 삭제되지만, 기본값과 일치하는 경우에만 삭제됩니다.
  ...
  <Collapser className="freq-link" id="logging-home-dir" title="logging_home_dir">
    로그 전달자를 위한 바이너리 및 기타 필수 파일이 포함된 디렉터리입니다.
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="http-server-cert" title="http_server_cert">
    HTTPS를 통해 통합 페이로드를 수신하는 데 사용되는 PEM 인코딩 인증서의 경로입니다. `http_server_key`로 구성하면 에이전트는 통합 페이로드 서버에 HTTPS를 사용합니다.
  ...
  <Collapser className="freq-link" id="http-server-key" title="http_server_key">
    HTTPS를 통해 통합 페이로드를 수신하는 데 사용되는 PEM 인코딩 키의 경로입니다. `http_server_cert`로 구성하면 에이전트는 통합 페이로드 서버에 HTTPS를 사용합니다.
  ...
  <Collapser className="freq-link" id="http-server-ca" title="http_server_ca">
    HTTPS 요청에 대한 클라이언트 인증서의 유효성 검사를 시행하는 데 사용되는 PEM 인코딩된 CA 인증서의 경로입니다. 설정을 하면, 에이전트는 이 CA를 기준으로 클라이언트 인증서를 검증하여 통합 페이로드 서버에 대한 상호 TLS(mTLS) 인증을 활성화합니다.
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="win-network-interface-v2" title="win_network_interface_v2">
    활성화되면 Windows 에이전트는 네트워크 메트릭에 `GetIfEntry`(32비트 카운터)가 아니라 `GetIfEntry2`(64비트 카운터)를 사용합니다. 이는 32비트 카운터가 오버플로우될 수 있는 고대역폭 인터페이스에 대해 보다 정확한 네트워크 통계를 제공합니다.

    <Callout variant="tip">
      이 플래그는 하위 호환성을 위해 유지되며 향후 릴리스에서 제거될 수 있습니다.
    </Callout>
