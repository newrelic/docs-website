...
인프라 에이전트는 [높은 수준의 보안 모드를](/docs/agents/manage-apm-agents/configuration/high-security-mode) 지원하지 않습니다.
...
<CollapserGroup>
  <Collapser id="linux-root" title="루트로 실행">
    에이전트가 루트 사용자로 실행되면 모든 시스템 메트릭 및 인벤토리에 대한 전체 액세스 권한이 있습니다.
  </Collapser>

  <Collapser id="linux-privileged" title="권한 있는 사용자로 실행">
    에이전트는 설치 프로세스 중에 확장 커널 기능이 부여되는 `nri-agent` 이라는 권한이 없는 사용자를 실행합니다. 따라서 권한이 있는 `nri-agent` 사용자는 일부 지표와 대부분의 인벤토리를 수집할 수 있습니다. 이 권한은 읽기 전용입니다\_.\_
  ...
  <Collapser id="linux-unprivileged" title="권한이 없는 사용자로 실행">
...
Windows 시스템에서 에이전트에는 다음과 같은 설치 및 계정 요구 사항이 있습니다:

* 설치에는 관리자 계정이 필요합니다.
* 기본적으로 에이전트는 로컬 시스템 로그온 컨텍스트를 사용하는 서비스로 실행되며, 이는 인증이 필요한 네트워크의 다른 서비스에 액세스할 수 없음을 의미합니다.
* 로컬 시스템의 대안으로, 서비스를 로컬 사용자, Active Directory/Microsoft Entra ID 사용자 또는 그룹 관리 서비스 계정/gMSA로 선택적으로 변경할 수 있습니다. 포함된 스크립트 `set-service-account.ps1` 은(는) 전환을 돕고 필요한 권한을 설정합니다.
...
<CollapserGroup>
  <Collapser id="directories" title="에이전트가 액세스하는 기본 디렉터리">
  ...
  <Collapser id="packages-commands" title="에이전트가 사용하는 패키지 및 명령">
    ...
    뉴렐릭는 일부 도구나 데이터 소스를 사용하여 여러 관측 기능에 대한 정보를 수집합니다. 다음은 몇 가지 주요 사용 사례입니다. 달리 명시하지 않는 한 뉴렐릭은 이 정보를 주로 [<DNT>**Inventory**</DNT> 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 에 사용합니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td>
            다양한 플러그인이 이 디렉토리를 통해 시스템 전체의 세부 정보를 수집합니다. 부품 <DNT>**Inventory**</DNT> 및 지표에 사용됩니다.
          </td>
        ...
        <tr>
          <td>
            다양한 플러그인이 이 디렉토리를 통해 `sysctl` 설정을 수집합니다. 부품 <DNT>**Inventory**</DNT> 및 지표에 사용됩니다.
          </td>
        ...
        <tr>
          <td>
            다양한 플러그인(`sshd_config`, `hostinfo`)이 이 디렉터리의 특정 구성을 읽습니다. 부품 <DNT>**Inventory**</DNT> 및 지표에 사용됩니다.
          </td>
...
에이전트는 [컴퓨트 지표](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page), [이벤트](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 및 [인벤토리](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 설정을 설명하는 JSON 형식의 데이터를 뉴렐릭의 뷰포인트에 주기적으로 보냅니다. 이러한 통신은 귀하의 계정에 대해 생성된 <InlinePopover type="licenseKey" />사용하는 에이전트와 연결됩니다. 뉴렐릭은 외부 에이전트로부터 데이터를 수신하면 모니터링 UI 에 새로운 지표, 이벤트 또는 설정 데이터를 표시합니다.
...
모든 코드는 표준 보안 도구(<DNT>Snyk</DNT>, <DNT>Dependabot</DNT>, <DNT>Trivy</DNT>)를 통해 의존성/종속성 교리를 검사합니다.
