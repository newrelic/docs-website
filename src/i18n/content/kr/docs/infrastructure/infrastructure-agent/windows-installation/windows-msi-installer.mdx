...
3. <DNT>PowerShell</DNT>을(를) 관리자로 열고, 절대 경로를 사용하여 설치 스크립트를 실행하십시오.

   **v1.73.0 이상 참고:** `/qn` 스위치를 사용하는 자동 설치의 경우, 명령에서 직접 `GENERATE_CONFIG=true` 및 `LICENSE_KEY`을(를) 제공 **해야 합니다**. 파라미터가 없는 자동 설치를 실행하면 실패하고 오류 1603과 함께 설치가 롤백됩니다.
   ...
   * 스크립트 설치의 경우, 추가 설정 파라미터를 명령에 직접 추가할 수 있습니다:
   ...
   고급 사용자 지정은 `NRIA_` 환경 변수 및 에이전트 설정 파일을 통해 사용할 수 있습니다. [에이전트 구성](/docs/infrastructure/install-infrastructure-agent/configuration/configure-infrastructure-agent/) 방법을 알아보십시오.

4. (선택 사항) MSI 설치 단계에서 파라미터를 전달하지 않은 경우, `C:\Program Files\New Relic\newrelic-infra\`에 위치한 `newrelic-infra.yml` 파일의 `license_key` 속성에 <InlinePopover type="licenseKey" />을(를) 수동으로 생성하고 추가해야 합니다. 완료되면 `newrelic-infra.yml` 파일은 다음과 같아야 합니다:
...
몇 분 정도 기다린 다음 [인프라 UI에서 서버를 확인합니다](/docs/infrastructure/infrastructure-ui-pages/infra-ui-overview). 몇 분을 기다린 후에도 데이터가 나타나지 않으면 다음 문제 진단, 해결 페이지를 따르십시오:

* [인프라 UI에 데이터가 표시되지 않음](/docs/infrastructure/new-relic-infrastructure/troubleshooting/no-data-appears-infrastructure)
* [Windows MSI 설치가 오류 1603으로 실패합니다](/docs/infrastructure/infrastructure-troubleshooting/troubleshoot-infrastructure/windows-msi-install-fails-error-1603)
