...
**[New Relic에서 Metric Streams를 설정하는](/docs/infrastructure/amazon-integrations/connect/aws-metric-stream-setup)**방법을 알아보세요.
...
예를 들어 경고 및 <InlinePopover type="dashboards"/>`sum` 또는 `count` 을 사용하면 실제 숫자의 두 배를 반환합니다. 여기에는 `.Sum` 접미사가 있는 메트릭을 사용하는 알림 및 대시보드가 포함됩니다.
