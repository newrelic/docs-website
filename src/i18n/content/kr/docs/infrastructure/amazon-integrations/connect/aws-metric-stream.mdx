...
두 가지 형태의 통합을 모두 설정한 경우 측정항목이 중복될 수 있습니다. 예를 들어, <InlinePopover type="alerts"/>`sum` 또는 `count` 사용하는 대시보드는 실제 숫자의 두 배를 반환합니다. 여기에는 접미사가 `.Sum` 인 측정항목을 사용하는 경고 및 대시보드가 포함됩니다.
