...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** 로 이동하여 폴링에서 메트릭 스트림으로 네임스페이스를 복제하십시오. **Migrate to AWS Cloudwatch metric streams** 버튼을 클릭한 다음 AWS Metric Streams 계정을 구성합니다.
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** 로 이동합니다. **Add an AWS account** \[AWS 계정 추가를]클릭한 다음 AWS 계정을 추가합니다. 폴링 통합을 사용하여 AWS 계정을 이미 연결한 경우에도 이 단계가 필요합니다.
