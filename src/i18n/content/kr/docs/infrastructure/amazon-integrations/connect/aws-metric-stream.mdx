...
New Relic은 현재 AWS와의 독립적인 [통합을](/docs/integrations/amazon-integrations/) 제공하여 50개 이상의 AWS 서비스에 대한 성능 지표 및 메타데이터를 수집합니다.
...
개별 통합에 의존하는 현재 시스템은 폴링 플릿에서 실행되고 여러 AWS API를 정기적으로 호출하여 지표와 메타데이터를 검색합니다. AWS CloudWatch를 사용하면 지표 수집 방법이 크게 개선되어 개별 통합 사용의 일부 제한 사항을 극복할 수 있습니다.
...
* 선택적으로 리소스 메타데이터를 사용한 사용자 지정 태그 및 CloudWatch 지표 보강은 [AWS Config 서비스](https://aws.amazon.com/config/pricing/) 를 기반으로 합니다.
