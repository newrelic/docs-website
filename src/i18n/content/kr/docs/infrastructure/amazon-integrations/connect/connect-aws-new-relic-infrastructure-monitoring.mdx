...
1. [**one.newrelic.com**](https://one.newrelic.com) **> Infrastructure > AWS** 로 이동합니다. 사용 가능한 서비스 타일 중 하나를 클릭합니다.

2. [IAM 콘솔](https://console.aws.amazon.com/iam/home?#/roles) 에서 **역할 생성** 을 클릭한 다음 **다른 AWS 계정** 을 클릭합니다.

   * `Account ID` 의 경우 `754728514883` 을 사용합니다.
   * `Require external ID` 상자를 선택합니다.
   * `External ID` 에 New Relic 계정 ID를 입력합니다.
   * `Require MFA` (다단계 인증)으로 설정을 활성화 **하지 마십시오** .

3. **정책** 연결: `ReadOnlyAccess` 을(를) 검색하고 `ReadOnlyAccess` (이)라는 정책의 확인란을 선택한 후 **다음: 검토** 를 클릭합니다. 또는 [자체 관리형 정책을 생성](/docs/integrations/amazon-integrations/getting-started/integrations-managed-policies) 하고 모니터링하려는 AWS 서비스에 따라 New Relic에 부여하는 권한을 제한할 수 있습니다.
...
6. **예산** 정책 구성: 새 역할에 대한 **역할 요약** 을 보는 동안 다음을 선택합니다. **<Icon name="fe-plus-circle"/>인라인 정책을 추가** 합니다.

7. **사용자 지정 정책** 만들기: 정책 이름(예: `NewRelicBudget` )을 입력하고 다음 권한 설명을 추가한 다음 **정책 적용** 을 선택합니다.
...
8. New Relic UI로 돌아가서 AWS 계정 이름과 새 역할의 ARN을 입력합니다.

9. New Relic 인프라 통합으로 모니터링할 Amazon Web Services를 선택한 다음 **저장** 을 선택합니다.
...
1. [**one.newrelic.com**](http://one.newrelic.com) **> Infrastructure > AWS** 로 이동합니다.
