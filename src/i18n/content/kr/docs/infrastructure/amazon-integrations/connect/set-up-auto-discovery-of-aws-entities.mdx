...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="tip">
      기본적으로 뉴렐릭은 하루에 한 번씩 계정에서 활성화된 모든 지역을 검색합니다. <DNT>Auto-Discovery</DNT> 설정을 관리하여, 택시가 생성된 후에 [스캔 빈도와 지역을 업데이트](#select-region-and-frequency) 할 수 있습니다.
    </Callout>
  </Step>
  ...
  <Step>
    ...
    1. <DNT>**Launch your CloudFormation in AWS**</DNT> 을 클릭하여 AWS Console 로 이동합니다. CloudFormation 대시보드에는 Newrelric One에서 선택한 내용을 기반으로 구성된 파라미터와 변수가 포함되어 있으므로 변경 사항이 없는 한 추가 편집이 필요하지 않습니다.
...
<Steps>
  ...
  <Step>
    ...
    <DNT>Auto-Discovery</DNT> 활성화하려면 뉴렐릭 One의 확인 화면에서 <DNT>**Save**</DNT> 클릭하세요.
  </Step>
</Steps>
