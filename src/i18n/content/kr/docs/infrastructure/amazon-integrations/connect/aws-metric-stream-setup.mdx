...
그런 다음 이를 확인하려면 <InlinePopover type="alerts"/>데이터를 올바르게 처리하려면 고급 신호 설정을 구성하세요. AWS CloudWatch가 특정 지연을 통해 서비스로부터 지표를 수신하기 때문에 이러한 설정이 필요합니다. (예를 들어 Amazon은 EC2 지표의 90%가 생성된 후 7분 이내에 CloudWatch에서 사용할 수 있음을 보장합니다.) 또한 AWS에서 New Relic으로의 스트리밍 지표는 대부분 Firehose의 데이터 버퍼링으로 인해 최대 1분의 추가 지연을 추가합니다.
