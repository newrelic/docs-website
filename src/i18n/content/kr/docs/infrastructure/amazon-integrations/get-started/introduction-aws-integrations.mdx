...
* **CloudFormation으로 자동화**: CloudFormation은 선언적 템플릿을 사용하여 통합 설정을 모델링하고 자동화할 수 있는 AWS 서비스입니다. 수동 설정 오류를 줄이려면 이 방법을 사용하여 AWS 계정을 뉴렐릭에 연결하세요. 뉴렐릭 UI에서 통합을 완벽하게 설계하고 AWS 콘솔에 배포할 수 있습니다.

이 방법은 <DNT>IAM</DNT> 역할, <DNT>Amazon CloudWatch</DNT> 메트릭 스트림 및 <DNT>Amazon Kinesis Data Firehose</DNT> 전송 스트림을 포함하여 메트릭을 뉴렐릭으로 전달하는 리소스를 프로비저닝합니다. 메트릭 스트림은 리전별로 작동하므로 모니터하는 각 AWS 리전에 메트릭 스트림과 <DNT>Kinesis Data Firehose</DNT> 전송 스트림이 필요합니다. 이러한 구성 요소가 연결되는 방식은 다음 다이어그램을 참조하세요.
...
<figcaption>
  CloudWatch Metric Streams 통합이 메트릭을 뉴렐릭으로 전달하는 방법입니다. 모니터하는 각 AWS 리전에 메트릭 스트림과 Kinesis Data Firehose 전송 스트림이 필요합니다.
</figcaption>
