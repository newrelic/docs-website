...
* AWS Config는 AWS 계정에서 선택적으로 활성화할 수 있으며 사용자 지정 태그 및 리소스 메타데이터로 CloudWatch 지표를 보강하는 데 사용할 수 있습니다. AWS Config를 사용하면 기록된 구성 항목 수에 따라 요금이 부과됩니다. 자세한 내용은 [AWS Config 요금 페이지](https://aws.amazon.com/config/pricing/) 를 참조하십시오.
...
구성 프로세스를 따르면 Amazon Web Services의 데이터가 New Relic에 직접 보고됩니다. 가장 많이 사용되는 서비스에 대한 AWS 엔터티는 [New Relic Explorer](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts/) 에 나열됩니다. 측정항목 및 이벤트가 [데이터 탐색기](docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/) 에 나타납니다. AWS 데이터는 인프라 UI에서도 볼 수 있습니다.

AWS 데이터를 보려면 [**one.newrelic.com**](http://one.newrelic.com) **> Infrastructure > AWS** 로 이동합니다. 나열된 AWS 통합의 경우:
...
사전 구성된 대시보드와 **이벤트 탐색기** 대시보드 모두에서 NRQL 쿼리를 보고 재사용할 수 있습니다. 이를 통해 특정 요구 사항에 맞게 쿼리를 조정할 수 있습니다.
