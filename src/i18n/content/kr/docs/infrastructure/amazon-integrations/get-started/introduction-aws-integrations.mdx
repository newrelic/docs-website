...
CloudWatch 지표로 사용할 수 없는 데이터를 가져오기 위해 AWS CloudWatch Metric Streams 위에 추가 <DoNotTranslate>**API Polling**</DoNotTranslate> 통합을 활성화할 수 있습니다. 다음 통합은 지표 스트림으로 대체되지 않습니다.
...
* 지표 스트림 대신 폴링 통합이 활성화된 경우 뉴렐릭은 Amazon CloudWatch API 사용하여 모니터링하는 AWS 서비스에서 지표를 가져옵니다. 더 많은 통합을 활성화하면 CloudWatch API에 대한 호출 수가 늘어납니다. 해당 통합에 AWS 리소스를 추가하거나 더 많은 지역에 걸쳐 해당 통합을 확장하세요. 이로 인해 CloudWatch API에 대한 요청이 AWS에서 부여한

  <DoNotTranslate>**1 million free**</DoNotTranslate>

  한도를 초과하고 [CloudWatch 청구 금액이 늘어날](/docs/infrastructure/amazon-integrations/troubleshooting/cloudwatch-billing-increase) 수 있습니다.
...
AWS 데이터를 보려면: <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate> 으로 이동하세요. 나열된 AWS 통합의 경우:

* 활성 스트림의 경우 <DoNotTranslate>**Explore your data**</DoNotTranslate> 링크를 선택하세요.

  또는

* 다른 통합의 경우 사용 가능한 대시보드를 찾아보거나 <DoNotTranslate>**Explore Data**</DoNotTranslate> 링크를 클릭하세요.

사전 구성된 <InlinePopover type="dashboards"/>및 <DoNotTranslate>**Events explorer**</DoNotTranslate> 대시보드 모두에서 NRQL 쿼리를 보고 재사용할 수 있습니다. 이를 통해 특정 요구 사항에 맞게 쿼리를 맞춤화할 수 있습니다.
