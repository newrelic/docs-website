...
구성 프로세스를 따르면 Amazon Web Services의 데이터가 New Relic에 직접 보고됩니다. 가장 많이 사용되는 서비스에 대한 AWS 엔터티는 [New Relic Explorer](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts/) 에 나열됩니다. 측정항목 및 이벤트는 [측정항목 및 이벤트](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/) 에 표시됩니다. AWS 데이터는 인프라 UI에도 표시됩니다.

AWS 데이터를 보려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** 로 이동합니다. 나열된 AWS 통합의 경우:
