...
이 통합에 대한 선호되는 대안으로 사용자 지정 네임스페이스를 포함하여 AWS 서비스의 모든 CloudWatch 지표를 모니터링할 수 있습니다. 이렇게 하려면 [AWS CloudWatch Metric Streams integratio](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-metric-stream) \[AWS CloudWatch Metric Streams 통합을] 활성화하십시오.

또한 이 클라우드 기반 통합과 다른 데이터 세트를 보고하는 [ECS on-host integration](/docs/integrations/elastic-container-service-integration/get-started/introduction-amazon-ecs-integration) \[ECS 온호스트 통합을] 제공합니다.
...
이 통합의 데이터를 찾으려면 <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DNT> 으로 이동하여 Amazon ECS/ECR 통합 링크 중 하나를 선택하세요.
