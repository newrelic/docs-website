...
이 두 가지 유형의 데이터 외에 New Relic은 다른 데이터를 수집하지 않습니다. 다른 AWS CloudTrail 데이터는 이미 New Relic에서 인벤토리 변경 [이벤트](/docs/telemetry-data-platform/understand-data/new-relic-data-types/) 형식으로 보고하기 때문입니다.
...
인프라에서 통합 데이터를 찾으려면 [**one.newrelic.com**](https://one.newrelic.com) **> Infrastructure > AWS** 로 이동하여 AWS CloudTrail 통합 링크 중 하나를 선택하십시오.

이 통합은 측정항목 또는 인벤토리 데이터를 제공하지 않고 [이벤트 데이터](/docs/telemetry-data-platform/understand-data/new-relic-data-types/#events-new-relic) 만 제공합니다. 인프라의 [**이벤트** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 를 사용하여 이러한 이벤트의 타임라인을 볼 수 있습니다.
