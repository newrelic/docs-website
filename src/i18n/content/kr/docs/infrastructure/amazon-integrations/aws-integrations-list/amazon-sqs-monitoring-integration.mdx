...
[Amazon Simple Queue Service(Amazon SQS)는](https://aws.amazon.com/sqs/) 전송 중인 메시지를 저장하기 위한 호스팅 대기열을 제공합니다. 뉴렐릭의 SQS 통합은 전달, 지연, 수신된 메시지 수, 메시지 크기 및 기타 [지표](#metrics) 와 같은 대기열 관련 데이터를 보고합니다. SQS 데이터는 사전 구축된 <InlinePopover type="dashboards"/>에서 사용할 수 있으며 [맞춤 쿼리 및 차트를](/docs/infrastructure/integrations-getting-started/getting-started/use-integration-data-new-relic-insights) 만들 수도 있습니다. SQS 데이터의 변경 사항을 알리기 위해 공지 조건을 생성할 수도 있습니다.
