...
[Amazon Simple Queue Service(Amazon SQS)](https://aws.amazon.com/sqs/) 는 전송 중인 메시지를 저장하기 위한 호스팅 대기열을 제공합니다.New Relic의 SQS 통합은 전달, 지연 및 수신된 메시지 수, 메시지 크기 및 기타 [측정항목](#metrics) 과 같은 대기열 관련 데이터를 보고합니다.SQS 데이터는 사전 구축된 대시보드에서 사용할 수 있으며 [사용자 지정 쿼리 및 차트](/docs/infrastructure/integrations-getting-started/getting-started/use-integration-data-new-relic-insights) 를 만들 수도 있습니다.SQS 데이터의 변경 사항을 알리는 경고 조건을 생성할 수도 있습니다.
...
통합 데이터를 찾으려면 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS** 로 이동하여 SQS 통합 링크 중 하나를 선택하십시오.
