...
[새로운 Relic 인프라 통합](/docs/infrastructure/integrations-getting-started/getting-started/introduction-infrastructure-integrations) 에는 [향상된 Amazon RDS 데이터](http://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Monitoring.OS.html) 수집을 위한 통합이 포함됩니다. 이것은 데이터베이스 인스턴스가 실행되는 운영 체제에 대한 실시간 메트릭으로 기본 [New Relic RDS 통합](/docs/infrastructure/amazon-integrations/amazon-integrations/aws-rds-monitoring-integration) 을 보완합니다.
...
1. **AWS 콘솔 > CloudWatch > 로그** 에서 `RDSOSMetrics` 로그 그룹을 선택하고 **작업 >** **Lambda 구독 필터 생성** 을 적용합니다.
...
통합 데이터를 찾으려면 [**one.newrelic.com**](http://one.newrelic.com) **> Infrastructure > AWS** 로 이동하여 **RDS > Enhanced monitoring dashboard** 링크를 선택하십시오.
