...
[New Relic 인프라 통합](/docs/infrastructure/integrations-getting-started/getting-started/introduction-infrastructure-integrations) 에는 [향상된 Amazon RDS 데이터](http://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Monitoring.OS.html) 수집을 위한 통합이 포함됩니다. 이것은 데이터베이스 인스턴스가 실행되는 운영 체제에 대한 실시간 메트릭으로 기본 [New Relic RDS 통합](/docs/infrastructure/amazon-integrations/amazon-integrations/aws-rds-monitoring-integration) 을 보완합니다.

## 특징 [#features]

이 New Relic 통합을 통해 RDS Enhanced Monitoring을 모니터링하고 [경고](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information) 할 수 있습니다. 통합 데이터 및 경고를 사용하여 DB 프로세스를 모니터링하고 잠재적인 문제 지점을 식별하고 DB를 프로파일링하여 응답 및 비용을 개선하고 최적화할 수 있습니다.
...
1. Enable RDS Enhanced Monitoring이 필요한 RDS 인스턴스를 지정합니다. 인스턴스를 생성하거나 수정할 때 이를 지정할 수 있습니다. **모니터링** 아래에서 **향상된 모니터링 활성화** 를 **예로** 설정합니다. 데이터 **세분성** 을 15초로 설정하는 것이 좋습니다.
...
3. 서버리스 리포지토리에서 새 AWS Lambda 함수 생성: **Lambda > Create Function > Browse serverless App repository** 로 이동 **Show apps that create custom IAM roles or resource policies** 확인란을 선택하고 `NewRelic-log-ingestion` 을 검색합니다.
4. `LICENSE_KEY` 환경 변수를 New Relic 계정 [라이선스 키로](/docs/accounts/install-new-relic/account-setup/license-key) 채웁니다.
5. 모든 선택적 매개변수를 검토하고 사용 사례에 따라 조정하십시오.
6. **배포** 를 선택하여 새 CloudFormation 스택, `newrelic-log-ingestion` 이라는 새 함수 및 필요한 역할을 만듭니다.
...
1. **AWS Console > CloudWatch > Logs** 에서 `RDSOSMetrics` 로그 그룹을 선택하고 **Actions > Create Lambda subscription filter** 을 적용합니다.
2. **Lambda 함수** 의 경우 `newrelic-log-ingestion` 을 선택합니다.
3. **로그 형식** 드롭다운에서 로그 **형식** 으로 `JSON` 을 선택합니다.
4. 구독 **필터 이름** 을 입력합니다.
5. 하단에서 **스트리밍 시작** 버튼을 클릭하여 Lambda 구독 필터를 저장합니다.
...
통합 데이터를 찾으려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) >Infrastructure > AWS** 로 이동하여 **RDS > Enhanced monitoring dashboard** 링크를 선택하십시오.
