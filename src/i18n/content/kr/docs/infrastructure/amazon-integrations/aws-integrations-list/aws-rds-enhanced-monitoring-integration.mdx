...
이 New Relic 통합을 통해 RDS Enhanced Monitoring을 모니터링하고 [경고](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information) 할 수 있습니다. 통합 데이터를 사용할 수 있으며 <InlinePopover type="alerts"/>DB 프로세스를 모니터링하고 잠재적인 문제 지점을 식별할 뿐만 아니라 DB를 프로파일링하여 응답과 비용을 개선하고 최적화할 수 있습니다.
...
4. New Relic 계정으로 `LICENSE_KEY` 환경 변수를 채웁니다.
...
통합 데이터를 찾으려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** 로 이동하고 **RDS > 향상된 모니터링 대시보드** 링크를 선택하십시오.
