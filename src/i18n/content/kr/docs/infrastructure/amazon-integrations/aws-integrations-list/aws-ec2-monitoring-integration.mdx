...
해당 날짜 이전에 New Relic에서 모니터링한 EC2 인스턴스의 경우 레거시 태그 형식을 유지할 수 있는 옵션이 있습니다. 이러한 형식을 유지하려면 EC2 호스트의 New Relic 구성 UI로 이동하여 **Keep legacy metadata format** \[레거시 메타데이터 형식 유지를] 선택합니다. 이를 통해 현재 형식과 기존 형식이 모두 포함된 태그에 액세스할 수 있습니다. 레거시 형식을 비활성화하면 현재 형식만 사용할 수 있습니다. 레거시 형식을 비활성화하는 경우 해당 형식을 사용하는 대시보드 또는 경고 조건이 있는지 확인하는 것이 좋습니다.
