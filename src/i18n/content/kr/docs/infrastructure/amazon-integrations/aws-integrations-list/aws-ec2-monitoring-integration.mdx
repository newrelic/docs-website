...
[Amazon의 EC2](https://aws.amazon.com/ec2/) 는 Amazon 클라우드 컴퓨팅 플랫폼의 핵심 부분입니다. [구독 수준](https://newrelic.com/infrastructure/pricing) 에 관계없이 모든 New Relic Infrastructure 사용자는 New Relic Infrastructure 에이전트를 사용하여 호스트의 성능 및 상태에 대한 포괄적인 실시간 보기를 얻을 수 있습니다. New Relic의 EC2 통합은 [`ec2Describe*` 정책](/docs/infrastructure/infrastructure-integrations/getting-started/integrations-managed-policies) 을 사용하여 EC2 인스턴스에 대한 데이터를 표준 인프라 데이터에 추가합니다. 또한 인프라는 [Amazon EC2 사용자 지정 태그](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Using_Tags.html) 를 가져와 데이터에 추가합니다.
...
해당 날짜 이전에 New Relic에서 모니터링한 EC2 인스턴스의 경우 레거시 태그 형식을 유지할 수 있는 옵션이 있습니다(아래 참조). 이러한 형식을 유지하려면 EC2 호스트의 New Relic 구성 UI로 이동하여 **Keep legacy metadata format** 을 선택합니다. 이를 통해 현재 형식과 레거시 형식이 모두 있는 태그에 액세스할 수 있습니다. 레거시 형식을 비활성화하면 현재 형식만 사용할 수 있습니다. 레거시 형식을 비활성화하는 경우 해당 형식을 사용하는 대시보드 또는 경고 조건이 있는지 확인하는 것이 좋습니다.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        [**시스템** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage)
      </td>
    ...
    <tr>
      <td>
        [**인벤토리** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)
      </td>

      <td>
        호스트별 세부 구성 데이터, 특정 EC2 인스턴스 등을 검토합니다.
    ...
    <tr>
      <td>
        [**이벤트** 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change)
      </td>

      <td>
        환경의 변경 사항에 대한 라이브 피드에서 EC2 이벤트를 검색하고 봅니다.
      </td>
    </tr>
