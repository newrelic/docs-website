...
AWS Systems Manager 콘솔을 사용하여 **New Relic Infrastructure Monitoring Agent** 배포자 패키지를 한 번 설치 또는 업데이트하거나 설치 일정을 설정할 수 있습니다.
...
1. AWS 시스템 관리자 [콘솔을](https://console.aws.amazon.com/systems-manager)엽니다. 탐색 창에서 **Distributor를**선택하고 **Third party** 선택한 후 **New Relic Infrastructure Monitoring Agent** 타일을 선택합니다.
2. 설치 모드를 선택합니다. **New Relic Infrastructure Monitoring Agent** 패키지를 한 번에 설치 또는 업데이트하거나 설치를 예약할 수 있습니다. **Distributor 패키지**설치에 대한 자세한 내용은 [AWS Systems Manager Distributor 설명서를](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor-working-with-packages-deploy.html)참조하십시오.
