...
AWS Systems Manager 콘솔을 사용하여 <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> 배포자 패키지를 한 번 설치 또는 업데이트하거나 설치 일정을 설정할 수 있습니다.
...
1. AWS 시스템 관리자 [콘솔을](https://console.aws.amazon.com/systems-manager) 엽니다. 탐색 창에서 <DoNotTranslate>**Distributor**</DoNotTranslate> 선택하고, <DoNotTranslate>**Third party**</DoNotTranslate> 선택하고, <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> 타일을 선택합니다.

2. 설치 모드를 선택하세요. <DoNotTranslate>**New Relic infrastructure agent**</DoNotTranslate> 패키지를 한 번 설치 또는 업데이트하거나 설치를 예약할 수 있습니다. <DoNotTranslate>**Distributor packages**</DoNotTranslate> 설치에 대한 자세한 내용은 [AWS 시스템 관리자 배포자 설명서를](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor-working-with-packages-deploy.html) 참조하십시오.
...
6. <DoNotTranslate>**Targets**</DoNotTranslate> 섹션에서 태그를 지정하거나 인스턴스 또는 장치를 수동으로 선택하거나 리소스 그룹을 지정하여 이 작업을 실행할 관리형 노드를 선택합니다. 목록에 관리형 노드가 표시되지 않으면 [다음 AWS 문서를](https://docs.aws.amazon.com/systems-manager/latest/userguide/troubleshooting-managed-instances.html) 참조하십시오.

7. <DoNotTranslate>**Concurrency**</DoNotTranslate> 에 대한 <DoNotTranslate>**Rate Control**</DoNotTranslate> 의 경우 동시에 명령을 실행하기 위한 목표, 숫자 또는 백분율을 지정합니다.

   <Callout variant="tip">
     태그 또는 리소스 그룹을 지정하여 대상을 선택했고 얼마나 많은 관리 노드가 대상인지 확실하지 않은 경우 백분율을 지정하여 동시에 문서를 실행할 수 있는 대상 수를 제한하십시오.
   </Callout>

8. <DoNotTranslate>**Error threshold**</DoNotTranslate> 의 경우, 관리 노드의 수 또는 백분율에서 실패한 후 다른 쿼리, 목표에서 명령 실행을 중지할 시기를 지정합니다. 예를 들어 세 개의 오류를 지정한 경우 네 번째 오류가 수신되면 Systems Manager는 명령 전송을 중지합니다. 명령을 계속 처리 중인 관리 노드에서도 오류가 전송될 수 있습니다.

9. <DoNotTranslate>**Verify the installation**</DoNotTranslate>. 설치를 실행한 후 명령 상태 영역에서 진행 상황을 확인하세요. 성공 상태가 표시되면 설치가 성공한 것입니다.
