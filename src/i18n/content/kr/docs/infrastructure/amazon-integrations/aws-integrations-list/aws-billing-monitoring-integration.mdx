...
5. 새 사용자 정의 정책 이름을 입력하십시오. 예: <var>NewRelicBudget</var> .
...
인프라에서 통합 데이터를 찾으려면 [**one.newrelic.com**](http://one.newrelic.com) **> 인프라 > AWS** 로 이동하여 AWS Billing 통합 링크 중 하나를 선택하십시오.
  ...
  >
    ...
    * 예상 비용
      ...
      **NRQL 예** : 실제 지출, 예측, 한도 및 사용량 대 금전적 비용이 있는 다양한 예산 테이블의 경우 다음 NRQL 쿼리를 실행합니다.
      ...
      <Callout variant="tip">
        `providerAccountId` 은 New Relic의 클라우드 서비스 계정 ID입니다. **제공자 계정** 옆에 있는 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS** 에서 찾을 수 있습니다.
      </Callout>
  </Collapser>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        * `name`: 예산의 이름입니다. 계정 내에서 고유합니다.
        * `startTime`: 예산이 적용되는 기간의 시작 날짜입니다.
        * `endTime`: 예산이 적용되는 기간의 종료 날짜입니다.
        * `limitAmount`: 예산을 상한선으로 하여 추적하려는 총 비용 또는 사용량입니다.
        * `limitUnit`: 예산 예측, 실제 지출 또는 예산 임계값(예: 달러 또는 GB)에 사용되는 측정 단위입니다.
        * `timeUnit`: 예산이 실제 지출과 예상 지출을 재설정할 때까지의 시간입니다. 유효한 값: 월별, 분기별, 연간
        * `type`: 이 예산이 금전적 비용 또는 사용량을 추적하는지 여부.
      </td>
    </tr>
