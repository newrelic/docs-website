...
* **AWS console \[AWS 콘솔] > Billing and cost management \[청구 및 비용 관리] > Billing preferences \[청구 기본 설정]** 에서 **Receive Billing Alerts** \[청구 알림 수신] 확인란을 활성화합니다. 이는 Amazon이 CloudWatch를 통해 사용 요금을 보고하는 데 필요합니다. 실제로 AWS로부터 알림을 받을 필요는 없습니다.
...
인프라 UI에서 통합 데이터를 찾으려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** 로 이동하고 AWS Billing 통합 링크 중 하나를 선택하십시오.
  ...
  >
    ...
    * 예상 비용
      ...
      **NRQL 예** : 실제 지출, 예측, 한도 및 사용량 대 금전적 비용이 있는 다양한 예산 테이블의 경우 다음 NRQL 쿼리를 실행합니다.
      ...
      <Callout variant="tip">
        `providerAccountId` 은 New Relic의 클라우드 서비스 계정 ID입니다. **Provider account** \[공급자 계정 옆에] 있는 **[one.newrelic.com > 모든 기능](https://one.newrelic.com/all-capabilities) > 인프라 > AWS** 에서 찾을 수 있습니다.
      </Callout>
  </Collapser>
