...
* <DoNotTranslate>**AWS console > Billing and cost management > Billing preferences**</DoNotTranslate>
  ...
  확인란을 활성화합니다. 이는 Amazon이 CloudWatch를 통해 사용 요금을 보고하는 데 필요합니다. 실제로 AWS로부터 알림을 받을 필요는 없습니다.

<Callout variant="important">
  청구(비용) 대시보드에 액세스하려면 <DoNotTranslate>**Receive Billing Alerts**</DoNotTranslate> 확인란을 선택하기만 하면 됩니다.
</Callout>
...
1. AWS 관리 콘솔에서 <DoNotTranslate>**Services > IAM > Roles**</DoNotTranslate> 선택합니다.

2. 뉴렐릭에 대해 구성한 역할을 선택한 다음 <DoNotTranslate>**Permissions**</DoNotTranslate> 선택합니다.

3. 선택한 역할의 <DoNotTranslate>**Inline policy**</DoNotTranslate> 탭에서 확장을 선택하세요. <Icon name="fe-chevron-down"/> 아이콘이 이미 있는 경우 아이콘을 선택하거나 역할에 새 인라인 정책을 추가하는 옵션을 선택합니다.

4. 사용자 지정 정책을 추가하는 옵션을 선택합니다.

5. 새 사용자 정의 정책 이름을 입력하십시오. 예: NewRelicBudget.

6. <DoNotTranslate>**Policy Document**</DoNotTranslate> 에 다음 권한 설명을 입력합니다.
...
인프라 UI에서 통합 데이터를 찾으려면 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate> 으로 이동하여 AWS 결제 통합 링크 중 하나를 선택하세요.
...
<CollapserGroup>
  ...
  >
    ...
    * 예상 비용
      ...
      <DoNotTranslate>**NRQL example**</DoNotTranslate>: 실제 지출, 예측, 한도, 사용량 및 금전적 비용이 포함된 다양한 예산 테이블을 보려면 다음 NRQL 쿼리를 실행하세요.
      ...
      <Callout variant="tip">
        `providerAccountId` 은 뉴렐릭의 클라우드 서비스 계정 ID입니다. <DoNotTranslate>**Provider account**</DoNotTranslate> 옆의 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate> 에서 찾을 수 있습니다.
      </Callout>
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: 서비스 이름별로 공급자를 그룹화하려면 다음 NRQL 쿼리를 실행하세요.
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: 계정 이름별로 공급자를 그룹화하려면 다음 NRQL 쿼리를 실행하세요.
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: 특정 AWS 계정 ID에 대한 AWS 서비스의 비용 분할을 이해하려면 다음 NRQL 쿼리를 실행하십시오.
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: AWS EC2 서비스에 대한 AWS 계정의 비용 분할을 이해하려면 다음 NRQL 쿼리를 실행하십시오.
