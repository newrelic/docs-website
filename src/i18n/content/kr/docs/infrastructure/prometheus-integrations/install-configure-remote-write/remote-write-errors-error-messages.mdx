...
<CollapserGroup>
  <Collapser
    ...
    title="구성 오류"
  >
    구성 파일의 원격 쓰기 URL에 누락되거나 잘못된 문자(예: 엔드포인트, 라이선스 키 또는 `prometheus_server` 이름) 또는 파일에 정보가 잘못 배치되면 Prometheus 서버가 시작되지 않고 원격 쓰기가 작동하지 않습니다. 또는 Prometheus 서버 로그에 오류가 표시됩니다.
  </Collapser>
