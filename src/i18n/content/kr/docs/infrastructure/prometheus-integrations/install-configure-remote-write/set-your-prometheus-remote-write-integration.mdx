...
[Prometheus Operator](https://github.com/prometheus-operator/prometheus-operator) 를 사용하는 경우 데이터를 보고하려는 계정의 New Relic <LicenseKey/> 비밀을 생성해야 합니다. API 키가 `Ingest - License` 유형인지 확인하십시오.
...
<CollapserGroup>
  <Collapser
    ...
    title="X-라이센스 키"
  >
    귀하의 <LicenseKey/> 는 API 키가 아닙니다. 라이센스 키는 인증 및 데이터를 쓸 계정을 식별하는 데 사용됩니다. 다른 New Relic 계정에 쓰기 위해 Prometheus를 구성하는 경우 각 원격 쓰기 URL에 다른 키를 사용하십시오.
  </Collapser>
