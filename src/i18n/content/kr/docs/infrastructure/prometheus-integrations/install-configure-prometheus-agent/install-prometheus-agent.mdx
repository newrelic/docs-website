...
1. Kubernetes 통합을 설치합니다. [여기에서](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure) 설치 방법을 확인하세요.

<Callout variant="tip">
  또는 [Helm을 사용하여 통합을 배포하기](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm) 위한 수동 지침도 제공합니다.

  `newrelic-prometheus-agent.enabled=true` 옵션을 설정하여 Prometheus 에이전트를 사용 설정해야 합니다.
</Callout>

2. 통합이 올바르게 구성되었는지 확인하려면 **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Query your data** 이동하고 이 NRQL 쿼리를 실행하여 데이터가 보고되었는지 확인하십시오.
...
1. New Relic 계정에 [Prometheus 에이전트용 대시보드를](https://newrelic.com/instant-observability/prometheus-agent) 설치합니다.
