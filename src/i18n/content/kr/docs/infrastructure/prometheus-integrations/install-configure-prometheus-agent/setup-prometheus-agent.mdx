...
<Callout variant="tip">
  `values.yaml` 파일에 뉴렐릭 <InlinePopover type="licenseKey" />추가해야 합니다.
</Callout>

<CollapserGroup>
  <Collapser id="nri-bundle" title="Helm 번들을 사용하여 설치됨">
    ```yaml
    global:
  ...
  <Collapser id="standalone-helm-release" title="독립 실행형 Helm 릴리스">
...
Prometheus 에이전트는 Kubernetes 의 가장 인기 있는 통합에서 지표를 스크랩하도록 구성할 수 있습니다. 즉시 모니터링을 시작하려면 대시보드 세트와 <InlinePopover type="alerts" />가 포함된 [통합 목록을](/docs/infrastructure/prometheus-integrations/integrations-list/integrations-list-intro) 살펴보세요.

해당 목록은 뉴렐릭의 Prometheus 에이전트 helm 차트의 [value.yaml](https://github.com/newrelic/newrelic-prometheus-configurator/blob/main/charts/newrelic-prometheus-agent/values.yaml) 에 정의되어 있습니다. 이 목록을 수정할 수 있지만 일부 <InlinePopover type="dashboards" />(는) 맞춤 라벨이나 값을 사용하면 기본적으로 작동하지 않을 수 있습니다.
...
* 안내 설치를 사용하셨다면 <DNT>**Scrape all Prometheus endpoints**</DNT> 옵션을 선택하세요.
...
`target_discovery.filter` 구성 매개변수를 사용하여 Prometheus가 스크랩하는 대상을 필터링합니다. `label` 및 `annotations` 레이블을 사용하여 현재 조건으로 필터링하고 모든 조건에 대해 `AND` 연산자를 사용하십시오.
...
`extra_metric_relabel_config` 매개변수를 사용하여 [metric\_relabel\_config](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#relabel_config) 매개변수의 항목을 추가하는 필터를 적용할 수 있습니다. 이 매개변수는 `static_targets.jobs` , `kubernetes.jobs` 및 `newrelic_remote_write` 에 대한 `extra_write_relabel_configs` 매개변수에 있습니다.
...
<CollapserGroup>
  <Collapser id="collapser-source" title="이름이 'prefix_'로 시작하는 메트릭을 필터링합니다.">
    ```yaml
    - source_labels: [__name__]
      regex: 'prefix_.+'
      action: drop
  ...
  <Collapser id="collapser-source" title="'k8s.io/app=appLabelValue'가 있는 메트릭을 필터링합니다. 쿠버네티스 레이블">
    ```yaml
  ...
  <Collapser id="collapser-source" title="이름이 'prefix_'로 시작하고 'k8s.io/app=appLabelValue'가 있는 메트릭을 필터링합니다. 쿠버네티스 라벨">
    ```yaml
  ...
  <Collapser id="collapser-source" title="이름이 'prefix_'로 시작하는 메트릭을 제외한 모든 메트릭을 필터링합니다.">
    ```yaml
    - source_labels: [__name__]
      regex: 'prefix_.+'
      action: keep
...
<CollapserGroup>
  <Collapser id="collapser-source" title="'prefix_'로 시작하는 측정항목 이름에 'new_label=newLabelValue' 라벨을 추가합니다.">
    ```yaml
    - source_labels: [__name__]
      regex: 'prefix_.+'
  ...
  <Collapser id="collapser-source" title="라벨이 'label_name'인 모든 측정항목을 필터링합니다.">
