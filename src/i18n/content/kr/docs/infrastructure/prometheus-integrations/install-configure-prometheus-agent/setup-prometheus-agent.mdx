...
Prometheus 에이전트는 Kubernetes에서 가장 널리 사용되는 통합에서 측정항목을 스크랩하도록 구성할 수 있습니다. 대시보드 세트가 포함된 [통합 목록을](/docs/infrastructure/prometheus-integrations/integrations-list/integrations-list-intro) 살펴보세요. <InlinePopover type="alerts"/>즉시 모니터링을 시작합니다.
...
* 안내식 설치를 사용한 경우 **Scrape only metrics from Prometheus integrations** \[Prometheus 통합에서 측정항목만 스크랩 옵션을] 선택 취소하세요.<img title="Kubernetes Guided Install" alt="Kubernetes Guided Install" src={infrastructurePrometheusScrapeOnlyMetrics}/>
