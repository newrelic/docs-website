...
<Callout variant="tip">
  `values.yaml` 파일에 New Relic 라이센스 키를 추가해야 합니다. 라이센스 키와 이를 찾는 방법에 대해 자세히 알아보려면 [기본 API 키](/docs/apis/intro-apis/new-relic-api-keys/#overview-keys) 에 대해 읽어보십시오.
</Callout>
...
* 가이드 설치를 사용한 경우 **Scrape only metrics from Prometheus integrations **(Prometheus 통합에서 스크랩 전용 메트릭) 옵션을 선택 취소합니다.<img title="Kubernetes Guided Install" alt="Kubernetes Guided Install" src={infrastructurePrometheusScrapeOnlyMetrics}/>
