...
[Traefik](https://doc.traefik.io/traefik/) 은 서비스 게시를 재미있고 쉬운 경험으로 만들어주는 오픈 소스 에지 라우터입니다. 시스템을 대신하여 요청을 수신하고 이를 처리할 책임이 있는 구성 요소를 찾습니다.
...
1. Prometheus용 Kubernetes에서 Traefik 프록시를 설정하여 메트릭 엔드포인트를 검색하려면 [Traefik 설명서를](https://doc.traefik.io/traefik/getting-started/quick-start-with-kubernetes/) 따르십시오.
...
2. Prometheus 모니터링을 설정합니다. Prometheus 지표는 New Relic과 통합되어야 합니다. Prometheus 에이전트 또는 원격 쓰기 통합을 사용할 수 있습니다. [Prometheus 지표를 보내는 방법을](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/) 참조하십시오.
   ...
   <Callout variant="important">
     ...
     이 경우 `traefik` 문자열을 포함하는 `app.kubernetes.io/name` , `app.newrelic.io/name` , `k8s-app` 레이블 중 하나로 포드 또는 엔드포인트를 식별해야 합니다.
   </Callout>
...
4. 기본 제공 대시보드 및 [경고](/docs/alerts-applied-intelligence/new-relic-alerts/learn-alerts/introduction-alerts/) 에 액세스하려면 [Traefik 빠른 시작을](https://newrelic.com/instant-observability/traefik) 설치하십시오.
   ...
   <Callout variant="important">
     대시보드의 일부 차트에는 `traefik` 문자열을 포함하는 `app.kubernetes.io/name` , `app.newrelic.io/name` , `k8s-app` 레이블 중 하나를 사용하여 팟(Pod) 또는 엔드포인트를 식별해야 하는 조건이 있는 쿼리가 포함되어 있습니다.
   </Callout>
...
이 통합에 의해 노출된 다양한 메트릭 세트는 [Traefik 설명서](https://doc.traefik.io/traefik/observability/metrics/overview/) 에 정의되어 있습니다.
...
이 통합을 통해 골든 메트릭, 엔티티 대시보드, 워크로드 및 감시와 같은 [엔티티 기능](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) 의 전체 세트를 지원하는 `Traefik` 엔티티를 사용할 수 있습니다.
