...
<CollapserGroup>
  ...
  >
    ...
    **3. 메트릭을 집계하고 차트로 표시합니다.**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) > Query your data** : 이 예는 FROM Metric SELECT keyset() WHERE metricName = 'container_memory_usage_bytes'를 실행한 후 표시되는 데이터를 보여줍니다.
    </figcaption>
    ...
    **4. ID별로 메트릭을 봅니다.**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) > Query your data** : 이 예는 FROM Metric SELECT latest(container_memory_usage_bytes) FACET id를 실행한 후 표시되는 데이터를 보여줍니다.
    </figcaption>
    ...
    **5. 선택한 ID를 쿼리에 추가합니다.**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) > Query your data** : 이 예는 `From Metric select latest(container_memory_usage_bytes) where id = "/" timeseries` 실행 후 표시되는 데이터를 보여줍니다.
    </figcaption>
  </Collapser>
</CollapserGroup>
