...
<CollapserGroup>
  ...
  >
    ...
    **Docker: **
    ...
    이 예에서는 Redis 내보내기를 스크래핑한다고 가정합니다. 클러스터의 엔드포인트당 연결된 Redis 클라이언트 수를 보려면 다음을 수행합니다.
    ...
    **쿠버네티스: **
    ...
    이 예에서는 Redis 내보내기가 설치된 Redis 파드가 있다고 가정합니다. 기본 네임스페이스에서 파드당 연결된 Redis 클라이언트 수를 보려면 다음을 수행합니다.
  ...
  <Collapser
    id="avg-free-memory"
    title={<><img src={osDocker} title="Docker icon" alt="Docker icon" style={{ height: '35px', width: '40px' }}/>Docker: 스크래핑된 엔드포인트에 대한 평균 여유 메모리 보기</>}
  >
  ...
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8.png" alt="img-integration-k8.png" style={{ height: '35px', width: '40px' }}/>쿠버네티스: 배포에서 파드의 평균 메모리 사용량 보기</>}
  >
...
Prometheus 통합 데이터에 대한 NRQL 쿼리 결과를 보려면 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**로 이동합니다. 보다 자세한 내용은 [쿼리 빌더 문서](/docs/chart-builder/use-chart-builder/get-started/introduction-chart-builder)를 참조하십시오.
