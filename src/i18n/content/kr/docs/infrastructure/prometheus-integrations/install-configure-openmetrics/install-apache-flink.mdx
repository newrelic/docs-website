---
title: New Relic에서 Apache Flink 대시보드 가져오기
tags:
  - Integrations
...
* [설치 안내](https://one.newrelic.com/nr1-core?account=3501165&state=a33ce5fc-d32b-e0b8-3941-30f13e4bc00b) 에 따라 인프라 에이전트로 시스템을 계측하십시오. 또는 원하는 경우 명령줄을 통해 [인프라 에이전트를 설치할](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-infrastructure-monitoring-agent-linux) 수 있습니다.
...
Prometheus OpenMetrics를 설치한 후 `nri-prometheus-config.yml` 파일을 구성해야 합니다. 구성 파일은 [nri-prometheus](https://github.com/newrelic/nri-prometheus/blob/main/configs/nri-prometheus-config.yml.sample) 저장소의 스니펫과 일치해야 합니다.
...
## Apache Flink 메트릭 및 로그를 대시보드로 가져오기

Prometheus OpenMetrics로 New Relic을 설치, 통합 및 구성한 후 ApacheFlink 빠른 시작을 설치할 수 있습니다. New Relic에서 대시보드를 찾으려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Dashboards** 로 이동한 다음 **Apache Flink** 를 선택합니다.
