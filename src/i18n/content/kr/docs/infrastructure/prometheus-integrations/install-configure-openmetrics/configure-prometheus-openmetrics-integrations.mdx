...
<DNT>**Recommendation:**</DNT> `LICENSE_KEY` 이라는 환경 변수로 뉴렐릭 클러스터 키를 구성하세요. 뉴렐릭이 [상호 TLS 인증 비밀](/docs/integrations/prometheus-integrations/install-configure/add-mutual-tls-prometheus-endpoints) 에서 환경 변수를 로드할 수 있으므로 이는 보다 안전한 환경을 제공합니다.
...
설정을 다시 로드하려면 통합을 다시 시작하세요. <DNT>**Recommendation:**</DNT> 배포를 복제본 0개까지 축소한 후 다시 복제본 1개로 축소합니다.
