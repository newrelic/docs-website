...
달리 명시하지 않는 한, 뉴렐릭과 Prometheus OpenMetrics 통합에 대한 설정 옵션은 docker 및 Kubernetes 환경 모두에 적용됩니다. 최소한 다음 설정 값은 <DoNotTranslate>**required**</DoNotTranslate> 입니다.

* <InlinePopover type="licenseKey"/>
* [클러스터 이름](#definitions-configuration-file)

<DoNotTranslate>**Recommendation:**</DoNotTranslate> `LICENSE_KEY` 이라는 환경 변수로 뉴렐릭 클러스터 키를 구성하세요. 뉴렐릭이 [상호 TLS 인증 비밀](/docs/integrations/prometheus-integrations/install-configure/add-mutual-tls-prometheus-endpoints) 에서 환경 변수를 로드할 수 있으므로 이는 보다 안전한 환경을 제공합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}/>Kubernetes 포트 및 엔드포인트 경로</>}
  >
  ...
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}/>예: Kubernetes 포트 및 경로에 대한 레이블</>}
  >
...
Prometheus OpenMetrics 통합 <DoNotTranslate>**does not**</DoNotTranslate> 설정 파일을 변경할 때 설정을 자동으로 다시 로드합니다.
...
설정을 다시 로드하려면 통합을 다시 시작하세요. <DoNotTranslate>**Recommendation:**</DoNotTranslate> 배포를 복제본 0개까지 축소한 후 다시 복제본 1개로 축소합니다.
