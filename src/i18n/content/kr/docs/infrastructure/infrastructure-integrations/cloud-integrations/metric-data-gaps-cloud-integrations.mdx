...
다음은 측정항목 데이터의 간격을 표시할 수 있는 측정항목 목록입니다. 가능하면 이러한 메트릭에 대한 경고를 설정하지 마십시오. 잘못된 긍정을 생성할 수 있다는 것을 알고 있기 때문입니다.
...
일부 메트릭은 일반적인 클라우드 공급자 API(CloudWatch, Stackdriver, Azure Monitor)에 없으며 대신 서비스 API에서 가져옵니다. 각 클라우드 서비스 공급자는 데이터를 처리하고 서비스와 상호 작용하는 고유한 서비스 API를 가지고 있습니다. 예를 들어 AWS CloudWatch에 지표가 없는 경우 New Relic은 AWS ECS 서비스 API에서 지표를 가져옵니다.