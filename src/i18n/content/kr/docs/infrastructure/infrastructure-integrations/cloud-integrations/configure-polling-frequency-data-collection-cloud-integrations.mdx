...
새로운 Relic 클라우드 통합은 클라우드 제공업체의 API에서 데이터를 가져옵니다. 데이터는 일반적으로 AWS CloudWatch, Azure Monitor, GCP Stackdriver와 같은 모니터링 API에서 수집되고 인벤토리 메타데이터는 특정 서비스의 API에서 수집됩니다.
