...
<CollapserGroup>
  <Collapser id="service" title="서비스 계정(권장)">
    ...
    조직에서 [도메인 제한 제약 조건](https://cloud.google.com/resource-manager/docs/organization-policy/restricting-domains)을 사용하는 경우, 조직에서 적용하는 제약 조건에 따라 뉴렐릭을 허용하도록 정책을 업데이트해야 합니다:

    * **`iam.allowedPolicyMemberDomains`**: 뉴렐릭 도메인 ID `C02x1gp26`을(를) 허용합니다.
    * **`iam.managed.allowedPolicyMembers`**: 뉴렐릭 GCP 조직 ID `740422149647` 또는 GCP 프로젝트 번호 `397727728820`를 허용합니다.
