...
[New Relic](/docs/infrastructure/integrations-getting-started/getting-started/introduction-infrastructure-integrations) 과 [Google Kubernetes Engine](https://cloud.google.com/kubernetes-engine/) (GKE)의 통합은 Google 인프라에서 호스팅되는 클러스터에 대한 데이터를 보고합니다. [New Relic의 Infrastructure UI](/docs/infrastructure) 에서 GKE 인스턴스 데이터를 모니터링하고 경고할 수 있으며 [사용자 지정 쿼리 및 차트 대시보드](/docs/insights) 를 만들 수 있습니다.

## 통합 활성화 [#activate]

통합을 활성화하려면:

1. 인프라 계정에서 GCP 통합을 활성화하기 전에 [인프라 에이전트를 설치](/docs/infrastructure/new-relic-infrastructure/installation) 했는지 확인하세요.
...
[인벤토리 데이터](/docs/integrations/new-relic-integrations/getting-started/understand-use-data-infrastructure-integrations#overview) 는 서비스 또는 호스트의 상태 또는 구성에 대한 정보입니다. UI에서 인벤토리 데이터를 검토할 수 있습니다.

Google Kubernetes Engine 통합은 다음 속성을 사용하여 클러스터 개체에 대한 구성 정보를 보고합니다. 또한 컨테이너 레이블은 노드 및 포드로 전파됩니다.

### gcp/kubernetes/컨테이너 [#inventory-virtual-machine]
...
### gcp/kubernetes/노드 [#inventory-node]
...
### gcp/kubernetes/포드 [#inventory-pod]
