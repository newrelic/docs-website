...
통합을 활성화하고 몇 분 정도 기다리면( [폴링 빈도](#polling)기준) New Relic UI에 데이터가 나타납니다. 대시보에 대한 링크를 포함하여 [데이터를 찾고 사용하기](/docs/infrastructure/integrations/find-use-infrastructure-integration-data)위해<InlinePopover type="dashboards"/>ards 및 알림 설정은 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > GCP > (통합 선택)**으로 이동합니다.
