...
에이전트는 [컴퓨팅 메트릭](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage) , [이벤트](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 및 [인벤토리](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 구성을 설명하는 JSON 형식 데이터를 New Relic의 인프라 엔드포인트로 정기적으로 보냅니다. 이러한 커뮤니케이션은 귀하의 계정에 대해 생성된 <LicenseKey/> 를 사용하여 에이전트와 연결됩니다. New Relic은 외부 에이전트로부터 데이터를 수신하면 인프라 모니터링 UI에 새로운 메트릭, 이벤트 또는 구성 데이터를 표시합니다.
