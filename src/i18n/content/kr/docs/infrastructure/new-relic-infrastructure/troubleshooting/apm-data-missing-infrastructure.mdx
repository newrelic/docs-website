...
APM과 인프라 에이전트가 동일한 호스트에 설치되어 동일한 New Relic을 사용하는 경우 <InlinePopover type="licenseKey"/>또는 동일한 조직에 있는 계정의 라이선스 키를 사용하면 APM 데이터가 **infrastructure** \[인프라] 에 나타나야 하며 그 반대의 경우도 마찬가지입니다. 이 APM 인프라 연결이 보이지 않으면 다음 문제 해결 팁을 따르십시오.
...
<CollapserGroup>
  ...
  <Collapser
    id="hostnames"
    title="APM 및 인프라 UI에서 호스트 이름이 동일한지 확인하십시오."
  >
  ...
  <Collapser
    id="gap-fqdn-issue"
    title="Kubernetes 통합 단계가 완료되었는지 확인하십시오."
  >
