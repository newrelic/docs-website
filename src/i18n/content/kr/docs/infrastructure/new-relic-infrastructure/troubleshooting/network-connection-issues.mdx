...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. <DoNotTranslate>**Resolve the domain:**</DoNotTranslate> `nslookup` 또는 `dig` 사용하여 Linux 시스템이 도메인을 IP 주소로 확인할 수 있는지 확인합니다.
    ...
    2. <DoNotTranslate>**Ping the server:**</DoNotTranslate> 위에서 얻은 IP 주소를 ping하여 서버에 연결할 수 있는지 확인하세요.
    ...
    3. <DoNotTranslate>**Traceroute:**</DoNotTranslate> 시스템과 서버 사이의 네트워크 경로를 확인하려면 `traceroute` 사용하세요.
    ...
    4. <DoNotTranslate>**Curl:**</DoNotTranslate> 엔드포인트에 연결할 수 있는지 확인하려면 `curl` 사용하세요.
