...
### 에이전트 플러그인 보고 줄이기 [#reduce\_plugins][#reduce_plugins]
...
<CollapserGroup>
  <Collapser id="enable_disable" title="플러그인 활성화 및 비활성화 방법">
  ...
  <Collapser
    id="selinux"
    title={<>SELinux 비활성화 <InlineCode>
      semodule -l
    </InlineCode> (Linux만 해당)</>
    }
  >
  ...
  <Collapser id="sysctl" title="Sysctl 축소 또는 비활성화(Linux만 해당)">
  ...
  <Collapser id="additional_plugins" title="줄이거나 비활성화할 추가 플러그인">
...
### 호스트 내 통합 검토 [#on\_host\_integrations][#on_host_integrations]
