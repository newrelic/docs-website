...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        `SystemSample` CPU, 메모리, 디스크, 네트워크 등 전체 서버의 현재 전반적인 상태를 설명하는 데이터가 포함되어 있습니다. 5초마다 이 데이터의 스냅샷을 찍어 `SystemSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DoNotTranslate>**Systems**</DoNotTranslate> UI 탭](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#system) 에 나타납니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `ProcessSample` 단일 시스템에서 실행되는 프로그램에서 자세한 리소스 사용 정보를 수집합니다. 모든 활성 프로세스에 대해 20초마다 이 데이터의 스냅샷을 찍어 `ProcessSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DoNotTranslate>**Processes**</DoNotTranslate> UI 페이지](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#processes) 에 나타납니다.
    ...
    <tr>
      ...
      <td>
        `StorageSample` 서버와 연결된 단일 저장 장치를 나타냅니다. 각 샘플은 장치, 장치가 사용하는 파일 시스템 유형, 현재 사용량 및 용량에 대한 기술 정보를 수집합니다. 마운트된 각 파일 시스템에 대해 20초마다 이 데이터의 스냅샷을 찍어 `StorageSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DoNotTranslate>**Storage**</DoNotTranslate> UI 페이지](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#storage) 에 나타납니다.
    ...
    <tr>
      ...
      <td>
        `NetworkSample` 서버와 연결된 각 네트워크 장치에 대한 기술 및 상태 정보를 캡처합니다. 여기에는 장치의 인터페이스와 주소 정보는 물론 현재 사용 데이터도 포함됩니다. 연결된 각 네트워크 인터페이스에 대해 10초마다 이 데이터의 스냅샷을 찍어 `NetworkSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DoNotTranslate>**Network**</DoNotTranslate> UI 페이지](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#network) 에 나타납니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `ContainerSample` 각 docker 컨테이너에 대한 기술 및 상태 정보를 수집합니다. 여기에는 컨테이너의 ID, 이름, 이미지, 이미지 이름은 물론 CPU, 메모리 및 네트워킹에 대한 지표가 포함됩니다. 각 컨테이너에 대해 15초마다 이 데이터의 스냅샷을 찍어 `ContainerSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DoNotTranslate>**Containers**</DoNotTranslate> UI 페이지](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#containers) 에 나타납니다. 자세한 내용은 [Docker 모니터링 을](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/docker-instrumentation-infrastructure) 참조하세요.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `InfrastructureEvent` 시스템의 라이브 상태에서 발생하는 변경 사항(델타)을 설명합니다. 인벤토리 또는 시스템 상태가 추가, 제거 또는 변경되면 해당 활동을 기록하는 `InfrastructureEvent` 가 생성됩니다. 이 데이터는 [<DoNotTranslate>**Events**</DoNotTranslate> UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 에 나타납니다.
      </td>
    </tr>
  </tbody>
</table>
