...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `StorageSample` 서버와 연결된 단일 저장 장치를 나타냅니다. 각 샘플은 장치, 장치가 사용하는 파일 시스템 유형, 현재 사용량 및 용량에 대한 기술 정보를 수집합니다. 마운트된 각 파일 시스템에 대해 20초마다 이 데이터의 스냅샷을 찍어 `StorageSample` 이벤트로 패키징한 다음 뉴렐릭으로 전송합니다. 이 데이터는 [<DNT>**Storage**</DNT> UI 페이지](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#storage) 에 나타납니다.
