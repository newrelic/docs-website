...
* 더 나은 [호스트 필터 세트를](/docs/infrastructure/new-relic-infrastructure/filter-group/filter-sets-organize-your-infrastructure-hosts) 만듭니다.
...
인프라 모니터링 에이전트를 사용해 보고 싶으십니까? 무료 [로 New Relic 계정을 만드](https://newrelic.com/signup) 십시오! 신용 카드가 필요하지 않습니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        `SystemSample` CPU, 메모리, 디스크 및 네트워크를 포함하여 전체 서버의 현재 전체 상태를 설명하는 데이터를 포함합니다. 5초마다 이 데이터의 스냅샷을 만들어 `SystemSample` 이벤트로 패키징한 다음 New Relic으로 전송합니다. 이 데이터는 [**호스트** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-hosts-page-measure-resource-usage) 에 나타납니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `ProcessSample` 단일 시스템에서 실행되는 프로그램에서 자세한 리소스 사용 정보를 수집합니다. 모든 활성 프로세스에 대해 20초마다 이 데이터의 스냅샷을 만들고 이를 `ProcessSample` 이벤트로 패키징한 다음 New Relic으로 전송합니다. 이 데이터는 [**프로세스** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-processes-page-inspect-process-performance) 에 나타납니다.

        <Callout variant="important">
          프로세스 메트릭은 2020년 7월 20일 이후에 생성된 계정에 대해 기본적으로 New Relic으로 전송되지 않습니다. 이 데이터를 인프라 모니터링 UI로 가져오려면 [프로세스 메트릭을 활성화하십시오](/docs/infrastructure/install-configure-manage-infrastructure/configuration/infrastructure-configuration-settings#enable-process-metrics) .
        </Callout>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `StorageSample` 서버와 연결된 단일 저장 장치를 나타냅니다. 각 샘플은 장치, 사용하는 파일 시스템 유형, 현재 사용량 및 용량에 대한 설명 정보를 수집합니다. 마운트된 각 파일 시스템에 대해 20초마다 이 데이터의 스냅샷을 만들고 이를 `StorageSample` 이벤트로 패키징한 다음 New Relic으로 전송합니다. 이 데이터는 [**스토리지** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-storage-page-evaluate-disk-usage-efficiency) 에 나타납니다.
    ...
    <tr>
      ...
      <td>
        `NetworkSample` 서버와 연결된 각 네트워크 장치에 대한 설명 및 상태 정보를 캡처합니다. 여기에는 장치의 인터페이스 및 주소 정보와 현재 사용 데이터가 포함됩니다. 연결된 각 네트워크 인터페이스에 대해 10초마다 이 데이터의 스냅샷을 만들고 이를 `NetworkSample` 이벤트로 패키징한 다음 New Relic으로 전송합니다. 이 데이터는 [**네트워크** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-network-page-measure-compare-capacity) 에 나타납니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `ContainerSample` 각 Docker 컨테이너에 대한 설명 및 상태 정보를 수집합니다. 여기에는 컨테이너의 ID, 이름, 이미지, 이미지 이름은 물론 CPU, 메모리 및 네트워킹에 대한 메트릭이 포함됩니다. 각 컨테이너에 대해 15초마다 이 데이터의 스냅샷을 만들고 이를 `ContainerSample` 이벤트로 패키징한 다음 New Relic으로 전송합니다. 이 데이터는 [**컨테이너** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-hosts-page-measure-resource-usage) 에 나타납니다. 자세한 내용은 [Docker 모니터링](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/docker-instrumentation-infrastructure) 을 참조하십시오.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        `InfrastructureEvent` 시스템의 라이브 상태에서 발생하는 변경 사항(델타)을 설명합니다. 인벤토리 또는 시스템 상태가 추가, 제거 또는 변경되면 New Relic은 해당 활동을 기록하는 `InfrastructureEvent` 을 생성합니다. 이 데이터는 [**이벤트** UI 페이지](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change) 에 나타납니다.
      </td>
    </tr>
  </tbody>
</table>
...
[Amazon Elastic Compute Cloud(EC2) 계정을 인프라 모니터링에 연결](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/amazon-aws-ec2-integration-infrastructure) 하면 [Amazon EC2 인스턴스](/docs/infrastructure/infrastructure-integrations/amazon-integrations/aws-ec2-integration) 의 데이터가 보고됩니다. Amazon EC2 관련 속성은 모든 이벤트에서 사용할 수 있는 공통 속성입니다. 이러한 속성은 EC2 API에서 가져옵니다. CloudWatch 정보는 수집되지 않습니다.

이러한 속성과 해당 값은 Amazon이 노출 데이터를 변경하는 경우 변경될 수 있습니다.
...
<CollapserGroup>
  ...
  >
    Amazon Web Services가 New Relic에 제공하는 메타데이터를 변경하면 수집된 다른 속성과 값도 사용할 수 있습니다.
  </Collapser>
</CollapserGroup>

이러한 속성의 하위 집합은 EC2 인스턴스에 설치될 때 Infrastructure Agent에서 수집됩니다.
