...
두 측정항목과 동시에 일치하는 `sum()` 또는 `count()` 같은 일부 집계를 방지하려면 측정기준에 대해 `IS NULL` 또는 `IS NOT NULL` 값을 사용하세요. 예를 들어:
...
각 폴링 통합 및 해당 이름에서 사용 가능한 측정항목에 대한 참조는 [개별 통합에 대한 문서를 확인하세요](https://docs.newrelic.com/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).
