...
## 차원 메트릭 명명 규칙 [#metric-naming-convention]

Azure Monitor API에서 가져온 메트릭은 다음 규칙에 따라 New Relic에 [차원 메트릭](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) 으로 저장됩니다.

* 메트릭은 모두 소문자로 된 Azure 리소스 유형의 접두사가 붙습니다. 여기서 `/` 은 `.` 로 바뀌고 `Microsoft.` 는 `azure.` 로 바뀝니다.
...
지원되는 Azure 리소스 유형에 대한 자세한 내용은 [Azure Monitor Metrics 설명서 웹 사이트를](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/supported-metrics/metrics-index) 참조하세요.
...
## API 폴링 지표 [#azure-metrics-table]

각 폴링 통합 및 해당 이름에서 사용 가능한 측정항목에 대한 참조는 [개별 통합에 대한 문서를 확인하세요](/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).

다음 표는 Azure Polling 통합 및 해당 [차원 메트릭](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) 변환에서 수집한 메트릭의 포괄적이지 않은 목록입니다.
