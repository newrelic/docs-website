...
통합 데이터를 보려면 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Azure** 로 이동하여 Cosmos DB 통합을 선택합니다.
...
Cosmos DB 통합은 다음 메타데이터를 사용하여 항목 유형 `azure/cosmosdb/account/` 에 대한 인벤토리 데이터를 보고합니다.

* `documentEndpoint`: 문서 끝점입니다.
* `databaseAccountOfferType`: 데이터베이스 계정 제안 유형입니다.
* `consistencyPolicy`: Cosmos DB 데이터베이스 계정에 대한 일관성 정책입니다.
* `defaultConsistencyLevel`: Cosmos DB 데이터베이스 계정의 기본 일관성 수준입니다.
* `kind`: 데이터베이스 계정 생성 시 설정한 데이터베이스 계정의 종류.
* `resourceGroupName`: Cosmos DB 데이터베이스 계정이 속한 Azure 리소스 그룹 이름입니다.
* `regionName`: Azure DocumentDB 데이터베이스 계정이 배포된 지역 이름입니다.
* `type`: `Microsoft.DocumentDB/databaseAccounts` 인 Azure 리소스 유형입니다.