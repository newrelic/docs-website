...
<Callout title="메모">
  Azure Cost Management 데이터는 기존(폴링) Azure 통합을 통해서만 수집됩니다. 이 통합은 **없습니다** [Azure Monitor 통합](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/)을 통해 사용할 수 없습니다. 비용 데이터는 이벤트 기반이며 여러 리소스와 서비스에 걸쳐 있으므로, Azure Monitor 통합에서 요구하는 방식처럼 단일 Azure 리소스 유형에 매핑되지 않습니다.

  기존 Azure 통합에서 Azure Monitor 통합으로 마이그레이션하는 경우, Azure Cost Management 데이터를 계속 수집하려면 기존 Azure 통합을 활성화된 상태로 유지하십시오.
</Callout>