...
2. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure** 로 이동하고 **Azure Monitor** 통합을 찾습니다.
...
2. 새 **Azure Monitor metrics** \[Azure Monitor 메트릭] 통합을 활성화하고 **Save Changes** \[변경 사항 저장] 버튼을 클릭합니다.<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. **Azure Monitor metrics** \[Azure Monitor 메트릭] 통합을 사용할 수 있게 됩니다<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}>.</img>
