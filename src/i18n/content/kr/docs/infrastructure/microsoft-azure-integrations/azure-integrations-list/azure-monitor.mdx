...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="경고 조건"
  >
    Azure Monitor 통합 메트릭에서 [NRQL 경고 조건](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions) 을 만들 수 있습니다.
    ...
    * **집계 기간** : 집계 기간이 1분인 경우 평가 오프셋을 8분으로 설정하는 것이 좋습니다.
    ...
    자세한 내용은 [NRQL 경고 생성 방법을](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions) 참조하십시오.
  </Collapser>
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure로 이동하고 "Manage Services" (서비스 관리)를 클릭합니다**.
2. 새로운 **Azure Monitor metrics** \[Azure Monitor 메트릭] 통합을 활성화하고 **Save Changes** \[변경 내용 저장] 단추를 클릭합니다.<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. **Azure Monitor metrics** \[Azure Monitor 메트릭] 통합을 사용할 수 있게 됩니다.<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}/>
4. [여기](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/#set-up) 에 설명된 대로 새 통합을 활성화하고 구성합니다.<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
