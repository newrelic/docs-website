...
Azure Monitor 통합은 지원되는 모든 Azure 서비스에서 모든 Azure 메트릭을 모니터링합니다. 활성화하면 설정한 폴링 간격에 따라 Azure 플랫폼 서비스를 쿼리합니다. 활성화되면 다음을 얻을 수 있습니다.
...
## Azure Monitor 통합 사용 [#set-up]

다음은 New Relic에서 Azure Monitor 통합을 설정하기 위한 지침입니다. 또는 이 통합을 사용하는 [Azure Native New Relic Service](/docs/infrastructure/microsoft-azure-integrations/get-started/azure-native) 를 사용할 수 있습니다.
...
Azure Monitor 통합은 이전 Azure 솔루션을 개선합니다.
...
Azure Monitor 통합 전에 Azure를 모니터링하려면 메트릭 및 메타데이터를 검색하기 위해 서비스별 Azure API가 필요했습니다. 반대로 새로운 Azure Monitor 통합은 [사용 가능한 모든 Azure Monitor 메트릭](https://learn.microsoft.com/en-us/azure/azure-monitor/essentials/metrics-supported) 을 검색하여 Azure 지원을 강화하고 새로운 Azure 서비스 및 메트릭의 가용성을 가속화합니다.
