...
2. Enable the new **Azure Monitor metrics** integration and click the **Save Changes** button.<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. The **Azure Monitor metrics** integration becomes available.<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}/>
4. Enable and configure the new integration as described [here](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/#set-up).<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
