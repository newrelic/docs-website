...
<CollapserGroup>
  <Collapser
    ...
    title="마이크로소프트 애저 비용"
  >
    새로운 통합을 통해 모든 서비스에서 모든 지표를 수집할 수 있다는 점을 고려하면 <DoNotTranslate>**the amount of API calls to Azure will depend on the number of resources that are monitored by New Relic**</DoNotTranslate>. 통합의 폴링 시위를 설정하면 이러한 API 호출의 양을 제어하는 데 도움이 될 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Aggregation window**</DoNotTranslate>
      ...
      : 집계 기간이 1분인 경우 평가 오프셋을 8분으로 설정하는 것이 좋습니다.
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure**</DoNotTranslate> 으로 이동한 다음 <DoNotTranslate>**Manage Services**</DoNotTranslate> 클릭하세요.
2. 새 <DoNotTranslate>**Azure Monitor metrics**</DoNotTranslate> 통합을 활성화하고 <DoNotTranslate>**Save Changes**</DoNotTranslate> 클릭합니다.<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. <DoNotTranslate>**Azure Monitor metrics**</DoNotTranslate> 통합을 사용할 수 있게 됩니다.<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}/>
4. 새 통합을 활성화하고 구성합니다.<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
5. 설정이 완료되면 <DoNotTranslate>**Manage services**</DoNotTranslate> 설정에서 이전 폴링 통합을 비활성화할 수 있습니다.
