...
1. <DNT>
   ...
   필드에서 ECS 클러스터를 검색하거나 ECS 클러스터 이름을 입력합니다.
...
<Callout variant="important">
  Cgroup 버전 V2를 사용하는 EC2 노드에서 `nri-docker` 통합은 docker API 에서 모든 컨테이너 지표를 수집합니다. 수집된 지표의 변형 목록은 [`ContainerSample`](/attribute-dictionary/?event=ContainerSample) 속성을 참조하세요.
</Callout>
