...
New Relic의 ECS 통합은 [Amazon ECS](https://docs.aws.amazon.com/ecs/index.html) 환경의 성능 데이터를 보고하고 표시합니다.
...
* [AWS CloudFormation을 사용하여 설치](#cloud-formation-install)
* [자동 스크립트를 사용하여 설치](#auto-script-install)
* [수동으로 설치](#manual-install)

## CloudFormation을 사용하여 설치 [#cloud-formation-install]
...
1. New Relic의 ECS 통합 작업을 등록하려면 [이 스택](https://console.aws.amazon.com/cloudformation/home#/stacks/create/review?templateURL=https://nr-downloads-main.s3.amazonaws.com/infrastructure_agent/integrations/ecs/cloudformation/task/master.yaml&stackName=NewRelicECSIntegration) 을 배포하십시오. 원하는 지역에 스택을 배포하고 있는지 확인합니다. 이 스택은 다음 리소스를 생성합니다.

   * [라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 저장하는 비밀입니다.
   * 라이센스 키에 액세스하기 위한 정책입니다.
   * 라이선스 키에 대한 액세스 권한이 있는 ECS 작업 `ExecutionRole` 으로 사용할 인스턴스 역할.
   * EC2 및 외부(ECS Anywhere) 시작 유형의 경우: New Relic Infrastructure ECS 통합 작업을 등록합니다.

2. 시작 유형에 대한 추가 지침을 따르십시오.
   ...
   <CollapserGroup>
     ...
     >
       ...
       EC2 시작 유형을 선택합니다. 그러면 클러스터에 `newrelic-infra` 이라는 서비스가 생성됩니다.
     </Collapser>
     ...
     >
       ...
       1. 배포할 사이드카 컨테이너가 있는 작업 정의 예제를 다운로드합니다.
       ...
       2. 이 작업 정의의 `newrelic-infra` 컨테이너를 모니터링하려는 작업 정의에 사이드카로 추가합니다. 이 예제 작업에서 애플리케이션의 컨테이너는 자리 표시자 `busybox` 컨테이너를 대체합니다.
     </Collapser>
   </CollapserGroup>
...
* 권장 사항: 클러스터 및 서비스에 대한 정보를 포함하여 다른 ECS 데이터를 가져오는 [ECS 클라우드 통합](/docs/integrations/amazon-integrations/aws-integrations-list/aws-ecsecr-monitoring-integration) 을 설치하십시오.
* [권장 경고 조건](/docs/ecs-integration-recommended-alert-conditions) 을 참조하십시오.
* 이 프로세스에서 생성된 [AWS 리소스](#aws-resources) 를 이해합니다.
...
5. [라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) 와 클러스터 이름을 지정하여 설치 프로그램을 실행합니다.
...
6. [Fargate 시작 유형](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/launch_types.html) (EC2 시작 유형 아님)에 대한 추가 단계:
   ...
   * 배포할 사이드카 컨테이너가 있는 작업 정의 예제를 다운로드합니다.
   ...
   * 이 작업 정의의 단일 컨테이너를 모니터링하려는 작업 정의에 사이드카로 추가합니다.
...
한 가지 [설치 옵션](#install-overview) 은 [자동 설치 프로그램 스크립트](#auto-script-install) 에서 수행하는 단계를 수동으로 수행하는 것입니다. `awscli` 도구를 사용하여 이 작업을 수행하는 방법을 설명합니다.
...
2. [New Relic 라이선스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 Systems Manager(SSM) 매개변수로 저장합니다.
...
5. 정책 `NewRelicSSMLicenseKeyReadAccess` 및 `AmazonECSTaskExecutionRolePolicy` 을 역할에 연결합니다.
...
6. 자세한 지침을 보려면 시작 유형을 선택하십시오.
   ...
   <CollapserGroup>
     ...
     >
       ...
       1. New Relic ECS 통합 작업 정의 템플릿 파일을 다운로드합니다.
...
* 몇 분 정도 기다린 다음 [UI에서 데이터를 찾습니다](/docs/ecs-integration-understand-use-data) .
* 권장 사항: 클러스터 및 서비스에 대한 정보를 포함하여 추가 ECS 데이터를 가져오는 별도의 통합인 [ECS 클라우드 통합](/docs/integrations/amazon-integrations/aws-integrations-list/aws-ecsecr-monitoring-integration) 을 설치하십시오.
* [권장 경고 조건](/docs/ecs-integration-recommended-alert-conditions) 을 참조하십시오.
* 이 프로세스에서 생성된 [AWS 리소스](#aws-resources) 를 이해합니다.
...
* 시스템 관리자(SSM) 매개변수 `/newrelic-infra/ecs/license-key` 을(를) 생성합니다. 이 시스템 매개변수에는 New Relic [라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) 가 포함되어 있습니다.

* 라이선스 키로 SSM 매개변수에 대한 액세스를 활성화하는 IAM 정책 `NewRelicSSMLicenseKeyReadAccess` 을 생성합니다.
...
* [작업 실행 역할](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_execution_IAM_role.html) 로 사용되는 IAM 역할 `NewRelicECSTaskExecutionRole` 을 생성합니다. 역할에 연결된 정책:
  ...
  * `NewRelicSSMLicenseKeyReadAccess` (설치 프로그램이 생성).
...
* EXTERNAL(ECS Anywhere) [시작 유형](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/launch_types.html) 의 경우 이 작업도 수행됩니다.
  ...
  * [데몬 스케줄링](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_services.html) 전략 및 EXTERNAL(ECS Anywhere) 시작 유형을 사용하여 등록된 작업에 대한 서비스 `newrelic-infra-external` 를 생성합니다.