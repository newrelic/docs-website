...
2. 역할에 연결된 정책 나열:
   ...
   이것은 3개의 정책 `AmazonECSTaskExecutionRolePolicy` , `AmazonEC2ContainerServiceforEC2Role` 및 <LicenseKey/> 에 대한 읽기 액세스 권한을 부여해야 하는 세 번째 정책을 반환해야 합니다. 다음 예에서 정책 이름은 `NewRelicLicenseKeySecretReadAccess` 입니다.
...
3. 기본 정책 버전을 검색합니다.
   ...
   그러면 정책 권한이 검색됩니다. <LicenseKey/> 를 저장하기 위해 AWS Secrets Manager를 사용한 경우 작업 `secretsmanager:GetSecretValue` 에 대한 항목이 있거나 AWS Systems Manager Parameter Store를 사용한 경우 `ssm:GetParameters` 에 대한 항목이 있어야 합니다.
