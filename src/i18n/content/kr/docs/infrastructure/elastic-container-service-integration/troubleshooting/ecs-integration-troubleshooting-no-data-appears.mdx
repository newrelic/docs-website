...
2. 역할에 연결된 정책 나열:
   ...
   이렇게 하면 3개의 정책 `AmazonECSTaskExecutionRolePolicy` , `AmazonEC2ContainerServiceforEC2Role` 및 [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key)에 대한 읽기 액세스 권한을 부여해야 하는 세 번째 정책이 반환됩니다. 다음 예에서 정책 이름은 `NewRelicLicenseKeySecretReadAccess` 입니다.
...
3. 기본 정책 버전을 검색합니다.
   ...
   그러면 정책 권한이 검색됩니다. AWS Secrets Manager를 사용하여 [라이선스 키를](/docs/apis/intro-apis/new-relic-api-keys/#license-key)저장한 경우 작업 `secretsmanager:GetSecretValue` 에 대한 항목이 있거나 AWS Systems Manager Parameter Store를 사용한 경우 `ssm:GetParameters` 에 대한 항목이 있어야 합니다.
