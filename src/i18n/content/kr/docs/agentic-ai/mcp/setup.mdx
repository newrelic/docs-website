...
Claude Code는 OAuth(권장) 및 API 키 인증 방식을 모두 지원합니다. OAuth를 사용하려면 Claude Code 버전 2.1.186 이상이 필요합니다. 이전 버전을 사용하는 경우, Claude 내부에서 \{…} 대신 \{…}을 통해 인증해야 합니다.OAuth를 사용하려면 Claude Code 버전 2.1.186 이상이 필요합니다. 이전 버전을 사용하는 경우, Claude 내부에서 \{…} 대신 \{…}을 통해 인증해야 합니다.
...
2. `~/.kiro/settings/mcp.json`을(를) 열어(또는 생성하여) MCP 서버를 설정하고 다음 설정을 추가합니다:
...
3. 터미널을 열고 `kiro-cli`을(를) 실행하십시오. 뉴렐릭 MCP 서버가 자동으로 연결됩니다.
...
VS Code는 두 인증 방법 모두에서 계속 완벽하게 지원됩니다. VS Code(버전 1.91 이상)가 필요합니다.
