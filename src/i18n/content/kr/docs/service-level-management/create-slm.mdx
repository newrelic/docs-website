...
* **[one.newrelic.com](https://one.newrelic.com) > Service levels** 으로 이동합니다. 워크로드를 포함하여 계정 전반에 걸쳐 모든 엔터티와 SLI를 연결할 수 있습니다.
* APM 서비스, 브라우저 애플리케이션 또는 종합 모니터의 **서비스 수준** 페이지에서. SLI는 해당 특정 엔터티와 연결됩니다. 이 시작점을 사용하면 New Relic은 사용 가능한 최신 데이터를 기반으로 이 엔터티 유형에 대한 가장 일반적인 서비스 수준 지표를 자동으로 생성합니다.
...
SLI를 만든 후에는 서비스 수준 목록 페이지에서 **...** 메뉴를 클릭한 다음 `Edit` 을 클릭하여 수정할 수 있습니다.
