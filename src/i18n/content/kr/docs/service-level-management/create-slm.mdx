...
#### UI에서 SLI를 정의하는 쿼리 보기 [#view-query-ui]
...
#### API를 통해 SLI를 정의하는 쿼리 보기 [#view-query-api]
...
<CollapserGroup>
  ...
  현재 지원하는 두 가지 집계 함수는 `count()` 및 `sum()`입니다. `count()` 함수는 유효/양호/불량 이벤트의 수를 계산할 때 이벤트 데이터와 함께 사용하는 것이 좋습니다. sum()은 이벤트 데이터 또는 차원 메트릭에 미리 집계된 카운터가 있는 경우에 유용합니다.
