...
* **[one.newrelic.com >All capabilities](https://one.newrelic.com/all-capabilities) > Service levels** 으로 이동합니다. 워크로드를 포함하여 계정 전체의 모든 엔터티와 SLI를 연결할 수 있습니다.

* **Service levels** \[서비스 수준] 페이지에서

  <InlinePopover type="apm"/>

  서비스, 주요 트랜잭션, 브라우저 애플리케이션 또는 합성 모니터. SLI는 해당 특정 엔터티와 연결됩니다. 이 시작점을 사용하는 경우 New Relic은 사용 가능한 최신 데이터를 기반으로 이 엔터티 유형에 대한 가장 일반적인 서비스 수준 지표를 자동으로 생성합니다.
...
<CollapserGroup>
  ...
  >
    ...
    현재 지원하는 네 가지 집계 함수는 `count()`, `sum()`, `getField()` 및 `getCdfCount()` 입니다. `Count` 및 `sum` 는 모든 이벤트 유형에 사용할 수 있으며 `getField` 및 `getCdfCount` 은 **메트릭** 에서 선택할 때만 사용할 수 있습니다.
