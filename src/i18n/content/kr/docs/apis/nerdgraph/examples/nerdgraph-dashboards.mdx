...
#### 위젯 링크 및 설명 [#widget-links-descriptions]

위젯에 링크와 설명을 추가하려면 `dashboardUpdateWidgetsInPage` 사용할 수도 있습니다. 답변 링크는 답변 제목에 클릭 가능한 링크로 표시되며, 사용자가 답변을 가리킬 때 답변 설명은 툴팁으로 표시됩니다.

<Callout variant="important">
  위젯 링크 URL은 `http://` 또는 `https://` 프로토콜을 사용해야 하며 2,048자를 초과할 수 없습니다.
</Callout>

<CollapserGroup>
  <Collapser id="update-widget-link-description" title="위젯 링크 및 설명을 업데이트하세요.">
    ```graphql
    mutation UpdateWidgetsInPage($pageGuid: EntityGuid!, $widgets: [DashboardUpdateWidgetInput!]!) {
      dashboardUpdateWidgetsInPage(guid: $pageGuid, widgets: $widgets) {
        errors {
  ...
  <Collapser id="remove-widget-link-description" title="위젯 링크와 설명을 삭제하세요.">
    링크 또는 설명을 제거하려면 해당 필드를 `null` 으로 설정하세요.
...
### 템플릿 변수 가시성 [#variable-visibility]

템플릿 변수는 보기 모드에서 변수 표시줄에 표시되지 않지만 정상적으로 작동합니다. 변수의 `options` 객체에서 `hiddenOnVariablesBar` 필드를 사용하세요.

* **필드 이름:** `hiddenOnVariablesBar`
* **유형:** 부울
* **기본값:** `false` (변수가 표시됨)

<CollapserGroup>
  <Collapser id="create-hidden-variable" title="숨겨진 변수가 포함된 대시보드를 만드세요">
    ```graphql
    mutation {
      dashboardCreate(
  ...
  <Collapser id="update-variable-visibility" title="변수를 업데이트하여 숨기거나 표시하세요.">
    변수의 가시성을 업데이트하려면 업데이트된 `hiddenOnVariablesBar` 값을 포함하는 complete가시성 설정을 포함하세요.
...
템플릿 변수 표시 여부에 대한 자세한 내용은 [변수 표시 및 숨기기를](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility) 참조하세요.
