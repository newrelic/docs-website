...
4. <DoNotTranslate>**Direct PUT or other sources**</DoNotTranslate>

   사용하고 뉴렐릭의 JSON 이벤트 형식(예: S3, Redshift 또는 OpenSearch)과 호환되는 대상을 지정합니다.
...
4. <DoNotTranslate>**Require external ID**</DoNotTranslate>

   선택하고 내보내려는 뉴렐릭 계정의 [계정 ID를](/docs/accounts/accounts-billing/account-structure/account-id) 입력하세요.

5. <DoNotTranslate>**Permissions**</DoNotTranslate>

   클릭한 다음 위에서 생성한 정책을 선택합니다.
...
4. Event Hubs 네임스페이스가 구현하다, 배포되면,
...
4. <DoNotTranslate>**Send**</DoNotTranslate>
...
5. 생성된 정책을 클릭하고
   ...
   을 복사합니다. 이를 저장하여 이벤트 허브에 데이터를 인증하고 전송하는 데 사용합니다.
...
<CollapserGroup>
  ...
  >
    ...
    4. [객체 Lambda 액세스 포인트를 생성합니다](https://docs.aws.amazon.com/AmazonS3/latest/userguide/olap-create.html). 이 액세스 포인트에는 다음 설정이 있어야 합니다.
       ...
       * 이 Lambda 액세스 포인트의
         ...
         는 S3 버킷에 설정한 액세스 포인트로 설정되어야 합니다.
  ...
  >
    ...
    * 이 역할에는 `s3:PutObject` 및 `s3:GetObject` 권한이 있는 정책이 있어야 합니다.
    ...
    4. <DoNotTranslate>**Create function**</DoNotTranslate> 을(를) 클릭합니다.

    5. Java 11 런타임 환경을 선택합니다.

    6. <DoNotTranslate>**Change default execution role > Use an existing role**</DoNotTranslate> 클릭합니다. 여기에 2단계에서 생성한 역할을 입력합니다.

    7. 아래로 스크롤하여 <DoNotTranslate>**Create function**</DoNotTranslate> 클릭합니다.

    8. 함수가 생성되면 <DoNotTranslate>**Upload from**</DoNotTranslate> 클릭하고 드롭다운 메뉴에서 <DoNotTranslate>**.zip or .jar file**</DoNotTranslate> 선택하세요.

    9. 팝업 상자에서 <DoNotTranslate>**Upload**</DoNotTranslate> 클릭하고 1단계에서 생성된 ZIP 파일을 선택합니다.

    10. 업로드가 완료되면 <DoNotTranslate>**Save**</DoNotTranslate> 클릭하여 팝업 상자를 종료하세요.

    11. 핸들러를 추가하여 <DoNotTranslate>**Runtime settings**</DoNotTranslate> 편집합니다. 제공된 함수에서 핸들러는 <DoNotTranslate>`metadatasetter.App::handleRequest`</DoNotTranslate> 입니다.

    12. 이제 남은 일은 이 Lambda 함수가 S3 객체 생성 시 트리거되도록 활성화하는 것입니다. 설정을 시작하려면 <DoNotTranslate>**Add trigger**</DoNotTranslate> 클릭하세요.

    13. 다운 드롭 메뉴에서 소스로 <DoNotTranslate>**S3**</DoNotTranslate> 선택하세요.

    14. <DoNotTranslate>**Bucket**</DoNotTranslate> 필드에 메타데이터를 적용하려는 S3 버킷의 이름을 입력합니다.

    15. 이벤트 유형에서 기본 <DoNotTranslate>**All object create events**</DoNotTranslate> 를 제거합니다. 이벤트 유형 드롭다운 메뉴에서 <DoNotTranslate>**PUT**</DoNotTranslate> 선택하세요.

    16. <DoNotTranslate>**Recursive invocation**</DoNotTranslate> 상자를 선택한 다음 오른쪽 하단에서 <DoNotTranslate>**Add**</DoNotTranslate> 클릭합니다.
...
2. 스트리밍 분석 작업의 왼쪽 창에서

   <DoNotTranslate>**Inputs**</DoNotTranslate>

   클릭한 다음 설정한 입력을 클릭합니다.
...
5. 화면 측면에서
   ...
   탭을 사용하여 이 화면에서 이벤트 허브를 쿼리할 수 있습니다.