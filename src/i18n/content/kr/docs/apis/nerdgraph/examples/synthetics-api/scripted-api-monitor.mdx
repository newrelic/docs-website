---
title: 스크립트 기반 API 모니터 관리
tags:
  - Synthetics
  - APIs
  - NerdGraph
  - Examples
metaDescription: How to use New Relic NerdGraph API to create and update scripted API monitors for synthetic monitoring.
freshnessValidatedDate: never
translationType: machine
---

뉴렐릭을 사용하면 NerdGraph를 사용하여 [스크립트 API 모니터를](/docs/synthetics/synthetic-monitoring/scripting-monitors/write-synthetic-api-tests) 만들 수 있습니다. 스크립트 API 모니터는 브라우저 없이 API 및 서비스를 테스트하기 위해 사용자 지정 JavaScript 코드를 실행하고, HTTP requests 보내고 응답을 프로그래밍 방식으로 검증합니다. 이 튜토리얼에서는 NerdGraph API를 사용하여 스크립트 기반 API 모니터 생성을 자동화하는 방법에 대한 예제를 제공합니다.

## 스크립팅된 API 모니터 만들기 [#create-scripted-api]

<DNT>`syntheticsCreateScriptApiMonitor`</DNT> 뮤테이션을 사용하면 스크립트 기반 API 모니터를 생성할 수 있습니다. 이 변형을 사용하면 JavaScript 코드를 실행하여 API 엔드포인트를 검증하는 사용자 지정 API 테스트를 설정할 수 있습니다.

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `accountId`
      </td>

      <td>
        정수
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 생성될 뉴렐릭 

        [계정 ID입니다](/docs/accounts/accounts-billing/account-structure/account-id)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.public`
      </td>

      <td>
        정렬
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 검사를 실행하는 

        [표시 위치](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors/#setting-location)

         제외(예: 

        `["US_EAST_1", "US_WEST_1"]`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.name`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        스크립트로 작성된 API 모니터의 표시 이름입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.period`
      </td>

      <td>
        열거형
      </td>

      <td>
        네
      </td>

      <td>
        모니터링이 실행되는 빈도. 옵션: 

        `EVERY_MINUTE`

        , 

        `EVERY_5_MINUTES`

        , 

        `EVERY_10_MINUTES`

        , 

        `EVERY_15_MINUTES`

        , 

        `EVERY_30_MINUTES`

        , 

        `EVERY_HOUR`

        , 

        `EVERY_6_HOURS`

        , 

        `EVERY_12_HOURS`

        , 

        `EVERY_DAY`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용하는 런타임 유형입니다. 

        `NODE_API`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용하는 런타임 유형 버전입니다. 현재 Node.js 버전에 

        `22.20.0`

         을(를) 사용하세요(권장). 

        `16.10`

         과(와) 같은 이전 특정 버전도 허용되지만 향후 지원이 중단될 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용되는 언어입니다. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.script`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 실행하는 자바스크립트 코드입니다. 이것은 base64로 인코딩된 텍스트가 아니라 일반 텍스트여야 합니다. 이 스크립트는 `$http` 객체를 사용하여 HTTP requests 보내고 API 테스트를 수행할 수 있습니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.status`
      </td>

      <td>
        열거형
      </td>

      <td>
        네
      </td>

      <td>
        모니터 상태. 옵션: 

        `ENABLED`

         (모니터가 활성화되어 점검을 수행 중), 

        `DISABLED`

         (모니터가 비활성화됨).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.apdexTarget`
      </td>

      <td>
        뜨다
      </td>

      <td>
        아니요
      </td>

      <td>
        SLA 보고서를 채우는 데 사용되는 모니터의 Apdex 목표(초 단위)입니다. 기본값은 7.0초입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

```graphql
mutation {
  syntheticsCreateScriptApiMonitor(
    accountId: ACCOUNT_ID
    monitor: {
      locations: { public: ["LOCATION_1", "LOCATION_2"] }
      name: "YOUR_MONITOR_NAME"
      period: PERIOD
      runtime: {
        runtimeType: "RUNTIME_TYPE"
        runtimeTypeVersion: "RUNTIME_TYPE_VERSION"
        scriptLanguage: "SCRIPT_LANGUAGE"
      }
      script: "SCRIPT_CONTENT"
      status: STATUS
      apdexTarget: APDEX_TARGET
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsCreateScriptApiMonitor": {
      "errors": null
    }
  }
}
```

모니터를 생성하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 스크립트 기반 API 모니터를 업데이트합니다. [#update-scripted-api]

기존 스크립트 기반 API 모니터는 <DNT>`syntheticsUpdateScriptApiMonitor`</DNT> 뮤테이션을 사용하여 업데이트할 수 있습니다. 이 기능을 사용하면 이미 생성된 스크립트 기반 API 모니터의 설정을 수정할 수 있습니다.

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        업데이트하려는 모니터의 고유 엔티티 

        [GUID입니다](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.public`
      </td>

      <td>
        정렬
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터가 검사를 실행하는 

        [표시 위치](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors/#setting-location)

         제외(예: 

        `["US_EAST_1", "US_WEST_1"]`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.name`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        스크립트 기반 API 모니터의 업데이트된 표시 이름입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.period`
      </td>

      <td>
        열거형
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터링이 실행되는 빈도. 옵션: 

        `EVERY_MINUTE`

        , 

        `EVERY_5_MINUTES`

        , 

        `EVERY_10_MINUTES`

        , 

        `EVERY_15_MINUTES`

        , 

        `EVERY_30_MINUTES`

        , 

        `EVERY_HOUR`

        , 

        `EVERY_6_HOURS`

        , 

        `EVERY_12_HOURS`

        , 

        `EVERY_DAY`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용하는 런타임 유형입니다. 

        `NODE_API`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용하는 런타임 유형 버전입니다. 현재 Node.js 버전에 

        `22.20.0`

         을(를) 사용하세요(권장). 

        `16.10`

         과(와) 같은 이전 특정 버전도 허용되지만 향후 지원이 중단될 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용되는 언어입니다. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.script`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터가 실행하는 자바스크립트 코드입니다. 이것은 base64로 인코딩된 텍스트가 아니라 일반 텍스트여야 합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.status`
      </td>

      <td>
        열거형
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터 상태. 옵션: 

        `ENABLED`

         (모니터가 활성화되어 점검을 수행 중), 

        `DISABLED`

         (모니터가 비활성화됨).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.apdexTarget`
      </td>

      <td>
        뜨다
      </td>

      <td>
        아니요
      </td>

      <td>
        SLA 보고서를 채우는 데 사용되는 모니터의 Apdex 목표(초 단위)입니다. 기본값은 7.0초입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

```graphql
mutation {
  syntheticsUpdateScriptApiMonitor(
    guid: ENTITY_GUID
    monitor: {
      locations: { public: ["LOCATION_1", "LOCATION_2"] }
      name: "YOUR_MONITOR_NAME"
      period: PERIOD
      runtime: {
        runtimeType: "RUNTIME_TYPE"
        runtimeTypeVersion: "RUNTIME_TYPE_VERSION"
        scriptLanguage: "SCRIPT_LANGUAGE"
      }
      script: "SCRIPT_CONTENT"
      status: STATUS
      apdexTarget: APDEX_TARGET
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsUpdateScriptApiMonitor": {
      "errors": null
    }
  }
}
```

모니터를 업데이트하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 스크립트 기반 API 모니터의 런타임을 업그레이드하세요 [#upgrade-monitor-runtime]

최신 Node.js 런타임을 사용하도록 스크립트 API 모니터를 업그레이드할 수 있습니다. 최신 Node.js 릴리스의 경우 버전 `22.20.0` 을(를) 사용하는 것을 권장합니다.

<Callout variant="important">
  **Node.js 버전 관리**: 특정 Node.js 버전(예: 16.10)의 수명이 종료됩니다. **2026년 11월 18일**에 여전히 이전 Node.js 버전을 사용하는 스크립트 API 모니터는 Node.js 22로 강제 업그레이드됩니다. 현재 지원되는 버전은 `22.20.0`입니다. 모니터에서 최신 Node.js 업데이트 및 기능을 사용하려면 이 날짜 이전에 `runtimeTypeVersion: "22.20.0"` (으)로 업그레이드하세요.
</Callout>

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        업그레이드하려는 모니터의 고유 엔티티 

        [GUID입니다](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        런타임 유형입니다. 

        `NODE_API`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        런타임 버전. 가장 최신 Node.js 버전에는 

        `22.20.0`

         을(를) 사용하세요(권장). 

        `16.10`

         과 같은 이전 특정 버전도 허용되지만 더 이상 사용되지 않을 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        언어를 펼쳐보세요. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

Node.js 버전 22.20.0으로 업그레이드 (권장):

```graphql
mutation {
  syntheticsUpdateScriptApiMonitor(
    guid: "ENTITY_GUID"
    monitor: {
      runtime: {
        runtimeType: "NODE_API"
        runtimeTypeVersion: "22.20.0"
        scriptLanguage: "JAVASCRIPT"
      }
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsUpdateScriptApiMonitor": {
      "errors": null
    }
  }
}
```

모니터 런타임을 업그레이드하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## VSE 활성화된 상대 위치 간에 스크립트된 API 모니터 이동 [#move-monitor]

VSE 지원 반대 위치 간에 스크립트된 API 모니터를 이동할 수 있습니다. 검증된 실행(VSE)이 활성화된 항체 위치 간에 모니터를 이동할 때 보안 검증을 위해 HMAC를 재생성하려면 펼쳐를 포함해야 합니다.

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        이동하려는 모니터의 고유 엔티티 

        [GUID입니다](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.private`
      </td>

      <td>
        정렬
      </td>

      <td>
        네
      </td>

      <td>
        로케이션 설정의 제외. 각 위치에는 VSE가 활성화된 위치에 대해 

        `guid`

         과 

        `vsePassword`

         필요합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.private.guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        군단의 GUID, 목표 군대 로케이션입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.private.vsePassword`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        개구리, 목표 몬스터 로케이션의 VSE 비밀번호입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.script`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 실행하는 자바스크립트 코드입니다. 이는 VSE가 활성화된 위치 간 이동 시 HMAC를 재생성하기 위해 반드시 포함되어야 합니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

```graphql
mutation {
  syntheticsUpdateScriptApiMonitor(
    guid: "ENTITY_GUID"
    monitor: {
      locations: {
        private: [{
          guid: "LOCATION_GUID",
          vsePassword: "YOUR_VSE_PASSWORD"
        }]
      },
      script: "SCRIPT_CONTENT"
    }
  ) {
    errors {
      description
      type
    }
    monitor {
      status
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환하며 모니터링 상태를 포함합니다.

```json
{
  "data": {
    "syntheticsUpdateScriptApiMonitor": {
      "errors": null,
      "monitor": {
        "status": "ENABLED"
      }
    }
  }
}
```

모니터를 이동하는 데 문제가 발생하면 `errors` 에는 무엇이 잘못되었는지 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 스크립트 기반 API 모니터를 삭제합니다. [#delete-monitor]

스크립트 기반 API 모니터가 더 이상 필요하지 않은 경우 <DNT>`syntheticsDeleteMonitor`</DNT> 뮤테이션을 사용하여 영구적으로 제거할 수 있습니다.

모니터를 삭제하려면 [&apos;합성 모니터 삭제](/docs/apis/nerdgraph/examples/synthetics-api/ping-monitor/#delete-monitor) &apos; 섹션을 참조하십시오.

## 스크립트 API 모니터 쿼리 [#query-scripted-api-monitors]

NerdGraph를 사용하여 메타데이터, 스크립트, 맵 ID를 검색하거나 스크립팅된 API 모니터의 상태를 확인하세요.

자세한 내용은 다음을 참조하세요:

* [모든 모니터 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors): 모든 신세틱 모니터 및 해당 설정을 나열합니다.
* [모니터 ID를 엔티티 GUID에 매핑](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-guid-mapping): 구형 모니터 ID를 엔티티 GUID로 마이그레이션합니다.
* [런타임 업그레이드 상태](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-runtime-upgrade-all): 모니터가 최신 런타임 업그레이드 준비가 되었는지 확인합니다.
* [모니터 스크립트 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitor-script) - 스크립트된 모니터에서 JavaScript 코드를 가져옵니다.

전체 쿼리 예제 목록을 보려면 [신세틱스 데이터 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data) 문서를 참조하세요.