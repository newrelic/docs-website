...
## 난독화 룰 생성 [#create-obfuscation-rules]

뉴렐릭 플랫폼 또는 NerdGraph에서 직접 수집 난독화 룰을 생성하여 민감한 데이터가 수집되기 전에 마스크할 수 있습니다. 이 룰은 모든 브라우저 이벤트에 적용됩니다. 플랫폼 또는 NerdGraph를 통해 설정된 룰은 APM이 주입된 애플리케이션, 복사/붙여넣기 설치 및 NPM 설치에 적용됩니다. 플랫폼 또는 JavaScript를 통해 룰을 생성하려면 [브라우저 에이전트에서 민감한 데이터 난독화](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/)를 참조하세요.

NerdGraph를 통해 난독화 룰을 생성하려면:

<CollapserGroup>
  <Collapser id="retrieve" title="1단계: 난독화 룰 가져오기">
    다음 쿼리를 사용하여 브라우저 애플리케이션의 현재 난독화 룰을 검색합니다:
    ...
    <Callout variant="important">
      `agentApplicationSettingsUpdate` 뮤테이션은 기존 룰을 제공하는 새로운 룰 세트로 덮어쓰므로, 업데이트하기 전에 현재 룰을 검색해야 합니다. 기존 룰을 유지하려면 뮤테이션에서 보내는 새로운 룰 세트에 해당 룰을 포함해야 합니다.
    </Callout>
  </Collapser>

  <Collapser id="update" title="2단계: 난독화 룰 업데이트">
    `agentApplicationSettingsUpdate` 뮤테이션을 사용하여 난독화 룰을 업데이트합니다. 다음 속성을 정의하여 새 룰을 추가하거나 기존 룰을 수정할 수 있습니다:

    * `regex`: 난독화하려는 데이터와 일치하는 정규식 패턴입니다. 예를 들어, `\b\d{3}-?\d{2}-?\d{4}\b` 을(를) 사용하여 사회 보장 번호와 일치시킬 수 있습니다.
    * `replacement`: 일치하는 데이터를 대체할 문자열입니다. 예를 들어 `XXX-XX-XXXX` 을(를) 사용하여 주민등록번호를 마스크할 수 있습니다.
    * `eventFilter`: 룰을 적용해야 하는 브라우저 이벤트 유형의 어레이입니다. 예를 들어, `["AjaxRequest"]` 을(를) 지정하여 AJAX 요청 이벤트에만 룰을 적용할 수 있습니다.
...
## AJAX 페이로드 캡처 구성 [#configure-payload-capture]

NerdGraph를 사용하여 브라우저 애플리케이션에 대한 AJAX 페이로드 캡처 설정을 검색하고 업데이트할 수 있습니다. 이 설정은 AJAX 호출에 대한 요청 및 응답 페이로드 캡처 여부를 제어하며 [애플리케이션 설정의 페이로드 캡처 옵션](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/#payload-capture)에 해당합니다.

두 단계로 페이로드 캡처 설정을 활성화할 수 있습니다:

1. [현재 페이로드 캡처](#query-payload-capture) 설정을 검색합니다.

2. [`capturePayloads` 모드](#update-payload-capture) 를 다음 값 중 하나로 설정합니다:

   * `none`: 페이로드 데이터가 캡처되지 않습니다.
   * `failures`: 상태 코드가 `0`이거나 `400` 이상인 requests 또는 `response body`에 GraphQL 오류 서명이 있는 requests에 대해서만 페이로드를 캡처합니다.
   * `all`: 모든 requests에 대한 페이로드를 캡처합니다.

<CollapserGroup>
  <Collapser id="query-payload-capture" title="페이로드 캡처 설정 쿼리">
    다음 쿼리를 사용하여 브라우저 애플리케이션에 대한 현재 페이로드 캡처 설정을 검색합니다:
  ...
  <Collapser id="update-payload-capture" title="페이로드 캡처 설정 업데이트">
    `agentApplicationSettingsUpdate` 뮤테이션을 사용하여 페이로드 캡처 설정을 업데이트합니다:
    ...
    변수에서 `guid` 및 `capturePayloads` 옵션을 설정합니다.
