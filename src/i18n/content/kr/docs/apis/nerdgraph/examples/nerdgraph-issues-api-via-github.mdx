---
title: 'Nerdgraph 자습서: 알림 및 적용된 인텔리전스 문제 구성'
metaDescription: Configure issues in alerts and applied intelligence
translationType: machine
---

이 문서는 귀하와 귀하의 팀이 문제 및 사건에 대한 데이터를 쿼리한 다음 NerdGraph API를 사용하여 기능을 구성하는 방법을 배울 수 있는 장소입니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `INCIDENT_ADDED` , `USER_ACTION` , `INCIDENT_CLOSED` 등이 될 수 있는 문제 이벤트...
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    ...
    여기서 `entityGuides` 및 `entityTypes` 은 [New Relic 엔티티에 대해 알아보기](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) 문서에 정의된 엔티티 속성입니다.
    ...
    * 그리고 이미 본 것처럼 `cursor`
    ...
    우선순위 `CRITICAL` 및 상태의 문제를 찾아봅시다. `CLOSED`
...
<CollapserGroup>
  ...
  >
    ...
    엔터티 가이드, 엔터티 유형, 문제 우선 순위 또는 문제 상태 또는 이들의 조합을 기준으로 인시던트를 필터링합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="인시던트 이벤트 나열"
  >
    필터링 데이터를 제공하지 않으면 지난 24시간 동안의 사건 이벤트 데이터를 받게 됩니다.
  ...
  >
    ...
    * 그리고 이미 본 것처럼 `cursor`
    ...
    인시던트 이벤트를 쿼리하여 'incidentId'를 얻을 수 있도록 합니다.
...
<CollapserGroup>
  ...
  >
    ...
    해당 `mutations` 사용하여 `issueTt` l 및 `gracePeriod` 환경 구성 매개변수를 변경할 수도 있습니다.
...
`mutations` 사용하면 해당 issueId로 주어진 문제를 확인하거나 확인 취소하고 해결할 수 있습니다.
...
2 export API_KEY=my_api_key를 사용하여 셸로 내보냅니다.
