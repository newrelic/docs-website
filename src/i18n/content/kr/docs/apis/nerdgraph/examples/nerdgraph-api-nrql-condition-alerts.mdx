...
2. 다음 중 하나를 수행하여 관련 `policyID` 을(를) 찾으십시오.
   ...
   * **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (policies)** 으로 이동합니다. 정책을 선택합니다. 정책 이름 아래에서 ID를 찾습니다.
