...
<DNT>**Golden metrics**</DNT> 및 <DNT>**golden tags**</DNT> 엔터티에 대해 가장 중요하다고 간주되는 [엔터티](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic) 에 대한 정보입니다. 우리는 이 정보를 사용하여 뉴렐릭 전체에 걸쳐 엔터티에 대한 간략한 개요를 표시합니다. 이 [공개 저장소](https://github.com/newrelic/entity-definitions#golden-tags) 에서 골든 메트릭 및 태그의 표준 정의를 보고 이에 기여할 수 있습니다.
...
* `metrics`: 새 NRDB 쿼리는 골든 메트릭으로 표시됩니다.
  ...
  * `select`: NRDB 쿼리의 `SELECT` 절입니다. 이 필드는
