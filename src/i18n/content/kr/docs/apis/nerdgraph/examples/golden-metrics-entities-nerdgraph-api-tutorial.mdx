...
* `metrics`: 새 NRDB 쿼리는 골든 메트릭으로 표시됩니다.
  ...
  * `select`: NRDB 쿼리의 `SELECT` 절입니다. 이 필드는 <DNT>**required**</DNT> 입니다.
  * `from`: NRDB 쿼리의 `FROM` 절입니다.
  * `where`: 항목 유형 필드를 식별하기 위한 보완적인 `WHERE` 절.
  * `facet`: `FACET` 에 대한 필드입니다.
  * `title`: 골든 메트릭의 제목입니다. 이 필드는 선택 사항입니다.
  * `name`: 골든 등급의 이름입니다. 이 필드는 <DNT>**required**</DNT> 입니다.

<CollapserGroup>
  <Collapser id="override-account" title="APM 애플리케이션 유형에 대한 골든 메트릭 재정의">
    ```graphql
    mutation {
      entityGoldenMetricsOverride(
...
메트릭 입력 객체는 NRDB 쿼리의 각 부분을 섹션별로 구분하여 정의합니다. 이러한 섹션 중 하나는 `eventId` 이며, 이는 골든 메트릭으로 사용하려는 *NRDB 이벤트* 내의 GUID를 정의하는 필드를 식별하는 데 사용됩니다. 예를 들어:
...
[뉴렐릭&apos;s 워크로드는](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)) 그룹의 상태 및 성능 데이터에 대한 집계된 보기를 제공합니다. 워크로드의 각 유형별로 표시되는 시계열 차트는 워크로드 계정의 골든 인덱스로 정의됩니다.
...
<CollapserGroup>
  <Collapser id="override-workload" title="APM 애플리케이션 재정의">
    ```graphql
    mutation {
      entityGoldenMetricsOverride(
