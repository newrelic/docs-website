...
* `expiration.ignoreOnExpectedTermination`: `true` 인 경우 신호가 끊어질 것으로 예상되면 인시던트가 열리지 않습니다. 신호가 손실될 것으로 예상됨을 나타내려면 엔터티에 태그 `termination: expected` 이 있어야 합니다. 기본값은 `false` 입니다.
