...
이 문서는 NerdGraph API를 사용하여 <InlinePopover type="apm"/>에이전트를 구성하는 예를 보여주는 곳입니다.

## 서버 측 구성 구성 [#server-side-config]

<InlinePopover type="apm"/>에이전트가 NerdGraph를 통해 변경된 설정 값을 사용하려면 [서버 측 설정을](/docs/apm/agents/manage-apm-agents/configuration/server-side-agent-configuration#requirements) 활성화해야 합니다.
