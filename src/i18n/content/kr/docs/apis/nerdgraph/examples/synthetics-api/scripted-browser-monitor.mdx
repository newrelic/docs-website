...
<DNT>`syntheticsCreateScriptBrowserMonitor`</DNT> 변형을 사용하여 멀티미디어 브라우저 모니터를 만들 수 있습니다. 이 변형을 사용하면 브라우저에서 JavaScript 코드를 실행하는 사용자 지정 스크립트 모니터링을 설정할 수 있습니다.
    ...
    <tr>
      ...
      <td>
        ...
        `monitor.location`
         ...
         은(는) 두 가지 값을 허용합니다: 프라이빗 로케이션 및 공개 위치. 프라이빗 로케이션, 공개 위치 또는 둘 다 선택할 수 있습니다:
...
<DNT>`syntheticsUpdateScriptBrowserMonitor`</DNT> 변형을 사용하여 기존 브라우저 브라우저 모니터를 업데이트할 수 있습니다. 이를 통해 이미 생성된 멀티미디어 브라우저 디스플레이의 설정을 수정할 수 있습니다.
    ...
    <tr>
      ...
      <td>
        ...
        `monitor.location`
         ...
         은(는) 두 가지 값을 허용합니다. 1.은 프라이빗 로케이션이고 2.는 공개 위치입니다. 프라이빗 로케이션, 공개 위치 또는 둘 다 선택할 수 있습니다:
...
펼쳐보기 브라우저 모니터가 더 이상 필요하지 않으면 <DNT>`syntheticsDeleteMonitor`</DNT> 변형을 사용하여 영구적으로 제거할 수 있습니다.

모니터를 삭제하려면 [&apos;합성 모니터 삭제](/docs/apis/nerdgraph/examples/synthetics-api/ping-monitor/#delete-monitor) &apos; 섹션을 참조하십시오.

## 스크립트 브라우저 모니터 쿼리 [#query-scripted-browser-monitors]

NerdGraph를 사용하여 메타데이터, 스크립트, 맵 ID를 검색하거나 스크립트 브라우저 모니터의 상태를 확인하십시오.
...
* [모니터 스크립트 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitor-script) - 스크립트된 모니터에서 JavaScript 코드를 가져옵니다.
