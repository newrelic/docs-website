...
쉽게 시작하려면, [NerdGraph API 탐색기](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) 를 사용하여 팀 정보를 생성하고 편집하세요. 아래 예제의 각 필드에 대한 정의는 NerdGraph에서 확인할 수 있습니다. GraphiQL IDE를 사용하는 데 도움이 필요하면 [NerdGraph API 탐색기 튜토리얼](/docs/apis/nerdgraph/get-started/nerdgraph-explorer/)을 참조하십시오. 팀 관리를 위한 사용자 지정 권한을 설정해야 하는 경우 [팀용 사용자 지정 역할 생성](/docs/apis/nerdgraph/examples/nerdgraph-teams-custom-role-tutorial)을 참조하십시오.
...
* `parentId` 계층 구조 트리에서 상위 팀 ID를 나타냅니다.
...
* `hierarchyLevelId`: 조직 설정([조직 설정](#update-team-settings) 참조)을 통해 구성되는 계층 구조에서 이 팀의 위치를 나타내는 `TeamsHierarchyLevelEntity` 의 ID입니다.
* `managers`: 이 팀의 관리자 또는 주요 연락 담당자인 사용자 엔티티 GUID의 목록입니다. 관리자는 팀원이어야 합니다. 사용자 GUID를 찾으려면 [사용자 ID 검색](#user-id)을 참조하십시오.
* `onCallMembers`: 정규 근무 시간 외의 연락처를 나타내는 `OnCallMember` 엔티티의 목록입니다. 각 엔티티에는 사용자의 엔티티 GUID가 포함된 ID 필드가 필요합니다. 대기 멤버는 팀 멤버여야 합니다. 사용자 GUID를 찾으려면 [사용자 ID 검색](#user-id)을 참조하십시오.
* `parentId`계층 구조에서 상위 팀의 ID입니다.
...
* `managers` 사용자 엔티티 GUID 목록을 허용합니다. 사용자 GUID를 찾으려면 [사용자 ID 검색을](#user-id) 참조하세요.
* `onCallMembers` `OnCallMember` 엔티티의 목록을 허용합니다. 각 엔티티에는 사용자의 엔티티 GUID가 포함된 ID 필드가 필요합니다. 사용자 GUID를 찾으려면 [사용자 ID 검색](#user-id)을 참조하십시오.

## 계층 수준 업데이트 [#update-hierarchy-level]

계층 레벨의 이름을 업데이트하려면 `entityManagementUpdateTeamsHierarchyLevel` 뮤테이션을 사용하십시오. [조직 설정 쿼리](#update-team-settings) 의 `hierarchyLevelOrder` 필드에서 계층 수준 ID를 찾을 수 있습니다.
...
* `name` 계층 수준에서 유일하게 업데이트 가능한 속성입니다.
* `id` 은 업데이트할 `TeamsHierarchyLevelEntity` 의 ID입니다. [조직 설정 쿼리](#update-team-settings) 에서 반환된 `hierarchyLevelOrder` 개에서 이러한 ID를 찾을 수 있습니다.
...
* `hierarchyLevelOrder`계층 구조의 순서를 내림차순으로 정의하는 것으로, 첫 번째 요소가 가장 높은 수준이고 마지막 요소가 가장 낮은 수준입니다. 각 요소는 `TeamsHierarchyLevelEntity` 의 `id` 입니다.
