---
title: 'NerdGraph 튜토리얼: 대시보드 위젯 만들기 및 구성'
tags:
  - APIs
  - NerdGraph
  - Examples
metaDescription: Use New Relic NerdGraph to add and configure dashboards.
freshnessValidatedDate: never
translationType: machine
---

New Relic 대시보드 API를 사용하면 [NerdGraph](https://api.newrelic.com/graphiql) 를 사용하여 [대시보드](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards) 를 구축할 수 있습니다. 이 문서에서는 대시보드에 추가할 수 있는 다양한 유형의 위젯과 API를 사용하여 위젯을 만들고 가져오는 방법에 대해 설명합니다.

## 위젯 스키마 및 유형 [#widget-schema]

쿼리 및 변형 출력을 위한 위젯 GraphQL 스키마는 다음과 같습니다.

```graphql
type Widget {
  id
  visualization
  layout
  title
  linkedEntities
  configuration
  rawConfiguration
  description
  link
}
```

어디에:

* `id`: 위젯의 ID입니다.
* `visualization`위젯의 시각화 유형을 문자열로 나타냅니다. 예를 들면: `viz.line`, `viz.area`. [아래 예시들을](#typed-widget-data) 참조하세요.
* `layout`: 대시보드에서 위젯의 위치와 크기입니다. 최대 열 수는 12개입니다.
* `title`: 위젯의 제목입니다.
* `linkedEntities`: 위젯과 관련된 대시보드 엔터티입니다. 현재 대시보드 엔터티만 지원합니다. [패싯 연결 기능](/docs/query-your-data/explore-query-data/dashboards/filter-new-relic-one-dashboards-facets) 에서 사용됩니다.
* `configuration`: 답변의 [입력된 설정](#widget-typed) 입니다. 이는 가장 일반적인 위젯([area](#area), [bar](#bar), [billboard](#billboard), [line](#line), [markdown](#markdown), [pie](#pie), [table)](#table) 을 편리하게 타입으로 표현한 GraphQL 형식입니다.
* `rawConfiguration`: 해당 논점의 유형이 [지정되지 않은 설정입니다](#widget-untyped). 이는 [히트맵](#heatmap), [히스토그램](#histogram), [bullet](#bullet), [퍼널](#funnel) 등과 같은 유형화된 설정에 포함되지 않는 속성을 저장하는 자유 형식 방식(JSON 스칼라)입니다. 이 경우 `configuration` 은 null이고 `rawConfiguration` 에는 위젯에 필요한 모든 데이터가 들어 있습니다.
* `description`: 위젯에 대한 추가적인 맥락을 제공하는 선택적 텍스트로, 사용자가 위젯을 가리킬 때 툴팁으로 표시됩니다.
* `link`위젯 제목을 클릭 가능하게 만들어 관련 대시보드, 문서 또는 외부 리소스로 이동할 수 있도록 하는 선택적 URL입니다. `http://` 또는 `https://` 프로토콜을 사용해야 하며 2,048자를 초과할 수 없습니다.

### 위젯 유형 [#types-widget]

대시보드 API는 유형이 지정된 위젯과 유형이 지정되지 않은 위젯을 지원합니다.

<CollapserGroup>
  <Collapser id="widget-typed" title="입력된 위젯">
    유형이 지정된 위젯은 [area](#area) , [bar](#bar) , [billboard](#billboard) , [line](#line) , [markdown](#markdown) , [pie](#pie) 및 [table](#table) 입니다. 필요한 필드에 대한 지원을 제공하는 NerdGraph를 사용하여 직접 구축할 수 있습니다.

    입력으로:

    * `configuration` 만 제공하면 됩니다.

    * `visualization` 제공된 구성에서 추론할 수 있는 한 null일 수 있습니다.

      API는 `rawConfiguration` 에서 제공한 구성과 JSON의 원시 표현을 출력으로 제공합니다. 따라서 `rawConfiguration` 은 항상 모든 위젯 구성의 상위 집합입니다.
  </Collapser>

  <Collapser id="widget-untyped" title="유형이 지정되지 않은 위젯">
    유형이 지정되지 않은 위젯은 [총알](#bullet) , [히스토그램](#histogram) , [인벤토리](#inventory) 등과 같은 다른 모든 위젯입니다.

    유형이 지정되지 않은 위젯의 경우 제공된 JSON의 모양을 알고 유효성을 검사합니다. 예를 들어 [히트맵](#heatmap) 에는 미리 알고 있는 특정 구성이 있으므로 유효성을 검사할 수 있습니다.

    입력으로:

    * `configuration` 이 경우에는 고려되지 않습니다.

    * `rawConfiguration` 제공되어야 합니다. 아래의 [예를](#untyped-widget-data) 참조하십시오.

    * `visualization` null일 수 없으며 `viz.heatmap` 또는 `viz.bullet` 과 같은 고정 문자열이어야 합니다.

      출력으로 API는 입력에 제공된 내용으로 정확하게 응답합니다.
  </Collapser>
</CollapserGroup>

## 입력된 위젯 정의 [#typed-widget-data]

### 지역

영역은 여러 nrql 쿼리를 지원합니다.

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="area-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="area-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="area-input" title="입력">
    ```graphql
    {
      widgets: {
        configuration: {
          area: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
            }
          }
        }
        rawConfiguration: null
      }
    }
    ```
  </Collapser>

  <Collapser id="area-output" title="산출">
    ```json
    {
      "visualization": "viz.area",
      "configuration": {
        "area": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
            }
          ]
        },
        "bar": null,
        "billboard": null,
        "line": null,
        "markdown": null,
        "pie": null,
        "table": null
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 술집

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="bar-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="bar-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="bar-input" title="입력">
    ```graphql
    {  
      widgets: {
        configuration: {
          bar: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction FACET appName"
            }
          }
        }
        rawConfiguration: null
      }  
    }
    ```
  </Collapser>

  <Collapser id="bar-output" title="산출">
    ```json
    {
      "visualization": "viz.bar",
      "configuration": {
        "area": null,
        "bar": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction FACET appName"
            }
          ]
        },
        "billboard": null,
        "line": null,
        "markdown": null,
        "pie": null,
        "table": null
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction FACET appName"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 광고 게시판

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="billboard-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="billboard-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>

    <tr>
      <td>
        `thresholds`

        *\[물체]*
      </td>

      <td>
        선택 과목. 쿼리 결과를 다른 그룹으로 분류하기 위한 임계값 배열:

        * `alertSeverity`: `NOT_ALERTING` , `WARNING` 및 `CRITICAL` 일 수 있습니다.
        * `value`: 경고 그룹의 임계값입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="billboard-input" title="입력">
    ```graphql
    {  
      widgets: {
        configuration: {
          billboard: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction FACET appName"
            }
            thresholds: {
              alertSeverity: CRITICAL
              value: 3
            }
          }
        }
        rawConfiguration: null
      }
    }
    ```
  </Collapser>

  <Collapser id="billboard-output" title="산출">
    ```json
    {
      "visualization": "viz.billboard",
      "configuration": {
        "area": null,
        "bar": null,
        "billboard": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction FACET appName"
            }
          ],
          "thresholds": [
            {
              "alertSeverity": "CRITICAL",
              "value": 3
            }
          ]
        },
        "line": null,
        "markdown": null,
        "pie": null,
        "table": null
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction FACET appName"
          }
        ],
        "thresholds": [
          {
            "alertSeverity": "CRITICAL",
            "value": 3
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 선

Line은 여러 nrql 쿼리를 지원합니다.

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="line-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="line-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="line-input" title="입력">
    ```graphql
    {
      widgets: {
        configuration: {
          line: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
            }
          }
        }
        rawConfiguration: null
      }
    }
    ```
  </Collapser>

  <Collapser id="line-output" title="산출">
    ```json
    {
      "visualization": "viz.line",
      "configuration": {
        "area": null,
        "bar": null,
        "billboard": null,
        "line": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
            }
          ]
        },
        "markdown": null,
        "pie": null,
        "table": null
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction FACET appName TIMESERIES"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 가격 인하

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="markdown-account_id">
      <td>
        `text`

        *문자열*
      </td>

      <td>
        마크다운 위젯에 추가할 콘텐츠입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="markdown-input" title="입력">
    ```graphql
    {
      widgets: {
        configuration: {
          markdown: {
            text: "My markdown"
          }
        }
        rawConfiguration: null
      }  
    }
    ```
  </Collapser>

  <Collapser id="markdown-output" title="산출">
    ```json
    {
      "visualization": "viz.markdown",
      "configuration": {
        "area": null,
        "bar": null,
        "billboard": null,
        "line": null,
        "markdown": {
          "text": "My markdown"
        },
        "pie": null,
        "table": null
      },
      "rawConfiguration": {
        "text": "My markdown"
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 파이

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="pie-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="pie-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="pie-input" title="입력">
    ```graphql
    {
      widgets: {
        configuration: {
          pie: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction FACET appName"
            }
          }
        }
        rawConfiguration: null
      }  
    }
    ```
  </Collapser>

  <Collapser id="pie-output" title="산출">
    ```json
    {
      "visualization": "viz.pie",
      "configuration": {
        "area": null,
        "bar": null,
        "billboard": null,
        "line": null,
        "markdown": null,
        "pie": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction FACET appName"
            }
          ]
        },
        "table": null
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction FACET appName"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 테이블

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="table-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="table-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="table-input" title="입력">
    ```graphql
    {
      widgets: {
        configuration: {
          table: {
            nrqlQueries: {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction"
            }
          }
        }
        rawConfiguration: null
      }
    }
    ```
  </Collapser>

  <Collapser id="table-output" title="산출">
    ```json
    {
      "visualization": "viz.table",
      "configuration": {
        "area": null,
        "bar": null,
        "billboard": null,
        "line": null,
        "markdown": null,
        "pie": null,
        "table": {
          "nrqlQueries": [
            {
              "accountId": 12345678,
              "query": "SELECT count(*) FROM Transaction"
            }
          ]
        }
      },
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

## 유형이 지정되지 않은 위젯 정의 [#untyped-widget-data]

### 면적(사전 정의된 측정항목) [#areap]

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `duration`

        *긴*
      </td>

      <td>
        요청된 시간 창의 기간(밀리초)입니다.

        * `endTime` 과 함께 제공되면 시간 창은 지정된 시간에 끝나는 마지막 `x` 밀리초로 설정됩니다.
        * `endTime` 이 null이면 시간 창은 지금 끝나는 마지막 `x` 밀리초로 설정됩니다.
      </td>
    </tr>

    <tr>
      <td>
        `endTime`

        *긴*
      </td>

      <td>
        선택 과목. 시간 창의 끝(밀리초)입니다.
      </td>
    </tr>

    <tr>
      <td>
        `entityIds`

        *\[정수]*
      </td>

      <td>
        데이터를 가져올 소스 에이전트 ID의 배열입니다.
      </td>
    </tr>

    <tr>
      <td>
        `metrics`

        *\[물체]*
      </td>

      <td>
        `type` `SCOPE_BREAKDOWN` 전용. 가져올 메트릭 목록입니다.
      </td>
    </tr>

    <tr id="linep-presentation">
      <td>
        `type`

        *문자열*
      </td>

      <td>
        사전 정의된 차트의 유형입니다. `APPLICATION_BREAKDOWN` , `BACKGROUND_BREAKDOWN` , `BROWSER_BREAKDOWN` , `GC_RUNS_BREAKDOWN` , `SCOPE_BREAKDOWN` , `SOLR_BREAKDOWN.`
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="linep-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.area"
        configuration: null
        rawConfiguration: {
          type:SCOPE_BREAKDOWN
          endTime: null
          entityIds: [1]
          metrics: [{
            name: "Mobile/Activity/Name/Display"
          }]
          duration: 3600
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="linep-output" title="산출">
    ```json
    {
      "visualization": "viz.area",
      "configuration": null,
      "rawConfiguration": {
        "type": "SCOPE_BREAKDOWN",
        "endTime": null,
        "entityIds": [
          1
        ],
        "metrics": [
          {
            "name": "Mobile/Activity/Name/Display"
          }
        ],
        "duration": 3600
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 총알

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="bullet-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="bullet-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>

    <tr>
      <td>
        `limit`

        *뜨다*
      </td>

      <td>
        모든 쿼리 결과가 비교되는 목표입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="bullet-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.bullet"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [
            {
              accountId: 12345678,
              query: "SELECT count(*) FROM Transaction"
            }
          ]
          limit: 2.4
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="bullet-output" title="산출">
    ```json
    {
      "visualization": "viz.bullet",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT count(*) FROM Transaction"
          }
        ],
        "limit": 2.4
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 이벤트 피드

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="event-feed-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="event-feed-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="event-feed-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.event-feed"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [      
            {
              accountId: 12345678,
              query: "SELECT * FROM InfrastructureEvent"
            }
          ]    
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="event-feed-output" title="산출">
    ```json
    {
      "visualization": "viz.event-feed",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT * FROM InfrastructureEvent"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 깔때기

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="funnel-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="funnel-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="funnel-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.funnel"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [      
            {
              accountId: 12345678,
              query: "SELECT funnel(SESSION,
                WHERE name ='Controller/about/main' AS 'Step 1',
                WHERE name ='Controller/about/careers' AS 'Step 2')
                FROM PageView"
            }
          ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="funnel-output" title="산출">
    ```json
    {
      "visualization": "viz.funnel",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT funnel(SESSION, \n WHERE name ='Controller/about/main' AS 'Step 1', \n WHERE name ='Controller/about/careers' AS 'Step 2') \n FROM PageView"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 히트맵

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="heatmap-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="heatmap-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="heatmap-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.heatmap"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [      
            {
              accountId: 12345678,
              query: "SELECT histogram(duration) FROM Transaction Facet appName"
            }
          ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="heatmap-output" title="산출">
    ```json
    {
      "visualization": "viz.heatmap",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT histogram(duration) FROM Transaction Facet appName"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 히스토그램

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="histogram-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="histogram-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="histogram-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.histogram"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [      
            {
              accountId: 12345678,
              query: "SELECT histogram(duration) FROM Transaction"
            }
          ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="histogram-output" title="산출">
    ```json
    {
      "visualization": "viz.histogram",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT histogram(duration) FROM Transaction"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 목록

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="inventory-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="inventory-filters">
      <td>
        `filters`

        *물체*
      </td>

      <td>
        인프라 소스에 적용할 필터의 맵입니다.
      </td>
    </tr>

    <tr id="inventory-presentation">
      <td>
        `sources`

        *\[끈]*
      </td>

      <td>
        데이터를 가져올 인프라 소스 목록입니다. 자세한 내용은 [인벤토리 항목이 어떻게 보이는지](/docs/infrastructure/infrastructure-ui-pages/infra-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) 에 대한 문서를 참조하세요.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="inventory-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "infra.inventory"
        configuration: null
        rawConfiguration: {
          accountId: 12345678,  
          filters: {
            providerAccountId: "1"
          }
          sources: [ "aws/elastocache/cluster", "aws/elasticache/node" ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="inventory-output" title="산출">
    ```json
    {
      "visualization": "infra.inventory",
      "configuration": null,
      "rawConfiguration": {
        "accountId": 12345678,
        "filters": {
          "providerAccountId": "1"
        },
        "sources": [
          "aws/elastocache/cluster",
          "aws/elasticache/node"
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### JSON

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="json-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr id="json-presentation">
      <td>
        `query`

        *문자열*
      </td>

      <td>
        위젯에 대한 데이터를 제공하는 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 입니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="json-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.json"
        configuration: null
        rawConfiguration: {
          nrqlQueries: [      
            {
              accountId: 12345678,
              query: "SELECT * FROM Transaction"
            }
          ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="json-output" title="산출">
    ```json
    {
      "visualization": "viz.json",
      "configuration": null,
      "rawConfiguration": {
        "nrqlQueries": [
          {
            "accountId": 12345678,
            "query": "SELECT * FROM Transaction"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 선(메트릭) [#linem]

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `compareWith`

        *물체*
      </td>

      <td>
        선택 과목. 쿼리할 추가 시간 창입니다. 예를 들어:

        ```graphql
        {
          offsetDuration: 86400000
          presentation: { "#c001", "Yesterday" }
        }
        ```
      </td>
    </tr>

    <tr id="linem-account_id">
      <td>
        `duration`

        *긴*
      </td>

      <td>
        요청된 시간 창의 기간(밀리초)입니다.

        * `endTime` 과 함께 제공되면 시간 창은 지정된 시간에 끝나는 마지막 `x` 밀리초로 설정됩니다.
        * `endTime` 이 null이면 시간 창은 지금 끝나는 마지막 `x` 밀리초로 설정됩니다.
      </td>
    </tr>

    <tr id="linem-presentation">
      <td>
        `endTime`

        *긴*
      </td>

      <td>
        시간 창의 끝(밀리초)입니다.
      </td>
    </tr>

    <tr>
      <td>
        `entityIds`

        *\[정수]*
      </td>

      <td>
        데이터를 가져올 소스 에이전트 ID의 배열입니다.
      </td>
    </tr>

    <tr>
      <td>
        `facet`

        *문자열*
      </td>

      <td>
        선택 과목. 주어진 속성으로 데이터를 패싯합니다. `host` , `agent` , `application` 또는 `mobile_version` 일 수 있습니다.
      </td>
    </tr>

    <tr>
      <td>
        `limit`

        *정수*
      </td>

      <td>
        선택 과목. 반환할 시리즈의 최대 양입니다.
      </td>
    </tr>

    <tr>
      <td>
        `metrics`

        *\[물체]*
      </td>

      <td>
        가져올 메트릭 목록입니다.
      </td>
    </tr>

    <tr>
      <td>
        `orderBy`

        *문자열*
      </td>

      <td>
        선택 과목. 결과를 내림차순으로 정렬하는 데 사용됩니다.
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="linem-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "viz.line"
        configuration: null
        rawConfiguration: {
          accountId: 12345678,      
          duration: 36000
          endTime: null
          entityIds: [ 100 ]
          facet: null
          limit: null
          metrics: [{
            name: "Errors/all"
            values: [ "error_rate" ]
          }]
          orderBy: null
          compareWith: []
          rawMetricName: "Errors/all"
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="linem-output" title="산출">
    ```json
    {
      "visualization": "viz.line",
      "configuration": null,
      "rawConfiguration": {
        "accountId": 12345678,
        "duration": 36000,
        "endTime": null,
        "entityIds": [
          100
        ],
        "facet": null,
        "limit": null,
        "metrics": [
          {
            "name": "Errors/all",
            "values": [
              "error_rate"
            ]
          }
        ],
        "orderBy": null,
        "compareWith": [],
        "rawMetricName": "Errors/all"
      }
    }
    ```
  </Collapser>
</CollapserGroup>

### 서비스 맵

<table>
  <thead>
    <tr>
      <th width="25%">
        위젯 데이터 요소
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr id="service-map-account_id">
      <td>
        `accountId`

        *정수*
      </td>

      <td>
        데이터를 가져올 소스 계정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `additionalEntities`

        *\[물체]*
      </td>

      <td>
        선택 과목. 포함할 추가 엔터티의 배열입니다. 대상은 엔터티 다운스트림이고 소스는 업스트림입니다. `Type` , `vendor` , `target` 및 `source` 은 선택사항입니다.
      </td>
    </tr>

    <tr>
      <td>
        `deemphasizedConditions`

        *\[물체]*
      </td>

      <td>
        선택 과목. 이러한 조건을 가진 엔티티는 지도에서 희미하게 보입니다.
      </td>
    </tr>

    <tr id="service-map-account_id">
      <td>
        `hiddenEntities`

        *\[물체]*
      </td>

      <td>
        선택 과목. 지도에서 제외할 엔티티입니다.
      </td>
    </tr>

    <tr id="service-map-presentation">
      <td>
        `primaryEntities`

        *\[물체]*
      </td>

      <td>
        지도를 시작하는 기본 엔터티(가장 업스트림).
      </td>
    </tr>
  </tbody>
</table>

<CollapserGroup>
  <Collapser id="service-map-input" title="입력">
    ```graphql
    {
      widgets: {
        visualization: "topology.service-map"
        configuration: null
        rawConfiguration: {
          accountId: 12345678,       
          entitySearchQuery: "alertStatus IN ('RED')"
          additionalEntities: [{
            guid: "AAA"
            type: "APPLICATION"
            vendor: "Company inc."
            sourceGuid: "BBB"
            targetGuid: "CCC"
          }]
          deemphasizedConditions: {
            alertStatus: [ GREEN ]
            entityType: [ APM_APPLICATION_ENTITY ]
          }
          hiddenEntities: [{
            guid: "XXX_2"
            sourceGuid: "YYY_2"
            targetGuid: "ZZZ_2"
          }]
          primaryEntities: [{
            guid: "AAA"
          }]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="service-map-output" title="산출">
    ```json
    {
      "visualization": "topology.service-map",
      "configuration": null,
      "rawConfiguration": {
        "accountId": 12345678,
        "entitySearchQuery": "alertStatus IN ('RED')",
        "additionalEntities": [
          {
            "guid": "AAA",
            "type": "APPLICATION",
            "vendor": "Company inc.",
            "sourceGuid": "BBB",
            "targetGuid": "CCC"
          }
        ],
        "deemphasizedConditions": {
          "alertStatus": [
            "GREEN"
          ],
          "entityType": [
            "APM_APPLICATION_ENTITY"
          ]
        },
        "hiddenEntities": [
          {
            "guid": "XXX_2",
            "sourceGuid": "YYY_2",
            "targetGuid": "ZZZ_2"
          }
        ],
        "primaryEntities": [
          {
            "guid": "AAA"
          }
        ]
      }
    }
    ```
  </Collapser>
</CollapserGroup>