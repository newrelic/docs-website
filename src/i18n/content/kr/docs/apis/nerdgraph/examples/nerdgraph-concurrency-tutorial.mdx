...
New Relic의 NerdGraph API는 **사용자당 동시 요청 수를 25개로** 제한합니다. 다음 코드 예제는 이러한 제한을 피하거나 HTTP `429` 상태 코드를 수신하기 위한 시작점으로 사용할 수 있습니다. 아래의 두 예는 많은 New Relic 계정에 대해 병렬로 쿼리를 실행하는 동일한 사용 사례를 따르지만 여전히 NerdGraph 동시성 제한에 속합니다.
