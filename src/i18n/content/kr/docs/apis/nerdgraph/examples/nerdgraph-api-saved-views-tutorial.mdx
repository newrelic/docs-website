...
[NerdGraph를](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) 사용하여 개인 또는 공유 저장된 보기를 보고, 만들고, 업데이트하고, 삭제할 수 있습니다.
...
* `filter`: 필터 필드를 사용하면 아래 매개변수를 필터 결과에 전달할 수 있습니다.

  * `capabilities`: 필터 필드 `capabilities` 는 텍스트로 필터링하고 저장된 보기가 속한 기능을 반환하는 선택적 문자열 배열입니다. 예를 들어 `infrastructure.hosts` 제공하면 반환된 저장된 보기가 "infrastructure.hosts" 페이지에 속하는 저장된 보기로 필터링됩니다.
