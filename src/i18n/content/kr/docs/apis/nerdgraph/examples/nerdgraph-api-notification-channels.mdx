...
경고 알림 채널을 생성하려면 생성하려는 특정 유형의 알림 채널(예: 이메일, Slack 등)과 이를 구성하는 데 필요한 세부정보(채널에 따라 다름)를 알아야 합니다. 유형). 알림 채널이 생성되면 하나 이상의 [경고 정책](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/ "경고 및 응용 인텔리전스 | 새로운 유물 문서") 과 연결할 수 있습니다. 연결되면 해당 채널은 조건을 위반할 때 해당 정책의 알림을 받습니다.
...
경고 알림 채널을 만드는 것만으로는 충분하지 않습니다. 채널을 만든 후에는 하나 이상의 [정책](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/ "경고 및 응용 인텔리전스 | 새로운 유물 문서") 에 연결해야 합니다. 정책에 연결되면 해당 정책의 조건이 위반될 때 채널에서 경고 알림을 받을 수 있습니다.
...
알림 채널이 그 유용성보다 오래 지속된 경우(예: 폐기된 이메일 목록), 알림 알림을 보내는 [정책](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/ "경고 및 응용 인텔리전스 | 새로운 유물 문서") (들)에서 해당 채널을 분리할 때가 되었습니다. 이 API 호출은 채널 자체를 그대로 유지하지만 지정된 정책에서 제거합니다.
