...
뉴렐릭을 사용하면 [워크로드](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)) 라는 그룹으로 함께 그룹화할 수 있습니다. 이를 통해 팀이나 프로젝트에서 사용하는 전체 스택을 더욱 효과적으로 모니터링할 수 있습니다.
...
[워크로드에서 표시되는 차트를 사용자 지정하는 방법에 대한 게시물](https://support.newrelic.com/s/hubtopic/aAX8W0000008c8P/how-to-customize-the-charts-you-see-on-a-workload)도 참조하십시오.
...
* `account`[워크로드 계정 ID입니다](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#workload-account). 값은 계정 간에 이동할 수 없으므로 나중에 이 값을 변경할 수 없습니다.

* `name`: 워크로드에 대해 사용자에게 친숙한 이름이 있는 문자열입니다.

* `scopeAccounts`[스코프 계정은](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#scope-accounts) 엔티티 데이터를 가져오는 계정입니다. 스코프 계정은 워크로드 계정과 동일한 상위 계정 또는 엔터프라이즈 파트너십 아래의 그룹에 속해야 합니다.

* 워크로드의 엔터티를 정의하려면 다음 옵션 중 하나 또는 둘 다를 사용할 수 있습니다.

  * `entitySearchQueries`: 이를 통해 [제외의 제외를 동적으로 생성](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#dynamic) 할 수 있습니다. 각 쿼리에 이름을 지정할 필요는 없습니다. 다음은 동적 쿼리의 예입니다.
