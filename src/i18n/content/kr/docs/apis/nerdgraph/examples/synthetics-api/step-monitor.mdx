...
<DNT>`syntheticsCreateStepMonitor`</DNT> 뮤테이션을 사용하여 스텝 모니터를 생성할 수 있습니다. 이 변형을 사용하면 미리 정의된 일련의 작업을 통해 다단계 브라우저 모니터링을 설정할 수 있습니다.
...
<DNT>`syntheticsUpdateStepMonitor`</DNT> 뮤테이션을 사용하여 기존 스텝 모니터를 업데이트할 수 있습니다. 이 기능을 사용하면 이미 생성된 단계 모니터의 설정을 수정할 수 있습니다.
...
단계 모니터가 더 이상 필요하지 않으면 <DNT>`syntheticsDeleteMonitor`</DNT> 변이를 사용하여 영구적으로 제거할 수 있습니다.

모니터를 삭제하려면 [&apos;합성 모니터 삭제](/docs/apis/nerdgraph/examples/synthetics-api/ping-monitor/#delete-monitor) &apos; 섹션을 참조하십시오.

## 스텝 모니터 쿼리 [#query-step-monitors]

NerdGraph를 사용하여 메타데이터, 단계 설정, 맵 ID를 조회하거나 단계 모니터의 상태를 확인하십시오.
...
* [모니터 단계 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitor-steps) - 단계 모니터에서 단계 설정을 조회합니다.
