...
[NerdGraph API](/docs/introduction-new-relic-graphql) 를 사용하여 [NRQL 쿼리](/docs/insights/nrql-new-relic-query-language/using-nrql/introduction-nrql) 를 만들 수 있습니다.

## NerdGraph를 사용한 쿼리 개요 [#overview]

New Relic 플랫폼 [쿼리 빌더](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) 또는 NerdGraph를 사용하여 NRQL 쿼리를 실행할 수 있습니다. NerdGraph는 [여러 계정](#cross-account-query) 에 대한 [쿼리 및 비동기 쿼리](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial) 실행과 같이 UI에서 수행할 수 없는 몇 가지 작업을 수행할 수 있는 기능을 제공합니다.
...
* 모든 NRQL 쿼리에는 [NRQL 쿼리 제한](/docs/query-your-data/nrql-new-relic-query-language/get-started/rate-limits-nrql-queries) 이 적용됩니다.

## NerdGraph를 사용한 기본 NRQL 쿼리 [#basic-queries]

NerdGraph를 사용하여 기본 NRQL 쿼리를 만들려면:

1. [NerdGraph 탐색기](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) 로 이동합니다.
...
{ 
   actor {
      nrql(accounts: [<var>ACCOUNT_ID_1</var>, <var>ACCOUNT_ID_2</var>, <var>ACCOUNT_ID_3</var>], 
      options: {}, 
      query: "<var>NRQL_QUERY</var>", 
      timeout: 70) 
         {   results}  
   }  
}
