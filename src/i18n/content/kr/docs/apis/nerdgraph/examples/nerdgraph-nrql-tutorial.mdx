...
[쿼리 작성기](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) 또는 NerdGraph를 사용하여 NRQL 쿼리를 실행할 수 있습니다. NerdGraph는 [여러 계정](#cross-account-query) 에 대한 쿼리 및 [비동기 쿼리](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial) 실행과 같이 UI에서 할 수 없는 몇 가지 작업을 수행할 수 있는 기능을 제공합니다.
