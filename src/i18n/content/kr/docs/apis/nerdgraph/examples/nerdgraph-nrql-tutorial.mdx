...
원시 데이터를 반환하는 것 외에도 애플리케이션에서 사용할 데이터에 대한 포함 가능한 차트 링크를 가져올 수 있습니다. 예를 들어, [소니](/docs/insights/insights-data-sources/default-data/apm-default-event-attributes-insights#transaction-event) 의 단일 개수 대신 시간 경과에 따른 버킷 개수의 시계열을 보여주는 [차트를](/docs/insights/use-insights-ui/manage-dashboards/chart-types#widget-types) 생성할 수 있습니다. `embeddedChartUrl` 사용하여 쿼리에 `TIMESERIES` 추가합니다.
