---
title: 간단한 브라우저 모니터링을 관리하세요
tags:
  - Synthetics
  - APIs
  - NerdGraph
  - Examples
metaDescription: How to use New Relic NerdGraph API to create and update simple browser monitors for synthetic monitoring.
freshnessValidatedDate: never
translationType: machine
---

뉴렐릭을 사용하면 NerdGraph를 사용하여 [간단한 브라우저 모니터를](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors) 만들 수 있습니다. 간단한 브라우저 모니터는 웹 구성에 대한 자동화된 브라우저 기반 검사를 수행하고 전체 브라우저 환경에서 페이지를 로드하여 실제 사용자 인스턴스 합성을 시뮬레이션합니다. 이 튜토리얼에서는 NerdGraph API를 사용하여 간단한 브라우저 모니터링 도구를 자동화하여 생성하는 방법에 대한 예제를 제공합니다.

## 간단한 브라우저 모니터 만들기 [#create-simple-browser]

<DNT>`syntheticsCreateSimpleBrowserMonitor`</DNT> 뮤테이션을 사용하면 간단한 브라우저 모니터를 만들 수 있습니다.

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `accountId`
      </td>

      <td>
        정수
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 생성될 뉴렐릭 

        [계정 ID입니다](/docs/accounts/accounts-billing/account-structure/account-id)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.browsers`
      </td>

      <td>
        정렬
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 작업을 실행하는 데 사용할 브라우저입니다. 지원되는 브라우저: 

        `CHROME`

        , 

        `FIREFOX`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.devices`
      </td>

      <td>
        정렬
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 작업을 실행하는 데 사용할 장치입니다. 지원되는 기기: 

        `DESKTOP`

        , 

        `MOBILE_LANDSCAPE`

        , 

        `MOBILE_PORTRAIT`

        , 

        `TABLET_LANDSCAPE`

        , 

        `TABLET_PORTRAIT`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.public`
      </td>

      <td>
        정렬
      </td>

      <td>
        네
      </td>

      <td>
        모니터가 검사를 실행하는 

        [표시 위치](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors/#setting-location)

         제외(예: 

        `["US_EAST_1", "US_WEST_1"]`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.name`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        간단한 브라우저 모니터의 표시 이름입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.period`
      </td>

      <td>
        열거형
      </td>

      <td>
        네
      </td>

      <td>
        모니터링이 실행되는 빈도. 옵션: 

        `EVERY_MINUTE`

        , 

        `EVERY_5_MINUTES`

        , 

        `EVERY_10_MINUTES`

        , 

        `EVERY_15_MINUTES`

        , 

        `EVERY_30_MINUTES`

        , 

        `EVERY_HOUR`

        , 

        `EVERY_6_HOURS`

        , 

        `EVERY_12_HOURS`

        , 

        `EVERY_DAY`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용하는 런타임 유형입니다. 

        `CHROME_BROWSER`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용하는 런타임 유형 버전입니다. 최신 Chrome 버전을 자동으로 사용하려면 

        `LATEST`

         을(를) 사용하세요(권장). 

        `134`

         또는 

        `140`

         와 같은 이전 특정 버전도 허용되지만 지원이 중단될 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터에서 사용되는 언어입니다. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.status`
      </td>

      <td>
        열거형
      </td>

      <td>
        네
      </td>

      <td>
        모니터 상태. 옵션: 

        `ENABLED`

         (모니터가 활성화되어 점검을 수행 중), 

        `DISABLED`

         (모니터가 비활성화됨).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.uri`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        모니터링할 URL(예: 

        `https://example.com`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.customHeaders`
      </td>

      <td>
        물체
      </td>

      <td>
        아니요
      </td>

      <td>
        요청에 포함할 사용자 지정 HTTP 헤더입니다. 각 헤더에는 

        `name`

         및 

        `value`

         속성이 있습니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.enableScreenshotOnFailureAndScript`
      </td>

      <td>
        부울
      </td>

      <td>
        아니요
      </td>

      <td>
        작업 실행 중 오류가 발생하거나 스크립트가 실행될 때 스크린샷을 캡처합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.responseValidationText`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        검증이 통과되려면 응답에 반드시 포함되어야 하는 텍스트입니다. 지정된 경우, 응답 본문에서 해당 텍스트를 찾을 수 없으면 모니터링이 실패합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.useTlsValidation`
      </td>

      <td>
        부울
      </td>

      <td>
        아니요
      </td>

      <td>
        TLS/SSL 인증서를 검증할지 여부. SSL 인증서 체인의 유효성을 확인하려면 

        `true`

         으로 설정하십시오.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.apdexTarget`
      </td>

      <td>
        뜨다
      </td>

      <td>
        아니요
      </td>

      <td>
        SLA 보고서를 채우는 데 사용되는 모니터의 Apdex 목표(초 단위)입니다. 기본값은 7.0초입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

```graphql
mutation {
  syntheticsCreateSimpleBrowserMonitor(
    accountId: ACCOUNT_ID
    monitor: {
      browsers: [BROWSERS]
      devices: [DEVICES]
      locations: { public: ["LOCATION_1", "LOCATION_2"] }
      name: "YOUR_MONITOR_NAME"
      period: PERIOD
      runtime: {
        runtimeType: "RUNTIME_TYPE"
        runtimeTypeVersion: "RUNTIME_TYPE_VERSION"
        scriptLanguage: "SCRIPT_LANGUAGE"
      }
      status: STATUS
      uri: "MONITORED_URI"
      advancedOptions: {
        customHeaders: { name: "HEADER_NAME", value: "HEADER_VALUE" }
        enableScreenshotOnFailureAndScript: ENABLE_SCREENSHOT
        responseValidationText: "VALIDATION_TEXT"
        useTlsValidation: TLS_VALIDATION
      }
      apdexTarget: APDEX_TARGET
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsCreateSimpleBrowserMonitor": {
      "errors": null
    }
  }
}
```

모니터를 생성하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 간단한 브라우저 모니터 업데이트 [#update-simple-browser]

<DNT>`syntheticsUpdateSimpleBrowserMonitor`</DNT> 뮤테이션을 사용하면 기존의 간단한 브라우저 모니터를 업데이트할 수 있습니다. 이 기능을 사용하면 이미 생성된 간단한 브라우저 모니터의 설정을 수정할 수 있습니다.

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        업데이트하려는 모니터의 고유 엔티티 

        [GUID입니다](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.browsers`
      </td>

      <td>
        정렬
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터가 작업을 실행하는 데 사용할 브라우저입니다. 지원되는 브라우저: 

        `CHROME`

        , 

        `FIREFOX`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.devices`
      </td>

      <td>
        정렬
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터가 작업을 실행하는 데 사용할 장치입니다. 지원되는 기기: 

        `DESKTOP`

        , 

        `MOBILE_LANDSCAPE`

        , 

        `MOBILE_PORTRAIT`

        , 

        `TABLET_LANDSCAPE`

        , 

        `TABLET_PORTRAIT`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.locations.public`
      </td>

      <td>
        정렬
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터가 검사를 실행하는 

        [표시 위치](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors/#setting-location)

         제외(예: 

        `["US_EAST_1", "US_WEST_1"]`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.name`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        간단한 브라우저 모니터의 업데이트된 표시 이름입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.period`
      </td>

      <td>
        열거형
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터링이 실행되는 빈도. 옵션: 

        `EVERY_MINUTE`

        , 

        `EVERY_5_MINUTES`

        , 

        `EVERY_10_MINUTES`

        , 

        `EVERY_15_MINUTES`

        , 

        `EVERY_30_MINUTES`

        , 

        `EVERY_HOUR`

        , 

        `EVERY_6_HOURS`

        , 

        `EVERY_12_HOURS`

        , 

        `EVERY_DAY`

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용하는 런타임 유형입니다. 

        `CHROME_BROWSER`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용하는 런타임 유형 버전입니다. 최신 Chrome 버전을 자동으로 사용하려면 

        `LATEST`

         을(를) 사용하세요(권장). 

        `134`

         또는 

        `140`

         와 같은 이전 특정 버전도 허용되지만 지원이 중단될 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터에서 사용되는 언어입니다. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.status`
      </td>

      <td>
        열거형
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터 상태. 옵션: 

        `ENABLED`

         (모니터가 활성화되어 점검을 수행 중), 

        `DISABLED`

         (모니터가 비활성화됨).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.uri`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        모니터링할 URL(예: 

        `https://example.com`

        ).
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.customHeaders`
      </td>

      <td>
        물체
      </td>

      <td>
        아니요
      </td>

      <td>
        요청에 포함할 사용자 지정 HTTP 헤더입니다. 각 헤더에는 

        `name`

         및 

        `value`

         속성이 있습니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.enableScreenshotOnFailureAndScript`
      </td>

      <td>
        부울
      </td>

      <td>
        아니요
      </td>

      <td>
        작업 실행 중 오류가 발생하거나 스크립트가 실행될 때 스크린샷을 캡처합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.responseValidationText`
      </td>

      <td>
        문자열
      </td>

      <td>
        아니요
      </td>

      <td>
        검증이 통과되려면 응답에 반드시 포함되어야 하는 텍스트입니다. 지정된 경우, 응답 본문에서 해당 텍스트를 찾을 수 없으면 모니터링이 실패합니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.advancedOptions.useTlsValidation`
      </td>

      <td>
        부울
      </td>

      <td>
        아니요
      </td>

      <td>
        TLS/SSL 인증서를 검증할지 여부. SSL 인증서 체인의 유효성을 확인하려면 

        `true`

         으로 설정하십시오.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.apdexTarget`
      </td>

      <td>
        뜨다
      </td>

      <td>
        아니요
      </td>

      <td>
        SLA 보고서를 채우는 데 사용되는 모니터의 Apdex 목표(초 단위)입니다. 기본값은 7.0초입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

```graphql
mutation {
  syntheticsUpdateSimpleBrowserMonitor(
    guid: ENTITY_GUID
    monitor: {
      browsers: [BROWSERS]
      devices: [DEVICES]
      locations: { public: ["LOCATION_1", "LOCATION_2"] }
      name: "YOUR_MONITOR_NAME"
      period: PERIOD
      runtime: {
        runtimeType: "RUNTIME_TYPE"
        runtimeTypeVersion: "RUNTIME_TYPE_VERSION"
        scriptLanguage: "SCRIPT_LANGUAGE"
      }
      status: STATUS
      uri: "MONITORED_URI"
      advancedOptions: {
        customHeaders: { name: "HEADER_NAME", value: "HEADER_VALUE" }
        enableScreenshotOnFailureAndScript: ENABLE_SCREENSHOT
        responseValidationText: "VALIDATION_TEXT"
        useTlsValidation: TLS_VALIDATION
      }
      apdexTarget: APDEX_TARGET
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsUpdateSimpleBrowserMonitor": {
      "errors": null
    }
  }
}
```

모니터를 업데이트하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 간단한 브라우저 모니터의 런타임을 업그레이드하세요 [#upgrade-monitor-runtime]

새로운 Chrome 런타임을 사용하도록 단순 브라우저 모니터를 업그레이드할 수 있습니다. 모니터가 최신 Chrome 릴리스에 맞춰 자동으로 최신 상태를 유지하도록 `LATEST` 버전을 사용하는 것을 권장합니다.

<Callout variant="important">
  **Chrome 버전 관리**: 뉴렐릭은 Chrome 런타임을 위한 `LATEST` 버전 모델로 전환하고 있습니다. 특정 Chrome 버전(예: 134 및 140)은 수명이 종료되며 앞으로는 `LATEST` 만 유지 관리됩니다. **2026년 8월 18일**에 여전히 이전 Chrome 버전을 사용하는 모니터는 최신 안정화 버전(Chrome 147 이상)으로 강제 업그레이드됩니다. Chrome이 새 버전을 출시하면, 1주일 이내에 단순 브라우저 모니터에서 `LATEST` (으)로 사용할 수 있습니다. 모니터가 최신 Chrome 업데이트 및 기능을 자동으로 수신하도록 하려면 이 날짜 이전에 `runtimeTypeVersion: "LATEST"` (으)로 업그레이드하세요.
</Callout>

### 입력스피커, 변수

<table>
  <thead>
    <tr>
      <th>
        매개변수
      </th>

      <th>
        데이터 형식
      </th>

      <th>
        꼭 필요한가요?
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `guid`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        업그레이드하려는 모니터의 고유 엔티티 

        [GUID입니다](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors)

        .
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeType`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        런타임 유형입니다. 

        `CHROME_BROWSER`

         만 허용되는 값입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.runtimeTypeVersion`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        런타임 버전. 가장 최신 Chrome 버전을 자동으로 사용하려면 

        `LATEST`

         을(를) 사용하십시오(권장). 

        `134`

         또는 

        `140`

         와 같은 이전 특정 버전도 허용되지만 지원이 중단될 예정입니다.
      </td>
    </tr>

    <tr>
      <td>
        `monitor.runtime.scriptLanguage`
      </td>

      <td>
        문자열
      </td>

      <td>
        네
      </td>

      <td>
        언어를 펼쳐보세요. 

        `JAVASCRIPT`

         만 허용되는 값입니다.
      </td>
    </tr>
  </tbody>
</table>

### 샘플 요청

최신 Chrome 버전으로 업그레이드(권장):

```graphql
mutation {
  syntheticsUpdateSimpleBrowserMonitor(
    guid: "ENTITY_GUID"
    monitor: {
      runtime: {
        runtimeType: "CHROME_BROWSER"
        runtimeTypeVersion: "LATEST"
        scriptLanguage: "JAVASCRIPT"
      }
    }
  ) {
    errors {
      description
      type
    }
  }
}
```

### 샘플 응답

성공적인 응답은 오류 발생 시 `null` 반환합니다.

```json
{
  "data": {
    "syntheticsUpdateSimpleBrowserMonitor": {
      "errors": null
    }
  }
}
```

모니터 런타임을 업그레이드하는 데 문제가 있는 경우 `errors` 에는 오류가 발생한 이유를 설명하는 `description` 및 `type` 필드가 있는 객체가 포함됩니다.

## 간단한 브라우저 모니터를 삭제하세요 [#delete-monitor]

간단한 브라우저 모니터가 더 이상 필요하지 않은 경우 <DNT>`syntheticsDeleteMonitor`</DNT> 변이를 사용하여 영구적으로 제거할 수 있습니다.

모니터를 삭제하려면 [&apos;합성 모니터 삭제](/docs/apis/nerdgraph/examples/synthetics-api/ping-monitor/#delete-monitor) &apos; 섹션을 참조하십시오.

## 단순 브라우저 모니터 쿼리 [#query-simple-browser-monitors]

단순 브라우저 모니터의 메타데이터를 조회하거나, ID를 매핑하거나, 상태를 확인하려면 NerdGraph를 사용하세요.

자세한 내용은 다음을 참조하세요:

* [모든 모니터 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitors): 모든 신세틱 모니터 및 해당 설정을 나열합니다.
* [모니터 ID를 엔티티 GUID에 매핑](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-guid-mapping): 구형 모니터 ID를 엔티티 GUID로 마이그레이션합니다.
* [런타임 업그레이드 상태](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-runtime-upgrade-all): 모니터가 최신 런타임 업그레이드 준비가 되었는지 확인합니다.

전체 쿼리 예제 목록을 보려면 [신세틱스 데이터 쿼리](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data) 문서를 참조하세요.