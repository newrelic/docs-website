...
<CollapserGroup>
  ...
  >
    다양한 엔터티 유형에는 이와 관련된 특정 데이터가 있습니다. 이 NerdGraph 쿼리 예제는 엔터티 유형에 관계없이 모든 엔터티의 이름과 APM의 오류율을 요청합니다. <InlinePopover type="browser"/>, 그리고 <InlinePopover type="mobile"/>엔터티.
