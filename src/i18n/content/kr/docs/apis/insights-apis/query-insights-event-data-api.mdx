...
1. [one.newrelic.com/api-keys](https://one.newrelic.com/api-keys) 로 이동하세요. EU 데이터 센터를 사용하는 경우 [one.eu.newrelic.com/api-keys](https://one.eu.newrelic.com/api-keys) 로 이동하세요. 대신에.
2. API 키 페이지의 오른쪽 상단에 있는 **Create a key button** \[키 만들기 버튼을] 클릭합니다.
3. 새 키를 구성합니다.

* 제공된 필드에 새 키의 이름을 입력합니다.
* 설명(예: "쿼리 키")을 추가합니다.

4. **Save** \[저장을] 클릭하여 새 키를 생성합니다.
