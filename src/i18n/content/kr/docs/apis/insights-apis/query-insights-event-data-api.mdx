...
<Callout variant="tip">
  <DNT>**This API is no longer the preferred way to query New Relic data.**</DNT> 그렇게 하려면 [NerdGraph를](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/) 사용하세요.
</Callout>

## 요구 사항 및 권장 사항 [#requirements]

<DNT>**This API is no longer the preferred way to query New Relic data.**</DNT> 최상의 결과를 얻으려면 [NerdGraph를](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) 사용하여 데이터를 쿼리해야 합니다.
...
1. [one.newrelic.com/api-keys](https://one.newrelic.com/api-keys) 로 이동하세요. EU 데이터센터를 사용하는 경우 [one.eu.newrelic.com/api-keys](https://one.eu.newrelic.com/api-keys) 로 이동하세요. 대신에.

2. API 키 페이지 오른쪽 상단에 있는 <DNT>**Create a key button**</DNT> 클릭합니다.
...
4. 새 키를 생성하려면 <DNT>**Save**</DNT> 클릭하세요.

## 2. API 쿼리 요청 생성 [#create-request]

[쿼리 키를 생성하거나 편집할](#register) 때 템플릿으로 사용할 수 있는 `curl` 쿼리 예시가 표시됩니다. 다음 쿼리 규칙을 따르지 않으면 예제 쿼리가 작동하지 않습니다.
...
* URL에는 유효한 [계정 ID가](/docs/accounts-partnerships/accounts/account-setup/account-id) 포함되어야 합니다.
...
<DNT>**Powershell**</DNT> 사용하여 API를 통해 이벤트를 쿼리할 수 있습니다.
