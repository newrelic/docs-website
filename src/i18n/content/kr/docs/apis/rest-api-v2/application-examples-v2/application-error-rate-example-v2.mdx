...
이것은 New Relic Data API(v2)를 사용하여 특정 기간 동안 애플리케이션의 평균 오류율을 얻는 방법의 예입니다. 이 값은 [APM **요약** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 의 [오류율 차트](#avg-error-image) 위에 백분율로 표시됩니다.
...
평균 백분율은 앱 [**요약** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 의 **오류율** 차트 위에 표시됩니다. New Relic은 다음 공식을 사용하여 계산합니다.

```
Application Error Rate = 100 * Errors/all:error_count / (HttpDispatcher:call_count + OtherTransaction/all:call_count)
```

다음은 앱의 APM 요약 페이지에 표시되는 **오류율** 차트의 예입니다.

<img
  title="APM Overview page: Error rate percentage"
  alt="APM Overview page: Error rate percentage"
  src={cropApmOverviewError}
/>

<figcaption>
  **APM > (선택한 앱) > Monitoring > Summary:** New Relic REST API v2를 사용하여 앱의 **APM 요약** 페이지에 나타나는 평균 오류율 백분율을 반환할 수 있습니다.
</figcaption>
