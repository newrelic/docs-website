...
* $&#x7B;APPID} 및 $&#x7B;APIKEY} 를 계정의 [애플리케이션 ID](/docs/apm/apis/requirements/identification-code) 및 [API 키로](/docs/apis/rest-api-v2/requirements/rest-api-key) 대체해야 합니다.
...
단일 호스트의 평균 처리량을 찾으려면 지정된 기간 동안 단일 명령을 사용하여 `HttpDispatche` : `:requests_per_minute` 메트릭을 가져오고 숫자 $&#x7B;HOST}를 지정합니다. 호스트를 식별하려면 [UI 또는 REST API](/docs/apis/rest-api-v2/requirements/listing-host-instance-application-server-ids#locating_host_id) 를 사용하십시오.
...
단일 호스트에서 웹이 아닌 앱에 대한 평균 처리량을 얻으려면 지정된 기간 동안 단일 명령을 사용하여 메트릭 `OtherTransaction:requests_per_minute` 을 얻고 숫자 $&#x7B;HOST}를 지정합니다. 호스트를 식별하려면 [UI 또는 REST API](/docs/apis/rest-api-v2/requirements/listing-host-instance-application-server-ids#locating_host_id) 를 사용하십시오.
