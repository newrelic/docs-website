...
애플리케이션의 [**요약** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 에 있는 **웹 트랜잭션 시간** 차트에 표시되는 [메트릭 타임슬라이스 데이터](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) 는 웹 앱의 구성에 따라 다릅니다. 가능한 구성 요소는 다음과 같습니다.
...
* 이 예의 `${APPID}` 및 `${APIKEY}` 변수를 특정 [애플리케이션 ID](/docs/apm/apis/requirements/identification-code) 및 [API 키로](/docs/apis/rest-api-v2/requirements/rest-api-key#viewing) 바꿔야 합니다.
...
2015년 2월 현재 New Relic 에이전트는 데이터베이스 시간을 특정 데이터베이스 유형으로 구분하여 표시합니다. 특정 에이전트 버전 요구 사항은 [데이터베이스 및 느린 쿼리 기능 가용성](/docs/apm/applications-menu/monitoring/databases-slow-queries-page#availablity) 을 참조하세요.
...
**웹 트랜잭션 시간** 차트에 일반 **데이터베이스** 범주가 표시되면 이러한 계산을 사용하지 마십시오. 대신 [데이터베이스](#database) 계산을 사용하십시오.
...
## (Ruby) 데이터베이스 총 시간 [#ruby_database]
