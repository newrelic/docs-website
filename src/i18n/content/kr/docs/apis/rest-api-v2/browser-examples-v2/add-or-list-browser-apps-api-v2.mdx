...
뉴렐릭에 앱을 추가하려면 다음 명령에서 `$API_KEY` 을 [뉴렐릭 API 키로](/docs/apis/rest-api-v2/requirements/api-keys#rest-api-key) 바꾸고, `${STRING}` 을 앱 이름으로 바꿉니다. API 탐색기에서 동일한 작업을 수행하려면 API 키를 사용하고 <DNT>**[api.newrelic.com/docs](https://api.newrelic.com/docs) &gt; Browser applications &gt; POST create**</DNT> 로 이동하세요.
...
브라우저-모니터 앱 목록을 보려면 다음 명령에서 `$API_KEY` 을 [뉴렐릭 API 키로](/docs/apis/rest-api-v2/requirements/api-keys#rest-api-key) 바꾸세요. API 탐색기에서 동일한 작업을 수행하려면 API 키를 사용하고 <DNT>**[api.newrelic.com/docs](https://api.newrelic.com/docs) &gt; Browser Applications &gt; GET List**</DNT> 로 이동하세요.
