...
1. [API 탐색기](https://api.newrelic.com/docs)로 이동한 다음, <DNT>**Select an account**</DNT> 드롭다운 메뉴에서 계정 이름을 선택합니다.
2. [api.newrelic.com/docs](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json)에서 API Explorer의 <DNT>**Application host**</DNT> 페이지로 이동합니다.
...
<CollapserGroup>
  <Collapser id="host_id" title="REST API Explorer를 사용한 호스트 ID 및 요약 데이터">
    ...
    1. [API Explorer](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json)로 이동한 다음, <DNT>**Select an account**</DNT> 드롭다운 메뉴에서 계정 이름을 선택합니다.
...
인스턴스 ID의 의미는 사용 중인 뉴렐릭 언어 에이전트에 따라 다릅니다. REST API에서 이 ID를 조회할 수 있습니다.
...
`INSTANCE_ID`을(를) 사용하여 인스턴스에 대한 요약 메트릭뿐만 아니라 특정 지표 타임슬라이스 값을 조회할 수 있습니다. 사용 가능한 메트릭에 대한 자세한 내용은 [REST API Explorer 애플리케이션 인스턴스](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__instances__instance_id__metrics_json) 페이지를 사용하십시오.
