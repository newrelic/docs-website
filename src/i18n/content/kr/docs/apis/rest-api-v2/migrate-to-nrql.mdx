...
| 값(RPM) | NRQL 함수 | | ------------- | ---------------------------------------------------------------------------------- | | `score` | `apdex(newrelic.timeslice.value)` | | `s` | `apdex(newrelic.timeslice.value)` 또는 `count(newrelic.timeslice.value)` | | `t` | `apdex(newrelic.timeslice.value)` 또는 `sum(newrelic.timeslice.value)` | | `f` | `apdex(newrelic.timeslice.value)` 또는 `sum(newrelic.timeslice.value['totalExclusive'])`| | `count` | `apdex(newrelic.timeslice.value)` | | `value` | `apdex(newrelic.timeslice.value)` | | `threshold` | `max(newrelic.timeslice.value)` | | `threshold_min` | `min(newrelic.timeslice.value)` |
