...
뉴렐릭 REST API Explorer(v2)를 사용 중이시라면 다이아몬드를 사용하실 수 있습니다.<img title="icon-api-diamond" alt="icon-api-diamond" src="/images/apis-and-data_icon_REST-API-explorer-diamond.webp" />
...
기본적으로 API 시간 입력은 UTC(Universal Time Coordinated)를 사용합니다. UTC를 오프셋하려면 API 호출에서 더하기 또는 빼기 `00:00` 사용합니다. 이를 통해 다른 시간대의 시간을 입력할 수 있습니다. 그러나 이 <DNT>**does not**</DNT> 은 항상 UTC인 출력 시간을 수정합니다.
...
<CollapserGroup>
  <Collapser
    id="offset-minus"
    title={<>이 API 호출은 UTC에서 동쪽으로 2시간( <InlineCode>
      -02:00
    </InlineCode> )의 시간대를 표시합니다.</>
    }
  >
    ```sh
    curl -X GET "https://api.newrelic.com/v2/applications/$APP_ID/metrics/data.json" \
  ...
  <Collapser
    id="offset-plus"
    title={<>이 API 호출은 UTC에서 서쪽으로 +8시간( <InlineCode>
      %2B08:00
    </InlineCode> )의 시간대를 표시합니다.</>
    }
  >
