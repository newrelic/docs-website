...
Synthetics REST API를 사용하여 ping, 단순 브라우저, 스크립트 브라우저 및 API 테스트 모니터와 같은 모든 유형의 [합성 모니터](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors#types-monitors) 를 만들고 관리하십시오. 모든 합성 모니터링 데이터는 REST API를 통해 사용할 수 있습니다.

Synthetics REST API를 사용하려면 해당 기능과 [사용자 키](/docs/apis/rest-api-v2/getting-started/introduction-new-relic-rest-api-v2#api_key) 를 허용하는 사용자 역할이 있어야 합니다.

사용 가능한 모든 New Relic API에 대한 개요는 API [소개를](/docs/apis/getting-started/introduction-new-relic-apis) 참조하세요.
...
최신 버전의 Synthetics API(v3)에는 다음 기능이 추가되었습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        v3부터 Synthetics API는 유효성 검사 실패가 발생했을 때 최대한 많은 정보를 반환하려고 시도합니다. 이렇게 하면 요청에 무엇이 잘못되었는지 파악하는 데 도움이 됩니다. API는 이전 API 버전에서 발생한 첫 번째 유효성 검사 오류가 아니라 모든 유효성 검사를 실행하고 실패한 유효성 검사 메시지를 반환합니다.
      </td>
    </tr>
...
[Synthetics REST API](/docs/apis/synthetics-rest-api) 를 사용하려면 합성 모니터를 관리하고 [사용자 키](/docs/apis/rest-api-v2/getting-started/introduction-new-relic-rest-api-v2#api_key) 를 사용할 수 있어야 합니다(REST API 키는 작동하지 않음).

이 API는 모든 Synthetics 모니터에 사용할 수 있습니다. ( [스크립팅된 브라우저 및 API 테스트 모니터](#scripted-api-monitors-api) 에 대한 추가 API 방법을 사용하여 해당 모니터와 연결된 스크립트를 업데이트할 수도 있습니다.) 모든 합성 데이터는 API를 통해 사용할 수 있습니다. API 예제는 cURL 명령을 보여줍니다.
...
<CollapserGroup>
  ...
  >
    ...
    특정 모니터 ID를 사용하고 [Synthetics REST API 속성](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api) 을 특정 값으로 바꾸십시오.
  ...
  >
    ...
    PATCH 요청은 전체 엔터티를 업데이트하는 것이 아니라 New Relic Synthetics 모니터의 개별 속성을 업데이트하기 위한 것이므로 업데이트하려는 속성만 제공할 수 있습니다.
  ...
  <Collapser
    ...
    title="기존 모니터 삭제"
  >
    New Relic Synthetics에서 기존 모니터를 삭제하려면 <var>$API_ENDPOINT</var> `/v3/monitors/$MONITOR_ID` 에 DELETE 요청을 보냅니다.
  ...
  >
    New Relic Synthetics에서 유효한 위치 목록을 검색하려면 다음 명령을 사용하십시오.
...
<CollapserGroup>
  <Collapser
    ...
    title="모니터 스크립트 가져오기"
  >
    계정에 대한 New Relic Synthetics의 특정 `SCRIPT_BROWSER` 또는 `SCRIPT_API` 모니터와 연결된 스크립트를 보려면 <var>$API_ENDPOINT</var> `/v3/monitors/$MONITOR_ID/script` 에 GET 요청을 보내십시오. 예를 들어:
  ...
  <Collapser
    ...
    title="스크립트 모니터 추가"
  >
    REST API를 사용하여 New Relic Synthetics에 새 스크립팅된 모니터를 추가하려면:
  ...
  <Collapser
    ...
    title="모니터 스크립트 업데이트"
  >
    계정에 대한 New Relic Synthetics의 특정 `SCRIPT_BROWSER` 또는 `SCRIPT_API` 모니터와 연결된 스크립트를 업데이트하려면 `scriptText` (필수)가 포함된 JSON 페이로드와 함께 PUT 요청을 <var>$API_ENDPOINT</var> `/v3/monitors/$MONITOR_ID/script` 로 보냅니다.
