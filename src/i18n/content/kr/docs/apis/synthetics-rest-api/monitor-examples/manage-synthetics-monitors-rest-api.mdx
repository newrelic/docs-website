...
US 기반 계정(기본값)의 경우 다음 엔드포인트를 사용하세요:
...
[JP 데이터센터](/docs/accounts/accounts-billing/account-setup/choose-your-data-center) 계정의 경우 다음 엔드포인트를 사용하세요:
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="create-monitor" title="모니터 만들기">
    NerdGraph 최신 런타임을 사용하여 합성 모니터를 관리하려면 [NerdGraph API를](/docs/apis/nerdgraph/examples/synthetics-api/overview/) 사용하십시오.
