...
[일본](/docs/accounts/accounts-billing/account-setup/choose-your-data-center) (JP) 엔드포인트:
...
<CollapserGroup>
  <Collapser id="multipart-form-data-request" title="multipart/form-data를 사용한 요청">
    #### 지원되는 분야
  ...
  <Collapser id="application-json-request" title="애플리케이션/json으로 요청">
    #### 지원되는 분야
...
<CollapserGroup>
  <Collapser id="create-a-new-table" title="새 테이블 만들기">
  ...
  <Collapser id="update-a-table" title="기존 테이블 업데이트">
...
<Collapser id="download-an-existing-table" title="기존 테이블 다운로드">
...
<Collapser id="delete-an-existing-table" title="기존 테이블 삭제">
...
<Collapser id="list-tables" title="계정에 대해 업데이트된 목록 테이블">
