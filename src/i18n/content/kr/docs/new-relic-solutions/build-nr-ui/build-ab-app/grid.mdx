...
차트를 정렬하려면 `Grid` 에 배치하세요. `Grid` 은 콘텐츠를 12열 행으로 구성하는 데 사용하는 컨테이너입니다. `GridItem` 는 `Grid` 의 빌딩 블록입니다. `GridItem` 이 처리해야 하는 행의 양을 결정하려면 _1_ 과 _12_ 사이에서 `GridItem` 의 `columnSpan` 지정합니다. 따라서 차트를 뷰 너비에 걸쳐 늘리려면 `columnSpan` 이 _12_ 인 `GridItem` 에 차트를 배치합니다. 12개의 차트를 단일 행에 맞추려면 _1_ 의 `columnSpan` 를 사용하십시오.
...
<Steps>
  ...
  <Step>
    ...
    각 차트의 `columnSpan` 레이아웃 계획과 일치합니다. 12열 행의 절반을 차지하는 모든 차트의 `columnSpan` 은 _6_ 이고, 전체 행을 차지하는 모든 차트의 `columnSpan` 은 _12_ 입니다. `ChartGroup` 자체가 아닌 `ChartGroup` 의 각 차트는 `GridItem` 에 있습니다. `ChartGroup` 차트의 위치가 아닌 동작을 연결합니다.
