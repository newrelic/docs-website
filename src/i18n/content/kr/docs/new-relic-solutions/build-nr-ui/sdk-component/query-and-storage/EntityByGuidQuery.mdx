...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>
        `graphql-tag` 에 의해 AST로 구문 분석된 GraphQL 조각 문서. 쿼리 구성 요소는 엔터티에서 사용할 수 있는 가장 일반적으로 사용되는 필드를 반환합니다. 쿼리에서 반환된 엔터티에 대한 추가 필드를 요청하려는 경우 이 소품을 사용할 수 있습니다. 프래그먼트 이름은 `EntityFragmentExtension` 이어야 하며 `EntityOutline` 유형에 적용되어야 합니다.
