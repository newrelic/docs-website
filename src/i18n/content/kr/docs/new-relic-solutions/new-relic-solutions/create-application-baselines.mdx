...
* [**합성**](/docs/synthetics) : 합성 모니터링으로 웹 프런트엔드 및 API를 [모니터링](/docs/synthetics) 합니다. 경우에 따라 APM 또는 인프라를 사용하여 온프레미스 환경을 구성하지 못할 수 있습니다. 예를 들어 조직의 정책이 방화벽 뒤에 에이전트를 설치하는 것을 금지할 수 있습니다. 이러한 경우 응용 프로그램에 웹 프론트엔드가 있는 경우 기준을 설정하는 기능을 제공하면서 에이전트가 아닌 모니터링을 제공하는 Synthetics를 사용하십시오.
...
<CollapserGroup>
  <Collapser
    ...
    title="APM 배포"
  >
    애플리케이션 스택에 [APM 에이전트를 설치합니다](/docs/agents/manage-apm-agents/installation/install-agent) . APM 에이전트 설치 단계는 언어에 따라 다릅니다.
  </Collapser>
  ...
  <Collapser
    ...
    title="합성 모니터 만들기"
  >
    Synthetics는 웹사이트, 중요한 비즈니스 트랜잭션 및 API 엔드포인트를 모니터링하는 자동화되고 스크립트 가능한 도구 모음입니다. 시작하려면 [모니터를 추가하십시오](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors#adding-monitors) .
...
[데이터 탐색기](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/) 와 [NRQL 쿼리 언어](/docs/insights/nrql-new-relic-query-language/using-nrql/introduction-nrql) 를 사용하여 New Relic에서 KPI 및 비즈니스 메트릭 데이터를 찾을 수 있습니다. [대시보드를 구축](/docs/dashboards/new-relic-one-dashboards/get-started/introduction-new-relic-one-dashboards) 하여 해당 KPI의 성과를 추적할 수도 있습니다.
