...
* [**종합 모니터링**](/docs/synthetics) : 종합 모니터링으로 웹 프런트엔드 및 API를 [모니터링](/docs/synthetics) 합니다. 경우에 따라 APM 또는 인프라를 사용하여 온프레미스 환경을 구성하지 못할 수 있습니다. 예를 들어 조직의 정책이 방화벽 뒤에 에이전트를 설치하는 것을 금지할 수 있습니다. 이러한 경우 애플리케이션에 웹 프론트엔드가 있는 경우 기준을 설정하는 기능을 제공하면서 에이전트가 아닌 모니터링을 제공하므로 종합 모니터링을 사용합니다.
