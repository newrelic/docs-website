...
워크로드에 있는 엔터티의 경고 상태에서 파생되는 워크 **Health status** \[로드 상태] 보기는 워크로드가 어떻게 작동하는지 알려줍니다.
...
워크로드를 사용하면 복잡한 시스템의 일부인 엔터티를 그룹화하고 워크로드의 모든 엔터티 상태를 요약하는 단일 전역 값을 얻을 수 있습니다. 따라서 워크로드 작동이 중지되는 시점을 신속하게 감지하거나 잠재적 사고 또는 서비스 품질 손실을 예상할 수 있습니다.
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** 으로 이동하여 **More > Workload views** 를 클릭합니다.
2. 보기 추가를 클릭합니다.
