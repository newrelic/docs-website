...
* 필수 합성 모니터 또는 서비스에 중요한 사고가 발생한 경우 워크로드가 중단된 것으로 표시되기를 원할 수 있습니다.
* 그러나 호스트 인프라에 특정 중복성과 중단에 대한 복원력이 있는 경우 단일 호스트에 경고 인시던트가 있다고 해서 워크로드 상태를 **Operational** \[운영] 에서 변경할 필요가 없을 수 있습니다.
