...
데이터를 쿼리하고 차트 및 대시보드를 생성하는 방법이었던 Insights 제품은 더 이상 사용되지 않으며 해당 기능 집합이 [New Relic 플랫폼](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one/) 의 핵심 부분으로 이전되었습니다. 이 전환 및 새로운 기능에 대한 자세한 내용은 [Insights to New Relic 마이그레이션 가이드를](/docs/new-relic-one/use-new-relic-one/core-concepts/transition-new-relic-one-insights/) 참조하십시오.
...
| updatedAt            | updatedAt                                                                                                          |                                             |
| title                | 이름                                                                                                                 |                                             |
| editable             | [permissions](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#dashboards-permissions) | `editable` 및 `visibility` 이(가) 동일한 개념으로 병합됨 |
| visibility           | [permissions](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#dashboards-permissions) | `editable` 및 `visibility` 이(가) 동일한 개념으로 병합됨 |
| description          | description                                                                                                        |                                             |
| metadata             | -                                                                                                                  | GraphQL API에서 버전 관리 불필요                     |
| icon                 | -                                                                                                                  | New Relic으로 번역되지 않음                         |
| grid_column_count    | -                                                                                                                  | New Relic의 기본 12열 대시보드                      |
| filter               | -                                                                                                                  | 아직 New Relic으로 번역되지 않음                      |
...
| presentation.notes                  | -                                | 아직 New Relic으로 번역되지 않음                                                                                                             |
| layout                              | layout                           |                                                                                                                                    |
...
실제로는 같지만 다른 유형의 쿼리를 통해 얻은 위젯 시각화를 그룹화하여 위젯 시각화를 단순화했습니다. 예를 들어, 선 위젯은 쿼리 유형에 관계없이 동일한 방식으로 표시됩니다(예: 이전 `line_chart` vs. `comparison_line_chart` Insights).
...
[Insights API](/docs/apis/insights-apis/insights-dashboard-api/) 에서 새로운 New Relic 대시보드 API로의 마이그레이션을 용이하게 하고자 합니다. 이전 REST 엔드포인트가 새 GraphQL 쿼리 또는 변형에 매핑되는 방법을 보여주는 몇 가지 예를 아래에서 찾으십시오.

### List (GET) -> entitySearch 쿼리 [#entitysearch]

대시보드는 엔티티 개념을 수용합니다. 그들은 이제 New Relic의 엔티티 생태계의 또 다른 엔티티입니다.
...
시스템 상태를 변경하는 작업은 GraphQL API의 돌연변이입니다. `dashboardCreate` 변형에 필요한 입력을 제공하여 대시보드를 만들 수 있습니다. GraphQL API는 자체 설명을 목표로 하지만 Nerdgraph 문서는 [대시보드 위젯을 빌드하는 방법에](/docs/apis/nerdgraph/examples/create-widgets-dashboards-api) 대한 문서와 같은 필드에 대한 일부 정보를 제공할 수 있습니다.
...
`dashboardDelete` 변형을 사용하면 엔티티 guid를 제공하여 기존 대시보드를 삭제할 수 있습니다.
