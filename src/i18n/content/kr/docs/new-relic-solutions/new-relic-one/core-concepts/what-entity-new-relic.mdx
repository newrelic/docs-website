...
뉴렐릭 옵저빌리버티는 <DoNotTranslate>**entities**</DoNotTranslate> 개념을 기반으로 구축되었습니다. 이 문서에서는 엔터티를 정의하는 방법, 엔터티로 수행할 수 있는 작업, 고유한 엔터티 또는 엔터티 그룹을 만드는 방법에 대해 설명합니다.
...
뉴렐릭 관점에서 볼 때 <DoNotTranslate>**entity**</DoNotTranslate> 의도적으로 광범위한 개념입니다. 엔터티는 a) 뉴렐릭에 데이터를 보고하거나 우리가 액세스할 수 있는 데이터를 포함하고 b) 고유한 엔터티 ID로 식별되는 모든 것입니다. 대부분의 엔터티에서 ID는 `entityGuid` [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#attribute) 으로 표시됩니다.
...
모니터링되는 서비스 및 시스템의 방대한 목록에서 도움이 되지 않는 대규모 데이터 스트림을 제공하는 것이 아니라 <DoNotTranslate>**practical information about your business-important entities**</DoNotTranslate> 제공하는 것이 우리의 목표이기 때문에 엔터티와 해당 관계에 중점을 두는 것이 중요합니다. 엔터티 수준의 더 많은 인사이트를 통해 복잡하고 현대적인 시스템을 더 효과적으로 모니터링하고 문제를 해결할 수 있습니다.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com)**</DoNotTranslate>: 뉴렐릭을 열면 모니터 엔터티의 개요를 제공하는 <DoNotTranslate>**All entities**</DoNotTranslate> 페이지로 이동하게 됩니다. 이 페이지에서 엔터티에 대한 메타데이터를 보거나 엔터티를 클릭하여 성능 세부 정보를 볼 수 있습니다.
</figcaption>
...
엔터티의 GUID 및 기타 메타데이터를 UI 에서 찾으려면 : 임의 [의 엔터티 목록](/docs/new-relic-solutions/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts#find) 에서 엔터티의 <Icon name="fe-more-horizontal"/> 아이콘을 클릭하고 <DoNotTranslate>**See metadata & tags**</DoNotTranslate>을 클릭합니다.
...
이러한 속성은 우리가 받는 텔레메트리에서 합성되도록 되어 있습니다. <DoNotTranslate>**Do not**</DoNotTranslate> 영향과 결과를 알지 못하는 경우 이를 설정하세요.
...
[엔터티 탐색기](/docs/new-relic-solutions/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts#find), 내비게이터 및 Lookout 보기에서 엔터티를 볼 때 오른쪽 창에 해당 <DoNotTranslate>**Related entities**</DoNotTranslate> 가 표시됩니다. 현재 엔터티에 직접 연결된 다양한 엔터티를 시각화해 줍니다. 관련 엔터티에 대한 중요한 지표를 빠르게 보고 그리드의 연결된 모든 부분을 통해 한 엔터티에서 다른 엔터티로 이동할 수 있습니다.
...
관계가 자동으로 감지되지 않으면 [NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/#manual-relationships) 사용하여 수동으로 생성하거나 뉴렐릭 UI 에서 <DoNotTranslate>**Related entities**</DoNotTranslate>의 <DoNotTranslate>**Add/edit related entities**</DoNotTranslate> 링크를 사용하여 생성할 수 있습니다. 현재는 서비스 엔터티 간의 호출/호출자 관계를 수동으로만 생성할 수 있습니다.
