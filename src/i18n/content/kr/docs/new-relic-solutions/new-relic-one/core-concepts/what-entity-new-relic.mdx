...
엔터티 필터 표시줄은 기본 엔터티 목록 보기, APM, <InlinePopover type="browser"/>UI, 인프라 UI 등을 포함한 많은 New Relic UI 페이지에서 사용할 수 있습니다.필터 표시줄을 사용하여 엔터티를 탐색하고 유용한 필터를 저장하는 방법을 알아보려면 [엔터티 필터를](/docs/new-relic-solutions/new-relic-one/core-concepts/search-filter-entities) 참조하세요.
...
관계가 자동으로 감지되지 않으면 [NerdGraph API를](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/#manual-relationships) 사용하여 수동으로 생성하거나 **Related entities** \[관련 항목] 의 **Add/edit related entities** \[관련 항목 추가/편집] 링크를 사용하여 New Relic UI에서 이를 수행할 수 있습니다. 현재는 서비스 엔터티 간의 호출/호출자 관계를 수동으로만 생성할 수 있습니다.
