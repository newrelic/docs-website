...
3. 팝업 **Add a new tag...** \[메뉴의 새 태그 추가...] 필드에서 형식의 태그를 `accountId: 123456789`추가합니다.<img title="Add tags in New Relic" alt="Add tags in New Relic" src={solutionsTagManagement}/>
...
* New Relic UI에서: 엔터티의 인프라 페이지에서 태그를 만들고 값을 할당합니다. 이러한 태그를 쿼리하려면 NRQL에서 쿼리할 때 태그 이름에 `tags.` 접두사를 추가해야 합니다.
...
<CollapserGroup>
  ...
  <Collapser
    id="infra-teams"
    title="인프라에 대한 특정 엔터티를 소유한 내부 팀 찾기"
  >
  ...
  >
    CPU가 있는 다른 인프라 엔터티에서 일부 부하 테스트를 수행하려고 한다고 가정해 보겠습니다. `A`라는 그룹의 항목에 한 가지 처리를 적용하고, `B`이라는 그룹의 항목에 다른 처리를 적용하고, `control group`라는 처리가 없는 하나의 항목 그룹을 유지할 수 있습니다.
