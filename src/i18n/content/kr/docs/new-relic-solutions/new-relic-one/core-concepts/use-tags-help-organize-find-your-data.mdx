...
* 더 나은 구성 및 검색
  ...
  및 [워크로드](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster).
...
<CollapserGroup>
  ...
  >
    ...
    * `telemetry.sdk.language`: 라는 이름의 태그로 저장됩니다. `language`
...
2. 페이지 상단의 엔터티 이름 근처에 있는 **태그** 버튼을 클릭합니다.
3. 태그 위젯의 새 **Add a new tag...** \[태그 추가...] 필드에서 형식의 태그를 `accountId: 123456789` 추가합니다.<img title="Add tags in New Relic" alt="Add tags in New Relic" src={solutionsTagManagement}/>
...
<CollapserGroup>
  ...
  >
    ...
    * 단일 태그에 `regions: EMEA | US | LATAM` 과 같은 여러 값을 포함하지 마십시오. `region: emea` , `region: us` 및 `region: latam` 과 같이 세 가지 다른 태그를 사용하는 것이 좋습니다. 여러 태그를 일치시키려면 필터 UI의 고급 옵션을 사용하여 일치시킬 수 있습니다.
      ...
      * [경고 인시던트](/docs/alerts-applied-intelligence/overview/#concepts-terms) 에 대한 주의 사항: 이는 하나의 키 이름 인스턴스만 지원합니다. 여러 키 이름을 사용하는 경우 해당 인시던트에 추가할 키 이름이 무작위로 선택됩니다.
  </Collapser>
...
기능마다 태그를 다르게 처리합니다. 다음은 [NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language/) 을 사용하여 태그 데이터를 쿼리하거나 [NRQL 상태 경고](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions) 를 생성하는 방법에 대한 몇 가지 세부 정보입니다.
...
<CollapserGroup>
  ...
  <Collapser
    id="infra-teams"
    title="인프라 모니터링을 위한 특정 엔터티를 소유한 내부 팀 찾기"
  >
