...
<figcaption>
  여기서는 `accountId` 태그를 사용하여 특정 계정으로 [워크로드를](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)) 필터링하는 것을 볼 수 있습니다.
</figcaption>
...
* <InlinePopover type="dashboards" />과 [비밀번호를](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)) 더 잘 정리하고 검색하세요.
...
<CollapserGroup>
  ...
  <Collapser id="simple-tags" title="태그를 단순하게 유지">
    ...
    * 단일 태그에 `regions: EMEA | US | LATAM` 과 같은 여러 값을 포함하지 마십시오. `region: emea` , `region: us` 및 `region: latam` 과 같이 세 가지 다른 태그를 사용하는 것이 좋습니다. 여러 태그를 일치시키려면 필터 UI의 고급 옵션을 사용하여 일치시킬 수 있습니다.
      * [공지 이벤트](/docs/alerts-applied-intelligence/overview/#concepts-terms) 에 대한 주의 사항: 이는 하나의 키 이름만 지원합니다. 여러 개의 키 이름을 사용하는 경우, 그중 하나가 무작위로 선택되어 해당 문제에 추가됩니다.
  </Collapser>
