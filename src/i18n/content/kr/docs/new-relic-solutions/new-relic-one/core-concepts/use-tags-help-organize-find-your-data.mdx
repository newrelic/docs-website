...
New Relic에서 태그는 모니터링되는 앱 및 호스트, 에이전트, 대시보드 및 [워크로드](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster) 와 같은 다양한 데이터 세트에 추가되는 키:값 쌍(예: `team: operations` )입니다. 몇 가지 중요한 [속성](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute) 을 태그로 제공합니다(예: 앱 이름 및 언어와 같은 앱 메타데이터, 호스트 이름 및 AWS 리전과 같은 호스트 메타데이터). 사용자 정의 태그를 추가할 수도 있습니다.
...
<Callout variant="tip">
  태그는 이전에 New Relic UI에서 `labels` 이라고 불렸습니다. 때때로 코드에서 `label` 을 볼 수 있습니다. 예를 들어 일부 APM 에이전트 구성 파일은 태그에 대해 `label` 구성 옵션을 사용합니다.
</Callout>
...
* 허용되는 문자:
  ...
  * 문자는 UTF-8이어야 합니다.
  * [NerdGraph를 사용하여 태그를 추가](/docs/apis/nerdgraph/examples/nerdgraph-tagging-api-tutorial) 할 때 태그 키의 대시( `-` )는 빼기 기호로 해석됩니다. 태그 키에 대시가 있는 경우 `` `key-name` `` 과 같이 백틱을 사용합니다.
...
<CollapserGroup>
  ...
  >
    ...
    NRQL 경고 조건과 관련된 태그를 추가하거나 삭제할 수도 있습니다. UI를 통해 조건 태그를 추가하거나 삭제하려면:
  ...
  >
    ...
    * [호스트 내 통합](/docs/integrations/host-integrations/host-integrations-list) : 일부 속성은 기본적으로 태그로 사용할 수 있습니다. 통합별 `labels` 구성 옵션을 사용하여 태그를 추가할 수도 있습니다(예: [Apache 통합](/docs/integrations/host-integrations/host-integrations-list/apache-monitoring-integration#config) 에 대한 레이블 구성 참조).
