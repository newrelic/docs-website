...
* **트랜잭션이 자동으로 계측되지 않습니다** . 애플리케이션이 자동으로 계측되도록 하려면 사용 중인 에이전트에 대한 [호환성 및 요구 사항 설명서](/docs/apm/new-relic-apm/getting-started/introduction-apm) 를 읽으십시오. 애플리케이션이 자동으로 계측되지 않거나 특정 활동의 계측을 추가하려는 경우 [사용자 지정 계측](/docs/agents/manage-apm-agents/agent-data/custom-instrumentation#implementing) 을 참조하십시오.
