...
* 녹색: 현재 이 엔티티 성능에 대한 인시던트가 없습니다.
* 노란색: 이 엔터티에 대한 열린 경고 사건이 있습니다.
* 빨간색: 이 엔터티에 대해 열려 있는 중요한 사건이 있습니다.
* 회색: 에이전트가 보고하지 않음. 이는 엔티티에 설치된 에이전트가 데이터를 보고하지 않음을 의미합니다. 이것은 [데이터베이스 또는 외부](#externals) 에서 예상되는 동작입니다.
* 주황색: 비정상적인 행동.

## API를 사용하여 종속성 이해 [#api]

[NerdGraph](/docs/apis/graphql-api/get-started/introduction-new-relic-nerdgraph) 를 사용하여 서비스 맵에서 사용 가능한 동일한 관계 연결을 발견할 수 있습니다. 자세한 정보와 예는 [NerdGraph GraphiQL 관계 API 자습서](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/) 를 참조하십시오.
