...
현재 서비스 맵 기능을 사용하는 방법에 대한 정보입니다. 이전 서비스 맵 기능 사용에 대한 도움말은 [레거시 APM 서비스 맵](/docs/understand-dependencies/understand-system-dependencies/service-maps/service-maps-apm) 을 참조하십시오.

서비스 맵을 사용하면 환경 전반에 걸쳐 종속성을 빠르고 쉽게 시각화할 수 있습니다. 시스템 전체에서 모든 [엔터티](/docs/what-entity-new-relic) 가 함께 작동하는 방식을 확인하는 데 도움이 됩니다. 서비스 맵을 사용하여 문제를 해결하고 환경이 함께 작동하는 방식을 확인하며 문제가 다운스트림 영향을 받지 않도록 할 수 있습니다.
...
## 엔터티에 대한 연결 추가 또는 제거 [#add-remove]

서비스 맵을 보려면 [one.newrelic.com](https://one.newrelic.com) 에서 **Explorer** 를 클릭하십시오. 보려는 엔터티를 선택하면 사이드바에서 서비스 맵을 선택할 수 있습니다.

지도는 업스트림 및 다운스트림 서비스를 보여줍니다. 왼쪽에 있는 엔터티는 업스트림, 오른쪽에 있는 엔터티는 다운스트림입니다. 엔터티에 대한 연결을 추가하거나 제거하려면:

1. 변경하려는 지도의 엔터티 위로 마우스를 가져갑니다.
2. **더 많은 연결 추가 또는 제거를** 클릭합니다.
3. 연결 목록에서 지도에 표시할 엔터티에 대한 확인란을 선택된 상태로 유지합니다. 선택하지 않은 엔티티는 지도에서 제거됩니다.

## 경고에 대해 색상으로 구분됨 [#alerts]

맵의 각 엔티티는 성능에 따라 색상을 표시합니다.
...
* <Icon
  ...
  회색: 엔터티에 대해 설정된 경고 조건이 없습니다.

* <Icon name="fe-square"/>

  흰색: 에이전트가 보고하지 않음. 이는 엔티티에 설치된 에이전트가 데이터를 보고하지 않음을 의미합니다. 이것은 [데이터베이스 또는 외부](#externals) 에서 예상되는 동작입니다.
...
New Relic One 서비스 맵에서 특정 엔티티를 볼 수 없는 경우 [문제 해결: New Relic One에서 누락되거나 난독화된 데이터를](/docs/troubleshooting-missing-or-obfuscated-data-new-relic-one) 참조하십시오.
