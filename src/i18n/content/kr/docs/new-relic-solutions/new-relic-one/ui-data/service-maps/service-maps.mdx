...
서비스 맵은 프런트엔드에서 백엔드까지 전체 아키텍처를 시각적으로 표현하여 시스템을 구성하는 상호 연결된 구성 요소에 대한 포괄적인 개요를 제공합니다. 이 맵은 애플리케이션, 데이터베이스, 호스트, 서버 및 프로세스 외부 서비스( [웹 외부 서비스](%5B#externals%5D) 라고 함) 간의 관계를 동적으로 표시합니다.
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM > (엔티티 선택) > Service map**: 서비스 맵은 종속성과 성능을 보여줍니다.
</figcaption>
...
2. 엔터티를 선택합니다.
3. 왼쪽에서 **Service map** \[서비스 맵 을] 클릭합니다.

### 특정 엔터티의 지도 [#map-specific-entity]

엔터티 위에 커서를 놓고 **Map from this entity** \[이 엔터티에서 매핑을] 클릭하면 해당 엔터티의 관점에서 엔터티가 어떻게 관련되어 있는지 볼 수 있습니다. 이에 따라 서비스 맵이 업데이트됩니다.
...
* **Health status** \[건강 상태]: 영향을 받은 엔터티 주변의 건강 상태 색상을 제거합니다.

* **Entity types** \[항목 유형]: 지도에서 특정 항목을 숨깁니다.

* **Timewarp**: 지난 3시간 동안 개체 상태가 어떻게 변경되었는지 살펴보세요.

* **Relationship depth** \[관계 깊이]:

  * **Depth 1**: (기본값) 상태에 관계없이 기본 엔터티에 직접 연결된 모든 엔터티를 표시합니다.
  * **Depth 2**: 기본 엔터티에서 2홉 떨어진 저하된 엔터티(경고 포함)에 중점을 둡니다. 기본 엔터티에 직접 연결된 엔터티는 이 보기에 포함되지 않습니다.
  * **Depth 3**: 깊이 2와 유사하게 깊이 3은 경고가 있는 저하된 엔터티에 집중하지만 기본 엔터티에서 3홉 떨어진 엔터티로 검색을 확장합니다. 기본 엔터티에 대한 직접 연결은 제외된 상태로 유지됩니다.
