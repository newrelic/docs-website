...
<Steps>
  <Step>
    ...
    `nr1.json` **Circular progress bar** \[원형 진행률 표시줄] 시각화의 [메타데이터 파일](/explore-docs/custom-viz/configuration-options/) 입니다. 이 파일을 사용하여 `VictoryPie` 의 `colorScale` 필드에 해당하는 구성 가능한 `colorScale` 옵션을 추가합니다.
  </Step>

  <Step>
    차트 색상을 맞춤설정하려면 필드 [`collection`](/explore-docs/custom-viz/configuration-options/#collection) [`string`](/explore-docs/custom-viz/configuration-options/#string) 중 를 추가하세요.
  ...
  <Step>
    ...
    이렇게 하면 막대 모서리가 둥글게 되는 대신 기본 90도로 되돌아갑니다. 로컬 서버가 실행되는 동안 _index.js_ 에 대한 변경 사항을 자동으로 인식합니다. 따라서 브라우저에서 시각화를 보고 업데이트를 확인하세요.
