...
* 귀하의 New Relic [사용자 키](https://docs.newrelic.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#user-api-key).
* New Relic 계정의 [계측 애플리케이션](https://docs.newrelic.com/docs/agents/manage-apm-agents/installation/install-agent) .
...
<Steps>
  ...
  <Callout variant="important">
    New Relic 계정의 [지역을](https://docs.newrelic.com/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers) 설정해야 합니다. `-r` 사용하여 `us` 또는 `eu` 설정합니다.
  </Callout>

  <Step>
    지원서 세부정보를 확인하세요.

    이제 New Relic으로 계측한 애플리케이션에 태그를 추가합니다. [태그는](https://docs.newrelic.com/docs/new-relic-one/use-new-relic-one/core-concepts/tagging-use-tags-organize-group-what-you-monitor) 엔터티를 구성하고 필터링하는 데 도움이 되는 키-값 쌍입니다. [엔터티](https://docs.newrelic.com/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic) (예: 애플리케이션)에는 최대 100개의 키-값 쌍이 연결될 수 있습니다.

    New Relic CLI를 사용하여 애플리케이션을 검색하기 전에 [계정 ID](https://docs.newrelic.com/docs/accounts/install-new-relic/account-setup/account-id) 와 [애플리케이션 이름을 New Relic에](https://docs.newrelic.com/docs/agents/manage-apm-agents/app-naming/name-your-application) 기록하거나 복사하십시오. New Relic 플랫폼에서 애플리케이션을 찾으려면 둘 다 필요합니다.
  </Step>

  <Step>
    애플리케이션 세부 정보를 JSON 개체로 검색합니다.
  ...
  <Step>
    ...
    응용 프로그램 배포가 잘못되는 경우가 많습니다. [배포 마커는](https://docs.newrelic.com/docs/apm/new-relic-apm/maintenance/record-monitor-deployments) 애플리케이션 데이터에 첨부될 때 배포를 추적하고 발생한 문제를 해결하는 데 도움이 되는 레이블입니다.
  ...
  <Step>
    배포의 개정 및 타임스탬프에 대한 JSON 응답을 확인합니다.
...
New Relic CLI에서 [사용 가능한 모든 명령을](https://github.com/newrelic/newrelic-cli/blob/master/docs/cli/newrelic.md) 살펴보십시오. 예를 들어 [`workload create`](https://github.com/newrelic/newrelic-cli/blob/master/docs/cli/newrelic_workload_create.md) 사용하여 [New Relic 워크플로를](https://docs.newrelic.com/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-one-workloads-isolate-resolve-incidents-faster)만들 수 있습니다.
