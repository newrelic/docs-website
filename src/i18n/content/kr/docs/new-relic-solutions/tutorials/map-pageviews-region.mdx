...
* Nerdpack: 뉴렐릭의 JavaScript, JSON, CSS 및 기타 파일의 표준 컬렉션으로, 기능과 디자인을 제어합니다. 자세한 내용은 [Nerdpack 파일 구조를](/explore-docs/nerdpack-file-structure) 참조하세요.
...
<Steps>
  <Step>
    ...
    이 시점에서 결과가 없으면 쿼리에 오류가 없는지 확인하세요. 쿼리가 정확하다면 [브라우저 에이전트가](https://docs.newrelic.com/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent) 설치되지 않았을 수 있습니다.
  </Step>
  ...
  <Step>
    ...
    1. `pageviews-app` 로 이동하여 구조를 확인하세요. 여기에는 뉴렐릭의 앱 런처에 표시될 설명과 아이콘을 사용자 정의할 수 있는 런처 폴더가 포함되어 있습니다. 여기에는 각각 세 개의 기본 파일인 `index.js`, `nr1.json` 및 `styles.scss` 가 포함된 `nerdlets` 도 포함되어 있습니다. 이 가이드의 일부로 이러한 파일 중 일부를 편집하게 됩니다. 자세한 내용은 [Nerdpack 파일 구조를](/explore-docs/nerdpack-file-structure) 참조하세요.
  ...
  <Step>
    ### `TableChart` 구성요소 가져오기 [#import-table-component]
...
<Steps>
  <Step>
    ### `TextField` 구성요소 가져오기 [#import-textfield]

    `TableChart` 구성요소에서와 마찬가지로 뉴렐릭에서 [`TextField` 구성요소를](/client-side-sdk/index.html#components/TextField) 가져와야 합니다.
