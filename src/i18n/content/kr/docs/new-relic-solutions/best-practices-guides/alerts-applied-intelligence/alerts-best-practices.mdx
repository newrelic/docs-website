...
가능한 경우 정책 범위를 단일 [엔터티](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic/) 로 구성합니다. [문제가](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/specify-when-alerts-create-incidents/) 발생할 때 알림을 받아야 하는 필수 팀에 정책을 할당합니다. 이렇게 하면 정책을 중앙 집중화하고 집중적으로 유지할 수 있습니다.
...
## 조건 임계값 및 인시던트 설정 [#threshold-practices]
...
임계값(빨간색)과 경고(노란색)의 두 가지 인시던트 임계값이 있습니다. 위의 제안을 염두에 두고 다양한 기준으로 이러한 임계값을 정의합니다.
...
2. 24시간 30분 후에 새 인시던트를 열도록 신호 손실을 설정합니다(조정할 수 있지만 늦게 실행되는 배치 작업을 허용하는 것이 좋습니다).
...
## 문제 기본 설정 정의 [#incident-practices]

문제 알림을 받을 시기를 결정하여 문제가 발생할 때 대응할 수 있습니다.

알림을 처음 사용하는 경우 [문제 기본 설정 옵션에](https://discuss.newrelic.com/t/relic-solution-alert-incident-preferences-are-the-key-to-consistent-alert-notifications/40867)대해 자세히 알아보세요.

기본 문제 기본 설정은 정책 내의 모든 조건을 하나의 문제로 결합합니다. 기본 문제 기본 설정을 변경하여 받는 문제 및 문제 알림의 수를 늘리거나 줄입니다.

조직 내의 각 팀에는 서로 다른 요구 사항이 있습니다. 문제 기본 설정을 결정할 때 팀에 두 가지 중요한 질문을 합니다.

* 문제가 발생할 때마다 알림을 받고 싶습니까?
* 유사한 알림을 모두 그룹화하고 한 번만 알림을 받고 싶습니까?

정책 및 해당 조건의 범위가 더 넓은 경우(예: 여러 엔터티의 성능 관리) 받는 문제의 양을 늘립니다. 두 가지 문제가 반드시 서로 관련이 있는 것은 아니기 때문에 더 많은 알림이 필요합니다.

정책 및 해당 조건에 집중된 범위(예: 한 엔터티의 성능 관리)가 있는 경우 기본 문제 기본 설정을 선택합니다. 두 가지 문제가 서로 관련되어 있거나 팀이 이미 알림을 받고 기존 문제를 수정하는 경우 알림이 덜 필요합니다.

[알림 채널 모범 사례를](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices/#channel-practices)사용하여 문제 알림을 받는 방법을 결정합니다.

## 알림 채널 선택 [#channel-practices]

가장 유용한 채널과 정책에 맞게 알림을 맞춤화하여 알림 피로를 피하고 올바른 담당자가 관심 있는 문제를 체계적으로 수신하고 대응하도록 도울 수 있습니다.
...
문제가 발생하면 최신 정보를 유지하거나 문제를 해결해야 하는 팀과 개인에게 알립니다.

최신 정보를 유지하려면 이메일과 같이 덜 방해가 되는 알림 채널을 선택하십시오.

중요한 알림 및 문제 응답의 경우 PagerDuty 또는 Slack과 같이 방해가 되는 알림 채널을 선택하세요.
...
필요할 때 정책, 특정 엔터티 및 조건을 묵음 처리할 수도 있습니다. 문제는 계속 열 수 있지만 알림을 받지는 않습니다.
