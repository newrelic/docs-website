...
이를 통해 동일한 AWS 태그(예: `ECTag_Role='Kafka'`)를 사용하여 인프라의 일부로 필터링하고 여러 방법으로 데이터를 분할할 수 있습니다. 추가적으로 우리의 <InlinePopover type="alerts"/>[엔터티 필터 보기는](/docs/new-relic-solutions/new-relic-one/core-concepts/search-filter-entities) 태그 기반이며 동적이므로 이러한 태그와 일치하는 인스턴스를 자동으로 추가/제거하여 사용자에게 클라우드 인프라에 따라 확장되는 실시간 보기를 제공합니다.
