...
1. 모니터를 추가하려면 **[one.newrelic.com](http://one.newrelic.com)** > **Synthetics** 로 이동하십시오. ( [EU 기반 계정](/docs/using-new-relic/welcome-new-relic/get-started/introduction-eu-region-data-center) 이 있는 경우 **[one.eu.newrelic.com](http://one.eu.newrelic.com)** 으로 이동합니다.) **Create monitor** 를 클릭합니다.
...
<img
  ...
  title="Synthetics monitor critical alert"
...
이 정보의 상위 레벨 보기를 얻으려면 [모니터 인덱스](/docs/new-relic-one-monitors-index) 를 사용하여 볼 수 있습니다.  
핑 모니터에 대한 요약을 보거나 간단한 스크립트 모니터에 [대한 개요](/docs/synthetics/new-relic-synthetics/pages/synthetics-overview-page-view-monitors-performance) 를 얻으십시오. 이 페이지는 시간 경과에 따른 애플리케이션 성능에 대한 더 깊은 통찰력을 제공합니다.

### 탐색기에서 모니터 보기 [#howto-2]
...
<CollapserGroup>
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com) > Explorer > Synthetic monitors** 로 이동합니다.
  ...
  >
    ...
    [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics** 으로 이동합니다.
  </Collapser>
</CollapserGroup>

## 4. 개별 모니터 결과 보기 [#view]

전 세계 여러 곳에서 액세스하는 웹 앱의 성능을 확인해야 합니다. [결과](/docs/synthetics/new-relic-synthetics/pages/synthetics-results-access-individual-monitor-runs) 페이지는 개발에서 프로덕션에 이르기까지 모든 것이 사용자 경험에 어떻게 영향을 미치는지 보여줍니다. 나열된 항목을 정렬하여 문제 영역이나 비정상적인 결과를 더 잘 식별할 수 있습니다. 위치별로 필터링하여 다른 위치의 모니터 성능을 비교하십시오. (아래의 '네트워크 타이밍' 그래프는 특정 기간 동안의 웹페이지 성능에 대한 스냅샷을 제공합니다.) 이렇게 하려면 다음을 수행합니다.

1. [**one.newrelic.com**](http://one.newrelic.com/) > **Synthetics** 으로 이동합니다.
2. **모니터** 탭으로 이동하여 모니터를 선택합니다.
...
1. [**one.newrelic.com**](http://one.newrelic.com/) 으로 이동한 다음 **Synthetics** 를 클릭합니다.
