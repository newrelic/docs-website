...
* **Dynamic anomaly alerts** \[동적 이상 경고를 사용하면] 계절별 패턴과 성장 추세가 다양한 애플리케이션에 대한 동적 경고 임계값을 쉽게 결정하고 설정할 수 있습니다(이로 인해 정상적인 동작을 정의하는 임계값을 설정하기가 어렵습니다). 이것들

  <InlinePopover type="alerts"/>

  애플리케이션의 과거 측정항목 데이터를 기반으로 모델링된 이상 현상을 사용하세요.

각 경고 정책에는 필요한 만큼의 조건이 포함될 수 있으며 각 경고 조건에는 다음 세 가지 구성 요소가 포함됩니다.

* 조건 유형(메트릭, 외부 서비스 등)

* 정책이 대상으로 하는 [엔터티](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#alert-entity) (예: APM 앱,
