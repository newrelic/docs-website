...
* **동적 이상 경고를 사용하면** 계절별 패턴과 성장 추세가 다양한 애플리케이션에 대한 동적 경고 임계값을 쉽게 결정하고 설정할 수 있습니다(이로 인해 정상적인 동작을 정의하는 임계값을 설정하기가 어렵습니다). 이러한 경고는 애플리케이션의 기록 메트릭 데이터에서 모델링된 변칙을 사용합니다.
