...
APM을 어떻게 사용하는지 아는 것과 뉴렐릭의 애플리케이션 성능 모니터링 소프트웨어를 잘 사용하는 방법을 아는 것은 또 다른 문제입니다. 다음은 귀하가 <InlinePopover type="apm"/>전문가이자 팀의 핵심 자산이 되는 데 도움이 되도록 고안된 몇 가지 모범 사례입니다.
...
* [엔터티는](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#alert-entity) 해당 정책이, 목표(예: APM 앱,
...
* 공지 공지 채널을 설정하려면 [표준 절차를 따르세요](/docs/alerts-applied-intelligence/notifications/intro-notifications).
...
구현하다, 배포하다 에이전트는 자동으로 <DNT>**(preferred as a method to avoid errors)**</DNT>:
...
3. 새 파일을 기존 파일과 비교하여 설정 파일을 수정합니다. 특히,
