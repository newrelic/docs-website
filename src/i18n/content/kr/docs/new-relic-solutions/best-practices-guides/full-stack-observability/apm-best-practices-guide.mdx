...
핵심 성과 지표가 급증하거나 하락하면 조직의 개인과 팀에 알려야 합니다. [Alerting in New Relic](/docs/alerts/new-relic-alerts/getting-started/alerting-new-relic) 은 최종 사용자에게 영향을 미치기 전에 문제를 감지할 수 있는 동적 기준선을 포함하는 도구 세트를 제공합니다.
...
* **동적 기준 경고** 를 사용하면 계절적 패턴과 성장 추세가 다양한 애플리케이션에 대한 동적 경고 임계값을 쉽게 결정하고 설정할 수 있습니다(이로 인해 정상적인 동작을 정의하는 임계값을 설정하기 어렵습니다). 이러한 경고는 애플리케이션의 기록 메트릭 데이터에서 모델링된 기준선을 사용합니다.
...
* 동적 기준 경고를 설정하고 애플리케이션을 선택 [하려면 표준 절차를 따르십시오](/docs/alerts/new-relic-alerts/defining-conditions/create-baseline-alert-conditions) . 예측된 기준이 있는 메트릭의 미리 보기가 표시됩니다. 해당 응용 프로그램에 대한 메트릭을 선택하고 해당 기준을 볼 수 있습니다. 그런 다음 임계값 슬라이더를 사용하여 임계값이 기준선 예측을 얼마나 가깝게 따르기를 원하는지 설정할 수 있습니다.
...
1. 메뉴 모음에서 **More > Key transactions** 를 선택한 **Add more** 를 선택합니다. 그런 다음 앱 및 웹 트랜잭션을 선택하거나 선택한 트랜잭션에서 **Track as key transaction** 선택합니다.
...
* [사용자 관리 문서 원본](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model) 보기
* [New Relic One 사용자 관리 문서](/docs/accounts/accounts-billing/new-relic-one-user-management/introduction-managing-users) 보기