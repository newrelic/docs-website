...
더 많은 것을 얻으세요 [<InlinePopover type="browser"/>](/docs/browser/browser-monitoring/getting-started/introduction-browser-monitoring), 귀하의 웹사이트와 사용자에 대한 더 깊은 가시성을 제공합니다. 실제 사용자 모니터링을 통해 문제를 더 빠르게 찾고 고객에게 더 나은 경험을 제공하는 데 도움이 되는 모범 사례는 다음과 같습니다.
...
New Relic은 브라우저 모니터링을 포함하여 당사의 모든 제품에 걸쳐 [통합된 경고를](/docs/alerts-applied-intelligence/overview/#concepts-terms) 제공하므로 귀하는 항상 최신 정보를 얻을 수 있습니다. 설정하는 것이 좋습니다 <InlinePopover type="alerts"/>시작하는 데 도움이 되는 샘플 알림과 함께 핵심 웹 바이탈을 모니터링하려면 다음을 수행하세요.
