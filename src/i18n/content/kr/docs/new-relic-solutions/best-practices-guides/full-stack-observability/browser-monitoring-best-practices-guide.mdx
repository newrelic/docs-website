...
1. New Relic UI의 상단 메뉴에서 **서비스 수준** 을 선택합니다. 맨 오른쪽의 드롭 목록에 있을 수 있습니다.
...
## 9. 기준 경고 사용 [#alerts-anomalies]

**비정상적인 행동 및 이벤트에 대한 경고**

일부 메트릭은 특정 임계값에 대해 쉽게 추적할 수 있지만 다른 유형의 데이터는 더 주기적이거나 정상으로 간주되는 범위가 가변적일 수 있습니다. 트래픽 처리량이 이에 대한 좋은 예입니다. 상당히 다양한 주기를 가질 수 있지만 대규모 트래픽 감소 또는 급증은 사용자 트래픽을 방해하는 파손 또는 트래픽 급증을 유발하는 DDoS 공격의 중요한 지표가 될 수 있습니다. [기준선 경고](/docs/alerts/new-relic-alerts/defining-conditions/create-baseline-alert-conditions) 는 경고에서 더 많은 신호 대 잡음비를 생성하기 위해 정상적인 활동의 예상 "대역"을 만드는 데 도움이 될 수 있습니다.

### 그것을하는 방법

1. **경고 및 AI** 에서 경고 **로 이동을** 선택하고 왼쪽 열에서 **정책** 을 클릭합니다.
2. **(+) 새 경고 정책** 을 선택하여 새 경고 정책을 만들고 의미 있는 이름을 지정합니다.
3. **조건 만들기를** 클릭하여 첫 번째 조건을 만든 다음 제품에 대한 **브라우저** 를 선택하고 조건에 대한 **메트릭 기준선** (기본값이어야 함)을 선택합니다. 그런 다음 다음 **을 클릭하고 엔터티를 선택** 합니다.
4. 경고하려는 응용 프로그램의 확인란을 클릭하고 **다음, 임계값 정의** 를 클릭합니다.
