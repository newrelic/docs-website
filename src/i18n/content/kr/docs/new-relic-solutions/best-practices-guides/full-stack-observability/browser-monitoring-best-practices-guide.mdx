...
브라우저 SPA 모니터링은 새 [브라우저 에이전트 설치](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent)에 대해 기본적으로 활성화됩니다. 이 구성을 편집하려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > Settings > Application settings**으로이동합니다.
