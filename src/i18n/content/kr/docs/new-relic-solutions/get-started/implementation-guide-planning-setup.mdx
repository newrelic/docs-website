...
* <DNT>
  ...
  관리 업무를 수행하려면 사용자를 핵심 또는 전체 플랫폼 사용자로 지정해야 합니다. [사용자 유형에 대해 자세히 알아보세요.](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type)
...
* <DNT>
  ...
  * 모든 것에 대한 액세스 권한을 부여하는 기본

    <DNT>
      [**Admin**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#default-groups)
    </DNT>

    [그룹](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#default-groups) 에 추가합니다.

  * 특정 그룹의 사용자를 추가/제거할 수 있는 기능을 부여하는

    <DNT>
      [**Group admin**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#group-admin)
    </DNT>

    [역할을](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#group-admin) 부여합니다.
...
* <DNT>
  ...
  (를) 사용하면 모니터 엔터티 간의 관계를 빠르게 확인할 수 있어 문제를 보다 쉽게 해결할 수 있습니다. 강력한 태그 전략을 갖는 것이 중요합니다. 예를 들어 모든 사람이 동일한 태그 명명 구조를 사용하고 동일한 태그를 일관되게 사용하는 것이 좋습니다. 아직 태그 지정 전략이 없다면 조직을 위한 태그 지정 전략을 만드는 것을 고려해 보세요.

* <DNT>
    [**workloads**](/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-incidents-faster)
  </DNT>

  [기능을](/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-incidents-faster) 사용하면 관련 앱, 서비스, 호스트 및 기타 엔터티를 비즈니스 관련 그룹으로 그룹화하고 해당 정보를 하나의 대시보드에서 볼 수 있습니다. 관련 엔터티를 워크로드로 그룹화하는 방법에 대해 잠시 생각해 보세요.
...
* <DNT>
  ...
  : 지표, 이벤트, 로그 및 트레이스의 네 가지 핵심 데이터 유형에 대한 API 입니다. 당사의 데이터 보고 도구는 이러한 API를 통해 데이터를 전송하며 API를 직접 사용할 수도 있습니다. 기본 데이터 수집 키는

  <InlinePopover type="licenseKey"/>

  입니다. 인스턴스 키는 특정 계정 ID와 연결되어 있습니다. 더욱 자동화된 설치 옵션에는 귀하에게 꼭 맞는 인스턴스 키가 포함되어 있습니다.
...
* <DNT>
  ...
  . 이는 데이터 쿼리부터 뉴렐릭 기능 구성에 이르기까지 데이터 수집을 제외한 모든 작업에 사용됩니다. NerdGraph에는 각 사용자에게 고유한 개인 키인
...
Pro 및 Enterprise [버전](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#editions) 에서만 다중 계정, 사용자 정의 그룹 및 역할 추가 기능, 5명 이상의 전체 플랫폼 사용자 보유 기능이 허용됩니다. 이는 무료 또는 표준 버전을 사용하는 경우 사용자 계획이 덜 중요하다는 것을 의미합니다.
