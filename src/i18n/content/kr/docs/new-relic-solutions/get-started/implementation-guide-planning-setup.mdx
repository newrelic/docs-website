...
* **데이터 수집 API** : 지표, 이벤트, 로그 및 추적의 네 가지 핵심 데이터 유형에 대한 API입니다. 당사의 데이터 보고 도구는 이러한 API를 통해 데이터를 전송하며 API를 직접 사용할 수도 있습니다. 주요 데이터 수집 키는 [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key)입니다. 라이센스 키는 특정 계정 ID에 연결되어 있습니다. 보다 자동화된 설치 옵션에는 올바른 라이센스 키가 포함됩니다.
