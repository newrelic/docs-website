...
*이것은 [구현 가이드](/docs/new-relic-solutions/get-started/implementation-guide-intro) 의 두 번째 부분입니다.*
...
* 다음 중 하나 이상을 포함할 수 있는 애플리케이션 모니터링을 설정합니다.
  ...
  * 언어별 앱 프레임워크 중 일부를 모니터링하는 <InlinePopover type="apm" />에이전트입니다.
  * OpenTelemetry, StatsD 또는 기타 서비스 및 API에 대한 통합을 포함하여 애플리케이션 모니터링을 위한 당사의 다른 솔루션입니다.

* 앱에 웹 연결 인터페이스가 있는 경우: 설정<InlinePopover type="browser" />
...
<CollapserGroup>
  <Collapser id="manual-infra-install" title="인프라에 대한 수동 설치 옵션">
...
* <InlinePopover type="apm" />에이전트: 앱 모니터링에 가장 인기 있는 선택입니다. 아직 APM 에이전트를 설치하지 않은 경우(예: 안내 설치를 통해) [<DNT>**Add your data**</DNT> UI 페이지](https://one.newrelic.com/marketplace?state=b0897c4b-8fb0-eac2-7d6a-613337a6e03d) 로 이동하여 언어를 선택하세요. 완료되면 [APM 요약 UI](/docs/apm/apm-ui-pages/monitoring/apm-summary-page-view-transaction-apdex-usage-data) 및 [분산 추적 UI](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui) 에서 앱 데이터를 찾으세요.
...
* 뉴렐릭에 이미 [쿼리를](/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster) 설정했다면, 해당 쿼리에 예상되는 데이터가 포함되어 있는지 확인하십시오. (그것에 대해서는 나중에 더 자세히 이야기하겠습니다.)
