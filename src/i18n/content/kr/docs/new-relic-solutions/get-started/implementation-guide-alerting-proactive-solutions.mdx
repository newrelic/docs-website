...
<figcaption>
  New Relic <InlinePopover type="alerts"/>UI는 계정의 경고 조건 상태를 보여줍니다.
</figcaption>
...
경고 알림 메시지를 사용자에게 라우팅하기 위해 **워크플로** (사고가 알림을 생성하는 방법 및 전송되는 데이터에 대한 규칙) 및 **알림 대상** (알림이 전송되는 위치)을 사용합니다. 조직 전체에서 일관되고 유지 관리가 가능하도록 설정하는 방법을 계획하는 것이 좋습니다. Slack 또는 PagerDuty와 같은 다른 서비스와 통합하는 경우 장기적으로 이러한 통합을 제어하고 유지하는 방법을 고려하십시오.
