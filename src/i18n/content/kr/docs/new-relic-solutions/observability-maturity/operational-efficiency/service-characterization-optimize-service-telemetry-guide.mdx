...
서비스에 제공하는 이름은 <DoNotTranslate>**namespace**</DoNotTranslate> (에이전트 데이터를 찾을 위치)을 제공합니다. 뉴웰릭이 서비스의 동작을 이해하기 위해 사용하는 가장 중요한 전략 중 하나는 비슷한 것을 함께 집계하고 집계에서 파생된 공통점을 사용하여 차이를 격리하는 것입니다.
...
* <DoNotTranslate>**Defect detection:**</DoNotTranslate>
  ...
  간단한 참/거짓 결과를 생성하는 엔드포인트에 대한 테스트를 인코딩함으로써 운영 팀은 개별 오류를 격리하여 서비스 제공의 무결성이 손상되었는지 확인할 수 있습니다.

* <DoNotTranslate>**Baselining:**</DoNotTranslate>

  신세틱스 또는 기계 테스트는 제어 관점에서 서비스 제공의 일관성을 평가할 수 있는 예측 가능한 조건 세트를 제공합니다.
...
합성 테스트는 각각 고유한 초점을 가진 다양한 테스트 옵션을 제공합니다. 자세한 내용은 [합성 모니터링 문서를](/docs/synthetics/) 참조하세요.

서비스 개발자의 관점에서 가장 많이 사용되는 모니터 유형은 <DoNotTranslate>**Endpoint availability**</DoNotTranslate> 입니다. 이 모니터 유형은 HTTP 요청 조건을 보는 기능을 제공합니다. 이는 액세스 가능한 API 에 대한 POST 또는 GET만큼 간단할 수도 있고, 셀레니엄 모니터링 펼쳐보기가 요청을 연속적으로 평가하여 다단계 프로세스의 기능적 무결성을 확인하는 여러 단계를 포함할 수도 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Cost model:**</DoNotTranslate> 회사의 개발자 FTE 모델 및 프로젝트 추정에 따라 다릅니다.

    <DoNotTranslate>**Estimation:**</DoNotTranslate> 일반적으로 개발자가 뉴렐릭 리뷰 기능을 사용하여 효과를 발휘하는 데는 몇 시간이 걸립니다.
  ...
  >
    ...
    <DoNotTranslate>**Estimation:**</DoNotTranslate> 이는 프로젝트 범위와 필요한 계측 작업의 양에 따라 달라지는 경향이 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**KPI:**</DoNotTranslate> 서비스 품질(비즈니스 KPI)
