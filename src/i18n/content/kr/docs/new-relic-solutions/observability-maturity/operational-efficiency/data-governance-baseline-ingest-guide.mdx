...
데이터 수집 거버넌스 방식의 이 단계에서는 현재 조직에서 생성 중인 모든 원격 분석에 대한 상위 수준 보기를 확보해야 합니다. 이 단위는 수집 통계를 계정, 원격 측정 유형 및 애플리케이션과 같은 다양한 그룹으로 나누는 데 중점을 둡니다. 이 수치는 [Optimize 데이터 수집](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-optimizing) 및 [예측 데이터 수집](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-growth) 단계를 알리는 데 사용됩니다.
...
<CollapserGroup>
  ...
  >
    사용량 기반인 [New Relic One 가격 책정 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 의 경우 원격 측정 데이터와 사용자 모두 사용량과 비용에 기여합니다. 이 가이드는 원격 분석 데이터의 가치를 극대화하는 데 중점을 둡니다. 이 섹션에서 사용자에 대한 언급은 사용자와 데이터의 균형을 맞추기 위한 다양한 옵션을 이해하는 데 도움이 됩니다.

    사용 계획에는 세 가지 일반적인 유형이 있습니다. 사용 계획은 조직의 수집 대상을 설정하는 방법에 영향을 미칠 수 있습니다.

    ### 연간 자금 풀

    [연간 자금 풀](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#usage-plans) 계약이 있는 경우 데이터 수집을 위한 월별 목표 예산이 있을 수 있습니다. 예를 들어, 하루에 5TB 및 100명의 전체 플랫폼 사용자를 목표로 설정했을 수 있습니다. 이러한 유형의 계획 데이터에서 사용자는 "교환"될 수 있지만 조직의 다른 이해 관계자와 이를 논의하여 관찰 가능성 목표에 적합한 조합을 얻을 수 있도록 하는 것이 가장 좋습니다. 일부 고객은 한 해 동안 소비 변동성을 계획할 것이지만 지금은 월별 소비 예산이 연간 총 자금 풀 금액을 12로 나눈 값이라고 가정해 보겠습니다.
    ...
    _월간 GB 수집 목표를 얻기 위해 변환하려면 $0.25로 나눕니다._
    ...
    Standard 에디션의 경우 매월 최대 100GB의 데이터를 무료로 수집할 수 있습니다. 현재 무료 계정으로 작업 중인 경우 월별 수집 목표를 100GB로 고려할 수 있습니다. 무료 계정에서는 매월 100GB를 초과하여 수집되는 추가 데이터에 대해 GB당 0.25 USD가 청구됩니다.

    이러한 주제에 대한 자세한 내용은 [New Relic One 가격 책정](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 을 참조하십시오.
  </Collapser>
    ...
    <CollapserGroup>
      ...
      >
        <Callout
          ...
          title="사용 시기"
        >
          쿼리에 이벤트 수준 단위가 필요하고 계정에 어떤 맞춤 이벤트가 있는지 잘 모르는 경우 `entityType()` 연산자를 사용합니다.
        </Callout>
        ...
        특정 이벤트가 소비하는 데이터의 양을 알기 위해 더 깊이 드릴해야 하는 경우 패싯 절에서 `entityType()` 을 사용하여 해당 결과를 얻을 수 있습니다.

        이전 쿼리에 `FACET entityType()` 절을 추가하면 다음이 제공됩니다.
...
3. 해당하는 경우: 계정 드롭다운에서 기본 또는 최상위 계정을 선택합니다.
4. `Done` 을(를) 클릭합니다.
