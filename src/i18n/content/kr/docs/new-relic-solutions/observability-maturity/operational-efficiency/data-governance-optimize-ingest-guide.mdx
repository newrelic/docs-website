...
<CollapserGroup>
  ...
  <Collapser id="on-host-integrations" title="호스트 내 통합">
    ...
    * `METRICS`: 측정항목만 수집하려면 `true` 으로 설정합니다.
    * `INVENTORY`: 인벤토리 수집만 활성화하려면 `true` 으로 설정합니다.
    * `TOPIC_LIST`: 모니터링할 주제 이름의 JSON 배열입니다. topic\_mode가 list로 설정된 경우에만 적용됩니다.
  ...
  <Collapser id="prometheus-metrics-sources" title="Prometheus 메트릭 소스">
    ...
    **POMI 구성 매개변수:**
    ...
    카디널리티가 청구 가능한 GB당 수집과 직접적으로 연관되어 있지는 않지만, 뉴렐릭은 카디널리티에 대해 특정 비율 제한을 유지합니다. Prometheus 클러스터에서 카디널리티를 시각화할 수 있는 것은 매우 중요할 수 있습니다.

    <Callout variant="tip">
      뉴렐릭 계정에는 100만 카디널리티 제한이 있지만, 최대 1,500만 카디널리티까지 요청할 수 있습니다. 변경을 요청하려면 뉴렐릭 계정 담당자에게 문의하세요. 보다 자세한 내용은 [메트릭 API 제한](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes)을 참조하십시오.
    </Callout>

    이미 Prometheus 서버를 실행 중인 경우, POMI 또는 `remote_write`을(를) 활성화하기 전에 해당 서버에서 카디널리티 추정을 실행할 수 있습니다.
...
다음 질문에 답하면 최적화 계획을 개발하고 실행하는 능력에 대한 자신감을 키우는 데 도움이 됩니다. `Baselining` 섹션의 [데이터 수집 기준](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-dashboard) 및 [데이터 수집 엔티티 분석](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-entity-breakdown-dashboard) 대시보드를 사용할 수 있습니다. 설명된 대로 해당 대시보드를 설치하고 이러한 질문 중 몇 개에 답할 수 있는지 확인하세요.
