...
<CollapserGroup>
  <Collapser
    ...
    title="UI를 통해"
  >
    왼쪽 메뉴에서 선택하여 앱의 페이지 보기 UI를 확인하세요. 표시되는 내용이 아래 스크린샷과 많이 일치하는 경우 메모를 작성하고 문제 해결 방법에 대한 이 가이드의 지침을 따르세요.<img src={omaOeScBrowserGroupedPageViews} alt="Browser Page URL Grouping" title="Browser Page URL Grouping"/>
  </Collapser>
...
<CollapserGroup>
  <Collapser
    ...
    title="UI를 통해"
  >
    왼쪽 메뉴에서 앱을 선택하고 _groupedRequestUrl_ 별로 그룹화하여 앱의 AJAX 그룹화를 확인하세요. 표시되는 내용이 아래 스크린샷과 많이 일치하는 경우 메모를 작성하고 문제 해결 방법에 대한 이 가이드의 지침을 따르세요.<img src={omaOeScBrowserGroupedAjaxRequests} alt="AJAX Grouping" title="AJAX Grouping"/>
  </Collapser>
