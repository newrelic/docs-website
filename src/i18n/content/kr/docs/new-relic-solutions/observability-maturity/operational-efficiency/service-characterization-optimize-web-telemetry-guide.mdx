...
열악한 사용자 환경을 감지하고 해결하는 데 필요한 웹 망원경이 있는지 확인하세요. 이 가이드는 올바른 데이터를 얻고 있는지, 그리고 이를 최대한 활용할 수 있도록 <InlinePopover type="browser"/>이 구성되었는지 확인하는 방법을 보여줌으로써 이를 수행하는 데 도움이 될 것입니다.
...
<DoNotTranslate>**Page views**</DoNotTranslate> UI의 페이지 URL은 페이지 성능을 더 잘 관리하는 데 도움이 되도록 자동으로 그룹화됩니다. 자동 그룹화를 결정하는 알고리즘은 웹 앱이 처음으로 업로드될 때 실행됩니다. 현재 웹 트래픽이 앱을 처음 사용할 때와 많이 다르다면 그룹이 너무 적은 것일 수 있습니다.
