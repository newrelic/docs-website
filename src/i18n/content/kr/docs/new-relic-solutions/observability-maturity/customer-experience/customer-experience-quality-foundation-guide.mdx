...
이 <DoNotTranslate>**Quality Foundation**</DoNotTranslate> 가이드는 디지털 고객 경험을 의미 있는 방식으로 이해하는 데 도움이 되는 표준 관행을 만드는 방법에 관한 것입니다. 이 구현 가이드는 다음을 수행하는 데 도움이 됩니다.
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 업타임 및 가용성 개선
  ...
  >
    이 KPI는 [코어 웹 바이탈](https://web.dev/vitals/) 의 일부입니다. 콘텐츠가 포함된 최대 페인트(LCP)는 사용자가 새 페이지로 이동한 후 가장 큰 이미지를 로드하는 데 걸리는 시간을 측정합니다.
  ...
  <Collapser
    id="core-web-kpi"
    title="다음 페인트와의 상호작용(INP)"
  >
    이 KPI는 [코어 웹 바이탈](https://web.dev/vitals/) 의 일부입니다. 웹페이지가 클릭이나 탭과 같은 사용자 작업에 시각적으로 얼마나 빨리 반응하는지 측정합니다.

    <DoNotTranslate>**Goal:**</DoNotTranslate> 모든 페이지 또는 적어도 가장 중요한 페이지에 대해 75% 백분위수에 대해 INP를 200밀리초 이상으로 줄입니다.
    ...
    * 심각: > 500밀리초
    ...
    INP 레버 값은 Google 팀에서 정의합니다. 레버값, 경계 및 그 뒤에 있는 지원 논리는 [Google의 web.dev 사이트](https://web.dev/articles/inp) 에서 찾을 수 있습니다.
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> 모든 페이지 또는 적어도 가장 중요한 페이지에 대해 75% 백분위수에 대해 0.1 이하의 점수를 유지합니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> CDN, 네트워크 및 서비스 성능을 개선하여 첫 번째 바이트까지 걸리는 시간을 단축합니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> AJAX 응답 시간을 측정하고 개선합니다.
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> HTTP 오류율을 측정하고 줄여 고객이 사이트를 방문한 목적을 수행할 수 있는지 확인하세요.
...
이 <DoNotTranslate>**Quality foundation**</DoNotTranslate> 가이드는 사용자 유지, 전환 및 만족도를 최적화하기 위해 애플리케이션에서 개선이 필요한 부분을 식별하는 데 도움이 됩니다. 사물이 어디에 있는지보다는 어디로 가야 하는지가 더 중요합니다.
...
브라우저 앱과 페이지를 검토하여 뉴렐릭에 보고할 것으로 예상되는 모든 것이 실제로 보고되고 있는지 확인하세요. 브라우저 모니터링 UI에서 <DoNotTranslate>**Page Views**</DoNotTranslate> 탭을 검토하거나 다음 쿼리를 실행하면 됩니다.
...
고객 경험 데이터를 여러 세그먼트로 나누어 이해하기 쉽고 실행 가능하게 만듭니다. 이 경우 세그먼트는 데이터 그룹을 나타냅니다. [세그먼트 허용 목록](/docs/browser/new-relic-browser/configuration/group-browser-metrics-urls/#adding) 에서와 같이 URL 섹션을 참조하지 않습니다.

다음 진술을 고려하십시오.

* 우리의 사용자 환경 중 대부분은 3초 이상으로 다음 페인트에 복합작용을 합니다.
* 평균적으로 가장 큰 콘텐츠가 포함된 페인트까지 2초가 소요됩니다.
* 지난 주 페이지뷰는 100만 회였습니다.

비교 대상:

* 미국, 캐나다, EMEA 지역의 대부분의 사용자는 다음 페인트에 2초 이상의 나노복합을 경험했습니다. 말레이시아 및 인도네시아 사용자 경험 4초; 우리는 이것을 조사하고 있습니다.
