...
* `nrAQMIncident` 에서 `NrAiIncident` 로 전환하면 KPI 수치가 크게 변경됩니다. 이는 `NrAiIncident` 이벤트가 임계값 사고가 발생한 횟수를 추적하는 반면 `nrAQMIncident` 이벤트는 경보 알림을 추적하기 때문입니다. 숫자는 변경되었지만 KPI와의 기본 관계 및 이들 간의 상대적 값은 변경되지 않았습니다. 혼란의 위험을 줄이기 위해 AQM 참가자들에게 이에 대해 교육해야 합니다.
