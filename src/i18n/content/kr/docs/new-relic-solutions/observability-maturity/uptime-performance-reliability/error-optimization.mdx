...
<CollapserGroup>
  ...
  >
    ...
    **모범 사례** 오류는 프런트엔드 및 백엔드 애플리케이션 모두에서 발생하지만 일반적으로 프런트엔드에서 측정됩니다. [퍼널 쿼리는](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/funnels-evaluate-data-series-related-events) 전환 측정에 널리 사용되지만 주어진 기간 동안의 총 세션 수에 대한 전환 수를 계산하여 훨씬 더 간단하게 측정할 수 있습니다.
  ...
  >
    ...
    **모범 사례** 오류는 프런트엔드 및 백엔드 애플리케이션 모두에서 발생하지만 일반적으로 프런트엔드에서 측정됩니다. 개선하고 있는 부분과 관련된 프런트엔드 애플리케이션의 페이지 보기를 추적하여 오류 개선이 사용자 참여에 미치는 영향을 측정합니다.
  ...
  >
    ...
    **모범 사례** 오류는 프런트엔드 및 백엔드 애플리케이션 모두에서 발생하지만 일반적으로 프런트엔드에서 측정됩니다. 주어진 기간 동안 사이트에 액세스하는 사용자 수를 추적하여 오류 개선이 사용자 참여에 미치는 영향을 측정합니다. 사용자를 추적하기 위해 사용자 지정 계측을 추가하지 않은 경우 대신 세션을 추적할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 2단계에서는 유효한 이벤트 소스로 `MobileSession` 선택합니다. 잘못된 응답의 소스로 `MobileCrash` 선택하세요.
    ...
    * 2단계에서는 유효한 이벤트 소스로 `MobileRequest` 선택합니다. 잘못된 응답의 소스로 `MobileRequestError` 선택하세요.
