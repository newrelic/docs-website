...
#### 각 APM 서비스에 대한 서비스 수준 생성

서비스 생성 2단계에서 **Success** SLI를 선택합니다.

#### 각 브라우저 앱에 대한 서비스 수준 만들기

서비스 생성 2단계에서 **Success** SLI를 선택합니다.

#### 각 모바일 앱에 대한 서비스 수준 생성

* 모바일 충돌에 대한 서비스 수준 생성: 2단계에서 유효한 이벤트의 소스로 `MobileSession` 을 선택합니다. 잘못된 응답의 원인으로 `MobileCrash` 을(를) 선택하십시오.
* 모바일 요청 오류에 대한 서비스 수준 생성: 2단계에서 유효한 이벤트의 소스로 `MobileRequest` 을 선택합니다. 잘못된 응답의 원인으로 `MobileRequestError` 을(를) 선택하십시오.

### 각 서버리스 애플리케이션에 대한 서비스 수준 생성
...
* 2단계에서 유효한 이벤트의 경우 `AWSLambdaInvoation` 을 선택하고 잘못된 응답의 경우 `AwsLambdaInvocationError` 을 선택합니다.
...
### 각 OpenTelemetry 애플리케이션에 대한 서비스 수준 생성
...
더 극단적이거나 복잡한 오류가 발생하면 다른 팀에 도움을 요청해야 할 수도 있습니다. 오류 받은 편지함을 Jira에 연결하면 도움이 될 수 있습니다. 오류 받은 편지함을 Jira에 연결하면 오류 그룹에 연결된 티켓을 쉽게 만들 수 있습니다. Jira 템플릿을 통해 Jira로 전송되는 정보를 제어할 수 있습니다.

### 오류를 Slack에 연결 [#slack]

오류 받은 편지함으로 들어오는 오류의 속도가 감소함에 따라 정규 팀 세션은 더 이상 시간을 잘 사용하지 못할 수 있습니다. 대안은 오류 받은 편지함을 Slack에 연결하고 a) 순환하는 사람을 지정하여 채널을 모니터링하고 오류 그룹이 들어오는 대로 해결/무시/할당하거나 b) 팀이 오류 그룹에 사전 대응하도록 허용합니다.

### CodeStream 사용 [#codestream]

많은 오류 그룹이 해결하려면 코드 변경이 필요합니다. [CodeStream을 New Relic과 통합](/docs/codestream/start-here/what-is-codestream) 하면 문제가 되는 코드를 IDE에서 직접 열어 코드를 직접 조사할 수 있습니다. 또한 개발자가 검토할 수 있도록 특정 코드 줄에 메모와 주석을 남길 수 있으며 그 반대의 경우도 마찬가지입니다.

CodeStream을 오류 받은 편지함과 통합하면 버전 번호를 보거나 SHA를 커밋할 수 있는 것과 같이 오류 그룹에 대한 더 많은 컨텍스트를 얻을 수 있습니다. 또한 오류 받은 편지함을 중앙 집중식으로 사용하여 코드 문제를 식별, 논의 및 수정하면 코드 문제에 효율적으로 대응하고 작업 중복을 피할 수 있습니다.
