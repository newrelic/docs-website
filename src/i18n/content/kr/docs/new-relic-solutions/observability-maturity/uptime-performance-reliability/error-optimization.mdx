...
<CollapserGroup>
  <Collapser id="kpi-error-bus-conversion-rate" title="전환율">
    ...
    <DNT>**Goal:**</DNT> 사용자 여정 완료율과 사용자 세션율을 높입니다.

    <DNT>
      **Best practices**
    </DNT>

    오류는 프론트엔드와 백앤드 설계 모두에서 발생하지만 일반적으로 프론트엔드에서 측정됩니다. [퍼널 쿼리는](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/funnels-evaluate-data-series-related-events) 전환을 측정하는 데 널리 사용되지만, 특정 기간 동안의 총 세션 수에 대한 전환 수를 세는 방식으로 훨씬 더 간단하게 측정할 수 있습니다.
    ...
    전환율을 개선하는 더 많은 사례를 보려면 [퍼널 하단 분석 가이드를](/docs/new-relic-solutions/observability-maturity/customer-experience/bottom-funnel-analysis-customer-journey-guide) 확인하세요. 이 가이드에서는 사용자 여정의 마지막 단계부터 시작하여 전환율을 개선하는 방법을 설명합니다.
  </Collapser>

  <Collapser id="kpi-error-bus-page-views" title="페이지 조회수">
    페이지 조회수를 계산하여 참여 증가 또는 감소를 측정합니다.

    <DNT>**Goal:**</DNT> 페이지 뷰 증가

    <DNT>
      **Best practices**
    </DNT>

    오류는 프론트엔드와 백앤드 설계 모두에서 발생하지만 일반적으로 프론트엔드에서 측정됩니다. 프런트엔드 애플리케이션에서 개선이 필요한 부분과 관련된 페이지 뷰를 추적하여 오류 개선이 사용자 참여에 미치는 영향을 측정합니다.
  ...
  <Collapser id="kpi-error-bus-users" title="사용자 수">
    페이지 조회수를 계산하여 참여 증가 또는 감소를 측정합니다.

    <DNT>**Goal:**</DNT> 귀하의 사이트에 접속하는 사용자 수를 늘리세요

    <DNT>
      **Best practices**
    </DNT>

    오류는 프론트엔드와 백앤드 설계 모두에서 발생하지만 일반적으로 프론트엔드에서 측정됩니다. 특정 기간 동안 사이트에 접속한 사용자 수를 추적하여 오류 개선이 사용자 참여에 미치는 영향을 측정합니다. 사용자를 추적하기 위해 사용자 지정 측정을 추가하지 않은 경우 대신 세션을 추적할 수 있습니다.
...
<CollapserGroup>
  <Collapser id="kpi-error-operational-error-rate" title="오류율">
    오류율은 요청에 대한 오류의 비율입니다.

    오류는 300개 이상의 HTTP 응답 코드, 처리되지 않은 예외, 모바일 충돌 또는 팀의 누군가가 오류로 구성된 이벤트일 수 있습니다.

    <DNT>**Goal:**</DNT> 관리하는 애플리케이션 전반의 오류율을 줄이세요.

    <DNT>
      **Best practices**
    </DNT>

    이는 오류 추적 개선을 위한 진행 상황을 추적하는 데 사용할 주요 KPI입니다. 오류율을 개선하기 위한 단계에는 중요하지 않은 오류를 걸러내는 것과 중요한 오류를 해결하는 것이 포함됩니다.
  </Collapser>

  <Collapser id="kpi-error-operational-error-mean-time" title="오류를 닫는 평균 시간">
    ...
    <DNT>**Goal:**</DNT> 오류율을 줄여 문제 종결 평균 시간을 단축합니다.

    <DNT>
      **Best practices**
    </DNT>

    문제가 발생할 때 다음을 보다 빠르게 감지할 수 있도록 강력한 오류 사례를 구축하세요.
...
* <InlinePopover type="apm" />, <InlinePopover type="browser" />, 모바일 모니터링, 서버리스 모니터링 또는 OpenTelemetry 솔루션에서 오류가 발생하는지 확인하세요.
...
오류를 최적화하려는 애플리케이션 및 서비스 목록을 정의하십시오. 오류 최적화 프로세스를 수행하는 팀은 이러한 앱과 서비스에 대한 완전한 책임과 제어 권한을 가져야 합니다. 결정하셨다면, 이 경우에 대한 [날짜를](/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster) 설정하세요.
...
설정하는 데는 몇 분밖에 걸리지 않습니다. [워크로드 지침을](/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster) 참조하세요.
...
귀하의 위치에서 <DNT>**Service levels**</DNT> 탭으로 이동합니다. 워크로드에서 각 부분에 대한 믿음율을 측정하는 서비스 범위를 만듭니다. 이는 서비스 수준 UI의 2단계에서 구성됩니다. 각 서비스 영역에 대해 WHERE 절을 사용하여 고려되어서는 안 되는 좋은 requests 이나 오류를 필터링합니다.
...
<CollapserGroup>
  <Collapser id="apm-service-level" title="각 애플리케이션 서비스에 대한 오류율 서비스 수준 생성">
    서비스 생성 2단계에서는 <DNT>**Success**</DNT> SLI를 선택합니다.
  ...
  <Collapser id="browser-service-level" title="각 브라우저 애플리케이션에 대한 오류율 서비스 수준 생성">
    * 서비스 생성 2단계에서는 <DNT>**Success**</DNT> SLI를 선택합니다.
  ...
  <Collapser id="mobile-service-level" title="각 모바일 애플리케이션에 대한 오류율 서비스 수준 생성">
    모바일 충돌에 대한 서비스 수준 생성

    * 2단계에서는 유효한 이벤트의 소스로 `MobileSession` 선택합니다. 잘못된 응답의 소스로 `MobileCrash` 선택하세요.
    ...
    * 2단계에서는 유효한 이벤트의 소스로 `MobileRequest` 선택합니다. 잘못된 응답의 소스로 `MobileRequestError` 선택하세요.
  ...
  <Collapser id="serverless-service-level" title="각 서버리스 애플리케이션에 대한 오류율 서비스 수준 생성">
  ...
  <Collapser id="otel-service-level" title="각 개방형 원격 분석 애플리케이션에 대한 오류율 서비스 수준 생성">
