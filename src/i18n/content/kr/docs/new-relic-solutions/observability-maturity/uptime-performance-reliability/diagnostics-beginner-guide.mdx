...
<Callout variant="tip">
  최신 APM 에이전트에서는 로그인 컨텍스트 및 분산 추적이 모두 기본적으로 활성화되어 있습니다. (한 동안 에이전트를 업데이트하지 않은 경우 에이전트를 정기적으로 [업데이트하는](/docs/new-relic-solutions/new-relic-one/install-configure/update-new-relic-agent) 것이 좋습니다.)
