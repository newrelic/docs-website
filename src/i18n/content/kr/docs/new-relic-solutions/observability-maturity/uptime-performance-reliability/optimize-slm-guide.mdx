---
title: '서비스 수준 관리: 서비스 경계를 정의하고 기대치를 설정하여 서비스 수준 목표 충족'
tags:
...
현실은 귀하의 서비스가 "디지털 제품"이고 그 제품은 사용자가 얼마나 잘 받는가에 달려 있다는 것입니다. 귀하의 기술은 최대한 복잡하지만 외부 API를 제외하고는 대부분 인터넷에서 볼 수 없는 폐쇄형 시스템입니다. 디지털 제품 상태의 총체는 해당 제품이 요청에 응답하고, 콘텐츠를 렌더링하고, 데이터를 처리하는 능력입니다.

디지털 제품(서비스) 상태에 대한 가장 중요한 질문은 다음과 같습니다.

1. 얼마나 빠르고 성공적으로 응답을 전달할 수 있습니까?
2. 클라이언트가 우리 서비스에 연결할 수 있습니까?
3. 고객이 콘텐츠를 빠르고 성공적으로 렌더링할 수 있습니까?
4. 데이터를 빠르고 성공적으로 처리할 수 있습니까?

### 서비스 상태가 아닌 것은?

서비스 상태는 진단 데이터와 다릅니다. 서비스 상태는 독점적인 인프라 데이터가 아니며 독점적인 최종 사용자 클라이언트 성능 데이터도 아닙니다. 흥미롭게도 많은 사람들이 상태를 식별하기 위해 먼저 실제 사용자 모니터링(RUM) 또는 종단 간 트랜잭션 데이터(분산 추적)를 살펴보지만 더 많은 질문이 남아 있습니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="입력 성능"
  >
    입력 성능은 애플리케이션에 연결하는 기능으로 간단히 측정됩니다. 이를 전통적으로 "가동 시간"이라고 합니다. 우리는 NRQL 쿼리와 동일한 예를 사용할 것입니다.
...
* [서비스 수준 관리에 대한 무료 온라인 과정 이수](https://learn.newrelic.com/self-paced-service-level-management)
* [서비스 수준 관리 UI](https://docs.newrelic.com/docs/service-level-management/intro-slm) 검토
...
2. 애플리케이션 이름은 [서비스 특성화 가이드](/docs/new-relic-solutions/observability-maturity/operational-efficiency/service-characterization-optimize-telemetry-data-guide) 에 설명된 대로 친숙한 명명 규칙을 따릅니다.
3. [New Relic Explorer 에서 애플리케이션을 찾는](/docs/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts/#find) 방법에 익숙합니다.
...
**참고:** 이 작업에서는 " [SLI 설정 및 출력](#establishing-output-sli) " 섹션에서 배운 기술을 기반으로 합니다.
...
위의 [SLI 설정 및 출력](#establishing-output-sli) 섹션에 설명된 대로 서비스 경계 애플리케이션을 식별합니다.
...
첫 번째 거래(#3)에는 "구매"와 관련이 있다는 내용이 표시됩니다. 이제 "구매" 기능 서비스 수준을 생성할 수 있습니다.
...
아래 스크린샷에 밑줄이 그어진 것처럼 서비스 수준의 두 쿼리에 이 조건을 추가하고 **앞에 `AND `을 붙입니다** .
...
동일한 쿼리에 `AND error is false` 을 추가합니다.

축하합니다! 이제 비즈니스 언어로 된 기능에 대한 "대기 시간" 및 "성공" 서비스 수준이 결합되었습니다.
