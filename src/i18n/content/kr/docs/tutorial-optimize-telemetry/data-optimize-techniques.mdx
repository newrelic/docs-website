...
<CollapserGroup>
  ...
  <Collapser id="browser-agent" title="브라우저 에이전트">
    ...
    * URL의 프로토콜, 포트, 검색 및 해시는 거부 목록에서 무시됩니다.
    ...
    1. <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT> 으로 이동하여 <DNT>**Browser**</DNT> 클릭합니다.
    2. 앱을 선택하세요.
    3. 왼쪽 탐색 메뉴에서 <DNT>**App settings**</DNT> 클릭합니다.
    4. <DNT>**Ajax request deny list**</DNT> 아래에 적용할 필터를 추가합니다.
    5. 에이전트 설정을 업데이트하려면 <DNT>**Save application settings**</DNT> 선택하세요.
  ...
  <Collapser id="log-forwarders" title="로그 포워더">
    ...
    * `config_file`: 기존 Fluent Bit 설정 파일의 경로입니다. 소스가 겹치면 뉴렐릭의 <InlinePopover type="logs" />에 중복된 메시지가 생성됩니다.
  ...
  <Collapser id="prometheus-metrics-sources" title="Prometheus 메트릭 소스">
    ...
    * 메모리 제한: 1Gb 1024(1G)
    ...
    이미 Prometheus 서버를 실행 중인 경우, POMI 또는 `remote_write`을(를) 활성화하기 전에 카디널리티 추정을 실행할 수 있습니다.
  ...
  <Collapser id="cloud-integration" title="클라우드 통합">
    ...
    * <DNT>**Billing**</DNT>: AWS CloudWatch 청구서를 관리해야 하는 경우 폴링 빈도를 줄이는 것이 좋습니다. 이 작업을 수행하기 전에 클라우드 통합에 설정된 공지 조건이 이 축소의 영향을 받지 않는지 확인하세요.
    * <DNT>**New services**</DNT>: 새로운 서비스나 설정을 구현, 배포하고 데이터를 더 자주 수집하려는 경우 일시적으로 폴링 빈도를 늘리는 것이 좋습니다.
...
<CollapserGroup>
  <Collapser id="logs" title="로그">
    ...
    * <DNT>**Manage data**</DNT> 아래에서 <DNT>**Drop filters**</DNT> 클릭하고 &apos;디버그 로그 삭제&apos;라는 필터를 생성하고 활성화합니다.
    * 규칙이 작동하는지 확인합니다.
