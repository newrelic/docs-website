...
또한 문제가 발생할 때 알림을 받을 수 있도록 <InlinePopover type="alerts"/>설정해야 합니다. 당사의 [수집 알림 가이드를](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/usage-queries-alerts) 사용하여 다양한 알림을 생성할 수 있지만 최소한 다음 두 가지 알림을 설정하는 것이 좋습니다.
...
대시보드를 설치한 후 <DNT>**Lookout**</DNT> 보기를 사용하여 수집 패턴을 대략적으로 확인할 수 있습니다. 거기에서 NRQL 쿼리를 생성하여 모든 수집을 이상적으로 검색할 수 있습니다. 예를 들어, 다음 NRQL 쿼리는 지난 24시간 동안 `usageMetric` 이(가) 이상적으로 수행한 모든 수집에 대한 정보를 제공합니다.
...
2. <DNT>**Install this quickstart**</DNT> 선택합니다.

3. [대시보드 가져오기 기능을](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#dashboards-import) 사용하여 <InlinePopover type="apm"/>, <InlinePopover type="browser"/>, 모바일 모니터링 또는 쿠버네티스 클러스터가 포함된 계정에 대시보드를 설치합니다. 이 대시보드를 여러 계정에 설치할 수 있지만 파트너십 소유자 계정(POA)에는 이 대시보드를 설치하지 마세요.

   <Callout variant="tip">
     상위/하위 계정 구조가 있는 경우 대시보드를 상위 계정에 설치하고 대시보드를 수정하여 하나의 대시보드에 계정별 차트가 모두 포함되도록 할 수 있습니다.
   </Callout>

4. <DNT>**Done**</DNT> 선택합니다.

5. 퀵스타트 설치가 완료되면 <DNT>**Data governance entity breakdowns**</DNT> 대시보드를 열어 데이터에 액세스하세요.
