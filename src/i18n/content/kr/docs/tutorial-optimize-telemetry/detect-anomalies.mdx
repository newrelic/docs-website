...
당신은 또한 설정해야합니다 <InlinePopover type="alerts"/>문제가 발생하면 이를 알리는 데 도움이 됩니다. [수집 경고 가이드를](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/usage-queries-alerts) 사용하여 다양한 경고를 생성할 수 있지만 최소한 다음 두 경고를 설정하는 것이 좋습니다.
...
3. 다음을 포함하는 모든 계정에 대시보드를 설치합니다.
   ...
   , 모바일 모니터링 또는 [대시보드 가져오기 기능을](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#dashboards-import) 사용하는 Kubernetes 클러스터. 이 대시보드를 여러 계정에 설치할 수 있지만 파트너십 소유자 계정(POA)에는 이 대시보드를 설치하지 마세요.
