...
CodeStream의 관찰 가능성 **섹션** 을 통해 개발자는 IDE에서 바로 자신이 담당하는 코드와 관련된 오류를 발견할 수 있습니다. 할당된 오류 외에도 IDE에서 연 각 리포지토리에 대한 최근 오류가 나열됩니다.
...
코드 수준 메트릭은 코드가 메서드 수준에서 수행되는 방식에 대한 자세한 통찰력을 제공합니다. New Relic의 Python 및 Ruby APM 에이전트가 자동으로 계측하는 각 방법에 대해 CodeLens라고 하는 각 방법 위에 삽입된 텍스트 줄을 통해 표시되는 황금 신호를 볼 수 있습니다. 응답 시간, 처리량 및 오류율은 지난 30분 동안 표시됩니다.
...
<Callout variant="important">
  Python의 경우 코드 수준 측정항목은 [지원되는 Python 프레임워크](/docs/apm/agents/python-agent/getting-started/instrumented-python-packages) 로 구현된 [Python APM 에이전트](/docs/apm/agents/python-agent/getting-started/introduction-new-relic-python) 에서 모니터링하는 프로젝트에만 사용할 수 있습니다.

  Ruby의 경우 코드 수준 메트릭은 Rails 애플리케이션에서만 사용할 수 있으며 Ruby 메서드는 수동 추적입니다.
</Callout>
