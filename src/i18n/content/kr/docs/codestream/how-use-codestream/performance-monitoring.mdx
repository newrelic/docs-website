...
CodeStream의 관찰 가능성 **섹션** 은 개발자가 IDE에서 현재 선택된 리포지토리와 연결된 각 서비스에 대한 골든 메트릭 및 오류를 볼 수 있도록 하여 코드가 수행되는 방식에 대한 보기를 제공합니다. 각 서비스는 트리에 자체 노드를 가지고 있으며 확장하면 서비스의 골든 메트릭, 사용자에게 할당된 오류 및 5개의 가장 최근 오류가 표시됩니다.
