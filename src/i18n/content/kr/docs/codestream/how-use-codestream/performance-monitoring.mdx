...
코드 수준 지표는 메서드 수준에서 코드가 어떻게 수행되고 있는지에 대한 자세한 통찰력을 제공합니다. New Relic APM 에이전트에 의해 자동으로 계측된 각 메서드에 대해 각 메서드 위에 삽입된 텍스트 줄을 통해 황금 신호가 표시됩니다. 이 텍스트 줄을 CodeLens라고 합니다. 지난 30분 동안의 응답 시간, 처리량 및 오류율이 표시됩니다.
...
* **PHP:** PHP 에이전트 버전 10.4.0 이상이 필요합니다. [PHP 에이전트 구성에서도 코드 수준 메트릭을 활성화](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#inivar-code-level-metrics) 해야 합니다.
