...
코드 수준 메트릭은 메서드 수준에서 코드가 어떻게 수행되고 있는지에 대한 자세한 통찰력을 제공합니다. New Relic Python 및 Ruby APM 에이전트에 의해 자동으로 계측되는 각 방법에 대해 CodeLens라고 하는 각 방법 위에 삽입된 텍스트 줄을 통해 표시되는 황금 신호를 볼 수 있습니다. 응답 시간, 처리량 및 오류율은 지난 30분 동안 표시됩니다.
...
<Callout variant="important">
  코드 수준 메트릭을 사용하려면 [분산 추적](/docs/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing) 을 활성화해야 합니다.

  또한 Python의 경우 코드 수준 측정항목은 [지원되는 Python 프레임워크](/docs/apm/agents/python-agent/getting-started/instrumented-python-packages) 로 구현된 프로젝트에만 사용할 수 있습니다. Ruby의 경우 코드 수준 메트릭은 수동 추적이 있는 Rails 애플리케이션 및 Ruby 메서드에만 사용할 수 있습니다.
</Callout>
