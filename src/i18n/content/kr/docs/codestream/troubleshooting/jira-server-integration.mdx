...
3. 양식을 작성하시오:
   ...
   * **Consumer Key **의 경우 위의 2단계에서 애플리케이션 링크를 생성할 때 입력한 소비자 키를 사용합니다. 그런 다음 위의 1단계에서 만든 개인 키의 전체 내용을 PCKS8 형식으로 복사합니다. 파일 이름은 **jira_privatekey.pcks8 **입니다. 해당 내용을 **Private Key **필드에 붙여넣고 **Submit **을 클릭합니다.

     <img alt="Configure Jira Server" src={configureJiraServer1}/>

4. 그런 다음 애플리케이션 링크를 사용하여 계정에 대한 액세스를 승인하는 Jira Server 인스턴스로 이동합니다. 완료되면 IDE로 돌아가서 다음과 유사한 내용이 표시되어야 합니다.<img alt="Completed Jira Server Integration" src={completedJiraServerIntegration1}/>
