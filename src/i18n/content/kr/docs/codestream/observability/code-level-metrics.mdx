...
코드 수준 메트릭은 코드가 메서드 수준에서 어떻게 수행되는지에 대한 자세한 통찰력을 제공합니다. 측정항목은 계측된 방법 위의 편집기에 바로 표시되므로 성능 데이터가 현재 작업에 영향을 미칠 수 있습니다. CodeStream은 또한 코드에서 성능이 저하된 메서드를 식별하여 성능 문제가 발생할 때 사전에 해결할 수 있도록 해줍니다.

## 에디터 내 성능 데이터 [#codelenses]
...
CodeStream은 New Relic APM 에이전트가 계측한 각 방법 위의 CodeLens에서 지난 30분 동안의 평균 지속 시간과 오류율을 보여줍니다. CodeStream 창에서 현재 선택된 서비스에 대한 지표가 계산됩니다. 다른 서비스를 확장하면 측정항목이 그에 따라 업데이트됩니다.
...
CodeLense는 성능 문제가 있는 코드 섹션에서 작업할 때 유용하지만, 성능 문제는 너무 중요해서 우연히 발견할 수 없습니다! CodeStream은 **Code-Level Metrics** \[코드 수준 지표] 섹션의 코드에서 성능이 낮은 메서드를 자동으로 호출합니다.
...
**Code-Level Metrics** \[코드 수준 지표] 섹션 외에도 CodeStream은 VS Code 또는 JetBrains IDE의 [데스크톱 알림을](/docs/codestream/codestream-settings/account-settings/#notifications) 통해 성능이 저하된 메서드에 대해 알려줍니다.
