...
<CollapserGroup>
  ...
  >
    **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 으로 이동합니다. 애플리케이션을 클릭하면 애플리케이션 테스트 효율성, 취약점, 적용되는 API, 메소드 호출 등을 확인할 수 있습니다.
  </Collapser>
  ...
  >
    ...
    * 귀하의 기능에 대한 효율성 수준: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 으로 이동하세요. 귀하의 기능을 검색하고 요약 섹션을 확인하세요. 애플리케이션의 IAST 적용 범위가 낮은 경우 애플리케이션에 추가 테스트 사례를 추가하여 더 높은 수준의 테스트 효율성을 얻으세요.

    * IAST 적용 범위: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 으로 이동합니다. 해당 애플리케이션을 검색하고 클릭하세요. 요약 섹션에서 IAST 분석 적용 범위가 높고 취약점이 발견되지 않았는지 확인하세요. 이는 애플리케이션이 안전하다는 의미입니다.
  ...
  >
    **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 에 해당 기능이 표시되지 않으면 다음을 확인하세요.
  ...
  >
    ...
    * **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 에서 귀하의 기능을 확인하세요. 애플리케이션이 시작되고 트래픽이 생성됩니다.
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > 테스트 및 애플리케이션** 으로 이동합니다.

    2. **Application tests** \[기능 테스트] 탭에서 애플리케이션을 검색하고 클릭하세요.

    3. **APIs covered** \[다루는 API], **methods calls** \[메서드 호출], **application testing efficiency** \[애플리케이션 테스트 효율성을] 포함한 테스트 세부 정보를 확인하세요.

       또한, 로그 레벨을 **debug/finest** 로 설정하면 `nr-security-home/logs/java-security-collector.log` 파일에서 **Fuzz request received** \[수신된 Fuzz 요청을] 검색할 수 있습니다. 이는 IAST 분석이 진행 중임을 나타냅니다.
  </Collapser>
  ...
  >
    ...
    * 기능 검색: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications** 으로 이동하세요.
  ...
  >
    * Golang의 경우 애플리케이션에서 사용하는 라이브러리 및 프레임워크에 필요한 계측 패키지를 가져왔는지 확인하세요.
      ...
      해당 항목을 가져왔는지 확인하세요. [<InlinePopover type="apm"/>](https://github.com/newrelic/go-agent?tab=readme-ov-file#service-frameworks)각 모듈에 대한 패키지입니다 .
    ...
    * HTTP 프로토콜 엔드포인트를 여는 경우 엔드포인트 이름 주위에 `newrelic.WrapListen` 함수를 배치하여 해당 엔드포인트에 대해 연구소 스캐닝을 활성화합니다.
      ...
      <Callout variant="important">
        Linux 환경에서는 이 단계를 건너뛸 수 있습니다.
      </Callout>

    * **Outbound HTTP Request** \[아웃바운드 HTTP 요청] 또는 외부 서비스 호출과 같은 특정 사례의 경우 [애플리케이션 방법을 업데이트](/docs/apm/agents/go-agent/instrumentation/instrument-go-segments/#go-external-segments) 해야 합니다.
  </Collapser>
  ...
  <Collapser
    id="jira-availability"
    title="JIRA 통합을 사용할 수 있나요?"
  >
    아직은 아니지만, 가까운 시일 내에 추가할 계획입니다.
  </Collapser>
  ...
  >
    IAST는 관찰된 요청을 기반으로 애플리케이션에 새 요청을 보냅니다. 이로 인해 신용 카드에 여러 건의 요금이 청구되는 등 고객 데이터가 바람직하지 않게 조작될 수 있습니다. New Relic은 현재 운영 환경에서 IAST를 사용할 것을 권장합니다. 우리는 이러한 환경에서 신세틱스 데이터를 기반으로 테스트 사례를 기반으로 합니다. 우리의 계획은 프로덕션에서 사용할 수 있는 IAST 버전을 제공하는 것입니다.
  </Collapser>
</CollapserGroup>