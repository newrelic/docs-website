...
현재 뉴렐릭은 신세틱 모니터링 API v1과 v3의 두 가지 버전을 지원합니다. 버전 3은 2016년 10월에 출시되었습니다. <DoNotTranslate>**Version 1 is deprecated**</DoNotTranslate> 은(는) 결국 더 이상 사용할 수 없게 됩니다. 종료 날짜는 발표되지 않았습니다. 그러나 v1에 대한 추가 개발이나 수정은 이루어지지 않습니다.
...
<CollapserGroup>
  ...
  >
    ...
    성공적인 요청은 <DoNotTranslate>**200 OK**</DoNotTranslate> 응답을 반환합니다. [반환된 데이터](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api#api-attributes) 는 다음 형식의 JSON 개체입니다.
  ...
  >
    ...
    잘못된 모니터 ID는 <DoNotTranslate>**404 Not Found**</DoNotTranslate> 오류를 반환합니다. 지정된 모니터가 존재하지 않습니다.
  </Collapser>
  ...
  >
    ...
    또한 REST API를 통해 <DoNotTranslate>**add a scripted monitor**</DoNotTranslate> 에 [추가 API 엔드포인트를](/docs/apis/synthetics-rest-api/monitor-examples/manage-synthetics-monitor-scripts-rest-api) 호출하여 방금 생성된 모니터에 대한 스크립트를 보냅니다.
    ...
    성공적인 요청은 `location` 헤더에 지정된 새로 생성된 모니터의 URI와 함께 <DoNotTranslate>**201 Created**</DoNotTranslate> 응답을 반환합니다. 가능한 오류 코드는 다음과 같습니다.

    * <DoNotTranslate>**400 Bad Request**</DoNotTranslate>

      : 모니터 값 중 하나 이상이 잘못되었거나 요청 형식이 잘못되었습니다. 예를 들어 빈도가 범위를 벗어났거나 지정된 위치 중 하나 이상이 잘못되었습니다(응답 본문의 오류 메시지 참조).
  ...
  >
    ...
    요청이 성공하면 본문이 비어 있는 <DoNotTranslate>**204 No Content**</DoNotTranslate> 응답이 반환됩니다. 가능한 오류 코드는 다음과 같습니다.
  ...
  >
    ...
    요청이 성공하면 본문이 비어 있는 <DoNotTranslate>**204 No Content**</DoNotTranslate> 응답이 반환됩니다. 요청이 실패하면 <DoNotTranslate>**404 Not Found**</DoNotTranslate> 응답이 반환됩니다. 지정된 모니터가 존재하지 않습니다.
  </Collapser>
...
일반 API 외에도 <DoNotTranslate>**scripted browser**</DoNotTranslate> (`SCRIPT_BROWSER`) 및 <DoNotTranslate>**api test**</DoNotTranslate> (`SCRIPT_API`) 모니터 유형에 대한 여러 API 메서드가 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    성공적인 요청은 <DoNotTranslate>**200 OK**</DoNotTranslate> 응답을 반환합니다. 반환된 데이터는 다음 형식의 JSON 개체입니다.
    ...
    * <DoNotTranslate>**403 Forbidden:**</DoNotTranslate>
      ...
      지정된 모니터가 SCRIPT_BROWSER 또는 SCRIPT_API 유형이 아닙니다.
  ...
  <Collapser
    ...
    title="모니터 스크립트 업데이트"
  >
    계정에 대한 뉴렐릭의 특정 `SCRIPT_BROWSER` 또는 `SCRIPT_API` 모니터와 연결된 스크립트를 업데이트하려면 `scriptText` (필수)이 포함된 JSON 페이로드와 함께 `https://synthetics.newrelic.com/synthetics/api/v1/monitors/{id}/script` 에 PUT 요청을 보냅니다. `scriptLocations` 데이터는 [<DoNotTranslate>**Verified Script Execution**</DoNotTranslate>](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations) 설정된 독립 로케이션에만 필요합니다.
    ...
    * <DoNotTranslate>**400 Bad Request:**</DoNotTranslate>
      ...
      `scriptText` 또는 `hmac` 에 대해 잘못된 BASE64 인코딩 문자열입니다.

    * <DoNotTranslate>**403 Forbidden:**</DoNotTranslate>

      지정한 모니터가 펼쳐보기 또는 펼쳐보기 유형이 아닙니다.
