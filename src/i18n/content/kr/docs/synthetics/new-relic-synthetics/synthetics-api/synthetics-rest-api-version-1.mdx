...
현재 New Relic은 Synthetics API v1 및 v3의 두 가지 버전을 지원합니다. 버전 3은 2016년 10월에 릴리스되었습니다. **버전 1은 더 이상 사용되지 않으며** 결국 더 이상 사용할 수 없게 됩니다. 종료 날짜는 발표되지 않았습니다. 그러나 v1에 대한 추가 개발이나 수정은 없습니다.

<Callout variant="caution">
  권장 사항: [v3 Synthetics API](/docs/apis/synthetics-rest-api/monitor-examples/manage-synthetics-monitors-via-rest-api) 를 사용하여 새 모니터를 만들고 v1 스크립트를 v3 스크립트로 마이그레이션하십시오.
</Callout>
...
Synthetics REST API를 호출하려면 관리자의 API 키를 사용해야 합니다. 계정의 REST API 키가 작동하지 않습니다.

<Callout variant="caution">
  Synthetics REST API는 계정의 요청 속도를 초당 3개의 요청으로 제한합니다. 이 임계값을 초과하는 요청은 429 응답 코드를 반환합니다.
</Callout>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="특정 모니터 가져오기"
  >
    New Relic에서 단일 기존 모니터를 보려면 `https://synthetics.newrelic.com/synthetics/api/v1/monitors/{id}` 에 GET 요청을 보냅니다. 다음 예에서 <var>&#x7B;id}</var> 를 특정 모니터 ID로 바꿉니다.
  ...
  >
    ...
    다음 예에서 <var>&#x7B;id}</var> 를 특정 모니터 ID로 바꾸고 [Synthetics REST API 속성](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api) 을 특정 값으로 바꿉니다.
  ...
  <Collapser
    ...
    title="기존 모니터 삭제"
  >
    New Relic에서 기존 모니터를 삭제하려면 DELETE 요청을 `https://synthetics.newrelic.com/synthetics/api/v1/monitors/{id}` 에 보냅니다. 다음 예에서 [<var>&#x7B;id}</var>](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api) 를 특정 모니터 ID로 바꿉니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="모니터 스크립트 가져오기"
  >
    계정의 New Relic에서 특정 `SCRIPT_BROWSER` 또는 `SCRIPT_API` 모니터와 연결된 스크립트를 보려면 `https://synthetics.newrelic.com/synthetics/api/v1/monitors/{id}/script` 에 GET 요청을 보냅니다. <var>&#x7B;id}</var> 를 특정 모니터 ID로 바꿉니다. 예를 들어:
  ...
  >
    ...
    <var>&#x7B;id}</var> 를 특정 모니터 ID로 바꿉니다. 다음은 스크립트의 예입니다.
