...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="모니터 스크립트 업데이트"
  >
    계정에 대한 뉴렐릭의 특정 `SCRIPT_BROWSER` 또는 `SCRIPT_API` 모니터와 연결된 스크립트를 업데이트하려면 `scriptText` (필수)이 포함된 JSON 페이로드와 함께 `https://synthetics.newrelic.com/synthetics/api/v1/monitors/{id}/script` 에 PUT 요청을 보냅니다. `scriptLocations` 데이터는 [<DNT>**Verified Script Execution**</DNT>](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations) 설정된 독립 로케이션에만 필요합니다.
