...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            **핑:** 무시할 수 있음<br/> **스크립트 API:** 7<br/> **간단한 브라우저:** 27<br/> **스크립트 브라우저:** 28
  ...
  <Collapser
    ...
    title={<><img src={integrationK8} title="Kubernetes icon" alt="Kubernetes icon" style={{ height: '35px', width: '40px' }}/>Kubernetes 컨테이너 오케스트레이션 시스템 환경 요구 사항(CPM v3.0.0 이상)</>}
  >
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            **CPU(vCPU/코어):** 0.5 - 1<br/> **메모리:** 1250Mi - 3000Mi
        ...
        <tr>
          ...
          <td>
            **루트 볼륨:** 최소 50GiB(노드 + PV)<br/> **영구 볼륨(PV):** 최소 20GiB
        ...
        <tr>
          ...
          <td>
            **핑:** 무시할 수 있음<br/> **스크립트 API:** 11<br/> **단순 브라우저:** 33<br/> **스크립트 브라우저:** 31
        ...
        <tr>
          ...
          <td>
            Kubernetes 클러스터가 [Kubernetes v1.15](https://kubernetes.io/blog/2019/06/19/kubernetes-1-15-release-announcement/) 를 지원하는 것이 좋습니다. <Callout variant="caution">Kubernetes v1.21 이상의 경우 minion 릴리스 [v3.0.61](https://docs.newrelic.com/docs/release-notes/synthetics-release-notes/containerized-private-minions-release-notes/containerized-private-minion-cpm-3061) 이상을 사용하십시오.</Callout>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <Callout
              ...
              title="EOL 공지"
            >
              2022년 8월 이후에는 원래 비공개 미니언 Helm 차트 URL을 포함하여 여러 기능에 대한 지원을 중단합니다. 이 전환을 쉽게 준비하는 방법을 비롯한 자세한 내용은 [Explorers Hub 게시물](https://discuss.newrelic.com/t/synthetics-minion-helm-chart-url-update/188119) 을 참조하세요.
            </Callout>
...
1. **\[one.newrelic.com** ]( [https://one.newrelic.com](https://one.newrelic.com) )으로 이동합니다. >Synthetics > Private locations\*\*.

2. **비공개 위치** 색인에서 CPM을 할당할 비공개 위치를 찾습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 쓰기 가능한 Docker UNIX 소켓은 `/var/run/docker.sock` 또는 `DOCKER_HOST` [환경 변수](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#environment-variables) 에 마운트됩니다. 자세한 내용은 Docker의 [데몬 소켓 옵션](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-socket-option) 을 참조하십시오.
      ...
      <Callout variant="caution">
        호스트의 코어 수는 CPM이 호스트에서 동시에 실행할 수 있는 러너 컨테이너 수를 결정합니다. 메모리 요구 사항은 예상 러너 컨테이너 수로 조정되므로 리소스 경합을 방지하기 위해 **동일한 호스트에서 여러 CPM을 실행하지 않는** 것이 좋습니다.
      </Callout>
...
<CollapserGroup>
  ...
  >
    1. 삭제하려는 미니언 포드의 <var>MINION_POD_INSTALLATION_NAME</var> 을 가져옵니다.
...
사용자 지정 npm 모듈로 컨테이너화된 비공개 미니언을 구성하고, 실행 사이에 데이터를 보존하고, 환경 변수를 사용하는 등의 작업을 수행할 수 있습니다. 자세한 내용은 [CPM 구성](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration) 을 참조하십시오.
