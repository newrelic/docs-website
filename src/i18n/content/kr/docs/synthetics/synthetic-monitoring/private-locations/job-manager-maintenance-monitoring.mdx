...
* `:8080/status/check`: 신세틱스 Job Manager가 수행하는 내부 상태 점검에 대한 세부 정보를 제공합니다. `HTTP 200` 상태가 정상임을 의미합니다.
...
신세틱스 작업 관리자 로그를 확인하여 신세틱스 작업 관리자의 상태를 모니터링할 수 있습니다.

<CollapserGroup>
  <Collapser id="review-docker-logs" title="도커 로그 검토">
  ...
  <Collapser id="review-podman-logs" title="Podman 로그 검토">
  ...
  <Collapser id="review-kubernetes-logs" title="Kubernetes 로그 검토">
...
<CollapserGroup>
  <Collapser id="docker-debug-logs" title="도커 디버그 로그 활성화">
    <Callout variant="tip">
  ...
  <Collapser id="podman-debug-logs" title="Podman 디버그 로그 활성화">
    <Callout variant="tip">
  ...
  <Collapser id="kubernetes-debug-logs" title="Kubernetes 디버그 로그 활성화">
    <Callout variant="tip">
...
## 디버깅 정보 검색 [#retrieve-debugging-information]

문제 해결에 도움이 되도록 신세틱 작업 관리자에 대한 디버깅 정보를 조회할 수 있습니다. 여기에는 신세틱 작업 관리자 파드 및 해당 파드가 실행 중인 노드에 대한 정보가 포함됩니다.

<CollapserGroup>
  <Collapser id="retrieve-kubernetes-debugging" title="Kubernetes용 검색">
  ...
  <Collapser id="retrieve-openshift-debugging" title="OpenShift용 검색">
