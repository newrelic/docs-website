...
다른 사람이 독립 미니언 또는 신세틱스 작업 관리자를 사용하여 [스크립트 브라우저](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers) 또는 [API 테스트를](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-api-tests) 할당하는 것을 방지하려면 검증된 스크립트 실행을 추가하세요. 그런 다음 미니언 또는 신세틱스 작업 관리자를 변경하려면 본인에게만 알려진 암호가 필요합니다.

뉴렐릭 UI의 배열 위치 목록에는 <DoNotTranslate>**VSE**</DoNotTranslate> 열이 포함되어 있습니다. 자물쇠 <Icon name="fe-lock"/> 아이콘은 해당 위치에 대해 확인된 스크립트 실행이 설정되었음을 나타냅니다.
...
<Callout variant="important">
  이 제한에는 뉴클릭 지원 담당자도 포함됩니다. 당사 수집기는 귀하의 암호를 저장하지 않으므로 당사 지원팀은 귀하의 암호를 복구하거나 재설정할 수 없습니다. 암호를 잊어버린 경우 [미니언 <DoNotTranslate>**Overview**</DoNotTranslate> 페이지](/docs/synthetics/new-relic-synthetics/private-locations/install-configure-private-minions#configure) 에서 암호를 변경한 다음 해당 위치에 할당된 각 모니터를 업데이트해야 합니다.
</Callout>
...
1. <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Private locations](https://one.newrelic.com/synthetics-nerdlets/private-location-list)**</DoNotTranslate> > (해당 위치 선택)로 이동하세요. 해당 위치의 줄임표 아이콘을 선택하고 <DoNotTranslate>**Edit**</DoNotTranslate> 클릭하세요. 확인된 스크립트 실행을 활성화한 다음 저장합니다.
...
3. 신세틱스 UI에서 해당 위치에 할당된 모니터를 선택하세요. 그런 다음 <DoNotTranslate>**Settings > General**</DoNotTranslate> 선택합니다.
...
2. <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**</DoNotTranslate> > (할당된 모니터 클릭) > <DoNotTranslate>**Settings > General**</DoNotTranslate> 로 이동합니다.
...
2. <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Private locations](https://one.newrelic.com/synthetics-nerdlets/private-location-list)**</DoNotTranslate> (주소를 선택하세요)로 이동하세요. <DoNotTranslate>**Enable verified script execution**</DoNotTranslate> 확인란을 선택 취소한 후 저장하세요.

## 기타(레거시) [#enabling]

컨테이너화된 프라이빗 미니언을 사용하지 않는 경우 다음을 수행하여 확인된 스크립트 실행을 활성화합니다. 안전한 장소에 암호를 기록해 두십시오.

1. 웹 브라우저에서 `https://MINION_IP_ADDRESS` 의 미니언

   <DoNotTranslate>**Overview**</DoNotTranslate>

   페이지로 이동합니다(예: `https://1.2.3.4`).
...
3. <DoNotTranslate>**Verified script execution**</DoNotTranslate>
   ...
   확인란을 선택합니다. 암호를 입력한 후 저장하세요. 암호를 안전한 곳에 기록해 두십시오.
...
4. <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Private locations](https://one.newrelic.com/synthetics-nerdlets/private-location-list)**</DoNotTranslate>
   ...
   (주소를 선택하세요)로 이동하세요. 해당 위치의 줄임표 아이콘을 선택하고

   <DoNotTranslate>**Edit**</DoNotTranslate>

   클릭하세요. 확인된 스크립트 실행을 활성화한 후 저장하세요.

5. 신세틱스 UI에서 해당 위치에 할당된 모니터를 선택하세요. 그런 다음
...
암호를 변경하거나 확인된 스크립트 실행을 비활성화하려면 동일한 기본 프로세스에 따라 미니언의 IP 주소로 이동하여 <DoNotTranslate>**Advanced settings**</DoNotTranslate> 을 업데이트하세요. 그런 다음 <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> 로 이동하여 프로세스를 완료하세요.