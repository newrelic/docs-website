...
당사의 신세틱 모니터링 작업 관리자는 프라이빗 [로케이션 에 할당된 신세틱](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring/#types-of-synthetic-monitors) [모니터](https://www.docker.com/resources/what-container) 를 수락하고 실행하는 [컨테이너](/docs/synthetics/synthetic-monitoring/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations) 기반 리소스입니다. 작업 관리자는 도커 컨테이너 시스템 환경, Podman 컨테이너 시스템 환경, Kubernetes 컨테이너 오케스트레이션 시스템 환경 또는 OpenShift 컨테이너 시스템 환경에서 작동할 수 있습니다.

<Callout variant="important">
  Node.js 16/Chrome 134 런타임에서 Node.js 22/Chrome 146으로 마이그레이션하는 경우, 마이그레이션 전략, 배포 예시 및 문제 해결 단계는 [프라이빗 로케이션 런타임 전환 가이드](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) 를 참조하십시오.
</Callout>
...
<CollapserGroup>
  <Collapser id="docker-requirements" title="Docker container system environment requirements">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 이상 (v447 이상 권장)<br /> <DNT>**macOS:**</DNT> 10.11 이상<br /> <DNT>**Windows:**</DNT> Windows 10 64비트 이상
    ...
    <Callout variant="caution">
      신세틱스 Job Manager v447(Ubuntu 24.04를 기본 이미지로 사용)부터는 최신 glibc 및 seccomp 요구 사항으로 인해 RHEL 7 및 4.18 이전 버전의 Linux 커널을 사용하는 기타 배포판은 더 이상 지원되지 않습니다. 안정적인 사용 환경을 위해서는 RHEL 8/9 또는 그에 상응하는 최신 Linux 배포판을 사용하십시오.
    </Callout>
  </Collapser>

  <Collapser id="podman-requirements" title="Podman container system environment requirements">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 이상 (v447 이상 권장)<br />
          </td>
        </tr>
  ...
  <Collapser id="kubernetes-requirements" title="Kubernetes container orchestration system environment requirements">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 이상 (v447 이상 권장) <br /><DNT>**macOS:**</DNT> 10.11 이상<br />
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> [4.18 이상 (v447 이상 권장)](https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-prerequisites#minimum-resource-requirements)<br /> <DNT>**macOS:**</DNT> 10.11 이상<br />
...
동일한 호스트에서 둘 이상의 도커 또는 Podman 로케이션 컨테이너를 실행하는 경우 포트 충돌이 발생합니다. 포트 경합을 방지하려면 각 작업 관리자를 서로 다른 호스트에서 실행해야 합니다.
...
<CollapserGroup>
  <Collapser id="docker-update" title="Docker start procedure">
  ...
  <Collapser id="podman-update" title="Podman start procedure">
    1. [개인 위치 키](#private-location-key) 를 찾습니다.
    ...
    작업 관리자 컨테이너의 로그를 보려면 다음 단계를 따르세요.
  ...
  <Collapser id="kubernetes-install" title="Kubernetes start procedure">
    1. [개인 위치 키](#private-location-key) 를 찾습니다.
...
<CollapserGroup>
  <Collapser id="docker-stop" title="Docker stop procedure">
  ...
  <Collapser id="podman-stop" title="Podman stop procedure">
  ...
  <Collapser id="kubernetes-delete" title="Kubernetes delete procedure">
...
<CollapserGroup>
  <Collapser id="docker-dependencies" title="Docker dependencies">
...
<CollapserGroup>
  <Collapser id="podman-dependencies" title="Podman dependencies">
...
<CollapserGroup>
  <Collapser id="run-non-root" title="Run as non-root user for Docker">
