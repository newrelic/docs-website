...
import syntheticPrivateLocationAdd from 'images/synthetic_screenshot-crop_private-location-add.png'

import syntheticConfigureDeployPrivateLocations from 'images/synthetic_screenshot-crop_configure-deploy-private-locations.png'

import syntheticDeletePrivateLocation from 'images/synthetic_screenshot-crop_delete-private-location.png'

import syntheticEditMonitorSettingsRotatePrivateLocation from 'images/synthetic_screenshot-crop_edit-monitor-settings-rotate-private-location.png'

import syntheticEditPrivateLocationRotate from 'images/synthetic_screenshot-crop_edit-private-location-rotate.png'

import syntheticGeneratePrivateLocationKey from 'images/synthetic_screenshot-crop_generate-private-location-key.png'

import syntheticMonitorSettings from 'images/synthetic_screenshot-crop-monitor-settings.png'
  ...
  >
    ...
    <Steps>
      <Step>
        ...
        1. **[one.newrelic.com > Synthetic monitoring \[종합 모니터링\] > Private locations \[개인 위치\]](https://one.newrelic.com/synthetics/private-location-list)** 로 이동합니다. 그런 다음 **Create private location** \[비공개 위치 생성을] 클릭합니다.<img title="Screenshot showing where to add a new private location" alt="Screenshot showing where to add a new private location" src={syntheticPrivateLocationAdd}/>

        2. 개인 위치 이름을 입력하고 **Enable verified script execution** \[확인된 스크립트 실행 활성화를] 선택합니다. 그런 다음 **Generate key** \[키 생성을] 클릭하여 새 개인 위치를 생성합니다.
        ...
        3. 작업 관리자나 개인 미니언을 새로운 개인 위치로 업데이트하세요. **Configure and deploy** \[구성 및 배포] 섹션에는 샘플 코드가 포함되어 있습니다.
      ...
      <Step>
        ...
        1. **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics)** 으로 이동합니다. 모니터 오른쪽에 있는 세 개의 점 **...** 을 클릭하고 **Monitor settings** \[모니터 설정을] 선택합니다.
        ...
        2. **Select locations** \[위치 선택] 탭에서 이전 위치를 비활성화하고 새 위치를 활성화합니다. 그런 다음 모니터를 저장합니다.
      ...
      <Step>
        ...
        1. **[one.newrelic.com > Synthetic monitoring \[종합 모니터링\] > Private locations \[개인 위치\]](https://one.newrelic.com/synthetics/private-location-list)** 로 이동합니다.

        2. 예전 위치를 찾아보세요. 필터 상자에서 위치 이름이나 ID로 필터링할 수 있습니다.

        3. 오른쪽에 있는 줄임표 아이콘 **...** 을 클릭한 다음 **Edit** \[편집] 을 선택합니다.
