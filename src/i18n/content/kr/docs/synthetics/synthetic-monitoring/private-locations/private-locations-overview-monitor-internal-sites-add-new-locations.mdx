...
신세틱 모니터링에서 a는 신세틱스 작업 관리자의 모음입니다. 신세틱스 작업 관리자는 신세틱스 UI 통해 설정된 작업을 수신하고 관리하는 컨테이너화된 기능입니다.

로케이션에는 신세틱스 작업 관리자를 원하는 수만큼 포함할 수 있습니다. 로케이션을 사용하면 신세틱 모니터링 범위를 새로운 지리적 위치로 확장하고 인트라넷 사이트와 같은 뒤에 있는 웹사이트를 모니터링할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        미국 기반 계정의 경우(기본값): `https://synthetics-horde.nr-data.net/`
        ...
        JP 기반 계정의 경우: `https://synthetics-horde.jp.nr-data.net/`
      </td>
    </tr>
    ...
    <tr id="compatibility-outbound-network">
      ...
      <td>
        신세틱스 작업 관리자는 작업을 수신하고 처리하기 위해 신세틱 모니터링의 엔드포인트 무리에 연결해야 합니다. 방화벽 규칙에서 직접 액세스를 허용하지 않는 경우 [프록시 액세스를 구성](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#FAQ) 해야 합니다.
...
[신세틱스 작업 관리자를 설치하기](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) 전에, 독립 로케이션을 생성해야 합니다.
...
4. 선택 사항: 다음 추가 설정을 구성합니다.
   ...
   * <DNT>**[Verified script execution](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations)**</DNT>스크립트를 이 위치에 할당하거나 작업 관리자를 이 위치에 추가할 때 암호를 요구합니다.

5. <DNT>**Create**</DNT> 선택합니다.

위치를 생성한 후 신세틱 모니터링은 <DNT>My location</DNT> 탭에 <DNT>**Private location key**</DNT> 을 나열하므로 [작업 관리자를 설치할](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) 수 있습니다.
...
각 신세틱스 작업 관리자는 분당 약 75개의 핑 모니터 검사를 실행할 수 있습니다. 특정 위치의 작업 대기열이 증가하는 경우 [추가 작업 관리자를 추가하십시오](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager). 작업 관리자의 용량은 네트워크 성능 및 하드웨어 리소스에 따라 달라질 수 있습니다.
...
신세틱 모니터링에는 위치 및 신세틱스 작업 관리자를 관리하는 도구가 포함되어 있습니다. 또한 [신세틱스 작업 관리자를 설치하여](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) 작업 대기열이 쌓이면 비울 수도 있습니다.
...
<CollapserGroup>
  <Collapser id="clear-job-queue" title="작업 대기열 지우기">
    ...
    이는 작업 관리자가 처리할 수 있는 속도보다 예약된 작업 수가 더 빠르게 증가할 때, 예를 들어 작업 관리자가 오프라인 상태일 때 유용합니다.
  </Collapser>
  ...
  <Collapser id="view-status" title="상태 보기">
    개별 신세틱스 작업 관리자의 상태를 보려면 목록에서 상위 위치 이름을 선택하십시오.

    녹색 [상태 표시기는](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#health-status) 활성 신세틱스 작업 관리자를 식별합니다.
  </Collapser>
  ...
  <Collapser id="delete-location" title="위치 삭제">
    ...
    이는 해당 위치에 배정된 작업 관리자를 차단하는 것은 아닙니다. 작업 관리자는 수동으로 종료하거나 다른 담당자에게 재배정해야 합니다.
  </Collapser>
  ...
  <Collapser id="enable-disable-vse" title="확인된 스크립트 실행 활성화 또는 비활성화">
    ...
    * [VSE\_PASSPHRASE](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-configuration/#environment-variables) 환경 변수를 작업 관리자에게 전달해야 합니다. 그런 다음 <DNT>**Private locations**</DNT> 탭에서 <DNT>**Edit**</DNT> 클릭하고 메뉴에서 <DNT>**Verified script execution**</DNT> 옵션을 활성화합니다.
  </Collapser>

  <Collapser id="rotate-private-location-keys" title="개인 위치 키 순환">
    레지스터 로케이션은 레지스터 로케이션 키를 사용합니다. 이를 통해 배치로케이션 작업 관리자가 실행할 작업을 가져오고 그 결과를 다시 뉴렐릭 플랫폼에 보고할 수 있습니다.
    ...
    <Steps>
      <Step>
        ...
        5. 새로운 독립 위치로 작업 관리자를 업데이트하세요. <DNT>**Configure and deploy**</DNT> 섹션에는 샘플 코드가 포함되어 있습니다.
