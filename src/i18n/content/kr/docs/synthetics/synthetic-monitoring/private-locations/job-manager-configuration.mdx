...
## 도커, Podman, Kubernetes 및 OpenShift에 대한 크기 고려 사항 [#kubernetes-sizing]
...
**방정식:** $$C\_req = (J\_processed + Q\_growth) \times D\_j$$

* $C\_req$ = **필요한 CPU 코어 수**
* $J\_processed$ = 분당 **처리** 되는 작업의 속도.
* $Q\_growth$ = `jobManagerHeavyweightJobs` 대기열이 분당 **증가하는** 속도입니다.
* $D\_j$ = 작업의 **평균 소요 시간** (분)입니다.
...
* $C\_req$ = **필요한 CPU 코어 수**
* $N\_m$ = 실행하려는 중량 모니터의 총 **수입니다** .
* $D\_j$ = 작업의 **평균 소요 시간** (분)입니다.
* $P\_m$ = 모니터의 **주기** (분)입니다(예: 5분마다 실행되는 모니터의 주기는 5입니다).
...
* **작업 기간($D\_j$):** 평균에는 **시간 초과된** 작업(대개 \~3분)도 포함되어야 합니다. 이러한 작업은 전체 기간 동안 핵심을 유지합니다.
...
**1. 분당 처리된 작업 찾기($J\_processed$):** 이 쿼리는 지난 하루 동안 완료된 핑이 없는(중량급) 작업의 수를 세고 분당 평균 비율을 보여줍니다.
...
**2. 분당 대기열 증가율 찾기($Q\_growth$):** 이 쿼리는 시계열 차트에서 `jobManagerHeavyweightJobs` 대기열의 분당 평균 증가율을 계산합니다. 0보다 큰 선은 대기열이 늘어나고 있음을 나타내고, 0보다 작은 선은 대기열이 줄어들고 있음을 나타냅니다.
...
**3. 평균 작업 기간(분) 찾기($D\_j$):** 이 쿼리는 완료된 비핑 작업의 평균 실행 기간을 찾고 그 결과를 밀리초에서 분으로 변환합니다. `executionDuration` 호스트에서 작업을 실행하는 데 걸린 시간을 나타냅니다.
...
**4. 중량 모니터의 총 수($N\_m$)를 찾습니다.** 이 쿼리는 중량 모니터의 고유한 수를 찾습니다.
...
**5. 평균 헤비급 모니터 빈도($F\_j$) 찾기:** 배열 위치의 `jobManagerHeavyweightJobs` 대기열이 증가하는 경우 기존 결과에서 평균 모니터 빈도를 계산하는 것은 정확하지 않습니다. 이는 [Synthetic Monitor](https://one.newrelic.com/synthetics) 페이지의 모니터 목록에서 추산해야 합니다. 올바른 뉴렐릭 계정을 선택했는지 확인하고 `privateLocation` 로 필터링해야 할 수도 있습니다.
...
런타임 크기를 조정할 때 고려해야 할 주요 사항은 단일 SJM 인스턴스의 최대 처리량이 **분당 약 15개의 중량 작업** (스크립트 API 및 브라우저 모니터)이라는 것입니다. 이는 SJM당 처리되는 작업의 순수한 개수보다 여러 SJM 간의 작업의 효율적인 경쟁을 선호하는 내부 스레딩 전략 때문입니다.

이 처리량 상한에 도달하기 전에 단일 SJM에 대한 최대 유효 `parallelism` 계산하려면 평균 작업 기간을 사용할 수 있습니다.

$$평행도\_최대 \약 15 \시간 \수학\_m$$

여기서 $\mathrmajd\_m$은 평균 작업 기간(분)입니다.

모니터링 요구 사항이 이 최대 15개 작업/분 제한을 초과하는 경우 구현하다, 배포하다 여러 SJM을 사용하여 **확장** 해야 합니다. [작업 대기열이 커지는지 확인하여](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-maintenance-monitoring/) 더 많은 인스턴스가 필요한지 알아볼 수 있습니다.

`parallelism` 설정은 특정 런타임의 파드가 동시에 실행되는 횟수를 제어하며, 도커 및 Podman SJM의 `HEAVYWEIGHT_WORKERS` 환경 변수와 동일합니다. `completions` 설정은 `CronJob` 이 해당 런타임에 대해 다른 Kubernetes 작업을 시작하기 전에 특정 런타임의 파드가 얼마나 완료되어야 하는지 제어합니다. 효율성을 향상시키려면 `completions` `parallelism` 값의 6\~10배로 설정해야 합니다.

다음 방정식은 각 런타임에 대한 `completions` 및 `parallelism` 의 시작점으로 사용할 수 있습니다.

$$완료 = \frac300ajd\_s$$

여기서 $\mathrmajd\_s$는 평균 작업 기간(초)입니다.

$$평행도 = \fracsj\_5m완료$$

여기서 $\mathrmsj\_5m$은 5분당 신세틱스 작업 수입니다.

다음 쿼리를 사용하여 상대 이동의 평균 지속 시간과 비율을 얻을 수 있습니다.
...
더 높은 총 처리량을 달성하려면 동일한 Kubernetes 네임스페이스에 여러 SJM Helm 릴리스를 설치할 수 있습니다. 각 SJM은 로드 밸런싱, 장애 조치 보호 및 증가된 총 작업 처리량을 제공하여 동일한 위치의 작업을 놓고 경쟁합니다.

여러 개의 SJM 릴리스를 설치하는 경우 각 릴리스에 고유한 이름을 제공해야 합니다. 모든 외부는 해당 `values.yaml` 파일에서 **동일한 독립 로케이션 키로** 구성되어야 합니다. 필수는 아니지만 더 짧고 관리하기 쉬운 리소스 이름을 만들려면 `fullnameOverride` 설정하는 것이 좋습니다.

예를 들어, `sjm-alpha` 및 `sjm-beta` 이라는 두 개의 SJM을 `newrelic` 네임스페이스에 설치하려면 다음을 수행합니다.
...
작업 대기열이 커지는 것을 막기 위해 필요한 만큼 많은 SJM에 대해 이 패턴을 계속할 수 있습니다. 각 SJM에 대해 평균 작업 기간과 인스턴스당 분당 약 15개 작업 제한을 기준으로 `parallelism` 및 `completions` 합리적인 값으로 설정합니다.