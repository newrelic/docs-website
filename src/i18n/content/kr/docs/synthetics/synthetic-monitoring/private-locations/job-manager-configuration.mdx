    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            미국 기반 계정(기본값)의 경우 엔드포인트는 다음과 같습니다: `https://synthetics-horde.nr-data.net.`
            ...
            JP 기반 계정의 경우 엔드포인트는 다음과 같습니다: `https://synthetics-horde.jp.nr-data.net/`
...
SJM에서는 사용자 정의 노드 모듈을 제공합니다. 이를 통해 사용자 지정 [노드 모듈](https://docs.npmjs.com/about-packages-and-modules) 세트를 만들고 스크립트 기반 모니터(스크립트 기반 API 및 스크립트 기반 브라우저)에서 신세틱 모니터링에 사용할 수 있습니다.
...
## 프라이빗 로케이션을 위한 사용자 지정 CA 인증서 구성 [#custom-ca-certificates]

신세틱 모니터가 사설 인증 기관(CA)으로 보호되는 내부 HTTPS 엔드포인트에 연결해야 하는 경우, 사용자 지정 CA 인증서를 SJM에 직접 마운트할 수 있습니다.

SJM은 시작 시 이러한 인증서를 자동으로 감지하여 각 런타임 컨테이너에 전달합니다. 이를 통해 사용자 지정 런타임 이미지를 빌드하거나 유지 관리할 필요 없이 모니터가 내부 엔드포인트를 신뢰할 수 있습니다.

### 인증서 요구 사항

SJM이 인증서를 성공적으로 로드하려면 다음 기준을 충족해야 합니다:

* **형식:** 인증서는 `.pem` 파일 확장자를 가진 PEM 형식이어야 합니다.
* **권한:** 파일은 UID 2000(런타임 컨테이너를 실행하는 루트가 아닌 사용자)이 읽을 수 있어야 합니다.
* **인증서 유형:** CA 인증서만 로드됩니다. `CA:TRUE` 기본 제약 조건 확장이 없는 모든 리프 인증서(서버 또는 클라이언트 인증서)는 자동으로 무시됩니다.

### 배포 환경

인증서를 마운트하려면 아래에서 컨테이너 오케스트레이션 플랫폼을 선택하세요.

<CollapserGroup>
  <Collapser id="custom-ca-docker" title="도커">
    PEM 파일이 포함된 로컬 호스트 디렉터리를 SJM 컨테이너 경로 `/var/lib/newrelic/synthetics/certs` 에 읽기 전용 볼륨으로 마운트합니다.
    ...
    **작동 방식:** SJM은 시작 시 이 마운트를 감지하고 생성하는 모든 동적 컨테이너에 인증서를 읽기 전용으로 전파합니다.
  </Collapser>

  <Collapser id="custom-ca-podman" title="포드만">
    Podman 환경 ― 특히 SELinux가 활성화된 상태에서 실행되는 환경 ― 의 경우 올바른 보안 컨텍스트를 구성하려면 볼륨 마운트에 `:z` 또는 `:Z` 플래그를 추가해야 합니다.
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes(Helm)">
    Helm을 통해 SJM을 쿠버네티스 클러스터에 배포할 때 사용자 지정 CA 번들을 일반 Kubernetes 시크릿으로 관리할 수 있습니다.

    **1. Kubernetes 시크릿 생성**

    뉴렐릭 에이전트 네임스페이스 내의 시크릿에 PEM 인증서 파일을 저장합니다:
    ...
    **2. Helm 값 업데이트**
    ...
    사용자 지정 `values.yaml` 설정에서 새로 생성된 시크릿을 참조합니다:
    ...
    **작동 방식:** 시작 시 SJM은 이 시크릿을 마운트하고 새로 생성된 모든 실행 파드에 자동으로 전달합니다.
  </Collapser>
</CollapserGroup>

## mTLS 클라이언트 인증서 [#mtls-client-certificates]

모니터가 상호 TLS(mTLS)를 적용하는 내부 엔드포인트에 연결되는 경우 ― 클라이언트가 인증서를 제시해야 함 ― 타겟 호스트명에 매핑된 클라이언트 인증서를 제공할 수 있습니다. 모니터는 올바른 인증서를 자동으로 제시합니다. 스크립트 변경은 필요하지 않습니다.

**요구 사항:**

* 인증서 및 키 파일은 PEM 형식이어야 합니다
* 호스트명을 인증서 및 키 파일에 매핑하는 `cert-map.json` 파일이 필요합니다
...
호스트명 일치 우선순위: 정확한 일치 → 와일드카드(`*.domain.com`) → `default` 캐치올 → 인증서 없음(비 mTLS 연결은 영향을 받지 않음).

<CollapserGroup>
  <Collapser id="mtls-docker" title="도커">
    `cert-map.json` 및 참조된 인증서와 키 파일이 포함된 디렉터리를 마운트하십시오:
    ...
    런타임 컨테이너 내부의 DNS를 통해 mTLS 서버 호스트명을 확인할 수 없는 경우 `RUNTIME_EXTRA_HOSTS`을(를) 사용하여 호스트명 매핑을 추가하십시오:
  ...
  <Collapser id="mtls-podman" title="포드만">
    클라이언트 인증서 디렉터리를 마운트하십시오. SELinux의 경우 `:z` 또는 `:Z`을(를) 추가하십시오:
  ...
  <Collapser id="mtls-kubernetes" title="Kubernetes">
    `cert-map.json` 및 참조된 모든 인증서와 키 파일이 포함된 Kubernetes 시크릿을 생성하십시오:
    ...
    시크릿은 모든 런타임 파드에 읽기 전용으로 자동 마운트됩니다.
  </Collapser>
</CollapserGroup>

## 인증서 지원 제한 사항 [#certificates-support-limitations]

별도로 명시되지 않는 한 다음 제한 사항은 사용자 지정 CA 인증서 및 mTLS 클라이언트 인증서 모두에 적용됩니다:

* **인증서 변경 시 재시작 필요** ― 인증서는 컨테이너 시작 시 로드됩니다. 인증서를 추가, 제거 또는 교체한 후 변경 사항을 적용하려면 작업 관리자 컨테이너(또는 Kubernetes의 런타임 파드)를 재시작하십시오.
* **지원되는 런타임** ― 사용자 지정 인증서 기능은 스크립트 API 및 스크립트 브라우저 모니터에만 적용됩니다. Ping 모니터는 영향을 받지 않습니다.
* **인증서당 고유한 주체 CN(mTLS 전용)** ― 각 클라이언트 인증서에는 고유한 주체 일반 이름(CN)이 있어야 합니다. Chrome은 CN을 사용하여 호스트명별로 올바른 인증서를 식별하고 자동 선택합니다.
...
<CollapserGroup>
  <Collapser id="docker-storage" title="도커용">
  ...
  <Collapser id="podman-storage" title="Podman의 경우">
  ...
  <Collapser id="kubernetes-storage" title="Kubernetes용">
    ### Kubernetes
...
<CollapserGroup>
  <Collapser id="docker-podman-sizing" title="도커 및 Podman용">
    ...
    * $R\_\{growth}$ = `jobManagerHeavyweightJobs` 큐가 분당 **증가하는** **속도** 입니다.
    ...
    * $D\_\{avg,m}$ = 대용량 작업의 **평균 소요 시간(** **분)**.
    * $P\_\{avg,m}$ = 무거운 모니터의 **평균 주기(** **분)** (예: 5분마다 작동하는 모니터는 $P\_\{avg,m} = 5$입니다).
    ...
    **2. 분당 대기열 증가율($R\_\{growth}$) 찾기**: 이 쿼리는 시계열 차트에서 `jobManagerHeavyweightJobs` 대기열의 분당 평균 증가율을 계산합니다. 선이 0보다 위에 있으면 대기열이 증가하고 있음을 나타내고, 선이 0보다 아래에 있으면 대기열이 감소하고 있음을 나타냅니다.
    ...
    **4. 평균 작업 지속 시간(분) 찾기($D\_\{avg,m}$)**: 이 쿼리는 완료된 비핑 작업의 평균 실행 지속 시간을 찾고 결과를 밀리초에서 분으로 변환합니다. `executionDuration` 호스트에서 작업이 실행되는 데 걸린 시간을 나타냅니다.
    ...
    **5. 평균 헤비급 감시 기간($P\_\{avg,m}$) 찾기:** 다루로케이션의 `jobManagerHeavyweightJobs` 대기열이 증가하는 경우 기존 결과에서 평균 감시 기간을 계산하는 것은 정확하지 않습니다. 이는 [합성 모니터](https://one.newrelic.com/synthetics) 페이지에 있는 모니터 목록을 참고하여 추정해야 합니다. 올바른 뉴렐릭 계정을 선택했는지 확인하고 `privateLocation` 로 필터링해야 할 수도 있습니다.
  ...
  <Collapser id="k8s-sizing" title="Kubernetes 및 OpenShift용">
    ...
    **예상 병렬 처리량:** 이는 5분 작업 부하를 처리하는 데 필요한 동시 실행 작업자(워커) 수를 결정합니다.
    ...
    여기서 $N\_m$은 **5분당 작업 수** 입니다. 이 $P\_\{est}$ 값은 **예상 병렬 처리량** 입니다.
    ...
    **3. 대기열 증가를 모니터링합니다.**
    ...
    작업을 상대 로케이션으로 보내도록 구성된 합성 모니터를 사용하여 `pingJobs` 및 `jobManagerHeavyweightJobs` 에 대한 시계열 선 차트에서 대기열 증가를 확인합니다.

    * `pingJobs` 큐의 기울기가 양수이면 `ping-runtime.replicaCount` 증가시키고 재배포합니다.
    * `jobManagerHeavyweightJobs` 큐의 기울기가 양수이면 큐가 더 이상 증가하지 않을 때까지(음의 기울기) `parallelism` 과 `completions` 비례적으로 증가시킵니다.
    ...
    **4. 파드 실행 상태에 따라 튜닝**
    ...
    큐가 줄어들거나 0이 되면 10분 이상 &quot;실행 중&quot; 상태인 `node-api-runtime` 및 `node-browser-runtime` 파드를 확인합니다. 이는 병렬 처리 수준이 너무 높게 설정되어 있어 필요한 것보다 파드가 더 많다는 것을 나타냅니다.

    불필요한 자원 낭비를 방지하기 위해 `parallelism` 과 `completions` 감소시켜 각 &quot;실행 중인&quot; 런타임 파드의 수명을 줄이십시오. Kubernetes 작업의 실행 시간을 5분으로 설정했다면, 런타임 파드는 5분 이내에 실행 상태를 유지해야 합니다. 즉, 파드가 생성되자마자 실행할 작업을 빠르게 수신하고 완료되어야 합니다.
    ...
    **5. 필요한 경우 규모를 확장하십시오.**
    ...
    구현, 배포 추가에 대한 자세한 내용은 [여러 SJM 구현, 배포를 사용하여 확장을](#scaling-out-with-multiple-sjm-deployments) 참조하세요.
    ...
    단일 SJM의 15 작업/분 처리량을 넘어 확장하려면 **여러 개의 별도 SJM Helm 릴리스**를 설치해야 합니다.
    ...
    1. **병렬 처리 설정:** *각* SJM에 대해 `parallelism` 단일 SJM이 너무 많은 장기 실행 런타임 파드를 생성하지 않고 처리할 수 있는 최대값으로 설정합니다. 이를 통해 자원을 낭비하지 않고 각 SJM의 잠재적 처리량을 극대화할 수 있습니다.
    2. **집합 완성:** *각* SJM에 대해 `completions` 도 동일한 고정 값으로 설정합니다. 필요에 따라 조정하여 대상, 목표를 런타임별 Kubernetes 작업 유지 시간 5분(예: node-browser-runtime 및 node-api-runtime)으로 설정합니다.
    ...
    더 짧고 관리하기 쉬운 리소스 이름을 만들려면 `fullnameOverride` 을 설정하는 것이 좋습니다. 예를 들어, `sjm-alpha` 과 `sjm-beta` 라는 이름의 SJM 두 개를 `newrelic` 네임스페이스에 설치하려면 (둘 다 고정된 병렬 처리 및 자동 완성 기능을 사용하는 동일한 `values.yaml` 사용함):
