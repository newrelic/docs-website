...
**1. 분당 처리된 작업 속도 찾기($R\_proc$):** 이 쿼리는 지난 하루 동안 완료된 핑이 없는(중량급) 작업의 수를 세고 분당 평균 속도를 보여줍니다.
...
**2. 분당 대기열 증가율($R\_growth$) 찾기:** 이 쿼리는 시계열 차트에서 `jobManagerHeavyweightJobs` 대기열의 분당 평균 증가율을 계산합니다. 0보다 큰 선은 대기열이 늘어나고 있음을 나타내고, 0보다 작은 선은 대기열이 줄어들고 있음을 나타냅니다.
...
**3. 중량 모니터의 총 수($N\_mon$) 찾기:** 이 쿼리는 중량 모니터의 고유한 수를 찾습니다.
...
**4. 평균 작업 기간(분) 찾기($D\_avg,m$):** 이 쿼리는 완료된 비핑 작업의 평균 실행 기간을 찾고 결과를 밀리초에서 분으로 변환합니다. `executionDuration` 호스트에서 작업을 실행하는 데 걸린 시간을 나타냅니다.
...
$$평행도*최대 \약 15 \times D*avg,m$$

여기서 $D\_avg,m$은 **분 단위** 의 **평균 중량 작업 기간** 입니다.
...
여기서 $D\_avg,s$는 **평균 작업 기간** **초** 입니다.

$$평행도 = \fracN\_m완료$$

여기서 $N\_m$은 5분마다 실행해야 하는 합성 작업의 **수** 입니다 **.**
