    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            배포할 Node API 런타임 `CronJobs` 의 수입니다. 언제든지 실행할 최대 동시 Node API 작업 수입니다. [추가 세부정보](#kubernetes-sizing) .
        ...
        <tr>
          ...
          <td>
            분당 완료할 Node API 런타임 `CronJobs` 의 수입니다. 처리량을 향상시키려면 병렬 처리와 함께 이 설정을 늘리십시오. 병렬 처리가 증가할 때마다 이 값을 늘려야 하며 완료는 항상 병렬 처리보다 크거나 같아야 합니다. . API 런타임 작업이 실행되지 않는 기간이 있는 경우 이 설정을 늘리십시오. [추가 세부정보](#kubernetes-sizing) .
...
<CollapserGroup>
  ...
  >
    ...
    Node API 및 Node Browser 런타임은 `parallelism` 및 `completions` 설정의 조합을 사용하여 독립적으로 크기가 조정됩니다. 이러한 설정에 대한 이상적인 구성은 고객 요구 사항에 따라 달라집니다.

    `parallelism` 설정은 K8 클러스터에서 동시에 실행할 수 있는 해당 유형(노드 API 또는 노드 브라우저)의 작업 수를 제어합니다. `parallelism` 설정은 컨테이너화된 비공개 미니언(CPM)의 `synthetics.heavyWorkers` 구성과 동일합니다. K8s 클러스터에 [리소스 요청 및 제한 값](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#kubernetes-requirements) 을 기반으로 이 수의 동시 런타임을 실행하는 데 사용할 수 있는 리소스가 충분한지 확인하십시오.
