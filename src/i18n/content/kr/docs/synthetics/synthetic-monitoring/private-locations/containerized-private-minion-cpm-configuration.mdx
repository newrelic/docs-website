...
이 문서는 [컨테이너화된 프라이빗 미니언 (CPM)](/docs/install-containerized-private-minions-cpms) 구성 과정을 안내합니다.

다음을 수행하여 CPM을 사용자 정의할 수 있습니다.

* [환경 변수를](#environment-variables) 사용하여 컨테이너화된 프라이빗 미니언을 구성하세요.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**REQUIRED.**</DNT> 레지스터 로케이션 웹 페이지에서 찾을 수 있는 등록 로케이션의 UUID입니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            CPM이 모니터를 제공하기 위해 적절한 엔드포인트에 연결할 수 있는지 확인하세요.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            공용 인터넷 액세스에 대한 CPM 확인을 관리하기 위해 Minion Network Healthcheck가 비활성화된 상태입니다.기본값은 &apos;false&apos;이며, &apos;true&apos;로 설정하면 CPM이 이 상태 확인을 우회합니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            사용자가 정의한 핵심 가치 쌍의 로컬 호스팅 세트입니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**REQUIRED if `synthetics.privateLocationKeySecretName` is not set**</DNT>. 독립 로케이션 웹페이지에서 찾을 수 있는 UUID/[제국 로케이션 키입니다](/docs/synthetics/synthetic-monitoring/private-locations/install-containerized-private-minions-cpms#private-location-key) .
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            `user-defined-variable.json` 파일에 대한 볼륨의 경로입니다. 자세한 내용은 [사용자 정의 변수를](/docs/synthetics/synthetic-monitoring/private-locations/containerized-private-minion-cpm-configuration#vars-scripted-monitors) 참조하세요.
          </td>
        </tr>
...
모든 디렉터리 및 파일 <DNT>**must**</DNT> 에는 읽기/쓰기 권한이 있는 `3729` 그룹 소유권이 할당됩니다. 이렇게 하면 `uid: 1000` 및 `gid: 3729` 사용하는 Runner가 마운트된 모든 볼륨에 액세스할 수 있습니다. 그러나 미니언은 루트(`uid: 0`) 또는 `[2000, 4000]` 범위(포함) 사이의 모든 `uid` 로 실행될 수 있습니다. 자세한 내용은 [Kubernetes](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#kubernetes-env-config) 또는 [Docker 에서 루트가 아닌 사용자 로 실행을 참조하세요.](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#docker-env-config)
...
* `kubectl cp` 사용하여 영구 볼륨(<DNT>**PV**</DNT>)에 디렉터리를 추가할 수 있습니다. 그러나 파일 권한이 적절하게 설정되어 있는 한 대체 접근 방식이 지원됩니다.
...
1. [npm 공식 지침에](https://docs.npmjs.com/files/package.json) 따라 디렉토리 루트에 `package.json` 을 포함하는 디렉토리를 만듭니다. `dependencies` 필드에 포함된 모든 항목은 시작 시 CPM에 의해 설치되며 해당 개인 미니언에서 모니터를 실행할 때 사용할 수 있게 됩니다.
   ...
   <CollapserGroup id="custom-module-dir">
     <Collapser id="example-module-directory" title="사용자 정의 모듈 디렉토리">
     ...
     <Collapser id="example-overrides" title="Node.js 버전별 재정의">
       루트 수준 `package.json` 을 재정의하는 Node.js 버전별로 `package.json` 선언할 수 있습니다. 이를 통해 [런타임의 Node.js 버전이](/docs/synthetics/new-relic-synthetics/scripting-monitors/scripted-monitor-runtime-environment) 더 이상 의존성/종속성과 호환되지 않는 이벤트에서 모니터 런타임별로 모니터 펼쳐보기를 업데이트할 수 있습니다. [첫 번째 예](#example-module-directory) 에서 볼 수 있듯이 로컬 모듈은 여전히 버전별 디렉터리 내에서 정의될 수 있습니다. 특정 Node.js 버전에 대해 `package.json` 가 정의되지 않은 경우 루트 수준 `package.json` 의존성/종속성을 설치하는 데 사용됩니다.
...
2. 사용자 정의 모듈 디렉토리와 `package.json` 을 생성하면 이를 Docker 및 Kubernetes용 CPM에 적용할 수 있습니다.
   ...
   <CollapserGroup id="apply-to-docker-kubernetes">
     ...
     <Collapser id="kubernetes" title="Kubernetes">
       ...
       2. 미니언 패드에서 사용자 정의 모듈 디렉토리를 사용할 수 있는지 확인하세요. `kubectl cp` 을 호스트에서 미니언으로 복사하는 한 가지 방법으로 사용할 수 있습니다. 예를 들어:
...
로컬 및 호스팅 모듈과 함께 [Node.js 모듈을](/docs/synthetics/new-relic-synthetics/scripting-monitors/import-nodejs-modules) 사용할 수도 있습니다. CPM에서 사용하는 맞춤 모듈을 변경하려면 `package.json`를 수정하고 CPM을 재부팅하세요. 재부팅하는 동안 설정 변경 사항을 감지한 다음 정리하고 다시 설치합니다.
