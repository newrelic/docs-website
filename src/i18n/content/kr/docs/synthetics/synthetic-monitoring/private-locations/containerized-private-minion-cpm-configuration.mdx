...
모든 디렉터리 및 파일 <DoNotTranslate>**must**</DoNotTranslate> 에는 읽기/쓰기 권한이 있는 `3729` 그룹 소유권이 할당됩니다. 이렇게 하면 `uid: 1000` 및 `gid: 3729` 사용하는 Runner가 마운트된 모든 볼륨에 액세스할 수 있습니다. 그러나 미니언은 루트(`uid: 0`) 또는 `[2000, 4000]` 범위(포함) 사이의 모든 `uid` 로 실행될 수 있습니다. 자세한 내용은 [Kubernetes](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#kubernetes-env-config) 또는 [Docker 에서 루트가 아닌 사용자 로 실행을 참조하세요.](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#docker-env-config)
