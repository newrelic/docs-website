...
사용자 지정 npm 모듈은 CPM 전용입니다. 이를 통해 임의의 [npm 모듈](https://docs.npmjs.com/about-packages-and-modules) 세트를 제공하고 Synthetics의 스크립팅된 모니터에서 사용할 수 있습니다.
...
1. [npm 공식 가이드라인](https://docs.npmjs.com/files/package.json) 에 따라 디렉토리 루트에 `package.json` 이 포함된 디렉토리를 생성합니다. `dependencies` 필드에 포함된 모든 항목은 시작 시 CPM에 의해 설치되고 해당 비공개 미니언에서 모니터를 실행할 때 사용할 수 있습니다.
   ...
   선택적으로 루트 수준 `package.json` 을 노드 버전별 디렉터리로 재정의할 수 있습니다. 이렇게 하면 런타임의 [노드 버전](/docs/synthetics/new-relic-synthetics/scripting-monitors/scripted-monitor-runtime-environment) 이 더 이상 종속성과 호환되지 않는 경우 모니터 런타임별로 스크립트를 업데이트할 수 있습니다. 아래에서 이에 대한 예를 참조하십시오.
   ...
   <CollapserGroup id="custom-module-dir">
     ...
     <Collapser
       ...
       title="노드 버전별 재정의"
     >
       루트 수준 `package.json` 을 재정의할 노드 버전당 `package.json` 를 선언할 수 있습니다. 이렇게 하면 런타임의 [노드 버전](/docs/synthetics/new-relic-synthetics/scripting-monitors/scripted-monitor-runtime-environment) 이 더 이상 종속성과 호환되지 않는 경우 모니터 런타임별로 모니터 스크립트를 업데이트할 수 있습니다. [첫 번째 예](#example-module-directory) 에서 볼 수 있듯이 로컬 모듈은 여전히 버전별 디렉토리 내에서 정의할 수 있습니다. 특정 노드 버전에 대해 `package.json` 가 정의되지 않은 경우 루트 수준 `package.json` 이 종속성을 설치하는 데 사용됩니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            **`synthetics.privateLocationKey` 이 설정되지 않은 경우 필수입니다** . Synthetics 개인 위치와 연결된 인증 키가 포함된 `privateLocationKey` 키가 포함된 Kubernetes 비밀의 이름입니다.
          </td>
        </tr>
