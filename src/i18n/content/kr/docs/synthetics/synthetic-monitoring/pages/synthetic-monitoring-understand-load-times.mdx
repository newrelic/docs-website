...
모니터의 **리소스** 페이지에 액세스하려면: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) >Synthetics > (모니터 선택) > Resources** 로 이동합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Synthetics > (모니터 선택) > Resources > (리소스 선택) > (리소스 로드 선택)** : 개별 리소스 로드를 선택하면 연결 시간, 요청 헤더 및 응답 헤더에 대한 자세한 분석을 볼 수 있습니다. [HTTP 아카이브 1.2 사양](http://www.softwareishard.com/blog/har-12-spec/#timings) 에서 가져온 네트워크 타이밍 구조.
</figcaption>
