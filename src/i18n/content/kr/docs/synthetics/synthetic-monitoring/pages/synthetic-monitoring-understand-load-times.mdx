...
모니터의 **Resources** \[리소스] 페이지에 액세스하려면 **[one.newrelic.com](https://one.newrelic.com/synthetics-nerdlets) > Synthetic monitoring > (모니터 선택) > Resources** 로 이동하세요.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/synthetics-nerdlets) > Synthetic monitoring > (모니터 선택) > Resources**: **Resources** \[리소스] 페이지를 사용하여 웹 사이트의 각 리소스 성능을 분석하고 [개별 리소스 로드](#resource-results) 에 대한 자세한 지표에 액세스합니다.
</figcaption>
...
* **Load time** \[로드 시간]: 모든 리소스의 평균 로드 시간을 리소스 유형(예: `jpeg`, `json` 또는 `javascript`)별로 분류하여 그래프로 표시합니다. 이 그래프를 사용하여 시간이 지남에 따라 웹사이트의 로드 시간이 어떻게 변하는지 이해하세요.
