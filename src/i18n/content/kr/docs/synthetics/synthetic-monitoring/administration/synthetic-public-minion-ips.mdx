...
<Callout variant="important">
  ...
  **2025년 5월 14일** 까지 허용 목록을 업데이트하지 않으면 신세틱스 검사가 시스템에 연결하지 못할 수 있으며, 이로 인해 연결이 실패하고 알림이 발생할 수 있습니다.
</Callout>
...
<Callout variant="important">
  ...
  **2025년 5월 14일** 이후 JSON의 IP 범위 목록이 업데이트되어 더 이상 사용되지 않는 이전 IP 범위가 제거됩니다. 이 날짜 전에 새로운 IP 범위를 추가하고 기존 IP 범위를 제거하여 허용 목록을 업데이트하는 것이 중요합니다.
