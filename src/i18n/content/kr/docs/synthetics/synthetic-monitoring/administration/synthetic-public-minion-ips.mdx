...
<Callout variant="important">
  뉴렐릭에서 제공하는 전체 전역 범위를 사용자 목록에 추가하는 것이 좋습니다. JSON 파일이 업데이트되어 새로운 IP 범위에 대한 지역별 분류가 포함되었습니다. 새로운 목록에 언급되지 않은 지역은 계속 기존 IP를 사용합니다.
  ...
  **2025년 4월 14일** 이후, JSON의 IP 범위 목록이 업데이트되어 더 이상 사용되지 않는 기존 IP 범위가 제거됩니다. 이 날짜 전에 새로운 IP 범위를 추가하고 기존 IP 범위를 제거하여 허용 목록을 업데이트하는 것이 중요합니다.
