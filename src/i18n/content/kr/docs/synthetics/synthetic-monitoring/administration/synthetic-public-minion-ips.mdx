---
title: 합성 모니터 공용 미니이온 IP
tags:
  - Synthetics
  - Synthetic monitoring
  - Administration
metaDescription: 'Ensure New Relic can monitor your services by adding synthetic monitoring''s IPs and DNS names to your allow list, based on a JSON file.'
translationType: machine
---

New Relic은 공개 미니언을 사용하여 [합성 모니터](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors) 를 실행합니다. 이 미니언은 전 세계의 다양한 데이터 센터에 배치되어 모니터 실행을 담당합니다.

이 때문에 방화벽이 동기화 네트워크 요청을 허용하는지 확인하십시오. 미니언 IP 주소는 JSON 형식으로 공용으로 사용할 수 있으므로 쉽게 읽고 구문 분석할 수 있다.

**선택적 단계** :

* 이러한 파일을 기반으로 허용 목록에 대한 규칙을 자동화할 수 있습니다.
* [사용자 지정 헤더](/docs/synthetics/new-relic-synthetics/administration/identify-synthetics-requests-your-app) 를 사용하여 허용 목록에 IP 주소를 추가할 수 있습니다.
* S3 URL 경로에서 `production` 은 미국 기반 계정을 나타내고 `eu` 은 [EU 기반 계정을](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers) 나타냅니다.

## IP 주소는 개인 데이터가 아닙니다. [#not-personal-data]

미니언즈는 서버에 배치되며 에이전트는 비개인 신임 정보를 사용하여 활성화됩니다. 서버에서 실행되는 미니언즈와 관련된 IP 주소는 데이터 보호 및 개인정보 보호법에 따라 개인 데이터가 아닙니다. 자세한 정보는 [합성 보안 문서](/docs/synthetics/new-relic-synthetics/getting-started/security-new-relic-synthetics)를 참조하십시오.

## IP 주소에 대한 매일 JSON 목록 [#dynamic-ip-addresses]

릴리스된 위치에 대한 IP 주소는 변경될 수 있습니다. 변경이 필요한 경우 이메일을 통해 변경하기 전에 미리 고객에게 알리려고 합니다. 또한 업데이트를 위해 [탐색 허브](https://discuss.newrelic.com/) 를 확인할 수 있습니다.

<Callout variant="important">
  [새 IP 주소 범위](/docs/synthetics/synthetic-monitoring/administration/new-synthetic-public-minion-ips) 에서는 방화벽 설정을 다시 구성해야 합니다.
</Callout>

<table style={{ width: "300px" }}>
  <thead>
    <tr>
      <th>
        미국 계정
      </th>
    </tr>

    <tr>
      <td>
        * [IP 전용 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/production/ip.json)
        * [DNS 이름-전용 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/production/dnsname.json)
        * [IP및 DNS 이름 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/production/ip-dnsname.json)
        * [IP 범위 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/production/ip-ranges.json)
      </td>
    </tr>
  </thead>
</table>

<table style={{ width: "300px" }}>
  <thead>
    <tr>
      <th>
        EU 계정
      </th>
    </tr>

    <tr>
      <td>
        * [IP 전용 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/eu/ip.json)
        * [DNS 이름-전용 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/eu/dnsname.json)
        * [IP및 DNS 이름 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/eu/ip-dnsname.json)
        * [IP 범위 목록](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/eu/ip-ranges.json)
      </td>
    </tr>
  </thead>
</table>

## 공용 미니이온 위치 및 위치 레이블 [#location]

다음 표는 합성의 공개 미니언 위치와 해당 위치 레이블을 상호 참조합니다. [SyntheticCheck](/docs/insights/explore-data/attributes/synthetics-default-attributes-insights#syntheticcheck-table) 및 [SyntheticRequest](/docs/insights/explore-data/attributes/synthetics-default-attributes-insights#syntheticrequest-table) 이벤트에서 `location` 및 `locationLabel` 속성을 [쿼리](/docs/query-your-data) 할 수 있습니다.

<table>
  <thead>
    <tr>
      <th style={{ width: "300px" }}>
        공개 미니언 위치
      </th>

      <th>
        위치 레이블
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        AWS_AP_EAST_1
      </td>

      <td>
        "홍콩, HK"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AP_SOUTH_1
      </td>

      <td>
        "Mumbai, IN"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AP_SOUTHEAST_1
      </td>

      <td>
        "싱가포르, SG"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AP_NORTHEAST_2
      </td>

      <td>
        "서울, KR"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AP_NORTHEAST_1
      </td>

      <td>
        "도쿄, JP"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AP_SOUTHEAST_2
      </td>

      <td>
        "시드니, AU"
      </td>
    </tr>

    <tr>
      <td>
        AWS_US_WEST_1
      </td>

      <td>
        "San Francisco, CA, USA"
      </td>
    </tr>

    <tr>
      <td>
        AWS_US_WEST_2
      </td>

      <td>
        "미국 오리건주 포틀랜드"
      </td>
    </tr>

    <tr>
      <td>
        AWS_US_EAST_2
      </td>

      <td>
        "Columbus, OH, USA"
      </td>
    </tr>

    <tr>
      <td>
        AWS_US_EAST_1
      </td>

      <td>
        "Washington, DC, USA"
      </td>
    </tr>

    <tr>
      <td>
        AWS_CA_CENTRAL_1
      </td>

      <td>
        "몬트리올, 퀘벡, CA"
      </td>
    </tr>

    <tr>
      <td>
        AWS_SA_EAST_1
      </td>

      <td>
        "상파울루, BR"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_WEST_1
      </td>

      <td>
        "더블린, IE"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_WEST_2
      </td>

      <td>
        "런던, 영국, 영국"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_WEST_3
      </td>

      <td>
        "프랑스 파리"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_CENTRAL_1
      </td>

      <td>
        "프랑크푸르트, DE"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_NORTH_1
      </td>

      <td>
        "Stockholm, SE"
      </td>
    </tr>

    <tr>
      <td>
        AWS_EU_SOUTH_1
      </td>

      <td>
        "밀란, IT"
      </td>
    </tr>

    <tr>
      <td>
        AWS_ME_SOUTH_1
      </td>

      <td>
        "Manama, BH"
      </td>
    </tr>

    <tr>
      <td>
        AWS_AF_SOUTH_1
      </td>

      <td>
        "케이프타운, ZA"
      </td>
    </tr>
  </tbody>
</table>