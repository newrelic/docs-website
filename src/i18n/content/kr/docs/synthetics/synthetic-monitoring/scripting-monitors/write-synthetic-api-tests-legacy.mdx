...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate>: 펼쳐보기 편집기는 [펼쳐보기 명령을 단순화하기 위한 기능, 선택기 및 기타 요소를 제안합니다(GitHub에서 사용 가능)](https://github.com/request/request).
</figcaption>
...
<DoNotTranslate>**For SSL and agentOptions:**</DoNotTranslate> SSL 옵션을 설정하거나 [`agentOptions`](https://github.com/request/request#using-optionsagentoptions) 객체를 제공하는 경우 HTTP 요청이 계측된 글로벌 에이전트를 사용하도록 하려면 요청 `options` 객체의 `agent` 속성을 `$globalAgents.https` 또는 `$globalAgents.http` 로 설정해야 합니다.
