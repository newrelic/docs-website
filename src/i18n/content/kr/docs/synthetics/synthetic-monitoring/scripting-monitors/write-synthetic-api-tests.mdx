...
스크립팅된 API 모니터는 API 엔드포인트를 검사하여 올바르게 작동하는지 확인합니다. 스크립트된 API 모니터를 생성하려면 <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create a monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate> 으로 이동한 다음 <DoNotTranslate>**Endpoint availability**</DoNotTranslate> 타일을 선택합니다.
