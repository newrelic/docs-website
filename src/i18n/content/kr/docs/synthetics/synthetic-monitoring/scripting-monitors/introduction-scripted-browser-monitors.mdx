...
스크립트 브라우저는 웹 사이트를 탐색하고, 특정 작업을 수행하고, 특정 요소가 있는지 확인하는 스크립트 브라우저를 통해 사용자 정의 사용자 경험을 에뮬레이트합니다. 스크립트 브라우저를 생성하려면 <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create a monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate> 으로 이동한 다음 <DoNotTranslate>**User flow / functionality**</DoNotTranslate> 타일을 선택하세요.

사용 가능한 모든 기능 목록은 [스크립트 브라우저 참조를](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100/) 참조하세요.

스크립트 모니터는 \[셀레니엄 WebDriverJS]([https://www.selenium.dev/documentation/webdriver/ )](https://www.selenium.dev/documentation/webdriver/) 에 의해 구동됩니다. 펼쳐보기가 실행될 때마다 뉴렐릭은 웹사이트를 탐색하고 각 펼쳐보기 단계를 따르는 완전히 가상화된 셀레니엄 기반 Google Chrome 브라우저를 만듭니다. 신세틱 모니터링에는 펼쳐보기를 단순화하기 위한 기능, 로케이터 및 기타 요소를 제안하는 IDE 스타일의 펼쳐보기 편집기가 포함되어 있습니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="HTML ID로 찾기"
  >
    <DoNotTranslate>**exact**</DoNotTranslate> HTML `id` (예: `id="edit-submit"`)로 요소를 찾습니다. 이는 페이지 요소를 찾는 간단한 방법이지만 변경될 가능성이 없는 `id` 을 지정하는 데 주의하세요.
  ...
  <Collapser
    ...
    title="링크 텍스트로 찾기"
  >
    표시된 링크 텍스트로 요소를 찾습니다. 예를 들어, `<a href="http://example.com>your link text here</a>`. <DoNotTranslate>**exact**</DoNotTranslate> 링크 텍스트(`$driver.By.linkText`) 또는 부분 일치(`$driver.By.partialLinkText`)로 찾을 수 있습니다.
  ...
  <Collapser
    ...
    title="이름으로 찾기"
  >
    <DoNotTranslate>**exact**</DoNotTranslate> HTML `name` (예: `name="search-query-field"`)로 요소를 찾습니다. 이는 검색 창과 같은 입력 필드에 가장 일반적으로 사용됩니다.
