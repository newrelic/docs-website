...
<CollapserGroup id="non-scripted-errors">
  <Collapser className="freq-link" id="exceed-duration" title="오류: 65초 후 작업 시간이 초과되었습니다.">
    ### 문제
  ...
  <Collapser className="freq-link" id="host-connect-timeout" title="네트워크 오류: (HOST) [HOST./IP ADDRESS]에 연결 실패: 연결 시간 초과">
    ### 문제
  ...
  <Collapser className="freq-link" id="host-connect-refused" title="네트워크 오류: (HOST) [HOST./IP ADDRESS]에 연결 실패: 연결이 거부되었습니다.">
    ### 문제
  ...
  <Collapser className="freq-link" id="xxx-response" title="HTTPError: 서버가 HTTP XXX 응답 코드로 응답했습니다.">
    ### 문제
  ...
  <Collapser className="freq-link" id="ssl-error" title="SSLVerificationError: (오류)">
    ### 문제
    ...
    <DNT>**[one.newrelic.com &gt; Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**</DNT> &gt; (모니터 선택) &gt; <DNT>**General &gt; Advanced options**</DNT> 로 이동한 다음 <DNT>**Verify SSL check**</DNT> 를 비활성화합니다.

    ### 원인

    SSLVerificationError 실패는 선택적 <DNT>**Verify SSL check**</DNT> 이 대상, 대상 호스트에 대해 실패한 결과입니다.
  ...
  <Collapser className="freq-link" id="response-error" title="ResponseValidationError: 응답에 예상 문자열이 포함되어 있지 않습니다.">
    ### 문제
  ...
  <Collapser className="freq-link" id="networkerror-timeout" title="NetworkError: 읽기 시간 초과">
    ### 문제
  ...
  <Collapser className="freq-link" id="networkerror-socket-closed" title="NetworkError: 소켓이 닫혔습니다.">
    ### 문제
  ...
  <Collapser className="freq-link" id="networkerror-host-unreachable" title="NetworkError: 호스트에 대한 경로 없음(호스트에 연결할 수 없음)">
    ### 문제
    ...
    신세틱 모니터링의 배치 위치에서 오류가 발생하는 경우 신세틱스 Job Manager의 네트워크 설정이 제대로 구성되어 있는지, 타겟 호스트명이 신세틱스 Job Manager 가상 배열 인터페이스를 통해 해결 및 접근 가능한지 확인하세요.
  ...
  <Collapser className="freq-link" id="httperror-redirects" title="HTTPError: 서버가 너무 많은 리디렉션을 보냈습니다(20).">
    ### 문제
  ...
  <Collapser className="freq-link" id="networkerror-resolution" title="NetworkError: 호스트에 대한 DNS 확인 실패: (HOST)">
    ### 문제
    ...
    <DNT>**Private synthetic monitoring’s locations:**</DNT> 신세틱스 Job Manager의 네트워크 설정이 올바른지 확인하세요. 타겟, 목표호명이 내부 호스트인 경우, 신세틱스 Job Manager가 호스트를 확인할 수 있는 네트워크의 내부 DNS 서비스를 사용하고 있는지 확인하십시오. 신세틱스 Job Manager 및 호스트에서 생성되는 러너 컨테이너(Ping이 아닌 검사를 실행하기 위해)는 호스트 `/etc/resolv.conf` 에서 DNS 설정을 상속해야 합니다.

    <DNT>**Docker:**</DNT> 신세틱스 Job Manager의 도커 실행 명령에 사용되는 `–dns` 또는 `-network` 과 같은 네트워크 인수는 작업 관리자 컨테이너에서만 사용되며 러너 컨테이너에서는 사용되지 않습니다. DNS가 `127.0.0.1` 와 같은 루프백 인터페이스를 가리키는 경우 [도커 데몬 수준](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-configuration-file) 에서 DNS 구성을 정의하거나 `dnsmasq` 과 같은 도구를 사용하여 러너가 도커 브리지 인터페이스에서 DNS requests 전달하도록 합니다.

    <DNT>**Public synthetic monitoring locations:**</DNT> 타겟, 타겟 사이트의 DNS 레코드가 Google 퍼블릭 DNS 및 Amazon에서 제공하는 DNS와 같은 공용 DNS 서비스에서 조회될 수 있는지 확인하십시오.
    ...
    신세틱 관찰 기록 위치에서 DNS 확인 관련 모니터 오류가 나타나는 경우, 이는 해당 위치에 대한 신세틱스 작업 관리자의 잘못된 네트워크 설정으로 인해 발생하는 경우가 많습니다.
  </Collapser>

  <Collapser className="freq-link" id="blockedrequesterror-url" title="BlockedRequestError: (URL)">
    ### 문제
