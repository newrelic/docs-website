...
<Callout variant="tip">
  프라이빗 로케이션에서 모니터를 실행 중인 경우, SJM 배포 단계 및 프라이빗 로케이션 관련 문제 해결에 대해서는 [프라이빗 로케이션 런타임 전환 가이드](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) 를 참조하십시오.
</Callout>
...
모든 프로미스에 `.then()`, `.catch()`이(가) 있거나 `await`되었는지 확인합니다.
