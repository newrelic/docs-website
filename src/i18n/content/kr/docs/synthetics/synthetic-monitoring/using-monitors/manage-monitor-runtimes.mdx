...
기존 모니터는 생성 시 사용된 런타임으로 기본 설정됩니다. 중요한 모니터가 향후 수명 종료 프로세스 중에 중단되지 않도록 하려면 공용 모니터를 더 빨리 변환하는 것이 좋습니다. 변환하다:

1. <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**</DoNotTranslate>

   으로 이동한 다음 업그레이드하려는 모니터를 선택하세요.
...
4. <DoNotTranslate>**Validate**</DoNotTranslate>
...
<Callout variant="tip">
  모니터의 런타임 버전이 확실하지 않은 경우 [모니터를 편집](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime/#runtime-convert) 하고 '모니터 구성' 탭에서 <DoNotTranslate>**"Runtime"**</DoNotTranslate> 드롭다운을 확인하세요. `runtimeType = 'NODE_API'` 이 있는 `SyntheticCheck` 이벤트에서 `runtimeTypeVersion` 속성을 쿼리할 수도 있습니다.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Browser**</DoNotTranslate>
      ...
      : Chrome 117(이를 위해서는 신세틱스-node-browser-runtime 이미지 버전 2.2 이상이 필요합니다. Chrome 112 또는 100은 이전 브라우저 런타임 이미지에서 사용됩니다.)

    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>

      : 4.1.0 ( `$browser` 및 `$driver` 을 통해 노출된 이전 버전과 호환되는 셀레니엄 웹 드라이브 3.6 구문으로 `$selenium` 및 `$webDriver` 를 통해 노출됨)
...
<Callout variant="tip">
  모니터 버전이 확실하지 않으면 <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets) > Upgrade monitors**</DoNotTranslate> 으로 이동하세요. 최신 런타임 버전을 사용하는 경우 <DoNotTranslate>**Upgrade monitors**</DoNotTranslate> 옵션이 표시되지 않습니다.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 3.6.0 ( `$browser` 및 `$driver` 을 통해 노출됨)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 3.5.0 ( `$browser>` 및 `$driver` 을 통해 노출됨)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 2.47.0( `$browser>` 및 `$driver` 을 통해 노출됨)
  ...
  >
    ...
    * <DoNotTranslate>**Selenium WebDriver**</DoNotTranslate>
      ...
      : 2.44.0( `$browser>` 및 `$driver` 을 통해 노출됨)
