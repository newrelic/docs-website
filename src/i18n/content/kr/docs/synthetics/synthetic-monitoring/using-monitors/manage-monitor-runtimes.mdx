...
1. **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**으로 이동한 다음 업그레이드할 모니터를 선택합니다.
2. **일반 을**클릭합니다.
...
<CollapserGroup>
  ...
  >
    ...
    * **Browser** \[브라우저]: Chrome 117(개인 위치의 경우 합성 노드 브라우저 런타임 이미지 버전 2.2 이상이 필요합니다. Chrome 112 또는 100은 이전 브라우저 런타임 이미지에서 사용됩니다.)
...
<Callout variant="tip">
  모니터 버전이 확실하지 않은 경우 **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets) > Upgrade monitors**로 이동하십시오. 최신 런타임 버전을 사용 중인 경우 **모니터 업그레이드** 옵션이 표시되지 않습니다.
</Callout>
