...
<CollapserGroup>
  <Collapser
    id="NewUpgrade"
    title={<>Chrome 100, Node 16 이상으로 업그레이드</>}
  >
    기존 모니터는 기본적으로 생성 시 사용된 런타임으로 설정됩니다. 향후 수명 종료 프로세스 중에 중요한 모니터가 손상되는 것을 방지하려면 나중에보다 빨리 공용 모니터를 변환하는 것이 좋습니다. 변환하다:

    1. [one.newrelic.com](https://one.newrelic.com/nr1-core) 으로 이동하여 **Synthetics** 를 클릭한 다음 업그레이드할 모니터를 선택하십시오.
    2. **설정** 탭에서 **일반** 을 클릭합니다.
    3. 드롭다운 메뉴를 사용하여 현재 런타임 버전을 전환합니다.
    4. **유효성 검사** 를 클릭하여 모니터가 새 런타임에서 작동하는지 확인하십시오. 필요한 경우 스크립트 스크립트를 수정합니다.
    5. 구하다.
  </Collapser>
  ...
  <Collapser
    ...
    title={<>레거시 런타임 간 전환</>}
  >
    모니터가 새로운 기능에 액세스하고 예상대로 작동하도록 하려면 모니터 **업그레이드** 버튼이 표시될 때마다 모니터를 업그레이드하십시오. [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > Upgrade monitors** 로 이동하여 모니터를 업그레이드할 수도 있습니다.
...
Node 16.10.0 런타임 릴리스부터 API 런타임은 [브라우저 런타임](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime) 과 별도로 관리됩니다. 이것은 차세대 런타임이라고도 합니다. 지원 중단된 `request` 모듈 대신 `got` 모듈을 기반으로 하는 최초의 스크립트 API 런타임입니다. `got` 모듈은 `$http` 객체를 통해 `request` 호환 형식으로 노출됩니다.
...
<CollapserGroup>
  <Collapser
    id="Node16"
    title={<>노드 16.10.0( <strong>최신</strong> )</>}
  >
    노드 16.10.0 세부 정보:
    ...
    * **지원되는 타사 모듈:**
      ...
      * [구글 인증 라이브러리](https://www.npmjs.com/package/google-auth-library) 7.14.0
      * 11.8.3 [을 얻었다](https://www.npmjs.com/package/got)
...
<Callout variant="tip">
  모니터 버전이 확실하지 않은 경우 [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > Upgrade monitors** 로 이동하십시오. 이미 최신 런타임 버전을 사용 중인 경우 **Upgrade monitors** 옵션이 표시되지 않습니다.
</Callout>
...
<CollapserGroup>
  <Collapser
    id="v06"
    title={<>버전 0.6.x( <strong>최신</strong> )</>}
  >
    합성 모니터 버전 0.6.x 세부 정보:

    <Callout variant="important">
      합성 모니터링 런타임은 노드 10에서 지원되는 async/await 구문을 지원하지 않습니다.
    </Callout>
