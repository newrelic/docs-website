...
<CollapserGroup>
  ...
  <Collapser
    ...
    title={<>레거시 런타임 간 전환</>}
  >
    모니터가 새로운 기능에 액세스하고 예상대로 작동하도록 하려면 모니터 **업그레이드** 버튼이 표시될 때마다 모니터를 업그레이드하십시오. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Synthetics > Upgrade monitors** 로 이동하여 모니터를 업그레이드할 수도 있습니다.
...
<Callout variant="tip">
  모니터 버전이 확실하지 않은 경우 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Synthetics > Upgrade monitors** 로 이동하십시오. 이미 최신 런타임 버전을 사용 중인 경우 **모니터 업그레이드** 옵션이 표시되지 않습니다.
</Callout>
