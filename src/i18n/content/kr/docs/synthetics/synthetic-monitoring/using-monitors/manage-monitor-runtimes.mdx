...
<CollapserGroup>
  ...
  <Collapser
    ...
    title={<>레거시 런타임 간 전환</>}
  >
    모니터가 새로운 기능에 액세스하고 예상대로 작동하도록 하려면 모니터 **Upgrade monitors** 버튼이 표시될 때마다 모니터를 업그레이드하십시오. **\[one.newrelic.com** ]( [https://one.newrelic.com](https://one.newrelic.com) )으로 이동하여 모니터를 업그레이드할 수도 있습니다. > Synthetics > Upgrade monitors\*\*.
...
노드 16.10.0부터 시작 런타임 릴리스부터 API 런타임은 [브라우저 런타임](/docs/synthetics/synthetic-monitoring/using-monitors/new-runtime) 과 별도로 관리됩니다. 이것은 더 이상 사용되지 않는 `request` 모듈 대신 `got` 모듈을 기반으로 하는 첫 번째 스크립팅된 API 런타임입니다. `got` 모듈은 `$http` 객체를 통해 `request` 호환 형식으로 노출됩니다. `$http` 객체가 제공하는 `request` 유사 환경은 Node 16 및 최신 스크립트 API 런타임에서 직접 `request` 을 사용하려는 모든 고객에게도 반환됩니다.
...
<Callout variant="tip">
  모니터 버전이 확실하지 않은 경우 **\[one.newrelic.com** ]( [https://one.newrelic.com](https://one.newrelic.com) )으로 이동하세요. > Synthetics > Upgrade monitors **. 이미 최신 런타임 버전을 사용 중인 경우 **업그레이드 모니터\*\* 옵션이 표시되지 않습니다.
</Callout>
