...
1. [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > (모니터 선택) 으로** 이동합니다.
...
<figcaption>
  [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > (모니터 선택) > Failures > (점검 실패 선택):** 실패한 모니터 점검에 대한 세부 정보를 보려면 클릭하십시오. <Icon name="fe-rotate-cw"/>**Run check** 합니다.
</figcaption>
