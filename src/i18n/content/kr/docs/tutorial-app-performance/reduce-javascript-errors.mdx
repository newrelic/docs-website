...
* **Error instances** \[오류 인스턴스]는 **Event Log** \[이벤트 로그] 및 **Stack trace** \[스택 추적] 페이지로 이동합니다. JavaScript 파일로 인해 오류가 발생했는지 여부 또는 인라인 코드의 일부를 수정해야 하는지 여부와 같은 특정 오류에 대한 자세한 정보를 찾을 수 있습니다.
* **Event Log** \[이벤트 로그] 섹션을 스크롤하여 수정해야 할 모든 항목을 찾았는지 확인하십시오. 해결해야 할 사항이 더 있으면 이벤트 로그에서 알려줍니다.
