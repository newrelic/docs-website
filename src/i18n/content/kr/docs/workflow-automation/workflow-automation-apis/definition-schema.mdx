...
워크플로우 상수는 현재 실행 중인 워크플로우에 대한 런타임 메타데이터에 대한 접근을 제공합니다. 이러한 상수는 모든 정의에서 자동으로 사용 가능하며 표현식 구문 `${{ .workflowConstants.<constantName> }}` 을 사용하여 참조할 수 있습니다.
...
상수는 JQ 표현식과 함께 사용하여 값을 변환할 수 있습니다.
...
`DateTime` 액션을 사용하여 타임스탬프를 변환합니다.
...
<Callout variant="important">
  * 런타임 상수는 읽기 전용이며 런타임에 의해 자동으로 제공됩니다.
  * `startedAt` 타임스탬프는 밀리초(에포크 형식) 단위입니다.
  * 상수는 도표우 전체의 모든 표현 컨텍스트에서 사용할 수 있습니다.
  * `workflowInputs` 과 달리 상수는 정의에서 선언할 필요가 없습니다.
</Callout>
...
루프는 컬렉션(리스트, 맵, 객체)을 순회하며 각 반복마다 `index` 및 `element` 변수를 자동으로 생성합니다. 이러한 루프 변수는 `${{ .steps.<loopStepName>.loop.element }}` 또는 를 사용하여 루프 내에서만 접근할 수 있습니다. `${{ .steps.<loopStepName>.loop.index }}`
...
워크플로우 상수, 선택자, 대기 단계, NRDB 쿼리, 스위치 문을 포함한 여러 워크플로우 기능을 보여주는 완전한 워크플로우 예제입니다.
