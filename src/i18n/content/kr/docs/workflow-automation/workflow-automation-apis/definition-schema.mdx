...
워크플로우를 종료하고 사용자 지정 메시지로 실패를 보고하는 단계입니다. `condition` 이(가) 존재하는 경우 먼저 유효성을 검사하며, 조건이 `true`인 경우에만 워크플로우가 중지됩니다. `message`을(를) 제공하지 않으면 기본 메시지 `failed as requested from step: failureStep` 이(가) 사용됩니다.
