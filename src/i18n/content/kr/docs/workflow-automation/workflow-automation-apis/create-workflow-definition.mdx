---
title: 워크플로 정의 만들기
tags:
  - workflow automation
  - workflow
  - CreateWorkflowDefinition
  - workflow automation API
metaDescription: Create a workflow definition.
freshnessValidatedDate: never
translationType: machine
---

이 API를 사용하여 워크플로우 정의를 생성하십시오. 워크플로우 정의가 이미 존재하는 경우 오류를 반환합니다.

## 입력스피커, 변수

이 뮤테이션을 호출할 때 다음 파라미터는 필수 또는 선택 사항입니다.

<table>
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        매개변수
      </th>

      <th>
        유형
      </th>

      <th>
        필수의
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `scope`
      </td>

      <td>
        물체
      </td>

      <td>
        네
      </td>

      <td>
        계정 범위. 

        `id`

         (계정 ID)이 포함되어 있습니다.
      </td>
    </tr>

    <tr>
      <td>
        `definition`
      </td>

      <td>
        물체
      </td>

      <td>
        네
      </td>

      <td>
        워크플로우 정의. 

        `yaml`

         (base64로 인코딩된 워크플로우 YAML)을 포함합니다.
      </td>
    </tr>

    <tr>
      <td>
        `tags`
      </td>

      <td>
        \[EntityTagInput]
      </td>

      <td>
        아니요
      </td>

      <td>
        워크플로우 정의에 적용할 태그입니다. 각 태그에는 

        `key`

         (String) 및 

        `values`

         ([String])이 필요합니다.
      </td>
    </tr>
  </tbody>
</table>

## 샘플 요청

쿼리플로우를 생성하고 편집하려면 [쿼리빌더를](https://one.newrelic.com/nerdgraph-graphiql?state=c8579e50-f154-fec6-22b8-f9fdcfc1990a) 사용하세요. [NerdGraph API 탐색기를](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) 참조하세요.

```graphql
  mutation {
    workflowAutomationCreateWorkflowDefinition(
      scope: {
        id: "11933347"
      }
      definition: {
        yaml: ""
      }
      tags: [{key: "environment", values: ["production"]}]
    ) {
      definition {
        name
      }
    }
  }
```

## 관련 주제 [#related-topics]

<DocTiles>
  <DocTile title="정의 스키마" path="/docs/workflow-automation/workflow-automation-apis/definition-schema">
    워크플로우를 생성하기 전에 워크플로우 YAML 구조를 이해하십시오.
  </DocTile>

  <DocTile title="흐름 정의 검증" path="/docs/workflow-automation/workflow-automation-apis/validate-workflow-definition">
    생성하기 전에 YAML 구문을 테스트하세요.
  </DocTile>

  <DocTile title="흐름 흐름 시작" path="/docs/workflow-automation/workflow-automation-apis/start-workflow-run">
    새로 생성된 창을 실행하세요
  </DocTile>

  <DocTile title="흐름 흐름 정의 업데이트" path="/docs/workflow-automation/workflow-automation-apis/update-workflow-definition">
    기존 흐름 정의 수정
  </DocTile>
</DocTiles>