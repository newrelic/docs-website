...
<CollapserGroup>
  <Collapser id="aws.lambda.invoke" title="Lambda 호출하기">
  ...
  <Collapser id="aws.lambda.updateFunctionConfiguration" title="업데이트 기능 설정">
  ...
  <Collapser id="aws.lambda.getFunction" title="특정 Lambda 함수 설명">
    특정 Lambda 함수에 대한 설정 세부 정보, 코드 위치 및 기타 메시데이터를 검색합니다.
  ...
  <Collapser id="aws.lambda.listAliases" title="특정 Lambda 함수의 별칭을 나열하세요.">
    특정 Lambda 함수에 대한 별칭 목록을 반환합니다. 별칭은 함수의 버전을 가리키는 포인터입니다.
  ...
  <Collapser id="aws.bedrock.invokeAgent" title="베드락 에이전트를 호출합니다">
    베드락 에이전트를 호출합니다.
