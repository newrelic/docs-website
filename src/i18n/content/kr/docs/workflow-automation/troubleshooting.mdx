...
### Autopilot Slack RCA 워크플로우

**RCA 콘텐츠가 없는 빈 Slack 메시지:** `analysisRaw`이(가) 응답을 반환했지만 `analysisText`이(가) 콘텐츠를 추출할 수 없습니다 — `finalAnswer`이(가) 예상된 `.card.body` 구조가 아닙니다. 선택기 `expression`이(가) 한 줄에 있고 <DNT>Autopilot</DNT> 프롬프트가 구조화된 카드 응답을 반환하는지 확인하십시오. 표준 선택기 패턴은 [Autopilot 및 Slack을 사용한 공지 RCA](/docs/workflow-automation/workflow-examples/integrations/autopilot-slack-rca)를 참조하십시오.

**Slack의 &quot;<DNT>Autopilot</DNT> RCA Failed&quot; 메시지:** `analysisRaw`이(가) 비어 있었습니다 — <DNT>Autopilot</DNT>이(가) `finalAnswer`을(를) 반환하지 않았습니다. `issueId`이(가) 유효하고 활성화된 공지 이슈 UUID이고 <DNT>Autopilot</DNT> 프롬프트가 올바른 형식인지 확인하십시오.

**입력 검증에서 워크플로우 실패:** `issueId` 및 `slackDestinationId`은(는) 모두 UUID 형식이어야 합니다. 공지 워크플로우에서 트리거할 때 `{{ issueId }}`을(를) 사용하십시오.
