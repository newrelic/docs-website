...
차트에서 경보를 시작하는 것은 New Relic 경보 조건을 설정하는 가장 쉬운 방법 중 하나입니다. 처음부터 NRQL 쿼리를 생성하는 대신 차트를 사용하면 기존 NRQL 조건을 사용하여 프로세스를 시작할 수 있습니다. 이 예에서는 오류 응답 차트를 사용하여 경고 조건을 설정합니다.
...
<Steps>
  <Step>
    ...
    1. 대시보드로 이동합니다. 그런 다음 알림을 생성하려는 차트를 찾고 **...** 메뉴를 클릭한 다음 **Create alert condition** \[알림 조건 생성을] 선택합니다.
    2. 경고 조건을 만드는 데 사용할 신호 또는 데이터 스트림을 선택합니다. 간단하게 유지하려면 첫 번째 조건에 대한 **Build a query** \[쿼리 작성] 옵션을 선택합니다.
    3. 필요한 경우 모니터링하려는 항목과 일치하도록 NRQL 쿼리를 수정합니다. 지금은 쿼리를 그대로 두는 것이 좋지만 경고 및 NRQL 쿼리를 더 잘 처리하면 쿼리를 변경하여 거의 모든 데이터 임계값을 모니터링할 수 있습니다.
