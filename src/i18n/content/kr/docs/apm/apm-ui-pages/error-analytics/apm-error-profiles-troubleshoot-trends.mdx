---
title: 'APM 오류 프로필: 추세 문제 해결'
tags:
  - APM
  - APM UI pages
  - Error analytics
metaDescription: New Relic's error profiles feature in the classic APM view shows trends and anomalies that help you compare and troubleshoot error events.
translationType: machine
---

import apmErrorProfiles from 'images/apm_screenshot-full_error-profiles.webp'

<Callout
  variant="important"
  title="EOL 공지"
>
  2022년 3월부터 오류 클래식 UI를 비롯한 여러 기능에 대한 지원이 중단됩니다. 이 전환을 쉽게 준비하는 방법을 포함한 자세한 내용은 [지원 포럼 게시물](https://discuss.newrelic.com/t/important-upcoming-changes-to-capabilities-and-support-across-errors-classic-labels-service-infrastructure-on-host-snmp-integration-and-inventory-data-in-cloud-integrations/175370) 을 참조하십시오.
</Callout>

DevOps 전문가가 앱에서 오류의 원인을 추적해야 하는 경우 원인을 식별하기가 쉽지 않을 수 있습니다. APM의 오류 프로필은 한 이벤트 세트를 다른 이벤트 세트와 자동으로 비교합니다.

각 오류 프로필은 이벤트에 대한 서로 다른 값의 빈도의 상당한 차이에 대한 시각적 세부 정보를 제공합니다. 각 [속성](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#attribute) 에 대해 오류 프로필에는 다음이 포함됩니다.

* 가장 많이 벗어난 값에 대해 오류 속성이 어떻게 분포되어 있는지 보여주는 원형 차트
* 오류 속성의 분포를 오류가 없는 트랜잭션의 분포와 비교하는 테이블

이렇게 하면 앱 오류를 해결하는 데 더 많은 추측을 할 수 있습니다. 오류를 안전하게 무시할 수 있는지 또는 새 배포, 코드 편집, 고객 커뮤니케이션 또는 기타 작업으로 오류 해결을 시도해야 하는지를 보다 쉽게 결정할 수 있습니다.

## 오류 프로필 속성 예 [#error-profile-attributes]

오류 프로필에 액세스하려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > Errors > Error profiles**으로 이동합니다.

<img
  title="APM: Error profiles example"
  alt="APM: Error profiles example"
  src={apmErrorProfiles}
/>

<figcaption>
  오류 프로필을 사용하여 앱에 대한 오류 이벤트 빈도의 추세와 상당한 차이를 해결합니다.
</figcaption>

오류 프로필은 오류가 아닌 것과 비교하여 상당히 다른 특성을 가진 [속성](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#attribute) 모음입니다. 이벤트 집합이 정상적인 것(예: 지정된 기간 동안의 모든 트래픽과 비교한 오류) 또는 유사한 기준 간의 차이(예: 두 개의 서로 다른 호스트)를 나타내는 경우 속성은 "비정상적"입니다.

오류는 다음과 같은 이벤트와 관련될 수 있습니다.

* 특정 [웹 트랜잭션](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) 이름 또는 [웹이 아닌 트랜잭션](/docs/apm/transactions/intro-transactions/monitor-background-processes-other-non-web-transactions) 이름, JVM 스레드 이름 등
* 고유한 유형의 오류 메시지, 클래스 등
* 무작위 고객 상호 작용 예를 들어, 특정 오류는 단일 고객의 계정에서 발생하지만 일반적인 트래픽은 다양한 계정에서 발생합니다.
* 외부 통화 수 또는 지속 시간
* 생태계의 호스트, 클러스터 에이전트 ID 등의 타이밍 차이
* 기타 이상

## 오류 프로필 기준 선택 [#criteria]

앱의 [**오류** 페이지](/docs/apm/applications-menu/error-analytics/error-analytics-explore-events-behind-errors) 에 있는 기준에 따라 New Relic은 비정상적인 경향을 중요도별로 분석하고 나열합니다. 선택한 기준에는 다음이 포함됩니다.

* [시간 창](/docs/data-analysis/user-interface-functions/view-your-data/time-picker-setting-time-periods-view-data)
* **오류** 페이지 필터
* **오류** 페이지 또는 **오류 프로필** 탭의 검색 기준

오류 프로필 결과를 조사하고 더 깊이 파고들려면 앱의 오류 프로필 기준을 추가하거나 변경하십시오. **오류 프로필** 탭은 업데이트된 기준과 일치하는 오류를 구별하는 특성을 표시하도록 새로 고쳐집니다.

<CollapserGroup>
  <Collapser
    id="profile-criteria-example"
    title="오류 프로필 기준 예"
  >
    앱의 **오류 프로필** 탭에는 현재 여러 오류 클래스 또는 메시지가 표시됩니다. 특정 오류 클래스 또는 메시지로 필터링하려면 다음 옵션 중 하나를 사용하십시오.

    * **오류** 페이지의 시간 선택기에서 시간 범위를 변경합니다. 예를 들어 기본값(30분)을 **지난 24시간, 지금 종료 로** 변경합니다.

    * **오류** 페이지의 필터에서

      <Icon name="fe-filter"/>

      섹션: **그룹 목록으로 돌아가기를** 선택한 다음 다른 **오류 그룹** , **오류 속성** 또는 **사용자 정의 속성** 을 선택합니다.

    * **오류** 페이지의 필터에서

      <Icon name="fe-filter"/>

      섹션: 목록에서 특정 항목을 클릭하여 필터 범위를 좁힙니다. 예를 들어, 여러 오류 메시지가 나열되면 관심 있는 메시지만 클릭합니다.

    * **오류 프로필** 탭의 검색에서

      <Icon name="fe-search"/>

      상자에 `class` , `message` 또는 기타 검색 값을 입력합니다.
  </Collapser>
</CollapserGroup>

## 오류 프로필 결과 분석 [#details]

앱의 오류 프로필에서 속성 결과에 대한 세부 정보를 검사하려면 클래식 APM 보기를 사용해야 합니다.

1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Events > Errors** 로 이동한 다음 **Show new view** 를 전환하여 APM이 **Errors** 페이지의 클래식 보기를 표시하도록 합니다.
2. **오류** 페이지에서 **오류 프로필** 탭을 선택합니다.
3. **오류 프로필** 탭에서 현재 선택한 [오류 프로필 기준](#criteria) 과 일치하는 오류 속성 목록을 검토합니다.
4. 특정 속성의 세부 정보를 보려면 클릭합니다.
5. 특정 오류 세부 정보를 강조 표시하려면 특성에 대한 원형 차트 세그먼트 또는 테이블 행 위로 마우스를 가져갑니다.
6. 앱 오류의 특정 속성을 조사하려면 **오류 프로필** 탭의 검색 창에 이름을 입력하거나 현재 선택된 [오류 프로필 기준을](#criteria) 변경하세요.

차이가 큰 값을 비교하여 속성의 오류를 구별하는 특성을 식별합니다. 오류 프로필 결과의 비교 데이터와 [오류 추적 세부 정보](/docs/apm/applications-menu/error-analytics/error-analytics-manage-error-traces) 는 추가 문제 해결 및 오류 해결을 위해 취해야 할 단계를 결정하는 데 도움이 될 수 있습니다.

## 오류 및 무오류 분포 [#distribution]

오류의 속성에 따라 오류에 대해 속성이 오류가 아닌 경우와 다르게 분포되는 경우가 있습니다.

<CollapserGroup>
  <Collapser
    id="top"
    title="최고 편차 값"
  >
    New Relic은 앱의 오류에 대한 각 속성을 분석하고 기준과 일치하는 오류 분포를 오류가 없는 트랜잭션과 비교합니다. 이러한 오류 사이의 비율이 오류가 없는 트랜잭션과 거의 동일하다면 속성에 디버깅에 유용한 정보가 많이 포함되어 있지 않습니다.

    New Relic은 상위 편차 값만 표시하도록 각 속성에 대한 오류 프로필의 원형 차트 및 테이블을 제한합니다. 비율이 거의 같으면 New Relic은 오류 프로필에 비율을 포함하지 않습니다.
  </Collapser>

  <Collapser
    id="other"
    title="기타 카테고리"
  >
    상위 편차 값 이후에 오류 프로필은 나머지를 **기타** 범주로 그룹화합니다. 이렇게 하면 이러한 오류에 대해 다른 값에 집중하는 데 도움이 됩니다.
  </Collapser>

  <Collapser
    id="no-value"
    title="값 범주 없음"
  >
    값이 비정상적으로 존재하거나 오류에 없는 경우 오류 프로필 목록에 **값 없음** 범주가 표시될 수 있습니다.
  </Collapser>

  <Collapser
    id="filters"
    title="필터"
  >
    필터를 선택하지 않은 경우 프로필에는 오류가 나타내는 모든 구별되는 특성이 집계되어 표시됩니다.

    **예:** 특정 종류의 트랜잭션은 전체 트래픽의 20%를 구성하고 오류의 80%를 담당합니다. 오류 프로필은 `Transaction` 속성에 예상치 못한 비율을 표시합니다.

    [오류 프로필 기준](#criteria) 을 조정하여 프로필 결과를 더 자세히 드릴다운하면 특정 오류 이벤트를 보다 효과적으로 문제를 해결하고 해결할 수 있습니다.
  </Collapser>
</CollapserGroup>