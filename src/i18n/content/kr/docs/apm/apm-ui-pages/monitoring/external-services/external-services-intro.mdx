...
* **Data \[데이터]:** 기존 외부 서비스와 달리 분산 추적의 트랜잭션 수준 세부 정보는 메트릭이 아닌 샘플링을 기반으로 합니다. 이 샘플링된 데이터는 분산 추적에 연결되어 트랜잭션 성능을 향상시키는 요인에 대한 더 깊은 통찰력을 제공할 수 있습니다.

<Callout variant="tip">
  외부 서비스 기능은 지원하지 않습니다. <InlinePopover type="browser"/>그리고 <InlinePopover type="mobile"/>데이터.
</Callout>
