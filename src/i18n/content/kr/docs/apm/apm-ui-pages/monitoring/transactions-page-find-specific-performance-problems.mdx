...
APM의 **Transactions** \[트랜잭션] 페이지는 성능 문제를 미세 조정하거나 오류를 해결하기 위한 좋은 후보가 될 수 있는 [소니를](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) 식별하는 데 도움이 됩니다. 이 페이지에는 선택한 앱의 프로세서 **Requests** \[요청], [작업 처리 시간, 응답 시간](#wall_clock_time) 및 관련 [처리량](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#throughput) (분당 요청 수 또는 **rpm**)을 기준으로 한 상위 20개 트랜잭션이 나열되어 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    **Transaction** \[프로세서] 페이지의 **Top 20 transactions** \[상위 20개 프로세서] 차트와 선택한 프로세서의 **App performance** \[앱 성능] 창에는 데이터를 다음과 같이 볼 수 있는 옵션이 포함되어 있습니다.
