...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitor > Databases > (추적 선택) > Trace details:** 앱의 성능 문제에 기여할 수 있는 특정 데이터베이스 및 인스턴스에 대한 정보를 보려면 **데이터베이스** 아이콘을 선택합니다. .
    </figcaption>
  </Collapser>
  ...
  >
    ...
    * 성능에 예기치 않은 영향을 줄 수 있는 이상값을 식별합니다.
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitor > Service map > (맵 선택) > (데이터 저장소 노드 선택)** : **인스턴스** 노드를 선택하여 성능 문제를 더 자세히 드릴다운할 수 있습니다. 선택한 노드 내에서 개별 인스턴스를 활성화하거나 비활성화할 수 있으며 맵을 저장할 때 선택 사항이 저장됩니다.
      </figcaption>
  </Collapser>
