...
* **C SDK:** 데이터 저장소 세그먼트에 대한 [C SDK 호환성](/docs/agents/c-sdk/instrumentation/instrument-your-app-c-sdk#datastore-segments) 을 참조하세요.
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Databases > (데이터베이스 작업 선택) > (느린 쿼리 선택) > Trace details:** 다음은 특정 데이터베이스 및 인스턴스를 식별하는 느린 쿼리 추적의 예입니다.
    </figcaption>
  </Collapser>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Monitor > Databases > (추적 선택) > Trace details:** 앱의 성능 문제에 기여할 수 있는 특정 데이터베이스 및 인스턴스에 대한 정보를 보려면 **데이터베이스** 를 선택합니다. 상.
    </figcaption>
  </Collapser>
  ...
  <Collapser
    ...
    title="서비스 맵 세부정보 예시"
  >
    환경에 성능 문제가 있고 호출 응용 프로그램과 특정 데이터베이스 인스턴스 간의 성능 문제의 영향을 해결하고 평가하려고 합니다.

    [APM **서비스 맵** 페이지](/docs/data-analysis/service-maps/get-started/service-maps-visualize-monitor-apps-entire-architecture) 를 사용하여 데이터베이스 및 외부 서비스를 포함한 앱의 연결 및 종속성에 대한 빠른 개요를 확인하십시오. 각 데이터 저장소 유형에는 맵에 고유한 노드가 있습니다. 선택한 [서비스 맵 세부 정보](/docs/data-analysis/service-maps/get-started/customize-your-service-maps#moving-map) 에서 다음을 수행할 수 있습니다.

    * [애플리케이션](/docs/data-analysis/service-maps/get-started/service-maps-visualize-monitor-your-apps-entire-architecture#health-performance) 또는 [외부 서비스와 데이터 저장소 인스턴스](/docs/data-analysis/service-maps/get-started/service-maps-visualize-monitor-your-apps-entire-architecture#identify-external-issues) 간의 연결에 대해 색상으로 구분된 상태를 검토합니다. (New Relic은 간단한 기준 기술을 사용하여 지난 15분 동안의 성과를 지난 주 평균과 비교합니다.)

    * 서비스 맵의 시계열 차트에서 특정 앱 또는 인스턴스 유형을 선택한 다음 **응답 시간** 또는 **분당 요청 수** (처리량)에서 예상치 못한 성능 급증을 검토합니다. (이렇게 하면 리소스 또는 다른 서비스의 처리 시간에 영향을 미치는 이상값 또는 "노이즈 이웃"을 더 쉽게 식별할 수 있습니다.)

    * 개별 인스턴스(최대 100개 인스턴스)를 활성화하거나 비활성화하여 차트를 필터링할 데이터 저장소 노드를 선택합니다. 지도를 저장할 때 선택 사항이 저장됩니다.

    * 성능에 예기치 않은 영향을 줄 수 있는 이상값을 식별합니다.

      서비스 맵으로 문제가 있는 데이터베이스 또는 인스턴스를 식별하면 트랜잭션 추적 및 느린 쿼리 추적은 물론 데이터베이스 공급업체 도구를 사용하여 문제를 추가 진단할 수 있습니다.
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Monitor > Service map > (맵 선택) > (데이터 저장소 노드 선택)** : **인스턴스** 노드를 선택하여 성능 문제를 더 자세히 드릴다운할 수 있습니다. 선택한 노드 내에서 개별 인스턴스를 활성화하거나 비활성화할 수 있으며 맵을 저장할 때 선택 사항이 저장됩니다.
      </figcaption>
  </Collapser>
