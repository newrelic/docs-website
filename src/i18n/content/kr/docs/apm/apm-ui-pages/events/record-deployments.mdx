...
<Callout title="배포용 REST API의 수명이 종료됩니다." variant="important">
  이 페이지에 문서화된 REST API v2 배포 엔드포인트(`/v2/applications/{app_id}/deployments.json`)는 구형 배포 v0 API와 함께 [2027년 7월 31일에 수명이 종료됩니다](/eol/2026/07/eol-07-31-26-rest-api-v2). 해당 날짜 이후에는 사용할 수 없습니다.

  대신 NerdGraph `changeTrackingCreateEvent` 뮤테이션을 사용하여 [변경 추적](/docs/change-tracking/overview/) 기능으로 배포를 기록하십시오. 엔드포인트별 매핑은 [REST API v2에서 NerdGraph로 마이그레이션](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments) 가이드를 참조하십시오. 아래의 REST 지침은 마이그레이션 중인 기존 통합을 위한 참고용으로 유지됩니다.
</Callout>

<Callout variant="tip">
  <InlinePopover type="apm" />, <InlinePopover type="browser" />및 <InlinePopover type="mobile" />에 대해 변경 추적을 사용할 수 있습니다.
</Callout>
...
뉴렐릭 [REST API v2](/docs/apis/rest-api-v2/requirements/new-relic-rest-api-v2-getting-started) 또는 [NerdGraph](/docs/change-tracking/config/nerdgraph/)를 사용하여 새 배포를 기록하고 과거 배포 목록을 검색할 수 있습니다. NerdGraph를 사용하십시오. REST API v2 배포 엔드포인트는 [2027년 7월 31일에 수명이 종료됩니다](/eol/2026/07/eol-07-31-26-rest-api-v2). 또한 일부 APM 에이전트는 배포를 자동으로 기록하는 에이전트별 메서드를 제공합니다.
...
<Callout title="이러한 REST 엔드포인트는 2027년 7월 31일에 수명이 종료됩니다" variant="important">
  아래에 표시된 REST API v2 배포 엔드포인트는 [2027년 7월 31일에 수명이 종료되며](/eol/2026/07/eol-07-31-26-rest-api-v2) 해당 날짜 이후에는 작동이 중지됩니다. NerdGraph `changeTrackingCreateEvent` 뮤테이션으로 마이그레이션하려면 [REST API v2에서 NerdGraph로 마이그레이션](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments) 가이드를 참조하십시오. 이 지침은 기존 통합을 마이그레이션하는 동안 참고용으로 유지됩니다.
</Callout>

New Relic REST API v2를 사용하여 배포를 기록하고 과거 배포 목록을 가져올 수 있습니다.

* 이 문서의 예제에서는 `curl` 을(를) 명령줄 도구로 사용합니다. 하지만 REST requests를 보내는 데 어떤 방법이든 사용할 수 있습니다. [API Explorer](https://api.newrelic.com/docs/#/Applications/post_applications__application_id__deployments_json)를 사용하여 배포를 생성하고 확인할 수도 있습니다.
...
<Callout variant="important">
  아래에 참조된 REST API v2 메서드는 [2027년 7월 31일에 수명이 종료됩니다](/eol/2026/07/eol-07-31-26-rest-api-v2). 대신 NerdGraph `changeTrackingCreateEvent` 뮤테이션을 사용하여 배포를 기록하려면 [마이그레이션 가이드](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments)를 참조하십시오.
</Callout>

일부 에이전트에는 배포를 기록하는 추가 방법이 있습니다.

* <DNT>**All agents**</DNT>: 뉴렐릭 [REST API v2](#api-instructions)를 사용하십시오.
* <DNT>**C**</DNT>: SDK 전용 메서드가 없습니다. [REST API](#api-instructions)를 사용하십시오.
* <DNT>**Go**</DNT>: 에이전트별 메서드가 없습니다. [REST API](#api-instructions)를 사용하십시오.
...
* <DNT>**Node.js**</DNT>: 에이전트별 메서드가 없습니다. [REST API](#api-instructions)를 사용하십시오.
