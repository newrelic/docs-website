...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="요청 매개변수"
  >
    트랜잭션에서 매개변수를 요청합니다. Node.js 에이전트는 기본적으로 GET 매개변수를 캡처합니다. POST 매개변수를 캡처하려면 [`addCustomAttribute()`](https://newrelic.github.io/node-newrelic/API.html#addCustomAttribute) Node.js 에이전트 API 호출을 사용하세요.
  </Collapser>
</CollapserGroup>
