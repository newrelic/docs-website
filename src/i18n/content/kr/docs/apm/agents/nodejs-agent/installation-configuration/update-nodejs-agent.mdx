...
<DNT>**Recommendation:**</DNT> 업데이트된 버전을 프로덕션으로 이동하기 전에 테스트하십시오. 문제가 있는 경우 Node.js 에이전트 [문제 해결 절차를](/docs/agents/nodejs-agent/troubleshooting/troubleshooting-your-nodejs-installation) 따르세요.
    ...
    <tr>
      ...
      <td>
        * <DNT>**v6.4.2-v6.14.0:**</DNT>
          ...
          에이전트 v6.4.2부터 이러한 속성은 더 이상 사용되지 않는 것으로 표시되었지만 새 속성에 대한 전환 기간을 제공하기 위해 그대로 유지되었습니다.

        * <DNT>**v7:**</DNT>

          http 응답 속성 보고가 활성화되면 `http.statusCode` 및 `http.statusText` 보고됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v5 - v6:**</DNT>
          ...
          에이전트 v5.0.0부터 이 방법은 더 이상 사용되지 않는 것으로 표시되었지만 새 방법에 대한 전환 기간을 제공하기 위해 그대로 두었습니다.

        * <DNT>**v7:**</DNT>

          애플리케이션은 API 메소드 [`transactionHandle.ignore()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api/#transaction-handle-ignore) 을 사용해야 합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v6:**</DNT>
          ...
          v8.0.2까지 @koa/router에 대한 지원이 추가되었습니다. @koa/router v8.0.3 이상은 현재 지원되지 않습니다.

        * <DNT>**v6:**</DNT>

          Node.js v12에 대한 지원이 추가되고 Node.js v6, 7, 9에 대한 지원이 제거되었습니다. 지원되는 최소 버전은 Node.js v8입니다.
    ...
    <tr>
      ...
      <td>
        * <DNT>**v1 - v5:**</DNT>
          ...
          `max_samples_per_minute` 에이전트가 분당 수집하는 최대 이벤트 수를 정의했습니다. 이전에 `max_samples_per_minute` 상한으로 사용한 경우, 데이터가 서버에 삭제되는 것을 방지하기 위해 이전 값, 한계를 유효한 최대값으로 낮추어야 할 수도 있습니다. 10,000 미만을 권장합니다.

        * <DNT>**v5:**</DNT>

          에이전트 v5 이하에서는 `max_samples_stored` 이 [뉴렐릭 수집기](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector) 와 통신할 수 없는 경우 에이전트 저장소의 최대 수를 정의했습니다.

        * <DNT>**v6:**</DNT>

          `max_samples_stored` 의 새로운 동작은 에이전트가 분당 수집하는 최대 이벤트 수를 정의합니다. 이 숫자보다 많은 경우 에이전트는 통계 샘플링을 수집합니다. 이 설정 사용법은 다른 APM 에이전트와 일치합니다.

        * <DNT>**Troubleshooting tips:**</DNT>

          이벤트 데이터가 누락된 경우 `max_samples_stored` 값을 늘리거나 기본값으로 되돌립니다. `max_samples_stored` 값은 10,000을 초과할 수 없습니다.
      </td>
    </tr>
  </tbody>
</table>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v2 - v4:**</DNT>
          ...
          에이전트 v2.7.0부터 이러한 메서드는 더 이상 사용되지 않는 것으로 표시되었지만 새 메서드에 대한 전환 기간을 제공하기 위해 그대로 두었습니다.

        * <DNT>**v5:**</DNT>

          애플리케이션은 API 메소드 [`newrelic.addCustomAttribute()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#add-custom-attribute) 및 [`newrelic.addCustomAttributes()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#add-custom-attributes) 사용해야 합니다. 이는 기존 방법을 즉시 대체할 수 있는 방법입니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v2 - v4:**</DNT>
          ...
          에이전트 v2.0.2부터 이러한 메서드는 더 이상 사용되지 않는 것으로 표시되었지만 새 메서드에 대한 전환 기간을 제공하기 위해 그대로 두었습니다.

        * <DNT>**v5:**</DNT>

          애플리케이션은 API 메소드 [`newrelic.startWebTransaction()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#startWebTransaction) 및 [`newrelic.startBackgroundTransaction()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#startBackgroundTransaction) 로 업데이트해야 합니다.

        * <DNT>**Troubleshooting tips:**</DNT>

          이러한 새로운 방법은 기존 방법을 즉시 대체하는 것이 **아닙니다** . 이러한 새로운 방법으로 전환하기 전에 [맞춤형 로그아웃 가이드를](/docs/agents/nodejs-agent/supported-features/nodejs-custom-instrumentation#web-txn) 검토하세요.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v3 - v4:**</DNT>
          ...
          에이전트 v3.3.0부터 이 메서드는 `newrelic.startSegment()`(를) 위해 더 이상 사용되지 않는 것으로 표시되었습니다.

        * <DNT>**v5:**</DNT>

          애플리케이션은 [`newrelic.startSegment()`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#startSegment) 으로 업데이트되어야 합니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v4:**</DNT>
          ...
          npm 패키지 [`node-cassandra-cql`](https://www.npmjs.com/package/node-cassandra-cql) 을 사용하면 Cassandra에 대한 데이터스토어 지표를 얻을 수 있습니다.

        * <DNT>**v5:**</DNT>

          Cassandra 데이터스토어 메트릭을 얻으려면 [`cassandra-driver`](https://www.npmjs.com/package/cassandra-driver) 패키지로 마이그레이션해야 합니다. 이 모듈은 동일한 개발자가 적극적으로 유지 관리합니다. `node-cassandra-cql` 패키지는 `cassandra-driver` 패키지를 위해 2014년 2월에 중단되었습니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>**v2 - v4:**</DNT>
          ...
          에이전트 v2.7.0부터 이러한 설정 모범 사례로 인해 경고가 로그되고 값이 자동으로 새 속성 설정으로 업그레이드되었습니다.

        * <DNT>**v5:**</DNT>

          경고는 계속 기록되지만 설정은 무시되며 더 이상 자동 업그레이드가 발생하지 않습니다.

        * <DNT>**Troubleshooting tips:**</DNT>

          `ignored_params` 설정에 나열된 모범 사례가 있는 경우 이를 [`attributes.exclude`](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#attributes_exclude) 로 이동해야 합니다.
      </td>
    </tr>
  </tbody>
</table>
        ...
        <tr>
          ...
          <td>
            * <DNT>**v2:**</DNT>
              ...
              v2 에이전트를 사용하면, 폴리머 없이 뉴렐릭에 연결되도록 에이전트를 구성하는 것이 가능했습니다.

            * <DNT>**v3:**</DNT>

              Node.js 에이전트 v3는 통신을 보호하기 위해 항상 TLS를 사용하여 뉴렐릭 서버에 연결합니다. 에이전트가 이를 비활성화하도록 구성된 경우 경고가 기록되고 설정이 무시됩니다.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            * <DNT>**v2:**</DNT>
              ...
              v2 에이전트는 경로 탐지(`/users/:userId`) 및 검증 탐지기(`/users?userId=123`), 탐지 이름(`userId`)과 같은 요청을 탐지하도록 수집합니다.

            * <DNT>**v3:**</DNT>

              v3 에이전트는 모든 요청에 대해 접두사를 붙이고 문자열 `request.parameters.` 사용합니다. 예를 들어,`userId` `request.parameters.userId` 로 표시됩니다.

            * <DNT>**Troubleshooting tips:**</DNT>

              대시보드, 공지 사항 또는 요청에 따른 기타 NRQL 쿼리가 있는 경우 새로운 정품 인증 이름으로 업데이트하세요.
          </td>
        </tr>
      </tbody>
    </table>
...
<CollapserGroup>
        ...
        <tr>
          ...
          <td>
            * <DNT>**v1:**</DNT>
              ...
              v1 에이전트를 사용하면 구성 속성 `rules.name` 및 `rules.ignore` 에 정의된 규칙이 역순으로 적용되었습니다. 목록의 첫 번째 규칙이 마지막으로 적용되었습니다.

            * <DNT>**v2:**</DNT>

              Node.js 에이전트 v2는 정의된 순서대로 규칙을 적용하므로 목록의 첫 번째 규칙이 먼저 적용됩니다.

            * <DNT>**Troubleshooting tips:**</DNT>

              v1 에이전트에서 명명 규칙을 사용하고 문제를 발견한 경우 설정에서 규칙 순서를 반대로 바꾸세요.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            * <DNT>**v1:**</DNT>
              ...
              v1 에이전트는 HTTP 서버의 `request` 이벤트에서 각 리스너에 대해 새 트랜잭션을 시작했습니다. `request` 이벤트에 여러 리스너가 있는 애플리케이션에서 이로 인해 거의 항상 이름이 올바르게 지정되지 않은 외부 트랜잭션이 생성되었습니다.

            * <DNT>**v2:**</DNT>

              v2 에이전트는 발생된 각 `request` 이벤트에 대해 단일 트랜잭션만 생성합니다.

            * <DNT>**Troubleshooting tips:**</DNT>

              여러 개의 `request` 이벤트 리스너를 사용하고 추가 트랜잭션을 제거하기 위해 `newrelic.ignoreTransaction()` 에 대한 호출을 추가한 경우 해당 호출을 제거하십시오.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            * <DNT>**v1:**</DNT>
              ...
              v1 에이전트가 아웃바운드 HTTP 요청 객체에서 발생한 처리되지 않은 `error` 이벤트를 삼켰습니다.

            * <DNT>**v2:**</DNT>

              v2 에이전트는 이 동작을 제거합니다. 대신 v2 에이전트는 일반적인 Node.js 실행을 변경하지 않습니다. 이는 `error` 이벤트가 항상 발생함을 의미합니다.

            * <DNT>**Troubleshooting tips:**</DNT>

              아웃바운드 요청을 하고 있고 현재 `error` 이벤트를 수신하지 않는 경우 리스너를 추가하고 애플리케이션에 맞게 오류를 처리하십시오.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  >
    ...
    * 업데이트된 에이전트 버전에서만 사용할 수 있는 새로운 기능을 사용할 수 있는 기능 없이 New Relic Node.js v1 에이전트에 남아 있습니다.
      ...
      <DNT>**Recommendation:**</DNT> 가능한 한 빨리 최신 버전의 Node.js로 업그레이드하세요.
  </Collapser>
