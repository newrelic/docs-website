...
<CollapserGroup>
  ...
  >
    이 예는 비동기 작업을 제어하기 위해 `async` / `await` 를 사용하여 코드를 계측하는 방법을 보여줍니다. 이를 위해서는 Node.js 에이전트 v2.3.0 이상과 함께 Node 8 이상과 New Relic을 사용해야 합니다.
  ...
  >
    ...
    <Callout variant="important">
      `startSegment` 함수는 Node Agent 3.3.0 릴리스에서 사용할 수 있습니다.
    </Callout>
  </Collapser>
</CollapserGroup>