...
import apmMessagingBreakdownTable from 'images/apm_screenshot-crop_messaging-breakdown-table.png'

import apmTrxSlowestComponents from 'images/apm_screenshot-crop_trx-slowest-components.png'
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (앱 선택) > Transactions > (거래 선택):** 트랜잭션의 **Breakdown table** 는 대기열 작업을 `MessageBroker` 메트릭으로 분류하고 `Put` 로 레이블을 지정합니다(메시지 게시). 또는 `Take` (메시지 수신).
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (앱 선택) > Summary > (트랜잭션 추적 선택):** 이 예에서 선택한 트랜잭션 추적의 **Summary** 은 컬러 막대 차트로 RabbitMQ 작업을 보여줍니다. 가장 **Slowest components** 섹션에는 RabbitMQ `Put` 및 `Take` 결과도 나열됩니다.
</figcaption>
