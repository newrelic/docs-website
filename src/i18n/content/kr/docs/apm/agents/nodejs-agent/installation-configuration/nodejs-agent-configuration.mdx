...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-root" title="sampler.root">
    ...
    원격 상위 항목이 없는 루트 스팬-트랜잭션(수신되는 `traceparent` 또는 `newrelic` 분산 추적 헤더가 없음)에 대한 샘플링 결정 방식을 제어합니다. 유효한 값은 다음과 같습니다:

    * `adaptive` (기본값): 내장된 뉴렐릭의 적응형 샘플링 알고리즘을 사용합니다.
    * `always_on`: 모든 스팬을 샘플링합니다.
    * `always_off`: 스팬을 샘플링하지 않습니다.
    * `trace_id_ratio_based`: 트레이스 ID에 의해 결정된 일정 비율의 트레이스를 샘플링합니다.

    예를 들어 구성 파일에서 이를 설정하려면 다음을 사용합니다:
    ...
    `trace_id_ratio_based`로 설정된 경우 `ratio` (`0` 에서 `1` 사이의 숫자)도 제공해야 합니다.
    ...
    이 값을 `adaptive`로 설정하면, 선택적으로 이 샘플러에 대해서만 샘플링 타깃(분당 1-120개 트랜잭션)을 재정의할 수 있습니다:
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="sampler.remote_parent_sampled">
    ...
    traceparent 헤더에 샘플링 플래그 `01`이 있는 경우, 이를 통해 스팬 샘플링을 처리하는 방법을 제어할 수 있습니다. 허용되는 문자열 값은 다음과 같습니다:
    ...
    * `trace_id_ratio_based`: 트레이스 ID에 의해 결정된 일정 비율의 트레이스를 샘플링합니다.
    ...
    두 설정이 모두 적용될 수 있는 경우 이 설정이 [`remote_parent_not_sampled`](#dt-sampler-remote-parent-not-sampled) 보다 우선합니다.
    ...
    이 값을 `trace_id_ratio_based`로 설정하는 경우, `0`-`1`의 `ratio` 값을 제공해야 합니다. 이 값을 `adaptive`로 설정하면, 선택적으로 이 샘플러에 대해서만 샘플링 타깃을 재정의할 수 있습니다. 동일한 구문에 대해서는, [`sampler.root`](#dt-sampler-root) 아래의 객체 형식 및 환경 변수 예시를 참조하고 `root`를 `remote_parent_sampled` 로 대체합니다.
  </Collapser>
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    traceparent 헤더에 샘플링 플래그 `00`이 있는 경우, 이를 통해 스팬 샘플링을 처리하는 방법을 제어할 수 있습니다. 허용되는 문자열 값은 다음과 같습니다:
    ...
    * `trace_id_ratio_based`: 트레이스 ID에 의해 결정된 일정 비율의 트레이스를 샘플링합니다.
    ...
    이 설정은 [`remote_parent_sampled`](#dt-sampler-remote-parent-sampled)가 적용되지 않는 경우 즉, traceparent 샘플링된 플래그가 `00`인 경우에만 적용됩니다.
    ...
    이 값을 `trace_id_ratio_based`로 설정하는 경우, `0`-`1`의 `ratio` 값을 제공해야 합니다. 이 값을 `adaptive`로 설정하면, 선택적으로 이 샘플러에 대해서만 샘플링 타깃을 재정의할 수 있습니다. 동일한 구문에 대해서는, [`sampler.root`](#dt-sampler-root) 아래의 객체 형식 및 환경 변수 예시를 참조하고 `root`를 `remote_parent_not_sampled` 로 대체합니다.
  </Collapser>
  ...
  <Collapser id="dt-sampler-adaptive-sampling-target" title="sampler.adaptive_sampling_target">
    ...
    적응형 샘플링을 위한 샘플링 타깃으로 기본/적응형 샘플러를 구성할 때 사용되며, 단위는 분당 트랜잭션 수입니다. `[1, 120]`(포함) 범위 내에 있어야 합니다. 뉴렐릭에 연결할 때 이 값은 수집기로 전송되며, 연결 응답에서 반환된 `sampling_target` 값은 에이전트의 적응형 샘플링에서 유효한 샘플링 타깃으로 사용됩니다.
...
사용량 기반 요금을 정확하게 계산하기 위해 에이전트가 호스트 및 클라우드 환경 정보를 감지하는 방법을 설정합니다. 이 설정은 앱 `newrelic.js` 구성 파일의 `utilization` 섹션에 나타납니다.
...
<CollapserGroup>
  <Collapser id="utilization-detect-aws" title="detect_aws">
    ...
    프로세스가 실행 중인 VM에 대한 정보를 얻기 위해 에이전트가 AWS에 연결을 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-pcf" title="detect_pcf">
    ...
    프로세스가 Pivotal Cloud Foundry에서 실행 중인지 감지하기 위해 에이전트가 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azure" title="detect_azure">
    ...
    에이전트가 프로세스가 실행 중인 VM에 대한 정보를 가져오기 위해 Azure에 연결을 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azurefunction" title="detect_azurefunction">
    ...
    에이전트가 실행 중인 Azure Functions에 대한 정보를 얻기 위해 환경 변수 및 호출 컨텍스트를 읽으려고 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-docker" title="detect_docker">
    ...
    프로세스가 실행 중인 컨테이너에 대한 정보를 얻기 위해 에이전트가 파일을 읽으려고 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-gcp" title="detect_gcp">
    ...
    프로세스가 실행 중인 VM에 대한 정보를 얻기 위해 에이전트가 GCP에 연결을 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect-kubernetes" title="detect_kubernetes">
    ...
    프로세스가 실행 중인 컨테이너에 대한 정보를 얻기 위해 에이전트가 쿠버네티스에 연결을 시도할지 여부를 정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-logical-processors" title="logical_processors">
    ...
    에이전트가 호스트에 대해 보고하는 논리 프로세서 수를 재정의합니다.
  </Collapser>
  ...
  <Collapser id="utilization-billing-hostname" title="billing_hostname">
    ...
    청구/사용량 목적을 위해 에이전트가 보고하는 hostname을 재정의합니다.
  </Collapser>
  ...
  <Collapser id="utilization-total-ram-mib" title="total_ram_mib">
    ...
    에이전트가 호스트에 대해 보고하는 총 RAM 용량(MiB)을 재정의합니다.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    <Callout variant="important">
      더 이상 사용되지 않으며 에이전트 버전 15부터 삭제됩니다. 대신 `gcp_cloud_run.use_instance_as_host`를 사용하십시오.
    </Callout>

    활성화되면 GCP 메타데이터 ID를 사용하여 실행 중인 애플리케이션(Service, Worker Pool, Job)의 hostname을 설정합니다.
  </Collapser>
...
이 설정들은 에이전트가 제공하는 다양한 OpenTelemetry 기반 기능을 제어합니다. 이 설정은 앱의 `newrelic.js` 구성 파일의 `opentelemetry` 섹션에 나타납니다.

<Callout variant="important">
  이 설정은 OpenTelemetry 기능 세트가 생성되는 동안 변경될 수 있습니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    전체 OpenTelemetry 기능에 대한 전역 스위치입니다. `false`로 설정하면 그 하위 기능이 어떻게 설정되었든, 다른 OpenTelemetry 하위 기능(`traces`, `logs`, `metrics` 등)이 활성화되지 않습니다.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    OpenTelemetry 계측(예: `@fastify/otel`)을 뉴렐릭 에이전트에 연결할 수 있도록 합니다.
  </Collapser>
  ...
  <Collapser id="opentelemetry-logs-enabled" title="opentelemetry.logs.enabled">
    ...
    `true`인 경우, OpenTelemetry 전용 API를 통해 내보낸 로그를 뉴렐릭으로 전송하도록 에이전트가 OpenTelemetry 로그 API를 자동으로 구성합니다.

    <Callout variant="important">
      이 기능은 [애플리케이션 로그 포워딩](#application-logging-forwarding-enabled)에 따라 달라집니다. ― `application_logging.forwarding.enabled`도 마찬가지로 `true`로 설정되어야 합니다.
    </Callout>
  </Collapser>

  <Collapser id="opentelemetry-metrics-enabled" title="OpenTelemetry.메트릭.enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    `true`인 경우, 에이전트에 의해 계측된 애플리케이션 엔터티에 첨부된 메트릭을 뉴렐릭으로 전송하도록 에이전트가 OpenTelemetry 메트릭 API를 자동으로 구성합니다.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-interval" title="opentelemetry.metrics.export_interval">
    ...
    OpenTelemetry 메트릭을 뉴렐릭으로 전송하려는 시도 간격으로, 단위는 밀리초 수입니다. `opentelemetry.metrics.export_timeout` 이상이어야 합니다.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-timeout" title="opentelemetry.metrics.export_timeout">
    ...
    OpenTelemetry 메트릭 내보내기 작업이 성공적으로 완료되기 전까지 허용되는 밀리초 수입니다. 시간 초과가 발생하면 OpenTelemetry 진단 API를 통해 보고됩니다.
  </Collapser>
</CollapserGroup>

## Apollo Server 계측 [#apollo-server-instrumentation]

이 설정에서 사용자는 에이전트의 Apollo Server(GraphQL) 계측 동작을 정의할 수 있습니다. 이 설정은 앱 `newrelic.js` 구성 파일의 `apollo_server` 섹션에 나타납니다.
...
<CollapserGroup>
  <Collapser id="apollo-server-scalars" title="apollo_server.scalars">
    ...
    `GraphQLScalarType` 반환 유형으로 해결된 필드의 타이밍 캡처를 활성화합니다. 스칼라 값을 반환하기 위해 시간이 많이 소요되는 계산을 수행할 때 필요할 수 있습니다. 미리 계산된 스칼라 필드를 다수 반환하는 쿼리에는 권장하지 않습니다.

    <Callout variant="tip">
      쿼리/뮤테이션 리졸버는 스칼라 유형을 반환하는 경우에도 항상 캡처됩니다.
    </Callout>
  </Collapser>
  ...
  <Collapser id="apollo-server-introspection-queries" title="apollo_server.introspection_queries">
    ...
    [IntrospectionQuery](https://www.graphql-js.org/api-v16/utilities/#introspectionquery)에 대한 타이밍 캡처를 활성화합니다.
  </Collapser>
  ...
  <Collapser id="apollo-server-service-definition-queries" title="apollo_server.service_definition_queries">
    ...
    Apollo Federated Gateway Server에서 수신된 [서비스 정의 쿼리](https://www.apollographql.com/docs/federation/federation-spec/#fetch-service-capabilities)에 대한 타이밍 캡처를 활성화합니다.
  </Collapser>
  ...
  <Collapser id="apollo-server-health-check-queries" title="apollo_server.health_check_queries">
    ...
    Apollo Federated Gateway Server에서 수신한 [상태 확인 쿼리](https://www.apollographql.com/docs/federation/api/apollo-gateway/#servicehealthcheck)에 대한 타이밍 캡처를 활성화합니다.
  </Collapser>
  ...
  <Collapser id="apollo-server-field-metrics" title="apollo_server.field_metrics">
    ...
    Apollo 쿼리에서 관찰된 모든 필드 및 리졸버 인수에 대한 메트릭 캡처를 활성화합니다. GraphQL 스키마에서 사용되지 않는 필드를 식별하기 위한 것입니다.
  </Collapser>
</CollapserGroup>
