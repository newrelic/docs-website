...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    활성화된 경우, 에이전트는 GCP 메타데이터 인스턴스 ID를 사용하여 실행 중인 애플리케이션의 hostname을 설정합니다.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    ...
    `gcp_use_instance_as_host`도 함께 활성화되어 있는 경우, 실행 중인 애플리케이션의 hostname을 설정할 때 GCP Cloud Run 리비전(`K_REVISION`)이 인스턴스 ID 앞에 추가되어 hostname이 `${K_REVISION}-${instance_id}`가 됩니다.
  </Collapser>
</CollapserGroup>
