...
Node.js VM 데이터로 선별된 차트를 보려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Node VMs** 으로 이동합니다. 자세한 내용은 [Node.js VM 통계 페이지](/docs/agents/nodejs-agent/supported-features/node-vms-statistics-page) 를 참조하세요.

데이터는 [데이터 탐색기](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer) 에서도 사용할 수 있습니다.
