...
Node.js VM 데이터로 선별된 차트를 보려면 **[one.newrelic.com > All capabilties](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Node VMs** 으로 이동합니다. 자세한 내용은 [Node.js VMs statistics page](/docs/agents/nodejs-agent/supported-features/node-vms-statistics-page) \[Node.js VMs 통계 페이지를] 참조하세요.
