...
일부 Node.js VM 메트릭은 내장된 Node.js API를 사용하여 기본적으로 수집되는 반면, 다른 메트릭은 선택 사항인 [`@newrelic/native-metrics`](https://www.npmjs.com/package/@newrelic/native-metrics) 패키지가 필요합니다:

* **기본 샘플러(네이티브 모듈 필요 없음):** 메모리(`Memory/*`), 이벤트 루프 대기열 대기 시간(`Events/wait`) 및 CPU(`CPU/*`)는 항상 각각 `process.memoryUsage()`, `setTimeout` 지연 측정 및 `process.cpuUsage()` 을(를) 사용하여 수집됩니다.
* **네이티브 모듈 샘플러:** 가비지 컬렉션(`GC/*`) 및 정밀한 이벤트 루프 CPU 사용량(`Nodejs/EventLoop/CPU/Usage`)에는 `@newrelic/native-metrics`이(가) 필요합니다. 모듈이 설치되지 않거나 로드에 실패하면, 이 메트릭은 표시되지 않으며 에이전트는 기본 샘플러로만 계속 실행됩니다.
...
<CollapserGroup>
  <Collapser id="time-in-garbage-collection" title="가비지 수집 시간">
    <Callout variant="important">`@newrelic/native-metrics`이(가) 필요합니다. 이 메트릭은 기본적으로 수집되지 않습니다.</Callout>
  ...
  <Collapser id="memory" title="메모리">
    <Callout variant="tip">기본적으로 수집됩니다. 네이티브 모듈이 필요하지 않습니다.</Callout>
  ...
  <Collapser id="cpu" title="CPU">
    <Callout variant="tip">Node.js v6.1.0에서 기본적으로 수집됩니다 이상. 네이티브 모듈이 필요하지 않습니다.</Callout>
  ...
  <Collapser id="event-loop" title="이벤트 루프">
    <Callout variant="important">`@newrelic/native-metrics`이(가) 필요합니다. 이 메트릭은 기본적으로 수집되지 않습니다.</Callout>
