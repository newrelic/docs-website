...
Node.js 에이전트를 사용하면 웹 페이지에 [브라우저 계측을](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) 추가할 수 있습니다. 사용 <InlinePopover type="browser"/>Node.js 에이전트를 사용하려면 Node.js 에이전트의 [최신 릴리스가](/docs/agents/nodejs-agent/installation-configuration/installing-maintaining-nodejs#upgrading) 있는지 확인하세요.
