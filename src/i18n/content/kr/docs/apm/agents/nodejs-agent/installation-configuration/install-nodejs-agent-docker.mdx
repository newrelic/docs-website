...
5. 에이전트가 활성화된 상태에서 Docker 앱을 실행하려면 [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 와 [앱 이름을](/docs/agents/manage-apm-agents/app-naming/name-your-application) 환경 변수로 `docker run` 명령어에 추가하세요.
