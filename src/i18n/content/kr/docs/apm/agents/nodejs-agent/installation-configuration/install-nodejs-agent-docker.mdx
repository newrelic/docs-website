...
5. 에이전트가 활성화된 상태에서 Docker 앱을 실행하려면 <InlinePopover type="licenseKey" /> 와 [앱 이름](/docs/agents/manage-apm-agents/app-naming/name-your-application) 을 `docker run` 명령어에 환경 변수로 추가하세요.
