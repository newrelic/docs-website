...
<CollapserGroup>
  ...
  >
    ...
    * `handle` 은 계측하려는 함수를 정의합니다.
    ...
    프로세서 시작 시 `newrelic.getTransaction()` 호출하여 수동으로 사용자 정의 프로세서를 <DNT>**must**</DNT> 처리하고 완료되면 `transaction.end()` 호출합니다. 뉴렐릭은 `newrelic.startWebTransaction()` 이 호출될 때 프로세서 타이밍을 시작하고 `transaction.end()` 호출될 때 프로세서를 종료합니다.
  ...
  >
    ...
    * `group` 는 선택사항이며 [사용자 인터페이스의<DNT>**transaction type**</DNT> ](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)를 통해 유사한 작업을 그룹화할 수 있습니다. `name` 와 마찬가지로 `group` 도 정적이어야 합니다.
    * `handle` 은 계측하려는 전체 백그라운드 작업을 포함하는 함수를 정의합니다.

    New Relic은 자동 계측 및 [`startSegment()`](#startSegment) 을 통한 수동 계측으로 캡처되는 모든 측정항목을 캡처합니다.

    프로세서 시작 시 `newrelic.getTransaction()` 호출하여 수동으로 사용자 정의 프로세서를 <DNT>**must**</DNT> 처리하고 완료되면 `transaction.end()` 호출합니다. 뉴렐릭은 `newrelic.startBackgroundTransaction()` 이 호출될 때 프로세서 타이밍을 시작하고 `transaction.end()` 호출될 때 프로세서를 종료합니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `value` 이 객체인 경우 `count`, `total`, `min`, `max` 및 `sumOfSquares` 키를 포함해야 하며 모두 숫자 값을 가지며 다른 키는 무시됩니다. 이 양식은 스스로 데이터를 작성하고 정기적으로 보고하는 데 유용합니다. 예를 들어, `setInterval` 에서. 이러한 값은 동일한 지표에 대해 이전에 수집된 값과 집계됩니다. 이러한 키의 이름은 플랫폼 API에서 사용되는 키의 이름과 일치합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    * `attributes` 은 키 및 값 쌍의 객체여야 합니다. 키는 255자 미만이어야 하고 값은 문자열, 숫자 또는 부울이어야 합니다.
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="사용자 지정 이벤트 녹화">
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="오류 개체와 함께 로그 이벤트 기록">
...
## 트랜잭션 핸들 메소드 [#transaction-handle-methods]
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="분산 추적 헤더 생성">
  ...
  >
    ...
    * `Unknown`
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="들어오는 분산 추적 헤더 수락">
  ...
  >
    ...
    * `DistributedTracePayload#text`: 페이로드의 JSON 표현을 반환합니다.
      ...
      <CollapserGroup>
        <Collapser id="example-link-bg-txn" title="중첩된 백그라운드 트랜잭션 연결">
          ```js
          // Call newrelic.getTransaction to retrieve a handle on the current transaction.
    ...
    * `DistributedTracePayload#httpSafe`: 페이로드의 base64로 인코딩된 JSON 표현을 반환합니다.
      ...
      <CollapserGroup>
        <Collapser id="example-payload-outgoing-request" title="나가는 요청에 페이로드 배치">
          ```js
          // Call newrelic.getTransaction to retrieve a handle on the current transaction.
          var transactionHandle = newrelic.getTransaction();
          var payload = transactionHandle.createDistributedTracePayload();
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="맞춤 속성 추가">
        ```js
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="사용자 정의 속성 추가">
        ```js
        const attributes = {
          attribute1: 'value1',
          attribute2: 2
  ...
  <Collapser
    title={<InlineCode>newrelic.addCustomSpanAttribute(이름, 값)</InlineCode>
    }
  >
    ```js
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="사용자 지정 범위 속성 추가">
        ```js
  ...
  <Collapser
    title={<InlineCode>newrelic.addCustomSpanAttributes(속성)</InlineCode>
    }
  >
    ```js
  ...
  >
    ...
    [<InlinePopover type="browser" />](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs)) 활성화하기 위해 HTML 페이지 헤더에 삽입할 HTML 스니펫을 반환합니다. HTML은 브라우저에 작은 JavaScript 파일을 가져오고 페이지 타이머를 시작하도록 지시합니다.
  </Collapser>
  ...
  >
    ...
    이 함수를 여러 번 호출하면 이전에 정의된 이 콜백 함수 버전이 대체됩니다. 예:
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `true` 인 경우 활성 트랜잭션이 없을 때까지 에이전트가 종료되지 않습니다.
          </td>
        </tr>
      </tbody>
    </table>
...
<CollapserGroup>
  ...
  >
    <DNT>**rules.name**</DNT> 의 프로그래밍 방식 버전입니다. 명명 규칙이 추가되면 Node.js 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 설정을 통해 추가할 수도 있습니다. 두 가지 유효성 검사가 모두 필요합니다.
  </Collapser>
  ...
  >
    <DNT>**rules.ignore**</DNT> 의 프로그래밍 방식 버전입니다. 무시 규칙이 추가되면 Node.js 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 설정을 통해 추가할 수도 있습니다. 이 조치가 필요합니다.
  </Collapser>
</CollapserGroup>