...
<CollapserGroup>
  ...
  >
    ...
    * `handle` 은 계측하려는 함수를 정의합니다.
    ...
    프로세서 시작 시 `newrelic.getTransaction()` 호출하여 수동으로 사용자 정의 프로세서를 <DoNotTranslate>**must**</DoNotTranslate> 처리하고 완료되면 `transaction.end()` 호출합니다. 뉴렐릭은 `newrelic.startWebTransaction()` 이 호출될 때 프로세서 타이밍을 시작하고 `transaction.end()` 호출될 때 프로세서를 종료합니다.
  ...
  >
    ...
    * `group` 는 선택사항이며 [사용자 인터페이스의](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)

      <DoNotTranslate>[**transaction type**](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)</DoNotTranslate>

      [ ](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)를 통해 유사한 작업을 그룹화할 수 있습니다. `name` 와 마찬가지로 `group` 도 정적이어야 합니다.

    * `handle` 은 계측하려는 전체 백그라운드 작업을 포함하는 함수를 정의합니다.

    New Relic은 자동 계측 및 [`startSegment()`](#startSegment) 을 통한 수동 계측으로 캡처되는 모든 측정항목을 캡처합니다.

    프로세서 시작 시 `newrelic.getTransaction()` 호출하여 수동으로 사용자 정의 프로세서를 <DoNotTranslate>**must**</DoNotTranslate> 처리하고 완료되면 `transaction.end()` 호출합니다. 뉴렐릭은 `newrelic.startBackgroundTransaction()` 이 호출될 때 프로세서 타이밍을 시작하고 `transaction.end()` 호출될 때 프로세서를 종료합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="규칙을 무시하는 예"
  >
    <DoNotTranslate>**socket.io**</DoNotTranslate> 사용하는 경우 즉시 규칙을 무시하는 사용 사례를 갖게 됩니다. Socket.io 장기 폴링이 응답 시간 지표를 지배하고 앱의 Apdex 지표에 영향을 미치지 않도록 하려면 다음과 같은 규칙을 추가하세요.
...
<CollapserGroup>
  ...
  >
    <DoNotTranslate>**rules.name**</DoNotTranslate> 의 프로그래밍 방식 버전입니다. 명명 규칙이 추가되면 Node.js 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 설정을 통해 추가할 수도 있습니다. 두 가지 유효성 검사가 모두 필요합니다.
  </Collapser>
  ...
  >
    <DoNotTranslate>**rules.ignore**</DoNotTranslate> 의 프로그래밍 방식 버전입니다. 무시 규칙이 추가되면 Node.js 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 설정을 통해 추가할 수도 있습니다. 이 조치가 필요합니다.
  </Collapser>
</CollapserGroup>