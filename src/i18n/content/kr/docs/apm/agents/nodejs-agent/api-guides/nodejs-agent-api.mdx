...
<CollapserGroup>
  ...
  >
    ...
    에이전트를 정상적으로 종료하려면 이 방법을 사용하십시오.
...
<CollapserGroup>
  ...
  >
    **rules.name** 의 프로그래밍 방식 버전입니다. 명명 규칙이 추가되면 노드 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 구성을 통해 추가할 수도 있습니다. 두 매개변수가 모두 필요합니다.
  </Collapser>
  ...
  >
    **rules.ignore** 의 프로그래밍 방식 버전입니다. 무시 규칙이 추가되면 노드 프로세스가 다시 시작될 때까지 제거할 수 없습니다. Node.js 에이전트의 구성을 통해 추가할 수도 있습니다. 이 매개변수는 필수입니다.
  </Collapser>
</CollapserGroup>