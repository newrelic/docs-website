...
뉴렐릭의 [Node.js 에이전트를](/docs/agents/nodejs-agent/getting-started/introduction-new-relic-nodejs) 사용하면 [Google App Engine(GAE) 가변형 환경](https://cloud.google.com/appengine/docs/flexible/nodejs/) 에 상주하는 모니터를 사용할 수 있습니다. GAE Flex 앱에 뉴렐릭을 추가하면 인사이트 앱의 상태와 성능에 대한 정보를 제공하고 [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm)및 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)에서 볼 수 있는 지표로 GAE를 확장합니다.
...
1. <InlinePopover type="licenseKey"/>포함하여 [뉴렐릭의 Node.js 에이전트를 설치](/docs/agents/nodejs-agent/installation-configuration/install-nodejs-agent) 하려면 표준 절차를 따르세요. `newrelic` 모듈을 `package.json` 파일에 저장해야 합니다.
...
Google App Engine은 구현하다, 배포하다 패키지에 Node.js 앱의 `newrelic.js` 설정 파일을 자동으로 포함합니다. 배포가 완료될 때까지 기다린 후 [APM <DNT>**Summary**</DNT> 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 에서 GAE Flex 앱 데이터를 확인하세요.
...
<CollapserGroup>
  ...
  >
    ...
    3. 뉴렐릭에서 GAE Flex 앱 데이터를 보려면 [APM](/docs/apm/applications-menu/monitoring/apm-overview-page)

       <DNT>
         [**Summary**](/docs/apm/applications-menu/monitoring/apm-overview-page)
       </DNT>

       [페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 로 이동하세요.
  </Collapser>
</CollapserGroup>
