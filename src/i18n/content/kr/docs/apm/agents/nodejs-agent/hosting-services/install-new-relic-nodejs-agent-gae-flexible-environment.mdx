...
1. 표준 절차에 따라 <InlinePopover type="licenseKey" /> 를 포함하여 [New Relic의 Node.js 에이전트를 설치](/docs/agents/nodejs-agent/installation-configuration/install-nodejs-agent) 합니다. `newrelic` 모듈을 `package.json` 파일에 저장해야 합니다.
