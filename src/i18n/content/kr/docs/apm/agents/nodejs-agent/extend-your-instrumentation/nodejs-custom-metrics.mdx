...
API 호출을 통해 임의의 성능 데이터(예: 타이밍 또는 컴퓨터 리소스 데이터)를 기록합니다. [데이터 탐색기](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer) 를 사용하여 사용자 지정 메트릭을 검색하고 사용자 지정 가능한 차트를 만들고 해당 차트를 New Relic 대시보드에 추가하십시오. 사용자 지정 메트릭을 사용하면 New Relic 내에서 모니터링이 통합됩니다.
...
<Callout variant="important">
  Node Agent v5 이전 버전의 경우 측정항목 앞에 `Custom/` 을 추가해야 합니다. 에이전트는 내부 메트릭과의 충돌을 방지하기 위해 사용자 지정 메트릭을 자동으로 추가하지 않습니다.
</Callout>
...
사용자 지정 메트릭을 보려면 [데이터 탐색기](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer) 를 사용하여 메트릭을 검색하고 사용자 지정 가능한 차트를 만들고 해당 차트를 New Relic 대시보드에 추가하십시오.