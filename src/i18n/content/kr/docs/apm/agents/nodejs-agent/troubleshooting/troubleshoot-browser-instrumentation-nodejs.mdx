...
New Relic Node.js 에이전트를 통해 [브라우저 애플리케이션을 계측하는 데](/docs/nodejs/page-load-timing-in-nodejs) 문제가 있는 경우 [표준을 따르세요. <InlinePopover type="browser"/>문제 해결 절차](/docs/new-relic-browser/troubleshooting-page-view-monitoring). Node.js에 대한 몇 가지 추가 팁은 다음과 같습니다.
