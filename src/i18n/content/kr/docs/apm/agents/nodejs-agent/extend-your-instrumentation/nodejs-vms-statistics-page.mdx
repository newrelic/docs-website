...
**노드 VM** 페이지의 일부 차트에서는 Node.js 에이전트와 함께 [New Relic Native Metrics Module](https://www.npmjs.com/package/@newrelic/native-metrics) 을 설치해야 합니다. 설치 지침은 [노드 VM 측정](/docs/agents/nodejs-agent/supported-features/node-vm-measurements) 을 참조하십시오.

## 노드 VM 통계 보기 [#view-data]
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Node VMs** : 여러 서버가 동일한 애플리케이션에 보고할 때 페이지는 서버별로 차트를 나눕니다. 페이지 상단의 **서버** 필터에서 서버를 선택하여 개별 서버에 대한 데이터를 보고 추가 세부 정보를 얻을 수 있습니다.
</figcaption>

이러한 측정의 급증 또는 상승 추세를 특정 거래와 연관시키려면:

1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) >Node VMs** 으로 이동합니다.
...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            노드 프로세스를 대신하여 시스템 커널에서 보낸 시간을 벽시계 시간으로 나눈 값입니다.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  >
    ...
    각 이벤트 루프 턴의 일부로 노드는 보류 중인 I/O가 완료될 때까지 기다립니다. I/O 이벤트가 수신되면 이벤트 루프는 이와 관련된 콜백을 실행합니다. 따라서 분당 틱 수는 I/O 이벤트가 발생하는 빈도에 해당합니다.
  </Collapser>
