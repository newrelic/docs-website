...
<figcaption>
  **[one.newrelic.com](http://one.newrelic.com) > APM > (앱 선택) > Node VMs** : 여러 서버가 동일한 애플리케이션에 보고하는 경우 페이지는 서버별로 차트를 나눕니다. 페이지 상단의 **서버** 필터에서 서버를 선택하여 개별 서버에 대한 데이터를 보고 추가 세부 정보를 얻을 수 있습니다.
</figcaption>

이러한 측정의 급증 또는 상승 추세를 특정 거래와 연관시키려면:

1. **[one.newrelic.com](http://one.newrelic.com)** **> APM > (앱 선택) > Node VMs** 으로 이동합니다.
