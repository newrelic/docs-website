...
<CollapserGroup>
  ...
  >
    ...
    <InlinePopover type="browser"/>에 대한 모든 속성을 켜거나 끕니다. 이는 PageView 대상으로 전송되는 데이터입니다. `attributes.enabled` 이 `false` 인 경우 이 구성 설정이 어떻게 설정되었는지에 관계없이 브라우저 모니터링에 속성이 전송되지 않습니다.
  </Collapser>
</CollapserGroup>
