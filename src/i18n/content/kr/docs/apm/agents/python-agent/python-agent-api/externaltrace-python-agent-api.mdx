...
* <DNT>
  ...
  : 컨텍스트 관리자 형식은 `ExternalTrace` 입니다.

* <DNT>
    **The wrapper**
  </DNT>

  : 래퍼 형식은 `ExternalTraceWrapper` 입니다. 데코레이터를 사용하지 않고 래핑된 함수를 반환하는 데 사용할 수 있습니다.

* <DNT>
    **The path-based wrapper**
  </DNT>

  : 경로 기반 래퍼 형식은 `wrap_external_trace` 입니다. 이는 Monkey 패치를 통해 지정된 객체에 `ExternalTraceWrapper` 적용합니다. 이는 데코레이터와 동일한 모범 사례와 추가 `module` 및 `object_path` 모범 사례를 사용합니다.
