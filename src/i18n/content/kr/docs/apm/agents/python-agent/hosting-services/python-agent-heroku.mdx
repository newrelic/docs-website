...
<CollapserGroup>
  <Collapser id="via_heroku" title="Heroku 웹사이트를 통해">
    ...
    2. <DNT>**Select an app**</DNT> 에서 뉴렐릭 앱을 선택하세요.
...
Heroku에 Python 에이전트와 같은 타사 Python 패키지를 설치하려면 <DNT>**pip**</DNT> 사용하세요. Heroku는 프로젝트의 루트 디렉터리에서 <DNT>**requirements.txt**</DNT> 파일을 자동으로 찾습니다. 프로젝트를 Heroku에 푸시하면 해당 파일에 나열된 모든 항목이 설치됩니다.
...
2. <DNT>**Django users:**</DNT> 값 앞에 `newrelic-admin run-program` 추가하여 `Procfile` 의 `web` 항목을 수정합니다. 예를 들어:
...
<InlinePopover type="licenseKey" />은(는) 귀하의 뉴렐릭 계정에 고유합니다.
...
이렇게 하면 연결이 생성되고 <DNT>**Python Agent Test**</DNT> 애플리케이션 아래에 테스트 트랜잭션 데이터가 보고됩니다. 테스트 실행 결과를 캡처하고 데이터를 사용하여 문제를 해결합니다. 추가 지원이 필요한 경우 [Python 에이전트 문제 해결 절차를](/docs/agents/python-agent/troubleshooting/no-data-appears-python) 따르세요.
...
<Callout variant="caution">
  Python 버전 2.7.4 또는 Django 1.4 이전 웹 프레임워크의 내장 개발 호스트를 사용하지 마세요. 대신 <DNT>**gunicorn**</DNT> 또는 <DNT>**uWSGI**</DNT> 사용하세요.

  <DNT>**wsgiref**</DNT> 모듈을 사용하는 WSGI 호스트는 Python 버전 2.7.4 이전의 개발 호스트에 대해 WSGI를 완전히 준수하지 않았습니다. 이로 인해 Python 에이전트가 올바른 데이터를 보고할 수 없게 되었습니다.
</Callout>
...
WSGI 호스트를 기능하기 위해 <DNT>**newrelic-admin**</DNT> 래퍼 프로그램을 사용하는 경우, 인스턴스 키, 전설적인 이름 등에 대한 설정은 Heroku 에서 설정한 환경 변수에서 선택됩니다. 에이전트 구성 파일에 설정한 추가 설정도 적용됩니다. 그런 다음 에이전트가 뉴렐릭에 등록하면 에이전트가 사용할 최종 설정을 생성하기 위해 모든 서버 측 설정도 병합됩니다.