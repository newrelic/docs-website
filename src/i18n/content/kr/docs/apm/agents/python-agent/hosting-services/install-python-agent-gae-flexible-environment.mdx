...
1. <InlinePopover type="licenseKey" />포함하여 [Python 에이전트를 설치](/docs/agents/python-agent/getting-started/python-agent-quick-start#install) 하려면 표준 절차를 따르세요.
...
<CollapserGroup>
  ...
  <Collapser id="deploy-docker-image-to-gae" title="4. 초기화된 GAE 가변형 환경에 Docker 이미지 배포">
...
권장 사항: 다음을 추가하여 [상태 확인을 비활성화](https://cloud.google.com/appengine/docs/flexible/python/configuring-your-app-with-app-yaml#health_checks) 하도록 `app.yaml` 구성하세요.
