...
<Callout variant="important">
  Python 에이전트의 경우,[에이전트 버전 v7.0.0.166](/docs/release-notes/agent-release-notes/python-release-notes/python-agent-70000166) [부터 인력 추적 기능이](/docs/apm/transactions/cross-application-traces/introduction-cross-application-traces) 더 이상 사용되지 않으며 향후 에이전트 릴리스에서 제거될 예정입니다. 이제 [분산 추적](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing) 기능을 사용할 수 있습니다. 분할 추적은 끝부분 추적을 개선하며 복잡한 분할 시스템의 모니터링 활동에 권장됩니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="custom-client-http-outgoing" title="발신 요청">
    ```py
  ...
  <Collapser id="custom-client-http-incoming" title="수신 응답">
    ```py
    import newrelic.agent
...
<CollapserGroup>
  <Collapser id="custom-client-non-http-outgoing" title="발신 요청">
    ```py
    import newrelic.agent
  ...
  <Collapser id="custom-client-non-http-incoming" title="수신 응답">
    ```py
    import newrelic.agent
...
WSGI 서버 계측에 대한 자세한 내용 [은 wsgi\_application API 설명서](/docs/agents/python-agent/python-agent-api/wsgi_application) 를 참조하십시오.
...
<CollapserGroup>
  <Collapser id="custom-non-http-server-example" title="계측 예시">
    ```py
    import newrelic.agent
