...
## 데코레이터와 자기 성찰 [#decorators\_and\_introspection][#decorators_and_introspection]
...
모듈과 함수의 이름이 올바르게 지정되었는지 확인하려면 표준 라이브러리 <DNT>**functools**</DNT> 모듈의 `wraps()` 데코레이터를 사용하여 내부 데코레이터 함수를 래핑하세요.
...
## 거래 식별 [#identifying\_a\_transaction][#identifying_a_transaction]
...
이러한 교차 일치를 수행하거나 이 상황에 도움이 될 수 있는 일부 추가 세부 정보를 소니에 대해 간단히 캡처하려는 경우 에이전트 API 사용하여 소니에 대해 사용자 지정 매개변수를 기록할 수 있습니다. 오류가 발생하거나 느린 칩셋 트레이스가 기록되어 UI 에 표시되는 경우, 귀하가 제공하는 이러한 추가 프로세서는 오류 또는 트레이스 세부 정보의 <DNT>**Custom parameters**</DNT> 섹션 아래에 표시됩니다.

이러한 추가 세부정보를 추가하려면 <DNT>**newrelic.agent**</DNT> 모듈에서 `add_custom_attribute()` 함수를 사용하면 됩니다.
...
사용자 지정 시위에 사용되는 값은 <DNT>**json**</DNT> 모듈에서 제공하는 JSON 인코더로 직렬화할 수 있는 모든 값이 될 수 있습니다. 여기에는 튜플, 목록 및 사전이 포함됩니다.

권장 사항: 값을 정수, 러닝 또는 문자열 값으로 제한하세요. 더 복잡한 데이터 구조를 사용하는 경우 너무 큰 데이터 구조를 연결하지 마십시오. 데이터 구조는 수집될 때까지 캐시되고 패키지된 다음 핵심 애플리케이션으로 전송되어야 하기 때문입니다.

값을 JSON으로 직렬화할 수 없는 경우 오류 세부 정보 또는 느린 트랜잭션이 폐기될 수 있으므로 간단한 값도 선호됩니다.

## 맞춤 측정항목 기록 [#recording\_custom\_metrics][#recording_custom_metrics]
...
맞춤 지표를 기록하려면 <DNT>**newrelic.agent**</DNT> 모듈에서 `record_custom_metric()` 함수를 사용하세요. 지표 이름과 값이라는 두 가지 인수를 사용합니다. 값은 정수 또는 부동 소수점 값이어야 합니다.
