...
* New Relic
  ...
  를 가지고 있습니다. (아직 계정이 없다면 [New Relic 계정에 가입하세요](https://newrelic.com/signup) .)
...
패키지를 설치하면 `newrelic-admin script` 가 Python 실행 파일과 동일한 디렉터리에 있게 됩니다. 에이전트 구성 파일을 만들려면 Python 실행 파일의 디렉터리에서 이 명령을 실행하고 <LicenseKey/> 를 포함합니다.
