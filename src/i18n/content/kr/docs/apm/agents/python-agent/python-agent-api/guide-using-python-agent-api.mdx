...
[브라우저 모니터링 에이전트](/docs/browser/new-relic-browser/installation-configuration/add-apps-new-relic-browser) 를 페이지에 자동으로 추가하거나, <InlinePopover type="browser"/>에이전트 JavaScript 스니펫. APM 에이전트 API 호출을 사용하여 브라우저 에이전트를 제어할 수도 있습니다. 자세한 내용은 [브라우저 에이전트 및 Python 에이전트를](/docs/agents/python-agent/supported-features/page-load-timing-python) 참조하세요.
