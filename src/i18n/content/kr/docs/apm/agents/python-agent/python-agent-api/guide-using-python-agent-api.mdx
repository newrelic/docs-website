...
## 세그먼트 및 세그먼트 속성 이름 동적으로 지정하기 [#dynamically-name-segments-and-segment-attributes]

세그먼트 및 세그먼트 속성 값의 이름을 동적으로 지정하려면, 다양한 파라미터에 대해 정적 값 대신 함수를 허용하는 커스텀 측정 API 호출을 사용하십시오. 에이전트 버전 13.0.0 이전에는, Lambda 함수를 사용하여 설정 파일에서 이 작업을 수행할 수도 있습니다. 이는 이제 커스텀 측정 API 호출에서만 지원됩니다.

다음 API 호출은 동적 이름 지정을 지원합니다:
