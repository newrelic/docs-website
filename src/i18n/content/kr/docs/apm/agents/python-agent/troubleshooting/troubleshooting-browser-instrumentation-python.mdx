...
Python 에이전트에 문제가 [있는 라이브러리](/docs/agents/python-agent/supported-features/page-load-timing-python) 에 문제가 있는 경우 <InlinePopover type="browser" />에 대한 [표준 문제 해결 절차를](/docs/browser/new-relic-browser/installation-configuration/troubleshooting-browser-monitoring-installation) 따르세요. Python에 대한 추가 팁은 다음과 같습니다.
...
<CollapserGroup>
  <Collapser id="solution-content" title="콘텐츠 구조를 확인하십시오.">
  ...
  <Collapser id="solution-compression" title="콘텐츠 압축이 방해되지 않는지 확인합니다.">
  ...
  <Collapser id="solution-compression" title="조건부 블록에 대한 페이지 소스를 확인하십시오.">
