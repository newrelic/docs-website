...
Python 에이전트가 보고하는 모든 사용자 정의 측정항목은 접두사 `Custom/` 으로 시작해야 합니다. 일반적으로 범주 이름과 레이블 세그먼트가 뒤에 옵니다. `Custom/` 측정항목을 사용하지 않으면 [데이터 탐색기](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer) 에서 맞춤 측정항목을 선택하지 못할 수 있습니다.
