...
<CollapserGroup>
  ...
  >
    **오류 - 에이전트 구성에 라이센스 키가 설정되지 않았습니다.**
    ...
    에이전트 구성 파일 또는 `NEW_RELIC_LICENSE_KEY` 환경 변수를 통해 <InlinePopover type="licenseKey" /> 가 지정되었는지 확인하십시오. 라이센스 키로 문제를 수정하거나 [support.newrelic.com](https://support.newrelic.com) 에서 지원을 받으십시오.
  </Collapser>
  ...
  >
    **오류 - 데이터 수집기가 에이전트에서 잘못된 라이센스 키를 제공했음을 나타냅니다.**
    ...
    에이전트가 사용하는 값은 `...` 입니다. <InlinePopover type="licenseKey" /> 문제를 해결하거나 [support.newrelic.com](https://support.newrelic.com) 에서 지원을 받으십시오.
  </Collapser>
