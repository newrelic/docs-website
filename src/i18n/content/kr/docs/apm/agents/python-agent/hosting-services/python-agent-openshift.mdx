...
Python 에이전트가 데이터를 올바른 계정에 보고하려면 New Relic 계정의 [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 알려야 합니다. OpenShift의 경우 라이선스 키를 제공하는 가장 안전한 방법은 `rhc env set` 명령을 사용하여 애플리케이션 구성에 구성된 환경 변수를 사용하는 것입니다. 이렇게 하면 GIT 리포지토리에 민감한 정보가 저장되지 않으며 여러 물리적 호스트에서 호스팅되는 확장된 웹 애플리케이션을 사용하는 경우에도 작동합니다.
