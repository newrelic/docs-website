...
* **경로 기반 래퍼:** 경로 기반 래퍼 호출은 `wrap_asgi_application` 입니다. 계측 범위에서 ASGI 개체를 변수로 참조할 수 없는 경우 이것을 사용하십시오. 이는 추가 `module` 및 `object_path` 매개변수가 있는 데코레이터와 동일한 매개변수를 사용합니다.
