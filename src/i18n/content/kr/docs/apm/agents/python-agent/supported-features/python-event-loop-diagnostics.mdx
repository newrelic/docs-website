...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitoring > Transactions > (트랜잭션 선택):** 트랜잭션의 **분석 테이블** 은 asyncio로 이벤트 루프에 플래그를 지정합니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) >Transactions > (트랜잭션 추적 선택) > Trace details:** **추적 세부 정보** 페이지에는 트랜잭션 추적의 각 세그먼트(메서드 또는 함수 호출)에 대한 분석이 포함된 테이블이 있습니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Query your data > Advanced (NRQL)** : 고급(NRQL)을 사용하여 이벤트 루프를 찾는 쿼리를 작성합니다.
</figcaption>
