...
이 예에서 /some/path/newrelic.ini는 [Python 에이전트 설치](/docs/agents/python-agent/getting-started/python-agent-quick-start) 중에 생성된 구성 파일의 복사본 위치를 나타냅니다. 구성 파일은 웹 애플리케이션에서 읽을 수 있어야 합니다.
