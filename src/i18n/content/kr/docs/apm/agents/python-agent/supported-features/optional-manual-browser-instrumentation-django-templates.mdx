...
Python 에이전트는 자동으로 추가할 수 있습니다. <InlinePopover type="browser"/>모든 HTML 페이지 응답에. Django, Flask, Pyramid 등 가장 널리 사용되는 Python 웹 프레임워크의 경우 [자동 브라우저 계측을](/docs/agents/python-agent/supported-features/page-load-timing-python) 사용하세요.
