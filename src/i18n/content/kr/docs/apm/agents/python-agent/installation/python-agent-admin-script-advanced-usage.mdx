...
Python 에이전트 관리 스크립트는 [설치 프로세스](/docs/apm/agents/python-agent/getting-started/introduction-new-relic-python/#installation) 중에 사용됩니다. Python 에이전트 패키지가 설치되면 `newrelic-admin` 스크립트도 Python 설치 또는 가상 환경의 `bin` 디렉터리에 설치됩니다.
...
샘플 에이전트 설정 파일을 생성합니다. `license_key` 옵션은 <InlinePopover type="licenseKey"/>입니다.
...
우리 서버에 연결할 때 테스트는 귀하의 계정에 `Python Agent Test` 라는 애플리케이션을 생성합니다. 이 애플리케이션에는 소량의 시뮬레이션된 웹 트랜잭션 데이터와 오류가 보고됩니다. 원하는 경우 일정 시간이 지난 후 귀하의 계정에 있는 이 애플리케이션이 <DNT>**Applications**</DNT> 목록에서 삭제될 수 있습니다.
...
Heroku에서 애플리케이션을 호스팅할 때 개별 환경 변수가 사용됩니다. 환경 변수는 뉴렐릭 애드온을 추가하면 Heroku 환경에 맞게 자동으로 설정됩니다. 따라서 <DNT>**Procfile**</DNT> 에서 이 옵션과 함께 `newrelic-admin` 명령을 사용할 때 Heroku를 사용하여 위의 환경 변수를 설정할 필요가 없습니다.
...
`sudo` 를 사용하여 WSGI 를 다른 사용자로 시작하는 경우 기본 <DNT>**sudo**</DNT> 설정은 일반적으로 <DNT>**sudo**</DNT> 가 실행되는 셸의 사용자 환경 변수가 무시된다는 점에 유의하세요. 이 경우 특정 환경 변수 전달을 허용하도록 <DNT>**sudoers**</DNT> 설정을 변경해야 합니다. 또는 환경 변수를 설정하고 `newrelic-admin` 아래에서 WSGI 애플리케이션을 실행하는 쉘 스크립트를 작성하십시오. WSGI 애플리케이션을 다른 사용자로 시작하려면 WSGI 애플리케이션에서 직접 실행하는 대신 해당 스크립트에서 <DNT>**sudo**</DNT> 실행하세요.

<DNT>**supervisord**</DNT> 구성 내의 환경 변수 설정이 올바르게 전달되지 않는 경우 <DNT>**supervisord**</DNT> 사용할 때 환경 변수를 설정하고 WSGI 애플리케이션을 실행하기 위해 추가 래퍼 스크립트를 사용해야 할 수도 있습니다 `newrelic-admin` .
...
포트 값이 `0` 이면 데이터 수집기와 통신하는 데 SSL 연결이 사용되는지 여부에 따라 포트 번호( `80` 또는 `443` 가 자동으로 선택된다는 의미입니다.
...
1. [](/docs/apis/rest-api-v2/requirements/api-keys)[](/docs/agents/python-agent/installation-configuration/python-agent-configuration#agent-configuration-file)섹`newrelic.ini`션 `[newrelic]` 에 다음과 유사한 줄을 추가하여 에이전트 구성 파일 ( )에 API 키를 추가합니다.
...
<CollapserGroup>
  ...
  >
    ...
    환경 변수를 별도의 구성 설정에서 설정해야 하는 <DNT>**supervisord**</DNT> 와 같은 프로세스 관리 시스템을 사용하는 경우 명령과 동일한 줄에 설정할 수 없습니다.

    예를 들어, <DNT>**supervisord**</DNT> 에서 다음을 사용할 수 있습니다. `YOUR_COMMAND_OPTIONS` 기존 명령 옵션으로 대체하십시오.
  ...
  >
    ...
    `run-python` 사용하면 항상 <DNT>**newrelic-admin**</DNT> 가 설치된 Python 설치 또는 가상 환경에 설치된 것과 동일한 `python` 실행 파일이 사용됩니다.
  </Collapser>
  ...
  >
    실행하는 <DNT>**newrelic-admin**</DNT> 프로그램은 애플리케이션이 사용하는 것과 동일한 Python 설치 또는 가상 환경에서 가져와야 합니다. 서로 다른 Python 설치의 프로그램/구성 요소를 혼합할 수 없습니다. 이렇게 하면 에이전트가 제대로 실행되지 않습니다.

    가상 환경에서 <DNT>**supervisord**</DNT> 과 같은 프로세스 관리 시스템을 사용하는 경우 다음 설정을 사용할 수 있습니다.
