...
* 표준(헤드 기반 샘플링): 추적이 도착하기 전에 수락하고 분석할 추적의 비율을 결정합니다. 이는 추적이 어떻게 도움이 되는지 확인할 수 있는 견고한 출발점을 제공합니다. Python 에이전트 7.0.0.166 이상에서는 기본적으로 켜져 있습니다.
...
* [이전 Python 에이전트](#older-agents): 이전 Python 에이전트가 있는 경우 추적 옵션
* [수동 계측](#manual-instrumentation): 자동 계측이 작동하지 않는 경우 팁
...
시작하기 전에 먼저 [요구 사항을](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing/#requirements)충족하는지 확인하십시오.
...
<CollapserGroup>
  <Collapser id="python-version" title="Python 에이전트 호환성">
...
<CollapserGroup>
  <Collapser id="python-config-standard" title="Python 에이전트 구성">
...
<CollapserGroup>
  <Collapser id="calling-service" title="호출 서비스 계측">
    ...
    2. 에이전트 API 호출을 호출하여 트레이스 파이로드를 생성합니다( [Python 에이전트 API](/docs/agents/python-agent/python-agent-api/create_distributed_trace_payload) 참조).
