...
<Callout variant="imporant">
  함수가 아직 계측되지 않은 경우 [function_trace()](/docs/apm/agents/python-agent/python-agent-api/functiontrace-python-agent-api) 를 사용하여 백그라운드 작업 내에서 해당 함수에 대한 함수 추적을 만들 수 있습니다. 또는 [구성 파일](/docs/apm/agents/python-agent/custom-instrumentation/python-custom-instrumentation-config-file/#listing_functions) 을 사용하여 계측되지 않은 기능을 계측할 수 있습니다.
</Callout>
