...
* <DNT>
  ...
  : 경로 기반 래퍼 형식은 `wrap_message_transaction` 입니다. 이는 Monkey 패치를 통해 지정된 객체에 `MessageTransactionWrapper` 적용합니다. 이는 데코레이터와 동일한 모범 사례와 추가 `module` 및 `object_path` 모범 사례를 사용합니다.
