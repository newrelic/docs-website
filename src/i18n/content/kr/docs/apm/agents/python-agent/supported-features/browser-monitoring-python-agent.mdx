...
<Callout variant="important">
  Python 에이전트 버전 9.4.0 이상부터 get_browser_timing_footer API는 더 이상 사용되지 않으며 항상 빈 문자열을 반환합니다. `get_browser_timing_footer()` 의 이전 기능이 이제 `get_browser_timing_header()` 에 포함되었습니다.
...
3. 모니터링하려는 **모든** 페이지의 요청 처리기에 이러한 호출을 배치하고 "NONCE"를 CSP nonce로 바꿉니다.
