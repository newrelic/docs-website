...
인수 없이 `initialize` 호출하는 경우 이미 [`NEW_RELIC_LICENSE_KEY`](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 환경 변수로 [라이센스 키를](/docs/agents/python-agent/installation-configuration/python-agent-configuration#license_key) 지정했어야 합니다. 또는 `NEW_RELIC_CONFIG_FILE` 및 `NEW_RELIC_ENVIRONMENT` 을 설정할 수 있습니다. 그런 다음 에이전트는 누락된 인수 대신 해당 값을 읽습니다.
