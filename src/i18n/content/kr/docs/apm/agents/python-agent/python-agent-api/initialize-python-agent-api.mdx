...
인수 없이 `initialize` 을 호출하는 경우 [`NEW_RELIC_LICENSE_KEY`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#license_key) 환경 변수를 사용하여 [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key#find-heroku) 를 이미 지정해야 합니다. 또는 `NEW_RELIC_CONFIG_FILE` 및 `NEW_RELIC_ENVIRONMENT` 을 설정할 수 있습니다. 그러면 에이전트는 누락된 인수 대신 해당 값을 읽습니다.
