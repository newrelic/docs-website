...
<DNT>**before**</DNT> 에 액세스하면 에이전트가 초기화되고 전역 설정에는 사용자 환경 변수의 재정의와 함께 기본 구성 설정이 적용됩니다. <DNT>**after**</DNT> 에이전트 초기화에 액세스한 경우 전역 설정에는 전역 설정이기도 한 모든 에이전트 구성 파일 설정이 포함됩니다(구성 파일에 전역 설정 개체보다 적은 설정이 포함되어 있기 때문).
