...
* <DNT>
  ...
  래퍼 호출은 `WSGIApplicationWrapper` 입니다. 스택될 수 있는 개별 WSGI 애플리케이션 구성 요소에 대해 둘 이상의 위치에서 래퍼를 사용할 수 있습니다. 이 경우 마주친 첫 번째 래퍼는 프로세서의 시작을 표시하고, 에이전트는 해당 첫 번째 래퍼를 기반으로 하여 강제, 목표 앱을 결정합니다(다음 래퍼는 무시합니다).

* <DNT>
    **The path-based wrapper:**
  </DNT>

  경로 기반 래퍼 호출은 `wrap_wsgi_application` 입니다. 계측 범위에서 WSGI 개체를 변수로 참조할 수 없는 경우 이를 사용하세요. 이는 추가 `module` 및 `object_path` 매개변수를 포함하는 데코레이터와 동일한 매개변수를 사용합니다.
