...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Monitor > Transactions > (트랜잭션 선택):** 트랜잭션의 **분석 테이블** 은 큐 작업을 RabbitMQ에 대한 `MessageBroker` 측정항목으로 분류하고 `Put` (게시 메시지) 또는 `Take` (메시지 수신).
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > (트랜잭션 추적 선택):** 이 예에서 선택한 트랜잭션 추적의 **요약** 은 색상이 지정된 막대 차트에서 RabbitMQ 작업을 보여줍니다. 가장 **느린 구성요소** 섹션에는 RabbitMQ `Put` 및 `Take` 결과도 나열됩니다.
</figcaption>