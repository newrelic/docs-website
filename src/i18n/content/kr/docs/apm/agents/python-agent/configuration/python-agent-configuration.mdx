...
<CollapserGroup>
  ...
  >
    ...
    이 설정을 활성화하면 APM 환경 탭에 표시되는 에이전트 시작 시 패키지 및 버전 정보가 캡처됩니다. <Callout variant="tip">패키지 수가 많은 애플리케이션에서 이 설정을 활성화하면 모든 패키지 및 버전 정보를 캡처하므로 CPU 스파이크가 발생할 수 있습니다. 이러한 경우에는 이 설정을 비활성화하는 것이 좋습니다. </Callout><Callout variant="caution">이 설정을 비활성화하면 오래된 패키지의 취약점을 탐지하는 기능이 비활성화됩니다.</Callout>
  </Collapser>
</CollapserGroup>
