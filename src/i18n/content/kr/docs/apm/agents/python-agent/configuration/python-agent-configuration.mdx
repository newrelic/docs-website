...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="OpenTelemetry.활성화됨">
    ...
    활성화된 경우, 설치된 모든 OpenTelemetry API와 대부분의 [OpenTelemetry 계측된 프레임워크](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) 가 사용됩니다.
  ...
  <Collapser id="opentelemetry.traces.enabled" title="OpenTelemetry.트레이스.enabled">
    ...
    활성화된 경우, 기존 뉴렐릭 계측 라이브러리가 존재하는지 여부와 관계없이 모든 OpenTelemetry 스팬 및 트레이스 API가 사용됩니다. 향후 릴리스에서 포함 및 제외 설정이 추가되면 이 동작이 변경됩니다.
  </Collapser>
</CollapserGroup>
