...
유일한 필수 Python 에이전트 구성 설정은 <InlinePopover type="licenseKey" />입니다. 볼륨 키는 에이전트가 애플리케이션 데이터를 보고하는 계정을 식별합니다. 애플리케이션을 호스팅하는 방법에 따라 라이센스 키는 설정 파일이나 환경 변수를 통해 제공될 수 있습니다.
...
<figcaption>
  Python 에이전트를 사용하면 요청별 옵션이 서버 측 구성을 재정의합니다. 활성화된 경우 서버 측 구성은 서버 측 값이 비어 있는 경우에도 에이전트 구성 파일의 <DNT>**all**</DNT> 해당 값을 재정의합니다. 에이전트 구성 파일은 환경 변수를 재정의합니다. 환경 변수는 에이전트 기본값을 재정의합니다.
</figcaption>

각 구성 방법에 대한 자세한 설명은 다음과 같습니다.

<CollapserGroup>
  <Collapser className="freq-link" id="agent-configuration-file" title="에이전트 구성 파일">
    ...
    * `NEW_RELIC_CONFIG_FILE` 환경 변수를 설정합니다. `newrelic-admin` 래퍼 스크립트를 사용하는 경우 래퍼 스크립트가 에이전트를 자동으로 호출하므로 환경 변수를 사용해야 합니다.
      ...
      <Collapser className="freq-link" id="agent-configuration-file-ini" title="`.ini` 설정 파일 형식">
        설정 파일은 Microsoft Windows `.ini` 파일과 유사한 구조를 사용합니다. 자세한 내용은 Python ConfigParser 모듈의 [파일 형식 설명서를](https://docs.python.org/3/library/configparser.html) 참조하세요.
      ...
      <Collapser className="freq-link" id="agent-configuration-file-toml" title="`.toml` 설정 파일 형식">
        <Callout variant="tip">
          Python 3.11부터 설정을 위한 새로운 `.toml` 구문이 [표준 라이브러리](https://docs.python.org/3/library/tomllib.html) 에 추가되었습니다. 에이전트 버전 `10.3.0` 이 새로운 형식을 사용할 수 있는 지원이 추가되었습니다.
        </Callout>

        설정 파일은 Tom의 Obvious Minimal Language `.toml` 파일을 사용합니다. 자세한 내용은 [공식 파일 형식 문서를](https://toml.io/en/) 참조하세요.
  ...
  <Collapser className="freq-link" id="server-side-configuration" title="서버 측 구성">
    [서버 측 설정을 사용](/docs/agents/manage-apm-agents/configuration/server-side-agent-configuration) 하면 뉴렐릭 UI의 특정 설정을 구성할 수 있습니다. 이렇게 하면 여러 호스트에서 실행되는 경우에도 모든 에이전트에 변경 사항이 자동으로 적용됩니다. 사용 가능한 경우 이 문서에는 개별 구성 옵션 아래에 서버 측 구성에 대한 UI 레이블이 <DNT>**Server-side label**</DNT> 으로 포함되어 있습니다.

    <Callout variant="important">
      서버 측 구성이 활성화된 경우 에이전트는 UI에 설정된 <DNT>**could**</DNT> 구성 파일의 모든 값을 무시합니다. UI 값이 비어 있어도 에이전트는 이를 빈 문자열로 처리하고 에이전트 구성 파일을 사용하지 않습니다.
    </Callout>
  </Collapser>

  <Collapser className="freq-link" id="environment-variables" title="환경 변수">
    환경 변수를 사용하면 특정 핵심 설정에 대한 기본값을 재정의할 수 있습니다. 동등한 설정이 에이전트 구성 파일에 명시적으로 나열되어 있는 경우 구성 파일 설정이 환경 변수보다 우선합니다. 사용 가능한 경우 환경 변수는 아래 개별 구성 옵션 아래에 <DNT>**Environ variable**</DNT> 으로 설명되어 있습니다.
  ...
  <Collapser className="freq-link" id="per-request-configuration" title="요청별 구성">
    ...
    <CollapserGroup>
      <Collapser id="example-apache-mod-wsgi" title="예: Apache/mod_wsgi 앱 이름">
        Apache/mod\_wsgi 서버에서 `SetEnv` 지시문을 사용하여 구성 설정을 재정의할 수 있습니다(선택적으로 `Location` 또는 `Directory` 블록 내부). 예를 들어 완전한 가상 호스트 또는 해당 가상 호스트에 대해 WSGI 애플리케이션에서 처리하는 URL의 하위 집합에 대해 [앱 이름](#app_name) 을 재정의할 수 있습니다.
      </Collapser>
    </CollapserGroup>
...
<Collapser className="freq-link" id="config-file-format-example-ini" title="`.ini` 설정 파일 형식">
  ```ini
  [newrelic]
  ... default settings
...
<Collapser className="freq-link" id="config-file-format-example-toml" title="`.toml` 설정 파일 형식">
  ```ini
  [tool.newrelic]
  ... default settings
...
<CollapserGroup>
  <Collapser id="license_key" title="license_key(필수)">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    뉴렐릭 계정의 <InlinePopover type="licenseKey" />을(를) 지정합니다. 이 키는 앱의 지표를 뉴렐릭 계정과 연결합니다.
  </Collapser>

  <Collapser id="app_name" title="app_name(강력 권장)">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="log_file" title="로그 파일">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="high_security" title="높은 보안">
    ...
    높은 보안 모드는 특정 보안 설정을 시행하고 해당 설정이 무시되는 것을 방지하므로 민감한 데이터가 당사로 전송되지 않습니다. 높은 수준의 보안 모드를 활성화하면 요청 매개변수가 수집되지 않으며 원시 SQL을 보낼 수 없습니다.

    높은 보안 모드를 활성화하려면 로컬 <DNT>**.ini**</DNT> 구성 파일에서 `true` 로 설정하고 <DNT>**and**</DNT> <DNT>**Account settings**</DNT> 페이지에서 활성화하세요. 자세한 내용은 [높은 보안을](/docs/accounts-partnerships/accounts/security/high-security) 참조하세요.
  </Collapser>
  ...
  <Collapser id="audit-log-file" title="감사_로그_파일">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="labels" title="레이블(태그)">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    [태그를](/docs/apm/new-relic-apm/maintenance/labels-categories-organize-your-apps-servers)추가합니다. *이름:값을* 콜론 `:`으로 구분하여 지정하고 추가 태그는 세미콜론 `;`으로 구분합니다.

    <CollapserGroup>
      <Collapser id="example-2-labels" title="두 개의 태그">
        ```ini
  ...
  <Collapser id="api-key" title="api_key">
    ...
    [newrelic-admin record-deploy](/docs/agents/python-agent/installation-configuration/python-agent-admin-script#record-deploy) 와 함께 사용할 [api\_key](/docs/apis/rest-api-v2/requirements/api-keys) 를 설정합니다.
  </Collapser>
  ...
  <Collapser id="apdex-t" title="apdex_t">
    ...
    [apdex\_t](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#apdex_t) 에 대한 용어집 항목을 참조하십시오.

    <Callout variant="important">
      이는 AWS Lambda에 대해 활성화된 `serverless_mode` 이 설정된 경우 구성 파일 또는 환경 변수에서만 설정됩니다. 그렇지 않으면 `apdex_f` 값을 설정하는 데 사용되는 UI 애플리케이션 설정의 값으로 로컬 `apdex_t` 값이 재정의됩니다.
    </Callout>
  </Collapser>
</CollapserGroup>
...
속성은 프로세서 트레이스, 트레이스 오류, <InlinePopover type="browser" />및 트랜잭션 이벤트에 대한 정보를 제공하는 핵심 가치 쌍입니다. 아래의 일반 속성 설정을 사용하여 4개 대상 모두에 대한 속성을 구성하는 것 외에도 대상별로 구성할 수도 있습니다.
...
<CollapserGroup>
  <Collapser id="cfg-attributes-enabled" title="속성.활성화">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="cfg-attributes-include" title="속성.포함">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="cfg-attributes-exclude" title="속성.제외">
    <table>
      <tbody>
        <tr>
          <th>
...
<Callout variant="important">
  AI 데이터를 캡처하려면 [분산 추적을](/docs/apm/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration/#dt-main) 활성화해야 합니다. Python 에이전트 버전 7.0.0.166 이상에서는 기본적으로 켜져 있습니다.
</Callout>
...
<CollapserGroup>
  ...
  <Collapser id="txn-tracer-threshold" title="transaction_tracer.transaction_threshold">
    ...
    트랜잭션 추적을 수집할 임계값(초)입니다. 컨트롤러 작업의 응답 시간이 이 임계값을 초과하면 에이전트는 트랜잭션 추적을 기록합니다. 유효한 값은 양수 float 또는 `apdex_f` ( [apdex\_t의](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#apdex_t) 4배)입니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser-content-type" title="browser_monitoring.content_type">
    ...
    <CollapserGroup>
      <Collapser id="example-instrument-xhtml-xml" title="도구 xhtml+xml 페이지 응답">
...
<CollapserGroup>
  <Collapser id="custom-insights-events-enabled" title="custom_insights_events.enabled">
    ...
    [`record_custom_event()`](/docs/agents/python-agent/python-agent-api/recordcustomevent-python-agent-api/) 통해 Event API에 이벤트 기록을 허용합니다.
...
<Callout variant="important">
  분산 추적을 활성화하면 [애플리케이션 간 추적이](#cross-application-tracer) 비활성화되고 APM 기능에 다른 영향을 미칩니다. 교차 애플리케이션 추적에서 마이그레이션하는 경우 [전환 가이드를](/docs/transition-guide-distributed-tracing) 읽어보세요.
</Callout>
...
<CollapserGroup>
  <Collapser id="distributed-tracing-enabled" title="distribution_tracing.enabled">
    <table>
      <tbody>
        <tr>
          <th>
...
* [실시간 스트리밍은](/docs/agents/manage-apm-agents/agent-data/real-time-streaming) 5초마다(분당 12회) 데이터를 전송하지만 이벤트 수집 설정은 여전히 <DNT>**minute**</DNT> 당 이벤트 속도에 영향을 미칩니다. 실시간 스트리밍을 활성화하거나 비활성화하기 위해 이러한 설정을 변경할 필요는 없습니다.
...
<CollapserGroup>
  ...
  <Collapser id="harvest-limits-custom-event-data" title="event_harvest_config.harvest_limits.custom_event_data">
    ...
    * Python 에이전트의 변형이 뉴렐릭으로 보낼 수 있는 분당 커스텀 대시보드 수를 제한합니다.
    * [AI 모니터링을](/docs/ai-monitoring/intro-to-ai-monitoring) 위해 에이전트를 구성할 때 에이전트가 LLM 이벤트의 최대량을 캡처하도록 최대값 `100000` 으로 설정합니다.
  </Collapser>
  ...
  <Collapser id="harvest-limits-span-event-data" title="event_harvest_config.harvest_limits.span_event_data">
    ...
    * [AI 감시](/docs/ai-monitoring/intro-to-ai-monitoring) 용 에이전트를 구성할 때 에이전트가 에이전트 트레이스의 최대량을 캡처하도록 최대값 `10000` 으로 설정합니다.
  </Collapser>
...
## 물리적 메모리에 대한 런타임 지표 [#runtime-metrics]

<Callout variant="important">
  [Python 에이전트 버전 9.9.0 이상이](/docs/agents/python-agent/installation-configuration/upgrade-python-agent) 필요합니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="runtime-metrics-reporting-enabled" title="memory_runtime_pid_metrics.enabled">
    ...
    활성화되면 에이전트는 프로세스 ID별로 런타임 지표를 생성하고 전송합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="NEW_RELIC_METADATA_COMMIT" title="NEW_RELIC_METADATA_COMMIT">
    ...
    커밋 샤. 전체 sha를 사용하거나 처음 7자만 사용할 수 있습니다(예: 734713b).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="application_logging.forwarding.custom_attributes" title="application_logging.forwarding.custom_attributes">
    ...
    퀴렐릭에게 전달되는 모든 로그 이벤트에 사용자 정의 속성으로 추가하기 위한 키/값 쌍이 있는 컬렉션입니다. 값은 &quot;key1:value1;key2:value2&quot;와 같은 형식이어야 합니다.
  </Collapser>
  ...
  <Collapser id="application_logging.forwarding.labels.enabled" title="application_logging.forwarding.labels.enabled">
    ...
    에이전트가 뉴렐릭으로 보내기 위해 로그 레코드에 라벨을 추가할지 여부를 전환합니다.
  </Collapser>
  ...
  <Collapser id="application_logging.forwarding.labels.exclude" title="application_logging.forwarding.labels.exclude">
    ...
    로그에 라벨을 포함하도록 활성화할 때 제외할 라벨 이름의 대소문자를 구분하지 않는 목록입니다. 이 속성은 와일드카드나 정규식을 지원하지 않습니다. <Callout variant="important">레이블을 속성으로 추가할 때 에이전트는 키에 `tags.` 접두사를 붙입니다. 이 접두사는 제외 필터링 규칙과 일치할 때 포함되지 <b>않습니다</b> .</Callout>
  </Collapser>
  ...
  <Collapser id="application_logging.forwarding.context_data.include" title="application_logging.forwarding.context_data.include">
    ...
    `context_data` 에 대해 속성이 활성화된 경우 이 목록에 있는 모든 속성 키가 트랜잭션 추적을 통해 당사로 전송됩니다. 자세한 내용은 [상담원 속성 규칙을](/docs/apm/other-features/attributes/agent-attributes) 참조하세요.

    <Callout variant="important">
      컨텍스트 속성을 추가할 때 에이전트는 로깅 프레임워크 컨텍스트의 속성에 대해서는 `context.` , 사전 메시지 컨텍스트의 속성에 대해서는 `message.` 키 접두사를 붙입니다.

      포함/제외 필터링 규칙과 일치할 때 이러한 접두사는 포함되지 <b>않습니다</b> .
    </Callout>
  </Collapser>
  ...
  <Collapser id="application_logging.forwarding.context_data.exclude" title="application_logging.forwarding.context_data.exclude">
    ...
    이 목록에 있는 모든 속성 키는 context\_data로 전송되지 않습니다. 자세한 내용은 [상담원 속성 규칙을](/docs/apm/other-features/attributes/agent-attributes) 참조하세요.
...
## 계측 설정 [#instrumentation-settings]

<Callout variant="important">
  [Python 에이전트 버전 8.7.1 이상이](/docs/agents/python-agent/installation-configuration/upgrade-python-agent)필요합니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="instrumentation-graphql-capture-introspection-queries" title="instrumentation.graphql.capture_introspection_queries">
    ...
    활성화되면 에이전트는 GraphQL에서 자체 검사 쿼리에 대한 트랜잭션을 캡처합니다.
  </Collapser>
</CollapserGroup>
...
<Callout variant="important">
  [Python 에이전트 버전 9.1.0 이상이](/docs/agents/python-agent/installation-configuration/upgrade-python-agent) 필요합니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="machine-learning-enabled" title="machine_learning.enabled">
    ...
    기계 학습 측정항목에 대한 에이전트 속성 수집을 활성화하려면 `true` 로 설정합니다.
  </Collapser>
  ...
  <Collapser id="inference-events-value-enabled" title="machine_learning.inference_events_value.enabled">
    ...
    원시 추론 값 캡처를 활성화하려면 `true` 로 설정합니다.
  ...
  <Collapser id="ml-insights-events-enabled" title="ml_insights_events.enabled">
    ...
    [`record_ml_event()`](/docs/agents/python-agent/python-agent-api/recordmlevent-python-agent-api/) 을 통해 Event API에 기계 학습 이벤트 기록을 허용합니다.
...
<CollapserGroup>
  ...
  <Collapser id="slow-sql" title="slow_sql.활성화">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="strip_exception_messages_enabled" title="strip_exception_messages.enabled">
    ...
    활성화된 경우 민감한 정보가 실수로 캡처되는 것을 방지하기 위해 예외 메시지가 [수집기](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector) 로 전송되기 전에 오류 추적에서 제거됩니다. 이 옵션은 높은 보안 모드에서 자동으로 활성화됩니다.
  ...
  <Collapser id="startup-timeout" title="시작_시간 초과">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    기본적으로 에이전트는 첫 번째 트랜잭션(웹 또는 [비웹](/docs/apm/transactions/intro-transactions/monitor-background-processes-other-non-web-transactions))을 수신할 때 시작됩니다. 그런 다음 에이전트는 병렬로 시작하여 이 초기 요청이 지연되지 않도록 합니다. 그러나 에이전트는 등록이 완료될 때까지 데이터를 수집할 수 없기 때문에 이 초기 요청의 세부 정보를 기록하지 않습니다. 이것은 New Relic이 시작되는 동안 첫 번째 커플 트랜잭션을 지연시키지 않기 위해 대부분의 웹 애플리케이션에 권장되는 구성입니다.
  ...
  <Collapser id="shutdown-timeout" title="종료_시간 초과">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="compressed-content-encoding" title="압축된 콘텐츠 인코딩">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="package-reporting-enabled" title="package_reporting.enabled">
    ...
    이 설정을 활성화하면 APM 환경 탭에 표시되는 에이전트 시작 시 패키지 및 버전 정보가 캡처됩니다.

    <Callout variant="tip">
      패키지 수가 많은 애플리케이션에서 이 설정을 활성화하면 모든 패키지 및 버전 정보를 캡처하므로 CPU 스파이크가 발생할 수 있습니다. 이러한 경우에는 이 설정을 비활성화하는 것이 좋습니다.
    </Callout>

    <Callout variant="caution">
      이 설정을 비활성화하면 오래된 패키지의 취약점을 탐지하는 기능이 비활성화됩니다.
    </Callout>
  </Collapser>
  ...
  <Collapser id="NEW_RELIC_STARTUP_DEBUG" title="NEW_RELIC_STARTUP_DEBUG">
    ...
    <Callout variant="caution">
      이 환경 변수 설정에는 해당 구성 파일 설정이 없습니다. 관련 코드는 구성 파일을 읽기 전에 실행되기 때문입니다. 에이전트 시작 후 포괄적인 디버그 로깅을 위해서는 [로그 레벨을](#log_level) `debug` 으로 설정하세요.
    </Callout>
  </Collapser>

  <Collapser id="NEW_RELIC_K8S_OPERATOR_ENABLED" title="NEW_RELIC_K8S_OPERATOR_활성화됨">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    쿠버네티스 클러스터에 에이전트가 주입되었을 때 보고하는 데 사용되는 정보 설정입니다.

    <Callout variant="caution">
      이 설정은 에이전트의 이 기능을 활성화하거나 비활성화하지 **않습니다** .
    </Callout>
  </Collapser>

  <Collapser id="NEW_RELIC_AZURE_OPERATOR_ENABLED" title="NEW_RELIC_AZURE_OPERATOR_활성화됨">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    이는 에이전트가 Microsoft Azure Container 앱에 삽입될 때 보고하는 데 사용되는 정보 설정입니다.
...
<CollapserGroup>
  ...
  <Collapser id="heroku-dyno_name_prefixes_to_shorten" title="heroku.dyno_name_prefixes_to_shorten">
    ...
    일반적으로 에이전트는 뒤에 점과 프로세스 ID가 있는 dyno 이름을 보고합니다(예: <DNT>**worker.3**</DNT> ). 후행 데이터 없이 보고할 접두사를 지정하여 이 후행 데이터를 제거할 수 있습니다(예: <DNT>**worker**</DNT> ).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="example-disable-mysqldb" title="예: MySQLdb 데이터베이스 쿼리 계측 비활성화">
