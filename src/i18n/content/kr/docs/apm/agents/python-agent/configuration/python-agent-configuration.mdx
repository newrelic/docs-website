...
<figcaption>
  Python 에이전트를 사용하면 요청별 옵션이 서버 측 구성을 재정의합니다. 활성화된 경우 서버 측 구성은 서버 측 값이 비어 있는 경우에도 에이전트 구성 파일의 <DoNotTranslate>**all**</DoNotTranslate> 해당 값을 재정의합니다. 에이전트 구성 파일은 환경 변수를 재정의합니다. 환경 변수는 에이전트 기본값을 재정의합니다.
</figcaption>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="서버 측 구성"
  >
    [서버 측 설정을 사용](/docs/agents/manage-apm-agents/configuration/server-side-agent-configuration) 하면 뉴렐릭 UI의 특정 설정을 구성할 수 있습니다. 이렇게 하면 여러 호스트에서 실행되는 경우에도 모든 에이전트에 변경 사항이 자동으로 적용됩니다. 사용 가능한 경우 이 문서에는 개별 구성 옵션 아래에 서버 측 구성에 대한 UI 레이블이 <DoNotTranslate>**Server-side label**</DoNotTranslate> 으로 포함되어 있습니다.

    <Callout variant="important">
      서버 측 구성이 활성화된 경우 에이전트는 UI에 설정된 <DoNotTranslate>**could**</DoNotTranslate> 구성 파일의 모든 값을 무시합니다. UI 값이 비어 있어도 에이전트는 이를 빈 문자열로 처리하고 에이전트 구성 파일을 사용하지 않습니다.
    </Callout>
  </Collapser>
  ...
  <Collapser
    ...
    title="환경 변수"
  >
    환경 변수를 사용하면 특정 핵심 설정에 대한 기본값을 재정의할 수 있습니다. 동등한 설정이 에이전트 구성 파일에 명시적으로 나열되어 있는 경우 구성 파일 설정이 환경 변수보다 우선합니다. 사용 가능한 경우 환경 변수는 아래 개별 구성 옵션 아래에 <DoNotTranslate>**Environ variable**</DoNotTranslate> 으로 설명되어 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    높은 보안 모드를 활성화하려면 로컬 <DoNotTranslate>**.ini**</DoNotTranslate> 구성 파일에서 `true` 로 설정하고 <DoNotTranslate>**and**</DoNotTranslate> <DoNotTranslate>**Account settings**</DoNotTranslate> 페이지에서 활성화하세요. 자세한 내용은 [높은 보안을](/docs/accounts-partnerships/accounts/security/high-security) 참조하세요.
  </Collapser>
  ...
  >
    ...
    [태그를](/docs/apm/new-relic-apm/maintenance/labels-categories-organize-your-apps-servers)추가합니다. _이름:값을_ 콜론 `:`으로 구분하여 지정하고 추가 태그는 세미콜론 `;`으로 구분합니다.
...
* [실시간 스트리밍은](/docs/agents/manage-apm-agents/agent-data/real-time-streaming) 5초마다(분당 12회) 데이터를 전송하지만 이벤트 수집 설정은 여전히

  <DoNotTranslate>**minute**</DoNotTranslate>

  당 이벤트 속도에 영향을 미칩니다. 실시간 스트리밍을 활성화하거나 비활성화하기 위해 이러한 설정을 변경할 필요는 없습니다.
...
<CollapserGroup>
  ...
  >
    ...
    이 목록에 있는 모든 속성 키는 context_data로 전송되지 않습니다. 자세한 내용은 [상담원 속성 규칙을](/docs/apm/other-features/attributes/agent-attributes) 참조하세요.
...
<CollapserGroup>
  ...
  >
    ...
    쿠버네티스 클러스터에 에이전트가 주입되었을 때 보고하는 데 사용되는 정보 설정입니다. <Callout variant="caution">이 설정은 에이전트의 이 기능을 활성화/비활성화하지 **않습니다** .</Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    일반적으로 에이전트는 뒤에 점과 프로세스 ID가 있는 dyno 이름을 보고합니다(예: <DoNotTranslate>**worker.3**</DoNotTranslate> ). 후행 데이터 없이 보고할 접두사를 지정하여 이 후행 데이터를 제거할 수 있습니다(예: <DoNotTranslate>**worker**</DoNotTranslate> ).
  </Collapser>
</CollapserGroup>
