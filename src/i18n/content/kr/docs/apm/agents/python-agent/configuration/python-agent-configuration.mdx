...
<CollapserGroup>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    * `adaptive` 기본 동작이며, 트레이스를 샘플링할지 여부를 결정하기 위해 샘플링 결정을 적응 샘플링 알고리즘에 위임합니다.
    * `trace_id_ratio_based` 샘플링 결정을 트레이스 ID 비율 기반 샘플링 알고리즘에 위임하여 트레이스를 샘플링할지 여부를 결정합니다. 이 옵션을 사용하는 경우 비율이 필수이므로 [distributed\_tracing.sampler.remote\_parent\_sampled.trace\_id\_ratio\_based.ratio를](#distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio) 대신 사용하십시오.
    * `always_on`: 샘플링된 원격 상위 항목이 있는 트레이스를 항상 샘플링합니다.
    * `always_off`원격 부모가 샘플링된 트레이스는 절대 샘플링하지 않습니다.
  </Collapser>

  <Collapser id="distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio" title="분산 추적.샘플러.원격 부모 샘플링.추적 ID 비율 기반.비율">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    * `adaptive` 기본 동작이며, 트레이스를 샘플링할지 여부를 결정하기 위해 샘플링 결정을 적응 샘플링 알고리즘에 위임합니다.
    * `trace_id_ratio_based` 샘플링 결정을 트레이스 ID 비율 기반 샘플링 알고리즘에 위임하여 트레이스를 샘플링할지 여부를 결정합니다. 이 옵션을 사용하는 경우 비율이 필수이므로 [distributed\_tracing.sampler.remote\_parent\_not\_sampled.trace\_id\_ratio\_based.ratio를](#distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio) 대신 사용하십시오.
    * `always_on`: 샘플링되지 않은 원격 부모가 있는 트레이스를 항상 샘플링합니다.
    * `always_off`: 샘플링되지 않은 원격 부모가 있는 트레이스를 샘플링하지 않습니다.
  </Collapser>

  <Collapser id="distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio" title="분산 추적.샘플러.원격 부모-샘플링 안 됨.트레이스 ID 비율 기반.비율">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="distributed-tracing-sampler-root" title="분산 추적.샘플러.루트">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    * `adaptive` 기본 동작이며, 트레이스를 샘플링할지 여부를 결정하기 위해 샘플링 결정을 적응 샘플링 알고리즘에 위임합니다.
    * `trace_id_ratio_based` 샘플링 결정을 트레이스 ID 비율 기반 샘플링 알고리즘에 위임하여 트레이스를 샘플링할지 여부를 결정합니다. 이 옵션을 사용하는 경우 비율이 필수이므로 [distributed\_tracing.sampler.root.trace\_id\_ratio\_based.ratio를](#distributed-tracing-sampler-root-trace-id-ratio-based-ratio) 대신 사용하십시오.
    * `always_on`현재 서비스에서 발생하고 진입 호출에 분산 추적 헤더가 없는 트랜잭션만 샘플링합니다.
    * `always_off`현재 서비스에서 발생하고 진입 호출에 분산 추적 헤더가 없는 트랜잭션은 절대 샘플링하지 않습니다.
  </Collapser>

  <Collapser id="distributed-tracing-sampler-root-trace-id-ratio-based-ratio" title="분산 추적.샘플러.루트.트레이스 ID 비율 기반.비율">
    <table>
      <tbody>
        <tr>
          <th>
...
<CollapserGroup>
  <Collapser id="harvest-limits-analytic-event-data" title="event_harvest_config.harvest_limits.analytic_event_data">
    ...
    <Callout variant="warning">
      이 설정은 버전 11.0.0부터 더 이상 사용되지 않습니다. 대신 [`transaction_events.max_samples_stored`](#transaction-events-max-samples-stored) 사용하세요.
    </Callout>
  ...
  <Collapser id="harvest-limits-custom-event-data" title="event_harvest_config.harvest_limits.custom_event_data">
    ...
    <Callout variant="warning">
      이 설정은 버전 11.0.0부터 더 이상 사용되지 않습니다. 대신 [`custom_insights_events.max_samples_stored`](#custom-insights-events-max-samples-stored) 사용하세요.
    </Callout>
  ...
  <Collapser id="harvest-limits-span-event-data" title="event_harvest_config.harvest_limits.span_event_data">
    ...
    <Callout variant="warning">
      이 설정은 버전 11.0.0부터 더 이상 사용되지 않습니다. 대신 [`span_events.max_samples_stored`](#cfg-tt-max-samples-stored) 사용하세요.
    </Callout>
  ...
  <Collapser id="harvest-limits-error-event-data" title="event_harvest_config.harvest_limits.error_event_data">
    ...
    <Callout variant="warning">
      이 설정은 버전 11.0.0부터 더 이상 사용되지 않습니다. 대신 [`error_collector.max_event_samples_stored`](#error-max-event-samples-stored) 사용하세요.
    </Callout>
  ...
  <Collapser id="event-harvest-limits-ml-event-data" title="event_harvest_config.harvest_limits.ml_event_data">
    ...
    끌어 당기는 에이전트가 뉴렐릭으로 보낼 수 있는 분당 ML 이벤트 수에 대한 제한입니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="event-harvest-config-harvest-limits-log-event-data" title="event_harvest_config.harvest_limits.log_event_data">
    ...
    <Callout variant="warning">
      이 설정은 버전 11.0.0부터 더 이상 사용되지 않습니다. 대신 [`application_logging.forwarding.max_samples_stored`](#application-logging-forwarding-max-samples-stored) 사용하세요.
    </Callout>
...
<CollapserGroup>
  ...
  <Collapser id="instrumentation-kombu-consumer-enabled" title="instrumentation.kombu.consumer.enabled">
    ...
    활성화되면 에이전트는 Kombu에서 소비자 MessageTransactions를 수행합니다.
  </Collapser>
  ...
  <Collapser id="instrumentation-kombu-ignored-exchanges" title="instrumentation.kombu.ignored_exchanges">
    ...
    Kombu에서 MessageTransactions를 캡처할 때 무시할 교환의 공백으로 구분된 목록입니다.
  </Collapser>
</CollapserGroup>

## 미들웨어 측정, 로그 필터 [#middleware-filters]

<Callout variant="important">
  [끌어당김 버전 10.17.0 이상이](/docs/agents/python-agent/installation-configuration/upgrade-python-agent) 필요합니다.
</Callout>
...
<CollapserGroup>
  <Collapser id="instrumentation-middleware-django-enabled" title="instrumentation.middleware.django.enabled">
    ...
    이 기능을 활성화하면 에이전트가 Django의 미들웨어를 모니터링합니다.
  </Collapser>
  ...
  <Collapser id="instrumentation-middleware-django-include" title="instrumentation.middleware.django.include">
    ...
    미들웨어 측정 및 과학적 필터링에 대한 규칙은 [미들웨어 필터링 페이지에서](/docs/apm/agents/python-agent/supported-features/django-middleware-filtering/#django-middleware-rules) 확인할 수 있습니다.
  </Collapser>
...
## 하이브리드 에이전트 설정 [#otel-settings]

<Callout variant="important">
  [끌어당김 버전 11.5.0 이상이](/docs/agents/python-agent/installation-configuration/upgrade-python-agent) 필요합니다.
</Callout>

하이브리드 에이전트 기능에 대한 자세한 내용은 [이 페이지](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support)를 참조하십시오.

하이브리드 에이전트 설정에 사용할 수 있는 설정은 다음과 같습니다:
...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="OpenTelemetry.활성화됨">
    ...
    v11.5.0부터 v13.0.0까지: 활성화된 경우, 설치된 모든 OpenTelemetry API 및 대부분의 [OpenTelemetry 계측된 프레임워크](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) 가 사용됩니다. v13.0.0+부터: 활성화된 경우, 뉴렐릭에서는 지원하지 않지만 [OpenTelemetry에서 지원하는](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) 모든 OpenTelemetry API 및 프레임워크가 사용됩니다.

    <Callout variant="important">
      [OpenTelemetry의 계측된 프레임워크](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) 에 있는 대부분의 라이브러리는 지원되지만, AWS 및 gRPC 계측 라이브러리는 계속해서 뉴렐릭의 계측을 사용합니다. 네이티브 계측이 포함된 ML/AI 라이브러리는 현재 명시적으로 비활성화되어 있으며 향후 릴리스에서 지원될 예정입니다.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.enabled" title="OpenTelemetry.트레이스.enabled">
    ...
    v11.5.0부터 v13.0.0까지: 활성화된 경우, 기존 뉴렐릭 계측 라이브러리가 존재하는지 여부와 관계없이 모든 OpenTelemetry 스팬 및 트레이스 API가 사용됩니다. v13.0.0+부터: 활성화된 경우, OpenTelemetry 스팬 API를 사용하는 모든 수동 호출과 뉴렐릭에서는 지원하지 않지만 [OpenTelemetry에서 지원하는](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) 프레임워크의 스팬이 사용됩니다.
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.exclude" title="OpenTelemetry.트레이스.exclude">
    ...
    사용자가 뉴렐릭에 보고되는 트레이서를 제외할 수 있도록 합니다. 자세한 내용은 [아래 섹션](#include-exclude-rules) 을 참조하십시오.

    <Callout variant="important">
      [파이썬 에이전트 버전 13.0.0 이상](/docs/agents/python-agent/installation-configuration/upgrade-python-agent)이 필요합니다.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.include" title="OpenTelemetry.트레이스.include">
    ...
    사용자가 뉴렐릭에 보고할 트레이서를 포함할 수 있도록 합니다. 자세한 내용은 [아래 섹션](#include-exclude-rules) 을 참조하십시오.
...
포함 및 제외 규칙 \[#include-exclude-rules]

뉴렐릭은 모니터링에 포함하거나 제외할 트레이서를 결정할 때 다음 규칙을 따릅니다.

<CollapserGroup>
  <Collapser id="rule-precedence" title="활성화는 포함 및 제외보다 우선합니다.">
    `opentelemetry.traces.enabled` : 이 플래그(flag)는 include 및 exclude 설정보다 우선합니다.
  ...
  <Collapser id="rule-trace-included" title="트레이싱이 활성화된 경우 트레이스가 포함됩니다.">
    트레이싱이 활성화된 경우, 뉴렐릭에서 지원하지 않는 모든 커스텀 OpenTelemetry 트레이서 및 OpenTelemetry 라이브러리의 트레이서는 기본적으로 모니터링됩니다.
  ...
  <Collapser id="rule-exclude-wins" title="제외는 항상 포함을 대체합니다.">
    동일한 트레이서가 포함 및 제외 목록에 나열되어 있으면 해당 트레이서는 제외됩니다.
  ...
  <Collapser id="rule-tracers-case-sensitive" title="트레이서는 대소문자를 구분합니다.">
    트레이서는 대소문자를 구분합니다.
...
<CollapserGroup>
  ...
  <Collapser id="cross-application-tracer" title="cross_application_tracer.enabled">
    ...
    <Callout variant="caution">
      [크로스 애플리케이션 추적(CAT) 기능은](/docs/apm/transactions/cross-application-traces/introduction-cross-application-traces) 버전 12.0.0에서 완전히 제거되었습니다.
    </Callout>
  </Collapser>
