---
title: 'PHP 에이전트 설치: AWS Linux, RedHat, CentOS'
tags:
  - Agents
  - PHP agent
  - Installation
...
PHP 에이전트가 코드를 자동 계측하므로 애플리케이션 모니터링을 시작할 수 있습니다. AWS Linux 2, RedHat 또는 CentOS를 사용하여 APM용 New Relic의 PHP 에이전트를 설치하려면 이 절차를 따르십시오. **예외:** 이전 버전이 설치된 [경우 에이전트를 대신 업그레이드하십시오](/docs/agents/php-agent/installation/upgrading-php-agent) .
