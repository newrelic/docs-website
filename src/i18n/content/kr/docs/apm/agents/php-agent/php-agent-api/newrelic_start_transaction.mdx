...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        선택 과목. New Relic 에이전트의 `newrelic.ini` 파일에 설정된 <LicenseKey/> 가 기본값입니다.

        트랜잭션이 다른 New Relic 계정으로 보고되도록 하려면 다른 <LicenseKey/> 를 제공하십시오. 설정된 경우 이 라이센스는 INI 파일에 구성된 모든 디렉토리별 및 글로벌 기본 라이센스를 대체합니다.
      </td>
    </tr>
  </tbody>
</table>
...
이 예시에서는 앱 이름과 <LicenseKey/> 를 수동으로 지정합니다.
