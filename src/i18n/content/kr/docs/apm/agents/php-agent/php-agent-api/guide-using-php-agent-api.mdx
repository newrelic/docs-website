...
페이지에 자동으로 추가하거나 JavaScript 코드 조각을 복사하여 붙여넣어 특정 페이지에 배포하여 [브라우저 에이전트를 설치할](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent) 수 있습니다. 당신은 또한 제어할 수 있습니다 <InlinePopover type="browser"/>에이전트를 사용하여 <InlinePopover type="apm"/>에이전트 API 호출. 자세한 내용은 [브라우저 모니터링 및 PHP 에이전트를](/docs/agents/php-agent/features/new-relic-browser-php-agent) 참조하세요.