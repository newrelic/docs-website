...
PHP 에이전트는 Drupal 웹사이트에 대한 특수 메트릭을 캡처합니다. 이러한 메트릭이 수집되면 **Drupal** 탭이 New Relic 사용자 인터페이스에 나타납니다. Go to **\[one.newrelic.com** ]( [https://one.newrelic.com](https://one.newrelic.com) ) > APM > (Drupal 앱 선택)\*\*.
