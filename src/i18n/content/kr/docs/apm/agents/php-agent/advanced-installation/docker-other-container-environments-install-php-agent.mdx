...
3. `newrelic.ini` 파일에서 `newrelic.appname` 및 `newrelic.license` 항목을 사용하여 애플리케이션 이름과
...
<CollapserGroup>
  ...
  >
    ...
    * `PHP_AGENT_URL`: PHP 에이전트 버전의 다운로드 URL입니다. 최신 버전의 에이전트를 찾으려면
    ...
    * `YOUR_LICENSE_KEY`: 이것을
      ...
      로 바꾸세요. `"REPLACE_WITH_REAL_KEY"` 은 PHP 에이전트의 기본

      <DoNotTranslate>**newrelic.ini**</DoNotTranslate>

      파일에 있는 실제 문자열입니다. 해당 문자열을 편집하지 마세요. `sed` 명령은 기본 문자열을 따옴표로 묶은 실제 40자 키로 바꿉니다.
