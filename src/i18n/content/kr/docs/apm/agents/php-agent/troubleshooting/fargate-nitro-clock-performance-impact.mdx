...
PHP 에이전트는 타이밍 활동을 위해 시스템의 클록 소스에 크게 의존합니다. 지속적인 클록 확인 때문에 효과를 거두려면, 시간을 확인하기 위한 반복적인 호출이 전체 성능을 지배하지 않도록 PHP 에이전트에 vDSO 기반 클록 소스가 필요합니다. 클록 소스를 확인/변경하는 방법에 대한 자세한 내용은 [여기](https://support.newrelic.com/s/hubtopic/aAX8W0000008a7r/relic-solution-php-agent-cpu-overhead-tips)에서 확인할 수 있습니다.
