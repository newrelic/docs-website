    ...
    <tr>
      ...
      <td>
        **애플리케이션 이름과 New Relic 라이선스 키를 구성합니다.**
        ...
        설치 프로세스에서 애플리케이션 이름과 라이선스 키를 묻는 메시지가 표시됩니다. <LicenseKey/> 는 **계정 설정** 의 계정 정보 섹션에 나타납니다. 헤드리스 설치의 경우 [`debconf` 를 사용하여 라이센스 키와 애플리케이션 이름을 미리 시드](#preseeding) 할 수도 있습니다.
      </td>
    </tr>
...
헤드리스 설치의 경우 <LicenseKey/> 와 [애플리케이션 이름](/docs/site/naming-your-application) 을 미리 설정할 수 있습니다. 배포판에서 제공하는 기본 php5 패키지를 사용해야 합니다. 미리 설정하려면 [`debconf-set-selections`](http://manpages.ubuntu.com/manpages/trusty/en/man1/debconf-set-selections.1.html) 를 사용하여 `debconf` 데이터베이스에 값을 설정합니다.
  ...
  <tbody>
    ...
    <tr>
      <td>
        ...
        귀하의 <LicenseKey/> . 이 문자열은 `newrelic.ini` }에서 [`newrelic.license`](/docs/php/php-agent-phpini-settings#inivar-license) 를 설정합니다.
      </td>
