...
New Relic은 웹 트랜잭션 이름과 New Relic의 JavaScript 헤더 및 바닥글을 삽입할 위치를 식별할 수 있습니다. <InlinePopover type="browser"/>. 그러나 최종 사용자의 경험을 개선하고 성능을 높이며 코드 유지 관리를 더 쉽게 만들기 위해 프레임워크 개발자는 이 작업을 직접 수행해야 합니다.
