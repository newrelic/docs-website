...
<CollapserGroup>
  ...
  >
    1. 표준 절차에 따라 특정 앱 서버용 [New Relic PHP 에이전트를 설치하고](/docs/agents/php-agent/installation/php-agent-installation-overview) <InlinePopover type="licenseKey"/>.
  ...
  >
    이 예에서 Dockerfile은 Debian의 단일 애플리케이션에 대한 애플리케이션 파일로 일반 PHP 이미지를 확장합니다. 당신은 당신을 추가할 수 있습니다 <InlinePopover type="licenseKey"/>Dockerfile에 직접 연결하거나 `docker run` 명령어에서 환경 변수를 사용하세요.
