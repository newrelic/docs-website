...
APM의 [PHP 에이전트](/docs/agents/php-agent/getting-started/introduction-new-relic-php) 를 사용하면 [Google App Engine(GAE) 가변형 환경](https://cloud.google.com/appengine/docs/flexible/php/) 에 있는 애플리케이션을 모니터링할 수 있습니다. GAE flex 앱에 New Relic을 추가하면 앱의 상태와 성능에 대한 통찰력을 얻을 수 있고 [APM](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm) , [브라우저 모니터링](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) 및 대시보드에서 볼 수 있는 메트릭으로 GAE를 확장할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    1. 표준 절차에 따라 특정 앱 서버용 [New Relic PHP 에이전트를 설치하고](/docs/agents/php-agent/installation/php-agent-installation-overview) [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 받으세요.
  ...
  >
    이 예에서 Dockerfile은 Debian의 단일 애플리케이션에 대한 애플리케이션 파일로 일반 PHP 이미지를 확장합니다. [New Relic 라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 Dockerfile에 직접 추가하거나 `docker run` 명령에서 환경 변수를 사용할 수 있습니다.
  ...
  >
    ...
    3. New Relic에서 GAE flex 앱 데이터를 보려면 [APM **개요** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 로 이동하십시오.
  </Collapser>
</CollapserGroup>
