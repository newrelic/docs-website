...
Docker 컨테이너 또는 다른 컨테이너에 [PHP 에이전트](/docs/agents/php-agent/getting-started/introduction-new-relic-php) 를 설치하여 하나 이상의 PHP 애플리케이션을 모니터링할 수 있습니다. 이것은 [PHP 에이전트 요구 사항](/docs/agents/php-agent/getting-started/php-agent-compatibility-requirements) 을 충족하는 컨테이너에 대해 지원됩니다.
...
예제 응용 프로그램을 보려면 [New Relic의 Explorers Hub](https://discuss.newrelic.com/t/relic-solution-php-agent-and-daemon-containers/84841) 로 이동하십시오. 수명이 짧은 애플리케이션 컨테이너를 사용하는 경우 PHP 에이전트의 데몬에 대해 별도의 컨테이너를 사용하는 것이 좋습니다.
...
3. `newrelic.ini` 파일에서 `newrelic.appname` 및 `newrelic.license` 항목으로 애플리케이션 이름과 [라이선스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 설정합니다.
...
<Callout variant="caution">
  기본적으로 첫 번째 트랜잭션은 에이전트가 데몬 시작 및 응용 프로그램 연결 초기화를 트리거하도록 합니다. 성능상의 이유로 에이전트는 연결을 초기화하기 전에 해당 작업이 완료될 때까지 기다리지 않습니다. 이로 인해 컨테이너가 시작된 후 처음 몇 개의 트랜잭션이 손실될 수 있습니다. 이 손실을 방지하려면 `newrelic.ini` 파일의 `newrelic.daemon.start_timeout` 및 `newrelic.daemon.app_connect_timeout` 항목을 모두 우리의 (권장 값)\[https&#x3A;//discuss.newrelic.com/t/php-troubleshooting-framework-configuration/119980]으로 설정하십시오. 각각 5초와 15초.
</Callout>
...
2. 에이전트를 설치하려면 New Relic의 [tar 파일 다운로드 사이트](/docs/agents/php-agent/installation/php-agent-installation-tar-file) 에서 PHP 에이전트 패키지를 다운로드하고 `install` 인수와 함께 `newrelic-install` 스크립트를 실행합니다.
3. `newrelic.ini` 파일의 `newrelic.license` 및 `newrelic.appname` 항목을 통해 애플리케이션 이름과 [라이선스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 설정합니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `YOUR_LICENSE_KEY`: 이것을 [New Relic 라이센스 키로](/docs/accounts/install-new-relic/account-setup/license-key) 교체하십시오. `"REPLACE_WITH_REAL_KEY"` 은 PHP 에이전트에 대한 기본 **newrelic.ini** 파일의 실제 문자열입니다. 해당 문자열을 편집하지 마십시오. `sed` 명령은 해당 기본 문자열을 따옴표로 묶은 실제 40자 키로 바꿉니다.
