...
<Steps>
  ...
  <Step>
    ...
    1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Add data**로 이동합니다.
    2. **Elasticsearch**를검색하고 클릭합니다.
    3. 계정을 선택하고 안내된 설치 단계를 따릅니다.

    다른 방식으로 통합을 설치해야 하는 경우 [Apache 모니터링 통합을](/docs/infrastructure/host-integrations/host-integrations-list/elasticsearch/elasticsearch-integration)참조하십시오.
  </Step>
  ...
  <Step>
    ## AWS Identity and Access Management(IAM) 통합 설치 [#install-the-AWS-integration]

    AWS IAM 통합은 계정당 인증서, 계정당 그룹, 계정당 MFA 디바이스, 계정당 역할, 계정당 사용자와 같은 지표를 수집합니다.

    통합을 설치하려면:

    1. **[one.newrelic.com](https://one.newrelic.com/) > Add data**로 이동합니다.
    2. **Integrate your AWS account**를검색하여 클릭합니다.
    3. 계정을 선택하고 안내된 설치 단계를 따릅니다.

    이 통합에 대한 자세한 내용은 [AWS IAM 통합을](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-iam-monitoring-integration)참조하십시오.
  </Step>
  ...
  <Step>
    ...
    3. [인프라 에이전트를 다시 시작하십시오](/docs/infrastructure/install-infrastructure-agent/manage-your-agent/start-stop-restart-infrastructure-agent/).
  </Step>

  <Step>
    ## Magento 비즈니스 인사이트 대시보드 설치 [#install-dashboard]

    1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Dashboards**로 이동합니다.
