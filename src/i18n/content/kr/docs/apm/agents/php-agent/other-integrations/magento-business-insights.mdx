...
<Steps>
  ...
  <Step>
    ...
    다른 방식으로 통합을 설치해야 하는 경우 [Elasticsearch monitoring integration](/docs/infrastructure/host-integrations/host-integrations-list/elasticsearch/elasticsearch-integration) \[Elasticsearch 모니터링 통합을] 참조하십시오.
  </Step>
         ...
         #   host: CustomMagentoMySQL # custom host name atribute
           ...
           db_queries:
             ...
             - name: MYSQLCustomerEntityRegisteredUsers
               run: SELECT count(*) AS 'Registered Accounts' FROM `customer_entity` GROUP BY DATE(created_at);
