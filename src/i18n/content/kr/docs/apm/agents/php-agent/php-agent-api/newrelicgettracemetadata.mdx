...
* `trace.id`: 현재 실행 중인 추적 식별자입니다. 트랜잭션이 이 기능을 지원하지 않거나 분산 추적이 비활성화된 경우 빈 값이 반환됩니다.반환:
* `span.id`: 현재 실행 중인 스팬 식별자입니다. 트랜잭션이 이 기능을 지원하지 않거나 분산 추적이 비활성화된 경우 빈 값이 반환됩니다.
