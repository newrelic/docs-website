...
## 자동 계측 사용 또는 비활성화 [#auto\_instrumentation][#auto_instrumentation]
...
## 에이전트 API를 통해 수동으로 계측 [#manual\_instrumentation][#manual_instrumentation]
...
### nonce와 같은 사용자 정의 속성 추가 [#nonce-attribute]

사이트에서 `nonce` 속성을 요구하는 [콘텐츠 보안 정책(CSP)을](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) 사용하는 경우 두 API 함수 모두에 `false` 전달하면 `<script>` 태그 없이 원시 JavaScript 문자열이 반환됩니다. 그런 다음 필요한 속성을 사용하여 태그를 직접 만들 수 있습니다.
...
`false` 인수로 전달하면 함수가 일반적으로 출력하는 스크립트 태그가 억제되므로 필요한 속성을 가진 `<script>` 태그로 원시 JavaScript 문자열을 감쌀 수 있습니다.
...
## 다양한 프레임워크에 대한 수동 계측 [#manual\_generic][#manual_generic]
...
<CollapserGroup>
  <Collapser id="manual_wp" title="워드프레스">
  ...
  <Collapser id="manual_drupal" title="드루팔">
...
브라우저 모니터링을 활성화하고 앱에 대한 트래픽을 생성하면 앱의 [브라우저 <DNT>**Summary**</DNT> 페이지](/docs/browser/new-relic-browser/getting-started/browser-overview-page-website-performance-summary) 에 데이터가 표시됩니다. 데몬을 처음 시작할 때(또는 PHP 에이전트가 새로운 기능을 만날 때) 데몬이 설정을 확인하는 동안 짧은 지연이 있습니다. 그 이후에는 뉴렐릭 JavaScript 스니펫을 사용할 수 있습니다.
