...
<CollapserGroup>
  ...
  >
    ...
    2. 마지막 닫기 레이아웃 태그 바로 앞에 바닥글을 생성하는 호출을 삽입합니다.
       ...
       <Callout variant="important">
         Drupal 7.15에서는 <DoNotTranslate>**Compress cached pages**</DoNotTranslate> 기본적으로 켜져 있습니다. <DoNotTranslate>**Cache pages for anonymous users**</DoNotTranslate> 도 선택하면 JavaScript(newrelic.js)가 익명 사용자를 위해 제공되는 페이지에 삽입되지 않습니다. 이는 Drupal의 페이지가 캐시에 저장되기 전에 데이터베이스에서 직접 압축되어(gzip 사용) 뉴렐릭의 PHP 에이전트가 HTML을 구문 분석할 기회가 없기 때문입니다. 이러한 상황에서는 수동 계측이 익명 사용자의 데이터를 캡처할 수 있는 더 나은 기회를 제공합니다.
       </Callout>
...
브라우저 모니터링을 활성화하고 앱에 대한 트래픽을 생성하면 앱의 [브라우저 <DoNotTranslate>**Summary**</DoNotTranslate> 페이지](/docs/browser/new-relic-browser/getting-started/browser-overview-page-website-performance-summary) 에 데이터가 표시됩니다. 데몬을 처음 시작할 때(또는 PHP 에이전트가 새로운 기능을 만날 때) 데몬이 설정을 확인하는 동안 짧은 지연이 있습니다. 그 이후에는 뉴켈릭 JavaScript 스니펫을 사용할 수 있습니다.
