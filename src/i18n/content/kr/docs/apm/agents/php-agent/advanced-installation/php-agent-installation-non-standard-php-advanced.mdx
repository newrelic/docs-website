...
2. `newrelic.ini` 파일의 클러스터 키 설정을 <InlinePopover type="licenseKey"/>로 바꿉니다. 또한 필요에 따라 다른 세션도 변경합니다(예: `newrelic.appname`).
...
앱에서 트래픽을 생성한 후 [APM <DNT>**Summary**</DNT> 페이지](/docs/apm/apm-ui-pages/monitoring/apm-summary-page-view-transaction-apdex-usage-data/) 에 결과가 나타날 때까지 3-5분 정도 기다립니다.
