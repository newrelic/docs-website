...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        고유한 맞춤 정의 지문으로 오류를 그룹화하고 [`ErrorGroupCallback`](/docs/apm/agents/java-agent/api-guides/java-agent-api-register-error-group-callback-to-set-fingerprint) 인터페이스를 구현하여 New Relic으로 보낼 오류에 대한 그룹화 키를 생성하는 데 사용합니다. 지문은 오류 받은 편지함 UI에서 오류 메시지를 그룹화하는 데 사용됩니다.
      </td>
    ...
    <tr>
      <td>
        콜백 기능을 통해 나만의 에러 지문을 설정하세요. 제공된 오류에 대한 그룹화 키를 생성하는 데 사용되는 [`ErrorGroupCallback`](/docs/apm/agents/java-agent/api-guides/java-agent-api-register-error-group-callback-to-set-fingerprint) 를 등록합니다.
      </td>
