...
<CollapserGroup>
  ...
  >
    ...
    * `request.headers.userAgent`
      ...
      사용자 지정이든 표준이든 추가 요청 헤더를 캡처하려면 [구성 파일 문서를](https://docs.newrelic.com/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file/#tt-custom-request-headers) 참조하거나 [Java 에이전트 API](/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes#api-java)의 `addCustomParameter()` 메서드를 사용하세요.
