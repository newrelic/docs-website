...
2. 왼쪽 메뉴에서 <DoNotTranslate>**SERVER SETTINGS > Java and JVM**</DoNotTranslate>을 선택합니다.

3. 에이전트 API를 사용하는 경우: <DoNotTranslate>**ColdFusion Class Path**</DoNotTranslate> 필드에서 `newrelic-api.jar`로의 경로를 지정합니다.

4. <DoNotTranslate>**JVM Arguments**</DoNotTranslate> 필드에 `-javaagent` 인수를 추가합니다.

   ```
   -javaagent:/full/path/to/newrelic.jar
   ```

5. <DoNotTranslate>**Submit Changes**</DoNotTranslate>를 선택한 다음 ColdFusion 서버를 다시 시작합니다.
...
1. Glassfish 콘솔에서 <DoNotTranslate>**Application Server > JVM Settings > JVM options**</DoNotTranslate>를 선택합니다.

2. JVM 옵션 페이지에서 <DoNotTranslate>**Add JVM option**</DoNotTranslate>을 선택합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Play 2.0"
  >
    1. `start` 스크립트가 포함된 <DoNotTranslate>**unzipped**</DoNotTranslate> 배포로 시작합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="앱 서버 Solr"
  >
    <DoNotTranslate>**application server**</DoNotTranslate>에서 Solr를 실행할 때 해당 애플리케이션 서버의 지침에 따라 `-javaagent` 플래그를 추가합니다. 또한 애플리케이션 서버에 대해 JMX가 활성화되어 있는지 확인합니다. APM UI의 Solr 페이지에 데이터가 표시되지 않으면 [Solr 데이터 문제 해결 절차](/docs/agents/java-agent/troubleshooting/solr-data-not-appearing-apm-solr-tab-java)를 따르십시오.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Windows 사용"
  >
    1. <DoNotTranslate>**Start > Apache Tomcat X.Y.Z. > Configure Tomcat > Java**</DoNotTranslate>을 선택합니다.

    2. <DoNotTranslate>**Java Options**</DoNotTranslate> 텍스트 상자에 인수를 입력합니다. 경로 구분 기호로 슬래시(`/`)를 사용합니다. Tomcat 6의 경우 `-javaagent` 인수 뒤에 줄 바꿈을 추가합니다.

       ```
       -javaagent:/full/path/to/newrelic.jar
       ```

    3. <DoNotTranslate>**Apply**</DoNotTranslate>를 선택한 다음 Tomcat을 다시 시작합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    1. 관리 콘솔에서 <DoNotTranslate>**Environments > Servers > (select a server) > Server Start > Arguments**</DoNotTranslate>로 이동합니다.

    2. <DoNotTranslate>**Arguments**</DoNotTranslate>에서 다음을 추가합니다.
...
1. 관리 콘솔에서 <DoNotTranslate>**Servers > Application servers > (select a server) > Configuration > Service Infrastructure > Java and Process Management**</DoNotTranslate>를 선택합니다.

2. <DoNotTranslate>**Process Definition > Additional Properties**</DoNotTranslate>를 선택한 다음 <DoNotTranslate>**Java Virtual Machine**</DoNotTranslate>를 클릭합니다.

3. <DoNotTranslate>**Generic JVM arguments**</DoNotTranslate> 필드에서 `newrelic.jar` 파일에 대한 `-javaagent` 인수를 추가합니다.

   ```
   -javaagent:/full/path/to/newrelic.jar
   ```

4. <DoNotTranslate>**Apply**</DoNotTranslate>를 선택한 다음 <DoNotTranslate>**Save**</DoNotTranslate>를 클릭합니다.
