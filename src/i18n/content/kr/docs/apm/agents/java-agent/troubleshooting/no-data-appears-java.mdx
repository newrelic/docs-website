...
6. New Relic Java 에이전트와 관련된 오류에 대한 애플리케이션 서버 로그를 확인하십시오. Java 에이전트 데이터가 보고되지 않도록 하는 응용 프로그램 서버 로그에서 관찰 가능한 오류의 예는 문제 해결 페이지 [Error Bootstrap New Relic Java Agent](/docs/agents/java-agent/troubleshooting/error-bootstrapping-new-relic-java-agent) 및 [Errors Java App Server](/docs/agents/java-agent/troubleshooting/errors-starting-java-app-server) 를 참조하십시오.
