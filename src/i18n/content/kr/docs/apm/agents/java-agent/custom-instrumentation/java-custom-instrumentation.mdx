...
New Relic은 많은 프레임워크에서 자동으로 [메트릭과 이벤트](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data) 를 수집합니다. [지원되는 프레임워크 또는 구성요소](/docs/agents/java-agent/getting-started/compatibility-requirements-java-agent) 를 사용하는 경우 기본적으로 일부 트랜잭션 정보를 볼 수 있으며 사용자 정의 계측을 사용하여 기본적으로 에이전트가 보고하는 정보를 보완할 수 있습니다.
