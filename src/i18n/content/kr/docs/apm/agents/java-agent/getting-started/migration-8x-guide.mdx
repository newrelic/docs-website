...
<Callout variant="important">
  버전 8.0-8.7에서 제거된 속성이 8.8.0에 다시 도입되었습니다. 이는 여전히 해당 속성을 사용하는 고객에게 이전 버전과의 호환성을 제공합니다. <InlinePopover type="alerts"/>또는 대시보드.
</Callout>

## 트랜잭션 및 범위 속성 변경 및 제거 [#changed_and_removed_attributes]

APM 에이전트 전체에 일관성을 제공하고 업계 표준에 더 잘 부합하기 위해 Java 에이전트의 트랜잭션 및 범위 속성을 변경했습니다. Java 에이전트 버전 7.6.0부터 새로운 표준 속성인 **http.statusCode** 및 **http.statusText**를 도입하고 **httpResponseCode**, **response.status** 로 표시했습니다. 및 **response.statusMessage** 더 이상 사용되지 않습니다.

| 더 이상 사용되지 않는 속성        | 새로운 속성             | 데이터 유형 변경     |
| ---------------------- | ------------------ | ------------- |
| **httpResponseCode**   | **http.status코드**  | **문자열을 INT로** |
| **응답 상태**              | **http.status코드**  | **문자열을 INT로** |
| **response.status메시지** | **http.status텍스트** | **해당 없음**     |
...
2. 반환된 각 경고에 대해 **Alerts & AI > [Alerts Conditions](https://one.newrelic.com/alerts-ai/condition-builder/condition-list)** UI에서 ID 또는 이름을 사용하여 경고를 찾을 수 있습니다.
...
**http.statusCode**는 **httpResponseCode** 또는 **response.status** 에 대한 참조를 대체합니다. 이는 또한 다른 데이터 유형을 갖습니다. 이전에는 다음과 같은 참조를 사용했습니다.
...
**http.statusText** 는 **response.statusMessage** 에 대한 참조를 대체합니다. 이 속성의 데이터 유형은 변경되지 않았습니다.
