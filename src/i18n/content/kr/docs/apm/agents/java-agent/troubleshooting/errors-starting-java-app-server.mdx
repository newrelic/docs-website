...
이 문제를 해결하려면 `-javaagent` 스위치에서 `newrelic.jar` 파일의 전체 경로를 지정하십시오. 예를 들어, `newrelic.jar` 파일이 Tomcat 홈 디렉터리 <DNT>**/home/tomcat**</DNT> 내의 `newrelic` 디렉터리에 있는 경우 다음과 같이 스위치를 지정합니다.
