...
우리의 자바 <InlinePopover type="apm"/>에이전트는 애플리케이션 모니터링을 시작할 수 있도록 코드를 자동 계측합니다. 일반 APM 설치 프로세스에 대한 개요를 보려면 여기를 읽으십시오. 보다 구체적인 지침을 보려면 [Java 앱 모니터링을](/docs/apm/agents/java-agent/getting-started/monitor-your-java-app) 사용하여 Java 앱 구성에 대한 특정 리소스를 찾으십시오.
