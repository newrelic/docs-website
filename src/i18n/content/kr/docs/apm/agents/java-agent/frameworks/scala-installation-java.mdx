...
<CollapserGroup>
  ...
  >
    애플리케이션의 `project/build.scala` 파일에 있는 appDependencies 메서드에 다음 행을 추가합니다(SV를 Scala 버전으로, XYZ를 사용하는 [Java 에이전트 버전](/docs/agents/java-agent/installation/update-java-agent#procedures) 으로 대체).
  ...
  >
    애플리케이션의 `project/build.sbt` 파일에 다음 행을 추가합니다(SV를 Scala 버전으로, XYZ를 사용하는 [Java 에이전트 버전](/docs/agents/java-agent/installation/update-java-agent#procedures) 으로 대체).
...
1. Scala 구성 파일에 다음 정보를 추가합니다.
   ...
   <CollapserGroup>
     <Collapser
       ...
       title={<><strong>project/build.scala 파일을 사용하여 구성</strong></>}
     >
       애플리케이션의 `project/build.scala` 파일에 있는 appDependencies 메서드에 다음 줄(XYZ를 사용하는 [Java 에이전트 버전](/docs/agents/java-agent/installation/update-java-agent#procedures) 으로 바꿈)을 추가합니다.
     ...
     <Collapser
       ...
       title={<><strong>project/build.sbt 파일을 사용하여 구성</strong></>}
     >
       애플리케이션의 `project/build.sbt` 파일에 다음 줄을 추가합니다(XYZ를 사용하는 [Java 에이전트 버전](/docs/agents/java-agent/installation/update-java-agent#procedures) 으로 대체).
