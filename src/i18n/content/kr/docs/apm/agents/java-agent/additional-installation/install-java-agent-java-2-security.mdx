...
1. `SOLR_SECURITY_MANAGER_ENABLED=false` 환경 변수를 설정하거나 `solr.in.sh/solr.in.cmd` 시작 파일에서 설정합니다.
...
2. 정책 파일에 다음 텍스트를 추가하십시오. 경로 끝에 있는 하이픈 `-` 을 유지해야 합니다. 이렇게 하면 대상, 대상 폴더에 있는 모든 jar 파일에 필요한 권한이 부여됩니다.
   ...
   <Callout variant="tip">
     자바 9+를 사용 중이고 액세스 거부 오류가 발생하는 경우, 추가 권한(`java.lang.RuntimePermission "accessSystemModules"` 등)을 부여해야 할 수 있습니다.
   </Callout>
