...
Linux 에이전트 환경의 뉴렐릭 인프라 에이전트가 뉴렐릭 에이전트 에이전트와 정확히 동일한 이름 문자열을 반환하도록 호스트명 설정을 구성합니다. <DNT>**Recommendation:**</DNT> FQDN(정규화된 도메인 이름) 설정을 편집합니다. 에이전트는 운영 시스템의 FQDN 설정에서 호스트명을 읽으므로 호스트명을 거기에 설정하면 두 에이전트가 단일 호스트명을 공유하게 됩니다.
