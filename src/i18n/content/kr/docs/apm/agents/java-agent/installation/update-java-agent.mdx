...
1. <DoNotTranslate>**entire**</DoNotTranslate> [에이전트](/docs/agents/manage-apm-agents/troubleshooting/find-agent-root-directory#java-agent) 에이전트를 다른 위치에 백업합니다. 해당 디렉토리의 이름을 `NewRelic_Agent#.#.#` 로 바꾸십시오. 여기서 `#.#.#` 는 에이전트 버전 번호입니다.
...
4. (선택 사항) 우리의 [에이전트 에이전트 API](/docs/apm/agents/java-agent/api-guides/guide-using-java-agent-api/) 또는 [Scala API](/docs/apm/agents/java-agent/frameworks/scala-installation-java/#using-the-scala-api) 와 같은 에이전트 API를 사용하는 경우 API 의존성/종속성을 최신 버전으로 업그레이드하는 것이 좋습니다. 최신 에이전트 버전은 계속해서 API를 지원하지만 새로운 기능이 누락될 수 있습니다. Maven과 같은 빌드 도구를 사용하는 경우 API 버전을 최신 버전으로 업그레이드하세요. 예를 들어 Gradle과 함께 중력 에이전트 API 사용하는 경우 `${agent.version}` 를 최신 버전으로 바꿉니다.
