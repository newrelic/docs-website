...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택) > Transactions > Transaction trace > Trace details** : 토큰과 연결된 모든 비동기 메서드 호출은 <Icon name="fe-search"/>**비동기** 아이콘입니다.
    </figcaption>
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택) > Transactions > Transaction trace > Trace details:** **Drilldown** 열에서 단일 비동기 세그먼트가 호출됩니다.
    </figcaption>
  </Collapser>
</CollapserGroup>
