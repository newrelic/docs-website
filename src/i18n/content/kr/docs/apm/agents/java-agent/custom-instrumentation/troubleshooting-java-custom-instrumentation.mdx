...
**Java 앱만 해당:** New Relic UI에서 직접 XML 파일을 편집하려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (Java 앱 선택) > Settings > Live Instrumentation** 으로 이동합니다. 여기에서 다음을 수행할 수 있습니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Events > Thread profiler:** 메서드가 회색으로 표시된 경우( **Instrumentation not allowed** ), 스레드 프로필 트리를 확장하여 계측할 수 있는 중첩 트랜잭션(흰색)을 찾습니다.
</figcaption>
