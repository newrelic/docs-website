...
[Heroku](https://devcenter.heroku.com/articles/newrelic) 는 Scala를 포함한 다양한 에이전트 언어로 웹 애플리케이션을 호스팅하기 위한 PaaS(Platform as a Service) 솔루션입니다. New Relic을 사용하면 [APM](/docs/apm/new-relic-apm) 의 메트릭으로 Heroku를 확장할 수 있으며 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser).
