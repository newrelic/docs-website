...
APM UI의 추적 세부 정보 폭포 보기에서 UI는 비동기 활동을 가로 x축에서 겹치는 것으로 표시합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Summary** : APM **Summary** 페이지에서 Java 앱의 비동기 활동으로 인해 응답 시간(파란색 선)이 총 트랜잭션 시간보다 짧을 수 있습니다. 메서드가 반환되기 전에 이전 메서드가 모두 완료될 때까지 기다릴 필요가 없기 때문입니다.
</figcaption>
