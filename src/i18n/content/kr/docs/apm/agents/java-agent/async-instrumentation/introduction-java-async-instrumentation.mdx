...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM 및 서비스 > (앱 선택) > 요약** : APM **요약** 페이지에서 Java 앱의 비동기 활동으로 인해 응답 시간(파란색 선)이 총 트랜잭션 시간보다 짧아질 수 있습니다. 메서드가 반환되기 전에 이전 메서드가 모두 완료될 때까지 기다릴 필요가 없기 때문입니다.
</figcaption>
