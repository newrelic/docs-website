...
New Relic의 비동기식 계측을 통해 APM UI는 작업에 **Async** 플래그를 지정합니다. 예를 들어 위의 방법이 New Relic에 의해 계측된 경우(자동으로 또는 사용자 지정 계측을 통해) APM UI는 다음과 같이 [**추적 세부 정보** 페이지](/docs/apm/transactions/transaction-traces/transaction-trace-details) 에 표시됩니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Transactions > Transaction trace > Trace details** : APM이 Java 애플리케이션의 비동기 활동을 감지하면 **드릴다운** 열에 **비동기** 플래그로 표시됩니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Summary** : APM **요약** 페이지에서 Java 앱의 비동기 활동으로 인해 응답 시간(파란색 선)이 총 트랜잭션 시간보다 짧아질 수 있습니다. 메서드가 반환하기 전에 이전 메서드가 모두 완료될 때까지 기다릴 필요가 없기 때문입니다.
</figcaption>
