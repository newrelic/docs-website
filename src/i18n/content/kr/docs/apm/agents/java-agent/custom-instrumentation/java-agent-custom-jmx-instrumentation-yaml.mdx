...
맞춤 JMX 데이터는 <InlinePopover type="apm"/>차트에 표시되지 않지만 [데이터를 쿼리](#display_metrics) 하고 이를 사용하여 맞춤 차트를 만들 수 있습니다.
...
YAML 파일에 설정된 대로 <DNT>**JMX**</DNT> 또는 카운터에 대한 직접 경로를 검색하는 방법은 다음과 같습니다.
