...
로그 서버는 APM 의 <DNT>**JVMs**</DNT> 페이지에 나타나는 다양한 지표를 수집하고 보고합니다. ( Heroku 통해 뉴렐릭을 사용하는 경우 <DNT>**JVMs**</DNT> 페이지 이름은 <DNT>**[Instances](/docs/accounts-partnerships/partnerships/heroku-new-relic/heroku-instances-dashboard)**</DNT>입니다.)
...
1. <DNT>
     **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs > (select a JVM)**
   </DNT>
...
2. 선택한 JVM 의
   ...
   페이지에서 뉴렐릭의 표준 [사용자 인터페이스 기능을](/docs/accounts-partnerships/education/getting-started-new-relic/standard-dashboard-features) 사용하여 자세한 정보를 살펴보세요.

3. 다른 측정항목을 보려면 [사용 가능한 탭](#dashboard-tabs) 을 선택합니다.

4. 선택 사항: [스레드 약력러를](/docs/applications-menu/thread-profiler) 사용하여 실행 스레드에 대한 데이터에서 병목현상, 병목지점을 찾으려면
...
<img
  ...
  src="/images/apm_screenshot-crop_jvms.webp"
/>

<figcaption>
  <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs**</DNT>: 앱 서버가 보고하는 지표에 따라 JVM 페이지에 하나 이상의 탭이 나타납니다.
</figcaption>
    ...
    <tr>
      ...
      <td>
        ...
        * <DNT>
          ...
          풀의 활성 및 유휴 스레드 수입니다. 지원되는 앱 서버에서 차트에는 앱 서버에 있는 각 스레드 풀과 각 스레드 풀의 최대 스레드 수에 대한 활성 스레드 비율이 표시됩니다.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        * <DNT>
          ...
          애플리케이션에 대한 활성, 시간 초과로 인해 무효화 및 무효화된 HTTP 세션 수입니다. 지원되는 앱 서버에서는 차트에 앱 서버에 있는 각 애플리케이션이 표시됩니다.
      </td>
    </tr>
...
지원되는 앱 서버의 경우 에이전트는 JMX 데이터 소스 지표를 수집합니다. 이 지표는 <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs > Data sources**</DNT> 에서 볼 수 있습니다. 이러한 지표를 보려면 데이터 소스가 앱 서버의 JMX 모니터링에 대해 올바르게 구성되었는지 확인하세요.
    ...
    <tr>
      ...
      <td>
        * <DNT>
          ...
          데이터 소스에 사용 가능한 최대 연결 수입니다. 이는 일반적으로 구성 가능한 시위입니다.

        * <DNT>
            **Active connections:**
          </DNT>

          데이터 소스의 현재 활성 연결 수입니다.

        * <DNT>
            **Idle connections:**
          </DNT>

          데이터 소스의 현재 유휴 연결 수입니다.
      </td>
    </tr>
...
WebLogic에서는 JMX가 기본적으로 활성화되어 있습니다. 뉴렐릭은 다음과 같은 <DNT>**mbeans**</DNT> 지표를 수집합니다.
