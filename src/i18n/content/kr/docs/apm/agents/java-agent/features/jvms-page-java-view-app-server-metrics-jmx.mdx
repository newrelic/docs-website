...
로그 서버는 APM 의 <DoNotTranslate>**JVMs**</DoNotTranslate> 페이지에 나타나는 다양한 지표를 수집하고 보고합니다. ( Heroku 통해 뉴렐릭을 사용하는 경우 <DoNotTranslate>**JVMs**</DoNotTranslate> 페이지 이름은 <DoNotTranslate>**[Instances](/docs/accounts-partnerships/partnerships/heroku-new-relic/heroku-instances-dashboard)**</DoNotTranslate>입니다.)

최소한 각 앱 서버는 메모리에서 메트릭 데이터를 수집하고 보고합니다. JMX 메트릭 활성화에 대한 지침은 앱 서버의 설명서를 참조하십시오.

JVM 측정 단위를 보려면 다음을 수행합니다.

1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs > (select a JVM)**</DoNotTranslate>
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs**</DoNotTranslate>: 앱 서버가 보고하는 지표에 따라 JVM 페이지에 하나 이상의 탭이 나타납니다.
</figcaption>
...
지원되는 앱 서버의 경우 에이전트는 JMX 데이터 소스 지표를 수집합니다. 이 지표는 <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitoring > JVMs > Data sources**</DoNotTranslate> 에서 볼 수 있습니다. 이러한 지표를 보려면 데이터 소스가 앱 서버의 JMX 모니터링에 대해 올바르게 구성되었는지 확인하세요.
...
<CollapserGroup>
  ...
  >
    ...
    3. <DoNotTranslate>**Enable Performance Monitoring Infrastructure (PMI)**</DoNotTranslate>

       확인란을 선택합니다.

    4. <DoNotTranslate>**Currently monitored statistic set**</DoNotTranslate>
  ...
  >
    ...
    4. <DoNotTranslate>**Custom**</DoNotTranslate>
    ...
    5. <DoNotTranslate>**Messages**</DoNotTranslate>
...
WebLogic에서는 JMX가 기본적으로 활성화되어 있습니다. 뉴렐릭은 다음과 같은 <DoNotTranslate>**mbeans**</DoNotTranslate> 지표를 수집합니다.
