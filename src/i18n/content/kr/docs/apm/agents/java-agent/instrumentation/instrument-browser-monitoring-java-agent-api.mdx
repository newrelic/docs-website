...
그만큼 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)기능을 사용하면 JavaScript 스니펫(또는 "에이전트")을 사용하여 앱의 웹페이지를 계측함으로써 사용자가 Java 애플리케이션과 상호 작용하는 방식에 대한 가시성을 얻을 수 있습니다. UI를 통해 브라우저 에이전트를 설치하려면 [브라우저 에이전트 설치](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent) 절차를 따르세요. 예를 들어 다음을 수행할 수 있습니다.
...
2. 자동 계측을 비활성화합니다.
   ...
   <CollapserGroup>
     ...
     >
       `browser_monitoring` 섹션 아래의 `disabled_auto_pages` 플래그를 사용합니다. 예를 들어, `testpage_1.jsp` 및 `testpage_2.jsp` 에서 자동 계측을 비활성화하려면 다음을 사용합니다.
...
4. 시간 추적을 활성화하려면 `com.newrelic.api.agent.NewRelic.getBrowserTimingHeader()` 메서드를 추가하세요. 특정 프레임워크에 대한 절차는 아래를 참조하세요.

   <dd>
     <Callout variant="important">
       [Java 에이전트 8.9.0](/docs/release-notes/agent-release-notes/java-release-notes/java-agent-890) 부터 [getBrowserTimingFooter() API](https://newrelic.github.io/java-agent-api/javadoc/index.html?com/newrelic/api/agent/NewRelic.html) 메서드 호출은 더 이상 사용되지 않습니다. 이제 `getBrowserTimingHeader()` 호출 시 전체 브라우저 스크립트가 포함됩니다.
     </Callout>
   </dd>
   ...
   <CollapserGroup>
     <Collapser
       ...
       title="JSP 수동 계측"
     >
       헤더를 가능한 한 `<head>` 태그 상단에 가깝게 삽입하되 메타 태그 뒤에 삽입하세요.
     ...
     >
       8.xx를 [최신 에이전트 버전](/docs/release-notes/agent-release-notes/java-release-notes) 으로 대체하여 `build.sbt` 에 다음을 추가합니다.
     ...
     >
       8.xx를 [최신 에이전트 버전](/docs/release-notes/agent-release-notes/java-release-notes) 으로 대체하여 `Build.scala` 에 다음을 추가합니다.
     ...
     >
       ...
       4. 각 앱에서 애플리케이션의 다른 페이지에서 사용하는 Velocity 템플릿을 편집합니다. `<link>` 및 `<script>` 선언 앞과 메타 태그 뒤의 `<head>` 섹션 내에서 이 태그를 추가합니다.
