...
<Callout variant="tip">
  이 도구에 대한 피드백, 질문 또는 문제가 있는 경우 [New Relic의 Explorers Hub](https://discuss.newrelic.com/tags/javaagent) 에 게시물을 만들고 게시물에 `java_incubator` 태그를 지정하세요.
</Callout>
...
<CollapserGroup>
  ...
  >
    <table>
      <tbody>
        <tr>
          ...
          <td>
            [모듈 다운로드](https://download.newrelic.com/newrelic/java-agent/extensions/lettuce-4.0.jar)
          </td>
        </tr>
    ...
    이 계측 패키지는 Lettuce 4.x Redis 드라이버를 계측합니다. Redis에 대한 호출이 데이터 저장소 호출로 표시되도록 합니다.
  </Collapser>
  ...
  >
    <table>
      <tbody>
        <tr>
          ...
          <td>
            [모듈 다운로드](https://download.newrelic.com/newrelic/java-agent/extensions/lettuce-5.0.jar)
          </td>
        </tr>
    ...
    이 계측 패키지는 Lettuce 5.x Redis 드라이버를 계측합니다. Redis에 대한 호출이 데이터 저장소 호출로 표시되도록 합니다.
  </Collapser>
