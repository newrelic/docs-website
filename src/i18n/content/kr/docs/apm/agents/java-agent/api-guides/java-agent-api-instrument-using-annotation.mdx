...
## 주석에 대한 에이전트 구성 [#configure]

기본적으로 구성 설정 `enable_custom_tracing` 은 Java 에이전트에서 @Trace 주석이 작동하는 데 필요한 설정인 `true` 으로 설정됩니다. 이 설정은 기본적으로 `newrelic.yml` 에 포함 **되지 않습니다** .

이 설정을 구성 파일에 통합해야 하는 유일한 경우는 @Trace 주석을 모두 비활성화하려는 경우입니다. 이렇게 하려면 `newrelic.yml` 의 `common` 스탠자에 `enable_custom_tracing: false` (두 개의 공백으로 시작)을 설정하십시오.

이러한 주석이 모두 포함되어 있으므로 `newrelic-api.jar` 이 클래스 경로에 나타나는지 확인하십시오.

## @추적하다 [#trace]

메소드에 `@Trace` 주석을 추가하면 해당 메소드에 대해 측정을 수행해야 함을 Java 에이전트에 알립니다.

메서드 호출을 사용자 지정 추적으로 추가하려면 메서드에 `@Trace` 주석을 추가합니다.
...
<CollapserGroup>
  ...
  >
    ...
    `@Trace(metricName="YourMessageHere")` 에서와 같이 `metricName` 을 설정하면 이 메서드에 소요된 시간이 모든 트랜잭션 추적에서 <var>YourMessageHere</var> 로 표시됩니다.

    `@Trace(metricName="YourMessageHere", dispatcher=true)` 에서와 같이 디스패처와 함께 `metricName` 을 설정하면 트랜잭션 이름이 APM **트랜잭션** 페이지에 <var>YourMessageHere</var> 로 표시되지만 이 메서드에 소요된 시간은 트랜잭션 추적에서 <var>YourMessageHere</var> 로 표시되지 않습니다.
