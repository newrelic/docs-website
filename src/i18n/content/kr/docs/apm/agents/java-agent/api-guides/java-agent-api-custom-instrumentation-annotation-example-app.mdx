...
<CollapserGroup>
  <Collapser id="complete-api-example" title="전체 API 호출 예시">
    ```java
...
<CollapserGroup>
  <Collapser className="freq-link" id="import-packages" title="필요한 패키지 가져오기">
  ...
  <Collapser className="freq-link" id="set-annotation" title="트랜잭션 추적에 대해 @Trace 설정">
  ...
  <Collapser className="freq-link" id="name-transactions" title="웹 트랜잭션에 대한 사용자 지정 이름 만들기">
  ...
  <Collapser className="freq-link" id="ignore-apdex" title="비공개 데이터 수집 시 Apdex 우회">
  ...
  <Collapser className="freq-link" id="record-user-id" title="사용자 ID 기록">
  ...
  <Collapser className="freq-link" id="get-custom-metric" title="프로모션 데이터 수집">
  ...
  <Collapser className="freq-link" id="control-handler" title="핸들러에 지침 보내기">
  ...
  <Collapser className="freq-link" id="include-browser" title="HTTP 응답에 페이지 로드 타이밍 코드 포함">
    ...
    * `<head>` 태그 뒤에 헤더를 설정하세요.

    * 푸터를 `<body>` 태그 끝에 설정하세요.
  ...
  <Collapser className="freq-link" id="complete-response" title="HTTP 응답 완료">
