...
사용자 정의 계측 편집기를 사용하려면: **[one.newrelic.com](https://one.newrelic.com/all-capabilities)APM & services > (Java 앱 선택) > Settings > Instrumentation** 으로 이동하십시오. 사용자 정의 계측 편집기를 사용하여 다음을 수행합니다.
...
2. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) AAPM & services > (앱 선택) > Settings > Instrumentation** 으로 이동합니다. **최근에 수집된 스레드 프로필** 목록이 표시될 때까지 페이지 맨 아래로 스크롤한 다음 가장 최근의 스레드 프로필을 선택합니다.
