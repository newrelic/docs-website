  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        보안상의 이유로 Custom Instrumentation Editor를 사용한 사용자 정의 속성 캡처는 기본적으로 **비활성화되어** 있으며 [높은 보안 모드를](/docs/agents/java-agent/getting-started/apm-agent-security-java) 사용하는 동안에는 활성화할 수 없습니다. 사용자 정의 계측 편집기를 사용하여 사용자 정의 속성을 보고하고 Java 에이전트가 높은 보안 모드에 있는 것을 원하지 않는 경우 [높은 보안 모드를](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-enable_high_security) 비활성화한 후 **newrelic.yml** 의 `common:` 블록에 다음 텍스트를 추가합니다. :
