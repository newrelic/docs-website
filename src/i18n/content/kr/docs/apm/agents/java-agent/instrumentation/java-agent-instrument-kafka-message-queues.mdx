...
2. 측정항목 탐색기를 사용하여 측정항목을 찾습니다. 이 메트릭 폴더에서 Kafka 메트릭을 찾을 수 있습니다.
   ...
   Kafka Streams를 사용하는 경우 이 메트릭 폴더에서 스트림 메트릭을 찾을 수 있습니다.

   ```
   Kafka/Streams/KafkaStreamsMetricName
   ```

   예를 들어 `poll-latency-avg` 측정항목은 다음 위치에 있습니다.
...
그렇지 않고 `kafka-streams-spans` 계측 모듈을 사용하려는 경우:
