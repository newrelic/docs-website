...
트레이스 관찰자는 모든 트레이스를 수집하고 분석하는 뉴렐릭 AWS기반 서비스입니다. <a href="/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer" target="_blank">트레이스 관찰자 설정</a> 의 지침을 따르세요. 완료되면 트레이스 관찰자 정보를 가지고 여기로 돌아와 에이전트를 구성하는 다음 단계를 계속 진행하세요.
...
<CollapserGroup>
  <Collapser id="java" title="자바 에이전트 호환성">
...
<CollapserGroup>
  <Collapser id="java-config-standard" title="자바 에이전트 구성">
