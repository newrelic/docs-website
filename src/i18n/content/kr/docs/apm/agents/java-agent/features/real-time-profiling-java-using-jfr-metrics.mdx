...
<Callout variant="tip">
  Java 에이전트 릴리스 8.0.0 이상을 사용하면 JVM을 다시 시작하지 않고도 JFR 프로파일링 구성을 동적으로 변경할 수 있습니다. **Settings > Application > Java Flight Recorder** \[설정 > 애플리케이션 > Java Flight Recorder] 에서 설정을 전환하거나 `jfr.enabled` 구성을 변경하여 이를 수행합니다.
</Callout>
