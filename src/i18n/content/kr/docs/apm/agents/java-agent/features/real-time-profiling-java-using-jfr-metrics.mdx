...
데이터를 보려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (서비스 선택) > More views > Realtime Profiling Java**로 이동하십시오.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (select service) > Realtime Profiling Java**: JVM 클러스터 타임라인 보기는 클러스터 전체에서 JVM 동작을 보여줍니다.
</figcaption>
...
JVM 세부 정보 페이지를 찾으려면 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (서비스 선택) > Realtime Profiling Java > (JVM 선택)** 으로 이동합니다.
