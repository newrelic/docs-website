...
* [New Relic Java 에이전트 JFR 서비스](https://github.com/newrelic/newrelic-jfr-core/blob/main/README.md#new-relic-java-agent-jfr-service) (권장) - 플래그십 [New Relic Java 에이전트](/docs/agents/java-agent/) 에 내장된 JFR 모니터링. 추가 설정이 필요하지 않습니다 [. Java 에이전트를 설치](/docs/agents/java-agent/installation/install-java-agent/) 하고 [JFR 서비스가 활성화](/docs/agents/java-agent/configuration/java-agent-configuration-config-file/#jfr-real-time-profiling) 되어 있는지 확인하고 JFR 데이터가 Java 에이전트와 동일한 APM 애플리케이션으로 유입되는지 확인하십시오. [New Relic Java 에이전트 버전 7.0.0 이상이](/docs/release-notes/agent-release-notes/java-release-notes/java-agent-700/) 필요합니다.
...
데이터를 보려면 **[one.newrelic.com](https://one.newrelic.com) > Explorer > (서비스 선택) > More Views > Realtime Profiling Java** 로 이동하십시오.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > (서비스 선택) > Realtime Profiling Java** : JVM 클러스터 타임라인 보기는 클러스터 전반의 JVM 동작을 보여줍니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Explorer > (서비스 선택) > Realtime Profiling Java > (JVM 선택)** : 각 JVM에 대한 세부 정보를 볼 수 있습니다.
</figcaption>
...
<figcaption>
  **[one.newrelic.com](one.newrelic.com/) > Explorer > (서비스 선택) > Realtime Profiling Java > (JVM 선택)** : 플레임 그래프를 포함하여 각 JVM에 대한 세부 정보를 볼 수 있습니다.
</figcaption>