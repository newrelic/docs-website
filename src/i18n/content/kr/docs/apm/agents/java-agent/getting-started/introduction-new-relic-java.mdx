...
[뉴렐릭 UI에서 앱 성능을 보려면](/docs/apm/applications-menu/monitoring/apm-overview-page) <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Summary**</DoNotTranslate>로 이동합니다. APM 사용자 인터페이스에는 전용 [<DoNotTranslate>**JVM metrics**</DoNotTranslate> 페이지](/docs/agents/java-agent/features/jvm-metrics-page), [트랜잭션](/docs/apm/transactions) 및 [오류](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) 세부정보, Java 스레드를 샘플링하고 스택 추적을 보고하는 [스레드 프로파일러 도구](/docs/apm/applications-menu/events/thread-profiler-tool) [등](/docs/apm/applications-menu)이 포함되어 있습니다.
