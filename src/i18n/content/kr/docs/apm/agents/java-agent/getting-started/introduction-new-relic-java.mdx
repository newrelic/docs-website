...
[뉴렐릭 UI에서 앱의 성능을 보려면,](/docs/apm/applications-menu/monitoring/apm-overview-page) **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Summary**로 이동합니다. APM 사용자 인터페이스에는 전용 [**JVM 메트릭** 페이지](/docs/agents/java-agent/features/jvm-metrics-page) , [트랜잭션](/docs/apm/transactions) 및 [오류](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) 세부 정보, Java 스레드를 샘플링하고 스택 트레이스를 보고하는 [스레드 프로파일러 도구](/docs/apm/applications-menu/events/thread-profiler-tool) 등이 포함[되어 있습니다](/docs/apm/applications-menu).
