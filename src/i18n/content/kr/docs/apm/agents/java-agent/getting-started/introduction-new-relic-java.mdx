...
[뉴렐릭 UI에서 앱의 성능을 보려면](/docs/apm/applications-menu/monitoring/apm-overview-page) **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Summary**로 이동합니다.APM 사용자 인터페이스에는 전용 [**JVM 메트릭** 페이지](/docs/agents/java-agent/features/jvm-metrics-page), [트랜잭션](/docs/apm/transactions) 및 [오류](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) 세부 정보, Java 스레드를 샘플링하고 스택 트레이스를 보고하는 [스레드 프로파일러 툴](/docs/apm/applications-menu/events/thread-profiler-tool) 등이 포함[됩니다](/docs/apm/applications-menu).

또한[에이전트 툴을 확장](#extend-instrumentation)하고 [데이터 탐색기](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer)와 [대시보드](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards)를 포함한 다른 뉴렐릭 제품으로 데이터를 탐색할 수 있습니다.
