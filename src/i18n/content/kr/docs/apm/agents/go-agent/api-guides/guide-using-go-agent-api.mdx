...
에이전트에 콜백 함수를 제공하여 발견된 각 오류에 원하는 [오류 그룹을](/docs/errors-inbox/errors-inbox) 동적으로 적용할 수 있습니다. Go 에이전트 구성 옵션 [`newrelic.ConfigSetErrorGroupCallbackFunction`](/src/content/docs/apm/agents/go-agent/configuration/go-agent-configuration#error-collector) 을 사용하여 에이전트에 콜백을 제공합니다.
