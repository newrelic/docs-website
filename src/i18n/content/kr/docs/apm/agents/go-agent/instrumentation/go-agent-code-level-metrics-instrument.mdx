...
기본적으로 Go 에이전트에는 계측 중인 에이전트에 도달하기 위해 에이전트 자체 내부에 있는 호출 스택의 기능을 건너뛰는 논리가 포함되어 있습니다. [구성 가이드](/docs/apm/agents/go-agent/configuration/go-agent-code-level-metrics-config) 에는 `ConfigCodeLevelMetricsIgnorePrefix` 설정을 통해 이 휴리스틱을 전역적으로 변경하는 방법에 대한 정보가 있지만 단일 트랜잭션에 대해 맞춤 네임스페이스 무시 접두사를 제공할 수 있습니다. 트랜잭션에 `WithIgnorePrefix` 옵션을 추가하면 됩니다.
...
이 변경으로 이름이 `github.com/ignore/this/` 으로 시작하는 패키지의 모든 함수는 계측 중인 함수를 찾기 위해 건너뜁니다.
