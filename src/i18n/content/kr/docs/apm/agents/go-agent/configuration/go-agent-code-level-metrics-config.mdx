...
코드 레벨 지표를 사용하면 [뉴렐릭 CodeStream 확장 프로그램을](/docs/codestream/observability/code-level-metrics) 사용하는 개발자가 코드의 개별 메서드와 함께 IDE에 상황에 맞게 표시되는 <InlinePopover type="apm"/>데이터를 볼 수 있습니다. 이를 통해 개발자는 코드를 작성하고 검토할 때 성능 문제를 보다 적극적으로 해결할 수 있습니다.
