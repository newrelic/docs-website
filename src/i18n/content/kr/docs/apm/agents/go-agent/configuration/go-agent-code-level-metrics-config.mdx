...
코드 수준 지표를 통해 [New Relic CodeStream 확장을](/docs/codestream/observability/code-level-metrics) 사용하는 개발자는 <InlinePopover type="apm"/>코드의 개별 메서드와 함께 IDE에 상황에 따라 데이터가 표시됩니다. 이를 통해 개발자는 코드를 작성하고 검토할 때 성능 문제를 보다 적극적으로 해결할 수 있습니다.
