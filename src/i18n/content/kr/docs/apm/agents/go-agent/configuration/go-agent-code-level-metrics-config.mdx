...
<CollapserGroup>
  ...
  >
    ...
    * `NewApplication` 함수를 통해 구성하는 경우 `ConfigCodeLevelMetricsIgnoredPrefix` 옵션을 추가하여 원하는 수의 접두사 문자열을 별도의 문자열 인수로 전달합니다.
    ...
    * 환경 변수를 통해 구성하는 경우 `NEW_RELIC_CODE_LEVEL_METRICS_IGNORED_PREFIX` 을 원하는 접두사(또는 쉼표로 구분된 접두사 목록)로 설정합니다.
