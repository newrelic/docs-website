...
[Go 에이전트를](/docs/agents/go-agent/get-started/introduction-new-relic-go) 사용하면 [Google App Engine(GAE) 가변형 환경](https://cloud.google.com/appengine/docs/flexible/go/) 에 있는 애플리케이션을 모니터링할 수 있습니다. <DNT>GAE</DNT> Flex 앱에 뉴렐릭을 추가하면 인사이트 앱의 성능과 성능에 대한 정보를 얻을 수 있으며 [APM](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm) 및 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)에서 볼 수 있는 지표로 <DNT>GAE</DNT> 이(가) 확장됩니다.

여기에서는 [맞춤 런타임을](https://cloud.google.com/appengine/docs/flexible/custom-runtimes/about-custom-runtimes) 구성하여 <DNT>GAE</DNT> flex 앱에 뉴렐릭을 추가하는 방법을 설명하고 docker 사용하여 Go 앱을 구현하다, 배포하다의 예를 제공합니다.

<Callout variant="important">
  Go 에이전트는 커스텀 런타임을 사용하여 GAE 가변형 환경에서 실행될 수 있습니다. 다른 환경의 제한으로 인해 GAE 표준 환경이나 Google App Engine의 ['네이티브 모드'](/docs/accounts-partnerships/partnerships/google-cloud-platform-gcp/google-app-engine-environment#native-mode) 설치를 사용하지 마세요.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    3. 뉴렐릭에서 GAE Flex 앱 데이터를 보려면 [APM <DNT>**Summary**</DNT> 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 로 이동하세요.
  </Collapser>
</CollapserGroup>
...
권장사항: 다음을 추가하여 상태 확인을 비활성화하도록 `app.yaml` 구성하세요.
