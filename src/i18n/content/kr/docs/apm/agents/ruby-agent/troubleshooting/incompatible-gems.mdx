...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="마카라"
  >
    **문제:** [makara](https://github.com/instacart/makara) gem은 기본 연결의 이름을 수정하여 기본 ActiveRecord를 기반으로 하는 데이터베이스 어댑터를 감지하는 Ruby 에이전트의 기능을 마스킹합니다. 쿼리를 적절하게 난독화하려면 원래 어댑터 이름이 필요합니다. 일부 트랜잭션 추적은 MySQL 콘텐츠 대신 ActiveRecord 콘텐츠를 표시합니다.

    **솔루션:** 알려진 해결 방법이 없습니다. `makara` gem을 제거하거나 계속 사용하세요. 과도한 난독화는 예상되는 동작입니다.
  </Collapser>
</CollapserGroup>