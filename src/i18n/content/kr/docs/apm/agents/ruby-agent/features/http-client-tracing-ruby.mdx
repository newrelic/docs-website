...
Ruby 에이전트는 Excon 버전 **0.56.0 이상을 지원합니다.** Excon 계측은 `Excon.defaults` 의 `:middlewares` 키에 Excon 미들웨어를 추가하는 기능에 의존하므로 애플리케이션이 `Excon.defaults` 를 수정하는 경우 `:middlewares` 키의 값을 유지해야 합니다.
