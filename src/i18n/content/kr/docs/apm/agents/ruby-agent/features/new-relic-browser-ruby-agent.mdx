...
Ruby 에이전트를 사용하면 자동 또는 수동으로 웹 페이지에 [브라우저 모니터링 에이전트](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser) 계측을 추가할 수 있습니다. 사용하려면 <InlinePopover type="browser"/>UI에서 절차에 따라 [브라우저 에이전트를 설치합니다](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent). 그런 다음 이 섹션의 절차에 따라 Ruby 에이전트를 설정하세요.
