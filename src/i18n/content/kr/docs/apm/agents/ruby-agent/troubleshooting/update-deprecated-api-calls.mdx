...
New Relic Ruby 에이전트 버전 [4.0.0](/docs/releases/ruby) 에서는 한동안 사용되지 않던 여러 API가 삭제되었습니다. 방법 중 하나를 사용하는 경우 Ruby 에이전트 버전 4.0.0 이상으로 업그레이드하기 전에 API를 권장 대체품으로 업데이트해야 합니다.
