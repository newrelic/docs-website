...
이는 속성을 활성화 또는 비활성화하는 속성과 New Relic이 [대상](/docs/agents/manage-apm-agents/agent-data/agent-attributes#destinations) 에 포함하거나 제외할 속성을 결정할 때 사용하는 규칙을 설명합니다. 여기에는 New Relic 에이전트 속성 출시로 더 이상 사용되지 않거나 제거된 Ruby 에이전트 속성에 대한 요약도 포함됩니다.
