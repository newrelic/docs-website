...
가비지 컬렉션(GC)은 사용되지 않는 데이터 개체를 찾아 다른 프로세스에서 사용할 수 있도록 해당 메모리 공간을 회수합니다. New Relic Ruby 에이전트는 CRuby 1.9.2 이상 또는 Ruby Enterprise Edition에서 실행되는 애플리케이션의 가비지 수집에 소요된 시간에 대한 정보를 수집할 수 있지만 애플리케이션에서 이 기능을 명시적으로 활성화해야 합니다.
