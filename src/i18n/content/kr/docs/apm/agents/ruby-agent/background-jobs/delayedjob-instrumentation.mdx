...
작업이 모니터링되지 않는 것으로 나타나면 작업자가 시작될 때 생성된 <DoNotTranslate>**newrelic_agent.log**</DoNotTranslate> 파일을 검토하세요. 에이전트가 `Delayed` 감지하고 서버와 통신하는지 여부를 나타내야 합니다. 로그를 찾을 수 없거나 작업이 표시되지 않는 이유를 여전히 확인할 수 없는 경우 [support.newrelic.com](https://support.newrelic.com "링크가 새 창에서 열립니다.") 에서 지원을 받으십시오.