...
또한 이 기능을 사용하려면 CRuby **1.9.2 이상** 과 호환되는 Ruby 버전이 있어야 합니다. 각 개별 측정에 대한 아래 섹션에서는 각 측정의 수집을 지원하는 Ruby 버전에 대해 설명합니다.
...
모든 Ruby 버전에서 모든 메트릭을 수집할 수 있는 것은 아닙니다. 일반적으로 최신 버전의 CRuby를 사용 중인 경우 가장 완전한 데이터를 얻을 수 있습니다. 아래 목록은 다음 위치에서 사용할 수 있는 항목을 정확히 설명합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="가비지 수집기 실행 빈도"
  >
    **사용 가능** : CRuby 1.9.2+, JRuby 1.7+, Rubinius 2.x

    얼마나 자주 가비지 수집기가 Ruby 프로세스를 실행하기 위해 중지해야 합니까? CRuby 2.1+에서 이것은 메이저 및 마이너 GC 실행으로 분할됩니다. 이 숫자는 GC 실행당 처리된 요청 수로 UI에 표시됩니다.
  ...
  >
    **사용 가능** : CRuby 2.1+
    ...
    CRuby 2.1 이상에서 전역 캐시는 더 작은 클래스별 캐시 세트로 분할되어 한 클래스에 대한 캐시 무효화 변경이 관련되지 않은 다른 클래스에 대한 캐시 항목을 버리지 않습니다.
