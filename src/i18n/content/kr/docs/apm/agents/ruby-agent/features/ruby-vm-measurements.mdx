...
Ruby VM의 성능에 대한 데이터를 봅니다. [New Relic에 로그인하고](https://one.newrelic.com) **APM** 을 클릭한 다음 앱을 선택하고 **Monitoring > Ruby VMs** 으로 이동합니다. 이 페이지는 애플리케이션이 Ruby 에이전트 버전 3.8.0 이상을 사용하는 경우에만 나타납니다.
