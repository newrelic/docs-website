...
<Callout variant="caution">
  개발자 모드는 <DNT>**deprecated**</DNT> 이며 [루비 에이전트 버전 4.1.0](/docs/release-notes/agent-release-notes/ruby-release-notes/ruby-agent-410333) 부터 더 이상 지원되지 않습니다.
</Callout>
...
개발자 모드를 설치하려면 [루비 에이전트를 플러그인 또는 gem으로 설치하세요](/docs/ruby/ruby-agent-installation) . 작업 중인 `RAILS_ENV` 에 대한 <DNT>**newrelic.yml**</DNT> 에서 [`developer_mode`](/docs/agents/ruby-agent/installation-configuration/ruby-agent-configuration#developer_mode) true로 설정해야 할 수도 있습니다. 개발자 모드는 `RAILS_ENV` = `development` 인 경우에만 기본적으로 켜져 있습니다.

<DNT>**newrelic.yml**</DNT> 파일은 설치의 일부로 생성되지만 유효한 <InlinePopover type="licenseKey" />가질 필요는 없습니다.

## 알려진 문제 [#known\_issues][#known_issues]

개발자 모드의 알려진 문제는 다음과 같습니다.

<CollapserGroup>
  <Collapser id="rails-23" title="Rails 2.3 이상이 필요합니다.">
  ...
  <Collapser id="pow-compatibility" title="Pow와 호환되지 않음">
    개발자 모드는 Mac OS X용 제로 구성 랙 서버인 [Pow](http://pow.cx/) 에서 지원되지 않습니다.
  </Collapser>
</CollapserGroup>

## 개발자 모드 사용 [#using\_devmode][#using_devmode]

Thin, WEBrick 또는 이와 유사한 것을 사용하여 데스크탑에서 애플리케이션을 실행하는 경우 서버에서 URL `/newrelic` 을 열어 개발자 모드 사용자 인터페이스를 확인하십시오.

<Callout variant="caution">
  데스크탑에서 Passenger, Unicorn 또는 기타 분기형 서버를 사용하는 경우, 서버를 여러 개 장착하는 경우 데이터가 사라지는 이상한 동작을 발견할 수 있습니다. 다음에 나오는 <DNT>**Troubleshooting**</DNT> 도움말을 참조하세요.
</Callout>
...
<CollapserGroup>
  <Collapser id="disappearing-request" title="사라지는 요청">
    Unicorn 또는 기타 다중 작업자 디스패처에 이 문제가 있는 경우 작업자 수를 1로 제한해 보십시오.
  </Collapser>

  <Collapser id="undefined-method-rack-request" title="Rack::Request에 대한 정의되지 않은 메서드 경로">
  ...
  <Collapser id="activerecord-reloads" title="ActiveRecord 다시 로드">
