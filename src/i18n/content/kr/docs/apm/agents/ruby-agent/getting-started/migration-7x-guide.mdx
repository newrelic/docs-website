...
**잠재적인 문제:** `:trace_only` 옵션을 `NewRelic::Agent.notice_error` 에 전달하면 더 이상 작동하지 않습니다.

**솔루션:** `:trace_only` 을[`:expected`](/docs/agents/ruby-agent/api-guides/sending-handled-errors-new-relic/) 속성으로 바꿉니다.
