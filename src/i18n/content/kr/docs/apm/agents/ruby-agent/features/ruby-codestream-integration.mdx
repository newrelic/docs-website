...
[New Relic CodeStream](/docs/codestream/start-here/what-is-codestream) 을 Ruby 에이전트와 통합하여 상황에 맞는 APM 데이터를 IDE에 직접 표시합니다. 이 통합을 통해 코드를 작성하고 검토할 때 편집기에서 코드 수준 프로덕션 원격 분석을 시각화합니다.
...
<Callout variant="important">
  New Relic CodeStream 통합은 Ruby 에이전트 버전 8.8.0 이상에서 사용할 수 있으며 기본적으로 비활성화되어 있습니다. 통합을 활성화하려면 `newrelic.yml` }에서 `code_level_metrics.enabled = true` 을 설정하거나 `NEW_RELIC_APPLICATION_CODE_LEVEL_METRICS_ENABLED=true` 를 환경 변수로 설정합니다.
</Callout>
