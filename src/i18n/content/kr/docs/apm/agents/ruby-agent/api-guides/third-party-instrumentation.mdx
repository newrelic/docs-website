...
<Callout variant="tip">
  뉴렐릭은 <DNT>Ruby</DNT> 에이전트에 대한 커뮤니티 기여를 받습니다. 계측하고자 하는 gem이 있거나 제안할 기능이 있는 경우, GitHub의 `newrelic/newrelic-ruby-agent` 저장소에서 [이슈](https://github.com/newrelic/newrelic-ruby-agent/issues/new/choose)를 열거나 [pull 요청](https://github.com/newrelic/newrelic-ruby-agent)을 제출하여 아이디어를 공유해 주십시오.
</Callout>

## 타사 확장 찾기

누구나 <DNT>Ruby</DNT> 에이전트를 기반으로 하는 gem을 작성할 수 있습니다. 이러한 확장을 추적하고 <DNT>Ruby</DNT> 에이전트를 빌드하는 다른 gem에 대한 링크를 제공하기 위해 [extends\_newrelic\_rpm](https://github.com/newrelic/extends_newrelic_rpm) 이라는 보관된 저장소에서 이에 대한 이전 예제를 확인할 수 있습니다.

이러한 확장은 뉴렐릭에서 지원하지 **않습니다**. 뉴렐릭은 고객을 위한 서비스로 이러한 링크를 수집했습니다. 해당 gem과 관련된 이슈는 GitHub의 각 프로젝트에 보고해야 합니다.

## 보석으로서의 확장

타사 확장을 생성하기로 선택한 경우, 계측된 라이브러리당 하나의 gem을 권장합니다. 예를 들어, `newrelic-redis`은(는) `redis` gem(에이전트 내에도 자동으로 존재함)에 대한 계측을 제공합니다.
