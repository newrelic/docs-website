...
2. Sinatra 앱에서 Sinatra <DoNotTranslate>**require**</DoNotTranslate> 지시어 바로 아래에 다음을 추가하세요.

   ```
   require 'newrelic_rpm'
   ```

3. `RACK_ENV` <DoNotTranslate>**newrelic.yml**</DoNotTranslate> 파일의 설정 정의에 해당하는 환경으로 설정합니다. 예를 들어 개발, 스테이징, 생산 등이 있습니다.

<DoNotTranslate>**Note:**</DoNotTranslate> 개발자 모드는 Rails에서만 작동합니다.
