...
자체 코드에서 처리 중인 오류 데이터를 뉴렐릭으로 보내려면 오류 핸들러 내에서 <DNT>Ruby</DNT> 에이전트 API [`NewRelic::Agent.notice_error`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#notice_error-instance_method) 호출을 사용하세요.
...
`Proc` 기반 콜백을 에이전트에 제공하여 발견된 각 오류에 원하는 [오류 그룹을](/docs/errors-inbox/errors-inbox/#how-groups-work) 동적으로 적용할 수 있습니다. Ruby 에이전트 API [`NewRelic::Agent.set_error_group_callback`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#set_error_group_callback-instance_method) 를 사용하여 에이전트에 콜백을 제공하십시오.
...
트랜잭션 및 오류는 New Relic Ruby 에이전트에 알려진 경우 사용자 ID와 연관될 수 있습니다. Ruby 에이전트 API [`NewRelic::Agent.set_user_id`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#set_user_id-instance_method) 를 사용하여 에이전트에 사용자 ID를 제공하십시오.
