...
New Relic for Ruby를 사용하면 Rails, Roda 또는 Sinatra 애플리케이션 내의 특정 요청에 대한 계측을 선택적으로 비활성화할 수 있습니다.
...
`newrelic_ignore_enduser` 사용하면 에이전트가 캡처에 사용되는 자바스크립트를 자동으로 삽입하지 못하게 됩니다. [<InlinePopover type="browser"/>](/docs/agents/ruby-agent/features/page-load-timing-ruby)데이터. 서버측 계측은 영향을 받지 않습니다.
