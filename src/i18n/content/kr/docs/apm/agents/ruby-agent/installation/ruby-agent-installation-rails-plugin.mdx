...
에이전트를 설치한 후 `newrelic.yml` 파일을 애플리케이션의 `config` 하위 디렉토리에 복사합니다. New Relic에 로그인하면 **계정 설정** 에서 [라이선스 키](/docs/subscriptions/license-key) 가 포함된 새로운 `newrelic.yml` 를 다운로드할 수 있습니다.
