...
## New Relic Rails 플러그인 설치 [#Installing\_the\_Plugin][#Installing_the_Plugin]
...
## 구성 파일 업데이트 [#Configuration\_file][#Configuration_file]

에이전트를 설치한 후 `newrelic.yml` 파일을 애플리케이션의 `config` 하위 디렉터리에 복사합니다. 뉴렐릭에 로그인할 때 <DNT>**Account settings**</DNT> 에서 <InlinePopover type="licenseKey" />이 포함된 새로운 `newrelic.yml` 를 다운로드할 수 있습니다.

<Callout variant="important">
  설치 프로세스의 일부로 기본 [응용 프로그램 이름](/docs/site/naming-your-application) 을 의미 있는 이름으로 변경합니다.
</Callout>

## newrelic.yml 파일 업데이트 [#Updating\_newrelicyml][#Updating_newrelicyml]
...
## Ruby 에이전트 업데이트 [#Update\_the\_Agent][#Update_the_Agent]
...
## Rails 플러그인 제거 [#Uninstalling\_Plugin][#Uninstalling_Plugin]
