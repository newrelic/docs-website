...
****Capistrano가 원격 빌드 머신에서 배포 알림 레시피를 실행하는 경우 빌드 머신에 [라이센스 키가](/docs/apis/intro-apis/new-relic-api-keys/#license-key)있어야 합니다 . 유효한 `newrelic.yml` 파일을 빌드 시스템에 복사하거나(Capistrano [Before Hook](https://capistranorb.com/documentation/getting-started/before-after/) 사용 가능) Capistrano 구성에서 `set :newrelic_license_key, 'YOUR_LICENSE_KEY'` 를 호출할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        사용할 New Relic [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 입니다. 기본적으로 이것은 지정된 `rails_env` 에 대한 `newrelic.yml` 파일의 정의에서 가져옵니다. 이는 [REST API 키](/docs/apis/getting-started/intro-apis/access-rest-api-keys) 와 동일 **하지 않습니다** .
      </td>
    </tr>
