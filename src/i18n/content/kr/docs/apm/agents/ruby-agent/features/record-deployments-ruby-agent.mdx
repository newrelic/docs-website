...
<Callout variant="caution">
  기록 구현에 대한 지원, `newrelic deployments` 명령을 사용한 배포는 더 이상 사용되지 않으며 에이전트 버전 10.0.0에서 제거될 예정입니다. 사용자는 뉴렐릭의 API를 통해 직접 녹음 구현, 배포로 마이그레이션해야 합니다.

  자세한 내용은 [변경 추적 설명서를](/docs/change-tracking/change-tracking-introduction/) 참조하세요.
</Callout>
