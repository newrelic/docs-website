...
<CollapserGroup>
  ...
  >
    백그라운드 앱이 이미 Ruby 에이전트를 실행 중인 비 Rails 애플리케이션인 경우 **newrelic.yml** 파일을 백그라운드 작업을 시작하는 디렉터리 또는 **config** 하위 디렉터리에 복사합니다. <InlinePopover type="licenseKey" /> 가 포함되어 있는지 확인하십시오.
