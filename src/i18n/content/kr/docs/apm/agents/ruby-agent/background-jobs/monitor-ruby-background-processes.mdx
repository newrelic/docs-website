...
그런 다음 [`manual_start`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic%2FAgent%3Amanual_start) 을 호출하고 `:sync_startup => true` 옵션을 전달합니다.
...
프로세스가 에이전트 [수집 주기](/docs/using-new-relic/welcome-new-relic/get-started/glossary#harvest-cycle) 보다 짧은 경우 대기 중인 모든 데이터가 전송되도록 [`::NewRelic::Agent.shutdown`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent:shutdown) 을 사용하여 에이전트를 수동으로 종료해야 합니다.
