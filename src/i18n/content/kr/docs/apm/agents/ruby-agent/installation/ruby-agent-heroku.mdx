...
[Heroku](https://devcenter.heroku.com/articles/newrelic) 는 루비를 포함한 다양한 에이전트 언어로 웹 제작을 호스팅하기 위한 PaaS(Platform as a Service) 솔루션입니다. 뉴렐릭을 Heroku 사용하면 [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm)및 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser)지표를 사용하여 확장할 수 있습니다. 이 페이지에서는 뉴렐릭용 루비 에이전트와 함께 Heroku 호스팅 서비스로 사용하기 위한 특별 고려 사항을 설명합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="비 레일 애플리케이션"
  >
    비 Rails 애플리케이션과 함께 New Relic용 Ruby 에이전트를 사용하는 경우 Heroku 사용자는 리포지토리에 플러그인을 수동으로 설치해야 합니다. 예를 들어 Sinatra 앱에서 `newrelic_rpm` gem을 `Gemfile` 에 추가한 후 앱에 다음 코드를 추가합니다.
...
`newrelic.yml` 파일에서 New Relic을 구성하거나 [환경 변수를](/docs/agents/ruby-agent/installation-configuration/ruby-agent-configuration) 사용하여 구성 파일의 값보다 우선할 수 있습니다. `heroku config:set` ) 사용하여 Heroku 애플리케이션에 대한 에이전트 설정을 수정합니다.

예를 들어 [`custom_insights_events.max_samples_stored`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#custom_insights_events-max_samples_stored) 설정을 500으로 설정하려면:
...
동적으로 생성된 다른 dyno 유형이 있는 경우 [`heroku.dyno_name_prefixes_to_shorten`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#heroku) 구성 설정을 사용하여 이러한 다른 dyno 유형에 동일한 집계를 적용합니다.
...
4. [`log_level`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#log_level) 을 `debug` 로 변경하고 파일을 저장합니다. 들여쓰기를 수정하지 마십시오.
...
9. `newrelic.yml` (를) 다시 편집하고 [`log_level`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#log_level) 을 이전 설정으로 변경합니다. 파일을 저장합니다.
