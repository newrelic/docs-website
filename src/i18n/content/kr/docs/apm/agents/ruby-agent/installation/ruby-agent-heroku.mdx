...
[Heroku](https://devcenter.heroku.com/articles/newrelic) 는 Ruby를 포함한 다양한 에이전트 언어로 웹 애플리케이션을 호스팅하기 위한 PaaS(Platform as a Service) 솔루션입니다. New Relic을 사용하면 다음을 사용하여 Heroku를 확장할 수 있습니다. [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm)그리고 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser)측정항목. 이 페이지에서는 New Relic용 Ruby 에이전트와 함께 Heroku를 호스팅 서비스로 사용하기 위한 특별 고려 사항을 설명합니다.
