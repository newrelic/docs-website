...
* **auto:** 자동 Ruby 메서드 동작 선택을 통해 활성화되는 기본값
* **비활성화됨:** 계측 비활성화됨
* **prepend:** Ruby 메서드를 사용하도록 강제합니다.
* **chain:** Ruby 메소드 체이닝을 강제로 사용
...
수집된 gRPC 데이터는 여러 페이지의 New Relic UI에서 확인할 수 있습니다. 예를 들어 [요약 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 에서 gRPC 클라이언트 요청은 웹 트랜잭션으로 나타나고 **웹 트랜잭션 시간** , **처리량** 및 **오류율** 차트에 데이터를 제공합니다.

gRPC 데이터는 [분산 추적 페이지](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent/) 에서도 사용할 수 있습니다.
...
6. 목록에서 트레이스를 선택하고 클릭합니다.

7. **모두 확장 을** 클릭합니다.

8. 추적 범위의 결과 목록에서 개별 추적 범위를 클릭합니다.

9. 오른쪽 창에서 **속성** 탭을 클릭합니다.

   * **클라이언트 인스턴스:** `grpc_status` , `grpc_type` , `http.method` 및 `http.url` 속성이 모두 표시되어야 합니다. 클라이언트 요청이 실패한 경우( `0` 이외의 `grpc_status` 값을 가짐) 추가 `grpc_message` 속성이 gRPC 상태 메시지와 함께 나타납니다.
   * **서버 인스턴스:** `request.grpc_type` , `request.method` 및 `request.uri` 속성이 모두 표시되어야 합니다.
...
gRPC 기반 Ruby 예외가 발생하면 예외에 대한 정보가 **Events > Errors** 페이지에 나타납니다.
