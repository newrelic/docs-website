...
8. 오른쪽 창에서
   ...
   * <DNT>**Client instances:**</DNT>
     ...
     `grpc_status`, `grpc_type`, `http.method` 및 `http.url` 속성이 모두 표시되어야 합니다. 클라이언트 요청이 실패하는 경우( `0` 이외의 `grpc_status` 값을 가짐) 추가 `grpc_message` 속성이 gRPC 상태 메시지와 함께 표시됩니다.

   * <DNT>**Server instances:**</DNT>

     `request.grpc_type`, `request.method` 및 `request.uri` 속성이 모두 표시되어야 합니다.
