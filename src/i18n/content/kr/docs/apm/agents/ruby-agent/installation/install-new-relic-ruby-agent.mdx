   ...
   <table>
     ...
     <tbody>
       ...
       <tr>
         ...
         <td>
           Roda, Sinatra 또는 Rails를 사용 **하지 않는** Rack 애플리케이션에서 자동 브라우저 애플리케이션 모니터링 및 교차 애플리케이션 추적을 사용하려면 에이전트에서 제공하는 추가 [Rack 미들웨어를](/docs/agents/ruby-agent/frameworks/rack-middlewares#manual) 수동으로 포함해야 합니다. New Relic gem을 목록에서 가능한 한 아래에 배치하여 gem이 초기화될 때 그 위에 있는 프레임워크가 계측되도록 합니다.
         </td>
       </tr>
     </tbody>
   </table>
