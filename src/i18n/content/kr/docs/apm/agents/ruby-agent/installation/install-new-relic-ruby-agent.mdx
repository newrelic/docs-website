...
<Callout variant="important">
  docker 컨테이너에서 에이전트를 사용하는 경우 에이전트 <DoNotTranslate>**within each**</DoNotTranslate> 컨테이너를 설치합니다.
</Callout>
   ...
   <table>
     ...
     <tbody>
       <tr>
         ...
         <td>
           * <DoNotTranslate>**Rails**</DoNotTranslate>: Rails는 자동으로 `Bundler.require` 호출하고 애플리케이션 시작 중에 `newrelic_rpm` 필요하도록 합니다.

           * <DoNotTranslate>**Roda, Sinatra**</DoNotTranslate>: Roda, Sinatra 또는 다른 프레임워크를 사용하는 경우 `require 'newrelic_rpm'` 수동으로 호출해야 합니다. 또한 무한 추적을 사용하는 경우 수동으로 `require 'newrelic/infinite_tracing'` 호출하세요.
