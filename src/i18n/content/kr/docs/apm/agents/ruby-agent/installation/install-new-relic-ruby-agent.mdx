...
## 보석 설치 [#Installing\_the\_Gem][#Installing_the_Gem]

<Callout variant="important">
  docker 컨테이너에서 에이전트를 사용하는 경우 에이전트 <DNT>**within each**</DNT> 컨테이너를 설치합니다.
</Callout>
   ...
   <table>
     ...
     <tbody>
       <tr>
         ...
         <td>
           * <DNT>**Rails**</DNT>: Rails는 자동으로 `Bundler.require` 호출하고 애플리케이션 시작 중에 `newrelic_rpm` 필요하도록 합니다.

           * <DNT>**Roda, Sinatra**</DNT>: Roda, Sinatra 또는 다른 프레임워크를 사용하는 경우 `require 'newrelic_rpm'` 수동으로 호출해야 합니다. 또한 무한 추적을 사용하는 경우 수동으로 `require 'newrelic/infinite_tracing'` 호출하세요.
       ...
       <tr>
         ...
         <td>
           Roda, Sinatra, Rails를 사용 **하지 않는** Rack 애플리케이션에서 자동 브라우저 애플리케이션 모니터링을 사용하려면 에이전트에서 제공하는 추가 [Rack 미들웨어를](/docs/agents/ruby-agent/frameworks/rack-middlewares#manual) 수동으로 포함해야 합니다. New Relic gem을 목록에서 가능한 한 낮은 위치에 배치하여 gem이 초기화될 때 그 위의 프레임워크가 계측될 수 있도록 합니다.
         </td>
       </tr>
     </tbody>
   </table>

## 구성 파일 설치 [#Configuration\_file][#Configuration_file]
...
3. <InlinePopover type="licenseKey" />포함하도록 `license_key` 값을 편집합니다.
...
## 프로덕션 환경 외부에 에이전트 설치 [#Outside\_production\_environments][#Outside_production_environments]
...
## Ruby 에이전트 gem 제거 [#Uninstalling\_Gem][#Uninstalling_Gem]
