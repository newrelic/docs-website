...
1. 아직 [만들지 않았다면 New Relic 계정](https://newrelic.com/signup) 을 만드십시오. 영원히 무료입니다. 또는 [로그인](https://one.newrelic.com/) 합니다.
2. [계정 드롭다운](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) 에서 **계정 설정** 을 선택합니다.
3. **계정 설정** 페이지에서 **깨끗한 구성 파일 다운로드** 섹션을 찾아 `newrelic.yml` 파일을 클릭합니다.
4. `newrelic.yml` 파일을 애플리케이션의 `config` 하위 디렉토리에 복사합니다.
5. [기본 응용 프로그램 이름을 의미 있는 이름으로 변경합니다](/docs/agents/manage-apm-agents/app-naming/name-your-application) .
