...
<CollapserGroup>
  ...
  <Collapser id="log_level" title="log_level">
    ...
    로그 메시지의 세부 정보 수준을 설정합니다. 상세 수준이 높아지는 순서대로 가능한 로그 레벨은 다음과 같습니다: `fatal`, `error`, `warn`, `info` 또는 `debug`.
  </Collapser>
  ...
  <Collapser id="active_support_custom_events_names" title="active_support_custom_events_names">
    ...
    구독하고 계측할 ActiveSupport 커스텀 대시보드 이름의 어레이입니다. 예: \[one.커스텀 대시보드, another.이벤트, a.third.이벤트]
  </Collapser>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    `true`인 경우, 에이전트는 ActiveRecord 메트릭, 스팬 및 트랜잭션 트레이스 세그먼트의 이름을 지정할 때 클래스 이름 대신 ActiveRecord 모델의 테이블 이름을 사용합니다. 이는 여러 모델이 데이터베이스 테이블을 공유할 때 카디널리티를 줄일 수 있습니다. 기본값은 `false`입니다.
  </Collapser>
  ...
  <Collapser id="capture_memcache_keys" title="capture_memcache_keys">
    ...
    `true`인 경우 에이전트는 트랜잭션 트레이스에서 memcache 키를 캡처합니다.
  </Collapser>
  ...
  <Collapser id="config_path" title="구성_경로">
    ...
    `newrelic.yml` 경로입니다. 정의되지 않은 경우 에이전트는 다음 디렉터리를 순서대로 확인합니다: `config/newrelic.yml` -&gt; `newrelic.yml` -&gt; `$HOME/.newrelic/newrelic.yml` -&gt; `$HOME/newrelic.yml`
  </Collapser>
  ...
  <Collapser id="exclude_newrelic_header" title="exclude_newrelic_header">
    ...
    `true`인 경우 아웃바운드 requests에 대한 `newrelic` 분산 추적 헤더를 억제합니다.
  </Collapser>
  ...
  <Collapser id="force_install_exit_handler" title="force_install_exit_handler">
    ...
    `true`인 경우, 에이전트가 다른 프레임워크 내에 내장된 서비스로 실행되는 Sinatra를 감지할 때와 같이 일반적으로 에이전트가 이를 건너뛰는 경우에도 에이전트가 종료 핸들러(종료하기 전에 캐시된 모든 데이터를 수집기로 보냄)를 설치하도록 강제합니다. Sinatra는 자체 `at_exit` 블록 내에서 전체 애플리케이션을 실행하므로, 에이전트는 충돌을 피하기 위해 기본적으로 자체 `at_exit` 핸들러를 건너뜁니다. 이 설정은 이를 재정의합니다. 참고: 이 설정이 효과를 발휘하려면 `send_data_on_exit`도 `true` 이어야 합니다.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    파일 이름을 제외한 에이전트 로그 파일의 경로를 정의합니다. 에이전트 로그를 표준 출력으로 보내려면 이를 `STDOUT`(으)로 설정하십시오.
  </Collapser>
  ...
  <Collapser id="thread_local_tracer_state" title="thread_local_tracer_state">
    ...
    `true`인 경우 트레이서 상태 스토리지는 스레드 로컬이고, 그렇지 않으면 파이버 로컬입니다.
  </Collapser>
  ...
  <Collapser id="simplify_action_cable_broadcast_metrics" title="simplify_action_cable_broadcast_metrics">
    ...
    `true`인 경우, Action Cable 브로드캐스트 호출에 대한 메트릭에는 특정 브로드캐스트에 대한 `broadcasting` 값이 포함되지 않습니다. `false`로 기본 설정됩니다. 기본값은 향후 주요 버전 릴리스에서 변경될 수 있습니다.
  </Collapser>
  ...
  <Collapser id="ignored_middleware_classes" title="ignored_middleware_classes">
    ...
    에이전트가 무시할 미들웨어 클래스 또는 모듈 이름 목록입니다. 예: \[&quot;Rack::Cors&quot;, &quot;ActionDispatch::Reloader&quot;]
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    ...
    `true`인 경우, [`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments) 에 도달하면 에이전트는 해당 트랜잭션에서 이후에 생성된 모든 세그먼트에 대한 배타적 시간 기록도 중지합니다. 이는 세그먼트가 많은 매우 오래 실행되는 트랜잭션의 메모리 사용량을 줄일 수 있지만, 세그먼트 한계에 도달하면 트랜잭션 내 세그먼트의 타이밍 데이터가 덜 정확해지는 단점이 있습니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-expected_status_codes" title="error_collector.expected_status_codes">
    ...
    쉼표로 구분된 HTTP 상태 코드 및/또는 상태 코드 범위 목록입니다(예: `404, 429, 500-599`). 오류에 상태 코드가 있는 경우, 일치하는 상태 코드와 관련된 오류는 예상된 것으로 처리됩니다.
  </Collapser>
  ...
  <Collapser id="error_collector-ignore_status_codes" title="error_collector.ignore_status_codes">
    ...
    쉼표로 구분된 HTTP 상태 코드 및/또는 상태 코드 범위 목록입니다(예: `404, 429, 500-599`). 오류에 상태 코드가 있는 경우, 일치하는 상태 코드와 연결된 오류는 무시됩니다.
  </Collapser>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    오류 백트레이스의 최대 프레임 수를 정의합니다. 이 양을 초과하는 백트레이스는 기본적으로 중간에서 잘리며 스택 추적의 시작과 끝을 보존합니다. 잘림 위치를 사용자 지정하려면 [`error_collector.backtrace_truncate_location`](#error_collector-backtrace_truncate_location)을(를) 참조하십시오.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    프레임 수가 [`error_collector.max_backtrace_frames`](#error_collector-max_backtrace_frames)을(를) 초과할 때 백트레이스에서 자를 위치를 지정합니다. 옵션은 `top` (처음부터 프레임 제거), `middle` (처음과 끝을 보존하면서 중간에서 프레임 제거) 또는 `end` (끝에서 프레임 제거)입니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser_monitoring-version" title="browser_monitoring.version">
    ...
    뉴렐릭이 삽입하는 브라우저 에이전트 로더의 버전을 `1.317.0`과(와) 같이 고정하십시오. 설정하지 않은 경우 뉴렐릭은 사용 가능한 최신 버전을 삽입합니다. 현재 사용 가능하고 지원되는 버전을 확인하려면 [브라우저 에이전트 EOL 정책](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/)을 참조하십시오.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="application_logging-forwarding-log_level" title="application_logging.forwarding.log_level">
    ...
    로그 이벤트가 뉴렐릭으로 전달되기 위해 가져야 하는 최소 레벨을 설정합니다. 구성된 레벨 및 더 높은 심각도의 로그가 전달됩니다. 예를 들어 `debug`은(는) 모든 로그 이벤트를 전달하는 반면, `error`은(는) `error`, `fatal` 및 `unknown` 이벤트만 전달합니다. 유효한 값, 가장 낮은 심각도부터 가장 높은 심각도 순: `debug`, `info`, `warn`, `error`, `fatal`, `unknown` (대소문자 구분 안 함).

    이 순서는 루비의 `Logger::Severity` 상수의 정수 값을 기반으로 합니다([https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb](https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb) 참조).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="ai_monitoring-record_content-enabled" title="ai_monitoring.record_content.enabled">
    ...
    `false`인 경우, LLM 계측(현재는 OpenAI만 해당)은 LlmChatCompletionMessage 이벤트의 `content` 속성이나 LlmEmbedding 이벤트의 `input` 속성을 캡처하지 않습니다. 이는 LLM과 송수신되는 민감한 데이터가 기록되는 것을 방지하기 위한 선택적 보안 설정입니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="custom_insights_events-max_samples_stored" title="custom_insights_events.max_samples_stored">
    ...
    한 번에 메모리에 버퍼링할 커스텀 대시보드의 최대 수를 지정하십시오. [AI 모니터링](/docs/ai-monitoring/intro-to-ai-monitoring)을 위해 에이전트를 구성할 때 최대값을 `100000`으로 설정합니다. 이를 통해 에이전트가 최대 LLM 이벤트 수를 캡처하도록 보장합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="datastore_tracer-database_name_reporting-enabled" title="datastore_tracer.database_name_reporting.enabled">
    ...
    `false`인 경우, 에이전트는 트랜잭션 또는 느린 SQL 트레이스에 `database_name` 파라미터를 추가하지 않습니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="disable_active_job_step_names" title="disable_active_job_step_names">
    ...
    `true`인 경우, Rails Active Job Continuations의 단계 이름이 메트릭 이름에 포함되지 않습니다. 이는 메트릭 카디널리티를 감소시키지만 개별 단계 성능에 대한 가시성도 감소시킵니다. 단계가 많은 작업이나 동적으로 이름이 지정된 단계로 인해 메트릭 카디널리티 문제가 발생하는 경우에만 이를 `true` (으)로 설정하십시오.
  </Collapser>
  ...
  <Collapser id="disable_active_record_notifications" title="disable_active_record_notifications">
    ...
    `true`인 경우 Active Record 4+에 대한 계측이 비활성화됩니다.
  </Collapser>
  ...
  <Collapser id="disable_gc_profiler" title="disable_gc_profiler">
    ...
    `true`인 경우, 가비지 컬렉션에 소요된 시간을 측정하기 위한 `GC::Profiler`의 사용을 비활성화합니다.
  </Collapser>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    `true` 인 경우 Sinatra용 에이전트 미들웨어를 비활성화합니다. 이 미들웨어는 [분산 추적](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing), [페이지 로드 시간 측정](/docs/browser/new-relic-browser/getting-started/new-relic-browser), [오류 수집](/docs/apm/applications-menu/events/view-apm-error-analytics) 과 같은 고급 기능 지원을 담당합니다.
  </Collapser>
  ...
  <Collapser id="disable_puma_instrumentation" title="disable_puma_instrumentation">
    ...
    `true`인 경우, Puma 마스터 프로세스에서 `Ruby/Puma/*` 메트릭을 샘플링하는 Puma 서버 통계 측정을 비활성화합니다. 이 측정은 기본적으로 비활성화되어 있습니다. 활성화되면 에이전트는 이러한 메트릭을 전달하기 위해 Puma 마스터 프로세스에서 보고 스레드를 시작합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-root" title="분산 추적.샘플러.루트">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    이 설정은 원격 상위가 없는 트랜잭션(뉴렐릭 에이전트의 이 인스턴스 내에서 시작되는 트레이스)에 대한 트랜잭션 샘플링 동작을 제어합니다. 사용 가능한 값은 `adaptive` (기본값), `always_on`, `always_off` 및 `trace_id_ratio_based`입니다.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    이 설정은 원격 부모를 샘플링할 때 트랜잭션 샘플링의 동작을 제어합니다. 사용 가능한 값은 `adaptive` (기본값), `always_on`, `always_off`, `trace_id_ratio_based` 입니다.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    이 설정은 원격 부모가 샘플링되지 않을 때 트랜잭션 샘플링의 동작을 제어합니다. 사용 가능한 값은 `adaptive` (기본값), `always_on`, `always_off`, `trace_id_ratio_based` 입니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="측정, 계측.active_support_notifications.active_support_events">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    에이전트가 구독하는 ActiveSupport 공지 이벤트 이름(주로 캐싱 이벤트 및 메시지 직렬화기 이벤트)의 허용 목록입니다. 여기에 포함되지 않은 모든 이벤트 이름은 무시됩니다. `cache_fetch_hit.active_support`와(과) 같은 완전한 이벤트 이름만 제공하십시오. 별표나 정규식 패턴을 제공하지 마시고, 백슬래시로 문자를 이스케이프하지 마십시오.

    가능한 이벤트 이름의 전체 목록은 Rails 문서의 [캐싱 이름](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) 및 [메시지 이름](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) 목록을 참조하십시오.
  </Collapser>
  ...
  <Collapser id="instrumentation-semantic_logger" title="instrumentation.semantic_logger">
    ...
    시작 시 semantic\_logger 라이브러리의 자동 계측을 제어합니다. `auto`, `prepend`, `chain`, `disabled` 중 하나일 수 있습니다.
  </Collapser>

  <Collapser id="instrumentation-parallel" title="측정, 사용법.병렬">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    시작 시 병렬 라이브러리의 자동 측정, 자동 실행을 제어합니다. `auto`, `prepend`, `chain`, `disabled` 중 하나일 수 있습니다.
  </Collapser>
  ...
  <Collapser id="instrumentation-grpc-host_denylist" title="instrumentation.grpc.host_denylist">
    ...
    뉴렐릭이 트래픽을 무시해야 하는 gRPC 호스트명과 일치하는 정규식 패턴의 쉼표로 구분된 목록입니다. 뉴렐릭의 gRPC 클라이언트 계측은 이러한 패턴 중 하나와 일치하는 호스트에 대한 호출 추적을 건너뛰고, gRPC 서버 계측은 일치하는 호스트에서 실행 중인 서버가 수신한 requests 추적을 건너뜁니다. 예: `"private.com$,exception.*" Empty by default, meaning no traffic is ignored.`.
  </Collapser>
  ...
  <Collapser id="instrumentation-logging" title="측정, 사용량.logging">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    시작 시 로깅 라이브러리의 자동 측정, 자동 트랜잭션을 제어합니다. `auto`, `prepend`, `chain`, `disabled` 중 하나일 수 있습니다.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger" title="instrumentation.rails_event_logger">
    ...
    Rails.event의 계측을 구조화된 로그로 제어합니다. 다음 중 하나일 수 있습니다: `enabled`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    구조화된 로그 이벤트로 캡처할 `Rails.event` 이름의 어레이입니다. 예: \[user.signup, payment.processed, order.created]. 모든 `Rails.event` 공지를 캡처하려면 비워 두십시오. 이벤트 페이로드의 `:level` 키를 통해 재정의되지 않는 한 이벤트는 레벨 `UNKNOWN` (필터링되지 않도록 보장)로 로그됩니다.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="OpenTelemetry.활성화됨">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    뉴렐릭을 통해 전송되는 모든 OpenTelemetry 신호를 비활성화하기 위한 전역 설정 옵션입니다. `false`인 경우, 뉴렐릭으로 OpenTelemetry 신호가 전송되지 않습니다. `true`인 경우, 신호별 활성화 설정 옵션(예: opentelemetry.traces.enabled)이(가) 해당 신호 유형의 텔레메트리가 뉴렐릭에 보고될지 여부를 결정합니다.
  </Collapser>

  <Collapser id="opentelemetry-traces-enabled" title="OpenTelemetry.트레이스.enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetry Spans에서 칩셋 트레이스 세그먼트 및 타임슬라이스 메트릭을 생성할 수 있습니다. 이는 OpenTelemetry 스팬에 대한 뉴럴릭 UI 경험을 개선하는 데 도움이 될 것입니다.
  </Collapser>

  <Collapser id="opentelemetry-traces-include" title="OpenTelemetry.트레이스.include">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetry 트레이서의 쉼표로 구분된 목록(문자열 형식, 예:) &quot;AppTracer1,OpenTelemetry:: 측정, 리소스::Bunny::측정, 리소스&quot;), **해당** 트레이스가 뉴렐릭으로 전송됩니다.
  </Collapser>

  <Collapser id="opentelemetry-traces-exclude" title="OpenTelemetry.트레이스.exclude">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetry 트레이서의 쉼표로 구분된 목록(문자열 형식, 예:) &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;), 해당 트레이스가 뉴렐릭으로 전송되지 **않습니다**.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="process_host-display_name" title="process_host.display_name">
    ...
    [뉴렐릭 UI에 표시할](/docs/apm/new-relic-apm/maintenance/add-rename-remove-hosts#display_name) 커스텀 호스트명을 지정합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="puma-sample_rate" title="puma.sample_rate">
    ...
    Puma 마스터 프로세스에서 수집된 Puma 서버 통계 샘플 간의 시간(초)입니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rake-tasks" title="rake.tasks">
    ...
    자동으로 계측할 Rake 작업의 어레이를 지정하십시오. 이 설정 옵션은 어레이를 정규식 목록으로 변환합니다. 이 목록에 추가되지 않으면 어떠한 rake 작업도 계측되지 않습니다. 기본적으로 모든 작업을 허용하려면 `rake.tasks: [.+]`을(를) 사용하십시오. 자세한 내용은 [뉴렐릭 Rake 계측 문서를](/docs/apm/agents/ruby-agent/background-jobs/rake-instrumentation) 참조하십시오.
  </Collapser>
  ...
  <Collapser id="rake-connect_timeout" title="rake.connect_timeout">
    ...
    Rake 작업을 실행하기 전에 연결이 완료될 때까지 대기할 시간(초)을 정의합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rules-ignore_url_regexes" title="rules.ignore_url_regexes">
    ...
    제외할 URI와 일치하는 패턴 목록을 지정하여 에이전트가 무시할 트랜잭션을 정의하십시오. 자세한 내용은 [특정 트랜잭션 무시에 대한 문서](/docs/agents/ruby-agent/api-guides/ignoring-specific-transactions/#config-ignoring)를 참조하십시오.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="security-enabled" title="보안 활성화">
    ...
    `true`인 경우, 보안 에이전트가 시작됩니다(에이전트가 자체 이벤트 루프에서 실행됩니다).
  </Collapser>
  ...
  <Collapser id="security-mode" title="보안.모드">
    ...
    보안 에이전트가 작동할 모드를 정의합니다. 현재 `IAST`만 지원됩니다.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-insecure_settings" title="security.exclude_from_iast_scan.iast_detection_category.insecure_settings">
    ...
    `true`인 경우 심각도가 낮은 안전하지 않은 설정의 감지를 비활성화합니다. 예를 들어 해시, 암호화, 쿠키, 난수 생성기 및 신뢰 경계가 있습니다.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-nosql_injection" title="security.exclude_from_iast_scan.iast_detection_category.nosql_injection">
    ...
    `true`인 경우 IAST 스캔에서 NoSQL 주입 탐지를 비활성화합니다.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-xpath_injection" title="security.exclude_from_iast_scan.iast_detection_category.xpath_injection">
    ...
    `true`인 경우, IAST 스캔에서 XPath 주입 탐지를 비활성화합니다.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-ssrf" title="security.exclude_from_iast_scan.iast_detection_category.ssrf">
    ...
    `true`인 경우, IAST 스캔에서 서버 측 요청 위조(SSRF) 감지를 비활성화합니다.
  </Collapser>
  ...
  <Collapser id="security-scan_controllers-iast_scan_request_rate_limit" title="security.scan_controllers.iast_scan_request_rate_limit">
    ...
    IAST 스캔에 대해 분당 허용되는 최대 HTTP requests 수를 설정합니다. 12-3600(기본값) 사이의 정수여야 합니다.
  </Collapser>
  ...
  <Collapser id="security-iast_test_identifier" title="security.iast_test_identifier">
    ...
    연속 통합/연속 배포(CI/CD) 환경에서 IAST를 실행할 때 여러 테스트 실행을 구분하기 위한 고유한 테스트 식별자입니다. 예를 들어 빌드 번호입니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="sidekiq-args-include" title="sidekiq.args.include">
    ...
    어떤 Sidekiq 작업 인수가 뉴렐릭에 보고될지 필터링하기 위한 허용 목록 역할을 총괄적으로 수행할 문자열 어레이입니다. Sidekiq 인수를 캡처하려면 별도의 `attributes.include` 설정 옵션에도 `job.sidekiq.args.*`을(를) 추가해야 합니다. 고급 일치를 허용하기 위해 이 어레이의 각 문자열은 `Regexp.new`을(를) 통해 정규식으로 변환됩니다. 작업 인수 해시의 경우, 키 또는 값이 일치하면 해당 쌍이 포함됩니다. 일치하는 모든 작업 인수 어레이 요소 및 작업 인수 스칼라가 포함됩니다.
  </Collapser>
  ...
  <Collapser id="sidekiq-args-exclude" title="sidekiq.args.exclude">
    ...
    어떤 Sidekiq 작업 인수가 뉴렐릭에 보고될지 필터링하기 위한 거부 목록 역할을 총괄적으로 수행하는 문자열 어레이입니다. Sidekiq 인수를 캡처하려면 별도의 `attributes.include` 설정 옵션에도 `job.sidekiq.args.*`을(를) 추가해야 합니다. 고급 일치를 허용하기 위해 이 어레이의 각 문자열은 `Regexp.new`을(를) 통해 정규식으로 변환됩니다. 작업 인수 해시의 경우 키 또는 값이 일치하면 해당 쌍이 제외됩니다. 일치하는 모든 작업 인수 어레이 요소 및 작업 인수 스칼라가 제외됩니다.
  </Collapser>
  ...
  <Collapser id="sidekiq-separate_transactions" title="sidekiq.separate_transactions">
    ...
    `true`인 경우, 웹 트랜잭션 중에 Sidekiq 작업이 실행될 때 에이전트가 새 트랜잭션을 시작합니다. 이를 통해 Sidekiq 작업 실행 시간이 웹 트랜잭션 메트릭에 포함되는 것을 방지합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="slow_sql-use_longer_sql_id" title="slow_sql.use_longer_sql_id">
    ...
    `true`인 경우, 에이전트는 느린 SQL 트레이스에 대해 더 긴 `sql_id`을(를) 생성합니다. `sql_id`은(는) 유사한 쿼리를 집계하는 데 사용됩니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="span_events-max_samples_stored" title="span_events.max_samples_stored">
    ...
    단일 하베스트에서 보고되는 스팬 이벤트의 최대 수를 정의합니다. `1`에서 `10000` 사이의 모든 정수가 유효합니다. [AI 모니터링](/docs/ai-monitoring/intro-to-ai-monitoring)을 위해 에이전트를 구성할 때 최대값을 `10000`로 설정합니다. 이는 에이전트가 최대량의 분산 트레이스를 캡처하도록 보장합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="strip_exception_messages-enabled" title="strip_exception_messages.enabled">
    ...
    `true`인 경우 에이전트는 [허용된 클래스 목록](#strip_exception_messages-allowed_classes)에 있는 예외를 제외한 모든 예외에서 메시지를 제거합니다. [high security mode](/docs/accounts-partnerships/accounts/security/high-security) \[높은 수준의 보안 모드에서] 자동으로 활성화됩니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    `true`인 경우, Google Cloud Run(Service, Worker Pool, Job)에서 실행될 때 에이전트는 GCP 인스턴스 ID를 호스트명으로 보고합니다.
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    `true`인 경우 에이전트는 Google Cloud Run에서 GCP 인스턴스 ID 앞에 Cloud Run 리비전 이름을 추가하여 호스트명(`{revision}-{instance id}`)을 구성합니다. 리비전 이름은 Cloud Run Service의 `K_REVISION`, Cloud Run Worker Pool의 `CLOUD_RUN_REVISION`, Cloud Run Job의 `CLOUD_RUN_EXECUTION`에서 가져옵니다. `utilization.gcp_cloud_run.use_instance_as_host`도 `true`가 아니라면, 아무런 영향을 미치지 않습니다.
  </Collapser>
  ...
  <Collapser id="utilization-detect_in_parallel" title="병렬 처리 감지">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    `true` 인 경우, 에이전트는 에이전트 시작 시간을 단축하기 위해 환경 감지를 병렬로 수행합니다.
  </Collapser>
</CollapserGroup>

## 받은편지함 구성 오류 [#errors-inbox-configuration]

다음 환경 변수 중 하나를 설정하면 소프트웨어의 어떤 버전에서 오류가 발생했는지 식별하는 데 도움이 됩니다.

* `NEW_RELIC_METADATA_SERVICE_VERSION` 배포된 코드 버전(대부분의 경우 `1.2.3`로 표시되는 버전)이 포함된 이벤트 데이터에 대해 `tags.service.version`을 생성하지만 항상 그런 것은 아닙니다.
* `NEW_RELIC_METADATA_RELEASE_TAG` 릴리스 태그(예: `v0.1.209` 또는 `release-209`)가 포함된 이벤트 데이터에 `tags.releaseTag`을(를) 생성합니다.
* `NEW_RELIC_METADATA_COMMIT` 커밋 SHA를 포함하는 이벤트 데이터에 대해 `tags.commit`을 생성합니다. 전체 SHA를 사용하거나 처음 7자만 사용할 수 있습니다(예: `734713b`).

[에러스 인박스(errors inbox)](/docs/errors-inbox/version-tracking/)는 오류를 생성하는 소프트웨어 버전을 자동으로 추적합니다.