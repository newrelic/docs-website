...
<CollapserGroup>
  ...
  >
    ...
    에이전트가 비동기 트랜잭션 내부에 범위를 올바르게 중첩할 수 있도록 시작 시 Thread 클래스의 자동 계측을 제어합니다. 이렇게 하면 에이전트가 생성된 모든 스레드를 자동으로 추적할 수 없습니다( `instrumentation.thread.tracing` 참조). \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
