...
<CollapserGroup>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    `true`인 경우, 에이전트는 Active Record 메트릭, 스팬 및 트랜잭션 트레이스 세그먼트의 이름을 지정할 때 클래스 이름 대신 Active Record 모델의 테이블 이름을 사용합니다. 여러 모델이 데이터베이스 테이블을 공유할 때 카디널리티를 줄일 수 있습니다. 기본값은 `false`입니다.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    에이전트 로그 파일의 파일 이름을 제외한 경로를 지정합니다. 에이전트 로그를 표준 출력으로 보내려면 이 값을 STDOUT으로 설정하십시오.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    오류 백트레이스의 최대 프레임 수를 정의합니다. 이 양을 초과하는 백트레이스는 기본적으로 중간에서 잘리며 스택 추적의 시작과 끝을 보존합니다. 잘림 위치를 커스텀하려면 `error_collector.backtrace_truncate_location` 을(를) 참조하십시오.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    프레임 수가 `error_collector.max_backtrace_frames` 초과할 때 백트레이스의 어느 부분을 잘라낼지 지정합니다. 옵션은 `top` (시작 부분에서 프레임 제거), `middle` (시작과 끝은 유지하면서 중간 부분에서 프레임 제거) 또는 `end` (끝 부분에서 프레임 제거)입니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-root" title="분산 추적.샘플러.루트">
    ...
    이 설정은 크리렐 에이전트의 이 외부 부모 트레이스가 없는 트랜잭션에 대한 소니 샘플링 동작을 제어합니다. 사용 가능한 값은 `adaptive` (기본값), `always_on`, `always_off`, `trace_id_ratio_based` 입니다.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="측정, 계측.active_support_notifications.active_support_events">
    ...
    에이전트가 구독해야 하는 Active Support 서버 자체에 특정한 Active Support 공지 이벤트의 허용 목록입니다. Active Support 라이브러리 관련 이벤트는 주로 캐싱에 중점을 둡니다. 이 목록에 포함되지 않은 이벤트 이름은 에이전트에서 무시됩니다. &apos;cache\_fetch\_hit.active\_support&apos;와 같이 전체 이벤트 이름을 제공하십시오. 별표나 정규 표현식 패턴을 제공하지 마시고, 백슬래시로 문자를 이스케이프하지 마십시오. 가능한 모든 Active Support 이벤트 이름 목록을 보려면 Rails 공식 문서의 [캐싱 이름 목록](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) 과 [메시지 이름 목록을](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) 참조하세요.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    구조화된 로그 이벤트로 캡처할 Rails.event 이름의 어레이입니다. 예를 들어,

    * user.signup
    * payment.processed
    * order.created 모든 Rails 이벤트 공지를 캡처하려면 비워 두세요. 이벤트 페이로드의 :level 키를 통해 재정의되지 않는 한 이벤트는 UNKNOWN 레벨로 로그가 기록됩니다(절대 필터링되지 않도록 보장합니다).
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="OpenTelemetry.활성화됨">
    ...
    뉴렐릭을 통해 전송되는 모든 OpenTelemetry 신호를 비활성화하는 전역 설정 옵션입니다. 만약 false이면 뉴렐릭에게 OpenTelemetry 신호가 전송되지 않습니다. 만약 사실이라면, 신호별로 활성화된 구성 옵션(예: OpenTelemetry.트레이스.활성화됨) 해당 신호 유형의 텔레메트리가 뉴렐릭에 보고될지 여부를 결정합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    `true`인 경우, 에이전트는 Google Cloud Run에서 실행될 때 GCP 인스턴스 ID를 호스트명으로 보고합니다.
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    `true`인 경우, 에이전트는 GCP 인스턴스 ID 앞에 `K_REVISION` 값을 추가하여 Google Cloud Run에서 호스트명(`{K_REVISION}-{instance id}`)을 형성합니다. `utilization.gcp_cloud_run.use_instance_as_host` 도 `true`가 아니면 아무런 효과가 없습니다.
  </Collapser>
