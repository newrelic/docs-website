...
* 두 개의 공백으로만 들여쓰기합니다.
* **error_collector** 와 같은 스탠자에서 적절한 경우에만 들여씁니다.
...
<CollapserGroup>
  ...
  >
    ...
    New Relic [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key).
  </Collapser>
  ...
  >
    ...
    지원 중단됨 에이전트 버전 3.5.0 이상의 [경우 New Relic UI를 통해 Apdex T를 설정하십시오](/docs/apm/new-relic-apm/apdex/changing-your-apdex-settings) <b>.</b>
  </Collapser>
  ...
  >
    ...
    <b>newrelic.yml</b> 에 대한 경로입니다. 정의되지 않은 경우 에이전트는 <b>config/newrelic.yml</b> , <b>newrelic.yml</b> , <b>$HOME/.newrelic/newrelic.yml</b> 및 <b>$HOME/newrelic.yml</b> 디렉터리를 순서대로 확인합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <b>지원 중단됨</b> 대신 [`transaction_tracer.attributes.enabled`](#transaction_tracer-attributes-enabled) 을(를) 사용하십시오.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <b>지원 중단됨</b> 대신 [`error_collector.attributes.enabled`](#error_collector-attributes-enabled) 을(를) 사용하십시오.
  </Collapser>
  ...
  >
    ...
    `true` 인 경우 에이전트는 [TransactionError 이벤트](/docs/insights/new-relic-insights/decorating-events/error-event-default-attributes-insights) 를 수집합니다.
  </Collapser>
  ...
  >
    ...
    <b>지원 중단됨</b> 대신 `error_collector.ignore_classes` 을(를) 사용하십시오. 에이전트가 무시해야 하는 오류 클래스의 쉼표로 구분된 목록을 지정하십시오.

    <Callout variant="caution">
      서버 측 구성은 모든 환경 구성보다 이 설정에 우선합니다. 이는 환경 변수가 서버 측 구성보다 우선하는 다른 모든 구성 설정과 다릅니다.
    </Callout>
  </Collapser>
  ...
  >
    ...
    수확 주기당 보고되는 최대 [TransactionError 이벤트](/docs/insights/new-relic-insights/decorating-events/error-event-default-attributes-insights) 수를 정의합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    <b>지원 중단됨</b> 대신 [`browser_monitoring.attributes.enabled`](#browser_monitoring-attributes-enabled) 을(를) 사용하십시오.
  </Collapser>
</CollapserGroup>

## 분석 이벤트

[New Relic 대시보드](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards) 는 소프트웨어에 대한 데이터와 비즈니스에 대한 정보를 수집하고 시각화하는 리소스입니다. 이를 통해 실시간 대시보드를 빠르고 쉽게 생성하여 최종 사용자 경험, 클릭스트림, 모바일 활동 및 서버 트랜잭션에 대한 즉각적인 답변을 얻을 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    <b>사용 중단됨</b> 대신 [`transaction_events.attributes.enabled`](#transaction_events-attributes-enabled) 을 사용하세요.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단</b> 다음을 참조하십시오. [transaction_events.enabled](docs/agents/ruby-agent/configuration/ruby-agent-configuration#transaction_events-enabled) .

    `true` 인 경우 분석 이벤트 샘플링을 활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>폐기됨</b> : [transaction_events.max_samples_stored](docs/agents/ruby-agent/configuration/ruby-agent-configuration#transaction_events-max_samples_stored) 를 참조하십시오.

    단일 수집에서 보고된 최대 요청 이벤트 수를 정의합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.bunny](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-bunny) .

    `true` 이면 bunny gem의 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.curb](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-curb) .

    `true` 이면 연석 보석에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>지원 중단됨</b> 대신 [`disable_sequel_instrumentation`](#disable_sequel_instrumentation) 을(를) 사용하십시오.
  </Collapser>
  ...
  >
    ...
    `true` 이면 DataMapper 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.memcache](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-memcache) 를 참조하세요.

    `true` 인 경우 dali gem에 대한 계측을 비활성화합니다.
  </Collapser>

  <Collapser
    id="disable_dalli_cas_client"
    title="disable_dali_cas_client"
  >
    <table>
      <tbody>
        <tr>
    ...
    `true` 인 경우 dali gem의 추가 CAS 클라이언트 지원에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b>[instrumentation.delayed_job](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-delayed_job) 을 참조하세요.

    `true` 이면 [Delayed::Job 계측](/docs/agents/ruby-agent/background-jobs/delayedjob) 을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.excon](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-excon) .

    `true` 이면 excon gem에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.memcached](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-memcached) 를 참조하세요.

    `true` 인 경우 memcached gem에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.memcache-client를](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-memcache-client) 참조하세요.

    `true` 이면 memcache-client gem에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    `true` 인 경우 Memcache 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    `true` 인 경우 GC::Profiler를 사용하여 가비지 수집에 소요된 시간을 측정할 수 없습니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.grape](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-grape) .

    `true` 인 경우 에이전트는 Grape 계측을 설치하지 않습니다.
  </Collapser>

  <Collapser
    id="disable_httpclient"
    title="disable_http클라이언트"
  >
    <table>
      <tbody>
        <tr>
    ...
    <b>사용되지 않음</b> [instrumentation.httpclient](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-httpclient) 를 참조하십시오.

    `true` 인 경우 httpclient gem에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.httprb](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-httprb) .

    `true` 인 경우 에이전트는 http.rb gem에 대한 계측을 설치하지 않습니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.mongo](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-mongo) .

    `true` 인 경우 에이전트는 [Mongo gem에 대한 계측을](/docs/agents/ruby-agent/frameworks/mongo-instrumentation) 설치하지 않습니다.
  </Collapser>
  ...
  >
    ...
    `true` 이면 에이전트가 계측에서 타사 미들웨어를 래핑하지 않습니다(Rack::Builder 또는 Rails를 통해 설치되었는지 여부에 관계없이).
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.net_http](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-net_http) 를 참조하세요.

    `true` 인 경우 Net::HTTP에 대한 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.puma_rack](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-puma_rack) 을 참조하세요.

    `true` 인 경우 에이전트가 Puma::Rack::Builder의 `to_app` 메서드에 연결하여 애플리케이션 시작 중에 계측할 gem을 찾는 것을 방지합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.puma_rack_urlmap](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-puma_rack_urlmap) 을 참조하세요.

    `true` 인 경우 에이전트가 Puma::Rack::URLMap에 연결하여 미들웨어 추적을 설치하는 것을 방지합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.rack](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-rack) .

    `true` 이면 에이전트가 Rack::Builder의 `to_app` 메서드에 연결하여 애플리케이션 시작 중에 계측할 gem을 찾는 것을 방지합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b>[instrumentation.rack_urlmap](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-rack_urlmap) 을 참조하십시오.

    `true` 인 경우 에이전트가 Rack::URLMap에 연결하여 미들웨어 추적을 설치하지 못하도록 합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.rake](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-rake) .

    `true` 인 경우 Rake 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.redis](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-redis) 를 참조하세요.

    `true` 인 경우 에이전트는 [Redis용 계측을](/docs/agents/ruby-agent/frameworks/redis-instrumentation) 설치하지 않습니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.resque](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-resque) .

    `true` 이면 [Resque 계측](/docs/agents/ruby-agent/background-jobs/resque-instrumentation) 을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용 중단됨</b> [instrumentation.sinatra](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-sinatra) 를 참조하세요.

    `true` 이면 [Sinatra 계측](/docs/agents/ruby-agent/frameworks/sinatra-support) 을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    <b>사용되지 않음</b> 참조: [instrumentation.typhoeus](docs/agents/ruby-agent/configuration/ruby-agent-configuration#instrumentation-typhoeus) .

    `true` 인 경우 에이전트는 장티푸스 보석에 대한 계측을 설치하지 않습니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    일반적으로 에이전트는 후행 점 및 프로세스 ID가 있는 dyno 이름을 보고합니다(예: <b>worker.3</b> ). 후행 데이터 없이 보고할 접두사를 지정하여 이 후행 데이터를 제거할 수 있습니다(예: <b>worker</b> ).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    추적 관찰자에게 전송되는 데이터의 압축 수준을 구성합니다. \[없음|낮음|중|높음] 중 하나일 수 있습니다. 기본값은 '높음'입니다. 압축을 비활성화하려면 수준을 '없음'으로 설정하십시오.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    시작 시 ActiveSupport::Logger의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 Excon의 자동 계측을 제어합니다. \[활성화|비활성화] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    New Relic에서 트래픽을 무시할 gRPC 호스트 이름과 일치하는 쉼표로 구분된 호스트 이름 패턴 목록을 지정합니다. New Relic의 gRPC 클라이언트 계측은 이러한 패턴과 일치하는 호스트로 스트리밍되는 트래픽을 무시하고 New Relic의 gRPC 서버 계측은 호스트 이름이 이러한 패턴과 일치하는 호스트에서 실행 중인 서버의 트래픽을 무시합니다. 기본적으로 gRPC 계측이 자체적으로 활성화된 경우 트래픽이 무시되지 않습니다. 예: "private.com$,exception.\*"
  </Collapser>
  ...
  >
    ...
    시작 시 Memcache에 대한 dali gem의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 Mongo의 자동 계측을 제어합니다. \[활성화|비활성화] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 Net::HTTP의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    Puma::Rack의 자동 계측을 제어합니다. 활성화되면 에이전트는 Puma::Rack::Builder의 `to_app` 메서드에 연결하여 애플리케이션 시작 중에 계측할 gem을 찾습니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 Puma::Rack::URLMap의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    Rack의 자동 계측을 제어합니다. 활성화되면 에이전트는 Rack::Builder의 `to_app` 메서드에 연결하여 애플리케이션 시작 중에 계측할 gem을 찾습니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 Rack::URLMap의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
  ...
  >
    ...
    시작 시 갈퀴의 자동 계측을 제어합니다. \[auto|prepend|chain|disabled] 중 하나일 수 있습니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <b>DEPRECATED</b> `true` 인 경우 Resque에서 트랜잭션 추적 및 추적된 오류에 대한 작업 인수 캡처를 활성화합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    <b>DEPRECATED</b> `true` 인 경우 Sidekiq에서 트랜잭션 추적 및 추적된 오류에 대한 작업 인수 캡처를 활성화합니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    느린 SQL 추적을 위해 더 긴 sql_id를 생성합니다. sql_id는 유사한 쿼리의 집계에 사용됩니다.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    단일 수확에서 보고되는 최대 범위 이벤트 수를 정의합니다. 1에서 10000 사이의 모든 정수가 유효합니다.
  </Collapser>
</CollapserGroup>
