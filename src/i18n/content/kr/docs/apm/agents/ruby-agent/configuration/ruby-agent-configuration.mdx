...
<CollapserGroup>
  ...
  >
    ...
    귀하의 New Relic [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) .
  </Collapser>
  ...
  >
    ...
    이 에이전트에서 보낸 데이터에 적용할[레이블 이름](/docs/data-analysis/user-interface-functions/labels-categories-organize-your-apps-servers) 및 값의 사전입니다. 콜론으로 구분된 `:` 쌍의 세미콜론으로 구분된 `;` 문자열로 표현될 수도 있습니다. 예를 들어, `<var>Server</var>:<var>One</var>;<var>Data Center</var>:<var>Primary</var>` .
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    `true` 이면 ActiveStorage 계측을 비활성화합니다.
  </Collapser>
  ...
  >
    ...
    `true` 이면 ActiveRecord 4, 5, 6에 대한 계측을 비활성화합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    추적 관찰자 호스트의 호스트 이름을 구성합니다. 구성된 경우 일반적인 에이전트 샘플링 결정과 관계없이 추가 샘플링 결정을 위해 기록된 모든 범위를 추적 관찰자에게 보내 꼬리 기반 샘플링을 활성화합니다.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <strong>
      이 구성 옵션은 현재 연결할 수 없습니다. 사용하지 마십시오.
    </strong>

    `true` 인 경우 에이전트가 Ruby on Rails를 사용하는 애플리케이션에 있을 때 config/initializers가 실행된 후 시작됩니다.
  </Collapser>
</CollapserGroup>
