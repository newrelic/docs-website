...
* 프로세서에 대해 <InlinePopover type="dashboards" />생성하려면 관습 속성을 추가할 수 있습니다.
...
* <DNT>**Custom events:**</DNT> 뉴렐릭에서는 [이벤트](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#event) 데이터가 기본적인 데이터 유형입니다. 이벤트 데이터는 특정 시점의 단일 이벤트에 대한 기록을 나타냅니다. 이는 특정 세부 정보를 보거나 쿼리하는 데 유용합니다.
* <DNT>**Custom event attributes:**</DNT> 이벤트에 대한 추가 데이터를 포함하려면 사용자 정의 대시보드에 키/값 쌍([사용자 정의 속성](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#event))을 추가할 수 있습니다.
* <DNT>**Custom metrics:**</DNT> [지표슬라 타임이스 데이터는](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#metric) 뉴렐릭이 집계한 데이터의 통계적 측정값으로 UI 에서 확인하고 차트로 표시할 수 있습니다. 일반적으로 지표 데이터는 이벤트 데이터보다 보존 기간이 더 깁니다.
...
<CollapserGroup>
  ...
  <Collapser id="custom-attributes" title="사용자 정의 이벤트 속성">
  ...
  <Collapser id="custom-metrics" title="맞춤 측정항목">
