...
<DNT>C SDK</DNT> 의 특성으로 인해 뉴렐릭에 보고되는 데이터를 직접 제어할 수 있습니다. 데이터 개인정보 보호를 보장하고 뉴렐릭이 수신하는 정보 유형을 제한하기 위해 귀하가 API 호출에 제공한 것 외에는 어떤 고객 데이터도 캡처되지 않습니다. 또한 <DNT>C SDK</DNT> 은 HTTPS를 통해 뉴렐릭에 모든 데이터를 보고합니다.
...
기본적으로 이는 <DNT>C SDK</DNT> 가 다음과 같은 잠재적으로 민감한 데이터를 처리하는 방법입니다. 코드 값 및 예제를 포함한 자세한 내용은 [GitHub의 데이터 구조에 대한 C SDK 설명서를](https://newrelic.github.io/c-sdk/annotated.html) 참조하세요.
  ...
  <tbody>
    <tr id="name">
      ...
      <td>
        ...
        * 이름: 기본적으로 `newrelic_new_config()` 에서 반환된 설정은 `instance_reporting` `true` 로 설정된 데이터스토어 세그먼트를 활성화합니다.
        * 데이터베이스 이름: 기본적으로 `newrelic_new_config()` 에서 반환된 설정은 `database_name_reporting` `true` 로 설정된 데이터스토어 세그먼트를 활성화합니다.
      </td>
    </tr>
