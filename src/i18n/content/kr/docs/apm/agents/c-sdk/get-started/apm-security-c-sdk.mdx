    ...
    <tr id="name">
      ...
      <td>
        ...
        * <DNT>**Instance names:**</DNT>
          ...
          기본적으로 `newrelic_new_config()` 에서 반환된 설정은 `instance_reporting` `true` 로 설정된 데이터 스토어 세그먼트를 활성화합니다.

        * <DNT>**Database names:**</DNT>

          기본적으로 `newrelic_new_config()` 에서 반환된 설정은 `database_name_reporting` `true` 로 설정된 데이터 스토어 세그먼트를 활성화합니다.
      </td>
    </tr>
