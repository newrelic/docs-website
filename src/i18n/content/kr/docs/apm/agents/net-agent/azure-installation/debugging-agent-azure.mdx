...
[뉴렐릭의 .NET 에이전트 for Azure를 설치한](/install/dotnet/?deployment=azure) 후 다음 문제 중 하나 이상이 발생합니다.
...
* 웹 앱 다시 시작을 포함하여 [설치 및 실행 단계를 따랐는지](/install/dotnet/?deployment=azure) 확인하세요.
* .NET 에이전트가 제대로 작동하려면 <DNT>**must**</DNT> [통합 인사이트를 비활성화](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) 하세요.
* <DNT>**must**</DNT> 앱이 .NET 에이전트를 시작하기 위한 트래픽을 받고 있습니다.
...
4. 해당 프로세스에 대한 로그가 생성되고 있는지 확인할 수 있도록 애플리케이션이 실행 중인 프로세스 ID(PID)를 기록해 두세요. Kudu에 내장된 <DNT>**Process Explorer**</DNT> 사용하여 PID를 찾을 수 있습니다.
...
2. <DNT>**Process Explorer**</DNT> 에서 모니터링하려는 프로세스를 찾고 PID 열의 숫자를 기록해 둡니다. 웹 애플리케이션을 모니터링하려는 경우 프로세스 이름은 Kudu 프로세스 자체를 나타내는 SCM 레이블 없이 `w3wp.exe` 일 가능성이 높습니다. 에이전트는 SCM(Kudu) 프로세스를 축소하면 안 됩니다.
3. 애플리케이션 프로세스를 찾았으면 `Properties` 버튼을 클릭합니다.
4. <DNT>**Modules**</DNT> 탭을 클릭하고 `NewRelic.Profiler.dll` 찾으세요. 이 DLL은 에이전트가 애플리케이션을 모니터링하는 데 필요합니다. 찾을 수 없는 경우 올바른 환경 변수가 있는지 확인하고(자세한 내용은 [3단계](#step-three) 참조) [Microsoft 삽입 인사이트가 비활성화되어 있는지](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) 확인하세요.
...
1. <DNT>**Process Explorer**</DNT> 에서 애플리케이션 프로세스를 찾고 `Properties` 버튼을 클릭합니다.
2. <DNT>**Environment**</DNT> 탭을 선택합니다.
...
<CollapserGroup>
  <Collapser id="env-var-framework-extension" title=".NET Framework: New Relic Azure 사이트 확장을 통해 설치할 때 환경 변수 확인">
    Kudu의 <DNT>**Process Explorer**</DNT> 에 있는 <DNT>**Environment**</DNT> 탭에 표시되는 환경 변수는 다음과 유사해야 합니다.
  ...
  <Collapser id="env-var-framework-nuget" title=".NET Framework: NuGet을 통해 설치할 때 환경 변수 확인">
    NuGet 패키지의 경우 환경 변수를 [애플리케이션 설정으로 추가해야 합니다](/install/dotnet/?deployment=azure&azure=azurenugetframework).
    ...
    환경 변수를 변경한 경우 Azure Portal을 통해 애플리케이션을 다시 시작해야 합니다. 그런 다음 변경 사항이 성공적으로 적용되었는지 확인하기 위해 <DNT>**Process Explorer**</DNT> 에서 새 프로세스 PID를 검사합니다.
  </Collapser>

  <Collapser id="env-var-core-windows-extension" title=".NET Core(Windows): New Relic Azure 사이트 확장 사용 시 환경 변수 확인">
    Kudu의 <DNT>**Process Explorer**</DNT> 에 있는 <DNT>**Environment**</DNT> 탭에 표시되는 환경 변수는 다음과 유사해야 합니다.
  ...
  <Collapser id="env-var-core-windows-nuget" title=".NET Core(Windows): NuGet 패키지 사용 시 환경 변수 확인">
    NuGet 패키지의 경우 환경 변수를 [애플리케이션 설정으로 추가해야 합니다](/install/dotnet/?deployment=azure&azure=azureframeworkcore). Kudu의 <DNT>**Process Explorer**</DNT> 에 있는 <DNT>**Environment**</DNT> 탭에 표시되는 환경 변수는 다음과 유사해야 합니다.
  ...
  <Collapser id="env-var-core" title=".NET Core(Linux): NuGet 패키지 사용 시 환경 변수 확인">
    NuGet 패키지의 경우 환경 변수를 [애플리케이션 설정으로 추가해야 합니다](/install/dotnet/?deployment=azure&azure=azureframeworkcore).
    ...
    `newrelic.config` 파일의 <DNT>**log**</DNT> 노드에 디렉터리 속성이 포함되어 있는지 확인하세요. Visual Studio에서 NuGet을 통해 프로젝트에 추가된 `newrelic.config` 파일을 편집할 수 없는 경우 애플리케이션에서 이 파일의 로컬 복사본을 만들어야 합니다.
...
* ..NET Core 및 .NET 프레임워크에 대한 자동화된 프레임워크에 대해서는 [호환성 페이지를](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/) 참조하세요.
