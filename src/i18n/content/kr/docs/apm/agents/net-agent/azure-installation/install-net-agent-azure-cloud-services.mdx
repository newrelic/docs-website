...
이 문서에서는 Microsoft의 Azure Cloud Services 플랫폼에 APM의 .NET 에이전트를 설치하는 방법을 설명합니다. 이는 [Microsoft Azure용 인프라 통합을](/docs/integrations/microsoft-azure-integrations/getting-started/introduction-azure-monitoring-integrations) 설치하는 것과 다릅니다. 가장 관련성이 높은 지침을 사용하고 있는지 확인하려면 먼저 [.NET 에이전트 설치 개요](/docs/agents/net-agent/installation/install-enable-new-relic-net-agent) 를 참조하십시오.
...
<CollapserGroup>
  ...
  >
    ...
    3. `:INSTALL_NEWRELIC_AGENT` 블록 내에서 `IF %ERRORLEVEL% EQU 0` 문을 찾아 조건문에 다음 문을 추가합니다.
       ...
       이 예에서 `newrelic.cmd` 배치 파일은 사용자 정의 계측 파일을 `D:\ProgramData\New Relic\.NET Agent\` 또는 `%NR_HOME%` 의 **Extensions** 폴더에 복사합니다. 이 예에서는 `MyInstrumentation.xml` 을 사용하지만 파일 이름과 복사 명령이 일치하고 파일이 유효한 XML이면 모든 이름이 작동합니다.
  </Collapser>
</CollapserGroup>
...
New Relic에서 성능을 보려면 애플리케이션이 트래픽을 수신해야 합니다. 데이터가 나타나기 시작하려면 몇 분 정도 기다려야 할 수 있습니다. 데이터가 표시되지 않으면 Azure Cloud Services에 대한 [문제 해결 절차](/docs/agents/net-agent/azure-troubleshooting/azure-cloud-services-no-data-appears) 를 참조하세요.

APM에서 앱의 성능을 보려면: **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택)으로** 이동합니다. [APM **요약** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 가 자동으로 나타납니다. 또한 [오류](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) , [데이터베이스 및 인스턴스 성능 문제](/docs/apm/applications-menu/features/analyze-database-instance-level-performance-issues) 등에 대한 [자세한](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm) 정보를 볼 수 있습니다.

2017년 10월 이전에 New Relic 앱을 만든 경우 Azure Portal을 사용하여 New Relic 계정 블레이드를 선택할 수 있습니다. [SAML 싱글 사인온(SSO)](/docs/accounts-partnerships/accounts/saml-single-sign/new-relic-partners-saml-sso) 을 사용하여 APM에 자동으로 로그인됩니다. **New Relic Accounts > choose your application** 으로 이동하여 Azure Portal에서 애플리케이션의 오류율 및 처리량 데이터를 볼 수도 있습니다.