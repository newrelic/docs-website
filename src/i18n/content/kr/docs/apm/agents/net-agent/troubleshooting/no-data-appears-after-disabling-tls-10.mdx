...
<CollapserGroup>
  <Collapser id="windows-registry" title="1단계. Windows 레지스트리에서 TLS 프로토콜을 활성화합니다.">
  ...
  <Collapser id="windows-registry" title="2단계. .NET 기본 프로토콜 켜기">
    ...
    <CollapserGroup>
      <Collapser id="strongcrypto" title="Windows 레지스트리에서 강력한 암호화 속성 활성화">
        <Callout variant="caution">
          다음 단계를 주의 깊게 따르십시오. 레지스트리를 잘못 수정하면 심각한 문제가 발생할 수 있습니다. 권장 사항: 레지스트리를 수정하기 전에 백업을 만드십시오.
        </Callout>
      ...
      <Collapser id="app-code" title="앱 코드에 프로토콜 포함">
