...
<DNT>**Exception:**</DNT> Microsoft의 SCOM(System Center Operations Manager)과 충돌하는 경우 [SCOM에 대한 문제 해결 절차를](/docs/agents/net-agent/troubleshooting/resolve-net-scom-conflicts) 따르십시오.

<CollapserGroup>
  <Collapser id="check-conflicts" title="1. 프로파일러 충돌을 확인합니다.">
    프로파일러 충돌이 있는지 확인하려면 다음을 수행하십시오.

    1. <DNT>**Process Explorer**</DNT> 관리자로 실행하세요.

    2. 앱의 프로세스를 찾으려면 앱을 마우스 오른쪽 버튼으로 클릭한 다음 <DNT>**Properties &gt; Environment**</DNT> 선택하세요.
  ...
  <Collapser id="compare-registry-keys" title="2. 필요한 경우 레지스트리 키를 프로세스 탐색기에 비교하십시오.">
    충돌하는 프로파일러가 비활성화되었지만 여전히 문제가 있는 경우 레지스트리 키를 [<DNT>**Process Explorer**</DNT>](https://technet.microsoft.com/en-us/sysinternals/bb896653.aspx) 과 비교하여 어떤 프로파일러가 있는지 확인하세요. 이 문제가 발생한 시스템에서 IIS 앱의 <DNT>**WAS**</DNT> 및 <DNT>**W3SVC REG**</DNT> 를 확인하거나 서비스 키 또는 IIS가 아닌 앱 키를 확인하세요.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <DNT>**cmd**</DNT> 사용하는 경우 <DNT>**WAS**</DNT> 및 <DNT>**W3SVC REG**</DNT> 또는 서비스/비IIS 앱에 대해 다음을 수행합니다. 다음은 IIS 애플리케이션의 예입니다.

            1. 명령줄에 `regedit` 을 입력합니다.
            2. <DNT>**Registry Editor**</DNT> 에서 <DNT>**HKEY\_LOCAL\_MACHINE &gt; SYSTEM &gt; CurrentControlSet &gt; Services**</DNT> 선택합니다.
            3. 해당하는 경우 <DNT>**WAS**</DNT> 폴더를 선택합니다. <DNT>**WAS**</DNT> 폴더의 레지스트리 키 목록에서 <DNT>**Environment**</DNT> 두 번 클릭하고 [뉴렐릭의 CLSID](#clsid) 및 환경 변수가 텍스트 상자에 나타나는지 확인합니다.
            4. 해당하는 경우 <DNT>**W3SVC**</DNT> 폴더를 선택합니다. <DNT>**W3SVC**</DNT> 폴더의 레지스트리 키 목록에서 <DNT>**Environment**</DNT> 두 번 클릭하고 [뉴렐릭의 CLSID](#clsid) 및 환경 변수가 텍스트 상자에 나타나는지 확인합니다.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <Collapser id="restore-registry-keys" title="3. 새 릴릭 레지스트리 키 또는 환경 변수를 복원합니다.">
