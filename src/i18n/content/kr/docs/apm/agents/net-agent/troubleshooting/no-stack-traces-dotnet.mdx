...
<CollapserGroup>
  ...
  >
    이는 .NET 에이전트가 500 오류를 처리하는 방법에 대한 [일반적인 동작](#cause) 입니다. 스택 추적을 강제로 보고하려면 자체 코드에서 New Relic .NET API를 호출하세요. `NewRelic.NoticeError(Exception)` 메서드를 실행하면 `Exception` 로 표시되는 스택 역추적과 함께 오류가 보고됩니다. 이 메서드 및 해당 오버로드에 대한 자세한 내용은 [GitHub에서 New Relic의 .NET 에이전트 API를](/docs/agents/net-agent/net-agent-api/noticeerror-net-agent-api/) 참조하세요.
  </Collapser>
</CollapserGroup>
