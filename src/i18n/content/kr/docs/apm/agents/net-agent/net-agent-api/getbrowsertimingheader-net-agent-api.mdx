...
<Callout variant="important">
  에이전트가 웹 어셈블리 코드를 계측할 수 없기 때문에 이 API는 Blazor 웹 어셈블리에 대해 지원되지 않습니다. 다음 예는 Blazor 서버 애플리케이션에만 해당됩니다. Blazor 웹 어셈블리 페이지에 브라우저 에이전트를 추가하는 [복사-붙여넣기 방법](https://docs.newrelic.com/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#copy-paste) 을 사용합니다.
</Callout>
