...
.NET 런타임에 프로파일링 활성화 여부와 사용할 프로파일러를 알려주는 데 사용되는 세 가지 환경 변수 세트( `COR_ENABLE_PROFILING`, `COR_PROFILER` 및 `COR_PROFILER_PATH` 가 있습니다. 이러한 변수의 이름과 각각의 올바른 값은 애플리케이션이 .NET Framework 또는 .NET Core용으로 빌드되었는지 여부에 따라 다릅니다.
