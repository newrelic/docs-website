...
## IIS 호스팅 애플리케이션에 대한 환경 변수 설정 [#iis]

애플리케이션이 IIS에서 호스팅되는 경우, 시스템 전체 환경 변수로 설정하는 대신(또는 그에 추가하여) IIS의 `ApplicationHost.config` 파일을 직접 편집하여 애플리케이션 풀 단위로 환경 변수를 설정할 수 있습니다. 이는 머신의 모든 프로세스가 아닌 특정 애플리케이션 풀에 대해서만 .NET 에이전트를 활성화하거나 설정하려는 경우에 유용합니다.

기본적으로 `C:\Windows\System32\inetsrv\config\ApplicationHost.config`에서 `ApplicationHost.config`을 찾을 수 있습니다. `<applicationPools>` 섹션 내에서 애플리케이션 풀을 나타내는 각 `<add>` 요소는 `<environmentVariables>` 하위 요소를 포함할 수 있으며, 이 하위 요소에는 해당 애플리케이션 풀에 대해 설정하려는 각 환경 변수에 대한 `<add name="..." value="..." />` 항목이 포함됩니다.

예를 들어, 이름이 `TestApplication`인 애플리케이션 풀에 대해 .NET 에이전트를 활성화하고 몇 가지 일반적인 설정 옵션을 설정하려면:
...
또한 특정 풀의 자체 `environmentVariables` 설정이 이를 재정의하지 않는 한, 시스템 전체 환경 변수와 동일한 방식으로 기본적으로 모든 애플리케이션 풀에 적용되는 환경 변수를 `applicationPoolDefaults`을(를) 사용하여 설정할 수 있습니다.

<Callout title="메모">
  `ApplicationHost.config`을 직접 편집하려면 관리자 권한이 필요하며, 변경 사항을 적용하려면 애플리케이션 풀을 재활용(또는 IIS 다시 시작)해야 합니다. 잘못된 설정으로 인해 IIS가 사이트나 애플리케이션 풀을 시작하지 못할 수 있으므로 편집하기 전에 항상 파일을 백업하십시오.
</Callout>

이 설정 요소에 대한 자세한 내용은 [applicationPoolDefaults environmentVariables](https://learn.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/applicationpooldefaults/environmentvariables/)에 대한 Microsoft 문서를 참조하십시오.