...
<CollapserGroup>
  ...
  >
    .NET 프레임워크 버전을 목표로 지정하려면 호환성 문제를 일으킬 수 있는 엄격한 런타임 검사가 적용된다는 점에 유의하세요. <DNT>**Recommendation:**</DNT> 가능하다면 이 옵션을 사용하는 대신 [새 파이프라인을 사용하도록 .NET에 지시하세요](#use-new-pipeline) .
