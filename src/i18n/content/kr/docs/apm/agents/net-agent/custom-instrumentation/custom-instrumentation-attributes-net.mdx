...
`[Transaction]` 속성으로 장식된 메소드는 트랜잭션이 이미 존재하지 않는 경우에만 새 트랜잭션을 생성합니다. 이전에 시작된 트랜잭션 **내** 에서 `[Transaction]` 로 장식된 메서드가 호출되면 대신 `[Trace]` 속성으로 처리되어 기존 트랜잭션에 대한 추가 정보를 제공합니다.
...
<Callout variant="important">
  **디버그** 모드로 Visual Studio에서 애플리케이션을 실행하면 일부 메서드가 New Relic 추적에 표시되지 않을 수 있습니다. New Relic에 메서드가 표시되도록 하려면 명령줄을 통해 릴리스 모드에서 애플리케이션을 실행하세요.
</Callout>
