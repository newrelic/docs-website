...
<CollapserGroup>
  ...
  >
    IIS에서 호스팅되는 [WCF 서비스는](/docs/agents/net-agent/instrumentation/instrumenting-wcf-applications) `ExecuteRequestHandler` 세그먼트 아래에 <DNT>**WCF**</DNT> 세그먼트를 올바르게 중첩하지 않습니다. 두 세그먼트는 보고된 [총 시간이](/docs/data-analysis/user-interface-functions/response-time#response-time-total-time) 정확하더라도 프로세서 트레이스 내에서 형제로 표시됩니다.
  </Collapser>
