...
Docker 컨테이너에서 .NET 에이전트를 설정하는 방법은 설정에 따라 다릅니다. 다음은 요구 사항 및 고려 사항에 대한 개요입니다.

* 모니터링할 컨테이너에 에이전트를 설치해야 합니다.
* 표준 설치 절차 중 하나를 사용하여 에이전트를 설치합니다. [Windows](/docs/apm/agents/net-agent/installation/install-net-agent-windows) \| [리눅스](/docs/apm/agents/net-agent/installation/install-net-agent-linux)
* 에이전트와 함께 작동하도록 Docker 설정을 구성합니다. 이 문서에서 Docker 파일의 몇 가지 예를 제공했습니다.
* [필요한 환경 변수](/docs/agents/net-agent/configuration/net-agent-configuration#environment-variables) 를 설정하여 .NET 에이전트를 활성화합니다.
* .NET 에이전트는 런타임에 설치되고 활성화되어야 합니다.

애플리케이션이 트래픽을 수신하는 경우 데이터가 몇 분 이내에 표시되어야 합니다. 표시되지 않으면 [데이터가 표시되지 않음](/docs/apm/agents/net-agent/troubleshooting/no-data-appears-net) 을 참조하십시오.
