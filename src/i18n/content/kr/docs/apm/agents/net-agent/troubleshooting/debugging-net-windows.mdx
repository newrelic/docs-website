...
<CollapserGroup>
  ...
  <Collapser id="env-var-core" title=".NET Core: 환경 변수 확인">
    ...
    <Callout variant="tip">
      시스템 전체가 아닌 프로세스별로 이 환경 변수를 설정하는 방법에 대한 팁은 [환경 변수 설정에 대한 포럼 게시물](https://knowledge.newrelic.com/s/article/Setting-dot-NET-Core-agent-environment-variables-per-process)을 참조하십시오.
    </Callout>
  </Collapser>
</CollapserGroup>
