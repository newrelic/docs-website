...
올바른 변수가 설정되어 있고 모니터링하려는 애플리케이션이 해당 변수에 액세스할 수 있는 경우 앱(에이전트??)이 .NET Framework인지 .NET Core인지에 따라 특정 환경 변수 세트가 표시되어야 합니다.
...
<CollapserGroup>
  ...
  >
    환경 탭에 나타나는 환경 변수는 다음과 유사해야 합니다.
    ...
    * `COR_ENABLE_PROFILING` 이 프로세스에 대해 프로파일링을 활성화해야 함을 .NET CLR에 알립니다. .NET Framework 프로파일러(.NET 에이전트 포함)가 작동 **하려면** `1` 로 설정해야 합니다.
    * `COR_PROFILER` New Relic 프로파일러를 프로세스에 로드할 프로파일러로 식별합니다. 프로세스 탐색기에 표시된 CLSID가 위의 CLSID와 일치하지 않으면 시스템에 여러 프로파일러가 설치되어 있고 다른 프로파일러가 현재 프로세스를 모니터링하도록 구성되어 있는 것입니다. 이것은 [프로파일러 충돌](/docs/agents/net-agent/troubleshooting/profiler-conflicts) 로 알려져 있습니다.
    * `NEWRELIC_INSTALL_PATH` 모니터링에 필요한 New Relic 파일을 찾을 위치를 프로파일러에 알려줍니다. 에이전트를 기본 경로에 설치한 경우 경로는 위의 항목과 일치해야 합니다.

    환경 변수가 위에 표시된 것과 일치하지 않으면 .NET 에이전트 설치 프로그램을 다시 실행하고 기본값으로 복원하라는 메시지가 표시되면 `Repair` 를 선택할 수 있습니다.
  ...
  >
    ...
    * `CORECLR_ENABLE_PROFILING` 이 프로세스에 대해 프로파일링을 활성화해야 함을 .NET Core 런타임에 알립니다. .NET Core 프로파일러(.NET 에이전트 포함)가 작동 **하려면** `1` 로 설정해야 합니다.
    * `CORECLR_PROFILER` New Relic 프로파일러를 프로세스에 로드할 프로파일러로 식별합니다. 프로세스 탐색기에 표시된 CLSID가 위의 CLSID와 일치하지 않으면 시스템에 여러 프로파일러가 설치되어 있고 다른 프로파일러가 현재 프로세스를 모니터링하도록 구성되어 있는 것입니다. 이것은 [프로파일러 충돌](/docs/agents/net-agent/troubleshooting/profiler-conflicts) 로 알려져 있습니다.
    * `CORECLR_NEWRELIC_HOME` 시스템에 .NET Core 에이전트가 설치된 위치의 경로입니다.
    * `CORECLR_PROFILER_PATH` 시스템의 프로파일러 dll에 대한 경로입니다.
...
위의 단계로 문제가 해결되지 않으면 지원팀에 문의하거나 [Explorers Hub 포럼](https://discuss.newrelic.com/tag/dotnetagent) 에서 도움을 요청하는 것이 좋습니다.