...
[**트랜잭션** 페이지](/docs/applications-menu/transactions-dashboard#tx_viewing) 및 [트랜잭션 추적](/docs/traces/viewing-transaction-traces) 에서 이러한 메트릭을 볼 수 있습니다. 거래를 보려면: **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택) > Monitor > Transactions > Type > (유형 선택)**으로 이동합니다. [API를 사용하여 카테고리를 변경](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api) 하지 않는 한 유형은 `Non-web/Custom` 입니다.
