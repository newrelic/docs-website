...
1. 생성한 파일에서 속성 값 `TransactionName` , `AssemblyName` , `NameSpace.ClassName` 및 `MethodName` 을 사용자 지정합니다. 트리거 메서드와 트리거 메서드에서 호출하는 메서드 모두에 대해 이러한 값을 사용자 지정합니다.

   <Callout variant="tip">
     이러한 값은 대소문자를 구분합니다.
   </Callout>

   * `TransactionName`: 트랜잭션 이름을 정의합니다. `metricName` 속성은 선택 사항입니다. 생략하면 트랜잭션 이름은 `NameSpace.ClassName/MethodName` 이 됩니다. 거래 범주는 `Custom` 입니다. 결과 전체 측정항목 이름은 `OtherTransaction/Custom/TransactionName` 입니다. `Custom` 에서 트랜잭션 범주를 변경하려면 [SetTransactionName](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api) API 호출을 사용하세요. New Relic UI는 [트랜잭션 유형 필드](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm/#agent-net) 의 범주 아래 트랜잭션을 그룹화합니다.
...
[**트랜잭션** 페이지](/docs/applications-menu/transactions-dashboard#tx_viewing) 및 [트랜잭션 추적](/docs/traces/viewing-transaction-traces) 에서 이러한 메트릭을 볼 수 있습니다. 거래를 보려면: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Monitor > Transactions > Type > (유형 선택)으로** 이동합니다. [API를 사용하여 카테고리를 변경](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api) 하지 않는 한 유형은 `Non-web/Custom` 입니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Monitoring > Transactions > Type > (선택된 유형)** : **유형** 메뉴를 사용하여 사용자 정의 트랜잭션을 봅니다.
</figcaption>
...
이 간단한 콘솔 앱은 트랜잭션 생성을 보여줍니다. 애플리케이션을 몇 번 실행하면 [트랜잭션 페이지](/docs/apm/applications-menu/monitoring/transactions-page) ( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (앱 선택) > Transactions > Type** )에서 생성한 트랜잭션을 볼 수 있습니다. **더미** 세그먼트는 트랜잭션 분석 테이블과 모든 트랜잭션 추적에서 볼 수 있습니다.
