...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitoring > Transactions > Type > (선택된 유형)** : **유형** 메뉴를 사용하여 사용자 정의 트랜잭션을 봅니다.
</figcaption>
...
이 간단한 콘솔 앱은 트랜잭션 생성을 보여줍니다. 애플리케이션을 몇 번 실행하면 [트랜잭션 페이지](/docs/apm/applications-menu/monitoring/transactions-page) ( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Transactions > Type** )에서 생성한 트랜잭션을 볼 수 있습니다. **더미** 세그먼트는 트랜잭션 분석 테이블과 모든 트랜잭션 추적에서 볼 수 있습니다.
