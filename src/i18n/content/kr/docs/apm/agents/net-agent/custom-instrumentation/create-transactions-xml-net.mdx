...
[**트랜잭션** 페이지](/docs/applications-menu/transactions-dashboard#tx_viewing) 및 [트랜잭션 추적](/docs/traces/viewing-transaction-traces) 에서 이러한 메트릭을 볼 수 있습니다. 거래를 보려면: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitor > Transactions > Type > (유형 선택)으로** 이동합니다. [API를 사용하여 카테고리를 변경](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api) 하지 않는 한 유형은 `Non-web/Custom` 입니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Monitoring > Transactions > Type > (선택한 유형)** : **유형** 메뉴를 사용하여 사용자 정의 트랜잭션을 봅니다.
</figcaption>
...
이 간단한 콘솔 앱은 트랜잭션 생성을 보여줍니다. 애플리케이션을 몇 번 실행하면 [트랜잭션 페이지](/docs/apm/applications-menu/monitoring/transactions-page) ( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (앱 선택) > Transactions > Type** )에서 생성한 트랜잭션을 볼 수 있습니다. **Dummy** 세그먼트는 트랜잭션 분석 테이블과 모든 트랜잭션 추적에서 볼 수 있습니다.
