...
* <DNT>
  ...
  누락된 데이터의 경우: [기본 문제 해결 방법을](#basic-solutions) 참조하세요.

* <DNT>
    **Other situations:**
  </DNT>

  기본 솔루션으로 문제가 해결되지 않으면 데이터 부족이 시스템에 더 본질적일 수 있습니다. 자세한 내용은 [고급 문제 해결 솔루션을](#advanced-solutions) 참조하세요.

* <DNT>
    **Microsoft Azure users:**
  </DNT>

  [Azure 문제 해결 솔루션을](/docs/agents/net-agent/azure-troubleshooting) 참조하세요.

* <DNT>
    **No browser data appears:**
  </DNT>

  [브라우저 에이전트 사용 시 문제 해결 팁을](/docs/agents/net-agent/troubleshooting/no-page-load-timing-data-appears-net) 참조하세요.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <DNT>**.NET Core:**</DNT> Kestrel 요구 사항을 확인하세요.
      </td>
    ...
    <tr>
      <td>
        <DNT>**.NET Framework:**</DNT> IIS를 다시 시작하십시오.
      </td>
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DNT>**.NET Framework:**</DNT> 레지스트리 권한을 확인하세요.
      </td>
    ...
    <tr>
      <td>
        <DNT>**.NET Framework:**</DNT> Windows 이벤트 뷰어를 실행합니다.
      </td>
    ...
    <tr>
      <td>
        <DNT>**.NET 8:**</DNT> 런타임 진단이 활성화되어 있는지 확인하세요
      </td>

      <td>
        .NET 8부터 환경 변수 `DOTNET_EnableDiagnostics=0` (또는 `COMPlus_EnableDiagnostics=0`)를 설정하면 CLR 프로파일링이 비활성화되어 에이전트가 작동할 수 없게 됩니다. 이 설정은 일반적으로 읽기 전용 파일 시스템 구현, 배포 시나리오에서 권장되지만 진단 포트를 비활성화하는 경우에만 사용됩니다. 프로파일링을 활성화한 상태에서 진단 시스템의 특정 하위 구성 요소를 비활성화하는 방법에 대한 [Microsoft 설명서를](https://learn.microsoft.com/en-us/dotnet/core/tools/dotnet-environment-variables#dotnet_enablediagnostics) 참조하십시오.
      </td>
    </tr>
