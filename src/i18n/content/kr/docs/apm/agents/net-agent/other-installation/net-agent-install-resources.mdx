...
이 문서에는 [New Relic .NET 에이전트 설치 절차](/docs/agents/net-agent/installation/new-relic-net-agent-install-introduction) 에서 참조하는 리소스 및 절차에 대한 자세한 설명이 포함되어 있습니다.

<Callout variant="important">
  이 문서는 독립형 문서로 읽을 수 **없습니다** . 설치 지침은 [.NET 에이전트 설치 소개를](/docs/agents/net-agent/installation/install-enable-new-relic-net-agent) 참조하세요.
</Callout>
...
New Relic 이외의 .NET 모니터링 솔루션은 이 프로파일링 API를 사용할 수 있습니다. 그러나 한 번에 하나의 서비스만 프로파일러를 사용할 수 있습니다. 즉, 과거에 .NET 모니터링 서비스를 사용하고 완전히 비활성화/제거하지 않은 경우 프로파일러가 여전히 사용 중일 수 있으므로 .NET 에이전트를 활성화하려고 할 때 [프로파일러 충돌](/docs/agents/net-agent/troubleshooting/profiler-conflicts) 이 발생할 수 있습니다.

## .NET 에이전트 다운로드 라이브러리 [#download-library]

New Relic .NET 에이전트 [다운로드 라이브러리](https://download.newrelic.com/dot_net_agent/latest_release/) 에는 특정 [설치 절차](/docs/agents/net-agent/installation/install-enable-new-relic-net-agent#common-installs) 에서 참조되는 설치 파일 패키지가 포함되어 있습니다. 이러한 파일에 대한 자세한 내용은 다운로드 라이브러리의 [ReadMe 파일](https://download.newrelic.com/dot_net_agent/latest_release/Readme.txt) 을 참조하십시오.

## 수동 .NET 에이전트 설치용 Zip 파일(Windows) [#install-zip-file]

ZIP 파일을 사용하여 에이전트를 수동으로 설치하려면 모니터링하려는 애플리케이션에 대한 올바른 파일을 선택하십시오.

1. [.NET 에이전트 다운로드 사이트](http://download.newrelic.com/dot_net_agent/latest_release/ "링크가 새 창에서 열립니다.") 로 이동하여 애플리케이션의 런타임 및 아키텍처와 일치하는 파일을 가져옵니다.

   * .NET 프레임워크(32비트): `newrelic-agent-win-x86-VERSION.zip`
   * .NET 프레임워크(64비트): `newrelic-agent-win-x64-VERSION.zip`
   * .NET 코어(32비트): `newrelic-netcore20-agent-win-x86-VERSION.zip`
   * .NET 코어(64비트): `newrelic-netcore20-agent-win-x64-VERSION.zip`
...
3. 모니터링하려는 프로세스에 대한 환경 변수를 설정합니다.
   ...
   <CollapserGroup>
     ...
     <Collapser
       ...
       title="수동 설치를 위한 .NET Core 환경 변수"
     >
       .NET 코어의 경우 다음과 같은 변수가 필요하다.
...
* **라이센스 키** . [New Relic 라이센스 키](/docs/accounts/install-new-relic/account-setup/license-key) 를 설정하기 위한 환경 변수는 다음과 같습니다.
...
<CollapserGroup>
  ...
  >
    ...
    2. [다운로드 사이트](https://download.newrelic.com/dot_net_agent/latest_release/ "링크가 새 창에서 열립니다.") 에서 .NET Framework용 스크립트 가능한 설치 프로그램 패키지를 다운로드합니다. 이러한 패키지 중 하나를 다운로드하거나 사용하면 [라이센스 조건](/docs/licenses/license-information/agent-licenses/net-agent-licenses) 에 동의하고 수락하는 것입니다.
    ...
    5. 옵션이 없는 단순 설치의 경우 다음 명령을 사용하십시오.
       ...
       다음 명령을 실행하여 .NET Framework 에이전트를 제거합니다.
  ...
  >
    ...
    2. [.NET 에이전트 다운로드 사이트](https://download.newrelic.com/dot_net_agent/latest_release/ "링크가 새 창에서 열립니다.") 에서 .NET Core 에이전트의 스크립트 가능한 설치 프로그램 패키지를 다운로드합니다. 이러한 패키지 중 하나를 다운로드하거나 사용하면 [라이센스 조건](/docs/licenses/license-information/agent-licenses/net-agent-licenses) 에 동의하고 수락하는 것입니다.
       ...
       <table>
         ...
         <tbody>
           ...
           <tr>
             ...
             <td>
               **필수** . <var>로컬</var> (앱별) 설치인지 <var>전역</var> (시스템 전체) 설치인지 결정합니다.
             </td>
           </tr>
