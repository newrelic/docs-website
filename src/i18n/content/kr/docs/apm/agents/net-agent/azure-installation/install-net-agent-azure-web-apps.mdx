...
1. New Relic .NET 에이전트의 앱 설정을 추가하기 전에 [라이센스 키가](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 있는지 확인하십시오.
...
[](/docs/apis/intro-apis/new-relic-api-keys/#license-key)**애플리케이션 설정** 으로 라이선스 키 와 앱 이름을 설정하면 `newrelic.config` 이(가) 재정의됩니다.
