    ...
    <table>
      ...
      <tbody>
        ...
        <tr id="220">
          ...
          <td>
            버전 2.19.3.0은 `GetBrowserTimingFooter()` 및 `GetBrowserTimingHeader()` API 호출이 모두 필요한 마지막 .NET 에이전트 버전입니다. <InlinePopover type="browser"/>. 에이전트 버전 2.20.24.0 이상에서는 `GetBrowserTimingHeader()` 만 호출하면 됩니다. `GetBrowserTimingFooter()` 은 효과가 없습니다. **권장사항:** 코드에서 이러한 참조를 제거하세요.
          </td>
        </tr>
