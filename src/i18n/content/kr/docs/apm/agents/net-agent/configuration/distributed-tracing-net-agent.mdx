...
<CollapserGroup>
  <Collapser id="calling-service" title="호출 서비스 계측">
    ...
    1. 아웃바운드 메시지가 트랜잭션 컨텍스트 내에서 전송되는지 확인하세요. 이를 위해서는 [사용자 정의가](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/) 필요할 수 있습니다.
    2. 전송되는 메시지에 추적 데이터를 삽입하기 위해 에이전트 API 호출을 실행합니다( [.NET 에이전트 API](/docs/agents/net-agent/net-agent-api/itransaction#InsertDistributedTraceHeaders) 참조).
  </Collapser>

  <Collapser id="called-service" title="호출된 서비스 계측">
    호출된 서비스를 계측하려면:

    1. 들어오는 메시지가 거래의 맥락 내에서 수신되었는지 확인하세요. 이를 위해서는 [사용자 정의가](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/) 필요할 수 있습니다.
    2. 수신 메시지에서 추적 데이터를 수락하기 위해 에이전트 API 호출을 실행합니다( [.NET 에이전트 API](/docs/agents/net-agent/net-agent-api/itransaction#AcceptDistributedTraceHeaders) 참조).
  </Collapser>
</CollapserGroup>