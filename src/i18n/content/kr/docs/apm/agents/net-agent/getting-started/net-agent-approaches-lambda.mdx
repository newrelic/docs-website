...
다음 AWS Lambda 함수 유형은 추가 설정 없이 자동으로 감지되고 계측됩니다(에이전트 버전 10.29.0 기준):

* `Amazon.Lambda.AspNetCoreServer.APIGatewayProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.APIGatewayHttpApiV2ProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.ApplicationLoadBalancerFunction`

`NEW_RELIC_LAMBDA_HANDLER` 환경 변수를 정규화된 핸들러 경로(예: `MyAssembly::MyNamespace.MyClass::MyMethod`)로 설정하면 기타 핸들러 유형(SQS, SNS, Kinesis, S3, DynamoDB 트리거 및 사용자 지정 핸들러 등)도 계측됩니다. 여기에는 `ILambdaContext` 파라미터를 받는 모든 핸들러 메서드가 포함됩니다.
...
* SQS의 인바운드 분산 추적을 위해서는 업스트림 프로듀서가 W3C 트레이스 헤더(`traceparent`, `tracestate`)를 SQS 메시지 속성으로 주입해야 합니다. 에이전트는 존재하는 경우 이를 자동으로 읽습니다.
