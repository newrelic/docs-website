...
.NET 에이전트가 처리량이 높은 <DNT>**.NET Framework**</DNT> 애플리케이션에 연결된 경우 몇몇 고객이 높은 스레드 경합을 관찰했다고 보고했습니다. 프로세스 덤프 및 스택 추적을 검토하면 `AppDomain.GetData()` 호출로 인해 많은 스레드가 차단될 수 있습니다.
