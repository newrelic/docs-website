...
* `50`: `InstrumentAllNETFramework` 가 활성화된 에이전트를 설치합니다(아래 설명 참조).
...
모니터링하려는 응용 프로그램이 IIS에서 호스팅되지 않는 경우 몇 가지 추가 주의 사항 및 작업 항목과 함께 [표준 설치 절차](#msi) 를 따릅니다.

* [표준 설치 절차](#msi) 를 따를 때 **Instrument all** 을 활성화하는 것이 좋습니다.
* 표준 설치 지침을 완료하면 해당 애플리케이션에 대한 에이전트를 명시적으로 활성화해야 합니다(아래 설명 참조).
* 대부분의 비 IIS 애플리케이션은 자동으로 계측되지 않으므로( [.NET Core](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-core) \| [.NET Framework](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-framework) ) New Relic에 데이터 보고를 받으려면[사용자 지정 계측을 설정](/docs/apm/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation) 해야 할 수 있습니다.

다음은 .NET Framework 및 .NET Core용 에이전트를 활성화하기 위한 지침입니다.
...
<CollapserGroup>
  ...
  >
    [`newrelic.config` 파일](/docs/apm/agents/net-agent/configuration/net-agent-configuration/#application-instrumentation) (글로벌 또는 로컬)을 사용하여 모니터링할 애플리케이션을 선택할 수 있습니다. 구성 파일에서 각 애플리케이션에 대해 하위 `applications` 요소가 있는 `instrumentation` 요소와 다른 하위 요소인 `application` 을 추가합니다. instrumentation 요소에서 확장자가 포함된 기본 파일 이름을 제공합니다(예: `<application name="DataServices.exe" />` ).
...
### .NET Core용 에이전트 활성화 [#enable-net-core]
...
### .NET Core 및 사용자 지정 계측 [#custom-instrumentation]

ASP.NET Core 앱 및 일부 다른 .NET Core 프레임워크는 [자동으로 계측](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements-net-core/#app-frameworks) 되지만 대부분의 다른 .NET Core 프레임워크는 설치 후 [사용자 지정 계측](/docs/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation) 이 필요합니다.

추가 도움말 [은 문제 해결 문서](/docs/apm/agents/net-agent/troubleshooting/no-data-appears-net) 를 참조하십시오.
