...
<Callout variant="important">
  자동 계측은 .NET Framework 앱에서만 사용할 수 있으며 .NET Core 앱은 [수동 계측을](#manual_instrumentation) 사용해야 합니다. .NET Framework 또는 Core 에이전트에서 모니터링하는지 여부에 관계없이 ASP.NET Core 애플리케이션에는 자동 계측을 사용할 수 없습니다.
</Callout>
