...
.NET 에이전트를 사용하면 다음을 추가할 수 있습니다. [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)귀하의 웹페이지에 대한 계측. .NET 에이전트와 함께 브라우저를 사용하기 전에 [.NET 에이전트 릴리스 정보 를](/docs/release-notes/agent-release-notes/net-release-notes) 참조하고 [최신 .NET 에이전트 릴리스가 설치되어](/docs/agents/net-agent/installation/install-net-agent-windows) 있는지 확인하세요.
...
1. **`newrelic.config`** 편집하고 [`browserMonitoring` `autoInstrument` 플래그를](/docs/agents/net-agent/installation-configuration/net-agent-configuration#browsermon-autoInstrument) `true` 로 설정하여 자동 측정, 자동 로그를 전역적으로 활성화했는지 확인하세요.
