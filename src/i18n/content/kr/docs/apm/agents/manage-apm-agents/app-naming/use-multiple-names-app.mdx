...
일반적으로 두 개의 로그가 동일한 앱 이름, 에이전트 언어 및 [볼륨 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 보고되면 뉴렐릭은 해당 데이터를 하나의 뉴렐릭-모니터 앱으로 집계합니다. 또한 앱의 뉴렐릭 [APM <DNT>**Overview**</DNT> 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page#infra-server) 에서 개별 호스트 및 제외에 대한 데이터를 볼 수도 있습니다.
...
여러 이름을 사용하여 데이터를 보고하는 경우 해당 기능의 모든 <InlinePopover type="browser" />데이터도 동일한 설정을 사용하여 여러 기능으로 그룹화됩니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        [newrelic\_app\_config\_t\* 구성](/docs/agents/c-sdk/install-configure/install-c-sdk-compile-link-your-code#app-name) 에서 세미콜론 `;` 으로 구분된 최대 3개의 이름을 식별합니다. .
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="example1" title="하나의 앱, 세 개의 클러스터">
  ...
  <Collapser id="example2" title="하나의 앱에 대한 두 개의 인스턴스">
    ...
    * 두 개의 앱: 각 데이터센터(`East` 및 `West`)마다 하나씩
