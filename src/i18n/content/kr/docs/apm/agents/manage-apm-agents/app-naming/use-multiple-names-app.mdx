...
일반적으로 두 개의 로그가 동일한 앱 이름, 에이전트 언어 및 [볼륨 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 보고되면 뉴렐릭은 해당 데이터를 하나의 뉴렐릭-모니터 앱으로 집계합니다. 또한 앱의 뉴렐릭 [APM <DoNotTranslate>**Overview**</DoNotTranslate> 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page#infra-server) 에서 개별 호스트 및 제외에 대한 데이터를 볼 수도 있습니다.
...
<Callout variant="important">
  세션 트레이스 데이터는 <DoNotTranslate>**only**</DoNotTranslate> 나열된 첫 번째 데이터에 보고됩니다. 다른 브라우저 데이터는 최대 3개의 DB 각각에 채워지지만, 세션 트레이스 데이터는 가장 구체적인 DB로 제한됩니다.
</Callout>
