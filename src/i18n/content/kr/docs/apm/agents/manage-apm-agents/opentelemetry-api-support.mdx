...
<CollapserGroup>
  ...
  <Collapser id="ruby-config" title="Ruby 에이전트 구성">
    ...
    루비 에이전트는 Trace용 OpenTelemetry API 삽입 및 소스화된 OpenTelemetry API의 존재를 감지하여 HTML 템플릿을 생성합니다. 라이브러리 또는 여러분의 디버그 코드가 OpenTelemetry API를 호출할 때, 뉴렐릭 루비 에이전트는 OpenTelemetry 지원이 활성화된 경우 해당 호출을 뉴렐릭 데이터로 변환합니다.

    * **필수 조건**: 하이브리드 에이전트 기능을 사용하려면 애플리케이션에 OpenTelemetry API가 있어야 합니다.

    ### 지원되는 측정, 로그

    루비 에이전트는 세 가지 유형의 OpenTelemetry 측정과 함께 작동합니다.

    #### 1. 네이티브 측정, 로그

    OpenTelemetry 가 내장된 라이브러리, 로그 호출이 지원됩니다. 이러한 라이브러리가 OpenTelemetry API 호출하면 루비 에이전트는 이를 캡처하여 뉴렐릭 텔렙리로 변환합니다.

    #### 2. 타사 측정, 로그(OpenTelemetry-ruby-contrib)

    [OpenTelemetry-ruby-contrib 저장소](https://github.com/open-telemetry/opentelemetry-ruby-contrib) 에서 gem을 설치하면 해당 gem이 OpenTelemetry API를 호출할 때 해당 호출이 처리되어 뉴럴릭 데이터로 변환됩니다.

    사용 가능한 루비 측정의 전체 목록을 보려면 [OpenTelemetry 레지스트리를](https://opentelemetry.io/ecosystem/registry/) 참조하세요.

    #### 3. 사용자 정의/수동 측정, 리뷰

    OpenTelemetry 지원이 활성화되면 독창적인 코드에서 OpenTelemetry API를 호출하면 뉴렐릭 텔레메트리가 자동으로 생성됩니다.

    ### 알려진 제한 사항

    * **API 지원 제한**: 이번 릴리스에서는 TRAS API 만 지원됩니다.
    * **지표 및 기록**: 해당사항 없음
    * **제외 목록**: OpenTelemetry 와 뉴럴릭이 동일한 방법을 계측했을 때 중복 텔레메트리를 방지하는 자동화 작업이 진행 중입니다. 현재로서는 중복을 방지하기 위해 측정, 계측 항목 중 하나를 수동으로 비활성화해야 할 수도 있습니다.
