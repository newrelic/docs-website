...
<CollapserGroup>
  ...
  <Collapser id="python-config" title="Python 에이전트 구성">
    ...
    **환경 변수 사용:**
    ...
    <Callout variant="important">
      OpenTelemetry 지원이 활성화되면 에이전트는 `opentelemetry-api` 이(가) 설치되어 있는지 자동으로 감지하고, 뉴렐릭에 해당 라이브러리에 대한 계측 후크가 있더라도 지원되는 모든 라이브러리에 대해 OpenTelemetry 계측을 사용하도록 전환합니다.
    </Callout>
    ...
    1. **네이티브 계측**: 네이티브 OpenTelemetry 계측이 내장되어 있고 뉴렐릭에서 지원하지 않는 라이브러리의 경우, 에이전트는 네이티브 OpenTelemetry 계측을 사용합니다. 사용자가 내장된 뉴렐릭 계측 후크 대신 OpenTelemetry 계측을 사용하려는 경우, 해당 라이브러리를 `opentelemetry.traces.include` 설정에 추가할 수 있습니다.

    2. **라이브러리 계측**: OpenTelemetry 계측 라이브러리가 감지되고 뉴렐릭이 아직 계측 후크를 제공하지 않는 경우, 에이전트는 OpenTelemetry 계측을 사용합니다. 네이티브 계측과 유사하게, 사용자가 내장된 뉴렐릭 계측 훅 대신 OpenTelemetry 계측을 사용하려는 경우 해당 라이브러리를 `opentelemetry.traces.include` 설정에 추가할 수 있습니다.
    ...
    <Callout variant="caution">
      AWS 및 gRPC 계측 라이브러리는 뉴렐릭의 계측을 계속 사용합니다. 네이티브 계측이 포함된 ML/AI 라이브러리는 현재 명시적으로 비활성화되어 있으며 향후 릴리스에서 지원될 예정입니다.
    </Callout>
    ...
    **예시:**
    ...
    2. **자동 전환**: 발견되었고 뉴렐릭이 이 라이브러리를 아직 지원하지 않는 경우, 에이전트는 OpenTelemetry 라이브러리의 계측을 사용합니다.
  ...
  <Collapser id="ruby-config" title="Ruby 에이전트 구성">
    ...
    <Callout variant="important">
      OpenTelemetry 지원이 활성화되면 에이전트는 `opentelemetry-api` 이(가) 설치되어 있는지 자동으로 감지합니다. 하이브리드 에이전트 기능을 사용하려면 이 gem을 설치해야 합니다.
    </Callout>
    ...
    **예시:**
    ...
    ✅ **지원됨**: `OpenTelemetry::Trace::Span#add_link` API를 사용하는 스팬 링크

    #### Spans의 이벤트

    ✅ **지원됨**: `OpenTelemetry::Trace::Span#add_event` API를 사용하는 스팬의 이벤트
