...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        활성화하려는 계정의 [계정 ID를](/docs/accounts/accounts-billing/account-structure/account-id) 확인하세요. 그런 다음 해당 URL `https://one.newrelic.com/admin-portal/apm-agents/hsm?account=YOUR_ACCOUNT_ID` 로 이동하려면 이를 사용하십시오. 해당 페이지에서 고보안 모드를 설정할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        고보안 모드에서는 다음과 같은 애플리케이션 프로파일링 데이터 보고가 허용되지 않습니다.

        * [JFR을 사용한 자바 실시간 프로파일링](/docs/apm/agents/java-agent/features/real-time-profiling-java-using-jfr-metrics/)
      </td>
    </tr>
  </tbody>
</table>
