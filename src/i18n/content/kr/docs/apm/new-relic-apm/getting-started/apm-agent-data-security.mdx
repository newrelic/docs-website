...
New Relic이 수신하는 정보를 제한하려면 [높은 보안 모드를](#high-security) 활성화할 수 있습니다. 높은 수준의 보안 모드나 기본 설정이 비즈니스 요구에 적합하지 않은 경우 [사용자 정의](#custom) 설정을 적용할 수 있습니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="높은 보안 모드"
  >
    에이전트가 [높은 수준의 보안 모드](/docs/agents/manage-apm-agents/configuration/high-security-mode) 에 있는 경우 [기본 설정은](#default) 사용자가 변경할 수 없도록 잠깁니다.

    또한 높은 수준의 보안 모드는 에이전트에 따라 사용자 정의 이벤트, 사용자 정의 계측, 사용자 속성, 예외 메시지 또는 메시지 대기열 매개변수에 제한을 적용합니다.
