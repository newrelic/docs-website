...
APM 에이전트의 새 버전에는 기본적으로 이러한 기능이 모두 활성화되어 있으므로 지원되는 구현을 위해 APM 에이전트를 설치하면 기본적으로 로그 보고를 받을 수 있습니다.

특정 APM 에이전트에 대해 이 기능의 한 부분 또는 두 부분 모두를 비활성화할 수도 있습니다. 예를 들어 APM 에이전트가 New Relic 메타데이터를 추가하도록 하여 컨텍스트 기능에서 로그인을 사용하지만 전달을 비활성화하고 대신 다른 전달자(예: 인프라 에이전트 또는 타사 로그 서비스)를 사용할 수 있습니다. . 구성 지침은 에이전트마다 다릅니다. 로그 비활성화에 대한 자세한 내용 [은 APM에 대한 자동 로그 관리](/docs/logs/logs-context/disable-automatic-logging) 를 참조하십시오.
