...
APM 에이전트의 경우 컨텍스트 로그인 기능은 다음 두 가지 작업을 수행합니다.

* 중요한 New Relic 메타데이터(예: `span.id` , `trace.id` , `hostname` , `entity.guid` , `entity.name` 등)로 앱 로그를 장식하여 다른 New Relic 플랫폼 환경과 관련하여 로그를 볼 수 있습니다.
* 로그를 New Relic에 직접 전달하므로 타사 도구가 필요하지 않습니다.

APM 에이전트의 새 버전에는 기본적으로 이 두 가지 기능이 모두 활성화되어 있으므로 지원되는 프레임워크에 대한 APM 에이전트를 설치하면 응용 프로그램 로그 보고를 바로 사용할 수 있습니다.

특정 APM 에이전트에 대해 이 기능의 한 부분 또는 두 부분 모두를 비활성화할 수도 있습니다. 예를 들어 APM 에이전트가 New Relic 메타데이터를 추가하도록 하여 로그 데코레이션 기능을 사용할 수 있지만 로그 전달을 비활성화하고 대신 별도의 로깅 에이전트(예: 인프라 에이전트 또는 타사 로깅)를 사용할 수 있습니다. 에이전트). 구성 지침은 에이전트마다 다릅니다. 로그 비활성화에 대한 자세한 내용 [은 APM에 대한 자동 로그 관리](/docs/logs/logs-context/disable-automatic-logging) 를 참조하십시오.

컨텍스트에서 로그의 기능에 대해 자세히 알아보려면 [예제 사용 사례](#response-time-example) 를 참조하십시오. 이 예는 엔지니어링 팀이 앱의 느린 응답 시간과 증가하는 오류율 문제를 해결하기 위해 이를 어떻게 사용했는지 설명합니다.

로그 보고 옵션에 대한 개요는 로그 [소개를](/docs/logs/get-started/get-started-log-management#integrate-logs) 참조하십시오.
...
최신 APM 에이전트에는 기본적으로 활성화된 컨텍스트의 로그(로그 데코레이션 및 로그 전달)가 있습니다. 다음은 해당 문서에 대한 링크와 함께 컨텍스트에서 로그 및 로그 전달을 지원하는 APM 에이전트에 대한 정보입니다.
...
* Kubernetes를 사용하는 경우 Kubernetes API가 아닌 계측을 통해 로그를 장식한다는 점에 유의하십시오. 이것은 파일 시스템에 로그를 기록하는 것과 별개입니다. 로그는 호스트에 닿지 않거나 API를 호출할 수 있는 위치에 존재하지 않습니다.
