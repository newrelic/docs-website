...
당사의 [로그 인 컨텍스트 기능](/docs/logs/logs-context/logs-in-context) 을 사용하면 다른 New Relic 데이터의 컨텍스트에서 로그 데이터를 볼 수 있으므로 더 쉽게 상관 관계를 찾고 문제를 해결할 수 있습니다. 또한 APM 에이전트는 애플리케이션 로그 데이터에 관련 속성을 자동으로 추가할 수 있습니다. 이렇게 하면 컨텍스트에서 로그를 매우 쉽게 사용할 수 있습니다. 애플리케이션은 에이전트를 사용하여 로그 데이터를 New Relic에 직접 전달할 수도 있습니다.

로그는 로그가 속한 애플리케이션, 트랜잭션 또는 오류의 맥락에서 더 가치가 있습니다. 컨텍스트 기능에서 로그를 사용하면 APM에서 직접 애플리케이션, 분산 추적 및 오류의 로그를 볼 수 있으므로 문제 해결 방식이 변경됩니다.
...
## 로그인 컨텍스트의 이점 [#benefits]

APM 로그인 컨텍스트를 사용하여 다음을 수행할 수 있습니다.
...
2. [최신 APM 에이전트 버전](#agents) 으로 업데이트합니다.
...
당사의 최신 APM 에이전트는 타사 소프트웨어를 설치하거나 유지 관리할 필요 없이 자동으로 컨텍스트를 추가하고 로그를 전달합니다. 로그에는 `span.id` , `trace.id` , `hostname` , `entity.guid` , `entity.name` 등과 같은 속성이 자동으로 포함됩니다. 이 메타데이터는 로그를 추적, 범위, 인프라 데이터 및 New Relic의 기타 원격 분석에 연결하여 문제를 더 쉽게 해결할 수 있도록 합니다.

다음은 해당 문서에 대한 링크와 함께 컨텍스트에서 로그 및 로그 전달을 지원하는 APM 에이전트에 대한 정보입니다.
...
APM 에이전트가 자동 컨텍스트 로그인 솔루션을 아직 지원하지 않는 경우 컨텍스트 솔루션의 [수동 로그를](/docs/logs/logs-context/logs-in-context/#enable-logs) 계속 사용하고 인프라 에이전트 또는 지원되는 타사 전달자를 통해 로그를 전달할 수 있습니다.
