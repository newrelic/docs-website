...
<CollapserGroup>
  ...
  >
    1. 다음 지침에 따라 APM 에이전트를 비활성화합니다.
       ...
       * [**PHP**](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-enabled): `newrelic.so` 및 [`newrelic-daemon`](/docs/agents/php-agent/getting-started/new-relic-daemon-processes) 구성요소를 모두 제거하거나 비활성화합니다. `newrelic.enabled` `false`로 설정합니다.
    ...
    3. UI(및 [브라우저 모니터링의 모든 관련 앱)에서](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#select-apm-app) APM 애플리케이션을 제거하려면 다음 단계를 사용하십시오.
       ...
       * **UI에서 앱을 삭제합니다.** **[one.newrelic.com > All capabilties](https://one.newrelic.com/all-capabilities) > APM & services > (앱/서비스 선택) > Settings > Applicatio**으로 이동 하고 **Delete application** 버튼을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    * **UI에서 삭제하십시오.** **[one.newrelic.com > All capabilties](https://one.newrelic.com/all-capabilities) > Browser > (앱 선택) > Settings > Application settings**으로 이동하고 **Delete application** \[애플리케이션 삭제를]버튼을 클릭합니다.
  </Collapser>
  ...
  >
    ...
    * **UI를 사용하여 제거하십시오.** **[one.newrelic.com > All capabilties](https://one.newrelic.com/all-capabilities) > Mobile > (앱 선택) > Settings > Application** 으로 이동하고 **Delete application** \[애플리케이션 삭제를] 클릭합니다.
