...
<CollapserGroup>
  ...
  >
    1. 다음 지침에 따라 APM 에이전트를 비활성화합니다.
       ...
       * <DNT>[**Go**](/docs/agents/go-agent/instrumentation/go-agent-configuration#enabled)</DNT>

         : `Enabled` `false` 로 설정합니다.

       * <DNT>[**Java**](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-agent_enabled)</DNT>

         : `agent_enabled` `false` 로 설정합니다.

       * <DNT>[**.NET**](/docs/agents/net-agent/installation-configuration/net-agent-configuration#app-cfg-agent-enabled)</DNT>

         : `Newrelic.AgentEnabled` `false` 로 설정합니다.

       * <DNT>[**Node.js**](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#enabled)</DNT>

         : `agent_enabled` `false` 로 설정합니다.

       * <DNT>[**PHP**](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-enabled)</DNT>

         : `newrelic.so` 및 [`newrelic-daemon`](/docs/agents/php-agent/getting-started/new-relic-daemon-processes) 구성요소를 모두 제거하거나 비활성화합니다. `newrelic.enabled` `false` 로 설정합니다.

       * <DNT>[**Python**](/docs/agents/python-agent/installation-configuration/python-agent-configuration#monitor_mode)</DNT>

         : `monitor_mode` `false` 로 설정합니다.

       * <DNT>[**Ruby**](/docs/agents/ruby-agent/installation-configuration/ruby-agent-configuration#agent_enabled)</DNT>
    ...
    3. UI(및 [브라우저 모니터링의 모든 관련 앱)에서](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#select-apm-app) APM 애플리케이션을 제거하려면 다음 단계를 사용하십시오.
       ...
       * <DNT>**Delete the app from the UI.**</DNT>

         <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app/service) > Settings > Application**</DNT>

         으로 이동하여

         <DNT>**Delete application**</DNT>
  ...
  >
    ...
    * <DNT>**Delete an associated APM app.**</DNT>

      브라우저 앱이 [APM 애플리케이션에 연결된](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#select-apm-app) 경우 APM 애플리케이션을 삭제하면 브라우저 애플리케이션도 제거됩니다. [APM 앱 삭제 지침을](#remove-apm-apps) 참조하세요.

    * <DNT>**Delete it from the UI.**</DNT>
  ...
  >
    ...
    * <DNT>**Remove it using the UI.**</DNT>

      <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > Settings > Application**</DNT>
    ...
    * <DNT>**Use NerdGraph to delete an entity.**</DNT>

      위의 작업을 완료했는데도 UI 에 해당 앱이 계속 표시된다면 [NerdGraph를 사용하여 관련 엔터티를 삭제할](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/#delete-entities) 수 있습니다. 엔터티 ID를 찾는 방법은 [엔터티를](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic/#find) 참조하세요.
  </Collapser>
</CollapserGroup>
