...
**Python 에이전트만 해당:** Apache/mod_wsgi 3.4 이상을 사용하는 경우 mod_wsgi는 각 요청에 대해 `X-Queue-Start` 헤더에 해당하는 항목을 WSGI Environ 사전에 자동으로 삽입합니다. 이는 Apache가 처음 요청을 수락한 특정 시점을 표시합니다. 별도의 `X-Request-Start` 또는 `X-Queue-Start` 헤더가 웹 서버의 프런트엔드 또는 Apache 자체에 수동으로 구성되지 않은 경우 mod_wsgi에 의해 설정된 값이 선택되어 Python 에이전트에서 사용됩니다.
