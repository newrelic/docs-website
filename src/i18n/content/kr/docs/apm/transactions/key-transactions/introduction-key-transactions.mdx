...
<CollapserGroup>
  ...
  >
    ...
    귀하가 소음을 생성한 후에는 뉴 사우스 릭스 생태계의 일부가 됩니다. <DNT>**entity list view**</DNT> 아래에서 방금 생성한 `getPlans` 가전을 포함하여 모든 프로세서를 볼 수 있습니다.
  </Collapser>
  ...
  >
    ...
    <Callout variant="important">
      주요 트랜잭션 Apdex 설정은 해당 앱에 설정된 T 값을 재정의합니다. 예를 들어, 주요 트랜잭션은 <DNT>**Satisfied**</DNT> 으로 보고될 수 있지만 동일한 응답 시간을 가진 동일한 앱의 다른 트랜잭션은 <DNT>**Frustrated**</DNT> 일 수 있습니다.
    </Callout>
...
APM에서는 [주요 트랜잭션](/docs/apm/transactions/key-transactions/introduction-key-transactions/) 에 <DNT>**tag**</DNT> 를 추가하여 팀, 환경 또는 우선순위 수준을 빠르게 결정할 수 있도록 주요 트랜잭션을 구성하고 필터링할 수 있습니다. 이러한 태그는 또한 공지 공지를 받을 때 문제를 격리할 수 있도록 컨텍스트를 제공합니다.
...
<CollapserGroup>
  ...
  >
    ...
    1. <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT> 으로 이동하여 주요 프로세서를 선택하세요.

    2. 페이지 상단의 파티 이름 근처에서 <DNT>**See metadata and manage tags**</DNT> 메뉴를 선택하세요.

    3. 팝업 메뉴의 <DNT>**Add a new tag...**</DNT> 필드에 `accountId: 123456789` 형식의 태그를 추가합니다.
