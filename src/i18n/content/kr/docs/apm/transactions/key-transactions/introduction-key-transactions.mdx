...
귀하의 팀은 `PlanService` 의 가장 중요한 비즈니스 거래인 `getPlans` (계획 페이지를 로드함)를 식별하는 것으로 시작합니다. 그런 다음 `getPlans` 주요 거래로 만들고 관련 태그를 적용하고 더 큰 [엔티티 생태계](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) 의 맥락에서 분석해야 합니다.
...
<CollapserGroup>
  ...
  >
    ...
    4. 프로세서 세부정보 보기에서
       ...
       <DoNotTranslate>**Make key transaction**</DoNotTranslate>

       선택합니다. REST API를 사용하여 주요 트랜잭션 추적을 생성 및 중지함으로써 이 단계를 자동화할 수도 있습니다.
    ...
    귀하가 소음을 생성한 후에는 뉴 사우스 릭스 생태계의 일부가 됩니다. <DoNotTranslate>**entity list view**</DoNotTranslate> 아래에서 방금 생성한 `getPlans` 가전을 포함하여 모든 프로세서를 볼 수 있습니다.
  </Collapser>
  ...
  >
    ...
    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Key transactions**</DoNotTranslate>
  ...
  >
    ...
    <Callout variant="important">
      주요 트랜잭션 Apdex 설정은 해당 앱에 설정된 T 값을 재정의합니다. 예를 들어, 주요 트랜잭션은 <DoNotTranslate>**Satisfied**</DoNotTranslate> 으로 보고될 수 있지만 동일한 응답 시간을 가진 동일한 앱의 다른 트랜잭션은 <DoNotTranslate>**Frustrated**</DoNotTranslate> 일 수 있습니다.
    </Callout>
    ...
    2. 색인에서 이름을 바꾸려는
       ...
       <DoNotTranslate>**key transaction**</DoNotTranslate>

       을 선택합니다.

    3. 왼쪽 탐색의 설정 영역에서 애플리케이션을 선택합니다.

    4. <DoNotTranslate>**Apdex T**</DoNotTranslate>

       제목 아래에 새 값(초)을 입력한 후 저장합니다.
  ...
  >
    ...
    4. <DoNotTranslate>**stop tracking**</DoNotTranslate>
...
APM에서는 [주요 트랜잭션](/docs/apm/transactions/key-transactions/introduction-key-transactions/) 에 <DoNotTranslate>**tag**</DoNotTranslate> 를 추가하여 팀, 환경 또는 우선순위 수준을 빠르게 결정할 수 있도록 주요 트랜잭션을 구성하고 필터링할 수 있습니다. 이러한 태그는 또한 공지 공지를 받을 때 문제를 격리할 수 있도록 컨텍스트를 제공합니다.
...
<CollapserGroup>
  ...
  >
    ...
    1. <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> 으로 이동하여 주요 프로세서를 선택하세요.

    2. 페이지 상단의 파티 이름 근처에서 <DoNotTranslate>**See metadata and manage tags**</DoNotTranslate> 메뉴를 선택하세요.

    3. 팝업 메뉴의 <DoNotTranslate>**Add a new tag...**</DoNotTranslate> 필드에 `accountId: 123456789` 형식의 태그를 추가합니다.
...
주요 트랜잭션을 생성한 후에는 이를 <InlinePopover type="alerts"/>및 응용 인텔리전스 도구 및 워크로드에 연결하여 가시성을 높일 수 있습니다.
