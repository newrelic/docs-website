...
[APM](/docs/apm/new-relic-apm) 에서 <DNT>**transaction trace**</DNT> 는 애플리케이션의 단일 [트랜잭션](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) 에 대한 자세한 스냅샷을 제공합니다. A 프로세서 트레이스는 사용 가능한 함수 호출, 데이터베이스 호출 및 외부 호출을 기록합니다. 프로세서 트레이스를 사용하여 성능 문제를 해결하고 앱 작동 방식에 대한 자세한 하위 수준 인사이트 정보를 얻을 수 있습니다.
...
* 1분 길이의 [수확 주기](/docs/new-relic-solutions/get-started/glossary/#harvest-cycle) 동안 임계값( [Apdex T 값의](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) 4배 또는 특정 시간(초))을 위반하는 모든 트랜잭션이 트랜잭션 풀에 추가됩니다.
...
2. [추가 세부정보를](/docs/apm/transactions/transaction-traces/transaction-traces-summary-page) 보려면

   <DNT>
     **Transaction traces**
   </DNT>

   섹션에서 프로세서 트레이스를 클릭하세요.
...
1. <DNT>
   ...
   [페이지](/docs/apm/transactions/transaction-traces/transaction-traces-trace-details-page) 로 이동합니다.
