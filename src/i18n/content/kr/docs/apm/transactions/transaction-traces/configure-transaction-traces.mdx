...
서버 측 설정 설정을 뉴렐릭 에이전트에 사용할 수 있는 경우 다음에서 확인할 수 있습니다: <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Application > Server-side agent configuration**</DNT>.
...
* 주요 프로세서가 프로세서 트레이스를 생성할 가능성을 높이려면
...
<Callout variant="important">
  표준 Apdex 설정이 아닌 초 수로 설정된 레즈비언 트레이스 레버 값 [을](#fields) 사용하는 경우, 초 수 레즈비언 규칙은 <DNT>**all**</DNT> 바이올린에 대한 경계 값으로 작동합니다. 사용자 정의 Apdex 설정이 있는 주요 프로세서의 경우.
</Callout>
...
[데이터 보안상의 이유로](/docs/apm/transactions/transaction-traces/security-options-transaction-traces) 칩셋 트레이스는 잠재적으로 민감한 HTTP 요청 속성( <DNT>**parameters**</DNT> 이라고도 함)을 수집하지 않습니다. 트레이스는 뉴렐릭이 <DNT>**agent attributes**</DNT> 이라고 부르는 몇 가지 기본 HTTP 요청 속성을 수집합니다. 속성 수집 설정을 편집하려면 특정 뉴렐릭 에이전트를 참조하세요.
...
<CollapserGroup>
  <Collapser
    ...
    title="모든 거래 추적 삭제"
  >
    선택한 앱의 <DNT>**all**</DNT> 트레이스를 영구적으로 삭제하려면 다음 단계를 따르세요.

    1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitor > Transactions**</DNT> 으)로 이동합니다.

    2. <DNT>**Delete all traces**</DNT> 을(를) 클릭합니다.
