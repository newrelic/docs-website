...
두 가지 주요 APM [트랜잭션](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) 범주는 **웹** 및 **비웹** 입니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="C SDK"
  >
    [웹이 아닌 트랜잭션을 계측하기](/docs/agents/c-sdk/instrumentation/instrument-your-app-c-sdk#instrument-c-txn) 위한 절차를 따르십시오.
  </Collapser>
  ...
  <Collapser
    ...
    title=".그물"
  >
    기본적으로 APM .NET 에이전트는 IIS ASP 작업자를 계측합니다. 독립 실행형(WCF) 서비스, 콘솔 앱 및 기타 Windows 서비스와 같은 기타(비 IIS) .NET 응용 프로그램을 계측하려면 [사용자 지정 응용 프로그램](/docs/agents/net-agent/features/instrumenting-custom-applications) [계측 및 WCF 응용 프로그램](/docs/agents/net-agent/features/instrumenting-wcf-applications) 계측을 참조하세요.
...
New Relic UI에서 웹이 아닌 거래 데이터를 보려면 다음 페이지의 기본 차트로 이동한 다음 드롭다운에서 **웹이 아닌** 옵션을 선택합니다.

* New Relic Explorer: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Explorer > (앱 선택) > Summary** 이동합니다.
* APM: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Summary**으로 이동합니다.
* 트랜잭션: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (앱 선택) > Monitor > Transactions** 으로 이동합니다.