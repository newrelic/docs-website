...
[Heroku](https://devcenter.heroku.com/articles/newrelic) 는 다양한 에이전트 언어로 웹 제작을 호스팅하기 위한 PaaS(Platform as a Service) 솔루션입니다. 뉴렐릭을 사용하면 <InlinePopover type="apm"/>및 <InlinePopover type="browser"/>과 같은 모니터링 솔루션의 지표로 Heroku 확장할 수 있습니다. 뉴렐릭 추가 기능은 Java, Node.js, PHP, 파이썬, 루비.
...
[Heroku용 뉴렐릭 부가 기능을](https://devcenter.heroku.com/articles/newrelic) 설치한 후 <InlinePopover type="apm"/>에이전트에 대한 설정 절차를 따르세요.
...
<CollapserGroup>
  ...
  >
    ...
    Rails가 아닌 버전으로 루비 에이전트를 사용하는 경우 Heroku 사용자는 루비 에이전트를 클러스터에 수동으로 설치해야 합니다. 예를 들어 Sinatra 앱에서 `newrelic` gem을 <DNT>**Gemfile**</DNT> 에 추가한 후 앱에 다음 코드를 추가합니다.
...
<CollapserGroup>
  <Collapser
    ...
    title="라이센스 키"
  >
    <InlinePopover type="licenseKey"/>는 애플리케이션이 보고하는 계정을 식별합니다. 앱에서 사용하는 라이선스 키를 확인하려면 다음 안내를 따르세요.
...
<CollapserGroup>
  ...
  >
    ...
    3. 추가 기능 목록에서 New Relic을 선택합니다.
       ...
       Heroku 통해 로그인하는 경우 <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) > Switch account**</DNT>을(를) 선택하면 일반 뉴렐릭 계정이 표시되지 않습니다.
  </Collapser>
  ...
  >
    ...
    2. 일반 New Relic 계정에서 다른 계정으로 전환하려면: [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 로 이동하여 계정을 선택하십시오.
       ...
       뉴렐릭을 통해 직접 로그인하는 경우 <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) > Switch account**</DNT>을 선택하면 Heroku 의 뉴렐릭 추가 기능 계정이 표시되지 않습니다.
  </Collapser>
</CollapserGroup>

## 배포 알림 설정 [#deployment]

Heroku 추가 기능은 계정당 <DNT>**one application**</DNT> 에 대한 배포 공지를 뉴렐릭에게 자동으로 보냅니다. 추가 기능 계정에 여러 기능을 추가하는 경우 뉴렐릭 REST API 사용하여 추가 기능에 대한 배포 공지를 수동으로 보내야 합니다.
...
<Callout variant="important">
  Heroku 추가 기능에 사용자를 추가하면 뉴렐릭의 사용자에 대한 사용자 레코드가 생성됩니다. 하지만 Heroku 애드온에서 사용자를 제거해도 뉴렐릭에서는 사용자 기록이 자동으로 제거되지 않습니다. 대신, Heroku 추가 기능에서 사용자를 제거한 후 뉴렐릭 사용자 레코드도 수동으로 제거해야 합니다. <DNT>**User Management**</DNT> 페이지로 이동하면 됩니다.
</Callout>
