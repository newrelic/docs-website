...
<CollapserGroup>
  ...
  >
    ...
    Rails가 아닌 애플리케이션과 함께 Ruby 에이전트를 사용하는 경우 Heroku 사용자는 저장소에 플러그인을 수동으로 설치해야 합니다. 예를 들어, Sinatra 앱에서 `newrelic` gem을 **Gemfile** 에 추가한 후 앱에 다음 코드를 추가합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="계정 URL"
  >
    New Relic 애드온을 여러 번 설치하고 Heroku 앱이 New Relic에 보고하는 데 사용하는 URL을 확인해야 하는 경우 에이전트 로그 `reporting to` 하는 행을 찾습니다.
...
<CollapserGroup>
  ...
  >
    ...
    3. 추가 기능 목록에서 New Relic을 선택합니다.
       ...
       Heroku를 통해 로그인하면 **[계정 드롭다운](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) > 계정 전환** 을 선택할 때 일반 New Relic 계정이 표시되지 않습니다.
  </Collapser>
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com)** 에서 New Relic에 로그인합니다.

    2. 일반 New Relic 계정에서 다른 계정으로 전환하려면: \*\*[계정 드롭다운](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) 으로 이동하여 계정을 선택합니다.

       New Relic을 통해 직접 로그인하는 경우 **[계정 드롭다운](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#account-dropdown) > 계정 전환** 을 선택할 때 Heroku의 New Relic 추가 기능 계정이 표시되지 않습니다.
  </Collapser>
</CollapserGroup>
