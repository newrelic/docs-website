...
1. <DNT>**authentication domain ID**</DNT>
   ...
   ( [뉴렐릭 인증 UI의](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more) 최상위 및

   <DNT>**SCIM bearer token**</DNT>

   (인증 UI 에서 "SAML 2.0 엔드포인트로))을 가져와 OneLogin 앱의 해당 필드에 입력하세요.

2. 다음 섹션에 설명된 모든 설정이 완료될 때까지

   <DNT>**API Connection**</DNT>

   비활성화된 상태로 둡니다. 모든 설정을 완료한 후 연결을 활성화하세요.
...
사용자와 함께 New Relic에 보낼 그룹 유형을 결정하세요. 조직에서 Active Directory 또는 LDAP를 사용하는 경우 보안 그룹을 사용하여 New Relic에서 사용자 권한을 정의하도록 선택할 수 있습니다. 또 다른 합리적인 그룹 선택은 OneLogin 역할입니다.
...
7. 중요: [많은 뉴렐릭 기능이 해당 설정을 사용하기](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting) 때문에 사용자의 시간대를 업데이트하는 것이 중요합니다. 기본 형식은 UMT입니다.

   <DNT>**Edit New Relic by Organization login for user**</DNT>

   페이지에서 IANA 시간대 데이터베이스 형식(Olson 시간대 데이터베이스 형식이라고도 함)으로 사용자 시간대를 입력하고

   <DNT>**Save**</DNT>

   클릭합니다. 사용자는 [자신만의 시간대를 설정할](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting) 수도 있습니다.

8. <DNT>**Roles**</DNT>

   사용하여 뉴렐릭 그룹을 정의하는 경우 사용자의 특수 페이지에서 사용자에게 적합한 역할을 클릭한 다음
