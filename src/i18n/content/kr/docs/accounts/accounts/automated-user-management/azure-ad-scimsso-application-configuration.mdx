...
[자동화된 사용자 관리](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) 를 통해 SCIM을 통해 ID 공급자로부터 New Relic 사용자를 가져오고 구성할 수 있습니다. 이 가이드는 New Relic Azure AD SCIM/SSO 애플리케이션을 구성하는 방법에 대한 Azure AD 관련 세부 정보를 제공합니다.
...
5. 다음 섹션에서 사용자 및 그룹 구성을 완료할 때까지 **Enabled** 스위치를 **Off** 로 설정한 상태로 두십시오. 모든 구성이 준비되면 이 페이지로 돌아와 스위치를 **On** 으로 설정하십시오.

6. **저장** 을 클릭합니다.

다음은 기본값으로 채워진 속성 매핑 페이지의 예입니다. 상황에 따라 값이 다르게 구성될 수 있습니다.

![속성을 설정하는 방법을 보여주는 화면 캡처.](./images/Azure_AD_Provisioning_Attribute_Mapping_2_0.png "Azure_AD_Provisioning_Attribute_Mapping.png")
