...
4. 다음으로 이 필드를 갖도록 Okta 사용자 프로필을 구성합니다. 단계:
   ...
   * 해당 프로필에 새로운 **New Relic user type** \[New Relic 사용자 유형] 속성을 추가합니다([Okta 사용자 프로필 지침 참조](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas)). 이를 설정하는 방법은 사용자 유형 정의에 대한 자체 설정 및 기본 설정에 따라 다릅니다. 사용자 유형의 예상 값은 `Basic User`, `Core User` 및 `Full User` 입니다. 아래는 정보가 채워진 예시입니다.
