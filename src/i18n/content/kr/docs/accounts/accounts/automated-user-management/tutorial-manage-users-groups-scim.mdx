...
이 튜토리얼은 SCIM API를 사용하여 [New Relic One 사용자 모델](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 에서 사용자를 관리하기 위한 몇 가지 일반적인 절차를 안내합니다. SCIM API를 사용하면 [사용자 관리 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 외부에서 프로그래밍 방식으로 사용자 및 그룹을 보고, 만들고, 업데이트하고, 삭제할 수 있습니다.
...
사용자 그룹을 New Relic으로 가져오는 작업을 마치면 **조직 및 액세스** UI를 사용하여 그룹에 특정 역할 및/또는 계정에 대한 액세스 권한을 부여하는 액세스 권한을 생성해야 합니다. 자세한 내용은 [사용자 관리 개념](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts) 을 참조하십시오.
...
SCIM API를 사용하여 `/scim/v2/Users` 에 `POST` 요청을 보내 사용자를 생성할 수 있습니다. 다음 사용자 속성이 **필요합니다** .

* `userName` 이 식별자는 인증 도메인 내에서 고유 **해야** 합니다. 사용자의 이메일 주소를 사용합니다.
* `emails` `userName` 과 동일합니다. 사용자의 이메일 주소입니다. ( `emails` 이라고 하지만 이 절차에서는 하나만 입력하십시오.)
* `active` New Relic 내에서 사용자가 활성 또는 비활성 상태여야 하는지 여부를 나타내는 부울입니다.

최상의 사용자 경험을 위해 다음 속성을 제공하는 것이 좋습니다.

* `name.givenName` 사용자의 이름 또는 이름.
...
<Callout variant="important">
  반환된 사용자 `id` 를 기록해 두십시오. 나중에 사용자를 업데이트하려면 요청에 동일한 ID를 제공해야 합니다.
</Callout>
...
SCIM API를 사용하여 `POST` 요청을 `/scim/v2/Groups` 로 보내 그룹을 생성할 수 있습니다. **필요한** 유일한 그룹 속성은 다음과 같습니다.
...
<Callout variant="important">
  반환된 그룹 `id` 를 기록해 둡니다. 나중에 그룹이나 그 구성원을 업데이트하려면 요청에 동일한 ID를 제공해야 합니다.
</Callout>
...
SCIM API는 사용자 업데이트를 위한 `PUT` 및 `PATCH` 방법을 모두 지원합니다. `PATCH` 사용에 대한 자세한 내용은 [SCIM API 지원 작업](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management/#supported-actions) 및 [RFC 7644](https://datatracker.ietf.org/doc/html/rfc7644) 를 참조하세요. 이 튜토리얼은 `PUT` 메소드를 사용하여 사용자의 속성을 업데이트하는 방법을 보여줍니다.

New Relic은 모든 사용자 속성이 요청 본문에 포함될 필요가 **없으며** 업데이트하려는 속성만 필요합니다. `/scim/v2/Users/${ID}` 에 `PUT` 요청을 보내 사용자를 업데이트합니다.
...
SCIM API는 그룹 업데이트를 위한 `PUT` 및 `PATCH` 방법을 모두 지원합니다. 이 자습서에서는 `PATCH` 방법으로 그룹 구성원을 업데이트하는 방법을 보여줍니다. `PUT` 사용에 대한 자세한 내용은 [SCIM API 지원 작업](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management/#supported-actions) 및 [RFC 7644](https://datatracker.ietf.org/doc/html/rfc7644) 를 참조하십시오.

`PATCH` 는 요청에 전체 구성원 목록을 지정할 필요 없이 그룹 구성원을 추가하거나 제거하는 데 편리합니다. 그룹에 사용자를 추가하려면 다음 작업 매개변수를 사용하십시오.

* `op` 로 설정된 `Add`
* `members` 설정된 `path`
* 그룹에 추가할 각 사용자 ID와 함께 `{"value": "${USER_ID}"}` 목록으로 설정된 `value`

`/scim/v2/Groups/${ID}` 에 `PATCH` 요청을 보내 그룹 구성원을 업데이트합니다.
...
* `op` 로 설정된 `Remove`
* `members` 설정된 `path`
* `value` 이 그룹에서 제거할 각 사용자 ID와 함께 `{"value": "${USER_ID}"}` 목록으로 설정됨
...
인증 도메인에서 사용자를 제거하려면 `DELETE` 요청을 `/scim/v2/Users/${ID}` 로 보냅니다.
...
마찬가지로 인증 도메인에서 그룹을 제거 `DELETE` 요청을 `/scim/v2/Groups/${ID}` 로 보냅니다.
...
* 사용자 그룹이 New Relic에 있으면 사용자에게 특정 역할 및 특정 계정에 대한 액세스 권한을 부여하는 액세스 권한을 할당해야 합니다. [액세스 권한 부여에 대해 자세히 알아보세요.](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts)
...
사용자의 유형 속성은 [사용자 정의 스키마](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management#user-type) `urn:ietf:params:scim:schemas:extension:newrelic:2.0:User` 에 정의되어 있습니다. 생성 또는 업데이트 요청에 이 스키마와 `nrUserType` 문자열 속성을 포함하여 사용자 유형을 설정합니다.

`nrUserType` 의 유효한 값은 다음과 같습니다.

* `전체 사용자`
* `핵심 사용자`
* `기본 사용자`

새 `Basic user` 를 만들려면 `POST` 요청 `/scim/v2/Users` 를 보내고 사용자 지정 New Relic 스키마 확장을 포함합니다.
...
사용자 유형을 업데이트하려면 `PUT` 요청 `scim/v2/Users/${ID}` 를 보내고 사용자 지정 New Relic 스키마 확장을 포함합니다.
