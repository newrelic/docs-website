...
New Relic 앱(Azure AD, Okta 및 OneLogin)이 있는 ID 공급자가 있는[경우 해당](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) . SCIM API는 해당 ID 제공자를 사용하지 않거나 앱에서 사용할 수 없는 추가 구성(예: **사용자 유형 관리** )에 SCIM API를 사용하려는 조직을 위한 것입니다. 추가 제한 사항:
