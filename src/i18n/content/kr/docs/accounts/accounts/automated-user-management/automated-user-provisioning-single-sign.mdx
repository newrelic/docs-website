...
5. 권장 사항: ID 공급자에 시간대를 설정하세요. 이 작업이 수행되는 방법은 사용하는 서비스에 따라 다릅니다. 시간대를 설정하지 않으면 UI는 UTC 시간대("Olson" 형식이라고도 알려진 IANA 형식으로 지정됨(예: "America/Los_Angeles"))를 사용합니다. 사용자는 설정을 재정의하고 [자신의 시간대를 설정할](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting) 수도 있습니다.
