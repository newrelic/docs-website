...
New Relic을 사용하면 Azure AD, Okta 또는 OneLogin과 같은 ID 공급자에서 New Relic 사용자를 가져오고 업데이트하고 비활성화할 수 있는 AUM(자동 사용자 관리)을 설정할 수 있습니다.
...
* 싱글 사인온(SSO)을 위한 SAML 2.0 표준을 지원합니다.

* SCIM 2.0 표준을 지원합니다.

* [사용자 모델](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 관련 요구 사항:

  * 이 기능을 사용하려면 [New Relic One 사용자 모델](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 에 있어야 하며 해당 모델에 사용자를 생성합니다. [원래 사용자 모델](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 을 사용 중이라면 New Relic 계정 담당자에게 문의하세요.
  * 구성에는 [**인증 도메인 관리자** 역할](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#roles) 이 필요합니다( [기본 그룹 **Admin**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#default-groups) 의 사용자에게 이 역할이 있음).

* New Relic 앱이 있는 세 가지 ID 공급자(Azure AD, Okta 및 OneLogin)가 있습니다. 다른 ID 제공자의 경우 [SCIM API](/docs/accounts/accounts/automated-user-management/scim-support-automated-user-management) 를 사용할 수 있습니다.

* 이를 활성화하기 전에 먼저 ID 제공자 서비스에서 [사용자 그룹](/docs/accounts/accounts/automated-user-management/roles-permissions-automated-user-management) 을 설정하고 해당 그룹이 액세스할 수 있는 New Relic 역할 및 계정을 생각하는 것이 좋습니다.
...
1. 먼저 [요구 사항](#requirements) 을 검토하는 것이 중요합니다.
2. [인증 도메인 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#ui) 에서 새 인증 도메인을 만듭니다.
