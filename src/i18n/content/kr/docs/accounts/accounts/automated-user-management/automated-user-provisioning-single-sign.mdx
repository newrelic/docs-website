...
4. 위 서비스 중 하나를 <DoNotTranslate>**don't**</DoNotTranslate> 사용하는 경우 다음을 수행해야 합니다.
...
5. <DoNotTranslate>**Recommended**</DoNotTranslate>: ID 공급자에 시간대를 설정합니다. 이 작업이 수행되는 방법은 사용하는 서비스에 따라 다릅니다. 시간대를 설정하지 않으면 UI는 UTC 시간대("Olson" 형식이라고도 알려진 IANA 형식으로 지정됨(예: "America/Los_Angeles"))를 사용합니다. 사용자는 설정을 재정의하고 [자신만의 시간대를 설정할](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting) 수도 있습니다.
