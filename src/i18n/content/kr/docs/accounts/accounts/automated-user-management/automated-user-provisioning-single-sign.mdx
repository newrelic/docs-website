...
* [인증 도메인이](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more) SCIM으로 설정되면

  <DNT>**must**</DNT>

  SCIM을 사용하여 그룹과 사용자를 관리할 수 있으며 UI를 통해 관리할 수는 없습니다. 인증 도메인이 SCIM으로 설정되면 비SCIM 설정으로 변경할 수 없습니다.
...
* 권한 관련 요구 사항:
  ...
  * [사용자 유형](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) 이 핵심 사용자 또는 전체 플랫폼 사용자여야 하며
    ...
    [ 관리자 설정이](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings) 있는 그룹에 속해 있어야 합니다.
...
4. 위 서비스 중 하나를 <DNT>**don't**</DNT> 사용하는 경우 다음을 수행해야 합니다.
...
5. <DNT>**Recommended**</DNT>: ID 공급자에 시간대를 설정합니다. 이 작업이 수행되는 방법은 사용하는 서비스에 따라 다릅니다. 시간대를 설정하지 않으면 UI는 UTC 시간대("Olson" 형식이라고도 알려진 IANA 형식으로 지정됨(예: "America/Los_Angeles"))를 사용합니다. 사용자는 설정을 재정의하고 [자신만의 시간대를 설정할](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting) 수도 있습니다.
