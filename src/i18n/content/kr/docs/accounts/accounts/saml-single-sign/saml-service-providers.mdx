...
뉴렐릭 SSO 설정 페이지를 찾으려면 [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 <DNT>**Account settings**</DNT>을 클릭하고 <DNT>**Security and authentication**</DNT>을 클릭한 다음 <DNT>**Single sign-on**</DNT>를 클릭하세요.

이 UI가 표시되지 않으면 [요구 사항](#requirements) 을 검토하세요.

SAML SSO를 최적으로 설정하는 방법은 아래 지침과 팁을 참조하세요.

## New Relic에서 지원하는 공급자 [#saml\_providers][#saml_providers]

[원래 사용자 모델](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 의 사용자는 뉴렐릭이 현재 SSO 통합을 지원하는 SAML 서비스 공급 업체 목록을 찾을 수 있습니다. 뉴렐릭 [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 <DNT>**Account settings &gt; Security and authentication &gt; Single sign-on**</DNT>을 선택하세요. 해당 UI가 표시되지 않으면 [최신 사용자 모델을](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models) 사용하고 있기 때문일 수 있습니다. 이 경우 [다른 방법을 사용하여 SAML SSO를 설정](/docs/accounts/accounts-billing/new-relic-one-user-management/introduction-saml-scim) 하게 됩니다.
...
<CollapserGroup>
  <Collapser id="domain-allow-list" title="권장: SAML SSO 프로세스를 우회하도록 도메인 소유권 주장">
    귀하의 조직이 Pro 또는 파트 에디션을 사용하는 경우, 귀하의 계정 이름을 우리의 사용자 목록에 배치하도록 요청할 수 있으며, 이는 SAML SSO 활성화 프로세스를 간소화합니다. 귀하의 사용자 이메일 주소에 귀하가 신청한 인구와 일치하는 조각이 있는 경우, 뉴렐릭은 자동으로 해당 사용자를 <DNT>**Active**</DNT> 사용자로 추가하고 현재 사용자 유형을 유지합니다.
  ...
  <Collapser id="set-up-saml" title="SAML SSO 설정">
    ...
    2. 이동: <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) &amp;gt; Account settings &amp;gt; Security and authentication &amp;gt; Single sign-on**</DNT>.

    3. <DNT>**SAML single sign-on**</DNT> 페이지에서 뉴렐릭 SAML 서비스 공급업체 세부정보를 검토하세요.

    4. SAML ID 공급자 인증서를 업로드하려면 <DNT>**Choose file**</DNT> 선택한 다음 표준 절차에 따라 파일을 선택하고 저장합니다.

    5. 사용자가 Single Sign-On에 사용할 <DNT>**Remote login URL**</DNT> 을 지정합니다.

    6. 조직의 SAML 통합이 로그아웃을 위한 리디렉션 URL을 제공하는 경우 <DNT>**Logout landing URL**</DNT>; 그렇지 않으면 비워 두십시오.
    ...
    7. 변경 사항을 저장합니다.
       ...
       SSO 설정을 올바르게 구성하고 저장하면 <DNT>**Test**</DNT> 페이지가 자동으로 나타납니다. 각 테스트 후에 뉴렐릭은 진단 결과가 포함된 SAML SSO 페이지로 돌아갑니다.

       돌아가서 구성 설정을 변경하려면 <DNT>**1 CONFIGURE**</DNT> 선택합니다.
       ...
       <Callout variant="caution">
         [SAML SSO비활성화](/docs/subscriptions/maintaining-sso-settings) 하면 뉴렐릭이 자동으로 모든 <DNT>**Pending**</DNT> 사용자를 <DNT>**Active**</DNT>로 표시합니다. 나중에 SAML SSO 다시 활성화하기로 결정하면 뉴렐릭은 소유자를 제외한 모든 사용자를 자동으로 <DNT>**Pending**</DNT>로 표시하며 소유자는 이메일로 계정 액세스를 확인해야 합니다.
       </Callout>

       ### 세션 시간 초과에 대한 로그아웃 URL 추가 [#timeout\_url][#timeout_url]

       뉴렐릭의 <DNT>**Session configuration**</DNT> 기능을 사용하려면 [](/docs/subscriptions/setting-session-timeouts#saml_session)SAML SSO활성화된 계정에 대한 로그아웃 URL이 필요합니다. 로그아웃 URL 없이 SAML SSO 이미 구성, 테스트 및 활성화한 경우 뉴롤릭은 자동으로 계정 관리자를 채용하여 계정 소유자에게 알립니다. 또한 귀하가 계정 소유자인 경우 뉴렐릭은 자동으로 <DNT>**Session configuration**</DNT> 에서 SAML Single Sign-On으로 직접 이동하고 로그아웃 URL을 추가할 수 있는 링크를 제공합니다.

       <Callout variant="important">
         로그아웃 URL <DNT>**cannot**</DNT> 에는 URL의 아무 곳에나 `newrelic.com` 포함되어 있습니다.
       </Callout>

       <DNT>**Session configuration**</DNT> 기능에는 SAML 인증 브라우저 세션이 다시 인증되도록 [자동 시간 초과를](/docs/subscriptions/setting-session-timeouts#saml_reauthentication) 선택하는 옵션도 포함되어 있습니다.
  </Collapser>

  <Collapser id="add-users-saml" title="SAML 계정에 사용자 추가">
    SAML SSO 및 SCIM 문서에 대한 개요를 보려면 먼저 [SAML SSO 및 SCIM 소개를](/docs/accounts/accounts-billing/new-relic-one-user-management/introduction-saml-scim) 읽어보세요.
    ...
    <DNT>**without**</DNT> SAML SSO가 활성화된 조직의 경우 소유자 또는 관리자는 이메일 확인 없이 새 사용자를 추가할 수 있습니다.
    ...
    1. 계정 소유자 또는 관리자가 새 사용자를 추가합니다. 다음으로 이동하세요. <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) &amp;gt; Account settings &amp;gt; Account &amp;gt; Summary**</DNT>.

    2. [도메인을 신청](#domain-allow-list) 하지 않은 경우 사용자는 <DNT>**Pending**</DNT> 으로 표시되고 확인 이메일이 전송됩니다. (보류 중인 사용자는 공지 공지 등 뉴렐릭 제품 공지를 받을 수 없습니다.)

    3. 사용자는 이메일의 링크를 선택하여 계정을 확인하고 SAML 공급자의 로그인 URL로 연결됩니다.

    4. 사용자가 SAML SSO 엔드포인트(<DNT>Auth0</DNT>, <DNT>Okta</DNT>, <DNT>OneLogin</DNT>, <DNT>Ping Identity</DNT>, <DNT>Salesforce</DNT> 등)에 성공적으로 로그인하면 뉴렐릭은 사용자를 <DNT>**Active**</DNT>로 플래그 지정합니다.

       <Callout variant="caution">
         SAML SSO 비활성화하면 뉴렐릭이 자동으로 모든 <DNT>**Pending**</DNT> 사용자를 <DNT>**Active**</DNT>로 표시합니다. 나중에 SAML SSO 다시 활성화하기로 결정하면 뉴렐릭은 소유자를 제외한 모든 사용자를 자동으로 <DNT>**Pending**</DNT>로 표시하며 소유자는 이메일로 계정 액세스를 확인해야 합니다.
       </Callout>
  </Collapser>

  <Collapser id="saml-multiple-accounts" title="여러 계정에 대한 SAML">
    SAML 프로토콜에서 <DNT>**entity ID**</DNT> 는 SAML 공급자에 대한 서비스 공급 업체(뉴웰릭)를 고유하게 식별합니다. 뉴렐릭의 기본 엔터티 ID는 `rpm.newrelic.com` 입니다. SAML 지원 계정이 하나만 있는 경우에는 이 방법으로 충분합니다.
    ...
    2. 또한 <DNT>**Step 1. Configure**</DNT> 페이지의 <DNT>**Entity ID**</DNT> 행에서 <DNT>**Use custom entity ID**</DNT> 선택합니다.
  ...
  <Collapser id="maintain-saml" title="SSO 설정 유지">
    ...
    ### SAML SSO 정보 업데이트 [#sso\_procedures][#sso_procedures]
    ...
    3. 뉴렐릭과의 SAML 통합을 일시적으로 끄고 설정을 업데이트하려면 <DNT>**Disable SAML login**</DNT> 선택하세요.

    4. 선택사항: 기존 SAML 인증서를 변경하려면 <DNT>**Choose file**</DNT> 선택합니다. 표준 절차에 따라 파일을 선택하고 저장한 다음 저장합니다.

    5. 선택사항: 기존 SSO URL을 변경하려면 <DNT>**Remote login URL**</DNT> 또는 <DNT>**Logout landing URL**</DNT> 을 복사하여 붙여넣거나 입력한 다음 저장합니다.

       ### 이메일 주소 [#user\_email][#user_email]
    ...
    6. 이동: <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) &amp;gt; Account settings &amp;gt; Account &amp;gt; Summary**</DNT>.

    7. <DNT>**Users**</DNT> 목록에서 새 사용자 추가, 기존 사용자 역할 편집 또는 삭제 옵션을 선택합니다.
  ...
  <Collapser id="delete-sso" title="SSO 구성 삭제">
    이 기능이 적용되는 New Relic 사용자를 포함한 요구 사항은 [요구 사항](#requirements) 을 참조하십시오.
    ...
    1. SAML SSO 로그인 URL을 사용하여 New Relic에 로그인합니다.
    2. 뉴렐릭 메뉴 표시줄에서 다음을 선택합니다: <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) &amp;gt; Account settings &amp;gt; Security and authentication &amp;gt; Single sign-on**</DNT>.
    3. <DNT>**Delete SAML Configuration**</DNT> 선택합니다.
    4. 확인 프롬프트에서 <DNT>**OK**</DNT> 선택합니다.
  </Collapser>

  <Collapser id="partners-saml" title="파트너 및 SAML SSO">
