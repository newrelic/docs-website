...
[New Relic One 가격 책정 모델에는 두 가지 버전이 있으며](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/core-users-release/#overview) 현재 사용 중인 버전은 청구 대상 사용자를 계산하는 방법에 영향을 미치므로 해당 사용자를 쿼리하는 방법에 영향을 줍니다.
...
2022년 5월에 사용 이벤트에서 `estimatedCost` 속성을 더 이상 사용하지 않으며 UI에서 이번 달의 예상 비용을 제거합니다( [자세히 알아보기](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal) ). 현재 쿼리 또는 경고 조건에서 `estimatedCost` 속성을 사용하는 경우 해당 속성을 제거하거나 아래의 권장 쿼리로 교체해야 합니다.
...
<CollapserGroup>
  ...
  >
    2022년 5월에 `estimatedCost` 속성을 더 이상 사용하지 않습니다. [예상 비용 변경에 대한 자세한 내용은 위의 참고 사항을](#estimated-cost) 참조하십시오.

    다음은 이번 달의 데이터 수집 비용 추정치를 얻기 위한 쿼리입니다. 이 쿼리를 실행하려면 조직의 GB당 수집 데이터 비용을 알아야 합니다.
...
<CollapserGroup>
  ...
  >
    ...
    이 쿼리는 핵심 사용자가 있는 New Relic One 가격의 기본 버전을 사용하는 조직에 적용됩니다. 이에 대한 자세한 내용과 사용자 수 쿼리 방법은 사용자 [쿼리](#user-queries) 를 참조하세요.
  </Collapser>
  ...
  >
    2022년 5월에 `estimatedCost` 속성이 더 이상 사용되지 않습니다( [자세히 알아보기](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#estimated-cost-removal) ). 해당 속성을 사용하는 경우 여기에서 권장하는 쿼리로 대체해야 합니다.
...
<Callout variant="tip">
  복잡한 조직의 데이터 수집 관리에 대한 고급 심층 분석 [은 관찰 가능성 성숙도 프로그램의 데이터 거버넌스 모듈을](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-intro/) 참조하십시오.
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        **이것은 2022년 5월에 제거됩니다.** 자세한 내용 [은 예상 비용 제거를](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal) 참조하십시오.

        사용량 및 메트릭 비용을 기반으로 비용 추정치를 계산합니다. 이것은 인보이스에 표시되는 금액이 아니라 예상 비용입니다.
      </td>
    </tr>
  </tbody>
</table>