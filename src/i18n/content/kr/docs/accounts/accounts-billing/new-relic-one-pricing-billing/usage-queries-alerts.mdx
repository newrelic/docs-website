...
사용자 수를 표시하려면 다음을 참조하세요. [](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#usage-ui). 그보다 더 자세한 내용이 필요한 경우 아래의 NRQL 쿼리 예시를 사용할 수 있습니다. 사용자 계산 방법에 대한 자세한 내용은 [청구 가능 사용자 계산을](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/user-count-billing) 참조하세요.
...
<CollapserGroup>
  ...
  >
    ...
    * 사용자 유형별로 쿼리하는 데 사용되는 속성은 `BasicUsersBillable` 및 `FullUsersBillable` 입니다.
    ...
    그만큼 [](/docs/accounts/accounts-billing/new-relic-one-pricing-users/pricing-billing#billing-usage-ui)전체 플랫폼 사용자와 기본 사용자 수를 보여줍니다. 사용된 쿼리는 다음과 같습니다.
...
[NrMTDConsumption 이벤트는](/attribute-dictionary/?event=NrMTDConsumption) [사용량 경고](#alerts) 에 사용되어야 합니다.
...
* 우리의
  ...
  [메트릭 타임슬라이스 데이터](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data) 라고 알려진 데이터 유형을 보고합니다. 이 데이터는 다른 데이터 유형처럼 NRDB에 저장되지 않으므로 이 데이터에 `bytecountestimate()` 을 사용하려고 하면 혼란스러운 결과가 발생할 수 있습니다. 이 데이터를 쿼리할 때 지표 이름이 아닌 애플리케이션 이름으로 패싯을 지정하는 것이 좋습니다.