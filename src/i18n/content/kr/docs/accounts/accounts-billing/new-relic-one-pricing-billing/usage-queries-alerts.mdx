...
2022년 5월에 사용 이벤트에서 `estimatedCost` 속성을 더 이상 사용하지 않고 UI에서 이번 달의 예상 비용을 제거했습니다( [자세히 알아보기](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#estimated-cost-removal) ). 현재 쿼리 또는 경고 조건에서 `estimatedCost` 속성을 사용하는 경우 해당 속성을 제거하거나 아래의 권장 쿼리로 교체해야 합니다.
