...
* 우리의
  ...
  모바일 모니터링은 [메트릭 타임슬라이스 데이터](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data) 라고 알려진 데이터 유형을 보고합니다. 이 데이터는 다른 데이터 유형처럼 NRDB에 저장되지 않으므로 이 데이터에 `bytecountestimate()` 을 사용하려고 하면 혼란스러운 결과가 발생할 수 있습니다. 이 데이터를 쿼리할 때 지표 이름이 아닌 애플리케이션 이름으로 패싯을 지정하는 것이 좋습니다.