...
기존 그룹을 보려면 [계정 드롭다운](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown) 에서 **관리** 를 클릭한 다음 **액세스 관리** 를 클릭합니다. **액세스 권한 부여 및 그룹** 탭에서 기존 그룹과 할당된 액세스 권한을 볼 수 있습니다. (이에 대한 자세한 내용은 [1단계](#create-organization) 로 돌아가십시오).
...
<CollapserGroup>
  ...
  >
    ...
    3. 다음으로 **액세스 권한** 아래에서 다음 중 하나를 선택합니다.
       ...
       * **조직** : [조직 범위의 역할](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles) 에서 선택할 수 있도록 하려면 선택합니다. 조직 및 사용자 관리를 제어하는 역할입니다. (이러한 사용자는 이미 계정 범위 역할에 속해 있어야 합니다. 이는 대부분의 사용자에게 해당되지만 그렇지 않은 경우 해당 사용자가 조직에 속하지 않는다는 메시지가 표시될 수 있습니다.)
  ...
  >
    API를 통해 액세스 권한을 관리하는 방법은 [NerdGraph 사용자 관리](/docs/apis/nerdgraph/examples/nerdgraph-user-mgmt#concepts) 를 참조하십시오.
  </Collapser>
</CollapserGroup>
