...
이 글을 읽고 있다면 이미 [New Relic 조직을](/docs/accounts/accounts-billing/account-structure/new-relic-account-structure) 갖고 있을 가능성이 높습니다. [New Relic에 가입](https://newrelic.com/signup) 하면 New Relic 조직이 생성됩니다. 조직 구조는 New Relic 고객을 나타냅니다. 여기에는 계정, 사용자, 데이터 등 고객의 New Relic 사용과 관련된 모든 것이 포함됩니다.

New Relic 조직이 생성되면 단일 계정이 포함됩니다. 무료 또는 표준 버전을 사용하는 경우 조직은 단일 계정만 가질 수 있지만 Pro 및 Enterprise 버전에서는 계정을 추가할 수 있습니다. 각 계정에는 고유한 [계정 ID가](/docs/accounts/accounts-billing/account-structure/account-id) 있으며 해당 ID는 API 호출과 같은 일부 계정별 작업에 사용됩니다.
...
역할 및 권한 작동 방식에 대한 자세한 내용은 [기능 을](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#permissions) 참조하십시오.
