...
<CollapserGroup>
  ...
  <Collapser id="manage-user-type-scim" title="SCIM으로 사용자 유형 관리">
    ...
    * Azure Active Directory: 사용자 유형을 사용하여 SCIM용 Azure AD 구성에 대한 뉴렐릭 포럼 토론을 [여기에서](https://support.newrelic.com/s/hubtopic/aAX8W0000008d6MWAQ/relic-solution-make-full-core-basic-users-in-azure-active-directory-w-scim) 참조하십시오.
...
* [사용자 이름/비밀번호](#username-authentication): 사용자는 이메일과 비밀번호를 통해 로그인합니다.
* [SAML SSO](#saml): 사용자는 ID 공급자를 사용하여 SAML Single Sign-On(SSO)을 통해 로그인합니다. 설정 방법을 알아보려면 아래 설명을 확인하시기 바랍니다.

### 사용자 이름/비밀번호 인증 [#username-authentication]

이것은 기본 인증 방법입니다. 사용자는 이메일 주소와 비밀번호를 사용하여 로그인합니다. 추가 보안을 위해 [다중 인증(MFA)](#mfa)을 활성화할 수도 있습니다.

<Callout variant="important">
  모바일 앱 로그인 시 MFA 이메일 인증을 받으려면 앱에 이메일 주소를 입력한 후 모바일 브라우저에서 로그인 화면을 열어야 합니다.
</Callout>
...
다중 인증(MFA)은 인증 도메인의 사용자에게 신원을 확인하도록 요구하여 데이터에 보안 계층을 추가합니다.

활성화되면 사용자는 로그인할 때마다 인증 이메일을 받습니다. 첫 번째 MFA 필수 로그인 시, 사용자는 10개의 백업 코드도 받습니다. 이 코드는 이메일에 액세스할 수 없는 경우에 사용됩니다. 사용자에게 이러한 코드를 안전한 위치에 저장하도록 안내하십시오.

뉴렐릭 모바일 앱에 대한 MFA 인증을 완료하려면 앱에 이메일을 입력한 다음 프롬프트에 따라 모바일 브라우저에서 로그인 화면을 여세요.

**MFA를 활성화하려면:**

1. [one.newrelic.com &gt; Administration &gt; Authentication Domains](https://onenr.io/08jqqPYdvjl)으로 이동합니다
2. 필요한 도메인을 선택합니다.
3. MFA 토글을 활성화합니다. 활성화되면, 해당 인증 도메인의 사용자에게 다음 로그인 및 이후의 모든 로그인 시 이메일을 확인하라는 프롬프트가 표시됩니다.
