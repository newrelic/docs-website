...
<Callout variant="important">
  이 문서는 [New Relic One 사용자 모델](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) 의 사용자를 관리하기 위한 것입니다. 원래 사용자 모델에서 사용자를 관리하려면 [원래 사용자](/docs/accounts/original-accounts-billing/original-users-roles) 를 참조하십시오.
</Callout>
...
이러한 기능에 대한 액세스 권한이 있는지 확인하려면 [인증 도메인 설정 UI](#ui) 로 이동하여 설정을 구성할 수 있는지 확인할 수 있습니다.

이러한 설정을 구성하기 위한 요구 사항:

* 이 기능은 [New Relic One 사용자 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-users/users-roles) 의 사용자를 관리하기 위한 것입니다. 원래 사용자 모델을 사용하는 사용자의 경우 [원래 계정 관리](/docs/accounts/original-accounts-billing) 를 참조하세요.

* 이러한 설정을 구성하려면 [Pro 또는 Enterprise 버전](https://newrelic.com/pricing) 이 필요합니다.

* 이 설정을 편집하려면 [**인증 도메인 관리자** 역할](/docs/accounts/accounts-billing/new-relic-one-pricing-users/users-roles#standard-roles) 이 있는 그룹에 속해 있어야 합니다.

* 자동화된 사용자 관리라고도 하는[SCIM 프로비저닝](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) 에는 Pro 또는 Enterprise 버전이 필요합니다.

* SAML SSO에는 Pro 또는 Enterprise 버전이 필요합니다. SAML 지원에는 다음이 포함됩니다.
...
인증 도메인을 보고 구성하려면 [계정 드롭다운](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown) 에서 **관리 > 조직 및 액세스 > 인증 도메인** 으로 이동합니다.
...
## 사용자 출처: 매뉴얼 대 SCIM [#source-users]
...
3. 다음으로 인증 도메인 UI로 이동합니다. [계정 드롭다운](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#account-dropdown) 에서 **조직 및 액세스** 를 클릭한 다음 **인증 도메인** 을 클릭합니다. 아직 도메인이 없는 경우 SAML 인증 사용자에 사용할 [새 도메인을 만드](#ui) 십시오.
...
7. **귀하** 가 제공 에서 ID 제공자가 제공한 **SSO 대상 URL** 을 설정하십시오. **SAML 메타데이터의 소스로** 이동하고 POST 바인딩 URL을 찾아 이를 찾을 수 있습니다. `https://newrelic.oktapreview.com/app/newreliclr/1234567890abcdefghij/sso/saml` 처럼 보입니다.
...
11. SAML만 활성화하는 경우 New Relic에서 그룹을 생성하고 액세스 권한을 할당해야 합니다. (SCIM을 활성화했다면 이미 이 단계를 완료한 것입니다.) 액세스 권한 부여는 사용자에게 New Relic 계정에 대한 액세스 권한을 부여하는 것입니다. 액세스 권한이 없으면 사용자는 New Relic에서 프로비저닝되지만 계정 액세스 권한은 없습니다. 이 작업을 수행하는 방법을 배우려면 다음을 수행하십시오.

* [액세스 권한 부여 작동 방식 알아보기](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts)
* [액세스 권한 부여 자습서](/docs/accounts/accounts-billing/new-relic-one-user-management/tutorial-add-new-user-groups-roles-new-relic-one-user-model/) 를 읽으십시오.
...
<CollapserGroup>
  ...
  >
    기본 기능은 New Relic에서 사용자 유형을 관리하는 것입니다. [SCIM을 통해 사용자 유형을 관리](#source-users) 하지 않는 경우 사용할 옵션입니다. 이에 의해 관리되는 관리 옵션은 다음과 같습니다.
    ...
    * 사용자 유형 업그레이드 요청을 처리하는 방법을 관리하는 기능. 이러한 옵션에는 다음이 포함됩니다.
      ...
      * **사용자가 자체 업그레이드하도록 허용** : 기본 사용자가 승인 없이 직접 상위 사용자 유형으로 즉시 업그레이드할 수 있도록 합니다. 이를 통해 이러한 사용자는 문제에 보다 신속하게 대응할 수 있습니다.
      * **검토 필요** : 이 옵션을 사용하면 사용자가 업그레이드를 요청할 때 관리자( **인증 도메인 관리자** 역할을 가진 사용자)가 이메일을 수신하고 [**사용자 관리** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 에서 해당 요청을 승인하거나 거부할 수 있습니다.
  </Collapser>
