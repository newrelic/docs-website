...
<DoNotTranslate>**authentication domain**</DoNotTranslate> 은 [(는) 프로비저닝(추가 및 업데이트) 방식](#source-users) , [인증(로그인) 방식](#authentication) 등 동일한 사용자 관리 설정으로 관리되는 뉴렐릭 사용자 그룹입니다.
...
* 인증 도메인을 보거나 편집하려면 사용자는 다음을 수행해야 합니다.
  ...
  * <DoNotTranslate>[**Authentication domain**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings)</DoNotTranslate>
    ...
    [ 관리 설정이](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings) 있는 그룹에 속해 있어야 합니다.
...
인증 도메인을 보고 구성하려면 [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 <DoNotTranslate>**Administration > Authentication domains**</DoNotTranslate> 로 이동하세요.

기존 도메인이 있는 경우 왼쪽에 있습니다. 대부분의 조직에는 최대 2개 또는 3개의 도메인이 있습니다. 하나는 수동 기본 설정이고 다른 하나는 ID 공급자 관련 설정입니다.

인증 도메인 UI 페이지에서 새 도메인을 생성하려면 <DoNotTranslate>**Create authentication domain**</DoNotTranslate> 클릭합니다. 설정 옵션에 대한 자세한 내용을 보려면 계속 읽으세요.
...
* <DoNotTranslate>**Manual**</DoNotTranslate>
  ...
  [ UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 에서 뉴렐릭에 수동으로 추가됨을 의미합니다.

* <DoNotTranslate>**SCIM:**</DoNotTranslate>

  자동화된 사용자 관리 기능을 사용하면 SCIM 프로비저닝을 사용하여 ID 공급자로부터 사용자를 가져오고 관리할 수 있습니다.

이 설정에 대한 참고 사항:

* <DoNotTranslate>**Source of users**</DoNotTranslate>

  (를) 전환할 수 없습니다. 즉, 이미 설정된 인증 도메인에 대해 이를 변경하려면 새 도메인을 만들어야 함을 의미합니다.
...
<DoNotTranslate>**Authentication Domain UI**</DoNotTranslate> 에서 사용자 프로비저닝 방법으로 SCIM을 선택한 경우 [사용자 유형을](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) 관리하는 방법에 대한 두 가지 옵션이 있습니다.

* <DoNotTranslate>**Manage user type in New Relic**</DoNotTranslate>

  : 기본 옵션입니다. 뉴렐릭에서 사용자의 사용자 유형을 관리할 수 있습니다.
...
* <DoNotTranslate>**Manage user type with SCIM**</DoNotTranslate>
  ...
  이(가) 됩니다. ID 공급자를 통해서만 이를 변경하고 관리할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>[**User management**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where)</DoNotTranslate>
      ...
      [ UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 에서 사용자 유형을 변경하는 기능.
  ...
  >
    [SCIM API](/docs/accounts/accounts/automated-user-management/tutorial-manage-users-groups-scim#manage-user-type) 사용하면 뉴렐릭 사용자 관리 UI 대신 ID 공급자에서 사용자 유형을 관리할 수 있습니다. <DoNotTranslate>**When you enable this, you can no longer change or manage your users' user type from New Relic.**</DoNotTranslate>
    ...
    1. 인증 도메인
       ...
       에 [SCIM이 활성화](#source-users) 되어 있는지 확인하세요.
    ...
    * <DoNotTranslate>**Manage user type with SCIM**</DoNotTranslate>
      ...
      (를) 활성화하면 기본 옵션은 사용자 유형에 따라 액세스가 제한된 사용자에게 업그레이드하고 더 많은 액세스를 잠금 해제하려면 관리자 또는 IT 부서에 문의해야 한다는 메시지가 표시되는 것입니다.
...
2. SCIM이 아닌 SAML SSO를 활성화하고 Azure, Okta 또는 OneLogin을 사용하는 경우 <DoNotTranslate>**only**</DoNotTranslate> 관련 앱을 구성하려면 다음 지침을 따르세요.
   ...
   <CollapserGroup>
     ...
     >
       ...
       6. 조직별로 뉴렐릭 추가 페이지에서 두 개의
          ...
          클릭합니다. 나중에 설정이 완료되고 프로비저닝이 시작된 후에 기능이 표시되도록 하겠습니다.

       7. Okta에서 새로 생성된 애플리케이션을 열고

          <DoNotTranslate>**Assignments**</DoNotTranslate>

          탭으로 이동합니다. 여기에서 인증을 위한 사용자 및 그룹을 추가할 수 있습니다.
       ...
       8. 거기에서
          ...
          표시되며, 아래 일반 지침의 9단계에서 이 필드를 편집해야 합니다. 오른쪽에서
          ...
          아래에서 아래 일반 지침의 6단계와 7단계에 필요한 URL을 찾을 수 있습니다.
     </Collapser>
...
3. 다음으로 인증 도메인 UI로 이동합니다. [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 <DoNotTranslate>**Administration**</DoNotTranslate> 클릭한 다음 <DoNotTranslate>**Authentication domains**</DoNotTranslate> 클릭합니다. 아직 도메인이 없다면 SAML 인증 사용자에게 사용할 [새 도메인을 생성하세요](#ui) .

4. <DoNotTranslate>**Authentication**</DoNotTranslate> 아래에서 <DoNotTranslate>**Configure**</DoNotTranslate> 클릭합니다. <DoNotTranslate>**Method of authenticating users**</DoNotTranslate> 아래에서 <DoNotTranslate>**SAML SSO**</DoNotTranslate> 선택합니다.

5. Okta, OneLogin 또는 Azure AD 앱을 사용하는 경우 이 단계를 건너뛸 수 있습니다. <DoNotTranslate>**Provided by New Relic**</DoNotTranslate> 아래에는 뉴렐릭 관련 정보가 있습니다. 이를 ID 공급자 서비스의 관련 필드에 배치해야 합니다. 어디로 가는지 확실하지 않은 경우 ID 공급자 문서를 참조하세요.

6. <DoNotTranslate>**Provided by you**</DoNotTranslate> 아래에 <DoNotTranslate>**Source of SAML metadata**</DoNotTranslate> 을 입력합니다. 이 URL은 ID 공급자가 제공하며 다른 이름으로 불릴 수도 있습니다. SAML V2.0 메타데이터 표준을 준수해야 합니다. 귀하의 ID 제공자 <DoNotTranslate>**doesn't**</DoNotTranslate> 가 동적 설정을 지원하는 경우 <DoNotTranslate>**Upload a certificate**</DoNotTranslate> 사용하여 이를 수행할 수 있습니다. PEM으로 인코딩된 x509 인증서여야 합니다.

7. <DoNotTranslate>**Provided by you**</DoNotTranslate> 아래에서 ID 공급자가 제공한 <DoNotTranslate>**SSO target URL**</DoNotTranslate> 설정합니다. <DoNotTranslate>**Source of SAML metadata**</DoNotTranslate> 로 이동하여 POST 바인딩 URL을 찾아 이를 찾을 수 있습니다. 다음과 같습니다: `https://newrelic.oktapreview.com/app/newreliclr/1234567890abcdefghij/sso/saml`.

8. ID 제공업체에 로그아웃을 위한 리디렉션 URL이 있는 경우 이를 <DoNotTranslate>**Logout redirect URL**</DoNotTranslate> 에 입력합니다. 그렇지 않으면 공백으로 두십시오.
...
12. Okta만 해당: Okta의 뉴렐릭 앱으로 돌아가서
...
1. <DoNotTranslate>[**User management**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks/#where)</DoNotTranslate>
   ...
   [ UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks/#where) 에서 전환기의 인증 도메인을 선택합니다.
...
* <DoNotTranslate>**Automatic approval**</DoNotTranslate>
  ...
  : 이를 통해 사용자는 승인 없이 스스로 상위 사용자 유형으로 즉시 업그레이드할 수 있습니다. 이를 통해 사용자는 문제에 보다 신속하게 대응할 수 있습니다.

* <DoNotTranslate>**Require review**</DoNotTranslate>

  : 이 옵션을 사용하면 사용자가 업그레이드를 요청할 때

  <DoNotTranslate>[**Authentication domain**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings)</DoNotTranslate>

  [ 관리 설정](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings) 으로 사용자를 관리하고 이메일을 받고

  <DoNotTranslate>[**User management**](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where)</DoNotTranslate>

  [ UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 에서 해당 요청을 승인하거나 거부할 수 있습니다.
