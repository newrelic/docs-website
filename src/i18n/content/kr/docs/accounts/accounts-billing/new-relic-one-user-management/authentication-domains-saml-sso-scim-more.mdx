...
인증 도메인을 보고 구성하려면 [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 **Administration > Access management > Authentication domains** 으로 이동합니다.
...
3. 다음으로 인증 도메인 UI로 이동합니다. [사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) 에서 **액세스 관리** 를 클릭한 다음 **인증 도메인** 을 클릭합니다. 아직 도메인이 없는 경우 SAML 인증 사용자에 사용할 [새 도메인을 만듭니다](#ui) .
  ...
  >
    ...
    * 사용자 유형 업그레이드 요청을 처리하는 방법을 관리하는 기능. 이러한 옵션에는 다음이 포함됩니다.
      ...
      * **검토 필요** : 이 옵션을 사용하면 관리자( [**인증 도메인** 관리 설정](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings) 이 있는 사용자)는 사용자가 업그레이드를 요청할 때 이메일을 수신하고 [**사용자 관리** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) 에서 해당 요청을 승인하거나 거부할 수 있습니다.
        ...
        * 개별 사용자는 24시간 슬라이딩 윈도우 동안 6개의 업그레이드 요청으로 제한됩니다. 예를 들어 오전 8시에서 오전 10시 사이에 6개의 업그레이드 요청을 한 경우 다음 업그레이드 요청을 하기 전에 다음 날 오전 8시까지 기다려야 합니다.
  </Collapser>
