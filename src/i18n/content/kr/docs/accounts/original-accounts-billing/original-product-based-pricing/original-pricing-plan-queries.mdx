...
<Callout variant="important">
  이 문서는 당사의 원래 제품 기반 가격 모델을 참조합니다. 가격 변경에 대한 자세한 내용은 [가격 모델 개요를](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models/) 참조하세요.
</Callout>
...
이는 다양한 제품(예: <InlinePopover type="apm"/>, <InlinePopover type="browser"/>, 관측 모니터링 등)별로 청구되는 가격 모델인 [원래 가격 모델](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models/) 의 뉴렐릭 조직에 영향을 미칩니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 인프라: CU 기반 가격만
      ...
      APM의 경우 클라우드 기반 동적 컴퓨팅 리소스가 많은 경우 CU 기반 가격이 최선의 선택입니다. 이러한 이유로 CU 기반 가격 책정을 <DNT>**cloud pricing**</DNT> 이라고도 합니다.
  ...
  >
    ...
    호스트 기반 가격을 사용하면 뉴렐릭은 한 달에 사용한 <DNT>**equivalent hosts**</DNT> 수를 기준으로 요금을 청구합니다. 하나의 <DNT>**equivalent host**</DNT> 은(는) 750시간 동안 뉴렐릭에 연결된 [호스트](#understand) (표준 월 크기로 750시간 사용)로 정의됩니다. 호스트가 한 시간 동안 언제든지 뉴렐릭에 연결되어 있으면 해당 시간이 호스트 계산에 포함됩니다.

    이 시간은 여러 호스트에 나누어질 수 있습니다. 예를 들어, 한 달 동안 250시간 동안 각각 뉴렐릭에 연결된 3개의 호스트가 있을 수 있습니다. 이 시간의 합은 1 <DNT>**equivalent host**</DNT> 과 같습니다.
  ...
  >
    뉴클렉 컴퓨팅이 [호스트 기반 가격 책정](#host-based) 과 [CU 기반 가격 책정](#compute-unit) 방식을 모두 이해하려면 <DNT>**host**</DNT> 라는 단어가 어떻게 사용되는지 이해하는 것이 중요합니다. 호스트는 다음 중 하나일 수 있습니다.
    ...
    * <DNT>**cloud instance**</DNT> 은 퍼블릭 클라우드에서 실행되는 가상 머신 유형입니다. 이런 맥락에서 가상 머신과 클라우드 인스턴스는 JVM(Java Virtual Machine) 및 컨테이너와 다릅니다.
      ...
      뉴렐릭 가격 계산을 위해 <DNT>**month**</DNT> 은 750시간으로 정의됩니다.
  </Collapser>
</CollapserGroup>
