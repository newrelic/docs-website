...
<CollapserGroup>
  <Collapser
    ...
    title="APM CU 기반 구독"
  >
    [원래 가격 책정 모델을](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing)사용하는 계정의 경우 사용량 UI를 통해 데이터 사용량을 예측 가능하게 추적하고 이것이 가격 책정에 어떤 영향을 미칠 수 있는지 확인할 수 있습니다. 이 문서는 New Relic이 청구 가능한 사용량을 계산하는 방법을 설명합니다. <InlinePopover type="apm"/>CU 기반 가격 책정(호스트 기반 가격 책정 아님)이 있고 사용 가능한 구독 사용 속성과 New Relic UI 또는 API 호출에서 사용할 [예제 NRQL 쿼리](#query-examples) 도 제공하는 계정.

    ### 데이터 생성 [#details]

    하루에 한 번, <InlinePopover type="apm"/>계정은 다음에 대한 `NrDailyUsage` [이벤트를](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) 생성합니다.
    ...
    * 지난 24시간 동안 애플리케이션 인스턴스가 존재했던 모든 고유 호스트
    ...
    3. **평균 일일 계산 단위** 에 이번 달의 일 수를 곱합니다.
       ...
       일부 이전 <InlinePopover type="apm"/>에이전트는 컨테이너를 호스트로 잘못 계산할 수 있으며, 이로 인해 CU(컴퓨팅 유닛) 사용량이 과도하게 보고될 수 있습니다.
    ...
    * 지난 24시간 동안 애플리케이션 인스턴스가 실행된 모든 고유 호스트
    ...
    3. 계정 또는 계정 그룹에 대한 **평균 일일 등가 호스트** 를 참조하십시오.
       ...
       <CollapserGroup>
         ...
         >
           이 쿼리는 "호스트 사용량 보고서"(각 호스트에서 실행 중인 애플리케이션 목록)에 제공된 데이터를 반환합니다. 참고 동안 <InlinePopover type="apm"/>사용량은 시간 단위로 계산되며 시간당 사용량 데이터는 더 이상 API 또는 UI를 통해 사용할 수 없습니다.
       ...
       일부 이전 New Relic <InlinePopover type="apm"/>에이전트는 컨테이너를 호스트로 잘못 계산할 수 있으며, 이로 인해 호스트 기반 사용량이 과도하게 보고될 수 있습니다.
  ...
  >
    ...
    1. 이동: **[사용자 메뉴](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) > View your usage** .
  ...
  >
    ...
    * API 중 하나를 사용하여 이 데이터를 프로그래밍 방식으로 사용하십시오.
      ...
      <CollapserGroup>
        ...
        >
          이 쿼리는 특정 계정에 대해 현재 저장된 Insights 관리 이벤트 수를 [이벤트 네임스페이스](#insightsEventNamespace) 별로 분류하여 알려줍니다. 이 쿼리에서 YOUR_ACCOUNT_ID를 귀하의 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 로 바꾸십시오.
        ...
        >
          이 쿼리는 [이벤트 네임스페이스](#insightsEventNamespace) 별로 계정 목록에 대해 현재 저장된 Insights 관리 이벤트 수를 알려줍니다. 검색어의 LIST_OF_ACCOUNT_ID를 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 목록으로 바꿔야 합니다.
        ...
        >
          이 쿼리는 지난 7일 동안 계정을 생성한 제품별로 다른 New Relic 제품 구독에 포함된 Insights 관리 이벤트가 계정에 대해 얼마나 많이 저장되었는지 알려줍니다. 쿼리에서 YOUR_ACCOUNT_ID를 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 로 바꿔야 합니다.
        ...
        >
          이 쿼리는 다양한 New Relic 제품(예: <InlinePopover type="apm"/>`Transaction` 이벤트), 지정된 상위 계정 ID 아래의 각 하위 계정에 대해. 고유한 NRQL 쿼리를 생성할 때 YOUR_ACCOUNT_ID를 특정 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id)로 바꾸십시오.
        ...
        >
          이 쿼리는 지난 7일 동안 지정된 상위 계정 ID에 대해 각 하위 계정에서 사용 중인 Insights Pro 구독의 이벤트 수를 알려줍니다. 고유한 NRQL 쿼리를 생성할 때 쿼리의 YOUR_MASTER_ACCOUNT_ID를 특정 상위 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 로 바꿔야 합니다.
