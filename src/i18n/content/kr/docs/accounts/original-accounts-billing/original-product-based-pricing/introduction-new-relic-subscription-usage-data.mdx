...
<Callout variant="important">
  이 문서는 원래 [제품 기반 가격 책정](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing) 에 대한 계정을 위한 것입니다. 최신 가격 책정 모델을 사용 중인 경우 [New Relic One 가격 책정](/docs/accounts/accounts-billing/new-relic-one-pricing-users/pricing-billing) 모델을 참조하세요. 어느 쪽을 사용 중인지 확실하지 않습니까? [가격 책정 모델 개요 를](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models) 참조하십시오.
</Callout>
...
<Callout variant="tip">
  계정 **소유자** 및 **관리자** 만 사용 UI를 볼 수 있습니다. 그러나 계정의 모든 사용자는 `NrDailyUsage`[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 를 사용하여 사용 데이터를 쿼리할 수 있습니다.
</Callout>
...
![새로운 유물 사용 UI ](./images/New-Relic-usage-UI.png "new-relic-usage-ui.png")
...
New Relic은 사용 데이터를[`NrDailyUsage` 이벤트 유형](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 으로 저장합니다. 사용 차트에는 UI에 표시되는 것과 유사한 관련 NRQL 쿼리가 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * 지난 24시간 동안 애플리케이션 인스턴스가 존재했던 모든 고유 호스트
    ...
    이 두 가지 유형의 이벤트를 통해 사용 데이터를 다양한 방식으로 쿼리하고 분석할 수 있습니다. 애플리케이션 이벤트를 쿼리하려면 `Application` 의 `usageType` 속성 값을 사용하십시오. 호스트 이벤트를 쿼리하려면 `Host` 의 `usageType` 속성 값을 사용하십시오.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            해당 호스트의 고유 식별자입니다. 호스트가 AWS에 있는 경우 AWS에서 제공한 인스턴스 ID를 사용합니다. 다른 호스트의 경우 New Relic은 호스트 ID를 할당합니다. 이 값이 생성되는 방법에 대한 자세한 내용은 [`hostID`](/docs/apm-cu-based-usage-attributes-queries#hostid) 를 참조하십시오.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            에이전트가 보고한 호스트 이름입니다. 이 값에 대한 자세한 내용은 [`agentHostname`](/docs/apm-cu-based-usage-attributes-queries#agenthostname) 을 참조하십시오.
          </td>
        </tr>
    ...
    <CollapserGroup>
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                이 쿼리에는 최근 24시간 동안의 사용량만 포함됩니다. 더 오랜 기간 동안 집계된 사용량을 보려면 `SINCE` 절을 변경하십시오.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                NRQL은 반환되는 행 수를 제한합니다. 반환된 최대 행 수보다 더 많은 호스트가 있고 전체 호스트 집합을 가져오려면 더 많은 `WHERE` 절을 사용하여 쿼리 범위를 좁혀 데이터의 하위 집합(예: `WHERE cloudInstanceId LIKE “%0”` )을 다음으로 반환할 수 있습니다. 데이터를 AWS 인스턴스 ID의 마지막 문자를 모듈로 최대 16개 그룹으로 나눕니다.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                `Business rule` 은 다음 두 가지 속성으로 대체되었습니다.

                * `apmComputeUnitRule` 은 에이전트에서 사용할 수 있었던 데이터에 따라 호스트 크기가 계산되는 방식을 정의합니다. 컨테이너 환경( `update_agent_for_container_environment` )에서 보다 정확한 호스트 이름을 제공하기 위해 에이전트를 업데이트해야 하는지 여부를 더 이상 설명하지 않습니다.
                * `missingCpuData` , `missingRamData` 및 `instanceSizeCapped` 는 값 `missing_data` 가 `apmComputeUnitRule` 속성에 있을 때 의미하는 바를 설명하는 부울 플래그입니다.
              </td>
            </tr>
          </tbody>
        </table>
      ...
      >
        ...
        Docker 컨테이너의 이름을 잘못 지정한 호스트는 `containerId` 값의 첫 번째 부분과 동일한 `hostId` 값을 갖습니다. `apmComputeUnitRule != 'cloud_provider_data'` 절은 클라우드 공급자가 이름을 지정하여 임시 컨테이너의 이름을 따서 명명되지 않은 호스트를 고려 대상에서 제거합니다.
  ...
  >
    ...
    하루에 한 번 APM 계정은 다음에 대한 `NrDailyUsage`[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 를 생성합니다.
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                NRQL은 반환되는 행 수를 제한합니다. 반환된 최대 행 수보다 더 많은 호스트가 있고 전체 호스트 집합을 가져오려면 더 많은 `WHERE` 절로 쿼리 범위를 좁혀 데이터의 하위 집합(예: `WHERE cloudInstanceId LIKE “%0”` )을 반환하여 나눌 수 있습니다. 데이터를 AWS 인스턴스 ID의 마지막 문자를 모듈로 최대 16개 그룹으로 묶습니다.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                `Business rule` 이 다른 속성으로 대체되었습니다. 에이전트를 업데이트해야 하는지 여부를 설명하는 데 더 이상 사용되지 않습니다.
              </td>
            </tr>
          </tbody>
        </table>
  ...
  >
    ...
    모니터링되는 모든 응용 프로그램에 대해 시간당 한 번 New Relic Browser 계정은 `NrUsage`[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 를 생성합니다. 각 이벤트는 지난 1시간 동안의 사용량을 요약합니다.

    브라우저 사용 데이터를 조회할 때 `Browser` 의 `productLine` 속성 값을 사용하십시오. 자세한 내용은 [브라우저 쿼리 예제](#query-examples) 를 참조하십시오.
    ...
    <Callout variant="tip">
      계정 **소유자** 및 **관리자** 만 사용 UI를 볼 수 있습니다. 그러나 계정의 모든 사용자는 `NrUsage`[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 를 사용하여 사용 데이터를 쿼리할 수 있습니다.
    </Callout>
    ...
    * 일반적인 SPA 아키텍처 앱의 경우 표준 페이지 로드보다 경로 변경이 더 많습니다.
    ...
    Insights에 대해 비용을 지불하고 현재 Insights 라이선스가 SPA 모니터링으로 생성된[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 양에 충분하지 않은 경우 데이터 사용 계획을 초과했을 때 알려드립니다. 이를 해결하기 위해 다음 옵션을 사용할 수 있습니다.
  ...
  >
    ...
    하루에 한 번, 모니터링되는 모든 애플리케이션에 대해 New Relic Mobile 계정은 `NrDailyUsage`[이벤트](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) 를 생성합니다. 각 이벤트는 최근 24시간을 포함하여 이번 달의 모바일 사용량을 요약합니다.

    모바일 사용 데이터를 쿼리하려면 `Mobile` 의 `productLine` 속성 값을 사용하십시오. 자세한 내용은 [모바일 쿼리 예제](#query-examples) 를 참조하십시오.
    ...
    * 여러 앱이 있는 계정의 경우 총 계정 사용량은 해당 계정에 속한 모든 앱의 MAU 합계를 나타냅니다.
    ...
    <CollapserGroup>
      ...
      >
        ...
        해당 월의 청구 가능한 MAU 수는 이 차트의 최대 수입니다. 또한 다음과 같이 `since` 절에서 해당 월의 마지막 날을 사용하여 이 쿼리를 사용하여 주어진 월의 청구 가능 번호를 얻을 수 있습니다.
  ...
  >
    ...
    하루에 한 번 New Relic Infrastructure 계정은 지난 24시간 동안 애플리케이션 인스턴스가 있었던 모든 고유 호스트에 대해 `NrDailyUsage` [이벤트](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) 를 생성합니다. 모든 Infrastructure 이벤트에는 `Infrastructure` 의 `productLine` 속성 값이 있습니다. 자세한 내용은 [쿼리 예제](#query-examples) 를 참조하십시오.
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                NRQL은 결과를 1000개로 제한합니다. 1000개 이상의 호스트가 있고 전체 호스트 집합을 가져오려는 경우 더 많은 `WHERE` 절로 쿼리 범위를 좁혀 데이터의 하위 집합을 반환할 수 있습니다(예: `WHERE agentHostname LIKE ...` to 데이터를 그룹으로 나눕니다).
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                ...
                * `infrastructureComputeUnitRule` 은 에이전트에서 사용할 수 있었던 데이터에 따라 호스트 크기를 계산하는 방법을 정의합니다. 컨테이너 환경( `update_agent_for_container_environment` )에서 보다 정확한 호스트 이름을 제공하기 위해 에이전트를 업데이트해야 하는지 여부를 더 이상 설명하지 않습니다.
                * `missingCpuData` , `missingRamData` 및 `instanceSizeCapped` 는 이제 값 `missing_data` 가 `infrastructureComputeUnitRule` 속성에 있을 때 값이 의미하는 것을 설명하는 부울 플래그입니다.
              </td>
            </tr>
          </tbody>
        </table>
  ...
  >
    ...
    * New Relic UI 또는 API 호출에서 사용할 수 있는 구독 사용 속성 및 [예제 NRQL 쿼리](#query-examples) .
    ...
    하루에 한 번 Insights Pro 구독이 있는 계정은 모든 [이벤트 네임스페이스](#namespace) 에 대해 `NrDailyUsage` [이벤트](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) 를 생성합니다. `NrDailyUsage` 이벤트에는 해당 네임스페이스 아래의 이벤트 수가 포함됩니다.
    ...
    * API 중 하나를 사용하여 이 데이터를 프로그래밍 방식으로 사용하십시오.
    ...
    <Callout variant="tip">
      Insights는 [하루에 한 번](#data) 사용 이벤트를 작성합니다. 이것이 이러한 쿼리가 `since 24 hours ago` 또는 `timeseries 1 day` 이후를 사용하는 이유입니다.
    </Callout>
    ...
    <CollapserGroup>
      ...
      >
        이 쿼리는 지정된 상위 계정 ID 아래의 각 하위 계정에 대해 다양한 New Relic 제품(예: APM `Transaction` 이벤트)과 관련된 다양한 이벤트 범주에 대한 총, 지불 및 포함된 보존 기간을 알려줍니다. 고유한 NRQL 쿼리를 생성할 때 <var>YOUR_ACCOUNT_ID</var> 를 특정 [계정 ID](/docs/accounts/install-new-relic/account-setup/account-id) 로 바꿔야 합니다.
  ...
  <Collapser
    ...
    title="합성 구독"
  >
    [원래 가격 모델](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing) 계정의 경우 이것은 Synthetics 계정의 청구 가능한 사용량을 계산하는 방법을 설명합니다. 이 문서는 또한 사용 가능한 구독 사용 속성과 New Relic UI에서 또는 API 호출과 함께 사용할 [예제 NRQL 쿼리](#query-examples) 를 제공합니다.

    ### 데이터 생성 [#data]

    New Relic Synthetics 구독 수준은 한 달 동안 사용된 비 [핑](/docs/synthetics-subscription-usage-attributes-queries#label) 모니터 검사 수를 기반으로 합니다. 하루에 한 번 모든 Synthetics 모니터 위치는 모든 [모니터 유형](#type) 에 대해 `NrDailyUsage` [이벤트](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) 를 생성합니다. 이 이벤트는 지난 24시간 동안 해당 위치 및 모니터 유형에 대한 Synthetics 사용량을 요약합니다.

    모든 Synthetics 사용 이벤트의 `productLine` 속성 값은 `Synthetics` 입니다. 자세한 내용은 합성 [쿼리 예제](#query-examples) 를 참조하십시오.

    ### 사용량 계산 [#calculation]

    New Relic Synthetics 구독 수준은 한 달 동안 사용된 비 [핑](/docs/synthetics-subscription-usage-attributes-queries#label) 모니터 검사 수를 기반으로 합니다. 모니터 점검이 시간이 지남에 따라 상당히 안정적이라면 이번 달의 최종 사용량을 추정할 수 있습니다.
    ...
    다음은 Synthetics 구독 사용 데이터와 함께 사용할 수 있는 NRQL 쿼리의 몇 가지 예입니다. NRQL 쿼리를 실행하고 대시보드에서 결과 차트를 사용할 수 있습니다.
    ...
    <CollapserGroup>
      ...
      >
        이 쿼리는 지난 달의 청구 가능한 모니터 확인 차트를 생성합니다. `SIMPLE` 의 `syntheticsType` 유형은 유료 사용량에 포함되지 않는 [핑 모니터](/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors#types-monitors) 를 나타냅니다.
