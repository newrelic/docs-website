...
* 새로운 유물 계정. 계정이 없으면 [무료로 만드세요](https://newrelic.com/signup) .
...
AWS는 [OpenTelemetry Lambda Collector](https://github.com/open-telemetry/opentelemetry-lambda) , [OpenTelemetry Java SDK](https://github.com/open-telemetry/opentelemetry-java-instrumentation) 및 [ADOT Auto Instrumentation Agent](https://github.com/aws-observability/aws-otel-java-instrumentation) 를 포함하는 관리형 계층을 게시합니다.
...
1. New Relic 계정에서 [New Relic 라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-keys) 를 생성하고 복사합니다.
...
SAM 및 CloudFormation 템플릿의 경우 함수 속성에 다음을 추가합니다. `your-license-key-here` 을 [New Relic 라이선스 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-keys) 바꾸고 `otlp.nr-data.net:4317` 을 해당 지역의 New Relic OpenTelemetry Endpoint로 바꿔야 합니다.
...
<Callout variant="important">
  `your-license-key-here` 을 New Relic 라이선스 키로 바꾸고 `otlp.nr-data.net:4317` 을 New Relic 지역에 적합한 엔드포인트로 바꿉니다. 위의 목록을 참조하십시오.
</Callout>
...
Java 자동 계측 에이전트용 ADOT Lambda 계층은 AWS Lambda의 시작 시간에 현저한 영향을 미치며 일반적으로 [프로비저닝된 동시성](https://docs.aws.amazon.com/lambda/latest/dg/provisioned-concurrency.html) 과 함께 이를 사용하여 초기화하는 동안 초기 요청에서 시간 초과를 일으키지 않고 프로덕션 요청을 제공해야 합니다. 사용 사례에 적합한 설정을 결정하기 위해 먼저 비프로덕션 환경에서 이 구성을 테스트하는 것이 좋습니다.
