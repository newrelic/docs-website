...
SAM 및 CloudFormation 템플릿의 경우 함수 속성에 다음을 추가합니다. `your-license-key-here` 을 <LicenseKey/> 바꾸고 `otlp.nr-data.net:4317` 을 해당 지역의 New Relic OpenTelemetry Endpoint로 바꾸십시오.
...
<Callout variant="important">
  `your-license-key-here` 을 New Relic <LicenseKey/> 바꾸고 `otlp.nr-data.net:4317` 을 New Relic 지역에 적합한 엔드포인트로 바꿉니다. 위의 목록을 참조하십시오.
</Callout>
