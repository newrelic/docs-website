...
* `dotnetcore2.1` 또는 `dotnetcore3.1` 런타임에서 실행되는 .NET Lambda 함수. 아직 계정이 없다면 [지금 만드세요](https://docs.aws.amazon.com/lambda/latest/dg/lambda-csharp.html) .
...
<Callout variant="important">
  `your-license-key-here` New Relic [라이센스 키로](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 바꾸고 `otlp.nr-data.net:4317` New Relic 지역에 적합한 엔드포인트로 바꿉니다(위 목록 참조).
</Callout>
