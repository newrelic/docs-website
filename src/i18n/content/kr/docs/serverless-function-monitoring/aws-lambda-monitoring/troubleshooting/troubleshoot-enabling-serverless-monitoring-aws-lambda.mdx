...
* CloudWatch 오류 "HTTP 오류 401: 권한이 없습니다." 이는 잘못된 API 키 때문입니다. [AWS 구성 활성화 단계](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/enable-new-relic-monitoring-aws-lambda#connect-aws) 의 `--nr-api-key` 인수는 REST API 키와 다른 [사용자 키](/docs/apis/get-started/intro-apis/types-new-relic-api-keys#user-api-key) 를 사용합니다.
...
* 설치, 통합 및 계측 단계를 올바르게 완료했으며 함수가 CloudWatch에 로그를 전송하지만 UI에서 특정 종속성(또는 모든 추적)에 대한 추적을 볼 수 없습니다. 이는 레이어 병합 순서(Lambda 레이어를 사용하는 경우) 또는 가져오기 순서(수동으로 계측하는 경우)로 인해 발생할 수 있습니다.
  ...
  * 노드 함수를 수동으로 계측하는 경우 [로깅이 활성화](/docs/apm/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration/#logging_config) 되어 있는지 확인하고 모니터링할 것으로 예상되는 종속성을 가져오기 전에 함수가 **newrelic** 을 가져오는지 확인하십시오.
