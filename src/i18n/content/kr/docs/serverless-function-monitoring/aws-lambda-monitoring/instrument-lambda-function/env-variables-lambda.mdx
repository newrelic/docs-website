---
title: AWS Lambda의 환경 변수
metaDescription: Learn how to set and use environment variables in AWS Lambda functions for configuration and secrets management.
freshnessValidatedDate: never
tags:
  - AWS Lambda
  - Environment Variables
  - Configuration
  - Secrets Management
translationType: machine
---

환경 변수는 코드 외부에 설정과 비밀을 저장하는 방법입니다. 다음 표에서는 Lambda 함수를 NEWLick으로 모니터링하기 위해 설정할 수 있는 필수 변수와 런타임별 변수를 표시합니다.

## 필수 환경 변수

런타임에 관계없이 Lambda 함수를 New Relic으로 모니터링하려면 다음 환경 변수를 설정해야 합니다.

<table>
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        환경 변수
      </th>

      <th>
        설명
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `NEW_RELIC_ACCOUNT_ID`
      </td>

      <td>
        귀하의 뉴렐릭 계정 ID를 지정합니다
      </td>
    </tr>

    <tr>
      <td>
        `NEW_RELIC_LAMBDA_HANDLER`
      </td>

      <td>
        함수의 실제 핸들러를 찾는 데 사용되는 핸들러 래퍼를 지정합니다. Go 런타임의 경우 이 변수는 필수가 아닙니다.
      </td>
    </tr>

    <tr>
      <td>
        `NEW_RELIC_LICENSE_KEY`
      </td>

      <td>
        Secrets Manager를 재정의하여 뉴렐릭 수집 키를 지정합니다.
      </td>
    </tr>

    <tr>
      <td>
        `NEW_RELIC_APM_LAMBDA_MODE`
      </td>

      <td>
        Lambda 함수에 대한 APM 모니터링을 활성화하려면 

        `true`

         으로 설정하세요.
      </td>
    </tr>

    <tr>
      <td>
        `NEW_RELIC_TRUSTED_ACCOUNT_KEY`
      </td>

      <td>
        존재하는 경우 귀하의 뉴렐릭 계정 ID 또는 부모 ID를 지정합니다.
      </td>
    </tr>
  </tbody>
</table>

## 추가 환경 변수

런타임에 따라 다음 환경 변수를 설정하여 NEWLamda Monitoring Lambda 추가로 구성할 수 있습니다.

<Tabs>
  <TabsBar>
    <TabsBarItem id="nodejs">Node.js</TabsBarItem> <TabsBarItem id="python" /><TabsBarItem id="java" /><TabsBarItem id="Lambda extension">Lambda 끌어서</TabsBarItem> <TabsBarItem id=".NET" /><TabsBarItem id="Go">Go</TabsBarItem> <TabsBarItem id="Ruby">루비.NET 확장</TabsBarItem>
  </TabsBar>

  <TabsPages>
    <TabsPageItem id="nodejs">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `NEW_RELIC_USE_ESM`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              **더 이상 사용되지 않음**

               : 이 기능은 향후 Node.js 람다 계층 버전에서 제거될 예정입니다. 대신 핸들러를 다음으로 설정하세요. 

              `/opt/nodejs/node_modules/newrelic-esm-lambda-wrapper/index.handler`
            </td>
          </tr>

          <tr>
            <td>
              `NODE_OPTIONS`
            </td>

            <td />

            <td>
              `--experimental-loader newrelic/esm-loader.mjs`
            </td>

            <td>
              **필수**

              : ESM이 활성화된 경우 이 환경 변수를 설정하여 ESM 로더를 사용합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_AI_MONITORING_ENABLED`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              **응답 스트리밍에 필요**

              : 뉴렐릭 UI 에서 AI 모니터링 응답을 활성화하려면 

              `true`

               으로 설정합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_NO_CONFIG_FILE`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              Lambda에서 환경 변수를 사용하려면 다음을 설정합니다. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_ENABLED`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 에이전트 로깅을 활성화합니다. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_LEVEL`
            </td>

            <td>
              `info`
            </td>

            <td>
              `fatal`

              , 

              `error`

               , 

              `warn`

               , 

              `info`

               , 

              `debug`

               , 

              `trace`
            </td>

            <td>
              에이전트 로그 레벨 설정
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG`
            </td>

            <td>
              `stdout`
            </td>

            <td>
              `stdout`
            </td>

            <td>
              서버리스 모드의 경우 에이전트 로그 경로를 

              `stdout`

               로 설정합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_NATIVE_METRICS_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              `false`

               로 설정하여 콜드 스타트 기간을 줄이도록 비활성화하지만 VM 메트릭은 수집하지 않습니다.
            </td>
          </tr>
        </tbody>
      </table>

      [Node.js 설정 문서](/docs/apm/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration/) 에서 더 많은 환경 변수를 찾을 수 있습니다.
    </TabsPageItem>

    <TabsPageItem id="python">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_NO_CONFIG_FILE`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              Lambda에서 환경 변수를 사용하려면 다음을 설정합니다. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG`
            </td>

            <td>
              `stderr`
            </td>

            <td>
              `stderr`
            </td>

            <td>
              서버리스 모드의 경우 에이전트 로그 경로를 

              `stderr`

               로 설정합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_LEVEL`
            </td>

            <td>
              `info`
            </td>

            <td>
              `critical`

              , 

              `error`

               , 

              `warning`

               , 

              `info`

               , 

              `debug`
            </td>

            <td>
              에이전트 로그 레벨 설정
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_SERVERLESS_MODE_ENABLED`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 서버리스 모드를 활성화하세요. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_PACKAGE_REPORTING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 콜드 스타트 시간을 개선하십시오. 

              `false`
            </td>
          </tr>
        </tbody>
      </table>

      [끌어서 설정 문서](/docs/apm/agents/python-agent/configuration/python-agent-configuration/) 에서 더 많은 환경 변수를 찾을 수 있습니다.
    </TabsPageItem>

    <TabsPageItem id="java">
      <Callout variant="important">
        새로운 자바 에이전트 계층은 자바 17 이상이 필요합니다. 이것은 구형 [자바 솔루션](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/instrument-your-own/#console)과는 별개의 솔루션입니다. 2026년 4월 10일 이전에 애플리케이션에 서버리스 모니터링을 사용 중이거나 자바 8 또는 11을 사용 중인 경우, Open Tracing을 기반으로 하는 구형 솔루션을 계속 사용하십시오.
      </Callout>

      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `AWS_LAMBDA_EXEC_WRAPPER`
            </td>

            <td />

            <td>
              `/opt/newrelic-java-handler`
            </td>

            <td>
              **필수**

              : 뉴렐릭 에이전트 레이어 사용 시 

              `/opt/newrelic-java-handler`

               로 설정해야 합니다. 이는 AWS Lambda가 레이어에 포함된 에이전트 래퍼 스크립트를 가리키도록 합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_SERVERLESS_MODE_ENABLED`
            </td>

            <td>
              `true`

               (레이어에 미리 설정됨)
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              서버리스 모드를 활성화합니다. 

              `AWS_LAMBDA_FUNCTION_NAME`

               이 존재할 때 자동으로 감지됩니다. 레이어에서 

              `true`

               로 사전 구성되어 있습니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_FILE_PATH`
            </td>

            <td>
              `/tmp/logs/`

               (레이어에 미리 설정됨)
            </td>

            <td>
              `/tmp/logs/`
            </td>

            <td>
              기본적으로 중력 에이전트 로그 파일이 절대 경로에 있도록 설정됩니다. 

              `/tmp/logs/newrelic_agent.log`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_PRIMARY_APPLICATION_ID`
            </td>

            <td />

            <td />

            <td>
              뉴렐릭 계정 ID를 설정하세요
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_DEBUG`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이 값을 설정하여 디버그 로깅을 활성화하십시오. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>
        </tbody>
      </table>

      [지속 저항 설정 문서](/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file/) 및 [지속 서버 모니터링](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda) 에서 더 많은 환경 변수를 찾을 수 있습니다.
    </TabsPageItem>

    <TabsPageItem id="Go">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG`
            </td>

            <td>
              `stdout`
            </td>

            <td>
              `stdout`
            </td>

            <td>
              서버리스 모드의 경우 에이전트 로그 경로를 

              `stdout`

               로 설정합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_LEVEL`
            </td>

            <td />

            <td>
              `debug`

               디버그용 또는 정보용 (비워두면)
            </td>

            <td>
              에이전트 로그 레벨 설정
            </td>
          </tr>
        </tbody>
      </table>
    </TabsPageItem>

    <TabsPageItem id="Ruby">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>
        </tbody>
      </table>
    </TabsPageItem>

    <TabsPageItem id=".NET">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `CORECLR_ENABLE_PROFILING`
            </td>

            <td>
              `0`
            </td>

            <td>
              `0`

              , 

              `1`
            </td>

            <td>
              **필수**

              : 이것을 설정하여 프로파일링을 활성화합니다. 

              `1`
            </td>
          </tr>

          <tr>
            <td>
              `CORECLR_PROFILER`
            </td>

            <td />

            <td>
              `{36032161-FFC0-4B61-B559-F6C5D41BAE5A}`
            </td>

            <td>
              **필수**

              : .NET 에이전트를 활성화하려면 이것을 

              `{36032161-FFC0-4B61-B559-F6C5D41BAE5A}`

               으로 설정하세요.
            </td>
          </tr>

          <tr>
            <td>
              `CORECLR_NEWRELIC_HOME`
            </td>

            <td />

            <td>
              `/opt/lib/newrelic-dotnet-agent`
            </td>

            <td>
              **필수**

              : .NET 에이전트를 활성화하려면 이것을 

              `/opt/lib/newrelic-dotnet-agent`

               으로 설정하세요.
            </td>
          </tr>

          <tr>
            <td>
              `CORECLR_PROFILER_PATH`
            </td>

            <td />

            <td>
              `/opt/lib/newrelic-dotnet-agent/libNewRelicProfiler.so`
            </td>

            <td>
              **필수**

              : .NET 에이전트를 활성화하려면 이것을 

              `/opt/lib/newrelic-dotnet-agent/libNewRelicProfiler.so`

               으로 설정하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              내보내는 추적을 활성화하여 트레이스를 생성합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때 뉴렐릭 UI에서 APM 엔티티 이름으로 사용되는 애플리케이션 이름을 설정합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_ENABLED`
            </td>

            <td>
              `1`
            </td>

            <td>
              `1`

              , 

              `0`
            </td>

            <td>
              이것을 설정하여 에이전트 로깅을 비활성화합니다. 

              `0`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_CONSOLE`
            </td>

            <td>
              `0`
            </td>

            <td>
              `1`

              , 

              `0`
            </td>

            <td>
              CloudWatch에 로그를 출력하려면 이것을 

              `1`

               으로 설정합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_LEVEL`
            </td>

            <td>
              `info`
            </td>

            <td>
              `info`

              , 

              `debug`

               , 

              `finest`
            </td>

            <td>
              에이전트 로그 레벨 설정
            </td>
          </tr>
        </tbody>
      </table>

      [.NET 설정 문서](/docs/apm/agents/net-agent/configuration/net-agent-configuration/) 및 [레이어 없는 HTML](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/enable-serverless-monitoring-aws-lambda-layerless/#dotnet) 메서드에서 더 많은 환경 변수를 찾을 수 있습니다.
    </TabsPageItem>

    <TabsPageItem id="Lambda extension">
      <table>
        <thead>
          <tr>
            <th>
              환경 변수
            </th>

            <th>
              기본값
            </th>

            <th>
              옵션
            </th>

            <th>
              설명
            </th>
          </tr>
        </thead>

        <tbody>
          <tr>
            <td>
              `NEW_RELIC_APP_NAME`
            </td>

            <td />

            <td />

            <td>
              `NEW_RELIC_APM_LAMBDA_MODE` 이(가) `true`일 때, 익스텐션은 콜드 스타트 연결 호출 중에 이 값을 APM 엔티티 이름으로 사용합니다. 엔티티 그룹화, 다중 리전 보고, 교차 런타임 동작 및 비 Lambda 서비스와의 공유 이름 지정에 대한 자세한 내용은 [APM 모드의 Lambda 함수에 대한 NEW\_RELIC\_APP\_NAME](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) 을 참조하십시오.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LAMBDA_EXTENSION_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 확장 기능을 비활성화합니다. 

              `false`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LICENSE_KEY_SECRET`
            </td>

            <td>
              `NEW_RELIC_LICENSE_KEY`
            </td>

            <td />

            <td>
              AWS Secrets Manager에서 사용자 정의 비밀 이름 설정
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_DATA_COLLECTION_TIMEOUT`
            </td>

            <td>
              10초
            </td>

            <td />

            <td>
              시간 초과 기간을 설정합니다. 

              `Telemetry client error`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_LOGS_ENABLED`
            </td>

            <td>
              `true`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 

              `[NR_EXT]`

               로그 줄을 비활성화합니다. 

              `false`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_LOG_LEVEL`
            </td>

            <td>
              `INFO`
            </td>

            <td>
              `INFO`

              , 

              `DEBUG`

               , 

              `TRACE`

               , 

              `ERROR`

               , 

              `WARN`

               , 

              `ALL`
            </td>

            <td>
              `NR_EXT`

               로그인 라인에 대한 로그인 레벨 설정
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_SEND_FUNCTION_LOGS`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 기능 로그를 보내세요 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_SEND_EXTENSION_LOGS`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이것을 설정하여 확장 로그를 보내세요 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_SEND_PLATFORM_LOGS`
            </td>

            <td>
              `false`
            </td>

            <td>
              `true`

              , 

              `false`
            </td>

            <td>
              이 값을 설정하여 플랫폼 로그를 전송합니다. 

              `true`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_EXTENSION_SEND_LOGS`
            </td>

            <td />

            <td>
              `platform`

              , 

              `extension`

               , 

              `function`

               , 

              `all`
            </td>

            <td>
              보낼 로그를 결정합니다. 

              `all`

               또는 

              `function`

              , 

              `extension`

               및 

              `platform`

               로그로 설정하거나 쉼표로 구분된 목록을 사용하십시오(예를 들어: 

              `platform,extension`

              ). 이 변수는 개별 로그 유형 변수보다 우선합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LOG_ENDPOINT`
            </td>

            <td>
              `https://log-api.newrelic.com/log/v1`
            </td>

            <td />

            <td>
              로그 엔드포인트를 설정합니다. 지역에 따라 URL을 선택합니다:

              * 미국 지역(기본값):<br />`https://log-api.newrelic.com/log/v1`
              * EU 지역:<br />`https://log-api.eu.newrelic.com/log/v1`
              * JP 지역:<br />`https://log-api.jp.nr-data.net/log/v1`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_TELEMETRY_ENDPOINT`
            </td>

            <td>
              `https://cloud-collector.newrelic.com/aws/lambda/v1`
            </td>

            <td />

            <td>
              텔레메트리 엔드포인트를 설정합니다. 지역에 따라 URL을 선택합니다:

              * 미국 지역(기본값):<br />`https://cloud-collector.newrelic.com/aws/lambda/v1`
              * EU 데이터센터:<br />`https://cloud-collector.eu01.nr-data.net/aws/lambda/v1`
              * JP 데이터센터:<br />`https://cloud-collector.jp.nr-data.net/aws/lambda/v1`
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_IGNORE_EXTENSION_CHECKS`
            </td>

            <td />

            <td />

            <td>
              **더 이상 사용되지 않습니다.**

               문자열 내에서 쉼표로 구분된 값을 제공하여 검사를 우회합니다. 

              `handler`

              , 

              `agent`

              , 

              `sanity`

              , 

              `vendor`

               또는 

              `all`

               문자열 값을 사용하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LICENSE_KEY_SECRET`
            </td>

            <td />

            <td />

            <td>
              AWS Secrets Manager에서 새 암호가 포함된 비밀의 **이름** 이나 **ARN을** 지정하세요.

              <br />

              <br />

              **메모:**

              * `` `NEW_RELIC_LICENSE_KEY` `` 이 설정되지 않은 경우에만 사용됩니다.
              * 비밀번호는 Lambda 함수와 동일한 AWS 지역에 있어야 합니다.
              * Lambda 함수의 실행 역할에는 이 비밀에 대한 `` `secretsmanager:GetSecretValue` `` 권한이 필요합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LICENSE_KEY_SSM_PARAMETER_NAME`
            </td>

            <td />

            <td />

            <td>
              사용자의 뉴렐릭 클러스터 키가 포함된 AWS Systems Manager 레지스트리 스토어에서 시위의 **이름** 또는 **ARN을** 지정합니다.

              <br />

              <br />

              **메모:**

              * `` `NEW_RELIC_LICENSE_KEY` `` 이 설정되지 않은 경우에만 사용됩니다.
              * SSM 참가자는 Lambda 함수와 동일한 AWS 지역에 있어야 합니다.
              * 귀하의 Lambda 함수 실행 역할에는 이 보고서에 대한 `` `ssm:GetParameter` `` 권한이 필요합니다.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_CLOUD_AWS_ACCOUNT_ID`
            </td>

            <td />

            <td />

            <td>
              뉴렐릭가 모니터 부분 간의 관계를 올바르게 매핑하고 표시할 수 있도록 하려면 데이터베이스 및 Lambda 함수와 같은 모니터 리소스가 있는 AWS 계정 ID를 제공하세요.
            </td>
          </tr>

          <tr>
            <td>
              `NEW_RELIC_LAMBDA_EXTENSION_PROXY`
            </td>

            <td />

            <td />

            <td>
              익스텐션의 아웃바운드 트래픽에 대한 프록시를 구성하며 Lambda 함수 트래픽에는 영향을 주지 않습니다.

              <br />

              <br />

              * **지원:** `http://`, `https://`, `socks5://` 스킴
              * **인증:** `user:pass` 지원(로그에서 마스킹됨)
              * **폴백:** 설정되지 않은 경우 `HTTPS_PROXY` 또는 `HTTP_PROXY` 을(를) 사용합니다.

              <Callout variant="tip" title="메모">
                Localhost 트래픽은 절대 프록시 처리되지 않습니다.
              </Callout>
            </td>
          </tr>
        </tbody>
      </table>

      [설명서](https://github.com/newrelic/newrelic-lambda-extension/blob/cf86fb53f0bd816e1d09d07f7b5c5ab4841010a0/config/config.go#L41-L55) 에서 뉴렐릭 확장에 대한 더 많은 환경 변수를 확인하세요.
    </TabsPageItem>
  </TabsPages>
</Tabs>

## 다음 단계

<DocTiles>
  <DocTile title="Lambda 모니터링 설치 및 구성" path="/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/configure-serverless-aws-monitoring">
    Lambda 함수에 대한 측정, 로그 및 설정 시작하기
  </DocTile>

  <DocTile title="데이터 및 UI" path="/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/instrument-your-own">
    뉴렐릭을 사용하여 AWS Lambda 함수를 모니터링하는 방법을 알아보세요.
  </DocTile>
</DocTiles>