...
이 업그레이드는 뉴렐릭 Lambda 레이어 또는 OpenTelemetry(ADOT) 레이어로 계측된 Lambda 함수에 적용됩니다. 두 레이어 유형 모두 동일한 APM UI 경험을 제공합니다.

<Callout variant="tip">
  **Lambda 관리형 인스턴스(LMI) 사용자**: 업그레이드 단계가 필요하지 않습니다. LMI 함수는 자동으로 감지되어 APM 모드에서 실행되도록 구성됩니다 ― `NEW_RELIC_APM_LAMBDA_MODE = true`을(를) 설정하지 않은 경우에도 마찬가지입니다. LMI에서는 Serverless 모드가 지원되지 않으며, APM 모드만 사용할 수 있습니다. LMI 설정에 대한 자세한 내용은 [계층화된 계측](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/instrument-your-own)을 참조하십시오.
</Callout>
...
### 뉴렐릭 UI를 사용하여 업데이트 [#ui]

뉴렐릭 UI 사용하여 기존 Lambda 함수를 업그레이드할 수 있습니다.

**뉴렐릭 플랫폼에서 기존 Lambda 함수를 업그레이드하려면 다음 단계를 따르세요.**

1. <DNT>**[one.newrelic.com &gt; All capabilities &gt; Serverless Functions](https://one.newrelic.com/nr1-core?state=23b04845-da13-2b11-5826-9c20326276d9)**</DNT> (으)로 이동 &gt; Lambda 함수 엔티티를 선택하십시오.
2. Summary \[요약] 페이지에서 <DNT>**Instrument for APM**</DNT>을(를) 클릭하십시오.
3. APM용 Lambda 함수 페이지에서 API 키를 입력한 다음 <DNT>**Continue**</DNT>을(를) 클릭합니다.
4. 기본적으로 현재 Lambda 엔티티가 업그레이드 대상으로 선택됩니다. 업그레이드할 다른 Lambda 함수도 선택할 수 있습니다. <DNT>**Generate New Relic CLI command**</DNT>을(를) 클릭합니다.
5. 명령어를 복사하여 터미널 또는 AWS 터미널에서 실행하세요. 이 명령은 선택한 함수에 뉴럴릭 Lambda 레이어를 설치합니다.
6. 설치를 테스트하려면 <DNT>**Continue**</DNT>을(를) 클릭하십시오. 이 테스트는 뉴렐릭 Lambda 레이어가 올바르게 설치되었는지, 그리고 함수가 뉴렐릭으로 데이터를 전송하는지 여부를 확인합니다.
7. 테스트가 성공하면 성공 메시지가 표시됩니다. 뉴렐릭 APM 인터페이스에서 Lambda 함수를 보려면 <DNT>**See your data in APM**</DNT>을(를) 클릭하십시오.
...
AWS Lambda 함수를 계측한 후, 뉴렐릭 APM 인터페이스에서 데이터를 찾아 사용할 수 있습니다. OpenTelemetry(ADOT 계층)를 통해 계측된 Lambda 함수는 이제 일관된 **Summary** \[요약] 페이지와 `apm.*` 형식의 정규화된 메트릭을 포함하여, 뉴렐릭 APM 에이전트로 계측된 함수와 동일한 APM UI 환경을 표시합니다.
...
3. 표시된 목록에서 Lambda 함수를 선택하여 데이터를 확인하십시오.

<Callout variant="important">
  뉴렐릭 APM UI는 `APM + Serverless Convergence` 업그레이드 이전에 계측된 Lambda 함수에 대한 이력 데이터를 표시하지 않습니다. 이력 데이터를 보려면 <DNT>**APM Summary**</DNT> 페이지에서 <DNT>**see serverless functions**</DNT>을(를) 클릭하십시오.
</Callout>

### 요약 [#summary]

<DNT>**Summary**</DNT> 페이지는 응답 시간, 처리량 및 오류율을 포함하여 Lambda 함수에 대한 가장 중요한 성능 데이터의 빠른 보기를 제공합니다. 이 페이지는 APM으로 계측된 Lambda 함수와 OpenTelemetry(ADOT)로 계측된 Lambda 함수 모두에 사용할 수 있습니다.

### CloudWatch 지표 [#cloudwatch-metrics]

<DNT>**CloudWatch metrics**</DNT> 탭은 뉴렐릭의 메트릭 합성 레이어를 통해 AWS CloudWatch에서 가져온 Lambda 성능 데이터를 표시합니다. 차트에는 다음이 포함됩니다:
...
이 탭은 APM으로 계측된 Lambda 함수와 OpenTelemetry(ADOT)로 계측된 Lambda 함수 모두에서 사용할 수 있습니다.

### Lambda 메트릭 [#lambda-metrics]

<DNT>**Lambda metrics**</DNT> 탭은 뉴렐릭 Lambda 계층에서 직접 가져온 성능 데이터를 표시합니다. 두 개의 섹션으로 구성됩니다:

* **비용**: 호출 및 메모리 사용량 차트.
* **성능**: 콜드 스타트, 시간 초과, 반복자 연령 및 동시 실행.

<Callout variant="note">
  **Lambda 관리형 인스턴스(LMI)** 모드에서 실행되는 Lambda 함수의 경우 비용 섹션에 **Billed duration** \[청구된 기간] 차트가 표시되지 않습니다. LMI 청구는 EC2 비용, 관리 수수료 및 vCPU 사용량을 기반으로 합니다 ― Lambda 실행 기간이 아닙니다.
</Callout>

### 업무 [#transactions]

<DNT>**Transactions**</DNT> 탭에는 기간 및 처리량과 같은 성능 데이터와 함께 Lambda 트랜잭션이 나열됩니다. **AWS Request ID** 열은 고유한 AWS 요청 식별자로 각 Lambda 실행을 식별하므로, 특정 호출의 APM 트랜잭션 트레이스로 드릴다운하여 해당 특정 Lambda 실행에 대한 자세한 정보를 확인할 수 있습니다.

### 분산 추적 [#distributed-tracing]

<DNT>**Distributed tracing**</DNT> 탭에는 모니터되는 Lambda 함수가 포함된 분산 트레이스가 표시됩니다. 자세한 내용은 [분산 추적](/docs/distributed-tracing/concepts/introduction-distributed-tracing/)을 참조하십시오.

### 오류 [#errors]

<DNT>**Errors**</DNT> 탭에는 Lambda 함수에 대한 오류 및 예외가 표시됩니다. 오류율, 오류 백분율 또는 오류 클래스별로 필터링하고 스택 추적으로 드릴다운할 수 있습니다.

### 로그 [#logs]

<DNT>**Logs**</DNT> 탭에는 Lambda 함수의 최근 로그 메시지가 표시됩니다. 자세한 내용은 [로그](/docs/logs/new-relic-logs/get-started/introduction-new-relic-logs)를 참조하십시오.
