...
<Callout variant="tip">
  New Relic Lambda monitoring supports Lambda Managed Instance (LMI) functions. LMI functions are automatically detected and run in APM mode — no extra configuration is required.
</Callout>
...
## Configure AWS Lambda monitoring [#config]
...
<Steps>
  ...
  <Step>
    ...
    <Tabs>
      ...
      <TabsPages>
        <TabsPageItem id="1">
          뉴렐릭이 Lambda 함수를 모니터하려면 뉴렐릭과 AWS 간의 통합을 설정해야 합니다. 지연시간을 줄이고 폴링 requests의 스로틀링을 방지하려면 [메트릭 스트림](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) 을 사용하는 것이 좋습니다.
        </TabsPageItem>
