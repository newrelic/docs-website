...
<CollapserGroup>
  <Collapser id="go" title="가다">
    ...
    7. Lambda를 한 번 이상 호출합니다. 그러면 다음 단계가 작동하기 위해 있어야 하는 CloudWatch 로그 그룹이 생성됩니다.
       ...
       당사의 래퍼는 Lambda 실행에 대한 데이터를 수집하고, JSON 메시지를 생성하며, 이를 CloudWatch 로그에 기록합니다. 다음으로, [해당 로그를 뉴렐릭으로 전송하도록 CloudWatch를 구성합니다](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/cloud-watch-fallback).
  </Collapser>

  <Collapser id="java" title="자바">
    <Callout variant="tip">
      이 솔루션은 APM 자바 에이전트를 사용하지 않습니다. AWS Lambda에서 APM 자바 에이전트를 사용하려면 [자바 에이전트 Serverless 솔루션 문서](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda)를 참조하십시오. APM 에이전트 기반 솔루션은 SDK 기반 솔루션과 함께 실행해서는 안 됩니다.
    </Callout>

    SDK 기반 솔루션을 사용하는 자바의 AWS Lambda 모니터링은 두 개의 Open Tracing 의존성/종속성을 사용합니다:
  ...
  <Collapser id="python" title="파이썬">
    ...
    9. Lambda를 한 번 이상 호출합니다. 그러면 다음 단계가 작동하기 위해 있어야 하는 CloudWatch 로그 그룹이 생성됩니다.
       ...
       뉴렐릭 데코레이터는 Lambda 실행에 대한 데이터를 수집하고, JSON 메시지를 생성하며, 이를 CloudWatch 로그에 기록합니다. 다음으로, [해당 로그를 뉴렐릭으로 전송하도록 CloudWatch를 구성하십시오](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/cloud-watch-fallback).
  </Collapser>
