...
New Relic 서비스는 원격 분석을 수신하여 이를 [`AwsLambdaInvocation`](/attribute-dictionary/?event=AwsLambdaInvocation), [`AwsLambdaInvocationError`](/attribute-dictionary/?event=AwsLambdaInvocationError), [`Span`](/attribute-dictionary/?event=Span) 및 사용자 정의 이벤트로 처리하고 모든 것을 원격 분석 데이터베이스인 NRDB에 저장합니다.
