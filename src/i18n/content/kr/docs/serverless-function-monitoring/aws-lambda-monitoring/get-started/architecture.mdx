...
3. [새로운 유물 람다 확장](#lambda-extension)
4. [New Relic의 백엔드](#backend)
5. [새로운 유물 람다 UI](#lambda-ui)
...
Lambda 함수는 기존 서비스와 완전히 다르기 때문에 이를 관리하는 경험은 기존 APM 경험과 약간 다릅니다. [New Relic platfor](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one/) 내에서 Lambda 함수에는 함수에 대한 가장 중요한 정보를 신속하게 표시하고 로깅 및 분산 추적 기능과 밀접하게 통합되는 사용자 지정 UI가 있습니다.
...
함수 로그를 [`aws-log-ingestion`](https://github.com/newrelic/aws-log-ingestion) Lambda 함수로 파이프하는 로그 구독 필터를 추가하면 해당 Cloudwatch 로그 라인을 복구하고 다른 플랫폼 원격 측정과 함께 New Relic에 전달할 수 있습니다.
