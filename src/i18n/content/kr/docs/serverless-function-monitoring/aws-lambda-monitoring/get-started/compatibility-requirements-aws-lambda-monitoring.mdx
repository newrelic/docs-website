...
* 파이썬: `python3.7`, `python3.8`, `python3.9`, `python3.10`, `python3.11`
...
성능 영향을 최소화하기 위해 우리는 Java 및 .NET Core에 대해 다른 접근 방식을 취했습니다. 뉴렐릭은 이러한 런타임을 위한 [OpenTracing](https://opentracing.io/) SDK를 제공합니다. 이 접근 방식을 사용하려면 통합할 코드가 조금 더 필요합니다. .NET 6과 같은 최신 런타임의 경우 [트레이스에서는 뉴렐릭 및 OpenTelemetry가 포함된 .NET Lambda 함수를](/docs/serverless-function-monitoring/aws-lambda-monitoring/opentelemetry/lambda-opentelemetry-dotnet/) 사용할 수 있습니다.
