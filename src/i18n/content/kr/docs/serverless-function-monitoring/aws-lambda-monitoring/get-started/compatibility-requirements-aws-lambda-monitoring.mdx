...
Python과 Node.js는 Lambda 생태계에서 단연코 가장 인기 있는 언어입니다. Node 및 Python용 [New Relic Lambda Layers](https://layers.newrelic-external.com/) 에는 최신 New Relic Agent 버전이 포함되어 있으며 즉시 사용 가능한 최소한의 구성으로 풍부한 계측을 제공합니다.

마찬가지로 Go는 New Relic Go 에이전트를 사용합니다. 최신 AWS Lambda API는 AWS `go1.x` 런타임에서 지원되지 않으므로 자체 호스팅 Go 람다 함수를 빌드하는 것이 좋습니다. 다행히 Go용 AWS Lambda SDK는 이 기능을 잘 지원합니다. New Relic은 에이전트 모듈을 최신 상태로 유지할 것을 권장합니다. 지원은 3.9.0 이전 에이전트 버전에 대해 제한됩니다.
