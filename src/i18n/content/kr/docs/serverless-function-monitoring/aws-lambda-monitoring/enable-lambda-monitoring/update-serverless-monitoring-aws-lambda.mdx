...
2. `newrelic-log-ingestion` 함수를 설치한 각 지역에 대해 다음 명령어를 실행하여 <var>YOUR_REGION</var> 을 지역 식별자(예: `us-west-2` )로 바꿉니다.
...
1. 다음을 실행하여 <var>YOUR_REGION</var> 을 해당 지역(예: `us-west-2` )으로 바꿉니다.
   ...
   이 명령은 여러 필드를 출력하며 그 중 하나는 방금 생성한 변경 세트에 대한 ARN인 ChangeSetId입니다. 해당 ARN을 복사합니다.
...
현재 New Relic의 로그 관리가 활성화되어 있지 않지만 다음을 수행하고 싶은 경우:
...
2. `newrelic-log-ingestion` 함수를 설치한 각 지역에 대해 다음 명령어를 실행하여 <var>YOUR_REGION</var> 을 해당 지역(예: `us-west-2` )으로 바꿉니다.
