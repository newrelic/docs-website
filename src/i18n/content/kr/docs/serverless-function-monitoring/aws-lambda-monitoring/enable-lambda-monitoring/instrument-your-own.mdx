...
5. New Relic [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 암호에 대한 액세스를 허용하도록 실행 역할 수정

   1. 이름이 지정된 암호의 ARN 찾기 `NEW_RELIC_LICENSE_KEY`
   2. 함수의 실행 역할에 다음과 같은 새 인라인 정책을 추가합니다(SECRET_ARN을 위에서 찾은 값으로 대체).
