...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="서버리스 프레임워크"
  >
    서버리스 프레임워크는 서버리스 애플리케이션을 위한 널리 사용되는 개발 및 배포 도구입니다. 이는 Node.js로 작성되었으며 AWS의 경우 대부분 CloudFormation 템플릿 위에서 더 높은 수준의 추상화 역할을 합니다. Node, Python, Ruby, Java 및 .NET 함수에 적합합니다.
...
<CollapserGroup>
  ...
  >
    ...
    5. <DNT>**Subscription filter pattern to**</DNT> `?REPORT ?NR_LAMBDA_MONITORING ?"Task timed out" ?RequestId` 을 설정합니다. 또는 `LOGGING_ENABLED` 환경 변수를 사용하여 [모든 로그를 로그로 스트리밍하는](#stream-all-logs) 경우 이 필드를 비워 두세요.
