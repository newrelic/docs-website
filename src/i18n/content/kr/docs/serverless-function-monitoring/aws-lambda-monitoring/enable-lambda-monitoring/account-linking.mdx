...
이 통합 단계에서는 원격 분석을 New Relic 계정으로 보낼 수 있도록 New Relic <InlinePopover type="licenseKey" /> 도 AWS Secrets Manager 서비스에 저장합니다.
...
`newrelic-lambda` CLI는 보안 강화를 위해 New Relic <InlinePopover type="licenseKey" /> 를 [AWS Secret Manager](https://aws.amazon.com/secrets-manager/) 의 비밀로 추가합니다.

<Callout variant="tip">
  **AWS Secrets Manager에 New Relic 라이선스 키 저장**

  귀하의 <InlinePopover type="licenseKey" /> 는 New Relic에 대해 귀하를 식별하고 인증하여 귀하의 원격 측정을 귀하의 New Relic 계정과 연결할 수 있도록 합니다. 원격 측정을 보내는 각 기능은 이 값에 액세스해야 하며 안전하게 관리되어야 합니다. AWS Secrets Manager는 이러한 문제를 해결합니다.
...
<CollapserGroup>
  ...
  >
    ...
    다음에 설명된 대로 <InlinePopover type="licenseKey" /> 암호를 수동으로 구성하는 것을 잊지 마십시오.
    ...
    2. AWS CLI 또는 AWS CloudFormation 콘솔을 사용하여 `LicenseKey` 매개변수를 제공하여 템플릿을 설치합니다. "INGEST - LICENSE"라고 표시됩니다. 위의 인프라 UI로 구성한 계정의 <InlinePopover type="licenseKey" /> 를 사용해야 합니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <InlinePopover type="licenseKey" /> 는 New Relic 계정에 연결되어 있고 암호는 라이선스 키가 포함된 AWS 관리 암호의 이름입니다. `NEW_RELIC_LICENSE_KEY` 가 설정되지 않은 경우 `NEW_RELIC_LICENSE_KEY_SECRET` 에서 키를 찾습니다. 키 내용이 올바르게 형식화되었는지 확인하려면 [CLI](#rec) 를 사용하여 람다 모니터링을 활성화하는 것이 좋습니다.
...
조직에서 AWS Secrets Manager 사용을 허용하지 않는 경우 New Relic Lambda Extension은 `NEW_RELIC_LICENSE_KEY` 환경 변수를 허용합니다. `newrelic-lambda integrations install` 명령에서 `--disable-license-key-secret` 플래그를 추가합니다. 그런 다음 이 환경 변수를 Lambda 함수 구성의 <InlinePopover type="licenseKey" /> 설정합니다.

### 여러 AWS 리전 및 계정 [#multiple-regions-accounts]

`newrelic-lambda` CLI는 `--aws-region` 매개변수를 사용하여 리전당 한 번 실행해야 합니다. 동일한 연결된 계정 이름을 사용하면 도구에서 계정 연결이 이미 생성되었음을 감지합니다. <InlinePopover type="licenseKey" /> 시크릿은 각 리전에서 생성해야 합니다.
