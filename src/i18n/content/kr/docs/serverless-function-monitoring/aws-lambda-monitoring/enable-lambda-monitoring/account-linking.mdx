...
이 통합 단계에서는 귀하의 뉴렐릭 <InlinePopover type="licenseKey"/>도 AWS Secrets Manager 서비스에 저장하여 귀하의 뉴렐릭 계정으로 텔레메트리를 보낼 수 있습니다.
...
* 뉴렐릭 계정. 관리자 역할이나
  ...
  [추가 기능 역할이](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model#add-on) 있어야 합니다.
...
<CollapserGroup>
  ...
  >
    ...
    지역을 구성하려면 이 [환경 변수를](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.html) 사용하여 기본 지역을 재정의하세요.
...
`newrelic-lambda` CLI는 보안 강화를 위해 [AWS Secret Manager](https://aws.amazon.com/secrets-manager/) 의 비밀로 뉴렐릭 <InlinePopover type="licenseKey"/>(를) 추가합니다.
...
<Callout variant="tip">
  ...
  귀하의 <InlinePopover type="licenseKey"/>는) 귀하를 뉴렐릭으로 식별하고 인증하여 귀하의 델메트리를 귀하의 뉴렐릭 계정과 연결할 수 있게 해줍니다. 텔레메트리를 보내는 각 함수는 이 값에 액세스해야 하며 안전하게 관리되어야 합니다. AWS Secrets Manager는 이러한 문제를 해결합니다.
...
<CollapserGroup>
  ...
  >
    ...
    다음에 설명된 대로 <InlinePopover type="licenseKey"/>비밀을 수동으로 구성하는 것을 잊지 마세요.
    ...
    2. AWS CLI 또는 AWS CloudFormation 콘솔을 사용하여 템플릿을 설치하고 `LicenseKey` 파라미터를 제공합니다. "INGEST - LICENSE"라는 라벨이 붙습니다. 위의 인프라 UI로 구성한 계정에 대해 <InlinePopover type="licenseKey"/>를 사용해야 합니다.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Node, Python, 루비, Java에 필요합니다. .NET의 경우 선택 사항입니다.
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <InlinePopover type="licenseKey"/>은(는) 뉴렐릭 계정과 연결되어 있고, 비밀은 라이선스 키가 포함된 AWS 관리형 비밀의 이름입니다. `NEW_RELIC_LICENSE_KEY` 설정되지 않은 경우 `NEW_RELIC_LICENSE_KEY_SECRET` 에서 키를 찾습니다. 키 내용의 형식이 올바른지 확인하려면 [CLI를](#rec) 사용하여 람다 모니터링을 활성화하는 것이 좋습니다.

            <InlinePopover type="licenseKey"/>에 대해 자세히 알아보세요.
          </td>
        </tr>
      </tbody>
    </table>
...
Lambda 함수를 업로드했지만 뉴렐릭의 <DoNotTranslate>**Amazon Web Services -> Lambda functions**</DoNotTranslate> 섹션에 권한이 표시되지 않습니다.
...
두 개의 통합이 이미 생성된 경우 뉴렐릭의 <DoNotTranslate>**Infrastructure > AWS**</DoNotTranslate> 에서 <DoNotTranslate>**Unlink this account**</DoNotTranslate> 클릭하여 유지할 하나를 선택하고 다른 통합을 연결 해제하세요.
