...
쿼리는 데이터가 [OpenTelemetry Go 런타임 측정, 리소스 저장소](https://opentelemetry.io/docs/specs/semconv/runtime/go-metrics/) 에 의해 생성된다고 가정합니다.

<Callout variant="important">
  Go 런타임 메트릭에 대한 [의미 규칙은](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/runtime/go-metrics.md) 아직 개발 중입니다. Go 런타임 페이지는 이전 버전의 규칙을 목표로 합니다. `OTEL_GO_X_DEPRECATED_RUNTIME_METRICS` 환경 변수를 활성화하여 이전 규칙을 활성화해야 합니다. 자세한 내용은 측정 및 계측에 대한 [설명서를](https://github.com/open-telemetry/opentelemetry-go-contrib/blob/9a6e0bf68e1da495be52faf152b5b4e6fb09d475/instrumentation/runtime/internal/x/README.md) 참조하세요.

  뉴렐릭은 새로운 규칙이 안정되면 결국 이를 지지할 것입니다.
</Callout>
...
## 뉴렐릭에서 오류가 표현되는 방식 [#errors]

애플리케이션의 오류 정보는 APM UI의 두 곳에 표시됩니다:

* **오류율**: [요약 페이지](#summary-page)에 표시되며, 메트릭 데이터에서 파생됩니다. `http.server.request.duration` 또는 `rpc.server.call.duration` 과 같은 OpenTelemetry 메트릭에 `error.type` 속성이 포함된 경우, 해당 데이터 포인트는 오류율에 반영됩니다.
* **에러스 인박스(errors inbox)**: 스팬 데이터로 구동되는 [에러스 인박스(errors inbox) 페이지](#errors-inbox-page)를 확인하십시오. 스팬은 상태가 `ERROR` (으)로 설정될 때(예: `otel.status_code = ERROR`) 오류로 분류됩니다.

### 오류 무시 [#ignore-errors]

뉴렐릭 언어 에이전트는 HTTP 상태 코드 또는 오류 클래스별로 오류를 무시하도록 구성할 수 있어, 오류율에 영향을 미치거나 에러스 인박스(errors inbox)에 나타나는 것을 방지할 수 있습니다. OpenTelemetry에서는 동등한 에이전트 설정이 없지만, 데이터가 뉴렐릭에 도달하기 전에 기본 데이터를 조작하여 동일한 효과를 얻을 수 있습니다.

#### 오류율에서 무시된 오류

오류율은 우리가 [트랜잭션을 파생하는](#transaction-mapping) 것과 동일한 메트릭 데이터에서 파생됩니다. `http.server.request.duration` 또는 `rpc.server.call.duration` 과 같은 메트릭에 `error.type` 속성이 포함된 경우, 해당 데이터 포인트는 오류율에 반영됩니다. 메트릭 데이터 포인트에서 `error.type` 속성을 제거하면 오류로 계산되는 것을 방지합니다.

<Callout variant="important">
  `error.type` 속성은 이전 버전의 OpenTelemetry 의미 체계 규칙에 존재하지 않았습니다. HTTP 및 RPC 의미론적 규칙의 이전 버전을 따르는 이전 계측을 지원하기 위해 뉴렐릭은 `http.status_code` 및 `rpc.grpc.status_code` 속성을 활용하여 오류율을 산출합니다. 이러한 속성은 이전 `http.server.duration` 및 `rpc.server.duration` 메트릭에 존재했습니다.
</Callout>

#### 에러스 인박스(errors inbox)에서 무시된 오류

에러스 인박스(errors inbox)는 스팬 데이터를 기반으로 합니다. 스팬은 상태가 `ERROR` (즉, `otel.status_code = ERROR`)로 설정될 때 오류로 분류됩니다. 스팬 상태 설정을 해제하여 더 이상 `ERROR` 이(가) 아니게 하면 스팬이 에러스 인박스(errors inbox)에 표시되는 것을 방지합니다.

#### 예시: OpenTelemetry Collector로 데이터 변환

[OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) 는 전송 중인 메트릭 및 스팬 데이터를 변환하여 특정 오류를 억제하는 데 사용할 수 있습니다. [무시된 오류 예제](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors) 는 다음을 수행하는 방법을 시연하는 작동하는 수집기 설정을 제공합니다:

* 오류율에서 제외하려면 메트릭 데이터 포인트에서 `error.type` 속성을 제거하십시오.
* 에러스 인박스(errors inbox)에 오류 스팬이 표시되지 않도록 스팬 상태 설정을 해제합니다.
