...
<Steps>
  ...
  <Step>
    ...
    * **리눅스:** `/etc/nrdot-collector-host/config.yaml`
    ...
    <Callout variant="important">
      핵심 범주 토글을 활성화하고 데이터 누락이 발견되면 스크래퍼가 완료하는 데 더 많은 시간을 확보하기 위해 `timeout` 과 `collection_interval` 값을 모두 늘리세요. 스크래핑 주기가 겹치는 것을 방지하려면 타임아웃 값을 항상 `collection_interval` 보다 작게 유지하십시오.
