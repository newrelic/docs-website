...
NRDOT Collector를 사용하여 Microsoft SQL Server 데이터베이스를 모니터할 수 있습니다. NRDOT 수집기는 뉴렐릭 전용 구성 요소가 포함된 사전 구성된 배포판입니다.

다음 방법을 사용하여 NRDOT Collector를 구성할 수 있습니다:

* [안내 설치](/docs/opentelemetry/database/otel-mssql/#guided): 안내 설치 방법은 SQL Server 모니터링을 위해 NRDOT Collector를 설치하고 구성하는 데 도움이 되도록 뉴렐릭 플랫폼에서 단계별 지침을 제공합니다.
* [수동 설치](/docs/opentelemetry/database/otel-mssql/#manual): 수동 설치 방법을 사용하면 이 문서의 지침에 따라 NRDOT Collector를 직접 설정할 수 있습니다. 이 방법은 직접 수행하는 방식을 선호하거나 특정 설정 요구 사항이 있는 사용자에게 이상적입니다.

<Tabs>
  <TabsBar>
    <TabsBarItem id="guided">안내 설치</TabsBarItem> <TabsBarItem id="manual">수동 설치</TabsBarItem>
  </TabsBar>
  ...
  <TabsPages>
    <TabsPageItem id="guided">
      ...
      7. 설치 후 <DNT>Test your integration</DNT> 화면에서 **연결 테스트** 를 클릭하여 NRDOT Collector가 SQL Server 인스턴스에 연결하고 뉴렐릭으로 데이터를 전송할 수 있는지 확인합니다. 테스트가 성공하면 확인 메시지가 표시됩니다.

      ### APM-데이터베이스 상관관계 설정 [#optional-service-id]
      ...
      <Steps>
        ...
        <Step>
          ...
          * **리눅스:** `/etc/nrdot-collector-host/config.yaml`
          ...
          <Callout variant="important">
            핵심 범주 토글을 활성화하고 데이터 누락이 발견되면 스크래퍼가 완료하는 데 더 많은 시간을 확보하기 위해 `timeout` 과 `collection_interval` 값을 모두 늘리세요. 스크래핑 주기가 겹치는 것을 방지하려면 타임아웃 값을 항상 `collection_interval` 보다 작게 유지하십시오. 예를 들어:
...
<CollapserGroup>
  ...
  <Collapser id="buffer-metrics" title="데이터베이스 버퍼 메트릭">
    이러한 정보를 수집하려면 `enable_database_buffer_metrics: true` 활성화하세요.
