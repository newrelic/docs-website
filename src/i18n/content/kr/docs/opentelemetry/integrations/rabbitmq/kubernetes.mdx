...
자동 파드 검색 기능이 있는 [NRDOT Collector](https://github.com/newrelic/nrdot-collector-releases/releases/latest) (권장) 또는 [OpenTelemetry Collector](https://github.com/open-telemetry/opentelemetry-collector-contrib) 사용하여 쿠버네티스 클러스터에서 실행 중인 RabbitMQ 모니터링합니다. 이 가이드는 Kubernetes 옵저버 기반의 동적 브로커 감지 기능을 사용하여 Helm 통해 수집기를 구현하고 배포하는 방법을 안내합니다.
    ...
    <TabsPageItem id="nrdot-k8s">
      <Steps>
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="k8s-reqs-nrdot" title="쿠버네티스 클러스터 요구 사항">
              * **Kubernetes 버전**: 1.19 이상
              * **kubectl 접근**: 구현, 배포, 서비스 및 RBAC 리소스를 생성하기 위한 적절한 RBAC 권한으로 구성되었습니다.
            </Collapser>

            <Collapser id="rabbitmq-reqs-nrdot" title="RabbitMQ 요구 사항">
            ...
            <Collapser id="newrelic-reqs-nrdot" title="뉴렐릭 요구 사항">
              * **계정**: 활성 뉴렐릭 계정
              * 귀하의 계정에 대한 **클러스터** [키](/docs/apis/intro-apis/new-relic-api-keys/#license-key)
              * [Kubernetes OpenTelemetry 기본 매니페스트 설치를](/docs/kubernetes-pixie/k8s-otel/install/#manifest-install)완료했습니다.
            </Collapser>
          </CollapserGroup>
        </Step>

        <Step>
          ### 관리 엔드포인트를 확인하세요 [#verify-endpoint-nrdot]
        ...
        <Step>
          ### NRDOT Collector구성하세요 [#configure-nrdot]

          Kubernetes 매니페스트를 사용하여 NRDOT Collector를 설치할 수 있습니다. Helm 차트 지원 기능이 곧 추가될 예정입니다.

          Kubernetes OpenTelemetry 매니페스트 기본 설치를 완료한 후 다음 단계에 따라 RabbitMQ 모니터링을 구성하십시오.

          1. 수집기 이미지를 NRDOT Collector 사용하도록 업데이트하세요.
          ...
          2. RabbitMQ 모니터링을 위해 [`deployment-configmap.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-k8s-otel-collector/examples/k8s/rendered/deployment-configmap.yaml) 을 업데이트하세요.

             `deployment-config.yaml` 내용 전체를 RabbitMQ 전용 설정으로 바꾸세요.
          ...
          3. RabbitMQ 설정을 사용자 지정하세요.

             * `rule` 을 RabbitMQ 파드 레이블에 맞게 업데이트하세요.
             * `admin`/`password` RabbitMQ 자격 증명에 맞게 변경하세요.
             * `your-cluster-name` 실제 클러스터 이름으로 바꾸세요.
             * `otlp_http/newrelic` 수출업체 아래의 `endpoint` 을 업데이트하세요. 해당 지역에 적합한 엔드포인트를 선택하려면 OTLP 엔드포인트 설정 [문서를](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) 참조하세요.

          4. 업데이트된 매니페스트를 적용하고 구현 및 배포를 다시 시작하십시오.
    ...
    <TabsPageItem id="otelcol-k8s">
      <Steps>
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="k8s-reqs-otelcol" title="쿠버네티스 클러스터 요구 사항">
              * **Kubernetes 버전**: 1.19 이상
              * **kubectl 접근**: 구현, 배포, 서비스 및 RBAC 리소스를 생성하기 위한 적절한 RBAC 권한으로 구성되었습니다.
        ...
        <Step>
          ### 관리 엔드포인트를 확인하세요 [#verify-endpoint-otelcol]
        ...
        <Step>
          ...
          <Tabs>
            <TabsBar>
              <TabsBarItem id="us-otelcol-k8s">미국 지역</TabsBarItem> <TabsBarItem id="eu-otelcol-k8s">EU 지역</TabsBarItem>
            </TabsBar>
