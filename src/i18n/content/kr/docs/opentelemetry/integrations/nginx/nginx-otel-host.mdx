...
{\* &lt;Callout variant=&quot;important&quot;&gt; \*\*시작하기 전에\*\*: \[NGINX OpenTelemetry 개요]\(/docs/OpenTelemetry/통합/nginx/nginx-otel-overview/)를 검토하여 무엇을 이해해야 하는지, 그리고 이 통합의 이점을 이해하십시오. \*\*기타 NGINX 모니터링 옵션:\*\* - \*\*NGINX Plus\*\*를 모니터링하려면 \[모니터 NGINX Plus with OpenTelemetry]\(/docs/OpenTelemetry/통합/nginx-plus/nginx-plus-otel/)를 참조하세요. - \*\* Kubernetes의 NGINX\*\*를 모니터링하려면 \[ OpenTelemetry 사용하는 Kubernetes 의 모니터 NGINX]\(/docs/OpenTelemetry/통합/nginx/nginx-otel-Kubernetes/)를 참조하세요. &lt;/Callout&gt; &lt;Callout variant=&quot;tip&quot;&gt; \*\*설정 시간\*\*: 약 20분 | \*\*기술 수준\*\*: 중급 (기본적인 Linux/NGINX 지식 필요) &lt;/Callout&gt; */}
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-data" title="데이터 수집 확인">
    ...
    5. 뉴렐릭에 초기 데이터가 나타날 때까지 기다린 후 [NGINX에 접속하여](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data) 데이터 수집을 확인하십시오.
  </Collapser>
</CollapserGroup>

## 5단계: (선택 사항) NGINX 로그 전달 [#forward-logs]

지표 외에도 포괄적인 모니터링 및 문제 해결, 해결을 위해 NGINX [액세스 및 오류 로그를](https://docs.nginx.com/nginx/admin-guide/monitoring/logging/) 뉴렐릭으로 보낼 수 있습니다. 이러한 로그는 [핵심 NGINX 지표를](/docs/opentelemetry/integrations/nginx/nginx-otel-metrics-reference#core-metrics) 보완하고 자세한 요청 수준 인사이트를 제공합니다.
