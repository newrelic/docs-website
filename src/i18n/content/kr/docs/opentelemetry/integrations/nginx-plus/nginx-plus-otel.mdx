...
<CollapserGroup>
  ...
  <Collapser id="step2-enable-additional-metrics-otelcol" title="(선택 사항) 추가 참조 활성화">
    기본적으로 설정은 핵심 정보(연결, HTTP requests 및 SSL)만 수집합니다. 서버 영역, 위치 영역 또는 캐시 영역과 같은 추가 영역 범주를 활성화하려면 다음을 수행하십시오.

    1. 아래 [메트릭 목록](#metrics)에서 **활성화하려는 메트릭을 선택하십시오**. 해당 영역이 어떤 범주에 속하는지 기록해 두세요(예: HTTP 서버 영역, 위치 영역, 캐시).
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-dashboard" title="뉴렐릭에서 데이터 확인">
    [NGINX 대시보드](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data) 에 접속하여 데이터 수집 여부를 확인하십시오.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="plus-server-zone" title="HTTP 서버 영역">
    <Callout variant="important">
      **참고:** 이러한 기능은 기본적으로 비활성화되어 있습니다. 이를 활성화하려면 [(선택 사항) OpenTelemetry Collector Contrib에 대한 추가 메트릭 활성화를](#step2-enable-additional-metrics-otelcol) 참조하세요.
    </Callout>
