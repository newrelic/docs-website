---
title: 호스트에서 레디스 모니터링(OpenTelemetry)
tags:
  - Integrations
  - OpenTelemetry
  - Redis
...
* root 또는 sudo 권한이 있는 Linux 호스트
* 실행 중이고 액세스 가능한 레디스 ― 버전 6.0 이상 권장(4.0 이상은 축소된 메트릭 세트로 작동함)
* 레디스 엔드포인트(기본값 `localhost:6379`)에 대한 네트워크 연결
* 뉴렐릭의 [OTLP 엔드포인트](/docs/opentelemetry/best-practices/opentelemetry-otlp)에 대한 아웃바운드 HTTPS(포트 443)
* 해당 호스트에 설치된 OpenTelemetry Collector ― NRDOT 수집기 또는 OpenTelemetry Collector Contrib ― [설치 옵션](#installation-options)에서 선택할 수 있습니다.
* Prometheus 수신자 경로의 경우에만 레디스와 함께 실행되는 [redis\_exporter](https://github.com/oliver006/redis_exporter)

각 경로의 설정 단계는 [NRDOT 수집기](/docs/opentelemetry/nrdot/nrdot-collector/), `otelcol-contrib` 또는 redis\_exporter 등 여전히 필요한 모든 설치를 다룹니다.
...
<Tabs>
  ...
  <TabsPages>
      ...
      <Steps>
        <Step>
          ...
          * 처리된 메트릭을 OTLP를 통해 뉴렐릭으로 **내보내기**
          ...
          다음 설정을 붙여넣고, 레디스 `endpoint` 이(가) `localhost:6379`이(가) 아닌 경우 업데이트합니다:
          ...
          | `memory_limiter`             | 호스트를 보호하기 위해 수집기 메모리 사용량을 제한합니다(512 MiB 소프트 제한, 128 MiB 스파이크).                                    |
        ...
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="auth-requirepass" title="구형 비밀번호(requirepass)">
              단일 공유 비밀번호(redis.conf의 `requirepass`)를 사용하는 레디스 인스턴스의 경우:
              ...
              환경 파일에 `REDIS_PASSWORD` 을(를) 추가합니다:
            ...
            <Collapser id="auth-acl" title="ACL 사용자 이름 + 비밀번호(레디스 6+)">
              사용자별 자격 증명이 있는 액세스 제어 목록을 사용하는 레디스 6+ 인스턴스의 경우:

              **1단계:** 레디스 서버에 모니터링 전용 사용자를 생성합니다(관리자 액세스 필요):

              ```bash
              redis-cli ACL SETUSER otel-monitor on >your-monitor-password ~* +info +ping
              ```

              이것은 `INFO` 및 `PING` 만 실행할 수 있는 사용자를 생성합니다 ― 메트릭 수집에 필요한 최소한의 조건입니다.

              **2단계:** 수신자 설정에 자격 증명을 추가합니다:
            ...
            <Collapser id="auth-tls" title="TLS/SSL 암호화된 연결">
              암호화된 연결이 필요한 레디스 인스턴스의 경우:
              ...
              <Callout variant="tip">
                레디스에서 자체 서명된 인증서를 사용하고 확인을 건너뛰려는 경우(프로덕션 환경에서는 권장하지 않음) `ca_file`을(를) 제공하는 대신 `insecure: true` 을(를) 설정합니다.
              </Callout>
            </Collapser>
          </CollapserGroup>
        </Step>
        ...
        <Step>
          ...
          레디스 Cluster 모니터링은 현재 Prometheus 수신기 방식(`redis_exporter` 사용)이 필요합니다. NRDOT Collector의 기본 레디스 수신기는 클러스터 메트릭에 필요한 `CLUSTER INFO` 명령을 아직 지원하지 않습니다. 클러스터 모니터링 설정을 위해 [Prometheus 수신기](/docs/opentelemetry/integrations/redis/host/#prometheus) 탭을 사용하세요.
        </Step>
        ...
        <Step>
          ...
          | `OTELCOL_OPTIONS`             | 네   | 수집기가 레디스 구성 파일을 가리키도록 합니다.                                                                                     |
        ...
        <Step>
          ...
          `status` 명령은 `Active: active (running)`을(를) 표시해야 합니다. `Active: failed`가 표시되면 `journalctl -u nrdot-collector -n 100 --no-pager` 을(를) 사용하여 로그를 확인합니다 ― 가장 일반적인 원인은 YAML 들여쓰기 오류와 연결할 수 없는 레디스 엔드포인트입니다.

          그런 다음 메트릭이 뉴렐릭에 도달하는지 확인합니다. 다시 시작한 후 약 1분 정도 기다렸다가 [쿼리 빌더](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/)에서 다음 쿼리를 실행합니다:
    ...
    <TabsPageItem id="otel-contrib">
      <Steps>
        <Step>
          ...
          아직 없는 경우 [OpenTelemetry Collector Contrib](https://github.com/open-telemetry/opentelemetry-collector-contrib/releases/latest) 를 설치합니다:
          ...
          설치 후 수집기는 `otelcol-contrib.service`이라는 systemd 서비스로 사용할 수 있습니다.
        </Step>

        <Step>
          ### 레디스 모니터링 설정 [#configure-otel]
        ...
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="otel-auth-requirepass" title="구형 비밀번호(requirepass)">
              단일 공유 비밀번호(redis.conf의 `requirepass`)를 사용하는 레디스 인스턴스의 경우:
            ...
            <Collapser id="otel-auth-acl" title="ACL 사용자 이름 + 비밀번호(레디스 6+)">
              액세스 제어 목록(ACL)을 사용하는 레디스 6+ 인스턴스의 경우:
            ...
            <Collapser id="otel-auth-tls" title="TLS/SSL 암호화된 연결">
              암호화된 연결이 필요한 레디스 인스턴스의 경우:
        ...
        <Step>
          ...
          레디스 Cluster 모니터링은 현재 Prometheus 수신기 방식(`redis_exporter` 사용)이 필요합니다. OpenTelemetry Collector Contrib의 기본 레디스 수신기는 클러스터 메트릭에 필요한 `CLUSTER INFO` 명령을 아직 지원하지 않습니다. 클러스터 모니터링 설정을 위해 [Prometheus 수신기](/docs/opentelemetry/integrations/redis/host/#prometheus) 탭을 사용하세요.
        </Step>
        ...
        <Step>
          ...
          `status` 명령은 `Active: active (running)`을(를) 표시해야 합니다. `Active: failed`가 표시되면 `journalctl -u otelcol-contrib -n 100 --no-pager` 을(를) 사용하여 로그를 확인합니다 ― 가장 일반적인 원인은 YAML 들여쓰기 오류와 연결할 수 없는 레디스 엔드포인트입니다.
    ...
    <TabsPageItem id="prometheus">
      <Steps>
        <Step>
          ### redis\_exporter 설치 [#install-exporter]

          [redis\_exporter](https://github.com/oliver006/redis_exporter) 는 포트 9121에서 Prometheus 형식으로 레디스 메트릭을 노출합니다.
          ...
          `redis_exporter`에 대한 systemd 서비스를 생성합니다:
          ...
          `redis_up 1`이(가) 표시되어야 합니다.

          <CollapserGroup>
            <Collapser id="exporter-auth" title="레디스 인증 및 TLS">
              레디스에 비밀번호가 필요한 경우:
              ...
              TLS 암호화된 레디스의 경우 `rediss://` 체계를 사용합니다:
        ...
        <Step>
          ...
          * OTel Contrib: `/etc/otelcol-contrib/redis-prometheus-config.yaml`
          ...
          | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
          | `health_check`               | 수집기가 실행 중인지 확인할 수 있도록 `0.0.0.0:13133` 에 상태 엔드포인트를 노출합니다.                                                                              |
          | `prometheus` 수신기             | 10초마다 `redis_exporter` 엔드포인트(기본값 `localhost:9121`)를 스크랩하고 익스포터 자체의 `go_*`, `process_*`, `promhttp_*` 및 `redis_exporter_*` 메트릭을 삭제합니다. |
          | `memory_limiter`             | 호스트를 보호하기 위해 수집기 메모리 사용량을 제한합니다(512 MiB 소프트 제한, 128 MiB 스파이크).                                                                        |
          | `resource_detection`         | 호스트를 감지하고 `host.name` 및 `host.id`을(를) 추가하여 레디스 메트릭을 기본 호스트 엔티티에 연결합니다.                                                                |
        ...
        <Step>
          ### 환경 변수 및 구성(config) 경로 설정 [#env-prometheus]
          ...
          | `NEW_RELIC_LICENSE_KEY`       | 네   | 귀하의 뉴렐릭 [수집 라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key).                             |
          | `OTEL_EXPORTER_OTLP_ENDPOINT` | 네   | 해당 리전의 뉴렐릭 OTLP 엔드포인트. 자세한 내용은 [뉴렐릭 OTLP 엔드포인트](/docs/opentelemetry/best-practices/opentelemetry-otlp)를 참조하세요. |
          | `OTELCOL_OPTIONS`             | 네   | 수집기가 레디스 프로메테우스 설정 파일을 가리키도록 합니다.                                                                              |
        ...
        <Step>
          ...
          **OTel Collector Contrib**의 경우:
          ...
          `status` 명령은 `Active: active (running)`을(를) 표시해야 합니다. `Active: failed`가 표시되면 `journalctl -u nrdot-collector -n 100 --no-pager` (또는 `otelcol-contrib`)을(를) 사용하여 로그를 확인하세요 ― 가장 일반적인 원인은 YAML 들여쓰기 오류, 연결할 수 없는 `redis_exporter` 또는 실행되지 않는 익스포터입니다.
...
* **[Kubernetes 설치](/docs/opentelemetry/integrations/redis/kubernetes)**: DaemonSet 패턴을 사용하여 Kubernetes에 배포합니다.
