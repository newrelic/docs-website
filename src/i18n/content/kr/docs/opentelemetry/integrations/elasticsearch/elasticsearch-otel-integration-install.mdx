...
* **OpenTelemetry Collector** - [OpenTelemetry Collector Contrib이](https://github.com/open-telemetry/opentelemetry-collector-contrib/releases/latest) 호스트에 설치되어 실행 중입니다. systemd 서비스 유닛이 올바르게 생성되도록 공식 패키지(.deb 또는 .rpm)를 통해 설치하십시오.
...
<CollapserGroup>
  ...
  <Collapser id="logging-config" title="로그 활성화(파일로그 수신기)">
    **선택 사항:** 지표 외에 Elasticsearch 로그 파일을 뉴렐릭으로 보내려는 경우 이를 포함합니다.
    ...
    Elasticsearch 로그를 수집하고 전달하려면 `filelog` 수신자 설정을 추가하세요. `otelcol-contrib` 사용자가 로그 파일에 `read` 액세스 권한을 가지고 있는지 확인하십시오.
...
2. OTLP 엔드포인트를 `environment.conf` 에 입력하세요. `YOUR_LICENSE_KEY` 뉴렐릭 볼륨 키로 바꾸고 `YOUR_OTLP_ENDPOINT` 해당 지역에 적합한 엔드포인트로 바꾸세요. 올바른 엔드포인트를 선택하려면 OTLP 엔드포인트 설정 [문서를](https://docs.newrelic.com/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) 참조하세요.
