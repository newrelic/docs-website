...
이 페이지에는 OpenTelemetry 사용하여 프레임 아파치 Kafka를 수집했을 때 수집된 지표가 기록되어 있습니다. 지표는 [Kafka 지표 수신기](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver), JMX 지표(OpenTelemetry 지표 에이전트 또는 Prometheus JMX 내보내기 도구) 및 클라이언트 측 [OpenTelemetry 지표](https://opentelemetry.io/docs/languages/java/automatic/) 지표를 통해 수집됩니다.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            파티션의 동기화된 복제본 수. 이 메트릭은 수집기 파이프라인에서 필터링되어 제외됩니다 ― 토픽당 집계된 `kafka.partition.replicas_in_sync.total` 만 유지됩니다.
          </td>
...
* **자체 호스팅 Kafka**: [OpenTelemetry 자바 에이전트](/docs/opentelemetry/integrations/kafka/self-hosted#java-agent) 또는 [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/self-hosted#prometheus)
* **Kubernetes(자체 관리형)**: [OpenTelemetry 자바 에이전트](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#java-agent) 또는 [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#prometheus)
* **Kubernetes (Strimzi)**: 뉴렐릭 사용자 정의 설정을 갖춘 [Prometheus JMX 내보내기](/docs/opentelemetry/integrations/kafka/kubernetes-strimzi#configure-jmx-exporter)

두 방법 모두 아래에 설명된 것과 동일한 Kafka 브로커 및 JVM 메트릭 세트를 수집합니다.

<CollapserGroup>
  <Collapser id="jmx-kafka-metrics" title="Kafka 브로커 메트릭">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            리더 선출 횟수. 리더 선출이 발생할 때만 나타나며, 안정적인 클러스터에서는 발생하지 않습니다.
          </td>
...
## Kafka 클라이언트 메트릭 [#kafka-client-metrics]
...
<CollapserGroup>
  <Collapser id="producer-metrics" title="프로듀서 지표">
    ...
    | `kafka.producer.incoming_byte_rate`        | 수신 바이트 비율(브로커의 응답) | 클라이언트 ID, 노드 ID |
    | `kafka.producer.incoming_byte_total`       | 총 수신 바이트(브로커의 응답)  | 클라이언트 ID, 노드 ID |
  ...
  <Collapser id="consumer-metrics" title="소비자 기준">
    ...
    | `kafka.consumer.partition_assigned_latency_avg` | 평균 파티션 할당 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
    | `kafka.consumer.partition_assigned_latency_max` | 최대 파티션 할당 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
    | `kafka.consumer.partition_revoked_latency_avg`  | 평균 파티션 회수 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
    | `kafka.consumer.partition_revoked_latency_max`  | 최대 파티션 철회 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
    | `kafka.consumer.partition_lost_latency_avg`     | 평균 파티션 손실 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
    | `kafka.consumer.partition_lost_latency_max`     | 최대 파티션 손실 지연시간(ms). 소비자 그룹 리밸런싱 중에만 발생합니다. | 클라이언트 ID |
