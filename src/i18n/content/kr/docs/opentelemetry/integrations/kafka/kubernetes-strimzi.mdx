...
* JMX가 활성화된 [Strimzi 연산자를](https://strimzi.io/) 통해 Kafka를 구현하고 배포합니다. Strimzi Kafka 리소스에서 Kafka 클러스터에 JMX가 활성화되어 있는지 확인하십시오.
...
<Steps>
  ...
  <Step>
    ### 라이선스 키로 비밀 키를 생성하세요. [#create-secret]
  ...
  <Step>
    ### 구현하다, 배포하다 OpenTelemetry Collector [#deploy-collector]

    #### 3.1 사용자 지정 수집기 이미지 생성 [#build-image]

    런타임과 JMX 스크래퍼를 사용하여 사용자 정의 수집기 이미지를 생성합니다. NRDOT Collector (뉴렐릭 배포판) 또는 OpenTelemetry Collector 중에서 선택하십시오.
      ...
      <TabsPages>
        <TabsPageItem id="nrdot">
          ...
          <Callout variant="important">
            **버전 호환성**: 이 가이드에서는 JMX Scraper 1.52.0 및 NRDOT Collector를 사용합니다. 최상의 결과를 얻으려면 최신 버전을 사용하십시오.

            <CollapserGroup>
              <Collapser id="nrdot-version-compatibility" title="최신 NRDOT Collector 버전을 확인하세요.">
                **최신 버전을 확인하세요**:

                * NRDOT Collector: [NRDOT Collector 릴리스 페이지를](https://github.com/newrelic/nrdot-collector-releases/releases/latest) 방문하여 최신 버전을 찾고 Dockerfile에서 `{NRDOT_VERSION}` 버전으로 교체하십시오.
                * JMX 스크래퍼: [OpenTelemetry Java Contrib 릴리스](https://github.com/open-telemetry/opentelemetry-java-contrib/releases/latest)확인
              </Collapser>
            </CollapserGroup>

            **타겟, 목표대로**: 아래 예시에서는 `linux_amd64` 사용합니다. 시스템 아키텍처에 맞는 올바른 바이너리를 찾으려면 [NRDOT Collector 릴리스](https://github.com/newrelic/nrdot-collector-releases/releases/latest) 페이지를 참조하십시오(예: `linux_amd64`, `linux_arm64`). Dockerfile에서 다운로드 URL을 accordingly 업데이트하십시오.
          </Callout>
        ...
        <TabsPageItem id="otel">
          <Callout variant="important">
            **버전 호환성**: 이 가이드에서는 JMX Scraper 1.52.0 및 OpenTelemetry Collector 0.145.0을 사용합니다. 이전 수집기 버전에는 호환성 목록에 이 스크래퍼의 해시가 포함되어 있지 않을 수 있습니다. 최상의 결과를 얻으려면 이 가이드에 나와 있는 대로 최신 버전을 사용하십시오.
    ...
    **이미지를 빌드하고 푸시합니다.**
    ...
    Dockerfile을 저장한 후 이미지를 빌드하고 컨테이너 레지스트리에 푸시하세요.
