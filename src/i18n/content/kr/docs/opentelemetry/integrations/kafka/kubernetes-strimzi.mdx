...
<Steps>
          ...
          **2단계. 매니페스트 파일 생성**
          ...
          <CollapserGroup>
            <Collapser id="manifest-nrdot" title="NRDOT Collector 사용(권장)">
              ...
              **1단계. `collector-rbac.yaml`생성** - Kubernetes API 접근을 위한 RBAC 설정:
              ...
              **2단계. `collector-configmap.yaml`생성** - OpenTelemetry Collector 설정:
              ...
              **3단계. `collector-deployment.yaml`생성** - OpenTelemetry Collector 배포:
            ...
            <Collapser id="manifest-otel" title="OpenTelemetry Collector 사용">
              ...
              **1단계. `collector-rbac.yaml`생성** - 위의 NRDOT 옵션과 동일합니다(RBAC 설정은 동일합니다).

              **2단계. `collector-configmap.yaml`생성** - 위의 NRDOT 옵션과 동일합니다(설정은 동일합니다).

              **3단계. `collector-deployment.yaml`생성** - OpenTelemetry Collector 배포(이미지만 다름):
  ...
  <Step>
    ...
    **구성 매개변수**
    ...
    자바 에이전트는 코드 변경 없이 [즉시 사용 가능한 Kafka 계측](https://opentelemetry.io/docs/zero-code/java/spring-boot-starter/out-of-the-box-instrumentation/) 을 제공하여 다음을 캡처합니다:
  ...
  <Step>
    ### (선택 사항) Kafka 브로커 로그 전달 [#forward-broker-logs]

    Kafka 브로커 로그를 수집하여 뉴렐릭으로 전송하려면, 수집기 설정에 filelog 수신기를 추가하세요.

    <CollapserGroup>
      <Collapser id="configure-log-collection-strimzi" title="로그 수집 구성">
        ...
        **구성 매개변수**
        ...
        | 매개변수                                                  | 설명                                                                                                                                                                                                                                                                   |
        | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
        | `filelog/kafka_broker_0.include`                      | `/var/log/kafka/server.log` 을(를) 브로커 파드 내의 실제 Kafka 로그 경로로 업데이트하세요. Strimzi의 경우, 로그는 일반적으로 stdout에 기록됩니다 ― 이를 파일로 리디렉션하려면 [Strimzi 로그 설정](https://strimzi.io/docs/operators/latest/configuring.html#property-kafka-logging-reference) 을 사용하거나 대신 사이드카 로그 수집기를 사용하세요. |
      ...
      <Collapser id="find-broker-logs-in-new-relic-strimzi" title="뉴렐릭에서 내 로그인 찾기">
...
Strimzi Kafka 사용자 지정 리소스, JMX Exporter 설정, OTel Collector 설정 및 샘플 생산자/소비자 애플리케이션이 포함된 완전하게 작동하는 예제는 [뉴렐릭 OpenTelemetry 예제 저장소](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/collector/kafka/k8s-strimzi)에서 확인할 수 있습니다.
