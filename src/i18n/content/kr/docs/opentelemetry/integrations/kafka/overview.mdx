...
* **JMX 메트릭 수집** (브로커 성능, JVM 데이터 및 운영 관련 인사이트):

  * **자체 호스팅 Kafka**: 사용자 정의 JMX 설정이 포함된[OpenTelemetry 잔류 에이전트는](https://github.com/open-telemetry/opentelemetry-java-instrumentation) Kafka 브로커에 연결된 잔류 에이전트로 실행되어 OTLP를 통해 수집기의 [OTLP 수신기](https://github.com/open-telemetry/opentelemetry-collector/tree/main/receiver/otlpreceiver)로 지표를 보냅니다.
  * **Kubernetes (Strimzi)**: 뉴렐릭 사용자 정의 설정이 포함된 [Prometheus JMX 내보내기는](https://github.com/prometheus/jmx_exporter) 수집기의 [Prometheus 수신기](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/prometheusreceiver)에 의해 스크랩된 포트 9404에 지표를 노출합니다.
