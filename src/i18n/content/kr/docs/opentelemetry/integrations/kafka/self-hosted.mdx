    ...
    <TabsPageItem id="java-agent">
      ...
      <Steps>
        ...
        <Step>
          ...
          | `NEW_RELIC_OTLP_ENDPOINT`          | OTLP 인제스트 엔드포인트. 미국 리전(기본값)에는 `https://otlp.nr-data.net:4317`을(를), EU 리전에는 `https://otlp.eu01.nr-data.net:4317`을(를), JP 리전에는 `https://otlp.jp.nr-data.net:4317`을(를) 사용하십시오. 다른 설정에 대해서는 [OTLP 엔드포인트 설정](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)을 참조하세요. |
        </Step>
