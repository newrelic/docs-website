...
<Steps>
  ...
  <Step>
    ...
    * OTLP 엔드포인트: `https://otlp.nr-data.net:4317` (미국 지역) 또는 `https://otlp.eu01.nr-data.net:4317` (유럽 지역)을 사용합니다. 다른 엔드포인트 설정에 대해서는 [OTLP 엔드포인트 구성을](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)참조하세요.
  </Step>
...
<CollapserGroup>
  ...
  <Collapser id="ingest-optimization" title="데이터 수집 줄이기">
    ...
    **1. JMX 설정에서 추가 메트릭 섹션을 제거합니다.**
    ...
    `~/opentelemetry/jmx-custom-config.yaml` 에서 이 주석 아래의 모든 내용(파일 끝까지)을 삭제하세요.
    ...
    **2. Kafkametrics 수신기에서 소비자 오프셋 메트릭을 비활성화합니다.**
    ...
    `~/opentelemetry/kafka-config.yaml` 에서 `kafkametrics` 수신자 `metrics` 섹션에 다음을 추가합니다.
