...
<Steps>
  ...
  <Step>
    ...
    수집기 사이드카 컨테이너를 포함하는 새 ECS 작업 정의를 생성합니다. NRDOT Collector(뉴렐릭의 배포판) 또는 OpenTelemetry Collector 중에서 선택한 다음, 컨테이너 플랫폼에 대한 작업 정의를 선택합니다:
    ...
    <Tabs>
      ...
      <TabsPages>
        <TabsPageItem id="ec2-nrdot">
          <Callout variant="tip">
            **NRDOT Collector** 는 뉴렐릭 지원이 제공되는 OpenTelemetry Collector의 뉴렐릭 배포판입니다. 이 설정에서 사용되는 `awsecscontainermetrics` 및 `hostmetrics` 수신기를 번들로 제공하므로, 이전 단계에서 저장한 것과 동일한 수집기 설정으로 실행됩니다.
          </Callout>
