...
이 페이지에서는 뉴렐릭이 OpenTelemetry [뉴렐릭 OTLP 엔드포인트 를](/docs/opentelemetry/best-practices/opentelemetry-otlp) 통해 수신한 지표를 처리하는 방법을 설명합니다. OpenTelemetry 지표를 뉴렐릭과 통합하려면 이 엔드포인트로 내보낼 지표 소스를 구성해야 합니다. 엔드포인트 설정 요구 사항을 검토하세요. OpenTelemetry 지표를 사용하여 서비스를 구성하는 방법에 대한 자세한 내용은 [OpenTelemetry APM 모델링을](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-intro) 참조하세요.
