...
<CollapserGroup>
  <Collapser id="iast-results" title="¿Dónde encuentro los resultados del IAST?">
  ...
  <Collapser id="see-my-application" title="No veo mi aplicación en IAST">
    ...
    * El log de la aplicación para saber si hay algún problema.
    ...
    * Proxy o firewall bloqueando el acceso. Agregue a la lista blanca las siguientes IP `18.188.255.152, 18.116.243.59, 18.219.12.171` para el dominio <DNT>**csec.nr-data.net**</DNT>, las siguientes IP `3.13.224.132, 18.218.78.150, 13.58.129.59` para el dominio <DNT>**csec-gov.nr-data.net**</DNT> y las siguientes IP `3.122.48.53, 63.176.82.192, 3.123.117.125` para el dominio <DNT>**csec.eu01.nr-data.net**</DNT>. Pero nuestra recomendación es agregar dominios a la lista blanca en lugar de las IP, ya que estas IP pueden cambiar en cualquier momento.
    * El certificado TLS es correcto. Agregue el certificado de CA de Let&apos;s Encrypt (descargar desde [Certificados de Let&apos;s Encrypt](https://letsencrypt.org/certificates/#root-certificates)) a su almacén de confianza local. Incluya tanto los certificados raíz como los intermedios (ISRG Root X1 &amp; Let&apos;s Encrypt R3) para establecer la cadena de confianza completa.
    * Se admite framework o la categoría de vulnerabilidades de su aplicación.
  </Collapser>

  <Collapser id="agent-working" title="No estoy seguro si el agente de seguridad funciona.">
    ...
    * En el directorio `nr-security-home/logs` , busque el archivo `LANGUAGE-security-collector-init.log` . Reemplace `LANGUAGE` en el nombre del archivo con el que está utilizando. Busque estos pasos para ver dónde está el problema:
      ...
      * \[PASO-8]: Ve un primer evento enviado para validación, lo que significa que el agente de seguridad se inició correctamente.
      ...
      A continuación se muestra un extracto del archivo de log `LANGUAGE-security-collector-init.log` del agente de seguridad:
  ...
  <Collapser id="be-sure-iast-working" title="No estoy seguro de que IAST esté funcionando">
    ...
    3. Vea los detalles de la prueba, incluidos <DNT>**APIs covered**</DNT>, <DNT>**methods calls**</DNT> y <DNT>**application testing efficiency**</DNT>.
       ...
       Además, si establece el nivel de logs en <DNT>**debug/finest**</DNT>, puede buscar <DNT>**Fuzz request received**</DNT> en el archivo `nr-security-home/logs/java-security-collector.log` . Esto muestra que el análisis IAST está en progreso.
  </Collapser>

  <Collapser id="looking-for-vulns" title="No estoy seguro de que IAST esté buscando vulnerabilidades">
  ...
  <Collapser id="tested-app" title="Probé una aplicación con vulnerabilidades conocidas, pero IAST no la encontró">
  ...
  <Collapser id="stability-issues" title="Tengo problemas de estabilidad de la aplicación">
    ...
    También puede consultar el archivo de log instantáneo en la carpeta `nr-security-home/logs/snapshots` . El archivo de log le muestra el estado del agente de seguridad, el uso de recursos y los últimos cinco errores.
  </Collapser>

  <Collapser id="unknown-files" title="Veo archivos y directorios desconocidos en el directorio de mi aplicación">
  ...
  <Collapser id="not-responding" title="Mi aplicación no responde o falla">
  ...
  <Collapser id="query-vulns-nrdb" title="¿Puedo consultar todas las vulnerabilidades de la aplicación desde NRDB?">
  ...
  <Collapser id="false-positive" title="Veo una vulnerabilidad que no es una vulnerabilidad real (falso positivo)">
  ...
  <Collapser id="golang-vuln" title="No se detectaron vulnerabilidades para mi aplicación Golang">
  ...
  <Collapser id="iast-windows" title="¿IAST es compatible con el entorno Windows?">
  ...
  <Collapser id="apis-available" title="¿Hay API disponibles para descargar los resultados de IAST?">
    No, no tenemos ninguna API para descargar los resultados de IAST.
  </Collapser>

  <Collapser id="iast-not-prod" title="¿Por qué no debería utilizarse IAST en producción?">
