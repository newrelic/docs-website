...
<CollapserGroup>
  <Collapser id="proxy-chart" title="Uso de la sección `proxy` en el gráfico de Helm">
    Puede emplear la sección `proxy` de su archivo [`values.yaml`](/docs/kubernetes-pixie/k8s-otel/install/#config-params) para la configuración del gráfico `nr-k8s-otel-collector`.
