...
## Compatibilidad con variantes y proveedores de Kubernetes [#enable-provider]
...
Si su proveedor es GKE Autopilot, no cambie el valor de `daemonset.containerSecurityContext.privileged` del valor predeterminado, ya que este proveedor no admite contenedores privilegiados.

Si utiliza Amazon EKS, actualice la configuración de Terraform de su clúster o utilice la AWS Console para habilitar el procesador `resourcedetection` mediante uno de los siguientes métodos:

* Aumente el límite de saltos de IMDS: establezca el `http_put_response_hop_limit` en 2 para sus grupos de nodos.
* Habilita la identidad de pod: agrega el complemento `eks-pod-identity-agent` a tu módulo de EKS.
...
Si su proveedor es OpenShift y tiene `receivers.filelog` habilitado (el valor predeterminado), realice las siguientes actualizaciones en `daemonset.containerSecurityContext` para garantizar que el logging tenga los privilegios necesarios para funcionar:
