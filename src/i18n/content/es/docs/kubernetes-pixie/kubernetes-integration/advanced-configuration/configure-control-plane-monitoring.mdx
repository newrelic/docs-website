...
<img title="Diagram showing a possible configuration scraping etcd with mTLS and API server with bearer Token." alt="Diagram showing a possible configuration scraping etcd with mTLS and API server with bearer Token. The monitoring is a DaemonSet deployed on master nodes only." src="/images/kubernetes_diagram_integration-cp.webp" />
...
<Callout variant="tip">
  Si aún no puede ver los datos del plano de control, pruebe la solución descrita en [Resolución de problemas de integración deKubernetes : No ver los datos](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/kubernetes-integration-troubleshooting-not-seeing-data/).
</Callout>