...
<CollapserGroup>
  <Collapser
    ...
    title="Obtenga la configuración YAML para la integración."
  >
...
<Callout variant="tip">
  Este formato se aplica a la [integración de Kubernetes v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3). Para la versión anterior v2, consulte la sección [siguiente](#configure-values-v2).
</Callout>
...
<Callout variant="tip">
  Observe que estamos especificando `--tls --port 10250`. Es posible que las versiones anteriores de la integración hubieran funcionado sin esto; a partir de [la versión 3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3) de la integración de Kubernetes es obligatorio especificarlas. El motivo de este cambio es que la integración ahora se conecta a Kubelet utilizando el nodeIP en lugar de `localhost`; el primero requiere TLS mientras que el segundo no.
</Callout>
...
#### Legacy v2) Agregue la configuración a `values.yaml` [#configure-values-v2]

<CollapserGroup>
  <Collapser
    id="example-config-map"
    title="Configuración de ejemplo para v2"
  >
    <Callout variant="tip">
      Este formato se aplica a la integración legacy Kubernetes v2. Para versiones más recientes, consulte la sección [anterior](#configure-values).
    </Callout>

    Este ejemplo muestra la configuración YAML para la integración de Redis agregada al archivo `values.yml` del gráfico `nri-bundle` .
...
Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**</DoNotTranslate>, seleccione <DoNotTranslate>**Third party services**</DoNotTranslate> y luego seleccione el dashboard del servicio. Debería ver los datos que se informan.
...
* La integración de Kubernetes no se actualiza automáticamente. Para obtener mejores resultados, [actualice](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure#update) periódicamente.
...
Si está utilizando nuestra [integración de Kubernetes v3](/docs/kubernetes-pixie/kubernetes-integration/get-started/changes-since-v3), este ConfigMap ya estará montado en el contenedor requerido.
