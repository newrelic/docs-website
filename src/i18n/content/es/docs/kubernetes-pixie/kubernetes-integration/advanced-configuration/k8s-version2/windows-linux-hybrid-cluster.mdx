...
<Callout variant="important">
  **El monitoreo de nodos de Windows ahora está disponible a nivel general (GA) en la integración de New Relic Kubernetes v3.** El enfoque de instalación híbrida descrito en esta página (usando un chart de `newrelic-infrastructure` v2 independiente para Windows) está obsoleto y se eliminará en una versión futura. Use el [proceso de instalación guiada](/install/kubernetes/) en su lugar — admite nodos de Linux y Windows en una sola instalación de forma predeterminada.
</Callout>
...
<Steps>
  <Step>
    ...
    * Pueden existir otros subcharts en `nri-bundle` que no estén representados en este archivo. Si desea agregar esos subcharts, entonces agregue sus pares de clave o valor a su archivo yaml.
