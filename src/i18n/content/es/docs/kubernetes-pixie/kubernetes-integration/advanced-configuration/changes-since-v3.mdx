...
* Tenga en cuenta que KSM v2+ desactiva las etiquetas métricas de forma predeterminada. Puedes habilitar el seguimiento de las etiquetas objetivo métricas utilizando las `metric-labels-allowlist` opciones [aquí](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) descritas en tu clúster de Kubernetes.
