...
* Tenga en cuenta que KSM v2+ desactiva las etiquetas métricas de forma predeterminada. Puedes habilitar el seguimiento de las etiquetas objetivo métricas utilizando las `metricLabelsAllowlist` opciones [aquí](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/cli-arguments.md) descritas en tu clúster de Kubernetes.
...
Las siguientes opciones del agente anteriormente tenían "alias" en la raíz del archivo `values.yml` y son <DoNotTranslate>**no longer available**</DoNotTranslate>:
