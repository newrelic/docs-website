...
Tenga en cuenta que la nueva [arquitectura`nri-kubernetes` v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/changes-since-v3/) aún no es compatible con Windows. Instalamos el gráfico `newrelic-infrastructure` como versión independiente, que le permite monitor su carga de trabajo de Windows.
...
<Steps>
  ...
  <Step>
    ### Definir `newrelic-infrastructure` para nodos de Windows [#newrelic-infra]
    ...
    Si está ejecutando una versión diferente del grupo de nodos de Windows o del grupo de nodos, ajuste la sección `windowsOsList` con los valores apropiados. Puede ver estos valores en nuestra sección [de instalación de Windows](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#windows-install) .
  </Step>
  ...
  <Step>
    ...
    Una vez que genere algunos datos, podrá verlos en New Relic. Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes**</DoNotTranslate> y luego seleccione su clúster.
  </Step>
</Steps>
