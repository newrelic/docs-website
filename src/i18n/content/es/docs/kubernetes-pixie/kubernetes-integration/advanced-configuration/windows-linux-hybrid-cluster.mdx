...
Tenga en cuenta que la nueva [arquitectura`nri-kubernetes` v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) aún no es compatible con Windows. Instalamos el gráfico `newrelic-infrastructure` como versión independiente, que le permite monitor su carga de trabajo de Windows.
