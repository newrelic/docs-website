...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="No hay metadatos Kubernetes en la transacción del atributo"
  >
    ### Problema
    ...
    5. Inicie un proxy para el servidor API de Kubernetes ejecutando este comando en una ventana de terminal y manténgalo funcionando.

       ```shell
       kubectl proxy --port=8001
       ```

    6. Cree un nuevo pod en su clúster, esto hará que el servidor API intente comunicar con el webhook. Este comando creará una casilla ocupada.

       ```shell
       kubectl create -f https://git.io/vPieo
       ```

    7. Recupere el registro del servidor API .

       ```shell
       curl localhost:8001/logs/kube-apiserver.log > apiserver.log
       ```

    8. Elimine el contenedor de BusyBox.

       ```shell
       kubectl delete -f https://git.io/vPieo
       ```

    9. Inspeccione el registro en busca de errores.
    ...
    10. Si no hay entradas log ni en el registro del servidor API ni en el despliegue de inyección de metadatos, significa que el webhook no se registró correctamente.

    11. Cerciorar de que el trabajo de configuración de la inyección de metadatos se ejecutó correctamente inspeccionando el resultado de este comando:

        ```shell
        kubectl get job newrelic-metadata-setup
        ```

    12. Si el trabajo no se completa, investigue el registro del trabajo de configuración:

        ```shell
        kubectl logs job/newrelic-metadata-setup
        ```

    13. Cerciorar de que `CertificateSigningRequest` esté aprobado y emitido ejecutando este comando:

        ```shell
        kubectl get csr newrelic-metadata-injection-svc.default
        ```

    14. Cerciorar de que el secreto TLS esté presente ejecutando este comando:

        ```shell
        kubectl get secret newrelic-metadata-injection-secret
        ```

    15. Asegúrese de que el paquete de CA esté presente en la configuración del webhook mutante:

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    16. Asegúrese de que el `TargetPort` del recurso <DNT>**Service**</DNT> coincida con el <DNT>**Port**</DNT> del contenedor de <DNT>**Deployment**</DNT>:
