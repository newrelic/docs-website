...
Para vincular su aplicación y Kubernetes, debe poder desplegar `MutatingWebhookConfiguration`s en su clúster de Kubernetes.
...
La inyección de metadatos se incluye cuando [instala nuestra integración usando Helm](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm). Solo asegúrese de que, cuando esté configurando el gráfico `nri-bundle` , el webhook que inyecta los metadatos esté habilitado:
...
De forma predeterminada, nuestro gráfico utiliza [kube-webhook-certgen](https://github.com/kubernetes/ingress-nginx/tree/main/images/kube-webhook-certgen) para generar automáticamente los certificados necesarios para que se ejecute el webhook.

Sin embargo, si tiene instalado [cert-manager](https://cert-manager.io/) , puede configurar nuestro gráfico para usar cert-manager en su lugar, lo que puede simplificar significativamente el proceso de implementación:
...
* Si su certificado/par de claves está en otro formato, consulte la [base de conocimientos de Digicert](https://knowledge.digicert.com/solution/SO26449.html) para obtener más ayuda.
...
Y vuelva a ejecutar el [comando de instalación](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm#install-k8-helm).
...
<CollapserGroup>
  <Collapser
    ...
    title="No hay metadatos Kubernetes en APM o rastreo distribuido transacción"
  >
    ### Problema
    ...
    2. El secreto se creará correctamente y el pod `k8s-metadata-injection` podrá iniciarse.
    3. Tenga en cuenta que la nueva versión del manifiesto y del `nri-bundle` ya están actualizadas con la versión correcta de la imagen.
  </Collapser>

  <Collapser
    id="no-metadata-in-transactions-attributes"
    title="No hay metadatos Kubernetes en el atributo de transacción"
  >
    ### Problema
    ...
    1. Verifique que las variables de entorno se estén inyectando correctamente siguiendo las instrucciones descritas en el paso [Validar su instalación](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) .

    2. Si no están presentes, obtenga el nombre del pod de inyección de metadatos ejecutando:
    ...
    3. En otra terminal, cree un nuevo pod (por ejemplo, consulte [Validar su instalación](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection)) e inspeccione el registro de metadatos de inyección desplegable en busca de errores. Por cada pod creado debería haber un conjunto de 4 entradas nuevas en el registro como:
    ...
    4. Para comprobar si el apiserver no puede comunicarse con el webhook, debe inspeccionar el registro del apiserver en busca de errores como:
       ...
       5. Inspeccione el registro en busca de errores.
          ...
          <Callout variant="tip">
            Uno de los [requisitos para la inyección de metadatos](#network-req) es que al apiserver se le debe permitir la salida al pod que se ejecuta en el clúster. Si encuentra errores relacionados con tiempos de espera de conexión o conexiones fallidas, asegúrese de verificar los grupos de seguridad y las reglas de firewall del clúster.
          </Callout>

    5. Si no hay entradas log ni en el registro del apiserver ni en los metadatos de inyección desplegable, significa que el webhook no se registró correctamente.

    6. Asegúrese de que el trabajo de configuración de inyección de metadatos se haya ejecutado correctamente inspeccionando el resultado de:

       ```shell
       kubectl get job newrelic-metadata-setup
       ```

    7. Si el trabajo no se completa, investigue el registro del trabajo de configuración:

       ```shell
       kubectl logs job/newrelic-metadata-setup
       ```

    8. Asegúrese de que `CertificateSigningRequest` esté aprobado y emitido ejecutando:

       ```shell
       kubectl get csr newrelic-metadata-injection-svc.default
       ```

    9. Asegúrese de que el secreto TLS esté presente ejecutando:

       ```shell
       kubectl get secret newrelic-metadata-injection-secret
       ```

    10. Asegúrese de que el paquete de CA esté presente en la configuración del webhook mutante:

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    11. Asegúrese de que el `TargetPort` del recurso <DoNotTranslate>**Service**</DoNotTranslate> coincida con el <DoNotTranslate>**Port**</DoNotTranslate> del contenedor de <DoNotTranslate>**Deployment**</DoNotTranslate>:
