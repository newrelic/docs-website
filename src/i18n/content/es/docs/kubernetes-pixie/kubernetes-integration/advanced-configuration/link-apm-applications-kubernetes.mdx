...
<CollapserGroup>
  ...
  <Collapser id="no-metadata-in-transactions-attributes" title="No hay metadatos Kubernetes en la transacción del atributo">
    ...
    1. Verifique que las variables de entorno se hayan inyectado correctamente en los nuevos pods siguiendo las instrucciones descritas en la sección [Validar la inyección de metadatos](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection).

    2. Si no existen, en primer lugar, siga los logs del servicio de webhook:

       ```shell
       kubectl logs -l app.kubernetes.io/name=nri-metadata-injection -f
       ```

    3. En otra terminal, cree un nuevo pod e inspeccione los logs del despliegue de inyección de metadatos en busca de errores. Consulte la sección [Validar la inyección de metadatos](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) para crear un nuevo pod. Por cada pod creado, debería haber una entrada de log, asumiendo que el [nivel de log (logLevel)](https://github.com/newrelic/k8s-metadata-injection/blob/main/charts/nri-metadata-injection/README.md#values) esté en INFO o inferior:
    ...
    7. Asegúrese de que los trabajos de configuración de inyección de metadatos se hayan ejecutado correctamente inspeccionando la salida de este comando:
