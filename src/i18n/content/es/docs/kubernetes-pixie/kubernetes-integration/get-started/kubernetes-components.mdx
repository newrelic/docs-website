...
Los clientes deben habilitar el seguimiento de cualquier métrica objetivo en KSM si esas métricas no están habilitadas de forma predeterminada. Por ejemplo, KSM v2+ deshabilita las etiquetas y anotaciones métricas de forma predeterminada. Los clientes pueden habilitar las etiquetas de destino y anotaciones métricas para ser monitoreadas utilizando las opciones `metricLabelsAllowlist` o `metricAnnotationsAllowList` descritas [aquí](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) en su clúster de Kubernetes.
