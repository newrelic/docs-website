...
El monitoreo de Windows admite dos modos:

* **Modo privilegiado**, que utiliza contenedores HostProcess de Windows para recopilar métricas completas a nivel de nodo. El modo privilegiado está habilitado de forma predeterminada.
* **Modo sin privilegios**, que se ejecuta como un usuario de contenedor estándar con visibilidad reducida del host.

Para obtener detalles sobre cómo funcionan estos modos y orientación sobre cómo configurarlos de forma segura, consulte [Modo privilegiado frente a no privilegiado](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode). Para obtener instrucciones completas de configuración, consulte [Monitorear nodos de Windows](/docs/kubernetes-pixie/kubernetes-integration/installation/windows).
...
* Modo privilegiado de Windows en clústeres de Google GKE, ya que GKE no admite contenedores de Windows `HostProcess`. Aún puede monitorear nodos de Windows en GKE en [modo sin privilegios](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode/#windows-nodes).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Compatible con la versión 3.x de VKS, que se ejecuta en vSphere Supervisor 9.0 a 9.1 (vSphere 8.x y 9.x)
      </td>
    </tr>
  </tbody>
</table>
