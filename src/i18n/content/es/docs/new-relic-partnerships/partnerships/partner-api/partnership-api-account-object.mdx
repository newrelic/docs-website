...
Tenga en cuenta que esto es solo un ejemplo y que, para algunas cuentas, los [atributos`users` y `subscriptions` ](#account-attributes)son innecesarios y se ignoran.

<CollapserGroup>
  <Collapser id="account-request" title="Solicitud JSON de objeto de cuenta">
    ```
    {
      "account": {
        "name": "Sample Account",
...
<CollapserGroup>
  ...
  <Collapser id="example-show" title="Espectáculo">
    Pedido:
