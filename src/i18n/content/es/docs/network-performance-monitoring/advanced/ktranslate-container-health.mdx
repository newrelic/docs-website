...
2. En
   ...
   <DNT>**Find logs where**</DNT>

   , ingrese `collector.name:"ktranslate"` y haga clic en
...
<CollapserGroup>
  <Collapser
    ...
    title="¿Qué versión de KTranslate estoy ejecutando?"
  >
    Logs UI:
  ...
  <Collapser
    id="docker-arguments"
    title="¿Qué argumentos se pasaron a docker en tiempo de ejecución?"
  >
    Logs UI:
  ...
  <Collapser
    id="container-errors"
    title="¿Qué errores estoy experimentando?"
  >
  ...
  <Collapser
    id="match_attributes"
    title="¿Mi filtro match_attributes funciona en mi dispositivo?"
  >
    Logs UI:
...
1. Vaya a
   ...
   <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DNT>
...
<CollapserGroup>
  <Collapser
    ...
    title="¿Cuáles son las versiones actuales de mi aplicación KTranslate?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="top-level-health"
    title="¿Cuál es el estado de mi aplicación KTranslate?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="delivery-health"
    title="¿Cuál es la salud de mis entregas a New Relic?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="snmp-health"
    title="¿Cuál es el estado general de mi colección SNMP?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    ...
    title="¿Qué dispositivos fallan en la recopilación SNMP?"
  >
    ```sql
    SELECT
    max(snmp_fail)
  ...
  <Collapser
    id="netflow-health"
    title="¿Cuál es el estado de mi recopilación de datos de flujo?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="syslog-health"
    title="¿Cuál es el estado de mi colección syslog?"
  >
    ```sql
    FROM Metric
    SELECT
