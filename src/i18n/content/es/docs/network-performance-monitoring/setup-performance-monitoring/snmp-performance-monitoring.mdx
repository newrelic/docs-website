...
<CollapserGroup>
  ...
  >
    <Steps>
      ...
      <Step>
        Investigue los datos de rendimiento de su red en la New Relic <InlinePopover type="networkMonitoring"/>UI.
      </Step>
    </Steps>
  </Collapser>
  ...
  >
    ...
    <Steps>
      <Step>
        En un host Linux con Docker instalado, descargue la imagen <DoNotTranslate>**ktranslate**</DoNotTranslate> ejecutando una de las siguientes opciones:
      ...
      <Step>
        ...
        <Callout variant="tip">
          Recomendamos configurar `discovery.add_mibs: true` para automatizar la adición de todos los MIB descubiertos al atributo `global.mibs_enabled` . Además, se recomienda configurar `discovery.check_all_ips: true` para evitar problemas de detección en [dispositivos con posturas de seguridad estrictas](/docs/network-performance-monitoring/troubleshooting/snmp-discovery-no-devices). Si tiene intención de recopilar capturas SNMP, también recomendamos cambiar el valor predeterminado de `listener: 127.0.0.1:1620` a `listener: 0.0.0.0:1620`
        </Callout>
      </Step>
