...
<CollapserGroup>
  ...
  <Collapser id="debug" title="Diagnosticar problemas con el logging del agente">
    ...
    * `error`
    ...
    2. Reiniciar Agent Control:

    ```bash
      sudo systemctl restart newrelic-agent-control
    ```

    3. Si el log `file` está habilitado, verifique el archivo de log más reciente en el directorio de la configuración `path` (los archivos tienen fecha, por ejemplo, `2026-09-18.newrelic-agent-control.log`). O utilice su herramienta preferida de resolución de problemas de logs, como `journalctl -u newrelic-agent-control`.
  </Collapser>
...
Vea el archivo de log más reciente (los archivos de log se nombran con un prefijo de fecha, como `2026-02-09.newrelic-agent-control.log`):
...
Ejecute Agent Control desde la línea de comando para evitar problemas específicos del servicio. Lee su configuración desde la ruta de configuración local predeterminada (`local-data\agent-control\local_config.yaml`) automáticamente, por lo que no se necesita ningún indicador:
...
## Resolver problemas de Agent Control en el host con NRDiag

La herramienta de diagnóstico New Relic `NRDiag` es una utilidad que recopila recursos y logs relacionados con el control del agente en el host para la depuración. Esto se aplica tanto a los hosts de Linux como de Windows; la suite correspondiente detecta automáticamente el sistema operativo y ejecuta las comprobaciones pertinentes.

Siga estos pasos para recopilar todos los datos:

1. En su host, instale la herramienta `NRDiag` empleando la [guía de introducción](/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started).

2. Ejecute la suite de Agent Control en el host:
...
3. El resultado esperado debería parecer al siguiente reporte:
   ...
   <Callout variant="tip" title="consejo">
     En Windows, la tarea `AgentControl/Agent/Logs` recopila los archivos de log directamente de `C:\ProgramData\New Relic\newrelic-agent-control\log\agent-control\` en lugar de `journalctl`.
   </Callout>
