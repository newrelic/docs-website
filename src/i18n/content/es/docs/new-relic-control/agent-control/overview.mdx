...
* **Credenciales:** Agent Control puede obtener credenciales sensibles de proveedores de valores externos como Vault, e inyectarlas en la configuración del agente en tiempo de ejecución. Esto mejora la seguridad al mantener las credenciales fuera de la propia configuración.
...
* **Postura de seguridad:** estandarice cómo se obtienen las credenciales de los proveedores de valores para centralizar la gestión, automatizar las rotaciones y reducir la exposición.
...
## Definiciones de tipos de agentes

Cada agente administrado por Agent Control está respaldado por un **tipo de agente**, una especificación versionada que describe cómo Agent Control debe descargar, configurar y ejecutar ese agente. Admitir un nuevo agente siempre requiere una definición de tipo de agente; sin ella, Agent Control no sabe cómo administrarlo.

Los tipos de agente se identifican mediante un nombre calificado y una versión (por ejemplo, `newrelic/com.newrelic.infrastructure:0.1.0`). Cuando agrega un subagente a su configuración, hace referencia a un tipo de agente; Agent Control utiliza esa definición para obtener el artefacto correcto, aplicar los valores predeterminados de configuración y administrar el ciclo de vida del agente.

Para obtener la lista completa de los tipos de agentes disponibles, consulte [Tipos de agentes](/docs/new-relic-control/agent-control/agent-types#agent-types).
...
Para obtener información detallada sobre los permisos específicos que requiere cada agente, consulte [Permisos necesarios por tipo de agente](/docs/new-relic-control/agent-control/agent-types#required-permissions-per-agent-type).
