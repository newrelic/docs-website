...
**Funciones de consulta:** La pasarela no admite ciertas funciones en la cláusula WHERE para la métrica consulta. Las funciones incluyen:
...
**Ejemplo de consulta:** `FROM Metric SELECT * WHERE (%[type]='summary' AND %[count]=0) OR (%[type]='gauge' AND %[count]=1 AND %[latest]=0)`
...
7. Ejecute el comando de instalación que se muestra en pantalla:
   ...
   <Callout variant="tip">
     Reemplace `<VERSION_NUMBER>` con el número de versión actual de la puerta de enlace y `/path/to/values-newrelic-gateway.yaml` con el directorio donde se encuentra el archivo de configuración.
   </Callout>
