...
* `rules`: Una matriz de reglas de transformación.

  * `name`: Un identificador único para la regla.

  * `description`: Una explicación legible por humanos de qué hace la transformación.

  * `conditions`: una lista de expresiones booleanas de OTTL.

  * `statements`: una lista de acciones de OTTL (como `set` o `delete_key`) para realizar en los telemetry data.
