...
2. You deploy via the UI (upload YAML) to your gateway fleet.
3. Gateway applies the configuration to all gateway clusters in the fleet.
4. All clusters in the fleet process data identically using the same rules.

**Gestión de versiones:**

* Each configuration change creates a new version (per-fleet)
* View version history and roll back if needed.
* &quot;Needs deployment&quot; badge shows pending changes.
* Different fleets can maintain different configurations and versions.

## Próximos pasos

**Get started with gateway installation:**

* [Install with Flux](/docs/new-relic-control/pipeline-control/gateway/set-up-and-deploy-pipeline-control) - Full gateway lifecycle management via UI (install, upgrade, scale)
* [Install without Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux) - Configuration-only mode for reduced permissions
