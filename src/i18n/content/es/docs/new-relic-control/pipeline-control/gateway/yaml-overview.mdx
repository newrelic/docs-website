...
<Callout variant="tip">
  **Nota sobre el modo de instalación:** el comportamiento de los campos `version` y `autoscaling` depende del modo de instalación de su flota:

  * **Con Flux:** estos campos son completamente administrados a través de la UI. Los cambios se despliegan automáticamente en todos los clústeres de la flota.
  * **Sin Flux:** estos campos se pueden definir durante la instalación inicial para configurar el escalador automático horizontal de pods (HPA). Sin embargo, los cambios posteriores a la instalación a través de la UI se ignoran. Solo la sección `configuration.simplified/v2` (reglas de pipeline) continúa desplegándose automáticamente a través de `ConfigMap`. Debe actualizar manualmente el escalado mediante HPA y las versiones mediante actualizaciones de Helm. Consulta [Administrar el gateway sin Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway-after-installation) para obtener más detalles.
</Callout>

### Estructura de nivel superior

* `version`: Versión del formato de configuración (actualmente `"2.0.1"`)
* `autoscaling`: Configuración de escalado de réplicas de Gateway
* `configuration.simplified/v2`: Capa de abstracción simplificada para definir pipelines de telemetría
* `troubleshooting`: Configuración de depuración

### Sobrescrituras opcionales

* `nrExporterEndpoints`: anular los extremos de ingesta predeterminados de New Relic por canalización
...
| configuration.simplified/v2                | objeto   | Sí        | -           |
| troubleshooting.proxy                      | booleano | No        | false       |
| troubleshooting.requestTraceLogs           | booleano | No        | false       |
| nrExporterEndpoints.nrHost                 | cadena   | No        | -           |
| nrExporterEndpoints.metricsEndpoint        | cadena   | No        | -           |
| nrExporterEndpoints.tracesEndpoint         | cadena   | No        | -           |
| nrExporterEndpoints.logApiEndpoint         | cadena   | No        | -           |
| nrExporterEndpoints.eventApiEndpoint       | cadena   | No        | -           |
| nrExporterEndpoints.infraEventApiEndpoint  | cadena   | No        | -           |
| nrExporterEndpoints.otlpEndpoint           | cadena   | No        | -           |

Cada campo `nrExporterEndpoints` es independiente: configure solo los que necesite anular (por ejemplo, solo `nrHost`), y cualquier campo que omita volverá al extremo de ingesta de New Relic predeterminado de la cuenta.

`nrExporterEndpoints` los campos se dividen en dos grupos de sobrescritura independientes:

* `nrHost`, `metricsEndpoint`, `tracesEndpoint`, `logApiEndpoint`, `eventApiEndpoint` y `infraEventApiEndpoint` sobrescriben el destino para la telemetría exclusiva de New Relic (agente legacy).
* `otlpEndpoint` sobrescribe por separado el destino para la telemetría nativa de OTLP. Configurar los campos en un grupo no tiene ningún efecto en el otro.

`otlpEndpoint` debe ser un host y un esquema básicos (por ejemplo, `https://otlp.example.com`), sin ningún puerto incluido. Agregar un puerto hace que la configuración no supere la validación y no se guarde.
