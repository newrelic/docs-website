...
La IU del gateway de Pipeline Control proporciona una interfaz basada en formularios para crear y administrar reglas de gateway sin escribir YAML directamente. La UI organiza la lógica de procesamiento en tres categorías de reglas: tasa de muestreo, reglas de descarte y transformaciones. Cada categoría es configurable mediante cuadros de diálogo modales guiados.
...
Cree reglas de filtrado basadas en OTTL para descartar condicionalmente registros de telemetría completos o atributos específicos. Soporta:
