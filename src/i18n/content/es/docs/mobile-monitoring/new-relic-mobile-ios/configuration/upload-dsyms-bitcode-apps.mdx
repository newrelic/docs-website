...
En algunas circunstancias, la carga automática de archivos dSYM de New Relic puede fallar. Si la carga del dSYM falla, crea un archivo `upload_dsym_results`. Por ejemplo, si hay una falla de red y la carga del dSYM no se completa, `upload_dsym_results` contendrá un registro de lo que salió mal. Para obtener información adicional sobre cómo New Relic maneja las cargas de dSYM, consulte la [Comunidad técnica en línea de New Relic](https://support.newrelic.com/s/hubtopic/aAX8W0000008Yv9/relic-solution-ios-dsym-upload-deepdive).
...
<CollapserGroup>
  ...
  <Collapser id="command-line-manual-upload" title="A través de la línea de comando">
    ...
    2. Cargue el archivo zip dSYM usando el siguiente comando:
       ...
       * Para <DNT>**US accounts (default)**</DNT>:
