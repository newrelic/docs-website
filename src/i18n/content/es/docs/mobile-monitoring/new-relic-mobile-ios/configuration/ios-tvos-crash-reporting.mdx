...
Un <DoNotTranslate>**dSYM file**</DoNotTranslate> es un archivo de proyecto Xcode para <DoNotTranslate>**debug symbols**</DoNotTranslate>. Contiene los símbolos de depuración que permiten la traducción del informe de fallo inicial a información legible por humanos. Este proceso se conoce como simbolización.
...
* <DoNotTranslate>**If the app is running in a simulator:**</DoNotTranslate> Una aplicación que se ejecuta en el simulador no se puede utilizar con informes de fallos. Los fallos del simulador no se pueden simbolizar.

* <DoNotTranslate>**If the debugger is enabled:**</DoNotTranslate> Sólo puede haber un controlador de excepciones no detectadas registrado a la vez por aplicación. Si se ejecuta con el depurador adjunto, New Relic no capturará ni informará fallas.

* <DoNotTranslate>**If another crash reporter is enabled:**</DoNotTranslate> Si se registra otro controlador de excepciones no detectado después de que se inicia New Relic, se registra este mensaje de error:
