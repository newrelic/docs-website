...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Mobile > (select an app) > Summary**</DoNotTranslate>: vea datos de Flutter, realice un seguimiento de las solicitudes y errores HTTP y monitor el rendimiento de su aplicación a lo largo del tiempo.
</figcaption>

## (Recomendado) Instalación guiada [#guided-install]

Para instalar el agente Flutter, siga nuestra [instalación guiada](https://onenr.io/0gR70k5oZjo), ubicada directamente en la UI.
...
New Relic almacena tu registro de Flutter como evento personalizado. Puede consultar estos registros y crear paneles para ellos utilizando esta consulta NRQL:
