...
<Callout variant="important" title="Cambio importante en el informe de errores (1.9.0)">
  A partir del agente de React Native versión 1.9.0, Los errores de JavaScript se informan a través de un nuevo tipo de evento: `MobileJSError`. Con el nuevo Mobile Errors Protocol, los errores de JavaScript de React Native ya no se almacenan en el tipo de evento `MobileHandledException`.

  Si usa la versión 1.9.0 o superior, actualice las condiciones de alerta y los dashboards que consulten `MobileHandledException` en busca de errores de la capa de JavaScript para usar `MobileJSError` en su lugar. Por ejemplo, reemplace `SELECT count(*) FROM MobileHandledException WHERE platform = 'reactnative'` con `SELECT count(*) FROM MobileJSError`. También actualice sus scripts de compilación a la última versión para que [los mapas de origen se carguen correctamente](/docs/mobile-monitoring/new-relic-monitoring-react-native/react-native-agent-js-error-reporting) para el nuevo tipo de evento.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    Si compila con [EAS Build](https://docs.expo.dev/build/introduction/), consulte [Configurar el agente de React Native para EAS Build](/docs/mobile-monitoring/new-relic-monitoring-react-native/configure-eas-build-react-native) para configurar también la carga automática de source map, dSYM y el archivo de mapeo de Android.
  </Step>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Registrar errores personalizados](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-errors/#react)
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="v1.9.0" title="Agente React Native versión 1.9.0 y superior">
    Los errores de JavaScript y los rechazos de promesas se graban como eventos `MobileJSError`. Puede ver la traza de evento, los atributos y el rastreo del stack para cada error de JavaScript en New Relic.
  ...
  <Collapser id="v1.2.0" title="Agente React Native versión 1.2.0 a 1.8.x">
...
<CollapserGroup>
  <Collapser id="logs-v1.4.1" title="Agente React Native versión 1.4.1 y superior">
    A partir de la versión 1.4.1 del agente de React Native, Los logs de la consola de JavaScript se reportan como logs de eventos. Puede verlos en la [UI de logs](/docs/logs/ui-data/use-logs-ui) o consultarlos yendo a <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Query your data**</DNT> y ejecutando:
  ...
  <Collapser id="logs-v1.4.0" title="Agente React Native versión 1.4.0 o inferior">
