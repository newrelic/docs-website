...
<Steps>
  ...
  <Step>
    ...
    El token de la aplicación se utiliza para que New Relic autentique los datos de tu aplicación .NET MAUI.

    Para ver y copiar el token de su aplicación en la UI de New Relic:
  ...
  <Step>
    ...
    El plug-in móvil de .NET MAUI permite rastrear eventos de navegación dentro de [.NET MAUI Shell](https://learn.microsoft.com/en-us/dotnet/maui/fundamentals/shell/navigation). Hay dos formas de inicializar esto dependiendo del momento de inicio de su aplicación en `App.xaml.cs`.

    <DNT>**Option 1: In the App Constructor**</DNT> Puede llamar al método de seguimiento directamente en el constructor inmediatamente después de configurar el `MainPage`.
    ...
    <DNT>**Option 2: In the OnStart Method (Recommended for timing/NullReferenceException issues)**</DNT> Si el shell de su aplicación no está completamente inicializado durante la ejecución del constructor, llamar al rastreador de navegación puede provocar un `System.NullReferenceException`. Para evitar esto, mueve la llamada al método de ciclo de vida `OnStart()` de MAUI:
...
### Faltan datos de rastreo distribuido en la UI [#missing-distributed-tracing]
...
### La aplicación falla con TrackShellNavigatedEvents [#track-shell-crash]

Si experimenta un `System.NullReferenceException` al llamar a `CrossNewRelic.Current.TrackShellNavigatedEvents()` en el constructor `App()`, es probable que se deba a que el shell de la aplicación aún no ha arrancado por completo. Para resolver esto, mueva la llamada fuera del constructor y al método de ciclo de vida `OnStart()` de MAUI dentro de su archivo `App.xaml.cs` (consulte la sección [Eventos de seguimiento de pantalla](#screen-tracking-events) para ver ejemplos de código).