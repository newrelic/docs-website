...
* **Requisito del agente iOS:** La recopilación automática está **desactivada de forma predeterminada**. Para habilitarlo:

  1. Debe habilitar la marca de función `NRFeatureFlag_AutoCollectLogs` en el código de inicialización de su aplicación.
  2. El interruptor <DNT>**Mobile Logs**</DNT> está **On** en la interfaz de usuario de New Relic.
...
* **Recopilación automática:** El agente de iOS puede capturar automáticamente los logs enviados a la salida estándar (`stdout`) y al error estándar (`stderr`). Debe habilitar la feature flag `NRFeatureFlag_AutoCollectLogs` en el código de inicialización de su aplicación para la recopilación automática de logs.
