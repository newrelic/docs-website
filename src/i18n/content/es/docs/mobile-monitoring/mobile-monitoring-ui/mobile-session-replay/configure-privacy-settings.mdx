...
4. **Reglas de prioridad padre/hijo**

   Tenga en cuenta que si hay reglas en conflicto, el enmascaramiento de un padre tiene prioridad sobre las reglas del subárbol.

   *Ejemplo:* si un padre tiene el atributo `nr-block` pero un hijo tiene `nr-unmask`, todo el subárbol se reemplaza con el marcador de posición de rectángulo negro que bloquea el contenido; no se captura ningún contenido de los hijos en el subárbol.
