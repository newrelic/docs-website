...
| `processor_name`          | cadena   | `""`        | processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
| `num_traces`           | En t     | `50000`     | if rate\_limiter is enabled, it uses the num\_traces as max number of concurrent processing traces |
...
| `connection.pool_size`          | En t     | `10 * cores` | Base number of socket connections                                                                                                            |
| `connection.min_idle_conns`     | En t     | `0`          | Minimum number of idle connections which is useful when establishing new connection is slow. The idle connections are not closed by default. |
| `connection.max_idle_conns`     | En t     | `0`          | Maximum number of connections allocated by the pool at a given time. 0 no limit                                                              |
| `connection.conn_max_idle_time` | duración | `30m`        | Maximum amount of time a connection may be idle. Should be less than server&apos;s timeout.                                                  |
| `connection.conn_max_lifetime`  | duración | `0m`         | Maximum amount of time a connection may be reused.                                                                                           |
...
| **Producción estándar (15k spans/seg)**  | 4-12        | 4-12                      | Balanced                            |
