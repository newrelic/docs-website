...
| --------------------- | ------ | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connection.address`  | cadena | *requerido* | Redis connection string (format: `redis://host:port/db`). For cluster mode, use comma-separated addresses (e.g., `redis://node1:6379,redis://node2:6379`) |
| `connection.password` | cadena | `""`        | Redis password for authentication                                                                                                                         |
...
| `connection.pool_size`          | En t     | `20`        | Maximum number of socket connections per Redis node                        |
| `connection.min_idle_conns`     | En t     | `5`         | Minimum number of idle connections to maintain for quick reuse             |
| `connection.max_idle_conns`     | En t     | `10`        | Maximum number of idle connections to keep open                            |
...
| `connection.max_retries`        | En t     | `3`         | Maximum number of command retries before giving up                         |
...
| `connection.max_redirects`      | En t     | `5`         | Maximum number of redirects to follow in cluster mode                      |
...
| Parámetro          | Tipo   | Por defecto | Descripción                                                                    |
| ------------------ | ------ | ----------- | ------------------------------------------------------------------------------ |
| `data_compression` | cadena | `none`      | Compression algorithm for trace data. Options: `none`, `snappy`, `zstd`, `lz4` |
...
| `processor_name`          | cadena   | `""`        | Optional processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
| Parámetro           | Tipo     | Por defecto                       | Descripción                                                       |
| ------------------- | -------- | --------------------------------- | ----------------------------------------------------------------- |
| `max_retries`       | En t     | `2`                               | Maximum retry attempts for failed trace evaluations               |
...
| `max_traces_per_batch` | En t     | `1000`      | Maximum number of traces to evaluate per batch               |
...
| Parámetro                     | Tipo | Por defecto | Descripción                                                                                         |
| ----------------------------- | ---- | ----------- | --------------------------------------------------------------------------------------------------- |
| `partitions`                  | En t | `6`         | Number of partitions for load distribution across Redis                                             |
| `partition_workers`           | En t | `6`         | Number of concurrent evaluation workers                                                             |
| `partition_buffer_max_traces` | En t | `10000`     | Maximum traces buffered per partition before flushing (2 workers per partition process in parallel) |
...
| Parámetro                    | Tipo     | Por defecto  | Descripción                                                                                                 |
| ---------------------------- | -------- | ------------ | ----------------------------------------------------------------------------------------------------------- |
| `ingestion_workers`          | En t     | `6`          | Number of goroutines processing traces from the shared ingestion channel                                    |
| `ingestion_buffer_size`      | En t     | `10000`      | Capacity of the shared ingestion channel for buffering incoming traces                                      |
...
| `hashing_strategy`           | cadena   | `rendezvous` | Hashing algorithm for partition selection. Options: `rendezvous` (recommended, 3x faster) or `consistent`   |
...
| Parámetro    | Tipo   | Por defecto | Descripción                          |
| ------------ | ------ | ----------- | ------------------------------------ |
| `num_traces` | En t   | `50000`     | Maximum concurrent processing traces |
| `policies`   | matriz | *requerido* | Sampling policy definitions          |
...
| `evaluation_interval`  | duration | `1s`        | How often to check for traces ready for evaluation            |
| `max_traces_per_batch` | En t     | `1000`      | Maximum number of traces to evaluate per batch                |
| `partition_workers`    | En t     | `6`         | Number of concurrent evaluation workers processing partitions |
...
| Strategy                   | Load Distribution       | rendimiento                                | Best For                                                                                             |
| -------------------------- | ----------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
...
| Guión                                     | Partitions | Partition Workers | Reasoning                                |
| ----------------------------------------- | ---------- | ----------------- | ---------------------------------------- |
| **Desarrollo**                            | 2-4        | 2-4               | Minimal overhead, easy debugging         |
...
| Nombre de la métrica                                                 | Dimensiones                                    | Descripción                                           | Use Case                                           |
| -------------------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------- | -------------------------------------------------- |
