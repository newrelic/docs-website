...
* Si está utilizando Infinite Tracing, utilice el valor [YOUR\_TRACE\_OBSERVER\_URL](/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer#ui-endpoints) en lugar del extremo estándar.
...
En un minuto, la traza debería estar disponible en [nuestra UI de rastreo distribuido](https://one.newrelic.com/launcher/distributed-tracing-nerdlets.distributed-tracing). Para encontrarlo, ejecute una consulta para `trace.id`. En este ejemplo, fue `test-zipkin-trace-id-1`. Tenga en cuenta que busca por el [atributo transformado](#zipkin-transform) de `trace.id` (no `traceId`).
...
Tenga en cuenta que si estuviera usando Infinite Tracing, o tuviera una cuenta de New Relic en la región de la UE o JP, el [extremo](/docs/distributed-tracing/trace-api/trace-api-general-requirements-limits#requirements) sería diferente.
