...
A excepción de nuestra funcionalidad de [Infinite Tracing](#tail-based), la mayoría de nuestras herramientas de rastreo utilizan un enfoque de muestreo basado en la cabecera. Esto aplica filtros a spans individuales antes de que lleguen todos los spans de una traza, lo que significa que las decisiones sobre si aceptar spans se toman al principio (la &quot;cabecera&quot;) del proceso de filtrado. Esta estrategia de muestreo puede ser útil para capturar una muestra representativa de la actividad, evitando problemas de almacenamiento y rendimiento.
...
<CollapserGroup>
  <Collapser id="trace-origin-sampling" title="Agente de lenguaje: muestreo adaptativo">
    ### Muestreo modernizado y compatibilidad con OpenTelemetry

    Nuestros agentes de lenguaje <InlinePopover type="apm" />utilizan muestreo adaptativo para proporcionar visibilidad de alta fidelidad mientras mantienen el rendimiento del sistema. Este enfoque garantiza que se capture una muestra representativa de la actividad del sistema en diversas arquitecturas, incluidos los agentes nativos de New Relic y las integraciones de OpenTelemetry (OTel).

    ### Cómo funciona el muestreo adaptativo

    El primer servicio monitoreado en una traza distribuida—el **origen de la traza**—es responsable de la decisión de muestreo. Esta decisión se propaga a todos los servicios descendentes para garantizar una traza completa de extremo a extremo.

    Los agentes de New Relic utilizan un algoritmo adaptativo &quot;basado en objetivos&quot;. El agente analiza el rendimiento del minuto anterior para calcular la probabilidad de muestreo para el minuto actual. Por ejemplo, si su servicio procesa 100 transacciones por minuto y el objetivo de muestreo es 10, el agente se ajusta dinámicamente para seleccionar aproximadamente 1 de cada 10 transacciones.

    Para encontrar el objetivo de muestreo predeterminado y los límites de span para su agente, consulte la documentación específica del agente:
    ...
    * [Ruby](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent)
    ...
    <Callout variant="tip">
      **Solo rastreo distribuido estándar**: Si un agente genera más spans que su límite configurado en un minuto, se descartarán algunos spans, lo que resultará en [trazas fragmentadas](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces) en la IU. A las trazas se les asigna una prioridad aleatoria cuando se seleccionan para el muestreo; por lo tanto, si varios agentes necesitan descartar spans, pueden intentar mantener intactas las trazas de mayor prioridad descartando primero los spans de las trazas de menor prioridad.

      Esto no se aplica a Infinite Tracing, que envía todos los datos de trazas sin muestreo.
    </Callout>

    ### Alta fidelidad y entornos híbridos

    Como parte de nuestro compromiso con los estándares abiertos, las últimas versiones del agente de New Relic están diseñadas para la **compatibilidad híbrida**.

    **Soporte para la proporción de rastreo**: Los agentes modernos ahora pueden alinearse con las proporciones de muestreo nativas de OTel, lo que permite una lógica de muestreo consistente en entornos mixtos. Consulta [Soporte de la API de OpenTelemetry](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support) para obtener detalles de configuración.

    **Preservación de decisiones ascendentes**: En entornos de &quot;Modo mixto&quot; (donde están presentes tanto los SDK de OTel como los agentes de New Relic), nuestros agentes priorizan preservar las decisiones de traza iniciadas por los SDK de OTel ascendentes para evitar trazas fragmentadas.
  </Collapser>
