...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitor > Transactions > (select a transaction trace)**</DoNotTranslate>: Esto muestra la UI de la traza de la transacción antes de habilitar el rastreo distribuido, con un enlace a la transacción asociada. Con rastreo distribuido habilitado mostrará la URL del servicio.
    </figcaption>

    Si desea obtener más detalles sobre la actividad de la traza, debe ir a la página UI <DoNotTranslate>**Distributed tracing**</DoNotTranslate> y examinar esa traza.
  </Collapser>
