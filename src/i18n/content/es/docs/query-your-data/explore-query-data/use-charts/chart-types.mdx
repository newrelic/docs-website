...
<CollapserGroup>
  ...
  >
    ...
    Las vallas publicitarias también se pueden utilizar para comparar un valor único entre estados. Para una instancia, utilice la cláusula [`COMPARE WITH`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-compare) para comparar el mismo atributo en dos períodos de tiempo diferentes.
    ...
    <figcaption>
      El gráfico del cartel muestra un valor único en una fuente grande y fácil de leer. El color de fondo cambia según su umbral.
    </figcaption>
    ...
    * Si desea resaltar un gráfico cuando un valor cae por debajo de un valor determinado en lugar de por encima de él, establezca el umbral
    ...
    * Los campos de umbral solo estarán disponibles para el panel que presente un valor único. Para instancias, el uso de una cláusula `COMPARE WITH` deshabilitará la configuración del umbral en el panel de edición.
  ...
  >
    ...
    <Callout variant="tip">
      El tipo de gráfico con viñetas solo se puede utilizar para gráficos que presenten un único valor. Por ejemplo, una consulta que utilice [`COMPARE WITH`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-compare) no incluirá la viñeta como opción.
    </Callout>
  ...
  >
    ...
    * Y finalmente, compró el(los) artículo(s) en el carrito de compras.
      ...
      A diferencia de otras consultas, debes utilizar una declaración [`SELECT`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#state-select) [`funnel()`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#func-funnel) en tu consulta; El tipo de gráfico embudos no es seleccionable desde ningún otro tipo de consulta.
  ...
  >
    ...
    <figcaption>
      El gráfico de líneas traza una serie temporal para un atributo.
    </figcaption>
    ...
    Para gráficos de líneas, puede conectar gráficos con otras experiencias de New Relic. Esto le ayudará a profundizar en los datos y descubrir las causas fundamentales de los problemas que intenta resolver. Cuando hace clic en puntos de un gráfico que le interesan, verá opciones para ver información específica de la entidad.

    He aquí un ejemplo:

    1. Abra un gráfico en un dashboard que tenga información en la que desee profundizar.

       <img
         style={{ align: "left"}}
         title="Screenshot showing a chart with spikes"
         alt="Screenshot showing a chart with spikes"
         src="/images/viz_screenshot-crop_chart-with-spikes.webp"
       />

    2. Pase el cursor sobre la línea del gráfico para exponer un punto específico del gráfico.

       <img
         style={{ align: "left"}}
         title="Screenshot showing a cursor hovering over chart"
         alt="Screenshot showing a cursor hovering over chart"
         src="/images/viz_screenshot-crop_chart-hover.webp"
       />

    3. Haga clic en el punto del gráfico para revelar un menú contextual que muestra la entidad asociada con el punto de datos en el gráfico.

       <img
         style={{ align: "left"}}
         title="Screenshot showing a list of associated entites"
         alt="Screenshot showing a list of associated entites"
         src="/images/viz_screenshot-crop_list-of-associated-entities.webp"
       />

    4. Haga clic en la entidad que le interese y luego, en el menú desplegable, haga clic en uno de los enlaces de esa entidad (verá los enlaces <DNT>**Entity summary**</DNT> y <DNT>**Logs**</DNT> ).

       <img
         style={{ align: "left"}}
         title="Screenshot showing a list of related experiences"
         alt="Screenshot showing a list of related experiences"
         src="/images/viz_screenshot-crop_list-of-related-experiences.webp"
       />

       Al utilizar esta característica, tenga en cuenta lo siguiente:

    * La entidad solo aparece en el menú contextual si el widget de consulta define explícitamente la entidad consultada con `entityGuid`, `entity.guid`, `appName`, `entity.name` o `entityName` en la cláusula `WHERE` (código fuente).
    * La visualización no conoce su ubicación dentro de una capacidad, por lo que las conexiones pueden ser circulares hasta la próxima versión (por ejemplo, de un resumen de APM a un resumen de APM).
    * El conjunto inicial de acciones se limita al resumen y al registro de APM.
    * La característica solo se admite en gráficos de líneas y gráficos de áreas apiladas.
  </Collapser>
  ...
  >
    ...
    Puede utilizar una tabla como acceso directo a otro panel. Si una tabla tiene facetas por atributo, al seleccionar una entrada se accede a un dashboard existente [filtrado por ese atributo](/docs/query-your-data/explore-query-data/dashboards/filter-new-relic-one-dashboards-facets/).
    ...
    * Para gráficos con un rango de tiempo que depende del momento presente (por ejemplo, `since 30 minutes ago`), los datos en el CSV se generan a partir del momento presente, no desde el momento en que se generó inicialmente el gráfico.
      ...
      <figcaption>
        El tipo de gráfico de tabla, con su formato tabular, funciona bien para el panel de control, así como para compartir los datos en informes.
      </figcaption>
