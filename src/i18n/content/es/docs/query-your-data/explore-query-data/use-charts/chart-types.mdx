---
title: Tipos de gráficos
metaDescription: The chart types available when building queries in New Relic.
freshnessValidatedDate: never
translationType: machine
---

Al crear una consulta utilizando el [generador de consultas](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/), existe una variedad de tipos de visualizaciones para presentar sus datos en el formato óptimo.

Esta tabla contiene todos los tipos de gráficos. La disponibilidad de un tipo de gráfico para su consulta dependerá de:

* El tipo de consulta (algunos resultados de la consulta no coinciden con algunos tipos de gráficos)
* El modo de consulta (básico o avanzado)

## Gráfico de área [#widget-areachart]

Un gráfico de áreas muestra una serie temporal para un único atributo.

<img title="An area chart type example that shows the percentage of errors by host over time." alt="A screenshot of an area chart type example that shows the percentage of errors by host over time." src="/images/queries-nrql_screenshot-crop_area-type-chart.webp" />

<figcaption>
  El tipo de gráfico de áreas muestra una serie de tiempo para un único atributo.
</figcaption>

Para habilitar un gráfico de áreas, agregue un [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) a su consulta.

Puedes usar faceta para agregar hasta cinco atributos, separados por comas. Utilice la función [`TIMESERIES`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries) cuando utilice un `FACET` en una consulta NRQL.

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de áreas cuando desee
      </td>

      <td>
        Vea la medición de un único atributo durante un período de tiempo que usted especifique.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de áreas cuando desee
      </td>

      <td>
        Ver un gráfico que contiene más de un atributo. El [tipo de gráfico de líneas](#widget-linechart) es una buena opción; Puede ver varios atributos durante un rango de tiempo que usted especifique. El [tipo de gráfico de barras](#widget-barchart) le ofrece una vista comparativa entre los atributos; utiliza un total de cada atributo para el rango de tiempo que elijas.
      </td>
    </tr>
  </tbody>
</table>

## Gráfico de barras [#widget-barchart]

El gráfico de barras compara los valores de las categorías o series que devuelve su consulta. Es la visualización predeterminada para la mayoría de las consultas que no son de series temporales.

<img title="A bar chart comparing values across categories." alt="A screenshot of a bar chart comparing counts across several categories, with the y-axis scaled to the data range." src="/images/queries-nrql_screenshot-crop_bar-chart.webp" />

<figcaption>
  El gráfico de barras compara valores entre categorías o series.
</figcaption>

El gráfico de barras admite datos categóricos (que no son de series temporales) y de series temporales. Para una comparación categórica, agregue una cláusula [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) a la consulta, como:

```sql
SELECT count(*) AS 'Transaction Count', average(duration) AS 'Avg Duration (s)', percentile(duration, 95) AS '95th Percentile' FROM Transaction FACET appName SINCE 7 days ago LIMIT 10
```

Puedes usar `FACET` con hasta 5 atributos diferentes, separados por comas.

El gráfico de barras establece el rango del eje y automáticamente, y puede configurarlo para que se ajuste a sus datos.

Para comparar varias series con una sola serie más grande, use el [gráfico de barras simplificado](#widget-simplified-barchart) en su lugar. En comparación con el gráfico de barras simplificado, el gráfico de barras no proporciona:

* Desplazamiento de series infinitas
* Etiquetas horizontales sin colisiones
* La primera serie fijada al 100 % del ancho de la barra

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de barras cuando desee
      </td>

      <td>
        Compare los totales entre categorías o series con un eje y configurable. El gráfico de barras es la visualización predeterminada para la mayoría de las consultas que no son de series temporales.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de barras cuando lo desee
      </td>

      <td>
        Compare varias series con una única serie más grande. El [gráfico de barras simplificado](#widget-simplified-barchart) es una mejor opción para esa vista.
      </td>
    </tr>
  </tbody>
</table>

## Cartelera [#widget-billboard]

El tipo de gráfico de cartelera presenta los resultados como un valor único y muy visible. Es una opción cuando su consulta devuelve un solo número, como en esta consulta:

```sql
SELECT uniqueCount(session) FROM PageView SINCE 1 day ago
```

Un gráfico de cartelera es útil para consultas de estadísticas únicas como la función [Apdex](/docs/apm/new-relic-apm/apdex/apdex-measure-user-satisfaction) . Utilice vallas publicitarias para monitor los valores de un vistazo.

También puedes emplear vallas publicitarias para comparar un mismo valor en distintos estados. Por ejemplo, emplee la cláusula [`COMPARE WITH`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-compare) para comparar el mismo atributo en dos periodos de tiempo diferentes.

<Callout variant="important">
  Los gráficos de Billboard que usan la cláusula `COMPARE WITH` no mostrarán porcentajes ni se representarán correctamente con enlaces de gráficos públicos, imágenes exportadas y archivos PDF.
</Callout>

Para habilitar los datos de un gráfico de cartelera, busque un valor único. Puede ver varios valores agregando una faceta desde el menú desplegable.

<DNT>**Thresholds**</DNT>

Opcionalmente, puedes configurar Umbral **Good**, **Warning**, **Approaching critical**, **Critical** o **Neutral** en las carteleras.

<img title="A billboard chart." alt="A screenshot of a billboard chart." src="/images/queries-nrql_screenshot-crop_billboards-color.webp" />

<figcaption>
  El gráfico de cartelera muestra un único valor en una fuente extra grande, lo que puede ayudar a atraer la atención hacia un valor específico en un panel lleno de información. El color de fondo cambia según tu umbral.
</figcaption>

A continuación se ofrecen algunos consejos que le ayudarán con el umbral:

* Para porcentajes, utilice un formato decimalizado. Por ejemplo, si su umbral es 99,5%, divida entre 100 y establezca el valor en 0,995.
* Si desea resaltar un gráfico cuando un valor cae por debajo de un valor determinado en lugar de por encima de él, establezca el umbral <DNT>**Warning**</DNT> por encima del umbral <DNT>**Critical**</DNT> .

<DNT>**Thresholds per series**</DNT>

Empleando la opción **Thresholds per series**, puede establecer <DNT>**Good**, **Warning**, **Approaching critical**, **Critical**, or **Neutral**</DNT> umbral en los afiches publicitarios. Una vez configurado, si su servicio funciona correctamente, el recuadro completo del gráfico se mostrará en verde. Cuando se cruza un umbral, el gráfico se vuelve amarillo (advertencia) o rojo (crítico).

<img title="The billboard chart displays a single value in an extra-large font, which can help draw attention to a specific value in a busy dashboared. The background color changes based on your thresholds." alt="The billboard chart displays a single value in an extra-large font, which can help draw attention to a specific value in a busy dashboared. The background color changes based on your thresholds." src="/images/billboards-combine-1.webp" />

<DNT>**Billboard settings**</DNT>

Puedes configurar los siguientes ajustes para un gráfico de valla publicitaria:

* **Visual style**

<table>
  <thead>
    <tr>
      <th>
        Nombre del campo
      </th>

      <th>
        Descripción
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Display mode
      </td>

      <td>
        Seleccione el modo de visualización para el gráfico de la valla publicitaria. Puede elegir **Automático**, **Valor y etiqueta**, **Solo valor**, **Solo etiqueta** o **Ninguno**.
      </td>
    </tr>

    <tr>
      <td>
        Alignment
      </td>

      <td>
        Seleccione la alineación para el gráfico de la valla publicitaria. Puedes elegir **Stacked (Default)** o **Inline**.
      </td>
    </tr>
  </tbody>
</table>

* **Grid options**

<table>
  <thead>
    <tr>
      <th>
        Nombre del campo
      </th>

      <th>
        Descripción
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Value size
      </td>

      <td>
        Seleccione el tamaño del valor que se muestra en el gráfico de la cartelera.
      </td>
    </tr>

    <tr>
      <td>
        Label size
      </td>

      <td>
        Seleccione el tamaño de la etiqueta que se muestra en el gráfico de la cartelera.
      </td>
    </tr>

    <tr>
      <td>
        Columns amount
      </td>

      <td>
        Seleccione el número de columnas que se mostrarán en el gráfico de la cartelera.
      </td>
    </tr>
  </tbody>
</table>

* **Add link**

<table>
  <thead>
    <tr>
      <th>
        Nombre del campo
      </th>

      <th>
        Descripción
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Title
      </td>

      <td>
        Introduzca el título del enlace que se mostrará en la gráfica de la cartelera.
      </td>
    </tr>

    <tr>
      <td>
        Url
      </td>

      <td>
        Introduzca la URL del enlace que se mostrará en el gráfico de la cartelera.
      </td>
    </tr>
  </tbody>
</table>

<img title="A screenshot of the billboard settings page, showing the options to configure the visual style, grid options, and add a link." alt="A screenshot of the billboard settings page, showing the options to configure the visual style, grid options, and add a link." src="/images/billboard-settings.webp" />

<DNT>
  **Uses**
</DNT>

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de cartelera cuando lo desee
      </td>

      <td>
        Monitor los valores, incluido el umbral para los estados <DNT>**Critical**</DNT> y <DNT>**Warning**</DNT>, en una fuente grande y fácil de leer.
      </td>
    </tr>

    <tr>
      <td>
        Evite un tipo de gráfico de cartelera cuando
      </td>

      <td>
        Quiere ver datos durante un período de tiempo. El [tipo de gráfico de líneas](#widget-linechart) es una mejor opción para ver datos por tiempo.
      </td>
    </tr>
  </tbody>
</table>

## Gráfico de viñetas [#bullet-chart]

Utilice el tipo de gráfico de viñetas para representar el progreso hacia un objetivo o meta, llamado límite. Para utilizar este tipo de gráfico, ejecute una consulta que devuelva un solo número, como por ejemplo:

```sql
SELECT uniqueCount(session) FROM PageView SINCE 1 day ago
```

<img title="An example bullet chart that shows a count of unique sessions against a limit." alt="A screenshot of an example bullet chart that shows a count of unique sessions against a limit." src="/images/queries-nrql_screenshot-crop_bullet-type-chart.webp" />

<figcaption>
  El gráfico de viñetas muestra un valor comparado con un límite.
</figcaption>

Bullet tiene por defecto un límite que es el doble del valor de la consulta resultante. Para ajustar el límite, use el campo <DNT>**Limit**</DNT> para ingresar otro número. La bala se ajustará en consecuencia.

<Callout variant="tip">
  Solo se pueden emplear gráficos de viñetas para gráficos que presenten un único valor. Por ejemplo, una consulta que emplea [`COMPARE WITH`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-compare) no incluirá la viñeta como una opción.
</Callout>

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de viñetas cuando lo desee
      </td>

      <td>
        Ver un gráfico utiliza una fuente grande y fácil de leer que le ayuda a ver el valor rápidamente.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de viñetas cuando lo desee
      </td>

      <td>
        Ver datos que tienen más de un valor. Los tipos de gráficos de barras y líneas pueden ser mejores opciones en este caso.
      </td>
    </tr>
  </tbody>
</table>

## Embudos [#funnel]

Utilice [embudos](/docs/insights/nrql-new-relic-query-language/nrql-query-examples/funnels-evaluate-data-series-events) para evaluar una serie de tipos de datos relacionados que constituyen los pasos hacia un objetivo específico. Un embudos te permite preguntar: “De las personas que hicieron (A), ¿cuántas también hicieron (B), y de las personas que hicieron (A) y (B), cuántas también completaron (C)?”

<img title="An example funnel chart that shows visitors progressing from landing to browse to cart to purchase." alt="A screenshot of an example funnel chart that shows visitors progressing through the landing, browse, cart, and purchase steps on a shopping site." src="/images/queries-nrql_screenshot-crop_funnel-chart.webp" />

<figcaption>
  Ejemplo de gráfico de embudos para un sitio de compras.
</figcaption>

En este ejemplo, desea determinar cuántos de todos los visitantes que vieron la página de destino:

* Examinó los productos.

* Luego agregó uno o más productos a un carrito de compras.

* Y finalmente, compró el(los) artículo(s) en el carrito de compras.

  A diferencia de otras consultas, debe emplear una declaración [`SELECT`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#state-select) [`funnel()`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#func-funnel) en su consulta; el tipo de gráfico de embudos no se puede seleccionar desde ningún otro tipo de consulta.

  Además, el tipo de gráfico embudos solo está disponible para consultas con ocho pasos o menos. Una consulta embudos que incluye más de ocho pasos solo devolverá los resultados como JSON.

  ```sql
  SELECT funnel(session,
      WHERE pageUrl like '%/index.html' AS 'Landing',
      WHERE pageUrl LIKE '%/browse/phones' AS 'Browse',
      WHERE pageUrl LIKE '%/shoppingcart' AS 'Cart',
      WHERE pageUrl LIKE '%/checkout' AS 'Purchased')
  FROM PageView WHERE appName = 'WebPortal' AND duration < 1
  SINCE 1 week ago
  ```

  <table>
    <tbody>
      <tr>
        <td>
          Usa un embudos cuando quieras
        </td>

        <td>
          Desea evaluar visualmente las rutas que toman los visitantes después de llegar a una página que usted designa como página de destino.
        </td>
      </tr>

      <tr>
        <td>
          Evita un embudos cuando quieras
        </td>

        <td>
          Solo desea recuentos para el atributo en la cláusula [`WHERE`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#where-operator) . No podrá elegir embudos a menos que utilice `SELECT funnel` en su consulta.
        </td>
      </tr>
    </tbody>
  </table>

## Medidor [#widget-gauge]

El tipo de gráfico de medidor muestra un solo valor como un arco relleno dentro de un rango que usted define, lo que facilita ver de un vistazo dónde cae un valor en relación con su mínimo, máximo y cualquier umbral que establezca. Es una opción cuando su consulta devuelve un solo número, como en esta consulta:

```sql
SELECT average(cpuPercent) AS 'CPU Usage (%)' FROM SystemSample SINCE 1 hour ago
```

<img title="A dashboard of gauge charts." alt="A screenshot of a dashboard of gauge charts, each filling an arc to show where a single value falls within its range, with green, yellow, and red threshold colors." src="/images/queries-nrql_screenshot_gauge-chart.webp" />

<figcaption>
  Gráficos de medidor en un dashboard. Cada medidor llena un arco para mostrar dónde se encuentra un solo valor entre su mínimo y máximo, y los colores del umbral indican cuándo un valor ingresa a un rango de advertencia o crítico.
</figcaption>

Para usar un gráfico de medidor, ejecute una consulta que devuelva un solo valor. Puede mostrar múltiples medidores a la vez agregando una faceta. Una consulta facetada muestra una cuadrícula de medidores, uno para cada valor de faceta.

```sql
SELECT average(cpuPercent) FROM SystemSample FACET hostname SINCE 1 hour ago LIMIT 6
```

<DNT>**Thresholds**</DNT>

Opcionalmente, establezca umbrales para colorear el medidor según su valor. Los gráficos de medidor admiten los umbrales <DNT>**Good**</DNT>, <DNT>**Warning**</DNT>, <DNT>**Approaching critical**</DNT>, <DNT>**Critical**</DNT> y <DNT>**Neutral**</DNT>. Cada umbral entra en vigor a partir de un valor que especifique, y el medidor cambia de color a medida que el valor entra en cada rango. Use el umbral <DNT>**Neutral**</DNT> para marcar un rango que no sea ni saludable ni no saludable, para que el medidor no indique un estado bueno o malo para esos valores.

A continuación se ofrecen algunos consejos que le ayudarán con el umbral:

* Establezca cada umbral para que comience en un valor que elija. Un umbral se aplica hasta que comienza el siguiente, y el primer umbral comienza en el mínimo del medidor.
* Para resaltar un medidor cuando un valor es demasiado alto (más alto es peor), ordene sus umbrales de menor a mayor. Para resaltar cuando un valor es demasiado bajo (más alto es mejor, como la capacidad disponible), invierta el orden para que el umbral más severo comience en el mínimo.
* Utilice la configuración <DNT>**Show threshold markers**</DNT> para mostrar marcas de graduación en el arco en cada límite de umbral.

<DNT>**Gauge settings**</DNT>

Puede configurar los siguientes ajustes para un gráfico de medidor:

<table>
  <thead>
    <tr>
      <th>
        Nombre del campo
      </th>

      <th>
        Descripción
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Mínimo
      </td>

      <td>
        Establezca el valor más bajo del rango del medidor. El arco comienza a llenarse a partir de este valor.
      </td>
    </tr>

    <tr>
      <td>
        Máximo
      </td>

      <td>
        Establezca el valor más alto del rango del medidor. El arco está lleno en este valor.
      </td>
    </tr>

    <tr>
      <td>
        Visualización de etiquetas
      </td>

      <td>
        Seleccione qué etiquetas muestra el medidor. Puede elegir **Auto**, **Value and name**, **Value only**, **Name only** o **None**.
      </td>
    </tr>

    <tr>
      <td>
        Mostrar marcadores de umbral
      </td>

      <td>
        Muestre u oculte las marcas de verificación en el arco en cada límite de umbral.
      </td>
    </tr>
  </tbody>
</table>

<DNT>
  **Uses**
</DNT>

<table>
  <tbody>
    <tr>
      <td>
        Use un gráfico de medidor cuando desee
      </td>

      <td>
        Monitoree un solo valor frente a un rango fijo y umbrales, como el uso de CPU, memoria o disco como porcentaje de capacidad, en un elemento visual compacto y fácil de leer.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de medidor cuando desee
      </td>

      <td>
        Vea cómo cambia un valor con el tiempo, o muestre más de un solo valor por serie. El [tipo de gráfico de líneas](#widget-linechart) es una mejor opción para ver datos por tiempo.
      </td>
    </tr>
  </tbody>
</table>

## Mapa de calor [#widget-heatmap]

Un mapa de calor es una herramienta de visualización que emplea gradaciones de color para representar la frecuencia de los datos a lo largo de intervalos de tiempo. Este método resulta eficaz para interpretar rápidamente patrones de datos complejos.

<img title="An example heatmap chart showing a pageview breakdown by operating system." alt="A screenshot of an example heatmap chart showing a pageview breakdown by operating system." src="/images/queries-nrql_screenshot-crop_heatmap-chart.webp" />

<figcaption>
  El mapa de calor es una distribución facetada de valores donde la densidad del color representa los valores mostrados.
</figcaption>

Los mapas de calor representan visualmente el histograma dividiendo los datos en segmentos dentro de un rango definido, como de 0 a 10, empleando la densidad del color para reflejar la frecuencia.

<DNT>**Facet and TimeSeries in heat map**</DNT>

`Facet` permite la categorización de datos por atributos como regiones geográficas —como España, Brasil o Estados Unidos—, lo que permite realizar comparaciones entre diferentes segmentos. La integración de la característica `TIMESERIES` divide los datos del histograma en intervalos de tiempo iguales, representándolos efectivamente a lo largo de una línea de tiempo similar a una línea o área, y empleando la intensidad del color para resaltar los cambios de frecuencia durante periodos prolongados.

<CollapserGroup>
  <Collapser id="example-query-facet" title="Consulta usando FACET">
    ```bash
    SELECT histogram(duration, width:10, buckets:20) FROM PageView SINCE 1 month ago FACET countryCode
    ```

    <img title="A screenshot of an example heatmap chart showing a pageview breakdown by countryCode" alt="A screenshot of an example heatmap chart showing a pageview breakdown by countryCode." src="/images/nrql-query-facet-countryCode.webp" />
  </Collapser>

  <Collapser id="example-query-timeseries" title="Consulta empleando TIMESERIES">
    ```bash
    SELECT histogram(duration, width:10, buckets:20) FROM PageView SINCE 1 month ago TIMESERIES
    ```

    <img title="A screenshot of an example heatmap chart showing a pageview breakdown by Timeseries." alt="A screenshot of an example heatmap chart showing a pageview breakdown by Timeseries." src="/images/nrql-heatmap-TIMESERIES.webp" />
  </Collapser>

  Al integrar `Facet` con `TIMESERIES`, los mapas de calor ofrecen vistas tridimensionales y emplean información sobre herramientas para proporcionar un contexto detallado, como los códigos de país.

  <Collapser id="example-query-combining" title="Consulta que combina FACET y TIMESERIES">
    ```bash
    SELECT histogram(duration, width:10, buckets:20) FROM PageView SINCE 1 month ago TIMESERIES FACET countryCode
    ```

    <img title="A screenshot of an example heatmap chart showing a pageview breakdown by countryCode." alt="A screenshot of an example heatmap chart showing a pageview breakdown by countryCode." src="/images/heatmaps-countrycode.webp" />
  </Collapser>
</CollapserGroup>

Para habilitar un mapa de calor, use la función [`Histogram()`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#func-histogram) con un atributo numérico y use [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) para agregar hasta cinco atributos, separados por comas o use `TIMESERIES` junto con la función histograma.

Consulte [histograma: vea la distribución de datos](/docs/using-new-relic/user-interface-functions/view-your-data/histograms-view-data-distribution) para obtener información más detallada sobre los gráficos basados en histogramas.

<table style={{ fontSize: "12pt" }}>
  <tbody>
    <tr>
      <td>
        Utilice un mapa de calor cuando lo desee
      </td>

      <td>
        Visualice datos tabulares con múltiples atributos a lo largo del tiempo empleando gradaciones de color en lugar de valores numéricos.
      </td>
    </tr>

    <tr>
      <td>
        Evita un mapa de calor cuando quieras
      </td>

      <td>
        Obtener datos numéricos precisos para múltiples atributos a lo largo de un periodo. Considere usar líneas o gráficos de barras para representaciones numéricas exactas.
      </td>
    </tr>
  </tbody>
</table>

## Histograma [#widget-histogram]

<img title="An example histogram showing the distribution of transaction durations." alt="A screenshot of an example histogram showing the distribution of transaction durations." src="/images/crop-insights-histogram-chart.webp" />

<figcaption>
  Un histograma muestra la frecuencia en una serie de rangos de valores.
</figcaption>

Utilice histograma para mostrar la distribución de datos en períodos de tiempo. Consulte [histograma: vea la distribución de datos](/docs/using-new-relic/user-interface-functions/view-your-data/histograms-view-data-distribution) para obtener información más detallada sobre los gráficos basados en histogramas.

<table>
  <tbody>
    <tr>
      <td>
        Utilice un histograma cuando desee
      </td>

      <td>
        Gráfico de frecuencia de datos sobre una serie de valores.
      </td>
    </tr>

    <tr>
      <td>
        Evite un histograma cuando lo desee
      </td>

      <td>
        Vea los mismos datos que [el mapa](#widget-heatmap) [de calor](#widget-heatmap) o para ver los datos sin los intervalos de tiempo.
      </td>
    </tr>
  </tbody>
</table>

## JSON [#widget-json]

Utilice el JSON devuelto para revisar los datos sin procesar devueltos y para ayudar a desarrollar la integración con New Relic. Para implementar la integración, utilice [la consulta remota](/docs/insights/new-relic-insights/adding-querying-data/querying-your-data-remotely) para devolver JSON sin formato como servicio.

Para utilizar esto, seleccione el menú <DNT>**Share**</DNT> y haga clic en <DNT>**Copy JSON**</DNT>.

Ejemplo de consulta NRQL:

```sql
SELECT uniqueCount(name) FROM Transaction
WHERE http.statusCode = '200'
TIMESERIES
FACET appName
LIMIT 10
SINCE 24 hours ago
```

Ejemplo de respuesta JSON:

```json
{
  "metadata": {
    "contents": {
      "messages": [],
      "timeSeries": {
        "messages": [],
        "contents": [
          {
            "function": "uniquecount",
            "attribute": "name",
            "simple": true
          }
        ]
      },
      "bucketSizeMillis": 1800000
    },
    "eventTypes": [
      "Transaction"
    ],
    "eventType": "Transaction",
    "openEnded": true,
    "messages": [],
    "beginTimeMillis": 1556226069396,
    "endTimeMillis": 1556312469396,
    "beginTime": "2019-04-25T21:01:09Z",
    "endTime": "2019-04-26T21:01:09Z",
    "guid": "5238a2c6-328d-a5ff-9185-7f3f04c42d1e",
    "routerGuid": "1aea2ec5-0be8-3a60-7554-851e1b016eb0",
    "rawSince": "24 HOURS AGO",
    "rawUntil": "NOW",
    "rawCompareWith": "",
    "facet": "appName",
    "offset": 0,
    "limit": 10
  },
  "facets": [
    {
      "name": "RPM Combined Production",
      "beginTimeSeconds": 0,
      "endTimeSeconds": 0,
      "timeSeries": [
        {
          "results": [
            {
              "uniqueCount": 415
            }
          ],
          "beginTimeSeconds": 1556226069,
          "endTimeSeconds": 1556227869,
          "inspectedCount": 4589471
        },
        [
          {
            "uniqueCount": 1714
          }
        ],
        "beginTimeSeconds": 1556310669,
        "endTimeSeconds": 1556312469,
        "inspectedCount": 53341469
      }
    ],
    "total": {
      "results": [
        {
          "uniqueCount": 2651
        }
      ],
      "beginTimeSeconds": 0,
      "endTimeSeconds": 0,
      "inspectedCount": 5053841114
    }
  },
  "performanceStats": {
    "inspectedCount": 6088068932,
    "responseTime": 8077
  }
}
...
```

## Línea [#widget-linechart]

Un gráfico de líneas es útil para detectar tendencias a lo largo del tiempo.

<img title="An example line chart showing pageview count over time." alt="A screenshot of an example line chart showing pageview count over time." src="/images/nrql_screenshot-crop_line-type-chart.webp" />

<figcaption>
  El gráfico de líneas traza un serial temporal para un atributo.
</figcaption>

Cuando trabaje en modo de consulta básico, seleccione un único atributo que pueda trazar a lo largo del tiempo.

Cuando se usa NRQL, [`SELECT`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#state-select) uno o más atributos se pueden trazar en el tiempo y luego usar la función [`TIMESERIES`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries) .

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de líneas cuando desee
      </td>

      <td>
        Cree un gráfico que le permita ver fácilmente las tendencias durante un período de tiempo específico.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de líneas cuando lo desee
      </td>

      <td>
        Vea sus datos usando uno o más atributos y vea los valores asociados con cada atributo. Un gráfico de barras o de tablas es una opción.
      </td>
    </tr>
  </tbody>
</table>

## Gráfico circular [#widget-piechart]

Un gráfico circular es la representación gráfica de la tabla. Aparece como una opción para algunas funciones cuando se utiliza la cláusula [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facets) . Para utilizar este tipo de gráfico, ejecute una consulta que devuelva un valor para cada uno de un conjunto de atributos.

<img title="An example pie chart." alt="A screenshot of an example pie chart." src="/images/nrql_screenshot-crop_pie_chart.webp" />

<figcaption>
  El gráfico circular representa un conjunto de valores como parte de un todo.
</figcaption>

Para habilitar el gráfico circular, agregue un [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) en el menú desplegable.

Puede agregar solo un `FACET` en el modo de consulta básico. Si estás usando NRQL, puedes usar `FACET` para agregar hasta 5 atributos, separados por comas, y también incluir la función [`TIMESERIES`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries) .

<Callout variant="important">
  Las siguientes [funciones agregadoras](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#functions) requieren una cláusula `FACET` para ser visualizadas como gráficos circulares: `average`, `apdex`, `min`, `max`, `percentage`, y `percentile`. Por ejemplo, `SELECT percentile(duration) FROM Transaction FACET appName` es compatible con los gráficos circulares, pero `SELECT percentile(duration) FROM Transaction` no lo es.
</Callout>

Para `uniqueCount`, los porcentajes [pueden sumar más del 100%](#pie-percentage-problem).

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico circular cuando desee
      </td>

      <td>
        Cree un gráfico que presente gráficamente un conjunto de valores en relación con un todo. Un gráfico circular es eficaz para mostrar valores de un período de tiempo, por ejemplo.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico circular cuando lo desee
      </td>

      <td>
        Presente datos que no utilicen valores relacionados con un todo, como en los gráficos de líneas.
      </td>
    </tr>
  </tbody>
</table>

### Problema de porcentaje del gráfico circular [#pie-percentage-problem]

Cuando se utiliza un gráfico circular con la función de agregador [`uniqueCount`](/docs/insights/nrql-new-relic-query-language/nrql-resources/nrql-syntax-components-functions#func-uniqueCount) , los porcentajes pueden sumar más del 100%. Esto se debe a que el atributo que se cuenta de forma única puede estar presente en múltiples facetas. Por ejemplo, en la consulta `SELECT uniqueCount(user) FROM PageView FACET appName`, un único usuario puede utilizar varias aplicaciones. Cada uno de estos usuarios está incluido en el valor único para cada una de las facetas (aplicaciones) apropiadas, pero el número total de usuarios únicos no cambiará.

Para solucionar este problema, utilice un [gráfico de barras](#widget-barchart) o [una tabla](#widget-table) para proporcionar una visualización más precisa de los datos de `uniqueCount` .

## Gráfico de barras simplificado [#widget-simplified-barchart]

El gráfico de barras simplificado compara varias series con la serie más grande, que el gráfico siempre trata como el 100 % del ancho de la barra. Muestra las otras series en relación con esa serie más grande.

<img title="A simplified bar chart ranking series against the largest one." alt="A screenshot of a simplified bar chart where the largest series fills the full bar width and the other series appear relative to it." src="/images/queries-nrql_screenshot-crop_simplified-bar-chart.webp" />

<figcaption>
  El gráfico de barras simplificado clasifica un conjunto de series en relación con la más grande.
</figcaption>

Para habilitar un gráfico de barras simplificado, agregue una cláusula [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) a la consulta. Puede usar `FACET` con hasta 5 atributos diferentes, separados por comas. Debido a que el gráfico de barras es el predeterminado para estas consultas, ejecute su consulta y luego seleccione <DNT>**Simplified Bar**</DNT> en el desplegable <DNT>**Chart type**</DNT>.

El gráfico de barras simplificado proporciona desplazamiento de series infinitas y etiquetas horizontales sin colisiones, y fija la primera serie en el 100 % del ancho de la barra.

<table>
  <tbody>
    <tr>
      <td>
        Utilice un gráfico de barras simplificado cuando quiera
      </td>

      <td>
        Comparar varias series con la serie más grande, que el gráfico trata como el 100 % del ancho de la barra.
      </td>
    </tr>

    <tr>
      <td>
        Evite un gráfico de barras simplificado cuando desee
      </td>

      <td>
        Incluya mediciones a lo largo de un periodo de tiempo. Este tipo de gráfico no está disponible cuando se emplea una cláusula [TIMESERIES](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#sel-timeseries). El [tipo de gráfico de líneas](#widget-linechart) es una buena opción para esta visualización.
      </td>
    </tr>
  </tbody>
</table>

## Mesa [#widget-table]

Una tabla es la vista estándar para un conjunto de valores relacionados con un atributo específico (por ejemplo, el recuento de todas las páginas vistas por nombre de agente de usuario, detalles log , etc.). De forma predeterminada, todas las tablas se pueden ordenar por cualquier columna.

Si utiliza la cláusula [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet) , los resultados normalmente se devolverán como una tabla.

Puede utilizar una tabla como acceso directo a otro dashboard. Si una tabla tiene facetas por atributo, al seleccionar una entrada se accede a un dashboard existente [filtrado por ese atributo](/docs/query-your-data/explore-query-data/dashboards/filter-new-relic-one-dashboards-facets/).

También puede exportar un gráfico de tabla a un archivo CSV.

* Si se aplica un filtro a un dashboard, la descarga solo incluirá el subconjunto de datos basados en el filtro.
* Si se selecciona un rango de tiempo fijo específico, el CSV incluye los datos para ese período de tiempo.
* Si realiza una consulta con varios atributos, la exportación incluye columnas para todos los `FACET`de la consulta.
* Para gráficos con un rango de tiempo que depende del momento presente (por ejemplo, `since 30 minutes ago`), los datos en el CSV se generan a partir del momento presente, no desde el momento en que se generó inicialmente el gráfico.

<img title="An example table chart." alt="A screenshot of an example table chart." src="/images/nrql_screenshot-crop_table_chart.webp" />

<figcaption>
  El tipo de gráfico de tabla, con su formato tabular, funciona bien para el dashboard, así como para compartir los datos en informes.
</figcaption>

<table>
  <tbody>
    <tr>
      <td>
        Usa una mesa cuando quieras
      </td>

      <td>
        Cree una tabla compatible con texto que contenga sus datos. Esta tabla se puede compartir y utilizar en informes o importar a hojas de cálculo, por ejemplo.
      </td>
    </tr>

    <tr>
      <td>
        Evita una mesa cuando quieras.
      </td>

      <td>
        Vea sus datos en formato gráfico.
      </td>
    </tr>
  </tbody>
</table>