...
<Callout variant="tip">
  Si las variables anidadas saturan la barra de variables de su dashboard, puede ocultar las variables padre o de soporte que no requieren interacción del visualizador. Las variables ocultas continúan funcionando normalmente en segundo plano. Para obtener más información, consulta [Mostrar y ocultar variables](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility).
</Callout>