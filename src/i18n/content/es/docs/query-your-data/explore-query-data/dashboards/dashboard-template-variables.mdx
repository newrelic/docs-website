...
## Mostrar y ocultar variables [#variable-visibility]

Puedes ocultar las variables de plantilla de la barra de variables en el modo Vista mientras continúan funcionando normalmente. En el modo de edición, las variables ocultas se muestran con un icono <Icon name="fe-eye-off" />, para que los editores de dashboards puedan identificarlas y administrarlas.

Recomendamos ocultar las variables que:

* Saturar la barra de variables, como las [variables anidadas](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Controla la lógica del dashboard sin requerir la entrada del espectador
* Derivar valores automáticamente de otras variables

Para ocultar una variable, use la opción <DNT>**Hide on variables bar**</DNT> al [definir una variable de plantilla](#step1). Para configurar esto de forma programática, use el campo `hiddenOnVariablesBar` en la [API de dashboards de NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).

<Callout variant="tip">
  Para ocultar varias variables a la vez, utilice la [API de dashboards de NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards) para actualizar varias variables de forma programática, ya que la UI solo permite ocultar variables una por una.
</Callout>

## Requisitos y limitaciones [#requirements]

Las variables de plantilla solo se pueden utilizar en el contexto de la creación de widgets para dashboards personalizados. Por esta razón, algunas funciones relacionadas con consultas no funcionan. Por ejemplo, la opción `Export dashboard as PDF` no admite widgets con variables, y las consultas SQL [<DNT>Lens</DNT>](/docs/new-relic-lens/query-data) no admiten variables de plantilla. Consulta [variables NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#with-as-nrql-var) para usar variables en una consulta NRQL.
...
<Steps>
  <Step>
    ...
    Defina una variable de plantilla que usará en una consulta NRQL para crear un widget.

    Para definir una variable:

    1. Desde un nuevo dashboard sin variables, haz clic en el botón de edición <Icon name="fe-edit-2" /> y luego haz clic en el botón <DNT>**+ Add variable**</DNT>.
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            El botón **Include variable** determinará la configuración predeterminada, incluir o excluir, para esa variable en el dashboard. El usuario que visualiza el dashboard puede modificar esta configuración usando el interruptor **Include variable** en el menú desplegable de la variable. La configuración seleccionada por el usuario será válida durante la duración de la sesión. Para obtener detalles sobre dónde encontrar este interruptor y cómo afecta a las consultas, consulte [Incluir y excluir variables](#include-variable).
        ...
        <tr>
          ...
          <td>
            Opcional. Ocultar la variable de la barra de variables en el modo de visualización. Las variables ocultas siguen funcionando normalmente. En el modo de edición, las variables ocultas se muestran con un ícono <Icon name="fe-eye-off" />. Para obtener más información, consulte [Mostrar y ocultar variables](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>

    Después de definir su variable de plantilla, puede agregar un widget que utilice la variable de plantilla creada.
  </Step>
  ...
  <Step>
    ...
    5. (Opcional) Para personalizar el gráfico, seleccione el ícono <DNT>**See customization options**</DNT> junto al menú desplegable <DNT>**Chart type**</DNT>. Para ver las opciones disponibles, consulte [Personalizar sus gráficos](/docs/query-your-data/explore-query-data/use-charts/use-your-charts#customize-charts).

    6. Hacer clic <DNT>**Add to dashboard**</DNT>
...
El interruptor <DNT>**Include variable**</DNT>, activado de forma predeterminada, controla si el dashboard filtra por el valor seleccionado de la variable. Debido a que esta configuración es específica para esa variable en lugar de todas las variables en el dashboard, se encuentra dentro del propio desplegable de valores de la variable en lugar de en la barra de variables compartidas. Puede:

* Excluya la variable de la consulta desactivando el interruptor. Esto reemplaza la condición de la variable con un valor booleano neutral (`true`).
* Restaure tanto el interruptor como el valor a los valores predeterminados configurados de la variable usando <DNT>**Reset to default value**</DNT>.

Esta selección es válida solo para la sesión actual. Para establecer el estado predeterminado del interruptor al definir una variable, consulte [Valor predeterminado](#step1).
...
* La variable tiene más valores que el máximo permitido (por ejemplo, 5000 resultados máximos para únicos de forma predeterminada) o un número muy alto de valores. Deshabilitar la variable de forma predeterminada reduce la carga de la consulta.
...
<CollapserGroup>
  ...
  <Collapser id="titles-labels-on-your-list-variables" title="Emplee títulos o etiquetas en las variables de su lista">
    ...
    Por ejemplo, supongamos que tiene una variable como esta:
    ...
    Los títulos que has agregado se mostrarán en el menú desplegable de la variable. Esto no afectará la lógica de su variable o widgets en absoluto.
  ...
  <Collapser id="variables-widget-titles" title="Usar variables en los títulos de los widgets">
    Puede usar variables de plantilla en los títulos de los widgets para crear títulos dinámicos que se actualicen automáticamente al cambiar los valores de las variables. Esto es útil para proporcionar contexto sobre qué datos se están mostrando.

    Puedes agregar variables a los títulos de los widgets a través de la interfaz de usuario al editar un widget o mediante el [JSON del dashboard](/docs/query-your-data/explore-query-data/dashboards/manage-your-dashboard/#manage-json).

    Por ejemplo, si tiene una variable llamada `{{products}}` con valores como &quot;Alertas&quot;, &quot;APM&quot;, &quot;dashboards&quot; y &quot;Catálogo&quot;, puede crear un widget con un título dinámico como `Page views per nerdlet: {{products}}`:
    ...
    Cuando selecciona &quot;Alertas&quot; del menú desplegable de variables, el título del widget se actualiza automáticamente para mostrar el valor seleccionado. Si realizas una selección múltiple de &quot;Alertas&quot;, &quot;APM&quot; y &quot;dashboards&quot;, el título se actualiza para mostrar todos los valores seleccionados.

    Esta función funciona con cualquier tipo de variable de plantilla (Consulta, Lista o Campo de texto) y ayuda a los usuarios a entender qué datos están viendo sin tener que revisar los filtros de las variables.
  </Collapser>

  <Collapser id="variables-markdown-widgets" title="Usar variables en widgets de Markdown">
    Puede usar variables de plantilla en widgets de markdown para crear contenido de texto dinámico, documentación o diagramas que se actualicen según la selección de variables. Esto es particularmente poderoso para crear notas contextuales o visualizaciones como diagramas Mermaid.

    Por ejemplo, si tiene una variable `{{products}}` con valores como &quot;Alertas&quot;, &quot;APM&quot;, &quot;dashboards&quot; y &quot;Catálogo&quot;, puede usarla en contenido markdown de diferentes maneras:

    **Ejemplo 1: Contenido de texto dinámico**

    Puede incluir la variable (`{{products}}`) directamente en su texto markdown para crear documentación dinámica que se actualiza según la selección:
    ...
    **Ejemplo 2: Diagramas dinámicos de Mermaid**

    También puede usar la variable (`{{products}}`) dentro del texto markdown de los diagramas Mermaid para crear diagramas de flujo que se actualicen dinámicamente:
    ...
    En este ejemplo, el diagrama muestra un flujo de decisión donde la variable `{{products}}` determina qué datos conservar o descartar. Cuando cambia la selección de variables, el diagrama se actualiza para reflejar los productos seleccionados.

    <Callout variant="Preview" title="nota">
      Al editar un widget de markdown, el modo de vista previa muestra el nombre de la variable (por ejemplo, `{{products}}`), no el valor real. Sin embargo, cuando guarda y ve el dashboard, las variables se reemplazan por sus valores reales.
    </Callout>
  </Collapser>
</CollapserGroup>