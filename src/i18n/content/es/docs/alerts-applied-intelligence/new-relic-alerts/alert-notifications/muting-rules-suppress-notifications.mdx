...
Puede crear, habilitar, deshabilitar y administrar reglas de silenciamiento. Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate>. Habilite o deshabilite las reglas de silencio en cualquier momento. Haga clic en el <Icon name="fe-more-horizontal"/> icono en la fila de cada regla para editar y eliminar reglas.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate>: Puede crear reglas de silenciamiento complejas para apuntar a un conjunto pequeño o grande de notificaciones no deseadas.
</figcaption>
...
Para crear una regla de silenciamiento, vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate> y haga clic en <DoNotTranslate>**+ Add a rule**</DoNotTranslate>. Ingrese un nombre y una descripción (opcional) para la regla de silenciamiento y seleccione la cuenta a la que se aplicará la regla.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Muting rules**</DoNotTranslate>: opciones flexibles y potentes para programar sus reglas de silenciamiento.
</figcaption>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * `timeZone`: la zona horaria que se aplica al programa de reglas de silenciamiento. Ejemplo: 'América/Los_Ángeles'. Consulte [la lista de Wikipedia de zonas horarias de la base de datos tz](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones).
        * repetir: la frecuencia con la que se repite el programa de la regla de silenciamiento. Si no se repite, utilice nulo. Las opciones son DAILY, WEEKLY, MONTHLY.
        ...
        * repeatCount: el número de veces que se repite el programa de la regla de silenciamiento. Esto incluye el cronograma original. Por ejemplo, un `repeatCount` de 2 se repetirá una vez. Un `repeatCount` de 3 se repetirá dos veces. Nota: Se pueden utilizar `repeatCount` o `endRepeat` para finalizar un programa de reglas de silenciamiento. No proporcione ambos campos juntos.
        * weeklyRepeatDays: los días de la semana en los que una regla de silenciamiento debe repetirse cuando el campo de repetición está configurado en "WEEKLY". Ejemplo: \['MONDAY', 'WEDNESDAY'].
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Issues**</DoNotTranslate> y haga clic en un problema silenciado.
    </figcaption>
  </Collapser>
...
Para silenciar los resultados de consultas por facetas, utilice el atributo `tags.FACETED_ATTRIBUTE` , donde FACETED_ATTRIBUTE representa el atributo con el que realizó una [consulta NRQL`FACET` ](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#sel-facet). Por ejemplo: si su condición de alerta NRQL incluye `FACET host` en su consulta, puede apuntar a ese atributo `FACET` usando `tags.host`.
