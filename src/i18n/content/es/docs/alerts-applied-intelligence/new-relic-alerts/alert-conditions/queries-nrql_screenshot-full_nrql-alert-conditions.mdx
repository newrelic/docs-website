...
Puede utilizar [la consulta NRQL](/docs/insights/new-relic-insights/using-new-relic-query-language/using-nrql) para crear una condición de alerta. Una vez que haya definido su señal, puede definir aún más sus niveles de umbral crítico y de advertencia. Esto determina cuándo se crea un incidente de alerta.

Continúe leyendo para obtener más información sobre cómo hacer esto.
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (select a policy) > Add a condition**</DoNotTranslate>. Haga clic en <DoNotTranslate>**NRQL**</DoNotTranslate> y luego <DoNotTranslate>**Next, define thresholds**</DoNotTranslate>.
</figcaption>

<Callout variant="tip">
  Para obtener más información sobre conceptos clave relacionados con NRQL condición de alerta y alerta de transmisión, consulte [Alerta de transmisión: términos y conceptos clave](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts).
</Callout>

¿Listo para comenzar? Si aún no lo ha hecho, asegúrese de [registrarse para obtener una cuenta New Relic](https://newrelic.com/signup). Es gratis, para siempre.

## Crear una condición de alerta NRQL [#create]

Para crear una condición de alerta NRQL para una política:
...
2. Seleccione una política existente o haga clic en

   <DoNotTranslate>**New alert policy**</DoNotTranslate>

   para [crear una nueva política](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/create-edit-or-find-alert-policy).
...
Tenga en cuenta que editar una condición existente puede resultar en [restablecer su evaluación](#evaluation-resets).

## Crear una condición a partir de un gráfico [#create-chart]

Puede utilizar un gráfico para crear una condición de alerta NRQL.
...
<figcaption>
  Para crear una condición de alerta NRQL a partir de un gráfico, haga clic en el menú del gráfico <Icon name="fe-more-horizontal"/>y luego haga clic en <DoNotTranslate>**Create alert condition**</DoNotTranslate>.
</figcaption>

Una vez que haya nombrado y personalizado su condición, puede agregarla a una póliza existente o crear una nueva.

<Callout variant="caution">
  Una pequeña cantidad de nuestros gráficos más antiguos no incluyen la opción de crear una condición de alerta.
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `uniqueCount`
          ...
          <Callout variant="tip">
            Si utiliza el agregador `percentile` en una condición de alerta facetada con muchas facetas, puede provocar que aparezca el siguiente error:
    ...
    <tr>
      ...
      <td>
        Sólo un tipo de datos puede ser objetivo.
    ...
    <tr>
      ...
      <td>
        Utilice la cláusula `WHERE` para especificar una serie de una o más condiciones. Todos los [operadores](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#where-operators) son compatibles.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        Utilice la cláusula [`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-resources/nrql-syntax-components-functions) para separar sus resultados por atributo y alertar sobre cada atributo de forma independiente. No se permite ninguna cláusula `LIMIT` , pero toda consulta recibirá el máximo número de facetas posible.

        La consulta facetada puede devolver un máximo de 5000 valores para condiciones [estáticas y anormales](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries#threshold-types) .
...
Algunos elementos de NRQL utilizados en los gráficos no tienen sentido en el contexto de transmisión de alerta. A continuación se incluye una lista de los elementos incompatibles más comunes y sugerencias para reformatear una consulta de alerta NRQL para lograr el mismo efecto.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        Para condiciones NRQL, la propiedad equivalente de una señal es la ventana de duración de agregación.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        `histogram()` no es compatible con las alertas NRQL: las agregaciones de histogramas no se pueden formatear como series de tiempo. Para crear una alerta a partir de una parte de un histograma (por ejemplo, percentil 95), utilice la función de agregación [`percentile()`](/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#functions) .
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        Puede habilitar ventanas deslizantes en la UI. Al crear o editar una condición, vaya a <DoNotTranslate>**Fine-tune advanced signal settings > Data aggregation settings > Use sliding window aggregation**</DoNotTranslate>.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        En la consulta NRQL, la cláusula LIMIT se utiliza para controlar la cantidad de datos que devuelve una consulta, ya sea el número máximo de valores de faceta devueltos por la consulta FACET o el número máximo de elementos devueltos por la consulta SELECT \*.
    ...
    <tr>
      ...
      <td>
        [Las subconsultas](/docs/query-your-data/nrql-new-relic-query-language/get-started/subqueries-in-nrql) no son compatibles con la alerta de transmisión porque la ejecución de las subconsultas requiere múltiples pases a través de los datos.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  >
    Cree <InlinePopover type="alerts"/>restringido que se dirija a un segmento específico de sus datos, como algunos clientes clave o un rango de datos. Utilice la cláusula `WHERE` para definir esas condiciones.
...
1. `FROM` cláusula: ¿qué tipo de evento se debe capturar?
...
Para determinar cómo se manejarán los valores nulos, ajuste la configuración de pérdida de señal y llenado de espacios en la [UIde condición de alerta](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions/#signal-loss).

Puede evitar `NULL` valores por completo con un acceso directo de consulta de orden de operaciones. Para hacer esto, use una subcláusula `filter` y luego incluya todos los elementos de filtro dentro de esa subcláusula. El cuerpo principal de la consulta se ejecutará y devolverá datos, momento en el cual se ejecutará la cláusula `SELECT` y se aplicarán los elementos de filtro. La consulta devolverá un valor de `0` si los elementos del filtro no dan como resultado datos coincidentes. He aquí un ejemplo:
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        Para obtener más información, consulte [la descripción.](/docs/alerts/new-relic-alerts/defining-conditions/alert-condition-descriptions)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Las condiciones NRQL evalúan los datos según cómo se agregan, utilizando ventanas de agregación de 30 segundos a 6 horas, en incrementos de 15 segundos. Para obtener mejores resultados, recomendamos utilizar los métodos de agregación de flujo de eventos o temporizador de eventos.

        Para el método de agregación de cadencia, la cláusula `SINCE ... UNTIL` implícita que especifica qué minuto evaluar está controlada por su configuración [<DoNotTranslate>**delay/timer**</DoNotTranslate>](#time) . Dado que los datos muy recientes pueden estar incompletos, es posible que desees consultar los datos de hace 3 minutos o más, especialmente para:
    ...
    <tr>
      ...
      <td>
        Utilice el <DoNotTranslate>**Condition settings**</DoNotTranslate> para:
        ...
        * Proporcione una [descripción de incidente personalizada para la condición](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-incident-descriptions) que se incluirá en los incidentes y la notificación.
        * Agregue la URL del runbook para incluir los procedimientos de su organización para manejar incidentes. También puede agregar esta información a la [descripción personalizada del incidente](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-incident-descriptions).
      </td>
    </tr>
...
Cuando edita una condición NRQL existente, tiene la opción de agregar o eliminar la etiqueta asociada con la entidad de condición. Para hacer esto, haga clic en el botón <DoNotTranslate>**Manage tags**</DoNotTranslate> debajo del nombre de la condición. En el menú que aparece, agregue o elimine una etiqueta.
...
* Para [condiciones de anomalía](#threshold-types): la condición comienza de nuevo y se pierde todo el aprendizaje de anomalías.
...
* Cambiar la configuración "Agregar umbral de señal perdida"
...
* Cambiar la función de hora (cambiar de
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        [anomalía](/docs/alerts/new-relic-alerts/defining-conditions/create-anomaly-alert-conditions) (anomalía dinámica)
      </td>
...
<figcaption>
  Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (select a policy)**</DoNotTranslate> y luego <DoNotTranslate>**Add a condition**</DoNotTranslate>. La pérdida de señal solo está disponible para condiciones NRQL.
</figcaption>
...
* <DoNotTranslate>
  ...
  * Nodo GraphQL: [expiration.expirationDuration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)
...
* <DoNotTranslate>
  ...
  * Cerrar todos los incidentes abiertos actuales: Cierra todos los incidentes abiertos que estén relacionados con una señal específica. No necesariamente cerrará todos los incidentes por una condición. Si está alertando sobre un servicio efímero o sobre una señal esporádica, querrá elegir esta acción para asegurarse de que el incidente se cierre correctamente. El nombre del nodo GraphQL para esto es ["closeViolationsOnExpiration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)"
  * Abrir nuevo incidente: Esto abrirá un nuevo incidente cuando la señal se dé por perdida. Estas incidencias nos indicarán que se deben a una pérdida de señal. Según sus preferencias de incidentes, esto debería generar una notificación. El nombre del nodo GraphQL para esto [es "openViolationOnExpiration](/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)"
...
Puede configurar la [duración de la ventana de agregación](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts#aggregation-window) para elegir cuánto tiempo se acumulan los datos en una ventana de tiempo de transmisión antes de agregarlos. Puede configurarlo entre 30 segundos y 120 minutos. El valor predeterminado es un minuto.
...
Puede ajustar el [retraso/temporizador](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts#delay-timer) para coordinar [nuestro algoritmo de alerta de transmisión](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts) con el comportamiento de sus datos. Si sus datos son escasos o inconsistentes, es posible que desee utilizar el método de agregación del temporizador de eventos.
...
<Callout variant="important">
  Al crear condiciones NRQL para datos recopilados de [Infraestructura integrada en la nube](/docs/infrastructure/infrastructure-integrations/get-started/introduction-infrastructure-integrations) como AWS CloudWatch o Azure, le recomendamos que utilice el método de temporizador de eventos.
</Callout>
...
* <DoNotTranslate>**Last known value**</DoNotTranslate>
  ...
  : Esta opción inserta el último valor visto antes de que se produzca la evaluación. Mantenemos el estado del último valor visto durante 2 horas.
