---
title: Ver el estado de salud de la entidad y encontrar entidades sin condición de alerta
tags:
  - Alerts and applied intelligence
  - Alerts
  - Alert conditions
metaDescription: The color-coded health status indicates whether app or other entity has any alerts policies assigned to it.
freshnessValidatedDate: never
translationType: machine
---

import accountsAlertsGreen from 'images/accounts_icon_alerts-green.webp'

import accountsAlertsYellow from 'images/accounts_icon_alerts-yellow.webp'

import accountsAlertsRed from 'images/accounts_icon_alerts-red.webp'

import accountsAlertsGray from 'images/accounts_logo_alerts-gray.webp'

import accountsEventAlerts from 'images/accounts_icon_event-alerts-.webp'

import accountsCropAlertEventsGreen from 'images/accounts_icon_crop-alert-events-green.webp'

Con <InlinePopover type="alerts"/>puedes saber fácilmente si una entidad (el objetivo de la notificación) tiene una o más condiciones asociadas:

* Si es

  <DoNotTranslate>**yes**</DoNotTranslate>

  , su indicador de estado de salud en el índice seleccionado (APM, browser, etc.) estará codificado por colores según el estado actual. Para ver un resumen del incidente de alerta actual, pase el mouse sobre su indicador de estado de salud.

* Si es

  <DoNotTranslate>**no**</DoNotTranslate>

  , su indicador de estado de salud en el índice seleccionado aparecerá en gris.

<Callout variant="important">
  Para obtener más información sobre cómo funcionan juntas las condiciones y las políticas, consulte [conceptos y términos de alerta](/docs/alerts-applied-intelligence/overview/#concepts-terms).
</Callout>

## Excepciones

El indicador de estado de salud no aplica para:

* [Panel](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards)
* Entidades que tienen su [atributo alertable](https://github.com/newrelic/entity-definitions/blob/093b7151d33afa68d098bf5b10f02490b8ee03ff/docs/entities/lifecycle.md#alertable) establecido en falso en la definición de entidad

## Estado de salud codificado por colores [#colors]

Entidad en New Relic están codificados por colores automáticamente con su estado de salud. Por ejemplo, para ver el estado de sus <InlinePopover type="apm"/>aplicaciones, vaya a <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> y luego haga clic en <DoNotTranslate>**APM**</DoNotTranslate>. El índice <DoNotTranslate>**Applications**</DoNotTranslate> enumera todas <InlinePopover type="apm"/>entidades de productos y su estado de salud actual.

<table>
  <thead>
    <tr>
      <th width={250}>
        <DoNotTranslate>
          **Color**
        </DoNotTranslate>
      </th>

      <th>
        <DoNotTranslate>
          **Health status**
        </DoNotTranslate>
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-green.png" src={accountsAlertsGreen}/>

        Verde
      </td>

      <td>
        La entidad está operativa. Estamos recopilando datos que puede ver en la UI correspondiente. Actualmente no se reporta ningún incidente de alerta por ello.
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-yellow.png" src={accountsAlertsYellow}/>

        Amarillo
      </td>

      <td>
        La entidad está degradada. Se ha superado un [umbral de advertencia](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/define-thresholds-trigger-alert) .
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-red.png" src={accountsAlertsRed}/>

        Rojo
      </td>

      <td>
        Se ha superado un [umbral crítico](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/define-thresholds-trigger-alert) :

        * [La notificación](/docs/alerts/new-relic-alerts-beta/managing-notification-channels/notification-channels-controlling-where-send-alerts) se ha enviado según la [preferencia de acumulación de incidentes](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/specify-when-new-relic-creates-incidents) seleccionada.

        * El incidente aparece en el [índice](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/explore-incident-history)

          <DoNotTranslate>[**Incidents**](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/explore-incident-history)</DoNotTranslate>

          [ ](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/explore-incident-history).
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-gray.png" src={accountsAlertsGray}/>

        Gris
      </td>

      <td>
        Se desconoce el estado de la entidad. No estamos recibiendo datos de alerta para la entidad. Esto podría significar que las alertas no están configuradas para la entidad, que el sistema de informes no funciona o que la condición de alerta de la entidad no informa consistentemente una señal para la entidad.
      </td>
    </tr>
  </tbody>
</table>

## Transiciones del estado de salud [#health-status-tranistions]

La siguiente tabla describe las diferentes transiciones de estado de salud que una entidad puede soportar:

<table>
  <thead>
    <tr>
      <th width={150}>
        <DoNotTranslate>
          **From...**
        </DoNotTranslate>
      </th>

      <th width={150}>
        <DoNotTranslate>
          **To...**
        </DoNotTranslate>
      </th>

      <th>
        <DoNotTranslate>
          **Transition explanation**
        </DoNotTranslate>
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-gray.png" src={accountsAlertsGray}/>

        Gris
      </td>

      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-green.png" src={accountsAlertsGreen}/>

        Verde
      </td>

      <td>
        La entidad es evaluada por al menos una condición y los resultados muestran que no hay ningún incidente presente.
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-green.png" src={accountsAlertsGreen}/>

        Verde /

        <img title="Alerts v3: alert icons" alt="icon-alerts-red.png" src={accountsAlertsRed}/>

        Rojo
      </td>

      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-gray.png" src={accountsAlertsGray}/>

        Gris
      </td>

      <td>
        Posibles explicaciones:

        * La última condición asociada a la entidad ha sido eliminada y por lo tanto no hay ningún estado que informar.
        * La última condición asociada a la entidad ha sido deshabilitada y por lo tanto no hay ningún estado que informar.
        * La entidad ha dejado de reportar datos.
        * La entidad sólo está siendo objetivo por condiciones que evalúan a las entidades como parte de una agregación, no individualmente.
        * Las condiciones de la entidad actualmente no generan una señal consistente.
        * Hay un problema con la plataforma New Relic. Consulte la [página de estado de New Relic](https://status.newrelic.com/) para obtener actualizaciones.
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-green.png" src={accountsAlertsGreen}/>

        Verde
      </td>

      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-yellow.png" src={accountsAlertsYellow}/>

        Amarillo /

        <img title="Alerts v3: alert icons" alt="icon-alerts-red.png" src={accountsAlertsRed}/>

        Rojo
      </td>

      <td>
        Existe al menos un incidente abierto al momento que la entidad realiza la evaluación.
      </td>
    </tr>

    <tr>
      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-yellow.png" src={accountsAlertsYellow}/>

        Amarillo /

        <img title="Alerts v3: alert icons" alt="icon-alerts-red.png" src={accountsAlertsRed}/>

        Rojo
      </td>

      <td>
        <img title="Alerts v3: alert icons" alt="icon-alerts-green.png" src={accountsAlertsGreen}/>

        Verde
      </td>

      <td>
        Se ha cerrado el último incidente abierto asociado a la entidad.
      </td>
    </tr>
  </tbody>
</table>

## Ejemplo: aplicación sin condiciones [#example-v3-no-policies]

A continuación se muestra un ejemplo de una aplicación incluida en el índice <InlinePopover type="apm"/>que <DoNotTranslate>**is not**</DoNotTranslate> se asoció con alguna condición. Su estado de salud codificado por colores es gris claro, lo que indica que actualmente no hay condiciones dirigidas a la entidad o que las condiciones de la entidad no están generando una señal consistente.

<img
  title="032715crop-events-no-v3.png"
  alt="032715crop-events-no-v3.png"
  src={accountsEventAlerts}
/>

<figcaption>
  Vaya a <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> y luego haga clic en <DoNotTranslate>**Explorer**</DoNotTranslate>: este ejemplo muestra una aplicación que actualmente no está asociada con ninguna condición de alerta.
</figcaption>

Siga [los procedimientos estándar](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions) para agregarlo a una condición existente o crear una nueva condición para ella.

## Ejemplo: aplicación con condiciones [#example-v3-yes-policies]

A continuación se muestra un ejemplo de una aplicación incluida en el índice <InlinePopover type="apm"/>que <DoNotTranslate>**is**</DoNotTranslate> se asoció con una o más condiciones. Su estado de salud codificado por colores es verde, porque estamos recopilando datos para él y actualmente no hay ningún incidente de Advertencia (amarillo) o Crítico (rojo).

<img
  title="032715crop-events-green.png"
  alt="032715crop-events-green.png"
  src={accountsCropAlertEventsGreen}
/>

<figcaption>
  Vaya a <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DoNotTranslate> y luego haga clic en <DoNotTranslate>**Explorer**</DoNotTranslate>: este ejemplo muestra una aplicación que tiene una o más condiciones. Su estado de salud codificado por colores (verde) muestra que la aplicación no ha superado ningún umbral.
</figcaption>

<Callout variant="tip">
  Para ver la lista de índice [de incidentes abiertos](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/explore-incident-history) actualmente en todos los productos, no solo en esta entidad, seleccione <DoNotTranslate>**View all incidents**</DoNotTranslate>.
</Callout>