...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Una comprobación de ubicación fallida se considerará fallida hasta que realice una comprobación exitosa, **hasta un máximo de 24 horas**. El búfer de evaluación para las condiciones de Synthetics de múltiples ubicaciones tiene un TTL (tiempo de vida) de 24 horas. Por ejemplo: una ubicación puede fallar y luego volver a estar disponible de inmediato, pero la consideraremos fallida hasta que una comprobación programada se reporte como exitosa. Si no se recibe una señal de recuperación en 24 horas, la falla finalmente se descarta del búfer.
      </td>
    </tr>
  </tbody>
</table>
...
Para usar la API REST de alertas para administrar condiciones de múltiples ubicaciones, use el [explorador de la API REST](https://api.newrelic.com/docs/#/Deprecation%20Notice%20-%20Alerts%20Location%20Failure%20Conditions).