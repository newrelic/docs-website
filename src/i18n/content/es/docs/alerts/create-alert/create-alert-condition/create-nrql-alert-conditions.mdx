...
<Callout variant="important">
  Si no se reportan eventos (líneas), la pérdida de señal `cannot` evitarse incluso con los cambios mencionados anteriormente. Recomendamos establecer o mantener un `Lost Signal Threshold` para activar un evento de alerta si el evento deja de reportar por completo.
</Callout>

Para obtener más información, consulte nuestra [publicación del blog](https://knowledge.newrelic.com/s/article/How-Can-I-Figure-Out-When-To-Use-Gap-Filling-and-Loss-of-Signal) sobre la resolución de problemas para valores cero frente a nulos.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Para las condiciones NRQL, puede crear una [descripción](/docs/alerts/new-relic-alerts/defining-conditions/alert-condition-descriptions) personalizada para agregar a cada evento de alerta. Las descripciones se pueden mejorar mediante la sustitución de variables basada en los metadatos del evento de alerta específico.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Puede utilizar la detección de pérdida de señal para alertar cuando sus datos (una señal de telemetría) deban considerarse perdidos. Una pérdida de señal puede indicar que un servicio o entidad ya no está en línea o que un trabajo periódico falló al ejecutarse. También puede utilizar esto para asegurarse de que los eventos de alerta para datos esporádicos, como los recuentos de errores, se cierren cuando no se reciba ninguna señal.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * Proporcione una descripción personalizada del evento de alerta para la condición en la página <DNT>**Add details**</DNT> que se incluirá en los eventos de alerta y notificaciones.
        * Agregue la URL del runbook para incluir los procedimientos de su organización para la gestión de eventos de alerta. También puede agregar esta información a la descripción del evento de alerta personalizada.
      </td>
    </tr>
...
* Para los umbrales de &quot;durante al menos x minutos&quot;: debido a que la ventana de evaluación se ha restablecido, habrá un retraso de al menos x minutos antes de que se puedan reportar eventos de alerta.
...
* Cambiar la configuración &quot;cerrar eventos de alerta por pérdida de señal&quot;
...
* Alternar la configuración &quot;abrir evento de alerta por pérdida de señal&quot;
...
* <DNT>
  ...
  * Cerrar todos los eventos de alerta actualmente abiertos: esto cierra todos los eventos de alerta abiertos que están relacionados con una señal específica. No necesariamente cerrará todos los eventos de alerta para una condición. Si generas alertas sobre un servicio efímero o una señal esporádica, querrás elegir esta acción para asegurar que los eventos de alerta se cierren correctamente. El nombre del nodo GraphQL para esto es [`closeViolationsOnExpiration`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal).
  * Abrir nuevos eventos de alerta: esto abrirá un nuevo evento de alerta cuando la señal se considere perdida. Estos eventos de alerta indicarán que se deben a una pérdida de señal. Según sus preferencias de eventos de alerta, esto debería activar una notificación. El nombre del nodo GraphQL para esto es [`openViolationOnExpiration`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal).
  * Cuando habilites ambas acciones anteriores, primero cerraremos todos los eventos de alerta abiertos y luego abriremos un nuevo evento de alerta por pérdida de señal.
  * No abrir eventos de alerta de &quot;señal perdida&quot; en una terminación esperada. Cuando se espera que una señal finalice, puede optar por no abrir un nuevo evento de alerta. Esto es útil cuando sabe que se perderá una señal en un momento determinado y no desea abrir un nuevo evento de alerta para esa pérdida de señal. El nombre del nodo GraphQL para esto es [`ignoreOnExpectedTermination`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal).

<Callout variant="important">
  Para evitar que se abra un evento de alerta de pérdida de señal cuando <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>, debe agregar la etiqueta `termination: expected` a la entidad. Esta etiqueta nos indica que esperábamos que la señal terminara. Vea [cómo agregar la etiqueta directamente a la entidad](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/#add-tags). Tenga en cuenta que la etiqueta `hostStatus: shutdown` también evitará que se abra un evento de alerta de &quot;pérdida de señal&quot;. Para obtener más información, consulta [Crear una condición de &quot;host que no reporta&quot;](/docs/infrastructure/infrastructure-alerts/create-infrastructure-host-not-reporting-condition/#create-condition).
</Callout>
...
4. Elija qué debe suceder cuando se pierda la señal. Puede marcar cualquiera o todas las siguientes opciones: <DNT>**Close all current open alert events**</DNT>, <DNT>**Open new &quot;lost signal&quot; alert event**</DNT>, <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>. Estos controlan cómo se manejarán los eventos de alerta de pérdida de señal para la condición.
...
7. Si seleccionó <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>, debe agregar la etiqueta `termination: expected` a la entidad para evitar que se abra un evento de alerta de pérdida de señal. Vea [cómo agregar la etiqueta directamente a la entidad](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/#add-tags).

<Callout variant="tip">
  Quizás tengas curiosidad de saber por qué querrías tener <DNT>**Open new &quot;lost signal&quot; alert event**</DNT> y <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT> configurados como verdaderos. Piénsalo de esta manera: quieres recibir una notificación cuando pierdes la señal. Excepto que no quieres recibir una notificación el único momento en el que sabes que la señal se detendrá porque la programaste. En ese caso, establecerías ambos como verdaderos y, cuando esperes que se pierda la señal, agregarías la etiqueta `termination: expected` a la entidad relevante.
</Callout>

Los eventos de alerta abiertos debido a la pérdida de señal se cierran cuando:

* la señal regresa. Los eventos de alerta de pérdida de señal recién abiertos se cerrarán inmediatamente cuando se evalúen nuevos datos.
* expira la condición a la que pertenecen. De forma predeterminada, las condiciones caducan después de 3 días.
* cierra manualmente el evento de alerta con la opción <DNT>**Close all current open alert events**</DNT>.
...
<Callout variant="important">
  Inmediatamente después de crear una nueva condición de alerta de ventanas deslizantes o realizar cualquier acción que pueda causar un [restablecimiento de evaluación](#evaluation-resets), su condición necesitará tiempo para generar un &quot;búfer agregado&quot; durante la duración de la primera ventana de agregación. Durante ese tiempo, no se activarán eventos de alerta. Una vez que haya pasado esa única ventana de agregación, se habrá construido un &quot;buffer&quot; completo y la condición funcionará normalmente.
</Callout>
...
* <DNT>**None**</DNT>: (Predeterminado) Seleccione esta opción si no desea realizar ninguna acción en las ventanas de agregación vacías. Al evaluar, una ventana de agregación vacía restablecerá el temporizador de duración del umbral. Por ejemplo, si una condición establece que todas las ventanas de agregación deben tener puntos de datos por encima del umbral durante 5 minutos, y 1 de las 5 ventanas de agregación está vacía, entonces la condición no será un evento de alerta.
...
* NerdGraph es nuestra API recomendada para esto, pero si utiliza nuestra API REST, puede encontrar esta configuración en el explorador de la API REST en la sección <DNT>**&quot;signal&quot;**</DNT> de [la API de condiciones NRQL de alerta](https://api.newrelic.com/docs/#/Alerts%20NRQL%20Conditions).
