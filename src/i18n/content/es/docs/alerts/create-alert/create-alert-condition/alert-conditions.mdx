...
Una condición de alerta es una consulta en ejecución continua que mide un conjunto determinado de eventos contra un umbral definido y abre un [evento de alerta](/docs/alerts/organize-alerts/specify-when-alerts-create-events) cuando se alcanza el umbral durante una ventana de tiempo especificada.
...
* La [página<DNT>**Alerts Overview**</DNT> ](/docs/alerts/smart-alerts-coverage), para aplicar la cobertura de estándares en muchas entidades a la vez con alertas inteligentes (vista previa pública)
...
<Callout variant="important">
  Después de crear una condición de alerta, la plataforma de New Relic debe evaluarla antes de que pueda vincularse a una entidad, como una aplicación, servicio o monitor sintético. La evaluación comienza solo después de que la plataforma reciba una señal de datos válida que coincida con la consulta NRQL o la métrica para la condición. La evaluación puede tardar unos minutos en completarse. Para verificar que la condición recibe los datos correctos, ejecute su consulta en el [generador de NRQL](/docs/nrql/get-started/introduction-nrql-new-relics-query-language/).
</Callout>

Para todos los métodos, excepto para nuestro modo guiado, el proceso para crear una condición de alerta será *exactamente* el mismo que se describe en los pasos a continuación. A medida que configuras tu condición, consulta las [mejores prácticas de alertas](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices) para obtener orientación sobre cómo nombrar condiciones, agregar etiquetas, establecer umbrales efectivos y reducir el ruido.
...
<Steps>
  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="outlier-threshold" title="detección de valor atípico">
        ...
        * **Grupos de evaluación**: Análisis de segmentos por facetas como el entorno o la región.
        ...
        Obtenga más información sobre la detección de valor atípico en nuestra [documentación de detección de valor atípico](/docs/alerts/create-alert/set-thresholds/outlier-detection/).
      </Collapser>

      <Collapser id="disable-health-status-reporting" title="Reportes del estado de salud">
        En New Relic, las entidades se asocian con [estados de salud codificados por colores](/docs/alerts/create-alert/examples/view-entity-health-status-find-entities-without-alert-conditions/#colors). Puede ver el estado actual de estas entidades desde sus respectivos índices y mapas. Cuando una condición de alerta se asocia a una entidad, el estado de salud de la entidad es determinado por la condición de alerta. Si la alerta activa un evento de alerta, el estado de salud de la entidad cambia según el nivel de gravedad del evento de alerta.
  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="select-an-existing-policy" title="Seleccione una política existente">
        ...
        Obtenga más información sobre la creación de políticas [aquí](/docs/alerts/organize-alerts/specify-when-alerts-create-events).
      </Collapser>
      ...
      <Collapser id="create-a-new-policy" title="Crear una nueva política">
        ...
        3. Una incidencia para cada evento de alerta:
      ...
      <Collapser id="custom-alert event-description" title="Configurar una descripción de evento de alerta personalizada">
...
<CollapserGroup>
  ...
  <Collapser id="instance-alert-events" title="Condiciones de instancia de Java">
  ...
  <Collapser id="jvm-metrics" title="Condiciones métricas de salud de JVM (aplicaciones Java)">
    Para aplicaciones Java monitoreadas por APM, puede establecer [umbrales](/docs/using-new-relic/welcome-new-relic/get-started/glossary#alert-threshold) que abran un evento de alerta cuando el tamaño del heap o el número de hilos de una sola JVM esté fuera del rango operativo esperado.

    Evaluamos las violaciones de los umbrales de alerta individualmente para cada una de las [instancias seleccionadas](instance-alert-events) de la aplicación. Al crear su condición, seleccione <DNT>**JVM health metric**</DNT> como el tipo de condición para la política de alertas de su aplicación Java, luego seleccione cualquiera de los umbrales disponibles:
    ...
    * Tiempo de CPU de recolección de basura
      ...
      los eventos de alerta se cerrarán automáticamente cuando se cumpla el inverso del umbral, pero mediante la UI también puede cambiar el momento en que un [evento de alerta](/docs/new-relic-solutions/get-started/glossary#alert-event) se cierra forzosamente para una métrica de salud de JVM. El valor predeterminado es 24 horas.
  </Collapser>
  ...
  <Collapser id="app-response-percentiles" title="Condiciones percentil transacción web">
    ...
    2. Para definir los umbrales que abren el evento de alerta, ingrese el valor del tiempo de respuesta <DNT>**Percentile nth**</DNT>, luego seleccione su frecuencia (por encima, por debajo o igual a este valor).
  ...
  <Collapser id="dynamic-targeting-labels" title="Orientación dinámica con etiquetas para aplicaciones">
    ...
    El uso de la selección dinámica de objetivos con su condición también requiere que configure un [temporizador de cierre de eventos de alerta](/docs/alerts/alert-event-management/how-alert-condition-events-are-closed#time-limit).
...
Para [agregar más condiciones a una política](/docs/alerts/create-alert/create-alert-condition/alert-conditions):
...
<DocTiles>
  ...
  <DocTile title="Condición de alerta avanzada" path="/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/">
    Si ya configuró su condición de alerta, profundice con la configuración avanzada
  </DocTile>

  <DocTile title="Recibir notificaciones" path="/docs/alerts/get-notified/alert-event-workflows/">
