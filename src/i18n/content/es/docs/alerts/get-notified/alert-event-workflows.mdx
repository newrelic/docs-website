...
Los problemas son grupos de eventos de alerta que describen el problema subyacente de sus síntomas. Cuando se crea un nuevo evento de alerta, la inteligencia de incidentes abre una incidencia y evalúa otras incidencias abiertas en busca de correlaciones. Para obtener más información, consulta [Uso de Incident Intelligence](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/use-incident-intelligence/).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        El problema se ha cerrado debido al cierre de todos sus eventos de alerta, fin de TTL o ha estado `Inactive` por demasiado tiempo
      </td>
    ...
    <tr>
      ...
      <td>
        Un evento de alerta en la incidencia ha pasado de prioridad Alta y se ha convertido en una incidencia de prioridad Crítica
      </td>
    ...
    <tr>
      ...
      <td>
        Se agregó un evento de alerta a la incidencia, se cerró un evento de alerta o se fusionó una incidencia diferente con esta
      </td>

      <td>
        Se ha resuelto un evento de alerta
      </td>
...
3. Con flujo de trabajo, filtra tus incidencias y envíalas al destino correspondiente. Puede crear una consulta para filtrar los problemas correctos.
   ...
   <Callout variant="tip">
     Para configurar notificaciones separadas en eventos de alerta `WARNING`, filtre por problemas con una prioridad igual a `HIGH`.
   </Callout>
...
5. Recomendado: filtre los problemas por etiqueta de equipo para que todos los equipos puedan recibir notificaciones cuando su entidad esté incluida en un problema. Los pasos se pueden encontrar en la demostración a continuación:
   ...
   Para obtener algunos consejos sobre el uso de etiquetas, consulte [esta publicación del foro de soporte sobre patrones de flujo de trabajo](https://support.newrelic.com/s/hubtopic/aAX8W0000008dNf/relic-solution-patterns-for-implementing-alerts-workflows).

6. Opcional: Enriquezca sus datos.

   <CollapserGroup>
     <Collapser className="freq-link" id="enrichments" title="Cómo enriquecer los datos">
       Actualmente, solo puede enviar salidas JSON o numéricas a [destinos](/docs/alerts-applied-intelligence/notifications/destinations/) de Webhook y Jira. Además, puede enviar JSON a AWS EventBridge mediante las [variables de flujo de trabajo](https://docs.newrelic.com/docs/alerts/get-notified/custom-variables-alert-event-workflows/) que apuntan al resultado de Enrich data. Además, solo puede enviar imágenes a otros destinos como Slack, Pagerduty, ServiceNow (App) y correo electrónico. Para obtener mejores resultados en Webhook y Jira, use una consulta que tenga una salida de valor único, como `count`, `min` o `max`.
       ...
       * Para ver ejemplos de enriquecimiento, consulte [Ejemplos de enriquecimiento de datos de flujos de trabajo](/docs/alerts/get-notified/custom-variables-alert-event-workflows/#enrichment-examples).
...
7. Notificar: elija uno o más destinos y agregue un mensaje opcional.
   ...
   <Callout variant="tip">
     En cualquier canal de destino, comience a escribir y se abrirá un menú de variables. Verás los nombres de las [variables](/docs/alerts/get-notified/custom-variables-alert-event-workflows), que serán reemplazados por los valores de la variable en tiempo de ejecución. Para utilizar los resultados de los enriquecedores, use su nombre.
   </Callout>
