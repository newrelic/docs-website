...
<CollapserGroup>
  <Collapser className="freq-link" id="jira" title="Atlassian Jira">
    ...
    * El reconocimiento del problema de New Relic se activa cuando el estado del problema de Jira cambia a `in-progress`.
    ...
    Haga clic en una notificación de prueba con valores de campo predeterminados para ver la incidencia de Jira. Si tiene éxito, obtendrá un enlace para ver el evento de alerta en Jira.
  ...
  <Collapser className="freq-link" id="email" title="Correo electrónico">
    El destino de correo electrónico se crea automáticamente cuando eliges <DNT>**Email**</DNT> como el canal de notificación para un [flujo de trabajo](/docs/alerts/get-notified/alert-event-workflows/), por lo que no necesitas configurarlo desde el menú <DNT>**Destinations**</DNT>. Cada destino de correo electrónico es único para el flujo de trabajo asociado a él, lo que significa que puede aparecer como un duplicado en el feed de destinos.
  ...
  <Collapser className="freq-link" id="pagerduty" title="PagerDuty">
    Integra New Relic con PagerDuty para crear, actualizar, reconocer y resolver automáticamente eventos de alertas de PagerDuty.
  ...
  <Collapser className="freq-link" id="slack" title="Slack">
    ...
    Su espacio de trabajo de Slack debe tener instalada la [aplicación New Relic](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(o la [aplicación de la UE](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) para `one.eu.newrelic` clientes, o la [aplicación de JP](https://newrelic.slack.com/marketplace/A0ATV7NGLLD-new-relic-jp) para `one.jp.newrelic` clientes). Un administrador del espacio de trabajo debe aprobar la aplicación antes de que pueda instalarla individualmente.
