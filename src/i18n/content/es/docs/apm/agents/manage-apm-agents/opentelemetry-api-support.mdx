...
<CollapserGroup>
  ...
  <Collapser id="python-config" title="Configuración del agente Python">
    ...
    **Uso de variable de entorno:**
    ...
    <Callout variant="important">
      Cuando el soporte de OpenTelemetry está habilitado, el agente detectará automáticamente si `opentelemetry-api` está instalado y cambiará al uso de la instrumentación de OpenTelemetry para cualquier biblioteca compatible, incluso si New Relic tiene un hook de instrumentación para esa biblioteca.
    </Callout>
    ...
    1. **Instrumentación nativa**: Para bibliotecas con instrumentación nativa de OpenTelemetry incorporada y que no son compatibles con New Relic, el agente utilizará la instrumentación nativa de OpenTelemetry. Si el usuario desea usar la instrumentación de OpenTelemetry en lugar de los hooks de instrumentación integrados de New Relic, esa biblioteca se puede agregar a la configuración `opentelemetry.traces.include`.

    2. **Instrumentación de bibliotecas**: si se detecta una biblioteca de instrumentación de OpenTelemetry y New Relic aún no proporciona un hook de instrumentación, el agente utilizará la instrumentación de OpenTelemetry. De manera similar a la instrumentación nativa, si el usuario desea usar la instrumentación de OpenTelemetry en lugar de los hooks de instrumentación integrados de New Relic, esa biblioteca se puede agregar a la configuración `opentelemetry.traces.include`.
    ...
    <Callout variant="caution">
      Las bibliotecas de instrumentación de AWS y gRPC seguirán utilizando la instrumentación de New Relic. Las bibliotecas de ML/IA con instrumentación nativa están deshabilitadas explícitamente por ahora y tendrán soporte en una versión futura.
    </Callout>
    ...
    **Ejemplo:**
    ...
    2. **Cambio automático**: Si se encuentra y New Relic aún no es compatible con esta biblioteca, el agente utilizará la instrumentación de la biblioteca de OpenTelemetry
  ...
  <Collapser id="ruby-config" title="Configuración del agente Ruby">
    ...
    <Callout variant="important">
      Cuando el soporte de OpenTelemetry está habilitado, el agente detectará automáticamente si `opentelemetry-api` está instalado. Esta gema debe estar instalada para usar las características del agente híbrido.
    </Callout>
    ...
    **Ejemplo:**
    ...
    ✅ **Compatible**: Span Links mediante la API de `OpenTelemetry::Trace::Span#add_link`

    #### Eventos en Spans

    ✅ **Admitido**: eventos en Spans mediante la API de `OpenTelemetry::Trace::Span#add_event`

    ### Cómo funciona

    Cuando el soporte de OpenTelemetry está habilitado, el agente Ruby de New Relic:

    1. **Escucha** las llamadas a las API de rastreo de OpenTelemetry
    2. **Traduce** esas llamadas a modelos de datos de New Relic
    3. **Incorpora** spans de OpenTelemetry en las trazas de New Relic

    <Callout variant="information">
      De forma predeterminada, la instrumentación de OpenTelemetry se excluye para las bibliotecas que el agente Ruby de New Relic también instrumenta. Esto incluye bibliotecas instrumentadas de forma nativa y bibliotecas con paquetes de instrumentación en [opentelemetry-ruby-contrib](https://github.com/open-telemetry/opentelemetry-ruby-contrib).
    </Callout>
