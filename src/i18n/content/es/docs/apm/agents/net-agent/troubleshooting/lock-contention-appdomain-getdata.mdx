...
Cuando el agente .NET está conectado a la aplicación <DoNotTranslate>**.NET Framework**</DoNotTranslate> de alto rendimiento, algunos clientes informaron haber observado una alta contención de subprocesos. Al revisar los volcados del proceso y el rastreo del stack, es posible que muchos hilos estén bloqueados por llamadas a `AppDomain.GetData()`.
