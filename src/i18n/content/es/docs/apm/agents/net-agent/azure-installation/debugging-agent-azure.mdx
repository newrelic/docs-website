...
Después de [instalar el agente .NET de New Relic para Azure](/install/dotnet/?deployment=azure), tiene uno o más de los siguientes problemas:
...
* Asegúrese de que [se hayan seguido los pasos de instalación y ejecución](/install/dotnet/?deployment=azure), incluido el reinicio de la aplicación web.
* <DNT>**must**</DNT> [Deshabilitas la aplicación Insights](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) para que el agente .NET funcione correctamente.
* La aplicación <DNT>**must**</DNT> debe recibir tráfico para que el agente de .NET se inicie.
...
4. Tome nota del ID de proceso (PID) con el que se ejecuta su aplicación, para que pueda verificar si se está creando un log para ese proceso. Puede encontrar el PID utilizando el <DNT>**Process Explorer**</DNT> integrado de Kudu.
...
2. En <DNT>**Process Explorer**</DNT>, busque el proceso que está intentando monitor y anote el número en la columna PID. Si está intentando monitor una aplicación web, el nombre del proceso probablemente será `w3wp.exe` sin la etiqueta SCM, que hace referencia al proceso Kudu en sí. El agente no debe intervenir en el proceso SCM (Kudu).
3. Cuando hayas localizado el proceso de tu aplicación, haz clic en el botón `Properties` .
4. Haga clic en la pestaña <DNT>**Modules**</DNT> y busque `NewRelic.Profiler.dll`. Esta DLL es necesaria para que el agente monitor su aplicación. Si no puede encontrarlo, asegúrese de que estén presentes las variables de entorno correctas (más sobre esto en [el paso 3](#step-three)) y de que [la aplicación Microsoft Insights esté deshabilitada](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/).
...
1. En <DNT>**Process Explorer**</DNT>, ubique el proceso de su aplicación y haga clic en el botón `Properties` .
2. Seleccione la pestaña <DNT>**Environment**</DNT> .
...
<CollapserGroup>
  <Collapser id="env-var-framework-extension" title=".NET framework: verifique las variables de entorno al instalar a través de New Relic Azure Site Extension">
  ...
  <Collapser id="env-var-framework-nuget" title=".NET framework: verifique las variables de entorno al instalar a través de NuGet">
    Para el paquete NuGet, las variables de entorno [deben agregarse como configuración de la aplicación](/install/dotnet/?deployment=azure&azure=azurenugetframework).
  ...
  <Collapser id="env-var-core-windows-extension" title=".NET Core (Windows): compruebe las variables de entorno cuando utilice la extensión del sitio New Relic Azure">
    Las variables de entorno que aparecen en la pestaña <DNT>**Environment**</DNT> en <DNT>**Process Explorer**</DNT> de Kudu deberían ser similares a estas:
  ...
  <Collapser id="env-var-core-windows-nuget" title=".NET Core (Windows): busque variables de entorno cuando utilice el paquete NuGet">
    Para el paquete NuGet, las variables de entorno [deben agregarse como configuraciones de la aplicación](/install/dotnet/?deployment=azure&azure=azureframeworkcore). Las variables de entorno que aparecen en la pestaña <DNT>**Environment**</DNT> en <DNT>**Process Explorer**</DNT> de Kudu deberían ser similares a estas:
  ...
  <Collapser id="env-var-core" title=".NET Core (Linux): compruebe las variables de entorno cuando utilice el paquete NuGet">
    Para el paquete NuGet, las variables de entorno [deben agregarse como configuraciones de la aplicación](/install/dotnet/?deployment=azure&azure=azureframeworkcore).
...
* Para los framework instrumentados automáticamente para .NET Core y .NET framework, consulte nuestra [página de compatibilidad](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/).
