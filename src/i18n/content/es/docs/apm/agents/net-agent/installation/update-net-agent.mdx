...
## Verifique la versión de su agente .NET [#your\_version][#your_version]

1. Consulte las [notas de la versión](/docs/releases/dotnet) para encontrar la última versión del agente.
2. Verifique la versión del agente .NET que tiene instalado actualmente usando <DNT>**Windows Programs and Features**</DNT> o usando su administrador de paquetes de Linux.
...
## Actualizar el agente .NET (Windows) [#updating\_net][#updating_net]

<Callout variant="important">
  Antes de actualizar:

  * La actualización del agente requiere derechos de <DNT>**Administrator**</DNT> en el grupo de administradores de Windows. Para obtener más información, consulte la [documentación de permisos](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/#user-rights).
  ...
  * Si su aplicación está alojada en IIS, asegúrese de <DNT>**stop IIS**</DNT> antes de actualizar el agente .NET.
</Callout>

<CollapserGroup>
  <Collapser id="update-framework-standard" title="Instalación estándar">
    1. Asegúrese de tener [.NET 4.5 o superior instalado en su sistema](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/#net-version-framework).

    2. Detener IIS.

    3. Descargue y ejecute el paquete de instalación MSI adecuado para la [arquitectura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/#architecture-framework):
  ...
  <Collapser id="site-extention-update" title="Actualización del agente con la extensión de sitio de Azure de New Relic">
    Para actualizar el agente .NET para una aplicación web de Azure mediante la extensión de sitio de Azure de New Relic, siga las instrucciones de instalación para:

    * Elimine la extensión de sitio de New Relic Azure, esto desinstala el agente
    * Vuelva a instalar la última versión de la extensión del sitio de Azure de New Relic mediante [estas instrucciones de instalación](/install/dotnet/?deployment=azure&azure=azuresiteextension), que instalan la última versión del agente.
    * Reinicie la aplicación web para iniciar el agente actualizado.
  </Collapser>
</CollapserGroup>

## Actualizar el agente .NET (Linux) [#updating\_net\_core][#updating_net_core]
...
<CollapserGroup>
  <Collapser id="updating-core-zip" title="archivo tar.gz o deb">
    1. Cierre su(s) aplicación(es).
    2. Descargue el archivo apropiado para la [arquitectura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/#architecture-core) desde el [sitio de descarga](https://download.newrelic.com/dot_net_agent/latest_release/).
  ...
  <Collapser id="yum" title="yum (CentOS, Oracle Linux o RHEL)">
    1. Cierre su(s) aplicación(es).
  ...
  <Collapser id="apt" title="apt (Debian, Linux Mint o Ubuntu)">
    1. Cierre su(s) aplicación(es).
  ...
  <Collapser id="rpm" title="rpm (CentOS, Oracle Linux o RHEL)">
    1. Cierre su(s) aplicación(es).

    2. Descargue el archivo rpm adecuado para la [arquitectura](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/#architecture-core) desde el [sitio de descargas](https://download.newrelic.com/dot_net_agent/latest_release/).
...
## Actualizar agente de .NET framework anterior (inferior a 7.0) [#updating\_older\_net][#updating_older_net]
...
<CollapserGroup>
  <Collapser id="old-framework-versions" title="Detalles de la versión del agente de .NET framework legacy">
    <Callout variant="important">
      ...
      <tbody>
        ...
        <tr id="500">
          ...
          <td>
            ...
            Es posible que la [instrumentación personalizada](/docs/apm/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation/) deba actualizarse para usar [transacciones personalizadas](/docs/apm/agents/net-agent/custom-instrumentation/create-transactions-xml-net/). Para ver un ejemplo de lo que verá en la UI, consulte esta [publicación de la comunidad técnica en línea de New Relic](https://support.newrelic.com/s/hubtopic/aAX8W0000008YBU/change-in-nonweb-transaction-reporting-with-version-5x).
          </td>
        </tr>
        ...
        <tr id="220">
          ...
          <td>
            La versión 2.19.3.0 fue la última versión del agente de .NET que requirió las llamadas API `GetBrowserTimingFooter()` y `GetBrowserTimingHeader()` para <InlinePopover type="browser" />. En la versión del agente 2.20.24.0 o superior, solo necesita llamar a `GetBrowserTimingHeader()`. `GetBrowserTimingFooter()` no tiene ningún efecto. Recomendación: Elimine estas referencias del código.
          </td>
        </tr>
