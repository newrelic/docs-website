...
<CollapserGroup>
  ...
  >
    En este ejemplo, New Relic informa un error de `GetResponse()` <DoNotTranslate>**unless**</DoNotTranslate> el método `Foo()` está instrumentado. Siempre que `Foo` esté instrumentado, New Relic comienza una transacción cuando se llama a `Foo` y finaliza la transacción cuando finaliza `Foo` .
