...
Utilice estas opciones para instalar y configurar su agente a través del archivo newrelic.config. El agente .NET admite las siguientes categorías de opciones de configuración:
...
<CollapserGroup>
  ...
  >
    ...
    Si es falso, el agente utiliza un subproceso del grupo para completar el procesamiento de la transacción.
  ...
  >
    ...
    Si es falso, el agente intenta reutilizar la transacción existente cuando se encuentran métodos instrumentados personalizados en escenarios asíncronos/enhebrados.

    Si es verdadero, el agente intenta crear una nueva transacción cuando se encuentran métodos instrumentados personalizados en escenarios asíncronos/enhebrados.
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `2000` ([`sendDataOnExit`](#service-sendDataOnExit) habilitado)

            `120000` ([`sendDataOnExit`](#service-sendDataOnExit) deshabilitado)
          </td>
        </tr>
...
<CollapserGroup>
  ...
  >
    ...
    Para mayor seguridad, el agente .NET admite el uso de una contraseña de proxy ofuscada con el atributo contraseñaObfuscated. La contraseña del proxy ofuscada se genera mediante el siguiente comando [CLI de New Relic](https://github.com/newrelic/newrelic-cli) :
...
<CollapserGroup>
  ...
  >
    ...
    Enviar mensaje de log a la consola, además del archivo de log. Alternativamente, configure la variable de entorno `NEWRELIC_LOG_CONSOLE` en el entorno de la aplicación.
  </Collapser>
  ...
  >
    ...
    El directorio que contiene el archivo de registro generado por el agente. Si se omite, se utilizará de forma predeterminada un directorio llamado <DoNotTranslate>**logs**</DoNotTranslate> en el área de instalación del agente New Relic.

    Alternativamente, configure la variable de entorno `NEWRELIC_LOG_DIRECTORY` en el entorno de la aplicación.
  </Collapser>
  ...
  >
    ...
    Define el número de archivo de registro del agente a retener. Cuando se alcanza el número máximo de archivos de registro, se elimina el archivo de registro más antiguo. Un valor de `0` especifica que no hay un número máximo de archivos de registro.
...
Un atributo es un par de valores principales que determina las propiedades de un evento o transacción. Cada atributo se envía a APM traza de la transacción, APM error traza, Transaction evento, TransactionError evento o PageView evento. El elemento principal `attributes` habilita o deshabilita la recopilación de atributos para el agente .NET y define un atributo específico para recopilar o excluir. También puede configurar los ajustes del atributo según su destino: [recopilación de errores](#error-attributes), [traza de la transacción](#tracer-attributes), [instrumentación del navegador](#browser-attributes) y [evento de transacción](#transaction-attributes).

En este ejemplo, el agente excluye todos los atributos cuya clave comienza con <DoNotTranslate>**myApiKey**</DoNotTranslate> (<DoNotTranslate>**myApiKey.bar**</DoNotTranslate>, <DoNotTranslate>**myApiKey.value**</DoNotTranslate>), pero recopila el atributo personalizado <DoNotTranslate>**myApiKey.foo**</DoNotTranslate>.
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      Cuando se utiliza la variable de entorno `NEW_RELIC_ATTRIBUTES_INCLUDE` , se sobrescribe el conjunto predeterminado de inclusiones en newrelic.config. Consulte [aquí](#allow-all-headers-enabled) para obtener una lista detallada de las inclusiones y exclusiones predeterminadas.
    </Callout>
  </Collapser>
  ...
  >
    ...
    <Callout variant="caution">
      Cuando se utiliza la variable de entorno `NEW_RELIC_ATTRIBUTES_EXCLUDE` , se sobrescribe el conjunto predeterminado de exclusiones en newrelic.config. Consulte [aquí](#allow-all-headers-enabled) para obtener una lista detallada de las inclusiones y exclusiones predeterminadas.
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="ignoreClasses"
  >
    Una lista de nombres de clases completos que se ignorarán. El número máximo de combinaciones de clase de error y mensaje que <DoNotTranslate>**SHOULD**</DoNotTranslate> debe informar es 50. Si se enumeran más de 50, solo se utilizarán los primeros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate>
  </Collapser>
  ...
  >
    Un mapa opcional de nombres de clases completos para una lista de cadenas que coinciden con una subcadena del mensaje de error. El número máximo de combinaciones de clase de error y mensaje que <DoNotTranslate>**SHOULD**</DoNotTranslate> debe informar es 50. Si se enumeran más de 50, solo se utilizarán los primeros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate>
  </Collapser>
  ...
  >
    Una lista de nombres de clases completos que se marcarán como se esperaba. El número máximo de combinaciones de clase de error y mensaje que <DoNotTranslate>**SHOULD**</DoNotTranslate> debe informar es 50. Si se enumeran más de 50, solo se utilizarán los primeros 50 <DoNotTranslate>**SHOULD**</DoNotTranslate>
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    El valor predeterminado es `apdex_f`, que establece el umbral en cuatro veces el valor <DoNotTranslate>**apdex_t**</DoNotTranslate> de la aplicación. Para obtener más información sobre <DoNotTranslate>**apdex_t**</DoNotTranslate>, consulte [Apdex](/docs/site/apdex-measuring-user-satisfaction).
  ...
  >
    ...
    De forma predeterminada, `maxStackTrace` está establecido en 0, lo que deshabilita el rastreo del stack como parte de una traza de la transacción. Si este valor se establece mayor que 0, entonces se capturará el rastreo de la pila para la traza de la transacción.
  </Collapser>
...
<CollapserGroup>
  ...
  >
    Establezca el elemento `<spanEvents>` en `false` para deshabilitarlo mediante el archivo newrelic.config. Este elemento es hijo del elemento `<configuration>` .
...
<Callout variant="important">
  El rastreo multiaplicación ha quedado obsoleto a partir de [la versión 9.0.0 del agente](/docs/release-notes/agent-release-notes/net-release-notes/net-agent-9000/) y deshabilitado de forma predeterminada. Para utilizar CAT con v9+ del agente, debe configurar tanto `crossApplicationTracer.enabled = true` como `distributedTracing.enabled = false`. Habilitar [rastreo distribuido](/docs/understand-dependencies/distributed-tracing/get-started/introduction-distributed-tracing) deshabilitará [el rastreo multiaplicación](#cross_application_tracer).
</Callout>
...
Para obtener más detalles, consulte [nuestros documentos sobre el uso del contexto del inicio de sesión del agente .NET](/docs/logs/logs-context/net-configure-logs-context-all).
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Reenvío de log"
  >
    Utilice este subelemento para habilitar el reenvío del registro de su aplicación a New Relic. El valor predeterminado del atributo `enabled` es `true`. El valor predeterminado del atributo `maxSamplesStored` es `10000`.
  ...
  <Collapser
    id="contextData"
    title="Datos de contexto (atributo personalizado)"
  >
    ...
    Establezca el atributo `enabled` en `true` para habilitar la captura y el reenvío de datos de contexto de registro a New Relic. El valor predeterminado es `false`.

    El atributo `include` es una lista separada por comas de nombres de atributos que se incluirán cuando la captura de datos de contexto esté habilitada. El valor predeterminado es una cadena vacía, que significa "incluir todo".

    El atributo `exclude` es una lista separada por comas de nombres de atributos que se excluirán cuando la captura de datos de contexto esté habilitada. El valor predeterminado es una cadena vacía, que significa "no excluir nada".
  ...
  >
    Utilice este subelemento para habilitar la decoración del registro de su aplicación con metadatos de enlace de New Relic. El valor predeterminado del atributo `enabled` es `false`. Se requieren algunos cambios de configuración adicionales para la decoración log local; consulte [.NET: Configurar el registro en contexto](/docs/logs/logs-context/net-configure-logs-context-all#2-decorate) para obtener más información.
  </Collapser>
</CollapserGroup>
