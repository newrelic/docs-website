...
El elemento `applications` es un elemento secundario del elemento `instrumentation`. El elemento `applications` admite `application` elementos secundarios que especifican qué aplicaciones que no son web instrumentar. El elemento `application` contiene un atributo `name`. Además, a partir de la versión 10.48.0 del agente, existe un atributo `include` opcional con un valor predeterminado de &quot;true&quot;. Establezca este atributo en &quot;false&quot; para evitar que el agente instrumente ese proceso.
...
Alternativamente, a partir de la versión 10.48.0 del agente, establezca la(s) variable(s) de entorno `NEW_RELIC_INCLUDED_APPLICATION_NAMES` y/o `NEW_RELIC_EXCLUDED_APPLICATION_NAMES` en el entorno de la aplicación:
...
Tenga en cuenta que excluir tiene prioridad sobre incluir.
...
<CollapserGroup>
  ...
  <Collapser id="tracer-sqlMetadataCommentsEnabled" title="sqlMetadataCommentsEnabled">
    ...
    Cuando se establece en `true`, el agente agrega un comentario de SQL que contiene el GUID de la entidad de servicio al inicio de cada declaración de SQL instrumentada antes de que la base de datos ejecute la declaración. El comentario tiene el siguiente formato:

    ```sql
    /*nr_service_guid="YOUR_ENTITY_GUID"*/
    ```

    Utilice esta opción para correlacionar los datos de APM con el [monitoreo del rendimiento de consultas](/docs/infrastructure/infrastructure-data/query-performance-monitoring). El agente cambia el texto de la sentencia que envía a la base de datos, de modo que la base de datos recibe el comentario completo.

    El agente también registra el texto comentado. El comentario aparece en las trazas de consulta lenta, en los segmentos de la traza de la transacción y en el atributo `db.statement` de los eventos de span. Con el valor predeterminado de [`recordSql`](#tracer-recordSql) de `obfuscated`, el agente reemplaza el GUID de la entidad entre comillas con `?`, por lo que estos registros muestran `/*nr_service_guid=?*/`. Establezca `recordSql` en `raw` para mantener el GUID de la entidad en los datos de este agente.

    Esta opción está disponible a partir de la versión 10.52.0 del agente de .NET. También puede establecerla con la variable de entorno `NEW_RELIC_TRANSACTION_TRACER_SQL_METADATA_COMMENTS_ENABLED`.

    <Callout variant="important">
      Esta opción cambia el texto de cada sentencia SQL instrumentada. Una base de datos que almacena en caché los planes de consulta o las sentencias preparadas por texto de sentencia ve cada sentencia comentada como una nueva sentencia. Esto puede reducir la tasa de aciertos de caché y aumentar el trabajo de planificación de consultas en la base de datos. El efecto es mayor cuando más de un servicio consulta la misma base de datos, porque cada servicio agrega un GUID de entidad diferente a la misma sentencia.

      Habilite esta opción en un entorno de prueba primero y mida el efecto en su base de datos antes de habilitarla en producción.
    </Callout>
  </Collapser>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Como su nombre lo indica, este muestreador muestreará cada traza. Tenga en cuenta que habilitar este muestreador puede tener un impacto en el rendimiento de la aplicación y provocará un aumento en la ingesta de datos.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Lo opuesto de Always On; este muestreador no muestrea ningún rastro. Útil en casos muy específicos.
      </td>
    </tr>
  </tbody>
</table>
...
El elemento `<traceIdRatioBased>` tiene un atributo obligatorio `ratio`, que es un valor entre `0.0` y `1.0` que especifica el porcentaje de trazas que se deben muestrear.
...
### Kafka métrica [#kafka\_metrics][#kafka_metrics]

El elemento `kafka` es un elemento secundario del elemento `configuration`. Utilice `kafka` para configurar las métricas de Kafka que el agente recopila de los productores y consumidores de Kafka de su aplicación.

Compatible con el agente .NET v10.55.0 y superiores.

El elemento `kafka` admite el subelemento `metrics`. Todos los atributos de `metrics` son opcionales.
...
También puede configurar estos atributos con variables de entorno:
...
El subelemento `metrics` admite los siguientes atributos:
...
<CollapserGroup>
  <Collapser id="kafka-metrics-debug-enabled" title="debugEnabled">
    ...
    Habilite o deshabilite la recopilación de las métricas internas de Kafka que el agente genera a partir de las estadísticas que Kafka le reporta.

    Este atributo tiene prioridad sobre la configuración de la aplicación `NewRelic.KafkaInternalMetricsEnabled`. Si no configura `debugEnabled`, el agente aún respeta esa configuración de la aplicación.

    O emplee la variable de entorno `NEW_RELIC_KAFKA_METRICS_DEBUG_ENABLED` .
  </Collapser>
  ...
  <Collapser id="kafka-metrics-interval" title="intervalo">
    ...
    Este atributo establece el intervalo, en segundos, en el que Kafka reporta estadísticas al agente.

    El valor mínimo es de 5 segundos. El agente aumenta un valor menor a 5.

    Si no configura este atributo, el agente deriva el intervalo del ciclo de recolección de métricas. Si configura un intervalo de estadísticas en el propio cliente de Kafka, ese valor tiene prioridad sobre este atributo.

    O emplee la variable de entorno `NEW_RELIC_KAFKA_METRICS_INTERVAL` .
  </Collapser>
  ...
  <Collapser id="kafka-metrics-cluster-metrics-enabled" title="clusterMetricsEnabled">
    ...
    Habilite o deshabilite las métricas de producción y consumo de Kafka por clúster. Establezca este atributo en `true` para que el agente registre las métricas del clúster.

    O emplee la variable de entorno `NEW_RELIC_KAFKA_METRICS_CLUSTER_METRICS_ENABLED` .
  </Collapser>
</CollapserGroup>
