...
Los siguientes tipos de funciones Lambda de AWS se detectan e instrumentan automáticamente sin ninguna configuración adicional (a partir de la versión 10.29.0 del agente):

* `Amazon.Lambda.AspNetCoreServer.APIGatewayProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.APIGatewayHttpApiV2ProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.ApplicationLoadBalancerFunction`

Otros tipos de controladores (como SQS, SNS, Kinesis, S3, desencadenadores de DynamoDB y controladores personalizados) también son instrumentados cuando se establece la variable de entorno `NEW_RELIC_LAMBDA_HANDLER` en la ruta completa del controlador (por ejemplo, `MyAssembly::MyNamespace.MyClass::MyMethod`). Esto incluye cualquier método manejador que acepte un parámetro `ILambdaContext`.
...
* El rastreo distribuido entrante de SQS requiere que el productor ascendente inyecte los encabezados de rastreo de W3C (`traceparent`, `tracestate`) como atributos de mensaje de SQS. El agente los lee automáticamente cuando están presentes.
