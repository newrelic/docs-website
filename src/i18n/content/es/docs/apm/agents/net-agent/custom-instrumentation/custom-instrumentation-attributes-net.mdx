...
Los métodos decorados con el atributo `[Transaction]` solo crearán una nueva transacción cuando ya no exista ninguna. Cuando se llama a un método decorado con `[Transaction]` desde <DoNotTranslate>**within**</DoNotTranslate> una transacción iniciada anteriormente, se tratará como el atributo `[Trace]` y proporcionará más información sobre la transacción existente.
...
<Callout variant="important">
  Ejecutar su aplicación desde Visual Studio en modo <DoNotTranslate>**debug**</DoNotTranslate> puede evitar que algunos métodos aparezcan en New Relic traza. Para garantizar que los métodos aparezcan en New Relic, ejecute la aplicación en modo de lanzamiento a través de la línea de comando.
</Callout>
