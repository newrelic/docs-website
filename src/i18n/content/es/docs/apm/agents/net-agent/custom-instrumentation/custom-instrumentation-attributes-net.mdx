...
* Evite el instrumentado de métodos de nivel superior como `Main()`, ya que estos métodos no concluyen hasta que la aplicación finaliza, lo que puede impedir que se envíen datos a New Relic.

## Transacción llamada dentro de transacción [#tx-vs-trace]

Los métodos decorados con el atributo `[Transaction]` solo crearán una nueva transacción cuando no exista una. Cuando un método decorado con `[Transaction]` se llama desde <DNT>**within**</DNT> una transacción iniciada previamente, se tratará como el atributo `[Trace]` en su lugar, y proporcionará más información sobre la transacción existente.

<CollapserGroup>
  <Collapser
    id="example-tx-vs-trace"
    title={<>Ejemplo: llamar <InlineCode>
      Transaction
    </InlineCode> en una transacción ya iniciada</>
    }
  >
...
## Ejemplo: instrumentar cuatro métodos [#example-app]
...
* [Solucionar problemas de instrumentación personalizada basada en atributos](https://support.newrelic.com/s/hubtopic/aAX8W0000008a5g/relic-solution-troubleshooting-attributebased-custom-instrumentation-issues)
* [Construya fábricas de rastreadores de instrumentación personalizada a partir del agente .NET archivo de registro](https://knowledge.newrelic.com/s/article/Building-Custom-Instrumentation-Tracer-Factories-From-dot-NET-Agent)
