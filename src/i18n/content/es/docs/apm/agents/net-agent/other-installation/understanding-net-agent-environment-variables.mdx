...
## Configure las variables de entorno para las aplicaciones alojadas en IIS [#iis]

Si su aplicación está alojada en IIS, puede configurar las variables de entorno por grupo de aplicaciones editando directamente el archivo `ApplicationHost.config` de IIS, en lugar de (o además de) configurarlas como variables de entorno de todo el sistema. Esto es útil cuando desea habilitar o configurar el agente de .NET solo para grupos de aplicaciones específicos, en lugar de para todos los procesos de la máquina.

De forma predeterminada, encontrará `ApplicationHost.config` en `C:\Windows\System32\inetsrv\config\ApplicationHost.config`. Dentro de la sección `<applicationPools>`, cada elemento `<add>` que representa un grupo de aplicaciones puede contener un elemento secundario `<environmentVariables>`, que a su vez contiene entradas `<add name="..." value="..." />` para cada variable de entorno que desee configurar para ese grupo de aplicaciones.

Por ejemplo, para habilitar el agente de .NET y establecer algunas opciones de configuración comunes para un grupo de aplicaciones llamado `TestApplication`:
...
También puede usar `applicationPoolDefaults` para establecer variables de entorno que se apliquen a todos los grupos de aplicaciones de forma predeterminada, de la misma manera que lo harían las variables de entorno de todo el sistema, a menos que la configuración de `environmentVariables` propia de un grupo específico las anule.

<Callout title="Nota">
  Editar `ApplicationHost.config` directamente requiere privilegios de administrador, y debe reciclar un grupo de aplicaciones (o reiniciar IIS) para que los cambios surtan efecto. Haga siempre una copia de seguridad del archivo antes de editarlo, ya que una configuración incorrecta puede impedir que IIS inicie sitios o grupos de aplicaciones.
</Callout>

Para obtener más detalles sobre este elemento de configuración, consulte la documentación de Microsoft sobre [applicationPoolDefaults environmentVariables](https://learn.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/applicationpooldefaults/environmentvariables/).