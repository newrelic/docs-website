...
Debido a la naturaleza de <DNT>C SDK</DNT>, tiene control directo sobre qué datos se informan a New Relic. Para garantizar la privacidad de los datos y limitar los tipos de información que recibe New Relic, no se capturan datos de los clientes, excepto los que proporcione en las llamadas API. Además, <DNT>C SDK</DNT> informa todos los datos a New Relic a través de HTTPS.
...
Por defecto, así es como el <DNT>C SDK</DNT> maneja los siguientes datos potencialmente confidenciales. Para obtener más información, incluidos los valores de código y ejemplos, consulte la [documentación del SDK de C sobre estructuras de datos en GitHub](https://newrelic.github.io/c-sdk/annotated.html).
...
<table>
  <thead>
    <tr>
      ...
      <th>
        <DNT>C SDK</DNT> seguridad de los datos
      </th>
    </tr>
  </thead>
  ...
  <tbody>
    <tr id="name">
      ...
      <td>
        ...
        * Nombres de instancia: por defecto, la configuración devuelta por `newrelic_new_config()` habilita segmentos de almacenamiento de datos con `instance_reporting` establecido en `true`.
        * Nombres de bases de datos: por defecto, la configuración devuelta por `newrelic_new_config()` habilita segmentos de almacenamiento de datos con `database_name_reporting` establecido en `true`.
      </td>
    </tr>
