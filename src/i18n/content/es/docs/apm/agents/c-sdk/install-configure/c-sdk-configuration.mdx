...
Todos los demás valores de configuración son opcionales y, por lo general, no es necesario cambiar la configuración predeterminada. Sin embargo, cuando sea necesario, puede ajustar la configuración de <DNT>C SDK</DNT> de New Relic.

Este documento es una referencia rápida para usar algunas opciones básicas de configuración con la API de <DNT>C SDK</DNT>. Para obtener información detallada sobre cómo cambiar los ajustes de configuración, incluidos los valores de código y los ejemplos, consulte el [GUIDE.md de configuración de<DNT>C SDK</DNT> en GitHub](https://github.com/newrelic/c-sdk/blob/master/GUIDE.md#configuration).
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Para configurar o cambiar el nombre de la aplicación, configurar <InlinePopover type="licenseKey" />, nivel de logging, seguimiento de transacciones, seguimiento de almacenamiento de datos, etc., utilice:
...
Si cambia el nombre de su aplicación en sus ajustes de configuración, esto hará que la misma aplicación aparezca en la UI con un nuevo nombre. Cualquier dato histórico (basado en el programa [de retención de datos](/docs/apm/new-relic-apm/maintenance/apm-data-retention) ) solo existirá con el nombre anterior. (Para cambiar el nombre de su aplicación pero aún conservar los datos históricos, use la [configuración UI ](#change-ui)para cambiar el alias).
