...
Puede implementar trabajos en segundo plano personalizados para que aparezcan en la [página APM<DoNotTranslate>**Transactions**</DoNotTranslate> ](/docs/apm/applications-menu/monitoring/transactions-page)como <DoNotTranslate>**Non-web transactions**</DoNotTranslate>. Para monitor <DoNotTranslate>**Non-web transactions**</DoNotTranslate> mientras usa un framework no compatible, debe agregar instrumentación personalizada.
...
2. Utilice la directiva `add_transaction_tracer` <DoNotTranslate>**below**</DoNotTranslate> la definición del método

3. Agregue `:category => :task` para decirle al agente que esta traza es un <DoNotTranslate>**Non-web transaction**</DoNotTranslate>.
   ...
   Puede pasar una cadena a `:category`, pero los valores solo aparecerán en la página APM <DoNotTranslate>**Transactions**</DoNotTranslate> si la cadena comienza con `OtherTransaction/`.

## Monitor métodos de fondo personalizados [#custom_methods]

Usando la API del agente Ruby, puedes designar métodos específicos para trazar el <DoNotTranslate>**Non-web transactions**</DoNotTranslate>. Esto recopila trazas para trabajos de ejecución lenta y asocia errores capturados a la transacción.

Para instrumentar un método de clase, use la clase `singleton`.

Por ejemplo, un trabajo en segundo plano ejecuta periódicamente una tarea llamada `SalesOrganization#find_new_leads`.

1. Agregue el módulo `ControllerInstrumentation` <DoNotTranslate>**below**</DoNotTranslate> la definición del método.
...
<DoNotTranslate>**Note:**</DoNotTranslate> La mayoría de las opciones de configuración se pueden pasar al inicio manual.
...
La configuración de su <DoNotTranslate>**newrelic.yml**</DoNotTranslate> depende del contexto de la aplicación en segundo plano.
...
<CollapserGroup>
  ...
  >
    Si su aplicación en segundo plano no es una aplicación Rails que ya ejecuta el agente Ruby, copie su archivo <DoNotTranslate>**newrelic.yml**</DoNotTranslate> en el directorio donde lanzó el trabajo en segundo plano o en el subdirectorio <DoNotTranslate>**config**</DoNotTranslate> . Asegúrate de que incluya tu <InlinePopover type="licenseKey"/>.
  ...
  >
    Si su trabajo en segundo plano se ejecuta en el contexto de una aplicación web existente que ya está monitoreada con New Relic, el agente Ruby seleccionará automáticamente su archivo <DoNotTranslate>**newrelic.yml**</DoNotTranslate> existente. Los trabajos en segundo plano que inician el entorno Rails de su aplicación utilizarán la variable de entorno `RAILS_ENV` para determinar qué sección del archivo <DoNotTranslate>**newrelic.yml**</DoNotTranslate> leer.
  </Collapser>
</CollapserGroup>
...
1. El código de trabajador requiere comenzar <DoNotTranslate>**before**</DoNotTranslate> `newrelic_rpm` .
