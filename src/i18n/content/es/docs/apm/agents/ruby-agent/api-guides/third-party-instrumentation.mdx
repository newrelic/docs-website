...
Este documento detalla cómo instrumentar gemas de terceros con el agente de Ruby, así como algunas mejores prácticas para interactuar con el agente. Esto es útil si utiliza una gema que el agente <DNT>Ruby</DNT> no instrumenta por defecto, o si es un autor de gemas que desea agregar instrumentación para su biblioteca.

<Callout variant="tip">
  New Relic acepta contribuciones de la comunidad para el agente <DNT>Ruby</DNT>. Si tiene una gema que desea que sea instrumentada o una característica para proponer, en el respositorio `newrelic/newrelic-ruby-agent` en GitHub, comparta sus ideas abriendo un [issue](https://github.com/newrelic/newrelic-ruby-agent/issues/new/choose) o enviando un [pull request](https://github.com/newrelic/newrelic-ruby-agent).
</Callout>

## Encontrar extensiones de terceros

Cualquiera puede escribir una gema que se base en el agente <DNT>Ruby</DNT>. Ejemplos antiguos de esto están disponibles en un repositorio archivado llamado [extends\_newrelic\_rpm](https://github.com/newrelic/extends_newrelic_rpm) para rastrear estas extensiones y proporcionar enlaces a otras gemas que desarrollan el agente <DNT>Ruby</DNT>.

Estas extensiones **no** cuentan con el soporte de New Relic. New Relic recopiló estos enlaces como un servicio para nuestros clientes. Los problemas con esas gemas deben informarse a los proyectos respectivos en GitHub.

## Extensiones como joyas

Si elige crear una extensión de terceros, recomendamos una gema por biblioteca instrumentada. Por ejemplo, `newrelic-redis` proporciona instrumentación para la gema `redis` (que también existe automáticamente dentro de nuestro agente).
...
<CollapserGroup>
  ...
  <Collapser id="perform_action_with_newrelic_trace" title="perform_action_with_newrelic_trace">
    ...
    Consulte la [documentación completa de perform\_action\_with\_newrelic\_trace](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent/Instrumentation/ControllerInstrumentation#perform_action_with_newrelic_trace-instance_method) para obtener más información sobre los parámetros y el uso.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="multiverse" title="Multiverso: prueba con múltiples versiones de gemas">
