...
<CollapserGroup>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Si `true`, el agente usará el nombre de la tabla del modelo de Active Record en lugar del nombre de la clase al nombrar las métricas, los spans y los segmentos de la traza de la transacción de Active Record. Esto puede reducir la cardinalidad cuando varios modelos comparten una tabla de base de datos. Por defecto es `false`.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    Define una ruta al archivo de log del agente, excluyendo el nombre del archivo. Si desea enviar los logs de su agente a la salida estándar, establezca esto en STDOUT.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    Especifica dónde truncar el rastreo inverso cuando el número de marcos supera `error_collector.max_backtrace_frames`. Las opciones son `top` (eliminar marcos del inicio), `middle` (eliminar marcos del medio, conservando el inicio y el final) o `end` (eliminar marcos del final).
  </Collapser>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="instrumentation.active_support_notifications.active_support_events">
    ...
    Una matriz de lista de permitidos de eventos de notificaciones de Active Support específicos de la propia biblioteca Active Support a la que el agente debe suscribirse. Los eventos específicos de la biblioteca Active Support se centran principalmente en el almacenamiento en caché. El agente ignorará cualquier nombre de evento que no esté incluido en esta lista. Proporcione nombres de eventos completos como &apos;cache\_fetch\_hit.active\_support&apos;. No proporcione asteriscos ni patrones de expresiones regulares, y no escape ningún carácter con barras invertidas. Para obtener una lista completa de todos los nombres de eventos de Active Support posibles, consulte la [lista de nombres de almacenamiento en caché](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) y la [lista de nombres de mensajes](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) de la documentación oficial de Rails.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Una matriz de nombres de Rails.event para capturar como log de evento estructurado. Por ejemplo,

    * user.signup
    * payment.processed
    * order.created Deje en blanco para capturar todas las notificaciones de Rails.event. Los eventos se registran con el nivel UNKNOWN (asegurando que nunca se filtren) a menos que se sobrescriba mediante la clave :level en la carga del evento.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Una opción de configuración global para deshabilitar todas las señales de OpenTelemetry enviadas a través de New Relic. Si es falso, no se enviarán señales de OpenTelemetry a New Relic. Si es verdadero, la opción de configuración habilitada específica de la señal (p. ej. opentelemetry.traces.enabled) determina si la telemetría de ese tipo de señal se reportará a New Relic.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Si es `true`, el agente informa el ID de la instancia de GCP como el nombre de host cuando se ejecuta en Google Cloud Run.
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    Si `true`, el agente antepone el valor `K_REVISION` al ID de la instancia de GCP para formar el nombre de host (`{K_REVISION}-{instance id}`) en Google Cloud Run. No tiene efecto a menos que `utilization.gcp_cloud_run.use_instance_as_host` también sea `true`.
  </Collapser>
