...
<CollapserGroup>
  ...
  <Collapser id="log_level" title="log_level">
    ...
    Establece el nivel de detalle del mensaje de log. Los posibles niveles de logs, con mayor detalle, son: `fatal`, `error`, `warn`, `info` o `debug`.
  </Collapser>
  ...
  <Collapser id="active_support_custom_events_names" title="active_support_custom_events_names">
    ...
    Una matriz de nombres de eventos personalizados de ActiveSupport a los que suscribirse e instrumentar. Por ejemplo: \[one.custom.event, another.event, a.third.event]
  </Collapser>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Si es `true`, el agente usará el nombre de la tabla del modelo de ActiveRecord en lugar del nombre de la clase al nombrar las métricas, los spans y los segmentos de la traza de la transacción de ActiveRecord. Esto puede reducir la cardinalidad cuando varios modelos comparten una tabla de base de datos. Por defecto es `false`.
  </Collapser>
  ...
  <Collapser id="capture_memcache_keys" title="capture_memcache_keys">
    ...
    Si es `true`, el agente captura las claves de memcache en las trazas de la transacción.
  </Collapser>
  ...
  <Collapser id="config_path" title="config_path">
    ...
    Ruta a `newrelic.yml`. Si no está definido, el agente comprueba los siguientes directorios (en orden): `config/newrelic.yml` -&gt; `newrelic.yml` -&gt; `$HOME/.newrelic/newrelic.yml` -&gt; `$HOME/newrelic.yml`
  </Collapser>
  ...
  <Collapser id="exclude_newrelic_header" title="exclude_newrelic_header">
    ...
    Si es `true`, suprime el encabezado de rastreo distribuido `newrelic` en las requests salientes.
  </Collapser>
  ...
  <Collapser id="force_install_exit_handler" title="force_install_exit_handler">
    ...
    Si es `true`, fuerza al agente a instalar su manejador de salida (que envía todos los datos en caché al recolector antes de apagarse), incluso en los casos en que el agente normalmente lo omitiría, como cuando detecta que Sinatra se ejecuta como un servicio integrado dentro de otro framework. Sinatra ejecuta toda la aplicación dentro de su propio bloque `at_exit`, por lo que el agente omite su propio manejador `at_exit` de forma predeterminada para evitar conflictos. Esta configuración anula eso. Nota: `send_data_on_exit` también debe ser `true` para que esta configuración surta efecto.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    Define una ruta al archivo de log del agente, excluyendo el nombre del archivo. Si desea enviar los logs del agente a la salida estándar, establezca esto en `STDOUT`.
  </Collapser>
  ...
  <Collapser id="thread_local_tracer_state" title="thread_local_tracer_state">
    ...
    Si es `true`, el almacenamiento de estado del rastreador es local para el subproceso; de lo contrario, es local para la fibra.
  </Collapser>
  ...
  <Collapser id="simplify_action_cable_broadcast_metrics" title="simplify_action_cable_broadcast_metrics">
    ...
    Si `true`, las métricas para las llamadas de transmisión de Action Cable no incluirán el valor `broadcasting` para una transmisión dada. Por defecto es `false`. El valor predeterminado puede cambiar en una futura versión mayor.
  </Collapser>
  ...
  <Collapser id="ignored_middleware_classes" title="ignored_middleware_classes">
    ...
    Una lista de nombres de clases o módulos de middleware que el agente debe ignorar. Ejemplo: \[&quot;Rack::Cors&quot;, &quot;ActionDispatch::Reloader&quot;]
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    ...
    Si es `true`, una vez que se alcanza [`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments), el agente también dejará de registrar el tiempo exclusivo para cualquier segmento creado posteriormente en esa transacción. Esto puede reducir el uso de memoria para transacciones de muy larga duración con muchos segmentos, a costa de datos de tiempo menos precisos para los segmentos en la transacción si se alcanza el límite de segmentos.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-expected_status_codes" title="error_collector.expected_status_codes">
    ...
    Una lista separada por comas de códigos de estado HTTP y/o rangos de códigos de estado (por ejemplo, `404, 429, 500-599`). Los errores asociados con un código de estado coincidente, si el error tiene uno, se tratarán como se espera.
  </Collapser>
  ...
  <Collapser id="error_collector-ignore_status_codes" title="error_collector.ignore_status_codes">
    ...
    Una lista separada por comas de códigos de estado HTTP y/o rangos de códigos de estado (por ejemplo, `404, 429, 500-599`). Los errores asociados con un código de estado coincidente, si el error tiene uno, se ignorarán.
  </Collapser>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    Define el número máximo de marcos en un rastreo inverso de errores. Los rastreos inversos que superan esta cantidad se truncan en el medio de forma predeterminada, preservando el principio y el final del rastreo del stack. Consulte [`error_collector.backtrace_truncate_location`](#error_collector-backtrace_truncate_location) para personalizar la ubicación de truncamiento.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    Especifica dónde truncar el rastreo inverso cuando el número de marcos supera [`error_collector.max_backtrace_frames`](#error_collector-max_backtrace_frames). Las opciones son `top` (eliminar marcos del inicio), `middle` (eliminar marcos del medio, conservando el inicio y el final) o `end` (eliminar marcos del final).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser_monitoring-version" title="browser_monitoring.version">
    ...
    Fije la versión del cargador del agente del browser que inyecta New Relic, como `1.317.0`. Cuando no está configurado, New Relic inyecta la última versión disponible. Consulte la [política de EOL del agente del browser](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/) para saber qué versiones están disponibles y son compatibles actualmente.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="application_logging-forwarding-log_level" title="application_logging.forwarding.log_level">
    ...
    Establece el nivel mínimo que debe tener un log de evento para ser reenviado a New Relic. Los logs en el nivel configurado y cualquier gravedad mayor se reenvían, por ejemplo, `debug` reenvía todos los logs de eventos, mientras que `error` reenvía solo los eventos `error`, `fatal` y `unknown`. Valores válidos, de menor a mayor gravedad: `debug`, `info`, `warn`, `error`, `fatal`, `unknown` (no distingue entre mayúsculas y minúsculas).

    Este orden se basa en los valores enteros de las constantes `Logger::Severity` de Ruby (consulte [https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb](https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb)).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="ai_monitoring-record_content-enabled" title="ai_monitoring.record_content.enabled">
    ...
    Si es `false`, la instrumentación de LLM (OpenAI solo por ahora) no capturará el atributo `content` en los eventos LlmChatCompletionMessage ni el atributo `input` en los eventos LlmEmbedding. Esta es una configuración de seguridad opcional para evitar el registro de datos confidenciales enviados a los LLM y recibidos de estos.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="audit_log-endpoints" title="audit_log.endpoints">
    ...
    Lista de extremos permitidos para incluir en el log de auditoría.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="cloud-aws-account_id" title="AWS">
    ...
    El ID de cuenta de AWS asociado con esta aplicación.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="custom_insights_events-max_samples_stored" title="custom_insights_events.max_samples_stored">
    ...
    Especifique un número máximo de eventos personalizados para almacenar en la memoria a la vez. Al configurar el agente para el [monitoreo de IA](/docs/ai-monitoring/intro-to-ai-monitoring), establézcalo en el valor máximo `100000`. Esto garantiza que el agente capture la cantidad máxima de eventos de LLM.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="datastore_tracer-database_name_reporting-enabled" title="datastore_tracer.database_name_reporting.enabled">
    ...
    Si es `false`, el agente no agregará el parámetro `database_name` a la transacción ni a las trazas de SQL lentas.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="disable_active_job_step_names" title="disable_active_job_step_names">
    ...
    Si `true`, los nombres de los pasos de Rails Active Job Continuations no se incluirán en los nombres de las métricas. Esto reduce la cardinalidad de la métrica, pero también reduce la visibilidad del rendimiento de cada paso individual. Solo establezca esto en `true` si experimenta problemas de cardinalidad de métricas debido a trabajos con muchos pasos o pasos con nombres dinámicos.
  </Collapser>
  ...
  <Collapser id="disable_active_record_notifications" title="disable_active_record_notifications">
    ...
    Si es `true`, deshabilita la instrumentación para Active Record 4+.
  </Collapser>
  ...
  <Collapser id="disable_gc_profiler" title="disable_gc_profiler">
    ...
    Si es `true`, deshabilita el uso de `GC::Profiler` para medir el tiempo dedicado a la recolección de basura.
  </Collapser>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    Si `true`, deshabilita el middleware del agente para Sinatra. Este middleware es responsable de la compatibilidad con funciones avanzadas como [rastreo distribuido](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing), [medición del tiempo de carga de la página](/docs/browser/new-relic-browser/getting-started/new-relic-browser) y [recopilación de errores](/docs/apm/applications-menu/events/view-apm-error-analytics).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    Esta configuración controla el comportamiento del muestreo de transacciones para transacciones sin un elemento principal remoto (trazas que se originan dentro de esta instancia del agente de New Relic). Los valores disponibles son `adaptive` (el valor predeterminado), `always_on`, `always_off` y `trace_id_ratio_based`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Esta configuración controla el comportamiento del muestreo de transacciones cuando se muestrea un padre remoto. Los valores disponibles son `adaptive` (el valor predeterminado), `always_on`, `always_off` y `trace_id_ratio_based`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Esta configuración controla el comportamiento del muestreo de transacciones cuando un padre remoto no se muestrea. Los valores disponibles son `adaptive` (el valor predeterminado), `always_on`, `always_off` y `trace_id_ratio_based`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="instrumentation.active_support_notifications.active_support_events">
    ...
    Una lista de permitidos de nombres de eventos de notificación de ActiveSupport (principalmente eventos de almacenamiento en caché y eventos de serializador de mensajes) a los que se suscribe el agente. Se ignora cualquier nombre de evento que no esté incluido aquí. Proporcione solo nombres de eventos completos, como `cache_fetch_hit.active_support`. No proporcione asteriscos ni patrones de expresiones regulares, y no escape ningún carácter con barras invertidas.

    Para obtener la lista completa de posibles nombres de eventos, consulte la lista de [nombres de almacenamiento en caché](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) y [nombres de mensajes](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) de la documentación de Rails.
  </Collapser>
  ...
  <Collapser id="instrumentation-semantic_logger" title="instrumentation.semantic_logger">
    ...
    Controla la instrumentación automática de la biblioteca semantic\_logger al inicio. Puede ser uno de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-parallel" title="instrumentation.parallel">
    ...
    Controla la autoinstrumentación de la biblioteca paralela al inicio. Puede ser uno de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-grpc-host_denylist" title="instrumentation.grpc.host_denylist">
    ...
    Una lista separada por comas de patrones de expresiones regulares que coinciden con los nombres de host de gRPC cuyo tráfico New Relic debe ignorar. La instrumentación del cliente gRPC de New Relic omite el rastreo de las llamadas realizadas a un host que coincide con cualquiera de estos patrones, y su instrumentación del servidor gRPC omite el rastreo de las requests recibidas por un servidor que se ejecuta en un host coincidente. Por ejemplo, `"private.com$,exception.*" Empty by default, meaning no traffic is ignored.`.
  </Collapser>
  ...
  <Collapser id="instrumentation-logging" title="instrumentation.logging">
    ...
    Controla la autoinstrumentación de la biblioteca de logging al inicio. Puede ser uno de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger" title="instrumentation.rails_event_logger">
    ...
    Controla la instrumentación de Rails.event como logs estructurados. Puede ser uno de: `enabled`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Una matriz de nombres de `Rails.event` para capturar como log de evento estructurado. Por ejemplo: \[usuario.signup, payment.processed, order.created]. Deje en blanco para capturar toda notificación de `Rails.event`. Cada evento se registra con el nivel `UNKNOWN` (lo que garantiza que nunca se filtre) a menos que se anule mediante una clave `:level` en la carga del evento.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Una opción de configuración global para deshabilitar todas las señales de OpenTelemetry enviadas a través de New Relic. Si es `false`, no se enviarán señales de OpenTelemetry a New Relic. Si es `true`, la opción de configuración habilitada específica de la señal (p. ej. opentelemetry.traces.enabled) determina si la telemetría de ese tipo de señal se reportará a New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    Permite la creación de segmentos de Transaction Trace y métricas de timeslice a partir de Spans de OpenTelemetry. Esto ayudará a impulsar la experiencia de la interfaz de usuario de New Relic para los spans de OpenTelemetry.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-include" title="opentelemetry.traces.include">
    ...
    Una lista de rastreadores de OpenTelemetry delimitada por comas, representada como una cadena (p. ej. &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;), que **tendrán** sus trazas enviadas a New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-exclude" title="opentelemetry.traces.exclude">
    ...
    Una lista de rastreadores de OpenTelemetry delimitada por comas, representada como una cadena (p. ej. &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;), cuyas trazas **no** se enviarán a New Relic.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="puma-sample_rate" title="puma.sample_rate">
    ...
    Número de segundos entre muestras de estadísticas del servidor Puma tomadas en el proceso maestro de Puma.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rake-tasks" title="rake.tasks">
    ...
    Especifique una matriz de tareas de Rake para instrumentar automáticamente. Esta opción de configuración convierte la matriz en una lista de expresiones regulares. No se instrumentará ninguna tarea de rake a menos que se agregue a esta lista. Si desea permitir todas las tareas de forma predeterminada, use `rake.tasks: [.+]`. Para obtener más información, visite la [documentación de instrumentación de Rake de New Relic](/docs/apm/agents/ruby-agent/background-jobs/rake-instrumentation).
  </Collapser>
  ...
  <Collapser id="rake-connect_timeout" title="rake.connect_timeout">
    ...
    Define el número de segundos que se debe esperar para que se complete una conexión antes de ejecutar una tarea de Rake.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rules-ignore_url_regexes" title="rules.ignore_url_regexes">
    ...
    Defina las transacciones que desea que el agente ignore especificando una lista de patrones que coincidan con los URI a excluir. Para obtener más detalles, consulte [la documentación sobre cómo ignorar transacciones específicas](/docs/agents/ruby-agent/api-guides/ignoring-specific-transactions/#config-ignoring).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="security-agent-enabled" title="security.agent.enabled">
    ...
    Si es `true`, se carga el agente de seguridad (se realiza un “require” de Ruby).
  </Collapser>
  ...
  <Collapser id="security-enabled" title="security.enabled">
    ...
    Si es `true`, se inicia el agente de seguridad (el agente se ejecuta en su bucle de eventos).
  </Collapser>
  ...
  <Collapser id="security-validator_service_url" title="security.validator_service_url">
    ...
    Define la URL del extremo para publicar datos relacionados con la seguridad.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-insecure_settings" title="security.exclude_from_iast_scan.iast_detection_category.insecure_settings">
    ...
    Si es `true`, deshabilita la detección de configuraciones inseguras de baja gravedad. Por ejemplo, hash, criptografía, cookies, generadores aleatorios y límites de confianza.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-nosql_injection" title="security.exclude_from_iast_scan.iast_detection_category.nosql_injection">
    ...
    Si es `true`, deshabilita la detección de inyección NoSQL en los escaneos IAST.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-javascript_injection" title="security.exclude_from_iast_scan.iast_detection_category.javascript_injection">
    ...
    Si es `true`, deshabilita la detección de inyección de JavaScript en los escaneos de IAST.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-xpath_injection" title="security.exclude_from_iast_scan.iast_detection_category.xpath_injection">
    ...
    Si es `true`, deshabilita la detección de inyección de XPath en los escaneos de IAST.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-ssrf" title="security.exclude_from_iast_scan.iast_detection_category.ssrf">
    ...
    Si es `true`, deshabilita la detección de falsificación de solicitudes del lado del servidor (SSRF) en los escaneos de IAST.
  </Collapser>
  ...
  <Collapser id="security-scan_schedule-duration" title="security.scan_schedule.duration">
    ...
    Especifica la duración (en minutos) durante la cual se realiza el escaneo de IAST.
  </Collapser>
  ...
  <Collapser id="security-scan_controllers-iast_scan_request_rate_limit" title="security.scan_controllers.iast_scan_request_rate_limit">
    ...
    Establece el número máximo de requests HTTP permitidas por minuto para el escaneo de IAST. Debe ser un número entero entre 12 y 3600 (por defecto).
  </Collapser>
  ...
  <Collapser id="security-iast_test_identifier" title="security.iast_test_identifier">
    ...
    Un identificador de prueba único al ejecutar IAST en un entorno de CI/CD para diferenciar entre diferentes ejecuciones de prueba. Por ejemplo, un número de compilación.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="sidekiq-args-include" title="sidekiq.args.include">
    ...
    Una matriz de cadenas que servirá colectivamente como una lista de permitidos para filtrar qué argumentos de trabajos de Sidekiq se informan a New Relic. Para capturar cualquier argumento de Sidekiq, `job.sidekiq.args.*` también debe agregarse a la opción de configuración `attributes.include` separada. Cada cadena en esta matriz se convertirá en una expresión regular a través de `Regexp.new` para permitir coincidencias avanzadas. Para los hashes de argumentos de trabajos, si coincide una clave o un valor, se incluirá el par. Se incluirán todos los elementos de la matriz de argumentos de trabajos y los escalares de argumentos de trabajos coincidentes.
  </Collapser>
  ...
  <Collapser id="sidekiq-args-exclude" title="sidekiq.args.exclude">
    ...
    Una matriz de cadenas que servirá colectivamente como una lista de denegación para filtrar qué argumentos de trabajo de Sidekiq se reportan a New Relic. Para capturar cualquier argumento de Sidekiq, `job.sidekiq.args.*` también debe agregarse a la opción de configuración `attributes.include` separada. Cada cadena en esta matriz se convertirá en una expresión regular a través de `Regexp.new` para permitir coincidencias avanzadas. Para los hashes de argumentos de trabajo, si una clave o un valor coincide, el par se excluirá. Todos los elementos de la matriz de argumentos de trabajo y los escalares de argumentos de trabajo coincidentes se excluirán.
  </Collapser>
  ...
  <Collapser id="sidekiq-separate_transactions" title="sidekiq.separate_transactions">
    ...
    Si `true`, el agente inicia una nueva transacción cuando se ejecuta un trabajo de Sidekiq durante una transacción web. Esto evita que el tiempo de ejecución del trabajo de Sidekiq se incluya en las métricas de transacciones web.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="slow_sql-use_longer_sql_id" title="slow_sql.use_longer_sql_id">
    ...
    Si es `true`, el agente genera un `sql_id` más largo para trazas de SQL lentas. `sql_id` se usa para la agregación de consultas similares.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="span_events-max_samples_stored" title="span_events.max_samples_stored">
    ...
    Define el número máximo de eventos de span reportados desde una sola recolección. Cualquier número entero entre `1` y `10000` es válido. Al configurar el agente para el [monitoreo de IA](/docs/ai-monitoring/intro-to-ai-monitoring), establézcalo en el valor máximo `10000`. Esto garantiza que el agente capture la cantidad máxima de rastreo distribuido.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="strip_exception_messages-enabled" title="strip_exception_messages.enabled">
    ...
    Si es `true`, el agente elimina los mensajes de todas las excepciones, excepto las de la [lista de clases permitidas](#strip_exception_messages-allowed_classes). Habilitado automáticamente en el [modo de alta seguridad](/docs/accounts-partnerships/accounts/security/high-security).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Si `true`, el agente reporta el ID de la instancia de GCP como el nombre de host cuando se ejecuta en Google Cloud Run (servicios, grupos de trabajadores y trabajos).
  </Collapser>
  ...
  <Collapser id="utilization-detect_in_parallel" title="utilization.detect_in_parallel">
    ...
    Si `true`, el agente realizará la detección del entorno en paralelo para acelerar el tiempo de inicio del agente.
  </Collapser>
</CollapserGroup>

## Configuración Errors Inbox [#errors-inbox-configuration]

Configurar una de las siguientes variables de entorno le ayudará a identificar qué versiones de su software produjeron los errores.

* `NEW_RELIC_METADATA_SERVICE_VERSION` crea `tags.service.version` en los datos del evento que contienen la versión de su código que está desplegada, en muchos casos una versión semántica como `1.2.3`, pero no siempre.
* `NEW_RELIC_METADATA_RELEASE_TAG` crea `tags.releaseTag` en los datos del evento que contienen la etiqueta de lanzamiento (como `v0.1.209` o `release-209`).
* `NEW_RELIC_METADATA_COMMIT` crea `tags.commit` en los datos del evento que contienen el SHA del commit. Se puede utilizar el SHA completo o solo los primeros siete caracteres (por ejemplo, `734713b`).

[Errors Inbox](/docs/errors-inbox/version-tracking/) rastreará automáticamente qué versiones de su software están produciendo errores.