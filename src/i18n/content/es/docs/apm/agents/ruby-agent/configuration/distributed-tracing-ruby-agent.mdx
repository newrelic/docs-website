...
<Callout variant="tip">
  Cuando instala el agente New Relic Ruby, el rastreo distribuido estándar se activa de forma predeterminada. Si prefiere desactivarlo, consulte nuestra [guía de configuración](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration).
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser id="ruby-config" title="Configuración de seguimiento de Ruby Infinite">
...
<CollapserGroup>
  <Collapser id="calling-service" title="Instrumentar el servicio de llamadas">
    ...
    2. Invoque la llamada API del agente para generar un rastreo distribuido carga (consulte [Ruby API del agente](/docs/agents/ruby-agent/api-guides/guide-using-ruby-agent-api#distributed). <Callout variant="important">Para mantener el orden correcto de los tramos en una traza, cerciorar de <DNT>**generate the payload in the context of the span that sends it**</DNT>.</Callout>
