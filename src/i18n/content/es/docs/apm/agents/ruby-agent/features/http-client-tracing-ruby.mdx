...
El agente Ruby admite la versión <DoNotTranslate>**0.56.0 or higher.**</DoNotTranslate> de Excon. La instrumentación de Excon se basa en la capacidad de agregar un middleware Excon a la clave `:middlewares` de `Excon.defaults`, por lo que si su aplicación modifica `Excon.defaults` , debe asegurarse de conservar el valor de `:middlewares` tecla.

## Notas de acera [#curb]

El agente Ruby admite la versión <DoNotTranslate>**0.9.3 or higher.**</DoNotTranslate> de Curb. **No** se admite la instrumentación de Curb en JRuby.
