...
Ahora que ha confirmado que su sistema cumple con los requisitos:

1. **[Instale el agente Go](/docs/apm/agents/go-agent/installation/install-new-relic-go)** - Siga nuestra guía de instalación paso a paso
2. **[Configure el agente](/docs/apm/agents/go-agent/configuration)** - Personalice el comportamiento del agente para su entorno
3. **[Agregue instrumentación](/docs/apm/agents/go-agent/instrumentation)** - Monitoree operaciones específicas en su aplicación
4. **[Explore las funciones](/docs/apm/agents/go-agent/features)** - Descubra capacidades de monitoreo avanzadas

<Callout variant="tip">
  **¿Está usando un marco específico?** Muchas de las integraciones enumeradas anteriormente tienen instrucciones de configuración específicas. Consulte los [ejemplos de GitHub](https://github.com/newrelic/go-agent/tree/master/v3/integrations) para su marco para comenzar rápidamente.
</Callout>