...
<CollapserGroup>
  ...
  >
    ...
    2. Invoque la llamada API del agente para generar una carga útil distribuida de rastreo (consulte la [API del agente Python](/docs/agents/python-agent/python-agent-api/create_distributed_trace_payload)).
       ...
       <Callout variant="important">Para mantener el orden correcto de los tramos en una traza, asegúrese de <DoNotTranslate>**generate the payload in the context of the span that sends it**</DoNotTranslate>.</Callout>
