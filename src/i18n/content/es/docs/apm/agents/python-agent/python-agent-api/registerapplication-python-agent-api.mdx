...
Si el script en sí ejecuta el servidor WSGI, llame a `register_application` desde el hilo principal del programa en el archivo de script principal. Si usa Apache/mod_wsgi o uWSGI, realice la llamada desde el archivo de script WSGI y realice la llamada inmediatamente después `initialize`<DoNotTranslate>**.**</DoNotTranslate>
