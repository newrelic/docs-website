...
<CollapserGroup>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Este es el muestreador que se aplica cuando la traza tiene un padre remoto (es decir, la traza se originó en un servicio ascendente) que ha sido muestreado. Las opciones del muestreador son:

    * `adaptive` es el comportamiento predeterminado y delega la decisión de ejemplificación al algoritmo de ejemplificación adaptativa para determinar si se muestreará una traza.
    * `trace_id_ratio_based` delega la decisión de muestreo al algoritmo de muestreo basado en la proporción de ID de traza para determinar si se muestreará una traza. Si usa esta opción, la proporción es obligatoria, así que use [distributed\_tracing.sampler.remote\_parent\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio) en su lugar.
    * `always_on`: Siempre muestrea las trazas que tienen un padre remoto muestreado.
    * `always_off`: Nunca muestrea trazas que tienen un padre remoto muestreado.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio" title="distributed_tracing.sampler.remote_parent_sampled.trace_id_ratio_based.ratio">
    ...
    * Un número entre 0 (exclusivo) y 1 inclusivo.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Este es el muestreador que se aplica cuando una traza tiene un padre remoto (es decir, la traza se originó en un servicio ascendente) que no fue muestreado. Las opciones del muestreador son:

    * `adaptive` es el comportamiento predeterminado y delega la decisión de ejemplificación al algoritmo de ejemplificación adaptativa para determinar si se muestreará una traza.
    * `trace_id_ratio_based` delega la decisión de muestreo al algoritmo de muestreo basado en la proporción de ID de traza para determinar si se muestreará una traza. Si utiliza esta opción, la proporción es obligatoria, así que use [distributed\_tracing.sampler.remote\_parent\_not\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio) en su lugar.
  ...
  <Collapser id="distributed-tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    Este es el muestreador que se aplica al inicio de una traza. De forma predeterminada, el Muestreador adaptativo está configurado como el muestreador raíz. Las opciones del muestreador son:

    * `adaptive` es el comportamiento predeterminado y delega la decisión de ejemplificación al algoritmo de ejemplificación adaptativa para determinar si se muestreará una traza.
    * `trace_id_ratio_based` delega la decisión de muestreo al algoritmo de muestreo basado en la proporción de ID de traza para determinar si se muestreará una traza. Si utiliza esta opción, la proporción es obligatoria, así que use [distributed\_tracing.sampler.root.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-root-trace-id-ratio-based-ratio) en su lugar.
    * `always_on`: Siempre muestrea las transacciones que se originan en el servicio actual y no tienen un encabezado de rastreo distribuido presente para la llamada de entrada.
    * `always_off`: Nunca muestrea las transacciones que se originan en el servicio actual y no tienen un encabezado de rastreo distribuido presente para la llamada de entrada.
  </Collapser>
...
## Configuración del agente híbrido [#otel-settings]

<Callout variant="important">
  Requiere [la versión 11.5.0 o superior del agente de Python](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
</Callout>

Para obtener más información sobre las funcionalidades del agente híbrido, consulte [esta página](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support).

Las siguientes configuraciones están disponibles para la configuración del agente híbrido:
...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="opentelemetry.enabled">
    ...
    De v11.5.0 a v13.0.0: Si está habilitado, se utilizará cualquier API de OpenTelemetry y la mayoría de los [frameworks instrumentados de OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) que estén instalados. Desde la v13.0.0+: Si está habilitado, se utilizará cualquier API y framework de OpenTelemetry que sea [compatible con OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) pero no con New Relic.

    <Callout variant="important">
      Aunque la mayoría de las bibliotecas en los [frameworks instrumentados de OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) son compatibles, las bibliotecas de instrumentación de AWS y gRPC seguirán utilizando la instrumentación de New Relic. Las bibliotecas de ML/IA con instrumentación nativa están deshabilitadas explícitamente por ahora y tendrán soporte en una versión futura.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.enabled" title="opentelemetry.traces.enabled">
    ...
    De v11.5.0 a v13.0.0: Si se habilita, se utilizará cualquier API de span y traza de OpenTelemetry, independientemente de si existe una biblioteca de instrumentación de New Relic. Desde la v13.0.0+: Si está habilitado, se utilizará cualquier llamada manual que use la API de span de OpenTelemetry y los spans de frameworks que estén [admitidos por OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) pero no por New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.exclude" title="opentelemetry.traces.exclude">
    ...
    Permite a los usuarios excluir rastreador(es) para que no se reporten a New Relic. Consulte la [sección a continuación](#include-exclude-rules) para obtener más detalles.

    <Callout variant="important">
      Requiere [la versión 13.0.0 o superior del agente de Python](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.include" title="opentelemetry.traces.include">
    ...
    Permite a los usuarios incluir rastreador(es) para ser reportados a New Relic. Consulte la [sección a continuación](#include-exclude-rules) para obtener más detalles.
...
Reglas de inclusión y exclusión \[#include-exclude-rules]

New Relic sigue estas reglas al determinar qué rastreadores incluir o excluir para el monitoreo.

<CollapserGroup>
  <Collapser id="rule-precedence" title="Habilitado tiene prioridad sobre incluir y excluir.">
    `opentelemetry.traces.enabled` : este flag tiene prioridad sobre las configuraciones de inclusión (include) y exclusión (exclude).
  ...
  <Collapser id="rule-trace-included" title="La traza se incluye si el rastreo está habilitado.">
    Si el rastreo está habilitado, todos los rastreadores personalizados de OpenTelemetry y los rastreadores de las bibliotecas de OpenTelemetry no admitidos por New Relic se monitorean por defecto.
  ...
  <Collapser id="rule-exclude-wins" title="Excluir siempre reemplaza a incluir.">
    Si el mismo rastreador aparece en las listas de inclusión y exclusión, entonces el rastreador será excluido.
  ...
  <Collapser id="rule-tracers-case-sensitive" title="Los rastreadores distinguen entre mayúsculas y minúsculas.">
    Los rastreadores distinguen entre mayúsculas y minúsculas.
...
<CollapserGroup>
  ...
  <Collapser id="cross-application-tracer" title="cross_application_tracer.enabled">
    ...
    <Callout variant="caution">
      [El rastreo entre aplicaciones (CAT)](/docs/apm/transactions/cross-application-traces/introduction-cross-application-traces) se ha eliminado por completo en la versión 12.0.0.
    </Callout>
  </Collapser>
