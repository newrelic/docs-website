...
`external_trace` se utiliza para agregar más detalles a sus [trazas de la transacción](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#transaction-trace) en forma de segmentos adicionales. Cualquier llamada reportada con `external_trace` aparecerá en la pestaña de externos en APM. `external_trace` devuelve un [partial](https://docs.python.org/3.12/library/functools.html#functools.partial) de `ExternalTraceWrapper` que se puede usar como decorador para una función que llama a un servicio externo.
...
* <DNT>**The context manager**</DNT>: El formulario del administrador de contexto es `ExternalTrace`.
* <DNT>**The wrapper**</DNT>: El formulario contenedor es `ExternalTraceWrapper`. Se puede utilizar para devolver una función envuelta sin el uso de un decorador.
* <DNT>**The path-based wrapper**</DNT>: El formulario contenedor basado en ruta es `wrap_external_trace`. Esto aplica el `ExternalTraceWrapper` a un objeto determinado mediante parches de mono. Esto toma los mismos parámetros que el decorador más un parámetro `module` y `object_path` adicional.
...
### Parámetros para external\_trace [#main-parameters]
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Obligatorio. El nombre (o tipo) de la biblioteca externa en uso. Pasa ya sea una cadena que lo defina o una función que acepte los mismos parámetros que la función que se está envolviendo y devuelva la biblioteca.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Obligatorio. La URL que ha sido solicitada por la llamada a la biblioteca. Pasa una cadena que lo defina o una función que acepte los mismos parámetros que la función que se está envolviendo y devuelva la URL.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Opcional. El método que define el tipo de llamada que se realiza. Por lo general, son `get`, `post`, `put` o `delete`. Podría ser una función que acepta los mismos parámetros que la función que se está envolviendo y devuelve el método.
      </td>
    </tr>
  </tbody>
</table>

### parámetro para wrap\_external\_trace [#path-based-parameters]
...
### ejemplo de seguimiento\_externo [#decorator-example]
...
### ejemplo de wrap\_external\_trace [#path-based-example]
