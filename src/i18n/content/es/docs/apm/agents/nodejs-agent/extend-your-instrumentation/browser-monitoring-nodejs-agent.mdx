...
1. Al principio de la etiqueta `head` de su página html, inserte los resultados de `newrelic.getBrowserTimingHeader()` después de las metaetiquetas `CHARSET` .
   ...
   <DoNotTranslate>**Exception:**</DoNotTranslate> Para obtener la máxima compatibilidad con Internet Explorer, inserte los resultados de `newrelic.getBrowserTimingHeader()` <DoNotTranslate>**after**</DoNotTranslate> cualquier metaetiqueta `X-UA-COMPATIBLE HTTP-EQUIV` .
...
<CollapserGroup>
  ...
  >
    ...
    Usando <DoNotTranslate>**hapi**</DoNotTranslate>, en tu `app.js`:
