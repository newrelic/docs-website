...
* [Nuevo agente Node.js:](#new-agents) Pasos para el muestreo al tail-based y head-based para la instalación del nuevo agente
* [Agente Node.js anterior:](#older-agents) opciones de seguimiento si tiene un agente Node.js anterior
* [Instrumentación manual:](#manual-instrumentation) Consejos si la instrumentación automática no funciona

## Nuevo agente Node.js [#new-agents]
...
Si ya tiene algunos servicios instrumentados con este agente APM y desea incluirlos en rastreo distribuido, deberá habilitar rastreo distribuido manualmente para cada servicio. Consulte [Opciones para agentes Node.js anteriores](#older-agents).
...
<CollapserGroup>
  <Collapser
    id="node-config"
    title="Configuración de seguimiento infinito de Node.js"
  >
...
## Opciones para agentes Node.js más antiguos [#older-agents]
...
<CollapserGroup>
  <Collapser
    id="node-version"
    title="Compatibilidad del agente Node.js"
  >
    [Instale](/docs/agents/nodejs-agent/installation-configuration/install-nodejs-agent) o [actualice](/docs/agents/nodejs-agent/installation-configuration/upgrade-nodejs-agent) a la versión requerida del agente Node.js. Para obtener mejores resultados, actualice a la [última versión del agente Node.js.](/docs/release-notes/agent-release-notes/nodejs-release-notes)
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            Con W3C Trace Context: 6.4 o superior
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            7.3.0 (incluye W3C Trace Context)

            Entornos compatibles: Node.js versión 10.10.0 o superior
          </td>
        </tr>
      </tbody>
    </table>
...
### Configure su agente Node.js anterior [#configure-agents]
...
<CollapserGroup>
  ...
  >
    Aquí hay una descripción general de la configuración. Para obtener más ayuda con la configuración, consulte [Configuración del agente de Node.js.](/docs/apm/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#dt-main)
...
<CollapserGroup>
  ...
  >
    ...
    2. Invoque la llamada API del agente para generar una carga útil distribuida de rastreo (consulte la [API del agente de Node.js.](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#transaction-handle-createDistributedTracePayload)
    ...
    4. (Opcional) Identifique la llamada como una llamada externa (consulte la [API del agente de Node.js.](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#custom-instrumentation-api)
  </Collapser>
  ...
  >
    ...
    2. Si el agente New Relic en el servicio llamado no identifica una transacción New Relic, use la API del agente para declarar una transacción. Una forma de saber que una transacción no está en progreso es que el registro del agente de Node.js informará un error similar a este:
       ...
       Utilice [`startWebTransaction`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#startWebTransaction) para crear una transacción web o [`startBackgroundTransaction`](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#startBackgroundTransaction) para capturar una [transacción no web](/docs/apm/transactions/intro-transactions/monitor-background-processes-other-non-web-transactions).

    3. Extraiga la carga útil de la llamada que recibió (por ejemplo, en un encabezado).

    4. Invoque la llamada para aceptar la carga útil (consulte la [API del agente de Node.js.](/docs/agents/nodejs-agent/api-guides/nodejs-agent-api#transaction-handle-acceptDistributedTracePayload)
  </Collapser>
</CollapserGroup>
