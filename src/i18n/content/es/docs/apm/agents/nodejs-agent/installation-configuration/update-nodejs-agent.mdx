...
## Actualizar a la versión 14 del agente Node.js [#node-agent-v14]

Antes de actualizar a la versión 14 de Node.js, revise esta información sobre cambios importantes.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se eliminó el soporte para Node.js 20.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Se migró la instrumentación `@apollo/server` de un plug-in `@newrelic/apollo-server-plugin` a la instrumentación tradicional.
      </td>

      <td>
        * Los usuarios de Apollo server ya no necesitarán instalar y usar `@newrelic/apollo-server-plugin`. La instrumentación se aplicará automáticamente a cada instancia de Apollo Server.
        * Para obtener más información, consulte la [guía de migración de Apollo Server](#apollo-server-migration-guide).
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Se eliminó la instrumentación para `koa-router` y `koa-route`.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se eliminó el soporte para las políticas de licencia, aplicación y seguridad (`LASP`).
      </td>

      <td>
        * Esta característica quedó obsoleta y era configurable con `config.security_policies_token`.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se eliminó el soporte para la funcionalidad de rastreo multiaplicación (`CAT`).
      </td>

      <td>
        * CAT quedó obsoleto en el agente v8.4.0 y fue reemplazado por el rastreo distribuido para propagar el contexto entre servicios.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Se actualizó `config.distributed_tracing.exclude_newrelic_header` para que se establezca en `true` de forma predeterminada.
      </td>

      <td>
        * Esto fue reemplazado por encabezados W3C de forma predeterminada.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se eliminó el método de la API `shim.argsToArray` a favor de los parámetros rest.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se eliminó el método API `shim.prefixRouteParameters`.
      </td>

      <td>
        * La instrumentación de Fastify se actualizó para usar diagnósticos del canal de rastreo, por lo que este método ya no es necesario.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Se descartaron segmentos estáticos `MySQL` `Pool#query` en `mysql` y `mysql2`.
      </td>

      <td>
        * Esto creó un segmento innecesario, ya que los segmentos `Database/*` son más útiles.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: se actualizó la versión mínima compatible para las siguientes bibliotecas:
      </td>

      <td>
        * `@nestjc/cli` y `@nestjs/core` a 10.0.0
        * `bluebird` a 3.0.0
        * `mysql2` a 3.0.0
        * `next` a 14.0.0
        * `cassandra-driver` a 4.0.0
        * `fastify` a 4.0.0
      </td>
    </tr>
    ...
    <tr>
      <td>
        Se actualizó cómo se calculan el tiempo exclusivo y el tiempo total de la traza.
      </td>
...
### Guía de migración de Apollo Server [#apollo-server-migration-guide]

A partir de la versión 14.0.0 del agente, los clientes de apollo server ya no necesitarán instalar ni usar `@newrelic/apollo-server-plugin`. La instrumentación se aplicará automáticamente a cada instancia de Apollo Server. Sin embargo, si los clientes configuraron el plug-in, esto detallará el plan de migración.
...
* `captureScalars` ahora es `config.apollo_server.scalars` o `NEW_RELIC_APOLLO_SERVER_SCALARS`
* `captureIntrospectionQueries` ahora es `config.apollo_server.introspection_queries` o `NEW_RELIC_APOLLO_SERVER_INTROSPECTION_QUERIES`
* `captureServiceDefinitionQueries` ahora es `config.apollo_server.service_definition_queries` o `NEW_RELIC_APOLLO_SERVER_SERVICE_DEFINITION_QUERIES`
* `captureHealthCheckQueries` ahora es `config.apollo_server.health_check_queries` o `NEW_RELIC_APOLLO_SERVER_HEALTH_CHECK_QUERIES`
* `captureFieldMetrics` ahora es `config.apollo_server.field_metrics` o `NEW_RELIC_APOLLO_SERVER_FIELD_METRICS`

#### Llamada API

Había dos configuraciones anteriores que registraban funciones para ser llamadas, las cuales agregaban los valores principales como atributos personalizados ya sea en los segmentos de operación o de resolución. El agente sigue un patrón en el que los clientes pueden registrar devoluciones de llamada mediante métodos de API, y estos dos casos de uso se ajustan a ello.

##### customOperationAttributes

Esto es ahora `newrelic.setApolloOperationAttributesCallback`. Toma una devolución de llamada y, cuando está presente, a la devolución de llamada se le pasa el mismo argumento que antes. `requestContext`
...
Esto es ahora `newrelic.setApolloResolverAttributesCallback`. Toma una devolución de llamada y, cuando está presente, a la devolución de llamada se le pasa el mismo argumento que antes. `{ source, args, contextValue, info }`
...
Se verificó que la métrica `MySQL` `host:port` está registrada
