...
6. Agregue `-r newrelic` al script de inicio de su aplicación. Por ejemplo, si el punto de entrada de su aplicación es `./dist/server.js` , entonces usaría el indicador require de esta manera:
   ...
   <Callout variant="important">
     Si no puede utilizar el indicador de requisito `-r` , también puede utilizar `require('newrelic')` como primera línea del módulo principal de su aplicación. <DoNotTranslate>**Note**</DoNotTranslate> Si está utilizando [Babel](https://babeljs.io/docs/en/index.html) o un transpilador similar `require('newrelic')` causará problemas de instrumentación.
