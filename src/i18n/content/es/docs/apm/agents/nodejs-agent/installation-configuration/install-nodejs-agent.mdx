...
Para completar una instalación básica del agente de Node.js, puedes usar nuestra instalación guiada para una instalación automatizada (elige UE si estás en la UE, o JP si estás en la región de JP) o seguir las instrucciones de este documento. De cualquier manera, necesitas una cuenta de New Relic si aún no tienes una. ([¡Es gratis, para siempre!](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#how-pricing-works))
...
3. Utilice el comando `npm install newrelic` para cada aplicación que desee monitorear.

<Callout variant="important">
  Si utiliza módulos de Apollo Server, consulte la [documentación del plug-in de Apollo Server](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/apollo-server-plugin-nodejs/) para obtener detalles de instalación.

  Si utiliza Next.js, consulte la [guía de instrumentación del agente híbrido de Next.js](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/nextjs-instrumentation/).
</Callout>
...
5. Configure el agente a través del archivo `newrelic.js` o mediante [la variable de entorno](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#environment):
   ...
   * Personalice la configuración `license_key` con <InlinePopover type="licenseKey" />.
...
6. Agregue `-r newrelic` al script de inicio de su aplicación. Por ejemplo, si el punto de entrada de su aplicación es `./dist/server.js` , entonces usaría el indicador require de esta manera:
   ...
   <Callout variant="important">
     Para una aplicación Next.js, cargue nuestro agente mediante `NODE_OPTIONS`:
   ...
   <Callout variant="important">
     Si no puede usar el indicador require `-r`, también puede usar `require('newrelic')` como la primera línea del módulo principal de su aplicación. Sin embargo, si utiliza [Babel](https://babeljs.io/docs/en/index.html) o un transpilador similar, `require('newrelic')` causará problemas de instrumentación.
...
8. Genere algo de tráfico y luego espere unos minutos hasta que aparezcan los datos en la [UI de APM](/docs/apm/applications-menu/monitoring/apm-overview-page).
