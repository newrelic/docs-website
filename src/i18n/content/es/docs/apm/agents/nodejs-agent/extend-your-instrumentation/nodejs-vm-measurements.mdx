...
1. Asegúrese de que esté instalada una versión compatible de Node.js. <DoNotTranslate>**Recommendation:**</DoNotTranslate> Utilice la última versión LTS. No utilice versiones inferiores a la versión mínima admitida para el módulo `@newrelic/native-metrics` (v4 o superior).
...
3. <DoNotTranslate>**Windows users:**</DoNotTranslate> Ejecute el siguiente comando:
...
Para ver gráficos seleccionados con datos de máquina virtual (VM) de Node.js, vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Node VMs**</DoNotTranslate>. Para obtener más información, consulte la [página de estadísticas de la máquina virtual (VM) de Node.js.](/docs/agents/nodejs-agent/supported-features/node-vms-statistics-page)
