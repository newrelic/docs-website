...
freshnessValidatedDate: '2026-05-22T00:00:00.000Z'
...
Algunas métricas de la máquina virtual (VM) de Node.js se recopilan de forma predeterminada mediante las API integradas de Node.js, mientras que otras requieren el paquete opcional [`@newrelic/native-metrics`](https://www.npmjs.com/package/@newrelic/native-metrics):

* **Muestreadores predeterminados (no se requiere módulo nativo):** memoria (`Memory/*`), tiempo de espera de la cola del bucle de eventos (`Events/wait`) y CPU (`CPU/*`) siempre se recopilan usando `process.memoryUsage()`, medición de retraso de `setTimeout` y `process.cpuUsage()`, respectivamente.
* **Muestreadores de módulos nativos:** la recolección de basura (`GC/*`) y el uso preciso de la CPU del bucle de eventos (`Nodejs/EventLoop/CPU/Usage`) requieren `@newrelic/native-metrics`. Si el módulo no está instalado o falla al cargar, estas métricas no aparecerán y el agente continuará ejecutándose solo con los muestreadores predeterminados.
...
1. Asegúrese de que esté instalada una versión compatible de Node.js. Recomendación: utilice la versión LTS más reciente. No utilice versiones inferiores a la versión mínima admitida para el módulo `@newrelic/native-metrics` (v4 o superior).
...
<CollapserGroup>
  <Collapser id="time-in-garbage-collection" title="Tiempo en recolección de basura">
    <Callout variant="important">Requiere `@newrelic/native-metrics`. Esta métrica no se recopila de forma predeterminada.</Callout>
  ...
  <Collapser id="memory" title="Memoria">
    <Callout variant="tip">Recopilado por defecto. No se requiere módulo nativo.</Callout>
  ...
  <Collapser id="cpu" title="UPC">
    <Callout variant="tip">Recopilado de forma predeterminada en Node.js v6.1.0 o superior. No se requiere módulo nativo.</Callout>
  ...
  <Collapser id="event-loop" title="Bucle de eventos">
    <Callout variant="important">Requiere `@newrelic/native-metrics`. Esta métrica no se recopila de forma predeterminada.</Callout>
