...
<CollapserGroup>
  ...
  >
    Muchos controladores de base de datos utilizan una abstracción llamada `cursor`. Los cursores brindan la capacidad de iterar a través de los resultados de la consulta. Por ejemplo, el controlador <DoNotTranslate>**mongodb**</DoNotTranslate> proporciona cursores al ejecutar `find` consulta.
