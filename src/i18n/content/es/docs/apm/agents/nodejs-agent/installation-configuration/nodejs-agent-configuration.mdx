...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-root" title="sampler.root">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Controla cómo se toman las decisiones de muestreo para los spans raíz-transacciones que no tienen un elemento principal remoto (lo que significa que carecen de un encabezado de rastreo distribuido `traceparent` o `newrelic` entrante). Los valores válidos son:

    * `adaptive` (predeterminado): utiliza el algoritmo de muestreo adaptativo integrado de New Relic.
    * `always_on`: muestrea todos los spans.
    * `always_off`: no muestrea ningún span.
    * `trace_id_ratio_based`: muestrea una proporción fija de trazas, determinada de forma determinista por el ID de la traza.

    Por ejemplo, para configurar esto en el archivo de configuración, usaría:
    ...
    Cuando se establece en `trace_id_ratio_based`, también debe proporcionar un `ratio` (un número entre `0` y `1`):
    ...
    Cuando establece esto en `adaptive`, opcionalmente puede anular el objetivo de muestreo (a 1-120 transacciones/minuto) específicamente para este muestreador:
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="sampler.remote_parent_sampled">
    ...
    Cuando el encabezado traceparent tiene un indicador de muestreo de `01`, esto controla cómo manejar el muestreo de spans. Los valores de cadena permitidos son:
    ...
    * `trace_id_ratio_based`: muestrea una proporción fija de trazas, determinada de forma determinista por el ID de la traza.
    ...
    Esta configuración tiene prioridad sobre [`remote_parent_not_sampled`](#dt-sampler-remote-parent-not-sampled) cuando ambas podrían aplicarse.
    ...
    Cuando establece esto en `trace_id_ratio_based`, debe proporcionar un valor de `ratio` de `0`-`1`. Cuando establece esto en `adaptive`, opcionalmente puede anular el objetivo de muestreo específicamente para este muestreador. Para conocer la sintaxis equivalente, consulte los ejemplos de forma de objeto y variable de entorno en [`sampler.root`](#dt-sampler-root) y sustituya `remote_parent_sampled` por `root`.
  </Collapser>
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    Cuando el encabezado traceparent tiene un indicador de muestreo de `00`, esto controla cómo manejar el muestreo de spans. Los valores de cadena permitidos son:
    ...
    * `trace_id_ratio_based`: muestrea una proporción fija de trazas, determinada de forma determinista por el ID de la traza.
    ...
    Esta configuración solo tiene efecto cuando [`remote_parent_sampled`](#dt-sampler-remote-parent-sampled) no se aplica (es decir, el indicador de muestreo de traceparent es `00`).
    ...
    Cuando establece esto en `trace_id_ratio_based`, debe proporcionar un valor de `ratio` de `0`-`1`. Cuando establece esto en `adaptive`, opcionalmente puede anular el objetivo de muestreo específicamente para este muestreador. Para conocer la sintaxis equivalente, consulte los ejemplos de forma de objeto y variable de entorno en [`sampler.root`](#dt-sampler-root) y sustituya `remote_parent_not_sampled` por `root`.
  </Collapser>

  <Collapser id="dt-sampler-adaptive-sampling-target" title="sampler.adaptive_sampling_target">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    El objetivo de muestreo para el muestreo adaptativo, en transacciones por minuto, utilizado al configurar el muestreador predeterminado/adaptativo. Debe estar dentro del rango `[1, 120]` (inclusive). Al conectarse a New Relic, este valor se envía al recolector, y el valor `sampling_target` devuelto en la respuesta de conexión se utiliza como el objetivo de muestreo efectivo para el muestreo adaptativo en el agente.
...
Configure cómo el agente detecta la información del host y del entorno cloud para calcular con precisión sus precios basados en la utilización. Estas configuraciones aparecen en la sección `utilization` del archivo de configuración `newrelic.js` de su aplicación.

<CollapserGroup>
  <Collapser id="utilization-detect-aws" title="detect_aws">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta comunicarse con AWS para obtener información sobre la máquina virtual (VM) en la que se ejecuta el proceso.
  </Collapser>

  <Collapser id="utilization-detect-pcf" title="detect_pcf">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta detectar si el proceso se está ejecutando en Pivotal Cloud Foundry.
  </Collapser>

  <Collapser id="utilization-detect-azure" title="detect_azure">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta comunicarse con Azure para obtener información sobre la máquina virtual (VM) en la que se ejecuta el proceso.
  </Collapser>

  <Collapser id="utilization-detect-azurefunction" title="detect_azurefunction">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta leer las variables de entorno y el contexto de invocación para obtener información sobre la Azure Function en la que se ejecuta.
  </Collapser>

  <Collapser id="utilization-detect-docker" title="detect_docker">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta leer archivos para obtener información sobre el contenedor en el que se ejecuta el proceso.
  </Collapser>

  <Collapser id="utilization-detect-gcp" title="detect_gcp">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta comunicarse con GCP para obtener información sobre la máquina virtual (VM) en la que se ejecuta el proceso.
  </Collapser>

  <Collapser id="utilization-detect-kubernetes" title="detect_kubernetes">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si el agente intenta comunicarse con Kubernetes para obtener información sobre el contenedor en el que se ejecuta el proceso.
  </Collapser>

  <Collapser id="utilization-logical-processors" title="logical_processors">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Anula la cantidad de procesadores lógicos que el agente informa para el host.
  </Collapser>

  <Collapser id="utilization-billing-hostname" title="billing_hostname">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Sobrescribe el nombre de host que el agente informa para fines de facturación/utilización.
  </Collapser>

  <Collapser id="utilization-total-ram-mib" title="total_ram_mib">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Sobrescribe la cantidad total de RAM (en MiB) que el agente reporta para el host.
  </Collapser>

  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    <Callout variant="important">
      Obsoleto y se eliminará en la versión 15 del agente. Emplee `gcp_cloud_run.use_instance_as_host` en su lugar.
    </Callout>

    Cuando está habilitado, utilizará el ID de metadatos de GCP para establecer el nombre de host de la aplicación en ejecución (servicios, grupos de trabajadores y trabajos).
  </Collapser>

  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Si es `true`, el agente antepone el nombre de la revisión de Cloud Run al ID de la instancia de GCP para formar el nombre de host (`{revision}-{instance id}`) en Google Cloud Run. El nombre de la revisión proviene de `K_REVISION` en un servicio de Cloud Run, `CLOUD_RUN_REVISION` en un grupo de trabajadores de Cloud Run y `CLOUD_RUN_EXECUTION` en un trabajo de Cloud Run. No tiene efecto a menos que `utilization.gcp_cloud_run.use_instance_as_host` también sea `true`.
  </Collapser>

  <Collapser id="utilization-gcp-cloud-run-use-instance-as-host" title="gcp_cloud_run.use_instance_as_host">
    <table>
      <tbody>
        <tr>
          <th>
...
Estas configuraciones rigen las diversas características basadas en OpenTelemetry proporcionadas por el agente. Aparecen en la sección `opentelemetry` del archivo de configuración `newrelic.js` de su aplicación.

<Callout variant="important">
  Esta configuración está sujeta a cambios mientras el conjunto de características de OpenTelemetry está en desarrollo.
</Callout>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Interruptor global para toda la característica de OpenTelemetry. Si se establece en `false`, no se habilitará ninguna otra subcaracterística de OpenTelemetry (como `traces`, `logs` o `metrics`), independientemente de la propia configuración de esa subcaracterística.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    Habilita la integración de instrumentaciones de OpenTelemetry (por ejemplo, `@fastify/otel`) en el agente de New Relic.
  </Collapser>

  <Collapser id="opentelemetry-logs-enabled" title="opentelemetry.logs.enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Cuando `true`, el agente configura automáticamente la API de logs de OpenTelemetry para enviar los logs emitidos a través de la API específica de OTel a New Relic.

    <Callout variant="important">
      Esta característica depende del [reenvío de logs de aplicación](#application-logging-forwarding-enabled) — `application_logging.forwarding.enabled` también debe ser `true`.
    </Callout>
  </Collapser>

  <Collapser id="opentelemetry-metrics-enabled" title="opentelemetry.metrics.enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Cuando es `true`, el agente configura automáticamente la API de métricas de OpenTelemetry para enviar métricas a New Relic, adjuntas a la entidad de la aplicación instrumentada por el agente.
  </Collapser>

  <Collapser id="opentelemetry-metrics-export-interval" title="opentelemetry.metrics.export_interval">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    El número de milisegundos entre cada intento de enviar métricas de OpenTelemetry a New Relic. Debe ser mayor o igual que `opentelemetry.metrics.export_timeout`.
  </Collapser>

  <Collapser id="opentelemetry-metrics-export-timeout" title="opentelemetry.metrics.export_timeout">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    La cantidad de milisegundos que se permite una operación de exportación de métrica de OpenTelemetry antes de que deba completarse correctamente. Si se excede el tiempo de espera, se informa a través de la API de diagnóstico de OpenTelemetry.
  </Collapser>
</CollapserGroup>

## Instrumentación de Apollo Server [#apollo-server-instrumentation]

Estos ajustes personalizan el comportamiento de la instrumentación de Apollo Server (GraphQL) del agente. Aparecen en la sección `apollo_server` del archivo de configuración `newrelic.js` de su aplicación.

<CollapserGroup>
  <Collapser id="apollo-server-scalars" title="apollo_server.scalars">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Habilita la captura de tiempo para los campos resueltos con el tipo de retorno `GraphQLScalarType`. Esto puede ser deseable al realizar cálculos que requieren mucho tiempo para devolver un valor escalar. No se recomienda para consultas que devuelven una gran cantidad de campos escalares precalculados.

    <Callout variant="tip">
      Los resolutores de consulta/mutación siempre se capturan, incluso cuando devuelven un tipo escalar.
    </Callout>
  </Collapser>

  <Collapser id="apollo-server-introspection-queries" title="apollo_server.introspection_queries">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Habilita la captura de tiempos para una [IntrospectionQuery](https://www.graphql-js.org/api-v16/utilities/#introspectionquery).
  </Collapser>

  <Collapser id="apollo-server-service-definition-queries" title="apollo_server.service_definition_queries">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Habilita la captura de tiempos para una [consulta de definición de servicio](https://www.apollographql.com/docs/federation/federation-spec/#fetch-service-capabilities) recibida de un Apollo Federated Gateway Server.
  </Collapser>

  <Collapser id="apollo-server-health-check-queries" title="apollo_server.health_check_queries">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Habilita la captura de tiempos para una [consulta de verificación de estado](https://www.apollographql.com/docs/federation/api/apollo-gateway/#servicehealthcheck) recibida de un servidor Apollo Federated Gateway.
  </Collapser>

  <Collapser id="apollo-server-field-metrics" title="apollo_server.field_metrics">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Habilita la captura de métricas para cada campo y argumento de resolución visto para una consulta de Apollo. Destinado a identificar campos no utilizados en su esquema de GraphQL.
  </Collapser>
</CollapserGroup>
