...
[Heroku](https://www.heroku.com/) es una solución de plataforma como servicio (PaaS) para alojar aplicaciones web en varios lenguajes, incluido PHP. Con New Relic, puedes ampliar Heroku con métrica de [<InlinePopover type="apm" />](/docs/apm/new-relic-apm/getting-started/welcome-new-relic-apm)y [<InlinePopover type="browser" />](/docs/browser/new-relic-browser/getting-started/new-relic-browser).

## Prepara tu solicitud [#Preparing]

Antes de instalar el agente PHP, asegúrese de que su aplicación web PHP esté instalada y ejecutándose en Heroku. Para obtener más información, consulte [Primeros pasos con PHP en Heroku](https://devcenter.heroku.com/articles/getting-started-with-php).
...
<CollapserGroup>
  <Collapser id="via_heroku" title="A través del sitio web de Heroku">
  ...
  <Collapser id="via_toolbelt" title="A través del cinturón de herramientas Heroku">
...
Para solucionar problemas del agente PHP en Heroku, examine su archivo de log, que está almacenado en el log del servidor web de Heroku.
...
2. Utilice el archivo de log para solucionar el problema.

3. Si necesita más ayuda, [obtenga asistencia en support.newrelic.com](https://support.newrelic.com).

El agente PHP tiene por defecto el [nivel de logs](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-loglevel) `warning` . El soporte de New Relic también puede solicitar el logen el nivel `verbosedebug` de logs. Para cambiar el nivel de logs a `verbosedebug`, ejecute este comando del cinturón de herramientas de Heroku:
...
<Callout variant="caution">
  El nivel de logs `verbosedebug` genera rápidamente un gran volumen de datos. Utilice esta configuración solo si el soporte de New Relic lo solicita y elimínela tan pronto como recopile el resultado ejecutando este comando del cinturón de herramientas de Heroku:
