...
Para ver una aplicación de ejemplo, vaya al [Foro de soporte de New Relic](https://knowledge.newrelic.com/s/article/PHP-Agent-And-Daemon-Containers). Si utiliza contenedores de aplicación de corta duración, le recomendamos que utilice un contenedor independiente para el daemon del agente PHP.
...
<Callout variant="caution">
  De forma predeterminada, la primera transacción hace que el agente active el inicio de un daemon y la inicialización de una conexión de la aplicación. Por razones de rendimiento, el agente no espera a que se completen esas operaciones antes de inicializar la conexión. Esto puede resultar en la pérdida de las primeras transacciones después de que se inicia un contenedor. Para evitar esta pérdida, establezca las entradas `newrelic.daemon.start_timeout` y `newrelic.daemon.app_connect_timeout` en el archivo `newrelic.ini` en nuestros [valores recomendados](https://support.newrelic.com/s/hubtopic/aAX8W0000008bSo/troubleshooting-frameworks) de 5s y 15s, respectivamente.
</Callout>
...
<Callout variant="tip">
  ...
  * [Monitoreo de contenedores que ejecutan un único script PHP](https://support.newrelic.com/s/hubtopic/aAX8W0000008aNC/relic-solution-single-php-script-docker-containers)
  * [Consejos de resolución de problemas para el agente PHP.](https://support.newrelic.com/s/hubtopic/aAX8W0000008bSo/troubleshooting-frameworks)
</Callout>
