...
## Usar o desactivar la instrumentación automática [#auto\_instrumentation][#auto_instrumentation]
...
## instrumentar manual a través de la API del agente [#manual\_instrumentation][#manual_instrumentation]
...
### Agregar atributos personalizados como nonce [#nonce-attribute]

Si su sitio utiliza una [Política de seguridad de contenido (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) que requiere un atributo `nonce`, puede pasar `false` a ambas funciones de la API para que devuelvan la cadena de JavaScript sin procesar sin las etiquetas `<script>`. Luego puede construir las etiquetas usted mismo con los atributos requeridos:
...
Pasar `false` como argumento suprime las etiquetas de script que las funciones generan normalmente, de modo que puede envolver la cadena de JavaScript sin procesar con sus propias etiquetas `<script>` y cualquier atributo que necesite.
...
## instrumentar manual para varios marcos. [#manual\_generic][#manual_generic]
...
<CollapserGroup>
  ...
  <Collapser id="manual_drupal" title="Drupal">
    ...
    2. Inserte una llamada para generar el pie de página justo antes de la última etiqueta de diseño de cierre:
       ...
       <Callout variant="important">
         Drupal 10.2 introdujo un [nuevo cambio](https://www.drupal.org/node/3298551) que hace que establezca un encabezado `content-length`. El agente PHP de New Relic no puede inyectar automáticamente la autoinstrumentación del navegador cuando el campo de encabezado HTTP `Content-Length` está establecido. Para seguir usando el monitoreo de navegador, deshabilite la autoinstrumentación del navegador e inserte manualmente el encabezado y pie de página de JavaScript en sus plantillas.
       </Callout>
  </Collapser>
</CollapserGroup>
