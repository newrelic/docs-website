...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <DoNotTranslate>**Web transactions**</DoNotTranslate> son solicitudes de activos (o visitas de usuarios) a su sitio. Con el marco, esto suele ser manejado por un mecanismo de despacho MVC.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DoNotTranslate>**Metrics**</DoNotTranslate> Se denominan entidades que reportan información temporal. New Relic utiliza estas métricas para generar gráficos y páginas, como la [página APM <DoNotTranslate>**Summary**</DoNotTranslate> ](/docs/apm/applications-menu/monitoring/apm-overview-page). Por ejemplo, New Relic instrumentado llama a MySQL y genera métrica llamada `Database/select` o `Database/update` cada vez que un usuario selecciona o actualiza datos.
    ...
    <tr>
      ...
      <td>
        Durante cada período de cosecha, New Relic registra la transacción web más lenta que cae por debajo de un umbral definido por el usuario. Este es un <DoNotTranslate>**transaction trace**</DoNotTranslate>.
...
<DoNotTranslate>**Controller and action**</DoNotTranslate> (cuando se usa MVC):
...
<DoNotTranslate>**Action only**</DoNotTranslate> (sin usar MVC):
...
<CollapserGroup>
  ...
  >
    El encabezado <DoNotTranslate>**must**</DoNotTranslate> se insertará lo más cerca posible de la parte superior de la página generada. Inmediatamente después de la etiqueta `<meta http-equiv....>` está el lugar ideal. Debe preceder a <DoNotTranslate>**any**</DoNotTranslate> JavaScript, hojas de estilo CSS u otro contenido vinculado o incrustado.
    ...
    `newrelic_get_browser_timing_header()` toma un argumento opcional `bool` cuyo valor predeterminado es <DoNotTranslate>**true**</DoNotTranslate>. Indica si la cadena devuelta debe estar encerrada o no por una etiqueta script .

    <DoNotTranslate>**Recommendation:**</DoNotTranslate> Inserte el pie de página inmediatamente antes de la etiqueta de cierre de página `</body>` como lo último en la página generada.
    ...
    `newrelic_get_browser_timing_footer()` toma un `bool` opcional que por defecto es <DoNotTranslate>**true**</DoNotTranslate>. Indica si la cadena devuelta debe estar encerrada o no por una etiqueta script .
  </Collapser>
