...
Para los usuarios de WebSphere y WebLogic, el agente de Java también informa el nombre de instancia para cada instancia de servidor de aplicaciones activa, que es visible en la página APM <DoNotTranslate>**Settings > Environment**</DoNotTranslate> . Generalmente, este nombre lo proporciona el servidor de aplicaciones. Si se establece un nombre de instancia, se usará en lugar del nombre para mostrar en la UI de New Relic.
