---
title: Plantilla de archivo de configuración de agente de Java
tags:
  - Agents
  - Java agent
  - Configuration
...
Este documento contiene una versión con plantilla del archivo `newrelic.yml` que el agente de Java lee para su configuración. El formato del archivo de configuración es YAML. Si bien no es necesario tener conocimientos de YAML para editar el archivo, tenga cuidado de mantener la sangría adecuada. Obtenga más información sobre YAML en [Wikipedia](http://en.wikipedia.org/wiki/yaml).

Al realizar cambios en el archivo `newrelic.yml` , utilice un validador YAML para asegurarse de que la sintaxis sea precisa y siga estas reglas:

* Los archivos YML distinguen entre mayúsculas y minúsculas.
* Todas las sangrías están en incrementos de dos caracteres. Los datos en la misma sección del archivo deben usar el mismo nivel de sangría. Sangra las subsecciones con dos espacios adicionales.

Para obtener más información sobre estructura, directorios y configuración, consulte [Configuración del agente de Java](/docs/agents/java-agent/configuration/java-agent-configuration-config-file).
