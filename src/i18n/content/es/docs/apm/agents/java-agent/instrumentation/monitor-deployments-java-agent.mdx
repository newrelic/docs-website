...
New Relic te permite enviar información sobre el despliegue de la aplicación desde la línea de comando. Luego, puede ver información detallada en la [página<DoNotTranslate>**Deployments**</DoNotTranslate> ](/docs/apm/applications-menu/events/deployments-page)en la UI: vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > Applications > (select an app) > Events > Deployments**</DoNotTranslate>.
...
Por ejemplo, si configuró el marcador de despliegue y ejecutó el script para enviar 100 instancias de la aplicación, la página <DoNotTranslate>**Deployments**</DoNotTranslate> mostrará un despliegue para las 100 instancias. Si ejecuta el script nuevamente, mostrará otro despliegue.
...
El agente de Java no recoge la configuración de proxy del archivo `newrelic.yml` . <DoNotTranslate>**Recommendation:**</DoNotTranslate> Si usas un proxy en tu aplicación, usa cualquiera de estas opciones para registrar el despliegue:
