...
También puede monitor Java Management Extensions (JMX) a través de instrumentación personalizada. JMX es una forma de monitor y administrar aplicaciones, dispositivos y servicios. Puede implementar el monitoreo JMX a través de un [archivo <DoNotTranslate>**YAML**</DoNotTranslate> externo](/docs/java/custom-jmx-instrumentation-by-yml).