...
<CollapserGroup>
  ...
  >
    ...
    Si configura el `metricName` además del despachador, como en `@Trace(metricName="YourMessageHere", dispatcher=true)`, entonces el nombre de la transacción aparecerá como YourMessageHere en la página APM <DoNotTranslate>**Transactions**</DoNotTranslate> pero el tiempo empleado en este método no aparecerá como YourMessageHere en ninguna traza de la transacción.
