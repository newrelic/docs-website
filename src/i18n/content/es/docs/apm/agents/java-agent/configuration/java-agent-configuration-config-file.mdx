...
<CollapserGroup>
  ...
  <Collapser id="datastore_multihost_preference" title="datastore_multihost_preference">
    ...
    Si se detectan varias direcciones durante el ciclo de conexión para un almacén de datos, esta configuración determina qué dirección elegirá el agente:

    * `NONE`:El agente devuelve una dirección vacía. Este es el comportamiento predeterminado.
    * `FIRST`:Emplee la primera dirección detectada.
    * `LAST`:Emplee la última dirección detectada.
  ...
  <Collapser id="event_ingest_uri" title="event_ingest_uri">
    ...
    Evento ingest URI utilizado por alguna característica del agente. Por defecto el extremo de ingestión de producción estadounidense. Se puede configurar manualmente para enviar eventos a otros extremos de ingesta. Ejemplos:
  ...
  <Collapser id="metric_ingest_uri" title="metric_ingest_uri">
    ...
    Métrica ingest URI utilizada por algún agente característico. Por defecto el extremo de ingestión de producción estadounidense. Se puede configurar manualmente para enviar métrica a otros extremos de ingesta. Ejemplos:
...
<CollapserGroup>
  ...
  <Collapser id="cfg-circuitbreaker_gc_cpu_threshold" title="gc_cpu_threshold">
    ...
    Personaliza el porcentaje de tiempo de CPU de recolección de basura por encima del cual el interruptor debería activarse. Cuando el porcentaje de memoria heap libre es menor que `memory_threshold` y el tiempo de CPU dedicado a la recolección de basura es mayor que `gc_cpu_threshold`, el interruptor se activa. Para que sea menos probable que el interruptor se dispare, disminuya `memory_threshold` y/o aumente `gc_cpu_threshold`. Ajuste estos valores según sea necesario, en función del rendimiento operativo y el comportamiento de su aplicación.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="metadata_bypass_proxy" title="metadata_bypass_proxy">
    ...
    Cuando se establece como verdadero, esta configuración le permite omitir el proxy para el acceso al extremo de metadatos de cloud. El valor predeterminado es falso.
  </Collapser>
</CollapserGroup>
...
#### Lista de denegación de nivel de logs [#log-level-denylist]

El agente de Java puede omitir niveles de logs específicos al generar LogEvents que se reenvían a New Relic.

Puede controlarlo a través de la opción de configuración `log_level_denylist`, anidada en la sección `forwarding`. Esta configuración te permite definir una lista de cadenas separadas por comas que representan niveles de logs (p. ej. `log_level_denylist: "info, debug"`) que no deberían tener logs de eventos reenviados. Los nombres de nivel de logs no distinguen entre mayúsculas y minúsculas, pero por lo demás deben ser una coincidencia exacta. Estas configuraciones pueden ser anuladas por la propiedad del sistema con el prefijo `newrelic.config.application_logging.forwarding.log_level_denylist`. La única opción disponible es:
...
A partir de la versión `9.1.0` del agente Java, se ha agregado compatibilidad con la API de OpenTelemetry Tracing, Metrics y Logs, ofreciendo una experiencia de &quot;lo mejor de ambos mundos&quot; al combinar la visibilidad profunda de New Relic con las señales de OpenTelemetry.

Con esta funcionalidad, las API de OpenTelemetry Tracing, Metrics y Logs pueden usarse para instrumentar directamente su aplicación de forma personalizada, en combinación con las API del agente Java de New Relic, mientras que el agente Java integra todo a la perfección en una experiencia de APM familiar. Además, algunas bibliotecas que proporcionan instrumentación nativa de OpenTelemetry o de biblioteca independiente pueden utilizarse para brindar una visibilidad mejorada de su aplicación.

Las siguientes opciones de configuración determinan qué señales de OpenTelemetry se deben capturar e incorporar a la experiencia de New Relic APM.
...
<CollapserGroup>
  <Collapser id="cfg-opentelemetry-enabled" title="activado">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Establecer en true para permitir que se habiliten las señales individuales de OpenTelemetry, o en false para deshabilitar todas las señales de OpenTelemetry. El valor predeterminado es falso.

    Se puede establecer mediante la propiedad del sistema:

    ```ini
    -Dnewrelic.config.opentelemetry.enabled=false
    ```

    Se puede establecer mediante la variable de entorno:
  ...
  <Collapser id="cfg-opentelemetry-logs-enabled" title="logs.enabled">
    ...
    Establecer en true para habilitar las señales de Logs de OpenTelemetry cuando `opentelemetry.enabled=true`. El valor predeterminado es true.
  ...
  <Collapser id="cfg-opentelemetry-metrics-enabled" title="metrics.enabled">
    ...
    Establezca en true para habilitar las señales de métricas de OpenTelemetry cuando `opentelemetry.enabled=true`. El valor predeterminado es true.
  ...
  <Collapser id="cfg-opentelemetry-metrics-include" title="metrics.include">
    ...
    Una cadena delimitada por comas de medidores de OpenTelemetry (p. ej. &quot;MeterName1,MeterName2&quot;) cuyas señales deben incluirse. De forma predeterminada, se incluyen todos los Medidores. Esto anulará cualquier exclusión de medidores predeterminada en el agente, reactivándolos efectivamente.
  ...
  <Collapser id="cfg-opentelemetry-metrics-exclude" title="metrics.exclude">
    ...
    Una cadena delimitada por comas de medidores de OpenTelemetry (p. ej. &quot;MeterName3,MeterName4&quot;) cuyas señales deben excluirse. Esto tiene prioridad sobre todas las demás fuentes de inclusión/exclusión, deshabilitando efectivamente los Medidores listados.
  ...
  <Collapser id="cfg-opentelemetry-traces-enabled" title="traces.enabled">
    ...
    Establecer en true para habilitar las señales de trazas de OpenTelemetry cuando `opentelemetry.enabled=true`. El valor predeterminado es true.
  ...
  <Collapser id="cfg-opentelemetry-traces-include" title="traces.include">
    ...
    Una cadena de rastreadores de OpenTelemetry delimitada por comas (p. ej., &quot;TracerName1,TracerName2&quot;) cuyas señales deben incluirse. De forma predeterminada, todos los Tracers están incluidos. Esto anulará cualquier exclusión predeterminada del Tracer en el agente, reactivándolas efectivamente.
  ...
  <Collapser id="cfg-opentelemetry-traces-exclude" title="traces.exclude">
    ...
    Una cadena de rastreadores de OpenTelemetry delimitada por comas (p. ej., &quot;TracerName3,TracerName4&quot;) cuyas señales se deben excluir. Esto tiene prioridad sobre todas las demás fuentes de inclusión/exclusión, lo que deshabilita efectivamente los Tracers enumerados.
...
El agente se puede habilitar o deshabilitar selectivamente en función de la clase principal de inicio o el archivo jar ejecutable que se extrae de la línea de comandos. Esto es útil en situaciones en las que la variable de entorno &quot;JAVA\_TOOL\_OPTIONS&quot; está presente y contiene la bandera -javaagent, pero no queremos aplicar la instrumentación a todas las aplicaciones java en el entorno, por ejemplo, en un contenedor de Kubernetes.

<Callout variant="important">
  La configuración de omisión solo es compatible a través de una propiedad del sistema y una variable de entorno debido a lo temprano en el ciclo de vida del agente que se debe verificar la configuración de omisión.
</Callout>

Para fines de diagnóstico, si se define alguna de las variables de configuración de exclusión o inclusión, el agente imprimirá la línea de comandos extraída y los tokens de omisión analizados en stdout:
...
Si el agente está deshabilitado en función de una variable de configuración de inclusión u omisión, lo siguiente se volcará en stdout:
...
<CollapserGroup>
  ...
  <Collapser id="cfg-include-configuration" title="NEW_RELIC_STARTUP_JAVA_ARTIFACT_INCLUDES">
    ...
    Una lista separada por comas de clases principales, archivos jar ejecutables o herramientas/aplicaciones basadas en Java que el agente DEBERÍA instrumentar. Por ejemplo: `NEW_RELIC_STARTUP_JAVA_ARTIFACT_INCLUDES=myapp.jar`. La propiedad del sistema correspondiente es `newrelic.config.startup_java_artifact_includes`.
  </Collapser>
</CollapserGroup>
...
## Eliminación temporal del archivo Jar [#temporary-jar-file-removal]

En algunos entornos de clientes, los archivos jar temporales del agente no se limpian correctamente, aunque estos archivos jar están marcados para ser eliminados al finalizar la JVM. Esto se debe probablemente a la terminación anormal de la JVM. Esta configuración indica al agente que elimine cualquier archivo jar temporal del agente anterior al valor especificado al inicio.

<Callout variant="important">
  Esta configuración solo es compatible a través de una propiedad del sistema o una variable de entorno debido a lo temprano que debe ocurrir la eliminación del archivo en el ciclo de vida del agente.
</Callout>

Para fines de diagnóstico, si esta función está habilitada, se escribirá lo siguiente en la consola:

```
New Relic Agent: Removing stale temporary agent file jars from [temporaryFolder] older than [X] hour(s)
```

Si se elimina algún archivo, lo siguiente se escribirá en la consola:
...
<CollapserGroup>
  <Collapser id="cfg-tmp-file-deletion-configuration" title="NEW_RELIC_TEMP_JARFILE_AGE_THRESHOLD_HOURS">
    ...
    El valor es la cantidad de horas que debe tener un jar temporal para ser eliminado (solo números enteros). El valor predeterminado es 0, lo que deshabilita la verificación de eliminación de archivos. La propiedad del sistema correspondiente es `newrelic.config.temp_jarfile_age_threshold_hours`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="tt-exec_call_sql_regex_disabled" title="exec_call_sql_regex_disabled">
    ...
    En el caso de sentencias SQL grandes, la ejecución de expresiones regulares que intentan analizar las sentencias exec y call puede llevar una cantidad de tiempo significativa. Establecer esto como verdadero deshabilitará la ejecución de estas expresiones regulares complejas.
  ...
  <Collapser id="tt-sql_metadata_comments" title="sql_metadata_comments">
    ...
    Controla la adición de comentarios al inicio de las sentencias SQL ejecutadas para que sean utilizados por el producto de monitoreo del rendimiento de consultas de New Relic para la vinculación de entidades. El valor predeterminado es falso.

    Un comentario resultante se vería así: /*nr\_service\_guid=MTE3NDc2MDB8QVBNfAAAAAxJQEEEEE9OfDI4MTc5NDEIEA*/
  </Collapser>
  ...
  <Collapser id="tt-transaction_threshold" title="transaction_threshold">
    ...
    El umbral de tiempo utilizado para determinar cuándo una transacción es elegible para ser rastreada. Cuando el tiempo de respuesta de la transacción excede este umbral, la transacción se vuelve elegible para un [seguimiento de la transacción](/docs/apm/transactions/transaction-traces/transaction-traces). Si se muestrea, ese seguimiento de la transacción se registrará y se enviará a New Relic.
