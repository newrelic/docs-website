...
<CollapserGroup>
  ...
  <Collapser id="tt-sql_metadata_comments" title="sql_metadata_comments">
    ...
    Controla la adición de comentarios al inicio de las sentencias SQL ejecutadas para que sean utilizados por el producto New Relic Query Performance Monitoring para la vinculación de entidades. Una cadena vacía deshabilita los comentarios SQL (configuración predeterminada). Las opciones permitidas son:

    * `nr_service_guid` - El GUID de la entidad de New Relic que el agente está instrumentando

    Un comentario resultante se vería así: `/*nr_service_guid=MTE3NDc2MDB8QVBNfEFQUExJQ0FUSU9OfDI4MTc5NDEIEA*/`
  </Collapser>
