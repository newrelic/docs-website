...
1. Haga una copia de seguridad del directorio raíz del <DoNotTranslate>**entire**</DoNotTranslate> [agente de Java](/docs/agents/manage-apm-agents/troubleshooting/find-agent-root-directory#java-agent) en otra ubicación. Cambie el nombre de ese directorio a `NewRelic_Agent#.#.#`, donde `#.#.#` es el número de versión del agente.
...
4. (Opcional) Si está utilizando cualquiera de las API del agente como nuestra [API de agente de Java](/docs/apm/agents/java-agent/api-guides/guide-using-java-agent-api/) o una [API de Scala](/docs/apm/agents/java-agent/frameworks/scala-installation-java/#using-the-scala-api), le recomendamos que actualice su API de dependencia a la última versión. La versión más nueva del agente seguirá siendo compatible con la API, pero es posible que le falten nuevas características. Si está utilizando una herramienta de compilación como maven, actualice las versiones de API a la última. Por ejemplo, si estás usando la API agente de Java con gradle, reemplaza `${agent.version}` con la última versión:

   ```groovy
   implementation 'com.newrelic.agent.java:newrelic-api:${agent.version}'
   ```

5. Reinicie su proceso Java.
