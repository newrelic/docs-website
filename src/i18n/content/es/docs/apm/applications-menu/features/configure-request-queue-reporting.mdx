...
El agente PHP solo admite el encabezado `X-Request-Start` . Esto identifica la timestamp en microsegundos como un número entero, con un `t=` opcional en el valor del encabezado. Para asegurarse de que el encabezado se lea correctamente, verifique su `phpinfo()` en la sección <DoNotTranslate>**PHP Variables**</DoNotTranslate> y verifique que `_SERVER["HTTP_X_REQUEST_START"]` exista y tenga el formato esperado.
