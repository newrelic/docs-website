...
<Callout title="La API REST para despliegues está llegando al final de su vida útil" variant="important">
  Los extremos de despliegue de la API REST v2 (`/v2/applications/{app_id}/deployments.json`) documentados en esta página, junto con la API de despliegue v0 legacy, [alcanzan el fin de vida útil el 31 de julio de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Después de esa fecha, no estarán disponibles.

  En su lugar, registre los despliegues con la característica de [seguimiento de cambios](/docs/change-tracking/overview/) mediante la mutación `changeTrackingCreateEvent` de NerdGraph. Para los mapeos extremo por extremo, consulte la guía [Migrar de la API REST v2 a NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). Las instrucciones de REST a continuación se mantienen como referencia para las integraciones existentes que está migrando.
</Callout>

<Callout variant="tip">
  El seguimiento de cambios está disponible para <InlinePopover type="apm" />, <InlinePopover type="browser" />y <InlinePopover type="mobile" />.
</Callout>
...
Puede registrar nuevos despliegues y recuperar una lista de despliegues anteriores con la [API REST v2](/docs/apis/rest-api-v2/requirements/new-relic-rest-api-v2-getting-started) de New Relic o con [NerdGraph](/docs/change-tracking/config/nerdgraph/). Utilice NerdGraph: los extremos de despliegue de la API REST v2 [llegan al final de su vida útil el 31 de julio de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Además, algunos agentes APM ofrecen métodos específicos del agente para registrar despliegues automáticamente.
...
<Callout title="Estos extremos REST llegan al final de su vida útil el 31 de julio de 2027" variant="important">
  Los extremos de despliegue de la API REST v2 que se muestran a continuación [alcanzan el final de su vida útil el 31 de julio de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2) y dejarán de funcionar después de esa fecha. Migre a la mutación `changeTrackingCreateEvent` de NerdGraph, consulte la guía [Migrar de la API REST v2 a NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). Estas instrucciones permanecen como referencia mientras migra las integraciones existentes.
</Callout>

Puede utilizar la API REST v2 de New Relic para registrar el despliegue y obtener una lista del despliegue anterior.

* Los ejemplos en este documento usan `curl` como herramienta de línea de comando. Sin embargo, puede usar cualquier método para realizar sus requests REST. También puede crear y ver despliegues con el [API Explorer](https://api.newrelic.com/docs/#/Applications/post_applications__application_id__deployments_json).
...
<Callout variant="important">
  Los métodos de la API REST v2 mencionados a continuación [llegan al final de su vida útil el 31 de julio de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Registre los despliegues con la mutación `changeTrackingCreateEvent` de NerdGraph en su lugar; consulte la [guía de migración](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments).
</Callout>

Algunos agentes tienen métodos adicionales para grabar el despliegue:

* <DNT>**All agents**</DNT>: utilice la [API REST v2](#api-instructions) de New Relic.
* <DNT>**C**</DNT>: sin métodos específicos del SDK. Utilice la [API REST](#api-instructions).
* <DNT>**Go**</DNT>: sin métodos específicos del agente. Utilice la [API REST](#api-instructions).
...
* <DNT>**Node.js**</DNT>: sin métodos específicos del agente. Utilice la [API REST](#api-instructions).
