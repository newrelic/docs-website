...
<CollapserGroup id="non-scripted-errors">
  <Collapser className="freq-link" id="exceed-duration" title="ERROR: El trabajo se agotó después de 65 segundos">
    ### Problema
  ...
  <Collapser className="freq-link" id="xxx-response" title="HTTPError: el servidor respondió con el código de respuesta HTTP XXX">
    ### Problema
  ...
  <Collapser className="freq-link" id="ssl-error" title="Error de verificación SSL: (ERROR)">
    ### Problema
  ...
  <Collapser className="freq-link" id="response-error" title="ResponseValidationError: la respuesta no contenía la cadena esperada">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-timeout" title="Error de red: se agotó el tiempo de lectura">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-socket-closed" title="Error de red: el socket está cerrado">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-host-unreachable" title="NetworkError: No hay ruta al host (Host inalcanzable)">
    ### Problema
    ...
    Si la falla ocurre en una ubicación privada de monitoreo sintético, asegúrese de que la configuración de red del Synthetics Job Manager esté configurada correctamente y que el nombre de host de destino pueda resolverse y sea accesible a través de la interfaz de línea de comandos virtual del Synthetics Job Manager.
  ...
  <Collapser className="freq-link" id="httperror-redirects" title="HTTPError: el servidor nos envió demasiadas redirecciones (20)">
    ### Problema
  ...
  <Collapser className="freq-link" id="networkerror-resolution" title="NetworkError: Error en la resolución de DNS para el host: (HOST)">
    ### Problema
    ...
    <DNT>**Private synthetic monitoring’s locations:**</DNT> Confirme que la configuración de red para el Synthetics Job Manager sea correcta. Si el nombre de host de destino es interno, asegúrese de que el Synthetics Job Manager esté utilizando el servicio DNS interno de su red que pueda resolver el host. El Synthetics Job Manager y los contenedores runner que genera en el host (para ejecutar checks que no son de ping) deben heredar la configuración de DNS del host `/etc/resolv.conf`.

    <DNT>**Docker:**</DNT> Los argumentos de red como `–dns` o `-network` utilizados en el comando Docker run en el Synthetics Job Manager solo serán utilizados por el contenedor del job manager, pero no por los contenedores runner. Si el DNS apunta a la interfaz loopback como `127.0.0.1`, define una configuración de DNS a [nivel del daemon de Docker](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-configuration-file), o utiliza una herramienta como `dnsmasq` para asegurarte de que el runner reenvíe las solicitudes de DNS en la interfaz bridge de Docker.
    ...
    Si observa fallas de monitoreo relacionadas con la resolución de DNS en una ubicación privada de monitoreo sintético, esto suele deberse a que el Synthetics Job Manager de esa ubicación tiene una configuración de red no válida.
  </Collapser>

  <Collapser className="freq-link" id="blockedrequesterror-url" title="Error de solicitud bloqueada: (URL)">
    ### Problema
