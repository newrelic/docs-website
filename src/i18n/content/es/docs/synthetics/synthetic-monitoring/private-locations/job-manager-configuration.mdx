...
## Consideraciones de tamaño para Docker, Podman, Kubernetes y OpenShift [#kubernetes-sizing]

### Docker y Podman [#docker]
...
**La ecuación:** $$C\_req = (J\_processed + Q\_growth) \times D\_j$$

* $C\_req$ = **Núcleos de CPU requeridos**
* $J\_processed$ = La tasa de trabajos que se **procesan** por minuto.
* $Q\_growth$ = La velocidad a la que su cola `jobManagerHeavyweightJobs` **crece** por minuto.
* $D\_j$ = La **duración promedio** de un trabajo en minutos.
...
$$C\_req = N\_m \veces D\_j \veces \frac1P\_m$$

* $C\_req$ = **Núcleos de CPU requeridos**
* $N\_m$ = La **cantidad** total de monitores pesados que planea ejecutar.
* $D\_j$ = La **duración promedio** de un trabajo en minutos.
* $P\_m$ = El **periodo** del monitor en minutos (por ejemplo, un monitor que se ejecuta cada 5 minutos tiene un periodo de 5).
...
* **Duración del trabajo ($D\_j$):** su promedio debe incluir trabajos que **expiran** (generalmente \~3 minutos), ya que estos mantienen un núcleo durante toda su duración.
...
**1. Buscar trabajos procesados por minuto ($J\_processed$):** esta consulta cuenta la cantidad de trabajos sin ping (de gran peso) completados durante el último día y muestra la tasa promedio por minuto.
...
**3. Encontrar la duración promedio del trabajo en minutos ($D\_j$):** esta consulta encuentra la duración promedio de ejecución de trabajos sin ping completados y convierte el resultado de milisegundos a minutos. `executionDuration` representa el tiempo que tardó el trabajo en ejecutar en el host.
...
**4. Encontrar el número total de monitores pesados ($N\_m$):** esta consulta encuentra el recuento único de monitores pesados.
...
**5. Encuentre la frecuencia promedio de monitorización de peso pesado ($F\_j$):** si la cola `jobManagerHeavyweightJobs` de la ubicación privada está creciendo, no es preciso calcular la frecuencia promedio de monitorización a partir de los resultados existentes. Esto deberá estimar a partir de la lista de monitores en la página [Monitores Sintéticos](https://one.newrelic.com/synthetics). Cerciórate de seleccionar la cuenta New Relic correcta y es posible que debas filtrar por `privateLocation`.
...
Una consideración clave al dimensionar sus tiempos de ejecución es que una sola instancia de SJM tiene un rendimiento máximo de **aproximadamente 15 trabajos pesados por minuto** (API con scripts y monitores de navegador). Esto se debe a una estrategia de subprocesamiento interno que favorece la competencia eficiente de trabajos entre múltiples SJM sobre la cantidad bruta de trabajos procesados por SJM.

Puede emplear la duración promedio del trabajo para calcular el `parallelism` efectivo máximo para un solo SJM antes de alcanzar este límite de rendimiento:

$$Paralelismo\_máximo \aprox 15 \veces \mathrmajd\_m$$

Donde $\mathrmajd\_m$ es la duración promedio del trabajo en minutos.

Si sus necesidades de monitoreo exceden este límite de \~15 trabajos/minuto, debe **escalar horizontalmente** implementando múltiples instancias de SJM. Puede [verificar si su cola de trabajos está creciendo](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-maintenance-monitoring/) para ver si se necesitan más instancias.

La configuración `parallelism` controla cuántos pods de un entorno de ejecución en individua se ejecutan simultáneamente y es el equivalente de la variable de entorno `HEAVYWEIGHT_WORKERS` en Docker y Podman SJM. La configuración `completions` controla cuántos pods de un tiempo de ejecución particular deben completar antes de que `CronJob` pueda iniciar otro trabajo Kubernetes para ese tiempo de ejecución. Para mejorar la eficiencia, `completions` debe establecer entre 6 y 10 veces el valor de `parallelism`.

Las siguientes ecuaciones se pueden emplear como punto de partida para `completions` y `parallelism` para cada tiempo de ejecución.

$$Compleciones = \frac300ajd\_s$$

Donde $\mathrmajd\_s$ es la duración promedio del trabajo en segundos.

$$Paralelismo = \fracsj\_5mCompleciones$$

Donde $\mathrmsj\_5m$ es el número de trabajos Sintéticos cada 5 minutos.

La siguiente consulta se puede emplear para obtener la duración promedio y la tarifa para una ubicación privada.
...
#### Escalamiento horizontal con múltiples instancias de SJM

Para lograr un mayor rendimiento total, puede instalar varias versiones de SJM Helm en el mismo namespace Kubernetes. Cada SJM competirá por trabajos desde la misma ubicación privada, lo que proporciona equilibrio de carga, protección contra conmutación por error y un mayor rendimiento total de trabajos.

Al instalar varias versiones de SJM, debe proporcionar un nombre único para cada versión. Todas las instancias deben configurar con la **misma clave de ubicación privada** en su archivo `values.yaml`. Si bien no es obligatorio, se recomienda configurar `fullnameOverride` para crear nombres de recursos más cortos y manejables.

Por ejemplo, para instalar dos SJM llamados `sjm-alpha` y `sjm-beta` en el namespace `newrelic` :
...
Puede continuar este patrón para tantos SJM como sea necesario para evitar que la cola de trabajos crezca. Para cada SJM, configure `parallelism` y `completions` en un valor razonable basado en la duración promedio del trabajo y el límite de \~15 trabajos por minuto por instancia.