...
$$C\_req = (R\_proc + R\_crecimiento) \times D\_avg,m$$
...
$$C\_req = N\_mon \veces D\_promedio,m \veces \frac1P\_promedio,m$$
...
**1. Encuentre la tasa de trabajos procesados por minuto ($R\_proc$):** esta consulta cuenta la cantidad de trabajos sin ping (de gran peso) completados durante el último día y muestra la tasa promedio por minuto.
...
**4. Encontrar la duración promedio del trabajo en minutos ($D\_avg,m$):** esta consulta encuentra la duración promedio de ejecución de trabajos sin ping completados y convierte el resultado de milisegundos a minutos. `executionDuration` representa el tiempo que tardó el trabajo en ejecutar en el host.
...
$$Paralelismo*máximo \aprox. 15 \veces D*promedio,m$$

Donde $D\_avg,m$ es la **duración promedio del trabajo pesado** en **minutos**.
...
$$Compleciones = \frac300D\_avg,s$$

Donde $D\_avg,s$ es la **duración promedio del trabajo** en **segundos**.

$$Paralelismo = \fracN\_mCompleciones$$

Donde $N\_m$ es la **cantidad** de trabajos Sintético que necesita ejecutar cada **5 minutos.**
