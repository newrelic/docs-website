    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Para las cuentas basadas en EE. UU. (predeterminado), el extremo es: `https://synthetics-horde.nr-data.net.`
            ...
            Para las cuentas basadas en JP, el extremo es: `https://synthetics-horde.jp.nr-data.net/`
...
Se proporcionan módulos de nodo personalizados en el SJM. Le permiten crear un conjunto personalizado de [módulos de Node](https://docs.npmjs.com/about-packages-and-modules) y utilizarlos en monitores de script (API con script y navegador con script) para el monitoreo sintético.
...
## Configurar certificados de CA personalizados para ubicaciones privadas [#custom-ca-certificates]

Si sus monitores Sintéticos necesitan conectarse a extremos HTTPS internos protegidos por una autoridad de certificación (CA) privada, puede montar sus certificados de CA personalizados directamente en el SJM.

El SJM detecta automáticamente estos certificados y los pasa a cada contenedor de tiempo de ejecución al inicio. Esto garantiza que tus monitores confíen en tus extremos internos sin requerir que compiles o mantengas imágenes de tiempo de ejecución personalizadas.

### Requisitos de certificados

Para garantizar que el SJM cargue tus certificados correctamente, deben cumplir con los siguientes criterios:

* **Formato:** los certificados deben estar en formato PEM con una extensión de archivo `.pem`.
* **Permisos:** los archivos deben ser legibles por el UID 2000 (el usuario no root que ejecuta el contenedor de tiempo de ejecución).
* **Tipo de certificado:** solo se cargan los certificados de CA. Cualquier certificado de hoja (certificados de servidor o de cliente) que carezca de la extensión de restricción básica `CA:TRUE` se ignora automáticamente.

### Entornos de despliegue

Selecciona tu plataforma de orquestación de contenedores a continuación para montar tus certificados.

<CollapserGroup>
  <Collapser id="custom-ca-docker" title="Docker">
    Monte el directorio del host local que contiene sus archivos PEM en la ruta del contenedor del SJM `/var/lib/newrelic/synthetics/certs` como un volumen de solo lectura.
    ...
    **Cómo funciona esto:** el SJM detecta este montaje en el inicio y propaga los certificados como de solo lectura a cada contenedor dinámico que genera.
  </Collapser>

  <Collapser id="custom-ca-podman" title="Podman">
    Para entornos de Podman —especialmente aquellos que se ejecutan con SELinux habilitado—, debe agregar la bandera `:z` o `:Z` al montaje del volumen para configurar el contexto de seguridad correcto.
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes (Helm)">
    Al desplegar SJM en un clúster de Kubernetes mediante Helm, puede gestionar su paquete de CA personalizado como un secreto genérico de Kubernetes.

    **1. Crear el secreto de Kubernetes**

    Guarde su(s) archivo(s) de certificado PEM en un secreto dentro del namespace del agente de New Relic:
    ...
    **2. Actualizar sus valores de Helm**

    Haz referencia al secreto recién creado en tu configuración de `values.yaml` personalizada:
    ...
    **Cómo funciona esto:** en el inicio, el SJM monta este secreto y lo pasa automáticamente a todos los pods de ejecución recién generados.
  </Collapser>
</CollapserGroup>

## Certificados de cliente mTLS [#mtls-client-certificates]

Si sus monitores se conectan a extremos internos que aplican TLS mutuo (mTLS) — lo que requiere que el cliente presente un certificado —, puede proporcionar certificados de cliente mapeados a nombres de host objetivo. Los monitores presentan el certificado correcto automáticamente. No se requieren cambios en el script.

**Requisitos:**

* Los archivos de certificado y clave deben estar en formato PEM
* Se requiere un archivo `cert-map.json` de mapeo de nombres de host a archivos de certificado y clave

**Formato de cert-map.json:**
...
Prioridad de coincidencia del nombre de host: coincidencia exacta → comodín (`*.domain.com`) → `default` catch-all → sin certificado (las conexiones no mTLS no se ven afectadas).

<CollapserGroup>
  <Collapser id="mtls-docker" title="Docker">
    Monte un directorio que contenga `cert-map.json` y los archivos de certificado y clave referenciados:
    ...
    Si los nombres de host del servidor mTLS no se pueden resolver a través de DNS dentro de los contenedores de tiempo de ejecución, agregue mapeos de nombres de host usando `RUNTIME_EXTRA_HOSTS`:
  ...
  <Collapser id="mtls-podman" title="Podman">
    Monte el directorio de certificados del cliente. Para SELinux, agregue `:z` o `:Z`:
  ...
  <Collapser id="mtls-kubernetes" title="Kubernetes">
    Cree un secreto de Kubernetes que contenga `cert-map.json` y todos los archivos de certificados y claves referenciados:
    ...
    Haga referencia al secreto en sus valores de Helm:
    ...
    El secreto se monta como solo lectura en todos los pods de tiempo de ejecución automáticamente.
  </Collapser>
</CollapserGroup>

## Limitaciones del soporte de certificados [#certificates-support-limitations]

Las siguientes limitaciones se aplican tanto a los certificados de CA personalizados como a los certificados de cliente mTLS, a menos que se indique lo contrario:

* **Reinicio requerido para cambios de certificado** — Los certificados se cargan al inicio del contenedor. Después de agregar, eliminar o rotar certificados, reinicie el contenedor del administrador de trabajos (o los pods de tiempo de ejecución en Kubernetes) para que los cambios surtan efecto.
* **Tiempos de ejecución compatibles** — Las características de certificados personalizados se aplican solo a la API con script y a los monitores de browser con script. Los monitores de ping no se ven afectados.
* **CN de sujeto único por certificado (solo mTLS)** — Cada certificado de cliente debe tener un nombre común (CN) de sujeto único. Chrome usa el CN para identificar y seleccionar automáticamente el certificado correcto por nombre de host.
...
<CollapserGroup>
  <Collapser id="docker-storage" title="Para Docker">
  ...
  <Collapser id="podman-storage" title="Para Podman">
  ...
  <Collapser id="kubernetes-storage" title="Para Kubernetes">
    ### Kubernetes
...
## Consideraciones de tamaño [#vm-sizing]

<CollapserGroup>
  <Collapser id="docker-podman-sizing" title="Para Docker y Podman">
    ...
    * $R\_\{growth}$ = La **tasa** a la que su cola `jobManagerHeavyweightJobs` está **creciendo** por minuto.
    * $D\_\{avg,m}$ = La **duración promedio** de los trabajos pesados en **minutos**.
    ...
    **2. Encuentre la tasa de crecimiento de la cola por minuto ($R\_\{growth}$)**: Esta consulta calcula el crecimiento promedio por minuto de la cola `jobManagerHeavyweightJobs` en un gráfico de series de tiempo. Una línea por encima de cero indica que la cola está creciendo, mientras que una línea por debajo de cero significa que se está reduciendo.
    ...
    **4. Encontrar la duración promedio del trabajo en minutos ($D\_\{avg,m}$)**: Esta consulta encuentra la duración promedio de ejecución de los trabajos completados que no son de ping y convierte el resultado de milisegundos a minutos. `executionDuration` representa el tiempo que tardó el trabajo en ejecutarse en el host.
    ...
    **5. Encuentre el período promedio del monitor de peso pesado ($P\_\{avg,m}$):** Si la cola de `jobManagerHeavyweightJobs` de la ubicación privada está aumentando, no es preciso calcular el período promedio del monitor a partir de los resultados existentes. Esto deberá estimarse a partir de la lista de monitores en la página [Monitores sintéticos](https://one.newrelic.com/synthetics). Asegúrese de seleccionar la cuenta de New Relic correcta y es posible que deba filtrar por `privateLocation`.
  ...
  <Collapser id="k8s-sizing" title="Para Kubernetes y OpenShift">
    ...
    **Paralelismo estimado:** Esto determina cuántos trabajadores (pods) necesita que se ejecuten simultáneamente para manejar su carga de trabajo de 5 minutos.
    ...
    $$ P\_\{est} = \frac\{N\_m}\{Completions} $$

    Donde $N\_m$ es su **número de trabajos por cada 5 minutos**. Este valor $P\_\{est}$ es su **paralelismo estimado**.
