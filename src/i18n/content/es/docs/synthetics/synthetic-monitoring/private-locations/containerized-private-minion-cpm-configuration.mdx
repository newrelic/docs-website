...
* Configure [módulos personalizados](#custom-modules) para [el browser con secuencias de comandos](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers) en New Relic.
* Conserve los datos de lanzamiento con [almacenamiento de datos permanente](#preserve-data-volume).
* Utilice [variables de entorno](#environment-variables) en su configuración.
...
A todos los directorios y archivos <DoNotTranslate>**must**</DoNotTranslate> se les asignará propiedad de grupo como `3729` con permisos de lectura/escritura. Esto garantiza que el Runner, que utiliza `uid: 1000` y `gid: 3729`, tenga acceso a todos los volúmenes montados. Sin embargo, el minion puede ejecutarse como root (`uid: 0`) o con cualquier `uid` entre el rango de `[2000, 4000]`, inclusive. Para obtener más información, consulte Ejecutar como [no root en Kubernetes](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#kubernetes-env-config) o [Docker](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#docker-env-config).
...
Junto con los módulos locales y alojados, también puede utilizar [módulos de Node.js.](/docs/synthetics/new-relic-synthetics/scripting-monitors/import-nodejs-modules) Para cambiar los módulos personalizados utilizados por tus llamadas por minuto, modifica `package.json` y reinicia las llamadas por minuto. Detectará el cambio en la configuración durante el reinicio y luego lo limpiará y lo reinstalará.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DoNotTranslate>**REQUIRED.**</DoNotTranslate> UUID de la ubicación privada, tal como se encuentra en la página web de ubicación privada.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            Asegúrese de que sus llamadas por minuto puedan conectarse al extremo apropiado para poder servir a su monitor.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            El estado deshabilitado minion Network Healthcheck, para administrar las llamadas por minuto y verificar el acceso público a Internet. El valor predeterminado es "falso", cuando se establece como "verdadero", las llamadas por minuto omitirán esta verificación de estado.
          </td>
        </tr>
