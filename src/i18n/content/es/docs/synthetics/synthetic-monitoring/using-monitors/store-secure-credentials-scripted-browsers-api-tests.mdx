...
Puedes configurar credenciales seguras en New Relic o con la [API de Nerdgraph](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials). Las credenciales se almacenan de forma segura utilizando cifrado AES-GCM de 256 bits en reposo con claves administradas por [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/).
...
Puede agregar o actualizar credenciales seguras utilizando la interfaz de usuario o la [API NerdGraph de monitoreo sintético](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials). Nota: no se pueden ver los valores, solo las claves.
...
2. Para agregar una nueva credencial segura, busque el botón <DNT>**Create secure credential +**</DNT> . Si ya tiene credenciales agregadas, este botón está en la parte superior derecha.
   ...
   * Consejos para crear <DNT>**Value**</DNT>: Utilice cualquier combinación de caracteres alfanuméricos o especiales. 10000 caracteres como máximo. Este campo no es accesible a través de [la API de Nerdgraph](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials).
