...
En APM, algunos gráficos tienen la opción de [ver la consulta NRQL que generó ese gráfico](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language). Este es un buen punto de partida para comprender cómo consultar métricas. La consulta NRQL a continuación está ligeramente modificada del gráfico de tasa de errores en la <InlinePopover type="apm" />página de resumen.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        Esta métrica tiene tres atributos que representan los valores de datos codificados en el nombre de la métrica, `datastoreType`, `table` y `operation`:
...
## Consultas genéricas con el `newrelic.timeslice.value` [#generic-queries]
...
<CollapserGroup>
  <Collapser id="multiple-app-names" title="Faceta por varios nombres de aplicaciones">
  ...
  <Collapser id="rate" title="Tasa de rendimiento por minuto">
  ...
  <Collapser id="facet-by-host" title="Faceta por atributo relacionado con el host">
  ...
  <Collapser id="average" title="Consulta de duración media de una métrica">
  ...
  <Collapser id="average" title="Desglose el tiempo promedio dedicado a una transacción por los segmentos que la componen">
