...
## IU de ingesta de datos [#ui]

La pestaña <DNT>**Data ingestion**</DNT> se encuentra en la [IU de gestión de datos](/docs/data-apis/manage-data/manage-your-data). Muestra un resumen de tu uso de datos por origen, lo que te ayuda a entender qué datos se están enviando a New Relic.
...
La interfaz de usuario de ingesta de datos proporciona herramientas para ayudarle a monitorear y analizar sus datos:

<DNT>**Filters and controls:**</DNT>

* <DNT>**Time range selector:**</DNT> Elija entre las vistas **Auto**, **Daily**, **Weekly** o **Monthly** para analizar las tendencias de ingesta de datos durante el período de tiempo de su preferencia.

<Callout variant="important">
  La vista <DNT>**Auto**</DNT> no admite el selector de tiempo. Debido a que muestra sus datos en intervalos de 5 minutos, puede mostrar hasta un día a la vez. Para ver un rango de tiempo más largo, cambie a las vistas <DNT>**Daily**</DNT>, <DNT>**Weekly**</DNT> o <DNT>**Monthly**</DNT>.
</Callout>

* <DNT>**Data source filtering:**</DNT> Usa el menú desplegable y la función de búsqueda para filtrar y encontrar fuentes de datos específicas.

<DNT>**Visualizations:**</DNT>

* <DNT>**Data source table:**</DNT> Ver todas las fuentes de datos activas con sus respectivas cantidades de ingesta. La tabla muestra la contribución de cada origen a tu ingesta total de datos (como **BrowserEventsBytes**, **CustomEventsBytes**, **ApmEventsBytes**).
* <DNT>**Visual trend chart:**</DNT> Muestra patrones de ingesta a lo largo del tiempo con bandas codificadas por colores que representan diferentes fuentes de datos, lo que facilita la identificación de picos o tendencias.
* <DNT>**Usage by account:**</DNT> Un gráfico circular que muestra la distribución de la ingesta de datos en sus cuentas, presentando tanto el volumen total como el desglose porcentual.
* <DNT>**Usage by data source:**</DNT> Un gráfico circular que desglosa su ingesta total por fuente de datos (por ejemplo, **BrowserEventsBytes** podría mostrar el 95.17% de su ingesta total, y **CustomEventsBytes** el 3.33%).

Para obtener definiciones detalladas de las métricas mostradas, consulte [Fuentes de ingesta de datos](#sources-list).

<Callout variant="important">
  **Filtrado de cuentas multi-tenant**

  Para las organizaciones con cuentas multi-inquilino, puede filtrar la ingesta de datos por organización o cuenta usando los menús desplegables **Select Organization** y **Select Account**. Esto le permite ver los datos de todas las organizaciones y cuentas, o limitarlos a algunas específicas.
</Callout>

### Obtener más detalles [#detail]

Para ver detalles adicionales sobre una fuente de datos específica:

1. Haz clic en el nombre del origen de datos en la tabla
2. Utilice las opciones de filtrado adicionales que aparecen para profundizar aún más en esa fuente de datos específica.

<DNT>**Advanced options:**</DNT>

* Haz clic en <DNT>**View NRQL**</DNT> para ver la consulta subyacente de cualquier gráfico
* Consulta [Configuración de la cuenta](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings/#video-tour) para ver un recorrido en video de esta y otras interfaces de usuario de administración
* Para obtener detalles técnicos sobre cómo los gráficos muestran datos y mensajes de error, consulte [Detalles del gráfico](#technical-details-chart)

## Estimar la ingesta futura de datos [#estimate]

La página <DNT>**Data Ingestion**</DNT> incluye capacidades predictivas que le ayudan a pronosticar el uso futuro de datos de su organización.
...
3. Ver el pronóstico de uso de datos para 1 semana tanto en el gráfico como en la tabla

Para eliminar el pronóstico, seleccione <DNT>**Remove prediction**</DNT> del menú del gráfico.

<Callout variant="important">
  La previsión solo está disponible para los clientes <DNT>**Advanced Compute**</DNT> y solo en el <DNT>**Daily view**</DNT> del <DNT>**Data source time series chart**</DNT>.
</Callout>
