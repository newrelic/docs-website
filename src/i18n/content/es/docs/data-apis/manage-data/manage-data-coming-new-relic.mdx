...
## UIusuario de ingesta de datos [#ui]

La pestaña <DNT>**Data ingestion**</DNT> se encuentra en la [UIusuario de administración de datos](/docs/data-apis/manage-data/manage-your-data). La UI de ingesta de datos muestra un resumen de su uso por fuente de datos. Para organizaciones con varias cuentas, también puede ver el uso de cuentas específicas.
...
* <DNT>
  ...
  Si le preocupan escenarios específicos que causan picos repentinos en el conjunto de datos, active una condición de alerta para notificarle si eso ocurre. Para obtener consejos al respecto, consulte [Consulta de uso](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/usage-queries-alerts).
...
<CollapserGroup>
  <Collapser
    ...
    title="Administrar la ingesta de APM"
  >
  ...
  <Collapser
    ...
    title="Gestionar la ingesta de infraestructura"
  >
  ...
  <Collapser
    ...
    title="Administrar la ingesta log"
  >
    ...
    * [Reenvío de datoslog ](/docs/logs/forward-logs/enable-log-management-new-relic): los registros sin filtrar a veces pueden generar una gran cantidad de datos log . Puede ajustar la configuración del reenviador de registros para filtrar el registro de eventos desde el lado de envío log .
  ...
  <Collapser
    id="npm"
    title="Administrar Monitoreo de datos rojos"
  >
  ...
  <Collapser
    id="disable"
    title="Deshabilitar o eliminar herramientas de monitoreo"
  >
...
El gráfico [UIde ingesta de datos](#data-ingest-ui) le muestra un desglose de alto nivel de su uso de datos facturables. La siguiente tabla explica esas fuentes. En esta tabla, "grupo métrico de uso" se refiere al valor del atributo `usageMetric` de esa fuente en el evento `NrConsumption` .
...
<CollapserGroup>
  ...
  >
    La actividad de seguimiento de cambios se almacena en el namespace <DNT>**Deployment**</DNT>. Los cambios individuales que usted rastrea tienen un promedio de alrededor de 200 a 250 bytes para su ingesta.
  </Collapser>
