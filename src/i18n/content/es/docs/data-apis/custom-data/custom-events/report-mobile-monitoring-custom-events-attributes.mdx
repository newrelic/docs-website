...
Para ayudar con el análisis de fallos, puede utilizar el SDK para crear eventos `MobileBreadcrumb` y `MobileHandledException` . Estos eventos están disponibles para consulta y también se muestran en la [UI de la traza de evento](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/mobile-crash-event-trail).
...
<CollapserGroup>
  <Collapser id="record-purchase" title="Ejemplo de evento personalizado: seguimiento de compras">
  ...
  <Collapser id="track-user" title="Ejemplo de atributo: seguimiento de un usuario específico">
  ...
  <Collapser id="track-store" title="Ejemplo de atributo: realizar un seguimiento de una identificación de tienda específica">
  ...
  <Collapser id="track-actions" title="Ejemplo de atributo personalizado: realizar un seguimiento de una acción específica">
...
* Una sesión ha estado en curso durante 60 segundos.
