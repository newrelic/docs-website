...
<Callout variant="important">
  Revise la lista de [términos reservados utilizados por NRQL](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#reserved-words). El uso de términos reservados puede causar problemas.
</Callout>
...
<CollapserGroup>
      ...
      <tbody>
        <tr id="xml-java">
          ...
          <td>
            ...
            4. Defina un archivo de instrumentación XML utilizando la [UI de New Relic](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-quickly-customize-your-java-instrumentation). Esto puede requerir una configuración adicional en el bloque `common:` de tu <DNT>**newrelic.yml**</DNT>. Consulte <DNT>**Report custom attributes**</DNT> en [Opciones de instrumentación](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-instrument-ui#options) para obtener más detalles.
          </td>
        </tr>
  ...
  <Collapser id="net-att" title=".NET">
    ...
    1. Para cada método para el cual desee registrar un atributo, llame a [`AddCustomAttribute`](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#AddCustomAttribute).
  ...
  <Collapser id="ruby-att" title="Ruby">
    La recolección de atributos personalizados está habilitada por defecto en Ruby. Para recopilar atributos personalizados, llame al método [`add_custom_attributes`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#add_custom_attributes-instance_method). Por ejemplo, para registrar una variable llamada `@user_id`, incluya este código en el método padre:
...
## Monitoreo de navegador: Registro de atributo personalizado [#collecting\_browser][#collecting_browser]

El agente de navegador proporciona una API para especificar detalles adicionales asociados con eventos del navegador durante la carga de una página, ya sea [reenviando atributos de APM al monitoreo de navegador](/docs/insights/insights-data-sources/custom-data/insert-data-via-new-relic-browser#custom-attribute-forward-apm) o [especificando atributos personalizados mediante JavaScript](/docs/browser/new-relic-browser/browser-agent-spa-api/set-custom-attribute). Los valores reenviados desde el agente de APM son codificados e inyectados en los atributos del navegador por nuestro agente de navegador.

## Monitoreo de infraestructura: Registro de atributos personalizados [#collecting\_browser][#collecting_browser]
...
## Monitoreo de móviles: Registro de atributos personalizados [#collecting\_mobile][#collecting_mobile]

El agente móvil incluye API de llamada para registrar el atributo personalizado:

* Para obtener una descripción general de <InlinePopover type="mobile" />datos personalizados, consulte [Insertar evento personalizado y atributo](/docs/insights/insights-data-sources/custom-events/insert-custom-events-attributes-mobile-data)
