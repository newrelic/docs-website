...
<CollapserGroup>
  <Collapser id="ruby-example" title="Llamar a la API de eventos desde una aplicación Ruby">
...
<Callout variant="tip">
  Si su organización aloja datos en el centro de datos de la UE o JP, asegúrese de usar los extremos regionales adecuados. Consulte los [extremos de la región de la UE](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#eu-endpoints) o los [extremos de la región de JP](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#jp-endpoints).
</Callout>
...
2. Antes de crear eventos personalizados o atributos, revise nuestra lista de [términos reservados utilizados por NRQL](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#reserved-words).
3. [Genere JSON](#instrument) para el evento instrumentando su aplicación, consultando una API o algún otro método.
4. [Envíe una carga útil JSON comprimida](#submit-event) (por ejemplo, `gzip` o `deflate`) al extremo HTTPS usando curl en una solicitud <DNT>**POST**</DNT> .
5. Recomendación: configure [condiciones de alerta de NRQL](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) para que se le notifique cuando ocurran [errores de análisis](#errors-parsing).
...
<CollapserGroup>
  <Collapser id="json-guidelines" title="Directrices de formato JSON">
  ...
  <Collapser id="json-example" title="Ejemplo JSON">
  ...
  <Collapser id="limits" title="Límites y personajes restringidos">
...
<CollapserGroup>
  <Collapser id="numberone" title="Ejemplo de Linux/bash">
    ```bash
  ...
  <Collapser id="numbertwo" title="Ejemplo de Windows/PowerShell">
    ```powershell
...
* El `Api-Key` contiene el <InlinePopover type="licenseKey" />correcto.
...
Recomendación: además de verificar si hay un mensaje de éxito, cree una consulta NRQL de sus datos para verificar que estén disponibles.

<CollapserGroup>
  <Collapser id="success" title="Código de respuesta exitosa">
    <table>
      <thead>
        <tr>
  ...
  <Collapser id="errors-submission" title="Errores de envío">
  ...
  <Collapser id="errors-parsing" title="Errores de análisis">
...
El [evento`NrIntegrationError` ](/docs/telemetry-data-platform/manage-data/nrintegrationerror)te permite consultar y configurar <InlinePopover type="alerts" />en los datos personalizados que se envían a tu cuenta de New Relic. Recomendación: para obtener alertas de errores de análisis, [crea una condición de alerta NRQL](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) para `NrIntegrationError`. Usa esta consulta NRQL de ejemplo:
