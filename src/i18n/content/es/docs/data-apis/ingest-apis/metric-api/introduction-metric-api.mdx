...
Asegúrese de que la conectividad saliente en el puerto TCP 443 esté permitida para el [rango CIDR](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure) que coincida con su [región](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers). El método de configuración preferido es usar el nombre DNS `metric-api.newrelic.com` (EE. UU., por defecto), `metric-api.eu.newrelic.com` (UE) o `metric-api.jp.nr-data.net` (JP).
