...
Los logs del navegador se rastrean de forma predeterminada en el nivel `WARN` para los agentes <DNT>Pro</DNT> y <DNT>Pro+SPA</DNT>, pero no están disponibles para el agente Lite browser. Recomendamos que primero confirme que está utilizando el agente browser <DNT>**Pro**</DNT> o <DNT>**Pro + SPA**</DNT>.
...
* Un agente browser <DNT>**Pro**</DNT> o <DNT>**Pro + SPA**</DNT>:
...
<Steps>
  ...
  <Step>
    Asegúrese de que esté seleccionado el agente browser <DNT>**Pro**</DNT> o <DNT>**Pro + SPA**</DNT>.
  </Step>

  <Step>
    Activa **Automatically collect console logs** en **ON** para recopilar los mensajes enviados a la consola del navegador. <br />Activa **Collect logs manually** en **ON** para recopilar los logs enviados mediante programación a través de `newrelic.log()` o `newrelic.wrapLogger()`.
  </Step>
  ...
  <Step>
    ...
    * `ERROR` - recopilar logs con nivel `ERROR`
    * `WARN` - recopilar logs con el nivel `WARN` o `ERROR`
    * `INFO` - recopilar logs con el nivel `INFO`, `WARN` o `ERROR`
    * `DEBUG` - recopilar logs con el nivel `DEBUG`, `INFO`, `WARN` o `ERROR`
    * `TRACE` - recopilar logs con el nivel `TRACE`, `DEBUG`, `INFO`, `WARN` o `ERROR`
