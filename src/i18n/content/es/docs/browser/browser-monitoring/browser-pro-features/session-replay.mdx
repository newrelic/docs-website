...
<Callout variant="important">
  Session Replay se facturará a partir del 8 de mayo de 2024. Consulte [Consumo de datos](#plan-for-paid-consumption) para obtener más detalles.
</Callout>
...
* [Consumo de datos](#view-consumption): vea el consumo de datos y planifique la transición al consumo pago.
...
<Steps>
  ...
  <Step>
    ...
    * <DoNotTranslate>**User sessions**</DoNotTranslate> registra una muestra aleatoria de todas las sesiones de usuario.

    * <DoNotTranslate>**Errored session**</DoNotTranslate> registra una muestra aleatoria de todas las sesiones con errores. El agente almacena en buffer durante hasta 30 segundos en el lado del cliente y, si se producen errores, se grabará la sesión. Si no se produce un error, la repetición no se recopila.
  ...
  <Step>
    ...
    De forma predeterminada, todas las entradas del usuario se reemplazan con asteriscos. A continuación se muestra un ejemplo de cómo se ve una repetición con privacidad predeterminada:
    ...
    * Agregue manualmente nuestra clase CSS `nr-block` o atributo `data-nr-block` al HTML de su página web. Por ejemplo, si desea bloquear texto e imágenes irrelevantes de Session Replay, agregue `nr-block` a `<div class>`:
    ...
    Puede ignorar los cambios en un campo de entrada del usuario, lo que significa que el campo de entrada seguirá mostrándose en la repetición, pero no mostrará los cambios en el valor. Por ejemplo, podría ignorar la clase que contiene los campos de dirección de correo electrónico y contraseña.
...
Si es necesario, puede invocar mediante programación una Session Replay utilizando métodos API . A continuación se muestran algunos casos de uso:

* Permitir que el usuario opte por la grabación de interacciones. Por ejemplo, si aparece una ventana emergente que pregunta "¿Permitir que se grabe esta sesión para analizar el rendimiento?":

  1. Con el consentimiento del usuario, llame a `NREM.startSessionReplay` para iniciar la grabación.
  2. Una vez que se complete la sesión o se retire el consentimiento, use `NREM.stopSessionReplay` para dejar de capturar datos.

* Grabar sesiones según criterios específicos. Por ejemplo:
...
Para garantizar que todas las interacciones del usuario se capturen durante la grabación manual, establezca tasas de muestreo en 0% en la configuración de su aplicación.
...
Entre el 8 de abril y el 8 de mayo de 2024, aunque los bytes Session Replay son gratuitos, puedes ver tu consumo para ayudarte con la planificación una vez que los bytes se paguen:

1. Vaya a <DoNotTranslate>**Application Settings**</DoNotTranslate>.

2. Desplácese hacia abajo hasta <DoNotTranslate>**Session replay**</DoNotTranslate>.

3. En la sección <DoNotTranslate>**Session sample size**</DoNotTranslate> , haga clic en <DoNotTranslate>**View your account's consumption**</DoNotTranslate>.
...
### Plan de consumo pago [#plan-for-paid-consumption]

Después del 8 de mayo de 2024, Session Replay seguirá el mismo precio de consumo que los demás bytes de su browser . La cantidad de bytes producidos depende del recuento, la duración y los niveles de actividad del usuario de las sesiones, así como de la complejidad del DOM de su sitio.

La forma más precisa de proyectar su costo por repetición es habilitar la característica durante un período de prueba corto y medir su consumo real. Alternativamente, puedes usar un promedio de 5,3 MB por repetición como una aproximación aproximada, aunque ten en cuenta que el costo real puede variar significativamente. Una fórmula sencilla para proyectar sus costos aproximados es: `sessions` x `sampling_rate` x `gb_per_replay` x `cost_per_gb`

* Ejemplo: `sessions: 1 million` x `sampling_rate: 5%` x `gb_per_replay: 0.0053` x `cost_per_gb: $0.30` = `$79.50 for 50,000 replays`
...
Nos tomamos en serio la privacidad de los datos, por lo que creamos Session Replay con configuraciones de privacidad predeterminadas sólidas, diseñadas para limitar el posible intercambio de información confidencial de los clientes. Estas configuraciones [se pueden personalizar aún más](#configure-privacy-settings) para cumplir con sus requisitos de privacidad.

Session Replay no almacena capturas de pantalla ni vídeos. Solo registra los cambios de estado DOM esenciales necesarios para recrear la experiencia del usuario. Esto da como resultado que se almacene una cantidad mucho menor de datos.
