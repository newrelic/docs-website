...
La compatibilidad con mapas de origen es principalmente útil para "decodificar" JavaScript minimizado. JavaScript minimizado genera un error de rastreo del stack en su mayoría inútil en la página <DoNotTranslate>**Errors**</DoNotTranslate> del browser. La carga de mapas fuente convierte estos errores en un rastreo del stack comprensible, con referencias útiles a las líneas de código. Esta característica también puede resultar útil para código JavaScript empaquetado o transpilado.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Desde un marco de rastreo del stack expandido, seleccione <DoNotTranslate>**Remove file**</DoNotTranslate>.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  >
    <DoNotTranslate>**Generate source maps using UglifyJS:**</DoNotTranslate> Al "feear" archivos fuente, especifique un nombre de archivo de mapa fuente e incluya el contenido fuente original:
  ...
  >
    <DoNotTranslate>**Generate source maps using [Webpack](https://webpack.github.io/docs/configuration.html#devtool):**</DoNotTranslate> En la configuración de su paquete web de producción, simplemente especifique `source-map` para la propiedad `config.devtool` . La propiedad `sourceMapFilename` de `config.output` es opcional y su valor predeterminado es `[name].js.map`.
