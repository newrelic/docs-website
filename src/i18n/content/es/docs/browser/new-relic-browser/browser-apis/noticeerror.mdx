...
Si se captura un error y se vuelve a lanzar, no tendrá una traza de pila. Para estos casos, si desea que la API `newrelic.noticeError` reporte un seguimiento de pila para todos los tipos de navegadores, debe pasarlo a la API en la instrucción `catch`.
