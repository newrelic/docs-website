...
* Si está instalando el agente de browser mediante npm y creando un agente personalizado con funciones seleccionadas, debe habilitar la función `logging` al crear la instancia de `Agent`. Por ejemplo, agregue lo siguiente en el arreglo`features`:
