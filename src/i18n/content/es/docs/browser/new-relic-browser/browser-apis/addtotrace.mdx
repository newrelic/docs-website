...
* Si está instalando el agente del browser a través de npm y creando un agente personalizado con funciones seleccionadas, debe habilitar la función `session_trace` al crear la instancia `Agent`. En el arreglo `features`, agregue lo siguiente:
