...
Este ejemplo registra un evento PageAction cada vez que un usuario selecciona el enlace <DoNotTranslate>**Try Me**</DoNotTranslate> . El evento se registra con un `actionName` de `clickedTryMe`:
...
Luego puede consultar la cantidad de veces que se hizo clic en el botón <DoNotTranslate>**Try Me**</DoNotTranslate> con la siguiente consulta NRQL:
...
Este ejemplo captura la entrada del usuario (direcciones de correo electrónico) desde un formulario llamado <DoNotTranslate>**Signup**</DoNotTranslate>. El evento se registra con un `actionName` de `userSignup`:
