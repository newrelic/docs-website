...
* Si está instalando el agente del browser a través de npm y creando un agente personalizado con funciones seleccionadas, debe habilitar la función `jserrors` al crear la instancia `Agent`. En el arreglo `features`, agregue lo siguiente:
