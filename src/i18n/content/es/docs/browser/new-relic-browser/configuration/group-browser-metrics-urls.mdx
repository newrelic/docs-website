...
El orden no es importante cuando se enumeran varios segmentos. Sin embargo, asegúrese de que los segmentos que agregue <DoNotTranslate>**exactly match**</DoNotTranslate> tal como aparecen en sus URL. Si un segmento de ruta URL termina en `.html` o `.jsp`, la extensión debe incluirse en el segmento. Los segmentos distinguen entre mayúsculas y minúsculas.
