...
Nuestro <InlinePopover type="browser"/>admite la carga de [mapas de origen](/docs/new-relic-browser-source-maps), que se utilizan para eliminar el error de rastreo de la pila en la [página<DoNotTranslate>**Errors**</DoNotTranslate> ](/docs/errors-inbox/browser-tab). Este documento explica cómo utilizar la API para cargar mapas de origen utilizando la API del browser .
...
<CollapserGroup>
  ...
  >
    ...
    Puede encontrar la URL del archivo JavaScript de un error en browser, en la página <DoNotTranslate>**JS errors**</DoNotTranslate>. Consulte [monitoreo de mapas fuente del navegador](/docs/new-relic-browser-source-maps) para obtener más información sobre cómo encontrar estos errores en la UI.
  </Collapser>
  ...
  >
    ...
    * `https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.js`
      ...
      Si esto <DoNotTranslate>**doesn't**</DoNotTranslate> se aplica a usted y las URL de JS **no** tienen información de versión adjunta, deberá ayudar al agente especificando un [nombre de versión y un ID con la API](/docs/browser-api-newrelicaddrelease).
  </Collapser>
...
Cuando haya terminado, vaya a la [página<DoNotTranslate>**JS errors**</DoNotTranslate> ](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time)en el navegador, seleccione una agrupación de errores y vea si el rastreo de la pila de errores no se ha minimizado.
