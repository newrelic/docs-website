...
* <DoNotTranslate>**[First Input Delay (FID)](https://web.dev/fid/)**</DoNotTranslate>
  ...
  : mide la interactividad. Para brindar una buena experiencia al usuario, las páginas deben tener un FID de
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        Largest Contentful Paint es una de las tres métricas identificadas por Google como [Métricas web principales](https://web.dev/vitals/). Los valores de LCP de hasta 2,5 segundos se consideran "Buenos", entre 2,5 y 4,0 segundos se consideran "Necesita mejorar" y por encima de 4,0 segundos se consideran "Deficientes".
      </td>
    </tr>

    <tr>
      <td>
        `firstInteraction` y `firstInputDelay`
      </td>

      <td>
        Con la adición de [`firstInteraction`](/attribute-dictionary/?event=PageViewTiming&attribute=firstInteraction) y [`firstInputDelay`](/attribute-dictionary/?event=PageViewTiming&attribute=firstInputDelay), puede determinar rápidamente las formas en que su usuario interactúa con ese contenido visual. Estas métricas le dicen no solo cuándo interactuaron, sino qué tipo de interacción (mousedown, pointerdown, etc.) y cuánto tiempo les llevó recibir una respuesta de su sitio.

        La métrica `firstInputDelay` se encuentra en medio de las métricas `FirstContentfulPaint` y Time to Interactive (TTI). Mide el tiempo entre el momento en que se puede realizar una primera entrada y el momento en que el hilo principal del browser puede responder a cualquier interacción.
        ...
        Tiempo desde la primera entrada es una de las tres métricas identificadas por Google como [Métricas web principales](https://web.dev/vitals/). Los valores de FID de hasta 100 ms se consideran "Buenos", entre 100 y 300 ms se consideran "Necesita mejorar" y por encima de 300 ms se consideran "Deficientes".

        Para obtener una explicación más detallada, consulte nuestras [<InlinePopover type="browser"/>notas de la versión](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1153).
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * Cromo 77
          ...
          * métrica expuesta vía API: [Cambio de diseño acumulativo](https://web.dev/cls/) disponible vía [API de inestabilidad de diseño](https://github.com/WICG/layout-instability)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Estas métricas requieren la [API browser `addEventListener` ](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener). Esta API está disponible en todos los navegadores modernos, incluidos:

        * Safari de Apple
        * Google Chrome
        * Microsoft Internet Explorer (IE) versiones 9 o superiores
        * Mozilla Firefox
      </td>
    </tr>
...
Cumulative Layout Shift (CLS) es una métrica que mide la estabilidad del contenido de una página web. Para obtener una descripción completa, consulte [web.dev/cls](https://web.dev/cls/).
...
<CollapserGroup>
  ...
  >
    Muestre el percentil 95 del tiempo desde la primera entrada sobre una serie de tiempo, facetado por nombre de transacción y tipo de interacción:
  ...
  >
    Muestre un histograma de tiempos desde la primera entrada facetados por rangos de tiempo de primera interacción:
