...
<CollapserGroup>
  <Collapser id="moreforloop" title="Bucle para iterar a través de una lista, mapa o colección">
    Puede emplear `loop` para iterar a través de una lista, un mapa o una colección.

    Un bucle itera sobre una colección dada definida por `in`. Creará automáticamente un bucle de variables `index` y `element` para cada una de sus iteraciones. Se puede acceder a estas variables de bucle dentro del bucle con la expresión JQ `${{ .steps.<loopStepName>.loop.element }}` o `${{ .steps.<loopStepName>.loop.index }}`
    ...
    **Ejemplos**:
    ...
    A continuación se muestra cómo ejecutar el bucle `workflowInputs`. Tenga en cuenta que el resultado debe ser un tipo de colección.
    ...
    <Callout variant="important">
      ...
      * `element` Puede ser un tipo complejo si tiene una colección de elementos complejos.
    ...
    **Saltar dentro de un bucle**
    ...
    Se permite saltar entre pasos nombrados que pertenecen al mismo bucle for. No está permitido entrar o salir de un bucle for, a un bucle interno o externo, o entre dos bucles for diferentes.
    ...
    **Emplee break o continue en un bucle**
