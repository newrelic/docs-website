...
* **Tipo**: mapa de mapas
* Tamaño máx.: 100
* Un mapa de entradas de flujo de trabajo que acepta el flujo de trabajo.
* Ejemplo:
...
Las constantes de flujo de trabajo proporcionan acceso a metadatos en tiempo de ejecución sobre el flujo de trabajo que se está ejecutando actualmente. Estas constantes están disponibles automáticamente en todas las definiciones de flujo de trabajo y se pueden referenciar mediante la sintaxis de expresión `${{ .workflowConstants.<constantName> }}`.
...
Las constantes de flujo de trabajo se pueden usar con expresiones JQ para transformar valores:
...
Convirtiendo la marca de tiempo con la acción `DateTime`
...
<Callout variant="important">
  * Las constantes del flujo de trabajo son de solo lectura y proporcionadas automáticamente por el tiempo de ejecución
  * La marca de tiempo `startedAt` está en milisegundos (formato epoch)
  * Las constantes están disponibles en todos los contextos de expresión en todo el flujo de trabajo
  * A diferencia de `workflowInputs`, las constantes no necesitan declararse en la definición del flujo de trabajo
</Callout>
...
Un paso que finaliza el flujo de trabajo y reporta una falla con un mensaje personalizado. Si hay un `condition` presente, se valida primero y el flujo de trabajo se detiene solo cuando la condición es `true`. Si no proporciona un `message`, se usa el mensaje predeterminado `failed as requested from step: failureStep`.
