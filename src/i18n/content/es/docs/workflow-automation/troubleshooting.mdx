...
Esta página proporciona soluciones para problemas comunes que puede encontrar al usar la automatización de flujos de trabajo. Para obtener soporte adicional, consulte el [foro de soporte de New Relic](https://support.newrelic.com/s/) o comuníquese con el [soporte de New Relic](https://support.newrelic.com/).
...
1. **Verifica que [CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) esté configurado**: requerido para las métricas de EC2 en tiempo real
...
### Flujo de trabajo de RCA de Slack de Autopilot

**Mensaje de Slack vacío sin contenido de RCA:** `analysisRaw` devolvió una respuesta, pero `analysisText` no pudo extraer el contenido — `finalAnswer` no estaba en la estructura `.card.body` esperada. Confirme que el selector `expression` esté en una sola línea y que el símbolo <DNT>Autopilot</DNT> devuelva una respuesta de tarjeta estructurada. Consulte [RCA de alerta con Autopilot y Slack](/docs/workflow-automation/workflow-examples/integrations/autopilot-slack-rca) para conocer el patrón de selector canónico.

**Mensaje &quot;<DNT>Autopilot</DNT> RCA Failed&quot; en Slack:** `analysisRaw` estaba vacío — <DNT>Autopilot</DNT> no devolvió ningún `finalAnswer`. Verifique que `issueId` sea un UUID de problema de alerta válido y activo y que el símbolo <DNT>Autopilot</DNT> esté bien formado.

**El flujo de trabajo falla en la validación de entrada:** tanto `issueId` como `slackDestinationId` deben estar en formato UUID. Use `{{ issueId }}` al activar desde flujos de trabajo de alertas.
