...
<CollapserGroup>
  ...
  <Collapser id="storage-issues" title="Problemas de ruta de almacenamiento o permisos">
    **Problema**: los logs de ATP muestran errores o advertencias relacionados con el almacenamiento sobre la imposibilidad de crear directorios de almacenamiento.
    ...
    **Solución**:
    ...
    1. **Para instalaciones de systemd**: asegúrese de que el servicio se ejecute como el usuario `nrdot-collector` y que el archivo de servicio systemd incluya la directiva `StateDirectory`:
    ...
    2. **Para instalaciones manuales de binarios**: verificar que su usuario tenga permisos de escritura en `~/.local/state/`:
    ...
    3. **Verifique los logs del recolector** en busca de mensajes relacionados con el almacenamiento:
...
<CollapserGroup>
  <Collapser id="persistence-file" title="Configuración de almacenamiento y persistencia">
    ...
    * El almacenamiento se configura automáticamente en `/var/lib/nrdot-collector/`
    * El servicio systemd usa `StateDirectory` para crear y administrar el directorio
    * No se requiere configuración manual

    **Para instalaciones manuales de binarios:**

    * El almacenamiento usa `~/.local/state/nrdot-collector/` para usuarios regulares
    * No se requieren privilegios de root para el almacenamiento
