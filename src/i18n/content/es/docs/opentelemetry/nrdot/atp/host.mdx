...
ATP utiliza almacenamiento persistente para mantener el estado del proceso entre reinicios del recolector. Las rutas de almacenamiento se seleccionan automáticamente según su entorno de ejecución:

### Instalaciones de servicio Systemd (paquetes DEB/RPM)

Al ejecutarse como un servicio systemd, ATP utiliza automáticamente `/var/lib/nrdot-collector/` para el almacenamiento. La configuración del servicio de systemd incluye `StateDirectory=nrdot-collector` que:

* Crea automáticamente el directorio con la propiedad correcta (usuario/grupo `nrdot-collector`)
* Establece permisos seguros (`0700`)
* No se requiere la creación manual de directorios ni la configuración de permisos.

### Instalaciones manuales de binarios

Al ejecutarse como usuario normal, el binario del recolector utiliza automáticamente la ruta estándar de XDG -/.local/state/nrdot-collector/, lo que permite que ATP funcione sin privilegios de root.

### Comportamiento de respaldo

Si ninguna de las ubicaciones de almacenamiento admite escritura (por ejemplo, debido a problemas de permisos), ATP deshabilita automáticamente el almacenamiento y continúa operando solo con el estado en memoria. La característica de retención funciona normalmente durante el tiempo de ejecución, realizando un seguimiento de los procesos que exceden los umbrales para el `retention_minutes` configurado. Sin embargo, este estado de seguimiento se pierde al reiniciar el recolector, ya que no se persiste en el disco.

<Callout variant="tip">
  Para las instalaciones de systemd, no es necesario crear manualmente directorios de almacenamiento ni configurar permisos. El servicio gestiona esto automáticamente por usted.
</Callout>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Habilite el almacenamiento persistente. Las rutas de almacenamiento se determinan automáticamente según el contexto de ejecución:

            * **Servicio systemd de Linux**: `/var/lib/nrdot-collector/adaptiveprocess.db` (gestionado automáticamente por systemd)
            * **Binario manual de Linux**: `~/.local/state/nrdot-collector/adaptiveprocess.db` (estándar XDG, no requiere root)
            * **Windows**: `%LOCALAPPDATA%\nrdot-collector\adaptiveprocess.db`

            Si no hay una ruta de escritura disponible, el almacenamiento se deshabilita automáticamente y ATP opera solo con estado en memoria (la retención funciona durante el tiempo de ejecución, pero el estado se pierde en los reinicios).
          </td>
        </tr>
