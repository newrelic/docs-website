...
La consulta supone que los datos son producidos por la [biblioteca de instrumentación en tiempo de ejecuciónOpenTelemetry Go](https://opentelemetry.io/docs/specs/semconv/runtime/go-metrics/).

<Callout variant="important">
  Las [convenciones semánticas](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/runtime/go-metrics.md) para las métricas de tiempo de ejecución de Go aún están en desarrollo. La página de tiempo de ejecución de Go está dirigida a una versión anterior de las convenciones. Debe habilitar las convenciones antiguas habilitando la variable de entorno `OTEL_GO_X_DEPRECATED_RUNTIME_METRICS`. Para obtener más información, consulte la [documentación](https://github.com/open-telemetry/opentelemetry-go-contrib/blob/9a6e0bf68e1da495be52faf152b5b4e6fb09d475/instrumentation/runtime/internal/x/README.md) de la instrumentación.

  New Relic acabará dando soporte a las nuevas convenciones una vez que se estabilicen.
</Callout>
...
## Cómo se representan los errores en New Relic [#errors]

La información de errores de su aplicación aparece en dos lugares en la UI de APM:

* **Tasa de errores**: se muestra en la [página de resumen](#summary-page), derivada de los datos de métrica. Cuando una métrica de OpenTelemetry como `http.server.request.duration` o `rpc.server.call.duration` incluye un atributo `error.type`, ese punto de datos contribuye a la tasa de errores.
* **Errors Inbox**: consulta la [página Errors Inbox](#errors-inbox-page), controlada por datos de span. Un intervalo se clasifica como error cuando su estado se establece en `ERROR` (por ejemplo, `otel.status_code = ERROR`).

### Error omitido [#ignore-errors]

Los agentes de lenguaje de New Relic se pueden configurar para ignorar errores por código de estado HTTP o clase de error, lo que evita que afecten la tasa de errores o aparezcan en Errors Inbox. Con OpenTelemetry, no hay una configuración de agente equivalente, pero se puede lograr el mismo efecto manipulando los datos subyacentes antes de que lleguen a New Relic.

#### Error omitido en la tasa de errores

La tasa de errores se deriva de los mismos datos de métrica de los que [derivamos las transacciones](#transaction-mapping). Cuando una métrica como `http.server.request.duration` o `rpc.server.call.duration` incluye un atributo `error.type`, ese punto de datos contribuye a la tasa de errores. Eliminar el atributo `error.type` de un punto de datos métrico evita que se cuente como un error.

<Callout variant="important">
  El atributo `error.type` no existía en versiones anteriores de las convenciones semánticas de OpenTelemetry. Para dar soporte a la instrumentación más antigua que sigue versiones anteriores de las convenciones semánticas HTTP y RPC, New Relic aprovecha los atributos `http.status_code` y `rpc.grpc.status_code` para impulsar la tasa de errores. Estos atributos estaban presentes en las métricas `http.server.duration` y `rpc.server.duration` más antiguas.
</Callout>

#### Omitir errores en Errors Inbox

Errors Inbox está controlado por datos de span. Un intervalo se clasifica como error cuando su estado se establece en `ERROR` (es decir, `otel.status_code = ERROR`). Quitar el estado del span para que ya no sea `ERROR` evita que el span aparezca en Errors Inbox.

#### Ejemplo: transformar datos con el OpenTelemetry Collector

El [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) se puede utilizar para transformar datos de métricas y spans en tránsito para suprimir errores específicos. El [ejemplo de error omitido](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors) proporciona una configuración funcional del recolector que demuestra cómo:

* Elimine el atributo `error.type` de los puntos de datos métricos para excluirlos de la tasa de errores
* Elimina el estado del span para evitar que los spans de error aparezcan en Errors Inbox.
