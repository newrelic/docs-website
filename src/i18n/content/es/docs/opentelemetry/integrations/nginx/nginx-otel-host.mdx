...
Monitoree sus servidores NGINX autohospedados con OpenTelemetry para enviar métricas de rendimiento y telemetry data a New Relic.
...
* **Prometheus Receiver:** para entornos que ya ejecutan [nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter)
...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="prometheus">
      Utilice este enfoque si ya tiene [nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter) ejecutándose en su entorno, o si está migrando desde un stack de monitoreo basado en Prometheus.
      ...
      <CollapserGroup>
        <Collapser id="configure-prometheus-host" title="Paso 1: configurar el recolector">
          Esta configuración funciona tanto con **NRDOT** como con **OTel Collector Contrib**. Colóquelo en la ubicación de configuración adecuada:

          * **NRDOT:** `/etc/nrdot-collector/config.yaml`
          * **OTel Collector Contrib:** `/etc/otelcol-contrib/config.yaml`

          <Callout variant="tip">
            **Haga una copia de seguridad de su configuración existente antes de realizar cambios:**
          ...
          Reemplace el contenido del archivo con la siguiente configuración:

          <Callout variant="important">
            Reemplace `<YOUR_STUB_STATUS_ENDPOINT>` con su URL de estado de stub de NGINX (por ejemplo, `http://127.0.0.1:8080/nginx_status`) y `<DEPLOYMENT_NAME>` con un nombre único para este servidor NGINX.
          </Callout>
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  Su URL de estado stub de NGINX (por ejemplo, 

                  `http://127.0.0.1:8080/nginx_status`

                  ). Utilizado como el atributo de recurso 
              ...
              <tr>
                ...
                <td>
                  Nombre único para este servidor NGINX (por ejemplo, 

                  `production-web-01`

                  ). Aparece en los dashboards de New Relic.
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  Dirección del nginx-prometheus-exporter (predeterminado 

                  `localhost:9113`

                  ). Actualice si se ejecuta en un host o puerto diferente.
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  Con qué frecuencia recolectar métricas del exportador. Predeterminado: 
        ...
        <Collapser id="env-vars-prometheus-host" title="Paso 2: Establecer variables de entorno">
          ...
          Reemplace `YOUR_NEW_RELIC_LICENSE_KEY` con su <InlinePopover type="licenseKey" />.

          <Callout variant="tip">
            Para cuentas de la UE, use `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.eu01.nr-data.net:4318`. Consulte [Configurar extremo, puerto y protocolo](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) para obtener la lista completa de extremos regionales.
          </Callout>
        </Collapser>

        <Collapser id="start-prometheus-host" title="Paso 3: inicie el recolector">
          Aplique la configuración y reinicie el recolector. Reemplace `<collector-service>` con `nrdot-collector` o `otelcol-contrib`:
          ...
          Salida esperada: `Active: active (running)` sin errores.
        </Collapser>

        <Collapser id="verify-prometheus-host" title="Paso 4: Verifique los datos en New Relic">
          Después de unos minutos, ejecute esta consulta NRQL para confirmar que los datos están fluyendo. Reemplace `<DEPLOYMENT_NAME>` con el valor que estableció en la configuración:
