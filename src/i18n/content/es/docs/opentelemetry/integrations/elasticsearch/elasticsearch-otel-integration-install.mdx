...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="nrdot">
      <Steps>
        <Step>
          ...
          * Si no está instalado, siga la [guía oficial de instalación de NRDOT](/docs/opentelemetry/nrdot/nrdot-collector/) para instalar el recolector usando el administrador de paquetes (DEB/RPM), que configura el servicio systemd automáticamente.
        ...
        <Step>
          ...
          5. *(Opcional)* Para recopilar logs de Elasticsearch, agregue el receptor filelog. Asegúrese de que el usuario que ejecuta el servicio del recolector (nrdot-collector) tenga acceso de lectura a sus archivos de log de Elasticsearch:
    ...
    <TabsPageItem id="prometheus">
      Use este enfoque si ya tiene un [exportador de Elasticsearch de Prometheus](https://github.com/prometheus-community/elasticsearch_exporter) ejecutándose en su entorno, o si está migrando desde un stack de monitoreo basado en Prometheus.
      ...
      <Steps>
        <Step>
          ### Instale y configure el exportador de Elasticsearch [#install-exporter]

          El [exportador de Elasticsearch para Prometheus](https://github.com/prometheus-community/elasticsearch_exporter) expone las métricas de Elasticsearch en formato Prometheus en el puerto `9114`.
          ...
          2. Inicie el exportador (apúntelo a su instancia de Elasticsearch):
          ...
          3. Verifique que el exportador se esté ejecutando:
        ...
        <Step>
          ...
          * **OTel Collector Contrib:** `/etc/otelcol-contrib/config.yaml`
          ...
          <Collapser id="prometheus-full-config" title="Configuración completa del receptor de Prometheus">
            ```yaml
            receivers:
        ...
        <Step>
          ### Configurar variables de entorno [#env-vars-prom]
