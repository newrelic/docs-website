...
<Steps>
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="software-reqs" title="Requisitos de software">
        * **OpenTelemetry Collector**: Versión Contrib 0.140.0 o superior
  ...
  <Step>
    ### Instalar el OpenTelemetry Collector [#install-collector]
  ...
  <Step>
    ### Configurar el recolector [#configure]
  ...
  <Step>
    ### Verifique sus datos en New Relic [#verify]
...
<CollapserGroup>
  <Collapser id="no-data" title="No aparecen datos en New Relic">
    ...
    **5. Verifique su clave de licencia:**
    ...
    Asegúrese de que `NEWRELIC_LICENSE_KEY` esté configurado correctamente y no tenga espacios ni comillas adicionales.
  </Collapser>
  ...
  <Collapser id="connection-refused" title="Errores de conexión rechazada">
    ...
    **Verifique la configuración del endpoint:**\
    Asegúrese de que el `endpoint` en `/etc/otelcol-contrib/config.yaml` coincida con su configuración de RabbitMQ (nombre de host, puerto).
  </Collapser>
  ...
  <Collapser id="auth-failures" title="Fallas de autenticación (401 No autorizado)">
    ...
    **Verifique las credenciales:**\
    Compruebe que el nombre de usuario y la contraseña en `/etc/otelcol-contrib/config.yaml` coincidan con su usuario de RabbitMQ.
