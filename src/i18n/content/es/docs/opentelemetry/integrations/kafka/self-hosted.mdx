  ...
  <TabsPages>
    <TabsPageItem id="java-agent">
      ...
      <Steps>
        ...
        <Step>
          ...
          | `NEW_RELIC_OTLP_ENDPOINT`          | Extremo de ingesta de OTLP. Utilice `https://otlp.nr-data.net:4317` para la región de EE. UU. (predeterminado), `https://otlp.eu01.nr-data.net:4317` para la región de la UE o `https://otlp.jp.nr-data.net:4317` para la región de JP. Para otras configuraciones, consulte [Configure su extremo OTLP](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol). |
        </Step>
        ...
        <Step>
          ### Descargar el agente de Java de OpenTelemetry [#download-java-agent]
          ...
          El [agente de Java de OpenTelemetry](https://opentelemetry.io/docs/zero-code/java/agent/) se ejecuta como un agente de Java adjunto a sus brokers de Kafka, recopilando métricas de Kafka y JMX y enviándolas al recolector mediante OTLP:
        ...
        <Step>
          ...
          Para recopilar telemetría a nivel de aplicación de sus aplicaciones de productor y consumidor de Kafka, descargue el agente de Java de OpenTelemetry desde el paso [Descargar el agente de Java de OpenTelemetry](#download-java-agent) anterior.
          ...
          El agente de Java proporciona [instrumentación de Kafka lista para usar](https://opentelemetry.io/docs/zero-code/java/spring-boot-starter/out-of-the-box-instrumentation/) sin cambios de código, capturando la latencia de las solicitudes, las métricas de rendimiento, la tasa de errores y el rastreo distribuido.
    ...
    <TabsPageItem id="prometheus">
      ...
      <Steps>
        ...
        <Step>
          ### Descarga el expositor JMX de Prometheus [#download-jmx-exporter]
        ...
        <Step>
          ### Crear configuración de métricas JMX [#jmx-config-prometheus]
          ...
          <Callout variant="tip">
            **Personalizar métricas**: puede agregar o modificar patrones consultando los [ejemplos de Prometheus JMX Exporter](https://github.com/prometheus/jmx_exporter/tree/main/examples) y la [documentación de Kafka MBean](https://kafka.apache.org/documentation/#monitoring). Consulte la [documentación de reglas de JMX Exporter](https://github.com/prometheus/jmx_exporter/blob/main/docs/content/1.5.0/http-mode/rules.md) para configuración adicional.
          </Callout>
        </Step>

        <Step>
          ### Configurar los brokers de Kafka para utilizar JMX Exporter [#configure-brokers]
        ...
        <Step>
          ### Crear configuración del colector [#collector-config-prometheus]
          ...
          <CollapserGroup>
            <Collapser id="configuration-notes-prometheus" title="Notas de configuración">
              **Aspectos destacados de la arquitectura:**
              ...
              * **Nomenclatura de métricas**: transforma los nombres de Prometheus separados por `_`en nombres separados por `.`que coinciden con los dashboards de New Relic
            </Collapser>

            <Collapser id="additional-receiver-docs-prometheus" title="Documentación adicional del receptor">
        ...
        <Step>
          ### Establecer variables de entorno [#env-vars-prometheus]
        ...
        <Step>
          ### Instale e inicie el recopilador [#install-start-collector-prometheus]
          ...
          <Tabs>
            ...
            <TabsPages>
              <TabsPageItem id="nrdot-prom">
                <Callout variant="tip">
                  **NRDOT Collector** es la distribución de New Relic de OpenTelemetry Collector con soporte de New Relic para asistencia. Para obtener más información, consulta el [repositorio de GitHub de NRDOT Collector](https://github.com/newrelic/nrdot-collector-releases/tree/main/distributions/nrdot-collector).
                </Callout>
        ...
        <Step>
          ...
          <Callout variant="tip">
            ...
            <CollapserGroup>
              <Collapser id="app-collector-config-prometheus" title="Configuración de ejemplo del colector">
                ```yaml
                receivers:
                  otlp:
                    protocols:
...
Un ejemplo funcional completo con la configuración de Docker Compose, la configuración de OTel Collector, la configuración del agente de Java de OTel y aplicaciones de muestra de productor/consumidor está disponible en el [respositorio de New Relic OpenTelemetry Examples](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/collector/kafka/self-host-kafka).
...
<CollapserGroup>
  <Collapser id="initial-checks" title="Comprobaciones iniciales del sistema">
    ...
    **Verificar si el recopilador se está ejecutando**:
    ...
    Si no aparecen resultados, el recolector no se está ejecutando. Inícielo siguiendo el paso [Instale e inicie el recolector](#install-start-collector).
  ...
  <Collapser id="missing-jmx-metrics" title="Faltan métricas JMX de los brokers de Kafka">
    ...
    **Verifique la configuración del agente de Java**: asegúrese de que el comando de inicio coincida con el paso [Configurar el broker de Kafka](#configure-broker)
