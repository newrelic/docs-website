...
<Steps>
  ...
  <Step>
    ...
    **Parámetro de configuración**
    ...
    El agente de Java proporciona [instrumentación de Kafka lista para usar](https://opentelemetry.io/docs/zero-code/java/spring-boot-starter/out-of-the-box-instrumentation/) sin cambios de código, capturando:
    ...
    <CollapserGroup>
      <Collapser id="configure-log-collection-strimzi" title="Configurar la recopilación de logs">
        ...
        **Parámetro de configuración**
        ...
        | Parámetro                                             | Descripción                                                                                                                                                                                                                                                                                                                                                                                   |
        | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
        | `filelog/kafka_broker_0.include`                      | Actualice `/var/log/kafka/server.log` a la ruta real del log de Kafka dentro del pod del broker. Para Strimzi, los logs normalmente se escriben en stdout — utilice la [configuración de logging de Strimzi](https://strimzi.io/docs/operators/latest/configuring.html#property-kafka-logging-reference) para redirigirlos a un archivo, o utilice un recolector de logs sidecar en su lugar. |
...
Un ejemplo funcional completo con recursos personalizados de Strimzi Kafka, configuración de JMX Exporter, configuración de OTel Collector y aplicaciones de productor/consumidor de muestra está disponible en el [respositorio de ejemplos de New Relic OpenTelemetry](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/collector/kafka/k8s-strimzi).
