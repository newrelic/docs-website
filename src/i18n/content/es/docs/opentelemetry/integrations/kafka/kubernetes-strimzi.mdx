...
* Kafka desplegado vía [operador Strimzi](https://strimzi.io/) con JMX habilitado. Asegúrese de que su clúster de Kafka tenga JMX habilitado en el recurso Strimzi Kafka:
...
<Steps>
  ...
  <Step>
    ### Crear secreto con clave de licencia [#create-secret]
  ...
  <Step>
    ### Desplegar OpenTelemetry Collector [#deploy-collector]

    #### 3.1 Construir imagen de recopilador personalizada [#build-image]

    Cree una imagen de colector personalizada con un entorno de ejecución de Java y un scraper JMX. Elija entre NRDOT Collector (distribución de New Relic) u OpenTelemetry Collector:
      ...
      <TabsPages>
        <TabsPageItem id="nrdot">
          ...
          <Callout variant="important">
            **Compatibilidad de versiones**: Esta guía utiliza JMX Scraper 1.52.0 y NRDOT Collector. Para obtener mejores resultados, utilice las últimas versiones.

            <CollapserGroup>
              <Collapser id="nrdot-version-compatibility" title="Buscar la última versión de NRDOT Collector">
                **Verifique las últimas versiones**:

                * NRDOT Collector: Visite [NRDOT Collector releases](https://github.com/newrelic/nrdot-collector-releases/releases/latest) para encontrar la versión más reciente y reemplace `{NRDOT_VERSION}` en el Dockerfile
            ...
            **Arquitectura de destino**: El siguiente ejemplo utiliza `linux_amd64`. Consulte la página de [lanzamientos de NRDOT Collector](https://github.com/newrelic/nrdot-collector-releases/releases/latest) para encontrar el binario correcto para la arquitectura de su sistema (por ejemplo, `linux_amd64`, `linux_arm64`). Actualice la URL de descarga en el Dockerfile según corresponda.
          </Callout>
        ...
        <TabsPageItem id="otel">
          <Callout variant="important">
            **Compatibilidad de versiones**: Esta guía utiliza JMX Scraper 1.52.0 y OpenTelemetry Collector 0.145.0. Es posible que las versiones anteriores del recopilador no incluyan el hash de este scraper en su lista de compatibilidad. Para obtener los mejores resultados, utilice las versiones más recientes como se muestra en esta guía.
    ...
    **Construya y empuje la imagen:**
    ...
    Después de guardar el Dockerfile, construya y envíe la imagen a su registro de contenedores:
