...
Esta página documenta las métricas recopiladas al monitorear Apache Kafka con OpenTelemetry. Las métricas se recopilan mediante el [receptor de métricas de Kafka](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver), las métricas JMX (del Agente Java de OpenTelemetry o del Exportador JMX de Prometheus) y el [agente Java de OpenTelemetry](https://opentelemetry.io/docs/languages/java/automatic/) para las métricas del lado del cliente.
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Número de réplicas sincronizadas para una partición. Esta métrica es filtrada por el pipeline del recolector — solo se retiene el `kafka.partition.replicas_in_sync.total` agregado por tema.
          </td>
...
* **Kafka autohospedado**: [Agente Java de OpenTelemetry](/docs/opentelemetry/integrations/kafka/self-hosted#java-agent) o [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/self-hosted#prometheus)
* **Kubernetes (autogestionado)**: [agente de Java de OpenTelemetry](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#java-agent) o [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#prometheus)
* **Kubernetes (Strimzi)**: [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-strimzi#configure-jmx-exporter) con configuración personalizada de New Relic

Ambos métodos recopilan el mismo conjunto de métricas de broker de Kafka y JVM documentadas a continuación:

<CollapserGroup>
  <Collapser id="jmx-kafka-metrics" title="Métricas del broker de Kafka">
    Estas métricas proporcionan una telemetría integral del broker de Kafka, que incluye la salud del clúster, el rendimiento del broker y las métricas por tema.
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            El recuento de elección de líder. Solo aparece cuando ocurre una elección de líder; no se emite en clústeres estables.
          </td>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Tamaño de la cola de solicitudes en el broker
          </td>
...
## Métricas del cliente de Kafka [#kafka-client-metrics]
...
<CollapserGroup>
  <Collapser id="producer-metrics" title="Métricas del productor">
    ...
    | `kafka.producer.incoming_byte_rate`        | Tasa de bytes entrantes (respuestas de los brokers)  | client-id, node-id       |
    | `kafka.producer.incoming_byte_total`       | Total de bytes entrantes (respuestas de los brokers) | client-id, node-id       |
  ...
  <Collapser id="consumer-metrics" title="Métricas del consumidor">
    ...
    | `kafka.consumer.partition_assigned_latency_avg` | Latencia promedio de asignación de particiones (ms). Solo se emite durante los rebalanceos del grupo de consumidores. | identificador de cliente |
    | `kafka.consumer.partition_assigned_latency_max` | Latencia máxima de asignación de partición (ms). Solo se emite durante los rebalanceos del grupo de consumidores.     | identificador de cliente |
    | `kafka.consumer.partition_revoked_latency_avg`  | Latencia promedio de revocación de partición (ms). Solo se emite durante los rebalanceos del grupo de consumidores.   | identificador de cliente |
    | `kafka.consumer.partition_revoked_latency_max`  | Latencia máxima de revocación de partición (ms). Solo se emite durante los rebalanceos del grupo de consumidores.     | identificador de cliente |
    | `kafka.consumer.partition_lost_latency_avg`     | Latencia promedio de pérdida de partición (ms). Solo se emite durante los rebalanceos del grupo de consumidores.      | identificador de cliente |
    | `kafka.consumer.partition_lost_latency_max`     | Latencia máxima de pérdida de partición (ms). Solo se emite durante los rebalanceos de grupos de consumidores.        | identificador de cliente |
...
* [Consulte y visualice sus datos](/docs/opentelemetry/integrations/kafka/find-and-query-data) - Busque métricas en la UI de New Relic, escriba consultas NRQL, cree dashboards y configure alertas
* [Consultar tipos de datos de métricas](/docs/data-apis/understand-data/metric-data/query-metric-data-type/) - Aprenda técnicas avanzadas para trabajar con métricas de OpenTelemetry