...
Esta página documenta las métricas recopiladas al monitorear Apache Kafka con OpenTelemetry. Las métricas se recopilan a través del [receptor de métricas de Kafka](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver), el [receptor JMX](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver) y el [agente Java de OpenTelemetry](https://opentelemetry.io/docs/languages/java/automatic/) para métricas del lado del cliente.
...
* **[Receptor JMX](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver)**: Se conecta al puerto JMX (normalmente 9999) para recopilar métricas detalladas del broker y métricas de JVM
...
## Métricas del receptor JMX [#jmx-metrics]
...
1. **Sistema de destino Kafka predeterminado** - Métricas específicas de Kafka integradas de `target_system: kafka` ([kafka.yaml](https://github.com/open-telemetry/opentelemetry-java-contrib/blob/main/jmx-scraper/src/main/resources/kafka.yaml))
...
<CollapserGroup>
  <Collapser id="jmx-cluster-metrics" title="Métricas a nivel de clúster">
  ...
  <Collapser id="jmx-broker-metrics" title="Métricas del broker">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
...
Estas métricas adicionales de Kafka y métricas de JVM se recopilan cuando se utiliza un archivo de configuración JMX personalizado (como se muestra en la [configuración autohospedada](/docs/opentelemetry/integrations/kafka/self-hosted#jmx-config)). La configuración personalizada le permite recopilar métricas adicionales específicas de Kafka más allá del conjunto predeterminado, así como métricas de estado de JVM.

<CollapserGroup>
  <Collapser id="jmx-cluster-level-metrics" title="Métricas a nivel de clúster">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-broker-specific-metrics" title="Métricas específicas del broker">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-per-topic-metrics" title="Métricas por tema">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
...
* **[Consultar y visualizar sus datos](/docs/opentelemetry/integrations/kafka/find-and-query-data)** - Encuentre métricas en la interfaz de usuario de New Relic, escriba consultas NRQL, cree dashboards y configure alertas
