...
<CollapserGroup>
  ...
  <Collapser id="step2-enable-additional-metrics-otelcol" title="(Opcional) Habilitar métricas adicionales">
    ...
    1. **Identifica las métricas que deseas habilitar** de la [lista de métricas](#metrics) a continuación. Observe a qué categoría pertenecen (por ejemplo, Zonas de servidor HTTP, Zonas de ubicación, Caché).
    ...
    **Ejemplo: Habilitar métricas de zonas de servidor HTTP**
    ...
    3. **Reinicie el colector** después de realizar cambios:
       ```bash
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-dashboard" title="Verifique los datos en New Relic">
    Acceda a su [panel de NGINX](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data) para verificar la recolección de datos.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="plus-server-zone" title="Zonas del servidor HTTP">
    <Callout variant="important">
      **Nota:** Estas métricas están deshabilitadas de forma predeterminada. Para habilitarlas, consulte [(Opcional) Habilitar métricas adicionales](#step2-enable-additional-metrics-otelcol) para OpenTelemetry Collector Contrib.
    </Callout>
