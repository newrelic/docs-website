...
La API de Insights consulta de New Relic es una API REST para realizar consultas NRQL.
...
Para utilizar la API de Insights consulta, necesita una clave de consulta. Puede tener varias claves de consulta y cualquier consulta se puede utilizar para iniciar cualquier consulta API de Insights. Si tiene varios sistemas que consultan Insights o diferentes destinos de datos, New Relic recomienda utilizar varias claves de consulta para mejorar la seguridad de los datos.
...
1. Vaya a [one.newrelic.com/api-keys](https://one.newrelic.com/api-keys). Si utiliza el centro de datos de la UE, vaya a [one.eu.newrelic.com/api-keys](https://one.eu.newrelic.com/api-keys) en cambio.
...
4. Haga clic en <DNT>**Save**</DNT> para generar la nueva clave.

## 2. Cree la solicitud de consulta API [#create-request]

Cuando [cree o edite una consulta](#register), verá una consulta `curl` de ejemplo que puede usar como plantilla. La consulta de ejemplo no funcionará a menos que siga estas reglas de consulta:
...
* La URL debe contener un [ID de cuenta](/docs/accounts-partnerships/accounts/account-setup/account-id) válido.
...
La API de consulta de Insights devuelve datos JSON. A continuación se muestra un ejemplo de una consulta, su formato de solicitud de consulta y los datos devueltos:
