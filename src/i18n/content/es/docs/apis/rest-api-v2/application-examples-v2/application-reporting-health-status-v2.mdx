...
<DoNotTranslate>**Exception:**</DoNotTranslate> La API REST utiliza `orange` en lugar de `yellow`. Si la aplicación no está asociada a una política de alertas, se mostrará como `unknown`.