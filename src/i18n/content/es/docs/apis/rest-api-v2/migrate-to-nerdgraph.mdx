...
<Callout variant="important">
  La API REST v2 de New Relic (incluidos los extremos de alertas) y la API v0 de despliegues [llegarán al final de su vida útil el 31 de julio de 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Después de esta fecha, estos extremos ya no estarán disponibles. Migre sus integraciones a NerdGraph antes de esa fecha.
</Callout>
...
Esta sección trata sobre la migración del registro de despliegue desde ambas API REST que llegan al final de su vida útil el 31 de julio de 2027:

* Los extremos de despliegue de la API REST v2 (`https://api.newrelic.com/v2/applications/{app_id}/deployments.json`), que se muestran a continuación.
* La API v0 de despliegue legacy (`deployments.xml`), disponible tanto en los hosts `rpm` como en los `api` en cada región — `https://rpm.newrelic.com/deployments.xml` y `https://api.newrelic.com/deployments.xml` (EE. UU.), además de sus equivalentes `.eu.`.

Ambos migran al seguimiento de cambios de NerdGraph. Registre los despliegues con la mutación `changeTrackingCreateEvent` y consúltelos con NRQL.

<Callout variant="tip">
  ¿No sabe con seguridad dónde sigue registrando despliegues a través de estas API REST? El seguimiento de cambios agrega un atributo `newrelic.source` a cada evento, para que pueda encontrar el uso de REST existente en todo el entorno desde la UI de New Relic o con NRQL. Consulte [cómo se registran los cambios en todo el entorno](/docs/change-tracking/view-analyze-data/#recording-source) en la documentación de seguimiento de cambios.
</Callout>

<Callout variant="tip">
  La mutación de NerdGraph `changeTrackingCreateDeployment` más antigua es una mutación independiente, no REST, que **no** forma parte de este final de vida útil y sigue funcionando. New Relic recomienda `changeTrackingCreateEvent` para trabajos nuevos porque acepta `entitySearch` y admite atributos personalizados, categorías y tipos. Consulte [Rastrear cambios mediante NerdGraph](/docs/change-tracking/config/nerdgraph/) para ambas mutaciones.
</Callout>
...
<Callout variant="tip">
  Al igual que algunas otras operaciones de NerdGraph, `changeTrackingCreateEvent` acepta un `entitySearch`, lo que la convierte en la ruta de migración más fácil desde la API REST. Puede buscar por `name`, así como por `entityGuid` si lo tiene. Si su integración REST existente solo tiene el ID de la aplicación numérico, puede resolver la entidad directamente con `entitySearch: { query: "domainId = 'YOUR_APP_ID' AND domain = 'APM'" }` —no se necesita búsqueda de GUID. Consulte el [ejemplo de despliegue por ID de la aplicación](/docs/change-tracking/config/nerdgraph/#deployment-by-appid) y [Rastrear cambios usando NerdGraph](/docs/change-tracking/config/nerdgraph/) para ver todos los campos disponibles.
</Callout>
...
Para obtener más información, consulte [Rastrear cambios usando NerdGraph](/docs/change-tracking/config/nerdgraph/).
