...
### Actualizar widgets en una página [#update-widgets-in-page]
...
#### Enlaces y descripciones del widget [#widget-links-descriptions]

También puede utilizar `dashboardUpdateWidgetsInPage` para agregar enlaces y descripciones a los widgets. Los enlaces de widget aparecen como enlaces en los que se puede hacer clic en los títulos de widget, y las descripciones de widget se muestran como información sobre herramientas cuando los usuarios pasan el cursor sobre el widget.

<Callout variant="important">
  Las URL de enlace del widget deben usar los protocolos `http://` o `https://` y no pueden exceder los 2048 caracteres.
</Callout>

<CollapserGroup>
  <Collapser id="update-widget-link-description" title="Actualizar el enlace y la descripción del widget">
    ```graphql
    mutation UpdateWidgetsInPage($pageGuid: EntityGuid!, $widgets: [DashboardUpdateWidgetInput!]!) {
      dashboardUpdateWidgetsInPage(guid: $pageGuid, widgets: $widgets) {
        errors {
  ...
  <Collapser id="remove-widget-link-description" title="Eliminar enlace y descripción del widget">
    Para eliminar un enlace o una descripción, establezca el campo en `null`:
...
### Visibilidad de la variable de plantilla [#variable-visibility]

Puede ocultar las variables de plantilla de la barra de variables en el modo de visualización mientras continúan funcionando normalmente. Utilice el campo `hiddenOnVariablesBar` en el objeto `options` de la variable.

* **Nombre del campo:** `hiddenOnVariablesBar`
* **Tipo:** Booleano
* **Predeterminado:** `false` (las variables son visibles)

<CollapserGroup>
  <Collapser id="create-hidden-variable" title="Crear un dashboard con una variable oculta">
    ```graphql
    mutation {
      dashboardCreate(
  ...
  <Collapser id="update-variable-visibility" title="Actualice una variable para ocultarla o mostrarla">
    Para actualizar la visibilidad de una variable, incluya la configuración completa del dashboard con el valor de `hiddenOnVariablesBar` actualizado:
...
Para obtener más información sobre la visibilidad de las variables de plantilla, consulte [Mostrar y ocultar variables](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility).
