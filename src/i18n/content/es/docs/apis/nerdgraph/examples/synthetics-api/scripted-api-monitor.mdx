...
Puede crear un monitor de API con script empleando la mutación <DNT>`syntheticsCreateScriptApiMonitor`</DNT>. Esta mutación le permite configurar pruebas API personalizadas que ejecutan su código JavaScript para validar extremos de API.
...
Puede actualizar un monitor de API con script existente empleando la mutación <DNT>`syntheticsUpdateScriptApiMonitor`</DNT>. Esto le permite modificar la configuración de un monitor de API con script que ya se creó.
...
Cuando ya no se necesita un monitor de API con script, puedes eliminarlo de forma permanente empleando la mutación <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Consulta de monitores de API con script [#query-scripted-api-monitors]

Usa NerdGraph para obtener metadatos, scripts, ID de mapas o verificar el estado de tus monitores de API con scripts.
