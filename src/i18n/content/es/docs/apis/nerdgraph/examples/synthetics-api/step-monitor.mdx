...
Puede crear un monitor de pasos empleando la mutación <DNT>`syntheticsCreateStepMonitor`</DNT>. Esta mutación le permite configurar un monitoreo de navegador de varios pasos con un serial de acciones predefinidas.
...
Puede actualizar un monitor de pasos existente empleando la mutación <DNT>`syntheticsUpdateStepMonitor`</DNT>. Esto le permite modificar la configuración de un monitor de pasos que ya fue creado.
...
Cuando ya no se necesita un monitor de pasos, puedes eliminarlo de forma permanente empleando la mutación <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Monitores de pasos de consulta [#query-step-monitors]

Utilice NerdGraph para obtener metadatos, la configuración de pasos, los ID de mapas o verificar el estado de sus monitores de pasos.
...
* [Consulta de pasos del monitor](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitor-steps) - Recuperar la configuración de los pasos de los monitores de pasos.
