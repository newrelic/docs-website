...
Puede crear un monitor de ping empleando la mutación <DNT>`syntheticsCreateSimpleMonitor`</DNT>. Esta mutación le permite configurar el monitoreo para cualquier URL o extremo de acceso público.
...
Puede actualizar un monitor de ping existente empleando la mutación <DNT>`syntheticsUpdateSimpleMonitor`</DNT>. Esto le permite modificar la configuración de un monitor de ping que ya se creó.
...
Esta API le permite eliminar un monitor existente empleando la mutación <DNT>`syntheticsDeleteMonitor`</DNT> con el parámetro guid.
...
Utiliza NerdGraph para obtener metadatos, mapear ID o verificar el estado de tus monitores de ping.
