...
## Crear reglas de ofuscación [#create-obfuscation-rules]

Puede enmascarar datos confidenciales antes de que se recopilen creando reglas de ofuscación de ingesta directamente desde la plataforma de New Relic o NerdGraph. Estas reglas se aplican a todos los eventos del navegador. Las reglas configuradas a través de la plataforma o NerdGraph se aplican a aplicaciones inyectadas por APM, instalaciones de copiar y pegar e instalaciones de NPM. Para crear reglas a través de la plataforma o JavaScript, consulte [Ofuscar datos confidenciales en el agente del browser](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/).

Para crear reglas de ofuscación a través de NerdGraph:

<CollapserGroup>
  <Collapser id="retrieve" title="Paso 1: recuperar reglas de ofuscación">
    Recupere las reglas de ofuscación actuales para su aplicación de navegador utilizando la siguiente consulta:
    ...
    <Callout variant="important">
      Debe recuperar las reglas actuales antes de actualizarlas, ya que la mutación `agentApplicationSettingsUpdate` sobrescribirá cualquier regla existente con el nuevo conjunto de reglas que proporcione. Si desea conservar alguna regla existente, asegúrese de incluirla en el nuevo conjunto de reglas que envíe en la mutación.
    </Callout>
  </Collapser>

  <Collapser id="update" title="Paso 2: actualizar regla de ofuscación">
    Actualice las reglas de ofuscación usando la mutación `agentApplicationSettingsUpdate`. Puede agregar nuevas reglas o modificar las existentes definiendo las siguientes propiedades:

    * `regex`: el patrón de expresión regular que coincida con los datos que desea ofuscar. Por ejemplo, puede usar `\b\d{3}-?\d{2}-?\d{4}\b` para buscar coincidencias con números de Seguro Social.
    * `replacement`: la cadena que reemplazará los datos coincidentes. Por ejemplo, puede usar `XXX-XX-XXXX` para enmascarar los números de Seguro Social.
    * `eventFilter`: una matriz de tipos de evento de navegador a los que se debe aplicar la regla. Por ejemplo, puede especificar `["AjaxRequest"]` para aplicar la regla solo a los eventos de solicitud AJAX.
...
## Configurar la captura de carga de AJAX [#configure-payload-capture]

Puede recuperar y actualizar la configuración de captura de carga de AJAX para su aplicación de navegador utilizando NerdGraph. Esta configuración controla si se capturan las cargas de request y respuesta para las llamadas AJAX y corresponde a la [opción de captura de carga en la configuración de su aplicación](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/#payload-capture).

Puede habilitar la configuración de captura de carga en dos pasos:

1. Configuración para [recuperar la captura de carga actual](#query-payload-capture).

2. Establezca el [modo`capturePayloads` ](#update-payload-capture)en uno de los siguientes valores:

   * `none`: no se capturarán datos de carga.
   * `failures`: captura cargas solo para requests con un código de estado de `0`, un código de `400` o superior, o una firma de error de GraphQL en el `response body`.
   * `all`: captura cargas para todas las requests.

<CollapserGroup>
  <Collapser id="query-payload-capture" title="Consultar la configuración de captura de carga">
    Recupere la configuración actual de captura de carga para su aplicación de navegador utilizando la siguiente consulta:
  ...
  <Collapser id="update-payload-capture" title="Actualizar la configuración de captura de carga">
    Actualice la configuración de captura de carga usando la mutación `agentApplicationSettingsUpdate`:
    ...
    Configure las opciones `guid` y `capturePayloads` en las Variables:
...
* <DNT>**Control**</DNT>: Conserve autonomía sobre cuándo y cómo se aplican las actualizaciones.
* <DNT>**Confidence**</DNT>: Asegúrese de que se esté ejecutando una versión probada y aprobada del agente en todo momento.
* <DNT>**Testability**</DNT>: Pruebe fácilmente nuevas versiones en entornos aislados antes de decidir actualizar.
* <DNT>**Stability**</DNT>: Minimice las interrupciones inesperadas y mantenga un comportamiento constante de la aplicación.
* <DNT>**Efficiency**</DNT>: Reduzca el tiempo de despliegue y mitigue las dificultades de despliegue.
...
<CollapserGroup>
  <Collapser id="step1" title="Paso 1. Recupera la versión fijada actual">
  ...
  <Collapser id="step2" title="Paso 2. Revisa las versiones disponibles">
    Encuentre la versión del agente deseada en la [página de lanzamientos del agente New Relic Browser ](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/).
  ...
  <Collapser id="step3" title="Paso 3. Fija la versión deseada">
  ...
  <Collapser id="step4" title="Paso 4. Vuelva a desplegar su aplicación o reinicie su agente">
