...
<Steps>
  ...
  <Step>
    ...
    3. Tu [región New Relic](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers#verifying-account). Su región es `US` si su URL de New Relic es `one.newrelic.com`, `EU` si su URL está en `one.eu.newrelic.com` o `JP` si su URL está en `one.jp.newrelic.com`.
