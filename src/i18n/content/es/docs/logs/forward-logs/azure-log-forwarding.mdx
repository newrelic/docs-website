...
<Callout variant="tip">
  ¿Busca reenviar los logs de Azure VNet Flow? Consulte [Configurar el monitoreo de logs de Azure VNet Flow](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) para el reenvío de logs de VNet Flow dedicado con procesamiento delta incremental. Puede usar plantillas de despliegue de ARM y Bicep para la configuración automatizada.
</Callout>
...
11. Opcional: selecciona <DNT>**Authentication Mode**</DNT>. El valor predeterminado es `Local Authentication`. Elija `Managed Identity` si su entorno de Azure ha deshabilitado la autenticación local (claves de acceso compartido) para el cumplimiento de seguridad. Cuando se establece en `Managed Identity`, a la Function App se le asigna una identidad administrada asignada por el sistema y se le concede el rol `Azure Event Hubs Data Receiver` en el namespace de Event Hub — no se almacena ninguna cadena de conexión. Consulta [la información del modo de autenticación](#eventhub-authentication) para obtener más detalles.

12. Opcional: Configure los parámetros de procesamiento por lotes de EventHub (disponible en v2.8.0+) para optimizar el rendimiento:
...
13. Opcional: establezca en `true` los [registros de actividad de suscripción de Azure](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) que desea reenviar. Consulte [la información de suscripción](#subscription-activity-logs) en este documento para obtener más detalles.

14. Haga clic en <DNT>**Review + create**</DNT>, revise los datos que ha insertado y haga clic en <DNT>**Create**</DNT>.
...
### Opcional: configurar el modo de autenticación [#eventhub-authentication]

A partir de la versión 3.2.0, la plantilla de ARM admite dos métodos de autenticación para conectar la Function App al Event Hub:

* <DNT>**Local Authentication**</DNT> (predeterminado): la Function App se conecta mediante una cadena de conexión de clave de acceso compartido almacenada en la configuración de la aplicación. Este es el enfoque tradicional y coincide con versiones anteriores de la plantilla.

* <DNT>**Managed Identity**</DNT>: la Function App usa una identidad administrada asignada por el sistema con el rol integrado `Azure Event Hubs Data Receiver` en el namespace de Event Hub. No se almacenan secretos ni cadenas de conexión. Use esta opción cuando:

  * Su organización ha deshabilitado la autenticación local (claves de acceso compartido) en los namespace de Event Hub por cumplimiento de seguridad.
  * Prefiere la autenticación sin claves administrada completamente por Azure AD.

Cuando se selecciona `Managed Identity`, la plantilla automáticamente:

* Crea una identidad administrada asignada por el sistema en la Function App.
* Otorga a la identidad el rol `Azure Event Hubs Data Receiver` en el namespace de Event Hub.
* Configura la Function App con `EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace` (el nombre de host del namespace del centro de eventos) en lugar de una cadena de conexión. El entorno de ejecución de Azure Functions usa automáticamente tokens de Azure AD para la autenticación.
