...
1. Instale la integración de New Relic Kubernetes siguiendo los pasos de [esta página](/install/kubernetes). Esta integración incluye el complemento Kubernetes para registro.

2. Opcionalmente, puede ajustar aún más su instalación en [el Paso 4 desde la instalación guiada](/install/kubernetes) empleando las numerosas opciones de configuración disponibles en el [repositorio de newrelic-logging](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration). Sin embargo, recomendamos la configuración estándar, ya que es válida para la mayoría de usuarios.
