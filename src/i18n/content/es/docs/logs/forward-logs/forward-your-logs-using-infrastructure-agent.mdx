---
title: Reenvíe su registro utilizando el agente de infraestructura
tags:
  - Logs
...
* CentOS versión 7, 8 y 9 Stream (Rocky Linux y AlmaLinux también son compatibles)
* RedHat versión 7, 8 y 9
...
## Reenviar registro automáticamente con instalación guiada [#infra]
...
## Habilitar el reenvío de registros en el agente instalado manualmente [#manual]
...
### Paso 2. Establecer el parámetro de reenvío de registros [#parameters]
...
<CollapserGroup>
  <Collapser
    ...
    title="archivo"
  >
    Ruta al archivo de registro o archivos. El agente realiza un seguimiento de los cambios en el archivo de registro de forma similar al shell `tail -f` .
  ...
  <Collapser
    ...
    title="systemd"
  >
    Utilice el parámetro `systemd` para reenviar mensajes de registro recopilados por el daemon `journald` en entornos Linux. Este tipo de entrada requiere que el agente se ejecute en [modo raíz](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes).
  ...
  >
    ...
    * `unix_permissions:` El valor predeterminado es `0644` para sockets de dominio. Esto limita las entradas a procesos que se ejecutan como root. Puede utilizar `0666` para escuchar procesos que no sean raíz (bajo su propia responsabilidad).
      ...
      Cuando se ejecuta el agente en [modo privilegiado](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes), los puertos y sockets deben estar disponibles o ser propiedad de `nri-agent` y con `0666` permisos de archivo para que otros procesos puedan escribir registros en los sockets.
  ...
  <Collapser
    ...
    title="TCP"
  >
    Registro recuperado a través de conexiones TCP.
  ...
  >
    <Callout variant="important">
      Disponible desde el agente de infraestructura v.1.24.3 Solo compatible con Windows Server 2019 y posteriores. Utilice [winlog](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/#winlog) en su lugar para versiones anteriores.
    </Callout>

    Recopile eventos de los canales log de Windows utilizando la nueva API de registro de eventos de Windows mediante el [complemento Winevtlog Fluent Bit](https://docs.fluentbit.io/manual/pipeline/inputs/windows-event-log-winevtlog).

    <DoNotTranslate>
      **Parameters:**
    </DoNotTranslate>

    * `channel`: Nombre del registro del canal del que se recopilará.
    ...
    * `use-ansi`: Utilice codificación ANSI en mensajes de winevtlog. Usamos codificación ANSI de forma predeterminada en Windows Server 2016 y versiones anteriores, y UTF-8 en las más nuevas. Puede anular este comportamiento con este parámetro de configuración si estos valores predeterminados no se adaptan a su caso de uso.

      Todos los eventos se recopilan del canal especificado de forma predeterminada. Configure las secciones `collect-eventids` y `exclude-eventids` para evitar enviar registros no deseados a su cuenta de New Relic.
  ...
  >
    <Callout variant="important">
      Winlog solo puede recopilar el registro de eventos clásico. Al intentar capturar a otros, se recopilará silenciosamente el registro de la aplicación.
    </Callout>
    ...
    * `channel`: Nombre del registro del canal del que se recopilará. No funciona para canales personalizados.

    * `collect-eventids`: Lista de ID de eventos de Windows que se recopilarán y reenviarán a New Relic. Se admiten rangos de ID de eventos.

    * `exclude-eventids`: Lista de ID de eventos de Windows que se excluirán de la colección. Se admiten rangos de ID de eventos.

    * `use-ansi`: Utilice codificación ANSI en mensajes winlog. Usamos codificación ANSI de forma predeterminada en Windows Server 2016 y versiones anteriores, y UTF-8 en las más nuevas. Puede anular este comportamiento con este parámetro de configuración si estos valores predeterminados no se adaptan a su caso de uso.
...
<CollapserGroup>
  <Collapser
    ...
    title="Atributo"
  >
    Lista de atributos personalizados especificados como pares de valores principales que se pueden utilizar para enviar datos adicionales con el registro que luego puede consultar. El parámetro de configuración `attributes` se puede utilizar con cualquier origen log .

    <Callout variant="important">
      El parámetro de configuración `attributes` no agrega un atributo personalizado para el registro reenviado mediante la configuración externa de Fluent Bit (por ejemplo, usando el parámetro de configuración `fluentbit` . En este escenario, debes consultar la opción `record_modifier` en la [documentación de Fluent Bit](https://docs.fluentbit.io/manual/).
    </Callout>
  ...
  <Collapser
    ...
    title="atributo insertado automáticamente por el agente de infraestructura"
  >
    El agente de infraestructura inserta automáticamente el atributo log para su conveniencia. Algunos de ellos se insertan para cualquier log , mientras que otros dependen del parámetro de configuración que utilizó al configurar el reenviador de registros.
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            Nota: Si el registro capturado pertenece a una aplicación instrumentada usando APM, el campo `entity.guids` contiene tanto el GUID de la entidad de infraestructura, como el GUID de APM, separados por un delimitador de barra vertical ( | ).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            El [tipo de complemento de entrada de Fluent Bit](https://docs.fluentbit.io/manual/pipeline/inputs) subyacente utilizado para capturar el registro. Actualmente sus valores son `tail`, `systemd`, `winlog`, `syslog` y `tcp`.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            Indica la utilidad utilizada para capturar el registro. En este caso es el propio agente de infraestructura, por lo que este atributo siempre tiene el valor `nri-agent`.
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <Collapser
    ...
    title="max_line_kb"
  >
    Tamaño máximo de entradas/líneas log en KB. Si las entradas log exceden el límite, se omiten. El valor predeterminado es `128`.
  </Collapser>
  ...
  >
    ...
    `config_file:` ruta a un archivo de configuración de Fluent Bit existente. Tenga en cuenta que cualquier fuente superpuesta genera mensajes duplicados en nuestra UI de registro.
...
## Habilite el registro para su integración en el host [#on-host]
...
<CollapserGroup>
  <Collapser
    ...
    title="Registro de búsqueda elástica"
  >
    Copie o cambie el nombre del archivo `elasticsearch-log.yml.example` a `elasticsearch-log.yml` para habilitar el análisis automático de registros con formato JSON de Elasticsearch y el reenvío a New Relic. No es necesario reiniciar el agente.
  ...
  <Collapser
    id="nginx-logs"
    title="Registro NGINX"
  >
    Copie o cambie el nombre del archivo `nginx-log.yml.example` a `nginx-log.yml` para habilitar el acceso automático a NGINX y el análisis de errores de registros y el reenvío a New Relic. No es necesario reiniciar el agente.
  ...
  <Collapser
    id="rabbitmq-logs"
    title="Registro de Rabbitmq"
  >
    Copie o cambie el nombre del archivo `rabbitmq-log.yml.example` a `rabbitmq-log.yml` para habilitar el análisis de registros de errores automático de Rabbitmq y el reenvío a New Relic. No es necesario reiniciar el agente.
  ...
  <Collapser
    id="redis-logs"
    title="Registro de Redis"
  >
    Copie o cambie el nombre del archivo `redis-log.yml.example` a `redis-log.yml` para habilitar el análisis de registros de errores de Redis automático y el reenvío a New Relic. No es necesario reiniciar el agente.
...
## Habilite el reenvío de registros en el agente instalado usando el tarball de Linux [#tarball-install]
...
<CollapserGroup>
  ...
  <Collapser
    className="freq-link"
    id="no-data"
    title="No aparecen datos al seguir un archivo"
  >
    La característica de reenvío de registros requiere que el agente tenga permiso para leer las fuentes de datos. Al ejecutar el agente de infraestructura en [modos privilegiados o no privilegiados](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes), asegúrese de que el usuario que ejecuta `nri-agent` pueda leer el archivo de registro que desea reenviar (y cualquier directorio intermediario en su ruta).
  ...
  <Collapser
    className="freq-link"
    id="syslog"
    title="No aparecen datos al capturar a través de un socket Syslog"
  >
    La característica de reenvío de registros requiere que el agente tenga permiso para leer las fuentes de datos. Al ejecutar el agente de infraestructura en [modos privilegiados o no privilegiados](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes):
    ...
    * Si está utilizando sockets IP, asegúrese de que el puerto que está utilizando no sea uno reservado para el sistema (como el puerto `80`, por ejemplo).
      ...
      Si no aparecen datos después de habilitar la administración de registros, siga [los procedimientos estándar de administración de registros y resolución de problemas](/docs/logs/new-relic-logs/troubleshooting/no-data-appears-logs).
  </Collapser>

  <Collapser
    className="freq-link"
    id="proxy"
    title="No aparecen datos usando el proxy del agente de infraestructura"
  >
    Como se explica en las [pautas de configuración del agente de infraestructura](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#proxy), el parámetro `proxy` debe usar HTTP o HTTPS y tener el formato `https://user:password@hostname:port`. El agente puede analizar el parámetro sin HTTP o HTTPS, pero el reenviador de registros no puede. Verá un error como el siguiente en el registro detallado del agente:
    ...
    Para `HTTP` servidores proxy no es necesario configurar ningún certificado. El complemento carga los certificados del sistema y New Relic envía el registro al extremo de registro. Sin embargo, puede especificar el certificado autofirmado del proxy (archivo PEM) utilizando el parámetro `caBundleFile` o `caBundleDir` .
    ...
    * Para `HTTPS`, puede configurarlo de una de las siguientes maneras:
      ...
      * (Recomendado) Importe el certificado de proxy al grupo del sistema. Importe el certificado autofirmado del proxy (archivo PEM) mediante la herramienta MMC. Consulte [este enlace](https://www.ssls.com/knowledgebase/how-to-import-intermediate-and-root-certificates-via-mmc/) y en <DoNotTranslate>**Step 2**</DoNotTranslate> asegúrese de importarlo en su `Trusted Root Certification Authorities` en lugar de en el `Intermediate Certification Authorities`.
  ...
  <Collapser
    className="freq-link"
    id="agent-logs"
    title="Envío del registro del agente de infraestructura a New Relic"
  >
    Puede configurar el agente de infraestructura para que envíe su propio registro a New Relic. Esto es útil para resolver problemas con el reenvío de registros, el agente o al contactar [al soporte](https://support.newrelic.com/).

    <Callout variant="important">
      El registro de trazas genera una gran cantidad de datos muy rápidamente. Para reducir el consumo de espacio en disco y la ingesta de datos, cuando termine de generar el registro, asegúrese de configurar `level: info` (o menos).
    </Callout>

    Para reenviar el registro del agente de infraestructura a New Relic:

    1. Edite su archivo `newrelic-infra.yml` .

    2. Habilite el reenvío de registros a New Relic agregando el siguiente fragmento de configuración:
    ...
    3. [Reinicie el agente](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status) para cargar la nueva configuración.
       ...
       Esta configuración configura el agente en modo de resolución de problemas, pero el reenviador de registros (basado en [Fluent Bit](https://fluentbit.io/)) continuará en un modo no detallado.
  </Collapser>

  <Collapser
    className="freq-link"
    id="fluentbit-logs"
    title="Habilitar el modo detallado en el reenviador de registros (Fluent Bit)"
  >
    A veces puedes tener problemas con el propio reenviador de registros. Por ejemplo, puede haber problemas para acceder a un canal específico al enviar el registro de eventos de Windows o al acceder a un archivo de registro en particular. En estas situaciones, también puede habilitar el modo detallado para el reenviador de registros.
    ...
    2. Habilite el registro detallado de Fluent Bit agregando el siguiente fragmento de configuración:
  ...
  <Collapser
    className="freq-link"
    id="no-fb"
    title="Fluent Bit no comienza con el agente de infraestructura"
  >
    <Callout variant="important">
  ...
  <Collapser
    ...
    title="Error de tiempo de ejecución en Windows"
  >
    Puede aparecer uno de los siguientes mensajes de error al habilitar el reenvío de registros en Windows:
  ...
  <Collapser
    className="freq-link"
    id="too-many-files"
    title="Errores al seguir una gran cantidad de archivo de log (Linux)"
  >
    ...
    * `The user limit on the total number of inotify watches was reached or the kernel failed to allocate a needed resource`
    ...
    La tecnología subyacente que utilizamos para reenviar registros, [Fluent Bit](https://fluentbit.io/), abre un descriptor de archivo y establece una vigilancia de notificación para cada archivo que configure para ser reenviado. Además, al momento de escribir esta sección, Fluent Bit utiliza un conjunto adicional de 32 descriptores de archivos para su funcionamiento normal, con otro descriptor de archivos adicional cuando se apaga. Por lo tanto, <DoNotTranslate>**to capture a large amount of files you need to ensure that both the file descriptor and inotify watch limits are slightly greater than the amount of log files you wish to tail**</DoNotTranslate>.

    Las siguientes instrucciones resumen cómo aumentar estos límites si desea seguir 10,000 archivos de registro. Además, se supone que el agente de infraestructura está instalado en [modo de ejecución](/docs/infrastructure/install-infrastructure-agent/linux-installation/linux-agent-running-modes/) `root` y, por lo tanto, debe ejecutarse con el usuario `root` .
  ...
  <Collapser
    ...
    title="Instalar una versión particular de Fluent Bit (Linux)"
  >
    ...
    Para actualizar a una versión particular de Fluent Bit, simplemente ejecute (en los siguientes ejemplos, se usa la versión `2.2.2` ):
  ...
  <Collapser
    ...
    title="Revertir a fluent-bit 1.x después del agente de infraestructura 1.42.0 en Linux"
  >
...
## Desactivar reenvío de registros [#uninstall]

Para deshabilitar las capacidades de reenvío de registros, vaya a su directorio `logging.d` y elimine los archivos con la extensión `.yml` que se agregaron originalmente durante el proceso [de configuración](#configure) .
