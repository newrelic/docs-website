...
4. Si corresponde, [configure el extremo de la UE o Japón](#eu-configuration)
...
## Configure el plug-in de Fluentd para cuentas de la UE o Japón [#eu-configuration]

De forma predeterminada, el plug-in de Fluentd reenvía los logs a nuestro extremo de EE. UU.: `https://log-api.newrelic.com/log/v1`. Si su organización de New Relic se encuentra en nuestro [centro de datos de la UE o Japón](/docs/accounts/accounts-billing/account-setup/choose-your-data-center), debe configurar manualmente la propiedad `base_uri` en el extremo adecuado:

* UE: `https://log-api.eu.newrelic.com/log/v1`
* Japón: `https://log-api.jp.nr-data.net/log/v1`

Por ejemplo, para reenviar logs al extremo de la UE:
...
Para las cuentas de Japón, reemplace el `base_uri` en el bloque `<match>` con el extremo de Japón:
...
## Configurar la transformación de UTF-16LE a UTF-8 [#configure-utf-16le]
...
Los archivos de log japoneses codificados en Shift-JIS (CP932) se convierten a la [codificación UTF-8 requerida](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#general) y son necesarios para importar y enviar datos de log a New Relic.
