---
title: Envíe sus datos de registro con nuestra API log
tags:
  - Logs
  - Log management
  - Log API
...
Si nuestras [soluciones de reenvío de registros](/docs/logs/forward-logs/) no satisfacen sus necesidades, puede utilizar nuestra API log para enviar datos log directamente a New Relic a través de un extremo HTTP.

¿Quieres probar nuestra API log ? Si aún no lo ha hecho, cree su cuenta New Relic gratuita a continuación para comenzar a monitorear sus datos hoy.
...
4. Asegúrese de que su `Api-Key` o `License-Key` esté incluido en sus [encabezados](#auth-headers) o [parámetro de consulta](#query-parameters). Consulte los [ejemplos de registro JSON](#log-attribute-examples).
...
Su <InlinePopover type="licenseKey"/>sirve para autenticar su solicitud en la API log y determina la cuenta de New Relic donde se escribirán los mensajes de registro enviados. Debe pasarse como un encabezado HTTP o un parámetro de cadena de consulta.
...
La clave de licencia también se puede pasar como parámetro de cadena de consulta en la URL. Esto resulta útil al enviar registros desde fuentes basadas en la nube que no permiten encabezados de solicitud HTTP personalizados.
...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Este es el campo <DoNotTranslate>**main**</DoNotTranslate> mensaje de log que se busca de forma predeterminada
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            Campo principal para identificar registros y reglas de análisis coincidentes
          </td>
        </tr>
  ...
  >
    Cuando utilice el formato detallado para crear su cuerpo, debe ser un <DoNotTranslate>**JSON array**</DoNotTranslate> que contenga uno o más objetos JSON, cada uno de los cuales con el siguiente formato:
...
Restricciones sobre el registro enviado a la API log :

* Tamaño total de la carga útil:

  <DoNotTranslate>**1MB(10^6 bytes) maximum per POST**</DoNotTranslate>

  . Recomendamos encarecidamente utilizar compresión.

* La carga útil debe estar codificada como

  <DoNotTranslate>**UTF-8**</DoNotTranslate>
...
Límites de velocidad en el registro enviado a la API log :
...
Aceptamos cualquier carga útil JSON válida. La carga útil debe codificarse como <DoNotTranslate>**UTF-8**</DoNotTranslate>.
...
<CollapserGroup>
  ...
  <Collapser
    id="json-logs"
    title="Atributo de bloque de registro"
  >
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Este es el campo principal de mensajes de registro que se busca de forma predeterminada.
          </td>
        </tr>
...
Nuestras capacidades de administración de registros analizarán cualquier atributo `message` como JSON. El atributo JSON resultante en el mensaje analizado se agregará al evento. Si el atributo `message` no es JSON, se deja como está.
...
Esto dará como resultado que el siguiente atributo se almacene con el registro de evento:
...
Este mensaje `POST` daría como resultado que el siguiente mensaje de registro se almacene en New Relic:
...
A continuación se muestra un ejemplo de atributo de bloque de registro almacenado:
...
Explore [los datos de registro en su plataforma](/docs/logs/ui-data/use-logs-ui/).

* Obtenga una visibilidad más profunda de los datos de rendimiento de su aplicación y de su plataforma reenviando su registro con nuestras capacidades [de contexto de inicio de sesión](/docs/logs/logs-context/configure-logs-context-apm-agents/) .
