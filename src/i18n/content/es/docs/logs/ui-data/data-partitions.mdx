...
El namespace de una partición determina su período de retención. Ofrecemos tres opciones de retención:
...
* <DNT>**Direct to live archive:**</DNT> Omite la retención estándar por completo. Los logs enviados a una partición que usa este namespace van directamente al almacenamiento a largo plazo de Live Archives en lugar de llegar primero al almacenamiento estándar. Este namespace funciona mejor para los logs que rara vez necesita consultar pero que debe retener durante mucho tiempo, como los logs de cumplimiento o auditoría, ya que no paga por el almacenamiento de nivel estándar en datos que no planea mirar a diario. Esta opción requiere que habilite Live Archives en la cuenta. Para obtener más información, consulte [Almacenamiento de logs a largo plazo con Live Archives](/docs/logs/get-started/live-archives/).
...
## Controlar el acceso a las particiones [#data-access-control]

Además de organizar los datos de log, las particiones también sirven como base para controlar el acceso de los usuarios a datos de log específicos. Con [el control de acceso a datos](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control), puede crear políticas que restrinjan o permitan el acceso a particiones específicas para grupos de usuarios específicos.

Esto resulta útil cuando:

* Debes cerciorarte de que los equipos solo vean los logs relevantes para sus servicios.
* Usted tiene requisitos de cumplimiento que restringen el acceso a datos de log confidenciales.
* Desea evitar el acceso no autorizado a los logs que contienen información de identificación personal (PII).