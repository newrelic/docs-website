...
<CollapserGroup>
  ...
  >
    ...
    3. Configure y cree un nuevo formateador nrlogrus. Se debe pasar una aplicación válida y cualquier formateador Logrus válido. El formateador aprobado determinará la apariencia del registro que se escriba.

    ```go
    nrlogrusFormatter := nrlogrus.NewFormatter(newRelicApplication, &logrus.TextFormatter{})
    ```

    4. Configure el formateador de su logger en el formateador nrlogrus recién creado.
  ...
  >
    ...
    Cuando instrumente el registro dentro de una transacción, debe pasar esa transacción a nrslog. Esto creará un nuevo objeto logger para esa transacción que vincula el registro manejado por el agente con la transacción y los períodos en los que ocurrieron.
    ...
    De lo contrario, puedes usar el puntero del objeto de transacción de esta manera:
