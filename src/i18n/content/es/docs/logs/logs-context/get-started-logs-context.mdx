...
import apmApmsummaryViewLogs from 'images/apm_screenshot-crop_apmsummary-view-logs-.webp'

import apmLogsPatterns from 'images/apm_screenshot-crop_logs-patterns.webp'

import apmLogsContextPatternsTwo from 'images/apm_screenshot-crop_logs-context-patterns-two.webp'
...
Nuestro <InlinePopover type="apm"/>agente tiene una característica llamada <DoNotTranslate>**logs in context**</DoNotTranslate>. Para obtener más información sobre los beneficios de esta característica, consulte [logs en el contexto](/docs/logs/logs-context/logs-in-context).
...
* Reporta algo [de log métrica](#log-metrics). Estos se muestran en el
...
<figcaption>
  Profundice en su registro, traza y errores, todo desde la página APM <DoNotTranslate>**Summary**</DoNotTranslate> en New Relic.
</figcaption>
...
Si no puede o no quiere utilizar el reenvío de registros de APM , puede [reenviar su registro utilizando otra solución](/docs/logs/get-started/get-started-log-management#integrate-logs).
...
* Con la excepción del agente Node.js, el
  ...
  reenvío de registros no envía el log completo. [Conozca los detalles del reenvío de registros.](#forwarding-details)
...
<CollapserGroup>
  <Collapser
    ...
    title="Detalles del reenvío de registros"
  >
...
* Mantenga la [decoraciónlog ](#apm-logs-defined)en su lugar pero desactive el reenvío de registros del agente APM y en su lugar utilice su propia solución de reenvío de registros.
...
Las métricas de registro informadas por <InlinePopover type="apm"/>agente se muestran en el gráfico <DoNotTranslate>**Logs**</DoNotTranslate> de la página APM <DoNotTranslate>**Summary**</DoNotTranslate>. Estas métricas son independientes del [reenvío de registros característico](#apm-logs-defined) y no se pueden desactivar a nivel de cuenta. Para deshabilitar el registro métrico, consulte los documentos de configuración APM dedicados (por ejemplo, [esta opción de configuración `logging_metrics` para PHP](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#cfg-application_logging_metrics-enabled)).
...
<Callout variant="caution">
  Si desea utilizar el agente APM para enviar registros directamente desde su aplicación, debe deshabilitar o modificar las soluciones de reenvío de registros que actualmente recopilan registros de esas aplicaciones. De lo contrario, enviará registros duplicados, lo que resultará en una facturación doble.
</Callout>
...
Para obtener más información, siga los procedimientos para desactivar su [reenviador de registros específico](/docs/logs/forward-logs/enable-log-management-new-relic#log-forwarding).

## Administre sus límites de ingesta [#ingest]

<DoNotTranslate>**Example:**</DoNotTranslate> Su equipo de ingeniería está resolviendo un problema con su aplicación, por lo que aumenta temporalmente el volumen de registro recopilado por el agente APM para proporcionar un registro más granular. Sin embargo, si deja límites más altos durante varios días, esto podría provocar el envío de datos innecesarios que aumentarán su factura.
...
<CollapserGroup>
  <Collapser
    id="limits-ui"
    title="Ver límites de datos y uso en UI"
  >
    Para [revisar sus límites de datos en la UI](/docs/data-apis/manage-data/view-system-limits): en el [menú de usuario](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings), haga clic en <DoNotTranslate>**Manage your data > Limits**</DoNotTranslate>. O, para ver su uso, haga clic en <DoNotTranslate>**View your usage**</DoNotTranslate>.
  ...
  <Collapser
    id="query-estimate"
    title="Consulta NRQL : ejemplo de costo estimado"
  >
  ...
  <Collapser
    id="nrql-alert"
    title="Alerta NRQL : ejemplo de umbral de uso"
  >
...
<CollapserGroup>
  <Collapser
    id="when"
    title="¿Cuándo y dónde comenzaron los errores?"
  >
    Cuando el ingeniero mira la página <DoNotTranslate>**APM Summary**</DoNotTranslate> de la aplicación, ve muchos más registros de errores que conducen al aumento del tiempo de respuesta. Quieren ver si el registro puede exponer la causa raíz del problema.
    ...
    En la misma página UI , hacen clic en el gráfico <DoNotTranslate>**Logs**</DoNotTranslate> para poder revisar la vista <DoNotTranslate>**Log summary**</DoNotTranslate>. Esta vista les muestra cuándo se han recopilado registros instrumentados y cuándo esos registros tenían un nivel de gravedad `Error` o peor. También les muestra una lista de patrones de logs y qué porcentaje de todos los logs se ajustan a cada patrón.
  </Collapser>

  <Collapser
    id="patterns"
    title="¿Qué patrones surgen en el registro?"
  >
    En la UI <DoNotTranslate>**Log patterns**</DoNotTranslate>, han comenzado a aparecer mensajes de error raros con frecuencia y ahora aparecen en la lista. Esto ayuda al ingeniero a concentrarse en el registro problemático en lugar de en todas las ruidosas actualizaciones de estado.
  ...
  <Collapser
    id="log-details"
    title="¿Qué pueden revelar más detalles log ?"
  >
    El ingeniero quiere ver todos los valores contenidos en el log , por lo que hace clic en la vista <DoNotTranslate>**Log details**</DoNotTranslate>. Esto les ayuda a validar que el log en sí sea significativo. También les permite profundizar más en el entorno Kubernetes donde se ejecuta la aplicación o directamente en cualquier rastreo distribuido o <InlinePopover type="apm"/>error relacionado con el log .
    ...
    El ingeniero decide mirar el error APM donde puede ver un rastreo del stack completo. También pueden ver el registro desde su UI <DoNotTranslate>**Errors inbox**</DoNotTranslate>.
  ...
  <Collapser
    id="log-sampling"
    title="¿Qué datos de prueba adicionales ayudarán a solucionar el problema?"
  >
  ...
  <Collapser
    id="next-steps"
    title="Problema resuelto. ¿Ahora que?"
  >
