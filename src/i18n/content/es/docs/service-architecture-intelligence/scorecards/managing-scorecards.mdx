...
6. En la pantalla **Define maturity levels**, arrastre y suelte cada regla en el grupo de madurez adecuado (**Below basic**, **Basic maturity**, **Intermediate maturity** o **Advanced maturity** \[Madurez avanzada]), luego haga clic en **Next**. Para obtener más información, consulte [Madurez del servicio](/docs/service-architecture-intelligence/scorecards/service-maturity).
...
<Callout variant="tip" title="consejo">
  Utilice las mutaciones de NerdGraph para crear o administrar sus reglas de Scorecards. Para obtener más información, consulte el [tutorial de NerdGraph Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>
...
<Callout variant="tip">
  Puede asignar una regla a un [nivel de madurez](/docs/service-architecture-intelligence/scorecards/service-maturity) al definir los niveles de madurez durante la creación del Scorecard, o al editar la regla más adelante. Para establecer el peso de una regla o crear niveles de madurez personalizados, utilice el [tutorial de NerdGraph Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>
...
También puede habilitar o deshabilitar las reglas existentes para un Scorecard. Cuando edita una regla, puede cambiar su [nivel de madurez](/docs/service-architecture-intelligence/scorecards/service-maturity); los pesos de las reglas se configuran a través de la [API de NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
...
## Configurar notificaciones de reglas [#rule-notifications]

Puede recibir una notificación cuando la puntuación de una regla caiga por debajo de un umbral que establezca. Las notificaciones de Scorecards se basan en las [alertas](/docs/alerts/overview/) de New Relic: cuando crea una, Scorecards crea automáticamente una [política de alertas](/docs/alerts/organize-alerts/create-edit-or-find-alert-policy/) y una [condición](/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/) para la regla y adjunta un [flujo de trabajo](/docs/alerts/get-notified/alert-event-workflows/) que envía la notificación al destino elegido.

**Para configurar una notificación:**

1. Log sesión en su cuenta New Relic .

2. Vaya al **panel de navegación izquierdo &gt; All Capabilities &gt; Scorecards** y seleccione un Scorecard.

3. Abra la regla sobre la que desea recibir notificaciones.

4. En la página de la regla, haga clic en **Set up a notification**, luego complete el asistente:

   1. **Establecer un umbral**: recibirá una notificación cuando la puntuación de la regla caiga por debajo de este nivel.
   2. **Seleccionar una cuenta**: la cuenta donde se almacenan la política de alertas y la condición. Scorecards crea la política y la condición para usted automáticamente y nombra la política según la regla.
   3. **Agregar un flujo de trabajo**: elija el flujo de trabajo que entrega la notificación a su [destino](/docs/alerts/get-notified/destinations/) seleccionado.
   4. **Revisar y crear**: revise su configuración y cree la notificación.
