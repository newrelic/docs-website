    ...
    <TabsPageItem id="linux">
      ...
      * Versiones del kernel de Linux:
      ...
      <Callout variant="important">
        Algunas distribuciones de Linux permiten ejecutar versiones anteriores del kernel en versiones más recientes del sistema operativo. Aunque su distribución esté [listada a continuación](#supported-linux-distributions), el agente eBPF no funcionará si la versión del kernel es inferior a `5.8`. Lo mismo se aplica a los kernels más recientes: el agente no funcionará en versiones superiores al rango compatible.
      </Callout>
      ...
      * Red Hat Enterprise Linux `9.0` y superior
      * Ubuntu `20.04` y superior
      * Amazon Linux `2` y `2023`
      * Bottle Rocket `1.51.0` y superior
      * AlmaLinux `9.7` y superior
      * Oracle Linux `8` y superior
      * openSUSE Leap `15.5` y superior
      * SLES `15.7` y superior
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `otlp.eu01.nr-data.net:443` (Región de la UE)
        * `otlp.jp.nr-data.net:443` (región de JP)
        * `gov-otlp.nr-data.net:443` (Cuentas GOV/FedRAMP)
...
Si su entorno enruta el tráfico saliente a través de un proxy HTTP corporativo, consulte [Configurar un proxy para el <DNT>eBPF agent</DNT>](/docs/ebpf/proxy-configuration) para obtener instrucciones de configuración.
