---
title: Compatibilidad y requisitos de eBPF
metaDescription: 'Learn about New Relic eBPF agent compatibility requirements, supported databases, platforms, and querying capabilities.'
tags:
  - New Relic integrations with eBPF
  - New Relic eBPF agent
  - eBPF integration
  - eAPM
  - eBPF compatibility
  - eBPF requirements
freshnessValidatedDate: never
translationType: machine
---

Esta página proporciona información completa sobre compatibilidad para el agente eBPF New Relic, incluida la base de datos compatible, los requisitos de implementación y las capacidades de consulta de datos que se aplican a todos los métodos de instalación.

<Tabs>
  <TabsBar>
    <TabsBarItem id="k8s">Para Kubernetes</TabsBarItem> <TabsBarItem id="linux">Para Linux</TabsBarItem>
  </TabsBar>

  <TabsPages>
    <TabsPageItem id="k8s">
      * Asegúrese de que se cumplan todos los [requisitos de la distribución de Linux](/docs/ebpf/requirements/#kernel-requirements) para el nodo de Kubernetes.

      ### Entornos de Kubernetes [#kubernetes-environments]

      Se admiten los siguientes entornos de Kubernetes:

      * Servicio Amazon Elastic Kubernetes (EKS)
      * Motor Google Kubernetes (GKE)
      * Servicio Azure Kubernetes (AKS)
      * Red Hat OpenShift
      * Microk8s

      ### Soporte para el entorno de ejecución de contenedores [#container-runtime]

      Se admiten los siguientes tiempos de ejecución de contenedores:

      * Entorno de ejecución predeterminado de Docker (runc)
      * containerd
      * CRI-O
      * Podman
      * Youki
      * crun
    </TabsPageItem>

    <TabsPageItem id="linux">
      ### Requisitos del kernel [#kernel-requirements]

      * Versiones del kernel de Linux:

        * `x86-64` (amd64): `v5.8` a `v6.14`
        * `arm64` (aarch64): `v5.8` a `v6.14`

      <Callout variant="important">
        Algunas distribuciones de Linux permiten ejecutar versiones anteriores del kernel en versiones más recientes del sistema operativo. Aunque su distribución esté [listada a continuación](#supported-linux-distributions), el agente eBPF no funcionará si la versión del kernel es inferior a `5.8`. Lo mismo se aplica a los kernels más recientes: el agente no funcionará en versiones superiores al rango admitido, incluido `v7.x`.
      </Callout>

      ### Distribuciones de Linux compatibles [#supported-linux-distributions]

      Las siguientes distribuciones de Linux son compatibles:

      * CentOS `9` y superior
      * Debian `11` y superior
      * Red Hat Enterprise Linux `9.0` y superior
      * Ubuntu `20.04` y superior
      * Amazon Linux `2` y `2023`
      * Bottle Rocket `1.51.0` y superior
      * AlmaLinux `9.7`

      ### Requisito del sistema [#system-requirements]

      * Privilegios de root (Requeridos para la instalación y operación)
      * Un mínimo de `2 GB` de RAM
    </TabsPageItem>
  </TabsPages>
</Tabs>

### Matriz de soporte de base de datos [#database-support]

El agente eBPF puede monitorear la siguiente base de datos y sus versiones compatibles:

<table>
  <thead>
    <tr>
      <th>
        Base de datos
      </th>

      <th>
        Versiones compatibles
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        MySQL
      </td>

      <td>
        `v5.6.51`

         y superior
      </td>
    </tr>

    <tr>
      <td>
        MongoDB
      </td>

      <td>
        `v3.6.23`

         y superior
      </td>
    </tr>

    <tr>
      <td>
        Redis
      </td>

      <td>
        `v3.2.12`

         y superior
      </td>
    </tr>

    <tr>
      <td>
        PostgreSQL
      </td>

      <td>
        `v9.6.24`

         y superior
      </td>
    </tr>

    <tr>
      <td>
        MSSQL
      </td>

      <td>
        `v14.x`

         y superior
      </td>
    </tr>

    <tr>
      <td>
        DinamoDB
      </td>

      <td>
        `v2.0.0`

         y superior
      </td>
    </tr>
  </tbody>
</table>

### Soporte para protocolos adicionales [#protocol-support]

El agente eBPF también admite el monitoreo de estos protocolos:

* HTTP

* Apache Thrift

* gRPC

* Casandra

* kafka

* AMQP

  <Callout variant="tip">
    Para Cassandra, Kafka y AMQP, el agente eBPF no admite el enlace de segmentos.
  </Callout>

### Matriz de compatibilidad de Thrift [#thrift-support]

El agente eBPF admite los siguientes protocolos de transporte y capas de transporte de Thrift:

<table>
  <thead>
    <tr>
      <th>
        Tipo
      </th>

      <th>
        Soportado
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Protocolos de transporte
      </td>

      <td>
        `TBinaryProtocol`

        , 

        `TCompactProtocol`

        , 

        `TJSONProtocol`
      </td>
    </tr>

    <tr>
      <td>
        Capas de transporte
      </td>

      <td>
        `TSocket`

        , 

        `TFramedTransport`

        , 

        `TZlibTransport`
      </td>
    </tr>
  </tbody>
</table>

### Requisitos de red [#network-requirements]

Todas las implementaciones de eBPF requieren acceso a estos extremos:

<table>
  <thead>
    <tr>
      <th>
        Tipo de requisito
      </th>

      <th>
        Detalles
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Lista de URL permitidas
      </td>

      <td>
        * `otlp.nr-data.net:443`
        * `otlp.eu01.nr-data.net:443` (Región de la UE)
        * `otlp.jp.nr-data.net:443` (región de JP)
        * `https://downloads.newrelic.com`
      </td>
    </tr>

    <tr>
      <td>
        Lista de puertos permitidos
      </td>

      <td>
        * Para la comunicación externa habilite el puerto `443`
      </td>
    </tr>

    <tr>
      <td>
        Requisitos de la ruta de montaje
      </td>

      <td>
        * Sistema de archivos raíz (`/`)
        * Sistema de archivos del sistema (`/sys`)
      </td>
    </tr>
  </tbody>
</table>

<Callout variant="tip">
  El monitoreo de eBPF es independiente del lenguaje y proporciona métricas de latencia y rendimiento para cualquier aplicación. Sin embargo, [la vinculación de segmentos de transacciones](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm/#segments) depende del lenguaje y actualmente no es compatible con PHP y Node.js.
</Callout>

Si su entorno enruta el tráfico saliente a través de un proxy HTTP corporativo, consulte [Configurar un proxy para el <DNT>eBPF agent</DNT>](/docs/ebpf/proxy-configuration) para obtener instrucciones de configuración.

{/* ## Querying eBPF data [#querying-data]

Once the eBPF agent is deployed, telemetry data flows to New Relic and can be queried using NRQL. Here are common query patterns:

### Query spans

Including full-body requests and trace data:

```sql
FROM Span
SELECT * WHERE
deployment.name = '<deployment-name>' LIMIT MAX
```

### Query metrics

Including TCP stats and performance metrics:

```sql
FROM Metric
SELECT * WHERE
deployment.name = '<deployment-name>' LIMIT MAX
```

<Callout variant="tip">
  Replace `<deployment-name>` with the actual deployment name you configured during installation.
</Callout>

### Entity naming

The eBPF agent automatically generates entity names differently depending on the environment:

* **In hosts or Docker**: Names are a combination of the process name, its directory or container ID, and the listening port. For example, `ruby:/home/ubuntu/app:[5678]` or `java:f4aead533895:[8080]`.

* **In Kubernetes**: Names are derived from the service name, for example, `mysql-database-service`.

## Data access in New Relic [#data-access]

eBPF data is accessible through multiple New Relic interfaces:

1. **APM & Services UI**: Go to **[one.newrelic.com](https://one.newrelic.com) > APM & Services** and filter by `instrumentation.name = nr_ebpf`

2. **NRQL queries**: Use the query examples above to explore your data

3. **OpenTelemetry UI**: eBPF data integrates with New Relic's OpenTelemetry experience

For more information on accessing your data, see [OpenTelemetry APM UI](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-ui). */}

## Documentación relacionada [#related-docs]

<DocTiles>
  <DocTile title="Instalación de eBPF Kubernetes" path="/docs/ebpf/k8s-installation/">
    Aprenda a instalar el agente eBPF en Kubernetes.
  </DocTile>

  <DocTile title="Instalación de eBPF Linux" path="/docs/ebpf/linux-installation/">
    Aprenda a instalar el agente eBPF en hosts Linux.
  </DocTile>

  <DocTile title="resolución de problemas eBPF" path="/docs/ebpf/troubleshooting/no-ui-data/">
    Aprenda a solucionar problemas del agente eBPF.
  </DocTile>
</DocTiles>