...
3. **Verifique la configuración de filtrado de datos**: verifique que su entidad no esté siendo excluida por parámetros de configuración como `allDataFilters.dropServiceNameRegex` o `allDataFilters.dropNamespaces`.

4. **Reiniciar aplicaciones con conexiones gRPC/HTTP/2 persistentes**: las aplicaciones con conexiones gRPC/HTTP/2 persistentes deben reiniciarse después de que se inicie el agente eBPF para que el agente pueda generar entidades y monitorear el tráfico correctamente. Si su aplicación gRPC o HTTP/2 se estaba ejecutando antes de que el agente se iniciara, reinicie la aplicación para que el agente pueda interceptar las conexiones recién establecidas.
