---
title: Inteligencia de costos en la nube - Amazon Web Services
tags:
...
Configure su entorno de AWS para Cloud Cost Intelligence configurando la facturación consolidada, los informes de costos y uso, los roles y políticas de IAM y las regiones de AWS necesarias.

## Antes de que empieces

### Cuenta de AWS con las licencias adecuadas [#aws-account]

Asegúrese de que la [facturación consolidada esté configurada para AWS Organizations](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/consolidated-billing.html).
...
<Steps>
  <Step>
    ...
    2. Navegue al **S3 service** y haga clic en **Create bucket**.

    3. Ingrese un nombre de bucket único (por ejemplo, `<your-company-name>-billing`).

    4. Configurar los ajustes del bucket:
    ...
    5. **Etiquetas (Opcional)**: Agregue cualquier etiqueta si es necesario para la gestión de recursos de su organización y haga clic en **Create bucket**.
  ...
  <Step>
    ...
    2. Navegue a **Billing Dashboard** &gt; **Cost &amp; Usage Analysis** &gt; **Data Exports** y a continuación, haga clic en **Create**.

    3. Configurar los ajustes del reporte:
    ...
    4. Haz clic en **Next** y luego en **Create report**.
  ...
  <Step>
    ...
    Cree una política de IAM que otorgue permisos de CCI para acceder a sus datos CUR:

    1. Inicie sesión en **la consola de administración de AWS (AWS Management Console)**.
    2. Navegue a **IAM service** &gt; **Policies** y haga clic en **Create policy**.
    3. Cambie al **editor JSON (JSON editor)** y pegue la siguiente política:
    ...
    4. Haga clic en **Next**.
    5. Nombre la política `CCI-Access-Policy` (o su nombre preferido), agregue una descripción y haga clic en **Create policy**.
  </Step>
  ...
  <Step>
    ...
    1. Inicie sesión en **la consola de administración de AWS (AWS Management Console)**.
    2. Navegue a **IAM service** &gt; **Roles** y haga clic en **Create role**.
    3. Selecciona **Custom Trust Policy** en la sección de tipo de entidad de confianza y pega la siguiente definición de rol:
       ```json
       {
       "Version": "2012-10-17",
       "Statement": [
    ...
    4. Haga clic en **Next**.
    5. Adjunta la política creada en [Create IAM policy for CCI access](#create-iam-policy) y haz clic en **Next**.
    6. Nombra el rol `CCI-Access-Role` (o tu nombre preferido), agrega una descripción (opcional) y haz clic en **Create role**.
    7. Tenga en cuenta el ARN del rol de la página de resumen del rol.
  ...
  <Step>
    ### Acceda a la integración y seleccione la cuenta de New Relic [#access-integration]

    1. Vaya a **Cloud Cost Intelligence** en el panel izquierdo y haga clic en **Add the integration**.
    2. Seleccione su cuenta de New Relic y haga clic en **Continue**.
  </Step>
  ...
  <Step>
    ...
    * **Connection name** - Nombre predeterminado proporcionado. Edite para un nombre personalizado.
    * **Cross-account IAM role ARN** - ARN del rol de IAM para que Cloud Cost Intelligence acceda a su bucket de S3 (formato: `arn:aws:iam::<account-id>:role/<role-name>`).
    * **External ID**: copie el ID externo y agréguelo a su política de confianza de rol de IAM en AWS.
    * **Data export name** - Nombre del archivo de exportación de datos. Para el informe `cci-ingestion-test`, la ruta es `cost-and-usage-reports/cci-ingestion-test/data`.
    * **S3 bucket name** - Nombre exacto del bucket. Utilice la región `us-east-2` para un rendimiento óptimo.
  </Step>
  ...
  <Step>
    ...
    <Callout variant="important">
      Establece `sts:ExternalId` en el ID externo de [Proporciona detalles de conexión](#provide-connection-details). Activa STS para las regiones donde no está activo de forma predeterminada (por ejemplo, `ap-east-1`, `eu-south-1`).
    </Callout>
  </Step>
  ...
  <Step>
    ...
    Haz clic en **Test connection** para verificar la conexión a tu entorno AWS.

    <Callout variant="important">
      El procesamiento inicial de datos puede tardar unas horas después de la configuración.
    </Callout>
  </Step>
</Steps>
