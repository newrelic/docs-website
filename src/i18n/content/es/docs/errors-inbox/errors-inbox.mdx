...
Puede establecer uno de tres estados y filtrar su bandeja de entrada por estado.

* `Unresolved`: Este es el estado predeterminado de los grupos de errores.
* `Resolved`: configurar un error como resuelto lo ocultará de la vista predeterminada de la bandeja de entrada a menos que se actualicen los filtros para incluir errores resueltos. Si se produce un evento que coincide con la huella digital del grupo de errores después de marcar un grupo de errores como resuelto, se restablecerá automáticamente el estado a `Unresolved`. Esto puede resultar útil para identificar regresiones.
* `Ignored`: Esto ocultará el grupo de errores de la vista de la bandeja de entrada a menos que se actualicen los filtros para incluir el error omitido o hasta que deje de ignorar el grupo de errores.
