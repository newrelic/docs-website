...
## Eventos SecurityPolicyViolation (infracciones de CSP) [#policy-violations]

New Relic captura el evento `securitypolicyviolation` emitido por los navegadores y reporta cualquier ocurrencia de este tipo. Sin embargo, esto comienza en la versión 1.313.0 del agente y aún no está integrado en la UI, como la vista de Errors Inbox. Por lo tanto, estos datos solo están disponibles a través de NRQL en este momento. Consulta el [evento del diccionario de atributos](/attribute-dictionary/?event=SecurityPolicyViolation) para ver una lista de los atributos reportados.
