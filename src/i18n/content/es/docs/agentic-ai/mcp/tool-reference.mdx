...
<Callout variant="important">
  **Seguridad y acceso:** Cuando su agente de IA invoca cualquiera de las herramientas enumeradas a continuación (por ejemplo, `get_entity`, `analyze_deployment_impact`, `execute_nrql_query`), la acción que realiza está estrictamente regida por las licencias otorgadas a la clave de API de usuario de New Relic o al perfil OAuth que se configuró durante la configuración. La herramienta solo puede acceder a los datos, entidades y cuentas a las que su cuenta de usuario correspondiente tenga acceso a través del Control de Acceso Basado en Roles (RBAC) de New Relic. Si su clave carece de licencias para una acción específica (por ejemplo, ver alertas o ejecutar NRQL), la llamada a la herramienta fallará con un error de licencias.
</Callout>
