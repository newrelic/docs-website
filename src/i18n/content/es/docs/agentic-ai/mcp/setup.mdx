...
## Configuración de la CLI de Codex [#codex-cli]

### Método de clave API [#api-key]

1. Asegúrese de tener la [herramienta CLI de Codex instalada](https://developers.openai.com/codex/cli/) en su máquina.

2. Configure el servidor MCP editando el archivo de configuración ubicado en `~/.codex/config.toml` y agregue la siguiente configuración:
...
3. Autenticar y conectar:
   ...
   * Abra su terminal y ejecute `codex`.
   * Escriba el comando `/mcp` y presione Enter.
   * Verá el servidor de New Relic listado con una URL.
...
3. Autenticar y conectar:
   ...
   * El flujo de Oauth lo llevará a la página de autenticación donde deberá proporcionar sus credenciales para autenticarse.
   * Una vez autenticado, el navegador confirmará el éxito.
   * Abra su terminal y ejecute `codex`.
   * Escriba el comando `/mcp` y presione Enter.
   * Verá el servidor New Relic listado con una URL. Abra esa URL en su navegador.
