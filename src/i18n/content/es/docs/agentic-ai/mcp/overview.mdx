...
<Callout title="Membresía y roles del grupo" variant="important">
  Para acceder al servidor MCP es necesario ser afiliado a un grupo al que se le asignó uno de los siguientes [roles organizacionales](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#role-scopes) específicos: `Organization Read Only`, `Organization Manager` o `Organization Product Admin`. Cerciorar de que esta función esté activa antes de intentar acceder. Si su grupo actualmente no tiene uno de estos roles, comunicar con el administrador de su organización (un usuario con el tipo de usuario de plataforma **Core** o **Full** que tenga el rol `Authentication Domain Manager` o `Tenant Manager`) para que se lo otorgue.
</Callout>
