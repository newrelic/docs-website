...
<DoNotTranslate>**Recommendation:**</DoNotTranslate> Coloque todas las etiquetas `<meta>` inmediatamente después de la etiqueta `<head>` inicial. Algunas metaetiquetas tienen requisitos sobre qué tan cerca deben estar del comienzo de un documento HTML. [La codificación de caracteres](http://www.w3.org/TR/html-markup/syntax.html#character-encoding) debe declararse dentro de los primeros 512 bytes del documento HTML. Si el encabezado New Relic se coloca antes de la codificación de caracteres, es posible que la codificación de caracteres esté fuera de ese límite de 512 bytes. Colocar la metaetiqueta de codificación de caracteres inmediatamente después de la etiqueta `<head>` garantizará que el encabezado New Relic se coloque después de la codificación de caracteres.
