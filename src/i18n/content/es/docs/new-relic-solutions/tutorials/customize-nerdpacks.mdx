...
<Steps>
  <Step>
    ...
    `nr1.json` es el [archivo de metadatos de](/explore-docs/custom-viz/configuration-options/) la visualización de la **Circular progress bar** circular . Emplee este archivo para agregar una opción `colorScale` configurable, que corresponde al campo `colorScale` en `VictoryPie`.
  </Step>

  <Step>
    Agregue [`collection`](/explore-docs/custom-viz/configuration-options/#collection) de [`string`](/explore-docs/custom-viz/configuration-options/#string) campos para personalizar los colores de su gráfico:
  ...
  <Step>
    ...
    Esto revertirá las esquinas de la barra a los 90 grados predeterminados en lugar de redondearlas. Mientras su servidor local se está ejecutando, reconoce automáticamente los cambios en _index.js_. Entonces, vea su visualización en su browser para ver la actualización.
