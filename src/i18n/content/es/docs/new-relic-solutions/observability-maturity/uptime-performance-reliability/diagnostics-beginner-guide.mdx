...
| Punto de datos | New Relic | Fuentes de problemas comunes | |---|---|---| | Salida | APM, infraestructura, logs, NPM | aplicación, fuentes de datos, cambio de configuración de hardware, infraestructura, redes internas, proveedor externo (AWS, GCP) | | Entrada | Sintético, logs | Enrutamiento externo (CDN, gateways, etc.), enrutamiento interno, cosas en el Internet (ISP, etc.) | | Cliente | Browser, mobile | código de Browser o móvil |
