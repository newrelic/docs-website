...
<CollapserGroup>
  <Collapser id="linux-install-run" title="Linux">
    ...
    2. Ejecute lo siguiente:
       ...
       ```
       Recomendación: delimite su resolución de problemas con un [conjunto de tareas](#suites) (`-suites`) y cargue los archivos relevantes a su cuenta de New Relic con el [indicador de archivo adjunto](#automatic) (`-attach`).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser-run" title="Monitoreo del navegador">
  ...
  <Collapser id="docker" title="Contenedor docker">
...
<CollapserGroup>
  <Collapser id="list-available-scripts" title="Listar script disponible">
  ...
  <Collapser id="view-script" title="Ver un script">
  ...
  <Collapser id="run-script" title="Ejecutar un script">
  ...
  <Collapser id="run-script-with-args" title="Ejecutar un script con argumentos">
  ...
  <Collapser id="run-script-and-suites" title="Ejecute un script junto con suites">
...
<CollapserGroup>
  <Collapser id="linux-attach-examples" title="Linux y macOS">
    * Para sistemas de 64 bits:
...
De forma predeterminada, la CLI de Diagnostics carga los resultados en las cuentas de New Relic alojadas en el centro de datos de EE. UU. Si su cuenta está alojada en el centro de datos de la UE o Japón, agregue `-region eu` o `-region jp` al comando.
