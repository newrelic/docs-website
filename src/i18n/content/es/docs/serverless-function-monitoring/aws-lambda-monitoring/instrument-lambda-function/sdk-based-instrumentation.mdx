...
<CollapserGroup>
  ...
  <Collapser id="java" title="Java">
    <Callout variant="tip">
      This solution does not use the APM Java Agent. If you wish to use the APM Java Agent in AWS Lambda, please refer to our [Java Agent Serverless Solution Doc](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda). The APM agent based solution should not be run with the SDK based solution.
    </Callout>
