...
<CollapserGroup>
  ...
  >
    ...
    1. Establezca [variables de entorno de extensión](https://github.com/newrelic/newrelic-lambda-extension/blob/main/config/config.go) en su función:
       ...
       * `NEW_RELIC_LAMBDA_EXTENSION_ENABLED`: verdadero (predeterminado)
       * `NEW_RELIC_EXTENSION_SEND_FUNCTION_LOGS`: falso (predeterminado)

    2. Establezca las variables de entorno de su función `newrelic-log-ingestion` :

       * `INFRA_ENABLED`: verdadero
       * `LOGGING_ENABLED`: verdadero (si envía el log)
  ...
  >
    ...
    1. Establezca [variables de entorno de extensión](https://github.com/newrelic/newrelic-lambda-extension/blob/main/config/config.go) en su función:
       ...
       * `NEW_RELIC_LAMBDA_EXTENSION_ENABLED`: verdadero (predeterminado)
       * `NEW_RELIC_EXTENSION_SEND_FUNCTION_LOGS`: verdadero 2 Establezca las variables de entorno de su función `newrelic-log-ingestion` :
       * `INFRA_ENABLED`: verdadero
       * `LOGGING_ENABLED`: falso (deshabilite el reenvío de logpara evitar duplicados o configure el patrón de filtro de subscripción de CloudWatch para que coincida solo con `NR_LAMBDA_MONITORING` líneas)
