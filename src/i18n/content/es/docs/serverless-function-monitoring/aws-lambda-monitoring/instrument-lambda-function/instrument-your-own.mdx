...
<Callout variant="tip">
  **Instancia administrada de Lambda (LMI)**: la instrumentación en capas es compatible con las funciones LMI. Agregue la capa Lambda de New Relic como lo haría para cualquier función Lambda —las funciones LMI se ejecutan automáticamente en modo APM sin requerir `NEW_RELIC_APM_LAMBDA_MODE = true`.
</Callout>
...
<Callout variant="tip">
  Recomendamos integrar su cuenta de AWS utilizando la [integración de AWS](/docs/infrastructure/amazon-integrations/get-started/introduction-aws-integrations/) para descubrir y monitorear automáticamente sus funciones Lambda. Esto le permite aprovechar todo el poder de New Relic APM para sus funciones serverless.
</Callout>
...
Dependiendo de sus necesidades, puede elegir omitir la extensión y solo ver la telemetría en CloudWatch, omitir CloudWatch o usar CloudWatch como respaldo. La sección [CloudWatch](#CloudWatch) al final de este documento lo guiará a través de cada opción.
...
<CollapserGroup>
  ...
  <Collapser id="cloudformation" title="Plantillas de CloudFormation/SAM">
    ...
    1. Instale la [capa Lambda de New Relic](https://layers.newrelic-external.com/). A continuación se muestran ejemplos para las funciones Lambda del agente de Node.js y del agente de Java.
       ...
       **Ejemplo de Java legacy (basado en Open Tracing):**

       <Callout variant="important">
         Recomendado para clientes que usan el monitoreo sin servidor para Java antes del 10 de abril de 2026 o si usan Java 8 u 11.
       </Callout>
       ...
       **Nuevo ejemplo del agente de Java (capa ligera, recomendado):**

       <Callout variant="important">
         Esta es una solución separada de la solución basada en Open Tracing y solo está disponible desde el 10 de abril de 2026. La nueva capa del agente de Java requiere Java 17 o posterior. Los entornos de ejecución de Java 11 y versiones anteriores no son compatibles.
       </Callout>
       ...
       Hay dos variantes de capa disponibles para Java. Utilice la capa slim a menos que necesite instrumentación automática completa:

       * **Capa Slim (recomendada)**: solo instrumenta `RequestHandler` / `RequestStreamHandler`. Menor sobrecarga de inicio en frío.
       * **Capa completa**: todos los módulos de instrumentación automática habilitados. Mayor sobrecarga de arranque en frío.

       Consulte [Monitoreo del rendimiento de AWS Lambda Serverless para Java](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda) para obtener detalles sobre las dos variantes.
  ...
  <Collapser id="console" title="Instrumentación manual AWS">
    Si bien es más propenso a errores y requiere más mano de obra que los enfoques anteriores, puede modificar manualmente la configuración de una función Lambda para usar New Relic desde la consola AWS Lambda, para Node.js, Python, Ruby y Java (que incluye la solución legacy de Java basada en Open Tracing y la nueva solución alternativa basada en el agente APM).
    ...
    5. Vaya a [la lista de capas de New Relic](https://layers.newrelic-external.com/) y use la lista desplegable para seleccionar la región de AWS donde está alojada su función Lambda. Desde allí, ubique el ARN que coincida con la versión y arquitectura de Ruby (o de cualquier otro lenguaje) de su función Lambda. Debería haber dos opciones: X86 y ARM64. Utilice el botón **Copy to clipboard** o copie manualmente la cadena ARN.

       * Los clientes que usan Java encontrarán capas de Java de la antigua convención y capas del agente de Java de la nueva convención, que representan **2 soluciones distintas para el monitoreo de lambda de Java**.

         * **La capa Java legacy** se basa en Open Tracing, que se recomienda para los clientes que utilizan nuestro monitoreo sin servidor antes del 10 de abril de 2026 y para los clientes en Java 11 y/o 8. Todos los ARN contienen la subcadena `NewRelicJava`. Un ARN de ejemplo de dicha capa es:

           ```
           arn:aws:lambda:us-east-1:451483290750:layer:NewRelicJava17:21
           ```

         * **Nueva capa del agente de Java** se basa en el agente de Java de APM y se recomienda para nuevas aplicaciones de Java. Hay una capa completa y una capa ligera que reduce la visibilidad pero mejora el rendimiento de arranque en frío. **Este no es un reemplazo directo para la solución Open Tracing legacy**. Consulte nuestro [documento del agente APM](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda) para obtener más detalles. Todos los ARN contienen la subcadena `NewRelicAgentJava`. Un ejemplo de ARN de dicha capa es:
    ...
    * Java (solución legacy basada en Open Tracing):
    ...
    * Para la nueva solución del agente de Java no tiene que configurar el manejador.
