...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Especifica el envoltorio del manejador utilizado para encontrar el manejador real de tu función. Para los tiempos de ejecución de Go, esta variable no es requerida.
      </td>
    </tr>
  ...
  <TabsPages>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              Establece el nombre de la aplicación utilizado como nombre de la entidad de APM en la UI de New Relic cuando `NEW_RELIC_APM_LAMBDA_MODE` es `true`. Consulte [NEW\_RELIC\_APP\_NAME para funciones Lambda en modo APM](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) para obtener detalles sobre la agrupación de entidades, los informes de múltiples regiones, el comportamiento entre entornos de ejecución y la nomenclatura compartida con servicios que no son de Lambda.
            </td>
          </tr>
    ...
    <TabsPageItem id="java">
      <Callout variant="important">
        La nueva capa del agente de Java requiere Java 17 o posterior. Esta es una solución independiente de la [solución de Java](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/instrument-your-own/#console) legacy. Si está utilizando el monitoreo sin servidor para su aplicación antes del 10 de abril de 2026 o está en Java 8 u 11, por favor continúe usando la solución legacy que se basa en Open Tracing.
      </Callout>
      ...
      <table>
        ...
        <tbody>
            ...
            <td>
              **Requerido**
              ...
              `/opt/newrelic-java-handler`
               ...
               al usar la capa del agente de Java de New Relic. Esto apunta AWS Lambda al script contenedor del agente empaquetado en la capa.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Habilita el modo serverless. Detectado automáticamente cuando 

              `AWS_LAMBDA_FUNCTION_NAME`

               está presente. Preconfigurado para 
          ...
          <tr>
            ...
            <td>
              Por defecto, configura el archivo de log del agente de Java para que esté en la ruta absoluta 
          ...
          <tr>
            ...
            <td>
              Habilite el logging de depuración estableciendo esto en 
      ...
      Puede encontrar más variables de entorno en nuestra [documentación de configuración del agente de Java](/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file/) y en el [monitoreo sin servidor de Java](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda).
    </TabsPageItem>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>
              Cuando `NEW_RELIC_APM_LAMBDA_MODE` es `true`, la extensión usa este valor como el nombre de la entidad de APM durante la llamada de conexión de inicio en frío. Consulta [NEW\_RELIC\_APP\_NAME para funciones Lambda en modo APM](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/app-name-apm-lambda/) para obtener detalles sobre la agrupación de entidades, los informes multirregión, el comportamiento entre tiempos de ejecución y la nomenclatura compartida con servicios que no son de Lambda.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Envíe logs de plataforma configurando esto en 
          ...
          <tr>
            ...
            <td>
              Determina qué logs enviar. Establezca en 
              ...
              `platform`
               ...
               logs, o use una lista separada por comas (por ejemplo: 

              `platform,extension`

              ). Esta variable tiene prioridad sobre las variables individuales de tipo de log.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Establecer los logs extremos. Seleccione la URL según su región:

              * Región de EE. UU. (predeterminado):<br />`https://log-api.newrelic.com/log/v1`
              * Región de la UE:<br />`https://log-api.eu.newrelic.com/log/v1`
              * Región de JP:<br />`https://log-api.jp.nr-data.net/log/v1`
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Establezca el extremo de telemetría. Seleccione la URL según su región:

              * Región de EE. UU. (predeterminado):<br />`https://cloud-collector.newrelic.com/aws/lambda/v1`
              * Centro de datos de la UE:<br />`https://cloud-collector.eu01.nr-data.net/aws/lambda/v1`
              * Centro de datos de JP:<br />`https://cloud-collector.jp.nr-data.net/aws/lambda/v1`
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Para ayudar a New Relic a mapear y mostrar correctamente las relaciones entre sus entidades monitoreadas, proporcione el ID de la cuenta de AWS donde residen sus recursos monitoreados, como base de datos y función Lambda.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              Configura un proxy para el tráfico saliente de la extensión y no afecta el tráfico de la función Lambda.

              <br />

              <br />

              * **Admite:** esquemas `http://`, `https://` y `socks5://`
              * **Autenticación:** soporta `user:pass` (enmascarado en los logs)
              * **Respaldo:** usa `HTTPS_PROXY` o `HTTP_PROXY` si está sin establecer

              <Callout variant="tip" title="nota">
                El tráfico de localhost nunca pasa por un proxy.
              </Callout>
            </td>
          </tr>
        </tbody>
