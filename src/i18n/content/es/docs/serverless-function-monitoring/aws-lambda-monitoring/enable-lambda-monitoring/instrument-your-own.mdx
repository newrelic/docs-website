...
<CollapserGroup>
  ...
  >
    Framework Serverless es una herramienta popular de desarrollo e implementación para aplicaciones serverless. Está escrito en Node.js y, para AWS, actúa principalmente como una abstracción de nivel superior además de las plantillas de CloudFormation. Funciona bien para funciones de Node, Python y Java.
...
Como [se mencionó anteriormente,](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/architecture#cloudwatch) solíamos recomendar enviar su telemetría a través del registro de CloudWatch. Esta ruta aún puede funcionar, aunque está en desuso.
...
2. Cree un filtro de suscripción de registros de CloudWatch para invocar la función `newrelic-log-ingestion` con el registro de su función.
...
<CollapserGroup>
  <Collapser
    ...
    title="Proceso manual: transmitir el registro de CloudWatch a New Relic Lambda"
  >
    ...
    5. Establezca el <DoNotTranslate>**Subscription filter pattern to**</DoNotTranslate> `?REPORT ?NR_LAMBDA_MONITORING ?"Task timed out" ?RequestId`. Alternativamente, si está utilizando la variable de entorno `LOGGING_ENABLED` [, transmita todo su registro](#stream-all-logs) a nuestro registro, deje este campo en blanco.
...
Si bien es más propenso a errores y requiere más mano de obra que los enfoques anteriores, es posible modificar manualmente la configuración de una función Lambda para usar New Relic desde la consola AWS Lambda, para Node.js, Python y Java.
...
2. Para Python, Node.js y Java, también incluimos:
