...
<CollapserGroup>
  ...
  >
    framework sin servidor es una herramienta popular de desarrollo e implementación para aplicaciones sin servidor. Está escrito en Node.js y, para AWS, actúa principalmente como una abstracción de nivel superior además de las plantillas de CloudFormation. Funciona bien para funciones de Node, Python, Ruby, Java y .NET.
...
<CollapserGroup>
  ...
  >
    ...
    5. Establezca el <DNT>**Subscription filter pattern to**</DNT> `?REPORT ?NR_LAMBDA_MONITORING ?"Task timed out" ?RequestId`. Alternativamente, si está utilizando la variable de entorno `LOGGING_ENABLED` [, transmita todo su log](#stream-all-logs) a nuestro log, deje este campo en blanco.
