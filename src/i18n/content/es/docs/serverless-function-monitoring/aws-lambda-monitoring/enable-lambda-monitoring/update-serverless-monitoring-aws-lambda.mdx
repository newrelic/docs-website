...
* [Actualización a través del repositorio de aplicaciones AWS Serverless](#update-sar): utilice esto si lo habilitó mediante el [procedimiento manual](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/enable-new-relic-monitoring-aws-lambda#manual-nr-lambda).
...
3. Si no tiene nuestro registro habilitado, también deberá actualizar sus filtros de suscripción log de Amazon CloudWatch con el siguiente comando:
...
Si [instaló manualmente la función de ingesta desde el repositorio de aplicaciones de AWS Serverless](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/enable-new-relic-monitoring-aws-lambda#manual-nr-lambda) (y no utilizó la [CLI](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/enable-new-relic-monitoring-aws-lambda#cli)), actualice mediante este procedimiento:
...
## Habilitando la administración de registros
...
3. Luego haga cualquiera de las siguientes cosas:
   ...
   * O puede enviar el registro de funciones a New Relic directamente, sin pasar por CloudWatch y `newrelic-log-ingestion` Lambda. Para hacer esto, establezca la variable de entorno `NEW_RELIC_EXTENSION_SEND_FUNCTION_LOGS=true` en la configuración de su función Lambda.
     ...
     Si la suscripción log está presente mientras la extensión envía el registro, el registro se enviará dos veces, lo que generará log duplicados en New Relic.
     ...
     <Callout variant="caution">
       Las tarifas de ingesta de registros de CloudWatch pueden ser considerables, pero este paso debe tomarse con precaución. Asegúrese de que la integración de ingesta de New Relic Logs funcione bien y satisfaga sus necesidades antes de deshabilitar el registro de CloudWatch.
     </Callout>