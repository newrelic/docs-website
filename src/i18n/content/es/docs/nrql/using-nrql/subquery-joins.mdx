...
Este ejemplo une datos de infraestructura con datos de transacciones:
...
La imagen de arriba contiene dos conjuntos de datos: el porcentaje promedio de CPU de los contenedores de infraestructura (`ProcessSample`) y la duración promedio de la transacción de aplicaciones por contenedor. A menudo, los datos de diferentes fuentes están correlacionados. En este caso, puede determinar si el mayor uso de CPU de un contenedor está provocando una transacción más lenta utilizando la siguiente combinación de subconsulta:
