...
<CollapserGroup>
  ...
  >
    Esta consulta asigna valores `shopId` a un valor `storeCategory` más amplio mediante el uso de una cláusula `JOIN` en la tabla de búsquedas de datos. Esto le permite realizar agregaciones en `StoreEvent` registros basados en estas categorías más amplias y extraer nueva información valiosa de sus datos.
