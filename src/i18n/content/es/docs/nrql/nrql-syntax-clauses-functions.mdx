...
<CollapserGroup>
  <Collapser
    ...
    title="¿Nuevo en New Relic?"
  >
    Si eres nuevo en New Relic y quieres ver lo fácil que puede ser usar NRQL, puedes comenzar con la plataforma de forma gratuita y seguir el tutorial. Todo lo que tienes que hacer es:

    1. Si aún no lo ha hecho, cree su cuenta New Relic gratuita a continuación para comenzar a monitorear sus datos hoy.

    <InlineSignup/>

    2. Utilice nuestro [agente e integración](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations/) para recopilar datos automáticamente desde marcos y herramientas comunes.
    3. Visite la [guía de lanzamiento rápida](/docs/new-relic-solutions/get-started/quick-launch-guide) para obtener una ruta recomendada sobre cómo configurar New Relic. Puede comenzar a ingerir datos para consultar y estar listo para usar este o cualquiera de nuestros tutoriales.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="state-select"
    title={<>Requerido: <InlineCode>SELECT</InlineCode> declaración</>}
  >
    ```sql
    SELECT attribute ...
    ```
    ...
    * Etiquete los resultados devueltos en cada argumento con [la cláusula `AS` ](#sel-as).
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Tiempo medio de respuesta desde la semana pasada"
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-from"
    title={<>Requerido: cláusula <InlineCode>FROM</InlineCode></>}
  >
    ```sql
    SELECT ...
      FROM data type
    ...
    <CollapserGroup>
      <Collapser
        id="one-event"
        title="Consultar un tipo de datos"
      >
        Esta consulta devuelve el recuento de todas [las transaccionesAPM ](/docs/insights/new-relic-insights/decorating-events/insights-attributes#transaction-defaults)durante los últimos siete días:
      ...
      <Collapser
        id="multiple-events"
        title="Consultar múltiples tipos de datos"
      >
        Esta consulta devuelve el recuento de todas [las transaccionesAPM ](/docs/insights/new-relic-insights/decorating-events/insights-attributes#transaction-defaults)y [eventosbrowser ](/docs/insights/new-relic-insights/decorating-events/browser-default-attributes-insights#browser-attributes-table)durante los últimos tres días:
      ...
      <Collapser
        id="from-lookups"
        title="Consultar datos desde una tabla de búsquedas"
      >
...
<CollapserGroup>
  <Collapser
    className="freq-link"
    id="sel-as"
    title={<><InlineCode>AS</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
      AS 'label'
    ...
    Utilice la cláusula `AS` para etiquetar un atributo, agregador, paso en un embudos o el resultado de una función matemática con una cadena delimitada por comillas simples. La etiqueta se utiliza en el gráfico resultante. Tenga en cuenta que las etiquetas de la cláusula `AS` en los gráficos de series temporales no se mostrarán si se utiliza una cláusula `FACET` .

    <CollapserGroup>
      <Collapser
        id="math-as"
        title={<>Consulta usando funciones matemáticas y <InlineCode>AS</InlineCode></>}
      >
      ...
      <Collapser
        id="funnel-as"
        title={<>Consulta utilizando embudos y <InlineCode>AS</InlineCode></>}
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-compare"
    title={<><InlineCode>COMPARE WITH</InlineCode> cláusula</>}
  >
    ```sql
    ...
    * Sin `TIMESERIES`, `COMPARE WITH` genera un cartel con el valor actual y el cambio porcentual del valor `COMPARE WITH` .
    ...
    <DoNotTranslate>**Example**</DoNotTranslate>: esta consulta devuelve datos como un gráfico de líneas que muestra el percentil 95 de la semana pasada en comparación con el mismo rango hace una semana. Primero como un valor único, luego como un gráfico de líneas.
  ...
  <Collapser
    className="freq-link"
    id="extrapolate"
    title={<><InlineCode>EXTRAPOLATE</InlineCode> cláusula</>}
  >
    ...
    * Evento personalizado reportado vía <InlinePopover type="apm"/>API del agente
      ...
      Cuando se utiliza `EXTRAPOLATE` en una consulta NRQL que admite su uso, la relación entre <DoNotTranslate>**reported events**</DoNotTranslate> y <DoNotTranslate>**total events**</DoNotTranslate> se utiliza para extrapolar una aproximación cercana del total de datos no muestreados. Cuando se utiliza en una consulta NRQL que no admite su uso o que no ha utilizado datos de muestra, no tiene ningún efecto.
    ...
    * `stddev`
      ...
      <CollapserGroup>
        <Collapser
          ...
          title="Ejemplo de extrapolación del rendimiento"
        >
        ...
        <Collapser
          ...
          title="Ejemplo de extrapolación del rendimiento como serie temporal"
        >
  ...
  <Collapser
    className="freq-link"
    id="sel-facet"
    title={<><InlineCode>FACET</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
      FACET attribute
    ...
    <CollapserGroup>
      <Collapser
        id="faceted-query"
        title={<>Consulta facetada usando <InlineCode>count()</InlineCode></>}
      >
      ...
      <Collapser
        id="uniquecount"
        title={<>Consulta facetada usando <InlineCode>uniqueCount()</InlineCode></>}
      >
      ...
      <Collapser
        id="cohort-analysis"
        title="Agrupar resultados a lo largo del tiempo"
      >
  ...
  <Collapser
    className="freq-link"
    id="facet-as"
    title={<><InlineCode>FACET ... AS</InlineCode> cláusula</>}
  >
  ...
  <Collapser
    className="freq-link"
    id="sel-facet-cases"
    title={<><InlineCode>FACET CASES</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
      FACET CASES (
    ...
    <CollapserGroup>
      <Collapser
        ...
        title={<>Uso básico con <InlineCode>WHERE</InlineCode></>}
      >
        ```sql
      ...
      <Collapser
        ...
        title="Grupo basado en múltiples atributos."
      >
      ...
      <Collapser
        ...
        title={<>Etiquetar grupos con <InlineCode>AS</InlineCode></>}
      >
      ...
      <Collapser
        ...
        title={<>Datos de faceta que no coinciden con <InlineCode>OR</InlineCode></>}
      >
  ...
  >
    ...
    <Callout variant="tip">
      Debido a que las operaciones se realizan antes de que se aplique la cláusula `LIMIT` , `FACET ... ORDER BY` no afecta el tipo de resultados finales de la consulta, lo que será particularmente notable en los resultados de la consulta que no es de serie temporal.
    </Callout>
  ...
  <Collapser
    className="freq-link"
    id="sel-join"
    title={<><InlineCode>JOIN</InlineCode> cláusula</>}
  >
    ...
    * La cardinalidad de la combinación está limitada a 1:100, lo que significa que una única clave de combinación no puede asignarse a más de cien filas en el resultado de la subconsulta.
    ...
    Para ver en profundidad la cláusula JOIN, consulte el tutorial [sobre uniones de subconsultas NRQL](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/subquery-joins) .

    <CollapserGroup>
      <Collapser
        title={<><InlineCode>INNER JOIN</InlineCode> con una subconsulta facetada</>}
      ...
      >
        ...
        <figcaption>
          Ejemplo de <InlineCode>INNER JOIN</InlineCode> facetado
        </figcaption>
      </Collapser>

      <Collapser title={<><InlineCode>LEFT JOIN</InlineCode> con una subconsulta facetada</>}>
        ...
        <figcaption>
          Ejemplo de <InlineCode>LEFT JOIN</InlineCode> facetado
        </figcaption>
      </Collapser>

      <Collapser title={<><InlineCode>INNER JOIN</InlineCode> con una subconsulta no agregada</>}>
        ...
        <figcaption>
          Ejemplo de <InlineCode>INNER JOIN</InlineCode> no agregado
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
  ...
  <Collapser
    className="freq-link"
    id="sel-limit"
    title={<><InlineCode>LIMIT</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
      LIMIT count
    ...
    <CollapserGroup>
      <Collapser title={<>Consulta usando <InlineCode>LIMIT</InlineCode></>}>
  ...
  <Collapser
    className="freq-link"
    id="sel-offset"
    title={<><InlineCode>OFFSET</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
  ...
  <Collapser
    className="freq-link"
    id="sel-order-by"
    title={<><InlineCode>ORDER BY</InlineCode> cláusula</>}
  >
    La cláusula `ORDER BY` le permite especificar cómo desea ordenar los resultados de su consulta en la consulta que selecciona evento atributo _por fila_.
  ...
  <Collapser
    className="freq-link"
    id="show-event-types"
    title={<><InlineCode>SHOW EVENT TYPES</InlineCode> cláusula</>}
  >
    ```sql
    SHOW EVENT TYPES...
    ```
    ...
    <CollapserGroup>
      <Collapser
        id="avg-resp-time-query"
        title="Tipos de datos en el último día"
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-since"
    title={<><InlineCode>SINCE</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
    ...
    El valor de <DoNotTranslate>**default**</DoNotTranslate> es <DoNotTranslate>**1 hour ago**</DoNotTranslate>.
    ...
    Al utilizar NRQL, puede establecer una timestamp UTC, una hora relativa o una cadena DateTime. Consulte [Especificación de una hora](/docs/query-your-data/nrql-new-relic-query-language/get-started/query-time-range/#spec-time).
  ...
  <Collapser
    className="freq-link"
    id="slide-by"
    title={<><InlineCode>SLIDE BY</InlineCode> cláusula</>}
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title={<>Utilice <InlineCode>SLIDE BY</InlineCode> con intervalo <InlineCode>MAX</InlineCode> o <InlineCode>AUTO</InlineCode></>}
      >
        ...
        Esta consulta establecerá la ventana SLIDE BY en la granularidad de intervalo máxima.
  ...
  <Collapser
    className="freq-link"
    id="sel-timeseries"
    title={<><InlineCode>TIMESERIES</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
    ...
    * `TIMESERIES 30 seconds`
    ...
    <CollapserGroup>
      <Collapser
        id="set-interval"
        title="Utilice un intervalo establecido"
      >
      ...
      <Collapser
        id="timeseries-auto"
        title="Utilice un intervalo establecido automáticamente"
      >
      ...
      <Collapser
        id="timeseries-max"
        title={<>Utilice el intervalo <InlineCode>MAX</InlineCode></>}
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-until"
    title={<><InlineCode>UNTIL</InlineCode> cláusula</>}
  >
    ```sql
    SELECT ...
    ...
    El valor de <DoNotTranslate>**default**</DoNotTranslate> es <DoNotTranslate>**NOW**</DoNotTranslate>. Utilice únicamente `UNTIL` para especificar un punto final distinto del predeterminado.
  ...
  <Collapser
    className="freq-link"
    id="sel-where"
    title={<><InlineCode>WHERE</InlineCode> cláusula</>}
  >
    ...
    * Si especifica más de una condición, separe las condiciones por los operadores `AND` o `OR`.
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            Ejemplo: `animalType IN ('cat', 'dog', 'fish')`
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            El argumento de cadena para el operador `LIKE` acepta el signo de porcentaje (%) como comodín en cualquier parte de la cadena.
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Ejemplo de consulta con tres condiciones."
      >
  ...
  <Collapser
    ...
    title={<><InlineCode>WITH METRIC_FORMAT</InlineCode> cláusula</>}
  >
  ...
  <Collapser
    className="freq-link"
    id="with-as-nrql-var"
    title={<><InlineCode>WITH ... AS</InlineCode> cláusula (variables NRQL)</>}
  >
    ```sql
    FROM ...
    ...
    * La cláusula `WITH ... AS` puede ir antes, entre medio o directamente después de la cláusula `FROM`/`SELECT` .
    ...
    * Los nombres de variables no pueden incluir el símbolo `%` .
    ...
    <CollapserGroup>
      <Collapser
        id="basic-nrql-var"
        title="Uso básico de una variable."
      >
        ```sql
        FROM Transaction
      ...
      <Collapser
        ...
        title="Usando múltiples variables"
      >
        ```sql
        FROM Log
      ...
      <Collapser
        ...
        title="Usar variable en otras cláusulas"
      >
  ...
  <Collapser
    className="freq-link"
    id="sel-timezone"
    title={<><InlineCode>WITH TIMEZONE</InlineCode> cláusula</>}
  >
    ```sql
    ...
    * Zona horaria en la cadena de fecha y hora, utilizando la cláusula `WITH TIMEZONE` América/Los Ángeles, que es -0700 durante el horario de verano:
    ...
    Consulte [Establecer rango de tiempo en paneles y gráficos](/docs/insights/new-relic-insights/managing-dashboards-data/set-time-range-insights-dashboards-widgets) para obtener información detallada y ejemplos.
  </Collapser>
</CollapserGroup>
...
* [consulta intervalo de tiempo de datos métricos](/docs/query-data/nrql-new-relic-query-language/nrql-query-tutorials/query-metric-timeslice-data-nrql), que es nuestro tipo de datos métricos original reportado por nuestro APM, monitoreo de móviles y
...
<CollapserGroup>
  ...
  >
    Utilice la función `aggregationendtime()` para devolver la hora de la agregación relevante. Más específicamente, para un agregado determinado, la función `aggregationendtime()` proporciona la marca de timestamp del final del período de tiempo de esa agregación. Por ejemplo, en una consulta de series temporales, para un punto de datos que abarca una hora de datos, la función devolvería la timestamp del final de ese período de hora.
  </Collapser>
  ...
  >
    Utilice la función `apdex` para obtener una [puntuación de Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#score) para una sola transacción o para todas sus transacciones. La puntuación Apdex predeterminada es 0,5 segundos. El [atributo](/docs/insights/new-relic-insights/decorating-events/insights-attributes) puede ser cualquier atributo basado en el tiempo de respuesta, como [`duration`](/docs/insights/insights-data-sources/default-events-attributes/apm-default-event-attributes#txn-duration) o [`backendDuration`](/docs/insights/insights-data-sources/default-events-attributes/browser-default-events-attributes-insights#backend-duration). El argumento `t:` define un umbral [Apdex T](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) en la misma unidad de tiempo que el atributo elegido. Por ejemplo, si el atributo se mide en segundos, `t` será un umbral en segundos.

    La puntuación Apdex devuelta por la función `apdex()` se basa únicamente en el tiempo de ejecución. No tiene en cuenta los errores de APM. Si una transacción incluye un error pero se completa en [Apdex T](/docs/apm/new-relic-apm/getting-started/glossary#apdex_t) o menos, la función `apdex ()` calificará esa transacción como [satisfactoria](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#bullet-satisfied) .

    <CollapserGroup>
      <Collapser
        id="apdex-cust-attributes"
        title="Obtenga Apdex para clientes específicos"
      >
      ...
      <Collapser
        ...
        title="Obtenga Apdex para transacciones específicas"
      >
        Utilice el atributo `name` para devolver una puntuación para una transacción específica o devolver un Apdex general omitiendo `name`. Esta consulta devuelve una puntuación de Apdex para la transacción <DoNotTranslate>**Controller/notes/index**</DoNotTranslate> durante la última hora:
  ...
  >
    Utilice la función `filter()` para limitar los resultados de una de las funciones de agregador en su instrucción SELECT. Puede utilizar `filter()` junto con `FACET` o `TIMESERIES`. El filtro solo es útil cuando se seleccionan varias agregaciones diferentes, como por ejemplo:
  ...
  >
    Utilice la función `histogram()` para generar histograma. Se necesitan tres argumentos:

    * Nombre del atributo

    * Valor máximo del rango de muestra.

    * Número total de depósitos (entre 1 y 500, inclusive)

      <CollapserGroup>
        <Collapser
          id="histogram-response-times"
          title="Histograma de tiempo de respuesta del evento PageView"
        >
          Esta consulta da como resultado un histograma de tiempo de respuesta que varía hasta 10 segundos en 20 períodos.
        ...
        <Collapser
          ...
          title="Cubos de histograma de Prometheus"
        >
        ...
        <Collapser
          ...
          title="New Relic distribución métrica"
        >
        ...
        <Collapser
          ...
          title="Histograma con una cláusula FACET"
        >
  ...
  >
    Utilice la función `median()` para devolver la mediana de un atributo o el percentil 50. Para obtener más información sobre percentil consulta, consulte [percentile()](#func-percentile).
  ...
  >
    ...
    El primer argumento requiere una [función agregadora](#functions) contra el atributo deseado. Utilice exactamente dos argumentos (los argumentos posteriores a los dos primeros se ignorarán). Si el atributo no es numérico, esta función devuelve un valor del 100%.
  ...
  >
    ...
    * Omita `TIMESERIES` para generar un cartel y una hoja de atributos que muestre los valores agregados para el percentil.
      ...
      Si no aparece ningún percentil, el valor predeterminado es el percentil 95. Para devolver solo el valor del percentil 50, la mediana, también puedes usar [median()](#func-median).
    ...
    * Cuando utilice percentil en una subconsulta unida, tenga en cuenta que los resultados de la subconsulta son un tipo de datos compuesto, que asigna cada percentil a su valor calculado.
      ...
      Para hacer referencia a cualquiera de los valores de percentil individuales en la consulta externa, puede utilizar la función [`getField()`](#func-getfield) . Tenga en cuenta que la clave asignada es una representación de cadena de un valor doble, por lo que debe agregar ".0" a los números enteros. Por ejemplo, la clave para el percentil 95 es `'95.0'`.
  ...
  >
    ...
    <CollapserGroup>
      <Collapser title={<>Usando <InlineCode>tuple</InlineCode></>}>
...
<CollapserGroup>
  ...
  >
    ...
    * `*`: Captura de comodines, similar al uso de captura de expresiones regulares
    ...
    <CollapserGroup>
      ...
      <Collapser
        title={<><InlineCode>aparse()</InlineCode> valores específicos</>}
      ...
      <Collapser
        title={<><InlineCode>aparse()</InlineCode> múltiples valores</>}
  ...
  >
    ...
    * La consulta que contiene llamadas a `blob()` tiene un valor LÍMITE máximo de 20
    ...
    * `blob()` no se puede utilizar en consulta facetada o consulta de series temporales
      ...
      <CollapserGroup>
        <Collapser title={<>Usando <InlineCode>blob()</InlineCode> en el registro extendido</>}>
          ```sql
  ...
  >
    ...
    <CollapserGroup>
      ...
      <Collapser title="Resultados de la consulta de facetas mediante una cadena formateada">
        Esta consulta devuelve la duración promedio de configuración de la conexión desde el evento PageView, facetada por una cadena compuesta por la ciudad, región y país del usuario.
  ...
  >
    ...
    Las unidades _distinguen entre_ mayúsculas y minúsculas. Todas las unidades están en minúsculas, a menos que su especificación requiera que estén en mayúsculas. Por ejemplo, las unidades de datos `'bits'` son válidas para bits y `'By'` debe tener una `B` mayúscula para bytes.
  ...
  >
    ...
    * Expresión regular con sintaxis de captura (las expresiones regulares en NRQL usan una sintaxis similar a Python, `r'...'`)
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de una condición de cláusula <InlineCode>SELECT</InlineCode></>}>
        ...
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de una condición de cláusula <InlineCode>FACET</InlineCode></>}>
        ...
        <Collapser title={<><InlineCode>capture()</InlineCode> dentro de una condición de cláusula <InlineCode>WHERE</InlineCode></>}>
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> con un reparto numérico</>}>
      ...
      <CollapserGroup>
        <Collapser title={<><InlineCode>capture()</InlineCode> múltiples valores con variables NRQL</>}>
  ...
  >
    ...
    * 'base64url': utiliza el [estándar RFC4648 base-64 con URL y alfabeto seguro para nombres de archivos](https://datatracker.ietf.org/doc/html/rfc4648#section-5)
      ...
      <CollapserGroup>
        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> en un atributo de cadena</>}>
          ```sql
          FROM Span SELECT entity.guid, decode(entity.guid, 'base64') WHERE entity.guid IS NOT NULL
          ```
        </Collapser>

        <Collapser title={<>Usando <InlineCode>decode()</InlineCode> en un <InlineCode>FACET</InlineCode></>}>
          ```sql
          FROM Span SELECT count(*) WHERE entity.guid IS NOT NULL FACET entity.guid, decode(entity.guid, 'base64')
          ```
        </Collapser>

        <Collapser title={<>Usar <InlineCode>decode()</InlineCode> en una cláusula <InlineCode>WHERE</InlineCode></>}>
          ```sql
          FROM Span SELECT count(*) WHERE entity.guid IS NOT NULL AND decode(entity.guid, 'base64') NOT LIKE '%APM%'
          ```
        </Collapser>

        <Collapser title={<>Usar <InlineCode>decode()</InlineCode> con un atributo de tipo blob</>}>
          ```sql
          FROM Log
  ...
  >
    ...
    * Incluir: si está presente, la lista de inclusión limita `dimensions()` a esos atributos.

    * Excluir: si está presente, el cálculo `dimensions()` ignora esos atributos.
  ...
  >
    ...
    * 'base64url': utiliza el [estándar RFC4648 base-64 con URL y alfabeto seguro para nombres de archivos](https://datatracker.ietf.org/doc/html/rfc4648#section-5)
      ...
      <CollapserGroup>
        <Collapser title={<>Usar <InlineCode>encode()</InlineCode> en un atributo</>}>
          ```sql
  ...
  >
    ...
    * atributo: un valor de cadena que contiene una dirección IP por sí sola o con una longitud de prefijo en notación CIDR.

      * Puede ser un atributo de cadena o un literal de cadena entre comillas.
      * La dirección IP debe ser una dirección IPv4.

    * número: un valor entero que representa la longitud del prefijo.
    ...
    * cidrFormat: un valor booleano opcional que se utiliza para determinar si la salida de la dirección de red debe formatearse en notación CIDR. De forma predeterminada, esto será verdadero.
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        id="filter-eventtype"
        title={<>Utilice <InlineCode>eventType()</InlineCode> en la función <InlineCode>filter()</InlineCode></>}
      >
      ...
      <Collapser
        id="facet-eventtype"
        title={<>Utilice <InlineCode>eventType()</InlineCode> con <InlineCode>FACET</InlineCode></>}
      >
  ...
  >
    ...
    * condición: una expresión que puede evaluarse como verdadera o falsa
    * trueValue: este valor se devuelve si la expresión booleana es verdadera
    * falseValue: este valor opcional se devuelve si la expresión booleana es falsa o, si no se proporciona NULL, se devuelve
    ...
    <CollapserGroup>
      ...
      <Collapser title={<>Usar con <InlineCode>AND</InlineCode> y <InlineCode>OR</InlineCode></>}>
        ```sql
        FROM Log
          SELECT count(*)
      ...
      <Collapser title={<>Anidado <InlineCode>If()</InlineCode></>}>
  ...
  >
    ...
    * atributo: un valor de cadena JSON.
    * path: un valor de cadena opcional que se utiliza para hacer referencia directamente a una parte particular del JSON dentro del parámetro `attribute` . Consulte la sección [Referencia de sintaxis de ruta de análisis JSON](#json-parse-path-syntax-reference) a continuación.
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Hacer referencia a una clave/índice usando la sintaxis de corchetes"
      >
        <DoNotTranslate>
          **Referencing a key**
        </DoNotTranslate>
      ...
      <Collapser
        ...
        title="Mapeo claves JSON para atributo"
      >
    ...
    <Collapser
      ...
      title="Referencia de sintaxis de ruta de análisis JSON"
    >
      ...
      <table id="join-types">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              Devuelve el rango de valores de la lista desde el primer índice hasta el segundo, excluyendo el valor del segundo índice. En este caso, se devuelven los valores de la lista en los índices 0 y 1.
            </td>
          ...
          <tr>
            ...
            <td>
              Devuelve el rango de valores de la lista desde el principio hasta el segundo índice, excluyendo el valor del segundo índice. En este caso, se devuelven los valores de la lista en los índices 0 y 1.
            </td>
          ...
          <tr>
            ...
            <td>
              Devuelve todos los valores de la lista excepto el último n, donde n es un número negativo después de los dos puntos (es decir, \[:-n]). En este caso, se devolverá el valor de la lista en el índice 0.
            </td>
          ...
          <tr>
            ...
            <td>
              Devuelve el rango de valores de la lista desde el índice especificado hasta el final de la lista. En este caso, se devuelven los valores de la lista en los índices 1 y 2.
            </td>
          ...
          <tr>
            ...
            <td>
              Devuelve los últimos n valores de la lista, donde n es un número negativo antes de los dos puntos (es decir, \[-norte:]). En este caso, se devolverá el valor de la lista en el índice 2.
            </td>
          ...
          <tr>
            ...
            <td>
              Devuelve el valor principal especificado de todos los miembros de la lista. En este caso, el valor principal “id”.
            </td>
          ...
          <tr>
            <td>
              'valorC\[\*]\["etiqueta", otro]'
            </td>

            <td>
              Devuelve las claves especificadas de todos los miembros de la lista. En este caso, la “etiqueta” y “otro” valor principal.
            </td>
    ...
    <CollapserGroup>
      ...
      <Collapser
        ...
        title="Analizar un valor específico de un mensaje de log"
      >
      ...
      <Collapser
        ...
        title="Analizar múltiples valores anidados de una cadena JSON"
      >
    ...
    _Funciones relacionadas:_ [`mapKeys()`](#func-mapKeys), [`mapValues()`](#func-mapValues)
  </Collapser>
  ...
  >
    ...
    * str: el valor de la cadena que se escribirá en minúsculas
      ...
      * Si este argumento se evalúa como nulo, la función `lower()` devolverá nulo.
      ...
      <CollapserGroup>
        <Collapser title="Minúscula una cadena">
          ...
          <figcaption>
            <InlineCode>lower(str)</InlineCode> ejemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Función relacionada: [<InlineCode>upper()</InlineCode>](#func-upper)_
  </Collapser>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Extraiga una lista de claves dentro de una cadena JSON"
      >
        ```sql
      ...
      <Collapser
        ...
        title="Extraiga una lista de claves de una cadena JSON con claves anidadas"
      >
        ```sql
  ...
  >
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Extraiga una lista de valores dentro de una cadena JSON"
      >
        ```sql
      ...
      <Collapser
        ...
        title="Extraiga una lista de valores de una cadena JSON con valores anidados"
      >
        ```sql
  ...
  >
    Utilice la función `mod()` para devolver el módulo mínimo después de dividir el valor del atributo numérico proporcionado (el primer argumento o dividendo) por un valor numérico (el segundo argumento o divisor). Esta operación de módulo se puede utilizar dentro de una condición de cláusula WHERE para filtrar a un subconjunto arbitrario de resultados o dentro de una cláusula FACET como una forma de subdividir el conjunto de resultados.

    <CollapserGroup>
      <Collapser title={<><InlineCode>mod()</InlineCode> dentro de una condición de cláusula <InlineCode>WHERE</InlineCode></>}>
        ```sql
    ...
    <CollapserGroup>
      <Collapser title={<><InlineCode>mod()</InlineCode> dentro de una cláusula <InlineCode>FACET</InlineCode></>}>
        ```sql
  ...
  >
    ...
    * str: la cadena en la que se encuentra la subcadena.

      * Puede ser cualquier cosa que se evalúe como una cadena, incluida una cadena literal entre comillas, un atributo de cadena de consulta, una función que devuelve una cadena o incluso una subconsulta que devuelve un único valor de cadena.

    * substr: la cadena que se buscará dentro de str.

    * ocurrencia: indica qué ocurrencia de substr de la cual devolver la posición.
      ...
      * Si es negativo, encuentre la enésima aparición de la substr desde el final de str. La última aparición de substr sería la aparición -1.
      ...
      Alias: <InlineCode>indexOf(str, substr , occurrence)</InlineCode> - `indexOf()` es un nombre alternativo para la función `position()`
    ...
    <CollapserGroup>
      <Collapser title="Encuentre el índice posicional de una subcadena dentro de una URL">
        Esta consulta demuestra el uso de la función <InlineCode>position()</InlineCode> para encontrar el índice posicional de varias subcadenas dentro de una cadena. Aquí también se demuestra el uso de la función <InlineCode>position()</InlineCode> dentro de los argumentos de la función [<InlineCode>substring()</InlineCode>](#func-substring) .
        ...
        <figcaption>
          <InlineCode>position(str, substr , occurrence)</InlineCode> ejemplo
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
  ...
  >
    ...
    * str: la cadena de la que extraer una subcadena.

      * Puede ser cualquier cosa que se evalúe como una cadena, incluida una cadena literal entre comillas, un atributo de cadena de consulta, una función que devuelve una cadena o incluso una subconsulta que devuelve un único valor de cadena.
      * Si este argumento se evalúa como nulo, la función `substring()` devolverá nulo.

    * inicio: la posición dentro de str desde la cual comenzar la extracción.
    ...
    * length: la longitud, o número de caracteres, de la subcadena que se extraerá de str.

      * _Opcional_ : si no se proporciona la longitud, se incluirán todos los caracteres de str después de la posición inicial resuelta.
      ...
      <CollapserGroup>
        <Collapser title="Extraer varias partes de una cadena.">
          ...
          <figcaption>
            <InlineCode>substring(str, start , length)</InlineCode> ejemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Consulte la función [<InlineCode>position()</InlineCode>](#func-position) para ver ejemplos del uso de <InlineCode>substring()</InlineCode> y <InlineCode>position()</InlineCode> juntos._
  </Collapser>
  ...
  >
    ...
    * timestamp : una timestamp numérica que se traducirá en una cadena de fecha y hora. Puede ser un valor numérico o un atributo y se convertirá en un `long` internamente.

    * patrón: un patrón de fecha y hora opcional que se utiliza para formatear el resultado. Consulte la sección _Patrones para formatear y analizar_ en la [documentación de DatetimeFormatter](https://docs.oracle.com/javase/8/docs/api/java/time/format/DateTimeFormatter.html) para saber cómo construir una cadena de patrón.
    ...
    * zona horaria: un valor de zona horaria opcional que se utiliza para interpretar la cadena de fecha y hora (ej. UTC).
    ...
    <CollapserGroup>
      ...
      <Collapser title="Traducir una timestamp usando el parámetro de zona horaria">
        La siguiente consulta traduce el atributo `timestampValue` utilizando la cadena de patrón `yyyy-MM-dd` con la zona horaria establecida en 'América/Los_Angeles'. Esto devolverá la cadena de fecha y hora `1969-12-31`.
      ...
      <Collapser title={<>Traducir una timestamp usando la zona horaria en la cláusula <InlineCode>WITH TIMEZONE</InlineCode></>}>
  ...
  >
    ...
    * datestring: una cadena de fecha y hora que se traducirá a una timestamp (milisegundos de época). Puede ser un atributo de cadena o un literal de cadena entre comillas.

    * patrón: un patrón de fecha y hora opcional que se utiliza para analizar el parámetro de cadena de fecha. Consulte la sección _Patrones para formatear y analizar_ en la [documentación de DatetimeFormatter](https://docs.oracle.com/javase/8/docs/api/java/time/format/DateTimeFormatter.html) para saber cómo construir una cadena de patrón.
    ...
    * zona horaria: un valor de zona horaria opcional que se utiliza para interpretar el parámetro de cadena de fecha (ej. hora del Pacífico).
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="Interpretación parcial de fecha y hora al analizar cadenas de fecha y hora"
      >
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              ...
              <td>
                Utilizará el argumento de zona horaria o el valor CON ZONA HORARIA. El valor predeterminado es UTC.
              </td>
    ...
    <CollapserGroup>
      ...
      <Collapser title="Analizar una cadena de fecha y hora usando el parámetro de zona horaria">
        La siguiente consulta analiza la cadena de fecha y hora '2023-11-03 11:00:32' con la zona horaria establecida en 'América/Los_Angeles'. Esto devuelve el valor timestamp `1699034432000`.
      ...
      <Collapser title={<>Analizar una cadena de fecha y hora utilizando la zona horaria en la cláusula <InlineCode>WITH TIMEZONE</InlineCode></>}>
  ...
  >
    ...
    * str: el valor de la cadena que se escribirá en mayúsculas.
    ...
    * Si este argumento se evalúa como nulo, la función `upper()` devolverá nulo.
      ...
      <CollapserGroup>
        <Collapser title="Poner en mayúscula una cadena">
          ...
          <figcaption>
            <InlineCode>upper(str)</InlineCode> ejemplo
          </figcaption>
        </Collapser>
      </CollapserGroup>

      _Función relacionada: [<InlineCode>lower()</InlineCode>](#func-lower)_
  </Collapser>
</CollapserGroup>
