...
Para consultar estos datos, use el tipo de evento `Entity` (y `Relationships` o `entityRelationships` para consultas específicas de relaciones).
...
<CollapserGroup>
  ...
  <Collapser id="total-tx" title="Ver el estado de la entidad en un punto específico en el tiempo">
    * **Objetivo**: Un desarrollador está investigando un evento de alerta que ocurrió hace varias horas y necesita conocer la configuración o el estado exacto de un host específico (o contenedor, aplicación, etc.) en ese momento particular.
