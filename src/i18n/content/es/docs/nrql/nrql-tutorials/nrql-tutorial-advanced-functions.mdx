...
<Steps>
  <Step>
    La desviación estándar mide la cantidad de variación o dispersión dentro de un conjunto de valores. Utiliza una escala de bajo (valores cercanos al promedio) a alto (valores lejanos al promedio). La función `stddev()`le permite leer entre líneas de los promedios y comprender los valores reportados a un nivel más profundo. En este ejemplo, compara la desviación estándar del tiempo de respuesta de la transacción (&quot;duration&quot;) del último día con el día anterior.
...
<Steps>
  ...
  <Step>
    Puede usar la limitación en un atributo para imponer un límite superior o inferior a su valor. Esto es útil para cosas como garantizar que los valores atípicos extremos no sesguen la escala de un gráfico de series temporales. `clamp_max(duration, 10)` devuelve la duración, a menos que supere los 10, en cuyo caso devuelve 10. En pocas palabras, cualquier valor mayor que 10 sigue devolviendo 10. `clamp_min(duration,1)` hace lo inverso; si alguna duración es inferior a 1, se informa como igual a 1.
...
<Steps>
  ...
  <Step>
    ...
    El motor de expresiones regulares usa la sintaxis RE2. Si necesita eliminar caracteres, es posible que deba usar las secuencias de escape de doble barra invertida. (p. ej. `\\`).
...
Esperamos que NRQL le resulte útil para explorar sus datos y, si tiene más preguntas o encuentra problemas, no dude en [contactar al soporte de New Relic](https://support.newrelic.com/). O, si lo prefiere, puede compartir su experiencia con nuestra [comunidad en línea](https://support.newrelic.com/s/).