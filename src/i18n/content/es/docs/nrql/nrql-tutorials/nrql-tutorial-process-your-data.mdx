...
* Busque para incluir/excluir usando comodines con `LIKE`/`NOT LIKE` o limite los resultados a aquellos `IN` una lista o `NOT IN` esa lista
...
<Steps>
  <Step>
    Después de completar el [primer tutorial de NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-tutorial/), utilizó `count()`, `average()`, `sum()`, `max()` y `min()` para transformar sus datos de manera significativa. Pero ¿qué pasa si quieres encontrar y contar el valor único? Para encontrar el número de valor único registrado para un atributo durante un rango de tiempo específico, puede usar el `uniqueCount() function`. Solo necesita proporcionar el atributo que queremos inspeccionar como argumento. Por ejemplo, aquí hay una consulta para mostrar toda la API de llamada pública única:
...
Las funciones `latest()`/`earliest()` tienen un valor particular cuando se trata de datos dispersos o cuando se investiga cuándo algo comenzó o dejó de informar (suponiendo que New Relic todavía almacena los datos). La función `percentage()` puede mostrarle qué porcentaje de evento coincidió con un calificador en comparación con el conjunto de resultados general. Por último, puede utilizar `percentile()` como indicador clave de rendimiento estableciendo un objetivo para garantizar que el 90 % de todas las transacciones del usuario final superen la duración proporcionada.
...
<Steps>
  ...
  <Step>
    A veces, es posible que reciba una hora de evento en tiempo de época (Unix). Convenientemente, puede usar la marca de tiempo de época con las cláusulas`SINCE` y `UNTIL` para no tener que traducir manualmente estos valores a otro formato de fecha.
