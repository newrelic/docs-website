...
6. Confirme que ha configurado su Kinesis Data Firehose con los detalles de destino correctos:
   ...
   * Asegúrese de que <InlinePopover type="licenseKey" />contenga su clave de licencia de 40 caracteres hexadecimales.
   * Asegúrese de que se haya seleccionado el centro de datos correcto de EE. UU. (predeterminado), la UE o JP para su cuenta de New Relic. Consejo: si el `license_key` comienza con “eu”, entonces debe seleccionar el centro de datos de la UE. Si el `license_key` comienza con “jp”, entonces debe seleccionar el centro de datos de JP.
...
8. Consulte la documentación oficial de AWS Firehose sobre la resolución de problemas de uso e implementación:
...
9. Si aún experimenta problemas, comuníquese con el soporte de AWS para obtener más ayuda mediante la creación de un [caso de soporte](https://docs.aws.amazon.com/awssupport/latest/user/case-management.html). Incluya la información del flujo de AWS Firehose y [el extremo objetivo](/docs/infrastructure/amazon-integrations/connect/cloudwatch-metric-streams/aws-metric-stream-setup#manual-setup), el rango de tiempo del problema y cualquier mensaje de error que haya recibido.
...
* Al usar series temporales, la interfaz de usuario de New Relic podría realizar algún redondeo según los intervalos.
