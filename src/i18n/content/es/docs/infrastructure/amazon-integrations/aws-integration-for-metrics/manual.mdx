---
title: Integrar manualmente desde la consola de AWS
tags:
  - Integrate manually
  - Manual integration
...
Para una integración perfecta de su cuenta AWS con New Relic, [la integración a través de CloudFormation y CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation) es el enfoque más recomendado. Si no puede usar plantillas de CloudFormation y no prefiere usar Terraform, consulte el siguiente procedimiento para integrar manualmente desde la consola de AWS.
...
6. En la pantalla <DNT>**Choose data integration type**</DNT>, elija entre las siguientes opciones de integración:

   * **Monitoreo AWS en tiempo real (Recomendado)**: Esta opción es para los servicios que admiten CloudWatch Metric Streams.
   * **Sondeo de API**: esta opción es adecuada para servicios no compatibles con CloudWatch Metric Streams.

7. Desde la página <DNT>**Create role**</DNT> a la página <DNT>**Configure budgets policy**</DNT>, siga las instrucciones en pantalla y realice los pasos en la consola de AWS.

8. Luego de crear la política en la consola de AWS, cambie a la plataforma New Relic y continúe con la integración guiada hasta que vea sus datos de AWS en New Relic.
