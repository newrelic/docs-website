...
Recomendamos enviar los datos a una cuenta New Relic que no sea de producción donde pueda realizar pruebas de forma segura. Si esa no es una opción, los filtros de AWS CloudWatch Metric Stream están disponibles para incluir o excluir ciertos espacios de nombres que pueden causar problemas.
...
## Métricas no disponible con CloudWatch Metric Streams

Cuando las métricas están disponibles para AWS CloudWatch Metric Stream con más de 2 horas de retraso, estas métricas no se incluyen en la secuencia.

Ejemplos de espacios de nombres de AWS que pueden contener métricas que se agregan y exponen después de 2 horas incluyen: AWS DMS, AWS RDS, AWS DocDB, AWS S3 y AWS DAX. Consulte [la documentación de AWS](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-metric-streams-setup.html) para obtener más información.