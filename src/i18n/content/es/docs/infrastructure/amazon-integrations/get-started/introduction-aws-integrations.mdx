...
* **Automatizar con CloudFormation**: CloudFormation es un servicio de AWS que le permite modelar y automatizar la configuración de su integración mediante plantillas declarativas. Para reducir los errores de configuración manual, utilice este método para conectar su cuenta de AWS a New Relic. Puede diseñar completamente la integración en la UI de New Relic y desplegarla en la consola de AWS.

Este método aprovisiona los recursos que reenvían su métrica a New Relic, incluido un rol de <DNT>IAM</DNT>, un metric stream de <DNT>Amazon CloudWatch</DNT> y un flujo de entrega de <DNT>Amazon Kinesis Data Firehose</DNT>. Los metric streams operan por región, por lo que necesita un metric stream y un flujo de entrega de <DNT>Kinesis Data Firehose</DNT> en cada región de AWS que monitoree. Consulte el siguiente diagrama para ver cómo se conectan estos componentes.
...
<figcaption>
  Cómo una integración de CloudWatch Metric Streams reenvía métrica a New Relic. Necesita un metric stream y un flujo de entrega de Kinesis Data Firehose en cada región de AWS que monitoree.
</figcaption>
