...
Con la gestión de secretos, puede configurar el agente y las integraciones en el host para usar datos confidenciales (como contraseñas) sin tener que escribirlos como texto plano en los archivos de configuración. Actualmente, se admiten Hashicorp Vault, AWS KMS, CyberArk, Azure Key Vault, la ofuscación de la CLI de New Relic y los comandos personalizados.
...
Al usar variables de entorno en los archivos de configuración de integraciones on-host, se debe definir la configuración [`passthrough_environment`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#passthrough_-environment).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Configuración de recuperación de secretos de Azure Key Vault](#azure-key-vault-secrets)
      </td>
    </tr>
...
### Propiedades tls\_config [#tls-config-properties]
...
En referencia al ejemplo anterior, llame al nombre de usuario y la contraseña de esta manera:
...
Nota: Para Microsoft SQL Server, si usa un inicio de sesión de usuario con autenticación de Windows, debe anteponer su dominio al `USERNAME`. Por ejemplo:

```yaml
USERNAME: <domain>\${credentials.user}
```

## Secretos de Azure Key Vault [#azure-key-vault-secrets]

Para recuperar secretos de Azure Key Vault, establezca las siguientes propiedades en la sección `azure-key-vault`. Ambas claves son obligatorias:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        La URL de la instancia de Azure Key Vault de la que se recuperará el secreto

        Ejemplo: `https://my-vault.vault.azure.net/`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        El nombre del secreto a recuperar. El agente siempre recupera la última versión del secreto.
      </td>
    </tr>
  </tbody>
</table>

El agente se autentica en Azure Key Vault utilizando la cadena de credenciales predeterminada del SDK de Azure, probando los siguientes métodos en orden hasta que uno tenga éxito. No se requiere configuración adicional del agente:

* Variables de entorno `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` y `AZURE_TENANT_ID`
* Una identidad administrada asignada a la máquina virtual (VM) de Azure o al pod de AKS que ejecuta el agente (recomendado para producción)
* Credenciales de la CLI de Azure (`az login`), útiles para desarrollo y pruebas

<Callout variant="tip">
  Solo se admiten valores secretos. No se admite la recuperación de certificados o claves de Azure Key Vault.
</Callout>

El siguiente ejemplo recupera un secreto llamado `mssql-password` de un Azure Key Vault y lo usa como contraseña para una integración en el host de MSSQL:
...
<Callout variant="important">
  Recomendamos usar cualquiera de los proveedores de secretos compatibles en lugar de la ofuscación simple cuando sea posible. La ofuscación simple no garantiza la confidencialidad de sus credenciales, ya que el proceso puede revertirse.
