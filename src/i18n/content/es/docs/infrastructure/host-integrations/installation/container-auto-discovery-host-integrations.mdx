...
Para configurar el descubrimiento, configure una entrada `discovery` en su configuración de integración YAML usando las siguientes propiedades. Un contenedor coincidirá cuando <DoNotTranslate>**all**</DoNotTranslate> las propiedades coincidan con los valores definidos.

<Callout variant="tip">
  Los valores de las propiedades coincidirán con un <DoNotTranslate>**exact string**</DoNotTranslate>, a menos que el valor a coincidir esté establecido entre `/.../`. Los contenidos dentro de las barras se compararán como [expresiones regulares](https://en.wikipedia.org/wiki/Regular_expression).
</Callout>
