...
El [formato de configuraciónlegacy ](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-legacy-configuration-format)anterior también es compatible con el actual agente de infraestructura.
...
Aquí hay un ejemplo de configuración característica dos integración: nuestra [integración Docker ](https://github.com/newrelic/nri-docker)incorporada, que no requiere configuración, y nuestra [integraciónMySQL ](/docs/integrations/host-integrations/host-integrations-list/mysql-monitoring-integration):
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Lista opcional de argumentos de línea de comando cuando se utiliza `name` para proporcionar el ejecutable de integración. _Disponible desde la versión del agente <DoNotTranslate>**1.13.0**</DoNotTranslate>._
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**If the `exec` property is set**</DoNotTranslate>: La propiedad `name` solo proporciona un identificador para la instancia de integración. Este identificador se utiliza en el mensaje de registro y para proporcionar una [categoría/fuente de inventario](/docs/infrastructure/infrastructure-ui-pages/infra-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure) predeterminada en el formato `integration/<name>` (por ejemplo, `integration/nri-redis`). Esta ruta de inventario se puede anular con la opción de configuración `inventory_source` .

    * <DoNotTranslate>**If the `exec` property is not set**</DoNotTranslate>: El agente busca (y ejecuta) un ejecutable con el valor `name` en cualquiera de las siguientes carpetas:
  ...
  >
    ...
    Si alguna de las rutas/argumentos tiene espacios que son parte de un solo elemento, puede usar una notación de matriz YAML:
