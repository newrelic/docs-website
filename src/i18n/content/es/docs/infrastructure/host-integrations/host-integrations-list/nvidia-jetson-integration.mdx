...
<Steps>
             ...
             apis:
               - event_type: tegrastats
                 commands:
                   ...
                   - run: tail -n 3 path/to/tegrastats
                     split: horizontal
                     set_header: [pom_in_currect, pom_in_peak, pom_gpu_used, pom_gpu_total, pom_cpu_used, pom_cpu_total]
                     regex_match: true
                     split_by: "POM_5V_IN (\\d+)/(\\d+) POM_5V_GPU (\\d+)/(\\d+) POM_5V_CPU (\\d+)/(\\d+)"            
       ```
  </Step>
  ...
  <Step>
    ...
    3. Vea su dashboard yendo a

       <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > All Capabilities > Dashboards**</DoNotTranslate>
