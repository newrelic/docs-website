---
title: Integración temporal en la nube
tags:
...
<Steps>
  <Step>
    ...
    Para emplear la integración temporal en la nube, primero debe [instalar el agente de infraestructura](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) en el mismo host. El agente de infraestructura monitorea el host en sí, mientras que la integración que instalará en el siguiente paso amplía su monitoreo con datos temporales específicos, como base de datos e instancia métrica.
  </Step>

  <Step>
    ## Configurar `nri-flex` para la nube temporal [#configure-flex]

    Una vez que las métricas de Temporal Cloud estén expuestas en el extremo Prometheus, al que se puede acceder a través de `https://<account-id>.tmprl.cloud/prometheus`, deberá configurar su instalación en consecuencia.
  ...
  <Step>
    ...
    Aquí hay una consulta NRQL para verificar el recuento de solicitudes del servicio frontend de nube temporal:
