---
title: Integración de monitoreo de MongoDB
tags:
  - Integrations
...
Nuestra integración MongoDB recopila y envía métricas dimensionales desde [MongoDB](https://www.mongodb.com/).
...
<figcaption>
  dashboard instalado a través del [New Relic MongoDB monitor inicio rápido](https://newrelic.com/instant-observability/mongodb-prometheus).
</figcaption>

Para obtener datos de la instancia de MongoDB, nuestra integración MongoDB utiliza un [exportador Prometheus](https://github.com/percona/mongodb_exporter) de terceros, que expone Prometheus métrica en el puerto especificado en la configuración. La integración recopila estas métricas, las transforma en entidad, las filtra y luego las envía a New Relic.

Para instalar la integración de monitoreo de MongoDB, siga los siguientes pasos:

1. [Consultar requisitos](#req).
2. [Instalar y activar la integración](#install).
3. [Configurar la integración](#configuration).
4. [Buscar y utilizar datos](#find-and-use).

## Compatibilidad y requisitos [#req]

### Versiones de MongoDB [#MongoDB-versions]

Nuestra integración es compatible con MongoDB versiones 4.0 a 7.0, [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb) y MongoDB Atlas M10 o superior.
...
* Usuario de MongoDB con [permisos`clusterMonitor` y `read` ](#enable-instance). Consulte [Configurar su MongoDB](#setup-instance) para obtener más información.
* Para monitor MongoDB Atlas, el usuario aún necesita un entorno alojado para instalar el agente de infraestructura e integración y conectarse de forma remota a la instancia de Atlas.
...
<CollapserGroup>
  ...
  >
    1. Instale [el agente de infraestructura](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install) y reemplace la variable `INTEGRATION_FILE_NAME` con `nri-mongodb3`.
    ...
    4. Edite el archivo de configuración `mongodb3-config.yml` con su editor favorito. Consulte algunos [ejemplos de archivos de configuración](#examples).

    5. Para habilitar el análisis automático de errores de Mongodb, los registros y el reenvío, cree un archivo YAML con el siguiente contenido en `/etc/newrelic-infra/logging.d/elasticsearch-log.yml`. No es necesario reiniciar el agente.
  ...
  >
    1. Descargue el [instalador MSI](https://download.newrelic.com/infrastructure_agent/windows/integrations/nri-ibmmq/nri-mongodb3-amd64.msi) `nri-mongodb3`.
    ...
    4. Edite el archivo `mongodb3-config.yml` como se describe en los [ajustes de configuración](#config).
  </Collapser>

  <Collapser
    id="containerized-environments"
    title="Entornos en contenedores"
  >
    Hay disponible una [imagen de Docker](https://hub.docker.com/r/bitnami/mongodb-exporter/tags) que empaqueta el mismo exportador de Prometheus utilizado por la integración. Por lo tanto, una vez configurado y raspado el exportador, las métricas son las mismas que proporciona la integración.

    Los pasos a continuación brindan orientación para configurar la integración en entornos como Kubernetes, donde puede reportar Prometheus métrica desde diferentes fuentes de forma centralizada. Si solo necesita configurar la integración en un contenedor Docker , consulte [las instrucciones de Openmetrics Docker](#docker-openmetrics).

    #### Ejecutando el exportador

    Para monitor una instancia de MongoDB en Kubernetes o ECS, necesita ejecutar la imagen del exportador de MongoDB como un sidecar o como una carga de trabajo separada.

    Tenga en cuenta que ambos gráficos MongoDB Bitnami Helm proporcionan una forma estándar de desplegar y configurar el exportador MongoDB que puede utilizar como referencia.

    Por ejemplo, el siguiente `values.yaml` del gráfico `bitnami/mongodb` configura cada carga de trabajo de MongoDB con un exportador que se ejecuta como sidecar configurado como integración de NewRelic:
    ...
    Tenga en cuenta que, en lugar de `--collect-all`, también puede pasar el recolector uno por uno y desactivar cualquiera de ellos si es necesario: `--collector.dbstats --collector.collstats --collector.indexstats --collector.replicasetstatus --collector.topmetrics --collector.diagnosticdata`.

    #### Raspe la métrica

    Hay diferentes formas de extraer Prometheus métrica y reenviarlos a New Relic, consulta [nuestros documentos para saber más](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent).

    Ejecutando y raspando el exportador Bitnami o `percona/mongodb_exporter`, las métricas raspadas son las mismas y las <InlinePopover type="dashboards"/>funcionan. Sin embargo, la integración añade una etiqueta común a todas las métricas raspadas de un clúster MongoDB particular `mongodb_cluster_name` para poder agrupar métricas provenientes de diferentes clúster.

    <Callout variant="tip">
      El atributo `mongodb_cluster_name` para todas las métricas de MongoDB es necesario para el dashboard de MongoDB y debes agregarlo manualmente. Dependiendo del agente que raspe el Prometheus métrica, existen diferentes formas de agregar etiquetas comunes.
    </Callout>

    Por ejemplo, la siguiente configuración de agente de Prometheus agrega a todas las métricas de Prometheus que coincidan con `mongodb_.*` la etiqueta `mongodb_cluster_name=cluster-name-example`.
    ...
    Si está utilizando varios pods `percona/mongodb_exporter` para servir métricas de múltiples clústeres MongoDB, puede etiquetar cada uno con un `mongodb_cluster_name` separado usando una configuración como esta:
    ...
    Por otro lado, en un servidor Prometheus, puedes agregar atributo personalizado a todas las métricas con `external_labels`, o por trabajo con `static_configs`, o para escenarios más complejos `metric_relabel_config`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Integración de Prometheus Openmetrics para Docker (avanzado)"
  >
    Si necesita ejecutar la integración desde un contenedor, puede usar [la integración de Prometheus Openmentrics para Docker](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics).

    Ejecute [mongodb-exporter](https://hub.docker.com/r/percona/mongodb_exporter/tags), que es el mismo que los paquetes de integración, junto con [nri-prometheus](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics). Puede utilizar el ejemplo `docker-compose` siguiente como base para su configuración:
    ...
    Donde, `config.yaml` es el archivo de configuración de nri-prometheus. Consulte [los documentos correspondientes](/docs/infrastructure/prometheus-integrations/get-started/send-prometheus-metric-data-new-relic/#OpenMetrics) para obtener detalles adicionales.
    ...
    Suponiendo que el archivo Docker-compose está en la misma carpeta que la variable de entorno `config.yaml` y `${NEW_RELIC_LICENSE_KEY}` contiene su clave de licencia, puede ejecutarlo de la siguiente manera:
...
### Configurando tu MongoDB [#setup-instance]
...
<CollapserGroup>
  <Collapser
    id="setup-hosted"
    title="Servidores alojados MongoDB o Percona"
  >
    <Callout variant="tip">
      Complete estos pasos en los mongos a monitorear. Si desea recopilar métricas a nivel de mongod, como estadísticas de host o conjunto de réplicas, cree también el rol y el usuario en cada mongod.
    </Callout>

    1. En el shell de MongoDB, cambie a la base de datos de administración.

       ```
       use admin
       ```

    2. Utilice el siguiente comando para crear un nuevo usuario y asignarle los roles `clusterMonitor` y `readAnyDatabase` (ajuste el nombre de usuario y la contraseña según sea necesario).
    ...
    3. Edite el archivo de configuración YAML `mongodb3-config.yml` e informe los parámetros requeridos `mongodb_cluster_name` y `mongodb_uri`. Cambie otras [opciones de configuración](#instance-settings) si es necesario.

       Para obtener más información, consulte la [documentación de MongoDB Atlas](https://docs.mongodb.com/manual/reference/method/db.createUser/index.html).
  </Collapser>
  ...
  <Collapser
    ...
    title="Atlas"
  >
    1. Utilizando la CLI de Atlas, ejecute el siguiente comando para crear un nuevo usuario y asignarle los roles `clusterMonitor` y `readAnyDatabase` (ajuste el nombre de usuario y la contraseña según sea necesario).

       ```
       atlas dbuser create -u username -p password --role clusterMonitor,readAnyDatabase
       ```

       Alternativamente, puede utilizar la UI de Atlas para crear su usuario y asignarle los mismos privilegios específicos. Para obtener más información, consulte la [documentación de MongoDB Atlas](https://www.mongodb.com/docs/atlas/security-add-mongodb-users/#add-database-users).

    2. Edite el archivo de configuración YAML `mongodb3-config.yml` e informe los parámetros requeridos `mongodb_cluster_name` y `mongodb_uri`. Cambie otras [opciones de configuración](#instance-settings) si es necesario.
  </Collapser>
</CollapserGroup>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        URI de conexión de MongoDB. <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
...
### Configuración de ejemplo (mongodb3-config.yml) [#examples]
...
<CollapserGroup>
  ...
  >
    Esta es la configuración básica utilizada para recopilar todas las métricas. Agregue credenciales a su URI de mongodb si son necesarias. NOTA: Si su contraseña tiene caracteres especiales, deberá colocar comillas simples alrededor del URI.
  ...
  <Collapser
    ...
    title="Deshabilitar el recolector"
  >
    Puede desactivar selectivamente el selector. En este ejemplo, `TOP` métrica no se recopilará sumando `top_stats: false`. Tenga en cuenta que deshabilitar el recolector puede afectar su panel de control:
  ...
  <Collapser
    ...
    title="Monitoreo de múltiples instancias"
  >
    Esta es la misma configuración básica utilizada para recopilar métricas que muestra cómo ejecutar la integración dos veces conectándose a dos hosts. Tenga en cuenta que cada instancia de la integración generará un exportador adicional escuchando en un puerto diferente. Cada instancia necesitará un `mongodb_cluster_name` y `exporter_port` únicos.
  ...
  >
    Esta es la configuración básica que utiliza filtros para restringir las colecciones de monitores.<br/>En este ejemplo monitor todas las colecciones de la base de datos `db1` y monitor solo `collection2` de la base de datos `db2`. Agregue otros filtros separados por comas:
  ...
  <Collapser
    id="auth-source"
    title="Fuente de autenticación alternativa"
  >
    De forma predeterminada, la fuente de autenticación para cualquier conexión es `admin`. Si necesita configurar una fuente de autenticación alternativa, puede agregarla a su URI de MongoDB como `/auth_source`:
  ...
  <Collapser
    id="tls-config"
    title="Conexiones TLS"
  >
    Si necesita usar TLS para conectarse a su instancia, use el URI de MongoDB para agregar su parámetro TLS. Puede consultar la lista de parámetros permitidos en la [documentación oficial de Mongo](https://www.mongodb.com/docs/manual/reference/connection-string/#tls-options):
  ...
  <Collapser
    id="atlas-config"
    title="Conjunto de réplicas de Atlas"
  >
    Al conectarse a una instancia de Atlas, asegúrese de que la dirección IP donde se ejecuta la integración pueda conectarse a su instancia de Atlas. Establezca `mongo_direct_connect` en falso requerido para conexiones Atlas:
...
* El [de inicio rápido New Relic MongoDB monitor ](https://newrelic.com/instant-observability/mongodb-prometheus)está disponible a través de [Observabilidad instantánea (I/O)](https://newrelic.com/instant-observability/). Proporciona un dashboard que le permite explorar fácilmente sus datos, comprender el contexto y resolver problemas más rápido.
...
* métricas se adjuntan a la muestra métrica de la [entidad](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) `MONGODB_INSTANCE`, `MONGODB_DATABASE` y `MONGODB_COLLECTION`.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Tipo de aseveración según [listado oficial](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#asserts).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * _vacío_ - para instancia independiente y Atlas
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Nivel de preocupación por las operaciones de consulta de lectura (disponibles, linealizables, ...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#opreadconcerncounters).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo o estado de las conexiones a la base de datos (activas, disponibles, totales,...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#connections).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de recuento para el estado de bloqueo global de la base de datos (lectores, escritores, total). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#globallock).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de cursor para cursores abiertos de datos (fijado, sin tiempo de espera, total, ...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.metrics.cursor.open).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operación para acceso a documentos (insertado, eliminado, devuelto, actualizado). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.metrics.document).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operación para contadores de opciones (eliminar, obtener más, insertar, ...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#opcounters).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Modo para bloqueos de datos (R, W, r y w). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.locks).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Estado miembro en formato de cadena a partir de los [códigos de estatus oficiales](https://www.mongodb.com/docs/manual/reference/replica-states/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Nombre del clúster establecido en su [archivo de configuración](#instance-settings). Tenga en cuenta que esta dimensión se aplica a todas las métricas.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de operación para latencia (lecturas, escrituras, comandos). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#oplatencies).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Latencia buckets WiredTiger lectura/escritura rendimiento métrica (bucket1, bucket2, ...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#wiredtiger).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Recurso o tipo para bloqueos de datos (Global, Mutex, ParallelBatchWriterMode, ...). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.locks).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Estado del conjunto de réplicas. Un número entero que representa el estado de la réplica después de los [códigos de estado oficiales](https://www.mongodb.com/docs/manual/reference/replica-states/).
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Tipo de transacción concurrente para el motor de almacenamiento WiredTiger (escritura, lectura). [Más información](https://www.mongodb.com/docs/manual/reference/command/serverStatus/#mongodb-serverstatus-serverstatus.wiredTiger.concurrentTransactions).
      </td>
    </tr>
  </tbody>
</table>
...
Estas son las 3 [entidades](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/) creadas: `MONGODB_INSTANCE`, `MONGODB_DATABASE` y `MONGODB_COLLECTION`.

Las siguientes dimensiones métricas se capturan raspando el exportador y se vinculan a la entidad <DoNotTranslate>**MONGODB_INSTANCE**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="diagnostic-metrics"
    title="Diagnóstico métrico"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    id="oplog-metrics"
    title="Log de operaciones métrica"
  >
    <table>
      <thead>
        <tr>
  ...
  <Collapser
    ...
    title="Estado del servidor métrica"
  >
    <table>
      <thead>
        <tr>
...
Las siguientes dimensiones métricas se capturan raspando el exportador y se vinculan a la entidad <DoNotTranslate>**MONGODB_COLLECTION**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="collection-metrics"
    title="Estado de la colección métrica"
  >
    <table>
      <thead>
        <tr>
...
Las siguientes dimensiones métricas se capturan raspando el exportador y se vinculan a la entidad <DoNotTranslate>**MONGODB_DATABASE**</DoNotTranslate>:

<CollapserGroup>
  <Collapser
    id="dbstats-metrics"
    title="Base de datos Estado métrico"
  >
    <table>
      <thead>
        <tr>
...
## Migrar desde la versión legacy [#migrate]

Si está utilizando la versión [legacy](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration) , le recomendamos que pruebe esta nueva versión. Consulte la página [de migración](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-migration-guide) para obtener orientación.
...
Además, esta integración aprovecha un [exportador Prometheus creado por la comunidad](https://github.com/newrelic/newrelic-prometheus-exporters-packages/tree/main/exporters/mongodb3).
...
<CollapserGroup>
  <Collapser
    ...
    title="Prometheus se acaba el tiempo"
  >
    En algunas situaciones, cuando se debe informar una gran cantidad de métricas, la integración podría expirar al intentar eliminar el exportador de Prometheus. Se debería encontrar un error similar en el [archivo de registro](/docs/infrastructure/new-relic-infrastructure/troubleshooting/generate-logs-troubleshooting-infrastructure):
    ...
    Considere aumentar el valor predeterminado `scrape_timeout` para mitigar este problema. Para obtener más información sobre las opciones de configuración, consulte la [configuración](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration-new#instance-settings) de la instancia de MongoDB.
  </Collapser>
</CollapserGroup>