...
Nuestra integración de Redis informa datos de rendimiento críticos de su servidor [Redis](https://redis.io) a los productos New Relic. Puede ver estos [datos métricos](#metrics) y [datos de inventario](#inventory) en un dashboard prediseñado, crear políticas de alertas y crear consultas y gráficos personalizados. También puede especificar claves que sean importantes para su aplicación y obtener información sobre su longitud.
...
<CollapserGroup>
  ...
  <Collapser id="linux-install" title="Linux">
    ...
    4. Edite el archivo `redis-config.yml` según los métodos de conexión de su servidor Redis:
       ...
       <CollapserGroup>
         <Collapser id="socket-connect" title="Conectar con socket Unix">
         ...
         <Collapser id="tcp-connect" title="Conéctate con TCP">
...
<CollapserGroup>
  ...
  <Collapser id="basic-auth" title="Autenticación básica">
  ...
  <Collapser id="acl-auth" title="Autenticación ACL Redis 6+">
  ...
  <Collapser id="metrics-tls-connection" title="Sólo métrica con conexión TLS">
  ...
  <Collapser id="metrics-connect-socket" title="Conexión solo métrica sobre casquillo">
  ...
  <Collapser id="renamed-commands" title="Usar comandos renombrados">
  ...
  <Collapser id="multi-instance-keys-filtering" title="Monitoreo multiinstancia con filtrado de claves">
