...
Nuestra integración es compatible con PostgreSQL v17 y anteriores.
...
3. [Cree un usuario](#create-user) con licencias de `READ` en las funciones requeridas.

4. Copie el archivo de configuración de muestra ejecutando:

   ```shell
   sudo cp postgresql-config.yml.sample postgresql-config.yml
   ```

5. Edite el archivo de configuración `postgresql-config.yml` con su editor favorito. Vea algunos [ejemplos de archivos de configuración.](#examples).
...
<CollapserGroup>
  <Collapser id="example-postgresSQL-collection-config" title="Archivo de recopilación de configuración de PostgreSQL">
  ...
  <Collapser id="example-postgresSQL-SSL-config" title="Archivo de recopilación de configuración SSL de PostgreSQL">
    ```yml
    integrations:
      - name: nri-postgresql
        env:
  ...
  <Collapser id="example-postgresSQL-custom-query-config" title="Consulta personalizada de PostgreSQL">
    ```yml
    integrations:
      - name: nri-postgresql
        env:
  ...
  <Collapser id="example-postgresSQL-custom-query-config-file" title="Archivo de configuración de consulta personalizada de PostgreSQL">
    Se puede definir un archivo de configuración YAML adicional con uno o más SQL personalizados y la integración necesitará la ruta al archivo en el parámetro CUSTOM\_METRICS\_CONFIG.
...
<CollapserGroup>
  <Collapser id="databaseSample" title="PostgresqlBase de datosMuestra métrica">
    <table>
      <thead>
        <tr>
  ...
  <Collapser id="indexSample" title="PostgresqlIndexMuestra métrica">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="instanceSample" title="PostgresqlInstanciaMuestra métrica">
    <Callout variant="tip">
      Con PostgreSQL v17, cambiamos el nombre de algunas métricas para reflejar mejor sus tablas de origen. Puedes ver los nombres actualizados sangrados debajo de sus versiones anteriores.
    </Callout>
  ...
  <Collapser id="tableSample" title="PostgresqlTableMuestra métrica">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="pgBouncerSample" title="PgBouncerMuestra métrica">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
