...
Nuestra integración es compatible con cualquier servicio existente que se ajuste a la [API del complemento de Nagios](https://assets.nagios.com/downloads/nagioscore/docs/nagioscore/3/en/pluginapi.html).
...
<ButtonGroup>
  ...
  <ButtonLink role="button" to="https://onenr.io/07wkJ6D6oQL" variant="primary">
...
<CollapserGroup>
  <Collapser id="linux-permissions" title="Permisos de linux">
  ...
  <Collapser id="windows-permissions" title="Permisos de Windows">
...
<CollapserGroup>
  <Collapser
    ...
    title={<>Ejemplo de configuración <InlineCode>
      nagios-config.yml
    </InlineCode></>
    }
  >
...
<CollapserGroup>
  <Collapser id="config-parsing-failed" title="Error al analizar la configuración">
