...
1. Agregue su <InlinePopover type="licenseKey"/>al atributo `license_key` en su archivo `newrelic-infra.yml`, ubicado en `C:\Program Files\New Relic\newrelic-infra\`. Cuando haya terminado, su archivo `newrelic-infra.yml` debería tener el siguiente aspecto:

   ```yml
   license_key: YOUR_LICENSE_KEY
   ```

2. Para iniciar el servicio `newrelic-infra` , ejecute:
