---
title: Integración métrica de Azure
tags:
  - Integrations
...
Las métricas obtenidas del monitoreo de Azure API se almacenan en New Relic como [métricas dimensionales](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) siguiendo esta convención:

* Métricas tienen el prefijo de su tipo de recurso de Azure, todo en minúsculas, donde `/` se reemplaza por `.` y `Microsoft.` se reemplaza por `azure.`:
...
* El nombre de la métrica original de Azure mantiene su mayúscula original, sin espacios en blanco y tiene el prefijo del tipo de recurso, como se describe anteriormente:
...
Si el nombre de la métrica resultante no comienza con `azure.` después de estas transformaciones, el prefijo se antepone adicionalmente.

Para obtener más información sobre los tipos de recursos de Azure admitidos, consulte el [sitio web de documentación sobre monitoreo de Azure métrica](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/supported-metrics/metrics-index).

### El nombre de la métrica coincide con uno de los nombres de las dimensiones.

Cuando el nombre de la métrica y el nombre de una de las dimensiones son exactamente iguales, el nombre de esta dimensión no está en mayúscula, por lo que ambos valores son diferentes:

* `azure.network.applicationgateways.TlsProtocol` con dimensiones `azure.network.applicationgateways.tlsprotocol`

### Métrica con muchas combinaciones de dimensiones.

Una definición de métrica incluye tanto su nombre como las diferentes dimensiones. A veces, el mismo nombre métrico aparece en muchas métricas con diferentes combinaciones de dimensiones, por ejemplo:

* `azure.cache.redis.percentProcessorTime` con dimensiones `azure.cache.redis.ShardId`
* `azure.cache.redis.percentProcessorTime` sin dimensiones.
...
Coincide únicamente con la métrica sin la dimensión ShardId:

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NULL
```

Solo coincide con la métrica con la dimensión:

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NOT NULL
```

## API Polling métrica [#azure-metrics-table]

Para obtener una referencia sobre las métricas disponibles de cada una de las integraciones de encuestas y sus nombres, [consulte nuestro documento sobre la integración individual](/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).

La siguiente tabla es una lista no exhaustiva de las métricas recopiladas por la integración Azure Polling y sus traducciones [métricas dimensionales](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) .
...
<table>
  <tbody>
    ...
    <tr>
      <td>
        Conjuntos de escalado de máquina virtual (VM) de Azure
      </td>
