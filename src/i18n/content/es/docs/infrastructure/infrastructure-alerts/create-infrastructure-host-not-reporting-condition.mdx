...
2. Seleccione <DNT>**Host not reporting**</DNT> como <DNT>**Alert type**</DNT>.

3. Defina el umbral <DNT>**Critical**</DNT> para activar la notificación: mínimo 5 minutos, máximo 60 minutos.

4. Habilite la opción <DNT>**Don&apos;t trigger alerts for hosts that perform a clean shutdown**</DNT> si desea evitar alertas falsas cuando tenga hosts configurados para apagar mediante la línea de comando. Actualmente, esta función es compatible con todos los sistemas Windows y Linux que emplean systemd. <Callout variant="tip">Además, puede agregar la [etiqueta](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) `hostStatus: shutdown` a su host junto con la marca de la opción mencionada anteriormente. Esto evitará que se abran todos los incidentes <DNT>**Host not reporting**</DNT> para ese host, siempre que tenga esa etiqueta, independientemente de la versión del agente o del sistema operativo. Quitar la etiqueta permitirá que el sistema vuelva a abrir el incidente <DNT>**Host not reporting**</DNT> para ese host.</Callout>

   <Callout variant="tip">
     Como alternativa a la [etiqueta](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data/) `hostStatus: shutdown` , puede agregar la etiqueta `termination: expected` a su entidad host. Esto le indica a New Relic que se esperaba que el host se apagara y evitará que cualquier **Host not reporting** incidentes se abra siempre que la opción **Don&apos;t trigger alerts for hosts that perform a clean shutdown** esté seleccionada en la condición.
   </Callout>
