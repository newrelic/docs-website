...
Al aplicar filtros a los hosts y procesos que son importantes para su negocio, puede definir [umbrales de alerta](/docs/alerts/overview#alert-threshold) para decidir cuándo se abren los eventos de alerta y New Relic le envía una notificación por correo electrónico según las [preferencias de eventos de alerta](/docs/alerts/organize-alerts/specify-when-alerts-create-events) de la política. Estos ejemplos ilustran cómo utilizar la condición <DNT>**Process running**</DNT> de monitoreo de infraestructura para monitorear sus procesos.

<CollapserGroup>
  <Collapser id="processes-load" title="Asegúrese de que se estén ejecutando suficientes procesos para satisfacer la carga.">
    <DNT>**Problem:**</DNT> Algunos balanceadores de carga y servidores de aplicaciones funcionan ejecutando muchos procesos de trabajo en paralelo. Aquí, por ejemplo, es posible que desee un evento de alerta cuando se estén ejecutando menos de ocho procesos para un servicio como gunicorn.
  ...
  <Collapser id="one-critical" title="Monitor el inicio para detectar procesos críticos que requieren atención especial">
    ...
    <DNT>**Solution:**</DNT> Usa el umbral <DNT>**At least one process is running**</DNT> con filtros de condición configurados para un nombre de usuario y un ejecutable específico, para que New Relic pueda abrir un evento de alerta cuando el proceso se esté ejecutando.
  </Collapser>

  <Collapser id="one-job-length" title="Asegúrate de que un trabajo no demore demasiado">
    <DNT>**Problem:**</DNT> Tiene un trabajo que se ejecuta periódicamente y desea abrir un evento de alerta cuando ha estado ejecutándose por más tiempo que el número de minutos esperado.
...
Si crea la condición de alerta directamente con el monitoreo de infraestructura, New Relic enviará una notificación por correo electrónico cuando se supere el umbral definido para la condición de alerta, según las [preferencias de eventos de alerta](/docs/alerts/organize-alerts/specify-when-alerts-create-events) de la política. Su política de alertas define qué personal o equipos y qué canales de notificación utilizamos.