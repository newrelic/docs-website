...
3. Ajuste las URL para las cuentas de la UE, JP o FedRAMP si es necesario (consulte los comentarios en el archivo).
...
1. Usamos `dogstatsd_addr` aquí para poder agregar la etiqueta `consul.source.datacenter` a cada métrica. Esto le permite filtrar su dashboard por Consul Datacenter.
