...
* Asegúrese de que se haya seleccionado el centro de datos correcto, EE. UU. (predeterminado) o UE, para su cuenta de New Relic. Consejo: si la license\_key comienza con &quot;eu&quot;, entonces debe usar el indicador `NR_EU_REGION=true`.
* Para la región JP, el extremo se selecciona automáticamente en función de su clave de licencia, por lo que no se requiere ningún indicador adicional.
* Asegúrese de que no haya [`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/) relacionados con la integración de StatsD.
