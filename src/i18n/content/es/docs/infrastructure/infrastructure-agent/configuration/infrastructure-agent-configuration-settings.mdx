...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="logging-http-client-timeout" title="logging_http_client_timeout">
    Establece el tiempo de espera del cliente HTTP (en segundos) para el plugin de salida fluent-bit de New Relic. Esto controla cuánto tiempo esperará el reenviador de logging una respuesta al enviar datos de log a New Relic.

    Para obtener más información, consulte la [documentación del plugin de salida fluent-bit](https://github.com/newrelic/newrelic-fluent-bit-output#httpclienttimeout).
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="safe-bin-dir" title="safe_bin_dir">
    Directorio donde el agente busca ejecutables para las integraciones.
  ...
  <Collapser className="freq-link" id="agent-temp-dir" title="agent_temp_dir">
    Directorio donde el agente almacena archivos temporales (por ejemplo, configuración del reenviador de logs y datos de descubrimiento de integración). Este directorio se elimina en cada reinicio del agente, pero solo si aún coincide con su valor predeterminado.
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            Ventanas: `C:\ProgramData\New Relic\newrelic-infra\tmp`
          </td>
  ...
  <Collapser className="freq-link" id="logging-configs-dir" title="logging_configs_dir">
    Directorio que contiene archivos de configuración para el reenviador de logs.
  ...
  <Collapser className="freq-link" id="logging-home-dir" title="logging_home_dir">
    Directorio que contiene binarios y otros archivos necesarios para el reenviador de logs.
  ...
  <Collapser className="freq-link" id="logging-bin-dir" title="logging_bin_dir">
    Directorio que contiene binarios para el reenviador de logs.
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="http-server-cert" title="http_server_cert">
    La ruta a un certificado codificado en PEM utilizado para escuchar las cargas útiles de integración a través de HTTPS. Cuando se configura con `http_server_key`, el agente utiliza HTTPS para el servidor de carga útil de integración.
  ...
  <Collapser className="freq-link" id="http-server-key" title="http_server_key">
    La ruta a una clave codificada en PEM utilizada para escuchar las cargas útiles de integración a través de HTTPS. Cuando se configura con `http_server_cert`, el agente utiliza HTTPS para el servidor de carga útil de integración.
  ...
  <Collapser className="freq-link" id="http-server-ca" title="http_server_ca">
    La ruta a un certificado CA codificado en PEM utilizado para aplicar la validación del certificado del cliente para las solicitudes HTTPS. Cuando se configura, el agente verifica los certificados del cliente contra esta CA para habilitar la autenticación mutua TLS (mTLS) para el servidor de carga útil de integración.
