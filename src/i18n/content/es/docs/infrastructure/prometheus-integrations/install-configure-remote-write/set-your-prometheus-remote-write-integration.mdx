...
1. Introduzca un nombre para el servidor Prometheus que se conectará y su URL `remote_write` . <DNT>**Important:**</DNT> El nombre que ingreses para el servidor creará un atributo en tus datos. También será el nombre que identifique qué servidor Prometheus está enviando datos a New Relic.
...
2. Agregue una nueva URL `remote_write` a su archivo YML de Prometheus. Agregue esta información debajo de `global_config` en el archivo, en el mismo nivel de sangría que la sección `global` .
   ...
   <DNT>**European Union accounts:**</DNT> Si se conecta desde la UE, utilice la siguiente URL:
