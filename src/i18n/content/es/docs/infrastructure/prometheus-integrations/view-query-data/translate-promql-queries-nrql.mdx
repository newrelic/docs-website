...
<Callout variant="tip">
  Para ejecutar una consulta estilo PromQL en [New Relic One](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one), vaya al [modo avanzado estilo PromQL del generador de consultas](/docs/query-your-data/explore-query-data/chart-builder/use-advanced-promql-mode-specify-data).
</Callout>
...
<Callout variant="tip">
  Para ver cómo New Relic traduce la consulta estilo PromQL a NRQL, escriba una consulta en la [pestaña estilo PromQL del generador de consultas](/docs/query-your-data/explore-query-data/chart-builder/use-advanced-promql-mode-specify-data) y luego cambie a la [pestañaNRQL ](/docs/chart-builder/use-chart-builder/choose-data/use-advanced-nrql-mode-specify-data).
</Callout>
...
<CollapserGroup>
  <Collapser
    id="prom-ql-example"
    title="Ejemplo de consulta estilo PromQL"
  >
    <DoNotTranslate>
      **1. Start your query.**
    </DoNotTranslate>
    ...
    Esto trazará todas las series temporales métricas únicas para la entrada métrica.

    <DoNotTranslate>
      **2. Filter the query results.**
    </DoNotTranslate>

    Al observar los datos, puede agregar más parámetros de consulta para filtrar el número de series temporales métricas. Por ejemplo, si solo desea series temporales donde `id` sea `/`, la consulta de estilo PromQL será:
    ...
    <figcaption>
      <DoNotTranslate>**PromQL-style example:**</DoNotTranslate> Para filtrar los datos, ejecute esta consulta estilo PromQL: container_memory_usage_bytes&#x7B;id="/".
    </figcaption>
  </Collapser>

  <Collapser
    id="nrql-query-example"
    title="Ejemplo de consulta NRQL"
  >
    <DoNotTranslate>
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DoNotTranslate>: Este ejemplo muestra los datos que ves después de ejecutar FROM métrica SELECT keyset() WHERE metricName = 'container_memory_usage_bytes'.
    </figcaption>
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DoNotTranslate>: Este ejemplo muestra los datos que ves después de ejecutar FROM métrica SELECT latest(container_memory_usage_bytes) FACET id.
    </figcaption>
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DoNotTranslate>: Este ejemplo muestra los datos que se muestran después de ejecutar `From Metric select latest(container_memory_usage_bytes) where id = "/" timeseries`.
    </figcaption>
  </Collapser>
</CollapserGroup>
...
Tanto nuestro lenguaje de consulta estilo PromQL como NRQL proporcionan sintaxis para filtrar el número de series temporales métricas únicas.
