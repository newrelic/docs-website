---
title: Características de PromQL admitidas
tags:
  - Integrations
...
New Relic admite consultas estilo PromQL, y nuestro generador de consultas ofrece un modo de consulta estilo PromQL que traduce las consultas con sintaxis PromQL a la aproximación NRQL más cercana. Aunque el método de aproximación implica que algunos casos extremos no se admiten por completo, proporciona cobertura para una inmensa mayoría de consultas, admitiendo más del 99.5 % de las consultas en los 7.8 millones de descargas principales de dashboards de Grafana.

Aprenda cómo trabajamos con las consultas PromQL, así como las diferencias entre el PromQL estándar y nuestro lenguaje de consulta similar a PromQL.
...
<CollapserGroup>
  ...
  <Collapser title="Funciones de ordenamiento">
  ...
  <Collapser title="Funciones de utilidad">
    * [`label_join()`](https://prometheus.io/docs/prometheus/latest/querying/functions/#label_join)
  </Collapser>

  <Collapser title="Características específicas de Grafana">
    La lógica de traducción también tiene un soporte limitado para la sintaxis de grafana que no es parte del lenguaje promQL.

    * [`label_values()`](https://grafana.com/docs/grafana/latest/datasources/prometheus/template-variables/#use-query-variables)
  </Collapser>
</CollapserGroup>
...
Sin embargo, debido a que NRDB convierte los contadores acumulativos estilo PromQL a contadores `delta`, nuestra implementación no maneja estas funciones en el tipo de datos incorrecto y producirá respuestas diferentes o incorrectas.
...
* Para asegurar la estabilidad y el rendimiento de nuestro sistema para todos los usuarios, establecemos algunos límites sobre qué consultas pueden ejecutarse. En todos los casos, aplicamos un límite de 366 pasos en las consultas de rango. También devolvemos solo 100 series temporales de consultas por defecto.
