---
title: Página UI usuario de inventario de infraestructura
tags:
  - Infrastructure
...
New Relic puede recopilar información detallada sobre la configuración de un sistema por host, incluidos módulos del sistema, archivos de configuración, metadatos, paquetes, servicios, sesiones de usuario y más. La página <DoNotTranslate>**Inventory**</DoNotTranslate> proporciona una vista en tiempo real, filtrable y con capacidad de búsqueda de la configuración de cada host.

Utilice la página <DoNotTranslate>**Inventory**</DoNotTranslate> para:
...
Para ver y buscar sus datos de inventario: Vaya a <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Inventory**</DoNotTranslate>.
...
Utilice <DoNotTranslate>**Inventory**</DoNotTranslate> funciones de página para buscar información sobre un elemento particular en sus hosts:
...
<CollapserGroup>
  ...
  >
    ...
    Por ejemplo, si desea encontrar información relacionada con OpenSSL, busque `openssl`. El término de búsqueda vuelve a coincidir con el nombre del artículo del inventario.
  </Collapser>

  <Collapser
    id="view"
    title="Ver detalles del artículo del inventario"
  >
    Los detalles del elemento de inventario proporcionan información del host y del sistema para cada host en el que reside de acuerdo con el [nombre del elemento de inventario](#item-naming) de New Relic. Si tiene diferentes versiones del mismo elemento en otros hosts, New Relic lo detecta y las marca en la página de Inventario con la etiqueta <DoNotTranslate>**variant hosts**</DoNotTranslate> y enumera cada host que ejecuta cada versión.
  ...
  <Collapser
    id="alerts"
    title="Ver incidente alerta umbral del anfitrión"
  >
    Para ver [el incidente umbral de alerta](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-threshold) de uno o más hosts, seleccione el <DoNotTranslate>**Critical**</DoNotTranslate> del host <Icon name="fe-x-circle"/> icono o <DoNotTranslate>**Warning**</DoNotTranslate> <Icon name="fe-alert-triangle"/> icono.
  </Collapser>
</CollapserGroup>

## Recopilación de datos de inventario [#data-collection]

El inventario é coletado a partir del colector de datos integrado del agente de infraestructura, la integración de Amazon Elastic Compute Cloud (EC2), la integración del agente proporcionada por New Relic y la integración creada por los clientes. Los datos aparecen en la página <DoNotTranslate>**Inventory**</DoNotTranslate> y en otros lugares de nuestra UI de infraestructura.

<CollapserGroup>
  <Collapser
    id="linux"
    title="Datos del agente integrado de Linux"
  >
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Metadatos del agente de lenguaje APM
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            display_name del agente, proveedor de nube `instance-id, os.Hostname(), hostname -f, hostname`
          </td>
        </tr>
  ...
  <Collapser
    id="windows"
    title="Datos del agente integrado de Windows"
  >
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Display_name agente, proveedor de la nube instance-id, os.Hostname(), Registry (SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters (Domain, DhcpDomain, Hostname)
          </td>
        </tr>
  ...
  <Collapser
    id="cloud"
    title="Inventario de integración AWS en la nube"
  >
...
Para obtener una explicación técnica sobre el atributo utilizado para completar la página <DoNotTranslate>**Inventory**</DoNotTranslate>, consulte [Evento y atributo de infraestructura predeterminados](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#shared-attributes). Esto incluye un resumen de [eventos comunes por sistema operativo](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/default-infrastructure-attributes-events#events-os).

## Juegos de filtros [#filter-sets]

El componente UI del conjunto de filtros se utiliza para filtrar sus hosts y crear vistas guardadas. <DoNotTranslate>**It is only available on the Events and Inventory UI pages and is in the process of being deprecated.**</DoNotTranslate> Ha sido reemplazada por la característica [de vistas guardadas](/docs/infrastructure/infrastructure-ui-pages/infra-hosts-ui-page#ui-features) . Cualquier conjunto de filtros que cree a partir de este momento no se migrará a la característica de vistas guardadas.

<CollapserGroup>
  <Collapser
    id="filter-set"
    title="Cómo utilizar conjuntos de filtros"
  >
    ...
    * Entornos de prueba
    ...
    El conjunto de filtros de infraestructura predeterminado es <DoNotTranslate>**All hosts**</DoNotTranslate> y sirve como plantilla para crear conjuntos de filtros.
    ...
    2. Si
       ...
       <DoNotTranslate>**All hosts**</DoNotTranslate>

       no se muestra en la barra lateral izquierda, abra ese conjunto de filtros seleccionando

       <DoNotTranslate>**Saved filter sets > All hosts**</DoNotTranslate>
    ...
    3. En
       ...
       <DoNotTranslate>**Filter hosts**</DoNotTranslate>
    ...
    5. Haga clic en
       ...
       <DoNotTranslate>**Include**</DoNotTranslate>

       o

       <DoNotTranslate>**Exclude**</DoNotTranslate>

       (consulte [Lógica del conjunto de filtros](#logic)).
    ...
    8. Para nombrar su filtro, haga clic en
    ...
    1. En la barra lateral, haga clic en

       <DoNotTranslate>**Saved filter sets**</DoNotTranslate>
    ...
    <CollapserGroup>
      <Collapser
        id="match-string"
        title="Recomendado: seleccione valores haciendo coincidir una cadena"
      >
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                Si hace clic en <DoNotTranslate>**Include**</DoNotTranslate> y luego ingresa una cadena con la que desea que coincidan los valores, el filtro usa el comparador `LIKE`, lo que significa que los resultados incluyen cualquier entidad que sea similar a la cadena.
            ...
            <tr>
              ...
              <td>
                Si hace clic en <DoNotTranslate>**Exclude**</DoNotTranslate> y luego ingresa una cadena con la que desea que coincidan los valores, el filtro utiliza el comparador `NOT LIKE`, lo que significa que los resultados excluyen cualquier entidad que sea similar a la cadena.
      ...
      <Collapser
        ...
        title="Seleccionar valores individualmente"
      >
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td>
                Si hace clic en <DoNotTranslate>**Include**</DoNotTranslate> y luego hace clic en valores específicos, el filtro utiliza el comparador `IN`, lo que significa que el filtro busca entidades que coincidan exactamente con uno o más valores en su lista de selecciones.
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                Si hace clic en <DoNotTranslate>**Exclude**</DoNotTranslate> y luego hace clic en valores específicos, el conjunto de filtros utiliza el comparador `NOT IN`, lo que significa que el filtro devuelve todas las entidades que no coinciden exactamente con uno o más valores en su lista de selecciones.
              </td>
            </tr>
          </tbody>
        </table>
