...
* **Exigences système SAP**: Vérifiez que vos systèmes SAP répondent aux niveaux de version des composants logiciels requis, comme spécifié par New Relic Monitoring for SAP Solutions. Pour plus de détails sur les fournisseurs de données disponibles par type de système, consultez le [Référence des fournisseurs de données](/docs/sap-solutions/additional-resources/data-providers-reference).
...
* **Télécharger le rôle d’autorisation** [`NEWR_DATA_PROVIDER`](/docs/sap-solutions/additional-resources/additional-configuration/#upload-authorization-role).

* Créez un utilisateur système avec le rôle téléchargé pour les connexions RFC. Reportez-vous à [Créer l’ID utilisateur RFC New Relic](/docs/sap-solutions/additional-resources/additional-configuration/#create-rfc-user-id).

* Si vous monitorez une **base de données HANA**, créez un **utilisateur NRAGENT de base de données HANA** avec accès **Catalog Read** et **Monitoring** à l&apos;aide des commandes SQL suivantes :
...
* Modifiez les politiques de pare-feu pour autoriser le trafic HTTPS vers New Relic s&apos;il est bloqué.
  ...
  <Callout variant="tip">
    Ces connexions peuvent être définies dans le **Central Monitoring System** à l&apos;aide de la **SAP Transaction SM59** et testées. Reportez-vous à la section [Définir la destination HTTP vers New Relic Cloud](/docs/sap-solutions/additional-resources/additional-configuration/#set-http-destination-to-new-relic-cloud) pour plus de détails.
  </Callout>

* Pour chaque système monitoré spécifié, utilisez la transaction `SM59` et l&apos;ID du système distant pour créer une **RFC Destination** et tester les connexions. Reportez-vous à la section [Créer des destinations RFC pour les systèmes monitorés](/docs/sap-solutions/additional-resources/additional-configuration/#create-rfc-for-monitored-systems) pour plus de détails.
...
* Assurez-vous des éléments suivants pour monitoring de la base de données HANA distante :
  ...
  * Pour chaque base de données HANA distante, utilisez la transaction `SAP DBACOCKPIT` pour créer une connexion de base de données. Reportez-vous à [Créer une connexion de base de données](/docs/sap-solutions/additional-resources/additional-configuration/#create-database-connection) pour plus de détails.
...
Les clés de service sont également utilisées pour créer des [Destinations HTTP externes](/docs/sap-solutions/additional-resources/additional-configuration/#create-authentication-endpoint) et servent d&apos;hôte d&apos;API pour les fournisseurs de données d&apos;API.