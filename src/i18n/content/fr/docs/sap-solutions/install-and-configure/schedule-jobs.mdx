...
Planifiez trois tâches de collecte de données avec des variantes axées sur les groupes de fournisseurs de données suivants. Pour obtenir des instructions sur la planification des tâches du gestionnaire de collection avant `SP03`, consultez [Variantes et tâches avant SP03](/docs/sap-solutions/additional-resources/variants-and-jobs-before-sp03). Séparez les fournisseurs de données en trois groupes pour la planification.

* **TRACE** : Dans ce groupe se trouvent les fournisseurs de données liés à la base de données de performance (STAD) :

  * Inclut les fournisseurs de données de base de performances tels que `NR.SAP.TRANSACTION` (fournisseur de données trace ), `NR.SAP.TRANSACTION.COMPLETE` (collecte complète de données de base de performances avec processus RFC d&apos;arrière-plan parallèle), `NR.SAP.SYSTEM.PROCESS`, `NR_SAP_PROGRAM`, `NR_SAP_WORKLOAD_COMPLETE` et `NR.SAP.WORKLOAD.COMPLETE`.
...
* **IDOC** : Dans le système de production, la collecte des IDOC peut prendre un temps important :
  * Inclut les fournisseurs liés à IDOC tels que `NR_SAP_IDOC` (fournisseur principal de données d&apos;événement d&apos;état IDOC), `NR_SAP_IDOC_COMPLETE` (récupération complète d&apos;IDOC avec collecte BG RFC) et `NR_SAP_IDOC_Configuration` (téléchargement des données principales IDOC pour améliorer les performances du fournisseur de données IDOC).

* **SYSTEM** : Reste des fournisseurs de données.
