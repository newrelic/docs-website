...
* Accès au [**dernier** runtime Chrome](/docs/synthetics/synthetic-monitoring/using-monitors/manage-monitor-runtimes/#browserdependencies), afin que vos moniteurs de navigateur scriptés utilisent des versions de navigateur comme celles utilisées par vos utilisateurs finaux.
...
4. Utilisez la liste déroulante du champ <DNT>**Runtime**</DNT> pour changer la vue d&apos;exécution actuelle. Sélectionnez le dernier runtime pris en charge pour votre type de moniteur. La vue change selon que le moniteur est un moniteur d&apos;API ou un moniteur de navigateur. Pour les versions actuellement prises en charge, voir [Passer en revue les dépendances d&apos;exécution pour monitorer le navigateur](/docs/synthetics/synthetic-monitoring/using-monitors/manage-monitor-runtimes/#browserdependencies).
