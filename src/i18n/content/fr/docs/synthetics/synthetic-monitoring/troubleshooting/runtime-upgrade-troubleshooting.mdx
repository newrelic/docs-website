...
<Callout variant="tip">
  Si vous monitorez sur des sites privés, consultez le [guide de transition du runtime de site privé](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) pour les étapes de déploiement de SJM et le dépannage spécifique au site privé.
</Callout>
...
Assurez-vous que toutes les promesses ont `.then()`, `.catch()` ou sont `await`ed.
...
Attendez correctement la réponse en utilisant async/await :
...
### Connexions HTTP non fermées [#unclosed-http]

**Symptôme :** La validation du monitor échoue avec des erreurs d&apos;open handle **Cause :** Connexions HTTP effectuées dans des boucles sans attente ni nettoyage
...
Attendre et gérer correctement les connexions :
...
Lors de la validation d&apos;un monitorer créé dans un runtime plus ancien sur le [runtime Chrome**le plus récent** ](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100), `findElement` et d&apos;autres méthodes pour trouver et interagir avec les éléments de la page peuvent échouer en raison de différences dans la gestion des promesses. Si le moniteur réussit dans un runtime legacy, échoue dans le nouveau runtime, et que l&apos;élément est présent dans la capture d&apos;écran, améliorez votre logique de gestion des promesses.
