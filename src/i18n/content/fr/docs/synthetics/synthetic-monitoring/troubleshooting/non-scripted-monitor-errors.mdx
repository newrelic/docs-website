...
<CollapserGroup id="non-scripted-errors">
  ...
  <Collapser className="freq-link" id="networkerror-host-unreachable" title="NetworkError : Aucune route vers l'hôte (Hôte inaccessible)">
    ...
    Si l&apos;échec se produit sur un emplacement privé de monitoring synthétique, assurez-vous que la configuration réseau du Synthetics Job Manager est correctement configurée et que le nom d&apos;hôte cible est résoluble et accessible via l&apos;interface de ligne de commande virtuelle du Synthetics Job Manager.
  ...
  <Collapser className="freq-link" id="networkerror-resolution" title="NetworkError : échec de la résolution DNS pour l'hôte : (HOST)">
    ...
    <DNT>**Private synthetic monitoring’s locations:**</DNT> Vérifiez que la configuration réseau du Synthetics Job Manager est correcte. Si le nom d&apos;hôte cible est interne, assurez-vous que le Synthetics Job Manager utilise le service DNS interne de votre réseau capable de résoudre l&apos;hôte. Le Synthetics Job Manager et les conteneurs de runner qu&apos;il génère sur l&apos;hôte (pour exécuter des checks non-ping) doivent hériter de la configuration DNS de l&apos;hôte `/etc/resolv.conf`.

    <DNT>**Docker:**</DNT> Les arguments réseau tels que `–dns` ou `-network` utilisés dans la commande Docker run sur le Synthetics Job Manager ne seront utilisés que par le conteneur du gestionnaire de tâches, et non par les conteneurs de runner. Si le DNS pointe vers l&apos;interface de bouclage telle que `127.0.0.1`, définissez une configuration DNS au [niveau du daemon Docker](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-configuration-file) ou utilisez un outil comme `dnsmasq` pour vous assurer que le runner transmettra les requêtes DNS sur l&apos;interface bridge Docker.
    ...
    Si vous constatez des échecs de moniteur liés à la résolution DNS sur un emplacement privé de monitoring synthétique, cela est souvent dû à une configuration réseau non valide du Synthetics Job Manager pour cet emplacement.
  </Collapser>
