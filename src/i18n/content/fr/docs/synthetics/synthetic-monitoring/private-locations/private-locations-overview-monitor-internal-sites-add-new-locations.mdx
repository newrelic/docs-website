...
En monitoring synthétique, un emplacement privé est un ensemble de gestionnaires de tâches synthétiques. Les gestionnaires de tâches Synthetics sont des applications conteneurisées qui reçoivent et gèrent les tâches configurées via l&apos;interface utilisateur Synthetics.

Un emplacement privé peut contenir n&apos;importe quel nombre de gestionnaires de tâches Synthetics. Les emplacements privés vous permettent d&apos;étendre votre couverture de monitoring synthétique à de nouveaux emplacements géographiques et de monitorer des sites Web situés derrière votre pare-feu (comme un site intranet).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Pour les comptes basés aux États-Unis (par défaut) : `https://synthetics-horde.nr-data.net/`
        ...
        Pour les comptes basés au Japon : `https://synthetics-horde.jp.nr-data.net/`
      </td>
    </tr>
    ...
    <tr id="compatibility-outbound-network">
      ...
      <td>
        Le gestionnaire de tâches Synthetics doit se connecter au point de terminaison horde du monitoring synthétique pour recevoir et traiter des tâches. Si vos règles de pare-feu n&apos;autorisent pas l&apos;accès direct, vous devez [configurer l&apos;accès par proxy](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager/#FAQ).
...
Avant d&apos;[installer des gestionnaires de tâches Synthetics](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager), vous devez créer un emplacement privé.
...
4. Facultatif : configurez ces paramètres supplémentaires :
   ...
   * <DNT>**[Verified script execution](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations)**</DNT>: Exiger une phrase secrète lors de l&apos;affectation de scripts à cet emplacement, ou lors de l&apos;ajout de gestionnaires de tâches à cet emplacement.

5. Sélectionnez <DNT>**Create**</DNT>.

Une fois l&apos;emplacement créé, le monitoring synthétique répertorie votre <DNT>**Private location key**</DNT> dans l&apos;onglet <DNT>My location</DNT> pour que vous puissiez [installer un gestionnaire de tâches](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager).
...
Chaque gestionnaire de tâches Synthetics peut exécuter environ 75 vérifications de moniteur ping par minute. Si la file d&apos;attente des jobs pour un emplacement particulier s&apos;allonge, [ajoutez des gestionnaires de jobs supplémentaires](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager). La capacité d&apos;un gestionnaire de travaux peut varier en fonction des performances du réseau et des ressources matérielles.
...
Le monitoring synthétique inclut des outils pour gérer les emplacements et les gestionnaires de tâches synthétiques. Vous pouvez également [installer des gestionnaires de tâches Synthetics](/docs/synthetics/synthetic-monitoring/private-locations/install-job-manager) et vider la file d&apos;attente des tâches si elle est encombrée.
...
<CollapserGroup>
  <Collapser id="clear-job-queue" title="Vider la file d'attente des tâches">
    ...
    Ceci est utile lorsque le nombre de tâches planifiées augmente plus rapidement que les gestionnaires de tâches ne peuvent les traiter, par exemple lorsque le gestionnaire de tâches est hors ligne.
  </Collapser>
  ...
  <Collapser id="view-status" title="Afficher le statut">
    Pour afficher le statut d&apos;un gestionnaire de jobs Synthetics individuel, sélectionnez le nom de l&apos;emplacement parent dans la liste.

    L&apos;[indicateur d&apos;état de santé](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#health-status) vert identifie les gestionnaires de tâches Synthetics actifs.
  </Collapser>
  ...
  <Collapser id="delete-location" title="Supprimer un emplacement">
    ...
    Cela n&apos;arrête aucun gestionnaire de jobs affecté à cet emplacement. Les gestionnaires de tâches doivent être arrêtés manuellement ou réaffectés.
  </Collapser>
  ...
  <Collapser id="enable-disable-vse" title="Activer ou désactiver l'exécution de script vérifiée">
    ...
    * Vous devez passer la variable d&apos;environnement [VSE\_PASSPHRASE](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-configuration/#environment-variables) au gestionnaire de tâches. Ensuite, dans l&apos;onglet <DNT>**Private locations**</DNT>, cliquez sur <DNT>**Edit**</DNT> et, dans le menu, activez l&apos;option <DNT>**Verified script execution**</DNT>.
  </Collapser>

  <Collapser id="rotate-private-location-keys" title="Faire tourner les clés privées du site">
    Les emplacements privés utilisent une clé d&apos;emplacement privé. Ce jeton permet au gestionnaire de tâches de l&apos;emplacement privé de récupérer les tâches à exécuter et de renvoyer le résultat à la plateforme New Relic.
    ...
    <Steps>
      <Step>
        ...
        5. Mettez à jour vos gestionnaires de tâches avec le nouvel emplacement privé. La section <DNT>**Configure and deploy**</DNT> inclut un exemple de code :
