...
## Considérations de dimensionnement pour Docker, Kubernetes et OpenShift [#kubernetes-sizing]
...
**L&apos;équation :** $$C\_req = N\_m \times F\_j \times D\_j$$

* $C\_req$ = **Cœurs de processeur requis**
* $N\_m$ = Le **nombre** total de moniteurs lourds que vous prévoyez d&apos;exécuter.
* $F\_j$ = La **fréquence** moyenne du moniteur en tâches par minute (par exemple, un moniteur exécuté toutes les 5 minutes a une fréquence de 1/5 ou 0,2).
* $D\_j$ = La **durée moyenne** d&apos;un travail en minutes.
