    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Pour les comptes basés aux États-Unis (par défaut), le point de terminaison est : `https://synthetics-horde.nr-data.net.`
            ...
            Pour les comptes basés au Japon, le point de terminaison est : `https://synthetics-horde.jp.nr-data.net/`
...
Des modules de nœud personnalisés sont fournis dans le SJM. Ils vous permettent de créer un ensemble personnalisé de [modules Node](https://docs.npmjs.com/about-packages-and-modules) et de les utiliser dans des moniteurs scriptés (API scriptée et navigateur scripté) pour le monitoring synthétique.
...
## Configurer des certificats d&apos;autorité de certification personnalisés pour les sites privés [#custom-ca-certificates]

Si vos moniteurs Synthétiques doivent se connecter à des points de terminaison HTTPS internes sécurisés par une autorité de certification (CA) privée, vous pouvez monter vos certificats d&apos;autorité de certification personnalisés directement dans le SJM.

Le SJM détecte automatiquement ces certificats et les transmet à chaque conteneur d&apos;exécution au démarrage. Cela garantit que vos moniteurs font confiance à vos points de terminaison internes sans vous obliger à créer ou à maintenir des images d&apos;exécution personnalisées.

### Exigences relatives aux certificats

Pour vous assurer que le SJM charge vos certificats avec succès, ils doivent répondre aux critères suivants :

* **Format :** Les certificats doivent être au format PEM avec une extension de fichier `.pem`.
* **Permissions :** les fichiers doivent être lisibles par l&apos;UID 2000 (l&apos;utilisateur non root qui exécute le conteneur d&apos;exécution).
* **Type de certificat :** seuls les certificats CA sont chargés. Tous les certificats feuilles (certificats de serveur ou de client) qui ne disposent pas de l&apos;extension de contrainte de base `CA:TRUE` sont automatiquement ignorés.

### Environnements de déploiement

Sélectionnez votre plateforme d&apos;orchestration de conteneurs ci-dessous pour monter vos certificats.

<CollapserGroup>
  <Collapser id="custom-ca-docker" title="Docker">
    Montez le répertoire de l&apos;hôte local contenant vos fichiers PEM sur le chemin du conteneur SJM `/var/lib/newrelic/synthetics/certs` en tant que volume en lecture seule.
    ...
    **Comment cela fonctionne :** le SJM détecte ce montage au démarrage et propage les certificats en lecture seule à chaque conteneur dynamique qu’il crée.
  </Collapser>

  <Collapser id="custom-ca-podman" title="Podman">
    Pour les environnements Podman — en particulier ceux exécutés avec SELinux activé —, vous devez ajouter l&apos;indicateur `:z` ou `:Z` au montage de volume pour configurer le contexte de sécurité correct.
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes (Helm)">
    Lorsque vous déployez SJM sur un cluster Kubernetes via Helm, vous pouvez gérer votre bundle d&apos;autorités de certification personnalisées en tant que secret Kubernetes générique.

    **1. Créez le secret Kubernetes**

    Stockez votre (vos) fichier(s) de certificat PEM dans un secret au sein de l&apos;espace de nommage de votre agent New Relic :
    ...
    **2. Mettre à jour vos valeurs Helm**

    Référencez le secret nouvellement créé dans votre configuration `values.yaml` personnalisée :
    ...
    **Comment cela fonctionne :** au démarrage, le SJM monte ce secret et le transmet automatiquement à tous les pods d&apos;exécution nouvellement générés.
  </Collapser>
</CollapserGroup>

## Certificats client mTLS [#mtls-client-certificates]

Si vos monitorers se connectent à des points de terminaison internes qui appliquent le protocole TLS mutuel (mTLS) — exigeant que le client présente un certificat —, vous pouvez fournir des certificats clients modélisés sur les noms d’hôte cibles. Les monitorers présentent le certificat correct automatiquement. Aucune modification de script n’est requise.

**Exigences:**

* Les fichiers de certificat et de clé doivent être au format PEM
* Un fichier `cert-map.json` de modélisation des noms d’hôte vers les fichiers de certificat et de clé est requis

**Format de cert-map.json :**
...
Priorité de correspondance du nom d’hôte : correspondance exacte → caractère générique (`*.domain.com`) → `default` fourre-tout → aucun certificat (les connexions non-mTLS ne sont pas affectées).

<CollapserGroup>
  <Collapser id="mtls-docker" title="Docker">
    Montez un répertoire contenant `cert-map.json` et les fichiers de certificat et de clé référencés :
    ...
    Si les noms d’hôte du serveur mTLS ne sont pas résolubles via DNS dans les conteneurs d&apos;exécution, ajoutez une modélisation de nom d’hôte à l&apos;aide de `RUNTIME_EXTRA_HOSTS`:
  ...
  <Collapser id="mtls-podman" title="Podman">
    Montez le répertoire du certificat client. Pour SELinux, ajoutez `:z` ou `:Z`:
  ...
  <Collapser id="mtls-kubernetes" title="Kubernetes">
    Créez un Secret Kubernetes contenant `cert-map.json` et tous les fichiers de certificat et de clé référencés :
    ...
    Référencez le secret dans vos valeurs Helm :
    ...
    Le secret est monté automatiquement en lecture seule dans tous les pods d’exécution.
  </Collapser>
</CollapserGroup>

## Limitations de la prise en charge des certificats [#certificates-support-limitations]

Les limitations suivantes s&apos;appliquent à la fois aux certificats CA personnalisés et aux certificats clients mTLS, sauf indication contraire :

* **Redémarrage requis pour les modifications de certificats** — Les certificats sont chargés au démarrage du conteneur. Après avoir ajouté, supprimé ou effectué une rotation des certificats, redémarrez le conteneur du gestionnaire de tâches (ou les pods d’exécution dans Kubernetes) pour que les modifications prennent effet.
* **Runtimes pris en charge** — Les fonctionnalités de certificat personnalisé s&apos;appliquent uniquement aux API scriptées et aux moniteurs de navigateur scripté. Les moniteurs de ping ne sont pas affectés.
* **CN du sujet unique par certificat (mTLS uniquement)** — Chaque certificat client doit avoir un nom commun (CN) du sujet unique. Chrome utilise le CN pour identifier et sélectionner automatiquement le certificat correct par nom d’hôte.
...
<CollapserGroup>
  <Collapser id="docker-storage" title="Pour Docker">
  ...
  <Collapser id="podman-storage" title="Pour Podman">
  ...
  <Collapser id="kubernetes-storage" title="Pour Kubernetes">
    ### Kubernetes
...
## Considérations relatives au dimensionnement [#vm-sizing]

<CollapserGroup>
  <Collapser id="docker-podman-sizing" title="Pour Docker et Podman">
    ...
    * $C\_\{est}$ = **Cœurs de processeur estimés**.
    * $R\_\{proc}$ = Le **taux** de tâches lourdes **traitées** par minute.
    * $R\_\{growth}$ = Le **taux** auquel votre file d&apos;attente `jobManagerHeavyweightJobs` **augmente** par minute.
    * $D\_\{avg,m}$ = La **durée moyenne** des tâches lourdes en **minutes**.
    ...
    * $C\_\{est}$ = **Cœurs de processeur estimés**.
    * $N\_\{mon}$ = Le **nombre** total de **moniteurs** lourds que vous prévoyez d&apos;exécuter.
    * $D\_\{avg,m}$ = La **durée moyenne** d&apos;une tâche lourde en **minutes**.
    * $P\_\{avg,m}$ = La **période moyenne** des moniteurs lourds en **minutes** (par ex. un moniteur qui s&apos;exécute toutes les 5 minutes a $P\_\{avg,m} = 5$).
    ...
    * **Durée du job ($D\_\{avg,m}$) :** Votre moyenne doit inclure les jobs qui **tombent en timeout** (souvent 3 minutes), car ils occupent un cœur pendant toute leur durée.
    ...
    * **Débit total plus élevé**: le débit total pour votre site privé devient la somme du débit de chaque SJM (par exemple, deux SJM fournissent jusqu’à 30 tâches/minute).
    ...
    **1. Trouver le taux de tâches traitées par minute ($R\_\{proc}$)**: Cette requête compte le nombre de tâches non-ping (lourdes) terminées au cours de la dernière journée et affiche le taux moyen par minute.
    ...
    **2. Trouver le taux de croissance de la file d&apos;attente par minute ($R\_\{growth}$)**: Cette requête calcule la croissance moyenne par minute de la file d&apos;attente `jobManagerHeavyweightJobs` sur un graphique de séries temporelles. Une ligne au-dessus de zéro indique que la file d&apos;attente augmente, tandis qu&apos;une ligne en dessous de zéro signifie qu&apos;elle diminue.
    ...
    **3. Trouver le nombre total de moniteurs lourds ($N\_\{mon}$)**: Cette requête trouve le nombre unique de moniteurs lourds.
    ...
    **4. Trouver la durée moyenne des tâches en minutes ($D\_\{avg,m}$)**: Cette requête détermine la durée moyenne d&apos;exécution des tâches non-ping terminées et convertit le résultat de millisecondes en minutes. `executionDuration` représente le temps que la tâche a mis à s&apos;exécuter sur l&apos;hôte.
    ...
    **5. Déterminer la période moyenne du moniteur lourd ($P\_\{avg,m}$) :** Si la file d&apos;attente `jobManagerHeavyweightJobs` de l&apos;emplacement privé augmente, il est inexact de calculer la période moyenne du moniteur à partir des résultats existants. Ceci devra être estimé à partir de la liste des moniteurs sur la page [Moniteurs synthétiques](https://one.newrelic.com/synthetics). Assurez-vous de sélectionner le bon compte New Relic et vous devrez peut-être filtrer par `privateLocation`.
  ...
  <Collapser id="k8s-sizing" title="Pour Kubernetes et OpenShift">
    ...
    **Achèvements estimés :** Cela détermine le temps nécessaire à l&apos;achèvement de votre charge de jobs de 5 minutes.
    ...
    Où $D\_\{avg,m}$ est votre **durée moyenne d&apos;exécution des jobs en minutes**.
    ...
    **Parallélisme estimé :** Cela détermine le nombre de workers (pods) que vous devez exécuter simultanément pour gérer votre charge de travail de 5 minutes.
    ...
    Où $N\_m$ est votre **nombre de jobs par 5 minutes**. Cette valeur $P\_\{est}$ est votre **parallélisme estimé**.
    ...
    **5. Mettre à l&apos;échelle horizontalement si nécessaire**
    ...
    Pour évoluer au-delà du débit de 15 jobs/minute d&apos;un seul SJM, vous devez installer **plusieurs sorties SJM Helm distinctes**.
