...
Nos gestionnaires de tâches de monitoring synthétique sont des ressources basées sur un [conteneur](https://www.docker.com/resources/what-container)qui acceptent et exécutent le [monitorer Synthétique](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring/#types-of-synthetic-monitors) affecté au [site privé](/docs/synthetics/synthetic-monitoring/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations). Le gestionnaire de tâches peut fonctionner dans un environnement de système de conteneur Docker, un environnement de système de conteneur Podman, un environnement de système d&apos;orchestration de conteneur Kubernetes, ou un environnement de système de conteneur OpenShift.

<Callout variant="important">
  Si vous migrez des runtimes Node.js 16/Chrome 134 vers Node.js 22/Chrome 146, consultez le [Guide de transition du runtime de site privé](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide) pour obtenir des stratégies de migration, des exemples de déploiement et des étapes de dépannage.
</Callout>
...
## Exigences de version [#version-requirements]
...
<CollapserGroup>
  <Collapser id="docker-requirements" title="Configuration requise pour l'environnement du système conteneur Docker">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou supérieure (Recommandé pour v447+)<br /> <DNT>**macOS:**</DNT> 10.11 ou supérieure<br /> <DNT>**Windows:**</DNT> Windows 10 64 bits ou supérieure
    ...
    <Callout variant="caution">
      À partir de Synthetics Job Manager v447 (qui utilise Ubuntu 24.04 comme image de base), RHEL 7 et les autres distributions utilisant des versions de noyau Linux antérieures à 4.18 ne sont plus pris en charge en raison des exigences modernes de glibc et de seccomp. Pour une expérience stable, utilisez RHEL 8/9 ou une distribution Linux moderne équivalente.
    </Callout>
  </Collapser>

  <Collapser id="podman-requirements" title="Configuration requise pour l'environnement du système conteneur Podman">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou supérieur (Recommandé pour v447+)<br />
          </td>
        </tr>
  ...
  <Collapser id="kubernetes-requirements" title="Configuration requise pour l'environnement du système d'orchestration de conteneurs Kubernetes">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18 ou supérieur (recommandé pour v447+) <br /><DNT>**macOS:**</DNT> 10.11 ou supérieur<br />
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> [4.18 ou supérieur (recommandé pour v447+)](https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-prerequisites#minimum-resource-requirements)<br /> <DNT>**macOS:**</DNT> 10.11 ou supérieur<br />
...
Si vous exécutez plus d&apos;un conteneur d&apos;emplacement privé Docker ou Podman sur le même hôte, vous rencontrerez des conflits de ports. Pour éviter ce conflit de port, veillez à exécuter chaque gestionnaire de tâches sur un hôte distinct.
...
<CollapserGroup>
  <Collapser id="docker-update" title="Procédure de démarrage Docker">
    ...
    3. Exécutez le script approprié pour votre système. Adaptez la valeur par défaut commune `/var/run/docker.sock` dans les exemples suivants pour qu&apos;elle corresponde à votre système.
       ...
       * Windows:
       ...
       Consultez les logs de votre conteneur de gestionnaire de tâches :
  ...
  <Collapser id="podman-update" title="Procédure de démarrage de Podman">
    1. Localisez [la clé privée de votre site](#private-location-key).
    ...
    Pour afficher les logs de votre conteneur de gestionnaire de tâches :
  ...
  <Collapser id="kubernetes-install" title="Procédure de démarrage Kubernetes">
    1. Localisez [la clé privée de votre site](#private-location-key).
...
<CollapserGroup>
  <Collapser id="docker-stop" title="Procédure d'arrêt Docker">
  ...
  <Collapser id="podman-stop" title="Procédure d'arrêt Podman">
  ...
  <Collapser id="kubernetes-delete" title="Procédure de suppression Kubernetes">
    ...
    2. Supprimez le pod du gestionnaire de tâches :
...
<CollapserGroup>
  <Collapser id="docker-dependencies" title="Dépendances Docker">
...
<CollapserGroup>
  <Collapser id="podman-dependencies" title="Dépendances Podman">
...
<CollapserGroup>
  <Collapser id="run-non-root" title="Exécuter en tant qu'utilisateur non root pour Docker">
