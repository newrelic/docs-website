...
## Sécurisation de l&apos;accès aux données log [#securing-log-data]

Lorsqu&apos;on travaille avec des données log susceptibles de contenir des informations sensibles, il est important de contrôler qui peut accéder à des logs spécifiques. Voici les bonnes pratiques pour sécuriser vos données log :

* **Utiliser le contrôle d&apos;accès aux données**: Mettre en œuvre des politiques [de contrôle d&apos;accès aux données](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control) pour restreindre les utilisateurs autorisés à consulter [des partitions log](/docs/logs/ui-data/data-partitions/) spécifiques. Ceci est particulièrement important lorsque les logs contiennent des informations personnelles identifiables (IPI), des données de sécurité ou des informations sensibles pour l&apos;entreprise.

* **Soyez attentif aux risques d&apos;exposition des données**: certaines fonctionnalités permettent de contourner les restrictions d&apos;accès aux données. Examinez [les risques potentiels d&apos;exposition des données](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control#data-exposure-risks) et suivez ces bonnes pratiques :

  * Limitez les personnes ayant <DNT>**Modify**</DNT> accès à la capacité <DNT>**Data partitions rules**</DNT>, car cela permet à l&apos;utilisateur de reconfigurer les partitions et d&apos;accéder potentiellement à des données restreintes.
  * Restreindre l&apos;accès <DNT>**Modify**</DNT> à la fonctionnalité <DNT>**Query keys**</DNT> à l&apos;administrateur de confiance uniquement, car la clé de requête peut contourner les restrictions d&apos;accès aux données au niveau utilisateur.
  * Faites pivoter ou supprimez legacy API Insights qui ne respectent pas les politiques d&apos;accès aux données.
  * Créer des rôles personnalisés sans capacités de gestion des alertes pour les utilisateurs qui ne devraient pas accéder aux données sensibles via une condition d&apos;alerte ou une notification.

* **Alignez les partitions avec le contrôle d&apos;accès**: lors de la planification de votre stratégie de partitionnement, réfléchissez aux équipes qui devraient avoir accès à quels logs. Regroupez les logs par équipe, service ou niveau de sensibilité pour faciliter la gestion du contrôle d&apos;accès.
