...
L’espace de nommage d’une partition détermine sa période de rétention. Nous proposons trois options de rétention :
...
* <DNT>**Direct to live archive:**</DNT> Ignore entièrement la rétention standard. Les logs envoyés à une partition utilisant cet espace de nommage vont directement dans le stockage à long terme Live Archives au lieu d’arriver d’abord dans le stockage standard. Cet espace de nommage convient mieux aux logs pour lesquels vous avez rarement besoin d’effectuer des requêtes, mais que vous devez conserver pendant une longue période, tels que les logs de conformité ou d’audit, puisque vous ne payez pas pour le stockage de niveau standard sur des données que vous ne prévoyez pas de consulter au quotidien. Cette option nécessite que vous activiez Live Archives sur votre compte. Pour plus d’informations, consultez [Stockage de log à long terme avec Live Archives](/docs/logs/get-started/live-archives/).
