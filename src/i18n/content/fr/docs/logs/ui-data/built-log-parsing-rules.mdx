...
## Exemple d&apos;analyse [#parsing-example]
...
Lors de l&apos;agrégation de logs, il est important de fournir des métadonnées qui facilitent l&apos;organisation, la recherche et l&apos;analyse de ces logs. Une façon simple de procéder consiste à ajouter l&apos;attribut `logtype` aux messages de log lorsqu&apos;ils sont envoyés. Les [règles de parsing intégrées](/docs/logs/ui-data/built-log-parsing-rules) sont appliquées par défaut à certaines valeurs de `logtype`.
