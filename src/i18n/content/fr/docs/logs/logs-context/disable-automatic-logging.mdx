...
<Callout variant="important">
  La configuration côté serveur pour le transfert de logs d&apos;application est actuellement **prise en charge uniquement par l&apos;agent Java (version 8.19.0 ou ultérieure)**. Pour plus d&apos;informations, consultez [Configuration de l&apos;agent côté serveur](/docs/apm/agents/manage-apm-agents/configuration/server-side-agent-configuration).

  Pour tous les autres agents APM, vous devez gérer les paramètres de log via la [configuration côté client](#disable-config).
</Callout>
