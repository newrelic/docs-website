...
4. Le cas échéant, [configurez le point de terminaison EU ou Japon](#eu-configuration)
...
## Configurer le plug-in Fluentd pour les comptes UE ou Japon [#eu-configuration]

Par défaut, le plug-in Fluentd transmet le log à notre point de terminaison américain : `https://log-api.newrelic.com/log/v1`. Si votre organisation New Relic se trouve dans notre [data center de l&apos;UE ou du Japon](/docs/accounts/accounts-billing/account-setup/choose-your-data-center), vous devez définir manuellement la propriété `base_uri` sur le point de terminaison approprié :

* UE: `https://log-api.eu.newrelic.com/log/v1`
* Japon : `https://log-api.jp.nr-data.net/log/v1`

Par exemple, pour transférer les logs vers le point de terminaison UE :
...
Pour les comptes japonais, remplacez le `base_uri` dans le bloc `<match>` par le point de terminaison du Japon :
