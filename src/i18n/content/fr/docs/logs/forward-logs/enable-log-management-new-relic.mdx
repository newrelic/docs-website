...
4. Selon l’endroit où les données de votre compte sont stockées dans New Relic, utilisez le nom DNS `log-api.newrelic.com` (US), `log-api.eu.newrelic.com` (EU) ou `log-api.jp.nr-data.net` (Japon) pour la configuration.
