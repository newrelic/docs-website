...
4. Selon l&apos;endroit où les données de votre compte sont stockées dans New Relic, utilisez le nom DNS `log-api.newrelic.com` (États-Unis, par défaut), `log-api.eu.newrelic.com` (UE) ou `log-api.jp.nr-data.net` (Japon) pour la configuration.
