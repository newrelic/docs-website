...
<CollapserGroup>
  ...
  <Collapser id="dataflow-job" title="7. Exécutez le modèle en tant que tâche Dataflow.">
    ...
    * New Relic <InlinePopover type="licenseKey" />utilisée pour envoyer votre log
    ...
    Par défaut, cette commande envoie le log au data center américain. Pour les comptes de l’UE ou du Japon, consultez la section ci-dessous.

    <CollapserGroup>
      <Collapser id="eu-customers-gcp-dataflow" title="Comptes des régions Europe et Japon">
        Les comptes européens et japonais doivent également ajouter le paramètre `logsApiUrl` avec le point de terminaison approprié.
