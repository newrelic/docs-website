...
2. En option, vous pouvez affiner davantage votre installation dans [l&apos;installation guidée](/install/kubernetes) en utilisant les nombreuses options de configuration disponibles dans le [référentiel`newrelic-logging` ](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration). Cependant, nous recommandons la configuration standard, car elle est valable pour la plupart des utilisateurs.
   ...
   <Callout variant="important">
     Si vous [utilisez un secret Kubernetes](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-logging/values.yaml#L8-L25) pour stocker le New Relic <InlinePopover type="licenseKey" />, le chart `newrelic-logging` envoie par défaut le log aux points de terminaison d&apos;API américains. Si la clé de licence appartient à un compte UE, Japon ou FedRAMP et qu&apos;un secret est utilisé pour le stockage de la clé, vous devez mettre à jour le paramètre de point de terminaison avec la valeur appropriée à partir [des documents de référence de l&apos;API](/docs/logs/log-api/introduction-log-api/#endpoint). Voici des exemples de configuration pour les comptes de l&apos;UE et du Japon :
...
2. Le graphique Helm `newrelic-logging` peut être configuré pour transmettre les métriques internes du plugin à New Relic. Cela aide au monitoring et à dépanner le processus de transfert des log .
...
<DocTiles>
  <DocTile title="interface utilisateur du log" path="/docs/logs/ui-data/use-logs-ui/">
    Explorez les données de logging sur votre plateforme avec notre UI de logs
  </DocTile>

  <DocTile title="Les logs en contexte" path="/docs/logs/logs-context/logs-in-context/">
  ...
  <DocTile title="Alertes" path="/docs/alerts/overview/">
    Créez des alertes pour rester informé des problèmes les plus importants
  </DocTile>

  <DocTile title="Créer un dashboard" path="/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/">
