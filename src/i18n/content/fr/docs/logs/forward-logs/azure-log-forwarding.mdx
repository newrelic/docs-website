...
<Callout variant="tip">
  Vous cherchez à transférer les logs Azure VNet Flow ? Consultez [Configurer le monitoring des logs Azure VNet Flow](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring) pour le transfert de logs VNet Flow dédié avec traitement incrémentiel des deltas. Vous pouvez utiliser les modèles de déploiement ARM et Bicep pour une configuration automatisée.
</Callout>
...
11. Facultatif : sélectionnez <DNT>**Authentication Mode**</DNT>. La valeur par défaut est `Local Authentication`. Choisissez `Managed Identity` si votre environnement Azure a désactivé l&apos;authentification locale (clés d&apos;accès partagées) pour des raisons de conformité en matière de sécurité. Lorsqu&apos;il est défini sur `Managed Identity`, la Function App se voit attribuer une identité gérée affectée par le système et accorder le rôle `Azure Event Hubs Data Receiver` sur l&apos;espace de nommage du Hub d&apos;événement — aucune chaîne de connexion n&apos;est stockée. Consultez [les informations sur le mode d’authentification](#eventhub-authentication) pour plus de détails.

12. Facultatif : configurez les paramètres de traitement par lots EventHub (disponibles dans la version 2.8.0+) pour optimiser les performances :
...
13. Facultatif : définissez sur `true` les [logs d’activité de l’abonnement Azure](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) que vous souhaitez transférer. Consultez [les informations d’abonnement](#subscription-activity-logs) dans ce document pour plus de détails.

14. Cliquez sur <DNT>**Review + create**</DNT>, vérifiez les données que vous avez insérées et cliquez sur <DNT>**Create**</DNT>.
...
### Facultatif : configurer le mode d&apos;authentification [#eventhub-authentication]

À partir de la version 3.2.0, le modèle ARM prend en charge deux méthodes d&apos;authentification pour connecter la Function App au hub d&apos;événements :

* <DNT>**Local Authentication**</DNT> (par défaut) : l&apos;application de fonction se connecte à l&apos;aide d&apos;une chaîne de connexion de clé d&apos;accès partagé stockée dans les paramètres de l&apos;application. Il s&apos;agit de l&apos;approche traditionnelle et elle correspond aux versions précédentes du modèle.

* <DNT>**Managed Identity**</DNT>: la Function App utilise une identité managée affectée par le système avec le rôle intégré `Azure Event Hubs Data Receiver` sur l&apos;espace de nommage du Hub d&apos;événement. Aucun secret ni aucune chaîne de connexion n&apos;est stocké. Utilisez cette option lorsque :

  * Votre organisation a désactivé l&apos;authentification locale (clés d&apos;accès partagées) sur les espaces de nommage de Hub d&apos;événements pour des raisons de conformité de sécurité.
  * Vous préférez une authentification sans clé entièrement gérée par Azure AD.

Lorsque `Managed Identity` est sélectionné, le modèle automatiquement :

* Crée une identité managée affectée par le système sur la Function App.
* Accorde à l&apos;identité le rôle `Azure Event Hubs Data Receiver` sur l&apos;espace de nommage du hub d&apos;événements.
* Configure la Function App avec `EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace` (le nom d’hôte de l’espace de nommage du Hub d&apos;événements) au lieu d&apos;une chaîne de connexion. Le runtime Azure Functions utilise automatiquement des jetons Azure AD pour l&apos;authentification.
