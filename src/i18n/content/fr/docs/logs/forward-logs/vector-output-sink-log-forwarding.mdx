...
3. Ajoutez un snippet à votre fichier `vector.yaml` (situé dans `/etc/vector` par défaut), en remplaçant `YOUR_LICENSE_KEY` par votre New Relic <InlinePopover type="licenseKey" />:
