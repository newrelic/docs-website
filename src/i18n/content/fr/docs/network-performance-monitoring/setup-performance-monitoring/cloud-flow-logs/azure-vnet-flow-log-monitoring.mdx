...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="arm-deploy">
      ...
      * Déployer la Function App sur votre plan d’hébergement sélectionné avec le runtime Node.js 22 et les fonctions de logs de VNet Flow
      ...
      * Activer en option la mise en réseau privée avec l&apos;intégration VNet et des points de terminaison privés
      ...
      8. Assurez-vous que le [point de terminaison New Relic](/docs/logs/log-api/introduction-log-api/#endpoint) est défini sur celui correspondant à votre compte (US, EU, JP ou FedRAMP pour US Gov Cloud).
      ...
      11. (Facultatif) Sélectionnez la liste déroulante <DNT>**Function App Plan**</DNT>. Quatre valeurs : `FlexConsumption` (par défaut), `ElasticPremium`, `Basic` ou `Consumption`. Consultez [Choisir un plan d&apos;hébergement](#hosting-plans) ci-dessous pour savoir quand choisir chacun d&apos;eux.

      12. (Facultatif) Configurez les paramètres avancés :
      ...
      13. Cliquez sur <DNT>**Review + create**</DNT>, passez en revue la configuration et cliquez sur <DNT>**Create**</DNT>.

      14. De retour dans New Relic, à l’étape <DNT>**Create log partition**</DNT>, cliquez sur <DNT>**Create partition**</DNT>. Cela crée une partition de données `Log_VNET_Flows_Azure` (règle : `instrumentation.name = 'vnet-app'`, rétention STANDARD) afin que vos VNet Flow Logs soient stockés et fassent l’objet d’une requête séparément de vos autres logs. Si une partition `Log_VNET_Flows_Azure` existe déjà sur le compte, cliquez sur <DNT>**Continue**</DNT> pour ignorer cette étape.

      15. À l&apos;étape <DNT>**Test your logs**</DNT>, confirmez que les données arrivent en exécutant `SELECT * FROM Log_VNET_Flows_Azure`, puis cliquez sur <DNT>**See your data**</DNT>.

      ### Choisir un plan d’hébergement [#hosting-plans]

      Le modèle déploie la Function App sur l&apos;un des quatre plans d&apos;hébergement Azure, sélectionnés avec la liste déroulante <DNT>**Function App Plan**</DNT>. La valeur par défaut est `FlexConsumption`.
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>
              Toute région où la consommation Flex est généralement disponible. Modèle serverless moderne avec intégration VNet native qui évolue de manière élastique sans capacité préchauffée.
            </td>
          ...
          <tr>
            ...
            <td>
              Workloads de production ou en rafale dans les régions où Flex Consumption n&apos;est pas encore disponible, ou workloads avec un SLA strict sans démarrage à froid.
            </td>
          ...
          <tr>
            ...
            <td>
              Petits locataires ou déploiements de développement nécessitant un réseau privé. Option compatible VNet la moins chère. Ne convient pas aux workloads à haut débit.
            </td>
          ...
          <tr>
            ...
            <td>
              Déploiements de développement ou d’évaluation sur réseau public où un réseau privé n’est pas requis. Le moins cher globalement.
            </td>
      ...
      <Callout variant="important">
        `Consumption` ne prend pas en charge l’intégration Azure VNet. Si vous activez <DNT>**Disable Public Access To Storage Account**</DNT>, choisissez l’une des options `FlexConsumption`, `ElasticPremium` ou `Basic` — le déploiement échoue rapidement dans la première minute si vous combinez `Consumption` avec un réseau privé.
      </Callout>
      ...
      * Achemine tout le trafic sortant — y compris les appels à l’API New Relic Logs — via le sous-réseau d’intégration VNet.
    </TabsPageItem>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Point de terminaison d&apos;API New Relic Logs. La valeur par défaut est le point de terminaison américain. Remplacement pour d&apos;autres régions : 
