...
<CollapserGroup>
  ...
  >
    ...
    Avec la clause `PREDICT` , vous pouvez intégrer des prédictions concernant les tendances futures des données dans des graphiques linéaires basés sur des données historiques. Cette clause :
    ...
    * Définit la plage de prédiction à 20 % de la fenêtre de requête par défaut.
    * Traite les données historiques de la fenêtre de requête actuelle et des deux précédentes pour générer des prédictions par défaut.
    * Ne prend pas en charge [les données d&apos;intervalle de temps métrique](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) en mode Aperçu public.
    ...
    Vous pouvez également ajouter `PREDICT <model>`, `BY <time range>` et `USING <time range>` pour affiner la tendance de prédiction en fonction de vos besoins spécifiques. Pour indiquer la plage de temps et la durée des données historiques, utilisez `integer units`.
