    ...
    * Ne prend pas en charge [les données d&apos;intervalle de temps métrique](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data) en mode Aperçu public.
    ...
    <CollapserGroup>
      <Collapser id="predict-defaults" title="Valeurs par défaut">
        ...
        données historiques pour la formation : utilise les données historiques des trois dernières fenêtres de requête (la fenêtre actuelle et les deux qui la précèdent). Cependant, si vous fournissez un hyperparamètre `seasonality` supérieur à la fenêtre de requête, le modèle utilise à la place des données historiques couvrant une période égale à trois fois la saisonnalité.
