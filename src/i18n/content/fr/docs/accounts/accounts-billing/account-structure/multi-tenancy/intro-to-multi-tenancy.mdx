...
* Partage de compte : permet à certains utilisateurs au sein de l&apos;organisation gestionnaire de partager un compte avec une organisation gérée. Ceci est principalement utilisé pour les cas d&apos;utilisation collaboratifs, tels que les prestataires de services gérés (MSP) fournissant aux clients finaux un accès direct aux comptes qu&apos;ils monitorent.
...
3. Ajoutez des utilisateurs à un groupe désigné : les utilisateurs avec le type d&apos;utilisateur core ou full plateforme doivent être ajoutés à un groupe avec un [octroi d&apos;accès](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#access-grants) au niveau de l&apos;organisation leur attribuant le rôle Tenant Manager.
...
Plus tard, les utilisateurs de Wayne Enterprises et du widget Acme décident qu&apos;ils souhaitent avoir une compréhension plus pratique de ce qui se passe dans les comptes que vous monitorez pour eux. Pour résoudre ce problème, créez une organisation pour Wayne Enterprises et le widget Acme. Vous partagez ensuite les comptes du MSP de Betty avec leur nouvelle organisation correspondante. Enfin, vous provisionnez l&apos;utilisateur dans son organisation respective afin qu&apos;il puisse voir ses données :
...
<DocTiles>
  <DocTile title="Étape 1" path="/docs/accounts/accounts-billing/account-structure/multi-tenancy/org-creation">
    Créer des comptes et une organisation
  </DocTile>

  <DocTile title="Étape 2" path="/docs/accounts/accounts-billing/account-structure/multi-tenancy/delegated-administration">
    Gérer les utilisateurs et les accès
  </DocTile>

  <DocTile title="Étape 3" path="/docs/accounts/accounts-billing/account-structure/multi-tenancy/share-accounts">
    Comptes partagés (facultatif)
  </DocTile>
</DocTiles>