...
<CollapserGroup>
  ...
  <Collapser id="manage-user-type-scim" title="Gérer le type d'utilisateur avec SCIM">
    ...
    * Azure Active Directory : reportez-vous à la discussion du forum New Relic sur la configuration d&apos;Azure AD pour SCIM avec des types d&apos;utilisateurs [ici](https://support.newrelic.com/s/hubtopic/aAX8W0000008d6MWAQ/relic-solution-make-full-core-basic-users-in-azure-active-directory-w-scim).
...
* [Nom d&apos;utilisateur/mot de passe](#username-authentication): votre utilisateur se connecte via email et mot de passe.
* [SAML SSO](#saml): votre utilisateur se connecte via SAML single sign-on (SSO) en utilisant votre fournisseur d&apos;identité. Pour savoir comment configurer cela, continuez à lire.

### Authentification par nom d’utilisateur/mot de passe [#username-authentication]

Il s&apos;agit d&apos;une méthode d&apos;authentification par défaut. Les utilisateurs se connectent en utilisant leur adresse e-mail et leur mot de passe. Pour plus de sécurité, vous pouvez également activer [l’authentification multifacteur (MFA)](#mfa).

<Callout variant="important">
  Pour recevoir l&apos;e-mail de vérification MFA lors de la connexion à notre application mobile, vous devez saisir votre e-mail dans l&apos;application puis ouvrir l&apos;écran de connexion dans votre navigateur mobile.
</Callout>

### Authentification SAML SSO [#saml]
...
L&apos;authentification multifacteur (MFA) ajoute une couche de sécurité à vos données en exigeant que les utilisateurs d&apos;un domaine d&apos;authentification vérifient leur identité.

Lorsque cette option est activée, les utilisateurs reçoivent un e-mail de vérification à chaque connexion. Lors de leur première connexion nécessitant la MFA, les utilisateurs reçoivent également dix codes de sauvegarde. Ces codes sont utilisés s&apos;ils ne peuvent pas accéder à leur e-mail. Conseillez à vos utilisateurs de stocker ces codes dans un endroit sûr.

Pour terminer la vérification MFA pour l&apos;application New Relic Mobile, saisissez votre e-mail dans l&apos;application puis suivez le prompt pour ouvrir l&apos;écran de connexion dans votre navigateur mobile.

**Pour activer MFA :**

1. Accédez à [one.newrelic.com &gt; Administration &gt; Authentication Domains](https://onenr.io/08jqqPYdvjl)
2. Sélectionnez le domaine requis.
3. Activez le bouton MFA. Une fois activé, les utilisateurs de ce domaine d&apos;authentification reçoivent un prompt pour vérifier leur e-mail lors de leur prochaine connexion et de toutes les connexions suivantes.
