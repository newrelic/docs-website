...
<CollapserGroup>
  ...
  <Collapser id="month-gb" title="Utilisation des données mensuelles à ce jour">
    Cette requête affiche les données ingérées complètes de la plateforme actuelle. En d&apos;autres termes, il indique combien vos données vous seraient facturées pour ce mois-ci si vous étiez facturé maintenant.
...
## requête de comptage d&apos;utilisateurs [#user-queries]
...
### requête utilisateur pour organisation sans noyau utilisateur [#queries-non-core]
...
<CollapserGroup>
  <Collapser id="non-core-org-user-queries" title="requête utilisateur pour organisation sans noyau utilisateur">
