---
title: Optimisez votre ingestion de données
tags:
...
**La gouvernance de l&apos;ingestion de données** est une pratique visant à obtenir une valeur optimale pour les données télémétriques collectées par une organisation. Ceci est particulièrement important pour une organisation complexe qui comporte de nombreuses unités commerciales et groupes de travail. Il s’agit de la troisième partie d’un guide en quatre parties visant à optimiser votre ingestion de données New Relic et fait partie de [notre série sur la maturité de l’observabilité](/docs/new-relic-solutions/observability-maturity/introduction).

## Avant de commencer [#before-start]

Ce guide contient des recommandations détaillées pour optimiser votre ingestion de données. Avant d&apos;utiliser ce guide, nous vous recommandons de consulter [notre documentation générale sur la gestion des données](/docs/data-apis/manage-data/manage-your-data).
...
* [Priorisez vos objectifs d&apos;observabilité](#prioritize-objectives)
* [Développer un plan d&apos;optimisation](#develop-plan)
* [Utilisez des techniques de réduction des données pour exécuter votre plan](#use-reduction-techniques)
...
* Vous disposez des outils et techniques de la [base de référence de votre section d&apos;ingestion de données](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining) pour avoir une comptabilité appropriée de la provenance de vos données.
* Vous avez une bonne compréhension des [facteurs de valeur de maturité de l’observabilité](/docs/new-relic-solutions/observability-maturity/introduction). Cela sera crucial pour appliquer une valeur et une priorité aux groupes de télémétrie.
...
<Callout variant="tip">
  Nous vous recommandons de suivre le plan dans un outil de gestion des tâches que vous connaissez. Cela permet de gérer le plan d’optimisation et également de comprendre l’effet de chaque tâche d’optimisation. Vous pouvez utiliser ce [modèle de plan d’optimisation des données](https://docs.google.com/spreadsheets/d/1CimLpALwl1Z9f41vzbNWx00bGcED9XPV3s4ROqVEnr0/copy).
</Callout>
...
### Optimisation par la configuration [#optimization-through-configuration]
...
<CollapserGroup>
  <Collapser id="apm-agent" title="Agent APM">
    ...
    * `request.headers.userAgent`
    ...
    Pour un exemple de la riche configuration disponible en relation avec APM, consultez notre [documentation sur l&apos;agent Java](/docs/apm/agents/java-agent/attributes/java-agent-attributes/#requestparams)
    ...
    Nous avons la capacité de `collect`, `ignore` ou `mark as expected`. Pour plus d&apos;informations, voir [Gérer les erreurs APM](/docs/apm/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected).
    ...
    * La limite de taille de charge est de 1 Mo (10^6 octets) (compressé), donc le nombre d&apos;événements peut toujours être affecté par cette limite. Pour déterminer si les événements sont supprimés, consultez le log de l&apos;agent pour un message d&apos;état `413 HTTP`.
    ...
    Ceci est également hautement configurable via [la configuration des transactions commerciales](/docs/apm/transactions/transaction-traces/configure-transaction-traces). Dans de nombreux cas, le niveau et le mode de configurabilité seront spécifiques à la langue.
    ...
    * Tracing des transactions et seuil
    ...
    * requête de tracing de lentille
    * Profileur de filetage
    ...
    Le volume de données varie également selon que vous utilisez ou non [Infinite Tracing](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing).
  ...
  <Collapser id="browser-agent" title="Agent Browser">
    ...
    * Le protocole, le port, la recherche et le hachage d&apos;une URL sont ignorés par la liste de refus.
    ...
    1. Allez sur **[one.newrelic.com](https://one.newrelic.com/all-capabilities)**, et cliquez sur Browser.
  ...
  <Collapser id="mobile-agent" title="Agent mobile">
    ...
    [Les paramètres d&apos;analyse](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api/android-agent-configuration-feature-flags#analytics-settings) activent ou désactivent la collecte de données d&apos;événement. Ces événements sont signalés à New Relic et utilisés dans la page **Crash analysis** .
  ...
  <Collapser id="k8s-integration" title="Intégration Kubernetes">
    ...
    * N&apos;existe pas
    ...
    Consultez la liste complète des paramètres pouvant être modifiés dans le [fichier README du graphique](https://github.com/newrelic/nri-kubernetes/tree/main/charts/newrelic-infrastructure).
  ...
  <Collapser id="prometheus-metrics-sources" title="Sources des métriques Prometheus">
    ...
    **Paramètre de configuration POMI :**
    ...
    ### POMI : estimation de la cardinalité

    Bien que la cardinalité ne soit pas directement associée à l&apos;ingestion facturable par Go, New Relic maintient certaines limites de taux sur la cardinalité. Pouvoir visualiser la cardinalité d&apos;un cluster Prometheus peut être très important.

    <Callout variant="tip">
      Les comptes New Relic ont une limite de cardinalité de 1M, mais vous pouvez demander jusqu&apos;à 15M de cardinalité. Pour demander des modifications, contactez votre représentant de compte New Relic. Pour plus d’informations, consultez les [limites de l’API métrique](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes).
    </Callout>

    Si vous exécutez déjà le serveur Prometheus, vous pouvez y effectuer des estimations de cardinalité avant d&apos;activer POMI ou `remote_write`.
  ...
  <Collapser id="cloud-integration" title="Intégration dans le cloud">
    ...
    * **Nouveaux services**: Si vous déployez un nouveau service ou une nouvelle configuration et que vous souhaitez collecter des données plus souvent, vous souhaiterez peut-être augmenter temporairement la fréquence d&apos;interrogation.
    ...
    De plus en plus cloud d&apos;intégration offrent la possibilité de faire circuler les données via un [service de streaming](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-metric-stream) au lieu d&apos;utiliser l&apos;interrogation API. Cela s’est avéré réduire considérablement la latence. Un problème que certains utilisateurs ont observé est qu&apos;il n&apos;est pas aussi facile de contrôler le volume car le taux d&apos;échantillonnage ne peut pas être configuré.
...
#### Optimisation avec des règles de dépôt [#optimization-with-drop-rules]
...
<CollapserGroup>
  ...
  <Collapser id="drop-random-sample-of-events" title="Déposer un échantillon aléatoire de l'événement">
    ...
    L&apos;exemple suivant pourrait supprimer environ 25 % des spans :
...
Répondre aux questions suivantes vous aidera à développer la confiance dans votre capacité à développer et à exécuter des plans d’optimisation. Vous souhaiterez peut-être utiliser la [base de référence d&apos;ingestion de données](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-dashboard) et le dashboard [de répartition des entités d&apos;ingestion de données](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-entity-breakdown-dashboard) de la section `Baselining`. Installez ces dashboards comme décrit et voyez à combien de questions vous pouvez répondre.
...
| 1. Utilisez le dashboard de référence de la gouvernance d&apos;ingestion de données pour identifier un compte qui envoie une grande quantité de données log à New Relic.<br />2. Recherchez et sélectionnez ce compte à partir du sélecteur de compte.<br />3. Accédez à la page **logs** du compte et sélectionnez **patterns** dans le menu de gauche.<br />4. Passez en revue les log patterns affichés et donnez quelques exemples de log patterns de faible valeur. Qu’est-ce qui fait qu’ils ont une faible valeur ? Quelle réduction totale pourriez-vous obtenir en abandonnant ces logs ? |
| Sur la base de votre analyse globale de cette organisation, quelle télémétrie est sous-utilisée ?                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

## Conclusion [#conclusion]

La section processus nous a montré comment associer notre télémétrie à des moteurs ou objectifs de valeur d’observabilité spécifiques. Cela peut rendre la décision difficile d’optimiser l’ingestion de notre compte quelque peu plus facile. Nous avons appris à décrire un [plan d’optimisation](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-optimizing#develop-plan) de haut niveau qui optimise l’ingestion tout en protégeant nos objectifs. Enfin, nous avons découvert un [riche ensemble de recettes](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-optimizing#use-reduction-techniques) pour la configuration et les optimisations d&apos;ingestion basées sur des règles de suppression.