...
Par défaut, la CLI Diagnostics envoie les résultats aux comptes New Relic hébergés dans le data center américain. Si votre compte est hébergé dans le data center de l’UE ou du Japon, ajoutez `-region eu` ou `-region jp` à la commande.
