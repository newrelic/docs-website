...
Dans APM, certains graphiques ont l’option de [voir la requête NRQL qui a généré ce graphique](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language). C’est un bon point de départ pour comprendre comment faire une requête sur les métriques. La requête NRQL ci-dessous est légèrement modifiée par rapport au graphique de taux d’erreur sur la page de résumé <InlinePopover type="apm" />.
