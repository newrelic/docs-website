...
Assurez-vous que la connectivité sortante sur le port TCP 443 est autorisée vers la [plage CIDR](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure) qui correspond à votre [région](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers). La méthode de configuration préférée est d&apos;utiliser le nom DNS `insights-collector.newrelic.com` (US, par défaut), `insights-collector.eu01.nr-data.net` (EU) ou `insights-collector.jp.nr-data.net` (JP).
...
<Callout variant="tip">
  Si votre organisation héberge des données dans le data center de l&apos;UE ou JP, assurez-vous d&apos;utiliser les points de terminaison régionaux appropriés. Consultez les [points de terminaison de la région UE](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#eu-endpoints) ou les [points de terminaison de la région JP](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#jp-endpoints).
</Callout>
