...
Voici les services de point de terminaison New Relic disponibles via AWS PrivateLink. Les points de terminaison américains (par défaut) sont répartis sur trois zones de disponibilité AWS (us-east-1, us-east-2, us-west-2), présentés sous forme d’onglets distincts ci-dessous :
...
### Points de terminaison FedRAMP PrivateLink [#fedramp]

Pour plus d&apos;informations sur les points de terminaison PrivateLink conformes à FedRAMP, consultez [Connectivité AWS PrivateLink](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints#privatelink) dans notre guide FedRAMP.
