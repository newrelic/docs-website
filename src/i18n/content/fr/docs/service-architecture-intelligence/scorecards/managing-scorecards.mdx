...
6. Sur l&apos;écran **Define maturity levels**, faites glisser et déposez chaque règle dans la catégorie de maturité appropriée (**Below basic**, **Basic maturity**, **Intermediate maturity** ou **Advanced maturity**), puis cliquez sur **Next**. Pour plus d&apos;informations, consultez [Maturité du service](/docs/service-architecture-intelligence/scorecards/service-maturity).
...
<Callout variant="tip" title="conseil">
  Utilisez les mutations NerdGraph pour créer ou gérer vos règles de Scorecards. Pour plus d’informations, reportez-vous au [didacticiel NerdGraph Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>
...
<Callout variant="tip">
  Vous pouvez attribuer une règle à un [niveau de maturité](/docs/service-architecture-intelligence/scorecards/service-maturity) lorsque vous définissez les niveaux de maturité lors de la création du Scorecard, ou en modifiant la règle ultérieurement. Pour définir le poids d’une règle ou créer des niveaux de maturité personnalisés, utilisez le [didacticiel NerdGraph Scorecards](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
</Callout>
...
Vous pouvez également activer ou désactiver les règles existantes pour une Scorecard. Lorsque vous modifiez une règle, vous pouvez changer son [niveau de maturité](/docs/service-architecture-intelligence/scorecards/service-maturity); les poids des règles sont définis via l&apos;[API NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-scorecards-tutorial).
...
## Configurer les notifications de règle [#rule-notifications]

Vous pouvez être notifié lorsque le score d&apos;une règle tombe en dessous d&apos;un seuil que vous avez défini. Les notifications Scorecards sont basées sur les [alertes](/docs/alerts/overview/) New Relic : lorsque vous en créez une, Scorecards crée automatiquement une [règle d’alerte](/docs/alerts/organize-alerts/create-edit-or-find-alert-policy/) et une [condition](/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/) pour la règle et y associe un [workflow](/docs/alerts/get-notified/alert-event-workflows/) qui envoie la notification à la destination de votre choix.

**Pour configurer une notification :**

1. connectez-vous à votre compte New Relic.

2. Accédez au **volet de navigation de gauche &gt; All Capabilities &gt; Scorecards**, et sélectionnez un Scorecard.

3. Ouvrez la règle pour laquelle vous souhaitez être notifié.

4. Sur la page de la règle, cliquez sur **Set up a notification**, puis complétez l&apos;assistant :

   1. **Définir un seuil**: vous êtes averti lorsque le score de la règle tombe en dessous de ce niveau.
   2. **Sélectionner un compte**: le compte où la règle d’alerte et la condition sont stockées. Scorecards crée la règle d’alerte et la condition pour vous automatiquement et nomme la règle d’alerte d’après la règle.
   3. **Ajouter un workflow**: choisissez le workflow qui transmet la notification à la [destination](/docs/alerts/get-notified/destinations/) sélectionnée.
   4. **Vérifier et créer**: vérifiez vos paramètres et créez la notification.
...
Pour modifier ou supprimer une notification ultérieurement, ouvrez la page de la règle et cliquez sur **Manage notifications**.
