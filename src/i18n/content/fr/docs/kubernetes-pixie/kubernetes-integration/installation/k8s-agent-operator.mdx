...
Exécutez la commande suivante pour installer la CR d&apos;instrumentation :
...
<CollapserGroup>
  ...
  <Collapser id="namespace-label-selector" title="NameSpaceLabelSelector">
    Définit les pods auto-instrumentés au niveau de l’espace de nommage. Autorisé lorsque le CR et l’opérateur sont dans le même espace de nommage. Sinon, ce sélecteur doit être laissé vide.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [.NET sur Windows 2022](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2022-init/general)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [.NET sur Windows 2025](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2025-init/general)
      </td>
    </tr>
...
<Callout variant="tip">
  Si votre cluster utilise un runtime de conteneur qui nécessite un préfixe de registre explicite (tel que CRI-O), ajoutez `docker.io/` au début du chemin de l&apos;image. Par exemple : `image: docker.io/newrelic/newrelic-java-init:latest`.
</Callout>
...
* Obtenez des informations sur vos applications et résolvez les événements d&apos;alerte à l&apos;aide de la page [Résumé APM](/docs/apm/agents/manage-apm-agents/agent-data/triage-run-diagnostics/).
...
<CollapserGroup>
  ...
  <Collapser id="setup-operator" title="Puis-je configurer la connexion automatique de Kubernetes APM dans les nœuds Windows ?">
    Actuellement, la connexion automatique Kubernetes APM n&apos;est prise en charge que pour les applications .NET exécutées sur des nœuds Windows Server 2022 et Windows Server 2025.
  </Collapser>

  <Collapser id="setup-operator" title="Puis-je configurer la connexion automatique de Kubernetes APM dans les nœuds Fargate ?">
    EKS Fargate est pris en charge
  </Collapser>
</CollapserGroup>
