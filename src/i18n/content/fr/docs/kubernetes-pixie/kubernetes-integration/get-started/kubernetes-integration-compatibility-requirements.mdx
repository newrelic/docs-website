...
Le monitoring Windows prend en charge deux modes :

* **Mode privilégié**, qui utilise des conteneurs Windows HostProcess pour collecter des métriques complètes au niveau du nœud. Le mode privilégié est activé par défaut.
* **Mode non privilégié**, qui s&apos;exécute en tant qu&apos;utilisateur de conteneur standard avec une visibilité réduite sur l&apos;hôte.

Pour plus de détails sur le fonctionnement de ces modes et des conseils sur leur configuration sécurisée, consultez [Mode privilégié vs non privilégié](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode). Pour des instructions de configuration complètes, consultez [Monitorer les nœuds Windows](/docs/kubernetes-pixie/kubernetes-integration/installation/windows).

## Exigences du mode privilégié

Par défaut, le chart d&apos;intégration Kubernetes New Relic configure le sidecar de l&apos;agent Infrastructure pour s&apos;exécuter en **mode privilégié** (`privileged: true`). Ce niveau d&apos;accès est requis pour collecter des métriques système approfondies (telles que les E/S de stockage, les interfaces réseau physiques et les processus système) à partir des systèmes de fichiers `/proc` et `/sys` de l&apos;hôte sous-jacent.

Si les politiques de sécurité de votre cluster (par exemple, les Pod Security Standards ou les SCC OpenShift) n&apos;autorisent pas les conteneurs privilégiés, vous devrez peut-être désactiver ce paramètre.

Pour des instructions détaillées sur la désactivation du mode privilégié et une liste complète des métriques qui seront indisponibles, consultez [Mode privilégié vs non privilégié](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode).
...
* Mode privilégié Windows sur les clusters Google GKE, car GKE ne prend pas en charge les conteneurs Windows `HostProcess`. Vous pouvez toujours monitorer les nœuds Windows sur GKE en [mode non privilégié](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode/#windows-nodes).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        OKE utilise CRI-O comme runtime de conteneur par défaut. Lors de la configuration de l&apos;attachement automatique APM, ajoutez `docker.io/` au début des chemins d&apos;image de l&apos;Agent. Par exemple : `image: docker.io/newrelic/newrelic-java-init:latest`.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Compatible avec VKS version 3.x, fonctionnant sur vSphere Supervisor 9.0 à 9.1 (vSphere 8.x et 9.x)
      </td>
    </tr>
  </tbody>
</table>
