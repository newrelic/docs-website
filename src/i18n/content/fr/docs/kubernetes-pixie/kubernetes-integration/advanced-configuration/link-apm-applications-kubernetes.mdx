...
<CollapserGroup>
  ...
  <Collapser id="no-metadata-in-transactions-attributes" title="Aucune métadonnées Kubernetes dans l'attribut transaction">
    ...
    1. Vérifiez que les variables d&apos;environnement ont été correctement injectées dans les nouveaux pods en suivant les instructions décrites dans la section [Valider l&apos;injection de métadonnées](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection).

    2. S&apos;ils n&apos;existent pas, tout d&apos;abord, suivez les logs du service webhook :

       ```shell
       kubectl logs -l app.kubernetes.io/name=nri-metadata-injection -f
       ```

    3. Dans un autre terminal, créez un nouveau pod et inspectez les logs du déploiement de l&apos;injection de métadonnées pour rechercher des erreurs. Consultez la section [Valider l&apos;injection de métadonnées](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection) pour créer un nouveau pod. Pour chaque pod créé, il devrait y avoir une entrée de log, à condition que le [niveau de log (logLevel)](https://github.com/newrelic/k8s-metadata-injection/blob/main/charts/nri-metadata-injection/README.md#values) soit défini sur INFO ou inférieur :
    ...
    7. Vérifiez que les tâches de configuration de l&apos;injection de métadonnées se sont exécutées avec succès en inspectant la sortie de cette commande :
