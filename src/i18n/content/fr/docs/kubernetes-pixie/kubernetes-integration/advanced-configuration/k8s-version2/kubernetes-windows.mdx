...
<Callout variant="important">
  **Le monitoring des nœuds Windows est désormais disponible en version générale (GA) dans l&apos;intégration Kubernetes New Relic v3.** L&apos;approche d&apos;installation v2 décrite sur cette page est obsolète et sera supprimée dans une future sortie. Utilisez le [guide d&apos;installation de l&apos;intégration Kubernetes](/install/kubernetes) pour obtenir les instructions d&apos;installation actuelles, qui prennent en charge nativement les nœuds Windows.
</Callout>

Utilisez cette option lorsque vous avez un système Kubernetes basé sur Windows. Notez que l’intégration Windows présente plusieurs [limitations](#k8-windows-limitations).
