...
Notez que la nouvelle [architecture`nri-kubernetes` v3](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) ne prend pas encore en charge Windows. Nous installons le graphique `newrelic-infrastructure` en tant que sortie autonome, ce qui vous permet de monitorer votre charge de travail Windows .
