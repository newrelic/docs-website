...
<CollapserGroup>
  <Collapser id="proxy-chart" title="Utilisation de la section « proxy » dans le graphique Helm">
    Vous pouvez utiliser la section `proxy` de votre fichier [`values.yaml`](/docs/kubernetes-pixie/k8s-otel/install/#config-params) pour la configuration du graphique `nr-k8s-otel-collector`.
