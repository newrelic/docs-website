...
## Compatibilité avec les distributions et fournisseurs Kubernetes [#enable-provider]
...
Si votre fournisseur est GKE Autopilot, veuillez ne pas modifier la valeur par défaut de `daemonset.containerSecurityContext.privileged`, car les conteneurs privilégiés ne sont pas pris en charge par ce fournisseur.

Si vous utilisez Amazon EKS, mettez à jour la configuration Terraform de votre cluster ou utilisez l&apos;AWS Console pour activer le Processeur `resourcedetection` via l&apos;une des méthodes suivantes :

* Augmentez la limite de sauts IMDS : définissez le `http_put_response_hop_limit` sur 2 pour vos groupes de nœuds.
* Activer l&apos;identité de pod : ajoutez le module complémentaire `eks-pod-identity-agent` à votre module EKS.
...
Si votre fournisseur est OpenShift et que `receivers.filelog` est activé (par défaut), veuillez effectuer les mises à jour suivantes dans `daemonset.containerSecurityContext` pour garantir que le logging dispose des privilèges nécessaires pour fonctionner :
