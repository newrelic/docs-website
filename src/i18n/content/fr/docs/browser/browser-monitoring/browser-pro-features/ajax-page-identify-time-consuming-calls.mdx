...
Diagnostiquer les échecs AJAX est difficile lorsque vous ne disposez que de métadonnées telles que l&apos;URL, le code d&apos;état et la durée. Les framework modernes comme GraphQL rendent cela encore plus difficile — les erreurs peuvent se cacher à l&apos;intérieur du corps de la réponse même lorsqu&apos;une requête semble réussie.

Notre interface utilisateur <InlinePopover type="browser" /><DNT>AJAX</DNT> vous donne une visibilité complète sur les requests AJAX depuis les navigateurs vers des points de terminaison HTTP ou HTTPS externes. Vous pouvez identifier les appels chronophages ou en échec, capturer les charges complètes de requête et de réponse, et effectuer une trace des problèmes sur l&apos;ensemble de votre stack. Vous pouvez également examiner les problèmes par type d&apos;appareil, y compris les navigateurs de desktops, les appareils mobiles et les tablettes.
...
## Examiner les requests AJAX [#ajax-dashboard]
...
Une fois que vous avez ciblé une requête AJAX, il est temps d&apos;enquêter. Essayez certaines de ces stratégies :
...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            * Utilisez l’une des options <DNT>**Sort by**</DNT> disponibles dans l’interface utilisateur. Toutes les requests AJAX qui prennent plus de deux minutes sont filtrées automatiquement.
  ...
  <Collapser id="ajax-performancex" title="Obtenez des détails par appels ou pages vues">
    ...
    Le point de terminaison AJAX sur le graphique <DNT>**Average response time**</DNT> apparaît sous la forme de quatre ou deux périodes. Cela dépend s&apos;il existe [des en-têtes de tracing inter-applications](/docs/apm/transactions/cross-application-traces/cross-application-tracing#security).

    * Quatre périodes (<DNT>**Application time**</DNT>, <DNT>**Queue time**</DNT>, <DNT>**Network time**</DNT> et <DNT>**Callback time**</DNT>) : ces appels AJAX sont effectués sur le moniteur d&apos;applications par des agents <InlinePopover type="apm" />qui fournissent des en-têtes de tracing inter-applications.
    * Deux périodes (<DNT>**Response time**</DNT> et <DNT>**Callback time**</DNT>) : ces appels AJAX sont effectués sur des applications qui ne sont pas monitorées par des agents APM ou qui sont monitorées par des agents qui ne fournissent pas d&apos;en-têtes de tracing inter-applications.
  </Collapser>

  <Collapser id="payload-capture" title="Inspecter les charges de requête et de réponse">
    ...
    Lorsque les métadonnées telles que l&apos;URL, le code d&apos;état et la durée ne suffisent pas pour diagnostiquer une défaillance, l&apos;option de capture de charge vous donne une vue d&apos;ensemble. Elle ajoute quatre nouveaux attributs à chaque événement `AjaxRequest`, chacun tronqué à 4 Ko et interrogeable dans NRQL. Vous pouvez également utiliser le bouton <DNT>**View payload**</DNT> pour voir la charge dans l&apos;interface utilisateur des détails AJAX :
    ...
    * `response.body`
    ...
    Pour activer la capture de charge, accédez aux [paramètres de votre application et configurez l&apos;option de capture de charge](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/#enable-payload-capture).
    ...
    <figcaption>
      <DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &gt; Browser &gt; (select an app) &gt; AJAX &gt; (select an AJAX call)**</DNT>: cet exemple montre les détails de la requête pour un appel AJAX spécifique. Vous pouvez également afficher les détails de la requête par pages vues.
    </figcaption>

    ### Comment la capture de charge interagit avec la liste de refus [#deny-list-interaction]

    La capture de charge respecte votre [liste de refus de requêtes AJAX](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/) existante. Pour toute requête AJAX :
    ...
    * Si l&apos;URL ne figure pas sur la liste de refus, en fonction de vos paramètres de capture de charge, l&apos;agent capture les charges de requête et de réponse et applique des règles d&apos;obfuscation avant d&apos;envoyer les données à New Relic.
    ...
    ### Protection des PII [#pii-protection]

    L&apos;agent Browser obfusque automatiquement les modèles de données sensibles courants dans les charges capturées avant l&apos;ingestion, y compris les numéros de carte de crédit, les numéros de sécurité sociale et les clés courantes comme `password` et `token`. Vous pouvez également configurer des règles regex personnalisées pour masquer les données sensibles spécifiques à l&apos;entreprise. Pour plus d&apos;informations, consultez [Obfusquer les données de navigateur](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/#ingest-obfuscation).
  </Collapser>
...
* Contrôlez quelles URL sont éligibles pour la capture de charge en configurant la [liste de refus des requests AJAX](/docs/browser/new-relic-browser/configuration/filter-ajax-request-events/).