...
* L&apos;option `waitForEnd` nécessite v1.285.0+

* L&apos;option `targetPageLoad` nécessite v1.313.1+
...
* <DNT>**Page load interaction:**</DNT> Si l&apos;option `targetPageLoad` est utilisée, le handle est toujours lié à l&apos;interaction de chargement initial de la page, indépendamment de toute navigation douce qui pourrait être en cours.

<Callout variant="caution">
  L&apos;utilisation de `waitForEnd: true` conjointement avec `targetPageLoad: true` maintiendra l&apos;interaction de chargement initial de la page ouverte indéfiniment jusqu&apos;à ce que `.end()` soit appelé, modifiant la définition par défaut de l&apos;IPL. C&apos;est un cas d&apos;utilisation avancé et il doit être utilisé avec prudence.
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `targetPageLoad` - La valeur par défaut est `false`. S&apos;il est défini sur `true`, le handle renvoyé est lié à l&apos;interaction de chargement initial de la page au lieu de créer ou de cibler une interaction de soft navigation. C&apos;est la seule façon de cibler l&apos;interaction initialPageLoad.
      </td>
    </tr>
  </tbody>
</table>
