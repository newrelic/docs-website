...
* Si vous installez l&apos;agent browser via npm et créez un agent personnalisé avec des fonctionnalités sélectionnées, vous devez activer la fonctionnalité `generic_events` et/ou `session_trace` lors de la création de l&apos;instance `Agent`. Dans le tableau `features`, ajoutez ce qui suit :
