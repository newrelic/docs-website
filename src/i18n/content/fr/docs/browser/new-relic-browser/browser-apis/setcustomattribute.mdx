...
* Si vous installez l&apos;agent browser via npm et que vous construisez un agent personnalisé avec des fonctionnalités sélectionnées, vous devez activer au moins une fonctionnalité lors de la création de l&apos;instance `Agent`. Par exemple, ajoutez ce qui suit dans le tableau`features` :
