...
Browser rapporte de nombreux types de données différents pour vous aider à analyser les performances de votre site Web. Il <DNT>**only**</DNT> rapporte les données de consultation de page, à moins que vous n’ayez souscrit aux [fonctionnalités Pro](https://newrelic.com/pricing). Vous pouvez également [activer la fonctionnalité](/docs/browser/new-relic-browser/welcome-new-relic-browser/new-relic-browser#enhanced-features) pour les requests AJAX, les erreurs JavaScript et les traces de session.
...
<Callout variant="important">
  **Région JP :** les fournisseurs de CDN de New Relic ont des points de terminaison au Japon et les utilisent dans des conditions normales. Cependant, lors d’un événement de panne du CDN ou de problème de performances, le script de l’agent de navigateur peut être servi à partir de points de terminaison en dehors du Japon. Cela n’affecte pas l’endroit où vos données télémétriques sont envoyées ou stockées.
</Callout>
