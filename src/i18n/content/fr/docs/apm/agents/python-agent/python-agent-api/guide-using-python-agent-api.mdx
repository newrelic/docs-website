...
## Nommer dynamiquement les segments et les attributs de segment [#dynamically-name-segments-and-segment-attributes]

Pour nommer dynamiquement les segments et les valeurs d&apos;attribut de segment, utilisez les appels d&apos;API d&apos;instrumentation personnalisée, qui acceptent une fonction au lieu d&apos;une valeur statique pour divers paramètres. Avant la version 13.0.0 de l&apos;agent, vous pourriez également le faire dans le fichier de configuration en utilisant une fonction Lambda. Ceci est désormais uniquement pris en charge dans les appels d&apos;API d&apos;instrumentation personnalisée.

Les appels d&apos;API suivants prennent en charge le nommage dynamique :
