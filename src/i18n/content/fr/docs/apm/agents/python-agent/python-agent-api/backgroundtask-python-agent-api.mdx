...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Requis. Le nom de la transaction. Il pourrait s’agir d’une fonction qui accepte un paramètre callable\_name. La valeur par défaut est `None`. Par défaut, le nom sera celui de la fonction à laquelle le décorateur a été appliqué, mais vous pouvez le remplacer en fournissant ce nom.
      </td>
    </tr>
...
Un exemple d&apos;utilisation de `BackgroundTask` avec un nom dynamique :
