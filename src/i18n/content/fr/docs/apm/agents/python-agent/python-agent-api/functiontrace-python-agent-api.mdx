...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Facultatif. Le nom de la fonction. Il pourrait s’agir d’une fonction qui accepte un paramètre callable\_name. S&apos;il n&apos;est pas défini, la valeur par défaut est le nom capturé de la fonction.
      </td>
    </tr>
...
Si vous souhaitez nommer la métrique de manière dynamique (plutôt que de nommer la métrique à partir de l&apos;identifiant de la fonction appelée), vous pouvez fournir le nom en tant que fonction lors de la création de l&apos;objet `FunctionTraceWrapper`.
