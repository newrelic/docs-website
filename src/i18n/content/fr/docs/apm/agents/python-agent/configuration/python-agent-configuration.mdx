...
<CollapserGroup>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Il s&apos;agit de l&apos;échantillonneur appliqué lorsque la trace a un parent distant (c&apos;est-à-dire que la trace provient d&apos;un service en amont) qui a été échantillonné. Les options de l&apos;échantillonneur sont :

    * `adaptive` est le comportement par défaut et délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage adaptatif pour déterminer si une trace sera échantillonnée.
    * `trace_id_ratio_based` délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage basé sur le ratio d&apos;ID de trace pour déterminer si une trace sera échantillonnée. Si vous utilisez cette option, le ratio est requis, utilisez donc [distributed\_tracing.sampler.remote\_parent\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio) à la place.
    * `always_on`: Échantillonne toujours les traces qui ont un parent distant échantillonné.
    * `always_off`: N&apos;échantillonne jamais les traces qui ont un parent distant échantillonné.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote-parent-sampled-trace-id-ratio-based-ratio" title="distributed_tracing.sampler.remote_parent_sampled.trace_id_ratio_based.ratio">
    ...
    * Un nombre entre 0 (exclu) et 1 inclus.
  </Collapser>
  ...
  <Collapser id="distributed-tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Il s&apos;agit de l&apos;échantillonneur appliqué lorsqu&apos;une trace a un parent distant (c&apos;est-à-dire que la trace provient d&apos;un service en amont) qui n&apos;a pas été échantillonné. Les options de l&apos;échantillonneur sont :

    * `adaptive` est le comportement par défaut et délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage adaptatif pour déterminer si une trace sera échantillonnée.
    * `trace_id_ratio_based` délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage basé sur le ratio d&apos;ID de trace pour déterminer si une trace sera échantillonnée. Si vous utilisez cette option, le ratio est requis, utilisez donc [distributed\_tracing.sampler.remote\_parent\_not\_sampled.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-remote-parent-not-sampled-trace-id-ratio-based-ratio) à la place.
  ...
  <Collapser id="distributed-tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    C&apos;est l&apos;échantillonneur qui est appliqué au début d&apos;une trace. Par défaut, l&apos;Adaptive Sampler est configuré en tant qu&apos;échantillonneur racine. Les options de l&apos;échantillonneur sont :

    * `adaptive` est le comportement par défaut et délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage adaptatif pour déterminer si une trace sera échantillonnée.
    * `trace_id_ratio_based` délègue la décision d&apos;échantillonnage à l&apos;algorithme d&apos;échantillonnage basé sur le ratio d&apos;ID de trace pour déterminer si une trace sera échantillonnée. Si vous utilisez cette option, le ratio est requis, utilisez donc [distributed\_tracing.sampler.root.trace\_id\_ratio\_based.ratio](#distributed-tracing-sampler-root-trace-id-ratio-based-ratio) à la place.
    * `always_on`: Échantillonne toujours les transactions qui proviennent du service actuel et ne comportent pas d&apos;en-tête de tracing distribué pour l&apos;appel d&apos;entrée.
    * `always_off`: N&apos;échantillonne jamais les transactions qui proviennent du service actuel et ne possèdent pas d&apos;en-tête de tracing distribué pour l&apos;appel d&apos;entrée.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="cfg-tt-enabled" title="span_events.enabled">
    ...
    Ce paramètre peut être utilisé pour activer ou désactiver l&apos;envoi d&apos;spans par l&apos;agent Python.
  </Collapser>
...
## Paramètres de l&apos;agent hybride [#otel-settings]

<Callout variant="important">
  Nécessite [l&apos;agent Python version 11.5.0 ou supérieure](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
</Callout>

Pour plus d’informations sur les fonctionnalités de l’agent hybride, consultez [cette page](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support).

Les paramètres suivants sont disponibles pour la configuration de l&apos;agent hybride :
...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="opentelemetry.enabled">
    ...
    De v11.5.0 à v13.0.0 : Si activé, toute API OpenTelemetry et la plupart des [frameworks instrumentés OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) installés seront utilisés. À partir de la v13.0.0+ : Si cette option est activée, toutes les API et tous les frameworks OpenTelemetry qui sont [pris en charge par OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) mais pas par New Relic seront utilisés.

    <Callout variant="important">
      Bien que la plupart des bibliothèques dans les [frameworks instrumentés d’OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) soient prises en charge, les bibliothèques d’instrumentation AWS et gRPC continueront d’utiliser l’instrumentation de New Relic. Les bibliothèques de ML/IA avec instrumentation native sont explicitement désactivées pour l&apos;instant et seront prises en charge dans une future sortie.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.enabled" title="opentelemetry.traces.enabled">
    ...
    De la v11.5.0 à la v13.0.0 : Si activé, toutes les API de span et de trace OpenTelemetry seront utilisées, indépendamment de la présence d&apos;une bibliothèque d&apos;instrumentation New Relic existante. À partir de la v13.0.0+ : Si activé, tout appel manuel utilisant l&apos;API de span OpenTelemetry et les spans de frameworks qui sont [pris en charge par OpenTelemetry](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme) mais pas par New Relic seront utilisés.
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.exclude" title="opentelemetry.traces.exclude">
    <table>
      <tbody>
        <tr>
          ...
          <td>
            Liste de chaînes de caractères, séparées par des virgules
          </td>
        </tr>
    ...
    Permet aux utilisateurs d&apos;exclure le(s) traceur(s) des rapports envoyés à New Relic. Consultez la [section ci-dessous](#include-exclude-rules) pour plus de détails.

    <Callout variant="important">
      Nécessite [l’agent Python version 13.0.0 ou supérieure](/docs/agents/python-agent/installation-configuration/upgrade-python-agent).
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.include" title="opentelemetry.traces.include">
    ...
    Permet aux utilisateurs d&apos;inclure des traceur(s) à transmettre à New Relic. Consultez la [section ci-dessous](#include-exclude-rules) pour plus de détails.
...
Règles d&apos;inclusion et d&apos;exclusion \[#include-exclude-rules]

New Relic suit ces règles pour déterminer quels traceurs inclure ou exclure pour le monitoring.

<CollapserGroup>
  <Collapser id="rule-precedence" title="Activé a priorité sur inclure et exclure.">
    `opentelemetry.traces.enabled` : ce flag prévaut sur les paramètres d&apos;inclusion (include) et d&apos;exclusion (exclude).
  ...
  <Collapser id="rule-trace-included" title="La trace est incluse si le tracing est activé.">
    Si le tracing est activé, tous les traceurs OpenTelemetry personnalisés et les traceurs de bibliothèques OpenTelemetry non pris en charge par New Relic sont monitorés par défaut.
  ...
  <Collapser id="rule-exclude-wins" title="Exclure remplace toujours inclure.">
    Si le même traceur figure dans les listes d&apos;inclusion et d&apos;exclusion, alors le traceur sera exclu.
  ...
  <Collapser id="rule-tracers-case-sensitive" title="Les traceurs sont sensibles à la casse.">
    Les traceurs sont sensibles à la casse.
...
<CollapserGroup>
  ...
  <Collapser id="cross-application-tracer" title="cross_application_tracer.enabled">
    ...
    <Callout variant="caution">
      [Le tracing inter-application (CAT)](/docs/apm/transactions/cross-application-traces/introduction-cross-application-traces) a été complètement supprimé dans la version 12.0.0.
    </Callout>
  </Collapser>
