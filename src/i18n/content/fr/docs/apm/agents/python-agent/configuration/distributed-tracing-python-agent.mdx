...
* Tracing infini (échantillonnage en fin de workflow) : Notre service basé sur le cloud accepte toutes vos traces puis les trie pour trouver les plus importantes. Le tracing infini analyse toutes vos traces et vous offre des options configuration pour échantillonner les traces qui comptent le plus pour vous.

Que vous souhaitiez simplement tester le tracing standard distribué (échantillonnage en début de workflow) ou que vous souhaitiez également mettre en place un tracing infini (échantillonnage en fin de workflow), vous devez commencer par mettre en place le tracing standard.
...
<Steps>
  ...
  <Step>
    ### (Facultatif) Personnaliser le tracing infini [#customize-inf]
