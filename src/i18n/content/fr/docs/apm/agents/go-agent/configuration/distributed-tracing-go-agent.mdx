...
Le traçage distribué vous permet de visualiser le parcours complet de vos requêtes à travers un [système distribué](/docs/distributed-tracing/concepts/introduction-distributed-tracing). Pour l&apos;agent Go, nous proposons deux types de traçage distribué (pour plus de détails, consultez [Fonctionnement de l&apos;échantillonnage de spans](/docs/distributed-tracing/concepts/how-new-relic-distributed-tracing-works/#sampling)) :

* Standard (échantillonnage en tête) : avant l&apos;arrivée de toute trace, nous déterminons un pourcentage défini de traces à accepter et à analyser. Cela vous donne un point de départ solide pour voir comment le traçage peut vous aider. Il est activé par défaut dans les agents Go 3.16.0 et versions ultérieures. Par défaut, l&apos;agent Go échantillonne 10 traces par minute, avec une limite maximale de 2 000 spans par minute.
