...
<Callout variant="tip">
  New Relic accepte les contributions de la communauté pour l’agent <DNT>Ruby</DNT>. Si vous avez un gem que vous souhaitez voir instrumenté ou une fonctionnalité à proposer, dans le référentiel `newrelic/newrelic-ruby-agent` sur GitHub, partagez vos idées en ouvrant un [ticket](https://github.com/newrelic/newrelic-ruby-agent/issues/new/choose) ou en soumettant une [demande de tirage](https://github.com/newrelic/newrelic-ruby-agent).
</Callout>

## Trouver des extensions tierces

N’importe qui peut écrire une gem qui s’appuie sur l’agent <DNT>Ruby</DNT>. D’anciens exemples de cela sont disponibles dans un référentiel archivé appelé [extends\_newrelic\_rpm](https://github.com/newrelic/extends_newrelic_rpm) pour suivre ces extensions et fournir des liens vers d’autres gems qui construisent l’agent <DNT>Ruby</DNT>.

Ces extensions ne sont **pas** prises en charge par New Relic. New Relic a rassemblé ces liens en tant que service à nos clients. Les problèmes avec ces gems doivent être signalés aux projets respectifs sur GitHub.

## Les extensions comme des joyaux

Si vous choisissez de créer une extension tierce, nous recommandons une gem par bibliothèque instrumentée. Par exemple, `newrelic-redis` fournit une instrumentation pour la gem `redis` (qui existe également automatiquement au sein de notre agent).
...
<CollapserGroup>
  ...
  <Collapser id="notice_sql" title="NewRelic::Agent::Datastores.notice_sql">
    ...
    <Callout variant="caution">
      La fonctionnalité de tracing des transactions et de SQL lent de New Relic tentera d&apos;appliquer obfuscation à la requête transmise, mais il est possible qu&apos;un format de requête ne soit pas pris en charge et entraîne l&apos;exposition des informations de l&apos;utilisateur intégrées dans la requête capturée.
    </Callout>
  </Collapser>
