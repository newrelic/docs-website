...
<CollapserGroup>
  ...
  <Collapser id="log_level" title="log_level">
    ...
    Définit le niveau de détail du message de log. Les niveaux de log possibles, par ordre croissant de verbosité, sont : `fatal`, `error`, `warn`, `info` ou `debug`.
  </Collapser>
  ...
  <Collapser id="active_support_custom_events_names" title="active_support_custom_events_names">
    ...
    Un éventail de noms d’événements personnalisés ActiveSupport auxquels s’abonner et à instrument. Par exemple : \[one.custom.event, another.event, a.third.event]
  </Collapser>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Si `true`, l’agent utilisera le nom de la table du modèle ActiveRecord au lieu du nom de la classe lors du nommage des métriques, des spans et des segments de trace de transaction ActiveRecord. Cela peut réduire la cardinalité lorsque plusieurs modèles partagent une table de base de données. La valeur par défaut est `false`.
  </Collapser>
  ...
  <Collapser id="capture_memcache_keys" title="capture_memcache_keys">
    ...
    Si `true`, l’agent capture les clés memcache dans la trace de transaction.
  </Collapser>
  ...
  <Collapser id="config_path" title="config_path">
    ...
    Chemin vers `newrelic.yml`. Si non défini, l’agent vérifie les répertoires suivants (dans l’ordre) : `config/newrelic.yml` -&gt; `newrelic.yml` -&gt; `$HOME/.newrelic/newrelic.yml` -&gt; `$HOME/newrelic.yml`
  </Collapser>
  ...
  <Collapser id="exclude_newrelic_header" title="exclude_newrelic_header">
    ...
    Si `true`, supprime l’en-tête de tracing distribué `newrelic` sur les requests sortantes.
  </Collapser>
  ...
  <Collapser id="force_install_exit_handler" title="force_install_exit_handler">
    ...
    Si `true`, force l’agent à installer son gestionnaire de sortie (qui envoie toutes les données mises en cache au collecteur avant de s’arrêter), même dans les cas où l’agent l’ignorerait normalement, comme lorsqu’il détecte que Sinatra s’exécute en tant que service intégré dans un autre framework. Sinatra exécute l’application entière dans son propre bloc `at_exit`, l’agent ignore donc son propre gestionnaire `at_exit` par défaut pour éviter les conflits. Ce paramètre remplace ce comportement. Remarque : `send_data_on_exit` doit également être `true` pour que ce paramètre prenne effet.
  </Collapser>
  ...
  <Collapser id="log_file_path" title="log_file_path">
    ...
    Définit un chemin vers le fichier de log de l&apos;agent, sans le nom du fichier. Si vous souhaitez envoyer les logs de votre agent vers la sortie standard, définissez ce paramètre sur `STDOUT`.
  </Collapser>
  ...
  <Collapser id="simplify_action_cable_broadcast_metrics" title="simplify_action_cable_broadcast_metrics">
    ...
    Si `true`, les métriques des appels de diffusion Action Cable n&apos;incluront pas la valeur `broadcasting` pour une diffusion donnée. Par défaut `false`. La valeur par défaut peut changer lors d&apos;une future sortie de version majeure.
  </Collapser>
  ...
  <Collapser id="ignored_middleware_classes" title="ignored_middleware_classes">
    ...
    Une liste de noms de classes ou de modules de middleware que l&apos;agent doit ignorer. Exemple : \[&quot;Rack::Cors&quot;, &quot;ActionDispatch::Reloader&quot;]
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    ...
    Si `true`, une fois que [`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments) est atteint, l’agent arrêtera également d’enregistrer le temps exclusif pour tous les segments créés par la suite dans cette transaction. Cela peut réduire l’utilisation de la mémoire pour les transactions très longues avec de nombreux segments, au prix de données de chronométrage moins précises pour les segments de la transaction si la limite de segments est atteinte.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-expected_status_codes" title="error_collector.expected_status_codes">
    ...
    Une liste séparée par des virgules de codes d’état HTTP et/ou de plages de codes d’état (par exemple, `404, 429, 500-599`). Les erreurs associées à un code d’état correspondant, si l’erreur en a un, seront traitées comme prévu.
  </Collapser>
  ...
  <Collapser id="error_collector-ignore_status_codes" title="error_collector.ignore_status_codes">
    ...
    Une liste séparée par des virgules de codes d’état HTTP et/ou de plages de codes d’état (par exemple, `404, 429, 500-599`). Les erreurs associées à un code d’état correspondant, si l’erreur en a un, seront ignorées.
  </Collapser>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    Définit le nombre maximal de frames dans une trace d’erreur. Les backtraces dépassant cette quantité sont tronquées au milieu par défaut, préservant le début et la fin de la trace d’appels. Consultez [`error_collector.backtrace_truncate_location`](#error_collector-backtrace_truncate_location) pour personnaliser l’emplacement de la troncature.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    Spécifie où tronquer la trace de pile lorsque le nombre de cadres dépasse [`error_collector.max_backtrace_frames`](#error_collector-max_backtrace_frames). Les options sont `top` (supprimer les cadres du début), `middle` (supprimer les cadres du milieu, en conservant le début et la fin) ou `end` (supprimer les cadres de la fin).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="browser_monitoring-version" title="browser_monitoring.version">
    ...
    Épinglez la version du chargeur de l&apos;agent de navigateur que New Relic injecte, telle que `1.317.0`. S’il n’est pas défini, New Relic injecte la dernière version disponible. Consultez la [politique de fin de vie de l’agent de navigateur](/docs/browser/browser-monitoring/getting-started/browser-agent-eol-policy/) pour connaître les versions actuellement disponibles et prises en charge.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="application_logging-forwarding-log_level" title="application_logging.forwarding.log_level">
    ...
    Définit le niveau minimum qu’un événement de log doit avoir pour être transféré à New Relic. Les logs au niveau configuré et de toute gravité supérieure sont transférés, par exemple, `debug` transfère tous les événements de log, tandis que `error` transfère uniquement les événements `error`, `fatal`, et `unknown`. Valeurs valides, de la gravité la plus faible à la plus élevée : `debug`, `info`, `warn`, `error`, `fatal`, `unknown` (insensibles à la casse).

    Cet ordre est basé sur les valeurs entières des constantes `Logger::Severity` de Ruby (voir [https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb](https://github.com/ruby/logger/blob/113b82a06b3076b93a71cd467e1605b23afb3088/lib/logger/severity.rb)).
  </Collapser>
...
<CollapserGroup>
  <Collapser id="ai_monitoring-enabled" title="ai_monitoring.enabled">
    ...
    Si `false`, toute instrumentation LLM (OpenAI uniquement pour l&apos;instant) sera désactivée et aucune métrique, événement ou span ne sera envoyé. Le monitoring de l&apos;IA est automatiquement désactivée si le mode `high_security` est activé.
  </Collapser>
  ...
  <Collapser id="ai_monitoring-record_content-enabled" title="ai_monitoring.record_content.enabled">
    ...
    Si `false`, l&apos;instrumentation LLM (OpenAI uniquement pour le moment) ne capturera pas l&apos;attribut `content` sur les événements LlmChatCompletionMessage ou l&apos;attribut `input` sur les événements LlmEmbedding. Il s&apos;agit d&apos;un paramètre de sécurité facultatif pour empêcher l&apos;enregistrement des données sensibles envoyées à vos LLM et reçues de ces derniers.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="audit_log-endpoints" title="audit_log.endpoints">
    ...
    Liste des points de terminaison autorisés à inclure dans le log d’audit.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="cloud-aws-account_id" title="cloud.aws.account_id">
    ...
    L’ID de compte AWS associé à cette application.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="custom_insights_events-max_samples_stored" title="custom_insights_events.max_samples_stored">
    ...
    Spécifiez un nombre maximal d’événements personnalisés à mettre en mémoire tampon à la fois. Lors de la configuration de l’agent pour [monitoring de l’IA](/docs/ai-monitoring/intro-to-ai-monitoring), définissez la valeur maximale sur `100000`. Cela garantit que l’agent capture la quantité maximale d’événements LLM.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="datastore_tracer-database_name_reporting-enabled" title="datastore_tracer.database_name_reporting.enabled">
    ...
    Si `false`, l’agent n’ajoutera pas le paramètre `database_name` à la transaction ou aux traces SQL lentes.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="disable_active_job_step_names" title="disable_active_job_step_names">
    ...
    Si `true`, les noms d&apos;étape de Rails Active Job Continuations ne seront pas inclus dans les noms de métriques. Cela réduit la cardinalité des métriques mais réduit également la visibilité sur la performance des étapes individuelles. Définissez ceci sur `true` uniquement si vous rencontrez des problèmes de cardinalité de métriques dus à des jobs comportant de nombreuses étapes ou des étapes nommées dynamiquement.
  </Collapser>
  ...
  <Collapser id="disable_gc_profiler" title="disable_gc_profiler">
    ...
    Si `true`, désactive l&apos;utilisation de `GC::Profiler` pour mesurer le temps passé lors de la récupération de mémoire.
  </Collapser>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    Si `true`, désactive le middleware de l&apos;agent pour Sinatra. Ce middleware est responsable de la prise en charge des fonctionnalités avancées telles que [le suivi distribué](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing), [le chronométrage du chargement de la page](/docs/browser/new-relic-browser/getting-started/new-relic-browser) et [la collecte des erreurs](/docs/apm/applications-menu/events/view-apm-error-analytics).
  </Collapser>
  ...
  <Collapser id="disable_puma_instrumentation" title="disable_puma_instrumentation">
    ...
    Si `true`, désactive l’instrumentation des statistiques du serveur Puma qui échantillonne les métriques `Ruby/Puma/*` à partir du processus maître Puma. Cette instrumentation est désactivée par défaut. Lorsqu’elle est activée, l’agent démarre un thread de reporting dans le processus maître Puma pour fournir ces métriques.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-root" title="distributed_tracing.sampler.root">
    ...
    Ce paramètre contrôle le comportement de l’échantillonnage des transactions pour les transactions sans parent distant (les traces qui proviennent de cette instance de l’agent New Relic). Les valeurs disponibles sont `adaptive` (par défaut), `always_on`, `always_off` et `trace_id_ratio_based`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Ce paramètre contrôle le comportement de l&apos;échantillonnage des transactions lorsqu&apos;un parent distant est échantillonné. Les valeurs disponibles sont `adaptive` (la valeur par défaut), `always_on`, `always_off` et `trace_id_ratio_based`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Ce paramètre contrôle le comportement de l&apos;échantillonnage des transactions lorsqu&apos;un parent distant n&apos;est pas échantillonné. Les valeurs disponibles sont `adaptive` (par défaut), `always_on`, `always_off` et `trace_id_ratio_based`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="instrumentation.active_support_notifications.active_support_events">
    ...
    Une liste d’autorisation de noms d’événements de notification ActiveSupport (principalement des événements de mise en cache, et des événements de sérialisation de messages) auxquels l’agent s’abonne. Tout nom d’événement non inclus ici est ignoré. Fournissez uniquement des noms d’événements complets, tels que `cache_fetch_hit.active_support`. Ne fournissez pas d’astérisques ou de modèles regex, et n’échappez à aucun caractère avec des barres obliques inversées.

    Pour la liste complète des noms d’événements possibles, consultez la liste des [noms de mise en cache](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching), et des [noms de messages](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) de la documentation de Rails.
  </Collapser>
  ...
  <Collapser id="instrumentation-semantic_logger" title="instrumentation.semantic_logger">
    ...
    Contrôle l’auto-instrumentation de la bibliothèque semantic\_logger au démarrage. Peut être l&apos;un de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-parallel" title="instrumentation.parallel">
    ...
    Contrôle l&apos;auto-instrumentation de la bibliothèque parallèle au démarrage. Peut être l&apos;un de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-ethon" title="instrumentation.ethon">
    ...
    Contrôle l’auto-instrumentation d’ethon au démarrage. Peut être l’un de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-grpc-host_denylist" title="instrumentation.grpc.host_denylist">
    ...
    Une liste séparée par des virgules de modèles d’expressions régulières correspondant aux noms d’hôte gRPC dont New Relic doit ignorer le trafic. L’instrumentation du client gRPC de New Relic ignore le tracing des appels effectués vers un hôte correspondant à l’un de ces modèles, et son instrumentation du serveur gRPC ignore le tracing des requests reçues par un serveur s’exécutant sur un hôte correspondant. Par exemple, `"private.com$,exception.*" Empty by default, meaning no traffic is ignored.`.
  </Collapser>
  ...
  <Collapser id="instrumentation-httpx" title="instrumentation.httpx">
    ...
    Contrôle l’auto-instrumentation de httpx au démarrage. Peut être l’un de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-logging" title="instrumentation.logging">
    ...
    Contrôle l&apos;auto-instrumentation de la bibliothèque de logging au démarrage. Peut être l&apos;un de `auto`, `prepend`, `chain`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger" title="instrumentation.rails_event_logger">
    ...
    Contrôle l&apos;instrumentation de Rails.event sous forme de logs structurés. Peut être l&apos;un des suivants : `enabled`, `disabled`.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Un éventail de noms de `Rails.event` à capturer en tant qu’événements de log structurés. Par exemple : \[utilisateur.signup, payment.processed, order.created]. Laissez vide pour capturer toutes les notifications `Rails.event`. Les événements sont enregistrés dans le log avec le niveau `UNKNOWN` (garantissant qu’ils ne sont jamais filtrés), à moins d’être remplacés via une clé `:level` dans la charge de l’événement.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Une option de configuration globale pour désactiver tous les signaux OpenTelemetry envoyés via New Relic. Si défini sur `false`, aucun signal OpenTelemetry ne sera envoyé à New Relic. Si défini sur `true`, l’option de configuration d’activation spécifique au signal (par ex. opentelemetry.traces.enabled) détermine si la télémétrie de ce type de signal sera rapportée à New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    Permet la création de segments de traces de transaction et de métriques de tranches de temps à partir des spans OpenTelemetry. Cela aidera à piloter l&apos;expérience de l&apos;interface utilisateur New Relic pour les spans OpenTelemetry.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-include" title="opentelemetry.traces.include">
    ...
    Une liste de traceurs OpenTelemetry délimitée par des virgules, représentée sous forme de chaîne (par ex. &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;), dont les traces **seront** envoyées à New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-exclude" title="opentelemetry.traces.exclude">
    ...
    Une liste de traceurs OpenTelemetry délimitée par des virgules, représentée sous forme de chaîne (par ex. &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;), dont les traces ne seront **pas** envoyées à New Relic.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="puma-sample_rate" title="puma.sample_rate">
    ...
    Nombre de secondes entre les échantillons de statistiques du serveur Puma prélevés dans le processus maître Puma.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rake-tasks" title="rake.tasks">
    ...
    Spécifiez un éventail de tâches Rake à instrumenter automatiquement. Cette option de configuration convertit l&apos;éventail en une liste RegEx. Aucune tâche rake ne sera instrumentée à moins qu&apos;elle ne soit ajoutée à cette liste. Si vous souhaitez autoriser toutes les tâches par défaut, utilisez `rake.tasks: [.+]`. Pour plus d&apos;informations, consultez la [documentation sur l&apos;instrumentation New Relic Rake](/docs/apm/agents/ruby-agent/background-jobs/rake-instrumentation).
  </Collapser>
  ...
  <Collapser id="rake-connect_timeout" title="rake.connect_timeout">
    ...
    Définit le nombre de secondes à attendre pour qu’une connexion aboutisse avant d’exécuter une tâche Rake.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="rules-ignore_url_regexes" title="rules.ignore_url_regexes">
    ...
    Définissez les transactions que vous souhaitez que l’agent ignore en spécifiant une liste de modèles correspondant aux URI à exclure. Pour plus de détails, consultez [la documentation pour ignorer des transactions spécifiques](/docs/agents/ruby-agent/api-guides/ignoring-specific-transactions/#config-ignoring).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="security-agent-enabled" title="security.agent.enabled">
    ...
    Si `true`, l’agent de sécurité est chargé (un « require » Ruby est effectué).
  </Collapser>
  ...
  <Collapser id="security-validator_service_url" title="security.validator_service_url">
    ...
    Définit l’URL du point de terminaison pour la publication de données liées à la sécurité.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-insecure_settings" title="security.exclude_from_iast_scan.iast_detection_category.insecure_settings">
    ...
    Si défini sur `true`, désactive la détection des paramètres non sécurisés de faible gravité. Par exemple, hacher, crypto, cookie, générateurs aléatoires, et limite de confiance.
  </Collapser>
  ...
  <Collapser id="security-exclude_from_iast_scan-iast_detection_category-xpath_injection" title="security.exclude_from_iast_scan.iast_detection_category.xpath_injection">
    ...
    Si `true`, désactive la détection d’injection XPath dans les scans IAST.
  </Collapser>
  ...
  <Collapser id="security-scan_schedule-duration" title="security.scan_schedule.duration">
    ...
    Spécifie la durée (en minutes) pendant laquelle l&apos;analyse IAST est effectuée.
  </Collapser>
  ...
  <Collapser id="security-scan_controllers-iast_scan_request_rate_limit" title="security.scan_controllers.iast_scan_request_rate_limit">
    ...
    Définit le nombre maximum de requests HTTP autorisées par minute pour l’analyse IAST. Doit être un entier compris entre 12 et 3600 (par défaut).
  </Collapser>
  ...
  <Collapser id="security-iast_test_identifier" title="security.iast_test_identifier">
    ...
    Un identifiant de test unique lors de l’exécution d’IAST dans un environnement CI/CD (intégration et livraison continues) pour différencier les différentes exécutions de test. Par exemple, un numéro de build.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="sidekiq-args-include" title="sidekiq.args.include">
    ...
    Un éventail de chaînes qui servira collectivement de liste d’autorisation pour filtrer les arguments de tâche Sidekiq qui sont rapportés à New Relic. Pour capturer tout argument Sidekiq, `job.sidekiq.args.*` doit également être ajouté à l’option de configuration `attributes.include` distincte. Chaque chaîne de cet éventail sera transformée en expressions régulières via `Regexp.new` pour permettre une correspondance avancée. Pour les hacher d’arguments de tâche, si une clé ou une valeur correspond, la paire sera incluse. Tous les éléments d’éventail d’arguments de tâche et les scalaires d’arguments de tâche correspondants seront inclus.
  </Collapser>
  ...
  <Collapser id="sidekiq-args-exclude" title="sidekiq.args.exclude">
    ...
    Un éventail de chaînes qui serviront collectivement de liste de blocage pour filtrer les arguments de tâche Sidekiq signalés à New Relic. Pour capturer tout argument Sidekiq, `job.sidekiq.args.*` doit également être ajouté à l’option de configuration `attributes.include` distincte. Chaque chaîne de cet éventail sera transformée en expressions régulières via `Regexp.new` pour permettre une correspondance avancée. Pour les hacher d&apos;arguments de tâche, si une clé ou une valeur correspond, la paire sera exclue. Tous les éléments d&apos;éventail d&apos;arguments de tâche et les scalaires d&apos;arguments de tâche correspondants seront exclus.
  </Collapser>
  ...
  <Collapser id="sidekiq-separate_transactions" title="sidekiq.separate_transactions">
    ...
    Si `true`, l&apos;agent démarre une nouvelle transaction lorsqu&apos;une tâche Sidekiq est exécutée pendant une transaction web. Cela empêche le temps d&apos;exécution des tâches Sidekiq d&apos;être inclus dans les métriques de transaction web.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="slow_sql-use_longer_sql_id" title="slow_sql.use_longer_sql_id">
    ...
    Si `true`, l’agent génère un `sql_id` plus long pour les traces SQL lentes. `sql_id` est utilisé pour l’agrégation de requêtes similaires.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="span_events-max_samples_stored" title="span_events.max_samples_stored">
    ...
    Définit le nombre maximum d’événements span signalés à partir d’une seule collecte. Tout entier entre `1` et `10000` est valide. Lors de la configuration de l’agent pour [monitoring de l’IA](/docs/ai-monitoring/intro-to-ai-monitoring), définissez la valeur maximale sur `10000`. Cela garantit que l’agent capture la quantité maximale de traces distribuées.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="strip_exception_messages-enabled" title="strip_exception_messages.enabled">
    ...
    Si `true`, l’agent supprime les messages de toutes les exceptions, sauf celles de la [liste des classes autorisées](#strip_exception_messages-allowed_classes). Activé automatiquement en [mode haute sécurité](/docs/accounts-partnerships/accounts/security/high-security).
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Si `true`, l&apos;agent signale l&apos;ID d’instance GCP comme nom d’hôte lors de son exécution sur Google Cloud Run (Services, Worker Pools, et Jobs).
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    Si `true`, l’agent ajoute le nom de révision Cloud Run au début de l’ID d’instance GCP pour former le nom d’hôte (`{revision}-{instance id}`) sur Google Cloud Run. Le nom de révision provient de `K_REVISION` sur un Cloud Run Service, `CLOUD_RUN_REVISION` sur un Cloud Run Worker Pool, et `CLOUD_RUN_EXECUTION` sur un Cloud Run Job. N’a aucun effet à moins que `utilization.gcp_cloud_run.use_instance_as_host` ne soit également `true`.
  </Collapser>
  ...
  <Collapser id="utilization-detect_in_parallel" title="utilization.detect_in_parallel">
    ...
    Si `true`, l&apos;Agent effectue la détection de l&apos;environnement en parallèle pour accélérer le temps de démarrage de l&apos;Agent.
  </Collapser>
</CollapserGroup>

## Erreurs configuration de la boîte de réception [#errors-inbox-configuration]

Définir l&apos;une des variables d&apos;environnement suivantes vous aidera à identifier quelles versions de votre logiciel ont produit les erreurs.
...
* `NEW_RELIC_METADATA_RELEASE_TAG` crée `tags.releaseTag` sur les données d’événement contenant le tag de sortie (tel que `v0.1.209` ou `release-209`).
* `NEW_RELIC_METADATA_COMMIT` crée `tags.commit` sur les données d’événement contenant le SHA de commit. Le SHA complet peut être utilisé ou seulement les sept premiers caractères (par exemple, `734713b`).

[Errors Inbox](/docs/errors-inbox/version-tracking/) suivra automatiquement les versions de votre logiciel qui produisent des erreurs.