...
<CollapserGroup>
  ...
  <Collapser id="active_record_use_table_name" title="active_record_use_table_name">
    ...
    Si `true`, l&apos;agent utilisera le nom de la table du modèle Active Record au lieu du nom de la classe pour nommer les métriques, les spans et les segments de trace de transaction Active Record. Cela peut réduire la cardinalité lorsque plusieurs modèles partagent une table de base de données. La valeur par défaut est `false`.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    Définit le nombre maximal de frames dans une trace d’erreur. Les backtraces dépassant cette quantité sont tronquées au milieu par défaut, préservant le début et la fin de la trace d’appels. Consultez `error_collector.backtrace_truncate_location` pour personnaliser l&apos;emplacement de la troncature.
  </Collapser>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    Spécifie où tronquer la trace de pile lorsque le nombre de cadres dépasse `error_collector.max_backtrace_frames`. Les options sont `top` (supprimer les cadres du début), `middle` (supprimer les cadres du milieu, en conservant le début et la fin) ou `end` (supprimer les cadres de la fin).
  </Collapser>
...
<CollapserGroup>
  <Collapser id="instrumentation-active_support_notifications-active_support_events" title="instrumentation.active_support_notifications.active_support_events">
    ...
    Un tableau d&apos;autorisation des événements de notifications Active Support spécifiques à la bibliothèque Active Support elle-même auxquels l&apos;agent doit s&apos;abonner. Les événements spécifiques à la bibliothèque Active Support se concentrent principalement sur la mise en cache. Tout nom d&apos;événement non inclus dans cette liste sera ignoré par l&apos;agent. Fournissez des noms d&apos;événements complets tels que &apos;cache\_fetch\_hit.active\_support&apos;. Ne fournissez pas d&apos;astérisques ou de modèles regex, et n&apos;échappez à aucun caractère avec des barres obliques inversées. Pour une liste complète de tous les noms d&apos;événements Active Support possibles, consultez la [liste des noms de mise en cache](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-caching) et la [liste des noms de messages](https://edgeguides.rubyonrails.org/active_support_instrumentation.html#active-support-messages) de la documentation officielle de Rails.
  </Collapser>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    Un éventail de noms de Rails.event à capturer en tant qu&apos;événements de log structurés. Par exemple,

    * user.signup
    * payment.processed
    * order.created Laissez vide pour capturer toutes les notifications Rails.event. Les événements sont loggés avec le niveau UNKNOWN (garantissant qu&apos;ils ne sont jamais filtrés) sauf remplacement via la clé :level dans la charge de l&apos;événement.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="span_events-max_samples_stored" title="span_events.max_samples_stored">
    ...
    * Définit le nombre maximum d&apos;événements span signalés à partir d&apos;une seule collecte. Tout entier compris entre `1` et `10000` est valide.&apos;
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    Si `true`, l’agent rapporte l’ID d’instance GCP comme nom d’hôte lors de l’exécution sur Google Cloud Run.
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    Si `true`, l’agent ajoute la valeur `K_REVISION` au début de l’ID d’instance GCP pour former le nom d’hôte (`{K_REVISION}-{instance id}`) sur Google Cloud Run. N’a aucun effet à moins que `utilization.gcp_cloud_run.use_instance_as_host` ne soit également `true`.
  </Collapser>
