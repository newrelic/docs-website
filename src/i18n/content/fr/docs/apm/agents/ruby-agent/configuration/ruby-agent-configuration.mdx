...
<CollapserGroup>
  <Collapser id="agent_enabled" title="agent">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="api_key" title="api_key">
    ...
    Votre New Relic <InlinePopover type="userKey" />. Obligatoire lors de l&amp;apos;utilisation de l&amp;apos;API New Relic REST v2 pour enregistrer le déploiement à l&amp;apos;aide de la commande `newrelic deployments` .
  </Collapser>
  ...
  <Collapser id="exclude_newrelic_header" title="exclure_newrelic_header">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="log_file_name" title="enregistrer">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="log_file_path" title="enregistrer">
    <table>
      <tbody>
        <tr>
          <th>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-record_sql" title="transaction_tracer.record_sql">
    ...
    niveau d&apos;obfuscation pour la requête SQL signalé dans les nœuds trace de transaction.

    Par défaut, cette valeur est définie sur `obfuscated`, ce qui supprime les littéraux numériques et de chaîne.
...
<CollapserGroup>
  <Collapser id="ai_monitoring-enabled" title="ai_monitoring.activé">
    <table>
      <tbody>
        <tr>
          <th>
  ...
  <Collapser id="ai_monitoring-record_content-enabled" title="ai_monitoring.record_content.activé">
    <table>
      <tbody>
        <tr>
          <th>
...
## traceur inter-applications [#cross-application-tracer]
...
<CollapserGroup>
  <Collapser id="cross_application_tracer-enabled" title="cross_application_tracer.enabled">
    ...
    <DNT>**DEPRECATED**</DNT> Veuillez consulter : [distributor\_tracing.enabled](#distributed_tracing-enabled).

    Si `true`, active [le traçage inter-applications](/docs/agents/ruby-agent/features/cross-application-tracing-ruby/) lorsque `distributed_tracing.enabled` est défini sur `false`.
  </Collapser>
</CollapserGroup>
...
## traceur de magasin de données [#datastore-tracer]
...
<CollapserGroup>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    Si `true`, désactive l&amp;apos;agent middleware pour Sinatra. Ce middleware est responsable de la prise en charge des fonctionnalités avancées telles que [le traçage inter-applications](/docs/apm/transactions/cross-application-traces/cross-application-tracing), [le temps de chargement des pages](/docs/browser/new-relic-browser/getting-started/new-relic-browser) et [la collecte des erreurs](/docs/apm/applications-menu/events/view-apm-error-analytics).

    <Callout variant="important">
      Le tracing inter-applications est déconseillé au profit du [tracing distribué](/docs/apm/distributed-tracing/getting-started/introduction-distributed-tracing). Le tracing distribué est activé par défaut pour les versions 8.0.0 et supérieures de l&amp;apos;agent Ruby . Les intergiciels ne sont pas tenus de prendre en charge le tracing distribué.
...
<CollapserGroup>
  ...
  <Collapser id="infinite_tracing-compression_level" title="infinite_tracing.compression_level">
    ...
    Configurez le niveau de compression des données envoyées à l&apos;observateur de trace.

    Peut être l&apos;un des suivants : `:none`, `:low`, `:medium`, `:high`.

    Réglez le niveau sur `:none` pour désactiver la compression.
  </Collapser>
</CollapserGroup>
