...
<CollapserGroup>
  ...
  <Collapser id="api_key" title="api_key">
    ...
    # OBSOLÈTE : Le paramètre de configuration `api_key` est désormais obsolète. Sa seule utilisation était pour la commande `newrelic deployments` , qui est supprimée dans la version 10.0.0 de l&apos;agent.
  </Collapser>
...
<CollapserGroup>
  <Collapser id="cross_application_tracer-enabled" title="cross_application_tracer.enabled">
    ...
    Si `true`, active [le tracing inter-applications](/docs/agents/ruby-agent/features/cross-application-tracing-ruby/) lorsque `distributed_tracing.enabled` est défini sur `false`.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="disable_sinatra_auto_middleware" title="disable_sinatra_auto_middleware">
    ...
    <Callout variant="important">
      ...
      Pour continuer à utiliser le tracing inter-applications, mettez à jour les options suivantes dans votre fichier de configuration `newrelic.yml` :
...
<CollapserGroup>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_sampled" title="distributed_tracing.sampler.remote_parent_sampled">
    ...
    Ce paramètre contrôle le comportement de l&apos;échantillonnage des transactions lorsqu&apos;un parent distant est échantillonné et que l&apos;indicateur de trace est défini dans le traceparent. Les valeurs disponibles sont `default`, `always_on` et `always_off`.
  </Collapser>
  ...
  <Collapser id="distributed_tracing-sampler-remote_parent_not_sampled" title="distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    Ce paramètre contrôle le comportement de l&apos;échantillonnage des transactions lorsqu&apos;un parent distant n&apos;est pas échantillonné et que l&apos;indicateur de trace n&apos;est pas défini dans le traceparent. Les valeurs disponibles sont `default`, `always_on` et `always_off`.
  </Collapser>
</CollapserGroup>
