...
* Standard (échantillonnage en tête) : avant l&apos;arrivée de toute trace, nous déterminons un pourcentage défini de traces à accepter et à analyser. Cela vous donne un point de départ solide pour voir comment le traçage peut vous aider. Il est activé par défaut dans les agents Ruby 8.0.0 et versions supérieures. Par défaut, l&apos;agent Ruby échantillonne 10 traces par minute, avec une limite maximale de 1000 spans par minute.
