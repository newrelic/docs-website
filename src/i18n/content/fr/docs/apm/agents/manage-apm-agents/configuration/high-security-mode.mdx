...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Récupérez l&apos;[ID de compte](/docs/accounts/accounts-billing/account-structure/account-id) du compte que vous souhaitez activer. Utilisez-le ensuite pour accéder à cette URL : `https://one.newrelic.com/admin-portal/apm-agents/hsm?account=YOUR_ACCOUNT_ID`. Sur cette page, vous pouvez configurer le mode haute sécurité.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Empêche le profilage de l&apos;application
      </td>

      <td>
        Le mode haute sécurité ne permet pas de rapporter les données de profilage d&apos;application pour les éléments suivants :

        * [Profilage en temps réel pour Java avec JFR](/docs/apm/agents/java-agent/features/real-time-profiling-java-using-jfr-metrics/)
      </td>
    </tr>
  </tbody>
</table>
