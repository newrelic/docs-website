...
<CollapserGroup>
  ...
  <Collapser id="python-config" title="Python agent configuration">
    ...
    **Utilisation de la variable d&apos;environnement :**
    ...
    <Callout variant="important">
      Lorsque la prise en charge d&apos;OpenTelemetry est activée, l&apos;agent détecte automatiquement si `opentelemetry-api` est installé et bascule vers l&apos;instrumentation OpenTelemetry pour toutes les bibliothèques prises en charge, même si New Relic possède un hook d&apos;instrumentation pour cette bibliothèque.
    </Callout>
    ...
    1. **Instrumentation native**: pour les bibliothèques avec une instrumentation OpenTelemetry native intégrée et qui ne sont pas prises en charge par New Relic, l’agent utilisera l’instrumentation OpenTelemetry native. Si l’utilisateur souhaite utiliser l’instrumentation OpenTelemetry au lieu des hooks d’instrumentation intégrés de New Relic, cette bibliothèque peut être ajoutée au paramètre `opentelemetry.traces.include`.

    2. **Instrumentation de bibliothèque**: si une bibliothèque d&apos;instrumentation OpenTelemetry est détectée et que New Relic ne fournit pas déjà un hook d&apos;instrumentation, l&apos;agent utilisera l&apos;instrumentation OpenTelemetry. De façon similaire à l&apos;instrumentation native, si l&apos;utilisateur souhaite utiliser l&apos;instrumentation OpenTelemetry au lieu des hooks d&apos;instrumentation intégrés de New Relic, cette bibliothèque peut être ajoutée au paramètre `opentelemetry.traces.include`.
    ...
    <Callout variant="caution">
      Les bibliothèques d&apos;instrumentation AWS et gRPC continueront d&apos;utiliser l&apos;instrumentation de New Relic. Les bibliothèques de ML/IA avec instrumentation native sont explicitement désactivées pour l&apos;instant et seront prises en charge dans une future sortie.
    </Callout>
    ...
    **Exemple:**
    ...
    2. **Basculement automatique**: si elle est trouvée et que New Relic ne prend pas déjà en charge cette bibliothèque, l’agent utilisera l’instrumentation de la bibliothèque OpenTelemetry
  ...
  <Collapser id="ruby-config" title="Configuration de l'agent Ruby">
    ...
    <Callout variant="important">
      Lorsque la prise en charge d&apos;OpenTelemetry est activée, l&apos;agent détecte automatiquement si `opentelemetry-api` est installé. Ce gem doit être installé pour utiliser les fonctionnalités de l&apos;agent hybride.
    </Callout>
    ...
    **Exemple:**
    ...
    ✅ **Pris en charge**: liens de span utilisant l&apos;API `OpenTelemetry::Trace::Span#add_link`

    #### Événements sur les spans

    ✅ **Pris en charge**: événements sur les spans utilisant l&apos;API `OpenTelemetry::Trace::Span#add_event`

    ### Comment ça marche

    Lorsque la prise en charge d&apos;OpenTelemetry est activée, l&apos;agent Ruby New Relic :

    1. **Écoute** les appels vers les API de tracing OpenTelemetry
    2. **Traduit** ces appels en modèles de données New Relic
    3. **Intègre** les spans OpenTelemetry dans les traces New Relic

    <Callout variant="information">
      Par défaut, l&apos;instrumentation OpenTelemetry est exclue pour les bibliothèques que l&apos;agent Ruby New Relic instrumente également. Cela inclut les bibliothèques instrumentées nativement et les bibliothèques avec des packages d&apos;instrumentation dans [opentelemetry-ruby-contrib](https://github.com/open-telemetry/opentelemetry-ruby-contrib).
    </Callout>
