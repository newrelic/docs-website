...
## Mise à niveau vers la version 14 de l&apos;agent Node.js [#node-agent-v14]

Avant de mettre à niveau vers Node.js version 14, consultez ces informations concernant les changements majeurs.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Suppression du support de Node.js 20.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: migration de l&apos;instrumentation `@apollo/server` depuis un plug-in `@newrelic/apollo-server-plugin` vers une instrumentation traditionnelle.
      </td>

      <td>
        * Les utilisateurs du serveur Apollo n&apos;auront plus besoin d&apos;installer et d&apos;utiliser `@newrelic/apollo-server-plugin`. L&apos;instrumentation sera automatiquement appliquée à chaque instance de serveur Apollo.
        * Pour plus d’informations, consultez le [Guide de migration d’Apollo Server](#apollo-server-migration-guide).
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: instrumentation supprimée pour `koa-router` et `koa-route`.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suppression de la prise en charge des politiques de licence, d&apos;application et de sécurité (`LASP`).
      </td>

      <td>
        * Cette fonctionnalité a été dépréciée et était configurable avec `config.security_policies_token`.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suppression de la prise en charge de la fonctionnalité de tracing inter-applications (`CAT`).
      </td>

      <td>
        * CAT a été déprécié dans l&apos;agent v8.4.0 et remplacé par le tracing distribué pour propager le contexte entre les services.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: mise à jour de `config.distributed_tracing.exclude_newrelic_header` pour être défini sur `true` par défaut.
      </td>

      <td>
        * Ceci a été remplacé par les en-têtes W3C par défaut.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suppression de la méthode d&apos;API `shim.argsToArray` au profit des paramètres rest.
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: suppression de la méthode d&apos;API `shim.prefixRouteParameters`.
      </td>

      <td>
        * L&apos;instrumentation Fastify a été mise à jour pour utiliser les diagnostics du canal de tracing, cette méthode n&apos;est donc plus nécessaire.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: segments `MySQL` `Pool#query` statiques supprimés dans `mysql` et `mysql2`.
      </td>

      <td>
        * Cela a créé un segment inutile car les segments `Database/*` sont plus utiles.
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Mise à jour de la version minimale prise en charge pour les bibliothèques suivantes :
      </td>

      <td>
        * `@nestjc/cli` et `@nestjs/core` à 10.0.0
        * `bluebird` vers 3.0.0
        * `mysql2` vers 3.0.0
        * `next` vers 14.0.0
        * `cassandra-driver` vers 4.0.0
        * `fastify` vers 4.0.0
      </td>
    </tr>
    ...
    <tr>
      <td>
        Mise à jour de la façon dont sont calculés le temps exclusif et le temps total de la trace.
      </td>
...
### Guide de migration du serveur Apollo [#apollo-server-migration-guide]

À partir de la version 14.0.0 de l&apos;agent, les clients d&apos;apollo server n&apos;auront plus besoin d&apos;installer et d&apos;utiliser `@newrelic/apollo-server-plugin`. L&apos;instrumentation sera automatiquement appliquée à chaque instance de serveur apollo. Cependant, si les clients ont configuré le plug-in, cela détaillera le plan de migration.
...
* `captureScalars` est maintenant `config.apollo_server.scalars` ou `NEW_RELIC_APOLLO_SERVER_SCALARS`
* `captureIntrospectionQueries` est maintenant `config.apollo_server.introspection_queries` ou `NEW_RELIC_APOLLO_SERVER_INTROSPECTION_QUERIES`
* `captureServiceDefinitionQueries` est maintenant `config.apollo_server.service_definition_queries` ou `NEW_RELIC_APOLLO_SERVER_SERVICE_DEFINITION_QUERIES`
* `captureHealthCheckQueries` est maintenant `config.apollo_server.health_check_queries` ou `NEW_RELIC_APOLLO_SERVER_HEALTH_CHECK_QUERIES`
* `captureFieldMetrics` est maintenant `config.apollo_server.field_metrics` ou `NEW_RELIC_APOLLO_SERVER_FIELD_METRICS`

#### Appel d&apos;API

Il y avait deux configurations précédentes qui enregistraient des fonctions à appeler qui ajoutaient les valeurs clés en tant qu’attributs personnalisés sur les segments d’opération ou de résolveur. L&apos;agent suit un modèle dans lequel les clients peuvent enregistrer des rappels via des méthodes d&apos;API, et ces deux cas d&apos;utilisation y correspondent.

##### customOperationAttributes

C&apos;est maintenant `newrelic.setApolloOperationAttributesCallback`. Il prend un rappel et, lorsqu&apos;il est présent, le rappel reçoit le même argument que précédemment. `requestContext`
...
C&apos;est maintenant `newrelic.setApolloResolverAttributesCallback`. Il prend un rappel et, lorsqu&apos;il est présent, le rappel reçoit le même argument que précédemment. `{ source, args, contextValue, info }`
...
Vérifié que la métrique `MySQL` `host:port` est enregistrée
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Ajout de server.address aux spans amqplib.
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Les paramètres de route (URL) sont désormais stockés en tant qu&apos;attribut `request.parameters.route.*` sur les transactions, les segments racine et les spans.
      </td>

      <td>
        * Les paramètres de requête seront disponibles sous forme d&apos;attributs préfixés par request.parameters.\* sur les transactions et les spans.
        * Le paramètre de route sera disponible sous forme d&apos;attribut préfixé par request.parameters.route.\* sur les transactions et les spans.
