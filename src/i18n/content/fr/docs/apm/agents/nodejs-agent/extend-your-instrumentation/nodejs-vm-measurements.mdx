...
Certaines métriques de machine virtuelle Node.js sont collectées par défaut à l&apos;aide des API Node.js intégrées, tandis que d&apos;autres nécessitent le package optionnel [`@newrelic/native-metrics`](https://www.npmjs.com/package/@newrelic/native-metrics) :

* **Échantillonneurs par défaut (aucun module natif requis) :** la mémoire (`Memory/*`), le temps d&apos;attente de la file de la boucle d&apos;événements (`Events/wait`) et le CPU (`CPU/*`) sont toujours collectés à l&apos;aide de `process.memoryUsage()`, de la mesure du décalage de `setTimeout` et de `process.cpuUsage()` respectivement.
* **Échantillonneurs de modules natifs :** la récupération de mémoire (`GC/*`) et l’utilisation précise du CPU par la boucle d’événements (`Nodejs/EventLoop/CPU/Usage`) nécessitent `@newrelic/native-metrics`. Si le module n&apos;est pas installé ou s&apos;il ne parvient pas à se charger, ces métriques n&apos;apparaîtront pas et l&apos;agent continuera de s&apos;exécuter uniquement avec les échantillonneurs par défaut.
...
<CollapserGroup>
  <Collapser id="time-in-garbage-collection" title="Temps consacré à la collecte des ordures">
    <Callout variant="important">Nécessite `@newrelic/native-metrics`. Cette métrique n&apos;est pas collectée par défaut.</Callout>
  ...
  <Collapser id="memory" title="mémoire">
    <Callout variant="tip">Collecté par défaut. Aucun module natif requis.</Callout>
  ...
  <Collapser id="cpu" title="Processeur">
    <Callout variant="tip">Collecté par défaut sur Node.js v6.1.0 ou supérieur. Aucun module natif requis.</Callout>
