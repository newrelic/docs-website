...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    Lorsqu&apos;elle est activée, l&apos;agent utilise l&apos;ID d&apos;instance des métadonnées GCP pour définir le nom d&apos;hôte de l&apos;application en cours d&apos;exécution.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    ...
    Lorsqu’il est activé, et que `gcp_use_instance_as_host` est également activé, la révision GCP Cloud Run (`K_REVISION`) est ajoutée au début de l’ID d’instance lors de la définition du nom d’hôte de l’application en cours d’exécution, ce qui donne un nom d’hôte de `${K_REVISION}-${instance_id}`.
  </Collapser>
</CollapserGroup>
