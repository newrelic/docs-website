...
<CollapserGroup>
  ...
  <Collapser id="attributes_include" title="include">
    ...
    Le *paramètre d&apos;itinéraire* est `id` et a une valeur de `abc123`. Cela devient l&apos;attribut `request.parameters.route.id: abc123` sur la Transaction, le Segment racine et la Span. Cet exemple possède également un *paramètre de requête* de `id`, qui a une valeur de `true`. Cela deviendrait l&apos;attribut `request.parameters.id: true` sur la transaction, le segment racine et la span.
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-root" title="sampler.root">
    ...
    Contrôle la façon dont les décisions d&apos;échantillonnage sont prises pour les spans-transactions racines qui n&apos;ont pas de parent distant (ce qui signifie qu&apos;elles n&apos;ont pas d&apos;en-tête de tracing distribué `traceparent` ou `newrelic` entrant). Les valeurs valides sont :

    * `adaptive` (par défaut) : utilise l&apos;algorithme d&apos;échantillonnage adaptatif intégré de New Relic.
    * `always_on`: échantillonne tous les spans.
    * `always_off`: n’échantillonne aucune span.
    * `trace_id_ratio_based`: échantillonne un ratio fixe de traces, déterminé de manière déterministe par l’ID de trace.

    Par exemple, pour définir cela dans le fichier de configuration, vous utiliseriez :
    ...
    Lorsqu’il est défini sur `trace_id_ratio_based`, vous devez également fournir un `ratio` (un nombre entre `0` et `1`) :
    ...
    Lorsque vous définissez ceci sur `adaptive`, vous pouvez éventuellement remplacer la cible d&apos;échantillonnage (de 1 à 120 transactions/minute) spécifiquement pour cet échantillonneur :
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="échantillonneur.remote_parent_sampled">
    ...
    Lorsque l&apos;en-tête traceparent a un indicateur échantillonné de `01`, cela contrôle la manière de gérer l&apos;échantillonnage des spans. Les valeurs de chaîne autorisées sont :
    ...
    * `trace_id_ratio_based`: échantillonne un ratio fixe de traces, déterminé de manière déterministe par l’ID de trace.
    ...
    Ce paramètre a priorité sur [`remote_parent_not_sampled`](#dt-sampler-remote-parent-not-sampled) lorsque les deux pourraient s&apos;appliquer.
    ...
    Lorsque vous définissez ceci sur `trace_id_ratio_based`, vous devez fournir une valeur `ratio` de `0`à`1`. Lorsque vous définissez ceci sur `adaptive`, vous pouvez éventuellement remplacer la cible d’échantillonnage spécifiquement pour cet échantillonneur. Pour la syntaxe équivalente, consultez les exemples de forme d’objet et de variable d’environnement sous [`sampler.root`](#dt-sampler-root) et remplacez `root` par `remote_parent_sampled`.
  </Collapser>
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    Lorsque l&apos;en-tête traceparent a un indicateur échantillonné de `00`, cela contrôle la manière de gérer l&apos;échantillonnage des spans. Les valeurs de chaîne autorisées sont :
    ...
    * `trace_id_ratio_based`: échantillonne un ratio fixe de traces, déterminé de manière déterministe par l’ID de trace.
    ...
    Ce paramètre ne prend effet que lorsque [`remote_parent_sampled`](#dt-sampler-remote-parent-sampled) ne s&apos;applique pas (c&apos;est-à-dire que l&apos;indicateur échantillonné traceparent est `00`).
    ...
    Lorsque vous définissez ceci sur `trace_id_ratio_based`, vous devez fournir une valeur `ratio` de `0`à`1`. Lorsque vous définissez ceci sur `adaptive`, vous pouvez éventuellement remplacer la cible d’échantillonnage spécifiquement pour cet échantillonneur. Pour la syntaxe équivalente, consultez les exemples de forme d’objet et de variable d’environnement sous [`sampler.root`](#dt-sampler-root) et remplacez `root` par `remote_parent_not_sampled`.
  </Collapser>
  ...
  <Collapser id="dt-sampler-adaptive-sampling-target" title="sampler.adaptive_sampling_target">
    ...
    La cible d&apos;échantillonnage pour l&apos;échantillonnage adaptatif, en transactions par minute, utilisée lors de la configuration de l&apos;échantillonneur par défaut/adaptatif. Doit être dans la plage `[1, 120]` (inclus). Lors de la connexion à New Relic, cette valeur est envoyée au collecteur, et la valeur `sampling_target` renvoyée dans la réponse de connexion est utilisée comme cible d&apos;échantillonnage effective pour l&apos;échantillonnage adaptatif dans l&apos;agent.
...
[Les données Span](/docs/apm/distributed-tracing/ui-data/span-event) sont signalées pour [le tracing distribué](#distributed-tracing). Le tracing distribué doit être activé pour signaler les spans. Définissez la configuration de la span dans la section `span_events` . Les options incluent :
...
<CollapserGroup>
  ...
  <Collapser id="span-events-attributes-enabled" title="attributes.enabled">
    ...
    Ce paramètre peut être utilisé pour activer ou désactiver la création de rapports d&apos;attributs pour les spans. Si `attributes.enabled` au niveau racine est `false`, aucun attribut ne sera envoyé avec les spans, quelle que soit la manière dont cela est défini.
  </Collapser>
  ...
  <Collapser id="span-events-attributes-include" title="attributes.include">
    ...
    Si les attributs sont activés pour les spans, toutes les clés d&apos;attribut trouvées dans cette liste seront attachées aux spans. Pour plus d&apos;informations, consultez les [règles d&apos;attribut de l&apos;agent](/docs/apm/other-features/attributes/agent-attributes).
  </Collapser>
  ...
  <Collapser id="span-events-attributes-exclude" title="attributes.exclude">
    ...
    Toutes les clés d’attribut trouvées dans cette liste ne seront pas envoyées avec des spans. Pour plus d&apos;informations, consultez les [règles d&apos;attribut de l&apos;agent](/docs/apm/other-features/attributes/agent-attributes).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="infinite-tracing-batching" title="traitement par lots">
    ...
    Ce paramètre envoie des spans par lots de 250 ou toutes les 5 secondes.
...
<CollapserGroup>
  <Collapser id="code-level-metrics-enabled" title="code_level_metrics.enabled">
    ...
    Active ou non la capture d&apos;attributs supplémentaires sur les spans de middleware pour tous les frameworks Web Node.js qui aident à piloter [les métriques du niveau de code](/docs/codestream/observability/code-level-metrics). Les attributs supplémentaires sont : `code.filepath`, `code.function`, `code.lineno` et `code.column`.
  </Collapser>
</CollapserGroup>
...
Configurez la façon dont l’agent détecte les informations sur l’hôte et l’environnement cloud pour calculer avec précision votre tarification basée sur l’utilisation. Ces paramètres apparaissent dans la section `utilization` du fichier de configuration `newrelic.js` de votre application.
...
<CollapserGroup>
  <Collapser id="utilization-detect-aws" title="detect_aws">
    ...
    Indique si l’agent tente de contacter AWS pour obtenir des informations sur la machine virtuelle sur laquelle le processus s’exécute.
  </Collapser>
  ...
  <Collapser id="utilization-detect-pcf" title="detect_pcf">
    ...
    Si l&apos;agent tente de détecter si le processus s&apos;exécute sur Pivotal Cloud Foundry.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azure" title="detect_azure">
    ...
    Si l&apos;agent tente de contacter Azure pour obtenir des informations sur la machine virtuelle sur laquelle le processus s&apos;exécute.
  </Collapser>
  ...
  <Collapser id="utilization-detect-azurefunction" title="detect_azurefunction">
    ...
    Si l&apos;agent tente de lire les variables d&apos;environnement et le contexte d&apos;invocation pour obtenir des informations sur l&apos;Azure Function dans laquelle il s&apos;exécute.
  </Collapser>
  ...
  <Collapser id="utilization-detect-docker" title="detect_docker">
    ...
    Indique si l&apos;agent tente de lire des fichiers pour obtenir des informations sur le conteneur dans lequel le processus s&apos;exécute.
  </Collapser>
  ...
  <Collapser id="utilization-detect-gcp" title="detect_gcp">
    ...
    Indique si l’agent tente de contacter GCP pour obtenir des informations sur la machine virtuelle sur laquelle le processus s’exécute.
  </Collapser>
  ...
  <Collapser id="utilization-detect-kubernetes" title="detect_kubernetes">
    ...
    Si l&apos;agent tente de contacter Kubernetes pour obtenir des informations sur le conteneur sur lequel le processus s&apos;exécute.
  </Collapser>
  ...
  <Collapser id="utilization-logical-processors" title="logical_processors">
    ...
    Remplace le nombre de processeurs logiques que l&apos;agent signale pour l&apos;hôte.
  </Collapser>
  ...
  <Collapser id="utilization-billing-hostname" title="billing_hostname">
    ...
    Remplace le nom d’hôte que l&apos;agent signale à des fins de facturation/d&apos;utilisation.
  </Collapser>
  ...
  <Collapser id="utilization-total-ram-mib" title="total_ram_mib">
    ...
    Remplace la quantité de RAM totale (en Mio) que l&apos;agent signale pour l&apos;hôte.
  </Collapser>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    <Callout variant="important">
      Obsolète et sera supprimé dans la version 15 de l’agent. Veuillez plutôt utiliser `gcp_cloud_run.use_instance_as_host`.
    </Callout>

    Lorsqu’il est activé, il utilisera l’ID de métadonnées GCP pour définir le nom d’hôte de l’application en cours d’exécution (Services, Worker Pools, et Jobs).
  </Collapser>
...
Ces paramètres régissent les différentes fonctionnalités basées sur OpenTelemetry fournies par l’agent. Ils apparaissent dans la section `opentelemetry` du fichier de configuration `newrelic.js` de votre application.

<Callout variant="important">
  Cette configuration est sujette à modification pendant que l&apos;ensemble de fonctionnalités OpenTelemetry est en cours de développement.
</Callout>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    Commutateur global pour l’ensemble de la fonctionnalité OpenTelemetry. S’il est défini sur `false`, aucune autre sous-fonctionnalité OpenTelemetry (telle que `traces`, `logs` ou `metrics`) ne sera activée, quel que soit le propre paramètre de cette sous-fonctionnalité.
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    Permet de relier les instrumentations OpenTelemetry (par exemple, `@fastify/otel`) à l’agent New Relic.
  </Collapser>
  ...
  <Collapser id="opentelemetry-logs-enabled" title="opentelemetry.logs.enabled">
    ...
    Lorsque `true`, l’agent configure automatiquement l’API de logs OpenTelemetry pour envoyer les logs émis via l’API spécifique à OTel à New Relic.

    <Callout variant="important">
      Cette fonctionnalité dépend du [transfert de logs d&apos;application](#application-logging-forwarding-enabled) — `application_logging.forwarding.enabled` doit également être `true`.
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-enabled" title="opentelemetry.metrics.enabled">
    ...
    Lorsque `true`, l&apos;agent configure automatiquement l&apos;API de métriques OpenTelemetry pour envoyer des métriques à New Relic, attachées à l&apos;entité d&apos;application instrumentée par l&apos;agent.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-interval" title="opentelemetry.metrics.export_interval">
    ...
    Le nombre de millisecondes entre chaque tentative d&apos;envoi des métriques OpenTelemetry à New Relic. Doit être supérieur ou égal à `opentelemetry.metrics.export_timeout`.
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-timeout" title="opentelemetry.metrics.export_timeout">
    ...
    Le nombre de millisecondes accordé à une opération d&apos;exportation de métriques OpenTelemetry avant de devoir se terminer avec succès. Si le délai d&apos;attente est dépassé, il est signalé via l&apos;API de diagnostic OpenTelemetry.
  </Collapser>
</CollapserGroup>

## Instrumentation du serveur Apollo [#apollo-server-instrumentation]

Ces paramètres personnalisent le comportement de l’instrumentation Apollo Server (GraphQL) de l’agent. Ils apparaissent dans la section `apollo_server` du fichier de configuration `newrelic.js` de votre application.
...
<CollapserGroup>
  <Collapser id="apollo-server-scalars" title="apollo_server.scalars">
    ...
    Active la capture du timing pour les champs résolus avec le type de retour `GraphQLScalarType`. Cela peut être souhaité lors de l&apos;exécution de calculs chronophages pour renvoyer une valeur scalaire. Non recommandé pour les requêtes qui renvoient un grand nombre de champs scalaires précalculés.

    <Callout variant="tip">
      Les résolveurs de requête/mutation sont toujours capturés, même lorsqu’ils renvoient un type scalaire.
    </Callout>
  </Collapser>
  ...
  <Collapser id="apollo-server-introspection-queries" title="apollo_server.introspection_queries">
    ...
    Active la capture des timings pour une [IntrospectionQuery](https://www.graphql-js.org/api-v16/utilities/#introspectionquery).
  </Collapser>
  ...
  <Collapser id="apollo-server-service-definition-queries" title="apollo_server.service_definition_queries">
    ...
    Active la capture des temps pour une [requête de définition de service](https://www.apollographql.com/docs/federation/federation-spec/#fetch-service-capabilities) reçue d’un serveur Apollo Federated Gateway.
  </Collapser>
  ...
  <Collapser id="apollo-server-health-check-queries" title="apollo_server.health_check_queries">
    ...
    Active la capture des temps pour une [requête de vérification de l&apos;état](https://www.apollographql.com/docs/federation/api/apollo-gateway/#servicehealthcheck) reçue d&apos;un serveur Apollo Federated Gateway.
  </Collapser>
  ...
  <Collapser id="apollo-server-field-metrics" title="apollo_server.field_metrics">
    ...
    Active la capture des métriques pour chaque champ et argument de résolveur vus pour une requête Apollo. Conçu pour identifier les champs inutilisés dans votre schéma GraphQL.
  </Collapser>
</CollapserGroup>
