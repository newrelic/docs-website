...
### Ajouter des attributs personnalisés tels que le nonce [#nonce-attribute]

Si votre site utilise une [Content Security Policy (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) qui nécessite un attribut `nonce`, vous pouvez passer `false` aux deux fonctions d&apos;API pour qu&apos;elles renvoient la chaîne JavaScript brute sans les balises `<script>`. Vous pouvez ensuite construire les balises vous-même avec les attributs requis :
...
Passer `false` en argument supprime les balises de script que les fonctions génèrent normalement, vous pouvez donc entourer la chaîne JavaScript brute de vos propres balises `<script>` avec tous les attributs dont vous avez besoin.
