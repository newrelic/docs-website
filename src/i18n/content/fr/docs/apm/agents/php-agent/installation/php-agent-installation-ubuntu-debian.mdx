...
<Steps>
  <Step>
    ...
    1. Ajoutez le référentiel apt `http://apt.newrelic.com/debian/` à `sources.list`:
    ...
    2. Faites confiance à la clé GPG de New Relic :

       ```bash
       wget -O- https://download.newrelic.com/548C16BF.gpg | sudo gpg --dearmor -o /usr/share/keyrings/download.newrelic.com-newrelic.gpg
       ```

       Cette commande installe la clé GPG de New Relic utilisée par `apt` pour vérifier les signatures du package dans `http://apt.newrelic.com/debian/` apt référentiel ajouté à l&amp;apos;étape précédente.
  </Step>
