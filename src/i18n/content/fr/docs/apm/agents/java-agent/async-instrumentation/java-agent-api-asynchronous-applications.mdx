...
<CollapserGroup>
  <Collapser id="use-gettoken" title="1. Démarrez une transaction, puis créez et faites expirer un jeton">
    Considérez la méthode `parallelStream()` dans l’ snippet de code ci-dessous. Étant donné que certains des appels à `requestItemAsync()` se produiront sur un thread distinct, un jeton est créé et transmis pour relier ce travail asynchrone au thread demandeur.
