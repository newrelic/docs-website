...
* Standard (échantillonnage en tête) : avant l&apos;arrivée de toute trace, nous déterminons un pourcentage défini de traces à accepter et à analyser. Cela vous donne un point de départ solide pour voir comment le traçage peut vous aider. Il est activé par défaut dans les agents Java 7.4.0 et versions ultérieures. Par défaut, l&apos;agent Java échantillonne 120 traces par minute, avec une limite maximale de 2000 spans par minute.
