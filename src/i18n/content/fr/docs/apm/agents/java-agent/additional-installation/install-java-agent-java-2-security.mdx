...
2. Ajoutez le texte suivant à votre fichier de stratégie, assurez-vous de conserver le tiret `-` à la fin du chemin, qui accorde à tous les jars du dossier cible les permissions nécessaires.
   ...
   <Callout variant="tip">
     Si vous utilisez Java 9+ et rencontrez des erreurs d’accès refusé, vous devrez peut-être accorder des autorisations supplémentaires (telles que `java.lang.RuntimePermission "accessSystemModules"`).
   </Callout>
