...
<Callout variant="important">
  ...
  * Le tracing infini ne fonctionne pas si `enable_auto_app_naming` est activé.
</Callout>

Pour activer le tracing infini, activez le tracing distribué et ajoutez le paramètre supplémentaire ci-dessous. Pour un exemple, voir [Agent de langage : Configurer le tracing distribué](/docs/understand-dependencies/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing#java-config).
