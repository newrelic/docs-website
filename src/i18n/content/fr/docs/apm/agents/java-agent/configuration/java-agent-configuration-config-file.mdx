...
L&apos;agent peut être désactivé de manière sélective en fonction de la classe principale de démarrage ou du fichier jar exécutable extrait de la ligne de commande. Cela est pratique dans les situations où la variable d&apos;environnement « JAVA\_TOOL\_OPTIONS » est présente et contient l&apos;indicateur -javaagent mais nous ne voulons pas appliquer l&apos;instrumentation à toutes les applications Java de l&apos;environnement - par exemple dans un conteneur Kubernetes.

<Callout variant="important">
  La configuration de saut n&apos;est prise en charge que via une variable d&apos;environnement en raison du moment où la configuration de saut doit être vérifiée dans le cycle de vie de l&apos;agent.
</Callout>

À des fins de diagnostic, si la variable `NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS` est définie, l&apos;agent imprimera la ligne de commande extraite et le jeton de saut analysé sur stdout :
...
Si l&apos;agent est désactivé en fonction de la variable de saut définie, les éléments suivants seront transférés vers la sortie standard :
...
<CollapserGroup>
  <Collapser id="cfg-skip-configuration" title="NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS">
    ...
    Une liste séparée par des virgules des classes principales, des fichiers jar exécutables ou des outils/applications basés sur Java que l&apos;agent ne doit PAS instrument. Par exemple : `NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS=keytool,myapp.jar,IgnoreThisClass`.
  </Collapser>
</CollapserGroup>
