...
<CollapserGroup>
  ...
  <Collapser id="tt-sql_metadata_comments" title="sql_metadata_comments">
    ...
    Contrôle l&apos;ajout de commentaires au début des instructions SQL exécutées, utilisés par le produit New Relic Query Performance Monitoring pour la liaison d&apos;entités. Une chaîne vide désactive les commentaires SQL (paramètre par défaut). Les options autorisées sont :

    * `nr_service_guid` - Le GUID de l&apos;entité New Relic que l&apos;agent instrumente

    Un commentaire résultant ressemblerait à ceci : `/*nr_service_guid=MTE3NDc2MDB8QVBNfEFQUExJQ0FUSU9OfDI4MTc5NDEIEA*/`
  </Collapser>
