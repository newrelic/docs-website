...
<CollapserGroup>
  ...
  <Collapser id="event_ingest_uri" title="event_ingest_uri">
    ...
    Événement ingest URI utilisé par certaines fonctionnalités agent. La valeur par défaut est le point de terminaison d’ingestion de production américain. Peut être configuré manuellement pour envoyer l’événement à d’autres points de terminaison d’ingestion. Exemples :
  ...
  <Collapser id="metric_ingest_uri" title="metric_ingest_uri">
    ...
    URI d&apos;ingestion de métrique utilisé par certaines fonctionnalités de l&apos;agent. La valeur par défaut est le point de terminaison d&apos;ingestion de production américain. Peut être configuré manuellement pour envoyer des métriques à d&apos;autres points de terminaison d&apos;ingestion. Exemples :
...
<CollapserGroup>
  ...
  <Collapser id="cfg-circuitbreaker_gc_cpu_threshold" title="gc_cpu_threshold">
    ...
    Personnalisez le pourcentage de temps CPU de garbage collection au-dessus duquel le disjoncteur doit se déclencher. Lorsque le pourcentage de mémoire de tas libre est inférieur à `memory_threshold` et que le temps CPU consacré au garbage collection est supérieur à `gc_cpu_threshold`, le disjoncteur se déclenche. Pour rendre le disjoncteur moins susceptible de se déclencher, diminuez `memory_threshold` et/ou augmentez `gc_cpu_threshold`. Ajustez ces valeurs si nécessaire, en fonction des performances de fonctionnement et du comportement de votre application.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="aws-account_id" title="aws.account_id">
    ...
    Ce paramètre est lu par certaines de services cloud instrumentation afin que `cloud.resource_id` l&apos;attribut puisse être défini dans les spans respectives.
  ...
  <Collapser id="metadata_bypass_proxy" title="metadata_bypass_proxy">
    ...
    Lorsqu&apos;il est défini sur true, ce paramètre vous permet de contourner le proxy pour l&apos;accès au point de terminaison des métadonnées cloud. La valeur par défaut est false.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="échantillonneur.remote_parent_sampled">
    ...
    * `always_on`: Échantillonnez toujours les spans dont le parent en amont a été échantillonné.
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    * `default`: Utilisez les règles d&apos;échantillonnage standard de New Relic.
    * `always_on`: Échantillonnez toujours les spans dont le parent en amont n&apos;a pas été échantillonné.
...
#### Liste de refus des niveaux de log [#log-level-denylist]

L&apos;agent Java peut exclure des niveaux de log spécifiques de la génération de LogEvents qui sont transmis à New Relic.

Vous pouvez contrôler cela via l&apos;option de configuration `log_level_denylist`, imbriquée sous la section `forwarding`. Ce paramètre vous permet de définir une liste de chaînes séparées par des virgules représentant des niveaux de log (par ex. `log_level_denylist: "info, debug"`) dont les événements de log ne doivent pas être transmis. Les noms de niveau de log ne sont pas sensibles à la casse, mais doivent par ailleurs correspondre exactement. Ces paramètres peuvent être remplacés par la propriété système préfixée `newrelic.config.application_logging.forwarding.log_level_denylist`. La seule option disponible est :
...
À partir de la version `9.1.0` de l&apos;agent Java, la compatibilité avec l&apos;API de tracing, de métriques et de logs OpenTelemetry a été ajoutée, offrant une expérience &quot;le meilleur des deux mondes&quot; en combinant la visibilité approfondie de New Relic aux signaux OpenTelemetry.

Grâce à cette fonctionnalité, les API de tracing, de métriques et de logs OpenTelemetry peuvent être utilisées pour instrumenter votre application de manière personnalisée, en combinaison avec les API de l&apos;agent Java New Relic, tandis que l&apos;agent Java combine le tout de façon transparente pour offrir une expérience APM familière. De plus, certaines bibliothèques fournissant une instrumentation native OpenTelemetry ou une instrumentation de bibliothèque autonome peuvent être utilisées pour offrir une visibilité accrue sur votre application.

Les options de configuration suivantes déterminent quels signaux OpenTelemetry doivent être capturés et intégrés à l&apos;expérience New Relic APM.
...
<CollapserGroup>
  <Collapser id="cfg-opentelemetry-enabled" title="enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Définissez sur true pour permettre l&apos;activation de signaux OpenTelemetry individuels, et sur false pour désactiver tous les signaux OpenTelemetry. La valeur par défaut est false.

    Peut être défini via la propriété système :

    ```ini
    -Dnewrelic.config.opentelemetry.enabled=false
    ```

    Peut être défini via la variable d&apos;environnement :
  ...
  <Collapser id="cfg-opentelemetry-logs-enabled" title="logs.enabled">
    ...
    Définissez sur true pour activer les signaux de Logs OpenTelemetry lorsque `opentelemetry.enabled=true`. La valeur par défaut est true.
  ...
  <Collapser id="cfg-opentelemetry-metrics-enabled" title="metrics.enabled">
    ...
    Définir sur true pour activer les signaux de métriques OpenTelemetry lorsque `opentelemetry.enabled=true`. La valeur par défaut est true.
  ...
  <Collapser id="cfg-opentelemetry-metrics-include" title="metrics.include">
    ...
    Une chaîne de Meters OpenTelemetry délimitée par des virgules (par ex. &quot;MeterName1,MeterName2&quot;) dont les signaux doivent être inclus. Par défaut, tous les compteurs sont inclus. Cela écrasera toutes les exclusions de Meter par défaut dans l&apos;agent, ce qui aura pour effet de les réactiver.
  ...
  <Collapser id="cfg-opentelemetry-metrics-exclude" title="metrics.exclude">
    ...
    Une chaîne de Meters OpenTelemetry délimitée par des virgules (par ex. &quot;MeterName3,MeterName4&quot;) dont les signaux doivent être exclus. Cela prévaut sur toutes les autres sources d&apos;inclusion/exclusion, désactivant ainsi les compteurs listés.
  ...
  <Collapser id="cfg-opentelemetry-traces-enabled" title="traces.enabled">
    ...
    Définir sur true pour activer les signaux de traces OpenTelemetry lorsque `opentelemetry.enabled=true`. La valeur par défaut est true.
  ...
  <Collapser id="cfg-opentelemetry-traces-include" title="traces.include">
    ...
    Une chaîne de traceurs OpenTelemetry séparés par des virgules (par ex. &quot;TracerName1,TracerName2&quot;) dont les signaux doivent être inclus. Par défaut, tous les Tracers sont inclus. Cela remplacera toutes les exclusions par défaut du Tracer dans l&apos;agent, les réactivant ainsi.
  ...
  <Collapser id="cfg-opentelemetry-traces-exclude" title="traces.exclude">
    ...
    Une chaîne de traceurs OpenTelemetry séparés par des virgules (par ex. &quot;TracerName3,TracerName4&quot;) dont les signaux doivent être exclus. Ceci a la priorité sur toutes les autres sources d&apos;inclusion/exclusion, désactivant ainsi les Traceurs listés.
...
<CollapserGroup>
  ...
  <Collapser id="tt-sql_metadata_comments" title="sql_metadata_comments">
    ...
    Contrôle l&apos;ajout de commentaires au début des instructions SQL exécutées, utilisés par le produit New Relic Query Performance Monitoring pour la liaison d&apos;entités. La valeur par défaut est false.

    Un commentaire résultant ressemblerait à ceci : /*nr\_service\_guid=MTE3NDc2MDB8QVBNfAAAAAxJQEEEEE9OfDI4MTc5NDEIEA*/
  </Collapser>
  ...
  <Collapser id="tt-transaction_threshold" title="transaction_threshold">
    ...
    Le seuil de temps utilisé pour déterminer quand une transaction est éligible pour être tracée. Lorsque le temps de réponse de la transaction dépasse ce seuil, la transaction devient éligible pour une [trace de transaction](/docs/apm/transactions/transaction-traces/transaction-traces). Si elle est échantillonnée, cette trace de transaction sera enregistrée et envoyée à New Relic.
