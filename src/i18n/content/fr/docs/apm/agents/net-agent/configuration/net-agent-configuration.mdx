...
L&apos;élément `applications` est un enfant de l&apos;élément `instrumentation`. L&apos;élément `applications` prend en charge `application` éléments enfants qui spécifient les applications non web à instrumenter. L&apos;élément `application` contient un attribut `name`. De plus, à partir de la version 10.48.0 de l&apos;agent, il existe un attribut `include` facultatif avec une valeur par défaut de &quot;true&quot;. Définissez cet attribut sur &quot;false&quot; pour empêcher l&apos;agent d&apos;instrumenter ce processus.
...
Alternativement, à partir de la version 10.48.0 de l&apos;agent, définissez la ou les variables d&apos;environnement `NEW_RELIC_INCLUDED_APPLICATION_NAMES` et/ou `NEW_RELIC_EXCLUDED_APPLICATION_NAMES` dans l&apos;environnement de l&apos;application :
...
Notez que exclude est prioritaire sur include.
...
<CollapserGroup>
  ...
  <Collapser id="transaction-attributes" title="attributes">
    ...
    <Callout variant="caution">
      Lorsque Infinite Tracing et/ou le tracing infini sont activés, les informations de l&apos;événement de transaction sont appliquées à l&apos;événement Span racine de la transaction. Envisagez d&apos;appliquer tous les paramètres d&apos;attribut pour l&apos;événement de transaction à l&apos;événement de span et/ou de les appliquer en tant que [paramètres d&apos;attribut globaux](#agent-attributes).
    </Callout>
  </Collapser>
</CollapserGroup>
...
L&apos;instrumentation Browser peut également être activée ou désactivée via une variable d&apos;environnement :
...
<CollapserGroup>
  ...
  <Collapser id="tracer-sqlMetadataCommentsEnabled" title="sqlMetadataCommentsEnabled">
    ...
    Lorsqu&apos;il est défini sur `true`, l&apos;agent ajoute un commentaire SQL qui contient le GUID de l&apos;entité de service au début de chaque instruction SQL instrumentée avant que la base de données n&apos;exécute l&apos;instruction. Le commentaire a le format suivant :

    ```sql
    /*nr_service_guid="YOUR_ENTITY_GUID"*/
    ```

    Utilisez cette option pour corréler les données APM avec le [monitoring des performances des requêtes](/docs/infrastructure/infrastructure-data/query-performance-monitoring). L’agent modifie le texte de l’instruction qu’il envoie à la base de données, de sorte que la base de données reçoive le commentaire complet.

    L&apos;agent enregistre également le texte commenté. Le commentaire apparaît dans les traces de requêtes lentes, dans les segments de trace de transaction et dans l&apos;attribut `db.statement` des événements de span. Avec la valeur [`recordSql`](#tracer-recordSql) par défaut de `obfuscated`, l&apos;agent remplace le GUID de l&apos;entité entre guillemets par `?`, de sorte que ces enregistrements affichent `/*nr_service_guid=?*/`. Définissez `recordSql` sur `raw` pour conserver le GUID de l&apos;entité dans ces données d&apos;agent.

    Cette option est disponible à partir de la version 10.52.0 de l’agent .NET. Vous pouvez également la définir avec la variable d’environnement `NEW_RELIC_TRANSACTION_TRACER_SQL_METADATA_COMMENTS_ENABLED`.

    <Callout variant="important">
      Cette option modifie le texte de chaque instruction SQL instrumentée. Une base de données qui met en cache les plans de requête ou les instructions préparées par texte d&apos;instruction considère chaque instruction commentée comme une nouvelle instruction. Cela peut réduire le taux d&apos;accès au cache et augmenter le travail de planification des requêtes sur la base de données. L&apos;effet est plus important lorsque plus d&apos;un service interroge la même base de données, car chaque service ajoute un GUID d&apos;entité différent à la même instruction.

      Activez d&apos;abord cette option dans un environnement de test, et mesurez l&apos;effet sur votre base de données avant de l&apos;activer en production.
    </Callout>
  </Collapser>
...
L&apos;élément `<traceIdRatioBased>` possède un attribut requis `ratio`, qui est une valeur comprise entre `0.0` et `1.0` spécifiant le pourcentage de traces à échantillonner.
...
Les variables d&apos;environnement suivantes peuvent être utilisées pour configurer l&apos;échantillonneur trace distribuées aux différents niveaux d&apos;échantillonnage, en spécifiant l&apos;une des valeurs possibles pour le type d&apos;échantillonneur :
...
### Métriques Kafka [#kafka\_metrics][#kafka_metrics]

L’élément `kafka` est un enfant de l’élément `configuration`. Utilisez `kafka` pour configurer les métriques Kafka que l’agent collecte à partir des producteurs et consommateurs Kafka de votre application.

Pris en charge dans l’agent .NET v10.55.0 et versions ultérieures.

L’élément `kafka` prend en charge le sous-élément `metrics`. Tous les attributs de `metrics` sont facultatifs.
...
Vous pouvez également configurer ces attributs avec des variables d’environnement :
...
Le sous-élément `metrics` prend en charge les attributs suivants :
...
<CollapserGroup>
  <Collapser id="kafka-metrics-debug-enabled" title="debugEnabled">
    ...
    Activez ou désactivez la collecte des métriques Kafka internes que l&apos;agent construit à partir des statistiques que Kafka lui transmet.

    Cet attribut a priorité sur le paramètre d&apos;application `NewRelic.KafkaInternalMetricsEnabled`. Si vous ne définissez pas `debugEnabled`, l&apos;agent respecte toujours ce paramètre d&apos;application.

    Ou utilisez la variable d’environnement `NEW_RELIC_KAFKA_METRICS_DEBUG_ENABLED` .
  </Collapser>

  <Collapser id="kafka-metrics-interval" title="intervalle">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    Cet attribut définit l’intervalle, en secondes, auquel Kafka rapporte des statistiques à l’agent.

    La valeur minimale est de 5 secondes. L&apos;agent relève une valeur inférieure à 5.

    Si vous ne définissez pas cet attribut, l’agent déduit l’intervalle du cycle de collecte des métriques. Si vous configurez un intervalle de statistiques sur le client Kafka lui-même, cette valeur a priorité sur cet attribut.

    Ou utilisez la variable d’environnement `NEW_RELIC_KAFKA_METRICS_INTERVAL` .
  </Collapser>
  ...
  <Collapser id="kafka-metrics-cluster-metrics-enabled" title="clusterMetricsEnabled">
    ...
    Activez ou désactivez les métriques de production et de consommation Kafka par cluster. Définissez cet attribut sur `true` pour que l’agent enregistre les métriques du cluster.

    Ou utilisez la variable d’environnement `NEW_RELIC_KAFKA_METRICS_CLUSTER_METRICS_ENABLED` .
  </Collapser>
</CollapserGroup>
