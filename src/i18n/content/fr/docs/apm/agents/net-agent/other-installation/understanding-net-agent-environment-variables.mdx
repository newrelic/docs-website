...
## Définissez les variables d’environnement pour les applications hébergées dans IIS [#iis]

Si votre application est hébergée dans IIS, vous pouvez définir des variables d’environnement par pool d’applications en modifiant directement le fichier `ApplicationHost.config` d’IIS, au lieu de (ou en plus de) les définir comme variables d’environnement à l’échelle du système. Cela est utile lorsque vous souhaitez activer ou configurer l’agent .NET uniquement pour des pools d’applications spécifiques, plutôt que pour chaque processus sur la machine.

Par défaut, vous trouverez `ApplicationHost.config` à l’emplacement `C:\Windows\System32\inetsrv\config\ApplicationHost.config`. Dans la section `<applicationPools>`, chaque élément `<add>` représentant un pool d’applications peut contenir un élément enfant `<environmentVariables>`, qui contient à son tour des entrées `<add name="..." value="..." />` pour chaque variable d’environnement que vous souhaitez définir pour ce pool d’applications.

Par exemple, pour activer l&apos;agent .NET et définir quelques options de configuration courantes pour un pool d&apos;applications nommé `TestApplication`:
...
Vous pouvez également utiliser `applicationPoolDefaults` pour définir des variables d&apos;environnement qui s&apos;appliquent à tous les pools d&apos;applications par défaut, de la même manière que le feraient les variables d&apos;environnement à l&apos;échelle du système, à moins que les paramètres `environmentVariables` propres à un pool spécifique ne les remplacent.

<Callout title="Note">
  La modification directe de `ApplicationHost.config` nécessite des privilèges d&apos;administrateur, et vous devez recycler un pool d&apos;applications (ou redémarrer IIS) pour que les modifications prennent effet. Sauvegardez toujours le fichier avant de le modifier, car une configuration malformée peut empêcher IIS de démarrer des sites ou des pools d&apos;applications.
</Callout>

Pour plus de détails sur cet élément de configuration, consultez la documentation de Microsoft sur [applicationPoolDefaults environmentVariables](https://learn.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/applicationpooldefaults/environmentvariables/).