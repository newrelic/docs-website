...
Si vous consommez l&apos;API d’agent via `Microsoft.Extensions.DependencyInjection` (ou un conteneur similaire), il existe deux pièges qui désactiveront silencieusement votre instrumentation personnalisée sans produire d&apos;erreur. Les deux découlent du fait que [`IAgent`](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#IAgent) est un handle à longue durée de vie, mais que [`CurrentTransaction`](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#ITransaction) et [`CurrentSpan`](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#ISpan) sont des vues à l&apos;échelle de la requête qui doivent être relues à chaque appel.

### Enregistrer `IAgent` de manière différée, et non de manière anticipée

[`GetAgent()`](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#GetAgent) doit être appelé **après** que le profileur a fini de s&apos;attacher. Si vous le résolvez de manière anticipée pendant la construction du conteneur DI, vous risquez de mettre en cache l’espace réservé no-op que l’API renvoie avant que l’agent ne soit actif, laissant l’application avec une API silencieusement désactivée pour la durée de vie du processus. Utilisez une lambda de fabrique pour que l&apos;appel soit différé à la première résolution :
...
Pour un guide complet et exécutable, y compris des exemples d&apos;anti-modèles vérifiés à la compilation, consultez l&apos;[exemple d&apos;injection de dépendance (api-dependency-injection)](https://github.com/newrelic/newrelic-dotnet-examples/tree/main/api-dependency-injection) dans le référentiel `newrelic-dotnet-examples`.
...
<CollapserGroup>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Propriété donnant accès à la span en cours d&apos;exécution via l&apos;interface [ISpan](/docs/apm/agents/net-agent/net-agent-api/net-agent-api/#ISpan).
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <Collapser id="ITransaction" title="ITransaction">
    ...
    <CollapserGroup>
      ...
      <Collapser id="ITransaction.AddCustomAttribute" title="Ajouter un attribut personnalisé">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td>
                `arrays`, `Lists` et d&apos;autres types de `IEnumerable`
              </td>
      ...
      <Collapser id="CurrentSpan" title="CurrentSpan">
