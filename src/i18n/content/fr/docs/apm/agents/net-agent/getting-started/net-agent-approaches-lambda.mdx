...
Les types de fonctions AWS Lambda suivants sont automatiquement détectés et instrumentés sans aucune configuration supplémentaire (à partir de la version 10.29.0 de l&apos;agent) :

* `Amazon.Lambda.AspNetCoreServer.APIGatewayProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.APIGatewayHttpApiV2ProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.ApplicationLoadBalancerFunction`

D&apos;autres types de gestionnaires (tels que les déclencheurs SQS, SNS, Kinesis, S3, DynamoDB et les gestionnaires personnalisés) sont également instrumentés lorsque vous définissez la variable d&apos;environnement `NEW_RELIC_LAMBDA_HANDLER` sur le chemin d&apos;accès complet du gestionnaire (par exemple, `MyAssembly::MyNamespace.MyClass::MyMethod`). Cela inclut toute méthode de gestionnaire qui accepte un paramètre `ILambdaContext`.
...
* Le tracing distribué entrant depuis SQS nécessite que le producteur en amont injecte les en-têtes de la trace W3C (`traceparent`, `tracestate`) en tant qu&apos;attributs de message SQS. L&apos;agent les lit automatiquement lorsqu&apos;ils sont présents.
