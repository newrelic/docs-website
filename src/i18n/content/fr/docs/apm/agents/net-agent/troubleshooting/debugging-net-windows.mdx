...
<CollapserGroup>
  ...
  <Collapser id="env-var-core" title=".NET Core : vérifier les variables d'environnement">
    ...
    <Callout variant="tip">
      Pour obtenir des conseils sur la configuration de cette variable d&apos;environnement par processus plutôt qu&apos;à l&apos;échelle du système, consultez [notre article de forum sur la configuration des variables d&apos;environnement](https://knowledge.newrelic.com/s/article/Setting-dot-NET-Core-agent-environment-variables-per-process).
    </Callout>
  </Collapser>
</CollapserGroup>
