...
<Callout title="L’API REST pour les déploiements arrive en fin de vie" variant="important">
  Les points de terminaison de déploiement de l&apos;API REST v2 (`/v2/applications/{app_id}/deployments.json`) documentés sur cette page, ainsi que l&apos;API legacy de déploiement v0, [atteignent leur fin de vie le 31 juillet 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Après cette date, ils ne seront pas disponibles.

  Enregistrez les déploiements avec la fonctionnalité de [suivi des changements](/docs/change-tracking/overview/) à la place, en utilisant la mutation NerdGraph `changeTrackingCreateEvent`. Pour les modélisations point de terminaison par point de terminaison, consultez le guide [Migrate from REST API v2 to NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). Les instructions REST ci-dessous restent à titre de référence pour les intégrations existantes que vous migrez.
</Callout>

<Callout variant="tip">
  Le suivi des changements est disponible pour <InlinePopover type="apm" />, <InlinePopover type="browser" />, et <InlinePopover type="mobile" />.
</Callout>
...
Vous pouvez enregistrer de nouveaux déploiements et récupérer une liste des déploiements passés avec l&apos;[API REST v2](/docs/apis/rest-api-v2/requirements/new-relic-rest-api-v2-getting-started) de New Relic ou [NerdGraph](/docs/change-tracking/config/nerdgraph/). Utilisez NerdGraph : les points de terminaison de déploiement de l&apos;API REST v2 [atteignent leur fin de vie le 31 juillet 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). De plus, certains agents APM offrent des méthodes spécifiques à l&apos;agent pour enregistrer automatiquement les déploiements.
...
<Callout title="Ces points de terminaison REST atteignent leur fin de vie le 31 juillet 2027" variant="important">
  Les points de terminaison de déploiement de l’API REST v2 présentés ci-dessous [arrivent en fin de vie le 31 juillet 2027](/eol/2026/07/eol-07-31-26-rest-api-v2) et cesseront de fonctionner après cette date. Migrez vers la mutation NerdGraph `changeTrackingCreateEvent`, consultez le guide [Migrer de l’API REST v2 vers NerdGraph](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments). Ces instructions restent à titre de référence pendant que vous migrez les intégrations existantes.
</Callout>
...
<Callout variant="important">
  Les méthodes de l’API REST v2 référencées ci-dessous [arrivent en fin de vie le 31 juillet 2027](/eol/2026/07/eol-07-31-26-rest-api-v2). Enregistrez plutôt les déploiements avec la mutation NerdGraph `changeTrackingCreateEvent`, consultez le [guide de migration](/docs/apis/rest-api-v2/migrate-to-nerdgraph/#deployments).
</Callout>

Certains agents disposent de méthodes supplémentaires pour enregistrer les déploiements :

* <DNT>**All agents**</DNT>: utilisez l’ [API REST New Relic v2](#api-instructions).
* <DNT>**C**</DNT>: aucune méthode spécifique au SDK. Utilisez l’ [API REST](#api-instructions).
* <DNT>**Go**</DNT>: aucune méthode spécifique à l’agent. Utilisez l’ [API REST](#api-instructions).
...
* <DNT>**Node.js**</DNT>: aucune méthode spécifique à l’agent. Utilisez l’ [API REST](#api-instructions).
