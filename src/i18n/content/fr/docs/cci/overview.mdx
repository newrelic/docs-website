...
Une fois l&apos;installation terminée, Intelligence Coûts du cloud accède automatiquement à vos données de coûts et d&apos;utilisation à partir de votre conteneur de stockage d&apos;objets désigné. Ce processus commence après avoir accordé les autorisations d’accès nécessaires. Veuillez noter que notre service ne peut récupérer les rapports qu&apos;après leur exportation vers votre conteneur de stockage. Assurez-vous que le préfixe du chemin du dossier est configuré comme indiqué dans les [conditions préalables](/docs/cci/getting-started/prerequisites) et les instructions [de configuration](/docs/cci/getting-started/setup) respectives.
