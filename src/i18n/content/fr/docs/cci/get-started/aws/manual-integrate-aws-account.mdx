...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="important">
      N’oubliez pas le nom d’exportation pour l’intégration CCI. Assurez-vous que le préfixe du chemin S3 est `cost-and-usage-reports`. Le premier rapport est livré dans les 24 heures ; les rapports suivants sont livrés périodiquement. Votre exportation doit également inclure toutes les 114 colonnes requises pour que CCI puisse ingérer et traiter vos données de facturation : consultez le [dictionnaire de données](/attribute-dictionary) pour la liste complète.
    </Callout>
  </Step>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Intégrez vos services AWS à New Relic via les 

        [flux de métriques CloudWatch](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/)
