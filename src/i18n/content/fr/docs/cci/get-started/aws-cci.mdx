...
Configurez votre environnement AWS pour Cloud Cost Intelligence en configurant la facturation consolidée, les rapports de coûts et d&apos;utilisation, les rôles et stratégies IAM, et les régions AWS nécessaires.

## Avant de commencer

### Compte AWS avec les autorisations appropriées [#aws-account]

Assurez-vous que la [facturation consolidée est configurée pour AWS Organizations](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/consolidated-billing.html).
...
<Steps>
  <Step>
    ...
    2. Accédez au **S3 service** et cliquez sur **Create bucket**.

    3. Entrez un nom de compartiment unique (par exemple `<your-company-name>-billing`).

    4. Configurer les paramètres du compartiment :

       * Activer le versionnage
       * Désactiver l&apos;accès public (recommandé)
       * Activer le chiffrement par défaut (recommandé)

    5. **Balises (Facultatif)**: Ajoutez les balises nécessaires à la gestion des ressources de votre organisation et cliquez sur **Create bucket**.
  ...
  <Step>
    ...
    2. Accédez au **Billing Dashboard** &gt; **Cost &amp; Usage Analysis** &gt; **Data Exports**, puis cliquez sur **Create**.

    3. Configurer les paramètres du rapport :
    ...
    4. Cliquez sur **Next**, puis sur **Create report**.
  ...
  <Step>
    ...
    Créez une politique IAM qui accorde à CCI les autorisations d&apos;accès à vos données CUR :

    1. Connectez-vous à **la console de gestion AWS (AWS Management Console)**.
    2. Accédez à**IAM service** &gt; **Policies** et cliquez sur **Create policy**.
    3. Passez à l&apos; **éditeur JSON (JSON editor)** et collez la politique suivante :
    ...
    4. Cliquez sur **Next**.
    5. Nommez la politique `CCI-Access-Policy` (ou votre nom préféré), ajoutez une description et cliquez sur **Create policy**.
  </Step>
  ...
  <Step>
    ...
    1. Connectez-vous à **la console de gestion AWS (AWS Management Console)**.
    2. Accédez à **IAM service** &gt; **Roles** et cliquez sur **Create role**.
    3. Sélectionnez **Custom Trust Policy** dans la section type d&apos;entité approuvée et collez la définition de rôle suivante :
       ```json
       {
       "Version": "2012-10-17",
       "Statement": [
    ...
    4. Cliquez sur **Next**.
    5. Joignez la politique créée dans [Créer une politique IAM pour l&apos;accès CCI](#create-iam-policy) et cliquez sur **Next**.
    6. Nommez le rôle `CCI-Access-Role` (ou votre nom préféré), ajoutez une description (facultatif) et cliquez sur **Create role**.
    7. Notez l&apos;ARN du rôle à partir de la page de résumé du rôle.
  ...
  <Step>
    ### Accéder à l&apos;intégration et sélectionner le compte New Relic [#access-integration]

    1. Accédez à **Cloud Cost Intelligence** dans le panneau de gauche et cliquez sur **Add the integration**.
    2. Sélectionnez votre compte New Relic et cliquez sur **Continue**.
  </Step>
  ...
  <Step>
    ...
    * **Cross-account IAM role ARN** - ARN du rôle IAM pour que Cloud Cost Intelligence accède à votre compartiment S3 (format : `arn:aws:iam::<account-id>:role/<role-name>`).
    * **External ID** - Copiez l&apos;ID externe et ajoutez-le à votre stratégie d&apos;approbation de rôle IAM dans AWS.
    * **Data export name** - Nom du fichier d&apos;exportation de données. Pour le rapport `cci-ingestion-test`, le chemin d&apos;accès est `cost-and-usage-reports/cci-ingestion-test/data`.
    * **S3 bucket name** - Nom exact du bucket. Utilisez la région `us-east-2` pour des performances optimales.
  </Step>
  ...
  <Step>
    ...
    <Callout variant="important">
      Définissez `sts:ExternalId` sur l&apos;ID externe à partir de [Fournir les détails de la connexion](#provide-connection-details). Activez STS pour les régions où il n&apos;est pas actif par défaut (par exemple, `ap-east-1`, `eu-south-1`).
    </Callout>
  </Step>
  ...
  <Step>
    ...
    <Callout variant="important">
      Le traitement initial des données peut prendre quelques heures après la configuration.
    </Callout>
  </Step>
</Steps>
