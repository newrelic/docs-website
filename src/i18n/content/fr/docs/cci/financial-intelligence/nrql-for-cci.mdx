...
<CollapserGroup>
  <Collapser id="cloud-cost-attributes" title="Attribut CloudCost">
    Dans New Relic, accédez à **All Capabilities &gt; CloudCost Intelligence**. Ouvrez le panneau Console de requêtes inférieur et saisissez la requête NRQL suivante :
...
<CollapserGroup>
  <Collapser id="kubernetes-cost-attributes" title="Type d'événement de coût Kubernetes">
    ...
    Pour calculer les coûts Kubernetes précis, multipliez les paramètres `cpu_weightage + memory_weightage` dans votre requête de coût. L&apos;exemple suivant illustre le format de requête correct :

    ```sql
    SELECT SUM(net_unblended_cost * (cpu_weightage + memory_weightage)) FROM KubernetesCost SINCE '2025-09-01' UNTIL '2025-09-10'
    ```

    Cette requête multiplie `net_unblended_cost` par la somme des pondérations du processeur et de la mémoire pour fournir des répartitions de coûts précises basées sur l&apos;utilisation des ressources.
