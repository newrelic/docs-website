...
Récupérez votre <InlinePopover type="licenseKey" />(également appelé `ingest - license`) et définissez-le comme variable d&apos;environnement : `NEW_RELIC_INSERT_KEY`. [Cliquez ici](/docs/apis/intro-apis/new-relic-api-keys/#license-key) pour plus de détails et d&apos;instructions. Envoyez-vous des données vers la région EU ou JP de New Relic ? cliquez [ici](#EU-account-users) pour plus d&apos;instructions.
...
## Utilisateurs de comptes UE et JP [#EU-account-users]

Si vous utilisez un compte UE ou JP, envoyez-le comme paramètre lors de l&apos;appel `MLPerformanceMonitoring` si votre variable d&apos;environnement n&apos;est pas définie :
