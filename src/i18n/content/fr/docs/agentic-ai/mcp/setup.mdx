...
Claude Code prend en charge les méthodes d’authentification OAuth (recommandée) et par clé API. OAuth nécessite la version 2.1.186 de Claude Code ou plus tard. Si vous utilisez des versions plus anciennes, vous devriez vous authentifier via `/mcp` dans Claude au lieu de `claude mcp login`.
...
2. Configurez le serveur MCP en ouvrant (ou en créant) `~/.kiro/settings/mcp.json` et ajoutez la configuration suivante :
...
3. Ouvrez votre terminal et exécutez `kiro-cli`. Le serveur MCP New Relic se connectera automatiquement.
...
2. Redémarrez Claude Desktop. Sous Windows, les modifications peuvent ne pas prendre effet tant que l&apos;application Claude n&apos;est pas fermée via le Gestionnaire des tâches.
...
VS Code reste entièrement pris en charge avec les deux méthodes d’authentification. Vous avez besoin de VS Code (version 1.91 ou ultérieure).
