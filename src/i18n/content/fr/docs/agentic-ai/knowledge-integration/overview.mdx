...
* **Résolvez les incidents plus rapidement :** obtenez un accès immédiat au contexte pertinent à partir de vos runbooks et de votre documentation internes directement dans New Relic AI, ce qui permet un Délai moyen de résolution (MTTR) (MTTR) plus rapide.

* **Obtenez des recommandations exploitables et personnalisées :** Les réponses fournies sont spécifiques à votre environnement et basées sur vos propres bonnes pratiques et données historiques.

* **Répondez à des questions critiques et spécifiques au contexte :** trouvez facilement des solutions aux problèmes qui ont déjà été résolus. Posez des questions telles que :
...
Pour commencer à indexer votre contenu Confluence, vous avez besoin de :
