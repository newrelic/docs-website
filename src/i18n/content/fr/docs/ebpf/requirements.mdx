---
title: Compatibilité et exigences eBPF
metaDescription: 'Learn about New Relic eBPF agent compatibility requirements, supported databases, platforms, and querying capabilities.'
tags:
  - New Relic integrations with eBPF
  - New Relic eBPF agent
  - eBPF integration
  - eAPM
  - eBPF compatibility
  - eBPF requirements
freshnessValidatedDate: never
translationType: machine
---

Cette page fournit des informations complètes sur la compatibilité de l&apos;agent New Relic eBPF, notamment les données de base prises en charge, les exigences de déploiement et les capacités d&apos;interrogation des données qui s&apos;appliquent à toutes les méthodes d&apos;installation.

<Tabs>
  <TabsBar>
    <TabsBarItem id="k8s">Pour Kubernetes</TabsBarItem> <TabsBarItem id="linux">sous Linux</TabsBarItem>
  </TabsBar>

  <TabsPages>
    <TabsPageItem id="k8s">
      * Assurez-vous que tous les [prérequis de la distribution Linux](/docs/ebpf/requirements/#kernel-requirements) sont respectés pour le nœud Kubernetes.

      ### Environnements Kubernetes [#kubernetes-environments]

      Les environnements Kubernetes suivants sont pris en charge :

      * Amazon Elastic Kubernetes Service (EKS)
      * Google Kubernetes Engine (GKE)
      * Azure Kubernetes Service (AKS)
      * Red Hat OpenShift
      * Microk8s

      ### Prise en charge du runtime de conteneur [#container-runtime]

      Les environnements d&apos;exécution de conteneurs suivants sont pris en charge :

      * Runtime par défaut de Docker (runc)
      * containerd
      * CRI-O
      * Podman
      * Youki
      * crun
    </TabsPageItem>

    <TabsPageItem id="linux">
      ### Exigences du noyau [#kernel-requirements]

      * Versions du noyau Linux :

        * `x86-64` (amd64) : `v5.8` à `v7.1`
        * `arm64` (aarch64) : `v5.8` à `v7.1`

      <Callout variant="important">
        Certaines distributions Linux vous permettent d&apos;exécuter d&apos;anciennes versions du noyau sur des versions plus récentes du système d&apos;exploitation. Même si votre distribution est [listée ci-dessous](#supported-linux-distributions), l&apos;agent eBPF ne fonctionnera pas si la version du noyau est inférieure à `5.8`. La même chose s&apos;applique aux noyaux plus récents : l&apos;agent ne fonctionnera pas sur les versions supérieures à la plage prise en charge.
      </Callout>

      ### Distributions Linux prises en charge [#supported-linux-distributions]

      Les distributions Linux suivantes sont prises en charge :

      * CentOS `9` et versions ultérieures
      * Debian `11` et versions ultérieures
      * Red Hat Enterprise Linux `9.0` et versions ultérieures
      * Ubuntu `20.04` et versions ultérieures
      * Amazon Linux `2` et `2023`
      * Bottle Rocket `1.51.0` et versions ultérieures
      * AlmaLinux `9.7` et versions ultérieures
      * Oracle Linux `8` et versions ultérieures
      * openSUSE Leap `15.5` et versions ultérieures
      * SLES `15.7` et versions ultérieures

      ### exigences système [#system-requirements]

      * Privilèges root (requis pour l&apos;installation et le fonctionnement)
      * Un minimum de `2 GB` de RAM
    </TabsPageItem>
  </TabsPages>
</Tabs>

### matrice de support de base de données [#database-support]

L&apos;agent eBPF peut monitorer les données de base suivantes et leurs versions prises en charge :

<table>
  <thead>
    <tr>
      <th>
        base de données
      </th>

      <th>
        Versions prises en charge
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        MySQL
      </td>

      <td>
        `v5.6.51`

         et supérieur
      </td>
    </tr>

    <tr>
      <td>
        MongoDB
      </td>

      <td>
        `v3.6.23`

         et supérieur
      </td>
    </tr>

    <tr>
      <td>
        Redis
      </td>

      <td>
        `v3.2.12`

         et supérieur
      </td>
    </tr>

    <tr>
      <td>
        PostgreSQL
      </td>

      <td>
        `v9.6.24`

         et supérieur
      </td>
    </tr>

    <tr>
      <td>
        MSSQL
      </td>

      <td>
        `v14.x`

         et supérieur
      </td>
    </tr>

    <tr>
      <td>
        DynamoDB
      </td>

      <td>
        `v2.0.0`

         et supérieur
      </td>
    </tr>
  </tbody>
</table>

### Prise en charge de protocoles supplémentaires [#protocol-support]

L&apos;agent eBPF prend également en charge le monitoring de ces protocoles :

* HTTP

* Apache Thrift

* gRPC

* Cassandre

* Kafka

* AMQP

  <Callout variant="tip">
    Pour Cassandra, Kafka et AMQP, l&apos;agent eBPF ne prend pas en charge la liaison de segments.
  </Callout>

### Matrice de prise en charge Thrift [#thrift-support]

L&apos;agent eBPF prend en charge les protocoles de transport et les couches de transport Thrift suivants :

<table>
  <thead>
    <tr>
      <th>
        Type
      </th>

      <th>
        Soutenu
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Protocoles de transport
      </td>

      <td>
        `TBinaryProtocol`

        , 

        `TCompactProtocol`

        , 

        `TJSONProtocol`
      </td>
    </tr>

    <tr>
      <td>
        Couches de transport
      </td>

      <td>
        `TSocket`

        , 

        `TFramedTransport`

        , 

        `TZlibTransport`
      </td>
    </tr>
  </tbody>
</table>

### Exigences réseau [#network-requirements]

Tous les eBPF déployés nécessitent un accès à ces points de terminaison :

<table>
  <thead>
    <tr>
      <th>
        Type d&apos;exigence
      </th>

      <th>
        Détails
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        liste blanche d&apos;URL
      </td>

      <td>
        * `otlp.nr-data.net:443`
        * `otlp.eu01.nr-data.net:443` (Région UE)
        * `otlp.jp.nr-data.net:443` (région JP)
        * `gov-otlp.nr-data.net:443` (Comptes GOV/FedRAMP)
        * `https://downloads.newrelic.com`
      </td>
    </tr>

    <tr>
      <td>
        Liste d&apos;autorisation des ports
      </td>

      <td>
        * Pour activer la communication externe, le port `443`
      </td>
    </tr>

    <tr>
      <td>
        Exigences du chemin de montage
      </td>

      <td>
        * Système de fichiers racine (`/`)
        * Système de fichiers système (`/sys`)
      </td>
    </tr>
  </tbody>
</table>

<Callout variant="tip">
  Le monitoring eBPF est indépendant du langage et fournit des métriques de latence et de débit pour toute application. Cependant, [la liaison des segments de transaction](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm/#segments) dépend du langage et n&apos;est actuellement pas prise en charge pour PHP et Node.js.
</Callout>

Si votre environnement achemine le trafic sortant via un proxy HTTP d’entreprise, consultez [Configurer un proxy pour le <DNT>eBPF agent</DNT>](/docs/ebpf/proxy-configuration) pour obtenir des instructions de configuration.

{/* ## Querying eBPF data [#querying-data]

Once the eBPF agent is deployed, telemetry data flows to New Relic and can be queried using NRQL. Here are common query patterns:

### Query spans

Including full-body requests and trace data:

```sql
FROM Span
SELECT * WHERE
deployment.name = '<deployment-name>' LIMIT MAX
```

### Query metrics

Including TCP stats and performance metrics:

```sql
FROM Metric
SELECT * WHERE
deployment.name = '<deployment-name>' LIMIT MAX
```

<Callout variant="tip">
  Replace `<deployment-name>` with the actual deployment name you configured during installation.
</Callout>

### Entity naming

The eBPF agent automatically generates entity names differently depending on the environment:

* **In hosts or Docker**: Names are a combination of the process name, its directory or container ID, and the listening port. For example, `ruby:/home/ubuntu/app:[5678]` or `java:f4aead533895:[8080]`.

* **In Kubernetes**: Names are derived from the service name, for example, `mysql-database-service`.

## Data access in New Relic [#data-access]

eBPF data is accessible through multiple New Relic interfaces:

1. **APM & Services UI**: Go to **[one.newrelic.com](https://one.newrelic.com) > APM & Services** and filter by `instrumentation.name = nr_ebpf`

2. **NRQL queries**: Use the query examples above to explore your data

3. **OpenTelemetry UI**: eBPF data integrates with New Relic's OpenTelemetry experience

For more information on accessing your data, see [OpenTelemetry APM UI](/docs/opentelemetry/get-started/apm-monitoring/opentelemetry-apm-ui). */}

## Documentation associée [#related-docs]

<DocTiles>
  <DocTile title="Installation eBPF Kubernetes" path="/docs/ebpf/k8s-installation/">
    Découvrez comment installer l&apos;agent eBPF sur Kubernetes.
  </DocTile>

  <DocTile title="Installation Linux eBPF" path="/docs/ebpf/linux-installation/">
    Apprenez comment installer l&apos;agent eBPF sur les hôtes Linux.
  </DocTile>

  <DocTile title="dépannage eBPF" path="/docs/ebpf/troubleshooting/no-ui-data/">
    Apprenez à résoudre les problèmes liés à l&apos;agent eBPF.
  </DocTile>
</DocTiles>