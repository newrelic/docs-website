...
* **Données télémétriques cloisonnées :** après la collecte des données, la corrélation des logs d&apos;application avec la span du réseau et les mesures de infrastructure nécessite un travail manuel approfondi, conduisant à une observabilité incomplète et à une résolution tardive incident.
...
* **Collecte de données unifiée :** Fonctionnant au niveau du noyau, notre agent unique collecte simultanément des données d&apos;observabilité, notamment les spans d&apos;application, les logs, le trafic réseau et les métriques infrastructure, résolvant ainsi le problème des silos de données.
...
<Callout variant="tip">
  L&apos;agent eBPF est conforme aux normes HIPAA et FedRAMP, ce qui le rend adapté à une utilisation dans des environnements réglementés.
</Callout>
