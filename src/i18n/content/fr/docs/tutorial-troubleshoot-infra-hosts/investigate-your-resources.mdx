...
<Steps>
  <Step>
    ...
    * `host-tower-portland` a un événement d&apos;alerte critique.
...
Jusqu&apos;à présent, nous avons vu comment utiliser les données d&apos;infrastructure pour dépanner un événement d&apos;alerte lié aux ressources. Nous avons vu comment passer de milliers d&apos;hôtes à un ensemble d&apos;hôtes, puis corréler les données pour prendre une décision. La documentation suivante vous explique comment créer des dashboards personnalisés à l&apos;aide de métriques d&apos;infrastructure.
