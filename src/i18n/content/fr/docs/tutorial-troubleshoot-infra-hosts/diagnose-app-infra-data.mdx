...
<Steps>
    ...
    <Tabs>
      ...
      <TabsPages>
        <TabsPageItem id="hosts-via-alerts">
          Si vous ne savez pas par où commencer, nous vous recommandons de filtrer d&apos;abord vos hôtes en fonction de la gravité des alertes. En utilisant la vue d&apos;ensemble de la page de synthèse, vous pouvez voir que trois événements d&apos;alerte critiques se produisent dans votre système.
  ...
  <Step>
    ...
    Une fois que vous avez identifié un hôte lié à l&apos;événement d&apos;alerte, vous pouvez cliquer pour afficher les données concernant uniquement cet hôte. Dans ce scénario, nous avons choisi `apache-svr01`. Puisque nous essayons de résoudre un problème lié à une application, nous commençons par la carte des services sur la page de l&apos;hôte. Cette carte vous montre quelles applications dépendent de l&apos;hôte sélectionné.
  ...
  <Step>
    ...
    Ce filtre a élargi la portée de l&apos;événement d&apos;alerte au-delà de votre hôte `apache_svr01` initial, tout en limitant vos données à un ensemble pertinent. À partir d&apos;ici, vous pouvez commencer à analyser plus en détail quelle limitation de ressources affecte les performances.
  ...
  <Step>
    ...
    * La tuile <DNT>**Alerts**</DNT> affiche le nombre d&apos;hôtes actuellement en alerte avec des seuils critiques ou d&apos;avertissement. Une augmentation constante des alertes au fil du temps peut indiquer quand un changement a intensifié le comportement des événements d&apos;alerte.

    Les tuiles et les graphiques de métriques peuvent vous aider à déterminer le moment approximatif d&apos;un événement d&apos;alerte. Cela est particulièrement utile si la cause d&apos;un événement d&apos;alerte est due à une mise à jour d&apos;un fournisseur externe ou à un déploiement d&apos;une autre équipe. Si c&apos;est le cas, votre prochaine étape pour approfondir l&apos;analyse changerait.
  </Step>
</Steps>
...
Mais comment utilisez-vous vos données d&apos;infrastructure pour prendre une décision concernant l&apos;allocation des ressources ? La documentation suivante explique comment approfondir l&apos;analyse d&apos;un événement d&apos;alerte plus spécifique, comme le dépannage d&apos;une utilisation élevée du CPU.
