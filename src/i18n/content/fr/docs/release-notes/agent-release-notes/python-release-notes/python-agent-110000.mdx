...
* Garantit que les spans MCP ne sont enregistrées que lorsque monitoring de l&apos;IA est activée
...
* Ajoute une nouvelle instrumentation pour [AutoGen](https://github.com/microsoft/autogen)
  ...
  * Ajoute une instrumentation (spans nommées et événement `Llm` personnalisé) pour les agents et les outils dans [AutoGen](https://github.com/microsoft/autogen)
...
* Ajoute de nouveaux MCP (Model Context Protocol) nommés spans
  ...
  * Ajoute de nouvelles spans nommées pour les appels d&apos;outils MCP (Model Context Protocol) effectués via le gestionnaire d&apos;outils pour prendre en charge Streamable HTTP
...
* Garantit que les spans MCP (Model Context Protocol) ne sont enregistrées que lorsque monitoring de l&apos;IA est activée

  * Garantit que les spans MCP (Model Context Protocol) ne sont enregistrées que lorsque le paramètre `ai_monitoring.enabled` est activé.
