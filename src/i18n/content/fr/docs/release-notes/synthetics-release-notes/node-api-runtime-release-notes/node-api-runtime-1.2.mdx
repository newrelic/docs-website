---
subject: Node API runtime
releaseDate: '2026-09-03'
features: []
enhancements: []
bugs:
  - 'For the Scripted API monitors, updated script execution to run in an isolated context, preventing scripts from traversing the JavaScript prototype chain to access sandbox-restricted objects and functions.'
security: []
version: 1.2.267
translationType: machine
---

### Corrections

* **Moniteurs d&apos;API scriptés**: l&apos;exécution du script a été mise à jour pour s&apos;exécuter dans un contexte isolé, empêchant les scripts de traverser la chaîne de prototypes JavaScript pour accéder aux objets, et fonctions restreints par le bac à sable.

### Correctifs de sécurité

Les vulnérabilités suivantes ont été corrigées :

**Vulnérabilités du système d&apos;exploitation :**

* CVE-2024-2236
* CVE-2025-5278
* CVE-2025-6141
* CVE-2025-15649
* CVE-2026-7017
* CVE-2026-9538
* CVE-2026-12087
* CVE-2026-13221
* CVE-2026-13595
* CVE-2026-13757
* CVE-2026-18938
* CVE-2026-27171
* CVE-2026-27456
* CVE-2026-42250
* CVE-2026-48959
* CVE-2026-48962
* CVE-2026-53612
* CVE-2026-53613
* CVE-2026-53614
* CVE-2026-53615
* CVE-2026-53910
* CVE-2026-54371
* CVE-2026-57432
* CVE-2026-57433
* CVE-2026-59843
* CVE-2026-59845
* CVE-2026-59846
* CVE-2026-59847
* CVE-2026-59848
* CVE-2026-59850

**Vulnérabilités de la bibliothèque :**

* CVE-2026-55553