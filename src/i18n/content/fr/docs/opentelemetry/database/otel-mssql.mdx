...
<Steps>
  ...
  <Step>
    ...
    * **Linux:** `/etc/nrdot-collector-host/config.yaml`
    ...
    <Callout variant="important">
      Si vous activez les boutons de catégorie de métriques principales et que vous remarquez des lacunes dans les données, augmentez `timeout` et `collection_interval` pour donner plus de temps aux scrapers pour terminer. Gardez toujours la valeur du délai d&apos;attente inférieure à `collection_interval` pour éviter le chevauchement des cycles de scrape.
