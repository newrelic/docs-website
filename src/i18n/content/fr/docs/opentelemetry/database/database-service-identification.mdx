...
* Agent .NET : version `10.52.0` ou ultérieure
...
### Activer la corrélation APM-base de données pour l&apos;agent Java [#enable-correlation-java]
...
### Activer la corrélation APM-base de données pour l&apos;agent .NET [#enable-correlation-dotnet]

Pour activer cette corrélation, ajoutez la propriété `sqlMetadataComments` à la section `transactionTracer` de votre fichier de configuration de l&apos;agent .NET (`newrelic.config`). Pour plus d&apos;informations, consultez la [documentation de configuration de l&apos;agent .NET](https://docs.newrelic.com/docs/apm/agents/net-agent/configuration/net-agent-configuration/).

**Option 1 : via les variables d&apos;environnement (script) :** si vous utilisez un script de démarrage ou un wrapper pour lancer votre exécutable, définissez la variable `NEW_RELIC_TRANSACTION_TRACER_SQL_METADATA_COMMENTS_ENABLED` sur `1`.
...
**Option 2 : via `newrelic.config`:** vous pouvez également activer cela directement dans le bloc `<transactionTracer>` de votre fichier `newrelic.config` :
