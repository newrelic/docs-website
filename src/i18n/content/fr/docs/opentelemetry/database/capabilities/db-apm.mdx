...
<Callout variant="important">
  Les entités APM et de base de données sont liées en faisant correspondre l’adresse &amp; le port de connexion à la base de données de l’application avec l’adresse et le port de connexion de l’agent de monitoring de base de données. Cette liaison fonctionne de manière fiable lorsque les deux parties signalent exactement la même adresse et le même port. Cependant, la liaison automatique peut échouer si votre application achemine le trafic via un équilibreur de charge ou un proxy, ou si les deux composants utilisent des formats d’adresse différents (comme une adresse IP par opposition à un nom de domaine).
</Callout>
