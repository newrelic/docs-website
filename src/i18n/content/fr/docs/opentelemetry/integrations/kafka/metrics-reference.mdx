    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            Nombre de réplicas synchronisés pour une partition. Cette métrique est filtrée par le pipeline du collecteur — seul le `kafka.partition.replicas_in_sync.total` agrégé par topic est conservé.
          </td>
...
* **Kafka auto-hébergé**: [Agent Java OpenTelemetry](/docs/opentelemetry/integrations/kafka/self-hosted#java-agent) ou [exportateur JMX Prometheus](/docs/opentelemetry/integrations/kafka/self-hosted#prometheus)
* **Kubernetes (autogéré)**: [agent Java OpenTelemetry](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#java-agent) ou [exportateur JMX Prometheus](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#prometheus)
* **Kubernetes (Strimzi)**: [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-strimzi#configure-jmx-exporter) avec la configuration personnalisée New Relic

Les deux méthodes collectent le même ensemble de métriques de broker Kafka et de JVM documentées ci-dessous :

<CollapserGroup>
  <Collapser id="jmx-kafka-metrics" title="Métriques de broker Kafka">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Le nombre d&apos;élections du leader. Apparaît uniquement lorsqu&apos;une élection de leader se produit ; n&apos;est pas émis sur les clusters stables.
          </td>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Taille de la file d&apos;attente des requêtes sur le broker
          </td>
...
## Métriques de client Kafka [#kafka-client-metrics]
...
<CollapserGroup>
  <Collapser id="producer-metrics" title="Métriques du producteur">
    ...
    | `kafka.producer.incoming_byte_rate`        | Taux d&apos;octets entrants (réponses des brokers) | identifiant-client, identifiant-nœud |
    | `kafka.producer.incoming_byte_total`       | Total des octets entrants (réponses des brokers)   | identifiant-client, identifiant-nœud |
  ...
  <Collapser id="consumer-metrics" title="Métriques du consommateur">
    ...
    | `kafka.consumer.partition_assigned_latency_avg` | Latence moyenne d&apos;affectation de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs.  | client-id |
    | `kafka.consumer.partition_assigned_latency_max` | Latence maximale d&apos;affectation de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs. | client-id |
    | `kafka.consumer.partition_revoked_latency_avg`  | Latence moyenne de révocation de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs.       | client-id |
    | `kafka.consumer.partition_revoked_latency_max`  | Latence maximale de révocation de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs.      | client-id |
    | `kafka.consumer.partition_lost_latency_avg`     | Latence moyenne de perte de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs.            | client-id |
    | `kafka.consumer.partition_lost_latency_max`     | Latence maximale de perte de partition (ms). Émis uniquement lors des rééquilibrages de groupes de consommateurs.           | client-id |
...
* [Interroger et visualiser vos données](/docs/opentelemetry/integrations/kafka/find-and-query-data) - Trouver des métriques dans l&apos;interface utilisateur New Relic, écrire des requêtes NRQL, créer des dashboards et configurer des alertes
* [Interroger les types de données de métriques](/docs/data-apis/understand-data/metric-data/query-metric-data-type/) - Découvrez des techniques avancées pour travailler avec les métriques OpenTelemetry