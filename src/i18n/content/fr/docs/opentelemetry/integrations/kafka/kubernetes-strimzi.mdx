...
<Steps>
  ...
  <Step>
    ...
    **Étape 1. Créer la ConfigMap des métriques JMX**
    ...
    **Étape 2. Mettre à jour le cluster Kafka pour utiliser l&apos;exportateur JMX**
          ...
          <CollapserGroup>
            <Collapser id="helm-nrdot" title="Utilisation de NRDOT Collector (recommandé)">
              ...
              **Paramètres de configuration**
              ...
              | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
              | `config.receivers.kafkametrics/cluster.brokers`                                                            | Remplacer par votre service bootstrap Kafka                                                                             |
              | `config.receivers.prometheus/kafka-jmx.config.scrape_configs[0].kubernetes_sd_configs[0].namespaces.names` | Remplacer par l&apos;espace de nommage où votre cluster Kafka est déployé                                               |
              | `config.receivers.prometheus/kafka-jmx.config.scrape_configs[0].relabel_configs[strimzi_io_name].regex`    | Remplacez par le nom de votre cluster Strimzi Kafka suivi de `-kafka`                                                   |
              | `config.receivers.prometheus/kafka-jmx.config.scrape_configs[0].relabel_configs[strimzi_io_cluster].regex` | Remplacez par le nom de votre cluster Strimzi Kafka                                                                     |
          ...
          **Étape 2. Créer des fichiers manifestes**
          ...
          <CollapserGroup>
            <Collapser id="manifest-nrdot" title="Utilisation de NRDOT Collector (recommandé)">
              ...
              **Étape 1. Créer `collector-rbac.yaml`** - Configuration RBAC pour l&apos;accès à l&apos;API Kubernetes :
              ...
              **Étape 2. Créer `collector-configmap.yaml`** - Configuration du Collector OpenTelemetry :
              ...
              **Étape 3. Créer `collector-deployment.yaml`** - Déploiement de l&apos;OpenTelemetry Collector :
              ...
              **Paramètres de configuration**
              ...
              | paramètres                                                                                          | Description                                                                                                             |
              | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
              | `receivers.kafkametrics/cluster.brokers`                                                            | Remplacer par votre service bootstrap Kafka                                                                             |
              | `receivers.prometheus/kafka-jmx.config.scrape_configs[0].kubernetes_sd_configs[0].namespaces.names` | Remplacer par l&apos;espace de nommage où votre cluster Kafka est déployé                                               |
              | `receivers.prometheus/kafka-jmx.config.scrape_configs[0].relabel_configs[strimzi_io_name].regex`    | Remplacez par le nom de votre cluster Strimzi Kafka suivi de `-kafka`                                                   |
              | `receivers.prometheus/kafka-jmx.config.scrape_configs[0].relabel_configs[strimzi_io_cluster].regex` | Remplacez par le nom de votre cluster Strimzi Kafka                                                                     |
              | `processors.resource/cluster-name.attributes[kafka.cluster.name].value`                             | Remplacez par le nom de votre cluster Kafka (ceci sera utilisé pour identifier et filtrer vos métriques dans New Relic) |
            </Collapser>
            ...
            <Collapser id="manifest-otel" title="Utiliser le collecteur OpenTelemetry">
              ...
              **Étape 1. Créer `collector-rbac.yaml`** - Identique à l&apos;option NRDOT ci-dessus (la configuration RBAC est identique)

              **Étape 2. Créer `collector-configmap.yaml`** - Identique à l&apos;option NRDOT ci-dessus (la configuration est identique)

              **Étape 3. Créer `collector-deployment.yaml`** - Déploiement de l&apos;OpenTelemetry Collector (seule l&apos;image diffère) :
              ...
              **Paramètres de configuration**: mêmes paramètres que l&apos;option NRDOT ci-dessus. Consultez le tableau des paramètres de configuration pour plus de détails.
            </Collapser>
          </CollapserGroup>
    ...
    <Callout variant="tip">
      ...
      <CollapserGroup>
        <Collapser id="app-collector-config" title="Configuration du collecteur pour la télémétrie applicative">
          ...
          **Étape 2. Ajouter à la section exportateurs :**
          ...
          **Étape 3. Ajouter à la section service.pipelines :**
  ...
  <Step>
    ### (Facultatif) Transmettre les logs du broker Kafka [#forward-broker-logs]
    ...
    <CollapserGroup>
      <Collapser id="configure-log-collection-strimzi" title="Configurer la collecte des logs">
        Ajoutez ce qui suit à votre configuration du collecteur `values.yaml` :
        ...
        **Paramètres de configuration**
        ...
        | paramètres                                            | Description                                                                                                                                                                                                                                                                                                                                                                                                |
        | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
        | `filelog/kafka_broker_0.include`                      | Remplacez `/var/log/kafka/server.log` par le chemin d&apos;accès réel de vos logs Kafka dans le pod du broker. Pour Strimzi, les logs sont généralement écrits sur stdout — utilisez la [configuration de log de Strimzi](https://strimzi.io/docs/operators/latest/configuring.html#property-kafka-logging-reference) pour les rediriger vers un fichier, ou utilisez plutôt un collecteur de log sidecar. |
      ...
      <Collapser id="find-broker-logs-in-new-relic-strimzi" title="Trouvez vos logs dans New Relic">
...
| Logs      | `Log`                 | Logs des applications productrices et consommatrices (via l&apos;agent Java OTel) et logs du broker collectés via l&apos;étape facultative de transfert de logs |
...
Les logs des applications productrices et consommatrices instrumentées avec l&apos;agent Java OpenTelemetry, et les logs du broker collectés via l&apos;étape facultative de transfert de logs, sont stockés dans le type d&apos;événement `Log` :
...
Si vous déployez des applications de producteur ou de consommateur instrumentées avec l&apos;agent Java OpenTelemetry, les spans de producteur et de consommateur sont stockés dans le type d&apos;événement `Span` :
