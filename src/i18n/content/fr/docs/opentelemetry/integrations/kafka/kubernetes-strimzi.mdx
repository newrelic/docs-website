...
* Kafka déployé via l&apos;[opérateur Strimzi](https://strimzi.io/) avec JMX activé. Assurez-vous que JMX est activé sur votre cluster Kafka dans la ressource Kafka Strimzi :
...
<Steps>
  ...
  <Step>
    ### Créer un secret avec la clé de licence [#create-secret]
  ...
  <Step>
    ### Déployer le Collecteur OpenTelemetry [#deploy-collector]

    #### 3.1 Construire une image de collecteur personnalisée [#build-image]

    Créer une image de collecteur personnalisée avec le runtime Java et le scraper JMX. Choisissez entre NRDOT Collector (distribution New Relic) ou OpenTelemetry Collector :
      ...
      <TabsPages>
        <TabsPageItem id="nrdot">
          ...
          <Callout variant="important">
            **Compatibilité des versions**: Ce guide utilise JMX Scraper 1.52.0 et NRDOT Collector. Pour des résultats optimaux, utilisez les dernières versions.

            <CollapserGroup>
              <Collapser id="nrdot-version-compatibility" title="Rechercher la dernière version de NRDOT Collector">
                **Vérifier les dernières versions**:

                * NRDOT Collector : Consultez [NRDOT Collector releases](https://github.com/newrelic/nrdot-collector-releases/releases/latest) pour trouver la dernière version et remplacez `{NRDOT_VERSION}` dans le Dockerfile
            ...
            **Architecture cible**: L&apos;exemple ci-dessous utilise `linux_amd64`. Consultez la page [NRDOT Collector releases](https://github.com/newrelic/nrdot-collector-releases/releases/latest) pour trouver le binaire approprié à votre architecture système (par ex. `linux_amd64`, `linux_arm64`). Mettez à jour l&apos;URL de téléchargement dans le Dockerfile en conséquence.
          </Callout>
        ...
        <TabsPageItem id="otel">
          <Callout variant="important">
            **Compatibilité des versions**: Ce guide utilise JMX Scraper 1.52.0 et OpenTelemetry Collector 0.145.0. Les anciennes versions du collecteur peuvent ne pas inclure le hash de ce scraper dans leur liste de compatibilité. Pour de meilleurs résultats, utilisez les dernières versions comme indiqué dans ce guide.
    ...
    **Construire et pousser l&apos;image :**
    ...
    Après avoir enregistré le Dockerfile, construisez et poussez l&apos;image vers votre registre de conteneurs :
