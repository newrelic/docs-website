...
* `https://otlp.nr-data.net:4317` avec votre point de terminaison OTLP New Relic (utilisez `https://otlp.eu01.nr-data.net:4317` pour la région UE). Pour les autres points de terminaison et options de configuration, consultez [Configurez votre point de terminaison OTLP](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol).
