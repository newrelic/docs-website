...
<Steps>
  ...
  <Step>
    ...
    * Endpoint OTLP : utilise `https://otlp.nr-data.net:4317` (région US) ou `https://otlp.eu01.nr-data.net:4317` (région UE). Pour d&apos;autres configurations de point de terminaison, consultez [Configurer votre point de terminaison OTLP](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)
  </Step>
...
<CollapserGroup>
  ...
  <Collapser id="ingest-optimization" title="Réduire l'ingestion de données">
    ...
    **1. Supprimez la section Additional metrics de la configuration JMX**
    ...
    Dans `~/opentelemetry/jmx-custom-config.yaml`, supprimez tout ce qui suit ce commentaire (jusqu&apos;à la fin du fichier) :
    ...
    **2. Désactiver les métriques d&apos;offset des consommateurs dans le récepteur kafkametrics**
    ...
    Dans `~/opentelemetry/kafka-config.yaml`, ajoutez à la section `metrics` du récepteur `kafkametrics` :
