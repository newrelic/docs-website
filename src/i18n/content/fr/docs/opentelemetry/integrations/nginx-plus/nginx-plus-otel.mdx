...
<CollapserGroup>
  ...
  <Collapser id="step2-enable-additional-metrics-otelcol" title="(Facultatif) Activer les métriques supplémentaires">
    ...
    1. **Identifiez les métriques que vous souhaitez activer** à partir de la [liste des métriques](#metrics) ci-dessous. Notez la catégorie à laquelle ils appartiennent (par ex. Zones de serveur HTTP, Zones de localisation, Cache).
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-dashboard" title="Vérifiez les données dans New Relic">
    Accédez à votre [dashboard NGINX](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data) pour vérifier la collecte des données.
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="plus-server-zone" title="Zones de serveur HTTP">
    <Callout variant="important">
      **Remarque :** Ces métriques sont désactivées par défaut. Pour les activer, consultez [(Facultatif) Activer les métriques supplémentaires](#step2-enable-additional-metrics-otelcol) pour OpenTelemetry Collector Contrib.
    </Callout>
