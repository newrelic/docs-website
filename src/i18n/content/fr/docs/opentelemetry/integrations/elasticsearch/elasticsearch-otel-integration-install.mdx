...
* **OpenTelemetry Collector** - Vous devez avoir un OpenTelemetry Collector installé et en cours d&apos;exécution sur votre hôte pour monitorer Elasticsearch. Il existe deux distributions prises en charge :

  * **NRDOT (Recommandé) :** Suivez le [guide d&apos;installation officiel de NRDOT](/docs/opentelemetry/nrdot/nrdot-collector/) pour installer le collecteur sur votre hôte.
  * **OTel Contrib :** [OpenTelemetry Collector Contrib](https://github.com/open-telemetry/opentelemetry-collector-contrib/releases/latest) installé et en cours d&apos;exécution sur votre hôte. Installez via un paquet officiel (.deb ou .rpm) pour garantir que l&apos;unité de service systemd est créée correctement.
...
<CollapserGroup>
  ...
  <Collapser id="logging-config" title="Activer les logs (récepteur filelog)">
    **Facultatif :** Incluez ceci si vous souhaitez envoyer des fichiers logs Elasticsearch à New Relic en plus des métriques.
    ...
    Ajoutez la configuration du récepteur `filelog` pour collecter et transférer les logs Elasticsearch. Assurez-vous que l&apos;utilisateur exécutant le service de collecteur (par exemple, nrdot-collector ou otelcol-contrib) dispose d&apos;un accès en lecture à vos fichiers logs Elasticsearch.
