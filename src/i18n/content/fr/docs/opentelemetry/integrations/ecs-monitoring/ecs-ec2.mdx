...
<Steps>
  ...
  <Step>
    ...
    Créez une nouvelle définition de tâche ECS qui inclut le conteneur sidecar du collecteur. Choisissez entre NRDOT Collector (la distribution de New Relic) ou OpenTelemetry Collector, puis sélectionnez la définition de tâche pour votre plateforme de conteneurs :

    <Tabs>
      <TabsBar>
        <TabsBarItem id="ec2-nrdot">Collecteur NRDOT</TabsBarItem> <TabsBarItem id="ec2-otel">Collecteur OpenTelemetry</TabsBarItem>
      </TabsBar>

      <TabsPages>
        <TabsPageItem id="ec2-nrdot">
          <Callout variant="tip">
            **NRDOT Collector** est la distribution New Relic de l’OpenTelemetry Collector avec le support de New Relic pour l’assistance. Il regroupe les récepteurs `awsecscontainermetrics` et `hostmetrics` utilisés par cette configuration, il s’exécute donc avec la même configuration de collecteur que vous avez stockée à l’étape précédente.
          </Callout>
