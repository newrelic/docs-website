...
<Steps>
  ...
  <Step>
    ...
    Créez une nouvelle définition de tâche ECS pour Fargate qui inclut le conteneur sidecar du collecteur. Choisissez entre NRDOT Collector (la distribution de New Relic) ou OpenTelemetry Collector, puis sélectionnez la définition de tâche pour votre plateforme de conteneurs :

    <Tabs>
      <TabsBar>
        <TabsBarItem id="fargate-nrdot">Collecteur NRDOT</TabsBarItem> <TabsBarItem id="fargate-otel">Collecteur OpenTelemetry</TabsBarItem>
      </TabsBar>

      <TabsPages>
        <TabsPageItem id="fargate-nrdot">
          <Callout variant="tip">
            **NRDOT Collector** est la distribution New Relic de l&apos;OpenTelemetry Collector avec le support de New Relic pour l&apos;assistance. Il intègre le récepteur `awsecscontainermetrics` utilisé par cette configuration, il s&apos;exécute donc avec la même configuration de collecteur que vous avez enregistrée à l&apos;étape précédente.
          </Callout>

          <Callout variant="important">
            NRDOT Collector publie uniquement des images de conteneurs Linux. Pour monitorer les conteneurs Windows, utilisez l&apos;onglet **OpenTelemetry Collector**.
          </Callout>
