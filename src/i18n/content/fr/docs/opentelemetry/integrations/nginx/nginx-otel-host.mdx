...
/\* &lt;Callout variant=&quot;important&quot;&gt; \*\*Avant de commencer\*\* : Consultez la \[présentation de NGINX OpenTelemetry]\(/docs/opentelemetry/integrations/nginx/nginx-otel-overview/) pour comprendre ce que vous allez monitorer et les avantages de cette intégration. \*\*Autres options du monitoring NGINX :\*\* - Pour monitorer \*\*NGINX Plus\*\*, consultez \[Monitorer NGINX Plus avec OpenTelemetry]\(/docs/opentelemetry/integrations/nginx-plus/nginx-plus-otel/). - Pour monitorer \*\*NGINX sur Kubernetes\*\*, consultez \[Monitorer NGINX sur Kubernetes avec OpenTelemetry]\(/docs/opentelemetry/integrations/nginx/nginx-otel-kubernetes/). &lt;/Callout&gt; &lt;Callout variant=&quot;tip&quot;&gt; \*\*Temps de configuration\*\* : environ 20 minutes | \*\*Niveau de compétence\*\* : Intermédiaire (nécessite des connaissances de base sur Linux/NGINX) &lt;/Callout&gt; \*/
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-data" title="Vérifier la collecte de données">
    ...
    5. Attendez que les données initiales apparaissent dans New Relic, puis [accédez à votre dashboard NGINX](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data) pour vérifier la collecte des données.
  </Collapser>
</CollapserGroup>
