...
Monitorez vos serveurs NGINX auto-hébergés avec OpenTelemetry pour envoyer des métriques de performance et des données télémétriques à New Relic.
...
* **Prometheus Receiver :** pour les environnements exécutant déjà [nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter)
...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="prometheus">
      Utilisez cette approche si vous avez déjà [nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter) en cours d&apos;exécution dans votre environnement, ou si vous migrez depuis un stack de monitoring basé sur Prometheus.

      <Callout variant="tip">
        **Recommandé :** si vous n&apos;avez pas déjà un exportateur Prometheus en cours d&apos;exécution, utilisez plutôt les onglets **collecteur NRDOT** ou **OpenTelemetry Collector Contrib**. Ils se connectent directement au point de terminaison de statut stub NGINX sans nécessiter de composant exportateur supplémentaire.
      </Callout>

      <CollapserGroup>
        <Collapser id="configure-prometheus-host" title="Étape 1 : configurer le collecteur">
          ...
          * **Collecteur OTel Contrib :** `/etc/otelcol-contrib/config.yaml`
          ...
          <Callout variant="tip">
            **Sauvegardez votre configuration existante avant d&apos;effectuer des modifications :**
          ...
          <Callout variant="important">
            Remplacez `<YOUR_STUB_STATUS_ENDPOINT>` par votre URL de statut stub NGINX (par exemple `http://127.0.0.1:8080/nginx_status`) et `<DEPLOYMENT_NAME>` par un nom unique pour ce serveur NGINX.
          </Callout>
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  Votre URL de statut stub NGINX (par exemple 

                  `http://127.0.0.1:8080/nginx_status`

                  ). Utilisé comme attribut de ressource 
              ...
              <tr>
                ...
                <td>
                  Nom unique pour ce serveur NGINX (par exemple 

                  `production-web-01`

                  ). Apparaît dans les dashboards New Relic.
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  Adresse du nginx-prometheus-exporter (par défaut 

                  `localhost:9113`

                  ). Mettez à jour si vous l&apos;exécutez sur un hôte ou un port différent.
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  Fréquence de collecte des métriques à partir de l&apos;exportateur. Par défaut : 
        ...
        <Collapser id="env-vars-prometheus-host" title="Étape 2 : définir les variables d'environnement">
          ...
          Remplacez `YOUR_NEW_RELIC_LICENSE_KEY` par votre <InlinePopover type="licenseKey" />.

          <Callout variant="tip">
            Pour les comptes de l&apos;UE, utilisez `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.eu01.nr-data.net:4318`. Consultez [Configurer le point de terminaison, le port et le protocole](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) pour la liste complète des points de terminaison régionaux.
          </Callout>
        </Collapser>

        <Collapser id="start-prometheus-host" title="Étape 3 : démarrer le collecteur">
          Appliquez la configuration et redémarrez le collecteur. Remplacez `<collector-service>` par `nrdot-collector` ou `otelcol-contrib`:
          ...
          Sortie attendue : `Active: active (running)` sans erreurs.
        </Collapser>

        <Collapser id="verify-prometheus-host" title="Étape 4 : vérifier les données dans New Relic">
          Après quelques minutes, exécutez cette requête NRQL pour confirmer que les données circulent. Remplacez `<DEPLOYMENT_NAME>` par la valeur que vous avez définie dans la configuration :
