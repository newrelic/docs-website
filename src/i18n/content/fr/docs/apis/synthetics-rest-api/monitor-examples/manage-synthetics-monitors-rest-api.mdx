...
Pour les comptes basés aux États-Unis (par défaut), utilisez le point de terminaison suivant :
...
Pour les comptes du [data center JP](/docs/accounts/accounts-billing/account-setup/choose-your-data-center), utilisez le point de terminaison suivant :
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="create-monitor" title="Créer un moniteur">
    Veuillez utiliser les [API NerdGraph](/docs/apis/nerdgraph/examples/synthetics-api/overview/) pour gérer vos moniteurs synthétiques à l&apos;aide de nos derniers runtimes.
