...
Pour utiliser l&apos;[REST Synthétique monitoring API ](/docs/apis/synthetics-rest-api), vous devez avoir la possibilité de gérer le moniteur Synthetics et d&amp;apos;utiliser une [clé utilisateur](/docs/apis/rest-api-v2/getting-started/introduction-new-relic-rest-api-v2#api_key) (la API clé REST ne fonctionnera pas).
...
<Callout variant="caution">
  L&apos;monitoring API REST Synthétique limite le taux de d&apos;un compte requests à trois requests par seconde. Les demandes effectuées au-delà de ce seuil renverront un code de réponse `429` .
</Callout>
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="patch-monitor" title="Corriger un moniteur existant">
    ...
    requests PATCH sont destinées à mettre à jour les attributs individuels de votre moniteur Synthétique plutôt qu&apos;à mettre à jour l&apos;entité entière, vous ne pouvez donc fournir que l&apos;attribut que vous souhaitez mettre à jour.
