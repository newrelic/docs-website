---
title: "Tutoriel NerdGraph\_: créer, répertorier et révoquer les URL de partage public des graphiques"
tags:
  - APIs
  - NerdGraph
  - Examples
metaDescription: 'How to use New Relic NerdGraph APIs to create, list, and revoke live chart URLs'
freshnessValidatedDate: never
translationType: machine
---

Les graphiques sont des représentations visuelles de données qui vous aident à comprendre et à analyser les informations de vos applications, de votre infrastructure ou de vos services. Vous pouvez convertir ces graphiques en URL et les partager publiquement. Les liens vers des graphiques publics vous permettent de partager facilement des graphiques et des données avec d&apos;autres personnes, même s&apos;ils n&apos;ont pas de compte New Relic.

Avec la [gestion des accès configurée](/docs/query-your-data/explore-query-data/dashboards/share-charts-publicly/#access-management) de manière appropriée, vous pouvez créer, gérer et révoquer des URL de graphiques en direct accessibles publiquement à l’aide de requêtes et de mutations dans [NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/). Sinon, vous pouvez effectuer ces actions via l’ [UI de New Relic](/docs/query-your-data/explore-query-data/dashboards/share-charts-publicly).

<Callout variant="caution">
  Toute personne ayant accès aux URL des graphiques en direct peut afficher toutes les informations de la requête de graphique sous-jacente. Assurez-vous que les informations sont partagées avec prudence et conformément aux politiques et procédures internes de votre entreprise.
</Callout>

Pour créer, répertorier ou révoquer une URL de graphique en direct accessible au public à partir de NerdGraph : accédez à [GraphQL Explorer](https://one.newrelic.com/nerdgraph-graphiql?state=2f361eaf-e5b7-41a4-5eec-e6910bbc7c47), puis suivez ces instructions.

## Générer une URL de graphique en direct

Pour créer une URL de graphique en direct, vous devez d&apos;abord connaître votre `account ID`. Utilisez la mutation suivante pour récupérer votre `account ID`, puis vous pourrez créer l&apos;URL de votre graphique en direct.

<CollapserGroup>
  <Collapser id="fetch-account-id" title="Récupérez votre identifiant de compte">
    Pour créer des URL de graphiques de partage publics, vous avez besoin d&apos;un identifiant de compte. Cette API vous permet de récupérer votre identifiant de compte à l&apos;aide de la requête d&apos;acteur.

    #### Demande d&apos;échantillon

    ```graphql
    {
      actor {
        accounts {
          id
        }
      }
    }
    ```

    #### Exemple de réponse

    ```json
    {
      "data": {
        "actor": {
          "accounts": [
            {
              "id": xxxxxxxx
            },
            {
              "id": xxxxxxxx
            }
          ]
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="create-livechart-url" title="Créer une URL de graphique en direct">
    Cette API vous permet d&apos;exécuter une requête NRQL sur un compte spécifié et de récupérer une URL de graphique intégrée en fonction des résultats de la requête.

    #### Paramètres d&apos;entrée

    <table>
      <thead>
        <tr>
          <th>
            paramètres
          </th>

          <th>
            Type de données
          </th>

          <th>
            Est-ce obligatoire ?
          </th>

          <th>
            Description
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `id`
          </td>

          <td>
            Entier
          </td>

          <td>
            Oui
          </td>

          <td>
            Votre identifiant de compte fait référence à l&apos;identifiant unique associé au compte que vous souhaitez interroger.
          </td>
        </tr>

        <tr>
          <td>
            `query`
          </td>

          <td>
            Chaîne
          </td>

          <td>
            Oui
          </td>

          <td>
            La requête NRQL à exécuter.
          </td>
        </tr>
      </tbody>
    </table>

    #### Demande d&apos;échantillon

    ```graphql
    {
      actor {
        account(id: YOUR_ACCOUNT_ID) {
          nrql(query: "$query") {
            embeddedChartUrl
          }
        }
      }
    }

    # Parameters:
    #  {
    #    id: xxxxxxxx
    #    query: SELECT count(*) FROM Transaction TIMESERIES
    #  }
    ```

    #### Exemple de réponse

    ```json
    {
      "data": {
        "actor": {
          "account": {
            "nrql": {
              "embeddedChartUrl": "https://chart-embed.xxx-xxx.newrelic.com/charts/e187axxx-xxxx-449e-xxxx-4fb7a520xxxx"
            }
          }
        }
      }
    }
    ```
  </Collapser>
</CollapserGroup>

## Révoquer l&apos;URL d&apos;un graphique en direct

Pour révoquer une URL de graphique en direct spécifique, vous devez d&apos;abord répertorier toutes les URL de graphiques en direct. Utilisez la requête suivante pour afficher la liste des URL des graphiques en direct. Ensuite, vous pouvez sélectionner l&apos;UUID du graphique spécifique pour révoquer son URL.

<CollapserGroup>
  <Collapser id="list-livechart-urls" title="Listez toutes les URL des graphiques en direct">
    Cette API récupère les URL en direct pour le widget dashboard, en fournissant le titre, l&apos;URL, la date de création et le type de chacun.

    #### Demande d&apos;échantillon

    ```graphql
    {
     actor {
       dashboard {
         liveUrls {
           liveUrls {
             title
             url
             createdAt
             type
           }
           errors {
             description
           }
         }
       }
     }
    }
    ```

    #### Exemple de réponse

    ```json
    {
      "data": {
        "actor": {
          "dashboard": {
            "liveUrls": {
              "errors": null,
              "liveUrls": [
                {
                  "createdAt": 1753xxxx346xx,
                  "title": "",
                  "type": "WIDGET",
                  "url": "ttps://chart-embed.xxx-xxx.newrelic.com/herald/9ac583f4-b43e-4750-841b-9f1aa39cde00"
                },
                {
                  "createdAt": 1753xxxx572xx,
                  "title": "",
                  "type": "WIDGET",
                  "url": "https://chart-embed.xxx-xxx.newrelic.com/herald/5d81451a-4dfb-42de-9682-dae4d7fb7962"
                },
                {
                  "createdAt": 17289xxxx694xx,
                  "title": "",
                  "type": "WIDGET",
                  "url": "ttps://chart-embed.xxx-xxx.newrelic.com/herald/c1eac5ac-4a93-42d4-8b25-36078ecc8d79"
                }
              ]
            }
          }
        }
      }
    }
    ```
  </Collapser>

  <Collapser id="revoke-livechart-url" title="Révoquer l'URL d'un graphique en direct">
    Cette API vous permet de révoquer l&apos;URL en direct associée à un widget dashboard spécifique.

    #### Paramètres d&apos;entrée

    <table>
      <thead>
        <tr>
          <th>
            paramètres
          </th>

          <th>
            Type de données
          </th>

          <th>
            Est-ce obligatoire ?
          </th>

          <th>
            Description
          </th>
        </tr>
      </thead>

      <tbody>
        <tr>
          <td>
            `uuid`
          </td>

          <td>
            Chaîne
          </td>

          <td>
            Oui
          </td>

          <td>
            L&apos;identifiant unique de l&apos;URL publique en direct.
          </td>
        </tr>
      </tbody>
    </table>

    #### Demande d&apos;échantillon

    ```graphql
    mutation {
      dashboardWidgetRevokeLiveUrl(uuid: "YOUR_LIVE_CHART_UUID") {
        uuid
        errors {
          description
          type
        }
      }
    }

    # Parameters:
    #  {
    #    uuid: xxx1afc8-6d1f-xxxx-9a33-373f64212xxx
    #  }
    ```

    #### Exemple de réponse

    ```json
    {
      "data": {
        "dashboardWidgetRevokeLiveUrl": {
          "errors": null,
          "uuid": "xxx1afc8-6d1f-xxxx-9a33-373f64212xxx"
        }
      }
    }
    ```
  </Collapser>
</CollapserGroup>

## Sujets connexes

<DocTiles>
  <DocTile title="Introduction au partage public" path="/docs/query-your-data/explore-query-data/dashboards/get-started-sharing-dashboards-publicly">
    Introduction au partage public des dashboards et des graphiques, avec les détails sur la disponibilité et la tarification.
  </DocTile>

  <DocTile title="Partager des graphiques publiquement" path="/docs/query-your-data/explore-query-data/dashboards/share-charts-publicly">
    Partagez des graphiques individuels avec des utilisateurs externes via des URL en direct.
  </DocTile>
</DocTiles>