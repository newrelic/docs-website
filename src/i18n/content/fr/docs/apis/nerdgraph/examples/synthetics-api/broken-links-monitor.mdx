...
Vous pouvez créer un moniteur de liens brisés en utilisant la mutation <DNT>`syntheticsCreateBrokenLinksMonitor`</DNT>. Cette mutation vous permet de configurer monitoring des liens brisés sur n&apos;importe quelle page web.
...
Vous pouvez mettre à jour un moniteur de liens brisés existant en utilisant la mutation <DNT>`syntheticsUpdateBrokenLinksMonitor`</DNT>. Cela vous permet de modifier la configuration d&apos;un moniteur de liens brisés déjà créé.
...
Lorsqu&apos;un moniteur de liens brisés n&apos;est plus nécessaire, vous pouvez le supprimer définitivement à l&apos;aide de la mutation <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Requête pour les moniteurs de liens brisés [#query-broken-links-monitors]

Utilisez NerdGraph pour récupérer des métadonnées, mapper des ID ou vérifier le statut de vos moniteurs de liens brisés.
