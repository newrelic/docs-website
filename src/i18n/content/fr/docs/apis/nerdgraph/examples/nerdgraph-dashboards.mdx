...
### Mettre à jour les widgets sur une page [#update-widgets-in-page]
...
#### Liens et descriptions des widgets [#widget-links-descriptions]

Vous pouvez également utiliser `dashboardUpdateWidgetsInPage` pour ajouter des liens et des descriptions aux widgets. Les liens de widget apparaissent sous forme de liens cliquables sur les titres de widget, et les descriptions de widget s&apos;affichent sous forme d&apos;infobulles lorsque les utilisateurs survolent le widget.

<Callout variant="important">
  Les URL de lien de widget doivent utiliser les protocoles `http://` ou `https://` et ne peuvent pas dépasser 2 048 caractères.
</Callout>

<CollapserGroup>
  <Collapser id="update-widget-link-description" title="Mettre à jour le lien et la description du widget">
    ```graphql
    mutation UpdateWidgetsInPage($pageGuid: EntityGuid!, $widgets: [DashboardUpdateWidgetInput!]!) {
      dashboardUpdateWidgetsInPage(guid: $pageGuid, widgets: $widgets) {
        errors {
  ...
  <Collapser id="remove-widget-link-description" title="Supprimer le lien et la description du widget">
    Pour supprimer un lien ou une description, définissez le champ sur `null`:
...
### Visibilité des variables de template [#variable-visibility]

Vous pouvez masquer les variables de template de la barre des variables en mode Vue alors qu&apos;elles continuent de fonctionner normalement. Utilisez le champ `hiddenOnVariablesBar` dans l&apos;objet `options` de la variable.

* **Nom du champ :** `hiddenOnVariablesBar`
* **Type :** Booléen
* **Par défaut :** `false` (les variables sont visibles)

<CollapserGroup>
  <Collapser id="create-hidden-variable" title="Créer un dashboard avec une variable masquée">
    ```graphql
    mutation {
      dashboardCreate(
  ...
  <Collapser id="update-variable-visibility" title="Mettre à jour une variable pour la masquer ou l'afficher">
    Pour mettre à jour la visibilité d&apos;une variable, incluez la configuration complète du dashboard avec la valeur `hiddenOnVariablesBar` mise à jour :
...
Pour plus d&apos;informations sur la visibilité des variables de template, consultez la section [Afficher et masquer des variables](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility).
...
Avec NerdGraph, vous pouvez [créer des requêtes de données à partir d’un maximum de 30 comptes New Relic](/docs/apis/nerdgraph/examples/nerdgraph-nrql-tutorial#cross-account-query). Vous pouvez également créer un dashboard utilisant les données de ces comptes en ajoutant les ID de compte à l’éventail `accountIds`.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Nombre maximum de comptes dans une requête inter-comptes
      </td>
