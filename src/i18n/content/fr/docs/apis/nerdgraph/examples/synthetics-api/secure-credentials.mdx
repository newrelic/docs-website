...
Vous pouvez créer une authentification sécurisée en utilisant la mutation <DNT>`syntheticsCreateSecureCredential`</DNT>. Cette mutation vous permet de stocker en toute sécurité des informations sensibles auxquelles votre moniteur Synthétique peut accéder pendant l&apos;exécution script.
...
Vous pouvez mettre à jour une authentification sécurisée existante en utilisant la mutation <DNT>`syntheticsUpdateSecureCredential`</DNT>. Cela vous permet de modifier la valeur et la description tout en conservant le même nom de clé.
...
Vous pouvez supprimer une information d&apos;identification sécurisée en utilisant la mutation <DNT>`syntheticsDeleteSecureCredential`</DNT>. Une fois supprimée, toute fonction de monitoring faisant référence à ces informations d&apos;identification échouera jusqu&apos;à leur mise à jour.
...
Utilisez NerdGraph pour récupérer les métadonnées des informations d&apos;identification.

Pour plus d&apos;informations, consultez :

* [Requête d&apos;informations d&apos;identification sécurisées](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-secure-credentials) - Obtenir les métadonnées des informations d&apos;identification sécurisées (noms, GUID, dernière mise à jour).
