...
Vous pouvez créer un moniteur de pas en utilisant la mutation <DNT>`syntheticsCreateStepMonitor`</DNT>. Cette mutation vous permet de configurer monitoring en plusieurs étapes des navigateurs avec une série d&apos;actions prédéfinies.
...
Vous pouvez mettre à jour un moniteur de pas existant en utilisant la mutation <DNT>`syntheticsUpdateStepMonitor`</DNT>. Cela vous permet de modifier la configuration d&apos;un moniteur de pas déjà créé.
...
Lorsqu&apos;un moniteur de pas n&apos;est plus nécessaire, vous pouvez le supprimer définitivement à l&apos;aide de la mutation <DNT>`syntheticsDeleteMonitor`</DNT>.
...
## Moniteurs d&apos;étape de requête [#query-step-monitors]

Utilisez NerdGraph pour récupérer les métadonnées, la configuration des étapes, les ID de carte, ou vérifier le statut de vos étapes à monitorer.
...
* [Requête des étapes de monitorer](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-monitor-steps) - Récupérer la configuration des étapes à partir des monitorer d&apos;étapes.
