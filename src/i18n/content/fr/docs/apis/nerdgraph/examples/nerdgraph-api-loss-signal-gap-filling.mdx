...
<Callout variant="tip">
  ...
  Pour en savoir plus sur la perte de signal, le remplissage des lacunes, et comment demander l’accès à ces fonctionnalités, consultez [ce message du forum d’assistance](https://support.newrelic.com/s/hubtopic/aAX8W0000008b5p/announcing-new-relic-one-streaming-alerts-for-nrql-conditions).
</Callout>
...
* `expiration.ignoreOnExpectedTermination`: Si `true`, aucun incident ne sera ouvert lorsque la perte du signal est attendue. Pour indiquer qu&apos;un signal est censé être perdu, le tag `termination: expected` doit être présent sur l&apos;entité. La valeur par défaut est `false`. Pour les entités hôtes d&apos;infrastructure, le tag `hostStatus: shutdown` indiquera également que le signal est censé s&apos;arrêter et empêchera l&apos;ouverture d&apos;un événement d&apos;alerte.
