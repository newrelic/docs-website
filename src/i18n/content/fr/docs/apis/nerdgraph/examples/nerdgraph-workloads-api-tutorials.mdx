...
New Relic vous permet de regrouper des entités dans des regroupements appelés [charges de travail](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)). Cela permet une meilleure supervision de la pile complète utilisée par une équipe ou un projet.
...
Voir aussi [notre article sur la façon de personnaliser les graphiques que vous voyez dans votre workload](https://support.newrelic.com/s/hubtopic/aAX8W0000008c8P/how-to-customize-the-charts-you-see-on-a-workload).
...
* `account`: L&apos;[ID de compte de la charge de travail](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#workload-account). Les charges de travail ne peuvent pas être déplacées entre les comptes, il n&apos;est donc pas possible de modifier cette valeur ultérieurement.

* `name`: Une chaîne avec un nom convivial pour la workload.

* `scopeAccounts`: Les [comptes du périmètre](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#scope-accounts) sont les comptes à partir desquels les données des entités sont récupérées. Les comptes du périmètre doivent appartenir à un groupe sous le même compte parent ou partenariat d&apos;entreprise que le compte de charge de travail.

* Pour définir l&apos;entité dans la workload, vous pouvez utiliser l&apos;une ou les deux de ces options :

  * `entitySearchQueries`: Cela vous permet de [générer dynamiquement un tableau d&apos;entités](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\)#dynamic). Il n&apos;est pas nécessaire de nommer chaque requête. Voici un exemple de requête dynamique :
