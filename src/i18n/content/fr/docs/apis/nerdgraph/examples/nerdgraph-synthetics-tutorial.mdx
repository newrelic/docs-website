...
requête faire requests pour récupérer des données. Pour découvrir les fonctionnalités de requête supplémentaires disponibles pour votre entité Synthétique, consultez [le didacticiel API de l&apos;entité NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/).
...
<CollapserGroup>
  ...
  <Collapser id="private-examples" title="demande site privé">
  ...
  <Collapser id="downtime-query" title="requête monitorer temps d'arrêt">
    requête existante moniteur temps d&apos;arrêt. les détails de configuration sont stockés dans la balise.
  ...
  <Collapser id="credentials-example" title="requête d'informations d'identification sécurisées">
  ...
  <Collapser id="query-script-example" title="scriptde monitoring des requêtes">
  ...
  <Collapser id="query-runtime-upgrade-status" title="requête pour récupérer l'état de tous les tests de mise à niveau d'exécution">
  ...
  <Collapser id="query-runtime-upgrade-status-monitor" title="requête pour récupérer l'état d'un test de mise à niveau d'exécution par ID de moniteur">
