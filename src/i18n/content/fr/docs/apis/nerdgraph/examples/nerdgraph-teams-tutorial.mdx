...
Pour commencer facilement, utilisez notre [explorateur d’API NerdGraph](/docs/apis/nerdgraph/get-started/nerdgraph-explorer) pour créer et modifier les informations des équipes. Des définitions sont disponibles dans NerdGraph pour chacun des champs des exemples ci-dessous. Si vous avez besoin d’aide pour utiliser l’IDE GraphiQL, consultez notre [tutoriel sur l’explorateur d’API NerdGraph](/docs/apis/nerdgraph/get-started/nerdgraph-explorer/). Si vous avez besoin de configurer des autorisations personnalisées pour gérer Teams, consultez [Créer des rôles personnalisés pour Teams](/docs/apis/nerdgraph/examples/nerdgraph-teams-custom-role-tutorial).
...
* `parentId` Fait référence à l&apos;ID de l&apos;équipe parente dans l&apos;arborescence hiérarchique.
...
* `hierarchyLevelId`: L&apos;ID du `TeamsHierarchyLevelEntity` qui représente la position de cette équipe dans la hiérarchie, configurée via les paramètres de l&apos;organisation (voir [paramètres de l&apos;organisation](#update-team-settings)).
* `managers`: Une liste de GUID d&apos;entités utilisateur qui sont des responsables ou le point de contact principal de cette équipe. Les managers doivent être membres de l&apos;équipe. Pour trouver les GUID utilisateur, voir [Récupérer l&apos;ID utilisateur](#user-id).
* `onCallMembers`: Une liste de `OnCallMember` entités représentant des contacts en dehors des heures normales de travail. Chaque entité nécessite un champ ID contenant le GUID de l&apos;entité de l&apos;utilisateur. Les membres d&apos;astreinte doivent être membres de l&apos;équipe. Pour trouver les GUID utilisateur, voir [Récupérer l&apos;ID utilisateur](#user-id).
* `parentId`: L&apos;ID de l&apos;équipe parente dans la hiérarchie.
...
mutation {
  entityManagementUpdateTeam(
    ...
    teamEntity: {
      ...
      onCallMembers: [{id: "INSERT_USER_GUID_3"}, {id: "INSERT_USER_GUID_4"}]
    }) {
    entity {
      id
    }
...
* `managers` accepte une liste de GUID d&apos;entités utilisateur. Pour trouver les GUID utilisateur, voir [Récupérer l&apos;ID utilisateur](#user-id).
* `onCallMembers` accepte une liste de `OnCallMember` entités. Chaque entité nécessite un champ ID contenant le GUID de l&apos;entité de l&apos;utilisateur. Pour trouver les GUID utilisateur, voir [Récupérer l&apos;ID utilisateur](#user-id).

## Mettre à jour un niveau de hiérarchie [#update-hierarchy-level]

Utilisez la mutation `entityManagementUpdateTeamsHierarchyLevel` pour mettre à jour le nom d&apos;un niveau hiérarchique. Vous pouvez trouver les ID de niveau hiérarchique dans le champ `hierarchyLevelOrder` à partir de la [requête des paramètres de l&apos;organisation](#update-team-settings).
...
* `name` est le seul attribut modifiable pour un niveau de hiérarchie.
* Le `id` est l&apos;identifiant du `TeamsHierarchyLevelEntity` à mettre à jour. Vous trouverez ces ID dans le tableau `hierarchyLevelOrder` renvoyé par la [requête des paramètres de l&apos;organisation](#update-team-settings).
...
* `hierarchyLevelOrder`: Le tableau qui définit l&apos;ordre des niveaux hiérarchiques par ordre décroissant, le premier élément étant le niveau le plus élevé et le dernier le plus bas. Chaque élément est un `id` de `TeamsHierarchyLevelEntity`.
