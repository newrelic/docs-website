...
L&apos;objet d&apos;entrée des métriques définit les parties d&apos;une requête NRDB, divisées en sections. L&apos;une de ces sections est le `eventId`, qui sert à identifier le champ définissant le GUID dans l&apos;*événement NRDB* que vous souhaitez utiliser comme métrique clé. Par exemple :
