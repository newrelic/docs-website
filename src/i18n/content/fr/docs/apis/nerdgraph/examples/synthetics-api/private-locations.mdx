...
Vous pouvez créer un site privé en utilisant la mutation <DNT>`syntheticsCreatePrivateLocation`</DNT>. Cette mutation vous permet de configurer un nouveau site privé dans votre infrastructure monitoring où vous pouvez déployer des sous-secteurs privés ou des gestionnaires de tâches.
...
Vous pouvez mettre à jour un site privé existant en utilisant la mutation <DNT>`syntheticsUpdatePrivateLocation`</DNT>. Cela vous permet de modifier la configuration d&apos;un site privé déjà créé.
...
Vous pouvez vider la file d&apos;attente des tâches pour un site privé en utilisant la mutation <DNT>`syntheticsPurgePrivateLocationQueue`</DNT>. Ceci est utile lorsque vous devez supprimer un arriéré de tâches de monitoring Synthétique mises en file d&apos;attente qui peuvent s&apos;être accumulées en raison de problèmes de performance ou de problèmes de connectivité temporaires.
...
Lorsqu&apos;un site privé n&apos;est plus nécessaire, vous pouvez le supprimer définitivement en utilisant la mutation <DNT>`syntheticsDeletePrivateLocation`</DNT>.
...
Utilisez NerdGraph pour récupérer les métadonnées de localisation et les détails de configuration.

Pour plus d&apos;informations, consultez :

* [Requête de sites privés](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-private-locations) - Obtenir tous les sites privés avec leurs détails de configuration.
