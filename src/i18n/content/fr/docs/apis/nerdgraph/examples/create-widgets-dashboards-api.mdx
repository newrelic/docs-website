...
* `visualization`: Le type de visualisation du widget, sous forme de chaîne de caractères. Par exemple : `viz.line`, `viz.area`. Voir les [exemples ci-dessous](#typed-widget-data).
...
* `configuration`: La [configuration typée](#widget-typed) du widget. Il s&apos;agit d&apos;une représentation GraphQL au typage pratique des widgets les plus courants : [aire](#area), [barre](#bar), [billboard](#billboard), [ligne](#line), [markdown](#markdown), [camembert](#pie) et [tableau](#table).
* `rawConfiguration`: La [configuration non typée](#widget-untyped) du widget. Il s&apos;agit d&apos;un format libre (scalaire JSON) pour stocker les propriétés de widget non couvertes par la configuration typée, comme [heatmap](#heatmap), [histogramme](#histogram), [bullet](#bullet), [funnel](#funnel), etc. Dans ce cas, `configuration` est nul et `rawConfiguration` contient toutes les données requises par le widget.
* `description`: Texte facultatif fournissant un contexte supplémentaire sur le widget, affiché sous forme d&apos;infobulle lorsque les utilisateurs survolent le widget.
* `link`: URL facultative qui rend le titre du widget cliquable, permettant de naviguer vers des dashboards associés, de la documentation ou des ressources externes. Doit utiliser le protocole `http://` ou `https://` et ne pas dépasser 2 048 caractères.
