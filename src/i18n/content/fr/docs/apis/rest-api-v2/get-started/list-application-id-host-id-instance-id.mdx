...
1. Accédez à l&apos;[API Explorer](https://api.newrelic.com/docs), puis sélectionnez le nom de votre compte dans la liste déroulante <DNT>**Select an account**</DNT>.
2. Accédez à la page <DNT>**Application host**</DNT> de l&apos;explorateur d&apos;API à l&apos;[adresse api.newrelic.com/docs](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json).
...
<CollapserGroup>
  <Collapser id="host_id" title="ID hôte et données récapitulatives à l'aide de le REST API Explorer">
    ...
    1. Accédez à l’ [API Explorer](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json), puis sélectionnez le nom de votre compte dans la liste déroulante <DNT>**Select an account**</DNT>.
...
La signification de l&apos;ID d’instance dépend de l&apos;agent de langage New Relic utilisé. Vous pouvez répertorier cet ID à partir de l&apos;API REST.
...
Vous pouvez récupérer les métriques récapitulatives pour l&apos;instance ainsi que les valeurs d&apos;intervalle de temps métrique spécifiques à l&apos;aide du `INSTANCE_ID`. Pour plus de détails sur les métriques disponibles, utilisez la page [d’instance d’application API REST Explorer](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__instances__instance_id__metrics_json).
