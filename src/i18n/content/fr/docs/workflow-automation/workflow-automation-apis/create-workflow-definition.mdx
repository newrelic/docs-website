---
title: Créer une définition de workflow
tags:
  - workflow automation
  - workflow
  - CreateWorkflowDefinition
  - workflow automation API
metaDescription: Create a workflow definition.
freshnessValidatedDate: never
translationType: machine
---

Utilisez cette API pour créer une définition de workflow. Elle renvoie une erreur si la définition workflow existe déjà.

## Paramètres d&apos;entrée

Les paramètres suivants sont requis ou facultatifs lors de l&apos;appel de cette mutation.

<table>
  <thead>
    <tr>
      <th style={{ width: "200px" }}>
        paramètres
      </th>

      <th>
        Type
      </th>

      <th>
        Requis
      </th>

      <th>
        Description
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        `scope`
      </td>

      <td>
        Objet
      </td>

      <td>
        Oui
      </td>

      <td>
        La portée du compte. Contient 

        `id`

         (votre ID de compte).
      </td>
    </tr>

    <tr>
      <td>
        `definition`
      </td>

      <td>
        Objet
      </td>

      <td>
        Oui
      </td>

      <td>
        La définition du workflow. Contient 

        `yaml`

         (workflow YAML encodé en base64).
      </td>
    </tr>

    <tr>
      <td>
        `tags`
      </td>

      <td>
        \[EntityTagInput]
      </td>

      <td>
        Non
      </td>

      <td>
        Tags à appliquer à la définition de workflow. Chaque tag nécessite un 

        `key`

         (String) et un 

        `values`

         ([String]).
      </td>
    </tr>
  </tbody>
</table>

## Demande d&apos;échantillon

Utilisez le [générateur de requêtes](https://one.newrelic.com/nerdgraph-graphiql?state=c8579e50-f154-fec6-22b8-f9fdcfc1990a) pour créer et éditer un workflow. Voir [l&apos;explorateur d&apos;API NerdGraph](/docs/apis/nerdgraph/get-started/nerdgraph-explorer).

```graphql
  mutation {
    workflowAutomationCreateWorkflowDefinition(
      scope: {
        id: "11933347"
      }
      definition: {
        yaml: ""
      }
      tags: [{key: "environment", values: ["production"]}]
    ) {
      definition {
        name
      }
    }
  }
```

## Sujets connexes [#related-topics]

<DocTiles>
  <DocTile title="Schéma de définition" path="/docs/workflow-automation/workflow-automation-apis/definition-schema">
    Comprendre la structure YAML du workflow avant de le créer
  </DocTile>

  <DocTile title="Valider la définition du workflow" path="/docs/workflow-automation/workflow-automation-apis/validate-workflow-definition">
    Testez la syntaxe YAML de votre workflow avant de le créer
  </DocTile>

  <DocTile title="Démarrer l'exécution du workflow" path="/docs/workflow-automation/workflow-automation-apis/start-workflow-run">
    Exécutez votre workflow nouvellement créé
  </DocTile>

  <DocTile title="Mettre à jour la définition du workflow" path="/docs/workflow-automation/workflow-automation-apis/update-workflow-definition">
    Modifier une définition de workflow existante
  </DocTile>
</DocTiles>