...
* **Type**: map de maps
* Taille max. : 100
* Un mappage des entrées de workflow que le workflow accepte.
* Exemple:
...
Les constantes de workflow permettent d&apos;accéder aux métadonnées d&apos;exécution du workflow en cours d&apos;exécution. Ces constantes sont automatiquement disponibles dans toutes les définitions de workflow et peuvent être référencées à l&apos;aide de la syntaxe d&apos;expression `${{ .workflowConstants.<constantName> }}`.
...
Les constantes de workflow peuvent être utilisées avec des expressions JQ pour transformer des valeurs :
...
Conversion de l&apos;horodatage avec l&apos;action `DateTime`
...
<Callout variant="important">
  * Les constantes de workflow sont en lecture seule et fournies automatiquement par le runtime.
  ...
  * Contrairement à `workflowInputs`, les constantes n&apos;ont pas besoin d&apos;être déclarées dans la définition du workflow.
</Callout>
...
Une étape qui termine le workflow et signale un échec avec un message personnalisé. Si un `condition` est présent, il est d’abord validé, et le workflow s’arrête uniquement lorsque la condition est `true`. Si vous ne fournissez pas de `message`, le message par défaut `failed as requested from step: failureStep` est utilisé.
...
Un exemple complet de workflow illustrant plusieurs fonctionnalités de workflow, notamment les constantes de workflow, les sélecteurs, les étapes d&apos;attente, les requêtes NRDB et les instructions switch.
