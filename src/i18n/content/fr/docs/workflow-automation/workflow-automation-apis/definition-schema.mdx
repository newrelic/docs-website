...
<CollapserGroup>
  <Collapser id="moreforloop" title="Boucle pour parcourir une liste, une carte ou une collection">
    Vous pouvez utiliser le `loop` pour itérer une liste, une carte ou une collection.

    Une boucle itère sur une collection donnée définie par `in`. Elle créera automatiquement des variables de boucle `index` et `element` pour chacune de ses itérations. Ces variables de boucle sont accessibles dans la boucle avec l&apos;expression JQ `${{ .steps.<loopStepName>.loop.element }}` ou `${{ .steps.<loopStepName>.loop.index }}`
    ...
    **Exemples**:
    ...
    Ce qui suit montre comment exécuter la boucle `workflowInputs`. Notez que le résultat doit être de type collection.
    ...
    <Callout variant="important">
      ...
      * `in`: (obligatoire). Pour que la collection d&apos;entrée soit itérée, elle doit pouvoir être convertie en tableau Java.
      * `steps`: (obligatoire). Chaque itération, les étapes seront exécutées.
      * `element` et `index` sont automatiquement attribués dans le cadre de la boucle.
      * `index` est basé sur zéro.
      * `element` peut être un type complexe si vous avez une collection d&apos;éléments complexes.
    ...
    **Sauter à l&apos;intérieur d&apos;une boucle**
    ...
    Il est permis de passer d&apos;une étape nommée à une autre appartenant à la même boucle for. Il n&apos;est pas permis de sauter dans ou hors d&apos;une boucle for, vers une boucle interne ou externe, ou entre deux boucles for différentes.
    ...
    **Utiliser break ou continue dans une boucle**
