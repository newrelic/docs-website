...
<Callout variant="important">
  Les étapes ci-dessous concernent la configuration des **workflows de notification d&apos;alerte** (qui font partie de New Relic Alerts), et non **Workflow Automation**. Les workflows de notification d&apos;alerte acheminent les problèmes vers des destinations, y compris la destination **Workflow Automation** que vous avez configurée ci-dessus. Pour une documentation complète sur les workflows d&apos;alerte, consultez [Workflows d&apos;événements d&apos;alerte](/docs/alerts/get-notified/alert-event-workflows).
</Callout>
...
2. Pour configurer votre workflow notification, sur la page Configurer votre workflow notification, fournissez les informations requises :
   ...
   * Cliquez sur **Additional settings** pour enrichir vos données en créant des [requêtes NRQL](/docs/alerts/get-notified/alert-event-workflows/#enrichments) afin de collecter des données associées provenant de l&apos;ensemble de votre plateforme New Relic. Pour des exemples d&apos;enrichissement, consultez [Enrichissement des données de workflow](/docs/alerts/get-notified/custom-variables-alert-event-workflows/#enrichment-examples).
