...
<Callout variant="tip">
  **Pour envoyer des notifications à partir des workflows** : utilisez le catalogue d&apos;actions pour ajouter des actions de notification (comme `slack.chat.postMessage`) directement dans les étapes de votre workflow. Consultez [Actions Slack](/docs/workflow-automation/setup-and-configuration/actions-catalog/slack/slack-chat) et [Actions PagerDuty](/docs/workflow-automation/setup-and-configuration/actions-catalog/pagerduty/pagerduty-incident).
</Callout>
...
4. (Facultatif) Vous pouvez également ajouter d&apos;autres destinations telles que [Slack](/docs/alerts/get-notified/notification-integrations/#slack) pour recevoir des notifications en plus de déclencher le workflow :
