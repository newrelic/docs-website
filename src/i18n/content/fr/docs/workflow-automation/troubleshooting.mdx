...
3. **Vérifier les autorisations :** Assurez-vous que votre rôle ou utilisateur IAM dispose des autorisations AWS spécifiques requises par votre workflow. Consultez le tableau [Autorisations requises](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials#required-permissions).
...
2. **Pour les clés d&apos;accès :** Vérifiez que l&apos;ID de clé d&apos;accès et la clé d&apos;accès secrète sont correctement stockés dans le [gestionnaire de secrets](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials). Vérifiez à nouveau les valeurs dans [l&apos;explorateur NerdGraph GraphiQL](https://one.newrelic.com/nerdgraph-graphiql).
...
**Commencez ici :** Utilisez cette référence rapide pour diagnostiquer les problèmes courants.
...
Pour des instructions détaillées sur la visualisation de l&apos;état et de l&apos;historique d&apos;exécution des workflows, consultez [Voir les workflows](/docs/workflow-automation/manage-workflows/managing-workflow#view-workflows).
...
2. Examiner les logs d&apos;exécution :
   ...
   * Cliquez sur **View logs** sur l&apos;exécution échouée
   * Identifier l&apos;action spécifique qui a échoué

**Causes courantes :**

* **Identifiants non valides** - Jetons expirés ou syntaxe des secrets incorrecte
* **Permissions manquantes** - Permissions IAM ou accès API insuffisants
* **Ressources supprimées** - Les instances EC2, les canaux Slack ou les bases de données n&apos;existent plus
* **Limites de débit** - Trop de requêtes vers AWS, Slack ou d&apos;autres services
* **Délais d&apos;attente** - Les API externes ne répondent pas assez rapidement

**Étape suivante :** Corrigez le problème, puis [déclenchez manuellement le workflow](/docs/workflow-automation/workflow-automation-apis/start-workflow-run) pour vérifier qu&apos;il fonctionne.
...
### Désactivation de l&apos;automatisation des workflows (FCM)

**Problème :** Vous désactivez Workflow Automation via Feature Control Management (FCM) et devez garantir un arrêt propre.

<Callout variant="important">
  **Pour les clients désactivant FCM**: Assurez-vous que tous les futurs workflows programmés soient supprimés avant de désactiver Workflow Automation. Cela évite toute confusion et des alertes inutiles une fois la fonctionnalité désactivée.
</Callout>

**Solutions :**

1. **Supprimez d&apos;abord tous les workflows planifiés**: si vous avez désactivé FCM, assurez-vous que tous les futurs workflows planifiés soient supprimés pour éviter toute confusion ou alerte :

   * Accédez à **All Capabilities &gt; Workflow Automation**
   * Passez en revue chaque workflow et vérifiez les planifications actives
   * Cliquez sur les workflows avec des planifications pour afficher leurs détails.
   * Accédez à l&apos;onglet **Scheduled runs**
   * Supprimez toutes les planifications actives à l&apos;aide de l&apos;[API DeleteSchedule](/docs/workflow-automation/workflow-automation-apis/delete-schedule)

2. **Vérifiez que toutes les planifications sont supprimées**: confirmez qu&apos;aucune exécution planifiée ne reste active avant de désactiver FCM

3. **Désactiver FCM**: une fois toutes les planifications supprimées, vous pouvez désactiver Workflow Automation en toute sécurité via les paramètres FCM

<Callout variant="important">
  **Pourquoi supprimer des plannings ?** Les workflows planifiés ne peuvent pas s&apos;exécuter lorsque Workflow Automation est désactivé via FCM. Les planifications actives restant dans le système peuvent prêter à confusion ou générer des alertes inutiles.
</Callout>
...
1. **Vérifiez les autorisations SSM**: ajoutez ces autorisations à votre stratégie de rôle IAM :
...
**Canal Slack introuvable :** Utilisez l&apos;**ID** du canal Slack (`C01234ABCD`), et non le nom du canal. Retrouvez-le dans les détails du canal Slack.

**Les actions AWS échouent silencieusement :** Assurez-vous que les identifiants AWS disposent des permissions `ec2:StopInstances`, `ec2:ModifyInstanceAttribute`, `ec2:StartInstances` et `ssm:*`. Les permissions manquantes échouent silencieusement.

**Délai d&apos;attente SSM dépassé mais instance toujours en transition :** Si le délai d&apos;attente SSM est dépassé, l&apos;instance EC2 est peut-être encore en transition entre les états. Consultez la console AWS pour vérifier l&apos;état réel de l&apos;instance avant de relancer.

**Fuites de ressources et frais imprévus :** effectuez toujours un nettoyage. Structurez les workflows de sorte que le nettoyage s&apos;exécute même si les étapes précédentes échouent. Cela permet d&apos;éviter les fuites de ressources et des frais AWS imprévus.
...
1. **Vérifier que les [flux de métriques CloudWatch](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) sont configurés**: requis pour les métriques EC2 en temps réel
2. **Vérifier que l&apos;[intégration de monitoring EC2](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-ec2-monitoring-integration#activate) est active**: garantit que les données de l&apos;instance sont transmises à New Relic
...
### workflow RCA Slack Autopilot

**Message Slack vide sans contenu RCA :** `analysisRaw` a renvoyé une réponse mais `analysisText` n’a pas pu extraire le contenu — `finalAnswer` n’était pas dans la structure `.card.body` attendue. Confirmez que le sélecteur `expression` est sur une seule ligne et que le prompt <DNT>Autopilot</DNT> renvoie une réponse sous forme de carte structurée. Consultez [RCA d’alerte avec Autopilot et Slack](/docs/workflow-automation/workflow-examples/integrations/autopilot-slack-rca) pour connaître le modèle de sélecteur canonique.

**Message &quot;<DNT>Autopilot</DNT> RCA Failed&quot; dans Slack :** `analysisRaw` était vide, <DNT>Autopilot</DNT> n’a renvoyé aucun `finalAnswer`. Vérifiez que `issueId` est un UUID de problème d’alerte valide et actif, et que le prompt <DNT>Autopilot</DNT> est bien formé.

**Échec du workflow lors de la validation des intrants :** `issueId` et `slackDestinationId` doivent tous deux être au format UUID. Utilisez `{{ issueId }}` lors du déclenchement à partir des workflow d’alerte.
