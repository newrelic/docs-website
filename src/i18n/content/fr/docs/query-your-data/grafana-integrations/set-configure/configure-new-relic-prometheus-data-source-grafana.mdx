...
1. Dans New Relic, [créez une nouvelle clé de requête Insights](/docs/apis/intro-apis/new-relic-api-keys/#insights-query-key).
   ...
   <Callout variant="important">
     Dans Grafana, vous devrez placer ceci dans un en-tête HTTP <DNT>**X-Query-Key**</DNT> personnalisé (voir l’étape 7 ci-dessous), mais il s’agit de la même entité que la clé de requête New Relic.

     Si votre compte se trouve dans la **région JP**, les clés de requête Insights ne sont pas prises en charge. Créez plutôt une [clé API utilisateur](/docs/apis/intro-apis/new-relic-api-keys/#user-key) et utilisez-la avec l&apos;en-tête <DNT>**api-key**</DNT> à l&apos;étape 7.
   </Callout>
...
6. Entrez le <DNT>**URL**</DNT> correct :
   ...
   * US (par défaut) : [prometheus-api.newrelic.com](https://prometheus-api.newrelic.com)
...
7. Sous <DNT>**Custom Headers**</DNT>, sélectionnez <DNT>**Add Header**</DNT>. Pour les comptes US et EU, définissez le nom <DNT>**Header**</DNT> sur <DNT>**X-Query-Key**</DNT> et saisissez la clé de requête Insights de l&apos;étape 1. Pour les comptes JP, définissez le nom <DNT>**Header**</DNT> sur <DNT>**api-key**</DNT> et saisissez la clé API utilisateur de l&apos;étape 1.
...
<CollapserGroup>
  <Collapser id="" title="X-clé de requête (clé de requête)">
    ...
    **Exception pour la région JP**: la région JP ne prend pas en charge les clés de requête Insights. Utilisez une [clé API utilisateur](/docs/apis/intro-apis/new-relic-api-keys/#user-key) avec le nom d&apos;en-tête <DNT>**api-key**</DNT> au lieu de <DNT>**X-Query-Key**</DNT>.
  </Collapser>
