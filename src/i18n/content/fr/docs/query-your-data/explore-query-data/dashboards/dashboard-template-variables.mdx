...
## Afficher et masquer les variables [#variable-visibility]

Vous pouvez masquer les variables de template de la barre des variables en mode Affichage, et elles continueront de fonctionner normalement. En mode Édition, les variables masquées s&apos;affichent avec une icône <Icon name="fe-eye-off" />, afin que les éditeurs de dashboards puissent les identifier et les gérer.

Nous recommandons de masquer les variables qui :

* Encombrer la barre de variables, comme les [variables imbriquées](/docs/query-your-data/explore-query-data/dashboards/nested-variables)
* Contrôler la logique du dashboard sans nécessiter l&apos;intervention du lecteur
* Dériver automatiquement des valeurs à partir d&apos;autres variables

Pour masquer une variable, utilisez l&apos;option <DNT>**Hide on variables bar**</DNT> lors de la [définition d&apos;une variable de template](#step1). Pour configurer cela par programmation, utilisez le champ `hiddenOnVariablesBar` dans l&apos;[API des dashboards NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards).

<Callout variant="tip">
  Pour masquer plusieurs variables à la fois, utilisez l&apos;[API des dashboards NerdGraph](/docs/apis/nerdgraph/examples/nerdgraph-dashboards) pour mettre à jour plusieurs variables de manière programmatique, car l&apos;interface utilisateur ne permet de masquer les variables qu&apos;une par une.
</Callout>

## Exigences et limites [#requirements]

Les variables de template ne peuvent être utilisées que dans le cadre de la création de widgets pour des dashboards personnalisés. Pour cette raison, certaines fonctionnalités liées aux requêtes ne fonctionnent pas. Par exemple, l&apos;option `Export dashboard as PDF` ne prend pas en charge les widgets avec des variables, et les requêtes SQL [<DNT>Lens</DNT>](/docs/new-relic-lens/query-data) ne prennent pas en charge les variables de modèle. Consultez [Variables NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#with-as-nrql-var) pour utiliser des variables dans une requête NRQL.
...
<Steps>
  <Step>
    ...
    Définissez une variable de template que vous utiliserez dans une requête NRQL pour créer un widget.

    Pour définir une variable :

    1. Depuis un nouveau dashboard sans variables, cliquez sur le bouton de modification <Icon name="fe-edit-2" />, puis cliquez sur le bouton <DNT>**+ Add variable**</DNT>.
    ...
    2. Terminez le workflow <DNT>**Add variable**</DNT>. Voici quelques règles et conseils pour chacun des champs.
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            La bascule **Include variable** déterminera la configuration par défaut, inclure ou exclure, pour cette variable dans le dashboard. L&apos;utilisateur visualisant le dashboard peut modifier cette configuration en utilisant la bascule **Include variable** dans le menu de la liste déroulante de la variable. La configuration sélectionnée par l&apos;utilisateur sera valable pendant toute la durée de la session. Pour plus de détails sur l&apos;endroit où trouver cette bascule et sur la façon dont elle affecte les requêtes, consultez [Inclure et exclure des variables](#include-variable).
        ...
        <tr>
          ...
          <td>
            Facultatif. Masquer la variable de la barre des variables en mode Vue. Les variables cachées continuent de fonctionner normalement. En mode Édition, les variables masquées s&apos;affichent avec une icône <Icon name="fe-eye-off" />. Pour plus d&apos;informations, consultez [Afficher et masquer les variables](#variable-visibility).
          </td>
        </tr>
      </tbody>
    </table>

    Après avoir défini votre variable de template, vous pouvez ajouter un widget qui utilise la variable de template que vous avez créée.
  </Step>
  ...
  <Step>
    ...
    4. Ajoutez votre requête et cliquez sur <DNT>**Run**</DNT>.
       ...
       En revanche, la liste suivante de clauses NRQL n&apos;accepte pas les variables de modèle comme arguments : `AS`, `COMPARE WITH`, `LIMIT`, `OFFSET`, `SINCE`, `SLIDE BY`, `TIMESERIES`, `UNTIL` et `WITH`.

    5. (Facultatif) Pour personnaliser le graphique, sélectionnez l&apos;icône <DNT>**See customization options**</DNT> à côté du menu déroulant <DNT>**Chart type**</DNT>. Pour connaître les options disponibles, consultez [Personnaliser vos graphiques](/docs/query-your-data/explore-query-data/use-charts/use-your-charts#customize-charts).

    6. Cliquez <DNT>**Add to dashboard**</DNT>
...
La bascule <DNT>**Include variable**</DNT>, activée par défaut, contrôle si le dashboard filtre selon la valeur sélectionnée pour la variable. Étant donné que ce paramètre est spécifique à cette seule variable plutôt qu&apos;à toutes les variables du dashboard, il se trouve dans la liste déroulante des valeurs propre à la variable plutôt que dans la barre des variables partagées. Vous pouvez :

* Excluez la variable de la requête en désactivant le commutateur. Cela remplace la condition de la variable par une valeur booléenne neutre (`true`).
* Rétablissez la bascule et la valeur aux paramètres par défaut configurés de la variable à l&apos;aide de <DNT>**Reset to default value**</DNT>.

Cette sélection n&apos;est valable que pour la session en cours. Pour définir l&apos;état par défaut de la bascule lors de la définition d&apos;une variable, consultez [Valeur par défaut](#step1).
...
* La variable possède plus de valeurs que le maximum autorisé (par exemple, 5000 résultats maximum pour les valeurs uniques par défaut) ou un très grand nombre de valeurs. Désactiver la variable par défaut réduit la charge de la requête.
