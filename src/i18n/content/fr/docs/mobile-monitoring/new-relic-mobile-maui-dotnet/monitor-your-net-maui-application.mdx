...
<Steps>
  ...
  <Step>
    ...
    Le jeton d&apos;application est utilisé par New Relic pour authentifier les données de votre application .NET MAUI.

    Pour afficher et copier le jeton de votre application dans l&apos;UI de New Relic :
  ...
  <Step>
    ...
    Le plug-in mobile .NET MAUI vous permet de suivre les événements de navigation dans le [Shell .NET MAUI](https://learn.microsoft.com/en-us/dotnet/maui/fundamentals/shell/navigation). Il existe deux façons d&apos;initialiser cela en fonction du moment de démarrage de votre application dans `App.xaml.cs`.

    <DNT>**Option 1: In the App Constructor**</DNT> Vous pouvez appeler la méthode de suivi directement dans le constructeur immédiatement après avoir défini le `MainPage`.
    ...
    <DNT>**Option 2: In the OnStart Method (Recommended for timing/NullReferenceException issues)**</DNT> Si le shell de votre application n&apos;est pas entièrement initialisé lors de l&apos;exécution du constructeur, l&apos;appel du tracker de navigation peut entraîner un `System.NullReferenceException`. Pour éviter cela, déplacez l&apos;appel dans la méthode de cycle de vie MAUI `OnStart()` :
...
### Données manquantes de tracing distribué dans l&apos;interface utilisateur [#missing-distributed-tracing]
...
### Plantages de l&apos;application avec TrackShellNavigatedEvents [#track-shell-crash]

Si vous rencontrez un `System.NullReferenceException` lors de l&apos;appel de `CrossNewRelic.Current.TrackShellNavigatedEvents()` dans le constructeur `App()`, c&apos;est probablement parce que le shell de l&apos;application n&apos;est pas encore complètement bootstrap. Pour résoudre ce problème, déplacez l’appel hors du constructeur vers la méthode de cycle de vie MAUI `OnStart()` dans votre fichier `App.xaml.cs` (voir la section [Événements de suivi d’écran](#screen-tracking-events) pour des exemples de code).