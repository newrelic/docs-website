...
Dans certaines circonstances, l’upload automatique des fichiers dSYM par New Relic peut échouer. Si l’upload du dSYM échoue, il crée un fichier `upload_dsym_results`. Par exemple, s’il y a une panne de réseau et que l’upload du dSYM n’est pas terminé, `upload_dsym_results` contiendra un log de ce qui s’est mal passé. Pour plus d’informations sur la façon dont New Relic gère les uploads de dSYM, consultez la [communauté technique en ligne de New Relic](https://support.newrelic.com/s/hubtopic/aAX8W0000008Yv9/relic-solution-ios-dsym-upload-deepdive).
...
<CollapserGroup>
  ...
  <Collapser id="command-line-manual-upload" title="Par ligne de commande">
    ...
    2. Téléchargez le fichier zip dSYM à l&apos;aide de la commande suivante :
       ...
       * Pour <DNT>**US accounts (default)**</DNT>:
