...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Spécifiez quels types de build de variante doivent rapporter les cartes ProGuard à New Relic. Pour les applications React Native, cela contrôle également quels types de build de variante téléversent automatiquement la carte source JavaScript. Par défaut : `Release`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Définissez sur `false` pour désactiver l’instrumentation de webview.

        * Défaut: `true`

        Pris en charge lors de l&apos;utilisation avec l&apos;agent Android v7.6.15 et versions ultérieures.
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  <Collapser id="app-level" title="Activer les téléchargements de cartes pour plusieurs variantes">
    Plusieurs types de builds peuvent être spécifiés, séparés par des virgules. Pour les applications React Native, l&apos;ajout de `Debug` ici active également le téléchargement automatique de la source map JavaScript pour les builds de débogage :
  ...
  <Collapser id="app-level" title="Désactiver l’instrumentation Webview">
    Pour désactiver l&apos;instrumentation de webview
