    ...
    * ID du plug-in hérité (`newrelic`)
    ...
    <CollapserGroup>
      ...
      <Collapser id="buildscript" title="Appliquer le plugin en utilisant le bloc buildscript{}">
        ...
        1. Ajoutez cet snippet à votre build.gradle(.kts) de niveau supérieur déposer:
        ...
        2. Ajoutez cet snippet à votre fichier `settings.gradle(.kts)` via le bloc `pluginManagement {}` :
