...
<Steps>
  ...
  <Step>
    ...
    Ou, si votre projet est plus ancien, vous pouvez utiliser l&apos; legacy ID de plugin `newrelic` en ajoutant cet snippet:
...
<CollapserGroup>
  <Collapser id="v1.2.0" title="Agent React Native version 1.2.0 et supérieure">
    Les erreurs JavaScript et les rejets de promesses peuvent être vus dans l&apos;onglet `Handled Exceptions` dans New Relic One. Vous pourrez voir le suivi des événements, l&amp;apos;attribut et trace d&amp;apos;appels pour chaque erreur JavaScript enregistrée.
