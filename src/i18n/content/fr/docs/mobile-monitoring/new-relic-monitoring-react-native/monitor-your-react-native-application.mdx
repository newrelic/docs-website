...
<Callout variant="important" title="Changement majeur dans le signalement des erreurs (1.9.0)">
  À partir de la version 1.9.0 de l’agent React Native, Les erreurs JavaScript sont signalées via un nouveau type d’événement : `MobileJSError`. Avec le nouveau protocole Mobile Errors, les erreurs JavaScript React Native ne sont plus stockées dans le type d’événement `MobileHandledException`.

  Si vous utilisez la version 1.9.0 ou supérieure, mettez à jour toutes les conditions d’alerte et les dashboards qui effectuent une requête sur `MobileHandledException` pour les erreurs de la couche JavaScript afin d’utiliser `MobileJSError` à la place. Par exemple, remplacez `SELECT count(*) FROM MobileHandledException WHERE platform = 'reactnative'` par `SELECT count(*) FROM MobileJSError`. Mettez également à jour vos scripts de build vers la dernière version afin que [les source maps se chargent correctement](/docs/mobile-monitoring/new-relic-monitoring-react-native/react-native-agent-js-error-reporting) pour le nouveau type d’événement.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    Si vous compilez avec [EAS Build](https://docs.expo.dev/build/introduction/), consultez [Configurer l’agent React Native pour EAS Build](/docs/mobile-monitoring/new-relic-monitoring-react-native/configure-eas-build-react-native) pour configurer également le téléversement automatique des source maps, des dSYM, et des fichiers de modélisation Android.
  </Step>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Enregistrer les erreurs personnalisées](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-errors/#react)
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="v1.9.0" title="Agent React Native version 1.9.0 ou supérieure">
    Les erreurs JavaScript et les rejets de promesses sont enregistrés en tant qu’événements `MobileJSError`. Vous pouvez voir le suivi des événements, les attributs, et la trace d’appels pour chaque erreur JavaScript dans New Relic.
  ...
  <Collapser id="v1.2.0" title="Agent React Native version 1.2.0 à 1.8.x">
...
<CollapserGroup>
  <Collapser id="logs-v1.4.1" title="Agent React Native version 1.4.1 et supérieure">
    À partir de la version 1.4.1 de l&apos;agent React Native, Les logs de la console JavaScript sont signalés comme des événements de log. Vous pouvez les consulter dans l&apos;[interface utilisateur des logs](/docs/logs/ui-data/use-logs-ui) ou les interroger en accédant à <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Query your data**</DNT> et en exécutant :
  ...
  <Collapser id="logs-v1.4.0" title="Agent React Native version 1.4.0 et inférieure">
