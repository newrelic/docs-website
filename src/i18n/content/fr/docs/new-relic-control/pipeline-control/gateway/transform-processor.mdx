...
* `rules`: un éventail de règles de transformation.

  * `name`: Un identifiant unique pour la règle.

  * `description`: Une explication lisible par un humain de ce que fait la transformation.

  * `conditions`: Une liste d&apos;expressions booléennes OTTL.

  * `statements`: une liste d&apos;actions OTTL (telles que `set` ou `delete_key`) à effectuer sur les données télémétriques.
