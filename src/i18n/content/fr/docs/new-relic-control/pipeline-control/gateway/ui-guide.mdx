...
8. Cliquez sur **Save draft** pour enregistrer votre version de configuration nouvellement créée.
...
2. Sur la nouvelle page de déploiement, passez en revue les informations préremplies :

   * Un nom et une description par défaut sont fournis. Vous pouvez les éditer pour faciliter le suivi de vos modifications.
   * Le déploiement inclut automatiquement les agents associés à votre flotte actuelle.
   * Le système utilise par défaut votre version de configuration. Si vous le souhaitez, vous pouvez modifier ces détails.

3. Cliquez sur <DNT>**Start deployment**</DNT> pour appliquer la configuration.

4. Vous devez confirmer à nouveau que vous souhaitez lancer le déploiement. Cliquez à nouveau sur <DNT>**Start deployment**</DNT> pour confirmer.

Le déploiement prend quelques minutes pour se terminer.
