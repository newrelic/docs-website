...
<Callout variant="tip">
  **Remarque sur le mode d&apos;installation :** le comportement des champs `version` et `autoscaling` dépend du mode d&apos;installation de votre parc :

  * **Avec Flux :** ces champs sont entièrement gérés via l&apos;interface utilisateur. Les modifications se déploient automatiquement sur tous les clusters de la flotte.
  * **Sans Flux :** ces champs peuvent être définis lors de l&apos;installation initiale pour configurer l&apos;autoscaler horizontal de pods (HPA). Cependant, les modifications post-installation via l’interface utilisateur sont ignorées. Seule la section `configuration.simplified/v2` (règles de pipeline) continue de se déployer automatiquement via `ConfigMap`. Vous devez mettre à jour manuellement la mise à l&apos;échelle via HPA et les versions via des mises à niveau Helm. Reportez-vous à [Gérer le gateway sans Flux](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway-after-installation) pour plus de détails.
</Callout>

### Structure de niveau supérieur

* `version`: Version du format de configuration (actuellement `"2.0.1"`)
* `autoscaling`: Configuration de la mise à l&apos;échelle des réplicas du gateway
* `configuration.simplified/v2`: Couche d&apos;abstraction simplifiée pour définir des pipelines de télémétrie
* `troubleshooting`: Paramètres de débogage

### Remplacements facultatifs

* `nrExporterEndpoints`: remplacer les points de terminaison d&apos;ingestion New Relic par défaut par pipeline
...
| nrExporterEndpoints.nrHost                 | chaîne  | Non    | -      |
| nrExporterEndpoints.metricsEndpoint        | chaîne  | Non    | -      |
| nrExporterEndpoints.tracesEndpoint         | chaîne  | Non    | -      |
| nrExporterEndpoints.logApiEndpoint         | chaîne  | Non    | -      |
| nrExporterEndpoints.eventApiEndpoint       | chaîne  | Non    | -      |
| nrExporterEndpoints.infraEventApiEndpoint  | chaîne  | Non    | -      |
| nrExporterEndpoints.otlpEndpoint           | chaîne  | Non    | -      |

Chaque champ `nrExporterEndpoints` est indépendant : définissez uniquement ceux que vous devez remplacer (par exemple, juste `nrHost`), et tout champ que vous omettez revient au point de terminaison d’ingestion New Relic par défaut du compte.

`nrExporterEndpoints` les champs se divisent en deux groupes de remplacement indépendants :

* `nrHost`, `metricsEndpoint`, `tracesEndpoint`, `logApiEndpoint`, `eventApiEndpoint`, et `infraEventApiEndpoint` remplacent la destination pour la télémétrie propriétaire de New Relic (agent legacy).
* `otlpEndpoint` remplace séparément la destination pour la télémétrie native OTLP. La définition des champs dans un groupe n’a aucun effet sur l’autre.

`otlpEndpoint` doit être un hôte et un schéma simples (par exemple, `https://otlp.example.com`), sans aucun port inclus. L’ajout d’un port entraîne l’échec de la validation de la configuration et empêche son enregistrement.
