...
Lors de [la création de règles de gateway](/docs/new-relic-control/pipeline-control/create-pipeline-rules.mdx), sachez que certains attributs peuvent ne pas être disponibles au niveau du gateway. Si vous devez utiliser des attributs qui ne sont ajoutés que lors de l&apos;enrichissement, envisagez d&apos;utiliser une règle cloud plutôt qu&apos;une règle de gateway.
