...
<CollapserGroup>
  ...
  <Collapser id="debug" title="Diagnostiquer les problèmes de logs de l'agent">
    ...
    * `error`
    ...
    2. Redémarrer Agent Control :

    ```bash
      sudo systemctl restart newrelic-agent-control
    ```

    3. Si le log `file` est activé, vérifiez le fichier de log le plus récent dans le répertoire indiqué par le paramètre `path` (les fichiers sont datés, par ex. `2026-09-18.newrelic-agent-control.log`). Ou utilisez votre outil de dépannage de logs préféré tel que `journalctl -u newrelic-agent-control`.
  </Collapser>
...
Consulter le fichier de log le plus récent (les fichiers de log sont nommés avec un préfixe de date, comme `2026-02-09.newrelic-agent-control.log`) :
...
Exécutez Agent Control depuis la ligne de commande pour contourner les problèmes spécifiques au service. Il lit automatiquement sa configuration à partir du chemin de configuration local par défaut (`local-data\agent-control\local_config.yaml`), aucun indicateur n’est donc nécessaire :
...
## Dépanner Agent Control sur l’hôte avec NRDiag

L&apos;outil de diagnostic New Relic `NRDiag` est un utilitaire qui rassemble les ressources et les logs liés à Agent Control sur l&apos;hôte à des fins de débogage. Cela s&apos;applique à la fois aux hôtes Linux et Windows ; la suite correspondante détecte automatiquement votre OS et exécute les vérifications pertinentes.

Suivez ces étapes pour collecter toutes les données :

1. Sur votre hôte, installez l&apos;outil `NRDiag` à l&apos;aide du [guide de démarrage](/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started).

2. Exécutez la suite Agent Control sur l’hôte :
...
3. Le résultat attendu devrait ressembler au rapport suivant :
   ...
   <Callout variant="tip" title="conseil">
     Sous Windows, la tâche `AgentControl/Agent/Logs` collecte les fichiers de log directement à partir de `C:\ProgramData\New Relic\newrelic-agent-control\log\agent-control\` au lieu de `journalctl`.
   </Callout>
