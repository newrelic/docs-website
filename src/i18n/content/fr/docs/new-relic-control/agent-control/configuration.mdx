...
1. **Définissez la section `secretsProviders` :** configurez vos fournisseurs de secrets de manière centralisée dans cette section. Assurez-vous que chaque entrée correspond à un fournisseur pris en charge.
...
<Callout variant="tip">
  Pour la région UE, utilisez `https://opamp.service.eu.newrelic.com/v1/opamp` comme point de terminaison et `https://system-identity-oauth.service.eu.newrelic.com/oauth2/token` comme `token_url`.
</Callout>
...
### Configurer les agents [#windows-agents-config]

Agent Control sous Windows peut actuellement gérer les types d&apos;agents suivants :

* Agent d&apos;infrastructure New Relic : `newrelic/com.newrelic.infrastructure`. Prend en charge les [intégrations sur l&apos;hôte](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations/) et le transfert de logs via le Fluent Bit intégré.
* Distribution New Relic pour OpenTelemetry : `newrelic/com.newrelic.opentelemetry.collector`

Pour personnaliser la configuration locale de l&apos;agent, placez un fichier `values.yml` dans le répertoire values de l&apos;agent. Les variables disponibles sont les mêmes que pour les [hôtes Linux](#agents-config).
...
<CollapserGroup>
  <Collapser id="windows-remote-config-infra" title="Agent d'infrastructure avec transfert de logs">
    Paramètres de l&apos;agent d&apos;infrastructure avec transfert de logs Windows. Le `config_agent` avec un `license_key` valide est toujours obligatoire :
  ...
  <Collapser id="windows-remote-config-agent-control" title="Configuration du contrôle des agents">
    Une configuration valide pour Agent Control lui-même sous Windows, montrant comment gérer les agents :
