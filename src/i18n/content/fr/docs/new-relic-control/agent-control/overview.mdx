...
* **Identifiants :** Agent Control peut récupérer des identifiants sensibles depuis des fournisseurs de valeurs externes tels que Vault, en les injectant dans la configuration de l’agent lors de l’exécution. Cela renforce la sécurité en conservant les identifiants hors de la configuration elle-même.
...
* **Posture de sécurité :** standardisez la récupération des identifiants auprès des fournisseurs de valeurs pour centraliser la gestion, automatiser les rotations, et réduire l’exposition.
...
Chaque agent géré par Agent Control s’appuie sur un **type d’agent**, une spécification versionnée qui décrit comment Agent Control doit télécharger, configurer, et exécuter cet agent. La prise en charge d’un nouvel agent nécessite toujours une définition de type d’agent ; sans celle-ci, Agent Control ne sait pas comment le gérer.

Les types d’agents sont identifiés par un nom qualifié et une version (par ex. `newrelic/com.newrelic.infrastructure:0.1.0`). Lorsque vous ajoutez un sous-agent à votre configuration, vous faites référence à un type d’agent ; Agent Control utilise cette définition pour récupérer l’artefact correct, appliquer les valeurs par défaut de la configuration, et gérer le cycle de vie de l’agent.

Pour obtenir la liste complète des types d’agents disponibles, consultez [Types d’agents](/docs/new-relic-control/agent-control/agent-types#agent-types).
...
Pour obtenir des informations détaillées sur les autorisations spécifiques requises par chaque agent, consultez [Autorisations requises par type d’agent](/docs/new-relic-control/agent-control/agent-types#required-permissions-per-agent-type).
