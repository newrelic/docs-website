...
2. Insérez cette clé dans le JSON suivant, puis envoyez le JSON à notre point de terminaison. Remarque : si vous avez un compte New Relic EU ou JP, utilisez plutôt le [point de terminaison régional](/docs/distributed-tracing/trace-api/trace-api-general-requirements-limits#requirements) approprié.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Tout ensemble de paires valeur-clé qui ajoutent des détails communs sur les spans de la charge. Si une span contient un attribut qui a été défini dans `common`, la clé de l&apos;objet d&apos;attribut de span aura la priorité. Il est fortement recommandé d&apos;ajouter `duration.ms`, `name` et `parent.id` .
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        L&apos;identifiant de l&apos;appelant de ce span. La valeur est `null` s&apos;il s&apos;agit de la [racine span](/docs/apm/distributed-tracing/ui-data/understand-use-distributed-tracing-data#trace-grouping). la trace sans span racine ne sera pas affichée.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Le nom de l&apos;entité qui a créé cette plage. Si aucune valeur ou une chaîne vide n&apos;est fournie, la span est attribuée à une entité « INCONNUE » et s&apos;affichera comme telle dans l&apos;UI. Cette valeur doit être fournie pour obtenir une expérience complète dans l&apos;UI.
      </td>
    </tr>
  </tbody>
</table>
...
* Certains attributs sont [obligatoires](#required-attributes) et doivent être inclus soit dans le bloc facultatif `common`, soit dans chaque span.
* Les attributs recommandés et personnalisés peuvent être éventuellement inclus dans une liste de paires valeur-clé sous une clé nommée `attributes`, dans le bloc facultatif `common` et/ou dans chaque span.

Dans l&apos;exemple suivant `POST`, il y a deux spans, toutes deux dotées du trace.id `12345` et de l&apos;attribut personnalisé `host: host123.example.com`. Le premier span n&apos;a pas `parent.id`, il s&apos;agit donc de la racine de la trace ; le `parent.id` du deuxième span pointe vers l&apos;ID du premier.
...
Pour savoir comment contrôler la manière dont les spans s&apos;affichent dans New Relic (par exemple, en ajoutant des erreurs ou en définissant une span comme span de datastore ), consultez [Décorer les spans](/docs/apm/distributed-tracing/trace-api/trace-api-decorate-spans-attributes).
...
* [Découvrez comment décorer les spans](/docs/apm/distributed-tracing/trace-api/trace-api-decorate-spans-attributes) pour une expérience d&apos; UI plus riche et plus détaillée. Par exemple, vous pouvez faire en sorte que les spans s&apos;affichent comme des spans datastore ou afficher des erreurs.
