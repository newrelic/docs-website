...
| --------------------- | ------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connection.address`  | chaîne | *requis* | Redis connection string (format: `redis://host:port/db`). For cluster mode, use comma-separated addresses (e.g., `redis://node1:6379,redis://node2:6379`) |
| `connection.password` | chaîne | `""`     | Redis password for authentication                                                                                                                         |
...
| paramètres                              | Type    | Défaut  | Description                                                                                                                                       |
| --------------------------------------- | ------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connection.replica.read_only_replicas` | booléen | `true`  | Enable routing read commands to replica nodes. Default is `true` for improved scalability. Set to `false` if strong read consistency is required. |
| `connection.replica.route_by_latency`   | booléen | `false` | Route commands to the closest node based on latency (automatically enables read\_only\_replicas)                                                  |
| `connection.replica.route_randomly`     | booléen | `false` | Route commands to a random node (automatically enables read\_only\_replicas)                                                                      |
...
| paramètres         | Type   | Défaut | Description                                                                    |
| ------------------ | ------ | ------ | ------------------------------------------------------------------------------ |
| `data_compression` | chaîne | `none` | Compression algorithm for trace data. Options: `none`, `snappy`, `zstd`, `lz4` |
...
| `processor_name`          | chaîne   | `""`   | Optional processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
| `rate_limiter`         | booléen  | `false` | Enable blocking rate limiter for concurrent trace processing |
...
| `hashing_strategy`           | chaîne   | `rendezvous` | Hashing algorithm for partition selection. Options: `rendezvous` (recommended, 3x faster) or `consistent`   |
...
| `policies`   | éventail | *requis* | Sampling policy definitions          |
...
| Scénario                                  | Partitions | Partition Workers | Reasoning                                |
| ----------------------------------------- | ---------- | ----------------- | ---------------------------------------- |
| **Développement**                         | 2-4        | 2-4               | Minimal overhead, easy debugging         |
...
| Ressource             | Without Compression        | With lz4 Compression (25% reduction) |
| --------------------- | -------------------------- | ------------------------------------ |
...
| Nom métrique                                                         | Dimensions                                     | Description                                           | Use Case                                           |
| -------------------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------- | -------------------------------------------------- |
