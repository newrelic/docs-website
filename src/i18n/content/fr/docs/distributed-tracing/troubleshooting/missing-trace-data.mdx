...
### Problèmes avec les spans [#spans]

<CollapserGroup>
  <Collapser id="advanced-infinite" title="Infinite Tracing : spans manquantes">
  ...
  <Collapser id="export" title="Span manquante non exportée">
    Parfois, la propagation d&apos;en-tête est réussie, mais les informations de la span ne sont pas envoyées à New Relic. Par exemple, si OpenTelemetry n&apos;est pas instrumenté avec un exportateur New Relic, les détails du span ne parviennent jamais à New Relic.
    ...
    Le diagramme suivant montre également une propagation d&apos;en-tête réussie, mais il inclut un exportateur dans le service 2 qui envoie les détails de la span à New Relic (voir [API Trace](/docs/understand-dependencies/distributed-tracing/trace-api/introduction-trace-api)) :
  ...
  <Collapser id="sampling" title="Spans manquantes en raison du processus d'échantillonnage">
  ...
  <Collapser id="agent-limits" title="Spans manquantes en raison des limites de spans atteintes">
    Il existe [des limites quant au nombre de plages pouvant être collectées et affichées](/docs/apm/distributed-tracing/ui-data/understand-use-distributed-tracing-data#1k-limit). Si une application génère un très grand nombre de spans pour un seul appel, elle peut dépasser la limite de collecte de spans de l&apos;agent APM pour ce cycle de collecte. Cela pourrait entraîner des pertes de données et limiter considérablement le nombre de traces que l&apos;agent peut entièrement échantillonner et signaler.
  ...
  <Collapser id="late-spans" title="Travées manquantes en raison de travées envoyées en retard">
    Les intervalles doivent être envoyés dans les vingt-quatre dernières heures pour être sauvegardés. Si une plage a un horodatage plus ancien qu’un jour, elle sera supprimée. Cela se produit souvent lorsqu’il y a un décalage d’horloge (différences de synchronisation) entre les tâches système ou les tâches d’arrière-plan de longue durée.

    <Callout variant="important">
      Toutes les plages envoyées via le [protocole OpenTelemetry](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/protocol/otlp.md) (OTLP en abrégé) de plus de vingt-quatre heures ne seront pas écrites dans [NRDB](/docs/data-apis/get-started/nrdb-horsepower-under-hood/), produisant l’erreur NrIntegrationError suivante :
...
<CollapserGroup>
  ...
  <Collapser id="long-traces-short-backend" title="Trace longue avec des temps backend courts">
    ...
    Par exemple, la span racine pourrait renvoyer des microsecondes sous forme de millisecondes. Cela peut également se produire si la span racine est une application de navigateur. Lorsque vous utilisez un client externe comme un navigateur Web, vous pouvez rencontrer plus souvent un décalage d&apos;horloge (différences de synchronisation).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="spans-browser" title="Travées et transactions manquantes après l'activation d'une application de navigateur">
    Les anciennes versions de certains agents <InlinePopover type="apm" />sont incompatibles avec le tracing distribué pour les applications de navigateur. Si l&apos;application de navigateur envoie une requête AJAX à une application APM exécutant un agent incompatible, l&apos;agent APM peut ne pas enregistrer les données de transaction et de span pour cette requête.

    Si le tracing distribué est activé pour une application de navigateur et que vous ne voyez pas de données de transaction ou de span pour requests APM en aval, examinez les [données de navigateur dans les exigences de tracing distribué](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#requirements) et confirmez que toutes les applications exécutent des versions prises en charge de l&apos;agent APM.
  </Collapser>

  <Collapser id="browser-spans-missing" title="Ne pas voir les spans finales de l'utilisateur de l'application du navigateur">
    Si les traces semblent manquer de portées d’utilisateur final, assurez-vous d’avoir lu et compris les [exigences](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#requirements) de tracing distribué du navigateur et [les procédures d’activation](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#enable).

    Sur la [page UI <DNT>**AJAX**</DNT> ](/docs/browser/new-relic-browser/browser-pro-features/ajax-page-identifying-time-consuming-calls), il existe des liens vers l&apos;UI de tracing distribué, indépendamment du fait qu&apos;il existe ou non des spans finales d&apos;utilisateur présentes dans cette trace. Pour plus de détails sur les données qui génèrent des spans, voir [Exigences](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#requirements).
  </Collapser>

  <Collapser id="orphan-browser-spans" title="Les spans Browser ne sont pas connectées à d'autres spans">
    Les anciennes versions de certains agents <InlinePopover type="apm" />sont incompatibles avec le tracing distribué pour les applications de navigateur. Si les spans APM manquent systématiquement dans la trace qui inclut les applications de navigateur, veuillez vous référer aux [données du navigateur dans le tracing distribué des exigences](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#requirements) et confirmer que toutes les applications exécutent des versions prises en charge de l&apos;agent APM.

    Pour d&apos;autres causes de spans de navigateur orphelines, voir [Rapport de span Browser ](/docs/understand-dependencies/distributed-tracing/get-started/how-new-relic-distributed-tracing-works#browser-spans).
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="supporting-open-telemetry" title="Prise en charge d'OpenTelemetry">
    Les questions concernant l’intégration avec OpenTelemetry doivent être posées sur le [forum d’assistance](https://support.newrelic.com/s/).
  </Collapser>
</CollapserGroup>
