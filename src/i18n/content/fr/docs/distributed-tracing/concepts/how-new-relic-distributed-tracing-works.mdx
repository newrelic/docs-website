...
À l&apos;exception de notre fonctionnalité [Infinite Tracing](#tail-based), la plupart de nos outils de traçage utilisent une approche d&apos;échantillonnage en tête. Cela applique des filtres aux spans individuels avant que tous les spans d&apos;une trace n&apos;arrivent, ce qui signifie que les décisions d&apos;acceptation des spans sont prises au début (la « tête ») du processus de filtrage. Cette stratégie d&apos;échantillonnage peut être utile pour capturer un échantillon représentatif de l&apos;activité tout en évitant les problèmes de stockage et de performance.
...
<CollapserGroup>
  <Collapser id="trace-origin-sampling" title="Agents linguistiques : échantillonnage adaptatif">
    ### Échantillonnage modernisé et compatibilité OpenTelemetry

    Nos agents de langage <InlinePopover type="apm" />utilisent l&apos;échantillonnage adaptatif pour offrir une visibilité haute fidélité tout en maintenant les performances du système. Cette approche garantit qu&apos;un échantillon représentatif de l&apos;activité du système est capturé sur diverses architectures, y compris les agents New Relic natifs et les intégrations OpenTelemetry (OTel).

    ### Fonctionnement de l&apos;échantillonnage adaptatif

    Le premier service monitoré dans une trace distribuée — l&apos;**origine de la trace**— est responsable de la décision d&apos;échantillonnage. Cette décision se propage à tous les services en aval pour garantir une trace complète de bout en bout.

    Les agents New Relic utilisent un algorithme adaptatif &quot;basé sur une cible&quot;. L&apos;Agent analyse le débit de la minute précédente pour calculer la probabilité d&apos;échantillonnage de la minute en cours. Par exemple, si votre service traite 100 transactions par minute et que la cible d&apos;échantillonnage est de 10, l&apos;agent s&apos;ajuste dynamiquement pour sélectionner environ 1 transaction sur 10.

    Pour trouver la cible d&apos;échantillonnage par défaut et les limites de span de votre agent, consultez la documentation spécifique à l&apos;agent :
    ...
    * [Ruby](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent)
    ...
    <Callout variant="tip">
      **Traçage distribué standard uniquement**: si un agent génère plus de spans que sa limite configurée en une minute, certains spans seront rejetés, entraînant des [traces fragmentées](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces) dans l&apos;interface utilisateur. Une priorité aléatoire est attribuée aux traces lorsqu&apos;elles sont sélectionnées pour l&apos;échantillonnage ; ainsi, si plusieurs agents doivent rejeter des spans, ils peuvent tenter de conserver intactes les traces de priorité supérieure en rejetant d&apos;abord les spans des traces de priorité inférieure.

      Cela ne s&apos;applique pas à Infinite Tracing, qui envoie toutes les données de trace sans échantillonnage.
    </Callout>

    ### Haute fidélité et environnements hybrides

    Dans le cadre de notre engagement envers les standards ouverts, les dernières versions de l&apos;agent New Relic sont conçues pour une **compatibilité hybride**.

    **Prise en charge du ratio de traces**: Les agents modernes peuvent désormais s&apos;aligner sur les ratios d&apos;échantillonnage natifs d&apos;OTel, permettant une logique d&apos;échantillonnage cohérente dans les environnements mixtes. Consultez [Prise en charge de l&apos;API OpenTelemetry](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support) pour les détails de configuration.

    **Préservation des décisions en amont**: Dans les environnements en &quot;mode mixte&quot; (où les SDK OTel et les agents New Relic sont présents), nos agents privilégient la préservation des décisions de trace initiées par les SDK OTel en amont pour éviter les traces fragmentées.
  </Collapser>
