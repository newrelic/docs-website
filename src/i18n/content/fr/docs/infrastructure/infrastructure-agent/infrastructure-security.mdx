...
* Comme alternative au Système local, le service pourrait éventuellement être modifié pour utiliser un utilisateur local, un utilisateur Active Directory/Microsoft Entra ID, ou un compte de service géré de groupe/gMSA. Le script `set-service-account.ps1` inclus facilite la transition et définit les privilèges requis.
