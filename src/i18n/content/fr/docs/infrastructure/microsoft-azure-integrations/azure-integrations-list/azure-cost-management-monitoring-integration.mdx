...
<Callout title="Note">
  Les données d&apos;Azure Cost Management sont collectées uniquement via nos intégrations Azure traditionnelles (par interrogation). Cette intégration n&apos;est **pas** disponible via l&apos;[intégration Azure Monitor](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/). Les données de coût sont basées sur les événements et couvrent plusieurs ressources et services, elles ne correspondent donc pas à un seul type de ressource Azure comme l&apos;exige l&apos;intégration Azure Monitor.

  Si vous migrez des intégrations Azure traditionnelles vers l&apos;intégration Azure Monitor, gardez l&apos;intégration Azure traditionnelle activée pour continuer à collecter les données d&apos;Azure Cost Management.
</Callout>