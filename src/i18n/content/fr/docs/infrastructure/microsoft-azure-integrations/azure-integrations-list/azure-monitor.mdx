...
<Callout variant="important">
  ...
  Notez que les métriques peuvent toujours être différenciées par facettage à l&apos;aide du champ `collector.name`. Notez également que les limites de débit appliquées par Azure seront partagées entre toute intégration d’interrogation actuelle et Azure Monitor.
</Callout>