---
title: Métriques d'intégration Azure
tags:
  - Integrations
...
## Convention de dénomination métrique dimensionnelle [#metric-naming-convention]

Les métriques extraites de l&apos;API Azure Monitor sont stockées dans New Relic sous forme [de métriques dimensionnelles](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) suivant cette convention :

* Les métriques sont préfixées par leur type de ressource Azure , le tout en minuscules, où `/` est remplacé par `.` et `Microsoft.` est remplacé par `azure.`:
...
* Le nom de la métrique Azure d&apos;origine conserve sa casse d&apos;origine, débarrassée de tous les espaces, et est préfixé par le type de ressource, comme décrit ci-dessus :
...
Si le nom de la métrique résultant ne commence pas par `azure.` après ces transformations, le préfixe est ajouté en plus.

Pour plus d’informations sur les types de ressources Azure pris en charge, consultez le [site Web de documentation Azure Monitor Metrics](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/supported-metrics/metrics-index).

### Le nom de la métrique correspond à l&apos;un des noms des dimensions

Lorsque le nom de la métrique et le nom de l&apos;une des dimensions sont exactement identiques, le nom de cette dimension n&apos;est pas en majuscule afin que les deux valeurs soient différentes :

* `azure.network.applicationgateways.TlsProtocol` avec dimensions `azure.network.applicationgateways.tlsprotocol`

### Métriques avec de nombreuses combinaisons de dimensions

Une définition métrique inclut à la fois son nom et les différentes dimensions. Parfois, le même nom de métrique apparaît dans de nombreuses métriques avec différentes combinaisons de dimensions, par exemple :

* `azure.cache.redis.percentProcessorTime` avec dimensions `azure.cache.redis.ShardId`
* `azure.cache.redis.percentProcessorTime` sans dimensions.

Chacune de ces métriques est ingérée sous forme de série temporelle individuelle.

<Callout variant="tip">
  Certaines agrégations comme `sum()` ou `count()` renverront des résultats dupliqués si la combinaison de dimensions correcte n&amp;apos;est pas spécifiée.
</Callout>

Pour éviter que certaines agrégations telles que `sum()` ou `count()` correspondent aux deux métriques en même temps, utilisez `IS NULL` ou `IS NOT NULL` pour filtrer les dimensions. Par exemple:

Correspond aux deux métriques en même temps :

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC
```

Correspond uniquement à la métrique sans dimension ShardId :

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NULL
```

Correspond uniquement à la métrique avec la dimension :

```sql
  SELECT sum(azure.cache.redis.percentProcessorTime) FROM METRIC WHERE azure.cache.redis.ShardId IS NOT NULL
```

## Métriques de sondage API [#azure-metrics-table]

Pour une référence sur les métriques disponibles de chacune des intégrations de sondage et leurs noms, [consultez notre documentation sur l&apos;intégration individuelle](/docs/infrastructure/microsoft-azure-integrations/get-started/activate-azure-integrations/).

Le tableau suivant est une liste non exhaustive des métriques collectées par l’intégration Azure Polling et leurs traductions [de métriques dimensionnelles](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics) .
  ...
  <tbody>
    <tr>
      ...
      <th style={{ width: "275px" }}>
        Nom métrique dimensionnel (nouveau)
      </th>

      <th style={{ width: "275px" }}>
        Nom de l&apos;échantillon métrique (précédent)
      </th>
    </tr>
