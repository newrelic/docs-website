...
<Callout variant="tip">
  Pour exécuter une requête de style PromQL dans [New Relic One](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one), accédez au [générateur de requêtes avancé en mode PromQL](/docs/infrastructure/prometheus-integrations/view-query-data/view-query-your-prometheus-data/#build-query).
</Callout>
...
Pour apprendre à convertir une requête PromQL en une condition d&apos;alerte NRQL pouvant être utilisée avec New Relic, regardez cette courte vidéo (environ 15 secondes). 5:45 minutes).
