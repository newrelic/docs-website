...
<Callout variant="caution">
  Un compte peut avoir jusqu&apos;à 1 500 clusters HA Prometheus uniques. Si cette limite est dépassée, les données des clusters HA supplémentaires seront ignorées. Dans de tels cas, New Relic génère `PrometheusHAClusterLimit` événements [`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/).
</Callout>
