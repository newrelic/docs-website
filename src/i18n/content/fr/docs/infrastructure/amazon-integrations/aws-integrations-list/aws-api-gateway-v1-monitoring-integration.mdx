...
<Callout variant="tip">
  Les métriques API Gateway v1 (REST) sont disponibles à la fois via l&apos;[intégration des flux de métriques CloudWatch](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-metric-stream/) et l&apos;intégration d&apos;interrogation d&apos;API pour Amazon API Gateway.
</Callout>
...
Pour activer cette intégration, utilisez l&apos;une des méthodes suivantes :

* <DNT>**CloudWatch Metric Streams**</DNT> (recommandé) : suivez la procédure pour [configurer les flux de métriques CloudWatch](/docs/infrastructure/amazon-integrations/connect/aws-metric-stream-setup/).
* <DNT>**API polling**</DNT>: suivez les procédures standard pour [Connecter les services AWS à New Relic](/docs/infrastructure/infrastructure-integrations/getting-started/connect-aws-integrations-infrastructure).
...
La façon dont vous effectuez une requête sur vos données dépend de la méthode d&apos;ingestion :

* <DNT>**CloudWatch Metric Streams**</DNT>: les données sont rapportées sous forme de métriques dimensionnelles. Accédez à <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Metrics &amp; Events**</DNT> et filtrez par `aws.apigateway`.
* <DNT>**API polling**</DNT>: les données sont rapportées au [type d&apos;événement](/docs/data-apis/understand-data/new-relic-data-types/#event-data) `ApiGatewaySample`, que vous pouvez [interroger et explorer](/docs/using-new-relic/data/understand-data/query-new-relic-data).
