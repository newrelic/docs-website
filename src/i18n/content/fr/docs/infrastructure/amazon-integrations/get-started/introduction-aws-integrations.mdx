...
<figcaption>
  Ce diagramme illustre de quelles manières vous pouvez intégrer vos services AWS avec New Relics pour obtenir des logs et/ou des métriques.
</figcaption>
...
* **Automatiser avec CloudFormation**: CloudFormation est un service AWS qui vous permet de modéliser et d&apos;automatiser la configuration de votre intégration à l&apos;aide de modèles déclaratifs. Pour réduire les erreurs de configuration manuelle, utilisez cette méthode pour connecter votre compte AWS à New Relic. Vous pouvez concevoir entièrement l&apos;intégration sur l&apos;UI de New Relic et la déployer sur la console AWS.

Cette méthode provisionne les ressources qui transfèrent vos métriques vers New Relic, y compris un rôle <DNT>IAM</DNT>, un flux de métriques <DNT>Amazon CloudWatch</DNT> et un flux de diffusion <DNT>Amazon Kinesis Data Firehose</DNT>. Les flux de métriques fonctionnent par région, vous avez donc besoin d&apos;un flux de métriques et d&apos;un flux de diffusion <DNT>Kinesis Data Firehose</DNT> dans chaque région AWS que vous monitorez. Reportez-vous au schéma suivant pour voir comment ces composants se connectent.

<img width="70%;" title="AWS CloudWatch Metric Streams architecture" alt="Diagram of an AWS account showing AWS services, an IAM role, a CloudWatch metric stream, and a Kinesis Data Firehose delivery stream forwarding metrics from one AWS region to New Relic." src="/images/aws-metric-streams-architecture.webp" />

<figcaption>
  Comment une intégration de flux de métriques CloudWatch transfère les métriques vers New Relic. Vous avez besoin d&apos;un flux de métriques et d&apos;un flux de diffusion Kinesis Data Firehose dans chaque région AWS que vous monitorez.
</figcaption>
...
## Ingestion des logs AWS [#aws-logs-ingestion]

Pour connecter votre compte AWS à New Relic pour l&apos;ingestion des logs, reportez-vous à [Transférer les logs AWS des services vers New Relic](/install/aws-logs/).