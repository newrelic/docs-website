...
6. Confirmez que vous avez configuré votre Kinesis Data Firehose avec les détails de destination corrects :
   ...
   * Assurez-vous que le bon data center US (par défaut), EU, ou JP a été sélectionné pour votre compte New Relic. Astuce : si le `license_key` commence par « eu », alors vous devez sélectionner le data center UE. Si le `license_key` commence par « jp », alors vous devez sélectionner le data center JP.
