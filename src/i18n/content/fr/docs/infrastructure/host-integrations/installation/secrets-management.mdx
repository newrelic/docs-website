...
Avec la gestion des secrets, vous pouvez configurer l’agent, et les intégrations sur hôte pour utiliser des données sensibles (telles que des mots de passe) sans avoir à les écrire en texte clair dans les fichiers de configuration. Actuellement, Hashicorp Vault, AWS KMS, CyberArk, Azure Key Vault, l’obfuscation de la CLI New Relic, et les commandes personnalisées sont pris en charge.
...
Lors de l&apos;utilisation de variables d&apos;environnement dans les fichiers de configuration des intégrations on-host, le paramètre [`passthrough_environment`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#passthrough_-environment) doit être défini.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Configuration de la récupération du secret Azure Key Vault](#azure-key-vault-secrets)
      </td>
    </tr>
...
## Secrets Azure Key Vault [#azure-key-vault-secrets]

Pour récupérer des secrets à partir d&apos;Azure Key Vault, définissez les propriétés suivantes dans votre section `azure-key-vault`. Les deux clés sont requises :
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        L&apos;URL de l&apos;instance Azure Key Vault à partir de laquelle récupérer le secret

        Exemple: `https://my-vault.vault.azure.net/`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Le nom du secret à récupérer. L&apos;agent récupère toujours la dernière version du secret.
      </td>
    </tr>
  </tbody>
</table>

L’agent s’authentifie auprès d’Azure Key Vault à l’aide de la chaîne d’informations d’identification par défaut du SDK Azure, en essayant les méthodes suivantes dans l’ordre jusqu’à ce que l’une d’elles réussisse. Aucune configuration supplémentaire de l’agent n’est requise :

* Variables d’environnement `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, et `AZURE_TENANT_ID`
* Une identité managée attribuée à la machine virtuelle Azure ou au pod AKS exécutant l’agent (recommandé pour la production)
* Informations d’identification de la CLI Azure (`az login`), utiles pour le développement et les tests

<Callout variant="tip">
  Seules les valeurs secrètes sont prises en charge. La récupération des certificats ou des clés Azure Key Vault n’est pas prise en charge.
</Callout>

L’exemple suivant récupère un secret nommé `mssql-password` à partir d’un Azure Key Vault et l’utilise comme mot de passe pour une intégration sur hôte MSSQL :
