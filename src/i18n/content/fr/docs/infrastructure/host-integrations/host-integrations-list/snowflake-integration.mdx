...
<Steps>
  ...
  <Step>
    ...
    Exécutez la commande ci-dessous pour stocker les métriques Snowflake au format JSON, ce qui permet à nri-flex de les lire. Veillez à modifier `ACCOUNT`, `USERNAME` et `SNOWSQL_PWD` en conséquence.
