...
2. Consultez le document [Exigences pour l&apos;agent d&apos;infrastructure](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent/) pour vous assurer que votre système et toutes les intégrations sur l&apos;hôte que vous configurez répondent aux exigences.
3. Flex est fourni avec notre [agent d&apos;infrastructure](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/) version 1.10.7 ou supérieure fonctionnant sous Linux, Windows ou Kubernetes.
...
<Steps>
  ...
  <Step>
    ...
    L&apos;objectif est de traiter la sortie de la commande `df`, en affichant le système de fichiers et les blocs de 1 octet, tout en excluant les systèmes de fichiers déjà pris en charge par l&apos;agent. Si les systèmes de fichiers non pris en charge ne sont pas montés, supprimez les arguments `-x`.
