...
* `insights_data_center` (sous `insights`, par défaut : `us`) : si vous utilisez le data center NR EU pour votre compte, veuillez remplacer cette valeur par `eu` ou `EU`. Si vous utilisez le data center NR JP, veuillez remplacer cette valeur par `jp` ou `JP`. Sinon, vous pouvez laisser ce paramètre tel quel ou l’omettre complètement.
