...
<CollapserGroup>
  <Collapser id="entities-table" title="Entités Spanner">
    Une entité `GCPSPANNERINSTANCE` est synthétisée pour chaque instance Spanner, et une entité `GCPSPANNERDATABASE` pour chaque base de données qu’elle contient. Les deux types d’entités partagent le même type de ressource GCP monitorée `spanner_instance` sous-jacent (Spanner n’a pas de type de ressource monitorée distinct par base de données), et sont désambiguïsés par famille de métriques : les métriques de nœud, de CPU, de stockage, de session, de sauvegarde, et de région leader s’attachent à `GCPSPANNERINSTANCE`; les familles de métriques `api`, `query_stat`, `graph_query_stat`, `read_stat`, `transaction_stat`, `lock_stat`, `query_count`, et `row_deletion_policy`, qui portent un label de métrique `database` supplémentaire, s’attachent à `GCPSPANNERDATABASE`.
...
Ces métriques sont attachées à l’entité `GCPSPANNERINSTANCE`.
...
Spanner n’a pas de type de ressource GCP monitorée distinct par base de données ; ces métriques sont rapportées par rapport au même type de ressource `spanner_instance` que les métriques d’instance ci-dessus, avec une étiquette de métrique `database` supplémentaire, et s’attachent à l’entité `GCPSPANNERDATABASE`.
