...
<CollapserGroup>
  <Collapser id="service" title="Compte de service (recommandé)">
    ...
    Si votre organisation utilise une [contrainte de restriction de domaine](https://cloud.google.com/resource-manager/docs/organization-policy/restricting-domains), vous devrez mettre à jour la politique pour autoriser New Relic, en fonction de la contrainte que votre organisation applique :

    * **`iam.allowedPolicyMemberDomains`**: autoriser l&apos;ID de domaine New Relic `C02x1gp26`.
    * **`iam.managed.allowedPolicyMembers`**: autorisez l&apos;ID d&apos;organisation GCP New Relic `740422149647` ou le numéro de projet GCP `397727728820`.
