...
L&apos;exemple suivant montre une configuration typique pour le log l&apos;agent dans un système Linux (modifier le chemin du fichier lors de l&apos;exécution Windows ou d&apos;une autre plateforme) :
...
Par défaut, les journaux d’infrastructure sont formatés en texte :
