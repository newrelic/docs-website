...
Avec monitoring d&apos;infrastructure de New Relic, vous pouvez créer [des conditions d&apos;alerte](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-condition) directement dans le contexte de ce que vous monitorez actuellement avec New Relic.
...
9. Facultatif : définissez la limite de temps [<DNT>**Close open alert events after**</DNT>](#alert-event-time-limit) pour fermer automatiquement les événements d&apos;alerte ouverts après une certaine durée (par défaut, 24 heures pour les conditions d&apos;infrastructure).
...
Toute personne incluse dans les [canaux de notification](#notification) de la politique reçoit directement les notifications d&apos;alerte. De plus, toute personne disposant d&apos;autorisations pour votre compte New Relic peut consulter les événements d&apos;alerte d&apos;infrastructure et les événements d&apos;alerte individuels via l&apos;interface utilisateur.
...
3. À partir de la liste <DNT>**Events**</DNT>, sélectionnez l&apos;événement d&apos;alerte.
4. Pour [afficher des informations détaillées dans les alertes](/docs/alerts/alert-event-management/view-alert-event-details) concernant l&apos;événement d&apos;alerte sélectionné, sélectionnez le lien.
...
[Alerts](/docs/alerts/new-relic-alerts-beta/getting-started/new-relic-alerts) fournit un outil d&apos;alerte unique et coordonné pour l&apos;ensemble de vos produits New Relic. Cela vous permet de gérer des [politiques d&apos;alerte](/docs/alerts/overview#alert-policy) et des [conditions](/docs/alerts/overview#alert-condition) qui se concentrent sur les métriques des [entités](/docs/alerts/overview#alert-entity) qui comptent le plus pour vous, comme les conteneurs Docker, les JVM, et plus encore.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * Consultez les informations détaillées sur les [événements d&apos;alerte](/docs/new-relic-solutions/get-started/glossary#alert-event): utilisez l&apos;interface [Alerts](/docs/alerts/alert-event-management/view-alert-event-details) ou le [canal de notification](#notification) intégré à la politique associée.
      </td>
    </tr>
...
Le texte que vous saisissez dans le champ <DNT>**Description**</DNT> d&apos;une condition d&apos;alerte est transmis aux événements d&apos;alerte et aux notifications associés. Une description peut être utilisée à plusieurs fins, notamment :
...
Vous pouvez utiliser la substitution de modèle pour insérer des valeurs à partir des attributs de l&apos;événement d&apos;alerte associé. Le format du modèle est `{{attributeName}}`. Pour les attributs que vous pouvez utiliser lors de la création d&apos;une description, consultez les [attributs d&apos;événement d&apos;alerte](/docs/alerts/create-alert/condition-details/alert-event-attributes).
...
Si des tags d&apos;entité sont associés à votre événement d&apos;alerte, ils sont accessibles via le nom du tag d&apos;entité. Un exemple de cela serait `{{tag.aws.awsRegion}}`. Lorsque des tags d&apos;entité sont disponibles, ils sont inclus dans l&apos;événement d&apos;alerte et affichés lorsque vous consultez l&apos;événement d&apos;alerte.

Ce champ a une taille maximale de caractères de 4 000.

## Ajouter ou modifier l&apos;URL d&apos;un runbook [#runbook-url-infrastructure]

Le [processus de création de condition d&apos;alerte](#create-condition) inclut une option pour définir une URL pour les instructions de runbook. Cela vous permet de créer un lien vers des informations ou des procédures standard pour gérer un événement d&apos;alerte. Avant d&apos;ajouter ou de mettre à jour le lien, assurez-vous d&apos;utiliser une URL valide.
...
## limite de temps de l&apos;événement d&apos;alerte [#alert-event-time-limit]

La limite de temps des événements d&apos;alerte vous permet de définir une période après laquelle les événements d&apos;alerte seront clôturés de force. Par défaut, la limite de durée des événements d&apos;alerte est de 24 heures pour les conditions d&apos;infrastructure.

Pour ajouter ou mettre à jour la limite de temps d&apos;un événement d&apos;alerte d&apos;une condition d&apos;alerte :

1. Sélectionnez une condition d&apos;alerte et modifiez la limite de temps de l&apos;événement d&apos;alerte.
2. Sauvegarder la condition.
...
Pour plus d’informations, consultez [Bonnes pratiques pour le filtrage dans les alertes d’infrastructure](https://support.newrelic.com/s/hubtopic/aAX8W0000008Zfu/relic-solution-best-practice-for-filtering-in-infrastructure-alerts) dans le forum d’assistance de New Relic.