...
En appliquant des filtres aux hôtes et processus importants pour votre activité, vous pouvez définir des [seuils d&apos;alerte](/docs/alerts/overview#alert-threshold) pour décider quand des événements d&apos;alerte s&apos;ouvrent et quand New Relic vous envoie une notification par e-mail, selon les [préférences d&apos;événement d&apos;alerte](/docs/alerts/organize-alerts/specify-when-alerts-create-events) de la politique. Ces exemples illustrent comment utiliser la condition <DNT>**Process running**</DNT> de la supervision de l&apos;infrastructure pour monitorer vos processus.
...
<CollapserGroup>
  <Collapser id="processes-load" title="Assurez-vous que suffisamment de processus sont en cours d'exécution pour satisfaire la charge">
    <DNT>**Problem:**</DNT> Certains équilibreurs de charge et serveurs d&apos;application fonctionnent en exécutant de nombreux processus de travail en parallèle. Ici, par exemple, vous pouvez souhaiter un événement d&apos;alerte lorsque moins de huit processus sont en cours d&apos;exécution pour un service comme gunicorn.
  ...
  <Collapser id="one-critical" title="démarrage du moniteur pour les processus critiques qui nécessitent une attention particulière">
    ...
    <DNT>**Solution:**</DNT> Utilisez le seuil <DNT>**At least one process is running**</DNT> avec des filtres de condition définis sur un nom d&apos;utilisateur et un exécutable spécifique pour que New Relic ouvre un événement d&apos;alerte lorsque le processus est en cours d&apos;exécution.
  </Collapser>

  <Collapser id="one-job-length" title="Assurez-vous qu'un travail ne prenne pas trop de temps">
    <DNT>**Problem:**</DNT> Vous avez une tâche qui s&apos;exécute périodiquement et vous souhaitez ouvrir un événement d&apos;alerte lorsqu&apos;elle s&apos;exécute depuis plus longtemps qu&apos;un nombre de minutes prévu.
...
Si vous créez la condition d&apos;alerte directement avec le monitoring de l&apos;infrastructure, New Relic enverra une notification par e-mail lorsque le seuil défini pour la condition d&apos;alerte est franchi, en fonction des [préférences d&apos;événement d&apos;alerte](/docs/alerts/organize-alerts/specify-when-alerts-create-events) de la politique. Votre politique d&apos;alerte définit le personnel ou les équipes et les canaux de notification que nous utilisons.