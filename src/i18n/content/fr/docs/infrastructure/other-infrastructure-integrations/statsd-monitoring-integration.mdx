...
<CollapserGroup>
  <Collapser id="alert-example" title="Exemple d'alerte">
    ...
    Si une métrique avec une valeur supérieure à 50 est envoyée, alors un événement d&apos;alerte est créé et notifié. L&apos;événement d&apos;alerte est fermé automatiquement après 24 heures. Pour vérifier que l&apos;alerte fonctionne, exécutez cette commande :
...
* Assurez-vous que le bon data center, US (par défaut) ou EU, a été sélectionné pour votre compte New Relic. Astuce : si la license\_key commence par &quot;eu&quot; alors vous devez utiliser l&apos;indicateur `NR_EU_REGION=true`.
* Pour la région JP, le point de terminaison est automatiquement sélectionné en fonction de votre clé de licence, aucun indicateur supplémentaire n&apos;est donc requis.
* Assurez-vous qu’il n’y a pas de [`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/) liés à l’intégration StatsD.
