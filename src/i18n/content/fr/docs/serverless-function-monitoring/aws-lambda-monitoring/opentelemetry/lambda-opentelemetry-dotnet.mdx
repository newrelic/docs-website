...
<Steps>
  ...
  <Step>
    ...
    * `otlp.nr-data.net:4317`: si votre compte New Relic se trouve dans la région américaine (par défaut)
    * `otlp.eu01.nr-data.net:4317`:Si votre compte New Relic se trouve dans la région UE
    * `otlp.jp.nr-data.net:4317`: si votre compte New Relic est dans la région JP
...
Dans certains cas, vous pouvez voir des traces distribuées fragmentées dans New Relic avec cette configuration. Cela se produit lorsqu&apos;une trace démarre ou implique un service en dehors du contexte ADOT (par exemple, un service AWS géré). Les spans de ce service sont créés par X-Ray, et non par OpenTelemetry, et ADOT ne les transmet pas à New Relic. Bien que les traces paraissent fragmentées, elles fournissent néanmoins des informations détaillées complètes sur les performances au sein de la fonction Lambda ainsi que d&apos;autres services dont les spans ont été transférés à New Relic.
