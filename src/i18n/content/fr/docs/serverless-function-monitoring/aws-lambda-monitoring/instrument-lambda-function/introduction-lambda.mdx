...
<Callout variant="tip">
  Le monitoring New Relic Lambda prend en charge les fonctions Lambda Managed Instance (LMI). Les fonctions LMI sont automatiquement détectées et s’exécutent en mode APM : aucune configuration supplémentaire n’est requise.
</Callout>
...
## Configurer le monitoring AWS Lambda [#config]
...
<Steps>
  ...
  <Step>
    ...
    <Tabs>
      ...
      <TabsPages>
        <TabsPageItem id="1">
          Pour que New Relic puisse monitorer votre fonction Lambda, vous devez configurer une intégration entre New Relic et AWS. Nous vous recommandons d’utiliser [des flux métriques](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/) pour aider à réduire la latence et éviter la limitation de nos requests d’interrogation.
        </TabsPageItem>
