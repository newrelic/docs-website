...
1. Exécutez ce qui suit en remplaçant `YOUR_REGION` par votre région (par exemple, `us-west-2`).
   ...
   Si vous avez créé la stack CloudFormation existante avec le paramètre stack FunctionRole, spécifiez le nom du rôle de fonction avec `--parameter-overrides` comme ci-dessous.
