...
<CollapserGroup>
  <Collapser id="go" title="Go">
    ...
    7. Invoquez le Lambda au moins une fois. Cela crée un CloudWatch log groupe , qui doit être présent pour que l’étape suivante fonctionne.
       ...
       Notre wrapper collecte des données sur l&apos;exécution Lambda, génère un message JSON et l&apos;enregistre dans les logs CloudWatch. Ensuite, vous [configurerez CloudWatch pour envoyer ces logs à New Relic](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/cloud-watch-fallback).
  </Collapser>

  <Collapser id="java" title="Java">
    <Callout variant="tip">
      Cette solution n&apos;utilise pas l&apos;agent Java APM. Si vous souhaitez utiliser l&apos;agent Java APM dans AWS Lambda, veuillez vous reporter à notre [documentation sur la solution Serverless de l&apos;agent Java](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda). La solution basée sur l&apos;agent APM ne doit pas être exécutée avec la solution basée sur le SDK.
    </Callout>

    Monitoring d&apos;AWS Lambda en Java avec la solution basée sur le SDK utilise deux dépendances Open Tracing :
    ...
    * Notre traceur AWS Lambda OpenTracing : une implémentation de traceur OpenTracing conçue pour monitorer AWS Lambda. Il génère des spans, des événements d&apos;erreur, des événements de transaction, des traces d&apos;erreurs et fournit un support de tracing distribué.
    ...
    3. Dans la méthode `doHandleRequest`, appelez l&apos;API `LambdaTracing.instrument(...)` pour créer une span racine afin de trace l&apos;exécution de la fonction Lambda. C&apos;est également ici que vous définirez votre logique métier pour la fonction Lambda.
    ...
    * Création manuelle de span (alias instrumentation personnalisée)
