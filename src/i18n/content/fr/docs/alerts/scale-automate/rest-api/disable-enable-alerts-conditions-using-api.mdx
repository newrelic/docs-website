...
* Le `id` de la condition (disponible depuis l’ [API Explorer :](https://api.newrelic.com/docs/#/Alerts%20Conditions) <DNT>**Alerts Conditions &gt; GET &gt; List**</DNT>)
* Par défaut, ce point de terminaison utilise le data center américain. Si votre organisation héberge des données dans le data center de l&apos;UE, assurez-vous d&apos;utiliser les [points de terminaison d&apos;API appropriés pour les comptes de la région de l&apos;UE](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center#endpoints).
