...
Les problèmes sont des groupes d&apos;événements d&apos;alerte qui décrivent le problème sous-jacent de vos symptômes. Lorsqu&apos;un nouvel événement d&apos;alerte est créé, l&apos;intelligence des incidents ouvre un dossier et évalue les autres dossiers ouverts pour détecter des corrélations. Pour plus d&apos;informations, consultez [Utiliser Incident Intelligence](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/use-incident-intelligence/).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Le problème a été fermé en raison de la fermeture de tous ses événements d&apos;alerte, de la fin du TTL ou a été `Inactive` trop longtemps
      </td>
    ...
    <tr>
      ...
      <td>
        Un événement d&apos;alerte dans le problème est passé d&apos;une priorité Haute et est devenu un problème de priorité Critique
      </td>
    ...
    <tr>
      ...
      <td>
        Un événement d&apos;alerte a été ajouté au problème, un événement d&apos;alerte a été fermé ou un autre problème a été fusionné avec celui-ci
      </td>

      <td>
        Un événement d&apos;alerte a été résolu
      </td>
...
3. Avec le workflow, filtrez vos problèmes et envoyez-les à la destination appropriée. Vous pouvez créer une requête pour filtrer les bons problèmes.
   ...
   <Callout variant="tip">
     Pour configurer des notifications distinctes sur les événements d&apos;alerte `WARNING`, filtrez les problèmes dont la priorité est égale à `HIGH`.
   </Callout>
...
5. Recommandé : filtrer les problèmes par tag d&apos;équipe afin que toutes les équipes puissent être averties lorsque leur entité est incluse dans un problème. Les étapes peuvent être trouvées dans la démo ci-dessous :
   ...
   Pour obtenir des conseils sur l’utilisation des tags, consultez [cette publication du forum d’assistance sur les modèles de workflow](https://support.newrelic.com/s/hubtopic/aAX8W0000008dNf/relic-solution-patterns-for-implementing-alerts-workflows).

6. Optionnel : Enrichissez vos données.

   <CollapserGroup>
     <Collapser className="freq-link" id="enrichments" title="Comment enrichir les données">
       Actuellement, vous pouvez uniquement envoyer des sorties JSON ou numériques vers les [destinations](/docs/alerts-applied-intelligence/notifications/destinations/) Webhook et Jira. De plus, vous pouvez envoyer du JSON à AWS EventBridge en utilisant les [variables de workflow](https://docs.newrelic.com/docs/alerts/get-notified/custom-variables-alert-event-workflows/) pointant vers le résultat de l&apos;enrichissement des données. De plus, vous pouvez uniquement envoyer des images vers d&apos;autres destinations telles que Slack, Pagerduty, ServiceNow (App) et par e-mail. Pour obtenir de meilleurs résultats dans Webhook et Jira, utilisez une requête avec une sortie à valeur unique, telle que `count`, `min` ou `max`.
       ...
       * Pour des exemples d&apos;enrichissement, consultez la section [Exemples d&apos;enrichissement des données de workflow](/docs/alerts/get-notified/custom-variables-alert-event-workflows/#enrichment-examples).
...
7. Notifier : Choisissez une ou plusieurs destinations et ajoutez un message facultatif.
   ...
   <Callout variant="tip">
     Dans n&apos;importe quel canal de destination, commencez à taper et un menu de variables s&apos;ouvrira. Vous verrez les noms des [variables](/docs/alerts/get-notified/custom-variables-alert-event-workflows), qui seront remplacés par les valeurs de la variable lors de l&apos;exécution. Pour utiliser les résultats des enrichisseurs, utilisez leur nom.
   </Callout>
