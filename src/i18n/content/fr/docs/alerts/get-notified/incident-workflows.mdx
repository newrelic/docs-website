...
6. Optionnel : Enrichissez vos données.
   ...
   <CollapserGroup>
     <Collapser className="freq-link" id="enrichments" title="Comment enrichir les données">
       ...
       * Pour des exemples d’enrichissement, voir [Exemples d’enrichissement des données de workflow](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/custom-variables-incident-workflows/#enrichment-examples).
       ...
       * Remarque : le nom de la requête doit être unique, car vous l&apos;utiliserez pour décider comment inclure le résultat dans la charge utile de la notification qui sera envoyée par le workflow.
         ...
         <Callout variant="important">
           À ce stade, la représentation visuelle des enrichissements est envoyée à Slack et par e-mail. Pour les destinations ServiceNow et JIRA, nous recommandons de se limiter aux requêtes NRDB avec des résultats à valeur unique. Par exemple : nombre, min ou max.
