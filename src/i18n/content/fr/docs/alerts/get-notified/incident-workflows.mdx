...
## Comment les flux de travail sont-ils déclenchés ? [#workflows-triggered]
...
* Pour modifier le flux de travail : autorisations `Modify` pour `Applied intelligence > Workflows` et toutes les autorisations pour `Applied intelligence > Channels` et `applied_intelligence.create.workflow`.
...
6. <DNT>**Trigger Event:**</DNT> Cette colonne affiche l’événement de problème qui a déclenché la notification de workflow. Cliquer sur l&amp;apos;événement fournira plus d&amp;apos;informations sur la manière dont les flux de travail sont déclenchés.
