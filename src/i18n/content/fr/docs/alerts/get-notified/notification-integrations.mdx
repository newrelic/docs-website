...
<CollapserGroup>
  <Collapser className="freq-link" id="jira" title="Atlassian Jira">
    ...
    * La reconnaissance du problème New Relic se déclenche lorsque le statut du problème Jira passe à `in-progress`.
    ...
    Cliquez sur une notification de test avec les valeurs de champ par défaut pour afficher le ticket Jira. En cas de succès, vous obtiendrez un lien pour consulter l&apos;événement d&apos;alerte dans Jira.
  ...
  <Collapser className="freq-link" id="email" title="E-mail">
    La destination e-mail est créée automatiquement lorsque vous choisissez <DNT>**Email**</DNT> comme canal de notification pour un [workflow](/docs/alerts/get-notified/alert-event-workflows/), vous n&apos;avez donc pas besoin de la configurer depuis le menu <DNT>**Destinations**</DNT>. Chaque destination d&apos;e-mail est propre au workflow qui lui est associé, ce qui signifie qu&apos;elle peut apparaître en double dans le flux des destinations.
  ...
  <Collapser className="freq-link" id="pagerduty" title="PagerDuty">
    Intégrez New Relic à PagerDuty pour créer, mettre à jour, acquitter et résoudre automatiquement les événements d&apos;alerte PagerDuty.
  ...
  <Collapser className="freq-link" id="slack" title="Mou">
    ...
    Votre espace de travail Slack doit avoir l&apos;[application New Relic](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)(ou l&apos;[application UE](https://slack.com/apps/AS5D75HQQ-new-relic-eu?tab=more_info) pour les clients `one.eu.newrelic`, ou l&apos;[application JP](https://newrelic.slack.com/marketplace/A0ATV7NGLLD-new-relic-jp) pour les clients `one.jp.newrelic`) installée. Un administrateur de l’espace de travail doit approuver l’application avant que vous puissiez l’installer individuellement.
