...
Avec les conditions multi-emplacements, vous pouvez définir le nombre d&apos;emplacements qui doivent échouer simultanément pour déclencher un événement d&apos;alerte et vous envoyer une notification. Par exemple, si votre moniteur s&apos;exécute à six emplacements, vous pouvez définir une condition nécessitant l&apos;échec de quatre emplacements avant de recevoir une notification.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Un contrôle d&apos;emplacement en échec sera considéré comme en échec jusqu&apos;à ce qu&apos;il effectue un contrôle réussi, **jusqu&apos;à un maximum de 24 heures**. Le buffer d&apos;évaluation pour les conditions Synthetics multi-emplacements a un TTL (time-to-live) de 24 heures. Par exemple : un emplacement peut échouer puis redevenir immédiatement disponible, mais nous le considérerons en échec jusqu&apos;à ce qu&apos;une vérification planifiée soit signalée comme réussie. Si aucun signal de rétablissement n&apos;est reçu dans les 24 heures, la défaillance est finalement supprimée de la mémoire tampon.
      </td>
    </tr>
  </tbody>
</table>
...
<figcaption>
  Ce diagramme montre un exemple de la façon dont un paramètre de quatre emplacements en échec déclenchera un événement d&apos;alerte pour des défaillances survenant les unes après les autres. Notez que les vérifications d&apos;emplacement ayant échoué seront considérées comme telles jusqu&apos;à la prochaine vérification réussie.
</figcaption>
...
Pour utiliser l’API REST des alertes afin de gérer les conditions multi-emplacements, utilisez l’ [explorateur d’API REST](https://api.newrelic.com/docs/#/Deprecation%20Notice%20-%20Alerts%20Location%20Failure%20Conditions).