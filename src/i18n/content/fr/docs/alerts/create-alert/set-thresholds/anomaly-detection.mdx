...
* <DNT>**New Relic calculation**</DNT> (par défaut) : Détermine automatiquement la saisonnalité de chaque signal en fonction de plusieurs facteurs, notamment l&apos;âge des données, la cohérence des données et les fluctuations régulières.
  ...
  **Exemple :** Une architecture de microservices dynamique où vous monitorez des dizaines de services distincts dans une seule condition. Les services orientés utilisateur peuvent connaître des pics de trafic quotidiens, tandis que les services d&apos;arrière-plan peuvent ne présenter aucun schéma prévisible. Ce paramètre permet au système de s&apos;adapter à chaque signal spécifique sans réglage manuel.

* <DNT>**Hourly**</DNT>: Applique un modèle horaire à tous les signaux dans la condition de détection d&apos;anomalie.

  **Exemple :** Un serveur qui exécute une tâche cron gourmande en ressources au début de chaque heure, comme un script de synchronisation de données horaire ou un processeur de file d&apos;attente par lots. Ce paramètre permet de détecter des anomalies dans les modèles d&apos;utilisation du CPU et de la mémoire.

* <DNT>**Daily**</DNT>: Applique un modèle quotidien à tous les signaux dans la condition de détection d&apos;anomalie.

  **Exemple :** Une application d&apos;entreprise interne, telle qu&apos;un portail RH ou un intranet, qui enregistre systématiquement des taux de connexion élevés à partir de 8 h 00 et une baisse brutale après 17 h 00 chaque jour.

* <DNT>**Weekly**</DNT>: Applique un modèle hebdomadaire à tous les signaux dans la condition de détection d&apos;anomalie.

  **Exemple :** Une plateforme logicielle B2B qui enregistre un pic de trafic du lundi au jeudi, une baisse notable le vendredi et un trafic quasi nul le week-end.
...
* <DNT>**None**</DNT>: Désactive entièrement la saisonnalité, garantissant qu&apos;aucun modèle saisonnier n&apos;est pris en compte lors de l&apos;évaluation des signaux.
  ...
  **Exemple :** Un moniteur de taux d&apos;erreurs système critiques, tel que les erreurs HTTP 500 ou les délais d&apos;attente de connexion à la base de données. Ces métriques devraient idéalement rester à une ligne de base plate proche de zéro en permanence. Un pic à 14 h 00 est une anomalie, peu importe qu&apos;il se soit également produit hier à 14 h 00.
