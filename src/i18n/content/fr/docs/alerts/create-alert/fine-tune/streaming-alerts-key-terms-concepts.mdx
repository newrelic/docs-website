...
La <InlinePopover type="alerts" />plateforme de streaming recherche des événements d&apos;alerte en fonction des données présentes ou manquantes dans votre flux de données, ou [signal](/docs/using-new-relic/welcome-new-relic/get-started/glossary#signal), entrant dans New Relic.
...
À mesure que les données affluent dans New Relic, elles sont filtrées par la condition NRQL. Avant que les données ne soient évaluées, elles doivent répondre aux critères définis par la [clause `WHERE` de la requête NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions#sel-where). Au lieu d&apos;évaluer immédiatement ces données pour détecter des événements d&apos;alerte, les conditions d&apos;alerte NRQL collectent les données sur une période appelée fenêtre d&apos;agrégation. Un délai/minuteur supplémentaire permet aux points de données plus lents d&apos;arriver avant que la fenêtre ne soit agrégée.

Une fois le délai/temps du minuteur écoulé, New Relic agrège les données en un seul point de données. Alerts compare ensuite le point de données aux critères de seuil de la condition pour déterminer si un événement d&apos;alerte doit être ouvert.

Même si un point de données répond aux critères d&apos;un événement d&apos;alerte, un événement d&apos;alerte peut ne pas être ouvert. Un événement d&apos;alerte n&apos;est ouvert que lorsque les points de données répondent systématiquement aux critères de seuil sur une période donnée. Il s&apos;agit de la durée du seuil. Si les points de données dépassent le seuil pendant toute la durée du seuil, nous vous enverrons une notification en fonction des paramètres de votre politique.
...
<table class="alternate">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Une durée spécifique qui détermine si un événement d&apos;alerte est créé. Si votre condition NRQL spécifiée répond aux critères de seuil pendant la durée du seuil, un événement d&apos;alerte se produit.
...
Par exemple, supposons que vous monitorez l&apos;utilisation du processeur dans des fenêtres de durée d&apos;une minute et un délai de 3 minutes.
...
Par exemple, supposons que vous monitorez des données CloudWatch qui arrivent assez rarement. Vous utilisez une durée de fenêtre de 1 minute et une minuterie de 3 minutes.
...
La perte de signal se produit lorsqu&apos;aucune donnée ne correspond à la condition NRQL pendant une période spécifique. Une perte de signal est causée par divers facteurs. La clause `WHERE` de votre requête NRQL peut filtrer les données avant qu&apos;elles ne soient évaluées pour des événements d&apos;alerte. Cela peut également signifier qu&apos;un service ou une entité est hors ligne, ou qu&apos;une tâche périodique a échoué et qu&apos;aucune donnée n&apos;est envoyée à New Relic.

Pour éviter les notifications inutiles, vous pouvez choisir le délai d&apos;attente avant d&apos;être notifié par un événement d&apos;alerte de perte de signal. Vous pouvez utiliser la détection de perte de signal pour ouvrir des événements d&apos;alerte et être notifié lorsqu&apos;un signal est perdu. Sinon, vous pouvez utiliser une perte de signal pour fermer les événements d&apos;alerte pour les services éphémères ou les données sporadiques, telles que le nombre d&apos;erreurs.
...
<Callout variant="tip">
  ...
  Pour en savoir plus sur la perte de signal et le remplissage des lacunes, consultez [ce message du forum d’assistance](https://knowledge.newrelic.com/s/article/How-Can-I-Figure-Out-When-To-Use-Gap-Filling-and-Loss-of-Signal).
</Callout>