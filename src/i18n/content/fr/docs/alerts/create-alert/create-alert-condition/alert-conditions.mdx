...
* La [page<DNT>**Alerts Overview**</DNT> ](/docs/alerts/smart-alerts-coverage), pour appliquer la couverture des normes sur plusieurs entités à la fois avec des alertes intelligentes (aperçu public)
...
<Callout variant="important">
  Après avoir créé une condition d&apos;alerte, la plateforme New Relic doit l&apos;évaluer avant de pouvoir la lier à une entité, telle qu&apos;une application, un service ou un moniteur synthétique. L&apos;évaluation commence uniquement après que la plateforme a reçu un signal de données valide qui correspond à la requête NRQL ou à la métrique pour la condition. L&apos;évaluation peut prendre quelques minutes pour se terminer. Pour vérifier que la condition reçoit les données correctes, exécutez sa requête dans le [générateur NRQL](/docs/nrql/get-started/introduction-nrql-new-relics-query-language/).
</Callout>

Pour toutes les méthodes à l&apos;exception de notre mode guidé, le processus de création d&apos;une condition d’alerte sera *exactement* le même que celui décrit dans les étapes ci-dessous. Lors de la configuration de votre condition, consultez les [bonnes pratiques en matière d&apos;alertes](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices) pour obtenir des conseils sur la dénomination des conditions, l&apos;ajout de tags, la définition de seuils efficaces et la réduction du bruit.
...
<CollapserGroup>
  ...
  <Collapser id="dynamic-targeting-labels" title="Ciblage dynamique avec des étiquettes pour les applications">
    ...
    2. Lors de l&apos;identification de l&apos;entité, sélectionnez l&apos;onglet <DNT>**Labels**</DNT> . Recherchez une étiquette par nom ou sélectionnez une étiquette dans la liste des catégories.
       ...
       Vous pouvez également créer des conditions directement dans le contexte de ce que vous monitorez avec [l&apos;infrastructure](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information).
  </Collapser>
