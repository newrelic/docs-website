...
Avec l&apos;onglet <DNT>**Group errors**</DNT>, vous pouvez filtrer et regrouper dynamiquement les erreurs pour une analyse plus approfondie. Le regroupement des erreurs vous indique où se produisent vos erreurs JavaScript et fournit des outils pour vous aider à déterminer la cause première. Vous pouvez également suivre l&apos;étendue des erreurs grâce aux **Sessions impactées**, qui affiche automatiquement le nombre de sessions de navigateur affectées par chaque groupe d&apos;erreurs. De plus, vous pouvez interroger et créer des dashboards de données d&apos;erreurs JS dans le [générateur de requêtes](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder), ou utiliser l&apos;API du navigateur pour monitorer les erreurs gérées.
...
## Suivi de l&apos;impact des sessions [#session-impact]

Pour les applications de navigateur, New Relic suit automatiquement les **Sessions impactées** en tant que métrique clé dans Errors Inbox. Cette métrique indique le nombre de sessions de navigateur uniques affectées par les erreurs JavaScript et vous aide à comprendre l&apos;étendue des problèmes au sein de votre base d&apos;utilisateurs.

**Principaux avantages du suivi de l&apos;impact des sessions :**

* **Collecte automatique**: Aucune configuration supplémentaire requise - la métrique est collectée automatiquement pour toutes les entités de navigateur
* **Métrique de repli**: Lorsque les données d&apos;impact des utilisateurs ne sont pas disponibles, New Relic affiche les sessions impactées pour assurer la visibilité de l&apos;étendue des erreurs
* **Priorisation**: Vous aide à prioriser les corrections d&apos;erreurs en affichant les erreurs qui affectent le plus de sessions de navigateur
* **Alertes**: Vous pouvez créer des alertes basées sur les seuils de sessions impactées

Vous pouvez utiliser une requête pour les sessions impactées en utilisant NRQL :
...
## Événements SecurityPolicyViolation (violations CSP) [#policy-violations]

New Relic capture l&apos;événement `securitypolicyviolation` émis par les navigateurs et signale toutes ces occurrences. Cependant, cela commence dans la version 1.313.0 de l&apos;agent et n&apos;est pas encore intégré dans l&apos;UI telle que la vue Errors Inbox. Par conséquent, ces données ne sont disponibles que via NRQL pour le moment. Consultez l’ [événement du dictionnaire d’attributs](/attribute-dictionary/?event=SecurityPolicyViolation) pour une liste des attributs signalés.
