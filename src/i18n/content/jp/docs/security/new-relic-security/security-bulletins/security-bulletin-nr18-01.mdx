...
[Python 高セキュリティ モード](/docs/agents/python-agent/configuration/python-agent-configuration#high_security)の内部監査により、エージェントがトレース API を使用してメッセージ パラメータをカスタマイズすることにより、New Relic に追加データを送信できることが判明しました。開発者が関数トレース API またはメッセージ トレース API を使用して追加パラメータを渡す場合、カスタム パラメータが New Relic に送信されることがあります。高セキュリティ モードが有効な場合、これらは無効にする必要があります。
