...
ここでは、レガシー APM のサービスマップ機能を使用して、サービスマップを作成、カスタマイズ、および使用する方法について説明します。現行のサービスマップ機能の使用方法については、 [サービスマップの使用方法](/docs/understand-dependencies/understand-system-dependencies/service-maps/how-use-service-maps) を参照してください。
...
APM のサービスマップ機能は、デフォルトのマップや、自動的にデフォルトのマップを構築するアーキテクチャの提案を提供することで、最初のマップの作成を支援します（10 個以上のレポートアプリがある場合）。これらの提案されたマップを見るには、 **[one.newrelic.com](https://one.newrelic.com) > その他> サービスマップ> 提案されたマップ**.
...
<CollapserGroup>
  ...
  >
    ...
    3. [必要に応じて](#select-arrange-group-nodes) を再配置する。
       ...
       ![クロップインアウトゴーイングサービスマップ.png](src/images/crop-incoming-outgoing-service-maps.png "クロップインアウトゴーイングサービスマップ.png")
  ...
  >
    ...
    2. **App/Service** リストから、 ![](src/images/custom-node.png) **Create custom node** を選択します。
    ...
    11. Select **Save**.
        ...
        ![カスタムコネクションサービスマップを作成する。](src/images/crop-custom-connections-service-maps.png "カスタムコネクションサービスマップを作成する。")
    ...
    <tr>
      ...
      <td>
        ...
        * ドラッグセレクト
          ...
          <ImageSizing width="22px">![](src/images/sm-drag-select-toggle_0.png) </ImageSizing>
...
<CollapserGroup>
  ...
  >
    ...
    * drag-select <ImageSizing width="22px">![](src/images/sm-drag-select-toggle_0.png) </ImageSizing>アイコンをクリックすると、ドラッグセレクトモードに切り替わります。マウスをクリック＆ドラッグして、選択したいノードの周りに四角を描きます。
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            2つ以上のノードを選択し、 <ImageSizing width="22px">![](src/images/sm-grouping-icon.png) </ImageSizing>**Create group**.
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            ![グループからのノードの削除](src/images/crop-service-map-app-group.png "グループからのノードの削除")
          </td>
        </tr>
      </tbody>
    </table>
...
![ダッシュボードにサービスマップを追加](src/images/map_in_dashboard.png 'map_in_dashboard.png')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > ダッシュボード**:
  ダッシュボードにサービスマップを追加することで、報告されているデータとそのデータがどこから来ているのかというコンテキストを提供します。
</figcaption>
...
- <Icon style={{ color: '#A5C27C' }} name="fe-square" />
  ...
  緑色。サービスからの応答時間がベースラインの 1.75 倍以下であること。

- <Icon style={{ color: '#FCB95D' }} name="fe-square" />

  黄色。サービスからの応答時間がベースラインの 1.75 倍よりも長い。

- <Icon style={{ color: '#D75966' }} name="fe-square" />

  赤。サービスからの応答時間がベースラインの 2.5 倍よりも長い。
...
<CollapserGroup>
    ...
    **Discover your environment** 機能を使うと、ワンクリックですべてのアプリケーション、サービス、依存関係をまとめて見ることができます。この機能は、接続されているすべてのアプリケーションやサービス（外部機器やデータベースを含む）を検出し、ソフトウェア・アーキテクチャ環境全体を包括的に把握することができます。
    ...
    3. ノードの上にカーソルを置くと、そのノードの依存関係や、サービスマップの他の部分との接続方法が表示されます。個々のノードをクリックすると、ズームインして詳細が表示されます。
       ...
       **Discover your environment** 機能は、古いバージョンの New Relic エージェントを使用している場合は利用できない場合があります。バージョン情報については、 [Troubleshooting cross application tracing](/docs/apm/transactions/cross-application-traces/troubleshooting-cross-application-tracing#agents) を参照してください。また、多くの要素を持つ非常に複雑なサービスマップでは、この機能が利用できない場合があります。
       ...
       ![サービスマップのために環境を発見する方法を示すスクリーンショット。](src/images/discover_your_environment.png "Discover_your_environment.png")
  ...
  >
    ...
    ![サービスマップをズームアウトして信号機モードで表示する](src/images/traffic-lights_service-maps_APM.jpg "Traffic-Lights_Service-Maps_APM.jpg")
