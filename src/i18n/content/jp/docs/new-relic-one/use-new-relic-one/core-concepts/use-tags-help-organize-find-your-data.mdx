...
At New Relic, our tags are key:value pairs (like `team: operations`) added to various sets of data, like monitored apps and hosts, agents, dashboards, and [workloads](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster). We make some important [attributes](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute) available as tags (for example, app metadata like app name and language, and host metadata like host name and AWS region). And you can also add your own custom tags.

You can use tags in the UI to filter down to relevant data, as shown here:

![New Relic filtering workloads using tags](./images/new-relic-tags-workloads_0.png "new-relic-tags-workloads.png")

<figcaption>
  Here you can see an `accountId` tag being used to filter [workloads](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster) for a specific account.
</figcaption>

You can use tags to:

* Help organize data coming from a large number of sources and/or to a large number of New Relic accounts
* Identity teams, roles, environment, or regions to know who’s responsible for what (see example [use cases](#org-examples))
...
Before [adding tags](#tag-sources), we recommend first seeing how our default tags work for you. When you're ready to add your own tags, read the best practices below to get the most out of them.
...
<CollapserGroup>
  ...
  >
    You can apply tags [via the UI and API](#add-via-ui-api) or you can add tags using APM agent configuration:
...
![New Relic advanced tag filter query](./images/new-relic-advanced-tag-filter-query.png "new-relic-advanced-tag-filter-query.png")

<figcaption>
  Use the filter field at the top of the [entity explorer](/docs/new-relic-one/use-new-relic-one/ui-data/entity-explorer-view-performance-across-apps-services-hosts) to filter down to the entities you care about. You can use multiple filter conditions.
</figcaption>

To filter down to certain entities using tags:

1. From [one.newrelic.com](https://one.newrelic.com), click **Entity explorer**.
...
## Custom queries and charts [#query-tags]

As explained in [Tag sources](#tag-sources), some [attributes](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute) are available as tags. You can query attributes and create custom charts using [NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language).
