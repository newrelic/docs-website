...
New Relicのタグは、モニターするアプリとホスト、エージェント、ダッシュボード、および`ワークロード`など、様々なデータセットに追加された「キー：値」のペアです（[team: operations](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster)など）。いくつかの重要な[属性](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)はタグ（アプリ名や言語などのアプリのメタデータ、およびホスト名やAWSリージョンなどのホストのメタデータなど）として利用可能であり、独自のカスタムタグを追加することもできます。

以下に示すように、UIでタグを使用して関連データを絞り込むことができます。

![タグを使用したNew Relicフィルタリングワークロード](./images/new-relic-tags-workloads_0.png "new-relic-tags-workloads.png")<figcaption>ここでは、`accountId`タグを使用して特定のアカウントの[ワークロード](/docs/new-relic-one/use-new-relic-one/workloads/workloads-isolate-resolve-incidents-faster)をフィルターしています。</figcaption>
...
* [APMデータのクエリとチャート作成に役立つ](#query-apm-tags)
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="APMエージェントタグ"
  >
    タグは、[UIおよびAPIを通して](#add-via-ui-api)追加するか、APMエージェント設定を使用して追加できます。これらのタグの適用方法には違いがあります。
    ...
    * [Ruby](/docs/agents/ruby-agent/configuration/ruby-agent-configuration#labels)
    ...
    APMタグのクエリの詳細については、[APMタグのクエリ](#query-apm-tags)を参照してください。
  </Collapser>
...
[タグソース](#tag-sources)で説明されているように、一部の重要な[属性](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)はタグとして使用されます。[NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language)を使用してすべての属性をクエリし、カスタムチャートを作成し、[NRQLアラート条件](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions)を作成できます。