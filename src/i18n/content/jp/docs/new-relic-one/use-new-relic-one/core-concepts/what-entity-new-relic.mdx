...
New Relicモニタリングは、**エンティティ**のコンセプトを中心に構築されています。このドキュメントでは、エンティティの定義方法、エンティティを使用して何ができるか、エンティティまたはエンティティのグループを作成する方法について説明します。
...
* ほとんどのエンティティでは、そのGUIDは属性[`entityGuid`](/?event=SyntheticCheck&attribute=entityGuid)としてレポートされます。ワークロードについては、`workloadGuid`になります。[NRQLクエリ](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language)を実行して、そのGUIDでエンティティを検索できます。
...
* エンティティタイプに関する技術的な詳細については、[GitHubレポ](https://github.com/newrelic/entity-definitions/tree/main/definitions)を参照してください。エンティティタイプの`定義`ファイルには、次のような情報が表示されます。

  * `ドメイン`：たとえば、`APM`、`Infra`。
  * その`タイプ`：`アプリケーション`または`AWSECSCONTAINERINSTANCE`。
  * デフォルトの[タグ](/docs/new-relic-one/use-new-relic-one/core-concepts/use-tags-help-organize-find-your-data)。
  * `entityExpirationTime`：そのエンティティからのデータがUIで存続する期間。これは[データベースのデータ保持](/docs/telemetry-data-platform/manage-data/manage-data-retention)とは異なります。
...
ミューテーション`entityRelationshipUserDefinedDelete`を使用して関係を削除します。`ソース`と`ターゲット`は必須ですが、`タイプは`必須ではありません。ミューテーションが`タイプ`なしで実行される場合、2つのエンティティ間のすべての関係は削除されます。
