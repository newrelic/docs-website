...
これは、New Relicエージェントとの通信のためにNew Relic[コレクタ](/docs/apm/new-relic-apm/getting-started/glossary#collector)（例：`https://collector.newrelic.com`）が使用するネットワークやIPアドレス、ドメイン、ポート、エンドポイントのリストです。[](/docs/apm/new-relic-apm/getting-started/glossary#agent)[TLSがすべてのドメインについて必要です。](#tls)
