...
ご質問やご意見がある場合、また、.NET エージェントの **Instrumentation** エディタのベータ期間中にサポートが必要な場合は、 [dotnet-li-editor-beta@newrelic.com](mailto:dotnet-li-editor-beta@newrelic.com) までメールをお送りください。
...
- 権限のあるユーザーが、ローカルの XML ファイルではなく、New Relic UI からカスタムインスツルメンテーションを編集できるようになります。
...
![APM設定：.NETインスツルメンテーション](src/images/net-instrumentation.png 'APM設定：.NETインスツルメンテーション')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM> (select a .NET app)>
  Settings> Instrumentation:** **Instrumentation**
  エディタを使用すると、UIでカスタムインスツルメンテーションを簡単に表示または編集することができます。この
  XML の例では、アセンブリ名、クラス名、およびメソッド名を示しています。
</figcaption>
...
.NET エージェントは、 [収穫サイクル](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#harvest-cycle) （通常は数分）以内に新しいインスツルメンテーションを開始します。 **Current instrumentation** **Instrumentation** UI のセクションには、適用された変更のステータスが表示されます。

**Instrumentation** エディタからの XML は、エージェントを実行しているマシンのローカルインスツルメンテーションファイルに存在する XML と結合されます。エージェントは、最終（結合）XML の変更点を定期的に探し、エージェントやアプリケーションの再起動を必要とせずに、動的に変更点をピックアップします。
