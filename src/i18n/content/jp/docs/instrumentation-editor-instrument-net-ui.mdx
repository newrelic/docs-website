...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (.NET アプリを選択) > 設定 > インストルメンテーション:****インストルメンテーション**エディターは、UI でカスタム インストルメンテーションを表示または編集する簡単な方法を提供します。この XML の例は、アセンブリ名、クラス名、およびメソッド名を示しています。
</figcaption>
