...
<CollapserGroup>
  ...
  >
    ...
    * その他のすべてのアカウントで、当社の [オリジナルユーザーモデルにユーザーがいる場合](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#user-models): 最初にアカウントを作成する際、このフィールドは必須であり、定義できるユーザーはアカウント `オーナー` のみです。追加のユーザーを追加するには、 [Partnership API ユーザーオブジェクト](/docs/new-relic-partnerships/partnerships/partner-api/partnership-api-user-object) を使用します。
