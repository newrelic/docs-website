...
</CollapserGroup>## 製品のマッピング（product_id） [#api-product-mapping]アカウント作成の各コールでは、少なくとも 1 つの New Relic 製品タイプを指定する必要があります。API では、タイプには数値 `product_id` しか受け付けません。<Callout variant="important">
...
</Callout><Callout variant="important">
...
</CollapserGroup>## サブスクリプションAPIコール [#user-calls]本機をお使いになる前に、まず [要件](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partner-api/) をお読みください。以下に、サブスクリプション関連の API 関数の URL パターンを示します。使用する場合は、JSON オブジェクトと [Partner API key](/docs/accounts-partnerships/partnerships/partner-api/partner-api-reference#the-api-key) を含む HTTP ヘッダーを一緒に送信してください。例えば、以下のようになります。```
...
</table>## サブスクリプションAPIの例 [#examples]ここでは、 [オリジナル価格モデルの](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans) サブスクリプションを作成するためのAPIコールと、そのアカウントのサブスクリプションをリストアップしたJSONレスポンスの例を示します。<CollapserGroup>
  <Collapser
    id="call-example"
...
</CollapserGroup>## APIの例(v2)ここでは、 [オリジナル価格モデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans) サブスクリプションをリストアップ、表示、作成、更新するためのAPIのリクエストとレスポンスの例を示します。レスポンスの改行は読みやすくするためです。実際のレスポンスは連続した行として表示されます。<CollapserGroup>
  <Collapser
    id="example-list"
    title="リスト"
  >
