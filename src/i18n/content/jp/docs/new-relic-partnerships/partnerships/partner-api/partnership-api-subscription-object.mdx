...
サブスクリプションオブジェクトは、元の料金モデルを使用している場合にのみ使用できます。 [NewRelicOne価格設定モデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans)のアカウントはサポートしていません。詳細については、 [PartnershipAPIの要件](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partner-api/)をご覧ください。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="シンセティックス"
  >
    New Relic Syntheticsの料金体系では、許容されるモニタリングチェックのデフォルト数とデータ保持期間はサブスクリプションレベルによって異なります。製品IDの**整数**形式を使用して、サブスクリプションレベルを識別します。
    ...
    <Callout variant="tip">
      以前に非推奨のPartnershipAvailabilityMonitoring APIを使用していた場合は、 [Synthetics API](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-synthetics-api-tests)を使用してチェックをプロビジョニングし、 [REST APIを使用](/docs/alerts/rest-api-alerts/new-relic-alerts-rest-api/rest-api-calls-new-relic-alerts)してアラートをプロビジョニングし、顧客へのアラート通知を作成できます。
    </Callout>
  </Collapser>
