...
## サブスクリプションAPIの例 [#examples]ここでは、 [オリジナル価格モデルの](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans) サブスクリプションを作成するためのAPIコールと、そのアカウントのサブスクリプションをリストアップしたJSONレスポンスの例を示します。<CollapserGroup>
  <Collapser
    id="call-example"
...
## APIの例(v2)ここでは、 [オリジナル価格モデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-changes-pricing-user-model/#pricing-plans) サブスクリプションをリストアップ、表示、作成、更新するためのAPIのリクエストとレスポンスの例を示します。レスポンスの改行は読みやすくするためです。実際のレスポンスは連続した行として表示されます。<CollapserGroup>
  <Collapser
    id="example-list"
    title="リスト"
  >
