...
<CollapserGroup>
  ...
  >
    お客様のユーザーの [ユーザーモデル](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure) がこれらのクエリに影響することに注意してください。オリジナルのユーザーモデルを使用している場合は、アカウントごとにしか問い合わせできません。お客様のユーザーが New Relic One のユーザーモデルを利用している場合は、 [New Relic の組織](/docs/accounts/accounts-billing/new-relic-one-pricing-users/new-relic-account-structure) のトップレベルのアカウントに問い合わせてください。
    ...
    ユーザータイプ別の変化（フルプラットフォームユーザーとベーシックユーザー）に絞って見たい場合は、次のようにします。
