...
<CollapserGroup>
  <Collapser id="example-multiple-metrics" title="複数の指標を表示">
  ...
  <Collapser id="example-math" title="メートル法のデータを使った数学的演算">
  ...
  <Collapser id="example-filters-timeseries" title="フィルターを使って特定の時系列を選択">
  ...
  <Collapser id="example-view-raw-datapoints" title="メトリックの生データポイントを表示">
...
<CollapserGroup>
  <Collapser id="example-multiple-metrics-wildcard" title="ワイルドカードで複数の指標を表示">
  ...
  <Collapser id="example-math" title="ワイルドカードを使用したメートル法データの算術演算">
...
[`Metric`データには複数のタイプ](/docs/data-apis/understand-data/metric-data/metric-data-type/#metric-types)( `gauge`や`count`など) があり、各タイプには複数の関連する<DNT>**fields**</DNT>があります。 使用可能なフィールドの種類の詳細については、 [`getField()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#func-getfield)を参照してください。
...
<CollapserGroup>
  <Collapser id="example-list-names-host" title="特定のホストに対するすべてのメトリック名をリストアップ">
  ...
  <Collapser id="example-list-gauge-metrics" title="ゲージメトリクスの一覧">
    ...
    一致する`metricName`をターゲットにするには、 `%`ワイルドカードを使用することに注意してください。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="example-list-names-account" title="アカウント内のすべてのメトリック名を一覧表示">
    ```sql
  ...
  <Collapser id="example-show-keys-metric" title="特定のメトリックの属性キーを表示する">
    ```sql
