...
APM は、[メトリック タイムスライス データ](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data)の形式でメトリック データをレポートし、 [NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language)を使用して、このタイプのデータに対してクエリ、ファセット、およびアラートを実行できます。メトリクスとイベントのタイムスライス データを調べるには、 [メトリクスを調べる を](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer#blocks-metrics)参照してください。

## メートル法のタイムスライスデータを照会する理由は？ [#why]

私たちは[いくつかの方法で指標](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics)を報告します。さまざまなメトリック データの 1 つを、**metric timeslice data** \[メトリック タイムスライス データ]と呼びます。これは、APM で多くのチャートを生成するために使用されるデータのタイプです。 <InlinePopover type="mobile"/>、ブラウザー (詳細については、 [「メトリック タイムスライス データ](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data)」を参照してください)。

従来、このタイプのデータは、クエリ言語である NRQL を介してクエリすることはできませんでした。しかし今、私たちは人気を変換しています <InlinePopover type="apm"/>メトリクス タイムスライス データからより詳細なディメンション メトリクス データまでのメトリクス。これにより、NRQL および [NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-nrql-tutorial)経由でクエリできるようになります。 この機能により、これらの重要な APM メトリックの強力で詳細なカスタム ビジュアライゼーションを作成できます。これには、 [カスタム メトリック](/docs/agents/manage-apm-agents/agent-data/collect-custom-metrics)をクエリできることも含まれます。
...
APM では、一部のチャートには、 [そのチャートを生成した NRQL クエリを表示する](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language#what-is-nrql)オプションがあります。 これは、メトリクスのクエリ方法を理解するのに適したオプションです。以下で検討する NRQL クエリは、 <InlinePopover type="apm"/>まとめページ。
...
オリジナルの変換 <InlinePopover type="apm"/>メトリクス タイムスライス メトリクスをクエリに使用できるディメンション メトリクスに変換するプロセスは進行中のプロセスであり、完了していません。 このセクションで探しているメトリックが表示されない場合は、 [汎用クエリを](#generic-queries)参照してください。

ここでは、元の [APMメトリクスのタイムスライス](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data) メトリクスがどのように次元メトリクスに変換されるかを説明します。
...
これらの属性は、 <InlinePopover type="apm"/>上記の指標表。
...
<table style={{ width: "700px" }}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Java の場合 <InlinePopover type="apm"/>エージェント、 `host : port`
      </td>
    </tr>
...
## `newrelic.timeslice.value`メトリックを使用した一般的なクエリ [#generic-queries]
...
### `newrelic.timeslice.value`を使用する場合 [#when-to-use]
...
一部のメトリック タイムスライス名には、属性値がメトリック名のセグメントとして含まれています。たとえば、 <InlinePopover type="apm"/>エージェントは、次の形式を使用して外部通話の時間を追跡することにより、指標を報告します。
