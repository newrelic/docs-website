...
New Relic は[いくつかの方法でメトリクス](/docs/using-new-relic/data/understand-data/new-relic-data-types#metrics)を報告します。 <DoNotTranslate>**metric timeslice data**</DoNotTranslate> と呼ばれるメトリクス データの一種。これは、APM、<InlinePopover type="mobile"/>、 browserで多くのチャートを生成するために使用されるデータのタイプです (詳細については、 [「メトリック タイムスライス データ」](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data)を参照してください)。
