---
title: 累積メトリック レポートの詳細 (OTel および Prometheus)
tags:
  - Integrations
...
New Relic は、Prometheus と OpenTelemetry の累積データの送信をサポートし、取り込み時にデルタ変換を実行して、 [NRQL](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language)を介したそのデータとのやり取りをより簡単にし、プラットフォームの他のメトリクスとより整合させます。
...
[OpenTelemetry は、累積合計と累積ヒストグラムの収集と送信のサポートを定義します](https://opentelemetry.io/docs/reference/specification/metrics/data-model/#metric-points)。New Relic は、データをステートフルに並べ替えてバッファリングし、イベント時間の連続したメトリック測定間のデルタを出力します。時[系列](https://opentelemetry.io/docs/reference/specification/metrics/data-model/#timeseries-model)のデータの値が突然減少した場合、これをリセットとして扱い、その新しい測定値を独自のデルタ値として出力します (つまり、 `0`測定値が前にあったかのように)。
...
累積合計をデルタとしてクエリするには、New Relic に送信される他のデルタ メトリックと同じようにクエリを実行します。たとえば、メトリックの経時変化が必要な場合は、次のようにクエリできます。
...
合計をデルタに変換する場合、New Relic はデルタと一緒に累積値も発行して、最新の累積値をクエリできるようにします。累積値にアクセスするには、 [getField()](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#func-getfield)を使用し、次のようなクエリでグラフ化します。
...
OpenTelemetry は、[明示的にバインドされたヒストグラム](https://opentelemetry.io/docs/reference/specification/metrics/data-model/#histogram)と[指数ヒストグラム](https://opentelemetry.io/docs/reference/specification/metrics/data-model/#exponentialhistogram)の両方のサポートを定義しており、New Relic はどちらも受け入れることができます。累積ヒストグラムのクエリは、他のヒストグラムのクエリと同じです。これは、それをデルタに変換するためです。
...
ただし、累積合計とは異なり、ヒストグラムの元の累積表現は保存されないため、デルタ表現のみがクエリ可能です。

## トラブルシューティングと既知の制限 [#troubleshooting]

いくつかの異なるデータ レポートの状態と問題がどのように処理されるかについて、いくつかの詳細を次に示します。

### データの並べ替え [#reorder-data]

New Relic にデータポイントが順不同で到着する原因は、多くの場合にあることを理解しています。そのため、レポートの時系列に予期しないギャップが検出された場合は、データ ポイントをバッファリングして並べ替えます。バッファリングは制限されており、最終的にはデータポイントを「再シーケンスするには遅すぎる」と見なします。この場合、検出されたギャップ全体でデルタが計算され、時系列の処理が続行されます。
