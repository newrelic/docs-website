...
<CollapserGroup>
  ...
  >
    ...
    例：特定のトランザクションに費やされた時間に対する<InlinePopover type="apm"/>メトリックタイムスライスは、`WebTransaction/URI/foo`と命名され、これには午前10時20分から午前10時21分までの1分間のタイムスライスに対して0.793のレスポンスタイムとなります。このメトリクスは通常、`<category>/<class>/<method>`のようなパターンに従います。
