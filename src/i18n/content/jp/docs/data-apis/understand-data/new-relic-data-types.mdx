...
<CollapserGroup>
  ...
  >
    New Relic APM、ブラウザ、<InlinePopover type="mobile"/>は、<DoNotTranslate>**metric timeslice data**</DoNotTranslate>と呼ばれる単純なデータ形式でメトリクスをレポートし、表示します。メトリックタイムスライスは、以下の3つの部分で構成されます。つまり、メトリック名、メトリックを示す時間のセグメント（「タイムスライス」）、数値（測定結果）です。
  ...
  <Collapser
    ...
    title="イベントに関連付けられたメトリクス（Infrastructureモニタリングやその他の製品で使用）"
  >
...
まず、モニタリング業界の観点から見た<DoNotTranslate>**events**</DoNotTranslate>の定義について説明し、その後、どのようにしてNew Relicがイベントデータを処理しているかについていくつか具体的に説明します。
...
一部のイベントは保存されたレコードを生成し、そのレコードも一般的に、<DoNotTranslate>**event**</DoNotTranslate>と呼ばれます。
...
* <DoNotTranslate>**event**</DoNotTranslate>
  ...
  は一般的な用語のため、一部のNew Relicコンテキストでは、[NRQLを通じてクエリが可能なデータ型](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql#what-you-can-query)を指します。たとえば、NRQLクエリを実行すると、[検査済みイベント](/docs/insights/use-insights-ui/manage-account-data/rate-limits-insights#inspected-count-limits)の数が返されます。これは、クエリされたすべてのデータ型数です。

## ログデータ [#log-data]

まず、モニタリング業界の観点から見た<DoNotTranslate>**logs**</DoNotTranslate>の定義について説明し、その後、どのようにしてNew Relicがログレポーティングを処理しているかについていくつか具体的に説明します。
...
アプリケーションインフラストラクチャモニタリングの世界において、<DoNotTranslate>**tracing**</DoNotTranslate>は、プログラムやシステムの運用方法にまつわる情報をレポートするためのさまざまな方法を指す一般的な用語です。たとえば、<DoNotTranslate>**stack trace**</DoNotTranslate>はプログラムのサブルーチンに関する徹底的な情報を提供します。

多くのサービスやマイクロサービス全体に分散していることが多く、「トレーシング」は多くの場合、複雑な分散型環境を通じて伝播する時にリクエストをモニターする方法である<DoNotTranslate>**distributed tracing**</DoNotTranslate>を指します。
