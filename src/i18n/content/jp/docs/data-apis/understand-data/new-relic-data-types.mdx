...
<CollapserGroup>
  ...
  >
    New Relic APM、ブラウザは、 <InlinePopover type="mobile"/>**メトリックタイムスライスデータ**と呼ばれる単純なデータ形式でメトリクスをレポートし、表示します。メトリックタイムスライスは、以下の3つの部分で構成されます。つまり、メトリック名、メトリックを示す時間のセグメント（「タイムスライス」）、数値（測定結果）です。
