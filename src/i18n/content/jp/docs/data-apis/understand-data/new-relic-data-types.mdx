...
New Relicプラットフォームは、完全で効率的なシステム監視に必要と考える、次の4つの基本的なテレメトリーデータタイプについて構築されます。[メトリクス](#metrics)、[イベント](#event-data)、[ログ](#log-data)、および[トレース](#trace-data)（オブザーバビリティ業界では「MELT」と呼ばれることがよくあります）。[New Relicアカウントにサインアップ](https://newrelic.com/signup)し、当社のモニタリングサービスのいずれかをインストールすると、[データの処理](/docs/using-new-relic/cross-product-functions/install-configure/install-new-relic/)を開始できます。

## 当社のデータの概要

このドキュメントでは、MELTコアデータタイプ、その構成、機能での使用方法についてかなり技術的に説明します。基盤となるデータ構造を理解しなくても、ほとんどの機能を使用できます。この点をよく理解すると、[New Relicへのデータ入力](https://developer.newrelic.com/use-cases/collect-data-from-any-source)、UIに表示されるデータの理解、[データのクエリ](/docs/query-new-relic-data)に役立ちます。
...
<CollapserGroup>
  ...
  >
    ...
    New Relicでは、このメトリックデータは[`Metric`](/docs/data-ingest-apis/get-data-new-relic/metric-api/view-query-you-metric-data)データ型に関連付けられています。これは当社のプライマリメトリックデータ型であり、以下を含む多くのツールで使用されています。
    ...
    * [メトリックAPI](/docs/introduction-new-relic-metric-api)（上記のツールが使用する基盤となるAPI）および[テレメトリーSDK](/docs/telemetry-sdks-send-custom-telemetry-data-new-relic)。
    * [events-to-metricsサービス](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-events-metrics-service)

    その他のメトリックデータ型の一部は、次元メトリクスとしてクエリできます。例：
    ...
    * [一部のInfrastructureデータ](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/query-infrastructure-dimensional-metrics-nrql)
      ...
      このデータの詳細については、[`Metric`データ型](/docs/telemetry-data-platform/ingest-manage-data/understand-data/metric-data-type)についてをお読みください。このデータの取得および格納方法の詳細については、[メトリックAPIのドキュメント](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api)を参照してください。クエリのヒントについては、[メトリクスのクエリ例](/docs/data-ingest-apis/get-data-new-relic/metric-api/view-query-you-metric-data)を参照してください。
  </Collapser>
      ...
      <CollapserGroup>
        ...
        >
          ...
          **ActiveRecord**
          ...
          ActiveRecordは、Ruby on Railsアプリケーションが使用するオブジェクトリレーショナルマッピングAPIです。ここに示されたメトリクスは、ActiveRecordの`find`および`save`方法のパフォーマンスを測定します。
