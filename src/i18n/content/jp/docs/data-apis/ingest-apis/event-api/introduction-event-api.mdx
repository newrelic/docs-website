...
お客様の[リージョン](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)に対応する[CIDR範囲](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure)へのTCPポート443を用いたアウトバウンド通信を許可してください。推奨される設定方法は、DNS名`insights-collector.newrelic.com`（US（デフォルト））、`insights-collector.eu01.nr-data.net`（EU）、または`insights-collector.jp.nr-data.net`（JP）を使用することです。
