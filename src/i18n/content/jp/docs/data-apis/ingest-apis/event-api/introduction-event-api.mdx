...
お客様の リージョン に対応する[ CIDR範囲](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure) [](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)へのTCPポート443を用いたアウトバウンド通信を許可してください。推奨される設定方法は、DNS名`insights-collector.newrelic.com`（US、デフォルト）、`insights-collector.eu01.nr-data.net`（EU）、または`insights-collector.jp.nr-data.net`（JP）を使用することです。
...
<Callout variant="tip">
  お客様の組織のデータがEUまたはJPデータセンターでホストされている場合は、適切なリージョンのエンドポイントを使用していることを確認してください。[EUリージョンのエンドポイント](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#eu-endpoints)または[JPリージョンのエンドポイント](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#jp-endpoints)を参照してください。
</Callout>
