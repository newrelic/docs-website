...
TCPポート443のアウトバウンド接続が、[CIDR範囲](/docs/using-new-relic/cross-product-functions/install-configure/networks/#infrastructure)に一致する[地域](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)に使用できることを確認します。推奨される設定方法は、DNS名`metric-api.newrelic.com`（US、デフォルト）、`metric-api.eu.newrelic.com`（EU）、または`metric-api.jp.nr-data.net`（JP）を使用することです。
