...
Metric API で作成されたメトリクスでアラートを生成するには、 [NRQL アラート条件](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)を使用します。条件を定義するときに NRQL カテゴリを選択し、 `FROM Metric ...` NRQL クエリ構文を使用してそれを表現します。 これらのアラート条件を作成すると、 <InlinePopover type="alerts"/>は、利用可能な最も細かい粒度のデータ (生のメトリック データ ポイント) を自動的に使用して、アラートを評価します。
