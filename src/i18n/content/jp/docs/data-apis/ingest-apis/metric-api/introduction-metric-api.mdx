...
データを送信する New Relic アカウントには New Relic <InlinePopover type="licenseKey"/>が必要です。 制限と制限された属性の詳細については、 [「メトリクスAPI要件と制限」](/docs/metric-api-general-requirements-limits)を参照してください。
...
* <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT>から<DNT>**Explorer**</DNT>を選択し、サービスを探します。
...
メトリクスAPIで作成されたメトリクスを集計するには、 [NRQLアラート条件](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)を使用します。条件を定義するときにNRQLカテゴリを選択し、`FROM Metric ...` NRQLクエリ構文を使用して条件を表現します。 これらのアラート条件を作成すると、 <InlinePopover type="alerts"/>、利用可能な最も細かい粒度のデータ (生のメトリックス データ ポイント) を自動的に使用してアラートを評価します。
