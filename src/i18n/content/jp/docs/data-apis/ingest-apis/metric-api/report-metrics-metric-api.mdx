...
<Callout variant="tip">
  お客様の組織がEUまたはJPデータセンターでデータをホストする場合は、正しい地域のエンドポイントを使用していることを確認してください。このAPIの場合：

  * EUエンドポイント： `https://metric-api.eu.newrelic.com/metric/v1`
  * JPエンドポイント： `https://metric-api.jp.nr-data.net/metric/v1`
</Callout>
