...
2. ライセンスキーを以下のJSONに挿入し、そのJSONを当社の [エンドポイント](#api-endpoint) に送信します。
   ...
   この例では、 `memory.heap` という名前のメトリックに対する単一のメトリック・データ・ポイントを作成していますが、 [メトリック・タイプ](#supported-metric-types) を指定したり、 [オプション `共通` ブロック](#optional-map-attributes) を追加したりすることで、追加の属性やデータ・ポイントを作成することができます。
