...
メトリクスAPIの制限は、個別のアカウントレベルで適用されます。デフォルトのカーディナリティ制限は、[無料エディション](https://newrelic.com/pricing)の組織向けの3Mから、一部の有料組織向けの最大10.2Mまでの範囲です。組織の制限を理解するには、[制限 UI](/docs/data-apis/manage-data/view-system-limits)を参照してください。有料の組織は、リクエストに応じてカーディナリティを15Mまで増やすことができます。1分あたりの最大ペイロード数は、個別に100k以上に調整できます。メトリクスレート制限の変更をご希望の場合は、New Relicのアカウント担当者にご連絡いただくか、[サポートポータル](http://support.newrelic.com/)をご覧ください。
