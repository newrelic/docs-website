...
API制限は、個々のアカウント レベルで適用されます。 DPM とカーディナリティのデフォルトの制限は、[無料版](https://newrelic.com/pricing)の組織の場合は 3M、一部の有料組織の場合は最大 10.2M です。組織の制限を理解するには、[制限 UI を](/docs/data-apis/manage-data/view-system-limits)参照してください。有料組織の場合、リクエストに応じて DPM とカーディナリティを 15M まで増やすことができます。1 分あたりの最大ペイロードは、ケースバイケースで 100k 以上に調整できます。メトリクスのレート制限の変更をリクエストするには、 New Relicアカウント担当者に連絡するか、[サポート ポータル](http://support.newrelic.com/)にアクセスしてください。
