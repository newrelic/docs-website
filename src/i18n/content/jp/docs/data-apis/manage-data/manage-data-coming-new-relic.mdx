...
データ取り込みのコストを見積もる場合は、データ取り込みの[見積もり](/docs/data-apis/manage-data/calculate-data-ingest)を参照してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        一般的な`Mobile`イベント、 `MobileRequestError` 、 `MobileBreadcrumb` 、 `MobileSession` 、 `MobileHandledException` 、 `MobileCrash`を含む[モバイルイベント](/docs/insights/insights-data-sources/default-data/mobile-default-events-insights)。
