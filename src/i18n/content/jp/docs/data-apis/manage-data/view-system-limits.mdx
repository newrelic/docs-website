...
import limitsGraph from 'images/limits-graph.png';

import limitsTable from 'images/limits-table.png';
...
[制限は、子アカウントごと、および API 全体に適用されます。トラフィック量の多いアプリケーションを新たに監視し始めた場合や、データが急激に増加した場合などに制限に達する可能性があります。制限に達した場合、New Relic はデータの種類と、達した制限に応じて応答します。例えば、以下のようになります。](/docs/licenses/license-information/general-usage-licenses/new-relic-data-usage-limits-policies)

- [\* 当社では、データタイプごとに 1 分間の取り込みリクエスト数（RPM）に制限を設けています。この制限に達した場合、データの受け入れを停止し、その分だけ 429 のステータスコードを返します。](/docs/licenses/license-information/general-usage-licenses/new-relic-data-usage-limits-policies)
...
- メトリクスについては、アカウントごと、メトリクスごとに、ユニークな時系列の数（カーディナリティ）に制限を設けています。この制限に達した場合、集約されたデータは UTC 日の残りの時間はオフになります。
...
システム **Limits** ページ（ [account dropdown](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown) から、 `Manage your data` をクリックし、左側の `Limits` をクリック）では、お客様のアカウントが指定された期間にレート制限に遭遇した場合に表示されます。ページにはデフォルトで 24 時間が表示されますが、ページの右上からカスタム範囲を設定することができます。
...
<img
  src={limitsGraph}
  title="limits-graph.png"
  alt="リミットグラフのスクリーンショット"
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > [account
  dropdown](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown)
  > Manage your data> Limits**: **Limits**
  UIページで、カーディナリティ違反の制限問題を表示したチャートの例。
</figcaption>

より詳細な情報を追加したり、ダッシュボードを構築するには、グラフ上の **View NRQL** ボタンをクリックすると、このビューを支える NRQL が表示されます。グラフには、選択した期間中に到達した固有のリミットタイプがそれぞれ表示されます。これは、時間に基づく傾向を見つけるのに役立ちます。

**Limits** ページには、リミット名、それに関連するリミットイベントメッセージ、最後に発生した日時を確認できる表があります。表の中のリミットをクリックすると、いつ、何が起こったのかを詳しく見ることができます。

<img
  src={limitsTable}
  title="limits-table.png"
  alt="制限メッセージテーブルのスクリーンショット"
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > [account
  dropdown](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown)
  > Manage your data> Limits**: **Limits** UIページのLimit eventsテーブルの例。
</figcaption>
...
当社のインジェスト API には、より一般的な [アカウントレベルの制限を上書きする可能性のある追加の制限があります](#all_products) 。これらの制限は、これらの API を使用する当社のツールにも適用されることに注意してください（当社の [Telemetry SDKs](/docs/telemetry-data-platform/get-data-new-relic/getting-started/introduction-new-relic-data-ingest-apis-sdks#telemetry-sdk) または当社の [オープンソース Telemetry integrations](/docs/integrations/open-source-telemetry-integrations) など）。

- [メトリック API](/docs/telemetry-data-platform/get-data/apis/metric-api-limits-restricted-attributes) （次元的な測定基準）
- [イベントの API](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/introduction-event-api)
- [ログの API](/docs/logs/log-management/log-api/introduction-log-api)
- [トレースの API](/docs/understand-dependencies/distributed-tracing/trace-api/trace-api-general-requirements-limits)
