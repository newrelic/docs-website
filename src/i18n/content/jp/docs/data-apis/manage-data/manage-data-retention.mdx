...
<Callout variant="tip">
  [Data Plus オプション](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-plus)を使用すると、追加の 90 日間の保存期間と、より長いクエリ期間、FedRAMP および HIPAA コンプライアンス、追加のセキュリティ機能など、その他のエンタープライズ グレードの機能を利用できます。
</Callout>
...
データ保持を構成できるようにするには、Pro または Enterprise エディションが必要であり、データを[米国のデータ センター リージョン](/docs/accounts/accounts-billing/account-setup/choose-your-data-center)に置く必要があります。これらおよびその他のいくつかのアカウントの特性により、一部の New Relic 組織は保持を独自に調整することができなくなります。Pro または Enterprise 組織であり、米国のデータセンターを使用していて、保持を編集できない場合は、New Relic アカウント担当者にお問い合わせください。

[Data Plus オプション](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data-plus)では、さらに 90 日間の保持期間が提供されることに注意してください。

保持を表示および編集するための権限関連の要件:

* [新しいユーザー モデル](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models):

  * より高いデータ保持契約に変更するには、 [**課金**管理設定](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings)が必要です
  * 契約の範囲内で保持設定を編集するには、 [**Insights イベント保持**機能](/docs/accounts/accounts-billing/new-relic-one-user-management/user-capabilities/#data-retention)が必要です。
...
* データ プラスまたは元のデータ オプションの場合: データ保持を追加します。組織の 30 日間の追加データ保持には、30 日ごとに取り込まれた GB あたり 0.05 USD の費用がかかります。
