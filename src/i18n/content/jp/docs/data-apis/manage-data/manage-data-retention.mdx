...
New Relic の**データ保持**UI ページを使用すると、さまざまな種類のデータが保存される期間を確認および管理できます。機密情報を保護し、クエリをより速く返すためにデータを短期間保持するか、長期間にわたってデータと傾向を表示して分析できるように長期間保持します。
...
<figcaption>
  \[**データ保持**UI] ページから、\[**設定の編集**] をクリックして、各ソースのデータ保持を調整できます。
</figcaption>
...
デフォルトの期間を超えてデータ保持を延長すると、 [30 日あたり $0.05 の費用がかかり](/docs/licenses/license-information/usage-plans/new-relic-one-usage-plan-descriptions/#list-price)ます。
...
[要件を満たしている](#requirements)場合は、**データ保持**UI にリストされているデータ タイプの保持期間を増やすことができます。そのためには、そのページで \[**保持を調整**] をクリックします。

リテンション率の向上に関する重要な注意事項:

* **データ保持**UI にリストされているすべてのデータ型について、保持が全体的に増加します。特定のデータ型については増やすことはできません。
* 追加の 30 日間の保存期間ごとに、1 か月に取り込まれた GB あたり 0.05 USD のコストがかかります。
* 保持期間を長く設定すると、特定のデータ ソースの保持期間を短縮しても請求には影響しません。

### リテンションの削減に関するその他の注意事項 [#reduce-retention]

特定のデータ ソースの保持期間を短縮しても、その種類の既存のデータはすぐには消えません。そのデータは、最大数週間表示され、クエリで返される場合があります。誤ってデータを削除しないようにするための保護手段が用意されていますが、これは、[データをすばやく削除する](#data-deletion)ために保持値を減らすことは適切な戦略ではないことも意味します。

リテンションを減らすと、12 時間以内に有効になります。有効になると、そのタイプの新しいデータは新しい保持期間の間保持されます。
