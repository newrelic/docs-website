...
最近まで、請求の変更や契約の更新なしで、契約上の制限を超えてデータ保持を調整できるようにしてきました。3 月 31 日以降、データ保持が制限を超えると、ダッシュボード、データ分析、監査、およびコンプライアンスのニーズでの使用を含め、このデータにアクセスできなくなります。

契約の制限を超えてデータ保持を編集すると、延長された保持を反映するために、新しい請求の変更で保持をアップグレードするように求められます。長期的なデータ分析、ガバナンス、およびコンプライアンスのために、保持期間を延長することはビジネスに不可欠です。詳細については、以下を参照してください。
