...
データ保持期間ページには、次の2つのタブがあります。

* <DNT>**Account**</DNT>：アカウントスコープのデータの保持期間を管理します。
* <DNT>**Organization**</DNT>：組織スコープのデータ（AIチャットやコラボレーションデータなど）の保持期間を管理します。
...
## アカウントスコープと組織スコープの保持期間 [#account-vs-organization]

データ保持期間UIには、異なるスコープの保持期間を管理するための2つのタブがあります。

### Accountタブ

<DNT>**Account**</DNT>タブでは、ほとんどのNew Relicのデータ型の保持期間を管理します。Accountタブを使用する場合、アカウントピッカーで選択したアカウントに応じて、保持期間の設定を表示・変更する対象のアカウントが決まります。

### Organizationタブ

<DNT>**Organization**</DNT>タブでは、組織レベルで保存されているデータの保持期間を管理します。現在、これにはAIチャットの概要とコラボレーション関連のコミュニケーションスレッドが含まれます。

組織スコープの保持期間の主な特徴：

* 組織内のすべてのアカウントに適用されます。
* アカウントピッカーの選択は、この設定に影響しません。
* 基本のデータ保持期間は30日間です。
* 組織管理者ロールを持つユーザーのみが、変更を編集・保存できます。
* データは別のデータベースで管理されます。

<Callout variant="important">
  <DNT>**Organization**</DNT>タブを表示している場合、アカウントピッカーで選択したアカウントは、表示される内容や行う変更に影響しません。組織スコープの保持設定は、選択したアカウントに関係なく組織全体に適用されます。
</Callout>
...
### 組織スコープの保持要件 [#organization-requirements]

<DNT>**Organization**</DNT>タブでの保持期間の管理には以下の要件が適用されます。

* **編集できるユーザー：**組織管理者ロールを持つユーザーのみが、組織スコープの保持設定を編集・保存できます。
* **閲覧できるユーザー：**すべてのユーザーが組織スコープの保持設定を閲覧できます。
* **アカウントピッカー：**アカウントピッカーで選択されたアカウントは、組織スコープの保持設定に影響しません。この設定は組織全体に適用されます。

アクセス権があるはずなのに、組織スコープの保持期間を編集できない場合は、組織のアドミニストレーターまたはNew Relicの担当者にお問い合わせください。
...
<CollapserGroup>
  ...
  <Collapser id="apm-namespaces" title="APMのネームスペース">
    各APMネームスペースは、特定のイベントタイプにマッピングされます。このマッピングを理解することで、各イベントにどの保持期間が適用されるかが明確になります — たとえば、`ErrorTrace`、`SqlTrace`、および`TransactionTrace`は、ディストリビューティッド（分散）トレーシングのネームスペースには保存され**ません**。

    * `APM` ネームスペース：[`Transaction`](/attribute-dictionary/?event=Transaction)、[`TransactionError`](/attribute-dictionary/?event=TransactionError)
    * `APM errors` ネームスペース：[`ErrorTrace`](/attribute-dictionary/?event=ErrorTrace)
    * `APM error stack traces` ネームスペース：`ErrorTrace`イベントに関連付けられたスタックトレースデータ
    * `Transaction traces` ネームスペース：[`TransactionTrace`](/attribute-dictionary/?event=TransactionTrace)、[`SqlTrace`](/attribute-dictionary/?event=SqlTrace)

    `Span`イベント（[ディストリビューティッド（分散）トレーシング](/docs/distributed-tracing/concepts/introduction-distributed-tracing/)で使用）は、独自の保持期間を持つ別の**ディストリビューティッド（分散）トレーシング**ネームスペースに保存されることに注意してください。

    APMイベントの詳細については、[APMが報告するイベント](/docs/data-apis/understand-data/event-data/events-reported-apm/)を参照してください。
  </Collapser>

  <Collapser id="browser-namespaces" title="ブラウザのネームスペース">
    すべてのブラウザネームスペースのデフォルトの保持期間は同じです。各ブラウザネームスペースでのイベントの詳細は次のとおりです。

    * `Browser` ネームスペース： `BrowserPerformance` 、 `PageAction` 、 `PageView` 、 `UserAction`
    * `Browser events` ネームスペース：`AjaxRequest`、`BrowserInteraction`、 `PageViewTiming`
