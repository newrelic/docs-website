...
<CollapserGroup>
  ...
  >
    [メトリック タイムスライス データ](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data) は、 <InlinePopover type="apm"/>(カスタム メトリックとデフォルト メトリックの両方)、モバイル、およびブラウザー エージェント。 メトリック タイムスライスは、メトリック名、メトリックが表す時間のセグメント (「タイムスライス」)、および数値 (測定値) の 3 つの部分で構成されます。APM、ブラウザー、およびモバイル エージェントは、エラー率、帯域幅使用率、ガベージ コレクション時間などのさまざまなパフォーマンス メトリックについて、毎分数千のメトリック タイムスライスを収集できます。
  ...
  >
    ...
    * `Browser page view timing` 名前空間: `PageViewTiming`
    ...
    これらのイベントの詳細については、 [データ ディクショナリのブラウザ イベントを](https://docs.newrelic.com/attribute-dictionary/?dataSource=Browser+agent)参照してください。
  </Collapser>
