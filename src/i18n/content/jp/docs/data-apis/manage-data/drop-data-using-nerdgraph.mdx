...
* [ディメンション メトリック](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#dimensional-metrics) ( `Metric` データ型)。いくつかの注意事項:
...
* `DROP_ATTRIBUTES` 「raw」属性（ [集計関数](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#functions)が適用されていない属性）を持つ`SELECT`を使用する必要があります。 これは、 `SELECT *`を使用できないことも意味します。 さらに、データ型と

  <DNT>
    **cannot be dropped**
  </DNT>

  (イベント データの`timestamp`など) に不可欠な属性もいくつかあります。 これらを含めると登録は失敗します。
