...
<Callout variant="important">
  2026 年 2 月以降、 `SystemSample` 、 `ProcessSample` 、 `NetworkSample` 、 `StorageSample`のインフラストラクチャ イベントを対象とするドロップ ルールによって、集計データも正しくドロップされるようになります。
</Callout>
