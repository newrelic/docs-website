...
必要のないデータを取り込んでしまうこともあるかもしれません。 [一部のタイプのデータ](#requirements) をドロップすることができます。

* 重要ではない価値の低いデータを除外するために
* 潜在的にセンシティブなデータを除外するために

データの削除を選択した場合、新しいデータのみが影響を受けます。既存のデータ [の編集や削除はできません](/docs/telemetry-data-platform/ingest-manage-data/manage-data/manage-data-retention#data-deletion) 。詳しくは、NerdBytesのビデオ（7分9秒）をご覧ください。
...
<Callout variant="tip">
  Prometheusのリモートライトを使ってNew Relicにデータを送信している場合、YAML設定ファイルの `remote_write` セクションを変更することでもデータを落とすことができます。

  詳細については、 [Prometheus remote write を使ったデータのドロップ](/docs/integrations/prometheus-integrations/install-configure/remote-write-drop-data/) を参照してください。
</Callout>

## 要件

データをドロップする権限を持つユーザーロールが必要です。
...
* [デフォルトのインフラ監視イベント](/docs/infrastructure/manage-your-data/data-instrumentation/default-infrastructure-monitoring-data) および [インフラ統合](/docs/integrations/infrastructure-integrations/get-started/introduction-infrastructure-integrations) イベント、ただしこの例外を除く。
...
すべてのNRQL句がドロップルールの生成に意味を持つわけではありません。 [`WHERE`](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#sel-where) 句を用意して、特定の属性を持つデータを選択することができます。 `TIMESERIES`, `COMPARE WITH`, `FACET` などの他の機能を持つ節は使用できません。
