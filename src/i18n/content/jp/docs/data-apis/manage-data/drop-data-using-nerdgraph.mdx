...
[最初のクエリはメトリック・ロールアップを検索し、 `containerId` が新しいドロップ・ルールに従ってドロップされたため、0 に落ちるはずです。2つ目のクエリは、 `RAW` キーワードを使用してメトリックローを取得しますが、生データは新しいドロップルールの影響を受けないため、引き続き安定しているはずです。カーディナリティへの影響を確認する方法の詳細については、 ](/docs/data-apis/ingest-apis/metric-api/NRQL-high-cardinality-metrics/)[Understand and query high cardinality metrics](/docs/data-apis/ingest-apis/metric-api/NRQL-high-cardinality-metrics) を参照してください。
...
* 複雑な組織におけるデータインジェストの管理に関する高度なディープダイブについては、 [データガバナンスフレームワーク](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-intro/) をご覧ください。