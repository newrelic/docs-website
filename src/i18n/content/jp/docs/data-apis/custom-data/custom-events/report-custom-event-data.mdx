...
カスタム属性は、重要なビジネスおよび運用コンテキストを既存のイベントに追加するためによく使用されます。たとえば、New Relic の場合<InlinePopover type="browser"/> 、遅いリクエストまたは失敗したリクエストに関連付けられたユーザー名を追跡するカスタム属性を作成できます。これにより、そのデータを分析するためのクエリとカスタム グラフを作成できるようになります。

カスタム属性は、New Relic ソリューション (APM、ブラウザー、 <InlinePopover type="mobile"/>、インフラストラクチャ、合成モニタリングなど）、既存のイベントを独自のメタデータで装飾したいと考えています。
