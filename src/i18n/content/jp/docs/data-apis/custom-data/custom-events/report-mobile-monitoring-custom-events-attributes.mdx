...
デフォルトのカスタムセッションレベル属性を作成できます。 <InlinePopover type="mobile"/>モバイル エージェント SDK を使用したイベント。たとえば、iOS または Android アプリの一部の`username`属性を記録するには、[`setAttribute` API](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/create-attribute)を使用します。これらの属性はセッション関連の情報であり、複数のモバイル イベント タイプで共有されます。
