...
[Event API](/docs/insights/insights-data-sources/custom-data/introduction-event-api)を使用してカスタム イベントを送信することもできます (APM は必要ありません)。ただし、エージェント API で送信されるカスタム イベントは、[高セキュリティ モード](/docs/accounts-partnerships/accounts/security/high-security)と互換性がないことに注意してください。
...
独自のカスタム<InlinePopover type="apm" />イベントと[属性](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)を追加して、[クエリやチャート作成](/docs/using-new-relic/data/understand-data/query-new-relic-data)に使用することができます。 これは、[カスタム データを報告する](/docs/insights/insights-data-sources/custom-data/send-custom-data-insights)いくつかの方法のうちの 1 つです。
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="php-att" title="PHP">
    ...
    デフォルトでは、1 分あたりに記録されるカスタムイベントの最大数は 30,000 です。 [newrelic.custom\_events.max\_samples\_stored](/docs/apm/agents/php-agent/configuration/php-agent-configuration/#inivar-custom-events-maxsamples)構成設定を使用して、最大 100,000 まで増やすことができます。

    PHP アプリに[カスタム アトリビュート](/docs/apm/other-features/attributes/collecting-custom-attributes#php-att)を追加することもできます。
