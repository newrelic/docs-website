...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Python"
  >
    Pythonでは、カスタム属性の収集がデフォルトで有効になっています。カスタム属性を収集するには、属性を記録するメソッドごとに[`add_custom_parameter`](/docs/agents/python-agent/python-agent-api/add_custom_parameter)を呼び出します。
