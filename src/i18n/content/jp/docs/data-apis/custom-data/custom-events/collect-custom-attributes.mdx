...
<CollapserGroup>
  ...
  <Collapser id="ruby-att" title="Ruby">
    Ruby では、カスタム属性の収集がデフォルトで有効になっています。カスタム属性を収集するには、 [`add_custom_attributes`](https://www.rubydoc.info/gems/newrelic_rpm/NewRelic/Agent#add_custom_attributes-instance_method)メソッドを呼び出します。たとえば、 `@user_id`という名前の変数を記録するには、次のコードを親メソッドに含めます。
