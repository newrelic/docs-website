...
3. [Limited on unique metric-name/attribute-value combinations](#attributes-limit) に従って、メトリックに添付する属性を決定します。
   ...
   <DoNotTranslate>**Recommendation:**</DoNotTranslate> 別のクエリを実行して、30 秒間の最大カーディナリティが 50,000 を超えないことを確認します。 たとえば、次のクエリは、 `awsRegion` 、 `awsAvailabilityZone` 、および`commandName`属性を含めた場合、 `ProcessSample`イベントの過去 3 時間の 30 秒間に発生した最大カーディナリティを検索します。
