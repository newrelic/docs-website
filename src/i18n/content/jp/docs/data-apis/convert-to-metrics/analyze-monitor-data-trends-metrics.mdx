...
* <DNT>**Events, logs, spans:**</DNT>
  ...
  これらのタイプのデータは、特定の瞬間における単一のレコードを表します。 たとえば、システムへのリクエストごとにイベントが発生する場合があります。 このデータは、詳細なトラブルシューティングと分析に最適です。

* <DNT>**Metrics:**</DNT>

  これらは、イベント、ログ、またはスパンの集約ビューを提供します。 メトリックスは、より長い時間範囲にわたる傾向を示すのに適しています。 たとえば、サービスごとのリクエストの合計数を 1 つのメトリックに集計し、この情報を月ごとに調べることができます。
...
* <DNT>**Mutations**</DNT>
  ...
  既存のルールや設定を変更する操作（たとえば、新しいメトリクス ルールの作成）です。

* <DNT>**Queries**</DNT>

  既存のデータを取得します（たとえば、既存のメトリクス ルールを取得します）。
...
2. <DNT>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**</DNT>

   に移動して、クエリをボックスに貼り付けます。

3. 操作を実行するには、赤い

   <DNT>**Execute query**</DNT>

   ボタンをクリックします。 または、curl 形式を取得するには、
