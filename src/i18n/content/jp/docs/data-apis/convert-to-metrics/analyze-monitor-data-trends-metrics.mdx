...
<CollapserGroup>
  ...
  >
    ...
    **New Relic アカウント `123456`のリスト ルール `36` の例:**
...
2. **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer** に移動し、ボックスにクエリを貼り付けます。
3. 操作を実行するには、赤い **Execute query** \[クエリの実行] ボタンをクリックします。または、curl 形式を取得するには、 **Tools > Copy as CURL**を選択します。
