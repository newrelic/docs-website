...
高品質な運用環境を維持するためにできる最も重要なことの 1 つは、劣悪なユーザー エクスペリエンスを検出して解決するために必要な Web テレメトリを確保することです。このガイドでは、最適化に必要なデータを確実に取得できるようにする方法について説明します。 <InlinePopover type="browser"/>。私たちはあなたが次のことを確実に行えるようお手伝いします。
