...
<Steps>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        <DoNotTranslate>**Browser**</DoNotTranslate>で Web アプリケーションを開きます。 左側のメニューの下にある<DoNotTranslate>**Errors**</DoNotTranslate>ビューを開き、JavaScript エラーが表示されることを確認します。 アプリケーションのトラフィックがそれほど多くない場合は、エラーを確認するために 24 時間以上前までさかのぼる必要がある場合があります。<img src={omaOeScBrowserErrors} alt="Browser JavaScript Errors" title="Browser JavaScript Errors"/>
      </Collapser>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        イベント ログをキャプチャしているかどうかを確認するには、 <DoNotTranslate>**JS Errors**</DoNotTranslate>タブに移動します。 いくつかの異なるエラーをチェックして、イベント ログが表示されることを確認します。<img src={omaOeScBrowserErrorLog} alt="Browser JavaScript Error Logs" title="Browser JavaScript Error Logs"/>
      </Collapser>
    </CollapserGroup>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        <DoNotTranslate>**JS Errors**</DoNotTranslate>タブでいくつかのエラーを確認してください。 エラーイベントログの下にスタックトレースが表示されます。
      </Collapser>
  ...
  <Step>
    ...
    次に、ページビューのグループ化を確認します。 <DoNotTranslate>**Page views**</DoNotTranslate> UI のページ URL は自動的にグループ化され、ページのパフォーマンスをより適切に管理できるようになります。 自動グループ化を決定するアルゴリズムは、Web アプリが初めてインストゥルメントされたときに実行されます。 現在の Web トラフィックが、アプリが最初にインストゥルメントされたときと大きく異なる場合は、表示されるグループが少なすぎる可能性があります。
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        左側のメニューから選択して、アプリの<DoNotTranslate>**Page views**</DoNotTranslate> UI を確認します。 表示される内容が以下のスクリーンショットとよく似ている場合は、メモを取り、このガイドの指示に従って対処してください。<img src={omaOeScBrowserGroupedPageViews} alt="Browser Page URL Grouping" title="Browser Page URL Grouping"/>
      </Collapser>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        左側のメニューからアプリを選択し、 <DoNotTranslate>**groupedRequestUrl**</DoNotTranslate>でグループ化して、アプリの AJAX グループ化を確認します。 表示される内容が以下のスクリーンショットとよく似ている場合は、メモを取り、このガイドの指示に従って対処してください。<img src={omaOeScBrowserGroupedAjaxRequests} alt="AJAX Grouping" title="AJAX Grouping"/>
      </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Cost model:**</DoNotTranslate> 貴社の開発者 FTE モデルとプロジェクト見積もりによって異なります。

    <DoNotTranslate>**Estimation:**</DoNotTranslate> 通常、開発者がNew Relicインストゥルメンテーション機能を使いこなせるようになるには数時間かかります。
  ...
  >
    ...
    <DoNotTranslate>**Estimation:**</DoNotTranslate> これは、プロジェクトの範囲と必要なインストゥルメンテーション作業の量によって決まる傾向があります。
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**KPI:**</DoNotTranslate> サービス品質（ビジネスKPI）
