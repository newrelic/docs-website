...
<Steps>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        イベント ログをキャプチャしていることを確認するには、 **JS Errors** \[JS エラー]タブに移動します。いくつかの異なるエラーをチェックして、イベント ログが表示されることを確認します。<img src={omaOeScBrowserErrorLog} alt="Browser JavaScript Error Logs" title="Browser JavaScript Error Logs"/>
      </Collapser>
    </CollapserGroup>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        左側のメニューからアプリを選択して、アプリの**Page views** ページ ビューUI を確認します。表示されている内容が以下のスクリーンショットとよく似ている場合は、メモを取り、このガイドの指示に従って対処してください。<img src={omaOeScBrowserGroupedPageViews} alt="Browser Page URL Grouping" title="Browser Page URL Grouping"/>
      </Collapser>
  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="UI経由"
      >
        左側のメニューからアプリを選択し、「 _groupedRequestUrl_ 」でグループ化して、アプリの AJAX グループ化を確認します。表示されている内容が以下のスクリーンショットとよく似ている場合は、メモを取り、このガイドの指示に従って対処してください。<img src={omaOeScBrowserGroupedAjaxRequests} alt="AJAX Grouping" title="AJAX Grouping"/>
      </Collapser>
  ...
  <Step>
    ...
    次に、 [Web アプリケーションの変更を追跡する](/docs/change-tracking/change-tracking-graphql/)ための NerdGraph を基礎にして、パフォーマンス KPI、コンバージョン、ユーザー エンゲージメントに対する変更の影響を確認できるようにします。  
