...
{/* &lt;Calloutariant=&quot; important&quot;&gt; \*\*始める前に\*\*: \[NGINX OpenTelemetry概要]\(/docs/OpenTelemetry/統合/nginx/nginx-otel-overview/) を参照して、監視対象とこの統合の利点を理解してください。 \*\*その他の NGINX 監視オプション:\*\* - \*\*NGINX Plus\*\* を監視するには、\[ OpenTelemetryで NGINX Plus を監視する]\(/docs/OpenTelemetry/インテグレーション/nginx-plus/nginx-plus-otel/) を参照してください。 - \*\* Kubernetes上の NGINX を監視するには \*\*、\[ OpenTelemetryを使用してKubernetes上の NGINX を監視する]\(/docs/OpenTelemetry/インテグレーション/nginx/nginx-otel- Kubernetes /) を参照してください。 &lt;/Callout&gt; &lt;Callout variant=&quot;tip&quot;&gt; \*\*セットアップ時間\*\*: 約 20 分 | \*\*スキルレベル\*\*: 中級 (Linux/NGINX の基本的な知識が必要) &lt;/Callout&gt; */}
...
<CollapserGroup>
  ...
  <Collapser id="step4-verify-data" title="データ収集を確認する">
    ...
    5. 初期データが New Relic に表示されるまで待ってから、 [NGINX ダッシュボードにアクセスして](/docs/opentelemetry/integrations/nginx/find-and-query-your-data#find-data)データ収集を確認します。
  </Collapser>
</CollapserGroup>

## ステップ 5: (オプション) NGINX ログを転送する [#forward-logs]

メトリクスに加えて、NGINX の[アクセスとエラー ログを](https://docs.nginx.com/nginx/admin-guide/monitoring/logging/)New Relicに送信して、包括的な監視とトラブルシューティングを行うことができます。 これらのログは[コア NGINX メトリクス](/docs/opentelemetry/integrations/nginx/nginx-otel-metrics-reference#core-metrics)を補完し、詳細なリクエスト レベルのインサイトを提供します。
