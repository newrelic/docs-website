...
セルフホスト型NGINXサーバーをOpenTelemetryでモニターし、パフォーマンスメトリクスとテレメトリーデータをNew Relicに送信します。
...
* **Prometheus Receiver:** すでに[nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter)を実行している環境向け
...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="prometheus">
      環境ですでに[nginx-prometheus-exporter](https://github.com/nginxinc/nginx-prometheus-exporter)を実行している場合、またはPrometheusベースの監視スタックから移行する場合は、このアプローチを使用します。

      <Callout variant="tip">
        **推奨:** Prometheusエクスポーターをまだ実行していない場合は、代わりに**NRDOTコレクター**または**OpenTelemetry Collector Contrib**タブを使用してください。これらは、追加のエクスポーターコンポーネントを必要とせずに、NGINXスタブステータスエンドポイントに直接接続します。
      </Callout>

      <CollapserGroup>
        <Collapser id="configure-prometheus-host" title="ステップ 1：コレクターを設定する">
          この設定は、**NRDOT**と**OTel Collector Contrib**の両方で機能します。適切な設定場所に配置します：

          * **NRDOT:** `/etc/nrdot-collector/config.yaml`
          * **OTel Collector Contrib：** `/etc/otelcol-contrib/config.yaml`

          <Callout variant="tip">
            **変更を加える前に、既存の設定をバックアップします：**
          ...
          <Callout variant="important">
            `<YOUR_STUB_STATUS_ENDPOINT>`をNGINXスタブステータスURL (例: `http://127.0.0.1:8080/nginx_status`) に、`<DEPLOYMENT_NAME>`をこのNGINXサーバーの一意の名前に置き換えます。
          </Callout>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  nginx-prometheus-exporterのアドレス (デフォルトは

                  `localhost:9113`

                  )。異なるホストまたはポートで実行している場合は更新します。
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  エクスポーターからメトリクスをスクレイピングする頻度。デフォルト：
        ...
        <Collapser id="env-vars-prometheus-host" title="ステップ 2：環境変数を設定する">
          ...
          `YOUR_NEW_RELIC_LICENSE_KEY` <InlinePopover type="licenseKey" />に置き換えます。

          <Callout variant="tip">
            EUアカウントの場合は、`OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.eu01.nr-data.net:4318`を使用します。リージョンエンドポイントの完全なリストについては、[エンドポイント、ポート、およびプロトコルの設定](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)を参照してください。
          </Callout>
        </Collapser>

        <Collapser id="start-prometheus-host" title="ステップ 3：コレクターを起動する">
          設定を適用し、コレクターを再起動します。`<collector-service>`を`nrdot-collector`または`otelcol-contrib`に置き換えます：
          ...
          期待される出力：エラーなしの`Active: active (running)`。
        </Collapser>

        <Collapser id="verify-prometheus-host" title="ステップ 4：New Relicでデータを確認する">
          数分後、このNRQLクエリを実行して、データが流れていることを確認します。`<DEPLOYMENT_NAME>`を設定で指定した値に置き換えます：
