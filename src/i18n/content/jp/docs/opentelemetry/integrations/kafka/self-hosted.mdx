    ...
    <TabsPageItem id="java-agent">
      ...
      <Steps>
        ...
        <Step>
          ...
          | `NEW_RELIC_OTLP_ENDPOINT`          | OTLP取り込みエンドポイント。USリージョン（デフォルト）には`https://otlp.nr-data.net:4317`、EUリージョンには`https://otlp.eu01.nr-data.net:4317`、JPリージョンには`https://otlp.jp.nr-data.net:4317`を使用します。その他の設定については、[OTLPエンドポイントの設定](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)を参照してください。 |
        </Step>
