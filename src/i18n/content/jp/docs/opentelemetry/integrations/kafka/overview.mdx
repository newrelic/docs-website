...
* **JMX メトリクス コレクション**(ブローカーのパフォーマンス、 JVMデータ、運用上のインサイト):

  * **セルフホスト型 Kafka** : カスタム JMX 設定を使用した[OpenTelemetry Javaエージェントは、](https://github.com/open-telemetry/opentelemetry-java-instrumentation) Kafka ブローカーに接続されたJavaエージェントとして実行され、OTLP 経由でコレクターの[OTLP レシーバー](https://github.com/open-telemetry/opentelemetry-collector/tree/main/receiver/otlpreceiver)にメトリクスを送信します。
  * **Kubernetes (Strimzi)** : New Relicカスタム設定を使用した[Prometheus JMX Exporter は、](https://github.com/prometheus/jmx_exporter)コレクターの[Prometheus レシーバー](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/prometheusreceiver)によってスクレイピングされたメトリクスをポート 9404 で公開します
