...
このページには、 Apache Kafka をOpenTelemetryで監視したときに収集されたメトリクスが文書化されています。 メトリクスは[、Kafka メトリクス レシーバー](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver)、 [JMX レシーバー](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver)、およびクライアント側メトリクスの[OpenTelemetry Javaエージェント](https://opentelemetry.io/docs/languages/java/automatic/)を介して収集されます。

## メトリクスの収集方法 [#collection-methods]

OpenTelemetry Kafka 監視では、2 つの補完的な受信機を使用します。
...
* **[JMX レシーバー](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver)**: JMX ポート (通常は 9999) に接続して、詳細なブローカー メトリクスおよびJVMメトリクスを収集します。
...
## JMX レシーバー メトリクス [#jmx-metrics]

JMX レシーバーは、JMX (通常はポート 9999) を介して Kafka ブローカー MBean から詳細なメトリクスを収集します。 メトリクスは 2 つの設定を使用して収集されます。

1. **デフォルトの Kafka 目標システム**- `target_system: kafka`の組み込み Kafka 固有メトリクス ([kafka.yaml](https://github.com/open-telemetry/opentelemetry-java-contrib/blob/main/jmx-scraper/src/main/resources/kafka.yaml))
2. **カスタム JMX メトリクス**- カスタム設定で定義された追加の Kafka メトリクスとJVMメトリクス

### デフォルトの Kafka ターゲット システム メトリクス

これらのメトリクスは、 `target_system: kafka`を使用すると自動的に収集されます。

<CollapserGroup>
  <Collapser id="jmx-cluster-metrics" title="クラスターレベルのメトリクス">
  ...
  <Collapser id="jmx-broker-metrics" title="ブローカーメトリクス">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
...
これらの追加の Kafka メトリクスとJVMメトリクスは、カスタム JMX 設定ファイルを使用するときに収集されます ([セルフホスト型セットアップ](/docs/opentelemetry/integrations/kafka/self-hosted#jmx-config)に示されているように)。 カスタム設定を使用すると、デフォルトのセットを超えた追加の Kafka 固有のメトリクスや、 JVMヘルス メトリクスを収集できます。

<CollapserGroup>
  <Collapser id="jmx-cluster-level-metrics" title="クラスターレベルのメトリクス">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-broker-specific-metrics" title="ブローカー固有のメトリクス">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
  ...
  <Collapser id="jmx-per-topic-metrics" title="トピックごとのメトリクス">
    <table>
      <thead>
        <tr>
          <th style={{ width: "350px" }}>
