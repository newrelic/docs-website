...
このページには、 Apache Kafka をOpenTelemetryで監視したときに収集されたメトリクスが文書化されています。 メトリクスは、[Kafka メトリクス レシーバー](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver)、JMX メトリクス (OpenTelemetry Javaエージェントまたは Prometheus JMX Exporter から)、および[OpenTelemetry Javaエージェント](https://opentelemetry.io/docs/languages/java/automatic/)のクライアント側メトリクスを介して収集されます。

## メトリクスの収集方法 [#collection-methods]

OpenTelemetry Kafka 監視 では、補完的な 2 つの収集方法が使用されます。
...
* **JMX メトリクス**: 詳細なブローカー メトリクスおよびJVMメトリクスを収集するために、 OpenTelemetry Javaエージェント (自己ホスト型) または Prometheus JMX Exporter (Kubernetes) を介して収集されます。
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            パーティションの同期レプリカの数。このメトリクスはコレクターパイプラインによって除外されます — トピックごとに集約された`kafka.partition.replicas_in_sync.total`のみが保持されます。
          </td>
...
## JMXメトリクス [#jmx-metrics]

JMX メトリクスは、詳細な Kafka ブローカーとJVMテレメトリーを提供します。 これらのメトリクスは以下を使用して収集されます。

* **セルフホスト型Kafka**: [OpenTelemetry Javaエージェント](/docs/opentelemetry/integrations/kafka/self-hosted#java-agent)または[Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/self-hosted#prometheus)
* **Kubernetes（セルフマネージド）**: [OpenTelemetry Javaエージェント](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#java-agent)または[Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-self-managed#prometheus)
* **Kubernetes (Strimzi)** : [Prometheus JMX Exporter](/docs/opentelemetry/integrations/kafka/kubernetes-strimzi#configure-jmx-exporter)とNew Relicカスタム設定

どちらの方法でも、以下に記載されている Kafka ブローカーとJVMの同じセットを収集します。

<CollapserGroup>
  <Collapser id="jmx-kafka-metrics" title="Kafka ブローカーメトリクス">
    これらのメトリクスは、クラスターの健全性、ブローカーのパフォーマンス、トピックごとのメトリクスを含む、包括的な Kafka ブローカー テレメトリーを提供します。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            リーダー選出回数。リーダー選出が発生した場合にのみ表示されます；安定したクラスタでは出力されません。
          </td>
...
## Kafka クライアントメトリクス [#kafka-client-metrics]
...
<CollapserGroup>
  <Collapser id="producer-metrics" title="プロデューサーメトリクス">
    ...
    | `kafka.producer.incoming_byte_rate`        | 受信バイトレート（ブローカーからの応答）   | クライアントID、ノードID |
    | `kafka.producer.incoming_byte_total`       | 総受信バイト数（ブローカーからのレスポンス） | クライアントID、ノードID |
  ...
  <Collapser id="consumer-metrics" title="消費者メトリクス">
    ...
    | `kafka.consumer.partition_assigned_latency_avg` | 平均パーティション割り当てレイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。 | クライアントID |
    | `kafka.consumer.partition_assigned_latency_max` | 最大パーティション割り当てレイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。 | クライアントID |
    | `kafka.consumer.partition_revoked_latency_avg`  | 平均パーティション取り消しレイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。 | クライアントID |
    | `kafka.consumer.partition_revoked_latency_max`  | 最大パーティション取り消しレイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。 | クライアントID |
    | `kafka.consumer.partition_lost_latency_avg`     | 平均パーティション損失レイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。   | クライアントID |
    | `kafka.consumer.partition_lost_latency_max`     | 最大パーティション損失レイテンシ（ms）。消費者グループのリバランス中にのみ発行されます。   | クライアントID |
...
* [データを書いて視覚化する](/docs/opentelemetry/integrations/kafka/find-and-query-data)- New Relic UIでメトリクスを見つけ、 NRQL書き込みを書き、ダッシュボードを作成し、アラートをセットアップします
* [書きメトリクス データ型](/docs/data-apis/understand-data/metric-data/query-metric-data-type/)- OpenTelemetryを操作するための高度なテクニックを学びます