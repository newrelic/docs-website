          ...
          <CollapserGroup>
            <Collapser id="helm-nrdot" title="NRDOT Collector の使用 (推奨)">
              ...
              **設定パラメーター**
              ...
              | `resources.limits` および `resources.requests`                                                                | Adjust according to your workload needs                                                                   |
            </Collapser>
    ...
    <CollapserGroup>
      <Collapser id="configure-log-collection-strimzi" title="ログ収集を構成する">
        ...
        **設定パラメーター**
        ...
        | Multiple broker receivers                             | ブローカーが複数ある場合は、それぞれのブローカー ID を持つ個別の`filelog`レシーバー (例: `filelog/kafka_broker_1` 、 `filelog/kafka_broker_2`) を作成します。                                                                                                                                                                                                                                   |
...
| ログ     | `Log`    | Logs from producer and consumer applications (via OTel Java agent) and broker logs collected via the optional log forwarding step |
