...
* JMX が有効になっている[Strimzi オペレーター](https://strimzi.io/)経由でデプロイされた Kafka。Kafka クラスターの Strimzi Kafka リソースで JMX が有効になっていることを確認します。
...
<Steps>
  ...
  <Step>
    ### ライセンスキーを使用してシークレットを作成する [#create-secret]
  ...
  <Step>
    ### OpenTelemetry Collector をデプロイする [#deploy-collector]

    #### 3.1 カスタムコレクターイメージの構築 [#build-image]

    Java ランタイムと JMX スクレーパーを使用してカスタム コレクター イメージを作成します。NRDOT Collector (New Relic ディストリビューション) または OpenTelemetry Collector を選択します。
      ...
      <TabsPages>
        <TabsPageItem id="nrdot">
          ...
          <Callout variant="important">
            **バージョンの互換性**: このガイドでは、JMX Scraper 1.52.0 と NRDOT Collector を使用します。最良の結果を得るには、最新バージョンを使用してください。

            <CollapserGroup>
              <Collapser id="nrdot-version-compatibility" title="最新のNRDOT Collectorバージョンを確認する">
                **最新バージョンを確認してください**:

                * NRDOT Collector: [NRDOT Collector リリース](https://github.com/newrelic/nrdot-collector-releases/releases/latest)にアクセスして最新バージョンを見つけ、Dockerfile 内の`{NRDOT_VERSION}`を置き換えてください
            ...
            **目標アーキテクチャー**: 以下の例では、 `linux_amd64`を使用します。 [NRDOT Collectorリリース](https://github.com/newrelic/nrdot-collector-releases/releases/latest)ページを参照して、システムアーキテクチャー (例: `linux_amd64` 、 `linux_arm64`) に適したバイナリを見つけてください。 それに応じて、Dockerfile のダウンロード URL を更新します。
          </Callout>
        ...
        <TabsPageItem id="otel">
          <Callout variant="important">
            **バージョンの互換性**: このガイドでは、JMX Scraper 1.52.0 と OpenTelemetry Collector 0.145.0 を使用します。古いコレクターのバージョンでは、このスクレーパーのハッシュが互換性リストに含まれていない可能性があります。最良の結果を得るには、このガイドに示されている最新バージョンを使用してください。
    ...
    **イメージをビルドしてプッシュします。**
    ...
    Dockerfile を保存したら、イメージをビルドしてコンテナ レジストリにプッシュします。
