...
* **OpenTelemetry Collector** - モニターElasticsearchを実行するには、ホストにOpenTelemetry Collectorインストールされ、実行されている必要があります。 サポートされているディストリビューションは 2 つあります。

  * **NRDOT (推奨):**[公式 NRDOT インストレーション ガイド](/docs/opentelemetry/nrdot/nrdot-collector/)に従って、ホスト上でコレクターをセットアップします。
  * **OTel Contrib:** [OpenTelemetry Collector Contrib が](https://github.com/open-telemetry/opentelemetry-collector-contrib/releases/latest)ホストにインストールされ、実行されています。systemd サービス ユニットが正しく作成されるように、公式パッケージ (.deb または .rpm) を介してインストールします。
...
<Callout variant="tip">
  標準のコミュニティ バージョンよりも**NRDOT (New Relic Distribution of OpenTelemetry)**を使用することを強くお勧めします。New Relic が所有するコンポーネントとしては、次のようになります。

  * **最適化:** New Relic バックエンドで最高のパフォーマンスが得られるように事前構成されています。
  * **信頼性:**エンタープライズの安定性とセキュリティについて徹底的にテストされています。
  * **サポート:**トラブルシューティングを迅速化するために、New Relic サポート チームが全面的にサポートします。
</Callout>

## ステップ2: OpenTelemetry Collectorを構成する [#config]

Elasticsearchクラスタからメトリクスとログ収集を設定するには、NRDOTの場合は/etc/nrdot-collector/config.yaml、Collector Collectorの場合は/etc/otelcol-contrib/config.yamlで設定ファイルを作成または更新します。
...
<CollapserGroup>
  ...
  <Collapser id="logging-config" title="ログを有効にする（ファイルログレシーバー）">
    **オプション:** ElasticsearchファイルをMetrixに加えてNew Relicに送信する場合は、これを含めます。
    ...
    Elasticsearchログを収集して転送するための`filelog`レシーバー設定を追加します。 コレクター サービス (nrdot-コレクターまたは otelcol-contrib など) を実行しているユーザーに、 Elasticsearchログファイルへの読み取りアクセス権があることを確認してください。
...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="nrot-collector">
      /etc/ systemd /system/nrdot-collecter.service.d に以下の設定を追加します。 ファイルで、環境変数が \[Service] セクションの下に配置されていることを確認します。
      ...
      これらの変更を適用するには、systemd マネージャーをリロードし、コレクターを再起動します。
    ...
    <TabsPageItem id="otel-collector">
      /etc/systemd/system/otelcol-contrib.service.d に次の設定を追加します。ファイルで、環境変数が \[Service] セクションの下に配置されていることを確認します。
