...
[NRDOT Collector](https://github.com/newrelic/nrdot-collector-releases/releases/latest) (推奨) または自動 Pod 検出機能を備えた[OpenTelemetry Collector](https://github.com/open-telemetry/opentelemetry-collector-contrib)を使用して、 Kubernetesで実行されているRabbitMQ監視します。 このガイドでは、Kubernetes オブザーバーを活用した動的ブローカー検出を使用して、Helm 経由でコレクターをデプロイする方法について説明します。
    ...
    <TabsPageItem id="nrdot-k8s">
      <Steps>
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="k8s-reqs-nrdot" title="Kubernetesクラスタの要件">
              * **Kubernetes バージョン**: 1.19 以上
              * **kubectl access** : デプロイメント、サービス、RBAC リソースを作成するための適切な RBAC 権限が設定されている
            </Collapser>

            <Collapser id="rabbitmq-reqs-nrdot" title="RabbitMQの要件">
            ...
            <Collapser id="newrelic-reqs-nrdot" title="New Relicの要件">
              * **アカウント**: アクティブな New Relic アカウント
              * **ライセンスキー**: アカウントの[New Relicキー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)
              * 基本的な[Kubernetes OpenTelemetryマニフェストのインストール](/docs/kubernetes-pixie/k8s-otel/install/#manifest-install)が完了しました
            </Collapser>
          </CollapserGroup>
        </Step>

        <Step>
          ### 管理エンドポイントを確認する [#verify-endpoint-nrdot]
        ...
        <Step>
          ### NRDOT Collectorを構成する [#configure-nrdot]
          ...
          1. NRDOT Collector を使用するには、コレクター イメージを更新します。
          ...
          2. RabbitMQ監視の[`deployment-configmap.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-k8s-otel-collector/examples/k8s/rendered/deployment-configmap.yaml)を更新します。

             `deployment-config.yaml`コンテンツ全体を RabbitMQ 固有の設定に置き換えます。
          ...
          3. RabbitMQ設定をカスタマイズします。

             * RabbitMQ ポッドのラベルに合わせて`rule`を更新します
             * `admin` / `password` RabbitMQ の認証情報に合わせて変更します
             * `your-cluster-name`実際のクラスタ名に置き換えます
             * `otlp_http/newrelic`エクスポーターの下の`endpoint`を更新します。OTLP エンドポイント設定 [ドキュメント](/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol)を参照して、地域に適したエンドポイントを選択してください。

          4. 更新されたマニフェストを適用し、デプロイメントを再起動します。
        ...
        <Step>
          ### デプロイメントとデータ フローを確認する [#verify-nrdot]
    ...
    <TabsPageItem id="otelcol-k8s">
      <Steps>
        <Step>
          ...
          <CollapserGroup>
            <Collapser id="k8s-reqs-otelcol" title="Kubernetesクラスタの要件">
              * **Kubernetes バージョン**: 1.19 以上
              * **kubectl access** : デプロイメント、サービス、RBAC リソースを作成するための適切な RBAC 権限が設定されている
        ...
        <Step>
          ### 管理エンドポイントを確認する [#verify-endpoint-otelcol]
        ...
        <Step>
          ...
          <Tabs>
            <TabsBar>
              <TabsBarItem id="us-otelcol-k8s">米国地域</TabsBarItem><TabsBarItem id="eu-otelcol-k8s">EU地域</TabsBarItem>
            </TabsBar>
