---
title: セルフホスト型PostgreSQL監視用のNRDOT Collectorのインストールと設定
tags:
  - OpenTelemetry
  - NRDOT
  - PostgreSQL
...
<Steps>
  ...
  <Step>
    ...
    **監視ユーザーと権限を作成するには：**
    ...
    1. スーパーユーザー、または必要な権限を持つ別のユーザーとしてPostgreSQLインスタンスに接続します：
  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="full" title="フル機能設定">
        この設定により、PostgreSQLデータベースの監視に加えて、CPU、メモリ、ディスク、ネットワークなどのホストレベルのメトリクスが追加されます。
        ...
        <Callout variant="important">
          NRDOT Collectorを介したPostgreSQLインテグレーションは、標準の[データ取り込みの価格](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#data)に従います。フル機能の設定では、標準のセットアップよりも多くのデータが取り込まれるため、請求総額が増加する可能性があることにご注意ください。
        </Callout>
      </Collapser>
    </CollapserGroup>
    ...
    <Callout variant="tip">
      ...
      * [PostgreSQLデータベースをAPMとリンクする](/docs/opentelemetry/database/capabilities/db-apm): アプリケーションパフォーマンスとデータベース操作を関連付けるため。これにより、どのアプリケーションが特定のデータベースワークロードを生成しているかを正確に確認できます。
...
<DocTiles>
  <DocTile title="PostgreSQLデータベースをAPMとリンクします。" path="/docs/opentelemetry/database/capabilities/db-apm">
    アプリケーションパフォーマンスとデータベース操作を関連付ける方法を学習します。
  </DocTile>
