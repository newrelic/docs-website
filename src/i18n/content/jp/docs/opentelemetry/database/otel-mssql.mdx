...
NRDOT Collector を使用して、Microsoft SQL Server データベースをモニターできます。NRDOTコレクターは、New Relic固有のコンポーネントを含む事前設定されたディストリビューションです。

以下の方法でNRDOT Collectorを設定できます：

* [ガイド付きインストール](/docs/opentelemetry/database/otel-mssql/#guided): ガイド付きインストール方法では、SQL Server監視用のNRDOT Collectorのインストールと設定を支援するために、New Relicプラットフォーム上でステップバイステップの手順を提供します。
* [手動インストール](/docs/opentelemetry/database/otel-mssql/#manual): 手動インストールでは、このドキュメントの手順に従ってご自身でNRDOT Collectorをセットアップできます。この方法は、実践的なアプローチを好むユーザーや、特定の設定要件を持つユーザーに最適です。

<Tabs>
  <TabsBar>
    <TabsBarItem id="guided">ガイド付きインストール</TabsBarItem> <TabsBarItem id="manual">手動インストール</TabsBarItem>
  </TabsBar>
  ...
  <TabsPages>
    <TabsPageItem id="guided">
      ...
      7. インストレーション後、<DNT>Test your integration</DNT>画面で**\[Test connection]**をクリックして、NRDOT CollectorがSQL Serverインスタンスに接続し、New Relicにデータを送信できることを確認します。テストが成功すると、確認メッセージが表示されます。

      ### APMとデータベースの相関を設定する [#optional-service-id]
      ...
      <Steps>
        ...
        <Step>
          ...
          * **Linux：** `/etc/nrdot-collector-host/config.yaml`
          ...
          <Callout variant="important">
            コアのメトリクスカテゴリのトグルを有効にしてデータギャップに気づいた場合は、 `timeout`と`collection_interval`両方を増やして、スクレイパーが完了するまでの時間を増やしてください。 重複したスクレイピングサイクルを防ぐため、タイムアウト値は常に`collection_interval`未満に設定してください。例えば：
...
<CollapserGroup>
  ...
  <Collapser id="buffer-metrics" title="データベースバッファメトリクス">
    `enable_database_buffer_metrics: true`を有効にしてこれらのメトリクスを収集します。
