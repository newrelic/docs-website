...
<Steps>
  ...
  <Step>
    ...
    * **Linux：** `/etc/nrdot-collector-host/config.yaml`
    ...
    <Callout variant="important">
      コアのメトリクスカテゴリのトグルを有効にしてデータギャップに気づいた場合は、 `timeout`と`collection_interval`両方を増やして、スクレイパーが完了するまでの時間を増やしてください。 重複したスクレイピングサイクルを防ぐため、タイムアウト値は常に`collection_interval`未満に設定してください。
