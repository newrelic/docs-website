    ...
    <TabsPageItem id="auth">
      <Steps>
        ...
        <Step>
          ...
          <Callout variant="important">
            **重要な注意事項：** `<account_name>`のすべてのインスタンスを実際のWindowsアカウント名（例：`ComputerName\Username`）に置き換えます。SQL Server Management Studio（SSMS）またはsqlcmdユーティリティを使用して、これらのスクリプトを実行します。確認スクリプトは、不足している権限を正確に示します。
          </Callout>
        </Step>
