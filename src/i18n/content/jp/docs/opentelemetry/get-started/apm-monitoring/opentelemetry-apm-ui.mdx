...
## New Relicにおけるエラーの表現方法 [#errors]

アプリケーションからのエラー情報は、APM UI内の2か所に表示されます：

* **エラー率**: [概要ページ](#summary-page)に表示され、メトリクスデータから導出されます。`http.server.request.duration`または`rpc.server.call.duration`などのOpenTelemetryメトリクスに`error.type`属性が含まれる場合、そのデータポイントはエラー率に寄与します。
* **Errors Inbox**: スパンデータに基づく[Errors Inboxページ](#errors-inbox-page)を参照してください。ステータスが`ERROR`に設定されると、スパンはエラーとして分類されます（例：`otel.status_code = ERROR`）。

### エラーを無視する [#ignore-errors]

New Relic言語エージェントは、HTTPステータスコードまたはエラークラスによってエラーを無視するように設定でき、エラー率に影響を与えたり、Errors Inboxに表示されたりするのを防ぐことができます。OpenTelemetryでは、同等のエージェント設定はありませんが、New Relicに到達する前に基盤となるデータを操作することで、同じ効果を得ることができます。

#### エラー率でエラーを無視する

エラー率は、[トランザクションを導出する](#transaction-mapping)のと同じメトリクスデータから導出されます。`http.server.request.duration`または`rpc.server.call.duration`などのメトリクスに`error.type`属性が含まれている場合、そのデータポイントはエラー率に寄与します。メトリクスデータポイントから`error.type`属性を削除すると、エラーとしてカウントされなくなります。

<Callout variant="important">
  OpenTelemetryのセマンティック規約の以前のバージョンには、`error.type`属性は存在しませんでした。HTTPおよびRPCセマンティック規約の以前のバージョンに従う古い計装をサポートするために、New Relicは`http.status_code`および`rpc.grpc.status_code`属性を活用してエラー率を算出します。これらの属性は、古い`http.server.duration`および`rpc.server.duration`メトリクスに存在していました。
</Callout>

#### Errors Inboxでエラーを無視する

Errors Inboxはスパンデータに基づいています。ステータスが`ERROR`（つまり、`otel.status_code = ERROR`）に設定されている場合、スパンはエラーとして分類されます。スパンのステータスを未設定にして`ERROR`ではなくなるようにすると、スパンがErrors Inboxに表示されなくなります。

#### 例：OpenTelemetry Collectorを使用してデータを変換する

[OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) を使用して、転送中のメトリクスとスパンデータを変換し、特定のエラーを抑制することができます。[無視されたエラーの例](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/ignored-errors)は、以下の方法を示す動作するコレクターの設定を提供します：

* メトリクスデータポイントから`error.type`属性を削除して、エラー率から除外する
* エラースパンがErrors Inboxに表示されないように、スパンのステータスを未設定にします。
