...
New Relicは、OTLPペイロードの属性にさまざまな制限を課しています。一般的に、検証に対しては寛容な姿勢をとっており、データを完全に破棄するのではなく、属性を削除、切り捨て、またはその他の方法で変更します。属性が変更されるたびに、発生源での問題の追跡と修正に役立てるため、[NrIntegrationError](https://docs.newrelic.com/docs/data-apis/manage-data/nrintegrationerror/)を作成します。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        文字列型の属性値の長さは最大4095文字です。この制限を超える値は切り捨てられます。
      </td>
      ...
      <td>
        ...
        [OpenTelemetryのドキュメント](https://opentelemetry.io/docs/specs/otel/configuration/sdk-environment-variables/)
    ...
    <tr>
      ...
      <td>
        属性キーの最大長は255文字です。この制限を超えるキーは切り捨てられます。
      </td>
    ...
    <tr>
      ...
      <td>
        各属性ソースにはカウント制限が適用されます。超過した場合、OpenTelemetryセマンティック規約で定義された属性を優先して、超過分の属性が削除されます。制限は以下のとおりです：<br />

        * 最大リソース属性：64
        * 最大スコープ属性：8
        * 最大レコード属性（スパン、メトリクスデータポイント、ログ）：128
      </td>

      <td>
        `OTEL_ATTRIBUTE_COUNT_LIMIT=64`

        を設定することによる部分的な緩和。詳細については、
    ...
    <tr>
      ...
      <td>
        属性値は次のタイプに制限されています：文字列、整数、double、ブール値、およびバイト配列。複雑なタイプの属性値は切り捨てられます。ログ属性は、この制限から除外されます。
    ...
    <tr>
      ...
      <td>
        配列の最大長は64エントリです。この制限を超える属性は切り捨てられます。
      </td>
    ...
    <tr>
      ...
      <td>
        配列のエントリはすべて同じ型である必要があります。異なる型の配列エントリを持つ属性は削除されます。ログ属性はこの制限から除外されます。
    ...
    <tr>
      ...
      <td>
        配列のエントリは、次の型に制限されています：string、integer、double、およびboolean。複雑な型を持つ配列属性の値は削除されます。ログ属性はこの制限から除外されます。
    ...
    <tr>
      ...
      <td>
        バイト配列の最大長は128,000です。この制限を超える属性は切り捨てられます。
      </td>
...
**\[1]**: [OTLP属性タイプ](#otlp-attribute-types)を参照してください。
