...
New Relicエージェントは、通常の収集サイクルの一環として、[リアルタイムストリーミング](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/manage-apm-agents/agent-data/real-time-streaming)をサポートするエージェントバージョンに対して、5秒おきに[イベントデータ](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data)をNew Relicに送信します。
