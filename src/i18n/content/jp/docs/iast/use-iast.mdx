...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > IAST**に移動し、**Setup IAST with applications** \[アプリケーションで IAST をセットアップ を]クリックします。
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > IAST > Applications**に移動します。

2. **Tested applications** \[テスト済みのアプリケーション]で、アプリケーションを検索するか選択します。

3. **Application vulnerabilities** \[アプリケーションの脆弱性]タブで、アプリケーションで見つかった悪用可能な脆弱性をすべて確認します。<img title="Overview of the exploitable vulnerabilities found in your application." alt="Overview of the exploitable vulnerabilities found in your application." src={iastAppVuln}/>
...
5. さらに、アプリケーション ウィンドウの**Application coverage** \[アプリケーション カバレッジ]タブで、アプリケーションの各部分がどの程度脆弱であるかを確認します。<img title="Vulnerability coverage of your application." alt="Vulnerability coverage of your application." src={iastAppCoverage}/>
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > IAST > Exploitable Vulnerabilities**に移動します。<img title="Overview of the exploitable vulnerabilities found in all your applications." alt="Overview of the exploitable vulnerabilities found in all your applications." src={iastExploitablesAll}/>
...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > IAST**に移動します。
...
4. **Enable IAST** \[IAST を有効にする]ウィンドウで、手順に従ってアプリケーション構成を更新し、悪用可能な脆弱性をテストできるようにします。<img title="Enable IAST for your untested application" alt="Enable IAST for your untested application" src={iastEnableIast}/>
...
**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > Query Your Data**に移動し、次の NRQL クエリを実行します。
...
<CollapserGroup>
  ...
  >
    <Callout variant="important">
      [APM の高セキュリティ モード](/docs/apm/agents/manage-apm-agents/configuration/high-security-mode/)を使用している場合、エージェントは動作しません。
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > All capabilities > IAST > Applications > Tested applications**に移動し、アプリケーションをクリックして、**Application coverage** \[アプリケーション カバレッジ]タブをクリックすると、アプリケーションのテスト効率、対象となる API、およびメソッド呼び出しなどのデータが表示されます。
  </Collapser>
