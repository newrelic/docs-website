...
<CollapserGroup>
  ...
  >
    **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications**に移動します。 アプリケーションをクリックすると、アプリケーションのテスト効率、脆弱性、対象となる API、メソッド呼び出しなどのデータを確認できます。
  </Collapser>
  ...
  >
    ...
    * アプリケーションの効率のレベル: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications**に移動します。 アプリケーションを検索し、概要セクションを確認してください。 アプリケーションの IAST カバレッジが低い場合は、アプリケーションにテスト ケースを追加して、より高いレベルのテスト効率を実現します。

    * IAST の対象範囲: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications**に移動します。 アプリケーションを検索してクリックします。 概要セクションで、IAST 分析のカバレッジが高く、脆弱性が検出されないかどうかを確認します。つまり、アプリケーションが安全であることを意味します。
  ...
  >
    **[one.newrelic.com](https://one.newrelic.com) > ll capabilities > IAST > Tests & Applications**が表示されない場合は、次の点を確認してください。
  ...
  >
    ...
    * アプリケーション**は[、 one.newrelic.com](https://one.newrelic.com) > すべての機能 > IAST > テストとアプリケーション**に表示されます。 アプリケーションが開始され、トラフィックが生成されます。
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications**に移動します。

    2. **Application tests** \[アプリケーション テスト]タブでアプリケーションを検索し、クリックします。

    3. **APIs covered** \[対象となる API] 、**methods calls** \[メソッド呼び出し]、**application testing efficiency** \[アプリケーション テストの効率など]、テストの詳細を表示します。

       また、ログレベルを**debug/finest**に設定すると、 `nr-security-home/logs/java-security-collector.log`ファイルで**Fuzz request received** \[受信した Fuzz リクエスト]を検索できます。 これは、IAST 分析が進行中であることを示しています。
  </Collapser>
  ...
  >
    ...
    * アプリケーションを検索します。 **[one.newrelic.com](https://one.newrelic.com) > All capabilities > IAST > Tests & Applications**に移動します。
  ...
  >
    * Golang の場合は、アプリケーションが使用するライブラリとフレームワークに必要なインストルメンテーション パッケージをインポートしていることを確認してください。
      ...
      対応するものをインポートしたことを確認してください[<InlinePopover type="apm"/>](https://github.com/newrelic/go-agent?tab=readme-ov-file#service-frameworks)各モジュールのパッケージ。
    ...
    * HTTP プロトコル エンドポイントを開いている場合は、エンドポイント名の前後に`newrelic.WrapListen`関数を配置して、そのエンドポイントに対する脆弱性スキャンを有効にします。
      ...
      <Callout variant="important">
        Linux 環境を使用している場合は、この手順を省略できます。
      </Callout>

    * **Outbound HTTP Request** \[アウトバウンド HTTP リクエスト]または外部サービスへの呼び出しの特定のケースでは、[アプリケーション メソッドを更新する](/docs/apm/agents/go-agent/instrumentation/instrument-go-segments/#go-external-segments)必要があります。
  </Collapser>
  ...
  <Collapser
    ...
    title="JIRA 統合は利用できますか?"
  >
    まだありませんが、近い将来追加する予定です。
  </Collapser>
  ...
  >
    IAST は、監視されたリクエストに基づいて新しいリクエストをアプリケーションに送信します。 これにより、クレジット カードへの複数の請求など、顧客データの望ましくない操作が行われる可能性があります。 New Relic は現在、本番環境以外の環境で IAST を使用することを推奨しています。 これらの環境での外形監視データに基づいてテスト ケースを作成します。 私たちの計画は、運用環境で使用するための IAST のバージョンを提供することです。
  </Collapser>
</CollapserGroup>