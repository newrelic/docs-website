...
CodeStream と New Relic One が連携することで、コードのエラーを把握することができるだけでなく、APM エージェントによるコードのインスツルメンテーションを簡単に始めることができます。

CodeStream が New Relic One に接続されていると、スタックトレースのエラーから、IDE の問題のあるコード行に直接ジャンプすることができます。IDE に入ったら、スタックトレースをナビゲートし、 [チームメイトと協力して問題を解決することができます](/docs/codestream/how-use-codestream/performance-monitoring) 。
...
## CodeStream と New Relic の接続 [#connect]

CodeStream で New Relic のオブザーバビリティ機能を利用する前に、それらを接続する必要があります。

CodeStream と New Relic を接続するための要件です。
...
<Callout variant="caution">
  New Relic のユーザーは CodeStream
  でスタックトレースのエラーを共有することができます。CodeStream を New Relic
  に接続すると、CodeStream
  の組織に追加した新しいユーザーは、これらのエラーを見ることができます。
</Callout>
...
IDE でスタックトレースエラーを開くための条件

- [New Relic アカウント](https://newrelic.com/signup) (New Relic アカウントをお持ちでない方は、 [newrelic.com/signup](https://newrelic.com/signup) でサインアップしてください。永久に無料です！)
- [New Relic のユーザーキー](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher)
- [CodeStream と New Relic の接続](#connect)
- [APM モニタリングで New Relic に報告されるデータ](https://one.newrelic.com/launcher/nr1-core.explorer?pane=eyJuZXJkbGV0SWQiOiJucjEtY29yZS5saXN0aW5nIn0=&cards%5B0%5D=eyJuZXJkbGV0SWQiOiJucjEtaW5zdGFsbC1uZXdyZWxpYy5ucjEtaW5zdGFsbC1uZXdyZWxpYyIsImFjdGl2ZUNvbXBvbmVudCI6IlZUU09FbnZpcm9ubWVudCIsInBhdGgiOiJndWlkZWQifQ==)
...
- [APM エラーの場合は、リポジトリのコミットハッシュとリリースタグ](#apm)

New Relic のエラーインボックスは、顧客に影響を与える前にエラーを積極的に検出し、優先順位をつけ、対策を講じるための単一の場所です。CodeStream では、エラーから IDE の問題のあるコードに直接ジャンプすることができます。

<Callout variant="tip">APMのエラーに限る。</Callout>

### 誤差のある受信箱から IDE に移行する方法

**[one.newrelic.com/](https://one.newrelic.com/)** から、 **Errors Inbox** 、スタックトレースエラーをクリックし、 **Open in IDE** をクリックします。

## APM エラーと CodeStream [#apm]

スタックトレースエラーを IDE で表示するためには、CodeStream はエラーがどのリポジトリに関連しているか、また理想的にはエラーを発生させたコードのどのバージョンかを知る必要があります。

## リポジトリとエラーの関連付け [#repo-url]

APM、モバイル、ブラウザ、Lambda の監視を開始したら、リポジトリエンティティを作成して、すべてのサービスのエンティティと関連付ける必要があります。リポジトリエンティティを作成するためには、リポジトリのリモート URL を提供する必要があります。例えば、リモート URL には SSH 形式と HTTPS 形式があります。
...
IDE でエラーを開こうとしたときに、関連するリポジトリがない場合、CodeStream は関連付けを行うように促し、New Relic 上の指定されたエンティティからのすべてのエラーに対してその関連付けを保存します。しかし、継続的な手作業の手間が少なく、リモート URL の設定ミスなどエンドユーザーのミスの可能性を排除できるため、以下のいずれかの方法を使用することが望ましいでしょう。
...
<CollapserGroup>
  ...
  >
    ...
    ![リポジトリ接続ボタンのスクリーンショットです。](src/images/connect-repo1.png "リポジトリ接続ボタンです。")
...
## ビルド SHA やリリースタグとエラーの関連付け [#buildsha]
...
あるいは、リリースタグ（ `v0.1.209` や `release-209` など）を使うと、CodeStream がコードの正しいバージョンを見つけてくれます。
...
- `NEW_RELIC_METADATA_COMMIT` - コミットシャ。全体を含めることも、最初の 7 文字だけを含めることもできます。
...
## CodeStream による APM エージェントのインストール [#install-agents]

CodeStream 経由で New Relic APM エージェントをインストールするための要件です。

- [New Relic アカウント](https://newrelic.com/signup)
...
## Go と Pixie によるダイナミックロギング

- [New Relic アカウント](https://newrelic.com/signup)

- [New Relic ユーザーの API キー](https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher)

- [CodeStream と New Relic の接続](#connect)

- [Pixie が監視する Kubernetes クラスタ](/docs/auto-telemetry-pixie/install-auto-telemetry-pixie/)
