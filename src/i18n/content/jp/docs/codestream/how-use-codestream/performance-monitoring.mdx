...
コードレベルのメトリックは、コードがメソッドレベルでどのように実行されているかについての詳細な洞察を提供します。 NewRelicのPythonおよびRubyAPMエージェントによって自動的にインストルメント化されたメソッドごとに、CodeLensと呼ばれる各メソッドの上に挿入されたテキスト行を介して金色の信号が表示されます。過去30分間の応答時間、スループット、およびエラー率が表示されます。
...
<Callout variant="important">
  Pythonの場合、コードレベルのメトリックは、[サポートされているPythonフレームワーク](/docs/apm/agents/python-agent/getting-started/instrumented-python-packages)で実装された[PythonAPMエージェント](/docs/apm/agents/python-agent/getting-started/introduction-new-relic-python)によって監視されているプロジェクトでのみ使用できます。

  Rubyの場合、コードレベルのメトリックはRailsアプリケーションでのみ使用可能であり、Rubyメソッドは手動でトレースします。
</Callout>
