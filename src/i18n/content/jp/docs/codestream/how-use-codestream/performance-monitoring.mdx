...
* Ruby-コードレベルのメトリックは、Railsアプリケーションと手動トレースを使用するRubyメソッドでのみ使用できます。コードレベルのメトリック[は、Rubyエージェント構成でも有効にする](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#code_level_metrics-enabled)必要があります。VS Codeを使用している場合は、言語サーバーを有効にして、 [Ruby](https://marketplace.visualstudio.com/items?itemName=rebornix.Ruby)または[RubySolargraph](https://marketplace.visualstudio.com/items?itemName=castwide.solargraph)拡張機能もインストールする必要があります。
* .NET-コードレベルのメトリック[は、.NETエージェント構成で有効にする](/docs/apm/agents/net-agent/configuration/net-agent-configuration#code_level_metrics)必要があります。さらに、 **\[ツール]>\[オプション]>\[テキストエディター]>\[すべての言語]>\[CodeLens]**に移動し、\[CodeLensを有効にする]と\[CodeStreamコードレベルメトリックを表示する]の両方がオンになっていることを確認して、VisualStudioでCodeLensesが有効になっていることを確認します。
