...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="UIの利用"
  >
    New Relic へのデータの送信を開始したら、UI を使用して関連リポジトリに接続します。**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (アプリを選択)**から APM 概要ページに移動し、右下の **Repositories** \[リポジトリ] セクションを探します。
