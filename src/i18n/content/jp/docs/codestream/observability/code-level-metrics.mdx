...
* **Node.js:** Node.js エージェントのバージョン 10.5.0 以降が必要です。 コードベースに匿名関数が含まれている場合は、[変更追跡 (変更追跡機能)](/docs/change-tracking/change-tracking-introduction/)を使用してデプロイ情報を New Relic に送信するか、ビルド パイプラインの[環境変数を介してビルド SHA を送信する](/docs/codestream/observability/error-investigation/#buildsha)必要があります。
