...
CodeStream には、割り当てられたエラーと、IDE で開かれたコードで発生した最新の 5 つのエラーが表示されます。このセクションの見出しにあるドロップダウン リストを使用して時間枠を制御できます。このリストは、New Relic の[エラー受信箱](/docs/errors-inbox/errors-inbox/)に表示されるエラーのサブセットである可能性があることに注意してください。
