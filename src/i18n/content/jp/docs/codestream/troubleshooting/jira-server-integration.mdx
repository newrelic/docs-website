...
3. 記入してください。
   ...
   * **Consumer Key**には、上記の手順 2 でアプリケーション リンクを作成したときに入力したコンシューマー キーを使用します。次に、上記の手順 1 で作成した秘密鍵の内容全体を PCKS8 形式でコピーします。ファイルは **jira_privatekey.packs8**という名前にする必要があります。これらの内容を **Private Key** フィールドに貼り付けてから、 **Submit**をクリックします。
