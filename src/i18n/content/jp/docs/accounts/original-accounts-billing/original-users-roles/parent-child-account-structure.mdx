...
New Relicの組織では、メインのNew Relicアカウントは独自の[ライセンスキー](/docs/accounts-partnerships/accounts/account-setup/license-key)を持つ親アカウントとしての役割を果たします。各子アカウントにはそれぞれ独自のライセンスキー（および独自の[APIキー](/docs/apis/rest-api-v2/requirements/rest-api-key)）があります。各アプリケーションは1つのライセンスキーのみを使用できるため、各アプリケーションは1つのアカウントのみにレポートできます。
