...
<CollapserGroup>
  ...
  >
    [元の料金モデル](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing)のアカウントの場合、使用状況 UI を使用すると、データ使用量を予測どおりに追跡し、それが料金にどのように影響するかを確認できます。 このドキュメントでは、New Relic が請求可能な使用量を計算する方法について説明します。 <InlinePopover type="apm"/>CU ベースの価格設定 (ホストベースの価格設定ではない) のアカウントで、利用可能なサブスクリプション使用属性と、New Relic UI または API 呼び出しで使用する [NRQL クエリの例](#query-examples) も提供します。

    ### データ作成 [#details]

    1 日 1 回、 <InlinePopover type="apm"/>アカウントは次の`NrDailyUsage` [イベントを](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#events-new-relic) 生成します:
    ...
    * 過去24時間にアプリケーション・インスタンスが存在したすべてのホスト
    ...
    3. **Avg daily compute units** に当月の日数を掛けてください。
       ...
       いくつかの以前の <InlinePopover type="apm"/>エージェントは、コンテナーをホストとして誤ってカウントする可能性があり、計算ユニット (CU) の使用量が過剰に報告される可能性があります。
    ...
    * 過去24時間にアプリケーション・インスタンスが実行されたすべてのホスト
    ...
    3. **Avg daily equivalent hosts** for an account or grouping of accounts をご覧ください。
       ...
       <CollapserGroup>
         ...
         >
           このクエリは、「ホスト使用状況レポート」で提供されたデータ (各ホストで実行されているアプリケーションのリスト) を返します。その間、 <InlinePopover type="apm"/>使用量は時間単位でカウントされ、1 時間ごとの使用量データは API または UI からは利用できなくなりました。
       ...
       いくつかの以前の New Relic <InlinePopover type="apm"/>エージェントはコンテナーをホストとして誤ってカウントする可能性があり、ホストベースの使用量が過剰に報告される可能性があります。
  ...
  >
    ...
    1. **[ユーザー メニュー](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)> 使用状況を表示 に移動します**。
  ...
  >
    ...
    * 当社のAPIを使用してプログラム的にこのデータを使用します。
      ...
      <CollapserGroup>
        ...
        >
          このクエリは、特定のアカウントに対して現在保存されている Insights が管理するイベントの数を、イベントの[名前空間](#insightsEventNamespace)別に分類して示します。これらのクエリの YOUR_ACCOUNT_ID を[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)に置き換えてください。
        ...
        >
          このクエリは、アカウントのリストに対して現在保存されている Insights が管理するイベントの数を、イベントの[名前空間](#insightsEventNamespace)ごとに示します。クエリの LIST_OF_ACCOUNT_IDs を[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)のリストに置き換える必要があることに注意してください。
        ...
        >
          このクエリは、過去 7 日間に、他の New Relic 製品サブスクリプションに含まれる Insights が管理するイベントの数を、それらを作成した製品ごとにアカウントに保存したことを示します。クエリの YOUR_ACCOUNT_ID を[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)に置き換える必要があることに注意してください。
        ...
        >
          このクエリは、さまざまな New Relic 製品 (たとえば、 <InlinePopover type="apm"/>`Transaction`イベント)、指定された親アカウント ID の下の子アカウントごと。 独自の NRQL クエリを作成するときは、必ず YOUR_ACCOUNT_ID を特定の [アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)に置き換えてください。
        ...
        >
          このクエリは、指定された親アカウント ID の各子アカウントが過去 7 日間に使用した Insights Pro サブスクリプションのイベント数を示します。独自の NRQL クエリを作成するときは、クエリ内の YOUR_MASTER_ACCOUNT_ID を特定の親[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)に置き換えてください。
