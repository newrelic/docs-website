  ...
  >
    ...
    * 当社のAPIを使用してプログラム的にこのデータを使用します。
      ...
      <CollapserGroup>
        ...
        >
          このクエリは、指定された親アカウント ID の下の子アカウントごとに、さまざまな New Relic 製品 (APM `Transaction`イベントなど) に関連付けられたさまざまなイベント カテゴリの合計、有料、および含まれる保持期間を示します。独自の NRQL クエリを作成するときは、必ず YOUR_ACCOUNT_ID を特定の[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)に置き換えてください。
