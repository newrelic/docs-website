...
<Callout variant="tip">
  アカウント **オーナー** と **アドミン** のみが使用状況 UI を見ることができます。ただし、アカウント内の誰でも、 `NrDailyUsage` [event](/docs/data-apis/understand-data/new-relic-data-types/#event-data) を使って、利用状況データを照会できます。
</Callout>
...
New Relic は、利用状況データを [`NrDailyUsage` イベントタイプ](/docs/data-apis/understand-data/new-relic-data-types/#event-data) として保存します。利用状況チャートには、UI で表示されるものに近い NRQL クエリが関連付けられています。
...
<CollapserGroup>
  ...
  >
    ...
    1日1回、APMアカウントは、 `NrDailyUsage` [event](/docs/data-apis/understand-data/new-relic-data-types/#event-data) forを生成します。
  ...
  >
    ...
    1 時間に 1 回、監視対象のアプリケーションごとに、New Relic Browser アカウントが `NrUsage` [イベント](/docs/data-apis/understand-data/new-relic-data-types/#event-data) を生成します。各イベントには、過去 1 時間の使用量が要約されます。
    ...
    <Callout variant="tip">
      アカウント **オーナー** と **アドミン** のみが使用状況 UI を見ることができます。ただし、アカウント内の誰でも、 `NrUsage` [event](/docs/data-apis/understand-data/new-relic-data-types/#event-data) を使って、利用状況データを照会することができます。
    </Callout>
  ...
  >
    ...
    1 日に 1 回、監視対象のアプリケーションごとに、New Relic Mobile アカウントは `NrDailyUsage` [イベント](/docs/data-apis/understand-data/new-relic-data-types/#event-data) を生成します。各イベントは、現在の暦月、過去 24 時間までの Mobile 使用量を要約します。
