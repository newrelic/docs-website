...
利用データは、次の2件の[イベント](/docs/using-new-relic/data/understand-data/new-relic-data-types#events-new-relic)に添付されます。
...
New Relicアカウントの変更を閲覧するには、[`NrAuditEvent`](/docs/insights/insights-data-sources/default-data/nrauditevent-event-data-query-examples)イベントのクエリを行います。
...
この属性は、`NrMTDConsumption`イベントと`NrConsumption`イベントに添付されます。
