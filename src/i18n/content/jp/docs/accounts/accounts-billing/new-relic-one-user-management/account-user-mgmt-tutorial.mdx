...
既存のグループを表示するには、[アカウント ドロップダウン](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown)から、\[**管理**] をクリックし、\[**アクセス管理**] をクリックします。\[**グループ**] タブの下に、既存のグループと、それらに割り当てられているアクセス権が表示されます。(詳細については、[ステップ #1](#create-organization)に戻ってください)。
...
[NerdGraph API を使用して、ユーザー情報を更新し、ユーザーをグループに追加する](/docs/apis/nerdgraph/examples/nerdgraph-manage-users)こともできます。
