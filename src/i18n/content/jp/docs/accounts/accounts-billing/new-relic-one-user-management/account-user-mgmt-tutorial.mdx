...
既存のグループを表示するには、[アカウント ドロップダウン](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown)から、\[**管理**] をクリックし、\[**アクセス管理**] をクリックします。\[**アクセス許可とグループ**] タブの下に、既存のグループと、それらに割り当てられているアクセス権が表示されます。(詳細については、[ステップ #1](#create-organization)に戻ってください)。
...
<CollapserGroup>
  ...
  >
    ...
    1. **アクセス管理**UI から、\[**アクセス許可とグループ**] をクリックします。
    ...
    3. 次に、 **アクセス許可** で、以下のいずれかを選択します。
       ...
       * **組織**: [組織スコープのロール](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles)から選択できるようにするには、これを選択します。これらは、組織とユーザーの管理を管理する役割です。(これらのユーザーは、アカウント スコープのロールにも既に属している必要があることに注意してください。これはほとんどのユーザーに当てはまりますが、そうでない場合は、ユーザーが組織に属していないというメッセージが表示されることがあります)。
  ...
  >
    API を介してアクセス許可を管理する方法については、 [NerdGraph ユーザー管理](/docs/apis/nerdgraph/examples/nerdgraph-user-mgmt#concepts)を参照してください。
  </Collapser>
</CollapserGroup>
