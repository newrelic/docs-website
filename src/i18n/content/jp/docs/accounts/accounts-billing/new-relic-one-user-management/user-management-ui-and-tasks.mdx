...
* **アクセス管理**（[UIにリンク](https://one.newrelic.com/admin-portal/organizations/organization-detail)）：このUIを使用して、グループとロールを表示、作成および管理します。SAML SSOおよびSCIMプロビジョニングを設定するには、認証ドメインUIを使用します。
...
* **最終アクティビティ**：ユーザーが最後にNew Relicにログインした日付。これは、ユーザーを下位のユーザータイプにダウングレードする場合に便利です。
...
<CollapserGroup>
  ...
  >
    ...
    * **ユーザーの追加**：[**User management** UI](#where)に移動し、**Add user**選択します。New Relicにユーザーを直接追加すると、請求可能なユーザータイプのユーザーが`Pending`（New Relicにまだログインしていない）かどうかにかかわらず、すぐに請求対象になることに注意してください。

    * **ユーザーの招待**：[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)に移動し、**Invite a teammate**をクリックします。ユーザーの招待に関する詳細：
    ...
    ユーザーを表示および管理するには、[**User management** UI](#where)に移動します。
