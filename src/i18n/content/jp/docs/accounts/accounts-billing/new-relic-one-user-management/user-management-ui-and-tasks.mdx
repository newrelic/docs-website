...
* **Eメール**：ユーザーのEメールアドレス。ユーザーステータスに関するいくつかの注意事項が表示される場合があります。

  * `Pending`：このユーザーは追加されていますが、まだ初回ログインをしていません。`Pending`状態でも、ユーザーはプロビジョニング済みと見なされ、コアユーザーまたはフルプラットフォームユーザーであれば請求対象となります。
...
このページには**承認が必要な招待**タブがあります。これらのユーザーはまだプロビジョニング済みとは見なされず、請求対象ではありません。チームメイトを招待する方法の詳細については、[ユーザーの追加](#add-users)を参照してください。
...
<CollapserGroup>
  ...
  >
    ...
    * **ユーザーの追加**：[**User management** UI](#where)に移動し、**Add user**を選択します。ユーザーの追加に関する詳細：

      * ユーザーを直接追加するということは、New Relicにログインしたことがなく、`Pending`の場合でも、請求可能なユーザータイプを持つユーザーはすぐに請求対象になります。
      * 追加されたユーザーは確認メールを受信しますが、このメールは15日後に期限切れになります。再送信するには：ユーザー管理UIで、ユーザーの`Pending`アイコンにカーソルを合わせて、`Resend email`をクリックします。
