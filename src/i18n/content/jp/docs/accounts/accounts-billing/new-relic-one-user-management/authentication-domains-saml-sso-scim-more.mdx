...
<Callout variant="important">
  このドキュメントは、 [New Relic One のユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) でユーザーを管理するためのものです。オリジナルのユーザーモデルでのユーザー管理については、 [オリジナルユーザー](/docs/accounts/original-accounts-billing/original-users-roles) をご覧ください。
</Callout>
...
* [SCIMのプロビジョニング](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) 自動化されたユーザー管理としても知られていますが、ProまたはEnterpriseエディションが必要です。
...
## ユーザーのソース。マニュアルとSCIMの比較 [#source-users]
...
<CollapserGroup>
  ...
  >
    私たちのデフォルトの機能は、New Relic からユーザータイプを管理することです。これは、SCIM を通じてユーザータイプを管理する [でない場合に使用するオプションです。これに支配される管理オプションは以下の通りです。](#source-users)

    [](#source-users)

    * [](#source-users)

    * [でユーザータイプを変更できること ](#source-users)[**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where).

    * フルプラットフォームユーザーになるためのリクエストをどのように扱うかを管理する機能です。これらのオプションには以下が含まれます。

      * **ユーザーのセルフアップグレードを可能にする**: これにより、ベーシックユーザーとコアユーザーは、問題に迅速に対応するために、自分ですぐにフルプラットフォームユーザーにアップグレードすることができるようになります。
      * **Require review by admins**: この場合、admins は [**Authentication domain manager** role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#standard-roles) を持つユーザーを指します。このオプションでは、管理者は、フルプラットフォームユーザーになるためのリクエストがなされたときにメールを受け取り、 [**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) でそのリクエストを承認または拒否することができます。

    なお、ベーシックユーザーからコアユーザーへのアップグレードには審査は必要ありません。
  </Collapser>
