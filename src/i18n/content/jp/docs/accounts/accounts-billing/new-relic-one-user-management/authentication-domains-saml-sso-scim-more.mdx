...
<CollapserGroup>
  ...
  >
    ...
    * ユーザータイプのアップグレード要求をどのように処理するかを管理する機能です。これらのオプションは以下の通りです。
      ...
      * **Allow users to self-upgrade**: これにより、基本的なユーザーが、承認なしに、自分ですぐに上位のユーザータイプにアップグレードできるようになります。これにより、これらのユーザーは問題に対してより迅速に対応できるようになります。
      * **Require review**: このオプションでは、管理者（ **認証ドメインマネージャー** ロールを持つユーザー）は、ユーザーがアップグレードを要求すると電子メールを受け取り、 [**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) でそれらの要求を承認または拒否することができます。
  </Collapser>
