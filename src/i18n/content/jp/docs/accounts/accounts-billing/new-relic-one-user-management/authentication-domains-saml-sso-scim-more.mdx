...
認証ドメインを表示および構成するには:[ユーザー メニュー](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)から、 **\[管理] > \[アクセス管理] > \[認証ドメイン**] に移動します。
...
3. 次に、認証ドメイン UI に移動します。[ユーザー メニュー](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)から、\[**アクセス管理**] をクリックし、\[**認証ドメイン**] をクリックします。まだ持っていない場合は、SAML 認証ユーザーに使用[する新しいドメインを作成し](#ui)ます。
  ...
  >
    ...
    * ユーザータイプのアップグレード要求をどのように処理するかを管理する機能です。これらのオプションは以下の通りです。
      ...
      * **要確認**: このオプションを使用すると、ユーザーがアップグレードを要求すると、管理者 ( [**認証ドメイン**管理設定](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#admin-settings)のユーザー) にメールが送信され、 [**ユーザー管理**UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where)でそれらの要求を承認または拒否できます。
        ...
        * 個々のユーザーは、24 時間のスライディング ウィンドウで 6 回のアップグレード リクエストに制限されています。たとえば、午前 8 時から午前 10 時までの間に 6 回のアップグレード リクエストを行った場合、次のアップグレード リクエストを行う前に、翌日の午前 8 時まで待つ必要があります。
  </Collapser>
