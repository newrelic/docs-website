...
* ユーザーの**[ユーザータイプ](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type)**：ベーシックユーザーは無料で、コアユーザーとフルプラットフォームユーザーが請求対象です。ユーザータイプは、組織がチームメンバーに対してNew Relicを使用して期待する作業、そしてその作業からどれだけの価値が得られるかを期待することで決定されます。ユーザータイプは、ユーザーがアクセスできる最大許容機能を設定します。ユーザータイプは、権限の設定に使用することを意図していません。権限の設定を行うには、ロールを使用する必要があります。
* ユーザーの**ロール**：ユーザータイプが決定されると、必要に応じて、**ロール**を使用してユーザーのアクセスをより正確に制御できます。ロールは、New Relicで特定の作業を行う権限を付与する**機能**（APM設定を変更する機能など）で構成されます。[ユーザーグループ](#groups)に適用することで、役割を割り当てます。
...
[ProまたはEnterpriseエディション](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#editions)を使用する組織は、組織内に複数のアカウントを持つことができ、カスタムロールとグループを作成できます。Standardエディションの組織は、その組織で1つのアカウントのみ許可され、カスタムロールやグループの作成をすることができません。
...
* ユーザーのアクセスは、[ユーザータイプ](#user-type-groups-relation)と割り当てられたロールによって付与されるアクセス権に基づいています。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        このロールには、組織レベルの設定、ユーザー、請求を管理する機能を**除く**、すべてのNew Relicプラットフォーム機能が含まれています。これは、プラットフォーム機能（[設定を構成する機能など）の設定を許可するという意味で管理者ロールですが、組織レベルの管理者機能（<InlinePopover type="apm"/>](#admin-settings)を要する機能）は提供しません。
