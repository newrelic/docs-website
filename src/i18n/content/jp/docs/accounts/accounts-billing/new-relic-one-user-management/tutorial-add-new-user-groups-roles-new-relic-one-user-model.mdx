...
5. ドロップダウンから、アクセスを追加する**アカウント**を選択します。期待するアカウントが表示されない場合は、いくつかの理由が考えられます。 1つは、あなた自身がそのアカウントに対する適切な権限を持っていないということです。もう1つは、そのアカウントが実際には組織にないことです。詳細については、 [アクセスに影響を与える要因を](/docs/accounts/accounts-billing/account-structure/factors-affecting-access-features-data)参照してください。それでも問題が解決しない場合は、アカウント担当者にご相談ください。
