...
[New Relic の使用状況と請求](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing)を理解できるように、請求関連の使用状況を理解するための[使用状況 UI](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#usage-ui)を提供します。また、カスタム NRQL クエリを実行して追加の詳細を取得したり、使用量が特定のレベルに達した場合のアラート通知を作成したりできます。

使用状況 UI を使用するとき、または消費クエリを実行するときは、次の点に注意してください。

* 結果は概算であり、請求金額の計算に使用される値と正確には一致しません。
* これらのツールを使用して、使用率の高い領域を見つけて追跡します。
* クエリを使用してアラートを作成すると、制限に近づいているかどうかがわかります。

## データインジェスト使用量のクエリ [#data-queries]

[使用状況 UI には、](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#usage-ui)時間の経過とともに取り込まれたデータが表示されます。UI が提供するものよりも詳細な情報が必要な場合は、さまざまな NRQL クエリを使用できます。以下の例には 2 つのデータ型が含まれています。

* `NrConsumption`: これを使用して、時間ごとの使用状況の詳細を詳しく調べます。請求書の代理としてはあまり役に立ちません。
* `NrMTDConsumption`: これを使用して、毎月の合計使用量のおおよその概算を確認します。
...
* [取り込みにはカウントされません](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/data-ingest-billing/#usage-calculation)が、`bytecountestimate()`関数によって返されるデータにはいくつかの種類があります。つまり、この関数では、実際の使用量よりも**多くの**データが表示されることがあります。まれに使用量が少な目に表示される場合がありますが、その場合でも差異はごくわずかです。
...
* 当社のAPM、ブラウザモニタリング、モバイルモニタリングでは、[メトリックタイムスライスデータ](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data)と呼ばれるデータ型が報告されます。このデータは他のデータ型とは異なり、NRDBに保存されないため、このデータに対して`bytecountestimate()`を使用しようとすると、混乱を招く可能性があります。このデータをクエリする際は、メトリクス名ではなくアプリケーション名でファセットすることをお勧めします。