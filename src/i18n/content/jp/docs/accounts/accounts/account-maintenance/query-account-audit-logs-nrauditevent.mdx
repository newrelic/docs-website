...
## `NrAuditEvent`とは何ですか? [#attributes]
...
監査ログは、 [監査モード](/docs/agents/manage-apm-agents/configuration/log-audit-all-data-your-new-relic-agent-transmits) の構成とは異なります。 <InlinePopover type="apm"/>エージェント。 APM 監査モードは、アプリから送信されるすべてのデータに関する情報を記録します。
...
<CollapserGroup>
  ...
  <Collapser
    id="target-type"
    title="私のアカウントにはどのようなターゲット タイプがありますか?"
  >
