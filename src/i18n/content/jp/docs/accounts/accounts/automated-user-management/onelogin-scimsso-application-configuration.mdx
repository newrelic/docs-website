...
当社の[自動化されたユーザー管理](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign)により、SCIM を介して ID プロバイダーから New Relic ユーザーとグループをインポートして構成することができます。このガイドでは、New Relic OneLogin SCIM/SSO アプリケーションの設定方法に関する OneLogin 固有の詳細を提供します。
