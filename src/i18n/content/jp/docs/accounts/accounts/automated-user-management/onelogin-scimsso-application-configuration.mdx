...
<CollapserGroup>
  ...
  >
    ...
    <img
      ...
      alt="OneLogin Rulesの条件とアクションに関する詳細ページの画面キャプチャです。"
    />
...
<img
  ...
  alt="OneLogin Provisioningページの画面キャプチャです。"
/>
...
2. **\* **Require admin approval before this action is performed\*\* で、これらのオプションのチェックを外します。
   ...
   * **ユーザーの更新**
     ...
     <Callout variant="tip">これらのオプションをオフにしないと、管理者が承認するまでSCIMプロビジョニング要求は送信されません。
     </Callout>_ 設定 **OneLogin でユーザーが削除された場合、またはユーザーのアプリのアクセス権が削除された場合は、以下の操作を行ってください** ～ **削除** 。_ 設定 **OneLogin でユーザーアカウントが一時停止された場合、以下の操作を行います** ～ **一時停止**.\*\*
...
<img
  ...
  alt="OneLogin Parameterフォームのスクリーンショットで、設定するフィールドを示しています。"
/>
...
2. チェック **Include in User Provisioning**.
   ...
   <img
     ...
     alt="OneLogin Parameterの詳細ページのスクリーンショットです。"
   />
...
10. **Roles** を使って New Relic のケイパビリティグループを定義している場合は、ユーザーのアプリケーションページから、そのユーザーに適した役割をクリックし、 **Save User** をクリックします。### ステップ5.ユーザーのユーザータイプの設定 [#user-type]New Relic でユーザーがプロビジョニングされると、 [**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where) で確認できるようになります。SCIM 経由で New Relic にユーザーを追加しているが、 \*_\*\*SCIM 経由で [ユーザータイプ](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) を管理していない場合、そのユーザーは [ベーシックユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) としてスタートします。ユーザーをアップグレードするには、2つのオプションがあります。_ ユーザーの編集には、ユーザー管理UI [を使用します。](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#edit-user-type)
