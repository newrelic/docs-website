...
当社の [自動ユーザー管理](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) (AUM) では、SCIM を介して ID プロバイダーから New Relic のユーザーをインポートして設定することができます。このガイドでは、New Relic Okta SCIM/SSO アプリケーションの設定方法について、Okta 特有の詳細を説明しています。
...
2. **Integration** フォームから、 **Configure API integration** をクリックします。
3. **Enable API integration** のチェックボックスにチェックを入れます。
4. [ステップ1](#auth-domain) で保存した API トークンを、Okta New Relic アプリの **API トークン** フィールドに入力します。
5. オプション： **Test API credentials** をクリックして、New Relic への SCIM 接続が確立できることを確認します。接続が確立できた場合は、成功メッセージが表示されます。接続が確立されなかった場合は、API トークンを再入力してテストを再実行してください。
6. **Save** をクリックします。保存処理では、API 認証情報のテストが行われることに注意してください。New Relic への接続が確立されていない場合、保存は失敗します。
7. 新しく表示された **To App** フォームで、 **Edit** をクリックします。
8. **Create users** 、 **Update user attributes** 、 **Deactivate users** セクションの **Enable** チェックボックスをチェックします。
9. クリック **保存**.

## ステップ4.ユーザーとグループの割り当て [#assign-users]

まだOktaでユーザーグループを設定していない場合は、ユーザーグループを作成する必要があります。これらのグループは、後に New Relic で [アクセスグラント](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts) を割り当て、特定のアカウントの特定のロールへのアクセス権を与えるグループとなります。グループを作成する方法については、 [Okta のグループに関するドキュメント](https://help.okta.com/en/prod/Content/Topics/users-groups-profiles/usgp-about-groups.htm) を参照してください。
...
1つまたは複数のグループを追加したら、 [**ユーザー管理** UIページ](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) にアクセスして、追加したユーザーを確認できるようにしてください。

## ステップ5.ユーザーのユーザータイプの設定 [#user-type]

New Relic でユーザーがプロビジョニングされると、 [**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) で確認できるようになります。

SCIM 経由で New Relic にユーザーを追加しているが、 ****SCIM 経由で [ユーザータイプ](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) を管理していない場合、そのユーザーは [ベーシックユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure#user-type) としてスタートします。ユーザーを [フルプラットフォームユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure#user-type) に変更するには、2つのオプションがあります。

* ユーザーの編集には、ユーザー管理UI [を使用します。](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)
* [* Oktaからユーザータイプを管理する（後述）。](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)

[ユーザーのユーザータイプをOktaから管理するためのものです。](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)

1. [](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)

2. [](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)

   [New Relic ](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#edit-user-type)[認証ドメインUI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#manage-user-type-scim) にアクセスし、 **Enable Manage user type with SCIM** をクリックします。これを有効にすると、New Relic の UI からユーザータイプを管理することができず、Okta からのみ管理できるようになることに注意してください。
...
5. 次に、このフィールドを持つようにOktaのユーザープロファイルを設定します。手順は以下の通りです。
   ...
   * そのプロファイルに新しい **New Relic ユーザータイプ** 属性を追加します ([Okta ユーザープロファイルの説明を参照](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas))。これをどのように設定するかは、ユーザータイプを定義する際のお客様の設定や好みによります。なお、想定されるユーザータイプの値は、 `ベーシックユーザー` ベーシックユーザーの場合、 `フルユーザー` フルプラットフォームユーザーの場合です。以下は、情報を入力した例です。 ![Oktaのユーザープロファイルに新しい属性を設定する](./images/okta-add-user-type-to-profile.png "Oktaのユーザープロファイルに新しい属性を設定する")
...
7. 次に、その属性のマッピングを設定します。手順をご紹介します。
   ...
   * ユーザータイプをどのように設定したいかに基づいて設定します。ベーシックユーザーやフルプラットフォームユーザーとして設定する理由については、 [ユーザータイプ](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure#user-type) を参照してください。 [Okta 属性マッピングについてはこちらを参照してください。](https://help.okta.com/en/prod/Content/Topics/users-groups-profiles/usgp-about-attribute-mappings.htm)

## ステップ6.アクセスグラントの割り当て [#assign-users]

これらの手順が完了すると、 [ユーザー管理 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) にアクセスして、New Relic でユーザーを確認することができます。ユーザーがNew Relicに存在するようになったので、特定のアカウントの特定のロールへのアクセス権を付与する必要があります。これが行われていない場合、ユーザーはまだ New Relic へのアクセス権を持っていません。この方法については、以下を参照してください。
