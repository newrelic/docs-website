...
5. 次に、このフィールドを持つようにOktaのユーザープロファイルを設定します。手順は以下の通りです。
   ...
   * そのプロファイルに新しい **New Relic ユーザータイプ** 属性を追加します ([Okta ユーザープロファイルの説明を参照](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas))。これをどのように設定するかは、ユーザータイプを定義する際の設定や好みによります。なお、ユーザータイプとして想定される値は、 `ベーシックユーザー` 、 `コアユーザー` 、 `フルユーザー` となります。以下は、情報を入力した例です。 <img title="Oktaのユーザープロファイルに新しい属性を設定する" alt="Oktaのユーザープロファイルに新しい属性を設定する" src={oktaAddUserTypeToProfile}/>
