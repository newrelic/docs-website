...
5. 推奨: ID プロバイダーでタイム ゾーンを設定します。 これを行う方法は、使用するサービスによって異なります。 タイム ゾーンを設定しない場合、UI は UTC タイム ゾーン (IANA 形式、別名「Olson」形式、たとえば「America/Los_Angeles」で指定) を使用します。 ユーザーには、設定を上書きして[独自のタイムゾーンを設定する](/docs/accounts/accounts-billing/general-account-settings/default-time-zone-setting)オプションもあります。
