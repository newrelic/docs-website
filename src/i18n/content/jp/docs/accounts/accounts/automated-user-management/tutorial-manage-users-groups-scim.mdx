...
ユーザーグループをNewRelicに入れる作業が完了したら、**アクセス管理**UIを使用してアクセス許可を作成する必要があります。これにより、グループに特定の役割やアカウントへのアクセスを許可します。詳細については、 [ユーザー管理の概念](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts)を参照してください。
