...
このチュートリアルでは、SCIM API を使って New Relic One のユーザーモデルでユーザーを管理するための一般的な手順を説明します。SCIM API を使用すると、 [ユーザー管理 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) の外で、ユーザーやグループをプログラムで表示、作成、更新、削除することができます。
...
* [ユーザー管理の重要なコンセプト](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts)
...
ユーザーグループのNew Relicへの取り込みが完了したら、 **Organization and access** UIを使用して、アクセスグラントを作成する必要があります。アクセスグラントは、グループに特定のロールやアカウントへのアクセス権を与えるものです。詳しくは、 [user management concepts](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts) をご覧ください。
...
いくつかのユーザとグループを作成した後、それらは [User management UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) で見ることができます。また、SCIM API からそれらを取得することもできます。
...
* New Relic のユーザーは、デフォルトでは [ベーシックユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) としてスタートしますが、一部のユーザーを [フルプラットフォームユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) に変更するオプションがあります。これを行うには、 [**ユーザー管理** UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles#add-users) または [SCIM API](#manage-user-type) を使用します。
...
* ユーザーグループがNew Relicに登録されたら、アクセスグラントを割り当てる必要があります。アクセスグラントとは、ユーザーに特定のロールや特定のアカウントへのアクセス権を与えるものです。 [アクセスグラントについて詳しくはこちらをご覧ください。](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#understand-concepts)

## オプションユーザータイプの管理 [#manage-user-type]

SCIM API の統合が完了すると、New Relic に導入されたすべてのユーザーは基本ユーザーとしてスタートします。 [ユーザータイプ](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) を管理するために、デフォルトの方法である [ユーザー管理 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/add-manage-users-groups-roles/#where) を使用することができます。オプションとして、代わりに SCIM API を使用することもできます。これを行うには、認証ドメインの設定を更新して、 [ユーザータイプの制御を委任する](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#user-upgrade) をアイデンティティプロバイダーまたはカスタムアプリケーションに設定します。
...
* `フルユーザー` （フルプラットフォームユーザー向け）
* `ベーシックユーザー`

新しい `Basic User` を作成するには、 `POST` リクエスト `/scim/v2/Users` を送信し、New Relic のカスタムスキーマエクステンションを含めます。
