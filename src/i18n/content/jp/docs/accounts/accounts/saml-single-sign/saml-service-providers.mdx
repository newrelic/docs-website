...
## NewRelicがサポートするプロバイダー [#saml\_providers][#saml_providers]
...
<CollapserGroup>
  <Collapser id="domain-allow-list" title="推奨：SAMLSSOプロセスをバイパスするようにドメインを要求します">
  ...
  <Collapser id="set-up-saml" title="SAMLSSOを設定する">
    ...
    7. 変更を保存します。
       ...
       ### セッションタイムアウトのログアウトURLを追加します [#timeout\_url][#timeout_url]
  ...
  <Collapser id="add-users-saml" title="SAMLアカウントにユーザーを追加する">
    SAML SSOおよびSCIMドキュメントの概要については、最初にSAMLSSOおよびSCIMの[概要](/docs/accounts/accounts-billing/new-relic-one-user-management/introduction-saml-scim)をお読みください。
  ...
  <Collapser id="saml-multiple-accounts" title="複数のアカウントのSAML">
  ...
  <Collapser id="maintain-saml" title="SSO設定を維持する">
    ...
    ### SAMLSSO情報を更新する [#sso\_procedures][#sso_procedures]
    ...
    5. オプション: 既存の SSO URL を変更するには、 <DNT>**Remote login URL**</DNT>または<DNT>**Logout landing URL**</DNT>をコピーして貼り付け (または入力)、保存します。
       ...
       ### メールアドレス [#user\_email][#user_email]
  ...
  <Collapser id="delete-sso" title="SSO構成を削除する">
    この機能が適用されるNewRelicユーザーを含む要件については、[要件](#requirements)を参照してください。
  ...
  <Collapser id="partners-saml" title="パートナーとSAMLSSO">
