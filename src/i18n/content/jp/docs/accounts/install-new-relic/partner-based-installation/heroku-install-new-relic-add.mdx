...
[Heroku](https://devcenter.heroku.com/articles/newrelic)は、さまざまなエージェント言語で Web アプリケーションをホストするための Platform as a Service (PaaS) ソリューションです。 New Relic を使用すると、 <InlinePopover type="apm"/>や<InlinePopover type="browser"/>などの監視ソリューションのメトリクスを使用して Heroku を拡張できます。 New RelicアドオンはJava、Node.js、 PHP、Python、Ruby。
...
[Heroku 用の New Relic アドオン](https://devcenter.heroku.com/articles/newrelic)をインストールしたら、 <InlinePopover type="apm"/>エージェントの設定手順に従います。
...
<CollapserGroup>
  ...
  >
    ...
    弊社の Ruby エージェントを Rails 以外のアプリケーションで使用している場合、Heroku ユーザーはリポジトリにプラグインを手動でインストールする必要があります。 たとえば、Sinatra アプリでは、 `newrelic` gem を<DNT>**Gemfile**</DNT>に追加し、次のコードをアプリに追加します。
...
<CollapserGroup>
  <Collapser
    ...
    title="ライセンスキー"
  >
    <InlinePopover type="licenseKey"/>は、アプリケーションがレポートするアカウントを識別します。 アプリが使用しているライセンスキーを確認するには:
...
<CollapserGroup>
  ...
  >
    ...
    3. アドオンのリストから「New Relic」を選択します。
       ...
       Heroku 経由でサインインする場合、 <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) > Switch account**</DNT>を選択しても通常の New Relic アカウントは表示されません。
  </Collapser>
  ...
  >
    ...
    2. 通常の New Relic アカウントから別のアカウントに切り替えるには:[ユーザー メニュー](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)に移動し、アカウントを選択します。
       ...
       New Relic から直接サインインする場合、 <DNT>**[user menu](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings) > Switch account**</DNT>を選択しても Heroku の New Relic アドオン アカウントは表示されません。
  </Collapser>
</CollapserGroup>

## デプロイメント通知の設定 [#deployment]

Heroku アドオンは、アカウントごとに<DNT>**one application**</DNT>のデプロイメント通知を New Relic に自動的に送信します。 アドオン アカウントに複数のアプリケーションを追加する場合は、New Relic REST API を使用して、追加アプリケーションのデプロイメント通知を手動で送信する必要があります。
...
<Callout variant="important">
  Heroku アドオンにユーザーを追加すると、New Relic にそのユーザーのユーザー レコードが作成されます。 ただし、Heroku アドオンからユーザーを削除しても、ユーザー レコードは New Relic から自動的に削除されません。 代わりに、Heroku アドオンからユーザーを削除した後、New Relic ユーザー レコードも手動で削除する必要があります。 <DNT>**User Management**</DNT>ページに移動するとこれを実行できます。
</Callout>
