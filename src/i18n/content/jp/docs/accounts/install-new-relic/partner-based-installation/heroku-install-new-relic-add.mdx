...
[Heroku は、](https://devcenter.heroku.com/articles/newrelic)さまざまなエージェント言語で Web アプリケーションをホストするための Platform as a Service (PaaS) ソリューションです。New Relic を使用すると、次のような監視ソリューションのメトリクスを使用して Heroku を拡張できます。 <InlinePopover type="apm"/>そして<InlinePopover type="browser"/> 。New Relic アドオンは Java、Node.js、PHP、Python、Ruby。
