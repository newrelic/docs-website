...
2. カスタムイベントまたは属性を作成する前に、New Relicの[NRQLが使用する予約語](/docs/insights/insights-data-sources/custom-data/insights-custom-data-requirements-limits#reserved-words)のリストをご確認ください。
...
5. **推奨事項**：[NRQLアラート条件](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)をセットアップして、[パーシングエラー](#errors-parsing)の発生時に通知が送られるようにします。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            APIは、キー/値ペアのみを受け付けており、マップ/オブジェクトや配列には対応していません。このAPIは、文字列と数値（整数または浮動小数点数）に対応しています。詳細については、 [データ要件](/docs/insights/insights-data-sources/custom-data/insights-custom-data-requirements-limits)をご覧ください。
          </td>
        </tr>
...
**推奨事項**：成功メッセージの確認に加え、[Insightsデータエクスプローラー](/docs/insights/use-insights-ui/explore-data/event-explorer-query-chart-your-event-data)を使用することで、イベントが適切にレポートされていることを確認し、クエリを生成できます。
...
`NrIntegrationError`イベントでは、ご利用のNew Relicアカウントに送信されるカスタムデータをクエリしてアラートを設定できます。**推奨事項**：New Relic Alertsからパーシングエラーに関する通知を受け取るには、`NrIntegrationError`向けに[NRQL条件](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)を作成します。以下のNRQLクエリの例を使用してください：
...
クエリの方法の詳細については、[クエリデータ](/docs/using-new-relic/data/understand-data/query-new-relic-data)をご覧ください。
