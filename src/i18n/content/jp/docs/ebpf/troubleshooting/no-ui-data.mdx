...
3. **データ フィルタリング設定を確認します**。エンティティが`allDataFilters.dropServiceNameRegex`や`allDataFilters.dropNamespaces`などの設定によって除外されていないことを確認します。

4. **永続的なgRPC/HTTP/2接続を持つアプリケーションの再起動**：エージェントがエンティティを正しく生成し、トラフィックをモニターできるように、永続的なgRPC/HTTP/2接続を持つアプリケーションは、eBPFエージェントの起動後に再起動する必要があります。エージェントが起動する前にgRPCまたはHTTP/2アプリケーションが実行されていた場合は、エージェントが新しく確立された接続をフックできるように、アプリケーションを再起動してください。
