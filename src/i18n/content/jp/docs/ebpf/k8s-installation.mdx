...
<CollapserGroup>
  <Collapser id="general-configuration" title="一般設定">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            マルチアカウントのOTLPエクスポートルーティング用に、各KubernetesネームスペースをNew Relicライセンスキーにマッピングします。マップに存在しないネームスペースは、グローバルな
        ...
        <tr>
          ...
          <td>
            JSONエンコードされたネームスペースからライセンスキーへのマッピングを持つ

            `NAMESPACE_LICENSE_KEY_MAP`

            キーを含む、既存のKubernetesシークレットを指定します。values.yamlにライセンスキーを保存しないようにするには、
        ...
        <tr>
          ...
          <td>
            New Relicアカウントのリージョンを指定します。受け入れられる値：
        ...
        <tr>
          ...
          <td>
            ...
            ）を有効にします。これはFedRAMP準拠を有効にするための推奨される方法であり、リージョンベースの検出（
        ...
        <tr>
          ...
          <td>
            ...
            （アタッチされたAPMエージェント自体がアクティブにログを収集している場合、またはOTelエージェントがアタッチされている場合を除き送信）。詳細については、

            [自動検出とバックオフ](/docs/ebpf/logs/#automatic-backoff)
        ...
        <tr>
          ...
          <td>
            エージェントおよびエンティティのメタデータレポートを有効にします。デフォルトでは、エージェントはHTTP経由でエージェントおよびエンティティのメタデータをNew Relicにエクスポートします。メタデータレポートの HTTP エクスポートパスを完全に無効にするには、これを 
        ...
        <tr>
          ...
          <td>
            OTLPエンドポイントでTLSが有効になっているかどうか。

            `customOtlpEndpoint`

            とこのフィールドの両方が明示的に設定されている場合にのみ上書きされます。
          </td>
        ...
        <tr>
          ...
          <td>
            カスタムOTLPエンドポイント用のCA証明書を含むKubernetesシークレットの名前。
        ...
        <tr>
          ...
          <td>
            企業プロキシ経由でOTLPテレメトリーデータをルーティングするためのHTTP/HTTPSプロキシホスト名。空の場合、プロキシは使用されません。
          </td>
        ...
        <tr>
          ...
          <td>
            プロキシBASIC認証のユーザー名。空の場合、認証は使用されません。
          </td>
        ...
        <tr>
          ...
          <td>
            ...
            と組み合わせてのみ使用されます。セキュリティのため、パスワードを直接設定するのではなく、
        ...
        <tr>
          ...
          <td>
            プロキシ資格情報を含む既存のKubernetesシークレットの名前。シークレットには、キー
        ...
        <tr>
          ...
          <td>
            ...
            （エンティティのAIモニタリングデータをすでにレポートしている言語APMまたはOTelエージェントがない場合にのみ送信）。
          </td>
        ...
        <tr>
          ...
          <td>
            生成AIスパンをエクスポートするためのレイテンシベースのサンプリング閾値を定義します。
        ...
        <tr>
          ...
          <td>
            生成AIルートのエラー率の閾値を定義します。この閾値を超えると、ルートの対応するスパンがエクスポートされます。オプション：
        ...
        <tr>
          ...
          <td>
            ...
            の場合、生成AIインタラクションの完全なプロンプトと完了コンテンツをキャプチャします。
        ...
        <tr>
          ...
          <td>
            HTTPパスの正規化パターンを定義します。各パターンには、少なくとも1つの

            `*`

            ワイルドカードが含まれている必要があります。パスをそのまま維持する（自動クラスタリングをスキップする）には、除外パターンに

            `!`

            プレフィックスを使用します。エージェントは自動クラスタリングの前にこれらのパターンを適用し、これらは自動クラスタリングよりも優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            HTTPリクエスト/レスポンスの本文からキャプチャされる最大バイト数を設定します。デフォルト: 
            ...
            （1 KB）。New RelicがAIモニタリングのペイロードを切り捨てる場合は、この値を増やします。
          </td>
        ...
        <tr>
          ...
          <td>
            eBPF エージェントが報告するすべてのエンティティに追加されるカスタムラベル。これらのラベルは、環境変数
    ...
    <Callout variant="important">
      <DNT>eBPF agent</DNT>は、<DNT>`http://`</DNT>スキームを使用するHTTP CONNECTプロキシのみをサポートします。gRPCの制限により、<DNT>`https://`</DNT>スキーム（プロキシへの直接のTLS暗号化接続）はサポートされていません。ただし、テレメトリーデータは、エージェントとNew Relic OTLPエンドポイント間で常に**エンドツーエンド**でTLS暗号化されます。<DNT>`http://`</DNT>プレフィックスは、トンネルを確立するためにエージェントがプロキシと通信する方法のみを規定します。プロキシ自体は暗号化されたトラフィックのみを認識し、お客様のデータにアクセスすることはありません。完全なセットアップ手順については、[<DNT>eBPF agent</DNT>のプロキシの構成](/docs/ebpf/proxy-configuration)を参照してください。
    </Callout>
  </Collapser>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            エージェントがすべてのデータを破棄すべきKubernetesネームスペースのリスト。ネームスペースをフィルタリングするには、dropNamespacesまたはkeepNamespacesのいずれかを使用し、両方は使用しないでください。両方が指定された場合、keepNamespacesが優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            エージェントがすべてのデータを送信すべきKubernetesネームスペースのリスト。ネームスペースをフィルタリングするには、dropNamespacesまたはkeepNamespacesのいずれかを使用し、両方は使用しないでください。両方が指定された場合、keepNamespacesが優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            すべてのデータをフィルタリングするために一致させるポッドのラベル。空のマップは、ラベルに基づくフィルタリングが行われないことを意味します。例：

            `{ "app": "frontend", "env": "production" }`

            。ポッドラベルに基づくフィルタリングには、dropPodLabelsまたはkeepPodLabelsのいずれかを使用します。両方を使用することはできません。両方が提供された場合、keepPodLabelsが優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            すべてのデータを保持するために一致するポッドラベル。空のマップは、ラベルに基づくフィルタリングが行われないことを意味します。例：

            `{ "app": "frontend", "env": "production" }`

            。ポッドラベルに基づくフィルタリングには、dropPodLabelsまたはkeepPodLabelsのいずれかを使用し、両方は使用しないでください。両方が指定された場合、keepPodLabelsが優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            ドロップするk8sサービス名に一致する正規表現を定義します。例：

            `"kube-dns|otel-collector|\\bblah\\b"`

            。サービス名をフィルターするには、dropServiceNameRegex または keepServiceNameRegex のいずれかを使用します。両方は使用しないでください。両方が指定された場合、keepServiceNameRegexが優先されます。
          </td>
        ...
        <tr>
          ...
          <td>
            ...
            の設定のバイパスとして機能します。この正規表現に一致するサービス名は、

            `dropServiceNameRegex`

            によってデータがドロップされません。サービス名をフィルターするには、dropServiceNameRegex または keepServiceNameRegex のいずれかを使用します。両方は使用できません。両方が指定された場合、keepServiceNameRegexが優先されます。このフィールドは

            `allowServiceNameRegex`

            から名前が変更されました。古い名前は非推奨ですが、下位互換性のために引き続きサポートされます。
          </td>
  ...
  <Collapser id="log-data-filters" title="ログデータフィルター">
    eBPFエージェントが収集およびレポートするアプリケーションログを制御するようにフィルターを設定します。これらの設定は、`reportLogs`が`"true"`または`"auto"`に設定されている場合にのみ有効になります。
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            以下のフィルターに一致するエンティティからのログ収集を有効にします。
          </td>
        ...
        <tr>
          ...
          <td>
            エンティティから1分あたりに収集するログサンプルの最大数。この制限に達すると、収集サイクル全体で均等な分布を維持するためにイベントがサンプリングされます。
          </td>
        ...
        <tr>
          ...
          <td>
            STDOUTおよびSTDERRからのログを保持するエンティティ名に一致する正規表現。すべてのエンティティのログを転送するには、
        ...
        <tr>
          ...
          <td>
            ログファイルからログを保持するためにエンティティ名と一致する正規表現。

            `.*`

            を使用して、すべてのエンティティのログを転送します。
          </td>
  ...
  <Collapser id="protocol-tracing" title="プロトコルトレース設定">
    ...
    * DNS
    ...
    両方のエンジンがMySQLと同じワイヤプロトコルを使用するため、MySQLプロトコルトレース（`protocols.mysql`）はMariaDBおよびAurora MySQLも対象とします。設定には個別の`mariadb`または`aurora_mysql`セクションは必要ありません；`protocols.mysql`を有効または無効にすることで、サポートされているすべてのエンジンのトレースを制御します。エージェントは、サーバーのハンドシェイク中に接続ごとの特定のデータベースの種類を自動的に検出し、各レコードで報告します。この検出を手動で設定することはできません。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            スパンをエクスポートするためのレイテンシベースのサンプリング閾値を定義します。
