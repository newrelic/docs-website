...
<Steps>
  <Step>
    ...
    * `host-tower-portland` 重大なアラート違反があります。
  ...
  <Step>
    ...
    ここでは CPU の割合を扱っているため、問題がアプリに関連しているのか、マシンに関連しているのかを判断したいと考えています。これを行うには、サイドバーの **Latest events** \[最新のイベント] に対して **Processes running** \[プロセス実行] テーブルを確認します。このステップでは、マシンに直接変更が加えられたために問題が発生しているのか、それともプロセスがリソースを使い果たしているのかを明確にします。
  ...
  <Step>
    ...
    * より多くのエンドユーザーがそのコンポーネントにアクセスし、システムにストレスを与えています。この場合、その負荷を満たすためにより多くのリソースをプロビジョニングする必要があります。
    ...
    `host-tower-portland`の概要ページを見ると、異なるチャート間の時系列パターンを相関させることで、この状況にどのシナリオが当てはまるかを特定できます。ネットワーク トラフィックをインフラストラクチャのメトリクスと比較してみましょう。

    * この場合、 **Network traffic** \[ネットワーク トラフィック] グラフが CPU 使用率、メモリ使用量などのメトリック グラフと同様の傾向を示すことが期待されます。
    * リソースが負荷ではなくアプリの問題に関連している場合、ネットワーク時系列は通常の傾向、つまり急増や谷のない傾向になります。
    * ただし、負荷が CPU 使用率の高さの原因である場合は、ネットワーク時系列が他のメトリック グラフ全体の傾向を反映していることに気づくでしょう。

    **CPU usage** \[CPU 使用率] と **Network traffic** \[ネットワーク トラフィック]を比較してみましょう。
    ...
    * ただし、ネットワーク トラフィックは一貫した動作を示しています。リソースが枯渇しても、全体的な動作は時間の経過とともに変化するようには見えません。代わりに、時系列には規則的な山と谷が表示されます。
