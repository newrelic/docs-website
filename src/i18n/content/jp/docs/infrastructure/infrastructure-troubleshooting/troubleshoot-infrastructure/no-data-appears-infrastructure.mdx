...
インフラストラクチャエージェントをインストールして数分待っても、[インフラストラクチャ UI](/docs/infrastructure/infrastructure-ui-pages/infra-ui-overview)にデータが表示されません。
...
1. パッケージマネージャを使用して、Infrastructureエージェントがインストールされていることを確認します。
   ...
   <CollapserGroup>
     <Collapser id="apt-install-verify" title="aptのインストールを確認する（DebianまたはUbuntu）">
     ...
     <Collapser id="yum-install-verify" title="yumのインストールを確認する（Amazon Linux、CentOS、RHEL）">
     ...
     <Collapser id="windows-install-verify" title="Windows Serverのインストールを確認する">
...
2. [お使いのinitシステム](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status#init-system)で、エージェントが実行中であることを確認します。
   ...
   <CollapserGroup>
     <Collapser id="systemd-verify-status" title="SystemDでステータスを確認する">
     ...
     <Collapser id="systemv-verify-status" title="System Vでステータスを確認する">
     ...
     <Collapser id="upstart-verify-status" title="Upstartでステータスを確認する">
     ...
     <Collapser id="windows-verify-status" title="Windowsでステータスを確認する">
