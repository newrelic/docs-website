...
NewRelicとGoogleKubernetesEngine（GKE） [の統合により](/docs/infrastructure/integrations-getting-started/getting-started/introduction-infrastructure-integrations)、 [Google](https://cloud.google.com/kubernetes-engine/)のインフラストラクチャでホストされているクラスターに関するデータが報告されます。 [New RelicのインフラストラクチャUI](/docs/infrastructure)からGKEインスタンスのデータを監視してアラートを送信したり、[カスタムクエリやグラフダッシュボード](/docs/insights)を作成したりできます。
...
[インベントリデータ](/docs/integrations/new-relic-integrations/getting-started/understand-use-data-infrastructure-integrations#overview)は、サービスまたはホストのステータスまたは構成に関する情報です。 UIで在庫データを調べることができます。
