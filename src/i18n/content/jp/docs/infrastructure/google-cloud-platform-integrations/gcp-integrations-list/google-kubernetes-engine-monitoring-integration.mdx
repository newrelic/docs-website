...
統合を有効にして数分待つと ( [ポーリング頻度](#polling)に基づいて)、New Relic UI にデータが表示されます。ダッシュボへのリンクを含む [データを見つけて使用する](/docs/infrastructure/integrations/find-use-infrastructure-integration-data)には<InlinePopover type="dashboards"/>ards とアラートの設定については、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > GCP > (統合を選択)**に移動します。
