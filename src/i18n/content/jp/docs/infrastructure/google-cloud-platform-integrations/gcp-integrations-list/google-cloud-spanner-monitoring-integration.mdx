...
<CollapserGroup>
  <Collapser id="entities-table" title="Spannerエンティティ">
    各Spannerインスタンスに対して単一の`GCPSPANNERINSTANCE`エンティティが合成されます。`spanner_instance`および`spanner.googleapis.com/Instance`のGCPモニターリソースタイプの両方に対して発行されたメトリクスは、このエンティティにアタッチされ、該当する場合は`database`でファセット化されます。
