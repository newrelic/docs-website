...
デフォルトでは、 <DoNotTranslate>**highly recommend**</DoNotTranslate> GCP 基本ロール`Viewer`を使用します。これにより、「[既存のリソースやデータの表示 (ただし変更ではない) など、クラウドインフラストラクチャの状態に影響を与えない読み取り専用アクションの権限」が付与されます。](https://cloud.google.com/iam/docs/understanding-roles) このロールは Google によって自動的に管理され、新しい Google クラウド サービスがリリースまたは変更されると更新されます。
