...
<CollapserGroup>
  <Collapser id="service" title="サービスアカウント（推奨）">
    ...
    組織で[ドメイン制限の制約](https://cloud.google.com/resource-manager/docs/organization-policy/restricting-domains)を使用している場合は、組織が適用している制約に応じて、New Relicを許可するようにポリシーを更新する必要があります：

    * **`iam.allowedPolicyMemberDomains`**：New RelicのドメインID`C02x1gp26`を許可します。
    * **`iam.managed.allowedPolicyMembers`**: New Relic GCP組織ID`740422149647`またはGCPプロジェクト番号`397727728820`を許可します。
