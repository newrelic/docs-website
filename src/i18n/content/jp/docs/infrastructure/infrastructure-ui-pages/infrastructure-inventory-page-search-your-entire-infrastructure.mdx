...
<CollapserGroup>
  ...
  >
    1 つ以上のホストの [アラートしきい値インシデント](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-threshold)を表示するには、ホストの **Critical** \[重大] を選択します。 <Icon name="fe-x-circle"/> アイコンまたは **Warning** \[警告] <Icon name="fe-alert-triangle"/> アイコン。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    * テスト環境
    ...
    1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure** に移動し、 **Inventory** \[インベントリ] または **Events** \[イベント を]クリックします。
