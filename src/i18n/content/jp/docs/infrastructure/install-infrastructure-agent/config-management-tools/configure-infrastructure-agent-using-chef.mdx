...
New Relic クックブックは [一般公開されている Chef Supermarket](https://supermarket.chef.io/cookbooks/newrelic-install) で入手でき、New Relic Observability の取り組みを拡大するのに役立ちます。これを実現するために、 [New Relic CLI](https://github.com/newrelic/newrelic-cli) と [New Relic Open Installation リポジトリ](https://github.com/newrelic/open-install-library) を使用します。これは [コミュニティの支援による](https://github.com/newrelic/chef-install) 取り組みです。
...
現在、New Relic のインフラストラクチャとログの統合に Linux と Windows のサポートが含まれており、近い将来さらに多くのエージェントと統合が続く予定です。
...
### `newrelic_install` レシピを実行します [#execution]
