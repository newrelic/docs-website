...
サポートされている [Amazon Web Services のアカウントを持っていることを確認してください](http://aws.amazon.com/elasticbeanstalk/) 。また、New Relic のインストールには、 [New Relic のライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) が必要です。これは、 [アカウントのサインアップ](http://newrelic.com/signup "リンクが新しいウィンドウで開きます") の際に New Relic が提供する 40 文字の 16 進数の文字列です。
...
セットアップが成功した後、メトリクスが New Relic に表示されるまでに最大 15 分かかる場合があります。**[one.newrelic.com](https://one.newrelic.com)**でホストのインフラストラクチャ ページを表示します。
