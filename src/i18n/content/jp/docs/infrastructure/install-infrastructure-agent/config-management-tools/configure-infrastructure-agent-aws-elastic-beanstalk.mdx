...
サポートされている[Amazon Web Services アカウント](http://aws.amazon.com/elasticbeanstalk/)を持っていることを確認してください。さらに、New Relic のインストールには、 [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)が必要です。 これは、アカウントの[サインアップ](http://newrelic.com/signup "リンクが新しいウィンドウで開きます")時に New Relic が提供する 40 文字の 16 進数の文字列です。
...
2. オペレーティング システムに基づいて、次の内容をファイルに追加し、 `YOUR_LICENSE_KEY` New Relic [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)に置き換えます。
