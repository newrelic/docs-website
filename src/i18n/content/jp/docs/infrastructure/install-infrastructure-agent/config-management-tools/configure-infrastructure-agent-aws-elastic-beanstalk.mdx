...
サポートされている[Amazon Web Services アカウント](http://aws.amazon.com/elasticbeanstalk/)を持っていることを確認してください。さらに、New Relic のインストールには、<LicenseKey/>が必要です。これは、アカウントの[サインアップ](http://newrelic.com/signup "リンクが新しいウィンドウで開きます")時に New Relic が提供する 40 文字の 16 進数の文字列です。
...
2. オペレーティング システムに基づいて、次の内容をファイルに追加し、 `YOUR_LICENSE_KEY`を New Relic<LicenseKey/>に置き換えます。
