...
`newrelic_installer` モジュールを使用して、New Relic のインフラストラクチャ エージェント、ログ統合、PHP APM エージェントをインストールして構成する方法を学びます。Puppet の使用方法の説明については、 [Puppet のドキュメントを](https://docs.puppet.com)参照してください。これは [コミュニティの支援による](https://github.com/newrelic/puppet-install) 取り組みです。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `'logs'` - New Relic Infrastructure Agent のログ統合。\*_ `'infrastructure'`が必要です_
