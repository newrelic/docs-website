...
New Relic のインフラストラクチャ監視エージェントでの Ansible ロールには、 [サポートされているオペレーティング システムが](/docs/infrastructure/new-relic-infrastructure/getting-started/compatibility-requirements-new-relic-infrastructure#operating-systems)必要です。
...
<CollapserGroup>
  ...
  >
    ...
    * `NEW_RELIC_MYSQL_USERNAME` (オプション) 他に指定がない場合、デフォルトは `newrelic` になります。これは、 `mysql` 統合がセットアップするユーザー名であり、データ レポートの目的で統合の構成ファイル (例: `mysql-config.yml`) にも設定されます。詳細については、 [「MySQL 統合」](https://docs.newrelic.com/install/mysql/)を参照してください。
    * `NEW_RELIC_MYSQL_PASSWORD` (オプション) `NEW_RELIC_MYSQL_USERNAME`で指定されたユーザーのパスワード。詳細については、 [「MySQL 統合」](https://docs.newrelic.com/install/mysql/)を参照してください。
...
>
  ...
  * `apm-nodejs` エージェントのインストールは、 [PM2](https://pm2.keymetrics.io/)によって管理されるアプリでのみサポートされます。 `npm` や `yarn` などのパッケージ マネージャーを使用するか、他のインストール パスを介してエージェントをインストールするには、 [ドキュメント](https://docs.newrelic.com/docs/apm/agents/nodejs-agent/installation-configuration/install-nodejs-agent/)を参照してください。
