   ...
   <CollapserGroup>
     ...
     >
       ...
       <Callout variant="important">
         zypperパッケージマネージャには、GPGキーが予期したとおりに検証されない可能性があるという既知の問題があります。`Signature verification failed`などのエラーが発生した場合は、New Relicの[サポートフォーラム](https://discuss.newrelic.com/t/new-relic-infrastructure-installation/121241/3)で詳細をご覧ください。推奨される代替手段として、（`gpg --import`ではなく）`rpm --import`を使用してGPGキーをRPMの信頼済みキーのデータベースにインポートすることができます。Zypperはそこからキーを取得します。
       </Callout>
     </Collapser>
   </CollapserGroup>
