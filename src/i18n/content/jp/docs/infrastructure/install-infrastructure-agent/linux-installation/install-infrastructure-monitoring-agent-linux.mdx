...
Infrastructure モニタリングエージェントの使用を開始する際に最も迅速な方法では、[ガイド付きインストール](/docs/new-relic-one/use-new-relic-one/cross-product-functions/install-configure/new-relic-guided-install-overview/)を使用します。EU にお住まいの場合は、EU ガイド付きインストールをお試しください。いずれにしても、アカウントをお持ちでない場合は、New Relic アカウントが必要です。（永久無料です。）
...
ガイド付きインストールが機能しない場合は、エージェントを手動でインストールできます。Infrastructure をインストールする前に、次の点を確認してください。
...
- 有効な[New Relic ライセンスキー](/docs/accounts/install-new-relic/account-setup/license-key)がある。

Infrastructure を Linux でインストールするには、次の指示に従ってください。
...
4.  Infrastructure モニタリングエージェント リポジトリを追加します。
...
6.  `newrelic-infra`パッケージをルート（デフォルト）または特権、非特権ユーザーモードでインストールします。各実行モードの詳細については、[Linux エージェントの実行モード](/docs/infrastructure/install-configure-infrastructure/linux-installation/linux-agent-running-modes)を参照してください。
  ...
  <Collapser
    ...
    title="ルート（デフォルト）"
  >
    * **<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="Debian.png" alt="Debian.png" src={debian}/> Debianおよび<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="ubuntu アイコン" alt="ubuntu アイコン" src={ubuntuIcon}/> Ubuntu**：

          ```
          sudo apt-get install newrelic-infra -y
          ```

        * **<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="amazon linux.png" alt="amazon linux.png" src={amazonLinux}/> Amazon Linux、<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="centosアイコン" alt="centosアイコン" src={centosIcon}/>CentOS、<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="redhat アイコン" alt="redhat アイコン" src={redHatNew2}/> RHEL**：

          ```
          sudo yum install newrelic-infra -y
          ```

        * **<img style={{ width: '32px', height: '32px', verticalAlign: 'middle'}} title="suse アイコン" alt="suse アイコン" src={suseIcon}/> SLES**:
...
Infrastructure モニタリングエージェントをインストールまたはアップデートした後、[エージェントのステータスを開始、停止、または確認](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status)できます。
...
設定管理ツールを使って Infrastructure モニタリングエージェントをインストールするには、以下に関するドキュメントをご覧ください。

- <img title="Ansible" alt="Ansible" src={ansibleRedLogo} />
...
- <img title="Puppet" alt="Puppet" src={puppetLogo} />

  [Puppet](/docs/infrastructure/new-relic-infrastructure/config-management-tools/configure-new-relic-infrastructure-puppet)

## インストゥルメントされたホストで Docker コンテナ向けにインストール [#docker-root]

「[Infrastructure モニタリング向け Docker インストゥルメンテーション](/docs/infrastructure/new-relic-infrastructure/data-instrumentation/docker-instrumentation-infrastructure)」を参照してください。

## Azure 拡張機能を使用してインストール [#azure-extension]

「[Infrastructure 用の Azure 拡張機能](/docs/infrastructure/install-infrastructure-agent/linux-installation/azure-extensions-infrastructure)」を参照してください。

## tarball ファイルを使用してインストール [#custom-setups-tarball]

**カスタム**設定シナリオでは、[アシスト](/docs/infrastructure/install-configure-manage-infrastructure/linux-installation/tarball-assisted-install-infrastructure-linux)または[手動](/docs/infrastructure/install-configure-manage-infrastructure/linux-installation/tarball-manual-install-infrastructure-linux)モードで[tarball ファイル](https://download.newrelic.com/infrastructure_agent/binaries/linux/)を使用して、Infrastructure モニタリングエージェントをインストールできます。

これは、環境にデフォルトのインストレーション設定を適用する場合、またはリポジトリに`newrelic-infra`パッケージがないディストリビューションに Infrastructure モニタリングエージェントをインストールする必要がある場合に特に有用です。
...
標準的な手順に従って[Infrastructure モニタリングエージェントをアップデート](/docs/update-infrastructure-agent)してください。
...
トラフィックが生成されてから数分待ってから、[New Relic One UI でホストを確認](/docs/infrastructure-metrics)します。[データが表示されない](/docs/infrastructure/new-relic-infrastructure/troubleshooting/no-data-appears-infrastructure)場合、必要に応じてトラブルシューティングプロセスに従ってください。
