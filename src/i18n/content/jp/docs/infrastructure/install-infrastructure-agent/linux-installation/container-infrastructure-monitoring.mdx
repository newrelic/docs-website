...
Linux 用のインフラストラクチャ監視エージェントは、デフォルトで [コンテナ環境をサポートします](/docs/infrastructure/install-infrastructure-agent/linux-installation/docker-instrumentation-infrastructure) 。コンテナ OS を実行している場合、またはエージェントをコンテナとして展開する必要がある制限がある場合は、インフラストラクチャ監視エージェントのコンテナ化されたバージョンを実行できます。これにより、コンテナ自体だけでなく、基盤となるホストのメトリクスも監視できます。
...
Linux 用のインフラストラクチャ監視エージェントを試してみませんか?New Relic アカウントを無料で作成しましょう!クレジットカードは必要ありません。
...
1. New Relic で `newrelic-infra.yml` エージェント [構成ファイル](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) を作成します <InlinePopover type="licenseKey"/>.構成オプションの説明については、 [構成設定 を](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings)参照してください。
...
3. New Relic で `newrelic-infra.yml` エージェント [構成ファイル](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent) を作成します <InlinePopover type="licenseKey"/>.構成オプションの説明については、 [構成設定 を](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings)参照してください。
