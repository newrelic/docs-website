...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        バインドは、ホストのルート ボリュームをコンテナにマウントします。ホストのルートへの読み取り専用アクセスにより、エージェントはプロセスとストレージのメトリックだけでなく、ホストからインベントリ データも収集できるようになります。 `containerd` ランタイムを操作する場合は、このマウントを `/run/containerd/containerd.sock:/run/containerd/containerd.sock`に置き換えます。
      </td>
    </tr>
