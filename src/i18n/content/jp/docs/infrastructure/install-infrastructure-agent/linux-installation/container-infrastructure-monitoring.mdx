...
<DNT>**Recommendation:**</DNT> [`newrelic/infrastructure`イメージ](https://hub.docker.com/r/newrelic/infrastructure/)を拡張し、独自の`newrelic-infra.yml`エージェント[構成ファイル](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent)を使用します。 イメージがビルドされると、追加の設定を行うことなく、コンテナを簡単に起動できます。 dockerで環境変数を使用してシークレットを提供しないでください。
