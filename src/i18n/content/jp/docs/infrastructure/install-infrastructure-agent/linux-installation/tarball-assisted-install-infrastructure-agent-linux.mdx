...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        エージェントの特権レベル。可能な値は、 `ROOT` 、 `PRIVILEGED` 、または`UNPRIVILEGED`です。詳細については、 [エージェントの実行モード](/docs/infrastructure/new-relic-infrastructure/installation/install-infrastructure-linux#agent-mode-intro)に関するドキュメントを参照してください。
