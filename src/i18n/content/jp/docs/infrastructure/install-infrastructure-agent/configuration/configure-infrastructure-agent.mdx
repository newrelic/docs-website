...
設定ファイルのテンプレートは、 [Infrastructure config file template](https://github.com/newrelic/infrastructure-agent/blob/master/assets/examples/infrastructure/newrelic-infra-template.yml.example) にあります。また、 [環境変数](#Environment_Variables) を設定してコンフィグファイルを上書きしたり、 [コンフィグ管理ツール](#config-mgmt) （Ansible や Puppet など）を使用することもできます。
...
![インフラストラクチャ設定のカスケード（優先順位）.png](src/images/infrastructure-config-cascade-%28orders-of-precedence%29.png 'インフラストラクチャ設定のカスケード（優先順位）.png')
...
<Callout variant="important">
  構成設定の理解を深めるためには、各変数の詳細な説明を含む [Infrastructure
  configuration
  settings](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings)
  を参照してください。
</Callout>

## 設定ファイルの構成 [#config_structure]

`newrelic-infra.yml` を編集する際には、 [YAML フォーマット](https://en.wikipedia.org/wiki/YAML) に準拠し、以下のルールを遵守する必要があります。

- 設定オプションのケーシングを変更しないでください。YAML のキーは大文字と小文字を区別します。
- インデントレベルを尊重する。すべてのインデントは同じような増分で、通常は 2 つのスペース文字を使用します。ファイルの同じスタンザ内のデータは、同じレベルのインデントを使用する必要があります。サブスタンザのインデントは、さらにスペース 2 文字分を追加してください（ [カスタム属性](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings#custom-attributes) および [ネットワークインターフェースフィルタ](/docs/infrastructure/install-configure-infrastructure/configuration/infrastructure-configuration-settings#network-interface-filters) の例を参照）。
...
<Callout variant="important">
  [設定を変更した後は、必ずエージェント](/docs/infrastructure-start-stop-restart-check-agent-status)
  またはウェブサーバーを再起動してください。
</Callout>
