...
<CollapserGroup>
  ...
  >
    を指定します <InlinePopover type="licenseKey"/>New Relic アカウント用。 エージェントはこのキーを使用して、サーバーのメトリクスを New Relic アカウントに関連付けます。この設定は、標準のインストール プロセスの一部として作成されます。
  ...
  >
    [ログ転送のエージェント再試行](/docs/infrastructure/install-infrastructure-agent/manage-your-agent/infrastructure-agent-behavior/#retry)を有効にします。
  ...
  <Collapser
    ...
    title="logging_retry_limit"
  >
    組み込みの fluentbit ロギング フォワーダーの再試行制限を有効にします。整数値は意図した再試行回数です。その他の可能な値には、再試行回数を無限に設定する`False`や、再試行機能を完全にオフにする`no_retries`などがあります。
...
<CollapserGroup>
  <Collapser
    ...
    title="クラウド・マックス・リトライ・カウント（cloud_max_retry_count"
  >
  ...
  <Collapser
    ...
    title="クラウド・メタデータ・エクスペリシティ・セキュリティ"
  >
...
<CollapserGroup>
  ...
  >
    ...
    Windowsホストの場合： `PATHEXT`はMSWindows環境変数です。この機能は、すべてのコマンドラインから実行可能なファイルをマークするファイル拡張子を判別することです。これにより、エラーの報告がすべて解決されます'...一般的なFlex統合の場合、コマンドレット、関数、スクリプトファイルの名前として認識されません... "
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        `smart_level_entry_limit` エラーがログに記録されたときにログに記録される以前のデバッグメッセージの数を指します。たとえば、制限が`5`に設定されている場合、エラーがログに記録されるまでデバッグログがメモリにキャッシュされます。エラーがログに記録されると、前の5つのデバッグメッセージもログに記録されます。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="クラウド・セキュリティ・グループ・リフレッシュ・セキュリティ（cloud_security_group_refresh_sec"
  >
  ...
  <Collapser
    ...
    title="ネットワーク・インターフェイス・インターバル・秒"
  >
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="メトリクス・プロセス・サンプル・レート（metrics_process_sample_rate"
  >
  ...
  <Collapser
    ...
    title="メトリクス・ストレージ・サンプル・レート（metrics_storage_sample_rate"
  >
  ...
  <Collapser
    ...
    title="メトリクス・システム・サンプル・レート（metrics_system_sample_rate"
  >
...
<CollapserGroup>
  ...
  >
    ...
    エージェント/ホスト エンティティに関する情報を返します。エージェントがエージェント/ホスト エンティティに関する情報をまだ持っていない場合、応答ステータス コード_204_ (「コンテンツなし」) が返されます。したがって、エージェントがエンティティ データを提供するまでに、いくつかのリクエストが必要になる場合があります。
...
<CollapserGroup>
  ...
  >
    この設定オプションでは、newrelic-infra.exeのプロセスの優先度を次のいずれかの値にすることができます。
