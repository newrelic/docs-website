...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      <DNT>**Recommendation:**</DNT> この設定は変更しないでください。
    </Callout>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="統合のみ"
  >
    インフラストラクチャエージェント以外のソース ( OpenTelemetryコレクターや Prometheus ノード エクスポーターなど) からホストをインストゥルメントしていて、他のインフラストラクチャ サービスを監視するためにインフラストラクチャエージェントを引き続き使用する場合は、この設定を使用します。 有効にすると、エージェントはホスト メタデータで装飾されたホスト インベントリとインテグレーション テレメトリ (イベント メトリクスとインベントリ) を報告しますが、ホスト メトリクス (CPU、メモリ、ディスク、ネットワーク、プロセス) は無効になります。
