...
<Callout variant="important">
  これまで、これらの集計されたイベントバケットには、メトリクスごとに1つの平均値のみが含まれていました。2024年10月から11月にかけて、集計を段階的に移行し、代わりに[ゲージ形式](/docs/data-apis/understand-data/metric-data/metric-data-type/)を使用するようになります。これにより、`max()`、 `min()`、 `average()`の値を区別できるようになります。
