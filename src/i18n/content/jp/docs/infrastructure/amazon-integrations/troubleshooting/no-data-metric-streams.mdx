...
6. Kinesis Data Firehoseに正しい送信先情報が設定されていることを確認します。
   ...
   * New Relicアカウントに適切なデータセンター（US、EU、またはJP）が選択されていることを確認してください。ヒント：`license_key`が「eu」で始まる場合は、EUデータセンターを選択する必要があります。`license_key`が「jp」で始まる場合は、JPデータセンターを選択する必要があります。
