...
インフラストラクチャで統合データを見つけるには、 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、AWS CloudTrail 統合リンクのいずれかを選択します。

この統合はメトリックを提供しません。[イベントデータ](/docs/telemetry-data-platform/understand-data/new-relic-data-types/#events-new-relic)のみを提供します。インフラストラクチャの[**\[イベント\]**ページ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change)を使用して、これらのイベントのタイムラインを表示できます。
