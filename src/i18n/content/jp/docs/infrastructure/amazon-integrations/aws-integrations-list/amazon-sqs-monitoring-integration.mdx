...
[Amazon Simple Queue Service (Amazon SQS) は、](https://aws.amazon.com/sqs/)転送中のメッセージを保存するためのホスト型キューを提供します。 New Relicの SQS インテグレーションは、配信、遅延、受信されたメッセージの数、メッセージ サイズ、その他の[メトリック](#metrics)などのキュー関連のデータを報告します。 SQS データは、事前に構築された<InlinePopover type="dashboards"/>で使用でき、 [カスタム クエリとグラフ](/docs/infrastructure/integrations-getting-started/getting-started/use-integration-data-new-relic-insights)を作成することもできます。 SQS データの変更を通知するアラート条件を作成することもできます。
