...
[Amazon Simple Queue Service（Amazon SQS）](https://aws.amazon.com/sqs/)は、転送中のメッセージを保存するためのホストされたキューを提供します。New RelicのSQS統合は、配信、遅延、受信されたメッセージの数、メッセージサイズ、その他の[メトリック](#metrics)などのキュー関連データをレポートします。SQSデータは、事前に作成されたダッシュボードで利用でき、 [カスタムクエリやチャート](/docs/infrastructure/integrations-getting-started/getting-started/use-integration-data-new-relic-insights)を作成することもできます。アラート条件を作成して、SQSデータの変更を通知することもできます。
