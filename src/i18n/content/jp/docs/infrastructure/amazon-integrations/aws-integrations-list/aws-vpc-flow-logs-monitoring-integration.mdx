...
1. Serverless リポジトリから新しいAWS Lambda 関数を作成します。
...
2. `LICENSE_KEY`環境変数に New Relic アカウント
...
4. <DNT>
   ...
   を選択して、新しい CloudFormation スタック、 `newrelic-log-ingestion`という新しい関数、および必要なロールを作成します。
...
1. [CloudWatch マネジメントコンソール](https://console.aws.amazon.com/cloudwatch/home)から
...
3. [VPC Flow ログ監視を有効にした](#enable) ときに作成したNew Relic Lambda 関数`newrelic-log-ingestion` ( ) を選択し、
