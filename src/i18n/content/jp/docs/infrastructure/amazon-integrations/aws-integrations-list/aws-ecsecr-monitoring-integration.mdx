...
[データを照会して探索することができます](/docs/using-new-relic/data/understand-data/query-new-relic-data) `ComputeSample` [イベントタイプ](/docs/data-apis/understand-data/new-relic-data-types/#event-data) を使用します。クラスタデータには `プロバイダ` の値 `EcsCluster` を、クラスタサービスデータには `プロバイダ` の値 `EcsService` を使用します。
