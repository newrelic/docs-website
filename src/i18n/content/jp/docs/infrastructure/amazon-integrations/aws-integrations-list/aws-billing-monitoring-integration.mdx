...
New Relic の AWS Billing 統合は、すべてのアプリケーションと AWS アカウントの財務データを収集し、グループ化することで、チームがコストの可視化を容易にします。この統合は、異なるサブアカウントやサービスからデータを収集し、また、 [AWS 予算](http://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/budgets-managing-costs.html) を実際の支出と予測支出の観点から評価します。
...
[AWS バジェットを有効にする前に](#configure-budget) 、必要があります。

- AWS Billing のデータを収集するために、New Relic の AWS Console から AWS バジェットを設定します。
- **Receive Billing Alerts** のチェックボックスを有効にする **AWS コンソール> Billing> Billing preferences**.これは、Amazon が CloudWatch を介して使用料を報告するために必要です。実際に Amazon からアラートを受信する必要はありません。

<Callout variant="important">
  Billing (Costs) ダッシュボードにアクセスするには、 **Receive Billing Alerts**
  のチェックボックスを有効にするだけでよい。
</Callout>

## 統合を有効にする [#activate]

このインテグレーションを有効にするには、標準手順に従い、[AWS サービスを New Relic に接続します](/docs/infrastructure/infrastructure-integrations/getting-started/connect-aws-integrations-infrastructure)。

## AWS バジェットの有効化 [#configure-budget]

AWS のバジェットを有効にすると、New Relic はサービスの消費量だけでなく、AWS で設定したバジェットの使用量やコストの情報も取得できるようになります。

<Callout variant="tip">
  AWSにロールを作成していない場合は、バジェットポリシーを設定する前に、
  [AWSとNew
  Relic](/docs/infrastructure/infrastructure-integrations/getting-started/connect-aws-integrations-infrastructure#connect)
  を接続する手順に従ってください。
</Callout>

1. AWS Management コンソールから、 **Services** > **IAM** > **Roles** を選択します。
...
<Callout variant="tip">
  New Relic を構成して AWS メンバーアカウントから [AWS
  予算](http://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/budgets-managing-costs.html)
  データを取得する場合は、アカウント所有者が [Billing and Cost Management
  コンソールへの IAM
  ユーザーアクセスを有効にしていることを確認してください](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/grantaccess.html)
  。
</Callout>
...
データは、 `FinanceSample` [イベントタイプ](/docs/insights/use-insights-ui/getting-started/introduction-new-relic-insights#event-type) に添付されます。そのイベントタイプは、請求データの種類に応じて、4 種類の `プロバイダ` の値を持つことができます。

- [バジェット](#billing-budget)
- [AWS サービス費用](#billing-service-cost)
- [アカウントとサブアカウントの費用を集約したもの](#billing-account-cost)
- [未集計のアカウント/サブアカウントデータ、および未集計の AWS サービス費用](#billing-account-service)
...
[デフォルトのポーリング](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-polling-intervals-infrastructure-integrations) AWS Billing インテグレーションの情報です。

- New Relic のポーリング間隔：1 時間
- Amazon CloudWatch のデータ間隔。4 時間
...
また、AWS Billing インテグレーションでは、以下の [属性を収集します。](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#attribute) 。
