...
* **AWS console \[AWS コンソール] > Billing and cost management \[請求とコスト管理] > Billing preferences \[請求設定]**で**Receive Billing Alerts** \[請求アラートを受信する]チェックボックスを有効にします。 これは、Amazon が CloudWatch 経由で使用料金を報告するために必要です。 実際に AWS からアラートを受信する必要はありません。
