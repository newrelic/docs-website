...
Infrastructure で統合データを見つけるには、 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、AWS Billing 統合リンクのいずれかを選択します。
  ...
  >
    ...
    * 予想される費用
      ...
      **NRQL の例**: 実際の支出、予測、制限、および使用量と金銭的コストとの関係を示した異なる予算のテーブルについて、以下の NRQL クエリを実行します。
      ...
      <Callout variant="tip">
        `providerAccountId`は、NewRelicのクラウドサービスアカウントIDです。**[one.newrelic.com](https://one.newrelic.com) >インフラストラクチャ>AWS**の**プロバイダーアカウント**の横にあります。
      </Callout>
  </Collapser>
