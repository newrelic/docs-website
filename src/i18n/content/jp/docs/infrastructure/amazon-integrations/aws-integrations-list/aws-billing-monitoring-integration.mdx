...
* <DoNotTranslate>**AWS console > Billing and cost management > Billing preferences**</DoNotTranslate>
  ...
  チェックボックスを有効にします。 これは、Amazon が CloudWatch 経由で使用料を報告するために必要です。 実際に AWS からアラートを受信する必要はありません。

<Callout variant="important">
  \[請求 (コスト)] ダッシュボードにアクセスするには、 <DoNotTranslate>**Receive Billing Alerts**</DoNotTranslate>チェックボックスを有効にするだけです。
</Callout>
...
1. AWS マネジメントコンソールから、 <DoNotTranslate>**Services > IAM > Roles**</DoNotTranslate>を選択します。

2. New Relic 用に構成したロールを選択し、 <DoNotTranslate>**Permissions**</DoNotTranslate>を選択します。

3. 選択したロールの<DoNotTranslate>**Inline policy**</DoNotTranslate>タブから展開を選択します<Icon name="fe-chevron-down"/>すでに存在する場合はアイコンをクリックし、ロールに新しいインライン ポリシーを追加するオプションを選択します。

4. カスタムポリシーを追加するオプションを選択します。

5. 新しいカスタム ポリシー名を入力します。たとえば、NewRelicBudget。

6. 次の権限ステートメントを<DoNotTranslate>**Policy Document**</DoNotTranslate>に入力します。
...
インフラストラクチャUIで統合データを見つけるには、<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate> に移動し、 AWS Billing 統合リンクの 1 つを選択します。
...
<CollapserGroup>
  ...
  >
    ...
    * 予想される費用
      ...
      <DoNotTranslate>**NRQL example**</DoNotTranslate>: 実際の支出、予測、制限、使用量と金銭的コストを比較したさまざまな予算の表を取得するには、次の NRQL クエリを実行します。
      ...
      <Callout variant="tip">
        `providerAccountId` はNew Relicのクラウド サービス アカウント ID です。 これは、 <DoNotTranslate>**Provider account**</DoNotTranslate>の隣の<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DoNotTranslate>にあります。
      </Callout>
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: プロバイダーをサービス名でグループ化するには、次の NRQL クエリを実行します。
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: プロバイダーをアカウント名でグループ化するには、次の NRQL クエリを実行します。
  ...
  >
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: 特定のAWSアカウント ID の AWS サービスのコスト分割を理解するには、次のNRQL書き込みを実行します。
    ...
    <DoNotTranslate>**NRQL example**</DoNotTranslate>: AWS EC2 サービスの AWS アカウントのコスト分割を理解するには、次の NRQL クエリを実行します。
