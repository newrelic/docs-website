...
AWS X-Rayの統合を有効にするには、標準的な手順に従って、 [Connect AWS services to Infrastructure](/docs/integrations/amazon-integrations/get-started/connect-aws-infrastructure).
...
<Callout variant="important">
  ポーリング間隔が5分を超えると、トレースが失われることがあります。New Relic は 20 分以上経過したトレースデータを削除します。同様に、長時間実行されたトレースは不完全になる可能性があります。New Relic の X-Ray Monitoring は完了したトレースのみを収集し、20 分以上前のトレース内のスパンはドロップされます。
</Callout>

## データの検索と使用 [#find-data]

New Relic は X-Ray セグメントを span イベントにマッピングします。この統合を有効にすると、AWS X-Ray のトレースが [distributed tracing](/docs/apm/distributed-tracing/ui-data/understand-use-distributed-tracing-data) に表示されるようになります。

スパンをフィルタリングするには、 `newRelic.ingestPoint` 属性を使用します。この属性は、すべてのX-Rayスパンで `xray.polling` に設定されています。

## カスタムIAMポリシーの使用

AWSアカウントとNew Relic Infrastructureを接続する際にカスタムIAMポリシーを使用している場合、そのカスタムポリシーにはAWS X-Rayの統合を機能させるために以下のパーミッションが必要となります。
...
## [X-Rayとステップ機能](https://docs.aws.amazon.com/lambda/latest/dg/services-xray.html#services-xray-permissions)
