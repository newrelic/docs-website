...
<Callout variant="tip">
  API Gateway v1（REST）のメトリクスは、[CloudWatch Metric Streamsインテグレーション](/docs/infrastructure/amazon-integrations/aws-integrations-list/aws-metric-stream/)とAmazon API GatewayのAPIポーリングインテグレーションの両方を通じて利用できます。
</Callout>
...
このインテグレーションを有効にするには、以下のいずれかの方法を使用します：

* <DNT>**CloudWatch Metric Streams**</DNT> （推奨）：手順に従って[CloudWatch Metric Streamsをセットアップします](/docs/infrastructure/amazon-integrations/connect/aws-metric-stream-setup/)。
* <DNT>**API polling**</DNT>：標準手順に従い、[AWSサービスをNew Relicに接続します](/docs/infrastructure/infrastructure-integrations/getting-started/connect-aws-integrations-infrastructure)。
...
データのクエリ方法は、取り込み方法によって異なります：

* <DNT>**CloudWatch Metric Streams**</DNT>：データはディメンショナルメトリクスとして報告されます。<DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Metrics &amp; Events**</DNT>に移動して`aws.apigateway`でフィルタリングします。
* <DNT>**API polling**</DNT>：データは`ApiGatewaySample`[イベントタイプ](/docs/data-apis/understand-data/new-relic-data-types/#event-data)に報告され、これを[クエリして探索](/docs/using-new-relic/data/understand-data/query-new-relic-data)できます。
