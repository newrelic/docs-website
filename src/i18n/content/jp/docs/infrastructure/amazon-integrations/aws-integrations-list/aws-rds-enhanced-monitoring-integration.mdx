...
1. RDS 拡張監視を有効にする必要がある RDS インスタンスを指定します。 インスタンスを作成または変更するときにこれを指定できます。
...
3. Serverless リポジトリから新しいAWS Lambda 関数を作成します。
...
4. `LICENSE_KEY`環境変数に New Relic アカウント
...
2. <DNT>**Lambda function**</DNT>
   ...
   の場合は、 `newrelic-log-ingestion`を選択します。
...
5. 下部にある
   ...
   ボタンをクリックして、Lambda サブスクリプション フィルターを保存します。
