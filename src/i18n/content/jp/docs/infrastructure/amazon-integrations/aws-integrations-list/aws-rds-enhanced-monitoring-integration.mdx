...
4. `LICENSE_KEY`環境変数に New Relic アカウントの[ライセンス キーを](/docs/accounts/install-new-relic/account-setup/license-key)入力します。
...
統合データを見つけるには、 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、 **RDS > Enhanced monitoring dashboard**リンクを選択します。
