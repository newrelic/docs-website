...
この New Relic の統合により、RDS Enhanced Monitoring での監視と[アラート](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information)が可能になります。 統合データを使用して、 <InlinePopover type="alerts"/>DB プロセスを監視し、潜在的な問題点を特定するだけでなく、DB のプロファイリングを行うことで、応答とコストを改善および最適化することができます。
