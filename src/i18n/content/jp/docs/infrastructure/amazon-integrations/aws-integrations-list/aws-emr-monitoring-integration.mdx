...
[データをクエリして探索することができます](/docs/using-new-relic/data/understand-data/query-new-relic-data) `ElasticMapReduceClusterSample` [イベントタイプ](/docs/data-apis/understand-data/new-relic-data-types/#event-data), `provider` 値は `ElasticMapReduceCluster` です。
