...
New Relic の AWS Lambda 統合は、呼び出し数、エラー数、関数タイマー、その他の [メトリクス](#metrics)などのデータをレポートします。事前構築済みの Lambda データを表示できます <InlinePopover type="dashboards"/>また、 [カスタム クエリとチャートを作成します](/docs/infrastructure/integrations-getting-started/getting-started/use-integration-data-new-relic-insights)。
...
統合データを見つけるには、 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS** に移動し、Lambda 統合リンクの 1 つを選択します。
