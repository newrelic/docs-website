...
各ポーリング統合で利用可能なメトリクスとその名前のリファレンスについては、 [個別の統合に関するドキュメントを参照してください](https://docs.newrelic.com/docs/infrastructure/amazon-integrations/get-started/introduction-aws-integrations/)。

次の表は、AWS ポーリング統合によって収集されるメトリクスとその[次元メトリクスの](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types/#dimensional-metrics)変換の非包括的なリストです。
