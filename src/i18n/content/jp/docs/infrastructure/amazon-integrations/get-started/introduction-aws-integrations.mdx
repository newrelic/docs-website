...
AWSデータを表示するには、**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**に移動します。リストにあるAWSインテグレーションに関しては、
...
NRQLクエリは、事前設定された<InlinePopover type="dashboards"/>と**イベントエクスプローラー**ダッシュボードの双方で確認・再利用できます。これによって、クエリを具体的なニーズに合わせてカスタマイズできます。
