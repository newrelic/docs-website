...
追加の<DNT>**API Polling**</DNT>インテグレーションは、CloudWatchメトリクスとして利用できないデータを引き出すために、AWS CloudWatchメトリクスストリーム上で有効にできます。次のインテグレーションはメトリクスストリームで置き換えられません。
...
AWSデータを表示するには、 <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > AWS**</DNT>に移動します。 リストにあるAWSインテグレーションに関しては、

* アクティブなストリームの場合は、 <DNT>**Explore your data**</DNT>リンクを選択します

  または

* その他のインテグレーションについては、利用可能なダッシュボードを参照するか、 <DNT>**Explore Data**</DNT>リンクをクリックします

NRQLクエリは、事前設定された<InlinePopover type="dashboards"/>と<DNT>**Events explorer**</DNT>ダッシュボード双方で確認・再利用できます。これによって、クエリを具体的なニーズに合わせてカスタマイズできます。
