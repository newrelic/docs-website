...
* **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、 **Add an AWS account を**クリックしてから**Use metric streams**をクリックし、手順に従います。
...
3. New Relic UI の**メトリクス ストリーム**モードで**新しい AWS アカウントを追加し**ます。**[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、 **Add an AWS account を**クリックしてから**Use metric streams**をクリックし、手順に従います。

## データが正しく受信されているかどうかの確認 [#validate-data]

メトリックストリームからデータを受信していることを確認するには、次の手順に従います。

1. **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > AWS**に移動し、 **Stream**アカウントを検索します。
2. 次の点をチェックします。
