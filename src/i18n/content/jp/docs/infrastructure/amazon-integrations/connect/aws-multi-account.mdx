...
### AWS クロスアカウントアクセスの設定 [#setting-up-aws-cross-account-access]

監視アカウントとソースアカウントの両方のAWSマネジメントコンソールにアクセスできる必要があります。

1. 監視アカウントの AWS マネジメントコンソールに移動し、 <DNT>**Monitoring accounts sink ARN**</DNT>をコピーします。 次の手順に従ってください。

   * <DNT>**CloudWatch &gt; Settings**</DNT>に移動します。
   * <DNT>**Manage monitoring account**</DNT>をクリックします。
   * タブ<DNT>**Configuration details**</DNT>に移動します。
   * <DNT>**Monitoring accounts sink ARN**</DNT>コピーします。

   <Callout variant="important">
     各アカウントにはリージョンごとに 1 つのシンクを設定できます。複数のリージョンを監視する必要がある場合は、リージョンごとにシンクを設定する必要があります。
   </Callout>

2. ソースアカウントの AWS マネジメントコンソールに移動し、 <DNT>**Metrics**\*\*</DNT>リソースの[監視アカウントをリンクします](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Unified-Cross-Account-Setup.html)。

   * <DNT>**Cloudwatch &gt; Settings**</DNT>に移動します。
   * <DNT>**Source account configuration**</DNT>セクションで、 <DNT>**Configure**</DNT>をクリックします。
   * <DNT>**Metrics**</DNT>を選択します。
   * (オプション) <DNT>**Filter Metrics**</DNT>をマークしてフィルターを設定します。
   * 以前にコピーしたシンク ARN を<DNT>**Sink ARN**</DNT>フィールドに貼り付けます。
   * <DNT>**Link**</DNT>をクリックして確認します。

3. 監視アカウントの AWS マネジメントコンソールに戻ります。

   * <DNT>**CloudWatch &gt; Settings**</DNT>に移動します。
   * <DNT>**Manage monitoring account**</DNT>をクリックして、ソース アカウントがリストされていることを確認します。
   * <DNT>**CloudWatch &gt; Metrics &gt; Streams**</DNT>に移動し、テレメトリーを送信するメトリクス ストリームで<DNT>**Cross account status**</DNT>が有効になっていることを確認します。 そうでない場合は、編集し、 <DNT>**Metrics to be streamed &gt; Include source account metrics**</DNT>チェックをマークして保存します。
