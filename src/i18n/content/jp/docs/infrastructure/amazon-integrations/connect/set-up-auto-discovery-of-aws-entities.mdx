...
<Callout title="プレビュー">
  ...
  この機能は現在、弊社の[プレリリース ポリシー](/docs/licenses/license-information/referenced-policies/new-relic-pre-release-policy)に従ってプレビュー プログラムの一部として提供されています。<br />
</Callout>
...
<DNT>**Auto-Discovery**</DNT>機能は、**[アドバンス計算](https://newrelic.com/pricing/compute#pricing_plan-compute)**顧客のみが利用できます。 登録するには、 <DNT>**[one.newrelic.com &gt; Administration](https://one.newrelic.com/admin-portal) &gt; Preview &amp; Trials**</DNT>に移動し、 <DNT>**AWS Auto-Discovery**</DNT>に登録します。
    ...
    <CollapserGroup>
      <Collapser id="metrics-data-type" title="メトリクスデータタイプの場合">
        ...
        1. メトリクス統合タイプを選択します:
           ...
           * <DNT>**[Integration via CloudFormation and CloudWatch metric stream](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/)**</DNT> New Relicへのメトリクスの手動ストリームを作成するには、これが推奨される方法です。
      ...
      <Collapser id="log-data-type" title="ログデータタイプの場合">
        ...
        4. New RelicログAPIエンドポイントの選択: New Relicアカウントのリージョンに基づいて、システムが自動的にエンドポイントを設定します。
           ...
           * EUアカウントの場合、ログは以下に送信されます `https://log-api.eu.newrelic.com/log/v1`
           * 日本のアカウントの場合、ログは以下に送信されます `https://log-api.jp.nr-data.net/log/v1`
...
<Steps>
  ...
  <Step>
    ### AWSインテグレーションを管理する [#manage-aws-integrations]

    1. [New Relic](https://one.newrelic.com/)アカウントにログインし、 <DNT>**Infrastructure &gt; AWS**</DNT>に移動します。
    2. AWS アカウントで<DNT>Auto-Discovery</DNT>が有効になっているかどうかを確認するには、 <DNT>**Manage AWS Integrations**</DNT>をクリックします。
    3. AWS アカウントの自動検出を有効にするには、アカウントリストでそのアカウントの<DNT>**Install**</DNT>をクリックします。
    4. 自動検出の場合は、 <DNT>**Install**</DNT>をクリックします。
  </Step>
...
<CollapserGroup>
  ...
  <Collapser id="disable-auto-discovery" title="自動検出を無効にする">
    ...
    1. [New Relic](https://one.newrelic.com/)アカウントにログインし、 <DNT>**Infrastructure &gt; AWS**</DNT>に移動します。
    2. AWS アカウントで<DNT>Auto-Discovery</DNT>が有効になっているかどうかを確認するには、 <DNT>**Manage Auto-Discovery**</DNT>をクリックします。
