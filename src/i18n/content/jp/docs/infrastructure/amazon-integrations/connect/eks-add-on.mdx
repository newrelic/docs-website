...
* [Amazon Elastic Kubernetes Service (EKS)](https://docs.aws.amazon.com/eks/latest/userguide/what-is-eks.html)クラスター。[詳細については、](https://docs.aws.amazon.com/eks/latest/userguide/create-cluster.html)ユーザー ガイドを参照してください。
...
1. **EKS Cluster > Add-Ons > Get More add-ons**に移動します。
...
2. AWS Marketplace アドオンで、 New Relic Kubernetes Integration を検索、参照または移動し、チェックボックスを選択して、 **Next** \[次へ]をクリックします。
...
3. 必要に応じて、最新のデフォルト バージョンを選択するか、代替バージョンを選択し、 **Next** \[次へ]をクリックします。
