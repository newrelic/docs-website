...
[CloudWatch カスタムメトリックを取り込むには、カスタム ネームスペースがCloudWatch Metric Streams設定に表示される必要があります。 包含ルールまたは除外ルールによってフィルタリングされていないことを確認してください。](https://docs.aws.amazon.com/config/latest/developerguide/select-resources.html)
