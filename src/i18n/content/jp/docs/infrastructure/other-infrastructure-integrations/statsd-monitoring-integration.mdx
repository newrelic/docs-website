...
<CollapserGroup>
  <Collapser id="alert-example" title="アラート例">
    ...
    50 を超える値のメトリクスが送信されると、まとめイベントが作成されて通知されます。 人気イベントは24時間後に自動的に終了します。 集計が機能していることをテストするには、次のコマンドを実行します。
...
* New Relicアカウントに適切なデータセンター（US（デフォルト）またはEU）が選択されていることを確認してください。ヒント：license\_keyが「eu」で始まる場合は、`NR_EU_REGION=true`フラグを使用する必要があります。
* JPリージョンの場合、ライセンスキーに基づいてエンドポイントが自動的に選択されるため、追加のフラグは必要ありません。
* StatsDインテグレーションに関連する[`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/)がないことを確認してください。
