...
以下に示すいずれかの例で[パーソナル APIキー](https://docs.newrelic.co.jp/node/6276/draft#personal-api-key)を使用するには、'X-Api-Key： <var>&#x7B;api_key}</var> ' を 'Api-Key： <var>&#x7B;personal_api_key}</var> ' に置き換えます。
...
```
curl -v -X GET --header "X-Api-Key: <var>admin_api_key</var>" "https://infra-api.newrelic.com/v2/alerts/conditions?policy_id=111111"
```
...
```
curl -v -X GET --header "X-Api-Key: <var>admin_api_key</var>" "https://infra-api.newrelic.com/v2/alerts/conditions?policy_id=111111&offset=50&list=10"
```
...
```
curl -v -X GET --header "X-Api-Key: <var>admin_api_key</var>" "https://infra-api.newrelic.com/v2/alerts/conditions/<var>condition-id</var>"
```
...
```
curl -X POST 'https://infra-api.newrelic.com/v2/alerts/conditions' -H 'X-Api-Key:<var>admin_api_key</var>' -i -H 'Content-Type: application/json' -d '{"data":{<var>DATA object details</var>}}'
```
...
<CollapserGroup>
  ...
  >
    ```
    curl -X PUT 'https://infra-api.newrelic.com/v2/alerts/conditions/<var>condition-id</var>' -H 'X-Api-Key:<var>admin_api_key</var>' -i -H 'Content-Type: application/json' -d '{"data":{<var>DATA object details</var>}}'
    ```
  </Collapser>
</CollapserGroup>
...
```
curl -v -X DELETE --header "X-Api-Key: <var>admin_api_key</var>" "https://infra-api.newrelic.com/v2/alerts/conditions/<var>condition_id</var>"
```
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        ...
        ```
        curl -X POST 'https://infra-api.newrelic.com/v2/alerts/conditions' -H 'X-Api-Key:<var>admin_api_key</var>' -i -H 'Content-Type: application/json' -d
        '{
           "data":{
