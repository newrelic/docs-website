...
たとえば、 [フィルタセット](/docs/infrastructure/new-relic-infrastructure/filter-group/organize-infrastructure-hosts-filter-sets)を監視していて問題に気付いた場合、アラートを直接作成でき、[アラート](/docs/alerts/new-relic-alerts/getting-started/introduction-new-relic-alerts)から再作成する必要はありません。アラートを作成するには、フィルターセットをすぐに選択し、表示しているグラフからアラート条件を直接調整します。これにより、環境のアラートシステムをプロアクティブに管理および監視できます。アラート違反は、フィルターセット内のエンティティごとに作成されます。
...
アラート条件はアラート[ポリシー](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-policy)に適用されます。インフラストラクチャ監視UIから、既存のポリシーを選択するか、電子メール通知を使用して新しいポリシーを作成できます。他のタイプの[通知チャネル](/docs/alerts/new-relic-alerts/managing-notification-channels/notification-channels-controlling-where-send-alerts)を使用する場合は、アラートUI内から[新しいポリシー](/docs/alerts/new-relic-alerts/configuring-alert-policies/name-or-rename-alert-policy)を作成します。
...
1. **[one.newrelic.com](https://one.newrelic.com) > Infrastructure**に移動し、以下の Infrastructure monitoring ページのいずれかを選択します: [**Hosts**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage) 、 [**Processes**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-processes-page-inspect-process-performance) 、 [**Network**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-network-page-measure-compare-capacity) 、または[**Storage**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-storage-page-evaluate-disk-usage-efficiency) 。アラートを表示するグラフにマウスを合わせ、楕円を選択します<Icon name="fe-more-horizontal"/>アイコンをクリックし、\[**アラートの作成**] を選択します。
...
4. 個々のフィルターを作成するか、 [フィルターセット](/docs/infrastructure/new-relic-infrastructure/filter-group/filter-sets-organize-your-infrastructure-hosts)からすべてのフィルターをコピーして、アラート条件で使用するホストを識別します。

   <Callout variant="important">
     フィルターのルールについては、 [フィルターセットロジック](/docs/infrastructure/new-relic-infrastructure/filter-group/filter-sets-organize-your-infrastructure-hosts#logic) をご覧ください。
   </Callout>
...
<CollapserGroup>
  <Collapser
    ...
    title="アラートUIを使用する"
  >
    **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Alerts > Alert policies > New alert policy > Create new condition**に移動し、製品として**Infrastructure**を選択します。
  </Collapser>
  ...
  >
    ...
    2. 次のインフラストラクチャ監視ページのいずれかを選択します： [**ホスト**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage)、 [**プロセス**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-processes-page-inspect-process-performance)、 [**ネットワーク**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-network-page-measure-compare-capacity)、または[**ストレージ**](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-storage-page-evaluate-disk-usage-efficiency)。

    3. 警告するチャートにマウスを合わせ、省略記号を選択します
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Settings > Alerts**に移動し、 **Create alert condition**をクリックします。
...
ポリシーの[通知チャネル](#notification)に含まれる人は誰でも、アラート通知を直接受信します。さらに、New Relicアカウントの権限を持っている人は誰でも、ユーザーインターフェイスを介してインフラストラクチャアラートインシデントと個々の違反を表示できます。

1. **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Events**に移動します。
2. ホストまたは時間枠を変更するには、検索ウィンドウ、**フィルターセット**、または**時間**関数を使用します。
...
1. **[one.newrelic.com](https://one.newrelic.com) > インフラストラクチャー> 設定> アラート** にアクセスしてください。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * [メール通知](#notification)のみが必要なポリシーの場合: **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Settings > Alerts > Create a new policy**に移動し、必要に応じて 1 つ以上のメールアドレスを追加します。
...
詳細については、NewRelicのExplorersHub [でインフラストラクチャアラートをフィルタリングするためのベストプラクティスを](https://discuss.newrelic.com/t/relic-solution-best-practice-for-filtering-in-infrastructure-alerts/58065)参照してください。