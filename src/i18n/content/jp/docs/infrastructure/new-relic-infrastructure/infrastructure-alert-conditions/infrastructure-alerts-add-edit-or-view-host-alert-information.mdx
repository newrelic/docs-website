...
<Callout
  ...
  variant="important"
>
  インフラストラクチャ REST API には、アクティブな<DoNotTranslate>**and**</DoNotTranslate>と無効な条件の両方を含めて 3,700 個のアラート条件の制限があります。 API 、直接使用するかUI経由で使用するかに関わらず、3,700 アラート条件の制限を超えて追加のアラート条件を追加するすべてのリクエストを拒否します。
</Callout>

インフラストラクチャアラート条件をアラートポリシーに追加するには、次の手順に従います。

1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**</DoNotTranslate>に移動します。 集計したいチャートにマウスを移動し、省略記号を選択します<Icon name="fe-more-horizontal"/>アイコンをクリックし、 <DoNotTranslate>**Create alert condition**</DoNotTranslate>を選択します。

2. 意味のある条件名を入力します。

3. <DoNotTranslate>**Alert type**</DoNotTranslate>を選択するか、 [例](/docs/infrastructure/new-relic-infrastructure/infrastructure-alert-conditions/infrastructure-alerting-examples)を参照して選択するタイプを決定します。
...
5. 集計通知をトリガーするための<DoNotTranslate>**Critical**</DoNotTranslate> (必須) および<DoNotTranslate>**Warning**</DoNotTranslate> (使用可能な場合はオプション)[の κ](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#alert-threshold)を定義します。

6. オプション: 条件基準を事前に作成するが、現時点では集計通知を受信しない場合は、 [<DoNotTranslate>**Enabled**</DoNotTranslate>チェックボックス オプション](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information#enabled)をオフにします。
...
9. オプション: 一定の時間が経過するとオープン インシデントを自動的に閉じるように[<DoNotTranslate>**Close open incidents after**</DoNotTranslate>](#incident-time-limit)時間制限を設定します (インフラストラクチャ条件により、デフォルトでは 24 時間に設定されています)。

10. <DoNotTranslate>**Create**</DoNotTranslate>を選択します。
...
<CollapserGroup>
  <Collapser
    ...
    title="アラートUIを使用する"
  >
    <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Alert policies > New alert policy > Create new condition**</DoNotTranslate>に移動し、製品として<DoNotTranslate>**Infrastructure**</DoNotTranslate>を選択してください。
  </Collapser>
  ...
  >
    ...
    2. 集計したいチャートにマウスを移動し、省略記号を選択します
  ...
  >
    ...
    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Settings > Alerts**</DoNotTranslate>に移動し、 <DoNotTranslate>**Create alert condition**</DoNotTranslate>をクリックします。

    2. アラート状態に名前を付けて説明します。

    3. <DoNotTranslate>**Integrations**</DoNotTranslate>集計タイプをクリックし、使用する統合データ ソースを選択します。

    4. <DoNotTranslate>**Filter entities**</DoNotTranslate>ドロップダウンを使用して、条件を特定の属性に制限します。

    5. <DoNotTranslate>**Define thresholds**</DoNotTranslate>をクリックして条件の閾値を定義し、 <DoNotTranslate>**Create**</DoNotTranslate>をクリックします。
...
2. オプション: 検索ウィンドウまたは
   ...
   チェックボックスを使用して、1 つ以上のアラート条件を検索します。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * 表示、変更、無効化（または再有効化）、または削除:[インフラストラクチャ ](#maintain-alerts)
    ...
    <tr>
      ...
      <td>
        * [イベント](/docs/alerts/new-relic-alerts/getting-started/alerts-glossary#event)に関する概要情報を表示します。[インフラストラクチャ ](#view-alerts)
    ...
    <tr>
      ...
      <td>
        ...
        * [電子メール通知](#notification)のみが必要なポリシーの場合: <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Settings > Alerts > Create a new policy**</DoNotTranslate>に移動し、必要に応じて 1 つ以上の電子メール アドレスを追加します。
...
<DoNotTranslate>**Description**</DoNotTranslate>フィールドは、次のアラート条件タイプで使用できます。
...
アラート条件の<DoNotTranslate>**Description**</DoNotTranslate>フィールドに入力したテキストは、関連するインシデントおよび通知に下流に渡されます。 説明は、次のようなさまざまな目的で使用できます。
