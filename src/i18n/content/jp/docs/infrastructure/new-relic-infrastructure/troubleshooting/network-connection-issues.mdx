...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. **Resolve the domain:** `nslookup`または`dig`を使用して、Linux システムがドメインを IP アドレスに解決できることを検証します。
    ...
    2. **Ping the server:**上記で取得した IP アドレスに ping を実行して、サーバーに到達できることを確認します。
    ...
    3. **Traceroute:** `traceroute`を使用して、システムとサーバー間のネットワーク パスを確認します。
    ...
    4. **Curl:** `curl`を使用して、エンドポイントが到達可能かどうかを確認します。
...
1. [開始](https://docs.newrelic.com/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started)手順に従って、ホストに`NRDiag`をインストールします。
