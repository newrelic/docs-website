...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. <DoNotTranslate>**Resolve the domain:**</DoNotTranslate> `nslookup`または`dig`を使用して、Linux システムがドメインを IP アドレスに解決できることを検証します。
    ...
    2. <DoNotTranslate>**Ping the server:**</DoNotTranslate> 上記で取得した IP アドレスに ping を実行して、サーバーに到達できることを確認します。
    ...
    3. <DoNotTranslate>**Traceroute:**</DoNotTranslate> `traceroute`を使用して、システムとサーバー間のネットワーク パスを確認します。
    ...
    4. <DoNotTranslate>**Curl:**</DoNotTranslate> `curl`を使用して、エンドポイントが到達可能かどうかを確認します。
