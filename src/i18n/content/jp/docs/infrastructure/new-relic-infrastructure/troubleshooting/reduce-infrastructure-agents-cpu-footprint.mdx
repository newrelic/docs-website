...
<CollapserGroup>
  ...
  >
    * **単一のプラグインを無効にする：プラグイン**を無効にするには、対応するプロパティ値を`-1`に設定します。

    * **すべてのプラグインを無効にします。** `disable_all_plugins: true`

    * **選択したプラグインを有効にする：**特定のプラグインを有効にするには、 `disable_all_plugins`に例外を挿入します。たとえば、次の構成ではすべてのプラグインが無効になりますが、ネットワークインターフェイスプラグインは120秒ごとに報告します。
  ...
  >
    Sysctlプラグインは、 `/sys`ディレクトリ構造全体をウォークし、そこにあるすべてのファイルから値を読み取ります。これを無効にするか、間隔を短くすると、インフラストラクチャエージェントのCPUシステム時間が短縮される場合があります。
