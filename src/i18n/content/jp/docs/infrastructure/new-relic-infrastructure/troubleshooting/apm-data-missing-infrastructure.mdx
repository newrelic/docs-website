...
APM とインフラストラクチャ エージェントが同じホストにインストールされ、同じ New Relic [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)を使用する場合、APM データは **インフラストラクチャ**に表示され、その逆も同様です。 この APM インフラストラクチャ リンクが表示されない場合は、次のトラブルシューティングのヒントに従ってください。
