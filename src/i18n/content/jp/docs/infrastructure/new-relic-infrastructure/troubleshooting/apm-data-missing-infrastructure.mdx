...
APM とインフラストラクチャ エージェントが同じホストにインストールされ、同じ New Relic<LicenseKey/>を使用する場合、APM データは**インフラストラクチャ**に表示され、その逆も同様です。この APM インフラストラクチャ リンクが表示されない場合は、次のトラブルシューティングのヒントに従ってください。
