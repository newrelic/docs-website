...
<img
  title="New Relic One - Windowsサービス統合 - メトリクスデータ"
  alt="New Relic One - Windowsサービス統合 - メトリクスデータ"
...
[当社の統合は、New Relic エージェントがサポートする](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent) Windows の全バージョンに対応しています。

**バージョン1.12.1以上** 当社の [インフラストラクチャ監視エージェント](/docs/infrastructure/install-infrastructure-agent/windows-installation/install-infrastructure-monitoring-agent-windows) がサポート対象のホストにインストールされている必要があります。x86 Windows版はまだサポートされていません。
...
また、設定オプションの [`inventory_source`](/docs/infrastructure/host-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format) は、統合に対応していません。
...
[NRQL の条件を使って Windows サービスのアラートを](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-nrql-alert-conditions) 作成すると、監視したいサービスの状態変化を通知することができます。ここでは、Windows サービスデータを使用したアラート条件の例を 2 つ紹介します。
...
<img
  title="Windowsサービス統合アーキテクチャ"
  alt="Windowsサービス統合アーキテクチャ"
