...
<Steps>
  ...
  <Step>
    ...
    1. パス `/etc/newrelic-infra/integrations.d` に `lighttpd-http-config.yml` という名前のファイルを作成します <Callout variant="tip">一般的なファイル名は &lt;FILENAME>-config.yml のようになります</Callout>
