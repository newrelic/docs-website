...
<Steps>
  ...
  <Step>
    ...
    <InlinePopover type="browser"/>エージェントは、AJAX パフォーマンス、エラー率、期間、コアウェブバイタル、および効率などのアプリケーションおよびbrowser関連のメトリックを報告します。
  ...
  <Step>
    ...
    <DNT>**Note:**</DNT> 応答に応じて`sessionJSON`を変更し、アプリケーションで Stripe メトリックを取得する場所でこの関数を呼び出します。
