...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        監視対象のクラスターを一意に識別するためのユーザー定義の名前。 <DoNotTranslate>**Required**</DoNotTranslate> 。
      </td>
...
この統合からのデータは、 <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services > HAProxy**</DoNotTranslate>にアクセスすると見つかります。
...
HAProxy 統合は、HAProxy インスタンスの設定問題をキャプチャします。 データは、インフラストラクチャ[インベントリ ページ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)の<DoNotTranslate>**config/haproxy**</DoNotTranslate>ソースの下にあります。 [インベントリデータの詳細については、「インテグレーション データを理解する」](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data)を参照してください。
