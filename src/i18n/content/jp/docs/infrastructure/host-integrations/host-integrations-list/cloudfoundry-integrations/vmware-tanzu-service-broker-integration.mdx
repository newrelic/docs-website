...
2. <DoNotTranslate>**Import a Product**</DoNotTranslate>をクリックして、New Relic Service Broker for VMware Tanzu タイルをインポートします。
...
3. アップロードした「New Relic Service Broker」の右側にある<DoNotTranslate>**+**</DoNotTranslate>ボタンをクリックします。
...
16. アプリケーションを再ステージングします。 <DoNotTranslate>**Bind**</DoNotTranslate>ページの上部にあるリンクを選択するか、ターミナル ウィンドウで`cf restage`を実行して変更を加えることができます。
