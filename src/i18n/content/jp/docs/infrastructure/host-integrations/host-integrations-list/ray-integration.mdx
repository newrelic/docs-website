...
<Steps>
  <Step>
    ...
    Ray インテグレーションを使用するには、まず同じホストに[インフラストラクチャエージェントをインストールする](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/)必要があります。 インフラストラクチャエージェントはホスト自体を監視し、Ray インテグレーションは Ray クラスタ固有のデータを使用して監視を拡張します。  
  ...
  <Step>
    ## Ray 用に`nri-prometheus`設定する [#configure]
  ...
  <Step>
    ...
    Apache Druid クイックスタート を計測し、メトリックとアラートを確認するには、 `Install now`ボタンをクリックして[Ray クイックスタート ページ](https://newrelic.com/instant-observability/ray)をフォローすることもできます。
