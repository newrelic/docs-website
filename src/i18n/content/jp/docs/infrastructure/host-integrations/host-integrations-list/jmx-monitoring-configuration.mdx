...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Kubernetes"
  >
    Kubernetes 統合のインストール手順については [、「Kubernetes で実行されているサービスの監視」](/docs/monitor-service-running-kubernetes) を参照してください。
  </Collapser>
  ...
  >
    ...
    6. [構成設定](/docs/infrastructure/host-integrations/host-integrations-list/jmx-monitoring-configuration/)の説明に従って、 `jmx-config.yml` ファイルを編集します。
  ...
  >
    ...
    4. [構成設定](/docs/infrastructure/host-integrations/host-integrations-list/jmx-monitoring-configuration/)を使用して、 `jmx-config.yml` 構成ファイルを編集します。
