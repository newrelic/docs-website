...
* <DNT>**Advanced:**</DNT> インテグレーションは、パッケージマネージャ外でのインストレーションを可能にするために、 [tarball 形式](/docs/integrations/host-integrations/installation/install-host-integrations-built-new-relic#tarball)でも利用できます。
...
<CollapserGroup>
  ...
  <Collapser id="linux-install" title="Linux">
    ...
    4. Redisサーバーの接続方法に基づいて、`redis-config.yml`設定ファイルを編集します。
       ...
       <CollapserGroup>
         <Collapser id="socket-connect" title="Unix socketで接続する">
         ...
         <Collapser id="tcp-connect" title="TCPで接続する">
...
<CollapserGroup>
  ...
  <Collapser id="basic-auth" title="基本認証">
  ...
  <Collapser id="acl-auth" title="REDIS 6+ ACL認証">
  ...
  <Collapser id="metrics-tls-connection" title="TLS接続によるメトリクスのみ">
  ...
  <Collapser id="metrics-connect-socket" title="ソケット上のメトリクスのみの接続">
  ...
  <Collapser id="renamed-commands" title="名前を変更したコマンドの使用">
  ...
  <Collapser id="multi-instance-keys-filtering" title="キーフィルタリングによるマルチインスタンス監視">
