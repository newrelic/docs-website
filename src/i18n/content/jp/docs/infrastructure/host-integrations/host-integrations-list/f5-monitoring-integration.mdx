...
<CollapserGroup>
  <Collapser
    ...
    title="Linuxのインストール"
  >
    1. ファイル名`nri-f5`を使用して[、統合をインストールする](/docs/install-integrations-package)ための指示に従います。
...
* `{{}}`表記を使用して、環境変数の値を置き換えます。これには、インフラストラクチャエージェントv1.14.0以降が必要です。 [パススルーの使用](/docs/infrastructure/install-infrastructure-agent/configuration/configure-infrastructure-agent/#passthrough)の詳細を読むか、[環境変数の置換例](#envvar-replacement)を参照してください。
...
このインテグレーションは、オープンソース・ソフトウェアです。つまり、 [そのソースコードを閲覧して](https://github.com/newrelic/nri-f5 "リンクが新しいウィンドウで開きます。") 改良を送ったり、独自のフォークを作成して構築することができます。