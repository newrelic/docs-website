...
* 設定ファイルに平文で記載されているような、パスワードなどの機密情報を保護するには、secrets managementを使用します。詳しくは、 [secrets management](https://docs.newrelic.com/docs/integrations/host-integrations/installation/secrets-management) を使った方法をお読みください。
...
[New Relic Insights](/docs/insights) において、F5 BIG-IP のデータは、以下の Insights [イベントタイプ](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#event) に添付されています。
...
これらの属性は、Insights で `F5BigIpVirtualServerSample` イベントタイプを照会することで確認できます。
...
これらの属性は、Insights で `F5BigIpPoolSample` のイベントタイプを照会することで確認できます。
...
これらの属性は、Insights で `F5BigIpPoolMemberSample` イベントタイプを照会することで確認できます。
...
これらの属性は、Insights で `F5BigIpNodeSample` のイベントタイプを照会することで確認できます。
