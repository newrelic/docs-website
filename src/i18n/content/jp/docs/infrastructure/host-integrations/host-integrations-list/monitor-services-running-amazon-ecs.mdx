...
4. ポート 80 で`STATUS_URL`からのリクエストを処理するように NGINX ステータス ページが設定されている場合、インフラストラクチャ エージェントはその監視を開始します。5 分後、NGINX データが Infrastructure UI に表示されていることを確認します ( **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Third party services**または**[one.newrelic.com](https://one.newrelic.com) > Explorer > On-host** )。
