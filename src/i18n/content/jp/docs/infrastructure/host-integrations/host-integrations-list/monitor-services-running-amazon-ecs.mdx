...
4. NGINXステータスページがポート80の`STATUS_URL`からのリクエストを処理するように設定されている場合、インフラストラクチャエージェントはその監視を開始します。5分後、NGINXデータがインフラストラクチャUIに表示されていることを確認します（ [**one.newrelic.com**](http://one.newrelic.com) **>インフラストラクチャ>サードパーティサービス**、または**[one.newrelic.com](https://one.newrelic.com) >エクスプローラー>オンホスト**）。
