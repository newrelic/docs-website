...
<Steps>
  ...
  <Step>
    ## NRI-Flex を使用してメトリクスを取得する [#flex]

    Flex は New Relic インフラストラクチャ エージェントにバンドルされています。NRI-Flex を Vertica 用に設定し、フレックス設定ファイルを作成する必要があります。
  ...
  <Step>
    ...
    Vertica ダッシュボードはカスタム ダッシュボードとみなされ、**Dashboards** \[ダッシュボード]UI に表示されます。ダッシュボードの使用と編集に関するドキュメントについては、[ダッシュボードのドキュメント](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards)を参照してください。
