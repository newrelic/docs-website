...
4. お気に入りのエディタで `nginx-config.yml` 構成ファイルを編集します。いくつかの [構成ファイルの例を確認してください。](#examples) 。

5. 自動NGINXアクセス、エラーログ解析および転送を有効にするには、`nginx-log.yml.example`ファイルを`nginx-log.yml`にコピー（または名前変更）します。エージェントを再起動する必要はありません。
...
<CollapserGroup>
  <Collapser
    ...
    title="SQLユーザーの場合"
  >
    1. 新しいログインを作成し、 `CONNECT`と`VIEW`の権限を付与します。
    ...
    2. 次のステートメントを使用して、ユーザーに`READ`アクセス権限を付与します。
