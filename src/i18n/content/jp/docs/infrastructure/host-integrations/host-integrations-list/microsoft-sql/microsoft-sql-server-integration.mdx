...
* [インフラストラクチャ エージェントは](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic)、Linux または Windows OS ホスト、または SQL Server がインストールされている場所にリモート アクセスできるホストにインストールします。
...
Microsoft SQL Server 統合をインストールするには、ご使用の環境に応じた手順に従ってください。
...
<CollapserGroup>
  <Collapser id="32-bit-path" title="32ビットWindows">
    ```shell
    msiexec.exe /qn /i PATH_TO\nri-mssql-amd386.msi
    ```
  </Collapser>

  <Collapser id="64-bit-path" title="64ビットWindows">
    ```shell
    msiexec.exe /qn /i PATH_TO\nri-mssql-amd64.msi
    ```
  </Collapser>
...
4. お気に入りのエディタで `mssql-config.yml` 構成ファイルを編集します。いくつかの [構成ファイルの例を確認してください。](#examples) 。

### Linuxのインストール [#linux]

1. [インフラストラクチャ エージェントをインストールし](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install)、 `INTEGRATION_FILE_NAME`変数を`nri-mssql`に置き換えます。
...
Microsoft SQL Server をモニターするには、 `CONNECT` 、 `VIEW SERVER STATE` 、および`VIEW ANY DEFINITION`アクセス権限を持つユーザーが必要です。

<Callout variant="tip">
  Microsoft SQL Server での[ログイン](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-login-transact-sql?view=sql-server-2017)と[ユーザーの](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-user-transact-sql?view=sql-server-2017)作成の詳細については、Microsoft のドキュメントを参照してください。 [サーバー スコープ権限](https://learn.microsoft.com/en-us/sql/t-sql/statements/grant-server-permissions-transact-sql?view=sql-server-ver16#remarks)(newrelic ユーザーは 3 を使用します) と[データベース ロール](https://learn.microsoft.com/en-us/sql/relational-databases/security/authentication-access/database-level-roles?view=sql-server-ver16#fixed-database-roles)(newrelic ユーザーは何も取得せず、 `public`のみになります) に関するドキュメントもあります。
</Callout>

新しいログインを作成し、必要な権限を付与します。

<CollapserGroup>
  <Collapser id="sql-user" title="SQLユーザーの場合">
    1. 新しいログインを作成し、 `CONNECT` 、 `VIEW SERVER` 、 `VIEW ANY DEFINITION`権限を付与します。
    ...
    2. 次のステートメントを使用して、ユーザーに`public` DB アクセス権限を付与します。
  ...
  <Collapser id="domain-user" title="Windowsドメインユーザーの場合">
    1. 新しいログインを作成し、 `CONNECT` 、 `VIEW` 、 `VIEW ANY DEFINITION`権限を付与します。
...
<CollapserGroup>
  ...
  <Collapser id="basic-config-domain" title="Windowsドメイン認証を使用した基本構成">
  ...
  <Collapser id="multi-instance" title="マルチインスタンス接続">
  ...
  <Collapser id="metrics-connect-ssl" title="メトリック-SSL経由でのみ接続">
  ...
  <Collapser id="disable-buffer_reserve" title="バッファプールとパーティションリザーブメトリックの最適化">
...
インテグレーション データを検索するには、 <DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &amp;gt; Infrastructure &amp;gt; Third-party services**</DNT>に移動し、Microsoft SQL Server インテグレーション リンクの 1 つを選択します。
...
<CollapserGroup>
  <Collapser id="database-metric" title="データベースメトリック">
  ...
  <Collapser id="instance-metric" title="インスタンスメトリック">
  ...
  <Collapser id="wait-metric" title="待機メトリック">
