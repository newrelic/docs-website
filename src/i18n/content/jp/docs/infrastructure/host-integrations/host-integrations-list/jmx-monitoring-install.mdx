...
<CollapserGroup>
  ...
  <Collapser id="linux-install" title="Linux">
    ...
    5. <DNT>**Optional**</DNT>: Tomcat の監視に興味がある場合は、次のサンプル メトリクス ファイルを使用してください。
  ...
  <Collapser id="windows-install" title="ウィンドウズ">
    1. [.exeをダウンロードする New Relicの jmx インテグレーション用のインストーラ](https://download.newrelic.com/infrastructure_agent/windows/integrations/nri-jmx/nri-jmx-amd64-installer.exe)
...
<CollapserGroup>
  <Collapser id="jmx-instance" title="JMX インスタンスの設定">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            `COLLECTION_CONFIG` [アノテーションを使用して Kubernetes で`nri-jmx`を構成する](#metrics-collection-kubernetes-annotations)と便利です。
          </td>
        ...
        <tr>
          ...
          <td>
            エンタープライズごとに収集できるメトリックの数。 この制限を超えると、エンティティは報告されません。 制限が`0`の場合、制限がないことを意味します。
          </td>
  ...
  <Collapser id="labels" title="ラベルとカスタム属性">
...
<CollapserGroup>
  <Collapser id="domains" title="ドメイン">
...
<CollapserGroup>
  <Collapser id="jmx-configuration-configmap-example-using-helm" title="ヘルムチャートを使ったconfigMapの例（推奨）">
    ```yaml
    nri-bundle:
  ...
  <Collapser id="jmx-configuration-configmap-example-plain-manifest" title="プレーンなマニフェストを使ったconfigMapの例">
    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
...
<CollapserGroup>
  <Collapser id="host-connection-example" title="ホスト接続ファイルの例">
    ```yml
    integrations:
      - name: nri-jmx
        env:
  ...
  <Collapser id="collection-example" title="メトリクス収集ファイルの例">
    ```yml
    collect:
...
推奨事項:異なるアプリケーション間で同様のメトリクスには同じ命名スキームを使用してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        エンタープライズ タイプ`domain:`が先頭に付いた、これらのメトリックの JMX ドメイン名。
      </td>
    </tr>
...
JMX インテグレーションは、JMX インテグレーションの設定問題をキャプチャします。 データは、 [\[インベントリ\] ページ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)の<DNT>**config/jmx**</DNT>ソースで入手できます。 [インベントリデータの詳細については、「インテグレーション データを理解する」](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data)を参照してください。
...
<CollapserGroup id="troubleshooting-list">
  <Collapser id="integration-errors" title="エラーのログ検索">
  ...
  <Collapser id="metrics-limit" title="メトリクスの上限を超えた">
  ...
  <Collapser id="missing-dashboard" title="ダッシュボードがインフラストラクチャ UI に表示されない">
    構成`jvm-metrics.yml`ファイルが更新されていること、およびファイルへのパスが`jmx-config.yml`ファイルに列挙されていることを確認してください。
  </Collapser>

  <Collapser id="jmxterm" title="JMXTermによるトラブルシューティング">
