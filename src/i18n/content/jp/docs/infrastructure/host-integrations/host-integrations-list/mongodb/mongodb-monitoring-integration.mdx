...
当社の統合は、MongoDB v3 および v4 と互換性があります。
...
[MongoDB シェル](https://docs.mongodb.com/manual/mongo/)で、次のコマンドを実行して`listCollections`ロールと新しいユーザーを作成し、&#x7B; `clusterMonitor` } ロールと`listCollections`ロールを新しいユーザーに割り当てます。注: ユーザー名、パスワード、および同様のユーザー固有の値は置き換える必要があります。
...
<Callout variant="important">
  従来の構成/定義ファイルをまだ使用している場合は、 [標準の構成形式](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/)に関するドキュメントを参照してください。
</Callout>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="環境変数の置換"
  >
    この構成では、環境変数`MONGODB_HOST`を使用して、統合のHOST設定を設定しています。
  ...
  <Collapser
    ...
    title="フィルタ"
  >
    この構成では、フィルターを使用して、監視するデータベースとコレクションを絞り込みます。このサンプルフィルターを使用すると、統合は以下を収集します。
  ...
  >
    この構成では、同じ統合から複数のMongoDBサーバーを監視しています。最初のインスタンス（ `HOST: 1st_mongodb_host` ）の場合はメトリックとインベントリを収集し、2番目のインスタンス（ `HOST: 2nd_mongodb_host` ）の場合はメトリックのみを収集します。
