...
<Callout title="公開プレビュー">
  この統合は現在パブリック プレビューであり、近い将来、従来の統合に取って代わります。

  [レガシー](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-monitoring-integration) バージョンを使用している場合は、この新しいバージョンをテストすることをお勧めします。ガイダンスについては、 [移行](/docs/infrastructure/host-integrations/host-integrations-list/mongodb/mongodb-migration-guide) ページを確認してください。
</Callout>
...
<figcaption>
  [New Relic MongoDB Monitor クイックスタート](https://one.newrelic.com/nr1-core?state=c95ed954-7277-e44d-7fd7-351a77b3ff37)を介してインストールされたダッシュボード。
</figcaption>
...
当社の統合は、MongoDB 4.0 以降および [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb)と互換性があります。 MongoDB Atlas との互換性は近日公開予定です。
...
統合を構成するには、統合のYAML構成ファイル`mongodb3-config.yml`の構成を編集します。統合のYAML形式の構成では、必要なログインクレデンシャルを配置し、データの収集方法を構成できます。どのオプションを変更するかは、設定と設定によって異なります。構成ファイルには、すべての統合に適用できる共通の設定があります。

### MongoDBサーバーを有効にする [#enable-instance]

MongoDB シェルで次のコマンドを実行して、 `clusterMonitor`および`read`ロールを持つユーザーを作成します。注: ユーザー名、パスワードの値は置き換える必要があります。
...
2. 次のコマンドを使用して新しいユーザーを作成し、ユーザーに`clusterMonitor`と`read`の役割を割り当てます。
...
<CollapserGroup>
  <Collapser
    ...
    title="基本構成"
  >
    これは、すべてのメトリックを収集するために使用される基本構成です。**注**: 資格情報が必要な場合は、資格情報を mongodb UR に追加します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [構成ファイル](#instance-settings)で設定されたクラスター名。 **注:** このディメンションは、すべての指標に適用されます。
      </td>
    </tr>
