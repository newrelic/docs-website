...
* [`clusterMonitor`および`read`パーミッション](#enable-instance)を持つ MongoDB ユーザー。
...
指標を取得するには、統合に`clusterMonitor`および`readAnyDatabase`ロールを持つユーザーが必要です。環境に応じて、次の手順に従います。
...
<CollapserGroup>
  ...
  >
    [ユーザーの作成の](https://docs.mongodb.com/manual/reference/method/db.createUser/index.html)詳細については、MongoDB のドキュメントを参照してください。
...
<CollapserGroup>
  <Collapser
    ...
    title="アトラス"
  >
    [ユーザーの作成の](https://www.mongodb.com/docs/atlas/security-add-mongodb-users/#add-database-users)詳細については、MongoDB Atlas のドキュメントを参照してください。
    ...
    1. Atlas CLI を使用して次のコマンドを実行して新しいユーザーを作成し、 `clusterMonitor`および`readAnyDatabase`役割をユーザーに割り当てます (必要に応じてユーザー名とパスワードを調整します)。
       ...
       または、Atlas UI を使用してユーザーを作成し、同じ特定の権限を割り当てることもできます。
  </Collapser>
</CollapserGroup>
...
## mongodb3-config.yml サンプル ファイル [#examples]
...
* メトリックは、エンティティ`MONGODB_INSTANCE` 、 `MONGODB_DATABASE` 、および`MONGODB_COLLECTION`のメトリック サンプルに添付されます。
...
`MONGODB_INSTANCE` 、 `MONGODB_DATABASE` 、 `MONGODB_COLLECTION`の 3 つのエンティティが作成されます。
