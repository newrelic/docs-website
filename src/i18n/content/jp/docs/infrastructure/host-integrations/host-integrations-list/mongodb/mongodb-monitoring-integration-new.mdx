...
Bitnami エクスポーターを実行してスクレイピングすると、スクレイピングされるメトリックは同じで、ダッシュボードが機能します。ただし、統合により、特定の MongoDB クラスター`mongodb_cluster_name`から収集されたすべてのメトリックに共通のラベルが追加され、異なるクラスターからのメトリックをグループ化できるようになります。
