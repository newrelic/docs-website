...
<CollapserGroup>
  ...
  >
    ...
    Bitnami エクスポータまたは`percona/mongodb_exporter`を実行してスクレイピングすると、スクレイピングされたメトリクスは同じであり、 <InlinePopover type="dashboards"/>仕事。ただし、この統合により、特定の MongoDB クラスター`mongodb_cluster_name`から収集されたすべてのメトリックに共通のラベルが追加され、異なるクラスターからのメトリックをグループ化できるようになります。
