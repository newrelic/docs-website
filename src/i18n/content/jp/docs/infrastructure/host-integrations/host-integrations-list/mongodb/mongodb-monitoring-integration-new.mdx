...
指標を取得するには、統合に`clusterMonitor`および`read`ロールを持つユーザーが必要です。環境に応じて、次の手順に従います。
...
<CollapserGroup>
  ...
  >
    ...
    2. 次のコマンドを使用して新しいユーザーを作成し、 `clusterMonitor`と`read`の役割をユーザーに割り当てます (必要に応じてユーザー名とパスワードを調整します)。
