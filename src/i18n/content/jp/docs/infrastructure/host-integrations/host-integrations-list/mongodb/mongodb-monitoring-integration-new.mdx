...
当社の統合は、MongoDB バージョン 4.0 ～ 7.0、 [Percona Server](https://www.percona.com/software/mongodb/percona-server-for-mongodb) 、および MongoDB Atlas M10 以降と互換性があります。
...
<CollapserGroup>
  ...
  >
    ...
    4. お気に入りのエディタで `mongodb3-config.yml` 構成ファイルを編集します。[構成ファイルの例を](#examples)いくつか確認してください。

    5. Mongodb エラー ログの自動解析と転送を有効にするには、次の内容を含む YAML ファイルを `/etc/newrelic-infra/logging.d/elasticsearch-log.yml`に作成します。エージェントを再起動する必要はありません。
  ...
  >
    ...
    Bitnami エクスポータまたは`percona/mongodb_exporter`を実行してスクレイピングすると、スクレイピングされたメトリクスは同じであり、 <InlinePopover type="dashboards"/>は機能します。 ただし、統合では、特定の MongoDB クラスタから収集されたすべてのメトリクスに共通のラベルが追加されます。 `mongodb_cluster_name`これにより、異なるクラスタからのメトリクスをグループ化できるようになります。
    ...
    複数の`percona/mongodb_exporter`ポッドを使用して複数の MongoDB クラスターからのメトリクスを提供している場合は、次のような設定を使用して、それぞれに個別の`mongodb_cluster_name`のラベルを付けることができます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        監視対象のクラスタを一意に識別するためのユーザー定義の名前。 <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
    ...
    <tr>
      ...
      <td>
        MongoDB 接続 URI。 <DoNotTranslate>**Required**</DoNotTranslate>
      </td>
...
<CollapserGroup>
  <Collapser
    ...
    title="基本構成"
  >
    すべてのメトリクスを収集するための基本設定です。 必要に応じて、資格情報を mongodb URI に追加します。 注意: パスワードに特殊文字が含まれている場合は、URIを一重引用符で囲む必要があります。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * _空_- スタンドアロン インスタンスおよび Atlas の場合
      </td>
    </tr>
...
次のディメンションメトリックは、エクスポーターをスクレイピングしてキャプチャされ、エンティティ<DoNotTranslate>**MONGODB_INSTANCE**</DoNotTranslate>にリンクされます。
...
次のディメンションメトリックは、エクスポーターをスクレイピングしてキャプチャされ、エンティティ<DoNotTranslate>**MONGODB_COLLECTION**</DoNotTranslate>にリンクされます。
...
次のディメンションメトリックは、エクスポーターをスクレイピングしてキャプチャされ、エンティティ<DoNotTranslate>**MONGODB_DATABASE**</DoNotTranslate>にリンクされます。
...
さらに、この統合は[コミュニティによって作成された Prometheus エクスポータ](https://github.com/newrelic/newrelic-prometheus-exporters-packages/tree/main/exporters/mongodb3)を利用します。
