...
1. 手順に従って、ファイル名`nri-postgresql`を使用し[てインフラストラクチャ統合パッケージをインストールし](/docs/integrations/host-integrations/installation/install-host-integrations-built-new-relic)ます。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="PostgreSQL Azure/AWS 構成ファイル"
  >
    * Azure/AWS SSL 対応オプション: AWS RDS/Aurora および Azure フレキシブル マネージド データベース オファリングでは、通常、接続に SSL が必要です。この要件に対応するには、これらの設定を`true`に設定する必要があります。
    ...
    * 概要: これらの設定が完了すると、完全な Azure/AWS 構成ファイルは次のようになります。注: インフラ エージェントと Postgresql の統合は、データベース インスタンスにネットワーク アクセスできるホストにインストールする必要があります。複数のインスタンス構成の場合、以下に示すように、追加のインスタンスごとに`-name:`で始まるセクションを繰り返す必要があります。
