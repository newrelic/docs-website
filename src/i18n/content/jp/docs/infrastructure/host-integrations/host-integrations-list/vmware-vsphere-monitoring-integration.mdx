...
import infrastructureVmwareVsphereUi from 'images/infrastructure_screenshot-full_vmware-vsphere-ui.webp'
...
<img
  title=""
  alt=""
  src={infrastructureVmwareVsphereUi}
/>
...
<Callout variant="important">
  <DoNotTranslate>**Large environments:**</DoNotTranslate> 800 を超える環境では、インテグレーションがすべてのデータを報告できず、失敗する可能性があります。 すべてのメトリクスとイベントを保持する回避策を提供していますが、エンティティの登録は無効になります。 回避策を適用するには、設定ファイルに次の環境変数を追加します。
...
2. <DoNotTranslate>**Add data**</DoNotTranslate>
...
<CollapserGroup>
  ...
  >
    ...
    イベントは<DoNotTranslate>**Events**</DoNotTranslate>ページで利用でき、 `vSphereEvent`の下の`InfrastructureEvent`として[NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql)経由でクエリできます。 vSphere イベント データの例を次に示します。
  ...
  >
    パフォーマンス メトリクスは、VMware リソースの現在の状態をより適切に把握するのに役立ち、デフォルトで収集され、ページの下部で説明されているサンプルに含まれるメトリクスに収集できます。 <DoNotTranslate>**in addition**</DoNotTranslate>
