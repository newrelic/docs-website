...
<CollapserGroup>
  <Collapser
    ...
    title="Linuxのインストール"
  >
    1. [インフラストラクチャ エージェントをインストールし](/docs/integrations/host-integrations/installation/install-infrastructure-host-integrations/#install)、 `INTEGRATION_FILE_NAME`変数を`nri-vsphere`に置き換えます。
