...
この統合では、 [Metric API](/docs/data-ingest-apis/get-data-new-relic/metric-api/introduction-metric-api)と[Event API](/docs/insights/insights-data-sources/custom-data/introduction-event-api)を使用してデータを取り込みます。これらの API を使用するには、<InlinePopover type="licenseKey" />が必要です。
...
StatsD 統合をインストールするには、次のコマンドを実行し、 [New Relic アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)と<InlinePopover type="licenseKey" />を含めます。これにより、 `gostatsd`で使用される TOML 構成ファイルが生成されます。
...
<CollapserGroup>
  ...
  >
    以下は、Kubernetes 環境に StatsD をデプロイし、 `newrelic-statsd`という名前の StatsD サービスを作成するための Kubernetes マニフェストの例です。[アカウント ID](/docs/accounts/install-new-relic/account-setup/account-id)と<InlinePopover type="licenseKey" />を挿入する必要があります。
