...
StatsD統合をインストールするには、次のコマンドを実行し、NewRelic[アカウントID](/docs/accounts/install-new-relic/account-setup/account-id)とNewRelic[ライセンスキー](#requirements)を含めます。これにより、 `gostatsd`で使用されるTOML構成ファイルが生成されます。
...
<CollapserGroup>
  ...
  >
    以下は、Kubernetes環境にStatsDをデプロイし、 `newrelic-statsd`という名前のStatsDサービスを作成するためのKubernetesマニフェストの例です。[アカウントID](/docs/accounts/install-new-relic/account-setup/account-id)と[ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)を挿入する必要があります。
...
このインテグレーションは、オープンソース・ソフトウェアです。つまり、 [そのソースコードを閲覧して](https://github.com/newrelic/nri-statsd/ "リンクが新しいウィンドウで開きます。") 改良を送ったり、独自のフォークを作成して構築することができます。