...
1. **Oracle DB 12c以降を使用している場合は、** `ALTER SESSION`を使用してデータベースにアクセスし、ユーザーとユーザープロパティを管理します。 Oracle DBのバージョンが12c未満の場合は、このクエリを実行しないでください。

   ```
   ALTER SESSION set "_Oracle_SCRIPT"=true;
   ```

2. `CREATE USER`を使用して、データベースに新しいユーザーを追加します。 <var>USER_PASSWORD</var>を[新しいユーザーのパスワード](https://docs.oracle.com/en/database/oracle/oracle-database/12.2/dbseg/keeping-your-oracle-database-secure.html#GUID-451679EB-8676-47E6-82A6-DF025FD65156)に置き換えます。
   ...
   <Callout variant="tip">
     ユーザーメンテナンスに関する質問については、 [Oracle documentation](https://docs.oracle.com/en/database/oracle/oracle-database/index.html) を参照するか、システムまたはデータベースの管理者にお問い合わせください。
   </Callout>

3. ユーザーに`CONNECT`権限を付与します。

   ```
   GRANT CONNECT TO <var>USERNAME</var>;
   ```

4. 次のグローバルビューでユーザーに`SELECT`権限を付与します。
