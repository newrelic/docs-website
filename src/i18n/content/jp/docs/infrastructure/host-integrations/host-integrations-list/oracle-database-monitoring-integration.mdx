...
* [New Relic Infrastructure エージェントによってサポートされる](/docs/infrastructure/new-relic-infrastructure/getting-started/compatibility-requirements-new-relic-infrastructure)Linux ディストリビューション。

  <Callout variant="important">
    RHEL、CentOS、および OEL バージョン 7 より前はサポートされていません。
  </Callout>

* [インフラストラクチャエージェントがホストにまだインストールされていない場合は、インフラストラクチャエージェントをインストールします](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic)。

* インフラストラクチャエージェントが実行されているのと同じホストに[Oracle Instant Client](http://www.Oracle.com/technetwork/database/database-technologies/instant-client/downloads/index.html)をインストールします。

* 正しい[Oracle データベース ディレクトリ](https://docs.oracle.com/database/121/ADMQS/GUID-EC18C4A6-3BA5-4C14-9D76-B0DD62FEFFF2.htm#ADMQS12369)を参照するように、ルート ユーザーの`ORACLE_HOME`環境変数を構成します。

* 必要なグローバル ビューに対する[<DNT>**CONNECT**</DNT>権限と<DNT>**SELECT**</DNT>権限の両方を](#enable-instance)持つ Oracle データベース ユーザー。

* Oracle データベース リスナー`listener.ora`は、リモート接続を受け入れるように構成する必要があります。デフォルトでは、Oracle データベースはローカルホスト上の接続のみをリッスンします。
...
1. お持ちのデータベースの種類を選択してください。ユーザーの保守に関する質問については、 [Oracle のドキュメント](https://docs.oracle.com/en/database/oracle/oracle-database/index.html)を参照するか、システム管理者またはデータベース管理者に問い合わせてください。
   ...
   <CollapserGroup>
     <Collapser id="standalone-database" title="スタンドアロン データベース (Oracle DB 12c 以降)">
     ...
     <Collapser id="multitenant-database" title="マルチテナント データベース">
       ...
       新しいユーザーに、すべてのコンテナ オブジェクト (または「CONTAINER\_DATA」で PDB コンテナ名とルート コンテナ名を指定して特定のコンテナ) にアクセスする権限を付与します。
...
<CollapserGroup>
  ...
  <Collapser id="metrics-extended" title="拡張されたメトリクスとテーブルスペースのフィルタリングによるメトリクスオンリー">
  ...
  <Collapser id="metrics-skip" title="メトリックグループをスキップする">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Plscope\_settingsは、PL/SQLソースコード識別子およびSQLステートメントデータのコンパイル時の収集、相互参照、および保存を制御します。
          </td>
        </tr>
...
<CollapserGroup id="troubleshooting-list">
  <Collapser id="library-not-loaded" title="Oracleのライブラリが読み込めない">
