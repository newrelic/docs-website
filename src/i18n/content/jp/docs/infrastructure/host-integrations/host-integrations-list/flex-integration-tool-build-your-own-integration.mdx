...
2. 設定するシステムとオンホストインテグレーションが要件を満たしていることを確認するには、[Infrastructureエージェントの要件](/docs/infrastructure/install-infrastructure-agent/get-started/requirements-infrastructure-agent/)を参照してください。
3. Flexは、Linux、Windows、またはKubernetesで実行される[Infrastructureエージェント](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/)バージョン 1.10.7以降にバンドルされています。
...
<Steps>
  ...
  <Step>
    ...
    ここでの目標は、エージェントがすでにサポートしているファイルシステムを除外しながら、ファイルシステムと1バイトブロックを表示して、`df`コマンドの出力を処理することです。サポートされていないファイルシステムがマウントされていない場合は、`-x`引数を削除してください。
