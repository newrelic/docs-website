...
<Callout variant="tip">
  新しい OpenTelemetry オプションが利用可能になりました!代わりに[、 Elasticsearch OpenTelemetry](/docs/opentelemetry/integrations/elasticsearch/elasticsearch-otel-integration-overview)インテグレーションの使用を検討してください。 業界標準のOpenTelemetryプロトコルを使用して、ベンダーニュートラル監視、強化されたパフォーマンス、将来性のあるアーキテクチャーを提供します。
</Callout>
