...
<Callout variant="important">
  MySQL v8.0以降では、次のメトリクスはサポートされていません：`db.qCacheFreeMemoryBytes`、`db.qCacheHitRatio`および`db.qCacheNotCachedPerSecond`。
</Callout>

### TLSサポート [#supported-tls]

TLS認証には、最小バージョンのTLS1.2が必須です。
...
* MySQLがKubernetesまたはAmazon ECSで実行されていない場合、LinuxまたはWindows OSホストに、またはMySQLがインストールされている場所にリモートアクセスできるホストに[Infrastructureエージェントをインストール](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic)できます。その他の場合：
  ...
  * <img style={{ width: '32px', height: '32px'}} class="inline" title="Kubernetes" alt="Kubernetes" src={osk8}>で実行中の場合</img>

    Kubernetesで実行されている場合は、[これらの要件](/docs/monitor-service-running-kubernetes#requirements)をご覧ください。

  * <img style={{ width: '32px', height: '32px'}} class="inline" title="ECS" alt="ECS" src={osEcs}>で実行中の場合</img>
...
<CollapserGroup>
  ...
  >
    ...
    5. 自動MySQLエラーログ解析および転送を有効化するには、`mysql-log-win.yml.example`ファイルを`mysql-log-win.yml`にコピー（または名前変更）します。
       ...
       **Powershellの例：**
       ...
       ```
       Rename-Item -Path "C:\Program Files\New Relic\newrelic-infra\logging.d\mysql-log-win.yml.example" -NewName "mysql-log-win.yml"
       ```
...
* <img style={{ width: '32px', height: '32px'}} class="inline" title="Kubernetes" alt="Kubernetes" src={osk8}>で有効化した場合</img>

  Kubernetesで有効化した場合、[Kubernetes上で実行中のサービスの監視](/docs/monitor-service-running-kubernetes)を参照してください。

* <img style={{ width: '32px', height: '32px'}} class="inline" title="ECS" alt="ECS" src={osEcs}>で有効化した場合</img>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="MySQL AzureフレキシブルおよびAWS RDS/Aurora設定ファイル"
  >
    * Azure/AWS SSL対応オプション：Azureフレキシブルのマネージドデータベースサービスに接続するにはSSLが必要です。MySQLバージョンが5.7以上で、Auroraパラメーターグループでrequire_secure_transportがONに設定されている場合は、AWS RDS/AuroraにSSLが必要になる場合があります。SSL要件を満たすには、mysql-config.ymlのこの設定を`true`に設定する必要があります。
    ...
    * 概要：これらの設定が整うと、完全なAzure/AWS設定ファイルは以下のようになります。注意：infraエージェントとMySQLインテグレーションは、データベースインスタンスへのネットワークアクセスを持つホストにインストールする必要があります。
