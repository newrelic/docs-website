...
* Windows <img title="Windows" alt="Windows" src={windows} />

* Linux <img title="Linux" alt="Linux" src={linux} />
...
<CollapserGroup>
  ...
  >
    [設定ファイル](#examples)で拡張メトリクスフラグが有効化されており、[`cluster.slaveRunning`メトリック](#slaverunning)が`1`を返す場合に取得される追加メトリクス。詳細については、[MySQL文書](https://dev.mysql.com/doc/refman/5.7/en/show-slave-status.html)を参照してください。
