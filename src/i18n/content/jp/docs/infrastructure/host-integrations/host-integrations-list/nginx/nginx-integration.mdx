...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="NGINX Plus"
  >
    New Relicのインテグレーションは、以下の[HTTP API](http://nginx.org/en/docs/http/ngx_http_api_module.html)エンドポイント：`/nginx`、`/processes`、`/connections`、`/http/requests`および`/ssl`から、使用できるすべてのメトリックデータを取得します。
