...
4. お気に入りのエディターで、`nginx-config.yml`設定ファイルを編集します。[設定ファイルの例](#examples)をいくつかご覧ください。
...
* <img style={{ width: '32px', height: '32px'}} class="inline" title="Kubernetes" alt="Kubernetes" src={osk8}>経由で有効にされた場合</img>

  Kubernetesで有効化した場合、[Kubernetes上で実行中のサービスの監視](/docs/monitor-service-running-kubernetes)を参照してください。

* <img style={{ width: '32px', height: '32px'}} class="inline" title="ECS" alt="ECS" src={osEcs}>経由で有効にされた場合</img>
