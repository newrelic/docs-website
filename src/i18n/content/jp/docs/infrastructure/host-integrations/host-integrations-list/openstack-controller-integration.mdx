...
2. 以下のコマンドは、クローンされたリポジトリに存在する `openstack-agent` ディレクトリを開くために使用されます。
...
3. ファイル名 `config/os-config.json.template` を `config/os-config.json` に変更し、以下のようにファイルを編集します。
   ...
   * `nr_agent_home`を更新するには、クローンされたリポジトリから **config/** フォルダーの親ディレクトリを入力する必要があります。
...
4. 以下のコマンドを使用して、 **スクリプト** フォルダーの実行権限を付与します (**scripts/flex-osmetrics.sh** は New Relic Infrastructure Agent によって呼び出されます)。

```shell
chmod +x scripts/flex-osmetrics.sh
```

5. **config/os-config.json** でそのリソースの **enabled** を **false**に設定することで、リソースのキャプチャを無効にできます。
...
<CollapserGroup>
  ...
  >
    ...
    2. ファイル `flex-openstack-system.yml` を次の構成で更新します。
       ...
       * yaml ファイルの **variable_store** スタンザで、 openstack インストールの値を修正するために、 `authUrl` 、 `userDomainID` 、 `username` 、および `password` 変数の値を設定します。
  ...
  >
    ...
    2. ファイル `openstack.block_storage.yml` を次の構成で更新します。
       ...
       * yaml ファイルの **variable_store** で、 `nrAgentHome` 変数の値を New Relic openstack エージェントのホーム ディレクトリ ( `ohi-integration/` フォルダの親ディレクトリ) に設定します。
  ...
  >
    ...
    2. ファイル `openstack.keystone.yml` を次の構成で更新します。
       ...
       * yaml ファイルの **variable_store** で、 `nrAgentHome` 変数の値を New Relic openstack エージェントのホーム ディレクトリ ( `ohi-integration/` フォルダの親ディレクトリ) に設定します。設定ファイルが更新されると、次のようになります。
...
OpenStack クイックスタートをインストルメント化し、メトリクスとアラートを確認するには、 **Install now** \[今すぐインストール] ボタンがある OpenStack [コントローラー クイックスタート ページに](https://newrelic.com/instant-observability/openstack-controller) 従うこともできます。
