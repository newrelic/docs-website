...
MongoDB統合は、インベントリとメトリックを収集してMongoDBクラスターからプラットフォームに送信します。プラットフォームでは、主要なパフォーマンスメトリックを集約して視覚化できます。パフォーマンスのボトルネックを特定するために、mongos、mongods、configサーバー、およびデータベースとコレクションに関するデータを収集します。

統合をインストールし、収集するデータを確認するために読んでください。

## 互換性と要件 [#comp-req]

私たちの統合はMongoDBv3およびv4と互換性があります。 MongoDB Atlasは、ティアM10以降でサポートされています。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. ファイル名`nri-mongodb`を使用して[、統合をインストールする](/docs/install-integrations-package)ための指示に従います。
...
[MongoDBシェル](https://docs.mongodb.com/manual/mongo/)で、次のコマンドを実行して`listCollections`ロールと新しいユーザーを作成し、 `clusterMonitor`と`listCollections`のロールを新しいユーザーに割り当てます。注：<var>ユーザー名</var>、<var>パスワード</var>、および同様のユーザー固有の値を置き換える必要があります。
...
MongoDB統合は、完全なMongoDBクラスターまたはスタンドアロンのMongoDBインスタンスのいずれかをモニターするように構成できます。クラスタを監視するには、 `host` }引数と`port`[引数](#arguments)がクラスタ内の`mongos`インスタンスの1つを指している必要があります。スタンドアロンインスタンスを監視する場合は、sue `host`と`port`がデータベースを実行している`mongod`を指すようにします。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ジャーナル処理に使用されるメモリを含む、マップされたメモリの量（バイト単位）。
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            データベースプロセスによって現在使用されているメモリの量（バイト単位）。
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            mongodプロセスによって使用される仮想メモリの量（バイト単位）。
          </td>
        </tr>
...
この統合はオープンソースソフトウェアです。つまり[、ソースコードを参照して](https://github.com/newrelic/nri-mongodb "リンクが新しいウィンドウで開きます。")改善を送信したり、独自のフォークを作成してビルドしたりできます。