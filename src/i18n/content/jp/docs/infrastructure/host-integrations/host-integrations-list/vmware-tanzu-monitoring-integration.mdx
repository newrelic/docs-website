...
この統合は、VMware Tanzu Application Service バージョン 2.7 ～ 2.13、および Ops Manager バージョン 2.5 ～ 2.10 と互換性があります。BOSH 幹細胞は、Ubuntu Xenial に基づいている必要があります。
...
VMware Tanzu 統合をインストールしてアクティブ化すると、 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Third-party services > VMware Tanzu ダッシュボード**でデータと定義済みのチャートを見つけることができます。データを照会してカスタム グラフとダッシュボードを作成し、それらをアカウントに追加できます。
