...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. ファイル名`nri-couchbase`を使用して[、統合をインストールする](/docs/install-integrations-package)ための指示に従います。
...
このインテグレーションは、オープンソース・ソフトウェアです。つまり、 [そのソースコードを閲覧して](https://github.com/newrelic/nri-couchbase "リンクが新しいウィンドウで開きます。") 改良を送ったり、独自のフォークを作成して構築することができます。