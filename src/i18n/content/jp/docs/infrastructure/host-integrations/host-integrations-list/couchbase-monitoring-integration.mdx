...
当社のCouchbase[インテグレーション](/docs/infrastructure/host-integrations/get-started/introduction-host-integrations)は、Couchbaseクラスタからインベントリデータおよびメトリクスデータを収集してNew Relicプラットフォームに送信します。これにより、Couchbase環境の健全性をすばやく確認できます。クラスタ、ノード、クエリエンジン、およびバケットレベルでデータを収集するため、問題をその発生源までトレースできます。
...
<ButtonGroup>
  ...
  <ButtonLink role="button" to="https://onenr.io/0nQxm7D77wV" variant="primary">
    ガイド付きインストール（JPオプション）
  </ButtonLink>
</ButtonGroup>
...
インテグレーションデータは、[インテグレーションダッシュボード](/docs/infrastructure/infrastructure-data/infra-integration-data)にレポートされます。
