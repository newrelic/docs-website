...
<CollapserGroup>
  ...
  >
    ...
    6. Rabbitmqエラーログの自動解析と転送を有効にするには、 `rabbitmq-log.yml.example` ファイルを `rabbitmq-log.yml` にコピー（またはリネーム）します。エージェントを再起動する必要はありません。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ファイル記述子の合計数。RabbitMQでは、これは `fd_total` と表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ソケットとして利用可能なファイル記述子の合計数。RabbitMQでは、これは `sockets_total` と表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Erlangのプロセス制限。RabbitMQでは、これは `proc_total` となります。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        使用されるErlangのプロセス。RabbitMQでは、これは `proc_used` となります。
      </td>
    </tr>
...
<CollapserGroup id="troubleshooting-list">
  <Collapser
    ...
    title={<>ローカルノード名の取得エラー：exec: <InlineCode>"rabbitmqctl": executable file not found in $PATH</InlineCode></>}
  >
