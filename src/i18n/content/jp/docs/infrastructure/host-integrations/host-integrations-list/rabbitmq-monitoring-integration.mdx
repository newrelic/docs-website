...
New Relic RabbitMQ [on-host integration](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) は、クラスタ、vhost、キュー、エクスチェンジ、コンシューマーに関連する重要なメトリクスを含む、RabbitMQ サービスからのメトリクスおよび構成データをレポートします。
...
- Metric データは RabbitMQ バージョン 3.0 以上。
- Inventory データ用の RabbitMQ バージョン 3.7 以上。

インテグレーションをインストールする前に、以下の要件を満たしていることを確認してください。

- RabbitMQ [Management Plugin](https://www.rabbitmq.com/management.html) の設定。

- RabbitMQ のコマンドラインツールである [rabbitmqctl](#rabbitmqctl) 、ルートユーザーの `PATH` にある。

- **RabbitMQ が Kubernetes または Amazon ECS 上で** 実行されていない場合は、 [インフラストラクチャエージェント](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic) を RabbitMQ を実行しているホストにインストールする必要があります。それ以外の場合は
...
ガイド付きインストールで、RabbitMQ サービスをすばやくインスツルメンテーションし、テレメトリーデータを送信します。ガイド付きインストールでは、お客様の環境に合わせてカスタマイズされた CLI コマンドを作成し、New Relic CLI とインフラストラクチャ・エージェントをダウンロードしてインストールします。
...
ガイド付きインストールでは、インフラストラクチャエージェントを使用して RabbitMQ の統合を設定します。それだけでなく、環境で稼働している他のアプリケーションやログソースを検出し、どのアプリケーションを導入すべきかを推奨してくれます。

ガイド付きインストールは、ほとんどのセットアップで動作します。しかし、ニーズに合わない場合は、以下の他の方法で RabbitMQ サービスのモニタリングを始めることができます。

## インストールと有効化 [#install]

RabbitMQ インテグレーションをインストールするには、お使いの環境の指示に従ってください。
...
- オンホストでインストールした場合：統合の YAML 設定ファイル（ `rabbitmq-config.yml）の設定を編集する`.
...
RabbitMQ に関連する特定の設定は、設定ファイルの `env` セクションを使用して定義します。これらの設定は、RabbitMQ インスタンスへの接続や、その他のセキュリティ設定や機能を制御します。有効な設定のリストは、このドキュメントの次のセクションで説明します。

**クラスター環境**

クラスタ環境では、1 つのノードのみが `METRICS=false` および `INVENTORY=false` を使用し、残りのノードは `INVENTORY=true` および `METRICS=false` を使用する必要があります。

Kubernetes でクラスタ環境を実行している場合は、RabbitMQ を [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/) としてデプロイし、RabbitMQ ポッドからすべてのメトリクスを照会するようにエージェントを構成する必要があります。コンフィグファイル のオートディスカバリーマッチング条件 [をこの値に設定します。](https://github.com/newrelic/nri-rabbitmq/blob/master/rabbitmq-config.yml.k8s_sample)
...
[RabbitMQ インテグレーションは、Metrics(<strong> M</strong> )と Inventory(<strong> I</strong> )の両方の情報を収集します。以下の **Applies To** カラムをチェックして、各コレクションに使用できる設定を確認してください。](https://github.com/newrelic/nri-rabbitmq/blob/master/rabbitmq-config.yml.k8s_sample)
...
- `{{}}` 表記を使用して環境変数から値を置き換えます。これには、インフラストラクチャエージェント v1.14.0+が必要です。詳しくはこちら.
...
RabbitMQ 統合では、以下のメトリック・データ属性を収集します。各メトリック名の前には、queue.や node.のようにカテゴリー・インジケータとピリオドが付きます。

### RabbitMQ vhost サンプルイベント [#vhostsample]
...
### RabbitMQ ノードサンプルイベント [#nodesample]
...
### RabbitMQ 交換サンプルイベント [#exchangesample]
...
### RabbitMQ キューのサンプルイベント [#queuesample]
