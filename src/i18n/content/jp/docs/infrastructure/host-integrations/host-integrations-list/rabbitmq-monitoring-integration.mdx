...
<CollapserGroup>
  <Collapser
    ...
    title="Ansible"
  >
    [設定管理ツール](/docs/infrastructure/install-infrastructure-agent/linux-installation/install-configuration-management-tools) を使用して[ 、インフラストラクチャエージェント](/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring/) の使用を開始します。
  </Collapser>
  ...
  >
    ...
    5. 自動Rabbitmqエラーログの解析と転送を有効にするには、 `rabbitmq-log.yml.example`ファイルを`rabbitmq-log.yml`にコピー（または名前変更）します。エージェントを再起動する必要はありません。
...
* <DoNotTranslate>**Advanced:**</DoNotTranslate>
  ...
  インテグレーションは、パッケージマネージャの外部にインストールできるように、 [tarball 形式](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations/#tarball)でも利用できます。
...
RabbitMQ インテグレーションは、メトリクス ( <strong>M</strong> ) 情報とインベントリ ( <strong>I</strong> ) 情報の両方を収集します。 それぞれの特定のコレクションに使用できる設定を確認するには、以下の<DoNotTranslate>**Applies To**</DoNotTranslate>列を確認してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        処理できるキューの最大数を定義します。この数に達すると、すべてのキューが削除されます。 '0' と定義されている場合、制限は適用されません。これは、エージェントのメモリの増加を回避するために`DISABLE_ENTITIES=true`と共に使用されます。
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ノード ディスク アラーム ( `0`または`1` )。 `0`アラームが作動していないことを示し、 `1`アラームが作動していることを示します。 RabbitMQ では、これは`disk_free_alarm`として表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ホスト メモリ アラーム ( `0`または`1` )。 `0`アラームが作動していないことを示し、 `1`アラームが作動していることを示します。 RabbitMQ では、これは`mem_alarm`として表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ノードが実行中です（ `0`または`1` ）。 `0`ノードが実行されていないことを示し、 `1`ノードが実行中であることを示します。 RabbitMQ では、これは`running`として表示されます。
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        クライアントに配信されたがまだ確認されていない、キューあたりのメッセージの 1 秒あたりのレート。 RabbitMQ では、これは`message_stats.deliver_no_ack_details.rate`として表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        1 秒あたりキューあたり消費者に確認応答モードで配信されるメッセージのレート。 RabbitMQ では、これは`message_stats.deliver_details.rate`として表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        キューごとに、消費者に確認応答モードで配信されたメッセージ、消費者に非確認応答モードで配信されたメッセージ、 `basic.get`に応答して確認応答モードで配信されたメッセージ、および`basic.get`に応答して非確認応答モードで配信されたメッセージの合計。 RabbitMQ では、これは`message_stats.deliver_get`として表示されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        キューごとに、消費者に確認応答モードで配信されたメッセージ、消費者に非確認応答モードで配信されたメッセージ、 `basic.get`に応答して確認応答モードで配信されたメッセージ、および`basic.get`に応答して非確認応答モードで配信されたメッセージの合計の 1 秒あたりのレート。 RabbitMQ では、これは`message_stats.deliver_get_details.rate`として表示されます。
      </td>
    </tr>
...
インテグレーションは、 `/etc/rabbitmq/rabbitmq.conf`ファイルに RabbitMQ の設定ファイルをキャプチャします。 インベントリデータは[ RabbitMQ バージョン 3.7](http://www.rabbitmq.com/blog/2018/02/05/whats-new-in-rabbitmq-3-7/) [](/docs/infrastructure/integrations-getting-started/getting-started/understand-integration-data-data-types#inventory-data)[<DoNotTranslate>**Inventory**</DoNotTranslate>](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)以上でのみキャプチャされます。 インベントリデータは、 インフラストラクチャUI の ページ の`config/rabbitmq` ソースの下に表示されます。

<Callout variant="caution">
  `rabbit.conf`ファイルに入力した機密情報は、インフラストラクチャ UI の<DoNotTranslate>**Inventory**</DoNotTranslate>ページに表示されることに注意してください。 これには、AWS からの次のような項目が含まれます。
