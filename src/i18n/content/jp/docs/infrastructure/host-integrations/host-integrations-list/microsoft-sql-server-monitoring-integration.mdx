...
当社の Microsoft SQL Server [統合](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) は、お客様の MS SQL Server 環境からインベントリとメトリクスを収集して当社のプラットフォームに送信し、お客様は MS SQL Server 環境の健全性を確認することができます。データベースレベルとインスタンスレベルの両方のメトリクスを収集するので、問題の原因をピンポイントで特定することができます。

インテグレーションのインストールと、当社がどのようなデータを収集するかについてお読みください。

## 互換性および要件 [#about]

当社の統合には、Microsoft SQL Server 2008 R2 SP3 以上が必要です。
...
ガイド付きインストールにより、MS SQL Server 環境を迅速にインスツルメンテーションし、遠隔測定データを送信します。ガイド付きインストールでは、お客様の環境に合わせてカスタマイズされた CLI コマンドを作成し、New Relic CLI とインフラストラクチャー・エージェントをダウンロードしてインストールします。
...
ガイド付きインストールは、ほとんどのセットアップに対応しています。しかし、ニーズに合わない場合は、以下の他の方法で MS SQL Server 環境の監視を始めることができます。

## インストールと有効化 [#install]

Microsoft SQL Server インテグレーションをインストールするには

1. 最新の.MSI インストーライメージをこちらからダウンロードします。
...
3. [Infrastructure エージェントを再起動します](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status)。
...
### MS SQL Server の有効化 [#enable-instance]

監視対象の Microsoft SQL Server において、以下のスクリプトを実行して新しいユーザーを作成し、そのユーザーに `CONNECT` 、 `VIEW SERVER STATE` 、および読み取りのアクセス権限を付与します。
...
<Callout variant="important">
  もし、まだ当社のレガシーの設定/定義ファイルをお使いの場合は、この
  [のドキュメント](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/)
  を参考にしてください。
</Callout>

### インスタンスの設定 [#instance-settings]

SQL Server の統合では、Metrics(**M**)と Inventory(**I**)の両方の情報を収集します。表の中で、 **Applies to** 列を使って、各コレクションで利用できる設定を確認します。
