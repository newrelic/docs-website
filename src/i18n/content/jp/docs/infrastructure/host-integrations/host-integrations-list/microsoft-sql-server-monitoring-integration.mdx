...
* `CONNECT と VIEW SERVER STATE の両方、および READ のアクセス権限を持つ、 ユーザー権限を持つ SQL Server ユーザーまたはドメインユーザーが必要です。`````[](#users-privileges)
* [](#users-privileges)

## [クイックスタート [#quick]](#users-privileges)

[ガイド付きインストールにより、MS SQL Server 環境を迅速にインスツルメンテーションし、遠隔測定データを送信します。ガイド付きインストールでは、お客様の環境に合わせてカスタマイズされたCLIコマンドを作成し、New Relic CLIとインフラストラクチャー・エージェントをダウンロードしてインストールします。](#users-privileges)

[![ガイド付きインストールCLIのスクリーンショット。](./images/guided-install-cli.png "ガイド付きインストールCLI。")](#users-privileges)

[始める準備はできていますか？ボタンの1つをクリックし、試行してください。](#users-privileges)
...
<ButtonGroup>
  ...
  >
    EUガイド付きインストール
  </ButtonLink>](#users-privileges)
</ButtonGroup>

[ガイド付きインストールでは、インフラストラクチャ・エージェントを使用して Microsoft SQL Server の統合を設定します。それだけでなく、お客様の環境で稼働している他のアプリケーションやログソースを検出し、どのアプリケーションを導入すべきかを推奨します。](#users-privileges)

[ガイド付きインストールは、ほとんどのセットアップに対応しています。しかし、ニーズに合わない場合は、以下の他の方法でMS SQL Server環境の監視を始めることができます。](#users-privileges)

## [インストールと有効化 [#install]](#users-privileges)
...
1. [* 最新の.MSIインストーライメージをこちらからダウンロードします。
     ...
     </CollapserGroup>* アドミニストレーターのアカウントで、絶対パスを使用してインストールスクリプトを実行します。
...
2. [](#users-privileges)
   ...
   [In `C:\Program Files\New Relic\newrelic-infra\integrations.d\` directory, rename (or copy) `mssql-config.yml.sample` file to `mssql-config.yml`, and edit according to your ](#users-privileges)[configuration needs](/docs/infrastructure/host-integrations/host-integrations-list/microsoft-sql-server-monitoring-integration/#config).

3. Infrastructureエージェントを[再起動します](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status)。
