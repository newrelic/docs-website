...
当社の JMX [統合](/docs/integrations/host-integrations/getting-started/introduction-host-integrations) では、JMX でメトリクスを公開しているあらゆるアプリケーションを監視することができます。この統合には、JVM から主要なメトリクスを自動的に収集するデフォルトの収集ファイルが含まれています。また、YAML ファイルを使ってメトリクスの収集をカスタマイズし、任意のサブセットを収集することもできます。
...
- JMX が Kubernetes や Amazon ECS 上で 稼働していない場合は、 [JMX が稼働しているホストにインフラストラクチャエージェント](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic) をインストールする必要があります。それ以外の場合は
...
- 本製品は IIOP プロトコルに対応していません。

## インストールと有効化 [#install]

JMX インテグレーションをインストールするには、お使いの環境に応じた指示に従ってください。
...
- `nrjmx` と `jmx` は、システムの `PATH` に Java バイナリが必要です。これは、root ユーザーの `PATH` とは異なり、 `/root/.bashrc` で `PATH` 環境変数を変更しても影響はありません。 [PATH システム変数を変更する方法については、](https://www.java.com/en/download/help/path.html) をご覧ください。
...
<Callout variant="important">
  従来の設定・定義ファイルを使用している場合は、
  [標準設定書](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/)
  を参考にしてください。
</Callout>
...
メトリクス・コレクション定義ファイルは構造化された YAML ファイルで、どのようなメトリクスを収集するかをインテグレーションに伝えます。構成例については、 [メトリクス・コレクション・ファイルの例](#collection-example) を参照してください。

デフォルトの JVM メトリクス収集ファイル： `/etc/newrelic-infra/integrations.d/jvm-metrics.yml`

<Callout variant="tip">
  整理やメンテナンスを容易にするために、異なるコレクションファイルを書くことができます。例として、
  [設定ファイル](#collection-example) を参照してください。
</Callout>
...
<Callout variant="important">
  設定ファイルには、1つのインスタンスにつき200のメトリクスの制限があります。特定のインスタンスで制限を超えた場合は、New
  Relic に送信されません。New Relic にデータが表示されない場合は、
  [トラブルシューティング手順](#troubleshoot)
  を確認して、制限を超えていないかどうかを確認してください。
</Callout>
...
各 Bean は、コレクションから除外できる Bean の任意のリストである属性を含むことができる。各属性には、以下のキーが定義されています。

<Callout variant="important">
  マップ属性については、 `attr` または `attr_regex`
  のいずれかのキーを定義する必要があります。
</Callout>
...
- `metric_type`: この属性を収集するための New Relic のメトリックタイプ。オプションは
  ...
  - `rate`: データは 1 秒あたりのそのメトリックの変化として収集されます。
...
JMX クエリの詳細については、 [Oracle ObjectName](https://docs.oracle.com/javase/7/docs/api/javax/management/ObjectName.html) のドキュメントを参照してください。

### Kubernetes のアノテーションを利用したコレクションの設定 [#metrics-collection-kubernetes-annotations]

Kubernetes のアノテーションを使用して、コレクションの設定を提供することができます。そのためには、 `configMap` を Kubernetes クラスタにデプロイし、 `nri-jmx` アプリケーションの設定ファイルを作成する必要があります。

この設定ファイルでは、 [container auto-discovery](/docs/integrations/host-integrations/installation/container-auto-discovery) のコマンドを指定する必要があります。これにより、Kubernetes のアノテーションを含む統合設定でプレースホルダーを使用することができます。
...
次に、アノテーションを使ってコレクションの構成を定義します。例えば、アノテーションを使用した Tomcat のデプロイメントを示します。
...
デフォルトでは、サブフォルダ `connectors` がクラスパスにあります。このフォルダが存在しない場合は、nrjmx がインストールされているフォルダの下に作成してください。

たとえば、JBoss のサポートを追加するには、デフォルト（Linux）のライブラリパス `/usr/lib/nrjmx/` (`/usr/lib/nrjmx/connectors/`) の下に、 `connector` という名前のフォルダを作成し、カスタムコネクタの jar (`$JBOSS_HOME/bin/client/jboss-cli-client.jar`) をそこにコピーします。これで、JBoss に対して JMX クエリを実行できるようになります。
...
ここでは、収集されたすべての JVM ガベージコレクターをメタデータモニターで活用した、 [NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) クエリの例を示します。
...
JMX インテグレーションでは、以下のメトリックデータ属性を収集します。
...
{' '}
<Collapser id="missing-metrics" title="欠落したメトリクス">
  メトリクスが見つからない場合は、 [nrjmx
  tool](https://github.com/newrelic/nrjmx)
  でMBeanクエリの実行を試みて、MBeanクエリが有効であることを確認してください。また、JMXConsoleでクエリが有効であることを確認するには、お好みのツールを使用してください。
</Collapser>
...
>
  設定 `jvm-metrics.yml` ファイルが更新され、そのファイルのパスが
  `jmx-config.yml` ファイルに列挙されていることを確認します。
</Collapser>
