...
* [ガイド付きインストール](https://one.newrelic.com/nr1-core?account=3501165&state=ff641d8b-e722-855e-4c4a-c50c23969dc3)に従って、システムにインフラストラクチャ エージェントをインストルメント化します。
...
<CollapserGroup>
  ...
  >
    ...
    commands: 
        - run: - run: <var>YOUR_PATH_TO_DOWNLOADED_BINARY_FILE</var> <var> YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/config.yaml</var> <var>YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/queries/longest_queries.sql</var>
    ```
  </Collapser>
  ...
  >
    ...
    2. ファイル パスを`<var>YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY</var>/queries/credit_usage_by_warehouse.sql`として指定します。
    ...
    * 名前: CreditUsageByWarehouse エンティティ: スノーフレーク event_type: SnowflakeVirtualWarehouse custom_attributes: metric_type: snowflake.credit_usage_by_warehouseコマンド:

      * 実行:

        <var>YOUR_PATH_TO_DOWNLOADED_BINARY_FILE</var>

        <var>YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/config.yaml</var>

        <var>YOUR_PATH_TO_CLONED_REPOSITORY_DIRECTORY/credit_usage_by_warehouse.sql</var>
