...
New Relic では、Varnish Cache のデータは以下の Insights [イベントタイプ](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#event) に添付されています。
...
これらの属性は、Insightsで `VarnishLockSample` のイベントタイプを照会することで確認できます。
...
これらの属性は、Insightsで `VarnishMempoolSample` イベントタイプを照会することで見つけることができます。
