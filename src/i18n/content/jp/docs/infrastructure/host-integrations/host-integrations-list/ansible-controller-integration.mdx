...
インフラストラクチャ エージェントをインストールしてアプリに組み込むと、生データを**Metrics & events** \[メトリクスとイベント]で表示できます。デフォルトのダッシュボードは、生データをチャートやグラフに変換し、システムの健全性を鳥瞰的に表示します。デフォルトのダッシュボードをインストールするには、Ansible Automation Controller の[インスタント可観測性ページ](https://newrelic.com/instant-observability/ansible-automation-controller)にアクセスしてください ()。
