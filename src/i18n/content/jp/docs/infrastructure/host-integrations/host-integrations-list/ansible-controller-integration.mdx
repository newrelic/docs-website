...
<CollapserGroup>
  ...
  >
    Ansible Automation Platform にログインしたら、 <DoNotTranslate>**Users**</DoNotTranslate>セクションに移動し、 <DoNotTranslate>**admin**</DoNotTranslate>を選択して、ユーザーに<DoNotTranslate>**token**</DoNotTranslate>を追加します。 トークンをコピーして、希望のパスに保存します。
...
インフラストラクチャエージェントがインストールされ、アプリでインストゥルメントされると、 <DoNotTranslate>**Metrics & events**</DoNotTranslate>で生データを表示できます。 デフォルトのダッシュボードでは、生のデータをチャートやグラフに変換し、システムの健全性を俯瞰的に把握できます。 デフォルトのダッシュボードをインストールするには、Ansible Automation Controller の[インスタント オブザーバビリティ ページ](https://newrelic.com/instant-observability/ansible-automation-controller)にアクセスしてください。
