...
インフラストラクチャ エージェントと Prometheus OpenMetrics 統合で構築された Ansible Automation Platform 統合により、事前構築済みの一連の <InlinePopover type="dashboards"/>最も重要なクエリ データをすべて 1 か所で表示できます。
...
<figcaption>
  New Relic との統合を設定したら、データを <InlinePopover type="dashboards"/>これらのように、箱から出してすぐに。
</figcaption>
...
インフラストラクチャ エージェントをインストールし、アプリにインストルメント化すると、 **Metrics & events**で生データを表示できます。私たちのデフォルト <InlinePopover type="dashboards"/>その生データをチャートやグラフに変換して、システムの健全性を俯瞰できます。デフォルトのダッシュボードをインストールするには、Ansible Automation Controller の [インスタント オブザーバビリティ ページ](https://newrelic.com/instant-observability/ansible-automation-controller)にアクセスしてください)。
