...
<CollapserGroup>
  ...
  >
    ...
    次に、構成テンプレートを使用して、作成された「nri-prometheus-config.yml」という名前のファイルを更新します。
    ...
    5. `nri-prometheus-config.yml`ファイルは次のようになります。
  ...
  >
    ...
    Linux マシンでは、ログ ファイルは次の場所にあります。

    ```
    /etc/newrelic-infra/logging.d/
    ```

    上記のパスにログ ファイルが表示されない場合は、ログ ファイルを作成する必要があります。
