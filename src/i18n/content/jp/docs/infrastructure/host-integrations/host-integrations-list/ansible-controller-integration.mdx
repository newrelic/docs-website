...
インフラストラクチャ エージェントをインストールし、アプリにインストルメント化すると、 **メトリックとイベント**で生データを表示できます。デフォルトのダッシュボードは、その生データをチャートやグラフに変換し、システムの健全性を俯瞰します。デフォルトのダッシュボードをインストールするには、Ansible Automation Controller の [インスタント オブザーバビリティ ページ](https://newrelic.com/instant-observability/ansible-automation-controller)にアクセスしてください。
