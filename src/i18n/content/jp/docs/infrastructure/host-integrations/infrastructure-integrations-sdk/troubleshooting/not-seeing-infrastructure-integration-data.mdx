...
[Integrations SDK](/docs/integrations/integrations-sdk/getting-started/introduction-infrastructure-integrations-sdk) を使用してカスタム New Relic Infrastructure のオンホスト統合を作成しましたが、Infrastructure の UI にデータが表示されません。
...
<CollapserGroup>
  ...
  >
    ...
    **推奨：** [インフラストラクチャエージェントの設定で、ログファイル](/docs/infrastructure/new-relic-infrastructure/troubleshooting/generating-logs-troubleshooting-infrastructure) を構成します。これにより、エラーの種類を分けることができ、統合エラーをより簡単に発見することができます。
    ...
    統合のロードまたは実行にエラーが発生した場合、Infrastructure エージェントはエラー メッセージをログ ファイルに追加します。 `verbose` モードが無効になっていても、エラーはログに記録されます。
  ...
  >
    Infrastructureエージェントが統合を正しくロードしているかどうかを確認するために。
