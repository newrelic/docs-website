...
<CollapserGroup>
  ...
  >
    ...
    統合が構成ファイルで明示的に指定するのではなく、ホストの環境から構成を受け取ることを想定している場合は、インフラストラクチャ エージェントの [`passthrough_environment`](/docs/integrations/host-integrations/troubleshooting/pass-infrastructure-agent-parameters-host-integration) グローバル構成プロパティで必要な変数を設定する必要があります。
  </Collapser>
