...
<DoNotTranslate>**Recommendation:**</DoNotTranslate> Go を使用してインテグレーションを作成します。これは、オンホストインテグレーションと[インテグレーション構築ツール](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources)を作成するために使用する言語です。 ただし、インテグレーションはどの言語でも作成できます。
...
<CollapserGroup>
  ...
  >
    ...
    出力には、「きれいに印刷された」JSON ではなく、標準の JSON を使用します。 <DoNotTranslate>**Recommendation:**</DoNotTranslate>デバッグの目的で JSON を「きれいに印刷」するには、オプションのコマンドライン スイッチ (例: `--pretty` ) を含めます。
  </Collapser>
...
最小限のペイロードは、ヘッダー フィールドのみを含む JSON オブジェクトになります。 <DoNotTranslate>**Recommendation:**</DoNotTranslate>収集するデータがない場合は、 `stderr`に書き込まれたプログラムの戻りコードとログメッセージを使用します。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <DoNotTranslate>**Recommendation:**</DoNotTranslate> 逆ドメイン名を使用して一意のインテグレーション名を生成します。
      </td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        必須。 エンティティの種類。 これは、インフラストラクチャエージェントによって、 `name`と組み合わせて<DoNotTranslate>**unique identifier**</DoNotTranslate>を構成するためのネームスペースとして使用されます。
      </td>
    </tr>
...
<CollapserGroup>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> インテグレーションで使用されるイベント タイプが一意であることを保証するには、イベント タイプの前に会社名または頭字語を付けます。 たとえば、カスタム インテグレーションで Cassandra ノード メトリクスと Cassandra 列ファミリ メトリクスを異なるサンプルとしてキャプチャする場合は、それらを`MyOrgCassandraSample`や`MyOrgCassandraColumnFamilySample`などの異なるイベント タイプに保存します。
          ...
          <tr>
            ...
            <td>
              ...
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> New Relic UIでメトリクス間を移動するときに役立つように、メトリクスの前にカテゴリを付けます。 New Relicインテグレーションは現在以下を使用しています:
          ...
          <tr>
            ...
            <td>
              <DoNotTranslate>**Recommendation:**</DoNotTranslate> 元のメトリックス名に測定単位が含まれていない場合は、次の例のように単位接尾辞を使用して測定単位を指定します。
  ...
  >
    ...
    * 新規テーブルの作成
      ...
      このデータは、 [インフラストラクチャ<DoNotTranslate>**Events**</DoNotTranslate>ページ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-events-page-live-feed-every-config-change)と[インフラストラクチャ イベント ヒートマップ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range)で表示できます。 New Relic で`InfrastructureEvent`イベント タイプをクエリすることもできます。
  ...
  >
    ...
    * その他のシステムメタデータ
      ...
      このデータは、 [<DoNotTranslate>**Inventory**</DoNotTranslate>ページ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-inventory-page-search-your-entire-infrastructure)と[インフラストラクチャ イベント ヒートマップ](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/events-heatmap-examine-patterns-time-range)で表示できます。 在庫の変更に関連するデータをクエリすることもできます。
    ...
    * 値はスカラー型 (文字列または数値) <DoNotTranslate>**or**</DoNotTranslate>またはキー値の別のハッシュ オブジェクトのいずれかになります。 New Relic は階層をサポートしていますが、最終的な値ノードはスカラーである必要があります。
