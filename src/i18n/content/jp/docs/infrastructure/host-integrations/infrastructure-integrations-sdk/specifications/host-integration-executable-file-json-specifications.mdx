...
New Relic [インフラストラクチャ エージェントのオンホスト統合は、](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 実行可能ファイルと少なくとも 1 つの構成ファイルの少なくとも 2 つのファイルで構成されます。実行可能ファイルは、インフラストラクチャ監視エージェントによって消費され、New Relic に送信される JSON データを生成します。JSON オブジェクトを SDK 統合プロトコルと呼びます。
