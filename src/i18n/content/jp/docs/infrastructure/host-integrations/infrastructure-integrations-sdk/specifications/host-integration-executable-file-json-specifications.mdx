...
推奨事項: Go を使用して統合を作成します。これは、オンホスト インテグレーションと[インテグレーション構築ツール](/docs/integrations/integrations-sdk/getting-started/integrations-tutorials-build-resources)を作成するために使用する言語です。 ただし、どの言語でも統合を作成できます。
...
<CollapserGroup>
  ...
  >
    ...
    出力には、「きれいに印刷された」JSON ではなく、標準の JSON を使用します。 推奨事項: デバッグ目的で JSON を「きれいに出力」するには、オプションのコマンドライン スイッチ (例: `--pretty` ) を組み込みます。
  </Collapser>
...
最小限のペイロードは、ヘッダー フィールドのみを含む JSON オブジェクトになります。 推奨事項: 収集するデータがない場合は、プログラムの戻りコードと、 `stderr`に書き込まれたログメッセージを使用してください。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        推奨事項: 逆引きドメイン名を使用して、一意の統合名を生成します。
      </td>
    </tr>
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              推奨事項: インテグレーションで使用されるイベント タイプを確実に一意にするには、イベント タイプの前に会社名または頭字語を付けます。 たとえば、カスタムインテグレーションが Cassandra ノード メトリクスと Cassandra 列ファミリー メトリクスを異なるサンプルとしてキャプチャする場合、それらを異なるイベント タイプ ( `MyOrgCassandraSample`や`MyOrgCassandraColumnFamilySample`など) に保存します。
          ...
          <tr>
            ...
            <td>
              ...
              推奨事項: New Relic UIでメトリクス内を移動するときに役立つカテゴリをメトリクスの先頭に付けます。 New Relic統合は現在以下を使用しています:
          ...
          <tr>
            ...
            <td>
              推奨事項: 元のメトリックス名に測定単位が含まれていない場合は、次の例のように単位接尾辞を使用して測定単位を指定します。

              * パーセンテージ: `Percent`を使用します。例: `cpuUtilPercent` 。
              * 料金: `PerSecond`などの形式を使用します。秒は標準のレート測定ですが、 `PerMinute`や`PerDay`などの他の単位も使用できます。
              * バイト測定: `Bytes`を使用します。 推奨事項: メトリクスが別の単位 ( `Megabytes`など) でキャプチャされた場合は、それを`Bytes`に変換します。 例: `db.allMemtablesOffHeapSizeBytes` 。
              * 時間測定: `Milliseconds`を使用します。 推奨事項: メトリクスが別の単位 ( `Seconds`など) でキャプチャされている場合は、それを`Milliseconds`に変換します。 例えば： `query.readLatency50thPercentileMilliseconds`
            </td>
          </tr>
