...
<img title="new-relic-integrations-sdk-data-structure.png" alt="New Relic Integrations SDK data structure diagram" src={infrastructureSdkDataStructure} />
...
<CollapserGroup>
  <Collapser className="freq-link" id="data-output-spec" title="一般的な出力とJSONのフォーマット">
  ...
  <Collapser className="freq-link" id="errors-spec" title="実行ファイルの終了/クローズ">
...
2. 表示名。 [display\_name エージェント設定オプションで設定します。](/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#general)
3. エージェントが取得したホスト名

たとえば、プロトコル v3 を使用した統合が`localhost:3306`という名前のエンティティを返し、エージェントがベアメタル (クラウド プロバイダー インスタンス ID を持たない) で実行されている場合、display\_name は設定されておらず、ホスト名は`prod-mysql-01`の場合、エージェントは`localhost`を置き換えてエンティティ名`prod-mysql-01:3306`を生成します。
...
<CollapserGroup>
  <Collapser className="freq-link" id="metric-data" title="メトリックデータ">
  ...
  <Collapser className="freq-link" id="event-data" title="イベントデータ">
  ...
  <Collapser className="freq-link" id="inventory" title="在庫データ">
