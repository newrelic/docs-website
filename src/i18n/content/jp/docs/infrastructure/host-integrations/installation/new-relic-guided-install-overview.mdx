...
始める準備はできていますか? **Guided install** \[ガイド付きインストール] ボタンをクリックします。または、組織が [EU データセンター](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)にある場合は、 **EU Guided install** \[EU ガイド付きインストール]をクリックします。
...
当社のオープンソース エージェントと統合は、パフォーマンス メトリクスとインベントリ データをサーバーとアプリケーションから New Relic プラットフォームに送信します。事前に構築されたものを表示できます <InlinePopover type="dashboards"/>メトリック データを分析し、アラート ポリシーを作成し、独自のカスタム クエリとグラフを作成します。
