...
<CollapserGroup>
  ...
  >
    ...
    3. 次のコマンドを実行します。ここで、 `INTEGRATION_FILE_NAME`統合のファイル名を表します。詳細については、[統合のドキュメントを](/docs/integrations/host-integrations/host-integrations-list)参照してください。
  ...
  >
    ...
    3. 次のコマンドを実行します。ここで、 `INTEGRATION_FILE_NAME`統合のファイル名を表します。詳細については、 [「特定のオンホスト統合」](/docs/infrastructure/host-integrations/host-integrations-list)を参照してください。
  ...
  >
    ...
    5. 統合に関するドキュメントに戻り、構成手順を完了します。統合で JMX ツールの使用が必要な場合、その詳細については以下で説明します。
