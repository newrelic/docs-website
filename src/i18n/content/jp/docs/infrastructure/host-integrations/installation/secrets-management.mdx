...
シークレット管理を使用すると、エージェントとオンホストインテグレーションを設定し、機密データ（パスワードなど）を設定ファイルにプレーンテキストとして書き込むことなく使用できるようになります。現在、Hashicorp Vault、AWS KMS、CyberArk、Azure Key Vault、New Relic CLIの難読化、およびカスタムコマンドがサポートされています。
...
オンホストインテグレーション設定ファイルで環境変数を使用する場合は、 [`passthrough_environment`](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings#passthrough_-environment)設定を定義する必要があります。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Azure Key Vaultのシークレット取得設定](#azure-key-vault-secrets)
      </td>
    </tr>
...
## Azure Key Vaultシークレット [#azure-key-vault-secrets]

Azure Key Vaultからシークレットを取得するには、`azure-key-vault`セクションで次のプロパティを設定します。両方のキーが必要です：
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        シークレットを取得するAzure Key VaultインスタンスのURL

        例： `https://my-vault.vault.azure.net/`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        取得するシークレットの名前。エージェントは常に最新バージョンのシークレットを取得します。
      </td>
    </tr>
  </tbody>
</table>

エージェントは、Azure SDKのデフォルトの資格情報チェーンを使用してAzure Key Vaultで認証を行い、成功するまで次のメソッドを順番に試行します。追加のエージェント設定は必要ありません：

* 環境変数`AZURE_CLIENT_ID`、`AZURE_CLIENT_SECRET`、および `AZURE_TENANT_ID`
* エージェントを実行しているAzure VMまたはAKSポッドに割り当てられたマネージドID（本番環境で推奨）
* 開発およびテストに役立つ、Azure CLI（`az login`）からの認証情報

<Callout variant="tip">
  シークレット値のみがサポートされています。Azure Key Vaultの証明書またはキーの取得はサポートされていません。
</Callout>

次の例では、Azure Key Vaultから`mssql-password`という名前のシークレットを取得し、それをMSSQLオンホストインテグレーションのパスワードとして使用します：
