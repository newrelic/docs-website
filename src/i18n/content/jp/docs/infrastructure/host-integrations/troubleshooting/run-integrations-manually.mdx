---
title: インフラストラクチャ エージェント統合を手動で実行する (ドライラン)
...
[インフラストラクチャ エージェント統合が](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) 期待どおりに構成され、メトリクスを収集しているかどうかを知りたいと考えています。診断するには、インフラストラクチャ エージェントでドライラン フラグを使用するか、統合を手動で実行します。
