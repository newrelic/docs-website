...
## 特定のエージェントバージョンをインストールする [#install-specific-versions]
...
利用可能なインフラストラクチャエージェントのバージョンとそのリリースの詳細については、[インフラストラクチャエージェントのリリース ノート](/docs/release-notes/infrastructure-release-notes/infrastructure-agent-release-notes/)を参照してください。
