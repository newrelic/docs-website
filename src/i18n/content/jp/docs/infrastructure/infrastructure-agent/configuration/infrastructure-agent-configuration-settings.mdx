...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="logging-http-client-timeout" title="logging_http_client_timeout">
    New Relic fluent-bit出力プラグインのHTTPクライアントタイムアウト（秒単位）を設定します。この設定により、ログデータをNew Relicに送信するときに、ログフォワーダーが応答を待機する時間を制御します。

    詳細については、 [fluent-bit出力プラグインのドキュメント](https://github.com/newrelic/newrelic-fluent-bit-output#httpclienttimeout)を参照してください。
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="safe-bin-dir" title="safe_bin_dir">
    エージェントがインテグレーションの実行可能ファイルを検索するディレクトリ。
  ...
  <Collapser className="freq-link" id="agent-temp-dir" title="agent_temp_dir">
    エージェントが一時ファイル（ログフォワーダーの設定やインテグレーションの検出データなど）を保存するディレクトリ。このディレクトリは、デフォルト値のままである場合に限り、エージェントを再起動するたびに削除されます。
  ...
  <Collapser className="freq-link" id="logging-home-dir" title="logging_home_dir">
    ログフォワーダーのバイナリやその他の必要なファイルが格納されているディレクトリ。
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="http-server-cert" title="http_server_cert">
    HTTPS経由でインテグレーションペイロードをリッスンするために使用されるPEMエンコードされた証明書へのパス。`http_server_key`で構成されている場合、エージェントはインテグレーションペイロードサーバーにHTTPSを使用します。
  ...
  <Collapser className="freq-link" id="http-server-key" title="http_server_key">
    HTTPS経由でインテグレーションペイロードをリッスンするために使用されるPEMエンコードされたキーへのパス。`http_server_cert`で構成されている場合、エージェントはインテグレーションペイロードサーバーにHTTPSを使用します。
  ...
  <Collapser className="freq-link" id="http-server-ca" title="http_server_ca">
    HTTPS requestsのクライアント証明書の検証を強制するために使用されるPEMエンコードされたCA証明書へのパス。設定すると、エージェントはこのCAに照らしてクライアント証明書を検証し、インテグレーションペイロードサーバーの相互TLS（mTLS）認証を有効にします。
