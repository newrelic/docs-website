...
<CollapserGroup>
  <Collapser id="linux-root" title="ルートとして実行">
    エージェントがルートユーザーとして実行されると、すべてのシステムメトリクスとインベントリにアクセスできるようになります。
  </Collapser>

  <Collapser id="linux-privileged" title="特権ユーザーとして実行">
  ...
  <Collapser id="linux-unprivileged" title="非特権ユーザーとして実行">
...
<CollapserGroup>
  <Collapser id="directories" title="エージェントがアクセスするデフォルトのディレクトリ">
  ...
  <Collapser id="packages-commands" title="エージェントが使用するパッケージとコマンド">
