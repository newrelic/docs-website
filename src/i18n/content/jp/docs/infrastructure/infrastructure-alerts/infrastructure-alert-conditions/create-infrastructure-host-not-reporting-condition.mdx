...
インフラストラクチャ監視の**Host not reporting** \[ホストが報告しない][条件](/docs/infrastructure/new-relic-infrastructure/configuration/infrastructure-alerts-add-edit-or-view-host-alert-information)を使用して、インフラストラクチャ エージェントからのデータの受信が停止したときに通知します。 この機能を使用すると、ホストのグループに対して動的にアラートを発し、時間枠を 5 ～ 60 分に設定して、 <InlinePopover type="alerts"/>通知。
