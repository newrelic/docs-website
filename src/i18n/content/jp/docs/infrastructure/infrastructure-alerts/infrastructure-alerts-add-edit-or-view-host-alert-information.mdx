...
ポリシーの[通知チャネル](#notification)に含まれている人は誰でも、まとめ通知を直接受け取ります。 さらに、 New Relicアカウントの権限を持つ人は誰でも、ユーザー インターフェースを通じてインフラストラクチャ集中イベントおよび個別の大量イベントを表示できます。
...
3. <DNT>**Events**</DNT>リストから、集中イベントを選択します。
4. 選択したまとめイベントに関する[アラートの詳細情報を表示する](/docs/alerts/alert-event-management/view-alert-event-details)には、リンクを選択します。
...
[Alerts は](/docs/alerts/new-relic-alerts-beta/getting-started/new-relic-alerts)、すべての New Relic 製品にわたって単一の調整されたアラート ツールを提供します。これにより、 コンテナや JVM[ など、最も関心のあるエンティティの](/docs/alerts/overview#alert-entity) メトリクス[ に焦点を当てた](/docs/alerts/overview#alert-policy)[ ](/docs/alerts/overview#alert-condition)アラート ポリシー と 条件を 管理できます。Docker
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * [まとめイベント](/docs/new-relic-solutions/get-started/glossary#alert-event)に関する詳細情報を表示する:[アラート](/docs/alerts/alert-event-management/view-alert-event-details)UIまたは関連ポリシーに統合された[通知チャネル](#notification)を使用します。
      </td>
    </tr>
...
アラート条件の<DNT>**Description**</DNT>フィールドに入力したテキストは、関連する大量イベントと通知にダウンストリームで渡されます。 説明は、次のようないくつかの目的で使用できます。
...
テンプレート置換を使用して、関連するイベント イベントのプロパティから値を挿入できます。 テンプレートの形式は`{{attributeName}}`です。説明の作成時に使用できるプロパティについては、 [「まとめイベントのプロパティ」](/docs/alerts/create-alert/condition-details/alert-event-attributes)を参照してください。

使用可能な属性の1つは、特別な`{{tag.*}}`属性です。この属性プレフィックスは、ターゲット信号に含まれているタグ値、またはターゲット信号に関連付けられているエンティティタグにアクセスするために使用されます。

まとめイベントに関連付けられたエンティティ タグがある場合は、エンティティ タグ名を使用してアクセスできます。 例としては`{{tag.aws.awsRegion}}`が挙げられます。エンティティ タグが使用できる場合は、「大量イベント」に含まれており、「大量イベント」を表示すると表示されます。

このフィールドの最大文字サイズは4,000です。

## RunbookのURLを追加または編集する [#runbook-url-infrastructure]

[アラート条件作成プロセスに](#create-condition)は、ランブック手順の URL を設定するオプションが含まれています。これにより、大量イベントを処理するための情報または標準手順にリンクできます。 リンクを追加または更新する前に、有効な URL を使用していることを確認してください。
...
## まとめイベント制限時間 [#alert-event-time-limit]

集中イベントの時間制限を使用すると、集中イベントを強制的に終了するまでの期間を定義できます。 デフォルトでは、インフラストラクチャ状況の場合、集中イベントの制限時間は 24 時間です。

アラート条件のまとめイベントの制限時間を追加または更新するには:

1. アラート条件を選択し、イベントの制限時間を変更します。
2. 条件を保存します。
...
詳細については、New Relicのサポートフォーラムの[「インフラストラクチャアラートのフィルタリングのベストプラクティス」](https://support.newrelic.com/s/hubtopic/aAX8W0000008Zfu/relic-solution-best-practice-for-filtering-in-infrastructure-alerts)をご覧ください。