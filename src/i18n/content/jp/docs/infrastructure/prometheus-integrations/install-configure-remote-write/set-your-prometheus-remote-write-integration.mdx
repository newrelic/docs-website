...
[Prometheusオペレーター](https://github.com/prometheus-operator/prometheus-operator)を使用している場合は、データをレポートするアカウントのNew Relic<InlinePopover type="licenseKey" />でSecretを作成する必要があります。APIキーがタイプ`Ingest - License`であることを確認します。
...
<CollapserGroup>
  <Collapser
    ...
    title="X-ライセンスキー"
  >
    <InlinePopover type="licenseKey" />は、APIキーではありません。ライセンスキーは、認証とどのアカウントにデータを書き込むかの特定に使用します。さまざまなNew Relicアカウントに書き込むPrometheusを設定している場合は、それぞれのリモート書込みURLで異なるキーを使用します。
  </Collapser>
