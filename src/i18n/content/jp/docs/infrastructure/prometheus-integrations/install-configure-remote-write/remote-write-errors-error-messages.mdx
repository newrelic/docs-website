...
<CollapserGroup>
  <Collapser
    id=""
    title="コンフィギュレーションエラー"
  >
    構成ファイルのリモート書き込み URL に文字がないか正しくない (たとえば、エンドポイント、 <InlinePopover type="licenseKey"/>、または`prometheus_server`名)、またはファイル内の情報の配置が正しくない場合、Prometheus サーバーが起動しない、リモート書き込みが正しく機能しない、または Prometheus サーバー ログにエラーが表示されるという結果になります。
  </Collapser>

  <Collapser
    id=""
    title="400: バッドリクエストエラー"
  >
