...
<CollapserGroup>
  ...
  <Collapser title="429: レートリミットエラー">
    これは、一度に送信されるデータ量（カーディナリティなど）の[レート制限](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes/#rate-limit-incidents)に達したことを意味します。送信するPrometheusや一般的なメトリクスデータの量を減らすか、レート制限の引き上げを申請することで、トラブルシューティングできます。
  </Collapser>
</CollapserGroup>
