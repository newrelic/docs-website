...
特に明記されていない限り、Prometheus OpenMetrics インテグレーションとNew Relicの設定オプションは、 docker環境とKubernetes環境の両方に適用されます。 少なくとも、次の設定値は<DoNotTranslate>**required**</DoNotTranslate>です。

* <InlinePopover type="licenseKey"/>
* [クラスター名](#definitions-configuration-file)

<DoNotTranslate>**Recommendation:**</DoNotTranslate> New Relicキーを `LICENSE_KEY` という名前の環境変数として設定します。 これにより、New Relic は[相互 TLS 認証シークレット](/docs/integrations/prometheus-integrations/install-configure/add-mutual-tls-prometheus-endpoints)から環境変数を読み込むことができるため、より安全な環境が提供されます。
...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}/>Kubernetesのポートとエンドポイントのパス</>}
  >
  ...
  <Collapser
    ...
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" style={{ width: '30px', height: '25px' }}/>例。Kubernetesのポートとパスのラベル</>}
  >
...
Prometheus OpenMetrics インテグレーション<DoNotTranslate>**does not**</DoNotTranslate>は、設定ファイルに変更を加えると、設定を自動的に再読み込みします。
...
設定を再読み込みするには、インテグレーションを再起動してください。 <DoNotTranslate>**Recommendation:**</DoNotTranslate>デプロイメントをレプリカ 0 個にスケールダウンし、その後レプリカ 1 個に戻します。
...
以前の設定ファイルを使用してインテグレーションを実行するには:
