...
Prometheus `discovery`および`scrape`アノテーションを使用します。 利用可能なターゲットをスクレイピングし、それらのターゲットから公開されるすべてのデータを送信するように Prometheus OpenMetrics インテグレーションを設定すると、<DoNotTranslate>**all**</DoNotTranslate>New Relic の制限を超え、請求料金が増加する可能性があります。このような事態を防ぐには、インテグレーションのフィルタリング機能を使用してください。
