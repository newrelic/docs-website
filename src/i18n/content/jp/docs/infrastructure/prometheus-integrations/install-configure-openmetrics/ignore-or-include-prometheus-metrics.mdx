...
<CollapserGroup>
  ...
  >
    ...
    **出力例**
    ...
    フィルタリングが適用された後、 `go_` と `process_` のメトリクスはドロップされ、 `mysql_` のメトリクスは New Relic に出荷されます。
  ...
  >
    `プレフィックス` と `の両方を組み合わせて、 ignore_metrics セクションの下の` を除いて、より複雑なフィルタリングを行うことができます。

    **構成例**

    `coredns_` メトリクスのうち、 `coredns_dns_request_count_total` と `coredns_dns_responses_total` を除くすべてのメトリクスを削除する。
