...
演算子は、`prometheus`ラベル（クラスタを識別するラベル）の値を`<prometheus deployment namespace>/<prometheus deployment name>`に設定します。たとえば、Prometheusデプロイメントのネームスペースが`monitoring`で、デプロイメントの名前が`prometheus-cluster1`の場合、値は`monitoring/prometheus-cluster1`となります。
