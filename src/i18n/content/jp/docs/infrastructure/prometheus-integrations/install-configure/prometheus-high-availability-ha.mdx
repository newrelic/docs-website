...
<Callout variant="caution">
  1つのアカウントで最大1,500個の一意のPrometheus HAクラスタを持つことができます。この制限を超えると、追加のHAクラスタからのデータは破棄されます。そのような場合、New Relicは`PrometheusHAClusterLimit`の[`NrIntegrationError`](/docs/data-apis/ingest-apis/metric-api/troubleshoot-nrintegrationerror-events/)イベントを生成します。
</Callout>
