...
<CollapserGroup>
  ...
  <Collapser title="ユーティリティ関数">
    * [`label_join()`](https://prometheus.io/docs/prometheus/latest/querying/functions/#label_join)
  </Collapser>

  <Collapser title="Grafana固有の機能">
    翻訳ロジックは、PromQL言語には含まれないGrafanaの構文も一部サポートしています。
...
ただし、NRDBはPromQLスタイルの累積カウンターを`delta`カウンターに変換するため、New Relicの実装では、こうした関数を本来とは異なるデータ型に対して正しく処理できず、異なる結果や誤った結果が生じます。
