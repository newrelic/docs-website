...
<img style={{ width: '40px', height: '35px'}} title="img-integration-k8.png" alt="img-integration-k8.png" src={imgIntegrationK8S402X}/> **Kubernetes:** スクレーパーがKubernetesで動作している場合、New Relicはすべてのメトリクスに以下の属性も追加します。
...
<CollapserGroup>
  ...
  >
    <img style={{ width: '40px', height: '35px'}} title="Docker アイコン" alt="Docker アイコン" src={dockerLogoCrop}/> **Docker:** この例では、Redis エクスポーターをスクレイピングすることを想定しています。クラスター内のエンドポイントごとの接続されたRedisクライアントの数を表示するには、次のようにします。

    ```
    FROM Metric SELECT latest(redis_connected_clients) WHERE clusterName='my-cluster' FACET scrapedEndpoint TIMESERIES
    ```

    <img style={{ width: '40px', height: '35px'}} title="Kubernetes アイコン" alt="Kubernetes アイコン" src={imgIntegrationK8S402X}/> **Kubernetes:** この例では、Redis エクスポーターがインストールされた Redis ポッドがあることを想定しています。デフォルトの名前空間でポッドごとに接続されたRedisクライアントの数を表示するには、次のようにします。
