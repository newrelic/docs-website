...
Prometheus OpenMetrics や New Relic とのリモートライト統合で収集したメトリクスをクエリして可視化するには、 [NRQL](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-components-functions) を使用することができます。また、 [PromQL スタイルのクエリを NRQL](/docs/integrations/prometheus-integrations/view-query-data/translate-promql-queries-nrql) に変換するには、Grafana または [クエリビルダ](/docs/query-your-data/explore-query-data/query-builder/use-advanced-promql-style-mode-query-data) を使用します。

Docker と Kubernetes のすべてのメトリクスは、 `Metric` タイプに保存されます。

## OpenMetrics インテグレーションのデフォルト属性 [#default-attributes]

デフォルトでは、Docker と Kubernetes の統合のためのすべてのメトリクスに以下の属性が追加されます。
...
<ImageSizing width="40px" height="35px">
  ![img-integration-k8.png](src/images/img-integration-k8.png
  "img-integration-k8.png"){' '}
</ImageSizing>
**Kubernetes:** スクレーパーがKubernetesで動作している場合、New Relicはすべてのメトリクスに以下の属性も追加します。
...
デフォルトでは、以下の属性が Prometheus のリモートライトメトリクスに追加されます。
...
PromQL スタイルのクエリを構築するには、\[サポートされている PromQL の機能]](/docs/integrations/prometheus-integrations/view-query-data/supported-promql-features)に関するドキュメントを参照してください。
...
>
  ...
  >
    <ImageSizing width="40px" height="35px">![Docker アイコン](src/images/docker-logo-crop.png "Docker アイコン") </ImageSizing>**Docker:** この例では、Redis エクスポーターをスクレイピングすることを想定しています。クラスター内のエンドポイントごとの接続されたRedisクライアントの数を表示するには、次のようにします。

    ```
    FROM Metric SELECT latest(redis_connected_clients) WHERE clusterName='my-cluster' FACET scrapedEndpoint TIMESERIES
    ```

    <ImageSizing width="40px" height="35px">![Kubernetes アイコン](src/images/img-integration-k8.png "Kubernetes アイコン") </ImageSizing>**Kubernetes:** この例では、Redis エクスポーターがインストールされた Redis ポッドがあることを想定しています。デフォルトの名前空間でポッドごとに接続されたRedisクライアントの数を表示するには、次のようにします。
...
NRQL には、リモートライトや Prometheus OpenMetrics Integration（バージョン [2.0.0](https://github.com/newrelic/nri-prometheus/blob/main/CHANGELOG.md#200) 以降）によって取り込まれた Prometheus のヒストグラムで動作する 2 つの関数があります。 [`bucketPercentile()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions#func-bucket-percentile) 、 [`histogram()`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions#func-histogram) 。これらのリンクには、クエリの例が含まれています。
