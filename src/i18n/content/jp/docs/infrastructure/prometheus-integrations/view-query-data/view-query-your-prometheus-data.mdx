...
<ImageSizing width="40px" height="35px"><img src={imgIntegrationK8} title="img-integration-k8.png" alt="img-integration-k8.png"/> </ImageSizing>**Kubernetes:** スクレーパーがKubernetesで動作している場合、New Relicはすべてのメトリクスに以下の属性も追加します。
...
<CollapserGroup>
  ...
  >
    <ImageSizing width="40px" height="35px"><img src={dockerLogoCrop} title="Docker アイコン" alt="Docker アイコン"/> </ImageSizing>**Docker:** この例では、Redis エクスポーターをスクレイピングすることを想定しています。クラスター内のエンドポイントごとの接続されたRedisクライアントの数を表示するには、次のようにします。

    ```
    FROM Metric SELECT latest(redis_connected_clients) WHERE clusterName='my-cluster' FACET scrapedEndpoint TIMESERIES
    ```

    <ImageSizing width="40px" height="35px"><img src={imgIntegrationK8} title="Kubernetes アイコン" alt="Kubernetes アイコン"/> </ImageSizing>**Kubernetes:** この例では、Redis エクスポーターがインストールされた Redis ポッドがあることを想定しています。デフォルトの名前空間でポッドごとに接続されたRedisクライアントの数を表示するには、次のようにします。
