...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      <DNT>**PromQL-style example:**</DNT> 데이터를 필터링하려면 다음 PromQL 스타일 쿼리를 실행하세요: container_memory_usage_bytes&#x7B;id="/".
    </figcaption>
  </Collapser>
  ...
  >
    ...
    <figcaption>
      <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DNT>: この例は、実行後に表示されるデータを示しています FROM Metric SELECT keyset() WHERE metricName = 'container_memory_usage_bytes'。
    </figcaption>
    ...
    <figcaption>
      <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**</DNT>: この例は、実行後に表示されるデータを示しています FROM Metric SELECT latest(container_memory_usage_bytes) FACET id。
    </figcaption>
