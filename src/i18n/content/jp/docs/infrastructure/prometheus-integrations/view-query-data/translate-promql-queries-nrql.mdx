...
<Callout variant="tip">
  [New Relic One](/docs/new-relic-one/use-new-relic-one/get-started/introduction-new-relic-one)で PromQL スタイルの書き込みを実行するには、 [書き込みビルダーの高度な PromQL スタイル モード](/docs/infrastructure/prometheus-integrations/view-query-data/view-query-your-prometheus-data/#build-query)に移動します。
</Callout>
