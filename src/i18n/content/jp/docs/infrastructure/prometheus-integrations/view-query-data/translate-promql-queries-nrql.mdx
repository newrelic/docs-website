...
<CollapserGroup>
  ...
  >
    ...
    **3.指標を集計し、グラフ化する。**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > データのクエリ**: この例は、FROM Metric SELECT keyset() WHERE metricName = 'container_memory_usage_bytes' を実行した後に表示されるデータを示しています。
    </figcaption>
    ...
    **4.ID別にメトリクスを表示**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > データのクエリ**: この例は、 FROM Metric SELECT latest(container_memory_usage_bytes) FACET id を実行した後に表示されるデータを示しています。
    </figcaption>
    ...
    **5.選択したIDをクエリに追加します。**
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Query your data** : この例は、 `From Metric select latest(container_memory_usage_bytes) where id = "/" timeseries`を実行した後に表示されるデータを示しています。
    </figcaption>
  </Collapser>
</CollapserGroup>
