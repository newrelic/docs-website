...
<CollapserGroup>
  ...
  >
    ...
    **2.クエリの結果をフィルタリングします。**
    ...
    データを見て、さらにクエリ パラメーターを追加して、指標の時系列の数を絞り込むことができます。たとえば、 `id`が`/`である時系列のみが必要な場合、PromQL スタイルのクエリは次のようになります。
  ...
  >
    ...
    **3.指標を集計し、グラフ化する。**
    ...
    <figcaption>
      **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**: この例は、 FROM Metric SELECT keyset() WHERE metricName = 'container_memory_usage_bytes' を実行した後に表示されるデータを示しています。
    </figcaption>

    **4.ID別にメトリクスを表示**

    さまざまな`id`値を持つ一意の指標の時系列を表示するには、次のクエリを実行します。
    ...
    <figcaption>
      **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**: この例は、FROM Metric SELECT latest(container_memory_usage_bytes) FACET id を実行した後に表示されるデータを示しています。
    </figcaption>
    ...
    **5.選択したIDをクエリに追加します。**
    ...
    <figcaption>
      **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**: この例は、 `From Metric select latest(container_memory_usage_bytes) where id = "/" timeseries`を実行した後に表示されるデータを示しています。
    </figcaption>
  </Collapser>
</CollapserGroup>
