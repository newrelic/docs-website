...
import infrastructureByteRateEstimate from 'images/infrastructure_screenshot-crop_byte-rate-estimate.png'

import infrastructureSentvsCompressedChart from 'images/infrastructure_screenshot-crop_sent-vs-compressed-chart.png'

import infrastructureUncompressedvsCompressedChart from 'images/infrastructure_screenshot-crop_uncompressed-vs-compressed-chart.png'

import infrastructureCountEstimatevsUncompressedChart from 'images/infrastructure_screenshot-crop_count-estimate-vs-uncompressed-chart.png'
...
最後に、データが変換されてエンリッチメントが実行されると、リモート書き込みの非圧縮バイトと`bytescountestimate()`の違いが以下に表示されます。リストされている`bytecountestimate()`は、格納される前のデータの最終状態のバイト カウントの尺度です。
