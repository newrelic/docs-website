...
Prometheus OpenMetrics と New Relic for Docker や Kubernetes との統合のためにスクレーパーメトリクスを取得したい。

![Docker アイコン](src/images/docker-logo-crop.png 'Docker アイコン') **Docker:** `http://localhost:8080/metrics` エンドポイントを設定した場合、統合は自身のメトリクスもスクレイピングします。

![img-integration-k8s@2x.png](src/images/img-integration-k8.png 'img-integration-k8s@2x.png') **Kubernetes:** デフォルトでは、スクレーパーは自身のメトリクスもスクレイピングします。

## 解決策

スクレーパーの内部で何が起こっているかを知るために、New Relic との Docker または Kubernetes の統合に以下のメトリクスを使用します。
