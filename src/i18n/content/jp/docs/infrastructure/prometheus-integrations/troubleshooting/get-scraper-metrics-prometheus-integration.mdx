...
<img title="Docker アイコン" alt="Docker アイコン" src={dockerLogoCrop}/> **Docker:** `http://localhost:8080/metrics` エンドポイントを設定した場合、統合は自身のメトリクスもスクレイピングします。

<img title="img-integration-k8s@2x.png" alt="img-integration-k8s@2x.png" src={imgIntegrationK8}/> **Kubernetes:** デフォルトでは、スクレーパーは自身のメトリクスもスクレイピングします。
