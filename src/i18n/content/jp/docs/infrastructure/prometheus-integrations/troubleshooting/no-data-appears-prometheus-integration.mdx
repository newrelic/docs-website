...
<CollapserGroup>
  <Collapser
    id="docker"
    title={<><img src={osDocker} title="Docker icon" alt="Docker icon"/>Dockerのトラブルシューティング</>}
  >
    連携に問題がある場合
  ...
  <Collapser
    id="kubernetes"
    title={<><img src={osk8} title="img-integration-k8s@2x.png" alt="kubernetes icon"/>Kubernetesのトラブルシューティング</>}
  >
    連携に問題がある場合
