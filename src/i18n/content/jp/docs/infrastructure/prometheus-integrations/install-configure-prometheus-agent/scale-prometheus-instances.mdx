...
ただし、このアプローチは大規模なクラスターにはスケーラブルではない可能性があります。または、単一のポッドがノードで非常に多くの GB のメモリを消費することを望まないだけです。その場合は、水平方向のスケーリングを使用する必要がある場合があります。
...
各ターゲットのラベル変更ルールを設定するために、エージェントは指定されたターゲット`__address__`のハッシュを計算し、 `modulus`をハッシュに適用します。これは、モジュラスがシャードの総数です。次に、スクラップされたターゲットを含める必要があるシャードを認識します。
