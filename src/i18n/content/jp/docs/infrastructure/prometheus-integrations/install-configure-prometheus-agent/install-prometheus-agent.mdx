...
1. Kubernetes 統合をインストールします。インストール方法は[こちらを](/install/kubernetes)ご覧ください。
...
2. 統合が正しく構成されていることを確認するには、<DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Query your data**</DoNotTranslate> に移動し、次のNRQL書き込みを実行して、データが報告されているかどうかを確認します。
