...
メトリクスは、Prometheus OpenMetrics 統合`nri-prometheus. In some cases you may need to modify some metadata that has changed between Prometheus OpenMetrics integration, `nri-prometheus `, and the Prometheus agent, `newrelic-prometheus-agent\` を使用して送信されたのと同じ方法で送信されます。

<Callout variant="tip">
  Prometheus エージェントによって追加されたラベルの完全なリストについては、 [こちらを](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/default-metrics-and-metric-attributes#metric-labels)ご覧ください。
</Callout>
...
Kubernetes での Prometheus OpenMetrics 統合は NR Prometheus Agent に置き換えられましたが、一定期間 (2023 年 6 月) はサポートされます。New Relic は、お客様にスムーズでシームレスな移行を保証したいと考えています。

Prometheus OpenMetrics 統合`nri-prometheus` 、Prometheus エージェント`newrelic-prometheus-agent` 、またはその両方を保持できます。nri-prometheus を保持し、現在 Prometheus エージェントを使用しないようにするには、 [`values.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nri-bundle/values.yaml)ファイルを次のように設定します:
