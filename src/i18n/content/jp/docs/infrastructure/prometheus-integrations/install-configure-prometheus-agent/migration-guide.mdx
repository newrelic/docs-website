...
* 名前接尾辞`total` 、 `count` 、 `sum` 、または`bucket`のない Prometheus Counter メトリクスは、 <DNT>**Gauge**</DNT>として変換されます。
* 名前接尾辞`total` 、 `count` 、 `sum` 、または`bucket`を持つ Prometheus ゲージ メトリクスは、 <DNT>**Counter**</DNT>として変換されます。
