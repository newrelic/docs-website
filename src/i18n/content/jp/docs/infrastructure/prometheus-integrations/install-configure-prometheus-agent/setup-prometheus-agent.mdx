...
Prometheus エージェントは、Kubernetes で最も一般的な統合からメトリクスを収集するように構成できます。 一連のダッシュボードを含む[統合のリスト](/docs/infrastructure/prometheus-integrations/integrations-list/integrations-list-intro)を確認してください。 <InlinePopover type="alerts"/>すぐに監視を開始できます。
...
* ガイド付きインストールを使用した場合は、 **Scrape only metrics from Prometheus integrations** \[Prometheus 統合からメトリクスのみを収集] オプションの選択を解除します。<img title="Kubernetes Guided Install" alt="Kubernetes Guided Install" src={infrastructurePrometheusScrapeOnlyMetrics}/>
