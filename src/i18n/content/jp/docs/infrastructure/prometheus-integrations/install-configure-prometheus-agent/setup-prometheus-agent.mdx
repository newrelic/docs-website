...
* ガイド付きインストールを使用した場合は、 **Scrape only metrics from Prometheus integrations**\[Prometheus 統合からのみメトリックをスクレイプする] オプションの選択を解除します。<img title="Kubernetes Guided Install" alt="Kubernetes Guided Install" src={infrastructurePrometheusScrapeOnlyMetrics}/>
