...
* ガイド付きインストールを使用した場合は、 **Scrape all Prometheus endpoints** \[すべての Prometheus エンドポイントをスクレイピング]オプションを選択します。<img title="Kubernetes Guided Install" alt="Kubernetes Guided Install" src={infrastructurePrometheusScrapeOnlyMetrics}/>
