...
* [サービスディスカバリー](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent.mdx#k8s-target-discovery)を活用して、デプロイされた新しいサービスを自動的に監視します。
...
* さまざまな[スクレイピング間隔](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent#target_scrape_interval)、[メトリクスフィルター](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent#etup-prometheus-agent.mdx#drop-keep-metrics)、[ラベル管理](/docs/infrastructure/prometheus-integrations/install-configure-prometheus-agent/setup-prometheus-agent.mdx#etup-prometheus-agent.mdx#k8s-target-discovery)をサポートすることで、データの取り込みを完全に制御します。
...
New RelicのDocker用のPrometheus OpenMetricsインテグレーションを使用して、[Prometheus](https://prometheus.io/)エンドポイントを取得し、データをNew Relicに送信し、重要なメトリクスを1つのプラットフォームに保存して可視化できます。
...
* [Kubernetes用のPrometheusエージェント](/docs/integrations/prometheus-integrations/install-configure-prometheus-agent/install-prometheus-agent)を完了するための方法をお読みください。
