...
1. <DNT>
   ...
   フィールドで ECS クラスターを検索するか、ECS クラスターの名前を入力します。
...
<Callout variant="important">
  Cgroup バージョン V2 の EC2 ノードでは、`nri-docker` インテグレーションはdocker APIからすべてのコンテナ メトリックを収集します。 収集されたメトリクスのバリエーションのリストについては、 [`ContainerSample`](/attribute-dictionary/?event=ContainerSample)属性を参照してください。
</Callout>
