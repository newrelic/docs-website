...
4. <DNT>**Redirect URI**</DNT>
   ...
   で`Web`を選択し、サインオン URI として`https://www.newrelic.com`を追加します。

5. <DNT>**Register**</DNT>

   をクリックしてアプリケーションを作成します。
...
2. <DNT>**Client secrets**</DNT>
   ...
   をクリックします。 最長 2 年までの秘密の有効期限を選択します。 シークレットの有効期限が切れたら、手順に従って[アプリケーションの詳細を更新してください](#update-app)。

3. <DNT>**Client Secret**</DNT>

   の値をコピーして、後で使用するために保存します。
...
1. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure**</DNT>
   ...
   に移動し、追加する Azure サービスを選択します。

2. UIにおける手順に従い、インテグレーションをNew Relicで有効にします。

3. Azure アカウントの手順をすでに完了している場合は、手順の最後までスキップしてフォームに入力してください。 ( `Azure account name`には、

   <DNT>**Integrations**</DNT>

   ダッシュボードでアカウントを識別するために使用する名前を入力します。)
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        4. [Azure](https://portal.azure.com/)にサインインして
        ...
        5. 登録済みのアプリを見つけます (推奨される名前は`NewRelic-Integrations`です)。 利用可能なアプリの完全なリストを表示するには、検索フィールドの横にあるドロップダウン メニューを選択し、
