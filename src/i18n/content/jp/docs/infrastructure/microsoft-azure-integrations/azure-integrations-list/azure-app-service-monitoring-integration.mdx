...
* `AzureAppServiceWebAppSample`：アカウント内の各Azure AppServiceWebアプリを表します。[インベントリデータ](#inventory)の`provider`値は`AzureAppServiceWebApp`です。
* `AzureAppServiceHostNameSample`：アクセスに使用できるWebアプリにバインドされた各ドメイン名を表します。[インベントリデータ](#inventory)の`provider`値は`AzureAppServiceHostName`です。
