...
<Callout title="注記">
  Azure Cost Managementのデータは、従来の（ポーリング）Azureインテグレーションを通じてのみ収集されます。このインテグレーションは、**利用できません** [Azure Monitorインテグレーション](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/)を通じては。コストデータはイベントベースであり、複数のリソースやサービスにまたがるため、Azure Monitorインテグレーションで必要とされるような単一のAzureリソースタイプにはマッピングされません。

  従来のAzureインテグレーションからAzure Monitorインテグレーションに移行する場合は、Azure Cost Managementデータの収集を継続するために、従来のAzureインテグレーションを有効にしたままにしてください。
</Callout>