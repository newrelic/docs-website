...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="エンティティ"
  >
    エンティティの部分的なリストは、パブリック プレビューの一部として利用できます。 カスタムを作成できます<InlinePopover type="dashboards"/>そして<InlinePopover type="alerts"/>新しい指標に基づいて。
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities)> Infrastructure > Azure**に移動し、**Manage Services** \[サービスの管理]をクリックします。
2. 新しい**Azure Monitor metrics** \[Azure Monitor メトリック] 統合を有効にし、 **Save Changes** \[変更の保存]をクリックします。<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
...
4. 新しい統合を有効にして構成します。<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
