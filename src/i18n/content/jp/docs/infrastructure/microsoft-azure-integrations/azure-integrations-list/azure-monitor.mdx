...
<CollapserGroup>
  ...
  >
    新しいインテグレーションにより、すべてのサービスからすべてのメトリックを取り込むことができるため、 <DoNotTranslate>**the amount of API calls to Azure will depend on the number of resources that are monitored by New Relic**</DoNotTranslate> 。 インテグレーションのポーリング間隔の設定により、これらの API コールの量を制御することができます。
...
2. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure**</DoNotTranslate>
...
3. <DoNotTranslate>**Configure**</DoNotTranslate>
...
<CollapserGroup>
  ...
  >
    ...
    信号設定を構成するには、 <DoNotTranslate>**Condition Settings**</DoNotTranslate>で<DoNotTranslate>**Advanced Signal Settings**</DoNotTranslate>をクリックし、次の値を入力します。

    * <DoNotTranslate>**Aggregation window**</DoNotTranslate>

      : 1 分の集計ウィンドウでは、8 分の評価オフセットを設定することをお勧めします。
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure**</DoNotTranslate>に移動し、 <DoNotTranslate>**Manage Services**</DoNotTranslate>をクリックします。
2. 新しい<DoNotTranslate>**Azure Monitor metrics**</DoNotTranslate>インテグレーションを有効にして、 <DoNotTranslate>**Save Changes**</DoNotTranslate>をクリックします。<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. <DoNotTranslate>**Azure Monitor metrics**</DoNotTranslate>統合が利用可能になります。<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}/>
4. 新しい統合を有効にして設定します。<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
5. セットアップが完了したら、 <DoNotTranslate>**Manage services**</DoNotTranslate>設定で以前のポーリング統合を無効にすることができます。
