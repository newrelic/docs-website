...
2. 新しい **Azure Monitor メトリック** 統合を有効にし、 **Save Changes** \[変更の保存] ボタンをクリックします。<img title="Shows where to enable the Azure monitor" alt="Shows where to enable the Azure monitor" src={infrastructureAzureMonitorMetricsIntegration}/>
3. **Azure Monitor メトリクス** の 統合が利用可能になります<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}>。</img>
