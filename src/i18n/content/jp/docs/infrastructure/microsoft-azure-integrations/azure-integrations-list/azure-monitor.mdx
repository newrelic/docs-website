...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="アラート条件"
  >
    Azure Monitor 統合メトリックで[NRQL アラート条件](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions)を作成できます。
    ...
    * **集計ウィンドウ**: 1 分間の集計ウィンドウでは、8 分間の評価オフセットを設定することをお勧めします。
    ...
    詳細については、 [NRQL アラートの作成方法 を](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/nrql-alert-conditions)参照してください。
  </Collapser>
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Azure に移動し、「サービスの管理」をクリックします**。
...
3. **Azure Monitor metrics** \[Azure Monitor メトリクスの]統合が利用可能になります。<img title="Shows where azure monitor metrics integration" alt="Shows where azure monitor metrics integration" src={infrastructureAzureMonitorMetricsConfigure}/>
4. [ここで](/docs/infrastructure/microsoft-azure-integrations/azure-integrations-list/azure-monitor/#set-up)説明されているように、新しい統合を有効にして構成します。<img title="Shows where azure monitor enable toggle button" alt="Shows where azure monitor enable toggle button" src={infrastructureAzureMonitorEnableToggle}/>
