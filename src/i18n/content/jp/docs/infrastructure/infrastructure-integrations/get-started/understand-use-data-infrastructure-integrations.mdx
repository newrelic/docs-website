...
* ダッシュボードの表示：New Relic UIでダッシュボードを検索できます。**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure**にアクセスし、データを探します。
...
<CollapserGroup>
  ...
  >
    ...
    * メトリクス値は**属性**（[イベント](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#event-data)に添付されたキーと値のペア）として扱われます。たとえば、MySQLインテグレーションには「アクティブ接続」メトリクスがあります。これは、`MysqlSample`イベントの`connectionsActive`属性をクエリすることで見つかります。
