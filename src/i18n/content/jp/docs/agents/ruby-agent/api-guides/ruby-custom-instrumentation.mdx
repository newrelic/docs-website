...
## 初期化子のトレーシング [#tracing-initializers]
...
## トランザクションの命名 [#naming-transactions]
...
メソッドレベルのトレーシングに加え、バックグラウンドタスクなどのWeb以外のトランザクションも、Webトランザクションと同レベルのトランザクションおよびエラー詳細でインストゥルメントできます。詳しくは[Rubyのバックグラウンド処理とデーモンを監視する](/docs/ruby/monitoring-ruby-background-processes-and-daemons)を参照してください。

## 高度なカスタムインストゥルメンテーション [#advanced-tracing]

エージェントによってトレーシングコードが自動的にインストゥルメントされない場合、通常は標準の[メソッドトレーサー](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/ruby-agent/api-guides/ruby-custom-instrumentation#method_tracers)で十分です。
...
詳しくは、[New Relic RubyDocのTracer](http://rubydoc.info/github/newrelic/newrelic-ruby-agent/NewRelic/Agent/Tracer/)を参照してください。

## その他のヘルプ [#more_help]

その他のドキュメンテーションには、以下の資料があります。

* [カスタムインストゥルメンテーション](/docs/features/custom-instrumentation)（カスタムインストゥルメンテーションの概要）
* [Rubyカスタムメトリクス](/docs/ruby/ruby-custom-metrics)（APIコールを使用して任意のメトリクスを記録）
