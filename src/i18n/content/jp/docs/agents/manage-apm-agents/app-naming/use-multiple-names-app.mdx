...
tags:
  ...
  - App naming
metaDescription: >-
  Aggregate your app's data under a maximum of three different names by editing
  your New Relic agent's config file.
---
...
通常、2つのインスタンスが同じアプリ名、エージェント言語、ライセンスキーでレポートする場合、New RelicはNew Relicユーザーインタフェース内の1つのアプリにそのデータを集計します。アプリのNew Relic [APM **概要**ページ](/docs/apm/applications-menu/monitoring/apm-overview-page#infra-server)から個別のホストやインスタンスのデータを表示することも可能です。
...
デフォルトで、複数の名前のあるアプリは、トランザクションに対して複数のInsightsイベントを生成します（それぞれの名前に重複するトランザクション）。たとえば、アプリに3つの名前を付けると、トランザクションのInsightsイベント数の3倍となります。

Insightsイベントが重複しないようにするには、各重複するアプリ名の収集を無効にします。
...
<figcaption>
  重複するイベントが、複数の名前を持つアプリに収集されないようにするには、**[insights.newrelic.com](https://insights.newrelic.com) > データの管理 > データソースの設定**の順に移動します。
</figcaption>
