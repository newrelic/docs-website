...
1. **[one.newrelic.com/apm](https://one.newrelic.com) > APM >（アプリケーションを選択） > Settings > Application**の順に移動します。
2. **Application alias**フィールドに、選択したアプリケーションの新しい名前（エイリアス）を入力します。
...
## アプリケーションに複数の名前を使用する [#multiple-names]
