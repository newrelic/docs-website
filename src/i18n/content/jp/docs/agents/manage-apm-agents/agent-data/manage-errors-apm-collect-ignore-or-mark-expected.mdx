...
RubyエージェントおよびJavaエージェントの場合は、想定されるエラーとしてマーク付けできます。これらのエラーはAPMにレポートされ、表示されますが、[Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction)または[エラー率](/docs/apm/applications-menu/error-analytics/error-analytics-explore-events-behind-errors)（またはエラー率を基にした[アラート条件](/docs/alerts/new-relic-alerts/defining-conditions/define-alert-conditions)）には影響しません。
