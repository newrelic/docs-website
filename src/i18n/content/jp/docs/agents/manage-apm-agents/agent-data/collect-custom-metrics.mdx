...
New Relic APMエージェント、ブラウザエージェント、モバイルエージェントは、[メトリックタイムスライスデータ](/docs/using-new-relic/data/understand-data/new-relic-data-types#timeslice-data)というメトリックデータのタイプをレポートします。これらのエージェントを使用すると、アプリケーションからカスタムメトリックタイムスライスをレポートし、デフォルトNew Relicデータと一緒に表示します。APIコールを使用して以下のような任意のパフォーマンスデータを記録するには、カスタムメトリックを作成します。
...
`Custom/`で始まらないカスタムメトリック名はいずれも、その他のすべてのグループ化ルールの対象となります。それらの名前はInsightに表示されないか、New Relic UIに期待通りに表示されない可能性があります。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Browserはカスタムメトリックをサポートしていません。カスタムデータをBrowserに追加するオプションについては、「[Browserインストゥルメンテーション](/docs/browser/new-relic-browser/page-load-timing-resources/instrumentation-browser-monitoring)」を参照してください。
      </td>
    </tr>
  </tbody>
</table>
