...
<CollapserGroup>
  ...
  >
    データストアへの呼び出しをモニターするには、インストゥルメントされたトランザクション内のデータストアセグメントをインストゥルメントします。データストアセグメントは、New Relicの[**トランザクション**ページ](/docs/apm/applications-menu/monitoring/transactions-page)にある**内訳**表と**データベース**タブに表示されます。データストアセグメントは[APM`トランザクション`イベント](/docs/insights/insights-data-sources/default-events-attributes/apm-default-event-attributes)の[`databaseDuration`属性](https://docs.newrelic.co.jp/attribute-dictionary/transaction/databaseduration)としても表示できます。
