...
当社のPHPエージェントは、コードの自動インストゥルメンテーションを行うので、アプリケーションのモニターを開始できます。以下の手順に従って、AWS Linux 2、RedHat、またはCentOSを使用してNew Relic APM用PHPエージェントをインストールします。**例外:** 以前のバージョンをインストールしている場合、[エージェントをアップグレード](/docs/agents/php-agent/installation/upgrading-php-agent)します。
