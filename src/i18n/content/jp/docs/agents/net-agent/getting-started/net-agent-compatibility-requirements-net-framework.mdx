...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="アプリケーションの寿命"
  >
    .NETエージェントは、各[収集サイクル](https://docs.newrelic.co.jp/docs/using-new-relic/welcome-new-relic/getting-started/glossary)の終わりにデータをアップロードします（1分間に1回）。.NETアプリケーションがそれほど長く実行されない場合は、`newrelic.config`ファイルで、その`サービス要素`の`sendDataOnExit`属性を`true`に設定することができます。
  </Collapser>
</CollapserGroup>
...
.NETエージェントは、データストアプロセスを直接モニターしません。また、デフォルトでは、クエリのトレースでキャプチャされた.NET SQLパラメーターには、パラメーター化したクエリやストアドプロシージャのパラメーターはリストされません。SQLクエリパラメーターの収集は、エージェントの[設定](https://docs.newrelic.co.jp/docs/agents/net-agent/configuration/net-agent-configuration)で有効になります。
          ...
          <td>
            ...
            * [WCF](/docs/agents/net-agent/features/instrumenting-wcf-applications)
              ...
              * [**バインディングタイプ**](http://docs.microsoft.com/en-us/dotnet/framework/wcf/system-provided-bindings)（クライアントとサービスの両方）  
                ...
                \* ディストリビューティッド（分散）トレーシングは、これらの[ディストリビューティッド（分散）トレーシングAPI](https://docs.newrelic.co.jp/docs/understand-dependencies/distributed-tracing/enable-configure/enable-distributed-tracing)メソッドを使用して手動で実装できます。
