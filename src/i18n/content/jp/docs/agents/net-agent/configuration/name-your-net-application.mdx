...
[New Relic の .NETエージェント](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/net-agent/getting-started/introduction-new-relic-net)は、APMで報告および表示されるアプリケーション名を変更する方法をいくつか提供します。このドキュメントでは、これらの命名オプションの概要について説明します。

（.NET に固有ではない）APM アプリの命名の一般的な概要については、[アプリケーションに名前を付ける](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/manage-apm-agents/app-naming/name-your-application)を参照してください。データの連続性を損なうことなくアプリケーションの名前を変更するには、[UI エイリアス変更オプション](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/manage-apm-agents/app-naming/name-your-application#differences)を使用します。
...
アプリがアクティブである限り、データは数分以内に表示されます。そうでない場合は、[トラブルシューティング手順](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/net-agent/troubleshooting/no-data-appears-net)に従ってください。
...
<Callout variant="important">
  一部のメソッドは他のメソッドよりも[優先](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/net-agent/configuration/net-agent-configuration#config-options-precedence)されます。命名方法を選択する前に、設定がどのように影響を受けるかを理解してください。
</Callout>
      ...
      <td>
        ...
        <CollapserGroup>
          ...
          >
            ...
            <Callout variant="important">
              この方法は、.NET エージェント[バージョン 6.17.387.0 以降](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/agent-release-notes/net-release-notes)を使用している場合にのみ機能します。
            </Callout>
          </Collapser>
        </CollapserGroup>
      </td>
...
アプリがアクティブである限り、データは数分以内に表示されます。データが表示されない場合は、 [.NETアプリの設定ファイル](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/net-agent/installation-and-configuration/net-agent-configuration)（ `newrelic.config` または `app.config` ）に変更を加えたなら、管理コマンドから `IISRESET` を実行するのを忘れないでください。それ以外の場合は、[トラブルシューティング手順](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/net-agent/troubleshooting/no-data-appears-net)に従ってください。

## 複数のアプリ名を使用する [#multiple]

複数のアプリ名を使用して、アプリケーションからデータを集計できます。この操作をデータの「ロールアップ」と呼ぶことがあります。

.NET エージェントの場合、最大3つのアプリケーション名を設定できます。最初の名前はプライマリ アプリケーション名であり、2番目と3番目の名前は、より一般的なデータ集計カテゴリに使用されます。複数のアプリ名の仕組みの詳細については、「[複数のアプリ名を使用する](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/manage-apm-agents/app-naming/use-multiple-names-app)」をご覧ください。
