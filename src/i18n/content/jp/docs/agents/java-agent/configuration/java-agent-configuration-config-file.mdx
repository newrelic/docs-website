...
![java-config-order.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/styles/inline_660px/public/thumbnails/image/java%20config%20cascade.png?itok=EZhE5mat "java-config-order.png")
...
<CollapserGroup>
  ...
  >
    ...
    New Relicへのセキュアな接続を確立するためにエージェントが使用するカスタムSSL証明書バンドルへのパスを指定します。カスタムSSL証明書バンドルにNew Relicに接続するのに十分な証明書が含まれていない場合は、[必要な証明書をカスタム証明書バンドル](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/troubleshooting/ssl-or-connection-errors-java)にマージする必要があります。
  </Collapser>
  ...
  >
    ...
    <Callout variant="important">
      [Javaエージェント3.48.0](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/agent-release-notes/java-release-notes/java-agent-3480)では、SSLはデフォルトで有効になっており、これを無効にする設定オプションは非推奨となっています。[Javaエージェント4.0.0](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/agent-release-notes/java-release-notes/java-agent-400)では、SSLを無効にする機能は削除されています。
    </Callout>
  </Collapser>
  ...
  >
    ...
    アカウントの有効な[APIキーを挿入](https://jajp-newrelic-docs.onelink-translations.com/docs/apis/get-started/intro-apis/types-new-relic-api-keys#event-insert-key)です。これは、[JFRメトリックスを使用してプロファイリングを行うリアルタイムJava](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics)についてのみ必須です。
  </Collapser>
  ...
  >
    ...
    Reactor Nettyに対してエラーが報告されたかどうか。falseに設定すると、エラーは無視されます。

    <Callout variant="important">
      [Javaエージェント6.3.0](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/agent-release-notes/java-release-notes/java-agent-630)以降でのみ利用可能です。
    </Callout>
  </Collapser>
...
Javaエージェントは、JMXを使用してJVMデータを収集し、[リアルタイムJavaプロファイリング](http://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics)用のJFR（Java Flight Recorder）デーモンと通信します。
...
<CollapserGroup>
  ...
  >
    ...
    この設定を有効にすると、Javaエージェントがリンクメタデータを[JFRデーモン](https://github.com/newrelic/newrelic-jfr-core)に公開できるようになります。そうすることで、JFRデーモンはJavaエージェントによって生成されたエンティティGUIDを取得し、個別のエンティティとしてではなく、エージェントによって監視されているのと同じAPMアプリケーションにJFRデータをリンクできます。

    <Callout variant="important">
      Javaエージェントの[バージョン6.1.0以降](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/agent-release-notes/java-release-notes)に適用されます。
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    エージェントがスロークエリの`EXPLAIN`プランを捕捉するかどうかを決定します。 [MySQLとPostgreSQLでのみサポートされています。](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/getting-started/compatibility-requirements-java-agent#JDBC)
  </Collapser>
  ...
  >
    ...
    スロークエリと`EXPLAIN`プラン（[サポートされている場合](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/getting-started/compatibility-requirements-java-agent#JDBC)）の捕捉の上限となる、クエリ実行時間の秒単位の閾値。`record_sql` が`raw`または`obfuscated`に設定されている場合にのみ、スロークエリに関連します。`explain_enabled` が`true`に設定されている場合にのみ、`EXPLAIN`プランに関連します。
  </Collapser>
...
APMでは、New Relic言語エージェントAPIにより[カスタム イベントデータを記録](https://jajp-newrelic-docs.onelink-translations.com/docs/insights/insights-data-sources/custom-data/insert-custom-events-new-relic-apm-agents)でき、[これをクエリすることが可能です](/docs/query-your-data/explore-query-data/explore-data/introduction-querying-new-relic-data)。
...
<Callout variant="important">
  ディストリビューティッド（分散）トレーシングを有効にすると、[クロスアプリケーショントレーシング](#Cross_Application_Tracer)が無効になり、他のAPM機能に影響します。有効にする前に、[移行ガイド](https://jajp-newrelic-docs.onelink-translations.com/docs/transition-guide-distributed-tracing)を読んでください。
...
<Callout variant="important">
  ...
  * [Javaエージェント 5.12.1以降](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/installation/update-java-agent)。
  * Infinite Tracingは、`enable_auto_app_naming`が有効な場合は動作しません。
</Callout>

Infinite Tracing をオンにするには、ディストリビューティッド（分散）トレーシングを有効にして、以下の追加設定を加えます。例えば、「[言語エージェント：ディストリビューティッド（分散）トレーシングを構成する](https://jajp-newrelic-docs.onelink-translations.com/docs/understand-dependencies/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing#java-config)」をご覧ください。
...
<CollapserGroup>
  ...
  >
    ...
    有効なInfinite Tracing トレース オブザーバーのホストエントリを取得する方法については、[トレースオブザーバーを検索または作成する](https://jajp-newrelic-docs.onelink-translations.com/docs/understand-dependencies/distributed-tracing/enable-configure/language-agents-enable-distributed-tracing#provision-trace-observer)をご覧ください。
...
[スパンイベント](https://jajp-newrelic-docs.onelink-translations.com/docs/apm/distributed-tracing/ui-data/span-event)は[ディストリビューティッド（分散）トレーシング](#distributed-tracing)に関してレポートされます。スパンイベントをレポートするには、ディストリビューティッド（分散）トレーシングが有効になっていなければなりません。
