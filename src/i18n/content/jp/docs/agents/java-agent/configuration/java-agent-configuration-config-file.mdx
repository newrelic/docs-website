...
<CollapserGroup>
  ...
  >
    ...
    アカウントの有効な[APIキーを挿入](/docs/apis/get-started/intro-apis/types-new-relic-api-keys#event-insert-key)です。これは、[JFRメトリックスを使用してプロファイリングを行うリアルタイムJava](/docs/agents/java-agent/features/real-time-java-profiling-using-jfr-metrics)についてのみ必須です。
  </Collapser>
