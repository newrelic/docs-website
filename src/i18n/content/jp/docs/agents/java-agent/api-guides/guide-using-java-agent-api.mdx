...
これらのAPIでは、[ディストリビューティッド（分散）トレーシングを有効化](https://docs.newrelic.co.jp/docs/enable-distributed-tracing)する必要があります。ディストリビューティッド（分散）トレーシングのすべての設定オプションについては、[Javaエージェントの設定](https://docs.newrelic.co.jp/docs/agents/java-agent/configuration/java-agent-configuration-config-file#distributed-tracing)をご覧ください。

ディストリビューティッド（分散）トレーシングでは、リクエストが分散システムを経由するパスを見ることができます。以下のコールを利用してディストリビューティッド（分散）トレーシングを実装する一般的な案内については、[ディストリビューティッド（分散）トレーシングAPIの使用](https://docs.newrelic.co.jp/docs/enable-distributed-tracing#agent-apis)を参照してください。動作するAPIを確認するには、[KafkaでのJavaエージェントのディストリビューティッド（分散）トレーシングAPIの使用](https://github.com/newrelic/newrelic-java-examples/tree/main/newrelic-java-agent/distributed-tracing/kafka-examples)をご覧ください。

<Callout variant="important">
  [エージェントのバージョン6.4.0](https://docs.newrelic.co.jp/docs/release-notes/agent-release-notes/java-release-notes/java-agent-640)では、6.1.0で導入された`addCustomAttribute()`を除いて、以下のディストリビューティッド（分散）トレーシングAPIが導入されました。[非推奨のAPI](#deprecated-trace-apis)の代わりに、これらのAPIを使用することを強く推奨します。
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        ディストリビューティッド(分散)トレーシングのヘッダーを作成して、`Headers`データ構造に挿入します。このAPIは、`newrelic`ヘッダーとW3C Trace Contextヘッダー（`traceparent`と`tracestate`）の両方を挿入します。明示的にnewrelicヘッダーを[除外するようにエージェントが設定`されている場合は例外です。`](https://docs.newrelic.co.jp/docs/agents/java-agent/configuration/java-agent-configuration-config-file#dt-exclude_newrelic_header)

        [](https://docs.newrelic.co.jp/docs/agents/java-agent/configuration/java-agent-configuration-config-file#dt-exclude_newrelic_header)
      </td>
...
<Callout
  ...
  variant="caution"
>
  [エージェントのバージョン6.4.0](https://docs.newrelic.co.jp/docs/release-notes/agent-release-notes/java-release-notes/java-agent-640)では、以下のディストリビューティッド（分散）トレーシングAPIが非推奨になり、上の表のAPIで置き換えられました。エージェントをアップグレードして、非推奨APIの代わりに新しいAPIを使用することを強く推奨します。
</Callout>
