...
アプリケーションで[`Transactions`](https://docs.newrelic.com/docs/apm/applications-menu/monitoring/transactions-page)をインストゥルメントするには、以下のAPIを使用してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        [カスタムアトリビュート](https://docs.newrelic.com/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes)を`Transactions`と`TransactionEvents`に追加する
      </td>
    ...
    <tr>
      <td>
        アプリケーションの`Apdex`スコアの計算時に[Transaction](https://docs.newrelic.com/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction)を除外する
      </td>
...
これらのAPIでは、[ディストリビューティッド（分散）トレーシングを有効化](https://docs.newrelic.com/docs/enable-distributed-tracing)する必要があります。ディストリビューティッド（分散）トレーシングのすべての設定オプションについては、[Javaエージェントの設定](https://docs.newrelic.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#distributed-tracing)をご覧ください。

ディストリビューティッド（分散）トレーシングでは、リクエストが分散システムを経由するパスを見ることができます。以下のコールを利用してディストリビューティッド（分散）トレーシングを実装する一般的な案内については、[ディストリビューティッド（分散）トレーシングAPIの使用](https://docs.newrelic.com/docs/enable-distributed-tracing#agent-apis)を参照してください。動作するAPIを確認するには、[KafkaでのJavaエージェントのディストリビューティッド（分散）トレーシングAPIの使用](https://github.com/newrelic/newrelic-java-examples/tree/main/newrelic-java-agent/distributed-tracing/kafka-examples)をご覧ください。

<Callout variant="important">
  [エージェントのバージョン6.4.0](https://docs.newrelic.com/docs/release-notes/agent-release-notes/java-release-notes/java-agent-640)では、6.1.0で導入された`addCustomAttribute()`を除いて、以下のディストリビューティッド（分散）トレーシングAPIが導入されました。[非推奨のAPI](#deprecated-trace-apis)の代わりに、これらのAPIを使用することを強く推奨します。
</Callout>
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        ディストリビューティッド（分散）トレーシングのヘッダーを作成して、`Headers`データ構造に挿入します。このAPIは、`newrelic`ヘッダーとW3C Trace Contextヘッダー（`traceparent`と`tracestate`）の両方を挿入します。明示的にnewrelic[ヘッダー`を`除外するようにエージェントが設定](https://docs.newrelic.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#dt-exclude_newrelic_header)されている場合は例外です。
      </td>
    ...
    <tr>
      <td>
        分散トレースの[カスタム属性](https://docs.newrelic.com/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes)を`  SpanEvents  `に追加
      </td>
...
<Callout
  ...
  variant="caution"
>
  [エージェントのバージョン6.4.0](https://docs.newrelic.com/docs/release-notes/agent-release-notes/java-release-notes/java-agent-640)では、以下のディストリビューティッド（分散）トレーシングAPIが非推奨になり、上の表のAPIで置き換えられました。エージェントをアップグレードして、非推奨APIの代わりに新しいAPIを使用することを強く推奨します。
</Callout>
...
外部コールを追跡し、 [クロスアプリケーショントレーシング](https://docs.newrelic.com/docs/apm/transactions/cross-application-traces/cross-application-tracing)を追加するには、以下のAPIを使用します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        JavaエージェントAPIの[`@Trace`アノテーション](https://docs.newrelic.com/docs/agents/java-agent/custom-instrumentation/java-instrumentation-annotation)を使って、トレースされている外部HTTPサービス、データベースサーバー、メッセージキュー、またはその他の外部リソースへのコールを報告する
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        [カスタムメトリックス](https://docs.newrelic.com/docs/agents/manage-apm-agents/agent-data/custom-metrics)を作成および蓄積する。
      </td>
