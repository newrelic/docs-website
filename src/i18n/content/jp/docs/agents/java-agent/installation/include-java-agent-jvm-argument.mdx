...
詳細については、「[WebSpherePMIメトリックスの収集](/docs/agents/java-agent/features/jvm-metrics-page#default-pmi)」に関するドキュメントをご覧ください。
...
Wildflyで`-javaagent`引数を渡すには（Wildfly 11以降をお使いの場合は、[追加のインストール手順](/docs/agents/java-agent/additional-installation/wildfly-version-11-installation-java)を参照）：
