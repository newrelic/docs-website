...
当社のJavaエージェントは、コードを自動インストゥルメントするので、モニタリングアプリケーションを開始できます。当社のランチャーを使用するか、本ドキュメントの説明に従って基本的なJavaエージェントのインストレーションを実行できます。
...
* New Relic Diagnostics を使用して、デプロイの前後に[設定を検証](https://jajp-newrelic-docs.onelink-translations.com/docs/using-new-relic/cross-product-functions/troubleshooting/new-relic-diagnostics#h2-validate-your-config-file-settings)することをお勧めします。
