...
3. ポートスキャンのためにスキップされるデバイスの大規模なセットがある場合は、 `snmp-base.yaml`ファイルを編集し、 [check_all_ips](/docs/network-performance-monitoring/advanced/advanced-config/#discovery)のオプションを有効にして、ポートスキャンをスキップし、検出内のすべてのアドレスに対してSNMP資格情報を直接テストすることができます。 。このオプションを使用すると、検出を完了するのにかかる時間が大幅に長くなることに注意してください。