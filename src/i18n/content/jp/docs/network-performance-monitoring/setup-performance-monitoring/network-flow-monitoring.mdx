...
ネットワーク機器が New Relic One にネットワークデータを送信するように設定します。
...
- A New Relic [アカウント ID](/docs/accounts/accounts-billing/account-setup/account-id) 。
- A New Relic [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 。

### Linux ホストの前提条件 [#prerequisites-host]

- [Docker](https://docs.docker.com/engine/install/) を Linux ホストにインストールした状態。
- Docker ホストに SSH でアクセスし、新しいコンテナを起動することができます。

### ネットワークフローデータデバイスの前提条件 [#devices-prerequisites]

- **ktranslate** docker コンテナを実行しているホストにフローデータを送信するネットワークデバイスを設定しました。ここでは、いくつかのデバイスでネットワークフローデータ収集を設定する方法を紹介します。
...
NPM のフローモニタリングは、ネットワークフローデータの 4 つの主要タイプとその派生タイプをサポートしています。 **ktranslate** コンテナを実行する際には、 `-nf.source` オプションを使用して、監視したいメジャータイプを指定します。

<Callout variant="important">
  **ktranslate** コンテナは、一度に1種類のネットワーク・フロー・データ・タイプの監視しかサポートしていません。複数のタイプを監視したい場合は、それぞれにコンテナが必要になります。

IPFIX と NetFlow v9 は同じコンテナに送ることができますが、ベストプラクティスとして別のコンテナで実行することをお勧めします。
...
大規模なネットワークフローを収集するための戦略を計画する際、New Relic は 2000 フロー/秒（12 万フロー/分）ごとに 1CPU を推奨しています。負荷を分散するために小さなコンテナを多く実行するか、管理を統合するために大きなコンテナを少なくするかは、個人の好みによります。

## New Relic One でのネットワークフローデータ監視の設定 [#setup-network-flow-monitoring]
...
3. New Relic One の手順に従ってください。
   ![ネットワークフローのガイド付き設定](src/images/network-flows-guided-install.png
   <figcaption>
     one.newrelic.com &gt; Add more data ネットワークパフォーマンスモニタリング
     ネットワークフロー ネットワークフローデータのモニタリングを設定します
   </figcaption>
4. ネットワーク機器のパフォーマンスをよりよく把握するために、 [SNMP データモニタリングを設定する](/docs/network-performance-monitoring/setup-performance-monitoring/snmp-performance-monitoring) 。
5. [ネットワークのパフォーマンスデータを New Relic で可視化](/docs/network-performance-monitoring/monitoring-network-data/visualize-network-data).
