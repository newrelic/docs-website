...
3. New Relic One の手順に従ってください。 ![ネットワークフローのガイド付き設定](./images/network-flows-guided-install.png "ネットワークフローのガイド付き設定") > **' ' one.newrelic.com Add more data ネットワークパフォーマンスモニタリング ネットワークフロー ネットワークフローデータのモニタリングを設定します。 <figcaption>**[](https://one.newrelic.com)**> ********></figcaption>**
