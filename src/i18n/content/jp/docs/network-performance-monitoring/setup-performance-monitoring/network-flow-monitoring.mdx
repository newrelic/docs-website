...
3. New Relic UI で概説されている手順に従います。利用可能なインストール方法は、Docker または Linux パッケージ マネージャーです。
   ...
   <figcaption>
     **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Add more data > Network > Network Flows を選択して、**ネットワーク フロー データのモニタリングを設定します。
   </figcaption>
