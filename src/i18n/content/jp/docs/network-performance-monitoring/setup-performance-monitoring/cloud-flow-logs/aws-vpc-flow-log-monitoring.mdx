...
<img
  title="AWS VPC flow logs overview"
  alt="AWS VPC flow logs overview"
  src={overviewCloudFlowLogs}
/>

<figcaption>
  **[AWS VPC フロー ログを New Relic アカウントに](https://one.newrelic.com/nr1-core?&state=bd947742-3034-63b7-7196-8baaf36dd8d9)追加します**。
</figcaption>
...
<Callout variant="important">
  Kinesis Data Firehose 経由の AWS VPC フロー ログは、FedRAMP のお客様にはまだサポートされていません。それまでの間、 [FedRAMP 取り込み API](/docs/security/security-privacy/compliance/fedramp-compliant-endpoints#data-ingest-apis)を使用できます。
</Callout>

* 環境：

  * [AWS CLI（v 1.9.15+）がインストールされて](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html)います。
  * AWS VPC フロー ログを収集する AWS リージョンのリスト。
...
## New Relic で AWS VPC フロー ログの監視を設定する [#setup-aws-vpc-flow-logs]

AWS VPC フロー ログ用に設計されたダッシュボードを含むバンドルをインストールすることで、AWS VPC フロー ログをセットアップできます。

1. [AWS VPC フロー ログ](https://newrelic.com/instant-observability/aws-vpc-flow-logs/61d2ff67-f519-492c-a8de-6019b47d5bb2)の New Relic Instant Observability ページに移動します。<img title="AWS VPC flow logs I/O page" alt="AWS VPC flow logs instant observability page" src={awsVPCflowLogsIO}/>

   <figcaption>
     **[New Relic Instant Observability](https://newrelic.com/instant-observability/aws-vpc-flow-logs/61d2ff67-f519-492c-a8de-6019b47d5bb2)**を使用して、AWS VPC フロー ログの監視をセットアップします。
   </figcaption>
...
## 追加の AWS VPC フロー ログを送信する [#additional-aws-vpc-flow-logs]

さらに、追加の AWS VPC フロー ログをインストールする必要がある場合、またはカスタム ダッシュボードでバンドルをインストールしたくない場合は、次の手順に従います。

1. **[ガイド付きインストール プロセス](https://one.newrelic.com/nr1-core?&state=bd947742-3034-63b7-7196-8baaf36dd8d9)**を開始します。
2. **\[アカウントの選択**] ドロップダウンから、AWS VPC フロー ログを送信するアカウントを選択し、\[**続行**] をクリックします。
3. **\[データの選択**] セクションで、\[ **AWS VPC フロー ログ**] を選択し、\[**続行**] をクリックします。
4. **\[Select your install method** ] セクションで、 **CLI**に進み、\[ **Continue** ] をクリックします。

これらの手順の後、AWS Kinesis Firehose サービスを介して AWS VPC フロー ログを New Relic に送信するための新しいウィザードがポップアップ表示されます。
...
* **Flow Source ID**フィールドに、AWS VPC フロー ログを作成する VPC ID ( `vpc-MY_STRING` ) またはサブネット ID ( `subnet-MY_STRING` ) を入力します。
...
6. \[**続行**] をクリックして、New Relic のネットワーク監視セクションで AWS VPC フロー ログの調査を開始します。
