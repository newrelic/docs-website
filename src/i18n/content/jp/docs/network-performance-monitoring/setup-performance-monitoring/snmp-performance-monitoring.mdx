...
1. `snmp-base.yaml`構成ファイルを更新して、リスニングIPを`127.0.0.1`から`0.0.0.0`に変更します。_（これにより、Dockerコンテナーは`docker0`インターフェースで外部パケットをリッスンできます。）_
...
2. `docker run...`コマンドでポート転送を有効にして、ホストのUDP162に送信されたパケットをコンテナのUDP1620にリダイレクトします。
