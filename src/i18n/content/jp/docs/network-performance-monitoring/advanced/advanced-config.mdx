    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `mib_profile_dir`パス内のプロファイルの変更時に SNMP スレッドをリロードするようにウォッチャーを設定します。 デフォルトでは、 `false`に設定されています。
          </td>
        </tr>
      </tbody>
    </table>
...
デフォルトでは、`ktranslate` Dockerコンテナを手動で破棄し、再構築して、`mib_profile_dir` パスの SNMP プロファイルへの変更を組み込む必要があります。 Dockerイメージはパブリック[snmp-profiles リポジトリ](https://github.com/kentik/snmp-profiles)から入手可能な最新のプロファイルを取得するため、これはほとんどのデプロイメントにおける通常の動作です。 [カスタム プロファイル](/docs/network-performance-monitoring/advanced/snmp-profiles/#private)を提供する場合、この設定を使用して、コンテナがコンテナの基礎となる構成と SNMP プロファイルを自動的に更新できるようにすることができます。
