...
NewRelicネットワーク監視用に**ktranslateDocker**コンテナを実行しているときに、コンテナの状態を監視して、潜在的な問題をプロアクティブに検出できます。

**ktranslate**コンテナイメージには、実行時に利用可能な`-tee_logs=true`と`-metrics=jchf`の設定があり、ヘルスメトリックをNewRelicに直接送信できます。 New Relicガイド付きインストールを介してネットワーク監視をインストールする場合、これらはデフォルトで有効になっています。ネットワーク監視を手動でインストールする場合は、これらを設定することをお勧めします。
...
2. \[**ログの検索場所**]に`collector.name:"ktranslate"`と入力し、\[**ログのクエリ**]をクリックします。
...
1. **[one.newrelic.com](https://one.newrelic.com)**に移動し、 **Query your data**をクリックします。
