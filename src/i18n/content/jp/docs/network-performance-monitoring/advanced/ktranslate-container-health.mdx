...
**ktranslate**コンテナイメージには、実行時に利用可能な`-tee_logs=true`と`-metrics=jchf`の設定があり、ヘルスメトリックをNewRelicOneに直接送信できます。 New Relic Oneガイド付きインストールを介してネットワーク監視をインストールする場合、これらはデフォルトで有効になっています。ネットワーク監視を手動でインストールする場合は、これらを設定することをお勧めします。
...
`-tee_logs=true`オプションは、デバイスをポーリングするときにログをNewRelicOneに送信します。それらを表示するには、次の手順を実行します。
