...
<CollapserGroup>
  <Collapser
    ...
    title="実行している KTranslate のバージョンは?"
  >
    Logs UIです。
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Query your data**に移動します。
2. 以下のNRQLクエリのいずれかを入力してください。

<CollapserGroup>
  <Collapser
    id="ktranslate-versions"
    title="KTranslate アプリケーションの現在のバージョンは何ですか?"
  >
    ```sql
    FROM Metric
    SELECT
  ...
  <Collapser
    id="top-level-health"
    title="KTranslate アプリケーションの正常性はどの程度ですか?"
  >
    ```sql
    FROM Metric
    SELECT
