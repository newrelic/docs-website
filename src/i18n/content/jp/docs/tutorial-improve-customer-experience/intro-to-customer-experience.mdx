---
title: New Relic で顧客のデジタル エクスペリエンスを向上させる
metaDescription: 'New Relic observability maturity series: this guide gives you a quality foundation for optimizing your web application''s performance to improve your customer experience.'
freshnessValidatedDate: '2023-08-16T00:00:00.000Z'
translationType: machine
---

import cxWhatYouCanMeasureNr from 'images/cx_diagram_what-you-can-measure-nr.webp'

デジタル カスタマー エクスペリエンスとは、すべてのデジタル タッチ ポイント (Web、モバイル、IoT) にわたってユーザーがサービスをどのように操作するかの品質を指します。しかし、追跡する必要があることはたくさんあります。サービスの可用性、確実に使用できるようにするためのパフォーマンス、ユーザーに必要な情報を提供するコンテンツの品質、製品とコンテンツの関連性を管理する必要があります。ユーザーの関心を維持できるようにします。

New Relic は、デジタル エクスペリエンスのパフォーマンスを測定し、改善するのに役立ちます。このチュートリアルでは、このエクスペリエンスを有意義な方法で改善するために次のことを行う方法について説明します。

* グローバルな機能、事業分野、地域に関連した顧客エクスペリエンスを検討する
* ビジネスのステークホルダーが関心を持っていることを報告する
* 次に改善すべき点に優先順位を付ける

<img
  title="Digital Customer Experience (DCX): What you can measure with New Relic"
  alt="Digital customer experience"
  src={cxWhatYouCanMeasureNr}
/>

## 要件 [#requirements]

始める前に、プラットフォームのいくつかの特定の部分を稼働させておく必要があります。これらには次のものが含まれます。

* [すべてのページにインストールされているBrowserProエージェントのバージョン](/docs/browser/browser-monitoring/installation)

* [AJAX 整定時間と分散トレースに対して SPA が有効化](/docs/new-relic-solutions/best-practices-guides/full-stack-observability/browser-monitoring-best-practices-guide/#how-to-do-it)

* シンセティック・モニターの設定

  * [匿名ユーザー用に設定されたPingモニター](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors)
  * [ログインフローに設定されたスクリプトによる合成チェック](/docs/synthetics/synthetic-monitoring/using-monitors/store-secure-credentials-scripted-browsers-api-tests)
  * ドメインごと、ログイン フローごとにモニターを構成する必要があります。

また、次のような基本的な知識を持っていると役立ちます。

* [NewRelic合成モニタリング](/docs/synthetics/synthetic-monitoring/getting-started/get-started-synthetic-monitoring/)

* [ニューレリック](/docs/browser/browser-monitoring/getting-started/)

  <InlinePopover type="browser">[](/docs/browser/browser-monitoring/getting-started/)</InlinePopover>

* [NewRelicブラウザモニタリングUI](/docs/browser/browser-monitoring/getting-started/introduction-browser-monitoring/)

* [ブラウザ監視UIの新しいRelicSPAデータ](/docs/browser/single-page-app-monitoring/use-spa-data/view-spa-data-browser-ui/)

## はじめに [#get-started]

<DocTiles>
  <DocTile
    title="Establish your current state"
    path="/docs/tutorial-customer-experience/establish-current-state/"
  >
    現在の状態を確立してベースラインを作成する方法を学びます
  </DocTile>

  <DocTile
    title="Improve your customer experience"
    path="/docs/tutorial-customer-experience/improve-customer-experience/"
  >
    現在の状態のデータを使用して改善する方法を学びます
  </DocTile>

  <DocTile
    title="Analyze the bottom-of-the-funnel"
    path="/docs/tutorial-customer-experience/bottom-funnel-analysis/"
  >
    顧客エクスペリエンスの終わりを分析してコンバージョンを向上させる方法を学びます
  </DocTile>
</DocTiles>