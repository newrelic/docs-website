...
新しいバージョンの[CLI を](/docs/new-relic-solutions/tutorials/new-relic-cli)使用すると、GraphQL ミューテーションを送信してマーカーを作成できるようになります。CLI を使用している場合でも、この機能を利用するには CLI をアップグレードする必要があります。

CLI をインストールまたはアップグレードするには、[こちら](https://github.com/newrelic/newrelic-cli#installation--upgrades)の手順に従ってください。

## CLI を使用して変更マーカーを作成する [#make-marker-cli]

CLI にはさまざまなオプションがありますが (以下を参照)、 `create`コマンドは特定の New Relic エンティティに対する特定の変更をマークします。
