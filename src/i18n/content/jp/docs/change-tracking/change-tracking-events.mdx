    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            エンティティ検索属性を使用すると、New Relic 内で特定のエンティティを見つけることができます。特定のエンティティ`GUID`を使用して検索することも、 `entityGuid`が不明な場合は他の識別情報を指定して検索することもできます。`entityGuid` 、 New Relic計装またはセットアップ中にアプリケーションやマイクロサービスなどのシステム コンポーネントに割り当てる一意の識別子です。 エンティティの詳細については、 [New Relicエンティティ」](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#what-is-entity)を参照してください。
            ...
            * タグ: バッククォートの有無にかかわらず、タグを参照できます。 デフォルトのエンティティ`properties`および`tags`に基づいてエンティティをフィルタリングできます。

              * デフォルトのエンティティ `properties:`
              ...
              * `Tags:` タグは、通常、より具体的な`domainType`または`domainTypes`のグループにリンクされたメタデータです。例えば：
              ...
              * GUID でエンティティを検索するには: `"id = '<entity guid>'"`
              * REST v2 APIアプリケーション ID を使用してエンティティを検索するには: `"domainId = '<app id>' AND domain = 'APM'"`
              * OpenTelemetry サービスを見つけるには: `"name = '<service name>' AND domain = 'EXT' AND type = 'SERVICE' AND accountId = '<account id>'"`
          </td>
        </tr>
      </tbody>
    </table>
