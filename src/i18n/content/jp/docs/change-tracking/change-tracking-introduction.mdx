...
古い REST v2 APM デプロイメント API を使用している場合は、GraphQL API に移行して、APM サービス以外の[エンティティ タイプ](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#find)の変更のキャプチャを開始する必要があることに注意してください。GraphQL API を使用した変更追跡に切り替えると、現在の Jenkins と GitHub Actions の統合だけでなく、将来の CI/CD 統合も利用できるようになります。
...
### すべての展開データと変更データが 1 か所に [#single-source-truth]

変更追跡 API または統合を介してキャプチャしたすべての変更は、New Relic データベース (NRDB) に保存されます。NRQL と NerdGraph を使用して、展開のクエリとデータの変更を行うことができます。キャプチャした変更に関するデータをクエリする方法の例については、「 [NerdGraph を使用して変更を追跡する](/docs/change-tracking/change-tracking-graphql/#query-events)」を参照してください。
...
さまざまな方法で変更をキャプチャして分析できます。[NerdGraph (GraphQL) API](/docs/change-tracking/change-tracking-graphql)を直接呼び出すか、 [New Relic CLI](/docs/change-tracking/change-tracking-cli)を使用するか、次の CI/CD プラグインのいずれかをインストールします。
...
変更をキャプチャしたら、それらを使用して、テレメトリ、パフォーマンス、および品質データのコンテキスト化を開始します。変更追跡データの利用とクエリの詳細について[は、New Relic での変更を表示および分析する方法 を参照してください](/docs/change-tracking/change-tracking-view-analyze/)。
