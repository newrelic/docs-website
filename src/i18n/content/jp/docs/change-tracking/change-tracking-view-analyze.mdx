...
* **APM とダッシュボードのグラフ:** 時系列グラフにピンヘッドの付いた縦線が表示されます。 <img title="A screenshot showing a deployment in a time series chart" alt="A screenshot showing a deployment in a time series chart" src={trackingDeploymentinTimeseriesChart}>このマーカーは記録された変更を表し、ピンの頭をクリックすると、変更がエンティティの健全性と品質に与えた影響を掘り下げることができます。</img>
