...
<Callout title="限定プレビュー">
  UI に青いバナーが表示されている場合は、新しい UI の限定プレビューに参加しているため、このページの手順を使用する必要があります。このバナーが表示されない場合は、これらの[配置マーカー UI の手順](/docs/apm/apm-ui-pages/events/change-tracking/record-deployments#dep_procedures)に切り替えてください。

  当社のサポートチームは、一般に入手可能な New Relic 製品をカバーしています。プレビュー段階の製品は、「一般提供」とは見なされません。プレビュー プログラムに参加するよう招待された場合、または限定リリース コンポーネントを使用している場合は、アカウント チームが質問の連絡先になります。アカウント担当者に直接お問い合わせください。
</Callout>
...
デプロイを記録する[手順](/docs/apm/apm-ui-pages/events/change-tracking/record-deployments)を完了すると、UI でそのパフォーマンスを確認する準備が整います。スタック全体からのデータによってコンテキスト化された結果を、さまざまな方法で確認できます。

* **APM とダッシュボードのグラフ:**時系列グラフにピンヘッドの付いた縦線が表示されます。このマーカーは、記録された展開イベントを表します。ピンの頭をクリックすると、変更がエンティティの正常性と品質に与えた影響を掘り下げることができます。<img title="A screenshot showing a deployment in a time series chart" alt="A screenshot showing a deployment in a time series chart" src={trackingDeploymentinTimeseriesChart}/>

  <Callout variant="tip">ダッシュボードまたはエンティティの概要ページに移動しても、予期した展開マーカーが表示されない場合は、タイム ピッカーの選択を確認してください。あなたが見ている窓の外にあるかもしれません。</Callout>

* **エンティティ サイドバー:**デプロイが記録されているエンティティのサイドバーで、\[**デプロイのプレビュー**] をクリックして、記録されたデプロイ イベントを表示するテーブルを表示できます。これはデプロイ リスト ページで、時間範囲セレクター、テーブルのフィルタリングおよび並べ替えオプションを備えています。これらを使用して、展開のフィールドを絞り込んだり、特定の展開を見つけたりします。
...
NerdGraph (New Relic GraphQL API) または NRQL (New Relic データベースのクエリ言語) を介してデプロイメント データをクエリすることもできます。詳細については、 [New Relic NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/)の[紹介](docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language/)および NRQL の紹介 を参照してください。

### NRQL [#nrql-markers]

これは、開始するための簡単な NRQL クエリです。これは、[クエリ ビルダー](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder)から実行できます。
...
以下は、 [NerdGraph エクスプローラー](/docs/apis/nerdgraph/get-started/nerdgraph-explorer#find)で実行できる NerdGraph クエリの例です。
