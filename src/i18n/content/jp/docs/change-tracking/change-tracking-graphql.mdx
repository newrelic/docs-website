    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            version 属性値には任意の値を指定できますが、ビルド プロセス中に設定されたバージョン番号を使用することをお勧めします。これを行う方法の例については、以下の[Jenkins デプロイメント プラグインのドキュメントを](#jenkins-markers)参照してください。すでにバージョン番号を New Relic に報告している場合<InlinePopover type="mobile"/>または、他の何らかの方法で、ここでも同じものを使用して、バージョンごとにデータをファセット化して関連付けることができる今後の機能を設定することをお勧めします。
          </td>
        </tr>
      </tbody>
    </table>
