...
変更追跡のデプロイを記録するには、特別な Jenkins プラグインが必要です。Jenkins インスタンスからプラグインを簡単に追加または削除できます。Jenkins 管理者として次の手順を実行します。

### プラグインを追加する [#add-plugin]

プラグインを追加するには、次の手順を実行します。

1. \[ **Jenkins の管理] を**クリックします。
...
4. **Deploy Plugin**で、 **Choose File**をクリックし、プレビュー用に提供されたファイル`newrelic-deployment-notifiier.hpi`をアップロードします。

### プラグインを削除する [#remove-plugin]

次の手順で、プラグイン マネージャーでプラグインを削除できます。
