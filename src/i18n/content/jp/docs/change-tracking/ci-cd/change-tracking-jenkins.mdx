...
追跡したい変更を記録するには、Jenkins インスタンスに簡単に追加または削除できる特別なプラグインが必要です。
...
6. インストールを完了するには、実行中のジョブがないことを確認してから、Jenkins を再起動します。**Restart Jenkins when installation is complete and no jobs are running** \[インストールが完了し、ジョブが実行されていないときに Jenkins を再起動する]チェックボックスをオンにすることもできます。<img title="Screenshot showing the box to restart Jenkins" alt="Screenshot showing the box to restart Jenkins" src={trackingRestartJenkins}/>
