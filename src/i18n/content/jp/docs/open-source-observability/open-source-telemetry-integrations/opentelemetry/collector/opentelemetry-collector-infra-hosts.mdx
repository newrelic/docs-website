...
コレクターのセットアップは、NewRelicでOpenTelemetryをセットアップするより大きなプロセスの一部です。以下のコレクターの手順を実行する前に、 [前述のセットアップ手順](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-quick-start)を完了していることを確認してください。
...
## ステップ1：OpenTelemetryコレクターをインストールする [#install-generic]
...
<Callout variant="important">
  インフラストラクチャ監視を設定するには、 `collector-contrib`リリースに含まれているコンポーネントをインストールして構成する必要があります。たとえば、ホストレシーバーは、CPU、メモリ、ディスク、ネットワーク統計などの基本的なホストメトリックを収集する必要があり、 [OpenTelemetryコレクターコントリビュート](https://github.com/open-telemetry/opentelemetry-collector-contrib)リリースでのみ使用できます。
</Callout>

## 手順2：ホストレシーバーを使用してホスト監視を構成する [#host-receiver]
...
* `host.id`属性は、ホストメトリックに存在します。
* `service.name`属性と`container.id`属性は、ホストメトリックに存在しません。
...
`config.yaml`を次の推奨パラメーターに適合させます。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * クラウド環境の場合、メトリックが`host.id` （New Relic Oneでホストエンティティを識別するために必要）で装飾されるように、特定のリソース検出プロセッサを構成します。一般的なクラウド検出器は、GCPマシンの場合は`gce` 、AWS EC2の場合はec2、 `ec2`の場合は`azure`です。オーケストレーションされた環境では、追加のプロセッサーを使用できます。

        * オンプレミスシステム（またはサポートされていないクラウド環境）の場合、 `host.id`属性が必要です。 `resource`プロセッサを使用して、 `host.name`値（ `system`から）を新しい`host.id`属性としてコピーします。この値は、インストルメントされたすべてのホストで一意である必要があることに注意してください。
