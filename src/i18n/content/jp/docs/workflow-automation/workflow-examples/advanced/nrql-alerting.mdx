...
* New Relicアカウント
* メールの送信先（[通知の送信を](/docs/workflow-automation/setup-and-configure/create-destinations)参照）
...
**スケジュールするには**、cron 式`*/10 * * * *` (10 分ごと) で[CreateSchedule API](/docs/workflow-automation/workflow-automation-apis/create-schedule)を使用します。最小間隔は10分です。詳細については、[ワークフローの制限を](/docs/workflow-automation/limitations-and-faq/workflow-limits)参照してください。

## 関連トピック [#related-topics]

<DocTiles>
  <DocTile title="デプロイメントのロールバック" path="/docs/workflow-automation/workflow-examples/advanced/deployment-rollback">
    デプロイメントワークフローでNRQLを使用する
  </DocTile>

  <DocTile title="EC2管理" path="/docs/workflow-automation/workflow-examples/advanced/ec2-management">
    NRQL書き込みとインフラストラクチャ自動化を組み合わせる
  </DocTile>

  <DocTile title="NRQLアクション" path="/docs/workflow-automation/setup-and-configure/actions-catalog/newrelic/query/newrelic-nrdb">
    NRQLアクション構文を学ぶ
  </DocTile>
</DocTiles>