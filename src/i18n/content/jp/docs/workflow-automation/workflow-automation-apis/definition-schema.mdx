...
ワークフロー定数は、現在実行中のワークフローに関するランタイム メタデータへのアクセスを提供します。 これらの定数はすべてのワークフロー定義で自動的に利用可能であり、式構文`${{ .workflowConstants.<constantName> }}`を使用して参照できます。
...
ワークフロー定数は、JQ式と組み合わせて値変換に使用できます。
...
タイムスタンプを`DateTime`アクションで変換しています
...
<Callout variant="important">
  * ワークフロー定数は読み取り専用であり、ランタイムによって自動的に提供されます。
  * `startedAt`タイムスタンプはミリ秒単位（エポック形式）です。
  * 定数はワークフロー全体を通して、すべての式コンテキストで使用できます。
  * `workflowInputs`とは異なり、定数はワークフロー定義で宣言する必要はありません。
</Callout>
...
ワークフローを終了し、カスタムメッセージで失敗を報告するステップ。`condition`が存在する場合、それが最初に検証され、条件が`true`の場合にのみワークフローが停止します。`message`を指定しない場合、デフォルトのメッセージ`failed as requested from step: failureStep`が使用されます。
...
ワークフロー定数、セレクター、待機ステップ、NRDB クエリ、スイッチ ステートメントなどの複数のワークフロー機能を示す完全なワークフロー例。
