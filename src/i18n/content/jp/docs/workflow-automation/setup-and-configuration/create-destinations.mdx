...
<Callout variant="important">
  以下の手順は、**Workflow Automation** \[ワークフロー自動化]ではなく、**分割通知**ワークフロー (New Relicアラートの一部) を設定するための手順です。 アラート通知ワークフローは、上記で構成した **Workflow Automation** \[ワークフロー自動化] の送信先を含む送信先に問題をルーティングします。多数ワークフローの完全なドキュメントについては、 [「](/docs/alerts/get-notified/alert-event-workflows)まとめイベント ワークフロー」を参照してください。
</Callout>
