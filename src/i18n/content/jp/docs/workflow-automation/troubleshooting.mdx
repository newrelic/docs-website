...
このページでは、ワークフロー自動化を使用する際に発生する可能性のある一般的な問題の解決策を提供します。追加のサポートについては、[New Relicサポートフォーラム](https://support.newrelic.com/s/)を参照するか、[New Relicサポート](https://support.newrelic.com/)にお問い合わせください。

## AWS の認証情報と認証

### アクセス拒否エラー [#access-denied-errors]
...
3. **権限を確認する:** IAM ロールまたはユーザーに、ワークフローに必要な特定の AWS 権限があることを確認します。[必要な権限の](/docs/workflow-automation/setup-and-configure/set-up-aws-credentials#required-permissions)表を確認してください。
...
2. **アクセス キーの場合:**アクセス キー ID とシークレット アクセス キーの両方が [シークレット マネージャー](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials)に正しく保存されていることを確認します。[NerdGraph GraphiQL エクスプローラー](https://one.newrelic.com/nerdgraph-graphiql)で値を再確認します。
...
**ここから始めましょう:**このクイック リファレンスを使用して、一般的な問題を診断します。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        そのアクションの設定、資格情報、または入力データを修正します
      </td>
    </tr>

    <tr>
      <td>
        ワークフローは実行されるが、間違った結果が生成される
      </td>

      <td>
        ステップ間のデータ受け渡しをチェックする
      </td>

      <td>
        テンプレートの構文を確認する `${{ .steps.name.outputs.field }}`
      </td>
    </tr>

    <tr>
      <td>
        スケジュールされたワークフローが実行されない
      </td>
      ...
      <td>
        スケジュール設定とワークフローの状態（アクティブ）を確認する
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        新しいバージョンを使用するようにスケジュールされた実行を更新する
      </td>
    </tr>
  </tbody>
</table>

ワークフローのステータスと実行履歴を表示する詳細な手順については、 [「ワークフローの表示」を](/docs/workflow-automation/manage-workflows/managing-workflow#view-workflows)参照してください。
...
1. 実行履歴を確認します。
   ...
   * ワークフロー名をクリック
   * **Run history** \[実行履歴]を表示

2. 実行ログを確認します。

   * 失敗した実行の**View logs** \[ログを表示を]クリックします
   * 失敗した具体的なアクションを特定する

**一般的な原因:**

* **無効な資格情報**- 期限切れのトークンまたは不正なシークレット構文
* **権限不足**- IAM権限またはAPIアクセスが不十分です
* **削除されたリソース**- EC2 インスタンス、Slack チャネル、データベースはもう存在しません
* **レート制限**- AWS 、Slack、その他のサービスへのrequestsが多すぎる
* **タイムアウト**- 外部 API が十分な速さで応答しない

**次のステップ:**問題を修正し、[ワークフローを手動でトリガーして](/docs/workflow-automation/workflow-automation-apis/start-workflow-run)動作することを確認します。
...
**問題:**機能コントロール管理 (FCM) を通じてワークフロー自動化をオフにし、クリーンなシャットダウンを確実に行う必要があります。

<Callout variant="important">
  **FCM をオフにする顧客の場合**: ワークフロー オートメーションを無効にする前に、今後スケジュールされたワークフローがすべて削除されていることを確認してください。 これにより、機能が無効にされた後の混乱や不要なアラートを防ぐことができます。
</Callout>

**解決策:**

1. **最初にスケジュールされたワークフローをすべて削除します**。FCM をオフにした場合は、混乱やアラートを防ぐために、今後スケジュールされるワークフローがすべて削除されていることを確認してください。

   * **All Capabilities &gt; Workflow Automation**へ移動
   * 各ワークフローを確認し、アクティブなスケジュールを確認します
   * スケジュールのあるワークフローをクリックすると詳細が表示されます
   * **Scheduled runs** \[スケジュール実行]タブに移動します
   * [DeleteSchedule API](/docs/workflow-automation/workflow-automation-apis/delete-schedule)を使用してすべてのアクティブなスケジュールを削除します

2. **すべてのスケジュールが削除されていることを確認する**: FCM を無効にする前に、アクティブなスケジュール実行がないことを確認してください。

3. **FCM を無効にする**: すべてのスケジュールが削除されたら、FCM 設定を通じてワークフロー自動化を安全に無効にすることができます。

<Callout variant="important">
  **スケジュールを削除する理由は何ですか?**FCM を通じてワークフロー オートメーションが無効になっている場合、スケジュールされたワークフローは実行できません。 アクティブなスケジュールがシステム内に残っていると、混乱が生じたり、不要なアラートが生成されたりする可能性があります。
</Callout>
...
4. **シークレット内のトークンを確認する**: [シークレット マネージャー](/docs/workflow-automation/limitations-and-faq/workflow-best-practices#secure-credentials)に保存されている Slack トークンが正しく、期限切れになっていないことを確認します。
...
1. **SSM 権限を確認する**: IAM ロール ポリシーに次の権限を追加します。
...
**Slack チャネルが見つかりません:**チャネル名ではなく、Slack チャネル**ID** (`C01234ABCD`) を使用してください。Slack のチャンネル詳細で見つけてください。

**AWS アクションがサイレントに失敗します:** AWS 認証情報に`ec2:StopInstances` 、 `ec2:ModifyInstanceAttribute` 、 `ec2:StartInstances` 、および`ssm:*`権限があることを確認してください。権限が不足している場合は、何も表示されずに失敗します。

**SSM がタイムアウトしたが、インスタンスはまだ移行中:** SSM がタイムアウトしても、EC2 インスタンスはまだ状態間で移行している可能性があります。 再実行する前に、 AWS Console実際のインスタンスのステータスを確認してください。

**リソースの漏洩と予期しない請求:**常にクリーンアップしてください。前のステップが失敗した場合でもクリーンアップが実行されるようにワークフローを構成します。これにより、リソースの漏洩や予期しない AWS 料金の発生を防ぐことができます。
...
2. **反応形式を確認してください**: ワークフローで指定された絵文字を正確に使用してください (例: 承認の場合は`thumbsup`)
...
1. **[CloudWatch Metric Streams](/docs/infrastructure/amazon-integrations/aws-integration-for-metrics/via-cloudformation-cwms/)が設定されていることを確認する**: リアルタイムのEC2メトリクスに必要です
...
4. **シンプルなエンドポイントでテストする**: 基本的な JSON エンドポイント ([https://httpbin.org/json](https://httpbin.org/json)など) を使用して、テンプレートの仕組みが機能することを確認します。
...
<DocTiles>
  ...
  <DocTile title="ワークフローの制限" path="/docs/workflow-automation/limitations-and-faq/workflow-limits">
    システムの制約とタイムアウトを理解する
  </DocTile>
</DocTiles>