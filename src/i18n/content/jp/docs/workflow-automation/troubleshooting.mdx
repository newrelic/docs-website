...
### Autopilot Slack RCA ワークフロー

**RCAコンテンツのない空のSlackメッセージ：** `analysisRaw`は応答を返しましたが、`analysisText`はコンテンツを抽出できませんでした — `finalAnswer`が予期された`.card.body`の構造ではありませんでした。セレクター`expression`が1行に記述されていること、および<DNT>Autopilot</DNT>のプロンプトが構造化されたカード応答を返すことを確認してください。正規のセレクターパターンについては、[AutopilotとSlackを使用した集計RCA](/docs/workflow-automation/workflow-examples/integrations/autopilot-slack-rca)をご覧ください。

**Slackの&quot;<DNT>Autopilot</DNT> RCA Failed&quot;メッセージ：** `analysisRaw`は空でした — <DNT>Autopilot</DNT>は`finalAnswer`を返しませんでした。`issueId`が有効でアクティブな集計課題UUIDであり、<DNT>Autopilot</DNT>のプロンプトが適切な形式であることを確認してください。

**ワークフローが入力検証で失敗します：** `issueId`と`slackDestinationId`は両方ともUUID形式である必要があります。集計ワークフローからトリガーする場合は、`{{ issueId }}`を使用してください。
