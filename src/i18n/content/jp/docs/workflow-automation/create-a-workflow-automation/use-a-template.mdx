...
<CollapserGroup>
  ...
  <Collapser id="nrlens-sql-query" title="NRLens SQL実行">
    **機能:**

    NRLens (高度な書き込みツール) を使用して、 New Relicのデータカタログに対して SQL 書き込みを実行します。

    **いつ使うのか:**

    NRQLを超えた複雑な分析：複数テーブル結合、統計分析、メトリクス/イベント/ログを組み合わせたカスタムレポート。

    **必要なもの:**

    * **NRLens** : SQL クエリ ゲートウェイの資格情報
    * **New Relic** : 組織とアカウントID
    ...
    **使い方：**
    ...
    <Callout variant="tip">
      **NRLens は NRQL を拡張します。**リアルタイム クエリには NRQL を使用し、複雑な分析ワークロードには NRLens SQL を使用します。
    </Callout>
  </Collapser>
</CollapserGroup>
