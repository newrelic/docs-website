...
<CollapserGroup>
  ...
  <Collapser id="aws.lambda.updateFunctionConfiguration" title="更新機能の設定">
  ...
  <Collapser id="aws.lambda.getFunction" title="特定のLambda関数を説明する">
    特定のLambda関数の設定の詳細、コードの場所、その他のメタデータを取得します。
  ...
  <Collapser id="aws.lambda.listAliases" title="特定のLambda関数のエイリアスを一覧表示する">
    特定のLambda関数のエイリアスのリストを返します。 エイリアスは関数バージョンへのポインタです。
  ...
  <Collapser id="aws.bedrock.invokeAgent" title="Bedrockエージェントを呼び出す">
    Bedrock エージェントを呼び出します。
    ...
    ### Bedrock エージェントを実行するための AWS ロールの例
