...
新しい APM エージェント バージョンでは、これらの機能の両方がデフォルトで有効になっています。

この機能の一方または両方を無効にすることができます。装飾やログ転送を無効にする理由については、[制限事項](#limitations)を参照してください。たとえば、APM エージェントに New Relic メタデータを追加させたい場合は、ログ装飾機能を使用できますが、ログ転送を無効にして、代わりに別のログ エージェント (たとえば、当社のインフラストラクチャ エージェント、またはサードパーティのログ エージェント) を使用します。 .
