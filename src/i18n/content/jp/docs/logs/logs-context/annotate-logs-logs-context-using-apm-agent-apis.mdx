...
ログ データを他のテレメトリ データ (エラーや APM の分散トレースなど) と関連付けるため[に、コンテキストで APM ログを](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)使用できます。ロギング フレームワークが既存のログ イン コンテキスト ソリューションでサポートされていない場合は、APM エージェント API を使用してロギング ライブラリを構成できます。
