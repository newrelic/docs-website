...
当社の PHP APM エージェントを使用すると、**ログをコンテキスト**で取得できます。これにより、他の New Relic データのコンテキストでアプリのログを確認できます。この機能の一般的な情報について[は、コンテキスト内の APM ログ を](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)参照してください。
