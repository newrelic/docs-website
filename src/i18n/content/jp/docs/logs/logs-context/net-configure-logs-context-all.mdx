...
<CollapserGroup>
  ...
  >
    ...
    `include`および`exclude`構成オプションを使用して、コンテキスト データをフィルタリングできます。これらは、属性名のコンマ区切りリストです。両方のデフォルト値は空の文字列です。空の`include`フィールドは、「すべてを含める」ことを意味します。これらの包含/除外オプションは、他の[エージェント属性構成オプション](docs/apm/agents/net-agent/attributes/enable-disable-attributes-net/#attruls)と同じ規則に従います。
