...
このガイドは、以前に[ログ転送](/docs/logs/forward-logs/enable-log-management-new-relic/)または[コンテキスト内のログを](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/)構成した顧客に適用されます。どちらの機能も設定していない場合は、最新バージョンにアップグレードすることをお勧めします。 <InlinePopover type="apm"/>エージェントは[自動ログ転送とローカル装飾](/docs/apm/new-relic-apm/getting-started/get-started-logs-context/)を利用します。
...
1. ログを自動的に転送する最新の[APM エージェント](/docs/new-relic-one/install-configure/update-new-relic-agent/)に更新してから、ログ フォワーダーを無効にします。
