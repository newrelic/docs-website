...
Java <InlinePopover type="apm"/>エージェントを使用すると、 <DoNotTranslate>**logs in context**</DoNotTranslate>を取得でき、他の New Relic データのコンテキストでアプリのログを表示できます。 この機能の一般的な情報については、 [「APM ログのコンテキスト」を](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)参照してください。
...
<CollapserGroup>
  ...
  >
    ...
    3. プロジェクトの依存関係を更新して、必要に応じてDropwizard1.3拡張機能を含めます。
       ...
       <DoNotTranslate>**Gradle:**</DoNotTranslate> `build.gradle`ファイルに以下を追加します。
       ...
       <DoNotTranslate>**Maven:**</DoNotTranslate> `pom.xml`ファイルに以下を追加します。
  ...
  >
    ...
    3. プロジェクトの依存関係を更新して、必要に応じてLog4j2.x拡張機能を含めます。
       ...
       <DoNotTranslate>**Gradle:**</DoNotTranslate> 強調表示されたセクションを`build.gradle`ファイルに追加します。
       ...
       <DoNotTranslate>**Maven:**</DoNotTranslate> 強調表示されたセクションを`pom.xml`ファイルに追加します。
  ...
  >
    ...
    6. `NewRelicAsyncAppender`がロガーで使用されている<DoNotTranslate>**first**</DoNotTranslate>アペンダーであることを確認してください。 ルート ロガーのアペンダーを、前の手順で作成した`ASYNC`アペンダーに置き換えます。 次に、 `<root>`リスト内の`NewRelicAsyncAppender`に<DoNotTranslate>**after**</DoNotTranslate>他のアペンダーを一覧表示します。
  ...
  >
    ...
    * 依存関係に`spring-boot-starter-log4j2`がある場合は、 <DoNotTranslate>**log4j 2.x**</DoNotTranslate>を使用しています。 基本的な設定については[Spring Boot log4j 2.x のドキュメント](https://docs.spring.io/spring-boot/docs/current/reference/html/howto.html#howto-configure-log4j-for-logging)を、設定のカスタマイズについては[New Relic log4j 2 拡張機能を](#java-log4j-2x)参照してください。

    * Spring Boot を使用しているものの、 `starter-log4j2`を使用して**いない**場合は、デフォルトで<DoNotTranslate>**logback**</DoNotTranslate>が使用されます。 基本的な設定については[Spring Boot logback のドキュメント](https://docs.spring.io/spring-boot/docs/current/reference/html/howto.html#howto-configure-logback-for-logging)を、設定のカスタマイズについては[New Relic logback 拡張機能](#logback)を参照してください。
