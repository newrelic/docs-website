...
<CollapserGroup>
  ...
  >
    ...
    ログ転送機能を備えたエージェントバージョン（ [Pythonエージェント7.12.0.176以降](https://docs.newrelic.com/docs/release-notes/agent-release-notes/python-release-notes/)）をインストールし、構成を更新して転送を有効にするだけです。
  ...
  >
    手動インストールオプションを使用して、Pythonで監視されているAPMアプリのコンテキストでログを有効にすることもできます。
    ...
    3. [](/docs/logs/enable-log-management-new-relic/enable-log-monitoring-new-relic/enable-log-management-new-relic/)
    ...
    4. ログハンドラーのコンテキストでログを構成します。言語エージェントがログを転送および装飾できるようになる前は、ログフォーマッターをインスタンス化してログハンドラーに追加することで、コンテキスト内でログを有効にすることができました。この例では、デフォルトでログを`sys.stderr`に書き込む`StreamHandler`を使用していますが、任意のハンドラーを使用できます。ログハンドラーの構成の詳細については、 [Python.orgのドキュメント](https://docs.python.org/3/library/logging.handlers.html)を参照してください。
