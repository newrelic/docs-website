...
Go 言語の<InlinePopover type="apm"/>エージェントを使用すると、 <DoNotTranslate>**logs in context**</DoNotTranslate>を取得できます。これにより、他の New Relic データのコンテキストでアプリのログを確認できるようになります。 この機能の一般的な情報については、 [「コンテキスト内の APM ログ」](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)を参照してください。
...
<CollapserGroup>
  ...
  >
    ...
    インストゥルメントがトランザクション内でログを記録する場合、そのトランザクションを nrslog に渡す必要があります。 これにより、エージェントによって処理されたログをトランザクションとログが発生したスパンにリンクする、そのトランザクション用の新しいロガー オブジェクトが作成されます。

    トランザクションを含むコンテキストがある場合:

    ```go
    txnLogger := nrslog.WithContext(nrContext, logger)
    ```

    それ以外の場合は、次のようにトランザクション オブジェクト ポインターを使用できます。

    ```go
    txnLogger := nrslog.WithTransaction(txn, logger)
    ```
  </Collapser>
</CollapserGroup>
