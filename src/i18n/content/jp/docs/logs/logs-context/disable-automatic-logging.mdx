...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="APIを介して管理"
  >
    NerdGraph API エクスプローラーで `dataManagementUpdateFeatureSettings` を使用します: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**。GraphiQL UI に入力された例と関連ドキュメントを参照してください。
