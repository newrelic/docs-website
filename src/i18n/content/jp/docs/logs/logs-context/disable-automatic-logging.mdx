...
デフォルトでは、<InlinePopover type="apm"/>エージェントはアプリケーションログを重要なNew Relic情報で自動的に装飾し、それらのログをNew Relicに転送します。 この機能の仕組みと、この機能が有効になっているエージェントのバージョンの詳細については、 [「APM ログ イン コンテキスト」を](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)参照してください。
...
APM エージェントで[高セキュリティ モードが](/docs/apm/agents/manage-apm-agents/configuration/high-security-mode)有効になっている場合、アプリケーション ログの転送が無効になることに注意してください。
...
<CollapserGroup>
  <Collapser
    ...
    title="UIを介して設定を管理する"
  >
    <DNT>**Data management**</DNT> UIの <DNT>**Log settings**</DNT> UIページでは、アカウントの[APMログ](/docs/apm/new-relic-apm/getting-started/get-started-logs-context)(ログ装飾とログ転送の両方) を有効または無効にできます。
    ...
    親アカウントがこの設定をロックしていない限り、子アカウント レベルでは、 <InlinePopover type="apm"/>ログ転送を有効または無効にすることができます。 親アカウントが APM ログ転送をロックすると、子アカウントは親アカウントの有効または無効設定を継承し、変更できなくなります。
    ...
    1. New Relic UI の[ユーザー メニュー](/docs/accounts/accounts-billing/general-account-settings/intro-account-settings)から、
  ...
  <Collapser
    ...
    title="APIを介して管理"
  >
    NerdGraph API エクスプローラーの`dataManagementUpdateFeatureSettings`を使用します: <DNT>**[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**</DNT> 。 GraphiQL UI に入力された例と関連ドキュメントを参照してください。
  ...
  >
    ...
    <Callout variant="important">
      ログ転送を無効にしても、 APM <DNT>**Summary**</DNT> ページの <DNT>**Logs**</DNT> チャートに表示されるログ記録メトリックには影響しないことに注意してください。 詳細については、[ログメトリック](/docs/logs/logs-context/get-started-logs-context#apm-logs-defined)を参照してください。
    </Callout>

    ログ転送設定オプションの詳細については、特定の<InlinePopover type="apm"/>エージェントの手順を参照してください。
...
<CollapserGroup>
  ...
  >
    API または UI 経由で自動ログ転送が無効になっている場合、デフォルトでログを送信できるすべての<InlinePopover type="apm"/>言語エージェントで、そのアカウントのログ記録が今後無効になります。
  </Collapser>
  ...
  >
    <InlinePopover type="apm"/>ログをオフにすると、ログ データは転送されなくなります。ログ検査のその他の側面は同じままです。 エージェント内のすべてのログ関連アクティビティを完全に無効にするには、エージェントの[クライアント側の設定](#disable-config)を変更してログを無効にします。
  </Collapser>
  ...
  >
    <InlinePopover type="apm"/>データの取り込みには変更はありませんが、ログ データの取り込みは削除されます。 New Relic でのデータ管理の詳細については、[データ取り込みドキュメント](/docs/data-apis/manage-data/manage-data-coming-new-relic)を参照してください。
  </Collapser>
</CollapserGroup>
