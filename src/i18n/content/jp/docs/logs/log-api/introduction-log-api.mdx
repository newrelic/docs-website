...
<CollapserGroup>
  <Collapser id="simple-json" title="簡易JSONボディメッセージ">
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        配列は文字列に変換されてから保存されます。OpenTelemetry ログは同種配列をサポートします。詳細については、 [OpenTelemetry の配列](/docs/nrql/using-nrql/arrays-in-nrql/)を参照してください。
      </td>
    </tr>
  </tbody>
</table>
...
* ペイロードの合計サイズ: <DNT>**1MB(10^6 bytes) maximum per POST**</DNT> 。 圧縮を使用することを強くお勧めします。
...
<CollapserGroup>
  <Collapser id="http-requests" title="毎分のHTTPリクエスト数">
  ...
  <Collapser id="json-bytes" title="JSONバイト/分">
