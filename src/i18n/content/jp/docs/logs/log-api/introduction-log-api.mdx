...
4. `Api-Key` または `License-Key` が [headers](#json-headers) または [query parameters](#query-parameters) に含まれていることを確認してください。 [ログのJSONの例を参照](#log-attribute-examples).
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        [Insights のインサートキー](/docs/apis/intro-apis/new-relic-api-keys/#insights-insert-key) を使用することもできますが、ライセンスキーの方が望ましいです。
      </td>
    </tr>
  </tbody>
</table>
...
## HTTPクエリパラメータ [#query-parameters]

ライセンスキーは、クエリ文字列パラメータとして渡すこともできます。これは、カスタムHTTPリクエストヘッダーを許可していないクラウドベースのソースからログを送信する場合に便利です。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        あなたの [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 。ヘッダーを送信する際には、必ずこのキーを使用してください。
