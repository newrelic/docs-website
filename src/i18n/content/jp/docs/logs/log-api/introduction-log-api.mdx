...
<InlinePopover type="licenseKey" />は、Log API へのリクエストを認証する役割を果たし、送信されたログ メッセージが書き込まれる New Relic アカウントを決定します。HTTP ヘッダーまたはクエリ文字列パラメーターとして渡す必要があります。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        New Relic<InlinePopover type="licenseKey" />。[クエリ パラメータを介してこれを送信](#query-parameters)することもできます。
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <InlinePopover type="licenseKey" />。[これを HTTP ヘッダー経由で送信](#auth-header)することもできます。
      </td>
    </tr>
  </tbody>
</table>
