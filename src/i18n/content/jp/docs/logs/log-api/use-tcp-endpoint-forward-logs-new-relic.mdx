...
6. (オプション): `syslog` データにカスタム属性を追加します。
   ...
   * <DoNotTranslate>**Add custom attributes directly in the template string.**</DoNotTranslate> 文字列値の二重引用符 ( `"` ) をエスケープする必要があります。 例えば：
   ...
   * <DoNotTranslate>**Add custom attributes using a custom property variable in the template string.**</DoNotTranslate> この方法では、文字列値をエスケープする必要がなくなります。
