...
<CollapserGroup>
  ...
  >
    ...
    ![New Relicのログをコンテキストダイアグラムで表示](./images/new-relic-logs-in-context-diagram.jpg "New Relicへのログ転送のライフサイクル")
...
[ログをNewRelicに転送するためのソリューションをいくつか提供しています。](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/)

### [インフラストラクチャエージェント（推奨） [#forward-infra]](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/)

[ガイド付きインストールプロセスを使用して、](/docs/logs/enable-log-management-new-relic/configure-logs-context/configure-logs-context-apm-agents/)[ログ管理とインフラストラクチャ監視をすばやく簡単に一緒にインストール](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent/)してください。これは、アプリのログデータ**と**ホストのログデータの両方をNewRelicに取り込むための最も簡単なソリューションです。[これがどのよう](#enable)に機能するかのビデオをご覧ください。

インフラストラクチャログ転送に自動的に含まれるホストログが必要ない場合、またはすでにログフォワーダーがある場合は、サポートされている他のオプションを使用して、ログをNewRelicに転送できます。
