...
<Callout variant="Recommendation">
  Firelens イメージの 1 つをタスク定義の一部としてデプロイするときに、イメージ タグを固定することをお勧めします。 これにより、イメージ バージョンのアップグレードをより制御できるようになります。 `newrelic/logging-firelens-fluentbit` ECR リポジトリで使用可能なイメージタグは、次のコマンドで一覧表示できます ( [AWS CLI が適切にインストールされ](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html)、設定されている必要があります)。
