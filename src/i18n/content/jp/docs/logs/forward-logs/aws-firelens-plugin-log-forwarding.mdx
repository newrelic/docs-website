...
シークレットを追加するときは、 <DNT>**Plaintext**</DNT>タブを使用します。 シークレットを Secrets Manager に追加したら、 `logConfiguration`ブロックを使用してシークレットを参照できます。 `SECRET_NAME`を AWS シークレットの名前に置き換えます。 例えば：
