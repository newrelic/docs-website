...
FireLens ログを New Relic に転送すると、ログ データの収集、処理、探索、クエリ、アラートを行うための強化された<InlinePopover type="logs"/>機能が提供されます。
...
<Callout variant="Recommendation">
  Firelens イメージの 1 つをタスク定義の一部としてデプロイするときに、イメージ タグをピン留めすることをお勧めします。これにより、イメージ バージョンのより制御されたアップグレードが可能になります。 `newrelic/logging-firelens-fluentbit` ECR リポジトリで使用可能なイメージタグは、次のコマンドで一覧表示できます ( [AWS CLI が適切にインストールされ](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) 、設定されている必要があります)。
...
タスク定義で New Relic <InlinePopover type="licenseKey"/>が公開されないようにするには、 [AWS Secrets Manager](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data-secrets.html)サービスを使用することを強くお勧めします。

シークレットを追加するときは、 <DoNotTranslate>**Plaintext**</DoNotTranslate>タブを使用します。 シークレットを Secrets Manager に追加したら、 `logConfiguration`ブロックを使用してシークレットを参照できます。 `SECRET_NAME`を AWS シークレットの名前に置き換えます。 例えば：
...
`logConfiguration`ブロックを使用し、 `INSERT_API_KEY` New Relic <InlinePopover type="licenseKey"/>に置き換えます。 設定の詳細については、 [GitHub の FireLens タスク定義を](https://github.com/aws-samples/amazon-ecs-firelens-examples)参照してください。
