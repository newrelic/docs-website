...
タスク定義で New Relic<LicenseKey/>が公開されないようにするには、 [AWS Secrets Manager](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data-secrets.html)サービスを使用することを強くお勧めします。
...
INSERT_API_KEY を New Relic<LicenseKey/>に置き換えて、 `logConfiguration`ブロックを使用します。構成の詳細については、 [GitHub の FireLens タスク定義を](https://github.com/aws-samples/amazon-ecs-firelens-examples)参照してください。
