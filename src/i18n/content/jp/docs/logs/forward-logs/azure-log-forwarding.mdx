...
3. **Logging** \[ロギング]で、 `Microsoft Azure Event Hub` タイルをクリックします。
...
<CollapserGroup>
  ...
  >
    ...
    3. **Allow Blob public access** \[Blob のパブリック アクセスを許可する] で、**Disabled** \[無効にする]を選択します。<img title="Disabling public access to the blobs/containers in the Storage account" alt="Disabling public access to the blobs/containers in the Storage account" src={disablePublicBlobAccessStorageAccount}/>
    4. クリック **保存**.
  </Collapser>
  ...
  >
    ...
    4. **Platform settings** \[プラットフォーム設定] の **FTP state** \[FTP 状態] で **、 Disabled** \[無効] を選択します<img title="Disabling FTP access to the Function App" alt="Disabling FTP access to the Function App" src={disableFtpAccessStorageAccount}>。</img>
    5. クリック **保存**.
  </Collapser>
