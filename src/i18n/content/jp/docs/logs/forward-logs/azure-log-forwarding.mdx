...
Azure EventHub のログを New Relic に転送することで、ログデータの収集、処理、探索、クエリ、アラートなどのログ管理機能を強化することができます。

## Azure Resources Manager のログ [#requirements]

New Relic One が提供する Microsoft Azure Resources Manager (ARM)テンプレートは、Azure の設定をサポートします。

- [EventHub](#application-logs) のログを New Relic One に転送します。
- [アクティビティログ](#activity-logs) を EventHub を通して New Relic One に転送する。

セットアッププロセスは、どちらのユースケースでもほぼ同じです。 セットアッププロセスの一環として、New Relic に転送する Azure アクティビティログ [を以下のように選択できます。](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log)

- - 管理用 Azure のアクティビティログ
...
[このテンプレートは冪等である。EventHub からのログ転送を開始してから、同じテンプレートを再実行して、ステップ 10 を完了することで Azure アクティビティログの転送を設定することができます。](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log)
...
4. **Log ingestion** の下で、 ![Microsoft Azureのデータソースボタン](src/images/azure-tile.png) タイルをクリックします。
...
11. オプションです。 `true` 転送したい [Azure のアクティビティログ](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log) に設定します。
12. **Review + create** をクリックして、挿入したデータを確認し、 **Create** をクリックします。

## ログデータの閲覧 [#find-data]

すべての設定が正しく行われ、データが収集されていれば、この 2 つの場所にデータログが表示されるはずです。
...
Azure から送られてくるログのみを照会したい場合は、次のようなクエリを実行します。
...
## Azure リソースからのログ送信 [#application-logs]

デフォルトでは、このテンプレートはログを New Relic One に転送するために必要な機能とリソースのみを設定します。アクティビティログを転送するように設定することもできますが、Azure リソースからのログ転送はデフォルトではありません。ログを生成する任意のリソースからログを転送したい場合は、指定されたリソースの診断設定を作成して設定する必要があります。

例えば、Azure 上で動作している機能があり、そのログを New Relic One に転送したい場合、ログを EventHub に転送するための診断設定を行う必要があります。詳しくは、 [Microsoft documentation to create diagnostic settings for sendig platform logs and metrics to different destination](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/diagnostic-settings?tabs=CMD) をご覧ください。

## Azure のアクティビティログ [#activity-logs]

Azure アクティビティログの転送を有効にすることはオプションです。提供するものです。
