...
<Callout variant="tip">
  Azure VNetフローログの転送をお考えですか？増分デルタ処理を使用した専用のVNetフローログ転送については、[Azure VNetフローログ監視のセットアップ](/docs/network-performance-monitoring/setup-performance-monitoring/cloud-flow-logs/azure-vnet-flow-log-monitoring)を参照してください。自動セットアップには、ARMおよびBicepデプロイメントテンプレートを使用できます。
</Callout>
...
11. オプション：<DNT>**Authentication Mode**</DNT>を選択します。デフォルトは`Local Authentication`です。セキュリティコンプライアンスのためにAzure環境でローカル認証（共有アクセスキー）が無効になっている場合は、`Managed Identity`を選択してください。`Managed Identity`に設定すると、Function Appにシステム割り当てマネージドIDが割り当てられ、イベント Hubネームスペースの`Azure Event Hubs Data Receiver`ロールが付与されます — 接続文字列は保存されません。詳細については、[認証モードの情報](#eventhub-authentication)を参照してください。

12. オプション: EventHub バッチ処理 (v2.8.0 以降で利用可能) を構成して、パフォーマンスを最適化します。
...
13. オプション：転送する[Azureサブスクリプションアクティビティログ](https://docs.microsoft.com/en-us/azure/azure-monitor/essentials/activity-log)を`true`に設定します。詳細については、このドキュメント[のサブスクリプション情報](#subscription-activity-logs)を参照してください。

14. <DNT>**Review + create**</DNT>をクリックし、挿入したデータを確認して、 <DNT>**Create**</DNT>をクリックします。
...
### オプション：認証モードを構成する [#eventhub-authentication]

バージョン3.2.0以降、ARMテンプレートでは、Function Appをイベント Hubに接続するための2つの認証方法がサポートされています：

* <DNT>**Local Authentication**</DNT> （デフォルト）：Function App は、アプリケーション設定に保存されている共有アクセス キー接続文字列を使用して接続します。これは従来のアプローチであり、以前のバージョンのテンプレートと一致します。

* <DNT>**Managed Identity**</DNT>：Function App は、イベント Hub ネームスペースの`Azure Event Hubs Data Receiver`組み込みロールを持つシステム割り当てマネージド ID を使用します。シークレットや接続文字列は保存されません。次の場合にこのオプションを使用します：

  * お客様の組織は、セキュリティコンプライアンスのために、イベント Hub ネームスペースでのローカル認証（共有アクセスキー）を無効にしました。
  * Azure AD によって完全に管理されるキーレス認証を希望する場合。

`Managed Identity`が選択されると、テンプレートは自動的に：

* Function App にシステム割り当てマネージド ID を作成します。
* ID にイベント Hub ネームスペースの`Azure Event Hubs Data Receiver`ロールを付与します。
* 接続文字列の代わりに、`EVENTHUB_CONSUMER_CONNECTION__fullyQualifiedNamespace`（イベント Hubネームスペースのホスト名）を使用してFunction Appを構成します。Azure Functions ランタイムは、認証に Azure AD トークンを自動的に使用します。
