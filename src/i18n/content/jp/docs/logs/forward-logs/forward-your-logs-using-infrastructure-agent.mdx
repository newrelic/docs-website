...
<CollapserGroup>
  ...
  >
    ...
    `attributes`構成パラメータの一般的な使用法の 1 つは、 `logtype`属性を指定することです。この属性により、New Relic のログ管理機能でサポートされる[組み込みの解析ルール](/docs/logs/log-management/ui-data/parsing/#built-in-rules)の 1 つを利用できるようになります。
