...
[ログを New Relic に報告する](/docs/logs/get-started/get-started-log-management#integrate-logs)オプションの 1 つは、当社のインフラストラクチャ監視エージェントを使用することです。これにより、すべてのログ データを 1 か所で利用できるようになり、アプリケーションとプラットフォームのパフォーマンス データの両方をより詳細に可視化できます。
...
New Relic ログ管理で使用できるすべてのオプションの概要については、ログの[概要 を](/docs/logs/get-started/get-started-log-management#integrate-logs)参照してください。

インフラストラクチャ エージェントを使用してログをレポートするには、主に 2 つのルートがあります。

* インフラストラクチャ エージェント、APM エージェントなどのインストールに使用できる軽量のインストール ルートである[ガイド付きインストール](/docs/infrastructure/host-integrations/installation/new-relic-guided-install-overview)を使用します。ガイド付きインストールは、小規模な組織や New Relic をテストしたい組織に適しています。
* インフラストラクチャ エージェントの標準的な手動インストールを実行します。これには、ガイド付きインストールよりも多くの作業が必要になりますが、より多くのパワーを引き出すことができます。

インフラストラクチャ監視を介してログ レポートを設定する手順は次のとおりです。

1. まだ[作成していない場合は、NewRelicアカウントを作成します](https://newrelic.com/signup)。それは永遠に無料です。
2. ログの構成に必要な[システム要件](#requirements)を確認します。
3. [インフラストラクチャ エージェントのバージョン 1.11.4 以降がインストールされ](#install)ていることを確認します。(UI でガイド付きインストール プロセスを使用する場合は、次の手順をスキップします。)
...
7. ログ UI で[ログ データを調べます](#what-next)。New Relic で、インフラストラクチャ エージェントによって報告されたログ データを探します。

ホストの UI のログの例を次に示します。選択した期間のイベントのコンテキストでログを表示し、強調表示された属性の詳細データにドリルダウンできます。さらに詳細なデータを調べるには、クエリを実行するか、\[**ログで開く] を**クリックします。
...
<figcaption>
  イベントのコンテキストにおけるホストのログの例を次に示します。
</figcaption>
