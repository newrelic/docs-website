...
4. お客様のアプリケーションに適切な **Log group** を選択してください。\* フィルターの名前を入力します。
5. オプションです。 [フィルターパターンを入力](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/FilterAndPatternSyntax.html).
6. **Enable trigger** のチェックボックスをチェックし、 **Add** をクリックしてトリガーを作成します。
