...
4. `fluent-bit.conf`ファイルの末尾に次を追加して、入力、フィルター、および出力スタンザを設定します。プレースホルダー テキストを[New Relic ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)に置き換えます。
