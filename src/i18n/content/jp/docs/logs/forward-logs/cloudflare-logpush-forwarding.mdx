...
7. New Relic の**Logs Endpoint**を入力します。`<NR_LICENSE_KEY>`を[New Relic ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)に置き換えます。
   ...
   * 複数のCloudflareゾーンがあると、ゾーンごとに新しいライセンスキーが必要になる場合があります。
...
* `destination_conf`：エンドポイントURL、ライセンスキー、および次の文字列形式の形式で構成されるログの宛先：
  ...
  * `<NR_LICENSE_KEY>`: [UI または API から取得](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key)できる New Relic ライセンス キーに置き換えます。
