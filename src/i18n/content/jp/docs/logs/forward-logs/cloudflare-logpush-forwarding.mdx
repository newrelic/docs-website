...
7. New Relic の**Logs Endpoint**を入力します。`<NR_LICENSE_KEY>`を<InlinePopover type="licenseKey" />に置き換えます。
   ...
   * 複数のCloudflareゾーンがあると、ゾーンごとに新しい
...
* `destination_conf`: エンドポイント URL、<InlinePopover type="licenseKey" />、および次の文字列形式の形式で構成されるログの宛先。
  ...
  * `<NR_LICENSE_KEY>`: [UI または API から取得](/docs/apis/intro-apis/new-relic-api-keys/#manage-license-key)できる New Relic
