...
<CollapserGroup>
  <Collapser id="ingest-url" title="1.GCP Pub/SubのインジェストURLを生成します。">
  ...
  <Collapser id="create-gcp-topic" title="2.GCP Pub/Sub Topicを作成します。">
    ...
    2. <DNT>**Create Topic**</DNT>をクリックします。
    3. 意味のある<DNT>**Topic ID**</DNT>を入力し、必要に応じて他のオプションを構成します。
    4. <DNT>**Create Topic**</DNT>をクリックします。
  </Collapser>

  <Collapser id="configure-gcp-log-forwarding" title="3.New Relic にログを転送するための GCP Pub/Sub Topic を用意する。">
  ...
  <Collapser id="forward-logs" title="4.ログをNew Relicに転送するためのルーティングシンクを作成します。">
    ...
    2. <DNT>**Create Sink**</DNT>をクリックします。
    3. <DNT>**Sink name**</DNT>と<DNT>**Sink description**</DNT>を入力し、 <DNT>**Next**</DNT>をクリックします。
    4. <DNT>**Select sink service**</DNT>の下で<DNT>**Cloud Pub/Sub topic**</DNT>を選択し、以前に作成した[Pub/Sub トピック](#create-gcp-topic)を選択します。
    5. 必要に応じて残りのフィルターを設定し、 <DNT>**Create sink**</DNT>をクリックしてセットアップを完了します。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  <Collapser id="dataflow-login-gcp" title="1.Google Cloud Platformのアカウントにログインします。">
  ...
  <Collapser id="dataflow-repo" title="2.DataflowTemplatesのGitHubリポジトリをクローンします。">
  ...
  <Collapser id="dataflow-configuration" title="3.Dataflow forwarderをコンパイルして実行します。">
  ...
  <Collapser id="dataflow-bucket" title="4.Dataflowテンプレート用のGCPバケットを作成します。">
  ...
  <Collapser id="dataflow-account" title="5.サービスアカウントを作成します。">
    以下のコマンドを実行します。
  ...
  <Collapser id="dataflow-compile" title="6.PubsubToNewRelicテンプレートをコンパイルして公開します。">
    次のコマンドを実行します。
  ...
  <Collapser id="dataflow-job" title="7.テンプレートをDataflowジョブとして実行します。">
    ...
    * ログの送信に使用された New Relic<InlinePopover type="licenseKey" />

    デフォルトでは、このコマンドは米国のデータセンターにログを送信します。EUまたは日本のアカウントについては、以下のセクションをご覧ください。

    <CollapserGroup>
      <Collapser id="eu-customers-gcp-dataflow" title="ヨーロッパおよび日本地域のアカウント">
        ヨーロッパおよび日本のアカウントでは、適切なエンドポイントとともに`logsApiUrl`パラメーターも追加する必要があります。
  ...
  <Collapser id="dataflow-config" title="8.オプションです。Dataflowのログフォワーダジョブをチューニングします。">
