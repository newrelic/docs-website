...
6. <DNT>**HTTP endpoint URL**</DNT>の下で、<DNT>**New Relic logs - US**</DNT>を選択します。これは米国のエンドポイント（デフォルト）です（[https://aws-api.newrelic.com/firehose/v1](https://aws-api.newrelic.com/firehose/v1)）。EUのエンドポイントを使用するには、この手順の残りのステップを完了してから、[EUアカウントの設定](#configure-eu-stream)に進んでください。
...
## EUまたはJPアカウントにログを送信するようにストリームを構成する [#configure-eu-stream]

EUまたはJPデータセンターにNew Relicアカウントをお持ちの場合：
...
5. お使いの地域に適切なFirehoseエンドポイントに<DNT>**HTTP endpoint URL**</DNT>の値を更新します：
