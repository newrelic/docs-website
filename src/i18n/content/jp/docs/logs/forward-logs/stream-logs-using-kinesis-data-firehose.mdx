...
CloudWatch Logs や Kinesis ストリームに対応した他のログを New Relic に転送することで、ログデータの収集、処理、探索、問い合わせ、アラートなどのログ管理機能を強化することができます。

## New Relic の配信ストリームを作成します。 [#create-delivery-stream]

Kinesis Data Firehose から New Relic にログを転送するために。

1. [New Relic のライセンスキーがあることを確認してください](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 。

2. 手順に従い、 [Amazon Kinesis Data Firehose の配信ストリームを作成](https://docs.aws.amazon.com/firehose/latest/dev/basic-create.html) 、配信先として New Relic を選択します。AWS マネジメントコンソールで、 **Amazon Kinesis** にアクセスします。
...
5. 送信先を選択する際、 **サードパーティパートナー** をクリックし、 **New Relic をクリックします。**
   ...
   ![サードパーティパートナー - AWS Kinesis Firehose](src/images/3rd-party-firehose.png 'サードパーティパートナー - AWS Kinesis Firehose')

6. **HTTP エンドポイント URL** の下で、ドロップダウンから **New Relic logs - US** を選択します。

   **注**: EU にログを送信するには、このセクションの残りの手順を完了してから、 [EU アカウントの設定手順](#configure-eu-stream) に進んでください。

7. [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) を **API キー** フィールドに貼り付けます。

8. 残りの設定を行い、確認します。

## EU のアカウントにログを送信するようにストリームを設定する [#configure-eu-stream]

EU のデータセンターで New Relic アカウントをお持ちの方。
...
5. **HTTP エンドポイントの URL** の値を EU アカウント用の Firehose エンドポイントに更新してください。
