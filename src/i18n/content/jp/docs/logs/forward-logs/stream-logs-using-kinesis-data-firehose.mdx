...
2. 手順に従って[Amazon Kinesis Data Firehose 配信ストリームを作成し](https://docs.aws.amazon.com/firehose/latest/dev/basic-create.html)、送信先として New Relic を選択します。AWS マネジメントコンソールで、 <DoNotTranslate>**Amazon Kinesis**</DoNotTranslate>に移動します。
3. <DoNotTranslate>**Kinesis Data Firehose**</DoNotTranslate>を選択し、 <DoNotTranslate>**Create delivery stream**</DoNotTranslate>をクリックします。
4. ストリームの名前を入力し、データソースを選択します。
5. 宛先を選択する場合は、 <DoNotTranslate>**Third-party partner**</DoNotTranslate>クリックしてから <DoNotTranslate>**New Relic.**</DoNotTranslate>
6. <DoNotTranslate>**HTTP endpoint URL**</DoNotTranslate>で、 <DoNotTranslate>**New Relic logs - US**</DoNotTranslate>を選択します。 これは米国のエンドポイント ( [https://aws-api.newrelic.com/firehose/v1](https://aws-api.newrelic.com/firehose/v1) ) です。 EU インジケーターを使用するには、この手順の残りのステップを完了してから、 [EU アカウント設定](#configure-eu-stream)に進んでください。
7. <InlinePopover type="licenseKey"/>を<DoNotTranslate>**API key**</DoNotTranslate>フィールドに貼り付けます。
8. <DoNotTranslate>**Content encoding**</DoNotTranslate>が`GZIP`に設定されていることを確認してください。
9. `Buffer hints/Buffer size`の下に`1 MiB`の<DoNotTranslate>**buffer size**</DoNotTranslate>を設定します。 (デフォルトでは、 `Buffer hints`セクションが折りたたまれていることに注意してください。 詳細については、 [「バッファ設定」セクション](#buffer-settings)を参照してください。
...
2. ストリームが作成されたら、 [Amazon Kinesis コンソール](https://console.aws.amazon.com/kinesis/)に移動し、 <DoNotTranslate>**Delivery streams**</DoNotTranslate>をクリックします。

3. 新しく作成した配信ストリームの名前をクリックし、 <DoNotTranslate>**Configuration**</DoNotTranslate>タブを選択します。

4. <DoNotTranslate>**Destination settings**</DoNotTranslate>セクションの横にある<DoNotTranslate>**Edit**</DoNotTranslate>をクリックします。

5. EU アカウントの Firehose エンドポイントの<DoNotTranslate>**HTTP endpoint URL**</DoNotTranslate>の値を更新します。

   ```
   https://aws-api.eu.newrelic.com/firehose/v1
   ```

6. <DoNotTranslate>**Save changes**</DoNotTranslate>をクリックして、ストリームが更新されていることを確認します。
