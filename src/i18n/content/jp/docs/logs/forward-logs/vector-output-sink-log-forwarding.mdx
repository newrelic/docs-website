...
3. `vector.toml`ファイル (デフォルトでは`/etc/vector`にあります) にスニペットを追加し、YOUR_LICENSE_KEY を New Relic [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)に置き換えます。
