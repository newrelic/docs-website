...
Vector ログを New Relic に転送すると、機能が強化されます<InlinePopover type="logs"/>ログ データの収集、処理、探索、クエリ、アラートを行う機能。
...
3. `vector.toml`ファイル (デフォルトでは`/etc/vector`にあります) にスニペットを追加し、 `YOUR_LICENSE_KEY` New Relic に置き換えます<InlinePopover type="licenseKey"/> :
