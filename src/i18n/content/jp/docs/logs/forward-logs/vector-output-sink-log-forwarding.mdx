...
Vector ログをNew Relicに転送すると、ログ データの収集、処理、探索、書き込み、および大量の<InlinePopover type="logs"/>機能が強化されます。
...
3. `vector.toml`ファイル (デフォルトでは`/etc/vector`にあります) にスニペットを追加し、 `YOUR_LICENSE_KEY`を New Relic <InlinePopover type="licenseKey"/>に置き換えます。
