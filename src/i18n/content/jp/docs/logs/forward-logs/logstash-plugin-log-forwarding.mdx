...
Logstash ログをNew Relicに転送すると、ログ データの収集、処理、探索、書き込み、集計の機能が強化されます。<InlinePopover type="logs"/>
...
2. `logstash.conf`ファイルに、次のデータ ブロックを追加します。 必ずプレースホルダー テキストを New Relic <InlinePopover type="licenseKey"/>に置き換えてください。
