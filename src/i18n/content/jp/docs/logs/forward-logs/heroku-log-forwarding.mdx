...
Heroku ログを New Relic に転送すると、機能が強化されます<InlinePopover type="logs"/>ログ データの収集、処理、探索、クエリ、およびアラートを実行する機能。
...
3. ドレイン トークンを削除するには、以下のコマンドを実行します。`YOUR_NR_LICENSE_KEY` (/docs/apis/intro-apis/new-relic-api-keys/#license-key)、 `YOUR_NR_ACCOUNT_ID` (/docs/accounts/accounts-billing/account- Structure/) の値を必ず更新してください。 account-id/)、および最後のステップで取得した`herokuMappingId` :
