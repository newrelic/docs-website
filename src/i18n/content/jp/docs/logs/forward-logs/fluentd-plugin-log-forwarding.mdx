...
Fluentd ログを New Relic に転送すると、機能が強化されます。 <InlinePopover type="logs"/>ログ データの収集、処理、探索、クエリ、およびアラートを実行する機能。
...
1. `fluent.conf`ファイル、または td-agent を使用している場合は`td-agent.conf`に、次のデータ ブロックを追加し、プレースホルダー テキストを
