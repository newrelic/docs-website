...
4. 該当する場合は、[EUまたは日本のエンドポイントを設定](#eu-configuration)します。
...
## EUまたは日本アカウント用のFluentd プラグインを構成する [#eu-configuration]

デフォルトでは、FluentdプラグインはログをUSエンドポイントに転送します：`https://log-api.newrelic.com/log/v1`。New Relic組織が[EUまたは日本のデータセンター](/docs/accounts/accounts-billing/account-setup/choose-your-data-center)にある場合は、手動で`base_uri`プロパティを適切なエンドポイントに設定する必要があります：

* EU： `https://log-api.eu.newrelic.com/log/v1`
* 日本： `https://log-api.jp.nr-data.net/log/v1`

たとえば、ログをEUエンドポイントに転送するには：
...
日本のアカウントの場合は、`<match>`ブロック内の`base_uri`を日本のエンドポイントに置き換えます：
...
## UTF-16LE から UTF-8 への変換を構成する [#configure-utf-16le]
...
Shift-JIS（CP932）でエンコードされた日本語のログファイルは、[必須のUTF-8エンコーディング](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#general)に変換され、ログデータをNew Relicにインポートして送信するために必要です。
