...
2. オプションで、[`newrelic-logging` リポジトリ](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration) [で利用可能な多数の設定オプションを使用して、ガイド付きインストール](/install/kubernetes) でインストレーションをさらに調整できます。ただし、ほとんどのユーザーにとって有効なので、標準設定をお勧めします。
   ...
   <Callout variant="important">
     New Relic<InlinePopover type="licenseKey" />を保存するために[Kubernetesシークレットを使用している](https://github.com/newrelic/helm-charts/blob/master/charts/newrelic-logging/values.yaml#L8-L25)場合、`newrelic-logging`チャートはデフォルトで米国のAPIエンドポイントにログを送信します。ライセンスキーがEU、日本、またはFedRAMPアカウントに属しており、キーの保存にシークレットが使用されている場合は、[APIリファレンスドキュメント](/docs/logs/log-api/introduction-log-api/#endpoint)の適切な値を使用してエンドポイント設定を更新する必要があります。EUおよび日本のアカウントに対してこれを設定する方法の例を次に示します：
...
<DocTiles>
  <DocTile title="ログUI" path="/docs/logs/ui-data/use-logs-ui/">
    ログUIを使用してプラットフォーム全体のログデータを探索します
  </DocTile>

  <DocTile title="Logs in Context" path="/docs/logs/logs-context/logs-in-context/">
    コンテキスト内のログ機能を使用してログを転送することで、アプリケーションとプラットフォームのパフォーマンスデータの両方をより深く把握できます。
  </DocTile>

  <DocTile title="アラート" path="/docs/alerts/overview/">
    最も重要な問題について最新情報を入手するためのアラートを作成する
  </DocTile>

  <DocTile title="ダッシュボードを作成する" path="/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/">
    見たい特定のデータを収集してグラフ化する方法をご覧ください
  </DocTile>
</DocTiles>