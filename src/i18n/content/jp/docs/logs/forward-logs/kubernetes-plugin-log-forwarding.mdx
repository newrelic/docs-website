...
1. [このページ](/install/kubernetes) New RelicKubernetesの手順に従って、 インテグレーションをインストールします。このインテグレーションには、ログ用のKubernetesプラグインが含まれています。

2. オプションとして、[ガイド付きインストールのステップ 4](/install/kubernetes)で、[newrelic-logging リポジトリ](https://github.com/newrelic/helm-charts/tree/master/charts/newrelic-logging#configuration)で利用可能な多数の設定オプションを使用して、インストレーションをさらに調整することもできます。 ただし、ほとんどのユーザーにとって有効なので、標準設定をお勧めします。
