...
データ パーティションをプログラムで管理したい場合は、NerdGraph API エクスプローラーを使用できます: **[one.newrelic.com](https://one.newrelic.com) > All capabilities > Apps > NerdGraph API explorer**。[NerdGraph データ パーティションのチュートリアルでは、](/docs/apis/nerdgraph/examples/nerdgraph-data-partition-rules-tutorial/) この API を使用してデータ パーティションをクエリ、作成、削除する方法を示します。
