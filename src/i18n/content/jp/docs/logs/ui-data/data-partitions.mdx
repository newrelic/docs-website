...
## パーティションへのアクセスを制御する [#data-access-control]

ログ データを整理することに加えて、パーティションは特定のログ データへのユーザー アクセスを制御するための基盤としても機能します。[データ アクセス制御](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control)を使用すると、特定のユーザー グループに対して特定のパーティションへのアクセスを制限または許可するポリシーを作成できます。

これは次の場合に役立ちます:

* チームが自分のサービスに関連するログのみを参照できるようにする必要があります。
* 機密ログ データへのアクセスを制限するコンプライアンス要件があります。
* 個人を特定できる情報 (PII) を含むログへの不正アクセスを防止したい。