...
<Steps>
  <Step>
    ...
    1. ログ テーブルまたはログ詳細ビューの**Formatted** \[フォーマット済み] タブで、抽出する値を含むアンカー文字列を強調表示します。 この場合は、 `level=info msg="Running script"`強調表示します。 ログ テーブルでは次のようになります。<img title="Screenshot showing a highlighted section of a log" alt="Screenshot showing a highlighted section of a log" src={logsExampleLogRecordtoHighlight}/>
    ...
    2. **Create query time parsing rule** \[クエリ時間解析ルールの作成]オプションをクリックします。
       ...
       **ログテーブル:**<img title="Screenshot showing how you can begin with the logs table UI" alt="Screenshot showing how you can begin with the logs table UI" src={logsLogsTableEntryPoint}/>

       **ログ詳細ビュー**:<img title="Screenshot showing how you can start creating rules in the logs detail view" alt="Screenshot showing how you can start creating rules in the logs detail view." src={logsLogsDetailEntryPoint}/>
  </Step>
