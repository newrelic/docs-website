...
ルックアップ テーブルは、 [UI](#lookup-ui)または[ルックアップ サービス API](#lookups-service-api)を介して追加および管理できます。
...
### ルックアップ サービス API の使用 [#lookups-service-api]

あるいは、 [Lookups Service API](/docs/apis/lookups-service-api/lookups-service-api)を使用してテーブルを追加および管理することもできます。Lookups Service API は、CSV ファイルおよび JSON オブジェクトとしてのルックアップ テーブルのアップロードをサポートしています。
