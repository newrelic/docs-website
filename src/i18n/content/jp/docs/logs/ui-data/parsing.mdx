...
<CollapserGroup>
  ...
  >
    ...
    その場合は、 `json` [grok type](#grok-syntax)を使用できます。これは、grok パターンによってキャプチャされた JSON を解析します。`json` [grok type](#grok-syntax)を使用すると、適切にフォーマットされていないログから JSON を抽出して解析できます。たとえば、ログに日付/時刻の文字列がプレフィックスとして付けられている場合:
    ...
    * `json({"dropOriginal": true})`: 元の属性を削除します。`true` (デフォルト値) に設定すると、解析ルールは元の JSON 属性を削除します。
