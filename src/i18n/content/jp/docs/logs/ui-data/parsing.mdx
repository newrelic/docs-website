...
<CollapserGroup>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              JSON 構造化データ。詳細については[、プレーン テキストと混合した JSON の解析](#parsing-json)を参照してください。
            </td>
          </tr>
        </tbody>
      </table>
  ...
  >
    New Relic ログ パイプラインは、デフォルトでログ JSON メッセージを解析しますが、JSON ログ メッセージがプレーン テキストと混在している場合があります。この状況では、それらを解析し、JSON 属性を使用してフィルタリングできるようにすることも必要です。
