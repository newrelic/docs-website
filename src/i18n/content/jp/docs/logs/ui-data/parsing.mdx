...
[解析パターンは、ログメッセージを解析するための業界標準であるGrokを用いて指定されます。](/docs/using-new-relic/data/understand-data/query-new-relic-data)

[Grokは、正規表現のスーパーセットで、リテラルの複雑な正規表現の代わりに使用できる、組み込みの名前付きパターンを追加しています。例えば、 `(?:[+-]?(?:[0-9]+))` といった正規表現で整数にマッチすることを覚えておかなくても、 `%{INT} と書けば、同じ正規表現を表す Grok パターン INT を使用することができます。` と書けば、同じ正規表現を表すGrokパターン `INT` を使用することができます。マッチング文字列には、常に正規表現とGrokパターン名を組み合わせて使用することができます。](/docs/using-new-relic/data/understand-data/query-new-relic-data)

[Grokのパターンには構文があります。](/docs/using-new-relic/data/understand-data/query-new-relic-data)

[`%{pattern_name[:optional_extracted_attribute_name[:optional_type]]}。`](/docs/using-new-relic/data/understand-data/query-new-relic-data)

[どこで？](/docs/using-new-relic/data/understand-data/query-new-relic-data)

* [](/docs/using-new-relic/data/understand-data/query-new-relic-data)
* [`PATTERN_NAME` は、 ](/docs/using-new-relic/data/understand-data/query-new-relic-data)[Grok patterns](https://grokdebug.herokuapp.com/patterns) の一つです。 `grok-patterns` をクリックすると、よく使われているパターンが表示されます。パターン名は、正規表現を表す単なるユーザーフレンドリーな名前です。これらは、対応する正規表現と完全に一致します。
...
## [ログタイプ別の整理 [#logtype]](https://newrelic.com/blog/how-to-relic/how-to-use-grok-log-parsing)

[New Relic のログ・インジェスチョン・パイプラインは、ログイベントをログの解析方法を記述したルールにマッチさせることで、データを解析することができます。ログイベントを解析する方法は2つあります。](https://newrelic.com/blog/how-to-relic/how-to-use-grok-log-parsing)

* [](https://newrelic.com/blog/how-to-relic/how-to-use-grok-log-parsing)
* [](https://newrelic.com/blog/how-to-relic/how-to-use-grok-log-parsing)[組み込みルール](#built-in-rules) を使用します。
