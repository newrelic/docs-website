...
<CollapserGroup>
  ...
  >
    UNIX <DoNotTranslate>**epoch**</DoNotTranslate>タイムスタンプは、1970 年 1 月 1 日の 00:00:00 UTC から経過した秒数 (またはミリ秒) に基づいています。 `Fri, 03 Nov 2023 16:06:32 GMT`の 2 つの形式の例は次のとおりです。
...
<CollapserGroup>
  ...
  >
    次のような JSON ログを、 `timestamp`を<DoNotTranslate>**epoch**</DoNotTranslate>形式で受信した場合 (ミリ秒精度なし):
  ...
  >
    次のような<DoNotTranslate>**ISO8601**</DoNotTranslate>形式のタイムスタンプを持つ JSON ログを受信した場合:
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**ISO8601**</DoNotTranslate>形式のタイムスタンプを含むこのログメッセージ:
  ...
  >
    ...
    <DoNotTranslate>**epoch**</DoNotTranslate>形式のタイムスタンプを含む次のログメッセージ:
