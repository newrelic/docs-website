...
* デフォルトでは、New Relic はログが取り込まれると同時に、ログ`timestamp`属性 (予約済みキーワード) の値を割り当てます。これは取り込み時と呼ばれます。
* ログにタイムスタンプがない場合、New Relic は取り込み時にタイムスタンプを割り当てます。
...
1. サポートされている時間形式のいずれかで`timestamp`属性を含む[JSON 形式のログ](#timestamps_in_json_logs)を受信した場合、取り込みタイムスタンプ値がオーバーライドされます。代わりに、JSON 属性のタイムスタンプ値が表示されます。
...
<CollapserGroup>
  ...
  >
    Unix**epoch** \[エポック]タイムスタンプは、1970 年 1 月 1 日の 00:00:00 UTC から経過した秒数 (またはミリ秒) に基づいています。`Fri, 03 Nov 2023 16:06:32 GMT`の 2 つの形式の例は次のとおりです。
...
`timestamp`属性を持つ JSON 形式のログを受信した場合、デフォルトの取り込み値`timestamp`を元の JSON 値でオーバーライドします。タイムスタンプ フィールドがない場合、またはフィールドの形式が正しくない場合、メッセージはデフォルトで取り込みタイムスタンプになります。48 時間より古いタイムスタンプを持つペイロードはドロップされることに注意してください。
...
* 有効な JSON メッセージの場合は、「message」JSON フィールド内。当社のログ管理機能は、あらゆるメッセージ属性を JSON として解析します。解析されたメッセージ内の結果の JSON 属性がログに追加されます。
...
<CollapserGroup>
  ...
  >
    次のような JSON ログを`timestamp`で**epoch** \[エポック]形式 (ミリ秒精度なし) で受信した場合:
  ...
  >
    **ISO8601**形式のタイムスタンプを持つこのような JSON ログを受信した場合、次のようになります。
...
`datetime` Grok タイプを使用すると、サポートされていない形式のタイムスタンプを解析できます。これを行うには、ログのタイムスタンプ形式と一致する`pattern`を持つ`datetime` Grok タイプを含める必要があります。`pattern`は、[ここに](https://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html)ある Java の単純な日付と時刻のパターンを使用する必要があります。Grok 式がサポートされていないタイムスタンプ形式をサポートされている形式に変換する方法を示す次の例を取り上げます。
