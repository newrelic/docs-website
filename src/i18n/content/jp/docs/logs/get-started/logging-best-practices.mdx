...
* ログを転送する場合は、[New Relic Infrastructureエージェント](/docs/logs/forward-logs/forward-your-logs-using-infrastructure-agent)および/または[APMエージェント](/docs/apm/new-relic-apm/getting-started/get-started-logs-context#agents)を使用することをお勧めします。New Relicエージェントを使用できない場合は、他の対応エージェント（FluentBit、Fluentd、Logstashなど）を使用してください。
  ...
  * [Logstashの例](https://github.com/newrelic/logstash-examples)
    ...
    <Callout variant="important">
      ログが基盤となるホスト/コンテナのディレクトリに保存され、ログを収集するためにインフラストラクチャ エージェントによってインストルメント化されている場合、インフラストラクチャ エージェントと APM エージェントの両方によって収集された重複したログが表示されることがあります。重複したログを送信しないようにするには、[このガイド](/docs/logs/logs-context/upgrade-to-automatic-logs-context)の推奨事項を参照してください。
    </Callout>
...
* **ログ分析**および/または**APMログ監視クイックスタート**ダッシュボードをインストールして、ログデータに関するインサイトを迅速に取得できます。これらのダッシュボードを追加するには、**[one.newrelic.com](https://one.newrelic.com) > Add Data > Logging > Dashboards**に移動します。
