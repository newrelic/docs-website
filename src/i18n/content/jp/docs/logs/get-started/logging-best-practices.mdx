...
## ログデータアクセスのセキュリティ保護 [#securing-log-data]

機密情報が含まれる可能性のあるログ データを扱う場合、特定のログにアクセスできるユーザーを制御することが重要です。 ログ データを保護するためのベストプラクティスは次のとおりです。

* **データ アクセス制御の使用**:[データ アクセス制御](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control)ポリシーを実装して、特定の[ログ パーティション](/docs/logs/ui-data/data-partitions/)を表示できるユーザーを制限します。これは、ログに個人を特定できる情報 (PII)、セキュリティ データ、またはビジネス上の機密情報が含まれている場合に特に重要です。

* **データ漏洩のリスクに注意してください**: 特定の機能により、データ アクセス制限を回避できる場合があります。[潜在的なデータ漏洩リスク](/docs/accounts/accounts-billing/new-relic-one-user-management/data-access-control#data-exposure-risks)を確認し、次のベストプラクティスに従ってください。

  * <DNT>**Data partitions rules**</DNT>機能への<DNT>**Modify**</DNT>アクセス権を持つユーザーを制限します。これにより、ユーザーはパーティションを再構成し、制限されたデータにアクセスできる可能性があります。
  * 投稿キーはユーザー レベルのデータ アクセス制限を回避できるため、 <DNT>**Query keys**</DNT>機能への<DNT>**Modify**</DNT>アクセスを信頼できるアドミニストレータのみに制限します。
  * データ アクセス ポリシーを尊重しないレガシーInsights APIキーをローテーションまたは削除します。
  * アラート条件または通知を通じて機密データにアクセスすべきでないユーザーに対して、アラート管理機能のないカスタム ロールを作成します。

* **アクセス制御に合わせてパーティションを調整する**: パーティション戦略を計画するときは、どのチームがどのログにアクセスできるかを検討します。チーム、サービス、または機密レベルごとにログをグループ化して、アクセス制御の管理を容易にします。
