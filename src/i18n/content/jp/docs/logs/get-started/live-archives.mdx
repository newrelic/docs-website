...
<Steps>
  ...
  <Step>
    ...
    <Callout variant="tip">
      上記の手順では、標準の保持に加えてライブアーカイブが適用されるため、ログはまず標準ストレージに保存されます。コンプライアンスログや監査ログなど、標準ストレージに保存する必要がないログがある場合は、パーティションのネームスペースとして<DNT>**Standard**</DNT>ではなく<DNT>**Direct to live archive**</DNT>を選択してください。そのパーティションにログが送信された場合、標準ストレージを完全にスキップして、直接ライブアーカイブに送られます。詳細については、[ネームスペースの選択](/docs/logs/ui-data/data-partitions/#namespace)をご覧ください。
    </Callout>
  </Step>
