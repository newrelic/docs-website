...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        どのバリアントビルドタイプがProGuardマップをNew Relicにレポートするかを指定します。React Nativeアプリの場合、これはどのバリアントビルドタイプがJavaScriptソースマップを自動的にアップロードするかも制御します。デフォルト： `Release`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        webviewの計装を無効にするには、`false`に設定します。

        * デフォルト： `true`

        Androidエージェントv7.6.15以降を使用する場合にサポートされます。
      </td>
    </tr>
  </tbody>
</table>
...
<CollapserGroup>
  ...
  <Collapser id="app-level" title="複数のバリアントについてマップのアップロードを有効にする">
    カンマ区切りで、複数のビルドタイプを指定できます。React Nativeアプリの場合、ここに`Debug`を追加すると、デバッグビルド用のJavaScriptソースマップの自動アップロードも有効になります：
  ...
  <Collapser id="app-level" title="Webviewの計装を無効にする">
    webviewの計装を無効にするには
