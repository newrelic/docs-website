...
デフォルトでは、 <InlinePopover type="mobile"/>モバイル アプリケーションのクラッシュ レポートを有効にして、クラッシュの追跡と診断に役立てます。Android アプリケーションがクラッシュすると、エージェントはクラッシュ レポートを New Relic[コレクター](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#collector)にアップロードします。ProGuard または DexGuard を使用している場合は、クラッシュの詳細が自動的に処理され、難読化が解除されます。
