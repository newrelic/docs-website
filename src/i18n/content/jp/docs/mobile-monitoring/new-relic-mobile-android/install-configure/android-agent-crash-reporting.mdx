...
* [<DNT>**Crashes**</DNT>ページ](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/mobile-apps-crashes-dashboard)に難読化されたクラッシュ レポートが表示される場合は、 `mapping.txt`ファイルがアップロードされていない可能性があります。
...
2. コマンドターミナルを使用して、ProGuardまたはDexGuardのマッピングファイルをアップロードします。
   ...
   * <DNT>**EU account**</DNT>の場合:
     ...
     ```bash
     curl -v -F upload=@"<mapping.txt>" -H "X-APP-LICENSE-KEY:<APPLICATION_TOKEN>" https://mobile-symbol-upload.eu01.newrelic.com/symbol
     ```

   * <DNT>**JP account**</DNT>の場合:

     ```bash
     curl -v -F upload=@"<mapping.txt>" -H "X-APP-LICENSE-KEY:<APPLICATION_TOKEN>" https://mobile-symbol-upload.jp.nr-data.net/symbol
     ```
