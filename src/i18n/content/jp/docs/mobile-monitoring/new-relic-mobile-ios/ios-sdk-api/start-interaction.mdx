...
`stopCurrentInteraction: `が呼び出された場合、戻り値は`stopCurrentInteraction: `に渡す必要がある`interactionIdentifier`です。ただし、 `startInteractionWithName: `は最終的にインテリジェントに完了するため、start の呼び出し後に`stopCurrentInteraction: `を呼び出す必要はありません。
