...
2. New RelicフレームワークをXcodeプロジェクトに追加するには：**Finder**を使用して、`NewRelic.xcframework`フォルダーをXcodeプロジェクトにドラッグし、**フレームワーク、ライブラリ、組み込みコンテンツ**の下にある**Project navigator**ウィンドウにあるプロジェクトにドロップします。**埋め込み**ドロップダウンメニューの下で、**埋め込んで署名**が選択されていることを確認します。
...
4. ビルドスクリプトをターゲットの**ビルドフェーズ**に追加します。新しいビルドスクリプトが最後のビルドフェーズであることを確認してから、以下をペーストして、PUT_NEW_RELIC_APP_TOKEN_HEREをアプリケーショントークンに置き換えます。
