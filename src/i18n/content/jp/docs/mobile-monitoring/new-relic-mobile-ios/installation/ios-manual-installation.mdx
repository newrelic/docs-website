...
1. [New Relic 用の iOS SDK](https://jajp-newrelic-docs.onelink-translations.com/docs/release-notes/mobile-release-notes/ios-release-notes)をダウンロードし、解凍します。
2. New Relic フレームワークをXcodeプロジェクトに追加するには：**Finder**を使用して、`NewRelicAgent.framework`フォルダをXcodeプロジェクトにドラッグし、**フレームワーク、ライブラリ、組み込みコンテンツ**の下にある**Project navigator**.ウィンドウにあるプロジェクトにドロップします。**埋め込み**ドロップダウンメニューの下で、**埋め込んで署名**が選択されていることを確認します。
