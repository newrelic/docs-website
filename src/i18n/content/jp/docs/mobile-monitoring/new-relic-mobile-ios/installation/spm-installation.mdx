...
これらの手順は、Swift Package Manager を使用する iOS/tvOS アプリに適用されます。その他のタイプについては、 [iOS のインストールと構成を](/docs/mobile-monitoring-installation/ios-installation-and-configuration)参照してください。
...
デフォルトでは、New Relic エージェントは`info`レベルでログを記録します。ログ レベルを`verbose`以上に上げることが推奨されるのは、リリース ビルドではなく、デバッグの場合のみです。モバイル アプリの監視には、次の 6 つのログ レベルを使用できます。
