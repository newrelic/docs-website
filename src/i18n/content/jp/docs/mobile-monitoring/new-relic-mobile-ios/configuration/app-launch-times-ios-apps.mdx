...
iOS エージェントを使用すると、アプリのリリース時間 (「コールド時間」および「ホット時間」とも呼ばれる) を報告できます (ウォーム時間とプレウォーム時間は報告されません)。 デフォルトでは、アプリのリリース時間は機能フラグ[`NRFeatureFlag_AppStartMetrics`](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/configure-settings/#ios-applaunchtime)を使用して収集されます。
