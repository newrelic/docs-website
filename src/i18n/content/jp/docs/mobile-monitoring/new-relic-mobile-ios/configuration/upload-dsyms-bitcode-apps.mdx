...
2. <DNT>**Crashes**</DNT>表示します。
3. グループクラッシュの<DNT>**Total crashes**</DNT>から特定のクラッシュを選択します。
...
状況によっては、New RelicによるdSYMファイルの自動アップロードが失敗する場合があります。dSYMのアップロードが失敗した場合、`upload_dsym_results`ファイルが作成されます。たとえば、ネットワーク障害が発生してdSYMのアップロードが完了しなかった場合、`upload_dsym_results`には何が問題であったかを示すログが含まれます。New RelicがdSYMのアップロードを処理する方法の詳細については、[New Relicのオンラインテクニカルコミュニティ](https://support.newrelic.com/s/hubtopic/aAX8W0000008Yv9/relic-solution-ios-dsym-upload-deepdive)をご覧ください。
...
<CollapserGroup>
  ...
  <Collapser id="command-line-manual-upload" title="コマンドライン経由">
    ...
    2. 次のコマンドを使用して、dSYMzipファイルをアップロードします。
       ...
       * <DNT>**US accounts (default)**</DNT>の場合:
