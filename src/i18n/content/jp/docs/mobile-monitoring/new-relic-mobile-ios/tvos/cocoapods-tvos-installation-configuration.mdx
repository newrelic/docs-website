...
本手順は、Cocoapodsを使用するtvOSアプリにのみ適用されます。その他の種類の tvOS アプリについては、 [tvOS のインストールと構成](/docs/mobile-monitoring/new-relic-mobile-ios/tvos/new-relic-tvos-compatibility-requirements) を参照してください。
...
数分以内に、アプリのデータが表示されるようになります: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (select an app)**に移動します。
