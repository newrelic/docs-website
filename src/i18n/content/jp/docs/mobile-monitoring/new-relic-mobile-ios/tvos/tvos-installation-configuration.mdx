...
数分以内に、iOS アプリのデータが表示されるようになります: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (selected app)**に移動します。そうでない場合は、「[データが表示さ](/docs/mobile-monitoring/mobile-monitoring-installation/ios/no-data-appears-ios)れない」を参照してください。
