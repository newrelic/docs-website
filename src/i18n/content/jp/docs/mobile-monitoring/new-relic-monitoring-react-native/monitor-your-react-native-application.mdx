...
<Callout variant="important" title="エラー報告における破壊的変更（1.9.0）">
  React Nativeエージェントバージョン1.9.0以降、JavaScriptエラーは、新しいイベントタイプ：`MobileJSError`を通じて報告されます。新しいMobile Errors Protocolにより、React NativeのJavaScriptエラーは`MobileHandledException`イベントタイプに保存されなくなりました。

  バージョン1.9.0以降を使用している場合は、JavaScript層のエラーについて`MobileHandledException`をクエリするアラート条件とダッシュボードを更新して、代わりに`MobileJSError`を使用するようにしてください。たとえば、`SELECT count(*) FROM MobileHandledException WHERE platform = 'reactnative'`を`SELECT count(*) FROM MobileJSError`に置き換えてください。新しいイベントタイプに対して[ソースマップが正しくアップロードされる](/docs/mobile-monitoring/new-relic-monitoring-react-native/react-native-agent-js-error-reporting)ように、ビルドスクリプトも最新バージョンに更新してください。
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    [EAS Build](https://docs.expo.dev/build/introduction/)を使用してビルドする場合は、ソースマップ、dSYM、Androidマッピングファイルの自動アップロードも設定するために、「[EAS Build用のReact Nativeエージェントの設定](/docs/mobile-monitoring/new-relic-monitoring-react-native/configure-eas-build-react-native)」を参照してください。
  </Step>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [カスタムエラーを記録する](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/record-errors/#react)
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="v1.9.0" title="React Nativeエージェントのバージョン1.9.0またはそれ以降">
    JavaScriptエラーとPromiseの拒否は、`MobileJSError`イベントとして記録されます。New Relicで、各JavaScriptエラーのイベントトレイル、属性、およびスタックトレースを確認できます。
  ...
  <Collapser id="v1.2.0" title="React Nativeエージェントバージョン1.2.0～1.8.x">
...
<CollapserGroup>
  <Collapser id="logs-v1.4.1" title="React Nativeエージェントバージョン1.4.1以降">
    React Nativeエージェントバージョン1.4.1以降、JavaScriptコンソールのログは、ログイベントとして報告されます。これらは[ログUI](/docs/logs/ui-data/use-logs-ui)で表示するか、<DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) &gt; Query your data**</DNT>に移動して次を実行することでクエリできます：
  ...
  <Collapser id="logs-v1.4.0" title="React Nativeエージェントバージョン1.4.0以下">
