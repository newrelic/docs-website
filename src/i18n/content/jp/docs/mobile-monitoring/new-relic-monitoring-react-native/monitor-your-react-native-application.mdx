...
<Steps>
  <Step>
    ### エージェントのインストール [#step-one-of-install]
  ...
  <Step>
    ### React Native のセットアップ [#step-two-of-install]

    次に、 `index.js` を開き、次のコードを追加して New Relic を起動します。
  ...
  <Step>
    ### Androidのセットアップ [#step-three-android-setup]

    React NativeエージェントがAndroidと互換性があることを確認するには、Android固有のエージェントをインストールします。

    1. これらの [手順](/docs/mobile-monitoring/new-relic-mobile-android/install-configure/install-android-apps-gradle-android-studio)に従って、New Relic ネイティブ Android エージェントをインストールします。
  ...
  <Step>
    ### iOSのセットアップ [#step-four-iOS-setup]
  ...
  <Step>
    ### 自動リンクと再構築 [#step-five-autolinking-and-rebuilding]
...
4. **\[ダッシュボードに追加**] をクリックします<img title="Query your data" alt="This is an image of a sample query in the Query builder." src={mobileQueryExample}>。</img>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** > **Query your data** \[データをクエリする]: **クエリ ビルダー** を使用して、React アプリから JavaScript エラーを追跡するためのカスタム ダッシュボードを作成します。
</figcaption>
...
JavaScript からコンソール ログをキャプチャします。 **[one.newrelic.com](https://one.newrelic.com/all-capabilities)** に移動し、 **Query your data** \[データをクエリし]、次のクエリを追加して JavaScript コンソール ログを見つけます。
