...
<Steps>
  <Step>
    ...
    1. **[one.newrelic.com](https://onenr.io/0EjOpynz0Q6) > Browser & Mobile > (Android または iOS を選択)**に移動し、UI でインストルメンテーションの手順を完了します。
    2. UI の **Step 4** \[ステップ 4] から、アプリケーション トークン (該当する場合、iOS および Android 用の 1 つ) をコピーします。
  </Step>
  ...
  <Step>
    ...
    `main.ts` (Angular または Vue) または `index.tsx` (React) でアプリの初期化で Capacitor エージェントを開始します。

    1. 次のコードを追加して NewRelic を起動します (適切なアプリケーション トークンを置くことを忘れないでください)。
    ...
    2. `appToken` をステップ 1 のアプリケーション トークンに置き換えます。
  </Step>
