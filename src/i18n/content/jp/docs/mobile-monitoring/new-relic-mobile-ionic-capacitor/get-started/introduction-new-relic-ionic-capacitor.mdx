...
`index.tsx`を開き、次のコードを追加して NewRelic を起動します (適切なアプリケーション トークンを入れることを忘れないでください)。
...
AppToken はプラットフォーム固有です。Android アプリと iOS アプリ用に別のトークンを生成する必要があります。
...
Capacitor Plugin は、iOS および Android SDK エージェントと同じ API 呼び出しを利用します。[New Relic iOS SDK doc](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api)または[Android SDK](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api)を参照してください。
...
Capacitor Plugin は、JavaScript エラーのカスタム イベントを作成し、New Relic に報告します。UI では、これらの JavaScript エラー イベントをカスタム ダッシュボードで追跡できます。
