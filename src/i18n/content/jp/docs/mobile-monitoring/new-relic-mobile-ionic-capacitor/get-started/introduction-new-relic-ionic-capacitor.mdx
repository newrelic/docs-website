...
iOS および Android 用の公式 New Relic Capacitor エージェントを使用すると、開発者はモバイル エージェントを [Capacitor アプリケーション](https://capacitorjs.com/)に簡単に組み込むことができます。このプラグインは JavaScript で書かれており、New Relic のネイティブ エージェントを自動的に組み込み、モバイルの監視とパフォーマンスの可視化を提供します。
...
Capacitor エージェントは、iOS および Android SDK エージェントと同じ API 呼び出しを利用します。 [iOS SDK ドキュメント](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api) または [Android SDK ドキュメント](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api)を参照してください。
