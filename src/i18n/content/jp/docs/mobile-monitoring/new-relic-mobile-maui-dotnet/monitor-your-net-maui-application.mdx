...
<Steps>
  ...
  <Step>
    ...
    .NET MAUIモバイルプラグインを使用すると、[.NET MAUI Shell](https://learn.microsoft.com/en-us/dotnet/maui/fundamentals/shell/navigation)内のナビゲーションイベントを追跡できます。`App.xaml.cs`でのアプリの起動タイミングに応じて、これを初期化する方法は2つあります。

    <DNT>**Option 1: In the App Constructor**</DNT> `MainPage`を設定した直後に、コンストラクター内でトラッキングメソッドを直接呼び出すことができます。
    ...
    <DNT>**Option 2: In the OnStart Method (Recommended for timing/NullReferenceException issues)**</DNT> コンストラクターの実行中にアプリケーションシェルが完全に初期化されていない場合、ナビゲーショントラッカーを呼び出すと`System.NullReferenceException`が発生する可能性があります。これを回避するには、呼び出しをMAUIの`OnStart()`ライフサイクルメソッドに移動します：
...
### UIにディストリビューティッド（分散）トレーシング データがありません [#missing-distributed-tracing]
...
### TrackShellNavigatedEventsでアプリがクラッシュする [#track-shell-crash]

`App()`コンストラクターで`CrossNewRelic.Current.TrackShellNavigatedEvents()`を呼び出すときに`System.NullReferenceException`が発生した場合、アプリケーションシェルがまだ完全にブートストラップされていないことが原因である可能性が高いです。これを解決するには、呼び出しをコンストラクターから`App.xaml.cs`ファイル内のMAUI`OnStart()`ライフサイクルメソッドに移動します（コード例については、「[画面トラッキングイベント](#screen-tracking-events)」セクションを参照してください）。
