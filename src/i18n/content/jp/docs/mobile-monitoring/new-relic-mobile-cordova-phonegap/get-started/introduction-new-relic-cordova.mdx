...
iOS および Android 用の公式の New Relic Cordova プラグインを使用すると、開発者はモバイル エージェントを[Cordova アプリケーション](http://cordova.apache.org/)に簡単に埋め込むことができます。JavaScript で記述されたプラグインには、モバイル監視とパフォーマンスの可視性を提供する New Relic のネイティブ エージェントが自動的に含まれます。
