...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities)> Mobile > (アプリを選択) > Summary**: Unity データを表示し、HTTP リクエストとエラーを追跡し、アプリの経時的なパフォーマンスを監視します。
</figcaption>
...
<Steps>
  ...
  <Step>
    ...
    2. トークンを Unity プロジェクトに追加します。
       ...
       1. Unity IDE を開きます。
       2. **Tools > New Relic > Getting Started**に移動して、New Relic エディター ウィンドウを開きます。
       3. コピーしたアプリケーション トークンを`appToken = ""`というラベルの付いたフィールドに貼り付けます。アプリが iOS と Android の両方にデプロイされている場合は、プラットフォームごとに個別のトークンを提供する必要があります。
  ...
  <Step>
    ...
    1. New Relic Maven リポジトリ URL を Gradle ビルド設定に含めます。これを行うには、 `mainTemplate.gradle`ファイルを開きます。このファイルは通常、 `Assets/Plugins/Android folder`にあります。次に、次のように New Relic Maven URL を開きます。
    ...
    2. New Relic `classpath`をプロジェクト レベルの`baseProjectTemplate.gradle`ファイルに追加します。通常、このファイルは Unity プロジェクトの Android フォルダーにあります。
    ...
    Unity Studio の古いバージョンを利用している場合は、Gradle バージョン 7.0 以下をサポートする依存関係として、クラスパスの下位バージョンを組み込むことができます。
    ...
    3. `newrelic`プラグインを`launcherTemplate.gradle`ファイルの先頭に適用します。このファイルは通常、Unity プロジェクトの Android フォルダーにあります。
  ...
  <Step>
    ...
    1. Unity IDE で、 **Assets** \[アセット]に移動し、 **External Dependency Manager** \[外部依存関係マネージャー]を開きます。
    2. **iOS Resolver** \[iOS リゾルバー]をクリックします。
    3. 「Podfile Configurations」セクションで、次の設定を確認します。

    * **Add use_frameworks! to Podfile**が無効になっています
