...
<Steps>
  ...
  <Step>
    ...
    4. <DNT>**Add > Add NuGet packages**</DNT>
...
.NET MAUI エージェントをインストールした後、少なくとも 5 分間待ちます。 HTTP エラーと HTTP リクエスト UI ページで<DNT>**No HTTP data appears:**</DNT>が発生した場合は、 `HttpClient`で`HttpMessageHandler`を使用したことを確認してください。