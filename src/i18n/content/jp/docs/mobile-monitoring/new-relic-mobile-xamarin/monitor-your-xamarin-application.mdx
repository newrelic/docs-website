...
<Steps>
  ...
  <Step>
    ...
    2. **Add > Add NuGet packages**をクリックし、 `NewRelic.Xamarin.Plugin`を選択します。また、iOS エージェント バインディングを `Xamarin.iOS` プロジェクトに追加する必要があります。
    3. エージェントを追加する Xamarin.iOS プロジェクトを選択し、そのコンテキスト メニューを開きます。
    4. **Add > Add NuGet packages** \[追加] > \[NuGet パッケージの追加] を選択し、 `NewRelic.Xamarin.iOS.Binding`を選択します。
  </Step>
