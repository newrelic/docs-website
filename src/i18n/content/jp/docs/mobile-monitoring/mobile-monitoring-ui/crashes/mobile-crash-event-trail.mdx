...
<Callout variant="tip">
  この機能へのアクセスは、お客様の
  [サブスクリプションレベルによって異なります](http://newrelic.com/mobile-monitoring/pricing)
  。
</Callout>
...
![New Relic Mobileのクラッシュイベントの記録 ](src/images/New-Relic-Mobile-crash-event-trail.png 'Android-Insights-event-trail.jpg')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > モバイル>
  (モバイルアプリを選択)> クラッシュ分析> (選択したクラッシュタイプ)>
  イベントトレイル**:
  クラッシュイベントトレイルは、モバイルアプリのクラッシュに至るまでの活動を示しています。
</figcaption>
...
6. イベントトレイルの結果を New Relic で表示するには、 **Insights でセッションを開く** を選択します。
