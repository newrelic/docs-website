...
開発チームが[メールでクラッシュ通知](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/receive-crash-notifications-email)を受信すると、メールリンクを選択して、ユーザーインターフェースでクラッシュに関する詳細情報を確認したり[、クエリを介して](docs/query-your-data/explore-query-data/explore-data/introduction-querying-new-relic-data)グラフデータを直接分析したりできます。個々の**クラッシュの詳細**ページには、チームがレポートから解決まで問題を追跡するための追跡ツールも含まれています。
...
* ほとんどのクラッシュがいつ発生するかを特定します。たとえば、 [`Last Interaction`でフィルタリングし](#groups-filters)たり、 [**クラッシュレポート**ページ](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report)で[インタラクショントレイル](#export-xcode)を表示したりします。
...
1. **[one.newrelic.com](https://one.newrelic.com) >モバイル>（アプリを選択）>例外>クラッシュ分析**に移動します。
...
<CollapserGroup>
  ...
  >
    ...
    * [タイムピッカー](/docs/apm/new-relic-apm/ui-functions/time-picker-setting-time-periods-view-data)を使用して、現在選択されている期間を調整します。
  ...
  >
    ...
    * 他の履歴パターンを探すには、選択し[た期間](#time-period)を変更します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com) >モバイル>（アプリを選択）>例外>クラッシュ分析：**これは、**クラッシュロケーション**グループによってフィルタリングされた**上位5つのオカレンス**チャートの例です。グループまたはフィルターを選択して、クラッシュデータを任意の方法で分析します。
      </figcaption>
  </Collapser>
  ...
  >
    ...
    * 例外が最初と最後に発生したのはいつですか？
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/) >モバイル>（アプリを選択）>例外>クラッシュ分析**：任意の列で並べ替えて**クラッシュレポート**テーブルのパターンを分析し、レポートを選択してクラッシュの詳細を確認します。
      </figcaption>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com) >モバイル>（アプリを選択）>例外>クラッシュ分析>クラッシュプロファイル**：クラッシュしたセッションとクラッシュのないセッションを比較します。
    </figcaption>
  </Collapser>
  ...
  >
    **クラッシュレポート**テーブルから、特定の[**クラッシュレポート**](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report)にドリルダウンできます。ここから次のことができます。

    * セッション開始からクラッシュイベントまでの[インタラクショントレイル](/docs/mobile-monitoring/mobile-monitoring-ui/mobile-app-pages/interactions-page)を調べます。

    * 関連するスレッドの内訳を調べます。

    * Xcodeを使用してソースコードを調べることができるように、\[**クラッシュの詳細をエクスポート]**を選択します。

    * デバッグを容易にするために、クラッシュレポートを再記号化します。
    ...
    * チケットを提出し、クラッシュを解決します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com) >モバイル>（アプリを選択）>クラッシュ>クラッシュレポート：**これは、クラッシュの発生を再シンボリック化するオプションを含むインタラクショントレイルの例です。Xcodeを使用してソースコードを分析およびデバッグするには、\[**クラッシュの詳細をエクスポート**]を選択します。
      </figcaption>
  </Collapser>
</CollapserGroup>
