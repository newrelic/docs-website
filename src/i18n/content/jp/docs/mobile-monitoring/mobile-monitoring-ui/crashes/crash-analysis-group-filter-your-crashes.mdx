...
<CollapserGroup>
  ...
  >
    ...
    * 他の履歴パターンを探すには、選択し[た期間](#time-period)を変更します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > モバイル > (アプリを選択) > 例外 > クラッシュ分析:**これは、**クラッシュの場所**グループによってフィルタリングされた**上位 5 件の発生**グラフの例です。グループまたはフィルターを選択して、クラッシュ データを任意の方法で分析します。
      </figcaption>
  </Collapser>
  ...
  >
    ...
    * 例外が最初と最後に発生したのはいつですか？
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (アプリを選択) > Exceptions > Crash analysis** : 任意の列で並べ替えて**クラッシュ レポート**テーブルのパターンを分析し、任意のレポートを選択してクラッシュの詳細を確認します。
      </figcaption>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > モバイル > (アプリを選択) > 例外 > クラッシュ分析 > クラッシュ プロファイル**: クラッシュしたセッションとクラッシュしていないセッションを比較します。
    </figcaption>
  </Collapser>
  ...
  >
    ...
    * チケットを提出し、クラッシュを解決します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Mobile > (アプリを選択) > Crashes > Crash report:**これは、クラッシュの発生を再記号化するオプションを含むインタラクション トレイルの例です。Xcode を使用してソース コードを分析およびデバッグするには、\[**クラッシュの詳細をエクスポート**] を選択します。
      </figcaption>
  </Collapser>
</CollapserGroup>
