...
<CollapserGroup>
  ...
  >
    ...
    * チケットを提出し、クラッシュを解決します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > モバイル > (アプリを選択) > クラッシュ > クラッシュの種類 > (クラッシュを選択) > (スタック トレースとインタラクション トレイルまでスクロール****):**これは、再記号化するオプションを含むインタラクション トレイルの例です。クラッシュ発生。Xcode を使用してソース コードを分析およびデバッグするには、\[**クラッシュの詳細をエクスポート**] を選択します。
      </figcaption>
  </Collapser>
</CollapserGroup>
