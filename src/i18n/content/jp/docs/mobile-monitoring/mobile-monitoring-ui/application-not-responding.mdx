...
New Relic ANR スタックトレース内のJavaスタック フレームを自動的にシンボル化し、読み取り可能なメソッド名と行番号をプラットフォームに直接提供します。

<Callout variant="tip" title="注記">
  ネイティブ (NDK) スタック フレームは現在シンボル化されていません。ネイティブ スタック フレームの場合、 New Relicからスタックトレースをダウンロードし、 `ndk-stack`などのオフライン ツールを使用して手動でシンボル化できます。
</Callout>
