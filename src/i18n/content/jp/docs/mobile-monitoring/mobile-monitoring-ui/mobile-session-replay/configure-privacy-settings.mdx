...
4. **親/子優先順位ルール**
   ...
   競合するルールがある場合、親のマスキングはサブツリーのルールよりも優先されることに注意してください。

   *例：*親に`nr-block`属性があるが、子に`nr-unmask`がある場合、サブツリー全体がコンテンツをブロックする黒い長方形のプレースホルダーに置き換えられます；サブツリー内の子コンテンツはキャプチャされません。
