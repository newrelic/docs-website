...
HTTP およびネットワーク リクエストを監視すると、アプリのパフォーマンスを把握でき、アプリの改善に役立つデータが得られます。HTTP およびネットワークのリクエストとエラーを見つけるには、 [<InlinePopover type="mobile"/>](#view-mobile)NRQL を使用してUIまたはクエリ[`MobileRequest`および`MobileRequestError`イベントを実行します](#query-insights)。
