...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="ios">
      ...
      この API は、エラーのインスタンスとオプションの属性辞書を受け取り、 `recordHandledException`イベントを作成します。 イベント データは、 [<DoNotTranslate>**Handled exceptions**</DoNotTranslate>ページ](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/handled-exceptions-analyze-trends-prevent-crashes)や[<DoNotTranslate>**Crash events**</DoNotTranslate>トレイル](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/mobile-crash-event-trail)などの<InlinePopover type="mobile"/> UI で表示できます。 このデータを NRQL でクエリし、New Relic ダッシュボードでグラフ化することもできます。
