...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="ios">
      ...
      この API は、エラーのインスタンスとオプションの属性辞書を取得し、 `recordHandledException`イベントを作成します。イベントデータは、 <InlinePopover type="mobile"/>[**Handled exceptions** \[処理された例外\]ページ](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/handled-exceptions-analyze-trends-prevent-crashes)や[**Crash events** \[クラッシュ イベント\]トレイル](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/mobile-crash-event-trail)などの場所の UI。NRQL を使用してこのデータをクエリし、New Relic ダッシュボードでグラフ化することもできます。
