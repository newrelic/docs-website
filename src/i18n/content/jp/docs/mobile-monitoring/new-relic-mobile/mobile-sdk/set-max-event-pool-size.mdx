...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="ios">
      ...
      <Callout variant="important">
        イベントプールサイズを`1000`より大きく設定することはできますが、エージェントはネットワークデータに対して厳格な内部制限を設けています。イベントプールをどれほど大きく設定しても、収集サイクルごとに最大**1,000件のネットワークトランザクション**が記録および送信されます。
      </Callout>

      イベント収集サイクルのデフォルト値は60秒です。イベント収集サイクルの長さを変更するには、[最大イベントバッファ時間の設定](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/set-max-event-buffer-time/)も参照してください。
