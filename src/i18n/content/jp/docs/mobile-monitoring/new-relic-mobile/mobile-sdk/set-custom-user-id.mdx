    ...
    <TabsPageItem id="android">
      ...
      * すべてのクラッシュをフィルタリングして、各クラッシュタイプのユーザー（またはユーザーグループ）の発生状況を表示します。逆に、クラッシュリストをフィルタリングして、各クラッシュタイプの影響を受けたユーザーを表示します。ユーザーIDが電子メールアドレスとして表されている場合、それらのクラッシュに対応するソリューションが利用可能になったときに、簡単に連絡して知らせることができます。
      ...
      <Callout variant="important">
        Android エージェント v7.4.0 以降では以下の機能強化が追加されます

        1. `setUserId(value: string|null)`が呼び出されたときに、 `userID`が以前は null で、新しい値が null でない場合、現在のセッションは継続され、新しい`userID`が設定されます。
        2. `setUserId(value: string|null)`が呼び出されたときに、 `userID`が以前は null でなく、新しい`userID`が`null`を含めて異なる場合、現在のセッションは終了し、収集が実行されます。 新しい`userID`を使用して新しいセッションが作成されます。
      </Callout>
