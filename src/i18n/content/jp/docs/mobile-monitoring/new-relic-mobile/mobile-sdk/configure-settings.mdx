      ...
      * `.with` メソッドを使用して、エージェント開始呼び出しの一部として設定を変更します。例えば：
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              アプリケーションがバックグラウンド状態になったときにバックグラウンド レポートを有効または無効にします。有効にすると、エージェントはアプリがバックグラウンドで動作している間にキャプチャされたデータを報告し、[`background`プロパティが](/attribute-dictionary/?event=MobileSession&attribute=background)セッション プロパティに追加されます。
            </td>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              イベントの永続化を有効または無効にします。これにより、エージェントは強制終了およびプロセス終了のシナリオ中にイベントをキャプチャできます。
            </td>

            <td>
              イベントの永続化はデフォルトで有効になっています。無効にするには、次の機能フラグを追加します：
      ...
      * 迅速：
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              インターネット接続が利用できない場合にオフライン データ ストレージを有効または無効にします。アプリがインターネットにアクセスできないときにイベントが追加されると、イベントに[`offline`属性](/attribute-dictionary/?event=MobileSession&attribute=offline)が追加されます。オフライン ストレージの量を構成するには、 [「オフライン ストレージの最大サイズを設定する」を](/docs/mobile-monitoring/new-relic-mobile/mobile-sdk/set-max-offline-storage/#ios)参照してください。
            </td>
    ...
    <TabsPageItem id="react">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td>
              JavaScriptエラーの[`MobileJSError`イベント](/attribute-dictionary/?event=MobileJSError)としての自動報告を有効または無効にします。有効にすると、エージェントは処理されていないJavaScriptエラーをキャプチャし、追加の計装なしで報告します。

              デフォルト値は`true`です。React Nativeエージェントのバージョン1.9.0以降で利用可能です。
            </td>
