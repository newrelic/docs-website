  ...
  <TabsPages>
    <TabsPageItem id="android">
      ...
      * <DoNotTranslate>**Do not use `eventType` to name your custom events.**</DoNotTranslate>
        ...
        プロパティを作成してイベントに名前を付けるか、オプションの`name`問題を使用します。 カスタムイベントはたくさん作成できます。制限する必要があるのはイベントの種類のみです。
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>
              必須。 イベントの種類。 <DoNotTranslate>**Do not**</DoNotTranslate> `$eventType`を使用してカスタムイベントに名前を付けます。 代わりに、カスタムアトリビュートまたはオプションの`name`を使用してください。
            </td>
          </tr>
