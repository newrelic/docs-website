...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="ios">
      ...
      このメソッドは、カウントが 1 で、合計値が渡された値と等しい、形式`Custom/[Category]/[Name]`のメトリクスを記録します。 複数の呼び出しでは、 [<InlinePopover type="mobile" />の標準メトリクス集計ルール](/docs/mobile-monitoring/new-relic-mobile/getting-started/mobile-data-retention)に従ってカウントと値が集計されます。
