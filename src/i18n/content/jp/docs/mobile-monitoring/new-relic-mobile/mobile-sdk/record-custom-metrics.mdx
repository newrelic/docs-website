...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="ios">
      ...
      このメソッドは、カウントが 1 で、合計値が渡された値と等しい、形式`Custom/[Category]/[Name]`のメトリックを記録します。複数の呼び出しでは、標準の[メトリック集計ルールに従ってカウントと値が集計されます。 <InlinePopover type="mobile"/>](/docs/mobile-monitoring/new-relic-mobile/getting-started/mobile-data-retention)。
