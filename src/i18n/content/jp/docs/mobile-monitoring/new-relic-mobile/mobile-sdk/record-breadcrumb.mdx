...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="ios">
      ...
      この呼び出しを使用すると、イベント タイプとして `MobileBreadcrumb` を指定して [`recordCustomEvent` 呼び出し](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api/recordcustomevent-ios-sdk-ap) を使用した場合と同じ結果になります。この API の使用方法については、カスタム属性とイベントの送信についてお読みください。
