...
New Relicにアプリを追加するには、次のコマンドで `$API_KEY` を[New Relic APIキー](/docs/apis/rest-api-v2/requirements/api-keys#rest-api-key)に置き換え、`${STRING}` をアプリの名前に置き換えます。 APIエクスプローラーから同じタスクを実行するには、 APIキーを使用して <DNT>**[api.newrelic.com/docs](https://api.newrelic.com/docs) &gt; Browser applications &gt; POST create**</DNT> に移動します。
...
browserアプリの一覧を表示するには、次のコマンドで `$API_KEY` を[New Relic APIキー](/docs/apis/rest-api-v2/requirements/api-keys#rest-api-key)に置き換えます。 APIエクスプローラーから同じタスクを実行するには、 APIキーを使用して <DNT>**[api.newrelic.com/docs](https://api.newrelic.com/docs) &gt; Browser Applications &gt; GET List**</DNT> に移動します。
