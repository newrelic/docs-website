...
* これらの例の`${APPID}` } 変数と`${API_KEY}`変数を、特定の[アプリケーション ID](/docs/apm/apis/requirements/identification-code)と対応する[REST API キー](/docs/apis/rest-api-v2/requirements/rest-api-key#viewing)に置き換える必要があります。
...
`EndUser/RB:average_dom_content_load_time`は、ブラウザで HTML の解析と解釈に費やされた時間です。これは、ブラウザーの**DOM コンテンツ**イベントによって測定されます。
