...
New Relic ユーザーインターフェースに表示されるアプリケーション名は、構成設定で割り当てられたプライマリ名の人間が判読できる[エイリアス](/docs/agents/manage-apm-agents/app-naming/rename-your-application#app-alias)です。これを変更すると、アプリの名前はプラットフォーム内のあらゆる場所で更新されます。 <InlinePopover type="apm"/>、 <InlinePopover type="browser"/>、など。
