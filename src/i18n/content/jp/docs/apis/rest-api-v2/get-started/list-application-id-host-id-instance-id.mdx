...
1. [API Explorer](https://api.newrelic.com/docs)に移動し、<DNT>**Select an account**</DNT>ドロップダウンからアカウント名を選択します。
2. [api.newrelic.com/docs](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json)にあるAPI Explorerの<DNT>**Application host**</DNT>ページに移動します。
...
<CollapserGroup>
  <Collapser id="host_id" title="REST API Explorerを使ったホストIDとサマリーデータの取得">
    ...
    1. [API Explorer](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__hosts_json)に移動し、<DNT>**Select an account**</DNT>ドロップダウンからアカウント名を選択します。
...
インスタンスIDの意味は、使用されているNew Relic言語エージェントによって異なります。REST APIからこのIDを一覧表示できます。
...
`INSTANCE_ID`を使用して、インスタンスのサマリーメトリクスおよび特定のメトリックタイムスライス値を取得できます。利用可能なメトリクスの詳細については、[REST API Explorer アプリケーションインスタンス](https://api.newrelic.com/docs/#/Applications/get_applications__application_id__instances__instance_id__metrics_json)ページを使用してください。
