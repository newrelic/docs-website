...
APM エージェントによって監視される各アプリには、名前が割り当てられます。固有の`$APP_ID`もその名前に関連付けられています。`$APP_ID`は、アプリに関する APM 情報を取得するための基本です。`$APP_ID`の一覧表示と使用、および概要データの取得について詳しくは[、アプリ ID の一覧表示](/docs/apis/rest-api-v2/application-examples-v2/listing-your-app-id-metric-data-v2)をご覧ください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        New Relic の .NET Framework エージェントの場合、これは `AppDomain`
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) > APM > Applications > (アプリを選択) > JVMs**に移動します。
