...
<Callout title="機能の利用可否" variant="important">
  一部のリージョンではこの機能をご利用いただけません。お使いのリージョンでの利用可否を確認し、代替ソリューションを検討するには、[データセンターとリージョン別の提供状況](/docs/accounts/accounts-billing/account-setup/choose-your-data-center#feature-limitations-by-region)ページを参照してください。
</Callout>
