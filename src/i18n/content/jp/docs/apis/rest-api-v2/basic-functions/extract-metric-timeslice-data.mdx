...
要求された時間範囲の開始時刻が8日より前の場合、[データ集約スケジュール](#data-aggregation)により、データは1時間の期間に集約または平均化されています。これは、任意の1時間の間、単一のデータ値のみが使用可能であることを意味します。時間範囲内の1時間未満の期間でデータを取得すると、オーバーサンプリングが発生し、重複した値が返されます。 10個の値のみを返すと、オーバーサンプリングが防止され、チャートがよりスムーズになり、誤解を招く可能性のある「プラトー」効果が排除されます。

## 期間出力の制御 [#period]

出力データの粒度が細かすぎる場合や、返されるデータの期間が短すぎる場合があります。これを制御するには、各期間にレポートする**秒**数として`period=`パラメーターをqueryコマンドに含めます。仕様がNewRelicの[データ集約スケジュール](#data-aggregation)に従っていることを確認してください。

**例1：**[収集されたデータの粒度を要約したNew Relicのテーブル](#time-data-granularity)に続いて、リクエストは4日間（ `from=2018-02-13`および`to=2018-02-17` ）であるため、次のAPI呼び出しは通常30分の期間でデータを返します。 `period=3600`を追加すると、データは60分の期間として返されます。
