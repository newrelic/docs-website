...
- REST API を介して任意のメトリックのタイムスライスデータを照会することができます。

このドキュメントでは、REST API を使ってこれを行う方法を説明します。この API は、分単位のデータを一括して抽出することを目的としたものではありませんのでご注意ください。

## タイムベースのデータ [#time-based]

REST API および API Explorer が返す時間値はすべて UTC（協定世界時）です。 必要に応じて、データ収集のための時間値 [を必ず調整してください。](/docs/apm/apis/api-v2-examples/specifying-time-range-api-v2)

## タイムレンジの検討 [#time]

<Callout variant="important">
  データリクエストの最小時間範囲は1分（60秒）です。それ以下の時間のリクエストは、
  `422` のステータスコードとなり、データは返されません。New Relic は 1
  分間隔でしかデータを収集しません。
</Callout>

API では、UI と同じメカニズムでデータを要求します。要求するデータの時間範囲に依存します。目的は、返されるデータポイントの数を最適化し、消化しやすいグラフとレポートを提供することです。

たとえば、

- - **3 時間以下の時間範囲のデータを要求した場合** 、API は最初に収集した 1 分間のデータ値を返します。

  - 時間の範囲を **3 時間以上** にすると、返されるデータ値は 2 分間の平均値になります。

  - 時間の範囲を 6 時間以上 に拡大すると、返されるデータ値は 5 分間の平均値となり、以下のようになります。
...
要求された時間範囲の開始時刻が 8 日よりも古い場合、データは [データ集計スケジュール](#data-aggregation) により 1 時間単位に集計または平均化されています。つまり、1 時間の期間では、1 つのデータ値しか得られないということです。1 時間未満の時間帯のデータを取得すると、オーバーサンプリングが発生し、重複した値が返されることになります。10 個の値のみを返すことで、オーバーサンプリングを防ぎ、より滑らかなチャートを表示し、誤解を招く可能性のある"プラトー" 効果を排除することができます。
...
**例 1:** New Relic の [table summarizing granularity of collect data](#time-data-granularity) に従いますと、以下の API 呼び出しでは、リクエストが 4 日間 (`from=2018-02-13` and `to=2018-02-17`) であるため、通常は 30 分単位でデータが返されます。 `period=3600` を追加することで、データは 60 分単位で返されます。
...
- 上記のコマンド例では、時間範囲のデフォルト（30 分）の粒度よりも大きいので、1 時間の期間を要求することができます。
- 上記のコマンド例では、 **は、時間範囲のデフォルト（30 分）の粒度よりも小さいため、** 1 分単位の要求はできません。

**例 2：** > 7 日以上 3 週間以下の範囲を要求した場合、デフォルトの期間は 3 時間ですが、6 時間、12 時間、24 時間などの期間を指定することができます。ただし、デフォルトの 3 時間ではなく、1 時間を指定することはできません。
