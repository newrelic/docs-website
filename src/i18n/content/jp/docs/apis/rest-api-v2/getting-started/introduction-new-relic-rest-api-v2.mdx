...
[$&#x7B;APPID}](/docs/apis/rest-api-v2/requirements/finding-product-id)は、データがリクエストされているアプリケーションまたは製品そのものを指定します。このパラメーターに続く情報は、リクエストするデータに応じて異なります。
...
New Relic APIコールでは、コールヘッダーに[APIキー](/docs/apis/rest-api-v2/requirements/api-keys)が必要になります。APIキーは独自の方法でアカウントを識別し、アカウントデータへのアクセスを承認します。New Relicは、Unixシェルプログラミングからプレースホルダ$&#x7B;API_KEY}を借用しています。$&#x7B;API_KEY}は、必ずNew RelicアカウントからのAPIキーに置き換えてください。
