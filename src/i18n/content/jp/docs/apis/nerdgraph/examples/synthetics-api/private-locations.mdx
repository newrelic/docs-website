...
<DNT>`syntheticsCreatePrivateLocation`</DNT>変異を使用してプライベートロケーションを作成できます。 この変更により、監視インフラストラクチャに新しいプライベート ロケーションをセットアップして、プライベート ミニオンまたはジョブ マネージャーをデプロイできるようになります。
...
<DNT>`syntheticsUpdatePrivateLocation`</DNT>ミューテーションを使用して、既存のプライベート位置を更新できます。 すでに作成されているプライベートロケーションの設定を変更することができます。
...
<DNT>`syntheticsPurgePrivateLocationQueue`</DNT>ミューテーションを使用して、プライベートロケーションのジョブ キューをクリアできます。 これは、パフォーマンスの問題や一時的な接続の問題により蓄積された可能性のある、キューに入れられた合成モニター ジョブのバックログを削除する必要がある場合に役立ちます。
...
プライベートロケーションが不要になった場合は、 <DNT>`syntheticsDeletePrivateLocation`</DNT>ミューテーションを使用して完全に削除できます。
...
NerdGraphを使用して、ロケーションのメタデータと設定の詳細を取得します。

詳細については、次を参照してください：

* [プライベートロケーションのクエリ](/docs/apis/nerdgraph/examples/synthetics-api/query-synthetics-data#query-private-locations) — すべてのプライベートロケーションとその設定の詳細を取得します。
