...
* `id`: ウィジェットの ID。
* `visualization`ウィジェットの視覚化タイプを文字列で指定します。例えば： `viz.line` 、 `viz.area` 。[以下の例を](#typed-widget-data)ご覧ください。
...
* `configuration`ウィジェットの[型付き設定](#widget-typed)。これは、最も一般的なウィジェット（[エリア](#area)、[バー](#bar)、[ビルボード](#billboard)、[ライン](#line)、[マークダウン](#markdown)、[パイ](#pie)、[テーブル）](#table)を便利な型付きGraphQLで表現したものです。
* `rawConfiguration`ウィジェットの型[指定のない設定](#widget-untyped)。これは[、ヒートマップ](#heatmap)、[ヒストグラム](#histogram)、[ブレット](#bullet)、[ファネル](#funnel)などの型指定された設定ではカバーされていないウィジェット プロパティを保存する自由形式の方法 (JSON スカラー) です。 この場合、 `configuration`はnullであり、 `rawConfiguration`はウィジェットに必要なすべてのデータが含まれています。
* `description`ウィジェットに関する追加情報を提供するオプションのテキスト。ユーザーがウィジェットにカーソルを合わせたときにツールチップとして表示されます。
* `link`ウィジェットのタイトルをクリック可能にするオプションのURL。これにより、関連するダッシュボード、ドキュメント、または外部リソースへのナビゲーションが可能になります。`http://`または`https://`プロトコルを使用する必要があり、2,048文字を超えることはできません。
