...
### ページ内のウィジェットを更新する [#update-widgets-in-page]
...
#### ウィジェットのリンクと説明 [#widget-links-descriptions]

ウィジェットにリンクや説明を追加するには、 `dashboardUpdateWidgetsInPage`使用することもできます。ウィジェットのリンクは、ウィジェットのタイトル上でクリック可能なリンクとして表示され、ユーザーがウィジェットをポイントすると、ウィジェットの説明がツールチップとして表示されます。

<Callout variant="important">
  ウィジェットリンクのURLは`http://`または`https://`プロトコルを使用する必要があり、2,048文字を超えることはできません。
</Callout>

<CollapserGroup>
  <Collapser id="update-widget-link-description" title="ウィジェットのリンクと説明を更新します">
    ```graphql
    mutation UpdateWidgetsInPage($pageGuid: EntityGuid!, $widgets: [DashboardUpdateWidgetInput!]!) {
      dashboardUpdateWidgetsInPage(guid: $pageGuid, widgets: $widgets) {
        errors {
  ...
  <Collapser id="remove-widget-link-description" title="ウィジェットのリンクと説明を削除">
    リンクまたは説明を削除するには、フィールドを`null`に設定します。
...
### テンプレート変数の可視性 [#variable-visibility]

テンプレート変数は、表示モードの変数バーから非表示にできますが、非表示にしても通常どおり機能します。変数の`options`オブジェクト内の`hiddenOnVariablesBar`フィールドを使用してください。

* **フィールド名:** `hiddenOnVariablesBar`
* **タイプ:**ブール値
* **デフォルト:** `false` (変数は表示されます)

<CollapserGroup>
  <Collapser id="create-hidden-variable" title="非表示変数を含むダッシュボードを作成する">
    ```graphql
    mutation {
      dashboardCreate(
  ...
  <Collapser id="update-variable-visibility" title="変数を更新して、表示または非表示にします">
    変数の表示設定を更新するには、更新された値`hiddenOnVariablesBar`を含む完全なダッシュボード構成を含めます。
...
テンプレート変数の表示設定の詳細については、 [「変数の表示と非表示」](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility)を参照してください。
