...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        この解析ルールの[Grok](https://grokdebug.herokuapp.com/patterns#)パターン。たとえば、使用している Grok パターンの`logtype`を`logtype = 'alb'`などの[組み込みの解析ルール](/docs/logs/log-management/ui-data/built-log-parsing-rulesets/)に含めることができます。ただし、使用できるのは`logtype`に限定されません。任意の属性を一致基準として使用できます。
      </td>
    </tr>
