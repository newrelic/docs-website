...
[api.newrelic.com/graphiql](https://api.newrelic.com/graphiql) で NerdGraph を使用して、 [ログの解析ルール](https://docs.newrelic.com/docs/logs/log-management/ui-data/parsing/) を作成、照会、管理することができます。NerdGraphは、GraphQL形式のAPIエクスプローラーです。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        New Relic UI から使用される検索値。例： `logtype:alb` 。New Relic UI で有効な Lucene 関数の詳細については、 [logs query syntax](https://docs.newrelic.com/docs/logs/log-management/ui-data/query-syntax-logs/) に関するドキュメントを参照してください。
      </td>
    </tr>
