...
WHERE entityGuid IN ('ENTITY_GUID') 
  AND provider = 'DynamoDbTable'
TIMESERIES
```
...
* `metrics`：新しいNRDBクエリはゴールデンメトリックとして表示されます。
  ...
  * `select`: NRDB クエリの`SELECT`句。 このフィールドは<DNT>**required**</DNT>です。
  * `from`：NRDBクエリの`FROM`句。
  * `where`: エンティティ タイプ フィールドを識別するための補完的な`WHERE`句。
  * `facet`： `FACET`へのフィールド。
  * `title`：ゴールデンメトリックのタイトル。このフィールドはオプションです。
  * `name`: ゴールデンメトリクスの名前。 このフィールドは<DNT>**required**</DNT>です。

<CollapserGroup>
  <Collapser id="override-account" title="APMアプリケーションタイプのゴールデンメトリクスの上書き">
    ```graphql
    mutation {
      entityGoldenMetricsOverride(
...
メトリクス入力オブジェクトは、セクションに分割された NRDB 書き込みの部分を定義します。 これらのセクションの 1 つは`eventId`で、ゴールデン メトリクスとして使用する*NRDB イベント*内の GUID を定義するフィールドを識別するために使用されます。 例えば：
...
結果のオブジェクトでわかるように、 `where`句には*プロバイダー*フィールドのみが含まれます。システムは、デフォルトで`eventId`フィールドを含むwhere句を追加します。
...
[New Relic のワークロードは、](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\))エンティティのグループに関する健全性とパフォーマンス データの集約ビューを提供します。ワークロード内のエンティティ タイプごとに表示される時系列グラフは、ワークロード アカウントのゴールデン メトリクスによって定義されます。
...
<CollapserGroup>
  <Collapser id="override-workload" title="APMアプリケーションのオーバーライド">
    ```graphql
    mutation {
      entityGoldenMetricsOverride(
