...
インシデントの設定では、ポリシーで作成された各条件に対してインシデントがどのように作成されるかを構成します。 詳細については、 [「まとめイベント」の設定の選択](/docs/alerts/organize-alerts/specify-when-alerts-create-events)に関するドキュメントを参照してください。
