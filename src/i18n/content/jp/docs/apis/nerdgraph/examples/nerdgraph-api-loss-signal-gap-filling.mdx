...
* `expiration.ignoreOnExpectedTermination`: `true`の場合、信号が失われると予想されるときにインシデントは開かれません。 信号が失われると予想されることを示すには、エンティティにタグ`termination: expected`が存在する必要があります。 デフォルトは`false`です。
