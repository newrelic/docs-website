...
<Callout variant="tip">
  ...
  信号損失、ギャップの充填、これらの機能へのアクセスをリクエストする方法の詳細については、[このサポートフォーラムの投稿](https://support.newrelic.com/s/hubtopic/aAX8W0000008b5p/announcing-new-relic-one-streaming-alerts-for-nrql-conditions)を参照してください。
</Callout>
...
* `expiration.ignoreOnExpectedTermination`: `true`の場合、信号が失われると予想されるときにインシデントは開かれません。信号が失われると予想されることを示すには、エンティティにタグ`termination: expected`が存在している必要があります。デフォルトは`false`です。インフラストラクチャ ホスト エンティティの場合、タグ`hostStatus: shutdown`は信号が停止し、大量イベントの開催が妨げられることが予想されることも示します。
