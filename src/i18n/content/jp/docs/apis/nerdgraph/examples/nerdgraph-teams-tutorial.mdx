...
簡単に始めるには、[NerdGraph APIエクスプローラ](/docs/apis/nerdgraph/get-started/nerdgraph-explorer)を使用してチーム情報を作成および編集してください。以下の例の各フィールドの定義は、NerdGraphで確認できます。GraphiQL IDEの使用についてヘルプが必要な場合は、[NerdGraph APIエクスプローラーのチュートリアル](/docs/apis/nerdgraph/get-started/nerdgraph-explorer/)を参照してください。Teamsを管理するためのカスタム権限を設定する必要がある場合は、[Teamsのカスタムロールの作成](/docs/apis/nerdgraph/examples/nerdgraph-teams-custom-role-tutorial)を参照してください。
...
* `parentId` 階層ツリー内の親チーム ID を参照します。
...
* `hierarchyLevelId`: 階層内でのこのチームの位置を表す`TeamsHierarchyLevelEntity`の ID。これは組織設定によって構成されます ([組織設定を](#update-team-settings)参照)。
* `managers`：このチームのマネージャーまたは主な連絡窓口であるユーザーエンティティのGUIDのリスト。マネージャーはチームメンバーである必要があります。ユーザーGUIDを確認するには、[ユーザーIDの取得](#user-id)をご覧ください。
* `onCallMembers`：通常の営業時間外の連絡先を表す`OnCallMember`エンティティのリスト。各エンティティには、ユーザーのエンティティGUIDを含むIDフィールドが必要です。オンコールメンバーはチームメンバーである必要があります。ユーザーGUIDを確認するには、[ユーザーIDの取得](#user-id)を参照してください。
* `parentId`: 階層内の親チームの ID。
...
* `managers` ユーザー エンティティ GUID のリストを受け入れます。ユーザー GUID を見つけるには、 [「ユーザー ID の取得」を](#user-id)参照してください。
* `onCallMembers` `OnCallMember`エンティティのリストを受け入れます。各エンティティには、ユーザーのエンティティGUIDを含むIDフィールドが必要です。ユーザーGUIDを確認するには、[ユーザーIDの取得](#user-id)をご覧ください。

## 階層レベルを更新する [#update-hierarchy-level]

階層レベルの名前を更新するには、 `entityManagementUpdateTeamsHierarchyLevel`ミューテーションを使用します。[組織設定クエリ](#update-team-settings)の`hierarchyLevelOrder`フィールドで階層レベル ID を見つけることができます。
...
* `name` 階層レベルで唯一更新可能な属性です。
* `id`は更新する`TeamsHierarchyLevelEntity`の ID です。これらの ID は[、組織設定クエリ](#update-team-settings)によって返される`hierarchyLevelOrder`配列にあります。
...
* `hierarchyLevelOrder`: 階層レベルの順序を子孫順に定義する配列。最初の要素が最高レベルで、最後の要素が最低レベルになります。各要素は`TeamsHierarchyLevelEntity`の`id`です。
