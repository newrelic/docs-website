---
title: 'Nerdgraph チュートリアル: アラートと適用されたインテリジェンスの問題を構成する'
metaDescription: Configure issues in alerts and applied intelligence
translationType: machine
---

このドキュメントは、あなたとあなたのチームが問題やインシデントについてデータを照会し、NerdGraph API を使用して機能を構成する方法を学ぶための場所です。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        環境について学習するには[、応用インテリジェンスの概要を](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence/)参照してください。
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    ...
    カーソル ページネーションを使用すると、レスポンスから返された`nextCursor`が空になるまで、結果データを介してリクエストを作成し続けます。空の応答は、結果の最後に到達したことを意味します。
  ...
  >
    ...
    ここで、 `entityGuides`と`entityTypes`は[、New Relic エンティティについて学ぶ](/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/)ドキュメントで定義されているエンティティ属性です。
    ...
    * すでに見たように、 `cursor`
    ...
    優先度`CRITICAL`と状態の問題を見つけてみましょう `CLOSED`
...
<CollapserGroup>
  ...
  <Collapser
    id="filter-incidents"
    title="インシデントのフィルタリング"
  >
    ### カーソルのページネーション

    データをページ分割するには、最初のクエリに nextCursor フィールドを含める必要があります。

    カーソル ページネーションを使用すると、応答から返される nextCursor が空になるまで、結果データを介して要求を続けます。それはあなたがあなたの結果の終わりに到達することを意味します。
...
<CollapserGroup>
  ...
  >
    ...
    インシデント イベントを、エンティティ GUID、エンティティ タイプ、インシデントの優先度、インシデントの状態、またはこれらの任意の組み合わせでフィルター処理します。
...
環境の現在の構成パラメーターを取得するには、 `configByEnvironment`クエリを実行します。

<CollapserGroup>
  <Collapser
    className="freq-link"
    id="configbyenvironment"
    title="ConfigByEnvironment クエリ"
  >
    ### リクエスト
    ...
    対応する`mutations`を使用して、 `issueTt` l および`gracePeriod`環境構成パラメーターを変更することもできます。
...
`mutations`を使用すると、 issueId によって特定の問題を確認または確認解除して解決できます。
...
2 export API_KEY=my_api_key を使用してシェルにエクスポートします
