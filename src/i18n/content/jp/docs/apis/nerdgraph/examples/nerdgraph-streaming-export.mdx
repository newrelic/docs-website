...
* ストリーミング ルールを作成または管理するための[組織設定](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#admin-settings)を持つ[コアまたはフル プラットフォーム ユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type)。
...
5. アカウント番号を入力し、名前ボックスに目的の配信ストリーム名を入力します。
6. ポリシーを作成します。
