...
<Steps>
  <Step>
    ...
    1. Amazon Kinesis Data Firehose にアクセスします。
    2. 配信ストリームを作成します。
...
これらの手順が完了したら、NerdGraph を使用してエクスポート ルールを設定できます。 詳細については、 [「NerdGraph 呼び出しの重要なフィールド」](#fields)を参照してください。
...
<Steps>
  ...
  <Step>
    ...
    4. <DNT>**Retention**</DNT>の場合は、 <DNT>**Delete**</DNT> `Cleanup policy` 、および必要な`Retention time (hrs)`を選択します。

    <Callout variant="important">
      現在、ストリーミング エクスポートは、 <DNT>**Compact**</DNT>保持ポリシーを持つイベント ハブではサポートされていません。
    </Callout>

    5. イベント ハブが作成されたら、イベント ハブをクリックします。
  </Step>
...
## NerdGraph 呼び出しの重要なフィールド [#fields]
