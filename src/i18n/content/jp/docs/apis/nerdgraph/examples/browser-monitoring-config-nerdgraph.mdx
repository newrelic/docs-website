...
## 難読化ルールの作成 [#create-obfuscation-rules]

New RelicプラットフォームまたはNerdGraphから直接取り込み難読化ルールを作成することで、機密データが収集される前にマスクできます。これらのルールは、すべてのブラウザイベントに適用されます。プラットフォームまたはNerdGraphを介して設定されたルールは、APMが挿入されたアプリケーション、コピー/ペーストによるインストレーション、およびNPMインストレーションに適用されます。プラットフォームまたはJavaScriptを介してルールを作成するには、[Browserエージェントでの機密データの難読化](/docs/browser/new-relic-browser/configuration/obfuscate-browser-agent-data/)を参照してください。

NerdGraph経由で難読化ルールを作成するには：

<CollapserGroup>
  <Collapser id="retrieve" title="ステップ1：難読化ルールの取得">
    次のクエリを使用して、ブラウザアプリケーションの現在の難読化ルールを取得します：
    ...
    <Callout variant="important">
      `agentApplicationSettingsUpdate`ミューテーションは既存のルールを提供された新しいルールのセットで上書きするため、更新する前に現在のルールを取得する必要があります。既存のルールを保持したい場合は、ミューテーションで送信する新しいルールのセットにそれらを含めるようにしてください。
    </Callout>
  </Collapser>

  <Collapser id="update" title="ステップ2：難読化ルールの更新">
    ミューテーション`agentApplicationSettingsUpdate`を使用して難読化ルールを更新します。次のプロパティを定義することで、新しいルールを追加したり、既存のルールを変更したりできます：

    * `regex`：難読化するデータに一致する正規表現パターン。たとえば、`\b\d{3}-?\d{2}-?\d{4}\b`を使用して社会保障番号に一致させることができます。
    * `replacement`：一致したデータを置き換える文字列。たとえば、`XXX-XX-XXXX`を使用して社会保障番号をマスクできます。
    * `eventFilter`：ルールを適用するブラウザのイベントタイプの配列。たとえば、`["AjaxRequest"]`を指定して、AJAXリクエストイベントにのみルールを適用することができます。
...
## AJAXペイロードキャプチャの構成 [#configure-payload-capture]

NerdGraphを使用して、ブラウザアプリケーションのAJAXペイロードキャプチャ設定を取得および更新できます。この設定は、AJAX呼び出しのリクエストおよびレスポンスペイロードをキャプチャするかどうかを制御し、[アプリケーション設定のペイロードキャプチャオプション](/docs/browser/new-relic-browser/configuration/configure-ajax-request-events/#payload-capture)に対応します。

ペイロードキャプチャ設定は、次の2つのステップで有効にできます：

1. [現在のペイロードキャプチャ](#query-payload-capture)設定を取得します。

2. [`capturePayloads`モード](#update-payload-capture)を次のいずれかの値に設定します：

   * `none`：ペイロードデータはキャプチャされません。
   * `failures`：ステータスコードが`0`、コードが`400`以上、または`response body`にGraphQLエラーシグネチャがあるrequestsのペイロードのみをキャプチャします。
   * `all`：すべてのrequestsのペイロードをキャプチャします。

<CollapserGroup>
  <Collapser id="query-payload-capture" title="ペイロードキャプチャ設定をクエリする">
    次のクエリを使用して、ブラウザアプリケーションの現在のペイロードキャプチャ設定を取得します：
  ...
  <Collapser id="update-payload-capture" title="ペイロードキャプチャ設定を更新する">
    `agentApplicationSettingsUpdate`ミューテーションを使用してペイロードキャプチャ設定を更新します：
    ...
    変数で`guid`および`capturePayloads`オプションを設定します：
