...
<CollapserGroup>
  ...
  >
    さまざまなエンティティ タイプには、特定のデータが関連付けられています。この NerdGraph クエリの例では、エンティティの種類に関係なくすべてのエンティティの名前と、APM のエラー率を要求します。 <InlinePopover type="browser"/>、 そして<InlinePopover type="mobile"/>エンティティ。
