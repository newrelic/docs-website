...
NRQL クエリは、New Relic プラットフォーム[クエリ ビルダー](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder)または NerdGraph で実行できます。NerdGraph を使用すると、[複数のアカウントにまたがる](#cross-account-query)クエリや[非同期クエリ](/docs/apis/nerdgraph/examples/async-queries-nrql-tutorial)の実行など、UI では実行できないことを実行できます。
