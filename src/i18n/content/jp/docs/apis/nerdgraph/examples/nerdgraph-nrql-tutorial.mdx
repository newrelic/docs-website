...
生データを返すだけでなく、アプリケーションで使用するデータの埋め込み可能なチャート リンクを取得することもできます。 たとえば、 [トランザクション](/docs/insights/insights-data-sources/default-data/apm-default-event-attributes-insights#transaction-event)の単一のカウントの代わりに、時間の経過に伴うバケット化されたカウントの時系列を示す[グラフ](/docs/insights/use-insights-ui/manage-dashboards/chart-types#widget-types)を作成できます。 `embeddedChartUrl`を使用してクエリに`TIMESERIES`を追加します:
