...
<CollapserGroup>
  ...
  >
    ...
    * <DoNotTranslate>**Built-in suggestions.**</DoNotTranslate> 各[イベント タイプには、](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#event)独自の推奨属性セットが付属しています。 これらは、その重要性と人気に基づいて New Relic によって選択された属性です。

    * <DoNotTranslate>**Usage-based suggestions.**</DoNotTranslate> 一部の属性の提案は、アカウントで頻繁に使用されているクエリに基づいています。 これらの提案には、カスタムアトリビュートが含まれます。

    * <DoNotTranslate>**Role restriction.**</DoNotTranslate> [制限付きユーザーは、](/docs/accounts/accounts/roles-permissions/users-roles)アカウント関連のファセット提案にアクセスできません。
