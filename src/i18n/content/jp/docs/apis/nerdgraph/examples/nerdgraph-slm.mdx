...
New Relic を使用すると、アプリケーションに対して [サービスレベル](https://docs-preview.newrelic.com/docs/service-level-management/create-slm) を実装し、 [計画セッションやインシデント対応の際に UI から簡単に](https://docs-preview.newrelic.com/docs/service-level-management/consume-slm) 結果を消費し、目的を望ましいユーザーエクスペリエンスに調整するために構成を段階的に反復することができます。
...
<Callout variant="important">
  サービスレベルを作成するためには、APIキーに [、](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model) 、 [メトリックルール](/docs/data-apis/convert-to-metrics/analyze-monitor-data-trends-metrics/) を修正・削除する権限を持つロールが必要です。
</Callout>

## SLOでSLIを作る [#create-sli-slo]

SLIとSLOの作成と編集 [SLIとSLOの設定における基本的な概念（SLIが関連するエンティティなど）については、](https://docs-preview.newrelic.com/docs/service-level-management/create-slm#sli-entity) を参照してください。また、APMサービスやブラウザアプリケーションの最も一般的な指標の例については、そのドキュメントを参照してください。
