...
新しいロールに割り当てる各権限の一意の識別子を取得したら、次のミューテーションを使用して[ロール](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#roles)を作成します。
...
* `id`: 新しく作成されたカスタム ロールの一意の ID を返します。
  ...
  <Callout variant="important">
    ...
    * 例`permissionIds: [1, 2, 3]` 、権限クエリから取得した実際の権限 ID に置き換えます。
  </Callout>
