...
アラート通知チャネルを作成するには、作成する通知チャネルの特定の種類 (電子メール、Slack など) と、その構成に必要な詳細 (チャネルによって異なります) を把握する必要があります。タイプ）。通知チャネルを作成したら、1 つ以上の [アラート ポリシー](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/ "アラートと適用されたインテリジェンス| NewRelicのドキュメント")に関連付けることができます。関連付けられると、条件が違反されたときに、これらのチャネルはポリシーから通知を受け取ります。
...
アラート通知チャネルを作成するだけでは十分ではありません。チャネルを作成したら、それを 1 つ以上の [ポリシー](/docs/alerts-applied-intelligence/new-relic-alerts/alert-policies/ "アラートと適用されたインテリジェンス| NewRelicのドキュメント")に関連付ける必要があります。ポリシーに関連付けられると、チャネルは、そのポリシーの条件がしきい値を超えたときにアラート通知を受信できるようになります。
