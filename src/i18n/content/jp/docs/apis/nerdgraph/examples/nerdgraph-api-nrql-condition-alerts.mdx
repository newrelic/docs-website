...
2. 次のいずれかを実行して、関連する`policyID`を見つけます。
   ...
   * **[one.newrelic.com](https://one.newrelic.com/ "リンクが新しいウィンドウで開きます。")**にアクセスし、上部のナビゲーションで\[**Alerts & AI(アラートとAI)** ]をクリックしてから、\[**Alert conditions (Policies)(アラート条件（ポリシー）** ]をクリックします。ポリシーを選択します。ポリシー名の下でIDを見つけます。
...
<CollapserGroup>
  ...
  >
    ...
    したがって、返される型が静的条件の場合は、オブジェクト内の`valueFunction`が返され、返される型がベースライン条件の場合は、代わりに`baselineDirection`が返されます。応答例は次のとおりです。
