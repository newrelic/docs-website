...
[NerdGraphエクスプローラー](https://api.newrelic.com/graphiql)でフィールド定義を表示できますが、 `LOCATION` 、 `PERIOD` 、および`TAGS`をよりよく理解するのに役立つ場合があります。
...
<CollapserGroup>
  ...
  >
    ...
    * `EVERY_MINUTE` 毎分。
    * `EVERY_N_MINUTES`、ここで、「N」は分です。たとえば、 `EVERY_10_MINUTES`です。
    * `EVERY_HOUR` 1時間ごとに。
    * `EVERY_N_HOURS`、ここで、「N」は時間です。たとえば、 `EVERY_12_HOURS`です。
    * `EVERY_DAY` 1日1回。
  </Collapser>
  ...
  >
    ...
    * <var>BROWSER_TYPE</var>

      ：モニターが使用するブラウザー。受け入れられる値は「CHROME」のみです。

    * <var>BROWSER_VERSION</var>

      ：モニターで使用されているブラウザーのバージョン。受け入れられる値は「100」のみです。

    [スクリプト化されたAPIモニター](#scripted-API)の場合：
