...
* 関連するユーザー権限：合成UIの使用に適用されるのと同じ[Synthetics権限](/docs/synthetics/synthetic-monitoring/administration/user-roles-synthetic-monitoring)がNerdGraphの使用に適用されます
...
<CollapserGroup>
  ...
  >
    ...
    * <var>RUNTIME_TYPE_VERSION</var>
      ...
      ：モニターで使用されるランタイムタイプのバージョン。受け入れられる値は`16.10.0`のみです。
