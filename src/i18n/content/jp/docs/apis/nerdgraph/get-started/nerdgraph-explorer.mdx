...
<Steps>
  <Step>
    ...
    1. <DoNotTranslate>[one.newrelic.com > All capabilities > Apps > NerdGraph API explorer](https://one.newrelic.com/nerdgraph-graphiql)</DoNotTranslate>に移動します。
  ...
  <Step>
    ...
    赤い<DoNotTranslate>**Execute query**</DoNotTranslate>ボタンをクリックします。 結果はリクエストとほぼ同じ形になることに注意してください。 クエリビルダー内のすべてのフィールドは、使用可能なすべてのデータ型とその属性を記述する GraphQL スキーマを構成します。 各フィールドの詳細については、フィールドの横にある虫眼鏡アイコンをクリックしてください。
  ...
  <Step>
    ...
    * アカウント`id` (この値は、左下隅にある自分の名前をクリックしてから
