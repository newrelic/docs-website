...
[日本](/docs/accounts/accounts-billing/account-setup/choose-your-data-center)（JP）エンドポイント：
...
<CollapserGroup>
  <Collapser id="multipart-form-data-request" title="multipart/form-data を含むリクエスト">
    #### サポートされているフィールド
  ...
  <Collapser id="application-json-request" title="application/json によるリクエスト">
    #### サポートされているフィールド
...
<CollapserGroup>
  <Collapser id="create-a-new-table" title="新しいテーブルを作成する">
  ...
  <Collapser id="update-a-table" title="既存のテーブルを更新する">
...
<Collapser id="download-an-existing-table" title="既存のテーブルをダウンロードする">
...
<Collapser id="delete-an-existing-table" title="既存のテーブルを削除する">
...
<Collapser id="list-tables" title="アカウント用に更新されたリストテーブル">
