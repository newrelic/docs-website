...
米国ベースのアカウント（デフォルト）の場合、次のエンドポイントを使用します：
...
[JPデータセンター](/docs/accounts/accounts-billing/account-setup/choose-your-data-center)のアカウントの場合、次のエンドポイントを使用します：
...
<CollapserGroup>
  ...
  <Collapser className="freq-link" id="create-monitor" title="モニターを作成する">
    最新のランタイムを使用して合成モニターを管理するには、 [NerdGraph APIを](/docs/apis/nerdgraph/examples/synthetics-api/overview/)ご利用ください。
