...
<CollapserGroup>
  <Collapser id="get-all-monitors" title="モニターのスクリプトを取得する">
    お使いのアカウント用の合成モニターで特定の`SCRIPT_BROWSER`または`SCRIPT_API`に関連するスクリプトを表示するには、GETリクエストを$API\_ENDPOINT`/v3/monitors/$MONITOR_ID/script`に送信します。例：
  ...
  <Collapser id="update-monitor-script" title="モニターのスクリプトを更新する">
    特定の`SCRIPT_BROWSER`または`SCRIPT_API`モニターに関連するスクリプトを更新するには、`/v3/monitors/$MONITOR_ID/script`を含むJSONペイロード（必須）と共に、PUTリクエストを$API\_ENDPOINT`scriptText`に送信します。
