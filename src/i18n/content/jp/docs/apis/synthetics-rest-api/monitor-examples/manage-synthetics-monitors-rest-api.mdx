...
<CollapserGroup>
  ...
  >
    ...
    ```
    curl -v \
         -H 'X-Api-Key:<var>{Admin_User_Key}</var>' <var>{API_Endpoint}</var>/v3/monitors
    ```
  ...
  <Collapser
    ...
    title="特定のモニターを取得する"
  >
    単一のSyntheticsモニターを表示するには、`{API_Endpoint}/v3/monitors/{id}`にGETリクエストを送信します。次の例では、<var>&#x7B;id}</var>を特定のモニターIDに置き換えてください。

    ```
    curl -v \
         -H 'X-Api-Key:<var>{Admin_User_Key}</var>' <var>{API_Endpoint}</var>/v3/monitors/<var>{id}</var>
    ```
  ...
  >
    ...
    次の例では、<var>&#x7B;id}</var>を特定のモニターIDに置き換え、[Synthetic REST APIの属性](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api)を独自の値に置き換えてください。

    ```
    curl -v \
         -X PUT -H 'X-Api-Key:<var>{Admin_User_Key}</var>' \
         -H 'Content-Type: application/json' <var>{API_Endpoint}</var>/v3/monitors/<var>{id}</var> \
  ...
  >
    ...
    ```
    curl -v \
         -X PATCH -H 'X-Api-Key:<var>{Admin_User_Key}</var>' \
         -H 'Content-Type: application/json' <var>{API_Endpoint}</var>/v3/monitors/<var>{id}</var> \
  ...
  <Collapser
    ...
    title="既存のモニターを削除する"
  >
    New Relic Syntheticsで既存のモニターを削除するには、`{API_Endpoint}/v3/monitors/{id}`にDELETEリクエストを送信します。次の例では、[<var>&#x7B;id}</var>](/docs/apis/synthetics-rest-api/monitor-examples/payload-attributes-synthetics-rest-api)を特定のモニターIDに置き換えてください：

    ```
    curl -v \
         -H 'X-Api-Key:<var>{Admin_User_Key}</var>' \
         -X DELETE <var>{API_Endpoint}</var>/v3/monitors/<var>{id}</var>
    ```
  ...
  >
    ...
    ```
    curl -v \
         -X GET -H 'X-Api-Key:<var>{Admin_User_Key}</var>' <var>{API_Endpoint}</var>/v1/locations
    ```
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    お使いのアカウント用のNew Relic Syntheticsで特定の`SCRIPT_BROWSER`または`SCRIPT_API`モニターに関連するスクリプトを表示するには、GETリクエストを`{API_Endpoint}/v3/monitors/{id}/script`に送信してください。<var>&#x7B;id}</var>を特定のモニターIDに置き換えてください。例えば：

    ```
    curl -v
         -H 'X-Api-Key: <var>{Admin_User_Key}</var>' 
         <var>{API_Endpoint}</var>/v3/monitors/<var>{id}</var>/script
    ```
  ...
  >
    ...
    <var>&#x7B;id}</var>を特定のモニターIDに置き換えてください。これは、スクリプトの例です。
...
<CollapserGroup>
  ...
  >
    ...
    then
      ...
      done < <(curl -sS -i  -X POST -H "X-Api-Key:<var>$adminAPIKey</var>" -H 'Content-Type: application/json' https://synthetics.newrelic.com/synthetics/api/v3/monitors -d "$payload")
