...
<Callout variant="important">
  2024 年 10 月 22 日をもって、コンテナ化された プライベートミニオン (1分間あたりの呼出し回数) バージョンと レガシー合成バージョン[のサポートを終了](/whats-new/2024/04/whats-new-04-09-eol-synthetics-runtime-cpm)します。 2024 年 6 月 30 日以降、レガシー合成バージョンを使用した新しいモニターの作成がブロックされます。 外形監視 REST API 、レガシー合成バージョンを使用したモニター作成のみをサポートします。 パフォーマンスの低下を避けるため、最新のランタイムを使用して外形監視モニターを管理するには、 [NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-synthetics-tutorial/)を使用してください。
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    さらに、REST API 経由で<DoNotTranslate>**add the script for a scripted monitor**</DoNotTranslate>に[追加の API エンドポイントを呼び出し](#scripted-api-monitors-api)、作成したばかりのモニターのスクリプトを送信します。 [検証済みのスクリプト実行を](/docs/synthetics/new-relic-synthetics/private-locations/verified-script-execution-private-locations)有効にしてプライベートロケーションを使用している場合は、 [「検証済みのスクリプト実行があるスクリプトの場所」を](#scriptlocations)参照してください。
  ...
  >
    ...
    全て必須項目です。 ただし、モニター<DoNotTranslate>**cannot**</DoNotTranslate>の`TYPE`は変更する必要があります。
