...
フィルター ロジックを追加すると、システムは過去 7 日間のインシデント データを使用して[シミュレーションを](#simulations)自動的に実行し、決定を適用する前に検証できるようにします。
...
* <DNT>**Simulation**</DNT>シミュレーション相関では、シミュレートされた条件下で 2 つの別々のインシデントを分析してそれらの関係を理解します。これらのインシデントは、同じ根本的な問題から発生する場合もあれば、異なる問題から発生する場合もあります。重点は、個々のインシデント間の潜在的な原因要因または共通の特徴を特定することにあります。シミュレーションを使用すると、相関ロジックをリアルタイムで適用する前に、履歴データに対してテストおよび検証できます。

* <DNT>**Real-time correlation (decisions)**</DNT>: 対照的に、相関関係は個別の問題を対象としており、各問題には複数のインシデントが含まれる可能性があります。 AI モニタリングは、これらの複数のインシデントにわたるパターンを検出して接続し、より効率的な相関関係を実現するために根本的な問題を特定します。 相関関係ではライブ データ ストリームが活用され、プロンプトの特定と新たな問題への対応が可能になります。

### シミュレーションの使用 [#simulations]

シミュレーションでは、データの最後の 1 週間からの 2 つの別々のインシデントを分析して相関ロジックをテストし、発生した相関の数を表示します。これにより、問題のリアルタイム相関関係に適用する前に、意思決定ロジックを検証できます。シミュレーション時に表示される決定プレビュー情報の詳細は次のとおりです。
