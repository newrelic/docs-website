...
<InlinePopover type="alerts" />では、<DNT>**entity**</DNT>はアプリケーション、ブラウザ、ホストなどの監視のターゲットとして定義されます。[アラートUI](https://one.newrelic.com/alerts/)には、選択可能なエンティティが表示されます。REST APIおよびAPI Explorerを使用して、条件のエンティティを[追加](https://api.newrelic.com/docs/#/Alerts%20Entity%20Conditions/put_alerts_entity_conditions__entity_id__json)または[削除](https://api.newrelic.com/docs/#/Alerts%20Entity%20Conditions/delete_alerts_entity_conditions__entity_id__json)することもできます。
