  ...
  <Step>
    ...
    <CollapserGroup>
      <Collapser
        ...
        title="異常しきい値 (推奨)"
      >
        あなたのチームはこのアラート条件を作成しているので、Web レスポンス時間が通常より長くなると通知されます。 しかし、Web の トランザクションタイム がどれだけ長くなるかは気にせず、 トランザクションタイム が異常な動作をしているかどうかだけを知りたいとします。 この特定のユースケースでは、 <DNT>**anomaly threshold**</DNT>を使用することをお勧めします。 当社の異常検出機能は、システムの通常の動作を把握するためにデータを継続的に評価します。 異常閾値を設定すると、Web レスポンスタイムが期待されるパフォーマンスから逸脱した場合に、異常検出機能を使用してチームを集計できます。 ウェブ トランザクションタイムが異常に動作しているかどうかのみを知りたい場合は、_すべての_偏差の通知が必要なので、 <DNT>**upper and lower**</DNT>を選択します。 ただし、ウェブレスポンス時間が通常より_長く_かかった場合にのみアラートを通知したい場合は、 <DNT>**upper only.**</DNT>
