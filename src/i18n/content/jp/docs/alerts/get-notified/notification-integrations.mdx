...
<CollapserGroup>
  <Collapser className="freq-link" id="jira" title="アトラシアン Jira">
    ...
    * New Relic 問題の承認は、Jira 問題のステータスが`in-progress`に変化するとトリガーされます。
    ...
    デフォルトのフィールド値を持つテスト通知をクリックして、Jira の問題を表示します。成功すると、Jira で大人気イベントを表示するリンクが表示されます。
  ...
  <Collapser className="freq-link" id="email" title="メール">
    [ワークフロー](/docs/alerts/get-notified/alert-event-workflows/)の通知チャネルとして<DNT>**Email**</DNT>を選択すると、電子メールの送信先が自動的に作成されるため、 <DNT>**Destinations**</DNT>メニューからこれを構成する必要はありません。各電子メールの送信先は、それに関連付けられたワークフローに固有であるため、送信先フィードでは重複として表示される場合があります。
  ...
  <Collapser className="freq-link" id="pagerduty" title="PagerDuty">
    New Relic PagerDutyと統合して、 PagerDuty全体イベントを自動的に作成、更新、確認、解決します。
