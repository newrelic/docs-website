...
* **ワークフローオートメーション**： [ワークフロー](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/)で利用可能。 アラート条件に違反すると、[自動化されたインシデント対応ワークフロー](/docs/workflow-automation/setup-and-configuration/create-destinations#trigger-from-alerts)がトリガーされます。
