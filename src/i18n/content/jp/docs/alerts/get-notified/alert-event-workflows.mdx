...
問題は、症状の根本的な問題を説明する人気イベントのグループです。 新しい集中イベントが作成されると、インシデント インテリジェンスが問題をオープンし、他の未解決の問題の相関関係を評価します。 詳細については、 [「インシデント インテリジェンスの使用」を](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/use-incident-intelligence/)参照してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        この問題は、すべてのイベントが終了したか、TTL が終了したか、または長期間`Inactive`になっていたため、クローズされました。
      </td>
    ...
    <tr>
      ...
      <td>
        この問題の集中イベントが「高優先度」から「重要」問題になりました
      </td>
    ...
    <tr>
      ...
      <td>
        人気イベントがその号に追加されたか、盛んなイベントがクローズされたか、別の号がこの号にマージされました
      </td>
...
3. ワークフローを使用して、問題をフィルタリングし、関連する宛先に送信します。クエリを作成して、適切な問題をフィルター処理できます。
   ...
   <Callout variant="tip">
     `WARNING`イベントに個別の通知を設定するには、優先度が`HIGH`の問題でフィルタリングします。
   </Callout>
...
6. オプションです。データを充実させる
   ...
   <CollapserGroup>
     <Collapser className="freq-link" id="enrichments" title="データを充実させる方法">
       現在、Webhook および Jira の[送信](/docs/alerts-applied-intelligence/notifications/destinations/)先には JSON または数値出力のみを送信できます。さらに、Enrich データの結果を指す[ワークフロー変数](https://docs.newrelic.com/docs/alerts/get-notified/custom-variables-alert-event-workflows/)を使用して、JSON を AWS EventBridge に送信することもできます。また、画像は Slack、PagerDuty、ServiceNow (アプリ)、電子メールなどの他の送信先にのみ送信できます。 Webhook と Jira でより良い結果を得るには、 `count` 、 `min` 、 `max`などの単一値出力を持つクエリを使用します。
       ...
       * エンリッチメントの例については、 [「ワークフロー データ エンリッチメントの例」](/docs/alerts/get-notified/custom-variables-alert-event-workflows/#enrichment-examples)を参照してください。
...
7. 通知する1つまたは複数の送信先を選択し、任意のメッセージを追加します。
   ...
   <Callout variant="tip">
     任意の宛先チャネルで入力を開始すると、変数メニューが開きます。[変数](/docs/alerts/get-notified/custom-variables-alert-event-workflows)の名前が表示されます。これは実行時に変数の値に置き換えられます。エンリッチャーの結果を使用するには、その名前を使用します。
   </Callout>
