...
New Relic アラート[ワークフロー](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows)に使用される変数の説明。
...
<CollapserGroup>
  ...
  >
    アラート条件クエリがファセット化されている場合は、クエリでインシデントをトリガーする`FACET`値を使用できます。 たとえば、アラート条件が次の場合:
