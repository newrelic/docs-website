...
[この移行](https://support.newrelic.com/s/hubtopic/aAX8W0000008dKO/plan-to-upgrade-alert-notification-channels-to-workflows-and-destinations)はチームにとって何を意味しますか？以前の通知モデルでは、チームはさまざまな閾値とパラメーターを使用してアラート条件を作成していました。その条件が特定のポリシーに関連付けられており、違反が発生した場合に、チームにすぐに知らせたい場合は、<DNT>**notification settings**</DNT>を追加していました。通知設定により、どのデータをどこに送信するかがNew Relicに指示されていました。
