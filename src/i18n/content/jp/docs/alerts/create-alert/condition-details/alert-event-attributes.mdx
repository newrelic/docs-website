...
集中イベントを開始する条件によりイベントが生成され、重要な情報が下流に渡されます。

イベントの定義やその他の用語の詳細については、[用語集を](/docs/new-relic-solutions/get-started/glossary/)参照してください。

## まとめイベントとは何ですか? [#definition]

[一括イベント](/docs/new-relic-solutions/get-started/glossary#alert-event)とは、アラートポリシーで定義された条件に違反したイベントです。 このイベントにはさまざまな属性 (メタデータ) が添付されており、さまざまな機能でさまざまな属性を使用できます。

<Callout variant="important">
  まとめイベントとは、アラート機能を決定するために使用される概念です。 関連するプロパティの一部を NerdGraph 経由で書き込むことはできますが、まとめイベントを直接書き込むことはできません。
</Callout>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        NRQL またはインフラストラクチャ条件のみ。アラート条件の[タイトル](docs/alerts/create-alert/condition-details/title-template)または[説明テンプレート](/docs/alerts/create-alert/condition-details/custom-alert-event-descriptions)では使用できません。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        インシデントの一意の識別子。アラート条件の[タイトル](docs/alerts/create-alert/condition-details/title-template)または[説明テンプレート](/docs/alerts/create-alert/condition-details/custom-alert-event-descriptions)では使用できません。
      </td>
    </tr>
