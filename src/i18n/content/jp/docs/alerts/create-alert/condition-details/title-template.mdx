...
たとえば、電子商取引サイトを運営していて、サイトが 404 応答エラーを生成するたびにインシデントをトリガーするアラート条件を作成したとします。 タイトル テンプレートを使用すると、この条件によるインシデントを簡単に識別できます。

このアラート条件に`high error rate - checkout`という名前を付けました。 ただし、インシデントのタイトルには、エラーが発生している場所を反映させる必要があります。 この場合、このインシデントのエラーはチェックアウト エンティティで発生しました。
...
* **タグ属性**: タグには、 `{{tag.TAG_NAME}}`形式を使用します。

* **ファセット句タグ**: NRQL 条件で`FACET`句が使用されている場合は、 `tag`フォーマットを使用して、タイトル テンプレートでそれらの値を使用できます。 たとえば、NRQL クエリに`FACET hostName, cluster`含まれている場合は、次のように使用できます。
