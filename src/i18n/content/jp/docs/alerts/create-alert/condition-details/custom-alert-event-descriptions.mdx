...
一部のタイプの条件については、より適切なイベント応答のため、または他のシステムで使用するために、有用な情報を下流に渡す説明テンプレートを定義できます。 説明テンプレートは、UI または GraphQL を使用して作成できます。
...
一部の[タイプの条件](#requirements)については、条件とその人気イベントに関する情報をダウンストリームに渡す説明テンプレートを作成できます。 説明を使用すると、次のことが可能になります。
...
<DNT>**Description template**</DNT>フィールドを使用すると、一般イベントの生成時に一般的なセクションを特定の値に置き換える一般的なテンプレート構造を使用できます。 たとえば、説明にこれを使用したとします。
...
この条件に起因する集中イベントに詳細を入力すると、次のような通知が届きます。
...
説明テンプレートで使用できるプロパティは、[集中イベント](/docs/alerts/create-alert/condition-details/alert-event-attributes)に添付されているプロパティのサブセットです。 利用可能な属性とその使用方法に関する詳細は以下のとおりです。

* まとめイベント イベントのプロパティ:[説明テンプレートで使用できる標準プロパティ](/docs/alerts/create-alert/condition-details/alert-event-attributes)の場合は、形式`{{AN_ATTRIBUTE}}`を使用します。 例えば：
...
* タグ属性：タグには、 `{{tags.TAG_NAME}}`の形式を使用します。タグには次の2つのタイプがあります。
  ...
  * エンティティ関連タグ: <DNT>**For infrastructure conditions only.**</DNT>利用可能な[エンティティ関連タグを](/docs/new-relic-one/use-new-relic-one/core-concepts/tagging-use-tags-organize-group-what-you-monitor#)見つけるには、 [エンティティ エクスプローラー](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer-view-performance-across-apps-services-hosts)に移動してサービスの<DNT>**Metadata and tags**</DNT>下を調べるか、まとめイベントの詳細を表示します。 説明テンプレートでタグを使用する例を次に示します。
