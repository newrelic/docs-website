...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        位置情報チェックが失敗した場合、**最大24時間まで**、チェックが成功するまで失敗とみなされます。Multi-Location 外形監視条件の評価バッファには 24 時間の TTL (生存時間) があります。 例えば、ある場所でサービスが停止した後、すぐに利用可能になる場合でも、予定されたチェックが成功したと報告されるまでは、サービスが停止した状態とみなされます。24時間以内に復旧信号が受信されない場合、その障害情報は最終的にバッファから削除されます。
      </td>
    </tr>
  </tbody>
</table>
...
アラートREST APIを使用してマルチロケーション条件を管理するには、[REST APIエクスプローラー](https://api.newrelic.com/docs/#/Deprecation%20Notice%20-%20Alerts%20Location%20Failure%20Conditions)を使用してください。