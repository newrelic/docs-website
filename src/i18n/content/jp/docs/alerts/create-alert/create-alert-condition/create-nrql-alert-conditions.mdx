...
<Callout variant="important">
  イベント（行）が報告されない場合、上記の変更を行っても信号損失は回避`cannot`。イベントのレポートが完全に停止した場合にアラートをトリガーするための`Lost Signal Threshold`を設定または維持することをお勧めします。
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        NRQL条件の場合、各イベントに追加されるカスタムの[説明](/docs/alerts/new-relic-alerts/defining-conditions/alert-condition-descriptions)を作成できます。これらの説明は、特定のアラートに関連付けられたメタデータに基づく変数置換を強化します。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        信号損失検出を使用すると、データ（テレメトリー信号）が失われたと見なされる時点でアラートを出力できます。サービスまたはエンティティがオンラインではなくなったか、定期的なジョブの実行に失敗した可能性を示しています。エラーカウントなどの散発的なデータのアラートが、信号を受信していないときに確実に終了するためにも使用できます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * アラートと通知に含まれる<DNT>**Add details**</DNT>ページの条件について、アラートの説明を記入します。
        * アラート対応に関する自社の手順を含めるには、ランブックのURLを追加します。この情報をカスタムアラートの説明に追加することもできます。
      </td>
    </tr>
...
* 「x分以上」の閾値の場合：評価ウィンドウがリセットされたため、アラートが報告されるまで少なくともx分の遅延が発生します。
...
* <DNT>
  ...
  * 現在未解決のアラートをすべて閉じる：これにより、特定の信号に関連する未解決のアラートがすべて終了します。必ずしも、条件のすべてのアラートが終了するとは限りません。一時的なサービスまたは散発的な信号で警告している場合は、アラートが適切に終了するように、このアクションを選択することをお勧めします。このGraphQLノード名は[`closeViolationsOnExpiration`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal)です。
  * 新しいアラートを開く：これにより、信号が損失したと見なされると、新しいインシデントが発生します。これらのアラートは、信号の損失が原因であることを示します。アラートプリファレンスに基づいて、通知がトリガーされます。このGraphQLノード名は[`openViolationOnExpiration`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal)です。
  * 上記の両方のアクションを有効にすると、最初に未解決のアラートがすべて終了します。次に、信号が損失すると、新しいアラートが発生します。
  * 予想どおりに終了した場合は「信号損失」アラートを発生させないようにします。信号が終了すると予想される場合は、新しいアラートを開かないように設定できます。これは、特定のタイミングで信号損失が発生することがわかっていて、その信号損失に対して新しいアラートを開きたくない場合に便利です。このGraphQLノード名は[`ignoreOnExpectedTermination`](/docs/apis/nerdgraph/examples/nerdgraph-api-loss-signal-gap-filling/#loss-of-signal)です。

<Callout variant="important">
  <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>のときに信号損失アラートが発生しないようにするには、エンティティにタグ`termination: expected`を追加する必要があります。このタグは、信号が終了すると予想していたことを示します。詳細については[エンティティにタグを直接追加する方法](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/#add-tags)を参照してください。`hostStatus: shutdown`タグは、「信号損失」アラートの発生も防止します。詳細については[「報告しないホスト」の条件を作成する](/docs/infrastructure/infrastructure-alerts/create-infrastructure-host-not-reporting-condition/#create-condition)を参照してください。
</Callout>
...
4. 信号が失われたときに実行する操作を選択します。<DNT>**Close all current open alert events**</DNT>、<DNT>**Open new &quot;lost signal&quot; alert event**</DNT>、<DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>のオプションのいずれかまたはすべてを設定できます。これらのアクションによって、該当する状態に対して信号損失アラートがどのように処理されるかが制御されます。
...
7. <DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>を選択した場合は、信号損失アラートの発生を防ぐため、エンティティに`termination: expected`タグを追加する必要があります。詳細については[エンティティにタグを直接追加する方法](/docs/new-relic-solutions/new-relic-one/core-concepts/use-tags-help-organize-find-your-data/#add-tags)を参照してください。

<Callout variant="tip">
  ここで、<DNT>**Open new &quot;lost signal&quot; alert event**</DNT>と<DNT>**Do not open &quot;lost signal&quot; alert event on expected termination**</DNT>の両方をtrueに設定する理由について説明します。次のように考えてください。信号が失われたときに通知を受け取りたいとします。ただし、信号の停止が事前に予定されていて、その際には通知を受け取りたくないとします。この場合は両方をtrueに設定し、信号が失われると予想される場合は、関連するエンティティに`termination: expected`タグを追加します。
</Callout>

信号損失により発生したアラートは、次の場合に終了します。

* 信号が復旧した場合。新しく発生した信号損失アラートは、新しいデータが評価されるとすぐに閉じられます。
* それらが属する条件が期限切れになります。デフォルトでは、条件は3日後に期限切れになります。
* <DNT>**Close all current open alert events**</DNT>オプションを使用して、アラートを手動で閉じた場合。
...
<Callout variant="important">
  スライディングウィンドウのアラート条件を新規作成するか、または[評価リセット](#evaluation-resets)を引き起こす可能性のあるアクションを実行した直後、最初の集計ウィンドウの期間中の条件に「集約バッファ」を構築する時間が必要になります。その間、アラートはトリガーされません。単一の集計ウィンドウが経過すると、完全な「バッファ」が構築されて、条件は正常に機能します。
</Callout>
...
* <DNT>**None**</DNT>：（デフォルト）空の集計ウィンドウでアクションを実行したくない場合は、これを選択します。評価時に、空の集計ウィンドウは閾値期間タイマーをリセットします。たとえば、すべての集計ウィンドウに5分間の閾値を超えるデータポイントが必要であり、5つの集計ウィンドウの1つが空であるという条件が設定されている場合は、その条件はアラートにはなりません。
...
<Callout variant="tip">
  ...
  信号損失、ギャップの充填、これらの機能へのアクセスをリクエストする方法の詳細については、[こちらのサポートフォーラムの投稿](https://support.newrelic.com/s/hubtopic/aAX8W0000008b5p/announcing-new-relic-one-streaming-alerts-for-nrql-conditions)を参照してください。
</Callout>
...
* NerdGraphは推奨APIですが、REST APIを使用している場合、この設定は[アラートNRQL条件API](https://api.newrelic.com/docs/#/Alerts%20NRQL%20Conditions)の<DNT>**&quot;signal&quot;**</DNT>セクションにあるREST APIエクスプローラーにあります。
