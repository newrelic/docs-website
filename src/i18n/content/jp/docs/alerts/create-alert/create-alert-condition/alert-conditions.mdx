...
* [<DNT>**Alerts Overview**</DNT>ページ](/docs/alerts/smart-alerts-coverage)：スマートアラートを使用して多数のエンティティにStandardsカバレッジを一括適用できます（パブリックプレビュー）
...
ガイド付きモードを除き、アラート条件の作成手順は、以下に記載される手順と*まったく*同じです。実際に条件を設定する際は、[アラートのベストプラクティス](/docs/new-relic-solutions/best-practices-guides/alerts-applied-intelligence/alerts-best-practices)を参照し、条件の命名、タグの追加、効果的な閾値の設定、ノイズの削減に関するガイダンスをご確認ください。
