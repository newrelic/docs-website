  ...
  <Step>
    ...
    <CollapserGroup>
      ...
      <Collapser id="predict-alert" title="静的閾値を備えた予測アラート（パブリックプレビューで利用可能）">
        ...
        New Relicの<DNT>**Predictive Alerts**</DNT>は履歴データを使用して、静的閾値の違反を予測するモデルを構築します。どのくらい先の将来を予測するかを設定できます。違反が予想される場合は、予測的なイベントがトリガーされ、混乱が発生する前に対応できるようになります。

        <DNT>**Predictive Alerts**</DNT>を有効にするには：
        ...
        3. 将来どのくらいの期間に閾値を超える可能性があるかを予測するために、先読み時間を設定します。最大先読み時間は[ウィンドウ期間](/docs/alerts/create-alert/create-alert-condition/alert-conditions/#window-duration)の360倍です。
        ...
        <DNT>**Predictive Alerts**</DNT>の詳細については、[ドキュメント](/docs/alerts/create-alert/set-thresholds/predictive-alerts)を参照してください。
      </Collapser>
