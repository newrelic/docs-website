...
<CollapserGroup>
  ...
  >
    ...
    現在、サポートされている 2 つの集計関数は、 `count()` と `sum()`です。 `count()` 関数は、有効/良好/不良イベントの数をカウントするだけの場合に、イベント データで使用することをお勧めします。一方、 `sum()` は、イベント データまたはディメンション メトリクスに事前に集計されたカウンターがある場合に役立ちます。
