...
<CollapserGroup>
  <Collapser
    ...
    title="アラート"
  >
    インシデント インテリジェンスを有効にすることで、 <InlinePopover type="alerts"/>ポリシーを使用すると、監視している内容からコンテキストと相関関係を取得できます。 アラートからデータを取得するには:
...
提案された決定を有効にするには、 **Add to your decisions** \[決定に追加]をクリックします。有効化されると、決定はチームのメイン決定テーブルに表示されます。提案されたすべての決定では、作成者が New Relic AI (これは New Relic 応用インテリジェンスを指します) として表示されます。
