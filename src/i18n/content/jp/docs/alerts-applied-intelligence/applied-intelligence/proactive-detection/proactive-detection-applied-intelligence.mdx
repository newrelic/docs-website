...
アプライド マテリアルズのプロアクティブディテクションでは、 [APM](/docs/apm) で監視しているアプリケーションからの異常が、アクティビティストリームと異常情報フィードに自動的に表示されます。各異常をクリックすると、自動分析が行われます。異常の通知をSlackで配信したり、Webhookを設定して必要なときにメッセージを配信することができます。これらのイベントは、クエリ、カスタムダッシュボードの作成、アラートの送信に利用できます。Proactive Detectionの構成（対象となるアプリのグループ）を作成し、この構成をソースとして追加することで、異常がIncident Intelligenceを介して他のデータソースと自動的に相関するようになります。

## 要件

プロアクティブディテクションを使用するには、以下の条件を満たしている必要があります。

* [フルプラットフォームのユーザー](/docs/accounts/accounts-billing/new-relic-one-user-management/new-relic-one-user-model-understand-user-structure/#user-type) アクセスです。まだお持ちでない方は、 [New Relic アカウント](https://newrelic.com/signup) にサインアップしてください。
