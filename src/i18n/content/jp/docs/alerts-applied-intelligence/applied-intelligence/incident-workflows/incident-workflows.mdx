...
ワークフローは、ワークフロー フィルターに一致する課題の課題イベントによってトリガーされます。次のイベントは、ワークフローをトリガーし、関連する宛先エンドポイントに通知を送信します。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        問題の状態が CREATED から ACTIVATED に、または ACTIVATED から CLOSED に変わります。
      </td>

      <td>
        Webhook、Slack、Event Bridge、ServiceNow、メール、Jira、モバイル アプリ、PagerDuty
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        問題には 1 件のインシデントがあり、現在は 2 件のインシデントがあります
      </td>

      <td>
        Webhook、イベント ブリッジ、Jira、ServiceNow
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        この問題には 2 つのインシデントがオープンしており、現在は 1 つのインシデントがオープンしており、1 つのインシデントがクローズされています。
      </td>
    ...
    <tr>
      <td>
        CLOSE_INACTIVE または CLOSED_TTL
      </td>

      <td>
        非アクティブのため、問題はクローズされました
      </td>

      <td>
        課題に定義されたアイドル時間が経過し、課題が自動的にクローズされた
      </td>
    ...
    <tr>
      ...
      <td>
        ユーザーが New Relic の問題ページから問題をクローズまたは確認した
      </td>
    ...
    <tr>
      ...
      <td>
        問題の優先度が HIGH から CRITICAL に変更されました
      </td>
    ...
    <tr>
      ...
      <td>
        相関により 2 つの問題がマージされ、この問題はクローズされました
      </td>
    ...
    <tr>
      ...
      <td>
        相互関係により 2 つの問題がマージされ、他の問題はクローズされ、この問題は未解決のままになりました
      </td>

      <td>
        Webhook、Slack、Event Bridge、ServiceNow、メール、Jira、モバイル アプリ、PagerDuty (アカウント統合)
      </td>
    </tr>
  </tbody>
</table>
