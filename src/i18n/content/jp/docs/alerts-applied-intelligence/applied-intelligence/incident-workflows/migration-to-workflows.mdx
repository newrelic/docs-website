...
PagerDuty、Webhook、VictorOps、OpsGenie、および xMatters の通知のインシデント ID は、従来のインシデント ID を参照します。従来のインシデント ID はまもなく削除され、消費者は代わりに issue_id の使用を開始する必要があります。以前は、incident_id はすべての通知に存在していました。現在、最初の通知が送信されるまでにすべての問題に追加されるとは限りません。
