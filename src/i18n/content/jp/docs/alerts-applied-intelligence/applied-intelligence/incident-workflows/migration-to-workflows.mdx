...
ワークフローは、 <InlinePopover type="alerts"/>およびアプライドインテリジェンスの従来の通知システムに代わるものです。 これは朗報です。ワークフローは改良された柔軟な通知システムだからです。 ワークフローは、チームがスタックのより大きなコンテキスト内で潜在的なエラーを把握するのに役立ち、迅速かつ効率的なアクションを実行できます。
