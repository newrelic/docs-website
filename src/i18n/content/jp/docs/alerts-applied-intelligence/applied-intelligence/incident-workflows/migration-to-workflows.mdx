...
* issue_id が追加されました。消費者は、すべての統合を、インシデント ID の代わりに issue_id を使用するように切り替える必要があります。これは、インシデント ID がある時点で削除されるためです。
...
PagerDuty、Webhook、VictorOps、OpsGenie、および xMatters の通知のインシデント ID は、従来のインシデント ID を参照します。従来のインシデント ID はすぐに使用できるようになり、消費者は代わりに issue_id の使用を開始する必要があります。以前は、incident_id はすべての通知に存在していました。現在、最初の通知が送信されるまでにすべての問題に追加されるとは限りません。
