...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `SLIDE BY`句は、[スライディングウィンドウ](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-conditions-settings/#sliding-window-aggregation)と呼ばれる機能をサポートしています。スライディングウィンドウを使用すると、`SLIDE BY`データは、互いに重複する時間の「ウィンドウ」に収集されます。これらのウィンドウは、移動集計（移動平均など）が狭い時間枠からの集計よりも重要である場合に、変動の多い折れ線グラフを滑らかにするのに役立ちます。
...
null値の処理方法を決定するには、[アラート条件UI](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/understand-technical-concepts/streaming-alerts-key-terms-concepts/#signal-loss)で信号損失とギャップ充填の設定を調整します。
    ...
    <tr>
      ...
      <td>
        ...
        * `SyntheticCheck` データ：タイムアウトは3分かかる場合があるため、5分以上前のデータをクエリするとよいでしょう。
        ...
        さらに、クエリによって生成されるデータが断続的な場合、高度な信号設定の[`slide by`](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-conditions-settings/#sliding-window-aggregation)オプションの使用をご検討ください。
      </td>
    </tr>
...
* スライドごとの間隔を変更する（[スライディングウィンドウの集計](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-conditions-settings/#sliding-window-aggregation)条件のみ）
