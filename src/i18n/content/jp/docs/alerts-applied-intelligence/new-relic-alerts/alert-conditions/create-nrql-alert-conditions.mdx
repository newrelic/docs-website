...
2. 左側のペインで、**More views > Metrics explorer**を選択し、設定を追加します。

3. 右隅の<Icon name="fe-more-horizontal"/>アイコンをクリックし、**アラート条件を作成**を選択します。
...
4. [ポリシーからNRQLアラート条件を作成](#alert-condition-from-policy)で説明されている手順に従いますが、手順1は少し異なることに注意してください。これは、以前に設定したデータで作成済みのクエリが表示されるためです。
