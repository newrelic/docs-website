...
<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert Conditions & Policies**</DoNotTranslate>に移動して、ポリシーからNRQLアラート条件を作成します。次に、<DoNotTranslate>**+ New alert condition**</DoNotTranslate>をクリックします。
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert Conditions & Policies > + New alert condition**</DoNotTranslate>に移動します。
</figcaption>
...
<CollapserGroup>
  ...
  >
    1. <DoNotTranslate>**Build a query**</DoNotTranslate>
       ...
       オプションを選択します。このオプションを使用すると、信号を特定し、NRQLクエリを追加できます。クエリの追加でヘルプが必要な場合は、[NRQLの使用方法：クエリの仕組み](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-how-nrql-works/)を参照してください。
  ...
  >
    ...
    設定が完了したら、<DoNotTranslate>**Next**</DoNotTranslate>をクリックします。
  </Collapser>
  ...
  <Collapser
    ...
    title="手順3. 閾値の定義"
  >
    [閾値](/docs/alerts-applied-intelligence/new-relic-alerts/advanced-alerts/advanced-techniques/set-thresholds-alert-condition/)を定義すると、インシデントを開く内容を決定できます。定義したら、<DoNotTranslate>**Next**</DoNotTranslate>をクリックします。
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**New policy**</DoNotTranslate>を選択した場合：
    ...
    * <DoNotTranslate>**Save & set up notifications**</DoNotTranslate>をクリックします。

    <DoNotTranslate>**Existing policy**</DoNotTranslate>を選択した場合は、ポリシーを選択した後、<DoNotTranslate>**Save condition**</DoNotTranslate>をクリックします。
  </Collapser>
</CollapserGroup>
...
1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & Services**</DoNotTranslate>に移動してアプリを選択します。

2. 左側のペインで、<DoNotTranslate>**Metrics explorer**</DoNotTranslate>を選択し、設定を追加します。

3. チャートの右隅にある<Icon name="fe-more-horizontal"/>アイコンをクリックし、<DoNotTranslate>**Create alert condition**</DoNotTranslate>を選択します。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        UIでスライディングウィンドウを有効にすることができます。条件を作成または編集するときは、<DoNotTranslate>**Adjust to signal behavior > Data aggregation settings > Use sliding window aggregation**</DoNotTranslate>の順に移動します。
    ...
    <tr>
      ...
      <td>
        ...
        * インシデントと通知に含まれる
          ...
          ページの条件について、カスタムインシデントの説明を記入します。
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies)**</DoNotTranslate>に移動し、次に<DoNotTranslate>**+ New alert condition**</DoNotTranslate>に移動します。信号損失は、NRQL条件でのみ使用できます。
</figcaption>
...
* <DoNotTranslate>**None**</DoNotTranslate>
  ...
  ：（デフォルト）空の集計ウィンドウでアクションを実行したくない場合は、これを選択します。評価時に、空の集計ウィンドウは閾値期間タイマーをリセットします。たとえば、すべての集計ウィンドウに5分間の閾値を超えるデータポイントが必要であり、5つの集計ウィンドウの1つが空であるという条件が設定されている場合は、その条件はインシデントにはなりません。
...
* <DoNotTranslate>**Last known value**</DoNotTranslate>
  ...
  : このオプションは、評価が行われる前に最後に表示された値を挿入します。最後に表示された値の状態は最低2時間維持されます。設定された閾値の期間が2時間を超える場合、代わりにこの値がその期間に保持されます。
