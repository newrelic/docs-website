...
![NRQL条件および生成された結果の例のスクリーンショット。](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/nr1_nrql_alert_conditions.png "NRQL条件および生成された結果の例。")
...
[ネスト構造の集計クエリ](http://jajp-newrelic-docs.onelink-translations.com/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/nested-aggregation-make-ordered-computations-single-query)は、データに対してクエリを実行する強力な方法です。ただし、注意すべき重要な制限がいくつかあります。
...
<CollapserGroup>
  ...
  >
    信号喪失の詳細については、[NerdGraph API：信号の喪失とギャップの充填](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)を参照してください。
  </Collapser>
</CollapserGroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * 違反と通知に含まれる[条件について、カスタム違反の説明](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions)を記入します。
        * インシデント対応に関する自社の手順を含めるには、ランブックのURLを追加します。この情報を[カスタム違反の説明](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/alert-custom-violation-descriptions)に追加することもできます。
      </td>
    </tr>
...
![signal-loss-ui.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/signal-loss-ui_0.png "signal-loss-ui.png")
...
これらの設定は、GraphQL API（推奨）またはREST APIを使用して管理することもできます。特定の[Graph QL APIの例](http://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)については、こちらをご覧ください。
...
* **信号損失の有効期限**
  ...
  * GraphQL ノード：[expiration.expirationDuration](http://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)
...
* **信号喪失アクション**  
  ...
  * 現在、未解決の違反をすべて終了します。これにより、特定の信号に関連する未解決の違反がすべて終了します。必ずしも、条件のすべての違反が終了するとは限りません。一時的なサービスまたは散発的な信号で警告している場合は、違反が適切に終了するように、このアクションを選択することをお勧めします。この場合、GraphQLノード名は ["closeViolationsOnExpiration](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling) " です。
  * 未解決の新しい違反：これにより、信号が喪失した見なされると、新しい違反が発生します。これらの違反は、信号の損失が原因であることを示します。インシデントプリファレンスに基づいて、通知がトリガーされます。この場合、graphQLノード名は[、"openViolationOnExpiration](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alerts-nerdgraph/nerdgraph-api-loss-signal-gap-filling)" です。
...
![screenshot_advanced_signal_settings.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/screenshot_advanced_signal_settings2_0.png "screenshot_advanced_signal_settings.png")
...
[評価オフセット](/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts#h3-offset-evaluation)を調整して、データのレイテンシに合わせて[ストリーミングアルゴリズム](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts)を調整できます。データが到着するまでに時間がかかる場合は、評価オフセットを増やす必要があります。
