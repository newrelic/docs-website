...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        * データが集計される前に、ストリーミング時間枠に蓄積されるデータの長さを選択する[集計ウィンドウ](https://docs.newrelic.co.jp/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts)を設定できます。
          * デフォルトのストリーミング集計ウィンドウの期間は1分です。
          * 最小の集計ウィンドウは30秒です。
          * 最大の集計ウィンドウは15分です。

            <Callout variant="tip">
              ベースライン閾値は、設定可能な集計ウィンドウ期間をサポートしていません。デフォルトでは1分が使用されます。
            </Callout>
        * [評価オフセット](#time)を調整して、データのレイテンシに合わせて[ストリーミングアルゴリズム](https://docs.newrelic.co.jp/docs/alerts-applied-intelligence/new-relic-alerts/get-started/streaming-alerts-key-terms-concepts)を調整できます。データが到着するまでに時間がかかる場合は、評価オフセットを増やす必要があります。
        * データが散発的であるか、データのない集計ウィンドウが存在する場合は、それらのギャップをカスタム値または最後の既知の値で埋めることができます。デフォルト値は**なし**です。
        * <Callout variant="tip">
...
到着までに常に時間がかかるデータの場合、オフセット評価を使用して、NRQL条件の評価を一貫して遅らせることができます。長時間待機すると精度が上がりますが、レイテンシも長くなります。
...
<figcaption>
  集計ウィンドウとオフセット評価の切り替えによって、ストリーミングアラートデータ内のギャップの処理方法を選択できます。
</figcaption>

サポートされているレイテンシの合計は、集計ウィンドウ期間と評価オフセットの積になります。上のスクリーンショットの例では、サポートされているレイテンシは3分です（1分の集計ウィンドウが3つ）。

イベントタイプが[APMの言語エージェント](/docs/agents)から供給されており、多数のアプリケーションインスタンス（例：`Transactions`、`TransactionErrors`など）から集計されている場合は、1分間の集計ウィンドウで3分の評価オフセットを使用することをお勧めします。
...
ギャップの充填を使用すると、信号にデータがない場合に使用する値をカスタマイズできます。データストリームのギャップは、None（なし）、最後に受信した値、または静的な値で埋めることができます。

### データギャップ値を編集する方法：

* NRQL条件のUIで、**条件設定** > **信号の詳細設定**> **データギャップを埋める**を選択してから、**None（なし）**、**Last known value（最後の既知の値）**、または**Custom static value（カスタム静的値）**を選択します。
* Nerdgraph API（推奨）では、このノードは次の場所にあります。`actor : account : alerts : nrqlCondition : signal : fillOption | fillValue`
* REST APIエクスプローラーでは、[アラートNRQL条件API](https://rpm.newrelic.com/api/explore/alerts_nrql_conditions/list)の **"シグナル"** セクションにこれが表示されます。
