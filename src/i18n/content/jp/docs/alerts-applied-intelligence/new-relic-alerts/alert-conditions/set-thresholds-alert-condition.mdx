...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="任意の警告レベルを設定する"
  >
    閾値は、2つのレベルで設定できます：重要（必須）および警告（任意）。
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>
            ![icon-alert-critical.png](./imagess/icon-alert-critical.png "Alerts v3：重要アラートアイコン") 重要（赤）
          </td>
        ...
        <tr>
          <td>
            ![icon-alert-warning.png](./imagess/icon-alert-warning.png "Alerts v3：警告アラートアイコン") 警告（黄）
          </td>

          <td>
            オプション。通知を生成しません。システムの挙動に不安があるか注意を要するものの、通知を必要とするほどの重要性が無い場合に監視した場合は、警告による閾値を使用します。
          </td>
        </tr>
      </tbody>
    </table>
