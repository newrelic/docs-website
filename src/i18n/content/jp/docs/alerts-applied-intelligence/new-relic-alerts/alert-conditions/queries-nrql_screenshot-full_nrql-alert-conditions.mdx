...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) >（ポリシーを選択）> Add a condition**。**NRQL**、**Next, define thresholds**の順にクリックします。
</figcaption>
...
null値の処理方法を決定するには、[アラート条件UI](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#signal-loss)で信号損失とギャップ充填の設定を調整します。
...
* スライドごとの間隔を変更する（[スライディングウィンドウの集計](https://docs.newrelic.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#sliding-window-aggregation)条件のみ）
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions（ポリシー）> （ポリシーを選択）**、**Add a condition**の順に移動します。信号損失は、NRQL条件でのみ使用できます。
</figcaption>
...
[3つのストリーミング集計方法](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/streaming-alerts-key-terms-concepts/#aggregation-methods)から選択して、条件に最適な評価結果を得ることができます。
...
* **なし**:（デフォルト）空の集計ウィンドウでアクションを実行したくない場合は、これを選択します。評価時に、空の集計ウィンドウは閾値期間タイマーをリセットします。たとえば、すべての集計ウィンドウに5分間の閾値を超えるデータポイントが必要であり、5つの集計ウィンドウの1つが空であるという条件が設定されている場合は、その条件ではインシデントは開始されません。
