...
Alerts では、カスタマイズされたアラートソリューションを作成することで、ご利用システムのモニタリングが可能となります。
...
![New Relic Alertsのワークフロー図](src/images/new-relic-alert-creation-workflow-diagram.png 'new-relic-alert-creation-workflow-diagram.png')
...
2. そのポリシーに付属する[条件を作成します](/docs/alerts/new-relic-alerts/defining-conditions/define-alert-conditions)。条件を作成する際のステップには、以下が挙げられます。
   ...
   - 監視対象となるデータソースを選択します（たとえば、APM メトリックまたは NRQL クエリ）。
   - どの動きが違反を生成するのかを定義する[閾値を設定します](/docs/alerts/new-relic-alerts/defining-conditions/set-thresholds-alert-condition)。
   - オプション：[ランブック URL](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/provide-runbook-instructions-alerts)を含めます。これは、アラート通知の処理方法に関する標準的な手順の共有に使用します。

3. オプション：同じポリシーに条件をさらに追加します。

通知を受信する以外に、New Relic One でアラート[インシデント](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#alert-incident)または[イベント](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/view-events-their-products)詳細を確認できます。
