...
2. 条件が満たされたら、アラート違反を通知に変換する方法を制御するワークフローを設定します。[one.newrelic.com](https://one.newrelic.com)から、\[**検出と AI** ] をクリックし、\[**ワークフロー**] をクリックします。これを使用するためのヒントについては、 [ワークフローのドキュメント](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows)と、ワークフロー[のベスト プラクティスに関するこの投稿を](https://discuss.newrelic.com/t/relic-solution-patterns-for-implementing-alerts-workflows/188605)参照してください。
3. 1 つまたは複数の宛先を設定します (通知が送信される場所)。\[**検出と AI** ] から、\[**送信先**] をクリックします。ヒントについては、[宛先のドキュメント](/docs/alerts-applied-intelligence/notifications/destinations)を参照してください。
