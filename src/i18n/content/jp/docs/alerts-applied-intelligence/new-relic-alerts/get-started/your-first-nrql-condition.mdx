...
アプリケーションを New Relic に接続し、チャートやダッシュボードの探索を開始したら、次のステップとして、データ内の異常な動作について常にチームに最新情報を提供するためのアラートを作成することをお勧めします。 の<InlinePopover type="alerts"/>この機能により、New Relic のエクスペリエンスが、単なるデータの取り込みから、思慮深い効果的なアクションの実行まで向上します。
