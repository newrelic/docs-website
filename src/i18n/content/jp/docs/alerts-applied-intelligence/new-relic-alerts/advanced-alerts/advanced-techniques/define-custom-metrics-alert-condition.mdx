...
* インシデントを開始するクリティカルおよび警告のしきい値と期間。たとえば、 `5 units for at least 5 minutes` 。
* カスタム・メトリックの条件名（必須）。
...
1. **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts & AI > Alert conditions (Policies) > (ポリシーを選択)**に移動する か、 [新しいアラート ポリシーを追加します](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/name-or-rename-alert-policy)。
