...
![有用なカスタム情報を伝えるためのアラート条件説明欄の画面キャプチャー。](src/images/alerts_condition_description_field.png 'アラート状態の説明欄には、いくつかのコンテンツ例があります。')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com)** トップナビで、 **アラート&
  AI** 、 **アラートポリシー** 、 **（ポリシーを選択）** 、次に
  **（条件を選択）**: **+ Add custom violation description**
  をクリックしてフィールドを開く。
</figcaption>
...
![New Relicのアラート違反の説明 ](src/images/new-relic-alert-violation-description-screenshot.png 'New-relic-alert-violation-description-screenshot.png')
...
- [タグの属性です。タグの場合は、 `{{tags.TAG_NAME}}` という形式になります。タグには 2 種類あります。](/docs/alerts/new-relic-alerts/defining-conditions/violation-event-attributes)
...
## NRQL でカスタム違反の説明を作成 [#create-description-nrql]
...
- NRQL 条件と NerdGraph API の使用についての詳細は、 [NerdGraph API をご覧ください。NRQL 条件のアラート](/docs/alerts/alerts-nerdgraph/nerdgraph-examples/nerdgraph-api-alerts-nrql-conditions) 。
...
## Slack でユーザーに言及したり、チャンネルを通知する [#mention-in-slack]

[Slack](/docs/alerts-applied-intelligence/new-relic-alerts/alert-notifications/notification-channels-control-where-send-alerts#slack) に送信されることがわかっている説明文を作成するときに、 **@mention** ユーザーに言及したり、チャンネル全体の通知を生成したい場合があります（たとえば、 **@here** や **@channel** など）。これを実現するには、ユーザー ID またはチャンネル全体の通知を、 `<`と `>`の文字で囲まれた説明文に含めるだけです。
...
<Callout variant="important">
  なお、SlackではIncoming Webhooksでチャンネル全体に通知する際に、 `! @
  の代わりに` というプレフィックスを使用します。例えば、 `!here` または
  `!channel`.
</Callout>
