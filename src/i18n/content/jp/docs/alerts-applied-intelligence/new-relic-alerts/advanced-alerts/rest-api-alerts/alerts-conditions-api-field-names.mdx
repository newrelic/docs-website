...
<CollapserGroup>
  ...
  >
    `true`の場合、 `expiration_duration`時間内に信号が聞こえない場合、これにより現在開いているすべての違反が閉じられます。
  ...
  >
    trueの場合、 `expiration_duration`時間内にシグナルがない場合、シグナル損失違反が発生します。
  ...
  >
    ...
    データポイントは、タイムスタンプに基づいてまとめて収集され、バッチとして報告されます。カスタマイズ可能な集計ウィンドウにより、不規則なデータポイントや頻度の低いデータポイントでアラートを送信する際の柔軟性が向上し、誤った違反が少なくなります。
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                APMアプリケーションメトリックは、アラートをトリガーします（ベースラインしきい値を使用）。
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                ブラウザメトリックはアラートをトリガーします（ベースラインしきい値を使用）。
              </td>
            </tr>
  ...
  >
    指定した秒数が経過すると、インスタンスベースの違反を自動的に閉じるために使用します。
  ...
  >
    指定された時間数の後に、JVMヘルスメトリック違反を含むインスタンスベースの違反を自動的に閉じるために使用します。次のいずれかの値である必要があります。
