...
import alertsNewRelicAlertsBaselineThresholds from 'images/alerts_screenshot-full_new-relic-alerts-baseline-thresholds.png'

import alertsIconAlertCritical from 'images/alerts_icon_icon-alert-critical-.png'

import alertsAlertWarning from 'images/alerts_icon_alert-warning.png'

import alertsFacetedBaselineConditionQuery from 'images/alerts_screenshot-full_faceted-baseline-condition-query-.png'

import alertsBaselineOneFacet from 'images/alerts_screenshot-full_baseline-one-facet.png'

import alertsBaselinealertsdirection from 'images/alerts_screenshot-full_baselinealertsdirection.png'
...
* 違反を開く動作を設定します（「5分間を超えて逸脱」など）。
* 条件が上限違反、下限違反、または両方をチェックするかどうかを設定します。

データが予測された「通常の」動作から逸脱し、選択した基準を満たすと、インシデントが開かれ、問題の作成設定に応じて通知を受信する場合があります。
...
<img
  ...
  src={alertsNewRelicAlertsBaselineThresholds}
/>

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Policies >（create or select policy）> Create alert condition：**異常アラート条件により、異常動作が検出されたときにのみ違反を生成するインテリジェントな自己調整閾値を設定できます。
</figcaption>

異常条件を作成するには、**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Policies >（create or select policy）> Create alert condition**の順に移動します。[条件](/docs/alerts/new-relic-alerts/defining-conditions/define-alert-conditions#create-condition)の作成を開始するときに、以下の[データソース](#availability)のいずれかを選択します。
...
* [異常の方向](#upper-lower)を設定する際に、異常の上または下で発生する予期しない動作を監視するようにします。
...
* スライダーバーを使用して、
  ...
  <img title="Alerts v3: Critical alert icon" alt="icon-alert-critical.png" src={alertsIconAlertCritical}>プレビューチャートに異常の周囲のライトグレーの領域として表される</img>

  **クリティカル**閾値の感度を調整します。異常の周囲の帯域が狭いほど、感度が高くなり、より多くの違反が生成されます。

* オプション：

  <img title="Alerts v3: Warning alert icon" alt="icon-alert-warning.png" src={alertsAlertWarning}/>
...
<img
  ...
  src={alertsFacetedBaselineConditionQuery}
/>
  ...
  >
    ...
    * スループットなどのデータソースの場合は、突然の上向きの変動はよく発生することで、突然の下向きの変動は問題の兆候を示している可能性があるため、**Lower**設定を使用します。
      ...
      <img
        ...
        src={alertsBaselinealertsdirection}
      />
  </Collapser>
