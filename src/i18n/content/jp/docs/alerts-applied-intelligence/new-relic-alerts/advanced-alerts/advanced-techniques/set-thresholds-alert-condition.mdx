...
必須の重要閾値以外にも、深刻度の低い[警告レベル](#threshold-levels)に対して閾値を設定できます。
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            必須。インシデントが発生すると、ポリシーの[問題作成設定](/docs/alerts/new-relic-alerts/configuring-alert-policies/specify-when-new-relic-creates-incidents)や設定した[ワークフロー](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/)に応じて、通知が送信される場合があります。
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            オプション。警告レベルのインシデントが開き、未解決の問題にロールアップして発行される場合があります。システムの挙動に不安があるか注意を要するものの、クリティカルレベルの閾値を必要とするほどの重要性がない場合に監視する場合は、警告による閾値を使用します。
          </td>
        </tr>
      </tbody>
    </table>
