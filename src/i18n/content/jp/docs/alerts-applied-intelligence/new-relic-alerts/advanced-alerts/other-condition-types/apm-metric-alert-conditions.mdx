...
1. ツールバーの<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM**</DoNotTranslate>に移動します。

2. アラートを作成するサービスを探します。

3. サービスをクリックします。

4. 左側のナビゲーション ペインで、 <DoNotTranslate>**External services**</DoNotTranslate>をクリックします。

5. <DoNotTranslate>**Top 20 external services**</DoNotTranslate>の下で、必要なサービスを見つけてクリックします。
...
6. ページの右側に、 <DoNotTranslate>**Response time**</DoNotTranslate>と<DoNotTranslate>**External calls per minute**</DoNotTranslate> (平均値) のグラフが表示されているので、これら 2 つのオプションのどちらを集計するかを決定します。

7. 選択したグラフの右上隅にある省略記号アイコン<DoNotTranslate>**...**</DoNotTranslate>をクリックし、 <DoNotTranslate>**Create alert condition**</DoNotTranslate>を選択します。

   <Callout variant="tip">
     省略記号アイコン<DoNotTranslate>**...**</DoNotTranslate>が表示されない場合は、ページ上部の<DoNotTranslate>**Show new view**</DoNotTranslate>オプションが有効になっていることを確認してください。
   </Callout>
...
2. <DoNotTranslate>**Define your signal**</DoNotTranslate>では、NRQL クエリ構文が有効になるように、 `LIMIT` 、 `SINCE...AGO` 、および`TIMESERIES`句が削除されていることを確認します。
