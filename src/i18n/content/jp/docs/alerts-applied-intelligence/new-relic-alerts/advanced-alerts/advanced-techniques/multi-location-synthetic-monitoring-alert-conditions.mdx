...
1. **one.newrelic.com**から、 [**Alerts & AI**](https://one.newrelic.com/alerts-ai) \[アラートと AI]、 [**Alert policies**](https://one.newrelic.com/alerts-ai/condition-builder/policy-list) \[アラート ポリシー] の順に選択します。
2. クラシック アラート条件を作成するポリシーをクリックするか、新しいポリシーを作成します。
3. **New alert condition** \[新しいアラート条件]を選択します。
4. **Build a classic alert** \[クラシック アラートを作成する] を選択します。
5. **Synthetics** \[外形監視] と**Multiple location failures** \[複数の場所の障害] を選択します。
6. **Next** \[次へ] をクリックして、ターゲットとするモニターを選択します。
7. **Next** \[次へ] をクリックします。 **Set thresholds** \[閾値を設定する] ステップのフィールドに入力します。
8. **Save condition** \[条件を保存] をクリックします。
