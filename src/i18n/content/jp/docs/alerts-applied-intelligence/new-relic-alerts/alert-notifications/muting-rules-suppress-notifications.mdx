...
不要な通知に共通する要素を見つけたら、それらの要素を特にターゲットとするミュートルールを定義し、他の通知は通過させることができます。通知がミュートされていても、Alerts はそれらの違反に関するデータを収集します。ミューティングルールは、アラートのプロセスを妨げることなく、通知が送信される直前の時点で適用されます。
...
![ミューティングルールの管理](src/images/manage_muting_rules.png 'ミューティングルールの管理')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > アラート& AI>
  ミューティングルール**:
  複雑なミューティングルールを作成して、不要な通知の少なからぬセットを対象にすることができます。
</figcaption>
...
これを行うには、開始時刻と終了時刻を選択します。オプションで、ミューティングルールを 1 日中継続するように設定することもできます。

また、ミューティングルールのスケジュールにタイムゾーンを選択することもできます。デフォルトでは、ユーザー設定で選択されたタイムゾーンが使用されます。

![muting_rules_recurring.png](src/images/muting_rules_recurring_0.png 'muting_rules_recurring.png')
...
<Callout variant="important">
  **Repeat** の曜日チェックボックスは、 **Starts** と **Ends**
  の日付フィールドを上書きします。開始日を設定し、さらに曜日を選択した場合、ミューティングルールは開始日以降の最初の日に適用されます。
</Callout>

また、特定の日付または特定の回数を選択することで、再帰の終了時期を指定することもできます。

## NerdGraph でミューティング・ルールを管理 [#manage-with-nerdgraph]
...
- `alertsMutingRuleUpdate:` id とアカウント id によるミューティングルールの更新。
- `alertsMutingRuleDelete：` id とアカウント id でミューティングルールを削除します。
...
<CollapserGroup>
  ...
  >
    ![アラートインシデントのライフサイクルルール違反のミューティング](src/images/muting-rules-incident-violations.png "アラートインシデントのライフサイクルルール違反のミューティング")
  ...
  >
    ...
    ![アラートインシデントのライフサイクルミューティングルールのインシデント](src/images/muting-rules-open-incidents.png "アラートインシデントのライフサイクルミューティングルールのインシデント")
  ...
  >
    ![アラートインシデントのライフサイクルミューティングルールのクリティカルバイオレーション](src/images/muting-rules-violations_0.png "アラートインシデントのライフサイクルミューティングルールのクリティカルバイオレーション")
  ...
  >
    ...
    ![アラートインシデントのライフサイクルミューティングルールイベント](src/images/muting-rules-events.png "アラートインシデントのライフサイクルミューティングルールイベント")
...
- `IN`: 違反属性の値が、供給された値のリスト（最大 500 個）の中に存在する場合。
- `NOT_IN`: 与えられた値のリスト（最大 500 個）の中に違反属性の値が存在しない場合。
...
NerdGraph へのリクエストの詳細については、 [NerdGraph documentation](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph) 、 [GraphQL tutorials](/docs/apis/graphql-api/tutorials) を含む。
