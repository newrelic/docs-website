---
title: ポリシーの管理
tags:
  - Alerts
  - Alert policies
metaDescription: 'Read about how to create or rename a policy, and how to find existing policies.'
freshnessValidatedDate: never
translationType: human
---

import accountsGreenQuestionMark from 'images/accounts_icon_green-question-mark.webp'

[ポリシー](/docs/alerts-applied-intelligence/overview/#concepts-terms)とは、1 つ以上の条件のグループを指します。本ドキュメントでは、ポリシーの作成方法、また既存ポリシーの確認方法を解説します。[NerdGraph API](/docs/apis/nerdgraph/examples/nerdgraph-api-alerts-policies/)（GraphQL）または[REST API](/docs/alerts/rest-api-alerts/new-relic-alerts-rest-api/rest-api-calls-new-relic-alerts)も使用できます。

ポリシーが[アラート条件](/docs/alerts-applied-intelligence/new-relic-alerts/alert-conditions/create-alert-conditions/)、ワークフロー、その他のコンポーネントとどのように関わり合うかについての解説は、[コンセプト](/docs/alerts-applied-intelligence/overview/#concepts-terms)と[ワークフロー](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows/)を参照してください。

## ポリシーの命名と編成に関するベストプラクティス [#best-practices-policies]

New Relic <InlinePopover type="alerts"/>ポリシーは、アラート条件を一貫性のある管理可能な単位に整理するのに役立ちます。 単なる分類を超えて、インシデントの相関関係や潜在的な問題に関する通知の配布をある程度制御できます。

アラートポリシーと条件を構築するには、以下の3つの一般的な組織パターンをお勧めします。

<table>
  <thead>
    <tr>
      <th width={200}>
        <DoNotTranslate>
          **Policies organized by...**
        </DoNotTranslate>
      </th>

      <th>
        <DoNotTranslate>
          **Comments**
        </DoNotTranslate>
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        アーキテクチャー
      </td>

      <td>
        組織内のターゲットやリソースは、その状況が個別で管理されているかもしれません。共通部分には<DoNotTranslate>**environment**</DoNotTranslate>環境タイプ（本番、ステージング、インテグレーション、開発）または<DoNotTranslate>**target or resource**</DoNotTranslate>ターゲットまたはリソースタイプ（アプリケーションサーバーのコード、クライアントサイドのコード、データベース、ホストなど）が含まれる場合があります。

        これらのタイプのポリシーを識別しやすくするには、アーキテクチャーに基づいた名前を付けることができます。例：

        * Product A website （production）
        * Product B website （staging）
        * Mobile product app （iOS）
        * Hosts （production）
        * Databases （production）
      </td>
    </tr>

    <tr>
      <td>
        チーム
      </td>

      <td>
        特定のチームが、アーキテクチャーの一部を担当する場合があります。たとえば、`Team: Site engineering`、`Team: Website development`などのポリシーに名前を付けることができます。
      </td>
    </tr>

    <tr>
      <td>
        個人
      </td>

      <td>
        特定のリソースやメトリックを個人的にチェックしたい場合など、特定の個人用にポリシーを作成すると便利です。こうすることで、いつでも必要に応じて、条件の追加や削除のほか、閾値の調整を自由に行うことができ、その変更を他の人に伝える必要もなくなります。

        これらのポリシーを特定しやすくするには、`User: John Doe's metrics`、`User: Jane Doe (production)`のように、個人の名前を含めるとよいでしょう。
      </td>
    </tr>
  </tbody>
</table>

## アラートポリシーの作成 [#alert-policy-name]

アラートポリシーは1つのアカウントにのみ適用されます。New Relic組織で複数のアカウントをお持ちの場合、各アカウントに対してアラートポリシーを作成する必要があります。

1. [one.newrelic.com > Alerts > Alert Policies](https://one.newrelic.com/alerts-ai/condition-builder/policy-list)の順に移動します。

2. ポリシーリストページで、

   <Icon name="fe-plus-circle"/>

   <DoNotTranslate>**New alert policy**</DoNotTranslate>

   をクリックします。

3. ポリシーに[意味のある名前](#best-practices-policies)（最大64文字）を入力します。

4. [issue preference](/docs/alerts/new-relic-alerts/configuring-alert-policies/specify-when-new-relic-creates-incidents)を選択します。

5. <DoNotTranslate>**Create & close**</DoNotTranslate>

   を選択してポリシーを作成するか、

   <DoNotTranslate>**Set up notifications**</DoNotTranslate>

   を選択してポリシーを[ワークフロー](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows)に接続します。新しい問題が発生した場合や問題が更新された場合は、_PagerDuty_や_Slack_などのサービスで通知が届きます。

ここから引き続き、[条件の定義](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/define-alert-conditions)と[ワークフローを使用して通知の設定](/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/incident-workflows)に進むことができます。後ほど継続する場合は、<DoNotTranslate>**Alert policies**</DoNotTranslate>のインデックスからポリシーの名前を選択します。

## ポリシー名の変更 [#rename-policy]

利便性を考慮して、異なるポリシー、条件、通知チャネルに同じ名前を使用できます。固有の名前を使用する場合は、名前を追加、変更する前にインデックスを確認するか、検索してください。

既存のポリシーの名前を変更するには：

1. [one.newrelic.com > Alerts > Alert Policies](https://one.newrelic.com/alerts-ai/condition-builder/policy-list)の順に移動し、リスト内のポリシーをクリックします。

2. ポリシー

   <DoNotTranslate>**settings**</DoNotTranslate>

   タブをクリックして編集し、ポリシーに[意味のある名前](#best-practices-policies)（最大64文字）を入力して、

   <DoNotTranslate>**Save**</DoNotTranslate>

   をクリックします。

アラートではポリシーの条件を自動的に更新します。

## 既存のポリシーの表示 [#alert-policy-view]

ポリシーのインデックスには、ポリシーがアルファベット順にリストされます。既存のポリシーを表示または検索するには：

1. [one.newrelic.com > Alerts > Alert Policies](https://one.newrelic.com/alerts-ai/condition-builder/policy-list)の順に移動します。
2. 検索ボックスを使用する、列を並べ替える、またはリストをスクロールします。
3. 詳細情報を表示するには、ポリシー名をクリックします。

<Callout variant="tip">
  [特定のエンティティのポリシーと条件の情報](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/view-policy-conditions-new-relic-products)を表示するには、エンティティの製品のUIから<DoNotTranslate>**Settings > Alert conditions**</DoNotTranslate>ページを選択します。
</Callout>

## ポリシーのメンテナンスのクイックリファレンス [#quick-reference-policies]

ポリシーを作成したら、次の操作を実行できます。

* ポリシーに条件をさらに追加します。
* 条件の名前、ターゲット、または閾値を編集します。
* 条件を無効にします。
* ワークフローを使用して通知に接続します。
* ポリシーの名前を変更するか、削除します。

これは、ポリシーのメンテナンスに関する詳細な情報と手順へのリンクも含むクイックリファレンスです。

<CollapserGroup>
  <Collapser
    id="rename-policy"
    title="ポリシー名の変更"
  >
    ポリシーの[名前を変更する](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/name-or-rename-alert-policy#rename-policy)には、次の手順に従います。

    1. <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com) > Alerts > Alert Policies**</DoNotTranslate>

       に移動してから、ポリシーを選択します。

    2. ポリシーの

       <DoNotTranslate>**settings**</DoNotTranslate>

       タブをクリックして編集し、ポリシーに[意味のある名前](#best-practices-policies)（最大64文字）を入力して、

       <DoNotTranslate>**Save**</DoNotTranslate>

       をクリックします。
  </Collapser>

  <Collapser
    id="new-policy"
    title="新しいポリシーを作成する"
  >
    [新しいアラートポリシー](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/name-or-rename-alert-policy)を作成するには、次の手順に従います。

    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Alert Policies**</DoNotTranslate>

       に移動します。

    2. <DoNotTranslate>**New alert policy**</DoNotTranslate>

       をクリックします。

    3. 新しく作成されたポリシーをクリックし、

       <DoNotTranslate>**New Alert Condition**</DoNotTranslate>

       をクリックしてアラートを設定します。
  </Collapser>

  <Collapser
    id="disable-policy"
    title="ポリシーを無効化する"
  >
    ポリシーを直接無効にすることはできません。ただし、[ポリシーのすべての条件を無効にする](#)ことはできます。

    すべての条件を無効にすると、ポリシーは<DoNotTranslate>**Alert policies**</DoNotTranslate>インデックスのままになります。ポリシー自体は無効になっておらず、いつでもその条件の一部またはすべてを再度有効にできます。
  </Collapser>

  <Collapser
    id="delete-policy"
    title="ポリシーを削除する"
  >
    ポリシーを削除すると、製品エンティティ（ターゲット）に割り当てられた他のポリシーも引き続き適用されます。エンティティの<DoNotTranslate>**all**</DoNotTranslate>すべてのポリシーを削除すると、[色分けされた稼働ステータス](/docs/alerts/new-relic-alerts/configuring-alert-policies/identify-entities-without-alert-policies)インジケータが、アイコンとともに薄い緑色で表示されます。

    アカウントのポリシーを削除すると、復元できなくなります。代わりに、必要に応じて再作成する必要があります。

    ポリシーを完全に削除するには、次の手順に従います。

    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts > Alert Policies**</DoNotTranslate>に移動します。

    2. ポリシー行で、省略記号（...）ドロップダウンメニューをクリックし、<DoNotTranslate>**Delete**</DoNotTranslate>を選択します。

       削除されたポリシーでは、[問題](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/explore-incident-history)と[インシデント](/docs/alerts/new-relic-alerts-beta/reviewing-events/review-events-across-products)は読み取り専用リソースとして提供されます。
  </Collapser>

  <Collapser
    id="targets-policy"
    title="ポリシーに割り当てられたエンティティ（ターゲット）を確認する"
  >
    [エンティティ（ターゲット）にポリシーが割り当てられている](/docs/alerts/new-relic-alerts-beta/configuring-alert-policies/identify-entities-without-alert-policies)かどうかを確認するには、次の手順に従います。

    1. 製品（APM、ブラウザなど）に移動します。

    2. 選択した製品のインデックスから、エンティティの色分けされた稼働ステータスがアイコン付きの薄い緑色であるかどうかを確認します。<img title="Alerts v3: alert icons" alt="icon-alerts-q-green.png" src={accountsGreenQuestionMark}/>

       <Callout variant="tip">
         <DoNotTranslate>**Exception:**</DoNotTranslate> インフラストラクチャではこの機能を使用しません。
       </Callout>
  </Collapser>

  <Collapser
    id="incidents"
    title="インシデントが問題にロールアップされる方法を変更する"
  >
    [インシデント](/docs/alerts/new-relic-alerts-beta/reviewing-alert-incidents/specify-when-new-relic-creates-incidents)を問題にグループ化する方法（ポリシー別、条件別、またはターゲットと条件別）を選択するには、次の手順に従います。

    1. <DoNotTranslate>**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts > Alert Policies**</DoNotTranslate>

       に移動してから、ポリシーを選択します。

    2. <DoNotTranslate>**Settings**</DoNotTranslate>

       タブをクリックします。

    3. <DoNotTranslate>**Issue Creation Preference**</DoNotTranslate>

       を変更し、

       <DoNotTranslate>**Save**</DoNotTranslate>

       をクリックします。
  </Collapser>

  <Collapser
    id="link-to-ai"
    title="アラートインシデントのコンテキストを取得する"
  >
    New Relicアラートを使用すると、条件を[インシデントインテリジェンス](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence/#configure-source-nr-alerts)に接続できます。

    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Alerts**</DoNotTranslate>の順に選択します。

    2. 左側で<DoNotTranslate>**Correlate > Sources**</DoNotTranslate>の順に選択し、<DoNotTranslate>**Alerts**</DoNotTranslate>をクリックします。

    3. アラートに接続するポリシーを選択し、<DoNotTranslate>**Connect**</DoNotTranslate>をクリックします。

       <DoNotTranslate>**Sources > Alerts**</DoNotTranslate>で、アラートポリシーを追加、またはすでに接続しているポリシーを削除できます。
  </Collapser>
</CollapserGroup>