...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Destinations**に移動します。宛先を使用して、通知の送信先を選択します。
</figcaption>

1. **[one.newrelic.com](https://one.newrelic.com) > Alerts & AI > Enrich and Notify > Destinations**に移動します。宛先テーブルには、既存の宛先に関する情報が表示され、ユーザーは有効化、無効化、および変更できます。
