---
title: 目的地
tags:
  - Alerts and Applied Intelligence
  - Applied intelligence
  - Incident Intelligence
  - Destinations
metaDescription: Read about how to configure and set up Incident Intelligence destinations.
translationType: machine
---

<Callout title="アーリーアクセス">
  ここで説明されている機能は早期アクセスです。アーリーアクセスプログラムに参加されていない方は、これらの機能を使用することはできません。

関連する機能の詳細については、 [Alerts notification channels](/docs/alerts-applied-intelligence/new-relic-alerts/alert-notifications/notification-channels-control-where-send-alerts/) 、 [Incident Intelligence destinations](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/incident-intelligence-destination-examples/) 、 [Proactive Detection notifications](/docs/alerts-applied-intelligence/applied-intelligence/proactive-detection/proactive-detection-applied-intelligence/#set-up) のドキュメントを参照してください。

</Callout>

デスティネーションは、お客様の New Relic One データに関する通知を送信する場所です。デスティネーションとは、お客様が使用しているサードパーティシステムの固有の識別子です。

デスティネーション設定には、サードパーティのシステムと統合するための接続詳細が含まれており、New Relic One の様々なツールで使用することができます。

対応するデスティネーション・プラットフォームは以下の通りです。

- アトラシアン Jira
- ServiceNow
- スラック
- Webhook
- メール

これらの宛先やその他の宛先については、 [通知の統合](/docs/alerts-applied-intelligence/notifications/notification-integrations) を参照してください。

<Callout variant="tip">
  また、 `aiNotifications` [NerdGraph
  API](https://developer.newrelic.com/collect-data/get-started-nerdgraph-api-explorer/)
  を使って配信先を設定することも可能です。
</Callout>

## 必要な能力 [#required-roles]

デスティネーション設定には、特定の [機能が必要です](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#capabilities) 。

- 設定にアクセスするには、 `「View」` の機能が必要です。 `「Applied Intelligence:Destines」` または `「Alerts」` の機能が必要です。
- 設定を変更または削除するには： `Modify` の機能が必要です。 `Applied Intelligence:Destination` または `Alerts`.

## 目的地の管理 [#manage-destinations]

1. [one.newrelic.com](https://docs.newrelic.com/one.newrelic.com) 、 **Alerts& AI** 、左のナビで **Enrich and Respond** 、 **Destinations** をクリックしてください。目的地のテーブルには、既存の目的地に関する情報が表示され、ユーザーは有効化、無効化、および変更を行うことができます。
2. デスティネーションを追加するには、適切なプラットフォームタイルをクリックします。目的地の設定を変更するには、目的地テーブルで目的地の行をクリックします。

![目的地の概要ページです。](src/images/destinations-overview.png '目的地の概要ページ')

<figcaption>**one.newrelic.com> アラート& AI> デスティネーション**.</figcaption>

### デスティネーションの状態 [#manage-destinations]

デスティネーションには、イベントの処理や送信中に問題が発生したかどうかを示す「ステータス」値があります（上の画像のデスティネーションテーブルを参照）。認証や認可の問題のような一部のエラーでは、宛先の接続詳細の更新が必要になります。更新後、送信先のステータス値は"Default" に変更されます。

## 通知ログ [#notifications-log]

過去の通知イベントの詳細を見るには、 **Destination** メニューで、 **Notifications log** タブをクリックします。

通知ログでは、過去のすべての通知の履歴とステータスを確認することができます。ここでは、通知のステータスに加え、関連するエラーの詳細や目的地のチケット番号を確認することができます。

![探している情報を見つけやすくするフィルターツールの画面です。](src/images/filter-destinations.png '探している情報を簡単に見つけることができるフィルターツールです。')

<figcaption>
  配信先のログを、配信先の種類や送信者、ステータスなどでフィルタリングすることができます。
</figcaption>
