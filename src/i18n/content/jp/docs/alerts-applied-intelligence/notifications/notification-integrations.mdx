...
<CollapserGroup>
  ...
  >
    ...
    Slack ワークスペースには、 [New Relic アプリケーション](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)がインストールされている必要があります。アプリケーションを個別にインストールする前に、ワークスペース管理者がアプリケーションを承認する必要があります。アプリのサポートについては、 [support@newrelic.com](mailto:support@newrelic.com)にお問い合わせください。また、参加するプライベート チャンネルごとに招待状が必要です。
    ...
    **Allow** をクリックすると、目的のページに戻ります。
    ...
    メッセージを送信する宛先 (ワークスペース) と Slack チャネルを選択します。必要なワークスペースに事前定義された宛先がない場合は、新しい宛先を作成できます。プライバシー上の理由から、ユーザーはプライベート チャネルを選択するために一度認証を受ける必要があります。
