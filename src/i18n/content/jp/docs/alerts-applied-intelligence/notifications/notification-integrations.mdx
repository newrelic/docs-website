...
<CollapserGroup>
  ...
  >
    ...
    * 参加するプライベート チャンネルごとに招待状が必要です。
      ...
      **Allow** をクリックすると、目的のページに戻ります。
      ...
      メッセージの送信先 (ワークスペース) と Slack チャネルを選択します。必要なワークスペースに事前定義された宛先がない場合は、新しい宛先を作成できます。プライバシー上の理由から、プライベート チャネルを選択するには、ユーザーを一度認証する必要があることに注意してください。デフォルトの通知を使用するか、問題のペイロードからのフリー テキストまたは動的変数を使用して詳細を充実させることができます。[変数メニュー](/docs/alerts-applied-intelligence/notifications/message-templates/#variables-menu)から変数を選択し、[ハンドルバーの構文](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax)を適用して、ペイロードを充実させます。
  ...
  >
    ...
    * PagerDuty のインシデントが承認されると、New Relic のインシデントの承認がトリガーされます。
    ...
    6. リストで New Relic 統合を見つけてマークし、 **Add **をクリックします。<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}/>

    7. 右側をクリックして、 **Integration Key **を表示してコピーします。<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}/>
