...
<CollapserGroup>
  ...
  >
    ...
    * **APIキー**: このインテグレーションでは、REST APIキーの提供を求められます。
    ...
    デフォルトでは、New Relicによって作成されたPagerDutyインシデントへの状態の変更は、NewRelicに同期されます。<Callout variant="important">双方向同期は、PagerDutyインテリジェントアラートグループ化と完全には互換性がありません。関連するPagerDutyサービスのインシデントグループ化をオフにするか、NewRelicで双方向同期を無効にすることをお勧めします。</Callout>
