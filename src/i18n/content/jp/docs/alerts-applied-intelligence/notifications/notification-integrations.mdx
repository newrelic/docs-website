...
<CollapserGroup>
  ...
  >
    ...
    * **API トークン：** アトラシアンアカウントから生成されたものです。
      ...
      * JIRA 課題のステータスが進行中に変更されると、New Relic 課題の確認が行われます。
      ...
      1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローまたは **Add a new Workflow** ボタンをクリックします。

      2. デスティネーションを選択します。この段階で新しい目的地を作ることができます。
  ...
  >
    <Callout variant="important">
      近日提供予定: New Relic Workflows との新しい統合が ServiceNow アプリ ストアで利用可能になります。詳細については [、ここをクリック](https://forum.newrelic.com/s/hubtopic/aAX8W00000006D7/coming-soon-a-new-integration-between-new-relic-workflows-and-servicenow) してください。
    </Callout>
    ...
    * New Relic Issueの確認は、Servicnowのインシデントの状態がオープンから変更されたときに発生します。
      ...
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローをクリックするか、 **Add a new Workflow** ボタンをクリックして、ServiceNow の送信先を選択します。
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローまたは **Add a new Workflow** ボタンをクリックします。メッセージを送信する宛先 (ワークスペース) と Slack チャネルを選択します。必要なワークスペースに事前定義された宛先がない場合は、新しい宛先を作成できます。プライバシー上の理由から、ユーザーはプライベート チャネルを選択するために一度認証を受ける必要があります。プライベート チャネルを選択すると、ボットが自動的にチャネルに追加されます。
  ...
  >
    ...
    * **承認メカニズム（_オプション_）:。** `basic authentication`または`bearer token`にすることができます。
    ...
    4. リクエストのペイロードを設定します。
       ...
       **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローをクリックするか、 **Add a new Workflow** ボタンをクリックして Webhook の宛先を選択します。
  ...
  >
    ...
    * **AWSアカウントID** ：AWSアカウントID。これは12桁の数字です。
    ...
    2. イベント バスのルールを作成します。
       ...
       ワークフローに移動し、既存のワークフローまたは \[新しいワークフローを追加] ボタンをクリックして、イベントブリッジの宛先を選択します。New Relic から EventBridge に通知を送信する場合、メッセージ テンプレートを使用してその通知をカスタマイズできます。次の手順に従って、EventBridge 宛先のメッセージ テンプレートを構成します。
  ...
  >
    ...
    * PagerDuty のインシデントが承認されると、New Relic のインシデントの承認がトリガーされます。
    ...
    1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローをクリックするか、 **Add a new Workflow** ボタンをクリックして Pagerduy 通知機能を選択します。
    2. デスティネーションを選択します。この段階で新しい目的地を作ることができます。
    ...
    1. **Services**\[サービス] > **Service Director**\[サービス ディレクトリ] に移動し、統合を追加するサービスを選択します。

    2. **Integrations** タブを選択 **Add another integration**.

    3. リスト内で New Relic 統合を見つけてマークし、 **Add**\[追加]をクリックします。<img title="PagerDuty New Relic integration." alt="New Relic integration." src={accountsPagerDutyAddNewRelicIntegration}/>

    4. 右側をクリックして、 **Integration Key**\[統合キーを]表示し、コピーします。<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}/>
  ...
  >
    ...
    * **ユーザー ID:**現在ログインしているユーザーに基づいて自動的に入力されます。
      ...
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI > Workflows** に移動し、既存のワークフローをクリックするか、 **Add a new Workflow** ボタンをクリックしてモバイル通知機能を選択します。モバイル プッシュを設定するには、モバイル ノーティファイアをクリックし、目的の宛先を選択します。
  </Collapser>
