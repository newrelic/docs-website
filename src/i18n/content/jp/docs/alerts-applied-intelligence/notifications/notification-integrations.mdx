...
<CollapserGroup>
  ...
  >
    ...
    Slack ワークスペースには、 [New Relic アプリケーション](https://newrelic.slack.com/apps/AP92KQJS3-new-relic?tab=more_info)がインストールされている必要があります。アプリケーションは、ユーザーが個別にインストールする前に、ワークスペース管理者によって承認される必要があります。アプリのサポートについては、 [support@newrelic.com](mailto:support@newrelic.com)にお問い合わせください。また、参加したいプライベート チャンネルごとに招待状が必要になります。
    ...
    **Allow** をクリックすると、目的のページに戻ります。
    ...
    メッセージの送信先 (ワークスペース) と Slack チャネルを選択します。必要なワークスペースに事前定義された宛先がない場合は、新しい宛先を作成できます。プライバシー上の理由から、プライベート チャネルを選択するには、ユーザーを一度認証する必要があることに注意してください。

    デフォルトの通知を使用するか、課題ペイロードからのフリー テキストまたは動的変数を使用して詳細を充実させることができます。[変数メニュー](/docs/alerts-applied-intelligence/notifications/message-templates/#variables-menu)から変数を選択し、[ハンドルバー構文](/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax)を適用してペイロードを充実させます。
  ...
  >
    ...
    * **承認メカニズム（_オプション_）:。** `basic authentication` 、 `OAuth 2.0 `、または`bearer token`にすることができます。
      ...
      \###webhook イベント テンプレートを構成する \[#configure-webhook-event-template]
  ...
  >
    ...
    * **AWSアカウントID** ：AWSアカウントID。これは12桁の数字です。
    ...
    3. イベント ソースをイベント バスに関連付けます。AWS EventBridge コンソールで、ナビゲーション ペインの \[パートナー イベント ソース] を選択します。
    ...
    4. イベント バスのルールを作成します。
       ...
       New Relic から送信された通知に対応するには、New-Relic イベントをフィルター処理するイベント パターンを使用してルールを作成する必要があります。<img title="Event pattern with the newrelic partner prefix." alt="A screenshot of rule Event pattern." src={accountsBridgeEventPattern}/>
  ...
  >
    ...
    * PagerDuty のインシデントが承認されると、New Relic のインシデントの承認がトリガーされます。
    ...
    4. 右側をクリックして、 **Integration Key **を表示してコピーします。<img title="PagerDuty New Relic integration key." alt="New Relic integration key." src={accountsPagerDutyIntegrationOpened}/>
  ...
  >
    ...
    Webhook テンプレートを使用して、ワークフローから Splunk On-call に通知を送信する[ワークフロー用の Splunk On-Call Webhook テンプレート](https://help.victorops.com/knowledge-base/new-relic-integration-guide/#about-splunk-on-call-and-new-relic)
  </Collapser>
</CollapserGroup>
