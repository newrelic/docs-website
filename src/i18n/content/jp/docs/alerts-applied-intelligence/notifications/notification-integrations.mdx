...
<CollapserGroup>
  ...
  >
    ...
    4. \[通知] で**\[Webhook]**を選択し、次の情報を使用して通知メッセージをカスタマイズします。
       ...
       * ペイロードをカスタマイズして、 [ワークフロー変数を](https://docs.newrelic.com/docs/alerts-applied-intelligence/applied-intelligence/incident-workflows/custom-variables-incident-workflows/)関連する[FreshService パラメーター](https://api.freshservice.com/#create_ticket)に送信します。(ペイロードのサンプルを参照)

         * ワークフロー変数のヘルパー関数の詳細については、 [「ハンドルバーの構文」](https://docs.newrelic.com/docs/alerts-applied-intelligence/notifications/message-templates/#handlebars-syntax)を参照してください。
