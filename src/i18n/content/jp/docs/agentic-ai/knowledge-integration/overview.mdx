...
* **インシデントの迅速な解決:** New Relic AI 内で直接、社内のランブックやドキュメントから関連するコンテキストにすぐにアクセスできるようになり、平均解決時間 (MTTR) が短縮されます。

* **実用的でカスタマイズされた推奨事項を取得します。**提供される回答は、お客様の環境に固有であり、お客様自身のベストプラクティスと履歴データに基づいています。

* **重要なコンテキスト固有の質問に答えます。**以前に解決された問題に対する解決策を簡単に見つけることができます。次のような質問をします。
