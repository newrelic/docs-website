...
<CollapserGroup>
  ...
  <Collapser id="k8s-helm-failure" title="Helmリリースの失敗">
    ...
    Agent ControlではFleet Controlに安全に接続するために有効な認証資格情報が必要です。 最初に、この資格情報はAgent Control Instration UIによって自動的に生成され、値ファイル内の`systemIdentity.parentIdentity.clientId`フィールドと`systemIdentity.parentIdentity.clientSecret`フィールドによって表されます。 セキュリティ上の理由から、 Agent Controlのインストールに必要な資格情報は 12 時間後に期限切れになります。
...
<CollapserGroup>
  ...
  <Collapser id="debug" title="エージェントのログ記録に関する問題を診断する">
    ...
    * `error`
    ...
    3. `file`ログが有効になっている場合は、`path`設定のディレクトリにある最新のログファイルを確認してください（ファイルには日付が付いています。例：`2026-09-18.newrelic-agent-control.log`）。または、`journalctl -u newrelic-agent-control`などのお好みのログトラブルシューティングツールを使用してください。
  </Collapser>
...
最新のログファイルを表示します（ログファイルには`2026-02-09.newrelic-agent-control.log`のような日付プレフィックスが付けられています）：
...
サービス固有の問題を回避するには、コマンドラインからAgent Controlを実行します。デフォルトのローカル設定パス（`local-data\agent-control\local_config.yaml`）から設定を自動的に読み取るため、フラグは必要ありません：
...
## NRDiagを使用してホスト上のAgent Controlをトラブルシューティングします

New Relic診断ツール`NRDiag`は、デバッグのためにホスト上のAgent Controlに関連するリソースとログを収集するユーティリティです。これはLinuxとWindowsの両方のホストに適用されます；対応するスイートがOSを自動的に検出し、関連するチェックを実行します。

すべてのデータを収集するには、次の手順に従ってください：

1. ホストで、 [スタートガイド](/docs/new-relic-solutions/solve-common-issues/diagnostics-cli-nrdiag/diagnostics-cli-nrdiag/#get-started)を使用して`NRDiag`ツールをインストールします。

2. ホスト上のAgent Controlスイートを実行します：
...
3. 予想される出力は次のレポートのようになります。
   ...
   <Callout variant="tip" title="ヒント">
     Windowsでは、`AgentControl/Agent/Logs`タスクは`journalctl`ではなく`C:\ProgramData\New Relic\newrelic-agent-control\log\agent-control\`から直接ログファイルを収集します。
   </Callout>
