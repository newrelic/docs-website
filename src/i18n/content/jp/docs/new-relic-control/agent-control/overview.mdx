...
* **認証情報：**Agent Controlは、Vaultなどの外部値プロバイダーから機密の認証情報を取得し、実行時にエージェントの設定に注入できます。これにより、設定自体に認証情報を含めないことで、セキュリティが向上します。
...
* **セキュリティポスチャ：**取り扱いを一元化し、ローテーションを自動化し、露出を減らすために、バリュープロバイダーから資格情報を取得する方法を標準化します。
...
Agent Controlによって管理されるすべてのエージェントは、Agent Controlがそのエージェントをダウンロード、設定、実行する方法を記述したバージョン管理された仕様である**エージェントタイプ**によってサポートされています。新しいエージェントをサポートするには、常にエージェントタイプの定義が必要です。定義がないと、Agent Controlはその管理方法を認識できません。

エージェントのタイプは、修飾名とバージョンによって識別されます（例：`newrelic/com.newrelic.infrastructure:0.1.0`）。設定にサブエージェントを追加する際、エージェントのタイプを参照します；Agent Controlはその定義を使用して正しいアーティファクトを取得し、設定のデフォルトを適用して、エージェントのライフサイクルを管理します。

利用可能なエージェントのタイプの完全なリストについては、[エージェントのタイプ](/docs/new-relic-control/agent-control/agent-types#agent-types)を参照してください。
...
各エージェントに必要な特定の権限の詳細については、「[エージェントタイプごとの必須権限](/docs/new-relic-control/agent-control/agent-types#required-permissions-per-agent-type)」を参照してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Agent Controlは、メーカーの製品寿命までこれらのOSをサポートします：
