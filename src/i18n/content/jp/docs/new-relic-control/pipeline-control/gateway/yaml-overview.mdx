...
<Callout variant="tip">
  **インストレーションモードの注意:** `version`および`autoscaling`フィールドの動作は、フリートのインストレーションモードによって異なります：

  * **Fluxの場合:** これらのフィールドはUIを通じてフルマネージドされます。変更はフリート内のすべてのクラスタに自動的にデプロイされます。
  * **Fluxを使用しない場合:** これらのフィールドは、Horizontal Pod Autoscaler（HPA）を構成するために、初期インストレーション時に設定できます。ただし、インストレーション後のUI経由での変更は無視されます。`configuration.simplified/v2`セクション（パイプラインルール）のみが、引き続き`ConfigMap`経由で自動的にデプロイされます。HPAによるスケーリングとHelmアップグレードによるバージョンは、手動で更新する必要があります。詳細は[Fluxなしでのゲートウェイの管理](/docs/new-relic-control/pipeline-control/gateway/install-gateway-without-flux#manage-gateway-after-installation)を参照してください。
</Callout>
...
* `nrExporterEndpoints`：パイプラインごとにデフォルトのNew Relicインジェストエンドポイントをオーバーライドします
...
ゲートウェイの設定は、各ステップがその動作を定義し、outputフィールドを使用してパイプライン内の次のステップを指し示す、有向非巡回グラフ（DAG）構造に従っています。これにより、明示的なデータフローが作成されます：データはレシーバーを通じて入力され、プロセッサー（変換、フィルター、サンプリング）を経由して流れ、エクスポーターを通じて出力されます。
...
| nrExporterEndpoints.nrHost                | ストリング | いいえ | -     |
| nrExporterEndpoints.metricsEndpoint       | ストリング | いいえ | -     |
| nrExporterEndpoints.tracesEndpoint        | ストリング | いいえ | -     |
| nrExporterEndpoints.logApiEndpoint        | ストリング | いいえ | -     |
| nrExporterEndpoints.eventApiEndpoint      | ストリング | いいえ | -     |
| nrExporterEndpoints.infraEventApiEndpoint | ストリング | いいえ | -     |
| nrExporterEndpoints.otlpEndpoint          | ストリング | いいえ | -     |

各`nrExporterEndpoints`フィールドは独立しています：オーバーライドする必要があるもの（たとえば、`nrHost`のみ）だけを設定してください。省略したフィールドは、アカウントのデフォルトのNew Relic取り込みエンドポイントにフォールバックします。

`nrExporterEndpoints` フィールドは2つの独立したオーバーライドグループに分かれています：

* `nrHost`、`metricsEndpoint`、`tracesEndpoint`、`logApiEndpoint`、`eventApiEndpoint`、および`infraEventApiEndpoint`は、New Relic独自の（レガシーエージェント）テレメトリーの宛先をオーバーライドします。
* `otlpEndpoint` OTLPネイティブテレメトリーの宛先を個別にオーバーライドします。一方のグループのフィールドを設定しても、もう一方のグループには影響しません。

`otlpEndpoint` ポートを含まない、ベアホストとスキーム（たとえば、`https://otlp.example.com`）である必要があります。ポートを追加すると、設定の検証に失敗し、保存されません。
