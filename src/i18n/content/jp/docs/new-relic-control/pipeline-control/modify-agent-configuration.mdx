...
ゲートウェイがエージェントのテレメトリーデータを処理するには、エージェントのデータを New Relic に直接ではなくゲートウェイNew Relicにルーティングするようにエージェントを再設定する必要があります。 これを行うには、サンプルの New Relic NodeJS エージェント構成に示すように、 `NEW_RELIC_HOST`設定を使用してゲートウェイを指すようにエンドポイントの詳細を変更します。
