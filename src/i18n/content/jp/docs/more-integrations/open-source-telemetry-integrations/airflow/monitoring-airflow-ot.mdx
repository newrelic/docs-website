...
Airflow メトリクスをNew Relicに送信するには、 OpenTelemetryメトリクス[OpenTelemetry設定します](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/collector/opentelemetry-collector-intro/)。これにより、<InlinePopover type="licenseKey"/> を使用してデータがNew Relic [OTLP エンド ポイント](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-setup/#note-endpoints)に転送されます。
...
3. 認証のために、
   ...
   を環境変数`NEW_RELIC_LICENSE_KEY`に追加して、 `api-key`ヘッダーに値が設定されるようにします。
...
4. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > Services - OpenTelemetry > Airflow**</DoNotTranslate>

   に移動します。

5. <DoNotTranslate>**Metrics Explorer**</DoNotTranslate>
