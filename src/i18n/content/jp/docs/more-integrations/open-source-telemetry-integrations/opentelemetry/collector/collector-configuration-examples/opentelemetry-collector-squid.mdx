...
OpenTelemetry コレクターを使用して、Squid Cache Manager に関するメトリックを収集できます。コレクターは OpenTelemetry のコンポーネントであり、テレメトリ データを収集、処理し、New Relic (または任意のオブザーバビリティ バックエンド) にエクスポートします。
