...
テレメトリ データがプラットフォームに正常に配信されたことを検証するために使用できるツールは多数あります。最初の適切なステップは、[データ管理ハブ](/docs/data-apis/manage-data/manage-your-data/)をチェックしてデータの取り込みを[ファセット化](/docs/data-apis/manage-data/manage-data-coming-new-relic/#facet-data-ingest)し、さまざまなソースから到着するデータの量を判断することです。[データ エクスプローラー](/docs/query-your-data/explore-query-data/browse-data/introduction-data-explorer/#use-case-troubleshooting)または [クエリ ビルダー](/docs/query-your-data/explore-query-data/query-builder/use-advanced-nrql-mode-query-data)を使用して、 `instrumentation.provider`または`newrelic.source`属性によってファセットされたデータを探すこともできます。
