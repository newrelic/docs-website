...
OpenTelemetryで計測されたサービスのJVMsページでは、どのサービス・インスタンスに異常または不健全なパフォーマンス・パターンがあるかを特定することができます。応答時間、スループット、エラー・レート、ガベージ・コレクション時間、メモリ使用量などの主要メトリクスのサマリーに基づいて、比較するサービス・インスタンスをいくつか選択できます。次に、OpenTelemetry計測器によって収集されたそれらのインスタンスのJVMメトリクスを時系列チャートを使って比較し、問題を発見することができます。
...
<CollapserGroup>
  ...
  >
    ...
    * 非同期アップダウンカウンタは、OTLP非単調和としてエクスポートされます。

    デルタアグリゲーションの時間性を使用してメトリクスをエクスポートするように SDK を設定した場合 (New Relic でカウンターやヒストグラムの計測器が機能するために必要です)、非同期のアップダウンカウンターが非単調なデルタサムとしてエクスポートされます。New Relic では、非単調なデルタサムのデータに対して有用な分析を行うことができません。
    ...
    View API の設定方法は、OpenTelemetry Java エージェントを使用しているかどうかによって異なります。
