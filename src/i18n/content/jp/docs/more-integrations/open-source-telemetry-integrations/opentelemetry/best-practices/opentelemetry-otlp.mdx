...
* OTLP バージョンを確認してください: New Relic は[OTLP リリース v0.18.0](https://github.com/open-telemetry/opentelemetry-proto/releases/tag/v0.18.0)を使用します。 それ以降のバージョンはサポートされていますが、新しい機能はまだ実装されていません。 0.18.0 で利用できなくなった実験的な機能はサポートされていません。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        無限のトレース<br/>(エンドポイントの詳細については[ベストプラクティス](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-traces#infinite-tracing)を参照してください)
      </td>
...
## 設定: ペイロードのバッチ処理、圧縮、レート制限 [#payload-limits]
