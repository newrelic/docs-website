...
<CollapserGroup>
  ...
  >
    ...
    * JVM のリストをレンダリングするためのユニークな `service.instance.id` 属性です。
  ...
  >
    ...
    * OpenTelemetry Java エージェントを使用していない場合は、この簡単な [例を確認してください](https://github.com/newrelic/newrelic-opentelemetry-examples/pull/89/files#diff-da355ef6d1092534a55829e95160ab8468884bdd521f9018feeaaa66aea6ac5bR82-R86) `SdkMeterProvider` を構成する際にビューを登録する方法を示しています。

    * OpenTelemetry の Java エージェントを使用している場合は、 [のエクステンションで View API を設定する必要があります](https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/main/docs/agent-config.md#extensions) 。エクステンションを使用すると、SDK の設定（特に）にフックして、環境変数やシステムのプロパティでは利用できないプログラムによる設定を適用することができます。 この [の例では、エクステンションを使って ](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/java/agent-nr-config)[](https://github.com/newrelic/newrelic-opentelemetry-examples/blob/main/java/agent-nr-config/config-extension/src/main/java/com/newrelic/otel/extension/Customizer.java#L28-L37)`SdkMeterProvider`'s views' をカスタマイズする方法を示しています。
  </Collapser>
</CollapserGroup>
