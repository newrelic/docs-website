...
New Relic にデータを送信し、UI で表示するのがどのようなものかをすばやく確認できるように、インストルメンテーションを当社に任せたい場合は、これが最適なオプションです。
...
2. 次の 2 つの環境変数を設定して、New Relic アカウントにデータを送信します。
   ...
   * [インジェスト ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)を必ず使用してください。
...
1. 左ペインの \[ **Monitor **] セクションで、\[ **分散トレーシング** ] をクリックし、\[ **Fibonacci **trace group] をクリックします。
...
3. トレースを開いた後、 `Show in-process spans`をクリックし、結果のスパンをクリックすると、右側に詳細パネルが開きます。ユーザー入力が無効な場合に記録した例外を表示するには、 `View span events`をクリックします:
      ...
      <tbody>
        <tr>
          <td>
            ...
            * ヘッダー: これにより、データが送信される New Relic アカウントが決まります。`YOUR_LICENSE_KEY`を[取り込みライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)に置き換えます。
          </td>
        </tr>
