...
高レベルでは、アラートは 1 つ以上のアラート条件を含むアラート ポリシーを介して構成されます。アラート条件のインシデントは問題としてグループ化されます。最後に、問題は、問題を通知に変換するロジックを含むワークフローをトリガーできます。これらの概念を理解するのに役立つ図については、 [「アラートの概念」](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/basic-alerting-concepts)を参照してください。
