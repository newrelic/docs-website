...
* <DoNotTranslate>**Use the right endpoint**</DoNotTranslate>
  ...
  OTLP データをエンドポイントにエクスポートするように構成する必要があります。 これについて[は、 OpenTelemetryコレクターからデータを送信する方法に関するドキュメント](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/get-started/opentelemetry-set-up-your-app/#review-settings)で説明されています。

* <DoNotTranslate>**Avoided dropped data**</DoNotTranslate>

  データが欠落するのを避けるため、長さが 4095 文字を超える属性は[切り捨てること](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-otlp/#attribute-limits)をお勧めします。 これらのプラクティスを適用する例については、 [このコレクター構成を](https://github.com/newrelic/newrelic-opentelemetry-examples/tree/main/other-examples/collector/nr-config)参照してください。

* <DoNotTranslate>**Infrastructure**</DoNotTranslate>

  。インフラストラクチャ メトリックを収集するには、コレクター構成に[ホスト メトリック レシーバー](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/collector/opentelemetry-collector-infra-hosts)を含めます。 ホスト レシーバーを コレクター 設定 の一部として使用する場合、ホスト メトリックをホスト エンティティの一部として自動的に検出し、そのゴールデンメトリックを合成します (つまり、インフラストラクチャ エージェントを使用する場合と同じエクスペリエンスが得られます)。

* <DoNotTranslate>**Scaling**</DoNotTranslate>

  。大規模なOpenTelemetryチームでは、パフォーマンスと回復力のメリットの両方を考慮して、コレクターを拡張するための戦略を選択する必要があります。 コレクターのスケーリングの詳細については、 [OpenTelemetry のドキュメント](https://opentelemetry.io/docs/collector/scaling)を参照してください。

* <DoNotTranslate>**Kubernetes**</DoNotTranslate>

  OpenTelemetryインストゥルメントされたサービスがKubernetes環境で実行されている場合は、 [「 OpenTelemetryインストゥルメントされたアプリケーションをKubernetesにリンクする」](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/link-otel-applications-kubernetes)の手順に従ってください。 これにより、Kubernetes データが OpenTelemetry データと相関関係にあることが保証されます。

多くのお客様は、エージェントとゲートウェイの両方の展開方法を使用してコレクターを実行することを選択しています。

* <DoNotTranslate>**Agent**</DoNotTranslate>

  : この方法では、コレクターはアプリケーションと同じホスト (バイナリ、サイドカー、デーモンセットなど) 上で実行されます。

* <DoNotTranslate>**Gateway**</DoNotTranslate>

  : この方法では、クラスター、データセンター、またはリージョンごとに 1 つ以上のコレクター インスタンスがスタンドアロン サービスとして実行されます。
...
大まかに言うと、 <InlinePopover type="alerts"/>アラートポリシーを介して構成され、1 つ以上のアラート条件が含まれます。 アラート条件のインシデントは、問題としてグループ化されます。 最後に、問題は、問題を通知に変換するロジックを含むワークフローをトリガーできます。 これらの概念を理解するのに役立つ図については、 [「アラートの概念」](/docs/alerts-applied-intelligence/overview/#concepts-terms)を参照してください。
...
OpenTelemetry サービスを関連エンティティとともにグループ化するワークロードを作成することをお勧めします。 これには、インフラストラクチャ モニタリングによる テンプレート モニター、<InlinePopover type="apm"/>、 Kubernetes監視、<InlinePopover type="browser"/>、その他の テンプレート が含まれる可能性があります。 ワークロードは、通常、それらのエンティティを担当するチームに合わせて、エンティティをより管理しやすいチャンクにグループ化するのに役立ちます。 これは、数千のモニター インスタンスが存在する可能性のある大規模な環境では特に役立ちます。
