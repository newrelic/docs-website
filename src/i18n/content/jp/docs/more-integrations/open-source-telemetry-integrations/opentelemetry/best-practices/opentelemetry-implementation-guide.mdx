...
* これにより、OpenTelemetry コレクターの負荷テストを行い、本番環境のような負荷を十分に処理できることを確認する機会も提供されます。Collector のスケーリングに関するヒントについては、 [OpenTelemetry のドキュメント](https://opentelemetry.io/docs/collector/scaling)を参照してください。
