...
マップまたはエンティティの関係を表示するその他の機能には、**Related entities** \[関連エンティティ]、**Trace map** \[トレース マップなど]があります。

## 依存関係 [#dependencies]

New Relic エージェント: ✅ New Relic の OpenTelemetry: ❌

OpenTelemetry、**Service map** \[サービス マップ]、または**Related entities** \[関連エンティティの]ビューでは、特定のサービスのすべての依存関係にアクセスできます。
...
<CollapserGroup>
  ...
  >
    ...
    OpenTelemetry には New Relic に相当するものはありません<InlinePopover type="browser"/> New Relic APM エージェントで使用できるエージェントまたはブラウザ インストルメンテーション。OpenTelemetry-JavaScript SDK を使用している場合、エンド ユーザーからブラウザ データを生成および収集するために現在利用できる自動インスツルメンテーション ライブラリがいくつかあります。iOS および Android アプリケーション (Java および Swift) およびブラウザ アプリケーション (JavaScript) の作成に使用できる言語については、クライアント側のテレメトリを計測するための仕様がありますが、まだ利用できません。
...
<CollapserGroup>
  ...
  >
    ...
    オンホスト[統合](/docs/infrastructure/infrastructure-integrations/get-started/introduction-infrastructure-integrations#on-host)(NGINX、MySQL、Redis、Apache、RabbitMQ など) と[Kubernetes 統合](/docs/kubernetes-pixie/kubernetes-integration/get-started/introduction-kubernetes-integration/)は、New Relic の精選されたダッシュボードとエンティティのソースとして必要です。
  </Collapser>
...
設定できます<InlinePopover type="alerts"/> NRQL で記述された条件を使用した OpenTelemetry データ。 開始するには、チャートの「 **...** 」メニューをクリックしてください。
