...
<CollapserGroup>
  ...
  >
    ...
    このサポートは、一部のサービスが OpenTelemetry で計測され、他のサービスが APM エージェントで計測される分散システムで、トレース コンテキストをあるサービスから次のサービスに正常に伝達できることを意味します。これにより、複数の計測技術を使用している場合でも、システム全体のアクティビティを可視化できます。
  </Collapser>
  ...
  >
    ...
    APM エージェントは、標準トレースまたは無限トレースをそれぞれ有効にすることで実装できる、ヘッド ベースとテール ベースの両方のサンプリングをサポートします。
...
New Relic APM エージェントは[タイムスライス メトリック](/docs/data-apis/understand-data/metric-data/query-apm-metric-timeslice-data-nrql)を使用し、OpenTelemetry は[ディメンション メトリック](/docs/data-apis/understand-data/new-relic-data-types/#dimensional-metrics)を使用します。
...
このビューには、APM および/または OpenTelemetry サービスまたはホストが表示されます。
...
[デプロイを記録して](/docs/change-tracking/change-tracking-introduction/#start-tracking) 、OpenTelemetry サービスのアクティビティ フィードで確認できるようになりました。 また、 **Summary** ビューの 3 つのチャートに変更マーカーを表示することもできますが、New Relic APM では、トランザクションとデータベースのチャートにも表示されます。
...
<CollapserGroup>
  ...
  >
    ...
    APM サービスの New Relic UI を介して、サーバー側の構成を含むいくつかの構成オプションを利用できます。
