...
UI で単一のサービスを表示するときにアクセスできるサービス マップには、そのサービスに接続されたエンティティのセット全体が表示され、現在 New Relic エージェントによってのみサポートされています。相互接続されたサービスは、このマップに表示されるためにアクティブに報告されている必要があります。

OpenTelemetry データは、**関連エンティティ**、**自動マップ**、および**トレース マップ**機能を強化します。

## 依存関係 [#dependencies]

New Relic エージェント: ✅ New Relic の OpenTelemetry: ❌

OpenTelemetry の場合、**関連エンティティ**または**Automap**ビューにより、特定のサービスのすべての依存関係に移動できます。
...
OpenTelemetry には、デプロイメントの記録に関する仕様はありません。潜在的な回避策は、スパンに`service.version`属性を追加することです。これを使用して、環境内のさまざまなデプロイメント バージョンをフィルタリングできます。この回避策では、求めているのと同じ利点が得られない場合があります。これは、デプロイ マーカーの価値が、異常と一緒にアクティビティ ストリームに表示され、リアルタイムで表示できることにあるためです。ただし、フィルタリングを許可することで価値を提供できます。
