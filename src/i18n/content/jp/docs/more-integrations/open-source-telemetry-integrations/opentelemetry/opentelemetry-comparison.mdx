...
<CollapserGroup>
  ...
  >
    ...
    ゴールデン メトリクスを持つホスト エンティティは、データ ソースから独立して利用できます: New Relic インフラストラクチャ エージェントと OpenTelemetry コレクター ホスト レシーバー。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    New Relic エージェントの高セキュリティ モード機能によって適用されるデータの制限は、OpenTelemetry コレクターを構成または変更することで適用できますが、OTLP はそのような制御を指定していないため、New Relic はそれらを適用できませんでした。
  </Collapser>
