...
OpenTelemetry ログは New Relic のログと互換性があります。OpenTelemetry ログには、オプションで、クエリでログ データをファセットまたはフィルタリングするために使用できるディメンションに直接マップされる属性 (名前と値のペア) とリソース属性が含まれます。OpenTelemetry ログのメタデータ (例: `name` 、 `severity_text` 、および`trace_id` ) も、New Relic のディメンションに直接マッピングされます。 <InlinePopover type="logs"/>能力。現在、すべての OpenTelemetry ログ メッセージ タイプがサポートされています。
