...
OpenTelemetry は、サービスおよびホスト インストルメンテーションに安全でベンダーに依存しない [仕様を](https://github.com/open-telemetry/opentelemetry-specification) 提供するため、New Relic など、選択した個別のバックエンドにデータをエクスポートできます。OpenTelemetry は、サービスとホストのテレメトリ データを収集して転送する方法を標準化する API とライブラリの単一セットを提供します。
...
OpenTelemetryを検討する際に、テレメトリーデータをキャプチャするNew Relic <InlinePopover type="apm"/>エージェントも検討している可能性があります。
...
[一般的に、 New Relicの <InlinePopover type="apm"/>とインフラストラクチャエージェントは、サービスとホストのテレメトリデータをさらに収集します。 さらに、幅広い設定オプションと広範な自動インストゥルメント機能も提供します。](https://github.com/open-telemetry/opentelemetry-specification/blob/master/spec-compliance-matrix.md)
...
<Callout variant="important">
  New Relic の OpenTelemetry エクスポーターは廃止され、OTLP を使用して New Relic にデータをエクスポートするようになりました。
</Callout>
...
New Relic は、メトリクス信号の OTLP 取り込みのサポートを提供します。

ここでは、サポートされている OpenTelemetry データ型とそれに関連するマッピングを示します。詳細については、 [「OpenTelemetry メトリクス: ベスト プラクティス」](/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/best-practices/opentelemetry-best-practices-metrics)を参照してください。
...
New Relic は、ログ信号の OTLP 取り込みのサポートを提供します。上流仕様の成熟度は[安定してい](https://github.com/open-telemetry/opentelemetry-specification/blob/87a5ed7f0d4c403e2b336f275ce3e7fd66a8041b/specification/versioning-and-stability.md#stable)ます。

OpenTelemetry ログは New Relic のログと互換性があります。 OpenTelemetryログには、オプションで、クエリを使用してログ データをファセットまたはフィルター処理するために使用できるディメンションに直接マップされる 属性 (名前と値のペア) とリソース 属性が含まれます。 OpenTelemetry ログ メタデータ ( `name` 、 `severity_text` 、 `trace_id`など) も、New Relic の<InlinePopover type="logs"/>機能のディメンションに直接マップされます。 現在、すべてのOpenTelemetryログメッセージ タイプをサポートしています。
