...
[一般に、New RelicのAPMおよびインフラストラクチャエージェントは、サービスとホストのテレメトリデータをより多く収集します。さらに、幅広い構成オプションと広範な自動計装機能のセットを提供します。](https://github.com/open-telemetry/opentelemetry-specification/blob/master/spec-compliance-matrix.md)
