...
エラー受信ボックスには、特定のエンティティを介して[スコープレベル](/docs/errors-inbox/errors-inbox/#scoped)でアクセスするか、「エラー受信ボックス」というラベルの付いた上部のナビゲーションリンクを介してグローバルレベルでアクセスできます。グローバルエクスペリエンスでは、[ワークロード](/docs/new-relic-solutions/new-relic-one/workloads/use-workloads/)を設定する必要があります。
