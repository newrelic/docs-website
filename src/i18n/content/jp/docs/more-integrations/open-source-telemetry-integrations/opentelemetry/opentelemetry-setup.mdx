...
* アプリから直接：
  ...
  ![NewRelicへのアプリへの直接エクスポートを示す図](./images/native-otlp-no-collector.png "NewRelicへのアプリへの直接エクスポートを示す図")<figcaption>サービス内のOTLPエクスポーターは、NewRelicOTLPレシーバーに直接エクスポートできます。</figcaption>

* OpenTelemetryコレクターからのエクスポート：

  ![コレクターからNewRelicへのエクスポートを示す図](./images/native_otlp_with_collector.png "コレクターからNewRelicへのエクスポートを示す図")<figcaption>コレクターを、New Relic OTLPレシーバーを受信、処理、およびエクスポートするゲートウェイとして設定できます。または、アプリケーションで実行されるエージェントまたは同じホストで実行されるエージェントとしてコレクターを設定することもできます。</figcaption>
...
<CollapserGroup>
  ...
  >
    Per the [OpenTelemetry spec](https://github.com/open-telemetry/opentelemetry-specification/blob/b7473b5de0f55f921f896948442ebb274f58b584/specification/protocol/exporter.md#endpoint-urls-for-otlphttp) on endpoint URLs for OTLP/HTTP&#x3A; HTTPトラフィックを送信していて、非パーシグナルの環境変数(`OTEL_EXPORTER_OTLP_ENDPOINT`)を使用している場合は、単純に `OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.nr-data.net:{port}を設定することができます。` と設定すれば、エクスポーターは信号の種類に応じて適切なパスを追加するはずです（例： `v1/traces` または `v1/metrics` ）。

    シグナルごとの環境変数（例えば、 `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` および/または `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` ）を使用している場合は、適切なパスを設定することが必要です。例えば、 `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT=https://otlp.nr-data.net:4318/v1/traces` 。そうしないと404になってしまいます。なお、この環境変数は、信号ごとではない環境変数よりも優先されます。
  </Collapser>
</CollapserGroup>
...
* [追加のコレクター設定例](https://github.com/newrelic/newrelic-opentelemetry-examples)
