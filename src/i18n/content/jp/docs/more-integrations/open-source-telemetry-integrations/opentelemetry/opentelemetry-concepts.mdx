...
<CollapserGroup>
  ...
  >
    Infinite Tracing は New Relic のテールベースのサンプリングオプションです。OpenTelemetry で計測したサービスと組み合わせて使用することができます。Infinite Tracing の設定では、デフォルトの span エンドポイントをオーバーライドし、New Relic のトレースオブザーバーにテレメトリデータを送信する必要があります。

    <Callout variant="important">
      現在、Infinite Tracing は OTLP インジェストをサポートしていません。OpenTelemetry Collector の独自のインスタンスを実行し、 [New Relic exporter](/docs/integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-legacy-new-relic-exporters) を使用するように設定する必要があります。
    </Callout>
