...
<Steps>
  ...
  <Step>
    ...
    エクスポートは、OpenTelemetry プロジェクト用に設計された汎用テレメトリ データ配信プロトコルである[OpenTelemetry](https://github.com/open-telemetry/opentelemetry-proto/blob/main/docs/specification.md) Protocol (略して OTLP) に依存します。このプロトコルは、テレメトリ データをエンコードして送信する方法を記述しており、データ転送に自然な選択肢となります。各言語 SDK は、OTLP 経由でデータをエクスポートするように構成できる OTLP エクスポーターを提供します。
