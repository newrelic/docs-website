...
* gaDate から eolDate:
  ...
  * 廃止された`gauge`タイプのインジェストを使用しているアカウントは、 `cumulativeCount`タイプのインジェストにオプトインできます。
...
gaDate より前に累積単調合計を取り込んでいた場合、この 180 日間の期間により、影響を受けるメトリックを使用するクエリと<InlinePopover type="alerts"/>を更新する時間が与えられます。
...
コードベース内で OTLP インストゥルメンテーション ライブラリを使用している場合は、アプリケーションに[適切な言語 SDK](https://opentelemetry.io/docs/languages/o)を参照する必要があります。 個々の Metricus データ ポイントまたはリソース 属性レベルに属性`newrelic_metric_type=cumulativeCount`を追加する方法はどれでも十分です。
