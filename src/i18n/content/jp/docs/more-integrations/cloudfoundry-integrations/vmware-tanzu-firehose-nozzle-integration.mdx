...
ノズルは、Ops Manager でタイルとしてインストールすることも、CLI コマンド cf Push を使用して通常のアプリケーションとして展開することもできます。 その後、VMware Tanzu 導入の健全性とパフォーマンスを監視し、 <InlinePopover type="alerts"/>VMware Tanzu Firehose から収集されたメトリックに基づきます。
