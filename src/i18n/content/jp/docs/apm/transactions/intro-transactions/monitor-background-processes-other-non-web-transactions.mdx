...
New Relic APMの[トランザクション](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm)は、主に**ウェブ**と**非ウェブ**の2つのカテゴリーに分類されます。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Node.js"
  >
    [`startBackgroundTransaction()`](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/nodejs-agent/supported-features/nodejs-custom-instrumentation#background-txn)を使用します。
  </Collapser>
  ...
  <Collapser
    ...
    title="PHP"
  >
    トランザクションを非ウェブに指定するには：[`newrelic_background_job`](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/php-agent/php-agent-api/newrelic_background_job)を使用して、フラグを`true`に設定します。
...
* エンティティエクスプローラー：**[one.newrelic.com](https://one.newrelic.com) > Entity explorer > （エンティティを選択）> Summary**の順に移動します。
* APM：**[one.newrelic.com](https://one.newrelic.com) > APM > （アプリを選択） > Summary**の順に移動します。
* トランザクション：**[one.newrelic.com](https://one.newrelic.com) > APM >（アプリケーションを選択）> Monitor > Transactions**の順に移動します。
