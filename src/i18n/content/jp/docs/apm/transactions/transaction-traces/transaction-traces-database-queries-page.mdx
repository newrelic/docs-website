...
![APM - トランザクショントレース - データベースクエリタブ](src/images/APM-transaction-trace-database-queries.png 'APM-transaction-trace-database-queries.png')

<figcaption>
  Go to **[one.newrelic.com](https://one.newrelic.com) > APM> （アプリを選択）>
  Monitor> Transactions> （トランザクショントレースを選択）> Database queries**.
</figcaption>
