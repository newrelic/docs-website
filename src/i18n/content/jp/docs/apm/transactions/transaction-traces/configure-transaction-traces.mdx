...
ご利用のNew Relicエージェントでサーバーサイドコンフィグレーション設定が利用可能な場合、<DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Settings > Application > Server-side agent configuration**</DoNotTranslate>の順に選択すると確認できます。
...
<Callout variant="important">
  [トランザクショントレース閾値](#fields)を、デフォルトのApdex設定ではなく秒数で設定した場合、この秒数の閾値が、カスタムApdex設定をもつキートランザクションを含む<DoNotTranslate>**all**</DoNotTranslate>すべてのトランザクションに、閾値として適用されます。
</Callout>
...
[データセキュリティ上の理由](/docs/apm/transactions/transaction-traces/security-options-transaction-traces)で、トランザクショントレースには潜在的に機密性が高いHTTPリクエストの属性（<DoNotTranslate>**parameters**</DoNotTranslate>パラメーターとも呼ばれます）は含まれません。ただし、トレースは基本的なHTTPリクエスト属性をいくつか収集します。New Relicではこれを<DoNotTranslate>**agent attributes**</DoNotTranslate>と呼びます。属性収集の設定を編集するには、特定のNew Relicエージェントをご覧ください。
...
<CollapserGroup>
  ...
  >
    選択したアプリの<DoNotTranslate>**all**</DoNotTranslate>すべてのトレースを永久に削除するには：

    1. <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitor > Transactions**</DoNotTranslate>に移動します。

    2. <DoNotTranslate>**Delete all traces**</DoNotTranslate>をクリックします。
