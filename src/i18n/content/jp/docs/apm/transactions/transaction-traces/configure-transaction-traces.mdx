...
tags:
  ...
  - Transactions
  - Transaction traces
metaDescription: How to configure transaction trace settings in New Relic APM.
---
...
* [Java](https://jajp-newrelic-docs.onelink-translations.com/docs/agents/java-agent/configuration/java-agent-configuration-config-file#h2-transaction-tracer)
...
サーバーサイドコンフィグレーションをNew Relicエージェントで使用できる場合、次の場所で表示できます：**[one.newrelic.com](https://one.newrelic.com) > APM > （アプリケーションの選択） > Settings > Application > Server-side agent configuration**。
...
他のトランザクションと区別して監視したりアラートを受け取ったりするトランザクションがある場合は、[キートランザクション](/docs/apm/transactions/key-transactions/key-transactions-tracking-important-transactions-or-events)に設定します。これは、一般的な[Apdex T閾値](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#what-is)と異なる、[トランザクション固有のApdex](/docs/apm/new-relic-apm/apdex/change-your-apdex-settings#key-transaction)を使う場合などに有効です。
...
New Relicエージェントがトランザクションとして自動的にキャプチャしないアプリケーションのアクティビティ向けに、カスタムトランザクションを作成できます。詳細については、「[カスタムインストゥルメンテーション](/docs/agents/manage-apm-agents/agent-data/custom-instrumentation)」をご覧ください。
...
<CollapserGroup>
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) > APM >（アプリケーションを選択）> Monitor > Transactions**の順に移動します。
  ...
  >
    ...
    <Callout variant="tip">
      一般的な記録のクリーニングを行っている場合は、[データベースや遅いクエリのトレース](/docs/apm/applications-menu/monitoring/databases-slow-queries-page)、あるいは[エラートレース](/docs/apm/applications-menu/events/viewing-apm-errors-error-traces)も削除することを推奨します。こうすることで、Apdexやデプロイメント情報など他のアプリケーションデータを保持しつつ、潜在的に機密性がデータを削除できます。
    </Callout>
  </Collapser>
</CollapserGroup>
