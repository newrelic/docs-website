...
ご利用のNew Relicエージェントでサーバーサイドコンフィグレーション設定が利用可能な場合、**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services >（アプリを選択）> Settings > Application > Server-side agent configuration**で確認できます。
