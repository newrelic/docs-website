...
<figcaption>
  **APM エラー UI：** **エラー率**チャートには、フィルターが適用されている場合でも、常にすべてのエラーの割合が表示されます。さらにドリルダウンするには、**上位5**チャートと**エラートレース**テーブルについてグループ化とフィルタのオプションを使用します。Use the **Instance** dropdown to focus on a specific instance.
</figcaption>
