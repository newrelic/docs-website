...
New Relic Oneのエラーを使用すると、ユーザーエクスペリエンスに悪影響を与えるコード行を確認し、問題を再現するのに十分なデータを取得して修正することができます。これにより、本番環境で修正が機能していることを確認できます。

New Relic Oneで**エラー**を使用して次の操作を行います。
...
![WebPortal\_\_\_New_Relic_One.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/WebPortal___New_Relic_One.png "WebPortal___New_Relic_One.png")

<figcaption>
  **APM エラー UI：** **エラー率**チャートには、フィルターが適用されている場合でも、常にすべてのエラーの割合が表示されます。さらにドリルダウンするには、**上位5**チャートと**エラートレース**テーブルについてグループ化とフィルタのオプションを使用します。**Instance**ドロップダウンを使用して、特定のインスタンスにフォーカスします。
</figcaption>
...
2. [**上位5つのエラー**チャート](#top5-errors-chart)の一般的なパターン、および同一期間に発生したアラートを関連付けます。グループとフィルタを使用して、エラーイベントや属性を詳細に調べ、エラーメッセージ、トランザクション名、またはその他のグループのパターンを探します。
3. 以下に示す特定のスタックトレースの詳細、ログ、および属性（ホスト、URL、ユーザー、カスタムアトリビュート）を含む、[**エラー トレース**テーブル](#traces-table)の情報を調査して共有します。
...
<CollapserGroup>
  ...
  >
    ...
    * このエラーが最初と最後に発生したのはいつですか?
    ...
    発生が最も多いものから最も少ないもの以外の順にエラートレースデータを並べ替えると便利な場合があります。ソート順やフィルタオプションを変更して、自分やチームにとって最も重要なタイプのエラーのみに集中することができます。例えば：
...
ノイズの多いエラーを「予期されるエラー」としてマークすることで、そのようなエラーを減らします。このようなエラーは、報告されたエラー率やApdexメトリックには影響しません。詳細については、[エラーの管理](/docs/agents/manage-apm-agents/agent-data/manage-errors-apm-collect-ignore-or-mark-expected#expected)を参照してください。
...
### トランザクション外のエラー [#errors-outside-transactions]
