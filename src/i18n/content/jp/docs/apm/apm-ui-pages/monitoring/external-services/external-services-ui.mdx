...
import downstreamEndpoints from 'images/downstream-endpoints.png'

import specificTransaction from 'images/specific-transaction.png'

import tracingLinkInTable from 'images/tracing-link-in-table.png'
...
* APM: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Monitor > External services**に移動します。
...
3. 2 つのサービス間のトランザクションの内訳を表示します。
   ...
   <img title="Screenshot showing calls to various endpoints in downstream service" alt="Screenshot showing calls to various endpoints in downstream service" src={downstreamEndpoints}/>
...
4. 特定のトランザクションに最も時間がかかっていると判断した場合は、そのトランザクションをクリックして、特にその依存関係に注目します。
   ...
   <img title="Screenshot showing the transaction between the Order-Composer service and the warehouse endpoint in the Order-Status service" alt="Screenshot showing the transaction between the Order-Composer service and the warehouse endpoint in the Order-Status service" src={specificTransaction}/>
...
6. ドリルダウンで分散トレースを表示したいポイントに到達したら、右上の **List **をクリックし、次にテーブルの **Traces **をクリックします。

   <img title="Screenshot showing the trace link in the table view" alt="Screenshot showing the trace link in the table view" src={tracingLinkInTable}/>
