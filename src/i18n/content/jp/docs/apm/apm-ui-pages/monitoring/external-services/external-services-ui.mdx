...
APM で監視されるサービスを選択すると、左側のナビゲーション ペインから外部サービス機能を利用できるようになります。これを開くには、 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & Services**に移動します。アプリを選択し、 **Monitor** \[監視] セクションで **External services** \[外部サービス]をクリックします。
