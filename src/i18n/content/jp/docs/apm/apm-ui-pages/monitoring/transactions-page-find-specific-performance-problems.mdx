---
title: トランザクションページ：特定のパフォーマンス問題を突き止める
tags:
  - APM
  - APM UI pages
  - Monitoring
metaDescription: 'New Relic APM''s Transactions page shows app, browser, or other transaction details, average response time, and throughput as requests per minute or CPU usage.'
---

APMの**トランザクション**ページは、パフォーマンスの問題を微調整したり、エラーを解決したりするのに適した[トランザクション](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm)を特定するのに役立ちます。このページには、選択したアプリケーションのトランザクション**リクエスト**、[（ウォールクロックタイム](#wall_clock_time)のトップ5位のトランザクション（パーセント）、関連のある[スループット](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#throughput)（1分あたりのリクエスト数（ **rpm** ））がリストされます。
...
選択されたアプリケーションによっては、**Transactions**ページに[利用可能なトランザクションのタイプ](#txn-type-dropdown)から選択するためのドロップダウンが掲載されている場合があります。New Relicは、[リクエストのタイプ（](/docs/apm/applications-menu/features/request-queuing-tracking-front-end-time)[ウェブトランザクション](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#transaction)または[ウェブ以外のトランザクション](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#non-web-transaction)）によって処理時間を測定します。これには、以下の項目が含まれます。
...
1. 次のうち1つを行います。
   * **[one.newrelic.com](https://one.newrelic.com) > APM >（アプリケーションを選択）> Monitor > Transactions**の順に移動します。
   * **[one.newrelic.com](https://one.newrelic.com) > Entity explorer > (アプリを選択) > Monitor > Transactions**の順に進みます。
2. 該当する場合：利用可能な[タイプ](#tx_types)のトランザクションの表示を変更するには、**Type**を選択します。
...
チャートの背景が薄い赤色の場合は、アラート条件の[**Critical**閾値](/docs/alerts/new-relic-alerts/defining-conditions/define-thresholds-trigger-alert)を超過している期間を示します。[New Relic Alerts](/docs/alerts/new-relic-alerts/getting-started/introduction-new-relic-alerts)で[インシデントの詳細](/docs/alerts/new-relic-alerts/reviewing-alert-incidents/view-violation-event-details-incidents)を表示するには、チャートをクリックします。
...
<CollapserGroup>
  ...
  >
    ![その他のトランザクションタイプを選択できるドロップダウンを表示するスクリーンショット。](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/trx-type-dropdown.png "trx-type-dropdown.png")
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            これは、1分あたりの操作リクエストまたは**rpm**として表示される[スループット](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#throughput)の測定値です。（ウェブスループットの場合、1分あたりのリクエストは、1分あたりのページ数（**ppm**）と呼ばれることがあります）。1分あたりのリクエスト件数が最も多いトランザクションが、このランキングのトップに表示されます。
          </td>
        </tr>
      </tbody>
    </table>
  ...
  >
    ![チャートを変更するセレクターを表示するスクリーンショット。](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/chart_type_selectors.png "chart_type_selectors.png")
  ...
  >
    ブラウザの情報を表示するには、[New Relic Browserを有効にする](/docs/new-relic-browser/browser-settings)必要があります。次に、対応する[ブラウザのリクエスト](/docs/browser/new-relic-browser/additional-standard-features/page-views-understanding-your-sites-popularity)データを表示するには、次のいずれかのオプションを使用します。
  ...
  >
    ...
    トランザクションの**内訳テーブル**では、**（トランザクションあたりの）平均通話回数**列に色分けされたセグメント値が含まれます。これらは、閾値を超過しているインストゥルメントされたセグメントを示します。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        `*`アステリスクのマークが付いたApdexスコアは、そのトランザクション名に対してエージェントが収集したサンプル数が100以下であることを示しています。これらのトランザクションのデータは、スループットがより高いトランザクションのデータに比べて精度が低くなる可能性があります。
      </td>
    </tr>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        2. 利用可能であれば、**Summary**、**Trace details**、または**Database queries**の[表示を選択](/docs/apm/transactions/transaction-traces/introduction-transaction-traces#find-view)します。
        3. トランザクショントレースをフルスクリーン表示に拡大するには、フルスクリーン<Icon style={{color: '#4392AA'}} name="fe-maximize-2"/>
           アイコンを選択します。
    ...
    <tr>
      ...
      <td>
        <Callout variant="caution">
          [**Delete all traces**](/docs/traces/deleting-transaction-traces)を選択すると、それらを回復することはできません。
        </Callout>
      </td>
    </tr>
  </tbody>
