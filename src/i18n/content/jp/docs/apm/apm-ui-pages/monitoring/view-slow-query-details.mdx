...
APMでは、[トランザクショントレース](/docs/apm/transactions/transaction-traces/introduction-transaction-traces)が収集されると、[<DoNotTranslate>**Databases**</DoNotTranslate>](/docs/apm/applications-menu/monitoring/databases-slow-queries-page)ページで追加の<DoNotTranslate>**Slow query**</DoNotTranslate>データが利用できる場合があります。スロークエリデータとは何か、それを見つける場所、その設定方法を見てみましょう。
