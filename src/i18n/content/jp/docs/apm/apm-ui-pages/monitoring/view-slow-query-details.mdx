...
APMでは、[トランザクショントレース](/docs/apm/transactions/transaction-traces/introduction-transaction-traces)が収集されると、**[データベース](/docs/apm/applications-menu/monitoring/databases-slow-queries-page)**[ページに追加の**スロークエリ**](/docs/apm/applications-menu/monitoring/databases-slow-queries-page)**データが表示される場合があります。スロークエリデータの内容、検索場所、および設定方法について見てみましょう。**

**また、ヒストグラムとヒートマップを使用して、さまざまなメトリクスの値の分布を視覚化することもできます。低速ページのパフォーマンスを把握するのに役立ちます。方法については、この短いビデオ（4分18秒）をご覧ください。<Video
  id="Gh4Km9aJoBE"
  type="youtube"
...
</CollapserGroup>## 低速クエリのメトリックス [#slowsql_details]低速クエリのリストには、次のメトリックスが含まれています。<table>
  <thead>
    <tr>
      <th width={150}>
        **メトリック**
...
</table>さらに詳細を表示するには、個別のスロークエリを選択します：<table>
  <thead>
    <tr>
      <th width={150}>
        **メトリック**
...
</table>MySQLの場合、当社はクエリ解析を提供します。これは、クエリで発生した内容を説明し、スピードアップするために必要なものが何か説明します。その他のデータベースでは、可能であれば説明プランを表示します。## トラブルシューティング：スロークエリデータなし [#troubleshooting]スロークエリデータが検出されない場合、次のいずれかの理由が考えられます：* 低速クエリ機能が無効になっている。
