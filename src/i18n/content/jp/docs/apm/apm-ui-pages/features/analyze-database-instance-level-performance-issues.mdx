...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      <DoNotTranslate>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Monitor > Databases > (select a trace) > Trace details:**</DoNotTranslate> アプリのパフォーマンス問題の原因となっている特定のデータベースとインスタンスに関する情報を表示するには、<DoNotTranslate>**Database**</DoNotTranslate>アイコンを選択します。
    </figcaption>
  </Collapser>
  ...
  >
    顧客のビジネスに悪影響を与える前に、アプリとデータベースインスタンスの間のパフォーマンス問題を通知するには、[アラート](/docs/alerts/new-relic-alerts/getting-started/welcome-new-relic-alerts)を使用します。定義した<DoNotTranslate>**Critical**</DoNotTranslate>の閾値にエスカレートする場合、PagerDuty、webhookなどを介して、[適切な担当者](/docs/alerts/new-relic-alerts/managing-notification-channels/notification-channels-controlling-where-send-alerts)に自動的に通知する[アラートポリシー](/docs/alerts/new-relic-alerts/configuring-alert-policies/alert-policy-workflow)を作成できます。
