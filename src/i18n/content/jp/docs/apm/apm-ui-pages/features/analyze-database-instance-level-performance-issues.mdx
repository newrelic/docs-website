...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services >（アプリを選択）> Monitor > Databases >（トレースを選択）> Trace details：**アプリのパフォーマンスの問題の原因となっている特定のデータベースとインスタンスに関する情報を表示するには、**データベース**アイコンを選択します。
    </figcaption>
  </Collapser>
