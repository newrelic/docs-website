...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services >（select an app）> Databases >（select a database operation）>（select a slow query）> Trace details：**特定のデータベースとインスタンスを識別するスロークエリトレースの例を紹介します。
    </figcaption>
  </Collapser>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services >（select an app）> Monitor > Databases >（select a trace）> Trace details：**アプリのパフォーマンス問題の原因となっている特定のデータベースとインスタンスに関する情報を表示するには、**Database**アイコンを選択します。
    </figcaption>
  </Collapser>
  ...
  >
    ...
    * パフォーマンスに予期しない影響を与える可能性のある異常値を特定します。
      ...
      <figcaption>
        **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services >（select an app）> Monitor > Service map >（select a map）>（select a datastore node）**：**インスタンス**ノードを選択することで、パフォーマンスの問題をさらに掘り下げることができます。選択したノード内では、個々のインスタンスを有効または無効にすることができます。また、マップを保存すると、選択内容が保存されます。
      </figcaption>
  </Collapser>
