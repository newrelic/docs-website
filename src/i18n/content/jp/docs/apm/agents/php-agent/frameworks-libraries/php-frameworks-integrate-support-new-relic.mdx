...
New Relic は、Web トランザクション名と、New Relic の JavaScript ヘッダーとフッターを挿入する場所を識別できます。 <InlinePopover type="browser"/>。ただし、エンド ユーザーのエクスペリエンスを向上させ、パフォーマンスを向上させ、コードのメンテナンスを容易にするために、フレームワーク開発者はこの作業を自分で行う必要があります。
