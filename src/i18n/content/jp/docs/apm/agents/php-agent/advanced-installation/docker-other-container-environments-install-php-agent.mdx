...
アプリケーションの例を見るには、 [New Relic's Explorers Hub](https://discuss.newrelic.com/t/relic-solution-php-agent-and-daemon-containers/84841) にアクセスしてください。
...
4. デフォルトでは、最初のトランザクションにより、エージェントはデーモンの起動とアプリケーション接続の初期化をトリガーします。しかし、パフォーマンス上の理由から、エージェントはこれらの操作が完了するのを待たずに接続を初期化するため、コンテナが起動した後の最初の数件のトランザクションが失われる可能性があります。これを防ぐには、 `newrelic.daemon.start_timeout` と `newrelic.daemon.app_connect_timeout` の両方のエントリを `newrelic.ini` ファイルの中で推奨値に設定してください。
