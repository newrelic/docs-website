...
Drupal 7.15 では、**Compress cached pages** \[キャッシュされたページの圧縮] がデフォルトでオンになっています。**Cache pages for anonymous users** \[匿名ユーザーのページをキャッシュする] も選択すると、 <InlinePopover type="browser"/>匿名ユーザーに提供されるページには JavaScript は挿入されません。これは、Drupal のページがキャッシュに保存される前に (gzip を使用して) データベースから直接圧縮されるため、New Relic の PHP エージェントは HTML を解析する機会がないためです。
