...
2. `newrelic.ini`ファイル内のライセンスキー設定を<InlinePopover type="licenseKey"/>に置き換えます。 必要に応じて他のセッションも変更します (例: `newrelic.appname` )。
...
アプリでトラフィックを生成し、結果が[APM <DNT>**Summary**</DNT>ページ](/docs/apm/apm-ui-pages/monitoring/apm-summary-page-view-transaction-apdex-usage-data/)に表示されるまで 3 ～ 5 分待ちます。
