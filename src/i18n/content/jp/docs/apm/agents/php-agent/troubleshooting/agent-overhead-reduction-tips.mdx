...
パフォーマンスの低いクロックソースは、すべてのスパンの時間をチェックするエージェントに影響を及ぼす可能性があります。適切な[クロックソース](https://support.newrelic.com/s/hubtopic/aAX8W0000008a7rWAA/relic-solution-php-agent-cpu-overhead-tips)を使用していることを確認してください。Fargate はクロックソースの影響を受け、追加の[オーバーヘッド](/docs/apm/agents/php-agent/troubleshooting/fargate-nitro-clock-performance-impact/)を引き起こすことが知られています。