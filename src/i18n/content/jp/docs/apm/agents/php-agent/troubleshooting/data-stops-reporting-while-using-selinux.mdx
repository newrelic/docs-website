...
<CollapserGroup>
  ...
  >
    ...
    1. 次のコマンドを使用して SELinux `sysconfig`ファイルを編集します。

       ```
       vi /etc/sysconfig/selinux
       ```

    2. `SELINUX=`設定を見つけて、次のように変更します。
...
エージェントの PHP 拡張機能とデーモンは、デフォルトで`/tmp/.newrelic.sock` (Unix ソケット) を通じて通信します。PHP 拡張機能とデーモンが通信できるように SELinux が構成されていない場合、SELinux が`enforcing`モードに設定されている場合、これら 2 つのエージェント コンポーネントは相互に通信できなくなります。
