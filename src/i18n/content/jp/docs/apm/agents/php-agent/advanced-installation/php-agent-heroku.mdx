...
[Heroku は](//www.heroku.com/)、PHP を含むさまざまな言語で Web アプリケーションをホストするためのサービスとしてのプラットフォーム (PaaS) ソリューションです。New Relic を使用すると、次のメトリクスを使用して Heroku を拡張できます。 [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm/getting-started/welcome-new-relic-apm)そして[<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/new-relic-browser) 。
