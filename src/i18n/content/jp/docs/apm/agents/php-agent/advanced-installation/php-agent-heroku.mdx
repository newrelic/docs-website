...
PHPエージェントをインストールする前に、PHP WebアプリケーションがHerokuにインストールされ、実行されていることを確認してください。詳細については、[Getting Started with PHP on Heroku](https://devcenter.heroku.com/articles/getting-started-with-php)を参照してください。
