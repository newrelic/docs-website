...
[Heroku](//www.heroku.com/)は、PHP を含むさまざまな言語で Web アプリケーションをホストするための Platform as a Service (PaaS) ソリューションです。 New Relic を使用すると、 [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm/getting-started/welcome-new-relic-apm)と[<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/new-relic-browser)のメトリクスを使用して Heroku を拡張できます。
