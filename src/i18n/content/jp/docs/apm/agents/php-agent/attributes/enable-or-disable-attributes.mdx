...
<CollapserGroup>
  ...
  >
    ...
    すべての属性をオンまたはオフにします。 <InlinePopover type="browser"/>。これは、ページ ビュー イベントに添付されるデータです。`newrelic.attributes.enabled`が`false`の場合、このプロパティがどのように設定されているかに関係なく、属性はブラウザ監視に送信されません。
  </Collapser>
