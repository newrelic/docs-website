...
3. Web サーバーを再起動して、 `newrelic` がモジュールリストに表示されることを確認してください。
...
![phpinfo()のスクリーンショット](src/images/phpinfo_nr_0.png)

<figcaption>
  **サンプル `phpinfo()` ファイル**: PHP
  エージェントとプロキシデーモンの両方を設定した後、数分以内に New Relic [APM
  **Summary** ページ](/docs/apm/applications-menu/monitoring/apm-overview-page)
  にデータが表示されない場合は、phpinfo() ファイルの New Relic
  セクションを確認し、New Relic がインストールされていることを確認してください。
</figcaption>
