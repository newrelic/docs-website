...
セットアップが成功した後、メトリクスが New Relic に表示されるまでに最大 15 分かかる場合があります。[APM**概要**ページ](/docs/apm/applications-menu/monitoring/applications-overview-dashboard)を表示するには: **\[one.newrelic.com** ]( [https://one.newrelic.com](https://one.newrelic.com) ) に移動します。> APM > (アプリを選択)\*\*。