...
New Relic PHPエージェントを使用して、[「私のアプリが遅い」チュートリアル](/docs/journey-app-slow/root-causes/)でアプリのパフォーマンスの問題を解決します。
...
<figcaption>
  <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (select an app) > Summary**</DNT>：PHPエージェントをインストールすると、アプリのパフォーマンスの概要を表示できます。
</figcaption>
...
<DNT>**standard installations**</DNT>については、以下を参照してください。
...
その他の種類のPHPインストレーションや<DNT>**advanced installation**</DNT>に関するトピックについては、以下をご覧ください。
...
* [ページロードタイミング](/docs/agents/php-agent/features/page-load-timing-php)：PHPエージェントを
