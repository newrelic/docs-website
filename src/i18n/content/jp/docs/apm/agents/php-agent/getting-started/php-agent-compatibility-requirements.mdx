...
New Relicは、新しいPHPマイナーバージョンへの迅速な対応に取り組んでいます。私たちの目標は、60日以内に最新リリースのGAをサポートすることです。緊急のセキュリティニーズに対応するために開発の優先順位が変更される可能性がありますが、私たちはこのギャップを縮め、当社のエージェントが最新のPHPアプリケーションを監視する最も信頼性の高い方法であり続けるよう全力を尽くしています。
...
[ファイバー](https://www.php.net/manual/en/language.fibers.php)はバージョン12.10以降でサポートされています。ファイバーの計装を無効にするには、設定ファイルに`newrelic.fibers.disabled = true`を追加してください。
...
その他の一般的なPHPフレームワークのサポートを提案する場合は、[サポートフォーラム](https://support.newrelic.com/s/hubtopic/Topic__c/Default?c__categories=%5B%7B%22icon%22%3A%22standard%3Adefault%22%2C%22id%22%3A%22a6c8W000000EeskQAC%22%2C%22sObjectType%22%3A%22Category__c%22%2C%22title%22%3A%22PHP%20Agent%22%2C%22titleFormatted%22%3A%22PHP%20Agent%22%7D%5D)を閲覧し、`Feature Idea`を作成してください。詳細については、[PHPフレームワーク：New Relicのサポートのインテグレーション](/docs/apm/agents/php-agent/frameworks-libraries/php-frameworks-integrate-support-new-relic)を参照してください。
