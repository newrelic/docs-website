...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        PHP 8.0、8.1、または8.2が、PHPエージェントなどのオブザーバビリティ拡張機能を検出すると、[ジャストインタイムコンパイル](https://php.watch/articles/jit-in-depth)が無効になります。
...
* サポートされていない PHP バージョンまたはプラットフォーム (32 ビット OS、FreeBSD、ZTS ビルドなど) を使用している場合は、PHP エージェント パッケージの自動更新メカニズムを無効にすることをお勧めします。自動アップグレードを防止しないと、新しいバージョンのエージェントが誤ってインストールされる可能性があります。新しいバージョンでは、APM データ収集に不可欠な重要な機能がサポートされなくなる可能性があり、その結果、監視機能が中断される可能性があります。
...
PHPエージェントをインストールするには、以下に対するルートアクセスが**必要です**。

* PHP拡張ディレクトリへの読み取り/書き込みアクセス
* PHP設定ディレクトリ（推奨）または`php.ini`のPHP設定データへの読み取り/書き込みアクセス
* ウェブサーバーまたはPHPプロセスプールを再起動します

エージェントの実行にはルートアクセスは**必要ありません**。エージェントには、次のファイルへの読み取り/書き込みアクセスが必要です（これらのファイルの場所は[PHPエージェント設定でカスタマイズ](/docs/php/php-agent-phpini-settings)できます）。
...
このため、ベンダーによって正式にサポートされている最新バージョンの OS を常に使用することをお勧めします。エージェントの最新バージョンはサポート終了を過ぎた OS バージョンでも動作する可能性がありますが、古いバージョンの PHP エージェントのテストや正式なサポートは行われなくなりました。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        * ARM64は、PHPバージョン8.0、8.1、および8.2でのみサポートされます。
...
<table>
  <tbody>
    ...
    <tr>
      <td>
        [Drupal 6.x、7.x、8.xおよび9.1〜9.5](/docs/agents/php-agent/frameworks-libraries/drupal-specific-functionality)
      </td>
    ...
    <tr>
      <td>
        Laravel 6.x、7.x、8.x、9.xおよび10.x
      </td>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        PHPバージョン7.0～7.4および8.0～8.2では、コードレベルのメトリクスをレポートします（リリース10.4.0以降）。
      </td>
    </tr>
