...
インストレーションの際には、New Relic<InlinePopover type="licenseKey" />が必要です。ライセンスキーは40文字の16進数の文字列で構成され、[サインアップ](https://newrelic.com/signup)時にNew Relicから提供されます。詳細については、PHP[インストールスクリプト](/docs/php/the-newrelic-install-script)とPHP INIファイルの[ライセンスキー設定](/docs/php/php-agent-phpini-settings#inivar-license)を参照してください。
