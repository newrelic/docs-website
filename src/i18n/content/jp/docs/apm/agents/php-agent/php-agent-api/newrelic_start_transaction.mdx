...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        オプション。デフォルトは、New Relic エージェントの`newrelic.ini`ファイルに設定されている [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) です。

        トランザクションを別の New Relic アカウントに報告する場合は、別の [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) を提供します。 設定されている場合、このライセンスは、INI ファイルで構成されているすべてのディレクトリごとおよびグローバルな既定のライセンスに取って代わります。
      </td>
    </tr>
  </tbody>
</table>
...
この例では、アプリ名と [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)を手動で指定します。
