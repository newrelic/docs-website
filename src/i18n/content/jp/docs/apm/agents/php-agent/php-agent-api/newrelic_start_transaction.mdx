...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        オプション。デフォルトは、New Relic エージェントの`newrelic.ini`ファイルに設定されている<LicenseKey/>です。

        トランザクションを別の New Relic アカウントに報告する場合は、別の<LicenseKey/>を提供します。設定されている場合、このライセンスは、INI ファイルで構成されているすべてのディレクトリごとおよびグローバルな既定のライセンスに取って代わります。
      </td>
    </tr>
  </tbody>
</table>
...
この例では、アプリ名と<LicenseKey/>を手動で指定します。
