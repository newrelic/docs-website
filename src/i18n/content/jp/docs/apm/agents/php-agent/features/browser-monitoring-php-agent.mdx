...
## 自動インストルメンテーションの使用または無効化 [#auto\_instrumentation][#auto_instrumentation]
...
## エージェントAPIを介した手動での計測 [#manual\_instrumentation][#manual_instrumentation]
...
### nonceなどのカスタムアトリビュートを追加 [#nonce-attribute]

サイトで`nonce`属性を必要とする[コンテンツ セキュリティ ポリシー (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)を使用している場合は、両方の API 関数に`false`を渡して、 `<script>`タグのない生の JavaScript 文字列を返すことができます。次に、必要な属性を使用して自分でタグを構築できます。
...
引数として`false`を渡すと、関数が通常出力するスクリプト タグが抑制されるため、生の JavaScript 文字列を、必要な属性を持つ独自の`<script>`タグでラップできます。
...
## 様々なフレームワークに手動で対応 [#manual\_generic][#manual_generic]
...
<CollapserGroup>
  <Collapser id="manual_wp" title="ワードプレス">
