...
[WordPress ウェブサイトに New Relic をインストールする](/docs/agents/php-agent/installation/install-php-agent-shared-hosting-service)と、PHP エージェントは追加のメトリクスを受け取ります。**WordPress**ページが New Relic ユーザー インターフェイスに表示されます**[。one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services > (選択した WordPress アプリ)**に移動します。
