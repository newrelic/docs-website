...
[WordPress ウェブサイトに New Relic をインストールする](/docs/agents/php-agent/installation/install-php-agent-shared-hosting-service)と、PHP エージェントは追加のメトリクスを受け取ります。New Relic ユーザー インターフェイスに**WordPress**ページが表示されます。 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (WordPress アプリを選択)**に移動します。
