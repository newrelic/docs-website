...
このドキュメントでは、**インストール後**にNew Relic PHPエージェントを設定する方法について説明します。

## 開始する前に [#before-you-begin]

PHPエージェントをまだインストールしていない場合は、 [PHP エージェントインストレーションの概要](/docs/apm/agents/php-agent/installation/php-agent-installation-overview)を参照してください。

* エージェントのダウンロード
* インストレーションプロセスを完了する

エージェントがインストールされたら、ここに戻って構成設定を微調整してください。
...
* `newrelic.application_logging.forwarding.enabled` ログ転送を有効または無効にします。
* `newrelic.application_logging.forwarding.max_samples_stored` アプリからNew Relicに転送するログの数を制限します。
* `newrelic.application_logging.forwarding.log_level` アプリからNew Relicに転送するログの種類を選択します。
* `newrelic.application_logging.forwarding.labels.enabled` エージェント転送されたログメッセージによるカスタムタグ（ラベル）の送信を有効または無効にします。
* `newrelic.application_logging.forwarding.labels.exclude` エージェント転送ログメッセージでどのラベルを送信するかを選択できます。
...
<CollapserGroup>
  ...
  <Collapser id="cfg-application_logging_forwarding_labels-enabled" title="newrelic.application_logging.forwarding.labels.enabled">
    ...
    エージェント転送ログを含むカスタムタグ（ラベル）をNew Relicに送信できるようにします。

    この機能は、PHPエージェントの11.7.0リリースで追加されました。

    タグは、属性名の前にプレフィックス`tags.`が挿入されて、ログイベント上の属性として表示されます。
  ...
  <Collapser id="cfg-application_logging_forwarding_labels-exclude" title="newrelic.application_logging.forwarding.labels.exclude">
    ...
    エージェント転送ログとともに送信されるカスタムタグ（ラベル）のフィルタリングを有効にします。

    値はカンマで区切られた文字列のリストです。リスト内の各文字列は、大文字と小文字を区別せずに現在定義されているラベルと照合され、完全に一致するものは転送対象のラベルから削除されます。

    たとえば、次のラベルの定義を考えてみましょう。

    ```ini
    newrelic.labels = "Data Center, Business, Cost code"
    ```

    次の除外ルールでは、大文字と小文字を区別せずに比較が行われるため、 `Business`と`Cost code`ラベルが除外されます。
    ...
    <Callout variant="tip">
      `newrelic.labels` INI値を使用すると、コンマ（&quot;,&quot;）文字を含むラベルを定義できます。これらのラベルは、除外ルールを区切るためにコンマを使用するため、 `exclude` INI値を使用して除外することはできません。これらのラベルをログメッセージに追加しないようにする場合は、ラベル値にコンマ文字を使用しないことをお勧めします。
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="inivar-sampler-remote-parent-sampled" title="newrelic.distributed_tracing.sampler.remote_parent_sampled">
    ...
    このオプションは、上流エンティティからの親スパンがサンプリングされたときに、エージェントがサンプリングスパンをどのように処理するかを定義します。たとえば、remote\_parent\_sampled: always\_onを設定すると、上流のエンティティが親をサンプリングした場合、エージェントはすべてをサンプリングすることになります。

    設定可能な値は次のとおりです。

    * &quot;default&quot;：New Relicの標準サンプリングルールを使用する
    * &quot;always\_on&quot;：上流の親がサンプリングされたスパンを常にサンプリングする
    * &quot;always\_off&quot;：上流の親がサンプリングされたスパンを常にスキップする
  </Collapser>
  ...
  <Collapser id="inivar-sampler-remote-parent-not-sampled" title="newrelic.distributed_tracing.sampler.remote_parent_not_sampled">
    ...
    このオプションは、上流エンティティからの親スパンがサンプリングされなかった場合に、エージェントがサンプリングスパンをどのように処理するかを定義します。たとえば、remote\_parent\_not\_sampled: always\_offを設定すると、上流のエンティティが親をサンプリングしなかった場合、エージェントは何もサンプリングしません。

    設定可能な値は次のとおりです。

    * &quot;default&quot;：New Relicの標準サンプリングルールを使用する
    * &quot;always\_on&quot;：上流の親がサンプリングされなかったスパンを常にサンプリングする
    * &quot;always\_off&quot;：上流の親がサンプリングされなかったスパンのサンプリングを常にスキップする
  </Collapser>
  ...
  <Collapser id="inivar-err-enabled" title="newrelic.error_collector.enabled">
    ...
    エラーコレクターを有効または無効にします。有効にすると、エージェントはPHPエラーと例外を収集し、New Relic UIに報告します。

    ### PHPエージェントがエラーと例外を収集する方法

    PHPエージェントは、次の方法でエラーと例外を自動的に収集します。

    * **キャッチされない例外**：エージェントは、トランザクション実行中に発生するキャッチされない例外（PHP5では`Exception`のインスタンス、PHP 7以降では`Throwable`のインスタンス）を自動的にキャプチャします。
    * **PHP エラー**：エージェントは、設定されたエラーレベルに基づいてPHPエラーをキャプチャします（下記の[`error_collector.ignore_errors`](#inivar-err-ignore-errors)を参照）。
    * **手動エラー報告**：処理された例外やカスタムエラー条件など、自動的にキャプチャされないエラーまたは例外を手動で報告するには、 [`newrelic_notice_error()`](/docs/agents/php-agent/php-agent-api/newrelic_notice_error) API を使用します。
  ...
  <Collapser id="inivar-ignore-framework-exception-handler" title="newrelic.error_collector.ignore_framework_error_exception_handler">
    ...
    有効にすると、New Relicエラーコレクターはフレームワークエラーまたは例外ハンドラーによって処理されるエラーまたは例外を無視します。これは、フレームワークによって再スローされる例外のみを表示する場合、またはエラー/例外をフィルターし、計装が必要な際にNew Relic APIを手動で呼び出す、独自の処理ロジックを例外処理スタックに挿入する場合に便利です。
  </Collapser>
...
<CollapserGroup>
  <Collapser id="inivar-code-level-metrics-enabled" title="newrelic.code_level_metrics.enabled">
    ...
    <Callout variant="important">
      `newrelic.code_level_metrics.enabled` INI変数が10.4.0.316リリースで追加され、デフォルトで無効になっていました。コードレベルのメトリクスは、バージョン10.6.0～11.10.0ではデフォルトで有効になっています。コードレベルのメトリクスは、バージョン12.0.0以降、デフォルトで無効になっています。
    </Callout>
  </Collapser>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            PHPエージェントバージョン11.10に導入
          </td>
        </tr>
      </tbody>
    </table>

    Composer APIがパッケージデータのランタイム環境をサンプリングする頻度を制御します。`true`に設定すると、サンプリングはプロセスごとに1回だけ実行されます。falseの場合、Composerはリクエストごとに環境をサンプリングし、このパッケージ検出が実行される頻度を増やします。
  </Collapser>
</CollapserGroup>
