...
PHPエージェント10.1.0リリース以降、[Logs in context](/docs/apm/new-relic-apm/getting-started/get-started-logs-context%5D)のサポートがエージェントに直接追加され、[サポート対象のロギングフレームワーク](/docs/logs/logs-context/configure-logs-context-php)で簡単に使用できるようになりました。
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      エージェントリリース10.1.0では、エージェント設定ファイルでこの機能がデフォルトで有効になっています。ただし、ログ転送はデフォルトで無効になっています。その結果、デフォルトの設定では、ログメトリクスのみがNew Relicに送信されます。このリリースでは、ログ転送を手動で有効にする必要があります。PHPエージェントの今後のリリースでは、ログ転送がデフォルトで有効になります。
    </Callout>
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      エージェントリリース10.1.0では、エージェント設定ファイルでこの機能がデフォルトで無効になっています。
    ...
    ログをNew Relicに送信したくない場合は、これを`false`に設定します。
  </Collapser>
