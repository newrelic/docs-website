...
<CollapserGroup>
  ...
  >
    この例では、Dockerfile は汎用 PHP イメージを、Debian の単一アプリケーション用のアプリケーション ファイルで拡張します。[New Relic ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)を Dockerfile に直接追加するか、 `docker run`コマンドで環境変数を使用できます。
