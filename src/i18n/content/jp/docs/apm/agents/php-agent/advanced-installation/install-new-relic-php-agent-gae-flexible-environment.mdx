...
<CollapserGroup>
  ...
  >
    1. 標準的な手順に従って、特定のアプリ サーバーに[New Relic PHP エージェントをインストールし、](/docs/agents/php-agent/installation/php-agent-installation-overview)<InlinePopover type="licenseKey" />します。
  ...
  >
    この例では、Dockerfile は汎用 PHP イメージを、Debian の単一アプリケーション用のアプリケーション ファイルで拡張します。<InlinePopover type="licenseKey" />を Dockerfile に直接追加するか、 `docker run`コマンドで環境変数を使用できます。
