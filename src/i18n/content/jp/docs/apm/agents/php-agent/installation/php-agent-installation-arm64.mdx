...
2. `newrelic.ini` を作成し、正しい場所に配置したら、そのファイルを編集して以下を追加してください。
   ...
   * 有効な [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) を行に追加する `newrelic.license ="INSERT_YOUR_LICENSE_KEY"` .
   * [one.newrelic.com](https://one.newrelic.com) の行に表示されているアプリケーション名を変更する `newrelic.appname ="PHP Application"` （オプション）です。