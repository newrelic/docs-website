...
[ブラウザ エージェントをページに自動的に追加するか、JavaScript スニペットをコピーして貼り付けて特定のページに展開することで、ブラウザ エージェントをインストール](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent)できます。を制御することもできます。 <InlinePopover type="browser"/>エージェントを使用して<InlinePopover type="apm"/>エージェント API 呼び出し。詳細については、 [「ブラウザ監視と PHP エージェント」](/docs/agents/php-agent/features/new-relic-browser-php-agent)を参照してください。