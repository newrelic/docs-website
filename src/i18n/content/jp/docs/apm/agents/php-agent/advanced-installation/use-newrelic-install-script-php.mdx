...
- 正しい PHP モジュールを所定の位置にコピーします。
- サンプル `ini` ファイルをインストールします。
- New Relic のプロキシ・デーモンを設定する。

<Callout variant="important">
  スクリプトを実行する前に、
  [複数のインストールや標準外の場所での作業](#install-multiple)
  に関する情報を確認してください。
</Callout>
...
- tarball を介して。インストールスクリプトは、 `tar` アーカイブファイルを解凍したディレクトリにあります。 `tar` 配布物に含まれるスクリプトには、パッケージマネージャーが処理しないインストール手順が含まれています。

## 複数の PHP をインストールして作業する [#install-multiple]
...
このスクリプトには、 [install](#install-install) と [uninstall](#install-uninstall) という 2 つのモードがあります。引数なしで起動した場合、モードの選択を求められます。
...
{' '}
<Collapser id="license-key" title="2.New Relic のライセンスキーを入力します。">
  プロンプトで、New Relic
  [のライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)
  を入力します。このキーは、残りのインストールプロセスで作成される INI
  ファイルに挿入されます。
</Collapser>
...
{' '}
<Collapser id="archive-file" title="6.アーカイブファイルをメモする。">
  インストールアーカイブファイルの名前と場所に注意してください。このファイルは
  `/tmp/nrinstall-<var>nnnn</var>.tar` に置かれ、インストールログと New Relic
  テクニカルサポートのトラブルシューティングに役立つシステム情報が含まれています。
</Collapser>
...
## install_daemon モードの使用 [#install-daemon-mode]
...
## PHP インストールのトラブルシューティング [#troubleshoot]
