...
<CollapserGroup>
  ...
  >
    次のメソッドの<DNT>**one**</DNT>を使用してインストール モードを呼び出します。
  ...
  >
    プロンプトで、New Relic <InlinePopover type="licenseKey"/>を入力します。 このキーは、インストール プロセスの残りの過程で作成されるすべての INI ファイルに挿入されます。
  </Collapser>
  ...
  >
    ...
    <DNT>**Example:**</DNT> メニューには 7 つのオプションが表示されます。
  ...
  <Collapser
    ...
    title="7.設定を微調整します。"
  >
    New Relic を正常にインストールし、Web サーバーを再起動すると、アプリケーションに関するデータの収集を開始できます。 数分後、 [APM <DNT>**Summary**</DNT>ページ](/docs/apm/apm-ui-pages/monitoring/apm-summary-page-view-transaction-apdex-usage-data/)にデータが表示され始めます。
