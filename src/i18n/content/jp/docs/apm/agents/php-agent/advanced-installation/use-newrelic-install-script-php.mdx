...
インストールプロセスに問題があった場合、または [UI にデータが表示されない場合](/docs/agents/php-agent/troubleshooting/no-data-appears-php) 、New Relic のサポートに連絡する前に [PHP トラブルシューティング手順](/docs/agents/php-agent/troubleshooting) を確認してください。バグレポートには必ず [アーカイブファイル](#archive-file) を添付してください。また、ウェブサーバーが生成する [`phpinfo()`](/docs/agents/php-agent/troubleshooting/submitting-troubleshooting-results-php) 関数の出力も添付してください。
