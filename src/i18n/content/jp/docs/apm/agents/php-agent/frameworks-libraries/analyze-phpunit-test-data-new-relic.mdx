...
## PHPUnit テストイベントの有効化 [#phpunit-enable]

PHPUnit のテストイベントを有効にするには

1. `newrelic.phpunit_events.enabled` 設定を `newrelic.ini` ファイルから探して追加し、 `true` に設定してください。
2. 設定変更を有効にするために、Web サーバー（apache、PHP-FPM、Nginx など）を再起動してください。

<Callout variant="tip">
  例外メッセージは、イベントとともに収集・送信されます。また、PHPUnit の
  `--disallow-test-output`
  オプションを使うと、出力を行うテストに"危険なフラグを立てることができ、"
  テストイベントに問題のある出力が含まれるようになります。
</Callout>
...
<figcaption>
  `TestSuite` と `Test` のイベントを照会して作成した PHPUnit
  テストダッシュボードの例。
</figcaption>
...
上の例では、run `5bb37ccee2a1dbc7` をクリックすることで、2 つのテストのうち 1 つ、 `testFoo` が失敗したことがわかります。この例を構成する NRQL クエリは次のとおりです。
