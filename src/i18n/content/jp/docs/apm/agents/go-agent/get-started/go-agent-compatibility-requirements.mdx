...
システムが要件を満たしていることを確認できました。

1. **[Goエージェントをインストールする](/docs/apm/agents/go-agent/installation/install-new-relic-go)** - ステップバイステップのインストレーションガイドに従ってください
2. **[エージェントを構成する](/docs/apm/agents/go-agent/configuration)** - 環境に合わせてエージェントの動作をカスタマイズします
3. **[計装を追加する](/docs/apm/agents/go-agent/instrumentation)** - アプリケーションの特定の操作を監視します
4. **[機能の詳細](/docs/apm/agents/go-agent/features)** - 高度な監視機能を確認します

<Callout variant="tip">
  **特定のフレームワークを使用していますか?**上記のインテグレーションの多くには、特定のセットアップ手順が記載されています。すぐに開始するには、フレームワークの[GitHubの例](https://github.com/newrelic/go-agent/tree/master/v3/integrations)を確認してください。
</Callout>