...
ディストリビューティッド（分散）ト[レーシングを使用すると、分散システム](/docs/distributed-tracing/concepts/introduction-distributed-tracing)全体にわたるrequestsのプロセス全体を確認できます。 Go エージェントの場合、2 種類のディストリビューティッド（分散）トレーシングを提供します（詳細については、 [「スパン サンプリングの仕組み」](/docs/distributed-tracing/concepts/how-new-relic-distributed-tracing-works/#sampling)を参照してください）。

* 標準 (ヘッドベースサンプリング): トレースが到着する前に、受け入れて分析するトレースの設定パーセンテージを決定します。 これにより、トレースによってどのようなメリットが得られるかを確認するための確実な出発点が得られます。Go エージェント 3.16.0 以降ではデフォルトでオンになっています。デフォルトでは、Go エージェントは 1 分あたり 10 個のトレースをサンプリングし、最大スパン制限は 1 分あたり 2000 スパンです。
