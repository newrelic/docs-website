...
[Go エージェント](/docs/agents/go-agent/get-started/introduction-new-relic-go)を使用すると、 [Google App Engine (GAE) フレキシブル環境](https://cloud.google.com/appengine/docs/flexible/go/)にあるアプリケーションを監視できます。New Relic を GAE flex アプリに追加すると、アプリの健全性とパフォーマンスに関する洞察が得られ、 [APM](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm)で表示できるメトリクスで GAE を拡張し、 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)。
