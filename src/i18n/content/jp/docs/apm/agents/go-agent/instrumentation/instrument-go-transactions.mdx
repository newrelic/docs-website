...
## Web トランザクションと非 Web トランザクションの識別 [#go-monitoring]

他の多くの言語とは異なり、Go アプリケーションはコンパイルされたネイティブのバイナリファイルから実行されます。このため、Golang アプリケーションに New Relic を設定するには、ソースコードに New Relic のメソッドを手動で追加する必要があります。

APM では、トランザクションは、 [ウェブトランザクション](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) または [非ウェブトランザクション](/docs/apm/transactions/intro-transactions/monitor-background-processes-other-non-web-transactions) として識別されます。
...
複数のゴルーチンにわたってトランザクションを監視するには、 `Transaction.NewGoroutine()` を使用します。 `NewGoroutine` メソッドは、セグメントを作成する各 goroutine が必要とする Transaction への新しい参照を返します。 `NewGoroutine` を呼び出すのが、他のゴルーチンの開始の前でも後でも問題ありません。
...
## HTTP ハンドラをラップしてトランザクションを監視 [#http-handler-txns]

標準の HTTP ライブラリパッケージを使用している場合、 [関数のコードをインストルメントする代わりに、HTTP リクエストをラップしてトランザクションを作成することができます](/docs/agents/go-agent/get-started/new-relic-go-monitor-transactions#go-txn) 。

ここでは、HTTP ハンドラをラップする前と後の例を示します。
...
ハンドラでトランザクションにアクセスするには、 `newrelic.FromContext` API を使用します。これは、Go のバージョン 1.7 以降でのみ動作することに注意してください。例えば、以下のようになります。
...
New Relic for Go では、3 つの異なる方法でエラーを把握しています。
...
また、ログとアプリケーションのデータを統合して、トラブルシューティングをより簡単かつ迅速にできます。[コンテキストのログ](/docs/logs/logs-context/configure-logs-context-go/)では、エラーとトレースに関連するログメッセージをアプリケーションの UI で直接表示できます。
