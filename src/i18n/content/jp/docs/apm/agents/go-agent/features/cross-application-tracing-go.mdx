...
<CollapserGroup>
  ...
  >
    ...
    ただし、 `Transaction.SetWebRequest`および`Transaction.SetWebResponse`の呼び出しなしでトランザクションが開始され、 <DoNotTranslate>**will not**</DoNotTranslate>では CAT が有効になっています。
  ...
  >
    ...
    構造体リテラルを使用して`ExternalSegment`を作成することもできますが、このセグメント<DoNotTranslate>**will not**</DoNotTranslate>は CAT の対象となります。 このため、New Relic では[`newrelic.NewRoundTripper`または`newrelic.ExternalSegment`](/docs/agents/go-agent/get-started/instrument-go-segments#go-external-segments)使用を推奨しています。
