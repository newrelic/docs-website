...
<CollapserGroup>
  ...
  >
    ...
    ただし、 `Transaction.SetWebRequest`および`Transaction.SetWebResponse`の呼び出しなしでトランザクションが開始され、 <DNT>**will not**</DNT>では CAT が有効になっています。
  ...
  >
    ...
    構造体リテラルを使用して`ExternalSegment`を作成することもできますが、このセグメント<DNT>**will not**</DNT>は CAT の対象となります。 このため、New Relic では[`newrelic.NewRoundTripper`または`newrelic.ExternalSegment`](/docs/agents/go-agent/get-started/instrument-go-segments#go-external-segments)使用を推奨しています。
