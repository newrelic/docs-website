...
Goエージェントは、この優先順位に従って構成されます。有効にすると、サーバー側の設定は、サーバー側の値が空白のままであっても、 `newrelic.Config`構造体の対応する**すべて**の値を上書きします。
...
<figcaption>
  サーバー側の構成がGoエージェントで有効になっている場合、サーバー側の値が空白のままであっても、 `newrelic.Config`構造体の対応する**すべて**の値が上書きされます。
</figcaption>
...
<CollapserGroup>
  <Collapser
    ...
    title="サーバー側の設定（2.7.0以上）"
  >
    [サーバーサイドの設定](/docs/agents/manage-apm-agents/configuration/server-side-agent-configuration) は、 [Goエージェントのバージョン2.7.0以降で利用可能です](/docs/release-notes/agent-release-notes/go-release-notes) 。これにより、UIで特定の設定を行うことができます。これにより、複数のホストで動作している場合でも、変更内容がすべてのエージェントに自動的に適用されます。利用可能な場合、本ドキュメントでは、個々の設定オプションの下にあるサーバーサイドコンフィグのUIラベルを **サーバーサイドラベル** として記載しています。
    ...
    <Callout variant="caution">
      サーバー側の構成が有効になっている場合、エージェントは、UI で設定 **できる** `newrelic.Config`構造体の値を無視します。 UI 値が空の場合でも、エージェントはこれを空の値として扱い、 `newrelic.Config`値を使用しません。
    </Callout>
  </Collapser>

  <Collapser
    id="in-process-config"
    title={<>処理中<InlineCode>newrelic.Config</InlineCode>構造体</>}
  >
...
<CollapserGroup>
  ...
  >
    ...
    `true`の場合、エージェントはアプリから [New Relic コレクタ](//docs/new-relic-solutions/get-started/glossary/#collector)にデータを送信します。
...
New Relic CodeStream を使用して IDE からパフォーマンスを監視している場合は [、リポジトリをサービスに関連付け](/docs/codestream/how-use-codestream/performance-monitoring/#repo-association) 、 [ビルド SHA またはリリース タグをエラーに関連付けること](/docs/codestream/how-use-codestream/performance-monitoring/#buildsha)もできます。
...
NEW_RELIC_METADATA_SERVICE_VERSION を設定すると、イベント データにタグ`tag.service.version`が作成されます。このコンテキストでは、サービス バージョンはデプロイされるコードのバージョンであり、多くの場合、1.2.3 などのセマンティック バージョンですが、常にそうとは限りません。この情報を送信すると、展開されたソフトウェアのバージョンによってテレメトリをファセットできるため、エラーを生成しているソフトウェアのバージョンをすばやく特定できます。
