...
<CollapserGroup>
  ...
  >
    ...
    [高セキュリティモード](/docs/accounts-partnerships/accounts/security/high-security) 特定のセキュリティ設定を強制し、その設定が上書きされないようにすることで、エージェントが機密データを送信しないようにします。高セキュリティモードは以下のことを行います。
