...
アプリに New Relic の Go エージェントをインストールした場合は、以下を使用することもできます。 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)ブラウザのパフォーマンスを監視します。ブラウザ エージェントをインストールするには、 [コピー アンド ペーストの方法](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent#copy-paste-app)を使用するか、Go エージェント API を使用します。
