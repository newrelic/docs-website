...
<CollapserGroup>
  ...
  >
    <DNT>**Recommendation:**</DNT> New Relic[ディストリビューティッド（分散）トレーシングを](/docs/agents/go-agent/features/distributed-tracing-go)使用してアプリケーション間のアクティビティをトレースするために `StartExternalSegment` ヘルパーを使用するため、 ヘルパーを使用します。
