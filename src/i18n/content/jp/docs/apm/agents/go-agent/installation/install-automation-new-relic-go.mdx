...
現在サポートされているのは、次の Go パッケージとライブラリのみです。
...
1. [Go インストゥルメンテーション リポジトリの](https://github.com/newrelic/go-easy-instrumentation)クローンをシステム上のディレクトリに作成します。 例えば：
   ```sh
