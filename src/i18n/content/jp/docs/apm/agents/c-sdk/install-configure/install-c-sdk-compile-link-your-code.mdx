...
<CollapserGroup>
  <Collapser id="requirements" title="1. 要件を確認します。">
    1. アプリケーションが、New Relicの[C SDKに関する互換性と要件](/docs/c-agent-compatibility-requirements)を満たしていることを確認します。
    2. <InlinePopover type="licenseKey" />があることを確認してください。
  </Collapser>

  <Collapser id="header-file" title="2. 提供されたヘッダーファイルを含めます。">
    ```c
    #include "libnewrelic.h"
    ```
  </Collapser>

  <Collapser id="logging" title="3. ロギングを設定します。">
  ...
  <Collapser id="app-name" title="4. 意味のあるアプリケーション名を設定できるよう用意しておきます。">
  ...
  <Collapser id="instrumentation" title="5. コードのインストゥルメンテーションを終了します。">
  ...
  <Collapser id="example-program" title="6. アプリケーションをコンパイルおよびリンクします。">
  ...
  <Collapser id="daemon" title="7. デーモンを開始しログをチェックします。">
