    ...
    <tr id="name">
      ...
      <td>
        ...
        * <DNT>**Instance names:**</DNT>
          ...
          デフォルトでは、 `newrelic_new_config()`によって返される構成により、 `instance_reporting`が`true`に設定されたデータストア セグメントが有効になります。

        * <DNT>**Database names:**</DNT>

          デフォルトでは、 `newrelic_new_config()`によって返される構成により、 `database_name_reporting`が`true`に設定されたデータストア セグメントが有効になります。
      </td>
    </tr>
