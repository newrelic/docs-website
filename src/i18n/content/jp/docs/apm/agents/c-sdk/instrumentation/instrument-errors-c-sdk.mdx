...
<Callout variant="tip">
  エラートレースに関数コールを含めるには、GNUの `-rdynamic`
  リンカフラグを使用して、
  [](/docs/install-c-agent-compile-link-your-code#compile)をコンパイルする際にアプリをリンクします。
  `-rdynamic`
  リンカ・フラグを使用すると、より意味のあるエラー・トレースが得られます。
</Callout>
...
![APMエラー詳細のコンテキストのログ](src/images/logs-in-context-errors.png 'APMエラー詳細のコンテキストのログ')
