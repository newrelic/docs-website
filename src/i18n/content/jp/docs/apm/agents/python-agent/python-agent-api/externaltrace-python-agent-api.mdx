...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        必須。使用中の外部ライブラリの名前（またはタイプ）。それを定義する文字列、またはラップされる関数と同じパラメーターを受け取りライブラリを返す関数のいずれかを渡します。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        必須。ライブラリ呼び出しによってリクエストされたURL。それを定義する文字列、またはラップされる関数と同じパラメーターを受け取り、URLを返す関数のいずれかを渡します。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        オプション。行われる呼び出しの種類を定義するメソッド。これらは通常、`get`、`post`、`put`または`delete`です。ラップされる関数と同じパラメーターを受け取り、メソッドを返す関数にすることもできます。
      </td>
    </tr>
  </tbody>
</table>
