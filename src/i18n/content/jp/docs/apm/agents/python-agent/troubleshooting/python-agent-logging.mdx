...
監視対象のプロセスと New Relic コレクターの間で[送信される**すべての**データ](/docs/agents/manage-apm-agents/configuration/log-audit-all-data-your-new-relic-agent-transmits)に関する情報を記録および表示する必要がある場合は、短期間の[監査ログを](/docs/apm/agents/python-agent/configuration/python-agent-configuration/#audit-log-file)有効にすることができます。これは、デバッグや監査など、正確に何が送信されているかに関する詳細情報が必要な場合に便利です。
