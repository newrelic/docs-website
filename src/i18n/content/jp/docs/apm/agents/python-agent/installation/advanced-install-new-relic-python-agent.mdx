...
`generate-config`コマンドにより、`newrelic.ini`設定ファイルが作成されます。
...
2. `newrelic-admin run-program`スクリプトコマンド
...
* New Relic UIでデータを検索し、[標準的なユーザーインタフェース機能](/docs/data-analysis/user-interface-functions/view-your-data/standard-page-functions)に慣れ親しみます。
