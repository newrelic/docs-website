...
<CollapserGroup>
  <Collapser id="python-version" title="Pythonエージェントの互換性">
...
<CollapserGroup>
  <Collapser id="python-config-standard" title="Pythonエージェントの構成">
...
<CollapserGroup>
  <Collapser id="calling-service" title="通話サービスを計測する">
    ...
    2. ディストリビューティッド（分散）トレーシングペイロードを生成するためのエージェントAPI呼び出しを呼び出します（ [PythonエージェントAPI](/docs/agents/python-agent/python-agent-api/create_distributed_trace_payload)を参照）。
