...
分散トレースを使用すると、[分散システム](/docs/distributed-tracing/concepts/introduction-distributed-tracing)全体でのリクエストの全過程を確認できます。Pythonエージェントの場合、2種類の分散トレースを提供します（詳細については、 [スパンサンプリングの仕組み](/docs/understand-dependencies/distributed-tracing/get-started/how-new-relic-distributed-tracing-works#sampling)を参照してください）。

* 標準（ヘッドベースのサンプリング）：トレースが到着する前に、受け入れて分析するトレースの設定パーセンテージを決定します。これにより、トレースがどのように役立つかを確認するための確実な出発点が得られます。Pythonエージェント7.0.0.166以降ではデフォルトでオンになっています。
...
* [新しいPythonエージェント：](#new-agents)新しいエージェントのインストールのためのヘッドベースおよびテールベースのサンプリングの手順
* [古いPythonエージェント：古いPythonエージェント](#older-agents)がある場合のトレースオプション
