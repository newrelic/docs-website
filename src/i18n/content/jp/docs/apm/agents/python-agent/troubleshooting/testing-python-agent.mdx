...
<CollapserGroup>
  ...
  >
    **ERROR - エージェントの設定でライセンスキーが設定されていません。**
    ...
    であることを確認してください。 <InlinePopover type="licenseKey"/>エージェント構成ファイルまたは`NEW_RELIC_LICENSE_KEY`環境変数で指定されています。 ライセンスキーの問題を修正するか、 [support.newrelic.com](https://support.newrelic.com)でサポートを受けてください。
  </Collapser>
  ...
  >
    **ERROR - データコレクタは、エージェントから不正なライセンスキーが提供されたことを示しています。**
    ...
    エージェントが使用する値は`...`です。の問題を修正します <InlinePopover type="licenseKey"/>、または [support.newrelic.com](https://support.newrelic.com)でサポートを受けてください。
  </Collapser>
