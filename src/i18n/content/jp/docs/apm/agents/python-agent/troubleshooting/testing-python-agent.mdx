...
<CollapserGroup>
  ...
  >
    **ERROR - エージェントの設定でライセンスキーが設定されていません。**
    ...
    <InlinePopover type="licenseKey" />がエージェント構成ファイルまたは`NEW_RELIC_LICENSE_KEY`環境変数で指定されていることを確認してください。ライセンスキーの問題を修正するか、 [support.newrelic.com](https://support.newrelic.com)でサポートを受けてください。
  </Collapser>
