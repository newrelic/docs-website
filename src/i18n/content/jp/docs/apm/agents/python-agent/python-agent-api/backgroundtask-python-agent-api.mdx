...
* <DNT>**The context manager**</DNT>: 監視対象のタスクが実行時のみに決定される (インポート時ではない) 場合は、 `BackgroundTask`コンテキスト マネージャーを使用して、コード ブロックの実行を直接ラップできます。 (これは、 `BackgroundTaskWrapper`によって使用されるコンテキスト マネージャーです。)
* <DNT>**The wrapper**</DNT>: 追跡したい特定の関数がどこにあるかが事前にわかっている場合は、 `background_task`デコレータを使用できます。 ただし、トレースする必要があるすべての関数がわからない場合 (たとえば、関数がルーティング システムの一部として動的に検索されている場合)、その時点で`BackgroundTaskWrapper`を使用して関数をラップする必要があります。登録時または電話時。
* <DNT>**The path-based wrapper**</DNT>: パスベースのラッパー フォームは`wrap_background_task`です。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        必須。トランザクションの名前。callable\_name パラメーターを受け入れる関数である可能性があります。デフォルト値は`None`です。デフォルトでは、名前はデコレータが適用された関数の名前になりますが、名前を指定することでオーバーライドできます。
      </td>
    </tr>
