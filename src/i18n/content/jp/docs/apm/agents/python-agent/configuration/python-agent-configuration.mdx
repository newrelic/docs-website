...
## ハイブリッドエージェントの設定 [#otel-settings]

<Callout variant="important">
  [Python エージェント バージョン 11.5.0 以上が](/docs/agents/python-agent/installation-configuration/upgrade-python-agent)必要です。
</Callout>

ハイブリッド エージェントの構成には次の設定を使用できます。
...
<CollapserGroup>
  <Collapser id="opentelemetry.enabled" title="OpenTelemetryが有効">
    ...
    有効にすると、インストールされているOpenTelemetry APIおよび[OpenTelemetryメントされたフレームワーク](https://github.com/open-telemetry/opentelemetry-python-contrib/tree/main/instrumentation#readme)が使用されます。
  </Collapser>
  ...
  <Collapser id="opentelemetry.traces.enabled" title="OpenTelemetry.Trace.enabled">
    ...
    有効にすると、 OpenTelemetryスパンおよびトレース API が使用されます。
  </Collapser>
</CollapserGroup>
