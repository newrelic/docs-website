...
唯一必要な Python エージェント構成設定は、 <InlinePopover type="licenseKey"/>. ライセンス キーは、エージェントがアプリケーション データをレポートするアカウントを識別します。アプリケーションのホスト方法に応じて、構成ファイルまたは環境変数を介してライセンス キーを提供できます。
...
<CollapserGroup>
  ...
  >
    ...
    を指定します <InlinePopover type="licenseKey"/>New Relic アカウントの。 このキーは、アプリのメトリクスを New Relic アカウントに関連付けます。
  </Collapser>
