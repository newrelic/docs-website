...
必要な Python エージェント構成設定は、<InlinePopover type="licenseKey" />のみです。ライセンス キーは、エージェントがアプリケーション データをレポートするアカウントを識別します。アプリケーションのホスト方法に応じて、構成ファイルまたは環境変数を介してライセンス キーを提供できます。
