...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/) > APM & サービス > (アプリを選択) > 監視 > トランザクション > (トランザクションを選択):**トランザクションの**内訳テーブル**は、RabbitMQ の`MessageBroker`メトリックとしてキュー操作を分類し、それらを`Put`としてラベル付けします (メッセージ) または`Take` (メッセージを受信)。
</figcaption>
