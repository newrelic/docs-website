...
サンプル エージェント構成ファイルを生成します。`license_key`オプションは [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)です。
...
* `NEW_RELIC_LICENSE_KEY` - New Relic の [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)。
