...
引数なしで`initialize`を呼び出す場合は、 [`NEW_RELIC_LICENSE_KEY`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#license_key)環境変数で<LicenseKey/>を既に指定している必要があります。または、 `NEW_RELIC_CONFIG_FILE`と`NEW_RELIC_ENVIRONMENT`を設定できます。エージェントは、不足している引数の代わりにこれらの値を読み取ります。
