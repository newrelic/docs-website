...
引数なしで`initialize`呼び出す場合は、 [`NEW_RELIC_LICENSE_KEY`](/docs/agents/python-agent/installation-configuration/python-agent-configuration#license_key)環境変数を使用して<InlinePopover type="licenseKey" />すでに指定している必要があります。 あるいは、 `NEW_RELIC_CONFIG_FILE`と`NEW_RELIC_ENVIRONMENT`設定することもできます。 エージェントは、不足している引数の代わりにそれらの値を読み取ります。
