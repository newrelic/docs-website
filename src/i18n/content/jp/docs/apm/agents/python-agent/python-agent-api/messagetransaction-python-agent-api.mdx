...
* <DNT>
  ...
  : コンテキスト マネージャー フォームは`MessageTransaction`です。 デコレータと同じものが必要です。

* <DNT>
    **The wrapper**
  </DNT>

  : ラッパーフォームは`MessageTransactionWrapper`です。 デコレータを使用せずにラップされた関数を返すために使用できます。

* <DNT>
    **The path-based wrapper**
  </DNT>

  : パスベースのラッパー形式は`wrap_message_transaction`です。 これは、モンキーパッチングを通じて、指定されたオブジェクトに`MessageTransactionWrapper`を適用します。 これは、デコレータと同じ引数に加えて、追加の`module`と`object_path`引数を受け取ります。
