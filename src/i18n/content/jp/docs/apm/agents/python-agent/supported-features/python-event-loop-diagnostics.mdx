...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & サービス > (アプリを選択) トランザクション > (See transaction table をクリック****) > (トランザクションを選択):**トランザクションの**ブレイクダウン テーブル**は、asyncio でイベント ループにフラグを立てます。
</figcaption>
