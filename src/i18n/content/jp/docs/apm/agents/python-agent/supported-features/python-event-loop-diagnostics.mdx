...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (アプリを選択) > Monitoring > Transactions > (トランザクションを選択):**トランザクションの**ブレイクダウン テーブル**は、非同期でイベント ループにフラグを立てます。
</figcaption>
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (アプリを選択) > トランザクション > (トランザクション追跡を選択) > 追跡の詳細: 追跡**の**詳細**ページには、トランザクション追跡の各セグメント (メソッドまたは関数呼び出し) の内訳を示す表が含まれています。
</figcaption>
