...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (アプリを選択) > Monitoring > Transactions > (トランザクションを選択):**トランザクションの**内訳テーブル**は、asyncio でイベント ループにフラグを立てます。
</figcaption>
