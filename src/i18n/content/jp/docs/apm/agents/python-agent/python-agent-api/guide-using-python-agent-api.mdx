...
## セグメントとセグメント属性に動的に名前を付ける [#dynamically-name-segments-and-segment-attributes]

セグメントおよびセグメント属性値に動的に名前を付けるには、さまざまなパラメーターに対して静的な値ではなく関数を受け入れるカスタムインストゥルメンテーションAPIコールを使用します。エージェントバージョン13.0.0より前では、設定ファイルでLambda関数を使用してこれを行うこともできます。これは現在、カスタムインストゥルメンテーションAPIコールでのみサポートされています。

以下のAPIコールは動的ネーミングをサポートしています：
