...
[ブラウザ監視エージェントをページに自動的に追加するか、コピーして貼り付けて特定のページに展開することで、ブラウザ監視エージェントをインストール](/docs/browser/new-relic-browser/installation-configuration/add-apps-new-relic-browser)できます。 <InlinePopover type="browser"/>エージェントの JavaScript スニペット。APM エージェント API 呼び出しを使用してブラウザ エージェントを制御することもできます。詳細については、 [「ブラウザ エージェントと Python エージェント」](/docs/agents/python-agent/supported-features/page-load-timing-python)を参照してください。
