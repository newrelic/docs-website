...
Python エージェントは自動的に追加できます<InlinePopover type="browser"/>あらゆる HTML ページの応答に。Django、Flask、Pyramid などの最も一般的な Python Web フレームワークでは、[自動ブラウザ インストルメンテーション](/docs/agents/python-agent/supported-features/page-load-timing-python)を使用します。
