...
運用メトリックは、次の形式を使用して報告されます。 `GraphQL/operation/GraphQL/{operation_type}/{operation_name}/{deepest_unique_path}`
...
フィールドリゾルバーメトリックは、次の形式を使用して報告されます。 `GraphQL/resolve/GraphQL/{field_name}`
