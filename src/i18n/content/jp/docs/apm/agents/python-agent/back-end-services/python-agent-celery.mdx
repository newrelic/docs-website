...
<CollapserGroup>
  <Collapser id="run-celery-celery" title="セロリ">
  ...
  <Collapser id="run-celery-celeryd" title="Celeryd">
  ...
  <Collapser id="run-celery-initd" title="Initスクリプト">
...
<CollapserGroup>
  <Collapser id="multiple-app-names" title="複数のアプリケーション名の使用">
...
これらのエラーを無視するには、New Relic <DNT>**Application settings**</DNT> UI またはエージェント構成ファイルからこれを設定します。 UI変更は、 [設定の優先ルール](/docs/agents/python-agent/installation-configuration/python-agent-configuration#options)に従って構成ファイルの変更を上書きします。

エラーを無視する設定をUIから使用するため。

1. <DNT>**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**</DNT>から<DNT>**APM &gt; (select an app) &gt; Settings &gt; Application**</DNT>を選択します。
2. <DNT>**Server-side agent configuration**</DNT>を選択します。
3. <DNT>**Error collection**</DNT>から、無視するエラーをカンマで区切って入力します。
