...
<CollapserGroup>
  ...
  >
    ...
    2. 分散トレーシング ペイロードを生成するためのエージェントAPI呼び出しを呼び出します ( [RubyエージェントAPI](/docs/agents/ruby-agent/api-guides/guide-using-ruby-agent-api#distributed)を参照)。

       <Callout variant="important">トレース内のスパンの適切な順序を維持するには、 <DNT>**generate the payload in the context of the span that sends it**</DNT>必ず実行してください。</Callout>
