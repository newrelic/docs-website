...
<CollapserGroup>
  <Collapser id="upgrade" title="newrelic_rpm 3.8.0以上にアップグレードしてください。">
    前述の通り、newrelic\_rpm gemのバージョン3.8.0以降であれば、unicornの設定に関わらず自動的に正しく動作するはずです。
  </Collapser>

  <Collapser
    id="upload"
    title={<>アップロード <InlineCode>
      preload_app true
    </InlineCode></>
    }
  >
    ...
    <Callout variant="tip">
      ディレクティブは`preload_app = true`ではなく`preload_app true`です。 等号を含むフォームは、何も表示されずに失敗します。
    </Callout>
  </Collapser>

  <Collapser id="manual-start" title="エージェントを手動で起動する">
