...
本ドキュメントでは、これらのいずれかの方法で GAE flex アプリに New Relic を追加する方法を説明します。

- Google App Engine の [" ネイティブモード"](/docs/accounts-partnerships/partnerships/google-cloud-platform-gcp/google-app-engine-environment#native-mode) 標準的な GAE ランタイムでのインストール
- [カスタムランタイムを使用した Docker インストール](https://cloud.google.com/appengine/docs/flexible/custom-runtimes/about-custom-runtimes)

カスタムランタイムメソッドには、Sinatra アプリをデプロイする例が含まれています。特定のライブラリやヘッダーが必要な場合は、New Relic ではカスタムランタイムメソッドの使用を推奨しています。

## GAE のネイティブサポートによるデプロイ [#native]

Google App Engine [" ネイティブモード"](/docs/accounts-partnerships/partnerships/google-cloud-platform-gcp/google-app-engine-environment#native-mode) インストールを使用する際には、アプリのコードと `app.yaml` ファイルを提供します。Google App Engine は、あらかじめ構築された標準的な Docker イメージにデプロイします。

Sinatra や Rails をネイティブにサポートしてデプロイすること。
...
2. Ruby エージェントのインストール [設定ファイル](/docs/agents/ruby-agent/installation/install-new-relic-ruby-agent#Configuration_file).
...
## Docker でカスタムランタイムを構築 [#build-runtime]

<Callout variant="tip">
  Rubyアプリが特定のライブラリやヘッダーを必要とする場合、New
  Relicではカスタムランタイム方式の使用を推奨します。また、New Relic
  では、Google App Engine
  によるヘルスチェックの処理を許可することを推奨しています。
</Callout>
...
GAE フレキシブル環境での Ruby アプリの展開と設定については、以下をご覧ください。

- [Google App Engine のドキュメント](https://cloud.google.com/appengine/docs/flexible/ruby/) for Ruby
- [Google App Engine のチュートリアル](https://cloud.google.com/appengine/docs/flexible/ruby/tutorials) for Ruby
...
[カスタムランタイムを作成する場合は、アプリが大量のヘルスチェックリクエストを処理できなければなりません。そうしないと、アプリのデータが APM で正しく表示されない可能性があります。](https://cloud.google.com/appengine/docs/flexible/ruby/how-instances-are-managed)
...
## [GAE から New Relic エージェントのトラブルシューティングログを取得する \[#agent-logs\]](https://cloud.google.com/appengine/docs/flexible/ruby/how-instances-are-managed#health_checking)

[GAE flex 環境アプリのトラブルシューティングには、これらのリソースをご利用ください。](https://cloud.google.com/appengine/docs/flexible/ruby/how-instances-are-managed#health_checking)
...
- ログを表示するには、 [Cloud Platform Console の Log Viewer](https://cloud.google.com/appengine/docs/flexible/php/writing-application-logs) を使用します。
