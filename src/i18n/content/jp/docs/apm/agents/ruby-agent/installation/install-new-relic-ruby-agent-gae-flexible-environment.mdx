...
1. New Relic の標準手順に従って、 <InlinePopover type="licenseKey" />を含む[gem をインストールします](/docs/agents/ruby-agent/installation/install-new-relic-ruby-agent#Installing_the_Gem)。
...
<CollapserGroup>
  ...
  <Collapser id="configure-app-yaml" title="2.app.yamlの設定">
  ...
  <Collapser id="configure-dockerfile" title="3.Dockerfileの設定">
...
[* ヘルスチェックを<DNT>**handle**</DNT>するには、アプリに`_ah/health`のルートを追加します。* ヘルスチェック要求を<DNT>**ignore**</DNT>するには、アプリケーションの Ruby エージェント構成の`rules.ignore_url_regexes`構成設定に`'_ah/health'`を含めます。](https://cloud.google.com/appengine/docs/flexible/ruby/how-instances-are-managed#health_checking)
