...
* **prepend:** Ruby メソッドのプリペンドの使用を強制します
* **chain:** Ruby メソッド チェーンの使用を強制します。
...
収集された gRPC データは、New Relic UI のいくつかのページで確認できます。たとえば、 [\[概要\] ページ](/docs/apm/applications-menu/monitoring/apm-overview-page)では、gRPC クライアント リクエストが Web トランザクションとして表示され、 **Web トランザクション時間**、**スループット**、および**エラー率**のグラフにデータが提供されます。

gRPC データは、[分散トレース ページ](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent/)からも入手できます。
...
6. リストからトレースを選択し、クリックします。

7. \[**すべて展開]**をクリックします。

8. 得られたトレース スパンのリストから、個々のトレース スパンをクリックします。

9. 右側のペインで \[**属性**] タブをクリックします。
