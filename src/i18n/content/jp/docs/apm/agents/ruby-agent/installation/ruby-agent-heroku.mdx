...
[Heroku](https://devcenter.heroku.com/articles/newrelic)は、 Rubyを含むさまざまなエージェント言語で Web アプリケーションをホストするための Platform as a Service (PaaS) ソリューションです。 New Relic使用すると、Heroku [<InlinePopover type="apm"/>](/docs/apm/new-relic-apm)と[<InlinePopover type="browser"/>](/docs/browser/new-relic-browser) メトリックを使用して を拡張できます。このページでは、New Relic の Ruby エージェントを使用して Heroku をホスティング サービスとして使用する場合の特別な考慮事項について説明します。
...
<CollapserGroup>
  ...
  >
    Rails 以外のアプリケーションで Ruby エージェント for New Relic を使用している場合、Heroku ユーザーはプラグインをリポジトリに手動でインストールする必要があります。たとえば、Sinatra アプリでは、 `newrelic_rpm` gem を`Gemfile`に追加してから、次のコードをアプリに追加します。
...
`newrelic.yml`ファイルで New Relic を構成するか、 [環境変数を](/docs/agents/ruby-agent/installation-configuration/ruby-agent-configuration) 使用して構成ファイルの値よりも優先することができます。 `heroku config:set`を使用して、Heroku アプリケーションのエージェントの設定を変更します。

たとえば、 [`custom_insights_events.max_samples_stored`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#custom_insights_events-max_samples_stored) 設定を 500 に設定するには:
...
動的に作成される他の dyno タイプがある場合は、 [`heroku.dyno_name_prefixes_to_shorten`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#heroku) 構成設定を使用して、これらの他の dyno タイプに同じ集計を適用します。
...
4. [`log_level`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#log_level) を`debug`に変更し、ファイルを保存します。 インデントを変更しないでください。
...
9. `newrelic.yml`を再度編集し、 [`log_level`](/docs/apm/agents/ruby-agent/configuration/ruby-agent-configuration/#log_level) を以前の設定に変更します。 ファイルを保存します。
