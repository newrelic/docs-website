...
[New Relic CodeStream](/docs/codestream/start-here/what-is-codestream)をRubyエージェントと統合することにより、コンテキスト依存のAPMデータをIDEに直接表示します。この統合により、コードを記述およびレビューするときに、エディターでコードレベルの本番テレメトリを視覚化します。
...
<Callout variant="important">
  New Relic CodeStream統合は、Rubyエージェントバージョン8.8.0以降で利用可能であり、デフォルトでは無効になっています。統合を有効にするには、 `newrelic.yml`または`NEW_RELIC_APPLICATION_CODE_LEVEL_METRICS_ENABLED=true`の`code_level_metrics.enabled = true`を環境変数として設定します。
</Callout>
