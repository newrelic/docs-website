...
監査ログはデフォルトでは無効になっており、手動で有効にする必要があります。有効にするには、 **newrelic.yml**ファイルに次のコードを追加します: `audit_log.enabled: true` 。

デフォルトでは、New Relic Ruby エージェントは**、 log/newrelic_audit.log**にあるメイン New Relic エージェント ログの隣のファイルに監査ログを書き込みます。監査ログ ファイルの場所を制御するには、 `audit_log.path`構成パラメータを監査ログを保存する場所のフルパスに設定します。
