...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    ...
    `true`の場合、[`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments)に達すると、エージェントは、そのトランザクションでその後作成されたセグメントの排他的時間の記録も停止します。これにより、多くのセグメントを持つ非常に長時間実行されるトランザクションのメモリ使用量を削減できますが、セグメント制限に達した場合、トランザクション内のセグメントのタイミングデータの精度が低下するという代償を伴います。
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    エラーバックトレースにおけるフレームの最大数を定義します。デフォルトでは、設定値を超えるバックトレースは途中で切り捨てられ、スタックトレースの開始と終了は保持されます。切り捨て位置をカスタマイズするには、`error_collector.backtrace_truncate_location`を参照してください。
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="disable_puma_instrumentation" title="disable_puma_instrumentation">
    ...
    `true`の場合、Pumaマスタープロセスから`Ruby/Puma/*`メトリクスをサンプリングするPumaサーバー統計の計装を無効にします。この計装はデフォルトで無効になっています。有効にすると、エージェントはPumaマスタープロセスでレポートスレッドを開始し、これらのメトリクスを配信します。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    ...
    `true`の場合、Google Cloud Runで実行しているとき、エージェントはGCPインスタンスIDをhostnameとして報告します。
  </Collapser>
  ...
  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    ...
    `true`の場合、エージェントはGCPインスタンスIDの前に`K_REVISION`の値を付加し、Google Cloud Run上のhostname（`{K_REVISION}-{instance id}`）を形成します。`utilization.gcp_cloud_run.use_instance_as_host`も`true`でない限り、効果はありません。
  </Collapser>
