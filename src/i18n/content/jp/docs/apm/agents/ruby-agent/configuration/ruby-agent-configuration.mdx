...
<CollapserGroup>
  ...
  >
    ...
    シャットダウンする前にキャッシュされたすべてのデータをコレクターに送信する終了ハンドラーを強制的にインストールする必要があります。このオプションの既知の使用例は、Sinatra が別のフレームワーク内の埋め込みサービスとして実行され、エージェントが Sinatra アプリを検出し、結果として`at_exit`ハンドラーをスキップする場合です。Sinatra は従来、アプリケーション全体を`at_exit`ブロックで実行しますが、エージェントの`at_exit`ハンドラーもそのような状況でインストールされている場合、正しく動作しません。注: この設定と合わせて、 `send_data_on_exit`も`true`に設定する必要があります。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    起動時にelasticsearchライブラリの自動計測を制御します。`auto` 、 `prepend` 、 `chain` 、または`disabled`のいずれかです。
  </Collapser>
