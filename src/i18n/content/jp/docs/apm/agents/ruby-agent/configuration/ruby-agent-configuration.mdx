...
<CollapserGroup>
  ...
  >
    ...
    起動時にThreadクラスの自動インストルメンテーションを制御して、エージェントが非同期トランザクション内にスパンを正しくネストできるようにします。これにより、エージェントは作成されたすべてのスレッドを自動的にトレースできなくなります（ `instrumentation.thread.tracing`を参照）。 \[auto | prefix | chain|disabled]のいずれかである可能性があります。
  </Collapser>
