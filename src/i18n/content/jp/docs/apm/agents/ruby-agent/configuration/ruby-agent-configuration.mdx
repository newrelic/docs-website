...
<CollapserGroup>
  ...
  >
    ...
    あなたの新しいレリック <InlinePopover type="licenseKey"/>.
  </Collapser>
  ...
  >
    ...
    あなたの新しいレリック <InlinePopover type="userKey"/>.New Relic REST API v2 を使用して、 `newrelic deployments` コマンドを使用してデプロイメントを記録する場合に必要です。
  </Collapser>
  ...
  >
    ...
    `true`の場合、[高セキュリティ モード](/docs/accounts-partnerships/accounts/security/high-security)が有効になります。この設定を有効にする前に、高セキュリティ モードの影響を必ず理解してください。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    * [高セキュリティ モード](/docs/agents/manage-apm-agents/configuration/high-security-mode)を有効にすると、これは自動的に`obfuscated`に設定されます。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    **DEPRECATED**: [distribution_tracing.enabled](#distributed_tracing-enabled)を参照してください。
...
<CollapserGroup>
  ...
  >
    ...
    true の場合、エージェントは、 [allowlist](#strip_exception_messages-allowlist)内の例外を除くすべての例外からメッセージを削除します。[高セキュリティ モード](/docs/accounts-partnerships/accounts/security/high-security)では自動的に有効になります。
  </Collapser>
