...
<CollapserGroup>
  ...
  >
    ...
    起動時にelasticsearchライブラリの自動計測を制御します。\[auto|prepend|chain|disabled] のいずれかになります。
  </Collapser>
  ...
  >
    ...
    起動時の gRPC クライアントの自動計測を制御します。\[auto|prepend|chain|disabled] のいずれかになります。
  </Collapser>
  ...
  >
    ...
    起動時の gRPC サーバーの自動計測を制御します。\[auto|prepend|chain|disabled] のいずれかになります。
  </Collapser>
  ...
  >
    ...
    Puma::Rackの自動計測を制御します。有効にすると、エージェントはPuma :: Rack :: Builderの`to_app`メソッドにフックして、アプリケーションの起動時にインストルメントするgemを検索します。 \[auto | prefix | chain|disabled]のいずれかである可能性があります。
  </Collapser>
  ...
  >
    ...
    ラックの自動計装を制御します。有効にすると、エージェントはRack :: Builderの`to_app`メソッドにフックして、アプリケーションの起動時にインストルメントするgemを検索します。 \[auto | prefix | chain|disabled]のいずれかである可能性があります。
  </Collapser>
  ...
  >
    ...
    起動時の Thread クラスの自動計測を制御して、エージェントが非同期トランザクション内でスパンを正しくネストできるようにします。これにより、作成されたすべてのスレッドをエージェントが自動的にトレースできるようになりません ( `instrumentation.thread.tracing`を参照)。\[auto|prepend|chain|disabled] のいずれかになります。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <strong>
      この構成オプションは現在到達できません。使用しないでください。
    </strong>

    `true`の場合、エージェントが Ruby on Rails を使用するアプリケーションにある場合、config/initializers が実行された後に開始されます。
  </Collapser>
</CollapserGroup>
