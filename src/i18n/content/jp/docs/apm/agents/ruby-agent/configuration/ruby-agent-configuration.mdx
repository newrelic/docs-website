...
<CollapserGroup>
  ...
  <Collapser id="instrumentation-rails_event_logger-event_names" title="instrumentation.rails_event_logger.event_names">
    ...
    * user.signup
    * payment.processed
    * order.created Leave empty to capture all Rails.event notifications. Events are logged with level UNKNOWN (ensuring they&apos;re never filtered) unless overridden via :level key in the event payload.
  </Collapser>
