...
<CollapserGroup>
  ...
  <Collapser id="error_collector-backtrace_truncate_location" title="error_collector.backtrace_truncate_location">
    ...
    フレーム数が`error_collector.max_backtrace_frames`を超えた場合にバックトレース内のどこを切り捨てるかを指定します。オプションは、 `top` (先頭からフレームを削除)、 `middle` (先頭と末尾を保持したまま中間からフレームを削除)、または`end` (末尾からフレームを削除) です。
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="instrumentation-logging" title="計装.ロギング">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    起動時のロギングライブラリの自動インストゥルメンテーションを制御します。 `auto` 、 `prepend` 、 `chain` 、 `disabled`のいずれかになります。
  </Collapser>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="OpenTelemetryが有効">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    New Relicを介して送信されるすべてのOpenTelemetry信号を無効にするグローバル設定オプション。 false の場合、OpenTelemetry 信号は New Relic に送信されません。trueの場合、シグナル固有の有効な設定オプション（例：OpenTelemetry.トレース.有効) その信号タイプのテレメトリがNew Relicに報告されるかどうかを決定します。
  </Collapser>

  <Collapser id="opentelemetry-traces-enabled" title="OpenTelemetry.Trace.enabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetryスパンからの瞬間トレース セグメントとタイムスライス メトリクスの作成を有効にします。 これは、OpenTelemetry スパンの New Relic UI エクスペリエンスの向上に役立ちます。
  </Collapser>

  <Collapser id="opentelemetry-traces-include" title="OpenTelemetry.トレース.include">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetryのカンマ区切りのリスト。文字列として表されます(例: &quot;AppTracer1, OpenTelemetry ::計装::Bunny::計装&quot;)、トレースがNew Relicに送信され**ます**。
  </Collapser>

  <Collapser id="opentelemetry-traces-exclude" title="OpenTelemetry.トレース.exclude">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    OpenTelemetryのカンマ区切りのリスト。文字列として表されます(例: &quot;AppTracer1,OpenTelemetry::Instrumentation::Bunny::Instrumentation&quot;)、トレースはNew Relicに送信され**ません**。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-detect_in_parallel" title="利用率.並列検出">
    <table>
      <tbody>
        <tr>
          <th>
