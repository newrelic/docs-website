...
<CollapserGroup>
  ...
  >
    ...
    トレース オブザーバーに送信されるデータの圧縮レベルを構成します。\[none|low|medium|high] のいずれかを指定できます。デフォルトは「high」です。圧縮を無効にするには、レベルを「なし」に設定します
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    起動時の並行 Ruby ライブラリの自動計測を制御します。\[auto|prepend|chain|disabled] のいずれかになります。
  </Collapser>
