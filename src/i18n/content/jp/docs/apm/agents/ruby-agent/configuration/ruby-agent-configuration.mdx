...
<CollapserGroup>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_ACTIVE_RECORD_USE_TABLE_NAME`
          </td>
        </tr>
      </tbody>
    </table>

    `true`に設定すると、エージェントはActive Recordのメトリクス、スパン、およびトランザクショントレースセグメントを命名する際、クラス名ではなくActive Recordモデルのテーブル名を使用します。複数のモデルがデータベーステーブルを共有している場合、この設定によりカーディナリティを削減できます。デフォルトは`false`です。
  </Collapser>
  ...
  <Collapser id="simplify_action_cable_broadcast_metrics" title="simplify_action_cable_broadcast_metrics">
    ...
    `true`の場合、Action Cableブロードキャスト呼び出しのメトリクスには、指定されたブロードキャストの`broadcasting`の値は含まれません。デフォルトは`false`です。デフォルトは、今後のメジャーバージョンリリースで変更される可能性があります。
  </Collapser>
  ...
  <Collapser id="ignored_middleware_classes" title="ignored_middleware_classes">
    ...
    エージェントが無視すべきミドルウェアクラスまたはモジュール名のリスト。例：\[&quot;Rack::Cors&quot;, &quot;ActionDispatch::Reloader&quot;]
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="transaction_tracer-cap_segment_artifacts" title="transaction_tracer.cap_segment_artifacts">
    <table>
      <tbody>
        <tr>
          <th>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_TRANSACTION_TRACER_CAP_SEGMENT_ARTIFACTS`
          </td>
        </tr>
      </tbody>
    </table>

    `true`に設定すると、[`transaction_tracer.limit_segments`](#transaction_tracer-limit_segments)に達した後は、エージェントはそのトランザクションで以降に作成されるセグメントの排他時間の記録も停止します。これにより、多数のセグメントを含む非常に長時間実行されるトランザクションのメモリ使用量を削減できます。一方、セグメント制限に達した場合は、トランザクション内のセグメントのタイミングデータの精度が低下します。
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="error_collector-max_backtrace_frames" title="error_collector.max_backtrace_frames">
    ...
    エラーバックトレースにおけるフレームの最大数を定義します。デフォルトでは、設定値を超えるバックトレースは中間部分が切り捨てられ、スタックトレースの先頭と末尾が保持されます。切り捨て位置をカスタマイズするには、`error_collector.backtrace_truncate_location`を参照してください。
  </Collapser>
...
<CollapserGroup>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_DISABLE_ACTIVE_JOB_STEP_NAMES`
          </td>
        </tr>
      </tbody>
    </table>

    `true`に設定すると、Rails Active Job Continuationsのステップ名をメトリクス名に含めません。この設定により、メトリクスのカーディナリティは削減されますが、個々のステップのパフォーマンスを把握しづらくなります。ステップ数が多いジョブや動的に名前が付けられたステップが原因でメトリクスのカーディナリティの問題が発生している場合にのみ、`true`に設定してください。
  </Collapser>
  ...
  <Collapser id="disable_puma_instrumentation" title="disable_puma_instrumentation">
    <table>
      <tbody>
        <tr>
          <th>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_DISABLE_PUMA_INSTRUMENTATION`
          </td>
        </tr>
      </tbody>
    </table>

    `true`に設定すると、Pumaマスタープロセスから`Ruby/Puma/*`メトリクスをサンプリングするPumaサーバー統計の計装が無効になります。デフォルトでは、この計装は無効になっています。有効にした場合、エージェントはPumaマスタープロセスでレポートスレッドを開始し、これらのメトリクスを送信します。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="instrumentation-semantic_logger" title="instrumentation.semantic_logger">
    ...
    起動時のsemantic\_loggerライブラリの自動インストゥルメンテーションを制御します。設定可能な値は次のとおりです：`auto`、`prepend`、`chain`、`disabled`。
  </Collapser>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_INSTRUMENTATION_RAILS_EVENT_LOGGER`
          </td>
        </tr>
      </tbody>
    </table>

    Rails.eventを構造化ログとして計装するかどうかを制御します。設定可能な値は次のとおりです：`enabled`、`disabled`。
  </Collapser>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_INSTRUMENTATION_RAILS_EVENT_LOGGER_EVENT_NAMES`
          </td>
        </tr>
      </tbody>
    </table>

    構造化ログイベントとして取得するRails.event名の配列。例：

    * user.signup
    * payment.processed
    * order.created すべてのRails.event通知を取得するには、空のままにします。イベントペイロードの:levelキーで上書きされない限り、イベントはUNKNOWNレベルでログに記録されます（これにより、フィルターで除外されることはありません）。
  </Collapser>
...
<CollapserGroup>
  <Collapser id="puma-sample_rate" title="puma.sample_rate">
    <table>
      <tbody>
        <tr>
          <th>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_PUMA_SAMPLE_RATE`
          </td>
        </tr>
      </tbody>
    </table>

    Pumaマスタープロセスで取得されるPumaサーバー統計のサンプル間の秒数。
  </Collapser>
</CollapserGroup>
    ...
    <table>
      <tbody>
        ...
        <tr>
          ...
          <td>
            `NEW_RELIC_SIDEKIQ_SEPARATE_TRANSACTIONS`
          </td>
        </tr>
      </tbody>
    </table>
...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp_cloud_run-use_instance_as_host" title="utilization.gcp_cloud_run.use_instance_as_host">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    `true`に設定すると、Google Cloud Run上で実行されている場合、エージェントはGCPインスタンスIDをhostnameとして報告します。
  </Collapser>

  <Collapser id="utilization-gcp_cloud_run-include_revision_in_host" title="utilization.gcp_cloud_run.include_revision_in_host">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    `true`に設定すると、エージェントはGCPインスタンスIDの前に`K_REVISION`の値を付加し、Google Cloud Run上のhostname（`{K_REVISION}-{instance id}`）を生成します。ただし、`utilization.gcp_cloud_run.use_instance_as_host`も`true`に設定しない限り、効果はありません。
  </Collapser>
