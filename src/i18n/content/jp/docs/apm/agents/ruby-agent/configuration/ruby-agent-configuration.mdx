...
<CollapserGroup>
  ...
  <Collapser id="disable_active_job_step_names" title="disable_active_job_step_names">
    ...
    `true`の場合、Rails Active Job Continuationsのステップ名はメトリクス名に含まれません。これにより、メトリクスのカーディナリティは削減されますが、個々のステップのパフォーマンスの可視性も低下します。ステップ数が多いジョブや動的に名前が付けられたステップが原因でメトリクスのカーディナリティの問題が発生している場合にのみ、これを`true`に設定してください。
  </Collapser>
