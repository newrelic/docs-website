...
<Callout variant="tip">
  New Relicは、<DNT>Ruby</DNT>エージェントへのコミュニティからの貢献を受け付けています。インストゥルメントされた状態にしたいgemや提案したい機能がある場合は、GitHubの`newrelic/newrelic-ruby-agent`リポジトリで[issue](https://github.com/newrelic/newrelic-ruby-agent/issues/new/choose)を開くか、[プルリクエスト](https://github.com/newrelic/newrelic-ruby-agent)を送信してアイデアを共有してください。
</Callout>

## サードパーティ製エクステンションの検索

誰でも<DNT>Ruby</DNT>エージェント上に構築するgemを作成できます。これらの拡張機能を追跡し、<DNT>Ruby</DNT>エージェントを構築する他のgemへのリンクを提供するための古い例は、[extends\_newrelic\_rpm](https://github.com/newrelic/extends_newrelic_rpm)と呼ばれるアーカイブされたリポジトリで利用できます。

これらの拡張機能は、New Relicではサポートされて**いません**。New Relicは、顧客へのサービスとしてこれらのリンクを収集しました。これらのgemに関する問題は、GitHubの各プロジェクトに報告してください。

## 珠玉のエクステンション

サードパーティの拡張機能を作成する場合は、インストゥルメントされたライブラリごとに1つのgemを作成することをお勧めします。たとえば、`newrelic-redis`は`redis`gem（当社のエージェント内にも自動的に存在します）の計装を提供します。
