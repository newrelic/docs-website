...
## ネーミング・メトリクス [#metric\_names][#metric_names]
...
<CollapserGroup>
  <Collapser id="metric_names" title="メトリック名とパス">
  ...
  <Collapser id="metrics_and_stats" title="Rubyエージェントでのメトリクスとスタッツ">
    メトリックデータの収集に使用される2つの基本的なデータ構造があります： `MetricSpec`と`Stats` 。

    `MetricSpec`データがある識別子です。 次の疑似<DNT>Ruby</DNT>はメトリクスを定義します。
  ...
  <Collapser id="stat_policy" title="スタットアグリゲーションポリシー">
    ...
    * `min_call_time`：各min\_call\_timeのMin（）
    * `max_call_time`：各max\_call\_timeのMax（）
    * `sum_of_squares`： 添加
    * `begin_time`：各begin\_timeのMin（）
    * `end_time`：各end\_timeのMax（）
  </Collapser>
</CollapserGroup>
...
### increment\_metric(メトリック名, amount=1) [#increment\_metric][#increment_metric]
