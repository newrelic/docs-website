...
これらの URI には有用なデバッグ情報が含まれている可能性があるため、New Relic はこれらの URI を報告することを推奨しています。報告したくない機密データが URI に含まれている場合は、URI レポートを無効にすることができます。たとえば、機密データには電子メール アドレスやアカウント ID が含まれる場合があります。URI レポートを無効にするには、上記の属性名のいずれかを`attributes.exclude`リストに追加します。
