...
<CollapserGroup>
  ...
  >
    ...
    `true`の場合、エージェントは次から属性を取得します。 <InlinePopover type="browser"/>。
  </Collapser>
  ...
  >
    ...
    除外する属性のプレフィックス<InlinePopover type="browser"/> 。末尾にワイルドカードとして`*`を使用できます。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    **注:**すべてのユーザー属性のデフォルトは`true`です。ただし、デフォルトでは、リクエスト パラメータはすべての宛先に対して無効になっています。
