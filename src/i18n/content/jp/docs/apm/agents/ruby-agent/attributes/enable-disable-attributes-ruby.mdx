...
ここでは、属性を有効または無効にするプロパティと、[宛先](/docs/agents/manage-apm-agents/agent-data/agent-attributes#destinations)に対してどの属性を含めるか除外するかを決定するときに New Relic が使用するルールについて説明します。これには、New Relic エージェント属性のリリースで非推奨または削除された Ruby エージェント プロパティの概要も含まれます。
