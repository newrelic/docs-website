...
2. Sinatra アプリで、Sinatra <DoNotTranslate>**require**</DoNotTranslate>ディレクティブのすぐ下に以下を追加します。

   ```
   require 'newrelic_rpm'
   ```

3. `RACK_ENV`を、 <DoNotTranslate>**newrelic.yml**</DoNotTranslate>ファイル内の構成定義に対応する環境 (開発、ステージング、本番など) に設定します。

<DoNotTranslate>**Note:**</DoNotTranslate> 開発者モードは Rails でのみ機能します。
