...
**## エージェントの起動を確認する [#start_agent]Ruby エージェントは、ブラックリストに登録された実行ファイル名や rake タスク名、定数を検出しない限り、 `require 'newrelic_rpm'` と同時に、ほとんどの場合、自動的に起動します。これにより、一般的な rake タスクやインタラクティブなコンソールセッション中に起動することはありません。<CollapserGroup>
  <Collapser
