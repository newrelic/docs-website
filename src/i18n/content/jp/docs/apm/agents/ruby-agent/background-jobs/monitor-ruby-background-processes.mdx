...
<CollapserGroup>
  ...
  >
    バックグラウンド アプリが既に Ruby エージェントを実行している Rails 以外のアプリケーションである場合は、 **newrelic.yml**ファイルを、バックグラウンド ジョブを起動するディレクトリまたは**config**サブディレクトリにコピーします。<InlinePopover type="licenseKey" />が含まれていることを確認してください。
