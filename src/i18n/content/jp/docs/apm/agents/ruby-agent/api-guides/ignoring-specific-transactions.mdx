...
`newrelic_ignore_enduser`を使用すると、キャプチャに使用される JavaScript がエージェントによって自動的に挿入されなくなります。 [<InlinePopover type="browser"/>](/docs/agents/ruby-agent/features/page-load-timing-ruby)データ。サーバー側の計測は影響を受けません。
