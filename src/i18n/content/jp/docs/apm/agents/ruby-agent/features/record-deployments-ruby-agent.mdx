...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        使用する New Relic <InlinePopover type="licenseKey" />。 デフォルトでは、これは指定された`rails_env`の`newrelic.yml`ファイルの定義から取得されます。 これは[ RESTAPI キー ](/docs/apis/getting-started/intro-apis/access-rest-api-keys)と**同じではあり ません **。
      </td>
    </tr>
