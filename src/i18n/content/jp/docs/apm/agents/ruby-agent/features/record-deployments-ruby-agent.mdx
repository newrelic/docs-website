...
[****Capistrano がリモート ビルド マシンでデプロイ通知レシピを実行している場合、ビルド マシンには ](#Installation)[ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)が必要です。 有効な`newrelic.yml`ファイルをビルド マシンにコピーするか (おそらく Capistrano [Before Hook](https://capistranorb.com/documentation/getting-started/before-after/)を使用)、Capistrano 構成で`set :newrelic_license_key, 'YOUR_LICENSE_KEY'`を呼び出すことができます。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        使用する New Relic [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) 。 デフォルトでは、これは指定された`rails_env`の`newrelic.yml`ファイル内の定義から取得されます。これは[REST API キー](/docs/apis/getting-started/intro-apis/access-rest-api-keys)と同じではあり**ません**。
      </td>
    </tr>
