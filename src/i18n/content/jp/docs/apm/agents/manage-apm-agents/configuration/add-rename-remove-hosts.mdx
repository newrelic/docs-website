...
ホスト名を変更することはできません。ただし、 [ホストやクラスタの名前を冠したアプリケーションを作成することは可能です。](/docs/site/renaming-applications) 。たとえば、ホスト `ey01-s00057` と `ey01-s00058` が両方ともプロダクション [tier](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#tier) の一部である場合、 `App - Production` という名前のアプリケーションを作成し、これらの 2 つのホストがそのアプリケーションにレポートするようにできます。

## ホストの表示名の変更 [#display_name]

また、動的に割り当てられたホストを簡単に区別するために、ホストに表示名を割り当てることができます。この表示名は、アプリとその親ホストとの間のリンクには影響しません。表示名は APM UI に表示され、自動的に割り当てられた `host:port` の名前を装飾します。

以下の例では、 `QA Server` と `Local Dev Server` が表示名です。

![crop-cosmetic-label-hostname.png](src/images/crop-display-host-name_0.png 'crop-cosmetic-label-hostname.png')
...
- エージェント設定ファイルを編集または削除して無効にします。エージェントを無効にした後、すべてのアプリケーションホストインスタンス（Passenger、Jetty など）が完全に再起動されていることを確認してください。

時々、不正なアプリケーションサーバプロセスがデータを報告し続けることがあります。このような状況では、エージェントをシャットダウンし、データが報告されていないことを確認したにもかかわらず、 [アプリのヘルスステータスインジケータ](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#health-status) が緑色に表示されます。Web サーバに不正なプロセスがないか確認するか、 [support.newrelic.com](https://support.newrelic.com '新しいウィンドウで開くリンク') でサポートを受けてください。

古いホスト上のすべてのレポートアプリケーションがシャットダウンされると、古いホストが New Relic のリストから消え、新しいホストが表示されるまでに 10 分程度かかることがあります。
