...
[使用量ベースの料金モデルをご利用の](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing)お客様には、 [Enterprise エディション](https://newrelic.com/pricing)が必要です。元の価格モデルのお客様は、サブスクリプション レベルに応じてアクセスできます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        * [元のユーザー モデルのユーザーの場合](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model): アカウント所有者のみがこれを構成できます。**[one.newrelic.com](https://one.newrelic.com/all-capabilities)**にアクセスし、[アカウントのドロップダウン](/docs/using-new-relic/welcome-new-relic/get-started/glossary#account-dropdown)をクリックし、\[**アカウント設定**] を選択します。そのページで、\[**高セキュリティ モード]**を選択します。
