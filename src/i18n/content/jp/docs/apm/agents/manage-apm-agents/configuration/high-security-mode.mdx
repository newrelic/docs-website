...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        有効にするアカウントの[アカウント ID](/docs/accounts/accounts-billing/account-structure/account-id)を取得します。次に、それを使用してこの URL に移動します: `https://one.newrelic.com/admin-portal/apm-agents/hsm?account=YOUR_ACCOUNT_ID` 。そのページで、高セキュリティ モードを設定できます。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        高セキュリティ モードでは、次のアプリケーション プロファイリング データを報告できません。

        * [JFR を使用した Java のリアルタイム プロファイリング](/docs/apm/agents/java-agent/features/real-time-profiling-java-using-jfr-metrics/)
      </td>
    </tr>
  </tbody>
</table>
