...
通常、2 つのインスタンスが同じアプリ名、エージェント言語、[ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key)でレポートする場合、 New Relicそれらのデータを 1 つのNew Relic - モニター アプリに集約します。 アプリの New Relic [APM <DNT>**Overview**</DNT>ページ](/docs/apm/applications-menu/monitoring/apm-overview-page#infra-server)から個々のホストとインスタンスのデータを表示することもできます。
...
複数の名前を使用してアプリケーション データをレポートする場合、そのアプリケーションの<InlinePopover type="browser" />データも同じ設定を使用して複数のアプリケーションにグループ化されます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        [newrelic\_app\_config\_t\* config;](/docs/agents/c-sdk/install-configure/install-c-sdk-compile-link-your-code#app-name)で、セミコロン`;`で区切った最大3つの名前を識別します。
      </td>
    </tr>
...
<CollapserGroup>
  <Collapser id="example1" title="1つのアプリ、3つのクラスタ">
  ...
  <Collapser id="example2" title="1つのアプリの2つのインスタンス">
    ...
    * 2 つのアプリ: 各データセンターに 1 つ ( `East`と`West` )
