...
[サポートされているフレームワーク](/docs/agents/manage-apm-agents/installation/compatibility-requirements)を使用しているにもかかわらずAPMの[<DNT>**Transactions**</DNT>ページ](/docs/applications-menu/transactions-dashboard)にトランザクションが表示されない場合は、フレームワークインストゥルメンテーションが機能しているか確認するため[support.newrelic.com](https://support.newrelic.com)にサポートを依頼してください。
...
<CollapserGroup>
  ...
  >
    ...
    * [アノテーション](/docs/java/custom-instrumentation-by-annotation)：New Relicが個々のメソッドをインストゥルメントできるように、コードに<DNT>**@Trace**</DNT>アノテーションを追加してください。インストゥルメントするメソッドが数個程度の場合、アノテーションは簡単に実装できます。より複雑なインストゥルメンテーションを行う場合、あるいはコードを変更できない場合は、XMLを使用します。
...
アカウントがNew Relicに名前の異なるメトリックタイムスライスを送りすぎたり、それらのウェブトランザクションが適切に集計されない場合、[メトリクスのグループ化問題](/docs/agents/manage-apm-agents/troubleshooting/metric-grouping-issues)が起こります。たとえば、単一の`/user/controlpanel/`メトリック名ではなく、`/user/controlpanel/alice`、`/user/controlpanel/bob`および`/user/controlpanel/carol`が表示される場合があります。
