...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services >（アプリを選択）> Monitoring > Transactions >（トランザクションを選択）>（トレースを選択）：**これは、トランザクショントレースの**リクエスト属性**と**カスタムアトリビュート**のRubyエージェントの例です。正確な属性は、各自のエージェントおよび属性の設定によって異なります。
</figcaption>
