...
## Node.jsエージェントバージョン14へのアップグレード [#node-agent-v14]
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: Node.js 20のサポートを終了しました。
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>：`@apollo/server`の計装をプラグイン`@newrelic/apollo-server-plugin`から従来の計装に移行しました。
      </td>

      <td>
        * Apollo serverユーザーは、`@newrelic/apollo-server-plugin`をインストールして使用する必要がなくなります。計装は、すべてのapollo serverインスタンスに自動的に適用されます。
        * 詳細については、[Apollo Server移行ガイド](#apollo-server-migration-guide)を参照してください。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>：`koa-router`および`koa-route`の計装は削除。
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>：ライセンス、アプリケーション、およびセキュリティポリシーのサポートを削除しました（`LASP`）。
      </td>

      <td>
        * この機能は非推奨となり、`config.security_policies_token`で設定可能でした。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: クロスアプリケーショントレース（`CAT`）機能のサポートを削除しました。
      </td>

      <td>
        * CATはエージェントv8.4.0で非推奨となり、サービス間でコンテキストを伝播するためにディストリビューティッド（分散）トレーシングに置き換えられました。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: デフォルトで`config.distributed_tracing.exclude_newrelic_header`が`true`に設定されるように更新されました。
      </td>

      <td>
        * これはデフォルトでW3Cヘッダーに置き換えられました。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>：rest パラメーターを採用するため、`shim.argsToArray`APIメソッドを削除しました。
      </td>
    ...
    <tr>
      <td>
        <DNT>**BREAKING**</DNT>：`shim.prefixRouteParameters`APIメソッドを削除しました。
      </td>

      <td>
        * Fastify計装はトレースチャネル診断を使用するように更新されたため、このメソッドは不要になりました。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: `mysql`および`mysql2`の静的`MySQL` `Pool#query`セグメントをドロップしました。
      </td>

      <td>
        * `Database/*`セグメントの方が有用であるため、これにより不要なセグメントが作成されました。
      </td>
    </tr>

    <tr>
      <td>
        <DNT>**BREAKING**</DNT>: 以下のライブラリのサポートされる最小バージョンを更新しました：
      </td>

      <td>
        * `@nestjc/cli` および`@nestjs/core`を10.0.0に
        * `bluebird` 3.0.0へ
        * `mysql2` 3.0.0へ
        * `next` 14.0.0へ
...
### Apollo Server移行ガイド [#apollo-server-migration-guide]

エージェントの14.0.0以降、apollo serverの顧客は`@newrelic/apollo-server-plugin`をインストールして使用する必要がなくなります。計装はすべてのapollo serverインスタンスに自動的に適用されます。ただし、顧客がプラグインを設定した場合、移行計画の詳細が記載されます。
...
* `captureScalars` 現在は`config.apollo_server.scalars`または `NEW_RELIC_APOLLO_SERVER_SCALARS`
* `captureIntrospectionQueries` 現在は`config.apollo_server.introspection_queries`または `NEW_RELIC_APOLLO_SERVER_INTROSPECTION_QUERIES`
* `captureServiceDefinitionQueries` 現在は`config.apollo_server.service_definition_queries`または `NEW_RELIC_APOLLO_SERVER_SERVICE_DEFINITION_QUERIES`
* `captureHealthCheckQueries` 現在は`config.apollo_server.health_check_queries`または `NEW_RELIC_APOLLO_SERVER_HEALTH_CHECK_QUERIES`
* `captureFieldMetrics` 現在は`config.apollo_server.field_metrics`または `NEW_RELIC_APOLLO_SERVER_FIELD_METRICS`
...
現在は`newrelic.setApolloOperationAttributesCallback`です。コールバックを受け取り、存在する場合、コールバックには以前と同じ引数が渡されます。 `requestContext`
...
現在は`newrelic.setApolloResolverAttributesCallback`です。コールバックを受け取り、存在する場合、コールバックには以前と同じ引数が渡されます。 `{ source, args, contextValue, info }`
...
`MySQL``host:port`メトリクスが記録されていることを確認しました
