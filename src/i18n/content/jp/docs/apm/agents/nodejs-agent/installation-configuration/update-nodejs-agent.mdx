...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        **速報**: ESM ローダーが Node.js 20 をサポートするように更新されました。
      </td>
    ...
    <tr>
      <td>
        **速報**: デフォルトのコンテキスト マネージャーが AsyncLocalContextManager になるように更新されました。
      </td>
    ...
    <tr>
      <td>
        **速報**: `shim.handleCATHeaders` の名前が `shim.handleMqTracingHeaders`に変更されました。
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        **速報**: `config.transaction_tracer.record_sql` のデフォルトをオフから難読化に更新しました。これは、SQL ステートメントはキャプチャされますが、難読化されることを意味します。
      </td>
    </tr>

    <tr>
      <td>
        **速報**: ルート (URL) パラメータは、トランザクション、ルート セグメント、およびスパンの `request.parameters.route.*` 属性として保存されるようになりました。
      </td>
    ...
    <tr>
      <td>
        **速報**: WebFrameworkShim クラスから CaptureUrlParams を削除しました。
      </td>
    </tr>

    <tr>
      <td>
        **非推奨の通知**: shim.unwrap および shim.unwrapOnce は、複数回ラップされたアイテムをラップ解除しようとすると機能しなくなります。
      </td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        **速報**：「証明書」設定（通常はプロキシを使用）を使用する場合、エージェントには New Relic 証明書バンドルが自動的に含まれなくなりました。
      </td>
