...
New RelicのNode.jsエージェントを使用してMicrosoft Azure Web Appsをホスティングサービスとして使用する場合の特別な注意事項について説明します。

## 必要なこと [#compatibility]

[Node.jsエージェントの互換性と要件](/docs/agents/nodejs-agent/getting-started/compatibility-requirements-nodejs-agent)に加えて、このチュートリアルでは、プラットフォームにNode.jsとWindows Azure SDK for Node.jsが正しくインストールされていることを前提としています。
...
3. <DNT>**app settings**</DNT>に以下を追加します。
   ...
   * `new_relic_app_name`：Windows Azure Webサイト名
...
エージェントがHerokuやAzure Web AppsなどのPaaS環境で実行される場合、 `newrelic.js`内のすべての構成変数には、環境変数として設定できる対応する変数があります。設定ファイル内の変数を自由に組み合わせることができます。環境変数は[設定ファイルの設定を上書きします](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#methods-and-precedence)。
