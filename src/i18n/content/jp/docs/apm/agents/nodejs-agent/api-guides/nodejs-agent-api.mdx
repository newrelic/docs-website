...
<CollapserGroup>
  ...
  >
    ...
    * `グループ はオプションで、ユーザーインターフェース トランザクションタイプ を介して、類似したジョブをグループ化することができます。 名前 のように、 グループ は静的にする必要があります。`[****````](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)
    * [* `ハンドル` は、インストルメント化したいバックグラウンドジョブ全体を含む関数を定義します。](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)

    [](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)

    [New Relic は、自動インスツルメンテーションで取得されるすべてのメトリクスを取得します。また、 ](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)[`startSegment()`](#startSegment) による手動のインスツルメンテーションも行います。
