...
<CollapserGroup>
  ...
  >
    ...
    有効にするために HTML ページのヘッダーに挿入される HTML スニペットを返します。 [<InlinePopover type="browser"/>](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs)。HTML はブラウザに小さな JavaScript ファイルをフェッチし、ページ タイマーを開始するように指示します。
  </Collapser>
