...
<CollapserGroup>
  ...
  >
    ...
    * `handle`は、計測する関数を定義します。
    ...
    <DNT>**must**</DNT>では、 トランザクション の開始時に`newrelic.getTransaction()`を呼び出してカスタム トランザクション を手動で処理し、終了したら`transaction.end()`を呼び出します。 New Relic は、 `newrelic.startWebTransaction()`が呼び出されるとトランザクションの計測を開始し、 `transaction.end()`が呼び出されるとトランザクションを終了します。
  ...
  >
    ...
    * `group`はオプションであり、[ユーザー インターフェースの<DNT>**transaction type**</DNT>](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)を介して類似のジョブをグループ化することができます。 `name`と同様に、 `group`も静的である必要があります。
    * `handle`は、計測するバックグラウンド ジョブ全体を含む関数を定義します。

    New Relic は、自動インストルメンテーションと[`startSegment()`](#startSegment)を介した手動インストルメンテーションによってキャプチャされるすべてのメトリクスをキャプチャします。

    <DNT>**must**</DNT>では、 トランザクション の開始時に`newrelic.getTransaction()`を呼び出してカスタム トランザクション を手動で処理し、終了したら`transaction.end()`を呼び出します。 New Relic は、 `newrelic.startBackgroundTransaction()`が呼び出されるとトランザクションの計測を開始し、 `transaction.end()`が呼び出されるとトランザクションを終了します。
...
<CollapserGroup>
  ...
  >
    ...
    * `value`がオブジェクトの場合、 `count` 、 `total` 、 `min` 、 `max` 、 `sumOfSquares`のキーがすべて数値で含まれている必要があります。その他のキーは無視されます。 このフォームは、自分でメトリクスを集計し、定期的にレポートするのに役立ちます。たとえば、 `setInterval`から。 これらの値は、同じメトリックに対して以前に収集された値と集計されます。 これらのキーの名前は、プラットフォーム API で使用されるキーの名前と一致します。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    * `attributes`は、キーと値のペアのオブジェクトでなければなりません。キーは 255 文字未満である必要があり、値は文字列、数値、またはブール値である必要があります。
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="カスタムイベントの記録">
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="エラー オブジェクトを使用したログ イベントの記録">
...
## トランザクション処理方法 [#transaction-handle-methods]
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="分散型トレースヘッダの生成">
  ...
  >
    ...
    * `Unknown`
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="受信した分散型トレースヘッダーを受け入れる">
  ...
  >
    ...
    * `DistributedTracePayload#text`: ペイロードの JSON 表現を返します。
      ...
      <CollapserGroup>
        <Collapser id="example-link-bg-txn" title="ネストされたバックグラウンド・トランザクションのリンク">
          ```js
          // Call newrelic.getTransaction to retrieve a handle on the current transaction.
    ...
    * `DistributedTracePayload#httpSafe`: ペイロードの base64 でエンコードされた JSON 表現を返します。
      ...
      <CollapserGroup>
        <Collapser id="example-payload-outgoing-request" title="発信するリクエストにペイロードを置く">
          ```js
          // Call newrelic.getTransaction to retrieve a handle on the current transaction.
          var transactionHandle = newrelic.getTransaction();
          var payload = transactionHandle.createDistributedTracePayload();
...
<CollapserGroup>
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="カスタム属性の追加">
        ```js
  ...
  >
    ...
    <CollapserGroup>
      <Collapser id="example-link-bg-txn" title="カスタムスパン属性の追加">
        ```js
  ...
  >
    ...
    [<InlinePopover type="browser" />](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs)を有効にするために HTML ページのヘッダーに挿入される HTML スニペットを返します。 HTML はbrowserに小さな JavaScript ファイルを取得してページ タイマーを開始するように指示します。
  </Collapser>
  ...
  >
    ...
    この関数を複数回呼び出すと、このコールバック関数の以前に定義されたバージョンが置き換えられます。 例：
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            `true`の場合、アクティブなトランザクションがなくなるまでエージェントはシャットダウンしません。
          </td>
        </tr>
      </tbody>
    </table>
...
<CollapserGroup>
  ...
  >
    <DNT>**rules.name**</DNT>のプログラム バージョン。 命名規則を追加すると、Node.js プロセスを再起動するまで削除することはできません。 Node.js エージェントの設定から追加することもできます。 両方とも必須です。
  </Collapser>
  ...
  >
    <DNT>**rules.ignore**</DNT>のプログラム バージョン。 無視ルールが追加されると、Node.js プロセスが再起動されるまで削除することはできません。 Node.js エージェントの設定から追加することもできます。 これは必須です。
  </Collapser>
</CollapserGroup>