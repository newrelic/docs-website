...
<CollapserGroup>
  ...
  >
    ...
    * `handle`は、計測する関数を定義します。
    ...
    <DoNotTranslate>**must**</DoNotTranslate>では、 トランザクション の開始時に`newrelic.getTransaction()`を呼び出してカスタム トランザクション を手動で処理し、終了したら`transaction.end()`を呼び出します。 New Relic は、 `newrelic.startWebTransaction()`が呼び出されるとトランザクションの計測を開始し、 `transaction.end()`が呼び出されるとトランザクションを終了します。
  ...
  >
    ...
    * `group`はオプションであり、[ユーザー インターフェースの](/docs/apm/applications-menu/monitoring/transactions-dashboard#txn-type-dropdown)
      ...
      を介して類似のジョブをグループ化することができます。 `name`と同様に、 `group`も静的である必要があります。

    * `handle`は、計測するバックグラウンド ジョブ全体を含む関数を定義します。

    New Relic は、自動インストルメンテーションと[`startSegment()`](#startSegment)を介した手動インストルメンテーションによってキャプチャされるすべてのメトリクスをキャプチャします。

    <DoNotTranslate>**must**</DoNotTranslate>では、 トランザクション の開始時に`newrelic.getTransaction()`を呼び出してカスタム トランザクション を手動で処理し、終了したら`transaction.end()`を呼び出します。 New Relic は、 `newrelic.startBackgroundTransaction()`が呼び出されるとトランザクションの計測を開始し、 `transaction.end()`が呼び出されるとトランザクションを終了します。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="無視ルールの例"
  >
    <DoNotTranslate>**socket.io**</DoNotTranslate>を使用している場合は、すぐにルールを無視するユースケースが使用できます。 socket.io のロングポーリングがレスポンスタイム メトリックを支配し、アプリケーションの Apdex メトリックに影響を与えないようにするには、次のようなルールを追加します。
...
<CollapserGroup>
  ...
  >
    <DoNotTranslate>**rules.name**</DoNotTranslate>のプログラム バージョン。 命名規則を追加すると、Node.js プロセスを再起動するまで削除することはできません。 Node.js エージェントの設定から追加することもできます。 両方とも必須です。
  </Collapser>
  ...
  >
    <DoNotTranslate>**rules.ignore**</DoNotTranslate>のプログラム バージョン。 無視ルールが追加されると、Node.js プロセスが再起動されるまで削除することはできません。 Node.js エージェントの設定から追加することもできます。 これは必須です。
  </Collapser>
</CollapserGroup>