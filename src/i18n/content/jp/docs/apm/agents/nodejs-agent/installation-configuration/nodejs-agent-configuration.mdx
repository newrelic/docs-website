...
[ディストリビューティッド（分散）トレーシング](/docs/intro-distributed-tracing)では、リクエストが経由する分散システムのパスを見ることができます。設定ファイルを介して設定する場合は、以下のオプションを`distributed_tracing`セクションに配置してください。Node.jsエージェント7.2.0以降では、デフォルトでオンになっています。
