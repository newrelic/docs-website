...
<CollapserGroup>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    この設定を有効にすると、エージェントはGCPのメタデータから取得したインスタンスIDを使用して、実行中のアプリケーションのホスト名を設定します。
  </Collapser>
  ...
  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    ...
    この設定を有効にすると、`gcp_use_instance_as_host`も有効になっている場合、実行中のアプリケーションのホスト名を設定する際にGCP Cloud Runのリビジョン（`K_REVISION`）がインスタンスIDの先頭に付加され、ホスト名は`${K_REVISION}-${instance_id}`となります。
  </Collapser>
</CollapserGroup>
