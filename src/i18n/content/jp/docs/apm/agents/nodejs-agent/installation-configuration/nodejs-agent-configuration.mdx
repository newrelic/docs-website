...
<CollapserGroup>
  ...
  >
    ...
    これらの環境変数は、各設定オプションの下に**環境変数**として文書化されています。また、[環境変数を介してのみ設定](#environment-variable-overrides)できる、ほとんど使用されることがない2つの設定があります。より複雑なタイプを環境変数として指定する方法がわからない場合は、[リファレンスガイドを使用してください](#environment-variable-reference-guide)。
