...
<CollapserGroup>
  ...
  >
    ...
    [コード レベルのメトリクス](/docs/codestream/observability/code-level-metrics)の推進に役立つすべての Node.js Web フレームワークのミドルウェア スパンで追加の属性をキャプチャするかどうかを切り替えます。追加の属性は、 `code.filepath` 、 `code.function` 、 `code.lineno` 、および`code.column`です。
  </Collapser>
</CollapserGroup>
