...
<CollapserGroup>
  ...
  >
    ...
    ブラウザーのインストルメンテーション用の JavaScript ヘッダーを生成します。 `true`に設定すると、 [手動で有効にしない限り、エージェントはブラウザーの JS コードを自動的に挿入しません。 <InlinePopover type="browser"/>](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser)。 有効にして[ブラウザ タイミング ヘッダーを追加した](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs#procedures)場合でも、これを`false`に設定することで[アプリのブラウザ監視を無効に](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs#disabling)できます。
  </Collapser>
...
[スパン データは](/docs/apm/distributed-tracing/ui-data/span-event)[分散トレース](#distributed-tracing)用にレポートされます。 スパンをレポートするには分散トレースを有効にする必要があります。 `span_events`セクションでスパン構成を設定します。 オプションには次のものが含まれます。
...
無限トレースをオンにするには、以下の設定を追加します。 `infinite_tracing`セクションで Infinite Tracing 構成を設定します。 さらに、分散トレースを[有効にする](/docs/apm/agents/nodejs-agent/installation-configuration/distributed-tracing-nodejs-agent)必要があります。 Node.js APM エージェント バージョン 8.3.0 以降では、分散トレースがデフォルトで有効になっていることに注意してください。
