...
<CollapserGroup>
  ...
  >
    ...
    BrowserインストゥルメンテーションのJavaScriptヘッダーを生成します。`true`に設定すると、[ブラウザモニタリングを有効](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser)にしない限り、エージェントはブラウザJSコードのインジェクションを行いません。ブラウザモニタリングを有効にして[ブラウザタイミングヘッダーを追加した](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs#procedures)場合でも、この値を`false`に設定することで[アプリケーションのブラウザモニタリングを無効](/docs/agents/nodejs-agent/supported-features/page-load-timing-nodejs#disabling)にできます。
  </Collapser>
