...
<CollapserGroup>
  ...
  <Collapser id="log" title="filepath">
    ...
    New Relic エージェントのログファイルへのフルパス（ファイル名を含む）。デフォルト設定：`filepath: require('path').join(process.cwd(), 'newrelic_agent.log')`エージェントがログファイルを作成できない場合は、次のように動作します。

    * `console.error`を使用して`New Relic failed to open log file`エラーをログに記録する。
    * 後続のすべてのログメッセージをメモリにバッファリングする。
    * メモリバッファが128MiBの制限に達すると、エージェントは新しいログメッセージを破棄し、 `process.emitWarning`を介して次の警告を発行する。 `[NRWARN001] NewRelicWarning: Dropping log message, buffer would overflow.`

    ログファイル作成時に、エージェントはファイルの権限を0o600（所有者が読み書き可能）に設定します。

    * すべてのロギングを<DNT>**stdout**</DNT>に書き込むには、これを`stdout`に設定します。
    * すべてのロギングを<DNT>**stderr**</DNT>に書き込むには、これを`stderr`に設定します。

    <Callout variant="important">
      バージョン7.0.0より前のエージェントでは、ログファイルを作成できない場合、プロセスが終了してしまいました。
    </Callout>
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-root" title="sampler.root">
    ...
    ルートスパンのサンプリングの判断方法を制御します。ルートスパンとは、リモートの親を持たないトランザクションのスパン（受信した`traceparent`または`newrelic`のディストリビューティッド（分散）トレーシングヘッダーがないもの）のことです。有効な値は、次のとおりです。

    * `adaptive` （デフォルト）：標準で備わっているNew Relicの適応型サンプリングアルゴリズムを使用します。
    * `always_on`：すべてのスパンをサンプリングします。
    * `always_off`：スパンをサンプリングしません。
    * `trace_id_ratio_based`：トレースの一定割合をサンプリングします。対象はトレースIDに基づいて決定論的に選定されます。

    たとえば、設定ファイルを使って設定する場合は次のように記述します。
    ...
    `trace_id_ratio_based`に設定する場合は、`ratio`（`0`から`1`までの数値）も指定する必要があります。
    ...
    `adaptive`に設定すると、このサンプラーのみを対象に、サンプリングターゲット（1分あたり1～120トランザクション）を任意に上書きできます。
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="sampler.remote_parent_sampled">
    ...
    traceparentヘッダーに`01`のsampledフラグがある場合、これはスパンのサンプリングを処理する方法を制御します。設定可能な文字列値は次のとおりです。
    ...
    * `trace_id_ratio_based`：トレースの一定割合をサンプリングします。対象はトレースIDに基づいて決定論的に選定されます。
    ...
    両方が適用可能な場合、この設定が[`remote_parent_not_sampled`](#dt-sampler-remote-parent-not-sampled)よりも優先されます。
    ...
    `trace_id_ratio_based`に設定する場合は、`ratio`の値を`0`～`1`の範囲で指定する必要があります。`adaptive`に設定すると、このサンプラーのみを対象に、サンプリングターゲットを任意に上書きできます。また、[`sampler.root`](#dt-sampler-root)のオブジェクト形式と環境変数の例に従って、`root`を`remote_parent_sampled`に置き換えることでもこの設定が可能です。
  </Collapser>
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    traceparentヘッダーに`00`のsampledフラグがある場合、これはスパンのサンプリングを処理する方法を制御します。設定可能な文字列値は次のとおりです。
    ...
    * `trace_id_ratio_based`：トレースの一定割合をサンプリングします。対象はトレースIDに基づいて決定論的に選定されます。
    ...
    この設定は、[`remote_parent_sampled`](#dt-sampler-remote-parent-sampled)が適用されない場合（traceparentのsampledフラグが`00`の場合）にのみ有効になります。
    ...
    `trace_id_ratio_based`に設定する場合は、`ratio`の値を`0`～`1`の範囲で指定する必要があります。`adaptive`に設定すると、このサンプラーのみを対象に、サンプリングターゲットを任意に上書きできます。また、[`sampler.root`](#dt-sampler-root)のオブジェクト形式と環境変数の例に従って、`root`を`remote_parent_not_sampled`に置き換えることでもこの設定が可能です。
  </Collapser>
  ...
  <Collapser id="dt-sampler-adaptive-sampling-target" title="sampler.adaptive_sampling_target">
    ...
    デフォルト（適応型）サンプラーを設定する際に使用される、適応型サンプリングのサンプリングターゲット（1分あたりのトランザクション数）です。`[1, 120]`の範囲内（両端を含む）で指定する必要があります。New Relicへの接続時、この値がコレクターに送信され、接続応答で返される`sampling_target`の値がエージェントでの適応型サンプリングで使用されるサンプリングターゲットとなります。
...
<CollapserGroup>
  ...
  <Collapser id="infinite-tracing-batching" title="バッチ処理">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    この設定では、スパンを250個ずつ、または5秒ごとに送信します。

    出力量が大幅に増加するため、この設定を変更しないでください。
  </Collapser>

  <Collapser id="infinite-tracing-compression" title="圧縮">
    <table>
      <tbody>
        <tr>
          <th>
...
使用量に基づく料金を正確に算定できるよう、エージェントがホストおよびcloud環境に関する情報を検出する方法を設定します。これらの設定は、アプリケーションの`newrelic.js`設定ファイルの`utilization`セクションにあります。
...
<CollapserGroup>
  <Collapser id="utilization-detect-aws" title="detect_aws">
    ...
    エージェントがAWSに問い合わせて、プロセスが実行されているVMに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-pcf" title="detect_pcf">
    ...
    エージェントが、プロセスがPivotal Cloud Foundry上で実行されているかどうかを検出しようと試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-azure" title="detect_azure">
    ...
    エージェントがAzureに問い合わせて、プロセスが実行されているVMに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-azurefunction" title="detect_azurefunction">
    ...
    エージェントが環境変数と呼び出しコンテキストを読み取り、エージェントが実行されているAzure Functionに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-docker" title="detect_docker">
    ...
    エージェントがファイルを読み取り、プロセスが実行されているコンテナに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-gcp" title="detect_gcp">
    ...
    エージェントがGCPに問い合わせて、プロセスが実行されているVMに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-detect-kubernetes" title="detect_kubernetes">
    ...
    エージェントがKubernetesに問い合わせて、プロセスが実行されているコンテナに関する情報の取得を試みるかどうかを設定します。
  </Collapser>
  ...
  <Collapser id="utilization-logical-processors" title="logical_processors">
    ...
    エージェントがホストの論理プロセッサ数として報告する値を上書きします。
  </Collapser>
  ...
  <Collapser id="utilization-billing-hostname" title="billing_hostname">
    ...
    請求および使用量の算定のためにエージェントが報告するホスト名を上書きします。
  </Collapser>
  ...
  <Collapser id="utilization-total-ram-mib" title="total_ram_mib">
    ...
    エージェントがホストの合計RAM容量（MiB）として報告する値を上書きします。
  </Collapser>
  ...
  <Collapser id="utilization-gcp-use-instance-as-host" title="gcp_use_instance_as_host">
    ...
    <Callout variant="important">
      非推奨です。エージェントのバージョン15で削除されます。代わりに、`gcp_cloud_run.use_instance_as_host`を使用してください。
    </Callout>

    この設定を有効にすると、GCPメタデータIDを使用して、実行中のアプリケーション（Services、Worker Pools、Jobs）のホスト名を設定します。
  </Collapser>
  ...
  <Collapser id="utilization-gcp-cloud-run-include-revision-in-host" title="gcp_cloud_run.include_revision_in_host">
    ...
    `true`に設定すると、エージェントがGoogle Cloud Run上でホスト名を生成する際、GCPインスタンスIDの前にCloud Runリビジョン名を付加します（`{revision}-{instance id}`）。リビジョン名は、Cloud Run Serviceでは`K_REVISION`、Cloud Run Worker Poolでは`CLOUD_RUN_REVISION`、Cloud Run Jobでは`CLOUD_RUN_EXECUTION`から取得されます。ただし、この設定が有効になるには、`utilization.gcp_cloud_run.use_instance_as_host`も`true`に設定する必要があります。
  </Collapser>
...
これらの設定は、エージェントによって提供されるOpenTelemetryベースの各種機能を制御します。これらの設定は、アプリケーションの`newrelic.js`設定ファイルの`opentelemetry`セクションにあります。

<Callout variant="important">
  OpenTelemetryの機能群が開発中の間は、この設定が変更される可能性があります。
</Callout>
...
<CollapserGroup>
  <Collapser id="opentelemetry-enabled" title="opentelemetry.enabled">
    ...
    OpenTelemetry機能全体の有効・無効を切り替えるグローバルスイッチです。この設定を`false`にすると、他のOpenTelemetryサブ機能（`traces`、`logs`、`metrics`など）は、各サブ機能の設定に関係なく無効となります。
  </Collapser>
  ...
  <Collapser id="opentelemetry-traces-enabled" title="opentelemetry.traces.enabled">
    ...
    OpenTelemetry計装（`@fastify/otel`など）をNew Relicエージェントにブリッジできるようにします。
  </Collapser>
  ...
  <Collapser id="opentelemetry-logs-enabled" title="opentelemetry.logs.enabled">
    ...
    この設定を`true`にすると、エージェントは、OTel固有のAPIを通じて出力されたログをNew Relicに送信するよう、OpenTelemetryのログAPIを自動的に設定します。

    <Callout variant="important">
      この機能は[アプリケーションログ転送](#application-logging-forwarding-enabled)に依存しているため、`application_logging.forwarding.enabled`も`true`に設定する必要があります。
    </Callout>
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-enabled" title="opentelemetry.metrics.enabled">
    ...
    この設定を`true`にすると、エージェントは、エージェントは、メトリクスをNew Relicに送信し、エージェントによってインストゥルメントされたアプリケーションエンティティに関連付けるよう、OpenTelemetryメトリクスAPIを自動的に設定します。
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-interval" title="opentelemetry.metrics.export_interval">
    ...
    OpenTelemetryメトリクスをNew Relicに送信しようとする間隔（ミリ秒）です。`opentelemetry.metrics.export_timeout`以上に設定する必要があります。
  </Collapser>
  ...
  <Collapser id="opentelemetry-metrics-export-timeout" title="opentelemetry.metrics.export_timeout">
    ...
    OpenTelemetryメトリクスのエクスポート処理が正常に完了するまでに許容される時間（ミリ秒）です。タイムアウトした場合、OpenTelemetryの診断APIを通じて報告されます。
  </Collapser>
</CollapserGroup>

## Apollo Server計装 [#apollo-server-instrumentation]

これらの設定は、エージェントのApollo Server（GraphQL）計装の動作をカスタマイズします。これらの設定は、アプリケーションの`newrelic.js`設定ファイルの`apollo_server`セクションにあります。
...
<CollapserGroup>
  <Collapser id="apollo-server-scalars" title="apollo_server.scalars">
    ...
    戻り値の型が`GraphQLScalarType`であるフィールドの解決時間の取得を有効にします。スカラー値を返す際に時間のかかる計算を実行する場合に、この機能が適していることがあります。事前計算済みのスカラーフィールドを多数返すクエリでの使用は推奨されません。

    <Callout variant="tip">
      クエリ／ミューテーションのリゾルバーは、スカラー型を返す場合でも常に計測されます。
    </Callout>
  </Collapser>
  ...
  <Collapser id="apollo-server-introspection-queries" title="apollo_server.introspection_queries">
    ...
    [IntrospectionQuery](https://www.graphql-js.org/api-v16/utilities/#introspectionquery)の処理時間の取得を有効にします。
  </Collapser>
  ...
  <Collapser id="apollo-server-service-definition-queries" title="apollo_server.service_definition_queries">
    ...
    Apollo Federated Gateway Serverから受信した[サービス定義クエリ](https://www.apollographql.com/docs/federation/federation-spec/#fetch-service-capabilities)の処理時間の取得を有効にします。
  </Collapser>
  ...
  <Collapser id="apollo-server-health-check-queries" title="apollo_server.health_check_queries">
    ...
    Apollo Federated Gateway Serverから受信した[ヘルスチェッククエリ](https://www.apollographql.com/docs/federation/api/apollo-gateway/#servicehealthcheck)の処理時間の取得を有効にします。
  </Collapser>
  ...
  <Collapser id="apollo-server-field-metrics" title="apollo_server.field_metrics">
    ...
    Apolloクエリで検出されたすべてのフィールドとリゾルバー引数に関するメトリクスの取得を有効にします。GraphQLスキーマ内の未使用フィールドを特定するための機能です。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="filename" title="NEW_RELIC_CONFIG_FILENAME">
    デフォルトの`newrelic.js`とは異なる設定ファイル名を設定するために使用されます。これは、環境変数としてのみ利用できます。ご自分の設定ファイルで設定することはできません。
