...
<CollapserGroup>
  ...
  >
    ...
    コレクターが無視するエラー メッセージに関連付けられたクラスのリストを記述する JavaScript オブジェクト。次の構成では、正確な (大文字と小文字が区別される) メッセージ文字列`Undefined`および`Out of time`を含むタイプ`Error`のすべてのエラーが無視されます。
...
<CollapserGroup>
  ...
  >
    ...
    [コンテキスト内でのログの](/docs/logs/logs-context/configure-logs-context-nodejs/)自動生成を有効にします。
