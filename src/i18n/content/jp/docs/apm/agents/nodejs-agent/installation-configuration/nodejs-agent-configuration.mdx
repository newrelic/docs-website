...
<CollapserGroup>
  ...
  <Collapser id="dt-sampler-remote-parent-sampled" title="sampler.remote_parent_sampled">
    ...
    traceparentヘッダーに`01`のsampledフラグがある場合、これはスパンのサンプリングを処理する方法を制御します。許容される値は次のとおりです。

    * `always_on`：エージェントはスパンをサンプリングします
    * `always_off`：エージェントはスパンをサンプリングしません
    * `default`：エージェントは既存のNRサンプリングを使用します（デフォルト）
  ...
  <Collapser id="dt-sampler-remote-parent-not-sampled" title="sampler.remote_parent_not_sampled">
    ...
    traceparentヘッダーに`00`のsampledフラグがある場合、これはスパンのサンプリングを処理する方法を制御します。許容される値は次のとおりです。
