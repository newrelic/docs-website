...
基本的なNode.jsエージェントのインストレーションを完了するには、ガイド付きインストールを使用して自動インストールを行うか（EUにお住まいの場合はEUを、JP地域にお住まいの場合はJPを選択）、本ドキュメントに記載されている指示に従ってください。いずれにしても、アカウントをまだお持ちでない場合は、New Relicアカウントが必要です。（[永久無料です。](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#how-pricing-works)）
...
<Callout variant="important">
  Apollo Serverモジュールを使用している場合、インストレーションの詳細については、[Apollo Serverプラグインに関するドキュメント](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/apollo-server-plugin-nodejs/)を参照してください。
