...
基本的なNode.jsエージェントのインストレーションを完了するには、ガイド付きインストールを使用して自動インストールを行うか（EUリージョンの場合はEUを、JPリージョンの場合はJPを選択）、本ドキュメントに記載されている指示に従ってください。いずれにしても、New Relicアカウントをまだお持ちでない場合は、アカウントの作成が必要です。（[New Relicアカウントは永久無料でご利用いただけます](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#how-pricing-works)）
