...
6. アプリの起動スクリプトに`-r newrelic`を追加します。たとえば、アプリケーションのエントリポイントが`./dist/server.js`の場合、次のようにrequireフラグを使用します。
   ...
   <Callout variant="important">
     `-r`必須フラグを使用できない場合は、アプリのメインモジュールの最初の行を`require('newrelic')`として使用することもできます。<DoNotTranslate>**Note**</DoNotTranslate>[Babel](https://babeljs.io/docs/en/index.html)または類似のトランスパイラー`require('newrelic')`を使用している場合、インストゥルメンテーションの問題が発生します。
