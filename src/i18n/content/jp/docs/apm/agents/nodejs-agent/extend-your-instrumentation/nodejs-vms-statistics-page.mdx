...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Node VMs** : 複数のサーバーが同じアプリケーションにレポートしている場合、ページはサーバー別にチャートを分類します。ページの上部にある \[**サーバー]**フィルターからサーバーを選択すると、個々のサーバーのデータを表示し、追加の詳細を取得できます。
</figcaption>

これらの測定値のスパイクや上昇傾向を特定の取引と関連付ける。

1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Node VMs**に移動します。
