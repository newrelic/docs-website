...
<Callout variant="important">
  モデルがストリーミングをサポートしている場合は、ストリーミングのバリアントも計算されます。
</Callout>
...
モデルおよびプロバイダーは通常、プロバイダーのモジュールのインストゥルメンテーションによって推移的にサポートされます。

| プロバイダー | サポート対象 | 移行 | | -------------- | --------- | ------------ | | Azure OpenAI | ❌ | ❌ | | Amazon Bedrock | ❌ | ❌ | | OpenAI | ✅ | ✅ |
