...
<CollapserGroup>
  ...
  >
    ...
    * [Winston](https://github.com/winstonjs/winston) 3.0.0以降（エージェントバージョン8.11.0が必要）
    * [Pino](https://github.com/pinojs/pino) 7.0.0以降（エージェントバージョン8.11.0が必要）
    * [Bunyan](https://www.npmjs.com/package/bunyan) 1.8.12以降（エージェントバージョン9.3.0が必要）
  </Collapser>
  ...
  >
    ...
    * [Nest.js](https://nestjs.com/) 8.0.0以降（`nest start`コマンドを使用してアプリケーションを起動する場合は、スタートアップバイナリを変更してNew Relicエージェント`nest start --exec 'node -r newrelic'`を読み込みます）
    * [Next.js](https://www.npmjs.com/package/next) 12.0.9〜13.3.0（ミドルウェア インストゥルメンテーションには12.2.0以降が必要）
