...
5. エージェントを有効にして Docker アプリを実行するには[、ライセンス キー](/docs/accounts/install-new-relic/account-setup/license-key)と[アプリ名](/docs/agents/manage-apm-agents/app-naming/name-your-application)を`docker run`コマンドに環境変数として追加します。
