...
通常、 <InlinePopover type="browser"/>エージェントはページに自動的に追加されるか、JavaScript スニペットをコピー/ペーストしてデプロイされます。これらの推奨される方法の詳細については、 [「ブラウザ アプリを New Relic に追加する」](/docs/browser/new-relic-browser/installation-configuration/add-apps-new-relic-browser)を参照してください。
