...
<Callout variant="tip">
  アプリケーションはKubernetesクラスタ上で実行されていますか? [Kubernetes APM auto-attach](/docs/kubernetes-pixie/kubernetes-integration/installation/k8s-agent-operator/)を使用したインストール方法を試してください。
</Callout>
...
<CollapserGroup>
  ...
  <Collapser id="server" title="サーバーにデプロイされたアプリ">
  ...
  <Collapser id="lambda" title="AWSLambdaでデプロイされたアプリ">
  ...
  <Collapser id="azure" title="Windows 上の Azure Web アプリ">
...
* Node.jsエージェントで[<InlinePopover type="browser" />](/docs/apm/agents/nodejs-agent/extend-your-instrumentation/browser-monitoring-nodejs-agent)追加しています