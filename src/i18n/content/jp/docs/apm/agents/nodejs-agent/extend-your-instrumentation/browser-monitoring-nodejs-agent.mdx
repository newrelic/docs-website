...
Node.js エージェントを使用すると、Web ページに[ブラウザー インストルメンテーション](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)を追加できます。使用するには<InlinePopover type="browser"/> Node.js エージェントを使用する場合は、Node.js エージェントの[最新リリースを](/docs/agents/nodejs-agent/installation-configuration/installing-maintaining-nodejs#upgrading)使用していることを確認してください。
