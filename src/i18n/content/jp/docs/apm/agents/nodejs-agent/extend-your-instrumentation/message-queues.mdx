...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & サービス > アプリケーション > (アプリを選択) > トランザクション > (トランザクションを選択):**トランザクションの**内訳テーブル**は、キュー操作を`MessageBroker`メトリックとして分類し、 `Put` (メッセージを発行) または`Take` (メッセージを受信)。
</figcaption>
