...
[Node.js VM データで精選されたチャートを表示するには、 ****](https://github.com/nodejs/node-gyp#option-2)**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Node VMs**に移動します。詳細については、 [Node.js VM 統計ページ](/docs/agents/nodejs-agent/supported-features/node-vms-statistics-page)を参照してください。
