...
一部のNode.js VMメトリクスは、組み込みのNode.js APIを使用してデフォルトで収集されますが、その他のメトリクスにはオプションの[`@newrelic/native-metrics`](https://www.npmjs.com/package/@newrelic/native-metrics)パッケージが必要です：

* **デフォルトのサンプラー（ネイティブモジュール不要）:** メモリ（`Memory/*`）、イベントループキュー待機時間（`Events/wait`）、およびCPU（`CPU/*`）は、それぞれ`process.memoryUsage()`、`setTimeout`のラグ測定、および`process.cpuUsage()`を使用して常に収集されます。
* **ネイティブモジュールサンプラー:** ガベージコレクション（`GC/*`）および正確なイベントループCPU使用率（`Nodejs/EventLoop/CPU/Usage`）には、`@newrelic/native-metrics`が必要です。モジュールがインストールされていないか、ロードに失敗した場合、これらのメトリクスは表示されず、エージェントはデフォルトのサンプラーのみで実行を継続します。
...
<CollapserGroup>
  <Collapser id="time-in-garbage-collection" title="ガベージコレクションの時間">
    <Callout variant="important">`@newrelic/native-metrics`が必要です。このメトリクスはデフォルトでは収集されません。</Callout>
  ...
  <Collapser id="memory" title="メモリー">
    <Callout variant="tip">デフォルトで収集されます。ネイティブモジュールは不要です。</Callout>
  ...
  <Collapser id="cpu" title="CPU">
    <Callout variant="tip">Node.js v6.1.0 でデフォルトで収集されます以降。ネイティブモジュールは不要です。</Callout>
  ...
  <Collapser id="event-loop" title="イベントループ">
    <Callout variant="important">`@newrelic/native-metrics`が必要です。このメトリクスはデフォルトでは収集されません。</Callout>
