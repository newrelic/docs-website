...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="カスタムアトリビュート"
  >
    [`addCustomAttribute()`](https://newrelic.github.io/node-newrelic/API.html#addCustomAttribute) Node.jsエージェントAPIの呼び出しに追加される属性です。この属性のキー・ネームは、このメソッドを呼び出す際に指定したものに依存します。
  ...
  >
    [`noticeError()`](https://newrelic.github.io/node-newrelic/API.html#noticeError) Node.js エージェント API の呼び出しに追加される属性です。この属性のキーネームは、メソッドを呼び出す際に指定した内容によって異なります。
  ...
  <Collapser
    ...
    title="リクエストパラメータ"
  >
    トランザクションからパラメータを要求します。Node.js エージェントは、デフォルトで GET パラメータをキャプチャします。POSTパラメータを取得するには、 [`addCustomAttribute()`](https://newrelic.github.io/node-newrelic/API.html#addCustomAttribute) Node.jsエージェントのAPIコールを使用します。
  </Collapser>
</CollapserGroup>
