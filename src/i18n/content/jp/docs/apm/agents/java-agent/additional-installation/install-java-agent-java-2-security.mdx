---
title: WebLogic、WebSphere、およびSolr v9 用のJava 2 セキュリティ インストレーション
tags:
  - Agents
  - Java agent
...
WebLogic、WebSphere、またはSolr v9 ユーザーでJava 2 セキュリティ モデルを使用している場合、 Javaエージェントには[Javaインストレーション](/install/java)に加えて設定が必要です。
...
## Solr v9: Java 2 セキュリティ [#solr-java-2]

セキュリティマネージャは、Java 24以前の環境で実行されている場合、Solrバージョン9.xではデフォルトで有効になっています。これにより、エージェントが正常に動作しなくなります。

エージェントの適切な動作は、以下のいずれかの方法で実現できます。

1. 環境変数`SOLR_SECURITY_MANAGER_ENABLED=false`を設定するか、`solr.in.sh/solr.in.cmd`スタートアップファイルで設定します。

2. ポリシーファイルに次のテキストを追加してください。パスの末尾にあるダッシュ`-`は必ず保持してください。これにより、ターゲットフォルダ内のすべての jar に必要な権限が付与されます。
   ...
   <Callout variant="tip">
     Java 9以降を使用しており、アクセス拒否エラーが発生する場合は、追加の権限（`java.lang.RuntimePermission "accessSystemModules"`など）を付与する必要がある場合があります。
   </Callout>
