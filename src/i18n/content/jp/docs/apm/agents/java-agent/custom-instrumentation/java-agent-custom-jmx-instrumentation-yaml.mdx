...
カスタム JMX データは<InlinePopover type="apm"/>チャートには表示されませんが、[データをクエリし](#display_metrics)てカスタム チャートを作成することができます。
...
<CollapserGroup>
  ...
  >
    ...
    2. ファイルが指定されたディレクトリにあり、ファイルの拡張子が`.yml`であることを確認してください。
  </Collapser>
</CollapserGroup>
...
<DNT>**JMX**</DNT>を検索する方法、または YAML ファイルに設定されているカウンターへの直接パスを検索する方法は次のとおりです。
