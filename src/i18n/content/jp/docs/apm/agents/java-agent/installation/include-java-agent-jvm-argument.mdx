...
Resinで`-javaagent`引数をパスする場合は、この引数を`conf/resin.conf`または`conf/resin.xml`ファイルの`<jvm-args>`セクションに追加します。
...
Java 9で導入されたモジュールシステムは、例外`NoClassDefFoundError: java/sql/SQLException`を引き起こす可能性があります。これは、`-javaagent`プロパティが`conf/resin.conf`または`conf/resin.xml`ファイルに追加されている場合に発生します。Java 9以降を使用している場合は、`-javaagent`プロパティがこれらのファイルに含まれていないことを確認してください。
...
* オプション2 - `-javaagent`プロパティを`bin/resin.sh`スクリプトに追加します。

`-javaagent`プロパティを`bin/resin.sh`スクリプトに直接追加し、この行`exec $JAVA_EXE -jar lib/resin.jar $@`を次のように変更します。
