...
<CollapserGroup>
  ...
  >
    アプリケーションの `project/build.scala` ファイルの appDependencies メソッドに次の行を追加します ( `S.V` を Scala のバージョンに、 `X.Y.Z` を使用する [Java エージェントのバージョン](/docs/agents/java-agent/installation/update-java-agent#procedures) に置き換えます)。
  ...
  >
    アプリケーションの `project/build.sbt` ファイルに次の行を追加します ( `S.V` を Scala のバージョンに、 `X.Y.Z` を使用する[Java エージェントのバージョン](/docs/agents/java-agent/installation/update-java-agent#procedures) に置き換えます)。
...
Scala Future を返す非同期関数のセグメントを作成する場合は、 `TraceOps.asyncTraceFun`を使用します。これにより、セグメントのタイミングに関数の完了にかかる時間が含まれるようになります。
...
1. Scalaの設定ファイルに以下の情報を追加してください。
   ...
   <CollapserGroup>
     <Collapser
       ...
       title={<><strong>project/build.scalaを使った設定</strong> ファイル</>}
     >
       アプリケーションの `project/build.scala` ファイルの appDependencies メソッドに次の行を追加します ( `X.Y.Z` を使用する [Java エージェントのバージョン](/docs/agents/java-agent/installation/update-java-agent#procedures) に置き換えます)。
     ...
     <Collapser
       ...
       title={<><strong>project/build.sbt を使った設定</strong> ファイル</>}
     >
       アプリケーションの `project/build.sbt` ファイルに次の行を追加します ( `X.Y.Z` を使用する [Java エージェントのバージョン](/docs/agents/java-agent/installation/update-java-agent#procedures) に置き換えます)。
