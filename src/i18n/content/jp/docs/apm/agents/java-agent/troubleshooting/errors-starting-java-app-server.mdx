...
この問題を修正するには、 `-javaagent`スイッチで`newrelic.jar`ファイルへのフルパスを指定します。 たとえば、 `newrelic.jar`ファイルが Tomcat ホーム ディレクトリ<DNT>**/home/tomcat**</DNT>内の`newrelic`という名前のディレクトリにある場合は、次のようにスイッチを指定します。
