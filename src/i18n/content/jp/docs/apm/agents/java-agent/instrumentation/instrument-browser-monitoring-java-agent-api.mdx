...
2. オートインストルメントを無効にします。
   ...
   <CollapserGroup>
     ...
     >
       `browser_monitoring`スタンザの下で`disabled_auto_pages`フラグを使用します。たとえば、 **testpage_1.jsp**および**testpage_2.jsp**で自動計測を無効にするには、以下を使用します。
...
4. 時間追跡を開始するための適切なメソッドを追加します (例: `com.newrelic.api.agent.NewRelic.getBrowserTimingHeader()` 。
   ...
   <CollapserGroup>
     ...
     >
       <var>3.xx</var>を[最新のエージェント バージョン](/docs/release-notes/agent-release-notes/java-release-notes)に置き換えて、次を`build.sbt`に追加します。
     ...
     >
       <var>2.xx</var>を最新の[エージェント バージョン](/docs/release-notes/agent-release-notes/java-release-notes)に置き換えて、次を`Build.scala`に追加します。
     ...
     >
       1. すべてのブラウザー ページ ビューが 1 つの`/velocity`トランザクションにグループ化されないようにする (および[メトリックのグループ化の問題](/docs/agents/manage-apm-agents/troubleshooting/metric-grouping-issues)を回避する) には、 **newrelic.yml**ファイルで`enable_auto_transaction_naming`設定を無効にします。
