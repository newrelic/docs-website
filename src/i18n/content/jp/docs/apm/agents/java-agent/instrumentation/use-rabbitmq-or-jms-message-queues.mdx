...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services** に移動し、アプリをクリックします。**Monitor** \[モニター] セクションで **Transaction** \[トランザクション] をクリックし、トランザクションを選択して内訳表を表示します。
</figcaption>
