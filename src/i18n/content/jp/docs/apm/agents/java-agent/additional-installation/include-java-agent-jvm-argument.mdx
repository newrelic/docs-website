...
2. 左側のメニューから<DoNotTranslate>**SERVER SETTINGS > Java and JVM**</DoNotTranslate>を選択します。

3. エージェントAPIを使用している場合：<DoNotTranslate>**ColdFusion Class Path**</DoNotTranslate>フィールドで`newrelic-api.jar`へのパスを指定します。

4. <DoNotTranslate>**JVM Arguments**</DoNotTranslate>フィールドに、`-javaagent`引数を追加します。

   ```
   -javaagent:/full/path/to/newrelic.jar
   ```

5. <DoNotTranslate>**Submit Changes**</DoNotTranslate>を選択し、ColdFusionサーバーを再起動します。
...
1. Glassfishコンソールから<DoNotTranslate>**Application Server > JVM Settings > JVM options**</DoNotTranslate>を選択します。

2. JVMオプションページで、<DoNotTranslate>**Add JVM option**</DoNotTranslate>を選択します。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Play 2.0"
  >
    1. `start`スクリプトを含む<DoNotTranslate>**unzipped**</DoNotTranslate>の配布ファイルから開始します。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="アプリサーバー Solr"
  >
    <DoNotTranslate>**application server**</DoNotTranslate>でSolrを実行する際は、該当するアプリケーションサーバーの手順に従って、`-javaagent`フラグを追加してください。また、アプリケーションサーバーでJMXが有効化されていることを確認します。APM UIのSolrページにデータが表示されない場合は、[Solrデータのトラブルシューティング手順](/docs/agents/java-agent/troubleshooting/solr-data-not-appearing-apm-solr-tab-java)に従ってください。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Windowsの場合"
  >
    1. <DoNotTranslate>**Start > Apache Tomcat X.Y.Z. > Configure Tomcat > Java**</DoNotTranslate>を選択します。

    2. <DoNotTranslate>**Java Options**</DoNotTranslate>テキストボックスに、引数を入力します。パス区切り文字には、フォワードスラッシュ`/`を使用します。Tomcat 6の場合は、`-javaagent`引数の後に改行を追加します。

       ```
       -javaagent:/full/path/to/newrelic.jar
       ```

    3. <DoNotTranslate>**Apply**</DoNotTranslate>を選択し、Tomcatを再起動します。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    1. 管理コンソールから、<DoNotTranslate>**Environments > Servers > (select a server) > Server Start > Arguments**</DoNotTranslate>に移動します。

    2. <DoNotTranslate>**Arguments**</DoNotTranslate>から、以下を追加します。
...
1. アドミンコンソールから、<DoNotTranslate>**Servers > Application servers > (select a server) > Configuration > Service Infrastructure > Java and Process Management**</DoNotTranslate>を選択します。

2. <DoNotTranslate>**Process Definition > Additional Properties**</DoNotTranslate>を選択し、次に<DoNotTranslate>**Java Virtual Machine**</DoNotTranslate>を選択します。

3. <DoNotTranslate>**Generic JVM arguments**</DoNotTranslate>フィールドに、`newrelic.jar`ファイル用の`-javaagent`引数を追加します。

   ```
   -javaagent:/full/path/to/newrelic.jar
   ```

4. <DoNotTranslate>**Apply**</DoNotTranslate>を選択し、次に<DoNotTranslate>**Save**</DoNotTranslate>を選択します。
