...
<CollapserGroup>
  <Collapser id="grails-run-app" title="run-appで渡す">
  ...
  <Collapser id="grails-run-war" title="run-warでパス">
...
<CollapserGroup>
  <Collapser id="jboss-domain-mode" title="ドメインモードでの設定（6.x EAPおよび7.0 AS）">
  ...
  <Collapser id="jboss-standalone-mode" title="スタンドアロンモードでの設定（その他のバージョン）">
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>
            UnixまたはmacOS、6.x EAPまたは7.0.x AS以上
          </td>
...
<CollapserGroup>
  ...
  <Collapser id="play-2-3-etc" title="Play 2.3、2.4、2.5">
    1. `start`スクリプトを含む<DNT>**unzipped**</DNT>の配布ファイルから開始します。
...
<DNT>Solr</DNT>で`-javaagent`引数をパスする場合は、次の手順に従います。

<CollapserGroup>
  <Collapser id="solr-standalone-5" title="スタンドアロン Solr 5.x以上">
  ...
  <Collapser id="solr standalone-other" title="スタンドアロン Solr 4.x以下">
  ...
  <Collapser id="solr-app-server" title="アプリサーバー Solr">
...
<CollapserGroup>
  <Collapser id="tomcat-setenvsh" title="setenv.shの場合">
  ...
  <Collapser id="tomcat-setenvbat" title="setenv.batの場合">
  ...
  <Collapser id="tomcat-catalinash" title="catalina.shの場合（非推奨）">
    <Callout variant="tip">
  ...
  <Collapser id="tomcat-catalinabat" title="catalina.batの場合（非推奨）">
    <Callout variant="tip">
  ...
  <Collapser id="tomcat-windows" title="Windowsの場合">
  ...
  <Collapser id="apache-commons-daemon" title="Apache Commons Daemonの場合">
...
<CollapserGroup>
  <Collapser id="weblogic-nix" title="管理サーバー（LinuxまたはMacOS）">
  ...
  <Collapser id="weblogic-windows" title="管理サーバー（Windows）">
  ...
  <Collapser id="weblogic-managed-server" title="管理対象サーバーインスタンス">
...
## WebSphereコミュニティ [#Installing\_on\_Websphere\_Community\_Edition][#Installing_on_Websphere_Community_Edition]
...
## WebSphere Libertyプロファイル [#Installing\_on\_WebSphere\_Liberty\_Profile][#Installing_on_WebSphere_Liberty_Profile]
...
<CollapserGroup>
  <Collapser id="wildfly-domain-mode" title="ドメインモードで設定">
    1. 以下にあるサーバーグループのJVM属性を編集します。
  ...
  <Collapser id="wildfly-standalone-mode" title="スタンドアロンモードで設定">
    <table>
      <thead>
        <tr>
          <th style={{ width: "150px" }}>
