...
<CollapserGroup>
  ...
  >
    ...
    ディスパッチャーに加えて`metricName`を`@Trace(metricName="YourMessageHere", dispatcher=true)`のように設定した場合、トランザクション名はAPM**トランザクション**ページでYourMessageHereと表示されますが、このメソッドで費やした時間はすべてのトランザクショントレースで、YourMessageHereとして表示されません。
