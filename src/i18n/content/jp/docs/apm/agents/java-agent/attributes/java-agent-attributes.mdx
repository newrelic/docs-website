...
<CollapserGroup>
  ...
  >
    ...
    * `request.headers.userAgent`
      ...
      カスタムか標準かを問わず、追加のリクエスト ヘッダーをキャプチャする場合は、 [構成ファイルのドキュメント](https://docs.newrelic.com/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file/#tt-custom-request-headers) を参照するか、 [Java エージェント API](/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes#api-java)の `addCustomParameter()` メソッドを使用してください。
