...
## [UI オプション](/docs/agents/java-agent/custom-instrumentation/java-custom-instrumentation)

[](/docs/agents/java-agent/custom-instrumentation/java-custom-instrumentation)New Relic の UI を使用して、 [Custom Instrumentation Editor でインスツルメンテーションルールを定義するか、Java アプリの **Settings** で XML ファイルを編集することができます。](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-quickly-customize-your-java-instrumentation)
...
- サンプル XML ファイルのダウンロード
- 既存の XML ファイルを選択して編集します。
- 楽器の履歴を検索します。
...
![スレッドプロファイラ。ネストされたトランザクションの色](src/images/crop-java-custom-inst-colors.png 'スレッドプロファイラ。ネストされたトランザクションの色')

<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM> (select an app)> Events>
  Thread profiler:** メソッドが灰色に表示されている場合（**Instrumentation not
  allowed**
  ）、スレッドプロファイルツリーを展開して、インスツルメンテーション可能なネストされたトランザクション（白色）を探してみてください。
</figcaption>
...
## XML でトランザクションを分離 [#separate-transaction]

以下は、複数の `メソッド` 値を持つポイントカットの例です。 `nameTransaction` は、APM [**Transactions** breakdown table](/docs/apm/applications-menu/monitoring/transactions-page) のセグメントとしてではなく、独立したトランザクションとしてメソッドを分離するために XML に追加されています。
...
アプリケーションで非同期プロセスを使用している場合、Java API を使用して非同期ワーカーアクティビティを親トランザクションに接続することができる場合があります。デフォルトでは、これらのトランザクションはリンクされません。詳細については、 [Java asynchronous instrumentation](/docs/agents/java-agent/async-instrumentation/introduction-java-async-instrumentation) を参照してください。
