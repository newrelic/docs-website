...
[Heroku](https://devcenter.heroku.com/articles/newrelic)は、Scala を含むさまざまなエージェント言語で Web アプリケーションをホストするための Platform as a Service (PaaS) ソリューションです。 New Relic を使用すると、 [APM](/docs/apm/new-relic-apm)と[<InlinePopover type="browser"/>](/docs/browser/new-relic-browser)からのメトリクスを使用して Heroku を拡張できます。
