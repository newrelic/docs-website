...
また、[エージェントインストゥルメンテーションを拡張](#extend-instrumentation)し、当社の[データエクスプローラー](/docs/query-your-data/explore-query-data/data-explorer/introduction-data-explorer)や[ダッシュボード](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards)など、他のNew Relic製品でデータを調査することもできます。
