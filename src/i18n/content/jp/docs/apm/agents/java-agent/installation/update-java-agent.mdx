...
4. `Diff` 現在の`newrelic.yml`と新しくダウンロードしたZipファイルからの`newrelic.yml`の差分を実行し、必要に応じて[カスタマイズされた差分をアップデートします](#diff)。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        **非推奨のAPI：**
        ...
        `@Trace`アノテーションの`skipTransactionTrace`属性は非推奨になりました。代わりに、`excludeFromTransactionTrace`属性を使用します。
    ...
    <tr>
      ...
      <td>
        ...
        **SSL設定オプションを削除：**
        ...
        SSLは、常に、New Relic Serversとの通信に使用されるようになりました。`newrelic.yml`SSL設定と`-Dnewrelic.config.ssl`システムプロパティは使用されなくなりました。いずれかの値を`true`以外に設定すると、ログで警告が発生します。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        * Linux、Solaris、Windows、macOS 用 Oracle Hotspot JVMバージョン5.0
