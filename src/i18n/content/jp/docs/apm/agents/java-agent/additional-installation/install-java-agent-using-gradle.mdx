   ...
   <CollapserGroup>
     ...
     >
       ...
       * READMEをご覧ください
       ...
       3. エージェントをダウンロードして解凍するタスクを実行します。例えば、以下のようになります。
          ...
          <CollapserGroup>
            <Collapser title={<>zipファイルを操作するための<InlineCode>build.gradle</InlineCode>ファイルの例を次に示します。</>}>
              ```
              plugins {
                  id 'java'
                  id "de.undercouch.download" version "3.4.3"
     ...
     >
       ...
       2. New Relic アカウント作成時に受け取った `newrelic.yml` ファイルを探すか、使用しているエージェントの [バージョン](/docs/release-notes/agent-release-notes/java-release-notes) のものをダウンロードします。
          ...
          <CollapserGroup>
            <Collapser title={<>これは、個々のコンポーネント（JavaエージェントおよびAPI jar）を操作するための<InlineCode>build.gradle</InlineCode>の例です。</>}>
              ```
              plugins {
                  id 'java'
              }
