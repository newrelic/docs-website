...
2. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & services >（アプリを選択）> Settings > Instrumentation**の順に移動します。ページの下部にある**Recently collected thread profiles**リストが表示されるまでスクロールし、最新のスレッドプロファイルを選択します。
