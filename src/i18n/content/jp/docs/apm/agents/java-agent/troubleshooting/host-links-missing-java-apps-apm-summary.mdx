...
Linux エージェント環境のNew Relic InfrastructureエージェントがNew Relic Java エージェントとまったく同じ名前文字列を返すようにホスト名設定を構成します。 推奨事項: 完全修飾ドメイン名 (FQDN) 設定を編集します。 エージェントは OS の FQDN 設定からホスト名を読み取るため、そこにホスト名を設定すると、両方のエージェントが単一のホスト名を共有するようになります。
...
<CollapserGroup>
  ...
  <Collapser
    id="aws-example"
    title="例: <DNT>AWS Elastic Beanstalk</DNT> でのスクリプトホスト名設定"
  >
