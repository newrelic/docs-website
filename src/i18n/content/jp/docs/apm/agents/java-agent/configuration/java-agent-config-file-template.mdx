...
このドキュメントには、Javaエージェントが設定のために読む `newrelic.yml` ファイルのテンプレートが含まれています。設定ファイルの形式はYAMLです。ファイルの編集にYAMLの知識は必要ありませんが、適切なインデントを維持するように注意してください。YAMLについての詳細は [Wikipedia](http://en.wikipedia.org/wiki/yaml) をご覧ください。

`newrelic.yml` ファイルに変更を加える際には、YAMLバリデータを使用して構文が正確であることを確認し、以下のルールに従ってください。
