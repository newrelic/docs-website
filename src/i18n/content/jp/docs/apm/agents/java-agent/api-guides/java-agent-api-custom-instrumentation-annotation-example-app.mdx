...
<CollapserGroup>
  ...
  >
    API コールのこの部分は、設定したカスタム トランザクション名で APM の[<DNT>**Transactions**</DNT>ページ](/docs/apm/applications-menu/monitoring/transactions-dashboard)に表示されるように、 `storeId`値を含むウェブサイトに指示します。 1 つのストアへのリクエストは、同じ集合名で表示されます。
  ...
  <Collapser
    ...
    title="ユーザーIDの記録"
  >
    API コールのこの部分では、ブラウザ トレースを`userId`と結び付けることができるように、追加のメタデータを[ページ読み込みタイミング](/docs/browser/new-relic-browser/page-load-timing-resources/page-load-timing-process)リクエストに挿入します。 また、 `userId`トランザクションのカスタム問題として記録し、それが[状況トレースの懸念詳細に](/docs/apm/transactions/transaction-traces/transaction-traces-trace-details-page)表示されるようにします。 (ページの読み込みタイミングは、リアルユーザー監視または RUM と呼ばれることもあります。)
  ...
  >
    API コールのこの部分では、 `HttpServletResponse`に何を含めるかを定義します。 ページの読み込みタイミングを監視するため[の<InlinePopover type="browser"/>の手動監視](/docs/agents/java-agent/instrumentation/page-load-timing-java)(リアルユーザー監視または RUM とも呼ばれます):
