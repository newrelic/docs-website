...
<CollapserGroup>
  ...
  >
    API コールのこの部分は、設定したカスタム トランザクション名で APM の[<DoNotTranslate>**Transactions**</DoNotTranslate>ページ](/docs/apm/applications-menu/monitoring/transactions-dashboard)に表示されるように、 `storeId`値を含むウェブサイトに指示します。 1 つのストアへのリクエストは、同じ集合名で表示されます。
