...
<CollapserGroup>
  ...
  >
    API 呼び出しのこの部分では、 `HttpServletResponse`に何を含めるかを定義します。[手動計装用<InlinePopover type="browser"/>](/docs/agents/java-agent/instrumentation/page-load-timing-java)ページ読み込みタイミングを監視するには (リアル ユーザー監視または RUM とも呼ばれます):
