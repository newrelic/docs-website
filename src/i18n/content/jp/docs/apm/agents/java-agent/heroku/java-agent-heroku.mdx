...
[Heroku は](https://devcenter.heroku.com/articles/newrelic)、Java を含むさまざまなエージェント言語で Web アプリケーションをホストするためのサービスとしてのプラットフォーム (PaaS) ソリューションです。New Relic を使用すると、 [APM](/docs/apm/new-relic-apm)からのメトリクスを使用して Heroku を拡張できます。 [<InlinePopover type="browser"/>](/docs/browser/new-relic-browser)。
