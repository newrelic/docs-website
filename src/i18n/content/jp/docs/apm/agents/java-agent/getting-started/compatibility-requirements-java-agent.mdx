...
<CollapserGroup>
  ...
  >
    ...
    この表の一部の Java エージェント バージョンはサポートされなくなりましたが、参照のために引き続きリストされています。 サポートされている Java エージェントのバージョンのリストは[、Java エージェント EOL ポリシー](/docs/apm/agents/java-agent/getting-started/java-agent-eol-policy/)にあります。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    * Apache Httpclient 3.1 から 5.x へ
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        [自動インスツルメンテーションを有効にする](/docs/browser/new-relic-browser/installation/install-new-relic-browser-agent#select-apm-app)と、Java エージェントはブラウザの JavaScript エージェントを自動的に挿入します。 ブラウザー インジェクションを有効にすると、 [\[APM 概要\] ページ](/docs/apm/applications-menu/monitoring/apm-overview-page)でブラウザー データを表示し、特定のアプリの APM とブラウザー データをすばやく切り替えることができます。 構成オプションと手動インストルメンテーションについては、を参照してください。 [<InlinePopover type="browser"/>そして Java エージェント](/docs/agents/java-agent/instrumentation/page-load-timing-java)。
      </td>
    </tr>
