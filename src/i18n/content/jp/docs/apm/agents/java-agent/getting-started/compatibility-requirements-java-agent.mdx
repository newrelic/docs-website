...
<CollapserGroup>
  ...
  >
    ...
    イノベーションを継続し、お客様に新しい機能を効率的に提供するために、旧バージョンのJVMのエンジニアリングサポートとテクニカルサポートを終了する必要がある場合があります。以下の表では、**New Relicサポートなし**というカラムは、この状態にあるエージェントのバージョンを指しています。サポートされていないバージョンの場合、旧バージョンのJVMと互換性のあるバージョンのエージェントを引き続き使用できますが、新しい機能と修正は旧バージョンのエージェントには含まれません。最新のエージェントリリースを利用するには、現在サポートされているJVMバージョンにアップグレードすることをお勧めします。
    ...
    * Linux、Windows、macOS 用 Eclipse OpenJ9 バージョン 8～19

    * Linux、Windows、macOS用OpenJDKバージョン8～19

    * Linux、Windows、macOS用AdoptOpenJDKバージョン8～16

    * Linux、Windows、macOS用Temurinバージョン17〜19

    * Linux、Solaris、Windows、およびmacOS用Oracle Hotspot JVMバージョン8～19

    * Linux、Windows、macOS 用の Azul Zulu JVM バージョン8と11

    * Linux、Windows、macOS用のAzul Zulu JVMバージョン8～19

    * Linux、Windows、macOS用のAmazon Coretto JVM バージョン8、11、17〜19

    * Linux、Windows、macOS 用の Alibaba Dragonwell JVMバージョン8と11

      （Java 1.7）[Javaエージェント6.5.x](https://download.newrelic.com/newrelic/java-agent/newrelic-agent/6.5.3/newrelic-java-6.5.3.zip) \[ZIP | 16.8 MB] レガシーエージェントでのみ互換性あり：
...
<CollapserGroup>
  ...
  >
    ...
    * [Kafka Clients](/docs/agents/java-agent/instrumentation/use-kafka-message-queues) 0.11.0.0以降（ディストリビューティッド（分散）トレーシングおよびイベントデータ向け）
