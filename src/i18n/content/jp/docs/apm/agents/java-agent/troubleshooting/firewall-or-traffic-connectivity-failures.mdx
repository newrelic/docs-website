...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        New Relic の Java エージェントは、接続できない場合に大量のメトリック データをキャッシュできます。停止が数分間続いたとしても、一般に、これによってシステム内の情報が失われることはありません。 <InlinePopover type="apm"/>UI。 これらのメッセージは無視しても問題ありません。
      </td>
    </tr>
