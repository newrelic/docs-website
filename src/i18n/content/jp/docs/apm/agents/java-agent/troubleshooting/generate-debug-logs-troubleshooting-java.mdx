...
`newrelic.yml`ファイル内のログ ファイル設定を変更するだけの場合は、サーバーまたは JVM を再起動する必要はありません。ただし、その他の変更を行った場合は、サーバーまたは JVM を再起動する**必要があります**。
...
1. `newrelic.yml`ファイルを編集し、 [`log_level`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-log_level)を`finest`に変更します。これらの行は[**スペース 2 つ**でインデントし](#requirements)てください。
