...
`newrelic.yml` ファイル内のログファイルの設定を変更するだけなら、サーバーや JVM を再起動する必要はありません。しかし、それ以外の変更を行った場合は、 サーバーや JVM を再起動する必要があります。

`newrelic.yml` ファイルでは、必ず元の行のインデントを維持してください。YAML は"スペースセンシティブ" で、適切なインデントが必要です。
...
1. `newrelic.yml` ファイルを編集し、 [`log_level`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#cfg-log_level) を `finest` に変更してください。これらの行は必ず [インデント **2 スペース**](#requirements) を維持してください。
...
2. 十分なロギングデータを生成するために、Web アプリケーションを 5 分程度動かしてみてください。
...
<Callout variant="important">
  `最高級の`
  ログ設定は、非常に冗長なログレベルです。テスト後にディスクの消費量を減らすには、
  `log_level` を元の設定に戻してください。
</Callout>
