...
標準のディストリビューティッド（分散）トレーシング (head ベースサンプリング) を試すだけの場合でも、Infinite Tracing (tail ベースサンプリング) も設定する場合でも、まず標準のトレースを設定する必要があります。 <InlinePopover type="apm" />エージェント インストレーションを実行して、ヘッドベースサンプリングを開始できるようにします。 その後、Infinite Tracing を設定できます。これはオプションですが、推奨されます。
...
<CollapserGroup>
  <Collapser id="java" title="Javaエージェントの互換性">
...
<CollapserGroup>
  <Collapser id="java-config-standard" title="Javaエージェント構成">
