...
データを表示するには、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (サービスを選択) > More views > Realtime Profiling Java**に移動します。
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (サービスの選択) > Realtime Profiling Java**: JVM クラスターのタイムライン ビューには、クラスター全体の JVM の動作が表示されます。
</figcaption>
...
JVM の詳細ページを見つけるには、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM & Services > (サービスを選択) > Realtime Profiling Java > (JVM を選択)**に移動します。
