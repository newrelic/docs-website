...
<img
  title="New Relic Java flame graphのスクリーンショット"
  alt="New Relic Java flame graphのスクリーンショット。"
  src={jfrFlamegraphs}
/> <figcaption>**[one.newrelic.com](one.newrelic.com/) > Explorer> (select service)> Realtime Profiling Java> (select JVM)** ：炎のグラフなど、各JVMの詳細を見ることができます。</figcaption>
