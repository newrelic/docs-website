...
空きヒープ メモリのパーセンテージが`memory_threshold`未満で、ガベージ コレクションに費やされた CPU 時間が`gc_cpu_threshold`を超えると、サーキット ブレーカーが作動します。サーキット ブレーカーが作動すると、エージェントはトランザクション データの収集を停止します。APM UI で報告されるスループットは過小報告され、一定期間トランザクション追跡が表示されません。
