...
<CollapserGroup>
  ...
  <Collapser id="datastore_multihost_preference" title="datastore_multihost_preference">
    ...
    データストアの接続サイクル中に複数のアドレスが検出された場合は、この設定によってエージェントが選択するアドレスが決まります。

    * `NONE`：エージェントは空のアドレスを返します。これはデフォルトの動作です。
    * `FIRST`：最初に検出されたアドレスを使用します。
    * `LAST`：最後に検出されたアドレスを使用します。

    この設定はJavaエージェントバージョン8.25.0以降に存在します。
  </Collapser>
...
<CollapserGroup>
  ...
  <Collapser id="metadata_bypass_proxy" title="metadata_bypass_proxy">
    ...
    この設定をtrueにすると、cloudメタデータエンドポイントへのアクセス時にプロキシをバイパスできます。デフォルトはfalseです。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="ct-use_controller_class_and_method_for_spring_transaction_naming" title="use_controller_class_and_method_for_spring_transaction_naming">
    ...
    有効にすると、すべてのSpringコントローラートランザクションは、リクエストマッピング（例： `/api/v1/customer (POST)`）ではなく、コントローラークラス名とメソッド名（例：`/CustomerController/edit`）を使用して名前が付けられます。

    この設定は`enhanced_spring_transaction_naming`よりも優先され、複雑なURIパターンによるトランザクション名のカーディナリティの問題の発生を防ぐのに役立ちます。

    Spring 4.3.x以降（spring-4.3.0モジュール）およびSpring 6.x以降（spring-6.0.0モジュール）に適用されます。
...
#### ログレベル拒否リスト [#log-level-denylist]

Javaエージェントは、New Relicに転送されるLogEventsの生成から特定のログレベルを除外できます。

これを制御するには、`forwarding`セクションの下にネストされている`log_level_denylist`設定オプションを使用します。この設定では、ログイベントを転送しないログレベルをカンマ区切りの文字列で指定します（例：`log_level_denylist: "info, debug"`）。ログレベル名は大文字と小文字を区別しませんが、それ以外は完全に一致する必要があります。`newrelic.config.application_logging.forwarding.log_level_denylist`プリフィックス化システムプロパティを使用して、この設定を上書きできます。使用可能なオプションは次のみです。
...
Javaエージェントバージョン`9.1.0`以降、 OpenTelemetry Tracing、メトリクス、およびログAPIの互換性が追加されました。New Relicの強力な可視性とOpenTelemetryシグナルを組み合わせることで「両方の長所」を兼ね備えたエクスペリエンスを実現できます。

この機能を使用すると、 OpenTelemetry Tracing、メトリクス、ログAPIをNew Relic JavaエージェントAPIと組み合わせることでアプリケーションを直接カスタム計装できるようになり、また、Javaエージェントとこれらすべてを組み合わせて使い慣れたAPMエクスペリエンスを実現できるようになります。さらに、 OpenTelemetryネイティブまたは独立のライブラリ計装を提供する一部のライブラリを使用して、アプリケーションの可視性を向上させることができます。

次の設定オプションを使用して、どのOpenTelemetryシグナルをキャプチャしてNew Relic APMエクスペリエンスに組み込むかを決定します。
...
<CollapserGroup>
  <Collapser id="cfg-opentelemetry-enabled" title="enabled">
    ...
    各OpenTelemetryシグナルを有効にするには「true」に設定し、すべての OpenTelemetryシグナルを無効にするには「false」に設定します。デフォルトは「false」です。
  ...
  <Collapser id="cfg-opentelemetry-logs-enabled" title="logs.enabled">
    ...
    `opentelemetry.enabled=true`の場合にOpenTelemetry Logsシグナルを有効にするには「true」に設定します。デフォルトは「true」です。
  ...
  <Collapser id="cfg-opentelemetry-metrics-enabled" title="metrics.enabled">
    ...
    `opentelemetry.enabled=true`の場合にOpenTelemetry Metricsのシグナルを有効にするには「true」に設定します。デフォルトは「true」です。
  ...
  <Collapser id="cfg-opentelemetry-metrics-include" title="metrics.include">
    ...
    シグナルを含めるOpenTelemetryメーターのコンマ区切りの文字列（例：「MeterName1, MeterName2」）。デフォルトでは、すべてのメーターが含まれます。これにより、エージェント内のデフォルトのメーター除外設定が上書きされ、再度有効になります。
  ...
  <Collapser id="cfg-opentelemetry-metrics-exclude" title="metrics.exclude">
    ...
    シグナルを除外するOpenTelemetryメーターのコンマ区切りの文字列（例：「MeterName3, MeterName4」）。これは、他のあらゆる設定箇所の「含める/除外する」よりも優先され、列挙したメーターが無効になります。
  ...
  <Collapser id="cfg-opentelemetry-traces-enabled" title="traces.enabled">
    ...
    `opentelemetry.enabled=true`場合にOpenTelemetry Tracesのシグナルを有効にするには「true」に設定します。デフォルトは「true」です。
  ...
  <Collapser id="cfg-opentelemetry-traces-include" title="traces.include">
    ...
    シグナルを含めるOpenTelemetryトレーサーのコンマ区切り文字列（例：「TracerName1, TracerName2」）。デフォルトでは、すべてのトレーサーが含まれます。これにより、エージェント内のデフォルトのトレーサー除外設定が上書きされ、再度有効になります。
  ...
  <Collapser id="cfg-opentelemetry-traces-exclude" title="traces.exclude">
    ...
    シグナルを除外するOpenTelemetryトレーサーのコンマ区切り文字列（例：（例：「TracerName3, TracerName4」）。これは他のすべての包含/除外ソースよりも優先され、リストされているトレーサーを事実上無効にします。
...
エージェントは、スタートアップメインクラスまたはコマンドラインから抽出された実行可能jarファイルに基づいて、任意に有効または無効にすることができます。これは、「JAVA\_TOOL\_OPTIONS」環境変数が存在し、-javaagentフラグが含まれているものの、 Kubernetesコンテナ内など、環境内のすべてのJavaアプリに計装を適用したくない場合に便利です。

<Callout variant="important">
  エージェントのライフサイクルの早い段階でスキップ設定を確認する必要があるため、スキップ設定はシステムプロパティまたは環境変数を介してのみサポートされます。
</Callout>

診断目的で、excludeまたはinclude設定変数のいずれかが定義されている場合、エージェントは抽出されたコマンドラインと解析されたスキップトークンをstdoutに出力します。
...
includeまたはskip設定変数に基づいてエージェントが無効になっている場合、以下がstdoutにダンプされます。
...
<CollapserGroup>
  <Collapser id="cfg-skip-configuration" title="NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS">
    ...
    エージェントが計装すべきではないメインクラス、実行可能jarファイル、またはJavaベースのツール/アプリのカンマ区切りのリスト。例：`NEW_RELIC_STARTUP_JAVA_ARTIFACT_SKIPS=keytool,myapp.jar,IgnoreThisClass`。対応するシステムプロパティは`newrelic.config.startup_java_artifact_skips`です。
  </Collapser>
  ...
  <Collapser id="cfg-include-configuration" title="NEW_RELIC_STARTUP_JAVA_ARTIFACT_INCLUDES">
    ...
    エージェントが計装すべきメインクラス、実行可能jarファイル、またはJavaベースのツール/アプリのカンマ区切りのリスト。例：`NEW_RELIC_STARTUP_JAVA_ARTIFACT_INCLUDES=myapp.jar`。対応するシステムプロパティは`newrelic.config.startup_java_artifact_includes`です。
  </Collapser>
</CollapserGroup>
...
## 一時的なJarファイルの削除 [#temporary-jar-file-removal]

一部の顧客環境では、エージェントの一時jarがJVMの終了時に削除されるようにマークされているにもかかわらず、適切にクリーンアップされません。これはJVMの異常終了が原因であると考えられます。この設定は、起動時に指定された値より古い一時エージェントjarファイルを削除するようにエージェントに指示します。

<Callout variant="important">
  ファイルの削除はエージェントのライフサイクルの早い段階で実行する必要があるため、この設定はシステムプロパティまたは環境変数を介してのみサポートされます。
</Callout>

診断目的で、この機能を有効にすると、次の内容がコンソールに書き込まれます。

```
New Relic Agent: Removing stale temporary agent file jars from [temporaryFolder] older than [X] hour(s)
```

ファイルが削除された場合、次の内容がコンソールに書き込まれます。
...
<CollapserGroup>
  <Collapser id="cfg-tmp-file-deletion-configuration" title="NEW_RELIC_TEMP_JARFILE_AGE_THRESHOLD_HOURS">
    ...
    この値は、一時jarを削除するのに必要な経過時間数です（整数のみ）。デフォルト値は0で、ファイル削除チェックは無効になります。対応するシステムプロパティは`newrelic.config.temp_jarfile_age_threshold_hours`です。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  <Collapser id="tt-exec_call_sql_regex_disabled" title="exec_call_sql_regex_disabled">
    ...
    大きなSQLステートメントの場合、execステートメントやcallステートメントを解析しようとする正規表現の実行にかなりの時間がかかることがあります。この設定をtrueにすると、これらの複雑な正規表現の実行が無効になります。
  ...
  <Collapser id="tt-sql_metadata_comments" title="sql_metadata_comments">
    ...
    New Relicパフォーマンス監視製品がエンティティリンクに使用する、実行されたSQLステートメントの先頭へのコメントの追加を制御します。デフォルトはfalseです。

    生成されるコメントは次のようになります：/*nr\_service\_guid=MTE3NDc2MDB8QVBNfAAAAAxJQEEEEE9OfDI4MTc5NDEIEA*/
  </Collapser>
  ...
  <Collapser id="tt-transaction_threshold" title="transaction_threshold">
    ...
    トランザクションのトレースをいつ行えるかを決定するのに使用する時間閾値。トランザクショントレースの応答時間がこの閾値を超えると、そのトランザクションは[トランザクショントレース](/docs/apm/transactions/transaction-traces/transaction-traces)の対象になります。サンプリングされた場合、そのトランザクショントレースは記録され、 New Relicに送信されます。
