...
Javaエージェントの`7.6.0`リリース以降、[Logs in Context](/docs/logs/logs-context/java-configure-logs-context-all/)のサポートがエージェントに直接追加され、[対応のロギングフレームワーク](/docs/logs/logs-context/java-configure-logs-context-all/#automatic)で簡単に使用できるようになりました。
...
対応のロギングフレームワークを使用していて、エージェントを使用してアプリケーションログをNew Relicに送信する場合は、`forwarding`スタンザの下の設定から制御できます。この設定は、`newrelic.config.application_logging.forwarding`プレフィックス化システムプロパティによって上書きできます。利用可能なオプションは、以下のとおりです。
