...
<CollapserGroup>
  ...
  >
    ...
    まれに、従来の非同期 API を使用して非同期サーブレットまたは Jetty Continuation のライフサイクルを追跡するインストルメンテーションでメモリ リークが発生することがあります。メモリ リークは、一時停止されたものの再開されなかったリークされたトランザクションとして現れます。この構成は、そのようなシナリオの発生を防ぐための回避策として機能します。このようなメモリ リークが発生した場合にのみ、この回避策を有効にすることをお勧めします。
  </Collapser>
...
スタンザにネストされた設定については、スタンザ名を設定名に追加します。例：
...
<CollapserGroup>
  ...
  >
    ...
    この設定を使用して、すべての属性をオンまたはオフにすることができます。<Callout variant="important">セキュリティ上の理由から、 [カスタム インストルメンテーション エディター](/docs/agents/java-agent/custom-instrumentation/custom-instrumentation-editor-instrument-ui#options)を使用したカスタム属性のキャプチャは、デフォルトで`false`に設定されています。</Callout>
  </Collapser>
  ...
  >
    ...
    * `standard` : レポートには標準が使用されます (つまり、OTEL) 属性。この構成をお勧めします。これにより、機能への影響が最小限に抑えられ、属性レポートの取り込みレベルが最低レベルに維持されます。
    * `legacy` : レポートは再導入された HTTP 属性によって実行されます。この構成は現在または将来の機能に影響を与える可能性があります。
    * `both` : これはデフォルト設定であり、レポートは再導入された HTTP 属性と OTEL 属性によって実行されます。この構成では、データの取り込みも増加します。
  </Collapser>

  <dd>
    <Callout variant="important">
      [Java エージェント バージョン 8.8.0](/docs/release-notes/agent-release-notes/java-release-notes/java-agent-880)以降で利用可能です。デフォルト構成`both`では、データの取り込みが増加します。データ取り込みの増加を避けるには、 `http_attribute_mode` `legacy`または`standard`に設定してデフォルトをオーバーライドする必要があります。
    </Callout>
  </dd>
</CollapserGroup>
...
* `NEW_RELIC_METADATA_RELEASE_TAG `リリース タグを含むイベント データ (v0.1.209 など) に`tags.releaseTag`を作成します。またはリリース-209)。
* `NEW_RELIC_METADATA_COMMIT` コミット SHA を含むイベント データに`tags.commit`を作成します。sha 全体を使用することも、最初の 7 文字だけを使用することもできます (例: 734713b)。

エラー受信箱の今後のリリースでは、ソフトウェアのどのバージョンでエラーが発生しているかを自動的に追跡します。バージョン データは[CodeStream](/docs/codestream/how-use-codestream/performance-monitoring/#buildsha)に表示されます。
...
New Relic Securityエージェント設定は`security`スタンザで設定され、`newrelic.config.security`プリフィックス化システムプロパティを使用して[上書き](#System_Properties)できます。オプションに含まれるもの
