...
<CollapserGroup>
  ...
  <Collapser id="event_ingest_uri" title="event_ingest_uri">
    ...
    一部のエージェント機能で使用されるイベント取り込み用URI。デフォルトでは、USリージョンの本番環境向け取り込み用エンドポイントに設定されます。イベントの送信先を別の取り込み用エンドポイントに手動で設定することもできます。例：
  ...
  <Collapser id="metric_ingest_uri" title="metric_ingest_uri">
    ...
    一部のエージェント機能で使用されるメトリクス取り込み用URI。デフォルトでは、USリージョンの本番環境向け取り込み用エンドポイントに設定されます。メトリクスの送信先を別の取り込み用エンドポイントに手動で設定することもできます。例：
