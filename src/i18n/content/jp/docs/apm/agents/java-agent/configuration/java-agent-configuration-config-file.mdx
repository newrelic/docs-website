...
リアルタイムプロファイリングは、エージェントYAML内の`jfr`セクションで設定できます。この場合、`newrelic.config.jfr.`でプリフィックス化された[システムプロパティ](#System_Properties)、または`NEW_RELIC_JFR_`でプリフィックス化された[環境変数](#Environment_Variables)を使用します。
