...
コードレベルのメトリクスは、コードがメソッドレベルでどのように機能しているかについて詳細な洞察を提供します。自動的にまたは@Traceアノテーションによってインストゥルメントされた各メソッドについて、過去30分間のレスポンスタイム、スループット、およびエラー率が表示されます。コードレベルのメトリクスの詳細については、[CodeStreamによるパフォーマンス監視](/docs/codestream/how-use-codestream/performance-monitoring)を参照してください。
