...
Javaエージェントの`7.6.0`リリース以降、[Logs in context](/docs/logs/logs-context/java-configure-logs-context-all/)のサポートがエージェントに直接追加され、[サポート対象のロギングフレームワーク](/docs/logs/logs-context/java-configure-logs-context-all/#automatic)で簡単に使用できるようになりました。
