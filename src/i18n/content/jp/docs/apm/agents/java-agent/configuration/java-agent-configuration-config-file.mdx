...
`newrelic.yml`ファイルには、環境ごとに個別のセクションがあります。
...
New Relic は、 `common`セクションの設定をこれらの各環境に適用します。 [`newrelic.environment`](#newrelic-environment)システム プロパティを環境名に設定することで、他の環境をデフォルトとして選択できます。
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        * ファイルの同じセクション内のデータには同じレベルのインデントを使用します。
        * サブセクションをさらに 2 つのスペースでインデントします。
      </td>
    </tr>
...
これらのオプションは`common`セクションで設定します。 これらのオプションのいずれかを[オーバーライドする](#System_Properties)には、接頭辞`newrelic.config`の付いたシステム プロパティを使用します。
...
セクションにネストされた設定の場合は、設定名の前にセクション名を追加します。 例えば：
...
セクションにネストされた設定の場合は、設定名の前にセクション名を追加します。 たとえば、transaction_tracer セクションの`enabled`設定のシステム プロパティは`newrelic.config.transaction_tracer.enabled`です。

構成設定をオーバーライドすることに加えて、エージェントは次のシステム プロパティを認識します。
...
これらのオプションを設定するには、 `attributes`セクションを使用します。 それらを[オーバーライドする](#System_Properties)には、接頭辞`newrelic.config.attributes`の付いたシステム プロパティを使用します。

属性は、トランザクション追跡、追跡されたエラー、 <InlinePopover type="browser"/>、およびトランザクション イベント。 各宛先の下に`attribute`セクションがあります。 詳細については、 [「Java エージェントの属性」](/docs/java/java-agent-attributes) 、 [「属性の有効化と無効化」](/docs/java/enabling-and-disabling-attributes) 、および[「属性の例」](/docs/java/attribute-examples)を参照してください。
...
これらのオプションは`common`セクションで直接設定され、接頭辞付きのシステム プロパティを使用して[オーバーライド](#System_Properties)できます。
...
これらのオプションは`browser_monitoring`セクションで設定され、接頭辞`newrelic.config.browser_monitoring`の付いたシステム プロパティを使用して[オーバーライド](#System_Properties)できます。
...
サーキット ブレーカーをカスタマイズする場合は、それを`common`セクションに追加します。
...
`utilization`セクションでクラウド プラットフォームの使用率設定を設定します。これは、接頭辞`newrelic.config.utilization`の付いたシステム プロパティでオーバーライドできます。
...
`code_level_metrics`セクションでコードレベルのメトリクス オプションを設定します。 これらは、接頭辞`newrelic.config.code_level_metrics`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
`cross_application_tracer`セクションでクロスアプリケーション トレース オプションを設定します。 これらの設定は、接頭辞`newrelic.config.cross_application_tracer`の付いたシステム プロパティで[上書き](#System_Properties)できます。
...
`custom_insights_events`セクションでカスタム イベント設定を設定します。 これらの設定は、接頭辞`newrelic.config.custom_insights_events`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
<Callout variant="important">
  ...
  エージェントバージョン4.1.0の場合 以上の場合、YAML 設定ではネストされたセクションの書式設定が使用されます。
...
<CollapserGroup>
  ...
  >
    ...
    属性値の最大サイズ。それを超えると値は切り捨てられます。 デフォルトは`255`バイト (UTF-8 文字セット エンコードの場合) で、最大制限は`4095`です。

    この構成は Java エージェント バージョン 8.9.0 以降に存在します。
  </Collapser>
...
## クラストランス [#Class_Transformer]

`class_transformer`セクションでインストルメンテーション関連の設定を行います。 これらの設定は、接頭辞`newrelic.config.class_transformer`の付いた[システム プロパティ](#System_Properties)または接頭辞`NEW_RELIC_CLASS_TRANSFORMER_`の付いた[環境変数](#Environment_Variables)でオーバーライドできます。

<CollapserGroup>
  <Collapser
    id="ct-enhanced_spring_transaction_naming"
    title="強化された_spring_transaction_naming"
  >
    <table>
      <tbody>
        <tr>
    ...
    有効にすると、拡張された Spring トランザクションの名前付けにより、Spring コントローラーから発生するすべてのトランザクションに、定義されたルートと HTTP メソッドの後に名前が付けられます。 例: `/customer/v1/edit (POST)` 。 これには、WebMVC 関連のアノテーション ( `@RestController` 、 `@Controller` 、 `@RequestMapping`など) を使用してインターフェイス/クラスを実装または拡張するコントローラーが含まれます。

    デフォルトでは、これは`false`に構成されており、コントローラーのクラス名とメソッドに基づいてこれらのタイプのコントローラーのトランザクションに名前が付けられます。 例えば; `CustomerController/edit` 。 これは、以前のエージェント バージョンから引き継がれた命名ロジックです。 実際のクラスに存在するすべての関連アノテーションを備えた「標準」コントローラーでも、ルートと HTTP メソッドに基づいて名前が付けられます。
...
構成ファイルの`distributed_tracing`セクションでこれを手動でオーバーライドできます。 接頭辞付きのシステム プロパティ ( `newrelic.config.distributed_tracing` ) または環境変数 ( `NEW_RELIC_DISTRIBUTED_TRACING_ENABLED` ) を使用してこれをオーバーライドすることもできます。 以下の例を参照してください。
...
`error_collector`セクションでエラー コレクター設定を設定します。 特に明記されていない限り、これらの設定は、接頭辞`newrelic.config.error_collector`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。 エラーコレクターは、キャッチされなかった例外に関する情報をキャプチャし、表示するために New Relic に送信します。
    ...
    <table>
      <tbody>
        <tr>
          ...
          <td>
            完全修飾された`class_name`文字列のリストを含むセクション
          </td>
        </tr>
    ...
    <table>
      <tbody>
        <tr>
          ...
          <td>
            完全修飾された`class_name`とエラー クラスごとの`messages`のリストを含むセクション
          </td>
        </tr>
...
`external_tracer`セクションで外部トレース オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.external_tracer`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
`process_host`セクションでホスト名構成オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.process_host`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。

次のプロパティを使用して、UI に表示されるホスト名を構成します。
...
<CollapserGroup>
  ...
  >
    ...
    デフォルトでは、 [@TraceLambda](/docs/apm/agents/java-agent/api-guides/java-agent-api-instrument-using-annotation/#tracelambda)アノテーションは無効になっています。 ラムダ メソッドをトレースするためにアノテーション機能を使用するには、次のように機能を明示的に有効にする必要があります。
...
`jar_collector`セクションで jar コレクションの構成を設定します。 これらの構成オプションは、接頭辞`newrelic.config.jar_collector`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。 オプションには次のものが含まれます。
...
エージェント YAML の`jfr`セクションでリアルタイム プロファイリングを構成し、[システム プロパティ](#System_Properties)に接頭辞として`newrelic.config.jfr.`を付けるか、[環境変数](#Environment_Variables)に接頭辞として`NEW_RELIC_JFR_`を付けます。
...
これらのオプションを設定するには、 `jmx`セクションを使用します。 それらを[オーバーライドする](#System_Properties)には、接頭辞`newrelic.config.jmx`の付いたシステム プロパティを使用します。
...
`application_logging`セクションのコンテキスト構成でログを設定します。 これは、接頭辞`newrelic.config.application_logging`の付いたシステム プロパティで[オーバーライド](/docs/apm/agents/java-agent/configuration/java-agent-configuration-config-file#System_Properties)できます。 使用可能な唯一のオプションは次のとおりです。
...
サポートされているロギング フレームワークを使用していて、エージェントを使用してアプリケーション ログを New Relic に送信したい場合は、 `forwarding`セクションの設定を通じて制御できます。 これらの設定は、接頭辞`newrelic.config.application_logging.forwarding`の付いたシステム プロパティによってオーバーライドできます。 利用可能なオプションは次のとおりです。
...
Java エージェントは、コンテキスト データ (logback/slf4j のマップされた診断コンテキスト、log4j2 の ThreadContext) をキャプチャし、その内容を New Relic に転送されるログに属性として追加できます。 これは、 `forwarding`セクションの下にネストされている`context_data`セクションの設定を通じて制御できます。 これらの設定は、接頭辞`newrelic.config.application_logging.forwarding.context_data`の付いたシステム プロパティによってオーバーライドできます。 利用可能なオプションは次のとおりです。
...
Java エージェントでは、個々のログ行を装飾して送信できることに加えて、ロギング メトリック データをキャプチャできます。 このデータは、New Relic UI のログ チャートで見つけることができます。 `metrics`セクションでログ指標設定を構成できます。 これらの設定は、接頭辞`newrelic.config.application_logging.metrics`の付いたシステム プロパティでオーバーライドできます。 使用可能な唯一のオプションは次のとおりです。
...
`local_decorating`セクションでログ装飾設定を構成できます。 これを使用して、ローカル ログ行を装飾する方法を制御できます。 これらの設定は、接頭辞`newrelic.config.application_logging.local_decorating`の付いたシステム プロパティでオーバーライドできます。 使用可能な唯一のオプションは次のとおりです。
...
`message_tracer`セクションでメッセージ トレーサ オプションを設定します。 これらの設定は、接頭辞`newrelic.config.message_tracer`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
New Relic Security エージェントの構成は、 `security`セクションで設定できます。 これらの設定は、接頭辞`newrelic.config.security`の付いたシステム プロパティで[上書き](#System_Properties)できます。 オプションには次のものが含まれます。
...
[スパン イベントは](/docs/apm/distributed-tracing/ui-data/span-event)[分散トレース](#distributed-tracing)に対してレポートされます。 スパン イベントをレポートするには、分散トレースを有効にする必要があります。

`span_events`セクションでスパン イベントの構成を設定します。 これらの設定は、接頭辞`newrelic.config.span_events`の付いたシステム プロパティで[上書き](#System_Properties)できます。 オプションには次のものが含まれます。
...
`strip_exception_messages`セクションで除外例外オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.strip_exception_messages`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。 Java 例外メッセージを New Relic に報告するかどうかを制御するには、この設定を有効にします。
...
`thread_profiler`セクションでスレッド プロファイラ オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.thread_profiler`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
`transaction_events`セクションでトランザクション イベント オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.transaction_events`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。

トランザクション イベント データは、UI にヒストグラムとパーセンタイルを表示するために使用されます。

<Callout variant="important">
  以前は、このセクションは`analytics_events`と呼ばれていました。 構成ファイルがまだ`analytics_events`を使用している場合は、 `transaction_events`を使用するようにエージェントを更新します。
</Callout>
...
`transaction_segments`セクションでトランザクション セグメント オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.transaction_segments`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
...
`transaction_tracer`セクションでトランザクション トレーサ オプションを設定します。 これらのオプションは、接頭辞`newrelic.config.transaction_tracer`の付いたシステム プロパティで[オーバーライド](#System_Properties)できます。
