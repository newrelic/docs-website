...
<CollapserGroup>
  ...
  >
    ...
    [コードレベルのメトリクスは、](/docs/codestream/observability/code-level-metrics)エージェント バージョン 7.10.0 ではデフォルトで無効になっています。オンにするには、これを true に設定します。エージェント バージョン 7.11.0 以降、デフォルトは上記のように`true`です。
  </Collapser>
</CollapserGroup>
...
## Kafka メッセージ キュー [#kafka-message-queues]

Kafka インストルメンテーション構成の詳細については、 [「Instrument Kafka メッセージ キュー」](docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/)ページを参照してください。

* \[Kafka イベント コレクション(/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#collect-kafka-events)
* [Kafka ノードのメトリクス](/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#kafka-node-metrics)
* [Kafka 構成イベント](/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#kafka-config)
* [Kafka ストリーム トランザクション](/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#collect-kafka-streams-transactions)
* [Kafka 接続トランザクション](/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#collect-kafka-connect-transactions)
* [Kafka 分散トレース](/docs/apm/agents/java-agent/instrumentation/java-agent-instrument-kafka-message-queues/#collect-kafka-distributed-traces)
...
Java エージェント バージョン`8.7.0`以降、エージェントは`SlowTransaction`イベントを介して低速トランザクションを検出できるようになりました。これらのイベントは、トランザクションの長さが特定のしきい値を超えたときに記録されます。デフォルトのしきい値は 600000 ミリ秒 (10 分) です。
