...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        必須。 記録するイベント タイプの名前。 文字列が 255 文字を超えると、API 呼び出しがNew Relicに送信されません。 名前には英数字、アンダースコア`_` 、コロン`:`のみを含めることができます。 イベント タイプ名に関する追加の制限については、 [「予約語」](/docs/data-apis/custom-data/custom-events/data-requirements-limits-custom-event-data/#reserved-words)を参照してください。
      </td>
    </tr>
