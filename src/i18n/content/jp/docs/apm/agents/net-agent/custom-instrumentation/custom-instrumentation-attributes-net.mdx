...
* `Main()`などのトップレベルのメソッドをインストゥルメントすることは避けてください。これらのメソッドはアプリケーションが終了するまで完了しないため、データがNew Relicに送信されなくなる可能性があります。

## トランザクション内で呼び出されたトランザクション [#tx-vs-trace]

`[Transaction]`属性で装飾されたメソッドで新規トランザクションを作成するのは、既に存在するものがない場合に限られます。`[Transaction]`で装飾されたメソッドが、以前開始されたトランザクション<DNT>**within**</DNT>から呼び出された場合、`[Trace]`属性として処理され、既存トランザクションに関するより詳しい情報を提供します。

<CollapserGroup>
  <Collapser
    id="example-tx-vs-trace"
    title={<>例：すでに開始されたトランザクションで<InlineCode>
      Transaction
    </InlineCode>を呼び出す</>
    }
  >
...
* [属性ベースのカスタムインストゥルメンテーション関連問題のトラブルシューティング](https://support.newrelic.com/s/hubtopic/aAX8W0000008a5g/relic-solution-troubleshooting-attributebased-custom-instrumentation-issues)
* [.NETエージェントのログファイルから、カスタムインストゥルメンテーション トレーサーファクトリを構築する](https://knowledge.newrelic.com/s/article/Building-Custom-Instrumentation-Tracer-Factories-From-dot-NET-Agent)
