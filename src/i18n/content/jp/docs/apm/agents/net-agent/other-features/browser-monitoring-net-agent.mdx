...
<CollapserGroup>
  ...
  >
    自動インストゥルメンテーションを完全に無効にするには、 <DoNotTranslate>**`newrelic.config`**</DoNotTranslate>を編集し、 [`browserMonitoring` `autoInstrument`フラグ](/docs/agents/net-agent/installation-configuration/net-agent-configuration#browsermon-autoInstrument)を`false`に設定します。
...
1. <DoNotTranslate>**`newrelic.config`**</DoNotTranslate>を編集し、 [`browserMonitoring` `autoInstrument`フラグ](/docs/agents/net-agent/installation-configuration/net-agent-configuration#browsermon-autoInstrument)を`true`に設定して、自動インストゥルメンテーションがグローバルに有効になっていることを確認します。
