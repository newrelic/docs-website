...
<CollapserGroup>
  ...
  >
    対象とする .NET フレームワークのバージョンを指定する場合は、厳密な実行時チェックが課され、互換性の問題が発生する可能性があることに注意してください。 <DNT>**Recommendation:**</DNT>可能であれば、このオプションを使用する代わりに[新しいパイプラインを使用するように .NET に指示します](#use-new-pipeline)。
