...
<CollapserGroup>
  ...
  >
    IIS ホスト[WCF サービスは、](/docs/agents/net-agent/instrumentation/instrumenting-wcf-applications) <DNT>**WCF**</DNT>セグメントを`ExecuteRequestHandler`セグメントの下に適切にネストしません。 報告される[合計時間](/docs/data-analysis/user-interface-functions/response-time#response-time-total-time)は正確ですが、2 つのセグメントは trace 内では兄弟のように見えます。
  </Collapser>
