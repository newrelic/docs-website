...
これらの指標を表示するには、カスタム[ダッシュボード](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards)を作成します。または、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > More views の**下にある**Metric Explorer**を使用することもできます。
