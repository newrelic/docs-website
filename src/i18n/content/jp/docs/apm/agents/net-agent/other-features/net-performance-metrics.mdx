...
これらのメトリクスは、 **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services**で確認できます。アプリケーションを選択し、 **More views > Dotnet VMs**に移動します。
...
* one.newrelic.com で **Metric Explorer** を使用し **[、\[すべての機能\]](https://one.newrelic.com/all-capabilities) > \[APM とサービス]** > アプリケーションを選択し、 **\[その他のビュー]**に移動します。
