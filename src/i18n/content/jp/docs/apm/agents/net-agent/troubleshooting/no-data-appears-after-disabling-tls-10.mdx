...
<CollapserGroup>
  <Collapser id="windows-registry" title="Step 1.WindowsレジストリでTLSプロトコルを有効にします。">
  ...
  <Collapser id="windows-registry" title="Step 2..NETのデフォルトプロトコルをオンにする">
    ...
    <CollapserGroup>
      <Collapser id="strongcrypto" title="Windowsレジストリの強力な暗号プロパティを有効にする">
        <Callout variant="caution">
          以下の手順を慎重に実行してください。 レジストリを誤って変更すると、深刻な問題が発生する可能性があります。 推奨事項: レジストリを変更する前に、バックアップを作成してください。
        </Callout>
      ...
      <Collapser id="app-code" title="アプリのコードにプロトコルを含める">
