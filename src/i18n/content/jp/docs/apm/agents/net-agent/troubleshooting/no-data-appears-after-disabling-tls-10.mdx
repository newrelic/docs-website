...
`"Enabled"`が`0`に設定され、 `"DisabledByDefault"`が`1`に設定されている場合、TLS 1.0 は無効になります。
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="caution">
      以下の手順を慎重に実行してください。 レジストリを誤って変更すると、深刻な問題が発生する可能性があります。 推奨事項: レジストリを変更する前に、バックアップを作成してください。
    </Callout>

    Windows レジストリを TLS 1.2 に更新する方法の例を次に示します。 サーバーがクライアントとして New Relic に接続するため、 <DNT>**Client**</DNT>ロールに対して TLS を有効にする必要があります。
  ...
  >
    ...
    <CollapserGroup>
      ...
      >
        ...
        `SchUseStrongCrypto`値を .NET Framework レジストリ キーに追加すると、すべての .NET アプリで TLS 1.1 または 1.2 を使用できるようになります。32 ビットと 64 ビットの両方の .NET アプリケーションが TLS 1.1/1.2 を使用できるようにするには、両方のレジストリ キーを変更する必要があります。
