...
[New Relic の Azure 用 .NET エージェントをインストールした](/install/dotnet/?deployment=azure)後、次の 1 つ以上の問題が発生します。
...
* Web アプリの再起動を含め、[インストールと実行の手順が実行されている](/install/dotnet/?deployment=azure)ことを確認します。
* .NET エージェントが正しく動作するには[、アプリケーション サイトを無効にしてください (](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/) <DNT>**must**</DNT> )。
* アプリ<DNT>**must**</DNT>は、.NET エージェントを起動するためのトラフィックを取得しています。
...
4. アプリケーションが実行されているプロセス ID (PID) をメモしておくと、そのプロセスに対してログが作成されているかどうかを確認できます。 PID は、Kudu の組み込み<DNT>**Process Explorer**</DNT>を使用して見つけることができます。
...
2. <DNT>**Process Explorer**</DNT>で、監視しようとしているプロセスを見つけて、PID 列の番号を書き留めます。 Web アプリケーションを監視しようとしている場合、プロセス名は SCM ラベルのない`w3wp.exe`になる可能性があり、これは Kudu プロセス自体を参照します。 エージェントは SCM (Kudu) プロセスを監視すべきではありません。
3. アプリケーションのプロセスを見つけたら、 `Properties` ボタンをクリックします。
4. <DNT>**Modules**</DNT>タブをクリックし、 `NewRelic.Profiler.dll`を探します。この DLL は、エージェントがアプリケーションを監視するために必要です。 見つからない場合は、正しい環境変数が存在すること ([手順 3](#step-three)で詳しく説明します) と、 [Microsoft アプリケーション インサイトが無効になっていること](/docs/agents/net-agent/azure-troubleshooting/net-profiler-conflict-microsoft-application-insights/)を確認してください。
...
1. <DNT>**Process Explorer**</DNT>でアプリケーションのプロセスを見つけて、 `Properties`ボタンをクリックします。
2. <DNT>**Environment**</DNT>タブを選択します。
...
<CollapserGroup>
  <Collapser id="env-var-framework-extension" title=".NET Framework: New Relic Azure Site Extension 経由でインストールするときに環境変数を確認する">
    Kudu の<DNT>**Process Explorer**</DNT>の<DNT>**Environment**</DNT>タブに表示される環境変数は次のようになります。
  ...
  <Collapser id="env-var-framework-nuget" title=".NET Framework: NuGet 経由でインストールするときに環境変数を確認する">
    NuGet パッケージの場合、環境変数[をアプリケーション設定として追加する必要があります](/install/dotnet/?deployment=azure&azure=azurenugetframework)。
    ...
    環境変数に変更を加えた場合は、Azure Portal からアプリケーションを再起動してください。 次に、変更が成功したことを確認するために、 <DNT>**Process Explorer**</DNT>の新しいプロセス PID を検査します。
  </Collapser>

  <Collapser id="env-var-core-windows-extension" title=".NET Core (Windows): New Relic Azure Site Extension の使用時に環境変数を確認します">
    Kudu の<DNT>**Process Explorer**</DNT>の<DNT>**Environment**</DNT>タブに表示される環境変数は次のようになります。
  ...
  <Collapser id="env-var-core-windows-nuget" title=".NET Core (Windows): NuGet パッケージの使用時に環境変数を確認する">
    NuGet パッケージの場合、環境変数[をアプリケーション設定として追加する必要があります](/install/dotnet/?deployment=azure&azure=azureframeworkcore)。 Kudu の<DNT>**Process Explorer**</DNT>の<DNT>**Environment**</DNT>タブに表示される環境変数は次のようになります。
  ...
  <Collapser id="env-var-core" title=".NET Core (Linux): NuGet パッケージの使用時に環境変数を確認する">
    NuGet パッケージの場合、環境変数[をアプリケーション設定として追加する必要があります](/install/dotnet/?deployment=azure&azure=azureframeworkcore)。
    ...
    `newrelic.config`ファイルの<DNT>**log**</DNT>ノードにディレクトリ属性が含まれていることを確認してください。 Visual Studio で NuGet によってプロジェクトに追加された`newrelic.config`ファイルを編集できない場合は、アプリケーション内にこのファイルのローカル コピーを作成する必要があります。
...
* .NET Core および .NET フレームワーク用に自動的にインストゥルメントされたフレームワークについては、[互換性ページ](/docs/apm/agents/net-agent/getting-started/net-agent-compatibility-requirements/)を参照してください。
