...
<Callout variant="important">
  **IIS**の場合： `newrelic.config`または`app.config`ファイルを変更した後、管理コマンドプロンプトから`IISRESET`を実行します。ログレベルの調整にはリセットは必要ありません。
</Callout>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        * `appsettings.json` の `NewRelic.AgentEnabled` 設定により、 **エージェントが有効になります**。
        * `web.config` または `app.config` ファイル内の `NewRelic.AgentEnabled` 設定は **無視されます**。
      </td>
    </tr>
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_LICENSE_KEY`環境変数を設定します。
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_SEND_DATA_ON_EXIT`環境変数を設定します。
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_SEND_DATA_ON_EXIT_THRESHOLD_MS`環境変数を設定します。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_LOG`環境変数を設定します。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_APP_NAME`環境変数を設定します。
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_DISABLE_SAMPLERS`環境変数を設定します。
...
APM UI のデフォルトのホスト名ラベルが有用でない場合、New Relic UI でその名前を表示名で装飾することができます。アプリケーションのプロセスが再起動され、.NET エージェントが再びレポートすると、表示名が **サーバー** のドロップダウンリストに表示されます。このホスト名の設定は、 [アプリケーションの **Summary** ページ](/docs/apm/applications-menu/monitoring/apm-overview-page-view-transaction-apdex-usage-data) のホストリストには影響しません。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_HIGH_SECURITY`環境変数を設定します。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`MAX_TRANSACTION_SAMPLES_STORED`環境変数を設定します。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`MAX_EVENT_SAMPLES_STORED`環境変数を設定します。
...
`browserMonitoring`要素は`configuration`要素の子です。 `browserMonitoring` .NETアプリケーションでブラウザの監視を構成します。ブラウザは、エンドユーザーのパフォーマンスエクスペリエンスに関する洞察を提供します。これは、各ページのヘッダーとフッターに少量のJavaScriptコードを挿入することにより、ユーザーのブラウザーがWebページをダウンロードしてレンダリングするのにかかる時間を測定することで実現されます。
...
<CollapserGroup>
  ...
  >
    ...
    デフォルト値は`apdex_f`で、これはしきい値をアプリケーションの**apdex_t**値の4倍に設定します。 apdex_tの詳細については、 **Apdex**を参照して[ください](/docs/site/apdex-measuring-user-satisfaction)。
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_DISTRIBUTED_TRACING_ENABLED`環境変数を介して分散トレースを有効にします。
...
<Callout variant="important">
  クロスアプリケーション トレースは [、エージェント v9.0.0 の](/docs/release-notes/agent-release-notes/net-release-notes/net-agent-9000/) 時点で非推奨となり、デフォルトで無効になっています。エージェントの v9 以降で CAT を使用するには、 `crossApplicationTracer.enabled = true` と `distributedTracing.enabled = false`の両方を設定する必要があります。 [分散トレース](/docs/understand-dependencies/distributed-tracing/get-started/introduction-distributed-tracing) を有効にすると、 [クロスアプリケーション トレース](#cross_application_tracer)が無効になります。
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    または、アプリケーションの環境で`NEW_RELIC_ALLOW_ALL_HEADERS`環境変数を設定します。
...
2. ログ転送：有効にすると、エージェントはログデータをキャプチャしてNewRelicに送信します。
   ...
   * コンテキスト データ (カスタム属性): 有効にすると、エージェントはカスタム ログ属性をキャプチャして転送します。 `include` 要素と `exclude` 要素は、他のエージェント属性設定と [同じルール](/docs/apm/agents/net-agent/attributes/enable-disable-attributes-net/#attruls) に従って、含めるか除外する属性名のカンマ区切りのリストです。デフォルトでは両方とも空であるため、すべてのログ コンテキスト データがキャプチャされて転送されます。
...
<CollapserGroup>
  ...
  >
    `forwarding`要素の子であるこのサブ要素を使用して、エージェント バージョン 10.4.0 以降でロギング コンテキスト データ（カスタム属性）のキャプチャと転送を設定します。
...
<Callout variant="important">
  [ASP.NET Core アプリ](http://asp.net/)の場合、 `ASPNETCORE_ENVIRONMENT` 変数を設定すると、.NET エージェントは `appsettings.{environment}.json` から読み取ります。
</Callout>