...
`<traceIdRatioBased>`要素には必須の`ratio`属性があり、これはサンプリングするトレースの割合を指定する`0.0`から`1.0`までの値です。
...
### Kafkaメトリクス [#kafka\_metrics][#kafka_metrics]
...
<CollapserGroup>
  <Collapser id="kafka-metrics-debug-enabled" title="debugEnabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    または、 `NEW_RELIC_KAFKA_METRICS_DEBUG_ENABLED`環境変数を使用します。
  </Collapser>

  <Collapser id="kafka-metrics-interval" title="インターバル">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    または、 `NEW_RELIC_KAFKA_METRICS_INTERVAL`環境変数を使用します。
  </Collapser>

  <Collapser id="kafka-metrics-cluster-metrics-enabled" title="clusterMetricsEnabled">
    <table>
      <tbody>
        <tr>
          <th>
    ...
    または、 `NEW_RELIC_KAFKA_METRICS_CLUSTER_METRICS_ENABLED`環境変数を使用します。
  </Collapser>
</CollapserGroup>
