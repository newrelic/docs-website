...
`browserMonitoring`要素は`configuration`要素の子です。`browserMonitoring`が構成します<InlinePopover type="browser"/> .NET アプリケーション内で。ブラウザを使用すると、エンド ユーザーのパフォーマンス エクスペリエンスを把握できます。これは、各ページのヘッダーとフッターに少量の JavaScript コードを挿入することにより、ユーザーのブラウザーが Web ページをダウンロードしてレンダリングするのにかかる時間を測定することで実現されます。
