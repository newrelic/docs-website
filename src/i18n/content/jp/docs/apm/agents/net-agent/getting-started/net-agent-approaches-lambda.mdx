...
以下のAWS Lambda関数のタイプは、追加の設定なしで自動的に検出され、インストゥルメントされます（エージェントバージョン10.29.0現在）：

* `Amazon.Lambda.AspNetCoreServer.APIGatewayProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.APIGatewayHttpApiV2ProxyFunction`
* `Amazon.Lambda.AspNetCoreServer.ApplicationLoadBalancerFunction`

`NEW_RELIC_LAMBDA_HANDLER`環境変数を完全修飾ハンドラーパス（たとえば、`MyAssembly::MyNamespace.MyClass::MyMethod`）に設定すると、その他のハンドラータイプ（SQS、SNS、Kinesis、S3、DynamoDBトリガー、およびカスタムハンドラーなど）もインストゥルメントされます。これには、`ILambdaContext`パラメーターを受け入れる任意のハンドラーメソッドが含まれます。
...
* SQSからのインバウンドのディストリビューティッド（分散）トレーシングでは、アップストリームのプロデューサーがW3Cトレースヘッダー（`traceparent`、`tracestate`）をSQSメッセージ属性として注入する必要があります。エージェントは、存在する場合にこれらを自動的に読み取ります。
