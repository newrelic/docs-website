...
プロファイラーの競合を回避するには、他のプロファイラーを環境から完全に削除してから、IIS のレジストリキーまたはシステム環境変数が復元されていることを確認してください。

**例外：** Microsoft 社の System Center Operations Manager（SCOM）とのコンフリクトについては、SCOM 専用の [トラブルシューティング手順に従ってください](/docs/agents/net-agent/troubleshooting/resolve-net-scom-conflicts) 。
