...
<Callout variant="important">
  エージェントは Webassembly コードを計測できないため、この API は Blazor Webassembly ではサポートされていません。次の例は、Blazor サーバー アプリケーション専用です。ブラウザー エージェントを Blazor Webassembly ページに追加するには、 [コピーと貼り付けの方法](https://docs.newrelic.com/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#copy-paste)を使用します。
</Callout>
