...
正しい変数が設定されていて、監視しようとしているアプリケーションがそれらにアクセスできる場合、アプリ (エージェント??) が .NET Framework か .NET Core かによって、特定の環境変数のセットが表示されます。
...
<CollapserGroup>
  ...
  >
    ...
    * `COR_ENABLE_PROFILING` このプロセスに対してプロファイリングを有効にする必要があることを .NET CLR に通知します。.NET Framework プロファイラー (.NET エージェントを**含む**) が機能するには、 `1`に設定する必要があります。
    * `COR_PROFILER` プロセスにロードするプロファイラーとして New Relic プロファイラーを識別します。Process Explorer に表示される CLSID が上記のものと一致しない場合は、システムに複数のプロファイラーがインストールされており、プロセスを監視するために別のプロファイラーが現在構成されています。これは、[プロファイラーの競合](/docs/agents/net-agent/troubleshooting/profiler-conflicts)と呼ばれます。
    * `NEWRELIC_INSTALL_PATH` 監視に必要な New Relic ファイルの場所をプロファイラーに伝えます。エージェントをデフォルト パスにインストールした場合、パスは上記のエントリと一致する必要があります。

    環境変数が上記の内容と一致しない場合は、.NET エージェント インストーラーを再実行し、プロンプトが表示されたら`Repair`を選択してデフォルトに戻すことができます。
  ...
  >
    ...
    * `CORECLR_ENABLE_PROFILING` このプロセスに対してプロファイリングを有効にする必要があることを .NET Core ランタイムに伝えます。.NET Core プロファイラー (.NET エージェントを**含む**) が機能するには、 `1`に設定する必要があります。
    * `CORECLR_PROFILER` プロセスにロードするプロファイラーとして New Relic プロファイラーを識別します。Process Explorer に表示される CLSID が上記のものと一致しない場合は、システムに複数のプロファイラーがインストールされており、プロセスを監視するために別のプロファイラーが現在構成されています。これは、[プロファイラーの競合](/docs/agents/net-agent/troubleshooting/profiler-conflicts)と呼ばれます。
    * `CORECLR_NEWRELIC_HOME` .NET Core エージェントがシステムにインストールされている場所へのパスです。
    * `CORECLR_PROFILER_PATH` システム上のプロファイラー dll へのパスです。
...
上記の手順で問題が解決しない場合は、サポートに連絡するか[、エクスプローラー ハブ フォーラム](https://discuss.newrelic.com/tag/dotnetagent)でサポートを依頼することをお勧めします。