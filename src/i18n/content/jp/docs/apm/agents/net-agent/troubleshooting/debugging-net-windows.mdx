...
<CollapserGroup>
  ...
  <Collapser id="env-var-core" title=".NET Core: 環境変数を確認する">
    ...
    <Callout variant="tip">
      システム全体ではなくプロセスごとにこの環境変数を設定するヒントについては、[環境変数の設定に関するフォーラムの投稿](https://knowledge.newrelic.com/s/article/Setting-dot-NET-Core-agent-environment-variables-per-process)をご覧ください。
    </Callout>
  </Collapser>
</CollapserGroup>
