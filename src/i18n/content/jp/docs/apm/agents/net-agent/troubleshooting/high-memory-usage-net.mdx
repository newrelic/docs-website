...
New Relic の .NET エージェントをインストールした後、Microsoft Windows**Task Manager** \[タスク マネージャー]などの監視ツールでの`Working Set`メモリ使用量の増加が見られます。通常、増加は監視対象アプリケーションごとに約 100 MB です。これは物理マシンと仮想マシンの両方で発生する可能性があります。
