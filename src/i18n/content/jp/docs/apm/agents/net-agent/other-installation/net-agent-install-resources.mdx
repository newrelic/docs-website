...
* **アプリ名** 。 [意味のあるアプリケーション名](/docs/agents/manage-apm-agents/app-naming/name-your-application#app-name) の設定を推奨しますが、インストール後に設定することもできます。環境変数は
  ...
  [また、アプリの設定ファイルであるnewrelic.configや、API](/docs/agents/net-agent/configuration/name-your-net-application) で設定することもできます。## スクリプト可能なインストーラー（Windowsのみ） [#scriptable-installer]スクリプト可能なインストーラーは、.NETエージェントをインストールするためのPowerShellスクリプトを含むZIPアーカイブです。.NET Framework用と.NET Core用で別々のZIPアーカイブがあります。<Callout variant="important">
