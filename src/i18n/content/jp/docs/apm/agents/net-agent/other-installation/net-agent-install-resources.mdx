...
<Callout variant="important">
  このドキュメントは
  単独で読むことを意図したものではありません。インストール方法については、
  [Introduction to .NET agent
  install](/docs/agents/net-agent/installation/install-enable-new-relic-net-agent)
  をご覧ください。
</Callout>

## マイクロソフトの.NET プロファイラー [#profiler]
...
New Relic 以外の .NET モニタリングソリューションでも、このプロファイリング API を使用することができます。しかし、一度にプロファイラーを使用できるのは 1 つのサービスのみです。つまり、過去に.NET モニタリングサービスを使用していて、完全に無効化/削除していない場合は、プロファイラーがまだ使用されている可能性があり、.NET エージェントを有効にしようとすると、 [プロファイラーのコンフリクト](/docs/agents/net-agent/troubleshooting/profiler-conflicts) が発生します。

## .NET エージェントダウンロードライブラリ [#download-library]
...
## .NET エージェントを手動でインストールするための Zip ファイル(Windows) [#install-zip-file]

ZIP ファイルを使用してエージェントを手動でインストールする場合は、モニターするアプリケーションに適したファイルを選択してください。

1.  [.NET エージェントのダウンロードサイト](http://download.newrelic.com/dot_net_agent/latest_release/ '新しいウィンドウにリンクが開きます。') にアクセスして、アプリケーションのランタイムとアーキテクチャに合ったファイルを入手してください。

    - .NET Framework（32 ビット）： `newrelic-agent-win-x86-VERSION.zip`
    - .NET Framework (64-bit): `newrelic-agent-win-x64-VERSION.zip`
    - .NET Core（32 ビット）： `newrelic-netcore20-agent-win-x86-VERSION.zip`
...
3.  監視したいプロセスの環境変数を設定します。
    ...
    <Callout variant="caution">
      これらの環境変数をグローバルに設定することは推奨しません。これらの環境変数をグローバルに設定すると、システム上で実行されているすべての
      .NET プロセスがインスツルメンテーションされ、New Relic
      に報告される可能性があります。エージェントを手動でインストールしている場合は、監視したいプロセスのみにこれらの環境変数を設定する手段があることを想定しています。
    </Callout>
...
4.  アプリケーションを再起動します。 IIS を使用している場合は、IIS を再起動します。
...
.NET アプリケーションが、 \*\*自動的にインスツルメンテーションされないアプリケーションフレームワークを使用している場合や、アプリにフレームワークがない場合（コンソールアプリなど）は、インストール後、アプリのインスツルメンテーションを手動で設定する必要があります。そのための手順を以下に示します。
...
3. [](/docs/agents/net-agent/installation/new-relic-net-agent-install-introduction#install-instructions)[Custom instrument](/docs/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation) アプリケーションで、New Relic に報告するアクティビティを定義します。## インストールに関する環境変数 [#env-variables].NET エージェントのインストールでは、環境変数の設定が必要です。一部のインストール手順（IIS ホストの.NET アプリケーションなど）では、これらの環境変数は自動的に設定されます。その他のインストール手順では、手動で設定する必要があります。.NET Framework と .NET Core の両方のアプリケーションに必要な環境変数については、 [環境変数](/docs/agents/net-agent/configuration/net-agent-configuration#environment-variables) を参照してください。環境変数を手動で設定する必要があるインストールでは、環境変数を使って、その他の [設定オプション](/docs/agents/net-agent/configuration/net-agent-configuration) を設定することもできます。ここでは、よく設定される 2 つの項目について説明します。\* **ライセンスキー** 。 [New Relic のライセンスキー](/docs/accounts/install-new-relic/account-setup/license-key) を設定するための環境変数です。

   ```
   NEW_RELIC_LICENSE_KEY = <var>YOUR_LICENSE_KEY</var>
   ```

   また、 [newrelic.config ファイル](/docs/agents/net-agent/configuration/net-agent-configuration#service) や、 [アプリの config ファイル](/docs/agents/net-agent/configuration/net-agent-configuration#app-cfg-license-key) で設定することもできます。
...
- **アプリ名** 。 [意味のあるアプリケーション名](/docs/agents/manage-apm-agents/app-naming/name-your-application#app-name) の設定を推奨しますが、インストール後に設定することもできます。環境変数は
  ...
  [また、アプリの設定ファイルである newrelic.config や、API](/docs/agents/net-agent/configuration/name-your-net-application) で設定することもできます。## スクリプト可能なインストーラー（Windows のみ） [#scriptable-installer]スクリプト可能なインストーラーは、.NET エージェントをインストールするための PowerShell スクリプトを含む ZIP アーカイブです。.NET Framework 用と.NET Core 用で別々の ZIP アーカイブがあります。
