...
<CollapserGroup>
  ...
  >
    ...
    * ****が 常時点灯 の設定を使用していない場合は、合成モニターフリー ピンガーモニター を使用してみてはいかがでしょうか。``[](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors#monitor-type-ping)
    * ****`常時稼働 の設定を使用している場合、Synthetic モニタリングがワーカーロールのエンドポイントに到達できる必要があります。到達できない場合は、 Synthetics scripted monitor を使用して、サイトで実行してみてください。`[](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers)
  </Collapser>
</CollapserGroup>