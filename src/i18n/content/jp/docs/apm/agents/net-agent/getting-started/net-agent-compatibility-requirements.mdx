  ...
  <TabsPages>
    <TabsPageItem id="core">
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  ...
                  * Windows 11
                  ...
                  これらのOSのコンテナ化されたバージョンもサポートされています（`nanoserver`を除く）。
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  .NET Core 2.0+/.NET 5+ランタイムがサポートするすべてのx64 Linuxディストリビューションは、.NETエージェントによってサポートされています（コンテナ化されたバージョンを含みます）。完全なリストについては、使用しているランタイムのバージョンに関する[Microsoftのドキュメント](https://learn.microsoft.com/en-us/dotnet/core/install/linux)を参照してください。
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  .NET 5+ランタイムがサポートするすべてのARM64 Linuxディストリビューションは、次の既知の例外を除き、.NETエージェント（コンテナ化されたバージョンを含む）によってサポートされています。
      ...
      <CollapserGroup>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  * IBM DB2ドライバーまたはサーバーには、特定の最小または最大バージョンの要件はありません。
                  * 同期メソッド（たとえば、`ExecuteReader`、`ExecuteNonQuery`）のみが自動的にインストゥルメントされます。非同期メソッド（たとえば、`ExecuteReaderAsync`）はデフォルトではサポートされていません。
                  * `.NET Core`固有のパッケージ（`IBM.Data.DB2.Core`）は自動的にインストゥルメントされ**ません**。このパッケージを計装するには、[カスタムインストゥルメンテーション](/docs/apm/agents/net-agent/custom-instrumentation/introduction-net-custom-instrumentation)を使用する必要があります。
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  ...
                  * 最新の検証済み互換バージョン：26.7.0
                  * バージョン9.7.0以降は、.NETエージェントv10.52.0以降でサポートされます。

                  <DNT>**MySqlConnector**</DNT>

                  * サポートされている最小バージョン：1.0.1
                  * 最新の検証済み互換バージョン：2.6.2
                </td>
              </tr>
        ...
        <Collapser className="freq-link" id="background-jobs-workflows-core" title="バックグラウンドジョブとワークフロー">
          .NETエージェントは、以下のバックグラウンドジョブおよびワークフローライブラリへの .NETアプリケーション呼び出しのパフォーマンスを自動的にインストゥルメントします。
    ...
    <TabsPageItem id="framework">
      ...
      <CollapserGroup>
        ...
        <Collapser className="freq-link" id="operating-system-framework" title="オペレーティング·システム">
          ...
          * Windows Azure （OS Family 1、2、3）
          ...
          エージェントは、既知の例外である`nanoserver`を除き、サポートされているすべてのWindows OSのコンテナ化されたバージョンでの実行をサポートしています。
        </Collapser>
      ...
      <CollapserGroup>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  * IBM DB2ドライバーまたはサーバーには、特定の最小または最大バージョンの要件はありません。
                  * 同期メソッド（たとえば、`ExecuteReader`、`ExecuteNonQuery`）のみが自動的にインストゥルメントされます。非同期メソッド（たとえば、`ExecuteReaderAsync`）はデフォルトではサポートされていません。
                  * `.NET Core`固有のパッケージ（`IBM.Data.DB2.Core`）は自動的にインストゥルメントされ**ません**。
                </td>
              </tr>
        ...
        <Collapser className="freq-link" id="background-jobs-workflows-framework" title="バックグラウンドジョブとワークフロー">
          .NETエージェントは、以下のバックグラウンドジョブおよびワークフローライブラリへの .NETアプリケーション呼び出しのパフォーマンスを自動的にインストゥルメントします。
