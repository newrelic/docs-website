...
<CollapserGroup>
  ...
  >
    [`newrelic.config`ファイル](/docs/agents/net-agent/configuration/net-agent-configuration)（グローバルまたはローカル）を使用すると、モニターするアプリケーションを選択できます。設定ファイルで、各アプリケーションに子`application`要素を含む`instrumentation`要素を追加します。instrumentation要素に、インストゥルメントするベースファイル名を、拡張子と共に提供します（例：`<application name="DataServices.exe" />`）。
