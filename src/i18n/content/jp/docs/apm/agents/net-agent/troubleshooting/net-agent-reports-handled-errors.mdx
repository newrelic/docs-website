...
<CollapserGroup>
  ...
  >
    この例では、 New Relic `GetResponse()` からエラーを報告します。<DNT>**unless**</DNT> メソッド `Foo()` がインストゥルメントされました。 `Foo` がインストゥルメントされている限り、 New Relic `Foo` が呼び出されたときにトランザクションを開始し、`Foo` が終了したときにトランザクションを終了します。
