...
## IISでホストされるアプリケーションの環境変数を設定する [#iis]

アプリケーションがIISでホストされている場合、システム全体の環境変数として設定する代わりに（またはそれに加えて）、IISの`ApplicationHost.config`ファイルを直接編集することで、アプリケーションプールごとに環境変数を設定できます。これは、マシンのすべてのプロセスに対してではなく、特定のアプリケーションプールに対してのみ.NETエージェントを有効にするか、構成する場合に便利です。

デフォルトでは、`ApplicationHost.config`は`C:\Windows\System32\inetsrv\config\ApplicationHost.config`にあります。`<applicationPools>`セクション内で、アプリケーションプールを表す各`<add>`要素には`<environmentVariables>`子要素を含めることができます。この子要素には、そのアプリケーションプールに設定する各環境変数の`<add name="..." value="..." />`エントリが含まれます。

たとえば、.NETエージェントを有効にし、`TestApplication`という名前のアプリケーションプールのいくつかの一般的な設定オプションを設定するには：
...
また、特定のプールの独自の`environmentVariables`設定によって上書きされない限り、システム全体の環境変数と同様に、デフォルトですべてのアプリケーションプールに適用される環境変数を設定するために`applicationPoolDefaults`を使用することもできます。

<Callout title="注記">
  `ApplicationHost.config`を直接編集するにはアドミニストレーター権限が必要であり、変更を有効にするにはアプリケーションプールをリサイクルする（またはIISを再起動する）必要があります。不正な設定によりIISがサイトやアプリケーションプールを開始できなくなる可能性があるため、編集する前に必ずファイルをバックアップしてください。
</Callout>

この設定要素の詳細については、[applicationPoolDefaults environmentVariables](https://learn.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/applicationpooldefaults/environmentvariables/)に関するMicrosoftのドキュメントを参照してください。