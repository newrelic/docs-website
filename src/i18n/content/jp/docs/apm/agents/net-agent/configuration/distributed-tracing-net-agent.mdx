...
エージェント用の[ 標準のディストリビューティッド（分散）トレーシングは トレースの最大](#quick-start-apm) APM[10% をキャプチャしますが、すべてのデータを分析して最も関連性の高いトレースを見つけたい場合は、](/docs/understand-dependencies/distributed-tracing/infinite-tracing/introduction-infinite-tracing) 無限トレーシング を設定できます。
