...
トレース オブザーバーは、すべてのトレースを収集して分析するNew Relic AWSベースのサービスです。 <a href="/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer" target="_blank">トレース オブザーバーの設定</a>の手順に従います。 完了したら、トレース オブザーバー情報とともにここに戻り、次の手順に進んでエージェントを構成します。
...
<CollapserGroup>
  <Collapser id="dot-net-version" title=".NETエージェントの互換性">
...
<CollapserGroup>
  <Collapser id="dot-net-config-standard" title=".NETエージェントの設定">
