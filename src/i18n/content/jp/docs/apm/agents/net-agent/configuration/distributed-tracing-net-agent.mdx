...
<CollapserGroup>
  <Collapser id="calling-service" title="通話サービスを計測する">
    ...
    1. 送信メッセージがトランザクションのコンテキスト内で送信されることを確認します。 これには[カスタムインストゥルメンテーションが](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/)必要になる場合があります。
    2. エージェントAPI呼び出しを呼び出して、送信されるメッセージにトレース データを挿入します ( [.NET エージェントAPI](/docs/agents/net-agent/net-agent-api/itransaction#InsertDistributedTraceHeaders)参照)。
  </Collapser>

  <Collapser id="called-service" title="呼び出されたサービスを計測する">
    呼び出されたサービスをインストルメント化するには：

    1. 受信メッセージがトランザクションのコンテキスト内で受信されることを確認します。 これには[カスタムインストゥルメンテーションが](https://docs.newrelic.com/docs/apm/agents/net-agent/custom-instrumentation/custom-instrumentation-attributes-net/)必要になる場合があります。
    2. エージェントAPI呼び出しを呼び出して、受信メッセージからトレース データを受け取ります (「 [.NET API](/docs/agents/net-agent/net-agent-api/itransaction#AcceptDistributedTraceHeaders)を参照)。
  </Collapser>
</CollapserGroup>