...
これらのメトリックは、 [**Transactions** \[トランザクション\] ページ](/docs/applications-menu/transactions-dashboard#tx_viewing) および [トランザクション追跡](/docs/traces/viewing-transaction-traces)で表示できます。トランザクションを表示するには: **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (アプリを選択) > Monitor > Transactions > Type > (タイプを選択)**に移動します。[API を使用してカテゴリを変更しない](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api)限り、タイプは `Non-web/Custom` になります。
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (アプリを選択) > Monitoring > Transactions > Type > (選択したタイプ)**: **Type** \[タイプ] メニューを使用してカスタム トランザクションを表示します。
</figcaption>
...
このシンプルなコンソール アプリは、トランザクションの作成を示します。アプリケーションを数回実行すると、 [Transactions page](/docs/apm/applications-menu/monitoring/transactions-page) \[トランザクション] ページ ( **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > APM & services > (アプリを選択) > Transactions > Type**) に作成したトランザクションが表示されます。**ダミー** セグメントは、トランザクション内訳テーブルおよびトランザクション追跡に表示されます。
