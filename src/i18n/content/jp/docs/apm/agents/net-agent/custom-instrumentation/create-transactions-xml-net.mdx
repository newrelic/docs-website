...
これらのメトリックは、\[[**トランザクション\]**ページ](/docs/applications-menu/transactions-dashboard#tx_viewing)と[トランザクショントレース](/docs/traces/viewing-transaction-traces)で表示できます。トランザクションを表示するには： **[one.newrelic.com](https://one.newrelic.com)** **> APM>（アプリを選択）>モニター>トランザクション>タイプ>（タイプを選択）に**移動します。[API を使用してカテゴリを変更し](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api)ない限り、タイプは`Non-web/Custom`になります。
