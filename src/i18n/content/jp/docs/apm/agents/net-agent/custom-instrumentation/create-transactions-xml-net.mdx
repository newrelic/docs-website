...
これらの指標は、\[[**トランザクション\]**ページ](/docs/applications-menu/transactions-dashboard#tx_viewing)と[トランザクション追跡](/docs/traces/viewing-transaction-traces)で確認できます。トランザクションを表示するには: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Monitor > Transactions > Type > (タイプを選択) に**移動します。[API を使用してカテゴリを変更し](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api)ない限り、タイプは`Non-web/Custom`になります。
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Monitoring > Transactions > Type > (選択したタイプ)** : **Type**メニューを使用してカスタムトランザクションを表示します。
</figcaption>
...
この単純なコンソール アプリは、トランザクションの作成を示しています。アプリケーションを数回実行すると、作成した[トランザクションが \[トランザクション\] ページ](/docs/apm/applications-menu/monitoring/transactions-page)( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > トランザクション > タイプ**) に表示されます。**ダミー**セグメントは、トランザクション内訳テーブルとすべてのトランザクション追跡に表示されます。
