...
この単純なコンソール アプリは、トランザクションの作成を示しています。アプリケーションを数回実行すると、作成した[トランザクションが \[トランザクション\] ページ](/docs/apm/applications-menu/monitoring/transactions-page)( **[one.newrelic.com](https://one.newrelic.com/all-capabilities) APM & services > (アプリを選択) > Transactions > Type** ) に表示されます。**ダミー**セグメントは、トランザクション内訳テーブルとすべてのトランザクション追跡に表示されます。
