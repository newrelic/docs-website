...
これらの指標は、\[[**トランザクション\]**ページ](/docs/applications-menu/transactions-dashboard#tx_viewing)と[トランザクション追跡](/docs/traces/viewing-transaction-traces)で確認できます。トランザクションを表示するには: **[one.newrelic.com](https://one.newrelic.com) > APM > (アプリを選択) > Monitor > Transactions > Type > (タイプを選択) に**移動します。[API を使用してカテゴリを変更し](/docs/agents/net-agent/net-agent-api/settransactionname-net-agent-api)ない限り、タイプは`Non-web/Custom`になります。
