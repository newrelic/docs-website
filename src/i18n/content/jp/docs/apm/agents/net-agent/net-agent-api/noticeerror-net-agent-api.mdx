...
<Callout variant="tip">
  このAPIで報告されたエラーは、エージェントの設定で無視されるように設定されたHTTPステータスコード(例：404)になったトランザクション内で報告された場合でも、New Relicに送信されます。
</Callout>
