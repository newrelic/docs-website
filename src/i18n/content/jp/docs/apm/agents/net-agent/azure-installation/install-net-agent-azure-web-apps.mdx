...
New Relic でアプリケーションのパフォーマンスを表示するには、アプリケーションがトラフィックを受信する必要があります。データが表示されるまで数分かかる場合があります。データが表示されない場合は、Azure Web Apps の[トラブルシューティング手順](/docs/agents/net-agent/azure-troubleshooting/azure-web-apps-using-always-on-and-no-data-appears)を参照してください。New Relic でアプリのパフォーマンスを表示するには: **[one.newrelic.com](https://one.newrelic.com) > APM > (アプリを選択)**に移動します。[APM の**概要**ページ](/docs/apm/applications-menu/monitoring/apm-overview-page)が自動的に表示されます。
