...
NuGetインストール方法では、New Relicエージェントをアプリケーションにパッケージし、Azure Web App Hostで利用できるようにします。
...
NuGetを使用して.NETFrameworkエージェントをアップグレードする場合、 `newrelic.config`で行った変更はすべて上書きされます。変更を保持するには、プロジェクトの外部に`newrelic.config`を保存し、アップグレード後に復元します。
...
New Relic .NET Core エージェントは、Azure App Services 上の Linux および Windows アプリケーションをサポートします。Azure App Services のインストール手順は、 [Linux](/docs/agents/net-agent/installation/install-net-core-agent-linux) および [Windows](/docs/agents/net-agent/installation/install-net-agent-windows#installing_core) の .NET Core エージェントのインストール手順とは異なります。
...
1. New Relic .NET エージェントのアプリ設定を追加する前に、 [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) を確認してください。
...
アプリケーション設定でライセンスキーとアプリ名を**設定**すると、 `newrelic.config`が上書きされます。
...
New Relic でアプリケーションのパフォーマンスを確認するには、アプリケーションがトラフィックを受信する必要があります。データが表示され始めるまで、数分待つ必要があるかもしれません。データが表示されない場合は、 [トラブルシューティング手順](/docs/agents/net-agent/azure-troubleshooting/azure-web-apps-using-always-on-and-no-data-appears) for Azure Web Apps を参照してください。

New Relic でアプリのパフォーマンスを確認するには： **[one.newrelic.com](https://one.newrelic.com) > APM> (select an app)** にアクセスします。 [APM **Summary** ページ](/docs/apm/applications-menu/monitoring/apm-overview-page) が自動的に表示されます。

また、 [エラー](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) 、 [データベースとインスタンスのパフォーマンスの問題](/docs/apm/applications-menu/features/analyze-database-instance-level-performance-issues) 、 [さらに](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm) に関する詳細な情報を見ることができます。

2017 年 10 月以前に New Relic アプリを作成した場合は、Azure ポータルを使用して New Relic のアカウントブレードを選択します。 [SAMLシングルサインオン（SSO）](/docs/accounts-partnerships/accounts/saml-single-sign/new-relic-partners-saml-sso) でAPMに自動的にログインされます。また、Azureポータルで、 **New Relic Accounts> (select your app)** にアクセスすると、アプリケーションのエラーレートやスループットデータを見ることができます。
