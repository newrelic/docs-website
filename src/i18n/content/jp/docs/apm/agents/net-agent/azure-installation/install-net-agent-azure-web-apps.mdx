...
5. 新しいバージョンのエージェントを使用するために、ウェブアプリを再起動します。<Callout variant="important">
...
5. \[**構成**]から\[ **+****新しいアプリケーション設定**]を選択し、これらのキーと値のペアを追加します。<Callout variant="important">
...
</table>6. Azureポータルで、New Relic [app settings](#web-app-agent-settings) をAzure App Serviceに追加します。
...
</CollapserGroup>6. Azureポータルで、New Relic [app settings](#web-app-agent-settings) をAzure App Serviceに追加します。
...
4. 保存してWebアプリを再起動します。アプリケーション設定でライセンスキーとアプリ名を**設定**すると、 `newrelic.config`が上書きされます。アプリのKudu診断コンソールで、アプリの環境変数を確認するには、 **環境> 環境変数** をご覧ください。例えば：```
...
* *** アプリのルートにある`newrelic`ディレクトリ内のすべてのファイルがAzureに公開されていることを確認してください。* 環境変数が正しく設定されているか確認してください。**
