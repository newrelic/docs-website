...
6. Visual Studio でアプリケーションを開き、 [**Package manager** console](http://docs.nuget.org/docs/start-here/using-the-package-manager-console) から適切なコマンドを実行して New Relic NuGet パッケージをインストールします。
...
8. Azure ポータルで、監視したい Web アプリを開き、 **設定> 構成** を選択します。

9. **Configuration** から、 **+** **New application setting** を選択し、以下のキー／バリューペアを追加します。
...
10. Azureポータルで、New Relic [app settings](#web-app-agent-settings) をAzure App Serviceに追加します。

11. Webアプリを再起動します。## NuGetによるインストール（.NET Core） [#nuget-install-net-core]NuGetインストール方法では、New Relicエージェントをアプリケーションにパッケージし、Azure Web App Hostで利用できるようにします。New Relic .NET Core エージェントは、Azure App Services 上の Linux および Windows アプリケーションをサポートします。Azure App Services のインストール手順は、 [Linux](/docs/agents/net-agent/installation/install-net-core-agent-linux) および [Windows](/docs/agents/net-agent/installation/install-net-agent-windows#installing_core) の .NET Core エージェントのインストール手順とは異なります。NuGetを使用してAzure Web Appに.NETエージェントをインストールするには、以下の手順に従います。1. `NewRelic.Agent` NuGetパッケージをインストールします。

12. **log** ノードを、 `newrelic.config` ファイルにdirectory属性を追加して変更します。(**Note:** Visual Studioで、NuGetによってプロジェクトに追加された `newrelic.config` ファイルを編集できない場合は、アプリケーション内にこのファイルのローカルコピーを作成する必要があります)。
...
14. Azure ポータルで、監視したい Web アプリを開き、 **設定> 構成** を選択します。

15. **Configuration** から、 **+** **New application setting** を選択し、以下のキー／バリューペアを追加します。
...
16. Azureポータルで、New Relic [app settings](#web-app-agent-settings) をAzure App Serviceに追加します。

17. Webアプリを再起動します。## New Relicアプリの設定をAzure App Servicesに追加する [#web-app-agent-settings]アプリの設定を追加するには1. New Relic .NET エージェントのアプリ設定を追加する前に、 [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) を確認してください。

18. Azure ポータルで、監視したい Web アプリを開き、 **Configuration> Application settings** を選択します。

19. **Application settings** から、 **+** **New application setting** を選択し、以下のキー／バリューペアを追加します。
...
20. 保存してWebアプリを再起動します。**Application settings** でライセンスキーとアプリ名を設定すると、 `newrelic.config` が上書きされます。アプリのKudu診断コンソールで、アプリの環境変数を確認するには、 **環境> 環境変数** をご覧ください。たとえば、\`\`\`
