...
1. New Relic .NET エージェントのアプリ設定を追加する前に、 [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) があることを確認してください。
...
[](/docs/apis/intro-apis/new-relic-api-keys/#license-key)**アプリケーション設定** でライセンス キーとアプリ名を設定すると、 `newrelic.config`が上書きされます。
