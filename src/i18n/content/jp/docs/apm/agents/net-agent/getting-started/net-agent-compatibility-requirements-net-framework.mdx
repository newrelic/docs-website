...
エージェントには、フレームワーク、データベース、メッセージキューシステムなど、.NETエコシステムで最も人気ある一部のビルトイン・インストゥルメンテーションとメッセージキューシステムが含まれています。エージェントを[ダウンロードおよびインストール](/docs/agents/net-agent/installation/introduction-net-agent-install)後は、監視下のプロセス内で実行されます。エージェントは個別のプロセスまたはサービスを作成しません。
        ...
        <tr>
          ...
          <td>
            ...
            * `DownloadData`
              ...
              エージェントは、RestSharpバージョン105.2.3～106.6.10をサポートします。
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            アプリケーションがADO.NETを使用している場合、ADOダッシュボードを生成するNew Relicエージェントの組み合わせをインストールできます。必要な作業は次のとおりです。

            1. [インストレーションツール](https://one.newrelic.com/marketplace?account=2498654&state=e56ce690-3ecc-e8cf-e36e-b40d96ff24f2)の手順に従って、New Relic APM .NETエージェントとInfrastructureエージェントをインストールします。
