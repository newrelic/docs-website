...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="アプリケーションの寿命"
  >
    .NETエージェントは、各[収集サイクル](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#harvest-cycle)の終わりにデータをアップロードします（1分間に1回）。.NETアプリケーションがそれほど長く実行されない場合は、`newrelic.config`ファイルで、`サービス要素`の`sendDataOnExit`属性を`true`に設定できます。
  </Collapser>
</CollapserGroup>
...
<CollapserGroup>
  ...
  >
    ...
    * メッセージの配置と採用
      ...
      NServiceBus 5.0（6.0以降はサポートされない）：

    * メッセージの配置、採用、および[クロスアプリケーションのトレース](/docs/apm/transactions/cross-application-traces/cross-application-tracing)
