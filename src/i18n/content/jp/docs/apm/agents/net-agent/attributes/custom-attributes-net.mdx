...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        `arrays`、 `Lists` 、およびその他の`IEnumerable`タイプ
      </td>

      <td>
        JSON 配列としてシリアル化されます。 配列内の個々の要素は、上記に示した型変換規則に従います。null要素はフィルタリングされ、シリアル化された配列には含まれません。空の配列およびnull値のみを含む配列は記録されません。.NET エージェント バージョン 10.50.0 以降が必要です。
      </td>
    </tr>
