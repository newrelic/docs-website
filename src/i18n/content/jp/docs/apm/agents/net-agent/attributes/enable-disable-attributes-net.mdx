...
<CollapserGroup>
  ...
  >
    **newrelic.config**の`analyticsEvents`要素は廃止されました。 **newrelic.config**で[`transactionEvents`](/docs/agents/net-agent/installation-configuration/net-agent-configuration#transaction_events)要素を有効にします。
