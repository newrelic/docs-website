...
* スパン データが報告されたことを確認するには、アップロードしたスパンに対してNRQL クエリ[ ビルダー で](https://one.newrelic.com/launcher/nr1-core.home#pane=eyJuZXJkbGV0SWQiOiJucjEtY29yZS5zZWFyY2gtcm9vdCJ9&overlay=eyJvdmVybGF5SWQiOiJ3YW5kYS1kYXRhLWV4cGxvcmF0aW9uLm5ycWwtZWRpdG9yIiwibmVyZGxldElkIjoid2FuZGEtZGF0YS1leHBsb3JhdGlvbi5kYXRhLWV4cGxvcmVyIiwiaW5pdGlhbEFjdGl2ZUludGVyZmFjZSI6Im5ycWxFZGl0b3IiLCJpbml0aWFsQWNjb3VudElkIjoxfQ==) クエリを実行します。トレース データをレポートするために使用したのと同じ New Relic アカウント (同じ<InlinePopover type="licenseKey"/> ) を使用する必要があります。 クエリの例:
...
* スパンにタイムスタンプがある場合は、以下のガイドラインを満たすようにしてください。
  ...
  * `newrelic`-format タイムスタンプは

    <DNT>
      **milliseconds**
    </DNT>

    である必要があります。 [currentmillis.com](https://currentmillis.com)で現在の時刻をミリ秒単位で取得できます。

  * `zipkin`-format タイムスタンプは
