...
<CollapserGroup>
  ...
  >
    1. 次の手順を使用して、APMエージェントを無効にします。
       ...
       * <DNT>
         ...
         : `newrelic.so`コンポーネントと[`newrelic-daemon`](/docs/agents/php-agent/getting-started/new-relic-daemon-processes)コンポーネントの両方を削除するか無効にします。 `newrelic.enabled`を`false`に設定します。

       * <DNT>
           [**Python**](/docs/agents/python-agent/installation-configuration/python-agent-configuration#monitor_mode)
         </DNT>

         : `monitor_mode`を`false`に設定します。
  ...
  >
    ...
    * <DNT>
      ...
      ブラウザ アプリが[APM アプリケーションにリンクされている](/docs/browser/browser-monitoring/installation/install-browser-monitoring-agent/#select-apm-app)場合、APM アプリケーションを削除するとブラウザ アプリケーションも削除されます。 [APM アプリを削除する手順を](#remove-apm-apps)参照してください。
  ...
  >
    ...
    * <DNT>
      ...
      上記の手順を実行しても UI にそのアプリが表示される場合は、 [NerdGraph を使用して関連するエンティティを削除](/docs/apis/nerdgraph/examples/nerdgraph-entities-api-tutorial/#delete-entities)できます。 アプライアンス ID の検索方法については、 [「アプライアンス」](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic/#find)を参照してください。
  </Collapser>
</CollapserGroup>
