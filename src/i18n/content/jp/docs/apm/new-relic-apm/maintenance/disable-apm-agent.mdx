...
<CollapserGroup>
  ...
  >
    ...
    * [`enabled`](/docs/agents/go-agent/configuration/go-agent-configuration#enabled)構成設定を使用します。次に、アプリを再コンパイルして再起動します。 （これにより、インストルメンテーションがまだ実行されているため、オーバーヘッドが削減されることは**ありません**[。NewRelicコレクター](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#collector)へのデータの送信が妨げられるだけです。）
