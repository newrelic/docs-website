...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="PHP"
  >
    To disable the PHP agent, set the [`enabled`](/docs/agents/php-agent/configuration/php-agent-configuration#inivar-enabled) config setting to `false`, then restart the web server or PHP.
