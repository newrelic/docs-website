...
<CollapserGroup>
  ...
  >
    ...
    * [`agent_enabled`](/docs/agents/ruby-agent/configuration/ruby-agent-configuration#agent_enabled)構成設定を使用します。 これは、構成ファイルまたは環境ごとのセクションで実行できます。 その後、アプリサーバーを再起動します。
