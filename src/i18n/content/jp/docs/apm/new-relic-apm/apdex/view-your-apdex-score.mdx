...
<Callout variant="tip">
  Apdexはウェブアプリやサービスにのみ適用されます。UI の **Apdex** チャートは、
  [非ウェブ
  トランザクション](/docs/apm/transactions/intro-transactions/monitor-background-processes-other-non-web-transactions)
  の場合、空白で表示されます。すべてのアプリケーションやサービスの概要を把握するには、
  [New Relic
  Explorer](/docs/new-relic-one/use-new-relic-one/ui-data/new-relic-one-entity-explorer)
  をご利用ください。
</Callout>

## APM で Apdex のスコアを見る [#view_apdex]

Apdex の不満レベルである `Tolerating` ([`apdex_t`](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#apdex_t) ) と `Frustrated` ([`apdex_f`](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#apdex_f) ) は、サイトパフォーマンスの遅さがいかにアプリでの顧客体験の低下に寄与しているかを示しています。例えば、以下のようになります。

- 1.0:すべての回答が満足のいくものです。
- [Tolerating response](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction#levels) 半分はユーザーを満足させるものです。例えば、全ての回答が `Tolerating,` であれば、Apdex のスコアは 0.50 となります。
- 0.0：いずれの回答も満足できるものではありません。

Web アプリケーションやサービスの Apdex スコアを表示するには、以下の方法があります。
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        ![Apdexの隣にあるクエスチョンマークにカーソルを合わせると表示されるスクリーンショットです。](src/images/apdex_question_mark.png "apdex_question_mark.png")
      </td>
    </tr>
...
[APM の上位にあるトランザクション](/docs/apm/transactions/intro-transactions/transactions-new-relic-apm) [**トランザクション** ページ](/docs/apm/applications-menu/monitoring/transactions-page) は、パフォーマンスチューニングやエラー修正の候補になることが多いです。Apdex の不満足度が最も高いトランザクションを表示するには。
...
定義上、 **All transactions** の行は、不満のある回答がない場合でも、アプリの総不満の 100%に常に寄与します（0 の 100%は 0）。この状況では、100%といっても、すべてのトランザクションが不満であることを意味するわけではありません。この列の他のすべての値の合計が 100%です。

特定の取引の Apdex レベルに注目するには、以下の方法もあります。
...
- [リアルユーザーのブラウザパフォーマンス](/docs/browser/new-relic-browser/getting-started/browser-overview-website-performance-glance) アプリの観点から Apdex レベルを確認します。
...
## [クエリビルダで Apdex データを可視化する [#insights]](/docs/browser/new-relic-browser/browser-pro-features/filterable-geography-webpage-metrics-location)

[クエリビルダーを使って](/docs/browser/new-relic-browser/browser-pro-features/filterable-geography-webpage-metrics-location)

- [](/docs/browser/new-relic-browser/browser-pro-features/filterable-geography-webpage-metrics-location)
- [](/docs/browser/new-relic-browser/browser-pro-features/filterable-geography-webpage-metrics-location)[ダッシュボードの作成](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder) Apdex のデータを分析し、共有することができます。
