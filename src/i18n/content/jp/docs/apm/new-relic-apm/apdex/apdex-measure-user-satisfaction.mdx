...
たとえば、`T`が1.2秒の場合、応答が0.5秒で完了すれば、ユーザーは満足します。応答が1.2秒よりも長い場合、ユーザーは満足しません。応答が4.8秒よりも長い場合、ユーザーは不満に感じます。
...
Apdex T値は、アプリケーションサーバーとエンドユーザーのブラウザパフォーマンスに個別の値を指定することによって、それぞれのアプリケーションに定義できます。また、[キートランザクション](/docs/apm/transactions/key-transactions/introduction-key-transactions)の個々のApdex T閾値を定義することもできます。
...
<CollapserGroup>
  ...
  >
    ...
    1. **[one.newrelic.com ](https://one.newrelic.com)> （アプリケーションを選択） > Settings > Application > Application設定**の順に移動します。
  ...
  <Collapser
    className="freq-link"
    id="rum-apdex"
    title="エンドユーザー（ブラウザ）のApdex設定を変更する"
  >
    ブラウザモニタリングのデフォルトのApdex T閾値は7.0秒です。Apdex Tのデフォルトのエンドユーザー閾値を変更するには：

    1. **[one.newrelic.com](https://one.newrelic.com) > ブラウザ > （アプリケーションを選択） > Settings > Application設定**の順に移動します。
    2. **Settings**ページから、エンドユーザーの**Apdex T**値（秒）を設定します。
    3. **Save application settings**を選択します。
  </Collapser>
</CollapserGroup>
