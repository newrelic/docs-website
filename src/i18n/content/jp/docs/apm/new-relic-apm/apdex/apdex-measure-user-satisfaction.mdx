...
始める準備はできていますか？（まだサインアップしていない場合）アカウントにサインアップしてから、ロゴをクリックしてAPMエージェントをインストールし、ユーザー満足度の測定を開始します。数分で完了します！
...
* <DoNotTranslate>**Tolerating:**</DoNotTranslate>
  ...
  レスポンスタイムがTを超過し、4T以下である。この例では、4 x 1.2 = 4.8秒が最大許容レスポンスタイムになります

* <DoNotTranslate>**Frustrated:**</DoNotTranslate>

  レスポンスタイムが4Tより大きいか、リクエストがサーバーサイドエラーを返す。[エラー率が高い](/docs/apm/applications-menu/error-analytics/introduction-error-analytics)場合は、平均レスポンスタイムが許容範囲であっても、Apdexスコアが低くなることがあります
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        APMはこれらのアプリのパフォーマンスをモニターします。その[Apdex設定を定義する](/docs/apm/new-relic-apm/apdex/change-your-apdex-settings)には、APM UIを使用します。<DoNotTranslate>**Apdex**</DoNotTranslate>は[APM Summary](/docs/apm/applications-menu/monitoring/apm-overview-page)ページにあります。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        モニターすることを選んだ、[御社の事業にとって重要なトランザクション](/docs/apm/transactions/key-transactions/introduction-key-transactions)になります。<DoNotTranslate>**Key transactions**</DoNotTranslate>ページで[キートランザクションのApdex設定を定義](/docs/apm/transactions/key-transactions/create-update-key-transactions)できます。
...
<DoNotTranslate>**dissatisfaction percentage**</DoNotTranslate>とは、あるトランザクションが原因で、アプリケーションのユーザーが体験した不満の合計件数の割合です。
...
<CollapserGroup>
  ...
  >
    ...
    Apdex設定を変更すると、[APM <DoNotTranslate>**Summary**</DoNotTranslate>ページ](/docs/apm/applications-menu/monitoring/apm-overview-page)のメインチャートに黒い縦棒が表示されます。
  </Collapser>
</CollapserGroup>