...
[Apdex](https://www.apdex.org/index.php/about/)は、ウェブアプリケーションやサービスのレスポンスタイムについて、ユーザー満足度を計測するための業界標準です。これは、平均レスポンスタイムなどの歪曲されやすい従来のメトリックスの代わりになる不満の割合やApdexスコアなどのメトリックスを介して、アプリケーションのユーザー満足度を確認するための簡素化されたサービスレベル契約（SLA）ソリューションです。
...
<Table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        APMはこうしたアプリケーションのパフォーマンスをモニターします。その[Apdex設定を定義する](/docs/apm/new-relic-apm/apdex/change-your-apdex-settings)には、APM UIを使用してください。[APM概要](/docs/apm/applications-menu/monitoring/apm-overview-page)ページと[Browser概要](/docs/browser/new-relic-browser/getting-started/browser-overview-page-website-performance-summary)ページの**Apdex**をご覧ください。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Browserは、こうしたアプリケーションのエンドユーザー体験をモニターします。こうしたアプリケーションの[エンドユーザーApdex設定を定義する](/docs/browser/new-relic-browser/configuration/browser-settings-ui-options-apdex-geography)には、Browser UIを使用します。New Relicでは、[APM概要](/docs/apm/applications-menu/monitoring/apm-overview-page)ページ[と](/docs/browser/new-relic-browser/getting-started/browser-overview-page-website-performance-summary)Browser概要ページで**エンドユーザー**としてラベル付けされます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        モニターすることを選んだ、[御社の事業にとって重要なトランザクション](/docs/apm/transactions/key-transactions/introduction-key-transactions)になります。[キートランザクション向けApdex設定を定義する](/docs/apm/transactions/key-transactions/create-update-key-transactions)には、APMの**キートランザクション**ページにアクセスして、[キートランザクション概要](/docs/apm/transactions/key-transactions/key-transaction-charts-data)ページで確認します。
      </td>
    </tr>
  </tbody>
</Table>
...
* **不満：** レスポンスタイムが4Tより大きいか、リクエストがサーバーサイドエラーを返す場合。[エラー率が高い](/docs/apm/applications-menu/error-analytics/introduction-error-analytics)場合は、平均レスポンスタイムが許容範囲であっても、Apdexスコアが低くなることがあります。
