  ...
  >
    <CollapserGroup>
      ...
      >
        ...
        ![エンティティタイプの例](images/om-oe-dg-facet-entitytype-example.png)
      </Collapser>
      ...
      >
        ...
        ![ファセットメトリック名の例](images/om-oe-dg-facet-metricname-example.png)
      </Collapser>
    </CollapserGroup>
  </Collapser>
</CollapserGroup>
...
<Callout variant="caution">
  POAアカウントを使用している場合、POAアカウント自体は、AccountNameを使用することによって`consumingAccountName`に含まれないことに注意してください。これは、POAアカウントが実際のテレメトリを受信しないためです。ダッシュボードを通常の親アカウントにインストールする場合、その親アカウントの消費値は、親アカウントに直接取り込まれたデータとサブアカウントに送信されたデータの`sum`です。  
  たとえば、毎月の取り込みに直面すると、次のように表示される場合があります。

  * 親アカウントのインジェスト100GB
  * サブアカウント1 インジェスト50GB
  * サブアカウント2の取り込み：20GB `Parent Account Ingest`には、サブアカウント1と2のデータと、親アカウントに直接送信される追加の30GBが含まれます。
</Callout>
