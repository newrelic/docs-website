...
<CollapserGroup>
  ...
  >
    使用量ベースの[NewRelicOne価格設定モデル](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing)の場合、テレメトリデータとユーザーの両方が使用量とコストに影響します。このガイドは、テレメトリデータの価値を最大化することに焦点を当てています。このセクションでのユーザーの言及は、ユーザーとデータのバランスを取るためのさまざまなオプションを理解するのに役立ちます。

    利用計画には、一般的に3つのタイプがあります。利用プランは、組織のインジェストターゲットの設定方法に影響する場合があります。

    ### 年間資金プール

    [年間の資金プール](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#usage-plans)契約を結んでいる場合は、データを取り込むための1か月の目標予算がある可能性があります。たとえば、1日あたり5 TBの目標を設定し、100人のフルプラットフォームユーザーを設定したとします。このタイプの計画データでは、ユーザーは「トレードオフ」される可能性がありますが、可観測性の目標に適切な組み合わせが得られるように、組織内の他の利害関係者とこれについて話し合うのが最善です。一部のお客様は、年間の消費量の変動を計画しますが、今のところ、1か月の消費予算は、年間の総資金プール額を12で割ったものであると想定します。

    必要なフルプラットフォームユーザーとコアユーザーの数がわかっている場合は、次の式を使用できます。

    ```sql
    (monthly_target_spend - (num_fso_users*per_fso_cost) - (num_core_users*per_core_cost))/<var>YOUR_INGESTED_DATA_COST</var>
    ```
    ...
    Standardエディションでは、1か月あたり最大100GBのデータを無料で取り込むことができます。現在無料のアカウントで作業している場合は、100GBを毎月の摂取目標と見なすことができます。無料アカウントでは、1か月あたり100 GBを超えて取り込まれた追加データは、1GBあたり0.25ドルで請求されます。

    これらのトピックの詳細については[、NewRelicOneの価格設定](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing)を参照してください。
  </Collapser>
  ...
  >
    <CollapserGroup>
      ...
      >
        ...
        データ固有のイベントがどれだけ消費しているかを知るためにさらに深く掘り下げる必要がある場合は、ファセット句で`entityType()`を使用してその結果を取得できます。

        前のクエリに句`FACET entityType()`を追加すると、次のようになります。
