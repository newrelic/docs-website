...
<DoNotTranslate>**Data ingest governance**</DoNotTranslate> 組織によって収集されたテレメトリデータの最適な値を取得する方法です。 これは、多数の事業部門や作業グループを持つ複雑な組織にとって特に重要です。 これはNew Relicデータの取り込みを最適化するための 4 部構成のガイドの第 2 部であり、[オブザーバビリティの成熟度に関するシリーズ](/docs/new-relic-solutions/observability-maturity/introduction)の一部です。
...
<CollapserGroup>
  ...
  >
    ...
    すべての [エディション](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing#editions)で、毎月 100 GB のデータを無料で取り込むことができます。無料金額を超えるデータ取り込み価格の詳細については、 [定価表を](/docs/licenses/license-information/usage-plans/new-relic-usage-plan/#list-price)参照してください。
  </Collapser>
  ...
  >
    <CollapserGroup>
      ...
      >
        ...
        <figcaption>
          成長パターンを理解するために<DoNotTranslate>**COMPARE WITH**</DoNotTranslate>を使用する方法を示すサンプル チャート。
        </figcaption>
      </Collapser>
      ...
      >
        ...
        1 日間の取り込みレートの生の時系列のギザギザに注意してください。
        ...
        <figcaption>
          日次レート時系列<DoNotTranslate>**without**</DoNotTranslate>平滑化
        </figcaption>
        ...
        <figcaption>
          日次レート時系列<DoNotTranslate>**with**</DoNotTranslate>平滑化
        </figcaption>
      </Collapser>
      ...
      >
        ...
        <figcaption>
          <DoNotTranslate>**Metric**</DoNotTranslate>ネームスペースの<DoNotTranslate>**metricName**</DoNotTranslate>属性による取り込みのリスト
        </figcaption>
      </Collapser>
    </CollapserGroup>
  </Collapser>
...
<figcaption>
  NRQLを使用して、ターゲットの 30 日間の取り込み <DoNotTranslate>**target**</DoNotTranslate> を表す行をレンダリングできます。
</figcaption>
...
<figcaption>
  NRQL を使用して、毎日の取り込み<DoNotTranslate>**target**</DoNotTranslate>を表す行をレンダリングできます。
</figcaption>
...
2. <DoNotTranslate>**Baseline report**</DoNotTranslate>
...
データ取り込みガバナンスの他のフェーズを容易にするために、 <DoNotTranslate>**Optimize**</DoNotTranslate> 、 <DoNotTranslate>**Forecast**</DoNotTranslate>などの便利な列をレポートに追加します。 次のフィールドは、最適化と計画の決定に役立ちます。
...
<figcaption>
  Lookout を使用すると、 <DoNotTranslate>**usageMetric**</DoNotTranslate>による取り込みで異常を見つけることができます。
</figcaption>
...
<figcaption>
  Lookout を使用すると、 <DoNotTranslate>**consumingAccountName**</DoNotTranslate>による取り込みで異常を見つけることができます。
</figcaption>
...
5. クイックスタートのインストールが完了したら、 `Data governance entity breakdowns`ダッシュボードを開きます。
   ...
   <figcaption>
     エンティティ内訳ダッシュボードは、 <DoNotTranslate>**bytecountestimate()**</DoNotTranslate>を使用して、アプリケーションやクラスタ名などの有用な属性ごとにファセットを取り込みます。
   </figcaption>
...
幸いなことに、この強力な <InlinePopover type="dashboards"/>セットは[、 New Relic Instant から直接](https://onenr.io/0EPwJJO9Ow7)インストールできます。
