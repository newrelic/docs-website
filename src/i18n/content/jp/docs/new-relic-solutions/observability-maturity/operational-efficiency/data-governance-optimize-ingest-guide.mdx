...
<CollapserGroup>
  ...
  >
    モバイル監視とモバイル監視に重点を置いた新しいユーザー向けプラットフォームを担当するチーム<InlinePopover type="browser"/>予算を 50% 超過しています。取り込みのサイズを適切に調整する必要がありますが、**Customer experience** \[カスタマー エクスペリエンスの] 可観測性を犠牲にしないことに固執しています。
...
<CollapserGroup>
  ...
  >
    ...
    * `config_file`: 既存の Fluent Bit 構成ファイルへのパス。重複するソースがあると、New Relic でメッセージが重複します。
