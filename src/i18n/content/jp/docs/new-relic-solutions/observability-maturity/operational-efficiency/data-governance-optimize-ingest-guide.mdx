...
<CollapserGroup>
  ...
  <Collapser id="prometheus-metrics-sources" title="Prometheusのメトリクスソース">
    ...
    **POMI構成パラメーター：**
    ...
    カーディナリティは課金対象となるGB単位の取り込みデータ量に直接関連するわけではありませんが、New Relicはカーディナリティに対して一定のレート制限を設けています。Prometheusクラスタからのカーディナリティを可視化できることは、非常に重要になる場合があります。

    <Callout variant="tip">
      New Relicアカウントには100万のカーディナリティ制限がありますが、最大1500万のカーディナリティをリクエストできます。変更をご希望の場合は、New Relicのアカウント担当者にご連絡ください。詳細については、[メトリクスAPIの制限](/docs/data-apis/ingest-apis/metric-api/metric-api-limits-restricted-attributes)を参照してください。
    </Callout>

    すでにPrometheusサーバーを実行している場合は、POMIまたは`remote_write`を有効にする前に、そこでカーディナリティの見積もりを実行できます。
...
以下の質問に答えることで、最適化計画を策定して実行する能力に自信を持てるようになります。`Baselining`セクションの[データ取り込みベースライン](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-dashboard)および[データ取り込みエンティティ内訳](/docs/new-relic-solutions/observability-maturity/operational-efficiency/dg-baselining#install-entity-breakdown-dashboard)ダッシュボードを使用するとよいでしょう。説明されている通りにこれらのダッシュボードをインストールし、これらの質問のうちいくつ答えられるかを確認してください。
