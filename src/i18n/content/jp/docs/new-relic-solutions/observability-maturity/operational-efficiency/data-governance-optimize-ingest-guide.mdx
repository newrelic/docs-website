...
<CollapserGroup>
  ...
  >
    ...
    \[分析設定]（イベントデータの収集を有効または無効にします。これらのイベントはNew Relicデータベースに報告され、**クラッシュ分析**ページで使用されます。）

    \[エージェントロギング]を設定することもできます（イベントデータの収集を有効または無効にします。これらのイベントはNew Relicに報告され、**クラッシュ分析**ページで多かれ少なかれ冗長になるように使用されます。
