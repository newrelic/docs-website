...
ファネル分析の要点は、ユーザージャーニーの最後のパフォーマンスに焦点を当てることにより、コンバージョンを改善することです。
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        目標到達プロセスの成功率/コンバージョン率の下部
      </td>

      <td>
        ユーザーがアクションを完了して実際に完了する意図を示すのに十分な距離に達した後のコンバージョン率。この例は次のとおりです。<br/>
    ...
    <tr>
      ...
      <td>
        コンバージョンの価値\*業界のしきい値よりも遅い、目標到達プロセスの下部にあるページまたはインタラクションの数。
      </td>
    ...
    <tr>
      ...
      <td>
        コンバージョンの値\*目標到達プロセスの下部にあるバックエンドエラーの数
      </td>
...
* [関連するページにインストールされているブラウザプロ](/docs/browser/browser-monitoring/installation/)
...
[目標到達プロセスダッシュボードの下部をインポートします](#import-the-bottom-of-the-funnel-dashboard)  
[現在のパフォーマンスをキャプチャする](#capture-current-performance)

### じょうごの底がどこから始まるかを特定する

目標到達プロセスの下部は、ユーザーがジャーニーを完了する意図を示すのに十分な距離まで進んだ、ユーザージャーニーの最終ステップに焦点を当てています。
...
目標到達プロセスの下部を通るパスごとに、 [スクリプト化されたモニター](/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors/)があることを確認してください。目標は、ファネルサービスの下部が24時間稼働していることを確認することです。

たとえば、顧客の支払い設定に応じて異なる支払いAPIを呼び出すチェックアウトフローがある場合があります。

### 目標到達プロセスダッシュボードの下部をインポートします
...
公開されている[GitHubREADME](https://github.com/newrelic/oma-resource-center/blob/main/src/content/docs/oma/value-drivers/customer-experience/use-cases/bottom-of-the-funnel-analysis/README.md)に記載されている手順に従ってください。
...
1. GitHubリポジトリの[README](https://github.com/newrelic/oma-resource-center/blob/main/src/content/docs/oma/value-drivers/customer-experience/use-cases/bottom-of-the-funnel-analysis/README.md)の指示に従います。
2. 前の手順のダッシュボードを使用して、目標到達プロセスのパフォーマンスの下部を理解します。
...
* あなたはすでにファネル分析のボトムに精通しており、追跡や開発者の焦点を保証するために、異なるコホート間でパフォーマンスに十分な違いがあります。
...
* ユーザージャーニーが変わるときはいつでも、目標到達プロセスの下部にある手順が同じであるかどうかを再確認してください。必要に応じて、開発者スプリントの変更を組み込みます。
* サポートしている事業部門やその他の内部の利害関係者とメトリックをオープンに共有します。
* カスタマーエクスペリエンスSLOを再検討してください。目標到達プロセスの終わりに、より野心的なSLOを定義する必要がありますか？
* QualityFoundationKPIの[ビジネスクリティカルな](docs/new-relic-solutions/observability-maturity/aqm-implementation-guide/#perform-enablement-two)低下に関するアラートを作成します。
...
* あなたの会社の収益の増加。
* あなたが一緒に働いているように、ビジネスオーナーとの共通言語を作成した、または作成する道を進んでいる。新しいプロジェクトの承認と後援のための新しい道を開く。