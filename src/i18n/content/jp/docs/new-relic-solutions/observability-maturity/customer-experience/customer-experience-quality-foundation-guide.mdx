...
この<DoNotTranslate>**Quality Foundation**</DoNotTranslate>ガイドは、デジタル カスタマー エクスペリエンスを有意義な方法で理解するのに役立つ標準的な実践方法を作成することについて説明しています。 この実装ガイドは次のことに役立ちます。
...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> アップタイムとアベイラビリティーの向上
  ...
  >
    この KPI は[コアウェブバイタル](https://web.dev/vitals/)の一部です。 最大コンテンツフル ペイント (LCP) は、ユーザーが新しいページに移動した後、最大の画像を読み込むのにかかる時間を測定します。
  ...
  <Collapser
    id="core-web-kpi"
    title="次のペイントへのインタラクション (INP)"
  >
    この KPI は[コアウェブバイタル](https://web.dev/vitals/)の一部です。 クリックやタップなどのユーザーのアクションに対して Web ページが視覚的にどれだけ早く反応するかを測定します。

    <DoNotTranslate>**Goal:**</DoNotTranslate> すべてのページまたは少なくとも最も重要なページの 75% パーセンタイルに対して、INP を 200 ミリ秒以上に短縮します。
    ...
    * クリティカル: > 500 ミリ秒
    ...
    INP 閾値は Google のチームによって定義されています。 閾値とその背後にあるサポート ロジックは[、Google の web.dev サイト](https://web.dev/articles/inp)で見つけることができます。
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> すべてのページ、または少なくとも最も重要なページにおいて、75%パーセンタイルのスコアが0.1以下であること。
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> CDN、ネットワーク、サービスのパフォーマンスを向上させることで、ファーストバイトまでの時間を短縮。
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> AJAXの応答時間を測定して改善します。
  ...
  >
    ...
    <DoNotTranslate>**Goal:**</DoNotTranslate> お客様がサイトに来た目的を果たすことができるように、HTTPエラーレートを測定し、削減します。
...
この<DoNotTranslate>**Quality foundation**</DoNotTranslate>ガイドは、ユーザー維持、コンバージョン、満足度を最適化するために、アプリケーションのどこを改善する必要があるかを特定するのに役立ちます。 物事がどこにあるかということよりも、どこに行くかが重要です。
...
* [ニューレリック](/docs/browser/browser-monitoring/getting-started/)
...
browserアプリとページをレビューして、 New Relicに報告すると予想されるすべてのものが実際に報告されていることを確認します。 これを行うには、ブラウザ監視 UI の<DoNotTranslate>**Page Views**</DoNotTranslate>タブを確認するか、次のクエリを実行します。
...
多数のセグメントを含む URL が多数ある場合、URL が短縮されて、 `website.com/product/widget-name`が`website.com/` <DoNotTranslate>**or**</DoNotTranslate> `website.com/product/`になることがあります。 この例では、最初の短縮 URL は特に役に立ちませんが、2 番目の URL は製品の顧客体験データを集約するのに役立つ可能性があります。
...
カスタマー エクスペリエンス データをさまざまなセグメントに分割することで、理解しやすく実用的なものにします。この場合、セグメントはデータのグループを指します。[セグメント許可リスト](/docs/browser/new-relic-browser/configuration/group-browser-metrics-urls/#adding)とは異なり、URL のセクションを参照しません。

以下の記述について考えてみましょう。

* 当社のユーザー体験のほとんどは、次のペイントまでのインタラクションまでに 3 秒以上かかります。
* 最大のコンテンツフル・ペイントには、平均して2秒の時間がかかっています。
* 先週のページビューは100万でした。

と比較しています。

* 米国、カナダ、EMEA のユーザーのほとんどは、次のペイントへのインタラクションに 2 秒以上かかると感じています。 マレーシアとインドネシアのユーザー体験 4 秒;私たちはこれを調査中です。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <DoNotTranslate>**Basic:**</DoNotTranslate> 国ごとにグループ化します。 browserはリクエストの国コードが自動的に含まれるため、それをさらに分割するために何もする必要はありません。

        <DoNotTranslate>**Advanced:**</DoNotTranslate> ブラウザ監視で[カスタム アトリビュート](/docs/telemetry-data-platform/custom-data/custom-events/report-browser-monitoring-custom-events-attributes/)を使用して独自の地域プロパティを作成し、地域グループを地域 SLO グループと一致させます。
    ...
    <tr>
      ...
      <td>
        ...
        * ホテルの予約だけでなく、レンタカーも提供する旅行会社
        ...
        <DoNotTranslate>**Basic:**</DoNotTranslate> 製品ごとのパフォーマンスの内訳は次のとおりです。
        ...
        * `appName`とファセットによるグループ化：1つの製品をサポートするブラウザーに複数のアプリがある場合は、このアプローチを使用します。
        ...
        <DoNotTranslate>**Advanced:**</DoNotTranslate> カスタムbrowser [アトリビュートを使用して、製品オファリングをカスタム アトリビュート](/docs/telemetry-data-platform/custom-data/custom-events/report-browser-monitoring-custom-events-attributes) として ページに追加します。
      </td>
    </tr>
...
1. <InlinePopover type="dashboards"/>

   \> Query builder に移動します。
