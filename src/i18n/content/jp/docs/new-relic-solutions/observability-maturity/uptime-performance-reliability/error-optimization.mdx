...
#### APM サービスごとにサービス レベルを作成する

サービス作成のステップ 2 で、 **Success** SLI を選択します。

#### 各ブラウザ アプリのサービス レベルを作成する

サービス作成のステップ 2 で、 **Success** SLI を選択します。

#### モバイル アプリごとにサービス レベルを作成する

* モバイル クラッシュのサービス レベルを作成します。ステップ 2 で、有効なイベントのソースとして`MobileSession`を選択します。悪い応答のソースとして`MobileCrash`を選択してください。
* モバイル リクエスト エラーのサービス レベルを作成します。ステップ 2 では、有効なイベントのソースとして`MobileRequest`を選択します。悪い応答のソースとして`MobileRequestError`を選択してください。

### サーバーレス アプリケーションごとにサービス レベルを作成する
...
* ステップ 2 では、有効なイベントの場合は`AWSLambdaInvoation`を選択し、不適切な応答の場合は`AwsLambdaInvocationError`を選択します
...
### OpenTelemetry アプリケーションごとにサービス レベルを作成する
...
特殊なケースや複雑なエラーが発生すると、他のチームに助けを求める必要があることに気付くかもしれません。エラー インボックスを Jira にリンクすると、これに役立つ場合があります。エラー インボックスを Jira に接続すると、エラー グループに接続されたチケットを簡単に作成できます。Jira テンプレートを介して Jira に送信される情報を制御できます。

### エラーを Slack にリンクする [#slack]

エラー インボックスに届くエラーの速度が低下するにつれて、定期的なチーム セッションはもはや有効な時間の使い方ではない可能性があります。別の方法として、エラー インボックスを Slack にリンクし、a) チャネルを監視し、発生したエラー グループを解決/無視/割り当てる担当者をローテーション担当者に指定するか、b) チームがエラー グループに積極的に対応できるようにする方法があります。

### CodeStream を使用する [#codestream]

エラー グループの多くは、解決するためにコードの変更が必要になります。[CodeStream を New Relic と統合すると](/docs/codestream/start-here/what-is-codestream)、問題のあるコードを IDE で直接開いて、コードを直接調査できます。開発者がレビューできるように、コードの特定の行にメモやコメントを残すこともできます。

CodeStream をエラー インボックスと統合すると、バージョン番号の表示や SHA のコミットなど、エラー グループに関するより多くのコンテキストも得られます。さらに、コードの問題を特定、議論、および修正するための一元化された場所としてエラー インボックスを使用すると、コードの問題に効率的に対応し、作業の重複を避けることができます。
