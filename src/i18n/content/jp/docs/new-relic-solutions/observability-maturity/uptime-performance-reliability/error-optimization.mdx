...
<CollapserGroup>
  ...
  >
    ...
    **ベスト プラクティス** エラーはフロントエンド アプリケーションとバックエンド アプリケーションの両方で発生しますが、通常はフロントエンドで測定されます。[ファネル クエリは](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/funnels-evaluate-data-series-related-events) コンバージョンの測定によく使用されますが、特定の期間の合計セッション数に対するコンバージョン数をカウントすることで、さらに簡単に測定できます。
  ...
  >
    ...
    **ベスト プラクティス** エラーはフロントエンド アプリケーションとバックエンド アプリケーションの両方で発生しますが、通常はフロントエンドで測定されます。改善を行っている場所に関連するフロントエンド アプリケーションのページ ビューを追跡することにより、エラーの改善がユーザー エンゲージメントに与える影響を測定します。
...
* 私たちの
  ...
  、モバイル監視、サーバーレス監視、または OpenTelemetry ソリューションでエラーが発生しています。
...
<CollapserGroup>
  ...
  >
    ...
    * ステップ 2 では、有効なイベントのソースとして`MobileSession`を選択します。不正な応答のソースとして`MobileCrash`を選択します。
    ...
    * ステップ 2 では、有効なイベントのソースとして`MobileRequest`を選択します。不正な応答のソースとして`MobileRequestError`を選択します。
