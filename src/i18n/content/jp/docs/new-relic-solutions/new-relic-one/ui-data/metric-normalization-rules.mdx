...
メトリックのグループ化の問題とメトリックの正規化は、APMエージェント、モバイルエージェント、およびブラウザーエージェントによって報告される[メトリックタイムスライスデータ](/docs/data-apis/understand-data/new-relic-data-types/#timeslice-data)にのみ適用されます。
