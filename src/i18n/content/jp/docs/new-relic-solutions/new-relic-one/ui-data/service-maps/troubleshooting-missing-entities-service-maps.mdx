...
* [](/docs/apm/distributed-tracing/enable-configure/enable-distributed-tracing#agent-apis)
* [**トランザクションは自動的に計測されません**。アプリケーションが自動的にインストルメント化されるようにするには、使用しているエージェントの](/docs/apm/distributed-tracing/enable-configure/enable-distributed-tracing#agent-apis)[互換性と要件のドキュメント](/docs/apm/new-relic-apm/getting-started/introduction-apm)をお読みください。アプリケーションが自動的にインストルメント化されない場合、または特定のアクティビティのインストルメンテーションを追加する場合は、[カスタムインストルメンテーション](/docs/agents/manage-apm-agents/agent-data/custom-instrumentation#implementing)を参照してください。
