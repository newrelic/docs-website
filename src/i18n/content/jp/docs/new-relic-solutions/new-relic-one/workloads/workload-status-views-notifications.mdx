...
ワークロードの **Health status** \[健全性ステータス] ビューは、ワークロード内のエンティティのアラート ステータスから派生し、ワークロードがどのように動作しているかを示します。
...
ワークロードを使用すると、複雑なシステムの一部であるエンティティをグループ化し、ワークロード内のすべてのエンティティのステータスを要約する単一のグローバル値を取得できます。したがって、ワークロードの動作停止を迅速に検出したり、潜在的なインシデントやサービス品質の低下を予測したりできます。
