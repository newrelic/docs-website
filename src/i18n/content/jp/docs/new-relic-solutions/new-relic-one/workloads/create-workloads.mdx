...
* **scope accounts** \[スコープ アカウントは、] ワークロードがテレメトリ データを取得するアカウントです。ワークロードのスコープ アカウントのすべてにアクセスできないユーザーは、ワークロード内のすべてのデータを表示できない可能性があります。ワークロードを使用するチーム メンバーの場合は、ワークロードがデータを取得する Tea 関連アカウントを必ず追加してください。
