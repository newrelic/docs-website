...
UI または [NerdGraph API を](/docs/nerdgraph-workloads-tutorials) 使用してワークロードを作成できます。 UI を使用してワークロードを作成するには、次の手順に従います。
...
<CollapserGroup>
  ...
  >
    [カスタム ダッシュボードが](/docs/dashboards/new-relic-one-dashboards/get-started/introduction-new-relic-one-dashboards) あり、ワークロードを監視および操作するためにチームに関連するデータが既にわかっている場合は、それらをリンクできます。 <InlinePopover type="dashboards"/>あなたのワークロードから。ダッシュボードにフィルターを設定して、ワークロード固有のコンテキストにスコープを設定することもできます。ユーザーがワークロードからそのダッシュボードを選択すると、フィルターが既に適用された状態で開きます。
    ...
    たす <InlinePopover type="dashboards"/>ワークロードに:
    ...
    1. ワークロードの **Overview** ページから、ダッシュボードを選択します。
    2. 検索語を追加してダッシュボードをフィルタリングし、そのワークロードに関連するビューを表示します。
    3. 選択 **このワークロードのフィルタを保存**.
  </Collapser>
</CollapserGroup>
