...
<Callout variant="important">
  自動関係は、エンティティによってレポートされるテレメトリーに基づいて作成されます。これらの関係には、生存時間（TTL）があります。つまり、関係の作成に使用されるメトリクスが一定期間内に報告されない場合、自動的に削除されます。関係のデフォルトのTTLは<b>75分</b>です。TTLは、<b>関係のタイプに応じて10分から最大3日まで</b>変動します。
</Callout>
