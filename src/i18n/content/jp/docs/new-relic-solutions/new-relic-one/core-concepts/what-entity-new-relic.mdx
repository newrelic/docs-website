...
エンティティフィルターバーは、メインエンティティリストビュー、APM、ブラウザモニタリングUI、InfrastructureモニタリングUIなどを含む、いくつかのNew Relicエクスペリエンスで使用できます。これを使用してエンティティをフィルタリングし、ビューを保存する方法については、[エンティティのフィルタリング](/docs/new-relic-solutions/new-relic-one/core-concepts/search-filter-entities)を参照してください。
...
* エンティティのGUID、およびその他のメタデータを検索するには：[エンティティのリスト](/docs/new-relic-solutions/new-relic-one/core-concepts/new-relic-explorer-view-performance-across-apps-services-hosts#find)から、エンティティの

  <Icon name="fe-more-horizontal"/>

  アイコンをクリックし、**メタデータとタグを表示**をクリックします。
...
* エンティティタイプに関する技術的な詳細については、[GitHubリポジトリのエンティティドキュメント](https://github.com/newrelic/entity-definitions/tree/main)を参照してください。エンティティタイプの`definition`ファイルには、次のような情報が表示されます。
