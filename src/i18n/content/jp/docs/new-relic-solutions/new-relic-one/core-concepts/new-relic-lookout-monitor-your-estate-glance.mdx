...
特定のイベント属性を視覚化して分析するには、 **Show detailed view** \[詳細ビューの表示]を切り替えます。
...
\[トレース] タブは、エンティティに分散トレースが構成されており、選択した評価時間枠で使用できる場合にのみ表示されます。各カードにはトレースの概要が表示され、クリックすると詳細が表示されます。選択したエンティティにフィルターされた、[ 分散トレース アプリケーション](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui) への すべてのトレース** Explore all traces** \[リンクを探索します ]。