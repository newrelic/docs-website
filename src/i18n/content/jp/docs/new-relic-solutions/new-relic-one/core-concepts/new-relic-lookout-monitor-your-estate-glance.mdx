...
New Relic Lookout を使用すると、デジタル資産全体を可視化できます。追加の構成なしで資産内のすべてをクエリし、現在最も重要なデータを深く掘り下げます。
...
<figcaption>
  New Relic Lookout を使用すると、どこに注意を向けるべきかを正確に把握できます。色が明るいほど変化が激しく、円のサイズが大きいほど信号の偏差が大きくなります。次に、相関関係と異常な履歴をさらに掘り下げて、システム全体にどのように影響するかを確認します。構成は必要ありません。
</figcaption>
...
* New Relic Lookout には、 [Pro または Enterprise エディション](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#editions)が必要です。
...
デフォルトのビューは、アプリケーションごとに分類された3つの主要なサービスパフォーマンス指標（**スループット**、**応答時間**、および**エラー**）に関する洞察を提供します。これらのメトリックを分析して、過去5分間のデータの動作を前の1時間と比較して示します。
...
各アプリケーション (またはその他のファセット) は円で表されます。円のサイズは、そのアプリケーションの信号の偏差の大きさを示します。円の色は、前の 1 時間の標準偏差に基づいて、過去 5 分間に値が大幅に減少したか増加したかを示します (デフォルトの評価および比較時間ウィンドウ)。エンティティは、サイズ、色、および偏差率によって並べ替えられます。一度に最大 500 個のエンティティが表示されます。データをフィルタリングして、分析に焦点を当てることができます。
...
ターゲットがNewRelicアプリケーションの場合、関連する機能へのリンクとともに、上位のトランザクション、エラークラス、外部サービス、データベース操作を表示します。
...
New Relic Lookoutは、そのエンティティタイプに対して選択されたシグナルとほぼ同時に異なる動作を開始した他のシグナルを見つけることができます。シグナルの名前をクリックすると、その主要業績評価指標に焦点を合わせて分析が再実行されます。アカウントに関連付けられたエンティティがある場合、チャートのタイトルはNewRelicにリンクします。現在、相関関係はアカウント間で分析されないことに注意してください。表示される相関信号は、表示している元のアプリケーションのアカウントの他の人のためのものです。これは、問題に関連している可能性が高い相関関係にデータを集中させるのに役立ちます。
...
6. New Relic Lookout が分析するデフォルトのタイムウィンドウは、前の 1 時間と比較した最後の 5 分間です。時間コントロール(**View data from** および **Compare data to**)を使って、他の時間帯を対象にすることができます。
   ...
   <figcaption>
     **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Apps > New Relic Lookout** : New Relic データベース内のすべてのイベントおよびメトリック データは、クエリの**編集機能を使用してクエリ**できます。
   </figcaption>
