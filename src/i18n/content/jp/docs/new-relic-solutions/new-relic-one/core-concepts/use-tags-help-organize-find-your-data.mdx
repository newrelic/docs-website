...
<Callout variant="tip">
  以前、New Relic UIでは、タグは`labels`と呼ばれていました。コード内で`label`が使用されている場合があります。たとえば、当社の一部のAPMエージェント設定ファイルは、タグに対して`label`設定オプションを使用します。
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    NRQLアラート条件に関連付けられたタグを追加や削除することもできます。UIを通して条件タグを追加または削除するには：
  ...
  >
    ...
    * [オンホストインテグレーション](/docs/integrations/host-integrations/host-integrations-list)：一部の属性は、デフォルトでタグとして使用できます。インテグレーション固有の`labels`設定オプションを使用してタグを追加することもできます（例については、[Apacheインテグレーション](/docs/integrations/host-integrations/host-integrations-list/apache-monitoring-integration#config)のラベル設定を参照してください）。
