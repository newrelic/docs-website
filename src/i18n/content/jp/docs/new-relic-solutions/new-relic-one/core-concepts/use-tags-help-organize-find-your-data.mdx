...
<figcaption>
  ここでは、`accountId`タグを使用して特定のアカウントの[ワークロード](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\))をフィルターしています。
</figcaption>
...
* <InlinePopover type="dashboards" />と[ワークロード](\(/docs/new-relic-solutions/new-relic-one/workloads/workloads-isolate-resolve-alert-events-faster\))の整理と検索を改善します。
...
<CollapserGroup>
  ...
  <Collapser id="simple-tags" title="タグをシンプルに">
    ...
    * `regions: EMEA | US | LATAM`などの複数の値を単一のタグに含めることは避けてください。このような場合は、`region: emea`、`region: us`、および`region: latam`などの、3種類のタグを使用することをお勧めします。複数のタグを一致させる場合は、フィルターUIで高度なオプションを使用して行うことができます。
      * [アラートイベント](/docs/alerts-applied-intelligence/overview/#concepts-terms)に関する注意事項：これらは1つのキー名インスタンスのみをサポートします。複数のキー名を使用する場合は、その問題に追加されるキー名がランダムに選択されます
  </Collapser>
