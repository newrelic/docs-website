...
New Relic アプリケーションの構築を支援するために、New Relic One SDK を提供しています。 ここでは、GraphQL インターフェースである[NerdGraph](/docs/apis/nerdgraph/get-started/introduction-new-relic-nerdgraph/)を介してクエリとミューテーションを実行できる SDK クエリ コンポーネントの使用方法を学習できます。
...
* `CHART`: チャート作成エンジンで使用される形式であり、これも公開されています。
