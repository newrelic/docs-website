...
`package.json`、 `package-lock.json` 、および_node_modules は_、JavaScript アプリケーションを実行するために重要ですが、Nerdpacks に固有のものではありません。 これらのモジュールを微調整する必要がある場合は、JavaScript コースで学習できます。 とりあえず、このディレクトリ内で最も関連性の高いファイルの 1 つである`nr1.json`を見てみましょう。
...
ランチャーの`displayName`には「A/Bテスト Launcher」を使用し、 _nr1.json_の`description`には「A/Bテスト Nerdlet を開く」を使用します。
