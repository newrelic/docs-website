...
[output] ⁎  {green}my-awesome-visualization {blue}https://onenr.io/012ab3cd4Ef
```
...
視覚化のインスタンスをダッシュボードに追加するには、まず[Nerdpackを公開する](/docs/new-relic-solutions/new-relic-one/build-nr-apps/publish)必要があります。
...
2. 公開された視覚化を見つけてクリックします。
   ...
   視覚化が見つからない場合は、ブラウザのページを更新する必要があります。この新しいタイルに**\{&apos;/&apos;} Local** \[/ ローカル]ラベルが付いていないことに注意してください。これは、前の手順で公開およびサブスクライブした視覚化を表示しているためです。Nerdpack をまだローカルで提供している場合は、ラベル付きのタイルとラベルなしのタイルの 2 つがここに表示されることがあります。
