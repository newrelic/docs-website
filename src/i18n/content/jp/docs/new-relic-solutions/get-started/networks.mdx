...
ネットワークパフォーマンスや[データセキュリティ](/docs/security/new-relic-security/data-privacy/new-relic-security-protecting-your-data-content)を向上させるため、New Relicは広いIP範囲でCDNとDDoS保護サービスを使用しています。New Relicエージェントでは、お使いのファイアウォールで次のネットワークとポートへの発信接続を許可する必要があります。[IP接続](#data-ingest)を許可リストに追加するには、使用するオペレーティングシステムとファイアウォールに関する特定の手順に従ってください。
...
* `162.247.240.0/22` （プライベートデータセンター）
* `18.246.82.0/25` （AWS、US-WEST-2、2023年7月1日より有効）
* `3.145.244.128/25` （AWS、US-EAST-2、2023年7月1日より有効）

EU地域アカウントのネットワークブロック：

* `158.177.65.64/29` （プライベートデータセンター）
* `159.122.103.184/29` （プライベートデータセンター）
* `161.156.125.32/28` （プライベートデータセンター）
* `3.77.79.0/25` （AWS、EU-CENTRAL-1、2023年7月1日より有効）
