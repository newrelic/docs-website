...
* [**Synthetics**](/docs/synthetics) : [Synthetics](/docs/synthetics) で Web フロントエンドや API を監視します。時には、APMやInfrastructureでオンプレミス環境をインストルメントできない場合があります。例えば、組織のポリシーにより、ファイアウォールの後ろにエージェントをインストールすることが禁止されている場合があります。このような場合には、アプリケーションがWebフロントエンドであれば、Syntheticsを使用してください。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="New Relic Syntheticsモニターの作成"
  >
    New Relic Synthetics は、ウェブサイト、重要なビジネストランザクション、API エンドポイントを監視するための、自動化されたスクリプト可能なツール群です。開始するには [add a monitor](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors#adding-monitors).
...
これらのメトリクスの一部は、 [サービスマップ](/docs/using-new-relic/service-maps/get-started/introduction-service-maps) や、 [APM](/docs/apm/applications-menu/monitoring/apm-overview-page) 、\[ブラウザ](/docs/ /new-relic-browser/getting-started/browser-overview-page-website-performance-summary) **概要** のページで見ることができます。
...
New Relic では、 [データエクスプローラー](/docs/insights/insights-data-sources/default-events-attributes/view-default-data-other-new-relic-products-insights) と [NRQL クエリ言語](/docs/insights/nrql-new-relic-query-language/using-nrql/introduction-nrql) を使って、KPI やビジネス指標のデータを探すことができます。また、 [ダッシュボード](/docs/dashboards/new-relic-one-dashboards/get-started/introduction-new-relic-one-dashboards) を構築して、これらの KPI のパフォーマンスを追跡することができます。
...
デフォルトのインスツルメンテーションでは取得できないデータが必要になった場合、New Relic ではカスタムデータを簡単に取得することができます。
