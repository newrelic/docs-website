...
個人を作成 <InlinePopover type="dashboards"/>次に示すように、アプリケーションごとにそれらのダッシュボードを 1 つの [データ アプリ](/docs/insights/use-insights-ui/manage-dashboards/data-apps-build-collections-linked-dashboards)に収集します。この AWS 生産概要データ アプリには、AWS 生産予算に関連する一連のウィジェットが表示されます。データ アプリは、一連のトピックを段階的に説明し、環境またはサービス全体の明確な概要を提供するプレゼンテーションに最適です。
...
New Relic では、データに対して「異常クエリ」を作成することもできます。これらは、結果にハード リミットを設定せずに作成するクエリです。むしろ、[応用インテリジェンス](https://newrelic.com/platform/applied-intelligence)にパフォーマンス データを機械学習させ、データが異常値から大きく外れると警告します。
...
3. クエリを定義し、最近のパフォーマンスに基づいて、単純なスライダーと視覚化を使用して、制限付きで適用されるインテリジェンスがデータを分析する方法を決定します。
