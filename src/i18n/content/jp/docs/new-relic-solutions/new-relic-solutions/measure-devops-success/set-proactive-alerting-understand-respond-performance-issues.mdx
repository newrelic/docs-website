---
title: プロアクティブアラートの設定：パフォーマンス問題を理解し対応
tags:
  - New Relic solutions
  - New Relic solutions
...
* オプション：追加された[カスタムアトリビュートとイベント](/docs/insights/insights-data-sources/custom-data/send-custom-data-insights)。
...
ただし、SLOを全てアラートにする必要はありません。堅固なアラート戦略では、SLOをシンプルなセットにして、アクションに結びつくアラートにします。New Relicは多くの場合、最も成熟したDevOps顧客は一般に設定しているアラートが少なく、アラートでは自らの顧客の体験でいつ本当に問題が生じているかを示す中心となる一連のメトリックスに重点を置いていることを把握しています。その結果、New Relic DevOps顧客は多くの場合、アラート戦略の一環として[Apdex](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction)を使用し、アラートをユーザー満足度の低下の兆候に関連付けます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        [Syntheticsモニタリングのping](https://jajp-newrelic-docs.onelink-translations.com/docs/synthetics/synthetic-monitoring/getting-started/get-started-new-relic-synthetic-monitoring) を使用してサイトが稼働しているかどうかを判断し、[スクリプトチェック](https://jajp-newrelic-docs.onelink-translations.com/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors)を使用して最も重要な機能（チェックアウトカートなど）が動作していることを確認します。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [主要ハードウェア、仮想ホスト、コンテナ](https://jajp-newrelic-docs.onelink-translations.com/docs/infrastructure/infrastructure-monitoring/get-started/get-started-infrastructure-monitoring)、[ネットワーク](https://github.com/newrelic/nri-network-telemetry)、ストレージコンポーネントに対するKPIを設定します。
      </td>
    </tr>
...
![devops向けのプロアクティブなベースラインアラート](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/proactive-baseline-alerts-devops_0.png "proactive-baseline-alerts-devops.png")
...
![プロアクティブなベースラインアラート - Browserの閾値](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/proactive-baseline-alerts-thresholds.png "proactive-baseline-alerts-thresholds.png")
...
![outlier-alert.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/styles/inline_660px/public/thumbnails/image/outlier-alert.png?itok=mnd-aHPG "outlier-alert.png")
...
通知をオフにしたり無視したりするようにチームをトレーニングしていないことを確認するには、アラートを有効で最新の状態に保ちます。New Relicを使用してアプリケーションとインフラストラクチャのパフォーマンスを最適化し、パフォーマンスの向上に合わせて[New Relic Alerts](/docs/alerts)のポリシー条件を厳しくしていくことができます。アラートを最小限に抑えるには、[Incident Intelligence&#x7B;](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence)などのNew Relic Applied Intelligence機能を活用します。この機能は、関連するアラートを機械学習と手動入力を組み合わせた1つの解決可能な問題に関連付けます。アラートが正しく相関していることを確認し、最も正確なインシデント相関を取得するようにシステムをトレーニングします。

これ以外にも、アラートノイズを低減する方法には、フラップの検出と抑制があります。問題が「フラップ」している場合、問題はオープン状態と解決済み状態の間を循環し、循環するたびに新しいアラートが作成されます。[これらを効果的に処理する](https://blog.newrelic.com/product-news/how-to-use-decisions-applied-intelligence/)ことで、チームに送信されるアラートの総数を減らすことができます。最後に、メンテナンスをスケジュールする場合は、アラート[ミュートルール](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/new-relic-alerts/alert-notifications/muting-rules-suppress-notifications)を利用して、不要なアラートを抑制します。

アラートを有効な通知に調整したので、異常を事前に追跡して、問題がインシデントになる前に修正することができます。異常の[プロアクティブ検知](https://jajp-newrelic-docs.onelink-translations.com/docs/alerts-applied-intelligence/applied-intelligence/proactive-detection/proactive-detection-applied-intelligence)を有効にし、それらに一定レベルの通知を設定します。

![New Relicプロアクティブ検出 - 最近の異常](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/new-relic-recent-anomalies-view.png "new-relic-recent-anomalies-view.png")
...
![事前のベースラインアラート - ダッシュボード](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/proactive-baseline-alerts-dashboards.png "proactive-baseline-alerts-dashboards.png")

<figcaption>
  **[insights.newrelic.com](https://insights.newrelic.com) > All dashboards > （選択したダッシュボード）の順に移動します**
</figcaption>

[NRQL](/docs/insights/nrql-new-relic-query-language/using-nrql/introduction-nrql)を使用してダッシュボードを作成します。詳細な手順については、[Insightsへのアラートデータの送信](https://blog.newrelic.com/2016/03/21/sending-alerts-data-to-insights/)をご覧ください（Insightsはクエリインターフェイスではなくなりましたが、同じコンセプトが適用されます）。
