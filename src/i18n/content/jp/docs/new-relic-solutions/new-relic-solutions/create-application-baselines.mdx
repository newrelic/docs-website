...
* [**合成モニタリング**](/docs/synthetics)：合成モニタリングを使用してWebフロントエンドとAPIを[監視](/docs/synthetics)します。場合によっては、オンプレミス環境をAPMまたはインフラストラクチャでインストルメント化できないことがあります。たとえば、組織のポリシーにより、ファイアウォールの背後にエージェントをインストールすることが禁止されている場合があります。このような場合、アプリケーションにWebフロントエンドがある場合は、ベースラインを確立する機能を提供しながら、エージェント以外の監視を提供する合成監視を使用します。
