...
KPI を定義したら、 [ダッシュボード](/docs/dashboards)で簡単に視覚化できます。<InlinePopover type="dashboards"/>New Relic 製品が収集するすべてのデータを表示する単一の場所を提供します。ダッシュボード データは [イベント](/docs/using-new-relic/welcome-new-relic/get-started/glossary#event)で構成され、各イベントにはイベント タイプ、タイムスタンプ、およびキーと値の [属性が](/docs/using-new-relic/welcome-new-relic/get-started/glossary#attribute)あります。
