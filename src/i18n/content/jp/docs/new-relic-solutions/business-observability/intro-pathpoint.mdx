...
<CollapserGroup>
  ...
  >
    ...
    ステージは<DNT>**Person**</DNT>または<DNT>**Process**</DNT>ステージのいずれかとして定義されます。
  ...
  <Collapser
    ...
    title="タッチポイント"
  >
    タッチポイントは、Pathpoint モデルの最も細かいエンティティです。 これらは多くの場合、<InlinePopover type="apm"/> または <InlinePopover type="browser"/>による特定のアプリケーション モニターですが、任意のNRQLクエリをタッチポイントとして使用することもできます。
    ...
    * 赤: タッチポイントは障害状態にあり、早急な対応が必要な重大な問題があります。
    ...
    <Callout variant="tip">
      デフォルトでは、不健全なタッチポイントのみが表示されます。 パスポイントに関連付けられているすべてのタッチポイントを表示するには、 <DNT>**view all**</DNT>オプションをクリックします。
    </Callout>
    ...
    上の画像は、 <DNT>**Login people (PRC)**</DNT>タッチポイントに関連付けられたクエリを示しています。
  </Collapser>
</CollapserGroup>
