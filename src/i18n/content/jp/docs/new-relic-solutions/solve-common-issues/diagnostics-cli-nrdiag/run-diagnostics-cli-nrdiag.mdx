...
<Callout variant="important">
  システムが外部 IP アドレスに接続し **ない** ように構成されている場合、この方法は機能しません。 代わりに、 [出力ファイルを](/docs/using-new-relic/cross-product-functions/diagnostics-cli-nrdiag/interpret-nrdiag-output) 電子メールに添付して、New Relic サポートに送信してください。 問題がある場合 <InlinePopover type="licenseKey"/>、出力ファイルをメールでサポートに送信する必要もあります。
</Callout>
