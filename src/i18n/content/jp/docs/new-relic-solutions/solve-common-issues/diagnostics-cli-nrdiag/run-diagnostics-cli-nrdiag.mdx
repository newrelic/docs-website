...
<Callout variant="important">
  システムが外部 IP アドレスに接続し**ない**ように構成されている場合、この方法は機能しません。代わりに、 [出力ファイル](/docs/using-new-relic/cross-product-functions/diagnostics-cli-nrdiag/interpret-nrdiag-output)を電子メールに添付して、New Relic サポートに送信してください。[ ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key)に問題がある場合は、出力ファイルをメールでサポートに送信する必要もあります。
</Callout>
