...
これらの個別のウィジェットは、チームダッシュボードとして集約でき、さまざまな開発チームや会社全体と共有できます。ウェブ、モバイルアプリケーションのデータ、バックエンドのデータ、インフラストラクチャのデータ、Syntheticsのモニタリングデータなどを集めて、New Relic全体で単一のカスタムダッシュボードにまとめることができます。
