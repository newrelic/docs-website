...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="ブレッドクラム"
  >
    ブレッドクラムとカスタム属性を使用して、クラッシュをより適切に再現およびデバッグします。たとえば、 [Android](/docs/mobile-monitoring/new-relic-mobile-android/android-sdk-api/recordbreadcrumb)または[iOS](/docs/mobile-monitoring/new-relic-mobile-ios/ios-sdk-api/record-breadcrumb)用の`recordBreadrumb` API を使用して、次のようなクラッシュのデバッグに役立つ状態のセッション中の変化を追跡します。
