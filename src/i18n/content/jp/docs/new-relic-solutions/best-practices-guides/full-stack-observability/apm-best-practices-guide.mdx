...
APMの使い方を知ることも大事ですが、New Relicのアプリケーションパフォーマンス監視ソフトウェアの正確な使い方を知ることも大事です。ここでは、 <InlinePopover type="apm"/>のプロになり、チームにとって重要なメンバーとなるために役立ついくつかのベストプラクティスをご紹介します。

パフォーマンスの問題を解決するステップバイステップのチュートリアルをお探しですか？ [「私のアプリが遅い」のチュートリアル](/docs/journey-app-slow/root-causes/)をご覧ください。
...
* [エンティティ](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#alert-entity)（APMアプリケーション、
...
* アラート通知チャネルを設定するには、[標準的な手順に従ってください](/docs/alerts-applied-intelligence/notifications/intro-notifications)。
...
2. デプロイメント後のパフォーマンスを表示するには、選択したアプリケーションの
...
自動でのエージェントのデプロイ<DNT>**(preferred as a method to avoid errors)**</DNT>：
