...
[ブラウザ監視](/docs/browser/new-relic-browser/getting-started/introduction-new-relic-browser)を使用できるのは、 <InlinePopover type="apm"/>Java の場合、エンドユーザーのロード時間を測定します。このドキュメントでは、セットアップのベスト プラクティスについて説明します。 <InlinePopover type="browser"/>。Java アプリのブラウザ監視を設定する方法については、 [「ブラウザ監視と Java エージェント」](/docs/agents/java-agent/instrumentation/page-load-timing-java)を参照してください。
