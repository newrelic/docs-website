...
<Steps>
  ...
  <Step>
    ...
    1. モニターを追加するには、<DNT>**[one.newrelic.com &gt; All capabilities](https://one.newrelic.com/all-capabilities) &gt; Synthetic Monitoring**</DNT>の順に移動します。
       ...
       リージョンアカウントをお持ちの場合は、該当するURLを使用してください。

       * EU： <DNT>**[one.eu.newrelic.com](https://one.eu.newrelic.com)**</DNT>
  ...
  <Step>
    ...
    失敗しているモニターをすばやく特定するには、[Syntheticモニターのインデックス](/docs/synthetics/synthetic-monitoring/pages/synthetic-monitoring-summary)ページで、未解決のアラートがあるモニター、過去24時間の成功率、問題のある個所の数、モニターの実行間隔、モニタータイプを確認できます。モニターをクリックすると、[概要ページ](/docs/synthetics/new-relic-synthetics/pages/synthetics-overview-page-view-monitors-performance)が表示されます。概要ページでは、モニターが失敗している個所やその原因（最後のエラーメッセージ、エラーレスポンスコード、ドメイン別の所要時間）などを調べるための情報を確認できます。この情報を活用することで、経時的なアプリケーションのパフォーマンスに関する深い洞察が得られます。
