...
この情報の高レベルの情報を取得するには、[モニターインデックス](/docs/new-relic-one-monitors-index)を使用して、モニターの  
概要を表示したり、簡易モニターとスクリプト化されたモニターの[概要](/docs/synthetics/new-relic-synthetics/pages/synthetics-overview-page-view-monitors-performance)を取得したりできます。これらのページにより、時間の経過に伴うアプリケーションのパフォーマンスに関するより深い洞察を得ることができます。
