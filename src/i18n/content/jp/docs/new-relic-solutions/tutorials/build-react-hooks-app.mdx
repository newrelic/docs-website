...
`nr1` CLI バージョン 2.49.1 以降では、 [React フック](https://react.dev/reference/react/hooks)を使用して New Relic アプリケーションとカスタム視覚化を構築できます。このガイドでは、React フックの実際の動作に関する Nerdlet の例をいくつか紹介します。
