...
<Steps>
  <Step>
    ...
    `nr1.json` **Circular progress bar** \[円形プログレスバー]視覚化の[メタデータ ファイル](/explore-docs/custom-viz/configuration-options/)です。 このファイルを使用して、 `VictoryPie`の`colorScale`フィールドに対応する構成可能な`colorScale`オプションを追加します。
  </Step>

  <Step>
    [`collection`](/explore-docs/custom-viz/configuration-options/#collection)[`string`](/explore-docs/custom-viz/configuration-options/#string)グラフの色をカスタマイズするには、 のフィールドのうち を追加します。
  ...
  <Step>
    ...
    これにより、バーの角が丸くなるのではなく、デフォルトの 90 度に戻ります。 ローカル サーバーが実行中の場合、 _index.js_への変更は自動的に認識されます。 したがって、更新を確認するには、 browserで視覚化を表示してください。
