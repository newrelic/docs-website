...
Kubernetesインテグレーションをインストールする最も近道は、自動化されたインストーラを使用してマニフェストを生成することです。生成されたマニフェストは、インテグレーションDaemonSetsのみならず、[Kubernetesイベント](/docs/integrations/kubernetes-integration/kubernetes-events/install-kubernetes-events-integration)や[Prometheus OpenMetrics](/docs/integrations/prometheus-integrations/get-started/new-relic-prometheus-openmetrics-integration-kubernetes)、[New Relicログモニタリング](/docs/logs)など、その他のNew Relic Kubernetes製品の設定もバンドルします。
...
New Relicアカウント[がEU地域](/docs/using-new-relic/welcome-new-relic/get-started/our-eu-us-region-data-centers)にある場合は、[one.eu.newrelic.com](https://one.eu.newrelic.com/launcher/nr1-core.settings?pane=eyJuZXJkbGV0SWQiOiJrOHMtY2x1c3Rlci1leHBsb3Jlci1uZXJkbGV0Lms4cy1zZXR1cCJ9)からインストーラにアクセスします。
...
<CollapserGroup>
  <Collapser
    ...
    title={<><img src="https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/eks.png" alt="EKS" style={{ verticalAlign: 'middle' }}/> <Link to="#install-amazon-eks"> Amazon EKS</Link></>}
  >
  ...
  <Collapser
    ...
    title={<><img src="https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/cke.jpeg" alt="CKE" style={{ verticalAlign: 'middle' }}/> <Link to="#install-google-kubernetes-engine"> Google Kubernetes Engine （GKE）</Link></>}
  >
  ...
  <Collapser
    ...
    title={<><img src="https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/openshift.png" alt="OpenShift" style={{ verticalAlign: 'middle' }}/> OpenShiftコンテナプラットフォーム</>}
  >
    ...
    1. `<release_name>-newrelic-infrastructure`サービスアカウントを特権[Security Context Constraints](https://docs.openshift.com/enterprise/3.0/admin_guide/manage_scc.html)に追加します：
       ...
       system:serviceaccount:<mark><namespace></mark>:<release_name>-newrelic-infrastructure
       ```

       インストーラーによって提供されるデフォルトの`<release_name>`は`nri-bundle`です。
  ...
  <Collapser
    ...
    title={<><img src="https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/aks.png" alt="AKS" style={{ verticalAlign: 'middle' }}/> Azure Kubernetes Service (AKS)</>}
  >
  ...
  <Collapser
    className="freq-link"
    id="install-pks"
    title={<><img src="https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/pivotal.png" alt="PKS" style={{ verticalAlign: 'middle' }}/> Pivotal Container Service (PKS / VMware Tanzu)</>}
  >
    PKSにディプロイする場合、[自動化されたインストーラ](https://one.newrelic.com/launcher/nr1-core.settings?pane=eyJuZXJkbGV0SWQiOiJrOHMtY2x1c3Rlci1leHBsb3Jlci1uZXJkbGV0Lms4cy1zZXR1cCJ9)を使用することを推奨します。使用しない場合、[Helmを使用してKubernetesインテグレーションをインストール](https://jajp-newrelic-docs.onelink-translations.com/docs/integrations/kubernetes-integration/installation/install-kubernetes-integration-using-helm)に記載の手動についての説明に従います。
  </Collapser>
</CollapserGroup>
...
2. マニフェストファイルをダウンロード：
   ...
   ```
3. マニフェストの`DaemonSet`部分で、[New Relicライセンスキー](https://jajp-newrelic-docs.onelink-translations.com/docs/accounts-partnerships/accounts/account-setup/license-key)とクラスタ名を追加して、Kubernetesクラスタを識別します。**両方の値が必要です。**

   * **推奨事項**：マニフェストの`NRIA_PASSTHROUGH_ENVIRONMENT`または`NRIA_DISPLAY_NAME`の値は変更しないでください。
   * `YOUR_CLUSTER_NAME`は、New RelicのエンティティエクスプローラーでのクラスターのIDです。ご使用の環境で実行されているクラスターの名前と一致する必要はありません。
   * YAMLキーのパス：`spec.template.spec.containers.name.env`
   ...
   ```
4. マニフェストを実際の環境に合わせる必要がある場合、本ドキュメントの[設定](#configure-the-integration)セクションを確認してください。
...
2. New Relic `DaemonSet` ポッド用の[PriorityClass](https://kubernetes.io/docs/concepts/configuration/pod-priority-preemption/#priorityclass)を作成します。
   ...
   * デフォルトでは、`preemptionPolicy`は`PreemptLowerPriority`に設定されます。これにより、この優先度クラスが割り当てられたNew Relicポッドは、リソースを消費している優先度の低いポッドを削除できます。
...
<CollapserGroup>
  ...
  >
    ...
    3. マニフェストで、 [New Relicライセンスキー](/docs/accounts-partnerships/accounts/account-setup/license-key)とクラスタ名を追加して、ご自分のKubernetesクラスタを識別できるようにします。**両方の値が必要です。**
       ...
       ```
       env: - name: NRIA_LICENSE_KEY value: <var>YOUR_LICENSE_KEY</var> - name: CLUSTER_NAME value: <var>YOUR_CLUSTER_NAME</var>
       ```
...
## インテグレーションを設定する [#configure-integration]
...
<CollapserGroup>
  ...
  >
    デフォルトでは、ポッドで実行するプロセスについてのデータは、New Relicに送信されません。[`enable_process_metrics`](https://jajp-newrelic-docs.onelink-translations.com/docs/infrastructure/install-configure-manage-infrastructure/configuration/infrastructure-configuration-settings#enable-process-metrics)を`true`に設定して、有効にできます。
  ...
  <Collapser
    ...
    title="環境変数を使用する"
  >
    URLの設定にプロキシを使用する場合は、Kubernetesインテグレーションに渡すことができる[環境変数](https://jajp-newrelic-docs.onelink-translations.com/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent#Environment_Variables)を使用します。
  </Collapser>
  ...
  >
    Kubernetesコントロールプレーンのコンポーネントが、[デフォルト](https://jajp-newrelic-docs.onelink-translations.com/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring#discover-nodes-components)と異なる基本URLにあるメトリックスをエクスポートする場合、次の環境変数を使用します。これは、コントロールプレーンのコンポーネントのメトリックスのエンドポイントがSSLまたは代わりのポートを使用している場合、[OpenShift](http://learn.openshift.com/?extIdCarryOver=true&sc_cid=701f2000001OH7iAAG)などの環境で必要です。
    ...
    <Callout variant="important">
      カスタムの基本URLを所定のコントロールプレーンのコンポーネントについて定義しても、コントロールプレーンのコンポーネントのポッドに、[自動発見プロセスでサポートされたラベル](https://jajp-newrelic-docs.onelink-translations.com/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring#discover-nodes-components)の1つを含める必要があります。
    </Callout>

    <Callout variant="important">
      カスタムの`ETCD_ENDPOINT_URL`を定義できますが、ETCDでは常に`https`と[mTLS認証を設定する](https://jajp-newrelic-docs.onelink-translations.com/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring#mtls-how-to)必要があります。
    </Callout>
  </Collapser>
</CollapserGroup>
...
Kubernetesインテグレーションイメージの設定はエージェントのデフォルト設定ですが、これは必要に応じて変更できます。マニフェストでインストールする場合、マニフェストを編集し、`newrelic-infrastructure` DaemonSetの環境変数としてとして、必要な[エージェントの設定オプションを追加することで、infrastructureエージェント設定](docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings)を変更できます。

Helmでインストールする場合、[GitHubの例](https://github.com/newrelic/helm-charts/blob/6a8b2e5f3a9d7b5d6527cfab306a16c9c1378542/charts/newrelic-infrastructure/values.yaml#L120)に示されているように、`values.yaml`で必要な[infrastructureエージェント設定オプション](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings)を指定できます。

`config`オブジェクトを使用して、`configMap`を投入します。これは、`newrelic-infrastructure` DaemonSetによって作成されたポッド内のinfrastructureエージェント設定ファイルの場所に自動的に組み込まれます。
