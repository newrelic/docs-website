...
2. マニフェストファイルをダウンロード：
   ...
   ```
3. マニフェストの`DaemonSet`部分で、[New Relicライセンスキー](https://docs.newrelic.co.jp/docs/accounts-partnerships/accounts/account-setup/license-key)とクラスタ名を追加して、Kubernetesクラスタを識別します。**両方の値が必要です。**
...
<CollapserGroup>
  ...
  >
    デフォルトでは、ポッドで実行するプロセスについてのデータは、New Relicに送信されません。[`enable_process_metrics`](https://docs.newrelic.co.jp/docs/infrastructure/install-configure-manage-infrastructure/configuration/infrastructure-configuration-settings)を`true`に設定して、有効にできます。
  ...
  <Collapser
    ...
    title="環境変数を使用する"
  >
    URLの設定にプロキシを使用する場合は、Kubernetesインテグレーションに渡すことができる[環境変数](https://docs.newrelic.co.jp/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent)を使用します。
  </Collapser>
  ...
  >
    Kubernetesコントロールプレーンのコンポーネントが、[デフォルト](https://docs.newrelic.co.jp/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring)と異なる基本URLにあるメトリックスをエクスポートする場合、次の環境変数を使用します。これは、コントロールプレーンのコンポーネントのメトリックスのエンドポイントがSSLまたは代わりのポートを使用している場合、[OpenShift](http://learn.openshift.com/?extIdCarryOver=true&sc_cid=701f2000001OH7iAAG)などの環境で必要です。
    ...
    <Callout variant="important">
      カスタムの基本URLを所定のコントロールプレーンのコンポーネントについて定義しても、コントロールプレーンのコンポーネントのポッドに、[自動発見プロセスでサポートされたラベル](https://docs.newrelic.co.jp/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring)の1つを含める必要があります。
    </Callout>

    <Callout variant="important">
      カスタムの`ETCD_ENDPOINT_URL`を定義できますが、ETCDでは常に`https`と[mTLS認証を設定する](https://docs.newrelic.co.jp/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring)必要があります。
    </Callout>
  </Collapser>
</CollapserGroup>
