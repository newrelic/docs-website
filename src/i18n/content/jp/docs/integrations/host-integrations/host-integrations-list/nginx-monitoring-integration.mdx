...
NGINX[インテグレーション](/docs/integrations/host-integrations/getting-started/introduction-host-integrations)では、ご利用のNGINXデータベースからインベントリとメトリックスを収集して当社のプラットフォームに送信します。このプラットフォームで、接続やクライアントのリクエストについてのデータを確認し、問題の原因を見つけることができます。
