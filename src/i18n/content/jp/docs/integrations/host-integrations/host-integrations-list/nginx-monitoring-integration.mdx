...
環境変数は、ライセンスキーなどの構成設定の管理に使用された後、Infrastructureエージェントにパススルーされます。この機能の利用手順に関しては、[Infrastructureエージェントを設定する](https://docs.newrelic.co.jp/docs/infrastructure/new-relic-infrastructure/configuration/configure-infrastructure-agent)を参照してください。
