...
* MySQLがKubernetesまたはAmazon ECSで実行されて**いない**場合、LinuxまたはWindows OSホストに[Infrastructureエージェントをインストール](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic)できます。その他の場合：
