...
MySQL [インテグレーション](https://jajp-newrelic-docs.onelink-translations.com/docs/integrations/host-integrations/getting-started/introduction-host-integrations)では、ご利用のMySQLデータベースからインベントリとメトリックスを収集して当社のプラットフォームに送信します。このプラットフォームで、ご利用のデータベースサーバの健全性を確認し、メトリックデータを分析して、問題の原因を簡単に見つけることができます。
...
<CollapserGroup>
  ...
  >
    ...
    3. インテグレーションのディレクトリ`C:\Program Files\New Relic\newrelic-infra\integrations.d\`で、以下を実行してサンプル設定ファイルのコピーを作成します。
       ...
       管理者コマンドラインから、 [レプリケーション権限](https://dev.mysql.com/doc/refman/5.7/en/privileges-provided.html#priv_replication-client)付きのユーザーを作成します。
       ...
       ```
    4. ディレクトリをインテグレーションのフォルダに変更します。
    5. [環境設定](#config)で説明したとおり、`mysql-config.yml`ファイルを編集します。
    6. [Infrastructure エージェントを再起動します](https://jajp-newrelic-docs.onelink-translations.com/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status)。
