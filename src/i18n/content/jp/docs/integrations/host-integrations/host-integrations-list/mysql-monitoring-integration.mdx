...
MySQL[インテグレーション](https://docs.newrelic.com/docs/integrations/host-integrations/getting-started/introduction-host-integrations)では、ご利用のMySQLデータベースからインベントリとメトリクスを収集して当社のプラットフォームに送信します。このプラットフォームで、ご利用のデータベースサーバの健全性を確認し、メトリックデータを分析して、問題の原因を簡単に見つけることができます。
...
* MySQLをKubernetesまたはAmazon ECSで実行中で**ない**場合は、[Infrastructureエージェントは、MySQLを実行中のLinux OSホストにインストールされている](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic)必要があります。その他の場合：
