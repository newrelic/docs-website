...
3. **Policy**を添付します: `ReadOnlyAccess`を検索し、`ReadOnlyAccess`という名前のポリシーのチェックボックスを選択して、**Next: Review**をクリックします。または、[管理対象ポリシーを独自に作成して](/docs/integrations/amazon-integrations/getting-started/integrations-managed-policies)、監視するAWSサービスに基づいてNew Relicに与えるアクセス許可を制限できます。
