...
* KSM v2+ では、デフォルトでラベル メトリックが無効になっていることに注意してください。Kubernetes クラスターで [ここ](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) で説明されている `metric-labels-allowlist` オプションを使用すると、ターゲット ラベル メトリックの監視を有効にできます。
