...
Kubernetes 上で実行されるサービスがあり、それらのサービス [がインフラストラクチャ エージェント統合](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) (Cassandra、Redis、MySQL、Apache など) のいずれかによって監視できる場合は、Kubernetes 統合を介してそれらの統合を有効にすることができます。

## 始めましょう [#overview]

Kubernetes 統合は、 [インフラストラクチャ エージェント統合](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations)にバンドルされています。これにより、マニフェスト内に ConfigMap として存在する Kubernetes 統合の構成にセクションを追加することで、これらのサービスのデータを取得できるようになります。
...
Kubernetes のモニタリング サービスは [、インフラストラクチャ エージェントの統合](/docs/infrastructure/host-integrations/installation/install-infrastructure-host-integrations) と自動検出メカニズムを利用して、指定されたセレクターを持つポッドをポイントすることで機能します。
