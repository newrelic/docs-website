...
新しい[`nri-kubernetes` v3 アーキテクチャは](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/)まだ Windows をサポートしていないことに注意してください。 `newrelic-infrastructure`チャートはスタンドアロン リリースとしてインストールされ、Windows のログオンを監視できるようになります。
