...
<Callout variant="tip">
  それでもコントロール プレーン データが表示されない場合は、 [Kubernetesシューティング: データが表示されない」](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/kubernetes-integration-troubleshooting-not-seeing-data/)で説明されている解決策を試してください。
</Callout>