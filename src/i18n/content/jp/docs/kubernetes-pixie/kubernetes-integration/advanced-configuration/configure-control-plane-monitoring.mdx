...
<Callout variant="tip">
  それでもコントロールプレーンのデータが表示されない場合は、 [Kubernetesインテグレーションのトラブルシューティングで説明している解決方法を試してみてください。データが見えない](/docs/kubernetes-pixie/kubernetes-integration/troubleshooting/not-seeing-data) 。
</Callout>
