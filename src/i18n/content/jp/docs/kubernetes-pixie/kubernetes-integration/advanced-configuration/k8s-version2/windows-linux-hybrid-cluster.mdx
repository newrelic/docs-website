...
<Callout variant="important">
  New RelicKubernetesインテグレーション v3Windows は、新しい[`nri-kubernetes` v3 アーキテクチャー](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-version2/changes-since-v3/) で監視 ノードをサポートするようになりました。Windows ノードの監視を有効にするには、[ガイド付きインストール プロセスを使用します。](/src/install/kubernetes/intro)このプロセスは Linux ノードとWindowsノードの両方をサポートし、デフォルトでハイブリッド クラスタ インストレーションが可能になります。
</Callout>
