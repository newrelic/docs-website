...
<Callout variant="important">
  **New Relic Kubernetesインテグレーション v3で、Windowsノード監視が一般提供（GA）されました。**このページで説明されているハイブリッドインストレーションアプローチ（Windows用に別の`newrelic-infrastructure` v2チャートを使用）は非推奨となっており、将来のリリースで削除されます。代わりに[ガイド付きインストールプロセス](/install/kubernetes/)を使用してください。デフォルトで、単一のインストレーションでLinuxノードとWindowsノードの両方をサポートしています。
</Callout>
...
<Steps>
  <Step>
    ...
    * このファイルに示されていない他のサブチャートが`nri-bundle`に存在する場合があります。それらのサブチャートを追加する場合は、yamlファイルにそのキーと値のペアを追加します。
