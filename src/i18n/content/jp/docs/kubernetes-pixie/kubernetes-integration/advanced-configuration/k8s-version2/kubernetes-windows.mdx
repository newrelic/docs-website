...
<Callout variant="important">
  **New Relic Kubernetesインテグレーション v3で、Windowsノード監視が一般提供（GA）されました。**このページで説明されているv2のインストレーションアプローチは非推奨となっており、今後のリリースで削除されます。Windowsノードをネイティブにサポートする最新のインストレーション手順については、[Kubernetesインテグレーションのインストレーションガイド](/install/kubernetes)を参照してください。
</Callout>

WindowsベースのKubernetesシステムを使用している場合は、このオプションを使用してください。Windowsインテグレーションにはいくつかの[制限](#k8-windows-limitations)があります。
