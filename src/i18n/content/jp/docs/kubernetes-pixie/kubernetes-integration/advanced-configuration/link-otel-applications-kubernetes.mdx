...
* 次の環境変数を作成しました。
  ...
  * OTEL_EXPORTER_OTLP_ENDPOINT [お住まいの地域または目的に適したエンドポイント](src/content/docs/more-integrations/open-source-telemetry-integrations/opentelemetry/opentelemetry-setup.mdx/#review-settings)
  * NEW_RELIC_API_KEY（[ライセンスキーを取り込む](/docs/apis/intro-apis/new-relic-api-keys/#overview-keys)）

* クラスターにNewRelicの[Kubernetes統合](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure)をインストールしました
