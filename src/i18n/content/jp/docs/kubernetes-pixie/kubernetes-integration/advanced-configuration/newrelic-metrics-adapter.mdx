...
メトリクス・アダプタは、TLS によるセキュアなエンドポイントを介してメトリクスを公開します。

![クラスター内のNew Relicメトリクスアダプターを示す図](src/images/adapter-diagram.svg 'クラスタ内のnewrelic metrics adapterを示す図')

<figcaption>クラスター内のNew Relicメトリクスアダプター。</figcaption>
...
- クラスタに他の External Metrics Adapter がインストールされていないこと。

## インストレーション [#installation]

New Relic Metrics Adapter をインストールするために、 `newrelic-k8s-metrics-adapter` Helm チャートを提供していますが、これは New Relic Kubernetes の全コンポーネントをデプロイする際に使用する `nri-bundle` チャートにも含まれています。

1. まだインストールされていない場合は、 [私たちの Kubernetes インテグレーションをインストールしてください](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#installer) 。
2. 以下のコマンドで New Relic Metrics Adapter を含むインストールにアップグレードします。
...
- `newrelic-k8s-metrics-adapter.personalAPIKey`: 有効な New Relic に設定する必要があります [Personal API key](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#user-api-key).

- `newrelic-k8s-metrics-adapter.accountID`: メトリクスを取得する New Relic の有効なアカウントを設定する必要があります。
...
<Callout variant="tip">
  代わりに、 `values.yaml` ファイルを使用することもできます。このファイルは、
  `--values` フラグを付けて helm
  コマンドに渡すことができます。valuesファイルには、
  [configuration](#configuration)
  のセクションで説明したメトリクスの設定に必要なすべてのパラメータを含めることができます。
</Callout>
...
以下の例は、 `nri-bundle` チャートのインストールでメトリクスアダプタを有効にし、 `nginx_average_requests` メトリクスを設定する Helm の値ファイルです。
...
<Callout variant="caution">
  メトリクスのデフォルトのタイムスパンは1時間です。そのため、 `SINCE`
  句でクエリを定義し、環境やニーズに応じてタイムスパンを調整する必要があります。
</Callout>

次のような外部メトリックを消費する HPA があります。
...
HPA の定義に基づいて、コントローラ・マネージャは外部のメトリクス API からメトリクスをフェッチし、New Relic メトリクス・アダプタによって提供されます。
...
`clusterName` フィルターが、同じアカウントの他のクラスターからのメトリクスを除外するために、クエリに自動的に追加されていることに注意してください。これを削除するには、 `removeClusterFilter` 構成パラメータを使用します。また、 `cacheTTLSeconds` 構成パラメータで定義された期間、値がキャッシュされます。デフォルトは 30 秒です。
