...
<CollapserGroup>
  ...
  <Collapser id="no-metadata-in-transactions-attributes" title="トランザクション属性にKubernetesがありません">
    ...
    1. [「メタデータのインジェクションの検証」](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection)セクションで説明されている手順に従って、環境変数が新しい Pod に正しく挿入されたことを確認します。

    2. それらが存在しない場合は、まず、Webhookサービスのログを監視してください。

       ```shell
       kubectl logs -l app.kubernetes.io/name=nri-metadata-injection -f
       ```

    3. 別のターミナルで新しいポッドを作成し、メタデータ インジェクション デプロイメントのログにエラーがないか調べます。 新しいポッドを作成するには、 [「メタデータのインジェクションを検証する」](/docs/integrations/kubernetes-integration/metadata-injection/kubernetes-apm-metadata-injection#validate-injection)セクションを参照してください。 作成された各ポッドについて、 [ログレベル（logLevel）](https://github.com/newrelic/k8s-metadata-injection/blob/main/charts/nri-metadata-injection/README.md#values)がINFO以下であれば、ログエントリが存在するはずです。
    ...
    7. 次のコマンドの出力を検査して、メタデータ インジェクション セットアップ ジョブが正常に実行されたことを確認します。
