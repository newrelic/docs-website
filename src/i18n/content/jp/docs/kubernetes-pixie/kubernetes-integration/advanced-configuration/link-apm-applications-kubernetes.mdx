...
Kubernetes が`MutatingAdmissionWebhook`と通信するには、マスター ノード (またはクラスターの設定方法に応じて API サーバー コンテナー) が、クラスター内の他のすべてのノードのポッドへのポート 443 での HTTPS トラフィックの出力を許可する必要があります。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="トランザクション属性にKubernetesがありません"
  >
    ### 問題
    ...
    5. ターミナル ウィンドウでこのコマンドを実行して Kubernetes API サーバーへのプロキシを起動し、実行し続けます。

       ```shell
       kubectl proxy --port=8001
       ```

    6. クラスター内に新しいポッドを作成すると、API サーバーが Webhook との通信を試みます。 このコマンドはビジーボックスを作成します。
    ...
    10. APIサーバー ログにも メタデータ インジェクション デプロイメント にもログ エントリがない場合は、Webhook が正しく登録されなかったことを意味します。

    11. 次のコマンドの出力を調べて、メタデータ インジェクション セットアップ ジョブが正常に実行されたことを確認します。

        ```shell
        kubectl get job newrelic-metadata-setup
        ```

    12. ジョブが完了していない場合は、セットアップ ジョブのログを調べます。

        ```shell
        kubectl logs job/newrelic-metadata-setup
        ```

    13. 次のコマンドを実行して、 `CertificateSigningRequest`が承認され発行されていることを確認します。

        ```shell
        kubectl get csr newrelic-metadata-injection-svc.default
        ```

    14. 次のコマンドを実行して、TLS シークレットが存在することを確認します。

        ```shell
        kubectl get secret newrelic-metadata-injection-secret
        ```

    15. CAバンドルが変更中のWebhook構成に存在することを確認します。

        ```shell
        kubectl get mutatingwebhookconfiguration newrelic-metadata-injection-cfg -o json
        ```

    16. <DNT>**Service**</DNT>リソースの`TargetPort` <DNT>**Deployment**</DNT>のコンテナの<DNT>**Port**</DNT>と一致することを確認します。
