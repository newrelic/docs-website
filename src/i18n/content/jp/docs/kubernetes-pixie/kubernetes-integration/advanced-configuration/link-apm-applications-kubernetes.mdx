...
さらに、現在ベータリリースであるPixieを使用した自動テレメトリを使用して、Kubernetesクラスターのモニタリングをすばやく開始できます。このPixieのNewRelicへの統合には、言語エージェントは必要ありません。Pixieを使用した自動テレメトリの詳細については、 [こちら](/docs/full-stack-observability/observe-everything/get-started/get-started-auto-telemetry-pixie)をご覧ください。
