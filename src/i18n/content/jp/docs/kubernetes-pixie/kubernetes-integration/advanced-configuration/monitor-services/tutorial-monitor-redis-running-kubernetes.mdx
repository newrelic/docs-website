...
エラーがない場合は、インフラストラクチャ UI に Redis データが表示されるはずです。Redis ダッシュボードを見つけるには、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Infrastructure > Third party services**に移動し、 **Redis** ダッシュボードを選択します。
