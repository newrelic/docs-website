...
エラーがなければ、インフラストラクチャ UI に Redis データが表示されます。Redis ダッシュボードを見つけるには、 **[one.newrelic.com](https://one.newrelic.com) > Infrastructure > Third party services**に移動し、 **Redis**ダッシュボードを選択します。
