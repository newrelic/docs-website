...
`newrelic-infrastructure`チャートには、 `nrk8s-ksm` 、 `nrk8s-kubelet` 、 `nrk8s-controlplane`の 3 つのコンポーネントがあります。 最初のものはデプロイメントであり、次の 2 つは DaemonSet です。
...
ターゲット メトリックがデフォルトで有効になっていない場合、顧客は KSM でモニターできるようにターゲット メトリックを有効にする必要があります。 たとえば、KSM v2+ では、ラベルと注釈メトリクスがデフォルトで無効になっています。 顧客は、`metric-labels-allowlist` `metric-annotations-allowlist`[](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md)Kubernetesクラスタで、 ここで 説明する または オプションを使用して、ターゲット ラベルとアノテーション メトリックをモニターできるようにすることができます。
...
切り替え可能なすべてのスイッチの詳細は、 [`README.md`チャート](https://github.com/newrelic/nri-kubernetes/blob/main/charts/newrelic-infrastructure/README.md)で確認できます。
