...
ターゲット メトリックがデフォルトで有効になっていない場合は、KSM でターゲット メトリックを監視できるようにする必要があります。たとえば、KSM v2+ はデフォルトでラベルと注釈メトリックを無効にします。お客様は、 [ここで](https://github.com/kubernetes/kube-state-metrics/blob/main/docs/developer/cli-arguments.md) 説明する `metricLabelsAllowlist` または `metricAnnotationsAllowList` オプションを Kubernetes クラスタで使用して、ターゲット ラベルとアノテーション メトリックをモニタリングできるようにすることができます。
