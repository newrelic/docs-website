...
[Kubernetes インテグレーション](/docs/integrations/kubernetes-integration/get-started/introduction-kubernetes-integration)は、GKE、EKS、AKS、OKE、OpenShift などのさまざまなプラットフォームと互換性があります。それぞれのインテグレーションとの互換性は異なります。 このページにはさらに詳しい情報が掲載されています。
...
Windows監視は2つのモードをサポートしています：

* **特権モード**、Windows HostProcessコンテナを使用して完全なノードレベルのメトリクスを収集します特権モードはデフォルトで有効化されています。
* **非特権モード**は、ホストの可視性が制限された状態で標準のコンテナユーザーとして実行されます。

これらのモードの仕組みの詳細と安全な設定に関するガイダンスについては、[特権モードと非特権モード](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode)を参照してください。完全なセットアップ手順については、[Windowsノードのモニター](/docs/kubernetes-pixie/kubernetes-integration/installation/windows)を参照してください。

## 特権モードの要件

デフォルトでは、 New Relic Kubernetesインテグレーション チャートはインフラストラクチャエージェント サイドカーを**特権モード**(`privileged: true`) で実行するように設定します。 このレベルのアクセスは、基盤となるホストの`/proc`および`/sys`ファイルシステムからディープ システム メトリック (ストレージI/O 、物理ネットワーク インターフェース、システム プロセスなど) を収集するために必要です。

クラスタのセキュリティ ポリシー (Pod Security Standards や OpenShift 標準的契約条項など) が特権コンテナを許可していない場合は、この設定を無効にする必要がある場合があります。

特権モードを無効にする方法の詳細な手順と、使用できなくなるメトリックの完全なリストについては、 [「特権モードと非特権モード」](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode)を参照してください。
...
* GKEはWindows`HostProcess`コンテナをサポートしていないため、Google GKEクラスタ上のWindows特権モード。GKE上のWindowsノードは、引き続き[非特権モード](/docs/kubernetes-pixie/kubernetes-integration/advanced-configuration/k8s-privileged-mode/#windows-nodes)でモニターできます。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        OKEはデフォルトのコンテナランタイムとしてCRI-Oを使用します。APMの自動アタッチを設定する際は、エージェントイメージのパスの先頭に`docker.io/`を追加してください。例えば： `image: docker.io/newrelic/newrelic-java-init:latest` 。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        vSphere Supervisor 9.0～9.1（vSphere 8.xおよび9.x）で実行されるVKSバージョン3.xと互換性があります
      </td>
    </tr>
  </tbody>
</table>
