...
インストール方法によっては、[コントロールプレーンの監視](/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring)が利用できないか、追加の構成が必要になる場合があることに注意してください。

例えば：

* 管理対象クラスター（GKE、EKS、AKS）では`API Server`モニタリングのみが利用可能です。これは、 `ETCD` 、 `Scheduler` 、および`Controller manager`コンポーネントのモニタリングに必要なメトリックを公開するエンドポイントが利用できないためです。
* ランチャーの外部コントロールプレーンを監視するには、追加の構成が必要です
