...
<CollapserGroup>
  ...
  <Collapser id="namespace-label-selector" title="名前空間ラベルセレクター">
    ネームスペースレベルで自動インストゥルメントされたポッドを定義します。CRとオペレーターが同じネームスペースにある場合に許可されます。それ以外の場合、このセレクターは空のままにする必要があります。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        [Windows 2022 上の .NET](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2022-init/general)
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        [Windows 2025 上の .NET](https://hub.docker.com/repository/docker/newrelic/newrelic-dotnet-windows2025-init/general)
      </td>
    </tr>
...
configmapを使用する際は、以下の要件に注意してください。

* Javaの設定は`newrelic.yaml`という名前のキーに保存する必要があります。
* `configmap`にアプリケーション名またはライセンスキーを設定することはサポートされていません。
* これらのプロパティを設定するには、環境変数またはカスタムシークレットを使用してください。
...
<Callout variant="tip">
  クラスターが明示的なレジストリプレフィックスを必要とするコンテナランタイム（CRI-Oなど）を使用している場合は、イメージパスの先頭に`docker.io/`追加してください。例えば： `image: docker.io/newrelic/newrelic-java-init:latest` 。
</Callout>
...
* アプリケーションのインサイトを取得し、 [APM概要](/docs/apm/agents/manage-apm-agents/agent-data/triage-run-diagnostics/)ページを使用してまとめイベントを解決します。
...
<CollapserGroup>
  ...
  <Collapser id="setup-operator" title="Windows ノードで Kubernetes APM 自動アタッチを設定できますか?">
    現在、 Kubernetes APM自動アタッチは、 Windows Server 2022 およびWindows Server 2025 ノードで実行されている .NET アプリケーションでのみサポートされています。
  </Collapser>

  <Collapser id="setup-operator" title="Fargate ノードで Kubernetes APM 自動アタッチを設定できますか?">
    EKS Fargateがサポートされています。
  </Collapser>
</CollapserGroup>
