...
6. eAPM エージェント Helm チャートをインストールするには、 `your-custom-values.yaml` `values.yaml`ファイルへのパスに置き換えて、次を実行します。
...
<Callout variant="tip" title="ヒント">
  eAPM Helmチャートには複数の設定オプションが用意されています。 [`value.yaml`](https://github.com/newrelic/helm-charts/blob/master/charts/nr-ebpf-agent/values.yaml)ファイルを確認して、ニーズに合わせてインストレーションを調整します。
</Callout>

## eAPM 搭載の APM UI にアクセスする [#eapm-access]
...
## eAPM エージェントとAPMが連携する仕組み [#eapm-apm]
