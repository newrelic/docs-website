...
サーバー、VM、[特権のない](/docs/infrastructure/install-infrastructure-agent/linux-installation/linux-agent-running-modes)環境に、ガイド付きインストールを使用することをお勧めします。ガイド付きインストールは、Helmを使用しない場合は、必要な値が入力されたHelmコマンド、またはプレーンなマニフェストを提供できます。また、有効にする機能と収集するデータを制御することもできます。マネージドクラウドにインテグレーションをインストールする場合は、先に進む前に、これらの[予備ノート](#cloud-platforms)を参照してください。
