...
<Tabs>
  ...
  <TabsPages>
    <TabsPageItem id="1">
      ...
      <Steps>
          ...
          * newrelic dockerhub（`https://hub.docker.com/u/newrelic`）とGoogleのレジストリ（`registry.k8s.io`）ドメインが許可リストに追加されていることを確認してください。ここは、インストール時にコンテナイメージをプルする場所です。通常、`registry.k8s.io`はリージョンに応じてローカルレジストリドメイン（`asia-northeast1-docker.pkg.dev`など）にリダイレクトされるため、ホワイトリストに追加されるさらなるGoogleレジストリドメインを特定するには、[コマンド](https://kubernetes.io/blog/2023/03/10/image-registry-redirect/#how-can-i-check-if-i-am-impacted)に従う必要があります。
            ...
            <CollapserGroup>
              ...
              <Collapser
                ...
                title={<><img src={kubernetesCke} alt="CKE" style={{ verticalAlign: 'middle' }}>Google Kubernetes Engine (GKE 標準)</img></>}
              >
        ...
        <Step>
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>
                  [ガイド付きインストール](https://one.newrelic.com/marketplace?state=fc4edc3d-8214-a314-7432-3e62d7de43d7)
                </td>
              ...
              <tr>
                <td>
                  [ガイド付きインストール（EU）](https://one.eu.newrelic.com/marketplace?state=fc4edc3d-8214-a314-7432-3e62d7de43d7)
                </td>
        ...
        <Step>
          ...
          インストール UI 内でログ データの詳細をカスタマイズできます。
          ...
          [Pixie は](https://docs.px.dev/about-pixie/what-is-pixie/)、eBPF を使用してテレメトリ データを自動的に収集する、Kubernetes アプリケーション用のオープンソースの可観測性ツールです。クラスターに Pixie がインストールされていないが、 [New Relic プラットフォーム](/docs/kubernetes-pixie/auto-telemetry-pixie/get-started-auto-telemetry-pixie/)で Pixie の強力なテレメトリー データ収集と視覚化を活用したい場合は、 **「Pixie を介したサービスレベルの洞察、フルボディ リクエスト、およびアプリケーション プロファイルを有効にする」をオンにします**。
    ...
    <TabsPageItem id="4">
      ...
      2. `values.yaml`ファイルによる手動構成。このタブでは、その方法について説明します。
    ...
    <TabsPageItem id="5">
      GKE Autopilot クラスタに Kubernetes 統合をインストールするには 3 つの方法があります。
      ...
      <CollapserGroup>
        ...
        >
          上の**「ガイド付きインストール」**タブの指示に従ってください。

          ガイド付きインストールでは、GKE Autopilot クラスタにインストールするかどうかを尋ねられます。`Y`を入力すると、 `privileged` 、 `controlPlane` 、 `logging` 、および`pixie`が`false`に設定されます。インストールでは、kubelet スキームが http に、kubelet ポートが 10255 に設定されます。
        </Collapser>
        ...
        <Collapser
          ...
          title="マニフェスト"
        >
          次のフラグを使用してマニフェスト コマンドを更新します。

          * `privileged` 、 `controlPlane`および`logging`を`false`に設定します。
          * `"newrelic-infrastructure.kubelet.config.scheme":"http"`をペイロードに追加
          * `"newrelic-infrastructure.kubelet.config.port":"10255"`をペイロードに追加
          * ピクシーは現在 GKE Autopilot でサポートされていないため、コマンドからピクシー ペイロードを削除します。
