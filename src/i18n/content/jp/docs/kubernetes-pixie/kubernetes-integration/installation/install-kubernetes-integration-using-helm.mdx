...
5. `--debug`と`--dry-run`を指定せずにコマンドを実行して、Kubernetes統合をインストールします。
   ...
   <Callout variant="important">
     [サポートされている Kubernetes バージョン 1.24.x 以下のバージョンを](/docs/kubernetes-pixie/kubernetes-integration/get-started/kubernetes-integration-compatibility-requirements)使用していることを確認してください。ポッド セキュリティ ポリシーは、バージョン 1.25 以降で削除されました。
   </Callout>
