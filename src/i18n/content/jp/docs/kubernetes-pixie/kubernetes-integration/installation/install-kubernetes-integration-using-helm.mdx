...
5. `--debug`と`--dry-run`を指定せずにコマンドを実行して、Kubernetes統合をインストールします。
   ...
   <Callout variant="important">
     Kubernetes バージョン 1.27.x または [サポートされているそれ以前のバージョンを](/docs/kubernetes-pixie/kubernetes-integration/get-started/kubernetes-integration-compatibility-requirements)使用していることを確認してください。
   </Callout>
