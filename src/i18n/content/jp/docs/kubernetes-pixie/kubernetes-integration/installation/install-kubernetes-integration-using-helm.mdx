...
Helm を使用して Kubernetes 統合をインストールするには、New Relic [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) と Kubernetes クラスターの名前が必要です。

1. [ライセンス キーを](/docs/apis/intro-apis/new-relic-api-keys/#license-key)見つけてコピーします。
...
4. 次のコマンドを実行して、チャートですべてが適切に構成されていることを確認します。`--dry-run`と`--debug`を指定しているため、このステップでは何もインストールされないことに注意してください。
   ...
   * `global.licenseKey=YOUR_NEW_RELIC_LICENSE_KEY`: アカウントの有効な [ライセンス キー](/docs/apis/intro-apis/new-relic-api-keys/#license-key) に設定する必要があります。
