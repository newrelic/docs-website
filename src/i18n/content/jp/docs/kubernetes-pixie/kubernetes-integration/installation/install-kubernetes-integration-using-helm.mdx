...
5. `--debug`と`--dry-run`を指定せずにコマンドを実行して、Kubernetes統合をインストールします。
   ...
   <Callout variant="important">
     Kubernetes バージョン 1.26.x または [サポートされているそれ以前のバージョンを](/docs/kubernetes-pixie/kubernetes-integration/get-started/kubernetes-integration-compatibility-requirements)使用していることを確認してください。Kubernetes 1.27.x は現在、実験的リリース段階にあります。問題が見つかった場合は、support @ newrelic.com までご連絡ください。
   </Callout>
...
微調整できるフラグの完全なリスト ( [Scrape-interval](/docs/new-relic-solutions/observability-maturity/operational-efficiency/data-governance-optimize-ingest-guide#k8s-integration)など) は、チャートのリポジトリにあります。
