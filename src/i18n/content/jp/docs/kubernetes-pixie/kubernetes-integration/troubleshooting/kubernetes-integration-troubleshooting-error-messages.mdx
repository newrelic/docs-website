...
<CollapserGroup>
  ...
  >
    ...
    * ポッドが`ImagePullBackOff`ステータスの場合は、ネットワーク接続を確認して、 [ここに](/docs/kubernetes-pixie/kubernetes-integration/installation/kubernetes-integration-install-configure/#before-start)リストされているドメインからイメージを取得できるようにしてください。
