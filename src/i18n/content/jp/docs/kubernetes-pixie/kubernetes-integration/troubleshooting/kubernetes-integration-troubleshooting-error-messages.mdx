...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="サーバーに接続できません"
  >
    ガイド付きインストール中、以下のような出力エラーは、Kubernetes クライアントと Kubernetes API サーバーの間でネットワーク接続の問題が発生していることを示しています。ガイド付きインストールを再度実行する前に、Kubernetes クライアントが Kubernetes サービスに接続できることを確認してください。
