...
このページでは、Windowsベースのノードで実行する場合のNew RelicのKubernetesインテグレーションの制限事項とトラブルシューティングに関する情報を提供します。
...
`OS Name`と`OS Version`で始まる行を探します。これらの行は、OS情報とビルド番号を提供します。このチェックは環境に互換性があることを確認するためのものです；インストレーション中にこのバージョン情報を指定する必要はありません。
...
Windows用New Relicエージェントは`K8sNodeSample`や`K8sPodSample`などの[Kubernetesデータサンプル](/docs/kubernetes-pixie/kubernetes-integration/understand-use-data/find-use-your-kubernetes-data/#event-types)を送信します。適用される制限は、特権（HostProcess）モードで実行しているか、非特権モードで実行しているかによって異なります。

### 非特権モードの制限事項 [#unprivileged-mode-limitations]

非特権モード（`windows.privileged=false`）で実行している場合、標準のWindowsコンテナの分離により、エージェントがホストリソースに直接アクセスできなくなります。次のサンプルは生成されません：
...
さらに、標準のWindowsコンテナのポッド仕様に`hostNetwork: true`を設定することはできません。これはKubernetesの制限ですが、特権モードでHostProcessポッドを使用する場合には適用されません。

### Windows kubeletメトリクスのギャップ [#kubelet-gaps]

以下の[Kubernetesメトリクス](/docs/kubernetes-pixie/kubernetes-integration/understand-use-data/find-use-your-kubernetes-data/#event-types)は、Windows kubeletが公開していないか、Windowsには適用されないため、モードに関係なくすべてのWindowsノードで利用できません：
