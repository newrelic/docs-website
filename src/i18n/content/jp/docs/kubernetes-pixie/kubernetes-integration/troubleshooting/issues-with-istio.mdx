...
<CollapserGroup>
  ...
  >
    ...
    したがって、このような問題を軽減するには、newrelic 名前空間内の `nri-metadata-injection` ポッドに対する Istio サイドカー インジェクションを無効にすることをお勧めします。これを実現するには、 `nri-bundle`をインストールするときに `helm` コマンドに `--set-string nri-metadata-injection.podLabels."sidecar\.istio\.io/inject"=false` を含めます。
  </Collapser>
</CollapserGroup>