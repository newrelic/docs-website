...
Kubernetes[インテグレーションは、ガイド付きインストール](/install/kubernetes)を使用する場合でも、 [Helm 経由でインストールする](/docs/kubernetes-pixie/kubernetes-integration/installation/install-kubernetes-integration-using-helm)場合でも、デフォルトでインストールされます。
...
1. [イベント ページに移動します](https://one.newrelic.com):
...
3. 使用
   ...
   フィールドを使用します。 特定のイベント セットに焦点を合わせるには、フィルター セットを選択または変更します。
