...
New Relic Insights で `K8sSchedulerSample` イベントをクエリして、Scheduler データを確認します。詳細については、 [Configure control plane monitoring](http://docs.newrelic.com/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring) を参照してください。
...
`K8sEtcdSample` [event](/docs/telemetry-data-platform/understand-data/new-relic-data-types/#events-new-relic) に問い合わせて、ETCDデータを見ることができます。詳細については、 [Configure control plane monitoring](http://docs.newrelic.com/docs/integrations/kubernetes-integration/installation/configure-control-plane-monitoring) を参照してください。
...
New Relic Insights の `K8sEndpointSample` イベントにエンドポイントデータを問い合わせます。
...
New Relic Insights の `K8sHpaSample` イベントに、Horizontal Pod Autoscaler のデータを問い合わせます。
