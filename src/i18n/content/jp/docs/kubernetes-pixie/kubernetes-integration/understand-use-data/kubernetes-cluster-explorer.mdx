...
Kubernetes 統合[をインストール](/install/kubernetes)したら、クラスターで実行されるサービスの計測を開始できます。これを行う方法の詳細については[、Kubernetes で実行されている監視サービス](/docs/integrations/kubernetes-integration/link-apps-services/monitor-services-running-kubernetes/)のページを確認してください。
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes cluster explorer > Events**: すべての Kubernetes イベントを参照してフィルターし、ログとインフラストラクチャ データを詳しく調べます。
</figcaption>
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes cluster explorer > Summary**: トグルをクリックして Kubernetes Navigator UI に戻ります。
</figcaption>
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes**: ポッドをクリックすると、そのステータスと正常性に関する詳細情報が表示され、アプリケーション データとトレース、ログ、イベントがさらに詳しく表示されます。
</figcaption>
...
* [Kubernetes プラグインを使用して収集されたポッドとコンテナのログへのリンク。 ](/docs/logs/enable-logs/enable-logs/kubernetes-plugin-logs)

  <InlinePopover type="logs">[](/docs/logs/enable-logs/enable-logs/kubernetes-plugin-logs)</InlinePopover>

  [New Relicで](/docs/logs/enable-logs/enable-logs/kubernetes-plugin-logs)
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes cluster explorer**: すべての Kubernetes クラスターの属性とデータ ポイントを使用して、クラスター エクスプローラー ビューをフィルターできます。
</figcaption>
...
<figcaption>
  **[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Kubernetes cluster explorer**: タイムピッカーを使用すると、事前定義された複数のタイムスパンを選択できます。データを 1 分ごとに再ロードするには、自動更新チェックボックスをオンにします。
</figcaption>