...
<figcaption>
  NewRelicPixie統合ハイブリッドストレージモデル。NewRelicのUIから両方のPixieデータセットを操作できます。
</figcaption>

New RelicのUIは、短期ストレージと長期ストレージの両方でテレメトリデータを操作する方法を提供します。
...
* HTTP / 2ゴールデンメトリクス（スループット、レイテンシー、エラー率）
* HTTP/2リクエストスパン
