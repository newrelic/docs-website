...
3. 残りのフィールドに入力し、 **Continue** \[続行]をクリックして残りのインストールを完了します。
...
Pixie統合は、NewRelicのKubernetes統合の一部としてインストールされます。Kubernetes統合をアップグレードするには、 [Kubernetesインストールドキュメント](/install/kubernetes#update)をご覧ください。
