...
* クラスタに十分なメモリがあることを確認してください。 Pixieは、デフォルトのインストールのために、クラスター内の各ノードに2Gbのメモリーを必要とします。メモリ使用量を調整するために実行するアクションについては、この[Pixieメモリ管理の概要](/docs/auto-telemetry-pixie/manage-pixie-memory)を確認してください。
