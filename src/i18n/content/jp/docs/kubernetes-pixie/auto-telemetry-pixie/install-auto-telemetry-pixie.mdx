...
* [フルプラットフォームのユーザーであること](/docs/accounts/accounts-billing/new-relic-one-user-management/user-type) 。その他のユーザーに関する要件。

  * 当社の [New Relic One ユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) のユーザーは、Pixie 関連の [能力を持つロールを持つ ](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#capabilities)[グループ](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#groups) に割り当てられていなければなりません 。
  * 当社の [オリジナルユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) のユーザーは、 [制限された](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#roles) 。

さらに、

* データを保護するために取るべき行動については、この [Pixie data security overview](/docs/auto-telemetry-pixie/pixie-data-security-overview) を確認してください。
* 十分なメモリがあることを確認します。Pixie では、クラスタの各ノードに少なくとも 8Gb のメモリが必要です。大規模なクラスターでは、より多くのメモリが必要になる場合があります。
