...
2. ガイド付きインストールに使用するアカウントを選択し、 <DNT>**Continue**</DNT>をクリックします。 注: アカウントが 1 つしかない場合は、このオプションは表示されません。
3. 残りのフィールドに入力し、 <DNT>**Continue**</DNT>をクリックしてインストレーションの残りを完了します。
...
4. <DNT>**Choose install method**</DNT>ページで、 <DNT>**Helm**</DNT>または<DNT>**manifest**</DNT>いずれかを選択します。
   ...
   <CollapserGroup>
     <Collapser className="freq-link" id="helm-method" title="ヘルム方式">
     ...
     <Collapser className="freq-link" id="manifest-method" title="マニフェスト方式">
...
<Callout variant="important" title="EOL通知">
  2022年8月以降、Pixieとのインテグレーションのバージョン1を含むいくつかの機能のサポートを終了します。この移行の簡単な準備方法などの詳細については、[サポートフォーラムの投稿](https://support.newrelic.com/s/hubtopic/aAX8W0000008dL8/july-2022-end-of-life-announcements)を参照してください。
</Callout>
