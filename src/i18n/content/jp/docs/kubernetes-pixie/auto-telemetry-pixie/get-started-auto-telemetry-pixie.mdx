...
New Relic One で Pixie のデータを照会し、ダッシュボードを作成して一目でわかるモニタリングを行うことができます。データモデルとサンプルクエリはこちら [。](/docs/integrations/kubernetes-integration/understand-use-data/auto-telemetry-pixie-data-model)

## [フレイムグラフで使用量の急増を調査 [#flamegraph]](/docs/integrations/kubernetes-integration/understand-use-data/auto-telemetry-pixie-data-model)

[![ピクシーフレームグラフ](./images/flamegraph.png "ピクシーフレームグラフ")](/docs/integrations/kubernetes-integration/understand-use-data/auto-telemetry-pixie-data-model)

[アプリケーションが何を行っているかをすばやく確認できれば、デバッグ作業が格段に楽になります。Pixie のフレイムグラフ表示では、インスツルメンテーション、再デプロイメント、および再コンパイルは必要ありません。これは、Go、C+、Rust などのコンパイル済みの言語で動作します。また、フレイムグラフを見れば、アプリケーションがどの機能に時間を費やしているのか、どこにホットスポットがあるのかが一目瞭然です。フレイムグラフは、ディスク使用量やCPU使用率のように、階層的なリソース使用に特に有効です。フレームグラフの読み方の詳細については、 ](/docs/integrations/kubernetes-integration/understand-use-data/auto-telemetry-pixie-data-model)[the Pixie flame graph docs](https://docs.pixielabs.ai/tutorials/profiler#reading-the-flamegraph) を参照してください。
