...
* テレメトリーデータの短期保存:[\[Pixie によるライブ デバッグ\] タブを](/docs/kubernetes-pixie/auto-telemetry-pixie/understand-use-data/live-debugging-with-pixie)介したトラブルシューティングの強化、およびNew Relicに保存される前のテレメトリー データのサブセットの一時的な保存場所として。
