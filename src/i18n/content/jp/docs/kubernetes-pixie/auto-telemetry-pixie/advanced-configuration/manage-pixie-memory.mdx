...
* <DNT>**Collecting telemetry data**</DNT>
  ...
  : アプリケーション トラフィックや CPU プロファイルなどをトレースします。 これらの値は、処理されるときにどこかのメモリに保存される必要があります。

* <DNT>**Short-term storage of telemetry data**</DNT>

  :[Pixie タブによるライブ デバッグを](/docs/kubernetes-pixie/auto-telemetry-pixie/understand-use-data/live-debugging-with-pixie)介して トレース を実行し、 New Relicに保存される前のテレメトリーデータのサブセットの一時的な保存場所として使用します。
