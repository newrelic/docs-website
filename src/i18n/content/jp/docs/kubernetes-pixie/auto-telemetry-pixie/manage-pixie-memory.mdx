...
ほとんどのクラスターでは、デフォルトの`2Gi`メモリ構成を使用することをお勧めします。アプリケーションポッドの場合、デフォルトの`2Gi`メモリをノードの合計メモリの最大25％に設定することをお勧めします。たとえば、ノードの合計メモリが`4Gi`の場合、 `1Gi`メモリ制限を使用するようにPixieを構成する必要があります。
