...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `service.instance.id k8s.cluster.name k8s.namespace.name k8s.pod.name k8s.container.name`<br/>``<br/>``````<br/>
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <br><br>name = `正規化されたHTTPパス` <br/>`service.name` <br/>`trace.id` </br>`span.id`</br>  
    ...
    <tr>
      ...
      <td>
        ``http.host http.method http.path http.status_code http.url http.user_agent<br/>``<br/>````<br>``</br>
