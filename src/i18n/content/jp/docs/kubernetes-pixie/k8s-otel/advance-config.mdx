...
## Kubernetesフレーバー &amp; プロバイダーとの互換性 [#enable-provider]
...
プロバイダーが GKE Autopilot の場合、特権コンテナはこのプロバイダーではサポートされていないため、 `daemonset.containerSecurityContext.privileged`の値をデフォルト値から変更しないでください。

Amazon EKSを使用する場合は、クラスタのTerraform設定を更新するか、AWS Consoleを使用して、次のいずれかの方法で`resourcedetection`プロセッサを有効にします：

* IMDSホップ制限を増やす：ノードグループの`http_put_response_hop_limit`を2に設定します。
* ポッドアイデンティティの有効化：EKSモジュールに`eks-pod-identity-agent`アドオンを追加します。
...
プロバイダがOpenShiftで、 `receivers.filelog`が有効になっている場合（デフォルト）、ログ記録に必要な権限が付与されるように、 `daemonset.containerSecurityContext`で以下の更新を行ってください。
