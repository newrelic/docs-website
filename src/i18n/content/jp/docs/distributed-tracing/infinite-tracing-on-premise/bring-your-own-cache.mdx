...
| パラメータ                 | タイプ   | デフォルト | 説明                                                                                                                                                        |
| --------------------- | ----- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `connection.address`  | ストリング | *必須*  | Redis connection string (format: `redis://host:port/db`). For cluster mode, use comma-separated addresses (e.g., `redis://node1:6379,redis://node2:6379`) |
| `connection.password` | ストリング | `""`  | Redis password for authentication                                                                                                                         |
...
| パラメータ                           | タイプ | デフォルト   | 説明                                                                         |
| ------------------------------- | --- | ------- | -------------------------------------------------------------------------- |
| `connection.dial_timeout`       | 間隔  | `2s`    | Timeout for establishing new connections to Redis                          |
| `connection.read_timeout`       | 間隔  | `500ms` | Timeout for socket reads. Commands fail with timeout error if exceeded     |
| `connection.write_timeout`      | 間隔  | `500ms` | Timeout for socket writes. Commands fail with timeout error if exceeded    |
| `connection.pool_timeout`       | 間隔  | `3s`    | Time to wait for connection from pool if all connections are busy          |
...
| `connection.conn_max_idle_time` | 間隔  | `1m`    | Maximum time a connection may be idle before being closed                  |
| `connection.conn_max_lifetime`  | 間隔  | `30m`   | Maximum time a connection may be reused before being closed                |
| `connection.max_retries`        | int | `3`     | Maximum number of command retries before giving up                         |
| `connection.min_retry_backoff`  | 間隔  | `100ms` | Minimum backoff between retries                                            |
| `connection.max_retry_backoff`  | 間隔  | `1.5s`  | Maximum backoff between retries (exponential backoff capped at this value) |
...
| パラメータ              | タイプ   | デフォルト  | 説明                                                                             |
| ------------------ | ----- | ------ | ------------------------------------------------------------------------------ |
| `data_compression` | ストリング | `none` | Compression algorithm for trace data. Options: `none`, `snappy`, `zstd`, `lz4` |
...
| パラメータ                     | タイプ   | デフォルト | 説明                                                                                       |
| ------------------------- | ----- | ----- | ---------------------------------------------------------------------------------------- |
| `trace_window_expiration` | 間隔    | `30s` | How long to wait for spans before evaluating a trace                                     |
| `traces_ttl`              | 間隔    | `5m`  | Time-to-live for trace data in Redis                                                     |
| `cache_ttl`               | 間隔    | `30m` | Time-to-live for sampling decisions                                                      |
| `processor_name`          | ストリング | `""`  | Optional processor name for Redis keys and metrics (useful for multi-tenant deployments) |
...
| `in_flight_timeout` | 間隔  | Same as `trace_window_expiration` | Timeout for in-flight batch processing before considered orphaned |
| `recover_interval`  | 間隔  | `5s`                              | How often to check for orphaned batches                           |
...
| パラメータ                  | タイプ  | デフォルト   | 説明                                                           |
| ---------------------- | ---- | ------- | ------------------------------------------------------------ |
| `evaluation_interval`  | 間隔   | `1s`    | How often to check for traces ready for evaluation           |
...
| `ingestion_channel_timeout`  | 間隔    | `500ms`      | Maximum time to wait when sending traces to the ingestion channel. If exceeded, traces are dropped          |
| `ingestion_response_timeout` | 間隔    | `10s`        | Maximum time to wait for a worker to process and respond. Prevents indefinite blocking if workers are stuck |
| `hashing_strategy`           | ストリング | `rendezvous` | Hashing algorithm for partition selection. Options: `rendezvous` (recommended, 3x faster) or `consistent`   |
...
| `policies`   | アレイ | *必須*    | Sampling policy definitions          |
...
| Strategy                   | Load Distribution       | パフォーマンス                                    | Best For                                                                                             |
| -------------------------- | ----------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
...
| ----------------------------------------- | ---------- | ----------------- | ---------------------------------------- |
| **開発**                                    | 2-4        | 2-4               | Minimal overhead, easy debugging         |
...
| リソース                  | Without Compression        | With lz4 Compression (25% reduction) |
| --------------------- | -------------------------- | ------------------------------------ |
