...
<Tabs>
  ...
  <TabsPages>
    ...
    <TabsPageItem id="waterfall-view">
      ...
      * <DoNotTranslate>**Expand all/Collapse all**</DoNotTranslate>: このオプションを使用して、すべてのスパン (インプロセススパンを含む) の表示と折りたたまれたスパンの表示を切り替えます。

      * <DoNotTranslate>**Standard/Manual**</DoNotTranslate>: 標準モードでは、各プロセスの最初のスパンを含む半展開ビューが表示されます。 手動モードでは、展開したものだけが表示されます。
      ...
      * <DoNotTranslate>**Focus on**</DoNotTranslate>: スパンにエラーまたは異常がある場合は、これらのタイプごとにチェックボックスが表示されます。 これらのボックスをチェックすると、ウォーターフォール ビューでは他のタイプのスパンがすべて折りたたまれ、色分けされたエラーまたは異常なスパンのみが表示されます。 これは、問題のあるスパンを簡単に見つける方法です。<img style={{ width: "70%",align: "left" }} title="Screenshot showing the color coding of focus on" alt="Screenshot showing the color coding of focus on" src={distributedtracingFocusonExample}/>

      * <DoNotTranslate>**Maximize**</DoNotTranslate>: これにより、ウォーターフォールを拡大し、エンティティマップを非表示にすることで、画面上により多くのスパンを表示できるようになります。
...
* <DoNotTranslate>**Performance**</DoNotTranslate>
