...
* モニターbrowserアプリとモバイル アプリのトレースが見つかりません。 無限トレースは
  ...
  ではまだ利用できないため、無限トレースが有効なサービスにリクエストを送信しても、これらのサービスからのスパンはトレース ウォーターフォールに表示されません。

構成の競合があるトレースの場合、データの品質が高いため、UI には Infinite Tracing データのみが表示されます。(すべてのトレース データは NRQL 経由でクエリ可能です。)

[インストゥルメンテーション ステータス アプリを使用すると、トレース インストゥルメンテーション](https://one.newrelic.com/dt-setup-assistant)の概要を確認できます。 これを見つけるには、 <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Apps > Distributed tracing instrumentation status**</DNT>にアクセスしてください。
