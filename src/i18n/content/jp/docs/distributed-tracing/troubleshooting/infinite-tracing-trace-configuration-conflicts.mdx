...
* 監視対象のブラウザ アプリとモバイル アプリのトレースが欠落しています。Infinite Tracing はまだモバイルまたはモバイルでは利用できないため、

  <InlinePopover type="browser"/>

  、これらのサービスからのスパンは、Infinite Tracing が有効なサービスにリクエストを行ったときにトレース ウォーターフォールに表示されません。
