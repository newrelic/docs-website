...
* [計測ステータス アプリ](https://one.newrelic.com/dt-setup-assistant) を使用して、サンプリング レートを含むトレース計測の概要を確認します。これは、欠落しているスパンや断片化したトレースを理解するのに役立ちます。これを見つけるには、 **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Apps > Distributed tracing instrumentation status**に移動します。
