...
* EUデータセンター：`https://trace-api.eu.newrelic.com/trace/v1`（その他の[EUエンドポイント](/docs/using-new-relic/welcome-new-relic/getting-started/introduction-eu-region-data-center#endpoints)を参照）。
* JPデータセンター：`https://trace-api.jp.nr-data.net/trace/v1`（その他の[JPエンドポイント](/docs/accounts/accounts-billing/account-setup/choose-your-data-center/#jp-endpoints)を参照）。
* [Infinite Tracing](/docs/understand-dependencies/distributed-tracing/infinite-tracing/introduction-infinite-tracing) : [Trace オブザーバーのセットアップ](/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer)を完了すると、エンドポイントとして使用するカスタム YOUR\_TRACE\_OBSERVER\_URL 値を取得します。Trace API を使用する統合 (たとえば、 [Kamon レポーター](/docs/more-integrations/open-source-telemetry-integrations/kamon/kamon-reporter/#install)) を使用している場合は、そのエンドポイントとの統合を構成する必要があります。また[、トレース サービスのサンプリング](/docs/understand-dependencies/distributed-tracing/get-started/how-new-relic-distributed-tracing-works#tail-based)を調整して、スパンの 100% を送信することもできます。
...
* <DNT>**HTTP status code**</DNT> （同期）。 認証およびリクエスト エラーは、HTTP ステータス コードを介して通知されます。
  ...
  <CollapserGroup>
    <Collapser id="status-codes" title="HTTPステータスコード参照">
      <table>
        <thead>
          <tr>
