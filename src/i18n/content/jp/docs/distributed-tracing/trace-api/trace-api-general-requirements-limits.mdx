...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        **必要。**Trace API には<LicenseKey/>が必要です。これがヘッダーとクエリ パラメータの両方として提供される場合、値は一致する必要があります。
      </td>
    </tr>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              認証エラー。<LicenseKey/>が無効な場合、または Trace API を使用するために必要な資格がない場合に発生する可能性があります。
            </td>
          </tr>
...
* `NrIntegrationError` イベント (非同期)。JSON ペイロードのエラーまたはその他のセマンティック エラーは、<LicenseKey/>がリクエストに関連付けられているアカウントに保存されている[`NrIntegrationError`イベント](/docs/telemetry-data-platform/manage-data/nrintegrationerror)を介して非同期的に通知されます。このタイプのすべてのエラーでは、属性`newRelicFeature`は`Distributed Tracing`になり、 `requestId`はエンドポイント応答からの`requestId`になります。
