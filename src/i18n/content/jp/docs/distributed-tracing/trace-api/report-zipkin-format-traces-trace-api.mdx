...
データをレポートするアカウントの<InlinePopover type="licenseKey" />を取得します。

以下、curlリクエストを実行することになります。その際の注意事項です。

* ライセンスキーのプレースホルダーをライセンスキーに置き換えます。
* Infinite Tracing を使用している場合は、標準エンドポイントの代わりに[YOUR\_TRACE\_OBSERVER\_URL](/docs/understand-dependencies/distributed-tracing/infinite-tracing/set-trace-observer#ui-endpoints)値を使用します。
...
Infinite Tracingを使用している場合、またはEUまたはJP地域のNew Relicアカウントをお持ちの場合、[エンドポイント](/docs/distributed-tracing/trace-api/trace-api-general-requirements-limits#requirements)は異なることに注意してください。
