...
いくつかの期待される属性値は、私たちの分散型トレース UI に、いくつかの特定の [トレースプロパティ](/docs/apm/distributed-tracing/ui-data/understand-use-distributed-tracing-data#span-types) と詳細を表示させます。例えば、 `error.` 接頭辞を持つ属性を持つスパンは、UI がそのスパンにエラーを表示します。別の例として、 `db.` 接頭辞を持つ属性を持つスパンは、UI においてデータストア・スパンとして表示され、そのデータストア・クエリがハイライトされます。

スパンを装飾して、UI の特定のプロパティを表示することができます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ![New Relic の分散型トレーシングのエラーアイコン](src/images/new-relic-distributed-tracing-error-icon.png "new-relic-distributed-tracing-error-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicの分散型トレースの外部スパンアイコン](src/images/new-relic-distributed-tracing-external-icon.png "new-relic-distributed-tracing-external-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relic の分散型トレースのデータストアのスパンアイコン](src/images/new-relic-distributed-tracing-databases-icon.png "new-relic-distributed-tracing-database-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relic分散型トレーシングサービスのアイコン](src/images/new-relic-distributed-tracing-service-icon.png "new-relic-distributed-tracing-service-span-icon.png")
      </td>
    ...
    <tr id="in-process">
      ...
      <td>
        ![New Relic の分散型トレースのインプロセス・スパン・アイコン ](src/images/new-relic-distributed-tracing-inprocess-icon.png "new-relic-distributed-tracing-in-process-span-icon.png")
      </td>
...
- 1 つのスパンが複数のカテゴリーに分類されることがあります。例えば、 `external` は、 `datastore` よりも一般的なカテゴリであるため、あるスパンが `external` と `datastore` の両方に分類されている場合、UI では datastore スパンとして表示されます。

## JSON の例 [#examples]

ここでは、属性を使ってスパンのプロパティを設定する方法を示す JSON の例を紹介します。
