...
また、古いバージョンの APM エージェントを使用しており、分散トレースを使用していない場合は、分散トレースをロールアウトする前に[、APM 機能への影響を](#changes)確認することをお勧めします。
...
* 大規模なモノリシック サービスに APM を使用している場合、トレースごとに多数のサブプロセス スパンが存在する可能性があり、 [APM の制限](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#rules-limits)によってトレースが予想よりも少なくなる可能性があります。これは、APM エージェント インスツルメンテーションを使用して重要でないデータのレポートを無効にすることで解決できます。
