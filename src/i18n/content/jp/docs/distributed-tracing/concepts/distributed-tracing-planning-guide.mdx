...
<CollapserGroup>
  ...
  >
    ...
    <figcaption>
      **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > APM > (アプリを選択) > Monitor > Transactions > (トランザクション追跡を選択)** : これは、分散追跡が有効になる前のトランザクション追跡 UI と、関連するトランザクションへのリンクを示しています。分散トレースを有効にすると、サービスの URL が表示されます。
    </figcaption>
...
* 分散トレースは、リクエスト パス内のサービスからサービスへと[ヘッダー情報を伝達すること](/docs/understand-dependencies/distributed-tracing/get-started/how-new-relic-distributed-tracing-works#headers)によって機能します。一部のサービスは、ヘッダーを自動的に伝播しないプロキシまたはその他の仲介サービスを介して通信する場合があります。その場合、 `newrelic`ヘッダー値がソースから宛先に伝播されるようにプロキシを構成する必要があります。

## ディストリビューティッド（分散）トレーシングの有効化 [#enable]

APM機能への影響を認識し、ロールアウトについて考えたならば、分散型トレースを設定する準備ができています。 [セットアップオプション](/docs/distributed-tracing/concepts/quick-start) をご覧ください。