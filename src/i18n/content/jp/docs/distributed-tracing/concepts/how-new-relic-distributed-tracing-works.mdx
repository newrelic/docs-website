...
[Infinite Tracing](#tail-based)機能を除き、当社のトレーシングツールのほとんどは、ヘッドベースのサンプリングアプローチを使用します。このアプローチでは、トレース内のすべてのスパンが到着する前に、個々のスパンにフィルターを適用します。つまり、スパンを受け入れるかどうかの決定は、フィルタリングプロセスの最初（「ヘッド」）に行われます。このようなサンプリング戦略により、ストレージとパフォーマンスの問題を回避しながら、アクティビティの代表的なサンプルを得ることができます。
...
<CollapserGroup>
  <Collapser id="trace-origin-sampling" title="言語エージェント：適応サンプリング">
    ### 最新のサンプリングとOpenTelemetryとの互換性

    当社の<InlinePopover type="apm" />言語エージェントは、アダプティブサンプリングを利用して、システムパフォーマンスを維持しながら高精度の可視性を提供します。このアプローチにより、システムアクティビティの代表的なサンプルを、ネイティブのNew RelicエージェントやOpenTelemetry（OTel）統合など、さまざまなアーキテクチャにわたって確実にキャプチャできます。

    ### アダプティブサンプリングの仕組み

    ディストリビューティッド（分散）トレーシングの最初のサービスモニター（**トレースの起点**）がサンプリングの決定を担当します。この決定はすべての下流サービスに伝播され、完全なエンドツーエンドのトレースが保証されます。

    New Relicエージェントは、「ターゲットベース」のアダプティブアルゴリズムを使用します。エージェントは、前の1分間のスループットを分析して、現在の1分間のサンプリング確率を計算します。たとえば、サービスが1分あたり100件のトランザクションを処理し、サンプリングターゲットが10の場合、エージェントは10件のトランザクションのうち約1件を選択するように動的に調整します。

    エージェントのデフォルトのサンプリングターゲットとスパン制限を確認するには、エージェント固有のドキュメントを参照してください。
    ...
    * [Ruby](/docs/apm/agents/ruby-agent/configuration/distributed-tracing-ruby-agent)
    ...
    <Callout variant="tip">
      **標準ディストリビューティッド（分散）トレースのみ**：エージェントが1分間に設定された制限を超えるスパンを生成すると、一部のスパンが削除され、 UIで[トレースが断片化されます](/docs/distributed-tracing/ui-data/understand-use-distributed-tracing-ui/#fragmented-traces)。トレースは、サンプリング対象として選択される際にランダムな優先順位が割り当てられるため、複数のエージェントがスパンを削除する必要がある場合、最初に優先順位の低いトレースからスパンを削除して、優先順位の高いトレースをそのままの状態に保とうとします。

      これは、すべてのトレースデータをサンプリングなしで送信する無限トレースには適用されません。
    </Callout>

    ### 高精度のハイブリッド環境

    オープンスタンダードへの取り組みの一環として、New Relicエージェントの最新バージョンは**ハイブリッド互換性**を考慮して設計されています。

    **トレース比率のサポート**：最新のエージェントは、OTelネイティブのサンプリング比率に合わせることができるため、混合環境全体で一貫したサンプリングロジックが可能になります。設定の詳細については、 [OpenTelemetry APIサポート」](/docs/apm/agents/manage-apm-agents/opentelemetry-api-support)を参照してください。

    **上流の決定の保持**：「混合モード」環境（OTel SDK とNew Relicエージェントの両方が存在する）では、トレースの断片化を防ぐために、上流のOTel SDKによって開始されたトレース決定を保持することが優先されます。
  </Collapser>
