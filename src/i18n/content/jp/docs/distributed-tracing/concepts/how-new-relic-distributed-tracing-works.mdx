...
<CollapserGroup>
  ...
  >
    ...
    私たちの <InlinePopover type="apm"/>言語エージェントはブラウザやモバイル監視と組み合わせて使用されることが多く、当社の言語エージェントは [サンプリングを使用します](#trace-origin-sampling)。これは、バックエンド スパンよりも多くのブラウザー スパンとモバイル スパンが存在する可能性が高いことを意味し、その結果、ブラウザー スパンとモバイル アプリ スパンがバックエンド スパンから切断される可能性があります。フロントエンド スパンとバックエンド スパンを含むトレースのクエリに関するヒントについては、 [ブラウザ スパン データの検索 を](/docs/browser/new-relic-browser/browser-pro-features/browser-data-distributed-tracing#find-data)参照してください。
  </Collapser>
</CollapserGroup>
