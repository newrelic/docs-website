...
<CollapserGroup>
  ...
  >
    ...
    <img
      title="以下はデータの流れを示す2つの図です。1つはエージェント用で、もう1つはInfinite Tracingとのインテグレーション用です。"
      alt="以下はデータの流れを示す2つの図です。1つはエージェント用で、もう1つはInfinite Tracingとのインテグレーション用です。"
...
<img
  ...
  alt="New Relicディストリビューティッド（分散）トレーシング構造図"
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ディストリビューティッド（分散）トレーシングでは、トレース内の各作業セグメントは`スパン`で表され、各スパンには[`guid`](/attribute-dictionary/?event=Span&attribute=guid)属性があります。プロセス内の最後のスパンの`guid`は、受信サービス内の作業の最初のセグメントがトレース内のデータを接続するparentId[`属性としてこの`guid](/attribute-dictionary/?event=Span&attribute=parentId)を追加できるように、送信リクエストとともに送信されます。
      </td>
    </tr>
...
<img
  title="プロプライエタリヘッダーで失敗したトレースの図。"
  alt="プロプライエタリヘッダーで失敗したトレースの図。"
...
<CollapserGroup>
  ...
  >
    ...
    <img
      ...
      alt="さまざまなコンポーネント間のトランザクションのトリップを示す図。"
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        APMエージェントインスタンスごとの1分あたりの最大スパン数
      </td>

      <td>
        GoおよびPHP：1000。他のすべてのエージェント：2000。
      </td>
    </tr>
  </tbody>
</table>

Trace API の使用に関連するルールは、 [Trace API の要件と制限](/docs/distributed-tracing/trace-api/trace-api-general-requirements-limits) を参照してください。

その他の New Relic の制限については、 [Limits](/docs/data-apis/manage-data/view-system-limits#limits-ui) をご覧ください。

### トレース限界値超過 [#exceed-limits]

スパンレートの上限を超えると、 [`NrIntegrationError` event](/docs/telemetry-data-platform/manage-data/nrintegrationerror) が発生します。このNRQLクエリでレートリミットメッセージを取得することができます。
