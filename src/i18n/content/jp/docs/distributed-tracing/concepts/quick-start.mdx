...
分散トレーシングの初期設定を行い、必要なデータが得られない場合は、上級者向けの [Infinite Tracing](/docs/distributed-tracing/infinite-tracing/introduction-infinite-tracing) 機能を検討することをお勧めします。また、現在 New Relic APM エージェントを使用していて、分散トレーシングを有効にしたい場合は、 [プランニングガイド](/docs/distributed-tracing/concepts/distributed-tracing-planning-guide) をご覧ください。

始める準備はできていますか？まだお持ちでない場合は、[New Relic アカウントにサインアップしてください](https://newrelic.com/signup)。永久無料です。

ディストリビューショントレースを設定するには、一般的に 3 つのステップを行います。

![分散型トレースを設定するための3つのステップを示す図](src/images/arrow-step-diagram-trans.png '分散型トレースを設定するための3つのステップを示す図。')
...
![サービスの識別方法を示した環境図の例です。](src/images/identify-services.png 'サービスの識別方法を示した環境図の例です。')
