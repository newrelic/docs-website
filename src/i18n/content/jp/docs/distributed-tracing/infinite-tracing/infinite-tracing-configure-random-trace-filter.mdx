...
保持率の値を変更したい場合には、さまざまなユースケースがあります。以下の各ユースケースについて、Infinite Tracing設定Nerdletを使用するか、GraphQLチュートリアルの手順に従って変更を加えることができます。 [トレースオブザーバーでランダムサンプラーを更新します](/docs/apis/nerdgraph/examples/configure-infinite-tracing-graphql#updating-the-random-sampler)。
