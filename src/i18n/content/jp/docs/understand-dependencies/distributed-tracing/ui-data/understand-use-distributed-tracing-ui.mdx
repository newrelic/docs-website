...
3. [トレースの詳細ページ](#trace-details)で、エラーに端を発するリクエストルートに沿ってスパンを確認します。
4. エラークラスとメッセージに注意して、トレースのスパンからサービスに移動し、エラーが高頻度で発生していることを確認できます。

ディストリビューティッド（分散）トレーシングUIでオプションを詳しく調べます。

## ディストリビューティッド（分散）トレーシングUIを表示 [#open-dt-ui]
...
<CollapserGroup>
  ...
  <Collapser
    id="dt-launcher"
    title="すべてのアカウント全体でトレースを表示"
  >
    アクセス可能なすべてのアカウントからトレースを表示する場合、そのUIを表示するには次の2つの方法があります：

    * [one.newrelic.com](https://one.newrelic.com/)で、**データのブラウズ**の上にカーソルを移動し、**トレース**を選択します。

      または
    * [one.newrelic.com](https://one.newrelic.com/)で、**アプリ**をクリックし、**ディストリビューティッド（分散）トレーシング**をクリックします。

    <Callout variant="tip">
      トレースで一部のサービスのアカウントにアクセスできない場合、[こうしたサービスの詳細は難読化されます](/docs/understand-dependencies/distributed-tracing/troubleshooting/missing-trace-data#account-access)。
    </Callout>
  </Collapser>
</CollapserGroup>
...
<Callout variant="important">
  多くの結果を生成するクエリでは、ヒストグラムに誤検出が表示される場合があります。これは、トレースリストにないトレース結果がヒストグラムに表示されていることを示している可能性があります。
</Callout>
...
<table id="span-types">
  ...
  <tbody>
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのサービスアイコン](./imagess/new-relic-distributed-tracing-service-icon.png "new-relic-distributed-tracing-service-span-icon.png")
      </td>
    ...
    <tr id="in-process">
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングの処理中のスパンのアイコン](./imagess/new-relic-distributed-tracing-inprocess-icon.png "new-relic-distributed-tracing-in-process-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのデータストアスパンのアイコン](./imagess/new-relic-distributed-tracing-databases-icon.png "new-relic-distributed-tracing-database-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングの外部スパンのアイコン](./imagess/new-relic-distributed-tracing-external-icon.png "new-relic-distributed-tracing-external-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのブラウザスパンのアイコン](./imagess/distributed-tracing-browser-span-icon.png "distributed-tracing-browser-span-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングの外部スパンのアイコン](./imagess/new-relic-distributed-tracing-lambda-icon.png "new-relic-distributed-tracing-lambda-icon.png")
      </td>
...
<table id="span-types-table">
  ...
  <tbody>
    <tr id="connecting-lines">
      ...
      <td>
        ![New Relic ディストリビューティッド（分散）トレーシングをつなぐ線の画像](./imagess/new-relic-distributed-tracing-lines-image.png "new-relic-distributed-tracing-lines-image.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのエラーアイコン](./imagess/new-relic-distributed-tracing-error-icon.png "new-relic-distributed-tracing-error-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのデータストアスパンのアイコン](./imagess/new-relic-distributed-tracing-anomalous-icon.png "new-relic-distributed-tracing-anomalous-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングの親のないスパンのアイコン](./imagess/new-relic-distributed-tracing-orphan-icon.png "new-relic-distributed-tracing-fragmented-icon.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングの複数のアプリ名のインジケータ](./imagess/new-relic-distributed-tracing-multiapp-icon.png "new-relic-distributed-tracing-multiple-app-names-indicator.png")
      </td>
    ...
    <tr>
      ...
      <td>
        ![New Relicディストリビューティッド（分散）トレーシングのクライアントとサーバーの時間差のインジケータ](./imagess/new-relic-distributed-tracing-client-server-time-indicator.png "new-relic-distributed-tracing-client-server-time-indicator.png")
      </td>
...
<CollapserGroup>
  ...
  >
    ...
    * **アカウントのアクセス権限**. [マスター/サブアカウントの関係](https://docs.newrelic.co.jp/docs/accounts/install-new-relic/account-setup/manage-apps-or-users-sub-accounts)が、アクセスに影響を与えます。アクセスできるのがサブアカウントのみの場合、そのサブアカウントの詳細しか確認できません。マスターアカウントにアクセスできる場合、そのアカウントのサブアカウントの詳細を確認できます。
