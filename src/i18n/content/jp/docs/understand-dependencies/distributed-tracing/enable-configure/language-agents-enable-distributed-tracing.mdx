...
<CollapserGroup>
       ...
       <table>
         ...
         <tbody>
           <tr>
             ...
             <td>
               コピーして後ほど[ステップ3](#configure-agents)で<var>YOUR_TRACE_OBSERVER_HOST</var>として使用します。エージェントを設定します。
             </td>
           </tr>
           ...
           <tr>
             ...
             <td>
               （オプション）保存して後ほど<var>YOUR_TRACE_OBSERVER_URL</var>として[2b. テストデータをトレース オブザーバーに送信する](#test-observer)で使用します。
             </td>
           </tr>
         </tbody>
       </table>
  ...
  >
    ...
    5. テストで、成功を示す`HTTP/1.1 202 Accepted`が返されない場合、次の項目をチェックしもう一度行ってください:
       * Edgeアプリの値**その他のインテグレーションの場合**を<var>YOUR_TRACE_OBSERVER_URL</var>として使用したことを確認します。
       * <var>YOUR_TRACE_OBSERVER_URL</var>について挿入した値の引用符が一重引用符であることを確認します。
...
* 標準のディストリビューティッド（分散）トレーシング
* Infinite Tracing（[ステップ2](#get-endpoints)の<var>YOUR_TRACE_OBSERVER_HOST</var>が必要）
        ...
        <tr>
          ...
          <td>
            ...
            * 設定ファイル（`newrelic.config`）：
              ...
              ```
              <configuration . . . > <distributedTracing enabled="true" /> <infiniteTracing> <trace_observer host="<var>YOUR_TRACE_OBSERVER_HOST</var>" /> </infiniteTracing> </configuration>
              ```
            * 環境変数：
