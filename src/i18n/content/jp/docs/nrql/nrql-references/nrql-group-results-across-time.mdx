...
オプションの`format`問題を持つ関数の場合、受け入れられる値は`string`と`numeric`です。 省略した場合、デフォルトのフォーマット値は`string`になります。
...
<CollapserGroup>
  <Collapser id="facet-time-example" title="月別に集計">
  ...
  <Collapser id="facet-other-examples" title="FACET句を使ったその他のグループ化の例">
  ...
  <Collapser id="timeseries-chart-examples" title="TIMESERIESを指定してチャートを作成する例">
