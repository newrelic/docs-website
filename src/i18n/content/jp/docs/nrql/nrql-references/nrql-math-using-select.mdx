...
クランプ関数は、値の上界もしくは下界を設定します。たとえば、 `clamp_max(duration, 10)`は持続時間を返しますが、これが10を超える場合は10を返します。同様に、`clamp_min(duration, 1)`は1未満の値を一切返しません。
...
<Callout variant="tip">
  これは、何かがNULLもしくは0を返す際にも使用できます。`(zero) OR 1`は0を返し、 `(NULL) OR 1`1を返します。
</Callout>
