...
クラウド統合からのデータ (つまり、[ AWS ポーリング統合](/docs/infrastructure/amazon-integrations/aws-integrations-list) 、[ GCP](/docs/integrations/google-cloud-platform-integrations) 、および[ Azure](/docs/integrations/microsoft-azure-integrations/azure-integrations-list) からのメトリクス)[ を除き、次元メトリクスに基づく NRQL アラート もサポートされています。](/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries)AWS CloudWatch Metric Streams メトリクスは次元メトリクスおよび NRQL として取り込まれます<InlinePopover type="alerts"/>が推奨されます。
