...
<CollapserGroup>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> 期間属性に格納された単純な配列`[100, 110, 90, 100, 105]`を考えてみましょう。

    `getfield(durations, 2)` `90`を返します。

    関数`getfield()`の代わりに角括弧`[ ]`の省略表現を使用できます。

    <DoNotTranslate>**Example:**</DoNotTranslate> `durations[4]` `105`を返します。
  </Collapser>
  ...
  >
    ...
    <DoNotTranslate>**Example:**</DoNotTranslate> 値が`["US", "CA", "UK"]`である単純な配列`countries`を考えてみましょう。
