...
<CollapserGroup>
  ...
  >
    ...
    * パブリックプレビュー中は[メトリックタイムスライスデータ](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data#timeslice-data)はサポートされません。
    ...
    `PREDICT`句のデフォルトの動作については、[NRQL予測](/docs/query-your-data/explore-query-data/use-charts/nrql-predictions/#defaults)を参照してください。トレーニングモデルのハイパーパラメーターを手動で設定し、 `USING`および`BY`キーワードを追加することで、予測をカスタマイズできます。NRQLとその構文の詳細については、[NRQLドキュメント](/docs/nrql/get-started/introduction-nrql-new-relics-query-language)を参照してください。
