...
このデータをクエリするには、`Entity`イベントタイプを使用します（リレーションシップ固有のクエリには`Relationships`または`entityRelationships`を使用します）。
...
<CollapserGroup>
  ...
  <Collapser id="total-tx" title="特定の時点でのエンティティの状態を表示する">
    * **目的**: 開発者は数時間前に発生した集中イベントを調査しており、その特定の時点での特定のホスト (またはコンテナ、アプリケーションなど) の正確な設定または状態を知る必要があります。
