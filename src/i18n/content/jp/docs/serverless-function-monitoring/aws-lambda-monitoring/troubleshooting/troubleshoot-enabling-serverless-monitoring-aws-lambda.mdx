...
* CloudWatch エラー「HTTP エラー 401: 権限がありません。」これは、API キーが正しくないことが原因です。[Configure AWS enable ステップ](/docs/serverless-function-monitoring/aws-lambda-monitoring/get-started/enable-new-relic-monitoring-aws-lambda#connect-aws)の`--nr-api-key`引数は、REST API キーとは異なる[ユーザー キー](/docs/apis/get-started/intro-apis/types-new-relic-api-keys#user-api-key)を受け取ります。
