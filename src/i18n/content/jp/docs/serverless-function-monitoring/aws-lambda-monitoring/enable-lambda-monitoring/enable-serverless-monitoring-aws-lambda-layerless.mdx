...
このページでは、Lambda 関数を手動で計算する方法を学びます。 ランタイム言語別に整理されています。 まだお持ちでない場合は、以下の無料の New Relic アカウントを作成して、今すぐデータの監視を開始してください。
...
<CollapserGroup>
  ...
  >
    ...
    7. オプション: AWS の外部のローカル環境でサーバーレス モードでエージェントを実行するには、環境変数`NEW_RELIC_SERVERLESS_MODE_ENABLED`を`true`に設定します。 (これを AWS Lambda 環境で実行すると、エージェントは自動的にサーバーレス モードで実行されます。 <DNT>**Do not use this variable if you're running in AWS**</DNT> 。)
  ...
  >
    ...
    1. AWS ウェブコンソールの**Lambda**サービスセクションに移動します。 そこから、 New Relicに接続する Lambda関数 を見つけます。
    2. デフォルトの**Code** \[コード]タブで、**Layers** \[レイヤー]セクションまで下にスクロールし、**Add a layer** \[レイヤーの追加]ボタンをクリックします。
    3. [New Relicのレイヤーリスト](https://layers.newrelic-external.com/)に移動し、ドロップダウンリストを使用して、Lambda 関数がホストされているAWSリージョンを選択します。 そこから、アーキテクチャーに一致する`NewRelicLambdaExtension`レイヤーの ARN を見つけます。 オプションは X86 と ARM64 の 2 つあります。 **Copy to clipboard** \[クリップボードにコピー]ボタンを使用するか、ARN 文字列を手動でコピーします。
    4. AWS コンソールフォームの**Specify an ARN** \[ARN の指定]セクションで、New Relic Lambda レイヤーの ARN を貼り付けます。
    5. AWSコンソールフォームで、 **Add** \[追加]ボタンをクリックして、Lambda 関数にレイヤーを追加します。
    6. [.NET エージェント NuGet パッケージを](https://www.nuget.org/packages/NewRelic.Agent)プロジェクトに追加して、Lambda 関数に .NET エージェントを含めます。
    7. AWS Lambda コンソールを使用して、エージェントに [必要な Linux 環境変数](/docs/apm/agents/net-agent/configuration/net-agent-configuration/#net-core-env-variables)を設定します。 ベース パスは、アプリケーションのディレクトリの下の`newrelic`ディレクトリである必要があります。
    8. まだ設定されていない場合は、必要な New Relic 環境変数を設定します。
    ...
    * `NEW_RELIC_APP_NAME`
    ...
    10. プロジェクトを AWS Lambda アカウントに公開します。

    11. オプション: ログ記録を設定するには、AWS コンソールで[`NEWRELIC_LOG_CONSOLE`および`NEWRELIC_LOG_LEVEL`環境変数を](/docs/apm/agents/net-agent/configuration/net-agent-configuration/#log)使用します。

    12. 少なくとも1回Lambdaを呼び出します。これにより、CloudWatchロググループが作成されます。次のステップではこのグループが必要です。
