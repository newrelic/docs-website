...
基本的な例に加えて、ディストリビューティッド（分散）ト レーシング の[例で、ディストリビューティッド（分散）ト レーシング を](https://github.com/newrelic/newrelic-lambda-extension/tree/main/examples/sam/distributedtracing)重要なサーバーレス アプリケーションに統合する方法の例を示します。 これは、Node、Python、 Ruby 、Java 関数を使用して、Lambda 関数を呼び出す可能性のある 2 つの一般的なサービスである SQS と SNS の手動トレース伝播を示しています。
