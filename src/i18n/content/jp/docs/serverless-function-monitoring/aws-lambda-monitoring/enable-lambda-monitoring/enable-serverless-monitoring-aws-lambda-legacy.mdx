...
<CollapserGroup>
  ...
  >
    ...
    9. オプション：サーバーレスモードでのロギングを有効にするには、環境変数を次のように設定します。
       ...
       * `NEW_RELIC_LOG_LEVEL` は、デフォルトでは `info` に設定されており、Lambaで機能ログを送信するときにのみ使用されます。 [その他のレベルの構成](/docs/agents/nodejs-agent/installation-configuration/nodejs-agent-configuration#logging_config)を参照してください。
