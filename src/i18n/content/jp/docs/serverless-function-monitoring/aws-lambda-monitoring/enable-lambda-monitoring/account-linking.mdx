---
title: AWSとNew Relicアカウントをリンクする
...
この統合ステップでは、テレメトリーを New Relic アカウントに送信できるように、New Relic <InlinePopover type="licenseKey"/>も AWS Secrets Manager サービスに保存します。
...
* New Relic アカウント。 管理者ロールまたは

  <DoNotTranslate>**Infrastructure manager**</DoNotTranslate>

  [アドオン ロールが](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model#add-on)必要です。
...
<CollapserGroup>
  ...
  >
    ...
    リージョンを構成するには、次の[環境変数](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.html)を使用してデフォルトのリージョンをオーバーライドします。
...
`newrelic-lambda` CLI は、セキュリティを強化するために New Relic <InlinePopover type="licenseKey"/>を[AWS Secret Manager](https://aws.amazon.com/secrets-manager/)のシークレットとして追加します。
...
<Callout variant="tip">
  ...
  あなたの<InlinePopover type="licenseKey"/>は New Relic に対してあなたを識別し、認証することで、あなたのテレメトリーを New Relic アカウントに関連付けることができます。 テレメトリーを送信する各関数はこの値にアクセスする必要があり、安全に管理する必要があります。 AWS Secrets Manager はこれらの問題を解決します。
...
<CollapserGroup>
  ...
  >
    ...
    次に説明するように、 <InlinePopover type="licenseKey"/>シークレットを手動で構成することを忘れないでください。
    ...
    2. AWS CLI または AWS CloudFormation コンソールを使用して、 `LicenseKey`問題を指定してテンプレートをインストールします。 「INGEST - LICENSE」というラベルが付けられます。 上記のインフラストラクチャ UI で構成したアカウントには、必ず<InlinePopover type="licenseKey"/>を使用してください。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Node、Python、Ruby、Java に必須です。 .NET の場合はオプションです。
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <InlinePopover type="licenseKey"/>は New Relic アカウントに関連付けられており、シークレットはライセンスキーを含む AWS 管理シークレットの名前です。 `NEW_RELIC_LICENSE_KEY`が設定されていない場合は、 `NEW_RELIC_LICENSE_KEY_SECRET`でキーを探します。 キーの内容が正しくフォーマットされていることを確認するには、 [CLI](#rec)を使用してラムダ監視を有効にすることをお勧めします。

            <InlinePopover type="licenseKey"/>の詳細をご覧ください。
          </td>
        </tr>
      </tbody>
    </table>
...
Lambda関数をインストゥルメントされたようにしましたが、New Relic の<DoNotTranslate>**Amazon Web Services -> Lambda functions**</DoNotTranslate>セクションにインストゥルメントされたとして表示されません。
...
すでに 2 つの統合が作成されている場合は、New Relic の<DoNotTranslate>**Infrastructure > AWS**</DoNotTranslate>にある<DoNotTranslate>**Unlink this account**</DoNotTranslate>をクリックして、1 つを選択して保持し、もう 1 つのリンクを解除します。
