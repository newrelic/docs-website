...
パフォーマンスへの影響を最小限に抑えるために、Java と .NET Core では異なるアプローチを採用しました。 New Relic は、これらのランタイム用の[OpenTracing](https://opentracing.io/) SDK を提供します。 このアプローチでは、統合するためにもう少しコードが必要です。 .NET 6 などの新しいランタイムの場合は、 [New Relic と OpenTelemetry を使用して .NET Lambda関数をトレースすること](/docs/serverless-function-monitoring/aws-lambda-monitoring/opentelemetry/lambda-opentelemetry-dotnet/)ができます。
