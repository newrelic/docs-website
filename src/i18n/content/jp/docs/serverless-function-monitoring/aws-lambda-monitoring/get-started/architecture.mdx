...
ログサブスクリプションフィルタを追加して、関数ログを[`aws-log-ingestion`](https://github.com/newrelic/aws-log-ingestion) Lambda関数にパイプ接続することで、そのテレメトリログラインを復元し、より意味深いプラットフォームテレメトリとともにNew Relicに転送できます。
