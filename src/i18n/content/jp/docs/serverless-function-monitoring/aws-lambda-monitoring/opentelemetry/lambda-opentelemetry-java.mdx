...
SAM および CloudFormation テンプレートの場合、関数のプロパティに以下を追加します。`your-license-key-here`を<LicenseKey/>に置き換え、 `otlp.nr-data.net:4317`を地域の New Relic OpenTelemetry エンドポイントに置き換えてください。
...
<Callout variant="important">
  `your-license-key-here`を New Relic<LicenseKey/>に置き換え、 `otlp.nr-data.net:4317`を New Relic リージョンに適したエンドポイントに置き換えます。上記のリストを参照してください。
</Callout>
