...
まず、 [AWS Lambda 用の OpenTelemetry SDK](https://www.nuget.org/packages/OpenTelemetry.Instrumentation.AWSLambda/)と[OTLP エクスポータ パッケージ](https://www.nuget.org/packages/OpenTelemetry.Exporter.OpenTelemetryProtocol)を追加します。
