...
<Callout variant="tip">
  **Lambda Managed Instance（LMI）**：LMI関数ではレイヤー化された計装がサポートされています。他のLambda関数と同様にNew Relic Lambdaレイヤーをアタッチします — LMI関数は`NEW_RELIC_APM_LAMBDA_MODE = true`を必要とせずに自動的にAPMモードで実行されます。
</Callout>
...
<CollapserGroup>
  ...
  <Collapser id="console" title="AWSマニュアル インストゥルメンテーション">
    ...
    5. [New Relicのレイヤーリスト](https://layers.newrelic-external.com/)に移動し、ドロップダウンリストを使用して、Lambda関数がホストされているAWSリージョンを選択します。そこから、Lambda関数のRuby（またはその他の言語の）バージョンとアーキテクチャーに一致するARNを見つけます。オプションは X86 と ARM64 の 2 つあります。**Copy to clipboard** \[クリップボードにコピー]ボタンを使用するか、ARN 文字列を手動でコピーします。

       * Javaを使用する顧客は、**Java lambda監視のための2つの異なるソリューション**を表す、旧規約のJavaレイヤーと新規約のJavaエージェントレイヤーを見つけるでしょう。

         * **レガシーJavaレイヤー**はOpen Tracingに基づいており、2026年4月10日より前に当社のサーバーレス監視を使用する顧客、およびJava 11および/または8を使用する顧客に推奨されます。すべてのarnには部分文字列`NewRelicJava`が含まれています。このようなレイヤーの ARN の例は次のとおりです：

           ```
           arn:aws:lambda:us-east-1:451483290750:layer:NewRelicJava17:21
           ```

         * **新しいJavaエージェントレイヤー**はAPM Javaエージェントをベースにしており、新しいJavaアプリケーションに推奨されます。フルレイヤーと、可視性は低下しますがコールドスタートのパフォーマンスを向上させるSlimレイヤーがあります。**これは、レガシーのOpen Tracingソリューションのドロップイン置換ではありません**。詳細については、[APMエージェントのドキュメント](/docs/apm/agents/java-agent/getting-started/java-agent-approaches-lambda)を確認してください。すべてのarnには部分文字列`NewRelicAgentJava`が含まれています。そのようなレイヤーのARNの例は次のとおりです：
