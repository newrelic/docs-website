...
* [CLI 経由で更新](#update-cli): [CLI ツール](/docs/serverless-function-monitoring/aws-lambda-monitoring/instrument-lambda-function/instrument-your-own/#cli)を使用して Lambda 監視を有効にした場合は、これを使用します。
...
より詳細な[CLI コマンド](https://github.com/newrelic/newrelic-lambda-cli)のリストは GitHub で入手できます。
...
2. `newrelic-log-ingestion`関数をインストールしたリージョンごとに、次のコマンドを実行します。 `YOUR_REGION`リージョン識別子 (例: `us-west-2` ) に置き換えます。
...
1. `YOUR_REGION`ご使用のリージョンに置き換えて、次を実行します (たとえば、 `us-west-2` )。
   ...
   既存の CloudFormation スタックをスタック問題 FunctionRole で作成した場合は、以下のように`--parameter-overrides`で関数ロール名を指定します。
