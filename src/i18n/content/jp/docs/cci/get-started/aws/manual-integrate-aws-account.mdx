...
<Callout variant="important">
  次の手順では、 **AWS マネジメントコンソール**にサインインし、プロセス全体を通してサインインしたままにします。
</Callout>
...
<Steps>
  <Step>
    ...
    1. **S3 service**に移動し、 **Create bucket** \[バケットの作成]をクリックします。
    ...
    4. **タグ (オプション)** : 組織のリソース管理に必要な場合はタグを追加し、 **Create bucket** \[バケットの作成]をクリックします。
  ...
  <Step>
    ...
    1. **Billing Dashboard** &gt; **Cost &amp; Usage Analysis** &gt; **Data Exports**に移動し、 **Create** \[作成]をクリックします。
    ...
    3. **Next** \[次へ]をクリックし、 **Create report** \[レポートの作成]をクリックします。
  ...
  <Step>
    ...
    1. **IAM service** &gt; **Policies**に移動し、**Create policy** \[ポリシーの作成を]クリックします。
    2. **JSON エディター (JSON editor)**に切り替えて、次のポリシーを貼り付けます。
    ...
    3. **Next**\[次へ]をクリックします。
    4. ポリシーに`CCI-Access-Policy` (または任意の名前) という名前を付け、説明を追加して、 **Create policy** \[ポリシーの作成] をクリックします。
  </Step>
  ...
  <Step>
    ...
    1. **IAM service** &gt; **Roles**に移動し、**Create role** \[ロールの作成を]クリックします。
    2. 信頼されたエンティティ タイプ セクションで**Custom Trust Policy** \[カスタム信頼ポリシー]を選択し、次のロール定義を貼り付けます。
       ```json
       {
       "Version": "2012-10-17",
       "Statement": [
    ...
    3. **Next**\[次へ]をクリックします。
    4. [「CCI アクセス用の IAM ポリシーの作成」](#create-iam-policy)で作成したポリシーを添付し、 **Next** \[次へ]をクリックします。
    5. ロールに`CCI-Access-Role` (または任意の名前) という名前を付け、説明 (オプション) を追加して、 **Create role** \[ロールの作成]をクリックします。
    6. ロールの概要ページからロール ARN (Amazon リソース名) をメモします。
  ...
  <Step>
    ...
    2. **Manually integrate your AWS account** \[AWS アカウントを手動で統合する]を選択し、 **Continue** \[続行]をクリックします。
  </Step>
  ...
  <Step>
    ...
    * **接続名**: デフォルト名が提供されます。カスタム名を編集します。
    * **クロスアカウント IAM ロール ARN** : Cloud Cost Intelligence が S3 バケットにアクセスできるようにするには、IAM ロール ARN (Amazon リソース名) を入力します (形式: `arn:aws:iam::<account-id>:role/<role-name>`)。
    * **外部 ID** : 外部 ID をコピーし、AWS の IAM ロール信頼ポリシーに追加します。
    * **データエクスポート名**: データエクスポートファイルの名前を入力します。レポート`cci-ingestion-test`の場合、パスは`cost-and-usage-reports/cci-ingestion-test/data`です。
    * **S3 バケット名**: 正確なバケット名を入力します。最適なパフォーマンスを得るには、 `us-east-2`リージョンを使用してください。
  </Step>
