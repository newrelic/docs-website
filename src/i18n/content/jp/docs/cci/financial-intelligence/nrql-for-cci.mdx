...
<CollapserGroup>
  <Collapser id="cloud-cost-attributes" title="CloudCost プロパティ">
    New Relic で、 **All Capabilities &gt; CloudCost Intelligence**に移動します。一番下のコンソールドロワーを開いて、次のNRQLを入力します。
...
<CollapserGroup>
  <Collapser id="kubernetes-cost-attributes" title="Kubernetes コストイベントタイプ">
    ...
    正確なKubernetesコストを計算するには、コスト計算に`cpu_weightage + memory_weightage`問題を掛けます。 次の例は正しいクエリ形式を示しています。

    ```sql
    SELECT SUM(net_unblended_cost * (cpu_weightage + memory_weightage)) FROM KubernetesCost SINCE '2025-09-01' UNTIL '2025-09-10'
    ```

    このクエリは、 `net_unblended_cost`に CPU とメモリの重み付けの合計を掛けて、リソース使用量に基づいて正確なコスト割り当てを提供します。
