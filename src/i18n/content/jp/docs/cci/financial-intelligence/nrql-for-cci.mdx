...
クエリCloud Cost IntelligenceデータNRQLを使用) を使用してcloud支出を分析および最適化します。 詳細なクエリの属性を持つ 3 つのイベント タイプが利用可能です。
...
<CollapserGroup>
  <Collapser id="cloud-cost-attributes" title="CloudCost プロパティ">
    **All Capabilities &gt; Cloud Cost Intelligence**に移動し、クエリ コンソールを開いて入力します。
...
<CollapserGroup>
  <Collapser id="kubernetes-cost-attributes" title="Kubernetes コストイベントタイプ">
    **All Capabilities &gt; Cloud Cost Intelligence**に移動し、クエリ コンソールを開いて入力します。
    ...
    ```sql
    SELECT SUM(net_unblended_cost * (cpu_weightage + memory_weightage) + network_net_unblended_cost) FROM KubernetesCost SINCE '2025-09-01' UNTIL '2025-09-10'
    ```
