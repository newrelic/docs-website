...
さらに、エージェントの新しいバージョンでは、フィンガープリントまたはカスタム提供ラベルによる例外のグループ化がサポートされています。これを行うには、 `group` プロパティが目的の *文字列*に設定されたブール値の代わりにオブジェクトを返します。空の文字列、またはこの正確な仕様に準拠しないオブジェクトを指定すると、エラーが *無視される* `true` の場合と同じように扱われることに注意してください。この動作は、以前のバージョンと下位互換性があります。
