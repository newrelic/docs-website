...
API を使用して、スクリプトの初期化中やインライン イベント ハンドラーで発生したエラーなど、詳細なしで報告されるエラーに気付くこともできます。エラーは、New Relic が通常検出するその他のエラーとともに、 [**JavaScript エラー**ページ](/docs/errors-inbox/browser-tab)に表示されます。また、 [`JavaScriptError`イベント](/attribute-dictionary/?event=JavaScriptError)として記録されます。例えば：
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        必須です。ブラウザの [**JavaScript のエラー** ページ](/docs/errors-inbox/browser-tab) のデータを分析する際に使用できる、意味のあるエラーメッセージを提供してください。
      </td>
    </tr>
