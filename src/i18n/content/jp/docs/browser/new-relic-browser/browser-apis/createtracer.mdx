...
<Callout variant="important">
  SPA API の`createTracer`メソッドは非推奨になりました。タスクの継続時間をトレースする推奨される方法は、パフォーマンス[マーク](https://developer.mozilla.org/en-US/docs/Web/API/Performance/mark)やコールバック実行時間を測定する[メジャー](https://developer.mozilla.org/en-US/docs/Web/API/Performance/measure)を使用することです。 このメソッドは現在非アクティブになっており、JavaScript の実行をインタラクションとリンクしたり、インタラクションを開いたままにしたりする機能はなくなりました。
</Callout>
