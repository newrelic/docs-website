...
<Callout variant="important">
  SPA API の`createTracer`メソッドは非推奨になりました。 タスクの期間を追跡するための推奨される方法は、パフォーマンス[マーク](https://developer.mozilla.org/en-US/docs/Web/API/Performance/mark)を取得したり、[測定し](https://developer.mozilla.org/en-US/docs/Web/API/Performance/measure)たりすることです。 ブラウザエージェントの将来のバージョンでは、マークとメジャーが自動的にキャプチャされるようになり、その時点で`createTracer`のサポートは終了します。
</Callout>
