...
`newrelic.setErrorHandler()` API コールを使うと、ブラウザエージェントが捕捉した既知のエラーを選択的に無視することができます。この API コールは 1 つのエラーハンドラ関数を受け取り、ブラウザエージェントが捕捉した各エラーに対して呼び出されます。ハンドラーが `true` を返した場合、New Relic は そのエラーを記録しません。それ以外の場合は、そのエラーは通常通り処理されます。
