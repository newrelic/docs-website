...
待ち時間や JS の実行時間など、SPA インタラクションのサブコンポーネントを個別に時間を計測します。
...
このメソッドは、ブラウザ上で SPA インタラクションのサブコンポーネントを個別に計測する方法を提供します。各サブコンポーネントは、以下を測定します。

- コールバックが実行されるまでの待ち時間
- コールバックが呼び出されてからの JS 実行時間
...
`createTracer()` メソッドは、ラップされたコールバックメソッドを返すので、コードから呼び出す必要があります。返されたラップドコールバックは、呼び出されると 3 つのことを行います。

1. カスタムトレーサーの非同期部分の終了を記録します。
2. `createTracer()` に渡されたオリジナルのコールバックを、同じ引数とコンテキストで実行します。
3. 元のコールバックの実行を繰り返す。

[`BrowserInteraction`](/docs/insights/explore-data/attributes/browser-default-attributes-insights#browserinteraction-attributes) が `createTracer()` を実行すると、 [`BrowserTiming` イベント](/docs/insights/explore-data/attributes/browser-default-attributes-insights#browsertiming-attributes) が作成されます。コールバック中に作成された新しい XHR やカスタムトレースは、インタラクションの一部として含まれます。
...
単に特定の JavaScript 関数の呼び出しにかかる時間を測定したい場合は、その関数をコールバックとして `createTracer()` に渡すことができます。これにより、返されたラッパーのコールバックが直ちに呼び出されます。
...
### インストルメント化されていない非同期 API のトレース [#asynch-api-tracer]

デフォルトでは、ブラウザエージェントは、最も一般的な非同期関数の多くをラップしています(例えば、 `setTimeout()`)。しかし、自動的に処理されない関数もあります(例えば、 `requestAnimationFrame()`)。また、因果関係が容易に判断できないケースも発生します（例えば、Websocket 上のカスタム RPC 実装など）。このような場合には、 `createTracer()` を使用して、これらのインスツルメントされていない関数へのコールバックがインタラクションの一部とみなされるようにすることができます。

`createTracer()` を使用することは、 `setTimeout()`, `setImmediate()`, `Promise()`, `fetch()`, `XMLHttpRequest()` のような、ブラウザエージェントによってネイティブにラップされる非同期 API に基づく非同期スケジューリングシステムには必要ありません。これらの API によって作られた非同期の境界は、ブラウザエージェントによって自動的にブリッジされます。
...
![SPA非同期タイムラインイメージ](src/images/spa-trace-async-timeline.png 'spa-trace-async-timeline.png')
