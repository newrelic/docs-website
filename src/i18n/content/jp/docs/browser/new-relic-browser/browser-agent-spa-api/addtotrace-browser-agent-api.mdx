    ...
    <tr>
      ...
      <td>
        ...
        * オプションの名前/値ペア： `END` 、 `ORIGIN` 、 `TYPE`
          ...
          同じイベントオブジェクトを[`PageAction`](/docs/insights/explore-data/attributes/browser-default-attributes-insights#pageaction-list)としてNewRelicOneに送信する場合は、 `TYPE`属性を省略してください。 （ `TYPE`は、セッショントレース内でマークするイベントのタイプを説明する文字列です。）これを含めると、イベントタイプが上書きされ、 `PageAction`イベントが誤って送信されます。代わりに、イベント情報に`NAME`属性を使用してください。
      </td>
    </tr>
  </tbody>
</table>
