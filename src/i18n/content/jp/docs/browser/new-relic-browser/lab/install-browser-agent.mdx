...
<Callout
  ...
  title="ラボ"
>
  この手順は、New Relic を使用して Web アプリのトラブルシューティングを行う方法を説明するラボの一部です。 <InlinePopover type="browser"/>。
...
<Steps>
  ...
  <Step>
    上部のナビゲーション バーの右側にある **Add data**\[データの追加]をクリックします。
  ...
  <Step>
    **Browser & mobile** まで下にスクロールし、 **Browser monitoring**を選択します。
  ...
  <Step>
    展開方法として **Copy/paste JavaScript code**\[JavaScript コードをコピー/貼り付け] を選択します。
  ...
  <Step>
    **Name your app**\[アプリに名前を付ける]まで下にスクロールします。アプリに **Relicstaurants**という名前を付けて、 **Enable**\[有効にする]をクリックします。
...
アプリがブラウザー データを New Relic に送信するようになりました。 **Browser**の下の New Relic でこのデータを表示します。
...
<Steps>
  ...
  <Step>
    左側のナビゲーション バーから **Browser** に移動し、 **Relicstaurants**を選択します。
    ...
    ここでは、 **Page views with JavaScript errors**\[JavaScript エラーのあるページ ビュー]、 **Core web vitals**\[主要なウェブ バイタル]、 **User time on the site**\[サイトでのユーザー時間]、 **Initial page load and route changes**\[最初のページの読み込みとルートの変更]、アプリからのその他のパフォーマンス データが表示されます。
