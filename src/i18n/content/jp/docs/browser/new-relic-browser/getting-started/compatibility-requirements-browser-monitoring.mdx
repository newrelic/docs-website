...
私たちの<InlinePopover type="browser"/>には、ブラウザ監視機能を備えた堅牢なスタンドアロン製品を提供するための[厳格なセキュリティ対策が](/docs/browser/new-relic-browser/performance-quality/security-new-relic-browser)含まれています。[ブラウザ エージェントをインストールする](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser)前に、システムが次の要件を満たしていることを確認してください。
