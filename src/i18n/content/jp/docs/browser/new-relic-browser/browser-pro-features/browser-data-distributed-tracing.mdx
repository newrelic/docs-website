...
2. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (アプリを選択) > Application settings**に移動します。
3. \[**分散トレース**] トグルをオンにします。
...
<CollapserGroup>
  ...
  >
    ...
    2. ブラウザー監視エージェントと APM エージェントが CORS の分散トレースと[互換性](#requirements)があることを確認してください。該当する場合は、最新バージョンに更新します。

    3. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Browser > (アプリを選択) > Application settings**に移動します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        分散トレース UI では、エンドユーザー スパンは<img title="distributed-tracing-browser-span-icon.png" alt="New Relic distributed tracing browser span icon" src={distributedTracingBrowserSpanIcon} style={{height: "30px", width: "30px"}}>で示されます。</img>
