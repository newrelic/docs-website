...
<InlinePopover type="browser"/>エージェントと<InlinePopover type="apm"/>エージェントに必要な最小バージョンがあることを確認してください。
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="important">
      New Relic は、他のオリジンのサービスが正しく構成されていることを確認するための検証を実行できません。 これらのヘッダーを許可する方法がわからない場合は、 <DNT>**do not**</DNT> New Relic UI の承認済みリストにクロスオリジン リソースを追加してください。
    </Callout>
  </Collapser>
  ...
  >
    ...
    2. ブラウザ監視エージェントと<InlinePopover type="apm"/>エージェントが CORS のディストリビューティッド（分散）トレーシングと[互換性](#requirements)があることを確認してください。 該当する場合は最新バージョンに更新してください。

    3. <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Browser > (select an app) > Settings > Application settings**</DNT>に移動します。

    4. <DNT>**Distributed tracing**</DNT>トグルがまだ有効になっていない場合はオンにします。

    5. <DNT>**Cross-origin resource sharing (CORS)**</DNT>トグルをオンにします。

    6. <DNT>**Cross-origin resource sharing (CORS)**</DNT>の下で、クロスオリジン リソースを承認済みリストに追加します。 クロスオリジン リソースにプレフィックス`http://`または`https://`とドメイン名が含まれていることを確認してください。 HTTP (ポート 80) または HTTPS (ポート 443) のデフォルトと異なる場合は、ポート番号を含めます。

    7. エージェント設定を更新するには、 <DNT>**Save application settings**</DNT>選択します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        分散トレーシングUIでは、エンドユーザーのスパンは<img title="distributed-tracing-browser-span-icon.png" alt="New Relic distributed tracing browser span icon" src="/images/browser_icon_browser-icon.webp" style={{height: "30px", width: "30px"}}/>
