...
私たちの<InlinePopover type="browser"/>[ソース マップ](/docs/new-relic-browser-source-maps)のアップロードをサポートしています。ソース マップは、 [**Errors** \[エラー\] ページ](/docs/errors-inbox/browser-tab)でエラー スタック トレースの縮小を解除するために使用されます。このドキュメントでは、ブラウザ API を使用してソース マップをアップロードする API の使用方法について説明します。
