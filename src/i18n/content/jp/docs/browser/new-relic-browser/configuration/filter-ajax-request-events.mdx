...
[エージェント バージョン 1211](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/)以降では、ページによって行われたすべてのネットワーク リクエストは`AjaxRequest`イベントとして記録されます。**Application settings** \[アプリケーション設定]ページの拒否リスト構成オプションを使用して、イベントを記録するリクエストをフィルタリングできます。
