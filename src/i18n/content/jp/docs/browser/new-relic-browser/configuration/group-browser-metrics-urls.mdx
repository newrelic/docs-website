...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > ブラウザ > (アプリを選択) > 設定 > 許可リストのセグメント化 に**移動します。
...
ブラウザの監視では、データのグループ化に、 `ApplicationsController#show`などのサーバー側のコントローラアクション名の代わりにURLパターンを使用します。必要に応じて、関連するバックエンドAPMイベントへのリンクが含まれています。ただし、これはスタンドアロンサービスとしてのブラウザの主要なグループ化戦略ではありません。