...
特定のページまたはエンドポイントのパフォーマンスの内訳を探しています。 <InlinePopover type="browser"/>[**Page views** \[ページビュー\] UI ページ](/docs/browser/new-relic-browser/additional-standard-features/page-views-insights-your-sites-popularity)または[**AJAX** UI ページ](/docs/browser/new-relic-browser/browser-pro-features/ajax-dashboard-identifying-time-consuming-calls)。UI には、探している URL グループが表示されないか、 `/`と`/*`のみが表示されます。この動作は、 `BrowserInteraction`イベントの SPA データにも現れ、 `browserInteractionName` 、 `targetGroupedUrl` 、および`previousGroupedUrl`属性に適用されます。
