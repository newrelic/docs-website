...
多くの場合、アプリ サーバーのトランザクション数 (1 分あたりのリクエスト数または **rpm**) は、同じアプリケーションのブラウザー トランザクション数 (1 分あたりのページ数または **ppm**) よりも多くなります。すべてのページが Web に面している (API またはバックエンドのみのコンポーネントがない) サイトでも、記録されたブラウザー ページ ビューの 2 倍から 10 倍のトランザクションが発生する場合があります。
