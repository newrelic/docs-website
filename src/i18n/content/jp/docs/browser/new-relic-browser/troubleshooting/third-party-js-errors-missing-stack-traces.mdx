...
JavaScript (JS) エラー イベントは、 [<InlinePopover type="browser"/>JS エラー ページ](/docs/browser/new-relic-browser/browser-pro-features/javascript-errors-page-examining-errors-over-time)。デフォルトでは、エラー メッセージごとにグループ化されています。単一のエラー イベントを選択すると、その**Error Instances** \[エラー インスタンス] リストに詳細が表示されます。場合によっては、1 つ以上のインスタンスにスタック トレースがない場合があります。
