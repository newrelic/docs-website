...
ブラウザモニタリングでは、New Relicエージェントを活用して、ページが出来上がるのに合わせJavaScriptを動的にインジェクトすることで、より詳細なバックエンドのタイミング情報を収集することができます。Browserは、[シングルページアプリケーション（SPA）アーキテクチャー](/docs/browser/single-page-app-monitoring/get-started/add-apps-single-page-app-monitoring)によってアプリをモニターすることもできます。
