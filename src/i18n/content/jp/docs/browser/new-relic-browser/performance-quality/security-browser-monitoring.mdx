...
ブラウザは、さまざまな種類のデータをレポートして、Webサイトのパフォーマンスの分析を支援します。ブラウザは、[Pro機能](https://newrelic.com/pricing)に登録している場合を除いて、ページビューデータのみ<DNT>**only**</DNT>をレポートします。AJAX要求、JavaScriptエラー、およびセッショントレースの[機能を有効にする](/docs/browser/new-relic-browser/welcome-new-relic-browser/new-relic-browser#enhanced-features)こともできます。
...
<Callout variant="important">
  **JPリージョン：**New RelicのCDNプロバイダーは日本国内にエンドポイントがあり、平常時は日本国内のエンドポイントが使用されます。ただし、CDNの障害やパフォーマンス上の問題が発生した場合、Browserエージェントのスクリプトが日本国外のエンドポイントから配信されることがあります。その場合でも、テレメトリーデータの送信先や保存先には影響しません。
</Callout>
