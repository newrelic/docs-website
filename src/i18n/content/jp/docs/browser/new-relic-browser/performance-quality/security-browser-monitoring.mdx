...
<Callout variant="important">
  エンドユーザーがファイアウォールまたはプロキシの背後にいて、CDNまたはNew Relicの[ネットワーク](/docs/apm/new-relic-apm/getting-started/networks)（`bam.nr-data.net`および`bam-cell.nr-data.net`を含む）にアクセスできない場合、Browserモニタリングは機能しません。
</Callout>
