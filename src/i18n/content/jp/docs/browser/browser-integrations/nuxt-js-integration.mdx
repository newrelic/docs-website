...
Nuxt.jsダッシュボードを使用すると、アプリケーションエラー率、初期ページ読み込み平均、ルート変更、スループット、JavaScriptエラーのあるページビューを簡単に追跡できます。当社のInfrastructureエージェントとVue<InlinePopover type="browser"/>で構築されており、1か所で重要な情報をすべて表示できます。
