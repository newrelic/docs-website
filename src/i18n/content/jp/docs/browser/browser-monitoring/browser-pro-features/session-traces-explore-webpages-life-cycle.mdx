...
Google の <InlinePopover type="browser"/><DoNotTranslate>**Session traces**</DoNotTranslate> UIウェブページの全ライフサイクル中の負荷とインタラクション イベントの詳細なタイムラインを、最長 10 分間表示します。 セッション URL またはセッショントレース ID を選択すると、詳細なウォーターフォールの視覚化とヒート マップの概要を使用してメトリクスを調べ、以下に関連する問題を特定できます。
