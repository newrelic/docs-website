...
<Steps>
  ...
  <Step>
    サンプリング レートを編集して、ログを収集するユーザー セッションの割合を指定します。 0 ～ 100 の値を入力できます。

    <Callout variant="important">
      * ログのサンプリングは、他のサンプリング決定と調整され、同じセッションのデータ収集が最大化されます。 たとえば、ログのサンプリング レートを 5% に設定し、セッションリプレイ (SR) のサンプリング レートを 10% に設定すると、次のようになります。

        * セッションの約 5% でログと SR の両方が収集されます。
        * セッションの約 5% では SR データのみが収集されます。
        * 残りのセッションではどちらも行われません。

      * 作成する新しいブラウザ アプリでは、警告の詳細レベルと 100% のサンプリング レートで自動ログ収集がデフォルトで有効になります。
    </Callout>
  </Step>
</Steps>
