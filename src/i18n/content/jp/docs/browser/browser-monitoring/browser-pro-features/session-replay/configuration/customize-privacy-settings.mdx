...
6. <DNT>User input masking</DNT>セクション:

   1. すべてのユーザー入力フィールドからマスクを削除するには、すべてのチェックボックスをオフにします。
   2. マスクする特定の入力タイプのチェックボックスを選択します。これにより、他のすべての入力は表示されたままになり、特定のユーザー入力のみが確実にマスクされます。

7. 変更を適用するには、アプリを再デプロイします。

再デプロイ後、すべての静的テキストは通常どおり表示されますが、特別にマークされた要素のみがマスクされたままになります。追加情報：

* ユーザー入力の詳細を非表示にするには、[機密テキストのマスク](#mask-text)セクションを参照してください
* 静的テキストを表示するには、[静的テキストのマスク解除](#unmask-text)セクションを参照してください
* ユーザー入力コンテンツを無視するには、[ユーザー入力の無視](#ignore-input)セクションを参照してください
...
デフォルトでは、推奨設定を使用すると、すべての静的テキストがマスクされます。特定の静的テキスト要素を明示的にマスク解除する場合は、その特定の要素に`nr-unmask`クラスまたは`data-nr-unmask`属性を追加できます。これは、グローバルなマスクを有効にしたまま、特定の要素を表示したままにする必要がある場合に便利です。

<Callout variant="tip">
  `nr-unmask`および`data-nr-unmask`セレクタを使用するには、Browserエージェントバージョン1.256.0以降を使用する必要があります。
</Callout>

特定のテキスト要素をマスク解除するには、次のいずれかのオプションを使用します：
...
* コード内：HTML要素に`nr-unmask`CSSクラスまたは`data-nr-unmask`属性を追加します。
  ...
  たとえば、次を使用して他のテキストをマスクしたまま製品名を表示するには：
...
サイト上のコンテンツのセクション全体をブロックできます。つまり、そのセクションはセッションリプレイで空のプレースホルダーとして表示されます。たとえば、会社概要 Web ページに画像があり、それをキャプチャする必要がない場合は、その画像を含むクラスをブロックできます。
