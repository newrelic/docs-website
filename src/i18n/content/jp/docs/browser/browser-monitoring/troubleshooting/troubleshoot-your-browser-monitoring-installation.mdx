...
ブラウザ監視エージェントがAPMエージェントで監視されているアカウントに展開されている場合は、 [APMの展開](#apm-tips)を参照してください。ブラウザの監視エージェントがJavaScriptのコピー/貼り付け方法を使用してデプロイされた場合は、「[コピー/貼り付けのデプロイ](#manual-tips)」を参照してください。どちらを使用しているかわからない場合は、診断ツールが展開方法を返します。
...
<CollapserGroup>
  ...
  >
    ...
    1. **[one.newrelic.com](https://one.newrelic.com) >ブラウザ>（アプリを選択）>設定**に移動します。
  ...
  >
    最近のAPMエージェントは、ページ読み込みタイミングスクリプトをより正確に配置します。これらのトラブルシューティングのヒントを続行する前に、APMエージェントの[最新リリース](/docs/apm/new-relic-apm/maintenance/keeping-new-relic-date)があることを確認してください。
  ...
  >
    ...
    * [。ネット](/docs/agents/net-agent/features/page-load-timing-net#auto_instrumentation)
    ...
    * [ルビー](/docs/agents/ruby-agent/features/page-load-timing-ruby#auto_instrumentation)
