...
アプリケーションを次のように構成したばかりの場合<InlinePopover type="browser"/> 、データが表示されるまで数分待ちます。数分間待ってもデータが表示されない場合は、次のトラブルシューティングのヒントを試してください。これらのヒントの一部は、ブラウザー監視で JavaScript スニペットを APM 監視対象アプリに自動的に挿入できるようにして[アプリをデプロイした](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser)か、JavaScript スニペットを自分で挿入してアプリをデプロイしたかによって異なります。
...
<CollapserGroup>
  ...
  >
    ウェブページにJavaScriptエラーが表示される場合、またはNew Relic UIの[**JavaScriptエラー**ページ](/docs/errors-inbox/browser-tab)にデータが表示されない場合は、アプリで他のJavaScriptエラー監視フレームワークが有効になっているかどうかを確認してください。一部のJavaScriptエラーモニターは、ブラウザー監視の機能に干渉する場合があります。
  </Collapser>
