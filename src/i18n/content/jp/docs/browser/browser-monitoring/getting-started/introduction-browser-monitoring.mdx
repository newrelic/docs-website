...
<Callout variant="tip">
  Browserモニタリング、およびその他の[オブザーバビリティ プラットフォーム](https://one.newrelic.com)を使用するには、New Relicファミリーに参加してください。[サインアップ](https://newrelic.com/signup)して、わずか数秒で無料アカウントを作成できます。次に、毎月最大で100GBのデータを無料で取り込みます。期間無制限です。
</Callout>
...
![browser_intro.png](https://jajp-newrelic-docs.onelink-translations.com/sites/default/files/thumbnails/image/browser_intro_0.png "browser_intro.png")
...
[`PageViewTiming`イベント](https://jajp-newrelic-docs.onelink-translations.com/attribute-dictionary/pageview?attribute_name=&events_tids%5B%5D=10061)は、その他のイベントに依存しない、よりリアルタイムの配信メカニズムを提供します。追加のメトリックスには次のものがあります。
...
[シングルページアプリケーション（SPA）](/docs/browser/single-page-app-monitoring/browser-ui/view-spa-data-new-relic-browser)モニタリングは、ブラウザのインタラクションでのルート変更や最初のページのロード、同期および非同期のアクティビティを追跡します。当社の[BrowserエージェントとSPA API](/docs/browser/new-relic-browser/browser-agent-spa-api)を使用して、ブラウザ内で実行されるほぼすべてのものをモニターすることもできます。
...
3. [**Browser summary**ページ](/docs/browser/new-relic-browser/getting-started/browser-overview-website-performance-glance)で概要データを表示してから、ページロードタイミングの詳細とその他のUIデータにドリルダウンします。
