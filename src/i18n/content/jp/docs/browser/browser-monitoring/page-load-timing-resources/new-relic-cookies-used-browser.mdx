...
このドキュメントでは、<InlinePopover type="browser"/>とクッキーに関連するデータプライバシーと、クッキー自体の詳細について説明します。Browserエージェントのバージョン1220以前は、エンドユーザーブラウザでの[セッション追跡](/docs/browser/new-relic-browser/performance-quality/security-new-relic-browser#cookies)のために以下の[クッキー](/docs/browser/new-relic-browser/page-load-timing-resources/session-tracking)が作成されていました。
