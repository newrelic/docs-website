...
このドキュメントでは、 <InlinePopover type="browser" />と Cookie に関連するデータのプライバシーと、Cookie 自体の詳細について説明します。 Browserエージェント バージョン 1220[ ](/docs/browser/new-relic-browser/performance-quality/security-new-relic-browser#cookies)より前では、エンドユーザーのブラウザでの[ セッション追跡 のために次の Cookie が作成されていました。](/docs/browser/new-relic-browser/page-load-timing-resources/session-tracking)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ブラウザエージェントのバージョン v1217 以前では、エージェントはデフォルトでサードパーティ Cookie を設定します。 これらのエージェントのために、サードパーティ Cookie が BrowserAgent とNew Relicによって配置されます。 New Relic RelicCollector は、BrowserAgent が接続を確立してデータを送信するときに、サードパーティ Cookie を配置します。
        ...
        [Browseragent v1169](/docs/release-notes/new-relic-browser-release-notes/browser-agent-release-notes/browser-agent-v1169) [`JSESSIONID`](#jsessionid)[`NREUM`](#nreum)browserのリリースにより、<DNT>**Application settings**</DNT> New RelicUIの ページを使用して、 アプリのサードパーティ Cookie の収集 ( および ) をオフにできるようになりました。この構成値を false (オフ) に設定すると、イベント データにセッション情報が表示されなくなります。
...
<CollapserGroup>
  <Collapser id="jsessionid" title="JSESSIONID（非推奨）">
    `JSESSIONID`クッキーは、エージェントバージョン1220以降では非推奨です。クッキーによるセッショントラッキングは実行されなくなりました。

    New Relicがアプリケーションのセッション数を監視できるように、<DNT>**session identifier**</DNT> を保存していました。 Cookie 値は Jetty によって生成されます。
  ...
  <Collapser id="nreum-cookie" title="NREUM（非推奨）">
    <Callout variant="important">
  ...
  <Collapser id="nragent" title="NRAGENT（非推奨）">
