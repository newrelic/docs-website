...
* [New Relic Oneの価格設定](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing)：この価格設定モデルには、請求要因としてデータインジェストがあります。データのインジェストを減らすには、インストール後に、最新でないエージェントタイプにダウングレードすることを検討してください。
