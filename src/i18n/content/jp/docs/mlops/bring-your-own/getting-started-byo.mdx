...
<InlinePopover type="licenseKey" />（`ingest - license`とも呼ばれます）を取得し、環境変数として設定します：`NEW_RELIC_INSERT_KEY`。詳細と手順については、[こちらをクリック](/docs/apis/intro-apis/new-relic-api-keys/#license-key)してください。データをNew RelicのEUまたはJPリージョンに報告していますか？詳細な手順については、[こちら](#EU-account-users)をクリックしてください。
...
## EUおよびJPアカウントユーザー [#EU-account-users]

EUまたはJPアカウントを使用している場合、環境変数が設定されていなければ、`MLPerformanceMonitoring`呼び出し時にパラメーターとして送信してください：
