...
<LicenseKey/>( `ingest - license`とも呼ばれる) を取得し、環境変数`NEW_RELIC_INSERT_KEY`として設定します。詳細と手順については、[ここをクリックして](https://docs.newrelic.com/docs/apis/intro-apis/new-relic-api-keys/#license-key)ください。New Relic EU 地域にデータを報告していますか?詳細については、[ここをクリック](#EU-account-users)してください。
