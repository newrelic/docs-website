...
* **Inference data:**モデルの特徴と予測値をストリーミングします。推論データは、InferenceData という名前の[カスタム イベント](https://docs.newrelic.com/docs/data-apis/ingest-apis/introduction-event-api/)としてストリーミングされます。
* **Data metrics:**すべての生の推論データを送信する代わりに、特徴と予測に対する集計統計オプション (最小値、最大値、平均値、パーセンタイルなど) を選択します。これらは [メトリクス](https://docs.newrelic.com/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/)として自動的に送信されます。
* **Custom metrics:**独自のメトリクスを計算し、モデルのパフォーマンスまたはモデルデータを監視し、record_metrics 関数を使用してそれらを New Relic にストリーミングします。それらは [メトリクス](https://docs.newrelic.com/docs/data-apis/ingest-apis/metric-api/introduction-metric-api/)として送信されます。
