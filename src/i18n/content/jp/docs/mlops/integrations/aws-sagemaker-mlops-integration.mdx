...
Amazon SageMaker の New Relic との統合により、システム全体でマシンラーニングのパフォーマンスを計測、分析、トラブルシューティング、最適化できるようになります。モデルの入力または出力の変化、およびその 2 つの間の関係に素早く反応する能力を厳密に観察します。

次のステップで、Amazon SagaMaker のメトリクスとオブジェクト（AWS CloudWatch に送信される）を監視し、New Relic で [エンティティ](/docs/new-relic-one/use-new-relic-one/core-concepts/what-entity-new-relic/) と [ダッシュボード](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/) として表示します。

![Sagemaker、Cloudwatch、New Relicの流れを汲む。](src/images/aws-sm-flow.png 'Sagemaker、Cloudwatch、New Relicのフロー.png')

## AWS CloudWatch のメトリクスを New Relic にストリームする [#set-up-cloudwatch]
...
[Terraform コード](https://github.com/newrelic-experimental/terraform-cloudwatch-metric-streams) を使ってセットアップを自動化することができます。
...
<Callout variant="important">
  各エンティティのメトリクスは、メトリクスがNew
  Relicに届くと自動的に作成されるダッシュボードで確認できます。
</Callout>

## Amazon SageMaker でデータやモデルを監視し、メトリクスを CloudWatch に送信することができます。 [#model-monitor]

SageMaker は、エンドポイントのパフォーマンスを自動的に監視し、統計的なメトリクスを CloudWatch に送信します。詳細は、 [Endpoint CloudWatch Metrics](https://docs.aws.amazon.com/sagemaker/latest/dg/monitoring-cloudwatch.html) を参照してください。

Amazon SageMaker MLOps の統合からより多くの利益を得るには、Amazon SageMaker Model Monitor ツールを使用します。運用中の機械学習モデルの品質を監視し、メトリクスを CloudWatch に送信するために、スケジュールされた監視ジョブを定義する必要があります。
...
また、メトリックデータポイントを Amazon CloudWatch にパブリッシュし、名前空間を定義して、 [`put_metric_data` 関数](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/cloudwatch.html#CloudWatch.Client.put_metric_data) を使用して上記のいずれかを行うことができます。

ハイパーパラメータのチューニングに独自のアルゴリズムを使用する場合は、評価データを `stderr` または `stdout` に書き込むことで、少なくとも 1 つのメトリックを送信することを確認してください。詳しくは、 [how to define metrics in automatic model tuning](https://docs.aws.amazon.com/sagemaker/latest/dg/automatic-model-tuning-define-metrics.html) をご覧ください。サンプルノートブック [Develop, Train, Optimize and Deploy Scikit-Learn Random Forest](https://github.com/aws/amazon-sagemaker-examples/blob/master/sagemaker-python-sdk/scikit_learn_randomforest/Sklearn_on_SageMaker_end2end.ipynb) もご覧ください。

## MLOps のエンティティとダッシュボードを見る [#explore-entities-dashboards]

詳細な名前空間のために、 `aws-entities` (MLOps エンティティドメインの下)を生成します。これらのエンティティに対しては、すぐに使えるダッシュボードやビューを得ることができます。また、独自のダッシュボードを作成して、エンティティのビューの一部として表示されていないメトリクスを表示することもできます。
...
![Sagemakerの機械学習エンティティ](src/images/aws-sm-02.png 'Sagemaker Machine Learning entities.png')

- Amazon SageMaker のエンティティの 1 つから、エンドポイントのメトリクスを表示するダッシュボード

![機械学習エンドポイントのダッシュボード](src/images/aws-sm-03.png '機械学習用エンドポイントのダッシュボード')

- モデルデータエンティティのダッシュボード

![モデルデータエンティティのダッシュボード](src/images/aws-sm-04.png 'モデルデータのエンティティのダッシュボード.png')
