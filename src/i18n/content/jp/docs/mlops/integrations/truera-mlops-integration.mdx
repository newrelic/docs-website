...
4. **API キーを選択または作成します。** ダッシュボード管理には **アクティブなダッシュボード キーが** 必要であり、メトリクスの取り込みには **モデル メトリクス データ キーが** 必要です。 これらのキーは、
...
2. **アラート通知をセットアップする:**いくつかのダッシュボードを作成したら、データに関するアラートを受け取ることができます。 NRQLを作成するには

   <InlinePopover type="alerts"/>

   グラフから条件を選択するには、グラフ ウィジェットをクリックし、 **Create alert condition** \[アラート条件の作成]をクリックします。 条件に名前を付けてカスタマイズしたら、それを既存のポリシーに追加するか、新しいポリシーを作成できます。
