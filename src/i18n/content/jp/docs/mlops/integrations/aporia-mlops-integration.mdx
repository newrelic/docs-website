...
Aporia には New Relic との統合が含まれており、推論調査を示す New Relic 内のカスタマイズされたダッシュボードを使用して、MLOps インフラストラクチャの完全なモデル管理を提供します。

## AporiaとNew Relicの統合 [#integrate-aporia]

Aporiaを使用すると、Aporiaのモニターによって生成されたアラートをNew Relicのインシデントインテリジェンスエンジンと予測データに接続して、モデルのNewRelicに包括的な監視ダッシュボードを作成できます。

1. **Aporiaのコンソールにログイン**: 左側のナビバーで **Integrations** をクリックし、 **New Relic** を選択します。
2. **New Relic アカウントにログインします:** **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > +Add more data に**移動します。
3. **Aporia をクリックし**ます。検索バーに**Aporia**と入力します。
4. **Select your account ID:** Aporiaに統合させたいアカウントIDを選択してください。
5. **New Relic API キーを取得し**ます。Aporia アイコンをクリックしたら、ステップ 1 に従います。\[ **Prediction data & model quality alerts** ] で、 **\[Select** ] をクリックして既存のキーを選択するか、\[ **Create API key** ] をクリックします。これにより、New Relic にデータをレポートするために使用される New Relic [ライセンス キーが](/docs/apis/intro-apis/new-relic-api-keys/#license-key)作成されました。
6. **Aporiaにキーを貼り付けます:** 今度はAporiaのダッシュボードに行き、New Relic Integrationの画面で、 **New Relic insert token** の下にキーを貼り付けます。貼り付けたら、 **Save** をクリックしてください。
