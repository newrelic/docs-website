...
5. **Get a New Relic API key:** Aporiaのアイコンをクリックしたら、ステップ1に従います。 **Prediction data** の下で、 **Select** 、既存のキーを選択するか、 **Create API key** のどちらかをクリックします。これで [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) が作成されました。これは New Relic にデータを報告するために使われます。
