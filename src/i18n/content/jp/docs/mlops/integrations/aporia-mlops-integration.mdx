...
5. **Get a New Relic API key:** Aporiaのアイコンをクリックしたら、ステップ1に従います。 **Prediction data& model quality alerts** の下で、 **Select** 、既存のキーを選択するか、 **Create API key** のどちらかをクリックします。これにより、 [ライセンスキー](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) が作成され、New Relic へのデータ報告に使用されます。
