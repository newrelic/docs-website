...
3. **アラート通知をセットアップする:**いくつかのダッシュボードを作成したら、データに関するアラートを受け取ることができます。 NRQLを作成するには<InlinePopover type="alerts"/>チャートから条件を選択するには、チャート メニューをクリックします<Icon name="fe-more-horizontal"/>をクリックし、 **Create alert condition** \[アラート条件の作成]をクリックします。 条件に名前を付けてカスタマイズしたら、それを既存のポリシーに追加するか、新しいポリシーを作成できます。
