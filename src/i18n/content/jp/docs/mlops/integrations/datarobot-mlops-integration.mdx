...
1. **API キーを取得する:** **[one.newrelic.com](https://one.newrelic.com) > (アカウント メニュー) > API キー**に移動し、意味のある名前でアカウントのユーザー キーを作成します。後で使用できるように、この名前をメモしておいてください。API キーの[詳細については、ドキュメント](/docs/apis/get-started/intro-apis/new-relic-api-keys/)を参照してください。
2. **ダッシュボードを作成する:** **[one.newrelic.com](https://one.newrelic.com) > Dashboards**に移動し、 **Import dashboards**ボタンをクリックします。JSON コードをコピーして \[ **Paste your JSON field code]**に貼り付けます。
