...
<Callout variant="important">
  OpenAIを利用したアプリを計装するには、この古いインストレーション方法ではなく、AIモニタリングを使用することをお勧めします。[開始するには、ドキュメントを参照してください](/docs/ai-monitoring/intro-to-ai-monitoring/)。
</Callout>
...
<CollapserGroup>
  <Collapser id="install-with-python" title="Python アプリケーション用にインストールする">
    <Steps>
      <Step>
  ...
  <Collapser id="install-with-node" title="Node.jsでインストールする">
    <Steps>
      <Step>
...
## EUおよびJPアカウントのユーザー [#EU-account-users]

EUまたはJPアカウントを使用している場合は、`EVENT_CLIENT_HOST`を次のように設定します：
...
```bash
export EVENT_CLIENT_HOST="insights-collector.jp.nr-data.net"
```

```python
import os
os.environ["EVENT_CLIENT_HOST"] = "insights-collector.jp.nr-data.net"
```
...
2. <DNT>**Model Performance**</DNT>をクリックします。
3. <DNT>**Applications**</DNT>で検索します。
