...
入れ子になった集約では、 [NRQL クエリ](/docs/query-data/nrql-new-relic-query-language/getting-started/introduction-nrql) を完了してから、そのクエリの結果を使って追加の計算を行うことができます。ネストされた集約は、サブクエリが外側のクエリの FROM 節にある場合の SQL サブクエリまたはサブセレクトのクラスと同様の NRQL 機能を提供します。

## 複雑な質問にも1つのクエリで回答 [#why-matters]

ネストされた集約は、複数のクエリを構築することなく、このような質問に答えるのに役立ちます。

* 自分のアプリケーションの1分あたりのリクエスト数をカウントして、過去1時間の1分あたりの最大リクエスト数を取得することはできますか？
* すべてのサーバーの平均CPU使用率を計算して、使用率が90％以上のものだけをリストアップすることはできますか？
* すべてのユーザーセッションから、何パーセントがすぐに跳ね返ってきたかを把握できますか？
