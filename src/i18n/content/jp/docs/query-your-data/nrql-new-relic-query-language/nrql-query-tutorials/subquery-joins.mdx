...
* 結合されたサブクエリのデフォルトの [`LIMIT`](#sel-limit) は引き続き 10 で、最大 `LIMIT` は 5000 です。外側のクエリの `LIMIT` 内側のクエリに影響を与えないことに注意してください。
...
<CollapserGroup>
  ...
  >
    ...
    ヒント: 外部クエリに結合される結果の数を最大化するには、内部サブクエリに LIMIT MAX を含めます。現在の最大結果制限は 5000 行です。
  </Collapser>
