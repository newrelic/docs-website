...
<CollapserGroup>
  ...
  >
    <SideBySide>
      <Side>
        開始するのに最適な場所は、New Relic APM によって収集された`Transaction`と呼ばれる NRDB の単一のイベント タイプを調べることです。すべての NRQL クエリには`SELECT` } 句と`FROM`句が必要です。データを`SELECT`し、そのデータが`FROM`である場所を指定する必要があります。`Transaction`イベント タイプからすべてを選択する次の基本的なクエリから始めます。
