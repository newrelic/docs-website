...
<CollapserGroup>
  ...
  >
    `minuteOf()`関数を使用して、有効なタイムスタンプ値を保持する属性の分の部分 (つまり、0 ～ 59 分) のみを抽出します。これは、 `hourOf()` 、 `weekOf()`などの関数でも機能します。時間ベースの関数の完全なリストについては、 [時間にわたるグループ結果のドキュメント](/docs/query-your-data/nrql-new-relic-query-language/nrql-query-tutorials/nrql-group-results-across-time/#cohorts)の表を参照してください。
  </Collapser>
