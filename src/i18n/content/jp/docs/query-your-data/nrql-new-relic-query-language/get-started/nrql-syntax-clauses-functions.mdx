...
* [クエリ次元メトリクス](/docs/data-ingest-apis/get-data-new-relic/metric-api/view-query-you-metric-data)、当社のメトリックAPI、テレメトリSDKおよび[オープンソーステレメトリインテグレーション](https://docs.newrelic.com/docs/integrations/open-source-telemetry-integrations/)（OpenTelemetry、Kamon、Micrometerなど）といったAPIを使用するツールの一部によって報告されます。
...
NRQLで使用できる関数のリストは、以下のとおりです。以下の定義には、NRQLクエリの例が含まれます。

### 集計関数

集計関数を使用して、NRQLクエリのデータをフィルタリング、集計します。以下に、集計関数の使用に関する便利な情報を提供します。
...
* New Relic Insightsの[**コホート分析**ページ](/docs/insights/new-relic-insights/features/cohort-analysis)には、コホート分析関数が表示されます。コホート関数は、時間別にトランザクションを集計します。
