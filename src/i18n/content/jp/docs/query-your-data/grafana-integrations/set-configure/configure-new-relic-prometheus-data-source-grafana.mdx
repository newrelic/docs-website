...
1. New Relicで、[インサイトクエリキーを新規作成します](/docs/apis/intro-apis/new-relic-api-keys/#insights-query-key)。
   ...
   <Callout variant="important">
     Grafanaで、これをカスタム<DNT>**X-Query-Key**</DNT>HTTPヘッダーに入れる必要がありますが（以下のステップ7を参照）、これはNew Relicクエリキーと同じエンティティです。

     アカウントが**JPリージョン**にある場合、Insightsクエリキーはサポートされていません。代わりに[ユーザーAPIキー](/docs/apis/intro-apis/new-relic-api-keys/#user-key)を作成し、ステップ7で<DNT>**api-key**</DNT>ヘッダーと一緒に使用してください。
   </Callout>
...
6. 正しい<DNT>**URL**</DNT>を入力してください:
   ...
   * 米国（デフォルト）: [prometheus-api.newrelic.com](https://prometheus-api.newrelic.com)
...
7. <DNT>**Custom Headers**</DNT>の下で、<DNT>**Add Header**</DNT>を選択します。米国およびEUアカウントの場合は、<DNT>**Header**</DNT>の名前を<DNT>**X-Query-Key**</DNT>に設定し、ステップ1のInsightsクエリキーを入力します。JPアカウントの場合は、<DNT>**Header**</DNT>の名前を<DNT>**api-key**</DNT>に設定し、ステップ1のユーザーAPIキーを入力します。
...
<CollapserGroup>
  <Collapser id="" title="X-Query-Key（クエリキー）">
    ...
    **JPリージョンの例外**: JPリージョンはInsightsクエリキーをサポートしていません。<DNT>**X-Query-Key**</DNT>の代わりに、ヘッダー名<DNT>**api-key**</DNT>の[ユーザーAPIキー](/docs/apis/intro-apis/new-relic-api-keys/#user-key)を使用してください。
  </Collapser>

  <Collapser id="" title="X-Prometheus専用">
    <Callout variant="important">
  ...
  <Collapser id="" title="X-Prometheusサーバー">
...
1. <DNT>**Configuration &gt; Data Sources**</DNT>に移動します。
2. 削除するデータソースをクリックします。
3. ページの下部にある<DNT>**Delete**</DNT>ボタンをクリックします。