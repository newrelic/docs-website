...
* **イベント：**ここでは、[イベント](https://docs.newrelic.com/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data)や[ログ](https://docs.newrelic.com/docs/using-new-relic/data/understand-data/new-relic-data-types#log-data)、[トレースデータ](https://docs.newrelic.com/docs/using-new-relic/data/understand-data/new-relic-data-types#trace-data)を含む、当社のすべての[`メトリック`以外の型](/docs/using-new-relic/data/understand-data/new-relic-data-types#dimensional-metrics)のデータを指します。
...
<CollapserGroup id="basic-mode-tutorial">
  ...
  >
    ...
    * **イベントタイプ**。 [`トランザクション`](/attribute-dictionary?attribute_name=&events_tids%5B%5D=8236)イベントタイプでは、ページ上のボタンをユーザーがクリックするなど、ウェブサイトをユーザーが開いている間に行われることを説明するさまざまなデータを測定します。
  ...
  >
    次のステップでは、**404 page not found**エラーがどのトランザクションで発生したかを判断します。[トランザクション](/attribute-dictionary?attribute_name=&events_tids%5B%5D=8236)イベント型の`イベントデータ辞書`を参照している場合、このイベントには`httpResponseCode`と呼ばれる属性も含まれることが分かります。
