...
<Callout variant="tip">
  変数バーの煩雑さを軽減するために、ユーザー入力を必要としない親変数または補助変数を非表示にします。隠れた変数は引き続き正常に機能する。詳細については、 [「変数の表示と非表示」](/docs/query-your-data/explore-query-data/dashboards/dashboard-template-variables/#variable-visibility)を参照してください。
</Callout>