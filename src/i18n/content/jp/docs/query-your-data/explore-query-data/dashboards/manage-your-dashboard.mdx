...
<CollapserGroup>
  ...
  >
    許可レベルにかかわらず、クローンアイコンをクリックしてご利用のダッシュボードのクローンを作成できます。ダッシュボードは自動的にコピーされ、クローンが[インデックス](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#get-started)に追加されます。クローンには、[**パブリック - 読み取りと書き込み**許可](/docs/query-your-data/explore-query-data/dashboards/introduction-dashboards/#dashboards-permissions)があります。画面にポップアップ表示されるメッセージをクリックして、クローンされた新規ダッシュボードにアクセスします。

    クローン版のダッシュボードには、元のダッシュボードの名前の後に「コピー」が付けられます。たとえば、`This is my dashboard`という名前のダッシュボードのクローンを作成すると、クローンは`This is my dashboard copy`として作成されます。クローン作成時に名前を変更できます。
...
### コンテンツの問題 [#chart-error-messages]

チャートを追加すると、次のような問題が発生することがあります。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        データの取得中に予期しないエラーが発生しました。数分後にもう一度試してください。それでも問題が解決しない場合は、[サポート](https://support.newrelic.com/)にお問い合わせください。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        クエリに一致するエンティティがありません。指定された`appId`、`appName`、`entity.guid`を確認し、クエリを再度実行してください。  
