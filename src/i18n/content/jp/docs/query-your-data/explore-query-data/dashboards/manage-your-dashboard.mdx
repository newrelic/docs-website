...
<CollapserGroup>
  ...
  >
    ...
    複製版のダッシュボードには、元のダッシュボードの名前の後に「コピー」が付けられます。たとえば、`This is my dashboard`という名前のダッシュボードの複製を作成すると、複製は`This is my dashboard copy`と呼ばれます。複製の作成時に名前を変更できます。
...
以下の短いビデオで、Markdownウィジェットでできる内容の詳細をご覧ください。<Video id="_dLwy7xskBk" type="youtube"/>
...
* ドロップダウンメニューから、利用可能なオプション（`Last 30 minutes`から`Last 7 days`まで）を1つ選択します。
* カスタムメニューを使用して、特定の開始および終了タイムスタンプで時間範囲をカスタマイズします。

タイムピッカーの使用時にチャートの時間範囲を変更したくない場合：

1. **Edit chart**を選択します
2. カスタマイズUIから、**Dashboard Options**メニューを開きます
...
* 右上のメニューバーにある**PDFとしてダッシュボードをエクスポート**ボタンをクリックして、PDFファイルとしてダッシュボードをエクスポートできます。
* チャートをPNG画像またはリンクとして共有する：チャートメニューに移動し、**画像として取得**または**チャートリンクを取得**オプションのいずれかを選択します。
