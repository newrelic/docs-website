...
<DNT>**Include variable**</DNT>トグルを使用すると、希望する値を設定して、その値をダッシュボードに含めるか除外するかを選択できます。詳細については、[テンプレート変数の](#create-variables)セクションを参照してください。
...
テンプレート変数は、表示モードの変数バーから非表示にできますが、バックグラウンドでは正常に機能し続けます。編集モードでは、非表示の変数は<Icon name="fe-eye-off" />アイコンが表示されるので、ダッシュボード編集者はそれらを識別して管理できます。

次のような変数を非表示にすることを検討してください。

* 変数バーに[ネストされた変数](/docs/query-your-data/explore-query-data/dashboards/nested-variables)などの要素を追加する
* 視聴者の入力を必要としないパワーダッシュボードロジック
* 他の変数から値を自動的に導出する

表示設定を行うには、[テンプレート変数を定義する](#step1)際に<DNT>**Hide on variables bar**</DNT>オプションを使用します。プログラムによるアクセスの場合は、 [NerdGraph ダッシュボードAPI](/docs/apis/nerdgraph/examples/nerdgraph-dashboards)の`hiddenOnVariablesBar`フィールドを使用します。
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            任意。このトグルを有効にすると、表示モードで変数バーからこの変数が非表示になります。その変数は、舞台裏では正常に機能し続けています。編集モードでは、非表示の変数は<Icon name="fe-eye-off" />アイコン。詳細については、 [「変数の表示と非表示」](#variable-visibility)を参照してください。
          </td>
        </tr>
      </tbody>
    </table>
