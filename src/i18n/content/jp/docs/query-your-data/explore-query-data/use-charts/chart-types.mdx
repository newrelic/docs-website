...
<CollapserGroup>
  ...
  >
    ...
    ファセットを使用して属性を最大5つまで、カンマで区切って追加できます。NRQLクエリで を使用する場合は、[`TIMESERIES`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries) `FACET` 関数を使用します。
