...
<CollapserGroup>
  ...
  >
    ...
    オプションで、ビルボードの<DoNotTranslate>**Warning**</DoNotTranslate>および/または<DoNotTranslate>**Critical**</DoNotTranslate>閾値を設定できます。 設定が完了すると、サービスが正しく機能している場合、チャートのフルボックスは緑色になります。閾値を超えると、チャートは黄色（警告）または赤色（クリティカル）に変わります。クエリビルダーのチャート編集ペインの右側にある閾値フィールドにアクセスします。
    ...
    * 値が所定の値を上回るのでなく下回った場合にチャートを強調表示したい場合、
    ...
    <table>
      <tbody>
        <tr>
          ...
          <td>
            大きくて読みやすいフォントで、<DoNotTranslate>**Critical**</DoNotTranslate>および<DoNotTranslate>**Warning**</DoNotTranslate>状態にある閾値を含めた値を監視する場合。
          </td>
        </tr>
  ...
  >
    ...
    ブレットは、デフォルトで結果のクエリ値の2倍を限度とする値に設定されています。リミットを調整する場合は、<DoNotTranslate>**Limit**</DoNotTranslate>フィールドに別の数値を入力します。ブレットは、それに応じて変更されます。
  ...
  >
    ...
    これを使用するには、 <DoNotTranslate>**Share**</DoNotTranslate>メニューを選択し、 <DoNotTranslate>**Copy JSON**</DoNotTranslate>をクリックします。
  ...
  >
    ...
    折れ線グラフについては、グラフを他のNew Relicエクスペリエンスと結びつけることができます。これは、データをより深く調査し、解決しようとしている問題の根本原因を明らかにするのに役立ちます。グラフ内の興味のあるポイントをクリックすると、エンティティ固有の情報を表示するオプションが表示されます。

    次の例を見てみましょう。

    1. ダッシュボードで、詳しく調べたい情報を含むグラフを開きます。
    ...
    2. グラフの線の上にカーソルを置くと、グラフ上に特定のポイントが表示されます。
    ...
    3. グラフ上のポイントをクリックすると、グラフ内のデータポイントに関連するエンティティを表示するコンテキストメニューが表示されます。
    ...
    4. 関心のあるエンティティをクリックし、フライアウトメニューでそのエンティティのリンクのいずれかをクリックします（<DoNotTranslate>**Entity summary**</DoNotTranslate>と<DoNotTranslate>**Logs**</DoNotTranslate>リンクが表示されます）。
    ...
    * ウィジェットクエリが、`WHERE`句（ソースコード）の`entityGuid`、`entity.guid`、`appName`、`entity.name`または`entityName`でクエリされたエンティティを明示的に定義している場合にのみ、コンテキストメニューに表示されます。
    * ビジュアライゼーションは、機能内のその位置を認識しないため、接続が次のバージョンまで循環する可能性があります（たとえば、APM概要からAPM概要へ）。
    * 最初の一連のアクションは、APM概要とログに限定されます。
    * この機能は、折れ線グラフと積み上げ面グラフでのみサポートされます。
  </Collapser>
