...
<CollapserGroup>
  ...
  >
    ...
    アドバンストモードでは、ファセットを使用して属性を最大5つまで、カンマで区切って追加できます。NRQLクエリで`FACET`を使用する場合は、[`TIMESERIES`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries)関数を使用します。
