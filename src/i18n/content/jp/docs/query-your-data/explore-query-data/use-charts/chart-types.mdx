...
バーチャートは、クエリによって返されたカテゴリーまたはシリーズ間の値を比較します。これは、ほとんどの非時系列クエリのデフォルトの視覚化です。
...
<figcaption>
  バーチャートは、カテゴリーまたはシリーズ間の値を比較します。
</figcaption>

バーチャートは、カテゴリー（非時系列）データと時系列データの両方をサポートしています。カテゴリーの比較を行うには、次のようにクエリに[`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet)句を追加します：
...
複数のシリーズを1つの最大のシリーズと比較するには、代わりに[簡易バーチャート](#widget-simplified-barchart)を使用します。簡易バーチャートと比較して、バーチャートでは以下が提供されません：

* 無限シリーズスクロール
* 水平で重なりのないラベル
* バーの幅の100％に固定された最初のシリーズ
...
<table>
  <tbody>
    <tr>
      ...
      <td>
        設定可能なy軸を使用して、カテゴリーまたはシリーズ間の合計を比較します。バーチャートは、ほとんどの非時系列クエリのデフォルトの視覚化です。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        複数のシリーズを1つの最大のシリーズと比較します。そのビューには、[簡易バーチャート](#widget-simplified-barchart)が適しています。
      </td>
    </tr>
  </tbody>
</table>
...
閾値を設定し、値に応じてゲージを色分けできます（任意）。ゲージチャートは、<DNT>**Good**</DNT>、<DNT>**Warning**</DNT>、<DNT>**Approaching critical**</DNT>、<DNT>**Critical**</DNT>、<DNT>**Neutral**</DNT>の閾値をサポートしています。各閾値は指定した値から適用され、値がそれぞれの範囲に入るとゲージの色が変わります。<DNT>**Neutral**</DNT>の閾値を使用して、正常でも異常でもない範囲をマークすることで、ゲージがそれらの値に対して良い状態や悪い状態を示さないようにします。
...
簡易バーチャートは、複数のシリーズを最大のシリーズと比較します。チャートは常に最大のシリーズをバーの幅の100％として扱います。他のシリーズは、その最大のシリーズに対する相対値として表示されます。
...
簡易バーチャートは、シリーズの無限スクロールと水平で重なりのないラベルを提供し、最初のシリーズをバーの幅の100％に固定します。

<table>
  <tbody>
    <tr>
      <td>
        以下の場合は、簡易バーチャートを使用します。
      </td>

      <td>
        複数のシリーズを最大のシリーズと比較します。チャートは最大のシリーズをバーの幅の100％として扱います。
      </td>
    </tr>

    <tr>
      <td>
        以下の場合は、簡易バーチャートの使用を避けます。
      </td>
