...
バーチャートは、クエリによって返されたカテゴリー間またはシリーズ間の値を比較します。ほとんどの非時系列クエリで、デフォルトの可視化手法として使用されます。
...
<figcaption>
  バーチャートは、カテゴリー間またはシリーズ間の値を比較します。
</figcaption>

バーチャートは、カテゴリー（非時系列）データと時系列データの両方をサポートしています。カテゴリー間の比較を行うには、次のようにクエリに[`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet)句を追加します。
...
バーチャートはY軸の範囲を自動的に設定しますが、データに合わせて設定することもできます。

複数のシリーズを最大のシリーズのいずれかと比較する場合は、通常のバーチャートの代わりに[簡易バーチャート](#widget-simplified-barchart)を使用します。簡易バーチャートと比較して、通常のバーチャートには以下の機能がありません。
...
<table>
  <tbody>
    <tr>
      ...
      <td>
        設定可能なY軸を使用して、カテゴリー間またはシリーズ間の合計を比較します。ほとんどの非時系列クエリで、バーチャートがデフォルトの可視化手法として使用されます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        複数のシリーズを最大のシリーズのいずれかと比較します。この表示には、[簡易バーチャート](#widget-simplified-barchart)が適しています。
      </td>
    </tr>
  </tbody>
</table>
...
ゲージチャートは、定義した範囲内で単一の値を塗りつぶされた円弧として表示します。これにより、最小値、最大値、設定した任意の閾値に対し、その値がどこに位置するかを一目で確認できます。これは、このクエリから、クエリが単一の数を返す場合のオプションです。
...
<figcaption>
  ダッシュボード上のゲージチャート。各ゲージは円弧を塗りつぶして、単一の値が最小値と最大値の間のどこにあるかを示します。また、閾値の色分けにより、値が警告またはクリティカルの範囲に入っていることが分かります。
</figcaption>

ゲージチャートを使用するには、単一の値を返すクエリを実行します。ファセットを追加すると、一度に複数のゲージを表示できます。ファセットを追加したクエリでは、ファセット値ごとに1つずつゲージがグリッド表示されます。
...
閾値を設定し、値に応じてゲージを色分けできます（任意）。ゲージチャートは、<DNT>**Good**</DNT>、<DNT>**Warning**</DNT>、<DNT>**Approaching critical**</DNT>、<DNT>**Critical**</DNT>、<DNT>**Neutral**</DNT>の閾値をサポートしています。各閾値は指定した値から適用され、値がそれぞれの範囲に入るとゲージの色が変わります。<DNT>**Neutral**</DNT>の閾値を使用して、正常でも異常でもない範囲を示すことで、その範囲の値についてゲージが良好または不良な状態を示さないようにします。

閾値を使用する上で役立つヒントをいくつかご紹介します。

* 閾値を設定する際は、その閾値の適用を開始したい値を指定します。閾値は値が次の閾値に達するまで適用されます。最初の閾値はゲージの最小値から始まります。
* 値が高すぎる場合（高いほど悪い場合）にゲージを目立たせるには、閾値を低い順から高い順に並べます。値が低すぎる場合（空き容量など、高いほど良い場合）に目立たせたいときは、順序を逆にして、最も重大度の高い閾値が最小値から始まるようにします。
* <DNT>**Show threshold markers**</DNT>設定を使用すると、各閾値の境界で円弧に目盛りを表示できます。

<DNT>**Gauge settings**</DNT>

ゲージチャートには次の設定項目があります。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ゲージ範囲の最小値を設定します。円弧はこの値から塗りつぶされます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ゲージ範囲の最大値を設定します。円弧はこの値まで塗りつぶされます。
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ゲージに表示するラベルを選択します。選択可能なオプション：**Auto**（自動）、**Value and name**（値と名前）、**Value only**（値のみ）、**Name only**（名前のみ）、**None**（表示しない）。
      </td>
    </tr>
...
<table>
  <tbody>
    <tr>
      <td>
        次の場合には、ゲージチャートが適しています。
      </td>

      <td>
        CPU、メモリ、ディスク使用量を容量に対する割合で表示する場合など、単一の値を固定範囲や閾値に照らしてコンパクトに見やすい形で監視したい場合。
      </td>
    </tr>

    <tr>
      <td>
        次の場合には、ゲージチャートは適していません。
      </td>

      <td>
        値の経時変化を表示したい場合や、系列ごとに複数の値を表示したい場合。データの経時変化を表示するには、[折れ線グラフ](#widget-linechart)が適しています。
      </td>
    </tr>
  </tbody>
</table>
...
<Callout variant="important">
  次の[集計関数](/docs/insights/new-relic-insights/using-new-relic-query-language/nrql-reference#functions)を円グラフとして視覚化するには、 `FACET`句（`average`、`apdex`、`min`、 `max`、 `percentage`、`percentile`）が必要です。たとえば、`SELECT percentile(duration) FROM Transaction FACET appName`は円グラフに対応していますが、`SELECT percentile(duration) FROM Transaction`は対応していません。
</Callout>

`uniqueCount`では、パーセンテージを[100%以上に集計できます](#pie-percentage-problem)。
...
## 簡易バーチャート [#widget-simplified-barchart]

簡易バーチャートは、複数のシリーズを最大のシリーズと比較します。チャートでは常に最大のシリーズをバー全体の幅で表示されます。他のシリーズは、その最大のシリーズに対する相対的な幅で表示されます。
...
<figcaption>
  簡易バーチャートは、一連のシリーズを最大のシリーズと比較してランク付けします。
</figcaption>

簡易バーチャートを有効にするには、クエリに[`FACET`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-facet)句を追加します。`FACET`句は、カンマで区切られた最大5つの異なる属性に使用できます。これらのクエリではバーチャートがデフォルトであるため、クエリを実行してから、<DNT>**Chart type**</DNT>ドロップダウンから<DNT>**Simplified Bar**</DNT>を選択します。

簡易バーチャートは、シリーズのスクロール切り替えと、水平表示で重ならないラベルを備えており、最初のシリーズがバー全体の幅に固定されます。

<table>
  <tbody>
    <tr>
      <td>
        以下の場合は、簡易バーチャートを使用してください。
      </td>

      <td>
        複数のシリーズを最大のシリーズと比較します。チャートでは最大のシリーズをバー全体の幅で表示します。
      </td>
    </tr>

    <tr>
      <td>
        以下の場合は、簡易バーチャートを使用しないでください。
      </td>
