...
<CollapserGroup>
  ...
  >
    ...
    <Callout title="プレビュー">
      次の折れ線グラフ機能は、現在プレビュー段階です。
    </Callout>

    折れ線グラフについては、グラフを他のNew Relicエクスペリエンスと結びつける今後の機能を試すことができます。これは、データを深く掘り下げ、解決しようとしている問題の根本原因を明らかにする際に役立ちます。関心のあるグラフのポイントをクリックすると、エンティティ固有の情報を閲覧するオプションが表示されます。
    ...
    4. 関心のあるエンティティをクリックし、フライアウトメニューでそのエンティティのリンクのいずれかをクリックします（このプレビューでは、**エンティティの概要**と**ログ**リンクが表示されます）。
