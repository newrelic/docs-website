...
閾値を設定し、値に応じてゲージを色分けできます（任意）。ゲージチャートは、<DNT>**Good**</DNT>、<DNT>**Warning**</DNT>、<DNT>**Approaching critical**</DNT>、<DNT>**Critical**</DNT>の閾値をサポートしています。各閾値は指定した値から適用され、値がそれぞれの範囲に入るとゲージの色が変わります。
