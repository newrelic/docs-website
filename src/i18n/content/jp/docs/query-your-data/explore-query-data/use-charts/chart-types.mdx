...
<CollapserGroup>
  ...
  >
    ...
    ![平均CPU使用率を示す面グラフタイプの例のスクリーンショット。](src/images/chart_area.png "平均CPU使用率を示す面グラフタイプの例。")
    ...
    ファセットを使用して属性を最大5つまで、カンマで区切って追加できます。NRQLクエリで[FACET`を使用する場合は、`](/docs/insights/nrql-new-relic-query-language/nrql-reference/nrql-syntax-components-functions#sel-timeseries)TIMESERIES関数を使用します。
  ...
  >
    ...
    ![ブラウザの使用状況の比較を示すバーチャートの例のスクリーンショット。](src/images/chart_bar.png "ブラウザの使用状況の比較を示すバーチャートの例。")
  ...
  >
    ...
    ![総セッション数を示すビルボードチャートの例のスクリーンショット。](src/images/chart_billboard.png "総セッション数を示すビルボードチャートの例。")

    <figcaption>
      ビルボードチャートは、単一の値を大きく読みやすいフォントで表示します。
    </figcaption>

    データをビルボードチャートで使用できるようにするには、単一の値を検索します。ドロップダウンからファセットを追加することで、複数の値を表示できます。

    ビルボードで**Warning（警告）**と**Critical（クリティカル）**の閾値を設定でき、チャートを黄色または赤色でハイライト表示します。閾値フィールドは、チャート編集ペインの右側にあり、それぞれ黄色と赤色でハイライト表示されています。
  ...
  >
    ...
    ![ページビューの合計数のうち、一意のページビューの合計数を示すブレットチャートの例のスクリーンショット。](src/images/chart_bullet.png "ページビューの合計数のうち、一意のページビューの合計数を示すブレットチャートの例。")
  ...
  >
    ...
    ![アカウントに登録し、機能比較ページを訪問し、アップグレードしたユーザーの数を表示するNRQLファネルクエリの例のスクリーンショット。](src/images/crop-insights-funnels.png "アカウントに登録し、機能比較ページを訪問し、アップグレードしたユーザーの数を表示するNRQLファネルクエリの例。")
  ...
  >
    ...
    ![ページビューの内訳をオペレーティングシステムごとに示すヒートマップチャートの例のスクリーンショット。](src/images/chart_heatmap.png "オペレーティングシステムごとのページビューの内訳を示すヒートマップ グラフの例のスクリーンショット。")
  ...
  <Collapser
    ...
    title="ヒストグラム"
  >
    ![経時的なページビュー期間を示すヒストグラムの例のスクリーンショット。](src/images/crop-insights-histogram-chart.png "経時的なページビューの期間を示すヒストグラムの例。")
  ...
  >
    ...
    ![経時的なページビュー数を示す折れ線グラフの例のスクリーンショット。](src/images/chart_line.png "経時的なページビュー数を示す折れ線グラフの例。")
  ...
  >
    ...
    ![円グラフの例のスクリーンショット。](src/images/chart_pie.png "円グラフの例。")
  ...
  >
    ...
    * テーブルチャートはCSVファイルにエクスポートできます。この機能の詳細：
      ...
      * 現時点に依存する時間範囲のチャートの場合（たとえば、` 30分前以来`）、CSVのデータは、チャートが初めて生成されたときではなく、現時点から生成されます。
      ...
      ![表チャートの例のスクリーンショット。](src/images/chart_table.png "表チャートの例。")
