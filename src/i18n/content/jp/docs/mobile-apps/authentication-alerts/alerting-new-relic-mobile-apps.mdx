...
アカウント管理者は、AndroidおよびiOSデバイスでプッシュ通知を受信する構成を、 [アラート](/docs/alerts/new-relic-alerts-beta/getting-started/new-relic-alerts) から設定することができます。ユーザーチャネルをポリシーにアタッチすることで、任意のポリシーからアラートを受け取ることができます。

## 要件 [#requirements]

この機能は、 [オリジナルのユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) のユーザーのみが利用でき、 [New Relic One のユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models) のユーザーは利用できません。回避策として、 [メール通知チャンネル](#email) を使用することができます。
...
3. 必要に応じて、\[**確認**]を選択します。
4. 必要に応じて、追加の詳細を表示するには、\[**概要**]、\[**違反**] 、または\[**イベントログ**]を選択します。
