...
New Relic モバイル アプリを使用すると、どこにいてもデータにアクセスできます。 APM からインフラストラクチャ、ブラウザに至るまで、New Relic の機能全体から厳選されたダッシュボードを表示し、構築したダッシュボードにアクセスできます。 さらに詳しく調べる必要がある場合は、NRQL を使用してデータを直接クエリするか、ログを検索できます。 そして、受け取ることができるのは、 <InlinePopover type="alerts"/>アプリ内で直接確認できるため、すぐに調査を開始できます。
