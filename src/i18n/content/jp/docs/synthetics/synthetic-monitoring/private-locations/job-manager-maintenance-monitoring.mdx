...
* `:8080/status/check`: 外形監視 Job Manager が実行する内部ヘルス チェックの詳細を示します。 `HTTP 200`状態が正常であることを意味します。
...
外形監視 Job Manager のコンテナ ログを参照して、外形監視 Job Manager の状態を監視できます。

<CollapserGroup>
  <Collapser id="review-docker-logs" title="Dockerログを見直す">
  ...
  <Collapser id="review-podman-logs" title="Podmanログを確認する">
  ...
  <Collapser id="review-kubernetes-logs" title="Kubernetesログを見直す">
...
<CollapserGroup>
  <Collapser id="docker-debug-logs" title="Dockerデバッグログを有効にする">
    <Callout variant="tip">
  ...
  <Collapser id="podman-debug-logs" title="Podmanデバッグログを有効にする">
    <Callout variant="tip">
  ...
  <Collapser id="kubernetes-debug-logs" title="Kubernetesデバッグログを有効にする">
    <Callout variant="tip">
...
## デバッグ情報を取得する [#retrieve-debugging-information]

外形監視ジョブマネージャーに関するデバッグ情報を取得して、問題のトラブルシューティングに役立てることができます。これには、外形監視ジョブマネージャーのポッドと、それが実行されているノードに関する情報が含まれます。

<CollapserGroup>
  <Collapser id="retrieve-kubernetes-debugging" title="Kubernetes用Retrieve">
  ...
  <Collapser id="retrieve-openshift-debugging" title="OpenShift用に取得">
