...
ロケーションの作成後、[プライベートミニオンをインストール](/docs/synthetics/new-relic-synthetics/private-locations/install-containerized-private-minions-cpms#install-update)できるように、外形監視が<DNT>**Private location key**</DNT>のリストを表示します。
  ...
  <Collapser id="rotate-private-location-keys" title="プライベートロケーションキーのローテーション">
    ...
    <Steps>
      <Step>
        ...
        2. プライベートロケーション名を入力し、<DNT>**Enable verified script execution**</DNT>をオンにします。次に、<DNT>**Generate key**</DNT>をクリックして、新しいプライベートロケーションを作成します。
        ...
        3. ジョブマネージャーまたはプライベートミニオンを新しいプライベートロケーションで更新します。<DNT>**Configure and deploy**</DNT>セクションにはサンプルコードが含まれています。
