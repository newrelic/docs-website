...
Synthetic モニタリングでは、プライベートロケーションとは、プライベートミニオンの集合です。ミニオンはコンテナ化されたアプリケーションであり、Synthetics UI 経由でセットアップされたジョブを受け取り、管理します。

プライベートロケーションは、任意の数のプライベートミニオンを含めることができます。プライベートロケーションでは、Synthetic モニタリングの対象地域を拡張したり、イントラネットサイトなど、ファイアウォールの背後にあるウェブサイトをモニターすることができます。
...
<table>
  ...
  <tbody>
    ...
    <tr id="account-access">
      ...
      <td>
        アクセスの仕組みは、お使いの[ユーザーモデル](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-user-models)によって異なります。
        ...
        * New Relic Oneのユーザーモデル：アカウントへのユーザーアクセスは、[そのアカウントへのアクセスが許可されているかどうか](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts)によって異なります。
      </td>
    </tr>
...
**![screenshot-manage-private-locations.png](src/images/private-locations.png 'screenshot-manage-private-locations.png')**

<figcaption>
  [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > Private
  Locations**：**Private
  Locations**ページを使用して、プライベートロケーションを作成、編集、および削除します。
</figcaption>
...
ロケーションの作成後、Synthetics は、**プライベートミニオンをインストール**できるように、[プライベートロケーションキー](/docs/synthetics/new-relic-synthetics/private-locations/install-containerized-private-minions-cpms#install-update)をリストします。

## Ping モニターチェック [#checks]

各ミニオンは、1 分間に約 200 の ping モニターチェックを実行できます（月に 8,640,000 チェック）。特定のロケーションのジョブキューが大きくなる場合は、[ミニオンを追加](/docs/synthetics/new-relic-synthetics/private-locations/install-containerized-private-minions-cpms#install-update)します。次の項目により、ミニオンの正確な容量は異なります：
...
![screen-assign-to-private-location-synthetics-monitor.png](src/images/screen-assign-to-private-location-synthetics-monitor_0.png 'screen-assign-to-private-location-synthetics-monitor.png')

<figcaption>
  [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics > Create
  monitor**：プライベートロケーションとそのミニオンに新しいジョブを割り当てるには、**Create
  monitor**ページから名前を選択します。
</figcaption>
...
Synthetic モニタリングには、ロケーションおよび個別のミニオンを管理するツールが含まれます。[新しいミニオンをインストール](/docs/synthetics/new-relic-synthetics/private-locations/install-containerized-private-minions-cpms#install-update)し、ジョブキューが停滞する場合はクリアできます。
...
ローカルのプライベートロケーションから実行される Synthetic のスクリプト化モニターにプロキシサーバー構成を設定できます。詳細については、[Synthetics プロキシ設定](/docs/synthetics/new-relic-synthetics/scripting-monitors/set-proxy-settings-scripted-monitors)をご覧ください。
