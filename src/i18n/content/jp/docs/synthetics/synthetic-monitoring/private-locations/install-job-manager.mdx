...
シンセティック モニタリング ジョブ マネージャーは、 [Docker コンテナー](https://www.docker.com/resources/what-container)ベースのリソースであり、 [プライベート ロケーション](/docs/synthetics/new-relic-synthetics/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations)に割り当てられた[シンセティック モニター](/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors)を受け入れて実行します。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title={<><img src={integrationK8} title="Kubernetes icon" alt="Kubernetes icon" style={{ height: '35px', width: '40px' }}/>Kubernetes コンテナ オーケストレーション システムの環境要件</>}
  >
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            * Node API ランタイム ポッドに割り当てられたリソースは、ユーザーが構成できます。
            * CPUとメモリの両方の最大制限要求リソース比率は2です。
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            ...
            * ノード ブラウザ ランタイム ポッドに割り当てられたリソースは、ユーザーが設定できます。
            * CPUとメモリの両方の最大制限要求リソース比率は2です。
          </td>
        </tr>
...
合成ジョブ マネージャーを起動する前に、 [プライベート ロケーション キー](/docs/synthetics/new-relic-synthetics/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations#create-location)が必要です。Synthetics ジョブ マネージャーはキーを使用して New Relic に対して認証し、そのプライベートな場所に関連付けられたモニターを実行します。
...
高度な構成設定の詳細については、 [synthetics ジョブ マネージャーの構成](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-configuration)を参照してください。
...
<CollapserGroup>
  ...
  >
    1. 削除する Synthetics ジョブ マネージャー ポッドの<var>JOB_MANAGER_POD_INSTALLATION_NAME</var>を取得します。
...
<CollapserGroup>
  ...
  >
    ...
    * Docker ログを追跡し、モニターの正常性を確認するには、 [synthetics ジョブ マネージャーのメンテナンスと監視](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-maintenance-monitoring)を参照してください。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        プロキシによる New Relic との通信を設定するには、次の名前の[環境変数](/docs/synthetics/new-relic-synthetics/private-locations/job-manager-configuration#environment-variables)を使用します。 `HORDE_API_PROXY*.`
      </td>
    </tr>
