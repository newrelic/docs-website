...
当社の外形監視ジョブマネージャーは、[ プライベートロケーション に割り当てられた 外形監視モニター を受け付けて実行する、](/docs/synthetics/synthetic-monitoring/using-monitors/intro-synthetic-monitoring/#types-of-synthetic-monitors) [](/docs/synthetics/synthetic-monitoring/private-locations/private-locations-overview-monitor-internal-sites-add-new-locations)[コンテナ](https://www.docker.com/resources/what-container) ベースのリソースです。ジョブマネージャーは、Dockerコンテナシステム環境、Podmanコンテナシステム環境、Kubernetesコンテナオーケストレーションシステム環境、またはOpenShiftコンテナシステム環境で動作できます。

<Callout variant="important">
  Node.js 16/Chrome 134ランタイムからNode.js 22/Chrome 146に移行する場合は、移行戦略、デプロイメントの例、トラブルシューティングの手順について、[プライベートロケーションのランタイム移行ガイド](/docs/synthetics/synthetic-monitoring/private-locations/runtime-transition-guide)を参照してください。
</Callout>
...
<CollapserGroup>
  <Collapser id="docker-requirements" title="Dockerコンテナのシステム環境要件">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18以上（v447以降を推奨）<br /> <DNT>**macOS:**</DNT> 10.11以降<br /><DNT>**Windows:**</DNT> Windows 10 64ビット以上
    ...
    <Callout variant="caution">
      外形監視 Job Manager v447 (Ubuntu 24.04 をベースイメージとして使用) 以降、最新の glibc および seccomp の要件により、RHEL 7 および 4.18 より前の Linux カーネル バージョンを使用するその他のディストリビューションはサポートされなくなりました。 安定したエクスペリエンスを得るには、RHEL 8/9 または同等の最新の Linux ディストリビューションを使用してください。
    </Callout>
  </Collapser>

  <Collapser id="podman-requirements" title="Podman コンテナのシステム環境要件">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18以上（v447以降を推奨）<br />
          </td>
        </tr>
  ...
  <Collapser id="kubernetes-requirements" title="Kubernetes コンテナ オーケストレーション システム環境要件">
    <table>
      <thead>
        <tr>
          <th style={{ width: "265px" }}>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> 4.18以上（v447以降を推奨）<br /> <DNT>**macOS:**</DNT> 10.11以降<br />
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            <DNT>**Linux kernel:**</DNT> [4.18以上（v447以降を推奨）](https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/deploying_installer-provisioned_clusters_on_bare_metal/ipi-install-prerequisites#minimum-resource-requirements)<br /> <DNT>**macOS:**</DNT> 10.11以降<br />
...
同じホスト上で複数のDockerまたは Podman プライベートロケーション コンテナーを実行している場合、ポートの競合が発生します。 ポート競合を回避するには、各ジョブマネージャを別々のホストで実行するようにしてください。
...
<CollapserGroup>
  <Collapser id="docker-update" title="Docker 起動手順">
  ...
  <Collapser id="podman-update" title="Podman 起動手順">
    1. [プライベートロケーションキー](#private-location-key)を見つけます。
  ...
  <Collapser id="kubernetes-install" title="Kubernetes の起動手順">
    1. [プライベートロケーションキー](#private-location-key)を見つけます。
...
<CollapserGroup>
  <Collapser id="docker-stop" title="Docker 停止手順">
  ...
  <Collapser id="podman-stop" title="Podman 停止手順">
  ...
  <Collapser id="kubernetes-delete" title="Kubernetes 削除手順">
...
<CollapserGroup>
  <Collapser id="docker-dependencies" title="Dockerの依存関係">
...
<CollapserGroup>
  <Collapser id="podman-dependencies" title="Podmanの依存関係">
...
<CollapserGroup>
  <Collapser id="run-non-root" title="Docker の非 root ユーザーとして実行する">
