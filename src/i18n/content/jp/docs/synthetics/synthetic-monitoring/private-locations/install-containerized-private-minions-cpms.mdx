...
<CollapserGroup>
  ...
  >
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>
            OS とアーキテクチャは[dockerがサポートするプラットフォームで](https://docs.docker.com/engine/install/#supported-platforms)ある必要があります。
        ...
        <tr>
          ...
          <td>
            <DNT>**Ping:**</DNT> 無視できる<br /><DNT>**Scripted API:**</DNT> 7<br /> <DNT>**Simple browser:**</DNT> 27<br /> <DNT>**Scripted browser:**</DNT> 28
        ...
        <tr>
          ...
          <td>
            Docker エンジンのバージョンは[17.12.1-ce](https://docs.docker.com/engine/release-notes/17.12/)から始まり、バージョン 25 までです。
          </td>
        </tr>
    ...
    <Callout variant="caution">
      docker 1分間あたりの呼発信回数は、重大な変更のため、 docker Engine 26.0以降をサポートしていません。 docker 26+ のサポートを求めているお客様は、[外形監視 Job Manager](/docs/synthetics/synthetic-monitoring/private-locations/job-manager-transition-guide/)に移行する必要があります。
    </Callout>
  ...
  <Collapser
    ...
    title={<><img src="/images/synthetic_logo_k8logo.webp" title="Kubernetes icon" alt="Kubernetes icon" style={{ height: '35px', width: '40px' }} />Kubernetesコンテナ オーケストレーション システム環境要件 (1分間あたりの呼発信回数 v3.0.0 以降)</>
    }
  >
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <DNT>**Root volume:**</DNT> 最小 50 GiB (ノード + PV)<br /> <DNT>**Persistent volume (PV):**</DNT>最小 20 GiB
        ...
        <tr>
          ...
          <td>
            <DNT>**Ping:**</DNT> 無視できる<br /><DNT>**Scripted API:**</DNT> 11<br /> <DNT>**Simple browser:**</DNT> 33<br /> <DNT>**Scripted browser:**</DNT> 31
        ...
        <tr>
          ...
          <td>
            Kubernetesクラスタが[Kubernetes v1.15](https://kubernetes.io/blog/2019/06/19/kubernetes-1-15-release-announcement/)をサポートすることをお勧めします。

            <Callout variant="caution">
              Kubernetes v1.21 以降の場合は、ミニオン リリース[v3.0.61](/docs/release-notes/synthetics-release-notes/containerized-private-minions-release-notes/containerized-private-minion-cpm-3061)以降を使用します。
            </Callout>
          </td>
        </tr>
...
2. <DNT>**Private locations**</DNT>インデックスで、1 分間あたりの発信回数を割り当てるプライベート ロケーションを見つけます。
...
<CollapserGroup>
  ...
  >
    ...
    * 書き込み可能なDockerUNIXソケットは、 `/var/run/docker.sock`または`DOCKER_HOST` [環境変数](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration#environment-variables)にマウントされます。詳細については、Dockerの[デーモンソケットオプション](https://docs.docker.com/engine/reference/commandline/dockerd/#daemon-socket-option)を参照してください。
      ...
      <Callout variant="caution">
        ホスト上のコア数によって、1分間あたりの呼発信回数がホスト上で同時に実行できるランナー コンテナの数が決まります。 メモリ要件はランナー コンテナの予想される数に合わせて調整されるため、リソースの競合を避けるために<DNT>**not running multiple CPMs on the same host**</DNT>を推奨します。
      </Callout>
...
カスタム ノード モジュールを使用してコンテナ化されたプライベートミニオンを構成したり、リリース間でデータを保存したり、環境変数を使用したりできます。 詳細については、 [「1分間の呼発信回数の設定」を](/docs/synthetics/new-relic-synthetics/private-locations/containerized-private-minion-cpm-configuration)参照してください。
