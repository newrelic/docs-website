...
**2. 1 分あたりのキューの増加率 ($R\_growth$) を調べます。**このクエリは、時系列チャートで`jobManagerHeavyweightJobs`キューの 1 分あたりの平均増加を計算します。ゼロより上の線はキューが増加していることを示し、ゼロより下の線はキューが減少することを意味します。
...
$$並列度*最大 \approx 15 \times D* avg,m$$

ここで、$D\_avg,m$ は、**分単位**での**平均的なヘビーウェイトジョブの所要時間**です。
...
ここで、$D\_avg,s$ は**ジョブの平均所要時間****秒**です。

$$並列処理 = \fracN\_m完了数$$

ここで、$N\_m$ は**5 分**ごとに実行する必要がある合成ジョブの**数**です。
