...
## Docker、Kubernetes、OpenShift のサイズ設定に関する考慮事項 [#kubernetes-sizing]

### Docker [#docker]

プライベートロケーションを効率的に実行するには、監視ワークロードを処理するのに十分な CPU リソースをDockerホスト上にプロビジョニングする必要があります。 サイズはさまざまな要因によって左右されますが、ニーズはすぐに見積もることができます。

**同時実行されている各ヘビーウェイト モニター (つまり、各スクリプト ブラウザーまたは各スクリプト API テスト) ごとに 1 つの CPU コア**が必要になります。

現在のセットアップを診断する場合でも、将来のセットアップを計画する場合でも、必要なコアの数を計算するのに役立つ 2 つの式を以下に示します。
...
#### 式2: 新しい場所または将来の場所を予測する場合

新しいプライベートロケーションを設定している場合、またはモニターの追加を計画している場合は、この公式を使用して事前にニーズを予測してください。

**方程式:** $$C\_req = N\_m \times F\_j \times D\_j$$

* $C\_req$ =**必要なCPUコア数**
* $N\_m$ = 実行を計画している重量級モニターの合計**数**。
* $F\_j$ = 1 分あたりのジョブ数におけるモニターの平均**頻度**(たとえば、5 分ごとに実行されるモニターの頻度は 1/5 または 0.2)。
* $D\_j$ = ジョブの**平均所要時間**（分）。
...
これらの書き込みを[書き込みビルダー](/query-your-data/explore-query-data/get-started/introduction-querying-new-relic-data/)で実行して、診断式の入力を取得できます。 安定した平均値を得るために、時間範囲を十分に長い期間に設定してください。
...
**3. 平均ジョブ実行時間を分単位で検索します ($D\_j$):**このクエリは、完了した非 ping ジョブの平均実行時間を検索し、結果をミリ秒から分に変換します。`executionDuration`使用する理由は何ですか?これは、ホスト上でジョブを実行するのにかかった時間を表します。これが測定対象です。
