    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            米国ベースのアカウント（デフォルト）の場合、エンドポイントは次のとおりです： `https://synthetics-horde.nr-data.net.`
            ...
            JPベースのアカウントの場合、エンドポイントは次のとおりです： `https://synthetics-horde.jp.nr-data.net/`
...
SJMにはカスタムノードモジュールが用意されています。これらを使用すると、[ノード モジュール](https://docs.npmjs.com/about-packages-and-modules)のカスタマイズされたセットを作成し、外形監視用のスクリプト化された監視 (スクリプト化されたAPIおよびスクリプト化されたブラウザ) で使用することができます。
...
## プライベートロケーション用のカスタムCA証明書を構成する [#custom-ca-certificates]

Syntheticモニターがプライベート認証局（CA）によって保護された内部HTTPSエンドポイントに接続する必要がある場合、カスタムCA証明書を直接SJMにマウントできます。

SJMはこれらの証明書を自動的に検出し、起動時に各ランタイムコンテナに渡します。これにより、カスタムランタイムイメージを構築または保守することなく、モニターが内部エンドポイントを信頼することが保証されます。

### 証明書の要件

SJMが証明書を正常にロードできるようにするには、証明書が以下の基準を満たしている必要があります：

* **形式:** 証明書は、ファイル拡張子が`.pem`のPEM形式である必要があります。
* **パーミッション:** ファイルはUID 2000（ランタイムコンテナを実行する非rootユーザー）によって読み取り可能である必要があります。
* **証明書タイプ:** CA証明書のみがロードされます。`CA:TRUE`基本制約拡張がないリーフ証明書（サーバー証明書またはクライアント証明書）は自動的に無視されます。

### デプロイメント環境

証明書をマウントするには、以下のコンテナオーケストレーションプラットフォームを選択してください。

<CollapserGroup>
  <Collapser id="custom-ca-docker" title="Docker">
    PEMファイルを含むローカルホストディレクトリを、読み取り専用ボリュームとしてSJMコンテナパス`/var/lib/newrelic/synthetics/certs`にマウントします。
    ...
    **仕組み:** SJMは起動時にこのマウントを検出し、生成するすべての動的コンテナに証明書を読み取り専用として伝播します。
  </Collapser>

  <Collapser id="custom-ca-podman" title="ポッドマン">
    Podman環境（特にSELinuxを有効にして実行している環境）では、正しいセキュリティコンテキストを構成するために、ボリュームマウントに`:z`または`:Z`フラグを追加する必要があります。
  ...
  <Collapser id="custom-ca-kubernetes" title="Kubernetes（Helm）">
    Helm経由でSJMをKubernetesクラスタにデプロイする場合、カスタムCAバンドルを汎用のKubernetesシークレットとして管理できます。

    **1. Kubernetesシークレットを作成する**

    PEM証明書ファイルをNew Relicエージェントのネームスペース内のシークレットに保存します：
    ...
    **2. Helmの値を更新する**
    ...
    カスタム`values.yaml`設定で、新しく作成したシークレットを参照します：
    ...
    **仕組み:** 起動時に、SJMはこのシークレットをマウントし、新しく生成されたすべての実行ポッドに自動的に渡します。
  </Collapser>
</CollapserGroup>

## mTLSクライアント証明書 [#mtls-client-certificates]

モニターが相互TLS（mTLS）を強制する内部エンドポイントに接続する場合 — クライアントに証明書の提示を要求する場合 — ターゲットのホスト名にマッピングされたクライアント証明書を提供できます。モニターは正しい証明書を自動的に提示します。スクリプトの変更は必要ありません。

**要件：**

* 証明書とキーファイルはPEM形式である必要があります
* ホスト名を証明書およびキーファイルにマッピングする`cert-map.json`ファイルが必要です。
...
ホスト名の一致の優先順位：完全一致 → ワイルドカード（`*.domain.com`） → `default` キャッチオール → 証明書なし（非mTLS接続は影響を受けません）。

<CollapserGroup>
  <Collapser id="mtls-docker" title="Docker">
    `cert-map.json`と、参照される証明書およびキーファイルを含むディレクトリをマウントします:
    ...
    ランタイムコンテナ内のDNS経由でmTLSサーバーのホスト名を解決できない場合は、`RUNTIME_EXTRA_HOSTS`を使用してホスト名のマッピングを追加します：
  ...
  <Collapser id="mtls-podman" title="ポッドマン">
    クライアント証明書ディレクトリをマウントします。SELinuxの場合は`:z`または`:Z`を追加します:
  ...
  <Collapser id="mtls-kubernetes" title="Kubernetes">
    `cert-map.json`と参照されるすべての証明書およびキーファイルを含むKubernetesシークレットを作成します：
    ...
    シークレットは、すべてのランタイムポッドに自動的に読み取り専用でマウントされます。
  </Collapser>
</CollapserGroup>

## 証明書サポートの制限事項 [#certificates-support-limitations]

特に明記されていない限り、以下の制限はカスタムCA証明書とmTLSクライアント証明書の両方に適用されます：

* **証明書の変更には再起動が必要** — 証明書はコンテナの起動時に読み込まれます。証明書の追加、削除、またはローテーションを行った後、変更を適用するには、ジョブマネージャーコンテナ（またはKubernetesのランタイムポッド）を再起動します。
* **サポートされているランタイム** — カスタム証明書機能は、スクリプト化APIおよびスクリプト化ブラウザモニターにのみ適用されます。Pingモニターは影響を受けません。
* **証明書ごとに一意のサブジェクトCN（mTLSのみ）** — 各クライアント証明書には、一意のサブジェクト共通名（CN）が必要です。Chromeは、ホスト名ごとに正しい証明書を識別して自動選択するためにCNを使用します。
...
<CollapserGroup>
  <Collapser id="docker-storage" title="Dockerの場合">
  ...
  <Collapser id="podman-storage" title="Podman の場合">
  ...
  <Collapser id="kubernetes-storage" title="Kubernetesの場合">
    ### Kubernetes
...
## サイジングに関する考慮事項 [#vm-sizing]

<CollapserGroup>
  <Collapser id="docker-podman-sizing" title="DockerおよびPodmanの場合">
    ...
    * $C\_\{est}$ =**推定 CPU コア数**。
    * $R\_\{proc}$ = 1 分あたりに**処理さ**れる重量ジョブの**レート**。
    * $R\_\{growth}$ = `jobManagerHeavyweightJobs`キューが1分あたりに**増加する****割合**。
    * $D\_\{avg,m}$ = 重量級ジョブの**平均所要時間****（分）** 。

    この式は、システムが*処理している*ジョブとキューに*蓄積されて*いるジョブを追加して、実際のジョブ到着率を計算します。この合計負荷に平均ジョブ継続時間を掛けると、キューに入れずにすべての作業をクリアするために必要なコア数が正確にわかります。
    ...
    * $C\_\{est}$ =**推定 CPU コア数**。
    * $N\_\{mon}$ = 実行予定のヘビーウェイト**モニター**の**総数**。
    * $D\_\{avg,m}$ = 重量級作業の**平均所要時間****（分）** 。
    * $P\_\{avg,m}$ =**分単位**の重量モニターの**平均期間**(たとえば、5 分ごとに実行されるモニターの場合、$P\_\{avg,m} = 5$)。
    ...
    * **ジョブの所要時間（$D\_\{avg,m}$）:** これらは所要時間全体にわたってコアを保持するため、平均には**タイムアウト**するジョブ（多くの場合3分）を含める必要があります。
    ...
    **2. 1 分あたりのキュー増加率 ($R\_\{growth}$) を求めます**。このクエリは、時系列チャート上で`jobManagerHeavyweightJobs`キューの 1 分あたりの平均増加率を計算します。ゼロより上の線は待ち行列が増加していることを示し、ゼロより下の線は待ち行列が減少していることを示します。
    ...
    **4. ジョブの平均実行時間を分単位で取得します ($D\_\{avg,m}$)** : このクエリは、完了した非pingジョブの平均実行時間を取得し、結果をミリ秒から分に変換します。`executionDuration`ジョブがホスト上で実行された時間を表します。
    ...
    **5. 平均ヘビーウェイト モニター期間 ($P\_\{avg,m}$) を見つける:**プライベートロケーションの`jobManagerHeavyweightJobs`キューが増大している場合、既存の結果から平均モニター期間を計算することは正確ではありません。 これは[、合成モニター](https://one.newrelic.com/synthetics)ページのモニターのリストから推定する必要があります。 正しい New Relic アカウントを選択していることを確認してください。また、 `privateLocation`でフィルタリングする必要があるかもしれません。
  ...
  <Collapser id="k8s-sizing" title="KubernetesおよびOpenShiftの場合">
    ...
    * `parallelism`設定は、特定のランタイムのポッドが同時にいくつ実行されるかを制御します。
    * `completions`設定は、 `CronJob`がそのランタイムに対して別の Kubernetes ジョブを開始する前に完了する必要があるポッドの数を制御します。
    ...
    **推定完了数:** 5 分間のジョブ ロードが完了するまでにかかる時間を決定します。
    ...
    ここで、$D\_\{avg,m}$ は**ジョブの平均実行時間（分）**です。
    ...
    **推定並列処理:** 5 分間のジョブ負荷を処理するために同時に実行する必要があるワーカー (ポッド) の数を決定します。
    ...
    ここで、$N\_m$は**5分あたりのジョブ数**です。この $P\_\{est}$ 値は、**推定並列度**です。

    **2. Helmデプロイを実行する**

    推定された並列処理と完了値、およびノードあたりの CPU コア数と 1 分あたりに実行する必要がある ping モニターの数に基づいて、 `ping-runtime.replicaCount`最適な推測を使用して Helm デプロイを実行します。

    **3. キューの増加を監視する**

    プライベートロケーションにジョブを送信するように合成モニターを構成し、 `pingJobs`と`jobManagerHeavyweightJobs`の時系列折れ線グラフでキューの増加を確認します。

    * `pingJobs`キューの傾きが正の場合、 `ping-runtime.replicaCount`を増やして再デプロイします。
    * `jobManagerHeavyweightJobs`キューの傾きが正の場合、キューの成長が止まるまで（負の傾き） `parallelism`と`completions`比例して増やします。
    ...
    **4. ポッドの実行状態に基づいて調整する**
    ...
    キューが減少しているかゼロになっている場合は、10 分以上「実行中」状態にあるポッド`node-api-runtime`と`node-browser-runtime`をチェックします。これは、並列処理が高すぎる設定になっており、必要以上のポッドがあることを示しています。

    不必要にリソースを浪費しないようにするには、 `parallelism`と`completions`を減らして、各「実行中」ランタイム ポッドの存続期間を短縮します。Kubernetes ジョブの経過時間を 5 分にすることを目標としている場合、ランタイム ポッドが実行状態にある時間は 5 分未満である必要があります。つまり、ポッドが作成され、実行するジョブをすぐに受信して完了している必要があります。
    ...
    **5. 必要に応じてスケールアウトする**
    ...
    たとえば、 `parallelism: 100` 、 `completions: 600`ではキューがまだ増え続けていますが、10 分以上「実行中」状態のポッドが多数あり、Kubernetes ジョブの経過時間は 20 分です... `parallelism: 50` 、 `completions: 200`を設定し、さらに 2 つのデプロイメントを追加して水平方向にスケールアウトします。これにより、合計 150 個の並列ポッドが生成され、K8s ジョブの存続期間が 20 分未満に短縮されるとともに、長期間「実行中」のポッドの数も削減されます。K8s ジョブの AI モニタリングは 5 ～ 10 分です。

    デプロイメントの追加の詳細については、 [「複数の SJM デプロイメントによるスケールアウト」](#scaling-out-with-multiple-sjm-deployments)を参照してください。
    ...
    <Callout variant="tip">
      ...
      補完に大きな値を設定することは悪い考えではありませんが、K8s で cronjob の`TooManyMissedTimes`に関する警告イベントが発生する可能性があります。
    </Callout>

    ### 複数の SJM デプロイメントによるスケールアウト

    単一のSJMの15ジョブ/分のスループットを超えてスケールするには、**複数の個別のSJM Helmリリース**をインストールする必要があります。
    ...
    2. **設定の完了:***各*SJM に対して、 `completions`も同じ固定値に設定します。必要に応じて調整し、ランタイム (node-browser-runtime と node-api-runtime) ごとに 5 分の Kubernetes ジョブの経過時間をターゲットにします。
    ...
    並列処理と完了を静的な値に固定することで、容量の増減が**Helm リリースの追加または削除の**プロセスとして簡単になります。これにより、SJM が効果的に利用できる並列処理値よりも高い並列処理値でクラスター リソースが無駄に消費されることを回避できます。

    #### インストレーション例

    複数の SJM リリースをインストールする場合は、**リリースごとに一意の名前**を付ける必要があります。すべてのインスタンスは**同じプライベートロケーションキー**を使用して構成する必要があります。

    より短く、管理しやすいリソース名を作成するには、 `fullnameOverride`を設定することを強くお勧めします。たとえば、 `sjm-alpha`と`sjm-beta`という名前の 2 つの SJM を`newrelic`ネームスペースにインストールするには (両方とも固定並列処理と補完で同じ`values.yaml` を使用します):
    ...
    ジョブ キューが拡大しないように、必要な数の SJM に対してこのパターン (`sjm-charlie` 、 `sjm-delta`など) を継続できます。
  </Collapser>
</CollapserGroup>