...
## DockerとPodmanのサイズ設定に関する考慮事項 [#vm-sizing]
...
* **ジョブ実行時間 ($D\_\{avg,m}$):**平均値には、**タイムアウトする**ジョブ (多くの場合約 3 分) も含まれるべきです。これらのジョブは実行時間全体にわたってコアを保持します。
...
## Kubernetes と OpenShift のサイズ設定に関する考慮事項 [#kubernetes-sizing]
...
単一の SJM の 1 分あたり約 15 ジョブのスループットを超えて拡張するには、**複数の個別の SJM Helm リリースを**インストールする必要があります。
