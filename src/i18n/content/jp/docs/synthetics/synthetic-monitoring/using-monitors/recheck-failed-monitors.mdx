...
![Run check "をクリックすると、モニターの状態を再確認できます。](src/images/runcheck.png 'Run check "をクリックすると、モニターの状態を再確認できます。')

<figcaption>
  [**one.newrelic.com Synthetics (select a monitor) Failures (select a check
  failure): To view details about a failed monitor check, click Run check
  .**](http://one.newrelic.com/) **> > > >**
  <Icon name="fe-rotate-cw" />
</figcaption>

<Icon name="fe-rotate-cw" />
**Run check** をクリックすると、New Relic は結果の場所とモニターの現在の設定を使って直ちに再チェックを開始します。ジョブが完了すると、ボタンに再チェックのステータスが表示されます
(`成功` または `失敗`)。再チェックの **Results** ページを再度表示するには、ボタンを再度クリックします。
