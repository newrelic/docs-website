...
1. **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > 合成モニタリング >(モニターを選択)**に移動します。
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > 合成モニタリング >(モニターを選択) > 失敗 > (チェックの失敗を選択):**失敗したモニターチェックの詳細を表示するには、 <Icon name="fe-rotate-cw"/>**チェックを実行します**。
</figcaption>
