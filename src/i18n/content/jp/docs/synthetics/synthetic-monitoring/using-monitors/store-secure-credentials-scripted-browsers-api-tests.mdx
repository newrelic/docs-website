...
New Relic One または [API](/docs/apis/synthetics-rest-api/secure-credentials-examples/use-synthetics-secure-credentials-apis) で、安全なクレデンシャルを設定できます。認証情報は、 [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/) によって管理されるキーで、AES-GCM 256 ビット暗号化を使用して静止状態で安全に保存されます。
...
UIまたは[SyntheticsRESTAPI](/docs/apis/synthetics-rest-api/secure-credentials-examples/use-synthetics-secure-credentials-apis#add-secure-credential)を使用して、安全なクレデンシャルを追加または更新できます。値は表示できず、キーのみが表示されることに注意してください。
...
スクリプトの結果は、実際には`https://example.com/Pass123!`に移動したにもかかわらず、NewRelicSyntheticsが`https://example.com/_SECURECREDENTIAL_`に移動したことを示しています。これにより、安全なクレデンシャルの値が結果に表示されなくなります。
