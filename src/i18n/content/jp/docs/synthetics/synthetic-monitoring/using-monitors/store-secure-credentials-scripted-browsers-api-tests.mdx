...
New Relic または[Nerdgraph API](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials)を使用して、安全な認証情報を設定できます。認証情報は、 [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/)によって管理される鍵を使用した AES-GCM 256 ビット暗号化によって安全に保管されます。
...
UIまたは[外形監視 Nerdgraph API](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials)を使用して、セキュリティ認証情報を追加または更新できます。 値自体は表示できず、キーのみ表示されます。
...
2. 新しい安全な資格情報を追加するには、 <DNT>**Create secure credential +**</DNT>ボタンを探します。 資格情報がすでに追加されている場合、このボタンは右上にあります。
   ...
   * <DNT>**Value**</DNT>を作成するためのヒント：英数字または特殊文字の任意の組み合わせを使用できます。最大10000文字。このフィールドは[Nerdgraph API](/docs/apis/nerdgraph/examples/synthetics-api/secure-credentials)経由ではアクセスできません。
