...
<Callout variant="tip">
  pingモニターについては、 [View ping monitor
...
![シンセティック・モニタリング - 結果ページ](src/images/synthetic-monitoring-results.png 'シンセティック・モニタリング - 結果ページ')

<figcaption>
  [**one.newrelic.com**](http://one.newrelic.com/) **> Synthetics> (select a
  monitor)> Results> (select a result)**:
  ロードタイムやトータルロードサイズなど、各モニターチェックの詳細を表示します。
</figcaption>
