...
2. **[one.newrelic.com > Synthetic monitoring > Create a monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**の順に移動し、作成するモニターを選択します。初めて使用する場合は、pingまたはステップモニターを作成して開始することをお勧めします。
...
<CollapserGroup>
  ...
  >
    1. **[one.newrelic.com > Synthetic monitoring > Create monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**の順に移動します。
    2. pingモニターの場合は**可用性**タイルを、シンプルなブラウザモニターの場合は**ページ読み込みパフォーマンス**をクリックします。
    ...
    * **応答検証**のサブ文字列監視を有効にする[検証文字列](#response-validation)。

    * モニターが送信したリクエストに付随する**カスタムヘッダー**。

    * **SSLを検証**して、SSL証明書チェーンを検証します。次の構文を実行することによって複製することができます。
    ...
    * **Bypass HEADリクエスト**オプションは、デフォルトの`HEAD`リクエストをスキップし、代わりに`GET`動詞を使用します。
    * **リダイレクトは失敗**であり、新しいURLへのリダイレクトではなく、リダイレクトを失敗として分類します。
  ...
  >
    ...
    7. **Next: Write your script**を選択して[**スクリプト化ブラウザ**](/docs/synthetics/new-relic-synthetics/scripting-monitors/writing-synthetic-scripts) または[**APIテスト**](/docs/synthetics/new-relic-synthetics/scripting-monitors/writing-api-tests)のスクリプトを作成してから、**Validate**を選択して構文を検証します。

       <Callout variant="tip">
         複雑なスクリプトの場合、検証には最長1 分かかります。
       </Callout>

    8. **Create my monitor**を選択して確認します。
...
<figcaption>
  **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets) > General**：モニターを最新の状態に保ち、New Relicが常に最も適切なデータを受信できるようにします。
</figcaption>

モニターの作成後に、モニターの[タイプ](#setting-type)を変更することはできませんが、他のモニターの設定は編集できます。

1. **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**の順に移動し、編集するモニターを選択します。
...
<figcaption>
  **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets) > General**：Edit monitorページからSyntheticモニターを削除します。
</figcaption>
...
1. **[one.newrelic.com > Synthetic monitoring](https://one.newrelic.com/synthetics-nerdlets)**の順に移動し、編集するモニターを選択します。
2. **General**をクリックしてから、モニターを削除ボタンをクリックします。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="アラート"
  >
    モニターが失敗したときに<InlinePopover type="alerts"/>を受け取るメールアドレスを指定します。または、モニターを既存のアラートポリシーに接続すると、その他の通知オプションを使用できます。詳細については、[Syntheticsモニタリングにおけるアラート生成](/docs/synthetics/new-relic-synthetics/using-monitors/alerting-synthetics)をご覧ください。
  </Collapser>
  ...
  <Collapser
    ...
    title="Apdex T（オプション）"
  >
    モニターの**許容**レスポンスタイムの閾値を指定します。デフォルトの値は7 秒（7000ms）です。詳細については、[SLAレポートのメトリクスを理解する](/docs/synthetics/synthetic-monitoring/pages/synthetic-monitoring-aggregate-monitor-metrics/#understanding)を参照してください。
  </Collapser>
</CollapserGroup>
