...
4. \[**検証]**をクリックして、モニターが新しいランタイムで機能することを確認します。必要に応じて、スクリプトスクリプトを変更します。
5. 保存する。
...
スクリプトブラウザモニターの[スクリプト構文](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100)を変更する新しい言語を導入しています。新しいランタイムは、従来のランタイム構文と下位互換性があります。アップグレードプロセス中にモニターが破損しないようにするために、スクリプトログ出力で非推奨の警告を受け取る場合があります。
