...
* [**最新の**Chromeランタイム](/docs/synthetics/synthetic-monitoring/using-monitors/manage-monitor-runtimes/#browserdependencies)へのアクセスにより、スクリプト化ブラウザモニターでエンドユーザーが使用しているようなブラウザバージョンを使用できるようになります。
...
4. <DNT>**Runtime**</DNT>フィールドのドロップダウンメニューを使用して、現在のランタイムビューを切り替えます。お使いのモニタータイプでサポートされている最新のランタイムを選択します。モニターがAPIモニターであるかブラウザモニターであるかによって、ビューが変わります。現在サポートされているバージョンについては、[ブラウザモニターランタイムの依存関係の確認](/docs/synthetics/synthetic-monitoring/using-monitors/manage-monitor-runtimes/#browserdependencies)を参照してください。
