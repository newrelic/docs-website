---
title: Syntheticsモニター パブリックミニオンIP
tags:
  - Synthetics
  - Synthetic monitoring
  - Administration
metaDescription: 'Ensure New Relic can monitor your services by adding synthetic monitoring''s IPs and DNS names to your allow list, based on a JSON file.'
freshnessValidatedDate: never
translationType: human
---

New Relicは、パブリックミニオンを使用して[Syntheticモニター](/docs/synthetics/new-relic-synthetics/using-monitors/add-edit-monitors)を実行します。これらのミニオンは、世界中の異なるデータセンターにデプロイされ、ご使用のモニターを実行する役割を担います。

このため、ファイアウォール経由でSyntheticsネットワークリクエストのやり取りが可能であることを確認してください。ミニオンIPアドレスはJSON形式で公開されているため、読み込みや解析を簡単に行うことができます。

<Callout variant="important">
  許可リストまたは関連する設定には、[IP範囲リスト](#dynamic-ip-addresses)のすべての範囲が含まれていることを確認してください。リクエストはどちらの環境からも送信できます。
</Callout>

<DNT>**Optional steps**</DNT>：

* これらのファイルに基づいてお客様の許可リストのルールを自動化できます。
* [標準ヘッダー](/docs/synthetics/new-relic-synthetics/administration/identify-synthetics-requests-your-app)を使用するか、[pingまたはシンプルなブラウザモニター](/docs/synthetics/synthetic-monitoring/using-monitors/add-edit-monitors/#simple)、[スクリプト化されたAPIモニター](/docs/synthetics/synthetic-monitoring/scripting-monitors/write-synthetic-api-tests/#request-options)、または[スクリプト化されたブラウザモニター](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100#structure)にカスタムヘッダーを追加することで、Syntheticモニターのリクエストを識別できます。

## IPアドレスは個人データではありません [#not-personal-data]

ミニオンはサーバーにデプロイされ、エージェントは非個人的な認証情報を使用してアクティブ化されます。サーバーで実行しているミニオンに関連するIPアドレスは、データ保護法やプライバシー法における個人データではありません。詳しい情報については、[Syntheticsセキュリティドキュメント](/docs/synthetics/new-relic-synthetics/getting-started/security-new-relic-synthetics)を参照してください。

## IPアドレスのデイリーJSONリスト [#dynamic-ip-addresses]

リリースされた場所の IP アドレスは変更される可能性があります。変更が必要な場合は、変更前に事前に電子メールでお客様に通知するよう努めます。[サポート フォーラム](https://discuss.newrelic.com/) で更新を確認することもできます。リストされている IP 範囲は、New Relic で使用するために予約されており、他のユーザーが使用することはできません。

<Callout variant="important">
  IPアドレスを使用してパブリックロケーションから外形監視トラフィックを許可リストに追加する場合は、新しいIP範囲で許可リストを更新する必要があります。許可リストに追加する新しいIP範囲：

  * 152.38.128.0/19
  * 212.32.0.0/20
  * 64.251.192.0/20

  <DNT>**If no action is taken**</DNT>

  **2025年5月14日**までに許可リストを更新しないと、syntheticチェックがアプリケーションに接続できなくなり、接続が失敗してアラートがトリガーされる可能性があります。
</Callout>

<table style={{ width: "300px" }}>
  <thead>
    <tr>
      <th>
        EUおよび米国のアカウント
      </th>
    </tr>

    <tr>
      <td>
        * [IP範囲リスト](https://s3.amazonaws.com/nr-synthetics-assets/nat-ip-dnsname/production/ip-ranges.json)
      </td>
    </tr>
  </thead>
</table>

<Callout variant="important">
  New Relicが提供するグローバル範囲全体を「許可」リストに追加することをお勧めします。JSONファイルが更新され、新しいIP範囲の地域別の内訳が含まれるようになりました。新しいリストに記載されていない地域では、引き続き古いIPが使用されます。

  **新しいIP範囲：**

  <table>
    <thead>
      <tr>
        <th>
          地域
        </th>

        <th>
          場所
        </th>

        <th>
          IP範囲
        </th>
      </tr>
    </thead>

    <tbody>
      <tr>
        <td>
          米国東部1
        </td>

        <td>
          Washington, DC, USA
        </td>

        <td>
          152.38.128.64/26、152.38.132.0/24、64.251.196.0/24
        </td>
      </tr>

      <tr>
        <td>
          米国東部2
        </td>

        <td>
          Columbus, OH, USA
        </td>

        <td>
          152.38.129.96/27、64.251.197.0/24
        </td>
      </tr>

      <tr>
        <td>
          米国西部1
        </td>

        <td>
          San Francisco, CA, USA
        </td>

        <td>
          152.38.130.0/26、64.251.198.0/24、152.38.130.64/26、152.38.130.128/28
        </td>
      </tr>

      <tr>
        <td>
          米国西部2
        </td>

        <td>
          Portland, OR, USA
        </td>

        <td>
          152.38.131.64/26、64.251.199.0/24
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ西部1
        </td>

        <td>
          Dublin, IE
        </td>

        <td>
          212.32.1.128/25、212.32.1.64/26、212.32.1.32/28
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ西部2
        </td>

        <td>
          London, England, UK
        </td>

        <td>
          212.32.0.64/26、212.32.0.128/25、212.32.0.32/27、212.32.3.0/25
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ中部1
        </td>

        <td>
          Frankfurt, DE
        </td>

        <td>
          212.32.2.128/25、212.32.2.64/26
        </td>
      </tr>

      <tr>
        <td>
          アジアパシフィック南東部2
        </td>

        <td>
          Sydney, AU
        </td>

        <td>
          64.251.192.128/25、64.251.193.0/28、64.251.192.64/26
        </td>
      </tr>
    </tbody>
  </table>

  **2025年5月14日**以降、JSONのIP範囲リストが更新され、廃止予定の古いIP範囲が削除されます。この日付までに、新しいIP範囲を追加し、古いものを削除して、「許可」リストを更新することが重要です。

  **古いIP範囲：**

  <table>
    <thead>
      <tr>
        <th>
          地域
        </th>

        <th>
          場所
        </th>

        <th>
          IP範囲
        </th>
      </tr>
    </thead>

    <tbody>
      <tr>
        <td>
          米国東部1
        </td>

        <td>
          Washington, DC, USA
        </td>

        <td>
          44.202.178.0/24、44.202.180.0/23、44.210.68.0/24、44.210.110.0/25
        </td>
      </tr>

      <tr>
        <td>
          米国東部2
        </td>

        <td>
          Columbus, OH, USA
        </td>

        <td>
          3.145.224.0/24、3.145.225.0/25、3.145.234.0/24
        </td>
      </tr>

      <tr>
        <td>
          米国西部1
        </td>

        <td>
          San Francisco, CA, USA
        </td>

        <td>
          3.101.204.0/23、3.101.212.0/24、3.101.209.192/26
        </td>
      </tr>

      <tr>
        <td>
          米国西部2
        </td>

        <td>
          Portland, OR, USA
        </td>

        <td>
          35.89.46.0/23、35.92.27.0/24
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ西部1
        </td>

        <td>
          Dublin, IE
        </td>

        <td>
          3.251.231.0/24、3.251.230.64/26、3.252.47.0/25
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ西部2
        </td>

        <td>
          London, England, UK
        </td>

        <td>
          13.40.201.0/24、13.40.208.0/25、13.41.206.128/25、13.41.206.64/26
        </td>
      </tr>

      <tr>
        <td>
          ヨーロッパ中部1
        </td>

        <td>
          Frankfurt, DE
        </td>

        <td>
          3.71.170.0/24、3.71.103.96/27、3.75.4.128/25
        </td>
      </tr>

      <tr>
        <td>
          アジアパシフィック南東部2
        </td>

        <td>
          Sydney, AU
        </td>

        <td>
          3.26.252.0/24、3.26.245.128/25、3.27.51.0/25
        </td>
      </tr>
    </tbody>
  </table>
</Callout>

## パブリックミニオンロケーションとロケーションラベル [#location]

以下の表は、Syntheticパブリックミニオンロケーションとそのロケーションラベルの相互参照を示したものです。[SyntheticCheck](/docs/insights/explore-data/attributes/synthetics-default-attributes-insights#syntheticcheck-table)および[SyntheticRequest](/docs/insights/explore-data/attributes/synthetics-default-attributes-insights#syntheticrequest-table)イベントから`location`と`locationLabel`属性を[クエリ](/docs/query-your-data)できます。

NerdGraphを使用していますか？ [NerdGraph合成ドキュメント](/docs/apis/nerdgraph/examples/nerdgraph-synthetics-tutorial/#location-field)に示すように、`AWS_`プレフィックスは必要ありません。

<table>
  <thead>
    <tr>
      <th style={{ width: "300px" }}>
        パブリックミニオンロケーション
      </th>

      <th>
        ロケーションラベル
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        AWS\_AP\_EAST\_1
      </td>

      <td>
        &quot;Hong Kong, HK&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AP\_SOUTH\_1
      </td>

      <td>
        &quot;Mumbai, IN&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AP\_SOUTHEAST\_1
      </td>

      <td>
        &quot;Singapore, SG&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AP\_NORTHEAST\_2
      </td>

      <td>
        &quot;Seoul, KR&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AP\_NORTHEAST\_1
      </td>

      <td>
        &quot;Tokyo, JP&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AP\_SOUTHEAST\_2
      </td>

      <td>
        &quot;Sydney, AU&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_US\_WEST\_1
      </td>

      <td>
        &quot;San Francisco, CA, USA&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_US\_WEST\_2
      </td>

      <td>
        &quot;Portland, OR, USA&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_US\_EAST\_2
      </td>

      <td>
        &quot;Columbus, OH, USA&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_US\_EAST\_1
      </td>

      <td>
        &quot;Washington, DC, USA&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_CA\_CENTRAL\_1
      </td>

      <td>
        &quot;Montreal, Québec, CA&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_SA\_EAST\_1
      </td>

      <td>
        &quot;São Paulo, BR&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_WEST\_1
      </td>

      <td>
        &quot;Dublin, IE&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_WEST\_2
      </td>

      <td>
        &quot;London, England, UK&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_WEST\_3
      </td>

      <td>
        &quot;Paris, FR&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_CENTRAL\_1
      </td>

      <td>
        &quot;Frankfurt, DE&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_NORTH\_1
      </td>

      <td>
        &quot;Stockholm, SE&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_EU\_SOUTH\_1
      </td>

      <td>
        &quot;Milan, IT&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_ME\_SOUTH\_1
      </td>

      <td>
        &quot;Manama, BH&quot;
      </td>
    </tr>

    <tr>
      <td>
        AWS\_AF\_SOUTH\_1
      </td>

      <td>
        &quot;Cape Town, ZA&quot;
      </td>
    </tr>
  </tbody>
</table>