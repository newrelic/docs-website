...
New Relic の `$util.insights` は、合成モニタリングから報告される [イベント](/docs/using-new-relic/data/understand-data/new-relic-data-types#event-data) を設定・操作するためのツール群です。

`SyntheticCheck` [event](/docs/data-apis/understand-data/new-relic-data-types/#event-data) に、接頭辞 `custom` を持つカスタム属性として、カスタムデータを追加することができます。これらの属性は、イベント [のデフォルト属性](/docs/insights/new-relic-insights/decorating-events/synthetics-default-attributes-insights#syntheticcheck-table) に追加されます。
