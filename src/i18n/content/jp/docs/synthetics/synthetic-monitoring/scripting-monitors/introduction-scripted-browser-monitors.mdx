...
スクリプト化されたブラウザは、ウェブサイト内の移動や特定のアクションの実行、特定の要素があることの確認を行うブラウザのスクリプト化により、カスタムのユーザー体験を模倣します。スクリプト化ブラウザを作成するには、<DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create a monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate> に移動し、<DoNotTranslate>**User flow / functionality**</DoNotTranslate> タイルを選択します。

利用可能な全関数の一覧については、 [スクリプト化ブラウザリファレンス](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetic-scripted-browser-reference-monitor-versions-chrome-100/)を参照してください。

スクリプト化モニターは[Seleniumウェブドライバ](https://www.selenium.dev/documentation/webdriver/)で駆動されます。 スクリプトを実行するたびに、New Relicはウェブサイト内を移動し、スクリプトの各ステップを実行する、完全に仮想化されたSelenium駆動のGoogle Chromeブラウザを作成します。外形監視には、スクリプトの作成を単純化する関数、ロケーター、その他の要素を入力補完するIDE形式のスクリプトエディタが搭載されています。
...
要素は、`class`、`id`、`link text`、`name`またはXPathでも検索できます。これらの属性を検索するには、ブラウザの開発者ツールを使用、またはウェブサイトのソースコードを確認してください。ロケーター関数のリストについては、[ロケーター: ページ要素の検索](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions-050/#locators)をご覧ください。
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="HTML IDで特定"
  >
    要素をその<DoNotTranslate>**exact**</DoNotTranslate> HTML `id`（例： `id="edit-submit"`）で検索します。 これはページ要素を特定する簡単な方法ですが、変更される可能性が低い`id`を指定するように注意してください。

    ```
    $browser.findElement($driver.By.id("edit-submit"));
    ```

    要素の`id`で要素を検索する例については、[ページ要素を待機する](/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors/#wait-page-element)を参照してください。
  </Collapser>
  ...
  <Collapser
    ...
    title="リンクテキストで検索"
  >
    要素を表示中のリンクテキストで検索します（例：`<a href="http://example.com>your link text here</a>`）。<DoNotTranslate>**exact**</DoNotTranslate>リンクテキスト（`$driver.By.linkText`）または部分一致（`$driver.By.partialLinkText`）で検索できます。
    ...
    リンクテキストを使用してリンクを特定する事例については、[リンクへのアクセス](#locate-by-link-text)をご覧ください。
  </Collapser>
  ...
  <Collapser
    ...
    title="名前で特定"
  >
    <DoNotTranslate>**exact**</DoNotTranslate> HTML `name`（例：`name="search-query-field"`）で要素を検索します。 これは、検索ボックスなどの入力フィールドで一般的に使用されています。

    ```
    $browser.findElement($driver.By.name("search-query-field"));
    ```

    `name`で要素を検索する例については、[ウェブサイトにログインする](/docs/synthetics/synthetic-monitoring/getting-started/get-started-synthetic-monitoring/)を参照してください。
  </Collapser>
  ...
  >
    ...
    ウェブサイトの構造が変更されると、XPathセレクタが破損する場合があります。適切なXPathセレクターの作成のヒントについては、[XPathセレクターの最適化](https://blog.newrelic.com/engineering/optimize-xpath-selectors/)を参照してください。xpathで要素を検索する例については、[ウェブサイトを検索する](/docs/synthetics/synthetic-monitoring/getting-started/get-started-synthetic-monitoring/)を参照してください。
...
<CollapserGroup>
  ...
  >
    ...
    特定した要素をクリックする事例については、[リンクに移動する](#locate-by-link-text)をご覧ください。
  ...
  >
    ...
    一部のフィールドには、デフォルトのテキストが含まれている場合があります。テキストを送信する前に、これらのフィールドを[クリア](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions-050/#webelement)してください。
    ...
    フィールドにテキストを入力する事例については、[ウェブサイトを検索する](/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors/#locate-by-link-text)をご覧ください。
...
<CollapserGroup>
  ...
  >
    ...
    ページタイトルを待つ事例については、[ページの読み込みを待つ](/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors/#wait-page-title)をご覧ください。
  </Collapser>
  ...
  >
    ...
    特定のページ要素を待つ事例については、[ページ要素を待つ](/docs/synthetics/synthetic-monitoring/scripting-monitors/introduction-scripted-browser-monitors/#wait-page-element)をご覧ください。
