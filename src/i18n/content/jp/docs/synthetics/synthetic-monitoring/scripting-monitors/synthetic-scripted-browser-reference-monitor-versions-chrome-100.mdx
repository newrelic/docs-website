...
[Selenium Webdriver 4.1 API](https://www.selenium.dev/selenium/docs/api/javascript/index.html)を使用してスクリプト化ブラウザ監視を実行するには、スクリプトの構文で変数`$selenium`と`$webDriver`を使用していることを確認してください。[Selenium Webdriver API 3.6.0](http://seleniumhq.github.io/selenium/docs/api/javascript/index.html)にアクセスするには、[0.5.0以降のブラウザのリファレンスドキュメント](/docs/synthetics/synthetic-monitoring/scripting-monitors/synthetics-scripted-browser-reference-monitor-versions-050/)で説明されているように、変数`$driver`と`$browser`を使用します。
