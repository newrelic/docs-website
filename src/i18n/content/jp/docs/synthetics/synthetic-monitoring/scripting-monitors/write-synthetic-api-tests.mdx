...
`$http`オブジェクトが提供する`request`のような体験は、ノード16以降のスクリプト化されたAPIランタイムで、`request`を直接使用しようとする顧客にも返されます。
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > Synthetics > Create monitor**：スクリプトエディタは、[スクリプトコマンド（GitHubで使用可能）](https://github.com/request/request)を単純化するために、関数、セレクタ、その他の要素を提案します。
</figcaption>
