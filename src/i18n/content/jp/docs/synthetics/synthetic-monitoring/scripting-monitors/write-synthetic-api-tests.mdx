...
スクリプト化されたAPIモニターは、APIエンドポイントが正しく機能していることを確認します。スクリプト化されたAPIモニターを作成するには、 <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create a monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate>に移動し、 <DoNotTranslate>**Endpoint availability**</DoNotTranslate>タイルを選択します。
...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate>：スクリプトエディターは、[スクリプトコマンド（GitHubで使用可能）](https://github.com/request/request)を単純化するために、関数、セレクター、その他の要素を提案します。
</figcaption>
