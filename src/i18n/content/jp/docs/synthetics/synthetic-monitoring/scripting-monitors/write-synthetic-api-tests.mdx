...
<Callout variant="tip">
  他のAPIテストの例を表示して共有するには、エクスプローラーハブの[合成スクリプト](https://discuss.newrelic.com/tags/c/full-stack-observability/synthetic/81/script)セクションを参照してください。
</Callout>
...
<CollapserGroup>
  <Collapser
    ...
    title="インサイトPOSTの例"
  >
    この例は、静的な整数を含む[カスタムインサイトイベント](/docs/insights/new-relic-insights/adding-querying-data/inserting-custom-events)をPOSTします。
...
<CollapserGroup>
  <Collapser
    ...
    title="インサイト検証の例"
  >
    この例は、インサイトAPIに投稿してから、レスポンスが`{"success":true}`であることを検証します。
