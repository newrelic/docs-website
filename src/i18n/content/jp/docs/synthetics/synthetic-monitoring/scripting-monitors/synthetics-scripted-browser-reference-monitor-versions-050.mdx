...
<Callout variant="important">
  ...
  * パブリックロケーションの場合は、[ランタイムアップグレードUI](/docs/synthetics/synthetic-monitoring/using-monitors/runtime-upgrade-ui/)を使用して、モニターを最新のランタイムに更新します。
...
変数 `$driver` と `$browser` を使用することにより、[スクリプト化されたブラウザ](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers)は、モニターバージョン0.6.xでSeleniumウェブドライバAPI 3.6.0、またモニターバージョン0.5.xでSeleniumウェブドライバAPI 3.5.0にアクセスできます。
