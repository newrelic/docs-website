---
title: Syntheticsのスクリプト化ブラウザリファレンス （モニターバージョン0.5.0+）
tags:
  - Synthetics
...
このドキュメントは、Syntheticsモニターバージョン0.5.x以降で使用できるスクリプト化ブラウザ関数について説明します。以前のバージョンのモニターを使用している場合は、[モニターバージョン0.4.0以前のドキュメント](/docs/synthetics/new-relic-synthetics/scripting-monitors/synthetics-scripted-browser-reference)をご覧ください。
...
Synthetics[スクリプト化ブラウザ](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers)は、`$driver`および`$browser`変数経由でモニターバージョン0.6.x向け[Selenium Webdriver APIs 3.6.0](http://seleniumhq.github.io/selenium/docs/api/javascript/index.html)、またモニターバージョン0.5.x向け[Selenium Webdriver APIs 3.5.0](http://seleniumhq.github.io/selenium/docs/api/javascript/index.html)へのアクセスを提供します。

特に:

* `$driver`は、`selenium-webdriver`モジュールからのすべてのエクスポートを提供します（たとえば、`ActionSequence`、`Button`、`By`、`WebElement`など）。
* `$browser`は、`selenium-webdriver.WebDriver()`のSyntheticsモニタリングのインスタンスです。`get()`や`findElement()`といった主な基本`WebDriver` APIのほか、一部のSyntheticsカスタムAPIを公開します。

## トップレベル関数: スクリプトを作成する [#structure]
...
<table>
  ...
  <tbody>
    ...
    <tr id="browser-manage">
      ...
      <td>
        このインスタンスのオプションのインターフェイス。[クッキー、タイムアウト、その他のウィンドウオプション](#options)を管理できます。
...
## 拒否リスト：ワイルドカードを使用する [#wildcard-use]
...
以下の関数は、クッキー、タイムアウト、ウィンドウサイズなどのブラウザインスタンスのオプションを管理します。[`$browser.manage()`](#browser-manage)関数を通して、これらのオプションにアクセスします。
...
## ロケーター: ページ要素を検索する [#locators]

ロケーターは、`locator`インスタンスを作成するためのファクトリ関数の集合です。ロケーターは、[`$browser.findElement`](#browser-findElement)などの関数に渡すことができるDOM要素を見つけます。`$driver.By`を使用して、これらを呼び出します。
...
## WebElement: ページ要素を操作する [#webelement]

[`$browser.findElement`](#browser-findElement)または[`$browser.waitForAndFindElement`](#browser-waitForAndFindElement)などの関数がWebElementリファレンスを返す場合、これらの関数はその要素とのインタラクションに使用できます。たとえば、ボタンをクリックし、テキストを送信して入力を書式設定し、テストする要素の属性を取得します。
...
## ActionSequence: 複数アクションのリンク [#actionsequence]
...
<table>
  ...
  <tbody>
    ...
    <tr id="actionSequence-dragAndDrop">
      ...
      <td>
        「ドラッグ＆ドロップ」操作を実行する便利な関数。ターゲット要素は、別の要素の位置に移動するか、オフセット（ピクセル単位）で移動することができます。この位置は、2つのプロパティ`x`と`y`を含むオブジェクトです。`{x: x_offset, y: y_offset}`。
...
## 移動: ブラウザの履歴を移動する [#promises]

`$browser.navigate()`関数は、ブラウザ履歴の前後移動、ページの更新、新しいページへの移動を可能にする多数の関数を公開します。
...
<table>
  ...
  <tbody>
    ...
    <tr id="navigate-to">
      ...
      <td>
        現在のブラウザウィンドウで新規ウィンドウをロードします。`$browser.navigate().to()`は`$browser.get()`と同様です。
...
`$browser.wait`と併用すると、その条件が一致するまで、`until`がスクリプト実行を一時停止させます。詳細については、Seleniumの[WebDriver `until`のドキュメント](http://www.seleniumhq.org/docs/04_webdriver_advanced.jsp)を参照してください。
...
* [スクリプト化ブラウザの記述](/docs/synthetics/new-relic-synthetics/scripting-monitors/write-scripted-browsers) （複数の手順モニタリング用のWebDriverJSスクリプトの構築方法）
* [スクリプト化されたブラウザの例](/docs/synthetics/new-relic-synthetics/scripting-monitors/scripted-browser-examples) （コメントを含む、スクリプト化されたブラウザのサンプルコードと、Web サイトの検索や結果の読み込みの待機などの例）
* [New RelicのExplorers Hubにおけるスクリプトのリポジトリ](https://discuss.newrelic.com/tags/c/support-products-agents/synthetics/synthetics-script)（`synthetics-script`タグの付いたスクリプト、オンライン技術コミュニティによって作成・共有されるスクリプトの例）
