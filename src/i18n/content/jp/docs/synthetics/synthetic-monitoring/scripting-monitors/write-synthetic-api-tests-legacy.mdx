...
<figcaption>
  <DoNotTranslate>**[one.newrelic.com > Synthetic monitoring > Create monitor](https://one.newrelic.com/synthetics-nerdlets/monitor-create)**</DoNotTranslate>: スクリプト エディターは[、スクリプト コマンドを簡素化するために関数、セレクター、その他の要素を提案します (GitHub で入手可能)](https://github.com/request/request) 。
</figcaption>
...
<DoNotTranslate>**For SSL and agentOptions:**</DoNotTranslate> SSL オプションを設定するか、 [`agentOptions`](https://github.com/request/request#using-optionsagentoptions)オブジェクトを提供する場合は、HTTP リクエストがインストゥルメントされたグローバル エージェントを使用するように、リクエスト`options`オブジェクトの`agent`プロパティを`$globalAgents.https`または`$globalAgents.http`に設定する必要があります。
