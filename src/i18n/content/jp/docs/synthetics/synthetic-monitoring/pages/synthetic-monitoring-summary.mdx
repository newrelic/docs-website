...
モニターの**概要**ページにアクセスするには: **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Explorer > Synthetic monitoring > (select a monitor)**に移動します。**[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Synthetic monitoring >(select a monitor)**からもアクセスできます。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        これは、各モニター[の場所](/docs/synthetics/new-relic-synthetics/using-monitors/adding-editing-monitors#setting-location)のロード時間を示しています。[短い時間枠（30分など）を選択する](/docs/apm/new-relic-apm/ui-functions/time-picker-setting-time-periods-view-data)と、合成モニターは、各場所からのモニターの各実行の正確なロード時間を表示します。時間枠が長くなると、合成モニターは平均を表示するように解像度を調整します。
