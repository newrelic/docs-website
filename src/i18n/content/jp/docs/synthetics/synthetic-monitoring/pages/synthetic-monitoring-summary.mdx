...
![スクリーンシンセサイザーのオーバービュー](src/images/summary-page.png 'スクリーンシンセサイザーのオーバービュー')

<figcaption>
  **Summary**
  ページでは、ウェブサイトのパフォーマンスを高レベルで表示し、ロケーションチェック、アラート違反、モニターのメタデータへのショートカットが用意されています。
</figcaption>
...
<Callout variant="important">
  以下の表は、合成モニターのサマリーページの旧表示方法に適用されます。 **Show
  new view**
  をクリックすると、新しいキュレーションサマリー体験にアクセスできます。
</Callout>
