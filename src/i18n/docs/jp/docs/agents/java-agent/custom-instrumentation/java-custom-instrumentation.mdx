...
[New Relicは多くのフレームワークから自動的にメトリックスやイベント](/docs/data-analysis/metrics/analyze-your-metrics/data-collection-metric-timeslice-event-data)を収集します。[サポート対象のフレームワークまたはコンポーネント](/docs/agents/java-agent/getting-started/compatibility-requirements-java-agent)を使用している場合は、ボックスに関するトランザクション情報が表示されることがあり、エージェントがデフォルトでレポートする情報を補足するためにカスタムインストゥルメンテーションを使用できます。
...
カスタムインストゥルメンテーションによるJava管理拡張（JME）もモニターできます。JMXはアプリケーション、デバイス、サービスをモニターし、管理する1つの方法です。[外部**YAML**ファイルによるJMXモニタリングを実装できます。](/docs/java/custom-jmx-instrumentation-by-yml)
