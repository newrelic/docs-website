...
topics:
  ...
  - Ruby agent
  - Features
japaneseVersion: 'https://docs.newrelic.co.jp/docs/agents/ruby-agent/features/developer-mode'
watermark: 非推奨
---

<Callout variant="caution">
  Developerモードは排除されており、[Rubyエージェントのバージョン4.1.0](/docs/release-notes/agent-release-notes/ruby-release-notes/ruby-agent-410333)時点でサポートが終了しています。
</Callout>
...
Developerモードをインストールするには、[Rubyエージェントを](/docs/ruby/ruby-agent-installation)プラグインまたはgemとしてインストールします。また、作業を行っている`RAILS_ENV`の**newrelic.yml**において、[`developer_mode`](/docs/agents/ruby-agent/installation-configuration/ruby-agent-configuration#developer_mode)をtrueに設定する必要があるかもしれません。Developerモードがデフォルトでオンとなるのは、`RAILS_ENV` = `development`の時のみです。

インストレーションの一環として**newrelic.yml**ファイルは作成されるものの、有効な[ライセンスキー](/docs/accounts-partnerships/accounts/account-setup/license-key)を持っている必要はありません。
...
デスクトップでアプリケーションを実行する際に、薄いWEBrickや類似したものを使用している場合は、サーバーでURL `/newrelic`を開いてDeveloperモードのユーザーインターフェースを確認してください。
