...
topics:
  ...
  - Agent data
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/agents/manage-apm-agents/agent-data/custom-instrumentation
---
...
[サポートされているフレームワーク](/docs/agents/manage-apm-agents/installation/compatibility-requirements)を使っているもののNew Relic APM [**Transactions**ページ](/docs/applications-menu/transactions-dashboard)にトランザクションが見当たらない場合、フレームワークインストゥルメンテーションが機能しているか確認するため[support.newrelic.com](https://support.newrelic.com)にサポートを依頼してください。
...
<CollapserGroup>
  ...
  >
    Golangアプリはコンパイルされたネイティブバイナリファイルから実行するため、New Relicのインストゥルメンテーションはすべて手動で実施する必要があります。Goアプリにインストゥルメンテーションをセットアップする方法については、「[Instrument Goトランザクション](/docs/agents/go-agent/get-started/instrument-go-transactions)と[Instrument Goセグメント](/docs/agents/go-agent/get-started/instrument-go-transactions)」をご覧ください。
  </Collapser>
  ...
  >
    ...
    * [設定ファイル](/docs/python/python-instrumentation-by-config-file)：インストゥルメントする関数とメソッドを指定するにはエージェント設定ファイルを使います。エージェント設定ファイルは簡単にセットアップでき、コード変更も必要ありません。ただし、APIコールより柔軟性に劣ります。
    * [API コール](/docs/python/python-instrumentation-by-api)：New Relic Python APIを呼び出すコードを編集します。APIは設定ファイルを介したインストゥルメンテーションよりはるかに柔軟性に優れますが、コードの変更が必要です。
  ...
  >
    New RelicのRubyエージェントは、ターゲットメソッドを定義してそれらにトランザクショントレーサーを追加するためにAPIコールを使います。詳しくは、「[Rubyカスタムインストゥルメンテーション](/docs/ruby/ruby-custom-instrumentation)」をご覧ください。
  </Collapser>
</CollapserGroup>
