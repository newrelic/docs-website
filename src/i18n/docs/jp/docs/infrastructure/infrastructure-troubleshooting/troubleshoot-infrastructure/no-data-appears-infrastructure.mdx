...
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/infrastructure/infrastructure-troubleshooting/troubleshoot-infrastructure/no-data-appears-infrastructure
---

[コンテンツ](/docs) / [インフラストラクチャのモニタリング](/docs/infrastructure) / [トラブルシューティング](/docs/infrastructure/infrastructure-monitoring-troubleshooting) / [Infrastructureのトラブルシューティング](/docs/infrastructure/infrastructure-monitoring-troubleshooting/troubleshoot-infrastructure)

# データが表示されない（Infrastructure） [#page-title]

## 問題 [#problem]

New Relic Infrastructureエージェントをインストールして数分待っても、[Infrastructure UI](/docs/infrastructure/new-relic-infrastructure/infrastructure-ui-pages/infrastructure-compute-page-measure-resource-usage)にデータが表示されません。
...
1. パッケージマネージャを使用して、Infrastructureエージェントがインストールされていることを確認します。
   ...
   <CollapserGroup>
     ...
     >
       1. `dpkg`を使用して、エージェントがインストールされていることを確認します：

          ```
          dpkg -l | grep newrelic-infra
          ```
       2. `dpkg`が出力を返さない場合は、[aptでインストールする](/docs/infrastructure/new-relic-infrastructure/installation/install-infrastructure-linux#apt-based)を参照してください。
     </Collapser>
     ...
     >
       1. `rpm`を使用して、エージェントがインストールされていることを確認します：

          ```
          rpm -qa | grep newrelic-infra
          ```
       2. `rpm`が出力を返さない場合は、[yumでインストールする](/docs/infrastructure/new-relic-infrastructure/installation/install-infrastructure-linux#yum-based)を参照してください。
     </Collapser>
...
2. [お使いのinitシステム](/docs/infrastructure/new-relic-infrastructure/configuration/start-stop-restart-check-infrastructure-agent-status#init-system)で、エージェントが実行中であることを確認します。
   ...
   <CollapserGroup>
     ...
     >
       ...
       1. Powershellから、コマンド`get-service newrelic-infra`を実行します。このコマンドは、サービスのステータスを返します。
       2. エラー`Cannot find any service with service name newrelic-infra`が返される場合、標準の手順に従いエージェントを再インストールします。
     </Collapser>
   </CollapserGroup>
...
7. Insightsで次のような基本的なクエリを作成し、InfrastructureモニタリングUIに表示されない場合でもホストが正しくレポートを送信していることを確認します。
...
* [InfrastructureモニタリングからのNew Relic APMデータの欠落](/docs/infrastructure/new-relic-infrastructure/troubleshooting/apm-data-missing-infrastructure)
