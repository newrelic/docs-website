...
topics:
  ...
  - Manage your agent
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/infrastructure/install-infrastructure-agent/manage-your-agent/start-stop-restart-infrastructure-agent
---
...
## ![Linux](./images/otherlinux.png "Linux")Linux：エージェントの起動、停止、再起動、ステータス確認 [#linux]
...
* **System V** （Debian 7, SLES 11.4, CentOS 5, RHEL 5）：
  ...
  ```
* **Upstart**（Amazon Linux、CentOS 6、RHEL 6、Ubuntu 14.04以前）：

  ```
  sudo initctl <start|stop|restart|status> newrelic-infra
  ```

## ![Windows Server](./images/windows.png "Windows Server") Windows：エージェントの起動、停止、再起動、ステータス確認 [#windows]
...
* Windowsエージェントを起動または停止する：
  ...
  ```
  Stop-Service -Name "newrelic-infra" Start-Service -Name "newrelic-infra"
  ```
