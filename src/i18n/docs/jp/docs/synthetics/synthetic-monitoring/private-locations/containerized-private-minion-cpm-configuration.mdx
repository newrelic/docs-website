...
topics:
  ...
  - Private locations
japaneseVersion: >-
  https://docs.newrelic.co.jp/docs/synthetics/synthetic-monitoring/private-locations/containerized-private-minion-cpm-configuration
---

Read on to learn how to configure your [containerized private minion (CPM)](https://docs.newrelic.co.jp/docs/install-containerized-private-minions-cpms).
...
<Callout variant="caution">
  User-defined environment variables are not sanitized from logs. For sensitive information, consider using the [secure credentials](https://docs.newrelic.co.jp/docs/synthetics/new-relic-synthetics/using-monitors/secure-credentials-store-credentials-information-scripted-browsers) feature.
</Callout>
    ...
    <Table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            The Docker Registry where the Minion Runner image is hosted. Use this to override `quay.io` as the default (for example, `docker.io`).
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            The number of workers the minion will use to run lightweight jobs (`SIMPLE` ping jobs). If undefined, the minion will use `25 * NUM_CPUS` where `NUM_CPUS` is the number of CPUs available to the minion. The maximum allowed value for this variable is `1250`. For more information on monitor types, see [Types of Synthetics monitors](https://docs.newrelic.co.jp/docs/synthetics/new-relic-synthetics/getting-started/types-synthetics-monitors).
          </td>
        </tr>
