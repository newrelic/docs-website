---
title: ディストリビューティッド（分散）トレーシング：計画作成ガイド
contentType: page
template: basicDoc
topics:
...
* クロスアカウントでは、コールがアカウント（同じマスターアカウントを持つアカウントまたは同じ[顧客パートナーシップ](/docs/new-relic-partnerships/partner-integration-guide/partner-account-maintenance/partnership-accounts-users-subscriptions#partnerships)のアカウント）の境界を超えても、完全なトレースを表示できます。
...
<CollapserGroup>
  ...
  >
    ...
    **[one.newrelic.com](https://one.newrelic.com) >（アプリケーションを選択）> Monitor > Transactions >（トランザクショントレースを選択）**：ここには、関連するトランザクションへのリンクと一緒に、ディストリビューティッド（分散）トレーシングを有効にする前のトランザクショントレースのUIが表示されます。ディストリビューティッド（分散）トレーシングを有効にすると、サービスのURLが表示されます。
  ...
  >
    ...
    * APMによって監視されるエンティティに対してディストリビューティッド（分散）トレーシングが有効になっている場合、[サービスマップ](/docs/understand-dependencies/understand-system-dependencies/service-maps/introduction-service-maps)はMobileによって監視されるアプリケーションを表示しません。
    * HTTPリクエストのUIページの[アプリケーションサーバーのドリルダウン](/docs/mobile-monitoring/mobile-monitoring-ui/network-pages/http-requests-page#details)機能は使用できません。
  </Collapser>
</CollapserGroup>
