...
Here's the command to perform a new agent install using the MSI installer. (For changes to an existing installation, use the `ADDLOCAL` command, explained below.) Replace the highlighted sections with relevant values.
