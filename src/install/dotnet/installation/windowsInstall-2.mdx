...
1. Run the install wizard. Tips for using the install wizard:
   * When prompted, input your [New Relic license key](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key).
   * If you'll **only** be monitoring apps hosted with IIS, leave **Instrument all** disabled. Otherwise, enable **Instrument all** and follow additional instructions to [enable the agent for your application](#enabling-the-agent).
   * If you're changing an existing installation using the install wizard, select the **Change** option.
...
2. For each application you wish to monitor, give it a meaningful name. For tips on setting the app name, see [Name your app](/docs/agents/net-agent/installation-and-configuration/naming-your-net-application).
3. If your application is short-lived and exists for less than 60 seconds, it will require [configuration](/docs/agents/net-agent/troubleshooting/monitor-short-lived-net-processes) or it won't report data.
