...
1. To install the required packages, update `<YOUR-VERSION>` with your PostgreSQL version in the following command, and then run it:
    ...
    ```shell
    sudo yum install -y <YOUR_VERSION>-server postgresql<YOUR_VERSION>-contrib
    ```
...
9. Enable the extensions for your database, run:
...
10. Grant the required permissions, run:
