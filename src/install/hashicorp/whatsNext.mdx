...
For the standard on-host installation, this integration comes with a YAML config file, `Hashicorp-config.yml`. This configuration is where you can place required login credentials and configure how data is collected. Which options you change depends on your setup and preferences. It comes with a sample config file `Hashicorp-config.yml.sample` that you can copy and edit.

Specific settings related to Hashicorp are defined using the `env` section of the configuration file. These settings control the connection to your Hashicorp instance as well as other security settings and features. The list of valid settings is described in the next section of this document.

### Hashicorp configuration options [#config-options]

The Hashicorp integration collects both metrics(**M**) and inventory(**I**) information. In the table, use the **Applies To** column for the settings available to each collection:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Hostname or IP where Hashicorp is running.
      </td>
    ...
    <tr>
      ...
      <td>
        Port on which Hashicorp is listening.
      </td>
    ...
    <tr>
      ...
      <td>
        Path to the Hashicorp configuration file.
      </td>
    ...
    <tr>
      ...
      <td>
        Limit on number of Hashicorp Column Families.
      </td>
...
<CollapserGroup>
  ...
  >
    In this configuration we're using the environment variable `Hashicorp_HOST` to populate the `HOSTNAME` setting of the integration:
  ...
  >
    In this configuration we're monitoring multiple Hashicorp servers from the same integration. For the first instance (`HOSTNAME: 1st_Hashicorp_host`) we're collecting metrics and inventory while for the second instance (`HOSTNAME: 2nd_Hashicorp_host`) we will only collect metrics.
...
Hashicorp node metrics are attached to the `HashicorpSample` [event type](/docs/using-new-relic/data/understand-data/new-relic-data-types#events-new-relic). The Hashicorp integration collects these node metrics:
...
### Hashicorp column family metrics and metadata [#column-fam-metrics]

The Hashicorp integration retrieves column family metrics. Hashicorp [column family data](#column-fam-metrics) is attached to the `HashicorpColumnFamilySample` event type. It skips system keyspaces (`system`, `system_auth`, `system_distributed`, `system_schema`, `system_traces` and `OpsCenter`). To limit the performance impact, the integration will only capture metrics for a maximum of 20 column families.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The Hashicorp column family these metrics refer to.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        The Hashicorp keyspace that contains this column family.
      </td>
    </tr>
...
The integration captures configuration options defined in the Hashicorp configuration and reports them as [inventory data in the New Relic UI](/docs/find-use-infrastructure-integration-data).

## System metadata [#metadata]

The Hashicorp integration also collects these [attributes](/docs/accounts-partnerships/education/getting-started-new-relic/glossary#attribute) about the service and its configuration:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        The name of the cluster this Hashicorp node belongs to.
      </td>
    </tr>
  </tbody>
</table>
...
This integration is open source software. That means you can [browse its source code](https://github.com/newrelic/nri-Hashicorp) and send improvements, or create your own fork and build it.
