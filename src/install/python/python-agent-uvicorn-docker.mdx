...
Here are some instructions for doing automatic initialization with `docker run`. The examples in this step include Docker with Uvicorn, but if you were prompted to pick from  frameworks and servers, you can look ahead at those steps to help you with Docker.
