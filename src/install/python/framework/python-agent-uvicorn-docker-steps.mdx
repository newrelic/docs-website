...
optionType: docker
headingText: Set up Uvicorn in Docker
---

Here are some instructions for doing automatic initialization with Uvicorn and `docker run`. If you are using another supported framework with Docker, you'll need to adjust your syntax.
...
Insert this configuration in the same directory as your Python app.
...
1. Create the Dockerfile for your app in the same directory as your Python app.
    ...
    * For the `CMD` line, replace the uvicorn example with the syntax from the previous step.
...
-p 8000:8000 -it --rm --name INSERT_NAME_TO_DISPLAY_IN_DOCKER INSERT_A_SECONDARY_NAME_FOR_YOUR_APP
```

Note the following about this command:

* You need to insert your license key (the key–not the key ID)
