...
3. Grant select privileges to `newrelic@localhost`, with a maximum of 5 connections, by running:
   ```shell
   mysql -e "GRANT SELECT ON *.* TO 'newrelic'@'localhost' WITH MAX_USER_CONNECTIONS 5;"
   ```

4. Grant process privileges to `newrelic@localhost`, with a maximum of 5 connections, by running:
   ```shell
