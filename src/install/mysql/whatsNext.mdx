...
The MySQL Query Performance Monitoring collects the following metrics. The following metrics can be accessed via the `MysqlSample` event:
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Average CPU time in milliseconds spent executing this slow query. The `avg_cpu_time_ms` event attribute stores the average CPU time. For the AWS Aurora integration, the `avg_cpu_time_ms` metric is not exposed. As a result, its value is always `0`.
          </td>
        </tr>
        <tr>
          <td>
