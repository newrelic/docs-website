...
<CollapserGroup>
  ...
  >
    This configuration collects metrics every 30 seconds, inventory every 60 seconds and query performance monitoring every 30 seconds:
  ...
  <Collapser
    ...
    title="Metrics, Inventory and Query Performace Monitoring with TLS connection"
  >
    Use to connect to MySQL with TLS. You can add `TLS_INSECURE_SKIP_VERIFY : true` to disable the server name verification:
  ...
  <Collapser
    ...
    title="Metrics, Inventory and Query Performace Monitoring connecting over socket"
  >
    Use to connect to MySQL using a socket file. This is an alternative to a TCP Hostname/Port connection:
