...
<CollapserGroup>
  ...
  >
    ...
    * Summary: Once these settings are in place, the complete Azure/AWS configuration file should look like the one below. The <InlinePopover type="infrastructure" /> agent and MySQL integration should be installed on a host with network access to the database instances.
