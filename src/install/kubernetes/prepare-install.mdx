---
headingText: Prepare to install
...
<CollapserGroup>
  ...
  When you install the `nri-bundle` and `newrelic-logging` charts to your OpenShift cluster, you may notice that either the `newrelic-logging` pods are crashing or that no pod logs are making it to New Relic for this cluster. This could be caused by security constraints from OpenShift on `hostPath`. To address these, set the log `persistence` to `none` like this:   
