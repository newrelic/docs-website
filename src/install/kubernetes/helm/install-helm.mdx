...
If you want to use Helm to install the integration, you have these options:

1. Our guided install, which will provide a Helm command with the required fields pre-populated.
...
The `nri-bundle` chart is the one that is installed and configured by our [Kubernetes guided install](/install/kubernetes/?dropdown1=guided).

### Installing `nri-bundle` with Helm [#helm-nri-bundle]
