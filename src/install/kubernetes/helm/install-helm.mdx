...
4. Make sure everything is configured properly in the chart by running the following command. Notice that we're specifying `--dry-run` and `--debug`, so nothing will be installed in this step:
    ...
    Our Kubernetes charts have a comprehensive set of flags and tunables that can be edited to better fit your particular needs. Please, check our [Helm chart](https://github.com/newrelic/helm-charts/tree/master/charts/nri-bundle#readme) to see all the options you have.
