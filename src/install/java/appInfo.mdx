...
Guided install is a single CLI command you can run on your system to monitor your Java app. The CLI installs a java-introspector utility attached via our [infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent). This is a good way to trial ingesting data on our platform. For a more permanent solution, we recommend the manually installing the Java APM agent.
