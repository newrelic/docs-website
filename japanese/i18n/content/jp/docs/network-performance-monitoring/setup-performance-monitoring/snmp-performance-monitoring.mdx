...
* シスコ
  ...
  * [メラキ](https://documentation.meraki.com/General_Administration/Monitoring_and_Reporting/SNMP_Overview_and_Configuration)
...
<CollapserGroup>
  ...
  >
    ...
    3. `snmp-base.yaml`ファイルを編集し、 `discovery.cidrs` }属性と`discovery.default_communities`属性をネットワークに適した値に定義します。
       ...
       <Callout variant="tip">
         検出されたすべてのMIBの`global.mibs_enabled`属性への追加を自動化するには、 `discovery.add_mibs: true`を設定することをお勧めします。
       </Callout>
