...
1. [.NETエージェントのダウンロードサイト](http://download.newrelic.com/dot_net_agent/latest_release/ "リンクが新しいウィンドウで開きます。")にアクセスして、アプリケーションのランタイムとアーキテクチャに一致するファイルを取得します。

   * .NET Framework（32ビット）： `newrelic-agent-win-x86-VERSION.zip`
   * .NET Framework（64ビット）： `newrelic-agent-win-x64-VERSION.zip`
   * .NET Core（32ビット）： `newrelic-netcore20-agent-win-x86-VERSION.zip`
   * .NET Core（64ビット）： `newrelic-netcore20-agent-win-x64-VERSION.zip`
