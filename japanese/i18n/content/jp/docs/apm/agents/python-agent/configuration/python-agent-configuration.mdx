...
必要なPythonエージェント構成設定は、[ライセンスキー](/docs/subscriptions/license-key)のみです。ライセンスキーは、エージェントがアプリケーションデータを報告するアカウントを識別します。アプリケーションのホスティング方法に応じて、ライセンスキーは構成ファイルまたは環境変数を介して提供できます。
...
<CollapserGroup>
  ...
  >
    ...
    New Relic APMとCodeStreamを使用している場合は、[リポジトリを関連付ける](/docs/codestream/start-here/codestream-new-relic/#repo-url)[方法と、ビルドSHAまたはリリースタグ](/docs/codestream/start-here/codestream-new-relic/#buildsha)をエラー受信ボックスに関連付ける方法をご覧ください。
...
<CollapserGroup>
  ...
  >
    ...
    NewRelicアカウントの[ライセンスキー](/docs/subscriptions/license-key)を指定します。このキーは、アプリの指標をNewRelicアカウントに関連付けます。
  </Collapser>
  ...
  >
    ...
    高セキュリティモードを有効にするには、ローカルの**.ini**で`true`に設定します構成ファイル**を**作成し、UIのアカウント設定でアクティブ化します。詳細については、「[高セキュリティ](/docs/accounts-partnerships/accounts/security/high-security)」を参照してください。
  </Collapser>
...
<CollapserGroup>
  ...
  >
    ...
    <Callout variant="tip">
      この構成オプションは、PythonAgentバージョン6.4.0以降でのみ使用できます。
    </Callout>
  </Collapser>
...
<Callout variant="important">
  [Pythonエージェントバージョン7.0.0.166以降](/docs/agents/python-agent/installation-configuration/upgrade-python-agent)では、分散トレースがデフォルトで有効になっています。
...
分散トレースを使用すると、リクエストが分散システムを通過するときにたどるパスを確認できます。設定は次のとおりです。
...
* リアルタイムストリーミング（デフォルトで有効）を使用すると[、NewRelic](https://one.newrelic.com)はエンティティのイベントハーベスト制限を5秒間隔で表示します。これは、たとえば、設定ファイルで制限値1200を設定すると、NewRelicでは100として表示されることを意味します。
...
## 新しいRelicCodeStream設定 [#codestream-integration]
...
<CollapserGroup>
  ...
  >
    ...
    これを`false`に設定すると、コードレベルのメトリックのエージェント属性の収集が無効になります。
  </Collapser>
