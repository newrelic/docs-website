...
デフォルトでは、GCPプリミティブロール`Role Viewer`を使用することを**強くお勧め**します。これにより、「[既存のリソースやデータの表示（変更はしない）など、クラウドインフラストラクチャの状態に影響を与えない読み取り専用アクションの権限」が付与されます。](https://cloud.google.com/iam/docs/understanding-roles)このロールは自動的に管理されますGoogleによって作成され、新しいGoogleCloudサービスがリリースまたは変更されたときに更新されます。
