...
* UIのタグ形式： `tag.tagName`
* [属性](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#attribute)としてのレガシータグ形式：`provider.attributeName`。例：`provider.ec2InstanceId`、`provider.ec2State`、`provider.ec2AmiId`。
...
この表は、ユーザーが自分のEC2データを検索して使用できる場所を説明するものです：
...
`provider`の値が`Ec2Instance`の場合、`ComputeSample` [イベントタイプ](/docs/data-apis/understand-data/new-relic-data-types/#event-data)を使用して[データのクエリと調査](/docs/using-new-relic/data/understand-data/query-new-relic-data)を行えます。
