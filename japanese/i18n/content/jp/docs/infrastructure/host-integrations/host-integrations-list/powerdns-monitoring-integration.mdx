...
<CollapserGroup>
  <Collapser
    ...
    title="Linuxのインストール"
  >
    1. ファイル名`nri-powerdns`を使用して[、統合をインストールする](/docs/install-integrations-package)ための指示に従います。
...
このインテグレーションは、オープンソース・ソフトウェアです。つまり、 [そのソースコードを閲覧して](https://github.com/newrelic/newrelic-prometheus-exporters-packages "リンクが新しいウィンドウで開きます。") 改良点を送ったり、自分でフォークを作って構築することができます。
