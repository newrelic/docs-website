...
これらは、 `autodiscover_strategy` オプションが `zookeeper` に設定されている場合のみ関係します。
...
これらは、 `autodiscover_strategy` オプションが`bootstrap` に設定されている場合にのみ関連します。
...
`autodiscover_strategy` オプションが `zookeeper` に設定されている場合のみ該当します。
...
<CollapserGroup>
  ...
  >
    ...
    JMXが正しく設定されているかどうかを確認するには、Kafka統合を実行しているマシンから、各ブローカーに対して次のコマンドを実行します。 `PORT`, `USERNAME`, `PASSWORD` 変数を、ブローカーに対応するJMXの設定に置き換えます。
