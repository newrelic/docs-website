...
* 認証ドメインを表示または編集するには、ユーザーが必要です。
  ...
  * [**認証ドメイン管理者** ロール](/docs/accounts/accounts-billing/new-relic-one-pricing-users/users-roles#standard-roles) を持つグループに所属していること。
...
* SAML SSOは、Pro版またはEnterprise版が必要です。当社のSAML SSOのサポート内容は以下の通りです。
  ...
  * [ADFS（Active Directory Federation Services）について](http://technet.microsoft.com/en-us/library/hh831502.aspx)
...
* [](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#session-mgmt)
* [](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#session-mgmt)[アクセスグラントの仕組みについて](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts)
...
<CollapserGroup>
  ...
  >
    ...
    * ユーザータイプのアップグレード要求をどのように処理するかを管理する機能です。これらのオプションは以下の通りです。
      ...
      * **レビューが必要**：このオプションを使用すると、管理者（ [**認証ドメインマネージャー**の役](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#standard-roles)割を持つユーザー）は、ユーザーがアップグレードを要求したときに電子メールを受信し、 [**ユーザー管理**UI](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-ui-and-tasks#where)でそれらの要求を承認または拒否できます。
  </Collapser>
