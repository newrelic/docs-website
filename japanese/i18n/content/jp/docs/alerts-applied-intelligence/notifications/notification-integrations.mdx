...
<CollapserGroup>
  ...
  >
    ...
    * `caller`列のユーザーを取得するには、 `sys_user`テーブルの読み取り権限が必要です。
      ...
      すぐに使用できる非詳細な役割`personalize_choices` 、 `personalize_dictionary` 、 `rest_service` 、および`itil`には、上記の権限があります。
  ...
  >
    ...
    * **AWSアカウントID** ：AWSアカウントID。これは12桁の数字です。
    ...
    2. **イベントバスのルールを作成します**。New Relicから送信された通知に対応するには、New-Relicイベントをフィルタリングするイベントパターンを使用してルールを作成する必要があります。
