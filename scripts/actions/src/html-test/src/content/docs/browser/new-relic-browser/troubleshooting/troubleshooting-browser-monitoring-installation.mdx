...
If you have just configured your application with Browser monitoring, wait a few minutes for data to appear. If you have already waited a few minutes and you still do not see any data, try these troubleshooting steps.
...
<Callout variant="important">
  Some of these tips depend on whether you [deployed your app](/docs/browser/new-relic-browser/getting-started/adding-apps-new-relic-browser) by allowing Browser monitoring to automatically insert the JavaScript snippet into your APM-monitored app or by inserting the JavaScript snippet yourself.
</Callout>

## Solution

Before specific troubleshooting procedures, it's recommended you first run the [Diagnostics' browser check](/docs/agents/manage-apm-agents/troubleshooting/new-relic-diagnostics#browser-check). The information returned from the Diagnostics tool can be used when talking to New Relic support. For more about how to use the Diagnostics tool, see [Run Diagnostics](/docs/agents/manage-apm-agents/troubleshooting/new-relic-diagnostics#run-diagnostics).

If your Browser agent was deployed using an APM account, see [APM deployments](#apm-tips). If your Browser was deployed using the JavaScript copy/paste method, see [Copy/paste deployment](#manual-tips). If you are unsure which you have, the Diagnostics tool mentioned above will return the deployment method.

### Deployment via APM agent [#apm-tips]

These troubleshooting steps apply to problems with [Browser monitoring that is installed using an existing APM account](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser#select-apm-app):
...
<CollapserGroup>
  ...
  >
    ...
    1. Go to **[one.newrelic.com](https://one.newrelic.com) > Browser > (select an app) > Settings**.
  ...
  >
    If you see JavaScript errors on your webpage, or if you do not see any data on your [**JavaScript errors** page](/docs/browser/new-relic-browser/dashboard-details/javascript-errors-dashboard) in Browser monitoring, check if any other JavaScript error monitoring frameworks have been enabled for your app. Some JavaScript error monitors may interfere with Browser's functionality.
  </Collapser>
...
### Deployment via JS copy/paste [#manual-tips]

These troubleshooting steps apply to problems with the Browser agent [deployed using the HTML copy/paste method](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser#copy-paste-app):
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Check other JavaScript error monitors"
  >
    If you see JavaScript errors on your webpage, or if you do not see any data on your [**JavaScript errors** page](/docs/browser/new-relic-browser/browser-pro-features/js-errors-dashboard-examining-errors-over-time) in Browser, check if any other JavaScript error monitoring frameworks have been enabled for your app. Some JavaScript error monitors may interfere with Browser's functionality.
  </Collapser>
  ...
  >
    Using the [Copy/Paste installation method](/docs/browser/new-relic-browser/installation-configuration/adding-apps-new-relic-browser#copy-paste-app) in a .NET app that uses the Razor Framework, may result in this error:
    ...
    The error is caused by an `@` symbol in the Browser monitoring JavaScript snippet. The `@` symbol represents the beginning of a code block in Razor. The line that causes the problem is:
