...
2. Go to **[one.newrelic.com](https://one.newrelic.com) > (account dropdown) > Account settings**.
3. Download the agent for your platform.
4. Unzip the new agent download file, then copy `newrelic-api.jar` and `newrelic.jar` into the original [Java agent root directory](/docs/agents/manage-apm-agents/troubleshooting/find-agent-root-directory#java-agent).
5. `Diff` your current `newrelic.yml` with the newly downloaded `newrelic.yml` from the zip, and [update customized differences](#diff) as needed.
