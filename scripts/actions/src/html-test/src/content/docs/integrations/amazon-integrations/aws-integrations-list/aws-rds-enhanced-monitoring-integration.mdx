...
1. From **AWS Console > CloudWatch > Logs**, select `RDSOSMetrics` log group, and apply **Actions > Stream to AWS Lambda**.
2. For the **Lambda function**, select `newrelic-log-ingestion`.
3. From **Configure log format and filters**, select `JSON` as the **Log format**.
