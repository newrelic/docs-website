...
<Callout variant="tip">
  Once you start capturing JavaScript errors as events in Insights, set up [NRQL alerting](https://docs.newrelic.com/docs/alerts/new-relic-alerts/defining-conditions/create-alert-conditions-nrql-queries) so you can stay on top of your error data.
</Callout>
...
<CollapserGroup>
  ...
  >
    ...
    Once you have an application node on the map, you can add its connections to the map, including [New Relic Browser](/docs/browser) apps, databases, or external services (for example, third-party APIs). Every node you add is automatically connected to the map, so you see the shape of your application’s architecture emerge.
  ...
  <Collapser
    id="health-map"
    title="Use Entity explorer to analyze health"
  >
    Use the Entity explorer to see a high level overview of your services, which includes applications. Select the service to view a summary of your instrumented applications. You can then select dependencies to view the underlying infrastructure.

    To view this information at a glance, use [Entity explorer](https://one.newrelic.com/launcher/nr1-core.explorer) by going to [one.newrelic.com](https://one.newrelic.com) > Entity explorer.

    From Entity explorer, you can:
    ...
    **[one.newrelic.com](https://one.newrelic.com) > Entity explorer**: The Entity explorer allows you to see an overview of your services.
  </Collapser>
