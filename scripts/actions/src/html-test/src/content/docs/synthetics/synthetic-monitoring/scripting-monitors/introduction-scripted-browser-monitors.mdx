...
Synthetics blocks [popular analytics services'](http://pgl.yoyo.org/adservers/serverlist.php?hostformat=nohtml&showintro=0) scripts from running by default. You can allow scripts to run for a specified service(s). This allows the service's scripts to run and collect data like it would for a real user.
