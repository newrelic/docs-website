...
test('serializing yaml works', async () => {
  const yaml = `agentName: java
agentType: APM
...
whatsNextFilePath: 'src/install/java/whatsNext.mdx'`;
  ...
  const json = await serializeYaml(yaml);
  const deserialized = await deserializeJson(json);
  const json2 = await serializeYaml(deserialized);
  // this might not be the best test, but the deserialized JSON doesn't match
  // the original YAML character for character.
  // this is fine though, as long as the structure and values are the same.
