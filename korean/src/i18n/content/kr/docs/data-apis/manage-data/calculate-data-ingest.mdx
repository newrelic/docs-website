...
[사용량 기반 가격 책정 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 을 사용하는 New Relic 조직의 경우 여러 요소가 비용에 영향을 줄 수 있습니다.이 문서는 New Relic 데이터 수집 비용을 추정하는 데 도움이 됩니다.

## 시작하기 전에 [#before-you-start]

시작하기 전에 [사용량 기반 가격 책정 모델](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 의 작동 방식과 청구에 영향을 미치는 요인에 대해 충분히 이해하고 있어야 합니다.매월 100GB의 수집 데이터가 무료로 제공됩니다(무료로 제공되는 데이터 [에 대해 자세히 알아보기](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing/#free) ).
...
수집된 데이터의 양은 모니터링되는 항목의 종류, 사용되는 기능, 모니터링되는 애플리케이션의 동작 등에 따라 New Relic 조직마다 다를 수 있습니다.데이터 수집에 영향을 미치는 다양한 요소를 이해하여 대략적인 수집 범위의 어느 위치에 속하는지 파악하는 것이 중요합니다.예를 들어, 상황에 맞는 로그인 기능은 로그에 더 많은 데이터를 추가하고 백분율 양은 로그 라인의 크기에 따라 달라집니다(작은 로그 이벤트 = 더 높은 % 증가).이러한 변동성을 감안할 때 비용을 추정하는 가장 좋은 방법은 테스트 New Relic 계정을 설정한 다음 이를 통해 사용량을 추정하는 것입니다.실제 사용량은 [데이터 관리 UI](/docs/data-apis/manage-data/manage-data-coming-new-relic) 에 표시됩니다.
...
위에서 말했듯이 데이터 수집 및 관련 비용은 아키텍처 및 New Relic 설정에 따라 계정마다 크게 다를 수 있으므로 사용량을 추정하기 [위해 New Relic 계정을 만드는](#use-account) 것이 좋습니다.그러나 테스트 New Relic 계정을 만들고 싶지 않다면 대략적인 예상 비용을 자동으로 채우는 비용 추정기 스프레드시트를 사용할 수 있습니다.기본 제공 가정(사용 사례에 맞지 않을 수 있음)과 함께 제공하는 단순화된 입력을 기반으로 방향 안내 및 근사치를 위해 비용 추정기 스프레드시트를 사용할 수 있지만 실제 비용을 보장하지는 않습니다.시작하려면 [이 Google 스프레드시트](https://docs.google.com/spreadsheets/d/1natQ4YhmjDIH8YgkPblIRP2R8h07V4TNAw3EJDu91_g/edit?usp=sharing) 의 사본을 만드세요.
...
<CollapserGroup>
  ...
  >
    ...
    벤더마다 로그 데이터를 다르게 측정하기 때문에 기존 구현에서 New Relic의 로그 볼륨에 대한 기준 추정치를 쉽게 설정할 수 있는 방법이 없습니다.로그 볼륨을 추정하는 가장 좋은 방법은 샘플 양의 로그 데이터를 보내고 추정하는 것입니다.

    로그 이벤트는 디스크의 원래 원시 로그보다 항상 큰 JSON 개체로 저장 및 측정됩니다.
