...
새로운 Relic 충돌 보고서에는 충돌 응용 프로그램의 빌드 UUID도 포함됩니다. 빌드 UUID를 보려면:

1. **[one.newrelic.com](https://one.newrelic.com)** 으로 이동하여 상단 탐색 메뉴에서 **Mobile** 을 클릭합니다. 그런 다음 앱을 찾아 **Crashes > Crash Analysis** 을 클릭합니다.
2. [**충돌 목록** 테이블](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/view-mobile-app-crashes) 에서 아무 행이나 선택합니다.
3. 선택한 충돌 보고서의 [**충돌 세부정보** 페이지](/docs/mobile-monitoring/mobile-monitoring-ui/crashes/investigate-mobile-app-crash-report#viewing) 에서 속성 목록에서 `App Image Uuid` 을(를) 찾습니다.
...
2. 내보낸 충돌 보고서의 [**바이너리 이미지** 섹션](#binary-example) 에서 빌드 UUID를 찾습니다.

충돌 세부 정보에서 추출한 빌드 UUID를 사용하여 누락된 dSYM을 찾습니다.

## 이진 이미지 예 [#binary-example]

다음은 내보낸 충돌 보고서의 **바이너리 이미지** 섹션의 예입니다. 이 섹션에는 애플리케이션에 포함된 모든 동적으로 연결된 라이브러리에 대한 빌드 UUID와 기본 애플리케이션에 대한 빌드 UUID가 나열됩니다.
