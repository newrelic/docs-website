...
* `newrelic-k8s-metrics-adapter.config.externalMetrics.<var>external_metric_name</var>.<var>query</var>`: 다음과 같은 위치에 새 외부 측정항목을 추가합니다.

  * `<var>external_metric_name</var>`: 메트릭 이름입니다.
  * `<var>query</var>`: 메트릭 값을 가져오는 데 사용되는 기본 NRQL 쿼리입니다.
