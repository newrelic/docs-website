...
당사의 MongoDB 통합은 귀하의 MongoDB 클러스터에서 당사 플랫폼으로 인벤토리 및 메트릭을 수집하여 전송합니다. 여기서 핵심 성능 메트릭을 집계하고 시각화할 수 있습니다. 우리는 성능 병목 현상을 정확히 찾아내는 데 도움이 되도록 mongos, mongods 및 config 서버는 물론 데이터베이스 및 컬렉션에 대한 데이터를 수집합니다.

통합을 설치하고 수집하는 데이터를 보려면 계속 읽으십시오.

## 호환성 및 요구 사항 [#comp-req]

우리의 통합은 MongoDB v3 및 v4와 호환됩니다. MongoDB Atlas는 계층 M10 이상에서 지원됩니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. 파일 이름 `nri-mongodb` 을(를) 사용하여 [통합 설치](/docs/install-integrations-package) 지침을 따르십시오.
...
[MongoDB 셸](https://docs.mongodb.com/manual/mongo/) 에서 다음 명령을 실행하여 `listCollections` 역할과 새 사용자를 만든 다음 새 사용자에게 `clusterMonitor` 및 `listCollections` 역할을 할당합니다. 참고: <var>username</var> , <var>password</var> 및 유사한 사용자별 값은 바꿔야 합니다.
...
<Callout variant="important">
  레거시 구성/정의 파일을 계속 사용하는 경우 [표준 구성 형식](/docs/create-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/) 에 대한 설명서를 참조하여 도움을 받으십시오.
</Callout>
...
전체 MongoDB 클러스터 또는 독립 실행형 MongoDB 인스턴스를 모니터링하도록 MongoDB 통합을 구성할 수 있습니다. 클러스터를 모니터링하려면 `host` 및 `port` [인수](#arguments) 가 클러스터의 `mongos` 인스턴스 중 하나를 가리켜야 합니다. 독립 실행형 인스턴스를 모니터링하려면 sue `host` 및 `port` 가 데이터베이스를 실행하는 `mongod` 를 가리키도록 합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="환경 변수 교체"
  >
    이 구성에서는 환경 변수 `MONGODB_HOST` 를 사용하여 통합의 HOST 설정을 채웁니다.
  ...
  <Collapser
    ...
    title="다중 인스턴스 모니터링"
  >
    이 구성에서는 동일한 통합에서 여러 MongoDB 서버를 모니터링하고 있습니다. 첫 번째 인스턴스( `HOST: 1st_mongodb_host` )의 경우 측정항목과 인벤토리를 수집하는 반면 두 번째 인스턴스( `HOST: 2nd_mongodb_host` )의 경우 측정항목만 수집합니다.
...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-mongodb "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.