...
<CollapserGroup>
  ...
  >
    `true` 일 때 `expiration_duration` 시간 내에 신호가 들리지 않으면 현재 열려 있는 모든 위반이 닫힙니다.
  ...
  >
    true인 경우 `expiration_duration` 시간 내에 신호가 없을 때 신호 위반이 손실됩니다.
  ...
  >
    ...
    데이터 포인트는 타임스탬프를 기반으로 함께 수집되고 일괄 처리로 보고됩니다. 사용자 지정 가능한 집계 창은 불규칙하거나 덜 빈번한 데이터 포인트에 대해 경고할 때 더 큰 유연성과 더 적은 잘못된 위반을 제공합니다.
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                APM 애플리케이션 메트릭은 경고를 트리거합니다(기준 임계값 사용).
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                브라우저 메트릭은 경고를 트리거합니다(기준 임계값 사용).
              </td>
            </tr>
  ...
  <Collapser
    ...
    title="위반_시간_한도_초"
  >
    지정된 시간(초) 후에 인스턴스 기반 위반을 자동으로 닫는 데 사용합니다.
  ...
  <Collapser
    ...
    title="위반_닫기_타이머"
  >
    지정된 시간 후에 JVM 상태 메트릭 위반을 포함하여 인스턴스 기반 위반을 자동으로 닫는 데 사용합니다. 다음 값 중 하나여야 합니다.
