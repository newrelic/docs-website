...
<Callout variant="important">
  모니터 버전 0.4.0 이상에 대해 합성 스크립트 모니터에 대한 프록시 서버 구성을 설정할 수 있습니다.
</Callout>
...
<CollapserGroup>
  <Collapser
    id="setproxy-perurl"
    title={<>$network.setProxy(문자열 또는 개체 <var>PROXY URL</var> )</>}
  >
  ...
  <Collapser
    id="setproxy-http"
    title={<>$network.setProxyForHttp(문자열 또는 개체 <var>PROXY URL</var> )</>}
  >
    <Callout variant="tip">
  ...
  <Collapser
    id="setproxy-https"
    title={<>$network.setProxyForHttps(문자열 또는 개체 <var>PROXY URL</var> )</>}
  >
    <Callout variant="tip">
  ...
  <Collapser
    id="setproxy-pac"
    title={<>$network.setProxyPAC(문자열 <var>PAC SCRIPT URL</var> , 개체 <var>AUTH OBJECT</var> )</>}
  >
  ...
  <Collapser
    ...
    title={<>$network.setProxyAdvanced(객체 <var>PROXY 규칙 객체</var> )</>}
  >
    <Callout variant="tip">
      이 방법은 **Chrome** 전용입니다. 스크립트로 작성된 브라우저 모니터 유형에만 적용됩니다. 보다 유연하고 복잡한 프록시 구성을 허용하는 데 사용할 수 있습니다. 대부분의 경우 이 방법은 필요하지 않습니다.
    </Callout>
