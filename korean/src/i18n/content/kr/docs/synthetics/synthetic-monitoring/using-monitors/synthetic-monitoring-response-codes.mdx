...
합성 모니터는 [**리소스** 페이지](/docs/synthetics/new-relic-synthetics/pages/synthetics-resources-understand-load-times) 에서 볼 수 있는 표준 HTTP 응답 코드 이외의 여러 추가 응답 코드를 반환합니다. 이러한 응답 코드는 [Chromium의 응답 코드](https://code.google.com/p/chromium/codesearch#chromium/src/net/base/net_error_list.h&sq=package:chromium) 를 기반으로 하며 모니터 동작에 대한 자세한 정보를 제공합니다.

## 응답 코드

종합 모니터링은 다음과 같은 추가 응답 코드를 반환합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        `net::ERR_ABORTED, "An operation was aborted (due to user action)"` 스크립트가 시간 초과에 도달하고 실행기가 종료될 때 Chrome 관련 응답 코드입니다. 이를 위해 러너는 보류 중인 모든 요청을 종료합니다.
      </td>
    </tr>
