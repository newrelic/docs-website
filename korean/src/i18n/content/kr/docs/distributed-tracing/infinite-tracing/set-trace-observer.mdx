...
import browserMobileLambdaMenu from 'images/browser-mobile-lambda-menu.png'

import selectDataSources from 'images/select-data-sources.png'
...
New Relic APM 에이전트 또는 타사 통합을 사용하는 경우 새 추적 관찰자를 생성하려면:

1. [one.newrelic.com](https://one.newrelic.com) 으로 이동하여 **앱** 을 클릭한 다음 **내 앱** 에서 **New Relic Edge** 를 클릭합니다.
...
6. (선택 사항) 추적 관찰자가 작동하는지 확인하려면 샘플 추적 페이로드를 보내는 것이 좋습니다. [Trace API를](/docs/understand-dependencies/distributed-tracing/trace-api/introduction-trace-api) 직접 사용하는 경우 API 작동 방식을 배우는 데 도움이 됩니다.
   ...
   <CollapserGroup>
     ...
     >
       ...
       1. 데이터를 보고할 계정의 [라이선스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 를 가져와서 준비하세요.
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  이것을 수집 [라이센스 키로](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 바꾸십시오.
                </td>
              </tr>
...
다른 유형의 서비스(예: APM 에이전트)를 설정하는 동안 추적 관찰자를 이미 생성한 경우 추적 관찰자 생성 단계를 건너뛰고 [브라우저, 모바일 및 Lambda에 대한 초기 추적 관찰자 구성](#initial-config) 으로 바로 이동하여 선택할 수 있습니다. 당신의 데이터 소스.
...
4. **끝점** 드롭다운에서 **다른 통합** 의 경우 끝점 값을 복사하여 준비합니다. 이 값은 코드 예제에서 <var>YOUR_TRACE_OBSERVER_URL</var> 로 참조됩니다. 이것은 [추적 관찰자](#sample-payload) 를 테스트하는 데 사용됩니다.
          ...
          <table>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  이것을 [라이센스 키로 바꾸](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 십시오.
                </td>
              </tr>
...
1. 추적 관찰자 앱( **[one.newrelic.com](https://one.newrelic.com) > Apps > Your apps > New Relic Edge** )으로 돌아갑니다.
...
2. 추적 관찰자의 경우 행 끝으로 이동하여 세 개의 점을 클릭한 다음 **모바일, 브라우저, 람다 소스 수정** 을 클릭합니다.
   ...
   <img
     ...
     src={browserMobileLambdaMenu}
   />

3. 다음 사항을 염두에 두고 포함할 데이터 소스를 선택하십시오.

   * 필터를 사용하여 옵션을 좁힐 수 있습니다.
   * 브라우저 앱이 여러 이름을 가진 APM 에이전트에 연결된 경우 하나의 이름만 선택할 수 있습니다.
   * 항목이 회색이면 다른 추적 관찰자가 이미 사용 중입니다.

   <img
     title="Screenshot showing the list of data sources you want to send tracing data"
     alt="Screenshot showing the list of data sources you want to send tracing data"
     src={selectDataSources}
   />
