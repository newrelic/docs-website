...
* 1분 길이의 [수확 주기](/docs/using-new-relic/welcome-new-relic/getting-started/glossary#harvest-cycle) 동안 임계값( [Apdex T 값](/docs/apm/new-relic-apm/apdex/apdex-measuring-user-satisfaction) 의 4배 또는 특정 시간(초))을 위반하는 모든 트랜잭션은 트랜잭션 풀에 추가됩니다.
...
1. 다음 중 하나를 수행합니다.
   ...
   * New Relic Explorer: **[one.newrelic.com](https://one.newrelic.com) > Explorer > (앱 선택) > Monitor > Transactions** 로 이동합니다.
   * APM: **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택) > Monitor > Transactions** 으로 이동합니다.

2. **트랜잭션 추적** 섹션에서 트랜잭션 추적을 클릭하여 [추가 세부정보](/docs/apm/transactions/transaction-traces/transaction-traces-summary-page) 를 봅니다.
