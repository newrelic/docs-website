...
URL, 페이지 제목, 16진수 값, 세션 ID 및 고유하게 식별할 수 있는 값과 같은 고유한 값은 거래의 이름을 지정할 때 사용해서는 안 됩니다. 대신 해당 데이터를 [`AddCustomParameter()`](/docs/agents/net-agent/net-agent-api/add-custom-parameter) 호출을 사용하여 사용자 정의 매개변수로 트랜잭션에 추가하십시오.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        필수의. 다양한 유형의 거래를 구별하는 데 사용할 수 있는 이 거래의 범주입니다. 기본값은 **사용자 지정** 입니다. 처음 255자만 유지됩니다.
      </td>
    </tr>
