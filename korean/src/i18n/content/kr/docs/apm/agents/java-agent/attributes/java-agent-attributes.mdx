...
기본 및 [사용자 지정](/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes) 속성은 모두 APM 트랜잭션 추적, 분산 추적 및 오류 분석에서 볼 수 있습니다.[대시보드](/docs/query-your-data/explore-query-data/dashboards/introduction-new-relic-one-dashboards) 의 [APM 이벤트](/docs/insights/explore-data/attributes/apm-default-attributes-insights) 및 [브라우저 이벤트](/docs/insights/explore-data/attributes/browser-default-attributes-insights#browser-attributes-table) . 이러한 각 대상으로 보낼 속성을 정확히 사용자 지정할 수 있습니다.
...
<CollapserGroup>
  ...
  >
    ...
    * `request.headers.userAgent`
      ...
      사용자 지정이든 표준이든 추가 요청 헤더를 캡처하려면 [Java 에이전트 API](/docs/agents/manage-apm-agents/agent-data/collect-custom-attributes#api-java) 의 `addCustomParameter()` 메서드를 사용하세요.
...
<CollapserGroup>
  <Collapser
    ...
    title="사용자 속성 수집 활성화"
  >
    Java 에이전트 3.10.0 이상으로 사용자 속성을 수집하려면:
  ...
  >
    [`com.newrelic.instrumentation.servlet-user`](/docs/agents/java-agent/configuration/java-agent-configuration-config-file#servlet-user) 을 활성화한 후 몇 분 이내에 새 `user` 속성을 쿼리할 수 있습니다. 예를 들어, 이 [NRQL 쿼리](/docs/query-your-data/nrql-new-relic-query-language/get-started/introduction-nrql-new-relics-query-language) 를 사용하여 모든 사용자의 고유 수를 얻을 수 있습니다.
