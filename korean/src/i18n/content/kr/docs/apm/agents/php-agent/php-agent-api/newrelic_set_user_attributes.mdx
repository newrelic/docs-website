...
릴리스 4.4에서 `newrelic_set_user_attributes("a", "b", "c");` 호출은 `newrelic_add_custom_parameter("user", "a"); newrelic_add_custom_parameter("account", "b"); newrelic_add_custom_parameter("product", "c");` 호출과 동일합니다. 세 가지 매개변수가 모두 필요하지만 빈 문자열일 수 있습니다.
