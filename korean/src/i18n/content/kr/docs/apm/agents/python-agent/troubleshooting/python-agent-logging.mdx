...
모니터링되는 프로세스와 New Relic 수집기 간에 [전송되는 **모든** 데이터에](/docs/agents/manage-apm-agents/configuration/log-audit-all-data-your-new-relic-agent-transmits) 대한 정보를 기록하고 확인해야 하는 경우 짧은 기간 동안 [감사 로깅](/docs/agents/python-agent/installation-configuration/python-agent-configuration#audit-log-file) 을 활성화할 수 있습니다. 이것은 예를 들어 디버깅이나 감사에서 정확히 무엇을 전송하고 있는지에 대한 자세한 정보가 필요할 때 유용합니다.
