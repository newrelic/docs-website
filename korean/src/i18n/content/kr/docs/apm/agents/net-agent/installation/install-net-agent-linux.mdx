...
1. 에이전트를 설치합니다. 이러한 세부 정보는 패키지 관리자를 사용하는 모든 설치에서 동일합니다.
   ...
   * 설치 위치: `/usr/local/newrelic-netcore20-agent` .
   * `newrelic-netcore20-agent-path.sh` 파일은 `/etc/profile.d` 에 있으며 시스템 시작 시 `CORECLR_NEWRELIC_HOME` 환경 변수가 설정됩니다.
...
2. 패키지 관리자의 지침을 따르세요.
   ...
   <CollapserGroup>
     ...
     >
       1. **[download.newrelic.com/dot_net_agent/latest_release](https://download.newrelic.com/dot_net_agent/latest_release/)** 로 이동하여 [아키텍처](/docs/agents/net-agent/getting-started/compatibility-requirements-net-agent#architecture) 및 최신 `newrelic-netcore20-agent` .deb 패키지에 해당하는 URL을 복사합니다.
     ...
     >
       ...
       1. [download.newrelic.com/dot_net_agent/latest_release](https://download.newrelic.com/dot_net_agent/latest_release/) 로 이동하여 [아키텍처](/docs/agents/net-agent/getting-started/compatibility-requirements-net-agent#architecture) 및 최신 `newrelic-netcore20-agent` .rpm 패키지에 해당하는 URL을 복사합니다.
     ...
     <Collapser
       ...
       title="타르볼로 설치"
     >
       1. **[download.newrelic.com/dot_net_agent/latest_release](https://download.newrelic.com/dot_net_agent/latest_release)** 로 이동하여 [아키텍처](/docs/agents/net-agent/getting-started/compatibility-requirements-net-agent#architecture) 및 최신 `newrelic-netcore20-agent-VERSION.tar.gz` 패키지에 해당하는 URL을 복사합니다.
       ...
       4. `newrelic-netcore20-agent` 디렉토리를 `/usr/local` 또는 원하는 설치 위치로 이동합니다.

       5. 환경 변수 `CORECLR_NEWRELIC_HOME` 이 `newrelic-netcore20-agent` 디렉토리를 가리키고 해당 디렉토리가 모니터링되는 .NET 프로세스에서 읽을 수 있는지 확인합니다.

          <Callout variant="important">
            `CORECLR_NEWRELIC_HOME` 이 없으면 새로 만들고 `newrelic-netcore20-agent` 디렉토리를 가리킵니다.
          </Callout>
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="setenv.sh 사용"
  >
    `CORECLR_NEWRELIC_HOME` 을 제외하고 .NET 에이전트에 포함된 `source /usr/local/newrelic-netcore20-agent/setenv.sh` 스크립트는 환경 변수를 자동으로 구성합니다.
