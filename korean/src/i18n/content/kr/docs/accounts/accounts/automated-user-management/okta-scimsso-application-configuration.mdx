...
[자동화된 사용자 관리](/docs/accounts/accounts/automated-user-management/automated-user-provisioning-single-sign) 를 통해 SCIM을 통해 ID 공급자로부터 New Relic 사용자를 가져오고 구성할 수 있습니다. 이 가이드는 New Relic Okta SCIM/SSO 애플리케이션을 구성하는 방법에 대한 Okta 관련 세부 정보를 제공합니다.
...
New Relic 인증 도메인 UI로 이동하려면: [one.newrelic.com](https://one.newrelic.com) 에서 [계정 드롭다운](/docs/using-new-relic/welcome-new-relic/get-started/glossary/#account-dropdown) , **액세스 관리** , **인증 도메인** 을 차례로 클릭합니다.
...
8. **저장** 을 클릭합니다.
9. **로그인 탭으로** 이동합니다. 인증 도메인 필드에 New Relic의 [인증 도메인 UI](/docs/accounts/accounts-billing/new-relic-one-user-management/authentication-domains-saml-sso-scim-more/#ui) 에서 찾을 인증 도메인 ID를 입력합니다.

## 4단계. 사용자 및 그룹 할당 [#assign-users]

Okta에서 사용자 그룹을 아직 설정하지 않았다면 생성해야 합니다. 이들은 나중에 New Relic에서 [액세스 권한](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts#understand-concepts) 을 할당할 그룹이 될 것이며, 이는 해당 그룹에 특정 계정의 특정 역할에 대한 액세스 권한을 부여합니다. 그룹을 만드는 방법을 배우려면 그룹에 대한 [Okta의 설명서를](https://help.okta.com/en/prod/Content/Topics/users-groups-profiles/usgp-about-groups.htm) 참조하십시오.
...
4. 다음으로 이 필드를 갖도록 Okta 사용자 프로필을 구성합니다. 단계:
   ...
   * 해당 프로필에 새로운 **New Relic 사용자 유형 **속성을 추가합니다( [Okta 사용자 프로필 지침 ](https://developer.okta.com/docs/concepts/user-profiles/#universal-directory-schemas)참조). 이를 설정하는 방법은 사용자 유형을 정의하기 위한 설정 및 기본 설정에 따라 다릅니다. 사용자 유형에 대한 예상 값은 `Basic user` , `Core user` 및 `Full user` 입니다. 다음은 정보를 입력한 예입니다.
