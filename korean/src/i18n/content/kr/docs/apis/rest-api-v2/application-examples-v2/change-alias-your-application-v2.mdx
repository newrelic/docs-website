---
title: 애플리케이션의 별칭 변경(v2)
tags:
  - APIs
  - REST API v2
  - Application examples (v2)
metaDescription: 'How to use the New Relic REST API (v2) to set an alias for your application, in lieu of the primary app name set in the agent configuration.'
translationType: machine
---

New Relic의 사용자 인터페이스에 표시되는 응용 프로그램 이름은 구성 설정에 할당된 기본 이름에 대한 사람이 읽을 수 있는 [별칭](/docs/agents/manage-apm-agents/app-naming/rename-your-application#app-alias) 입니다. 변경하면 APM, 브라우저 모니터링 등 플랫폼의 모든 곳에서 앱 이름이 업데이트됩니다.

기본적으로 별칭은 에이전트 구성 파일에 사용된 이름과 동일합니다. 별칭을 변경해도 에이전트의 구성된 이름에는 영향을 미치지 않으며 데이터는 여전히 해당 이름을 사용하여 보고합니다.

## 애플리케이션 별칭 설정 및 Apdex 임계값 수정 [#set_app_alias]

New Relic REST API(v2)에서 앱 이름의 별칭을 변경하려면 이 명령을 사용합니다. [**Applications > Update**](https://rpm.newrelic.com/api/explore/applications/update) 를 선택하여 New Relic의 [API 탐색기](/docs/apis/rest-api-v2/api-explorer-v2/getting-started-new-relics-api-explorer) 에서 앱 별칭을 변경할 수도 있습니다.

* New Relic UI에서와 같이 애플리케이션을 표시하려는 `${APPID}` , `${API_KEY}` 및 별칭 `name` 을 제공해야 합니다.
* 또한 `APP_APDEX_THRESHOLD` , `BROWSER_APDEX_THRESHOLD` 및 모니터링이 활성화된 `BOOLEAN` ( `true` 또는 `false` )가 수정되지 않는 경우에도 제공해야 합니다.

```
curl -X PUT "https://api.newrelic.com/v2/applications/${<var>APPID</var>}.json" \
     -H "X-Api-Key:$<var>{API_KEY}</var>" -i \
     -H 'Content-Type: application/json' \
     -d \
'{
  "application": {
    "name": "<var>name</var>",
    "settings": {
      "app_apdex_threshold": <var>APP_APDEX_THRESHOLD</var>,
      "end_user_apdex_threshold": <var>BROWSER_APDEX_THRESHOLD</var>,
      "enable_real_user_monitoring": <var>BOOLEAN</var>
    }
  }
}'
```

이는 [APM UI](/docs/agents/manage-apm-agents/app-naming/rename-your-application) 를 사용하여 표시된 앱 이름을 변경하는 것과 같습니다. 이렇게 해도 데이터가 수집되는 [앱 식별자 이름](/docs/agents/manage-apm-agents/app-naming/rename-your-application#app-identifier) 은 변경 **되지 않습니다** . New Relic 사용자 인터페이스에 표시되는 앱 이름만 변경됩니다.