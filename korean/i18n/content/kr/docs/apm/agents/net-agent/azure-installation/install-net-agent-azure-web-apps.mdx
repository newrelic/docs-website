...
* [로컬 `newrelic.config` 파일 배치를 통해 활성화합니다](/docs/agents/net-agent/installation/install-net-agent-windows#newrelic-config-local) .
* [애플리케이션 구성 파일을 통해 활성화](/docs/agents/net-agent/installation/install-net-agent-windows#app-config)
...
4. Azure Portal에서 Azure App Service에 New Relic [앱 설정](#web-app-agent-settings) 을 추가합니다.

5. 새 버전의 에이전트를 사용하려면 웹 앱을 다시 시작하세요.

<Callout variant="important">
  사용자 지정 계측 XML을 추가하거나 기본 `newrelic.config` 파일을 수정해야 하는 경우 `%HOME%\NewRelicAgent\Framework` 에서 .NET Framework 에이전트를, `%HOME%\NewRelicAgent\Core` 에서 .NET Core 에이전트를 찾을 수 있습니다.
</Callout>

## NuGet(.NET Framework)을 사용하여 설치 [#nuget-install-net-framework]

NuGet 설치 방법은 Azure Web App 호스트에서 사용할 수 있도록 애플리케이션과 함께 New Relic Agent를 패키징합니다.
...
NuGet을 사용하여 .NET Framework 에이전트를 업그레이드하는 경우 `newrelic.config` 에서 변경한 사항을 덮어씁니다. 변경 사항을 유지하려면 `newrelic.config` 을 프로젝트 외부에 저장한 다음 업그레이드 후 복원하세요.
...
New Relic .NET Core 에이전트는 Azure App Services에서 Linux 및 Windows 애플리케이션을 지원합니다. Azure App Services의 설치 프로세스는 [Linux](/docs/agents/net-agent/installation/install-net-core-agent-linux) 및 [Windows](/docs/agents/net-agent/installation/install-net-agent-windows#installing_core) 용 .NET Core 에이전트 설치 절차와 다릅니다.
...
1. New Relic .NET 에이전트의 앱 설정을 추가하기 전에 [라이센스 키](/docs/apis/intro-apis/new-relic-api-keys/#ingest-license-key) 가 있는지 확인하십시오.
...
**애플리케이션 설정** 으로 라이선스 키와 앱 이름을 설정하면 `newrelic.config` 이(가) 재정의됩니다.
...
New Relic에서 성능을 보려면 애플리케이션이 트래픽을 수신해야 합니다. 데이터가 나타나기 시작하려면 몇 분 정도 기다려야 할 수 있습니다. 데이터가 표시되지 않으면 Azure Web Apps에 대한 [문제 해결 절차](/docs/agents/net-agent/azure-troubleshooting/azure-web-apps-using-always-on-and-no-data-appears) 를 참조하세요.

New Relic에서 앱의 성능을 보려면: **[one.newrelic.com](https://one.newrelic.com) > APM > (앱 선택)으로** 이동합니다. [APM **요약** 페이지](/docs/apm/applications-menu/monitoring/apm-overview-page) 가 자동으로 나타납니다.

또한 [오류](/docs/apm/applications-menu/error-analytics/introduction-error-analytics) , [데이터베이스 및 인스턴스 성능 문제](/docs/apm/applications-menu/features/analyze-database-instance-level-performance-issues) 등에 대한 [자세한](/docs/apm/new-relic-apm/getting-started/introduction-new-relic-apm) 정보를 볼 수 있습니다.

2017년 10월 이전에 New Relic 앱을 만든 경우 Azure Portal을 사용하여 New Relic 계정 블레이드를 선택할 수 있습니다. [SAML 싱글 사인온(SSO)](/docs/accounts-partnerships/accounts/saml-single-sign/new-relic-partners-saml-sso) 을 사용하여 APM에 자동으로 로그인됩니다. **New Relic Accounts > (앱 선택)** 으로 이동하여 Azure Portal에서 애플리케이션의 오류율 및 처리량 데이터를 볼 수도 있습니다.
