...
가비지 컬렉션(GC)은 사용되지 않는 데이터 개체를 찾아 다른 프로세스에서 사용할 수 있도록 해당 메모리 공간을 회수합니다. New Relic Ruby 에이전트는 MRI 1.9.2 이상 또는 Ruby Enterprise Edition에서 실행되는 애플리케이션의 가비지 수집에 소요된 시간에 대한 정보를 수집할 수 있지만 애플리케이션에서 이 기능을 명시적으로 활성화해야 합니다.
...
1. [New Relic에 로그인](https://one.newrelic.com/) 하고 Ruby 앱을 선택한 다음 **요약** 으로 이동합니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (Ruby 앱 선택) > Summary** : 웹 트랜잭션 시간 차트에서 가비지 수집 통계는 **GC Execution** 레이블이 지정됩니다.
</figcaption>

## 자세한 GC 측정항목 보기 [#detailed]

트랜잭션별로 가비지 컬렉션 메트릭을 보려면 **[one.newrelic.com](https://one.newrelic.com) > APM > (Ruby 앱 선택) > Transactions > (트랜잭션 선택)으로** 이동하십시오. 그런 다음 GC에 소요된 시간과 각 트랜잭션에 대해 GC가 호출된 평균 횟수를 볼 수 있습니다. 가비지 수집을 자세히 보려면 다른 레이블을 숨깁니다.
...
<figcaption>
  **[one.newrelic.com](https://one.newrelic.com) > APM > (Ruby 앱 선택) > Transactions > (트랜잭션 선택)** : 개별 트랜잭션을 선택하여 평균 시간 및 평균 호출 횟수를 포함한 자세한 GC 메트릭을 봅니다.
</figcaption>
