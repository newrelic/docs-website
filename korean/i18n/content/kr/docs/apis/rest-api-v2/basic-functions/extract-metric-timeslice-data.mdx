...
요청된 시간 범위의 시작 시간이 8일보다 오래된 경우 [데이터 집계 일정](#data-aggregation) 으로 인해 데이터가 집계되거나 평균 1시간 단위로 집계되었습니다. 즉, 한 시간 동안 단일 데이터 값만 사용할 수 있습니다. 시간 범위에서 1시간 미만의 기간에 데이터를 가져오면 오버샘플링이 발생하여 중복 값이 반환됩니다. 10개의 값만 반환하면 오버샘플링을 방지하고 더 부드러운 차트를 표시하여 오해의 소지가 있는 "고정" 효과를 제거합니다.

## 기간 출력 제어 [#period]

때때로 출력 데이터의 세분성이 너무 미세하거나 반환된 데이터의 기간이 너무 짧을 수 있습니다. 이를 제어하려면 쿼리 명령에 `period=` 매개변수를 각 기간이 보고할 시간( **초** )으로 포함합니다. 사양이 New Relic의 [데이터 집계 일정](#data-aggregation) 을 따르는지 확인하십시오.

**예 #1:** [수집된 데이터의 세분성을 요약하는 New Relic의 테이블](#time-data-granularity) 에 따라 다음 API 호출은 일반적으로 4일( `from=2018-02-13` 및 `to=2018-02-17` ) 동안의 요청이므로 30분 기간의 데이터를 반환합니다. `period=3600` 를 추가하면 데이터가 60분 단위로 반환됩니다.
