...
import LogPartitions from 'images/log-partition041422.png'

import LogPartitionView from 'images/log-partitions.png'
...
1. **[one.newrelic.com](http://one.newrelic.com) > 로그** 로 이동합니다.
...
6. 규칙의 **일치 기준** 설정: 기준과 정확히 일치하는 로그를 대상으로 지정하려면 **EQUALS** 를 선택하고 유사 항목 일치를 적용하려면 **LIKE** 를 선택하십시오.
7. **규칙 사용** 슬라이더를 클릭하고 **만들기** 를 클릭합니다.

<img
  title="Log data partitions rules"
  alt="Screenshot of Log partition rules in UI"
  src={LogPartitionView}
/>
...
모든 로그 계정의 기본 파티션은 `Log` 입니다. 파티션 규칙의 영향을 받지 않는 모든 로그는 기본적으로 `Log` 파티션에 저장됩니다.
