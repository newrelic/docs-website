...
* **APM** 서비스의 보고서 섹션에서
* APM 서비스 또는 브라우저 애플리케이션과 같은 SLI 또는 SLI 관련 엔터티를 포함하는 모든 **워크로드** . 특정 워크로드에서 SLI를 그룹화하려면 기존 워크로드에 APM 서비스 또는 브라우저 앱을 추가하거나 [새 워크로드를 생성해야 합니다](/docs/new-relic-one/use-new-relic-one/workloads/use-workloads/) .
...
일정 빈도로 발생하는 계획 및 검토 회의에서 보고하기 위해 기간별 보기를 사용합니다. 이 보기의 부가 가치는 지정된 달력 기간의 기간 동안 SLO 준수에 대한 더 긴 기록을 표시하는 것입니다.
...
* 기간을 주와 월(그리고 곧도 분기)로 전환할 수 있습니다.
...
* 오류 예산이 25% 아래로 내려가면 노란색으로 바뀝니다. 이는 해당 기간의 전체 예산이 거의 소진되었음을 의미합니다. 새로운 배포 및 변경 사항에 더 주의를 기울이고 일부 안정성 작업을 계획할 수 있습니다.
...
## 나쁜 응답 분석 [#analyze-bad]

SLO가 규정을 준수하지 않는 경우 원본 데이터를 분석하여 무엇이 잘못되었는지에 대해 특별히 중점을 두고 고객에게 미치는 영향을 더 잘 이해하고 싶을 것입니다. 이렇게 하려면 모든 서비스 수준에서 사용할 수 있는 **분석** 옵션을 사용하십시오.
...
**분석** 을 클릭하여 [쿼리 작성기](/docs/query-your-data/explore-query-data/query-builder/introduction-query-builder/) 를 열고 데이터를 드릴다운하여 SLO 누락의 원인과 영향을 더 잘 이해합니다. 원래 NRDB 불량 이벤트, 즉 SLO 준수를 손상시킨 이벤트를 나타내는 쿼리에서 시작합니다. 다음으로 [`FACET`](/docs/query-your-data/nrql-new-relic-query-language/get-started/nrql-syntax-clauses-functions/#sel-facet) 절을 사용하여 특정 속성(예: 계정, 클라이언트 ID, 요청 소스 등)을 분석하고 SLO에 특히 손상이 있는지 감지하는 것이 좋습니다. 우리는 이러한 해로운 가치를 "비방자"라고 부를 것입니다.

예를 들어, [트랜잭션](https://docs.newrelic.com/attribute-dictionary/?event=Transaction) 데이터의 경우 서비스에 대한 트랜잭션이 나머지보다 실패한 결과를 더 많이 반환하는지 알아보기 위해 [`name`](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=name) 을(를) 시도하고 패싯합니다. 어떤 클라이언트가 가장 많은 수의 실패한 결과를 얻고 있는지 알아보려면 [`request.uri`](https://docs.newrelic.com/attribute-dictionary/?event=Transaction&attribute=request.uri) 을(를) 시도하고 패싯합니다.

브라우저 [`PageViewTiming`](https://docs.newrelic.com/attribute-dictionary?event=PageViewTiming) 이벤트의 경우 [`deviceType`](https://docs.newrelic.com/attribute-dictionary?event=PageViewTiming&attribute=deviceType) , [`userAgentName`](https://docs.newrelic.com/attribute-dictionary/?event=PageViewTiming&attribute=userAgentName) , [`userAgentOS`](https://docs.newrelic.com/attribute-dictionary/?event=PageViewTiming&attribute=userAgentOS) 또는 [`countryCode`](https://docs.newrelic.com/attribute-dictionary/?event=PageViewTiming&attribute=countryCode) 로 패싯을 시도할 수 있습니다.
