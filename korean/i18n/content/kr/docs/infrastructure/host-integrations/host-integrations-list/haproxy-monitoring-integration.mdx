...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="Linux"
  >
    1. 파일 이름 `nri-haproxy` 을(를) 사용하여 [통합 설치](/docs/install-integrations-package) 지침을 따르십시오.
...
* 비밀 관리를 사용하여 암호와 같은 민감한 정보가 구성 파일의 일반 텍스트로 노출되지 않도록 보호하십시오. 자세한 내용은 [비밀 관리](/integrations/host-integrations/installation/secrets-management) 를 참조하십시오.
...
이 통합은 오픈 소스 소프트웨어입니다. 즉 [, 소스 코드를 탐색하고](https://github.com/newrelic/nri-haproxy "링크가 새 창에서 열립니다.") 개선 사항을 보내거나 고유한 포크를 만들어 빌드할 수 있습니다.