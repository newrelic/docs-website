...
[데이터 수집을 관리하는](/docs/data-apis/manage-data/manage-data-coming-new-relic) 한 가지 방법은 데이터 삭제 규칙을 설정하는 것입니다. [데이터 삭제를 지원하는 데이터](#requirements) 의 경우 다음을 수행할 수 있습니다.
...
데이터 삭제 규칙을 설정하면 해당 시점 이후의 데이터만 영향을 받습니다. 이미 수집된 기존 데이터는 [편집하거나 삭제할 수 없습니다](/docs/telemetry-data-platform/ingest-manage-data/manage-data/manage-data-retention#data-deletion) . 이 NerdBytes 비디오(7분 09초)에서 자세히 알아보십시오.
...
* APM 메트릭 타임슬라이스 데이터를 삭제하려면 [메트릭 정규화 규칙](/docs/new-relic-one/use-new-relic-one/ui-data/metric-normalization-rules) 을 사용할 수 있습니다.

## 요구 사항 [#requirements]

드롭 필터 규칙을 만들고 편집하는 기능은 사용 중인 사용자 모델에 따라 다릅니다.

* 최신 사용자 모델: "NRQL 삭제 규칙" [기능](/docs/accounts/accounts-billing/new-relic-one-user-management/user-capabilities) 이 있는 역할을 할당받아야 합니다.
* 원래 사용자 모델: [관리자 역할](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#roles) 이 있어야 합니다.
...
* 다음을 제외한 [기본 인프라 모니터링 이벤트](/docs/infrastructure/manage-your-data/data-instrumentation/default-infrastructure-monitoring-data) 및 [인프라 통합](/docs/integrations/infrastructure-integrations/get-started/introduction-infrastructure-integrations) 이벤트:

  * 다운샘플링된 `SystemSample` , `ProcessSample` , `NetworkSample` 및 `StorageSample` (시간 범위가 59분 이상)은 삭제할 수 없습니다. 이 데이터는 데이터 수집에 포함되지 않습니다.

* 다음 주의 사항이 있는 [차원 측정항목](/docs/telemetry-data-platform/ingest-manage-data/understand-data/new-relic-data-types#dimensional-metrics) :

  * 청구 영향: [사용량 기반 가격 책정](/docs/accounts/accounts-billing/new-relic-one-pricing-billing/new-relic-one-pricing-billing) 의 경우 삭제된 데이터는 청구할 수 없습니다. [원래 가격 모델](/docs/accounts/original-accounts-billing/product-pricing/product-based-pricing) 의 경우 삭제된 데이터에 대해 청구할 수 있습니다.
...
<Callout variant="caution">
  데이터 삭제를 결정할 때는 주의하십시오. 삭제한 데이터는 복구할 수 없습니다. 이 기능을 사용하기 전에 [아래의 주의 사항을 검토하십시오.](#caution)
</Callout>
...
[NerdGraph 탐색기](https://api.newrelic.com/graphiql) 에서 구성하고 호출할 수 있습니다.
...
* `DROP_ATTRIBUTES` "원시" 속성( [애그리게이터 기능](/docs/query-data/nrql-new-relic-query-language/getting-started/nrql-syntax-clauses-functions#functions) 이 적용되지 않은 속성)과 함께 `SELECT` 을 사용해야 합니다. 이것은 또한 `SELECT *` 을 사용할 수 없음을 의미합니다. 또한 데이터 유형에 필수적이며 **삭제할 수 없는** 일부 속성이 있습니다(예: 이벤트 데이터의 `timestamp` ). 포함하면 등록에 실패합니다.
...
<CollapserGroup>
  ...
  <Collapser
    ...
    title="나머지 데이터를 유지하면서 민감한 속성 삭제"
  >
...
* NRQL 드롭 규칙에 대한 커뮤니티 토론을 보려면 [Explorers Hub](https://discuss.newrelic.com/c/telemetry-data-platform/dashboards) 를 찾아보세요.
